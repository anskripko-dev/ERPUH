...
Функция ИнформацияДляТехническойПоддержки() Экспорт
	...
	Если ИТК_ОбщийКлиентСервер.ЭтоФайловаяИнформационнаяБаза() Тогда
...
Функция ПредставлениеИндексирования(Индексирование) Экспорт
	...
	Если Индексирование = Метаданные.СвойстваОбъектов.Индексирование.Индексировать Тогда
		Результат = НСтр("ru = 'Индексируется'; en = 'Indexed'");
	ИначеЕсли Индексирование = Метаданные.СвойстваОбъектов.Индексирование.ИндексироватьСДопУпорядочиванием Тогда
		Результат = НСтр("ru = 'Индексируется с доп. упорядочиванием'; en = 'Indexed with add ordering'");
	Иначе
		Результат = Неопределено;
	КонецЕсли;
...
Функция ОбластьМакетаСУчетомИнтерфейса85(Макет, Строка, Столбец) Экспорт
	
	Область = Макет.ПолучитьОбласть(Строка, Столбец);
	Если ИТК_ОбщийКлиентСервер.ЭтоИнтерфейс8_5() Тогда
		Область.Область(1, 1).ТипОтображенияРедактируемойЯчейки = Вычислить("ТипОтображенияРедактируемойЯчейкиТабличногоДокумента.Обычный");
	КонецЕсли;
