...
Функция ПроверкаИдентификатора(Идентификатор, ЗанятыеИдентификаторы = Неопределено, ДополнительныеСимволы = "") Экспорт
	...
	Если НекорректныйИдентификатор(Идентификатор, ДополнительныеСимволы) Тогда
		...
		Шаблон = НСтр("ru = 'Некорректное имя. Имя должно состоять из одного слова, начинаться с буквы или ""%1""
...
Функция ПараметрыПоискаПоПодобию(СтрокаПоиска, ОтборКоллекций = Истина) Экспорт
	
	Коллекция = Неопределено;
	Если ОтборКоллекций Тогда
		...
		Иначе
			СлэшРазделитель = "/";
		КонецЕсли;
		ЧастиКоллекция = СтрРазделить(НРег(СтрокаПоиска), СлэшРазделитель);
		
		Если ЧастиКоллекция.Количество() > 1 Тогда
			
			ЧастиКоллеций = СтрРазделить(ЧастиКоллекция[1], " ");
			Коллекция = СокрЛП(ЧастиКоллеций[0]);
			
		КонецЕсли;
		ЧастиИмени = СтрРазделить(ЧастиКоллекция[0], " ");
		
	Иначе
		ЧастиИмени = СтрРазделить(НРег(СтрокаПоиска), " ");
	КонецЕсли;
	
	Слово = СокрЛП(ЧастиИмени[0]);
	
	ДополнительныеСлова = Новый Массив;
	Для Индекс = 1 По ЧастиИмени.ВГраница() Цикл 
		
		ДополнительноеСлово = СокрЛП(ЧастиИмени[Индекс]);
...
Функция ПоискПоПодобиюНайденыСовпадения(Знач АнализируемаяСтрока, ПараметрыПоиска) Экспорт
...
Функция ТекстНедоступно() Экспорт
	
	Возврат НСтр("ru = '<Недоступно>';
				|en = '<not available>'");
