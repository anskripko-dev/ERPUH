...
Процедура ДобавитьКолонкуВТаблицуЗначений(Приемник, Имя, ТипЗначения, ИмяРодителя) Экспорт
	...
	ДобавитьЭлементКолонкуВТаблицуЗначений(Приемник, ИмяРодителя, Имя, ТипЗначения)
...
Процедура ДобавитьЭлементКолонкуВТаблицуЗначений(Форма, ИмяЭлементаРодителя, Имя, ТипЗначения, ИмяРеквизита = Неопределено, Заголовок = Неопределено) Экспорт
	...
	НоваяКолонка.УстановитьДействие("ОбработкаВыбора", "Подключаемый_ТаблицаЗначенийРеквизитОбработкаВыбора");
	НоваяКолонка.УстановитьДействие("АвтоПодбор", "Подключаемый_ТаблицаЗначенийРеквизитАвтоПодбор");
...
//	ВидСравненияКомпоновкиДанных - ВидСравнения - Вид сравнения отбора
//
Процедура ДобавитьУсловноеОформление(УсловноеОформление, Поля, Оформление, ЛевоеЗначение = Неопределено, ПравоеЗначение = Неопределено,
	...
	Иначе
		...
		Если СтрНайти(Поля, ",") Тогда
			
			Для Каждого Поле Из СтрРазделить(Поля, ",") Цикл 
				
				ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
				ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(СокрЛП(Поле));
	...
	Если ЗначениеЗаполнено(ЛевоеЗначение) Тогда
...
//
Процедура ДобавитьУсловноеОформлениеГруппой(УсловноеОформление, ИмяПоля, Оформление, Условия,
		Тип = Неопределено, ИдентификаторПользовательскойНастройки = Неопределено) Экспорт 
...
Функция ПолноеИмяФормыСписка(Объект) Экспорт
	...
	Иначе
		...
		ОбъектМетаданных = ИТК_Метаданные.МетаданныеСложногоОбъекта(Объект);
...
Функция ЭтоТемнаяТема() Экспорт
	...
	Возврат (НастройкиКлиентскогоПриложения.ТемаКлиентскогоПриложения = Вычислить("ТемаКлиентскогоПриложения.Темная"));
	
КонецФункции

Процедура УстановитьНазначениеДействияКоманды(Команда, НазначениеДействияКоманды) Экспорт
	
	Если НЕ ИТК_ОбщийКлиентСервер.ЭтоИнтерфейс8_5() Тогда
		Возврат;
	КонецЕсли;
	
	Команда.НазначениеДействия = Вычислить("НазначениеДействияКомандыФормы." + НазначениеДействияКоманды);
	
КонецПроцедуры

Процедура ОбработчикПриСозданииНаСервереПосле(Форма) Экспорт
	
	ОбработчикПриСозданииНаСервереОбработкаЭлементов(Форма);
	
	// Подсказки команд
	Для Каждого Команда Из Форма.Команды Цикл
		
		Если ПустаяСтрока(Команда.Подсказка) 
				И ЗначениеЗаполнено(Команда.Заголовок) Тогда
...
Процедура ОбработчикПриСозданииНаСервереОбработкаЭлементов(Элемент)
	
	Для Каждого ПодчиненныйЭлемент Из Элемент.ПодчиненныеЭлементы Цикл
		
		Если ТипЗнч(ПодчиненныйЭлемент) = Тип("ГруппаФормы") Тогда
			ОбработчикПриСозданииНаСервереОбработкаЭлементов(ПодчиненныйЭлемент);
		ИначеЕсли ТипЗнч(ПодчиненныйЭлемент) = Тип("ТаблицаФормы") Тогда
			
			Если ИТК_ОбщийКлиентСервер.ЭтоИнтерфейс8_5() Тогда
				
				ПодчиненныйЭлемент.ЧередованиеЦветовСтрок = Ложь;
				ПодчиненныйЭлемент.ВертикальныеЛинии = Ложь;
				ПодчиненныйЭлемент.ГоризонтальныеЛинии = Истина;
				
			Иначе
				
				ПодчиненныйЭлемент.ЧередованиеЦветовСтрок = Истина;
				ПодчиненныйЭлемент.ВертикальныеЛинии = Истина;
				ПодчиненныйЭлемент.ГоризонтальныеЛинии = Истина;
