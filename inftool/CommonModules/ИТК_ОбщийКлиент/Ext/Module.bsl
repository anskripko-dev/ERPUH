#Область СлужебныйПрограммныйИнтерфейс

Процедура ОбработчикПриОткрытии(Отказ) Экспорт
	
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		
		Отказ = Истина;
		
		Заголовок = ИТК_ТуллкитКлиентСервер.Представление();
		
		Шаблон = НСтр("ru = 'Инструменты %1, не предназначены для запуска в режиме обычного приложения (данный режим не поддерживается)';
						|en = '%1 tools are not designed to run in normal application mode (this mode is not supported)'");
		Текст = СтрШаблон(Шаблон, Заголовок);
		
		ПоказатьПредупреждение( , Текст, , Заголовок);
		
	#Иначе
		
		Если НЕ ИТК_ЗащитаЛицензированиеВызовСервера.ЛицензияПроверенаИКорректна() Тогда
			
			Отказ = Истина;
			
			РезультатПроверки = ИТК_ЗащитаЛицензированиеВызовСервера.ПроверитьЛицензию();
			Если ИТК_ЗащитаЛицензированиеКлиентСервер.ЭтоПустаяЛицензия(РезультатПроверки)
					ИЛИ ИТК_ЗащитаЛицензированиеКлиентСервер.ЭтоКорректнаяНеАктивированнаяЛицензия(РезультатПроверки) Тогда
				
				ЗаголовокВопроса = СтрШаблон(НСтр("ru = 'Лицензирование %1'; en = 'Licensing %1'"), ИТК_ТуллкитКлиентСервер.Представление());
				
				ОписаниеОповещения = Новый ОписаниеОповещения("ЗавершенВопросПерейтиКАктивацииЛицензии", ЭтотОбъект);
				
				КнопкиВопроса = Новый СписокЗначений;
				КнопкиВопроса.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Перейти в лицензирование'; en = 'Go to licensing'"));
				КнопкиВопроса.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru = 'Закрыть'; en = 'Close'"));
				
				ПоказатьВопрос(ОписаниеОповещения, РезультатПроверки.Ошибка, КнопкиВопроса, , КодВозвратаДиалога.Да, ЗаголовокВопроса);
				
			Иначе
				
				ЗаголовокПредупреждения = СтрШаблон(НСтр("ru = 'Лицензирование %1'; en = 'Licensing %1'"), ИТК_ТуллкитКлиентСервер.Представление());
				ПоказатьПредупреждение( , РезультатПроверки.Ошибка, , ЗаголовокПредупреждения);
				
			КонецЕсли;
			
		КонецЕсли;
		
	#КонецЕсли
	
КонецПроцедуры

Процедура ЗавершенВопросПерейтиКАктивацииЛицензии(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		ОткрытьФорму("ОбщаяФорма.ИТК_Лицензирование");
	КонецЕсли;
	
КонецПроцедуры

Функция ОповещениеЗапускПриложения(ДополнительныеПараметры) Экспорт
	
	Возврат (Новый ОписаниеОповещения("ПодключаемыйОбработчикЗапускПриложения", ЭтотОбъект, ДополнительныеПараметры));

КонецФункции

Процедура ПодключаемыйОбработчикЗапускПриложения(Результат, ДополнительныеПараметры) Экспорт
	
	НачатьЗапускПриложения(ПустойОбработчикОповещений(), ДополнительныеПараметры.Команда);

КонецПроцедуры

Функция ПустойОбработчикОповещений() Экспорт 
	
	Возврат (Новый ОписаниеОповещения("ПодключаемыйПустойОбработчикОповещений", ЭтотОбъект));
	
КонецФункции

Процедура ПодключаемыйПустойОбработчикОповещений(Результат, ДополнительныеПараметры) Экспорт 
	
	ПустойОбработчик = Истина;
	
КонецПроцедуры

Функция ПустойОбработчикОповещенийДополнительныеПараметры(ДополнительныеПараметры) Экспорт 
	
	Возврат (Новый ОписаниеОповещения("ПодключаемыйПустойОбработчикОповещенийДополнительныеПараметры", ЭтотОбъект));
	
КонецФункции

Процедура ПодключаемыйПустойОбработчикОповещенийДополнительныеПараметры(ДополнительныеПараметры) Экспорт 
	
	ПустойОбработчик = Истина;
	
КонецПроцедуры

Процедура ОткрытьНастройки(Владелец = Неопределено, Инструмент = Неопределено, Раздел = Неопределено) Экспорт
	
	Параметры = Новый Структура;
	Параметры.Вставить("Инструмент", Инструмент);
	Параметры.Вставить("Раздел", Раздел);
	
	ОткрытьФорму("ОбщаяФорма.ИТК_Настройки", Параметры, Владелец, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

Процедура ОткрытьВсеНастройки(Владелец = Неопределено, Инструмент = Неопределено, Раздел = Неопределено) Экспорт
	
	Параметры = Новый Структура;
	Параметры.Вставить("Инструмент", Инструмент);
	Параметры.Вставить("Раздел", Раздел);
	
	ОткрытьФорму("ОбщаяФорма.ИТК_ВсеНастройки", Параметры, Владелец, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

Процедура ОткрытьИнформациюОНовойВерсии() Экспорт
	
	ОткрытьФорму("ОбщаяФорма.ИТК_ИнформацияОНовойВерсии");
	
КонецПроцедуры

Процедура ОткрытьЗапросИсточника(Владелец, Колонки, Источник) Экспорт
	
	Если Не ЗначениеЗаполнено(Колонки) Тогда
		
		ПоказатьПредупреждение( , НСтр("ru = 'Нет колонок в источнике (таблице значений)';
										|en = 'No columns in source (value table)'"), ,
									ИТК_КонсольРазработчикаКлиентСервер.Представление());
		Возврат;
		
	КонецЕсли;
	
	Параметры = Новый Структура;
	Параметры.Вставить("Колонки", Колонки);
	Параметры.Вставить("Источник", Источник);
	
	ОткрытьФорму("Обработка.ИТК_КонсольРазработчика.Форма.ЗапросИсточника", Параметры, Владелец, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

Функция ПредставлениеТипаКлиента() Экспорт
	
	#Если МобильноеПриложениеКлиент Тогда
	
		Результат = НСтр("ru = 'Мобильное приложение';
						|en = 'Mobile app'");
	
	#ИначеЕсли ВебКлиент Тогда
	
		Результат = НСтр("ru = 'Веб-клиент';
						|en = 'Web client'");
	
	#ИначеЕсли ТонкийКлиент Тогда
	
		Результат = НСтр("ru = 'Тонкий клиент';
						|en = 'Thin client'");
	
	#ИначеЕсли ТолстыйКлиентУправляемоеПриложение Тогда
	
		Результат = НСтр("ru = 'Толстый клиент (управляемое приложение)';
						|en = 'Thick client (managed application)'");
	
	#ИначеЕсли ТолстыйКлиентОбычноеПриложение Тогда
	
		Результат = НСтр("ru = 'Толстый клиент (обычное приложение)';
						|en = 'Thick client (regular application)'");
	
	#КонецЕсли
	
	Возврат Результат;
  	
КонецФункции

Функция ЭтоWindowsКлиент() Экспорт
	
	СистемнаяИнформация = Новый СистемнаяИнформация;
	
	Возврат (СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Windows_x86
	             ИЛИ СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Windows_x86_64);
	
КонецФункции

Функция ЭтоLinuxКлиент() Экспорт
	
	СистемнаяИнформация = Новый СистемнаяИнформация;
	Возврат СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Linux_x86 ИЛИ СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Linux_x86_64;
	
КонецФункции

Функция ИнформацияДляТехническойПоддержки() Экспорт
	
	Результат = Новый Массив;
	
	СистемнаяИнформация = Новый СистемнаяИнформация;
	
	Текст = НСтр("ru = 'Клиент:';
				|en = 'Client:'");
	Результат.Добавить(Текст);

	Результат.Добавить(ПредставлениеТипаКлиента());	
	
	// Версия ОС
	Шаблон = НСтр("ru = 'ОС: %1 (%2)';
					|en = 'OS: %1 (%2)'");
	Текст = СтрШаблон(Шаблон, СистемнаяИнформация.ТипПлатформы, СистемнаяИнформация.ВерсияОС);
	Результат.Добавить(Текст);
	
	// Язык системы
	Шаблон = НСтр("ru = 'Язык: %1';
					|en = 'Language: %1'");
	Текст = СтрШаблон(Шаблон, ТекущийЯзыкСистемы());
	Результат.Добавить(Текст);
	
	// Режим запуска
	Шаблон = НСтр("ru = 'Режим запуска: %1';
					|en = 'Launch mode: %1'");
	Текст = СтрШаблон(Шаблон, ТекущийРежимЗапуска());
	Результат.Добавить(Текст);

	Возврат СтрСоединить(Результат, Символы.ПС);
	
КонецФункции

Функция КоличествоСтрокВДереве(Строка) Экспорт
	
	Результат = 0;
	Для Каждого ПодчиненнаяСтрока Из Строка.ПолучитьЭлементы() Цикл 
		
		Результат = Результат + 1;
		Результат = Результат + КоличествоСтрокВДереве(ПодчиненнаяСтрока);
		
	КонецЦикла;
	
	Возврат Результат;
	
КонецФункции

Функция ЗаполненаКолонкаДерева(Строка, Колонка) Экспорт
	
	Результат = Ложь;
	Для Каждого ПодчиненнаяСтрока Из Строка.ПолучитьЭлементы() Цикл 

		ЗаполненаКолонкаВСтроках = ЗаполненаКолонкаДерева(ПодчиненнаяСтрока, Колонка);
		Если ЗаполненаКолонкаВСтроках
				ИЛИ ЗначениеЗаполнено(ПодчиненнаяСтрока[Колонка]) Тогда
			
			Результат = Истина;
			Прервать;
			
		КонецЕсли;

	КонецЦикла;
	
	Возврат Результат;
	
КонецФункции

#КонецОбласти