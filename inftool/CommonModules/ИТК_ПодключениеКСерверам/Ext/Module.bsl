...
Процедура ЗавершитьСеансы(НомераСеансов) Экспорт
	
	ИмяТекущейБазы = ИТК_ОбщийКлиентСервер.ИмяТекущейИнформационнойБазы();
	ОбщиеНастройки = ИТК_Настройки.Загрузить();
	...
	Для Каждого Подключение Из ПодключенияККластерам Цикл
		...
		Если ОписаниеПодключения.Подключение = ИТК_ПодключениеКСерверамКлиентСервер.ТипПодключенияCOM() Тогда
			ЗавершитьСеансыCOM(Соединение, ОписаниеПодключения, ИмяТекущейБазы, НомераСеансов);
		ИначеЕсли ОписаниеПодключения.Подключение = ИТК_ПодключениеКСерверамКлиентСервер.ТипПодключенияRAS() Тогда
			ЗавершитьСеансыRAS(Соединение, ОписаниеПодключения, ИмяТекущейБазы, НомераСеансов);
		КонецЕсли;
...
Функция ПроверкаПодключения(Параметры)
	...
	Если Параметры.Подключение = ИТК_ПодключениеКСерверамКлиентСервер.ТипПодключенияCOM()
			И ИТК_ОбщийКлиентСервер.ЭтоФайловаяИнформационнаяБаза() Тогда
...
Процедура ЗавершитьСеансыCOM(Соединение, ОписаниеПодключения, ИмяБазы, НомераСеансов)
	
	Для Каждого Кластер Из Соединение.GetClusters() Цикл
		
		Попытка
			Соединение.Authenticate(Кластер, ОписаниеПодключения.Администратор, ОписаниеПодключения.Пароль);
		Исключение
			Продолжить;
		КонецПопытки;
		
		ИнформационныеБазы = Соединение.GetInfoBases(Кластер);
		Для Каждого ИнформационнаяБаза Из ИнформационныеБазы Цикл
			
			Если ИнформационнаяБаза.Name <> ИмяБазы Тогда
				Продолжить;
			КонецЕсли;
			
			Сеансы = Соединение.GetInfoBaseSessions(Кластер, ИнформационнаяБаза);
			Для Каждого Сеанс Из Сеансы Цикл
				
				Если НомераСеансов.Найти(Сеанс.SessionID) <> Неопределено Тогда
					Соединение.TerminateSession(Кластер, Сеанс, СообщениеЗавершенияСеанса());
				КонецЕсли;
...
Процедура ЗавершитьСеансыRAS(Соединение, ОписаниеПодключения, ИмяБазы, НомераСеансов)
	
	Для Каждого Кластер  Из Соединение.ПолучитьКластеры() Цикл
		...
		Для Каждого ИнформационнаяБаза Из ИнформационныеБазы Цикл
			...
			Если ИнформационнаяБаза.Имя <> ИмяБазы Тогда
				Продолжить;
			КонецЕсли;
			...
			Для Каждого Сеанс Из Сеансы Цикл
				...
				Если НомераСеансов.Найти(Сеанс.НомерСеанса) <> Неопределено Тогда
					Сеанс.ЗавершитьСеанс(СообщениеЗавершенияСеанса());
				КонецЕсли;
...
Функция СообщениеЗавершенияСеанса()
	Возврат НСтр("ru = 'Сеанс завершен администратором'; en = 'The session was terminated by the administrator'");
КонецФункции
