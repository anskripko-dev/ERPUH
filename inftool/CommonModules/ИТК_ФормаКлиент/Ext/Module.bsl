...
Процедура ВыборТипаЗавершен(Результат, ДополнительныеПараметры) Экспорт
	...
	ОбработкаЗавершенВводЗначенияВФорме(Элемент, Результат, ДополнительныеПараметры, Форма[Реквизит], Результат);	
...
//	ОписаниеТипов - ОписаниеТипов - Описание типов параметра
//
Процедура АктивизацияЗначенияВФорме(Элемент, Значение, Знач ДоступныеТипы = Неопределено, УстанавливатьОграничениеТипа = Истина) Экспорт
	...
	ИначеЕсли ТипЗнч(ПроверяемоеЗначение) = Тип("Строка") Тогда
		...
		ОграничениеТипа = Истина;
		КнопкаВыбора = ЗначениеЗаполнено(ПроверяемоеЗначение);
		КнопкаВыпадающегоСписка = Ложь;
		КнопкаОчистки = ЗначениеЗаполнено(ПроверяемоеЗначение);
	...
	Иначе
		...
		ОграничениеТипа = Истина;
		РедактированиеТекста = Истина;
		КнопкаВыпадающегоСписка = НЕ ВыбиратьТип ИЛИ ЗначениеЗаполнено(СписокВыбора);
	...
	КонецЕсли;
	Элемент.КнопкаОткрытия = КнопкаОткрытия;
	...
	Если УстанавливатьОграничениеТипа
			И ОграничениеТипа Тогда
...
Процедура ОткрытьКартуМаршрутаБизнесПроцесса(ПолноеИмя, ФормаВладелец, ОписаниеОповещения = Неопределено) Экспорт
	
	Параметры = Новый Структура;
	Параметры.Вставить("ПолноеИмя", ПолноеИмя);
	
	ОткрытьФорму("ОбщаяФорма.ИТК_КартаМаршрутаБизнесПроцесса", Параметры,
		ФормаВладелец, , , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Функция ОбработкаЗавершенВводЗначенияВФорме(Элемент, РезультатВвода, ДополнительныеПараметры, ЗначениеПриемник, ОписаниеТипов, Модифицированность = Неопределено) Экспорт
	...
	Иначе
		...
		АктивизацияЗначенияВФорме(Элемент, НовоеЗначение);
...
Процедура ОбработкаТаблицаЗначенийПриАктивизацииЯчейки(Форма, Элемент) Экспорт
	...
	Если ЭлементКолонка.Вид = ВидПоляФормы.ПолеВвода Тогда
		
		ЗначениеЯчейки = ИТК_ФормаКлиент.ЗначениеКолонкиТаблицыЗначений(Элемент, ЭлементКолонка);
		ИТК_ФормаКлиент.АктивизацияЗначенияВФорме(ЭлементКолонка, ЗначениеЯчейки, ЭлементКолонка.ДоступныеТипы);
...
Процедура ОбработкаТаблицаЗначенийРеквизитНачалоВыбора(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	ЭлементТаблица = Форма.ТекущийЭлемент;
	ТекущееЗначение = ЗначениеКолонкиТаблицыЗначений(ЭлементТаблица, Элемент);
	ДоступныеТипы = Элемент.ДоступныеТипы;
	
	ПараметрыОповещенияВводЗначенияЗавершен = Новый Структура;
	ПараметрыОповещенияВводЗначенияЗавершен.Вставить("Форма", Форма);
	ПараметрыОповещенияВводЗначенияЗавершен.Вставить("ЭлементТаблица", ЭлементТаблица);
	...
	Если Элемент.ВыбиратьТип Тогда
		...
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("Форма", Форма);
		ДополнительныеПараметры.Вставить("ЭлементТаблица", ЭлементТаблица);
		ДополнительныеПараметры.Вставить("Элемент", Элемент);
		ОписаниеОповещенияЗавершение = Новый ОписаниеОповещения("ВыборТипаЗначенияКолонкиЗавершен", ЭтотОбъект, ДополнительныеПараметры);
		
		ДополнительныеПараметры = Новый Структура;
		ОткрытьРедактированиеТипаЗначения(Элемент, НСтр("ru = 'Колонки'; en = 'Column'"),
			ИТК_Перечисления.РежимРедактированияТипаЗначенияВыборТипа(), ДоступныеТипы, ОписаниеОповещенияЗавершение,
	...
	ИначеЕсли ТипЗнч(ТекущееЗначение) = Тип("Тип") Тогда
		...
		ОткрытьРедактированиеТипаЗначения(Элемент, "", ИТК_Перечисления.РежимРедактированияТипаЗначенияРедактированиеТипа(),
	...
	ИначеЕсли ТипЗнч(ТекущееЗначение) = Тип("МоментВремени") Тогда
		...
		ОписаниеОповещенияЗавершение = Новый ОписаниеОповещения("ВводЗначенияВКолонкеЗавершен", ЭтотОбъект, ПараметрыОповещенияВводЗначенияЗавершен);
		ОткрытьРедактированиеМоментаВремени(Элемент, ТекущееЗначение, СтандартнаяОбработка, ОписаниеОповещенияЗавершение);
...
Функция ЗначениеКолонкиТаблицыЗначений(ЭлементТаблица, ЭлементКолонка) Экспорт
...
Процедура УстановитьЗначениеКолонкиТаблицыЗначений(ЭлементТаблица, ЭлементКолонка, Значение) Экспорт
...
Процедура ВыборТипаЗначенияКолонкиЗавершен(Результат, ДополнительныеПараметры) Экспорт
	...
	Форма = ДополнительныеПараметры.Форма;
	ЭлементКолонка = ДополнительныеПараметры.Элемент;
	НовоеЗначение = Результат.ПривестиЗначение(Неопределено);
	УстановитьЗначениеКолонкиТаблицыЗначений(ДополнительныеПараметры.ЭлементТаблица, ЭлементКолонка, НовоеЗначение);
	
	Если СтрНайти(Форма.ИмяФормы, ".ИТК_РедактированиеТаблицыЗначений") Тогда
		ОбработкаЗавершенВводЗначенияВФорме(ЭлементКолонка, Результат, ДополнительныеПараметры,
			НовоеЗначение, ЭлементКолонка.ДоступныеТипы, Форма.Модифицированность);
	Иначе
		ОбработкаЗавершенВводЗначенияВФорме(ЭлементКолонка, Результат, ДополнительныеПараметры,
			НовоеЗначение, ЭлементКолонка.ДоступныеТипы);
	КонецЕсли;
...
Процедура ВводЗначенияВКолонкеЗавершен(Результат, ДополнительныеПараметры) Экспорт
	...
	УстановитьЗначениеКолонкиТаблицыЗначений(ДополнительныеПараметры.ЭлементТаблица, ДополнительныеПараметры.Элемент, Результат);
...
Процедура ДеревоУстановитьПодчиненнымПометку(Строка, Пометка, ИмяПометки = "Пометка") Экспорт
	...
	Для Каждого ПодчиненнаяСтрока Из Строка.ПолучитьЭлементы() Цикл 
		...
		ДеревоУстановитьПодчиненнымПометку(ПодчиненнаяСтрока, Пометка, ИмяПометки);
...
Процедура ОткрытьСравнениеТекстов(Форма, Текст1, Текст2, ТипЯзыка) Экспорт
	...
	Если Редактор = ИТК_Перечисления.РедакторОбычный() Тогда
		...
		#Если ТолстыйКлиентУправляемоеПриложение Тогда
			...
			ПоказатьСравнениеФайловМодально(ИмяПервогоФайла, ИмяВторогоФайла, СпособСравненияФайлов.ТекстовыйДокумент);
...
Процедура ОткрытьВыборРолей(Владелец, ВыбранныеРоли, ОписаниеОповещенияЗавершение, СтандартнаяОбработка = Неопределено) Экспорт
	...
	ОткрытьСписокЗначений(Владелец, ИТК_Перечисления.ВариантФормыСписокЗначенийВыборНесколькихЗначениеСПредставлением(),
...
Процедура ФильтрыФлажкамиСостояниеПометки(Элемент, Пометка) Экспорт
	
	Элемент.Отображение = ОтображениеКнопки.КартинкаИТекст;
	
	Если Пометка Тогда
		Картинка = БиблиотекаКартинок.ИТК_ФлажокИстина;
	Иначе
		Картинка = БиблиотекаКартинок.ИТК_ФлажокЛожь;
	КонецЕсли;
	Элемент.Картинка = Картинка;
	
КонецПроцедуры

Функция ФильтрыФлажкамиСостояниеОсновнойПометки(Пометки) Экспорт
	
	ВсеСняты = Истина;
	ВсеУстановлены = Истина;
	Для Каждого Пометка Из Пометки Цикл 
...
Процедура ФильтрыФлажкамиОпределитьСостояниеОсновнойПометки(Элемент, Пометка) Экспорт
	
	Элемент.Отображение = ОтображениеКнопки.КартинкаИТекст;
	
	Если Пометка = Истина Тогда
		Картинка = БиблиотекаКартинок.ИТК_ФлажокИстина;
	ИначеЕсли Пометка = Ложь Тогда
		Картинка = БиблиотекаКартинок.ИТК_ФлажокЛожь;
	Иначе
		Картинка = БиблиотекаКартинок.ИТК_ФлажокНеопределено;
	КонецЕсли;
...
Процедура СостояниеКнопкиПереключателя(Элемент, Пометка) Экспорт
	
	Элемент.Отображение = ОтображениеКнопки.КартинкаИТекст;
	Элемент.Пометка = Пометка;
	
	Если Пометка Тогда
		Картинка = БиблиотекаКартинок.ИТК_ПереключательИстина;
	Иначе
		Картинка = БиблиотекаКартинок.ИТК_ПереключательЛожь;
	КонецЕсли;
	Элемент.Картинка = Картинка;
	
	Если Пометка Тогда
		ЦветТекста = Новый Цвет(0, 150, 70); // Цвет акцентирования
	Иначе
...
Функция СерверныйВызовВКонтекстеФормы(Форма, ИмяМетода, ДополнительныеПараметры = Неопределено) Экспорт
	
	Попытка
		Результат = Форма.ИТК_СерверныйВызовВКонтекстеФормы(ИмяМетода, ДополнительныеПараметры);
	Исключение
...
Процедура ОткрытьУсловноеОформление(Владелец, УсловноеОформление, СтандартнаяОбработка = Неопределено, ОписаниеОповещенияЗавершение = Неопределено) Экспорт
	
	СтандартнаяОбработка = Ложь;
	
	ПараметрыФорма = Новый Структура;
	ПараметрыФорма.Вставить("УсловноеОформление", УсловноеОформление);
	
	ОткрытьФорму("ОбщаяФорма.ИТК_УсловноеОформление", ПараметрыФорма, Владелец, , , , ОписаниеОповещенияЗавершение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

Процедура ОткрытьРедактированиеПользовательскогоАлгоритма(ПараметрыФормы, ОписаниеОповещенияЗавершение = Неопределено, Владелец = Неопределено) Экспорт
	ОткрытьФорму("Обработка.ИТК_ПользовательскиеАлгоритмы.Форма.РедактированиеПользовательскогоАлгоритма", ПараметрыФормы, Владелец, , , , ОписаниеОповещенияЗавершение);
КонецПроцедуры

&НаКлиенте
Функция ВыделенныеСтрокиКоллекции(Элемент) Экспорт
	
	Если ЗначениеЗаполнено(Элемент.ВыделенныеСтроки) Тогда
		Возврат Элемент.ВыделенныеСтроки;
	КонецЕсли;
	
	Результат = Новый Массив;
	
	Если Элемент.ТекущаяСтрока <> Неопределено Тогда
		Результат.Добавить(Элемент.ТекущаяСтрока);
	КонецЕсли;
...
Функция НайтиСтрокуДерева(Родитель, ИмяКолонки, ИскомоеЗначение) Экспорт
