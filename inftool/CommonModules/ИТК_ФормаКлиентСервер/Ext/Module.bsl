...
Процедура ОтображениеСостоянияТабличногоДокумента(Элемент, Текст = Неопределено, Картинка = Неопределено) Экспорт
	...
	ОтображениеСостояния.Видимость = Элемент.Видимость;
...
Процедура ОформитьСтрокуПоискаПоПодобию(Элемент, ДополнительныйТекст = Неопределено) Экспорт
	...
	Если ИТК_ОбщийКлиентСервер.ЭтоИнтерфейс8_5() Тогда
		Элемент.Картинка = БиблиотекаКартинок.ИТК_ПоискДанных;
	КонецЕсли;
...
Процедура УстановитьКнопкуПоУмолчанию(Элемент, Установить = Истина) Экспорт
	
	Если ИТК_ОбщийКлиентСервер.ЭтоИнтерфейс8_5() Тогда
		
		Если Установить Тогда
			Элемент.Важность = Вычислить("ВажностьКнопки.Главная");
		Иначе
			Элемент.Важность = Вычислить("ВажностьКнопки.Обычная");
		КонецЕсли;
		
	КонецЕсли;
	Элемент.КнопкаПоУмолчанию = Установить;
	
КонецПроцедуры

Процедура УстановитьАвтоШиринуВТаблице(Элемент, Значение) Экспорт
	
	Если ИТК_ОбщийКлиентСервер.ЭтоИнтерфейс8_5() Тогда
		Элемент.АвтоШиринаВТаблице = Вычислить("АвтоШиринаВТаблице." + Значение);
	КонецЕсли;
