...
Процедура Обработать(Параметры, АдресРезультата = Неопределено) Экспорт
	...
	ПараметрыВыполнения.Вставить("КодВыполненияВКонтексте", ИТК_КодКлиентСервер.КодВыполненияВКонтексте(Контекст));
	...
	Для ИндексСтроки = 0 По КоличествоСтрок - 1 Цикл
		...
		Ошибка = ОбработатьСтроку(ОбрабатываемаяСтрока, ОписаниеАлгоритма, Контекст, ПараметрыВыполнения);
		ИТК_ДлительныеОперацииКлиентСервер.СообщениеКонецОбработкиОбъекта(Ошибка);
...
Функция ВыполнитьКодОбработки(Код, ЗначенияПараметров, ДополнительныеСвойства, ОбрабатываемыеДанные = Неопределено) Экспорт
	...
	Результат = ИТК_КодКлиентСервер.ВыполнитьСКонтекстом(Код, Контекст, ПараметрыВыполнения);
	Возврат Результат;
...
//
Функция ОбработатьСтроку(ОбрабатываемаяСтрока, ОписаниеАлгоритма, Контекст, ПараметрыВыполнения)
	...
	Если ОбработкаВТранзакции Тогда
		...
		НачатьТранзакцию();
		Результат = ИТК_КодКлиентСервер.ВыполнитьСКонтекстом(ОписаниеАлгоритма.Код, Контекст, ПараметрыВыполнения);
