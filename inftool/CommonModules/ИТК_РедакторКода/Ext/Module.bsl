...
Функция Инициализация(Форма, Имя, ДополнительныеСвойства = Неопределено) Экспорт
	...
	Если ИТК_ОбщийКлиентСервер.ЭтоИнтерфейс8_5() Тогда
		ВысотаПоУмолчанию = 3;
	Иначе
		ВысотаПоУмолчанию = 10;
	КонецЕсли;
	Высота = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеСвойства, "Высота", ВысотаПоУмолчанию);
	...
	Если ИТК_ОбщийКлиентСервер.ЭтоИнтерфейс8_5()
			И ИспользуетсяMonaco Тогда
		ЭлементКод.Рамка = Новый Рамка(ТипРамкиЭлементаУправления.Одинарная);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Свойства) Тогда
		...
		Для Каждого Свойство Из Свойства Цикл 
			...
			Если ТипЗнч(ЗначениеСвойства) = Тип("Структура") Тогда
				...
				Если ЗначениеСвойства.Свойство("Свойства") Тогда
					СвойстваПоля = ЗначениеСвойства.Свойства;
				КонецЕсли;
			...
			ОписаниеСвойства = ОписаниеПользовательскогоОбъекта(Кэш, ОписаниеТиповЗначения, СвойстваПоля);
...
Функция ПримерыКодаДляНастроек(Знач КодЯзыка) Экспорт
	...
	|	ObjectType = ITK_Enumerations.ItemDataQuery();
	|EndIf;'", КодЯзыка);
	Результат.Вставить("ОригинальныйТекст", ОригинальныйТекст);
