...
Процедура ДополнительнаяИнициализация(Форма, Элемент, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ОригинальныйТекст = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "ОригинальныйТекст", Текст);
	ТипЯзыка = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "ТипЯзыка", ИТК_РедакторКодаКлиентСервер.ТипЯзыка(Элемент));
	Подсказка = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "Подсказка", "");
	...
	УстановитьОригинальныйТекст(Элемент, ОригинальныйТекст);
...
Процедура ПодключаемыйОбработчикКоманд(Форма, Имя, ОповещениеОЗавершении = Неопределено) Экспорт
	...
	ИначеЕсли СтрНайти(Имя, "КонструкторВыделенногоЗапроса") Тогда
		...
		ПараметрыКонструктора.Вставить("РежимКомпоновкиДанных", ИТК_РедакторКодаКлиентСервер.РежимКомпоновкиДанных(Элемент));
		
		ИТК_ЗапросКлиент.ОткрытьКонструктор(ВыделенныйТекст, ОповещениеОЗавершении, ПараметрыКонструктора);
...
Процедура ПоказатьСравнение(Владелец, Текст1, Текст2, ТипЯзыка) Экспорт
	...
	Если Текст1 = Текст2 Тогда
		ПоказатьПредупреждение( , НСтр("ru = 'Тексты идентичны'; en = 'The texts are identical'"));
		Возврат;
	КонецЕсли;
...
Процедура Сравнение(Элемент, Знач Текст, ДополнительныеПараметры) Экспорт
	...
	ДвустороннееСравнение = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "ДвустороннееСравнение", Ложь);
	defaultView.compare(Текст, ДвустороннееСравнение, Истина);
	
	СворачиваниеОдинаковыхСтрок = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "СворачиваниеОдинаковыхСтрок", Ложь);
	Если СворачиваниеОдинаковыхСтрок Тогда
		defaultView.hideUnchangedBlocks();
	КонецЕсли;
...
Процедура УстановитьГраницыВыделения(Элемент, НачалоСтроки, НачалоКолонки, КонецСтроки = Неопределено, КонецКолонки = Неопределено) Экспорт
	...
	Если ИТК_РедакторКодаКлиентСервер.ЭтоРедакторКодаMonaco(Элемент) Тогда
		...
		Если defaultView.editor <> Неопределено Тогда
...
Процедура ОбновитьЭлементыКонтекстногоМеню(Группа, ДоступноРедактирование)
	...
	Для Каждого Элемент Из Группа.ПодчиненныеЭлементы Цикл 
		...
		Элемент.Видимость = ДоступноРедактирование 
								ИЛИ СтрНайти(Элемент.Имя, "КодНаВстроенномЯзыке")
...
Процедура ПрименитьНастройкиРедактораКОбъекту(Элемент, Настройки)
	...
	Если ЭтоРежимКод Тогда
		УстанавливаемаяТема = Настройки.Тема;
	ИначеЕсли РежимЯзыкаМонако = "bsl_query" Тогда
		УстанавливаемаяТема = "bsl-white-query";
	ИначеЕсли РежимЯзыкаМонако = "xml" Тогда
		УстанавливаемаяТема = "xml-white";
	Иначе
		УстанавливаемаяТема = Неопределено;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(УстанавливаемаяТема) Тогда
		
		Если ИТК_ФормаКлиентСерверПовтИсп.ЭтоТемнаяТема() Тогда
			
			Если УстанавливаемаяТема = "xml-white" Тогда
				УстанавливаемаяТема = "vs-dark";
			Иначе
				УстанавливаемаяТема = СтрЗаменить(УстанавливаемаяТема, "white", "dark");
			КонецЕсли;
			
		КонецЕсли;
		
		defaultView.setTheme(УстанавливаемаяТема);
	...
	ОтключитьСворачиваниеБлоков = НЕ Настройки.СворачиваниеБлоков;
	defaultView.setOption("disableFolding", ОтключитьСворачиваниеБлоков);
	
	// Отображение различий
	defaultView.setOption("showDiffDecorations", Настройки.ОтображениеРазличий);
	ЭлементГруппаИзменения = Элемент.КонтекстноеМеню.ПодчиненныеЭлементы.Найти(Элемент.Имя + "КонтекстноеМенюГруппаИзменения");
	Если ЭлементГруппаИзменения <> Неопределено Тогда
		ЭлементГруппаИзменения.Видимость = Настройки.ОтображениеРазличий;
	КонецЕсли;
