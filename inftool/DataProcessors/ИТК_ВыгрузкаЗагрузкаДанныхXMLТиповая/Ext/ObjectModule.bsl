...
Процедура Инициализация() Экспорт
	...
	// ITK1 {
	ДеревоМетаданных.Колонки.Добавить("ИТК_Отбор");
	// }
...
Процедура ЗаписьДанныхОбъектногоТипа(СтрокаДереваМетаданных, ЗаписьXML, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов = Ложь)
	...
	// ITK1 {
	Если СтрокаДереваМетаданных.ИТК_Отбор = Неопределено Тогда // Заполнен отбор???
		РезультатЗапроса = ПолучитьРезультатЗапросаСОграничениями(СтрокаДереваМетаданных);
	Иначе
		РезультатЗапроса = ИТК_ПолучитьРезультатЗапросаПоОтборуКомпоновкиДанных(СтрокаДереваМетаданных);
	КонецЕсли;
	// ITK1 }
...
Функция ИТК_ПолучитьРезультатЗапросаПоОтборуКомпоновкиДанных(СтрокаДереваМетаданных)
	
	// ITK1 {
	// Подготовка схемы запроса
	...
	ТекстЗапроса = ПолучитьТекстЗапросаПоСтроке(СтрокаДереваМетаданных, Ложь);
	НаборДанных.Запрос = ТекстЗапроса;
	
	// Загрузка настроек получение итогового запроса
	КомпоновщикНастроек = Новый КомпоновщикНастроекКомпоновкиДанных;
	КомпоновщикНастроек.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(СхемаКомпоновкиДанных));
	
	НастройкиКомпоновкиДанных = КомпоновщикНастроек.Настройки;
	
	ИТК_ЗаполнитьОтбор(НастройкиКомпоновкиДанных.Отбор, СтрокаДереваМетаданных.ИТК_Отбор.Отбор);
	
	// Заполнение настроек вывода
	...
	ГруппировкаКомпоновкиДанных.ПоляГруппировки.Элементы.Добавить(Тип("АвтоПолеГруппировкиКомпоновкиДанных"));
	ГруппировкаКомпоновкиДанных.Выбор.Элементы.Добавить(Тип("АвтоВыбранноеПолеКомпоновкиДанных"));
	
	Для Каждого ДоступноеПоле Из НастройкиКомпоновкиДанных.Выбор.ДоступныеПоляВыбора.Элементы Цикл
		
		ПолеВыборки = НастройкиКомпоновкиДанных.Выбор.Элементы.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
		Если ДоступноеПоле.Поле = Новый ПолеКомпоновкиДанных("СистемныеПоля") Тогда
			Продолжить;
		КонецЕсли;
		ПолеВыборки.Поле = ДоступноеПоле.Поле;
	...
	МакетКомпоновкиДанных = КомпоновщикМакета.Выполнить(СхемаКомпоновкиДанных,
		НастройкиКомпоновкиДанных, , , Тип("ГенераторМакетаКомпоновкиДанных"));
	
	Запрос = Новый Запрос;
	Запрос.Текст = МакетКомпоновкиДанных.НаборыДанных.НаборДанных.Запрос;
...
Процедура ИТК_ЗаполнитьОтбор(Отборы, ОтборИсточника) Экспорт
	
	// ITK1 {
	Для Каждого ЭлементОтбораИсточника Из ОтборИсточника.Элементы Цикл 
		
		ТипЭлементаОтбора = ТипЗнч(ЭлементОтбораИсточника);
		
		НовыйЭлементОтбора = Отборы.Элементы.Добавить(ТипЭлементаОтбора);
		ЗаполнитьЗначенияСвойств(НовыйЭлементОтбора, ЭлементОтбораИсточника);
		
		Если ТипЭлементаОтбора = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			ИТК_ЗаполнитьОтбор(НовыйЭлементОтбора, ЭлементОтбораИсточника);
		КонецЕсли;
