...
Function GetPictureForSource(Val TableName, 
	...
	If Pos > 0 Then
		ParentItemPicture = PictureLib.ExternalDataSource;
		Image = PictureLib.ExternalDataSourceCube;
	EndIf;

	If TypeOf(SourceTypes) <> Type("Array") Then
		SourceTypes = New Array;

		AddSourceType(SourceTypes, "Catalog.", PictureLib.Catalog,, "Catalogs");
		AddSourceType(SourceTypes, "Справочник.", PictureLib.Catalog,, "Справочники");

		AddSourceType(SourceTypes, "Document.", PictureLib.Document,, "Documents");
		AddSourceType(SourceTypes, "Документ.", PictureLib.Document,, "Документы");

		AddSourceType(SourceTypes, "FilterCriterion.", PictureLib.FilterCriterion,, "FilterCriteria");
		AddSourceType(SourceTypes, "КритерийОтбора.", PictureLib.FilterCriterion,, "КритерииОтбора");

		AddSourceType(SourceTypes, ".Recalculation", PictureLib.RotateCounterclockwise, True, "Recalculations", 0);
		AddSourceType(SourceTypes, ".Перерасчет", PictureLib.RotateCounterclockwise, True, "Перерасчеты", 0);

		AddSourceType(SourceTypes, "CalculationRegister.", PictureLib.CalculationRegister, True, "CalculationRegisters");
		AddSourceType(SourceTypes, "РегистрРасчета.", PictureLib.CalculationRegister, True, "РегистрыРасчета");

		AddSourceType(SourceTypes, "InformationRegister.", PictureLib.InformationRegister, True, "InformationRegisters");
		AddSourceType(SourceTypes, "РегистрСведений.", PictureLib.InformationRegister, True, "РегистрыСведений");

		AddSourceType(SourceTypes, "AccumulationRegister.", PictureLib.AccumulationRegister, True, "AccumulationRegisters");
		AddSourceType(SourceTypes, "РегистрНакопления.", PictureLib.AccumulationRegister, True, "РегистрыНакопления");

		AddSourceType(SourceTypes, "Report.", PictureLib.Report,, "Reports");
		AddSourceType(SourceTypes, "Отчет.", PictureLib.Report,, "Отчеты");

		AddSourceType(SourceTypes, "DataProcessor.", PictureLib.DataProcessor, , "DataProcessors");
		AddSourceType(SourceTypes, "Обработка.", PictureLib.DataProcessor, , "Обработки");

		AddSourceType(SourceTypes, "Enum.", PictureLib.Enum,, "Enums");
		AddSourceType(SourceTypes, "Перечисление.", PictureLib.Enum,, "Перечисления");

		AddSourceType(SourceTypes, "Constant.", PictureLib.Constant,, "Constants");
		AddSourceType(SourceTypes, "Константа.", PictureLib.Constant,, "Константы");
		AddSourceType(SourceTypes, "Константы (Изменения)", PictureLib.Constant,, "Constants (Changes)");

		AddSourceType(SourceTypes, "DocumentJournal.", PictureLib.DocumentJournal,, "DocumentJournals");
		AddSourceType(SourceTypes, "ЖурналДокументов.", PictureLib.DocumentJournal,, "ЖурналыДокументов");

		AddSourceType(SourceTypes, "ChartOfCharacteristicTypes.", PictureLib.ChartOfCharacteristicTypes,, "ChartsOfCharacteristicTypes");
		AddSourceType(SourceTypes, "ПланВидовХарактеристик.", PictureLib.ChartOfCharacteristicTypes,, "ПланыВидовХарактеристик");

		AddSourceType(SourceTypes, "ExternalDataSource.", PictureLib.ExternalDataSourceTable, True, "ExternalDataSources",, PictureLib.ExternalDataSource);
		AddSourceType(SourceTypes, "ВнешнийИсточникДанных.", PictureLib.ExternalDataSourceTable, True, "ВнешнийИсточникДанных",, PictureLib.ExternalDataSource);

		AddSourceType(SourceTypes, "AccountingRegister.", PictureLib.AccountingRegister, True, "AccountingRegisters");
		AddSourceType(SourceTypes, "РегистрБухгалтерии.", PictureLib.AccountingRegister, True, "РегистрыБухгалтерии");

		AddSourceType(SourceTypes, "Task.", PictureLib.Task, True, "Tasks");
		AddSourceType(SourceTypes, "Задача.", PictureLib.Task, True, "Задачи");

		AddSourceType(SourceTypes, "ChartOfAccounts.", PictureLib.ChartOfAccounts, True, "ChartsOfAccounts");
		AddSourceType(SourceTypes, "ПланСчетов.", PictureLib.ChartOfAccounts, True, "ПланыСчетов");

		AddSourceType(SourceTypes, "ChartOfCalculationTypes.", PictureLib.ChartOfCalculationTypes,, "ChartsOfCalculationTypes");
		AddSourceType(SourceTypes, "ПланВидовРасчета.", PictureLib.ChartOfCalculationTypes,, "ПланыВидовРасчета");

		AddSourceType(SourceTypes, "ExchangePlan.", PictureLib.ExchangePlan, True, "ExchangePlans");
		AddSourceType(SourceTypes, "ПланОбмена.", PictureLib.ExchangePlan, True, "ПланыОбмена");

		AddSourceType(SourceTypes, "BusinessProcess.", PictureLib.BusinessProcess, True, "BusinessProcesses");
		AddSourceType(SourceTypes, "БизнесПроцесс.", PictureLib.BusinessProcess, True, "БизнесПроцессы");

		AddSourceType(SourceTypes, "Sequence.", New Picture(GetTemplate("PictureSequences")), True, "Sequences");
		AddSourceType(SourceTypes, "Последовательность.", New Picture(GetTemplate("PictureSequences")), True, "Последовательности");

		AddSourceType(SourceTypes, " ", 20, True, NStr("ru='Временные таблицы'; SYS='QueryEditor.TemporaryTables'", "ru")); // добавить NSTr

		AddSourceType(SourceTypes, "NestedTable", PictureLib.QueryWizardTempTable);
		AddSourceType(SourceTypes, "NestedQuery", PictureLib.QueryWizardNestedQuery);
		AddSourceType(SourceTypes, "TempTable", PictureLib.QueryWizardTempTable);
	...
	If Image = Undefined Then
		...
		If (Pos1 > 0) OR (Pos2 > 0) Then
			If (Pos1 = StrLen(TableName) - StrLen(".Changes") +1)
				...
				Image = PictureLib.ExternalDataSourceTable;
			EndIf;
		EndIf;
	EndIf;
		...
		For Each SourceType In SourceTypes Do
			...
			If ((F > 0) AND (F = SourceType["StartPos"]))
				...
				If (N >= 2)
					...
					EndIf;
					Image = PictureLib.QueryWizardTempTable; // для вложенной таблицы
					Break;
				Else
					ParentItemPicture = SourceType["ParentItemPicture"];
					Image = SourceType["Image"];
	...
	If Image = Undefined Then
		...
		If (Pos = 0) AND (StrLen(TmpTableName) > 0) Then // для временной таблицы
			If NOT(TablesGroup) Then
				Image = PictureLib.QueryWizardTempTable;  // это для временной таблицы
			Else
				Image = PictureLib.ExternalDataSource; // а это для группы внешнего источника
			EndIf;
		EndIf;
	EndIf;
...
Function GetPictureForAvailableField(Val Field, 
	...
	If TypeOf(Field) = Type("QuerySchemaExpression")  Then
		...
		If Find(Presentation, " ")
			...
			OR Find(Presentation, "-") Then
		    	Return PictureLib.CustomExpression;
		EndIf;
	...
	ElsIf TypeOf(Field) = Type("QuerySchemaAvailableField") Then
		If Field.Role.Dimension Then
			Return PictureLib.Dimension;
		ElsIf Field.Role.Resource Then
			Return PictureLib.Resource;
		EndIf;
	ElsIf TypeOf(Field) = Type("QuerySchemaNestedTableColumn")
		...
		Return PictureLib.QueryWizardTempTable;
	EndIf;

	Return PictureLib.Attribute;
EndFunction
