// ITK 20 + {
&НаКлиенте
Перем СвязиТаблица1, СвязиТаблица2, СвязиВсе1, СвязиВсе2, СвязиУсловие, СвязиСОшибкой, СвязиНоваяСтрока, СвязиУсловиеСОшибкой Экспорт;
...
&AtServer
Function MakeCache(Val ItemsForChaching, Val CacheField, Val NestedFieldsInOneMap = False, Val SaveCount = False)
	Var Item;
	Var Cache;
	Var SubItem;
	Var SubItemsCache;
	...
	For Each Item In ItemsForChaching Do
		If Item.GetItems().Count() = 0 Then  // Если нет вложенных строк
			If SaveCount Then
				CurrentCount = Cache.Get(Item[CacheField]);
				If CurrentCount = Undefined Then
					CurrentCount = 0;
				EndIf;
				Cache.Insert(Item[CacheField], CurrentCount + 1);
			Else
		...
		Else
			If NOT(NestedFieldsInOneMap) Then
				...
				For Each SubItem In Item.GetItems() Do
					If SaveCount Then
						CurrentCount = SubItemsCache.Get(SubItem[CacheField]);
						If CurrentCount = Undefined Then
							CurrentCount = 0;
						EndIf;
						SubItemsCache.Insert(SubItem[CacheField], CurrentCount + 1);
					Else
			...
			Else
				For Each SubItem In Item.GetItems() Do
					If SaveCount Then
						CurrentCount = Cache.Get(SubItem[CacheField]);
						If CurrentCount = Undefined Then
							CurrentCount = 0;
						EndIf;
						Cache.Insert(SubItem[CacheField], CurrentCount + 1);
					Else
...
Procedure HidePagesServer()
	...
	QuerySchema = GetFromTempStorage(QueryWizardAddress);
	Query = GetSchemaQuery(QuerySchema, CurrentQuerySchemaSelectQuery, NestedQueryPositionAddress);
	Элементы["DataCompositionPage"].Видимость = (ТипЗнч(Query) = Тип("ЗапросВыбораСхемыЗапроса"));
	// }
...
Procedure FixChanges()
	...
	If PagesState["AdditionallyItemsPage"] Then
		...
		Else
			ChangeDropTableAtCache(TempTableName);
		EndIf;	
	EndIf;
...
Procedure FillUnions(Val Operators)
	...
	If Rebuild Then
		...
		For Pos = 0 To Count - 1 Do
			...
			ИТК_КонструкторЗапросов.ПриДобавленииКолонкиЗапросаВОбъединения(NewFormElement); // ITK43+
...
&AtServer
Function GetFieldUsageCount(Val Name, Val CacheGroupingFields, Val CacheSummingFields)
	Var UsedInGrouping;
	Var UsedInSumming;
	Var Item;
	Var CacheValue;
	...
	UsedInGrouping = 0;
	If TypeOf(CacheGroupingFields) = Type("Map") Then
		CacheValue = CacheGroupingFields.Get(Name);
		If CacheValue <> Undefined Then
			If TypeOf(CacheValue) = Type("Number") Then
				UsedInGrouping = CacheValue;
			Else
				UsedInGrouping = 1;
			EndIf;
		EndIf;
	Else
		For Each Item In CacheGroupingFields Do
			If Item["Name"] = Name Then
				UsedInGrouping = UsedInGrouping + 1;
			EndIf;
		EndDo;
	EndIf;

	UsedInSumming = 0;
	If TypeOf(CacheSummingFields) = Type("Map") Then
		CacheValue = CacheSummingFields.Get(Name);
		If CacheValue <> Undefined Then
			If TypeOf(CacheValue) = Type("Number") Then
				UsedInSumming = CacheValue;
			Else
				UsedInSumming = 1;
			EndIf;
		EndIf;
	Else
		For Each Item In CacheSummingFields Do
			If Item["Name"] = Name Then
				UsedInSumming = UsedInSumming + 1;
			EndIf;
		EndDo;
	EndIf;

	Return UsedInGrouping + UsedInSumming;
EndFunction

&AtServer
Function ShouldSkipField(Val Name, Val CurrentIndex, Val CacheGroupingFields, Val CacheSummingFields, Val AllFields)
	Var UsedCount;
	Var ProcessedCount;
	Var Expression;
	Var Pos;
	...
	UsedCount = GetFieldUsageCount(Name, CacheGroupingFields, CacheSummingFields);

	ProcessedCount = 0;
	For Pos = 0 To CurrentIndex - 1 Do
		Expression = AllFields.Get(Pos);
		If TypeOf(Expression) = Type("QuerySchemaExpression") Then
			If String(Expression) = Name Then
				ProcessedCount = ProcessedCount + 1;
			EndIf;
		ElsIf TypeOf(Expression) = Type("QuerySchemaNestedTable") Then
			For Each Item In Expression.Fields Do
				If String(Item) = Name Then
					ProcessedCount = ProcessedCount + 1;
				EndIf;
			EndDo;
		EndIf;
	...
	Return ProcessedCount < UsedCount;
EndFunction
...
Procedure FillGroupings(Val AllFieldsTree, Val GroupingFieldsTree, Val SummingFieldsTree, Query, Val Operator, MainObject)
	...
	If (Count > 0)
		...
		CacheSummingFields = MakeCache(CacheSummingFields, "Name", True, True);
	EndIf;
	...
	For Pos = 0 To Count-1 Do
		...
		If TypeOf(Expression) = Type("QuerySchemaExpression")  Then
			...
			If ShouldSkipField(ExpressionString, Pos, CacheGroupingFields, CacheSummingFields, AllFields) Then
				Continue;
			EndIf;
...
Procedure QueryOnCurrentPageChange(Item, CurrentPage)
    ...
    ElsIf CurrentPageName = "DropTablePage" Then
    	CurrentQueryControlsState(False, False);
...
Procedure ConditionDelete(Command)
	...
	CurrentItem = Conditions.FindByID(CurrentRow);
	Conditions.GetItems().Delete(Conditions.GetItems().IndexOf(CurrentItem));
...
Procedure ConditionCopy(Command)
	...
	CurrentItem = Conditions.FindByID(Items.Conditions.CurrentRow);
	NewItem = Conditions.GetItems().Add();
	NewItem["Condition"] = CurrentItem["Condition"];
	NewItem["Prefix"] = "*";
...
Procedure ConditionAddFromFields(Command)
	...
	CurrentItem = AllFieldsForConditions.FindByID(CurrentRow);

	If CurrentItem["Type"] = 2 Then
		// ITK4 * {
		// AddConditionFromFields(CurrentItem["Name"], CurrentItem["Presentation"]);
		ITKДобавитьПолеУсловия(CurrentItem);
		// }
	EndIf;
EndProcedure
...
Procedure ConditionsDrag(Item, DragParameters, StandardProcessing, Row, Field)
	...
	StandardProcessing = False;
	CurrentItem = AllFieldsForConditions.FindByID(Items.AllFieldsForConditions.CurrentRow);
	If CurrentItem["Type"] = 2 Then
		// ITK4 * {
		// AddConditionFromFields(CurrentItem["Name"], CurrentItem["Presentation"]);
		ITKДобавитьПолеУсловия(CurrentItem);
...
Procedure IndexDelete(Command)	
	...
	CurrentItem = Indexes.FindByID(Items.Indexes.CurrentRow);
	Index = Indexes.GetItems().IndexOf(CurrentItem);
...
Procedure IndexAdd(Command)
	...
	CurrentItem = AllFieldsForIndex.FindByID(Items.AllFieldsForIndex.CurrentRow);
	FieldIndex = CurrentItem["Index"];
	If (FieldIndex < 0) OR (CurrentItem["Type"] <> 2) Then
		Return;
	EndIf;
	...
	AddIndexAtClient(ParentIndex, FieldIndex, CurrentItem["Name"], CurrentItem["Presentation"], CurrentItem["Picture"]);

	If CurrentItem["Name"] = "" Then
		AllFieldsForIndex.GetItems().Delete(AllFieldsForIndex.GetItems().IndexOf(CurrentItem));
	EndIf;
...
Procedure ShowQueryEditor(Val QueryText, OldQueryText = "", Val StartRow = -1, Val StartCol = -1)
	...
	Form = OpenForm("DataProcessor.ИТК_QueryWizard2831.Form.QueryEditor", Params, ThisForm,,,,Notification,
...
Procedure OrderAdd(Command)
	...
	CurrentRow = Items.AllFieldsForOrder.CurrentRow;
	If CurrentRow = Undefined Then
		Return;
	EndIf;
	
	CurrentItem = AllFieldsForOrder.FindByID(CurrentRow);
	FieldIndex = CurrentItem["Index"];
	If (FieldIndex < 0) OR (CurrentItem["Type"] <> 2) Then
		Return;
	EndIf;
...
Procedure SetCurrentTab()
	...
	Else
		Items.Query.CurrentPage = Items.AdditionallyPage;
	EndIf;
...
Procedure GroupAdd(Command)
	...
	NeedUpdate = False;
	Rows = Items.AllFieldsForGrouping.GetRowsForProcessing();
	GroupingSet = 0;
...
Procedure GroupDelete(Command)
	...
	Rows = Items.GroupingFields.GetRowsForProcessing();
	For Each CurrentRow In Rows Do
		If (CurrentRow = Undefined) Then
			Return;
		EndIf;
...
Procedure SummDelete(Command)
	...
	While Items.SummingFields.GetRowsForProcessing().Count() > 0 Do
		CurrentRow = Items.SummingFields.GetRowsForProcessing()[0];
		If (CurrentRow = Undefined) Then
			Return;
		EndIf;
		...
		If ParentItems = Undefined Then
			Break;
		EndIf;		
		ChangeSummAtCache(2, ParentIndex, CurrentItems["Index"]);
		DeleteIndex = ParentItems.Indexof(CurrentItems);
		If DeleteIndex = Undefined Then
			Break;
		EndIf;
		Index = DeleteIndex;
		ParentItems.Delete(DeleteIndex);
	EndDo;

	If Index = Undefined Or Parent = Undefined Then
		Return;
	EndIf;
	
	ParentItems = Parent.GetItems();
	If ParentItems = Undefined Or ParentItems.Count() = 0 Then
	...
	If Index >= ParentItems.Count() Then
		Index = ParentItems.Count() - 1;
	EndIf;

	If Index >= 0 Then
		Items.SummingFields.CurrentRow	= ParentItems.Get(Index).GetID();
	EndIf;
...
Procedure GroupingFieldsDrag(Item, DragParameters, StandardProcessing, Row, Field)
	...
	EndIf;
	If DragParameters.Value = "DragFromSumm" Then
		Rows = Items.SummingFields.GetRowsForProcessing();
		For Each CurrentRow In Rows Do
			If (CurrentRow = Undefined) Then
				Return;
			EndIf;
...
&AtClient
Procedure GroupingFieldsOnHover(Item, Line, Field)
	Var Visible;
	
	Visible = AllowGrouping;
	//Items.GroupingFieldsRemoveSet.Visible = Visible;
EndProcedure
...
Procedure SummingFieldsDrag(Item, DragParameters, StandardProcessing, Row, Field)
	...
	If DragParameters.Value = "DragFromGroup" AND AllowGrouping = False Then
		While Items.GroupingFields.GetRowsForProcessing().Count() > 0 Do
			CurrentRow = Items.GroupingFields.GetRowsForProcessing()[0];
			If (CurrentRow = Undefined) Then
				Return;
			EndIf;
			CurrentItems = GroupingFields.FindByID(CurrentRow);
...
Procedure TotalsFieldsDelete(Command)
	...
	While Items.TotalsGroupingFields.GetRowsForProcessing().Count() > 0 Do
		CurrentRow = Items.TotalsGroupingFields.GetRowsForProcessing()[0];
		If (CurrentRow = Undefined) Then
			Return;
		EndIf;
...
Procedure TotalsFieldsAdd(Command)
	...
	Rows = Items.AllFieldsForTotals.GetRowsForProcessing();
	For Each CurrentRow In Rows Do
		If (CurrentRow = Undefined) Then
			Return;
		EndIf;
...
Procedure TotalsExpressionsDelete(Command)
	...
	While Items.TotalsExpressions.GetRowsForProcessing().Count() > 0 Do
		CurrentRow = Items.TotalsExpressions.GetRowsForProcessing()[0];
		If (CurrentRow = Undefined) Then
			Return;
		EndIf;
...
Procedure AvailableFieldsDelete(Command)
	...
	CurrentRows = Items.AvailableFields.GetRowsForProcessing();
...
Procedure SourcesDelete(Command)
	...
	CurrentRows = Items.Sources.GetRowsForProcessing();
	If CurrentRows.Count() > 0 Then
		CurrentRow = CurrentRows[0];
		If (CurrentRow = Undefined) Then
			Return;
...
Procedure AvailableFieldsAdd(Command)
	...
	Rows = Items.Sources.GetRowsForProcessing();
	...
	PagesItems = PreFillAtClient();
	AvailableFieldsAddAtServer(PagesItems, Rows);
	PostFillAtClient(PagesItems);
EndProcedure

&AtServer
Procedure AvailableFieldsAddAtServer(Val PagesItems, Val Rows)
	//Var Rows;
	Var CurrentRow;
	Var CurrentItems;
...
Procedure AvailableFieldsAddAll(Command)
	...
	PagesItems = PreFillAtClient();
	AvailableFieldsAddAllAtServer(PagesItems, Rows);
	PostFillAtClient(PagesItems);
EndProcedure

&AtServer
Procedure AvailableFieldsAddAllAtServer(Val PagesItems, Val Rows)
	//Var Rows;
	Var CurrentRow;
	Var CurrentItems;
...
Procedure SourcesAdd(Command)
	...
	Rows = Items.AvailableTables.GetRowsForProcessing();
	For Each CurrentRow In Rows Do
		If (CurrentRow = Undefined) Then
			Return;
		EndIf;
...
Procedure SourcesAddAll(Command)
	...
	EndDo;
	PagesItems = PreFillAtClient();
	SourcesAddAllAtServer(PagesItems, Rows);
	PostFillAtClient(PagesItems);
EndProcedure

&AtServer
Procedure SourcesAddAllAtServer(Val PagesItems, Val Rows)
	Var QuerySchema;
	//Var Rows;
	Var CurrentRow;
	Var CurrentItems;
...
Procedure ExpressionChange(Val IsExpression, Val NewExpression = Undefined, Val IsExpressionChanged = Undefined)
    ...
    OpenForm("DataProcessor.ИТК_QueryWizard2831.Form.ArbitraryExpression", Params, ThisForm,,,,Notification,
             FormWindowOpeningMode.LockOwnerWindow);
EndProcedure
...
Procedure QueryTypeOnChange(Item)
	...
	// ITK 48 + {
	ИТК_КонструкторЗапросовКлиент.ПослеПереключенияТипаЗапроса(ЭтотОбъект);
	// }
EndProcedure
...
Procedure SourcesParametersOfTheVirtualTable(Command)
	...
	OpenForm("DataProcessor.ИТК_QueryWizard2831.Form.AvailableTableParameters", Params, ThisForm,,,,Notification,
             FormWindowOpeningMode.LockOwnerWindow);
EndProcedure
...
Procedure SourcesOnActivateRow(Item)
	...
	If CurrentItems["ValueType"] = "QuerySchemaTable" Then
		// ITK 47 * {
		//Items.SourcesSourcesChangeTheCurrentItem.Enabled = False;
		Items.SourcesSourcesChangeTheCurrentItem.Enabled = (СтрНайти(CurrentItems["TableName"], ".") = 0);
		// ITK 47 }
		Items.SourcesSourcesParametersOfTheVirtualTable.Enabled = False;
...
Procedure SourcesCreateDescriptionOfTheTemporaryTable(Command)
	...
	Params.Insert("NestedQueryPositionAddress", NestedQueryPositionAddress);
	Notification = New NotifyDescription("TableParametersChanged", ThisForm, Params);
	OpenForm("DataProcessor.ИТК_QueryWizard2831.Form.TempTable", Params, ThisForm,,,,Notification,
             FormWindowOpeningMode.LockOwnerWindow);
EndProcedure
...
Procedure SourcesChangeTheCurrentItem(Command)
	...
	// ITK 47 + {
	ElsIf CurrentItems["ValueType"] = "QuerySchemaTable" Then
		
		FixChanges();
		
		ЭлементВременнаяТаблица = ИТК_ОбщийКлиентСервер.СписокЗначенийНайтиПоПредставлению(
			Элементы.CurrentQuerySchemaSelectQuery.СписокВыбора, CurrentItems.TableName);
		CurrentQuerySchemaSelectQuery = ЭлементВременнаяТаблица.Значение;
...
Procedure EditNestedQuery(Val NotificationName, Val NestedQuerySourceIndex, Val IsNew, Val QueryText = Undefined)
	...
	OpenForm("DataProcessor.ИТК_QueryWizard2831.Form.QueryWizard", Params,, True,,,
...
Procedure SourcesBeforeEditEndHandler()	
	...
	SetPageState("AvailableFieldsPage", True);
	SetPageState("ConditionsPage", True);
	SetPageState("ConditionsFieldsPage", True);
	FillPagesAtClient();
EndProcedure	
...
Procedure SourcesReplaceTheTable(Command)
	...
	OpenForm("DataProcessor.ИТК_QueryWizard2831.Form.TableSelecting", Params, ThisForm,,,,Notification,
             FormWindowOpeningMode.LockOwnerWindow);
EndProcedure
...
Procedure AvailableTablesSelection(Item, SelectedRow, Field, StandardProcessing)
	...
	// ITK5 + {
	Если ИТК_ОбщийКлиентСервер.ПоддерживаетсяПлатформой("8.5") 
			И Item.ТекущиеДанные.ПолучитьРодителя() = Неопределено Тогда
		Возврат;
	КонецЕсли;
	// }
	SourcesAdd(Undefined);
...
Procedure IndexesTableDelete(Command)	
	...
	CurrentRows = Items.IndexesTable.GetRowsForProcessing();
	If CurrentRows.Count() > 0 Then
		CurrentRow = CurrentRows[0];
		If (CurrentRow = Undefined) Then
			Return;
...
&AtClient
Procedure SummingFieldsEdit(Команда)
	Var CurrentItem;
	
	StandardProcessing = False;
	CurrentItem = SummingFields.FindByID(Items.SummingFields.CurrentRow);
...
Procedure DataCompositionRequiredJoinsParametersOfTheVirtualTable(Command)
	...
	OpenForm("DataProcessor.ИТК_QueryWizard2831.Form.DataCompositionAvailableTableParameters", Params, ThisForm,,,,Notification,
             FormWindowOpeningMode.LockOwnerWindow);
EndProcedure
...
Procedure DataCompositionFieldsDrag(Item, DragParameters, StandardProcessing, Row, Field)
	...
	If (DragParameters.Value = "DragFromAllFieldsForDataCompositionFields") Then
		NamesArray = new Array();
		StandardProcessing = False;
		Rows = Items.AllFieldsForDataCompositionFields.GetRowsForProcessing();
		For Each CurrentRow In Rows Do
			If (CurrentRow = Undefined) Then
				Return;
			EndIf;
...
Procedure DataCompositionFieldsCopy(Command)
	...
	Rows = Items.DataCompositionFields.GetRowsForProcessing();
	For Each CurrentRow In Rows Do
...
Procedure DataCompositionFieldsDelete(Command)
	...
	CurrentRows = Items.DataCompositionFields.GetRowsForProcessing();
	If CurrentRows.Count() > 0 Then
		CurrentRow = CurrentRows[0];
		CurrentItems = DataCompositionFields.FindByID(CurrentRow);
...
Procedure DataCompositionFieldsDeleteButton(Command)
	...
	While Items.DataCompositionFields.GetRowsForProcessing().Count() > 0 Do
		CurrentRow = Items.DataCompositionFields.GetRowsForProcessing()[0];
		CurrentItems = DataCompositionFields.FindByID(CurrentRow);
		DataCompositionFields.GetItems().Delete(DataCompositionFields.GetItems().IndexOf(CurrentItems));
	EndDo;
	SetPageState("DataCompositionFieldsPage", True);
...
Procedure DataCompositionFiltersDrag(Item, DragParameters, StandardProcessing, Row, Field)
	...
	If (DragParameters.Value = "DragFromAllFieldsForDataCompositionFilters") Then
		NamesArray = new Array();
		StandardProcessing = False;
		Rows = Items.AllFieldsForDataCompositionFilters.GetRowsForProcessing();
		For Each CurrentRow In Rows Do
			If (CurrentRow = Undefined) Then
				Return;
			EndIf;
...
Procedure DataCompositionFiltersCopy(Command)
	...
	Rows = Items.DataCompositionFilters.GetRowsForProcessing();
	For Each CurrentRow In Rows Do
...
Procedure DataCompositionFiltersDelete(Command)
	...
	CurrentRows = Items.DataCompositionFilters.GetRowsForProcessing();
	If CurrentRows.Count() > 0 Then
		CurrentRow = CurrentRows[0];
		CurrentItems = DataCompositionFilters.FindByID(CurrentRow);
...
Procedure DataCompositionFiltersDeleteButton(Command)
	...
	While Items.DataCompositionFilters.GetRowsForProcessing().Count() > 0 Do
		CurrentRow = Items.DataCompositionFilters.GetRowsForProcessing()[0];
		CurrentItems = DataCompositionFilters.FindByID(CurrentRow);
		DataCompositionFilters.GetItems().Delete(DataCompositionFilters.GetItems().IndexOf(CurrentItems));
	EndDo;
	SetPageState("DataCompositionFiltersPage", True);
...
Procedure CharacteristicsCopy(Command)
	...
	Rows = Items.Characteristics.GetRowsForProcessing();
	For Each CurrentRow In Rows Do
		If (CurrentRow = Undefined) Then
			Return;
		EndIf;
...
Procedure CharacteristicsDelete(Command)
	...
	CurrentRows = Items.Characteristics.GetRowsForProcessing();
	If CurrentRows.Count() > 0 Then
		CurrentRow = CurrentRows[0];
		CurrentItems = Characteristics.FindByID(CurrentRow);
...
Процедура ПодключаемыйITKСвязиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование) Экспорт
	...
	Если СвязиСОшибкой = Неопределено Тогда
		СвязиСОшибкой = Ложь;
	КонецЕсли;
	Если НЕ СвязиСОшибкой Тогда
		СвязиНоваяСтрока = НоваяСтрока;
...
Процедура ПодключаемыйITKСвязиПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования) Экспорт
	...
	Если ОтменаРедактирования Тогда
		...
		Если СвязиНоваяСтрока Тогда
			
			Если СвязиСОшибкой Тогда
				ЭтотОбъект["ITKСвязи"].Удалить(ТекущиеДанные);
			КонецЕсли;
	...
	Если НЕ СвязиНоваяСтрока
		...
		СвязиНоваяСтрока = Истина;
		
	КонецЕсли;
		
	Если СвязиНоваяСтрока Тогда
		ТипИзмененияСвязи  = 1; // Добавление связи
	ИначеЕсли Элемент.ТекущийЭлемент = Элементы.ITKСвязиУсловие Тогда
		ТипИзмененияСвязи  = 4; // Заменить условие
	Иначе
		ТипИзмененияСвязи  = 3; // Изменить тип соединения
	КонецЕсли;
	...
	ТекстОшибки = ОбновитьСвязи();
	СвязиСОшибкой = ЗначениеЗаполнено(ТекстОшибки);
	Если СвязиСОшибкой Тогда
		
		Если СвязиНоваяСтрока Тогда
			
			ВосстановленнаяСтрока = ЭтотОбъект["ITKСвязи"].Добавить();
			ЗаполнитьЗначенияСвойств(ВосстановленнаяСтрока, ТекущиеДанные);
			
		Иначе
			СвязиУсловиеСОшибкой = ТекущиеДанные.Условие;
		КонецЕсли;
		
		ОповещениеОповещения = Новый ОписаниеОповещения("ИТК_ЗавершенВопросОбновлениеСвязейПрошлоСОшибкойЗавершение", ЭтотОбъект);
		
		ШаблонТекстаВопроса = НСтр("ru = 'В условии связи ошибка:
									|%1'; en = 'There is an error in the connection condition:
									|%1'");
		
		КнопкиВопроса = Новый СписокЗначений;
		КнопкиВопроса.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Продолжить редактирование'; en = 'Continue editing'"));
		КнопкиВопроса.Добавить(Неопределено, НСтр("ru = 'Отменить изменения'; en = 'Canceled changes'"));
		ПоказатьВопрос(ОповещениеОповещения, СтрШаблон(ШаблонТекстаВопроса, ТекстОшибки), КнопкиВопроса, , КодВозвратаДиалога.Да);
		
	Иначе
		СвязиУсловиеСОшибкой = Неопределено;
	КонецЕсли;
...
Функция ОбновитьСвязи()
	...
	If (PagesItems <> Undefined)
		...
		ТекстОшибки = ErrorMessages;
		ErrorMessages = Неопределено;
		NewQueryBatchCount = 0;
...
&НаКлиенте
Процедура ИТК_ЗавершенВопросОбновлениеСвязейПрошлоСОшибкойЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		
		СвязиСОшибкой = Неопределено;
		СвязиУсловиеСОшибкой = Неопределено;
	...
	Иначе
		...
		Если НЕ СвязиНоваяСтрока Тогда
			ТекущиеДанные.Условие = СвязиУсловиеСОшибкой;
		КонецЕсли;
		
		ТекущийЭлемент = Элементы.ITKСвязиУсловие;
		Элементы.ITKСвязи.ИзменитьСтроку();
...
&НаСервере
Процедура ИТК_УдалитьИзЗапросаИтогиАвтоупорядочивание() Экспорт
	ИТК_КонструкторЗапросов.УдалитьИзЗапросаИтогиАвтоупорядочивание(ЭтотОбъект);
КонецПроцедуры

&НаКлиенте
Procedure ИТК_SourcesПередНачаломИзменения(Элемент, Отказ)
	
	// ITK 50
	ТекущиеДанные = Sources.НайтиПоИдентификатору(Элемент.ТекущаяСтрока);
