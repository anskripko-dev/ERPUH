...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If ClientApplication.CurrentInterfaceVariant() = ClientApplicationInterfaceVariant.Version8_5 Then
		Items.Cancel.Visible = False;
		Items.OK.Title = NStr("ru = 'Сохранить'; SYS = 'QueryWizard.Save'", "ru");
	EndIf;
...
Procedure FieldsBeforeExpandHandler()
	...
	Try
		DataProcessor = GetForm("DataProcessor.ИТК_QueryWizard2831.Form.QueryWizard");
	Except
...
Procedure BeforeClose(Cancel, Exit, MessageText, StandardProcessing)
	...
	If Exit = True Then
		...
		Cancel = True;
		MessageText = NStr("ru='Редактор произвольного выражения будет закрыт'; SYS='QueryEditor.ArbitraryExpressionWillClose'", "ru");
		Return;
	EndIf;
...
Procedure ShowQueryWizard(Command)
	...
	If (Expression <> "") Then
		Params.Insert("QueryText", Expression);	
	EndIf;
	
	Notification = New NotifyDescription("QueryChanged", ThisForm, Params);
	
	OpenForm("ExternalDataProcessor.QueryWizard28.Form.QueryWizard", Params,, True,,,
		Notification, FormWindowOpeningMode.LockOwnerWindow);
EndProcedure

&AtClient
Procedure QueryChanged(Result, Params) Export
	Var ErrorMessage;
	Var StartCol;
	Var StartRow;
	Var CurrentItems;
