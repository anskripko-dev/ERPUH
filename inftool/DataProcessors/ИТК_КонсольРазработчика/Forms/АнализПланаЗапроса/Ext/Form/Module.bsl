...
Процедура ОформитьМусорныеСтроки(Строка)
	...
	Для Каждого ВложеннаяСтрока Из Строка.Строки Цикл 
		...
		Если СтрНачинаетсяС(ВложеннаяСтрока.Контекст, "Config.") Тогда
			...
			ИТК_Форма.ДобавитьУсловноеОформление(УсловноеОформление, "ПланЗапроса", Новый Структура("ЦветТекста", ИТК_ОбщийКлиентСервер.ЦветПалитрыСерый()), "ПланЗапроса.UID", ВложеннаяСтрока.UID);
			...
			Пока Родитель <> Неопределено Цикл 
				...
				Если Родитель.Строки.Количество() = 1 Тогда
					ИТК_Форма.ДобавитьУсловноеОформление(УсловноеОформление, "ПланЗапроса", Новый Структура("ЦветТекста", ИТК_ОбщийКлиентСервер.ЦветПалитрыСерый()), "ПланЗапроса.UID", Родитель.UID);
				Иначе 
					Прервать;
				КонецЕсли;
