...
Функция ВернутьТекстЗапросаРасхождений(РазделВГО, Отбор=Неопределено) Экспорт
	...
	ТекстЗапросаОбъединениеД =
		...
		|"; // @Non-NLS объединяем отдельные запросы в таблицу: ДебиторыРазвернуто
	ТекстЗапросаОбъединениеК =
		"////////////////////////////////////////////////////////////////////////////////
		|// Запрос для объединения данных кредиторов
		|"; // @Non-NLS объединяем отдельные запросы в таблицу: КредиторыРазвернуто
	ТекстЗапросаГруппировка =
		"////////////////////////////////////////////////////////////////////////////////
		|// Запрос для группировки значений Дебиторов
		|"; // @Non-NLS группируем данные в таблицу Дебиторы и Кредиторы
	ТекстЗапросаОсновнойРазвернуто =
		"////////////////////////////////////////////////////////////////////////////////
		|// Запрос для вычисления расхождений
		|"; // @Non-NLS соединяем таблицу дебиторов с таблицей кредиторов и вычисляем расхождения
	ТекстЗапросаОсновной =
		"////////////////////////////////////////////////////////////////////////////////
		|// Запрос для группировки расхождений
		|"; // @Non-NLS группируем таблицу расхождений
	...
	Если КолРесурсов = 0 Тогда // ПоказательСверки
		ТекстСообщения = НСтр("ru = 'Некорректная настройка сверки (%1). Необходим хотя бы один ресурс.'");
		ВызватьИсключение СтрШаблон(ТекстСообщения, РазделВГО);
		Возврат "";
	КонецЕсли;
	...
	Для Каждого Аналитика_ Из мАналитик Цикл
		...
		ИначеЕсли Аналитика_.ТипДляЭлиминации = Перечисления.ТипыАналитикЭлиминации.Контрагент Тогда
			...
			ИмяАналитикиКонтрагента = ИмяАналитики;
	...
	Если ИмяАналитикиОрганизации = "" ИЛИ ИмяАналитикиКонтрагента = "" ИЛИ ИмяАналитикиВалютыВзаиморасчетов = "" Тогда
		...
		ТекстНедостающихАналитик = ТекстНедостающихАналитик + ?(ТекстНедостающихАналитик="","",", ")
			+ ?(ИмяАналитикиКонтрагента = "",НСтр("ru = '""Контрагент""'"),"");
		ТекстНедостающихАналитик = ТекстНедостающихАналитик + ?(ТекстНедостающихАналитик="","",", ")
...
Функция РассчитатьРасхождения(Знач Отбор, ТекстОшибки, флОтладка = Ложь) Экспорт
	...
	Попытка
		...
		Пока Истина Цикл // остановка по значению флага флРезультатыПрочитаны
			...
			КурсГруппы = УправлениеРабочимиПроцессамиУХ.ПересчитатьПоКурсу(1, ВалютаГруппы, ВалютаРУ, Перечисления.ВидыКурсов.КурсНаМоментНачисления, Сценарий, ПериодСценария, ТекДата);
			...
			Пока ВыборкаЭтапов.Следующий() Цикл
				...
				Пока ВыборкаНастройкиСверки.Следующий() Цикл
					...
					// Рассчитываем для организаций исходные данные
					Для Каждого ПериметрСверки ИЗ ТЗПериметрСверки Цикл
						
						ТабОрганизаций = ПолучитьТаблицуИсходныхДанныхДляРасчетаРасхождений(
								ПериодСценария, Сценарий, ВыборкаИсточниковДанных, ПериметрСверки);
								
						КоличествоОрганизаций = ТабОрганизаций.Количество();

						// Обходим все пары организаций
						Для НомерОрганизацииДт = 0 По КоличествоОрганизаций-1 Цикл
							Дебитор = ТабОрганизаций[НомерОрганизацииДт].Организация;
							...
							ДанныеОрганизацииДт = ТабОрганизаций[НомерОрганизацииДт].Данные;
							...
							Для НомерОрганизацииКт = 0 По КоличествоОрганизаций-1 Цикл
								...
								Кредитор = ТабОрганизаций[НомерОрганизацииКт].Организация;
								...
								ДанныеОрганизацииКт = ТабОрганизаций[НомерОрганизацииКт].Данные;
								...
								Если флОтладка Тогда
									ВремСтруктураПараметровЗапроса = Новый Структура("ПараметрыОтчета,ОтборРегистра,ТЗОрганизаций", ПараметрыОтчета, ОтборРегистра, ТабОрганизаций);
...
Функция ПолучитьЗоныРасчетаРасхождений(Знач Отбор, Знач флОтладка)
	...
	|	ШаблоныСверкиВГО.Этап КАК Этап,
	|	ШаблоныСверкиВГО.ШаблонСверкиВГО КАК ШаблонСверкиВГО,
					...
					Пока ВыборкаОрганизация.Следующий() Цикл
						...
						Пока ВыборкаРазделВГО.Следующий() Цикл
							...
							Иначе
								...
								Текст = НСтр("ru = 'Раздел <%1> по <%2>(<%3>, <%4>)'");
								Текст = СтрШаблон(Текст, ВыборкаРазделВГО.РазделВГО, ВыборкаРазделВГО.Организация, 
													ВыборкаРазделВГО.ПериодСценария, ВыборкаРазделВГО.Сценарий);
...
// Возвращает соответствие с описанием аналитик расхождения ВГО для указанной расшифровки СКД.
...
Функция ВернутьТекстЗапросаДляРасшифровки(Параметры, ДопПараметры, ДопПоляОтчета) Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Иначе
			...
			ТекстСторонаСверки = СторонаСверки;
			Если СторонаСверки = Перечисления.СтороныСверкиВГО.Дебитор Тогда
				ТекстСторонаСверки = Метаданные.Перечисления.СтороныСверкиВГО.ЗначенияПеречисления.Дебитор.Имя;
			ИначеЕсли СторонаСверки = Перечисления.СтороныСверкиВГО.Кредитор Тогда
				ТекстСторонаСверки = Метаданные.Перечисления.СтороныСверкиВГО.ЗначенияПеречисления.Кредитор.Имя;
			Иначе
			...
			ТекстЗапроса = ТекстЗапроса +
				"	ЗНАЧЕНИЕ(Перечисление.СтороныСверкиВГО." + ТекстСторонаСверки + ") КАК СторонаСверки,
				|";
		КонецЕсли;
		...
		Для Каждого СтрокаСвязи Из мСтрокОсновныхАналитик Цикл
			...
			ИмяПоля = ?(СтрокаСвязи.Поле = "", "Значение", СтрокаСвязи.Поле);
			ТекстЗапроса = ТекстЗапроса + "	ЗначенияПоказателейОтчетов." + ИмяПоля + " КАК " + ИмяАналитики_ + "," + Символы.ПС;
		...
		ТекстЗапроса = ТекстЗапроса +
			...
			|	И НЕ ЗначенияПоказателейОтчетов.Версия.ЗаписьВДополнительнойВалюте
...
Функция ТекстЗапроса_ИсточникиДанных()
	...
	|ГДЕ
	|	СтатусыПериодовСценариев.Сценарии = &Сценарий
	|	И СтатусыПериодовСценариев.Периоды В
	|			(ВЫБРАТЬ
	|				т.ПериодСценария
	|			ИЗ
	|				втПериоды КАК т)
	...
	|ВЫБРАТЬ
	|	РазделыСверкиВГО.Ссылка КАК РазделСверкиВГО,
	|	ШаблоныСверкиВГО.УправлениеПериодом КАК УправлениеПериодом,
	|	ШаблоныСверкиВГО.Этап КАК Этап,
	|	ШаблоныСверкиВГО.ШаблонСверкиВГО КАК ШаблонСверкиВГО
	|ПОМЕСТИТЬ РазделыСверкиВГО
	|ИЗ
...
Функция ПолучитьШаблонСверкиПоСценариюПериоду(Сценарий, ПериодСценария) Экспорт
	...
	Результат = ПредопределенноеЗначение("Справочник.ШаблоныСверкиВГО.ПустаяСсылка");
	
	Регламент = УправлениеРабочимиПроцессамиУХ
		.ПолучитьВерсиюРегламентаПодготовкиОтчетности(Сценарий, ПериодСценария);
		
	Если Не ЗначениеЗаполнено(Регламент) Тогда
		Возврат Результат;
	КонецЕсли;
	
	ШаблонВГО = ОбщегоНазначенияУХ.ЗначениеРеквизитаОбъекта(Регламент, "ШаблонСверкиВГО");
	Если ЗначениеЗаполнено(ШаблонВГО) Тогда
		Возврат ШаблонВГО;	
	КонецЕсли;
...
Функция ВернутьТекстСчета(Текст, Знач ПрефиксДт, Знач ПрефиксКт, Префикс) Экспорт
	...
	СимволыПлюса = "+";
	ДлинаПрефикса = Макс(СтрДлина(ПрефиксДт), СтрДлина(ПрефиксКт));
		...
		Если Состояние_ = 1 Тогда
			...
			Если СтрНайти(СимволыПрефикса, ТекСимвол) > 0 Тогда
				...
				ИначеЕсли НРег(Префикс) = НРег(ПрефиксКт) Тогда
					Префикс = ПрефиксКт;
					Состояние_ = 2;
				ИначеЕсли СтрДлина(Префикс) = ДлинаПрефикса Тогда
					// похож, но не префикс
					Состояние_ = 2;
		...
		ИначеЕсли Состояние_ = 2 Тогда
			...
			Если СтрНайти(СимволыПрефикса, ТекСимвол) > 0 Тогда
				...
				Если НРег(МожетПрефикс) = НРег(ПрефиксДт) ИЛИ НРег(МожетПрефикс) = НРег(ПрефиксКт) Тогда
					...
					Прервать;
				ИначеЕсли СтрДлина(МожетПрефикс) = ДлинаПрефикса Тогда
...
#Область РеглЗадание_ПроверитьСрокиВыполненияСверкиВГО
...
Процедура ПроверитьСрокиВыполненияСверкиВГО() Экспорт
	...
	ТекстЗапроса.Добавить(ТекстЗапроса_ПериодыДляОповещений(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_втКИ(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_ПользователиДляОповещений(НомераТаблиц));
		
	Запрос = Новый Запрос(СтрСоединить(ТекстЗапроса, "
											  |
											  |;
											  |"));
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;//передаем втКИ, втРасхожденияВГО 
	Запрос.УстановитьПараметр("ДатаРасчета", ТекущаяДата());
	
	РезультатПакета = Запрос.ВыполнитьПакет();
	Если РезультатПакета[НомераТаблиц.ПериодыДляОповещений].Пустой() Тогда
		Возврат;// нет УОП для оповещений	
	ИначеЕсли РезультатПакета[НомераТаблиц.ПользователиДляОповещений].Пустой() Тогда
		Возврат;// нет пользователей для оповещений	
	КонецЕсли;
	
	КонтекстПисьма = Неопределено;
	ПользователиВГО = РезультатПакета[НомераТаблиц.ПользователиДляОповещений].Выгрузить();
	
	ВыборкаПериоды = РезультатПакета[НомераТаблиц.ПериодыДляОповещений].Выбрать();
	Пока ВыборкаПериоды.Следующий() Цикл
		
		Запрос.Текст = ТекстЗапроса_втРасхожденияВГО(НомераТаблиц, ВыборкаПериоды);
		
		Запрос.УстановитьПараметр("Сценарий", 		ВыборкаПериоды.Сценарий);
		Запрос.УстановитьПараметр("ПериодСценария", ВыборкаПериоды.ПериодСценария);
		
		ПодготовитьКонтекстПисьмаРасхождений(КонтекстПисьма, ВыборкаПериоды);
		НетРасхождений = Запрос.Выполнить().Пустой();
		Если НетРасхождений Тогда
			Продолжить;		
		КонецЕсли;
		
		Для Каждого СтрокаТаб Из ПользователиВГО Цикл
			
			Запрос.Текст = ТекстЗапроса_РасхожденияДляПользователя(НомераТаблиц);
			Запрос.УстановитьПараметр("Пользователь", СтрокаТаб.Пользователь);
			ПодготовитьКонтекстПисьмаРасхождений(КонтекстПисьма, СтрокаТаб);
			
			СформироватьПисьмоСверки(КонтекстПисьма, Запрос.Выполнить().Выгрузить());	
...
Процедура СформироватьПисьмоСверки(КонтекстПисьма, Расхождения)

	Если Расхождения.Количество() = 0 Тогда
		Возврат;	
	КонецЕсли;
	
	ЗаголовокПисьма = НСтр("ru = 'Истекает срок сверки ВГО <%1> (%2)'") + Символы.ПС;
	ЗаголовокПисьма = СтрШаблон(ЗаголовокПисьма, КонтекстПисьма.ПериодСценария, КонтекстПисьма.Сценарий);
	
	ТекстПисьма = УправлениеЭлектроннойПочтойУХ.ВернутьТекстПисьма(
		ЗаголовокПисьма + ПолучитьВебТаблицу(Расхождения), ТекстПисьма);
	
	Письмо = Новый ИнтернетПочтовоеСообщение;
	
	Письмо.Отправитель 	= КонтекстПисьма.Отправитель;
	Письмо.Тема 		= ЗаголовокПисьма;
	
	Письмо.Получатели.Добавить(КонтекстПисьма.АдресЭП);
	Письмо.Тексты.Добавить(ТекстПисьма, КонтекстПисьма.ТипТекста);
	
	УправлениеЭлектроннойПочтойУХ.ЗаписатьПисьмоВРегистр(Письмо, КонтекстПисьма.Пользователь);

КонецПроцедуры

Функция ПолучитьВебТаблицу(ТабИсточник)
	...
	Для Каждого стр Из ТабИсточник Цикл
		...
		ТекстСтр = СтрШаблон(ШаблонСтроки, 
						стр.Организация,
						стр.Контрагент,
						стр.РазделВГО,
						стр.Сумма,
						стр.ВалютаВзаиморасчетов	
					);
		ТекстПисьма.Добавить(ТекстСтр);
		
	КонецЦикла;
	
	Возврат СтрСоединить(ТекстПисьма, Символы.ПС);
	
КонецФункции

Функция ПодготовитьКонтекстПисьмаРасхождений(КонтекстПисьма, Источник)

	Если КонтекстПисьма = Неопределено Тогда
	     КонтекстПисьма = Новый Структура(
		 				"Отправитель, ЗаголовокПисьма, ТипТекста, Сценарий, ПериодСценария, АдресЭП, Пользователь",
					    УчетнаяЗаписьДляРассылкиУведомлений(),
						НСтр("ru = 'Истекает срок сверки ВГО.'"), 
						ТипТекстаПочтовогоСообщения.HTML);
	КонецЕсли;
	
	ЗаполнитьЗначенияСвойств(КонтекстПисьма, Источник);

КонецФункции

Функция ТекстЗапроса_ПериодыДляОповещений(НомераТаблиц)
	
	НомераТаблиц.Вставить("ПериодыДляОповещений", НомераТаблиц.Количество());
	...
	|ГДЕ
	|	т.СтатусБлокировки = 0
	|	И НЕ т.УправлениеПериодомСценария.ПериодЗакрыт
	|	И т.ВерсияРегламента.ШаблонСверкиВГО <> ЗНАЧЕНИЕ(Справочник.ШаблоныСверкиВГО.ПустаяСсылка)
	|	И ВЫБОР
	|			КОГДА НЕ т.ВерсияРегламента.ИспользоватьПроцесс
	|					ИЛИ т.УправлениеПериодомСценария.ДатаНачалаПроцесса = ДАТАВРЕМЯ(1, 1, 1)
	|				ТОГДА т.Периоды.ДатаНачала
	|			ИНАЧЕ т.УправлениеПериодомСценария.ДатаНачалаПроцесса
	|		КОНЕЦ <= ДОБАВИТЬКДАТЕ(&ДатаРасчета, ДЕНЬ, -т.ВерсияРегламента.ШаблонСверкиВГО.ДнейДоОповещенияОРасходениях)";
    
КонецФункции

Функция ТекстЗапроса_втКИ(НомераТаблиц)

	НомераТаблиц.Вставить("втКИ", НомераТаблиц.Количество());
	
	Возврат
	"ВЫБРАТЬ
	|	тКИ.Организация КАК Организация,
	|	тКИ.Пользователь КАК Пользователь,
	|	тКИ.АдресЭП КАК АдресЭП,
	|	МАКСИМУМ(тКИ.ЭтоКуратор) КАК ЭтоКуратор
	|ПОМЕСТИТЬ втКИ
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|					т.Значение
	|				ИЗ
	|					Константа.РольКураторВГО КАК т)) КАК тКИ
	|
	|СГРУППИРОВАТЬ ПО
	|	тКИ.Пользователь,
	|	тКИ.АдресЭП,
...
Функция ТекстЗапроса_ПользователиДляОповещений(НомераТаблиц)

	НомераТаблиц.Вставить("ПользователиДляОповещений", НомераТаблиц.Количество());
	
	Возврат
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	втКИ.Пользователь КАК Пользователь,
	|	втКИ.АдресЭП КАК АдресЭП
	|ИЗ
	|	втКИ КАК втКИ";

КонецФункции

Функция ТекстЗапроса_РасхожденияДляПользователя(НомераТаблиц)
	
	НомераТаблиц.Вставить("РасхожденияДляПользователя", НомераТаблиц.Количество());
	
	Возврат
	"ВЫБРАТЬ
	|	втРасхожденияВГО.Организация КАК Организация,
	|	втРасхожденияВГО.Контрагент КАК Контрагент,
	|	втРасхожденияВГО.РазделВГО КАК РазделВГО,
	|	втРасхожденияВГО.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
	|	СУММА(втРасхожденияВГО.Сумма) КАК Сумма
	|ИЗ
	|	втРасхожденияВГО КАК втРасхожденияВГО
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втКИ КАК втКИ
	|		ПО втРасхожденияВГО.Организация = втКИ.Организация
	|			И (втКИ.Пользователь = &Пользователь)
	|ГДЕ
	|	ЕСТЬNULL(втРасхожденияВГО.Сумма, 0) <> 0
	|
	|СГРУППИРОВАТЬ ПО
	|	втРасхожденияВГО.Организация,
	|	втРасхожденияВГО.Контрагент,
	|	втРасхожденияВГО.ВалютаВзаиморасчетов,
	|	втРасхожденияВГО.РазделВГО
	|
	|УПОРЯДОЧИТЬ ПО
	|	Организация,
	|	Контрагент,
...
Функция ТекстЗапроса_втРасхожденияВГО(НомераТаблиц, ВыборкаПериоды)
	
	ТекстЗапроса = Новый Массив;
	ПовторноеВыполнение = НомераТаблиц.Свойство("втРасхожденияВГО");
	НомераТаблиц = Новый Структура;//подготовка нового запроса
	
	Если ОбщегоНазначенияПовтИспУХ.РассчитыватьРасхожденияПорталаВГО() Тогда
		
		Если ПовторноеВыполнение Тогда
			ТекстЗапроса.Добавить(МСФОЗапросыУХ.ТекстЗапроса_УничтожитьТаблицу(НомераТаблиц, "втРасхожденияВГО"));
		КонецЕсли;
		
		ТекстЗапроса.Добавить(ТекстЗапроса_втРасхожденияОффлайнВГО(НомераТаблиц));
		
	Иначе 
						
		Параметры = Новый Структура("ПериодСценария, Сценарий, ПоВерсии, ИменаГруппировок, ИменаОтборов, ИмяТаблицыСверкаВГО",
				ВыборкаПериоды.ПериодСценария, ВыборкаПериоды.Сценарий, Ложь, "", "", "втРасхожденияВГО");
		
		ТекстЗапросаВГО = ТекстЗапроса_ОтчетВГО(Параметры);
		ТекстЗапросаВГО = СтрЗаменить(ТекстЗапросаВГО, "&Дата", "&ДатаРасчета");
		ТекстЗапросаВГО = СтрЗаменить(ТекстЗапросаВГО, "РасхожденийОсталосьУрегулировать", "Сумма");
			
		Если ПовторноеВыполнение Тогда
			Для каждого НомерИсточник Из Параметры.НомераТаблиц Цикл
				ТекстЗапроса.Добавить(МСФОЗапросыУХ.ТекстЗапроса_УничтожитьТаблицу(НомераТаблиц, НомерИсточник.Ключ));
			КонецЦикла;
		КонецЕсли;

		Для каждого НомерИсточник Из Параметры.НомераТаблиц Цикл //мб неправильный порядок таблиц
			НомераТаблиц.Вставить(НомерИсточник.Ключ, НомераТаблиц.Количество());
		КонецЦикла;
		
		ТекстЗапроса.Добавить(ТекстЗапросаВГО);
		
	КонецЕсли;
	
	ТекстЗапроса.Добавить(ТекстЗапроса_ЕстьРасхождения(НомераТаблиц));
	
	Возврат СтрСоединить(ТекстЗапроса, ПроведениеСерверУХ.РазделительЗапросовВПакете());

КонецФункции

Функция ТекстЗапроса_втРасхожденияОффлайнВГО(НомераТаблиц)
	
	НомераТаблиц.Вставить("втРасхожденияВГО", НомераТаблиц.Количество());
	...
	|	СУММА(ИнформацияСверкиВГОПостоянная.СуммаРасхождения) КАК Сумма
	|ПОМЕСТИТЬ втРасхожденияВГО
	|ИЗ
	|	РегистрСведений.ИнформацияСверкиВГОПостоянная КАК ИнформацияСверкиВГОПостоянная
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИнформацияСверкиВГО.СрезПоследних(
	|				&ДатаРасчета,
	|				ПериодСценария = &ПериодСценария
	|					И Состояние В (ЗНАЧЕНИЕ(Перечисление.СостояниеСверкиВГО.НеЗагружен), ЗНАЧЕНИЕ(Перечисление.СостояниеСверкиВГО.НеРассчитан), ЗНАЧЕНИЕ(Перечисление.СостояниеСверкиВГО.Несогласован), ЗНАЧЕНИЕ(Перечисление.СостояниеСверкиВГО.Спор))
	|					И Сценарий = &Сценарий) КАК ИнформацияСверкиВГОСрезПоследних
	...
	|					И Состояние В (ЗНАЧЕНИЕ(Перечисление.СостояниеСверкиВГО.НеЗагружен), ЗНАЧЕНИЕ(Перечисление.СостояниеСверкиВГО.НеРассчитан), ЗНАЧЕНИЕ(Перечисление.СостояниеСверкиВГО.Несогласован), ЗНАЧЕНИЕ(Перечисление.СостояниеСверкиВГО.Спор))
	|					И Сценарий = &Сценарий
	|					 ) КАК ИнформацияСверкиВГОСрезПоследних
	|		ПО ИнформацияСверкиВГОПостоянная.ПериодСценария = ИнформацияСверкиВГОСрезПоследних.ПериодСценария
	|			И ИнформацияСверкиВГОПостоянная.Сценарий = ИнформацияСверкиВГОСрезПоследних.Сценарий
	|			И ИнформацияСверкиВГОПостоянная.Этап = ИнформацияСверкиВГОСрезПоследних.Этап
	|			И ИнформацияСверкиВГОПостоянная.Отправитель = ИнформацияСверкиВГОСрезПоследних.Отправитель
...
Функция ТекстЗапроса_ЕстьРасхождения(НомераТаблиц)
	
	НомераТаблиц.Вставить("ЕстьРасхождения", НомераТаблиц.Количество());
	
	Возврат
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ИСТИНА КАК ЕстьРасхождения
	|ИЗ
	|	втРасхожденияВГО КАК т
	|ГДЕ
	|	ЕСТЬNULL(т.Сумма, 0) <> 0";
...
Функция ТекстЗапроса_ПоказателиЭлиминации(ТекстЗапроса, НомераТаблиц, Параметры = Неопределено) Экспорт
	
	ТекстЗапроса.Добавить(ТекстЗапроса_ПериодыСценария(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_Регламенты(НомераТаблиц));
	
	ТекстЗамены = ТекстЗапроса_Контрагенты(НомераТаблиц);
	
	ТекстЗамены = СтрЗаменить(ТекстЗамены, "&ТекстОтбораОрганизации", "ИСТИНА");
	ТекстЗамены = СтрЗаменить(ТекстЗамены, "&ЭтоОрганизацияВГО", "ИСТИНА");
	ТекстЗамены = СтрЗаменить(ТекстЗамены, "&ЭтоКонтрагентВГО", "ИСТИНА");
	
	
	ТекстЗапроса.Добавить(ТекстЗамены);
	ТекстЗапроса.Добавить(ТекстЗапроса_Организации(НомераТаблиц));
	
	ТекстЗапроса.Добавить(ТекстЗапроса_ШаблонСверки(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_РазделыСверки(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_ПоказателиОтчетов(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_ВидыОтчетов(НомераТаблиц));
	
	ТекстЗапроса.Добавить(ТекстЗапроса_ИспользуемыеВерсии(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_КурсыВалют(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_ИспользуемыеЗПО(НомераТаблиц, Параметры));

	ТекстЗапроса.Добавить(ТекстЗапроса_Урегулирования(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_Комментарии(НомераТаблиц));
	
	//ТекстЗапроса.Добавить(ТекстЗапроса_СверкаВГО(НомераТаблиц, ИмяТаблицыСверкаВГО));
...
Функция ТекстЗапроса_РасхожденияВГО(Параметры = Неопределено, ПараметрОрганизация = Неопределено, ПараметрКонтрагент = Неопределено) Экспорт
	...
	ИмяТаблицыСверкаВГО = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ИмяТаблицыСверкаВГО");
	...
	Разделитель = ОбщегоНазначенияУХВызовСервера.ТекстРазделителяЗапросовПакета();
	ЗаменыВГО = ПолучитьЗаменыЗапросаВГО(Параметры, ПараметрОрганизация, ПараметрКонтрагент);
	
	Параметры.Вставить("НомераТаблиц", НомераТаблиц);
...
Функция ТекстЗапроса_Организации(НомераТаблиц)
...
Функция ТекстЗапроса_ПериодыСценария(НомераТаблиц = Неопределено)
	
	Если НомераТаблиц <> Неопределено Тогда
		НомераТаблиц.Вставить("втПериоды", НомераТаблиц.Количество());
	КонецЕсли;
...
Функция ТекстЗапроса_ШаблонСверки(НомераТаблиц)
	
	НомераТаблиц.Вставить("втШаблонСверки", НомераТаблиц.Количество());
	...
	|	РазделыСверкиВГО.Ссылка.ВидЗначения КАК ВидКурса
...
Функция ТекстЗапроса_РазделыСверки(НомераТаблиц)
	
	НомераТаблиц.Вставить("втРазделыСверки", НомераТаблиц.Количество());
	...
	|	ПоказателиОтчетов.ВидКурса В (ЗНАЧЕНИЕ(Перечисление.ВидыКурсов.СреднийКурсЗаПериодМСФО), ЗНАЧЕНИЕ(Перечисление.ВидыКурсов.СреднийКурсЗаПериод), ЗНАЧЕНИЕ(Перечисление.ВидыКурсов.СреднийКурсЗаПериодЦБ)) КАК ЭтоСреднийКурс,
...
Функция ТекстЗапроса_ПоказателиОтчетов(НомераТаблиц)
	...
	|	т.ЭтоСреднийКурс КАК ЭтоСреднийКурс,
	|	т.Показатель.ГруппаРаскрытия.Валютная КАК ЕстьВалютнаяСумма,
	|	ВЫБОР
	|		КОГДА т.ГруппаРаскрытияОтбор.ВидАналитики6 <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)
	|			ТОГДА 6
	|		КОГДА т.ГруппаРаскрытияОтбор.ВидАналитики5 <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)
	|			ТОГДА 5
	|		КОГДА т.ГруппаРаскрытияОтбор.ВидАналитики4 <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)
	|			ТОГДА 4
	|		КОГДА т.ГруппаРаскрытияОтбор.ВидАналитики3 <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)
	|			ТОГДА 3
	|		КОГДА т.ГруппаРаскрытияОтбор.ВидАналитики2 <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)
	|			ТОГДА 2
	|		КОГДА т.ГруппаРаскрытияОтбор.ВидАналитики1 <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)
	|			ТОГДА 1
	|		КОГДА т.ВидОтчета.ВидАналитики6 <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)
	|			ТОГДА 6
	|		КОГДА т.ВидОтчета.ВидАналитики5 <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)
	|			ТОГДА 5
	|		КОГДА т.ВидОтчета.ВидАналитики4 <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)
	|			ТОГДА 4
	|		КОГДА т.ВидОтчета.ВидАналитики3 <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)
	|			ТОГДА 3
	|		КОГДА т.ВидОтчета.ВидАналитики2 <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)
	|			ТОГДА 2
	|		КОГДА т.ВидОтчета.ВидАналитики1 <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК НомерЗПО,
	...
	|			ТОГДА 6
	|		КОГДА т.ВидОтчета.ВидАналитики1
	|			ТОГДА 1
	|		КОГДА т.ВидОтчета.ВидАналитики2
	|			ТОГДА 2
	|		КОГДА т.ВидОтчета.ВидАналитики3
	|			ТОГДА 3
	|		КОГДА т.ВидОтчета.ВидАналитики4
	|			ТОГДА 4
	|		КОГДА т.ВидОтчета.ВидАналитики5
	|			ТОГДА 5
	|		КОГДА т.ВидОтчета.ВидАналитики6
	|			ТОГДА 6
	|		ИНАЧЕ 0
...
Функция ТекстЗапроса_ВидыОтчетов(НомераТаблиц)

	НомераТаблиц.Вставить("втВидыОтчетов", НомераТаблиц.Количество());
...
Функция ТекстЗапроса_ИспользуемыеВерсии(НомераТаблиц)
...
Функция ТекстЗапроса_КурсыВалют(НомераТаблиц)
	...
	НомераТаблиц.Вставить("втКурсыВсехВалют", 	НомераТаблиц.Количество());
    НомераТаблиц.Вставить("втКурсыВалют", 		НомераТаблиц.Количество());
	
	Возврат
	"ВЫБРАТЬ
	|	т.Валюта КАК Валюта,
	|	ВидыКурсов.ЭтоСреднийКурс КАК ЭтоСреднийКурс,
	|	т.Валюта В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			т.ВалютаВГО
	|		ИЗ
	...
	|	ВЫБОР
	|		КОГДА ВидыКурсов.ЭтоСреднийКурс
	|			ТОГДА т.СреднийКурсЗаПериодМСФО
	|		ИНАЧЕ т.КурсНаКонецПериода
	|	КОНЕЦ КАК Курс,
	|	т.Кратность КАК Кратность,
	|	т.ПериодКурса КАК ПериодОтчета
	|ПОМЕСТИТЬ втКурсыВсехВалют
	|ИЗ
	|	РегистрСведений.ПериодическиеКурсы КАК т
	...
	|	КурсВалюты.ЭтоСреднийКурс КАК ЭтоСреднийКурс,
	...
	|	КурсВалютыВГО.ПериодОтчета КАК ПериодОтчета
	|ПОМЕСТИТЬ втКурсыВалют
	|ИЗ
	|	втКурсыВсехВалют КАК КурсВалюты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втКурсыВсехВалют КАК КурсВалютыВГО
	|		ПО КурсВалюты.ЭтоСреднийКурс = КурсВалютыВГО.ЭтоСреднийКурс
	|			И КурсВалюты.ПериодОтчета = КурсВалютыВГО.ПериодОтчета
	|			И (НЕ КурсВалюты.ЭтоВалютаВГО)
...
Функция ТекстЗапроса_ИспользуемыеЗПО(НомераТаблиц, Параметры = Неопределено)
	...
	Возврат СтрЗаменить(ТекстЗапроса_СводЗПО(НомераТаблиц), "втИсточник", СтрШаблон("(%1)", СтрСоединить(ТекстЗапроса, ТекстОбъединить))); 
...
Функция ТекстЗапроса_ЗПО()
	...
	|	КОНЕЦ КАК Валюта,
	|	втВерсии.ПериодОтчета КАК ПериодОтчета,
	|	втПоказатели.ЭтоСреднийКурс КАК ЭтоСреднийКурс,
...
Функция ТекстЗапроса_КэшЗПО()
	...
	|	ВЫБОР
	|		КОГДА втПоказатели.ЕстьВалютнаяСумма
	|				И тЗПО.АналитикаВалюта <> ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
	|			ТОГДА тЗПО.АналитикаВалюта
	|		ИНАЧЕ втВерсии.ВалютаОтчета
	|	КОНЕЦ КАК Валюта,
	|	втВерсии.ПериодОтчета КАК ПериодОтчета,
	|	втПоказатели.ЭтоСреднийКурс КАК ЭтоСреднийКурс,
	|	ВЫБОР
	|		КОГДА втПоказатели.ЕстьВалютнаяСумма
	|			ТОГДА тЗПО.ЗначениеВалюта
	|		ИНАЧЕ тЗПО.Значение
	|	КОНЕЦ КАК Значение
	|ИЗ
	...
	|		ИНАЧЕ втВерсии.ВалютаОтчета
	|	КОНЕЦ,
	|	втВерсии.ПериодОтчета,
	|	втПоказатели.ЭтоСреднийКурс,
	|	ВЫБОР
	|		КОГДА втПоказатели.ЕстьВалютнаяСумма
	|			ТОГДА тЗПО.ЗначениеВалюта
	|		ИНАЧЕ тЗПО.Значение
...
Функция ТекстЗапроса_СводЗПО(НомераТаблиц)
	
	НомераТаблиц.Вставить("втЗПО", НомераТаблиц.Количество());
	...
	|		КОГДА МАКСИМУМ(втКурсыВалют.КурсВГО) * МАКСИМУМ(втКурсыВалют.Кратность) = 0
	|			ТОГДА 0
	...
	|	И т.ЭтоСреднийКурс = втКурсыВалют.ЭтоСреднийКурс
	|	И т.ПериодОтчета = втКурсыВалют.ПериодОтчета
	|
	|СГРУППИРОВАТЬ ПО
	|	т.Валюта,
	|	ВЫБОР
...
Функция ТекстЗапроса_Урегулирования(НомераТаблиц)
	
	НомераТаблиц.Вставить("втУрегулирования", НомераТаблиц.Количество());
...
Функция ТекстЗапроса_Комментарии(НомераТаблиц)
	
	НомераТаблиц.Вставить("втПредметыКомментирования", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("Комментарии", НомераТаблиц.Количество());
...
Функция ТекстЗапроса_СверкаВГО(НомераТаблиц, ИмяТаблицы = Неопределено)
	
	ИмяТаблицыПоместить = ?(ИмяТаблицы = Неопределено, "РасхожденияВГО", ИмяТаблицы);
	НомераТаблиц.Вставить(ИмяТаблицыПоместить, НомераТаблиц.Количество());
	...
	ТекстЗамены = ?(ЗначениеЗаполнено(ИмяТаблицы), "ПОМЕСТИТЬ " + ИмяТаблицы, "");
	Возврат СтрЗаменить(ТекстЗапроса, "ПОМЕСТИТЬ РасхожденияВГО", ТекстЗамены);
