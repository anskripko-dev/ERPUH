...
//		- Консолидирующая-  Консолидирующая организация
//
...
Процедура ЗаполнитьРегистрПроводкиДляЭлиминации(ПараметрыЭлиминации) Экспорт
	...
	Попытка
		...
		Пока ВыборкаНастройкиСверки.Следующий() Цикл
			...
			ПараметрыДляЗапросаЭлиминации.Вставить("ОписаниеТехСчета", ОписаниеТехСчета);
				...
				Пока ДанныеПроводки.Следующий() Цикл
					...
					Если флИсключитьВыручку И (НЕ флИсключитьНРП ИЛИ ДанныеПроводки.ЭтоКредитор) Тогда
						...
						ЗаписатьПроводкуЭлиминацииВыручки(
							РегПроводок, НомерЗаписи, ДанныеПроводки, ПараметрыДляЗапросаЭлиминации, флИсключитьНРП);
					...
					Если флИсключитьДанныеОС И ДанныеПроводки.ЭтоКредитор Тогда
						...
						Контрагент_ФВ = ПолучитьФВ(ДанныеПроводки.Контрагент, ТЗОрганизаций, ПараметрыДляЗапросаЭлиминации.ВалютаГр);
						...
						СтрокаТаблицыВНА_Покупатель.ВалютаФункц = Контрагент_ФВ;
			...
			Если флИсключитьНРП Тогда
				...
				Для НомерОрганизации = 0 По КоличествоОрганизацийНРП-1 Цикл
					...
					ПокупательФВ = ПолучитьФВ(Организация, ТЗОрганизаций, ПараметрыДляЗапросаЭлиминации.ВалютаГр);
				    
					ПараметрыДляЗапросаЭлиминации.Организация = Организация;
					ПараметрыДляЗапросаЭлиминации.Вставить("ФункциональнаяВалюта", ПокупательФВ);
					...
					ЗапросЭлиминации.Параметры.Вставить("Организация", Организация);
					ЗапросЭлиминации.Параметры.Вставить("ФункциональнаяВалюта", ПокупательФВ);
					...
					Кост = ?(ИтогоЗакупки = 0, 0, ИтогоСебестоимостьПродаж / ИтогоЗакупки);
					КостГр = ?(ИтогоСебестоимостьПродажГр = 0, 0, ИтогоСебестоимостьПродажГр / ИтогоЗакупкиГр);
					...
					Краспр = ?(ИтогоСебестоимостьПродаж = 0, 0, НРПвыб / ИтогоСебестоимостьПродаж);
					КраспрГр = ?(ИтогоСебестоимостьПродажГр = 0, 0, НРПвыбГр / ИтогоСебестоимостьПродажГр);
					...
					Пока Проводка.Следующий() Цикл
						...
						ЗаписатьПроводкуЭлиминацииНРППродано(
							РегПроводок, НомерЗаписи, Проводка, ПараметрыДляЗапросаЭлиминации);
					...
					Краспр = ?(ИтогоЗакупки = 0, 0, НРПост / ИтогоЗакупки);// к распределению
					КраспрГр = ?(ИтогоЗакупкиГр = 0, 0, НРПостГр / ИтогоЗакупкиГр);
...
// Функция - Транслировать счет, Получает счет после трансляции
//
// Параметры:
//	Трансляция - СправочникСсылка.ШаблоныТрансляций, трансляция по правилам которой будет найден счет.
...
Функция ПолучитьФВ(Организация, ТЗОрганизаций, ФВПоУмолчанию = Неопределено)

	СтрокаТаб = ТЗОрганизаций.Найти(Организация, "Организация");
	
	Если СтрокаТаб = Неопределено Тогда
		Возврат ФВПоУмолчанию;
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаб, "ФункВалюта") 
		И ЗначениеЗаполнено(СтрокаТаб.ФункВалюта) Тогда
		Возврат СтрокаТаб.ФункВалюта;
	КонецЕсли;  
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаб, "ВалютаФункц") 
		И ЗначениеЗаполнено(СтрокаТаб.ВалютаФункц) Тогда
		Возврат СтрокаТаб.ВалютаФункц;
	КонецЕсли;  
...
Функция ПолучитьТекстЗапросаПолученияДанныхДляЭлиминации(НомераТаблиц)
	...
	|	&Консолидирующая КАК ГлавнаяОрганизация
	...
	|			И (ВерсииРегламентовПодготовкиОтчетностиОрганизационныеЕдиницы.ОрганизационнаяЕдиница = &Консолидирующая)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ПО Регламент.ОсновнаяВалюта = КурсыВалютВВалютеГруппы.Валюта
	|ГДЕ
	|	Регламент.Ссылка В
	|			(ВЫБРАТЬ
	...
	|			ИЗ
	|				УправлениеПериодом КАК УправлениеПериодом)
	|	И Регламент.ОрганизационнаяЕдиницаРодитель = &Консолидирующая
...
Функция ПолучитьДанныеДляЭлиминации(ДатаСреза, ПараметрыЭлиминации)
	...
	Запрос.УстановитьПараметр("Консолидирующая",  		ПараметрыЭлиминации.Консолидирующая);
...
Процедура ЗаполнитьРегистрПроводкиДляЭлиминацииПоказателейОтчетов(
	...
	Пока ВыборкаИсточниковДанных.Следующий() Цикл
		...
		КонецЦикла; // Для Каждого Организация Из мОрганизаций Цикл
		
	КонецЦикла; // Пока ВыборкаИсточниковДанных.Следующий() Цикл
...
Функция ВернутьТекстЗапросаЭлиминации(ПараметрыЭлиминации, Отбор=Неопределено)
	...
	ТекстЗапросаОбъединение =
		...
		|"; // @Non-NLS объединяем отдельные запросы в таблицу: ОбъединениеИД
	ТекстЗапросаОсновной =
		"////////////////////////////////////////////////////////////////////////////////
		|// Запрос для вычисления сумм элиминации
		|"; // @Non-NLS расчитываем суммы элиминации и делаем трансляцию счетов
...
Функция ВернутьТекстЗапросаДопТаблицДляЭлиминации(БезТрансляции)
	...
	Иначе
		...
		ТекстЗапроса =
			...
			|	МАКСИМУМ(СоответствияСчетовДляТрансляции.СчетПриемник) КАК СчетПриемник,
			|	МАКСИМУМ(ИсточникиДанныхДляРасчетов.Ссылка) КАК ИсточникДанных,
			|	ВЫБОР
			...
			|			И (ИсточникиДанныхДляРасчетов.СпособИспользования = ЗНАЧЕНИЕ(Перечисление.СпособыИспользованияОперандов.ДляФормулРасчета))
			|			И (ИсточникиДанныхДляРасчетов.ПотребительРасчета.СчетБД = СоответствияСчетовДляТрансляции.СчетПриемник)
			|			И (СоответствияСчетовДляТрансляции.СчетИсточник В (ИсточникиДанныхДляРасчетов.Счет, ИсточникиДанныхДляРасчетов.ПоказательОтбор.СчетБД))
			|
			|СГРУППИРОВАТЬ ПО
...
Функция ПроверитьЧтоЗаполненыСчетаДтКт(ПараметрыЭлиминации, ДанныеЗаполнения, СчетДтИсточник, СчетКтИсточник, СчетДт, СчетКт)
	...
	Контекст = Новый Структура("ЭтоКредитор");
	ЗаполнитьЗначенияСвойств(Контекст, ДанныеЗаполнения);
	
	Если НЕ ЗначениеЗаполнено(СчетДт) И НЕ ЗначениеЗаполнено(СчетДтИсточник) Тогда
		ЗаписатьОшибкуВПротоколЭлиминации(ПараметрыЭлиминации, ДанныеЗаполнения, Нстр("ru = 'Не получилось определить счет дебета.'"));
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(СчетКт) И НЕ ЗначениеЗаполнено(СчетКтИсточник) Тогда
...
Процедура ЗаписатьПроводкуЭлиминацииВыручки(РегПроводок, НомерЗаписи, ДанныеЗаполнения, ПараметрыЭлиминации, флИсключитьНРП)
	
	СторнироватьДт = ДанныеЗаполнения.ЭтоКредитор;
	Если СторнироватьДт И флИсключитьНРП Тогда
		СторнироватьДт = Ложь;//в случае сторно выручки МПЗ, саму выручку сторнируем по Кт: Дт 000 Кт 
	КонецЕсли;
	
	Если НЕ СторнироватьДт И ПараметрыЭлиминации.РазделВГО.СпособЭлиминации = Перечисления.СпособыЭлиминации.ЭлиминацияОСиНМА Тогда
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		ЗаполнитьЗначенияСубконто(
			ДанныеЗаполнения, НоваяСтрока, ПараметрыЭлиминации.КолАналитик,
			ИсточникДанныхТ, ?(СторнироватьДт, "СубконтоДт", "СубконтоКт"));
		
		ЗаполнитьЗначенияСубконтоТехСчетаЭлиминации(
			НоваяСтрока, ПрефиксТехСчета, ПараметрыЭлиминации, ДанныеЗаполнения);
...
Процедура ЗаполнитьЗначенияСубконтоТехСчетаЭлиминации(ПриемникДанных, ПрефиксТехСчета, ПараметрыЭлиминации, ДанныеЗаполнения)
	...
	Контекст = Новый Структура("ЭтоКредитор, Контрагент", Истина);
	ЗаполнитьЗначенияСвойств(Контекст, ДанныеЗаполнения);//для проводки НРП
	
	Если Контекст.ЭтоКредитор Тогда
		ЗначенияЗаполнения.Отправитель = ПараметрыЭлиминации.Организация;
		ЗначенияЗаполнения.Получатель = Контекст.Контрагент;
	Иначе
		ЗначенияЗаполнения.Отправитель = Контекст.Контрагент;
		ЗначенияЗаполнения.Получатель = ПараметрыЭлиминации.Организация;
	КонецЕсли;
	
	// Заполняем субконто проводки.
	ЗначенияСубконтоТехСчет = ПолучитьЗначенияСубконтоТехСчета(ПараметрыЭлиминации.ОписаниеТехСчета, ЗначенияЗаполнения);
...
Процедура ЗаписатьОшибкуВПротоколЭлиминации(ПараметрыЭлиминации, ДанныеЗаполнения, Знач ТекстОшибки, Знач ТехническаяИнформация="")
	...
	ТекстОшибки = СтрШаблон(Нстр("ru = '%1
		...
		|Периметр: %4'"), 
					...
					ПараметрыЭлиминации.Сценарий, 
					ПараметрыЭлиминации.ПериодОтчета, 
					ПараметрыЭлиминации.Консолидирующая);
					
	Если ЗначениеЗаполнено(ДанныеЗаполнения) Тогда
		Контекст = Новый Структура("ИсточникДанных");
		ЗаполнитьЗначенияСвойств(Контекст, ДанныеЗаполнения);
		
		ТекстОшибки = СтрШаблон(Нстр("ru = '%1
			|Источник данных: %2
			|Раздел ВГО: %3'"), 
		              	ТекстОшибки, Контекст.ИсточникДанных, ПараметрыЭлиминации.РазделВГО);
		Для Ном = 1 По ПараметрыЭлиминации.КолАналитик Цикл
	...
	Если ЗначениеЗаполнено(ПравилаИспользованияПолейЗапроса) Тогда
		Для Каждого НастройкиСоответствия Из ПравилаИспользованияПолейЗапроса Цикл
			...
			Попытка
				...
				НомерСтрокой = Сред(НастройкиСоответствия.КодАналитики, 10);
				Если СтрДлина(НомерСтрокой) = 1 Тогда
