...
Процедура ПриСозданииНаСервере(Форма, ИменаСубконто) Экспорт
	...
	Форма.КэшируемыеЗначения.Вставить("ВыбытиеИнвестиций", ИмяДокумента = "ВыбытиеИнвестиций" ИЛИ ИмяДокумента = "РеализацияМСФО");
...
Функция ПолучитьДанныеУправленияФормой(ОрганизацияИнвестор, ОбъектИнвестирования, ДатаСреза = Неопределено) Экспорт
	...
	Результат = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(Запрос.Выполнить().Выгрузить()[0]);
...
Процедура СформироватьДвижения(Движения, ДополнительныеСвойства, Отказ) Экспорт
	...
	Иначе
		...
		Если Движения.Найти("СамостоятельноеПризнаниеКонтроля") <> Неопределено 
			И МодульКИК.ОпределитьУсловияВидимостиИДвиженияПоСамостоятельнумуКонтролю(Реквизиты) Тогда
...
Процедура ВыполнитьКонтрольРезультатовПроведения(Реквизиты, Отказ) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = НСтр("ru = ' недостаточное количество, требуется '");
		ОшибкаКоличества = ?(Выборка.Количество < 0, ТекстОшибки + (-Выборка.Количество), "");
		ТекстОшибки = НСтр("ru = ' недостаточная стоимость, требуется '");
		ОшибкаСуммы = ?(Выборка.Сумма < 0,  + (-Выборка.Сумма), "");
		
		ТекстОшибки = НСтр("ru = 'На счете учета <%1> не достаточно средств для операции:'");
		ОбщегоНазначенияУХ.СообщитьОбОшибке(
					СтрШаблон(ТекстОшибки, Выборка.Счет)
...
Функция ПолучитьБалансовуюСтоимость(Организация, ДатаДокумента, ДокументСсылка, СчетУчета, ОрганизацияОбъектИнвестирования) Экспорт
	...
	МодульРСБУ = ОбщегоНазначения.ОбщийМодуль("РегистрыБухгалтерии.Хозрасчетный");
	Отбор = Новый Структура("Организация, Счет,Субконто1", Организация, СчетУчета.СчетСсылка, КонтрагентыПоОрганизации[0]);
	
	ТабОстатки = МодульРСБУ.Остатки(
	...
	СчетУчетаСчетСсылка = СчетУчета.СчетСсылка; // ПланСчетовСсылка
	Если ТабОстатки.Количество() = 0 Тогда
		Возврат 0;
	ИначеЕсли СчетУчетаСчетСсылка.Вид = ВидСчета.Пассивный Тогда
