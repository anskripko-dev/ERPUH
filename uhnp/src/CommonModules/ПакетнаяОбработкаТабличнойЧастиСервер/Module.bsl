...
Функция ДанныеДляОбработкиСтроки(ИмяИсточника, КэшированныеЗначения) Экспорт
	...
	Если Результат = Неопределено Тогда
		...
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось получить данные при пакетной обработке строк табличной части
										...
										|
										|%3';
										|en = 'Cannot get data during batch processing of table rows
										|There is no data for the row with ID ""%1"" for the %2 table.
										|
										|Data of the processed row:
										|
										|%3'"),
		...
		Иначе
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Пакетная обработка строк в табличной части.';
					|en = 'Batch processing of rows in the table.'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
// Заполняется служебный флаг ДоступноОформлениеОтчетаДавальцу.
// Флаг взводится, если в строке указано давальческое назначение версии 2.4 и в нем заполнен договор или заказ давальца.
// Если флаг установлен и в назначении указан заказ давальца, то дополнительно взводит флаг ОформитьОтчетДавальцу
// Если флаг сброшен, то дополнительно сбрасывает флаг ОформитьОтчетДавальцу.
//
// Параметры:
//	ТекущаяСтрока - Структура - структура со свойствами строки документа.
...
// Заполняет служебный флаг ВладелецИзделияДоступен.
// Флаг взводится, если в строке не указано давальческое назначение.
// Если флаг взведен, то становится доступно для редактирования поле "Владелец изделия".
// Если флаг сброшен, то дополнительно очищается значение поля "Владелец изделия".
//
// Параметры:
//	ТекущаяСтрока - Структура - структура со свойствами строки документа.
//	СтруктураДействий - Структура - структура с действиями, которые нужно произвести.
...
Процедура ЗаполнитьВладелецИзделияДоступен(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	ДанныеДляОбработки = ДанныеДляОбработкиСтроки("ВладелецИзделияДоступен", КэшированныеЗначения);
	
	ТекущаяСтрока.ВладелецИзделияДоступен = ДанныеДляОбработки[0].ВладелецИзделияДоступен;
	
	Если Не ТекущаяСтрока.ВладелецИзделияДоступен Тогда
		ТекущаяСтрока.ВладелецИзделия = Неопределено;
	КонецЕсли;
...
// Заполняет реквизиты табличной части Обеспечение в этапе производства: ВариантОбеспечения, Склад, Обособленно.
// 
// Вариант обеспечения: для номенклатуры с типом Работа устанавливается Не обеспечивать, в остальных случаях К обеспечению.
// Склад: заполняется по схеме обеспечения номенклатуры и подразделению этапа.
// Обособленно: флаг получают
//  - производимые полуфабрикаты
//  - номенклатура с флагом Обособленная закупка
//  - давальческие материалы
//  - материалы заказа с флагом Обособленное обеспечение материалов.
// 
// Через параметры действия можно отключить заполнение некоторых полей, а так же заполнить все строки 
// одним предопределенным значением.
//
Процедура ПроверитьЗаполнитьОбеспечениеВЭтапеПроизводства(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	ЗаполнятьВсегда = НЕ ПараметрыДействия.ЗаполнятьТолькоПустые;
	...
	Если ПараметрыОбеспечения <> Неопределено Тогда
		...
		Если ПараметрыДействия.ЗаполнитьВариантОбеспечения Тогда
			Если ЗначениеЗаполнено(ПараметрыДействия.ВариантОбеспечения) Тогда
				ТекущаяСтрока.ВариантОбеспечения = ПараметрыДействия.ВариантОбеспечения;
			ИначеЕсли ЗаполнятьВсегда ИЛИ НЕ ЗначениеЗаполнено(ТекущаяСтрока.ВариантОбеспечения) Тогда
				ТекущаяСтрока.ВариантОбеспечения = ПараметрыОбеспечения[0].ВариантОбеспечения;
			КонецЕсли;
		КонецЕсли;
		
		Если ПараметрыДействия.ЗаполнитьСклад Тогда
			Если ЗначениеЗаполнено(ПараметрыДействия.Склад) Тогда
				ТекущаяСтрока.Склад = ПараметрыДействия.Склад;
			Иначе
				ЗаполнитьПоДаннымОбеспечения = ЗаполнятьВсегда
					ИЛИ НЕ ЗначениеЗаполнено(ТекущаяСтрока.Склад);
				ЕстьОграничение = ПараметрыДействия.СкладТолькоСобственнаяЦеховаяКладовая
					И НЕ ПараметрыОбеспечения[0].ЭтоКладоваяТекущегоПодразделения;
				
				Если ЗаполнитьПоДаннымОбеспечения И НЕ ЕстьОграничение Тогда
					ТекущаяСтрока.Склад = ПараметрыОбеспечения[0].Склад;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		Если ПараметрыДействия.ЗаполнитьОбособленно Тогда
			Если ЗначениеЗаполнено(ПараметрыДействия.Обособленно) Тогда
				ТекущаяСтрока.Обособленно = ПараметрыДействия.Обособленно;
			Иначе
				// На Обособленно не действует параметр ЗаполнятьТолькоПустые т.к. нельзя
				// достоверно установить, что значение не заполнено (значение Ложь может быть установлено осознанно)
				ТекущаяСтрока.Обособленно = ПараметрыОбеспечения[0].Обособленно Или ТекущаяСтрока.Производится;
			КонецЕсли;
		КонецЕсли;
...
// Заполняет склад на основании схемы обеспечения номенклатуры и подразделения документа.
// В параметрах действия передается имя реквизита Склад в вызывающем объекте.
//
Процедура ПроверитьЗаполнитьСкладПроизводства(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если ПараметрыЗаполнения <> Неопределено Тогда
		...
		Для каждого ИмяПоляСклад Из СписокПолей Цикл
			ТекущаяСтрока[ИмяПоляСклад] = ПараметрыЗаполнения[0].Склад;
		КонецЦикла;
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.Характеристика) Тогда
			...
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Номенклатуре ""%1"" с характеристикой ""%2"" уже сопоставлена другая номенклатура поставщика (%3)';
					|en = 'Another vendor part number (%3) is already mapped to the ""%1"" item with the ""%2"" variant'"),
		...
		Иначе
			...
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Номенклатуре ""%1"" уже сопоставлена другая номенклатура поставщика (%2)';
					|en = 'Another vendor part number (%2) is already mapped to the ""%1"" item'"),
				ТекущаяСтрока.Номенклатура,
				ДанныеПоСтроке.КоличествоНоменклатурыПартнера);
...
Процедура ПриДобавленииОбработчиковСтрокКоллекции(ОбработчикиСтрок) Экспорт
	...
	ОбработчикиСтрок.Добавить("ЗаполнитьВладелецИзделияДоступен",
		"ПакетнаяОбработкаТабличнойЧастиСервер.ЗаполнитьВладелецИзделияДоступен");
...
Процедура ДополнитьТекстыЗапросовИсточниковДанных(Действия, КэшированныеЗначения)
	...
	//-- Устарело_Переработка24
	ВнутренняяПереработкаСервер.ДополнитьТекстЗапросаВладелецИзделияДоступен(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	//-- НЕ УТКА
	...
	//++ НЕ УТКА
	ОбеспечениеПроизводства.ДополнитьТекстЗапросаПроверитьЗаполнитьОбеспечениеВЭтапеПроизводстваПакетно(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	ОбеспечениеПроизводства.ДополнитьТекстЗапросаПроверитьЗаполнитьСкладПроизводства(Действия, ОписаниеЗапроса, КэшированныеЗначения);
	//-- НЕ УТКА
	...
	Если КоллекцияСтрок = Неопределено
		Или ТекущаяСтрокаВКоллекции = Неопределено Тогда
		 ВызватьИсключение НСтр("ru = 'Некорректно встроен механизм пакетной обработки строк табличных частей.
										...
										|en = 'The tool for batch processing of table rows is built in incorrectly.
										|The Undefined value is passed in the required parameters.'");
	КонецЕсли;
