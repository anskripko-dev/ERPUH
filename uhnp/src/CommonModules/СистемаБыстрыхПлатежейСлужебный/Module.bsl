...
Функция ПереопределитьСообщениеПользователю(
	...
	ИначеЕсли КодОшибки = КодОшибкиНеверныйФорматЗапроса()
		...
		Возврат НСтр("ru = 'Регистрация кассовой ссылки запрещена. Обратитесь в банк для получения кассового QR-кода.';
					|en = 'QR code payment link registration is prohibited. Contact the bank to receive a cash account QR code.'");
		...
		Если Не ЗначениеЗаполнено(ПараметрыНастройки.НастройкаПодключения) Тогда
			ЗаписатьИнформациюВЖурналРегистрации(
				НСтр("ru = 'Не удалось определить настройку подключения к Системе быстрых платежей.';
					|en = 'Не удалось определить настройку подключения к Системе быстрых платежей.'"),
				Ложь);
