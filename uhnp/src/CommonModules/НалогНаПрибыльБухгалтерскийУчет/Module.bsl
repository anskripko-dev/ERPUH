...
Процедура ЗаписатьОтражениеИнвестиционногоВычета(Проводки, КонтекстРасчета)
	...
	Пока Выборка.Следующий() Цикл
		...
		СодержаниеПризнание  = НСтр("ru = 'Признание отложенного налогового актива';
									|en = 'Признание отложенного налогового актива'", ОбщегоНазначения.КодОсновногоЯзыка());
		ВидДвиженияПризнание = ВидДвиженияБухгалтерии.Дебет;
		СодержаниеПогашение  = НСтр("ru = 'Погашение отложенного налогового актива';
									|en = 'Погашение отложенного налогового актива'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СодержаниеПроводокИзмененияНалогаВДобавочномКапитале(Знач СтавкаБудущегоНалога)
	...
	СодержаниеПроводки = НСтр("ru = 'Изменение налога на прибыль от операций, результат которых не включается в чистую прибыль (убыток)';
								|en = 'Изменение налога на прибыль от операций, результат которых не включается в чистую прибыль (убыток)'",
			Метаданные.ОсновнойЯзык.КодЯзыка);			
	...
	СодержаниеПроводки = НСтр("ru = 'Списание налога на прибыль от операций, результат которых не включается в чистую прибыль (убыток)';
								|en = 'Списание налога на прибыль от операций, результат которых не включается в чистую прибыль (убыток)'",
			Метаданные.ОсновнойЯзык.КодЯзыка);			
	...
	СодержаниеПроводки = НСтр("ru = 'Пересчет налога на прибыль от операций, результат которых не включается в чистую прибыль (убыток) на ставке %1%%';
								|en = 'Пересчет налога на прибыль от операций, результат которых не включается в чистую прибыль (убыток) на ставке %1%%'",
			Метаданные.ОсновнойЯзык.КодЯзыка);			
...
Процедура РассчитатьРасходПоНалогу(ТаблицаРасчета, КонтекстРасчета)
	...
	Для каждого ОбщийРасчет Из ТаблицаРасчета Цикл
		...
		Если ЕстьДанныеИНВ Тогда
			...
			ПорядокРасчета = СтрШаблон(
				НСтр("ru = 'Оборот по дебету счета %1 в корреспонденции со счетом %2';
					|en = 'Оборот по дебету счета %1 в корреспонденции со счетом %2'", Расчет.КодЯзыка),
		...
		Если ЕстьДанныеИНВ Тогда
			ПорядокРасчета = СтрШаблон(
				НСтр("ru = 'Оборот по кредиту счета %1 в корреспонденции со счетом %2 (со знаком ""минус"") + (%3)';
					|en = 'Оборот по кредиту счета %1 в корреспонденции со счетом %2 (со знаком ""минус"") + (%3)'", Расчет.КодЯзыка),
				КодСчета(ПланыСчетов.Хозрасчетный.РасчетыСБюджетом),
				КодСчета(КоррСчетДляРасчетыСБюджетом),
...
Процедура ДобавитьПроводкуИзмененияВариантаНалогообложенияПрибыли(Проводки, КонтекстРасчета, 
	...
	ШаблонПроводки = НСтр("ru = 'Пересчет отложенного налога по разнице ставок %1%%';
							|en = 'Пересчет отложенного налога по разнице ставок %1%%'", Метаданные.ОсновнойЯзык.КодЯзыка);
