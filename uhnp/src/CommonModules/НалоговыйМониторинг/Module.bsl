...
Процедура ЗапросыНалоговогоМониторингаРегистрацияПередЗаписью(Источник, Отказ) Экспорт
	ПланыОбмена.ЗапросыНалоговогоМониторинга.МеханизмРегистрацииОбъектовПередЗаписью(Источник, Отказ);
КонецПроцедуры

Процедура ЗапросыНалоговогоМониторингаРегистрацияДокументаПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	ПланыОбмена.ЗапросыНалоговогоМониторинга.МеханизмРегистрацииОбъектовПередЗаписьюДокумента(Источник, Отказ, РежимЗаписи, РежимПроведения);
КонецПроцедуры

Процедура ЗапросыНалоговогоМониторингаРегистрацияКонстантыПередЗаписью(Источник, Отказ) Экспорт
	// Вставить содержимое обработчика.
КонецПроцедуры

Процедура ЗапросыНалоговогоМониторингаРегистрацияНабораПередЗаписью(Источник, Отказ, Замещение) Экспорт
	ПланыОбмена.ЗапросыНалоговогоМониторинга.МеханизмРегистрацииОбъектовПередЗаписьюРегистра(Источник, Отказ, Замещение);
КонецПроцедуры

Процедура ЗапросыНалоговогоМониторингаРегистрацияУдаленияПередУдалением(Источник, Отказ) Экспорт
	ПланыОбмена.ЗапросыНалоговогоМониторинга.МеханизмРегистрацииОбъектовПередУдалением(Источник, Отказ);
КонецПроцедуры
...
Процедура УдалитьПанельРазделовРекурсивно(КоллекцияИсточник, СоставИзменен)
	...
	ЧислоИсточников = КоллекцияИсточник.Количество();
	
	Для ОбратныйИндекс = 1 По ЧислоИсточников Цикл
		
		ЭлементКоллекции = КоллекцияИсточник.Получить(ЧислоИсточников - ОбратныйИндекс);
		...
		ИначеЕсли (ТипЗнч(ЭлементКоллекции) = Тип("ЭлементНастройкиСоставаИнтерфейсаКлиентскогоПриложения")) И (ЭлементКоллекции.Имя = "ПанельРазделов") Тогда
			КоллекцияИсточник.Удалить(ЧислоИсточников - ОбратныйИндекс);
...
Процедура ЗаполнитьСпискиВыбораВЭлементахУправленияИзДокумента(ЭлементФормы, ИмяДокумента, Макет, Область) Экспорт
	Макет = Документы[ИмяДокумента].ПолучитьМакет(Макет).ПолучитьОбласть(Область);
	Для Инд = 1 По Макет.ВысотаТаблицы Цикл
		Код = Макет.Область(Инд, 1, Инд, 1).Текст;
		Наименование = Макет.Область(Инд, 2, Инд, 2).Текст;
		ЭлементФормы.СписокВыбора.Добавить(Код, ?(ПустаяСтрока(Код), "", Код + " - " + Наименование));
...
Функция ПолучитьСтруктуруНастроекПоУмолчанию(Период, Пользователь) Экспорт
	...
	Запрос.Текст = 
		...
		|	РеквизитыПоУмолчаниюНалоговогоМониторингаСрезПоследних.НачалоПериода КАК НалоговыйПериодДатаНачала,
		|	РеквизитыПоУмолчаниюНалоговогоМониторингаСрезПоследних.ОкончаниеПериода КАК НалоговыйПериодДатаОкончания,
