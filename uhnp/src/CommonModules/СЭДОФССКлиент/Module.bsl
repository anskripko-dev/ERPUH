	...
	Если Контекст.ДанныеФайла = Неопределено Тогда
		Если Контекст.Загружен = Истина Тогда
			Шаблон = НСтр("ru = 'Документ ""%1"" загружен, но файл отказа пустой. Предположительно это ошибка в данных xml-сообщения (на стороне СФР). Рекомендуется обратиться в техническую поддержку.';
							|en = 'The ""%1"" document is imported, but the refusal file is empty. This might be an error in the XML message data (on the Social Insurance Fund side). We recommend that you contact technical support.'");
		Иначе
			Шаблон = НСтр("ru = 'Не найден файл ""%1"". Возможно документ еще не получен из СЭДО.';
							|en = 'The ""%1"" file is not found. The document might not have been received from the EDI yet.'");
		КонецЕсли;
...
Процедура ПредложитьПодтвердитьПолучение(Контекст) Экспорт
	...
	Иначе
		...
		ТекстВопроса = НСтр("ru = 'Рекомендуется подтвердить получение документа ""%1"".';
							|en = 'We recommend that you confirm the receipt of the ""%1"" document.'");
