...
Функция СоздатьПарольВнешнегоПользователя() Экспорт
	НастройкиВхода = ПользователиСлужебный.НастройкиВхода().ВнешниеПользователи;
	...
	ПараметрыПароля = ПараметрыПароля(МинимальнаяДлинаПароля, Истина);
	
	Возврат СоздатьПароль(ПараметрыПароля);
КонецФункции
...
Функция ПолучитьВидСобытияПоОбъекту(Ссылка)  Экспорт
	...
	Если ТипСсылки = Тип("ДокументСсылка.АккредитацияПоставщика") Тогда
		...
		ИначеЕсли РешениеПоДокументу = Перечисления.ВидыРешенийПоДокументуАккредитации.РешениеНеПринято Тогда
			Возврат Справочники.ВидыСобытийОповещений.ОповещениеОбОтправкеНаИсправлениеАккредитации;
		КонецЕсли;
...
Процедура ОтправитьНаПроверку(Ответственный,Документ) Экспорт
	...
	Попытка 
		...
		Если ТипЗНЧ(Документ) = ТИП("ДокументСсылка.АккредитацияПоставщика") Тогда
		Оповещение = Справочники.ВидыСобытийОповещений.ОповещениеОНеобходимостиПроверкиАккредитацииПоставщика; 
		ТекстОповещений	= "Необходимо проверить аккредитацию поставщика " + Документ; 
		Иначе
		Оповещение = Справочники.ВидыСобытийОповещений.ОповещениеОНеобходимостиПроверкиКвалификацииПоставщика;
		ТекстОповещений = "Необходимо проверить квалификацию поставщика по документу " + Документ;
		КонецЕсли;
		АдресЭП = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(Ответственный,Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты);
		АккредитацияПоставщиковУХ.СоздатьИОтправитьПисьмо(ТекстОповещений, Строка(Оповещение), АдресЭП);
		...
		Если АдресЭП <> "" Тогда
			...
			Движение.Получатель = Ответственный; 
			Движение.Оповещение = Оповещение;
			Движение.АдресОтправки = АдресЭП;
			Движение.ТекстОповещений = ТекстОповещений;   
			ДанныеОбъекта = СериализоватьОбъект(Документ);
			СписокРассылки = Новый Массив;
			СписокРассылки.Добавить(Ответственный);
			
			Движение.ВерсияДокумента = Новый ХранилищеЗначения(Документ, Новый СжатиеДанных(9));
			НаборЗаписей.Записать(Истина);    
		КонецЕсли;
		СтруктураДополнительныхПараметров = Новый Структура;
		ЗадачаОбъект = Справочники.Задачи.СоздатьЭлемент();
		ЗадачаОбъект.СвязанныйОбъект = Документ;
		ЗадачаОбъект.Записать();
				
		ПараметрыОповещения = МодульУправленияОповещениямиУХ.ПараметрыОповещения();
		ПараметрыОповещения.Вставить("КатегорияОповещений", Оповещение.КатегорияСобытия);
		ПараметрыОповещения.Вставить("ВидОповещения", Оповещение);
		ПараметрыОповещения.Вставить("Пользователь", Ответственный);
		ПараметрыОповещения.Вставить("Сообщение", ТекстОповещений);
		ПараметрыОповещения.Вставить("ТекстПочтовогоСообщения", ТекстОповещений);
		
		МодульУправленияОповещениямиУХ.СоздатьОповещениеПользователю(ПараметрыОповещения);
...
Функция ПолучитьВсеТребованияПоНоменклатуре(ДатаСреза,
	...
	Запрос.Текст = 
		...
		|			&ДатаСреза,
		|			Организация = &Организация
		|				И (Номенклатура В ИЕРАРХИИ (&мГруппИНоменклатуры)
		|  ИЛИ  Номенклатура.ТоварнаяКатегория В  (&мГруппИНоменклатуры) ИЛИ Номенклатура В  (&мГруппИНоменклатуры))
		|	
		|) КАК ТребованияКвалификацииКНоменклатуреПоставщиковСрезПоследних
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
