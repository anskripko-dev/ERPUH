...
Процедура ЗаполнитьМаркируемуюПродукциюДокумента(Контекст, ВидМаркируемойПродукции, ТаблицаМаркируемойПродукции, Сценарий = 0) Экспорт
	...
	ИначеЕсли ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(Контекст, "ПроизводствоБезЗаказа") Тогда
		РезультатЗапроса = ЗапросМаркируемойПродукцииПроизводствоБезЗаказа(Контекст, ВидМаркируемойПродукции, Сценарий);
	ИначеЕсли ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(Контекст, "ДвижениеПродукцииИМатериалов") Тогда
		РезультатЗапроса = ЗапросМаркируемойПродукцииДвижениеПродукцииИМатериалов(Контекст, ВидМаркируемойПродукции);
	//-- НЕ УТ

	//++ НЕ УТКА
	ИначеЕсли ИнтеграцияИСУТКлиентСервер.ЭтоДокументПоНаименованию(Контекст, "ЭтапПроизводства2_2") Тогда
		РезультатЗапроса = ЗапросМаркируемойПродукцииЭтапПроизводства(Контекст, ВидМаркируемойПродукции, Сценарий);
	//-- НЕ УТКА
	Иначе
		Возврат;
	КонецЕсли;
...
Функция ЗапросМаркируемойПродукцииПроизводствоБезЗаказа(Документ, ВидМаркируемойПродукции, Сценарий)
...
Функция ЗапросМаркируемойПродукцииДвижениеПродукцииИМатериалов(Документ, ВидМаркируемойПродукции)
	...
	|ИЗ
	|	ВТ_Товары КАК Товары
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ДвижениеПродукцииИМатериалов.Серии КАК Серии
	|		ПО Товары.Документ = Серии.Ссылка
	|			И Товары.Номенклатура = Серии.Номенклатура
	|			И Товары.Характеристика = Серии.Характеристика
	|			И Товары.Назначение = Серии.Назначение
...
//++ НЕ УТКА
Функция ЗапросМаркируемойПродукцииЭтапПроизводства(Документ, ВидМаркируемойПродукции, Сценарий)
