...
Процедура ОбработатьЭлектронноеПредставлениеРегламентированногоОтчета(
	...
	Если ОсновнойФайл <> Неопределено Тогда
			...
			Исключение
				ТекстСообщения = СтрШаблон(
					НСтр("ru = 'Файл ""%1"" не является файлом выгрузки в формате XML.';
						|en = 'Файл ""%1"" не является файлом выгрузки в формате XML.'"), ОсновнойФайл.ИмяФайла);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
				
				ОписаниеОшибки = ИнформацияОбОшибке().Описание;
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Сверки данных учета с данными налогового органа по ЕНС';
												|en = 'Сверки данных учета с данными налогового органа по ЕНС'"),
		...
		Иначе
			...
			Исключение
				ТекстСообщения = СтрШаблон(
					НСтр("ru = 'Файл ""%1"" не является файлом выгрузки в текстовом формате.';
						|en = 'Файл ""%1"" не является файлом выгрузки в текстовом формате.'"), ОсновнойФайл.ИмяФайла);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
...
Функция ПредставлениеДокумента(Ссылка, Наименование, ПредставлениеПериода, ПредставлениеВида)
	...
	Если ЗначениеЗаполнено(ПредставлениеПериода) Тогда
		ПредставлениеДок = ПредставлениеДок + СтрШаблон(НСтр("ru = ' за %1';
															|en = ' за %1'"), НРег(ПредставлениеПериода));
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПредставлениеВида) Тогда
		ПредставлениеДок = ПредставлениеДок + СтрШаблон(НСтр("ru = ' (%1)';
															|en = ' (%1)'"),
			?(СтрНачинаетсяС(ПредставлениеВида, "-"), НСтр("ru = 'П';
															|en = 'П'"), ПредставлениеВида));
	КонецЕсли;
