...
Процедура ЗаменитьТекстЗапроса_ЭтапыОплатыВызовСервера_ДатыПоКалендарю(Запрос)Экспорт 
	...
	Если Запрос.Текст <> ТекстЗапросаЭталон Тогда   
		
		ВызватьИсключение НСтр("ru = 'Общий модуль: ЭтапыОплатыВызовСервера
									|Функция: ДатыПоКалендарю
...
Процедура ЗаменитьТекстЗапроса_ЭтапыОплатыВызовСервера_СдвигМеждуДатамиПоКалендарю(Запрос)Экспорт 
	...
	Если Запрос.Текст <> ТекстЗапросаЭталон Тогда   
		...
		ВызватьИсключение НСтр("ru = 'Общий модуль: ЭтапыОплатыВызовСервера
									|Функция: СдвигМеждуДатамиПоКалендарю
									|Ошибка: После обновления ERP изменился текст запроса'");
...
Процедура МассивКалендарейВТекст(ГрафикРаботы)Экспорт
	
	Если ТипЗнч(ГрафикРаботы) = Тип("Массив") Тогда
		ГрафикРаботы = СтрСоединить(ГрафикРаботы, ",");
	КонецЕсли;	
