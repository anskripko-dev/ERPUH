...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	Исключения = ТипыДокументовИсключенияДляПодключаемойКоманды();
...
Процедура ВыполнитьЗагрузкуРезультатовКонтроля(Форма) Экспорт
	...
	ПараметрыКонтроля.Вставить("РеквизитыКонтроляВДокументе", ТипыДокументовИсключенияДляПодключаемойКоманды());
...
Процедура ВыполнитьКонтрольДокументаПередПроведением(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	СинхронизироватьПометкуУдаленияКлючейКонтроля(Источник.Ссылка, Источник.ПометкаУдаления);
	СинхронизироватьДокументыКорректировкиПланов(Источник.Ссылка, Источник, РежимЗаписи);
	...
	ВидыБюджетныхКонтролей = Новый Массив;
	ВидыБюджетныхКонтролей.Добавить(ПланыВидовХарактеристик.ВидыКонтроляДокументов.КонтрольБюджетныхЛимитов); 
	ВидыБюджетныхКонтролей.Добавить(ПланыВидовХарактеристик.ВидыКонтроляДокументов.КонтрольБюджетныхРезервов);
	...
	Для Каждого Строка Из НарушенныеКонтроли Цикл
		...
		КонтролиВидаКонтроля = КонтролиДокумента.Скопировать(СтруктураПоиска, "КлючКонтроля").ВыгрузитьКолонку("КлючКонтроля");
		
		БлокироватьПроведение = ОписаниеКонтроля.Объект.БлокироватьПроведение(КонтролиВидаКонтроля);
		Если БлокироватьПроведение Тогда  									

			ШаблонСообщения = НСтр("ru = 'Нарушен %1. Проведение документа %2 блокировано.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Строка.ВидКонтроля, Источник.Ссылка);
			
			Если Не ВидыБюджетныхКонтролей.Найти(Строка.ВидКонтроля) = Неопределено Тогда
				ШаблонСообщения = НСтр("ru = 'Для увеличения доступного лимита введите документ ""%1"" либо ""%2"".'");  
				ТекстСообщения = ТекстСообщения + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
								Метаданные.Документы.ЗаявкаНаКорректировкуПланов.Представление(), Метаданные.Документы.КорректировкаПланов.Представление());  
			КонецЕсли;
			
			Если Сообщения.Найти(ТекстСообщения) = Неопределено Тогда
...
Процедура КонтролируемыеДокументыПриЗаписиОбработчик(Источник) Экспорт
	...
	Если Не Источник.ДополнительныеСвойства.Свойство("КонтрольДокумента") Тогда   
		Возврат;
	КонецЕсли;
	...
	СвернутьРезультатыКонтроля(КонтрольДокумента.РезультатыКонтроля);
	...
	Для Каждого Проверка Из КонтрольУХПовтИсп.ЗарегистрированныеПроверки(ИмяДокумента) Цикл
		ВидКонтроля = Проверка.ВидКонтроля;
		Если КонтрольДокумента.РезультатыКонтроля.Найти(ВидКонтроля, "ВидКонтроля") = Неопределено Тогда
			НаборЗаписей = РегистрыСведений.РезультатыКонтроляДокументовПоВидамКонтроля.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.Документ.Установить(Источник.Ссылка);
			НаборЗаписей.Отбор.ВидКонтроля.Установить(ВидКонтроля);
			НаборЗаписей.Записать(Истина);
		КонецЕсли;
	КонецЦикла;
...
Процедура КлючКонтроляПриУдалении(КлючКонтроляСсылка) Экспорт
	
	УдалитьРезультатыКонтроляДокументов(КлючКонтроляСсылка);
...
Процедура ВывестиПредупрежденияЗаявки(Форма) Экспорт
	...
	ДанныеПредупреждений = ЗаявкиНаОперации.ПолучитьПредупрежденияПоЗаявке(Объект);
...
Процедура ПодготовитьФормуНаСервере(Форма, РодительДляЭлементовКонтроля) Экспорт
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
		...
		ДобавитьРеквизитыСтраницыКонтроль(Форма, Реквизиты, НЕ ДокументИмеетТабЧастьРезультатыКонтроля(Форма.Объект.Ссылка));
		Форма.ИзменитьРеквизиты(Реквизиты);
		
		ДобавитьСтраницуКонтроляДокумента(Форма, РодительДляЭлементовКонтроля);
...
Процедура ДобавитьРеквизитыСтраницыКонтроль(Форма, Реквизиты, ДобавитьРезультатыКонтроляПоВидамКонтроля = Ложь) Экспорт
	
	// Заменитель Объект.ЕстьПревышениеЛимитов
	Если НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ЕстьНарушения") Тогда
	...
	Если НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "НадписьПредупреждения") Тогда
	...
	ИмяРеквизита = РеквизитыЭлементы.ИмяРеквизита;
	Если НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "НадписьПредупреждения") Тогда
		Реквизиты.Добавить(Новый РеквизитФормы(ИмяРеквизита, Новый ОписаниеТипов("ТаблицаЗначений")));
		Реквизиты.Добавить(Новый РеквизитФормы("ВидКонтроля", Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ВидыКонтроляДокументов"), ИмяРеквизита));
		Реквизиты.Добавить(Новый РеквизитФормы("ТребуетсяПроверка", Новый ОписаниеТипов("Булево"), ИмяРеквизита));
		Реквизиты.Добавить(Новый РеквизитФормы("КонтрольНарушен", Новый ОписаниеТипов("Булево"), ИмяРеквизита));
		Реквизиты.Добавить(Новый РеквизитФормы("ВремяПроверки", ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя), ИмяРеквизита));
		Реквизиты.Добавить(Новый РеквизитФормы("ИндексКартинки", ОбщегоНазначения.ОписаниеТипаЧисло(1,0), ИмяРеквизита, НСтр("ru = 'Индекс картинки'")));
		Реквизиты.Добавить(Новый РеквизитФормы("ДополнительнаяИнформация", ОбщегоНазначения.ОписаниеТипаСтрока(0), ИмяРеквизита, НСтр("ru = 'Дополнительная информация'")));
		Реквизиты.Добавить(Новый РеквизитФормы("РезультатСтрока", ОбщегоНазначения.ОписаниеТипаСтрока(0), ИмяРеквизита, НСтр("ru = 'Результат'")));
		Реквизиты.Добавить(Новый РеквизитФормы("АдресРезультата", ОбщегоНазначения.ОписаниеТипаСтрока(100), ИмяРеквизита));
	КонецЕсли;
...
Процедура ДобавитьСтраницуКонтроляДокумента(Форма, Размещение) экспорт
	...
	РеквизитыЭлементы = КонтрольУХКлиентСервер.ИменаРеквизитовЭлементовКонтроля();
	Если Форма.Элементы.Найти(РеквизитыЭлементы.ИмяСтраницы) <> неопределено Тогда
	...
	ТипКонтролируемогоОбъекта = ТипЗнч(Форма.Объект.Ссылка);
	
	Исключения = ТипыДокументовИсключенияДляПодключаемойКоманды();
	Если Исключения.Найти(ТипКонтролируемогоОбъекта) <> неопределено
		И КонтрольУХВстраивание.СоздаватьКомандуКонтрольДокумента(ТипКонтролируемогоОбъекта) Тогда
...
Функция ПолучитьПредставлениеАналитикСтатьиБюджета(Данные) Экспорт
	
	Результат = Новый Массив;
	
	Если ТипЗнч(Данные.СтатьяБюджета) = Тип("СправочникСсылка.ГруппыСтатейБюджета") Тогда
		
	ИначеЕсли ЗначениеЗаполнено(Данные.СтатьяБюджета) Тогда
...
Функция ТипыДокументовИсключенияДляПодключаемойКоманды() Экспорт
	...
	Результат = Новый Массив;
	Результат.Добавить(Тип("ДокументСсылка.УдалитьОперативныйПлан"));
	Результат.Добавить(Тип("ДокументСсылка.ОперативныйПлан"));
	Результат.Добавить(Тип("ДокументСсылка.Резервирование"));
	Если НЕ ОбщегоНазначения.ПодсистемаСуществует("ПереопределяемыеОбъектыУП") Тогда
		ТипДокументаПлана = "ДокументСсылка.ЗаявкаНаОбеспечение";
		Результат.Добавить(Тип(ТипДокументаПлана)); 
	Иначе              
		ТипДокументаПлана = "ДокументСсылка.ПланЗакупок";
		Результат.Добавить(Тип(ТипДокументаПлана));
	КонецЕсли;	
...
//
Функция ПолучитьИсточникиДокумента(Объект, ПараметрыКонтроля = неопределено) Экспорт
	...
	Попытка
		Источники = МенеджерОбъекта.ИсточникиДокумента(Объект, ПараметрыКонтроля);
	Исключение
...
//
Процедура ЗаполнитьПараметрыКонтроляДокументаИзДанныхФормы(Форма, ИменаРеквизитовФормы) Экспорт
	
	ИмяРеквизита = КонтрольУХКлиентСервер.ИмяРеквизитаФормы_ДополнительныеПараметрыКонтроля();
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, ИмяРеквизита) Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыКонтроля = Новый Структура(ИменаРеквизитовФормы);
	
	ДобавляемыеРеквизиты = Новый Массив;
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(ИмяРеквизита, Новый ОписаниеТипов));
	Форма.ИзменитьРеквизиты(ДобавляемыеРеквизиты);
	
	Форма[ИмяРеквизита] = Новый ФиксированнаяСтруктура(ПараметрыКонтроля);
	
КонецПроцедуры

// Выполняет интерактивный контроль источников документа.
...
// Выпоняется контроль источников документа и возвращается результат контроля. 
Функция ВыполнитьКонтрольИВернутьРезультат(ПараметрыКонтроля) Экспорт
	...
	Результат.Колонки.Добавить("ВидКонтроля", Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ВидыКонтроляДокументов"));
	Результат.Колонки.Добавить("ТребуетсяПроверка", Новый ОписаниеТипов("Булево"));
	Результат.Колонки.Добавить("КонтрольНарушен", Новый ОписаниеТипов("Булево"));
	Результат.Колонки.Добавить("ДанныеКонтроля");
		...
		Если СтрокаРезультат.ТребуетсяПроверка Тогда
			...
			Если СтрокаРезультат.ТребуетсяПроверка Тогда
				...
				Иначе
					СтрокаРезультат.КонтрольНарушен = Проверка.Объект.ВыполнитьКонтроль(ПараметрыКонтроля, ДанныеДляКонтроля);
				КонецЕсли;
				СтрокаРезультат.ДанныеКонтроля = ДанныеДляКонтроля;
			КонецЕсли;
...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт  
	...
	ВключитьОповещение = ОповещатьОНеобходимостиОбновленияПараметровЛимитирования(); 
	
	Если Не ВключитьОповещение Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыЛимитированияВидовБюджетов = Новый Массив;	
	
	ДобавитьВидБюджетаВОповещение(ПараметрыЛимитированияВидовБюджетов, ПланыВидовХарактеристик.ВидыБюджетов.БюджетДвиженияДенежныхСредств);
	ДобавитьВидБюджетаВОповещение(ПараметрыЛимитированияВидовБюджетов, ПланыВидовХарактеристик.ВидыБюджетов.БюджетДоходовИРасходов); 
	...
	ПараметрыРаботыКлиента.Вставить("ПараметрыЛимитированияВидовБюджетов", ПараметрыЛимитированияВидовБюджетов);  
	
	Параметры.Вставить("ПланированиеИКонтроль", Новый ФиксированнаяСтруктура(ПараметрыРаботыКлиента));  
	
КонецПроцедуры 

Функция ДокументИмеетТабЧастьРезультатыКонтроля(КонтролируемыйДокумент) Экспорт
	
	Возврат КонтролируемыйДокумент.Метаданные().ТабличныеЧасти.Найти("РезультатыКонтроля") <> неопределено;
	
КонецФункции

Функция ПолучитьРезультатыКонтроля(КонтролируемыйДокумент) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	МАКСИМУМ(РезультатыКонтроляДокументовПоВидамКонтроля.Период) КАК ВремяПроверки,
	|	РезультатыКонтроляДокументовПоВидамКонтроля.ВидКонтроля КАК ВидКонтроля
	|ПОМЕСТИТЬ ВТ_АктуальныеПериоды
	|ИЗ
	|	РегистрСведений.РезультатыКонтроляДокументовПоВидамКонтроля.СрезПоследних(, Документ = &КонтролируемыйДокумент) КАК РезультатыКонтроляДокументовПоВидамКонтроля
	|
	|СГРУППИРОВАТЬ ПО
	|	РезультатыКонтроляДокументовПоВидамКонтроля.ВидКонтроля
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ИЗ
	|	РегистрСведений.РезультатыКонтроляДокументовПоВидамКонтроля.СрезПоследних(, Документ = &КонтролируемыйДокумент) КАК РезультатыКонтроляДокументовПоВидамКонтроля
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_АктуальныеПериоды КАК ВТ_АктуальныеПериоды
	|		ПО РезультатыКонтроляДокументовПоВидамКонтроля.Период = ВТ_АктуальныеПериоды.ВремяПроверки
	|			И РезультатыКонтроляДокументовПоВидамКонтроля.ВидКонтроля = ВТ_АктуальныеПериоды.ВидКонтроля";  
...
Функция ВерсииСоглашений(Основание, Регистраторы = Неопределено) Экспорт
	
	Если Регистраторы = Неопределено Тогда
		Регистраторы = Новый Массив;
	КонецЕсли;
	
	ЭтоВерсияСоглашения = ДоговорыКонтрагентовВстраиваниеУХКлиентСервер.ЭтоВерсияСоглашения(Основание);
	Если Не ЗначениеЗаполнено(Основание) ИЛИ НЕ ЭтоВерсияСоглашения Тогда
		Возврат Регистраторы;
	КонецЕсли;
	
	Регистраторы.Добавить(Основание);
	ВерсииСоглашений(Основание.ДокументОснование, Регистраторы);
...
Процедура ЗагрузитьРезультатыКонтроля(ПараметрыКонтроля)
	...
	СтрокаКонтрольЛимита = неопределено;
	СтрокаКонтрольРезерва = неопределено;
	
	ВсеДанныеКонтроля = Новый Соответствие;
	...
	КонтролиДокумента = КонтрольУХПовтИсп.ЗарегистрированныеПроверки(ИмяДокумента);
	Для Каждого Проверка Из КонтролиДокумента Цикл
		...
		Если СтрокаРезультат.ТребуетсяПроверка Тогда
			...
			//
			//СтрокаРезультат.АдресРезультата = ПоместитьВоВременноеХранилище(ДанныеДляКонтроля, ПараметрыКонтроля.УИДФормы);
			ВсеДанныеКонтроля.Вставить(Проверка.ВидКонтроля, ДанныеДляКонтроля);
	...
	// 
	Для Каждого Проверка Из КонтролиДокумента Цикл
		Если Проверка.Свойство("ПриЗагрузке") И Проверка.ПриЗагрузке = Истина Тогда
			Проверка.Объект.ПриЗагрукзеРезультатовКонтроля(ВсеДанныеКонтроля);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого СтрокаРезультат Из ПараметрыКонтроля.Результат Цикл
		ДанныеДляКонтроля = ВсеДанныеКонтроля[СтрокаРезультат.ВидКонтроля];
		Если ДанныеДляКонтроля <> неопределено Тогда
			СтрокаРезультат.АдресРезультата = ПоместитьВоВременноеХранилище(ДанныеДляКонтроля, ПараметрыКонтроля.УИДФормы);
		КонецЕсли;
	КонецЦикла;
...
Процедура СинхронизироватьПометкуУдаленияКлючейКонтроля(ДокументПланированияСсылка, ПометкаУдаления)

	УстановитьПривилегированныйРежим(Истина);
	// Синхронизировать пометку удаления документа и объекта планирования
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Объект", ДокументПланированияСсылка);
	Запрос.УстановитьПараметр("ПометкаУдаления", ПометкаУдаления);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Ключи.Ссылка КАК Ссылка,
	|	Ключи.ПометкаУдаления КАК ПометкаУдаления
	|ИЗ
	|	Справочник.КлючиКонтроляБюджетныхЛимитовРезервов КАК Ключи
	|ГДЕ                   
	|	Ключи.ДокументРезервирования = &Объект
	|	И Ключи.ПометкаУдаления <> &ПометкаУдаления";
	
	Результат = Запрос.Выполнить();
	
	Блокировка = Новый БлокировкаДанных;
	Элемент = Блокировка.Добавить("Справочник.КлючиКонтроляБюджетныхЛимитовРезервов");
...
Процедура СинхронизироватьДокументыКорректировкиПланов(ДокументСсылка, ДокументОбъект, РежимЗаписи)

	УстановитьПривилегированныйРежим(Истина);  
		
	ТипыДокументов = Новый Массив;
	ТипыДокументов.Добавить(Тип("ДокументСсылка.КорректировкаПланов"));
    ТипыДокументов.Добавить(Тип("ДокументСсылка.ЗаявкаНаКорректировкуПланов"));  
	
	Если Не ТипыДокументов.Найти(ТипЗнч(ДокументСсылка)) = Неопределено Тогда
		Возврат;
	КонецЕсли;   
	...
	ЭтоПометкаНаУдаление = ДокументОбъект.ПометкаУдаления 
						 И (Не ДокументСсылка.ПометкаУдаления = ДокументОбъект.ПометкаУдаления); 
		
	ЭтоОтменаПроведения = (РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения);	
	
	Если Не ЭтоПометкаНаУдаление И Не ЭтоОтменаПроведения Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЗаявкаНаКорректировкуПланов.Ссылка КАК ЗаявкаНаКорректировку
	|ПОМЕСТИТЬ ВТ_Заявки
	|ИЗ
	|	Документ.ЗаявкаНаКорректировкуПланов КАК ЗаявкаНаКорректировкуПланов
	|ГДЕ
	|	ЗаявкаНаКорректировкуПланов.ДокументОснование = &ДокументОснование
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КорректировкаПланов.Ссылка КАК ДокументКорректировки,
	|	КорректировкаПланов.ПометкаУдаления КАК ПометкаУдаления,
	|	КорректировкаПланов.Проведен КАК Проведен
	|ПОМЕСТИТЬ ВТ_Документы
	|ИЗ
	|	Документ.КорректировкаПланов КАК КорректировкаПланов
	|ГДЕ
	|	КорректировкаПланов.ДокументОснование В
	|			(ВЫБРАТЬ
	|				ВТ_Заявки.ЗаявкаНаКорректировку КАК ДокументОснование
	|			ИЗ
	...
	|
	|ВЫБРАТЬ
	|	ВТ_Заявки.ЗаявкаНаКорректировку,
	|	ВТ_Заявки.ЗаявкаНаКорректировку.ПометкаУдаления,
	|	ВТ_Заявки.ЗаявкаНаКорректировку.Проведен
	|ИЗ
	|	ВТ_Заявки КАК ВТ_Заявки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_Документы.ДокументКорректировки КАК ДокументКорректировки,
	|	ВТ_Документы.ПометкаУдаления КАК ПометкаУдаления,
	|	ВТ_Документы.Проведен КАК Проведен
	|ИЗ
	|	ВТ_Документы КАК ВТ_Документы
	|ГДЕ
	|	ВЫБОР
	|			КОГДА &ЭтоОтменаПроведения
	|				ТОГДА ВТ_Документы.Проведен
	|			КОГДА &ЭтоПометкаНаУдаление
	|				ТОГДА НЕ ВТ_Документы.ПометкаУдаления
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ";
	...
	Запрос.УстановитьПараметр("ЭтоПометкаНаУдаление", 	ЭтоПометкаНаУдаление);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ТекстСообщения = "";
		Попытка
			ДокументОбъектКорректировка = ВыборкаДетальныеЗаписи.ДокументКорректировки.ПолучитьОбъект();  
			ДокументОбъектКорректировка.ДополнительныеСвойства.Вставить("ОтключитьКонтрольУХ");
			
			Если ЭтоПометкаНаУдаление Тогда
				ДокументОбъектКорректировка.УстановитьПометкуУдаления(Истина); 
				ШаблонСообщения = Нстр("ru='Помечен на удаление документ: %1'");
				ТекстСообщения = СтрШаблон(ШаблонСообщения, ВыборкаДетальныеЗаписи.ДокументКорректировки);
			ИначеЕсли ЭтоОтменаПроведения Тогда 
				ДокументОбъектКорректировка.Записать(РежимЗаписиДокумента.ОтменаПроведения);
				ШаблонСообщения = Нстр("ru='Отмена проведения документа: %1'");
				ТекстСообщения = СтрШаблон(ШаблонСообщения, ВыборкаДетальныеЗаписи.ДокументКорректировки);  
			КонецЕсли; 
		...
		Исключение     
			...
			ШаблонСообщения = Нстр("ru='Ошибка при %1 документа %2: %3'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ?(ЭтоПометкаНаУдаление, 
										Нстр("ru='пометке на удаление'"), Нстр("ru='отмене проведения'")), 
										ВыборкаДетальныеЗаписи.ДокументКорректировки, КраткоеПредставление);
...
Процедура УдалитьРезультатыКонтроляДокументов(КлючКонтроляСсылка)
	
	УстановитьПривилегированныйРежим(Истина);
	// Синхронизировать пометку удаления документа и объекта планирования
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("КлючКонтроляСсылка", КлючКонтроляСсылка);
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	РезультатыКонтроляДокументовПоВидамКонтроля.КлючКонтроля КАК КлючКонтроля
	|ИЗ
	...
	|ГДЕ
	|	РезультатыКонтроляДокументовПоВидамКонтроля.КлючКонтроля = &КлючКонтроляСсылка";
	...
	Блокировка = Новый БлокировкаДанных;
	Элемент = Блокировка.Добавить("РегистрСведений.РезультатыКонтроляДокументовПоВидамКонтроля");
	Элемент.Режим = РежимБлокировкиДанных.Исключительный;
	Элемент.УстановитьЗначение("КлючКонтроля", КлючКонтроляСсылка);
	Блокировка.Заблокировать();	
	
	НаборЗаписей = РегистрыСведений.РезультатыКонтроляДокументовПоВидамКонтроля.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.КлючКонтроля.Установить(КлючКонтроляСсылка);
...
//
Функция ОповещатьОНеобходимостиОбновленияПараметровЛимитирования()  
	...
	ИначеЕсли Не ПравоДоступа("Изменение", Метаданные.РегистрыСведений.ПараметрыЛимитирования) Тогда
		Возврат Ложь; // Пользователь не имеет доступа к регистру сведений.
	КонецЕсли;
	
	ВключитьОповещение = Истина;
	КонтрольУХПереопределяемый.ПриОпределенииНеобходимостиПоказаПредупрежденияОбИзмененииПериодичности(ВключитьОповещение);
	
	Возврат ВключитьОповещение;
	
КонецФункции  

Процедура ДобавитьВидБюджетаВОповещение(ПараметрыЛимитированияВидовБюджетов, ВидБюджета)
	...
	ПериодичностьЛимитирования = КонтрольЛимитовУХ.ПериодичностьЛимитирования(ВидБюджета);  
	...
	Если ИндексПериодичностьОП > ИндексПериодичностьЛимитирования Тогда   				
		ПараметрыЛимитированияВидовБюджетов.Добавить(ВидБюджета);		
	КонецЕсли;
	
КонецПроцедуры

Процедура СвернутьРезультатыКонтроля(РезультатыКонтроля)
	
	Если РезультатыКонтроля.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	МетаданныеРС = Метаданные.РегистрыСведений.РезультатыКонтроляДокументовПоВидамКонтроля;     
	
	СоставКолонок = Новый Массив; 
	СоставКолонок.Добавить("Период");
	
	КолонкиГруппировок = "Период,";
	Для Каждого Измерение Из МетаданныеРС.Измерения Цикл
		КолонкиГруппировок = КолонкиГруппировок + Измерение.Имя + ",";
		СоставКолонок.Добавить(Измерение.Имя);
	КонецЦикла;   
	
	КолонкиСуммирования = "";
	Для Каждого Ресурс Из МетаданныеРС.Ресурсы Цикл
		КолонкиСуммирования = КолонкиСуммирования + Ресурс.Имя + ",";
		СоставКолонок.Добавить(Ресурс.Имя);
	КонецЦикла;
	
	ТаблицаСоответствует = Истина;
	Для Каждого ЭлементКоллекции Из СоставКолонок Цикл
		Если РезультатыКонтроля.Колонки.Найти(ЭлементКоллекции) = Неопределено Тогда
			ТаблицаСоответствует = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла; 
	
	Если Не ТаблицаСоответствует Тогда
		Возврат;
	КонецЕсли;  
	
	РезультатыКонтроля.Свернуть(КолонкиГруппировок, КолонкиСуммирования);      
