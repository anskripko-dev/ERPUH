...
Функция АнализСебестоимостиВыпущеннойПродукции_ТекстЗапросаМатериальные() Экспорт
	ТекстЗапроса = "
		...
		|	И СебестоимостьТоваров.Период МЕЖДУ &НачалоПериода И &КонецПериода ИЛИ &КонецПериода = ДАТАВРЕМЯ(1,1,1)
		|{ГДЕ
		|	СебестоимостьТоваров.Организация.*
		|}
		|
		...
		|		И РасчетСебестоимости.Ссылка.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ПустаяСсылка)
		...
		|	КОНЕЦ КАК ПостояннаяРазница,
		// При среднескользящей движения делаются разными регистраторами, поэтому регистратор дял связи наборов данных нужно очищать.
		|	ВЫБОР
		|		КОГДА ПериодыДействияСреднескользящей.Организация ЕСТЬ NULL
		|			ТОГДА ЗатратыНЗП.Регистратор
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК Регистратор
		|ИЗ
		...
		|		ПО ПериодыДействияСреднескользящей.Организация = ЗатратыНЗП.Организация
		|		И ПериодыДействияСреднескользящей.ПериодДействия = НАЧАЛОПЕРИОДА(ЗатратыНЗП.Период, МЕСЯЦ)
