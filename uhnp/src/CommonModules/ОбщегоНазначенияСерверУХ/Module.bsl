...
Функция ВернутьТаблицуДоступныхПериодов() Экспорт
	...
	Для Каждого Элемент Из Перечисления.Периодичность Цикл
		...
		НоваяСтрока.ДатаПериода   = КэшируемыеПроцедурыУХ.ВернутьОтносительнуюДату(ТекущаяДата, Элемент, -1);
...
// Заменяет текст в скобках на уникальные идентификаторы.
// Возвращает структуру, содержащую элементы "СтрокаРезультат", "СписокЗамен".
// Замененный текст помещается в элемент структуры "СписокЗамен".
Функция ЗаменитьТекстВСкобках(Знач ИсходнаяСтрока, СкобкаОткрывающая, СкобкаЗакрывающая) Экспорт
	
	СтрокаРезультат = ИсходнаяСтрока;
	СписокЗамен = Новый СписокЗначений;
	
	ПозицияОткрывающая = СтрНайти(СтрокаРезультат, СкобкаОткрывающая);
	
	Пока ПозицияОткрывающая > 0 Цикл
		
		ПозицияЗакрывающая = СтрНайти(СтрокаРезультат, СкобкаЗакрывающая,, ПозицияОткрывающая);
		
		Если ПозицияЗакрывающая > 0 Тогда
			УИД = Новый УникальныйИдентификатор();
			УИДСтрока = Строка(УИД);
			
			ТекстВСкобках = Сред(СтрокаРезультат, ПозицияОткрывающая + 1, ПозицияЗакрывающая - ПозицияОткрывающая - 1);
			
			СписокЗамен.Добавить(УИДСтрока, ТекстВСкобках);
			СтрокаРезультат = СтрЗаменить(СтрокаРезультат, ТекстВСкобках, УИДСтрока);
			
			ПозицияЗакрывающая = СтрНайти(СтрокаРезультат, СкобкаЗакрывающая,, ПозицияОткрывающая); 
			ПозицияОткрывающая = СтрНайти(СтрокаРезультат, СкобкаОткрывающая,, ПозицияЗакрывающая);
		Иначе
			Результат = Новый Структура("СтрокаРезультат, СписокЗамен", СтрокаРезультат, СписокЗамен);
			Возврат Результат;
		КонецЕсли;
