...
Функция НастройкиУправленияДоговорами()Экспорт
	
	НастройкиУправленияДоговорами = Новый Структура;	
	
	НастройкиУправленияДоговорами.Вставить("СоздаватьЗаявкиНаОплатуПланируемыеПоступленияДС", ПолучитьФункциональнуюОпцию("СоздаватьЗаявкиНаОплатуПланируемыеПоступленияДС"));
	
	НастройкиУправленияДоговорами.Вставить("СоздаватьЗаявкиНаРасходПланируемыеДоходы", ПолучитьФункциональнуюОпцию("СоздаватьЗаявкиНаРасходПланируемыеДоходы"));
	
	Возврат НастройкиУправленияДоговорами;
	
КонецФункции // НастройкиДоговоров()
...
Процедура ПриСозданииНаСервереВерсииСоглашения(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Источники.Добавить(Объект.ДоговорКонтрагента.Метаданные());
	Источники.Добавить(Объект.Ссылка.Метаданные());
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ДоговорСНерезидентом") Тогда
		...
		Форма.ДоговорСНерезидентом = (ЗначениеЗаполнено(РеквизитыКонтрагента.СтранаРегистрации) 
									И РеквизитыКонтрагента.СтранаРегистрации <> Справочники.СтраныМира.Россия);
	...
	Если РаботаСДоговорамиКонтрагентовУХКлиентСервер.ЕстьЗеркалированиеДоговора(Объект.ВидДоговораУХ) Тогда
		...
		Форма.КонтрагентОрганизацииУХ = РаботаСКонтрагентамиУХ.КонтрагентОрганизации(Объект.Организация);
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ВалютаРегламентированногоУчета") Тогда
		Форма.ВалютаРегламентированногоУчета = ОбщегоНазначенияПовтИспУХ.ПолучитьВалютуРегламентированногоУчета();	
	КонецЕсли;
	
	ОрганизацииВызовСервераУХ.ДобавитьПараметрыВыбораОрганизацийРО(Форма, "Организация");
	
	Если Не ПравоДоступа("Изменение", Метаданные.НайтиПоТипу(ТипЗнч(Объект.Ссылка))) Тогда    
		Элементы.ПодменюРедактировать.Видимость = Ложь;
	КонецЕсли; 
	
	УстановитьПризнакАктуализацииГрафиковРасчетов(Форма);
...
Процедура ПриЗаписиНаСервереВерсияСоглашения(Форма, Отказ, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	Если ТекущийОбъект.ДополнительныеСвойства.Свойство("ВстречныйДоговор") И ЗначениеЗаполнено(ТекущийОбъект.ДополнительныеСвойства.ВстречныйДоговор)
		Тогда
		Если ПараметрыЗаписи.Свойство("ВстречныйДоговор")  Тогда
			ПараметрыЗаписи.ВстречныйДоговор = ТекущийОбъект.ДополнительныеСвойства.ВстречныйДоговор;
			Форма.ЗаписатьВстречныеДоговоры = Истина;
		Иначе			
			ПараметрыЗаписи.Вставить("ВстречныйДоговор", ТекущийОбъект.ДополнительныеСвойства.ВстречныйДоговор);			
			Форма.ЗаписатьВстречныеДоговоры = Истина;
		КонецЕсли;
	...
	Если ПараметрыЗаписи.Свойство("ВстречныйДоговор") И ЗначениеЗаполнено(ПараметрыЗаписи.ВстречныйДоговор) Тогда
		...
		// Уже записано перед записью
		
	Иначе 
		
		ТекущийОбъект.ДополнительныеСвойства.Удалить("ВстречныйДоговор");		
...
Процедура ПослеЗаписиНаСервереВерсияСоглашения(Форма, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	Если ЗначениеЗаполнено(ТекущийОбъект.ДоговорКонтрагента) Тогда
		Форма.АктуальнаяВерсия = РегистрыСведений.ВерсииГрафиковРасчетов.ПолучитьАктуальнуюВерсиюФинансовогоИнструмента(ТекущийОбъект.ДоговорКонтрагента);
		Форма.ЗначениеВРеквизитФормы(ТекущийОбъект.ДоговорКонтрагента.ПолучитьОбъект(), "ДоговорОбъект"); 
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ВстречныйДоговор") Тогда
			Форма.ВстречныйДоговор = РегистрыСведений.СвязанныеДоговоры.ПолучитьВстречныйДоговор(ТекущийОбъект.ДоговорКонтрагента);	
		КонецЕсли;
	КонецЕсли;
	...
	ОчиститьОтложенныйГрафикРасчетов(Форма);
...
Процедура ПриИзмененииКонтрагентаСервер(Форма) Экспорт
	...
	Форма.ДоговорСНерезидентом = (ЗначениеЗаполнено(РеквизитыВладельца.СтранаРегистрации) 
							  И РеквизитыВладельца.СтранаРегистрации <> Справочники.СтраныМира.Россия);
...
Процедура ИнициализироватьПереченьОперацийГрафика(Форма, Знач ВидДоговораУХ = Неопределено) Экспорт
	...
	Если Элементы.Найти("ГрафикРасчетовЛинейныйОперацияГрафика")<>Неопределено Тогда
...
Процедура ОбновитьТаблицуГрафикРасчетов(Форма, СтруктураДействий = Неопределено, ОписаниеГрафика = Неопределено) Экспорт
	...
	Для Каждого ТекСтрокаГрафик Из Объект.ГрафикРасчетов Цикл
		...
		//Добавим признак ручной корректировки
		КолонкаГрафика = РаботаСДоговорамиКонтрагентовУХ.ИмяКолонкиГрафика(ТекСтрокаГрафик.ОперацияГрафика, ОписаниеГрафика, Истина);
		Если ПустаяСтрока(КолонкаГрафика) Тогда
			Продолжить;
		КонецЕсли;
		
		НоваяСтрокаГрафика[КолонкаГрафика] = ТекСтрокаГрафик.РучнаяКорректировка;
	...
	Для Каждого ОписаниеСекцииГрафика Из ОписаниеГрафика Цикл
		...
		ЧисловыеКолонки.Добавить(СекцияГрафика.КолонкаПриходКорректировка);
		...
		ЧисловыеКолонки.Добавить(СекцияГрафика.КолонкаРасходКорректировка);
	...
	ОбновитьИтоги(Форма, ТабГрафик, ОписаниеГрафика);
...
Процедура ОбновитьИтоги(Форма, ГрафикРасчетов, ОписаниеГрафика)
	
	Для каждого ЭлементОписания Из ОписаниеГрафика Цикл
		
		ОписаниеСекции = ЭлементОписания.Значение;
		
		Если ОписаниеСекции.Свойство("ОписаниеИтогов") Тогда
			
			Для каждого ЭлементОписанияИтогов Из ОписаниеСекции.ОписаниеИтогов Цикл
				
				ОписаниеСекцииИтога = ЭлементОписанияИтогов.Значение;
				
				Форма[ОписаниеСекцииИтога.ИмяРеквизитаИтог] = ЗначениеИтогаПоПравилу(ГрафикРасчетов, ОписаниеСекцииИтога, ОписаниеСекции);
...
Функция ЗначениеИтогаПоПравилу(ГрафикРасчетов, ОписаниеСекцииИтога, ОписаниеСекции)

	Если ОписаниеСекцииИтога.Правило = "Сумма" Тогда
		Возврат ГрафикРасчетов.Итог(ОписаниеСекцииИтога.КолонкаСДанными);
	ИначеЕсли ОписаниеСекцииИтога.Правило = "РазностьПриходРасход" Тогда
		Возврат ГрафикРасчетов.Итог(ОписаниеСекции.КолонкаПриход) - ГрафикРасчетов.Итог(ОписаниеСекции.КолонкаРасход);
	КонецЕсли;
...
Процедура ПеренестиФактВГрафик(Форма, ДатаПереноса, СтруктураДействийПересчета = Неопределено, ИсключаемыеЭСЗ = Неопределено) Экспорт
	...
	НовыйГрафикРасчетов = РаботаСДоговорамиКонтрагентовУХ.ПеренестиФактВГрафик(ТабГрафикРасчетов, ДатаПереноса, 
		Объект.ДоговорКонтрагента, ОбъектРасчетов, Форма.ОписаниеГрафика, Форма.ОперацииГрафика, ИсключаемыеЭСЗ);
		
	Если СтруктураДействийПересчета <> Неопределено Тогда		
		ФинансовыеИнструментыУХ.ПересчетГрафика(Объект, 0, Форма.ОписаниеГрафика, Форма.ОперацииГрафика, СтруктураДействийПересчета);		
	КонецЕсли;		
...
Процедура АктуализироватьГрафикРасчетовНаФорме(Форма, ДатаПереноса, СтруктураДействий = Неопределено) Экспорт
	...
	ВидДоговора = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДоговорКонтрагента, "ВидДоговораУХ");   
	
	ЭтоКредитЗаймДепозит = УправлениеДоговорамиУХКлиентСерверПовтИсп.ЭтоДоговорДепозит(ВидДоговора)
		Или УправлениеДоговорамиУХКлиентСерверПовтИсп.ЭтоДоговорКредит(ВидДоговора) 
		Или УправлениеДоговорамиУХКлиентСерверПовтИсп.ЭтоДоговорЗаем(ВидДоговора); 
	
	Если ЭтоКредитЗаймДепозит Тогда
		НовыйГрафикРасчетов = РаботаСДоговорамиКонтрагентовУХ.АктуализироватьГрафикРасчетовКредитЗаймДепозитНаФорме(
															ОбъектРасчетов, ДатаПереноса, СтруктураДействий);		
	Иначе
		НовыйГрафикРасчетов = РаботаСДоговорамиКонтрагентовУХ.АктуализироватьГрафикРасчетовНаФорме(ОбъектРасчетов, ДатаПереноса);		
	КонецЕсли;      	  
	
	Если Не НовыйГрафикРасчетов = Неопределено Тогда
		Объект.ГрафикРасчетов.Загрузить(НовыйГрафикРасчетов);	
	КонецЕсли;
	...
	ОбновитьЭлементыАктуализацииГрафиковРасчетов(Форма, Ложь, Истина); 
	
КонецПроцедуры  

Процедура УстановитьПризнакАктуализацииГрафиковРасчетов(Форма) Экспорт
	
	Объект = Форма.Объект; 
	Элементы = Форма.Элементы;
	
	ЭтоДокументСоглашенияАктуализации = ЭтоДокументАктуализацииГрафиковРасчетов(Объект); 
	
	Если Не ЭтоДокументСоглашенияАктуализации Тогда
		Возврат;
	КонецЕсли;
	
	МассивДобавляемыхРеквизитов = Новый Массив;    
	
	НовыйРеквизит = Новый РеквизитФормы("ИзменениеГрафикаРасчета", Новый ОписаниеТипов("Булево"), "", "", Ложь);  
	МассивДобавляемыхРеквизитов.Добавить(НовыйРеквизит);				
	
	Форма.ИзменитьРеквизиты(МассивДобавляемыхРеквизитов);  
	                
	ТекстЗаголовка = НСтр("ru = 'Объект расчетов зарегистрирован к актуализации графика расчетов'");  
	ПараметрыЭлемента = Новый Структура("ЦветТекста, Ширина", 
					ЦветаСтиля.ЦветТекстаВнимание, 50);  
	
	ФормыУХ.СоздатьДекорациюФормы(Элементы, "АктуализацияГрафикаРасчетовНадпись", ТекстЗаголовка, , 
				Элементы.ГруппаГрафик, Элементы.ГрафикРасчетов, ПараметрыЭлемента);    
				
	ОбъектРасчетов = Объект.ДоговорКонтрагента;					
				
	ЭтоОтложенныйГрафикРасчетов = РегистрыСведений.ОбъектыРасчетовКАктуализацииГрафиков.ЭтоОтложенныйГрафикРасчетов(ОбъектРасчетов);  
	ОбновитьЭлементыАктуализацииГрафиковРасчетов(Форма, ЭтоОтложенныйГрафикРасчетов, Ложь);
	
КонецПроцедуры 

Функция ЭтоДокументАктуализацииГрафиковРасчетов(Объект) 
	
	ДокументыРасчетов = Новый Массив;
	ДокументыРасчетов.Добавить(Тип("ДокументСсылка.ВерсияСоглашенияКоммерческийДоговор"));
	
	ЭтоРасчеты = Не (ДокументыРасчетов.Найти(ТипЗнч(Объект.Ссылка)) = Неопределено);
    ...
    Возврат ЭтоРасчеты Или ЭтоКредитЗаймДепозит;
		
КонецФункции

Процедура ОбновитьЭлементыАктуализацииГрафиковРасчетов(Форма, ЭтоОтложенныйГрафикРасчетов, ИзменениеГрафикаРасчета)
	
	Элементы = Форма.Элементы;
	
	Элементы.ГруппаГрафик.Картинка = ?(ЭтоОтложенныйГрафикРасчетов, 
						БиблиотекаКартинок.СообщениеОПроблемах, Новый Картинка);   
						
	Если Элементы.Найти("АктуализацияГрафикаРасчетовНадпись") <> Неопределено Тогда 
		Элементы.АктуализацияГрафикаРасчетовНадпись.Видимость = ЭтоОтложенныйГрафикРасчетов;	
	    Форма.ИзменениеГрафикаРасчета = ИзменениеГрафикаРасчета;
	КонецЕсли;
	
КонецПроцедуры 

Процедура ОчиститьОтложенныйГрафикРасчетов(Форма)
	
	Перем ЭтоИзменениеГрафика;
	...
	Для Каждого ЭлементКоллекции Из Реквизиты Цикл
		Если ЭлементКоллекции.Имя = "ИзменениеГрафикаРасчета" 
			И Форма.ИзменениеГрафикаРасчета Тогда 
			ЭтоИзменениеГрафика = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла; 
	
	Если ЭтоИзменениеГрафика = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	РегистрыСведений.ОбъектыРасчетовКАктуализацииГрафиков.УдалитьГрафикРасчетов(Объект.ДоговорКонтрагента);    
...
Функция ТребуетсяГрафикРасчетов(Форма)
	...
	Если НЕ ЗначениеЗаполнено(ВидДоговораУХ) Тогда
		Возврат Истина;	
	Иначе		
		Возврат РаботаСДоговорамиКонтрагентовУХПовтИсп.ВедетсяГрафикРасчетов(ВидДоговораУХ);
	КонецЕсли;
КонецФункции
...
Процедура ПриСозданииЧтенииНаСервере(Форма)
	...
	СоздатьРеквизитыИЭлементыФормыБезакцептноеСписание(Форма);
	ЗаполнитьЭлементыСтруктурыЗадолженности(Форма, МенеджерВерсииСоглашения);
...
Процедура СоздатьКолонкиИРеквизитыГрафикаРасчетовНаФорме(Форма) Экспорт
	...
	МассивКолонокОписаний = СтрРазделить("КолонкаПриход КолонкаПриходКорректировка КолонкаРасход КолонкаРасходКорректировка КолонкаОстаток", " ");
	...
	Для каждого ЭлементОписания Из ОписаниеГрафика Цикл
		...
		Для каждого ИмяКолонкиОписания Из МассивКолонокОписаний Цикл
			Для каждого Разрез Из МассивПланФакт Цикл
				...
				Если Не ЗначениеЗаполнено(ОписаниеСекции[ИмяКолонкиОписания]) Тогда
					Продолжить;
				КонецЕсли;
	...
	//Удалим значения из массива, т.к. для данных колонок не нужно создавать элементы
	ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(МассивКолонокОписаний,"КолонкаПриходКорректировка");
	ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(МассивКолонокОписаний,"КолонкаРасходКорректировка");
	
	НовыйРеквизит = Новый РеквизитФормы("Отступ", Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(1)), 
		ПутьКТаблице, "Отступ");
	МассивДобавляемыхРеквизитов.Добавить(НовыйРеквизит);  
	
	ДобавитьРеквизитыИтогаДляПодвала(МассивДобавляемыхРеквизитов, ОписаниеГрафика);
	...
	Для каждого ЭлементОписания Из ОписаниеГрафика Цикл
		...
		Для каждого ИмяКолонкиОписания Из МассивКолонокОписаний Цикл
			...
			Для каждого Разрез Из МассивПланФакт Цикл
				...
				Колонка.ПутьКДаннымПодвала = ПутьКДаннымИтога(ОписаниеСекции, ИмяКолонкиОписания, Разрез);//ОписаниеСекции[ИмяКолонкиОписания] + ПостФиксРеквизитаИтога(); 
				
				Если ИмяКолонкиОписания = "КолонкаОстаток" Тогда
					Колонка.ЦветТекстаПодвала = ЦветаСтиля.ЦветОсобогоТекста;
				КонецЕсли;
...
Процедура СоздатьКолонкиГрафикаРасчетовЛинейныйНаФорме(Форма) Экспорт
	...
	Если ТипЗнч(Форма.Объект.Ссылка) = Тип("ДокументСсылка.ВерсияСоглашенияКоммерческийДоговор") Тогда
		Элемент = Элементы.Добавить("ГрафикРасчетовЛинейныйДокументПланирования", Тип("ПолеФормы"), ТаблицаФормыГрафик);
		Элемент.Вид = ВидПоляФормы.ПолеНадписи;
		Элемент.ПутьКДанным = "Объект.ГрафикРасчетов.ДокументРезервирования";
	КонецЕсли;
...
Процедура ОбновитьЗаявкиПоГрафикуОплаты(Параметры) Экспорт

	ОбъектыРасчетов = Новый Массив;  
	ОбъектыРасчетов.Добавить(Параметры.ДоговорКонтрагента);

	Результат = РасчетГрафиковОперацийУХ.СформироватьЗаявкиПоГрафикуОплаты(
		ОбъектыРасчетов, 
		Параметры.ДатаНачалаДействия,Параметры.ДатаОкончанияДействия);
		
	Если ТипЗнч(Результат) = Тип("Массив") И Результат.Количество() Тогда  
		
		Для Каждого СозданныйДокумент Из Результат Цикл
		
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Создан документ ''%1'''"),СозданныйДокумент); 
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, СозданныйДокумент);
			
		КонецЦикла;
			
	КонецЕсли;
	
	Если ТипЗнч(Параметры) = Тип("Структура") И Параметры.Свойство("ВызовИзПлатежногоКалендаря") Тогда 
		Параметры.Вставить("Результат", Результат);	
	КонецЕсли;
		
КонецПроцедуры

Процедура ДобавитьРеквизитыИтогаДляПодвала(МассивДобавляемыхРеквизитов, ОписаниеГрафика)
	...
	Для каждого ЭлементОписания Из ОписаниеГрафика Цикл
		...
		Если ОписаниеСекции.Свойство("ОписаниеИтогов") Тогда
			...
			Для каждого ЭлементОписанияИтогов Из ОписаниеСекции.ОписаниеИтогов Цикл
				...
				НовыйРеквизит = Новый РеквизитФормы(ОписаниеСекцииИтога.ИмяРеквизитаИтог, Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(15,2)));
				МассивДобавляемыхРеквизитов.Добавить(НовыйРеквизит);
...
Функция ПутьКДаннымИтога(ОписаниеСекции, ИмяКолонкиОписания, Разрез)
	
	Если ОписаниеСекции.Свойство("ОписаниеИтогов")  Тогда    
		
		ОписаниеИтогов = ОписаниеСекции.ОписаниеИтогов;
		ИмяКолонки = ОписаниеСекции[ИмяКолонкиОписания];
		ИмяКолонкиСПостфиксом = ИмяКолонки+Разрез.Постфикс;
		
		Если ОписаниеИтогов.Свойство(ИмяКолонкиСПостфиксом) Тогда
		    Возврат ОписаниеИтогов[ИмяКолонкиСПостфиксом].ИмяРеквизитаИтог;
		КонецЕсли;
...
Процедура СоздатьРеквизитыИЭлементыФормыБезакцептноеСписание(Форма)
	
	Элементы = Форма.Элементы;
	
	ГруппаБезакцептноеСписание = Элементы.Найти("ГруппаБезакцептноеСписание");
	
	Если ГруппаБезакцептноеСписание = Неопределено Тогда 
		Возврат;
	КонецЕсли;
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ЭлементыСтруктурыЗадолженности") Тогда
		Возврат;
	КонецЕсли;
	
	НовыйРеквизит = Новый РеквизитФормы("ЭлементыСтруктурыЗадолженности",Новый ОписаниеТипов("СписокЗначений"));
	
	МассивДобавляемыхРеквизитов.Добавить(НовыйРеквизит);	

	НовыйРеквизит = Новый РеквизитФормы("БезакцептноеСписание",Новый ОписаниеТипов("Булево"));
	
	МассивДобавляемыхРеквизитов.Добавить(НовыйРеквизит);	
	
	НовыйРеквизит = Новый РеквизитФормы("БезакцептноеСписаниеНадпись",Новый ОписаниеТипов("Строка"));
	
	МассивДобавляемыхРеквизитов.Добавить(НовыйРеквизит);	

	Форма.ИзменитьРеквизиты(МассивДобавляемыхРеквизитов);
	
	ЗаголовокЭлемента = НСтр("ru = 'Безакцептное списание'");
	
	Элемент = Элементы.Добавить("БезакцептноеСписание", Тип("ПолеФормы"),ГруппаБезакцептноеСписание);
	Элемент.Вид = ВидПоляФормы.ПолеФлажка;
	Элемент.Заголовок = ЗаголовокЭлемента; 
	Элемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право; 
	Элемент.ПутьКДанным = "БезакцептноеСписание";
	Элемент.УстановитьДействие("ПриИзменении", "Подключаемый_БезакцептноеСписаниеПриИзменении");

	Элемент = Элементы.Добавить("БезакцептноеСписаниеНадпись", Тип("ПолеФормы"),ГруппаБезакцептноеСписание);
	Элемент.Вид = ВидПоляФормы.ПолеНадписи;
	Элемент.Заголовок = ЗаголовокЭлемента;
	Элемент.Гиперссылка = Истина;
	Элемент.ПутьКДанным = "БезакцептноеСписаниеНадпись";
	Элемент.УстановитьДействие("Нажатие", "Подключаемый_БезакцептноеСписаниеНадписьНажатие");
	
КонецПроцедуры

Процедура ЗаполнитьЭлементыСтруктурыЗадолженности(Форма, МенеджерВерсииСоглашения)
	...
	Форма.ЭлементыСтруктурыЗадолженности = ДоговорыКонтрагентовВстраиваниеУХ.ЭлементыСтруктурыЗадолженностиПоОписаниюГрафика(МенеджерВерсииСоглашения.ОписаниеГрафика(Объект.ВидДоговораУХ));
...
Процедура ЗаполнитьПараметрыОперацийГрафикаПоУмолчанию(Форма) Экспорт
