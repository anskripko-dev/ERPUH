...
Функция Источник_ВзаиморасчетыПоДоговору(Объект)
	...
	Взаиморасчеты.Колонки.Добавить("Контрагент", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"));
...
Функция ПланыДокумента(Источник) Экспорт
	...
	Если ДанныеОбъекта.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов
		ИЛИ ДанныеОбъекта.РежимИспользованияГрафика <> Перечисления.РежимыИспользованияГрафика.АвтоматическоеСозданиеПлатежныхПозиций Тогда
		Возврат ТаблицаПланов;                                                                 
	КонецЕсли;
