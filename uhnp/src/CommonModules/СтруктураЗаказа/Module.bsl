	...
	Для каждого Строка Из ЦепочкиПеремещений Цикл
		...
		Пока Сч < ПараметрыРасчета.МаксГлубинаРасчета Цикл
			...
			Шаблон = НСтр("ru = 'Обнаружено зацикливание перемещений: %1 \%2 %3';
							|en = 'Обнаружено зацикливание перемещений: %1 \%2 %3'");
		...
		Если Сч = ПараметрыРасчета.МаксГлубинаРасчета Тогда
			...
			Шаблон = НСтр("ru = 'Превышена максимальная глубина расчета структуры заказа (%1)';
							|en = 'Превышена максимальная глубина расчета структуры заказа (%1)'");
			ТекстСообщения = СтрШаблон(Шаблон, ПараметрыРасчета.МаксГлубинаРасчета);
			ЗаписьЖурналаРегистрации(СтруктураЗаказа.ИмяСобытияЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,,Строка.ЗаказНаПроизводство,
				ТекстСообщения);
		КонецЕсли;
...
Процедура ВыполнитьПоток(НаименованиеЗадания, ИмяПроцедуры, ПараметрыПроцедуры, ПротоколРасчета)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтрШаблон(НСтр("ru = 'Расчет структуры заказа: %1 (%2)';
																	|en = 'Расчет структуры заказа: %1 (%2)'"), НаименованиеЗадания, МестоВОчереди + 1);
