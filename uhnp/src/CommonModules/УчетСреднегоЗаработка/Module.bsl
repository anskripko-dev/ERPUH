				...
				Если ВыводитьСообщения Тогда 
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = '%1: отсутствуют отработанные дни в расчетном периоде, расчетный период изменен на %2 - %3';
							|en = '%1: отсутствуют отработанные дни в расчетном периоде, расчетный период изменен на %2 - %3'"), 
						КадровыеДанные[0].ФамилияИО,
...
Функция ОписаниеПолитикиИсключенияДублирующихПремий(ИмяВарианта) Экспорт
	...
	ИначеЕсли ИмяВарианта = ИмяВариантаИсключенияПремийБезИсключения() Тогда
		...
		Описание.Описание = НСтр("ru = 'Учитываются все премии.
									   ...
									   |Automatic exclusion of duplicate bonuses is not used.'");
		Описание.ПодробноеОписание =
			НСтр("ru = 'Что бы задействовать механизм исключения дублирующих премий
					   ...
					   |en = 'To exclude duplicate bonuses,
					   |set the bonus indicator for at least one calculation type (in the calculation type form).'");
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий     = НСтр("ru = 'Заполнение регистра ""Данные для расчета среднего заработка по начислениям"".';
										|en = 'Fill in the ""Data for average earnings calculation by accruals"" register.'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Удаление записей регистра ""Данные для расчета среднего заработка по начислениям"".';
										|en = 'Delete the ""Data for average earnings calculation by accruals"" register records.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение периода действия в данных для расчета общего среднего заработка по начислениям.';
									|en = 'Fill the validity period in data for calculating the total average earnings on accruals.'");
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при формировании движений документа по регистру Данные для расчета среднего заработка по начислениям: %1';
					|en = 'Error when generating the document records in the Data for calculating average earnings by accruals register: %1'"),
