...
Функция ТекстЗапросаДебиторскаяЗадолженностьФормаПодбора(Параметры) Экспорт
	...
	Источник1 = Схема.ПакетЗапросов[0].Операторы[0].Источники[0].Источник;
	Источник2 = Схема.ПакетЗапросов[0].Операторы[1].Источники[0].Источник;   
	Источник3 = Схема.ПакетЗапросов[0].Операторы[2].Источники[1].Источник;
	Источник4 = Схема.ПакетЗапросов[0].Операторы[3].Источники[1].Источник;
	...
	Если ЗначениеЗаполнено(ТекстУсловия) Тогда
		...
		Источник1.Параметры[1].Выражение = Новый ВыражениеСхемыЗапроса(ТекстУсловия);
		Источник2.Параметры[1].Выражение = Новый ВыражениеСхемыЗапроса(ТекстУсловия);
		Источник3.Параметры[1].Выражение = Новый ВыражениеСхемыЗапроса(ТекстУсловия);
		Источник4.Параметры[1].Выражение = Новый ВыражениеСхемыЗапроса(ТекстУсловия);
	КонецЕсли;
	...
	Если Параметры.Свойство("НаДату") ИЛИ ЗначениеЗаполнено(Параметры.НаДату) Тогда
		...
		Источник3.Параметры[0].Выражение = Новый ВыражениеСхемыЗапроса("&НаДату");
		Источник3.Параметры[0].Выражение = Новый ВыражениеСхемыЗапроса("&НаДату");
	КонецЕсли;
...
Функция ТекстЗапросаДебиторскаяЗадолженность()
	...
	|	СУММА(РасчетыСКлиентамиПоСрокамОстатки.ДолгОстаток) КАК СуммаОбязательства
	...
	|	И РасчетыСКлиентамиПоСрокамОстатки.ДатаПлановогоПогашения <> ДАТАВРЕМЯ(1, 1, 1)
	|
	|СГРУППИРОВАТЬ ПО
	|	РасчетыСКлиентамиПоСрокамОстатки.АналитикаУчетаПоПартнерам.Организация,
	...
	|	РасчетыСКлиентамиПоСрокамОстатки.РасчетныйДокумент,
	...
	|	МАКСИМУМ(РасчетыСПоставщикамиПоСрокамОстатки.ДатаПлановогоПогашения),
	|	СУММА(РасчетыСПоставщикамиПоСрокамОстатки.ПредоплатаОстаток)
	|ИЗ
	...
	|	И РасчетыСПоставщикамиПоСрокамОстатки.ДатаПлановогоПогашения <> ДАТАВРЕМЯ(1, 1, 1)
	|
	|СГРУППИРОВАТЬ ПО
	|	РасчетыСПоставщикамиПоСрокамОстатки.АналитикаУчетаПоПартнерам.Организация,
	|	РасчетыСПоставщикамиПоСрокамОстатки.АналитикаУчетаПоПартнерам.Контрагент,
	...
	|
	|ВЫБРАТЬ
	|	РасчетыСКлиентамиПоДокументамОстатки.АналитикаУчетаПоПартнерам.Организация,
	|	РасчетыСКлиентамиПоДокументамОстатки.АналитикаУчетаПоПартнерам.Контрагент,
	|	РасчетыСКлиентамиПоДокументамОстатки.АналитикаУчетаПоПартнерам.Договор,
	...
	|	МИНИМУМ(НАЧАЛОПЕРИОДА(РасчетыСКлиентамиПоДокументам.Период, ДЕНЬ)),
	|	МИНИМУМ(РасчетыСКлиентамиПоДокументам.ДатаПлатежа),
	...
	|		ПО РасчетыСКлиентамиПоДокументам.АналитикаУчетаПоПартнерам = РасчетыСКлиентамиПоДокументамОстатки.АналитикаУчетаПоПартнерам
	|			И РасчетыСКлиентамиПоДокументам.ЗаказКлиента = РасчетыСКлиентамиПоДокументамОстатки.ЗаказКлиента
	|			И РасчетыСКлиентамиПоДокументам.РасчетныйДокумент = РасчетыСКлиентамиПоДокументамОстатки.РасчетныйДокумент
	|			И РасчетыСКлиентамиПоДокументам.Валюта = РасчетыСКлиентамиПоДокументамОстатки.Валюта
	|			И (РасчетыСКлиентамиПоДокументам.ДатаПлатежа <> ДАТАВРЕМЯ(1, 1, 1))
	|ГДЕ
	...
	|
	|ВЫБРАТЬ
	|	РасчетыСПоставщикамиПоДокументамОстатки.АналитикаУчетаПоПартнерам.Организация,
	|	РасчетыСПоставщикамиПоДокументамОстатки.АналитикаУчетаПоПартнерам.Контрагент,
	|	РасчетыСПоставщикамиПоДокументамОстатки.АналитикаУчетаПоПартнерам.Договор,
	...
	|	МИНИМУМ(НАЧАЛОПЕРИОДА(РасчетыСПоставщикамиПоДокументам.Период, ДЕНЬ)),
	|	МИНИМУМ(РасчетыСПоставщикамиПоДокументам.ДатаПлатежа),
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСПоставщикамиПоДокументам.Остатки КАК РасчетыСПоставщикамиПоДокументамОстатки
	|		ПО РасчетыСПоставщикамиПоДокументам.АналитикаУчетаПоПартнерам = РасчетыСПоставщикамиПоДокументамОстатки.АналитикаУчетаПоПартнерам
	|			И РасчетыСПоставщикамиПоДокументам.ЗаказПоставщику = РасчетыСПоставщикамиПоДокументамОстатки.ЗаказПоставщику
	|			И РасчетыСПоставщикамиПоДокументам.РасчетныйДокумент = РасчетыСПоставщикамиПоДокументамОстатки.РасчетныйДокумент
	|			И РасчетыСПоставщикамиПоДокументам.Валюта = РасчетыСПоставщикамиПоДокументамОстатки.Валюта
	|			И (РасчетыСПоставщикамиПоДокументам.ДатаПлатежа <> ДАТАВРЕМЯ(1, 1, 1))
	|ГДЕ
	|	РасчетыСПоставщикамиПоДокументамОстатки.ДолгОстаток > 0
	|
	|СГРУППИРОВАТЬ ПО
	|	РасчетыСПоставщикамиПоДокументамОстатки.АналитикаУчетаПоПартнерам.Организация,
	|	РасчетыСПоставщикамиПоДокументамОстатки.АналитикаУчетаПоПартнерам.Контрагент,
...
Функция ТекстЗапросаДебиторскаяЗадолженностьДляОповещенияЗавершение()
	...
	|	ВТ_ДебиторскаяЗадолженность.Договор.Менеджер КАК Ответственный
	|ИЗ
	|	ВТ_ДебиторскаяЗадолженность КАК ВТ_ДебиторскаяЗадолженность,
...
Функция ТекстЗапросаПросроченнаяДебиторскаяЗадолженностьЗавершение()
	...
	|	ВТ_ДебиторскаяЗадолженность.Валюта КАК Валюта,
	|	ВТ_ДебиторскаяЗадолженность.РасчетныйДокумент КАК РасчетныйДокумент,
	|	МАКСИМУМ(ВТ_ДебиторскаяЗадолженность.ДатаВозникновения) КАК ДатаВозникновения,
	|	МАКСИМУМ(ВТ_ДебиторскаяЗадолженность.ДатаПлановогоПогашения) КАК ДатаПлановогоПогашения,
	|	ДОБАВИТЬКДАТЕ(МАКСИМУМ(ВТ_ДебиторскаяЗадолженность.ДатаПлановогоПогашения), ДЕНЬ, ДнейПросрочки.Значение) КАК ДатаПлановойПросрочки,
	...
	|	ВЫРАЗИТЬ(ВТ_ДебиторскаяЗадолженность.Договор КАК Справочник.ДоговорыКонтрагентов).Менеджер КАК Ответственный
	|ПОМЕСТИТЬ ВТ_Данные
	|ИЗ
	|	ВТ_ДебиторскаяЗадолженность КАК ВТ_ДебиторскаяЗадолженность,
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ДебиторскаяЗадолженность.Организация,
	|	ВТ_ДебиторскаяЗадолженность.Контрагент,
	|	ВТ_ДебиторскаяЗадолженность.Договор,
	|	ВТ_ДебиторскаяЗадолженность.ОбъектРасчетов,
	|	ВТ_ДебиторскаяЗадолженность.Валюта,
	|	ВТ_ДебиторскаяЗадолженность.РасчетныйДокумент,
	|	ВТ_ДебиторскаяЗадолженность.СуммаОбязательства,
	|	ДнейПросрочки.Значение
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
