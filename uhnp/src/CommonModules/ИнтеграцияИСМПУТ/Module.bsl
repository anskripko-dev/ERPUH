...
//++ НЕ УТ
		Или ТипПроверяемогоДокумента = Тип("ДокументСсылка.ДвижениеПродукцииИМатериалов")
//-- НЕ УТ
	Тогда
	...
	Исключение
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'При удалении записи о передаче кодов маркировки между организациями по документу %1 возникла ошибка:
			           |Текст ошибки: %2';
			           |en = 'An error occurred when deleting the record of transferring marking codes between companies for the %1 document:
			           |Error text: %2'"),
		...
		ОбщегоНазначенияИСВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			ТекстОшибки,
			НСтр("ru = 'ИСМП';
				|en = 'ISMT'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если ЭтоИндивидуальныйПредприниматель И ЭтоГруппаСкладов Тогда
		...
		Если ВыводитьСообщение Тогда
			...
			ШаблонТекстСообщения = НСтр("ru = 'При отгрузке пивной продукции от индивидуального предпринимателя недопустимо использование группы складов.
									...
									|en = 'When shipping beer products from an individual entrepreneur, it is not allowed to use a warehouse group.
									|Current value: %1. Specify a particular warehouse.'");
...
//-- НЕ УТКА
	...
	ИначеЕсли Сценарий = 1 Тогда
		...
		//++ НЕ УТ
		ОтразитьИзменениеКоличестваСырьяПроизводства(ДокументОбъект, ПараметрыОкончанияПроверки);
		//-- НЕ УТ
...
// Переносит маркируемую продукцию из формы проверки и подбора в документ производства
//   * Удаляет из табличной части сырья отсутствующую в данных проверки маркируемую продукцию.
//   * Добавляет не найденную продукцию в табличную часть сырья
//   * Не меняет прочие строки.
// 
// Параметры:
//  ДокументОбъект           - ДокументОбъект - документ к изменению
//  ПараметрыОкончанияПроверки - Структура - (См. ПроверкаИПодборПродукцииИСМП.ЗафиксироватьРезультатПроверкиИПодбора).
//
Процедура ОтразитьИзменениеКоличестваСырьяПроизводства(ДокументОбъект, ПараметрыОкончанияПроверки)
	
	ИмяТаблицыСырья = "МатериалыИРаботы";

//++ НЕ УТКА

	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЭтапПроизводства2_2") Тогда
		ИмяТаблицыСырья = "РасходМатериаловИРабот";
	КонецЕсли;

//-- НЕ УТКА

	Запрос = Новый Запрос;
	ТаблицаТовары = ДокументОбъект[ИмяТаблицыСырья].Выгрузить();
	Запрос.УстановитьПараметр("Товары", ТаблицаТовары);
	...
	ПараметрыУказанияСерий = НоменклатураСервер.ПараметрыУказанияСерий(ДокументОбъект, ОбщегоНазначения.МенеджерОбъектаПоСсылке(ДокументОбъект.Ссылка));
	ПараметрыУказанияСерий = ПараметрыУказанияСерий[ИмяТаблицыСырья];
	...
	ДокументОбъект[ИмяТаблицыСырья].Загрузить(ТаблицаТовары);
	...
	Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РазборкаТоваров Тогда
		...
		Если КоличествоСтрок > 1 Тогда
			ВызватьИсключение НСтр("ru = 'Подбор различной маркируемой номенклатуры в разборку товаров не допускается';
									|en = 'Cannot pick various marked items to reverse kitting'", );
...
//++ НЕ УТ
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.ДвижениеПродукцииИМатериалов") Тогда
		ОтразитьИзменениеКоличестваВТабличныхЧастяхТоварыСерииДвижениеПродукцииИМатериалов(ДокументОбъект, ПараметрыОкончанияПроверки);
//-- НЕ УТ
	КонецЕсли;
...
// Переносит маркируемую продукцию из формы проверки и подбора в документ Движение продукции и материалов
//   * Удаляет из табличных частей Товары, Серии отсутствующую в данных проверки маркируемую продукцию.
//   * Не меняет прочие товарные строки.
// 
// Параметры:
//  ДокументОбъект           - ДокументОбъект.ДвижениеПродукцииИМатериалов - документ к изменению
//  ПараметрыОкончанияПроверки - Структура - (См. ПроверкаИПодборПродукцииИСМП.ЗафиксироватьРезультатПроверкиИПодбора).
//
Процедура ОтразитьИзменениеКоличестваВТабличныхЧастяхТоварыСерииДвижениеПродукцииИМатериалов(ДокументОбъект, ПараметрыОкончанияПроверки)
	...
	СтруктураЗаполнения = СтруктураЗаполнения(
		СтруктураДействийПриДобавленииНовойСтрокиДвижениеПродукцииИМатериалов(ДокументОбъект),
		СтруктураДействийПриИзмененииСтрокиДвижениеПродукцииИМатериалов(ДокументОбъект),
		КлючПоиска,
		КлючСтрокиСерий);
	...
	Пока ВыборкаРасхождения.Следующий() Цикл
		...
		Если КоличествоНедостачаНеПризнать > 0 Или КоличествоНедостачаПризнать > 0
			...
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'При распределении произошла ошибка, сгруппируйте состав строк в исходном документе';
														|en = 'An error occurred during allocation. Group rows in the original document'"));
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьШапкуДокумента(ДокументОбъект, ДанныеЗаполнения, Запрос, ТекстОшибкиЗаполнения = "")
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ТекстОшибкиЗаполнения) Тогда
			ТекстОшибкиЗаполнения = СтрШаблон(НСтр("ru = 'По документу %1 отсутствуют данные для заполнения.';
													|en = 'По документу %1 отсутствуют данные для заполнения.'"), Основание);
		КонецЕсли;
...
Процедура ЗаполнитьВыводИзОборотаИСМПНаОснованииПроизводстваБезЗаказа(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ТабличнаяЧастьШтрихкоды = Новый Структура("ИмяТаблицыШтрихкодыУпаковок, ИмяКолонкиШтрихкодУпаковки", "ШтрихкодыУпаковокМатериалы", "ШтрихкодУпаковки");
	ЗаполнитьТабличныеЧастиВыводаИзОборотаИСМП(ДокументОбъект, ДанныеЗаполнения, "ПроизводствоБезЗаказа", ОписаниеТабличнойЧасти, ТабличнаяЧастьШтрихкоды);
...
Процедура ЗаполнитьВыводИзОборотаИСМПНаОснованииЭтапаПроизводства(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ТабличнаяЧастьШтрихкоды = Новый Структура("ИмяТаблицыШтрихкодыУпаковок, ИмяКолонкиШтрихкодУпаковки", "ШтрихкодыУпаковокМатериалы", "ШтрихкодУпаковки");
	ЗаполнитьТабличныеЧастиВыводаИзОборотаИСМП(ДокументОбъект, ДанныеЗаполнения, "ЭтапПроизводства2_2", ОписаниеТабличнойЧасти, ТабличнаяЧастьШтрихкоды);
...
Функция СтруктураДействийПриДобавленииНовойСтрокиДвижениеПродукцииИМатериалов(Объект)
...
Функция СтруктураДействийПриИзмененииСтрокиДвижениеПродукцииИМатериалов(Объект)
