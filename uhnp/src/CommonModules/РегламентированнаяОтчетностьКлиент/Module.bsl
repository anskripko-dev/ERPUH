...
Функция ВыполнениеКомандыОтменено(Форма, НаименованиеОтчета, Организация, НаименованиеКоманды = "") Экспорт
	...
	ИначеЕсли НаименованиеОтчета = "Форма 2-ТП (отходы)" Тогда 
		Предупреждение = НСтр("ru = 'Формат выгрузки не опубликован, для сдачи отчета следует воспользоваться личным кабинетом природопользователя';
								|en = 'Формат выгрузки не опубликован, для сдачи отчета следует воспользоваться личным кабинетом природопользователя'");
		ПоказатьИВернутьОшибку(Форма, Предупреждение);
		Возврат Истина;
...
Функция ПриЗаписиРегламентированногоОтчетаНаКлиенте(Форма, КодИФНС = "не применимо", Автосохранение = Ложь, Вариант = Неопределено) Экспорт
	...
	Если (Форма.Команды.Найти("РольНалоговыйИнспектор") <> Неопределено) 
		И Форма.СтруктураРеквизитовФормы.Свойство("мСохраненныйДок") Тогда
		
		КнопкаСохранить = Форма.Элементы.Найти("Записать");
		Если КнопкаСохранить = Неопределено Тогда
			КнопкаСохранить = Форма.Элементы.Найти("Сохранить");
			Если КнопкаСохранить = Неопределено Тогда
				Возврат Ложь;						
			КонецЕсли;	
		КонецЕсли;
		
		ПараметрыФормы = Новый Структура("Основание", Форма.СтруктураРеквизитовФормы.мСохраненныйДок);
		ОткрытьФорму("Документ.ЗапросДанныхНалоговогоМониторинга.Форма.ФормаДокумента", ПараметрыФормы, Форма);
	
		Возврат Ложь;// сохранение не используется для роли налоговый инспектор
		...
		Если СтруктураРеквизитовФормы.НастройкиВидимостиЭлементовФормы["ПостояннаяВидимость"] Тогда
			...
			Иначе
				ТекстПредупреждения = НСтр("ru = 'Автоматическое заполнение редакции формы будет реализовано в будущих версиях программы.';
											|en = 'Automatic filling of the form revision will be implemented in future application versions.'");
			КонецЕсли;
