...
// 
// Параметры:
//  Контекст - БухгалтерскаяОтчетностьБРО.НовыйКонтекстОтчета
// 
	...
	Для Каждого Показатель Из ПроцессорЗаполненияОтчета.ПравилаПолученияДанных Цикл
		...
		Если Не ОсобенныйРасчет Тогда
			...
			КомпонентыЗачета.Положительный.Пояснение  = НСтр("ru = 'положительное значение';
															|en = 'positive value'", ПроцессорЗаполненияОтчета.КодЯзыка);
			КомпонентыЗачета.Отрицательный.Пояснение  = НСтр("ru = 'отрицательное значение';
															|en = 'negative value'", ПроцессорЗаполненияОтчета.КодЯзыка);
...
Процедура ДобавитьЗаголовокРасшифровкиГруппыПоказателей(РасчетГруппы)
	...
	ЗаписьРасшифровки.Описание  = СтрШаблон(
		НСтр("ru = '%1 (%2) - за %3';
			|en = '%1 (%2) for %3'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НаименованиеГруппы,
...
Процедура ДобавитьИтогРасшифровкиГруппыПоказателей(РасчетГруппы)
	...
	ЗаписьРасшифровки.Описание  = НСтр("ru = 'Итого по группе показателей:';
										|en = 'Total by indicator group:'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ДобавитьПоясняющийЗаголовокРасшифровкиПоказателей(РасчетГруппы)
	...
	ЗаписьРасшифровки.Описание  = НСтр("ru = 'Ниже приведен подробный расчет каждого слагаемого';
										|en = 'Ниже приведен подробный расчет каждого слагаемого'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ДобавитьОписаниеПоказателяОтчета(РасчетГруппы, ИндексСоставаГруппы, ПорядковыйНомерПоказателя)
	...
	Если Не ЗначениеЗаполнено(ЗаполненныеЗначенияЗачетаПоказателей) Тогда
		ШаблонТекста = НСтр("ru = 'Итого по показателю:';
							|en = 'Total by indicator:'", ПроцессорЗаполненияОтчета.КодЯзыка);
		ДобавитьЗаписьРасшифровкиЗаполненияПоказателя(РасчетГруппы, "Итог", ШаблонТекста, Значение);
	Иначе
		ШаблонТекста = НСтр("ru = '%1 %2 в строку %3';
							|en = '%1 %2 to line %3'", ПроцессорЗаполненияОтчета.КодЯзыка); // <Что-то> <включено> в строку <такую-то>
		СловоВключено = НСтр("ru = 'включено';
							|en = 'enabled'", ПроцессорЗаполненияОтчета.КодЯзыка);
...
Процедура ДобавитьОписаниеСлагаемыхПоказателяОтчета(РасчетГруппы, Слагаемые, Уровень)
	...
	Для Каждого Слагаемое Из Слагаемые Цикл
		...
		ВыводитьПодчиненныеСлагаемые = (КоличествоПодчиненныхСлагаемых > 1 И КоличествоПодчиненныхСлагаемых < ЛимитОбъектовРасшифровки())
			Или (КоличествоПодчиненныхСлагаемых = 1 И (ЗначениеЗаполнено(Слагаемое.Строки[0].ПредметЗаписи)ИЛИ ТипЗнч(Слагаемое.Строки[0].ПредметЗаписи)= Тип("Число")));
...
Процедура СократитьЗначенияЗаполненияУпрощенногоОтчета(Процессор)
	...
	Для Каждого ПланОбобщеннойГруппыПоказателей Из ПланСокращения Цикл
		...
		Если РасшифровкаСоставаОбобщеннойГруппыПоказателей.Количество() > 1 Тогда
			...
			ЗаписьРасшифровки.Описание  = СтрШаблон(
				НСтр("ru = '%1 - за %2';
					|en = '%1 - за %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
			...
			ЗаписьРасшифровки.Описание  = НСтр(
				...
				|en = 'Это обобщенная группа показателей, она рассчитывается суммированием показателей (групп показателей), описанных ниже.
				|Среди таких показателей может определяться группа показателей, имеющая наибольший удельный вес в данной обобщенной группе показателей'",
			...
			ЗаписьРасшифровки.Описание = НСтр("ru = 'Итого по обобщенной группе показателей:';
												|en = 'Итого по обобщенной группе показателей:'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПравилаПолученияДанных(ЗаполняемыеПоказатели)
	...
	Для Каждого ОписаниеПоказателя Из ЗаполняемыеПоказатели.Показатели Цикл
		...
		Если ЭтоАлгоритмДенежныхПотоков(ОписаниеПоказателя.Заполнение.Идентификатор) Тогда
			Идентификатор = ИдентификаторАлгоритмаДенежныеПотоки();
		Иначе
			Идентификатор = ОписаниеПоказателя.Заполнение.Идентификатор;
		КонецЕсли;
		
		ОбщегоНазначенияБПКлиентСервер.ДополнитьРазделенныйМассив(ПравилаПолученияДанных, Идентификатор, ОписаниеПоказателя);
