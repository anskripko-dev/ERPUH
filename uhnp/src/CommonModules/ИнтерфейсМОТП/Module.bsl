...
Функция ЗапроситьСтатусыКодовМаркировкиПакетно(МассивИсходныхСтрок, ПараметрыЗапросаСтатусов) Экспорт
	...
	Для Каждого ЭлементМассива Из МассивИсходныхСтрок Цикл
		...
		ИспользоватьТоварнуюГруппу = ИспользоватьТоварнуюГруппуПриЗапросеСтатусаКодовМаркировки(СтрокаТаблицы.ВидПродукции);
		Если ИспользоватьТоварнуюГруппу Тогда
			ИмяТоварнойГруппы = ИнтерфейсИСМПОбщегоНазначения.ТоварнаяГруппа(СтрокаТаблицы.ВидПродукции);
		Иначе
			ИмяТоварнойГруппы = "";
		КонецЕсли;
		
		ПараметрыЗапроса = ПараметрыЗапросаПоТоварнымГруппам[ИмяТоварнойГруппы];
		Если ПараметрыЗапроса = Неопределено Тогда
			...
			ПараметрыЗапроса.Вставить("ЭтоПродукцияМОТП",   ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМОТП(СтрокаТаблицы.ВидПродукции));
			...
			Если ИспользоватьТоварнуюГруппу Тогда
				ПараметрыЗапроса.ПараметрыURL.Добавить(
					СтрШаблон("pg=%1", ИмяТоварнойГруппы));
			КонецЕсли;
			
			ПараметрыЗапросаПоТоварнымГруппам.Вставить(ИмяТоварнойГруппы, ПараметрыЗапроса);
				...
				Если ТипЗнч(ДанныеОбработки) = Тип("Массив") Тогда
					...
					Для Каждого ЭлементДанных Из ДанныеОбработки Цикл
						...
						Если КодОшибки = "404" Тогда
							...
							ПараметрыКодаМаркировки = ИнтерфейсИСМПОбщегоНазначения.ПараметрыКодаМаркировкиМОТП();
							Если ПараметрыЗапроса.ЭтоПродукцияМОТП Тогда
								ПараметрыКодаМаркировки.Статус = Перечисления.СтатусыКодовМаркировкиМОТП.Неопределен;
							Иначе
								ПараметрыКодаМаркировки.Статус = Перечисления.СтатусыКодовМаркировкиИСМП.Неопределен;
							КонецЕсли;
...
Функция ИспользоватьТоварнуюГруппуПриЗапросеСтатусаКодовМаркировки(ВидПродукции)
	
	Возврат ВидПродукции = Перечисления.ВидыПродукцииИС.Пиво
