...
Процедура ФормаДокумента_СкладПриИзменении(Форма, Элемент)
	...
	ЗаказПоставщикуКлиентСерверВнедрениеЕХ.ФормаДокумента_СкладПриИзмененииКлиентСервер(Форма.Объект); 
...
Процедура ПроверитьУстановитьСклад(Форма, Отказ, ПараметрыЗаписи)
	...
	ЭтоГруппаСкладов = ОбщегоНазначенияУХ.ПолучитьЗначениеРеквизита(Объект.Склад,"ЭтоГруппа");     
	Иначе
	ЭтоГруппаСкладов = Ложь;
	КонецЕсли;	
	флПроведение = ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение;
	
	Если Не ЭтоГруппаСкладов И флОдинСклад И флПроведение Тогда
