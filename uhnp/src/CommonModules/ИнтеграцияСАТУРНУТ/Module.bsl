...
Функция ТекстЗапросаТоварыДокументаБезСерийСНомеромГруппыЗатрат(ИмяДокумента, ИмяТЧТовары = ".МатериалыИРаботы")
	...
	Шаблон = СтрЗаменить(Шаблон, "ОтчетПереработчика2_5", ИмяДокумента);
	Шаблон = СтрЗаменить(Шаблон, ".МатериалыИРаботы", ИмяТЧТовары);
	Возврат Шаблон;
...
Процедура ЗаполнитьТабличнуюЧастьСырьеПроизводственнойОперации(ДокументОбъект, ДанныеЗаполнения, ТекстТоварыДокумента, ДополнительныеПараметры = Неопределено)
	...
	//++ НЕ УТ
	УстановитьПараметрЗапросаТаблицаСырье(Запрос, ДанныеЗаполнения, Истина);
	//-- НЕ УТ
...
Процедура ЗаполнитьПроизводственнуюОперациюСАТУРННаОснованииЭтапаПроизводства2_2(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ДополнительныеПараметры.Вставить("ИмяПоляИдентификатор", "Идентификатор");
	
	ЗаполнитьТабличнуюЧастьТоварыПроизводственнойОперации(ДокументОбъект, ДанныеЗаполнения, ТекстТоварыДокумента, ДополнительныеПараметры);
	
	ТекстСырьеДокумента = ИнтеграцияИСУТ.ТекстЗапросаЭтапПроизводства2_2()
		+ ИнтеграцияИС.РазделительЗапросовВПакете()
		+ ТекстЗапросаСырье("ТаблицаТоварыЭтапПроизводства2_2");
	
	ДополнительныеПараметры.Вставить("ИмяПоляИдентификатор", "ИдентификаторСтрокиТоваров");
	...
	ДополнительныеПараметры.Вставить("ИдентификаторЗначение", ИдентификаторСтрокиТоваров);
	
	ЗаполнитьТабличнуюЧастьСырьеПроизводственнойОперации(ДокументОбъект, ДанныеЗаполнения, ТекстСырьеДокумента, ДополнительныеПараметры);
...
Процедура ЗаполнитьПроизводственнуюОперациюСАТУРННаОснованииПроизводстваБезЗаказа(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ДополнительныеПараметры = Новый Структура();
	ДополнительныеПараметры.Вставить("ДетализацияДоНомераГруппыЗатрат", Новый Структура);
	ДополнительныеПараметры.Вставить("ИмяПоляИдентификатор",            "Идентификатор");
	
	ЗаполнитьТабличнуюЧастьТоварыПроизводственнойОперации(ДокументОбъект, ДанныеЗаполнения, ТекстПродукцияДокумента, ДополнительныеПараметры);
	
	ТекстСырьеДокумента = ИнтеграцияИСУТ.ТекстЗапросаПроизводствоБезЗаказаСДетализациейДоНомераГруппыЗатрат()
		+ ИнтеграцияИС.РазделительЗапросовВПакете()
		+ ТекстЗапросаСырьеСНомеромГруппыЗатрат("ТаблицаТоварыПроизводствоБезЗаказа");
...
Процедура ЗаполнитьПроизводственнуюОперациюСАТУРННаОснованииОтчетаПереработчика2_5(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ТекстСырьеДокумента = ИнтеграцияИСУТ.ТекстЗапросаОтчетПереработчика2_5СДетализациейДоНомераГруппыЗатрат()
		+ ИнтеграцияИС.РазделительЗапросовВПакете()
		+ ТекстЗапросаТоварыДокументаБезСерийСНомеромГруппыЗатрат("ОтчетПереработчика2_5", ".МатериалыИРаботы");
...
Функция ТаблицаСырьеПоСпецификациям(ДокументСсылка, ИмяДокумента, ИмяТЧ, ДетализацияДоНомераГруппыЗатрат = Ложь)
	...
	Если ДетализацияДоНомераГруппыЗатрат Тогда
		ТаблицаСырьеПоСпецификациям.Колонки.Добавить("НомерГруппыЗатрат", ОбщегоНазначения.ОписаниеТипаЧисло(10, 0, ДопустимыйЗнак.Неотрицательный));
	КонецЕсли;
...
Функция ТекстЗапросаСырьеСНомеромГруппыЗатрат(ИмяВТ)
	...
	|	ТаблицаСырьеПоСпецификациям.НомерГруппыЗатрат,
	...
	|	ВременнаяТаблица.НомерГруппыЗатрат КАК НомерГруппыЗатрат,
	...
	|	ТаблицаСырье.НомерГруппыЗатрат КАК НомерГруппыЗатрат,
	...
	|	ТаблицаСырье.Серия,
	|	ТаблицаСырье.НомерГруппыЗатрат
	|;
	|";
...
Процедура УстановитьПараметрЗапросаТаблицаСырье(Запрос, ДанныеЗаполнения, ДетализацияДоНомераГруппыЗатрат = Ложь)
	...
	ТаблицаСырьеПоСпецификациям = ТаблицаСырьеПоСпецификациям(ДанныеЗаполнения, ИмяДокумента, ИмяТЧ, ДетализацияДоНомераГруппыЗатрат);
