		...
		Если Раздел3Начисления.Следующий() Тогда
			...
			Если ПараметрыОтчета.ПравилаЗаполнения = "2025Кв1"
			 ...
			 ИЛИ ПараметрыОтчета.ПравилаЗаполнения = "2017Кв1" Тогда
				...
				НаимПоказателя100 = НСтр("ru = '6.2. суммы налога, подлежащие восстановлению в соответствии
					...
					|en = '6.2. tax amounts subject to re-application in accordance
					|with clause 3 of article 172 of the Tax Code of the Russian Federation'");
				...
				НаимПоказателя125 = НСтр("ru = 'Сумма налога, предъявленная налогоплательщику подрядными организациями (застройщиками
					...
					|en = 'The tax amount charged to the taxpayer by contractor companies (developers
					|or technical customers) during capital construction of fixed assets
					|(liquidation of fixed assets), assembly (disassembly), installation (dismantling) of fixed assets,
					|accepted for deduction in the manner specified in paragraph 5 of Article 172'");
			...
			ИначеЕсли ПараметрыОтчета.ПравилаЗаполнения = "2015Кв1" Тогда
				...
				НаимПоказателя100 = НСтр("ru = '5.2. суммы налога, подлежащие восстановлению в соответствии
					|с пунктом 3 статьи 172 Кодекса Налогового кодекса Российской Федерации';
					|en = '5.2. tax amounts subject to re-application in accordance
					|with clause 3 of article 172 of the Tax Code of the Russian Federation'");
...
Функция МакетыРасшифровкиПолей2025Кв1(Организация, Период)
	...
	РаздельныйУчетНДС = УчетнаяПолитикаПереопределяемый.РаздельныйУчетНДС(Организация, Период);
	МакетыРасшифровки = Новый Соответствие();
...
Процедура ДобавитьСведенияОПодписанте(ОсновныеСведения, ВыгружаемыйДокумент)
	...
	Если ЗначениеЗаполнено(ИФНС) И ЗначениеЗаполнено(ДанныеИФНС.Представитель) Тогда
		Если ТипЗнч(ИФНС.Представитель) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
			ФИОПодписанта = СведенияОФизЛице(ДанныеИФНС.Представитель, ВыгружаемыйДокумент.Дата);
...
Функция СведенияОбИП(ВыгружаемыйДокумент)
	...
	ДанныеОрганизации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВыгружаемыйДокумент.Организация,"НаименованиеПолное,ИндивидуальныйПредприниматель");
