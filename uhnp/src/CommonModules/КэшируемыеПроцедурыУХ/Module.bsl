...
Функция ПолучитьПараметрыОбъектаДляЗаписи(ИмяОбъекта, Повторно = Неопределено) Экспорт
	...
	ИначеЕсли Повторно = Неопределено Тогда
		...
		РаботаСОбъектамиМетаданныхУХ.ОбновитьОписаниеОбъектаБД(Неопределено,"Справочник",ИмяОбъекта,Справочники.ТипыБазДанных.ТекущаяИБ);
...
Функция ИнициализироватьТаблицуПротокола() Экспорт
	...
	ТаблицаПротокола.Колонки.Добавить("ОбъектДанных", ОбщегоНазначенияУХ.ПолучитьОписаниеТиповСтроки(0));
...
Функция ПолучитьПустыеЗначенияАналитик() Экспорт
	...
	Для Каждого ТекТип Из Метаданные.ПланыВидовХарактеристик.ВидыСубконтоКорпоративные.Тип.Типы() Цикл
		МассивПустыхЗначений.Добавить(Новый (ТекТип));		
	КонецЦикла;
...
Функция НеКонтролироватьНСИ(ИмяОбъекта) Экспорт
	
	МассивОбъектов=Новый Массив;
	МассивОбъектов.Добавить("БланкиОтчетов"); 
	МассивОбъектов.Добавить("ВидыОтчетов");
	МассивОбъектов.Добавить("ГруппыРаскрытия");
	МассивОбъектов.Добавить("ИсточникиДанныхДляРасчетов");
	МассивОбъектов.Добавить("ДокументыБД");
	МассивОбъектов.Добавить("КолонкиОтчетов");
	МассивОбъектов.Добавить("ПеречисленияБД");
	МассивОбъектов.Добавить("ПланыВидовХарактеристикБД"); 
	МассивОбъектов.Добавить("ПоказателиОтчетов");
	МассивОбъектов.Добавить("СправочникиБД");
	МассивОбъектов.Добавить("СтрокиОтчетов"); 
	МассивОбъектов.Добавить("СчетаБД");
	МассивОбъектов.Добавить("ПланыСчетовБД");
	МассивОбъектов.Добавить("РегистрыБухгалтерииБД");
	МассивОбъектов.Добавить("РегистрыНакопленияБД"); 
	МассивОбъектов.Добавить("РегистрыСведенийБД");
	МассивОбъектов.Добавить("СоответствиеВнешнимИБ");
	МассивОбъектов.Добавить("СоответствиеПолейДляОбменаВИБ");
	
	Возврат НЕ МассивОбъектов.Найти(ИмяОбъекта)=Неопределено;
	
КонецФункции // НеКонтролироватьНСИ() 

Функция ПолучитьИменаОбщихРеквизитов(ИмяОбъектаМетаданных) Экспорт 
	...
	Для каждого Реквизит Из Метаданные.ОбщиеРеквизиты Цикл
		...
		ИспользованиеРеквизита = Реквизит.Content.Find(ИмяОбъектаМетаданных);
...
Функция ПолучитьПоддерживаемыеОбъектыУниверсальныйФормат() Экспорт
	
	СоставОбмена=Новый ТаблицаЗначений;
	СоставОбмена.Колонки.Добавить("ТипОбъектаМетаданных",ПолучитьОписаниеТиповСтроки(150));
	СоставОбмена.Колонки.Добавить("ИмяОбъектаМетаданных",ПолучитьОписаниеТиповСтроки(150));
	
	ПоддерживаемыеОбъекты=ОбменДаннымиXDTOСервер.ПоддерживаемыеОбъектыФормата("СинхронизацияДанныхЧерезУниверсальныйФормат");
	ПоддерживаемыеОбъекты.Свернуть("Объект");
	
	Для Каждого Строка ИЗ ПоддерживаемыеОбъекты Цикл
		
		ДанныеОбъекта=СтрРазделить(Строка.Объект,"."); 
		
		Если ДанныеОбъекта[0]="Справочник"
			ИЛИ ДанныеОбъекта[0]="ПланВидовХарактеристик" Тогда
			
			НоваяСтрока=СоставОбмена.Добавить();
			
			НоваяСтрока.ТипОбъектаМетаданных=ДанныеОбъекта[0];
			НоваяСтрока.ИмяОбъектаМетаданных=ДанныеОбъекта[1];
			
		КонецЕсли;
		
	КонецЦикла; 
	
	ИмяПВХ="ВидыСубконтоХозрасчетные";
	
	Для Каждого Тип ИЗ Метаданные.ПланыВидовХарактеристик[ИмяПВХ].Тип.Типы() Цикл
		
		СтрокаТип=Метаданные.НайтиПоТипу(Тип).ПолноеИмя();
		ДанныеТипа=СтрРазделить(СтрокаТип,".");
		
		Если ДанныеТипа[0]="Перечисление" Тогда
			
			НоваяСтрока=СоставОбмена.Добавить();
			
			НоваяСтрока.ТипОбъектаМетаданных=ДанныеТипа[0];
			НоваяСтрока.ИмяОбъектаМетаданных=ДанныеТипа[1];
	...
	Для Каждого Элемент ИЗ Метаданные.ПланыСчетов Цикл 
		...
		ДанныеОбъекта=СтрРазделить(Элемент.ПолноеИмя(),".");
	...
	Возврат СоставОбмена;
		
КонецФункции // ПолучитьПоддерживаемыеОбъектыУниверсальныйФормат()
...
Функция ДатаКонцаПериода(ДатаВПериоде,Периодичность) Экспорт
	...
	ИначеЕсли Периодичность=Перечисления.Периодичность.ДевятьМесяцев Тогда
		Возврат ДобавитьМесяц(КонецГода(ДатаВПериоде),-3);	
...
Функция МассивРегистровФинансовогоУчета() Экспорт
	...
	Если НЕ Метаданные.Обработки.Найти("УстановкаСоответствияРегистров")=Неопределено Тогда
		
		ТаблицаОбъектов=Неопределено;
		
		Выполнить("ТаблицаОбъектов=Обработки.УстановкаСоответствияРегистров.ПолучитьТаблицуФинансовыхРегистров()");
...
Функция МаксКоличествоСубконто(ПланСчетовБД) Экспорт
	
	Если ЗначениеЗаполнено(ПланСчетовБД) Тогда
		
		Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПланСчетовБД,"МаксимальноеКоличествоСубконто");
...
КонецФункции // МаксКоличествоСубконто() 

Функция ПолучитьДанныеРегистра(РегистрБухгалтерии) Экспорт
	...
	Запрос.Текст="ВЫБРАТЬ
	             ...
	             |	РегистрыБухгалтерииБД.Ресурсы.(
	             |		Имя КАК Имя,
	             |		Синоним КАК Синоним,
	             |		Балансовый КАК Балансовый
	             |	) КАК Ресурсы,
	             |	РегистрыБухгалтерииБД.Измерения.(
	             |		Имя КАК Имя,
	             |		Балансовый КАК Балансовый,
	             |		Синоним КАК Синоним
	             |	) КАК Измерения
