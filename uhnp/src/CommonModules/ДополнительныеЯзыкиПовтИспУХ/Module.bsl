...
Функция ПолучитьДанныеДляФормул() Экспорт
	
	СтруктураДанных=Новый Структура;
	
	Если ОбщегоНазначенияКлиентСерверУХ.ИмяКонфигурации() = "Perform" Тогда // Это Perforn
		
		СтруктураДанных.Вставить("КодЯзыка","en");
		
	Иначе
			
		СтруктураДанных.Вставить("КодЯзыка",ОбщегоНазначения.КодОсновногоЯзыка());
		
	КонецЕсли;
	
    Языки = Константы.ДополнительныеЯзыкиВыводаОтчета.Получить().Получить();
	
	Если НЕ ТипЗнч(Языки)=Тип("ТаблицаЗначений") Тогда
		
		СтруктураДанных.Вставить("НомерЯзыка",0);
		Возврат СтруктураДанных;
		
	КонецЕсли;
			
	СтрЯзыкФормул=Языки.Найти(СтруктураДанных.КодЯзыка,"КодЯзыка"); 
	
	Если СтрЯзыкФормул=Неопределено Тогда
		
		СтруктураДанных.Вставить("НомерЯзыка",0);
		
	Иначе
		
		СтруктураДанных.Вставить("НомерЯзыка",СтрЯзыкФормул.ПорядковыйНомер);
		
	КонецЕсли;
	
	Возврат СтруктураДанных;
		
КонецФункции // ПолучитьДанныеДляФормул()
