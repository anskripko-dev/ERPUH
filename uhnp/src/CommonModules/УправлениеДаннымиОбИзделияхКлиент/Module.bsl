...
Процедура СохранитьРазвернутыеУровниДерева(Форма, СтрокаДерева = Неопределено) Экспорт
	...
	Форма.РазвернутыеУровниДереваКэш.РазвернутыеУровни = Новый Структура;
...
Процедура ВосстановитьРазвернутыеУровниДерева(Форма, СтрокаДерева = Неопределено) Экспорт
	...
	РазвернутыеУровни = Форма.РазвернутыеУровниДереваКэш.РазвернутыеУровни;
	Если РазвернутыеУровни = Неопределено ИЛИ РазвернутыеУровни.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура СохранитьСостояниеУровняДерева(Форма, Уровень, МассивИндексов, Дерево)
	...
	Для Индекс = 0 По СтрокиДерева.Количество() - 1 Цикл
		...
		Если Развернут ИЛИ Текущий Тогда
			...
			РазвернутыеУровни.Вставить("_"+Формат(РазвернутыеУровни.Количество(),"ЧН=0; ЧГ="), ДанныеСтроки);
...
Процедура ВосстановитьСостояниеУровняДерева(Форма, Уровень, МассивИндексов, Дерево)
	...
	Для Индекс = 0 По СтрокиДерева.Количество() - 1 Цикл
		...
		Для каждого КлючИЗначение Из РазвернутыеУровни Цикл
...
//  СПодчиненными - Булево - С подчиненными
Процедура РазвернутьВсеУровниДерева(Форма, СПодчиненными = Истина) Экспорт
	
	Форма.РазвернутыеУровниДереваКэш.РежимРазвернутьВсе = 1;
	
	СтрокиДерева = Форма.ДеревоСпецификаций.ПолучитьЭлементы();
	Для каждого СтрокаДерева Из СтрокиДерева Цикл
		Форма.Элементы.ДеревоСпецификаций.Развернуть(СтрокаДерева.ПолучитьИдентификатор(), СПодчиненными);
	КонецЦикла;
	
	Если Форма.РазвернутыеУровниДереваКэш.РежимРазвернутьВсе > 1 Тогда
		КонтрольныйУровень = Форма.РазвернутыеУровниДереваКэш.КонтрольныйУровень;
		Оповещение = Новый ОписаниеОповещения("РазвернутьВсеУровниДереваЗавершение", ЭтотОбъект, Новый Структура("Форма", Форма));
		ПоказатьВопрос(Оповещение,
			СтрШаблон(НСтр("ru = 'Достигнуто состояние дерева в %1 развернутых уровней. Продолжить?';
							|en = 'A tree state of %1 expanded levels is reached. Continue?'"), Формат(КонтрольныйУровень,"ЧГ=;")),
			РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
	
	Форма.РазвернутыеУровниДереваКэш.РежимРазвернутьВсе = 0;
	
КонецПроцедуры

Процедура РазвернутьВсеУровниДереваЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	Форма = ДополнительныеПараметры.Форма;
	
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		
		УправлениеДаннымиОбИзделияхКлиентСервер.ПересчитатьКонтрольныйУровеньДерева(Форма);
		
		РазвернутьВсеУровниДерева(Форма);
		
	КонецЕсли;
	
КонецПроцедуры

// Свернуть все уровни дерева.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
Процедура СвернутьВсеУровниДерева(Форма) Экспорт
	
	СтрокиДерева = Форма.ДеревоСпецификаций.ПолучитьЭлементы();
	Для каждого СтрокаДерева Из СтрокиДерева Цикл
		Форма.Элементы.ДеревоСпецификаций.Свернуть(СтрокаДерева.ПолучитьИдентификатор());
	КонецЦикла; 
...
Процедура ДеревоСпецификацийПередРазворачиванием(Форма, Идентификатор, Отказ) Экспорт
	...
	РазвернутыеУровниДереваКэш = Форма.РазвернутыеУровниДереваКэш;
	Если РазвернутыеУровниДереваКэш.РежимРазвернутьВсе > 0 Тогда
		КонтрольныйУровень = РазвернутыеУровниДереваКэш.КонтрольныйУровень;
		Если ТекущиеДанные.Уровень >= КонтрольныйУровень Тогда
			РазвернутыеУровниДереваКэш.РежимРазвернутьВсе = 2;
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	Иначе
		УправлениеДаннымиОбИзделияхКлиентСервер.ПересчитатьКонтрольныйУровеньДерева(Форма, ТекущиеДанные);
	КонецЕсли;
