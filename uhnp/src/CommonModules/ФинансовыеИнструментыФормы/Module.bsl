...
Процедура УстановитьУсловноеОформлениеФормыГрафика(УсловноеОформление, ОписаниеГрафика, ОперацииГрафика =  Неопределено) Экспорт
		...
		УстановитьУсловноеОформлениеНачальныхОстатков(УсловноеОформление, ОписаниеГрафика, ОперацииГрафика);
...
Процедура ПриСозданииНаСервере_СчетаУчета(Форма) Экспорт
	...
	ЭтоНоваяВерсия = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "ДокументОснование") 
						И ЗначениеЗаполнено(Объект.ДокументОснование);
	Если Форма.Параметры.Ключ.Пустая() И НЕ ЭтоНоваяВерсия Тогда // Заполним счета при создании ФИ
...
Процедура ДобавитьЭлементУсловногоОформленияЕстьКорректировка(УсловноеОформление, ИмяКолонки)
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ГрафикРасчетов" + ИмяКолонки);
	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		СтрШаблон("ГрафикРасчетов.%1Корректировка", ИмяКолонки), ВидСравненияКомпоновкиДанных.Заполнено,,,Истина);
...
Процедура УстановитьУсловноеОформлениеНачальныхОстатков(УсловноеОформление, ОписаниеГрафика, ОперацииГрафика)
	
	Если ОперацииГрафика = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ОперацииВводаНачальныхОстатков = ОперацииВводаНачальныхОстатков(ОперацииГрафика); 
	
	Если Не ОперацииВводаНачальныхОстатков.Количество() Тогда
		Возврат;
	КонецЕсли;
	
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ГрафикРасчетовЛинейныйДата");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ГрафикРасчетовЛинейныйСумма");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ГрафикРасчетовЛинейныйСуммаРасчет");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ГрафикРасчетовЛинейныйОперацияГрафика");
	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор, "Объект.ГрафикРасчетов.ОперацияГрафика", ВидСравненияКомпоновкиДанных.ВСписке, ОперацииВводаНачальныхОстатков,,Истина);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
	
КонецПроцедуры

Функция ОперацииВводаНачальныхОстатков(ОперацииГрафика)
	
	Результат = Новый СписокЗначений;
	
	Для Каждого ЭлементОперация Из ОперацииГрафика Цикл  
		
		Операция = ЭлементОперация.Ключ;
		ОписаниеОперации = ЭлементОперация.Значение;
		
		Если ОписаниеОперации.ЭтоВводНачальныхОстатков И ОписаниеОперации.ФактическийВводНачальныхОстатков Тогда 
			Если Результат.НайтиПоЗначению(Операция) = Неопределено Тогда
