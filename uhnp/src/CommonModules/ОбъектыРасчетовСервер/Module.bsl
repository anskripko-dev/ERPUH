			...
			Если НЕ ЗначениеЗаполнено(ОбъектРасчетов) И РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
				ВызватьИсключение (СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Не удалось найти объект расчетов для договора %1.';
										|en = 'Cannot find the AR/AP object for the contract: %1.'"),
									Договор));
			КонецЕсли;
			...
			Если НЕ ЗначениеЗаполнено(ОбъектРасчетов) И РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
				ВызватьИсключение (СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Не удалось найти объект расчетов для %1. Проверьте соответствие ключевых реквизитов заказу.';
										|en = 'Cannot find the AR/AP object for %1. Check whether the key attributes match the order.'"),
									ЗаказОснование));
			КонецЕсли;
			...
			Если НЕ ЗначениеЗаполнено(ОбъектРасчетов) И РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
				ВызватьИсключение (СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Не удалось найти объект расчетов для %1.';
										|en = 'Cannot find the AR/AP object for %1.'"),
									Объект));
			КонецЕсли;
		Иначе
			// Очищаем реквизит
		...
		Если НЕ ЗначениеЗаполнено(ОбъектРасчетов) И Не ОбъектРасчетовНеНужноЗаполнять 
			И (ЭтоСправочник И Не ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Объект, "Объект.ПометкаУдаления")
				...
				ВызватьИсключение (НСтр("ru = 'Не удалось найти объект расчетов.';
										|en = 'Cannot find the AR/AP object.'"))
		КонецЕсли;
