		...
		Для Каждого СтруктураОстаток Из ОстаткиСерийПоТовару Цикл
			...
			Если ОстатокТовара > 0 Тогда
				...
				Если ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.ЭтапПроизводства2_2"
					Или ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Обработка.РедактированиеПоследовательностиЭтаповПроизводства" Тогда
					ИсключаемыеСвойства = ИсключаемыеСвойства + ",КлючНоменклатура";
				КонецЕсли;
				//-- НЕ УТКА
...
Процедура ЗаполнитьСерииПоFEFOВМаршрутномЛисте(Объект, ПараметрыУказанияСерий)
	...
	Для Каждого СтрокаТоваров Из СтрокиТоваров Цикл
		...
		Для Каждого СтрокаОстатокСерии Из НайденныеОстаткиСерий Цикл
			...
			СтрокиКОбработке.Добавить(ИзменяемаяСтрока);
