...
Функция СформироватьСтруктуруПараметровФайлаВыгрузкиНаСервере(Адрес, ВерФормВыгрузки) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление временного файла регламентированной отчетности';
										|en = 'Delete a temporary file of local reporting'"),
	...
	Исключение
		...
		Иначе
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Журнал отчетов (статусы): ошибка записи';
											|en = 'Report log (statuses) : writing error'",
	...
	Если Подключен Тогда
		Попытка
			...
			Если ЗначениеЗаполнено(КодФСГС) И Не ЗначениеЗаполнено(Справочники.ОрганыФСГС.НайтиПоКоду(КодФСГС)) Тогда 
				Сообщение = "ru = 'Код органа ФСГС %1 подразделения не найден в справочнике органов ФСГС, проверьте наличие подключенного направления сдачи отчетности';
							|en = 'FSSS authority code %1 for operating unit is not found in FSSS authority catalog. Check if the reporting submission direction is connected'";
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка получения кода ФСГС подразделения';
											|en = 'The error is occurred when the FSO business unit code  is received'"),
