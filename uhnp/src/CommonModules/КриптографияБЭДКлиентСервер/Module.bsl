...
Функция ВидОшибкиПодписьНеверна() Экспорт
	...
	ОписаниеРешения = НСтр("ru = '<a href = ""%1"">Выполните</a> диагностику криптографии. Если это не помогло решить проблему,
			...
			|en = '<a href = ""%1"">Perform</a> cryptography diagnostics. If that did not solve the problem,
			|read the articles <a href = ""%2"">here</a> and <a href = ""%3"">here</a>.'");
...
Функция ВидОшибкиКриптография() Экспорт
	...
	ОписаниеРешения = СтрШаблон(НСтр("ru = '<a href = ""%1"">Выполните</a> диагностику криптографии';
									|en = '<a href = ""%1"">Diagnose</a> cryptography'"), "Выполните");
...
Функция ВидОшибкиПодписаниеНеквалифицированнымСертификатом()
	...
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Сертификат не прошел проверку';
										|en = 'The certificate failed verification'");
	ВидОшибки.ОписаниеПроблемы = НСтр("ru = 'Документ нельзя подписать выбранным сертификатом';
										|en = 'The document cannot be signed with the selected certificate'");
	ВидОшибки.ОписаниеРешения = НСтр("ru = 'Повторите подписание другим сертификатом.
									 ...
									 |en = 'Repeat signing with another certificate.
									 |For example, use a person certificate and machine-readable letter of authority
									 |or a CEO''s certificate.'");
	Возврат ВидОшибки;
КонецФункции
...
Функция ВидОшибкиНеквалифицированнаяЭлектроннаяПодпись()
	...
	ВидОшибки.ОписаниеПроблемы = НСтр("ru = 'Документ подписан неквалифицированным сертификатом';
										|en = 'The document is signed with an unqualified certificate'");
	ВидОшибки.ОписаниеРешения = НСтр("ru = 'Повторите подписание другим сертификатом.
									 |Используйте, например, сертификат физического лица и машиночитаемую доверенность
									 |или сертификат директора.';
									 |en = 'Repeat signing with another certificate.
...
Процедура ЗаполнитьОписаниеОшибкиПроваленнымиКритериями(ЧастиТекстаОшибки, Сертификат, Протокол,
	...
	Если Не Критерии.Общие.СертификатПроходитПроверкуМенеджераКриптографии Тогда
		ЧастиТекстаОшибки.Добавить(СтрШаблон(НСтр(
			...
			|	%1';
			|en = ' • the certificate failed the cryptography check:
	...
	Если Не Критерии.Общие.СертификатДействуетНаДатуПроверки Тогда
		ЧастиТекстаОшибки.Добавить(НСтр("ru = ' • Требуемый сертификат не прошел проверки по сроку действия при сверке с системными часами или временем подписи в файле';
										|en = ' • The required certificate did not pass the validity period checks when compared against system clock or signature time in the file'"));
	КонецЕсли;
	...
	Если Не Критерии.Общие.КлючЭлектроннойПодписиДействуетНаДатуПроверки Тогда
		...
		Если ЗначениеЗаполнено(ПериодКлюча.ДатаНачала) И ЗначениеЗаполнено(ПериодКлюча.ДатаОкончания) Тогда
			ЧастиТекстаОшибки.Добавить(СтрШаблон(НСтр("ru = ' • ключ электронной подписи не действует на дату %1';
														|en = ' • the digital signature key is not valid as of %1'"),
			ФразаДатыПроверкиВВинительномПадеже));
		Иначе
			ЧастиТекстаОшибки.Добавить(НСтр("ru = ' • в сертификате отсутствуют обязательные сведения о сроке действия ключа';
											|en = ' • the certificate does not contain the required information about the key validity period'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не Критерии.Общие.АккредитацияУЦДействуетНаДатуПроверки Тогда
		ЧастиТекстаОшибки.Добавить(СтрШаблон(НСтр(
			...
			|en = ' • the certificate authority that issued the certificate was not accredited as of %1'"),
			ФразаДатыПроверкиВВинительномПадеже));
	КонецЕсли;
	...
	Если Не Критерии.Общие.АккредитацияУЦДействуетНаДатуВыдачиСертификата Тогда
		...
		ЧастиТекстаОшибки.Добавить(СтрШаблон(НСтр(
			...
			|en = ' • the certificate authority that issued the certificate was not accredited as of the certificate issue date (%1)'"),
	...
	Если ЗначениеЗаполнено(Критерии.ИндивидуальногоПредпринимателя) И Не Критерии.ИндивидуальногоПредпринимателя.ВыданОрганомИсполнительнойВласти
		...
		ЧастиТекстаОшибки.Добавить(НСтр("ru = ' • сертификат выдан коммерческим удостоверяющим центром';
										|en = ' • the certificate was issued by a commercial certificate authority'"));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Критерии.СертификатаСотрудника) Тогда
		Если Не Критерии.СертификатаСотрудника.ВыданДо31082023 Тогда
			ЧастиТекстаОшибки.Добавить(НСтр("ru = ' • Сертификат не действует, так как выдан коммерческим аккредитованным удостоверяющим центром после 31.08.2023';
											|en = ' • The certificate is invalid as it is issued by a commercial accredited CA after 08/31/2023'"));
		КонецЕсли;
		Если Не Критерии.СертификатаСотрудника.ИспользуетсяДо01092024 Тогда
			Шаблон = НСтр("ru = ' • запрещено использование сертификата сотрудника после %1';
							|en = ' • cannot use the employee''s certificate after %1'");
...
Функция СформироватьОшибкуПроверкиСертификата(Сертификат, Протокол)
	...
	ШаблонОшибки = НСтр("ru = 'Сертификат не признан квалифицированным на дату проверки %1 по %2:
						|%3';
						|en = 'The certificate is not recognized as qualified on the check date (%1) by %2:
						|%3'");
	ФразаПричиныВНужномЧисле = ?(ЧастиТекстаОшибки.Количество() = 1, НСтр("ru = 'причине';
																			|en = 'reason'"), НСтр(
		"ru = 'следующим причинам';
		|en = 'following reasons'"));
...
Функция СформироватьОшибкуПроверкиПодписи(Сертификат, Протокол)
	...
	Иначе
		ШаблонОшибки = НСтр("ru = 'Сертификат не признан квалифицированным на дату проверки* %1 по %2:
							...
							|%3
							|
							|* The signature validity is checked on the check date, as the moment of signing the document is unknown
							|(there is no timestamp in the signature data)'");
	КонецЕсли;
	ФразаПричиныВНужномЧисле = ?(ЧастиТекстаОшибки.Количество() = 1, НСтр("ru = 'причине';
																			|en = 'reason'"), НСтр(
		"ru = 'следующим причинам';
	...
	ВидОперации = СтрШаблон(НСтр("ru = 'Проверка подписи, выполненной сертификатом:
								 |%1';
								 |en = 'Check the signature executed with the certificate:
								 |%1'"), МодульЭлектроннаяПодпись().ПредставлениеСертификата(
		Сертификат));
