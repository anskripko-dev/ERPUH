...
Функция НаименованиеПоКоду(Знач Код) Экспорт
	...
	ЗАГЛАВНЫЕБУКВЫ = "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯABCDEFGHIJKLMNOPQRSTUVWXYZ"; // @Non-NLS
	Код = СокрЛП(Код);
	Наименование = "";
	Для Инд = 1 По СтрДлина(Код) Цикл
		ТекСимв = Сред(Код, Инд, 1);
		Если СтрНайти(ЗаглавныеБуквы, ТекСимв) <> 0 Тогда // текущий символ - заглавный
...
Функция СтрокаМожетИспользоватьсяВКачествеКода(Знач Стр) Экспорт
	...
	РусАнглАлфавит = "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯABCDEFGHIJKLMNOPQRSTUVWXYZ"; // @Non-NLS
	РусАнглАлфавитИВспомогательныеСимволы = "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯABCDEFGHIJKLMNOPQRSTUVWXYZ_0123456789"; // @Non-NLS
...
Функция ПолучитьИмяФайлаПоВидуОтчета(ВидОтчета) Экспорт
	...
	Иначе
		...
		ПротоколируемыеСобытияУХ.ДобавитьЗаписьОшибка("ОбщийМодуль.ОбщегоНазначения.ПолучитьИмяФайлаПоВидуОтчета.1", , , ТекстСообщения);
		Возврат "";
...
Функция СформироватьТекстОтбора(ДеревоПараметровОтбора,ПостфиксДтКт="",ОсновнаяТаблицаРегистра=Ложь,ТолькоРегистратор=Ложь,мТаблицаПоказателейБД=Неопределено) Экспорт
	...
	Для Каждого СтрокаПараметр Из ДеревоПараметровОтбора.Строки Цикл
		...
		ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ГруппаИ Тогда
			...
			Если СтрокаПараметр.Строки.Количество()>0 Тогда
				...
				ТекстОтбор=ТекстОтбор+" И("+Сред(ТекстГруппа,СтрДлина("И")+2)+")"; 
		...
		ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ГруппаНЕ Тогда
			...
			Если СтрокаПараметр.Строки.Количество()>0 Тогда
				...
				ТекстГруппа=ПолучитьТекстГруппыУсловий(СтрокаПараметр.Строки,СтрокаПараметр.СпособВычисленияПараметра,НомерПараметра,ПостфиксДтКт,ТолькоРегистратор,мТаблицаПоказателейБД);
				ТекстОтбор=ТекстОтбор+" И НЕ("+Сред(ТекстГруппа,СтрДлина("И")+2)+")";
		...
		ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ГруппаИЛИ Тогда
			...
			Если СтрокаПараметр.Строки.Количество()>0 Тогда
				...
				ТекстГруппа=ПолучитьТекстГруппыУсловий(СтрокаПараметр.Строки,СтрокаПараметр.СпособВычисленияПараметра,НомерПараметра,ПостфиксДтКт,ТолькоРегистратор,мТаблицаПоказателейБД);
				ТекстОтбор=ТекстОтбор+" И ("+Сред(ТекстГруппа,СтрДлина("ИЛИ")+2)+")"; 
	...
	Возврат Сред(ТекстОтбор,СтрДлина("И") + 2);
		
КонецФункции // СформироватьТекстОтбора()

Функция ПолучитьТекстПриведенияКТипу(ПолеОтбора,СтрокаПараметр,мТаблицаПоказателейБД)
	
	МассивПутей=СтрРазделить(ПолеОтбора,".");
	ТипЗначения="";
	
	Если МассивПутей.Количество()=1 И СтрНайти(СтрокаПараметр.ПолеБД,"@")=0 Тогда
...
Функция ПреобразоватьПолеБД(СтрокаПараметр,мТаблицаПоказателейБД) Экспорт
	...
	Иначе
		...
		ПолеОтбора=Лев(СтрокаПараметр.ПолеБД,СтрНайти(СтрокаПараметр.ПолеБД,"@")-1);
	...
	ИначеЕсли (СтрНайти(ПолеОтбора,"Аналитика")>0 ИЛИ СтрНайти(ПолеОтбора,"Субконто")>0) Тогда
		
		Возврат ПолучитьТекстПриведенияКТипу(ПолеОтбора,СтрокаПараметр,мТаблицаПоказателейБД);
...
Функция ПолучитьТекстГруппыУсловий(КоллекцияСтрок,СпособВычисленияПараметра,НомерПараметра,ПостфиксДтКт,ТолькоРегистратор,мТаблицаПоказателейБД)
	...
	Для Каждого СтрокаПараметр Из КоллекцияСтрок Цикл
		...
		Если СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ГруппаИ Тогда
			...
			ТекстГруппа=ПолучитьТекстГруппыУсловий(СтрокаПараметр.Строки,СтрокаПараметр.СпособВычисленияПараметра,НомерПараметра,ПостфиксДтКт,ТолькоРегистратор,мТаблицаПоказателейБД);
			ТекстОтбор=ТекстОтбор+ТекстПредлог+"("+Сред(ТекстГруппа,СтрДлина("И")+2)+")";
			
		ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ГруппаНЕ Тогда
			
			ТекстГруппа=ПолучитьТекстГруппыУсловий(СтрокаПараметр.Строки,СтрокаПараметр.СпособВычисленияПараметра,НомерПараметра,ПостфиксДтКт,ТолькоРегистратор,мТаблицаПоказателейБД);
			ТекстОтбор=ТекстОтбор+ТекстПредлог+" НЕ("+Сред(ТекстГруппа,СтрДлина("И")+2)+")";
			
		ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ГруппаИЛИ Тогда
			
			ТекстГруппа=ПолучитьТекстГруппыУсловий(СтрокаПараметр.Строки,СтрокаПараметр.СпособВычисленияПараметра,НомерПараметра,ПостфиксДтКт,ТолькоРегистратор,мТаблицаПоказателейБД);
			ТекстОтбор=ТекстОтбор+ТекстПредлог+"("+Сред(ТекстГруппа,СтрДлина("ИЛИ")+2)+")";
...
Функция ПолучитьКонструкциюОтбора(СтрокаПараметр,мТаблицаПоказателейБД) Экспорт
	...
	Если СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.СохраненнаяНастройкаОтбора Тогда
		
		Возврат "%"+СокрЛП(СтрокаПараметр.УточнениеСпособаОпределения.Код)+"%#"+ПолеОтбора+"%"+СокрЛП(СтрокаПараметр.УточнениеСпособаОпределения.Код)+"%";
		
	ИначеЕсли СтрНайти(СтрокаПараметр.ПолеБД,"@")>0 Тогда
		
		МассивПутей=СтрРазделить(СтрокаПараметр.ПолеБД,"@");
		Возврат ПолеОтбора+" В (ВЫБРАТЬ Ссылка ИЗ "+СтрЗаменить(МассивПутей[1],"#","_")+"_"+СтрокаПараметр.ИмяПараметра+")";
	
	ИначеЕсли ТипЗнч(СтрокаПараметр.УточнениеСпособаОпределения)=Тип("СписокЗначений")
		...
		ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.СписокТиповЗначений Тогда
			...
			Возврат "("+Сред(ТекстУсловия,СтрДлина("ИЛИ")+3)+")";
	...
	Иначе
		...
		ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.НеРавноФиксированномуЗначению Тогда
			...
			Если ЗначениеЗаполнено(СтрокаПараметр.НастройкаСоответствия) ИЛИ СтрокаПараметр.ПолеБД="КоррСчетОборот" Тогда
...
Функция ПолучитьПредставлениеОтбора(СтрокаПараметр) Экспорт 
	...
	Если ТипЗнч(СтрокаПараметр.УточнениеСпособаОпределения)=Тип("СписокЗначений")
		...
		Если СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.НеВСпискеПоИерархии Тогда
			...
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '(НЕ %1 В ИЕРАРХИИ(%2))'"), ТекстПоля, ПолучитьТекстОтбора(СтрокаПараметр.УточнениеСпособаОпределения));
			
		ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.НеВСпискеФиксированныхЗначений Тогда			
			
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '(НЕ %1 В (%2))'"), ТекстПоля, ПолучитьТекстОтбора(СтрокаПараметр.УточнениеСпособаОпределения));
			
		ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.СписокПоИерархии Тогда
			
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 В ИЕРАРХИИ(%2)'"), ТекстПоля, ПолучитьТекстОтбора(СтрокаПараметр.УточнениеСпособаОпределения));
			
		Иначе
			
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 В (%2)'"), ТекстПоля, ПолучитьТекстОтбора(СтрокаПараметр.УточнениеСпособаОпределения));
	...
	Иначе
		...
		Если СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ПериметрОтчета Тогда
			...
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 В ПЕРИМЕТРЕ %2'"), ТекстПоля, ПолучитьТекстОтбора(СтрокаПараметр.УточнениеСпособаОпределения));
			
		ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.НеПериметрОтчета Тогда

			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1НЕ В ПЕРИМЕТРЕ %2'"), ТекстПоля, ПолучитьТекстОтбора(СтрокаПараметр.УточнениеСпособаОпределения));
...
Функция ПолучитьТекстОтбора(ДанныеОтбора) Экспорт
	...
	Если ТипЗнч(ДанныеОтбора)=Тип("СписокЗначений") Тогда
		...
		Для Каждого Строка Из ДанныеОтбора Цикл
			...
			ТекстОтбора=ТекстОтбора+","+?(ЗначениеЗаполнено(Строка.Представление),Строка.Представление,Строка.Значение);
			
		КонецЦикла; 
		
	ИначеЕсли ТипЗнч(ДанныеОтбора)=Тип("Структура") Тогда
		
		Если ДанныеОтбора.Свойство("МассивОтбора") Тогда 
			
			Для Каждого Элемент Из ДанныеОтбора.МассивОтбора Цикл 
				
				Если Элемент.Свойство("Ссылка") Тогда
					
					ТекстЭлемента=СтрЗаменить(СтрЗаменить(Элемент.Ссылка,"ЗНАЧЕНИЕ(",""),")","");
					
				Иначе
					
					ТекстЭлемента=Элемент.Наименование;
					
				КонецЕсли;
				
				ТекстОтбора=ТекстОтбора+","+ТекстЭлемента;
				
			КонецЦикла;
			
		Иначе
			
			ТекстОтбора=ТекстОтбора+","+ДанныеОтбора.ПредставлениеЗначения;
	...
	Для Каждого СтрокаПараметр Из ДеревоПараметровОтбора.Строки Цикл
		...
		Если СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ГруппаИ Тогда
			...
			Если СтрокаПараметр.Строки.Количество()>0 Тогда
				...
				ТекстГруппа=ПолучитьПредставлениеГруппыУсловий(СтрокаПараметр.Строки,СтрокаПараметр.СпособВычисленияПараметра,НомерПараметра);
				ТекстОтбор=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 И(%2)'"), ТекстОтбор, Сред(ТекстГруппа,СтрДлина("И")+2));
		...
		ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ГруппаНЕ Тогда
			...
			Если СтрокаПараметр.Строки.Количество()>0 Тогда
				...
				ТекстГруппа=ПолучитьПредставлениеГруппыУсловий(СтрокаПараметр.Строки,СтрокаПараметр.СпособВычисленияПараметра,НомерПараметра);
				ТекстОтбор=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 И НЕ(%2)'"), ТекстОтбор, Сред(ТекстГруппа,СтрДлина("И")+2));
		...
		ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ГруппаИЛИ Тогда
			...
			Если СтрокаПараметр.Строки.Количество()>0 Тогда
				...
				ТекстГруппа=ПолучитьПредставлениеГруппыУсловий(СтрокаПараметр.Строки,СтрокаПараметр.СпособВычисленияПараметра,НомерПараметра);
				ТекстОтбор=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 И (%2)'"), ТекстОтбор, Сред(ТекстГруппа,СтрДлина("ИЛИ")+2));
		...
		Иначе
			...
			СтрокаПараметр.ИмяПараметра="ПараметрИсточника"+НомерПараметра;
			ТекстОтбор=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 И %2'"), ТекстОтбор, ПолучитьПредставлениеОтбора(СтрокаПараметр));
			НомерПараметра=НомерПараметра+1;
...
Функция СформироватьПредставлениеСопоставления(ТаблицаСоответствияБД,Префикс="С") Экспорт
	...
	Для Каждого СтрокаСопоставлено Из МассивСопоставлено Цикл
		...
		//	ИЛИ СтрокаСопоставлено.АналитикаКонсолидации="Организация") И ПустаяСтрока(ТекстПоля) Тогда
		//	
		//	Продолжить;
		//	
		//КонецЕсли;
		...
		Если СтрокаСопоставлено.АналитикаКонсолидации="АналитикаВалюта" Тогда
			...
			ТекстСопоставлено=ТекстСопоставлено+",Валюта=";
			
		Иначе
			
			ТекстСопоставлено=ТекстСопоставлено+","+СтрЗаменить(СтрокаСопоставлено.АналитикаКонсолидации,"Аналитика",Префикс)+"=";
...
Процедура ОчиститьЗаписиРегистраПараметрическихНастроек(ПравилоОбработки=Неопределено) Экспорт
...
Процедура ЗаписатьПроцедуруРасчета(СтруктураДанных, ПроверятьИспользованиеОперандов=Истина, ФормироватьДанныеРасчетовПоказателя=Истина) Экспорт
	...
	Если Не ПустаяСтрока(СтруктураДанных.ТекстПроцедуры) Тогда
		...
		Если ФормироватьДанныеРасчетовПоказателя Тогда
	...
	Если ТипЗнч(СтруктураДанных.НазначениеРасчетов)=Тип("СправочникСсылка.ПравилаОбработки") Тогда
		...
		Если ПроверятьИспользованиеОперандов Тогда 
			
			ОтметитьИспользованиеОперандов(СтруктураДанных.НазначениеРасчетов,
				СтруктураДанных.ПотребительРасчета,
				ПолучитьСписокКодовОперандов(СтруктураДанных.ТекстПроцедуры),
...
Процедура ОбновитьДанныеРасчетовПоказателя(НазначениеРасчетов,ПотребительРасчета,ФормироватьДанныеРасчетов=Истина) Экспорт
	...
	Если ФормироватьДанныеРасчетов Тогда
	...
	Иначе
		...
		НаборЗаписей[0].ДанныеРасчетовПоказателя=Новый ХранилищеЗначения(Неопределено);
...
Функция ИнвертироватьПоляДтКт(ПолеБД) Экспорт
	...
	ИначеЕсли Прав(МассивПолей[0],СтрДлина("Дт"))="Дт" Тогда
		
		МассивПолей[0]=Лев(МассивПолей[0],СтрДлина(МассивПолей[0])-СтрДлина("Дт"))+"Кт";
		
	ИначеЕсли Прав(МассивПолей[0],СтрДлина("Кт"))="Кт" Тогда
		
		МассивПолей[0]=Лев(МассивПолей[0],СтрДлина(МассивПолей[0])-СтрДлина("Кт"))+"Дт";
...
Функция ДанныеИсточникаДляСтороныПроводки(ИсточникДанных) Экспорт
...
КонецФункции // ПолучитьИсточникДляСтороныПроводки() 

Функция ОбновитьНастройкиДляСтороныПроводки(ИсточникДанных) Экспорт
		
	Результат=ДанныеИсточникаДляСтороныПроводки(ИсточникДанных);	
...
Функция ПроверитьФормулуПоказателя(СтруктураДанных,ПроверятьОперанды=Истина, КодыОперандов = Неопределено) Экспорт
	...
	Если СтрПоказательТаб.Следующий() Тогда
		...
		ИначеЕсли СтрПоказательТаб.ПоказателиТекущегоОтчета Тогда
			...
			Если ЕстьОшибкиПроверкиВыражений(СтруктураДанных, КодыОперандов) Тогда
				Возврат Ложь;
			КонецЕсли;
...
Функция ПолучитьДанныеРасчетаПоказателя(Показатель,ЗапросПоказатели,ТекстФормула) Экспорт
	...
	ЗапросПоказатели.Текст = 
		"ВЫБРАТЬ
		|	ИсточникиПравила.КодПоказателя КАК КодПоказателя,
		|	ИсточникиПравила.КодОперанда КАК КодОперанда,
		|	ИсточникиПравила.ЕстьНестандартныеОтборы КАК ЕстьНестандартныеОтборы,
		|	ИсточникиПравила.ПоказательТекущегоОтчета КАК ПоказательТекущегоОтчета
		|ИЗ
	...
	Для Каждого Показатель Из ТабПоказатели Цикл
		...
		Для Каждого Код Из Показатель.СписокИспользуемых Цикл
			...
			Если ТабПоказателиПроверка.Найти(Код.Значение,"СокрКод")=Неопределено
				...
				#Если Клиент Тогда
					ТекстОшибок=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1Формула расчета показателя %2: не найден показатель с кодом %3'"), 
			...
			Если Код.Значение=Показатель.СокрКод Тогда
				...
				#Если Клиент Тогда
					ТекстОшибок=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1Формула расчета показателя %2 использует значение самого показателя'"), 
			...
			Если Не НайденаОшибка Тогда
				...
				Если НЕ СтрИспользуемыйПоказатель=Неопределено Тогда
					...
					Если НЕ ЭтапыЦикла.Найти(Показатель.СокрКод,"Операнд")=Неопределено Тогда
						...
						#Если Клиент Тогда
							ТекстОшибок=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1: обнаружена циклическая ссылка в расчетах'"), 
							            	ТекстОшибок+Символы.ПС+Показатель.СокрКод);
						#КонецЕсли
...
Функция ПолучитьТекстИсходнойТаблицы() Экспорт
...
Процедура ПодготовитьСтрокуОтбора(СтрокаОтбора,СпособПолучения,ВерсияПлатформы) Экспорт
	...
	Если СтрокаОтбора.ПолеБД="КоррСчетОборот" Тогда
...
Процедура ПодготовитьСтрокуОтбораВИБ(СтрокаОтбора,ВерсияПлатформы) Экспорт
	...
	Если СтрокаОтбора.СпособВычисленияПараметра = Перечисления.СпособыВычисленияПараметровОперандов.ФиксированноеЗначение
		...
		Если ПустаяСтрока(СтрокаОтбора.НастройкаСоответствия) И (НЕ Лев(СтрокаОтбора.ТаблицаАналитикиВИБ,СтрДлина("Простой"))="Простой") Тогда
...
Функция ИнициализироватьТаблицуМТабПоказателиЗапросы() Экспорт
	...
	РезультатФункции.Колонки.Добавить("ТекстВременныеТаблицы");
	РезультатФункции.Колонки.Добавить("ТекстОтбора");
	РезультатФункции.Колонки.Добавить("РегистрБД");
	РезультатФункции.Колонки.Добавить("ДлинаУточненияПериода",ОбщегоНазначенияУХ.ПолучитьОписаниеТиповЧисла(1,0));
	...
	РезультатФункции.Колонки.Добавить("ОсновнаяТаблицаРегистра", ОписаниеТиповБулево);
	...
	РезультатФункции.Колонки.Добавить("УчитыватьКоррСчета", ОписаниеТиповБулево);
...
Функция ИнициализироватьКэшТаблицТиповПолей()
	
	КэшТаблицыТиповПолей = Новый ТаблицаЗначений;
	КэшТаблицыТиповПолей.Колонки.Добавить("РегистрБД");
	КэшТаблицыТиповПолей.Колонки.Добавить("Счет");
	КэшТаблицыТиповПолей.Колонки.Добавить("КоррСчет");
	КэшТаблицыТиповПолей.Колонки.Добавить("ГруппаРаскрытия");
	КэшТаблицыТиповПолей.Колонки.Добавить("ОсновнаяТаблицаРегистра", ОбщегоНазначенияУХ.ПолучитьОписаниеТиповБулево());
	КэшТаблицыТиповПолей.Колонки.Добавить("ТаблицаТипов");
	
	Возврат КэшТаблицыТиповПолей;

КонецФункции

Функция ПолучитьТаблицуТиповПолей(СтрОперанд,КэшТаблицыТипов)
		
	СтруктураПоиска=Новый Структура;
	СтруктураПоиска.Вставить("РегистрБД",СтрОперанд.РегистрБД);
	СтруктураПоиска.Вставить("Счет",СтрОперанд.Счет);
	СтруктураПоиска.Вставить("КоррСчет",СтрОперанд.КоррСчет);
	СтруктураПоиска.Вставить("ГруппаРаскрытия",СтрОперанд.ГруппаРаскрытия);
	СтруктураПоиска.Вставить("ОсновнаяТаблицаРегистра",СтрОперанд.ОсновнаяТаблицаРегистра);
	
	МассивСуществующих=КэшТаблицыТипов.НайтиСтроки(СтруктураПоиска);
	
	Если МассивСуществующих.Количество()>0 Тогда
		
		Возврат МассивСуществующих[0].ТаблицаТипов;
		
	Иначе
		
		ТаблицаТипов=Справочники.ИсточникиДанныхДляРасчетов.ПолучитьТаблицуВозможныхПолей(СтрОперанд);
		НоваяСтрока=КэшТаблицыТипов.Добавить();
...
КонецФункции // ПолучитьТаблицуТиповПолей()

Процедура ОбработатьТаблицуПолей(СтрОперанд,КэшТаблицыТипов) 
	
	ТаблицаТипов=ПолучитьТаблицуТиповПолей(СтрОперанд,КэшТаблицыТипов);
	
	Для Каждого Строка ИЗ СтрОперанд.ПравилаИспользованияПолейЗапроса Цикл
		
		Если (СтрНайти(Строка.Поле,"Аналитика")>0 ИЛИ СтрНайти(Строка.Поле,"Субконто")>0) Тогда
			
			ДобавитьКонструкциюВыразить(Строка,ТаблицаТипов);
			
		КонецЕсли;
		
	КонецЦикла;
				
КонецПроцедуры // ОбработатьТаблицуПолей()

Процедура ДобавитьКонструкциюВыразить(Строка,ТаблицаТипов)
	
	ИсходноеПолеБД=Строка.Поле;	
	МассивПутей=СтрРазделить(ИсходноеПолеБД,".");
	
	СтрокаТип=ТаблицаТипов.Найти(МассивПутей[0],"ПолеБД");
	...
	Если МассивТипов.Количество()=1 Тогда
		...
		МассивПутей[0]= "(ВЫРАЗИТЬ("+МассивПутей[0]+" КАК "+МассивТипов[0]+"))";
		Строка.Поле= СтрСоединить(МассивПутей,".");
...
КонецПроцедуры // ПреобразоватьПолеБД() 

Процедура ОбработатьДанныеОперанда(СтрОперанд,КэшТаблицыТипов)
	
	Если ТипЗнч(СтрОперанд.РегистрБД)=Тип("СправочникСсылка.РегистрыБухгалтерииБД") Тогда
		
		ОбработатьТаблицуПолей(СтрОперанд,КэшТаблицыТипов);
		
	КонецЕсли;
	
	СтрОперанд.ТекстЗапросаМодуля=Справочники.ИсточникиДанныхДляРасчетов.ПолучитьТекстИсходнойТаблицы(СтрОперанд);	
	
КонецПроцедуры // ОбработатьДанныеОперанда() 
...
Процедура ЗаполнитьТабПараметрыПоОперанду(ОперандВход, ТабПараметры) Экспорт
	...
	// Проверим и при необходимости добавим отборы по сохраненным настройкам
	
	ДополнитьДанныеСохраненнымиНастройками(ОперандВход,"ТекстОтбора");
	ДополнитьДанныеОтборамиШаблона(ОперандВход,"ТекстОтбора");
	...
	Для Каждого СтрПараметр Из ОперандВход.ТаблицаПараметровОтбораБД Цикл
		...
		Если СтрПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ИнтервалПериодов Тогда
			
			ДобавитьПараметрыОтбораПоИнтервалу(СтрПараметр,ТабПараметры,УточнениеСпособовОпределенияОперанда)
...
Процедура ДобавитьПараметрыОтбораПоИнтервалу(СтрПараметр,ТабПараметры,УточненияСпособовОпределения)
	
	ДанныеГраницаНач=УточненияСпособовОпределения.Найти(СтрПараметр.ИмяПараметра+"_1");
	ДанныеГраницаКон=УточненияСпособовОпределения.Найти(СтрПараметр.ИмяПараметра+"_2");
	
	Если ДанныеГраницаНач=Неопределено ИЛИ ДанныеГраницаКон=Неопределено Тогда
	...
	НоваяСтрока = ТабПараметры.Добавить();
	ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрПараметр,,"ИмяПараметра,СпособВычисленияПараметра,ТипРасчета");
	НоваяСтрока.ИмяПараметра=СтрПараметр.ИмяПараметра+"_1";
	НоваяСтрока.СпособВычисленияПараметра=ДанныеГраницаНач.Значение;
	НоваяСтрока.УточнениеСпособаОпределения=ДанныеГраницаНач.Представление;
	НоваяСтрока.ТипРасчета=ДанныеГраницаНач.Тип;
	
	НоваяСтрока = ТабПараметры.Добавить();
	ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрПараметр,,"ИмяПараметра,СпособВычисленияПараметра,ТипРасчета");
	НоваяСтрока.ИмяПараметра=СтрПараметр.ИмяПараметра+"_2";
	НоваяСтрока.СпособВычисленияПараметра=ДанныеГраницаКон.Значение;
	НоваяСтрока.УточнениеСпособаОпределения=ДанныеГраницаКон.Представление;
	НоваяСтрока.ТипРасчета=ДанныеГраницаКон.Тип;
		
КонецПроцедуры // ДобавитьПараметрыОтбораПоИнтервалу() 
...
Функция ПодготовитьОперандыДляПолученияДанных(мОперанды,
	...
	СтруктураПропускаемыхЗначенийРБ = Новый Структура("КодАналитики", "Значение");
	СтруктураПоискаРБ = Новый Структура("Поле,КодАналитики,ФиксированноеЗначение");
		
	ШаблонТаблицыОперандов = мОперанды.СкопироватьКолонки("Код,Ссылка,Показатель,ТипЗначения,Счет,КоррСчет,Наименование,ПравилаИспользованияПолейЗапроса,Ресурс,АналитикВидаОтчета,ПоказательОтбор,ТолькоАналитическиеЗначения,ЧислоАналитикГруппыРаскрытия,ВидИтогаПоказателя,ТипЗначенияПоказателя,ТрансляцияДебет,ТрансляцияКредит,УчитыватьКоррСчета");
	...
	КэшТаблицыТипов=ИнициализироватьКэшТаблицТиповПолей();
	...
	Для Каждого Операнд Из мОперанды Цикл
		...
		ОбработатьДанныеОперанда(Операнд,КэшТаблицыТипов);
		
		ТекстВременныеТаблицы=	Обработки.КорректировкиЗначенийПоказателей.ПодготовитьТекстВременныеТаблицыОтбор(Операнд.ТаблицаПараметровОтбораБД)+
							Обработки.КорректировкиЗначенийПоказателей.ПодготовитьТекстВременныеТаблицыПоля(Операнд.ПравилаИспользованияПолейЗапроса);
		...
		Для Каждого СтрПоле Из Операнд.ПравилаИспользованияПолейЗапроса Цикл
			...
			Если Операнд.УчитыватьКоррСчета И СтрНайти(СтрПоле.Поле,"Субконто")>0 Тогда
				
				СтрПоле.Поле=СтрЗаменить(СтрПоле.Поле,"Субконто","Субконто"+Операнд.ДтКт);
		...
		Если Не СтрРесурс=Неопределено Тогда
			...
			Если Операнд.ОсновнаяТаблицаРегистра Тогда
				
				СтрРесурс.Поле=Операнд.РесурсРегистра;
		...
		Если ГруппироватьОперанды
			...
			И (НЕ Операнд.УчитыватьКоррСчета) 
				...
				Если Операнд.СпособПолучения=СтрокаОтбор.СпособПолучения И (НЕ (РазделятьПоПоказателям И СтрокаОтбор.ТаблицаОперандов.Найти(Операнд.Показатель,"Показатель")=Неопределено)) Тогда
					...
					Иначе 
						...
						Если Операнд.ТекстЗапросаМодуля=СтрокаОтбор.ТекстЗапроса
							...
							Если (Не СравниватьТаблицыПараметров) И ОбщегоНазначенияУХ.СравнитьТаблицыЗначений(Операнд.ПравилаИспользованияПолейЗапроса, 
								СтрокаОтбор.ТабПравилаИспользованияПолей[0].ПравилаИспользованияПолей,
								СтруктураПропускаемыхЗначенийРБ) Тогда
							...
							ИначеЕсли ОбщегоНазначенияУХ.СравнитьТаблицыВычисленияПараметров(ТабПараметры,СтрокаОтбор.ПравилаВычисленияПараметров)
								...
								Если Операнд.СпособПолучения= Перечисления.СпособыПолученияОперандов.ВнешниеДанныеРегистрБухгалтерии
									...
									Для Каждого СтрокаПравило Из СтрокаОтбор.ТабПравилаИспользованияПолей Цикл
										...
										Если ОбщегоНазначенияУХ.СравнитьТаблицыЗначений(Операнд.ПравилаИспользованияПолейЗапроса, 
											СтрокаПравило.ПравилаИспользованияПолей,СтруктураПропускаемыхЗначенийРБ) Тогда
		...
		Если НЕ ЕстьАналогичныйОтбор Тогда
			...
			НовыйОтбор.ОсновнаяТаблицаРегистра		= Операнд.ОсновнаяТаблицаРегистра;
			НовыйОтбор.ТекстВременныеТаблицы        = ТекстВременныеТаблицы;
			НовыйОтбор.УчитыватьКоррСчета           = Операнд.УчитыватьКоррСчета;
	...
	Для Каждого СтрЗапрос Из МассивОтбор Цикл
		...
		Для Каждого СтрОперанд Из СтрЗапрос.ТаблицаОперандов Цикл
			...
			Для Каждого СтрПоле Из СтрОперанд.ПравилаИспользованияПолейЗапроса Цикл
				...
				ИначеЕсли СтрНайти(СтрПоле.Поле,"Аналитика")>0 Тогда
					...
					НомерАналитики=Число(Сред(СтрПоле.Поле,СтрНайти(СтрПоле.Поле,"Аналитика")+СтрДлина("Аналитика"),1));
		...
		Для Каждого СтрТаблица Из ДанныеДляЗапроса Цикл
			...
			ТекстГруппировки=ТекстГруппировки+","+ИмяТаблицы+".Версия.ПериодОтчета";
		...
		СтрЗапрос.ТекстЗапроса=Сред(ТекстВыборки,СтрДлина("ОБЪЕДИНИТЬ ВСЕ") + 2);
	...
	Для Каждого СтрЗапрос Из мТабПоказателиЗапросы Цикл
		...
		Если СтрЗапрос.СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеРегистрБухгалтерии
			...
			ЕстьОбороты = СтрНайти(СтрЗапрос.ТекстЗапроса,"&ПараметрДтСчет")>0;
		...
		ИначеЕсли СтрЗапрос.СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеADO Тогда 
			...
			Если СтрЗапрос.ТабПравилаИспользованияПолей.Количество()>1 Тогда // По разному настроено соответствие, выбираем по наиболее полному
				...
				Если НЕ ЕстьСинхронизацияПоРеквизитам(СтрПравило.ПравилаИспользованияПолей) Тогда
					...
					Иначе
						...
						Если СтрЗапрос.СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеОстаткиИОборотыИзПланаСчетов Тогда
							...
							РабочаяТаблицаПолей=РасширениеИнтеграцииУХ.ПодготовитьТаблицуПолейДляЗапроса(СтрПравило.ПравилаИспользованияПолей,Истина);
							
						Иначе
							
							РабочаяТаблицаПолей=СтрПравило.ПравилаИспользованияПолей;
...
Функция ПолучитьТаблицуОперандаПоОбъекту(ИсточникОбъект) Экспорт
	...
	ТаблицаРеквизитов.Колонки.Добавить("ПересчитатьВОсновнуюВалюту");
	ТаблицаРеквизитов.Колонки.Добавить("ОсновнаяТаблицаРегистра");
	ТаблицаРеквизитов.Колонки.Добавить("ТекстОтбора");
	ТаблицаРеквизитов.Колонки.Добавить("ВидСреза");
	ТаблицаРеквизитов.Колонки.Добавить("ОбъектБД");
	ТаблицаРеквизитов.Колонки.Добавить("ТабличнаяЧастьБД");
	...
	ТаблицаРеквизитов.Колонки.Добавить("ДлинаУточненияПериода");
	ТаблицаРеквизитов.Колонки.Добавить("ГруппаРаскрытияИсточник");
	...
	ТаблицаРеквизитов.Колонки.Добавить("ТрансляцияДебет",ОбщегоНазначенияУХ.ПолучитьОписаниеТиповБулево());
	ТаблицаРеквизитов.Колонки.Добавить("ТрансляцияКредит",ОбщегоНазначенияУХ.ПолучитьОписаниеТиповБулево());
	ТаблицаРеквизитов.Колонки.Добавить("УчитыватьКоррСчета",ОбщегоНазначенияУХ.ПолучитьОписаниеТиповБулево());
	...
	НоваяСтрока.ДлинаУточненияПериода	=?(ТипЗнч(ИсточникОбъект.РегистрБД)=Тип("СправочникСсылка.РегистрыБухгалтерииБД"),ИсточникОбъект.РегистрБД.ДлинаУточненияПериода,0);
	...
	НоваяСтрока.ГруппаРаскрытияИсточник						=?(ИсточникОбъект.ПоказательОтбор.ГруппаРаскрытия				=NULL,Неопределено,ИсточникОбъект.ПоказательОтбор.ГруппаРаскрытия);
	...
	ТипБулево=ОбщегоНазначенияУХ.ПолучитьОписаниеТиповБулево();
	
	ПравилаИспользованияПолейЗапроса.Колонки.Добавить("СоздаватьНовые",ТипБулево);
	ПравилаИспользованияПолейЗапроса.Колонки.Добавить("РазделятьПоОрганизациям",ТипБулево);
	ПравилаИспользованияПолейЗапроса.Колонки.Добавить("ОбновлятьРеквизитыПриИмпорте",ТипБулево);
...
Функция ПолучитьТаблицуОперандов(НазначениеРасчетов=Неопределено,
	...
	|   ЕСТЬNULL(ВЫРАЗИТЬ(ИсточникиДанныхДляРасчетов.ПоказательОтбор КАК Справочник.ПоказателиОтчетов).ГруппаРаскрытия, ЗНАЧЕНИЕ(Справочник.ГруппыРаскрытия.ПустаяСсылка)) КАК ГруппаРаскрытияИсточник,
	...
	|	ИсточникиДанныхДляРасчетов.ПоказательТекущегоОтчета,
	|	ИсточникиДанныхДляРасчетов.ПересчитатьВОсновнуюВалюту,
	|	ИсточникиДанныхДляРасчетов.ЕстьНестандартныеОтборы,
	|	ИсточникиДанныхДляРасчетов.ВидСреза,
	...
	Если ЗначениеЗаполнено(ИсточникДанных) Тогда
		...
		Если ТипЗнч(ИсточникДанных)=Тип("Массив") Тогда
			...
			Запрос.Текст=Запрос.Текст+"
			|И ИсточникиДанныхДляРасчетов.Ссылка В(&ИсточникДанных)";
...
Функция ОбработатьТекстУпрощеннойФормулы(СтруктураДанных, ПроверятьОперанды=Истина, ПроверятьФормулы=Истина,ОбновлятьОперанды=Ложь) Экспорт
	...
	КодыОперандов = НовыеКодыОперандов();
	ИнициализироватьДанныеРазбораФормулы(СтруктураДанных, РезультатРазбора, КодыОперандов);
	ТекстДляПроверки = СтруктураДанных.ТекстДляПроверки;
	
	ТекстОшибки = "";
	Если СтруктураДанных.ПроизвольныйКод Тогда
		
		ЗаменитьОперандыНаЗначения(ТекстДляПроверки, КодыОперандов);
		
		Если ОбщегоНазначенияПовтИспУХ.ИспользуетсяМногопериоднаяОтчетность() Тогда
			ОбработкаРасчета = РасширениеБизнесЛогикиУХ.ПолучитьОбработкуЗаполненияДанных();
			ОбработкаРасчета.ПроверитьМодуль(ТекстДляПроверки, ТекстОшибки, Истина);
		Иначе
			ДокНО = Документы.НастраиваемыйОтчет.СоздатьДокумент();
			ДокНО.ПроверитьМодуль(ТекстДляПроверки, ТекстОшибки);
		КонецЕсли;
		
	КонецЕсли;
	
	Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
		ВывестиСообщениеОРезультатахПроверки(ТекстДляПроверки,, ТекстОшибки);
		Возврат Ложь;
	КонецЕсли;
	...
	Для Каждого СтрокаКод Из КодыОперандов Цикл
		...
		Если ТипЗнч(ЭлементРазбора)=Тип("Структура") Тогда
			...
			Если НастройкиОперанда.Свойство("Код") Тогда // Используется существующий источник
				...
				Если ОбновлятьОперанды Тогда 
					
					ИсточникОбъект=НастройкиОперанда.Ссылка.ПолучитьОбъект();
			...
			Иначе
				...
				Если ДанныеИсточника.Свойство("Ссылка") Тогда
					...
					СуществующийОбъект=ДанныеИсточника.Ссылка.ПолучитьОбъект();
					СуществующийОбъект.ПравилаИспользованияПолейЗапроса.Загрузить(ОбъектИсточник.ПравилаИспользованияПолейЗапроса.Выгрузить());
					СуществующийОбъект.УчитыватьКоррСчета=ОбъектИсточник.УчитыватьКоррСчета;
					СуществующийОбъект.ОбменДанными.Загрузка=Истина; 
	...
	Если ПроверятьФормулы Тогда
		...
		Возврат ПроверитьФормулуПоказателя(СтруктураДанных,ПроверятьОперанды, КодыОперандов);
...
Функция НовыеКодыОперандов()
	
	КодыОперандов = Новый ТаблицаЗначений;
	КодыОперандов.Колонки.Добавить("КодУпрощеннойФормулы");
	КодыОперандов.Колонки.Добавить("ТипЗначения");
	КодыОперандов.Колонки.Добавить("ДлинаКода", ОбщегоНазначенияУХ.ПолучитьОписаниеТиповЧисла(9));
	
	Возврат КодыОперандов;
	
КонецФункции

Процедура ИнициализироватьДанныеРазбораФормулы(СтруктураДанных, РезультатРазбора, КодыОперандов)
	
	Если ЗначениеЗаполнено(СтруктураДанных.НазначениеРасчетов.ТипБД) Тогда
		ТипБД = СтруктураДанных.НазначениеРасчетов.ТипБД;
	Иначе
		ТипБД = Справочники.ТипыБазДанных.ТекущаяИБ;
	КонецЕсли;
	
	РезультатРазбора = ОбщегоНазначенияУХ.ПолучитьРезультатРазбораТекстаФормулы(
		СтруктураДанных.ТекстПроцедуры,
		СтруктураДанных.НазначениеРасчетов,
		СтруктураДанных.ПотребительРасчета,
		СтруктураДанных.СпособИспользования,
		ТипБД,
		Неопределено,
		СтруктураДанных.ПроизвольныйКод);
	
	СравниватьТипы = Истина;
	ЗаполнитьКодыПоДаннымРазбора(КодыОперандов, РезультатРазбора, СравниватьТипы);
	
	ТекстДляПроверки = ВРег(СтруктураДанных.ТекстПроцедуры);
	СтруктураДанных.Вставить("ТекстДляПроверки", ТекстДляПроверки);
	
КонецПроцедуры

Процедура ЗаполнитьКодыПоДаннымРазбора(КодыОперандов, РезультатРазбора, СравниватьТипы = Истина)
	...
	Для Каждого КлючИЗначение Из РезультатРазбора Цикл
		...
		НоваяСтрока = КодыОперандов.Добавить();
		НоваяСтрока.КодУпрощеннойФормулы=КлючИЗначение.Ключ;
		Если КлючИЗначение.Значение.Свойство("ТипЗначения") 
			И ЗначениеЗаполнено(КлючИЗначение.Значение.ТипЗначения) Тогда
	...
	КодыОперандов.Сортировать("ДлинаКода Убыв");
	
КонецПроцедуры

Процедура ЗаменитьОперандыНаЗначения(ОбъектЗамены, КодыОперандов)
	
	ЗаменаВМассиве = Ложь;
	Если ТипЗнч(ОбъектЗамены) = Тип("Массив") Тогда
		СтрокаДляЗамены = СтрСоединить(ОбъектЗамены, "@");
		ЗаменаВМассиве = Истина;
	Иначе
		СтрокаДляЗамены = ОбъектЗамены;
	КонецЕсли;
	...
	Для Каждого СтрокаКод Из КодыОперандов Цикл
		...
		СтрокаДляЗамены = СтрЗаменить(СтрокаДляЗамены, ВРЕГ(СтрокаКод.КодУпрощеннойФормулы), ЗначениеПоУмолчанию);
		
	КонецЦикла;
	
	Если ЗаменаВМассиве Тогда
		ОбъектЗамены = СтрРазделить(СтрокаДляЗамены, "@");
	Иначе
		ОбъектЗамены = СтрокаДляЗамены;
	КонецЕсли;
	
КонецПроцедуры

Функция ЕстьОшибкиПроверкиВыражений(СтруктураДанных, КодыОперандов = Неопределено)
	
	Если КодыОперандов = Неопределено Тогда
		РезультатРазбора = Новый Соответствие;
		КодыОперандов = НовыеКодыОперандов();
		ИнициализироватьДанныеРазбораФормулы(СтруктураДанных, РезультатРазбора, КодыОперандов);
	КонецЕсли;
	
	ОшибкиПроверок = Новый Массив;
	ПроверитьВыраженияФормулы(СтруктураДанных.ТекстДляПроверки, КодыОперандов,
		ОшибкиПроверок, СтруктураДанных.ПотребительРасчета);
	ЗаменитьОперандыНаЗначения(СтруктураДанных.ТекстДляПроверки, КодыОперандов);
	
	Если ОшибкиПроверок.Количество() Тогда
		ВывестиСообщениеОРезультатахПроверки(СтруктураДанных.ТекстДляПроверки, ОшибкиПроверок);
		Возврат Истина;
	КонецЕсли;
...
Процедура ПроверитьВыраженияФормулы(ТекстДляПроверки, КодыОперандов, ОшибкиПроверок, Показатель, СравниватьТипы = Истина)
	
	УдалитьНезначащиеСимволы(ТекстДляПроверки);
	
	ЛогическиеВыражения = Новый Массив;
	ВыраженияРезультаты = Новый Массив;
	ВыраженияРезультаты.Добавить(ТекстДляПроверки);
	Если ЕстьУсловныйОператор(ТекстДляПроверки) Тогда
		ВыделитьВыраженияУсловногоОператора(ТекстДляПроверки, ЛогическиеВыражения, ВыраженияРезультаты, ОшибкиПроверок);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ЛогическиеВыражения) Тогда
		ЗаменитьОперандыНаЗначения(ЛогическиеВыражения, КодыОперандов);
		ПроверитьПреобразованиеКБулево(ЛогическиеВыражения, ОшибкиПроверок);
	КонецЕсли;
	
	ТипЗначенияПоказателя = ТипПоказателя(Показатель);
	ЗаменитьОперандыНаЗначения(ВыраженияРезультаты, КодыОперандов);
	ПроверитьВычислениеВыражений(ВыраженияРезультаты, ОшибкиПроверок, ТипЗначенияПоказателя, СравниватьТипы);
	
КонецПроцедуры

Процедура УдалитьНезначащиеСимволы(ТекстДляПроверки)
	
	ТекстДляПроверки = СокрЛП(ТекстДляПроверки);
	ТекстДляПроверки = СтрЗаменить(ТекстДляПроверки, Символы.ПС, "");
	
КонецПроцедуры

// Выделяет логические выражения и результаты условного (тернарного) оператора
// "?(ЛогическоеВыражение, ВыражениеИстина, ВыражениеЛожь)" из текста формулы.
Процедура ВыделитьВыраженияУсловногоОператора(Знач ТекстДляПроверки, ЛогическиеВыражения, ВыраженияРезультаты, ОшибкиПроверок)
	
	ПозицияНачала = НачалоУсловногоОператора(ТекстДляПроверки);
	ПозицияКонца  = КонецУсловногоОператора(ТекстДляПроверки, ПозицияНачала + 2);
	УсловныйОператор = Сред(ТекстДляПроверки, ПозицияНачала, ПозицияКонца - ПозицияНачала + 1);
	
	Выражения = ВыраженияУсловногоОператора(УсловныйОператор);
	Если Выражения.Количество() <> 3 Тогда
		ОшибкиПроверок.Добавить(НСтр("ru = 'Ошибка выделения выражений в операторе условия ""?()"". Измените формулу.'"));
		Возврат;
	КонецЕсли;
	
	ЛогическиеВыражения.Добавить(Выражения[0]);
	
	ПроверятьДелениеВыражениеИстина = Ложь;
	ПроверятьДелениеВыражениеЛожь = Ложь;
	ПроверяемыйОператор = ОператорСравненияСНулем(Выражения, ПроверятьДелениеВыражениеИстина, ПроверятьДелениеВыражениеЛожь);
	
	// Рекурсивная обработка условных операторов
	ВыражениеИстина = Выражения[1];
	Если ЕстьУсловныйОператор(ВыражениеИстина) Тогда
		ВыделитьВыраженияУсловногоОператора(ВыражениеИстина, ЛогическиеВыражения, ВыраженияРезультаты, ОшибкиПроверок);
	Иначе
		ДобавитьВыражениеРезультат(ВыражениеИстина, ВыраженияРезультаты, ПроверяемыйОператор, ПроверятьДелениеВыражениеИстина);
	КонецЕсли;
	
	ВыражениеЛожь = Выражения[2];
	Если ЕстьУсловныйОператор(ВыражениеЛожь) Тогда
		ВыделитьВыраженияУсловногоОператора(ВыражениеЛожь, ЛогическиеВыражения, ВыраженияРезультаты, ОшибкиПроверок);
	Иначе
		ДобавитьВыражениеРезультат(ВыражениеЛожь, ВыраженияРезультаты, ПроверяемыйОператор, ПроверятьДелениеВыражениеЛожь);
	КонецЕсли;
	
	Если ПозицияКонца + 1 < СтрДлина(ТекстДляПроверки) Тогда
		ТекстДляПроверки = Сред(ТекстДляПроверки, ПозицияКонца + 1);
		Если ЕстьУсловныйОператор(ТекстДляПроверки) Тогда
			ВыделитьВыраженияУсловногоОператора(ТекстДляПроверки, ЛогическиеВыражения, ВыраженияРезультаты, ОшибкиПроверок);
		КонецЕсли;
	КонецЕсли;
...
Функция ЕстьУсловныйОператор(Выражение)
	
	Возврат (СтрНайти(Выражение, "?(") > 0);
	
КонецФункции

Функция НачалоУсловногоОператора(Знач Строка)
	
	ВнутриСтроки = Ложь;
	
	Для Позиция = 1 По СтрДлина(Строка) - 1 Цикл
		Символ = Сред(Строка, Позиция, 1);
		СледующийСимвол = Сред(Строка, Позиция + 1, 1);
		
		Если Символ = """" Тогда
			ВнутриСтроки = НЕ ВнутриСтроки;
			Продолжить;
		КонецЕсли;
		
		Если НЕ ВнутриСтроки И Символ = "?" И СледующийСимвол = "(" Тогда
			Возврат Позиция;
		КонецЕсли;
	КонецЦикла;
...
Функция КонецУсловногоОператора(Знач Строка, Знач НачальнаяПозиция)
...
Функция ВыраженияУсловногоОператора(УсловныйОператор)
	
	ТекстВнутри = Сред(УсловныйОператор, 3, СтрДлина(УсловныйОператор) - 3);
	...
	Для Позиция = 1 По СтрДлина(ТекстВнутри) Цикл
		
		Символ = Сред(ТекстВнутри, Позиция, 1);
		...
		Если НЕ ВнутриСтроки Тогда
			...
			ИначеЕсли Символ = "," И УровеньВложенности = 0 Тогда
				Выражения.Добавить(СокрЛП(ТекущаяЧасть));
				ТекущаяЧасть = "";
				Продолжить;
			КонецЕсли;
...
Функция ОператорСравненияСНулем(Выражения, ПроверятьДелениеВыражениеИстина, ПроверятьДелениеВыражениеЛожь)
	
	Результат = "";
	ЛогическоеВыражение = Выражения[0];
	
	Если СтрНайти(ЛогическоеВыражение, "=") Тогда
		
		Операторы = СтрРазделить(ЛогическоеВыражение, "=<>", Ложь);
		Если Операторы.Количество() = 2 Тогда
			
			Если СокрЛП(Операторы[0]) = "0" Тогда
				ПроверятьДелениеВыражениеЛожь = Истина;
				Результат = Операторы[1];
			ИначеЕсли СокрЛП(Операторы[1]) = "0" Тогда
				ПроверятьДелениеВыражениеЛожь = Истина;
				Результат = Операторы[0];
			КонецЕсли;
			
		КонецЕсли;
		
	ИначеЕсли СтрНайти(ЛогическоеВыражение, "<") ИЛИ СтрНайти(ЛогическоеВыражение, ">") Тогда
		
		Операторы = СтрРазделить(ЛогическоеВыражение, "<>", Ложь);
		Если Операторы.Количество() = 2 Тогда
			
			Если СокрЛП(Операторы[0]) = "0" Тогда
				ПроверятьДелениеВыражениеИстина = Истина;
				Результат = Операторы[1];
			ИначеЕсли СокрЛП(Операторы[1]) = "0" Тогда
				ПроверятьДелениеВыражениеИстина = Истина;
				Результат = Операторы[0];
...
Процедура ДобавитьВыражениеРезультат(Выражение, ВыраженияРезультаты, ПроверяемыйОператор, ПроверятьДеление)
	
	Если ПроверятьДеление И СтрНайти(Выражение, "/") Тогда
		Если СтрНайти(Выражение, ПроверяемыйОператор) Тогда
			
			Выражение = СтрЗаменить(Выражение, ПроверяемыйОператор, "1");
			
		КонецЕсли;
	КонецЕсли;
	
	ВыраженияРезультаты.Добавить(Выражение);
	
КонецПроцедуры

Процедура ПроверитьПреобразованиеКБулево(ЛогическиеВыражения, ОшибкиПроверок)
	
	ЛогическиеВыражения = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ЛогическиеВыражения);
	
	Для Каждого Выражение Из ЛогическиеВыражения Цикл
		
		Попытка
			
			// АПК:487-выкл
			ЗначениеПроверки = Булево(Вычислить(Выражение));
			// АПК:487-вкл
			
		Исключение
			
			ОшибкиПроверок.Добавить(ИнформацияОбОшибке().Описание);
			Прервать;
...
Процедура ПроверитьВычислениеВыражений(ВыраженияРезультаты, ОшибкиПроверок, ТипЗначенияПоказателя, СравниватьТипы)
	
	ВыраженияРезультаты = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ВыраженияРезультаты);
	ПроверяемыеЗначения = Новый Массив;
	
	Для Каждого Выражение Из ВыраженияРезультаты Цикл
		
		Попытка
			
			// АПК:487-выкл
			ЗначениеПроверки = Вычислить(Выражение);
			// АПК:487-вкл
			ПроверяемыеЗначения.Добавить(ЗначениеПроверки);
	...
	Если СравниватьТипы Тогда
		...
		Для Каждого ЗначениеПроверки Из ПроверяемыеЗначения Цикл
			...
			Если ТипЗнч(ЗначениеПроверки) <> ТипЗначенияПоказателя Тогда
				
				ТекстПроверкиТипа = СтрШаблон(
					НСтр("ru = 'Тип значения показателя ""%1"" отличается от типа результата работы формулы ""%2"".'"),
					ТипЗначенияПоказателя, ТипЗнч(ЗначениеПроверки));
				ОшибкиПроверок.Добавить(ТекстПроверкиТипа);
...
Процедура ВывестиСообщениеОРезультатахПроверки(ТекстФормулы, ОшибкиПроверок = Неопределено, ТекстОшибки = "")
	
	Если ТипЗнч(ОшибкиПроверок) = Тип("Массив") Тогда
		
		ОшибкиПроверок = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ОшибкиПроверок);
		Для Каждого Описание Из ОшибкиПроверок Цикл
			ТекстОшибки = ТекстОшибки + ?(Не ПустаяСтрока(ТекстОшибки), Символы.ПС + Символы.Таб, "") + Описание;
		КонецЦикла;
		
	КонецЕсли;
	
	ОбработатьТекстОшибкиДелениеНаНоль(ТекстОшибки);
	
	ТекстСообщения = НСтр("ru = '%ТекстФормулы%: ошибка при разборе формулы расчета.
	|	%ОписаниеОшибки%'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТекстФормулы%", Строка(ТекстФормулы));
...
Процедура ОбработатьТекстОшибкиДелениеНаНоль(ТекстОшибки)
	
	ОшибкаДелениеНаНоль = НСтр("ru = 'Деление на 0'");
	Если СтрНайти(ТекстОшибки, ОшибкаДелениеНаНоль) Тогда
		ТекстОшибки = СтрЗаменить(ТекстОшибки, ОшибкаДелениеНаНоль, НСтр("ru = 'Не обработана проверка деления на 0.'"));
	КонецЕсли;
	
КонецПроцедуры

Функция ТипПоказателя(Показатель)
	
	ТипПеречисление = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Показатель, "ТипЗначения");
	
	Если ТипПеречисление = Перечисления.ТипыЗначенийПоказателейОтчетов.Булево Тогда
		Возврат Тип("Булево");
	ИначеЕсли ТипПеречисление = Перечисления.ТипыЗначенийПоказателейОтчетов.Строка Тогда
		Возврат Тип("Строка");
	ИначеЕсли ТипПеречисление = Перечисления.ТипыЗначенийПоказателейОтчетов.Дата Тогда
		Возврат Тип("Дата");
	иначе
		Возврат Тип("Число");
	КонецЕсли;
...
Функция ДанныеРегламентаПодготовкиОтчетности(УправлениеПериодом) Экспорт
	
	СтруктураДанных = Новый Структура;
	СтруктураДанных.Вставить("ВерсияОрганизационнойСтруктуры", Неопределено);
	СтруктураДанных.Вставить("АвтоматическиПересчитыватьЗависимыеПоказатели", Ложь);
	СтруктураДанных.Вставить("ПодставлятьЗначенияАналитикПоУмолчанию", Ложь);
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	УправлениеПериодомСценария.ВерсияОрганизационнойСтруктуры.Ссылка КАК ВерсияОрганизационнойСтруктуры,
		|	УправлениеПериодомСценария.ВерсияОрганизационнойСтруктуры.ПодставлятьЗначенияАналитикПоУмолчанию КАК ПодставлятьЗначенияАналитикПоУмолчанию,
		|	УправлениеПериодомСценария.ВерсияОрганизационнойСтруктуры.АвтоматическиПересчитыватьЗависимыеПоказатели КАК АвтоматическиПересчитыватьЗависимыеПоказатели
		|ИЗ
...
Функция ПолучитьДатуОкончанияПериода(ПериодОтчета) Экспорт
	
	Если ТипЗнч(ПериодОтчета)=Тип("Структура") Тогда
	...
	Если ТипЗнч(ПериодОтчета)=Тип("СправочникСсылка.Периоды") Тогда
		...
		|	Справочник.Периоды КАК Периоды
		|ГДЕ
		|	Периоды.Ссылка = &ПериодОтчета";
		
	Иначе
		
		Запрос.Текст="ВЫБРАТЬ
		|	МАКСИМУМ(Периоды.ДатаОкончания) КАК ДатаОкончания
		|ИЗ
		|	Справочник.Периоды КАК Периоды
		|ГДЕ
		|	Периоды.Ссылка В(&ПериодОтчета)";
	...
	Если Результат.Количество()>0 Тогда
		...
		Возврат КонецДня(Результат[0].ДатаОкончания);
		
	Иначе
		
		Возврат КонецДня(Дата(1, 1, 1));
		
	КонецЕсли;
			
КонецФункции // ПолучитьДатуОкончанияПериода()

Функция ПолучитьТекущуюПериодичность(ПериодОтчета)
	...
	Если ТипЗнч(ПериодОтчета)=Тип("СправочникСсылка.Периоды") Тогда
		...
		Результат=Запрос.Выполнить().Выгрузить();
		Возврат Результат[0].Периодичность;
		
	КонецЕсли;
		
КонецФункции // ПолучитьТекущуюПериодичность() 

Функция ПолучитьДатуНачалаПериода(ПериодОтчета) Экспорт
	...
	Иначе
		...
		Запрос.Текст="ВЫБРАТЬ
		|	МИНИМУМ(Периоды.ДатаНачала) КАК ДатаНачала
		|ИЗ
		|	Справочник.Периоды КАК Периоды
		|ГДЕ
		|	Периоды.Ссылка В(&ПериодОтчета)";
	...
	Если Результат.Количество()>0 Тогда
		...
		Возврат Результат[0].ДатаНачала;
...
Функция ПолучитьПараметрыНастройкиОтбора(НастройкаСоответствия)

	ДанныеСоответствия=ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НастройкаСоответствия,"СинхронизацияПоGUID,ТаблицаАналитикиТек,ТаблицаАналитикиВИБ");
	
	Если ДанныеСоответствия.СинхронизацияПоGUID Тогда
		
		СтруктураОтбора = Новый Структура;
		СтруктураОтбора.Вставить("СинхронизацияПоGUID",Истина);
		СтруктураОтбора.Вставить("НастройкаСоответствия",НастройкаСоответствия);
		
	Иначе
		
		СтруктураСправочники=Новый Структура;
		СтруктураСправочники.Вставить("СправочникБД",ДанныеСоответствия.ТаблицаАналитикиВИБ);
		СтруктураСправочники.Вставить("СправочникКонсолидации",ДанныеСоответствия.ТаблицаАналитикиТек);
		...
		СписокСинхронизации=МодульИнтеграции.ПолучитьТаблицуПолейДляОтбора(НастройкаСоответствия);
	...
	Возврат Новый ХранилищеЗначения(СтруктураОтбора);
	
КонецФункции // ПолучитьПараметрыНастройкиОтбора()

Функция ПолучитьСписокКоррСчетов(ЗначениеПараметра)
	...
	|ГДЕ
	|	СчетаБД.Ссылка В ИЕРАРХИИ(&ЗначениеПараметра)";
	
	Запрос.УстановитьПараметр("ЗначениеПараметра",ЗначениеПараметра);
	
	СписокСчетов=Новый СписокЗначений;
	СписокСчетов.ЗагрузитьЗначения(Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Код"));
	
	Возврат СписокСчетов;
	
КонецФункции // ПолучитьСписокКоррСчетов() 
...
Функция ПолучитьСвойстваПараметра(КонтекстОтчета,СтрокаПараметр) Экспорт
	...
	ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ВышестоящийПериод Тогда
		...
		ПериодОтчета = КонтекстОтчета.ПериодОтчета;
		Если ТипЗнч(КонтекстОтчета.ПериодОтчета) = Тип("Массив") Тогда
			ПериодОтчета = КонтекстОтчета.ПериодОтчета[0];
		КонецЕсли;
		СвойстваПараметра.Значение=ОбщегоНазначенияУХ.ПолучитьВышестоящийПериод(
			ПериодОтчета, ПолучитьЗначениеИзХранилища(СтрокаПараметр.УточнениеСпособаОпределения));
	...
	ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ДатаКонцаПериодаОтчета Тогда
		...
		СвойстваПараметра.ВидСравнения=ВидСравнения.Равно;
		СвойстваПараметра.Значение=КонецДня(ПолучитьДатуОкончанияПериода(КонтекстОтчета.ПериодОтчета));
	...
	ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ДатаНачалаПериодаОтчета Тогда
		...
		СвойстваПараметра.ВидСравнения=ВидСравнения.Равно;
		СвойстваПараметра.Значение=ПолучитьДатуНачалаПериода(КонтекстОтчета.ПериодОтчета);
		СтрокаПараметр.ИзКонтекста=Истина;
		СтрокаПараметр.ЗначениеИзКонтекста=СвойстваПараметра.Значение;
	...
	ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ПериодОтчетаСоСдвигом Тогда
		...
		СдвигПериодов=ПолучитьЗначениеИзХранилища(СтрокаПараметр.УточнениеСпособаОпределения);
		
		Если ТипЗнч(КонтекстОтчета.ПериодОтчета)=Тип("СправочникСсылка.Периоды") Тогда
			
			ОтнПериод=ОбщегоНазначенияУХ.глОтносительныйПериод(КонтекстОтчета.ПериодОтчета,СдвигПериодов);

			
		Иначе
			
			ОтнПериод=Новый Массив;
			
			Для Каждого Период ИЗ КонтекстОтчета.ПериодОтчета Цикл
				
				ОтнПериод.Добавить(ОбщегоНазначенияУХ.глОтносительныйПериод(Период,СдвигПериодов));
	...
	ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ДатаКонцаПериодаОтчетаСоСдвигом Тогда
		...
		ДатаПериода=ПолучитьДатуОкончанияПериода(КонтекстОтчета.ПериодОтчета);
		Периодичность=ПолучитьТекущуюПериодичность(КонтекстОтчета.ПериодОтчета);
		СдвигПериодов=ПолучитьЗначениеИзХранилища(СтрокаПараметр.УточнениеСпособаОпределения);
		
		СвойстваПараметра.ВидСравнения=ВидСравнения.Равно;
		СвойстваПараметра.Значение=КонецДня(ОбщегоНазначенияУХ.ДобавитьИнтервал(ДатаПериода, Периодичность, СдвигПериодов));
	...
	ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ДатаКонцаВышестоящегоПериода Тогда
		...
		Если ЗначениеЗаполнено(СтрокаПараметр.УточнениеСпособаОпределения) Тогда
			Периодичность=ПолучитьЗначениеИзХранилища(СтрокаПараметр.УточнениеСпособаОпределения);
		Иначе
			Периодичность=ПолучитьТекущуюПериодичность(КонтекстОтчета.ПериодОтчета);
		КонецЕсли;
		
		СвойстваПараметра.ВидСравнения=ВидСравнения.Равно;
		СвойстваПараметра.Значение=КонецДня(ОбщегоНазначенияУХ.ПолучитьПериодПоДате(ДатаПериода,Периодичность).ДатаОкончания);
	...
	ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ДатаНачалаПериодаОтчетаСоСдвигом Тогда
		...
		ДатаПериода=ПолучитьДатуНачалаПериода(КонтекстОтчета.ПериодОтчета);
		Периодичность=ПолучитьТекущуюПериодичность(КонтекстОтчета.ПериодОтчета);
		СдвигПериодов=ПолучитьЗначениеИзХранилища(СтрокаПараметр.УточнениеСпособаОпределения);
		
		СвойстваПараметра.ВидСравнения=ВидСравнения.Равно;
		СвойстваПараметра.Значение=ОбщегоНазначенияУХ.ДобавитьИнтервал(ДатаПериода, Периодичность, СдвигПериодов);
	...
	ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ДатаНачалаВышестоящегоПериода Тогда
		...
		СвойстваПараметра.ВидСравнения=ВидСравнения.Равно;
		СвойстваПараметра.Значение=ОбщегоНазначенияУХ.ПолучитьПериодПоДате(ДатаПериода,Периодичность).ДатаНачала;
	...
	ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ДатаНачалаГода Тогда
		...
		СвойстваПараметра.Значение=НачалоГода(ДатаПериода);
	...
	ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ДатаКонцаГода Тогда
		...
		СвойстваПараметра.Значение = КонецГода(ДатаПериода);
		СтрокаПараметр.ИзКонтекста = Истина;
		СтрокаПараметр.ЗначениеИзКонтекста = СвойстваПараметра.Значение;
		СтрокаПараметр.ЗначениеДляПроверки = СвойстваПараметра.Значение;
	...
	ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.АналогичныйПериодПредыдущегоГода Тогда
		...
		Иначе
			...
			Для Каждого Период ИЗ КонтекстОтчета.ПериодОтчета Цикл
				...
				ОтнПериод.Добавить(ОбщегоНазначенияУХ.глАналогичныйПериод(Период, -1));
	...
	ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.АналогичныйПериодГодаСоСдвигом Тогда
		...
		Если ТипЗнч(КонтекстОтчета.ПериодОтчета)=Тип("СправочникСсылка.Периоды") Тогда
			...
			ОтнПериод=ОбщегоНазначенияУХ.глАналогичныйПериод(КонтекстОтчета.ПериодОтчета, СдвигПериодов);
		...
		Иначе
			...
			Для Каждого Период ИЗ КонтекстОтчета.ПериодОтчета Цикл
				...
				ОтнПериод.Добавить(ОбщегоНазначенияУХ.глАналогичныйПериод(Период, СдвигПериодов));
	...
	ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ДатаНачалаГодаСоСдвигом Тогда
		...
		ДатаПериода=ПолучитьДатуНачалаПериода(КонтекстОтчета.ПериодОтчета);
		СдвигПериодов=ПолучитьЗначениеИзХранилища(СтрокаПараметр.УточнениеСпособаОпределения);
		ДатаГода=ОбщегоНазначенияУХ.ДобавитьИнтервал(ДатаПериода, Перечисления.Периодичность.Год, СдвигПериодов);
				
		СвойстваПараметра.ВидСравнения=ВидСравнения.Равно;
		СвойстваПараметра.Значение=НачалоГода(ДатаГода);
		СтрокаПараметр.ЗначениеДляПроверки=СвойстваПараметра.Значение;
	...
	ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ДатаКонцаГодаСоСдвигом Тогда
		...
		СвойстваПараметра.ВидСравнения=ВидСравнения.Равно;
		СвойстваПараметра.Значение=КонецГода(ДатаГода);
		СтрокаПараметр.ЗначениеДляПроверки=СвойстваПараметра.Значение;
		
		СтрокаПараметр.ИзКонтекста=Истина;
		СтрокаПараметр.ЗначениеИзКонтекста=СвойстваПараметра.Значение;
		
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаПараметр, "ПолеБД")
		И СтрокаПараметр.ПолеБД="КоррСчетОборот" Тогда
		
		СвойстваПараметра.Значение=ПолучитьЗначениеИзХранилища(СтрокаПараметр.УточнениеСпособаОпределения);
		СтрокаПараметр.ЗначениеИзКонтекста=ПолучитьСписокКоррСчетов(СвойстваПараметра.Значение);
	...
	ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ФункцияНаВстроенномЯзыкеЗначение
		...
		Если (НЕ СтрокаПараметр.Владелец().Колонки.Найти("НастройкаСоответствия")=Неопределено)
			И ЗначениеЗаполнено(СтрокаПараметр.НастройкаСоответствия) Тогда
			
			СтрокаПараметр.ТипРасчета="ПоКоду";
			СтрокаПараметр.УточнениеСпособаОпределения=ПолучитьПараметрыНастройкиОтбора(СтрокаПараметр.НастройкаСоответствия);
	...
	ИначеЕсли СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ПериметрОтчета
		...
		СвойстваПараметра.ВидСравнения = 
			?(СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ПериметрОтчета,ВидСравнения.ВСписке,ВидСравнения.НеВСписке);
		СвойстваПараметра.Значение = ПолучитьЗначениеПериметраПоОтбору(
			СтрокаПараметр.СпособВычисленияПараметра, СтрокаПараметр.УточнениеСпособаОпределения, КонтекстОтчета); 
...
Функция ПолучитьЗначениеПериметраПоОтбору(СпособВычисленияПараметра, УточнениеСпособаОпределения, ТекКонтекстОтчета) Экспорт
	...
	Если СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.НеПериметрОтчета Тогда
		// Элиминирующие организации также исключаем
		ТипыОрганизаций.Добавить(Перечисления.ТипыОрганизационныхЕдиниц.Элиминирующая)			
	КонецЕсли;
			
	ДанныеОтбораПоПериметру=ПолучитьЗначениеИзХранилища(УточнениеСпособаОпределения);
	...
	Если ТипЗнч(ДанныеОтбораПоПериметру)=Тип("Структура") Тогда
		...
		Если ДанныеОтбораПоПериметру.МассивОтбора.Количество()=0 Тогда
			...
			КонсолидирующаяОрганизация=ТекКонтекстОтчета.Организация;
		...
		СписокПериметр.ЗагрузитьЗначения(ОбщегоНазначенияУХ.глСоставПериметра(ТекКонтекстОтчета,,, ,,,,ТипыОрганизаций, КонсолидирующаяОрганизация));
		...
		УточнениеСпособаОпределения=Новый ХранилищеЗначения(ДанныеОтбораПоПериметру);
	...
	Иначе
		...
		ТекСоставПериметра = ОбщегоНазначенияУХ.глСоставПериметра(ТекКонтекстОтчета,,,КонсолидирующаяОрганизация,СписокОтбора,,,ТипыОрганизаций);
		
		// Функция СоставПериметра() возвращает массив организаций. Преобразуем к СЗ.
		...
		Если СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.НеПериметрОтчета Тогда 
			// Добавим в список организацию - владельца периметра
			Значение.Добавить(?(ЗначениеЗаполнено(КонсолидирующаяОрганизация),КонсолидирующаяОрганизация,ТекКонтекстОтчета.Организация));
		КонецЕсли;
...
Функция ПолучитьДанныеОтбораДляРасшифровки(КонтекстОтчета,СтрокаПараметр) Экспорт
	...
	СтруктураОтбора=Новый Структура;
	СтруктураОтбора.Вставить("ИмяПараметра",СтрокаПараметр.ИмяПараметра);
	СтруктураОтбора.Вставить("ПолеБД",СтрокаПараметр.ПолеБД);
	...
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаПараметр, "ПолеБД")
		...
		СтруктураОтбора.Вставить("УточнениеСпособаОпределения",ПолучитьСписокКоррСчетов(ПолучитьЗначениеИзХранилища(СтрокаПараметр.УточнениеСпособаОпределения)));
	...
	ИначеЕсли  СтрокаПараметр.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ИсходныйДокумент Тогда
		...
		СтруктураОтбора.Вставить("СпособВычисленияПараметра",Перечисления.СпособыВычисленияПараметровОперандов.ФиксированноеЗначение);
		СтруктураОтбора.Вставить("УточнениеСпособаОпределения",КонтекстОтчета.ИсходныйДокумент);
...
Процедура ИзменитьТекстОтбораПериод(СтрЗапрос)
	
	СтруктураПоиска=Новый Структура;
	СтруктураПоиска.Вставить("ПолеБД","Версия.ПериодОтчета");
	
	МассивОтборПериоды=СтрЗапрос.ПравилаВычисленияПараметров.НайтиСтроки(СтруктураПоиска);
	
	Для Каждого СтрокаПериод ИЗ МассивОтборПериоды Цикл
		
		Если СтрокаПериод.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.АналогичныйПериодГодаСоСдвигом
			ИЛИ СтрокаПериод.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.АналогичныйПериодПредыдущегоГода
			ИЛИ СтрокаПериод.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ВышестоящийПериод
			ИЛИ СтрокаПериод.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ПериодОтчета
			ИЛИ СтрокаПериод.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ПериодОтчетаСоСдвигом Тогда
			
			СтрЗапрос.ТекстЗапроса=СтрЗаменить(СтрЗапрос.ТекстЗапроса,"Версия.ПериодОтчета=&"+СтрокаПериод.ИмяПараметра,"Версия.ПериодОтчета В(&"+СтрокаПериод.ИмяПараметра+")");

		КонецЕсли;
		
	КонецЦикла;	
	
КонецПроцедуры // ИзменитьТекстОтбораПериод() 
...
Функция ПолучитьТаблицуПоИсточникуВнутрПоказатель(КонтекстОтчета,СтрЗапрос) Экспорт
	...
	Если ТипЗнч(КонтекстОтчета.ПериодОтчета)=Тип("Массив") Тогда
		...
		ИзменитьТекстОтбораПериод(СтрЗапрос);
...
Функция ПолучитьТаблицуПоИсточникуВнутрРегистрНакопления(КонтекстОтчета,СтрЗапрос) Экспорт
	...
	Документы.НастраиваемыйОтчет.СформироватьРабочийТекстЗапроса(КонтекстОтчета,СтрЗапрос);	

	Возврат ПолучитьТаблицуПоИсточникуВнутрПроизвольный(КонтекстОтчета,СтрЗапрос);
		
КонецФункции // ПолучитьТаблицуПоИсточникуВнутрРегистрНакопления()

Процедура ПроверитьВключитьМежОтчетныйПериод(СвойстваОтбора,СтрЗапрос)
	
	Если ТипЗнч(СтрЗапрос)=Тип("СтрокаТаблицыЗначений") Тогда
		
		ДанныеЗапроса=ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(СтрЗапрос);
		
	Иначе
		
		ДанныеЗапроса=СтрЗапрос;
		
	КонецЕсли;
	
	Если (НЕ ДанныеЗапроса.Свойство("ДлинаУточненияПериода"))
		ИЛИ ДанныеЗапроса.ДлинаУточненияПериода=0 Тогда
		
		Возврат;
		
	КонецЕсли;
	
	Если КонецДня(СвойстваОтбора.Значение)=КонецГода(СвойстваОтбора.Значение) Тогда
		
		ГраницаПериода=Новый МоментВремениСУточнениемПериода(КонецДня(СвойстваОтбора.Значение),РаботаСОбъектамиМетаданныхВнешнийУХ.ВерхняяГраницаУточненияПериода(ДанныеЗапроса.ДлинаУточненияПериода));
		
		СвойстваОтбора.Вставить("Значение",ГраницаПериода);
		
	КонецЕсли;	
	
КонецПроцедуры // ДобавитьМежОтчетныйПериод() 
...
Функция ПреобразоватьКолонкуКоррСчет(ТабРезультат,ПланСчетов,СтрокаКоррСчет) Экспорт
	...
	Для Каждого Колонка ИЗ ТабРезультат.Колонки Цикл
		...
		Если Колонка.Имя=СтрокаКоррСчет.КодАналитики Тогда
		...
		Запрос.Текст=Запрос.Текст+"
		|ТабРезультат."+Колонка.Имя+",";
		
	КонецЦикла;
	
	Запрос.Текст=Запрос.Текст+"
	|ЕСТЬNULL(СчетаБД.Ссылка,Неопределено) КАК "+СтрокаКоррСчет.КодАналитики+"
	|ИЗ ТабРезультат КАК ТабРезультат
	|ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СчетаБД КАК СчетаБД
	|ПО ТабРезультат."+СтрокаКоррСчет.КодАналитики+"=СчетаБД.Код
	|И СчетаБД.Владелец=&ПланСчетов";
...
КонецФункции // ПреобразоватьКолонкуКоррСчет() 
...
Функция ПолучитьТаблицуПоИсточникуВнутрПроизвольный(КонтекстОтчета,СтрЗапрос) Экспорт
	...
	Если ТипЗнч(КонтекстОтчета)=Тип("Структура")
		И КонтекстОтчета.Свойство("ЗапросДвижения") Тогда
		
		Запрос=КонтекстОтчета.ЗапросДвижения;
	...
	Для Каждого Параметр Из ТекПравилаВычисленияПараметров Цикл
		...
		Если Параметр.ИмяПараметра="ДатаКон" Тогда
							
			ПроверитьВключитьМежОтчетныйПериод(СвойстваОтбора,СтрЗапрос);
			Запрос.УстановитьПараметр(Параметр.ИмяПараметра,СвойстваОтбора.Значение);
	...
	Если НЕ (СписокСчетов=Неопределено ИЛИ СписокКоррСчетов=Неопределено) Тогда
		
		Запрос.УстановитьПараметр("СчетДт",СписокСчетов);
		Запрос.УстановитьПараметр("СчетКт",СписокКоррСчетов); 
		
	ИначеЕсли НЕ СписокСчетов=Неопределено Тогда
	...
	СтрокаКоррСчет=ПравилаИспользованияПолей.Найти("КоррСчетОборот","Синоним");
	
	Если НЕ СтрокаКоррСчет=Неопределено Тогда
		
		ТабРезультат=ПреобразоватьКолонкуКоррСчет(ТабРезультат,СтрЗапрос.ПланСчетов,СтрокаКоррСчет);
		
	КонецЕсли; 
	
	Для Каждого СтрПоле Из ПравилаИспользованияПолей Цикл
		
		Если СтрПоле.КодАналитики="Значение" 
			И СтрЗапрос.СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеРегистрБухгалтерии Тогда
...
Функция ПолучитьПараметрыРасчетаПоказателя(ОбъектРасчета,Показатель) Экспорт
	...
	|		ПО ПоказателиСинтетика.Версия = ВерсииЗначенийПоказателей.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
...
Функция ПолучитьПараметрыОтчетаПоРегиструБД(ОперандСсылка, КонтекстОтчета, ДополнительныйОтбор = Неопределено) Экспорт
	...
	СтруктураОтбораИсточник=Справочники.ИсточникиДанныхДляРасчетов.ПолучитьПараметрыОтбораПоИсточнику(ОперандСсылка);
				
	СтруктураПараметров=Новый Структура;
	СтруктураПараметров.Вставить("РегистрБД",				ОперандОбъект.РегистрБД);
	СтруктураПараметров.Вставить("ТипБД",					ОперандОбъект.ТипБД);
	СтруктураПараметров.Вставить("ИспользуемаяИБ",			КонтекстОтчета.ИспользуемаяИБ);
	СтруктураПараметров.Вставить("ФормироватьПриОткрытии",Истина);
	...
	СтруктураПараметров.Вставить("ТекстОтбораИсточник",		СтруктураОтбораИсточник.ТекстОтбораИсточник);
	СтруктураПараметров.Вставить("ИсточникДанных",			ОперандСсылка);
	...
	Для Каждого СтрПоле Из ОперандОбъект.ПравилаИспользованияПолейЗапроса Цикл
		...
		Если ОбщегоНазначенияУХ.ПринудительноеЗаполнениеПоля(СтрПоле)
			ИЛИ СтрПоле.Поле="КоррСчетОборот" Тогда
		...
		Иначе
			...
			СтруктураДетализация.Вставить("ПредставлениеПоля",ТекПараметрВИБ.ПредставлениеПоля);
			...
			ИначеЕсли ЗначениеЗаполнено(СтрПоле.ТаблицаАналитики) Тогда
				
				ДанныеАналитики=СтрРазделить(СтрПоле.ТаблицаАналитики,".",Ложь);
				СтруктураДетализация.Вставить("ТипМетаДанных",ДанныеАналитики[0]);
				СтруктураДетализация.Вставить("ИмяОбъектаМетаданных",ДанныеАналитики[1]);
	...
	Для Каждого СтрПараметр Из СтруктураОтбораИсточник.ПравилаВычисленияПараметровЗапроса Цикл
		...
		Иначе
			...
			ИначеЕсли СтрПараметр.ПолеБД="КоррСчетОборот" Тогда
			
				СтруктураПараметров.Вставить(ДанныеОтбора.ИмяПараметра,ДанныеОтбора.УточнениеСпособаОпределения);
				МассивОтбор.Добавить(ДанныеОтбора);
					
			Иначе
				
				Если ОтчетПоРегиструБухгалтерии И СтрНайти(ДанныеОтбора.ПолеБД,"Субконто")=0 Тогда
					
					СтруктураОтбора = Новый Структура;
					СтруктураОтбора.Вставить("ИмяПараметра", ДанныеОтбора.ИмяПараметра);
					СтруктураОтбора.Вставить("Значение", ДанныеОтбора.УточнениеСпособаОпределения);
					СтруктураОтбора.Вставить("НастройкаСоответствия", ДанныеОтбора.НастройкаСоответствия);
					СтруктураОтбора.Вставить("ПолеБД", ДанныеОтбора.ПолеБД);
					
					ОтборПоИзмерениям.Добавить(СтруктураОтбора);
	...
	Если ОтчетПоРегиструБухгалтерии Тогда
		...
		Если ОперандОбъект.УчитыватьКоррСчета Тогда
			
			СтруктураПараметров.Вставить("ВидОтчета","ОтчетПоПроводкам"); 
			СтруктураПараметров.Вставить("Счет",?(ОперандОбъект.ДтКт="Дт",ОперандОбъект.Счет,Справочники.СчетаБД.ПустаяСсылка()));
			СтруктураПараметров.Вставить("КоррСчет",?(ОперандОбъект.ДтКт="Кт",ОперандОбъект.Счет,Справочники.СчетаБД.ПустаяСсылка()));
			СтруктураПараметров.Вставить("ОтборПоИзмерениям",ОтборПоИзмерениям);
			СтруктураПараметров.Вставить("ОтборПоСубконто",МассивОтбор); 
			
			Если ТипЗнч(ДополнительныйОтбор)=Тип("Структура") Тогда
				
				Для Каждого КлючИЗначение ИЗ ДополнительныйОтбор Цикл
					
					Если ТипЗнч(КлючИЗначение.Значение)=Тип("СправочникСсылка.СчетаБД") Тогда
						
						Если ОперандОбъект.ДтКт="Дт" Тогда
							
							СтруктураПараметров.Вставить("КоррСчет",КлючИЗначение.Значение);
							
						Иначе
							
							СтруктураПараметров.Вставить("Счет",КлючИЗначение.Значение);
			...
			Если ЗначениеЗаполнено(СтруктураПараметров.Счет) И ЗначениеЗаполнено(СтруктураПараметров.КоррСчет) Тогда
				
				Для Каждого ДанныеПоля ИЗ СтруктураПараметров.МассивДетализация Цикл
					
					ДобавитьПризнакДтКт(ДанныеПоля,"ИмяПоля",ОперандОбъект.ДтКт);
					ДобавитьПризнакДтКт(ДанныеПоля,"ПредставлениеПоля",ОперандОбъект.ДтКт);
					ДобавитьПризнакДтКт(ДанныеПоля,"ПсевдонимПоля",ОперандОбъект.ДтКт);
					
				КонецЦикла;
					
				Для Каждого ДанныеПоля ИЗ СтруктураПараметров.МассивДополнительныйОтбор Цикл
					
					ДобавитьПризнакДтКт(ДанныеПоля,"ИмяПараметра",ОперандОбъект.ДтКт);
					ДобавитьПризнакДтКт(ДанныеПоля,"ПредставлениеПараметра",ОперандОбъект.ДтКт);
...
Процедура ДобавитьПризнакДтКт(ДанныеПоля,Ключ,ДтКт)
	
	Если СтрНайти(ДанныеПоля[Ключ],"Субконто")=0 Тогда
		
		Возврат;
		
	КонецЕсли;
	
	Если СтрНайти(ДанныеПоля[Ключ],ДтКт)>0 Тогда
		
		Возврат;
		
	КонецЕсли;
		
	ДанныеПоля[Ключ]=СтрЗаменить(ДанныеПоля[Ключ],"Субконто","Субконто"+ДтКт);
	
КонецПроцедуры // ДобавитьПризнакДтКт() 
...
Функция ПолучитьКонтекстОтчетаПоИсточнику(ИсточникДанных,ПравилаВычисленияПараметров)
	...
	Для Каждого СтрОтбор Из ПравилаВычисленияПараметров Цикл
		...
		Если СтрОтбор.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ПериодОтчета Тогда
			...
			МассивПериодов.Добавить(СтрОтбор.ЗначениеДляПроверки);
			
			КонтекстОтчета.Вставить("ПериодОтчета",МассивПериодов);
...
КонецФункции // ПолучитьКонтекстОтчетаПоИсточнику() 
...
Функция ПолучитьТаблицуДанныхПоИсточнику(КонтекстОтчета,ИсточникДанных,ТолькоПроверка=Ложь) Экспорт
	...
	Если КонтекстОтчета=Неопределено Тогда  // Определяем контекст по значениям для проверки
		...
		КонтекстОтчета=ПолучитьКонтекстОтчетаПоИсточнику(ИсточникДанных,ТабЗапросы[0].ПравилаВычисленияПараметров);
	...
	СтрЗапрос=ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(ТабЗапросы[0]);
...
Функция ПолучитьПланСчетовОрганизации(Организация,Сценарий,ПериодОтчета) Экспорт
	...
	Если НЕ Метаданные.ОбщиеМодули.Найти("КорректировкиЗначенийПоказателей32")=Неопределено Тогда
		...
		МодульКорректировкиЗначенийПоказателей32 = ОбщегоНазначения.ОбщийМодуль("КорректировкиЗначенийПоказателей32");
		Возврат МодульКорректировкиЗначенийПоказателей32.ПолучитьПланСчетовОрганизации(Организация,Сценарий,ПериодОтчета);
...
Функция ПодготовитьТекстЗапросаКРегистрам(ДанныеЗапроса) Экспорт
	...
	Если ВидОтчета=Неопределено И СписокВерсий=Неопределено Тогда // Готовим запрос по всем регистрам
		...
		Если ВыводитьАналитику Тогда
			...
			|		ПО ЗначенияПоказателейОтчетов6.Версия = ВерсииЗначенийПоказателей.Ссылка
			|		И НЕ ВерсииЗначенийПоказателей.ЧерноваяВерсия
			|ГДЕ
		...
		Если ДляТекстаПостроителя Тогда
			...
			|ИЗ
			|	("+Сред(ТекстЗапроса,СтрДлина("ОБЪЕДИНИТЬ ВСЕ") + 2)+") КАК ЗначенияПоказателейОтчетов
			|{ГДЕ
			|	ЗначенияПоказателейОтчетов.Аналитика1.*,
			|	ЗначенияПоказателейОтчетов.Аналитика2.*,
			|	ЗначенияПоказателейОтчетов.Аналитика3.*,
		...
		Иначе
			...
			СтруктураЗапроса.Вставить("ТекстЗапроса",СРЕД(ТекстЗапроса,СтрДлина("ОБЪЕДИНИТЬ ВСЕ") + 2));
			...
			Для Каждого СтрТаблица Из ТабНомеровАналитик Цикл
				...
				Если ЧислоАналитикТаблицы=АналитикВидаОтчета И СтрТаблица.Валютный=Истина Тогда
					...
					Иначе
						...
						ОтборНеИтоговоеЗначение=Истина;
				...
				Если ОтборНеИтоговоеЗначение Тогда
					
					ТекстЗапроса=ТекстЗапроса+"
					|И (НЕ ИтоговоеЗначение)";
...
Функция ПолучитьТаблицуАналитик(ВидОтчета,ОтборПоказателей,ТолькоЧисловые) Экспорт
	...
	|	ВЫБОР Когда ПоказателиОтчетов.ТипЗначения=ЗНАЧЕНИЕ(Перечисление.ТипыЗначенийПоказателейОтчетов.Число) 
	|	Тогда Истина Иначе Ложь Конец КАК ЧисловойПоказатель,
...
// Скрывает в макете строки, содержащие области со значениями,
...
// Скрывает в макете колонки, содержащие области со значениями,
...
Функция ПолучитьСчетаВИерархии(ПланСчетов,Счет,СписокСчетов,ВнутренниеДанные=Ложь) Экспорт
	...
	Если НЕ Счет.ГруппирующийСчет Тогда
		...
		Если СписокСчетов.НайтиПоЗначению(СчетКод)=Неопределено Тогда
	...
	Иначе
		...
		Пока Результат.Следующий() Цикл
			...
			Если СписокСчетов.НайтиПоЗначению(Результат.Код)=Неопределено Тогда
...
Функция ПолучитьДатуИзПараметра(ЗначениеПараметра, ТипРасчета, Сдвиг) Экспорт
	...
	Если ТипЗнч(Сдвиг) = Тип("Число") Тогда
		...
		//Для 9 месяцев делаем дискретные интервалы на год, без сдвига на 9
		Если Период = Перечисления.Периодичность.ДевятьМесяцев  Тогда
			РасчетнаяДата = ДобавитьМесяц(РасчетнаяДата, 12 * Сдвиг);
		КонецЕсли;
...
Процедура ОбъединитьЯчейкиВлевоИВправо(ЛеваяЯчейка, ПраваяЯчейка, ПоглощаемыеЯчейки)
	Перем ИндексПоследнейЯчейкиОбъединяемойВлево;
		...
		Если ЛеваяЯчейка <> Неопределено И ЛеваяЯчейка.Свойство("Текст") Тогда  
			...
			Для ИндексЯчейки = 0 По ПоглощаемыеЯчейки.Количество() - 1 Цикл
				...
				Если ШиринаПоглощаемаяВправо / ПоглощаемаяШирина >= ДоляЛевойЯчейки Тогда
					ИндексПоследнейЯчейкиОбъединяемойВлево = ИндексЯчейки;
					Прервать;
				КонецЕсли;
			КонецЦикла;
		Иначе
			ИндексПоследнейЯчейкиОбъединяемойВлево = -1;
		КонецЕсли;
	Иначе
		ИндексПоследнейЯчейкиОбъединяемойВлево = ПоглощаемыеЯчейки.Количество() - 1;
	КонецЕсли;
	Если ИндексПоследнейЯчейкиОбъединяемойВлево >= 0 Тогда
		ОбъединитьЯчейки(ЛеваяЯчейка, ПоглощаемыеЯчейки, 0, ИндексПоследнейЯчейкиОбъединяемойВлево);	
	КонецЕсли;
	Если ИндексПоследнейЯчейкиОбъединяемойВлево < ПоглощаемыеЯчейки.Количество() - 1 Тогда
		ОбъединитьЯчейки(ПраваяЯчейка, ПоглощаемыеЯчейки, ИндексПоследнейЯчейкиОбъединяемойВлево + 1, ПоглощаемыеЯчейки.Количество() - 1);	
	КонецЕсли;
...
Функция СформироватьЗаголовокОсновнойФормы(ДатаНач, ДатаКон, НазваниеОтчета, РежимВводаПериода) Экспорт
	...
	ИначеЕсли РежимВводаПериода =  1 Тогда   // на дату
		Если ДатаКон = '00010101000000' Тогда
			ОписаниеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'на текущее время %1'"), Формат(ТекущаяДата(), "ДФ = ""дд.ММ.гггг""; ДП = ""..."""));
			
		Иначе
			ОписаниеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'на конец дня %1'"), Формат(ДатаКон, "ДФ = ""дд.ММ.гггг""; ДП = ""..."""));
...
Процедура ПроверитьПорядокПостроителяОтчета(ПостроительОтчета) Экспорт
	...
	// то для корректной работы необходимо установить порядок по периоду и самому регистратору
...
Функция ИнициализироватьЗапрос(ВерсияРегламентовПодготовкиОтчетности, УникальныйИдентификатор = Неопределено, Адрес = Неопределено,СтрокаДопОтбора=Неопределено,ОтборОбъект=Неопределено) Экспорт
	...
	ШаблонПроцесса = РасширениеБизнесЛогикиУХ.УправлениеПериодом_ШаблонПроцесса(ВерсияРегламентовПодготовкиОтчетности);
	...
	Если НЕ ЗначениеЗаполнено(ШаблонПроцесса) Тогда
		...
		Если ТипЗнч(ОтборОбъект) = Тип("СправочникСсылка.Организации")
			И ЗначениеЗаполнено(ОтборОбъект) Тогда
			
			ТекстЗапроса = ТекстЗапроса+" 
			|	И Организации.ОрганизационнаяЕдиница =&ОтборОбъект	
			|;
			|////////////////////////////////////////////////////////////////////////////////";
		...
		Если ТипЗнч(ОтборОбъект) = Тип("СправочникСсылка.ДокументыБД")
			И ЗначениеЗаполнено(ОтборОбъект) Тогда
			
			// Отдельный документ согласование или настройки
			ТекстЗапросаОбъекты=
			"ВЫБРАТЬ
			|	ДокументыБД.Ссылка КАК ОбъектБД,
			|	""ДокументыБД"" КАК ТипОбъекта,
			|	ДокументыБД.РеквизитШаблонаДокумента,
		...
		ИначеЕсли ТипЗнч(ОтборОбъект) = Тип("СправочникСсылка.СправочникиБД") 
			И СтрокаДопОтбора="Согласуется" Тогда 
			
			//Отдельный справочник согласование
			ТекстЗапросаОбъекты=ТекстЗапросаОбъекты+"
			|ВЫБРАТЬ
			|	СправочникиБД.Ссылка КАК ОбъектБД,
			|	""СправочникиБД"" КАК ТипОбъекта,
			|	СправочникиБД.РеквизитШаблонаДокумента,
		...
		ИначеЕсли ОтборОбъект=Неопределено ИЛИ ТипЗнч(ОтборОбъект) = Тип("СправочникСсылка.Организации") Тогда
			
			// Нет отборов по объектам - выводим все
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ИспользоватьШаблоны Тогда
				...
				Если Метаданные.Документы.Найти(ТекНаименованиеОбъектаМетаданных) <> Неопределено Тогда
					ЗначениеРеквизитаШаблона = Метаданные.Документы[ТекНаименованиеОбъектаМетаданных].Реквизиты[Выборка.РеквизитШаблонаДокумента];
		...
		ТестЗапросаВидыОбъектов = Прав(ТестЗапросаВидыОбъектов, СтрДлина(ТестЗапросаВидыОбъектов)-(СтрДлина("ОБЪЕДИНИТЬ ВСЕ") + 2));
	...
	Иначе
		...
		ТекстЗапроса = 
			...
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЭтапыУниверсальныхПроцессов КАК ЭтапыУниверсальныхПроцессов
			|		ПО ОрганизационныеЕдиницыЭтаповПроцессов.ЭтапПроцесса = ЭтапыУниверсальныхПроцессов.Ссылка
			|ГДЕ
			|	ОрганизационныеЕдиницыЭтаповПроцессов.ВерсияРегламентаПодготовкиОтчетности = &ВерсияРегламентовПодготовкиОтчетности
			|	И НЕ ЕСТЬNULL(ЭтапыУниверсальныхПроцессов.ПометкаУдаления, ЛОЖЬ)
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			|		ПО ЭтапыПроцессовФормируемыеДокументы.Ссылка = ЭтапыУниверсальныхПроцессов.Ссылка
			|ГДЕ
			|	ЭтапыУниверсальныхПроцессов.Владелец = &ТекущийШаблон
	...
	ТекстЗапроса = ТекстЗапроса + ОбщегоНазначения.РазделительПакетаЗапросов() + 
		...
		|	ТаблицаОрганизаций.ЭтапПроцесса КАК ЭтапПроцесса,
		|	ТаблицаОрганизаций.ПрофильОрганизаций КАК ПрофильОрганизаций
		|ИЗ
...
#Область ПересчетПриИзмененииКурсовВалют 

Функция СформироватьКомментарийПриПересчетеКурса(Результат)
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Пересчет при изменении курса %1 валюты %2 на '"),Результат.ВидКурса,Результат.Валюта)+Формат(Результат.Отношение*100,"ЧДЦ=2")+"%";
...
Процедура ПересчитатьВалютныеПоказатели(Сценарий,ТаблицаИзмененныхКурсов) Экспорт
	...
	|	ПоказателиОтчетов.Владелец.КоличествоАналитик КАК ЧислоАналитикВидаОтчета,
	|	ПоказателиОтчетов.Владелец КАК ВидОтчета,
...
Процедура ПересчитатьПоказателиВДополнительныхВалютах(Сценарий, ТаблицаИзмененныхКурсов, МассивПересчитанных)
	
	Отказ=Ложь;
		
	ПересчитатьПриИзмененииКурсаОсновнойВалюты(Сценарий,ТаблицаИзмененныхКурсов,МассивПересчитанных,Отказ);
	...
	|				ИЛИ ПоказателиОтчетов.НеФинансовый)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура ПересчитатьПриИзмененииКурсаОсновнойВалюты(Сценарий,ТаблицаИзмененныхКурсов,МассивПересчитанных,Отказ)
	...
	Запрос.Текст =
		...
		|	ТаблицаИзменений.ВидКурса КАК ВидКурса,
		|	ТаблицаИзменений.Отношение КАК Отношение,
		|	ТаблицаИзменений.ПериодКурса КАК ПериодКурса,
		|	ВерсииЗначенийПоказателей.ВидОтчета КАК ВидОтчета,
		|	ВерсииЗначенийПоказателей.Владелец КАК ОписаниеВерсии
		|ПОМЕСТИТЬ ОписанияВерсий
		|ИЗ
		|	Справочник.ВерсииЗначенийПоказателей КАК ВерсииЗначенийПоказателей
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаИзменений КАК ТаблицаИзменений
		|		ПО ВерсииЗначенийПоказателей.ПериодОтчета = ТаблицаИзменений.ПериодКурса
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ОписанияВерсий.ВидКурса КАК ВидКурса,
		|	ОписанияВерсий.Отношение КАК Отношение,
		|	ОписанияВерсий.ВидОтчета КАК ВидОтчета,
		|	ОписанияВерсий.ОписаниеВерсии КАК ОписаниеВерсии,
		|	ВерсииЗначенийПоказателей.Ссылка КАК Версия
		|ПОМЕСТИТЬ ВерсииЗначенийПоказателей
		|ИЗ
		|	ОписанияВерсий КАК ОписанияВерсий
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВерсииЗначенийПоказателей КАК ВерсииЗначенийПоказателей
		|		ПО ОписанияВерсий.ОписаниеВерсии = ВерсииЗначенийПоказателей.Владелец
		|			И ОписанияВерсий.ПериодКурса = ВерсииЗначенийПоказателей.ПериодОтчета
		|			И (ВерсииЗначенийПоказателей.ЗаписьВДополнительнойВалюте)
		|			И (ВерсииЗначенийПоказателей.ЧерноваяВерсия = ЛОЖЬ)
		|			И (НЕ ВерсииЗначенийПоказателей.ПометкаУдаления)
		|;
	...
	Пока Результат.Следующий() Цикл
		...
		ТаблицаСинтетики=?(Результат.ЧислоАналитикВидаОтчета=0,"ЗначенияПоказателейОтчетовСинтетика","ЗначенияПоказателейОтчетов"+Результат.ЧислоАналитикВидаОтчета);
		ИзменитьЗаписиРегистраВДополнительнойВалютеОтОсновной(ТаблицаСинтетики,Результат);
		
		ИзменитьЗаписиРегистраВДополнительнойВалютеОтОсновной("ЗначенияПоказателейОтчетовВалютные",Результат);
		
		Для Индекс=Результат.ЧислоАналитикВидаОтчета+1 По ПараметрыСеанса.ЧислоДопАналитик Цикл
			
			ИзменитьЗаписиРегистраВДополнительнойВалютеОтОсновной("ЗначенияПоказателейОтчетов"+Индекс,Результат);
...
КонецПроцедуры // ПересчитатьПриИзмененииКурсаОсновнойВалюты() 

Процедура ИзменитьЗаписиРегистраВДополнительнойВалютеОтОсновной(ИмяРегистра,Результат)
	...
	Для Каждого Строка Из НаборЗаписей Цикл 	
		...
		Строка.Значение=Окр(Строка.Значение*Результат.Отношение,4);
				
	КонецЦикла;		
	
	НаборЗаписей.Записать();

КонецПроцедуры // ИзменитьЗаписиРегистраВДополнительнойВалютеОтОсновной
...
Функция ПолучитьОписаниеВерсии(ОбъектРасчета) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(УправлениеПериодом) Тогда
		...
		ТекстОшибки=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Невозможно определить документ управления периодами сценария для организации %1, периода %2 и сценария %3'"),
...
Процедура ПроверитьВозможностьЗаписи(ОбъектРасчета,ТаблицаИзмененныхПоказателей,Отказ) Экспорт
	...
	ШапкаОшибки=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Организация: %1, сценарий: %2, период: %3
	...
	ПолныеПраваУХ.ПроверитьВозможностьЗаписиДокументовПоПериодуСценария(ОбъектРасчета, Отказ, ШапкаОшибки);
	...
	СогласованиеЧерезРакурс = РасширениеПроцессыИСогласованиеУХ.СогласованиеЧерезРакурс(ОбъектРасчета.ВидОтчета);
	Если НЕ СогласованиеЧерезРакурс Тогда
		
		ТекстАналитики = СтрЗаменить(ЗначенияСтатусовПоказателейСинтетикаАналитика,
			"ЗначенияСтатусовПоказателейСинтетика", "ЗначенияСтатусовКлючевыхСрезов");
		
		ОтборПоАналитикам = "";
		Для ИндексАналитики = 1 По МаксАналитикОтчета Цикл
			АналитикаН = "Аналитика" + ИндексАналитики;
			ОтборПоАналитикам = ОтборПоАналитикам + "
			|	И ЗначенияСтатусовКлючевыхСрезов." + АналитикаН + " = &" + АналитикаН;
			Запрос.УстановитьПараметр(АналитикаН, ОбъектРасчета[АналитикаН]);
		КонецЦикла;
		
		Запрос.Текст =
		"ВЫБРАТЬ
		|	"""" КАК ПоказательОтчета,
		|	ЗначенияСтатусовКлючевыхСрезов.ЗначениеЗаблокировано КАК ЗначениеЗаблокировано,
		|	ЗначенияСтатусовКлючевыхСрезов.ИндексСостояния КАК ИндексСостояния" + ТекстАналитики + "
		|ИЗ
		...
		|	И ЗначенияСтатусовКлючевыхСрезов.Сценарий = &Сценарий
		|	И (ЗначенияСтатусовКлючевыхСрезов.ОрганизационнаяЕдиница = &Организация
		|		ИЛИ ЗначенияСтатусовКлючевыхСрезов.ОрганизационнаяЕдиница=ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка))
		|	И (ЗначенияСтатусовКлючевыхСрезов.Проект = &Проект
		|		ИЛИ ЗначенияСтатусовКлючевыхСрезов.Проект=ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка))
		|	И ЗначенияСтатусовКлючевыхСрезов.ПериодОтчета = &ПериодОтчета
		|	И (ЗначенияСтатусовКлючевыхСрезов.ЗначениеЗаблокировано = 1
		|		ИЛИ ЗначенияСтатусовКлючевыхСрезов.ИндексСостояния = 3)" + ОтборПоАналитикам;
	...
	Пока РезультатЗапроса.Следующий() Цикл
		...
		Если РезультатЗапроса.ЗначениеЗаблокировано>0 Тогда
			...
			ТекстОшибки=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 значения заблокированы.'"), ТекстОшибки);
			
		КонецЕсли;
		
		Если РезультатЗапроса.ИндексСостояния = 3 Тогда
			
			ТекстОшибки=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 данные утверждены.'"), ТекстОшибки);
		...
		Если НЕ СогласованиеЧерезРакурс Тогда
			...
			Если РезультатЗапроса.ЗначениеЗаблокировано > 0 Тогда
				ПричиныОтказа.Добавить(НСтр("ru = 'заблокированы'"));
			КонецЕсли;
			Если РезультатЗапроса.ИндексСостояния = 3 Тогда
				ПричиныОтказа.Добавить(НСтр("ru = 'утверждены'"));
			КонецЕсли;
			
			ТекстПричины = СтрСоединить(ПричиныОтказа, НСтр("ru = ' и '"));
			ТекстОшибки = СтрШаблон(НСтр("ru = ' данные ключевого среза %1.'"), ТекстПричины);
	...
	Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Вид отчета %1, %2 '"),
			Строка(ОбъектРасчета.ВидОтчета),
			Строка(ШапкаОшибки));
...
Процедура ЗаписатьПоказателиОтчетов(ОбъектРасчета,Отказ,ПроверятьВозможностьЗаписи=Истина) Экспорт
	...
	Если ОбъектРасчета.ПроверкаПриЗаписи Тогда
		...
		ШапкаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1"" не может быть записан:'"), ОбъектРасчета.ПредставлениеОбъекта);
	...
	КонецЕсли;
		...
		// вне зависимости от того, был ли изменен показатель в текущей версии.		
...
Процедура ЗаполнитьАналитикиПоУмолчанию(ОбъектРасчета,ТаблицаАналитики) Экспорт
	...
	МассивПустыхАналитик=Новый Массив;
	МассивПустыхАналитик.Добавить(Неопределено);
	
	Для Каждого Тип Из Метаданные.ПланыВидовХарактеристик.ВидыСубконтоКорпоративные.Тип.Типы() Цикл
		
		МассивПустыхАналитик.Добавить(Новый(Тип));
	...
	|	ТаблицаАналитики.ИтоговоеЗначение
	...
	Для Каждого Постфикс Из МассивПостфиксов Цикл
		...
		Запрос.Текст=Запрос.Текст+"
		|ВЫБОР Когда ТаблицаАналитики.Аналитика"+Постфикс+" В(&МассивПустыхАналитик)
		|Тогда Неопределено 
		|Иначе ТаблицаАналитики.Аналитика"+Постфикс+"
	...
	Для Каждого Постфикс Из МассивПостфиксов Цикл
		...
		Запрос.Текст=Запрос.Текст+"
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗначенияАналитикПоУмолчанию.СрезПоследних(&ДатаНачалаПериода, )КАК ЗначенияАналитикПоУмолчанию"+Постфикс+"
		|		ПО ГруппыРаскрытияВидыАналитик.ВидАналитики"+Постфикс+" = ЗначенияАналитикПоУмолчанию"+Постфикс+".ВидАналитики";
		
	КонецЦикла;
	
	Запрос.Текст=Запрос.Текст+"
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗначенияАналитикПоУмолчанию.СрезПоследних(&ДатаНачалаПериода, ) КАК ЗначенияАналитикПоУмолчаниюВалюта
	...
	|	ТаблицаАналитикиГруппыРаскрытия.ИтоговоеЗначение
	|ИЗ
	...
	Запрос.УстановитьПараметр("ДатаНачалаПериода",ОбъектРасчета.ПериодОтчета.ДатаНачала);
	Запрос.УстановитьПараметр("МассивПустыхАналитик",МассивПустыхАналитик);
...
Процедура ИнициализироватьВнутригрупповыеОперации(ОбъектРасчета,ТаблицаСоответствий, ДеревоОтчетов, ТекСоставПериметра, КэшГруппРаскрытия)
	...
	Если НЕ УправлениеРабочимиПроцессамиУХ.ОрганизацияЯвляетсяОбычной(ОбъектРасчета.Организация,ОбъектРасчета.Сценарий,ОбъектРасчета.ПериодОтчета) Тогда
...
Функция ПолучитьСписокВерсий(ОбъектРасчета,ВключатьПомеченныеНаУдаление=Ложь,ТолькоОсновнаяВалюта=Истина) Экспорт
	...
	Если  ОбъектРасчета.Свойство("ИдентификаторСеанса") Тогда
		...
		ЗапросВерсия.УстановитьПараметр("ИдентификаторСеанса",XMLСтрока(ОбъектРасчета.ИдентификаторСеанса));	
	...
	Для Индекс=1 ПО ПараметрыСеанса.ЧислоДопАналитик Цикл
		...
		ТекАналитикаВидаОтчета=?(КопированиеОбъекта,мКопируемыйДокумент["Аналитика"+Индекс],?(ЗначениеЗаполнено(ОбъектРасчета["Аналитика"+Индекс]),ОбъектРасчета["Аналитика"+Индекс],Неопределено));			
		
		Если Индекс=5 Тогда
			
			ЗапросВерсия.Текст=ЗапросВерсия.Текст+"	
			|И (ВерсииЗначенийПоказателей.Аналитика5 = &Аналитика5
			|ИЛИ ВерсииЗначенийПоказателей.Аналитика5=ЗНАЧЕНИЕ(Справочник.ПроизвольныйКлассификаторУХ.ПустаяСсылка))";	
			ЗапросВерсия.УстановитьПараметр("Аналитика5",ТекАналитикаВидаОтчета);
...
Функция ПроверитьВозможностьЗаписиПоКлючевымРеквизитам(ОбъектРасчета, Знач ТекстОшибки="") Экспорт
	...
	ПроверяемыеРеквизиты = ПроверяемыеКлючевыеРеквизиты();
	Аналитики = КлючевыеАналитикиСоставногоТипа(ОбъектРасчета.ВидОтчета);
	ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ПроверяемыеРеквизиты, Аналитики);
	
	ТекстыОшибок = Новый Массив;
	Для Каждого Реквизит Из ПроверяемыеРеквизиты Цикл
		Если НЕ ЗначениеЗаполнено(ОбъектРасчета[Реквизит.Ключ]) Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = '		- не заполнен реквизит ""%1""!'"), Реквизит.Значение);
	...
	Если ЗначениеЗаполнено(ТекстыОшибок) Тогда
		...
		ТекстыОшибок.Вставить(0, ТекстОшибки);
...
Функция ПроверяемыеКлючевыеРеквизиты() Экспорт
	
	Реквизиты = Новый Структура;
	Реквизиты.Вставить("ВидОтчета",          НСтр("ru = 'Вид отчета'"));
	Реквизиты.Вставить("Организация",        НСтр("ru = 'Организационная единица'"));
	Реквизиты.Вставить("Сценарий",           НСтр("ru = 'Сценарий'"));
	Реквизиты.Вставить("ПериодОтчета",       НСтр("ru = 'Период отчета'"));
	Реквизиты.Вставить("ПериодОкончания",    НСтр("ru = 'Период окончания отчета'"));
	Реквизиты.Вставить("УправлениеПериодом", НСтр("ru = 'Управление периодом отчета'"));
	Реквизиты.Вставить("ОсновнаяВалюта",     НСтр("ru = 'Основная валюта'"));
	
	Возврат Реквизиты;
	
КонецФункции

Функция КлючевыеАналитикиСоставногоТипа(ВидОтчета) Экспорт
	
	Аналитики = Новый Структура;
	
	ДанныеВидаОтчета = ОбщегоНазначенияПовтИспУХ.ДанныеВидаОтчета(ВидОтчета);
	Для Номер = 1 По ДанныеВидаОтчета.МаксКлючевыхАналитик Цикл
		
		АналитикаН = "Аналитика" + Номер;
		ТипЗначенияАналитики = ДанныеВидаОтчета["ТипЗначенияАналитики" + Номер];
		
		Если ТипЗначенияАналитики.Типы().Количество() > 1 Тогда
			Аналитики.Вставить(АналитикаН, НСтр("ru = 'Аналитика '") + Номер);
		КонецЕсли;
...
Процедура СкопироватьПодчиненныеОбъекты(Ссылка, ЗначениеКопирования, ПараметрыКопированияВход, Отказ) Экспорт
	...
	ПараметрыКопирования = Новый Массив;
	ПараметрыКопирования.Добавить("ГруппыРаскрытия");
	ПараметрыКопирования.Добавить("СтрокиОтчетов");
	ПараметрыКопирования.Добавить("КолонкиОтчетов");
	ПараметрыКопирования.Добавить("ПоказателиОтчетов");
	
	ПараметрыНастроек = УправлениеОтчетамиУХ.ПараметрыНастроекОбработкиШаблонов();
	ПараметрыНастроек.ШаблонДокумента = ЗначениеКопирования;
	ЗначенияНастроек = УправлениеОтчетамиУХ.НастройкиОбработкиШаблонов(ПараметрыНастроек);
	
	Для Каждого ПараметрВход Из ПараметрыКопированияВход Цикл
		ПараметрыКопирования.Добавить(ПараметрВход);
	КонецЦикла;
	...
	Для Каждого ПараметрКопирования Из ПараметрыКопирования Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Попытка
				...
				Если КопированиеБланковОтчетов тогда
					...
					Если Выборка.РежимБланка = 1 Тогда
						...
						РасширениеБизнесЛогикиУХ.СкопироватьНастройкиБланкаСТ(Выборка.Ссылка, НовЭл.Ссылка);
			...
			Если КопированиеБланковОтчетов Тогда
				...
				Если Выборка.Ссылка = ЗначенияНастроек.БланкДляОтображения Тогда
					ЗаписатьНастройкуПоУмолчаниюОбработкиШаблонов(Ссылка, "БланкДляОтображения", НовЭл.Ссылка, , Отказ);
				КонецЕсли;
				
				Если Выборка.Ссылка = ЗначенияНастроек.БланкДляИмпорта Тогда
					ЗаписатьНастройкуПоУмолчаниюОбработкиШаблонов(Ссылка, "БланкДляИмпорта", НовЭл.Ссылка, , Отказ);
				КонецЕсли;
				
				Если Выборка.Ссылка = ЗначенияНастроек.БланкСводнойТаблицы Тогда
					ЗаписатьНастройкуПоУмолчаниюОбработкиШаблонов(Ссылка, "БланкСводнойТаблицы", НовЭл.Ссылка, , Отказ);
				КонецЕсли;
				
			ИначеЕсли ПараметрКопирования = "ПравилаПроверки" Тогда
				
				Если Выборка.Ссылка = ЗначенияНастроек.ПравилоПроверки Тогда
					ЗаписатьНастройкуПоУмолчаниюОбработкиШаблонов(Ссылка, "ПравилоПроверки", НовЭл.Ссылка, , Отказ);
				КонецЕсли;
	...
	Если КопированиеПравилОбработки Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.Ссылка = ЗначенияНастроек.ПравилоОбработки Тогда
				ЗаписатьНастройкуПоУмолчаниюОбработкиШаблонов(Ссылка, "ПравилоОбработки", НовыйОбъект.Ссылка, , Отказ);
			КонецЕсли;
	...
	Попытка
		...
		Если СтруктураЭлементов.Свойство("БланкОтчета") Тогда
			...
			Если ЗапросПоиск.Выполнить().Пустой() Тогда
				...
				Если ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом() Тогда
					...
					БланкОтчета.Наименование =СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (сводная таблица)'"), СокрЛП(Ссылка.Наименование));
					...
					ЗаписатьНастройкуПоУмолчаниюОбработкиШаблонов(Ссылка, "БланкСводнойТаблицы", БланкОтчета.Ссылка, , Отказ);
					...
					БланкОтчета.ЕдиницаИзмеренияПоУмолчанию = Перечисления.ПорядкиОкругленияОтчетности.Окр1;
					БланкОтчета.Записать();
					ЗаписатьНастройкуПоУмолчаниюОбработкиШаблонов(Ссылка, "БланкДляОтображения", БланкОтчета.Ссылка, , Отказ);
					ЗаписатьНастройкуПоУмолчаниюОбработкиШаблонов(Ссылка, "БланкДляИмпорта", БланкОтчета.Ссылка, , Отказ);
		...
		Если СтруктураЭлементов.Свойство("ПравилоПроверки") Тогда
			...
			Если Результат.Пустой() Тогда
				...
				ЗаписатьНастройкуПоУмолчаниюОбработкиШаблонов(Ссылка, "ПравилоПроверки", ПравилоПроверки, , Отказ);
		...
		Если СтруктураЭлементов.Свойство("ПравилоОбработки") Тогда
			...
			Если ЗапросПоиск.Выполнить().Пустой() Тогда
				...
				ЗаписатьНастройкуПоУмолчаниюОбработкиШаблонов(Ссылка, "ПравилоОбработки", ПравилоОбработки.Ссылка, , Отказ);
				
			КонецЕсли;
			
		КонецЕсли;
		
		Если СтруктураЭлементов.Свойство("СпособФормированияОтчета") Тогда
			
			ПараметрыНастроек = ПараметрыНастроекОбработкиШаблонов();
			ПараметрыНастроек.ШаблонДокумента = Ссылка;
			СпособФормированияОтчета = ЗначениеНастройкиОбработкиШаблонов(ПараметрыНастроек, "СпособФормированияОтчета");
			
			Если НЕ ЗначениеЗаполнено(СпособФормированияОтчета) Тогда
				
				ЗаписатьНастройкуПоУмолчаниюОбработкиШаблонов(Ссылка, "СпособФормированияОтчета", Перечисления.СпособыФормированияОтчетов.ПоПравилуОбработки, , Отказ);
...
Процедура ПроверитьСброситьАктуальностьПараметрики(ОбъектРасчета) Экспорт
...
Функция ЕстьПересчетВВалютуОтчета(ПравилоОбработки,ПоказательОтчета=Неопределено)
...
КонецФункции // ЕстьПересчетВВалютуОтчета() 
...
Процедура ОбработатьПоПараметрическойНастройке(ОбъектРасчета,РежимОбработки,МассивКодовПоказателей=Неопределено) Экспорт
	...
	ПроверитьСброситьАктуальностьПараметрики(ОбъектРасчета);
	...
	Если РежимОбработки=Перечисления.СпособыОбработкиПоказателейОтчетов.Заполнение Тогда
		...
		ПересчитатьВВалютуОтчета=ЕстьПересчетВВалютуОтчета(ОбъектРасчета.ПравилоОбработки);
	...
	Если (Не ОбъектРасчета.ПоказателиДляЗаполнения = Неопределено) И (РежимОбработки = Перечисления.СпособыОбработкиПоказателейОтчетов.Заполнение) Тогда
		...
		Для Каждого ПоказательОбработки Из ОбъектРасчета.мПоказателиДляОбработки Цикл
			...
			Если Не ОбъектРасчета.ПоказателиДляЗаполнения.Найти(ПоказательОбработки.Показатель) = Неопределено Тогда
				...
				РассчитатьЗначениеПоказателя(ОбъектРасчета, РежимОбработки, ПоказательОбработки, РабочаяТаблица,ПересчитатьВВалютуОтчета);
...
Процедура РассчитатьЗначениеПоказателя(ОбъектРасчета,РежимОбработки,ПоказательОбработки,РабочаяТаблица,ПересчитатьВВалютуОтчета=Ложь) Экспорт
		...
		Если ТипЗнч(СтрЗапрос.ТаблицаОперандов)=Тип("ТаблицаЗначений") Тогда // Формируем построитель запроса для списка показателей
			...
			ИначеЕсли СтрЗапрос.СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеПроизвольныйЗапрос Тогда
				...
				Попытка
					...
					РасширениеИнтеграцииУХ.ПолучитьТаблицуДанныхПроизвольногоЗапросаВИБ(ОбъектРасчета,СтрЗапрос);
					
				Исключение
					
					ТекстСообщения = НСтр("ru = 'Системная ошибка. Подробности в полном протоколе.'");
					ПротоколируемыеСобытияУХ.ДобавитьЗаписьОшибка("УправлениеОтчетамиУХ.ПодготовитьТаблицуЗначенийОперандов.9_1", , , ТекстСообщения, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
					ФлагОшибкиИмпорта = Истина;
					Возврат;
	...
	Для Каждого СтрЗапрос Из ТабОперандыПроверка Цикл
		...
		Если ТипЗнч(СтрЗапрос.ТаблицаОперандов)=Тип("ТаблицаЗначений") Тогда // Формируем построитель запроса для списка показателей
			...
			ИначеЕсли СтрЗапрос.СпособПолучения=Перечисления.СпособыПолученияОперандов.ФункцияНаВстроенномЯзыке Тогда
				...
				Исключение
					ТекстОшибки=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Во время исполнения процедуры вычисления операнда 
...
Функция ПолучитьДанныеДляРасчетаДополнениеАналитик(ОбъектРасчета,ПоказательОбработки,РабочаяТаблица,СтрПоказательИтог,СпособОбработки)
	...
	Если ДанныеИсточников.Количество()>0 Тогда
		...
		Для Индекс=1 По ДанныеИсточников.Количество()-1 Цикл
			...
			|ПО "+?(ПустаяСтрока(ТекстСоединенияТек),"ИСТИНА",Сред(ТекстСоединенияТек,СтрДлина("И")+2));
...
Процедура ПересчитатьВВалютуОтчета(ОбъектРасчета,ПоказательОтчета) Экспорт
	
	Если НЕ ЕстьПересчетВВалютуОтчета(ОбъектРасчета.ПравилоОбработки,ПоказательОтчета) Тогда
	...
	|	ИсточникиДанныхДляРасчетов.ПотребительРасчета КАК ПотребительРасчета,
	|	ИсточникиДанныхДляРасчетов.Код КАК Код
	|ПОМЕСТИТЬ ОперандыКПересчету
	|ИЗ
	|	Справочник.ИсточникиДанныхДляРасчетов КАК ИсточникиДанныхДляРасчетов
	|ГДЕ
	|	ИсточникиДанныхДляРасчетов.НазначениеРасчетов = &НазначениеРасчетов
	|	И ИсточникиДанныхДляРасчетов.ПересчитатьВОсновнуюВалюту
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОперандыКПересчету.Код КАК Код,
	|	ВалютныеПоказатели.ВидКурса КАК ВидКурса
	|ПОМЕСТИТЬ КодВидКурса
	|ИЗ
	|	ОперандыКПересчету КАК ОперандыКПересчету
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВалютныеПоказатели КАК ВалютныеПоказатели
	|		ПО ОперандыКПересчету.ПотребительРасчета = ВалютныеПоказатели.Ссылка
	|;
	|ВЫБРАТЬ * ИЗ КодВидКурса";
	...
	|	ВидыКурсов.Ссылка КАК ВидКурса,
	|	ВЫБОР
	|		КОГДА ВидыКурсов.Ссылка = ЗНАЧЕНИЕ(Перечисление.ВидыКурсов.СреднийКурсЗаПериод)
	|			ТОГДА ПериодическиеКурсы.СреднийКурсЗаПериод
	|		КОГДА ВидыКурсов.Ссылка = ЗНАЧЕНИЕ(Перечисление.ВидыКурсов.СреднийКурсЗаПериодМСФО)
	|			ТОГДА ПериодическиеКурсы.СреднийКурсЗаПериодМСФО
	|		КОГДА ВидыКурсов.Ссылка = ЗНАЧЕНИЕ(Перечисление.ВидыКурсов.КурсНаНачалоПериода)
	|			ТОГДА ПериодическиеКурсы.КурсНаНачалоПериода
	|		КОГДА ВидыКурсов.Ссылка = ЗНАЧЕНИЕ(Перечисление.ВидыКурсов.КурсНаКонецПериода)
	|			ТОГДА ПериодическиеКурсы.КурсНаКонецПериода
	|		КОГДА ВидыКурсов.Ссылка = ЗНАЧЕНИЕ(Перечисление.ВидыКурсов.КурсНаКонецПредыдущегоПериода)
	|			ТОГДА ПериодическиеКурсы.КурсНаКонецПредыдущегоПериода
	|		КОГДА ВидыКурсов.Ссылка = ЗНАЧЕНИЕ(Перечисление.ВидыКурсов.СреднийКурсЗаПредыдущийПериод)
	|			ТОГДА ПериодическиеКурсы.СреднийКурсЗаПредыдущийПериод
	|		КОГДА ВидыКурсов.Ссылка = ЗНАЧЕНИЕ(Перечисление.ВидыКурсов.СреднийКурсЗаПериодЦБ)
	|			ТОГДА ПериодическиеКурсы.СреднийКурсЗаПериодЦБ
	|		КОГДА ВидыКурсов.Ссылка = ЗНАЧЕНИЕ(Перечисление.ВидыКурсов.СреднийКурсЗаПредыдущийПериодЦБ)
	|			ТОГДА ПериодическиеКурсы.СреднийКурсЗаПредыдущийПериодЦБ
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Курс,
	|	ПериодическиеКурсы.Кратность КАК Кратность
	|ПОМЕСТИТЬ КурсыВалютРегистр
	|ИЗ
	|	РегистрСведений.ПериодическиеКурсы КАК ПериодическиеКурсы,
	|	Перечисление.ВидыКурсов КАК ВидыКурсов
	|ГДЕ
	...
	|	КурсыВалют.ВидКурса КАК ВидКурса,
	|	ВЫБОР
	...
	|ПОМЕСТИТЬ КурсыВалютПересчетаВОсновнуюВалюту
	|ИЗ
	|	КурсыВалютРегистр КАК КурсыВалют
	|		ЛЕВОЕ СОЕДИНЕНИЕ КурсыВалютРегистр КАК КурсВалютыОтчета
	|		ПО КурсыВалют.ВидКурса = КурсВалютыОтчета.ВидКурса
	|			И (КурсВалютыОтчета.Валюта = &Валюта)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КодВидКурса.Код КАК Код,
	|	КурсыВалютПересчетаВОсновнуюВалюту.Валюта КАК Валюта,
	|	КурсыВалютПересчетаВОсновнуюВалюту.КоэффициентПересчета КАК КоэффициентПересчета
	|ПОМЕСТИТЬ КодКурсПересчета
	|ИЗ
	|	КодВидКурса КАК КодВидКурса
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ КурсыВалютПересчетаВОсновнуюВалюту КАК КурсыВалютПересчетаВОсновнуюВалюту
	|		ПО КодВидКурса.ВидКурса = КурсыВалютПересчетаВОсновнуюВалюту.ВидКурса
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	мЗначенияОперандов.КодОперанда КАК КодОперанда,
	|	мЗначенияОперандов.Аналитика1 КАК Аналитика1,
	|	мЗначенияОперандов.Аналитика2 КАК Аналитика2,
	|	мЗначенияОперандов.Аналитика3 КАК Аналитика3,
	|	мЗначенияОперандов.Аналитика4 КАК Аналитика4,
	|	мЗначенияОперандов.Аналитика5 КАК Аналитика5,
	|	мЗначенияОперандов.Аналитика6 КАК Аналитика6,
	|	мЗначенияОперандов.АналитикаВалюта КАК АналитикаВалюта,
	|	мЗначенияОперандов.Значение КАК Значение,
	|	мЗначенияОперандов.ЗначениеВалюта КАК ЗначениеВалюта,
	|	мЗначенияОперандов.ИтогПоПоказателю КАК ИтогПоПоказателю
	|ПОМЕСТИТЬ мЗначенияОперандов
	|ИЗ
	|	&мЗначенияОперандов КАК мЗначенияОперандов
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	мЗначенияОперандов.ЗначениеВалюта КАК ЗначениеВалюта,
	|	мЗначенияОперандов.ИтогПоПоказателю КАК ИтогПоПоказателю
	|ПОМЕСТИТЬ ЗначенияБезПересчета
	|ИЗ
	|	мЗначенияОперандов КАК мЗначенияОперандов
	|ГДЕ
	|	НЕ мЗначенияОперандов.КодОперанда В
	|				(ВЫБРАТЬ
	|					КодВидКурса.Код
	|				ИЗ
	|					КодВидКурса)
	...
	|	мЗначенияОперандов.Аналитика6 КАК Аналитика6,
	|	мЗначенияОперандов.АналитикаВалюта КАК АналитикаВалюта,
	|	ВЫРАЗИТЬ(мЗначенияОперандов.Значение КАК ЧИСЛО(18, 5)) КАК Значение,
	|	мЗначенияОперандов.ЗначениеВалюта КАК ЗначениеВалюта,
	|	мЗначенияОперандов.ИтогПоПоказателю КАК ИтогПоПоказателю
	|ПОМЕСТИТЬ ЗначенияКПересчету
	|ИЗ
	|	мЗначенияОперандов КАК мЗначенияОперандов
	|ГДЕ
	|	мЗначенияОперандов.КодОперанда В
	|			(ВЫБРАТЬ
	|				КодВидКурса.Код
	|			ИЗ
	|				КодВидКурса)
	|	И НЕ мЗначенияОперандов.ИтогПоПоказателю
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗначенияБезПересчета.КодОперанда КАК КодОперанда,
	|	ЗначенияБезПересчета.Аналитика1 КАК Аналитика1,
	|	ЗначенияБезПересчета.Аналитика2 КАК Аналитика2,
	|	ЗначенияБезПересчета.Аналитика3 КАК Аналитика3,
	|	ЗначенияБезПересчета.Аналитика4 КАК Аналитика4,
	|	ЗначенияБезПересчета.Аналитика5 КАК Аналитика5,
	|	ЗначенияБезПересчета.Аналитика6 КАК Аналитика6,
	|	ЗначенияБезПересчета.АналитикаВалюта КАК АналитикаВалюта,
	|	ЗначенияБезПересчета.Значение КАК Значение,
	|	ЗначенияБезПересчета.ЗначениеВалюта КАК ЗначениеВалюта,
	|	ЗначенияБезПересчета.ИтогПоПоказателю КАК ИтогПоПоказателю
	|ИЗ
	|	ЗначенияБезПересчета КАК ЗначенияБезПересчета
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ЗначенияКПересчету.КодОперанда,
	|	ЗначенияКПересчету.Аналитика1,
	|	ЗначенияКПересчету.Аналитика2,
	|	ЗначенияКПересчету.Аналитика3,
	|	ЗначенияКПересчету.Аналитика4,
	|	ЗначенияКПересчету.Аналитика5,
	|	ЗначенияКПересчету.Аналитика6,
	|	ЗначенияКПересчету.АналитикаВалюта,
	|	ЗначенияКПересчету.Значение * КодКурсПересчета.КоэффициентПересчета / 1000000,
	|	ЗначенияКПересчету.ЗначениеВалюта,
	|	ЗначенияКПересчету.ИтогПоПоказателю
	|ИЗ
	|	ЗначенияКПересчету КАК ЗначенияКПересчету
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ КодКурсПересчета КАК КодКурсПересчета
	|		ПО ЗначенияКПересчету.КодОперанда = КодКурсПересчета.Код
	|			И ЗначенияКПересчету.АналитикаВалюта = КодКурсПересчета.Валюта";
	
	Запрос.УстановитьПараметр("Сценарий",ОбъектРасчета.Сценарий);
	Запрос.УстановитьПараметр("Валюта",ОбъектРасчета.ОсновнаяВалюта);
	Запрос.УстановитьПараметр("ПериодКурса",ОбъектРасчета.ПериодОтчета);
	Запрос.УстановитьПараметр("мЗначенияОперандов",ОбъектРасчета.мЗначенияОперандов);
	
	ОбъектРасчета.Вставить("мЗначенияОперандов",Запрос.Выполнить().Выгрузить());
		
КонецПроцедуры // ПересчитатьВВалютуОтчета() 

Процедура РассчитатьПараметрическиНастроенныйПоказатель(ОбъектРасчета,ПоказательОбработки,РабочаяТаблица,СтрПоказательИтог,СпособОбработки,ПересчитыватьВВалютуОтчета=Ложь)
	
	Перем СтрИтог; // Строка, в которую будет помещено итоговое значение показателя
	
	Если ПересчитыватьВВалютуОтчета Тогда
		
		ПересчитатьВВалютуОтчета(ОбъектРасчета,ПоказательОбработки.Показатель);
...
Функция ПересчитатьЗначениеПоказателяПоКурсу(ОбъектРасчета,ЗначениеПоказателя, Показатель, ТекущаяВалюта, ВалютаПересчета,ПризнакОшибки=Ложь) Экспорт
	...
	Если (НЕ Показатель.НеФинансовый) 
		...
		Иначе
			...
			ШапкаОшибки   = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось пересчитать показатель %1 из валюты %2"" в валюту ""%3"":'"), 
	...
	Если ОбъектРасчета.ТрансформационнаяКорректировка Тогда
		...
		Если ЗначениеЗаполнено(РабочееЗначение) Тогда
			...
			Для Каждого КолонкаИзмерение Из ОбъектРасчета.МассивКолонокИзмерений Цикл
				...
				Если СтрНайти(КолонкаИзмерение,"_Валюта")>0 Тогда
			...
			Если МассивСтрок.Количество()>0 Тогда
				...
				СтрокаПроводки=МассивСтрок[0];
				
				Если ДанныеПоказателя.РесурсРегистра="Сумма" ИЛИ ДанныеПоказателя.РесурсРегистра="СуммаВВалютеУчета" Тогда
					
					СтрокаПроводки.Значение			= РабочееЗначение;
					
				ИначеЕсли ДанныеПоказателя.РесурсРегистра="СуммаВВалютеОтчетности" Тогда
					
					СтрокаПроводки.ПредставлениеЗначения			= РабочееЗначение;
					
				ИначеЕсли ДанныеПоказателя.РесурсРегистра="Количество" Тогда
					
					Если СтрНайти(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаПроводки.СчетДт,"ПризнакиУчета"),"Количественный")>0 Тогда
						
						СтрокаПроводки.КоличествоДт=РабочееЗначение;
						
					КонецЕсли;
					
					Если СтрНайти(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаПроводки.СчетКт,"ПризнакиУчета"),"Количественный")>0 Тогда
						
						СтрокаПроводки.КоличествоКт=РабочееЗначение;
				...
				Если ДанныеПоказателя.ВалютныйДт=Истина Тогда 
					...
					Если ДанныеПоказателя.РесурсРегистра="ВалютнаяСумма" ИЛИ ДанныеПоказателя.РесурсРегистра="СуммаВВалюте" Тогда
						
						СтрокаПроводки.ЗначениеВалютаДт=РабочееЗначение;
				...
				Если ДанныеПоказателя.ВалютныйКт=Истина Тогда
					...
					Если ДанныеПоказателя.РесурсРегистра="ВалютнаяСумма" ИЛИ ДанныеПоказателя.РесурсРегистра="СуммаВВалюте" Тогда
						...
						СтрокаПроводки.ЗначениеВалютаКт=РабочееЗначение;
...
Процедура ЗаполнитьАналитикиПринудительно(ТаблицаДанных,ПравилаИспользованияПолей,КонтекстОтчета=Неопределено) Экспорт
	...
	Для Каждого СтрПоле Из СтрФиксированныеЗначения Цикл
		...
		Если ТаблицаДанных.Колонки.Найти(СтрПоле.КодАналитики)=Неопределено Тогда
			...
			ТаблицаДанных.Колонки.Добавить(СтрПоле.КодАналитики, ОбщегоНазначенияУХ.ПолучитьОписаниеТиповПоОбъекту(СтрПоле.ФиксированноеЗначение));
	...
	Для Каждого СтрПоле Из СтрКонтекст Цикл
		...
		Если ТаблицаДанных.Колонки.Найти(СтрПоле.КодАналитики)=Неопределено Тогда 
			...
			Если ТипЗнч(ЗначениеЗаполнения)=Тип("Число") Тогда
				...
				ТипКолонки=ОбщегоНазначенияУХ.ПолучитьОписаниеТиповЧисла(18,5);
				
			ИначеЕсли ТипЗнч(ЗначениеЗаполнения)=Тип("Строка") Тогда
				
				ТипКолонки=ОбщегоНазначенияУХ.ПолучитьОписаниеТиповСтроки(255);
				
			ИначеЕсли ТипЗнч(ЗначениеЗаполнения)=Тип("Булево") Тогда
				
				ТипКолонки=ОбщегоНазначенияУХ.ПолучитьОписаниеТиповБулево();
				
			ИначеЕсли ТипЗнч(ЗначениеЗаполнения)=Тип("Дата") Тогда
				
				ТипКолонки=ОбщегоНазначенияУХ.ПолучитьОписаниеТиповДаты(ЧастиДаты.ДатаВремя); 
				
			Иначе
				
				ТипКолонки=Метаданные.ПланыВидовХарактеристик.ВидыСубконтоКорпоративные.Тип;
				
			КонецЕсли;
			
			ТаблицаДанных.Колонки.Добавить(СтрПоле.КодАналитики, ТипКолонки);
...
Функция ПолучитьИмяКолонкиЗначение(ПравилаИспользованияПолейЗапроса)
...
КонецФункции // ПолучитьИмяКолонкиЗначение() 
...
Функция ПолучитьДанныеРегистраВнутр(ОбъектРасчета,СтрЗапрос)
	...
	Если СтрЗапрос.ТаблицаОперандов.Количество()=1 Тогда
		...
		Если ТабРезультат.Колонки.Найти("Значение")=Неопределено Тогда
			
			КолонкаЗначение=ПолучитьИмяКолонкиЗначение(СтрЗапрос.ТаблицаОперандов[0].ПравилаИспользованияПолейЗапроса);
			
		Иначе
			
			КолонкаЗначение="Значение";
			
		КонецЕсли;
		
		Если ТабРезультат.Колонки[КолонкаЗначение].ТипЗначения.СодержитТип(Тип("Число")) Тогда
			
			ИтоговоеЗначение=ТабРезультат.Итог(КолонкаЗначение);
		...
		ИначеЕсли ТабРезультат.Количество()=1 Тогда
			...
			ИтоговоеЗначение=ТабРезультат[0][КолонкаЗначение];
			НоваяСтрока=ОбъектРасчета.мЗначенияОперандов.Добавить();
			НоваяСтрока.ИтогПоПоказателю=Истина;
			НоваяСтрока.КодОперанда=КодОперанда;
			НоваяСтрока.Значение=ИтоговоеЗначение;
		...
		Если ЕстьАналитики Тогда
			...
			Для Каждого СтрРезультат Из ТабРезультат Цикл
				...
				НоваяСтрока.Значение=СтрРезультат[КолонкаЗначение];
...
Процедура ПолучитьДанныеРегистраБухгалтерииТек(ОбъектРасчета,СтрЗапрос)
	...
	РазобратьСводныеДанныеРегистраБухгалтерииТек(ОбъектРасчета,ДанныеЗапроса.ТаблицаДанных,СтрЗапрос.ТаблицаОперандов,ДанныеЗапроса.ТабСчетаОперандов,ДанныеЗапроса.СоответствиеРесурсов,СтрЗапрос.ТабПравилаИспользованияПолей);
		
КонецПроцедуры // ПолучитьДанныеРегистраБухгалтерииТек()

Процедура ПоместитьЗначенияВТаблицуОперандовТек(ОбъектРасчета,ТабОперандов,ТабСчетаОперандов,СоответствиеРесурсов,МассивРесурсов,ЕстьРазворотПоАналитикам,СписокКодовОперандов=Неопределено)
...
Процедура РазобратьСводныеДанныеРегистраБухгалтерииТек(ОбъектРасчета,ТаблицаДанных,ТаблицаОперандов,ТабСчетаОперандов,СоответствиеРесурсов,ТабПравилаИспользованияПолей);
	
	Если ТаблицаДанных.Количество()=0 Тогда
		Возврат;
	КонецЕсли;
	
	МассивРесурсов=Документы.НастраиваемыйОтчет.ПолучитьТаблицуРесурсов(ТаблицаОперандов).ВыгрузитьКолонку("Ресурс");
	...
	Если ТабПравилаИспользованияПолей.Количество()=1 Тогда 
		...
		ПоместитьЗначенияВТаблицуОперандовТек(ОбъектРасчета,ТаблицаДанных,ТабСчетаОперандов,СоответствиеРесурсов,МассивРесурсов,ЕстьРазворотПоАналитикам);
	...
	Иначе
		...
		Для Каждого СтрПравило Из ТабПравилаИспользованияПолей Цикл
			...
			ПоместитьЗначенияВТаблицуОперандовТек(ОбъектРасчета,ТекТаблицаДанных,ТабСчетаОперандов,СоответствиеРесурсов,МассивРесурсов,НЕ (ПравилаИспользованияПолей.Количество()=1 И СтрНайти(ПравилаИспользованияПолей[0].КодАналитики,"Аналитика")=0),СтрПравило.СписокКодовОперандов);
...
Процедура ПолучитьДанныеВнутрТекущий(ОбъектРасчета,ТабРезультат,СтрЗапрос)
	...
	Если НЕ ЕстьИтог И ТабРезультат.Количество() > 0 Тогда
		...
		ИтоговоеЗначение=ОбщегоНазначенияУХ.ТЗИтог(ПромТаблицаОперанда,
													Неопределено,	
													"Значение",
													СтрЗапрос.Показатель.ВидИтога,
													ТекТипЗначения);
...
Функция ПодготовитьТаблицуРасчетаНечисловыхЗначений(ОбъектРасчета,РабочаяТаблица,ТекстСверткиИзмерения,ТекстСверткиОперанды,ТипЗначения=Неопределено)
	...
	Для Каждого Строка Из ТаблицаИзмерений Цикл
		...
		Для Каждого КолонкаОперанд Из МассивКолонокОперандов Цикл
			...
			НоваяСтрока[КолонкаОперанд]=ОбщегоНазначенияУХ.ТЗИтог(ТаблицаТек,
																	Неопределено,
																	КолонкаОперанд,
																	Перечисления.ВидыИтоговПоказателя.Сумма,
																	ТекТипЗначения);
...
Функция ПолучитьТаблицуЗначенийПоказателя(Сценарий,Организация,Знач Период,Знач Валюта=Неопределено,Знач ВидОтчета=Неопределено,Знач СчетБД=Неопределено,Знач ВидИтогаПоСчету=Неопределено,Знач Показатель=Неопределено, ОтборАналитикВидаОтчета=Неопределено) Экспорт
	...
	Если СтрНайти(ТекстЗапросаАналитика,"ЗначенияПоказателейОтчетовНечисловые")>0 
		...
		Запрос.Текст=Сред(ТекстЗапроса,СтрДлина("ОБЪЕДИНИТЬ ВСЕ") + 2);
	...
	Иначе
		...
		|ИЗ ("+Сред(ТекстЗапроса,СтрДлина("ОБЪЕДИНИТЬ ВСЕ") + 2)+") КАК ТекстВыборкиИтог
...
Процедура ОбновитьФормулыРасчетаПоказателиВПоказатели(ШаблонТрансляции,ПравилоОбработки,СчетПриемник=Неопределено) Экспорт
	...
	Пока ПоказательПриемник.Следующий() Цикл
		...
		УправлениеОтчетамиУХ.ОтметитьИспользованиеОперандов(ПравилоОбработки,
									ПоказательПриемник.Показатель, 
									Неопределено, 
									Перечисления.СпособыИспользованияОперандов.ДляФормулРасчета,СтруктураПараметров.ТекстПроцедуры);	
...
Процедура ОбновитьФормулыРасчетаРегистрВПоказатели(ШаблонТрансляции,ПравилоОбработки,СчетПриемник=Неопределено) Экспорт
	...
	Пока ПоказательПриемник.Следующий() Цикл
		...
		УправлениеОтчетамиУХ.ОтметитьИспользованиеОперандов(ПравилоОбработки,
															...
															Неопределено,
															Перечисления.СпособыИспользованияОперандов.ДляФормулРасчета,
															СтруктураПараметров.ТекстПроцедуры);	
		УправлениеОтчетамиУХ.ЗаписатьПроцедуруРасчета(СтруктураПараметров);
...
Функция ПолучитьДанныеЗакрытогоПериода(Организация)  Экспорт
	...
	Иначе
		...
		ДанныеПериода.Вставить("ДатаЗакрытияПериода",	Дата(1, 1, 1, 0, 0, 0));		
...
Процедура ПриИзмененииРегистраБухгалтерииТрансляцияПриЗаписи(Источник, Отказ, РежимЗаписи) Экспорт
	...
	// НП: «Распределение РБП» не отражается по МСФО.
	// Иначе подписка пишет РС ОтражениеДокументовПоШаблонам и роняет
	// «Отражение документов в регламентированном учете» при закрытии месяца.
	Если ЭтоДокументИсключенныйИзТрансляцииПоШаблонам(Источник) Тогда
	...
	Если НЕ Метаданные.ОбщиеМодули.Найти("КорректировкиЗначенийПоказателей32")=Неопределено Тогда
		...
		МодульКорректировкиЗначенийПоказателей32 = ОбщегоНазначения.ОбщийМодуль("КорректировкиЗначенийПоказателей32");
		МодульКорректировкиЗначенийПоказателей32.ПриИзмененииРегистраБухгалтерииТрансляция(Источник, Отказ, РежимЗаписи);
		
	Иначе
		
		ИмяОбработки="КорректировкиЗначенийПоказателей";
		
		Обработки[ИмяОбработки].ПриИзмененииРегистраБухгалтерииТрансляция(Источник, Отказ, РежимЗаписи);
		
	КонецЕсли;
	
КонецПроцедуры

// Возвращает Истина, если регистратор набора бухгалтерии не должен
// порождать записи РС ОтражениеДокументовПоШаблонам (трансляция НСБУ-МСФО).
//
Функция ЭтоДокументИсключенныйИзТрансляцииПоШаблонам(Источник)
	
	ИсходныйДокумент = Неопределено;
	
	Если Источник.Количество() > 0 Тогда
		
		ИсходныйДокумент = Источник[0].Регистратор;
	...
	Возврат ТипЗнч(ИсходныйДокумент) = Тип("ДокументСсылка.РаспределениеРасходовБудущихПериодов");
...
Процедура ОбновитьБланкОтчета(ВидОтчета,БланкОтчета) Экспорт
	...
	ИмяОбработки="МастерГенерацииМакетаОтчета";
...
Процедура ОбновитьСтруктуруОСВПриИзмененииСчета(ПланСчетов,Счет,ОбновитьТР) Экспорт
	...
	ОписаниеЗадания=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Обновление ОСВ %1 при изменении счета %2.'"), ВидОтчетаОСВ, 
	                	Счет);
...
Функция ДанныеУправлениеПериодомСценария(Сценарий, Период) Экспорт
	
	СтруктураДанных = Новый Структура;
	СтруктураДанных.Вставить("УправлениеПериодом", Неопределено);
	СтруктураДанных.Вставить("ВерсияРегламента", Неопределено);
...
Функция НайтиДокументУправлениеОтчетнымПериодом(СценарийВход, ПериодВход) Экспорт
	...
	Возврат ДанныеУправлениеПериодомСценария(СценарийВход, ПериодВход).УправлениеПериодом;
...
Функция ПолучитьВерсиюРегламентаПоПериодуИСценарию(СценарийВход, ПериодВход) Экспорт
	...
	Возврат ДанныеУправлениеПериодомСценария(СценарийВход, ПериодВход).ВерсияРегламента;
...
Функция ПолучитьАналитикиТекущегоГоризонта(УправлениеПериодом, ПериодСценария, ПериодСценарияОкончание, Сценарий, ВерсияРегламента) Экспорт
...
Процедура ДополнитьДанныеОтборамиШаблона(Операнд,ПолеТекстЗапроса) Экспорт 
	...
	Если НЕ ЗначениеЗаполнено(Операнд.ИсточникШаблон) Тогда
	...
	Запрос.УстановитьПараметр("Ссылка",Операнд.ИсточникШаблон);
	ДанныеШаблона=Запрос.Выполнить().Выгрузить()[0];
	
	Если ДанныеШаблона.ТаблицаПараметровОтбораБД.Количество()=0 Тогда // Нет дополнительных отборов в шаблоне
		
		Операнд[ПолеТекстЗапроса]=СтрЗаменить(Операнд[ПолеТекстЗапроса],"%ОтборШаблон%","");
		Возврат;
		
	КонецЕсли;
	
	ДополнитьДанныеСохраненнымиНастройками(ДанныеШаблона,"ТекстОтбора");
	...
	Операнд[ПолеТекстЗапроса]=СтрЗаменить(Операнд[ПолеТекстЗапроса],"%ОтборШаблон%"," И "+ДанныеШаблона.ТекстОтбора);
		
КонецПроцедуры // ДополнитьДанныеОтборамиШаблона()

Процедура ДополнитьДанныеСохраненнымиНастройками(Операнд,ПолеТекстЗапроса) Экспорт
	
	МассивСохраненныхНастроек=Операнд.ТаблицаПараметровОтбораБД.НайтиСтроки(Новый Структура("СпособВычисленияПараметра",Перечисления.СпособыВычисленияПараметровОперандов.СохраненнаяНастройкаОтбора));
	
	Для Каждого СтрНастройка Из МассивСохраненныхНастроек Цикл

	   ОбработатьСтрокуСохраненныхНастроек(Операнд,СтрНастройка,ПолеТекстЗапроса);
	   
	КонецЦикла;
		
КонецПроцедуры // ДополнитьДанныеСохраненнымиНастройками() 

Процедура ОбработатьСтрокуСохраненныхНастроек(Операнд,СтрНастройка,ПолеТекстЗапроса)
	
	СтрУточнение=Операнд.УточненияСпособовОпределения.Найти(СтрНастройка.ПолеБД,"ПолеБД");
	
	Запрос=Новый Запрос;
	Запрос.Текст="ВЫБРАТЬ
	             |	СохраненныеНастройкиОтборов.Ссылка КАК Ссылка,
	             |	СохраненныеНастройкиОтборов.Код КАК Код,
	             |	СохраненныеНастройкиОтборов.ТаблицаПараметровОтбораБД.(
	             |		ПолеБД КАК ПолеБД,
	             |		СпособВычисленияПараметра КАК СпособВычисленияПараметра,
	             |		ЗначениеДляПроверки КАК ЗначениеДляПроверки,
	             |		ТипРасчета КАК ТипРасчета,
	             ...
	             |		ИмяПараметра КАК ИмяПараметра
	             |	) КАК ТаблицаПараметровОтбораБД,
	             |	СохраненныеНастройкиОтборов.УточненияСпособовОпределения.(
	             |		ИмяПараметра КАК ИмяПараметра,
	             |		Значение КАК Значение,
	             |		Представление КАК Представление,
	             |		Пометка КАК Пометка,
	             ...
	             |		ПолеБД КАК ПолеБД
	             |	) КАК УточненияСпособовОпределения,
	             |	СохраненныеНастройкиОтборов.ТекстЗапросаМодуля КАК ТекстЗапросаМодуля,
	             |	СохраненныеНастройкиОтборов.ТекстОтбора КАК ТекстОтбора
	             |ИЗ
	             |	&СправочникНастройки КАК СохраненныеНастройкиОтборов
	             |ГДЕ
	             |	СохраненныеНастройкиОтборов.Ссылка = &Ссылка";
	
	Запрос.Текст=СтрЗаменить(Запрос.Текст,"&СправочникНастройки","Справочник.СохраненныеНастройкиОтборов");
	Запрос.УстановитьПараметр("Ссылка",СтрУточнение.Значение);
	
	ДанныеНастройки=Запрос.Выполнить().Выгрузить()[0];
	
	Для Каждого Строка ИЗ ДанныеНастройки.ТаблицаПараметровОтбораБД Цикл
		
		НоваяСтрока=Операнд.ТаблицаПараметровОтбораБД.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока,Строка,,"ПолеБД");
		
		Если Строка.ПолеБД="Ссылка" Тогда
			
			НоваяСтрока.ПолеБД=СтрНастройка.ПолеБД;
			
		Иначе
			
			НоваяСтрока.ПолеБД=СтрНастройка.ПолеБД+"."+Строка.ПолеБД;
			
		КонецЕсли;
			
	КонецЦикла; 
	
	Для Каждого Строка ИЗ ДанныеНастройки.УточненияСпособовОпределения Цикл
		
		НоваяСтрока=Операнд.УточненияСпособовОпределения.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока,Строка,,"ПолеБД");
	...
	ТекстДляОбработки=Операнд[ПолеТекстЗапроса];
	
	СтрокаПоиска="%"+СокрЛП(ДанныеНастройки.Код)+"%";
	
	МассивСтрок=ОбщегоНазначенияУХ.РазложитьСтрокуВМассивПодстрок(ТекстДляОбработки,СтрокаПоиска);
	
	Для Каждого Строка Из МассивСтрок Цикл 
		
		Если СтрНайти(Строка,"#")>0 Тогда
			
			ПолеБД=СтрЗаменить(Строка,"#","");
			
			Если ПустаяСтрока(ДанныеНастройки.ТекстОтбора) Тогда
				
				ТекстОтбора="ИСТИНА";
				
			Иначе
				
				ТекстОтбора=СтрЗаменить(ДанныеНастройки.ТекстОтбора,"Ссылка",ПолеБД); 
				
			КонецЕсли;
			
			ТекстДляОбработки=СтрЗаменить(ТекстДляОбработки,СтрокаПоиска+Строка+СтрокаПоиска,ТекстОтбора);
			
		КонецЕсли;
		
	КонецЦикла;	
	
	Операнд[ПолеТекстЗапроса]=ТекстДляОбработки;	
	
	Операнд.ТаблицаПараметровОтбораБД.Удалить(СтрНастройка);
	Операнд.УточненияСпособовОпределения.Удалить(СтрУточнение);
		
КонецПроцедуры // ОбработатьСтрокуСохраненныхНастроек() 
...
#Область НастройкиОбработкиШаблоновДокументов

Функция ПараметрыНастроекОбработкиШаблонов(НачальныеНастройки = Неопределено) Экспорт
	
	ПараметрыНастроек = Новый Структура;
	ПараметрыНастроек.Вставить("ШаблонДокумента", НЕОПРЕДЕЛЕНО);
	ПараметрыНастроек.Вставить("ВерсияРегламента", Справочники.ВерсииРегламентовПодготовкиОтчетности.ПустаяСсылка());
	ПараметрыНастроек.Вставить("УправлениеПериодом", Документы.УправлениеПериодомСценария.ПустаяСсылка());
	ПараметрыНастроек.Вставить("Организация", Справочники.Организации.ПустаяСсылка());
	ПараметрыНастроек.Вставить("Проект", Справочники.Проекты.ПустаяСсылка());
	
	Для Сч = 1 По ПараметрыСеанса.ЧислоДопАналитик Цикл
		ПараметрыНастроек.Вставить("Аналитика" + Сч, НЕОПРЕДЕЛЕНО);
	КонецЦикла;
	
	// Данные для расчета по документу
	ПараметрыНастроек.Вставить("Сценарий", Справочники.Организации.ПустаяСсылка());
	ПараметрыНастроек.Вставить("ПериодОтчета", Справочники.Проекты.ПустаяСсылка());
	
	Если НачальныеНастройки <> Неопределено Тогда
		
		ПараметрыНастроек.Вставить("ВидОтчета", Неопределено);
		ЗаполнитьЗначенияСвойств(ПараметрыНастроек, НачальныеНастройки);
	
		Если НЕ ЗначениеЗаполнено(ПараметрыНастроек.ШаблонДокумента) И
			ЗначениеЗаполнено(ПараметрыНастроек.ВидОтчета) Тогда
			ПараметрыНастроек.ШаблонДокумента = ПараметрыНастроек.ВидОтчета;
		КонецЕсли;
...
Функция ЗначенияНастроекОбработкиШаблонов(ПараметрыНастроек, ПоГруппе = Ложь) Экспорт
	
	// Выполним расчет дополнительных параметров, если необходимо
	УправлениеПериодом = ПараметрыНастроек.УправлениеПериодом;
	ВерсияРегламента = ПараметрыНастроек.ВерсияРегламента;
	
	Если НЕ ЗначениеЗаполнено(УправлениеПериодом)
		И ЗначениеЗаполнено(ПараметрыНастроек.Сценарий)
		И ЗначениеЗаполнено(ПараметрыНастроек.ПериодОтчета) Тогда
		
		ДанныеУправлениеПериодомСценария = ДанныеУправлениеПериодомСценария(
			ПараметрыНастроек.Сценарий, ПараметрыНастроек.ПериодОтчета);
		УправлениеПериодом = ДанныеУправлениеПериодомСценария.УправлениеПериодом;
		ВерсияРегламента = ДанныеУправлениеПериодомСценария.ВерсияРегламента;
	
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ВерсияРегламента)
		И ЗначениеЗаполнено(УправлениеПериодом) Тогда
		ВерсияРегламента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УправлениеПериодом, "ВерсияОрганизационнойСтруктуры");
	КонецЕсли;
	...
	Запрос.Текст =
		...
		|ИЗ
		|	РегистрСведений.НастройкиОбработкиШаблоновДокументов КАК ДанныеРегистра
		|ГДЕ
		|	ДанныеРегистра.ШаблонДокумента = &ШаблонДокумента
		|	И ДанныеРегистра.ВерсияРегламента В (&ВерсияРегламента, ЗНАЧЕНИЕ(Справочник.ВерсииРегламентовПодготовкиОтчетности.ПустаяСсылка))
		|	И ДанныеРегистра.УправлениеПериодом В (&УправлениеПериодом, ЗНАЧЕНИЕ(Документ.УправлениеПериодомСценария.ПустаяСсылка))
		|	И ДанныеРегистра.Организация В (&Организация, ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка))
		|	И ДанныеРегистра.Проект В (&Проект, ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка))
		|	И ДанныеРегистра.Аналитика1 В (&Аналитика1, НЕОПРЕДЕЛЕНО)
		|	И ДанныеРегистра.Аналитика2 В (&Аналитика2, НЕОПРЕДЕЛЕНО)
		|	И ДанныеРегистра.Аналитика3 В (&Аналитика3, НЕОПРЕДЕЛЕНО)
		|	И ДанныеРегистра.Аналитика4 В (&Аналитика4, НЕОПРЕДЕЛЕНО)
		|	И ДанныеРегистра.Аналитика5 В (&Аналитика5, НЕОПРЕДЕЛЕНО)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ДанныеРегистра.ШаблонДокумента КАК ШаблонДокумента,
		|	ДанныеРегистра.ЭлементНастройки КАК ЭлементНастройки,
		|	МАКСИМУМ(ДанныеРегистра.Вес) КАК Вес
		...
		|	ДанныеРегистра.ШаблонДокумента КАК ШаблонДокумента,
		|	МАКСИМУМ(ВЫБОР
		|			КОГДА ДанныеРегистра.ЭлементНастройки = ЗНАЧЕНИЕ(Перечисление.ЭлементыНастройкиОтчета.БланкДляОтображения)
		|				ТОГДА ДанныеРегистра.ЗначениеНастройки
		|			ИНАЧЕ NULL
		|		КОНЕЦ) КАК БланкДляОтображения,
		|	МАКСИМУМ(ВЫБОР
		|			КОГДА ДанныеРегистра.ЭлементНастройки = ЗНАЧЕНИЕ(Перечисление.ЭлементыНастройкиОтчета.БланкДляИмпорта)
		|				ТОГДА ДанныеРегистра.ЗначениеНастройки
		|			ИНАЧЕ NULL
		|		КОНЕЦ) КАК БланкДляИмпорта,
		|	МАКСИМУМ(ВЫБОР
		|			КОГДА ДанныеРегистра.ЭлементНастройки = ЗНАЧЕНИЕ(Перечисление.ЭлементыНастройкиОтчета.БланкСводнойТаблицы)
		|				ТОГДА ДанныеРегистра.ЗначениеНастройки
		|			ИНАЧЕ NULL
		|		КОНЕЦ) КАК БланкСводнойТаблицы,
		|	МАКСИМУМ(ВЫБОР
		|			КОГДА ДанныеРегистра.ЭлементНастройки = ЗНАЧЕНИЕ(Перечисление.ЭлементыНастройкиОтчета.ПравилоОбработки)
		|				ТОГДА ДанныеРегистра.ЗначениеНастройки
		|			ИНАЧЕ NULL
		|		КОНЕЦ) КАК ПравилоОбработки,
		|	МАКСИМУМ(ВЫБОР
		|			КОГДА ДанныеРегистра.ЭлементНастройки = ЗНАЧЕНИЕ(Перечисление.ЭлементыНастройкиОтчета.ПравилоПроверки)
		|				ТОГДА ДанныеРегистра.ЗначениеНастройки
		|			ИНАЧЕ NULL
		|		КОНЕЦ) КАК ПравилоПроверки,
		|	МАКСИМУМ(ВЫБОР
		|			КОГДА ДанныеРегистра.ЭлементНастройки = ЗНАЧЕНИЕ(Перечисление.ЭлементыНастройкиОтчета.СпособФормированияОтчета)
		|				ТОГДА ДанныеРегистра.ЗначениеНастройки
		|			ИНАЧЕ NULL
		|		КОНЕЦ) КАК СпособФормированияОтчета,
		|	МАКСИМУМ(ВЫБОР
		|			КОГДА ДанныеРегистра.ЭлементНастройки = ЗНАЧЕНИЕ(Перечисление.ЭлементыНастройкиОтчета.ВнешняяИнформационнаяБаза)
		|				ТОГДА ДанныеРегистра.ЗначениеНастройки
		|			ИНАЧЕ NULL
		|		КОНЕЦ) КАК ВнешняяИнформационнаяБаза,
		|	МАКСИМУМ(ВЫБОР
		|			КОГДА ДанныеРегистра.ЭлементНастройки = ЗНАЧЕНИЕ(Перечисление.ЭлементыНастройкиОтчета.ПериодичностьСвертывания)
		|				ТОГДА ДанныеРегистра.ЗначениеНастройки
		|			ИНАЧЕ NULL
		...
		|		ПО ДанныеРегистра.ШаблонДокумента = МаксимальныеВеса.ШаблонДокумента
		|			И ДанныеРегистра.ЭлементНастройки = МаксимальныеВеса.ЭлементНастройки
		|			И ДанныеРегистра.Вес = МаксимальныеВеса.Вес
		|
		|СГРУППИРОВАТЬ ПО
		|	ДанныеРегистра.ШаблонДокумента";
	...
	Если ПоГруппе Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст,
			...
			|		ПО ДанныеРегистра.ШаблонДокумента = ВидыОтчетов.Ссылка
			|ГДЕ
			|	ВидыОтчетов.Ссылка В ИЕРАРХИИ(&ШаблонДокумента)");
	КонецЕсли;
	
	Запрос.УстановитьПараметр("ВерсияРегламента", ВерсияРегламента);
	Запрос.УстановитьПараметр("УправлениеПериодом", УправлениеПериодом);
	Запрос.УстановитьПараметр("ШаблонДокумента", ПараметрыНастроек.ШаблонДокумента);
	Запрос.УстановитьПараметр("Организация", ПараметрыНастроек.Организация);
	Запрос.УстановитьПараметр("Проект", ПараметрыНастроек.Проект);
	Для Сч = 1 По ПараметрыСеанса.ЧислоДопАналитик Цикл
		Запрос.УстановитьПараметр("Аналитика" + Сч, ПараметрыНастроек["Аналитика" + Сч]);
	КонецЦикла;
...
Функция ЗначениеНастройкиОбработкиШаблонов(ПараметрыНастроек, ЭлементНастройки) Экспорт
	
	Возврат НастройкиОбработкиШаблонов(ПараметрыНастроек)[ЭлементНастройки];
	
КонецФункции

Функция НастройкиОбработкиШаблонов(ПараметрыНастроек) Экспорт
	
	СтруктураНастроек = Новый Структура;
	СтруктураНастроек.Вставить("БланкДляОтображения",		Справочники.БланкиОтчетов.ПустаяСсылка());
	СтруктураНастроек.Вставить("БланкСводнойТаблицы",		Справочники.БланкиОтчетов.ПустаяСсылка());
	СтруктураНастроек.Вставить("БланкДляИмпорта",			Справочники.БланкиОтчетов.ПустаяСсылка());
	СтруктураНастроек.Вставить("ПравилоОбработки",			Справочники.ПравилаОбработки.ПустаяСсылка());
	СтруктураНастроек.Вставить("ПравилоПроверки",			Справочники.ПравилаПроверки.ПустаяСсылка());
	СтруктураНастроек.Вставить("СпособФормированияОтчета",	Перечисления.СпособыФормированияОтчетов.ПустаяСсылка());
	СтруктураНастроек.Вставить("ВнешняяИнформационнаяБаза",	Справочники.ВнешниеИнформационныеБазы.ПустаяСсылка());
	СтруктураНастроек.Вставить("ПериодичностьСвертывания",	Перечисления.Периодичность.ПустаяСсылка());
	
	тЗначенияНастроек = ЗначенияНастроекОбработкиШаблонов(ПараметрыНастроек);
	Если тЗначенияНастроек.Количество() > 0 Тогда
		ЗаполнитьЗначенияСвойств(СтруктураНастроек, тЗначенияНастроек.Получить(0));
	КонецЕсли;
...
Процедура УстановитьНастройкиОбработкиШаблонов(ПараметрыНастроек, ЗначенияНастроек = Неопределено, Отказ = Ложь) Экспорт
	
	НаборЗаписей = РегистрыСведений.НастройкиОбработкиШаблоновДокументов.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.ШаблонДокумента.Установить(ПараметрыНастроек.ШаблонДокумента);
	НаборЗаписей.Отбор.ВерсияРегламента.Установить(ПараметрыНастроек.ВерсияРегламента);
	НаборЗаписей.Отбор.УправлениеПериодом.Установить(ПараметрыНастроек.УправлениеПериодом);
	НаборЗаписей.Отбор.Организация.Установить(ПараметрыНастроек.Организация);
	НаборЗаписей.Отбор.Проект.Установить(ПараметрыНастроек.Проект);
	Для Сч = 1 По ПараметрыСеанса.ЧислоДопАналитик Цикл
		НаборЗаписей.Отбор["Аналитика" + Сч].Установить(ПараметрыНастроек["Аналитика" + Сч]);
	КонецЦикла;
	...
	Попытка
		...
		Если ЗначенияНастроек = Неопределено Тогда
			...
			НаборЗаписей.Записать();
		Иначе
			ЗаписатьНастройкуОбработкиШаблонов(ПараметрыНастроек, ЗначенияНастроек, НаборЗаписей, "БланкДляОтображения");
			ЗаписатьНастройкуОбработкиШаблонов(ПараметрыНастроек, ЗначенияНастроек, НаборЗаписей, "БланкСводнойТаблицы");
			ЗаписатьНастройкуОбработкиШаблонов(ПараметрыНастроек, ЗначенияНастроек, НаборЗаписей, "ПравилоПроверки");
			ЗаписатьНастройкуОбработкиШаблонов(ПараметрыНастроек, ЗначенияНастроек, НаборЗаписей, "ПравилоОбработки");
			ЗаписатьНастройкуОбработкиШаблонов(ПараметрыНастроек, ЗначенияНастроек, НаборЗаписей, "СпособФормированияОтчета");
			ЗаписатьНастройкуОбработкиШаблонов(ПараметрыНастроек, ЗначенияНастроек, НаборЗаписей, "БланкДляИмпорта");
			ЗаписатьНастройкуОбработкиШаблонов(ПараметрыНастроек, ЗначенияНастроек, НаборЗаписей, "ПериодичностьСвертывания");
			ЗаписатьНастройкуОбработкиШаблонов(ПараметрыНастроек, ЗначенияНастроек, НаборЗаписей, "ВнешняяИнформационнаяБаза");
		КонецЕсли;
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ТекстСообщения = НСтр("ru = 'Возникла ошибка при установке настроек обработки шаблонов: %1'");
...
Процедура ЗаписатьНастройкуОбработкиШаблонов(ПараметрыНастроек, ЗначенияНастроек, НаборЗаписей, ИмяНастройки)
	
	Если ТипЗнч(ЗначенияНастроек) = Тип("Структура")
		И НЕ ЗначенияНастроек.Свойство(ИмяНастройки) Тогда
		Возврат;
	КонецЕсли;
	
	ЭлементНастройки = Перечисления.ЭлементыНастройкиОтчета[ИмяНастройки];
	
	НаборЗаписей.Отбор.ЭлементНастройки.Установить(ЭлементНастройки);
	НаборЗаписей.Очистить();

	НоваяЗапись = НаборЗаписей.Добавить();
	ЗаполнитьЗначенияСвойств(НоваяЗапись, ПараметрыНастроек);
	НоваяЗапись.ЭлементНастройки = ЭлементНастройки;
	НоваяЗапись.ЗначениеНастройки = ЗначенияНастроек[ИмяНастройки];
	
	НаборЗаписей.Записать();
	
КонецПроцедуры

Процедура ЗаписатьНастройкуПоУмолчаниюОбработкиШаблонов(ШаблонДокумента, ЭлементНастройки, ЗначениеНастройки, ДополнительныеОтборы = Неопределено, Отказ = Ложь) Экспорт
	
	ПараметрыНастроек = ПараметрыНастроекОбработкиШаблонов(ДополнительныеОтборы);
	ПараметрыНастроек.ШаблонДокумента = ШаблонДокумента;
	
	ЗначенияНастроек = Новый Структура;
	ЗначенияНастроек.Вставить(ЭлементНастройки, ЗначениеНастройки);
	
	УстановитьНастройкиОбработкиШаблонов(ПараметрыНастроек, ЗначенияНастроек, Отказ);
	
КонецПроцедуры

Функция ПредставлениеСпособаФормированияОтчета(СпособФормированияОтчета, ВнешняяИнформационнаяБаза = Неопределено, БланкДляИмпорта = Неопределено, Периодичность = Неопределено) Экспорт
	
	ПредставлениеОтчета = Строка(СпособФормированияОтчета);
	
	Если СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.ПоПравилуОбработки Тогда
		Если ЗначениеЗаполнено(ВнешняяИнформационнаяБаза) Тогда
			ПредставлениеОтчета = ПредставлениеОтчета + Символы.ПС +
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Внешняя ИБ: %1'"), ВнешняяИнформационнаяБаза);
		КонецЕсли;
	ИначеЕсли СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.Импорт Тогда
		Если ЗначениеЗаполнено(БланкДляИмпорта) Тогда
			ПредставлениеОтчета = ПредставлениеОтчета + Символы.ПС +
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Бланк для импорта: %1'"), БланкДляИмпорта);
		КонецЕсли;
		Если ЗначениеЗаполнено(ВнешняяИнформационнаяБаза) Тогда
			ПредставлениеОтчета = ПредставлениеОтчета + Символы.ПС +
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Внешняя ИБ: %1'"), ВнешняяИнформационнаяБаза);
	...
	ИначеЕсли СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.СвернувПоПериоду Тогда
		ПредставлениеОтчета = ПредставлениеОтчета + Символы.ПС +
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Периодичность: %1'"), Периодичность);
	ИначеЕсли НЕ ЗначениеЗаполнено(СпособФормированияОтчета)
		И ЗначениеЗаполнено(ВнешняяИнформационнаяБаза) Тогда
		// Для прочих шаблонов
		ПредставлениеОтчета = Строка(ВнешняяИнформационнаяБаза);
	КонецЕсли;
...
Процедура ЗаполнитьПараметрыОтображения(ЕдиницаИзмерения, УровеньТочности, ШаблонОтчета) Экспорт
	
	Если НЕ ЗначениеЗаполнено(ШаблонОтчета) Тогда
		Возврат;
	КонецЕсли;
	
	РеквизитыБланка = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ШаблонОтчета,
		"ЕдиницаИзмеренияПоУмолчанию, УровеньТочностиПоУмолчанию");
	
	ЕдиницаИзмерения = РеквизитыБланка.ЕдиницаИзмеренияПоУмолчанию;
	УровеньТочности = РеквизитыБланка.УровеньТочностиПоУмолчанию;
	
КонецПроцедуры

Процедура ЗаполнитьПараметрыОкругления(ЕдиницаИзмерения, Делитель, ФорматнаяСтрока = Неопределено) Экспорт
