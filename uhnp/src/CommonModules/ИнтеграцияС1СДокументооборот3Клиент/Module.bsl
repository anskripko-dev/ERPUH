...
Процедура НачатьСозданиеСвязанногоОбъектаДО(ОбъектИС, ОписаниеОповещения = Неопределено,
	...
	Иначе
		...
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Для объекта %1 нет подходящих правил интеграции.';
									|en = 'There are no suitable integration rules for the %1 object.'"), ОбъектИС);
		ПоказатьПредупреждение(,ТекстОшибки,, НСтр("ru = 'Не удалось создать объект 1С:Документооборота';
													|en = 'Cannot create a 1C:Document Management object'"));
	...
	Если ТипЗнч(ОбъектДОИлиСообщение) = Тип("Строка") Тогда // Сообщение об ошибке.
		...
		ПоказатьПредупреждениеИзДО(,ОбъектДОИлиСообщение,,
			НСтр("ru = 'Не удалось создать объект 1С:Документооборота';
				|en = 'Не удалось создать объект 1С:Документооборота'"));
...
Процедура ПоказатьПредупреждениеИзДО(ОписаниеОповещенияОЗавершении = Неопределено, ТекстПредупреждения, Таймаут = 0,
	...
	ЗаголовокСсылкиНаОбъектИС = СтрШаблон(
		НСтр("ru = 'Ссылка на объект в %1';
			|en = 'Reference to an object in %1'"),
	...
	Для НомерСтроки = 1 По ТекстовыйДокумент.КоличествоСтрок() Цикл
		...
		Если Не ПустаяСтрока(НавигационнаяСсылка) Тогда
			Если СтрНайти(ТекущаяСтрока, НСтр("ru = 'Ссылка на объект в ИС:';
												|en = 'Reference to the object in IS:'")) = 1 Тогда
			...
			ИначеЕсли Не УстановленаСсылкаНаОбъектДО Тогда
				...
				Если СтрНайти(ТекущаяСтрока, НСтр("ru = 'Ссылка на объект в ДО:';
													|en = 'Reference to the object in DI:'")) = 1 Тогда
...
Процедура ЗаменитьРольФайлов(СписокФайлов, ВыбраннаяРольID, ВладелецФайлов,
	...
	Для Каждого Файл Из ДанныеФайлов Цикл
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Изменена роль файла ""%1""';
										|en = 'The ""%1"" file role is changed'"), Файл.Наименование);
		...
		Если ЕстьИнтерфейсДокументовЭДО Тогда
			...
			Иначе
				ПоказатьПредупреждение(,НСтр("ru = 'Отразить в учете можно только формализованный документ ЭДО';
											|en = 'You can record only formalized EDI document in accounting'"));
			КонецЕсли;
...
Процедура ПриложенияЗадачиФайлыСохранитьКак(СтрокаДереваПриложений, Форма) Экспорт
	...
	Если Форма.Элементы.ДеревоПриложений.ВыделенныеСтроки.Количество() > 1 Тогда
		Для Каждого ВыбраннаяСтрока Из Форма.Элементы.ДеревоПриложений.ВыделенныеСтроки Цикл
			...
			Если ПриложениеМожноСохранитьНаДиск(ДанныеСтроки.ПриложениеТип) Тогда
				ВыделенныеФайлы.Добавить(ДанныеФайлаДляСохранения(ДанныеСтроки, Форма));
			КонецЕсли;
		КонецЦикла;
	Иначе
		Если ПриложениеМожноСохранитьНаДиск(СтрокаДереваПриложений.ПриложениеТип) Тогда
...
Процедура УстановитьДоступностьКомандПриложенийЗадачи(ТекущаяСтрока, КлючСтрокиДереваПриложений, ЗадачаID,
	...
	Форма.Элементы.ДеревоПриложенийКонтекстноеМенюСохранитьКак.Видимость =
		ПриложениеМожноСохранитьНаДиск(ТекущиеДанные.ПриложениеТип);
	...
	Форма.Элементы.ДеревоПриложенийКонтекстноеМенюСохранитьКак.Доступность =
		ПриложениеМожноСохранитьНаДиск(ТекущиеДанные.ПриложениеТип) И Не ТекущиеДанные.Зашифрован;
...
Функция ЭтапыДействияУникальны(Участники) Экспорт
	...
	Для Каждого ЭлементЭтап Из Участники.ПолучитьЭлементы() Цикл
		Если ЗначениеЗаполнено(ЭлементЭтап.НаименованиеЭтапа)
			...
			ПоказатьПредупреждение(,СтрШаблон(НСтр("ru = 'Повторяется этап ""%1""';
													|en = 'The ""%1"" stage is duplicated'"), ЭлементЭтап.НаименованиеЭтапа));
			Возврат Ложь;
		КонецЕсли;
...
// Возвращает текущие данные строки дерева связей.
//
// Параметры:
...
//
Функция ДеревоСвязейТекущиеДанные(ДеревоСвязей) Экспорт
...
Асинх Процедура НачатьУдалениеСвязи(Знач ДеревоСвязей, Знач ИсходныйОбъектДО) Экспорт
	...
	ТекущиеДанные = ДеревоСвязейТекущиеДанные(ДеревоСвязей);
...
Процедура ПоказатьСвязиОбъекта(ОбъектДО) Экспорт
	...
	ОткрытьФорму("Обработка.ИнтеграцияС1СДокументооборот3.Форма.СвязиОбъекта", ОбъектДО);
...
Функция СвязьМожноУдалить(ДеревоСвязейТекущиеДанные) Экспорт
	...
	Если ДеревоСвязейТекущиеДанные = Неопределено
			...
			Или ЗначениеЗаполнено(ДеревоСвязейТекущиеДанные.РасширениеФайла)
			Или ДеревоСвязейТекущиеДанные.ЭтоВеткаСвязи Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ДеревоСвязейТекущиеДанные.ПолучитьРодителя().ПолучитьРодителя() <> Неопределено Тогда
		// Это вложенная связь.
		Возврат Ложь;
	КонецЕсли;
...
Функция ПриложениеМожноСохранитьНаДиск(ПриложениеТип)
	
	Возврат (ПриложениеТип = "DMFile")
		Или (ПриложениеТип = "DMIncomingElectronicDocument")
		Или (ПриложениеТип = "DMOutgoingElectronicDocument");
