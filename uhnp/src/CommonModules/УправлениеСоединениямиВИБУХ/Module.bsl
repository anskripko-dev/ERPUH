...
Функция ПолучитьСоединениеADO(ВИБ,ПараметрыПодключения, КлючСоединения = Неопределено) Экспорт
	...
	ШаблонСообщения = НСтр("ru = 'При попытке ADO соединения с ИБ %1 произошла ошибка.'")
		+ Символы.ПС;
...
Функция ЗаполнитьДеревоЗначенийДляСправочникаВИБ(Параметры) Экспорт
	...
	ИначеЕсли ТипЗнч(База) = Тип("WSProxy")
		ИЛИ ТипЗнч(База) = Тип("Строка") Тогда
		
		ДеревоСправочника= РаботаСОбъектамиМетаданныхВнутреннийУХ.ЗаполнитьДеревоЗначенийДляСправочникаWS(База,РабочиеПараметры);
		
		Возврат ?(ДеревоСправочника=Неопределено,ДанныеСправочника,ДеревоСправочника);
		
	Иначе
		
		Возврат РаботаСОбъектамиМетаданныхВнешнийУХ.FillValueTreeForCatalog(База,РабочиеПараметры);
...
Функция ЗаполнитьДеревоЗначенийДляПеречисленияВИБ(Параметры) Экспорт
	...
	Если ТипЗнч(База) = Тип("WSProxy")
		...
		ДеревоСправочника= РаботаСОбъектамиМетаданныхВнутреннийУХ.ЗаполнитьДеревоЗначенийДляПеречисленияWS(База,РабочиеПараметры);
		
		Возврат ?(ДеревоСправочника=Неопределено,ДанныеСправочника,ДеревоСправочника);
		
	Иначе
		
		Возврат РаботаСОбъектамиМетаданныхВнешнийУХ.FillValueTreeForEnum(База,РабочиеПараметры);
...
Функция ПолучитьДанныеИзТаблицы(ВИБ, Таблица,КоличествоЗаписей) Экспорт
	...
	Попытка
		...
		Если СтрНайти(Врег(ADODB_Connection.Provider),"ORA")>0 
			ИЛИ СтрНайти(Врег(СтрокаСоединения),"POSTGRESQL")>0
			ИЛИ СтрНайти(Врег(СтрокаСоединения),"MSDASQL")>0 Тогда
