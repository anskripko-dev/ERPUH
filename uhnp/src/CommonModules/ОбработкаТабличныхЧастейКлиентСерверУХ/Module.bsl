...
Функция ПолучитьКэшируемыеЗначения_ВНА(СтрокаТЧ = Неопределено, ТЧ = Неопределено, СтруктураДействий = Неопределено, КэшированныеЗначения = Неопределено) Экспорт
	...
	Если ТребуетсяЗапросСервера Тогда
		...
		КэшированныеЗначенияСервера = МСФОВызовСервераУХ.ПолучитьКэшируемыеЗначения_ВНА(
			СтруктураПолученияКэша, СтруктураДействий);
...
Процедура ОбработатьСтрокуТЧ_РасходыФИ(СтрокаТЧ, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	КэшированныеЗначения = ПолучитьКэшируемыеЗначения_ВНА(
		СтрокаТЧ, Неопределено, СтруктураДействий, КэшированныеЗначения);
...
Процедура ОбработатьСтрокуТЧ_Инвестиции(ТекущаяСтрока, СтруктураДействий = Неопределено, КэшированныеЗначения = Неопределено) Экспорт
	...
	КэшированныеЗначения = ПолучитьКэшируемыеЗначения_Инвестиции(
		ТекущаяСтрока, Неопределено, СтруктураДействий, КэшированныеЗначения);
	...
	РассчитатьОбесценениеГудвилла(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
...
Процедура ОбработатьТЧ_Инвестиции(ТЧ, СтруктураДействий, КэшированныеЗначения = Неопределено) Экспорт
	...
	Если КэшированныеЗначения = Неопределено Тогда
		...
		КэшированныеЗначения = ПолучитьКэшируемыеЗначения_Инвестиции(
			Неопределено, ТЧ, СтруктураДействий);
...
Функция ПолучитьКэшируемыеЗначения_Инвестиции(СтрокаТЧ = Неопределено, ТЧ = Неопределено, СтруктураДействий = Неопределено, КэшированныеЗначения = Неопределено) Экспорт
	...
	Если ТребуетсяЗапросСервера Тогда
		...
		КэшированныеЗначенияСервера = МСФОВызовСервераУХ.ПолучитьКэшируемыеЗначения_Инвестиции(
			СтруктураДействий, Организации);
...
Функция ПолучитьКэшируемыеЗначения_РезервыДЗ(СтрокаТЧ = Неопределено, ТЧ = Неопределено, СтруктураДействий = Неопределено) Экспорт
	...
	ЕстьРассчитатьРезерв = СтруктураДействий.Свойство("РассчитатьРезервМСФО");
	
	ТипДоговоры = Тип("СправочникСсылка.ДоговорыКонтрагентов");
	ТипКонтрагенты = Тип("СправочникСсылка.Контрагенты");
	ТипБанковскийСчет = ВстраиваниеУХКлиентСервер.ТипБанковскиеСчетаОрганизаций();
	...
	Если СтруктураДействий.Свойство("ЗаполнитьДатуОкончанияОтсрочки") 
		...
		РеквизитыДоговораСтрокой = "УстановленСрокОплаты, ДатаОплаты, СрокОплаты";
		Если ИдентификацияПродуктаУХКлиентСервер.ЭтоЕХ() Тогда
			//В ЕРПУХ просрочка от даты окончания действия
			РеквизитыДоговораСтрокой = "ДатаОкончанияДействия, СрокОплаты";
		КонецЕсли;
	...
	Если СтруктураДействий.Свойство("РассчитатьРезервМСФО") 
		И (СтрокаТЧ <> Неопределено) 
		И ТипЗнч(ПолучитьСубконтоПоТипу(СтрокаТЧ, ТипДоговоры)) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
		Тогда
		
		РеквизитыДоговораСтрокой 
			= ?(РеквизитыДоговораСтрокой="", "", РеквизитыДоговораСтрокой + ",") 
	...
	ТребуютсяИсточникиКоэффициентов = СтруктураДействий.Свойство("ЗаполнитьКоэффициентыРезерва");
	
	Если СтрокаТЧ <> Неопределено И ЕстьРассчитатьРезерв Тогда
		
		Если ТребуютсяИсточникиКоэффициентов Тогда
			Контрагенты.Добавить(ПолучитьСубконтоПоТипу(СтрокаТЧ, ТипКонтрагенты));
			БанковскиеСчета.Добавить(ПолучитьСубконтоПоТипу(СтрокаТЧ, ТипБанковскийСчет));
		КонецЕсли;
		
		Договоры.Добавить(ПолучитьСубконтоПоТипу(СтрокаТЧ, ТипДоговоры));
		
	ИначеЕсли ТЧ <> Неопределено И ЕстьРассчитатьРезерв Тогда
	...
	ТекущийКонтекст = Новый Структура("Организация, ПериодОтчета, Сценарий", 
		Контекст.Организация, Контекст.ПериодОтчета, Контекст.Сценарий);
		
	Возврат МСФОВызовСервераУХ.ПолучитьКэшируемыеЗначения_РезервыДЗ(
		СтруктураДействий,
		ТекущийКонтекст,
		РеквизитыДоговораСтрокой,
		Договоры,
...
Функция ПолучитьДатуОкончанияОтсрочки(ТекущаяСтрока, КэшированныеЗначения = Неопределено) Экспорт
	...
	Договор = ПолучитьСубконтоПоТипу(ТекущаяСтрока, Тип("СправочникСсылка.ДоговорыКонтрагентов"));
	ТекущийДоговор = ПолучитьРеквизитыДоговора(
		Договор, КэшированныеЗначения);
		
	Если Не ЗначениеЗаполнено(ТекущийДоговор) Тогда
		Возврат ПустаяДата;
	КонецЕсли;
	
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоЕХ() Тогда //в ЕРПУХ нет ДатаОплаты
	
		Если ЗначениеЗаполнено(ТекущаяСтрока.ДатаДокументаРасчета)
			И ЗначениеЗаполнено(ТекущийДоговор.СрокОплаты) Тогда
			
			Возврат ОбщегоНазначенияУХ.ДобавитьДень(
				ТекущаяСтрока.ДатаДокументаРасчета, ТекущийДоговор.СрокОплаты);

		КонецЕсли;

		Возврат ТекущийДоговор.ДатаОкончанияДействия;
	
	КонецЕсли;
	
	Если (ТекущийДоговор.ДатаОплаты <> ПустаяДата) Тогда
		Возврат ТекущийДоговор.ДатаОплаты;
	ИначеЕсли ТекущаяСтрока.ДатаДокументаРасчета <> ПустаяДата Тогда 
		Возврат ОбщегоНазначенияУХ.ДобавитьДень(
			ТекущаяСтрока.ДатаДокументаРасчета, ТекущийДоговор.СрокОплаты);
	Иначе
		Возврат ПустаяДата;
...
Функция ПолучитьСубконтоПоТипу(СтрокаТЧ, ТипИзСубконто) Экспорт
	
	ЧислоСубконто = 3;
	Для й = 1 По ЧислоСубконто Цикл
		Если ТипЗнч(СтрокаТЧ["Субконто" + й]) = ТипИзСубконто Тогда
			Возврат СтрокаТЧ["Субконто" + й];
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьКоличествоДнейПросрочки(ТекущаяСтрока, ДатаОкончанияПериода) Экспорт
	...
	ОтклонениеОплаты = НачалоДня(ДатаОкончанияПериода) 
...
Функция ПолучитьКоэффициентРезерва(ТекущаяСтрока, КэшированныеЗначения = Неопределено) Экспорт
	...
	Для каждого СтрокаРасчет Из КэшированныеЗначения.РасчетныеЗначенияПараметров Цикл
		...
		Если (СтрокаРасчет.КлючСвязи = ТекущаяСтрока.КлючСвязи)
			И (СтрокаРасчет.ГруппаПараметраРасчета <> Неопределено) Тогда
...
Функция ВычислитьЗначениеПризнака(АлгоритмВычисляемогоПризнака, СтрокаДокументаРезерв, ДокументРезерв) Экспорт
	...
	Если Ложь Тогда 
		РасчетноеЗначение = ПолучитьДолюПросрочки(СтрокаДокументаРезерв);//используется в формуле предопределнного "КоэффициентСрокаПогашения"
	КонецЕсли;
	...
	Исключение
		...
		ШаблонОшибки = НСтр("ru = 'При выполнении алгоритма по договору <%1> произошла ошибка: '");
		ОбщегоНазначенияУХ.СообщитьОбОшибке(
			СтрШаблон(ШаблонОшибки, СтрокаДокументаРезерв.Договор) 
...
Функция ПолучитьДействияПозиций(ЗаполнитьСумму = Ложь, ЗаполнитьНДС = Ложь, ПолучитьЦену = Ложь, ПолучитьСтавкуНДС = Ложь) Экспорт

	Действия = Новый Структура;
	Если ПолучитьЦену Тогда
		Действия.Вставить("ПолучитьЦену");
	КонецЕсли;
	
	Если ПолучитьСтавкуНДС Тогда
		Действия.Вставить("ПолучитьСтавкуНДС");
	КонецЕсли;
	
	Если ЗаполнитьСумму Или ПолучитьЦену Тогда
		Действия.Вставить("ЗаполнитьСумму");
	КонецЕсли;
	
	Если ЗаполнитьНДС Или ЗаполнитьСумму Или ПолучитьЦену Или ПолучитьСтавкуНДС Тогда
		Действия.Вставить("ЗаполнитьНДС");
	КонецЕсли;
	
	Если Действия.Количество() Тогда
		Действия.Вставить("ЗаполнитьСуммаСНДС");
	КонецЕсли;
...
Функция ПолучитьДействияИнвестиций(ЗаполнитьЭффективнуюДолю = Истина, РассчитатьОтношениеКГруппе = Истина) Экспорт
	
	Действия = Новый Структура;
	
	Если ЗаполнитьЭффективнуюДолю Тогда
		Действия.Вставить("ЗаполнитьЭффективнуюДолю", Новый Структура("ЭффективнаяДоля"));
	КонецЕсли;
	
	Если РассчитатьОтношениеКГруппе Тогда
		Действия.Вставить("РассчитатьОтношениеКГруппе");
	КонецЕсли;
...
Процедура ОбработатьСтрокуТЧП(ТекущаяСтрока, Действия, КонтекстФормы) Экспорт

	//платеж
	Объект = КонтекстФормы.Объект;
	
	Если ТипЗнч(Действия) = Тип("Строка") Тогда
		Действия = Новый Структура(Действия);
	КонецЕсли;
	
	Если Действия.Свойство("ПолучитьПараметрыУчета") Тогда
		ТекущаяСтрока.ПараметрыУчетаВзаиморасчетов = МСФОВызовСервераУХ.ПолучитьПараметрыУчета(
				ТекущаяСтрока.СтатьяДвиженияДенежныхСредств, КонтекстФормы.КэшируемыеЗначения.ПланСчетовМСФО);
	КонецЕсли;
	
	Если Действия.Свойство("ЗаполнитьСубконтоПлатежа") Тогда
		ЗаполнитьСубконтоПлатежа(ТекущаяСтрока, Действия, КонтекстФормы);
	КонецЕсли;
	
	Если Действия.Свойство("ПолучитьСтавкуНДС") Тогда
		ТекущаяСтрока.СтавкаНДС = УчетМСФОПовтИспУХ.СтавкаПоУмолчанию(Объект.Организация);
	КонецЕсли;
		
	Если Действия.Свойство("ЗаполнитьНДС") Тогда
		СуммаВключаетНДС = Истина;//оплата всей суммы
		ТекущаяСтрока.СуммаНДС = СуммаНДС(ТекущаяСтрока.СуммаПлатежа, СуммаВключаетНДС, ТекущаяСтрока.СтавкаНДС);
	КонецЕсли;

КонецПроцедуры

Процедура ОбработатьСтрокуТЧА(ТекущаяСтрока, Действия, КонтекстФормы) Экспорт
	...
	Если Действия.Свойство("ПолучитьЦену") Тогда
		Цена = ПолучитьЦену(ТекущаяСтрока, Объект);
	...
	Если Действия.Свойство("ПолучитьПараметрыУчета") Тогда
		ТекущаяСтрока.ПараметрыУчета = МСФОВызовСервераУХ.ПолучитьПараметрыУчета(
				ТекущаяСтрока.Актив, КонтекстФормы.КэшируемыеЗначения.ПланСчетовМСФО);
	КонецЕсли;
	...
	Если Действия.Свойство("ЗаполнитьСумму") Тогда
		Если ТекущаяСтрока.Количество = 0 Тогда //услуга
	...
	Если Действия.Свойство("ЗаполнитьНДС") Тогда
		СуммаВключаетНДС = Объект.НДСВключенВСтоимость;
		ТекущаяСтрока.СуммаНДС = СуммаНДС(ТекущаяСтрока.Сумма, СуммаВключаетНДС, ТекущаяСтрока.СтавкаНДС);
	КонецЕсли;
	
	Если Действия.Свойство("ЗаполнитьСуммаСНДС") Тогда
		ТекущаяСтрока.СуммаСНДС = ТекущаяСтрока.Сумма
			+ ?(Объект.НДСВключенВСтоимость, 0, ТекущаяСтрока.СуммаНДС);
	КонецЕсли;
	
	ЗаполнитьИнвестиции(ТекущаяСтрока, Действия, КонтекстФормы);
		
КонецПроцедуры

Функция СуммаНДС(Сумма, СуммаВключаетНДС, СтавкаНДС) Экспорт
	
	ЗначениеСтавки = СтавкаНДСЧислом(СтавкаНДС);
	
	Возврат МСФОКлиентСерверУХ.РассчитатьСуммуНДС(
		Сумма, СуммаВключаетНДС, ЗначениеСтавки);
...
Функция СтавкаНДСЧислом(СтавкаНДС) Экспорт
	...
	Если ТипЗнч(СтавкаНДС) = Тип("СправочникСсылка.СтавкиНалоговМСФО") Тогда
		
		ЗначениеСтавки = УчетМСФОПовтИспУХ.ЗначениеСтавкиНДС(СтавкаНДС);
		
	ИначеЕсли ИдентификацияПродуктаУХКлиентСервер.ЭтоЕХ() Тогда
		
		#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
			Модуль = ОбщегоНазначения.ОбщийМодуль("УчетНДСЛокализация");
		#Иначе
			Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("УчетНДСЛокализация");
		#КонецЕсли
		ЗначениеСтавки = Модуль.СтавкаНДСПоПеречислению(СтавкаНДС);
		
	ИначеЕсли УчетМСФОПовтИспУХ.ЕстьРСБУ() Тогда //УХ, БМ, НМ
		
		#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
			Модуль = ОбщегоНазначения.ОбщийМодуль("УчетНДСВызовСервераПовтИсп");
		#Иначе
			Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("УчетНДСВызовСервераПовтИсп");
		#КонецЕсли
		ЗначениеСтавки = Модуль.ПолучитьСтавкуНДС(СтавкаНДС, Ложь);
...
Процедура ОбработатьСтрокуТЧ_РезервыМПЗ(ТекущаяСтрока, СтруктураДействий = Неопределено, КэшированныеЗначения = Неопределено) Экспорт
	...
	Если КэшированныеЗначения = Неопределено Тогда
		...
		КэшированныеЗначения = ПолучитьКэшируемыеЗначения_РезервыМПЗ(
			ТекущаяСтрока, Неопределено, СтруктураДействий, КэшированныеЗначения);
			
			КэшированныеЗначения.Вставить("ПланСчетовБДМСФО",
			МСФОВызовСервераУХ.ПланСчетовУП(Неопределено));
...
Функция ПолучитьКэшируемыеЗначения_РезервыМПЗ(СтрокаТЧ = Неопределено, ТЧ = Неопределено, СтруктураДействий = Неопределено, КэшированныеЗначения = Неопределено) Экспорт
	...
	Если СтруктураДействий.Свойство("ЗаполнитьСчетУчетаМСФО") 
		И НЕ СтруктураПолученияЗначений.Свойство("СчетаЗапасовНСБУ") Тогда
		...
		СтруктураПолученияЗначений.Вставить("СчетаЗапасовНСБУ", СчетаУчетаНСБУ);
	...
	Если СтруктураПолученияЗначений.Количество() <> 0 Тогда
		...
		КэшированныеЗначенияСервера = МСФОВызовСервераУХ.ПолучитьКэшируемыеЗначения_РезервыМПЗ(
			СтруктураПолученияЗначений, СтруктураДействий);
...
Функция ПолучитьКэшДеталиПоКлючуСтроки(КоллекцияДетали, КлючСтроки = Неопределено, ИмяКолонкиКлючСтроки = "КлючСтроки", КопироватьСтроки = Ложь) Экспорт
	...
	Иначе
		...
		Если КопироватьСтроки Тогда
			КэшПоКлючуСтроки.Вставить(КлючСтроки, КоллекцияДетали.Скопировать(
				Новый Структура(ИмяКолонкиКлючСтроки, КлючСтроки)));
		Иначе
			КэшПоКлючуСтроки.Вставить(КлючСтроки, КоллекцияДетали.НайтиСтроки(
				Новый Структура(ИмяКолонкиКлючСтроки, КлючСтроки)));
		КонецЕсли;
	...
	Иначе
		...
		#Если НаКлиенте Тогда
			...
			Для Каждого СтрокаОтбора Из КоллекцияДетали.НайтиСтроки(ОтборСтрок) Цикл
				Для каждого КолонкаИтога Из КолонкиИсточники Цикл
					ЗначенияИтогов.Вставить(КолонкаИтога.Ключ,
						СтрокаОтбора[КолонкаИтога.Ключ] + ЗначенияИтогов[КолонкаИтога.Ключ]);
				КонецЦикла;
			КонецЦикла;
...
Процедура УдалитьСтрокиДеталиПоКлючуСтроки(КоллекцияДетали, ИмяКолонкиКлючСтроки = "КлючСтроки", УдаляемыйКлючСтроки) Экспорт
	...
	СтрокиДляУдаления = КоллекцияДетали.НайтиСтроки(
		Новый Структура(ИмяКолонкиКлючСтроки, УдаляемыйКлючСтроки));
...
Процедура ЗаполнитьЗависимыеРеквизитыИсточникГруппаВНА(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Для каждого Реквизит Из ЗависимостиРеквизитов Цикл	
		...
		ТекущаяСтрока[Реквизит.Ключ] = ОбщегоНазначенияУХ.ПолучитьЗначениеРеквизита(
			ТекущаяСтрока.ИсточникГруппаВНАМСФО, Реквизит.Значение);
...
Процедура ЗаполнитьЗависимыеРеквизитыГруппыВНА(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
		...
		Для каждого КлючГруппаВНА Из ПараметрыДействия Цикл
			Для каждого КлючЗависимыйРеквизит Из КлючГруппаВНА.Значение Цикл
				ТекущаяСтрока[КлючЗависимыйРеквизит.Ключ] = ОбщегоНазначенияУХ.ПолучитьЗначениеРеквизита(
					ТекущаяСтрока[КлючГруппаВНА.Ключ], КлючЗависимыйРеквизит.Значение);
			КонецЦикла;
		КонецЦикла;
	...
	Иначе
		...
		Для каждого Реквизит Из СтрокиОбработки Цикл
			...
			ТекущаяСтрока[Реквизит.Приемник] = ОбщегоНазначенияУХ.ПолучитьЗначениеРеквизита(
				ТекущаяСтрока.ГруппаВНА, Реквизит.РеквизитГруппы);
...
Процедура ЗаполнитьЗависимыеРеквизитыПараметровАмортизации(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Для каждого Реквизит Из СтрокиОбработки Цикл	
		...
		ТекущаяСтрока[Реквизит.Приемник] = ОбщегоНазначенияУХ.ПолучитьЗначениеРеквизита(
			ТекущаяСтрока.ПараметрыАмортизацииВНАМСФО, Реквизит.РеквизитГруппы);
...
Процедура ЗаполнитьСчетаМСФОПоНеклассифицированным(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Для каждого СчетМСФО Из СоответствиеКолонок Цикл
		...
		ЗаполнитьСубконтоСчетаПриемникаПоИсточнику(
			ТекущаяСтрока, СчетМСФО.Значение, СчетМСФО.Ключ, ИменаСубконто);
...
Процедура ЗаполнитьКолонкиРасхождений(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Для каждого Расхождение Из КолонкиРасхождений Цикл
		ТекущаяСтрока[Расхождение.Имя + "Расхождение"] 
			= ТекущаяСтрока[Расхождение.Имя + "МСФО"] 
			- ТекущаяСтрока[Расхождение.Имя + "НСБУ"];
	КонецЦикла;
...
Процедура ЗаполнитьДоступностьПоИменамСубконто(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Для каждого СубконтоСчета Из ИменаСубконто Цикл
		...
		МСФОКлиентСерверУХ.УстановитьДоступностьСубконто(
			СчетИсточник, ТекущаяСтрока, ИменаСубконто);
...
Процедура ЗаполнитьПоОстаткам(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Для каждого ТекущиеПараметры Из ПараметрыДействия Цикл
		ТекущаяСтрока[ТекущиеПараметры.Значение.Сумма] 
			= КэшированныеЗначения.ЗаполнитьПоОстаткам.Получить(ТекущаяСтрока[ТекущиеПараметры.Ключ]);
	КонецЦикла;
...
Процедура ЗаполнитьСписаниеПереоценкиВД(СтрокаТЧ, СтруктураДействий, КэшируемыеЗначения)
	...
	ТекущаяСтоимость = СтрокаТЧ.БалансоваяСтоимостьВД + СтрокаТЧ.НакопленнаяПереоценкаВД + СтрокаТЧ.СуммаПереоценкиВД;
	Если ТекущаяСтоимость < (СтрокаТЧ.БалансоваяСтоимостьВД * КэшируемыеЗначения.ПроцентСниженияСтоимостиФИ / 100) Тогда		
...
Процедура РассчитатьОбесценениеГудвилла(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
...
Процедура РассчитатьЧА(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Если СтруктураДействий.Свойство("РассчитатьФинансовыйРезультат") Тогда
		...
		Если СтрокаПоКлючу = Неопределено Тогда
			ДоляМеньшинства = 0;
		Иначе
			ДоляМеньшинства = 100 - СтрокаПоКлючу.ЭффективнаяДоляНаКонец;
		КонецЕсли;
		
		ТекущаяСтрока.ФинансовыйРезультат = ТекущаяСтрока.ИзменениеЧистыхАктивов * ДоляМеньшинства / 100;
...
Процедура ЗаполнитьДатуОкончанияОтсрочки(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.ДатаПогашенияПоДокументуНСБУ) Тогда
		Возврат;//заполнена из РСБУ или по партиям ранее
	КонецЕсли;
...
Процедура ЗаполнитьКоличествоДнейОтсрочки(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	ТипДоговоры = Тип("СправочникСсылка.ДоговорыКонтрагентов");
	Договор = ПолучитьСубконтоПоТипу(ТекущаяСтрока, ТипДоговоры);
	РеквизитыДоговора = ПолучитьРеквизитыДоговора(Договор, КэшированныеЗначения);
	
	Если РеквизитыДоговора = Неопределено Тогда
		ТекущаяСтрока.ОтсрочкаДниМСФО = 0;
		Возврат;
	КонецЕсли;
	
	ДатаГашения = ТекущаяСтрока.ДатаПогашенияПоДокументуНСБУ;
	ДатаРасчета = ТекущаяСтрока.ДатаДокументаРасчета;
	
	Если ЗначениеЗаполнено(ДатаГашения) И ЗначениеЗаполнено(ДатаРасчета) Тогда
		
		Отклонение = НачалоДня(ДатаГашения) - НачалоДня(ДатаРасчета);
		ТекущаяСтрока.ОтсрочкаДниМСФО = ?(Отклонение > 0, Отклонение / (24*60*60), 0);
		
	Иначе
		
		Если Не ИдентификацияПродуктаУХКлиентСервер.ЭтоЕХ() Тогда //в ЕРПУХ не используется СрокОплаты
...
Процедура ЗаполнитьКоличествоДнейПросрочки(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	КонецПериода = СтруктураДействий.ЗаполнитьКоличествоДнейПросрочки.КонецПериода;
	Если Не ЗначениеЗаполнено(КонецПериода) 
		Или Не ЗначениеЗаполнено(ТекущаяСтрока.ДатаПогашенияПоДокументуНСБУ) Тогда
		Возврат;
	КонецЕсли;
	
	ТекущаяСтрока.ПросрочкаДниМСФО = ПолучитьКоличествоДнейПросрочки(
...
Процедура ЗаполнитьКоэффициентыРезерва(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Для каждого ПараметрРасчета Из КэшированныеЗначения.ТаблицыДляКоэффициентов.ПараметрыРасчета Цикл
		...
		Если ПараметрРасчета.ТипПараметраРасчетаРезервов = ТипыПараметров.ПоДоговору Тогда
			...
			СтрокаПараметра = ПолучитьСтрокуПараметраНеВычисляемый(
				ТаблицаНеВычисляемые, Договор, ПараметрРасчета.ПараметрРасчета);
		...
		ИначеЕсли ПараметрРасчета.ТипПараметраРасчетаРезервов = ТипыПараметров.ПоКонтрагенту Тогда
			...
			Контрагент		= ПолучитьСубконтоПоТипу(ТекущаяСтрока, Тип("СправочникСсылка.Контрагенты"));
			Если Не ЗначениеЗаполнено(Контрагент) Тогда
				Продолжить;
			КонецЕсли;
			
			СтрокаПараметра = ПолучитьСтрокуПараметраНеВычисляемый(
				ТаблицаНеВычисляемые, Контрагент, ПараметрРасчета.ПараметрРасчета);
				
			РасчетноеЗначение = СтрокаПараметра.РасчетноеЗначение;
			Коэффициент = СтрокаПараметра.РасчетноеЗначение;
			
		ИначеЕсли ПараметрРасчета.ТипПараметраРасчетаРезервов = ТипыПараметров.ПоБанку Тогда
			
			БанковскийСчет	= ПолучитьСубконтоПоТипу(ТекущаяСтрока,
				ВстраиваниеУХКлиентСервер.ТипБанковскиеСчетаОрганизаций());
			Если Не ЗначениеЗаполнено(БанковскийСчет) Тогда
				Продолжить;
			КонецЕсли;
			Банк = ОбщегоНазначенияУХ.ЗначениеРеквизитаОбъекта(БанковскийСчет, "Банк");
			
			СтрокаПараметра = ПолучитьСтрокуПараметраНеВычисляемый(
				ТаблицаНеВычисляемые, Банк, ПараметрРасчета.ПараметрРасчета);
...
Процедура РассчитатьРезервМСФО(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Контекст = Новый Структура("РучнойРасчетРезерваПоЗадолженности");
	Договор = ПолучитьРеквизитыДоговора(ТекущаяСтрока.Субконто2, КэшированныеЗначения);
		
	Если ЗначениеЗаполнено(Договор) Тогда
		ЗаполнитьЗначенияСвойств(Контекст, Договор);
	КонецЕсли;
	
	Если Контекст.РучнойРасчетРезерваПоЗадолженности <> Истина Тогда
		ТекущаяСтрока.НачислениеРезерваВВалютеМСФО 
			= ТекущаяСтрока.ОстатокЗадолженностиВВалютеМСФО * ТекущаяСтрока.КоэффициентРезерва;
	КонецЕсли;
...
Процедура ЗаполнитьСчетаЗадолженностиПоСчетуНСБУ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Если СчетаЗадолженностиНСБУ = Неопределено Тогда
		...
		ТекущаяСтрока.СчетЗадолженностиМСФО = МСФОВНАВызовСервераУХ.ПолучитьСчетМСФОТрансформации(
			ТекущаяСтрока.СчетЗадолженностиНСБУ, Контекст.Организация, Контекст.Сценарий, Контекст.ПериодОтчета);
...
Функция ПолучитьСтрокуПараметраВычисляемый(ТаблицаВычисляемые, ПараметрРасчета, РасчетноеЗначение)
	...
	Для каждого Узел Из ТаблицаВычисляемые.НайтиСтроки(Отбор) Цикл
		Если (Узел.ЗначениеКон <> NULL) И (Узел.ЗначениеНач <= РасчетноеЗначение) И (Узел.ЗначениеКон >= РасчетноеЗначение) Тогда
			Возврат Узел;
        КонецЕсли;
	КонецЦикла;
...
Процедура РассчитатьВычисляемыеПоля(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	ТекущаяСтрока.СтоимостьОстаткаПоЧистойЦенеПродажи
		= ТекущаяСтрока.КоличествоОстаткаЗапасовНаКонецПериода * ТекущаяСтрока.ЧистаяЦенаПродажиЗаЕдиницу;
	ТекущаяСтрока.Оборачиваемость
		= ПолучитьОборачиваемость(ТекущаяСтрока, КэшированныеЗначения);
...
Функция ПолучитьОборачиваемость(ТекущаяСтрока, КэшированныеЗначения)
	...
	РеквизитыНоменклатуры = КэшированныеЗначения.РеквизитыНоменклатуры.Получить(ТекущаяСтрока.Номенклатура);
	НормативнаяОборачиваемость = ?(РеквизитыНоменклатуры = Неопределено, 0, 
		?(ЗначениеЗаполнено(РеквизитыНоменклатуры.НормативнаяОборачиваемость), 
			РеквизитыНоменклатуры.НормативнаяОборачиваемость, 0));
	
	ТекущаяОборачиваемость = ?(КэшированныеЗначения.Свойство("ЧислоДнейПериода"),
		КэшированныеЗначения.ЧислоДнейПериода, НормативнаяОборачиваемость);//
	
	Если ТекущаяСтрока.СписаниеЗапасовЗаПериодМСФО = 0 Тогда
		Возврат НормативнаяОборачиваемость+1;
	ИначеЕсли ТекущаяСтрока.СтоимостьОстаткаЗапасовНаНачалоПериода 
			+ ТекущаяСтрока.СтоимостьОстаткаЗапасовНаКонецПериодаМСФО = 0 Тогда
	 	Возврат 1;
	Иначе 
		Возврат ТекущаяОборачиваемость 
		/ (2 * ТекущаяСтрока.СписаниеЗапасовЗаПериодМСФО 
			/ (ТекущаяСтрока.СтоимостьОстаткаЗапасовНаНачалоПериода 
				+ ТекущаяСтрока.СтоимостьОстаткаЗапасовНаКонецПериодаМСФО));
	КонецЕсли;
...
Функция ПолучитьРезервПоОбесценению(ТекущаяСтрока)
	...
	Иначе
		...
		Обесценение = (ТекущаяСтрока.СтоимостьОстаткаЗапасовНаКонецПериодаМСФО 
						- ТекущаяСтрока.СтоимостьОстаткаПоЧистойЦенеПродажи);
...
Процедура ПересчитатьВалютнуюСумму(ТекущаяСтрока, КэшированныеЗначения, РеквизитНач, РеквизитКон, ИзФункциональнойВалюты = Истина)
	...
	ВалютаДоговора = ПредопределенноеЗначение("Справочник.Валюты.ПустаяСсылка");
	
	Договор = ПолучитьСубконтоПоТипу(ТекущаяСтрока, Тип("СправочникСсылка.ДоговорыКонтрагентов"));
	Если ЗначениеЗаполнено(Договор) Тогда
		ВалютаДоговора = ПолучитьРеквизитыДоговора(Договор, КэшированныеЗначения).ВалютаВзаиморасчетов;
	КонецЕсли;
	...
	Иначе
		...
		ТекущаяСтрока[РеквизитКон] = РаботаСКурсамиВалютКлиентСервер
			.ПересчитатьПоКурсу(ТекущаяСтрока[РеквизитНач], КурсНач, КурсКон);
...
Процедура ЗаполнитьСубконтоИзТрансляции(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Если НЕ КэшированныеЗначения.Свойство("СоответствияСубконто", Кэш) Тогда
		Кэш = Новый Соответствие;
		КэшированныеЗначения.Вставить("СоответствияСубконто", Кэш);
	Иначе
		Кэш = КэшированныеЗначения.СоответствияСубконто;
	КонецЕсли;
...
Процедура ЗаполнитьСубконтоПлатежа(ТекущаяСтрока, Действия, КонтекстФормы)
	
	Объект = КонтекстФормы.Объект;
	
	СчетИсточник = ОбщегоНазначенияУХ.ЗначениеРеквизитаОбъекта(
		ТекущаяСтрока.ПараметрыУчетаВзаиморасчетов, "СчетУчета.Счет");
		
	Если Не ЗначениеЗаполнено(СчетИсточник) Тогда
		СчетИсточник = ПредопределенноеЗначение("Справочник.СчетаБД.ПустаяСсылка");
	КонецЕсли;
		
	МСФОКлиентСерверУХ.ОбновитьСубконтоСчета(
		КонтекстФормы, 
		"ПараметрыУчетаВзаиморасчетов", 
		ТекущаяСтрока, 
		"РасшифровкаПлатежа", 
		Ложь, 
		СчетИсточник);
			
	ЧислоСубконто = УчетМСФОПовтИспУХ.ПолучитьМаксКоличествоСубконто();//3
	Для НомерСубконто = 1 По ЧислоСубконто Цикл
		
		ИмяСубконто = "Субконто" + НомерСубконто;
		Если Не ТекущаяСтрока[ИмяСубконто + "Доступность"] Тогда
			Продолжить;
		ИначеЕсли ЗначениеЗаполнено(ТекущаяСтрока[ИмяСубконто]) Тогда
			Продолжить;//оставляем заполненным
		КонецЕсли;
		
		Если ТекущаяСтрока[ИмяСубконто] = ПредопределенноеЗначение("Справочник.Контрагенты.ПустаяСсылка") Тогда
			ТекущаяСтрока[ИмяСубконто] = Объект.Контрагент;
		ИначеЕсли ТекущаяСтрока[ИмяСубконто] = ПредопределенноеЗначение("Справочник.ДоговорыКонтрагентов.ПустаяСсылка") Тогда

			МСФОВызовСервераУХ.УстановитьДоговорКонтрагента(
				ТекущаяСтрока[ИмяСубконто],
				Объект.Контрагент,
				Объект.Организация);
...
Процедура ЗаполнитьИнвестиции(ТекущаяСтрока, Действия, КонтекстФормы)
	
	Объект = КонтекстФормы.Объект;
	
	Если Не Действия.Свойство("ЗаполнитьЭффективнуюДолю") И Не Действия.Свойство("РассчитатьОтношениеКГруппе") Тогда
		Возврат;
	КонецЕсли;
	
	Если Не КонтекстФормы.КэшируемыеЗначения.КэшОперации.ЕстьДоли Тогда
		Возврат;
	КонецЕсли;
	
	КэшИнвестиций = КонтекстФормы.КэшируемыеЗначения;
	КэшИнвестиций.Вставить("Сценарий", 			Объект.Сценарий);
	КэшИнвестиций.Вставить("ВыбытиеИнвестиций", ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.РеализацияМСФО"));
	
	ОИ = ?(ТипЗнч(ТекущаяСтрока.Актив)=Тип("СправочникСсылка.Контрагенты"), 
		ТекущаяСтрока.Актив.ОрганизационнаяЕдиница, ТекущаяСтрока.Актив);
	
	КонтекстСтроки = Новый Структура(
		"ОбъектИнвестирования, ПрямаяДоля, ОтношениеКГруппе, МСФОУчет, Дата, Ссылка, Организация",
		ОИ, ТекущаяСтрока.КоличествоДоп, ТекущаяСтрока.ОтношениеКГруппе, Истина);

	ЗаполнитьЗначенияСвойств(КонтекстСтроки, Объект);
	...
	#Если Клиент Тогда
	МодульИнвестицииКлиентСервер = ОбщегоНазначенияКлиент.ОбщийМодуль("ИнвестицииКлиентСервер");
	#Иначе
	МодульИнвестицииКлиентСервер = ОбщегоНазначения.ОбщийМодуль("ИнвестицииКлиентСервер");
	#КонецЕсли
	МодульИнвестицииКлиентСервер.ЗаполнитьДокумент(КонтекстСтроки, Действия, КэшИнвестиций);

	Если Действия.Свойство("ЗаполнитьЭффективнуюДолю") Тогда
		ТекущаяСтрока.ЭффективнаяДоля = КэшИнвестиций.ЭффективнаяДоля;	
	КонецЕсли;
	
	Если Действия.Свойство("ЗаполнитьЭффективнуюДолю") Тогда
		ТекущаяСтрока.ОтношениеКГруппе = КонтекстСтроки.ОтношениеКГруппе;
	КонецЕсли;

КонецПроцедуры

Функция ПолучитьЦену(ТекущаяСтрока, Объект)
	
	Если Объект.ТипЦен.Пустая() Или ТипЗнч(ТекущаяСтрока.Актив) <> Тип("СправочникСсылка.Номенклатура") Тогда
		Возврат 0;
	КонецЕсли;
	#Если Клиент Тогда
	МодульЦенообразование = ОбщегоНазначенияКлиент.ОбщийМодуль("Ценообразование");
	#Иначе
	МодульЦенообразование = ОбщегоНазначения.ОбщийМодуль("Ценообразование");
	#КонецЕсли
	Если МодульЦенообразование = Неопределено Тогда
		Возврат 0;	
	КонецЕсли;
	
	Возврат МодульЦенообразование.ПолучитьЦенуНоменклатуры(
									ТекущаяСтрока.Актив, 
									Объект.ТипЦен, 
									Объект.Дата, 
