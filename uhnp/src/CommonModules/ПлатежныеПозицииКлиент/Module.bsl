...
Функция ПолучитьПараметрыКурса(Форма)
	...
	ИменаРеквизитовКурса = ДвиженияБюджетированиеКлиентСерверУХ.СтруктураПараметровРасчетаКурса();
	...
	ПараметрыКурса = ПолучитьДанныеПоСтруктуре(Форма.Объект, ИменаРеквизитовКурса); 
	ЗаполнитьДатуОтгрузкиВПараметрахКурса(Форма, ПараметрыКурса);
...
Функция ПолучитьПараметрыДокумента(Форма)
	...
	РеквизитыДокумента.Вставить("ДоговорКонтрагента", 	Форма.РеквизитыДокумента.ДоговорКонтрагента.ИмяРеквизита);
	РеквизитыДокумента.Вставить("ДоговорКредитаДепозита", 	Форма.РеквизитыДокумента.ДоговорКредитаДепозита.ИмяРеквизита);
	РеквизитыДокумента.Вставить("ПлатежВУсловныхЕдиницах");
	РеквизитыДокумента.Вставить("ФиксированныйСчетОрганизации"); 
	
	
	РеквизитыДокумента.Вставить("СпособОпределенияКурсаПлатежа");
	РеквизитыДокумента.Вставить("КурсПлатежа");
	РеквизитыДокумента.Вставить("Контрагент"); 
	РеквизитыДокумента.Вставить("БанковскийСчетКонтрагента"); 
	РеквизитыДокумента.Вставить("КратностьПлатежа");
	РеквизитыДокумента.Вставить("ДатаФиксацииКурсаПлатежа");
	РеквизитыДокумента.Вставить("СдвигДатыФиксацииКурсаПлатежа");
	РеквизитыДокумента.Вставить("КурсПлатежаНеБолее");
	РеквизитыДокумента.Вставить("КурсПлатежаНеМенее");
	РеквизитыДокумента.Вставить("КурсПлатежаНеМенееВВалютеОплаты");
	РеквизитыДокумента.Вставить("КурсПлатежаНеБолееВВалютеОплаты");
...
Функция ПолучитьДанныеПоСтруктуре(Объект, Реквизиты)
	...
	Для Каждого КлючЗначение Из Реквизиты Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект,ИмяРеквизитаДокумента) Тогда
...
Процедура ЗаполнитьДатуОтгрузкиВПараметрахКурса(Форма, ПараметрыКурса)
    ПараметрыКурсаФормы = Форма.ПараметрыКурса;
	Если ТипЗнч(ПараметрыКурсаФормы) = Тип("ФиксированнаяСтруктура") И ПараметрыКурсаФормы.Свойство("ДатаОтгрузки") Тогда 
		ПараметрыКурса.Вставить("ДатаОтгрузки", ПараметрыКурсаФормы.ДатаОтгрузки);    
	КонецЕсли;
КонецПроцедуры
...
Процедура СоздатьДокументОплатыНаОснованииПозиции(МассивСсылок, ИмяДокумента)
	...
	Если ПлатежныеПозицииВызовСервера.СформироватьДанныеЗаполненияОплаты(
		...
		ИначеЕсли РаспределениеОплаты.Количество() > 1 Тогда
			...
			Если ПлатежныеПозицииВызовСервера.ИспользоватьФормуПомощникаСписанияНаЛицевыеСчета(МассивСсылок) Тогда
							ОткрытьФорму("Обработка.ПлатежныеПозиции.Форма.ФормаПомощникаСписанияНаЛицевыеСчета", 
