...
Процедура ВывестиСообщениеПревышенияПоКоличеству(ВозвращаемыеПараметры, ТекстОснования = "") Экспорт
	...
	Если ТипЗнч(ВозвращаемыеПараметры) = Тип("Структура")
		...
		ШаблонТекста = НСтр("ru = 'Превышение количества не предусмотрено для документа %1';
							|en = 'Quantity excess is not allowed for the %1 document'");
...
Функция ПроверитьПолучитьВыделенныеВСпискеСсылки(Список) Экспорт
	...
	Если МассивСсылок.Количество() = 0 Тогда   
		...
		// Код ERP - типовая ошибка ERP. Предупреждение, если список заявок пустой.
		//ПоказатьПредупреждение(, НСтр("ru = 'Команда не может быть выполнена для указанного объекта!';
		//								|en = 'Cannot execute this command for the specified object.'"));   
...
Функция ОбработатьЗаписьОбъектаВФорме(Форма, ПараметрыЗаписи, Отказ = Ложь) Экспорт
	...
	Если ПараметрыЗаписи.Свойство("НеОтправлятьНаСогласованиеПриПроведении") Тогда
		ПараметрыЗаписи.Удалить("НеОтправлятьНаСогласованиеПриПроведении");
	КонецЕсли;
	Результат = Форма.Записать(ПараметрыЗаписи);
...
Функция МинимальноДопустимаяВерсия() Экспорт
	...
	//++ НЕ УТ
	МинимальноДопустимаяВерсия = "2.5.17.74"; // для КА2
	
	//-- НЕ УТ
	
	//++ НЕ УТКА
	МинимальноДопустимаяВерсия = "2.5.17.74"; // для УП2
