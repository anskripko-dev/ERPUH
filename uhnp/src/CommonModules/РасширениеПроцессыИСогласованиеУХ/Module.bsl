...
Процедура НарисоватьПанельСогласования(ЭлементыВход, РодительскаяГруппаВход) Экспорт
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом() Тогда
		...
		Параметры.Добавить(ЭлементыВход);
		Параметры.Добавить(РодительскаяГруппаВход);
		ОбщегоНазначения.ВыполнитьМетодКонфигурации("МодульСогласованияДокументовУХ.НарисоватьПанельСогласования", Параметры);
	Иначе
		// Не отображаем панель соглсования.
	КонецЕсли;
КонецПроцедуры
...
Функция ВозможноУстановитьСостояние(ДокументОбъект, НовоеСостояние, ШапкаОшибки = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда
		...
		Возврат МодульУправлениеПроцессамиСогласованияУХ.ВозможноУстановитьСостояние(ДокументОбъект, НовоеСостояние, ШапкаОшибки);
...
Процедура ПриСозданииНаСервереСписок(Форма) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда				// не БМ, НМ
		...
		ОбщийМодульСогласованияДокументовУХ.ПриСозданииНаСервереСписок(Форма);
	КонецЕсли;
...
Процедура ПриСозданииНаСервереОбъект(Форма) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда				// не БМ, НМ
		ОбщийМодульСогласованияДокументовУХ = ОбщегоНазначения.ОбщийМодуль("МодульСогласованияДокументовУХ");
		ОбщийМодульСогласованияДокументовУХ.ПриСозданииНаСервереОбъект(Форма);
	КонецЕсли;
