...
//
Функция ПолучитьРеквизитыВНА(ГраницаИсточник, ВНА = Неопределено, Организация = Неопределено, ВидыУчета = Неопределено, Сценарий = Неопределено, ПоляВыбора = Неопределено, АлгоритмРСБУ = Неопределено) Экспорт
	...
	ЭтоГраницаИсточник = ТипЗнч(ГраницаИсточник) = Тип("Структура");
	Граница = ?(ЭтоГраницаИсточник, ГраницаИсточник.ДатаАктуальности, ГраницаИсточник);
	...
	ЕстьТУВНА = Истина;
	Запрос = Новый Запрос(УчетМСФОПовтИспУХ.ПолучитьКэшированныйТекстЗапросаВНА(ПоляВыбораСтрокой,, ЕстьТУВНА));
	...
	Если ЭтоГраницаИсточник И ГраницаИсточник.ТолькоПараметрыВНА Тогда
		//будут получены максимально актуальные параметры, 
		//но расчет амортизации по алгоритму РСБУ будет не актуален, например для отображения на форме элемента ВНА
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ГраницаДоДокумента {(&ГраницаДоДокумента)}", "");
	КонецЕсли;
...
Функция ПолучитьПараметрыОбъектаВНА(ОбъектВНА, ДатаАктуальности = Неопределено) Экспорт
	...
	Граница = Новый Структура("ДатаАктуальности, ТолькоПараметрыВНА", ДатаАктуальности, Истина);//получаем максимально актуальные сведения, без расчета амортизации 
	РеквизитыВНА = ПолучитьРеквизитыВНА(Граница, ОбъектВНА, Неопределено, Неопределено, Неопределено, ПоляВНА);
...
//		Остальные колонке необходимо рассчитать и заполнить. Колонки:
...
//			Курс - Число. Входящие данные. Изменять не следует.
//			ВНА - СправочникСсылка.ОсновныеСредства,СправочникСсылка.НематериальныеАктивы. Входящие данные. Ссылка на внеоборотный актив.
//			ПервоначальнаяСтоимостьДляРасчета - Число. Входящие данные. Для расчета амортизации за период (см. колонку "АмортизацияЗаПериодПоПереданнойСтоимости" ниже).
//			ПервоначальнаяСтоимостьДляРасчетаГр - Число. Входящие данные. Не изменять и не использовать.
//			Проводка - Входящие данные. Изменять не следует.
//			СчетУчетаАмортизации - СправочникСсылка.СчетаБД. По данным МСФО.
//			СчетУчетаЗатратПоАмортизации - СправочникСсылка.СчетаБД. По данным МСФО.
//			АмортизацияЗаПериодПоПереданнойСтоимости - Число. Сумма амортизации МСФО за период по переданной первоначальной стоимости,
//				которая записана в колонку "ПервоначальнаяСтоимостьДляРасчета". Все остальные параметры расчета амортизации берутся по
//				данным организации.
...
Функция ПолучитьГруппыВНА(КоллекцияСтрок, ПараметрыЗаполнения) Экспорт
	...
	РеквизитыВНА = ПолучитьРеквизитыВНА(Граница, ОбъектыВНА, 
...
Функция ПолучитьГраницыПериодаДокумента(КонтекстДокумента, СмещениеПериода = 0, ИспользоватьПериодОтчета = Неопределено, Периодичность = Неопределено) Экспорт
	...
	Если (ИспользоватьПериодОтчета = Истина) ИЛИ (ИспользоватьПериодОтчета = Неопределено) Тогда
		...
		Иначе
			...
			ПериодИсточник = ОбщегоНазначенияУХ.глОтносительныйПериодПоДате(
				КонтекстДокумента.ПериодОтчета.ДатаОкончания,
				КонтекстДокумента.ПериодОтчета.Периодичность,
				СмещениеПериода);
	...
	Иначе
		...
		ПериодичностьОтчетности = ПолучитьПериодичностьОтчетности(Периодичность);
...
Функция ПолучитьСчетМСФОТрансформации(СчетИсточник, Организация = Неопределено, Сценарий, ПериодОтчета) Экспорт
