				...
				Если СтрНайти(НайденнаяСумма, ",") > 0 Тогда
					...
					Исключение
						...
						ТекстОшибки = НСтр("ru = 'Некорректная вырезка суммы комиссии %1 из назначения платежа по причине:
											|%2';
											|en = 'Некорректная вырезка суммы комиссии %1 из назначения платежа по причине:
											|%2'");
						
						Комментарий = СтрШаблон(ТекстОшибки, Результат.Значение, ОписаниеОшибки);
						Событие = НСтр("ru = 'Данные';
										|en = 'Данные'", ОбщегоНазначения.КодОсновногоЯзыка())
							+ "." + НСтр("ru = 'Изменение';
										|en = 'Изменение'", ОбщегоНазначения.КодОсновногоЯзыка());
