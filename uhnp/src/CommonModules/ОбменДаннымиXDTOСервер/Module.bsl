...
Функция ПропускатьОбъектыСОшибкамиПроверкиПоСхеме(УзелИнформационнойБазы, НовоеЗначение = Неопределено) Экспорт
	...
	Иначе
		...
		Исключение
			...
			Событие = НСтр("ru = 'Обмен данными через универсальный формат.Выгрузка.Запись настройки обмена';
							|en = 'Обмен данными через универсальный формат.Выгрузка.Запись настройки обмена'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Для каждого ОтсутствующаяСсылка Из ОбъектXDTO.Строка Цикл
		...
		Если НЕ ОписаниеОтсутствующейСсылки.СсылкаНайденаИЗарегистрированаКОтправке Тогда
			...
			ОписаниеОшибки = НСтр("ru = 'Ссылка с идентификатором %1 по объекту формата %2 не будет повторно выгружена,
									...
									|en = 'Ссылка с идентификатором %1 по объекту формата %2 не будет повторно выгружена,
									|т.к. ей не найдено соответствие в этом приложении.'");
	...
	Для каждого ПодобраннаяОбъектнаяСсылка Из ОбъектыПовторнойВыгрузки Цикл
		...
		Иначе
			...
			ОписаниеОшибки = НСтр("ru = 'Объект %1 не будет повторно выгружен, т.к. не соответствует условиям фильтра.';
									|en = 'Объект %1 не будет повторно выгружен, т.к. не соответствует условиям фильтра.'");
