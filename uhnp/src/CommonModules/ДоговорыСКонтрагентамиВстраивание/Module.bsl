...
Процедура СтруктураРеквизитовДоговораРешения(СтруктураРеквизитов, ДоговорТип, ВидОперацииУХ) Экспорт
	...
	Если ДоговорТип = Тип("СправочникСсылка.ДоговорыМеждуОрганизациями") Тогда
		...
		Если ВидыОперацийУХКлиентСерверПовтИспПереопределяемый.ЭтоОплатаДенежныхСредствВДругуюОрганизацию(ВидОперацииУХ) Тогда
...
Функция ЭтоПлатежВУсловныхЕдиницах(ВалютаВзаиморасчетов, ВалютаПлатежа)
	...
	Если ЗначениеЗаполнено(ВалютаПлатежа) Тогда
		ПлатежВУсловныхЕдиницах = (Не ВалютаПлатежа = ВалютаВзаиморасчетов ИЛИ Не ВалютаПлатежа = ВалютаРегламентированногоУчета);
	Иначе 
		ПлатежВУсловныхЕдиницах = Ложь;
	КонецЕсли;
	
	Возврат ПлатежВУсловныхЕдиницах;
