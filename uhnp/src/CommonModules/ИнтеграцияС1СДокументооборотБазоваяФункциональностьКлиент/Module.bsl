...
Асинх Процедура ОтменитьРедактированиеФайла(Знач ID, Знач ОписаниеОповещенияОЗавершении = Неопределено,
	...
	ТекстСообщения = СтрШаблон(НСтр("ru = 'Файл %1 освобожден.';
									|en = 'The %1 file is unlocked.'"), ТекущийФайл.Наименование);
...
Асинх Процедура ПометитьНаУдалениеСнятьПометкуФайлов(Знач ВыделенныеФайлы, Знач ТекущаяПометкаУдаления,
	...
	Для Каждого Файл Из ВыделенныеФайлы Цикл
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Файл %1 удален.';
										|en = 'The %1 file is deleted.'"), Файл.Наименование);
		ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
...
Асинх Процедура СохранитьФайлыКакНачало(Знач Параметры) Экспорт
	...
	Если Не Ждать РасширениеРаботыСФайламиПодключеноАсинх() Тогда
		...
		ИначеЕсли Параметры.ТекущийФайл.Тип = "DMFileVersion" Тогда
			Адрес = ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ПолучитьВерсиюФайлаИПоместитьВХранилище(
				...
				Параметры.ТекущийФайл.ДатаМодификацииУниверсальная);
		Иначе
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Неизвестный тип файла: %1';
											|en = 'Unknown file type: %1'"), Параметры.ТекущийФайл.Тип);
		КонецЕсли;
	...
	Если Параметры.ОставшиесяФайлы.Количество() = 0 Тогда
		...
		Диалог.Фильтр = СтрШаблон(НСтр("ru = 'Все файлы (*.%1)|*.%1';
										|en = 'All files (*.%1)|*.%1'"), Параметры.ТекущийФайл.Расширение);
...
Процедура ЗакончитьРедактированиеСРасширениемПослеВопросаОбОсвобождении(Параметры)
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.СохранитьСведенияОРедактированииФайла(
		СведенияОРедактировании);
	ТекстСообщения = СтрШаблон(НСтр("ru = 'Файл %1 обновлен и освобожден.';
									|en = 'The %1 file is updated and unlocked.'"), Параметры.ТекущийФайл.Наименование);
	ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
	ДействиеНадФайлом.РедактированиеФайлаЗавершено = Истина;
	ДействиеНадФайлом.ИнформироватьФормуФайла = Истина;
	ДействиеНадФайлом.ИнформироватьФормуСпискаФайлов = Истина;
...
Процедура ОбновитьИзФайлаНаДискеПослеПомещенияФайлаБезРасширения(ОписаниеПомещенногоФайла, Параметры) Экспорт
	...
	Если ИнтеграцияС1СДокументооборотБазоваяФункциональностьВызовСервера.ОбновитьФайлДвоичнымиДаннымиВременногоХранилища(
			Параметры) Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Файл %1 обновлен.';
										|en = 'The %1 file is updated.'"), Параметры.ТекущийФайл.Наименование);
		ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
...
Процедура ОткрытьФайлыПослеПолучения(ПолныеИменаФайлов, Параметры) Экспорт
	...
	Если Параметры.НаЧтение = Ложь Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Начато редактирование файла %1.';
										|en = 'Started editing the %1 file.'"), Параметры.ТекущийФайл.Наименование);
		ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
...
Процедура СохранитьИзмененияСРасширениемПослеВопросаОбОсвобождении(Параметры)
	...
	ТекстСообщения = СтрШаблон(НСтр("ru = 'Изменения файла %1 сохранены.';
									|en = 'The %1 file changes are saved.'"), Параметры.ТекущийФайл.Наименование);
	ДействиеНадФайлом = ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентСервер.ДействиеНадФайлом();
	ДействиеНадФайлом.РедактированиеФайлаЗавершено = Истина;
	ДействиеНадФайлом.ИнформироватьФормуФайла = Истина;
	ДействиеНадФайлом.ИнформироватьФормуСпискаФайлов = Истина;
...
#Если МобильныйКлиент Тогда
	...
	ПоказатьПредупреждение(, НСтр("ru = 'В мобильном клиенте печать файлов не поддерживается.';
									|en = 'Printing files is not supported in the mobile client.'"));
	Возврат;
	
#ИначеЕсли ВебКлиент Тогда
	
	ПоказатьПредупреждение(, НСтр("ru = 'В веб-клиенте печать файлов не поддерживается.';
									|en = 'Printing files is not supported in the web client.'"));
	Возврат;
