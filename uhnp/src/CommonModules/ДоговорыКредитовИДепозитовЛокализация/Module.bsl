...
Процедура УстановитьВидимостьВидаДоходаИностранногоКонтрагента(Форма)
	...
	Если Элементы.Найти("ВидДоходаИностранногоКонтрагента") = Неопределено Тогда
		Возврат;
	КонецЕсли;
	#КонецОбласти
...
Функция ИспользоватьУникальныйНомерВалютногоКонтроля(Объект, КонтрагентЮрФизЛицо)
	...
	Если Объект.ВидДоговораУХ = Справочники.ВидыДоговоровКонтрагентовУХ.ЗаемПолученный
			ИЛИ Объект.ВидДоговораУХ = Справочники.ВидыДоговоровКонтрагентовУХ.ЗаемВыданный Тогда
			
			ЭтоВнутригрупповойКонтрагент = РаботаСКонтрагентамиУХ.ЭтоВнутригрупповойКонтрагент(Объект.Контрагент);
	...
	Возврат ХарактерыДоговоров.Найти(Объект.ХарактерДоговора) <> Неопределено
		И ТипыДоговоров.Найти(ТипДоговора) <> Неопределено
