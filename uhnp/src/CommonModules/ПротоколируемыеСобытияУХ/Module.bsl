...
Функция Завершить(Наименование, Описание = "", 
						...
						ОбъектРасчета=Неопределено,ЗаписатьВФоне=Ложь) Экспорт
	...
	Если (НЕ ЕстьПротоколПотока()) И ОбъектРасчета=Неопределено Тогда
		...
		Иначе
			...
			ОбъектСобытие = Справочники.ПротоколируемыеСобытия.СоздатьЭлемент();
			Возврат ОбъектСобытие.Записи.Выгрузить();
	...
	ВозвращатьТЗ=НЕ ЗаписыватьОбъект;
	
	Если ВозвращатьТЗ ИЛИ СтруктураОтбора.Количество()=0  Тогда  
		
		Отменить();
		
		Если ВозвращатьТЗ Тогда 
			
			ОбъектСобытие = Справочники.ПротоколируемыеСобытия.СоздатьЭлемент();
			ТаблицаЗаписей=ОбъектСобытие.Записи.ВыгрузитьКолонки();
			
			ДобавитьЗаписиПротокола(Неопределено,ТаблицаЗаписей,ПротоколПотока,КонтекстПротокола);
			
			Если (Не ОбъектРасчета=Неопределено) Тогда
				ДобавитьЗаписиПротокола(Неопределено,ТаблицаЗаписей,ОбъектРасчета.ТаблицаПротоколаСеанс,ОбъектРасчета);
			КонецЕсли;
			
			ОбъектСобытие=Неопределено;
	...
	СтруктураСобытия=Новый Структура("Наименование,Описание,Дата,Объект,ФайлИмпорта", Наименование, Описание, ТекущаяДата(), Объект,ФайлИмпорта);
	
	Если Не ЗаписатьВФоне Тогда
		
		ПротоколируемоеСобытие = Справочники.ПротоколируемыеСобытия.ДобавитьЭлементОбъект(СтруктураСобытия);
		...
		Попытка
			...
			КонецЕсли;
		Исключение
			// Уже могли быть ошибки в другой транзакции
		КонецПопытки;
	...
	Иначе
		...
		////////////////////////////////////////////////
		// Запись протокола в отдельном сеансе
		
		ПараметрыЗадания=Новый Массив;
		ПараметрыЗадания.Добавить(СтруктураСобытия);
		ПараметрыЗадания.Добавить(ПротоколПотока);
		ПараметрыЗадания.Добавить(КонтекстПротокола); 
		ПараметрыЗадания.Добавить(ОбъектРасчета);
		ПараметрыЗадания.Добавить(СтруктураОтбора);
		ПараметрыЗадания.Добавить(ОчищатьПредыдущие);
		ПараметрыЗадания.Добавить(ФормироватьПустое);
		
		ФоновоеЗадание = ФоновыеЗадания.Выполнить("ПротоколируемыеСобытияУХ.ЗаписатьДанныеПротокола", ПараметрыЗадания, Новый УникальныйИдентификатор, НСтр("ru = 'Запись протокола'"));	
		
	КонецЕсли;
	
Конецфункции 

Процедура ЗаписатьДанныеПротокола(СтруктураСобытия,
				ПротоколПотока,
				КонтекстПротокола,
				ОбъектРасчета,
				СтруктураОтбора,
				ОчищатьПредыдущие,
				ФормироватьПустое) Экспорт
	...
	Если Записи.Количество()>0 ИЛИ ФормироватьПустое Тогда
		...
		Если ЗначениеЗаполнено(СтруктураСобытия.Объект) Тогда
			
			Если Записи.Количество()>0 Тогда
				
				ЗаписатьПротоколВРегистр(
				СтруктураСобытия.Объект,
				,
				ПротоколируемоеСобытие.Ссылка);
				
			Иначе
				
				ЗаписатьПротоколВРегистр(СтруктураСобытия.Объект);
				
			КонецЕсли;
					
		КонецЕсли;
		
	ИначеЕсли ЗначениеЗаполнено(СтруктураСобытия.Объект) Тогда
		
		ЗаписатьПротоколВРегистр(СтруктураСобытия.Объект);	
		
	КонецЕсли;
			
КонецПроцедуры // ЗаписатьДанныеПротокола()
...
Функция ПолучитьТаблицуПротоколовПоОбъекту(ДанныеОбъекта) Экспорт
	...
	Если Не ПустаяСтрока(ТекстОтбор) Тогда
		...
		Запрос.Текст=СтрШаблон("%1
			|ГДЕ %2", Запрос.Текст, Сред(ТекстОтбор,СтрДлина("И") + 3));
...
Процедура ДобавитьЗапись(Событие, УровеньЖурналаРегистрации, ОбъектМетаданных, Данные, Комментарий, ТехническаяИнформация,КонтекстСобытия=Неопределено, ИспользоватьФоновоеЗадание = Ложь)
	...
	Если НЕ ТехническаяИнформация = Неопределено И Не ТехническаяИнформация = "" Тогда
