...
Процедура ЗаполнитьОперативныеВзаиморасчеты(ОсновныеПараметры, ДополнительныеСвойстваПроведения = Неопределено) Экспорт
	...
	ЗапросГлобальныхПеременных.Текст = ВзаиморасчетыВстраиваниеУХ.ЗаменитьТекстЗапроса_ЗаполнитьОперативныеВзаиморасчеты(ЗапросГлобальныхПеременных.Текст);
	#КонецОбласти
	...
	Для Каждого СтрокаОсновныхПараметровРасчета Из ТаблицаОсновныхПараметровРасчета Цикл
		...
		#Область УХ_Встраивание
		ГлобальныеПеременные.Вставить("СпособОпределенияКурсаПлатежа", СтрокаОсновныхПараметровРасчета.СпособОпределенияКурсаПлатежа); 
		#КонецОбласти
		...
		Для Каждого СтрокаЗачета Из ТаблицаЗачетовОплат Цикл
			...
			Иначе
				...
				Если НЕ СтрокаЗачета.ЭтоКорректировка Тогда
					...
					//Перенос долга производится по курсу остатка после переоценки
					Если СтрокаОплаты.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереносДолга Тогда
						СуммаСписанияРегл = СуммаСписания(0, СтрокаОтгрузкиПоставки.СуммаРеглДляПереоценки, СтрокаОтгрузкиПоставки.Сумма, СуммаСписания);
						СуммаСписанияУпр = СуммаСписания(0, СтрокаОтгрузкиПоставки.СуммаУпрДляПереоценки, СтрокаОтгрузкиПоставки.Сумма, СуммаСписания);
					Иначе
						СуммаСписанияРегл = СуммаСписания(0, СтрокаОплаты.СуммаРегл, СтрокаОплаты.Сумма, СуммаСписания);
						СуммаСписанияУпр = СуммаСписания(0, СтрокаОплаты.СуммаУпр, СтрокаОплаты.Сумма, СуммаСписания);
					КонецЕсли;
...
Процедура ВыполнитьПереоценкуНаДату(МассивОрганизаций, Период, Параметры) Экспорт
	...
	#Область УХ_Встраивание
	ТекстЗапроса = ВзаиморасчетыВстраиваниеУХ.ЗаменитьТекстЗапроса_ВыполнитьПереоценкуНаДату(ТекстЗапроса);
	#КонецОбласти
...
Функция ПолучитьКурсыВалют(Запрос, ДатаНачала, ГлобальныеПеременные)
	...
	Если ГлобальныеПеременные.ВариантКурсаДоговора = Перечисления.ВариантыКурсаДоговора.УстановленныйВДоговоре Тогда
		...
		#Область УХ_Встраивание
		Запрос.Текст = ВзаиморасчетыВстраиваниеУХ.ЗаменитьТекстЗапроса_ПолучитьКурсыВалют_ПервоеВхождение(Запрос.Текст);
		#КонецОбласти
	...
	#Область УХ_Встраивание
	Запрос.Текст = ВзаиморасчетыВстраиваниеУХ.ЗаменитьТекстЗапроса_ПолучитьКурсыВалют_ВтороеВхождение(Запрос.Текст);
	#КонецОбласти
