...
Процедура ПередЗаписьюКонстантыБЭД(Источник, Отказ) Экспорт
	...
	Если НедоступныеКонстанты.Найти(ИмяКонстанты) <> Неопределено Тогда
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Интерактивное изменение значения константы ""%1"" запрещено';
				|en = 'Cannot change the ""%1"" constant value manually'", "ru"), Синоним);
	КонецЕсли;
...
Функция КонстантыНедоступныеДляВключения()
	...
	СписокКонстант.Добавить(Метаданные.Константы.ИспользоватьКонтрольОтраженияВУчетеЭДО.Имя);
