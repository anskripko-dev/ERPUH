...
Процедура ПроверитьДанныеКартыКлиента(ПараметрыМетода, СтруктураОтвета) Экспорт
	...
	Если НЕ ИспользоватьКартыЛояльности Тогда
		...
		СтруктураОтвета.ErrorMessage = НСтр("ru = 'Карты лояльности не поддерживаются.';
											|en = 'Loyalty cards are not supported.'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
		...
		Если ВходящиеПараметры.Свойство(СвойствоИмя, ЗначениеСвойства) Тогда
			...
			Исключение
				...
				ШаблонОписанияОшибки = НСтр(
					...
					|""%3""';
					|en = 'An error occurred when receiving the %1 item by ID (%2):
