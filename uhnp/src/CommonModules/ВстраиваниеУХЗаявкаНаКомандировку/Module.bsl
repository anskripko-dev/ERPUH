...
Процедура ПередЗаписью(Объект, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Объект.ДополнительныеСвойства.Вставить("ПлатежнаяПозиция", 
		ПлатежныеПозиции.НоваяПлатежнаяПозицияПоДаннымДокументаЗаявкаНаКомандировку(Объект));
	...
	Если РеквизитСтатья.РазмещенВТЧ Тогда
		...
		Если (ИтогоПоСтатьямСуммаДокумента > 0 ) 
			И НЕ ВидыОперацийУХКлиентСерверПовтИсп.ЭтоПеремещениеВнутриОрганизации(Объект.ВидОперацииУХ) Тогда
...
Процедура ПодготовитьФормуНаСервере(Форма)
	...
	ПлатежныеПозиции.ИнициализироватьПлатежнуюПозициюЗаявки(Форма, "ЖелательнаяДатаПлатежа");
