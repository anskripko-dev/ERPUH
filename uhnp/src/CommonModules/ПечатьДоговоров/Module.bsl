...
Процедура ЗаполнитьТекстДоговораПоШаблону(ШаблонДоговора, ФорматированныйДокумент, СсылкаНаОбъект) Экспорт
	...
	Если ОценкаПроизводительностиВызовСервераПовтИсп.ВыполнятьЗамерыПроизводительности() Тогда
		ТипДокументСсылка = "ДокументСсылка.СчетНаОплатуПокупателю";
		Если ТипЗнч(СсылкаНаОбъект) = Тип(ТипДокументСсылка) Тогда
	...
	Если СведенияОСделке = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ШаблоныДоговоров = "ШаблоныДоговоров";	
...
Функция ЗначениеПараметра(Знач ИмяПараметра, СведенияОСделке, ЗначенияСоставногоПараметра, Успешно) Экспорт
	...
	ИначеЕсли ИмяПараметра = Врег("ГородПоПочтовомуАдресуКонтрагента") Тогда
		...
		ГородПоПочтовомуАдресу = НаселенныйПунктПоАдресу(
	...
	ИначеЕсли ИмяПараметра = Врег("ГородПоФактическомуАдресуКонтрагента") Тогда
		...
		ГородПоФактическомуАдресу = НаселенныйПунктПоАдресу(
	...
	ИначеЕсли ИмяПараметра = Врег("ГородПоЮридическомуАдресуКонтрагента") Тогда
		...
		ГородПоЮридическомуАдресу = НаселенныйПунктПоАдресу(
	...
	ИначеЕсли ИмяПараметра = Врег("СуммаНДСДоговора") Тогда
		...
		Если ЗначениеЗаполнено(СведенияОДоговоре.СтавкаНДС) Тогда
			Если СведенияОДоговоре.СтавкаНДС <> Перечисления.СтавкиНДС.БезНДС Тогда
				Результат = ОбщегоНазначенияБПВызовСервера.ФорматСумм(
					СведенияОДоговоре.СуммаНДС, СведенияОДоговоре.ВалютаВзаиморасчетов, "0,00");
			Иначе
				Результат = НСтр("ru = 'без налога (НДС)'");
			КонецЕсли;
		Иначе
	...
	ИначеЕсли ИмяПараметра = Врег("СтавкаНДСДоговора") Тогда
		...
		Если ЗначениеЗаполнено(СведенияОДоговоре.СтавкаНДС) Тогда
			Если СведенияОДоговоре.СтавкаНДС <> Перечисления.СтавкиНДС.БезНДС Тогда
				ПроцентНДС = ВстраиваниеУХВызовСервера.ПолучитьСтавкуНДС(СведенияОДоговоре.СтавкаНДС);
...
Функция ЦветФонаЗаменяемогоПараметра() Экспорт
	...
	МетаданныеЭлементаСтиля = Метаданные.ЭлементыСтиля.ЦветФонаВыделения;
...
Функция ЗначениеПараметраСуммаСНДСДоговораПрописью(СведенияОСделке, ЗначенияСоставногоПараметра, Успешно)
	...
	Если СведенияОДоговоре.СтавкаНДС = Справочники.СтавкиНДС.БезНДС Тогда
