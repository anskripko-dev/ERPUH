	...
	Если СписокТиповПараметра.Количество() > 0 Тогда
		Если ПримитивныйТип(СписокТиповПараметра[0].Значение) Тогда
			...
			Иначе
				...
				ОписаниеТиповСписок                       = Новый ОписаниеТипов("СписокЗначений");
				СтрокаПараметр.УточнениеСпособаОпределения = ОписаниеТиповСписок.ПривестиЗначение(ТекЗначениеПоля);
				СтрокаПараметр.УточнениеСпособаОпределения.ТипЗначения=ОписаниеТипов;
...
Процедура ОбработатьВыборЗначенияПараметра(СтрокаПараметр, ФормаОбъекта, ДанныеОбъекта, Элемент, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТекСпособВычисления = ПредопределенноеЗначение("Перечисление.СпособыВычисленияПараметровОперандов.ФункцияНаВстроенномЯзыкеЗначение")
		...
		ОткрытьФорму_РедактированиеПроцедур(СтрокаПараметр, ФормаОбъекта, СтандартнаяОбработка, ДанныеОбъекта);
	...
	ИначеЕсли ТекСпособВычисления = ПредопределенноеЗначение("Перечисление.СпособыВычисленияПараметровОперандов.СохраненнаяНастройкаОтбора") Тогда
		
		СтандартнаяОбработка = Ложь;		
		ОткрытьФорму_СохраненнаяНастройка(ДанныеОбъекта,СтрокаПараметр,Элемент); 
...
&НаКлиенте
Процедура ОткрытьФорму_РедактированиеПроцедур(СтрокаПараметр, ФормаОбъекта, СтандартнаяОбработка, ДанныеОбъекта)
	...
	СтруктураПараметров.Вставить("Заголовок", НСтр("ru = 'Расчет значения параметра'"));
	
	Если СтрНайти(ФормаОбъекта.ИмяФормы,"ИсточникиДанныхДляРасчетов")>0 
		И ЗначениеЗаполнено(ФормаОбъекта.ВидОтчета) Тогда
		
		СтруктураПараметров.Вставить("ВидОтчета",ФормаОбъекта.ВидОтчета);
		
	КонецЕсли;
	
	СпособПолучения = Неопределено;
	ДанныеОбъекта.Свойство("СпособПолучения", СпособПолучения);
	
	Если ЗначениеЗаполнено(СпособПолучения) Тогда
		
		СтруктураПараметров.Вставить("СпособПолучения", СпособПолучения);
...
&НаКлиенте
Процедура ОткрытьФорму_СохраненнаяНастройка(ДанныеОбъекта,СтрокаПараметр,Элемент)
	
	ТекущееЗначение=СтрокаПараметр.УточнениеСпособаОпределения;
	
	СтруктураОтбора=Новый Структура;
	СтруктураОтбора.Вставить("ИмяТаблицы",СтрЗаменить(СтрокаПараметр.ТипЗначения[0].Значение,".","_"));
	СтруктураОтбора.Вставить("Владелец",ДанныеОбъекта.ТипБД);
	ИмяФормы="Справочник.СохраненныеНастройкиОтборов.ФормаВыбора";
		
	ОткрытьФорму(ИмяФормы,Новый	Структура("Отбор,ТекущаяСтрока",СтруктураОтбора,ТекущееЗначение),Элемент); 
		
КонецПроцедуры // ОткрытьФорму_СохраненнаяНастройка() 
...
Процедура Подключаемый_ОткрытьФормуПоВыбранномуТипу(Результат, ДополнительныеПараметры) Экспорт
    ...
    Если ВыбранныйТип <> Неопределено Тогда
        ...
        Если ТекСпособВычисления = ПредопределенноеЗначение("Перечисление.СпособыВычисленияПараметровОперандов.ФиксированноеЗначение")
			...
			Если Не ТипЗнч(ТекЗначениеПоля)=ВыбранныйТип.Значение Тогда
				
				ТекЗначениеПоля=Неопределено; 
        ...
        ИначеЕсли ТекСпособВычисления = ПредопределенноеЗначение("Перечисление.СпособыВычисленияПараметровОперандов.СписокФиксированныхЗначений")
			...
			Если ТипЗнч(ТекЗначениеПоля)=Тип("СписокЗначений")
				И ТекЗначениеПоля.Количество()>0 И (НЕ ТипЗнч(ТекЗначениеПоля[0].Значение)=ВыбранныйТип.Значение) Тогда 
				
				ТекЗначениеПоля.Очистить(); 
