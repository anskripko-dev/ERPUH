...
Процедура ПриНачалеРаботыСистемы() Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда				// не БМ, НМ
		МодульСистемаВзаимодействияКлиентУХ = ОбщегоНазначенияКлиент.ОбщийМодуль("СистемаВзаимодействияКлиентУХ");
		МодульСистемаВзаимодействияКлиентУХ.ПодключениеСистемыВзаимодействияДляОповещений();
		
		МодульТелеграмСерверУХ = ОбщегоНазначенияКлиент.ОбщийМодуль("ТелеграмСерверУХ");
		МодульТелеграмСерверУХ.ПодключениеТелеграмДляОповещений();
	КонецЕсли;
...
Процедура ВключитьНапоминания() Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда
		ОбщийМодульРасширениеПроцессыИСогласованиеКлиентУХ = ОбщегоНазначенияКлиент.ОбщийМодуль("РасширениеПроцессыИСогласованиеКлиентУХ");
		ОбщийМодульРасширениеПроцессыИСогласованиеКлиентУХ.ЗапуститьПроверкуТекущихНапоминаний();
	КонецЕсли;
...
&НаКлиенте
Процедура ПроверитьМодифицированность(Форма, ИмяОбработчика, ПараметрыОбработчика=Неопределено) Экспорт
	
	ДанныеОбработчика = Новый Структура;
	ДанныеОбработчика.Вставить("Форма", Форма);
	ДанныеОбработчика.Вставить("ИмяОбработчика", ИмяОбработчика);
	ДанныеОбработчика.Вставить("ПараметрыОбработчика", ПараметрыОбработчика);
	
	Если Форма.Модифицированность Тогда
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ПроверитьМодифицированностьПослеВопроса", ЭтотОбъект, ДанныеОбработчика);
	...
	Иначе
		...
		ПроверитьМодифицированностьПослеВопроса(Неопределено, ДанныеОбработчика);
		
	КонецЕсли;
	
КонецПроцедуры

Процедура ПроверитьМодифицированностьПослеВопроса(Результат, ДанныеОбработчика) Экспорт
	
	Форма = ДанныеОбработчика.Форма;
	...
	ИмяОбработчика = ДанныеОбработчика.ИмяОбработчика;
	Если ДанныеОбработчика.ПараметрыОбработчика = Неопределено Тогда
		ИмяОбработчика = "Форма." + ИмяОбработчика + "()";
	Иначе
		ИмяОбработчика = "Форма." + ИмяОбработчика + "(ДанныеОбработчика.ПараметрыОбработчика)";
	КонецЕсли;
