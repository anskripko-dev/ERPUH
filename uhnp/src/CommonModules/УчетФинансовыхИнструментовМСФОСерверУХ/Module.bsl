...
Функция ПолучитьПараметрыРасчета(ПараметрыУчетаФИ) Экспорт
	...
	|	ПараметрыФИ.ВидСтавкиРыночная = ЗНАЧЕНИЕ(Справочник.ВидыКотировокФинансовыхИнструментов.ПустаяСсылка) КАК ДоступнаСтавкаДисконтирования,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПараметрыУчетаФинансовыхИнструментовМСФО КАК ПараметрыФИ
	|		ПО (ПараметрыФИ.Ссылка = &Ссылка)
	|			И т.Ссылка = ПараметрыФИ.ВидОбъектаФинансовогоХарактера");
...
Функция ТекстЗапросаПредыдущийГрафик()
	...
	|	СведенияОФИСрезПоследних.Валюта КАК Валюта
...
Функция ПолучитьДенежныеПотоки(ДанныеГрафика, Проценты = Ложь, ДопРасходУсл = Ложь, ДопРасходМод = Ложь, 
	...
	Если ВНА И ДанныеГрафика.Лизинг Тогда
		...
		Для каждого СтрокаТаб Из ДанныеГрафика.ВНА Цикл
			...
			СуммаМСФО = ВВалюту(СтрокаТаб.СуммаМСФО, ФВ, ВалютаФИ, СтрокаТаб.Дата);
	...
	Если Проценты Тогда
		Для каждого СтрокаТаб Из ДанныеГрафика.Проценты Цикл
			ДобавитьДенежныйПоток(ДенежныеПотоки, СтрокаТаб.Дата,
				СтрокаТаб.Уплата - СтрокаТаб.НеарендныйПлатеж - СтрокаТаб.НДС, ДатаНачалаГрафика, ДатаИскючения);
		КонецЦикла;
	КонецЕсли;
	...
	Если АвансыВкл Или АвансыИскл Тогда
		Для каждого СтрокаТаб Из ДанныеГрафика.АвансовыеПлатежи Цикл
			...
			Если АвансыВкл И СтрокаТаб.ВключенВОсновнойДолг Тогда
				ДобавитьДенежныйПоток(ДенежныеПотоки, СтрокаТаб.Дата, СуммаАванса, ДатаНачалаГрафика, ДатаИскючения);
			КонецЕсли;
...
Функция ВВалюту(Сумма, ВалютаИсточник, ВалютаПриемник, Дата) Экспорт
	...
	Если ВалютаИсточник = ВалютаПриемник Тогда
		Возврат Сумма;
	ИначеЕсли Не ЗначениеЗаполнено(ВалютаИсточник) Тогда
		Возврат Сумма;
	ИначеЕсли Не ЗначениеЗаполнено(ВалютаПриемник) Тогда
		Возврат Сумма;
	Иначе
