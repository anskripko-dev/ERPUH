...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	|ВЫБРАТЬ
	|	ВыходныеИзделия.НомерСтроки     КАК НомерСтроки,
	|	&ИмяТЧВыходныеИзделия           КАК ИмяТЧ,
	|	&ПредставлениеТЧВыходныеИзделия КАК ПредставлениеТЧ,
	|	ВЫБОР
	|		КОГДА (ВыходныеИзделия.Назначение.ТипНазначения В(
	...
	//++ Устарело_Переработка24
	|				ИЛИ ВыходныеИзделия.ОформитьОтчетДавальцу
	//-- Устарело_Переработка24
	|				ИЛИ ЛОЖЬ)
	|			И ВыходныеИзделия.Номенклатура.ПрослеживаемыйТовар
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ                           КАК ПрослеживаемоеДавальческоеИзделие
	|ИЗ
	|	ВыходныеИзделия КАК ВыходныеИзделия
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ВЫБОР
	|		КОГДА (ПобочныеИзделия.Назначение.ТипНазначения В(
	//++ Устарело_Переработка24
	|				ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.ДавальческоеПродукция22),
	|				ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.ДавальческоеМатериалы22),
	//-- Устарело_Переработка24
	...
	|				ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.Давальческое2_5))
	//++ Устарело_Переработка24
	|				ИЛИ ПобочныеИзделия.ОформитьОтчетДавальцу
	//-- Устарело_Переработка24
	|				ИЛИ ЛОЖЬ)
	...
	Запрос.УстановитьПараметр("ИмяТЧВыходныеИзделия", "ВыходныеИзделия");
	Запрос.УстановитьПараметр("ИмяТЧПобочныеИзделия", "ПобочныеИзделия");
	Запрос.УстановитьПараметр("ПредставлениеТЧВыходныеИзделия", Метаданные.Документы.ПроизводствоБезЗаказа.ТабличныеЧасти.ВыходныеИзделия.Синоним);
	Запрос.УстановитьПараметр("ПредставлениеТЧПобочныеИзделия", Метаданные.Документы.ПроизводствоБезЗаказа.ТабличныеЧасти.ПобочныеИзделия.Синоним);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		
		Если Выборка.ПрослеживаемоеДавальческоеИзделие Тогда
			
			ТекстСообщения =
				СтрШаблон(
					НСтр("ru = 'Запрещено выбирать назначение давальца для прослеживаемой номенклатуры (строка %1 списка ""%2"").';
						|en = 'You cannot select the provider assignment for traceable item (line %1 of the ""%2"" list).'"),
					Формат(Выборка.НомерСтроки, "ЧГ="),
					Выборка.ПредставлениеТЧ);
			
			Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(Выборка.ИмяТЧ, Выборка.НомерСтроки, "Назначение");
...
#Область ТекстСписаниеПродукцииНаАктивыПассивы // (Дт Документ.СчетУчета :: Кт Счет учета затрат подразделения)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтДвиженияПоПрочимАктивамПассивам КАК ДвиженияПоПрочимАктивамПассивам
	|		ПО Строки.Ссылка = ДвиженияПоПрочимАктивамПассивам.Ссылка
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	#Область ДвиженияПоПрочимАктивамПассивам
	...
	|	ДанныеРегистра.НастройкаСчетовУчета КАК НастройкаСчетовУчета
	|
...
Функция ТекстОприходованиеТоваровПоСебестоимостиНалоговыйУчет() Экспорт
	...
	|			И УчетнаяПолитикаОрганизаций.УчетПродукцииПереработчикаНа20Счете
	|			И ЕСТЬNULL(КорСтроки.ТипЗапасов, Строки.КорТипЗапасов) = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияПереработчика)
	|		ТОГДА ЕСТЬNULL(КорСтроки.Склад.Партнер, Строки.КорСклад.Партнер)
	|		ИНАЧЕ Строки.КорСклад
	|	КОНЕЦ КАК МестоУчетаДт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
