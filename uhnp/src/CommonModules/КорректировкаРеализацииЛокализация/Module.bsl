...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5 Тогда
		УчетПрослеживаемыхТоваровЛокализация.ПроверитьНаличиеПрослеживаемойНоменклатурыВТабличнойЧасти(Объект, Отказ);
	КонецЕсли;
	//-- НЕ УТКА
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОтветственноеХранение") Тогда
		...
		КомандаПечати.НаименованиеПечатнойФормы = НСтр("ru = 'Акт о приеме-передаче ТМЦ на хранение';
														|en = 'Certificate of inventory handover for storage'");
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОтветственноеХранение") Тогда
		...
		КомандаПечати.НаименованиеПечатнойФормы = НСтр("ru = 'Акт о возврате ТМЦ, сданных на хранение';
														|en = 'Certificate of stored inventory return'");
	КонецЕсли;
