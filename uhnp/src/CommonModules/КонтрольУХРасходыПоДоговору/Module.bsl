...
Функция ТребуетсяДляДокумента(ИмяДокумента) Экспорт
	...
	Если ИмяДокумента = "ВерсияСоглашенияКоммерческийДоговор" ИЛИ
		ИмяДокумента = "ЗаявкаНаРасходованиеДенежныхСредств" ИЛИ
		ИмяДокумента = "ЗаказПоставщику" ИЛИ
		ИмяДокумента = "РеализацияТоваровУслуг" ИЛИ
		ИмяДокумента = "ЗаявкаНаРасход" Тогда
		Возврат Истина; 
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Функция ОбработатьДанныеИсточника(ИнформацияДляКонтроля, Источник) Экспорт
	...
	Если ТипЗнч(Источник.ДанныеДляПроверки) = Тип("ТаблицаЗначений") Тогда
		
		ЭлементыОрганизации = Источник.ДанныеДляПроверки.ВыгрузитьКолонку("Организация");
		Организации = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ЭлементыОрганизации);
		ЗначениеОрганизации = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(Организации, "ГоловнаяОрганизация");
		
		Для Каждого СтрокаТаблицы Из Источник.ДанныеДляПроверки Цикл			
			Организация = ЗначениеОрганизации.Получить(СтрокаТаблицы.Организация);
	...
	КонецЦикла;
	Запрос.УстановитьПараметр("ЭтоВерсияСоглашения", 
		ТипЗнч(Источник.Документ) = Тип("ДокументСсылка.ВерсияСоглашенияКоммерческийДоговор"));	
...
Функция БлокироватьПроведение(КлючКонтроля) Экспорт
	...
	РежимКонтроля = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		ПланыВидовХарактеристик.ВидыКонтроляДокументов.КонтрольРасходовПоДоговору, "РежимКонтроля");
...
Функция СтруктураИсточник(Документ, Организация, КонтрольОплаты) Экспорт
	...
	ДанныеДляПроверки.Колонки.Добавить("Организация",	Метаданные.ОпределяемыеТипы.Организация.Тип);
	ДанныеДляПроверки.Колонки.Добавить("ЭтоОплата",		Новый ОписаниеТипов("Булево"));
	ДанныеДляПроверки.Колонки.Добавить("Контрагент",	Новый ОписаниеТипов("СправочникСсылка.Контрагенты"));
	ДанныеДляПроверки.Колонки.Добавить("Договор",		Метаданные.ОпределяемыеТипы.Договор.Тип);
	ДанныеДляПроверки.Колонки.Добавить("Валюта",		Новый ОписаниеТипов("СправочникСсылка.Валюты")); // Валюта взаиморасчетов
	...
	Проверка = Создать();
	КонтрольУХВстраивание.СтруктураИсточник(Проверка, Результат);
...
Функция ТекстЗапроса_ИтоговыйЗапрос()
	...
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(Запрос.Организация КАК Справочник.Организации).ГоловнаяОрганизация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|			ТОГДА Запрос.Организация
	|		ИНАЧЕ ВЫРАЗИТЬ(Запрос.Организация КАК Справочник.Организации).ГоловнаяОрганизация
