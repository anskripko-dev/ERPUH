...
//++ НЕ УТ
	...
	Если ТипДокумента = Тип("ДокументСсылка.ПроизводствоБезЗаказа") Тогда
		ИмяТаблицыТовары = "МатериалыИРаботы";
		ИмяТаблицыСерии = "";
		ИмяТаблицыШтрихкодов = "ШтрихкодыУпаковокМатериалы";
	КонецЕсли;
...
//++ НЕ УТКА
	...
	Если ТипДокумента = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
		ИмяТаблицыТовары = "РасходМатериаловИРабот";
		ИмяТаблицыСерии = "";
		ИмяТаблицыШтрихкодов = "ШтрихкодыУпаковокМатериалы";
	КонецЕсли;

	Если ТипДокумента = Тип("ДокументСсылка.ПроизводственнаяОперация2_2") Тогда
		ИмяТаблицыТовары = "МатериалыИРаботы";
		ИмяТаблицыСерии = "";
		ИмяТаблицыШтрихкодов = "ШтрихкодыУпаковокМатериалы";
	КонецЕсли;
...
Процедура ЗаполнитьОтчетПроизводстваЕГАИСНаОснованииПроизводствоБезЗаказа(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|ИТОГИ ПО
	|	Склад");
	МассивСтрок.Добавить(ШаблонКодыМаркировки("ОтчетОПроизводствеЕГАИС", "ПроизводствоБезЗаказа", "ШтрихкодыУпаковокМатериалы"));
	Запрос = Новый Запрос;
	Запрос.Текст = СтрСоединить(МассивСтрок);
	...
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	ВыборкаПоСкладам = РезультатЗапроса[РезультатЗапроса.Количество()-3].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	...
	АкцизныеМарки = РезультатЗапроса[РезультатЗапроса.Количество() - 1];
	ЗаполнитьТоварыАкцизныеМарки(ДокументОбъект, АкцизныеМарки,"ШтрихкодыУпаковокМатериалы",,"Сырье");
...
Процедура ЗаполнитьОтчетПроизводстваЕГАИСНаОснованииЭтапПроизводство2_2(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|		ПО ИСТИНА"
	 + ШаблонКодыМаркировки("ОтчетОПроизводствеЕГАИС", "ЭтапПроизводства2_2", "ШтрихкодыУпаковокМатериалы");
	...
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	АкцизныеМарки = РезультатЗапроса[РезультатЗапроса.Количество()-1];
	Сырье = РезультатЗапроса[РезультатЗапроса.Количество()-3];
...
//++ НЕ УТ
	...
	Если ТипЗнч(РезультатПроверки.ПроверяемыйДокумент) = Тип("ДокументСсылка.ПроизводствоБезЗаказа") Тогда
		РезультатПроверки.Сценарий = 1;
	КонецЕсли;
...
//++ НЕ УТКА
	...
	Если ТипЗнч(РезультатПроверки.ПроверяемыйДокумент) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
		РезультатПроверки.Сценарий = 1;
	КонецЕсли;
