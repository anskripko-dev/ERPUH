		...
		ПоставляемыеНастройкиСервер.ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Обработка поставляемой настройки %1 не требуется, т.к. настройка
					...
					|en = 'Processing of the %1 built-in setting is not required, as the setting
					|see implementation of the ОбновлятьАвтоматически method is set to False details
					|ПоставляемыеНастройкиПереопределяемый.ПриДобавленииНастроек
					| and ИнтеграцияПодсистемБИП.ПриДобавленииНастроек.'"),
		...
		ПоставляемыеНастройкиСервер.ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Данные поставляемой настройки %1 сохранены в кэше.';
					|en = 'Data of the %1 built-in setting is saved to cache.'"),
				ОписаниеФайла.Идентификатор),
			Ложь);
	КонецЕсли;
	...
	ПоставляемыеНастройкиСервер.ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Начало обработки Файл поставляемой настройки %1.';
				|en = 'Processing of the %1 built-in setting file is started.'"),
			ОписаниеФайла.Идентификатор),
		Ложь);
		...
		Если Не Обработан Тогда
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось обработать поставляемые данные настройки:
					|Идентификатор: %1
					|Версия: %2';
					|en = 'Cannot process the default master data for the setting
					|ID: %1
					|Version: %2'"),
				ОписаниеФайла.Идентификатор,
	...
	ПоставляемыеНастройкиСервер.ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Завершена обработка файла поставляемой настройки %1.';
				|en = 'The %1 built-in setting file is processed.'"),
			ОписаниеФайла.Идентификатор),
		Ложь);
	...
	Если РезультатПолученияКэша = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось получить файл настройки %1 из кэша.';
				|en = 'Cannot get the %1 setting file from the cache.'"),
			ОписаниеФайла.Идентификатор);
	КонецЕсли;
		...
		Иначе
			...
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось обработать поставляемые данные настройки:
					...
					|Версия: %2
					|Область данных: %3';
					|en = 'Cannot process the default master data of the setting:
					|ID: %1
					|Version: %2
					|Data area: %3'"),
				ОписаниеФайла.Идентификатор,
	...
	Исключение
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось обработать данные поставляемой настройки:
				...
				|An exception is thrown after the ПриЗагрузкеФайлаНастройки handler is finished:
				|%4'"),
			ОписаниеФайла.Идентификатор,
			ОписаниеФайла.Версия,
			ОбластьДанных,
