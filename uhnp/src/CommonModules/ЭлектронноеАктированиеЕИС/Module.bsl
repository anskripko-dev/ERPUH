			...
			Иначе
				...
				ТекстСообщения =
					НСтр("ru = 'ЭлектронноеАктированиеЕИС.ПриФормированияДанныхФайлаЭД: Не указана должность подписанта в справочнике подписантов ЕИС.';
						|en = 'Electronic UIS certification.When generating the EDI file data: The signer position is not specified in the catalog of UIS signers.'");
...
Функция ПолучитьДатуИзвещенияИзОткрытойЧастиЕИСНаСервере(НомерИзвещения)
	...
	Если НЕ РезультатЗагрузки.Статус Тогда
		ОписаниеОшибки = НСтр("ru = 'Электронный документооборот с ЕИС. При загрузке извещения с сервера ЕИС возникла ошибка:';
								|en = 'Электронный документооборот с ЕИС. При загрузке извещения с сервера ЕИС возникла ошибка:'", ОбщегоНазначения.КодОсновногоЯзыка())
