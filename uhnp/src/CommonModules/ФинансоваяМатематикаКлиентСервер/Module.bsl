...
Функция СтруктураПараметровРасчетаПроцентов() Экспорт
	...
	Результат.Вставить("ПорядокОпределенияСтавки", 0);
	Результат.Вставить("ГраницаПериодаПоследнийДеньМесяца", Ложь);
...
Функция СуммаСложныхПроцентовЗаПериод(ДатаНачала, ДатаОкончания, БазоваяСумма, Ставка, БазаДляРасчета = Неопределено) Экспорт
	
	Возврат БазоваяСумма * (Pow(1 + Ставка, КоэффициентДлительностиПериода(ДатаНачала, ДатаОкончания, БазаДляРасчета)) - 1);
...
Функция КоэффициентДлительностиПериода(ДатаНачала, ДатаОкончания, БазаДляРасчета = Неопределено)
	...
	ИначеЕсли БазаДляРасчета = ПредопределенноеЗначение("Справочник.СпособыРасчетаДнейПериодическихОпераций.ГодФактМесяц30") Тогда   
		
		Если ГодНачала = ГодОкончания Тогда
			
			ДнейВГоду = ДеньГода(КонецГода(ДатаНачала));
			
			Коэффициент = (30*(Месяц(ДатаОкончания) - Месяц(ДатаНачала)) + (1 + День(ДатаОкончания) - День(ДатаНачала))) / ДнейВГоду;
		...
		Иначе
			...
			Для ТекГод = ГодНачала По ГодОкончания Цикл
				...
				ДнейВГоду = ДеньГода(КонецГода(НачалоИнтервала));
				
				Коэффициент = Коэффициент + (30*(Месяц(ОкончаниеИнтервала) - Месяц(НачалоИнтервала)) + (1 + День(ОкончаниеИнтервала) - День(НачалоИнтервала))) / ДнейВГоду;
	...
	ИначеЕсли БазаДляРасчета = ПредопределенноеЗначение("Справочник.СпособыРасчетаДнейПериодическихОпераций.Год360МесяцФакт") Тогда
		...
		Коэффициент = ((ДатаОкончания - ДатаНачала)/86400 + 1) / 360;

	ИначеЕсли БазаДляРасчета = ПредопределенноеЗначение("Справочник.СпособыРасчетаДнейПериодическихОпераций.Год365МесяцФакт") Тогда
		
		Коэффициент = ((ДатаОкончания - ДатаНачала)/86400 + 1) / 365;
		
	ИначеЕсли БазаДляРасчета = ПредопределенноеЗначение("Справочник.СпособыРасчетаДнейПериодическихОпераций.Год360Месяц30") Тогда
		
		Коэффициент = (360 * (Год(ДатаОкончания) - Год(ДатаНачала)) + 30*(Месяц(ДатаОкончания) - Месяц(ДатаНачала)) + (1 + День(ДатаОкончания) - День(ДатаНачала)))/360
