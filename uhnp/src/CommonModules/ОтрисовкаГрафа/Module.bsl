...
Функция ОтобразитьГрафическуюСхему(КонтекстСхемы, ПолеФормы)
	...
	Исключение
		...
		Ошибка = НСтр("ru = 'Отрисовка графа: Не удалось прочитать сохраненную схему из файла <%1> по причине:%2 %3'");
		Ошибка = СтрШаблон(Ошибка, ТекущееИмяФайла, Символы.ПС, ОписаниеОшибки());
		ОбщегоНазначенияУХ.СообщитьОбОшибке(Ошибка);
