...
Процедура ОбработатьЗначенияАналитик(Форма, ОписаниеСтатьи, ДанныеВидАналитики, ДанныеАналитика, Действия) Экспорт
	...
	Для ИндексАналитики = 1 По ОписаниеСтатьи.КолвоАналитик Цикл
		...
		Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеАналитика, ПолеАналитики) Тогда
			Продолжить;
		КонецЕсли;
...
Функция ПолучитьПараметрыЛимитированияСтатьиБюджета(Форма, ИмяЭлемента) Экспорт
	...
	Для Каждого СвязьПараметровВыбора Из СвязиПараметров Цикл
		...
		Если СвязьПараметровВыбора.Имя = "Отбор.ПараметрыЛимитирования_ВидБюджета" Тогда
			Значение = ПолучитьРеквизитФормыПоПути(Форма, СвязьПараметровВыбора.ПутьКДанным);
...
Процедура ИнициализироватьСвязиАналитикСтатьиБюджета(Форма, ИмяЭлемента, ЭтоИзменениеЗначенияАналитики) Экспорт
	...
	Если ОписаниеСтатьи.ЭтоТаблица Тогда
		...
		Если ТекущаяСтрока = неопределено Тогда
			Возврат;
		КонецЕсли;
		Таблица = ПолучитьРеквизитФормыПоПути(Форма, ОписаниеСтатьи.ПутьКДанным);
	...
	Иначе
		...
		КонецЕсли;
		ЗначениеСтатьи = ПолучитьЗначениеПоПутиКДанным(Форма, ПутьКДанным);
		ДанныеВидАналитики	= ПолучитьРеквизитФормыПоПути(Форма, ОписаниеСтатьи.Аналитики.ВидАналитикиПуть);
		ДанныеАналитика		= ПолучитьРеквизитФормыПоПути(Форма, ОписаниеСтатьи.Аналитики.АналитикаПуть);
	КонецЕсли;
	
	//
	НастроитьСвязиАналитик(Форма, ОписаниеСтатьи, ДанныеВидАналитики, ДанныеАналитика);
...
Функция ВидЭлементовУправленияПериодов() Экспорт

	Возврат ВидКнопкиФормы.Гиперссылка; 

КонецФункции // ВидЭлементовУправленияПериодов()
...
Процедура ОчиститьАналитикуСтатьиПриИзмененииАналитикиСтатьи(ОписаниеСтатьи, ДанныеАналитика, ДанныеВидАналитики, НомерАналитики)
	...
	Для Индекс = НомерАналитики + 1 По КоличествоАналитикСтатьи() Цикл
		...
		Если ТребуетсяОчистка(СвязиАналитик, НомерАналитики) Тогда
			...
			Если НЕ ТипЗначенияАналитики = ТипЗнч(Неопределено) Тогда
