...
//
Функция ЗаменитьСпецификации(Замены, КонтролироватьЗначения = Ложь) Экспорт
...
Процедура ПриОпределенииНастроекРегламентныхЗаданий(Настройки) Экспорт
	...
	Настройка = Настройки.Добавить();
	Настройка.РегламентноеЗадание = Метаданные.РегламентныеЗадания.СверткаРегистраСтруктураЗаказа;
	Настройка.ФункциональнаяОпция = Метаданные.ФункциональныеОпции.ДинамическаяСтруктураЗаказовНаПроизводство;
	
	Настройка = Настройки.Добавить();
	Настройка.РегламентноеЗадание = Метаданные.РегламентныеЗадания.ОчисткаИсторииЗагрузкиВидовРабочихЦентров;
	Настройка.ФункциональнаяОпция = Метаданные.ФункциональныеОпции.ДинамическаяСтруктураЗаказовНаПроизводство;
...
Процедура СверткаРегистровРегл() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.СверткаРегистраСтруктураЗаказа);
...
Процедура УстановитьНастройкиРеглЗаданияСверткаРегистраСтруктураЗаказа(ПараметрыЗадания)
	
	Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		
		Отбор = Новый Структура();
		Отбор.Вставить("Метаданные", Метаданные.РегламентныеЗадания.СверткаРегистраСтруктураЗаказа.Имя);
		...
		Если Задания.Количество() <> 1 Тогда
			ЗаписьЖурналаРегистрации(
				СтруктураЗаказа.ИмяСобытияЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.РегламентныеЗадания.СверткаРегистраСтруктураЗаказа,,
...
Функция ИспользоватьРеглЗаданиеСверткаРегистраСтруктураЗаказа()
...
Процедура УстановитьИспользованиеРеглЗаданияСверткаРегистраСтруктураЗаказа() Экспорт
	
	ПараметрыЗадания = Новый Структура;
	ПараметрыЗадания.Вставить("Использование", ИспользоватьРеглЗаданиеСверткаРегистраСтруктураЗаказа());
	УстановитьНастройкиРеглЗаданияСверткаРегистраСтруктураЗаказа(ПараметрыЗадания);
	
КонецПроцедуры

Процедура УстановитьРасписаниеРеглЗаданияСверткаРегистраСтруктураЗаказа(РасписаниеРегламентногоЗадания) Экспорт
...
#Область ОчисткаИсторииЗагрузкиВидовРабочихЦентров

Процедура ОчиститьИсториюЗагрузкиВидовРабочихЦентровРегламент() Экспорт
	
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОчисткаИсторииЗагрузкиВидовРабочихЦентров);
	
	Если НЕ ИспользоватьРеглЗаданиеОчисткаИсторииЗагрузкиВидовРабочихЦентров() Тогда
		Возврат
	КонецЕсли;
	
	РегистрыНакопления.ДоступностьВидовРабочихЦентров.ОчиститьИсториюЗагрузкиВидовРабочихЦентров();
	
КонецПроцедуры

Процедура УстановитьНастройкиРеглЗаданияОчисткаИсторииЗагрузкиВидовРабочихЦентров(ПараметрыЗадания)
	
	Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		
		Отбор = Новый Структура();
		Отбор.Вставить("Метаданные", Метаданные.РегламентныеЗадания.ОчисткаИсторииЗагрузкиВидовРабочихЦентров.Имя);
		...
		Если Задания.Количество() <> 1 Тогда
			ЗаписьЖурналаРегистрации(
				...
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.РегламентныеЗадания.ОчисткаИсторииЗагрузкиВидовРабочихЦентров,,
				НСтр("ru = 'Не найдено регламентное задание: Очистка истории загрузки видов рабочих центров';
					|en = 'The scheduled job is not found: Clear load history of work center types'"));
			Возврат;
		КонецЕсли;
...
Функция ИспользоватьРеглЗаданиеОчисткаИсторииЗагрузкиВидовРабочихЦентров()
	
	НаборКонстант = Константы.СоздатьНабор("ДинамическаяСтруктураЗаказовНаПроизводство, МетодикаУправленияПроизводством");
	НаборКонстант.Прочитать();
	
	Возврат НаборКонстант.ДинамическаяСтруктураЗаказовНаПроизводство И УправлениеПроизводством.ИспользуетсяГрафикПроизводства(НаборКонстант);

КонецФункции

Процедура УстановитьИспользованиеРеглЗаданияОчисткаИсторииЗагрузкиВидовРабочихЦентров() Экспорт
	
	ПараметрыЗадания = Новый Структура;
	ПараметрыЗадания.Вставить("Использование", ИспользоватьРеглЗаданиеОчисткаИсторииЗагрузкиВидовРабочихЦентров());
	УстановитьНастройкиРеглЗаданияОчисткаИсторииЗагрузкиВидовРабочихЦентров(ПараметрыЗадания);
...
Процедура ВыполнитьПересчетРасчетВнутриЗамераВремени(ПараметрыДействия)
	...
	ТекстыЗапросов.Добавить(ТекстЗапросаПартии);
	ТекстыЗапросов.Добавить(ТекстЗапросаЗадания);
	...
	РезультатЗапросаПартии = РезультатЗапроса[0];
	РезультатЗапросаЗадания = РезультатЗапроса[1];
	ЕстьГрафик = НЕ РезультатЗапроса[2].Пустой();
	
	ТекстыЗапросов.Очистить();
	ТекстыЗапросов.Добавить("УНИЧТОЖИТЬ Партии");
	ТекстыЗапросов.Добавить(ТекстЗапросаПартии);
	ТекстыЗапросов.Добавить(ТекстЗапросаЗадания);
	Запрос.Текст = СтрСоединить(ТекстыЗапросов, ОбщегоНазначения.РазделительПакетаЗапросов());
	
	Пока Не РезультатЗапросаПартии.Пустой() Цикл
		
		РегистрыСведений.ЗаданияКРасчетуСтруктурыЗаказаРаспределениеЗапасов.ДобавитьЗаданияВСледующуюОчередь(РезультатЗапросаЗадания.Выгрузить(), ПараметрыДействия.Очередь);
		
		Если ЕстьГрафик Тогда
			РегистрыСведений.ГрафикЭтаповПроизводства2_2.ОчиститьГрафикИЗагрузкуПартии(МенеджерВременныхТаблиц);
		КонецЕсли;
		
		УдалитьПартииИзСтруктурыПоКлючамНаборов(РезультатЗапросаПартии.Выгрузить());
		
		РезультатЗапроса = Запрос.ВыполнитьПакетСПромежуточнымиДанными();
		РезультатЗапросаПартии = РезультатЗапроса[1];
		РезультатЗапросаЗадания = РезультатЗапроса[2];
	...
	Пока Не
		...
		РегистрыСведений.ЗаданияКРасчетуСтруктурыЗаказаРаспределениеЗапасов.ДобавитьЗаданияВСледующуюОчередь(РезультатЗапроса.Выгрузить(), ПараметрыДействия.Очередь);
		
		РезультатЗапроса = Запрос.Выполнить();
		
	КонецЦикла;
	
	РегистрыСведений.ЗаданияКРасчетуСтруктурыЗаказаГрафикПроизводства.ДобавитьЗадания(ПараметрыДействия.ПорцияРасчета);
...
// Рассчитывает задания к изменению структуры заказов после изменения очереди заказов.
//
// Параметры:
...
//
Процедура ОбновитьСтруктуруЗаказаПриИзмененииМестаВОчереди(
			Задания,
	...
	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("ЗаказыНаПроизводство", Задания.ВыгрузитьКолонку("Объект"));
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|
	|	Движения.Номенклатура   КАК Номенклатура,
	...
	|		И Движения.ЗаказНаПроизводство <> ISNULL(Движения.Назначение.Заказ, ЗНАЧЕНИЕ(Документ.ЗаказНаПроизводство2_2.ПустаяСсылка))
	|		И Движения.ВидСтроки В (
	|				ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.Потребление),
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		РегистрыСведений.ЗаданияКРасчетуСтруктурыЗаказаРаспределениеЗапасов.ДобавитьЗадания(РезультатЗапроса.Выгрузить());
...
Процедура ВыполнитьДействияВКонцеТактаРасчета(ГраницаРасчета) Экспорт
	...
	Если УправлениеПроизводствомПовтИсп.ИспользуетсяГрафикПроизводства() Тогда
		...
		Если Не РезультатЗапроса.Пустой() Тогда
			ГрафикПроизводстваСтруктурыЗаказа.ЗапуститьРасчетГрафикаПартий(ГраницаРасчета);
		КонецЕсли;
	Иначе
