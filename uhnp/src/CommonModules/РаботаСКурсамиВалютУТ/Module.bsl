		...
		И (ПараметрыВариантаКурсаДоговора.ВалютаВзаиморасчетов = ТекущаяВалюта 
			...
			Если ТекущаяВалюта <> НоваяВалюта Тогда
				Если ТекущаяВалюта = ПараметрыВариантаКурсаДоговора.ВалютаВзаиморасчетов Тогда
					...
					КурсТекущейВалюты = РегистрыСведений.КурсыВалютРасчетовПоДоговорам.ПолучитьЗначенияКурсаВалютыДоговораУХ(ПараметрыВариантаКурсаДоговора.Договор, Дата,ТекущаяВалюта,ПараметрыВариантаКурсаДоговора.ВалютаВзаиморасчетов);
					#КонецОбласти    
				...
				Если НоваяВалюта = ПараметрыВариантаКурсаДоговора.ВалютаВзаиморасчетов Тогда
					...
					КурсНовойВалюты = РегистрыСведений.КурсыВалютРасчетовПоДоговорам.ПолучитьЗначенияКурсаВалютыДоговораУХ(ПараметрыВариантаКурсаДоговора.Договор, Дата, НоваяВалюта, ПараметрыВариантаКурсаДоговора.ВалютаВзаиморасчетов);
					#КонецОбласти    
...
Процедура ЗаполнитьКурсДокументаПоУмолчанию(КурсЧислитель, КурсЗнаменатель, ВалютаДокумента, ВалютаВзаиморасчетов, Организация, Дата = Неопределено, Договор = Неопределено, ОбъектРасчетов = Неопределено) Экспорт
	...
	ИначеЕсли ВалютаДокумента = ВалютаРеглУчета И НЕ ВалютаВзаиморасчетов = ВалютаРеглУчета Тогда
		...
		Если ВариантКурсаДоговора = Перечисления.ВариантыКурсаДоговора.УстановленныйВДоговоре Тогда
			...
			СтруктураКурса = РегистрыСведений.КурсыВалютРасчетовПоДоговорам.ПолучитьЗначенияКурсаВалютыДоговораУХ(Договор, Дата, ВалютаДокумента, ВалютаВзаиморасчетов);
			#КонецОбласти
	...
	Иначе
		...
		Если ВариантКурсаДоговора = Перечисления.ВариантыКурсаДоговора.УстановленныйВДоговоре Тогда
			...
			КурсВалютыВзаиморасчетов = РегистрыСведений.КурсыВалютРасчетовПоДоговорам.ПолучитьЗначенияКурсаВалютыДоговораУХ(Договор, Дата, ВалютаДокумента, ВалютаВзаиморасчетов);
			#КонецОбласти
...
Функция ПараметрыВариантаКурсаДоговора() Экспорт
	...
	#Область УХ_Встраивание
	СтруктураПараметров.Вставить("ВариантКурсаДоговораУХ", Перечисления.ВариантыКурсаДоговора.Переменный);
	#КонецОбласти
...
Функция ТекстЗапросаКурсовПоОбъектуРасчетов()
	...
	#Область УХ_Встраивание
	ТекстЗапроса = ВзаиморасчетыВстраиваниеУХ.ЗаменитьТекстЗапроса_РаботаСКурсамиВалютУТ_ТекстЗапросаКурсовПоОбъектуРасчетов(ТекстЗапроса);
	#КонецОбласти
