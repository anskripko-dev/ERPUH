...
Функция СформироватьДанныеЗаполненияОплаты(МассивСсылок, ФормаОплаты, Направление, РаспределениеОплаты, МассивСсылокСРаспоряжениями) Экспорт
	...
	Если МассивСсылокСРаспоряжениями.Количество() > 1 Тогда
		ТекстЗапроса = ТекстЗапроса +  ОбщегоНазначения.РазделительПакетаЗапросов() + ТекстЗапросаВозможностьГруппировкиЗаявок();
	КонецЕсли;
	...
	Если ФормаОплаты = "Безналичная" Тогда
		Запрос.УстановитьПараметр("ТипМестаОплаты", ОбщегоНазначенияОПК.ТипОпределяемогоТипа(Метаданные.ОпределяемыеТипы.БанковскийСчетОрганизации)); 
	ИначеЕсли ФормаОплаты = "Наличная" Тогда
		Запрос.УстановитьПараметр("ТипМестаОплаты", ОбщегоНазначенияОПК.ТипОпределяемогоТипа(Метаданные.ОпределяемыеТипы.Касса));
	КонецЕсли;
...
Функция ИспользоватьФормуПомощникаСписанияНаЛицевыеСчета(МассивСсылок)Экспорт
	
	Если МассивСсылок.Количество() Тогда
		
		Ссылка = МассивСсылок[0];
		
		МетаданныеСсылка = Ссылка.Метаданные();
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ВидОперацииУХ", МетаданныеСсылка) И ОбщегоНазначения.ЕстьРеквизитОбъекта("ФормаОплатыЗаявки", МетаданныеСсылка) Тогда
			РеквизитыЗаявки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "ВидОперацииУХ,ФормаОплатыЗаявки");
			
			Если РеквизитыЗаявки.ВидОперацииУХ = Справочники.ВидыОперацийУХ.ПеречислениеПодотчетномуЛицу И РеквизитыЗаявки.ФормаОплатыЗаявки = Перечисления.ФормыОплаты.Безналичная Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
