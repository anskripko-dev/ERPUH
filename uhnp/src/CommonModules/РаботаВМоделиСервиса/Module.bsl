...
Процедура ПриДобавленииПараметровРаботыКлиента(Параметры) Экспорт
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Если ЗначениеЗаполнено(КодАбонента) Тогда
			ДопИнфо.Добавить(СтрШаблон(НСтр("ru = 'абонент %1';
											|en = 'Subscriber %1'"), Формат(КодАбонента, "ЧН=0; ЧГ=0")));
		КонецЕсли;
		
		КодПриложения = ЗначениеРазделителяСеанса();
		Если ЗначениеЗаполнено(КодПриложения) Тогда
			ДопИнфо.Добавить(СтрШаблон(НСтр("ru = 'приложение %1';
											|en = 'Application %1'"), Формат(КодПриложения, "ЧН=0; ЧГ=0")));
		КонецЕсли;
		...
		Исключение
			...
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Не удалось снять монопольную блокировку информационной базы.
					 |
					 |%1';
					 |en = 'Cannot remove the exclusive infobase lock.
					 |
					 |%1'"),
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Монопольная блокировка';
					|en = 'Exclusive lock'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				ТекстОшибки);
