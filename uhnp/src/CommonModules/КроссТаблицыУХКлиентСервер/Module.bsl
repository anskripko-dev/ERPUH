...
//
Процедура ЗаполнитьТаблицуИзКроссТаблицы(Объект, ОписаниеКТ, КроссТаблица, ТаблицаРасшифровки, Периоды, Таблица = Неопределено) Экспорт
...
Процедура РассчитатьИтогиКолонокИОбщие(ОписаниеКТ, Форма, РассчитыватьСтроки = Истина) Экспорт
	...
	Для Каждого КлючЗначение Из ОписаниеКТ.Схема.Поля Цикл
		...
		Если ТипЗнч(ДанныеКТ) = Тип("Массив") Тогда
		...
		Иначе
			...
			Для каждого СтрокаПериода Из АктивныеПериоды Цикл
				//Итоги.Вставить(ПрефиксДляКолонки + СтрокаПериода.ИмяКолонки, 0);
				Итог = ДанныеКТ.Итог(ОписаниеПоля.ПрефиксРеквизитаКолонки + СтрокаПериода.ИмяКолонки);
				Итоги.Вставить(ПрефиксДляКолонки + СтрокаПериода.ИмяКолонки, Итог);
				Итого = Итого + Итог;
			КонецЦикла;
			Итоги.Вставить(ИмяИтогПоля, Итого);
		КонецЕсли;
...
Процедура Старый_РассчитатьИтогиКолонокИОбщие(ОписаниеКТ, Форма, РассчитыватьСтроки = Истина) Экспорт
...
Процедура РассчитатьИтогиПоСтрокеКТ(ОписаниеКТ, СтрокаКТ, АктивныеПериоды) Экспорт
...
Функция ПолучитьИмяГруппы_Измерение(ОписаниеКТ, ИмяАналитики) Экспорт  
	
	Строка = ОписаниеКТ.Схема.СтруктураАналитик[ИмяАналитики];
	Группа = СтрЗаменить(ВРег(Строка.ГруппаСтрок), " ", "");
	ИмяЭлементаГруппы = ОписаниеКТ.Элементы.КроссТаблица + "ГруппаИзмерений" + Группа;

	Возврат ИмяЭлементаГруппы;
...
Процедура УстановитьВидимостьКолонкиРесурса(Форма, ОписаниеКТ, ОписаниеРесурса, ВидимостьКолонкиКТ) Экспорт
	...
	Для Каждого СтрокаПериод Из ТаблицаПериодов Цикл
		...
		ИмяГруппы = ПолучитьИмяЭлементаГруппаПоПериоду(ОписаниеКТ, СтрокаПериод);
...
Процедура УстановитьВидимостьКолонкиПоказателя(Форма, ОписаниеКТ, ОписаниеПоказателя, ВидимостьКолонкиКТ) Экспорт
	...
	Для Каждого СтрокаПериод Из ТаблицаПериодов Цикл
		...
		ИмяГруппы = ПолучитьИмяЭлементаГруппаПоПериодуПоказателю(ОписаниеКТ, СтрокаПериод, ОписаниеПоказателя);
