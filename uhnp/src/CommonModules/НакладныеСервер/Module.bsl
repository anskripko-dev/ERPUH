...
Функция ТекстЗапросаСостоянияПередач(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению)
	...
	|			И РаспоряженияНаОтгрузку.Склад        = Распоряжения.Склад
	|		ИМЕЮЩИЕ
	|			СУММА(РаспоряженияНаОтгрузку.КПередаче) < 0)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ЗаказКлиента,
	|	Склад
...
Функция ТекстЗапросаСостоянияОтчетовДавальцам2_5(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению)
	...
	|	(ВЫБРАТЬ
	|		Остатки.Распоряжение                                                 КАК ЗаказКлиента,
	|		ВЫРАЗИТЬ(Остатки.Распоряжение КАК Документ.ЗаказДавальца2_5).Договор КАК Договор,
	|		Остатки.Номенклатура                                                 КАК Номенклатура,
	|		Остатки.Характеристика                                               КАК Характеристика,
	|		Остатки.КодСтроки                                                    КАК КодСтроки,
	|		Остатки.Склад                                                        КАК Склад,
	|		Остатки.Серия                                                        КАК Серия,
	|		Остатки.КОформлениюОборот                                           КАК КОформлению
	|	ИЗ
	|		РегистрНакопления.РаспоряженияНаОтгрузку.Обороты(,,, &УсловиеОтбораОстатков И КодСтроки <> 0) КАК Остатки
	|	ГДЕ
	|		Остатки.КОформлениюОборот > 0
	|		И НЕ Остатки.Распоряжение.Ссылка ЕСТЬ NULL // для отработки RLS
	|		И НЕ Остатки.Склад.Ссылка ЕСТЬ NULL
...
Функция ТекстЗапросаСостоянияОтчетовДавальцам2_5ОстаткиБезЗаказа() Экспорт
	...
	|
	|	ВЫБРАТЬ
	|		ВЫРАЗИТЬ(Обороты.Распоряжение КАК Документ.ЗаказДавальца2_5).Договор КАК Договор,
	...
	|	ИЗ
	|		РегистрНакопления.РаспоряженияНаОтгрузку.Обороты(,,, &УсловиеОтбораОстатков) КАК Обороты
	|
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтгрузкаТоваровСХранения.Товары КАК Товары
	|			ПО Обороты.Распоряжение = Товары.ЗаказКлиента
