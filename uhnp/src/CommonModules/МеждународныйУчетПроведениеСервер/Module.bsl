...
Процедура ВернутьДокументыКОтражению(ДокументыКОтражению) Экспорт
	...
	Если МенеджерВременныхТаблиц <> Неопределено Тогда
		Отбор = Новый Структура("Регистратор", Неопределено);
		Пока Истина Цикл
			...
			Пока ВыборкаПоДокументам.Следующий() Цикл
				Отбор.Регистратор = ВыборкаПоДокументам.Регистратор;
...
Процедура ВыполнитьОффлайновыеРасчеты(Знач Организации = Неопределено, Знач ДатаОкончания = Неопределено, Знач Документы = Неопределено) Экспорт
	...
	Если ДокументыКРасчету.Количество() > 0 Тогда
		...
		ПараметрыВыполнения.ПересчитатьРегистрыОборотов = Истина;
...
Функция ПроводкиРегламентнойОперации(ДанныеОперации) Экспорт
	...
	УдалитьПроводкиБезСуммИКоличеств(ПараметрыФормированияПроводок, ТаблицаПроводок);
...
Процедура ВыключитьИтогиМеждународныхРегистров(ПараметрыОтраженияВМеждународномУчете)
	...
	Если МинимальныйПериодИтогов > ПараметрыОтраженияВМеждународномУчете.МинимальныйПериодИтогов Тогда
		ПараметрыОтраженияВМеждународномУчете.МинимальныйПериодИтогов = МинимальныйПериодИтогов;
	КонецЕсли;
	
	Если МаксимальныйПериодИтогов > ПараметрыОтраженияВМеждународномУчете.МаксимальныйПериодИтогов Тогда
		ПараметрыОтраженияВМеждународномУчете.МаксимальныйПериодИтогов = МаксимальныйПериодИтогов;
	ИначеЕсли МаксимальныйПериодИтогов < ПараметрыОтраженияВМеждународномУчете.МаксимальныйПериодИтогов Тогда
		...
		Для НомерПопытки = 1 По КоличествоПопытокОтключенияИтогов Цикл
			Попытка
				...
				ИначеЕсли ИзменитьМинимальныйПериодИтогов Тогда
					РегистрыБухгалтерии.Международный.УстановитьМинимальныйПериодРассчитанныхИтогов(НачалоПериодаОтключения);
				ИначеЕсли ИзменитьМаксимальныйПериодИтогов Тогда
					РегистрыБухгалтерии.Международный.УстановитьМаксимальныйПериодРассчитанныхИтогов(КонецПериодаОтключения);
				КонецЕсли;
				ПараметрыОтраженияВМеждународномУчете.ВосстановитьСостояниеИтогов = Истина;
				Прервать;
			Исключение
			...
			Попытка
				Если ИзменитьМинимальныйПериодИтогов И ИзменитьМаксимальныйПериодИтогов Тогда
					РегистрыБухгалтерии.МеждународныйБезКорреспонденции.УстановитьМинимальныйИМаксимальныйПериодыРассчитанныхИтогов(
						НачалоПериодаОтключения, КонецПериодаОтключения);
				ИначеЕсли ИзменитьМинимальныйПериодИтогов Тогда
					РегистрыБухгалтерии.МеждународныйБезКорреспонденции.УстановитьМинимальныйПериодРассчитанныхИтогов(НачалоПериодаОтключения);
				ИначеЕсли ИзменитьМаксимальныйПериодИтогов Тогда
					РегистрыБухгалтерии.МеждународныйБезКорреспонденции.УстановитьМаксимальныйПериодРассчитанныхИтогов(КонецПериодаОтключения);
				КонецЕсли;
				ПараметрыОтраженияВМеждународномУчете.ВосстановитьСостояниеИтогов = Истина;
				Прервать;
			Исключение
...
Процедура ВключитьИтогиМеждународныхРегистров(ПараметрыОтраженияВМеждународномУчете)
	...
	Если ИспользованиеТекущихИтогов <> ПараметрыОтраженияВМеждународномУчете.ИспользованиеТекущихИтогов Тогда
	...
	ИспользованиеТекущихИтогов = РегистрыБухгалтерии.МеждународныйБезКорреспонденции.ПолучитьИспользованиеТекущихИтогов();
	...
	МинимальныйПериодИтогов = РегистрыБухгалтерии.Международный.ПолучитьМинимальныйПериодРассчитанныхИтогов();
	МаксимальныйПериодИтогов = РегистрыБухгалтерии.Международный.ПолучитьМаксимальныйПериодРассчитанныхИтогов();
	
	ИзменитьМинимальныйПериодИтогов = МинимальныйПериодИтогов < ПараметрыОтраженияВМеждународномУчете.МинимальныйПериодИтогов;
	ИзменитьМаксимальныйПериодИтогов = МаксимальныйПериодИтогов < ПараметрыОтраженияВМеждународномУчете.МаксимальныйПериодИтогов;
	...
	Если ИзменитьМинимальныйПериодИтогов ИЛИ ИзменитьМаксимальныйПериодИтогов Тогда
		Для НомерПопытки = 1 По КоличествоПопытокВосстановленияИтогов Цикл
			Попытка
				...
				ИначеЕсли ИзменитьМинимальныйПериодИтогов Тогда
					РегистрыБухгалтерии.Международный.УстановитьМинимальныйПериодРассчитанныхИтогов(
						ПараметрыОтраженияВМеждународномУчете.МинимальныйПериодИтогов);
				ИначеЕсли ИзменитьМаксимальныйПериодИтогов Тогда
					РегистрыБухгалтерии.Международный.УстановитьМаксимальныйПериодРассчитанныхИтогов(
						ПараметрыОтраженияВМеждународномУчете.МаксимальныйПериодИтогов);
				КонецЕсли;
				Прервать;
			Исключение
	...
	МинимальныйПериодИтогов = РегистрыБухгалтерии.МеждународныйБезКорреспонденции.ПолучитьМинимальныйПериодРассчитанныхИтогов();
	МаксимальныйПериодИтогов = РегистрыБухгалтерии.МеждународныйБезКорреспонденции.ПолучитьМаксимальныйПериодРассчитанныхИтогов();
			...
			Попытка
				Если ИзменитьМинимальныйПериодИтогов И ИзменитьМаксимальныйПериодИтогов Тогда
					РегистрыБухгалтерии.МеждународныйБезКорреспонденции.УстановитьМинимальныйИМаксимальныйПериодыРассчитанныхИтогов(
						...
						ПараметрыОтраженияВМеждународномУчете.МаксимальныйПериодИтогов);
				ИначеЕсли ИзменитьМинимальныйПериодИтогов Тогда
					РегистрыБухгалтерии.МеждународныйБезКорреспонденции.УстановитьМинимальныйПериодРассчитанныхИтогов(
						ПараметрыОтраженияВМеждународномУчете.МинимальныйПериодИтогов);
				ИначеЕсли ИзменитьМаксимальныйПериодИтогов Тогда
					РегистрыБухгалтерии.МеждународныйБезКорреспонденции.УстановитьМаксимальныйПериодРассчитанныхИтогов(
						ПараметрыОтраженияВМеждународномУчете.МаксимальныйПериодИтогов);
				КонецЕсли;
				Прервать;
			Исключение
...
Процедура ДобавитьПроводкиСторно(ПараметрыФормированияПроводок, ТаблицаПроводок)
	...
	|	-ЕСТЬNULL(Международный.ВалютнаяСуммаДт, 0) КАК ВалютнаяСуммаДт,
	|	-ЕСТЬNULL(Международный.КоличествоДт, 0) КАК КоличествоДт,
	|	Международный.ПодразделениеКт КАК ПодразделениеКт,
	|	Международный.ВалютаКт КАК ВалютаКт,
	|	Международный.НаправлениеДеятельностиКт КАК НаправлениеДеятельностиКт,
	|	Международный.СчетКт КАК СчетКт,
	...
	|	-ЕСТЬNULL(Международный.ВалютнаяСуммаКт, 0) КАК ВалютнаяСуммаКт,
	|	-ЕСТЬNULL(Международный.КоличествоКт, 0) КАК КоличествоКт,
	|	-Международный.Сумма КАК Сумма,
	|	-Международный.СуммаПредставления КАК СуммаПредставления,
	|	Международный.Содержание КАК Содержание,
	|	Международный.ШаблонПроводки КАК ШаблонПроводки,
	...
	|	-ВЫБОР
	|		КОГДА Международный.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияБухгалтерии.Дебет)
	|			ТОГДА ЕСТЬNULL(Международный.ВалютнаяСумма, 0)
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ВалютнаяСуммаДт,
	|	-ВЫБОР
	|		КОГДА Международный.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияБухгалтерии.Дебет)
	|			ТОГДА ЕСТЬNULL(Международный.Количество, 0)
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК КоличествоДт,
	|	ВЫБОР
	|		КОГДА Международный.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияБухгалтерии.Дебет)
	...
	|		КОГДА Международный.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияБухгалтерии.Дебет)
	|			ТОГДА 0
	|		ИНАЧЕ ЕСТЬNULL(Международный.ВалютнаяСумма, 0)
	|	КОНЕЦ КАК ВалютнаяСуммаКт,
	|	-ВЫБОР
	|		КОГДА Международный.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияБухгалтерии.Дебет)
	|			ТОГДА 0
	|		ИНАЧЕ ЕСТЬNULL(Международный.Количество, 0)
	|	КОНЕЦ КАК КоличествоКт,
	|	-Международный.Сумма КАК Сумма,
	|	-Международный.СуммаПредставления КАК СуммаПредставления,
	|	Международный.Содержание КАК Содержание,
...
Процедура УдалитьПроводкиБезСуммИКоличеств(ПараметрыФормированияПроводок, ТаблицаПроводок)
	
	ПроводкиКУдалению = Новый Массив;
	
	Для каждого Проводка Из ТаблицаПроводок Цикл
		Если Проводка.Статус = Перечисления.СтатусыОтраженияВМеждународномУчете.ОтраженоВУчете
		 ИЛИ Проводка.Статус = Перечисления.СтатусыОтраженияВМеждународномУчете.КОтражениюВУчетеВручную Тогда
			ВсеРесурсыРавныНулю = Истина;
			
			Для Каждого ИмяРесурса Из ПараметрыФормированияПроводок.БалансовыеРесурсы Цикл
				Если Проводка[ИмяРесурса] <> 0 Тогда
					ВсеРесурсыРавныНулю = Ложь;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			
			Если НЕ ВсеРесурсыРавныНулю Тогда
				Продолжить;
			КонецЕсли;
			
			Для Каждого ИмяРесурса Из ПараметрыФормированияПроводок.НебалансовыеРесурсы Цикл
				Если Проводка[ИмяРесурса + "Дт"] <> 0 ИЛИ Проводка[ИмяРесурса + "Кт"] <> 0 Тогда
					ВсеРесурсыРавныНулю = Ложь;
					Прервать;
				КонецЕсли;
			КонецЦикла;
...
Функция ДанныеДляВозвратаКОтражениюВУчете(ДокументыКОтражению, Отбор)
	...
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ДанныеРегистра.Период, ДокументыКОтражению.Период) КАК Период,
	|	ЕСТЬNULL(ДанныеРегистра.Регистратор, ДокументыКОтражению.Регистратор) КАК Регистратор,
	|	ЕСТЬNULL(ДанныеРегистра.ПланСчетов, НЕОПРЕДЕЛЕНО) КАК ПланСчетов,
	|	ЕСТЬNULL(ДанныеРегистра.Организация, ДокументыКОтражению.Организация) КАК Организация,
	|	ЕСТЬNULL(ДанныеРегистра.ДатаОтражения, ДокументыКОтражению.ДатаОтражения) КАК ДатаОтражения,
	|	ЕСТЬNULL(ДанныеРегистра.Статус, ЗНАЧЕНИЕ(Перечисление.СтатусыОтраженияВМеждународномУчете.КОтражениюВУчете)) КАК Статус,
	|	ЕСТЬNULL(ДанныеРегистра.НастройкаФормированияПроводок, НЕОПРЕДЕЛЕНО) КАК НастройкаФормированияПроводок,
	|	ЕСТЬNULL(ДанныеРегистра.Комментарий, """") КАК Комментарий
	...
	|	ПО ОтражениеДокументовВМеждународномУчете.Организация = ПланыСчетовМеждународногоУчетаОрганизаций.Организация
	...
	|		ИЛИ ПланыСчетовМеждународногоУчетаОрганизаций.Настройка = ЗНАЧЕНИЕ(Справочник.НастройкиФормированияПроводокМеждународногоУчета.ПустаяСсылка))
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Период,
	|	Регистратор,
	...
	|	И НЕ (ПланыСчетовМеждународногоУчетаОрганизаций.Настройка ЕСТЬ NULL
	|		ИЛИ ПланыСчетовМеждународногоУчетаОрганизаций.Настройка = ЗНАЧЕНИЕ(Справочник.НастройкиФормированияПроводокМеждународногоУчета.ПустаяСсылка))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	ДанныеРегистра.Индексы.Добавить("Регистратор");
