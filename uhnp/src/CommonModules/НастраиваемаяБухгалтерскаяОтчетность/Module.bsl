...
Процедура НастроитьКомандыГенерацииНастроек(Форма) Экспорт
	...
	Если ГруппаКнопок <> Неопределено Тогда
		...
		Для Каждого РедакцияПравил Из РедакцииПравил Цикл
			...
			ИмяКоманды = СтрШаблон(НСтр("ru = 'ФормаСгенерироватьНастройкиФорма%1';
										|en = 'ФормаСгенерироватьНастройкиФорма%1'"), РедакцияПравил.РедакцияФормы);
...
// Переопределяет запрос встроенной отчетности СоздатьДенежныеПотоки().
//
// Параметры:
...
//
Процедура ПереопределитьДенежныеПотокиДДС(Процессор, Запрос) Экспорт
	
	// В настраиваемой отчетности для 57.03 по-умолчанию ВидДДС ПоступлениеОплатыОтКлиента
	Если Процессор.ДанныеАлгоритма.Свойство("ПереопределяемыеСчета") Тогда
		Запрос.Текст =
		"ВЫБРАТЬ
		...
		Для Каждого ЭлементПоказателя Из НайденныеСтрокиПоВидуДДС Цикл
			...
			Иначе
				...
				СообщениеОшибка = СтрШаблон(НСтр("ru = 'Неверно задана формула для функции %1.';
												|en = 'Incorrect formula is specified for the %1 function.'"), ИмяФункции);
	...
	Если ФормулаРасчета = "ДО" Тогда
		Если СвернутыйНДС > 0 Тогда
			...
			НаименованиеСоставляющей = НСтр("ru = 'Сумма НДС начисленного к уплате при расчетах с покупателями';
											|en = 'VAT amount to pay under customer AR/AP'");
			ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбласти, ПолноеНаименованиеСтроки,
				"+", НаименованиеСоставляющей,
			...
			НаименованиеСоставляющей = НСтр("ru = 'Сумма НДС начисленного к возмещению при расчетах с поставщиками';
											|en = 'VAT amount to reimburse under vendor AR/AP'");
			ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбласти, ПолноеНаименованиеСтроки,
				"-", НаименованиеСоставляющей,
			...
			НаименованиеСоставляющей = СтрШаблон(НСтр("ru = 'Сумма НДС перечисленного в бюджет (Оборот с кредита счета 51 в дебет счета 68.02, по виду движения денежных средств %1)';
														|en = 'The amount of VAT transferred to the budget (Turnover from credit account 51 to debit account 68.02 by the %1 cash flow kind)'"),
			...
			НаименованиеСоставляющей = СтрШаблон(НСтр("ru = 'Сумма НДС полученного из бюджета (Оборот с кредита счета 68.02 в дебет счета 51, по виду движения денежных средств %1)';
														|en = 'The amount of VAT received from the budget (Turnover from credit account 68.02 to debit account 51 by the %1 cash flow kind)'"),
		...
		Если ЕстьВалютоОбменныеОперации Тогда
			...
			Если ЗначениеПоказателяУсловное >= 0 Тогда
				НаименованиеСоставляющей = НСтр("ru = 'Сумма прочих платежей по текущим операциям в части валютно-обменных операций';
												|en = 'The amount of other payments under the current foreign exchange transactions'");
				Если ВыгодаПотеряВалютногоОбмена > 0 Тогда
					ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбласти, ПолноеНаименованиеСтроки,
						"-", НаименованиеСоставляющей,
			...
			Иначе
				...
				НаименованиеСоставляющей = НСтр("ru = 'Сумма прочих поступлений по текущим операциям в части валютно-обменных операций';
												|en = 'The amount of other receipts under the current foreign exchange transactions'");
				ДобавитьСтрокуРасшифровки(ТаблицаРасшифровки, ИмяОбласти, ПолноеНаименованиеСтроки,
					"-", НаименованиеСоставляющей,
	...
	Для Каждого СтрокаСчет Из Алгоритм.Счета Цикл
		...
		Для Каждого ЭлементПоказателя Из НайденныеСтрокиПоВидуДДС Цикл
			...
			НаименованиеСоставляющей = СтрШаблон(НСтр("ru = '%1 оборот по счету %2 /вид движения ""%3""/';
														|en = '%1 %2 account turnover / the ""%3"" cash flow kind/'"), НаименованиеОборота, СтрокаСчет.СчетКод, Строка(Алгоритм.ВидДвиженияДенежныхСредств));
			ТекущееЗначениеПоказателя = ТекущееЗначениеПоказателя + Слагаемое;
				...
				Для Каждого СтрокаНДС Из СтрокиНДС Цикл
					Если СтрокаНДС.ДО = 0 И СтрокаНДС.КО = 0 Тогда
						...
						Если СлагаемоеНДС < 0 Тогда
							...
							НаименованиеСоставляющей = СтрШаблон(НСтр("ru = '%1 оборот по счету %2 /вид движения ""%3""/ в части НДС';
																		|en = '%1 %2 account turnover / the ""%3"" cash flow kind/ in VAT'"), НаименованиеОборота, СтрокаСчет.СчетКод, Строка(Алгоритм.ВидДвиженияДенежныхСредств));
					...
					ИначеЕсли Алгоритм.ТипПоказателя = "ДО" И СтрокаНДС.ДО < 0 Тогда
						...
						НаименованиеСоставляющей = СтрШаблон(НСтр("ru = '%1 оборот по счету %2 /вид движения ""%3""/ в части денежных потоков комиссионеров или агентов';
																	|en = '%1 %2 account turnover / the ""%3"" cash flow kind/ in consignee or agent cash flows'"), НаименованиеОборота, СтрокаСчет.СчетКод, Строка(Алгоритм.ВидДвиженияДенежныхСредств));
			...
			Если ЭлементПоказателя.СчетДДС = Счет("51") Тогда
				Если Алгоритм.ТипПоказателя = "ДО" И ЭлементПоказателя.ВидДДС = ВидДДСПолучения Тогда
					...
					НаименованиеСоставляющей = СтрШаблон(НСтр("ru = 'Оборот с кредита счета 68.02 в дебет счета 51 /вид движения ""%1""/';
																|en = 'Turnover from credit account 68.02 to debit account 51 /""%1"" record type/'"), Строка(ЭлементПоказателя.ВидДДС));
				ИначеЕсли Алгоритм.ТипПоказателя = "КО" И Алгоритм.Вычитаемый И ЭлементПоказателя.ВидДДС = ВидДДСПеречисления Тогда
					Слагаемое = -РасчетыПоНДС.ПеречисленныйНДС;
					НаименованиеСоставляющей = СтрШаблон(НСтр("ru = 'Оборот с кредита счета 51 в дебет счета 68.02 /вид движения ""%1""/';
																|en = 'Turnover from credit account 51 to debit account 68.02 /""%1"" record type/'"), Строка(ЭлементПоказателя.ВидДДС));
				КонецЕсли;
				ТекущееЗначениеПоказателя = ТекущееЗначениеПоказателя + Слагаемое;
	...
	Если ФормулаРасчета = "ДО" Тогда
		...
		Если ЕстьВалютоОбменныеОперации Тогда
			...
			Иначе
				// Разница получилась отрицательной, она должна отразиться в строке 4129
				НаименованиеСоставляющей = НСтр("ru = 'Сумма прочих поступлений по текущим операциям в части валютно-обменных операций';
												|en = 'The amount of other incoming payments under the current foreign exchange transactions'");
				ПараметрыСтрокиРасшифровки = ПараметрыСтрокиРасшифровки();
				ПараметрыСтрокиРасшифровки.НаименованиеПоказателя = ПолноеНаименованиеСтроки;
				ПараметрыСтрокиРасшифровки.ЗнакОперации = "-";
				ПараметрыСтрокиРасшифровки.НаименованиеСлагаемого = НаименованиеСоставляющей;
...
Процедура ЗаполнитьОДДСПоВстроенномуАлгоритму(КонтекстВыполнения, Процессор)
				...
				ИначеЕсли ВыборкаДетальныеЗаписи.КодСтроки = "4111" Тогда
					...
					ЗаполнениеБухгалтерскойОтчетностиПравила.ДобавитьПоказательСвернутогоДенежногоПотокаВстроенная(Процессор,
						...
						НСтр("ru = 'Исключены суммы продаж в интересах комитента';
							|en = 'Sales amounts for the benefit of the consignor are excluded'"));
					УстановитьСчетаАлгоритмаОДДС(Процессор, ТекущийНаборСчетовДенежныеПотоки);
				ИначеЕсли ВыборкаДетальныеЗаписи.КодСтроки = "4119" Тогда
					...
					ЗаполнениеБухгалтерскойОтчетностиПравила.ДобавитьПоказательСвернутогоДенежногоПотокаВстроенная(Процессор,
						...
						НСтр("ru = 'Выгоды и потери от валютно-обменных операций';
							|en = 'Gains and losses from currency exchange transactions'"));
					УстановитьСчетаАлгоритмаОДДС(Процессор, ТекущийНаборСчетовДенежныеПотоки);
				...
				ИначеЕсли ВыборкаДетальныеЗаписи.КодСтроки = "4121" Тогда
					...
					ЗаполнениеБухгалтерскойОтчетностиПравила.ДобавитьПоказательСвернутогоДенежногоПотокаВстроенная(Процессор,
						...
						НСтр("ru = 'Исключены суммы приобретений в интересах комитента';
							|en = 'Purchase amounts for the benefit of the consignor are excluded'"));
						УстановитьСчетаАлгоритмаОДДС(Процессор, ТекущийНаборСчетовДенежныеПотоки);
	...
	Если КонтекстПоказателяНДС <> Неопределено Тогда
		...
		ЗаполнениеБухгалтерскойОтчетностиПравила.ДобавитьЗначениеСвернутогоДенежногоПотокаВстроенная(
			Процессор,
			"НДС",
			НСтр("ru = 'Налог на добавленную стоимость в составе расчетов с контрагентами';
				|en = 'Налог на добавленную стоимость в составе расчетов с контрагентами'", КодЯзыка));
...
Функция ПолучитьТаблицуДанныхПояснений5()
	...
	ДанныеКЗаполнению.Колонки.Добавить("ВидСчета", Новый ОписаниеТипов("ВидСчета"));
...
#Область ПоясненияДебиторскойКредиторскойЗадолженности

Функция ПолучитьДанныеЗаполненияЗадолженности(Организация, НачалоПериода, КонецПериода, МенеджерВременныхТаблиц)
	...
	|	ДанныеСтрок.Счет КАК Счет,
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА НЕ ХозрасчетныйВидыСубконто.ТолькоОбороты
	|				ТОГДА ХозрасчетныйВидыСубконто.НомерСтроки
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК ОстаточныеСубконто
	|ПОМЕСТИТЬ АПСчета
	|ИЗ
	|	ДанныеСтрок КАК ДанныеСтрок
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.Хозрасчетный.ВидыСубконто КАК ХозрасчетныйВидыСубконто
	|		ПО ДанныеСтрок.Счет = ХозрасчетныйВидыСубконто.Ссылка
	|ГДЕ
	|	ДанныеСтрок.СчетВид = ЗНАЧЕНИЕ(ВидСчета.АктивноПассивный)
	|	И НЕ ДанныеСтрок.Счет В
	|				(ВЫБРАТЬ
	...
	|		ПО ХозрасчетныйОстаткиИОбороты.Регистратор = Перевод.Регистратор
	|			И ХозрасчетныйОстаткиИОбороты.НомерСтроки = Перевод.НомерСтроки
	|		ЛЕВОЕ СОЕДИНЕНИЕ АПСчета КАК АПСчета
	|		ПО ХозрасчетныйОстаткиИОбороты.Счет = АПСчета.Счет
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	Итог.ВидЗадолженности КАК ВидЗадолженности,
	|	Итог.Счет КАК Счет,
	|	ЗНАЧЕНИЕ(ВидСчета.АктивноПассивный) КАК ВидСчета,
	|	МАКСИМУМ(Итог.НачальноеСальдо) КАК НачальноеСальдо,
	|	СУММА(Итог.Дебет) КАК Дебет,
	|	СУММА(Итог.Кредит) КАК Кредит,
	|	МАКСИМУМ(Итог.КонечноеСальдо) КАК КонечноеСальдо,
	...
	Запрос.Текст = 
		...
		|		ИНАЧЕ ОстаткиИОбороты.Счет.Вид
		|	КОНЕЦ КАК ВидСчета,
		|	ОстаткиИОбороты.СуммаНачальныйОстаток КАК НачальноеСальдо,
		|	ОстаткиИОбороты.СуммаОборотДт КАК Дебет,
		|	ОстаткиИОбороты.СуммаОборотКт КАК Кредит,
		...
		|		ПО ОстаткиИОбороты.Счет = ПереводЗадолженности.Счет
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВидыАПСчетов КАК ВидыАПСчетов
		|		ПО ОстаткиИОбороты.Счет = ВидыАПСчетов.Счет
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	РезервыПоСомнительнымДолгам.ВидЗадолженности,
		|	РезервыПоСомнительнымДолгам.Счет,
		|	РезервыПоСомнительнымДолгам.Счет.Вид,
		|	СУММА(РезервыПоСомнительнымДолгам.НачальноеСальдоКт),
		|	СУММА(РезервыПоСомнительнымДолгам.ОборотДт),
		|	0,
		|	СУММА(РезервыПоСомнительнымДолгам.КонечноеСальдоКт),
...
#Область ЗапросСтрок
	...
	Запрос.Текст = 
		...
		|	ДанныеСтрок.КодЯчейкиПрошлогоПериода КАК КодЯчейкиПрошлогоПериода,
		|	ДанныеСтрок.Счет КАК Счет,
		|	ДанныеСтрок.СчетВид КАК ВидСчета,
		|	ДанныеСтрок.СчетДолгосрочный КАК СчетДолгосрочный,
		|	НЕ СчетаРезервов.Счет ЕСТЬ NULL КАК СчетРезервов,
		|	ДанныеСтрок.ВидЗадолженности КАК ВидЗадолженности,
		|	ДанныеСтрок.Регламентированная КАК Регламентированная,
...
#КонецОбласти
	...
	ДанныеКЗаполнению = ПолучитьДанныеЗаполненияЗадолженности(
							Организация,
							НачалоПериодаОтчета,
							КонецПериодаОтчета,
							Запрос.МенеджерВременныхТаблиц);
	...
	Если ЗаполнятьПредыдущиеПериоды Тогда
		ДанныеКЗаполнениюПрошлыйПериод = ПолучитьДанныеЗаполненияЗадолженности(
											Организация,
											ДобавитьМесяц(НачалоПериодаОтчета, -12),
											КонецМесяца(ДобавитьМесяц(КонецПериодаОтчета, -12)),
											Запрос.МенеджерВременныхТаблиц);
...
Процедура ЗаполнитьСтрокуДебиторскойКредиторскойЗадолженности(КонтекстВыполнения, ДанныеЗаполнения, ДанныеСтроки, ПараметрыПоказателя)
	...
	Для Каждого СтрокаСчета Из ДанныеСтроки Цикл
		...
		Если СчетаРезультата.Количество()>0 Тогда
			...
			СчетВид = СчетаРезультата[0].ВидСчета;
			...
			Для Каждого СчетРезультата Из СчетаРезультата Цикл
				...
				// Делаем подсчет показателя
				Если СчетВид = ВидСчета.АктивноПассивный Тогда
		...
		Если СтрокаСчета.ВидЗадолженности = "Д" Тогда
			...
			Если Не СтрокаСчета.СчетРезервов Тогда
				...
				Если СчетВид = ВидСчета.Пассивный Тогда
					...
					Расшифровка.НаименованиеСлагаемого = СтрШаблон(
						НСтр("ru = 'Сальдо на начало периода по кредиту счета %1 развернутое по всем субконто';
							|en = 'Opening balance for account %1 credit expanded by all extra dimensions'"),
						СтрокаСчета.Счет);
				...
				Иначе
					...
					Расшифровка.ЗнакОперации = "+";
					Расшифровка.НаименованиеСлагаемого = СтрШаблон(
						НСтр("ru = 'Сальдо на начало периода по дебету счета %1 развернутое по всем субконто';
							|en = 'Opening balance for account %1 debit expanded by all extra dimensions'"),
						СтрокаСчета.Счет);
					Расшифровка.Сумма = СчетНачальноеСальдо;
					Расшифровка.ИмяРаздела = КонтекстВыполнения.ИмяРаздела;
					Расшифровка.Организация = КонтекстВыполнения.Организация;
				...
				Расшифровка.ЗнакОперации = "+";
				Расшифровка.НаименованиеСлагаемого = СтрШаблон(
					НСтр("ru = 'Оборот по дебету счета %1 развернутый по всем субконто';
						|en = 'Account %1 debit turnover expanded by all extra dimensions'"), СтрокаСчета.Счет);
				...
				Расшифровка.ЗнакОперации = "-";
				Расшифровка.НаименованиеСлагаемого = СтрШаблон(
					НСтр("ru = 'Оборот с кредита счета 91.01 по статьям субконто ""Прочие доходы и расходы"" с видом ""Штрафы, пени и неустойки к получению (уплате)"" и ""Проценты к получению (уплате)"" в дебет счета %1 развернутое по всем субконто';
						|en = 'Оборот с кредита счета 91.01 по статьям субконто ""Прочие доходы и расходы"" с видом ""Штрафы, пени и неустойки к получению (уплате)"" и ""Проценты к получению (уплате)"" в дебет счета %1 развернутое по всем субконто'"),
					СтрокаСчета.Счет);
				...
				Расшифровка.ЗнакОперации = "-";
				Расшифровка.НаименованиеСлагаемого = СтрШаблон(
					НСтр("ru = 'Оборот по дебету счета %1 по статьям субконто ""Виды платежей в бюджет (фонды)"" с видами ""Пени: доначислено/уплачено(самостоятельно)"", ""Пени: начислено/уплачено(по акту проверки)"" и ""Штраф: начислено/уплачено""';
						|en = 'Оборот по дебету счета %1 по статьям субконто ""Виды платежей в бюджет (фонды)"" с видами ""Пени: доначислено/уплачено(самостоятельно)"", ""Пени: начислено/уплачено(по акту проверки)"" и ""Штраф: начислено/уплачено""'"),
					СтрокаСчета.Счет);
				...
				Расшифровка.ЗнакОперации = "+";
				Расшифровка.НаименованиеСлагаемого = СтрШаблон(
					НСтр("ru = 'Оборот по кредиту счета %1 развернутый по всем субконто';
						|en = 'Account %1 credit turnover expanded by all extra dimensions'"), СтрокаСчета.Счет);
				...
				Расшифровка.ЗнакОперации = "-";
				Расшифровка.НаименованиеСлагаемого = СтрШаблон(
					НСтр("ru = 'Оборот с кредита счета %1 в дебет счета 91.02 по статьям субконто ""Прочие доходы и расходы"" с видом ""Списание дебеторской(кредиторской) задолженности""';
						|en = 'Оборот с кредита счета %1 в дебет счета 91.02 по статьям субконто ""Прочие доходы и расходы"" с видом ""Списание дебеторской(кредиторской) задолженности""'"),
					СтрокаСчета.Счет);
				...
				Расшифровка.ЗнакОперации = "+";
				Расшифровка.НаименованиеСлагаемого = СтрШаблон(
					НСтр("ru = 'Оборот по счету %1 в части реклассификации долгосрочных активов и обязательств';
						|en = 'Account %1 turnover in terms of reclassification of long-term assets and liabilities'"),
					СтрокаСчета.Счет);
				...
				Если СчетВид = ВидСчета.Пассивный Тогда
					...
					Расшифровка.ЗнакОперации = "-";
					Расшифровка.НаименованиеСлагаемого = СтрШаблон(
						НСтр("ru = 'Сальдо на конец периода по кредиту счета %1 развернутое по всем субконто';
							|en = 'Closing balance for account %1 credit expanded by all extra dimensions'"),
						СтрокаСчета.Счет);
				...
				Иначе
					...
					Расшифровка.ЗнакОперации = "+";
					Расшифровка.НаименованиеСлагаемого = СтрШаблон(
						НСтр("ru = 'Сальдо на конец периода по дебету счета %1 развернутое по всем субконто';
							|en = 'Closing balance for account %1 debit expanded by all extra dimensions'"),
						СтрокаСчета.Счет);
					Расшифровка.Сумма = СчетКонечноеСальдо;
					Расшифровка.ИмяРаздела = КонтекстВыполнения.ИмяРаздела;
					Расшифровка.Организация = КонтекстВыполнения.Организация;
			...
			Иначе
				...
				Расшифровка.ЗнакОперации = "+";
				Расшифровка.НаименованиеСлагаемого = СтрШаблон(
					НСтр("ru = 'Сальдо на начало периода по кредиту счета %1, развернутое по субконто ""Контрагенты"",""Договоры""';
						|en = 'Opening balance for credit of %1 account expanded by extra dimension ""Counterparties"", ""Contracts""'"),
					СтрокаСчета.Счет);
				...
				Расшифровка.ЗнакОперации = "+";
				Расшифровка.НаименованиеСлагаемого = СтрШаблон(
					НСтр("ru = 'Сальдо на конец периода по кредиту счета %1, развернутое по субконто ""Контрагенты"",""Договоры""';
						|en = 'Closing balance for credit of %1 account expanded by extra dimension ""Counterparties"", ""Contracts""'"),
					СтрокаСчета.Счет);
		...
		ИначеЕсли СтрокаСчета.ВидЗадолженности = "К" Тогда
			...
			Расшифровка.ЗнакОперации = "-";
			Расшифровка.НаименованиеСлагаемого = СтрШаблон(
				НСтр("ru = 'Оборот с кредита счета %1 в дебет счета 91.02 по статьям субконто ""Прочие доходы и расходы"" с видом ""Штрафы, пени и неустойки к получению (уплате)"" развернутое по всем субконто';
					|en = 'Оборот с кредита счета %1 в дебет счета 91.02 по статьям субконто ""Прочие доходы и расходы"" с видом ""Штрафы, пени и неустойки к получению (уплате)"" развернутое по всем субконто'"),
				СтрокаСчета.Счет);
			Расшифровка.Сумма = Мин(СчетШтраф, -СчетШтраф);
			Расшифровка.ИмяРаздела = КонтекстВыполнения.ИмяРаздела;
			Расшифровка.Организация = КонтекстВыполнения.Организация;
			...
			Расшифровка.ЗнакОперации = "-";
			Расшифровка.НаименованиеСлагаемого = СтрШаблон(
				НСтр("ru = 'Оборот с кредита счета %1 по статьям субконто ""Виды платежей в бюджет (фонды)"" с видами ""Пени: доначислено/уплачено(самостоятельно)"", ""Пени: начислено/уплачено(по акту проверки)"" и ""Штраф: начислено/уплачено""';
					|en = 'Оборот с кредита счета %1 по статьям субконто ""Виды платежей в бюджет (фонды)"" с видами ""Пени: доначислено/уплачено(самостоятельно)"", ""Пени: начислено/уплачено(по акту проверки)"" и ""Штраф: начислено/уплачено""'"),
				СтрокаСчета.Счет);
			Расшифровка.Сумма = Мин(СчетПени, -СчетПени);
			Расшифровка.ИмяРаздела = КонтекстВыполнения.ИмяРаздела;
			Расшифровка.Организация = КонтекстВыполнения.Организация;
			...
			Расшифровка.ЗнакОперации = "-";
			Расшифровка.НаименованиеСлагаемого = СтрШаблон(
				НСтр("ru = 'Оборот с кредита счета 91.01 в дебет счета %1 по статьям субконто  ""Прочие доходы и расходы"" с видом ""Списание дебеторской(кредиторской) задолженности""';
					|en = 'The turnover from account 91.01 credit to account %1 debit by items of the ""Incomes and expenses"" extra dimension with the ""Accounts payable crediting (receivable debiting)"" type'"),
				СтрокаСчета.Счет);
			Расшифровка.Сумма = Мин(СчетСписание, -СчетСписание);
			Расшифровка.ИмяРаздела = КонтекстВыполнения.ИмяРаздела;
			Расшифровка.Организация = КонтекстВыполнения.Организация;
...
Функция ПолучитьПараметрыПоказателя()
	...
	ПараметрыПоказателя.Вставить("ВыводитьДанныеНаКонецГода", Ложь);
...
Функция ПараметрыСтрок()
	...
	ПараметрыСтрок.Вставить("СчетчикДопСтрокФорма2025", Новый Соответствие);
...
Функция ЯчейкаОтчета(Строка, ПараметрыПоказателя)
...
Функция ОпределитьИмяОбластиПоказателя(ПараметрыПоказателя, КонтекстВыполнения)
	...
	Если ПараметрыПоказателя.УровеньСтроки = "0" Тогда
		...
		КонецЕсли;
	Иначе
		Если ЭтоДополнительнаяВписываемаяСтрока(ПараметрыПоказателя) Тогда
			ИмяОбластиПоказателя = БухгалтерскаяОтчетностьБРО.ИдентификаторЯчейки(
				СтрЗаменить(ПараметрыПоказателя.ПрефиксМногострочнойЧасти, "А", "Б"),
				ПараметрыПоказателя.КодГрафы)
		Иначе
			ИмяОбластиПоказателя = БухгалтерскаяОтчетностьБРО.ИдентификаторЯчейки(
				ПараметрыПоказателя.ПрефиксМногострочнойЧасти,
...
Функция ЗаполнитьПараметризируемыеНаименования(Наименование, КонтекстВыполнения, ПараметрыПоказателя = Неопределено)
	...
	Если ПараметрыПоказателя <> Неопределено И ПараметрыПоказателя.ВыводитьДанныеНаКонецГода Тогда
		ЗаголовокНачалоСравнительногоПериода = СтрШаблон(ЗаголовокШаблон, Формат(КонецГода(ДобавитьМесяц(КонтекстВыполнения.КонецПериодаОтчета,-2*12)), "ДФ = 'дд ММММ гггг'"));
	Иначе
...
Функция ЭтоДополнительнаяВписываемаяСтрока(ПараметрыПоказателя)
	
	Возврат ПараметрыПоказателя.УровеньСтроки = "А" И ПараметрыПоказателя.КодОсновнойСтроки <> "";
