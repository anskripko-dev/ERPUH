...
Процедура РешитьСЛУПлатформой_СебестоимостьПредприятия(ПараметрыРасчета)
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ЗагрузитьРешениеСЛУВоВременнуюТаблицу(
		...
		ТаблицаРешений,
		ПояснениеДляЗамера + НСтр("ru = ' (стоимость с НДС и без НДС)';
									|en = ' (cost incl. VAT and excl. VAT)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ЗагрузитьРешениеСЛУВоВременнуюТаблицу(
		...
		ПараметрыЗагрузкиСЛУ,
		ТаблицаРешений,
		ПояснениеДляЗамера + НСтр("ru = ' (трудозатраты)';
									|en = ' (labor costs)'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура РешитьСЛУПлатформой_СебестоимостьОрганизаций(ПараметрыРасчета)
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ЗагрузитьРешениеСЛУВоВременнуюТаблицу(
		...
		ПараметрыЗагрузкиСЛУ,
		ТаблицаРешений,
		ПояснениеДляЗамера + НСтр("ru = ' (стоимость регл и упр)';
									|en = ' (cost local and mgmt)'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура РешитьСЛУПлатформой_РезервыПредстоящихРасходов(ПараметрыРасчета)
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ЗагрузитьРешениеСЛУВоВременнуюТаблицу(
		...
		ПараметрыЗагрузкиСЛУ,
		ТаблицаРешений,
		ПояснениеДляЗамера + НСтр("ru = ' (постоянная разница)';
		...
		Если ЗначениеЗаполнено(СтрокаПротокола) Тогда
			...
			СтрокаПротокола =
				НСтр("ru = 'Стоимость некоторых аналитик номенклатуры не может быть рассчитана при решении СЛУ
					...
					|en = 'Cannot calculate the cost of some item dimensions when solving LES.
					|Reason: incorrect LES data (zero node weight and non-zero cost).'")
