...
Функция АналитикиБДДСПоЭлементуСтруктурыЗадолженности(ДоговорКонтрагента, ЭлементСтруктурыЗадолженности, НаправлениеДвижения) Экспорт
	...
	Если ТипЗнч(ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		...
		АналитикиБДДС = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДоговорКонтрагента, 
					"СтатьяДвиженияДенежныхСредств, АналитикаБДДС1,АналитикаБДДС2,АналитикаБДДС3, АналитикаБДДС4, АналитикаБДДС5,АналитикаБДДС6"); 
		Если ЗначениеЗаполнено(АналитикиБДДС.СтатьяДвиженияДенежныхСредств) Тогда
			Возврат АналитикиБДДС;		
		Иначе
			Возврат Неопределено;		
		КонецЕсли;
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		АналитикиБДДС = Новый Структура("СтатьяДвиженияДенежныхСредств, АналитикаБДДС1,АналитикаБДДС2,АналитикаБДДС3, АналитикаБДДС4, АналитикаБДДС5,АналитикаБДДС6"); 
		АналитикиБДДС.СтатьяДвиженияДенежныхСредств = Выборка.СтатьяБюджета;		
		Возврат АналитикиБДДС;
	КонецЕсли;
...
Процедура ДополнитьПараметрыЗаявкиВидомОперацииУХ(ПараметрыЗаявки)
	...
	Если Не ПараметрыЗаявки.Свойство("ВидОперацииУХ") Тогда
		...
		ИначеЕсли ПараметрыЗаявки.Свойство("ВидБюджета")
			И ПараметрыЗаявки.ВидБюджета = ОбщегоНазначения.ПредопределенныйЭлемент("ПланВидовХарактеристик.ВидыБюджетов.БюджетДвиженияДенежныхСредств") Тогда 	
			
			ВидОперацииУХ = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыОперацийУХ.ОплатаПоставщику");	
		Иначе
            ВидОперацииУХ = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыОперацийУХ.РасчетыСКонтрагентами");
		КонецЕсли;
