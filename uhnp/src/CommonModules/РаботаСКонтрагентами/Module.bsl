	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Получение данных юридического лица с ИНН %1';
				|en = 'Receive data of the legal entity with TIN %1'"),
			ИНН),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
	...
	Если ТелоОтветаОбъект.Получить("inn") = Неопределено Тогда
		Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось найти данные для юридического лица с ИНН %1.';
				|en = 'Cannot find data for the legal entity with TIN %1.'"),
			Результат.ИНН);
		ЗаписатьИнформациюВЖурналРегистрации(
			Результат.ОписаниеОшибки,
			ИдентификаторСервисаКонтрагент(),
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Успешно завершено получение данных юридического лица с ИНН %1';
				|en = 'Data of the legal entity with TIN %1 is received'"),
			ИНН),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Получение данных предпринимателя с ИНН %1';
				|en = 'Receive data of the entrepreneur with TIN %1'"),
			ИНН),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
	...
	Если ТелоОтветаОбъект.Получить("inn") = Неопределено Тогда
		Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось найти данные для предпринимателя с ИНН %1.';
				|en = 'Cannot find data for the entrepreneur with TIN %1.'"),
			Результат.ИНН);
		ЗаписатьИнформациюВЖурналРегистрации(
			Результат.ОписаниеОшибки,
			ИдентификаторСервисаКонтрагент(),
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Завершено получение данных предпринимателя с ИНН %1';
				|en = 'Data of the entrepreneur with TIN %1 is received'"),
			ИНН),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
...
Функция НаличиеПодразделенийПоИНН(ИНН) Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Синхронная проверка наличия подразделений по ИНН.';
			|en = 'Synchronous check for business unit availability by TIN.'"),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Успешно завершена синхронная проверка наличия подразделений по ИНН.';
			|en = 'Synchronous check for business unit availability by TIN is completed.'"),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Асинхронное получение сведений о подразделении юридического лица по уникальному номеру %1';
				|en = 'Asynchronous receipt of information about a legal entity business unit by unique number %1'"),
			УникальныйНомер),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Успешно завершено получение сведений о подразделении юридического лица по уникальному номеру %1';
				|en = 'Information about a legal entity business unit by unique number %1 is received'"),
			УникальныйНомер),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Асинхронное получение сведений о подразделении юридического лица по ИНН/КПП %1/%2';
				|en = 'Asynchronous receipt of information about a legal entity business unit by TIN/KPP %1/%2'"),
			ИНН,
			КПП),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
...
Процедура ПриДобавленииОписанийСервисовСопровождения(СервисыСопровождения) Экспорт
	...
	ОписательСервиса.Описание        = НСтр("ru = 'Быстрая проверка информации о контрагентах, автоматическое заполнение реквизитов контрагентов в различных документах, поиск контрагентов и другие полезные функции.';
											|en = 'Quick counterparty information check, automatic filling of counterparty details in documents, counterparty search, and other useful functions.'");
