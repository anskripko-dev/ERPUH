...
Функция ОписаниеСекцииГрафика() Экспорт
	...
	ОписаниеСекции.Вставить("КолонкаПриходКорректировка",     "");	// имя колонки с ручной корректировкой
	...
	ОписаниеСекции.Вставить("КолонкаПриходВстречная",          "");	
	...
	ОписаниеСекции.Вставить("КолонкаРасходКорректировка",     "");	// имя колонки с ручной корректировкой
	...
	ОписаниеСекции.Вставить("КолонкаРасходВстречная",         "");	
	...
	// Начальный остаток	
	ОписаниеСекции.Вставить("КолонкаПриходНачальныйОстатокОперация",        "");
	ОписаниеСекции.Вставить("КолонкаРасходНачальныйОстатокОперация",        "");
...
Процедура УстановитьТекстДлительность(ТекстДлительность, ДатаНачала, ДатаОкончания) Экспорт
	Если ЗначениеЗаполнено(ДатаНачала) И ЗначениеЗаполнено(ДатаОкончания) Тогда
		Длительность = Цел((ДатаОкончания - ДатаНачала)/86400);
...
Процедура УправлениеФормой_СчетаУчета(Форма, ЗаполнитьСчетаУчета = Неопределено, ЗаполнитьСчетВзаиморасчетов = Истина) Экспорт
	...
	Если ЭтоПараметры Тогда
		...
		Если НовыйИсточник <> ИсточникВидимостиСчетов Тогда
			...
			НовыеСчетаУчета = ФинансовыеИнструментыФормыВызовСервера
				.ПолучитьСчетаПоВидуФИ(ИсточникСчетов, Объект.Актив);
			
			Если НовыеСчетаУчета.Количество() = 0 И ЗначениеЗаполнено(Объект.ФИ) Тогда
				
				СчетаПоВиду = ФинансовыеИнструментыФормыВызовСервера
					.СчетаУчетаФиПоВидуФИ(Объект.ФИ, Объект.Актив);
					
				Если ЗначениеЗаполнено(СчетаПоВиду) Тогда
					НовыеСчетаУчета = СчетаПоВиду;
				КонецЕсли;
	...
	ИначеЕсли ЗаполнитьСчетаУчета Тогда // Форма документа с реквизитом "ФинансовыйИнструмент"
		...
		Если Не ЗначениеЗаполнено(ТекущийФИ) 
			И ЭтоДоговорФИ(КэшируемыеЗначения.ИмяДокумента) Тогда//Создание договора
			
			НовыеСчетаУчета = ФинансовыеИнструментыФормыВызовСервера.ПолучитьСчетаФИ(
				Объект.ВидДоговораУХ, КэшируемыеЗначения.ИмяДокумента, АктивПоИмениДокумента(Форма));
			
		Иначе 
			
			НовыеСчетаУчета = ФинансовыеИнструментыФормыВызовСервера.ПолучитьСчетаФИ(
				ТекущийФИ, КэшируемыеЗначения.ИмяДокумента, АктивПоИмениДокумента(Форма));
	...
	Если ЗаполнитьСчетВзаиморасчетов Тогда
		...
		ВидСчетВзаиморасчетов = ПредопределенноеЗначение("Справочник.ВидыСчетовФИ.СчетВзаиморасчетов");
		ПараметрыСчетВзаиморасчетов = КэшируемыеЗначения.СчетаУчета.Получить(ВидСчетВзаиморасчетов);
		
		Если ПараметрыСчетВзаиморасчетов <> Неопределено И ЗначениеЗаполнено(ПараметрыСчетВзаиморасчетов.Счет) Тогда
			
			//перенос уже заполненного счета взаиморасчетов
			НовыеСчетаУчета.Вставить(ВидСчетВзаиморасчетов, ПараметрыСчетВзаиморасчетов);
			
		ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "ДоговорКонтрагента")
			И ЗначениеЗаполнено(Объект.ДоговорКонтрагента) Тогда
			
			//договор заполнен ранее, но счет взаиморасчетов еще не заполнен
			
			РольСчета = ПолучитьРольСчета(Форма, Неопределено);
			Счет = ВстраиваниеУХВызовСервера.ПолучитьСчетВзаиморасчетов(
				Объект.ДоговорКонтрагента, РольСчета);
			ПараметрыСчетВзаиморасчетов = Новый Структура("Счет, Субконто1, Субконто2, Субконто3", Счет);
			
			НовыеСчетаУчета.Вставить(ВидСчетВзаиморасчетов, ПараметрыСчетВзаиморасчетов);
			
		ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "ДоговорАкцепта")
			И ЗначениеЗаполнено(Объект.ДоговорАкцепта) Тогда
			
			//договор заполнен ранее, но счет взаиморасчетов еще не заполнен
			РольСчета = ПолучитьРольСчета(Форма, Неопределено);
			Счет = ВстраиваниеУХВызовСервера.ПолучитьСчетВзаиморасчетов(
				Объект.ДоговорАкцепта, РольСчета);
			ПараметрыСчетВзаиморасчетов = Новый Структура("Счет, Субконто1, Субконто2, Субконто3", Счет);
	...
	//заменяем старые значения счетов и субконто на новые
	КэшируемыеЗначения.Вставить("СчетаУчета", НовыеСчетаУчета);
...
Процедура ЗаполнитьСчетаУчетаВерсии(Форма, ТекущийОбъект = Неопределено) Экспорт
	...
	ПеренестиСчетаВОбъект = ТипЗнч(Объект.Ссылка) <> Тип("СправочникСсылка.ПараметрыУчетаФИРСБУ")
		...
		И ТипЗнч(Объект.Ссылка) <> Тип("ДокументСсылка.ВерсияСоглашенияАккредитив")
		И ТипЗнч(Объект.Ссылка) <> Тип("ДокументСсылка.ВерсияСоглашенияБанковскаяГарантия");
	...
	Для каждого ТекущийСчет Из КэшируемыеЗначения.СчетаУчета Цикл
		...
		Если КэшируемыеЗначения.Свойство("ИменаСубконто") Тогда
			
			МСФОКлиентСерверУХ.УстановитьДоступностьСубконто(
				СтрокаСчет.Счет, СтрокаСчет, КэшируемыеЗначения.ИменаСубконто);

			МСФОКлиентСерверУХ.ОбновитьСубконтоСчета(
				Форма, "Счет", СтрокаСчет, "СчетаУчета", Ложь);
...
Функция ЭтоДоговорФИ(ИмяДокумента) Экспорт

	ИменаДокументов = "ВерсияСоглашенияКредит, ВерсияСоглашенияДепозит, ВерсияСоглашенияАккредитив, ВерсияСоглашенияБанковскаяГарантия";
	Возврат СтрНайти(ИменаДокументов, ИмяДокумента) > 0;
...
Функция ПолучитьТекущийФИ(Форма, ЭтоПараметры)
	...
	ИначеЕсли КэшируемыеЗначения.ИмяДокумента = "ВерсияСоглашенияКредит"
		Или КэшируемыеЗначения.ИмяДокумента = "ВерсияСоглашенияДепозит" 
		Или КэшируемыеЗначения.ИмяДокумента = "ВерсияСоглашенияАккредитив"
		Или КэшируемыеЗначения.ИмяДокумента = "ВерсияСоглашенияБанковскаяГарантия" Тогда
...
Процедура ЗаполнитьСчетВзаиморасчетовСПлательщиком(Форма, РольСчета = Неопределено) Экспорт	
	...
	Если Не ЗначениеЗаполнено(Договор) Тогда
		...
		Договор = Новый Структура(
			"ВалютаВзаиморасчетов, Организация, Подразделение, ГруппаФинансовогоУчета", 
			Объект.ВалютаНоминала, Объект.Организация, Объект.Подразделение, Неопределено);
		
	КонецЕсли;
	
	ЗаполнитьСчетПоВиду(Форма, "СчетВзаиморасчетовСПлательщиком",
		ВстраиваниеУХВызовСервера.ПолучитьСчетВзаиморасчетов(Договор, РольСчета));
