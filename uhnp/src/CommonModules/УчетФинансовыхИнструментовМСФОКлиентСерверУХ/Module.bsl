...
Процедура УправлениеФормой(Форма) Экспорт
	...
	ДатаНачалаПериодаОтчета = Форма.ДатаНачалаПериодаОтчета;
	ДатаОкончанияПериодаОтчета = Форма.ДатаОкончанияПериодаОтчета;
	
	ЕстьИзменениеГрафика = ЗначениеЗаполнено(Объект.ДатаПризнанияДоРекласса);
	ЭтоВыбытие = (Объект.ДатаОкончания <= Форма.ДатаОкончанияПериодаОтчета)
				И (Объект.ДатаОкончания >= Форма.ДатаНачалаПериодаОтчета);
	...
	Элементы.ОсновнойДолгОписание.Видимость 		= ЭтоЗадолженность;
	
	//тч ВНА
	Элементы.ГруппаВНА.Видимость	= ЭтоЛизинг;
	
	ВысотаСтрокиВНА = ?(Объект.ВНА.Количество() > 3, 1, 3);
	Элементы.ВНАВидОперации.Высота = ВысотаСтрокиВНА;
	Элементы.ВНАВНА.Высота = ВысотаСтрокиВНА;
	...
	Элементы.ДатаНачалаГрафика.Заголовок = ЗаголовокДатаНачалаГрафика(ЕстьИзменениеГрафика, ЭтоВыбытие);
		
	Элементы.РСП.ТолькоПросмотр = Не ПараметрыРасчета.ДоступнаСтавкаДисконтирования;
...
Функция ЗаголовокДатаНачалаГрафика(ЕстьИзменениеГрафика, ЭтоВыбытие) Экспорт
	
	ЗаголовокДатаНачала = НСтр("ru = 'Дата начала графика'");
	Если ЕстьИзменениеГрафика Тогда
		
		Если ЭтоВыбытие Тогда
			ЗаголовокДатаНачала = НСтр("ru = 'Дата изменения графика'");
		Иначе
			ЗаголовокДатаНачала = НСтр("ru = 'Дата изменения графика'");
		КонецЕсли;
...
Процедура ПодготовитьДанныеГрафика(ДанныеГрафика)
	...
	ДанныеГрафика.Вставить("ВключитьЭффектДляПервыхПроцентов",	ПараметрыУчетаФИ.ВидОбъектаФинансовогоХарактера.ВключитьЭффектДляПервыхПроцентов);
	...
	ДанныеГрафика.Вставить("ФВ",					ПолучитьФВ(ДанныеГрафика));
	...
	ДанныеГрафика.Вставить("ПериодичностьНачислений",	ПараметрыУчетаФИ.ВидОбъектаФинансовогоХарактера.ПериодичностьНачислений);
	
	БазаДляРасчетаПроцентов = ПараметрыУчетаФИ.ВидОбъектаФинансовогоХарактера.БазаДляРасчетаПроцентов;
	Если Не ЗначениеЗаполнено(БазаДляРасчетаПроцентов) Тогда
		БазаДляРасчетаПроцентов = ПредопределенноеЗначение("Справочник.СпособыРасчетаДнейПериодическихОпераций.ГодФактМесяцФакт");
	КонецЕсли;
	
	ДанныеГрафика.Вставить("БазаДляРасчетаПроцентов",	БазаДляРасчетаПроцентов);
	...
	Если ЗначениеЗаполнено(ДанныеГрафика.ВалютаФИ) 
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеГрафика.СтрокаФИ, "ВалютаФИ")
		И Не ЗначениеЗаполнено(ДанныеГрафика.СтрокаФИ.ВалютаФИ) Тогда
		ДанныеГрафика.СтрокаФИ.ВалютаФИ = ДанныеГрафика.ВалютаФИ;
	КонецЕсли;
...
Процедура ЗаполнитьДанныеГрафикаДо(ДанныеГрафика, СтруктураДействий)
	...
	Для каждого ИмяТЧ Из ИменаТЧ Цикл
		...
		Для ОбратныйИндекс = 1 По ЧислоСтрок Цикл
			...
			Если ЭтоДатаПредыдущейВерсии(СтрокаТаб.Дата, ДатаИзмененияГрафика, Ложь) Тогда
	...
	ДанныеГрафика.ГрафикМСФО.ЗагрузитьКолонку(
		ДанныеГрафика.ГрафикМСФО.ВыгрузитьКолонку("Проценты_Задолженность"),
		"Проценты_ЗадолженностьТочность");
		
	ДанныеГрафика.ГрафикМСФО.ЗагрузитьКолонку(
		ДанныеГрафика.ГрафикМСФО.ВыгрузитьКолонку("ЭСП_Проценты_Задолженность"),
		"ЭСП_Проценты_ЗадолженностьТочность");
...
Процедура СкорректироватьГраницыГрафика(ДанныеГрафика, СтруктураДействий)
	...
	ДанныеГрафика.СтрокаФИ.ДатаОкончания = Макс(ДатаОкончанияГрафика, ДанныеГрафика.СтрокаФИ.ДатаПризнания);
...
Процедура ЗаполнитьОсновнойДолгПоВНА(ДанныеГрафика, СтруктураДействий)
	...
	Для каждого СтрокаВНА Из ДанныеГрафика.ВНА Цикл
		...
		ИначеЕсли СтрокаВНА.ВидОперации = ПредопределенноеЗначение("Справочник.ВидыОпераций.ВозвратОбъектаЛизинга") Тогда
			...
			НоваяСтрока.Дата = СтрокаВНА.Дата;
			НоваяСтрока.ОсновнойДолг_Возврат = ВВалюту(СтрокаВНА.СуммаМСФО, ДанныеГрафика.ФВ, ДанныеГрафика.ВалютаФИ, СтрокаВНА.Дата);
			Если Не (ЕстьИзменениеГрафика И (СтрокаВНА.Дата = ДатаИзмененияГрафика)) Тогда// в дату изменения возврат доли
...
Процедура РассчитатьНачисленияПогашения(ДанныеГрафика, СтруктураДействий)
	...
	Если ЕстьИзменениеГрафика И Не ПризнатьНовыйФИ И ДанныеГрафика.ВключитьЭффектДляПервыхПроцентов Тогда
		ДанныеГрафика.Вставить("ВключитьЭффектПервойСтроке", Истина);
	КонецЕсли;
	...
	Для НомерСтроки = 0 По (ДанныеГрафика.ГрафикМСФО.Количество()-1) Цикл
		...
		ПогаситьПроценты(		ТекущаяСтрока, ПредыдущаяСтрока, ДенежныйПоток, ВозвратПоГрафику, СтавкаДисконтирования, ДанныеГрафика);
		...
		Если РассчитыватьДисконт Тогда
			...
			ПогаситьПроцентыЭСП(		ТекущаяСтрока, ПредыдущаяСтрока, ДенежныйПоток, ВозвратПоГрафику, ЭСП, ДанныеГрафика);
		...
		ДанныеГрафика.Удалить("ВключитьЭффектПервойСтроке");
...
Процедура ПогаситьПроценты(ТекущаяСтрока, ПредыдущаяСтрока, ДенежныйПоток, ВозвратПоГрафику, СтавкаДисконтирования, ДанныеГрафика)
	
	//Дисконт_Начисление - используется в случае изменения графика
	Долг = ПредыдущаяСтрока.ОсновнойДолг_Задолженность
		+ ПредыдущаяСтрока.Проценты_ЗадолженностьТочность
		- ТекущаяСтрока.Дисконт_Начисление;
		
	ДолгДляНачислений = Долг;
	Если ДанныеГрафика.Свойство("ВключитьЭффектПервойСтроке") И ДанныеГрафика.ВключитьЭффектПервойСтроке = Истина Тогда

		ИзменениеГрафика = ТекущаяСтрока.ДенежныйПоток;
		Если ДанныеГрафика.Лизинг И ДанныеГрафика.ВНА.Найти(ТекущаяСтрока.Дата, "Дата") <> Неопределено Тогда
			ИзменениеГрафика = ДанныеГрафика.ВНА.Найти(ТекущаяСтрока.Дата, "Дата").СуммаМСФО;
		КонецЕсли;
		ДолгДляНачислений = ДолгДляНачислений - ИзменениеГрафика;

	КонецЕсли;
		
	Проценты_НачислениеТочность = ПолучитьПроцентыЗаПериод(
		ДолгДляНачислений, СтавкаДисконтирования / 100, ТекущаяСтрока, ПредыдущаяСтрока, ДанныеГрафика);
...
Процедура ПогаситьПроцентыЭСП(ТекущаяСтрока, ПредыдущаяСтрока, ДенежныйПоток, ВозвратПоГрафику, ЭСП, ДанныеГрафика)
	
	Если ЭСП = 0 Тогда
		Возврат;//нет процентов по ЭСП
	КонецЕсли;

	Долг = ПредыдущаяСтрока.ЭСП_ОсновнойДолг_Задолженность
		+ ПредыдущаяСтрока.ЭСП_Проценты_ЗадолженностьТочность;
	...
	ЭСП_Проценты_НачислениеТочность = ПолучитьПроцентыЗаПериод(
		ДолгДляНачислений, ЭСП/100, ТекущаяСтрока, ПредыдущаяСтрока, ДанныеГрафика);
...
Процедура СкорректироватьОсновнойДолг(ДанныеГрафика, ТекущаяСтрока, ПредыдущаяСтрока)
	...
	Если ТекущаяСтрока.Проценты_Задолженность <> 0 Тогда
	...
	Если ТекущаяСтрока.ЭСП_Проценты_Задолженность <> 0 Тогда
...
Функция НачальнаяПредыдущаяСтрока(ДанныеГрафика, ВозвратПоГрафику, ЕстьИзменениеГрафика, ДатаИзмененияГрафика)
	...
	ДатаПризнания = ДанныеГрафика.СтрокаФИ.ДатаПризнания;
	...
	ИначеЕсли ЕстьИзменениеГрафика И ДанныеГрафика.ГрафикМСФО.Количество() Тогда // изменение старого графика 
		...
		ТекущаяСтрока = ДанныеГрафика.ГрафикМСФО.Найти(ДатаПризнания, "Дата");
		Если ТекущаяСтрока <> Неопределено Тогда
			ПредыдущаяСтрока = ДанныеГрафика.ГрафикМСФО.Получить(
				ДанныеГрафика.ГрафикМСФО.Индекс(ТекущаяСтрока)-1);
		ИначеЕсли ДатаПризнания < ДанныеГрафика.ГрафикМСФО[0].Дата Тогда 
			//изменение графика первая операция пересена на меньшую дату - редкость 
		Иначе
			
			ЧислоОпераций = ДанныеГрафика.ГрафикМСФО.Количество() - 1;
			Для й = 1 По ЧислоОпераций Цикл
				
				Если ДатаПризнания > ДанныеГрафика.ГрафикМСФО[й].Дата Тогда
					Продолжить;
				КонецЕсли;
				
				//й - первая операция позже даты изменения
				ПредыдущаяСтрока = ДанныеГрафика.ГрафикМСФО.Получить(й-1);
				Прервать;
...
Функция ПодготовитьСтрокиИзмененияВНА(ДанныеГрафика, ИзменениеОД)
	...
	БСВНА = ПолучитьБалансовуюСтоимостьВНА(ДанныеГрафика, ВНА, ИсключаемыеВНА);
		...
		Для каждого СтрокаВНА Из СтрокиВНА Цикл
			Если ВыбытиеТекущегоВНА Тогда
				...
				Если СтрокаВНА.ВидОперации = ПредопределенноеЗначение("Справочник.ВидыОпераций.ВозвратОбъектаЛизинга") Тогда
					...
					Если БСВНА.Количество() = 1 Тогда
						
						//Если 1 ВНА - в графике отражаем выбытие ЧПС, а при выбытии ВНА будет списана БС
						СтрокаВНА.СуммаМСФО = -ИзменениеОД;
...
Функция ПолучитьБалансовуюСтоимостьВНА(ДанныеГрафика, ВНА, ИсключаемыеВНА)
	...
	Если БСВНА.Количество() = 0 Тогда
		
		Шаблон = НСтр("ru = 'Объект <%1> не состоит на балансе на <%2>'");
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(Шаблон, СтрСоединить(ВНА, ", "), ДатаИзмененияГрафика), 
			Документ.Ссылка);
...
Функция ПолучитьПроцентыЗаПериод(ДенежныйПоток, Процент, ТекущаяСтрока, ПредыдущаяСтрока, ДанныеГрафика)
	
	Если ТекущаяСтрока.ЧислоДнейНачисления = 0 Тогда
		Возврат 0;
	КонецЕсли;
	
	ДатаНачала = ПредыдущаяСтрока.Дата + 86400;//ДатаНачала - первый день начисления процентов
	
	Возврат ФинансоваяМатематикаКлиентСервер.СуммаСложныхПроцентовЗаПериод(
		ДатаНачала,
		ТекущаяСтрока.Дата,
		ДенежныйПоток,
		Процент,
		ДанныеГрафика.БазаДляРасчетаПроцентов);
...
Функция РассчитыватьДисконт(ВидОбъектаФинансовогоХарактера, ДополнительныеРасходы)
	...
	ЭСП = ПредопределенноеЗначение("Перечисление.ВидыСтавокОбъектовФинансовогоХарактера.ЭффективнаяСтавка");
	СПД = ПредопределенноеЗначение("Перечисление.ВидыСтавокОбъектовФинансовогоХарактера.СтавкаПоДоговору");
	
	Если ВидОбъектаФинансовогоХарактера.ВидСтавкиРасчетаПроцентов = ЭСП Тогда
		Возврат Ложь;
	ИначеЕсли ВидОбъектаФинансовогоХарактера.ВидСтавкиРасчетаПроцентов = СПД Тогда
		Возврат ДополнительныеРасходы.Количество() > 0;
	Иначе // РСП
		Возврат Истина;
	КонецЕсли;
...
Процедура КэшироватьГрафикДоИзменения(ДанныеГрафика)
	...
	КэшПредыдущегоГрафика = УчетФинансовыхИнструментовМСФОСерверУХ.ПолучитьПараметрыПредыдущегоГрафика(ДанныеГрафика);
	...
	ДанныеГрафика.СтрокаФИ.ПараметрыУчетаФинансовогоИнструментаМСФО = КэшПредыдущегоГрафика.ПараметрыУчетаФИ;
	ДанныеГрафика.СтрокаФИ.ОстатокЗадолженностиНаНачалоПериодаМСФО	= КэшПредыдущегоГрафика.Задолженность;
	//ДанныеГрафика.СтрокаФИ.ВалютаФИ									= КэшПредыдущегоГрафика.Валюта;
...
Функция ВалютаФИ(ФИ)
	...
	ИначеЕсли ТипЗнч(ФИ) = Тип("СправочникСсылка.ЦенныеБумаги") Тогда
		Возврат ОбщегоНазначенияУХ.ПолучитьЗначениеРеквизита(ФИ, "ВалютаНоминала");
	Иначе	
		Возврат ОбщегоНазначенияУХ.ПолучитьЗначениеРеквизита(ФИ, "ВалютаВзаиморасчетов");
	КонецЕсли;

КонецФункции

Функция ПолучитьФВ(ДанныеГрафика)

	Если ДанныеГрафика.Свойство("ФВ") Тогда
		Возврат ДанныеГрафика.ФВ;
	КонецЕсли;

	Док = ДанныеГрафика.Документ;
	ФВ = МСФОВызовСервераУХ.ЗначениеУП("ВалютаУчета", 
		Док.Организация, Док.ПериодОтчета.ДатаОкончания, Док.Сценарий);
		
	ДанныеГрафика.Вставить("ФВ", ФВ);
	
	Если Не ЗначениеЗаполнено(ФВ) И ДанныеГрафика.ВНА.Количество() Тогда
		Сообщение = НСтр("ru = 'Не удалось найти учетную политику на дату <%1> (%2). Пересчет в функциональную валюту АФПП не возможен'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(Сообщение, Док.ПериодОтчета.ДатаОкончания, Док.Сценарий));
	КонецЕсли;
