...
Процедура УстановкаСтавкиНДС22() Экспорт
	...
	ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
		...
		НСтр("ru = 'Начат процесс замены Ставки НДС 20% на 22% с 01.01.2026г.';
			|en = 'Начат процесс замены Ставки НДС 20% на 22% с 01.01.2026г.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), 
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru = 'Закончено выполнение регламентной операции Замена ставки НДС 20% на 22% с 01.01.2026г.';
			|en = 'Закончено выполнение регламентной операции Замена ставки НДС 20% на 22% с 01.01.2026г.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсиюОсновныеСтавкиНДС(Параметры) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось обновить данные в регистре %ИмяРегистра% по причине: %Причина%';
								|en = 'Не удалось обновить данные в регистре %ИмяРегистра% по причине: %Причина%'");
...
Функция РассчитатьНДСЗаписиКнигиПокупокПоАвансуПолученному(ПараметрыРасчета)
	...
	|	ПО
	|		НДСАвансыПолученныеЗачеты.ДокументЗачетаАванса = СчетФактураВыданный.ДокументОснование
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|	
	|ВЫБРАТЬ РАЗЛИЧНЫЕ   // Корректировка реализации
	|	НДСАвансыПолученныеЗачеты.Организация          КАК Организация,
	|	НДСАвансыПолученныеЗачеты.ДокументЗачетаАванса КАК ДокументЗачетаАванса,
	|	СчетФактураВыданный.Ссылка                     КАК СчетФактура
	|ИЗ
	...
	|	ПО
	|		НДСАвансыПолученныеЗачеты.Организация = КорректировкаРеализации.Организация
	|		И НДСАвансыПолученныеЗачеты.ДокументЗачетаАванса = КорректировкаРеализации.Ссылка
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.СчетФактураВыданный.ДокументыОснования КАК СчетФактураВыданный
	|	ПО
...
Процедура ДополнитьИнформациейОРНПТ(Знач ДополнительнаяИнформация, Знач СтруктураПараметров, Запрос, ПоискРНПТПоОснованию = Ложь) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ОтчетПоКомиссииМеждуОрганизациями) Тогда
		...
		|
		|	ВЫБРАТЬ
		|		ОтчетПоКомиссииМеждуОрганизациямиТовары.Ссылка,
		|		ПериодыРозничныхПродаж.Документ
		|	ИЗ
		|		Документ.ОтчетПоКомиссииМеждуОрганизациями.ВидыЗапасов КАК ОтчетПоКомиссииМеждуОрганизациямиТовары
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПериодыРозничныхПродаж КАК ПериодыРозничныхПродаж
		|			ПО ОтчетПоКомиссииМеждуОрганизациямиТовары.Ссылка.Организация = ПериодыРозничныхПродаж.Организация
		|			И ОтчетПоКомиссииМеждуОрганизациямиТовары.Ссылка.Дата
		|				МЕЖДУ ПериодыРозничныхПродаж.НачалоПериода И ПериодыРозничныхПродаж.КонецПериода
		|	ГДЕ
		|		ОтчетПоКомиссииМеждуОрганизациямиТовары.Ссылка.Проведен
		|		И (НЕ ОтчетПоКомиссииМеждуОрганизациямиТовары.Покупатель.ЮрФизЛицо В (ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ЮрЛицо),
		|			ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ИндивидуальныйПредприниматель))
		|		ИЛИ ОтчетПоКомиссииМеждуОрганизациямиТовары.Покупатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка))
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		|	ВЫБРАТЬ
		...
		|	ГДЕ
		|		ОтчетПоКомиссииМеждуОрганизациямиТовары.Ссылка.Проведен
		|		И ОтчетПоКомиссииМеждуОрганизациямиТовары.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа),
		|			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга))
		|		И (НЕ ОтчетПоКомиссииМеждуОрганизациямиТовары.Покупатель.ЮрФизЛицо В (ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ЮрЛицо),
		|			ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ИндивидуальныйПредприниматель))
		|		ИЛИ ОтчетПоКомиссииМеждуОрганизациямиТовары.Покупатель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка))//ДополнительныеТаблицы");
		...
		Пока ВыборкаПоРНПТ.Следующий() Цикл 
			...
			Если ЗначениеЗаполнено(ВыборкаПоРНПТ.РегНомПросл) Тогда
				...
				Если СтруктураПараметров.ВерсияПостановленияНДС1137 >= 8 Тогда
					СтруктураСвРегНом.СтоимТовПросл=Окр(СтруктураСвРегНом.СтоимТовПросл,2);
				Иначе
