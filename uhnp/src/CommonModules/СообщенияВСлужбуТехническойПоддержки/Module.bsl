...
Функция ТекстТехническойИнформации() Экспорт
	...
	ШаблонТекста = НСтр("ru = 'Вид приложения: %1;
		...
		|Connection string: %3
		|Full access: %4
		|Administrator access rights: %5
		|Username for Online Support: %6
		|Password for Online Support: %7
		|Connection settings for Online Support:
		|	Domain zone: %8
		|Proxy settings: %9'");
...
Функция ОперацияОтправкиДанных(
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		Если РезультатОтправки.КодСостояния < 0 Тогда
			Результат.СообщениеОбОшибке = РезультатОтправки.СообщениеОбОшибке;
		Иначе
...
Функция ПереопределитьСообщениеПользователю(
	...
	ИначеЕсли КодОшибки = КодОшибкиПовторОтправкиЗапроса() Тогда
		Возврат НСтр("ru = 'Превышено количество попыток ввода отправки обращений. Повторите попытку позже.';
	...
	ИначеЕсли КодОшибки = КодОшибкиНеизвестнаяОшибкаСервиса() Тогда
		Возврат НСтр("ru = 'Не удалось подключиться к сервису. Сервис временно недоступен. Повторите попытку подключения позже.';
