		...
		Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Ссылка)) И Не ОбщегоНазначения.СсылкаСуществует(Ссылка)Тогда
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Объект типа ""%1"" не найден по идентификатору ""%2""';
					|en = 'The object of the ""%1"" type is not found by ID %2'"),
				ObjectID.type,
				ObjectID.ID);
			Если ВызыватьИсключение Тогда
				ВызватьИсключение ТекстСообщения;
	...
	Если ОтветНаЗапросыПакета.responses.Количество() = 2 Тогда
		...
		Если ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(Прокси, ДанныеДокумента, "DMError") Тогда
			...
			ИмяСобытия = НСтр("ru = 'Получение настроек доступности по состоянию объекта';
								|en = 'Получение настроек доступности по состоянию объекта'");
			...
			ТекстОшибки.Добавить(СтрШаблон(
				НСтр("ru = 'Не удалось получить настройки доступности по состоянию объекта ДО (ID = ""%1"", тип = ""%2"") по причине:';
					|en = 'Не удалось получить настройки доступности по состоянию объекта ДО (ID = ""%1"", тип = ""%2"") по причине:'"),
...
//   Булево
//
Функция ДоступенФункционалСозданияСвязей() Экспорт
	
	Возврат ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("3.0.19.29");
				...
				Если СтрокаФайл.ПриложениеТип = "DMIncomingElectronicDocument"
					...
					ПодходящийОбъект.ПредставлениеОбъектаИС = СтрШаблон(
						Нстр("ru = 'Отразить электронный документ ""%1"" в учете';
							|en = 'Record the ""%1"" electronic document in accounting'"),
...
//     * ДоступенФункционалСозданияСвязей - Булево - поддерживает ли веб-сервис Документооборота функционал
//         создания связей.
...
Функция ПредварительныеДанныеДляВыгрузкиВДО(Прокси, СписокОбъектовИС, ПодходящиеПравилаИнтеграции,
		ТребуетсяСписокВыражений = Истина, ТребуетсяСписокОбязательныхСвязей = Истина) Экспорт
	
	Данные = Новый Структура;
	Данные.Вставить("ДоступенФункционалФайлов", ДоступенФункционалФайлов());
	Данные.Вставить("ДоступенФункционалСозданияСвязей", ДоступенФункционалСозданияСвязей());
	...
	Если СписокОбъектовИС.Количество() = 0
			Или (Не ТребуетсяСписокВыгружаемыхТЧ
				...
				И Не Данные.ДоступенФункционалСозданияСвязей) Тогда
		Возврат Данные;
	КонецЕсли;
	...
	КонецЕсли;
	Если Данные.ДоступенФункционалСозданияСвязей Тогда
...
Функция СоздатьОбъектыДОПоПравилам(Прокси, СписокОбъектовИС, ПредварительныеДанные) Экспорт
	...
	Для Каждого ОбъектИС Из СписокОбъектовИС Цикл
		...
		Если ПредварительныеДанные.ДоступенФункционалСозданияСвязей Тогда
	...
	ШтрихкодУстановленПриСоздании =
		ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("3.0.19.29");
	...
	Если Форма.ТолькоВажныеСвязи Тогда
		...
		Форма.Элементы.ДекорацияТолькоВажныеТекст.Заголовок = СтрШаблон(
			НСтр("ru = 'Показаны только важные связи (%1). Чтобы увидеть все (%2), нажмите ""Показать все связи"".';
				|en = 'Only important links are shown (%1). To see all (%2), click ""Show all links"".'"),
	...
	Для Каждого ЭлементСоответствия Из ЗначенияКВыгрузке Цикл
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Не удалось выгрузить объект ""%1"" в 1С:Документооборот по причине:
				|%2';
				|en = 'Cannot export the ""%1"" object to 1C:Document Management due to:
				|%2'"),
		...
		Если Не ЗначениеЗаполнено(НастройкиАвторизации.ИмяПользователя) Тогда
			ВызватьИсключение
				НСтр("ru = 'Не установлено значение константы ""Имя пользователя для обмена с 1С:Документооборотом""';
					|en = 'The ""Username for exchange with 1C:Document Management"" constant value is not set.'");
		КонецЕсли;
...
Процедура ДобавитьОписаниеДопРеквизитов(СписокРеквизитовXDTO, СписокРеквизитов, Синонимы)
	...
	Для Каждого Реквизит Из СписокРеквизитов Цикл
		...
		РеквизитXDTO.synonym = СтрШаблон(НСтр("ru = 'Доп. реквизит: %1';
												|en = 'Additional attribute: %1'"), Реквизит.Наименование);
	...
	Исключение
		...
		Возврат ОписаниеОшибкиXDTO(
			НСтр("ru = 'Ошибка при обработке пакета сообщений';
				|en = 'An error occurred when processing the message package'"),
			ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Исключение
		...
		Возврат ОписаниеОшибкиXDTO(
			СтрШаблон(НСтр("ru = 'Ошибка при синхронизации пользователей ДО с пользователями %1';
							|en = 'An error occurred when syncing DI users with %1 users'"),
				ИнтеграцияС1СДокументооборотБазоваяФункциональность.СокращенноеНаименованиеКонфигурации()),
			ПолучитьОписаниеОшибки(ИнформацияОбОшибке()));
		...
		Если ПравилоИнтеграции.ТипОбъектаДО <> НастройкаСозданияСвязей.objectType.xdtoClassName Тогда
			ЗначенияКВыгрузке[ЗначениеРеквизитаИС].Добавить(СтрШаблон(
				НСтр("ru = 'Создание связи ""%1"". Тип объекта ДО ""%2"", указанный в правиле интеграции, не совпадает с типом объекта ДО ""%3"", указанным в настройке автоматического создания связей.';
					|en = 'Create the ""%1"" link. The ""%2"" DI object type specified in the integration rule does not match the ""%3"" DI object type specified in the automatic link creation setting.'"),
				ПредставлениеСвязи,
		...
		Если ПравилоИнтеграции.ВидДокументаДОID <> ВидДокументаДОИзНастройки Тогда
			ЗначенияКВыгрузке[ЗначениеРеквизитаИС].Добавить(СтрШаблон(
				НСтр("ru = 'Создание связи ""%1"". Вид документа ""%2"", указанный в правиле интеграции, не совпадает с видом документа ""%3"", указанным в настройке автоматического создания связей.';
					|en = 'Creating the ""%1"" link. The ""%2"" document type specified in the integration rule does not match the ""%3"" document type specified in the automatic links creation setting.'"),
				ПредставлениеСвязи,
	...
	Иначе
		ЗначенияКВыгрузке[ЗначениеРеквизитаИС].Добавить(СтрШаблон(
			НСтр("ru = 'Создание связи ""%1"". Не удалось определить подходящее правило интеграции. Количество найденных правил: %2.';
				|en = 'Create the ""%1"" link. Cannot identify a suitable integration rule. The quantity of found rules is %2.'"),
			ПредставлениеСвязи,
...
Процедура ЗаполнитьСписокСвязиКСозданию(Прокси, СвязиКСозданию, ОбъектИС, СписокАвтоматическиДобавляемыхСвязей)
	...
	ИмяСобытия = НСтр("ru = 'Автоматическое создание связанного объекта';
						|en = 'Автоматическое создание связанного объекта'");
				...
				Иначе
					ТекстОшибки = СтрШаблон(
						НСтр("ru = 'Не удалось выгрузить объект ""%1"" в 1С:Документооборот:
							|%2';
							|en = 'Не удалось выгрузить объект ""%1"" в 1С:Документооборот:
							|%2'"),
						ПодходящийОбъект.Ссылка,
