...
Процедура ОтразитьРасходыКУДиР(ПараметрыРасчета, Движения, Отказ) Экспорт
	...
	ЗаполнитьКУДиР2ПризнаннымиРасходами(ВременныеТаблицыРасходовНаОС, Движения);
					...
					Для каждого ЭлементКорректировки Из СтруктураКорректировки Цикл
						...
						Если ДанныеПлатежа["Доход"+ИмяКолонки] > 0 Тогда
							...
							Если ИмяКолонки = "УСН" И ДанныеПлатежа.ДоходНДС <> 0 И ДанныеПлатежа.ДоходУСНДляНДС <> 0 Тогда
								...
								ИначеЕсли ВыборкаПлатежей.СуммаНеПризнано <> 0 И ВыборкаПлатежей.СуммаНДСКомиссии <> 0 Тогда
									УменьшаемаяСуммаНДС = ВыборкаПлатежей.СуммаНДСКомиссии * УменьшаемаяСумма / ВыборкаПлатежей.СуммаНеПризнано;
								ИначеЕсли ВыборкаПлатежей.СуммаНеПризнано <> 0 Тогда // Поставка под принципиала
										УменьшаемаяСуммаНДС = ДанныеПлатежа.ДоходНДС - ВыборкаПлатежей.СуммаНДС;
								КонецЕсли;
				...
				Если ЕстьКорректировка Тогда
					...
					ИначеЕсли НеУчитыватьНДСВДоходах Тогда
						ДополнительнаяИнформация = НСтр("ru = 'Не признаны доходы в сумме НДС.';
														|en = 'Incomes in the VAT amount are not recognized.'", ОбщегоНазначения.КодОсновногоЯзыка());
					Иначе
...
Процедура ЗаполнитьЗаписьКУДиР(ЗаписьКУДиР, ДанныеЗаписи, ПризнаватьНДС)
	...
	Иначе
		ЗаписьКУДиР.Графа7 = ДанныеЗаписи.РасходыУСН - ?(ДанныеЗаписи.ПризнаватьРасходамиПриУСН , ДанныеЗаписи.РасходыНДС, 0);
		ЗаписьКУДиР.НДС = 0; 
	КонецЕсли;
...
Процедура ЗаполнитьКУДиР2ПризнаннымиРасходами(ВременнныеТаблицыРасходовНаОС, Движения)
	...
	ДанныеУчетнойПолитикиНДСПриУСН =ДанныеУчетнойПолитикиНДСПриУСН(ВременнныеТаблицыРасходовНаОС);
	...
	Для Каждого СтрокаРасходов Из ТаблицаРасходовКПризнанию Цикл
		...
		Отбор.Вставить("Организация", СтрокаРасходов.Организация);
		Отбор.Вставить("ДатаПринятияКУчету", НачалоМесяца(Макс(СтрокаРасходов.Графа3_ДатаВводаВЭксплуатацию, СтрокаРасходов.ДатаДокументаВозникновенияРасхода)));
		Строки = ДанныеУчетнойПолитикиНДСПриУСН.НайтиСтроки(Отбор);
		
		Если Строки.Количество() > 0 Тогда
			ПризнаватьНДС = Строки[0].ПризнаватьНДС;
		Иначе
			ПризнаватьНДС = Истина;
		КонецЕсли;
		...
		Если СтрокаРасходов.ПорядокУчета = ПорядокУчета.ВключитьВСоставАмортизируемогоИмущества
			...
			ЗаписьКУДиР.Графа5_ПервоначальнаяСтоимость = ЗаписьКУДиР.Графа5_ПервоначальнаяСтоимость - ?(ПризнаватьНДС, 0, СтрокаРасходов.НДС);
			
			Если СтрокаРасходов.Период = КонецГода(СтрокаРасходов.Период) Тогда
				ЗаписьКУДиР.Графа12_СуммаРасходовЗаОтчетнПериод = СуммаОплаты - ?(ПризнаватьНДС, 0,СуммаОплаты * СтрокаРасходов.НДС/СтрокаРасходов.СуммаРасходов);
			Иначе
				КварталовПрошло = Месяц(СтрокаРасходов.Период) / 3 - (4 - СтрокаРасходов.Графа9_КолКварталовВНалПериоде);
				ЗаписьКУДиР.Графа12_СуммаРасходовЗаОтчетнПериод = (СуммаПризнания - ?(ПризнаватьНДС, 0, ЗаписьРасхода.НДС)) * КварталовПрошло;
			КонецЕсли;
			ЗаписьКУДиР.Графа13_СуммаРасходовЗаКвартал = СуммаПризнания - ?(ПризнаватьНДС, 0, ЗаписьРасхода.НДС);
			ЗаписьКУДиР.Графа15_ОстатокРасходов = СуммаОплаты - ЗаписьКУДиР.Графа12_СуммаРасходовЗаОтчетнПериод - ?(ПризнаватьНДС, 0,СуммаОплаты * СтрокаРасходов.НДС/СтрокаРасходов.СуммаРасходов);
...
Функция РезультатЗапросаВыборкаРасходовНаОС(ВременныеТаблицыРасходовНаОС)
	...
	|	РасходыКПризнанию.ДатаВозникновения КАК ДатаДокументаВозникновенияРасхода,
...
Функция ТекстЗапросаПрочихРасходов()
	...
	|		ПО РасчетыСПоставщикамиПоСрокам.ДокументРегистратор = ДанныеРегистра.Регистратор
	|		И ДанныеРегистра.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КомиссияПоЭквайрингу)
	...
	|		И РасчетыСПоставщикамиПоСрокам.ДокументРегистратор ЕСТЬ NULL
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
...
Функция ДанныеУчетнойПолитикиНДСПриУСН(ВременныеТаблицыРасходовНаОС)
	...
	"ВЫБРАТЬ
	|	РасходыКПризнанию.Организация КАК Организация,
	|	РасходыКПризнанию.Графа3_ДатаВводаВЭксплуатацию КАК ДатаПринятияКУчету,
	|	МАКСИМУМ(НастройкиУчетаНДСПриУСН.Период) КАК ПериодУчетнойПолитики
	|ПОМЕСТИТЬ ПериодыУчетнойПолитики
	|ИЗ
	|	РасходыКПризнанию КАК РасходыКПризнанию
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиУчетаНДСПриУСН КАК НастройкиУчетаНДСПриУСН
	|		ПО РасходыКПризнанию.Организация = НастройкиУчетаНДСПриУСН.Организация
	|			И РасходыКПризнанию.Графа3_ДатаВводаВЭксплуатацию >= НастройкиУчетаНДСПриУСН.Период
	|	
	|СГРУППИРОВАТЬ ПО
	|	РасходыКПризнанию.Организация,
	|	РасходыКПризнанию.Графа3_ДатаВводаВЭксплуатацию
	|	
	|	
	|ОБЪЕДИНИТЬ ВСЕ
	|	
	|ВЫБРАТЬ
	|	РасходыКПризнанию.Организация,
	|	РасходыКПризнанию.ДатаВозникновения,
	|	МАКСИМУМ(НастройкиУчетаНДСПриУСН.Период)
	|	
	|ИЗ
	|	РасходыКПризнанию КАК РасходыКПризнанию
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиУчетаНДСПриУСН КАК НастройкиУчетаНДСПриУСН
	|		ПО РасходыКПризнанию.Организация = НастройкиУчетаНДСПриУСН.Организация
	|			И РасходыКПризнанию.ДатаВозникновения >= НастройкиУчетаНДСПриУСН.Период
	|	
	|СГРУППИРОВАТЬ ПО
	|	РасходыКПризнанию.Организация,
	|	РасходыКПризнанию.ДатаВозникновения
	...
	|	НАЧАЛОПЕРИОДА(ПериодыУчетнойПолитики.ДатаПринятияКУчету, МЕСЯЦ) КАК ДатаПринятияКУчету,
	|	НастройкиУчетаНДСПриУСН.ОсновноеНалогообложение <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС) КАК ПризнаватьНДС
	|ИЗ
	|	ПериодыУчетнойПолитики КАК ПериодыУчетнойПолитики
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиУчетаНДСПриУСН КАК НастройкиУчетаНДСПриУСН
	|		ПО ПериодыУчетнойПолитики.Организация = НастройкиУчетаНДСПриУСН.Организация
	|			И ПериодыУчетнойПолитики.ПериодУчетнойПолитики = НастройкиУчетаНДСПриУСН.Период";
...
Функция ИнициализироватьДанныеРезультата(Выборка)
	...
	СтруктураВозврата.Вставить("ЭлементРасходов");
	СтруктураВозврата.Вставить("СтатьяРасходов");
	СтруктураВозврата.Вставить("ПризнаватьРасходамиПриУСН", Истина);
