...
Процедура ОбработкаРасшифровкиДиаграммаРисков(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка) Экспорт
	Если ФормаОтчета.НастройкиОтчета.ПолноеИмя = "Отчет.ДиаграммаРисков" Тогда
		...
		НовыеПользовательскиеНастройки = УправлениеРискамиВызовСервера.УстановитьЗначениеПараметраВПользовательскиеНастройки("АнализРисков", "ПериодОтбора", НовыйПериодОтбора);
		// Вероятность.
		...
		Если ЗначениеЗаполнено(Вероятность) Тогда
			УправлениеРискамиВызовСервера.УстановитьЗначениеОтбораВПользовательскиеНастройкиОтчета("АнализРисков", "Вероятность", Вероятность, НовыеПользовательскиеНастройки);
		Иначе
		...
		Если ЗначениеЗаполнено(Ущерб) Тогда
			УправлениеРискамиВызовСервера.УстановитьЗначениеОтбораВПользовательскиеНастройкиОтчета("АнализРисков", "Ущерб", Ущерб, НовыеПользовательскиеНастройки);
		Иначе
			// Пропускаем установку отбора вероятности, т.к. значение не задаено.
		КонецЕсли;
