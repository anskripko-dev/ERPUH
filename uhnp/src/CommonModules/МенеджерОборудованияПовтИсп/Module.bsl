...
Функция КлассификаторОшибок() Экспорт
	...
	КлассОшибки = НовыйКлассОшибки(
		...
		НСтр("ru = 'Касса заблокировала продажу маркированных товаров, так как в течение длительного времени не удаётся передать уведомления о продаже или получить квитанции от системы маркировки. Обычно блокировка возникает при отсутствии обмена с ИСМ в течение около месяца. Необходимо восстановить передачу уведомлений и выполнить синхронизацию.';
			|en = 'Касса заблокировала продажу маркированных товаров, так как в течение длительного времени не удаётся передать уведомления о продаже или получить квитанции от системы маркировки. Обычно блокировка возникает при отсутствии обмена с ИСМ в течение около месяца. Необходимо восстановить передачу уведомлений и выполнить синхронизацию.'"),
		"Пользователь",
	...
	КлассОшибки = НовыйКлассОшибки(
		...
		НСтр("ru = 'Код маркировки не может быть обработан или проверен ККТ. Проверьте корректность КМ, а также отсутствие ограничений или блокировок по работе с маркировкой. После исправления данных повторите операцию. Если эти действия не помогли, обратитесь в техническую поддержку программы.';
			|en = 'Код маркировки не может быть обработан или проверен ККТ. Проверьте корректность КМ, а также отсутствие ограничений или блокировок по работе с маркировкой. После исправления данных повторите операцию. Если эти действия не помогли, обратитесь в техническую поддержку программы.'"),
	...
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаМаркировки", НСтр("ru = 'отсутствует.*КМ|маркировк|контрольн.*марка|код.*товара.*не распозн|маркир|работа.*маркированн|буфер чека';
														|en = 'отсутствует.*КМ|маркировк|контрольн.*марка|код.*товара.*не распозн|маркир|работа.*маркированн|буфер чека'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаМаркировки", НСтр("ru = 'КМ.*ранее не проверялся';
														|en = 'КМ.*ранее не проверялся'"));
	Результат.Добавить(КлассОшибки);
	...
	КлассОшибки = НовыйКлассОшибки(
		...
		НСтр("ru = 'Связь с ККТ или ФН отсутствует либо потеряна. Проверьте питание, кабели, порты и сетевое соединение. После восстановления связи повторите операцию. Если проблема сохраняется, обратитесь в техподдержку компании, обслуживающей оборудование.';
			|en = 'Связь с ККТ или ФН отсутствует либо потеряна. Проверьте питание, кабели, порты и сетевое соединение. После восстановления связи повторите операцию. Если проблема сохраняется, обратитесь в техподдержку компании, обслуживающей оборудование.'"),
		"Пользователь",
		ЛокальнаяОбработкаОшибки);
	ТаблицаГрупп.Вставить(КлассОшибки.Группа, КлассОшибки);
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаСвязи", НСтр("ru = 'соединение не установлено|ошибка.*обмена|устройство.*(не отвечает|занято)';
													|en = 'соединение не установлено|ошибка.*обмена|устройство.*(не отвечает|занято)'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаСвязи", НСтр("ru = 'сетевого обмена|соединение.*потеряно|не отвечает или недоступна|порт не *.|доступ.*интернет';
													|en = 'сетевого обмена|соединение.*потеряно|не отвечает или недоступна|порт не *.|доступ.*интернет'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаСвязи", НСтр("ru = 'нет\s+связи|timeout|timed\s*out|ком\s*порт|port|устройство\s+не\s+найдено|превышен\s+интервал|failed to connect|connection';
													|en = 'нет\s+связи|timeout|timed\s*out|ком\s*порт|port|устройство\s+не\s+найдено|превышен\s+интервал|failed to connect|connection'"));
	Результат.Добавить(КлассОшибки);
	...
	КлассОшибки = НовыйКлассОшибки(
		...
		НСтр("ru = 'Операция невозможна в текущем статусе ККТ. Приведите кассу в рабочее состояние, завершив оформление документов или закрыв смену. После этого повторите действие. Если проблема сохраняется, обратитесь в техническую поддержку компании, обслуживающей оборудование.';
			|en = 'Операция невозможна в текущем статусе ККТ. Приведите кассу в рабочее состояние, завершив оформление документов или закрыв смену. После этого повторите действие. Если проблема сохраняется, обратитесь в техническую поддержку компании, обслуживающей оборудование.'"),
		"Пользователь",
		ЛокальнаяОбработкаОшибки);
	ТаблицаГрупп.Вставить(КлассОшибки.Группа, КлассОшибки);
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаСтатусаККТ", НСтр("ru = '73h|24\s*час|Некорректное состояние ККТ|ККТ не зарегистрирована с указанной системой';
														|en = '73h|24\s*час|Некорректное состояние ККТ|ККТ не зарегистрирована с указанной системой'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаСтатусаККТ", НСтр("ru = 'смена.*(закрыта|открыта|не открыта|истекла)|отчет.*прерван|документ.*(закрыт|открыт|не закрыт)|функция.*не выполнима|invalid\s*mode';
														|en = 'смена.*(закрыта|открыта|не открыта|истекла)|отчет.*прерван|документ.*(закрыт|открыт|не закрыт)|функция.*не выполнима|invalid\s*mode'"));
	Результат.Добавить(КлассОшибки);
	...
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаПользовательскихРеквизитов", НСтр("ru = 'нет денег для выплаты|не.*(достаточно|хватает) наличности в кассе';
																		|en = 'нет денег для выплаты|не.*(достаточно|хватает) наличности в кассе'"));
	Результат.Добавить(КлассОшибки);
	...
	КлассОшибки = НовыйКлассОшибки(
		...
		НСтр("ru = 'Размер кассового чека превышает допустимый предел из‑за избыточного количества позиций. Для устранения проблемы сократите число позиций в чеке: разделите одну крупную покупку на несколько чеков меньшего объёма. Если это не помогло, обратитесь в техническую поддержку программы.';
			|en = 'Размер кассового чека превышает допустимый предел из‑за избыточного количества позиций. Для устранения проблемы сократите число позиций в чеке: разделите одну крупную покупку на несколько чеков меньшего объёма. Если это не помогло, обратитесь в техническую поддержку программы.'"),
		"Пользователь",
		ЛокальнаяОбработкаОшибки);
	ТаблицаГрупп.Вставить(КлассОшибки.Группа, КлассОшибки);
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаРазмерЧека", НСтр("ru = 'превышение размеров TLV';
														|en = 'превышение размеров TLV'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаРазмерЧека", НСтр("ru = 'позиций.*превысило.*лимит';
														|en = 'позиций.*превысило.*лимит'"));
	...
	КлассОшибки = НовыйКлассОшибки(
		...
		НСтр("ru = 'Некоторые обязательные поля документа отсутствуют либо заполнены некорректно. Проверьте реквизиты и исправьте ошибочные данные. После внесения исправлений повторите операцию. Если проблема сохраняется, обратитесь в техническую поддержку программы.';
			|en = 'Некоторые обязательные поля документа отсутствуют либо заполнены некорректно. Проверьте реквизиты и исправьте ошибочные данные. После внесения исправлений повторите операцию. Если проблема сохраняется, обратитесь в техническую поддержку программы.'"),
		"ТехподдержкаПрограммы",
		ЛокальнаяОбработкаОшибки);
	ТаблицаГрупп.Вставить(КлассОшибки.Группа, КлассОшибки);
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаПрограмныхРеквизитов", НСтр("ru = '(неверн|корректн).*(сумм|количеств)';
																	|en = '(неверн|корректн).*(сумм|количеств)'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаПрограмныхРеквизитов", НСтр("ru = '(неверный|нулевой) итог чека';
																	|en = '(неверный|нулевой) итог чека'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаПрограмныхРеквизитов", НСтр("ru = 'сумма.*(больше|меньше|не равна)';
																	|en = 'сумма.*(больше|меньше|не равна)'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаПрограмныхРеквизитов", НСтр("ru = 'невозможно записать реквизит агента|оплата по данному чеку не требуется|необходимо указать оплату';
																	|en = 'невозможно записать реквизит агента|оплата по данному чеку не требуется|необходимо указать оплату'"));
	Результат.Добавить(КлассОшибки);
	...
	КлассОшибки = НовыйКлассОшибки(
		...
		НСтр("ru = 'Фискальный накопитель не смог выполнить операцию из‑за ошибки ресурса или сбоя внутреннего интерфейса. Проверьте состояние ФН. При необходимости произведите замену ФН. Если проблема сохраняется, обратитесь в техническую поддержку компании, обслуживающей оборудование.';
			|en = 'Фискальный накопитель не смог выполнить операцию из‑за ошибки ресурса или сбоя внутреннего интерфейса. Проверьте состояние ФН. При необходимости произведите замену ФН. Если проблема сохраняется, обратитесь в техническую поддержку компании, обслуживающей оборудование.'"),
		"ТехподдержкаККТ",
		ЛокальнаяОбработкаОшибки);
	ТаблицаГрупп.Вставить(КлассОшибки.Группа, КлассОшибки);
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаФН", НСтр("ru = 'фн закрыт|нет ответа от фн';
												|en = 'фн закрыт|нет ответа от фн'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаФН", НСтр("ru = 'исчерпан.*ресурс.*|ресурс.*исчерпан.*|ресурс.*(фн|фд)';
												|en = 'исчерпан.*ресурс.*|ресурс.*исчерпан.*|ресурс.*(фн|фд)'"));
	Результат.Добавить(КлассОшибки);
	...
	КлассОшибки = НовыйКлассОшибки(
		...
		НСтр("ru = 'Произошло расхождение системного времени ККТ и фискального накопителя из‑за сбоя часов реального времени или некорректных настроек. Для устранения проблемы необходимо установить корректную дату и время в настройках ККТ. Также следует проверить синхронизацию с сервером времени и сверить текущую дату и время на оборудовании. Если проблема сохраняется, обратитесь в техническую поддержку компании, обслуживающей оборудование.';
			|en = 'Произошло расхождение системного времени ККТ и фискального накопителя из‑за сбоя часов реального времени или некорректных настроек. Для устранения проблемы необходимо установить корректную дату и время в настройках ККТ. Также следует проверить синхронизацию с сервером времени и сверить текущую дату и время на оборудовании. Если проблема сохраняется, обратитесь в техническую поддержку компании, обслуживающей оборудование.'"),
		"ТехподдержкаККТ",
		ЛокальнаяОбработкаОшибки);
	ТаблицаГрупп.Вставить(КлассОшибки.Группа, КлассОшибки);
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаВремениФН", НСтр("ru = 'сбой часов|неверн.*(дата|время|фн|фд)|(разница|расхожден).*времен|проверьте (дату|время)';
														|en = 'сбой часов|неверн.*(дата|время|фн|фд)|(разница|расхожден).*времен|проверьте (дату|время)'"));
	Результат.Добавить(КлассОшибки);
	...
	КлассОшибки = НовыйКлассОшибки(
		...
		НСтр("ru = 'Ошибка возникла во внутренних механизмах программы при обработке данных устройства. Встроенная библиотека не смогла корректно обработать параметры или идентификатор устройства. Для устранения проблемы обратитесь в техническую поддержку программы.';
			|en = 'Ошибка возникла во внутренних механизмах программы при обработке данных устройства. Встроенная библиотека не смогла корректно обработать параметры или идентификатор устройства. Для устранения проблемы обратитесь в техническую поддержку программы.'"),
		"ТехподдержкаБПО",
		ЛокальнаяОбработкаОшибки);
	ТаблицаГрупп.Вставить(КлассОшибки.Группа, КлассОшибки);
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаБиблиотеки", НСтр("ru = 'неверн.*идент|не найден.*устройств|идустройств|ошибка.*разбор.*xml|xml.*ошибка';
														|en = 'неверн.*идент|не найден.*устройств|идустройств|ошибка.*разбор.*xml|xml.*ошибка'"));
	Результат.Добавить(КлассОшибки);
	...
	КлассОшибки = НовыйКлассОшибки(
		...
		НСтр("ru = 'Печатающий механизм ККТ не готов к работе. Проверьте наличие бумаги, закрытие крышки и отсутствие механических блокировок. После устранения проблемы повторите операцию.';
			|en = 'Печатающий механизм ККТ не готов к работе. Проверьте наличие бумаги, закрытие крышки и отсутствие механических блокировок. После устранения проблемы повторите операцию.'"),
		"Пользователь",
		ЛокальнаяОбработкаОшибки);
	ТаблицаГрупп.Вставить(КлассОшибки.Группа, КлассОшибки);
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаАппаратная", НСтр("ru = 'бумаги|принтер|крышк|механическ|отрезчик|питани';
														|en = 'бумаги|принтер|крышк|механическ|отрезчик|питани'"));
	Результат.Добавить(КлассОшибки);
	...
	КлассОшибки = НовыйКлассОшибки(
		...
		НСтр("ru = 'Обновите основную поставку драйвера, выполните обновление прошивки ККТ. Если проблема повторяется, требуется обращение в техническую поддержку разработчика драйвера.';
			|en = 'Обновите основную поставку драйвера, выполните обновление прошивки ККТ. Если проблема повторяется, требуется обращение в техническую поддержку разработчика драйвера.'"),
		"ТехподдержкаККТ",
		ЛокальнаяОбработкаОшибки);
	ТаблицаГрупп.Вставить(КлассОшибки.Группа, КлассОшибки);
	...
	КлассОшибки = НовыйКлассОшибки(
		...
		НСтр("ru = 'Во время выполнения операции драйвер ККТ или программный модуль вернул ошибку. Перезапустите драйвер или кассу и убедитесь в корректности установленных обновлений. Выполните обновление прошивки ККТ. Если проблема повторяется, требуется обращение в техническую поддержку разработчика драйвера.';
			|en = 'Во время выполнения операции драйвер ККТ или программный модуль вернул ошибку. Перезапустите драйвер или кассу и убедитесь в корректности установленных обновлений. Выполните обновление прошивки ККТ. Если проблема повторяется, требуется обращение в техническую поддержку разработчика драйвера.'"),
		"ТехподдержкаККТ",
		ЛокальнаяОбработкаОшибки);
	ТаблицаГрупп.Вставить(КлассОшибки.Группа, КлассОшибки);
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаДрайвераККТ", НСтр("ru = 'некорректн(ая|ые|ый) (формат|параметр|операция)';
															|en = 'некорректн(ая|ые|ый) (формат|параметр|операция)'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаДрайвераККТ", НСтр("ru = 'неверн(ая|ые|ый) (формат|параметр|операция)';
															|en = 'неверн(ая|ые|ый) (формат|параметр|операция)'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаДрайвераККТ", НСтр("ru = 'неверн(ый|ое) (формат|значение) (команды|параметра)';
															|en = 'неверн(ый|ое) (формат|значение) (команды|параметра)'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаДрайвераККТ", НСтр("ru = 'ошибка программирования реквизита|ошибка записи заголовка пакета|поле.*отсутствует|некоррект.*поля|нет больше данных';
															|en = 'ошибка программирования реквизита|ошибка записи заголовка пакета|поле.*отсутствует|некоррект.*поля|нет больше данных'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаДрайвераККТ", НСтр("ru = '(модель|ккт|команда).*не поддерживается|не поддерживает данную команду|команда не разрешена|ошибка.*драйвер|прошивка ККТ не поддерживает|FFFFFFF8h';
															|en = '(модель|ккт|команда).*не поддерживается|не поддерживает данную команду|команда не разрешена|ошибка.*драйвер|прошивка ККТ не поддерживает|FFFFFFF8h'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаДрайвераККТ", НСтр("ru = 'ошибка лицензии|обновите прошивку';
															|en = 'ошибка лицензии|обновите прошивку'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаДрайвераККТ", НСтр("ru = 'фатальная ошибка ККТ|неизвестная ошибка|ошибка ККТ.*неверн|ошибка.*движка ккт';
															|en = 'фатальная ошибка ККТ|неизвестная ошибка|ошибка ККТ.*неверн|ошибка.*движка ккт'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаДрайвераККТ", НСтр("ru = 'ожидание команды продолжения печати|идет печать предыдущей команды';
															|en = 'ожидание команды продолжения печати|идет печать предыдущей команды'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаДрайвераККТ", НСтр("ru = 'нет устройства|сбой таблицы настроек|не поддерживается в данном режиме|не поддерживается в данной версии';
															|en = 'нет устройства|сбой таблицы настроек|не поддерживается в данном режиме|не поддерживается в данной версии'"));
	ДобавитьМаску(ТаблицаГрупп, "ОшибкаДрайвераККТ", НСтр("ru = 'Invalid password|Inout error|Command is invalid|служб.*UEMA';
															|en = 'Invalid password|Inout error|Command is invalid|служб.*UEMA'"));
	Результат.Добавить(КлассОшибки);
