...
Процедура ПодобратьАналогиПолуфабрикатов(СтрокиПотребности, ТаблицаМатериалов, РезервыПФ)
	...
	ТаблицаТоваров = ТаблицаМатериалов.Скопировать(,
	...
	ТаблицаТоваров.Колонки.Добавить("НазначениеОбеспечения", Новый ОписаниеТипов("СправочникСсылка.Назначения"));
	ТаблицаТоваров.Колонки.Добавить("Обособленно", Новый ОписаниеТипов("Булево"));
	ТаблицаТоваров.Колонки.Добавить("ЭтапРесурснойСпецификации", Новый ОписаниеТипов("СправочникСсылка.ЭтапыПроизводства"));
	ТипИсточника = ОбщегоНазначения.ОписаниеТипаЧисло(10);
	ТаблицаТоваров.Колонки.Добавить("Этап", ТипИсточника);
	
	Для Каждого СтрокаТЗ Из ТаблицаТоваров Цикл
		СтрокаТЗ.Этап = СтрокаТЗ.Идентификатор;
	КонецЦикла;
	
	Аналоги = АналогиМатериалов.ПолучитьТаблицуЗаменыДляФормированияЭтапов(ТаблицаТоваров, РезервыПФ, Ложь, ТипИсточника);
	Если Аналоги = Неопределено
		ИЛИ Аналоги.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для Индекс = 0 По Аналоги.Количество()-1 Цикл
		...
		Если НЕ Аналоги[Индекс].ЭтоАналог Тогда
			Продолжить;
		КонецЕсли;
		...
		Для каждого КлючИЗначение Из Аналог.ЗамененноеКоличествоПострочно Цикл
			Аналог.Идентификатор = КлючИЗначение.Ключ;
			КоличествоЗамены = КлючИЗначение.Значение;
			Прервать;
		КонецЦикла;
		...
		НоваяСтрока.СтрокаИндекс = СтрокаОбеспечениеИндекс;
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Аналог, "Разрешение, Номенклатура, Характеристика, Склад, ВариантОбеспечения, Количество");
		...
		Если Индекс = 0
			...
			НоваяСтрока = ТаблицаОбеспечение.Добавить();
			НоваяСтрока.СтрокаИндекс   = СтрокаОбеспечениеИндекс;
			НоваяСтрока.Количество     = КоличествоЗамены;
			...
			// Уменьшение потребности
			Потребность.Количество = Потребность.Количество - КоличествоЗамены;
			...
			//++ НЕ УТКА
				Если Обеспечение.ЗаменаНаАналог Тогда
					...
					Для каждого Аналог Из Потребность.Аналоги.НайтиСтроки(СтПоискаАналоги) Цикл
						...
						НоваяСтрока.ВариантОбеспечения = Аналог.ВариантОбеспечения;
...
Процедура ЗаполнитьСрокиОбеспеченияПролеживанияПотребностей(Потребности)
	...
	Для каждого Потребность Из Потребности Цикл
		...
		Для каждого Этап Из Потребность.ДанныеСпецификации.Этапы Цикл
			...
			Этап.ГотовКВыполнению = (ЗаписьГрафика.ДлительностьДоЗапуска = 0);
	...
	Для Сч = 1 По 2 Цикл
		...
		Для ИндексПотребность = 0 По Потребности.Количество()-1 Цикл
			...
			Если ТекПотребность.Обработана
				ИЛИ ТекПотребность.ПотомкиСвернуты
			...
			Если СводнаяПотребность.Аналоги.Количество() > 1 Тогда
				СводнаяПотребность.Аналоги.Свернуть("СтрокаИндекс, Разрешение, Номенклатура, Характеристика, Склад, ВариантОбеспечения", "Количество");
			КонецЕсли;
					...
					Если Количество > 0 Тогда
						...
						Если НайденныеСтроки.Количество() <> 0 Тогда
							...
							Если Потомок.Аналоги.Количество() > 1 Тогда
								Потомок.Аналоги.Свернуть("СтрокаИндекс, Разрешение, Номенклатура, Характеристика, Склад, ВариантОбеспечения", "Количество");
							КонецЕсли;
						КонецЕсли;
			...
			Для каждого Потребность Из МассивПотребностей Цикл
				...
				// Отметка свертки в родителях
				Родитель = Потребность.Родитель;
				Пока Родитель <> Неопределено Цикл
					Если Родитель.Обработана Тогда
						Прервать;
					КонецЕсли;
					Родитель.ПотомкиСвернуты = Истина;
					Родитель = Родитель.Родитель;
				КонецЦикла;
...
Функция ПодобратьПотребностиДляСвертки(Потребности, Потребность, СтруктураПодбора)
	...
	Для каждого СледующаяПотребность Из НайденныеСтроки Цикл
		...
		Если Потребности.Индекс(СледующаяПотребность) <= Потребности.Индекс(Потребность)
			...
			ИЛИ СледующаяПотребность.ПотомкиСвернуты
