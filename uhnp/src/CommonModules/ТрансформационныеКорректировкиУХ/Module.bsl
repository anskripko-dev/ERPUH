...
Функция СформироватьКорректировку(Реквизиты, Отказ = Ложь, ТаблицаПроводок = Неопределено) Экспорт
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru = 'Не удалось записать документ '") 
			+ ?(ОбработкаКЗП = Неопределено, "<...>", ОбработкаКЗП.ДокументОбъект) + ": " + ОписаниеОшибки();
...
Функция ПолучитьСоздатьДокументКорректировку(Реквизиты, Отказ) Экспорт
	...
	Если Реквизиты.Свойство("ОтборТК") Тогда
		Отбор = Реквизиты.ОтборТК;
	...
	ИначеЕсли Реквизиты.Свойство("ВидОперации") Тогда
	...
	Если КорректировкиСсылки.Количество() = 0 Тогда
		...
		Исключение
			...
			ОбщегоНазначенияУХ.СообщитьОбОшибке(
				НСтр("ru = 'Не удалось сформировать трансформационную корректировку: '") 
...
Функция ПолучитьЗапросПовторРеклассовВНА(Реквизиты)
	...
	|	&ТекстПовторРеклассаВНА КАК Комментарий,
	...
	ДатаОкончанияПредПериода = ОбщегоНазначенияУХ.ДобавитьДень(Реквизиты.ДатаНачала, -1);
	...
	Запрос.УстановитьПараметр("ТекстПовторРеклассаВНА", 			НСтр("ru = 'Повтор рекласса ВНА'"));
	Запрос.УстановитьПараметр("ДатаНачала", 						Реквизиты.ДатаНачала);
	Запрос.УстановитьПараметр("ДатаОкончания", 						КонецДняВключая(Реквизиты.ДатаОкончания));
	Запрос.УстановитьПараметр("Сценарий", 							Реквизиты.Сценарий);
	Запрос.УстановитьПараметр("Организация", 						Реквизиты.Организация);
...
Функция ПолучитьЗапросКорректировкиРегламента_ОперацииПериода(Реквизиты)
	...
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("ДатаНачала",		Реквизиты.ПериодОтчета.ДатаНачала);
...
Функция ПолучитьЗапросКорректировкиРегламента_ПовторПроводок(Реквизиты)
	...
	|	Корректировки.Сценарий КАК Сценарий,
	|	Корректировки.Валюта КАК Валюта,
	|	Корректировки.КонтрагентИнвестиция КАК КонтрагентИнвестиция,
	|	Корректировки.СчетДт КАК СчетДт,
	|	Корректировки.СубконтоДт1 КАК СубконтоДт1,
	|	Корректировки.СубконтоДт2 КАК СубконтоДт2,
	|	Корректировки.СубконтоДт3 КАК СубконтоДт3,
	...
	|	втРасчетныеДанные.Организация КАК Организация,
	|	втРасчетныеДанные.Сценарий КАК Сценарий,
	|	втРасчетныеДанные.КонтрагентИнвестиция КАК КонтрагентИнвестиция,
	|	ВЫБОР
	...
	|	&ТекстКурсоваяРазница КАК Комментарий,
	...
	|	втРасчетныеДанные.СчетИнвестицийПоДебету КАК СчетИнвестицийПоДебету
	|ПОМЕСТИТЬ втРасчетныеДанные
	|ИЗ
	...
	Запрос.УстановитьПараметр("ТекстКурсоваяРазница", 				НСтр("ru = 'Курсовая разница инвестиций при повторе проводок'"));
	Запрос.УстановитьПараметр("ДатаНачала", 						Реквизиты.ПериодОтчета.ДатаНачала);
	Запрос.УстановитьПараметр("ДатаОкончания", 						Реквизиты.ДатаОкончания);
...
Процедура ДобавитьПроводкуКорректировки(ТаблицаПроводки, СтрокаИсточник, СуммаПроводки, КомментарийПроводки, РесурсРегистра = "СуммаВВалютеУчета", 
	...
	Проводка.Содержание = КомментарийПроводки;
...
Функция ИспользуетсяТрансляцияСинтетики(КонтекстОтчета)
	...
	Запрос.УстановитьПараметр("ПустыеСсылкиПВХ",
		ПланыВидовХарактеристик.ВидыСубконтоКорпоративные.ПолучитьВозможныеПустыеСсылки());
...
Процедура ПередУдалениемИсходногоДокументаКорректировкиПередУдалением(Источник, Отказ) Экспорт
	...
	|ГДЕ
	|	ОбработанныеОбъектыБД.ОбработанныйОбъектСсылка = &ОбработанныйОбъектСсылка";
	
	Запрос.УстановитьПараметр("ОбработанныйОбъектСсылка",Источник.Ссылка);
	...
	Если Результат.Следующий() Тогда
		...
		НаборЗаписей.Отбор.ОбработанныйОбъектСсылка.Установить(Источник.Ссылка);
