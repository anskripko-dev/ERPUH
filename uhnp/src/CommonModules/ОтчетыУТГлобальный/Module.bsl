		...
		Если Результат.УспешноВыполнено Тогда
			...
			ИначеЕсли Задание.Ключ = "ОтражениеДокументовВБюджетировании" Тогда
				...
				Если ФормыКОповещению.Количество() Тогда
					...
					Обработчик = Новый ОписаниеОповещения("ФормаОтчетаПослеПодтвержденияПереформирования",
						ОтчетыУПКлиентПереопределяемый, ПараметрыОбработчика);
				...
				КонецЕсли;
			ИначеЕсли Задание.Ключ = "УправляющийПоток_ЗаданияКОтражениюЛимитовПоДаннымБюджетирования" Тогда
				ФормыКОповещению = ФормыКОповещению(Задание.Ключ, Задание.Значение);
				ПараметрыОбработчика = Новый Структура;
				ПараметрыОбработчика.Вставить("Формы", ФормыКОповещению);
				Если ФормыКОповещению.Количество() Тогда
					ТекстВопроса = НСтр("ru = 'Выполнено сторнирование резервов по лимитам. Переформировать отчет?';
										|en = 'Limit reserves are reversed. Rerun the report?'");
					Обработчик = Новый ОписаниеОповещения("ФормаОтчетаПослеПодтвержденияПереформирования",
						ОтчетыУПКлиентПереопределяемый, ПараметрыОбработчика);
					СписокОтветов = Новый СписокЗначений;
					СписокОтветов.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Переформировать';
