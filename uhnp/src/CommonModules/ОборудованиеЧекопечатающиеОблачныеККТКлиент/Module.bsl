...
Процедура НачатьВыполнениеКоманды(ОповещениеПриЗавершении, ИдентификаторКлиента, ИдентификаторУстройства, ПараметрыОперации, ПараметрыВыполнениеКоманды, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Команда = "CheckFiscalization" ИЛИ Команда = "PrintReceiptCorrection" Тогда
		...
		Если Результат.Свойство("РезультатПроверкиФЛК") Тогда
			ОбработатьРезультатФорматноЛогическогоКонтроля(Результат, ДополнительныеПараметры);
		Иначе
...
Процедура ОбработатьРезультатФорматноЛогическогоКонтроля(Результат, ДополнительныеПараметры) Экспорт
	
	РезультатВыполнения = МенеджерОборудованияКлиентСервер.ПараметрыВыполненияОперацииНаОборудовании(Ложь, Результат.ТекстОшибки, ДополнительныеПараметры.ИдентификаторУстройства);
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПриЗавершении, РезультатВыполнения);
