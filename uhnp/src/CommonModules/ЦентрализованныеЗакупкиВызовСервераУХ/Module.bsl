...
Функция РаспределитьСуммуПропорциональноКоэффициентам(Сумма,Массив)  Экспорт

	Возврат ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(Сумма,Массив);
...
Функция ВернутьТоварнуюКатегорию(МассивНоменклатураВход) Экспорт
	...
	Для Каждого ТекМассивНоменклатураВход Из МассивНоменклатураВход Цикл
		...
		ИначеЕсли ТипЗнч(ТекМассивНоменклатураВход) = Тип("СправочникСсылка.ТоварныеКатегории") ИЛИ  
			ТипЗнч(ТекМассивНоменклатураВход) = Тип("СправочникСсылка.КатегорииЗакупок")  Тогда
...
Процедура УстановитьДоступныеСтатьиЭлементаПоПараметруПотребностей(Знач Форма, Знач ИмяЭлемента, Знач Параметр, 
																	Знач СтруктураПриемник = Неопределено, Знач ВидБюджета = Неопределено, 
																	СвойстваАналитики = "СтатьяБюджета, Аналитика1, Аналитика2, Аналитика3, Аналитика4, Аналитика5, Аналитика6") Экспорт
	...
	СпособЗаполненияСтатьи = ПланированиеПотребностей.СпособЗаполненияСтатьи();    

	Если СпособЗаполненияСтатьи = Перечисления.ВидыЗаполненияСтатьиБюджета.ПоТоварнойКатегорииФН Тогда
		
		ПланированиеПотребностей.УстановитьДоступныеСтатьиЭлементаПоПараметруПотребностей(Форма, ИмяЭлемента, Параметр, ДоступныеСтатьи);  
		
		Если СтруктураПриемник <> Неопределено Тогда
			ПараметрСтатья = Новый Структура(СвойстваАналитики);
			ЗаполнитьЗначенияСвойств(СтруктураПриемник, ПараметрСтатья, СвойстваАналитики); 
		КонецЕсли;
		
	КонецЕсли;
	
	Если (СпособЗаполненияСтатьи = Перечисления.ВидыЗаполненияСтатьиБюджета.ПоТоварнойКатегорииФН Или
			СпособЗаполненияСтатьи = Перечисления.ВидыЗаполненияСтатьиБюджета.ПоТоварнойКатегории) И
			СтруктураПриемник <> Неопределено Тогда
		ПараметрСтатья = ПланированиеПотребностей.ПараметрыПотребности(Неопределено, Параметр, СвойстваАналитики, Новый Структура("ВидБюджета", ВидБюджета));  
		
		//Если статья недоступна для ФН, то не заполняем
		Если СпособЗаполненияСтатьи = Перечисления.ВидыЗаполненияСтатьиБюджета.ПоТоварнойКатегорииФН Тогда  
			Если ДоступныеСтатьи.Найти(ПараметрСтатья.СтатьяБюджета) = Неопределено Тогда 
				Возврат;
			КонецЕсли;
		КонецЕсли; 
...
Функция ТиповойСформироватьПечатнуюФормуПротоколаЗакупкиПоДокументу(
	...
	Истина);
	Если Не ПротоколСсылка.Проведен Тогда 
		ТекстСообщения = НСтр("ru = 'Печать протокола возможна только для проведенного документа'");		
...
Функция ПолучитьФлагиОсобогоПорядкаЗакупкиПоТоварнойКатегории(Номенклатура)
	...
	|   ИЛИ НоменклатураСОсобымПорядкомЗакупки.УровеньКонтроляНоменклатуры ССЫЛКА Справочник.КатегорииЗакупок)
...
Функция ЭтоФз223(Знач Объект) Экспорт
	ЭтоФз223 = Ложь;
	ТекКонтекст = Объект.Контекст;
	Имя = "СправочникСсылка.ЗакупочныеПроцедуры"; 
	Если ТипЗнч(ТекКонтекст) = Тип(Имя) Тогда
...
Функция ВернутьМассивОтбораПоТипуДокумента(ДокументБдВход, Знач Объект) Экспорт
	// Инициализация.
	РезультатФункции = Новый Массив;
	Если Объект.ВидМероприятия = Перечисления.ВидыМероприятий.ЗакупочнаяПроцедура И ЗначениеЗаполнено(ДокументБдВход) Тогда
		Наименование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументБдВход, "Наименование");
		Если Наименование = "ПротоколВыбораПобедителей" ИЛИ Наименование = "ПротоколОтменыЗакупочнойПроцедуры" 
				ИЛИ Наименование = "ОповещениеАккредитованныхПоставщиковОТоргах" ИЛИ Наименование = "КвалификацияПоставщика" Тогда
			НовыйПараметр = Новый ПараметрВыбора("Отбор.ЗакупочнаяПроцедура", Объект.Контекст);
...
Процедура ДеревоСтадийПередНачаломИзменения(Знач Элемент, Отказ, Знач Объект, Знач Действие, Знач ИзШаблона) Экспорт
		// Ограничение доступности стадии ФЗ-223.
		Если Элемент = "ДеревоСтадийСтадияГруппа" Тогда
			ПредопределеннаяСтадияФз223 = (ЭтоФз223(Объект) И ИзШаблона);
			...
			Если ПредопределеннаяСтадияФз223 Тогда
				...
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Стадия%", Строка(Действие));
		...
		Если ЦентрализованныеЗакупкиУХ.ДатыСтадийОпределеныВЗакупочнойПроцедуре(Действие) Тогда
			Отказ = Истина;
...
&НаСервере
Функция ПолучитьИнформациюОЗакупочнойПроцедуре(ОбъектКонтекст) Экспорт
	Если ТипЗнч(ОбъектКонтекст) = Тип("СправочникСсылка.ЗакупочныеПроцедуры") Тогда
		СпособВыбораПоставщика = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектКонтекст, "СпособВыбораПоставщика");
	Иначе
		СпособВыбораПоставщика = ПредопределенноеЗначение("Перечисление.СпособыВыбораПоставщика.ПустаяСсылка");
	КонецЕсли;
	Возврат СпособВыбораПоставщика;
КонецФункции		// ОтобразитьИнформациюОЗакупочнойПроцедуре()
