...
Функция ЕстьУПМСФО() Экспорт

	Запрос = Новый Запрос(
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	УчетнаяПолитикаМСФО.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.УчетнаяПолитикаМСФО КАК УчетнаяПолитикаМСФО");
...
Процедура ЗаполнитьРеквизитыСтруктурыГруппы(КонтекстВерсияРегламента, ДатаСреза, Сценарий, МеткаВремени, СтруктураГруппы, ИсточникВерсии = Неопределено) Экспорт
	...
	СтруктураГруппы = ПолучитьСтруктуруГруппыВерсии(
		КонтекстВерсияРегламента, Сценарий, МеткаВремени, ИсточникВерсии);
...
Функция ПолучитьРеквизитыОрганизаций(Организации, Сценарий = Неопределено, Период = Неопределено) Экспорт
	...
	ОтборСценарий = Неопределено;//все сценарии
	Если ЗначениеЗаполнено(Сценарий) Тогда
		ОтборСценарий = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Сценарий);
		ОтборСценарий.Добавить(ПредопределенноеЗначение("Справочник.Сценарии.ПустаяСсылка"));
	КонецЕсли;
...
Функция ПолучитьСтруктуруПеретаскивания(ОрганизацииИсточник, ШаблонУзла, Сценарий = Неопределено, ДатаУП = Неопределено) Экспорт 
	...
	Запрос.УстановитьПараметр("Сценарий", 		Сценарий);
	Запрос.УстановитьПараметр("Период", 		ДатаУП);
