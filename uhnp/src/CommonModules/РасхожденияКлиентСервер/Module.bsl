...
Функция ТекстПоясненияСпособОтработкиАктПослеОтгрузки(Форма, ТребуетсяСообщениеОНесоответствииСпособа)
	...
	ИначеЕсли Форма.ДоступныеСпособыОтработкиАктаПоСостояниюЭДО.НайтиПоЗначению(
		...
		ИначеЕсли Форма.Объект.ТипОснованияАктаОРасхождении = ПредопределенноеЗначение("Перечисление.ТипыОснованияАктаОРасхождении.ВозвратДавальцу") Тогда
			ТекстПояснения = НСтр("ru = 'Электронный документ по возврату сырья давальцу подтвержден. Возможно только исправление возврата.';
									|en = 'The electronic document on return of raw materials to provider is confirmed. You can only adjust the return.'");
		//-- НЕ УТКА
		КонецЕсли;
	...
	ИначеЕсли Не ДоступноИсправление Тогда
		...
		ИначеЕсли Форма.Объект.ТипОснованияАктаОРасхождении = ПредопределенноеЗначение("Перечисление.ТипыОснованияАктаОРасхождении.ВозвратДавальцу") Тогда
			ТекстПояснения = НСтр("ru = 'Электронный документ по возврату сырья давальцу был отклонен. Возможно только изменение возврата.';
									|en = 'The electronic document on return of raw materials to provider is rejected. You can only edit the return.'");
		//-- НЕ УТКА
		КонецЕсли;
