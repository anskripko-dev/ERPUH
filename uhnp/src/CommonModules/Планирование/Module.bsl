...
Процедура ОбработкаПроверкиЗаполненияВФорме(Таблица, СтруктураДействий, Отказ, ПроверяемыеРеквизиты, Знач КроссТаблица = Истина) Экспорт 
	...
	ЗаполнятьТоварнуюПозицию = Ложь;
	ТекстОшибкиТоварнаяПозиция 	= НСтр("ru='Не заполнена колонка ""Товарная позиция""'");
	#КонецОбласти
	...
	#Область УХ_Внедрение
	Если СтруктураДействий.Свойство("ЗаполнятьТоварнуюПозицию") Тогда 
		ЗаполнятьТоварнуюПозицию = Истина;
	КонецЕсли;
	#КонецОбласти
	...
	Для Каждого СтрокаТЧ Из Таблица Цикл
		...
		#Область УХ_Внедрение
		Если ЗаполнятьТоварнуюПозицию И НЕ ЗначениеЗаполнено(СтрокаТЧ.ТоварнаяПозиция) Тогда
			
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ТекстОшибкиТоварнаяПозиция + АдресОшибки,
				,
				ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(ПутьКТаблице, СтрокаТЧ.НомерСтроки, "ТоварнаяПозиция"),
				,
				Отказ);
	...
	Для каждого СтрокаКроссТаблицы Из КроссТаблица Цикл
		...
		Для каждого СтрокаПериод Из ТаблицаПериоды Цикл
			...
			Если НЕ СтруктураРеквизитов.Свойство("Формула_")
				...
				Если Параметры.Свойство("ПоляГруппировки") И СтрНайти(ВРег(Параметры.ПоляГруппировки), ВРег("ТоварнаяПозиция"))=0 Тогда
				#КонецОбласти
				Продолжить; 
				#Область УХ_Встраивание
				КонецЕсли;
...
Функция ПолучитьЗамещенныеПериоды(Параметры, ИмяДокумента , ИмяРеквизитаНачалоПериода = "НачалоПериода") Экспорт
	...
	КонецЕсли;
	#Область УХ_Встраивание
	Если Параметры.Свойство("ЭтоПланированиеПотребностей") И Параметры.ЭтоПланированиеПотребностей = Истина Тогда
		
		ТекстДляЗамены = "И ПланЗамещающий.Дата > &Дата";
		ТекстЗамены = "
		|	И ПланЗамещающий.СтатьяБюджета = &СтатьяБюджета
		|	И ПланЗамещающий.Аналитика1 = &Аналитика1
		|	И ПланЗамещающий.Аналитика2 = &Аналитика2
		|	И ПланЗамещающий.Аналитика3 = &Аналитика3
		|	И ПланЗамещающий.ЦФО = &ЦФО
		|	И ПланЗамещающий.Проект = &Проект
		|	И ПланЗамещающий.Организация = &Организация
		|	И ПланЗамещающий.Менеджер = &Менеджер
		|	И ПланЗамещающий.МестоПоставки = &МестоПоставки
		|	И ПланЗамещающий.Приоритет = &Приоритет";
		
		Запрос.Текст = СтрЗаменить(Запрос.текст, ТекстДляЗамены, ТекстДляЗамены + "
			|	"+ТекстЗамены);
		...
		// Отключить склад и подразделение
		Если Параметры.Свойство("Склад") Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПланЗамещающий.Склад = &Склад", "Истина");
		КонецЕсли;
		Если Параметры.Свойство("Подразделение") Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПланЗамещающий.Подразделение = &Подразделение", "Истина");
		КонецЕсли;
...
Функция ПроверитьПравилоЗаполнения(Знач ПравилоЗаполнения, Знач АдресСхемыПреобразованияДанных, Знач ВидПлана = Неопределено, Знач ИмяРеквизитаПравила = "", ОтборПоРесурсам) Экспорт 
	...
	ТекстОшибки = НСтр("ru = 'В составе полей, возвращаемых источниками заполнения количества, отсутствует поле ""Количество"" %1. Отредактируйте ""Выбранные поля"" или используйте другой источник';
						|en = 'Fields returned by the quantity population sources do not contain the ""Quantity"" field %1. Edit ""Selected fields"" or use another source'");
	Если НЕ ЕстьПолеКоличества Тогда
		
		Если НомераСтрокБезПоляКоличество.Количество() > 1 Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, НСтр("ru = 'в строках:';
																									|en = 'in lines:'") + " " + СтрСоединить(НомераСтрокБезПоляКоличество, ", "));
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, НСтр("ru = 'в строке';
																									|en = 'in line'") + " " + СтрСоединить(НомераСтрокБезПоляКоличество, ", "));
		КонецЕсли;
...
Процедура ПроверитьСтатусУтвержден(Объект, Отказ, РежимЗаписи, ТипПлана) Экспорт 
	...
	#Область УХ_Встраивание
	ИначеЕсли ТипПлана = Перечисления.ТипыПланов.ПланЗакупок И (Пользователи.РолиДоступны("АдминистраторЦЗ") 
		ИЛИ Пользователи.РолиДоступны("ДобавлениеИзменениеДанныхЦентрализованныхЗакупок")) Тогда
		Возврат;
	#КонецОбласти
	КонецЕсли;
