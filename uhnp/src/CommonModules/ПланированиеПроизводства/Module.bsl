...
Процедура РасчетГрафикаПроизводства() Экспорт
	...
	Если Настройки.ИспользуетсяПроизводство22 И Настройки.ПланируетсяГрафикПроизводства Тогда
		...
		УправлениеПроизводством.ПланироватьОчередьЗаказовНаПроизводство();
...
//
Функция РасписаниеРаботыПоГрафику(СписокГрафиков, НачалоПериода, ОкончаниеПериода, ПереходящиеСмены = Ложь) Экспорт

	Расписание = КалендарныеГрафики.РасписанияРаботыНаПериод(СписокГрафиков, НачалоПериода, ОкончаниеПериода, ПереходящиеСмены);
	...
	Для каждого СтрокаРасписание Из Расписание Цикл
		...
		СтрокаРасписание.Длительность = УправлениеПроизводствомКлиентСервер.ДлительностьИнтервалаРасписания(
			СтрокаРасписание.ВремяНачала, СтрокаРасписание.ВремяОкончания);
	КонецЦикла;
...
Функция РассчитатьОграниченияГрафикаПроизводстваПоМатериалам(Ссылка) Экспорт
	...
	ТекстЗапроса =
		...
		|		И НЕ РасчетДатыПоставкиПереопределяемый.ПлановаяДатаПоставки ЕСТЬ NULL";
...
Функция ОграниченияГрафикаПроизводстваПоМатериалам(Ссылка) Экспорт
	...
	ТекстЗапроса = 
		...
		|	И РасчетДатыПоставкиПереопределяемый.ПлановаяДатаПоставки ЕСТЬ НЕ NULL 
		|
		|УПОРЯДОЧИТЬ ПО
		|	Номенклатура,
		|	Характеристика,
...
Функция УчитываемыйРазмерБуфера(Буфер, ВидыРабочихЦентров)
	...
	Для каждого ИсполнительЭтапа Из ВидыРабочихЦентров Цикл
		...
		ИначеЕсли ИсполнительЭтапа.ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Смена Тогда
...
Функция ДатаСУчетомБуфера(Дата, Буфер, ГрафикРаботыИлиПодразделение, ЕдиницаИзмеренияБуфера) Экспорт
	...
	Если ЕдиницаИзмеренияБуфера = Перечисления.ЕдиницыИзмеренияВремени.День Тогда
		...
		Если Буфер > 0 Тогда
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВЫБРАТЬ ПЕРВЫЕ 2", "ВЫБРАТЬ ПЕРВЫЕ " + Формат(?(Цел(Буфер / 86400) = Буфер / 86400, Буфер / 86400, Цел(Буфер / 86400) + 1), "ЧН=0; ЧГ="));// @Query-part-1 @Query-part-2
...
Функция ЗаполнитьСпецификациюЗаказаПоСпискуНоменклатуры(Объект, 
	...
	Если СтрокиКОбработке.Количество() <> 0 Тогда
		...
		КэшированныеЗначенияКопия = ОбщегоНазначения.СкопироватьРекурсивно(КэшированныеЗначения);
		
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			СтрокиКОбработке,
			СтруктураДействий,
			Объект.ВозвратныеОтходы,
			КэшированныеЗначенияКопия);
	КонецЕсли;
