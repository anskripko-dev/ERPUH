...
Процедура ПоказатьСведенияОЗадолженности(Форма) Экспорт
	...
	Если Задолженность.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Сведения о задолженности были обновлены, задолженность отсутствует';
										|en = 'The debt information is updated. There is no debt'"));
...
Процедура КонтроллерСостояниеЭДОНажатие(Форма, СтандартнаяОбработка = Ложь) Экспорт
	...
	Если ТипЗнч(Форма.Объект.Ссылка) = Тип("ДокументСсылка.ВерсияСоглашенияКоммерческийДоговор") Тогда
		ОбъектУчета = Форма.Объект.ДоговорКонтрагента;
	Иначе
		ОбъектУчета = Форма.Объект.Ссылка;   
	КонецЕсли;
	#КонецОбласти
...
Асинх Процедура ОбработатьНаличиеСуществующегоДокументаДляВыбранногоФайлаXML(СуществующийДокумент,
	...
	Если ЗначениеЗаполнено(ИмяВыбранногоФайла) Тогда
		ТекстВопроса = СтрШаблон(НСтр("ru = '""%1"" такой документ уже был загружен ранее.
			|Открыть его?';
			|en = 'Such ""%1'' document has already been imported earlier.
			|Open it?'"), ИмяВыбранногоФайла);
	Иначе
...
Асинх Процедура ВывестиPDFДокументНаФорму(Форма, СведенияОPdfДокументе, ДанныеPdfДокумента) Экспорт
	...
	ТекстОшибки = НСтр("ru = 'Произошла ошибка при чтении файла. Файл поврежден и не может быть восстановлен.';
						|en = 'An error occurred while reading the file. The file is corrupted and cannot be restored.'", "ru");
	#Если ВебКлиент Тогда
		...
		Если Не Результат Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Ошибка подключения расширения работы с файлами. Просмотр недоступен.';
					|en = 'An error occurred while attaching 1C:Enterprise Extension. Viewing is unavailable.'", "ru"));
			Возврат;
		КонецЕсли;
		Попытка
		...
		Если ЗначениеЗаполнено(СведенияОРегистрации) Тогда
			...
			Если ЕстьВОчереди И ДанныеПоМЧДВОчереди.ВидЗапроса = 
				...
				ТекстВОчереди = НСтр("ru = 'В очереди на отзыв с';
									|en = 'Queued for revocation since'");
			...
			ИначеЕсли ЕстьВОчереди И ДанныеПоМЧДВОчереди.ВидЗапроса = 
				...
				ТекстВОчереди = НСтр("ru = 'В очереди на регистрацию с';
									|en = 'Queued for registration since'");
				ТекстЗаголовка = СтрШаблон(ШаблонЗаголовка, ТекстВОчереди, ДатаСоздания);
				ПоказыватьДатуОбновленияСтатуса = Ложь;
	...
	Иначе
		...
		Если ОшибкиФормирования.Количество() = 1
			...
			ТекстОшибки = НСтр("ru = 'Не удалось подготовить данные предварительного просмотра в Облачном ЭДО';
								|en = 'Cannot prepare preview data in Cloud EDI'");
	...
	Если Результат Тогда
		Если Не ЗначениеЗаполнено(Текст) Тогда
			Текст = НСтр("ru = 'Для работы с ЭДО необходимо выполнить настройку подключения.';
						|en = 'To operate with EDI, configure the connection.'");
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Заголовок) Тогда
			Заголовок = НСтр("ru = 'Подключение к ЭДО';
							|en = 'Connection to EDI'");
		КонецЕсли; 
...
Асинх Функция ПредложитьОтклонение(ПакетныйРежим) Экспорт
	Если ПакетныйРежим Тогда
		ТекстВопроса = НСтр("ru = 'Пакет документов еще не подписан. Отклонить пакет документов с указанием причины отказа?';
							|en = 'The document package is not signed yet. Reject the document package and specify the rejection reason?'");
	Иначе
		ТекстВопроса = НСтр("ru = 'Документ еще не подписан. Отклонить документ с указанием причины отказа?';
							|en = 'The document is not signed yet. Reject the document and specify the rejection reason?'");
	КонецЕсли;
	...
	Ответ = Ждать ВопросАсинх(ТекстВопроса, Кнопки, , КодВозвратаОтклонить, НСтр("ru = 'Аннулирование недоступно';
																				|en = 'Cancellation is unavailable'"));
...
Асинх Процедура ПоказатьВопросОПродолженииНастройкиПодключенияКЭДО(Текст, Заголовок, Форма)
	...
	ОбщаяЧастьВопроса = НСтр("ru = 'Перейти к настройке подключения ЭДО?';
							|en = 'Go to EDI connection settings?'");
