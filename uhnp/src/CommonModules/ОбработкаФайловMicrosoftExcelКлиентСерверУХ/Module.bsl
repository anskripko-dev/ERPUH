...
Функция ВернутьСписокЛистов(ИмяФайла, РасширениеФайла = "xlsx") Экспорт
	
	Если НРег(РасширениеФайла) = "xls" Тогда
		
		Возврат ВернутьСписокЛистовXLS(ИмяФайла);
		
	Иначе
		
		Возврат ВернутьСписокЛистовXLSX(ИмяФайла)
		
	КонецЕсли;
	
КонецФункции
	
Функция ВернутьСписокЛистовXLS(ИмяФайла) Экспорт
...
Функция ВернутьСписокЛистовXLSX(ИмяФайла) Экспорт
	...
	Попытка
		...
		Для Каждого Sheet Из ЭлементыSheets Цикл
			...
			НомерЛиста = НомерТекущегоЛиста;
			НомерТекущегоЛиста = НомерТекущегоЛиста + 1;
			СписокЛистов.Добавить(НомерЛиста, ИмяЛиста, Ложь);
		КонецЦикла;
		
	Исключение
		ТекстыОшибки = Новый Массив;
		ТекстыОшибки.Добавить(Нстр("ru = 'Не удалось открыть файл XLSX.'"));
		ТекстыОшибки.Добавить(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ТекстыОшибки.Добавить(Нстр("ru = 'Возможные причины ошибки описаны в справке к документу ""Экземпляр отчета"" в разделе ""Импорт""'"));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрСоединить(ТекстыОшибки, Символы.ПС));
	КонецПопытки;
