...
Функция ПолучитьСтруктуруРеквизитов(ОбъектРасчета,ВидОтчетаПроверка,ОрганизацияПроверка,СценарийПроверка,ПериодОтчетаПроверка,НазначениеРасчетовПроверка=Неопределено) Экспорт
	...
	Иначе
		...
		ПараметрыНастроек.ШаблонДокумента = ВидОтчетаПроверка;
		ПараметрыНастроек.Сценарий = СценарийПроверка;
		ПараметрыНастроек.Организация = ОрганизацияПроверка;
		...
		Иначе
			СтруктураРеквизитов.Вставить("ВИБ", ПолучитьВИБПоУмолчанию(ЗначенияНастроек.ПравилоОбработки));
		КонецЕсли;
...
Функция РассчитатьРекурсивноЗависимыеПоказателиПоТаблице(ТаблицаЗависимыхПоказателей,ОбъектРасчета,СтруктураКлючевыхРеквизитов,ПересчитыватьТекущие,ТрассировкаРасчета)
	...
	Если ЗаблокированныеПериоды.Количество()>0 Тогда
		...
		?(ТекСтрока.СтатусБлокировки=10,НСтр("ru = ' еще не открыт.'"),НСтр("ru = ' закрыт.'")));
...
Функция ЗаписатьИзмененияВРегистр(ОбъектРасчета)
	...
	ОбъектРасчета.Вставить("ПоследнийРедактировавший", Пользователи.АвторизованныйПользователь());
...
Процедура ИзменитьЗначенияДляОтчетовПоВнутригрупповымОперациям(ОбъектРасчета,ДеревоОтчетов) Экспорт
	...
	РассчитыватьЗависимые=Справочники.ВерсииРегламентовПодготовкиОтчетности.АвтоматическиПересчитыватьЗависимыеПоказатели(ОбъектРасчета.ПериодОтчета,ОбъектРасчета.Сценарий);
...
Функция ПолучитьТаблицуРаскрытияПоПараметрам(Организация, Сценарий, ВидОтчета, ПериодОтчета,
	...
	Если Не ЗначениеЗаполнено(ОсновнаяВалюта) Тогда
		...
		ДанныеПоВалютам = РегистрыСведений.ВалютыОтчетности.ОпределитьВалютыОтчета(ДокументУправленияПериодом.ВерсияОрганизационнойСтруктуры, ВидОтчета, Организация);
