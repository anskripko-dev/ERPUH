...
Функция СоздатьЗаписьСобытияПоСтруктуре(СтруктураВход) Экспорт
	...
	Если НЕ ОбщегоНазначения.ПодсистемаСуществует("БухгалтерияПредприятияПодсистемы") Тогда
    	БазовыйИсточник = Неопределено;
	Иначе
		БазовыйИсточник = ОбщегоНазначения.ПредопределенныйЭлемент("Документ.ЗаявкаНаРасходованиеДенежныхСредств.ПустаяСсылка");
	КонецЕсли;
	
	ИсточникЗапись					 = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураВход, "Источник", БазовыйИсточник);
...
Функция ПолучитьТекстОповещения(СтруктураОповещенияВход, ПоЭлектроннойПочтеВход)
	...
	ИсточникВход			 = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураОповещенияВход, "Источник", БазовыйИсточник);
...
Функция ОтправитьОповещениеВСистеме(СтруктураОповещенияВход)
	...
	Если Не ЗначениеЗаполнено(Пользователь) Тогда
		РезультатФункции = Ложь;
	Иначе
		ТекстТемыИСообщения   = ТекстТемыИСообщенияОповещения(СтруктураОповещенияВход, Ложь);
		ТекстСообщенияСистемы = ТекстТемыИСообщения.Сообщение;
		// Получение текста темы оповещения.
		Если Не ЗначениеЗаполнено(ТемаВход) Тогда
			ДокФормат = Новый ФорматированныйДокумент;
			ДокФормат.УстановитьHTML(ТекстТемыИСообщения.Тема, Новый Структура);
			ТекстТемы = ДокФормат.ПолучитьТекст();
			ДокФорматСообщения = Новый ФорматированныйДокумент;
			ДокФорматСообщения.УстановитьHTML(ТекстСообщенияСистемы, Новый Структура);
			ТекстСообщения = ДокФорматСообщения.ПолучитьТекст();
		Иначе
			ТекстТемы      = ТемаВход;
			ТекстСообщения = ТемаВход;
		КонецЕсли;
		...
		Если Пользователь.ОповещатьЧерезСистемуВзаимодействия Тогда
			Попытка
				СистемаВзаимодействияСерверУХ.СоздатьСообщение(Новый Структура("ФорматированныйДокумент, ТекстСообщения, Пользователь", ДокФорматСообщения, ТекстСообщения, Пользователь));			
			Исключение
				ТекстОшибки = "<" + Строка(ВидСобытия) + ">" + "<" + Строка(Пользователь) + ">";
				ОбщегоНазначенияУХ.СообщитьОбОшибке(НСтр("ru='Не удалось оповестить ';en='Failed to notify '") + ТекстОшибки + НСтр("ru=' пользователя по причине ';en=' for the user: '") + ОписаниеОшибки());
			КонецПопытки;
		КонецЕсли;
		
		Если Пользователь.ОповещатьЧерезТелеграм Тогда
			Попытка
				ТелеграмСерверУХ.СоздатьСообщение(Новый Структура("ФорматированныйДокумент, ТекстСообщения, Пользователь, ИдентификаторОповещения, Источник", ДокФорматСообщения, ТекстСообщения, Пользователь, Строка(СтруктураОповещенияВход.ОбщийИдентификаторОповещения), СтруктураОповещенияВход.Источник));
			Исключение
				ТекстОшибки = "<" + Строка(ВидСобытия) + ">" + "<" + Строка(Пользователь) + ">";
				ОбщегоНазначенияУХ.СообщитьОбОшибке(НСтр("ru='Не удалось оповестить ';en='Failed to notify '") + ТекстОшибки + НСтр("ru=' пользователя по причине ';en=' for the user: '") + ОписаниеОшибки());
			КонецПопытки;
...
Функция ТекстТемыИСообщенияОповещения(СтруктураОповещенияВход, ПоЭлектроннойПочтеВход)
	// Инициализация.
	...
	Результат.Вставить("Тема", РезультатФункции);
	// Добавим текст для вида события.
	Если ВидСобытияВход = Справочники.ВидыСобытийОповещений.ТребуетсяСогласование Тогда
		РезультатФункции	 = МодульРегламентныхЗаданийУХ.ДобавитьТекстСогласованияПоЗадаче(РезультатФункции, ЗадачаВход, ПользовательВход, ПоЭлектроннойПочтеВход, ЭтапВход);
	ИначеЕсли ВидСобытияВход = Справочники.ВидыСобытийОповещений.ТребуетсяВыборЭтапа Тогда				
	...
	Результат.Вставить("Сообщение", РезультатФункции);
	...
	Иначе	
		...
		Иначе
			Попытка	
				...
				ВложенияТелеграм = ТелеграмСерверУХ.ПолучитьТаблицуВложенныхФайлов(Источник, Истина);
				Для Каждого ВложениеТелеграм Из ВложенияТелеграм Цикл
					ДвоичныеДанныеФайла = РаботаСФайлами.ДвоичныеДанныеФайла(ВложениеТелеграм.Ссылка);
					Письмо.Вложения.Добавить(ДвоичныеДанныеФайла, ВложениеТелеграм.ИмяФайла);
				КонецЦикла;
				
				СписокТабличныхДокументов = Новый СписокЗначений;
				ТелеграмСерверУХ.ПолучитьСписокПечатныхФормОбъекта(Источник, СписокТабличныхДокументов, Истина);	
				ТелеграмСерверУХ.ИсточникНастраиваемыйОтчет(Источник, СписокТабличныхДокументов);
				
				Если ТипЗнч(Источник) = Тип("ДокументСсылка.НастраиваемыйОтчет") тогда
				...
				Если ЕстьПечатныеФормы Тогда
					...
					Письмо.Вложения.Добавить(СтрокаФайлов.ДвоичныеДанные, ИмяФайлаПечатныеФормы);
				КонецЕсли;
...
Процедура ОтправитьОповещенияПоСобытиям(ТаблицаОтправкиВход) Экспорт
	...
	ОбщийИдентификаторОповещения = Новый УникальныйИдентификатор;
	...
	Для Каждого ТекТаблицаОтправкиВход Из ТаблицаОтправкиВход Цикл
		...
		СтруктураОповещения.Вставить("ОбщийИдентификаторОповещения", ОбщийИдентификаторОповещения);
