...
// 	              * ОбъектНастройки - ОпределяемыйТип.ОбъектНастройкиСчетовМеждународногоУчета
...
Функция ПроверитьПравилаОтражения(МенеджерВременныхТаблиц, ПараметрыПроверки) Экспорт
	...
	ТаблицаОшибок.Колонки.Добавить("ОбъектУчета", Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов"));
...
Процедура НастроитьВариантыОтчетов(Настройки) Экспорт
	...
	Отчеты.АнализПроводокОперативногоУчета.НастроитьВариантыОтчета(Настройки);
	Отчеты.АнализКорреспонденцийОперативногоУчета.НастроитьВариантыОтчета(Настройки);
...
// Возвращает структуру параметров отражения записей регистров накопления в международном учете.
// 
// Параметры:
//  ИмяРегистра - Строка - имя регистра накопления - источника данных
// 
// Возвращаемое значение:
//  см. ФинансовыйУчетПоДаннымБалансовыхРегистров.ПараметрыОтраженияДвиженийВФинансовомУчете
//
Функция ПараметрыОтраженияДвиженийВМеждународномУчете(ИмяРегистра) Экспорт
	...
	ОписаниеПоляРегистратор = ПараметрыОтраженияВУчете.ПоляОтбораДвижений.Регистратор;
	
	ОписаниеПоляРегистра = ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписаниеПоляРегистра();
	ОписаниеПоляРегистра.ПутьКДанным = "ТИПЗНАЧЕНИЯ(" + ОписаниеПоляРегистратор.ПутьКДанным + ")"; //@query-part
	ОписаниеПоляРегистра.Заголовок = НСтр("ru = 'Тип регистратора';
											|en = 'Recorder type'");
	ОписаниеПоляРегистра.Тип = Новый ОписаниеТипов("Тип");
	ПараметрыОтраженияВУчете.ПоляОтбораДвижений.Вставить("ТипРегистратора", ОписаниеПоляРегистра);
...
// 	ОписаниеОбъектаУчета - См. ПланыВидовХарактеристик.СтатьиАктивовПассивов.ОписаниеОбъектаФинансовогоУчета 
...
// Возвращает структуру настроек финансового учета с установленными значениями свойств в соответствии
// с переданными настройками формирования проводок.
// 
// Параметры:
//  НастройкиФормированияПроводок - см. Справочники.НастройкиФормированияПроводокМеждународногоУчета.НастройкиФормированияПроводок
// 
// Возвращаемое значение:
//  см. ПланыВидовХарактеристик.СтатьиАктивовПассивов.НастройкиФинансовогоУчета
//
Функция НастройкиУчетаПоНастройкамФормированияПроводок(НастройкиФормированияПроводок) Экспорт
	
	НастройкиУчета = ПланыВидовХарактеристик.СтатьиАктивовПассивов.НастройкиФинансовогоУчета();
	
	Если НастройкиФормированияПроводок.ВариантФормированияПроводок = Неопределено
	 ИЛИ НастройкиФормированияПроводок.ВариантФормированияПроводок = Перечисления.ВариантыФормированияПроводок.СКорреспонденцией Тогда
		НастройкиУчета.ФормированиеКорреспонденций = Истина;
	Иначе
		НастройкиУчета.ФормированиеКорреспонденций = Ложь;
	КонецЕсли;
	
	Если НастройкиФормированияПроводок.СпособУчетаНесобственныхЦенностей = Неопределено
	 ИЛИ НастройкиФормированияПроводок.СпособУчетаНесобственныхЦенностей = Перечисления.СпособыУчетаНесобственныхПодконтрольныхЦенностей.ОтражаютсяНаЗабалансовыхСчетах Тогда
		НастройкиУчета.УчетНесобственныхЦенностей = Истина;
	Иначе
		НастройкиУчета.УчетНесобственныхЦенностей = Ложь;
	КонецЕсли;
	
	Если НастройкиФормированияПроводок.ВыручкаБезНДС = Истина
	 ИЛИ НастройкиФормированияПроводок.ВыручкаБезНДС = Неопределено Тогда
		НастройкиУчета.УчетВыручкиБезНДС = Истина;
	Иначе
		НастройкиУчета.УчетВыручкиБезНДС = Ложь;
	КонецЕсли;
	
	Если НастройкиФормированияПроводок.ВыручкаБезНДС = Ложь
	 ИЛИ НастройкиФормированияПроводок.ВыручкаБезНДС = Неопределено Тогда
		НастройкиУчета.УчетВыручкиСНДС = Истина;
	Иначе
...
Функция ПоляРегистраПоТипуСубконто(ТипЗначенияСубконто, ИмяРегистра)
	...
	Для каждого КлючИЗначение Из ПараметрыОтраженияВУчете.ИсточникиАналитики Цикл
...
Функция ЗапросДанныхФинансовыхРегистров(ПараметрыЗапроса, МенеджерВременныхТаблиц)
	...
	ИсточникиСоСрокамиОбращения.Колонки.Добавить("ОбъектУчета", Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов"));
	...
	ПризнакиУчетаКорреспонденций.Колонки.Добавить("КорОбъектУчета", Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов"));
	...
	ОбъектыНесобственныхЦенностей.Колонки.Добавить("ОбъектУчета", Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов"));
...
Функция ТекстыЗапросовЗаписейРегистров(Запрос)
	...
	НастройкиУчета = НастройкиУчетаПоНастройкамФормированияПроводок(НастройкиПроводок);
	
	ОбъектыФинансовогоУчета = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ОписаниеОбъектовФинансовогоУчета(НастройкиУчета);
	ПланыВидовХарактеристик.СтатьиАктивовПассивов.ДополнитьОписаниеОбъектовУчетаДляПолученияДанных(НастройкиУчета, ОбъектыФинансовогоУчета);
	...
	Для Каждого ИмяИсточникаДанных Из БалансовыеФинансовыеРегистры Цикл
		...
		Если НЕ ПустаяСтрока(ПараметрыОтраженияДвижений.УсловиеДебет) Тогда
		...
		Если НЕ ПустаяСтрока(ПараметрыОтраженияДвижений.УсловиеКредит) Тогда
		ИначеЕсли МетаданныеРегистра.ВидРегистра = СвойстваОбъектовМетаданных.ВидРегистраНакопления.Обороты Тогда
...
Процедура ОпределитьСчетаУчетаКорреспонденцийПоОбъектам(Запрос, ТекстыЗапросов)
	...
	|		)
	|	И ((ДвиженияДебетКредит.ОбъектУчетаДт <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности)
	|		И ДвиженияДебетКредит.ОбъектУчетаКт <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности))
...
Процедура ПроверитьОшибкиКорреспонденций(Запрос, ТекстыЗапросов, ТаблицаПроводок, ПараметрыФормированияПроводок)
	...
	|	ИЛИ ДвиженияСоСчетами.ОбъектУчетаДт = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ПустаяСсылка)
	|	ИЛИ ДвиженияСоСчетами.ОбъектУчетаКт = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ПустаяСсылка)
	...
	|		И ДвиженияСоСчетами.ОбъектУчетаДт <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности)
	...
	|			ИЛИ ДвиженияСоСчетами.ОбъектУчетаДт = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ВводОстатков)))
	|	ИЛИ (ДвиженияСоСчетами.СчетУчетаКт = ЗНАЧЕНИЕ(ПланСчетов.Международный.ПустаяСсылка)
	|		И ДвиженияСоСчетами.ОбъектУчетаКт <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности)
	|		И (ДвиженияСоСчетами.ПланСчетов.ПроводкиСКорреспонденцией
	|			ИЛИ ДвиженияСоСчетами.ИмяРегистраКт <> &ИмяРегистраСлужебныхОбъектовУчета
	|			ИЛИ ДвиженияСоСчетами.ОбъектУчетаКт = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ВводОстатков)))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
			...
			Иначе
				...
				Если НЕ ЗначениеЗаполнено(Выборка.СчетУчетаДт) И ЗначениеЗаполнено(Выборка.ОбъектУчетаДт)
				   И (ВыборкаПроводки.ПроводкиСКорреспонденцией
						...
						ИЛИ Выборка.ОбъектУчетаДт = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ВводОстатков)
				   И Выборка.ОбъектУчетаДт <> ПланыВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности Тогда
					СтатусПроводки = Перечисления.СтатусыОтраженияВМеждународномУчете.ОтсутствуютПравилаОтраженияВУчете;
					Если Выборка.ТипОбъектаНастройкиДт = Тип("СправочникСсылка.НастройкиСчетовУчетаПрочихОпераций") Тогда
						ТекстОшибки = СтрШаблон(
							НСтр("ru = 'В документе не указан счет учета по статье активов и пассивов ""%1""';
								|en = 'The ledger account for the ""%1"" asset and liability item is not specified in the document'"),
							Выборка.ОбъектУчетаДт);
					Иначе
						ЗарегистрироватьПустойСчетДляНастройки(Выборка, "Дт");
						ТекстОшибки = СтрШаблон(
							НСтр("ru = 'Не настроен %1 счет учета для объекта учета ""%2"" с %3';
								|en = 'The %1 ledger account is not set up for the ""%2"" accounting object with %3'"),
							НРег(ПредставлениеСрокаОбращения(Выборка, Запрос.Параметры, "Дт")),
							Выборка.ОбъектУчетаДт,
				...
				Если НЕ ЗначениеЗаполнено(Выборка.СчетУчетаКт) И ЗначениеЗаполнено(Выборка.ОбъектУчетаКт)
				   И (ВыборкаПроводки.ПроводкиСКорреспонденцией
						...
						ИЛИ Выборка.ОбъектУчетаКт = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ВводОстатков)
				   И Выборка.ОбъектУчетаКт <> ПланыВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности Тогда
					СтатусПроводки = Перечисления.СтатусыОтраженияВМеждународномУчете.ОтсутствуютПравилаОтраженияВУчете;
					Если Выборка.ТипОбъектаНастройкиКт = Тип("СправочникСсылка.НастройкиСчетовУчетаПрочихОпераций") Тогда
						ТекстОшибки = СтрШаблон(
							НСтр("ru = 'В документе не указан счет учета по статье активов и пассивов ""%1""';
								|en = 'The ledger account for the ""%1"" asset and liability item is not specified in the document'"),
							Выборка.ОбъектУчетаКт);
					Иначе
						ЗарегистрироватьПустойСчетДляНастройки(Выборка, "Кт");
						ТекстОшибки = СтрШаблон(
							НСтр("ru = 'Не настроен %1 счет учета для объекта учета ""%2"" с %3';
								|en = 'The %1 ledger account is not set up for the ""%2"" accounting object with %3'"),
							НРег(ПредставлениеСрокаОбращения(Выборка, Запрос.Параметры, "Кт")),
							Выборка.ОбъектУчетаКт,
...
Процедура ОпределитьСчетаУчетаОтдельныхКорреспонденций(Запрос, ТекстыЗапросов, ТекстыЗапросовРегистров)
	...
	Пока ВыборкаИмяРегистра.Следующий() Цикл
		...
		Если ИмяТаблицыРегистра <> ФинансовыйУчетПоДаннымБалансовыхРегистров.ИмяРегистраСлужебныхОбъектовУчета() Тогда
			...
			ТаблицаЗаменыСторно = "вт" + ИмяТаблицыРегистра + "ЗаменаСторно";
			ТаблицаЗаменыРеверса = "вт" + ИмяТаблицыРегистра + "ЗаменаРеверса";
...
Процедура ДобавитьТаблицыДвиженийДебетаИКредита(Запрос, ТекстыЗапросов)
	...
	|		ДвиженияРегистровСоСторно.ОбъектНастройки КАК ОбъектНастройки,
	|		ДвиженияРегистровСоСторно.МестоУчета КАК МестоУчета,
	...
	|		ДвиженияРегистровСоСторно.ДатаПогашения КАК ДатаПогашения,
...
Процедура СоединитьДвиженияПоКлючевымПолям(Запрос, ТекстыЗапросов)
	...
	|		КОГДА ДвиженияКредит.НесобственныеЦенности
	|			ТОГДА ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности)
...
Функция ТекстЗапросаАнализКорреспонденцийДляНастройки()
	...
	|ГДЕ
	|	ДвиженияДебетКредит.ОбъектУчетаДт <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности)
	|	И ДвиженияДебетКредит.ОбъектУчетаКт <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности)
	|";
...
Функция СтрокиВыборкиПолейПоОписаниюОбъектаУчета(СтрокиОбъектовУчета, ПараметрыОтраженияДвижений, ПараметрыЗапроса)
	...
	Иначе
		РесурсыУпр = ПараметрыОтраженияДвижений.РесурсыУпр;
	КонецЕсли;
	
	Если СтрокиОбъектовУчета[0].РесурсыРегл.Количество() > 0 Тогда
		РесурсыРегл = СтрокиОбъектовУчета[0].РесурсыРегл;
	Иначе
		РесурсыРегл = ПараметрыОтраженияДвижений.РесурсыРегл;
	КонецЕсли;
	
	Если СтрокиОбъектовУчета[0].РесурсыВал.Количество() > 0 Тогда
		РесурсыВал = СтрокиОбъектовУчета[0].РесурсыВал;
	Иначе
		РесурсыВал = ПараметрыОтраженияДвижений.РесурсыВал;
	КонецЕсли;
	
	Если СтрокиОбъектовУчета[0].РесурсыКоличество.Количество() > 0 Тогда
		РесурсыКоличество = СтрокиОбъектовУчета[0].РесурсыКоличество;
	Иначе
		РесурсыКоличество = ПараметрыОтраженияДвижений.РесурсыКоличество;
	КонецЕсли;
	
	СтрокиВыборкиРесурсов = Новый Структура(СтрСоединить(ПоляВыборкиЧисловыхРесурсов(), ","));
	СтрокиВыборкиРесурсов.СуммаУпр = ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиРесурса(РесурсыУпр);
	СтрокиВыборкиРесурсов.СуммаРегл = ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиРесурса(РесурсыРегл);
	СтрокиВыборкиРесурсов.СуммаВал = ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиРесурса(РесурсыВал);
	СтрокиВыборкиРесурсов.Количество = ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиРесурса(РесурсыКоличество);
...
Функция СтрокаВыборкиОбъектаУчета(СтрокиОбъектовУчета, ПараметрыЗапроса)
	...
	Для Каждого СтрокаОбъектаУчета Из СтрокиОбъектовУчета Цикл
		Если СтрокаОбъектаУчета.ОтборДанных.Элементы.Количество() > 0 Тогда
	...
	Если СтрокиВыбора.Количество() = 0 И ОбъектыБезОтбора.Количество() = 0 Тогда
		СтрокаВыборки = ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(ПланыВидовХарактеристик.СтатьиАктивовПассивов.ПустаяСсылка());
	...
	Иначе
		...
		Если ОбъектыБезОтбора.Количество() = 0 Тогда
			СтрокиИначе.Добавить(ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(ПланыВидовХарактеристик.СтатьиАктивовПассивов.ПустаяСсылка()));
		Иначе
...
Функция СтрокаВыборкиОбъектовПрочиеАктивыПассивы(СтрокиОбъектовУчета, ПараметрыЗапроса)
	
	СтрокиВыбора = Новый Массив();
	ПутьКДаннымОбъектУчета = ФинансовыйУчетПоДаннымБалансовыхРегистров.ПутьКДаннымСПсевдонимомИсточника("Статья");
	
	Для Каждого СтрокаОбъектаУчета Из СтрокиОбъектовУчета Цикл
		ОтборТолькоНаРавенствоСтатье = Истина;
		КоличествоЭлементовОтбора = СтрокаОбъектаУчета.ОтборДанных.Элементы.Количество();
		
		Если КоличествоЭлементовОтбора = 1 Тогда
			ЭлементОтбора = СтрокаОбъектаУчета.ОтборДанных.Элементы[0];
...
Функция СтрокаВыборкиПризнакаСторно(СтрокиВыборкиРесурсов)
...
Функция СтрокаВыборкиСуммыРесурса(СтрокаВыборкиРесурса, ШаблонВыборкиСуммы, СтрокаВыборкиСторно, УсловияНеНоль)
...
Функция СтрокаВыборкиСубконто(ВидСубконто, ТипЗначенияСубконто, ИмяРегистра, СтрокиОбъектовУчета)
	...
	Для Каждого СтрокаОбъектаУчета Из СтрокиОбъектовУчета Цикл
		ПутьКДаннымСубконто = СтрокаОбъектаУчета.ПутиКДаннымСубконто.Найти(ВидСубконто, "ВидСубконто");
		Если ПутьКДаннымСубконто <> Неопределено Тогда
			ПутьКДанным = ФинансовыйУчетПоДаннымБалансовыхРегистров.ПутьКДаннымСПсевдонимомИсточника(ПутьКДаннымСубконто.ПутьКДанным);
			Если СтрокаОбъектаУчета.ОтборДанных.Элементы.Количество() > 0 Тогда
				СтрокиКогда = Новый Массив();
				СтрокиКогда.Добавить(Символы.Таб);
				СтрокиКогда.Добавить("КОГДА"); //@Query-part
				...
				СтрокиВыбора.Добавить(СтрСоединить(СтрокиТогда, " "));
			Иначе
				ПутьКДаннымСубконтоБезОтбора = ПутьКДанным;
			КонецЕсли;
		ИначеЕсли СтрокаОбъектаУчета.ОтборДанных.Элементы.Количество() > 0 Тогда
			ОтборыДанныхБезПутейСубконто.Добавить(СтрокаОбъектаУчета.ОтборДанных);
		КонецЕсли;
	КонецЦикла;
	...
	Если НЕ ПустаяСтрока(ПутьКДаннымСубконтоБезОтбора) Тогда
		...
		Если ОтборыДанныхБезПутейСубконто.Количество() > 0 Тогда
			...
			СтрокиКогда.Добавить(ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаУсловияВыборкиДанных(ОтборыДанныхБезПутейСубконто));
		Иначе
...
Процедура ДобавитьВОтборГруппуЭлементовОтбора(ОтборПриемник, ГруппаЭлементов, ДтКт)
	...
	СкопироватьОтборКомпоновкиДанных(НоваяГруппа, ГруппаЭлементов, ДтКт);
...
Процедура ЗаполнитьБалансовыеИзмеренияИРеквизиты(НоваяПроводка, ВыборкаДвижения)
	...
	Если (ВыборкаДвижения.ПроводкиСКорреспонденцией
			...
			ИЛИ ВыборкаДвижения.ОбъектУчетаДт = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ВводОстатков)
	   И ВыборкаДвижения.ОбъектУчетаДт <> ПланыВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности Тогда
	...
	Если (ВыборкаДвижения.ПроводкиСКорреспонденцией
			...
			ИЛИ ВыборкаДвижения.ОбъектУчетаКт = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ВводОстатков)
	   И ВыборкаДвижения.ОбъектУчетаКт <> ПланыВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности Тогда
...
Процедура ЗаполнитьНебалансовыеИзмеренияИРесурсы(НоваяПроводка, ВыборкаДвижения, ДанныеРесурсов, СтрокаАналитики, ДтКт, Корреспонденция = Истина)
	...
	Если ВыборкаДвижения["ИмяРегистра" + ДтКт] <> ФинансовыйУчетПоДаннымБалансовыхРегистров.ИмяРегистраСлужебныхОбъектовУчета() Тогда
		...
		Если ВыборкаДвижения["ИмяРегистра" + КорДтКт] = ФинансовыйУчетПоДаннымБалансовыхРегистров.ИмяРегистраСлужебныхОбъектовУчета()
		   И ВыборкаДвижения["ОбъектУчета" + КорДтКт] <> ПланыВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности Тогда
	   ...
	   И ВыборкаДвижения["ОбъектУчета" + КорДтКт] <> ПланыВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности Тогда
		Для НомерКорСубконто = 1 По ВыборкаДвижения["КоличествоСубконто" + КорДтКт] Цикл
			Если НЕ ЗначениеЗаполнено(НоваяПроводка["Субконто" + КорДтКт + НомерКорСубконто])
				...
				Если НЕ ЗначениеЗаполнено(НоваяПроводка["Субконто" + КорДтКт + НомерКорСубконто])
					И ВыборкаДвижения["ОбъектУчета" + КорДтКт] = ПланыВидовХарактеристик.СтатьиАктивовПассивов.РасчетыСРозничнымиПокупателями
