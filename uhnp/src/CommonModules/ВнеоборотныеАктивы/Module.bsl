...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Ввод остатков узлов и компонентов амортизации';
		|en = 'OB entry: sub-assets and components of depreciation'", КодыЯзыков); // @НСтр-2
	...
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Перемещение НМА';
		|en = 'Transfer intangible assets'", КодыЯзыков); // @НСтр-2
	...
	Набор.Используется = Истина;
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Наработки объектов эксплуатации';
		|en = 'Asset activity record register'", КодыЯзыков); // @НСтр-2
...
Процедура НастроитьВариантыОтчетов(Настройки) Экспорт
	...
	Отчеты.КонтрольКорректностиЗаполненияОбъектовЭксплуатации.НастроитьВариантыОтчета(Настройки);
...
Процедура ПроверитьСтатьюПоПорядкуУчета(Объект, ИмяПоляСтатья, ПорядокУчета, ВидУчета, Отказ) Экспорт
	...
	Если МассивВариантовРаспределения.Найти(РеквизитыСтатьи[ИмяРеквизита]) = Неопределено Тогда
		...
		ТекстСообщения = НСтр("ru = 'Не поддерживается отражение расходов по статье ""%1"" при порядке учета ""%2"".
							  ...
							  |en = 'Cannot record expenses for the ""%1"" item with the ""%2"" accounting procedure.
							  |Change the accounting procedure or choose another item.'");
...
Функция ПроверитьНаличиеЗаданийКЗакрытиюМесяца(ПараметрыОбработчика, ИмяРегистраЗаданий) Экспорт
	...
	Если ВРег(ИмяРегистраЗаданий) = ВРег(Метаданные.РегистрыСведений.ЗаданияКРасчетуАмортизацииОС.Имя)
		...
		ШаблонСостояния = ?(Выборка.КоличествоОбъектов = 1,
			НСтр("ru = 'Требуется выполнение операции начиная с периода %1 по %2 пакету';
				|en = 'Perform month-end closing starting from %1 for the %2 package'", ОбщегоНазначения.КодОсновногоЯзыка()),
			НСтр("ru = 'Требуется выполнение операции начиная с периода %1 по %2 пакетам';
				|en = 'Perform month-end closing starting from %1 for the %2 packages'", ОбщегоНазначения.КодОсновногоЯзыка()));
							
	Иначе
		
		ШаблонСостояния = ?(Выборка.КоличествоОбъектов = 1,
			НСтр("ru = 'Требуется выполнение операции начиная с периода %1 по %2 объекту';
				|en = 'Perform month-end closing starting from %1 for the %2 object'", ОбщегоНазначения.КодОсновногоЯзыка()),
			НСтр("ru = 'Требуется выполнение операции начиная с периода %1 по %2 объектам';
				|en = 'Perform month-end closing starting from %1 for the %2 objects'", ОбщегоНазначения.КодОсновногоЯзыка()));
