...
Функция РусАнгАлфавитВспомогательныеСимволы(Символ) Экспорт
	...
	Возврат СтрНайти("АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯABCDEFGHIJKLMNOPQRSTUVWXYZ_0123456789",Символ)>0; // @Non-NLS
...
Функция РазностьДат(ДатаУменьшаемое, ДатаВычитаемое, Периодичность = "День")Экспорт 
	
	ПоказателиПериодичности = Новый Структура;
	ПоказателиПериодичности.Вставить("Год",12);
	ПоказателиПериодичности.Вставить("Квартал",3);
	ПоказателиПериодичности.Вставить("Месяц",1);
	ПоказателиПериодичности.Вставить("Неделя",-604800);
	ПоказателиПериодичности.Вставить("День",-86400);
	ПоказателиПериодичности.Вставить("Час",-3600);
	ПоказателиПериодичности.Вставить("Минута",-60);
	ПоказателиПериодичности.Вставить("Секунда",-1);
	
	Если ПоказателиПериодичности[Периодичность] > 0 Тогда 
		
		Результат = Цел((Год(ДатаУменьшаемое) * 12 + Месяц(ДатаУменьшаемое) - 1) / ПоказателиПериодичности[Периодичность])
					-Цел((Год(ДатаВычитаемое) * 12 + Месяц(ДатаВычитаемое) - 1) / ПоказателиПериодичности[Периодичность]) 
		
	Иначе    

		Результат = Цел(('00010101' - ДатаУменьшаемое) /  ПоказателиПериодичности[Периодичность])
					-Цел(('00010101' - ДатаВычитаемое) /  ПоказателиПериодичности[Периодичность]); 
...
// Процедура - Дополнить коллекцию из дерева
//
// Параметры:
//  Коллекция			 - <Массив, СписокЗначений> - дополняемая коллекция
//  ДеревоИлиЕгоЭлемент	 - <ДеревоЗначений, СтрокаДереваЗначений, ДанныеФормыДерево, ДанныеФормыЭлементДерева> - источник заполнения
//  Отбор				 - <Структура>	 - отбор для строк источника, значение может быть задано массивом.
//  ТолькоТекущийУровень - <Булево>	- дополнить только текущим уровнем без рекурсии
//  ЭтоДеревоЗначений	 - <Булево> - Истина - ДеревоЗначений, Ложь - ДанныеФормыДерево, Неопределено - определить
//
Процедура ДополнитьКоллекциюИзДерева(Коллекция, ДеревоИлиЕгоЭлемент, Отбор = Неопределено, ТолькоТекущийУровень = Ложь, ЭтоДеревоЗначений = Неопределено) Экспорт
	
	Если ЭтоДеревоЗначений = Неопределено Тогда
		ЭтоДеревоЗначений = ЭтоУзелДерева(ТипЗнч(ДеревоИлиЕгоЭлемент));
	КонецЕсли;
	
	Если ЭтоДеревоЗначений Тогда
		Узлы = ДеревоИлиЕгоЭлемент.Строки;
	Иначе 
		Узлы = ДеревоИлиЕгоЭлемент.ПолучитьЭлементы();
	КонецЕсли;
	...
	Для Каждого Узел Из Узлы Цикл
		...
		Если Отбор <> Неопределено Тогда
			...
			Для Каждого Условие Из Отбор Цикл
				...
				УсловиеСработало = (Узел[Условие.Ключ] = Условие.Значение);
				Если ТипЗнч(Условие.Значение) = Тип("Массив") Тогда
					УсловиеСработало = Условие.Значение.Найти(Узел[Условие.Ключ]) <> Неопределено;
				КонецЕсли;
		...
		Если ЗначениеНайдено Тогда
			Коллекция.Добавить(Узел);
		КонецЕсли;
		
		Если НЕ ТолькоТекущийУровень Тогда
			ДополнитьКоллекциюИзДерева(
				Коллекция, Узел, Отбор, ТолькоТекущийУровень, ЭтоДеревоЗначений);
		КонецЕсли;
...
Функция ЭтоУзелДерева(ТипИсточника) Экспорт

	Возврат Не (ТипИсточника = Тип("ДанныеФормыДерево") 
		ИЛИ ТипИсточника = Тип("ДанныеФормыЭлементДерева"));
...
Функция ПолучитьКлючПоНаименованию(Наименование) Экспорт
	...
	СтрокаЭталон = "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯABCDEFGHIJKLMNOPQRSTUVWXYZ_"; // @Non-NLS
...
Функция ПолучитьОписаниеТиповДоговора() Экспорт
	...
	МодульБухгалтерскийУчетКлиентСерверПереопределяемый = Неопределено;
	
	#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
		Если ОбщегоНазначения.ПодсистемаСуществует("БухгалтерияПредприятияПодсистемы") Тогда
			МодульБухгалтерскийУчетКлиентСерверПереопределяемый = ОбщегоНазначения.ОбщийМодуль("БухгалтерскийУчетКлиентСерверПереопределяемый");
		КонецЕсли;
	#Иначе
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("БухгалтерияПредприятияПодсистемы") Тогда
			МодульБухгалтерскийУчетКлиентСерверПереопределяемый = ОбщегоНазначенияКлиент.ОбщийМодуль("БухгалтерскийУчетКлиентСерверПереопределяемый");
		КонецЕсли;
	#КонецЕсли
	
	Если МодульБухгалтерскийУчетКлиентСерверПереопределяемый = Неопределено Тогда
		Возврат Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов");
	Иначе 
		Возврат МодульБухгалтерскийУчетКлиентСерверПереопределяемый.ПолучитьОписаниеТиповДоговора();
	КонецЕсли;
	
КонецФункции

// Функция возвращает имя текущей конфигурации 
// 
// Возвращаемое значение:
...
Функция ИмяКонфигурации() Экспорт
	...
	Возврат ОбщегоНазначенияУХВызовСервера.ИмяКонфигурации();
