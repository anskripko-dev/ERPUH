...
Процедура ОткрытьФормуВыбораСчетов(Форма) Экспорт
	...
	Поля = МСФОКлиентСерверУХ.ПоляОбъекта(Форма, "РежимРедактирования");	
	ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(Форма.КэшируемыеЗначения, Поля, Истина);
	
	Оповещение = Новый ОписаниеОповещения("Подключаемый_ПриИзмененииСчетов", 
		ФинансовыеИнструментыФормыКлиент, Новый Структура("Форма", Форма));
	
	ОткрытьФорму("Справочник.ПараметрыУчетаФИРСБУ.Форма.СчетаУчетаДокумента", 
		Форма.КэшируемыеЗначения, Форма, , , , Оповещение);
