...
//  ТекстыЗапроса - СписокЗначений Из Строка - Список текстов запроса провведения.
//  Регистры - Строка, Структура - Список регистров продения документа через запятую или в ключах структуры.
//  ДокументОбъект - ДокументОбъект.ЭтапПроизводства2_2 - документ объект.
//
...
#Область ТекстДвижениеМатериальныхЗатрат // (Дт 2Х :: Кт 2Х)
	...
	|		И НЕ Стоимости.РазделУчета В (ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаСкладах),
...
#Область ТекстСписаниеНаПрочиеАктивыВыходныхИзделий // (Дт ХХ.ХХ :: Кт 20)
	...
	|		И Строки.КорРазделУчета <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ВыпускПродукции)
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
...
Функция ТекстЗапросаТаблицаРНПТМатериаловВПроизводстве(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаТовары.Ссылка.ПартияПроизводства  КАК ПартияПроизводства,
	...
	|ИЗ
	|	Документ.ЭтапПроизводства2_2.ВидыЗапасовМатериалы КАК ТаблицаТовары
	|
	|ГДЕ
	...
	|	И ТаблицаТовары.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеРасходовНаПартииПроизводства)
	...
	|		ЗНАЧЕНИЕ(Перечисление.СтатусыЭтаповПроизводства2_2.Начат),
	|		ЗНАЧЕНИЕ(Перечисление.СтатусыЭтаповПроизводства2_2.Завершен))
	|	И ТаблицаТовары.АналитикаУчетаНоменклатуры.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|	И ТаблицаТовары.АналитикаУчетаНоменклатуры.Номенклатура.ПереноситьРНПТМатериала
	|	И НЕ ТаблицаТовары.НомерГТД = ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
	|";
...
Процедура УстановитьПараметрыВыборочнойРегистрацииКОтражениюВРеглУчете(Объект)
	...
	ТекстЗапросаТаблицаОтражениеДокументовВРеглУчете(
		Неопределено, ТекстыЗапроса, "ОтражениеДокументовВРеглУчете", Объект);
	...
	Если ТекстыЗапроса.Количество() > 0 Тогда
		...
		ТаблицыДвижений.Свойство("Таблица" + "ОтражениеДокументовВРеглУчете", ТаблицаОтражениеДокументовВРеглУчете);
		
		Для Каждого Строка Из ТаблицаОтражениеДокументовВРеглУчете Цикл
		...
		ПроведениеДокументов.ДобавитьТаблицуДанныхДокумента(
			Объект, "Таблица" + "ОтражениеДокументовВРеглУчете", ТаблицаОтражениеДокументовВРеглУчете);
...
Функция ТекстЗапросаТаблицаОтражениеДокументовВРеглУчете(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если ДокументОбъект <> Неопределено
	   И ПроведениеДокументов.ЕстьДополнительнаяТаблица(ДокументОбъект, "Таблица" + ИмяРегистра) Тогда
		Возврат "";
	КонецЕсли;
	
	МенеджерВременныхТаблиц = ?(Запрос = Неопределено, Неопределено, Запрос.МенеджерВременныхТаблиц);
	
	МассивТекстовЗапроса = Новый Массив;
	
	Если ДокументОбъект = Неопределено
	 Или ДокументОбъект.ВидыЗапасовИзделия.Количество() > 0 Тогда
		
		Если Не ПроведениеДокументов.ЕстьТаблицаЗапроса(
					"ДанныеДокументаШапка", ТекстыЗапроса, МенеджерВременныхТаблиц) Тогда
			
			Документы.ЭтапПроизводства2_2.ТекстЗапросаДанныеДокументаШапкаДекоратор(
				Запрос, ТекстыЗапроса, ДокументОбъект);
			
		КонецЕсли;
		
		Если Не ПроведениеДокументов.ЕстьТаблицаЗапроса(
					"ДанныеДокументаТаблицаВидыЗапасовИзделия", ТекстыЗапроса, МенеджерВременныхТаблиц) Тогда
			
			Документы.ЭтапПроизводства2_2.ТекстЗапросаДанныеДокументаТаблицаВидыЗапасовИзделияДекоратор(
				Запрос, ТекстыЗапроса, ДокументОбъект);
		...
		|		ТабличнаяЧасть.ВидЗапасовОрганизация             КАК Организация,
		|		НАЧАЛОПЕРИОДА(ТабличнаяЧасть.ДатаОперации, ДЕНЬ) КАК ДатаОтражения
		|	ИЗ
		|		ДанныеДокументаТаблицаВидыЗапасовИзделия КАК ТабличнаяЧасть";
	...
	Если ДокументОбъект = Неопределено
	 Или ДокументОбъект.ВидыЗапасовМатериалы.Количество() > 0 Тогда
		...
		Если Не ПроведениеДокументов.ЕстьТаблицаЗапроса(
					"ДанныеДокументаТаблицаВидыЗапасовМатериалы", ТекстыЗапроса, МенеджерВременныхТаблиц) Тогда
			
			Документы.ЭтапПроизводства2_2.ТекстЗапросаДанныеДокументаТаблицаВидыЗапасовМатериалыДекоратор(
				Запрос, ТекстыЗапроса, ДокументОбъект);
		...
		|		НАЧАЛОПЕРИОДА(ТабличнаяЧасть.ДатаОперации, ДЕНЬ) КАК ДатаОтражения
		|	ИЗ
		|		ДанныеДокументаТаблицаВидыЗапасовМатериалы КАК ТабличнаяЧасть
		|	ГДЕ
		|		ТабличнаяЧасть.АналитикаУчетаНоменклатурыНоменклатураТипНоменклатуры В
		|			(ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),
		|			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))";
	...
	Если ДокументОбъект = Неопределено
	 Или (ДокументОбъект.Статус = Перечисления.СтатусыЭтаповПроизводства2_2.Начат
			Или ДокументОбъект.Статус = Перечисления.СтатусыЭтаповПроизводства2_2.Завершен)
		 И ДокументОбъект.ВыходныеИзделия.Количество() > 0 Тогда
		...
		Если Не ПроведениеДокументов.ЕстьТаблицаЗапроса(
					"ДанныеДокументаТаблицаВыходныеИзделия", ТекстыЗапроса, МенеджерВременныхТаблиц) Тогда
			
			Документы.ЭтапПроизводства2_2.ТекстЗапросаДанныеДокументаТаблицаВыходныеИзделияДекоратор(
				Запрос, ТекстыЗапроса, ДокументОбъект);
		...
		|		ТабличнаяЧасть.Ссылка                                КАК Ссылка,
		|		ВЫБОР
		|			КОГДА ДанныеДокумента.ВнутренняяПереработка
		|				ТОГДА ДанныеДокумента.ОрганизацияДавалец
		|			ИНАЧЕ ДанныеДокумента.Организация
		|		КОНЕЦ                                                КАК Организация,
		|		НАЧАЛОПЕРИОДА(ТабличнаяЧасть.ДатаПроизводства, ДЕНЬ) КАК ДатаОтражения
		|	ИЗ
		|		ДанныеДокументаТаблицаВыходныеИзделия КАК ТабличнаяЧасть
		|			ЛЕВОЕ СОЕДИНЕНИЕ ДанныеДокументаШапка КАК ДанныеДокумента
		|			ПО ДанныеДокумента.Ссылка = ТабличнаяЧасть.Ссылка
		|	ГДЕ
		|		ТабличнаяЧасть.НоменклатураТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	...
	Если ДокументОбъект = Неопределено
	 Или (ДокументОбъект.Статус = Перечисления.СтатусыЭтаповПроизводства2_2.Начат
			Или ДокументОбъект.Статус = Перечисления.СтатусыЭтаповПроизводства2_2.Завершен)
		 И ДокументОбъект.ПобочныеИзделия.Количество() > 0 Тогда
		...
		Если Не ПроведениеДокументов.ЕстьТаблицаЗапроса(
					"ДанныеДокументаТаблицаПобочныеИзделия", ТекстыЗапроса, МенеджерВременныхТаблиц) Тогда
			
			Документы.ЭтапПроизводства2_2.ТекстЗапросаДанныеДокументаТаблицаПобочныеИзделияДекоратор(
				Запрос, ТекстыЗапроса, ДокументОбъект);
		...
		|		НАЧАЛОПЕРИОДА(ТабличнаяЧасть.ДатаПроизводства, ДЕНЬ) КАК ДатаОтражения
		|	ИЗ
		|		ДанныеДокументаТаблицаПобочныеИзделия КАК ТабличнаяЧасть
		|			ЛЕВОЕ СОЕДИНЕНИЕ ДанныеДокументаШапка КАК ДанныеДокумента
		|			ПО ДанныеДокумента.Ссылка = ТабличнаяЧасть.Ссылка
		|	ГДЕ
		|		ТабличнаяЧасть.НоменклатураТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	...
	Если ДокументОбъект = Неопределено
	 Или (ДокументОбъект.Статус = Перечисления.СтатусыЭтаповПроизводства2_2.Начат
			Или ДокументОбъект.Статус = Перечисления.СтатусыЭтаповПроизводства2_2.Завершен)
		 И ДокументОбъект.ОбеспечениеМатериаламиИРаботами.Количество() > 0 Тогда
		...
		Если Не ПроведениеДокументов.ЕстьТаблицаЗапроса(
					"ДанныеДокументаТаблицаОбеспечениеМатериаламиИРаботами",
					ТекстыЗапроса,
					МенеджерВременныхТаблиц) Тогда
			
			Документы.ЭтапПроизводства2_2.ТекстЗапросаДанныеДокументаТаблицаОбеспечениеМатериаламиИРаботамиДекоратор(
				Запрос, ТекстыЗапроса, ДокументОбъект);
		...
		|	ИЗ
		|		ДанныеДокументаТаблицаОбеспечениеМатериаламиИРаботами КАК ТабличнаяЧасть
		|			ЛЕВОЕ СОЕДИНЕНИЕ ДанныеДокументаШапка КАК ДанныеДокумента
		|			ПО ДанныеДокумента.Ссылка = ТабличнаяЧасть.Ссылка
		|	ГДЕ
		|		ТабличнаяЧасть.НоменклатураТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	...
	Если ДокументОбъект = Неопределено
	 Или (ДокументОбъект.Статус = Перечисления.СтатусыЭтаповПроизводства2_2.Начат
			Или ДокументОбъект.Статус = Перечисления.СтатусыЭтаповПроизводства2_2.Завершен)
		 И ДокументОбъект.РасходМатериаловИРабот.Количество() > 0 Тогда
		...
		Если Не ПроведениеДокументов.ЕстьТаблицаЗапроса(
					"ДанныеДокументаТаблицаРасходМатериаловИРабот", ТекстыЗапроса, МенеджерВременныхТаблиц) Тогда
			
			Документы.ЭтапПроизводства2_2.ТекстЗапросаДанныеДокументаТаблицаРасходМатериаловИРаботДекоратор(
				Запрос, ТекстыЗапроса, ДокументОбъект);
		...
		|	ИЗ
		|		ДанныеДокументаТаблицаРасходМатериаловИРабот КАК ТабличнаяЧасть
		|			ЛЕВОЕ СОЕДИНЕНИЕ ДанныеДокументаШапка КАК ДанныеДокумента
		|			ПО ДанныеДокумента.Ссылка = ТабличнаяЧасть.Ссылка
		|	ГДЕ
		|		ТабличнаяЧасть.НоменклатураТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	...
	|//ПОМЕСТИТЬ
	|ИЗ
	|	&ТекстЗапросаИсточник КАК ТабличнаяЧасть
	|
	|СГРУППИРОВАТЬ ПО
