...
Процедура ЗаказНаПроизводствоПриЗаписи(Документ, ДополнительныеСвойства, Отказ) Экспорт
	...
	ОбновитьГрафикВыпускаПродукцииЗаказа(Документ, МенеджерВременныхТаблицСтруктураЗаказа);
...
Процедура СформироватьЗаданияПакетноеФормированиеЭтапов(Ссылки, Очередь) Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	СтруктураЗаказа.КлючНабор В(&Идентификаторы) И СтруктураЗаказа.Требуется <> 0";
...
Процедура РассчитатьСостояниеПродукцииЗаказа(МенеджерВременныхТаблиц, ИмяВременнойТаблицы, ВызовИзОбработчикаОбновления = Ложь) Экспорт
	...
	|	План.ДатаПотребности       КАК ДатаПотребности,
	...
	|	Факт.ИдентификаторВГруппеИзделий     КАК ИдентификаторВГруппеИзделий,
	...
	|	Факт.Партия,
	|	Факт.ИдентификаторВГруппеИзделий,
	|	Факт.Подразделение,
	|	Факт.Этап,
	...
	|	План.Назначение,
	|	План.ЗаказНаПроизводство,
		// спецификацию не учитываем, т.к. по одной аналитике не может быть несколько спецификаций
	|	План.ДатаПотребности,
	|	План.НомерСтроки
	|;
	|
	...
	|	Факт.ЗаказНаПроизводство,
	|	ВЫБОР
	|		КОГДА Факт.ЗаказНаПоступление = НЕОПРЕДЕЛЕНО ТОГДА
	|			0
	|	ИНАЧЕ
	|			1
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА Факт.ДатаПоступления <> ДАТАВРЕМЯ(1,1,1) ТОГДА
	|			0
	|	ИНАЧЕ
	|			1
	|	КОНЕЦ,
...
Процедура ОбновитьГрафикВыпускаПродукцииЗаказа(Документ, МенеджерВременныхТаблиц)
	...
	Если Не Документ.Проведен ИЛИ НЕ Документ.Статус = Перечисления.СтатусыЗаказовНаПроизводство2_2.КПроизводству Тогда 
		...
		// график выпуска используется только в статусе "К производству"
		НаборЗаписей.Записать(Истина);
...
Процедура ЗаписатьРезультатРасчетаСостоянияПродукцииЗаказа(МенеджерВременныхТаблиц, ИмяВременнойТаблицы, Расчеты)
	...
	Для Каждого Расчет Из Расчеты Цикл
		...
		|	План.КлючНабор                                                                      КАК КлючНабор,
		|	План.ДлительностьДоВыпуска                                                          КАК ДлительностьДоВыпуска,
		|	МАКСИМУМ(План.ИдентификаторВГруппеИзделий)                                          КАК ИдентификаторВГруппеИзделий,
		...
		|		Движения.ДлительностьДоВыпуска
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		|	ВЫБРАТЬ
		...
		|		Движения.ЗаказНаПроизводство,
		|		Движения.КлючНоменклатура,
		|		Движения.ДлительностьДоВыпуска) КАК ТаблицаИзменений
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаИзменений.Номенклатура,
		|	ТаблицаИзменений.Характеристика,
		|	ТаблицаИзменений.Склад,
		...
		Если Не ИспользуетсяГрафикПроизводства Тогда // нормативный график
		...
		ОбщегоНазначенияУТ.УничтожитьВременныеТаблицы(МенеджерВременныхТаблиц, "СтруктураЗаказаПередЗаписью,СтруктураЗаказаНаборЗаписей");
