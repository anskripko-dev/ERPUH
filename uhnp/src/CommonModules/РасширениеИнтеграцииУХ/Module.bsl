...
Функция ПолучитьРабочуюТаблицуПоДаннымПереноса(ИспользуемаяИБ,СтруктураДанных) Экспорт
	
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом() Тогда
		
		МассивПараметров=Новый Массив;
		МассивПараметров.Вставить(0,ИспользуемаяИБ);
		МассивПараметров.Вставить(1,СтруктураДанных);

		Возврат РезультатФункцииИнтеграции("ОбменДаннымиНСИУХ.ПолучитьРабочуюТаблицуПоДаннымПереноса",МассивПараметров);	
...
Процедура ИзменитьЗначенияДляОтчетовПоВнутригрупповымОперациям(ОбъектРасчета,ДеревоОтчетов) Экспорт
	...
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом() Тогда
		...
		ВыполнитьМетодИнтеграции("ПересчетПоказателейУХ.ИзменитьЗначенияДляОтчетовПоВнутригрупповымОперациям",МассивПараметров);	
...
Процедура РассчитатьПоказателиЭффективности(Сценарий,Проект) Экспорт
	...
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом() Тогда
		...
		ВыполнитьМетодИнтеграции("Справочники.Проекты.РассчитатьПоказателиЭффективности",МассивПараметров);	
...
Функция РассчитатьЗависимыеПоказателиОтчетов(ОбъектРасчета,СтруктураКлючевыхРеквизитов,ТрассировкаРасчета=Ложь,ПересчитыватьТекущие=Истина) Экспорт
	...
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом() Тогда
		...
		Возврат РезультатФункцииИнтеграции("ПересчетПоказателейУХ.РассчитатьЗависимыеПоказателиОтчетов",МассивПараметров);	
