...
Функция ПараметрыДляПроведенияДокумента(Документ, Свойства) Экспорт
	...
	Если Свойства.РежимЗаписи <> РежимЗаписиДокумента.Запись Тогда
		Параметры.НезависимыеРегистры.Добавить(Метаданные.РегистрыСведений.ОбластиДействияАналоговМатериалов);
	КонецЕсли;
	//-- НЕ УТКА
...
Процедура ЗаписатьДанные(ТаблицыДляДвижений, Документ, МенеджерВременныхТаблиц, Отказ) Экспорт
	...
	//++ НЕ УТКА
	ИмяТаблицы = "Таблица" + "ОбластиДействияАналоговМатериалов";
	
	Если ТаблицыДляДвижений.Свойство(ИмяТаблицы) Тогда
		
		Набор = РегистрыСведений.ОбластиДействияАналоговМатериалов.СоздатьНаборЗаписей();
		Набор.Отбор.Разрешение.Установить(Документ);
...
Функция ДоступноОписаниеТипаПроизводственногоПроцесса() Экспорт
	...
	Результат = Ложь;
//++ НЕ УТКА
	Результат = ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПроизводством2_2")
...
Процедура ЗаполнитьСтрокиДерева(Параметры, Дерево, СтрокаДерева, ИспользованныеСпецификации = Неопределено, КлючСвязи = Неопределено) Экспорт
	...
	Пока УсловиеЦикла Цикл
		...
		Если ВыводитьЭтапы Тогда
			...
			СтрокаРодитель = ДобавитьСтрокуДерева(?(СтрокаДерева = Неопределено, Дерево, СтрокаДерева), Перечисления.ВидыСтрокДереваСпецификаций.Этап);
		...
		Если (Параметры.ВыводитьВыходныеИзделия ИЛИ КоличествоВыходныхИзделий > 0)
			...
			Если КоличествоВыходныхИзделий > 0 И ВыводитьЗаголовкиГруппировок Тогда
				СтрокаРодитель = ДобавитьСтрокуДерева(СтрокаЭтап, Перечисления.ВидыСтрокДереваСпецификаций.ВыходныеИзделия);
			...
			Для каждого Строка Из МассивСтрок Цикл
				...
				НоваяСтрока = ДобавитьСтрокуДерева(СтрокаРодитель, Перечисления.ВидыСтрокДереваСпецификаций.ВыходноеИзделие, Строка);
		...
		Если (Параметры.ВыводитьМатериалы ИЛИ КоличествоМатериалов > 0)
			...
			Если КоличествоМатериалов > 0 И ВыводитьЗаголовкиГруппировок Тогда
				СтрокаРодитель = ДобавитьСтрокуДерева(СтрокаЭтап, Перечисления.ВидыСтрокДереваСпецификаций.МатериалыИУслуги);
			...
			Для каждого Строка Из МассивСтрок Цикл
				...
				НоваяСтрока = ДобавитьСтрокуДерева(СтрокаРодитель, Перечисления.ВидыСтрокДереваСпецификаций.Материал, Строка);
				НоваяСтрока.Спецификация = Строка.ВложеннаяСпецификация;
				НоваяСтрока.ПроизводитсяВПроцессе = ЗначениеЗаполнено(Строка.ВложеннаяСпецификация);
		...
		Если Параметры.ВыводитьТрудозатраты И ЕстьСпецификация И КоличествоТрудозатрат > 0 Тогда
			...
			Если КоличествоТрудозатрат > 0 И ВыводитьЗаголовкиГруппировок Тогда
				СтрокаРодитель = ДобавитьСтрокуДерева(СтрокаЭтап, Перечисления.ВидыСтрокДереваСпецификаций.Трудозатраты);
				СтрокаРодитель.Номенклатура = "" + Перечисления.ВидыСтрокДереваСпецификаций.Трудозатраты + " (" + КоличествоТрудозатрат + ")";
			КонецЕсли;
			
			Для Каждого Строка Из МассивСтрок Цикл
				НоваяСтрока = ДобавитьСтрокуДерева(СтрокаРодитель, Перечисления.ВидыСтрокДереваСпецификаций.Трудозатрата, Строка);
		...
		Если ВыводитьОперации И КоличествоОпераций > 0 Тогда
			...
			Если ВыводитьЗаголовкиГруппировок Тогда
				СтрокаРодитель = ДобавитьСтрокуДерева(СтрокаЭтап, Перечисления.ВидыСтрокДереваСпецификаций.Операции);
				СтрокаРодитель.Номенклатура = "" + Перечисления.ВидыСтрокДереваСпецификаций.Операции + " (" + КоличествоОпераций + ")";
			КонецЕсли;
			
			Для Каждого Строка Из МассивСтрок Цикл
				НоваяСтрока = ДобавитьСтрокуДерева(СтрокаРодитель, Перечисления.ВидыСтрокДереваСпецификаций.Операция, Строка);
...
// 
// Возвращаемое значение:
//  Массив из см. СоздатьОписаниеФункцииАвтовыбора
//
// Пример:
...
//		* ВозвращаемоеЗначение - Строка,Массив,ОписаниеТипов,Неопределено
//		* Параметры - Структура
Функция СоздатьОписаниеФункцииАвтовыбора(ИмяФункции, ИмяМодуля = "", Представление = "", ВозвращаемоеЗначение = Неопределено, Параметры = Неопределено) Экспорт
	
	Результат = Новый Структура;
	
	Результат.Вставить("ИмяФункции",           ИмяФункции);
	...
	Если ПустаяСтрока(Результат.ИмяМодуля) Тогда
	...
	Если ВозвращаемоеЗначение <> Неопределено Тогда
		Если ТипЗнч(ВозвращаемоеЗначение) = Тип("Массив") ИЛИ ТипЗнч(ВозвращаемоеЗначение) = Тип("Строка") Тогда
			Результат.ВозвращаемоеЗначение = Новый ОписаниеТипов(ВозвращаемоеЗначение);
		ИначеЕсли ТипЗнч(ВозвращаемоеЗначение) = Тип("ОписаниеТипов") Тогда
			Результат.ВозвращаемоеЗначение = ВозвращаемоеЗначение;
		ИначеЕсли ТипЗнч(ВозвращаемоеЗначение) = Тип("Тип") Тогда
			Результат.ВозвращаемоеЗначение = Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ВозвращаемоеЗначение));
		КонецЕсли;
	КонецЕсли;
...
// Добавить строку дерева.
// 
// Параметры:
//  СтрокаРодитель - СтрокаДереваЗначений,ДеревоЗначений
//  ВидСтроки - ПеречислениеСсылка.ВидыСтрокДереваСпецификаций
//  СтрокаОснование - СтрокаТаблицыЗначений
// 
// Возвращаемое значение:
//  см. УправлениеДаннымиОбИзделияхКлиентСервер.СтрокаДереваСпецификацийСтруктурой
Функция ДобавитьСтрокуДерева(СтрокаРодитель, ВидСтроки, СтрокаОснование = Неопределено)
	
	СтрокиДерева = СтрокаРодитель.Строки; // КоллекцияСтрокДереваЗначений
...
Функция ОписаниеЗагружаемыхДанных(НастройкиИсточника) Экспорт
	...
	Если НастройкиИсточника.РазделыКОбработке.Свойство(УправлениеДаннымиОбИзделияхКлиентСервер.КлючРазделаЗаменыМатериалов()) Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(
			Описание.РеквизитыСлужебные,
			ОписаниеЭлементаЗагружаемыхДанных("РазрешениеНаЗаменуМатериалов", "ОбластиДействия").РеквизитыПрикладные,
			Истина);
		Результат.Вставить(Описание.Ключ, Описание);
...
Функция ИсключаемыеРеквизитыЭлементаЗагружаемыхДанных(ИмяОбъектаМетаданных, ИмяТЧ = "")
	...
	ИначеЕсли ИмяОбъектаМетаданных = "РазрешениеНаЗаменуМатериалов" Тогда
		...
		Результат.Добавить("ВариантЗамены");
...
Процедура ОбновитьКлючиСвязиОбъекта(Объект, ТолькоЗаполненные = Ложь) Экспорт
	...
	//++ НЕ УТКА
	ЗависимыеТабличныеЧасти.Вставить("АльтернативныеВидыРабочихЦентров", "КлючСвязиВидыРабочихЦентров");
	ЗависимыеТабличныеЧасти.Вставить("СоответствиеСвойств", "КлючСвязи");
	ЗависимыеТабличныеЧасти.Вставить("ОтборПоСвойствам", "КлючСвязи");
	//-- НЕ УТКА
