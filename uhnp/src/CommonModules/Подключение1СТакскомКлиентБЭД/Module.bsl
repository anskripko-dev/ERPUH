...
Функция КонфигурацияЗарегистрированаВСервисе1СТакском(ОшибкаОбращенияКВебСервису = Ложь) Экспорт
	...
	Возврат Подключение1СТакскомВызовСервераБЭД.КонфигурацияЗарегистрированаВСервисеИПП(
...
Процедура ОткрытьВебСтраницу(Знач АдресСтраницы, ЗаголовокОкна = "") Экспорт
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ЗаголовокОкна", ЗаголовокОкна);
	ДополнительныеПараметры.Вставить("Логин"        , Неопределено);
	ДополнительныеПараметры.Вставить("Пароль"       , Неопределено);
	...
	Если НеобходимаАвторизации
		И ПараметрыОткрытия.ЭтоПолноправныйПользователь Тогда
		
		// Получение URL для открытия страницы.
		Состояние(, , НСтр("ru = 'Переход на Портал 1С:ИТС';
							|en = 'Go to 1C:ITS Portal'"));
		РезультатПолученияURL =
			Подключение1СТакскомВызовСервераБЭД.URLДляПереходаНаСтраницуИнтегрированногоСайта(АдресСтраницы);
		Состояние();
		
		Если РезультатПолученияURL = URL Тогда
...
Процедура ПриПолученииРезультатаСозданияЗаявки(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Действие = "ОтправитьЗаявку" Тогда
		...
		ДвоичныеДанныеСертификата = Подключение1СТакскомВызовСервераБЭД.ДвоичныеДанныеСертификата(
			СертификатЭП,
			ПредставлениеСертификата);
...
//		Подключение1СТакскомВызовСервераБЭД.НовыйКонтекстВзаимодействия()
...
Процедура РасшифроватьКонтрольныйМаркерЭПСервера(
	...
	Исключение
		...
		Подключение1СТакскомВызовСервераБЭД.ЗаписатьОшибкуВЖурналРегистрации(СообщениеДляЖурналаРегистрации);
		ПоказатьПредупреждение(, СообщениеОбОшибкеДляПользователя);
		Возврат;
	КонецПопытки;
...
Функция ТекстТехническихПараметровЭДО(КонтекстВзаимодействия, Знач Сертификат = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Сертификат) Тогда
		ОтпечатокСертификата = Подключение1СТакскомВызовСервераБЭД.ОтпечатокСертификата(Сертификат);
	Иначе
...
//		подключения (ИнтернетПоддержкаПользователейОшибкаДоступаКИнтернетуБЭД)
...
Процедура ВыполнитьСценарий(
	...
	Исключение
		...
		Форма = ОткрытьФорму("ОбщаяФорма.ИнтернетПоддержкаПользователейОшибкаДоступаКИнтернетуБЭД", ПараметрыФормы);
...
Процедура ОбработкаКомандСервиса(
	...
	Если НЕ ПродолжитьВыполнение Тогда
		...
		Если КонтекстВзаимодействия = Неопределено Тогда
			...
			Подключение1СТакскомВызовСервераБЭД.ОбработкаКомандСервиса(
				Неопределено,
				ПараметрыЗапроса,
				КонтекстОбработчика,
				Ложь,
	...
	Если КонтекстОбработчика.ПроизошлаОшибка Тогда
		...
		Если КонтекстОбработчика.ДействияПриОшибкеДляСервера.Количество() > 0 Тогда
			...
			Подключение1СТакскомВызовСервераБЭД.ЗавершитьСеансИнтернетПоддержки(
				КонтекстВзаимодействия.КСКонтекст,
				КонтекстОбработчика);
...
Процедура ПередОткрытиемФормы(
	...
	Если ИмяОткрываемойФормы = "Обработка.Подключение1СТакскомБЭД.Форма.УникальныйИдентификаторАбонента" Тогда
	...
	ИначеЕсли ИмяОткрываемойФормы = "Обработка.Подключение1СТакскомБЭД.Форма.ЗаявкаНаРегистрациюАбонента"
...
Процедура ВыполнитьКомандуСервиса(
	...
	ИначеЕсли ИмяКоманды = "setcodesregion" Тогда
		...
		Подключение1СТакскомКлиентСерверБЭД.СохранитьВПараметрахКодыРегионов(
			КонтекстВзаимодействия.КСКонтекст,
			СтруктураКоманды);
...
Функция СформироватьПараметрыОткрытияФормы(КСКонтекст, ИмяОткрываемойФормы)
	...
	Если ИмяОткрываемойФормы = "Обработка.Подключение1СТакскомБЭД.Форма.ПодключениеИнтернетПоддержки" Тогда
	...
	ИначеЕсли ИмяОткрываемойФормы = "Обработка.Подключение1СТакскомБЭД.Форма.ОбщаяРегНомер" Тогда
		ПараметрыНовойФормы.Вставить("regnumber",
			ЗначениеСессионногоПараметра(КСКонтекст,"regnumber"));

		ПараметрыНовойФормы.Вставить("ПриНачалеРаботыСистемы", КСКонтекст.ПриНачалеРаботыСистемы);

	ИначеЕсли ИмяОткрываемойФормы = "Обработка.Подключение1СТакскомБЭД.Форма.ОбщаяПинкод" Тогда
		ПараметрыНовойФормы.Вставить("regnumber",
			ЗначениеСессионногоПараметра(КСКонтекст,"regnumber"));
		ПараметрыНовойФормы.Вставить("pincode",
			ЗначениеСессионногоПараметра(КСКонтекст,"pincode"));

	ИначеЕсли ИмяОткрываемойФормы = "Обработка.Подключение1СТакскомБЭД.Форма.ИнтернетПоддержкаПродуктаНеОказывается" Тогда

		Если КСКонтекст.Свойство("ПриНачалеРаботыСистемы") Тогда
			ПараметрыНовойФормы.Вставить("ПриНачалеРаботыСистемы", КСКонтекст.ПриНачалеРаботыСистемы);
		КонецЕсли;

	ИначеЕсли ИмяОткрываемойФормы = "Обработка.Подключение1СТакскомБЭД.Форма.УникальныйИдентификаторАбонента" Тогда
	...
	ИначеЕсли ИмяОткрываемойФормы = "Обработка.Подключение1СТакскомБЭД.Форма.ЗаявкаНаРегистрациюАбонента" Тогда
	...
	ИначеЕсли ИмяОткрываемойФормы = "Обработка.Подключение1СТакскомБЭД.Форма.ИзменениеТарифа" Тогда
...
// Обработка.Подключение1СТакскомБЭД.Форма.ВводДанных.
//
Процедура ВвестиДанные(КонтекстВзаимодействия, Форма, СтруктураКоманды)
	...
	ИмяФормыВводаДанных = "Обработка.Подключение1СТакскомБЭД.Форма.ВводДанных";
...
Процедура ОстановитьМеханизм(КонтекстВзаимодействия, СтруктураКоманды)
	...
	Если СокрЛП(Строка(КодОшибки)) <> "0" И КодОшибки <> Неопределено Тогда
		...
		Подключение1СТакскомВызовСервераБЭД.ЗаписатьОшибкуВЖурналРегистрации(КодОшибки);
		ПараметрыФормы = ПараметрыФормыОшибкиОбращения(КонтекстВзаимодействия);
...
// Возвращает параметры формы "ОбщаяФорма.ИнтернетПоддержкаПользователейОшибкаДоступаКИнтернетуБЭД",
...
Функция ИмяКонфигурации()
	...
	ПараметрыБЭД = СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиента().Подключение1СТакскомБЭД;
	Возврат ПараметрыБЭД.ИмяКонфигурации;
...
Функция ВерсияКонфигурации()
	...
	ПараметрыБЭД = СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиента().Подключение1СТакскомБЭД;
	Возврат ПараметрыБЭД.ВерсияКонфигурации;
...
Функция ТекКодЛокализации()
	...
	ПараметрыБЭД = СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиента().Подключение1СТакскомБЭД;
	Возврат ПараметрыБЭД.КодЛокализации;
...
Функция ВерсияОбработкиОбновления()
	...
	ПараметрыБЭД = СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиента().Подключение1СТакскомБЭД;
	Возврат ПараметрыБЭД.ВерсияОбработкиОбновления;
