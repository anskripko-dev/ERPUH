...
Процедура УстановитьУсловноеОформлениеОперацииЗаказаПоставщику(Форма) Экспорт
	...
	Элемент.Представление = НСтр("ru = 'Закупка у поставщика (неотфактурованная поставка)';
								|en = 'Закупка у поставщика (неотфактурованная поставка)'");
	...
	Элемент.Представление = НСтр("ru = 'Ввоз из ЕАЭС (неотфактурованная поставка)';
								|en = 'Ввоз из ЕАЭС (неотфактурованная поставка)'");
...
Функция РазобратьСоставныеПрослеживаемыеНомераГТД(ТоварыКСписанию, ТоварыКОприходованию, ПоляОбъектные = Неопределено, ПоляЧисловые = Неопределено, Отказ = Ложь) Экспорт
	...
	Пока КомплектующиеРНПТ.Следующий() Цикл
		...
		Если ИсходноеКоличество <> 0 Тогда
			...
			|en = 'An error occurred when performing the reverse kitting of traceable kits: cannot attribute RNPT number ""%1"" to a traceable finished product (component) as there is no finished product (component) with a suitable FEACN unit of measure (""%2"").
...
Процедура ЗаполнитьСписокВыбораНаименованиеВходящегоДокумента(Форма, Контрагент) Экспорт
	...
	|ИЗ
	|	(ВЫБРАТЬ ПЕРВЫЕ 5
	|		Таблица.НаименованиеВходящегоДокумента КАК Наименование
	|	ИЗ
	|		&ПолноеИмя КАК Таблица
	|	ГДЕ
	|		Таблица.Контрагент = &Контрагент
	|		И Таблица.НаименованиеВходящегоДокумента <> &ПустаяСтрока
	|	УПОРЯДОЧИТЬ ПО
...
Процедура СообщитьОбОшибкахНаличиеПрослеживаемыхРаботВДокументеЗакупки(Выборка, ДокументЗакупки, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстОшибки = НСтр("ru = 'Нельзя указывать прослеживаемую работу ""%Номенклатура%"" (строка %НомерСтроки% списка ""Товары""), для документа с типом хоз. операции ""%ХозяйственнаяОперация%""';
							|en = 'You cannot specify the ""%Номенклатура%"" traceable work (line %НомерСтроки% of the ""Items"" list) for the document with the ""%ХозяйственнаяОперация%"" business transaction type'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Номенклатура%",          Выборка.Номенклатура);
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%",           Выборка.НомерСтроки);
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ХозяйственнаяОперация%", ДокументЗакупки.ХозяйственнаяОперация);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Разборка составного номера ГТД (РНПТ)';
				|en = 'Decode the composite CCD number (RNPT)'",
