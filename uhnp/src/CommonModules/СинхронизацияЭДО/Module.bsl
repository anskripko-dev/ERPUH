...
Процедура ПрочитатьДополнительныеДанныеДокумента(ДополнительныеФайлы, Знач ОписаниеДополнительныхДанных,
	...
	ТекстСообщения = НСтр("ru = 'Ошибка при распаковке дополнительных файлов ЭДО';
							|en = 'An error occurred when unpacking additional EDI files'", 
...
Функция ВидОшибкиЕстьНеполученныеЭлектронныеДокументы() Экспорт
	...
	ВидОшибки.ОписаниеРешения = СтрШаблон(НСтр("ru = '<a href = ""%1"">Выберите</a> дальнейшее действие';
												|en = '<a href = ""%1"">Select</a> further action'"), "Выберите");
...
Процедура СинхронизироватьПереченьСпециальныхОбстоятельств(ТипСинхронизации,
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru = 'Обновление перечня специальных обстоятельств ФНС';
							|en = 'Update the list of special circumstances (FTS)'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка,,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
	...
	Для Каждого ИдентификаторЭДО Из ИдентификаторыЭДО Цикл
		...
		Если Токен = Неопределено Тогда
			ВидОперации = НСтр("ru = 'Подготовка запроса в сервис ЭДО';
								|en = 'Подготовка запроса в сервис ЭДО'");
			ТекстОшибки = НСтр("ru = 'Отсутствует токен доступа для авторизации в сервисе ЭДО';
								|en = 'Отсутствует токен доступа для авторизации в сервисе ЭДО'");
...
Функция ВидОшибкиНекорректныйТокенДоступаКСервисуЭДО(ИдентификаторУчетнойЗаписи) Экспорт
	...
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Ошибка авторизации в сервисе ЭДО';
										|en = 'Ошибка авторизации в сервисе ЭДО'");
	ВидОшибки.ОписаниеПроблемы =
		НСтр("ru = 'Сервис 1С-ЭДО не смог распознать предоставленный токен как допустимый для авторизации';
			|en = 'Сервис 1С-ЭДО не смог распознать предоставленный токен как допустимый для авторизации'");
	ОписаниеРешения = СтрШаблон(НСтр("ru = '<a href = ""%1"">Авторизоваться по сертификату</a> и повторить операцию.';
									|en = '<a href = ""%1"">Авторизоваться по сертификату</a> и повторить операцию.'"), "Авторизоваться");
