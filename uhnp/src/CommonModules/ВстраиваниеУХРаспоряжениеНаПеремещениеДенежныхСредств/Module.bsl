...
Процедура УстановитьДоступностьЭлементовПоСтатусу(Форма, ТолькоПросмотрЭлементов) Экспорт
	...
	Элементы.АналитикаЗачисления1.ТолькоПросмотр = ТолькоПросмотрЭлементов;
	Элементы.АналитикаЗачисления2.ТолькоПросмотр = ТолькоПросмотрЭлементов;
	Элементы.АналитикаЗачисления3.ТолькоПросмотр = ТолькоПросмотрЭлементов;
...
Процедура СоздатьЭлементыФормыДокумента(Форма) 
	...
	ДанныеАналитикиШапкиЗачисление = АСБ.Новый_АналитикиСтатьиБюджета("АналитикаЗачисления%1", "Объект", "");
	...
	АСБ.СтатьяБюджетов_ЭлементыАналитикиИзШаблона(ПараметрыСтатьиШапкиЗачисление, ИмяЭлемента, "АналитикаЗачисления%1", "Объект", "");
...
Процедура ПодготовитьФормуНаСервере(Форма)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДатаПлатежа) Тогда
		Для каждого Позиции Из Форма.ПлатежнаяПозиция Цикл
		    Позиции.ДатаИсполнения = Дата(1,1,1);		
		КонецЦикла;	
	КонецЕсли; 
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СдачаДенежныхСредствВБанк
			...
			ВстраиваниеУХРаспоряжениеНаПеремещениеДенежныхСредств.УстановитьБКОтправитель(Форма.ПлатежнаяПозиция, Объект.Касса, Объект.Валюта);
		ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзБанка
			...
			ВстраиваниеУХРаспоряжениеНаПеремещениеДенежныхСредств.УстановитьБКОтправитель(Форма.ПлатежнаяПозиция, Объект.БанковскийСчет, Объект.Валюта);
		КонецЕсли;
		
		Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеДенежныхСредствНаДругойСчет Тогда
			ВстраиваниеУХРаспоряжениеНаПеремещениеДенежныхСредств.УстановитьБКОтправитель(Форма.ПлатежнаяПозиция, Объект.БанковскийСчет, Объект.Валюта);
			ВстраиваниеУХРаспоряжениеНаПеремещениеДенежныхСредств.УстановитьБКПолучатель(Форма.ПлатежнаяПозиция, Объект.БанковскийСчетПолучатель);
		ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаДенежныхСредствВДругуюКассу Тогда
			ВстраиваниеУХРаспоряжениеНаПеремещениеДенежныхСредств.УстановитьБКОтправитель(Форма.ПлатежнаяПозиция, Объект.Касса, Объект.Валюта);
			ВстраиваниеУХРаспоряжениеНаПеремещениеДенежныхСредств.УстановитьБКПолучатель(Форма.ПлатежнаяПозиция, Объект.КассаПолучатель);
		КонецЕсли
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(Объект, ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ДанныеЗаполнения = неопределено Тогда
		...
		ДанныеЗаполнения.Вставить("ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.СдачаДенежныхСредствВБанк);
	КонецЕсли;
	...
	Если ОбщегоНазначенияУХ.СвойствоЗаполнено(ДанныеЗаполнения, "ВидОперацииУХ") Тогда
		
		СтатьяДвиженияДенежныхСредствПоУмолчанию 
			= ВидыОперацийУХВызовСервераПовтИсп.ОсновнаяСтатьяБюджетаПоВидуОперации(ДанныеЗаполнения.ВидОперацииУХ);
		
		Если Не ОбщегоНазначенияУХ.СвойствоЗаполнено(ДанныеЗаполнения, "СтатьяДвиженияДенежныхСредств") Тогда
			ДанныеЗаполнения.Вставить("СтатьяДвиженияДенежныхСредств", СтатьяДвиженияДенежныхСредствПоУмолчанию);
		КонецЕсли;
		
		Если Не ОбщегоНазначенияУХ.СвойствоЗаполнено(ДанныеЗаполнения, "СтатьяДвиженияДенежныхСредствЗачисление") Тогда
			ДанныеЗаполнения.Вставить("СтатьяДвиженияДенежныхСредствЗачисление", СтатьяДвиженияДенежныхСредствПоУмолчанию);
		КонецЕсли;
...
Функция НовоеСостояниеРазмещения(ОбъектСписаниеБезналичныхДС) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ДенежныеСредстваКВыплатеОстаткиИОбороты.ЗаявкаНаРасходованиеДенежныхСредств КАК ЗаявкаНаРасходованиеДенежныхСредств,
		|	СУММА(ДенежныеСредстваКВыплатеОстаткиИОбороты.СуммаПриход) - СУММА(ДенежныеСредстваКВыплатеОстаткиИОбороты.СуммаРасход) КАК СуммаОстаток
		|ИЗ
		|	РегистрНакопления.ДенежныеСредстваКВыплате.ОстаткиИОбороты(, , Регистратор, , ЗаявкаНаРасходованиеДенежныхСредств = &ЗаявкаНаРасходованиеДенежныхСредств) КАК ДенежныеСредстваКВыплатеОстаткиИОбороты
		|ГДЕ
		|	ДенежныеСредстваКВыплатеОстаткиИОбороты.Регистратор <> &Регистратор
		|
		|СГРУППИРОВАТЬ ПО
		|	ДенежныеСредстваКВыплатеОстаткиИОбороты.ЗаявкаНаРасходованиеДенежныхСредств";
	
		
	Запрос.УстановитьПараметр("Регистратор", ОбъектСписаниеБезналичныхДС.Ссылка);
	Запрос.УстановитьПараметр("ЗаявкаНаРасходованиеДенежныхСредств", ОбъектСписаниеБезналичныхДС.ДокументПланирования);
	...
	СуммаОстаток = СуммаОстаток - ОбъектСписаниеБезналичныхДС.СуммаДокумента;
	Если СуммаОстаток > 0 Тогда                                                                 
		НовоеСостояниеРазмещения = Перечисления.СостоянияИсполненияЗаявки.НеОбработана;
	ИначеЕсли НЕ ОбъектСписаниеБезналичныхДС.ПроведеноБанком Тогда
	...
	Возврат НовоеСостояниеРазмещения;
