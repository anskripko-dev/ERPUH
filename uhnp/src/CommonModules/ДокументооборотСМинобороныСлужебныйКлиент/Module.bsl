...
Процедура ПоискСертификатаПослеПолученияСвойствМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если Результат.Свойства = Неопределено Тогда
		ДокументооборотСМинобороныКлиентСервер.ВывестиОшибку(НСтр(СтрШаблон("ru = 'Сертификат криптографии не найден по причине: %1';
																			|en = 'Сертификат криптографии не найден по причине: %1'", Результат.ОписаниеОшибки), "ru"));
		...
		ДокументооборотСМинобороныКлиент.ЗаписатьВЖурналРегистрации(
			НСтр(
				СтрШаблон(
					...
					|en = 'Произошла ошибка при установке сертификата ''%1'' в хранилище ''%2'' c использованием компоненты'",
