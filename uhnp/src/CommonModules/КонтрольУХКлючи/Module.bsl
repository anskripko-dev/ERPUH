	...
	Для Каждого Реквизит Из ПараметрыКлюча.РегистрМетаданные.Измерения Цикл
		...
		Если Реквизит.Тип.Типы().Количество() = 1 Тогда
			...
			Если ТипИзмерения = Тип("Число") Тогда
				ПустоеЗначение = 0;
			ИначеЕсли ТипИзмерения = Тип("Дата") Тогда
				ПустоеЗначение = '00010101000000';
			ИначеЕсли ТипИзмерения = Тип("Строка") Тогда
				ПустоеЗначение = "";
			Иначе
...
// Создает ключи аналитики для временной таблицы, содержащей колонки ключа.
Процедура СоздатьКлючиДляВременнойТаблицы(ПараметрыКлюча, ИменаПолей, МенеджерВременныхТаблиц, ИмяВременнойТаблицы, ДополнитьКлюч = Ложь) Экспорт
	...
	//Иначе
		Если ИменаПолей = Неопределено Тогда
			ИменаПолей = ИменаПолейКоллекцииПоУмолчанию(ПараметрыКлюча);
		КонецЕсли;
		Запрос = Новый Запрос(ТекстЗначенияКлючейАналитикиВоВременнойТаблице(ПараметрыКлюча, ИменаПолей, ИмяВременнойТаблицы));
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Если Не ЗначениеЗаполнено(Выборка[ПараметрыКлюча.ИмяПоляАналитика]) Тогда
			КлючАналитики = ЗначениеКлючаАналитики(ПараметрыКлюча, Выборка);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры

// Процедура - Приводит колонку таблицы значений к типу
//
// Параметры:
...
//  Тип			 - ОписаниеТипов	 - 
//
Процедура ПривестиКолонкуКТипу(Коллекция, ИмяКолонки, Тип) Экспорт
	...
	СтараяКолонка = "_"+ИмяКолонки;
	Колонка.Имя = СтараяКолонка;
	Коллекция.Колонки.Добавить(ИмяКолонки, Тип);
	Значения = Коллекция.ВыгрузитьКолонку(СтараяКолонка);
	Коллекция.ЗагрузитьКолонку(Значения, ИмяКолонки);
	
КонецПроцедуры

// Обработчик подписки Ключи_ПриЗаписи. 
// Вызывается при записи объектов, включенных в один из ключей и предназначен для синхронизации пометки удаления объекта
// и ключа.
Процедура Ключи_ПриЗаписиПриЗаписи(Источник, Отказ) Экспорт
	...
	ИспользованиеВИзмеренияхКлючей = КонтрольУХПовтИсп.ИзмеренияКлючей()[ТипЗнч(Источник.Ссылка)];
	Если ИспользованиеВИзмеренияхКлючей = неопределено Тогда                                            
		Возврат;
	КонецЕсли;
	
	ТребуемаяПометка = Источник.ПометкаУдаления;
	
	//
	УстановитьПривилегированныйРежим(Истина);
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Значение", Источник.Ссылка);
	Запрос.УстановитьПараметр("ТребуемаяПометка", ТребуемаяПометка);
	Запрос.Текст = ТекстЗапроса_КлючиПоИспользованиюОбъекта(ИспользованиеВИзмеренияхКлючей);
	Результат = Запрос.Выполнить();
	...
	Пока Выборка.Следующий() Цикл
		...
		Объект.УстановитьПометкуУдаления(ТребуемаяПометка);
	КонецЦикла;
...
Функция ПолучитьПустоеЗначениеДляЗапроса(ПустоеЗначение)
	...
	ИначеЕсли ПустоеЗначение = 0 Тогда
		Возврат "0";
	ИначеЕсли ПустоеЗначение = '00010101000000' Тогда
...
Функция ТекстЗначенияКлючейАналитикиВоВременнойТаблице(ПараметрыКлюча, ИменаПолей, ИмяВременнойТаблицы)
	
	ПоляКоллекции = Новый Массив;
	ПоляГруппировки = Новый Массив;
	ПоляСоединения = Новый Массив;
	ШаблонПоля = "%1 КАК %2";
	ШаблонГруппировки= "Коллекция.%1";
	ШаблонСоединения = "Аналитика.%1 = Коллекция.%2";
	
	ПустоеЗначениеКлюча = СтрШаблон("ЗНАЧЕНИЕ(Справочник.%1.ПустаяСсылка)", ПараметрыКлюча.СправочникИмя);
	ПоляКоллекции.Добавить(СтрШаблон(ШаблонПоля, ПустоеЗначениеКлюча, ПараметрыКлюча.ИмяПоляАналитика));
	
	Для Каждого Измерение Из ПараметрыКлюча.мИзмерения Цикл
		ПоляКоллекции.Добавить(СтрШаблон(ШаблонПоля, "Коллекция."+ИменаПолей[Измерение], Измерение));
		ПоляСоединения.Добавить(СтрШаблон(ШаблонСоединения, Измерение, ИменаПолей[Измерение]));
		ПоляГруппировки.Добавить(Стршаблон(ШаблонГруппировки, ИменаПолей[Измерение]));
	КонецЦикла;
	...
	ТекстЗапроса = СтрШаблон(ШаблонЗапроса,
		...
		СтрСоединить(ПоляКоллекции, ","+Символы.ПС+Символы.Таб),
		ПараметрыКлюча.РегистрИмя,
		СтрСоединить(ПоляСоединения, Символы.ПС+Символы.Таб+Символы.Таб+Символы.Таб+"И "), //4
		ПараметрыКлюча.ИмяПоляАналитика, //5
		ПараметрыКлюча.СправочникИмя,
		СтрСоединить(ПоляГруппировки, ","+Символы.ПС+Символы.Таб)); //6
...
Функция ТекстЗапроса_КлючиПоИспользованиюОбъекта(ИспользованиеВИзмеренияхКлючей)
	...
	|ИЗ
	|	Справочник.%1 КАК Коллекция
	|ГДЕ
	|	Коллекция.%2 = &Значение
	|	И Коллекция.ПометкаУдаления <> &ТребуемаяПометка";
	
	Запросы = Новый Массив;
	
	Для Каждого Использование Из ИспользованиеВИзмеренияхКлючей Цикл
		Запросы.Добавить(СтрШаблон(ШаблонПодзапроса, Использование.Справочник, Использование.Реквизит));
	КонецЦикла;
	
	Возврат СтрШаблон(ШаблонЗапроса, СтрСоединить(Запросы, ОбщегоНазначенияОПК.ТекстОбъединитьВсеЗапроса()));
