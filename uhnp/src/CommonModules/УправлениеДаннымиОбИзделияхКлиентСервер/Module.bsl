...
// * Номенклатура - СправочникСсылка.Номенклатура,Строка - номенклатура
...
Функция РазвернутыеУровниДереваКэшКонструктор() Экспорт
	
	Результат = Новый Структура;
	
	Результат.Вставить("РазвернутыеУровни");
	Результат.Вставить("РежимРазвернутьВсе",    0);
	Результат.Вставить("КонтрольныйУровень",    50);
	Результат.Вставить("КонтрольныйУровеньШаг", 25);
	
	Возврат Результат;
	
КонецФункции

Процедура ПересчитатьКонтрольныйУровеньДерева(Форма, ДанныеСтроки = Неопределено) Экспорт
	
	РазвернутыеУровниДереваКэш = Форма.РазвернутыеУровниДереваКэш;
	
	Если ДанныеСтроки = Неопределено Тогда
		РазвернутыеУровниДереваКэш.КонтрольныйУровень = РазвернутыеУровниДереваКэш.КонтрольныйУровень
			+ РазвернутыеУровниДереваКэш.КонтрольныйУровеньШаг;
	ИначеЕсли ДанныеСтроки.Уровень > РазвернутыеУровниДереваКэш.КонтрольныйУровень Тогда
		РазвернутыеУровниДереваКэш.КонтрольныйУровень = РазвернутыеУровниДереваКэш.КонтрольныйУровеньШаг
			* (Цел(ДанныеСтроки.Уровень/РазвернутыеУровниДереваКэш.КонтрольныйУровеньШаг)+1);
	КонецЕсли;
...
Функция СтруктураЗначенияПараметраНазначенияСпецификаций() Экспорт
	...
	Результат.Вставить("Условие");
	Результат.Вставить("Значение");
	Результат.Вставить("Значение1");
	Результат.Вставить("Значение2");
