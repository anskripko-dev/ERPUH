...
Процедура УведомитьОбОтсутствииДоступа()
	...
	ПоказатьПредупреждение(, НСтр("ru = 'Проверка контрагентов сервисом ФНС не выполнена из-за ошибки подключения к сервису. Детальнее в журнале регистрации.';
									|en = 'Counterparty check is not performed by the FTS service due to the service connection error. For details, see the Event log.'"));
...
Процедура ОбработатьСостояниеКонтрагентаВСправочнике(Форма)
	...
	Иначе
		...
		Если Форма.РеквизитыПроверкиКонтрагентов.ИнтервалПроверкиРезультата <= 15 Тогда
