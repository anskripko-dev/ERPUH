...
Процедура ЗафиксироватьТипНоменклатуры(Форма, ИмяКнопкиФиксации, ИмяКолонкиНоменклатура) Экспорт
	...
	Иначе
		...
		СписокВсехТипов.Добавить(Тип("СправочникСсылка.Номенклатура"));
		СписокВсехТипов.Добавить(Тип("СправочникСсылка.ТоварныеКатегории")); 
		СписокВсехТипов.Добавить(Тип("СправочникСсылка.КатегорииЗакупок"));      
		
		Если ЗначениеЗаполнено(КолонкаНоменклатуры.ОграничениеТипа) Тогда 
			СписокТипов = Новый СписокЗначений;
			Для Каждого ТипСписка Из СписокВсехТипов Цикл        
				Если КолонкаНоменклатуры.ОграничениеТипа.СодержитТип(ТипСписка.Значение) Тогда  
					СписокТипов.Добавить(ТипСписка.Значение);
				КонецЕсли;
			КонецЦикла; 
		Иначе
			СписокТипов = СписокВсехТипов;	
		КонецЕсли;
...
Процедура ОбработкаЗаполненияПоступлениеТоваровУслуг(Источник, 
	...
	Форма = "Документ.ПоступлениеТоваровУслуг.Форма.ФормаВыбораСклада";										
	ОткрытьФорму(Форма);
КонецПроцедуры
...
Процедура ТиповойОбработчикСуммаТКПриИзменении(Форма,
											Контекст) Экспорт
	СтрокаТаблицы = Форма.Элементы[
		Контекст.ПараметрыВывода.ЭлементФормыКроссТаблицы].ТекущиеДанные;
	ЦентрализованныеЗакупкиКлиентСерверУХ.ОбработатьИзменениеСуммТККроссТаблицы(
		Контекст.Изменяемый.Объект.ЦенаВключаетНДС,
		Форма[Контекст.ИмяТаблицыПериодов],
		СтрокаТаблицы);
КонецПроцедуры
