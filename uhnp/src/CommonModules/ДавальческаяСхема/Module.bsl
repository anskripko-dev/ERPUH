...
Процедура Использование_ОстаткиПриемаВПереработку2_4(ПараметрыОбработчика) Экспорт
	...
	|
	|ВЫБРАТЬ
	|	ВЫРАЗИТЬ(ОстаткиТоваров.Распоряжение КАК Документ.ЗаказДавальца).Организация КАК Организация,
	...
	|															ОтборЗаказа.Распоряжение 
	|														ИЗ РегистрСведений.РаспоряженияНаОтгрузкуКВыполнению КАК ОтборЗаказа
	|														ГДЕ  ТИПЗНАЧЕНИЯ(ОтборЗаказа.Распоряжение) = ТИП(Документ.ЗаказДавальца))) КАК ОстаткиТоваров
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Константы КАК Константы
	|	ПО ИСТИНА
	|
	|ГДЕ
	|	НЕ Константы.ИспользоватьПроизводствоИзДавальческогоСырья
	|	И ВЫРАЗИТЬ(ОстаткиТоваров.Распоряжение КАК Документ.ЗаказДавальца).Организация В (&МассивОрганизаций)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Процедура ДополнитьТекстЗапросаОформлениеОтчетаДавальцу(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		...
		ОписаниеЗапроса.ТекстыЗапросов.Добавить(ТекстЗапроса, "ОформлениеОтчетаДавальцу");
...
Процедура ПроверитьКорректностьВыбораНазначенияДавальца(Объект, ПараметрыПроверки, Отказ) Экспорт
	...
	Если ПараметрыПроверки.ДавальческиеГруппировкиЗатрат.Найти(Объект.ГруппировкаЗатрат) <> Неопределено Тогда
		...
		ТекстСообщения =
			НСтр("ru = 'В строке %1 списка ""%2"" неправильно указана группа затрат.
					   |Материалы давальца должны быть включены в себестоимость продукции того же давальца.';
					   |en = 'In line %1 of the ""%2"" list, cost group is specified incorrectly.
