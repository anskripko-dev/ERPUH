...
Функция СкопироватьФайл(Источник, Приемник) Экспорт
	...
	ИначеЕсли Файл.ЭтоКаталог() Тогда
		...
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Источник%", Строка(Источник));	
		ПротоколируемыеСобытияУХ.ДобавитьЗаписьПримечание(ПРОТОКОЛИРУЕМОЕ_СОБЫТИЕ,,, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Путь %1 не является путем к файлу.'"), Источник), Источник);
		Возврат Ложь;
	КонецЕсли;
...
Функция ПоместитьФайлВоВременноеХранилище(ПутьКФайлу, Знач Адрес = Неопределено) Экспорт
	
	ПРОТОКОЛИРУЕМОЕ_СОБЫТИЕ = "ОбщийМодуль.РаботаСФайламиУХ.ПоместитьФайлВоВременноеХранилище";
