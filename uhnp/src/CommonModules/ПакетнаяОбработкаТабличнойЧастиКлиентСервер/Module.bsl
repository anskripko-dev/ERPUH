...
Функция ПоляСтрокиПоДействиям(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения, ДополнительныеПараметрыЗаполнения = Неопределено) Экспорт
	...
	Если СтруктураДействий.Свойство("ПроверитьЗаполнитьОбеспечениеВЭтапеПроизводства", СтруктураПараметровДействия) Тогда
		...
		ЧитаемыеПоля.Вставить("Производится");
		ЧитаемыеПоля.Вставить("Спецификация");
		ЧитаемыеПоля.Вставить("Склад");
		ЧитаемыеПоля.Вставить("ТипНоменклатуры");
		...
		ИзменяемыеПоля.Вставить("ВариантОбеспечения");
		ИзменяемыеПоля.Вставить("Обособленно");
		ИзменяемыеПоля.Вставить("Склад");
		
		ДополнитьОписаниеПолей(
			"ПроверитьЗаполнитьОбеспечениеВЭтапеПроизводства", ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	КонецЕсли;
	...
	Если СтруктураДействий.Свойство("ПроверитьЗаполнитьСкладПроизводства", СтруктураПараметровДействия) Тогда
		...
		ЧитаемыеПоля.Вставить("Номенклатура");
		ЧитаемыеПоля.Вставить("Характеристика");
		ЧитаемыеПоля.Вставить("Подразделение");
		...
		ДополнитьОписаниеПолей("ПроверитьЗаполнитьСкладПроизводства",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	КонецЕсли;
	//-- НЕ УТКА
	...
	Если СтруктураДействий.Свойство("ПроверитьЗаполнитьЦенуВыпуска") Тогда
		...
		ЧитаемыеПоля = Новый Структура();
		ЧитаемыеПоля.Вставить("ИспользоватьПлановуюСтоимость", Истина);
		ЧитаемыеПоля.Вставить("НомерСтроки");
		ЧитаемыеПоля.Вставить("Номенклатура");
		ЧитаемыеПоля.Вставить("Характеристика");
		ЧитаемыеПоля.Вставить("Серия", ПредопределенноеЗначение("Справочник.СерииНоменклатуры.ПустаяСсылка"));
		...
		ДополнитьОписаниеПолей("ПроверитьЗаполнитьЦенуВыпуска",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("УстановитьПризнакИспользованияПлановойСтоимости") Тогда
		...
		ЧитаемыеПоля.Вставить("Цена", 0);
		ЧитаемыеПоля.Вставить("Сумма", 0);
		ЧитаемыеПоля.Вставить("ДатаПроизводства");
		
		ИзменяемыеПоля = Новый Структура();
		ИзменяемыеПоля.Вставить("ИспользоватьПлановуюСтоимость", Ложь);
		
		ДополнитьОписаниеПолей("УстановитьПризнакИспользованияПлановойСтоимости",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ПроверитьЗаполнитьКлючСвязиСпецификации") Тогда
		...
		ИзменяемыеПоля = Новый Структура();
		ЧитаемыеПоля.Вставить("КлючСвязиСпецификация");
		
		ДополнитьОписаниеПолей("ПроверитьЗаполнитьКлючСвязиСпецификации",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ЗаполнитьНазначениеМатериалыВСтрокеСверхЗаказа") Тогда
		...
		ЧитаемыеПоля = Новый Структура();
		ЧитаемыеПоля.Вставить("ЗаказДавальца");
		ЧитаемыеПоля.Вставить("КодСтроки");
		ЧитаемыеПоля.Вставить("ТипНоменклатуры");
		...
		ДополнитьОписаниеПолей("ЗаполнитьНазначениеМатериалыВСтрокеСверхЗаказа",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ЗаполнитьПризнакДоступноОформлениеОтчетаДавальцу") Тогда
		...
		ИзменяемыеПоля = Новый Структура();
		ИзменяемыеПоля.Вставить("ДоступноОформлениеОтчетаДавальцу");
		ИзменяемыеПоля.Вставить("ОформитьОтчетДавальцу");
		
		ДополнитьОписаниеПолей("ЗаполнитьПризнакДоступноОформлениеОтчетаДавальцу",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ЗаполнитьВладелецИзделияДоступен") Тогда
		...
		ИзменяемыеПоля = Новый Структура();
		ИзменяемыеПоля.Вставить("ВладелецИзделияДоступен");
		ИзменяемыеПоля.Вставить("ВладелецИзделия");
		
		ДополнитьОписаниеПолей("ЗаполнитьВладелецИзделияДоступен", ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ЗаполнитьТипНазначения", СтруктураПараметровДействия) Тогда
		...
		Если ЗначениеЗаполнено(СтруктураПараметровДействия) Тогда
			...
			ИзменяемыеПоля.Вставить(СтруктураПараметровДействия.ИмяЗаполняемогоПоля);
			
			Если ЗначениеЗаполнено(СтруктураПараметровДействия.ИмяПоляТЧ) Тогда
				ЧитаемыеПоля.Вставить(СтруктураПараметровДействия.ИмяПоляТЧ);
			КонецЕсли;
			
		КонецЕсли;
		ДополнитьОписаниеПолей("ЗаполнитьТипНазначения",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ПересчитатьСуммуРеглЗабаланс") Тогда
		...
		ЧитаемыеПоля = Новый Структура();
		ЧитаемыеПоля.Вставить("СуммаЗабаланс", 0);
		
		ИзменяемыеПоля = Новый Структура();
		ИзменяемыеПоля.Вставить("СуммаРеглЗабаланс" , 0);
		
		ДополнитьОписаниеПолей("ПересчитатьСуммуРеглЗабаланс",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ПересчитатьЦенуЗабалансПоСуммеЗабаланс") Тогда
		...
		ИзменяемыеПоля = Новый Структура();
		ИзменяемыеПоля.Вставить("ЦенаЗабаланс" , 0);
		
		ДополнитьОписаниеПолей("ПересчитатьЦенуЗабалансПоСуммеЗабаланс",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	КонецЕсли;
	...
	Если СтруктураДействий.Свойство("ПересчитатьЦенуЗабалансЗаУпаковку") Тогда
		...
		ДополнитьОписаниеПолей("ПересчитатьЦенуЗабалансЗаУпаковку",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ЗаполнитьДействиеПоПерерабатываемымМатериалам") Тогда
		...
		ДополнитьОписаниеПолей("ЗаполнитьДействиеПоПерерабатываемымМатериалам",
			ИзменяемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
	...
	Если СтруктураДействий.Свойство("ПересчитатьЦенуПоСуммеЗаказДавальца", СтруктураПараметровДействия) Тогда
		
		ЧитаемыеПоля = Новый Структура();
		ЧитаемыеПоля.Вставить("СуммаУслуги");
		ИмяКоличества = "КоличествоУпаковок";
		Если ЗначениеЗаполнено(СтруктураПараметровДействия) Тогда
			ИмяКоличества = СтруктураПараметровДействия;
		КонецЕсли;
		...
		ДополнитьОписаниеПолей("ПересчитатьЦенуПоСуммеЗаказДавальца",
			ЧитаемыеПоля, ИзменяемыеПоля, КэшированныеЗначения);
