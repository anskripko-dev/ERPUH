...
Процедура ОбновитьНаименованиеЗаголовок(Объект, Форма = Неопределено) Экспорт
	...
	Форма.Заголовок = СтрШаблон("%1 %2 %3", Объект.Наименование, 
		?(ЗначениеЗаполнено(Объект.МеткаВремени), СтрШаблон(НСтр("ru = 'от %1'"), Формат(Объект.МеткаВремени, "ДЛФ=Д")), ""), 
...
Функция ПолучитьЗеркальныеСтрокиПриемники(СтруктураГруппы, СтрокаПриемник)
	...
	Иначе
		МассивСтрокПриемников = ПолучитьМассивСтрокОрганизационнойЕдиницы(
			СтрокаПриемник.ОрганизационнаяЕдиница, СтруктураГруппы);
	КонецЕсли;
...
Функция ПолучитьУзлыДерева(Дерево, Отбор, ТолькоТекущийУровень = Ложь)
	...
	ОбщегоНазначенияКлиентСерверУХ.ДополнитьКоллекциюИзДерева(
		СтрокиПриемники, Дерево, Отбор, ТолькоТекущийУровень);
...
Процедура УдалитьСтрокуСтруктурыГруппы(СтруктураГруппы, Строка) Экспорт
	...
	Иначе
		...
		ОбработатьСтрокуТЧ(
				...
				Новый Структура("ЗаполнитьКартинкуСтрок,ЗаполнитьЯвляетсяКонсолидирующей")
			);
		
		МассивСтрок = ПолучитьМассивСтрокОрганизационнойЕдиницы(
			ОрганизационнаяЕдиница,
			СтруктураГруппы,
			ТекущиеДанныеРодитель.ОрганизационнаяЕдиница);
...
Процедура ПереместитьСтрокуСтруктуруГруппы(СтруктураГруппы, Строка, Количество) Экспорт
	...
	Иначе
		...
		Если НЕ (Индекс + Количество < 0 ИЛИ Индекс + Количество > ТекущиеЭлементы.Количество() - 1) Тогда
			...
			МассивСтрок = РегламентыКлиентСервер.ПолучитьМассивСтрокОрганизационнойЕдиницы(
				Строка.ОрганизационнаяЕдиница,
				СтруктураГруппы,
				Родитель.ОрганизационнаяЕдиница);
...
Функция ДополнитьСтруктуруГруппы(СтруктураГруппы, ИдентификаторСтрокаПриемник,
	ПараметрыПеретаскивания, КэшПеретаскивания = Неопределено) Экспорт
	...
	ЗаполнитьОтношениеКГруппе = ОбщегоНазначенияКлиентСервер
		.СвойствоСтруктуры(КэшПеретаскивания, "ЗаполнитьОтношениеКГруппе");
...
Процедура ЗаполнитьВалюты(СтрокаТЧ, СтруктураДействий, КэшируемыеЗначения)
	...
	Если Не КэшируемыеЗначения.Свойство("ТекущийРодитель", ТекущийРодитель) 
		...
		Если ТипЗнч(СтрокаТЧ) = Тип("ДанныеФормыЭлементДерева") 
			И СтрокаТЧ.ПолучитьРодителя() <> Неопределено Тогда
			ТекущийРодитель = СтрокаТЧ.ПолучитьРодителя();
		КонецЕсли;
		
	КонецЕсли;
	
	ОбновитьДочерние = (РеквизитыДействия <> Неопределено)
		И РеквизитыДействия.Свойство("ОбновитьДочерние") 
		И (РеквизитыДействия.ОбновитьДочерние = Истина);
		
	ЕстьКэш	= КэшируемыеЗначения.Свойство("РеквизитыОрганизаций", РеквизитыОрганизаций);
		
	Если Не ЕстьКэш
		ИЛИ КэшируемыеЗначения.РеквизитыОрганизаций.Получить(СтрокаТЧ.ОрганизационнаяЕдиница) = Неопределено Тогда
		
		Организации = СтрокаТЧ.ОрганизационнаяЕдиница;
		Если Не ЕстьКэш И ОбновитьДочерние Тогда
		...
		Если КэшируемыеЗначения.Свойство("РеквизитыОрганизаций") Тогда
			...
			ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(
				КэшируемыеЗначения.РеквизитыОрганизаций, РеквизитыОрганизаций, Истина);
	...
	ОбновитьВалютыРекурсивно(СтрокаТЧ, ТекущийРодитель, РеквизитыОрганизаций, ОбновитьДочерние, КэшируемыеЗначения);
...
Процедура ОбновитьВалютыРекурсивно(СтрокаТЧ, ТекущийРодитель, РеквизитыОрганизаций, ОбновитьДочерние = Истина, КэшируемыеЗначения = Неопределено)
	...
	РеквизитыОрганизации = ПолучитьВалютыОрганизации(СтрокаТЧ, РеквизитыОрганизаций, КэшируемыеЗначения);
	
	ИсточникДерево = ОбщегоНазначенияКлиентСерверУХ.ЭтоУзелДерева(ТипЗнч(СтрокаТЧ));
	
	Если Не ЗначениеЗаполнено(СтрокаТЧ.ОсновнаяВалюта) Тогда
		СтрокаТЧ.ОсновнаяВалюта = РеквизитыОрганизации.ОсновнаяВалюта;
	КонецЕсли;
	
	УдалитьОсновнуюВалютуИзДополнительных(СтрокаТЧ);
	
	УзлыПоОрганизации = ПолучитьМассивСтрокОрганизационнойЕдиницы(
		СтрокаТЧ.ОрганизационнаяЕдиница, КэшируемыеЗначения.СтруктураГруппы);
		
	УзлыРодители = ПолучитьУзлыРодителиПоУзлам(УзлыПоОрганизации);
	
	ДобавитьДополнительнуюВалюту(СтрокаТЧ.ДополнительныеВалюты, СтрокаТЧ.ОсновнаяВалюта, РеквизитыОрганизации.ОсновнаяВалюта);
	...
	Для Каждого ТекущийРодитель Из УзлыРодители Цикл
	...
	ЗаполнитьВалютыСимметричныхСтрок(СтрокаТЧ, УзлыПоОрганизации);
	...
	Для Каждого ДочерняяСтрока Из СтрокиДочерние Цикл
		ОбновитьВалютыРекурсивно(ДочерняяСтрока, СтрокаТЧ, РеквизитыОрганизаций, ОбновитьДочерние, КэшируемыеЗначения);
	КонецЦикла;

КонецПроцедуры

Функция ПолучитьУзлыРодителиПоУзлам(Знач УзлыПоОрганизации)
	
	Перем УзелОрганизации, УзлыРодители;
	
	УзлыРодители = Новый Массив;
	Для Каждого УзелОрганизации Из УзлыПоОрганизации Цикл
		ДобавитьРодителейРекурсивно(УзлыРодители, УзелОрганизации);
	КонецЦикла;
	
	Возврат УзлыРодители;

КонецФункции

Функция ПолучитьВалютыОрганизации(СтрокаТЧ, РеквизитыОрганизаций, КэшируемыеЗначения)
	...
	Если РеквизитыОрганизации = Неопределено Тогда
		...
		ВалютаРУ = УчетМСФОПовтИспУХ.ВалютаРУ(СтрокаТЧ.ОрганизационнаяЕдиница);
		РеквизитыОрганизации = Новый Структура("ОсновнаяВалюта, ДополнительнаяВалюта", ВалютаРУ, ВалютаРУ);
		
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(РеквизитыОрганизации.ОсновнаяВалюта) Тогда
		
		ЕстьУПМСФО = Истина;
		Если Не КэшируемыеЗначения.Свойство("ЕстьУПМСФО", ЕстьУПМСФО) Тогда
			ЕстьУПМСФО = РегламентыВызовСервера.ЕстьУПМСФО();
			КэшируемыеЗначения.Вставить("ЕстьУПМСФО", ЕстьУПМСФО);
		КонецЕсли;
		
		Если ЕстьУПМСФО = Истина Тогда
		
			Шаблон = НСтр("ru = 'Для организации <%1> не указана учетная политика МСФО(<%2>). Будет использована валюта регламентированного учета'"); 
			Сообщить(СтрШаблон(Шаблон, СтрокаТЧ.ОрганизационнаяЕдиница, КэшируемыеЗначения.Сценарий));
		
		КонецЕсли;
		
		Если Не КэшируемыеЗначения.Свойство("ВалютаРУ") Тогда
			КэшируемыеЗначения.Вставить("ВалютаРУ",
				ОбщегоНазначенияУХВызовСервера.ПолучитьВалютуРегламентированногоУчета());
		КонецЕсли;
		
		РеквизитыОрганизации.Вставить("ОсновнаяВалюта", КэшируемыеЗначения.ВалютаРУ);
		
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(РеквизитыОрганизации.ДополнительнаяВалюта) Тогда
		
		Если Не КэшируемыеЗначения.Свойство("ВалютаУУ") Тогда
			КэшируемыеЗначения.Вставить("ВалютаУУ",
				ОбщегоНазначенияУХВызовСервера.ПолучитьВалютуУправленческогоУчета());
		КонецЕсли;
		
		РеквизитыОрганизации.Вставить("ДополнительнаяВалюта", КэшируемыеЗначения.ВалютаУУ);
...
Процедура ЗаполнитьВалютыСимметричныхСтрок(СтрокаТЧ, УзлыПоОрганизации)
	
	Для Каждого УзелПоОрганизации Из УзлыПоОрганизации Цикл
		
		Если СтрокаТЧ = УзелПоОрганизации Тогда
			// редактируемая строка - уже обновили валюты по родителям, 
			//добавленные валюты актуальны после редактирования
			Продолжить; 
		КонецЕсли;
		...
		ЗаполнитьЗначенияСвойств(
			УзелПоОрганизации, 
			СтрокаТЧ,
			"ОсновнаяВалюта, ДополнительныеВалюты");
		
	КонецЦикла;

КонецПроцедуры

Процедура ДобавитьРодителейРекурсивно(МассивРодителей, ТекущийЭлемент)
	
	ТипИсточника = ТипЗнч(ТекущийЭлемент);
	ЭтоДеревоЗначений = ОбщегоНазначенияКлиентСерверУХ.ЭтоУзелДерева(ТипИсточника);

	Если ЭтоДеревоЗначений Тогда
		РодительЭлемента = ТекущийЭлемент.Родитель;
	Иначе
		РодительЭлемента = ТекущийЭлемент.ПолучитьРодителя();
	КонецЕсли;
...
Процедура УдалитьОсновнуюВалютуИзДополнительных(СтрокаТЧ)
...
Процедура ЗаполнитьМетодИсключенияВГО(СтрокаТЧ, СтруктураДействий, КэшируемыеЗначения) Экспорт
	...
	РеквизитыОрганизации = ОбщегоНазначенияУХ.ПолучитьЗначенияРеквизитов(
		СтрокаТЧ.ОрганизационнаяЕдиница,
		"ИспользоватьВРегламентированномУчете,ИностранныйНалоговыйРезидент");
	
	Если РеквизитыОрганизации.ИспользоватьВРегламентированномУчете
			Или РеквизитыОрганизации.ИностранныйНалоговыйРезидент Тогда
		
		ВидОтношенияКГруппе = ОбщегоНазначенияУХ.ПолучитьЗначениеРеквизита(
			СтрокаТЧ.ОтношениеКГруппе, "ВидОтношенияКГруппе");
...
Функция ПолучитьМассивСтрокОрганизационнойЕдиницы(ОрганизационнаяЕдиница, СтруктураГруппы, Родитель = Неопределено) Экспорт 
	...
	ОбщегоНазначенияКлиентСерверУХ.ДополнитьКоллекциюИзДерева(
		МассивСтрок,
		СтруктураГруппы,
		Новый Структура("ОрганизационнаяЕдиница", ОрганизационнаяЕдиница));
