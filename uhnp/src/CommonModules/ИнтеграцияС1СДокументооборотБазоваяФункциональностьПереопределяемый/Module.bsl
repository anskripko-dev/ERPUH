...
Процедура ПриИзмененииСостоянияСогласования(ПредметСогласования, Состояние, ВызовИзФормыОбъекта, Установил,
	...
	Если Метаданные.ОпределяемыеТипы.Договор.Тип.СодержитТип(ТипЗнч(ПредметСогласования)) 
		 И ОбщегоНазначения.ЕстьРеквизитОбъекта("ВерсияСоглашения", ПредметСогласования.Метаданные()) Тогда 
		 
		ВерсияСоглашения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПредметСогласования,"ВерсияСоглашения");
		
		Если Состояние = Перечисления.СостоянияСогласованияВДокументообороте.НаСогласовании Тогда
			СтатусВход = Перечисления.СостоянияСогласования.НаУтверждении;
		ИначеЕсли Состояние = Перечисления.СостоянияСогласованияВДокументообороте.НеСогласован Тогда 
			СтатусВход = Перечисления.СостоянияСогласования.Отклонена;
		ИначеЕсли Состояние = Перечисления.СостоянияСогласованияВДокументообороте.Согласован Тогда
			СтатусВход = Перечисления.СостоянияСогласования.Согласовано;
		КонецЕсли;
		Если ЗначениеЗаполнено(СтатусВход) Тогда
			УправлениеПроцессамиСогласованияУХ.ПеревестиЗаявкуВПроизвольноеСостояние(ВерсияСоглашения, СтатусВход);
		КонецЕсли;
