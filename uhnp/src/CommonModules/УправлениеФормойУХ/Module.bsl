...
Функция ВосстановитьНастройки(Форма, Настройки, ОчищатьНастройки = Истина, ОтменятьВосстановлениеПриОшибке = Ложь) Экспорт
	...
	Для Каждого ЭлементНастроек Из ЗначениеИзСтрокиВнутр(ЗначениеВСтрокуВнутр(Настройки)) Цикл
		...
		Если Реквизит = Неопределено Тогда
			ТекстСообщения = НСтр("ru = 'У формы %1 не найден реквизит %2'");
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Форма.ИмяФормы, ЭлементНастроек.Ключ);
			ПротоколируемыеСобытияУХ.ДобавитьЗаписьПредупреждение(ПРОТОКОЛИРУЕМОЕ_СОБЫТИЕ,,, ТекстПредупреждения);
			Продолжить;
		КонецЕсли;
		...
		Если НЕ ЭлементНастроек.Значение = Неопределено Тогда
			Если ТипыРеквизита.Количество() > 0 И ТипыРеквизита.Найти(ТипЗнч(ЭлементНастроек.Значение)) = Неопределено Тогда
				ТекстСообщения = НСтр("ru = 'У формы %1 не восстановлен реквизит %2: Различаются типы значения у сохранненого значения (%3) и реквизита формы (%4)'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Форма.ИмяФормы, Реквизит.Имя, ТипЗнч(ЭлементНастроек.Значение), Реквизит.ТипЗначения);
		...
		Попытка
			...
			КонецЕсли;
		Исключение
			ТекстСообщения = НСтр("ru = 'У формы %1 не удалось восстановить реквизит %2'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Форма.ИмяФормы, Реквизит.Имя);	
			ПротоколируемыеСобытияУХ.ДобавитьЗаписьОшибка(ПРОТОКОЛИРУЕМОЕ_СОБЫТИЕ,,, ТекстОшибки, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			БылиОшибки = Истина;
		КонецПопытки;
	...
	Если БылиОшибки И ОтменятьВосстановлениеПриОшибке Тогда
		...
		Если НЕ ВосстановитьНастройки(Форма, НачальныеНастройкиФормы, Ложь) Тогда
			...
			ТекстСообщения = НСтр("ru = 'После неудачной загрузки настроек не удалось выполнить откат к прежнему состоянию формы %1'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Форма.ИмяФормы);
