...
Функция СоответствиеОбъектовВыбораВладельцам() Экспорт
	...
	// Выбор предложений поставщиков в лоте.
	ОбъектыИВладельцы.Вставить(Тип("ДокументСсылка.Лот"),"ДокументСсылка.ПредложениеПоставщика");
...
Функция БаллПоЗначению(Значение, ОписаниеБаллов) Экспорт
	...
	Для ТекБалл = 0 По 5 Цикл
		Если Значение >= ОписаниеБаллов["От"+ТекБалл] И
			(Значение < ОписаниеБаллов["До"+ТекБалл] ИЛИ ТекБалл = 5 И Значение = ОписаниеБаллов["До"+ТекБалл]) 
			ИЛИ
			Значение <= ОписаниеБаллов["От"+ТекБалл] И
			(Значение > ОписаниеБаллов["До"+ТекБалл] ИЛИ ТекБалл = 5 И Значение = ОписаниеБаллов["От"+ТекБалл]) 
			Тогда
			Возврат ТекБалл;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьСписокВыбораИмяТипаОбъекта(СписокВыбораВход) Экспорт
	...
	Для Каждого ТекТипы Из ОписаниеТиповВладельцыОбъектовВыбора.Типы() Цикл 
		Если ТекТипы = Тип("СправочникСсылка.Лоты") Тогда
			Продолжить;
		КонецЕсли;
