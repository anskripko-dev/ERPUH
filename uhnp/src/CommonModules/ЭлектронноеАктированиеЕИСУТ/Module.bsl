...
Функция ТекстЗапросаИдентификаторГосКонтракта(ТекстЗапроса) Экспорт
	...
	//++ НЕ УТ
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИдентификаторГосКонтракта", "
			|	ВЫБОР КОГДА НЕ ДанныеДокумента.Договор.ГосударственныйКонтракт = ЗНАЧЕНИЕ(Справочник.ГосударственныеКонтракты.ПустаяСсылка) ТОГДА
			|		ВЫБОР
			|			КОГДА ДанныеДокумента.Договор.ГосударственныйКонтракт.НомерГОЗ <> """" ТОГДА
			|				ДанныеДокумента.Договор.ГосударственныйКонтракт.НомерГОЗ
			|			КОГДА ДанныеДокумента.Договор.ГосударственныйКонтракт.НомерГОЗ = """"
			|					И ДанныеДокумента.Договор.ГосударственныйКонтракт.НомерИГК <> """" ТОГДА
			|				ДанныеДокумента.Договор.ГосударственныйКонтракт.НомерИГК
			|			ИНАЧЕ
			|				ДанныеДокумента.Договор.ИдентификаторГосКонтракта
			|		КОНЕЦ
			|	ИНАЧЕ
			|		ДанныеДокумента.Договор.ИдентификаторГосКонтракта
			|	КОНЕЦ");
...
Функция ПолучитьСведенияАдресаПоГАР(СведенияОбУчастнике, ВидАдреса)
	...
	Если КонтактнаяИнформация.Количество() = 0 Тогда
		...
		ИмяСобытия = НСтр("ru = 'ЭлектронноеАктированиеЕИС.ПриФормированииАдресаМестаПоставки';
							|en = 'Electronic UIS certification.When generating the delivery address'",
			КодОсновногоЯзыка);
		ПодробноеПредставлениеОшибки = НСтр("ru = 'Не удалось получить адрес участника.';
											|en = 'Cannot get the participant address.'",
			КодОсновногоЯзыка);
