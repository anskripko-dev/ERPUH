...
//
Функция ИндексыСПАРККонтрагента(Форма, Контрагент, ВидКонтрагента) Экспорт
...
Процедура ПерейтиВВебВерсиюСПАРК(
	...
	ИначеЕсли Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ЗапрещенДоступКИнтернетСервисам") Тогда
		
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<b>Переход в веб-версию СПАРК недоступен </b>
				|
				|Доступ к Интернет-сервисам в приложении запрещен администратором (""Интернет-поддержка и сервисы"" — ""Разрешить доступ к Интернет-сервисам"").';
				|en = '<b>Cannot go to SPARK online</b>
				|
				|Access to web services in the application is prohibited by the administrator (Online support and services > Allow access to web services).'"));
...
Функция ЗапроситьНовуюСправкуВСервисеСПАРКРиски(Контрагент) Экспорт
	Возврат СПАРКРискиВызовСервера.ЗапроситьНовуюСправкуВСервисеСПАРКРиски(Контрагент);
КонецФункции

Процедура ОткрытьСправкуСПАРКРиски(
		Результат,
		СправкаСсылка,
	...
	РезультатПолученияДанныхФайла = СПАРКРискиВызовСервера.ДанныеДляПросмотраФайлаСправки(
		СправкаСсылка,
...
Процедура ОбработатьОшибку(РезультатДействия, ОповещениеПриПодключенииИнтернетПоддержки) Экспорт
	...
	ИначеЕсли РезультатДействия.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ЗапрещенДоступКИнтернетСервисам") Тогда
	...
	ИначеЕсли РезультатДействия.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ТребуетсяОплатаИлиПревышенЛимит")
		...
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"ПодключитьТестовыйПериодИзСправкиЗавершение",
			ЭтотОбъект,
			ДополнительныеПараметры);
...
Процедура ПодключитьТестовыйПериодИзСправкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = ПредопределенноеЗначение("Перечисление.СостоянияПодключенияСервисов.Подключен") Тогда
		ВыполнитьОбработкуОповещения(
			ДополнительныеПараметры.ОповещениеПриПодключенииИнтернетПоддержки);
...
Функция АдресСтраницыПокупкиСервисаНаПортале1С()
	...
	Возврат ИнтернетПоддержкаПользователейКлиентСервер.URLСтраницыПорталаПоддержки(
		"/public/purchase/1C-Spark-risks/create");
	
КонецФункции

Функция АдресСтраницыВыбораПартнераДляПокупкеСервиса()
	
	Возврат "https://its.1c.ru/zakaz";
...
Функция АдресСтраницыПереходовВВебВерсиюСПАРК() Экспорт
	...
	Возврат ИнтернетПоддержкаПользователейКлиентСервер.URLСтраницыПорталаПоддержки(
		"/application/list/1C-Spark-risks/received-cards-info");
