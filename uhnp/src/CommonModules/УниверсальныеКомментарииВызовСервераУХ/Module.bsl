...
Функция ПодготовитьОписаниеПредметаКомментированияВГО(ПредметКомментирования, Реквизиты, ТекстОшибки) Экспорт
	...
	Если Константы.ОповеститьОКомментарииРасхожденияВГО.Получить() Тогда
		...
		ТаблицаАдресов = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(мОрганизаций,, 
						ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыКонтактнойИнформации.EmailОрганизации")); //СверкаВГОВызовСервераУХ.ПочтаОтветственныхЛицОрганизации(Реквизиты.Сценарий, Реквизиты.ПериодСценария, Реквизиты.Этап, мОрганизаций);
