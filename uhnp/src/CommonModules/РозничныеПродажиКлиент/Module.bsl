...
Процедура СообщитьОбОшибкеПриВыполненииОперацииНаУстройстве(Устройство, ТокенОперации, ОписаниеОшибки, ПутьКТабличнойЧасти="") Экспорт
	...
	Иначе
		ТекстСообщения = НСтр("ru = 'При выполнении операции на устройстве %Устройство% произошла ошибка:
		                            |""%ОписаниеОшибки%"".';
		                            |en = 'An error occurred when performing the operation on the %Устройство% device:
		                            |""%ОписаниеОшибки%"".'");
	КонецЕсли;
...
Процедура СообщитьОбОшибкеПриПечатиЧека(Форма, РезультатВыполнения, ЗаписатьВЖурналРегистрации = Истина) Экспорт
	Если Форма.ИмяФормы = РозничныеПродажиКлиентСервер.ИменаФормРМК().Найти(Форма.ИмяФормы) <> Неопределено Тогда
		...
		ТекстСообщения = НСтр("ru = 'При печати чека произошла ошибка. Чек не напечатан на фискальном устройстве.
									...
									|en = 'При печати чека произошла ошибка. Чек не напечатан на фискальном устройстве.
									|Дополнительное описание: %ДополнительноеОписание%'");
		...
		Если ГиперссылкаПоказатьПолностью Тогда
			СтрокаПоказатьПолностью = Новый ФорматированнаяСтрока(НСтр("ru = '(показать полностью)';
																		|en = '(показать полностью)'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),,,, "ПоказатьПолностьюОшибкуПриПечатиЧека");
