...
Функция ИмяТипаИзПолногоИмени(ПолноеИмя, ЭтоСсылка = Истина, ВызыватьИсключение = Истина) Экспорт
	...
	Если СоставИмени.Количество() < 2
		...
		Если ВызыватьИсключение Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Не удалось определить имя типа из строки ""%1""';
											|en = 'Cannot identify the type name from the ""%1"" string'"), ПолноеИмя);
		Иначе
			Возврат "";
		КонецЕсли;
	КонецЕсли;
...
Функция СообщениеОбОтсутствииПредпросмотра(ЗаголовокСообщенияВОбластиПредпросмотра) Экспорт
	...
	Возврат СообщениеВПредпросмотр(
		НСтр("ru = 'Данные предпросмотра отсутствуют';
			|en = 'No preview data'"), ЗаголовокСообщенияВОбластиПредпросмотра);
	...
	Возврат СообщениеВПредпросмотр(
		СтрШаблон("%1<br>
			...
			НСтр("ru = 'Данный файл занят вами на редактирование';
				|en = 'You locked this file for editing'"),
			НСтр("ru = 'Открыть';
				|en = 'Open'")),
...
Функция СтатусЭП(Неверна, ОписаниеОшибки) Экспорт
	...
	Возврат ?(Неверна, СтрШаблон(НСтр("ru = 'Недействительна: %1';
										|en = 'Invalid: %1'"), ОписаниеОшибки), НСтр("ru = 'Действительна';
																							|en = 'Valid'"));
