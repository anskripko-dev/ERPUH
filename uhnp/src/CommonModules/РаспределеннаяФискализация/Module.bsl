...
Функция ЧекиВОчередиНаФискализацию(КассаККМ = Неопределено, ОблачнаяККТ = Ложь) Экспорт
	...
	Запрос.Текст = 
		...
		|	РегистрСведений.ОчередьЧековККТ КАК ОчередьЧековККТ
		|ГДЕ
		|	(ОчередьЧековККТ.ДокументОснование, ОчередьЧековККТ.ИдентификаторЗаписи) В
		|			(ВЫБРАТЬ
		|				ОчередьЧековККТ.ДокументОснование КАК ДокументОснование,
		|				ОчередьЧековККТ.ИдентификаторЗаписи КАК ИдентификаторЗаписи
		|			ИЗ
		|				ИдентификаторыЧековВОчереди КАК ОчередьЧековККТ)
		|
		|УПОРЯДОЧИТЬ ПО
		|	ОчередьЧековККТ.Дата УБЫВ";
	
	ИдентификаторыЧековВОчереди = ИдентификаторыЧековВОчереди(МенеджерВременныхТаблиц, КассаККМ, ОблачнаяККТ);
	
	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ОчередьЧековККТ");
	ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
	ЭлементБлокировки.ИсточникДанных = ИдентификаторыЧековВОчереди;
...
Процедура ЗаписатьСтатусЧекаВОчереди(ПараметрыФискализации, СтатусЧека, ОборудованиеККТ = Неопределено, ТекстОшибки = Неопределено, СтатусОперацииВСервисе = Неопределено, ОбъектОбработан = Истина) Экспорт
	...
	Если ОборудованиеККТ <> Неопределено И ОборудованиеККТ.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ОблачнаяККТ Тогда
		ПараметрыФискализации.ОблачнаяККТ = Истина;
	КонецЕсли;
...
Функция ИдентификаторыЧековВОчереди(МенеджерВременныхТаблиц, КассаККМ = Неопределено, ОблачнаяККТ = Ложь)
	...
	|	ОчередьЧековККТ.ДокументОснование КАК ДокументОснование,
	|	ОчередьЧековККТ.ИдентификаторЗаписи КАК ИдентификаторЗаписи
	|ПОМЕСТИТЬ ИдентификаторыЧековВОчереди
	|ИЗ
	|	РегистрСведений.ОчередьЧековККТ КАК ОчередьЧековККТ
	|ГДЕ
	|	&Условие
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ИдентификаторыЧековВОчереди.ДокументОснование КАК ДокументОснование,
	|	ИдентификаторыЧековВОчереди.ИдентификаторЗаписи КАК ИдентификаторЗаписи
	|ИЗ
	|	ИдентификаторыЧековВОчереди КАК ИдентификаторыЧековВОчереди";
