...
Функция ПолучитьТаблицуДительностиСтадийЗакупки(СпособЗакупкиВход, ДополнительныеПараметрыВход) Экспорт
	...
	ШаблонМероприятия = Справочники.ШаблоныМероприятий.ВернутьШаблонЗакупки(?(ТипЗНЧ(СпособЗакупкиВход)=ТИП("СправочникСсылка.СпособыВыбораПоставщика"),СпособЗакупкиВход.СпособыВыбораПоставщика,СпособЗакупкиВход));
	Если ЗначениеЗаполнено(ШаблонМероприятия) Тогда
		...
		Для Каждого ТекТаблицаСтадийИсходная Из ТаблицаСтадийИсходная Цикл
			...
			// Проверка Переторжки.
			ЭтоПереторжка = ЦентрализованныеЗакупкиУХ.ЭтоСтадияПереторжка(ТекСтадия);
			...
			ЭтоКвалификационныйОтбор = ЦентрализованныеЗакупкиУХ.ЭтоСтадияКвалификационногоОтбора(ТекСтадия);
