...
Процедура ЗаполнитьОтветственныхЛицДокумента(Объект, ДанныеЗаполнения = Неопределено, ПерезаполнятьНепустыеРеквизиты = Ложь)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("УникальныйИдентификатор") Тогда
		Возврат;			// Не требуется заполнять ответственных по уникальному идентификатору. Возможно, это идентификатор платежной позиции.
	Иначе
		// Проверка выполнена успешно, продолжаем.
	КонецЕсли;
	#КонецОбласти
