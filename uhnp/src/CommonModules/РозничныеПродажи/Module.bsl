	...
	Если Выборка.Следующий() Тогда
		ПокупательПерсональныеДанные = СокрЛП(Выборка.Серия) + " " + СокрЛП(Выборка.Номер) + " " + СокрЛП(Выборка.КемВыдан)
			+ " " + Формат(Выборка.ДатаВыдачи, НСтр("ru = 'ДФ=dd.MM.yyyy';
													|en = 'ДФ=ММ/дд/гггг'")) + " " + Формат(Выборка.СрокДействия, НСтр("ru = 'ДФ=dd.MM.yyyy';
																													|en = 'ДФ=ММ/дд/гггг'"));
	КонецЕсли;
		...
		Если ОтчетОРозничныхПродажах <> Неопределено
			...
			Исключение
				...
				ОписаниеОшибки = НСтр("ru = 'При дозаполнении в ОРВ и ОРП данными возврата в смену с измененным видом оплаты по кассовой смене %КассоваяСмена% произошла ошибка.
		                                     |Дополнительное описание:
		                                     |%ДополнительноеОписание%';
		                                     |en = 'При дозаполнении в ОРВ и ОРП данными возврата в смену с измененным видом оплаты по кассовой смене %КассоваяСмена% произошла ошибка.
		                                     |Дополнительное описание:
		                                     |%ДополнительноеОписание%'");
				ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%КассоваяСмена%", СостояниеКассовойСмены.КассоваяСмена);
				ОписаниеОшибки = СтрЗаменить(
