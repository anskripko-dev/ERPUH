...
Функция ЗначениеВСтроку(Знач Значение, Знач Отступ = "") Экспорт
	...
	Если ТипЗначения = Тип("СписокЗначений") Тогда
		Стр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1Список значений(%2): %3'"), Стр + Отступ, Значение.Количество(), 
		      	Символы.ПС);
	...
	ИначеЕсли ТипЗначения = Тип("ТаблицаЗначений") Тогда
		Стр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1Таблица значений (строк %2, колонок %3): %4'"), Стр + Отступ, Значение.Количество(), 
		      	Значение.Колонки.Количество(), Символы.ПС);
		Ном = 1;
		Для каждого Значение1 Из Значение Цикл
			Стр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1	Строка №%2 = %3'"), Стр + Отступ, Ном, Символы.ПС);
			Ном = Ном + 1;
		...
		Стр = Сред(Стр, 1, СтрДлина(Стр)-1);
	Иначе
		Стр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1(тип: %2)'"), Стр + Отступ + Строка(Значение), Строка(ТипЗначения));
	КонецЕсли;
	Возврат Стр;
КонецФункции
...
//
Функция ДобавитьИнтервал(ДатаПериода, Периодичность, Смещение, ПриводитьКНачалуПериода = Истина) Экспорт
	...
	ИначеЕсли Периодичность = Перечисления.Периодичность.Неделя Тогда 
		Если ПриводитьКНачалуПериода Тогда
	...
	ИначеЕсли Периодичность = Перечисления.Периодичность.Декада Тогда
		...
		Иначе 
			НоваяДатаПериода = ДатаПериода + Смещение*11*24*3600;
		КонецЕсли;
...
// Получает ссылку на справочник Периоды по заданным датам начала и окончания периода
//
// Параметры:
...
// Получает ссылку на справочник Периоды по заданной дате и периодичности
//
// Параметры:
...
Функция ПолучитьПериодСтрокой(ДатаВПериоде, Периодичность) Экспорт
    ...
    Если Периодичность = "Неделя" И ТипЗнч(ДатаВПериоде) = Тип("Дата") Тогда
        СтрокаПериод = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1-ая %2%3'"), НеделяГода(ДатаВПериоде), СтрокаПериод, Формат(КонецНедели(ДатаВПериоде), "ДФ='"" - ""дд.ММ.гггг)'"));
    КонецЕсли;
...
Функция ИндексПеречисления(ЗначениеПеречисления, ИмяПеречисления) Экспорт
	...
	МетаданныеПеречисления = Метаданные.Перечисления[ИмяПеречисления].ЗначенияПеречисления;
	Для Сч = 0 По МетаданныеПеречисления.Количество()-1 Цикл
		Если ЗначениеПеречисления = Перечисления[ИмяПеречисления][МетаданныеПеречисления.Получить(Сч).Имя] Тогда
			Возврат Сч;	    
		КонецЕсли;
	КонецЦикла;
...
Функция МинимальнаяПериодичность(Периодичность1,Периодичность2) Экспорт
	...
	Если ИндексПеречисления(Периодичность1, "Периодичность") < ИндексПеречисления(Периодичность2, "Периодичность") Тогда
		Возврат Периодичность1;
	Иначе
...
Функция глОтносительныйПериод(локПериодОтчета, Смещение, ТихийРежим = Ложь, Реструктурировать = Истина) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(локПериодОтчета) Тогда
		Если НЕ ТихийРежим Тогда
			СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Невозможно определить аналогичный период года (смещение = %1): исходный период не задан '"), Смещение), , ,СтатусСообщения.Важное);
		КонецЕсли;
		Возврат Справочники.Периоды.ПустаяСсылка();
	КонецЕсли;
...
Функция глАналогичныйПериод(локПериодОтчета, Смещение) Экспорт
	...
	ИначеЕсли локПериодОтчета.Периодичность = Перечисления.Периодичность.Неделя Тогда
		
		ДатаНачалаОтносительногоПериода		= НачалоНедели(ДобавитьМесяц(локПериодОтчета.ДатаНачала, 12 * Смещение));
		Если Месяц(локПериодОтчета.ДатаНачала) <> Месяц(ДатаНачалаОтносительногоПериода) Тогда
			ДатаНачалаОтносительногоПериода = НачалоМесяца(ДатаНачалаОтносительногоПериода + 7 * 86400);		    
		КонецЕсли;		                                                                       
	...
	Если Результат.Пустой() Тогда
		СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Невозможно определить аналогичный период (смещение = %1): не существует искомый период в справочнике ""Периоды"" '"), Смещение), , , СтатусСообщения.Важное);
		Возврат Справочники.Периоды.ПустаяСсылка();
	Иначе
...
Функция ОпределитьПериодПоДатеЭкспорт(ДатаВПериоде, Периодичность) Экспорт

	ОпределитьПериодПоДате(ДатаВПериоде, Периодичность);
...
Функция ПолучитьЗначениеПоУмолчанию(Настройка, Знач Пользователь = Неопределено) Экспорт
	...
	Если Пользователь = Неопределено Тогда
		Пользователь = ПользователиСлужебный.АвторизованныйПользователь();//Пользователи.ТекущийПользователь();
	КонецЕсли;
	...
	Иначе
		Если ОбщегоНазначения.ПодсистемаСуществует("БухгалтерияПредприятияПодсистемы") Тогда
			...
			Возврат МодульБухгалтерскийУчетПереопределяемый.ПолучитьЗначениеПоУмолчанию(Настройка, Пользователь);
		ИначеЕсли НастройкаВРег = ВРег("ОсновнаяОрганизация") Тогда
			Возврат ЦФОПоУмолчанию(Пользователь);
		КонецЕсли;
	КонецЕсли;
...
Функция ЦФОПоУмолчанию(Пользователь)
...
Функция ЯзыкПоУмолчанию(Пользователь)

	ОсновнойЯзык = ХранилищеОбщихНастроек.Загрузить("ОсновнойЯзык",,, Пользователь);

	Если ОсновнойЯзык <> Неопределено Тогда
		Возврат ОсновнойЯзык;
	КонецЕсли;
	
	Язык = ПользователиИнформационнойБазы.ТекущийПользователь().Язык;
	Если Язык = Неопределено ИЛИ Язык = ОбщегоНазначенияПовтИспУХ.ПолучитьЗначениеКонстанты("ОсновнойЯзык") Тогда
		Возврат 0;
	КонецЕсли;
...
Функция ПроектПоУмолчанию(Пользователь)
...
Функция ПравоДоступаКОбъекту(Право, ТипОбъектаМетаданных, ИмяОбъектаМетаданных) Экспорт

	ОбъектМетаданных = Метаданные[ТипОбъектаМетаданных].Найти(ИмяОбъектаМетаданных);
...
Функция ПолучитьОписаниеТиповПоОбъекту(Объект) Экспорт
	...
	Если Метаданные.ПланыВидовХарактеристик.ВидыСубконтоКорпоративные.Тип.СодержитТип(ТипОбъекта) Тогда
		
		Возврат Метаданные.ПланыВидовХарактеристик.ВидыСубконтоКорпоративные.Тип;
		
	Иначе
		
		МассивТипов=Метаданные.ПланыВидовХарактеристик.ВидыСубконтоКорпоративные.Тип.Типы();
...
Функция ВыполнитьПреобразованиеТипов(Значение,ИсходныйТип,РезультирующийТип) Экспорт
	...
	Если ИсходныйТип="Строка" и РезультирующийТип="Дата" Тогда
		...
		ИначеЕсли СтрНайти(ДанныеДня,"/")>0 Тогда
			...
			Месяц=Число(?(СтрДлина(ЧастиДня[0])=1,"0"+ЧастиДня[0],ЧастиДня[0]));
			День=Число(?(СтрДлина(ЧастиДня[1])=1,"0"+ЧастиДня[1],ЧастиДня[1]));
...
Функция ПроверитьТипЗначенияВПодсистеме(Значение, ТипЗначения, ИмяПодсистемы) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует(ИмяПодсистемы) Тогда
		Возврат ТипЗнч(Значение) = Тип(ТипЗначения);
	Иначе
		Возврат Ложь;
	КонецЕсли;
	
КонецФункции // ПроверитьТипЗначенияВПодсистеме()

Функция СодержитТипВПодсистеме(ОписаниеСоставногоТипа, ПроверяемыйТип, ИмяПодсистемы) Экспорт
	
	Если ОбщегоНазначения.ПодсистемаСуществует(ИмяПодсистемы) Тогда
		Возврат ОписаниеСоставногоТипа.СодержитТип(Тип(ПроверяемыйТип));
	Иначе
		Возврат Ложь;
	КонецЕсли;
	
КонецФункции // СодержитТипВПодсистеме()
...
Функция СостояниеДокумента(Объект) Экспорт
	Возврат ВстраиваниеУХ.СостояниеДокумента(Объект);
КонецФункции
...
// Функция - Текст объединить запросы
//
// Параметры:
//  ТекстЗапроса - Текст - Текст исходного запроса
//	Отступ - Число - Число табуляций для отступа
// 
// Возвращаемое значение:
//   - Текст исходного запроса с текстом "ОБЪЕДИНИТЬ ВСЕ"
//
Функция ОбъединениеПакетаЗапросов(Знач ТекстЗапроса = Неопределено, Отступ = 0) Экспорт
	...
	Возврат ТекстЗапроса + Символы.ПС + Символы.ПС + ТекстОтступ + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС;	
...
#Область ОбработкиПодписок

Процедура ОбновитьСведенияОбИзмененииДокументов(Источник, Отказ) Экспорт
	
	МенеджерЗаписи = РегистрыСведений.СведенияОбИзмененииДокументов.СоздатьМенеджерЗаписи();
	МенеджерЗаписи.Объект = Источник.Ссылка;
	МенеджерЗаписи.ПоследнийРедактировавший = Пользователи.АвторизованныйПользователь();
	МенеджерЗаписи.ДатаИзменения = ТекущаяДатаСеанса();
	МенеджерЗаписи.Записать();
	
КонецПроцедуры

Процедура ОчиститьПовторноИспользуемыеЗначения(Источник, Отказ) Экспорт
...
Функция ПолучитьЗначениеПеременной(ИмяПараметра, Кэш = Неопределено, КэшИзменен = Ложь) Экспорт
	...
	Иначе
		СтрокаИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Невозможно обработать параметр ""%1"" для получения значения'"), 
		                   	ИмяПараметра);
...
Функция ПолучитьШапкуОшибки(ЗНАЧ ОбъектРасчета) Экспорт
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Вид отчета:%1; Сценарий: %2; Организация: %3; Период отчета: %4'"), 
	...
	Если ОбъектРасчета.Свойство("Проект") И ЗначениеЗаполнено(ОбъектРасчета.Проект) Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1; Проект: %2'"), ТекстОшибки, ОбъектРасчета.Проект);
	...
	Для Индекс=1 По ПараметрыСеанса.ЧислоДопАналитик Цикл
		...
		Если ОбъектРасчета.Свойство("Аналитика"+Индекс) И ЗначениеЗаполнено(ОбъектРасчета["Аналитика"+Индекс]) Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1; Аналитика%2: %3'"), ТекстОшибки, Индекс, ОбъектРасчета["Аналитика"+Индекс]);
	...
	Если ОбъектРасчета.Свойство("ПравилоОбработки") Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1; Правило расчета: %2'"), ТекстОшибки, ОбъектРасчета.ПравилоОбработки);
		
	КонецЕсли;
	
	Если ОбъектРасчета.Свойство("ИспользуемаяИБ") И ЗначениеЗаполнено(ОбъектРасчета.ИспользуемаяИБ) Тогда
		
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1; Используемая ИБ: %2'"), ТекстОшибки, ОбъектРасчета.ИспользуемаяИБ);
		
	КонецЕсли;
	
	Если ОбъектРасчета.Свойство("ПоказательОтчета") Тогда
		
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1; Показатель: %2'"), ТекстОшибки, ОбъектРасчета.ПоказательОтчета);
		...
		Если ТипЗнч(ИмяПеречисления)=Тип("Строка") Тогда
			...
			Исключение // Возможно, передан синоним
				...
				Попытка
					...
					Возврат ?(НЕ ЗначениеЗаполнено(НастройкаСоответствия),ПолучитьПустуюСсылкуПеречисление(ИмяОбъекта),ПолучитьЗначениеПеречисленияПоНастройке(ИмяОбъекта,ИмяПеречисления,НастройкаСоответствия));
	...
	Для каждого СтрокаТаблицы Из ТабличнаяЧасть Цикл
		...
		Если СтрокаВстречалась = Неопределено Тогда
			...
			Если ЗапретНезаполненныхЗначений и НЕ(ЗначениеЗаполнено(Значение)) Тогда
				...
				СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение реквизита ""%1"" в табличной части ""%2"" строка:%3 не может быть пустым'"), 
		...
		Иначе
			Если СтрокаВстречалась = 1 Тогда
				...
				СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Значение ""%1"" реквизита ""%2"" в табличной части ""%3"" не уникально'"), 
	...
	Для каждого ЭлементСтруктуры Из Объект.НеИзменяемыеРеквизиты() Цикл
		...
		Если Объект.Метаданные().ТабличныеЧасти.Найти(ИмяРеквизита) = Неопределено Тогда
			Если Объект[ИмяРеквизита] <> Объект.Ссылка[ИмяРеквизита] Тогда
				СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '- Запрещено изменение реквизита ""%1%2"".'"), 
		...
		Иначе
			Если ЗначениеВСтрокуВнутр(Объект[ИмяРеквизита].Выгрузить()) <> ЗначениеВСтрокуВнутр(Объект.Ссылка[ИмяРеквизита].Выгрузить()) Тогда
				СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '- Запрещено изменение табличной части ""%1%2"".'"), 
				                 	Объект.СтруктураРеквизитов, Объект.СтруктураРеквизитов[ИмяРеквизита]), Отказ,  ШапкаОшибки);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПометитьСправочникПоРеквизиту(ИмяСправочника,ИмяРеквизита,ЗначениеРеквизита,Пометка,Отказ) Экспорт
	...
	Если ТипЗнч(ИмяРеквизита)=Тип("Массив") Тогда
		...
		ТекстОтборПоРеквизитам="("+Сред(ТекстОтборПоРеквизитам,СтрДлина("ИЛИ")+2)+")";
...
Функция ИменаРеквизитовОбъекта(МетаданныеОбъекта) Экспорт
	...
	МассивОбщихРеквизитов=КэшируемыеПроцедурыУХ.ПолучитьИменаОбщихРеквизитов(МетаданныеОбъекта.Имя);
	
	Для Каждого Реквизит ИЗ МассивОбщихРеквизитов Цикл
		
		МассивИмен.Добавить(Реквизит);
...
Функция СформироватьЗаголовокДокумента(ДокументОбъект, НазваниеДокумента = "") Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 № %2 от %3 г.'"), НазваниеДокумента, Формат(ДокументОбъект.Номер,"ЧГ=0"), 
...
Функция ПолучитьДанныеОбъектаБД(ОписаниеОбъектаБД) Экспорт
	...
	Если ТипОбъектаМетаданных="Справочник" Тогда
		...
		Если НЕ ТаблицаАналитики="Справочник.ПроизвольныйКлассификаторУХ" Тогда 
...
Функция ЗаполнитьВидыАналитик(ОбъектАналитики, НеВключатьАналитикуВГО = Ложь, ДобавлятьПоказатели=Ложь,ПоказательОтбор=Неопределено) Экспорт
	...
	Иначе
		...
		Если ТипЗнч(ОбъектАналитики)=Тип("СправочникСсылка.ГруппыРаскрытия") Тогда
			...
			Если Не ПустаяСтрока(ВидАналитики) Тогда
				...
				НоваяСтрока.НаименованиеАналитики	= Строка(ВидАналитики);
				НоваяСтрока.ВидАналитики			= ВидАналитики;
				НоваяСтрока.ПредставлениеАналитики	= Нстр("ru = 'Валюта'");
				НоваяСтрока.ТипЗначенияАналитики	= ВидАналитики.ТипЗначения;
				НоваяСтрока.ТипДанныхАналитики		= НоваяСтрока.ТипЗначенияАналитики.Типы()[0];
				НоваяСтрока.КодАналитики			= "АналитикаВалюта";
				НоваяСтрока.ТаблицаАналитики		= ?(НоваяСтрока.ТипДанныхАналитики = Тип("Строка"), "", Метаданные.НайтиПоТипу(НоваяСтрока.ТипДанныхАналитики).ПолноеИмя());
		...
		Для Инд = 1 По ПараметрыСеанса.ЧислоДопАналитик Цикл
			...
			Если Не ПустаяСтрока(ВидАналитики) Тогда
				...
				НоваяСтрока.НаименованиеАналитики		= Строка(ВидАналитики);
				НоваяСтрока.ВидАналитики				= ВидАналитики;
				НоваяСтрока.ПредставлениеАналитики		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					Нстр("ru = 'Аналитика %1: %2'; en = 'Dimension %1: %2'"), Инд, Строка(ВидАналитики));
				НоваяСтрока.ТипЗначенияАналитики		= ВидАналитики.ТипЗначения;
				НоваяСтрока.ТипДанныхАналитики			= НоваяСтрока.ТипЗначенияАналитики.Типы()[0];
	...
	Если ДобавлятьПоказатели Тогда
		...
		Пока Результат.Следующий() Цикл
			...
			НоваяСтрока.ПредставлениеАналитики		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Показатель : %1'"), Результат.Наименование);
...
Функция ВыполнитьЗаменуЭлементов(ПравильныйЭлемент, МассивДублей, УдалятьДубли=Истина, ТаблицаДокументовСОшибками = Неопределено) Экспорт
	...
	МодульУправлениеНСИ=ОбщегоНазначения.ОбщийМодуль("УправлениеНСИ");
	
	Возврат МодульУправлениеНСИ.ВыполнитьЗаменуЭлементов(ПравильныйЭлемент, МассивДублей, УдалятьДубли, ТаблицаДокументовСОшибками);
...
Процедура ОбновитьАдресаЭлементовСтруктуры(БланкОтчета) Экспорт
	...
	Если ТипЗнч(ТаблДок)=Тип("ТабличныйДокумент") Тогда
		...
		Исключение
			...
			СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось обновить адреса элементов структуры вида отчета %1
			|:%2'"), 
...
Функция ПолучитьАдресаЭлементовСтруктуры(ВидОтчета,ЭлементСтруктуры,Элемент=Неопределено) Экспорт
	...
	БланкОтображенияПоУмолчанию = УправлениеОтчетамиУХ.ЗначениеНастройкиОбработкиШаблонов(ПараметрыНастроек, "БланкДляОтображения");
...
Функция ПолучитьЭлементСтруктурыПоАдресу(ВидОтчета,ЭлементСтруктуры,АдресЭлемента) Экспорт
	...
	Если БланкОтображенияПоУмолчанию=Неопределено Тогда
		...
		СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось определить бланк отображения, используемый по умолчанию для вида отчета %1'"), 
...
Функция ОпределитьТекстВидаИтога(ТекстФормулы,НачальныйИндекс) Экспорт
	...
	Если ВРЕГ(Сред(ТекстФормулы,НачальныйИндекс,СтрДлина("СНДРС")))="СНДРС" Тогда
		
		Возврат "СНДРС";
		
	ИначеЕсли ВРЕГ(Сред(ТекстФормулы,НачальныйИндекс,СтрДлина("СНКРС")))="СНКРС" Тогда
		
		Возврат "СНКРС";
		
	ИначеЕсли ВРЕГ(Сред(ТекстФормулы,НачальныйИндекс,СтрДлина("СКДРС")))="СКДРС" Тогда
		
		Возврат "СКДРС";
		
	ИначеЕсли ВРЕГ(Сред(ТекстФормулы,НачальныйИндекс,СтрДлина("СККРС")))="СККРС" Тогда
		
		Возврат "СККРС";
		
	ИначеЕсли ВРЕГ(Сред(ТекстФормулы,НачальныйИндекс,СтрДлина("СНС")))="СНС" Тогда
		
		Возврат "СНС";
		
	ИначеЕсли ВРЕГ(Сред(ТекстФормулы,НачальныйИндекс,СтрДлина("СКС")))="СКС" Тогда
		
		Возврат "СКС";
		
	ИначеЕсли ВРЕГ(Сред(ТекстФормулы,НачальныйИндекс,СтрДлина("СНД")))="СНД" Тогда
		
		Возврат "СНД";
		
	ИначеЕсли ВРЕГ(Сред(ТекстФормулы,НачальныйИндекс,СтрДлина("СНК")))="СНК" Тогда
		
		Возврат "СНК";
		
	ИначеЕсли ВРЕГ(Сред(ТекстФормулы,НачальныйИндекс,СтрДлина("СКД")))="СКД" Тогда
		
		Возврат "СКД";
		
	ИначеЕсли ВРЕГ(Сред(ТекстФормулы,НачальныйИндекс,СтрДлина("СКК")))="СКК" Тогда
		
		Возврат "СКК";
		
	ИначеЕсли ВРЕГ(Сред(ТекстФормулы,НачальныйИндекс,СтрДлина("СО")))="СО" Тогда
		
		Возврат "СО";
		
	ИначеЕсли ВРЕГ(Сред(ТекстФормулы,НачальныйИндекс,СтрДлина("ДО")))="ДО" Тогда
		
		Возврат "ДО";
		
	ИначеЕсли ВРЕГ(Сред(ТекстФормулы,НачальныйИндекс,СтрДлина("КО")))="КО" Тогда
		
		Возврат "КО";
		
	ИначеЕсли ВРЕГ(Сред(ТекстФормулы,НачальныйИндекс,СтрДлина("ОБ")))="ОБ"
		И НЕ ВРЕГ(Сред(ТекстФормулы,НачальныйИндекс,СтрДлина("ОБЪЕКТРАСЧЕТА")))="ОБЪЕКТРАСЧЕТА" Тогда
...
//
Функция ПолучитьРезультатРазбораТекстаФормулы(ТекстФормулы, НазначениеРасчетов, ПотребительРасчета, СпособИспользования, ТипБД, РегистрБД=Неопределено, ПроизвольныйКод = Ложь) Экспорт
	...
	|	ЕСТЬNULL(ИсточникиДанныхДляРасчетов.ПоказательОтбор.ТипЗначения, ИсточникиДанныхДляРасчетов.ТипЗначения) КАК ТипЗначения,
	...
	Пока Инд<=СтрДлина(РабочийТекстФормулы) Цикл
		...
		ИначеЕсли ТекСимвол="]" И ЕстьНачалоОперанда Тогда // Предполагается, что сформирован текст операнда формулы
			...
			Если Результат.Следующий() Тогда
				...
				СтруктураРазбора.Вставить("Код", 						СокрЛП(ТекстОперанда));
				СтруктураРазбора.Вставить("Ссылка",						Результат.Ссылка);				
				СтруктураРазбора.Вставить("ТипЗначения",				Результат.ТипЗначения);				
				СтруктураРазбора.Вставить("ПоказательТекущегоОтчета",	Результат.ПоказательТекущегоОтчета);				
				СтруктураРазбора.Вставить("ЕстьНестандартныеОтборы",	Результат.ЕстьНестандартныеОтборы);	
				
				ДанныеРазбора.Вставить("["+ТекстОперанда+"]", СтруктураРазбора);
...
Функция ПроверитьНаПравильностьСимвол(Символ)
	СтрокаЭталон = "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890"; // @Non-NLS
...
Функция УдалитьИзСтрокиЗарезервированныеСлова(ЗНАЧ РедактируемыйТекст,ИмяСправочника="") Экспорт
	...
	РедактируемыйТекст = СтрЗаменить(РедактируемыйТекст, "Dimension", "ANLT");
...
Функция ПолучитьВозможныйКодСправочника(Знач ПроектКода, Знач ДлинаКода, ИмяСправочника, Владелец = Неопределено) Экспорт
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТекстФункции", ТекстФункции);
...
Функция ПредставлениеДокументаПриПроведении(Источник) Экспорт
	...
	ПредставлениеДокумента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Проведение документа: %1'"), ПредставлениеДокумента);
...
Функция ПолучитьЗначениеКонстанты(ИмяКонстантыВход) Экспорт
	...
	Если Метаданные.Константы.Найти(ИмяКонстантыВход) = Неопределено Тогда
		Возврат Неопределено;	
	КонецЕсли;
	
	Возврат Константы[ИмяКонстантыВход].Получить();
...
// Размещает на форме в группе ГруппаРодительВход элемент с наименованием НаименованиеЭлементаВход, 
...
Функция ДобавитьГруппуЭлементов(НаименованиеНовогоРеквизитаВход, ГруппаРодительВход, ГруппировкаВход, ЭлементыВход, ЗаголовокВход = "", ОтображатьЗаголовокВход = Ложь) Экспорт
	НовыйЭлемент									 = ЭлементыВход.Вставить(НаименованиеНовогоРеквизитаВход, Тип("ГруппаФормы"), ГруппаРодительВход); // ГруппаФормы
...
// Функция возвращает структуру, в которой имя временной таблицы находится в ключе, а в значении содержимое временной таблицы (таблица значений)
