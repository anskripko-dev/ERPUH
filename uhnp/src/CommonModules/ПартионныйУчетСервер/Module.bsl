...
Процедура ИнициализироватьДанныеКонтроляИзменений(Запрос, ТекстыЗапроса, Документ) Экспорт
	...
	|				ТОГДА ЛОЖЬ
	//++ НЕ УТКА
	|			КОГДА ТИПЗНАЧЕНИЯ(&Регистратор) = ТИП(Документ.ЭтапПроизводства2_2)
	|				ТОГДА НАЧАЛОПЕРИОДА(Т.Период, ДЕНЬ) > Отбор.Период
	//-- НЕ УТКА
		...
		Для Каждого СтрокаОшибки Из РезультатыКонтроля.ОшибкиСебестоимостьТоваров Цикл
			...
			Если СтрокаОшибки.ЭтоОперацияПриход Тогда
				...
				ТекстСообщения = НСтр("ru = 'После текущего документа уже отражен расход по организации ""%1"", номенклатуре ""%2"" и месту хранения ""%3"".
					...
					|en = 'After the current document, the expense for the ""%1"" company, the ""%2"" item, and the ""%3"" storage location was recorded.
					|To reduce the previously recorded income, use the ""Storno"" document.'");
			...
			Иначе
				...
				ТекстСообщения = НСтр("ru = 'Дата расхода ранее последнего прихода по организации ""%1"", номенклатуре ""%2"" и месту хранения ""%3"".
					...
					|en = 'The expense date is earlier than the last receipt for the ""%1"" company, the ""%2"" item and the ""%3"" storage location.
					|Cannot identify the moving average cost. Set the document date later than %4.
					|In some types of documents, the expense date is specified in the document tables.'");
