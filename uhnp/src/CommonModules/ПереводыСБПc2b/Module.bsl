...
Функция НастройкиВозвратаОплаты(
	...
	Если ИдентификаторУчастника = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Идентификатор участника по данным оплаты не найден.';
								|en = 'Participant ID is not found by payment data.'");
	КонецЕсли;
	...
	Если Результат = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Настройки возврата по идентификатору участника не найдены.';
								|en = 'Return settings are not found by participant ID.'");
	КонецЕсли;
...
Процедура УдалитьИдентификаторыОперации(ДокументОперации) Экспорт
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru = 'Не удалось удалить данные операции (с2b) СБП.';
								|en = 'Cannot delete the Faster Payments System transaction data (c2b).'");
