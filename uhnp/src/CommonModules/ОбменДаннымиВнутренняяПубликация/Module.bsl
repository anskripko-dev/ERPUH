...
Процедура ВыполнитьОбменДаннымиПоСценарию(КодСценарияОбмена) Экспорт
	...
	Если Не ОчередьЗадачВыполнена(Сценарий, , , ПерваяЗадача) Тогда
		...
		ТекстСообщения = НСтр("ru = 'Обнаружены задачи синхронизации по сценарию предыдущей сессии.';
								|en = 'Found sync tasks based on the previous session''s scenario.'",
...
Процедура ОтменитьОчередьЗадач(Узел, Сценарий, ИдентификаторОбмена, ОбластьДанных) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			Событие = НСтр("ru = 'Обмен данными.Отмена очереди задач';
							|en = 'Обмен данными.Отмена очереди задач'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ОтметитьВыполнениеЗадачи(Задача, Ошибка) Экспорт
	...
	Если ЗначениеЗаполнено(ТекЗадача) Тогда
		...
		Исключение
			...
			Событие = НСтр("ru = 'Обмен данными.Отметка выполнения задачи';
							|en = 'Обмен данными.Отметка выполнения задачи'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПризнакОтменыПовторногоВыполненияЗадания() Экспорт
	...
	Возврат НСтр("ru = '#ОтменитьПовторноеВыполнениеЗадачи#';
				|en = '#ОтменитьПовторноеВыполнениеЗадачи#'");
	...
	Если Выборка.Следующий() Тогда
		...
		Если Состояние = "Запланировано" Или Состояние = "Выполняется" Тогда
			...
			Исключение
				...
				Событие = НСтр("ru = 'Обмен данными.Запись';
								|en = 'Обмен данными.Запись'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПроверитьБлокировкуЗадачи(Задача, Отказ)
	...
	Исключение
		...
		ШаблонТекста = НСтр("ru = 'Задача с идентификатором [%1] заблокирована.
			...
			|	Сценарий: %6';
			|en = 'Task with ID [%1] is locked.
			|Perhaps, it is still in progress.
			|	Created on: %2
			|	Infobase node: %3
			|	Number: %4
			|	Exchange ID: %5
			|	Scenario: %6'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура УстановитьФункциональныеОпцииДляУзла(Форма)
	...
	Исключение
		...
		Событие = НСтр("ru = 'Обмен данными.Установка функциональных опций';
						|en = 'Обмен данными.Установка функциональных опций'", ОбщегоНазначения.КодОсновногоЯзыка());
