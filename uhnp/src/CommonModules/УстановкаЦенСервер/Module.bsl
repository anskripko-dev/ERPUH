...
Функция ДобавитьДополнительныеДанные(ДополнительныеПараметры, ИмяВременногоФайла) Экспорт
	...
	Исключение
		СообщениеОбОшибке = НСтр("ru = 'Не удалось записать данные в файл. Подробности:';
								|en = 'Cannot save data to the file. Details:'") + " "
	...
	Исключение
		СообщениеОбОшибке = СтрШаблон(
			НСтр("ru = 'Файл не может быть загружен.
			           ...
			           |Подробности: %1';
			           |en = 'Cannot import the file.
			           |You can only import the files exported using the ""Excel -> Export..."" command.
		...
		Если ТекЭлементНоменклатура.ХарактеристикиИспользуются Тогда
			...
			Если Не ДобавленыХарактеристики Тогда
				ТекстОшибки = НСтр("ru = 'Для номенклатуры ""%1"" в строке %2 списка ""Таблица цен"" включено использование характеристик, но характеристики не указаны. Добавьте характеристики и укажите их цены или удалите данную номенклатуру из списка.';
									|en = 'Variants are enabled for the ""%1"" item in line %2 of the ""Price table"" list, but variants are not specified. Add variants and specify their prices, or delete this item from the list.'");
...
Функция ШаблонОшибкиВычисленияЦеныПоДаннымИБ()
	...
	Возврат НСтр("ru = 'Ошибка расчета цены с помощью запроса к данным ИБ по виду цен ""%1""';
				|en = 'An error occurred when calculating the price using an arbitrary request to infobase data for the ""%1"" price type'");
