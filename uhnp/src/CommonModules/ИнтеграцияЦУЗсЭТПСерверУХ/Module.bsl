...
Процедура ОбновитьСоставЭТП(НаименованиеЭТП=Неопределено) Экспорт
  ...
  ТЗЭТП = ПолучитьОписанияОбработокИнтеграцииСЭТП(НаименованиеЭТП);
	...
	Запрос.Текст = 
		...
		|		ПО ТЗЭТП.Наименование = ЭлектронныеТорговыеПлощадки.Наименование И НЕ ЭлектронныеТорговыеПлощадки.ПометкаУдаления
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
Функция ПолучитьПараметрыПодключенияКЭТП(ЭТП, Сотрудник) Экспорт
	...
	Запрос.Текст = 
		...
		|	""ИнтеграцияСЕИС_v40"" КАК ИмяОбработки,
		...
		|	1 КАК ВерсияОбработки,
		|	"""" КАК ИмяФормыДопНастроек,
		...
		|	Ложь КАК ОбменЧерезInternetExplorer
		|ИЗ
...
Функция ЭТП_Сотрудника(Сотрудник) Экспорт
	...
	Запрос.Текст = 
		...
		|	И НастройкиОбменаСЭТП.ЭТП.ПометкаУдаления = Ложь
		|СГРУППИРОВАТЬ ПО
...
Функция ПолучитьОписаниеТаблицыНоменклатурыОбъекта(
	...
	Попытка
		...
		Запрос.Текст =
			...
			|	ЕСТЬNULL(ТаблицаНоменклатурыДокумента.Номенклатура.КодОКДП.Код, """") КАК КодОКДП,
			...
			|	ТаблицаНоменклатурыДокумента.Номенклатура.КодОКВЭД2 КАК КодОКВЭД2,
			...
			|			КОНЕЦ
			|	КОНЕЦ,
			|	ЕСТЬNULL(ТаблицаНоменклатурыДокумента.Номенклатура.КодОКДП.Код, """"),
...
Функция ИдентификаторСтрокиЛота(СтрокаЛота) Экспорт
	...
	Префикс = "";
	Если ТипЗнч(Номенклатура) = Тип("СправочникСсылка.КатегорииЗакупок") Тогда
		Префикс = "k";
	КонецЕсли;
...
Функция ПолучитьОтборСтрокиЛотаПоИдентификатору(Идентификатор) Экспорт
	...
	Если флЭтоТоварнаяКатегория Тогда
		НоменклатураСсылка = Справочники.КатегорииЗакупок.ПолучитьСсылку(Новый УникальныйИдентификатор(УИД_Номенклатуры));
	Иначе
...
Функция ПолучитьВсеПодчиненныеФайлыЗакупочныеПроцедуры(Владелец, ДокументID = "", ДокументТип = "", 
	...
	ЗапросФайлыПоВладельцу = Новый Запрос(
		...
		|ИЗ
		|	Справочник.ВерсияДокументацииЗакупочныхПроцедурПрисоединенныеФайлы КАК Файлы
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РегистрСостоянийОбъектов.СрезПоследних КАК РегистрСостоянийОбъектовСрезПоследних
		|		ПО Файлы.ВладелецФайла = РегистрСостоянийОбъектовСрезПоследних.Объект
		|ГДЕ
		|	Файлы.ВладелецФайла.ЗакупочнаяПроцедура = &Владелец
		|	И НЕ Файлы.ПометкаУдаления
		|	И Файлы.ВладелецФайла.ПрисоединенныйФайл = Файлы.Ссылка
		|	И Файлы.ВладелецФайла.ВыгружатьДокументациюНаЕИС
		|	И Файлы.ВладелецФайла.Проведен = ИСТИНА
...
Функция ПолучитьФайлыВладельца(ВладелецФайлов) Экспорт
	ДокументID = "";
	ДокументТип = "";
	//Файлы = СвязанныеФайлыВызовСервера.СвязанныеФайлыПоВладельцу(        //todo msle после обновления процедуры БСП можно вернуть обратно
	Если ТипЗнч(ВладелецФайлов) = Тип("СправочникСсылка.ЗакупочныеПроцедуры")  Тогда
		Файлы = ПолучитьВсеПодчиненныеФайлыЗакупочныеПроцедуры(ВладелецФайлов);    
	Иначе 	
	
	Файлы = СвязанныеФайлыПоВладельцу(
													ВладелецФайлов, 
													ДокументID, 
													ДокументТип);         
	КонецЕсли;
...
Функция ДвоичныеДанныеФайла(ОписаниеФайла) Экспорт
	...
	ИначеЕсли ТипЗнч(ОписаниеФайла.СвязанныйФайл) = Тип("СправочникСсылка.ВерсияДокументацииЗакупочныхПроцедурПрисоединенныеФайлы") Тогда
		СтруктураВозврата = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаИДвоичныеДанные(ОписаниеФайла.СвязанныйФайл);
		Возврат СтруктураВозврата.ДвоичныеДанные;
...
Функция ПолучитьКодЗакупкиВЕИС(СпособВыбораПоставщика,ЗакрытыйКонкурс,ВЭлектроннойФорме, Наименование) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СпособыВыбораПоставщикаСправочник.СпособыВыбораПоставщика КАК СпособВыбораПоставщика
		|ПОМЕСТИТЬ СпособыВыбораПоставщика
		|ИЗ
		...
		|ГДЕ
		|	КодыЗакупокВЕИС.СпособВыбораПоставщика В
		|			(ВЫБРАТЬ
		|				СпособыВыбораПоставщика.СпособВыбораПоставщика
		|			ИЗ
		|				СпособыВыбораПоставщика КАК СпособыВыбораПоставщика)
...
Функция ПолучитьОписаниеЛотаВПланеЗакупок(Лот) Экспорт
	...
	Попытка
		...
		Запрос.Текст = 
			...
			|ИЗ
			|	Документ.Лот КАК СправочникЛоты
...
Функция guidСтрокиПланаЗакупокПредыдущейОтправки(ЭТП, 
	...
	ШаблонУИД =  НайтиОднуИзПодстрокВТексте(ТекстПоследнейОтправки, УИДы);
			Возврат "<guid>" +"7dd3b5c5-2cdc-4dd6-b97c-d6998de8396e";
...
Функция ПолучитьТребованияКСоставуДокументовПоЛоту(Лот) Экспорт
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	Документ.Лот.ТребованияКСоставуДокументов КАК ЛотыТребованияКСоставуДокументов
		|ГДЕ
...
Функция ЕдиноеМестоПоставкиВЗакупочнойПроцедуре(ЗакупочнаяПроцедура) Экспорт
	...
	РезультатЗапроса = Запрос.Выполнить();
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	Если ВыборкаДетальныеЗаписи.Следующий() И НЕ ВыборкаДетальныеЗаписи.ЕдиноеМестоПоставки = null
