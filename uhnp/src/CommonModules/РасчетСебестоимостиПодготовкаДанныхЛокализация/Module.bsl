...
Процедура ИсправитьПроблемыВДвиженияхИДокументах(ПараметрыРасчета) Экспорт
	...
	|			ПО НастройкиУчетаНДСПриУСН.Организация = Т.Организация
	|			И НастройкиУчетаНДСПриУСН.УчетнаяПолитикаСуществует
	|			И НастройкиУчетаНДСПриУСН.ОсновноеНалогообложение <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|	ГДЕ
	...
	|		И НастройкиУчетаНДСПриУСН.Организация ЕСТЬ NULL
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	//-- НЕ УТ
	|	
	...
	РасшифровкаКодовОшибок = Новый Соответствие;
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru = 'Различается заполненность ресурсов НДС (упр.) и НДС (регл.) в регистре';
											|en = 'Fullness of the VAT (man.) and VAT (local) resources is different in the register'", ОбщегоНазначения.КодОсновногоЯзыка()) + " НДСПредъявленный");
	РасшифровкаКодовОшибок.Вставить(2, НСтр("ru = 'При отключенном управленческом учете заполнен ресурс НДС (упр.) в регистре';
											|en = 'VAT (man.) resource is filled in with disabled management accounting in the register'", ОбщегоНазначения.КодОсновногоЯзыка()) + " НДСПредъявленный");
...
Процедура КорректировкаНачальныхОстатковРегистров(ПараметрыРасчета)
	...
	//++ НЕ УТ
	РасчетСебестоимостиПрикладныеАлгоритмы.ЗавершитьКорректировкиОстатков(ПараметрыРасчета);
	//-- НЕ УТ
