...
#Область ВыпускПродукцииСтоимость // (Дт 10.07, 2Х :: Кт 2Х)
	...
	|		ТОГДА
	|			ВЫБОР
	|				КОГДА Стоимости.КорСклад ССЫЛКА Справочник.ДоговорыКонтрагентов
	|				ТОГДА Стоимости.КорСклад.Партнер
	|				ИНАЧЕ Стоимости.ПодразделениеАналитики
	|			КОНЕЦ
...
#Область ТекстВыпускВозвратныхОтходов // (Дт 10.07, 20 :: Кт 2Х)
	...
	|		ТОГДА
	|			ВЫБОР
	|				КОГДА ЕСТЬNULL(КорСтроки.Склад, Строки.Склад) ССЫЛКА Справочник.ДоговорыКонтрагентов
	|				ТОГДА ЕСТЬNULL(КорСтроки.Склад.Партнер, Строки.Склад.Партнер)
	|				ИНАЧЕ ЕСТЬNULL(КорСтроки.ПодразделениеАналитики, Строки.ПодразделениеАналитики)
	|			КОНЕЦ
