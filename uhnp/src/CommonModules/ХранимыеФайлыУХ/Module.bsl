...
Функция ПолучитьХранимыйФайл(ХранимыйФайл, ПолноеИмяФайла, ПроверятьДатуФайла = Истина, ПроверятьТолькоЧтениеФайла = Истина, Период = Неопределено) Экспорт
	...
	Если ФайлНаДиске.Существует() И ФайлНаДиске.ПолучитьТолькоЧтение() Тогда
		...
		Если ПроверятьТолькоЧтениеФайла Тогда
			...
			ТекстСообщения = СтрШаблон(Нстр("ru = 'Файл ""%1""
						   |на Вашем компьютере имеет атрибут ""Только чтение"". Измените атрибут или укажите другое имя файла.'"), 
...
Функция ПроверитьНаличиеВерсии(ХранимыйФайл, ДатаИзмененияФайла, РазмерФайла = Неопределено, ПолноеИмяФайла = Неопределено) Экспорт
...
Функция ПоместитьХранимыйФайл_УР(ХранимыйФайл, СтруктураФайла, Комментарий = "") Экспорт
	...
	РасширениеФайла = НРег(СокрЛП(ХранимыйФайл.РасширениеФайла));
	ПолноеИмяФайла = ПолучитьИмяВременногоФайла(РасширениеФайла);
	ДвоичныеДанные.Записать(ПолноеИмяФайла);
	СтруктураФайла.СписокЛистов = ОбработкаФайловMicrosoftExcelКлиентСерверУХ.ВернутьСписокЛистов(ПолноеИмяФайла, РасширениеФайла);
	...
	ПараметрыФайла.Вставить("ИмяБезРасширения", ФайлНаДиске.ИмяБезРасширения);
	ПараметрыФайла.Вставить("РасширениеБезТочки", СтрЗаменить(ФайлНаДиске.Расширение,".",""));
	ПараметрыФайла.Вставить("ВремяИзмененияУниверсальное", СтруктураФайла.ДатаИзмененияФайла);
...
Функция ДобавитьХранимыйФайлИлиВерсию_УР(СтруктураОтбора, СтруктураФайла, КомментарийКВерсии ="", ХранимыйФайл = Неопределено, Период = Неопределено) Экспорт
	...
	Иначе
		...
		Попытка
			...
			Объект.Записать();
			ВерсияФайла=ПоместитьХранимыйФайл_УР(Объект.Ссылка, СтруктураФайла, КомментарийКВерсии);
...
Функция ПолучитьФильтрФайлов() Экспорт
	...
	Возврат НСтр("ru = 'Все файлы (*.*)|*.*|'")
	      + НСтр("ru = 'Файлы 1С (*.dt;*.cd;*.cf;*.cfu;*.epf)|*.dt;*.cd;*.cf;*.cfu;*.epf|'")
	      + НСтр("ru = 'Файлы баз данных 1С (*.dt;*.cd)|*.dt;*.cd|'")
	      + НСтр("ru = 'Файлы конфигураций 1С (*.cf;*.cfu)|*.cf;*.cfu|'")
	      + НСтр("ru = 'Внешние обработки (*.epf)|*.epf|'")
	      + НСтр("ru = 'Картинки (*.bmp;*.dib;*.rle;*.jpg;*.jpeg;*.tif;*.gif;*.png;*.ico;*.wmf;*.emf)|*.bmp;*.dib;*.rle;*.jpg;*.jpeg;*.tif;*.gif;*.png;*.ico;*.wmf;*.emf|'")
	      + НСтр("ru = 'Документ Microsoft Word (*.doc)|*.doc|'")
	      + НСтр("ru = 'Документ Microsoft Excell (*.xls)|*.xls|'")
	      + НСтр("ru = 'Документ Microsoft PowerPoint (*.ppt)|*.ppt|'")
	      + НСтр("ru = 'Документ Microsoft Visio (*.vsd)|*.vsd|'")
	      + НСтр("ru = 'Письмо электронной почты (*.msg)|*.msg|'")
	      + НСтр("ru = 'Текстовый документ (*.txt)|*.txt|'")
	      + НСтр("ru = 'Табличный документ (*.mxl)|*.mxl|'");
...
Функция ПолучитьВерсиюФайлаПоДатеСоздания(ХранимыйФайл, ДатаСоздания) Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ХранимыеФайлыОрганизацийПрисоединенныеФайлы.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.ХранимыеФайлыОрганизацийПрисоединенныеФайлы КАК ХранимыеФайлыОрганизацийПрисоединенныеФайлы
		|ГДЕ
		|	ХранимыеФайлыОрганизацийПрисоединенныеФайлы.ВладелецФайла = &ХранимыйФайл
		|	И ХранимыеФайлыОрганизацийПрисоединенныеФайлы.ДатаСоздания = &ДатаСоздания";
...
Процедура ОбновитьОбъектХранимыйФайл(ХранимыйФайл, ВерсияХранимогоФайла, ИмпортированныйОбъект) Экспорт
	
	Если НЕ ЗначениеЗаполнено(ХранимыйФайл) ИЛИ НЕ ЗначениеЗаполнено(ВерсияХранимогоФайла)
		ИЛИ НЕ ЗначениеЗаполнено(ИмпортированныйОбъект) Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ИмпортированныйОбъект", ИмпортированныйОбъект);
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ОбъектыХранимыхФайлов.ХранимыйФайл КАК ХранимыйФайл,
		|	ОбъектыХранимыхФайлов.ВерсияХранимогоФайла КАК ВерсияХранимогоФайла
		|ИЗ
		|	РегистрСведений.ОбъектыХранимыхФайлов КАК ОбъектыХранимыхФайлов
		|ГДЕ
		|	ОбъектыХранимыхФайлов.ХранимыйФайл = &ХранимыйФайл
		|	И ОбъектыХранимыхФайлов.ВерсияХранимогоФайла = &ВерсияХранимогоФайла
	...
	Если Результат.Пустой() Тогда
		...
		// Обновим данные о хранимом файле
		НаборЗаписей = РегистрыСведений.ОбъектыХранимыхФайлов.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.ИмпортированныйОбъект.Установить(ИмпортированныйОбъект);
		
		НаборЗаписей.Прочитать();
		НаборЗаписей.Очистить();
		
		НоваяЗапись = НаборЗаписей.Добавить();
		НоваяЗапись.ХранимыйФайл			= ХранимыйФайл;
		НоваяЗапись.ВерсияХранимогоФайла	= ВерсияХранимогоФайла;
		НоваяЗапись.ИмпортированныйОбъект	= ИмпортированныйОбъект;
...
Функция ПолучитьИмпортированнуюВерсию(ИнформацияОФайле, Объект) Экспорт
	...
	Выборка = Результат.Выбрать();
	Выборка.Следующий();
	Возврат Выборка.ВерсияХранимогоФайла;
	
КонецФункции // ПовторныйИмпортДанных() 

Функция ПолучитьВерсиюХранимогоФайла(ИнформацияОФайле, ХранимыйФайл) Экспорт
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	Справочник.ХранимыеФайлыОрганизацийПрисоединенныеФайлы КАК ХранимыеФайлыОрганизацийПрисоединенныеФайлы
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ХранимыеФайлыОрганизаций КАК ХранимыеФайлыОрганизаций
		|		ПО ХранимыеФайлыОрганизацийПрисоединенныеФайлы.ВладелецФайла = ХранимыеФайлыОрганизаций.Ссылка
		|ГДЕ
		|	ХранимыеФайлыОрганизаций.Ссылка = &Ссылка
		|	И ХранимыеФайлыОрганизаций.Наименование = &Наименование
		|	И ХранимыеФайлыОрганизацийПрисоединенныеФайлы.ДатаМодификацииУниверсальная = &ДатаМодификацииУниверсальная
		|	И ХранимыеФайлыОрганизацийПрисоединенныеФайлы.Размер = &Размер";
	
	Запрос.УстановитьПараметр("ДатаМодификацииУниверсальная", ИнформацияОФайле.ДатаМодификацииУниверсальная);
	Запрос.УстановитьПараметр("Наименование", ИнформацияОФайле.ИмяФайла);
	Запрос.УстановитьПараметр("Размер", ИнформацияОФайле.РазмерФайла);
	Запрос.УстановитьПараметр("Ссылка", ХранимыйФайл);
