	...
	Иначе
		...
		Исключение
			...
			ВызватьИсключение СтрШаблон(НСтр("ru = 'Не удалось добавить запись в регистр очереди ""%1"" по причине:
													|%2.';
													|en = 'Cannot add a record to the ""%1"" queue register. Reason:
...
Функция ИмяСобытияЗапускУправляющегоПотока()
	...
	Возврат НСтр("ru = 'Запуск управляющего потока';
				|en = 'Start main thread'", ОбщегоНазначения.КодОсновногоЯзыка());
