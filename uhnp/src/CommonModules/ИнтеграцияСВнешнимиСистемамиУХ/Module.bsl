...
Функция ПолучитьТаблицуДанныхДляВыгрузки(ДанныеНастройки,ТабПравилаРабочие,СписокОтбора)
	...
	|ПОМЕСТИТЬ ТекСоответствиеОбъектов
	|ИЗ
	...
	|;
	|ВЫБРАТЬ РабочаяТаблица.Ссылка КАК Reference";
				  	
	МассивСинонимов=Новый Массив;
	
	Для Каждого Строка ИЗ ТабПравилаРабочие Цикл 
		
		Если МассивСинонимов.Найти(Строка.Синоним)=Неопределено Тогда
			
			ТекстЗапроса=ТекстЗапроса+",
			|"+Строка.Поле+" КАК "+Строка.Синоним;
			
			МассивСинонимов.Добавить(Строка.Синоним);
	...
	ТекстЗапроса=ТекстЗапроса+",
	|ЕстьNULL(ТекСоответствиеОбъектов.ОбъектВнешнейИБ,Неопределено) КАК ОбъектВнешнейИБ,
	|ВЫБОР КОГДА ТекСоответствиеОбъектов.ОбъектВнешнейИБ ЕСТЬ NULL Тогда Ложь ИНАЧЕ Истина КОНЕЦ КАК ЕстьОбъектВнешнейИБ
	|ИЗ "+ДанныеНастройки.ТаблицаАналитикиТек+" КАК РабочаяТаблица
	|Левое Соединение ТекСоответствиеОбъектов ПО РабочаяТаблица.Ссылка=ТекСоответствиеОбъектов.ОбъектТекущейИБ 
	|ГДЕ ИСТИНА";
	
	Если НЕ СписокОтбора=Неопределено Тогда
		
		ТекстЗапроса=ТекстЗапроса+"
		|И РабочаяТаблица.Ссылка В(&СписокОтбора)";
		
		Запрос.УстановитьПараметр("СписокОтбора",СписокОтбора);
		
	КонецЕсли;
				
	Если ДанныеНастройки.ТипОбъектаКонсолидации="ВидСубконто"
		И ДанныеНастройки.ТаблицаАналитикиТек="Справочник.ПроизвольныйКлассификаторУХ" Тогда
		
		ТекстЗапроса=ТекстЗапроса+"
		|И РабочаяТаблица.Владелец=&Владелец";
		
		Запрос.УстановитьПараметр("Владелец",ДанныеНастройки.ИмяОбъектаМетаданных);
		
	КонецЕсли;
	
	Если СписокОтбора=Неопределено 
		И ДанныеНастройки.ТипОбъектаКонсолидации="Справочник" 
		ИЛИ ДанныеНастройки.ТипОбъектаКонсолидации="ВидСубконто" Тогда
		
		Если ОбщегоНазначенияУХ.ЕстьРеквизитСправочника(ДанныеНастройки.ИмяОбъектаМетаДанных,"НСИ_ТребуетСинхронизации") Тогда
			
			ТекстЗапроса=ТекстЗапроса+"
			|И (НЕ РабочаяТаблица.НСИ_ТребуетСинхронизации)";
			
		КонецЕсли;
		
		Если ОбщегоНазначенияУХ.ЕстьРеквизитСправочника(ДанныеНастройки.ИмяОбъектаМетаДанных,"НСИ_НеАктивный") Тогда
			
			ТекстЗапроса=ТекстЗапроса+"
			|И (НЕ РабочаяТаблица.НСИ_НеАктивный)";
	...
	Запрос.УстановитьПараметр("ИспользуемаяИБ",			ДанныеНастройки.ИспользуемаяИБ);
	Запрос.УстановитьПараметр("НастройкаСоответствия",	ДанныеНастройки.Ссылка);
	
	Возврат Запрос.Выполнить().Выгрузить();
					
КонецФункции // ПолучитьТаблицуДанныхДляВыгрузки()
...
КонецФункции // ПолучитьДанныеНастройкиДляВыгрузки()

Функция ВыгрузитьОбъектыВВИБ(ИспользуемаяИБ,
			РабНастройкаСоответствия,
			СписокОтбора=Неопределено,
			ВозвращатьСоответствиеОбъектов=Ложь,
			ОбновлениеДочернихЭлементов=Истина,
			ТекСоединениеВИБ=Неопределено,СоздаватьПомеченныеНаУдаление=Ложь) Экспорт
			
			
	ДанныеНастройки=ПолучитьДанныеНастройкиДляВыгрузки(РабНастройкаСоответствия);
	ДанныеНастройки.Вставить("ИспользуемаяИБ",ИспользуемаяИБ);
			
	ТаблицаАналитикиВИБ=ДанныеНастройки.ТаблицаАналитикиВИБ; 
	ИспользоватьWS=ИспользуемаяИБ.ИспользоватьWS;
	ИмяОбъектаВИБ=ДанныеНастройки.ИмяОбъектаВИБ;
	КодАналитики=СтрЗаменить(ДанныеНастройки.ТаблицаАналитикиВИБ,".","_");
	ДанныеНастройки.Вставить("КодАналитики",КодАналитики);
	...
	Если ДанныеНастройки.СинхронизацияПоGUID Тогда // Только соответствие объектов
		...
		Если ВозвращатьСоответствиеОбъектов Тогда
			...
			Запрос.УстановитьПараметр("НастройкаСоответствия",РабНастройкаСоответствия);
			...
			Если Не Результат.Пустой() Тогда
				...
				Пока Выборка.Следующий() Цикл
					...
					СтруктураДанных.Вставить("ТаблицаАналитикиВИБ",ДанныеНастройки.ТаблицаАналитикиВИБ);
					СтруктураДанных.Вставить("GUID",Выборка.ОбъектВнешнейИБ);
					
					СоответствиеОбъектов.Вставить(Выборка.ОбъектТекущейИБ+"_tt_"+ДанныеНастройки.ТаблицаАналитикиТек,СтруктураДанных); 
	...
	ИначеЕсли ДанныеНастройки.ТипОбъектаВИБ="Перечисление" Тогда
		
		Если ВозвращатьСоответствиеОбъектов Тогда
			
			СоответствиеОбъектов=Новый Соответствие;
			
			ПараметрОтбора=Новый СписокЗначений;
			КэшЗначений=Новый Соответствие;
			
			Для Каждого Элемент ИЗ СписокОтбора Цикл 
				
				ПараметрОтбора.Добавить(XMLСтрока(Элемент.Значение)); 
				КэшЗначений.Вставить(XMLСтрока(Элемент.Значение),Элемент.Значение);
				
			КонецЦикла;
			
			Запрос=Новый Запрос;
			Запрос.Текст="ВЫБРАТЬ
			             |	СоответствиеВнешнимИБСоответствиеЗначенийПеречислений.ЗначениеВнешнейИБ КАК ЗначениеВнешнейИБ,
			             |	СоответствиеВнешнимИБСоответствиеЗначенийПеречислений.ЗначениеТекущейИБ КАК ЗначениеТекущейИБ
			             |ИЗ
			             |	Справочник.СоответствиеВнешнимИБ.СоответствиеЗначенийПеречислений КАК СоответствиеВнешнимИБСоответствиеЗначенийПеречислений
			             |ГДЕ
			             |	СоответствиеВнешнимИБСоответствиеЗначенийПеречислений.Ссылка = &НастройкаСоответствия
			             |	И СоответствиеВнешнимИБСоответствиеЗначенийПеречислений.ЗначениеТекущейИБ В(&ПараметрОтбора)";
			...
			Если Не Результат.Пустой() Тогда
				...
				Пока Выборка.Следующий() Цикл
					...
					СтруктураДанных=Новый Структура;
					СтруктураДанных.Вставить("ТаблицаАналитикиВИБ",ДанныеНастройки.ТаблицаАналитикиВИБ);
					СтруктураДанных.Вставить("ЗначениеПеречисления",Результат.ЗначениеВнешнейИБ);
												
					СоответствиеОбъектов.Вставить(Выборка.ЗначениеТекущейИБ+"_tt_"+ДанныеНастройки.ТаблицаАналитикиТек,СтруктураДанных); 
	...
	КонтекстОтчета.Вставить("Организация");
	...
	ДобавитьСтрокуПометкаУдаления(ДанныеНастройки,ТабПравилаРабочие);
	
	ТаблицаДанных = ПолучитьТаблицуДанныхДляВыгрузки(ДанныеНастройки,ТабПравилаРабочие,СписокОтбора); 
	ТаблицаДанных = ДополнитьТаблицуВнешнимиGUID(ТаблицаДанных,ИспользуемаяИБ,ТабПравилаРабочие);
	
	ТаблицаДанных.Колонки.Добавить("ОбъектДляВыгрузки",РаботаСОбъектамиМетаданныхВнешнийУХ.ПолучитьОписаниеТиповСтроки(40));
	
	КэшОбъектовДляВыгрузки=Новый Соответствие;
	
	Для Каждого Строка ИЗ ТаблицаДанных Цикл
		
		Строка.ОбъектДляВыгрузки=XMLString(Строка.Reference);
		КэшОбъектовДляВыгрузки.Вставить(Строка.ОбъектДляВыгрузки,Строка.Reference);
	...
	РезультатЗаполнения = Экспорт_СоздатьОбновитьЭлементыНСИ(ТекСоединениеВИБ,ТаблицаДанных,ТабПравилаДляВыгрузки,СтруктураКлючевыхРеквизитов,СтруктураПодчиненныхРеквизитов,ОбновлениеДочернихЭлементов,СоздаватьПомеченныеНаУдаление);

	Если РезультатЗаполнения=Неопределено Тогда
				
		Возврат Новый Соответствие;
			
	ИначеЕсли Не ПустаяСтрока(РезультатЗаполнения.ОшибкиСинхронизации) Тогда
		
		ПротоколируемыеСобытияУХ.ДобавитьЗаписьОшибка("ДублированиеДанныхПриСинхронизации", , , РезультатЗаполнения.ОшибкиСинхронизации, Экспорт_ПолучитьСоответствиеТаблицДублейДляПротокола(ИспользуемаяИБ,РезультатЗаполнения.СоответствиеТаблицДублей));
	...
	ВыгрузитьПодчиненныеОбъекты(ТекСоединениеВИБ,ИспользуемаяИБ,РезультатЗаполнения.ТабОбъектовДляВыгрузки);	
	
	Если РабНастройкаСоответствия.СоответствияСвязанныхОбъектов.Количество()>0 Тогда
		
		ВыгрузитьДанныеСвязанныхОбъектов(ТекСоединениеВИБ,РабНастройкаСоответствия,РезультатЗаполнения.ТабОбработанныхОбъектов,КэшОбъектовДляВыгрузки,ИспользуемаяИБ);
	...
	Если Не ДанныеИзменения.Пустой() Тогда
		...
		РезультатЗаполнения.ТаблицаДанных.Индексы.Добавить("ОбъектДляВыгрузки");
		...
		Пока ИзмененныеОбъекты.Следующий() Цикл
			...
			Если (НЕ СтрОбъект=Неопределено) И ЗначениеЗаполнено(СтрОбъект.ОбработанныйОбъектБД)  Тогда
				...
				НаборЗаписей.Отбор.НастройкаСоответствия.Установить(РабНастройкаСоответствия);
	...
	ИначеЕсли ВозвращатьСоответствиеОбъектов И (НЕ ДанныеНастройки.ТипОбъектаКонсолидации="Перечисление") Тогда
		
		РезультатЗаполнения.ТаблицаДанных.Индексы.Добавить("ОбъектДляВыгрузки");
		
	КонецЕсли;	
	
	ИмяКолонкиОбъекта=КодАналитики;
	
	ЗаписатьСоответствияОбъектовПриВыгрузке(РабНастройкаСоответствия,РезультатЗаполнения.ТабОбработанныхОбъектов,КэшОбъектовДляВыгрузки,ИспользуемаяИБ);
	
	Если ВозвращатьСоответствиеОбъектов Тогда
		
		СоответствиеОбъектов=Новый Соответствие;		
		
		Для Каждого Строка ИЗ РезультатЗаполнения.ТабОбработанныхОбъектов Цикл
			
			Если ЗначениеЗаполнено(Строка.ОбработанныйОбъектБД) Тогда
				
				СтруктураДанных=Новый Структура;
				СтруктураДанных.Вставить("ТаблицаАналитикиВИБ",ДанныеНастройки.ТаблицаАналитикиВИБ);
				СтруктураДанных.Вставить("GUID",Строка.ОбработанныйОбъектБД);
				
				СоответствиеОбъектов.Вставить(Строка.ИсходныйОбъектБД+"_tt_"+ДанныеНастройки.ТаблицаАналитикиТек,СтруктураДанных);
...
Процедура ЗаписатьСоответствияОбъектовПриВыгрузке(НастройкаСоответствия,ТабОбработанныхОбъектов,КэшОбъектовДляВыгрузки,ИспользуемаяИБ)

	Запрос=Новый Запрос;
	Запрос.Текст="ВЫБРАТЬ
	             |	ТабОбработанныхОбъектов.ИсходныйОбъектБД КАК ИсходныйОбъектБД,
	             |	ТабОбработанныхОбъектов.ОбработанныйОбъектБД КАК ОбработанныйОбъектБД,
	             |	ТабОбработанныхОбъектов.ПредставлениеОбъектаВИБ КАК ПредставлениеОбъектаВИБ
	             |ПОМЕСТИТЬ ТабОбработанныхОбъектов
	             |ИЗ
	             |	&ТабОбработанныхОбъектов КАК ТабОбработанныхОбъектов
	             |;
	             |
	             |////////////////////////////////////////////////////////////////////////////////
	             |ВЫБРАТЬ
	             |	СоответствиеОбъектовТекущейИВнешнихИБ.ОбъектТекущейИБ КАК ОбъектТекущейИБ,
	             |	СоответствиеОбъектовТекущейИВнешнихИБ.ОбъектВнешнейИБ КАК ОбъектВнешнейИБ,
	             |	СоответствиеОбъектовТекущейИВнешнихИБ.НаименованиеОбъектаВнешнейИБ КАК НаименованиеОбъектаВнешнейИБ
	             |ПОМЕСТИТЬ УстановленныеСоответствия
	             |ИЗ
	             |	РегистрСведений.СоответствиеОбъектовТекущейИВнешнихИБ КАК СоответствиеОбъектовТекущейИВнешнихИБ
	             |ГДЕ
	             |	СоответствиеОбъектовТекущейИВнешнихИБ.НастройкаСоответствия = &НастройкаСоответствия
	             ...
	             |ВЫБРАТЬ
	             |	ТабОбработанныхОбъектов.ИсходныйОбъектБД КАК ИсходныйОбъектБД,
	             |	ЕСТЬNULL(УстановленныеСоответствия.ОбъектВнешнейИБ, НЕОПРЕДЕЛЕНО) КАК ОбъектВнешнейИБ,
	             |	ТабОбработанныхОбъектов.ОбработанныйОбъектБД КАК ОбработанныйОбъектБД,
	             |	ТабОбработанныхОбъектов.ПредставлениеОбъектаВИБ КАК ПредставлениеОбъектаВИБ,
	             |	УстановленныеСоответствия.НаименованиеОбъектаВнешнейИБ КАК НаименованиеОбъектаВнешнейИБ
	             |ИЗ
	             |	ТабОбработанныхОбъектов КАК ТабОбработанныхОбъектов
	             |		ЛЕВОЕ СОЕДИНЕНИЕ УстановленныеСоответствия КАК УстановленныеСоответствия
	             |		ПО ТабОбработанныхОбъектов.ОбработанныйОбъектБД = УстановленныеСоответствия.ОбъектВнешнейИБ
	             |ГДЕ
	             |	(ЕСТЬNULL(УстановленныеСоответствия.ОбъектВнешнейИБ, НЕОПРЕДЕЛЕНО) = НЕОПРЕДЕЛЕНО
	             |			ИЛИ НЕ УстановленныеСоответствия.НаименованиеОбъектаВнешнейИБ = ТабОбработанныхОбъектов.ПредставлениеОбъектаВИБ)";
	
	Запрос.УстановитьПараметр("ТабОбработанныхОбъектов",ТабОбработанныхОбъектов);
	...
	Пока Результат.Следующий() Цикл
		...
		ИсходныйОбъектБД=?(КэшОбъектовДляВыгрузки=Неопределено,Результат.ИсходныйОбъектБД,КэшОбъектовДляВыгрузки.Получить(Результат.ИсходныйОбъектБД));
		
		ЗаписатьСоответствиеОбъектовТекущейИВнешнейИБ(НастройкаСоответствия,
														ИспользуемаяИБ,
														ИсходныйОбъектБД,
														Результат.ОбработанныйОбъектБД,
														Результат.ПредставлениеОбъектаВИБ);
														
	КонецЦикла;	
	
КонецПроцедуры // ЗаписатьСоответствияОбъектов()

Функция ВИБ_ПолучитьСоответствиеСчетов(ИспользуемаяИБ,ТаблицаАналитикиВИБ,МассивСчетов) Экспорт
	
	Запрос=Новый Запрос;
	ТекстЗапроса="ВЫБРАТЬ
	|	РабочийПланСчетов.Код КАК СчетКод
	|ИЗ
	|	&РабочийПланСчетов КАК РабочийПланСчетов
	|ГДЕ
	|	РабочийПланСчетов.Ссылка В(&МассивСчетов)";
	
	Запрос.Текст=СтрЗаменить(ТекстЗапроса,"&РабочийПланСчетов",ТаблицаАналитикиВИБ);
	Запрос.УстановитьПараметр("МассивСчетов",МассивСчетов);
	МассивКодов=Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("СчетКод");
	
	СтруктураПараметров=Новый Структура;
	СтруктураПараметров.Вставить("ТаблицаАналитикиВИБ",	ТаблицаАналитикиВИБ);
	СтруктураПараметров.Вставить("МассивКодов",			МассивКодов); 

	ТекСоединениеВИБ = УправлениеСоединениямиВИБУХ.ПолучитьСоединение(ИспользуемаяИБ, 1);
			
	Если ТипЗнч(ТекСоединениеВИБ) = Тип("WSProxy") Тогда 
		
		ТаблицаСчетов=ТекСоединениеВИБ.GetAccountUUID(Новый ХранилищеЗначения(СтруктураПараметров, Новый СжатиеДанных(9))).Получить();
		
	ИначеЕсли ТипЗнч(ТекСоединениеВИБ) = Тип("Строка") Тогда 
		
		ТаблицаСчетов=РаботаСОбъектамиМетаданныхВнешнийУХ.ПолучитьОтвет1СШина(ТекСоединениеВИБ,"GetAccountUUID",СтруктураПараметров); 
	...
	Иначе
		...
		ТаблицаСчетов=РаботаСОбъектамиМетаданныхВнешнийУХ.GetAccountUUID(ТекСоединениеВИБ,СтруктураПараметров);
		
	КонецЕсли; 
	
	Запрос=Новый Запрос;
	ТекстЗапроса="ВЫБРАТЬ
	|	ТаблицаСчетов.GUID КАК GUID,
	|	ТаблицаСчетов.СчетКод КАК СчетКод
	...
	|ВЫБРАТЬ
	|	ТаблицаСчетов.СчетКод КАК СчетКод,
	|	ЕСТЬNULL(РабочийПланСчетов.Ссылка, НЕОПРЕДЕЛЕНО) КАК ИсходныйОбъектБД,
	|	ТаблицаСчетов.GUID КАК ОбработанныйобъектБД 	
	|ИЗ
	|	ТаблицаСчетов КАК ТаблицаСчетов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ &РабочийПланСчетов КАК РабочийПланСчетов
	|		ПО ТаблицаСчетов.СчетКод = РабочийПланСчетов.Код";
	...
	ТаблицаКЗаписи=РабочаяТаблица.СкопироватьКолонки();
	ТаблицаКЗаписи.Колонки.Добавить("ПредставлениеОбъектаВИБ",РабочаяТаблица.Колонки.СчетКод.ТипЗначения);
	
	СоответствиеОбъектов=Новый Соответствие;
	
	Для Каждого Строка ИЗ РабочаяТаблица Цикл
		
		Если ЗначениеЗаполнено(Строка.ИсходныйОбъектБД)
			И ЗначениеЗаполнено(Строка.ОбработанныйобъектБД) Тогда
			
			СтруктураДанных=Новый Структура;
			СтруктураДанных.Вставить("ТаблицаАналитикиВИБ",ТаблицаАналитикиВИБ);
			СтруктураДанных.Вставить("GUID",Строка.ОбработанныйОбъектБД);
						
			СоответствиеОбъектов.Вставить(XMLСтрока(Строка.ИсходныйОбъектБД)+"_tt_"+ТаблицаАналитикиВИБ,СтруктураДанных);
						
			НоваяСтрока=ТаблицаКЗаписи.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока,Строка);
			НоваяСтрока.ПредставлениеОбъектаВИБ=Строка.СчетКод;
			
		КонецЕсли;
		
	КонецЦикла;
	
	ЗаписатьСоответствияОбъектовПриВыгрузке(Справочники.СоответствиеВнешнимИБ.ПустаяСсылка(),ТаблицаКЗаписи,Неопределено,ИспользуемаяИБ);
	
	Возврат СоответствиеОбъектов;
		
КонецФункции // ВИБ_ПолучитьСоответствиеСчетов()

Процедура ВыгрузитьДанныеСвязанныхОбъектов(ТекСоединениеВИБ,ДанныеНастройки,Результат,КэшОбъектовДляВыгрузки,ИспользуемаяИБ)
	...
	Запрос.Текст="ВЫБРАТЬ
				 ...
				 |	СоответствиеВнешнимИБСоответствияСвязанныхОбъектов.НастройкаСоответствия.ТаблицаАналитикиТек КАК ТаблицаАналитикиТек,
	             |	ВЫБОР
	...
	СоответствиеОбъектов=Новый Соответствие;
	МассивСсылокТек=Новый Массив;
	МассивСсылокВИБ=Новый Массив;
	
	Для Каждого Строка ИЗ Результат Цикл 
		
		СсылкаТекущейИБ=КэшОбъектовДляВыгрузки.Получить(Строка.ИсходныйОбъектБД);
        МассивСсылокТек.Добавить(СсылкаТекущейИБ);
		МассивСсылокВИБ.Добавить(Строка.ОбработанныйОбъектБД);
		СоответствиеОбъектов.Вставить(СсылкаТекущейИБ,Строка.ОбработанныйОбъектБД);
			
	КонецЦикла;
		
	Для Каждого СтрокаНастройка ИЗ ТабСвязанныхОбъектов Цикл
		
		ТаблицаДляПереноса=Экспорт_ПолучитьРабочуюТаблицуСвязанныхОбъектов(ТекСоединениеВИБ,СтрокаНастройка,ДанныеНастройки.ТаблицаАналитикиТек,МассивСсылокТек,ИспользуемаяИБ);
		
		ТаблицаДляПереноса.Колонки.Добавить("_GUIDВнешнейИБ");
		
		Для Каждого Строка ИЗ ТаблицаДляПереноса Цикл
			
			Строка._GUIDВнешнейИБ=СоответствиеОбъектов.Получить(Строка.Ссылка);
			
		КонецЦикла;
		
		Если ТаблицаДляПереноса.Количество()>100 Тогда
			
			ТаблицаДляПереноса.Индексы.Добавить("_GUIDВнешнейИБ");
			
		КонецЕсли;
		
		ТаблицаДляПереноса.Колонки.Удалить("Ссылка");
		
		СтрокаНастройка.РабочаяТаблица=ТаблицаДляПереноса;
		
	КонецЦикла; 
	
	СтруктураДанных=Новый Структура;
	СтруктураДанных.Вставить("ТаблицаАналитикиВИБ",ДанныеНастройки.ТаблицаАналитикиВИБ); 
	СтруктураДанных.Вставить("МассивСсылокВИБ",МассивСсылокВИБ);
	СтруктураДанных.Вставить("ТабСвязанныхОбъектов",ТабСвязанныхОбъектов);
	
	РезультатВыгрузки=Экспорт_ЗаполнитьСвязанныеОбъекты(ТекСоединениеВИБ,СтруктураДанных);
		
КонецПроцедуры // ВыгрузитьДанныеСвязанныхОбъектов()

Функция Экспорт_ЗаполнитьСвязанныеОбъекты(ТекСоединениеВИБ,СтруктураДанных) 
	
	Если ТипЗнч(ТекСоединениеВИБ) = Тип("WSProxy") Тогда 
				
		Возврат ТекСоединениеВИБ.FillRelatedTableData(Новый ХранилищеЗначения(СтруктураДанных, Новый СжатиеДанных(9))).Получить();
		
	ИначеЕсли ТипЗнч(ТекСоединениеВИБ) = Тип("Строка") Тогда 
		
		Возврат РаботаСОбъектамиМетаданныхВнешнийУХ.ПолучитьОтвет1СШина(ТекСоединениеВИБ,"FillRelatedTableData",СтруктураДанных); 
		
	Иначе
		
		Возврат РаботаСОбъектамиМетаданныхВнешнийУХ.FillRelatedTableData(ТекСоединениеВИБ,СтруктураДанных);
		
	КонецЕсли;
	
	
КонецФункции // Экспорт_ЗаполнитьСвязанныеОбъекты() 

Функция Экспорт_ПолучитьРабочуюТаблицуСвязанныхОбъектов(ТекСоединениеВИБ,СтрокаНастройка,ТаблицаАналитикиТек,МассивСсылок,ИспользуемаяИБ)
	...
	Пока Результат.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Результат.РеквизитОбъектаКонсолидации) Тогда
			
			ПолеВИБ=УбратьПолеСсылка(Результат.РеквизитОбъектаВнешнейИБ);
			ПолеТек=УбратьПолеСсылка(Результат.РеквизитОбъектаКонсолидации);
			
			Если МассивСинонимов.Найти(ПолеВИБ)=Неопределено Тогда
				
				НоваяСтрока=ТаблицаПолей.Добавить();
				НоваяСтрока.ПолеБД=ПолеТек;
				НоваяСтрока.ПолеПриемник=ПолеВИБ;
				
				МассивСинонимов.Добавить(ПолеВИБ);
		...
		ИначеЕсли ЗначениеЗаполнено(Результат.ФиксированноеЗначение) Тогда
			...
			НоваяСтрока.ПолеПриемник=УбратьПолеСсылка(Результат.РеквизитОбъектаВнешнейИБ);
	...
	ДанныеВладельца.Вставить("ТаблицаАналитикиТек",ТаблицаАналитикиТек);
	ДанныеВладельца.Вставить("МассивСсылок",МассивСсылок);
	
	ДанныеДляВыборки=Новый Структура;
	ДанныеДляВыборки.Вставить("ТаблицаАналитикиТек",?(СтрокаНастройка.ТипОбъектаТек="ТабличнаяЧасть",ТаблицаАналитикиТек+"."+СтрокаНастройка.ИмяОбъектаТек,СтрокаНастройка.ТаблицаАналитикиТек));
	ДанныеДляВыборки.Вставить("ПолеСсылка",?(СтрокаНастройка.ТипОбъектаТек="ТабличнаяЧасть","Ссылка",СтрокаНастройка.ИзмерениеРегистраТек));
	ДанныеДляВыборки.Вставить("ТаблицаПолей",ТаблицаПолей);	
	
	ДанныеТаблицы=Экспорт_ПолучитьДанныеСвязаннойТаблицы(ДанныеВладельца,ДанныеДляВыборки);
	ТаблицаДляПереноса=Экспорт_ПолучитьТаблицуПоДаннымПереноса(ИспользуемаяИБ,ДанныеТаблицы);
			
	Возврат ТаблицаДляПереноса;
		
КонецФункции // ПолучитьРабочуюТаблицу()

Функция Экспорт_ПолучитьТаблицуПоДаннымПереноса(ДочерняяБаза,ДанныеДляПереноса) Экспорт
	...
	Иначе
		...
		СоответствиеСсылок=Экспорт_ПолучитьСоответствиеСсылок(ДочерняяБаза,ДанныеДляПереноса.ТаблицаЗначенийПолей); 
		
		Возврат Экспорт_ПолучитьРабочуюТаблицу(ДанныеДляПереноса.ТаблицаПереноса,СоответствиеСсылок);
		
	КонецЕсли;
		
КонецФункции // ПолучитьРабочуюТаблицуПоДаннымПереноса()

Функция Экспорт_ПолучитьРабочуюТаблицу(ИсходнаяТаблица,СоответствиеСсылок)
...
Функция ПолучитьЗапросПоТаблицамТек(УзелИнформационнойБазы)
	...
	Если НЕ ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
		...
		Запрос.Текст="ВЫБРАТЬ
		|	ТаблицаЗначенийПолей.ТаблицаАналитикиТек КАК ТаблицаАналитикиТек,
		|	ТаблицаЗначенийПолей.Ссылка КАК Ссылка,
		...
		|	И НЕ СоответствиеВнешнимИБ.НеИспользоватьПриЭкспорте
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|		ПО ТаблицаЗначенийПолей.ТаблицаАналитикиТек = НастройкиСоответствия.ТаблицаАналитикиТек
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ТабУстановленныеСоответствия.Ссылка КАК Ссылка,
		...
		|	ЕСТЬNULL(СоответствиеОбъектов.ОбъектВнешнейИБ, НЕОПРЕДЕЛЕНО) КАК ОбъектВнешнейИБ,
		...
		|			И ТабУстановленныеСоответствия.Ссылка = СоответствиеОбъектов.ОбъектТекущейИБ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Иначе
		...
		|	ТабУстановленныеСоответствия.СинхронизацияПоGUID КАК СинхронизацияПоGUID,
		|	ВЫБОР
		|		КОГДА СоответствиеОбъектов.ОбъектВнешнейИБ ЕСТЬ NULL
		|			ТОГДА ЕСТЬNULL(ПубличныеИдентификаторы.Идентификатор, НЕОПРЕДЕЛЕНО)
		|		ИНАЧЕ СоответствиеОбъектов.ОбъектВнешнейИБ
		|	КОНЕЦ КАК ОбъектВнешнейИБ,
		|	ТабУстановленныеСоответствия.ТаблицаАналитикиТек КАК ТаблицаАналитикиТек
		|ИЗ
		|	ТабУстановленныеСоответствия КАК ТабУстановленныеСоответствия
		|		ЛЕВОЕ СОЕДИНЕНИЕ СоответствиеОбъектов КАК СоответствиеОбъектов
		...
		|		ПО ТабУстановленныеСоответствия.Ссылка = ПубличныеИдентификаторы.Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
КонецФункции // ПолучитьЗапросПоТаблицамТек) 

Функция ПолучитьЗапросПоТаблицамТекиИменамНастроек(УзелИнформационнойБазы)
	...
	Если НЕ ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
		...
		|	ВЫБОР
		|		КОГДА НЕ НастройкиСоответствияИмена.НастройкаСоответствия ЕСТЬ NULL
		|			ТОГДА НастройкиСоответствияИмена.ТаблицаАналитикиВИБ
		|		ИНАЧЕ ЕСТЬNULL(НастройкиСоответствияТаблица.ТаблицаАналитикиВИБ, НЕОПРЕДЕЛЕНО)
		|	КОНЕЦ КАК ТаблицаАналитикиВИБ
		|ПОМЕСТИТЬ ТабУстановленныеСоответствия
		|ИЗ
		|	ТаблицаЗначенийПолей КАК ТаблицаЗначенийПолей
		|		ЛЕВОЕ СОЕДИНЕНИЕ НастройкиСоответствия КАК НастройкиСоответствияТаблица
		|		ПО ТаблицаЗначенийПолей.ТаблицаАналитикиТек = НастройкиСоответствияТаблица.ТаблицаАналитикиТек
...
КонецФункции // ПолучитьЗапросПоТаблицамТекиИменамНастроек() 

Функция Экспорт_ПолучитьСоответствиеСсылок(ДочерняяБаза,ТаблицаЗначенийПолей)
	...
	Если ТаблицаЗначенийПолей.Колонки.Найти("НастройкаСоответствия")=Неопределено Тогда
		...
		Запрос=ПолучитьЗапросПоТаблицамТек(РеквизитыИБ.УзелИнформационнойБазы);
		
	Иначе
		
		Запрос=ПолучитьЗапросПоТаблицамТекиИменамНастроек(РеквизитыИБ.УзелИнформационнойБазы);
	...
	Если ЗначениеЗаполнено(РеквизитыИБ.УзелИнформационнойБазы) Тогда
		...
		Возврат Экспорт_ВернутьСоответствиеСсылокСУчетомED(Результат,ДочерняяБаза);		
		
	Иначе
		
		Возврат Экспорт_ВернутьСоответствиеСсылок(Результат,ДочерняяБаза,ТипБД);
		
	КонецЕсли;
		
КонецФункции // ПолучитьСоответствиеСсылок()

Функция Экспорт_ВернутьСоответствиеСсылокСУчетомED(Результат,ДочерняяБаза)
	...
	Для Каждого СтрокаСоответствия ИЗ ТабНастройкиСоответствия  Цикл
		...
		Если ЗначениеЗаполнено(СтрокаСоответствия.НастройкаСоответствия) Тогда
			...
			ТаблицаАналитикиВИБ=СтрокаСоответствия.ТаблицаАналитикиВИБ;
		...
		Иначе
			...
			ТаблицаАналитикиВИБ=СтрокаСоответствия.ТаблицаАналитикиТек;
			МассивДанные=РабТаблицаЗначенийПолей.НайтиСтроки(Новый Структура("ТаблицаАналитикиТек",СтрокаСоответствия.ТаблицаАналитикиТек));
		
		КонецЕсли; 
		
		Если СтрНайти(ТаблицаАналитикиВИБ,"Перечисление.")>0 Тогда
			
			ИмяПеречисления=СтрЗаменить(ТаблицаАналитикиВИБ,"Перечисление.","");
		...
		Для Каждого СтрокаДанные ИЗ МассивДанные Цикл
			...
			Если ЗначениеЗаполнено(СтрокаДанные.ОбъектВнешнейИБ) Тогда
				
				СтруктураДанных=Новый Структура;
				СтруктураДанных.Вставить("ТаблицаАналитикиВИБ",ТаблицаАналитикиВИБ);
				СтруктураДанных.Вставить("GUID",СтрокаДанные.ОбъектВнешнейИБ);	
				СоответствиеСсылок.Вставить(СтрокаДанные.ПредставлениеGUID,СтруктураДанных);
			 ...
			 ИначеЕсли НЕ ИмяПеречисления=Неопределено Тогда
				 ...
				 СтруктураДанных=Новый Структура;
				 СтруктураДанных.Вставить("ТаблицаАналитикиВИБ",ТаблицаАналитикиВИБ);
				 СтруктураДанных.Вставить("ЗначениеПеречисления",ПолучитьЗначениеПеречисленияВИБ(ТаблицаАналитикиВИБ,СтрокаДанные.НастройкаСоответствия,XMLСтрока(СтрокаДанные.Ссылка)));
				 		 
				 СоответствиеСсылок.Вставить(СтрокаДанные.ПредставлениеGUID,СтруктураДанных);

			 ИначеЕсли ЗначениеЗаполнено(СтрокаДанные.Ссылка) Тогда
				 
				 Если НЕ ЗначениеЗаполнено(СтрокаСоответствия.НастройкаСоответствия) Тогда
					 
					 Если СтрНайти(ТаблицаАналитикиВИБ,"ПланСчетов.")>0 Тогда
						 
						 МассивСчетов.Добавить(СтрокаДанные.Ссылка);
						 
					 КонецЕсли;
					 
				 Иначе
					 
					 МассивСсылок.Добавить(СтрокаДанные.Ссылка);
		...
		Если МассивСсылок.Количество()=0
			И МассивСчетов.Количество()=0 Тогда
		...
		Если ЗначениеЗаполнено(СтрокаСоответствия.НастройкаСоответствия) Тогда
			...
			ТекСоответствиеСсылок=ИнтеграцияСВнешнимиСистемамиУХ.ВыгрузитьОбъектыВВИБ(ДочерняяБаза,
			СтрокаСоответствия.НастройкаСоответствия,МассивСсылок,Истина,Истина,Неопределено,Истина);
		...
		ИначеЕсли МассивСчетов.Количество()>0 Тогда 
			...
			ТекСоответствиеСсылок=ИнтеграцияСВнешнимиСистемамиУХ.ВИБ_ПолучитьСоответствиеСчетов(ДочерняяБаза,ТаблицаАналитикиВИБ,МассивСчетов);
	...
	Экспорт_ПроверитьСинхронизациюОбъектовВИБ(ДочерняяБаза,СоответствиеСсылок,РабТаблицаЗначенийПолей);
	
	Возврат СоответствиеСсылок;
	
	
	
КонецФункции // Экспорт_ВернутьСоответствиеСсылокСУчетомED()

Функция Экспорт_ВернутьСоответствиеСсылок(Результат,ДочерняяБаза,ТипБД)
	...
	Для Каждого СтрокаСоответствия ИЗ ТабНастройкиСоответствия  Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаСоответствия.НастройкаСоответствия) Тогда
			...
			ОбщегоНазначенияУХ.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Для типа информационной базы %1 не найдена настройка соответствия для объекта текущей ИБ %2'"),ТипБД,СтрокаСоответствия.ТаблицаАналитикиТек),,,СтатусСообщения.Важное);
			Продолжить;
		...
		Если СтрНайти(СтрокаСоответствия.ТаблицаАналитикиВИБ,"Перечисление.")>0 Тогда
			...
			ИмяПеречисления=СтрЗаменить(СтрокаСоответствия.ТаблицаАналитикиВИБ,"Перечисление.","");
		...
		Для Каждого СтрокаДанные ИЗ МассивДанные Цикл
			...
			Если ЗначениеЗаполнено(СтрокаДанные.ОбъектВнешнейИБ) Тогда
				...
				СтруктураДанных=Новый Структура;
				СтруктураДанных.Вставить("ТаблицаАналитикиВИБ",СтрокаДанные.ТаблицаАналитикиВИБ);
				СтруктураДанных.Вставить("GUID",СтрокаДанные.ОбъектВнешнейИБ);	
				СоответствиеСсылок.Вставить(СтрокаДанные.ПредставлениеGUID,СтруктураДанных);
			 ...
			 ИначеЕсли НЕ ИмяПеречисления=Неопределено Тогда
				 ...
				 СтруктураДанных=Новый Структура;
				 СтруктураДанных.Вставить("ТаблицаАналитикиВИБ",СтрокаДанные.ТаблицаАналитикиВИБ);
				 СтруктураДанных.Вставить("ЗначениеПеречисления",ПолучитьЗначениеПеречисленияВИБ(СтрокаДанные.ТаблицаАналитикиВИБ,СтрокаДанные.НастройкаСоответствия,XMLСтрока(СтрокаДанные.Ссылка)));
...
КонецФункции // Экспорт_ВернутьСоответствиеСсылок() 

Процедура Экспорт_ПроверитьСинхронизациюОбъектовВИБ(ДочерняяБаза,СоответствиеСсылок,РабТаблицаЗначенийПолей)
	...
	Если СоответствиеСсылок.Количество()<РабТаблицаЗначенийПолей.Количество() Тогда
		...
		Для Каждого СтрокаДанные ИЗ РабТаблицаЗначенийПолей Цикл
			...
			Если СинхронизированныйОбъект=Неопределено И ЗначениеЗаполнено(СтрокаДанные.Ссылка) Тогда
				
				МассивНеСинхронизированных.Добавить(Новый Структура("ТаблицаАналитикиТек,Ссылка",СтрокаДанные.ТаблицаАналитикиТек,СтрокаДанные.Ссылка));
	...
	Для Каждого Элемент ИЗ МассивНеСинхронизированных Цикл 
		...
		ТекстСообщения=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Не удалось сопоставить объект внешней ИБ %1 объекту текущей информационной базы: '"),ДочерняяБаза);
		ТекстОшибки=Строка(Элемент.Ссылка)+" ("+Элемент.ТаблицаАналитикиТек+")";
...
Функция ПолучитьЗначениеПеречисленияВИБ(ТаблицаАналитикиВИБ,НастройкаСоответствия,ЗначениеТекущейИБ)

	Если СтрНайти(ТаблицаАналитикиВИБ,"Перечисление.")=0
		ИЛИ (НЕ ЗначениеЗаполнено(НастройкаСоответствия)) Тогда
		
		Возврат ЗначениеТекущейИБ;
	...
	|ГДЕ
	|	СоответствиеВнешнимИБСоответствиеЗначенийПеречислений.Ссылка = &НастройкаСоответствия
	|	И СоответствиеВнешнимИБСоответствиеЗначенийПеречислений.ЗначениеТекущейИБ = &ЗначениеТекущейИБ";
	
	Запрос.УстановитьПараметр("НастройкаСоответствия",НастройкаСоответствия);
	Запрос.УстановитьПараметр("ЗначениеТекущейИБ",ЗначениеТекущейИБ);
	
	Результат=Запрос.Выполнить().Выбрать();
	
	Если Результат.Следующий() Тогда
		
		Возврат Результат.ЗначениеВнешнейИБ;
...
//
Функция Экспорт_ПолучитьДанныеСвязаннойТаблицы(ДанныеВладельца,ДанныеДляВыборки) Экспорт
	...
	ТекстЗапроса=ТекстЗапроса+"
	|ИЗ &ТаблицаАналитикиТек КАК СвязаннаяТаблица
	|ГДЕ &ДанныеОтбора";
	
	ТекстЗапроса=СтрЗаменить(ТекстЗапроса,"&ТаблицаАналитикиТек",ДанныеДляВыборки.ТаблицаАналитикиТек);
	ТекстЗапроса=СтрЗаменить(ТекстЗапроса,"&ДанныеОтбора","СвязаннаяТаблица."+ДанныеДляВыборки.ПолеСсылка+ " В(&МассивСсылок)");
	
	Запрос.Текст=ТекстЗапроса;
	Запрос.УстановитьПараметр("МассивСсылок",ДанныеВладельца.МассивСсылок);
	
	ТаблицаТек=Запрос.Выполнить().Выгрузить();
	
	Возврат Экспорт_ПолучитьДанныеДляПереноса(ТаблицаТек);
		
КонецФункции // ПолучитьДанныеСвязаннойТаблицыТек()

Процедура ДобавитьТипыВМассив(МассивВсеСсылки,МассивТипов)
	
	Для Каждого Тип Из МассивТипов Цикл
		
		МассивВсеСсылки.Добавить(Тип);
		
	КонецЦикла;
	
	
КонецПроцедуры // ДобавитьТипыВМассив() 

Функция ПолучитьОписаниеТиповСсылкаТекущаяИБ() Экспорт

	МассивВсеСсылки = Новый Массив;
	ДобавитьТипыВМассив(МассивВсеСсылки,Справочники.ТипВсеСсылки().Типы());
	ДобавитьТипыВМассив(МассивВсеСсылки,Перечисления.ТипВсеСсылки().Типы());
	ДобавитьТипыВМассив(МассивВсеСсылки,ПланыВидовХарактеристик.ТипВсеСсылки().Типы());
	ДобавитьТипыВМассив(МассивВсеСсылки,ПланыСчетов.ТипВсеСсылки().Типы());
		
	Возврат Новый ОписаниеТипов(МассивВсеСсылки);
	
КонецФункции 

Функция Экспорт_ПолучитьДанныеДляПереноса(ТаблицаДанных) Экспорт
			
	ТаблицаЗначенийПолей=Новый ТаблицаЗначений;
	ТаблицаЗначенийПолей.Колонки.Добавить("Ссылка",ПолучитьОписаниеТиповСсылкаТекущаяИБ());
	ТаблицаЗначенийПолей.Колонки.Добавить("GUID",КэшируемыеПроцедурыУХ.ПолучитьОписаниеТиповСтроки(40));
	ТаблицаЗначенийПолей.Колонки.Добавить("ТаблицаАналитикиТек",КэшируемыеПроцедурыУХ.ПолучитьОписаниеТиповСтроки(200));
	ТаблицаЗначенийПолей.Колонки.Добавить("ПредставлениеGUID",КэшируемыеПроцедурыУХ.ПолучитьОписаниеТиповСтроки(200));
	...
	Для ИндексДанных=0 По ТаблицаДанных.Колонки.Количество()-1 Цикл
	...
	Если ЕстьИменаНастроек Тогда
		...
		ТаблицаЗначенийПолей.Колонки.Добавить("НастройкаСоответствия",КэшируемыеПроцедурыУХ.ПолучитьОписаниеТиповСтроки(200));

	КонецЕсли;	
	
	КэшДанныхОбъектов=Новый Соответствие;
	...
	Для ИндексСтроки=0 По ТаблицаДанных.Количество()-1 Цикл
		...
		Для ИндексКолонки=0 ПО ТаблицаДанных.Колонки.Количество()-1 Цикл
			...
			ИначеЕсли ИмяКолонки="ИсходныйОбъектСсылка" Тогда
				
				СтрокаВыгрузки[ИмяКолонки]=XMLСтрока(ЗначениеПоля);
				
			ИначеЕсли ЗначениеПоля=Неопределено ИЛИ РаботаСОбъектамиМетаданныхВнешнийУХ.ПримитивныйТип(ТипЗнч(ЗначениеПоля)) Тогда
			...
			Иначе
				...
				ДанныеДляЗагрузки=РаботаСОбъектамиМетаданныхВнешнийУХ.ПолучитьДанныеОбъектаДляПереноса(Неопределено,ЗначениеПоля,КэшДанныхОбъектов,ПолеИмяНастройки);
				...
				Если НЕ ДанныеДляЗагрузки=Неопределено Тогда
					...
					Если ТаблицаЗначенийПолей.НайтиСтроки(Новый Структура("ПредставлениеGUID",ПредставлениеGUID)).Количество()=0 Тогда 
						...
						НоваяСтрока.Ссылка=ЗначениеПоля;
						НоваяСтрока.ТаблицаАналитикиТек=ДанныеДляЗагрузки.ТаблицаАналитики;
	...
	Если ЕстьИменаНастроек Тогда
		...
		ТаблицаЗначенийПолей.Свернуть("GUID,Ссылка,ТаблицаАналитикиТек,НастройкаСоответствия,ПредставлениеGUID");
		
	Иначе	
		
		ТаблицаЗначенийПолей.Свернуть("GUID,Ссылка,ТаблицаАналитикиТек,ПредставлениеGUID"); 
...
КонецФункции // Экспорт_ПолучитьДанныеДляПереноса()

Функция ДополнитьТаблицуВнешнимиGUID(ТаблицаДанных,ИспользуемаяИБ,ПравилаИспользованияПолей)
	...
	Для Каждого Строка ИЗ ПравилаИспользованияПолей Цикл
		...
		Если НЕ Строка.СинхронизацияПоGUID Тогда
		...
		Иначе
			...
			ТекстВыбор=ТекстВыбор+",
			|ЕСТЬNULL(Таблица_"+Строка.КодАналитики+".ОбъектВнешнейИБ,"""") КАК "+Строка.Синоним;
			
			ТекстСоединенияТаблица=ТекстСоединенияТаблица+"
			|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеОбъектовТекущейИВнешнихИБ КАК Таблица_"+Строка.КодАналитики+" ПО
			|ВТ."+Строка.Синоним+"=Таблица_"+Строка.КодАналитики+".ОбъектТекущейИБ
			|И Таблица_"+Строка.КодАналитики+".ИспользуемаяИБ=&ИспользуемаяИБ
			|И Таблица_"+Строка.КодАналитики+".НастройкаСоответствия=&НастройкаСоответствия_"+Строка.КодАналитики;
			
			Запрос.УстановитьПараметр("НастройкаСоответствия_"+Строка.КодАналитики,Строка.НастройкаСоответствия);	
			Запрос.УстановитьПараметр("ИспользуемаяИБ",ИспользуемаяИБ);
	...
	Запрос.Текст="Выбрать "+Сред(ТекстИменКолонок,2)+"
	|Поместить ВременнаяТаблица
	|ИЗ &ТаблицаДанных КАК ТаблицаДанных
	|;
	|ВЫБРАТЬ "+Сред(ТекстВыбор,2)+"
...
КонецФункции // ДополнитьТаблицуВнешнимиGUID() 
...
Функция ВИБ_ПолучитьДанныеДляСинхронизации(МассивСоставных,ТаблицаВИБ,ТабПравилаДляВыгрузки)
	
	СтрокаКолонки="";
			
	СтрокаКолонки=СтрСоединить(МассивСоставных,",");
	ДанныеДляСинхронизации=ТаблицаВИБ.Скопировать(,СтрокаКолонки);
	ДанныеДляСинхронизации.Свернуть(СтрокаКолонки);
	
	Для Каждого СтрПоле ИЗ МассивСоставных Цикл
		
		ДанныеДляСинхронизации.Columns.Add(СтрПоле+"_ВИБ");
		
	КонецЦикла;
	
	Для Каждого Строка ИЗ ДанныеДляСинхронизации Цикл
		
		Для Каждого СтрПоле ИЗ МассивСоставных Цикл
			
			КодАналитики=ТабПравилаДляВыгрузки.Найти(СтрПоле,"Синоним").КодАналитики;
			МассивПолей=ТабПравилаДляВыгрузки.НайтиСтроки(Новый Структура("КодАналитики,ТаблицаАналитики",КодАналитики,Строка[СтрПоле]));
			
			Если МассивПолей.Количество()>0 Тогда
				
				Строка[СтрПоле+"_ВИБ"]=МассивПолей[0].ТаблицаАналитикиВИБ;
				
			Иначе
				
				ОбщегоНазначенияУХ.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Не найдена настройка соответствия для объекта %1'"),Строка[СтрПоле]),,,СтатусСообщения.Важное);
	...
	Возврат Новый Структура("ДанныеДляСинхронизации,МассивСоставных",ДанныеДляСинхронизации,МассивСоставных);
	
	
КонецФункции // ВИБ_ПолучитьДанныеДляСинхронизации() 

Функция Экспорт_СоздатьОбновитьЭлементыНСИ(ТекСоединениеВИБ,
		ТаблицаДанных,
		ТабПравилаДляВыгрузки,
		СтруктураКлючевыхРеквизитов,
		СтруктураПодчиненныхРеквизитов,
		ОбновлениеДочернихЭлементов,
		СоздаватьПомеченныеНаУдаление)
	...
	СтруктураПараметров.Вставить("СоздаватьПомеченныеНаУдаление",СоздаватьПомеченныеНаУдаление);
	
	Если ТипЗнч(ТекСоединениеВИБ) = Тип("WSProxy")
		ИЛИ ТипЗнч(ТекСоединениеВИБ)=Тип("Строка") Тогда
		
		ТекущаяИБ=РаботаСОбъектамиМетаданныхВнешнийУХ.КонтекстМетаданных();
		ТаблицаВИБ=ПеренестиТаблицуДанныхНаСторонуВИБ(ТекущаяИБ,ТаблицаДанных);
	...
	МассивСоставных=Новый Массив;
	
	Для Каждого СтрПоле ИЗ ТабПравилаДляВыгрузки Цикл
		
		Если СтрПоле.НеИспользоватьДляСинхронизации
			ИЛИ СтрНайти(СтрПоле.Синоним,"vzvСсылка")=0 Тогда
			
			Продолжить;
			
		КонецЕсли;
		
		ДанныеПоля=СтрРазделить(СтрПоле.Поле,".");
		
		Если ДанныеПоля[ДанныеПоля.Количество()-1]="Ссылка" Тогда
			
			МассивСоставных.Добавить(СтрПоле.Синоним);
			
		КонецЕсли;
		
	КонецЦикла;
	
	Если МассивСоставных.Количество()>0 Тогда
		
		СинхронизацияПоСоставнымТипам=ВИБ_ПолучитьДанныеДляСинхронизации(МассивСоставных,ТаблицаВИБ,ТабПравилаДляВыгрузки);
		СтруктураПараметров.Вставить("СинхронизацияПоСоставнымТипам",СинхронизацияПоСоставнымТипам);
	...
	Если ТипЗнч(ТекСоединениеВИБ) = Тип("WSProxy")
		...
		СтруктураПараметров.Вставить("ТаблицаДанных",ТаблицаВИБ);
		СтруктураПараметров.Вставить("ТаблицаВИБ",ТаблицаВИБ);
						
		Если ТипЗнч(ТекСоединениеВИБ) = Тип("WSProxy") Тогда 
				
			РезультатВыполнения= ТекСоединениеВИБ.CreateRefreshMDMItems(Новый ХранилищеЗначения(СтруктураПараметров, Новый СжатиеДанных(9))).Получить();
				
			Возврат РезультатВыполнения; 
			
		Иначе
			
			Возврат РаботаСОбъектамиМетаданныхВнешнийУХ.ПолучитьОтвет1СШина(ТекСоединениеВИБ,"CreateRefreshMDMItems",СтруктураПараметров);
	...
	Иначе 
		...
		Возврат РаботаСОбъектамиМетаданныхВнешнийУХ.CreateRefreshMDMItems(ТекСоединениеВИБ,СтруктураПараметров);
		
	КонецЕсли;
		
КонецФункции // Экспорт_СоздатьОбновитьЭлементыНСИ()


Процедура ВыгрузитьПодчиненныеОбъекты(ТекСоединениеВИБ,ИспользуемаяИБ,ТабОбъектовДляВыгрузки)
	
	АналитикиДляВыгрузки=Новый ТаблицаЗначений;
	АналитикиДляВыгрузки.Колонки.Добавить("ТаблицаАналитики",РаботаСОбъектамиМетаданныхВнешнийУХ.ПолучитьОписаниеТиповСтроки(255));
	
	Для Каждого Строка ИЗ ТабОбъектовДляВыгрузки Цикл
		
		НоваяСтрока=АналитикиДляВыгрузки.Добавить();
		НоваяСтрока.ТаблицаАналитики=Строка.ТаблицаАналитики;
		
	КонецЦикла;
	
	АналитикиДляВыгрузки.Свернуть("ТаблицаАналитики");
	
	Результат=ПолучитьЗапросНастройкиСоответствия(ИспользуемаяИБ.ТипБД,АналитикиДляВыгрузки);
		
	Пока Результат.Следующий() Цикл
		
		ТекНастройкаСоответствия=Результат.НастройкаСоответствия;
		СписокОбъектов=Новый СписокЗначений;
		
		МассивДанныхВыгрузки=ТабОбъектовДляВыгрузки.НайтиСтроки(Новый Структура("ТаблицаАналитики",Результат.ТаблицаАналитики));
		
		СписокОбъектов=ПолучитьСписокЭлементовДляВыгрузки(Результат.ТаблицаАналитики,МассивДанныхВыгрузки);
...
Функция ПолучитьСписокЭлементовДляВыгрузки(ТаблицаАналитики,МассивДанныхВыгрузки) Экспорт
	 
   	Запрос=Новый Запрос;
	Запрос.Текст="ВЫБРАТЬ
	|СправочникБД.Ссылка
	|ИЗ "+ТаблицаАналитики+" КАК СправочникБД";
	...
	Для Каждого СтрокаДанных ИЗ МассивДанныхВыгрузки Цикл 
		...
		Для Каждого КлючИЗначение ИЗ СтрокаДанных.СтруктураРеквизитов Цикл
			...
			Если СтрНайти(КлючИЗначение.Ключ,СтрокаДанных.КодАналитики)=1 Тогда
				...
				ТекстОтборЭлемент=ТекстОтборЭлемент+" И "+ТекстПоля+"=&Параметр"+НомерПараметра;
			...
			ИначеЕсли СтрНайти(КлючИЗначение.Ключ,"НСИ_")>0 Тогда
				...
				ТекстОтборЭлемент=ТекстОтборЭлемент+" И СправочникБД."+КлючИЗначение.Ключ+"=&Параметр"+НомерПараметра;
				Запрос.УстановитьПараметр("Параметр"+НомерПараметра,КлючИЗначение.Значение);
		...
		Если НЕ ПустаяСтрока(ТекстОтборЭлемент) Тогда
			
			ТекстОтбор=ТекстОтбор+" ИЛИ ("+Сред(ТекстОтборЭлемент,(СтрДлина("И")+2))+")";
...
Функция ПолучитьЗапросНастройкиСоответствия(ТипБД,МассивТаблицАналитик) Экспорт

	Запрос=Новый Запрос;
	Запрос.Текст="ВЫБРАТЬ
	|	СоответствиеВнешнимИБ.Ссылка КАК НастройкаСоответствия,
	|	СоответствиеВнешнимИБ.ТаблицаАналитикиТек КАК ТаблицаАналитики,
	|	СоответствиеВнешнимИБ.ТаблицаАналитикиВИБ КАК ТаблицаАналитикиВИБ
	|ИЗ
	|	Справочник.СоответствиеВнешнимИБ КАК СоответствиеВнешнимИБ
	|ГДЕ
	|	СоответствиеВнешнимИБ.Владелец = &ТипБД
	|	И СоответствиеВнешнимИБ.ТаблицаАналитикиТек В(&МассивТаблицАналитик)";
	
	Запрос.УстановитьПараметр("ТипБД",ТипБД); 
	Запрос.УстановитьПараметр("МассивТаблицАналитик",МассивТаблицАналитик);
...
КонецФункции // ПолучитьНастройкиСоответствия()
...
Функция ПолучитьТаблицуПравилДляВыгрузки(ТабПравилаРабочие,ИспользуемаяИБ)
	...
	Для Каждого Строка ИЗ ТабПравилаРабочие Цикл
		...
		Если ЗначениеЗаполнено(Строка.НастройкаСоответствия) Тогда 
			...
			СтруктураСоответствие=КэшНастроекСоответствия.Получить(Строка.НастройкаСоответствия);
			
			Если СтруктураСоответствие=Неопределено Тогда
				
				СтруктураСоответствие=ПреобразоватьНастройкуСоответствияКПримитивным(Строка.НастройкаСоответствия,ИспользуемаяИБ);
				КэшНастроекСоответствия.Вставить(Строка.НастройкаСоответствия,СтруктураСоответствие);
				
			КонецЕсли;
			
			НоваяСтрока.НастройкаСоответствия=СтруктураСоответствие;
			НоваяСтрока.НастройкаСоответствияНаименование=СтруктураСоответствие.Наименование;
...
Функция ПреобразоватьНастройкуСоответствияКПримитивным(Знач НастройкаСоответствия,ИспользуемаяИБ,ПреобразоватьНастройкуРеквизит=Истина)
	...
	СтруктураНастройки.Вставить("ПроцедураПередЗаписьюВИБ",	Результат.ПроцедураПередЗаписьюВИБ);
	...
	Для Каждого Строка ИЗ ТекСоответствиеРеквизитов Цикл
		...
		Если ЗначениеЗаполнено(Строка.НастройкаСоответствия) И ПреобразоватьНастройкуРеквизит Тогда
			...
			НоваяСтрока.НастройкаСоответствия=ПреобразоватьПодчиненнуюНастройкуСоответствия(Строка.НастройкаСоответствия,ИспользуемаяИБ);
...
Функция ПреобразоватьПодчиненнуюНастройкуСоответствия(ПодчиненнаяНастройка,ИспользуемаяИБ)
	...
	Запрос.УстановитьПараметр("Ссылка",ПодчиненнаяНастройка);
	...
	Если Результат.СинхронизацияПоGUID Тогда
		...
		СтруктураНастройки.Вставить("СоответствиеОбъектовРоGUID",ПолучитьТаблицуСинхронизацииПоGUID(ПодчиненнаяНастройка,ИспользуемаяИБ)); 
...
КонецФункции // ПреобразоватьПодчиненнуюНастройкуСоответствия()
	...
	Для Каждого Строка ИЗ ТаблицаДанных Цикл
		...
		Для Каждого Колонка ИЗ ТаблицаДанных.Колонки Цикл
			...
			Иначе
				...
				ЗначениеАналитики=Строка[Колонка.Имя];
				
				Если ОбщегоНазначенияУХ.ПримитивныйТип(ТипЗнч(ЗначениеАналитики)) Тогда
...
Функция ПолучитьПараметрыОтбораПоНастройке(НастройкаОтбора,ДанныеНастройки,ЧислоПервых,ТабПравилаРабочие)
	
	СтрЗапрос=Новый Структура;
	
	ИсточникОбъект=Справочники.СохраненныеНастройкиОтборов.ПолучитьИсточникПоНастройке(НастройкаОтбора,ДанныеНастройки.Ссылка);
	ДанныеОтбора=УправлениеОтчетамиУХ.ПолучитьДанныеОперандовДляРасчета(Ложь,,,,ИсточникОбъект).мТабПоказателиЗапросы[0];
	
	ПсевдонимТаблицы=ДанныеНастройки.КодАналитики;
	
	СтрЗапрос.Вставить("ПравилаВычисленияПараметров",ДанныеОтбора.ПравилаВычисленияПараметров);
	
	ТекстЗапроса="ВЫБРАТЬ "+?(ЧислоПервых>0,"ПЕРВЫЕ "+Формат(ЧислоПервых,"ЧРГ=' '; ЧГ=0")+" " ,"")+"
	|ПРЕДСТАВЛЕНИЕ("+ПсевдонимТаблицы+".Ссылка) КАК "+ПсевдонимТаблицы+"vzvЭлементСсылкаПредставление";
	
	МассивСинонимов=Новый Массив;
	
	Для Каждого СтрПоле ИЗ ТабПравилаРабочие Цикл 
		
		Если МассивСинонимов.Найти(СтрПоле.Синоним)=Неопределено Тогда
			
			ТекстЗапроса=ТекстЗапроса+",
			|"+СтрПоле.Поле+" КАК "+СтрПоле.Синоним;
			
			МассивСинонимов.Добавить(СтрПоле.Синоним);
	...
	ТекстЗапроса=ТекстЗапроса+"
	|ИЗ "+ДанныеНастройки.ТаблицаАналитикиВИБ+" КАК "+ПсевдонимТаблицы+"
	|ГДЕ ИСТИНА";
			
	Если Не ПустаяСтрока(ИсточникОбъект.ТекстОтбора) Тогда
		
		ТекстЗапроса=ТекстЗапроса+" И "+ИсточникОбъект.ТекстОтбора;
		
	КонецЕсли;
			
	СтрЗапрос.Вставить("ТекстЗапроса",ТекстЗапроса);
	СтрЗапрос.Вставить("СпособПолучения",ДанныеОтбора.СпособПолучения);
	СтрЗапрос.Вставить("ПланСчетов");
	
	Возврат СтрЗапрос;
			 
КонецФункции // ПодготовитьПравилаВычисленияПараметров(НастройкаОтбора) 

Функция ПолучитьПараметрыОтбораПоУмолчанию(СписокОтбора,ДанныеНастройки,ЧислоПервых,ТабПравилаРабочие)
	
	СтрЗапрос=Новый Структура;
	ОтборПоСсылкам=Ложь;
	ПсевдонимТаблицы=ДанныеНастройки.КодАналитики;
	
	Если (НЕ СписокОтбора=Неопределено) И СписокОтбора.Количество()>0 Тогда
		
		Если ТипЗнч(СписокОтбора)=Тип("Массив") И ТипЗнч(СписокОтбора[0])=Тип("Строка") Тогда
			
			СтрЗапрос.Вставить("МассивGUID",СписокОтбора);
			СтрЗапрос.Вставить("ПравилаВычисленияПараметров",Новый ТаблицаЗначений);
			СтрЗапрос.Вставить("ТаблицаАналитикиВИБ",ДанныеНастройки.ТаблицаАналитикиВИБ);
		...
		Иначе	
			...
			НоваяСтрока.ИмяПараметра=ПсевдонимТаблицы;
			НоваяСтрока.ПолеБД		=ПсевдонимТаблицы;
			НоваяСтрока.ПредставлениеПараметра=ПсевдонимТаблицы;
			...
			НоваяСтрока.НастройкаСоответствия=ДанныеНастройки.Ссылка;
	...
	Если СтрЗапрос.Свойство("МассивGUID") Тогда
		
		ТекстЗапроса=ТекстЗапроса+"
		|ГДЕ Ссылка В(&МассивGUID)";
	...
	Иначе
		...
		Если СтрЗапрос.ПравилаВычисленияПараметров.Количество()>0 Тогда
			
			ТекстЗапроса=ТекстЗапроса+"
			|И Ссылка В(&"+ПсевдонимТаблицы+")";
...
КонецФункции // ПолучитьПараметрыОтбораПоУмолчанию() 

Функция ЗаполнитьНСИПоСоответствиям(ТаблицаДанных,ДанныеНастройки)
	...
	|	И СоответствиеОбъектовТекущейИВнешнихИБ.ИспользуемаяИБ = &ИспользуемаяИБ
	|;
	|ВЫБРАТЬ * Поместить ТаблицаДанных ИЗ &ТаблицаДанных КАК ТаблицаДанных
	|;
	|ВЫБРАТЬ ТаблицаДанных.*,
	|ЕСТЬNULL(ТекСоответствиеОбъектов.ОбъектТекущейИБ,Неопределено) КАК "+ДанныеНастройки.КодАналитики+",
	|ВЫБОР КОГДА ТекСоответствиеОбъектов.ОбъектТекущейИБ ЕСТЬ NULL Тогда ЛОЖЬ Иначе ИСТИНА Конец КАК ЕстьОбъектТекущейИБ
	|ИЗ ТаблицаДанных КАК ТаблицаДанных
	|ЛЕВОЕ СОЕДИНЕНИЕ ТекСоответствиеОбъектов КАК ТекСоответствиеОбъектов
	|ПО ТаблицаДанных."+ДанныеНастройки.КодАналитики+"vzvЭлементСсылка=ТекСоответствиеОбъектов.ОбъектВнешнейИБ";
...
КонецФункции // ЗаполнитьНСИПоСоответствиям()

Процедура ДобавитьСтрокуПометкаУдаления(ДанныеНастройки,ТабПравилаРабочие)
	
	НоваяСтрока=ТабПравилаРабочие.Добавить();
	НоваяСтрока.ИспользованиеВИБ=Перечисления.ИспользованиеРеквизита.ДляГруппыИЭлемента; 
	НоваяСтрока.КодАналитики=ДанныеНастройки.КодАналитики;
	НоваяСтрока.НастройкаСоответствия=ДанныеНастройки.Ссылка;
	НоваяСтрока.НеИспользоватьДляСинхронизации=Истина;
	НоваяСтрока.ОбновлятьРеквизитыПриИмпорте=Истина;
	НоваяСтрока.Поле="Ссылка.ПометкаУдаления";
	НоваяСтрока.Синоним=ДанныеНастройки.КодАналитики+"vzvПометкаУдаления";
	НоваяСтрока.РеквизитАналитики="ПометкаУдаления";
	НоваяСтрока.РеквизитАналитикиВИБ="ПометкаУдаления";
	НоваяСтрока.СоздаватьНовые=Истина; 
	НоваяСтрока.СпособЗаполнения=Перечисления.СпособыЗаполненияПолейИсточника.ПолеИсходнойТаблицы;;
    НоваяСтрока.ТаблицаАналитики=ДанныеНастройки.ТаблицаАналитикиТек;
	НоваяСтрока.ТаблицаАналитикиВИБ=ДанныеНастройки.ТаблицаАналитикиВИБ;
	НоваяСтрока.УровеньВложенности=2;
	
		
КонецПроцедуры // ДобавитьСтрокуПометкаУдаления() 
 
Функция ЗагрузитьОбъектыИзВИБ(ИспользуемаяИБ,
	 	НастройкаСоответствия,
		Организация=Неопределено,
		СписокОтбора=Неопределено,
		ВернутьСоответствиеСсылок=Ложь,
		ОбновлениеДочернихЭлементов=Истина,
		ЧислоПервых=0,НастройкаОтбора=Неопределено,СоздаватьПомеченныеНаУдаление=Ложь) Экспорт
	
	Использование77=(ИспользуемаяИБ.ТипБД.ВерсияПлатформы = Перечисления.ПлатформыВнешнихИнформационныхБаз.Предприятие77);
	
	ПравилаИспользованияПолей = ПолучитьОписаниеТаблицыПравилаИспользованияПолей();
	
	ДанныеНастройки=ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НастройкаСоответствия,"Ссылка,ТаблицаАналитикиВИБ,ТаблицаАналитикиТек,ТипОбъектаКонсолидации,ИмяОбъектаМетаданных");
	
	ТаблицаАналитикиВИБ=ДанныеНастройки.ТаблицаАналитикиВИБ;
	КодАналитики=СтрЗаменить(ТаблицаАналитикиВИБ,".","_");
	ДанныеНастройки.Вставить("КодАналитики",КодАналитики); 
	ДанныеНастройки.Вставить("ИспользуемаяИБ",ИспользуемаяИБ);
	
	НоваяСтрока=ПравилаИспользованияПолей.Добавить();
	НоваяСтрока.Поле=КодАналитики;
	НоваяСтрока.Синоним="Элемент";
	НоваяСтрока.СоздаватьНовые=Истина;
	НоваяСтрока.РазделятьПоОрганизациям=(ДанныеНастройки.ТипОбъектаКонсолидации="ВидСубконто" И ДанныеНастройки.ИмяОбъектаМетаданных.РазделятьПоОрганизациям);
    НоваяСтрока.ОбновлятьРеквизитыПриИмпорте=Истина;
	НоваяСтрока.КодАналитики=КодАналитики;
	...
	ТабПравилаРабочие=ПодготовитьТаблицуПолейДляЗапроса(ПравилаИспользованияПолей,Использование77,,ВернутьСоответствиеСсылок,НЕ ОбновлениеДочернихЭлементов,Истина);
	...
	Если ЗначениеЗаполнено(НастройкаОтбора) Тогда
		...
		СтрЗапрос=ПолучитьПараметрыОтбораПоНастройке(НастройкаОтбора,ДанныеНастройки,ЧислоПервых,ТабПравилаРабочие);
		
	Иначе
		
		СтрЗапрос=ПолучитьПараметрыОтбораПоУмолчанию(СписокОтбора,ДанныеНастройки,ЧислоПервых,ТабПравилаРабочие);
	...
	СтруктураЗапроса = ПодготовитьСтруктуруЗапроса(КонтекстОтчета,СтрЗапрос,Истина);
	
	Если ВернутьСоответствиеСсылок Тогда
	...
	СтруктураЗапроса.Вставить("ВозвращатьТаблицуЗначений",Истина);
	...
	РабочаяТаблицаДанных=ЗаполнитьНСИПоСоответствиям(ТаблицаДанных,ДанныеНастройки);
	
	ТаблицаСинхронизированных	= РабочаяТаблицаДанных.Скопировать(Новый Структура("ЕстьОбъектТекущейИБ",Истина));
	
	ТаблицаНЕСинхронизированных	= РабочаяТаблицаДанных.Скопировать(Новый Структура("ЕстьОбъектТекущейИБ",Ложь));
	ТаблицаНЕСинхронизированных.Колонки.Удалить(КодАналитики);
	...
	Если МассивСоставных.Количество()>0 Тогда
		...
		РезультатСинхронизации=СинхронизироватьПоСоставнымТипам(ТаблицаНЕСинхронизированных,ТабПравилаРабочие,СтруктураКлючевыхРеквизитов,СтруктураПодчиненныхРеквизитов,КонтекстОтчета,МассивСоставных);
		
	Иначе
		
		РезультатСинхронизации=ЗаполнитьНСИПоПравилам(ТаблицаНЕСинхронизированных,ТабПравилаРабочие,СтруктураКлючевыхРеквизитов,СтруктураПодчиненныхРеквизитов,КонтекстОтчета);	
		
	КонецЕсли;
	
	Если ТипЗнч(РезультатСинхронизации)=Тип("Строка") Тогда
		
		Сообщить(РезультатСинхронизации);
		Возврат Новый Соответствие;
		
	КонецЕсли;
	
	Если РезультатСинхронизации=Неопределено Тогда
		
		РезультатСинхронизации=ТаблицаСинхронизированных;
		
	Иначе
		
		ОбщегоНазначенияУХ.ЗагрузитьВТаблицуЗначений(ТаблицаСинхронизированных,РезультатСинхронизации);
	...
	ЗаполнитьНедостающиеАналитики(КонтекстОтчета,
										...
										СтруктураПодчиненныхРеквизитов,
										Ложь,
										ТабПравилаРабочие,СоздаватьПомеченныеНаУдаление);
										
	ЗаписатьСоответствияОбъектовПриЗагрузке(НастройкаСоответствия,РезультатСинхронизации,КодАналитики,ИспользуемаяИБ);									
	...
	Если ВернутьСоответствиеСсылок Тогда
		...
		Для Каждого Строка ИЗ РезультатСинхронизации Цикл
			
			ОбъектТекущейИБ=Строка[КодАналитики];
			
			Если ЗначениеЗаполнено(ОбъектТекущейИБ) Тогда
				
				СоответствиеСсылок.Вставить(Строка[ПолеСсылка]+"_tt_"+ТаблицаАналитикиВИБ,ОбъектТекущейИБ);
	...
	ЗагрузитьДанныеСвязанныхОбъектов(КонтекстОтчета.База,РезультатСинхронизации,ДанныеНастройки,ИспользуемаяИБ);
							
КонецФункции // ЗагрузитьЭлементыСправочникаВИБ()

Функция СинхронизироватьПоСоставнымТипам(ТаблицаНЕСинхронизированных,ТабПравилаРабочие,СтруктураКлючевыхРеквизитов,СтруктураПодчиненныхРеквизитов,КонтекстОтчета,МассивСоставных);
	
	СтрокаКолонки=СтрСоединить(МассивСоставных,",");
	ДанныеДляСинхронизации=ТаблицаНЕСинхронизированных.Скопировать(,СтрокаКолонки);
	ДанныеДляСинхронизации.Свернуть(СтрокаКолонки);
	
	Запрос=Новый Запрос;
	ТекстЗапроса="ВЫБРАТЬ * Поместить ДанныеДляСинхронизации
	|ИЗ &ДанныеДляСинхронизации КАК ДанныеДляСинхронизации
	|;
	|ВЫБРАТЬ
	|СоответствиеВнешнимИБ.ТаблицаАналитикиТек,
	|СоответствиеВнешнимИБ.ТаблицаАналитикиВИБ
	|Поместить НастройкиСоответствия
	|ИЗ Справочник.СоответствиеВнешнимИБ КАК СоответствиеВнешнимИБ 
	|ГДЕ СоответствиеВнешнимИБ.Владелец=&ТипБД
	...
	Для Каждого СтрПоле ИЗ МассивСоставных Цикл
		...
		ТекстПоля=ТекстПоля+",
		|ДанныеДляСинхронизации."+СтрПоле+" КАК "+СтрПоле+",
		|ЕстьNULL(НастройкиСоответствия_"+СтрПоле+".ТаблицаАналитикиТек,НЕОПРЕДЕЛЕНО) КАК "+СтрПоле+"_Тек";
		
		ТекстСоединения=ТекстСоединения+"
		|ЛЕВОЕ СОЕДИНЕНИЕ НастройкиСоответствия КАК НастройкиСоответствия_"+СтрПоле+"
		|ПО ДанныеДляСинхронизации."+СтрПоле+"=НастройкиСоответствия_"+СтрПоле+".ТаблицаАналитикиВИБ";
		
	КонецЦикла;
	
	ТекстЗапроса=ТекстЗапроса+"ВЫБРАТЬ "+Сред(ТекстПоля,2)+"
	|ИЗ ДанныеДляСинхронизации КАК ДанныеДляСинхронизации"+ТекстСоединения;
	
	Запрос.Текст=ТекстЗапроса;
	Запрос.УстановитьПараметр("ТипБД",КонтекстОтчета.ИспользуемаяИБ.ТипБД);
	Запрос.УстановитьПараметр("ДанныеДляСинхронизации",ДанныеДляСинхронизации);
	
	ДанныеДляСинхронизации=Запрос.Выполнить().Выгрузить(); 
	...
	Для Каждого Строка ИЗ ДанныеДляСинхронизации Цикл
		...
		Для Каждого СтрПоле ИЗ МассивСоставных Цикл
			...
			Если НЕ ЗначениеЗаполнено(Строка[СтрПоле+"_Тек"]) Тогда
				
				ОбщегоНазначенияУХ.СообщитьОбОшибке(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Для типа информационной базы %1 не найдена настройка соответствия для объекта %2'"),Запрос.Параметры.ТипБД,Строка[СтрПоле]),,,СтатусСообщения.Важное);
				НеНайденоСоответствие=Истина; 
				
			Иначе
				
				СтруктураПоиска.Вставить(СтрПоле,Строка[СтрПоле]);
				ДанныеТаблиц.Вставить(СтрЗаменить(СтрПоле,"vzvСсылка",""),Строка[СтрПоле+"_Тек"]); 
		...
		ТаблицаДляСинхронизации=ТаблицаНЕСинхронизированных.Скопировать(СтруктураПоиска); 
		
		КонтекстОтчета.Вставить("ДанныеТаблиц",ДанныеТаблиц);
		
		Результат=ЗаполнитьНСИПоПравилам(ТаблицаДляСинхронизации,ТабПравилаРабочие,СтруктураКлючевыхРеквизитов,СтруктураПодчиненныхРеквизитов,КонтекстОтчета);
		...
		Если ТабРезультат=Неопределено Тогда
			...
			ТабРезультат=Результат.СкопироватьКолонки();
			
		КонецЕсли;
		
		ОбщегоНазначенияУХ.ЗагрузитьВТаблицуЗначений(Результат,ТабРезультат);
...
КонецФункции // СинхронизироватьПоСоставнымТипам() 	

Функция ЗаполнитьНСИПоПравилам(ТаблицаДанных,ПравилаИспользованияПолей,СтруктураКлючевыхРеквизитов=Неопределено,СтруктураПодчиненныхРеквизитов=Неопределено,КонтекстОтчета=Неопределено)
	Перем Организация;
	Перем ПериодОтчета;
	Перем Сценарий; 
	...
	Для Каждого КодАналитики ИЗ СтруктураКлючевыхРеквизитов Цикл
		...
		СинхронизацияПоGUID=СтрПолеПравило.НастройкаСоответствия.СинхронизацияПоGUID;

		Если СтрНайти(ТекТаблицаАналитики,"Перечисление")>0 И (НЕ СинхронизацияПоGUID) Тогда
		...
		Для Каждого СтрРеквизит ИЗ СтруктураПодчиненныхРеквизитов Цикл
				...
				Для Каждого СтрПоле ИЗ МассивРеквизитов Цикл 
					...
					Если СтрПоле.УровеньВложенности=1 И СтрПоле.ТаблицаАналитики = ТекТаблицаАналитики Тогда 
						...
						Если СтрПоле.ТаблицаАналитики="Справочник.ПроизвольныйКлассификаторУХ" И ЗначениеЗаполнено(СтрПоле.АналитикаОперанда) Тогда
							...
							Если СтрПоле.РазделятьПоОрганизациям И ЗначениеЗаполнено(Организация) Тогда 
						...
						ИначеЕсли СтрПоле.ТаблицаАналитики="Справочник.ДоговорыКонтрагентов" И ЗначениеЗаполнено(Организация) Тогда
			...
			Иначе // Обрабатываем реквизиты составного типа
				...
				Для Каждого ТаблицаАналитики ИЗ СтрРеквизит.Значение Цикл
					...
					Если КонтекстОтчета.Свойство("ДанныеТаблиц") 
						И КонтекстОтчета.ДанныеТаблиц.Свойство(СтрРеквизит.Ключ) Тогда
						
						Если НЕ ТаблицаАналитики=КонтекстОтчета.ДанныеТаблиц[СтрРеквизит.Ключ] Тогда
...
КонецФункции // ЗаполнитьНСИПоПравилам()

Процедура ЗаписатьСоответствияОбъектовПриЗагрузке(НастройкаСоответствия,ТаблицаЗагрузки,КодАналитики,ИспользуемаяИБ)
	
	Запрос=Новый Запрос;
	ТекстЗапроса="ВЫБРАТЬ
	             |	&ПолеСсылкаТекущейИБ КАК ОбработанныйОбъектТекущейИБ,
	             |	&ПолеСсылкаВнешнейИБ КАК ИсходныйОбъектВнешнейИБ,
	             |	ВЫРАЗИТЬ(&ПолеПредставлениеСсылкиВнешнейИБ КАК СТРОКА(500)) КАК ПредставлениеОбъектаВнешнейИБ
	             |ПОМЕСТИТЬ ТабОбработанныхОбъектов
	             |ИЗ
	             |	&ТабОбработанныхОбъектов КАК ТабОбработанныхОбъектов
	             |;
	             ...
	             |////////////////////////////////////////////////////////////////////////////////
	             |ВЫБРАТЬ
	             |	ТабОбработанныхОбъектов.ОбработанныйОбъектТекущейИБ КАК ОбработанныйОбъектТекущейИБ,
	             |	ЕСТЬNULL(УстановленныеСоответствия.ОбъектВнешнейИБ, НЕОПРЕДЕЛЕНО) КАК ОбъектВнешнейИБ,
	             |	ТабОбработанныхОбъектов.ИсходныйОбъектВнешнейИБ КАК ИсходныйОбъектВнешнейИБ,
	             |	ТабОбработанныхОбъектов.ПредставлениеОбъектаВнешнейИБ КАК ПредставлениеОбъектаВнешнейИБ,
	             |	УстановленныеСоответствия.НаименованиеОбъектаВнешнейИБ КАК НаименованиеОбъектаВнешнейИБ
	             |ИЗ
	             |	ТабОбработанныхОбъектов КАК ТабОбработанныхОбъектов
	             |		ЛЕВОЕ СОЕДИНЕНИЕ УстановленныеСоответствия КАК УстановленныеСоответствия
	             |		ПО ТабОбработанныхОбъектов.ОбработанныйОбъектТекущейИБ = УстановленныеСоответствия.ОбъектТекущейИБ
	             |ГДЕ
	             |	(ЕСТЬNULL(УстановленныеСоответствия.ОбъектВнешнейИБ, НЕОПРЕДЕЛЕНО) = НЕОПРЕДЕЛЕНО
	             |			ИЛИ НЕ УстановленныеСоответствия.НаименованиеОбъектаВнешнейИБ = ТабОбработанныхОбъектов.ПредставлениеОбъектаВнешнейИБ)";

	ТекстЗапроса=СтрЗаменить(ТекстЗапроса,"&ПолеСсылкаТекущейИБ",КодАналитики);
	ТекстЗапроса=СтрЗаменить(ТекстЗапроса,"&ПолеСсылкаВнешнейИБ",КодАналитики+"vzvЭлементСсылка");
	ТекстЗапроса=СтрЗаменить(ТекстЗапроса,"&ПолеПредставлениеСсылкиВнешнейИБ",КодАналитики+"vzvЭлементСсылкаПредставление");
	
	Запрос.Текст=ТекстЗапроса;	
	Запрос.УстановитьПараметр("ТабОбработанныхОбъектов",ТаблицаЗагрузки);
	Запрос.УстановитьПараметр("НастройкаСоответствия",НастройкаСоответствия);
	Запрос.УстановитьПараметр("ИспользуемаяИБ",ИспользуемаяИБ);
	...
	Пока Результат.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Результат.ОбработанныйОбъектТекущейИБ) Тогда
		
		ЗаписатьСоответствиеОбъектовТекущейИВнешнейИБ(НастройкаСоответствия,
														ИспользуемаяИБ,
														Результат.ОбработанныйОбъектТекущейИБ,
														Результат.ИсходныйОбъектВнешнейИБ,
														Результат.ПредставлениеОбъектаВнешнейИБ); 
...
Функция ПолучитьСоответствиеСчетов(ИспользуемаяИБ,ТаблицаАналитикиВИБ,МассивСчетов) Экспорт
	...
	Если ТипЗнч(ТекСоединениеВИБ) = Тип("WSProxy") Тогда 
		...
		ТаблицаСчетов=ТекСоединениеВИБ.GetAccountCodes(Новый ХранилищеЗначения(СтруктураПараметров, Новый СжатиеДанных(9))).Получить();
		
	ИначеЕсли ТипЗнч(ТекСоединениеВИБ) = Тип("Строка") Тогда
		
		ТаблицаСчетов= РаботаСОбъектамиМетаданныхВнешнийУХ.ПолучитьОтвет1СШина(ТекСоединениеВИБ,"GetAccountCodes",СтруктураПараметров);
	...
	Иначе
		...
		ТаблицаСчетов=РаботаСОбъектамиМетаданныхВнешнийУХ.GetAccountCodes(ТекСоединениеВИБ,СтруктураПараметров);
	...
	|ВЫБРАТЬ
	|	ТаблицаСчетов.СчетКод КАК СчетКод,
	|	ЕСТЬNULL(РабочийПланСчетов.Ссылка, НЕОПРЕДЕЛЕНО) КАК Счет,
	|	ТаблицаСчетов.GUID КАК СчетvzvЭлементСсылка 	
	|ИЗ
	|	ТаблицаСчетов КАК ТаблицаСчетов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ &РабочийПланСчетов КАК РабочийПланСчетов
	|		ПО ТаблицаСчетов.СчетКод = РабочийПланСчетов.Код";
	...
	ТаблицаКЗаписи=РабочаяТаблица.СкопироватьКолонки();
	ТаблицаКЗаписи.Колонки.Добавить("СчетvzvЭлементСсылкаПредставление",РабочаяТаблица.Колонки.СчетКод.ТипЗначения);
	
	СоответствиеСсылок=Новый Соответствие;
	ИмяПланаСчетов=СтрЗаменить(ТаблицаАналитикиВИБ,"ПланСчетов.","");
	
	Для Каждого Строка ИЗ РабочаяТаблица Цикл
		
		Если ЗначениеЗаполнено(Строка.Счет) Тогда 
			
			СоответствиеСсылок.Вставить(Строка.СчетvzvЭлементСсылка+"_tt_"+ТаблицаАналитикиВИБ,Строка.Счет);
			НоваяСтрока=ТаблицаКЗаписи.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока,Строка);
			НоваяСтрока.СчетvzvЭлементСсылкаПредставление=Строка.СчетКод;
			
		Иначе
			
			ТекстСообщения=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Не удалось сопоставить счет с кодом %1 плана счетов %2 внешней ИБ %3 
			|счету аналогичного плана счетов текущей ИБ'"),Строка.Счет,ИмяПланаСчетов,ИспользуемаяИБ);
	...
	ЗаписатьСоответствияОбъектовПриЗагрузке(Справочники.СоответствиеВнешнимИБ.ПустаяСсылка(),ТаблицаКЗаписи,"Счет",ИспользуемаяИБ);
	
	Возврат СоответствиеСсылок;	
	
КонецФункции // ПолучитьСоответствиеСчетов() 

Процедура ЗагрузитьДанныеСвязанныхОбъектов(ТекСоединениеВИБ,Результат,ДанныеНастройки,ИспользуемаяИБ)
	...
	ЗаполнитьТЧОбъектов(ТаблицаСсылок,ТабСвязанныхОбъектов);
	ЗаполнитьСвязанныеРегистры(ТаблицаСсылок,ТабСвязанныхОбъектов);
	
	
КонецПроцедуры // ЗагрузитьДанныеСвязанныхОбъектов() 

Процедура ЗаполнитьТЧОбъектов(ТаблицаСсылок,ТабСвязанныхОбъектов)
	
	МассивТЧ=ТабСвязанныхОбъектов.НайтиСтроки(Новый Структура("ТипОбъектаТек","ТабличнаяЧасть"));
	...
	Для Каждого СтрокаСсылка ИЗ ТаблицаСсылок Цикл 
		...
		Для Каждого СтрокаТЧ ИЗ МассивТЧ Цикл 
			...
			МассивСтрок=СтрокаТЧ.РабочаяТаблица.НайтиСтроки(Новый Структура("_GUIDВнешнейИБ",СтрокаСсылка.GUIDВнешнейИБ));
		
			Если МассивСтрок.Количество()>0 Тогда
				
				СтруктураТЧ.Вставить(СтрокаТЧ.ИмяОбъектаТек,МассивСтрок);
		...
		Если СтруктураТЧ.Количество()>0 Тогда
			...
			ЗаписатьТЧОбъекта(СтрокаСсылка.СсылкаТекущейИБ,СтруктураТЧ);
...
Процедура ЗаписатьТЧОбъекта(Ссылка,СтруктураТЧ)
	...
	Исключение
		...
		ТекстСообщения=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Не удалось обновить данные табличных частей %1 объекта %2:
		|%3'"),Сред(ТекстТЧ,2),Ссылка,ТекстОшибки); 
...
Функция УбратьПолеСсылка(ПолеБД) Экспорт
	
	МассивПолей=СтрРазделить(ПолеБД,".");
	
	Если МассивПолей[МассивПолей.Количество()-1]="Ссылка" Тогда
		
		МассивПолей.Удалить(МассивПолей.Количество()-1);
		
	КонецЕсли;
	
	Возврат СтрСоединить(МассивПолей,".");
		
КонецФункции // УбратьПолеСсылка(ПолеБД) 
	...
	Пока Результат.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Результат.РеквизитОбъектаВнешнейИБ) Тогда
			...
			Если МассивСинонимов.Найти(Результат.РеквизитОбъектаКонсолидации)=Неопределено Тогда
				...
				НоваяСтрока=ТаблицаПолей.Добавить();
				НоваяСтрока.ПолеБД=УбратьПолеСсылка(Результат.РеквизитОбъектаВнешнейИБ);
...
Процедура ЗагрузитьОбъектыИзADO(ИспользуемаяИБ,НастройкаСоответствия,Организация=Неопределено,СписокОтбора=Неопределено,ОбновлениеДочернихЭлементов=Ложь) Экспорт
	...
	СтруктураЗапроса.Вставить("СтрокаСоединения",ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИспользуемаяИБ,"СтрокаПодключения"));
...
Процедура ДобавитьСтрокиПолейДляИмпорта(НаборЗаписей,НастройкаСоответствия,ТекИмяПоляВИБ,ТекИмяПоляКонсолидации,ТекПолеДопРеквизитов)
	...
	// Строка для поиска родителя при импорте данных
	Если ЗначениеЗаполнено(НастройкаСоответствия.ПолеРодительВИБ) 
		И ЗначениеЗаполнено(НастройкаСоответствия.ПолеРодительТек) Тогда
		
		НоваяСтрока=НаборЗаписей.Добавить();
		НоваяСтрока.РеквизитОбъектаВнешнейИБ=ТекИмяПоляВИБ+НастройкаСоответствия.ПолеРодительВИБ;
		...
		НоваяСтрока.РеквизитОбъектаКонсолидации=ТекИмяПоляКонсолидации+"ПоискРодителяvzp"+НастройкаСоответствия.ПолеРодительТек;
...
Процедура ADO_ДобавитьСтрокиПолейДляИмпорта(НаборЗаписей,НастройкаСоответствия,ТекИмяПоляВИБ,ТекИмяПоляКонсолидации,ТекПолеДопРеквизитов,ТекТаблицаАналитикиВИБ)
	...
	ТаблицаАналитикиВИБ=НастройкаСоответствия.ОписаниеОбъектаВИБ.Имя;
	...
	Если ЗначениеЗаполнено(НастройкаСоответствия.ПолеРодительВИБ) 
		...
		НоваяСтрока.РеквизитОбъектаВнешнейИБ="["+НастройкаСоответствия.ПолеРодительВИБ+"]";
				
		НоваяСтрока.НеИспользоватьДляСинхронизации=Истина;
		НоваяСтрока.ТипОбъектаКонсолидации=НастройкаСоответствия.ТипОбъектаКонсолидации;
		НоваяСтрока.ИмяОбъектаМетаданных=НастройкаСоответствия.ИмяОбъектаМетаданных;
		НоваяСтрока.ИспользованиеКонсолидация=Перечисления.ИспользованиеРеквизита.ДляГруппы;
		НоваяСтрока.ИспользованиеВИБ=Перечисления.ИспользованиеРеквизита.ДляЭлемента;
		НоваяСтрока.ИерархияКонсолидации=Перечисления.ВидИерархии.НетИерархии;
		НоваяСтрока.ИерархияВИБ=Перечисления.ВидИерархии.НетИерархии;
		НоваяСтрока.ТаблицаАналитики=ТекТаблицаАналитики;
...
Функция ПолучитьТаблицуДанныхПроизвольногоЗапросаВИБ(КонтекстОтчета,СтрЗапрос) Экспорт
	...
	ДанныеЗапроса= ПолучитьТаблицуДанныхПоЗапросу(СтруктураЗапроса);
	...
	Для Каждого СтрПоле Из ТекПравило Цикл
		...
		Если (НЕ (ДанныеЗапроса.ТаблицаПереноса.Колонки.Найти(СтрПоле.Поле)=Неопределено ИЛИ ПустаяСтрока(СтрПоле.КодАналитики)))
			И ДанныеЗапроса.ТаблицаПереноса.Колонки.Найти(СтрПоле.КодАналитики)=Неопределено Тогда 
			
			ДанныеЗапроса.ТаблицаПереноса.Колонки[СтрПоле.Поле].Имя=СтрПоле.КодАналитики;
			
		КонецЕсли;
		
	КонецЦикла;
	
	ТабОперандов=ТрансформироватьВнешниеДанные(КонтекстОтчета,ДанныеЗапроса,ТекПравило);
	...
	Иначе
		...
		Если НЕ ТолькоПроверка(КонтекстОтчета) Тогда 
			
			ПолучитьТаблицуОперандовВИБ(КонтекстОтчета,ТабОперандов,СтрЗапрос.ТаблицаОперандов,СтрЗапрос.ТабПравилаИспользованияПолей);
...
Функция ПолучитьТаблицуДанныхРегистраБухгалтерииВИБ(КонтекстОтчета,СтрЗапрос) Экспорт
	...
	СтруктураЗапроса = ПодготовитьСтруктуруЗапроса(КонтекстОтчета,СтрЗапрос,Истина);
	УстановитьПризнакПроверки(КонтекстОтчета,СтруктураЗапроса);	
		
	ДанныеВИБ= ПолучитьТаблицуДанныхПоЗапросу(СтруктураЗапроса);
	...
	Если СтруктураЗапроса.ТабСчетаОперандов.Количество()=1 Тогда // Отбор по единичному источнику
		...
		ТабОперандов=ТрансформироватьВнешниеДанные(КонтекстОтчета,ДанныеВИБ,ТекПравило);
		...
		Иначе
			...
			СтрокаКоррСчет=ТекПравило.Найти("КоррСчетОборот","Синоним");
			
			Если НЕ СтрокаКоррСчет=Неопределено Тогда
				
				Если ТипЗнч(ДанныеВИБ)=Тип("Структура") Тогда
					
					РабочаяТаблица=ПолучитьРабочуюТаблицуДляЗапроса(ТабОперандов,СтрокаКоррСчет.КодАналитики);
					ОбщегоНазначенияУХ.ЗагрузитьВТаблицуЗначений(ТабОперандов,РабочаяТаблица);
					
					ТабОперандов=УправлениеОтчетамиУХ.ПреобразоватьКолонкуКоррСчет(РабочаяТаблица,СтрЗапрос.ПланСчетов,СтрокаКоррСчет);
					
				Иначе
					
					ТабОперандов=УправлениеОтчетамиУХ.ПреобразоватьКолонкуКоррСчет(ТабОперандов,СтрЗапрос.ПланСчетов,СтрокаКоррСчет);
	...
	Иначе // Возвращаем структуру для дальнейшего разбора
		...
		ДанныеЗапроса.Вставить("ТаблицаДанных",ДанныеВИБ);
...
Процедура УстановитьПризнакПроверки(КонтекстОтчета,СтруктураЗапроса)
	 	
	Если ТолькоПроверка(КонтекстОтчета) Тогда
		
		СтруктураЗапроса.Вставить("ВозвращатьТаблицуЗначений",Истина);

	КонецЕсли;
	
КонецПроцедуры // УстановитьПризнакПроверки()

Функция ТолькоПроверка(КонтекстОтчета) Экспорт
	
	Перем ТолькоПроверка;
		
	КонтекстОтчета.Свойство("ТолькоПроверка",ТолькоПроверка);
	
	Возврат ТолькоПроверка=Истина; 	
	
КонецФункции // ТолькоПроверка()
...
Функция ПолучитьТаблицуДанныхРегистраНакопленияВИБ(КонтекстОтчета,СтрЗапрос) Экспорт
	...
	Если (НЕ СтруктураЗапроса.Свойство("ТабСчетаСписок")) ИЛИ СтруктураЗапроса.ТабСчетаСписок.Количество()=1 Тогда // Отбор по единичному источнику
		...
		Если (Не СтрЗначение=Неопределено) И ДанныеЗапроса.ТаблицаПереноса.Колонки.Найти("Значение")=Неопределено Тогда
			ДанныеЗапроса.ТаблицаПереноса.Колонки[СтрЗаменить(СтрЗначение.Поле,".","")].Имя="Значение";
		КонецЕсли;
	...
	Иначе // Возвращаем структуру для дальнейшего разбора в модуле настраиваемого отчета
		...
		ДанныеЗапроса=Новый Структура;
		ДанныеЗапроса.Вставить("ТаблицаДанных",ДанныеЗапроса);
...
Функция ПолучитьТаблицуДанныхADO(КонтекстОтчета,СтрЗапрос) Экспорт
	...
	СтруктураЗапроса.Вставить("СтрокаСоединения",ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КонтекстОтчета.ИспользуемаяИБ,"СтрокаПодключения"));
...
Функция ПодготовитьСтруктуруЗапроса(КонтекстОтчета,СтрЗапрос,ПроизвольныйЗапрос=Ложь) Экспорт
	...
	Если ТипЗнч(СтрЗапрос)=Тип("Структура") И СтрЗапрос.Свойство("МассивGUID") Тогда
		
		СтруктураЗапроса.Вставить("МассивGUID",СтрЗапрос.МассивGUID);
		СтруктураЗапроса.Вставить("ТаблицаАналитикиВИБ",СтрЗапрос.ТаблицаАналитикиВИБ);
...
Функция ПолучитьТаблицуДанныхПоЗапросу(СтруктураЗапроса) Экспорт
	...
	Если ТипЗнч(РезультатЗапроса)=Тип("Строка")
		ИЛИ РезультатЗапроса=Неопределено Тогда
...
Функция ПолучитьСтруктуруКлючевыхРеквизитов(ПравилаИспользованияПолей) 
	...
	Для Каждого Строка ИЗ ТабКолонкиТаблицы Цикл
		...
		Если (НЕ ЗначениеЗаполнено(Строка.АналитикаОперанда)) 
			ИЛИ ОбщегоНазначенияУХ.ПринудительноеЗаполнениеПоля(Строка) Тогда
			Продолжить;
		КонецЕсли;
		
		ТаблицаАналитики=Метаданные.НайтиПоТипу(Строка.АналитикаОперанда.ТипЗначения.Типы()[0]).ПолноеИмя();
		...
		Если СтрНайти(Строка.КодАналитики,"vzv")=0 Тогда
			...
			Если СтруктураКлючевыхРеквизитов[Строка.КодАналитики].Найти(ТаблицаАналитики)=Неопределено Тогда
				
				СтруктураКлючевыхРеквизитов[Строка.КодАналитики].Добавить(ТаблицаАналитики);
...
КонецФункции // ПолучитьСтруктуруКлючевыхРеквизитов() 

Функция ПодготовитьРабочуюТаблицуПоДаннымПереноса(КонтекстОтчета,ДанныеПереноса,ПравилаИспользованияПолей,Организация)
	
	УправлятьНСИ=КэшируемыеПроцедурыУХ.ИспользоватьУправлениеНСИ();
	
	ПроверятьСинхронизацию= УправлятьНСИ И КонтекстОтчета.Свойство("ИспользуемаяИБ") И КонтекстОтчета.ИспользуемаяИБ.НСИ_ТребуетСинхронизации;
	
	Если ПроверятьСинхронизацию Тогда
		
		РабочаяТаблица=ПолучитьРабочуюТаблицуДляЗапроса(ДанныеПереноса.ТаблицаПереноса);
	...
	ТаблицаДанных=ОбменДаннымиНСИУХ.ПолучитьРабочуюТаблицуПоДаннымПереноса(КонтекстОтчета.ИспользуемаяИБ,ДанныеПереноса,РабочаяТаблица,Организация);
	...
	Если ПроверятьСинхронизацию Тогда
		...
		СтруктураКлючевыхРеквизитов=ПолучитьСтруктуруКлючевыхРеквизитов(ПравилаИспользованияПолей);
		
		ПроверитьИСинхронизироватьТаблицуРезультатов(ТаблицаДанных,СтруктураКлючевыхРеквизитов,ПравилаИспользованияПолей,СтруктураОшибок);
	...
	Если КонтекстОтчета.Свойство("ДляАналитическихОтчетов") Тогда
		...
		ОпределитьКолонкуДанных(ТаблицаДанных,ПравилаИспользованияПолей);			
		ОчиститьСлужебныеДанные(ТаблицаДанных,ПравилаИспользованияПолей);
		
	КонецЕсли;
	
	Возврат ТаблицаДанных;
	
КонецФункции // ПодготовитьРабочуюТаблицуПоДаннымПереноса() 
...
Функция ТрансформироватьВнешниеДанные(ЗНАЧ КонтекстОтчета,ТаблицаДанных,ПравилаИспользованияПолей) Экспорт
	...
	КонтекстОтчета.Свойство("ТолькоПроверка",ТолькоПроверка);
	КонтекстОтчета.Свойство("Организация",Организация);
	...
	Если ТипЗнч(ТаблицаДанных)=Тип("Структура") Тогда
		...
		ТаблицаДанных.Вставить("ТолькоПроверка",ТолькоПроверка=Истина);
		
		Возврат ПодготовитьРабочуюТаблицуПоДаннымПереноса(КонтекстОтчета,ТаблицаДанных,ПравилаИспользованияПолей,Организация);
...
Функция ПолучитьТаблицуДанныхПоЗапросу77(СтруктураЗапроса,ТолькоОбъектИтоги=Ложь) Экспорт
	...
	Если НЕ ТаблицаДанных.Колонки.Найти("Счет")=Неопределено Тогда
		ТаблицаДанных.Колонки.Счет.Имя="Счет";
	КонецЕсли;
	
	Если НЕ ТаблицаДанных.Колонки.Найти("КоррСчет")=Неопределено Тогда
		ТаблицаДанных.Колонки.КоррСчет.Имя="КоррСчет";
	КонецЕсли;
...
Функция ADO_ПолучитьТаблицуДанныхПоЗапросу(СтруктураЗапроса, ДополнительныеСвойстваИмпорта = Неопределено) Экспорт
	...
	Если СтрНайти(Врег(СтруктураЗапроса.База.Provider),"ORA")>0
		ИЛИ СтрНайти(Врег(СтруктураЗапроса.СтрокаСоединения),"POSTGRESQL")>0
		ИЛИ СтрНайти(Врег(СтруктураЗапроса.СтрокаСоединения),"MSDASQL")>0 Тогда
			
		Возврат OPG_ПолучитьТаблицуДанныхПоЗапросу(СтруктураЗапроса, ДополнительныеСвойстваИмпорта); 
...
Функция OPG_ПолучитьТаблицуДанныхПоЗапросу(СтруктураЗапроса, ДополнительныеСвойстваИмпорта)
...
Функция ПодготовитьДанныеДляНастройкиЗапроса(СтруктураЗапроса) Экспорт
	...
	Для Каждого КлючИЗначение ИЗ СтруктураЗапроса.СтруктураПолей Цикл
		...
		Для Каждого Элемент Из КлючИЗначение.Значение Цикл
			...
			СтрокаСиноним=СтрЗаменить(СтрокаСиноним,"]","");
			СтрокаСиноним=СтрЗаменить(СтрокаСиноним,".","");
	...
	Для Каждого СтрокаВычисления Из СтруктураЗапроса.ТаблицаВычисленияПараметров Цикл
		...
		Если СтрокаВычисления.ПолеБД="КоррСчетОборот" Тогда
			...
			НоваяСтрока.ЗначениеПараметра   = СтрокаВычисления.ЗначениеИзКонтекста;
		
		ИначеЕсли СтрокаВычисления.ТипРасчета = "Простой"
		...
		ИначеЕсли ОтборПоГраницамПериода(СтрокаВычисления.ТипРасчета) Тогда
		...
		ИначеЕсли СтрокаВычисления.ТипРасчета = "ПоКоду" Тогда
			...
			Если СтруктураЗапроса.ПроизвольныйЗапрос Тогда
				...
				УточнениеНазначения=УправлениеОтчетамиУХ.ПолучитьЗначениеИзХранилища(СтрокаВычисления.УточнениеСпособаОпределения);
	...
	СтрДатаНач=ИспользуемыеПараметры.Найти("ДатаНач","ПолеБД"); 
	СтрДатаКон=ИспользуемыеПараметры.Найти("ДатаКон","ПолеБД");
	
	Если НЕ (СтрДатаНач=Неопределено ИЛИ СтрДатаКон=Неопределено) Тогда
		
		Если ТипЗнч(СтрДатаНач.ЗначениеПараметра)=Тип("Дата") 
			И ТипЗнч(СтрДатаКон.ЗначениеПараметра)=Тип("Дата")	
			И СтрДатаНач.ЗначениеПараметра>СтрДатаКон.ЗначениеПараметра Тогда
			
			СтрДатаНач.ЗначениеПараметра=СтрДатаКон.ЗначениеПараметра;
			
		КонецЕсли;
		
	КонецЕсли;
	
	Если СтруктураЗапроса.Свойство("МассивGUID") Тогда
	...
	Если СтруктураЗапроса.Свойство("ДлинаУточненияПериода") Тогда
		
		ДанныеЗапроса.Вставить("ДлинаУточненияПериода",	СтруктураЗапроса.ДлинаУточненияПериода);
		
	Иначе
		
		ДанныеЗапроса.Вставить("ДлинаУточненияПериода",	0);
		
	КонецЕсли;
	
	Если СтруктураЗапроса.Свойство("ВозвращатьТаблицуЗначений") Тогда
		
		ДанныеЗапроса.Вставить("ВозвращатьТаблицуЗначений",	Истина);
		
	КонецЕсли;
	
	Если СтруктураЗапроса.Свойство("ТаблицаНастроекСоответствия") Тогда
		
		 ДанныеЗапроса.Вставить("ТаблицаНастроекСоответствия",	СтруктураЗапроса.ТаблицаНастроекСоответствия);
...
Процедура ПроверитьЗаполнениеОбязательныхРеквизитов(ТаблицаДанных,ПравилаИспользованияПолей)
	...
	Для Каждого СтрПоле ИЗ ПравилаИспользованияПолей Цикл
		...
		Если (Не ЗначениеЗаполнено(СтрПоле.АналитикаОперанда))
			...
			ИЛИ ПустаяСтрока(СтрПоле.ТаблицаАналитики)
			ИЛИ СтрПоле.ТаблицаАналитики="Справочник.ПроизвольныйКлассификаторУХ" Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого ОбязательноеПоле ИЗ ТаблицаОбязательныхПолей Цикл
		...
		Запрос.Текст="ВЫБРАТЬ Количество(РабочаяТаблица." // @query-part
			+ОбязательноеПоле.ПолеОбъектаКонсолидации+") КАК ЧислоПолей ИЗ РабочаяТаблица ГДЕ РабочаяТаблица."+ОбязательноеПоле.ПолеОбъектаКонсолидации+"=&ПустоеЗначение";
...
Функция ПолучитьЭлементАналитики(СтрДанные,
	...
	СтрокаПоискРодителя=ДанныеРеквизитов.ПримитивныеРеквизиты.Найти(Истина,"ПоискРодителя");
	
	Если Не СтрокаПоискРодителя=Неопределено Тогда
		
		ПолучитьРодителяДляЗаписи(ДанныеРеквизитов,СтрокаПоискРодителя,СтрДанные,СтруктураЗаписи);
		СтруктураПоиска.Вставить("Родитель",СтруктураЗаписи.Родитель);
		СтруктураОбновление.Вставить("Родитель",СтруктураЗаписи.Родитель);
	...
	Попытка
		...
		Если ТипОбъектаМетаданных="Справочник" ИЛИ ТипОбъектаМетаданных="ПланВидовХарактеристик" 
			...
			Если ТипОбъектаМетаданных="Справочник" Тогда
				...
				Если ЭтоГруппа Тогда
					РабочийОбъект = Справочники[ИмяОбъектаМетаданных].СоздатьГруппу();
				Иначе
					РабочийОбъект = Справочники[ИмяОбъектаМетаданных].СоздатьЭлемент();
				КонецЕсли;
				
			Иначе
				
				Если ЭтоГруппа Тогда
					РабочийОбъект = ПланыВидовХарактеристик[ИмяОбъектаМетаданных].СоздатьГруппу();
				Иначе
					РабочийОбъект = ПланыВидовХарактеристик[ИмяОбъектаМетаданных].СоздатьЭлемент();
				КонецЕсли;
				
			КонецЕсли;
			
			ДлинаКода=РабочийОбъект.Метаданные().ДлинаКода;
			ЗаполнитьЗначенияСвойств(РабочийОбъект,СтруктураЗаписи);
			...
			Если ДлинаКода>0 Тогда
				...
				Если НЕ ЕстьСоответствиеПоКоду Тогда
					...
					СтруктураЗаписи.Вставить("Код",РабочийОбъект.Код);
			...
			Если ИмяОбъектаМетаданных="ПроизвольныйКлассификаторУХ" Тогда
				...
				РабочийОбъект.Владелец=ДанныеРеквизитов.ИмяОбъектаМетаДанных;
				
			ИначеЕсли ИмяОбъектаМетаданных="СчетаБД" Тогда
				
				РабочийОбъект.Владелец=УправлениеОтчетамиУХ.ПолучитьПланСчетовОрганизации(КонтекстОтчета.Организация,КонтекстОтчета.Сценарий,КонтекстОтчета.ПериодОтчета);
				
			КонецЕсли;
			
			Если ТипОбъектаМетаданных="ПланВидовХарактеристик" Тогда
				
				РабочийОбъект.ТипЗначения=ПолучитьОписаниеТипаЗначенияПВХ(КонтекстОтчета,ДанныеРеквизитов.ТаблицаАналитикиВИБ,СтрДанные[ДанныеРеквизитов.КодАналитики+"vzvЭлементСсылка"]);
			...
			Если ЗначениеЗаполнено(ДанныеДопЗаполнения.КорневойКаталог)
				И (НЕ ЗначениеЗаполнено(РабочийОбъект.Родитель)) Тогда
				
				РабочийОбъект.Родитель=ДанныеДопЗаполнения.КорневойКаталог;
				
			КонецЕсли; 
			
			Если ДанныеДопЗаполнения.ТребуетсяСогласование Тогда
				
				РабочийОбъект.НСИ_НеАктивный=Истина;
		...
		ИначеЕсли ТипОбъектаМетаданных="Документ" Тогда
			...
			РабочийОбъект=Документы[ИмяОбъектаМетаданных].СоздатьДокумент();	
			ЗаполнитьЗначенияСвойств(РабочийОбъект,СтруктураЗаписи);
		...
		Если ОбщегоНазначенияУХ.ЕстьРеквизитОбъекта("НСИ_ВИБ",РабочийОбъект) Тогда
			
			РабочийОбъект.НСИ_ВИБ=КонтекстОтчета.ИспользуемаяИБ;
			
			Если КонтекстОтчета.ИспользуемаяИБ.НСИ_ТребуетСинхронизации Тогда
				
				РабочийОбъект.НСИ_ТребуетСинхронизации=Истина;
				РабочийОбъект.НСИ_НеАктивный=Истина;
...
Процедура ЗаписатьСоответствиеОбъектовТекущейИВнешнейИБ(НастройкаСоответствия,
				ИспользуемаяИБ,
				ОбъектТекущейИБ,
				ОбъектВнешнейИБ,
				НаименованиеОбъектаВнешнейИБ="") Экспорт
	...
	НаборЗаписей.Отбор.ИспользуемаяИБ			.Установить(ИспользуемаяИБ);
	НаборЗаписей.Отбор.ОбъектТекущейИБ			.Установить(ОбъектТекущейИБ);
	НаборЗаписей.Отбор.ОбъектВнешнейИБ			.Установить(ОбъектВнешнейИБ);
	
	НоваяСтрока=НаборЗаписей.Добавить();
	
	НоваяСтрока.НастройкаСоответствия			= НастройкаСоответствия;
	НоваяСтрока.ОбъектТекущейИБ					= ОбъектТекущейИБ;
	НоваяСтрока.ИспользуемаяИБ					= ИспользуемаяИБ;
	НоваяСтрока.ОбъектВнешнейИБ					= ОбъектВнешнейИБ;
	НоваяСтрока.НаименованиеОбъектаВнешнейИБ	= НаименованиеОбъектаВнешнейИБ;
		
	НаборЗаписей.Записать(Истина);
		
КонецПроцедуры // ЗаписатьСоответствиеОбъектовТекущейИВнешнейИБ() 
...
Функция ПолучитьДанныеДопЗаполнения(НастройкаСоответствия)
	...
	Запрос.Текст= "ВЫБРАТЬ
	              ...
	              |	ВЫБОР
	              |		КОГДА УстановкаПараметровКонтроляНСИ.ТипКонтроляНСИ = ЗНАЧЕНИЕ(Перечисление.ТипыКонтроляНСИ.ЗаявкиНаИзменениеНСИ)
	              |			ТОГДА ИСТИНА
	              |		ИНАЧЕ ЛОЖЬ
	              |	КОНЕЦ КАК ТребуетсяСогласование
	              |ИЗ
	              |	Справочник.СоответствиеВнешнимИБ КАК СоответствиеВнешнимИБ
	              |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УстановкаПараметровКонтроляНСИ КАК УстановкаПараметровКонтроляНСИ
	              |		ПО СоответствиеВнешнимИБ.Ссылка = УстановкаПараметровКонтроляНСИ.НастройкаСоответствия
	              |ГДЕ
	              |	СоответствиеВнешнимИБ.Ссылка = &Ссылка";
	...
	Возврат Новый Структура("ПроцедураПередЗаписью,КорневойКаталог,ТребуетсяСогласование",
				Результат.ПроцедураПередЗаписью,
				Результат.КорневойКаталог,
				Результат.ТребуетсяСогласование);
...
Процедура ЗаполнитьНедостающиеАналитики(КонтекстОтчета,
										...
										СтруктураПодчиненныхРеквизитов,
										РежимРасшифровки,
										ПравилаИспользованияПолей,СоздаватьПомеченныеНаУдаление=Ложь) Экспорт
	...
	Если НЕ КонтекстОтчета.Свойство("Сценарий") Тогда
		КонтекстОтчета.Вставить("Сценарий",Справочники.Сценарии.ПустаяСсылка());
	КонецЕсли; 
	
	Если НЕ КонтекстОтчета.Свойство("ПериодОтчета") Тогда
		КонтекстОтчета.Вставить("ПериодОтчета",Справочники.Периоды.ПустаяСсылка());
	КонецЕсли;
	
	Если НЕ КонтекстОтчета.Свойство("Организация") Тогда
		КонтекстОтчета.Вставить("Организация",Справочники.Организации.ПустаяСсылка());
	КонецЕсли;
	...
	КэшНайденныхЗначений=Новый Соответствие;
	 ...
	 ТаблицаРеквизитов.Колонки.Добавить("ПоискРодителя",ОбщегоНазначенияУХ.ПолучитьОписаниеТиповБулево());
	  
	 Для Каждого КодАналитики ИЗ СтруктураКлючевыхРеквизитов Цикл
		 
		 // Добавим колонки для значений перечислений
		 
		 СтрПравило=ПравилаИспользованияПолей.Найти(КодАналитики.Ключ,"КодАналитики");
		 СинхронизацияПоGUID=СтрПравило.НастройкаСоответствия.СинхронизацияПоGUID;
		 		 
		 Если СтрНайти(КодАналитики.Значение[0],"Перечисление.")>0 И (НЕ СинхронизацияПоGUID) Тогда
		...
		КонецЕсли;
		 ...
		 Для Каждого СтрПоле ИЗ МассивРеквизитов Цикл
			 ...
			 Если НЕ ТаблицаИмпорта.Колонки.Найти(СтрПоле.Синоним)=Неопределено Тогда
				 ...
				 НоваяСтрока.ПоискРодителя					= СтрНайти(СтрПоле.РеквизитАналитики,"ПоискРодителяvzp")>0;
				 
				 ИмяРеквизитаОбъекта=?(НоваяСтрока.ПоискРодителя,"Родитель",СтрПоле.РеквизитАналитики);
				 
				 ДобавитьСтруктуруВКэш(КэшНайденныхЗначений,СтрПоле.ТаблицаАналитики,ИмяРеквизитаОбъекта,СтрПоле.РазделятьПоОрганизациям);
	 ...
	 Попытка
		 ...
		 Для Каждого СтрДанные Из ТаблицаИмпорта Цикл
			 ...
			 Для Каждого Аналитика ИЗ СтруктураКлючевыхРеквизитов Цикл
				 ...
				 ЕстьПометкаУдаления=НЕ ТаблицаИмпорта.Колонки.Найти(Аналитика.Ключ+"vzvПометкаУдаления")=Неопределено;
				 
				 Если ЕстьПометкаУдаления 
					 И СтрДанные[Аналитика.Ключ+"vzvПометкаУдаления"]
					 И (НЕ (ЗначениеЗаполнено(СтрДанные[Аналитика.Ключ]) ИЛИ СоздаватьПомеченныеНаУдаление)) Тогда
				...
				ИначеЕсли СтрНайти(ДанныеРеквизитов.ТаблицаАналитики,"ПланВидовХарактеристик.")>0 Тогда
				 ...
				 Если СтрНайти(ДанныеРеквизитов.ТаблицаАналитики,"Перечисление.")>0
					 И (НЕ ДанныеРеквизитов.НастройкаСоответствия.СинхронизацияПоGUID) Тогда
				 ...
				 Иначе 
					 ...
					 Если (КонтекстОтчета.Свойство("ОбновлениеДочернихЭлементов") ИЛИ ДанныеРеквизитов.ОбновлятьРеквизитыПриИмпорте) И ЗначениеЗаполнено(СтрДанные[Аналитика.Ключ]) И НЕ РежимРасшифровки Тогда
					 ...
					 Для Каждого СтрРеквизит ИЗ ДанныеРеквизитов.ФиксированныеРеквизиты Цикл
						 ...
						 Если НЕ РабочийОбъект[СтрРеквизит.ИмяРеквизита]=СтрРеквизит.ФиксированноеЗначение Тогда
					 ...
					 Если НЕ ФлагОшибкиИмпортаТек Тогда
						 ...
						 Для Каждого СтрПоля ИЗ ДанныеРеквизитов.ПримитивныеРеквизиты Цикл 
							 ...
							 Если СтрПоля.ИмяРеквизита="ЭтоГруппа" ИЛИ СтрПоля.ИмяРеквизита="Ref" Тогда
								 
								 Продолжить;
								 
							 ИначеЕсли НЕ ПроверитьВозможностьИспользованияРеквизита(СтрПоля.Использование,ЭтоГруппа) Тогда
								 
								 Продолжить;
								 
							 ИначеЕсли НЕ (ТипЗнч(СтрДанные[СтрПоля.ДанныеРеквизита])=Тип("Булево") ИЛИ ЗначениеЗаполнено(СтрДанные[СтрПоля.ДанныеРеквизита])) Тогда
								 
								 Продолжить;
								 
							 КонецЕсли;
						 
							 Если СтрПоля.ПоискРодителя Тогда
								 
								ПолучитьРодителяДляЗаписи(ДанныеРеквизитов,СтрПоля,СтрДанные,СтруктураЗаписи,РабочийОбъект.Ссылка,ЕстьИзменения);
										 
							 ИначеЕсли (НЕ СокрЛП(РабочийОбъект[СтрПоля.ИмяРеквизита])=СокрЛП(СтрДанные[СтрПоля.ДанныеРеквизита]))
						 ...
						 Если ЕстьИзменения Тогда 
							 ...
							 Исключение
								 ...
								 ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Наименование%", Строка(РабочийОбъект.Наименование));
								 ПротоколируемыеСобытияУХ.ДобавитьЗаписьОшибка("ОбщийМодуль.ИнтеграцияСВнешнимиСистемамиУХ.ЗаполнитьНедостающиеАналитики.1009", РабочийОбъект.МетаДанные(), , ТекстСообщения, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
								 
							 КонецПопытки;
							 
							 
							 РабочийОбъект = Неопределено;
				 ...
				 Если НЕ Результат.Пустой() Тогда
					 ...
					 Пока СтрНастройка.Следующий() Цикл
						 ...
						 Иначе
							 ...
							 ИнтеграцияСВнешнимиСистемамиУХ.ЗагрузитьОбъектыИзВИБ(КонтекстОтчета.ИспользуемаяИБ,СтрНастройка.Ссылка,Организация,
							 СписокЭлементов,,Истина,,,Истина);
...
Процедура ПолучитьРодителяДляЗаписи(ДанныеРеквизитов,СтрПоля,СтрДанные,СтруктураЗаписи,Ссылка=Неопределено,ЕстьИзменения=Ложь)
	
	ИмяРеквизитаДляПоиска=СтрЗаменить(СтрПоля.ИмяРеквизита,"ПоискРодителяvzp","");
	ЕстьГруппы=(ДанныеРеквизитов.ОбъектМетаДанных.ВидИерархии=Метаданные.СвойстваОбъектов.ВидИерархии.ИерархияГруппИЭлементов);
	
	// Получим родителя для записи элемента
	
	Запрос=Новый Запрос;
	ТекстЗапроса="ВЫБРАТЬ Ссылка
	|ИЗ &ТаблицаАналитики КАК ТаблицаАналитики
	|Где ТаблицаАналитики.Наименование=&РеквизитДляПоиска
	|И ИСТИНА";
	
	ТекстЗапроса=СтрЗаменить(ТекстЗапроса,"&ТаблицаАналитики",ДанныеРеквизитов.ТаблицаАналитики);
	ТекстЗапроса=СтрЗаменить(ТекстЗапроса,"Наименование",ИмяРеквизитаДляПоиска);
	
	Если ЕстьГруппы Тогда
		
		ТекстЗапроса=СтрЗаменить(ТекстЗапроса,"ИСТИНА","ТаблицаАналитики.ЭтоГруппа");
		
	КонецЕсли;
	
	Запрос.Текст=ТекстЗапроса;
		
	Запрос.УстановитьПараметр("РеквизитДляПоиска",СтрДанные[СтрПоля.ДанныеРеквизита]); 
	
	Результат=Запрос.Выполнить().Выбрать();
	
	Если Результат.Следующий() Тогда
		
		РодительДляЗаписи=Результат.Ссылка;
		
	Иначе
		
		РодительДляЗаписи=КэшируемыеПроцедурыУХ.ПолучитьПустуюСсылкуПоТаблицеАналитики(ДанныеРеквизитов.ТаблицаАналитики);
		
	КонецЕсли;
	
	СтруктураЗаписи.Вставить("Родитель",РодительДляЗаписи);
	
	// Проверим необходимость изменения родителя у существующего элемента
	
	Если ЗначениеЗаполнено(Ссылка) Тогда
		
		ТекстЗапроса="ВЫБРАТЬ ТаблицаАналитики.Родитель КАК Родитель
		|ИЗ &ТаблицаАналитики КАК ТаблицаАналитики
		...
		Если НЕ Результат.Родитель=РодительДляЗаписи Тогда
...
КонецПроцедуры // ПолучитьРодителяДляЗаписи() 
	...
	ТаблицаРеквизитов=Новый ТаблицаЗначений;
	 ...
	 Для Каждого СтрПоле ИЗ МассивРеквизитов Цикл
		 ...
		 Если НЕ КолонкиДанных.Найти(СтрПоле.Синоним)=Неопределено Тогда
			 ...
			 НоваяСтрока.НеИспользоватьДляСинхронизации	= СтрПоле.НеИспользоватьДляСинхронизации;
			 НоваяСтрока.Использование					= СтрПоле.ИспользованиеКонсолидация;
			 НоваяСтрока.ПоискРодителя					= СтрНайти(НоваяСтрока.ИмяРеквизита,"ПоискРодителяvzp")>0;
			 ...
			 ИмяРеквизитаОбъекта=?(НоваяСтрока.ПоискРодителя,"Родитель",МассивПуть[МассивПуть.Количество()-1]);
...
Процедура ПоместитьЗначенияВТаблицуОперандовВИБ(ОбъектРасчета,ТабОперандов,ТабСчетаОперандов,СоответствиеРесурсов,МассивРесурсов,ЕстьРазворотПоАналитикам,СписокКодовОперандов=Неопределено)
	...
	Для Каждого Колонка ИЗ ТабОперандов.Колонки Цикл
		Если СтрНайти(Колонка.Имя,"Аналитика")>0 Тогда
			...
			ЕстьРазворотПоАналитикам=Истина;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СтрАналитика ИЗ ТекТаблицаАналитик Цикл
		...
		НоваяСтрока.ИтогПоПоказателю = НЕ ЕстьРазворотПоАналитикам И НЕ РасширенныйАлгоритмРасчетаПоказателей;
		
		Если ЕстьРазворотПоАналитикам И НЕ РасширенныйАлгоритмРасчетаПоказателей Тогда
...
Функция ПолучитьРабочуюТаблицуДляЗапроса(ИсходнаяТаблица,КолонкиСтрока="") Экспорт
	
	РабочаяТаблица=Новый ТаблицаЗначений;
	
	Для Каждого Колонка ИЗ ИсходнаяТаблица.Колонки Цикл
		
		Если СтрНайти(Колонка.Имя,"Счет")>0 Тогда
			
			РабочаяТаблица.Колонки.Добавить(Колонка.Имя,ОбщегоНазначенияУХ.ПолучитьОписаниеТиповСтроки(50));
			
		ИначеЕсли СтрНайти(КолонкиСтрока,Колонка.Имя)>0 Тогда
			
			РабочаяТаблица.Колонки.Добавить(Колонка.Имя,ОбщегоНазначенияУХ.ПолучитьОписаниеТиповСтроки(150));
			
		ИначеЕсли СтрНайти(Колонка.Имя,"Субконто")>0 ИЛИ СтрНайти(Колонка.Имя,"Аналитика")>0 Тогда
				
			РабочаяТаблица.Колонки.Добавить(Колонка.Имя,Метаданные.ПланыВидовХарактеристик.ВидыСубконтоКорпоративные.Тип);
			
		ИначеЕсли Колонка.Имя="Содержание" Тогда
			
			РабочаяТаблица.Колонки.Добавить(Колонка.Имя,ОбщегоНазначенияУХ.ПолучитьОписаниеТиповСтроки(150));
			
		Иначе
			
			РабочаяТаблица.Колонки.Добавить(Колонка.Имя,ОбщегоНазначенияУХ.ПолучитьОписаниеТиповЧисла(18,5));
...
КонецФункции // ПолучитьРабочуюТаблицуДляРегистраБухгалтерии() 

Процедура РазобратьСводныеДанныеРегистраБухгалтерииВИБ(ОбъектРасчета,СтруктураДанных,ТаблицаОперандов,ТабСчетаОперандов,СоответствиеРесурсов,ТабПравилаИспользованияПолей);
	
	Если СтруктураДанных.ТаблицаПереноса.Количество()=0 Тогда
		Возврат;
	КонецЕсли;
	...
	ТаблицаДанных=ОбменДаннымиНСИУХ.ПолучитьРабочуюТаблицуПоДаннымПереноса(ОбъектРасчета.ИспользуемаяИБ,
						СтруктураДанных,
						ПолучитьРабочуюТаблицуДляЗапроса(СтруктураДанных.ТаблицаПереноса),ОбъектРасчета.Организация);
						
	Если ТабПравилаИспользованияПолей.Количество()=1 Тогда
		
		УправлениеОтчетамиУХ.ЗаполнитьАналитикиПринудительно(ТаблицаДанных,ТабПравилаИспользованияПолей[0].ПравилаИспользованияПолей,ОбъектРасчета);					
	...
	Иначе
		...
		Для Каждого СтрПравило ИЗ ТабПравилаИспользованияПолей Цикл
			...
			Для Каждого Колонка ИЗ ТаблицаДанных.Колонки Цикл
				...
				Если МассивРесурсов.Найти(ИмяПоля)=Неопределено
					И МассивПолей.Найти(ИмяПоля)=Неопределено Тогда
											
						ТекстПоля=ТекстПоля+",
						|ТаблицаДанных."+ИмяПоля+" КАК "+ИмяПоля;
			...
			УправлениеОтчетамиУХ.ЗаполнитьАналитикиПринудительно(ТекТаблицаДанных,ПравилаИспользованияПолей,ОбъектРасчета);					
...
Функция ПолучитьТаблицуДанныхДляРегистра77(СтруктураЗапроса, ПроверочнаяТаблица = Неопределено,ТолькоРегистр=Ложь) Экспорт
	...
	Исключение
		ВызватьИсключение(НСтр("ru = 'В 1С Предприятии 7.7 не установлена обработка ""GetTotals""'"));
...
Функция ADO_ПолучитьКонструкциюОтбора(Знач ПолеБД,СтрокаПараметр) Экспорт
	...
	Если ТипЗнч(СтрокаПараметр.УточнениеСпособаОпределения)=Тип("СписокЗначений")
		...
		Если СтрокаПараметр.УточнениеСпособаОпределения.Количество()>0 Тогда
			...
			Если ТипЗнч(СтрокаПараметр.УточнениеСпособаОпределения)=Тип("СписокЗначений") Тогда
				...
				Если ТипЗнч(СтрокаПараметр.УточнениеСпособаОпределения[0].Значение)=Тип("Строка") Тогда
					
					ПолеБД=ПолеБД+"))";
					
				КонецЕсли;
				
			ИначеЕсли ТипЗнч(СтрокаПараметр.УточнениеСпособаОпределения[0])=Тип("Строка") Тогда
	...
	Иначе
		...
		ИначеЕсли Прав(СтрокаПараметр.ИмяПараметра,2)="_1" Тогда
			
			Возврат ПолеБД+">=&&";
			
		ИначеЕсли Прав(СтрокаПараметр.ИмяПараметра,2)="_2" Тогда
			
			Возврат ПолеБД+"<=&&";
 
		Иначе
			
			Если ТипЗнч(СтрокаПараметр.УточнениеСпособаОпределения)=Тип("Строка") Тогда
...
Функция ADO_ПолучитьДанныеИзЗапроса(ТекстЗапроса, СтруктураЗапроса, ТипыКолонокРезультата, ДополнительныеСвойстваИмпорта = Неопределено)
	...
	Попытка	
		...
		Для Каждого Поле ИЗ мНаборЗаписей.Fields Цикл
			...
			Если ТипКолонки.Типы()[0]=Тип("Число") И СтруктураЗапроса.СтруктураРесурсов.Свойство(Поле.Name) Тогда
...
Функция ПолучитьТекстОтбораПоСписку(ИмяТаблицы,МассивСоответствий,СтруктураЗапроса)
	...
	Для Каждого Соответствие ИЗ МассивСоответствий Цикл
		...
		Для Каждого КлючИЗначение ИЗ Соответствие Цикл
			...
			ТекстОтборСтрока=ТекстОтборСтрока+" AND ("+?(СтрНайти(КлючИЗначение.Ключ,"))")>0,"LTRIM(RTRIM(","")+"["+ИмяТаблицы+"]."+СтрЗаменить(КлючИЗначение.Ключ,"&&",ПолучитьТекстПараметра(КлючИЗначение.Значение,СтруктураЗапроса))+")";
...
Функция ПолучитьТекстПараметра(ЗначениеПараметра,СтруктураЗапроса)
	...
	ИначеЕсли ТипЗнч(ЗначениеПараметра)=Тип("Дата") Тогда
		...
		ИначеЕсли СтрНайти(Врег(СтруктураЗапроса.База.Properties("DBMS Name").Value),"ORA")>0 Тогда
			
			Возврат "TO_DATE('"+Формат(ЗначениеПараметра,"ДФ=""dd.MM.yyyy HH:mm:ss""")+"', 'DD.MM.YYYY HH24:MI:SS')";
...
Процедура ПолучитьИдентификаторыВнешнейИБПоТекущему(ИспользуемаяИБ,ОбъектТекущейИБ,МассивОтбора)
	...
	УзелИнформационнойБазы=ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИспользуемаяИБ,"УзелИнформационнойБазы");
	...
	Если ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
		...
		|Объединить
		|ВЫБРАТЬ
		|	ПубличныеИдентификаторыСинхронизируемыхОбъектов.Идентификатор КАК ОбъектВнешнейИБ
		|ИЗ
		|	РегистрСведений.ПубличныеИдентификаторыСинхронизируемыхОбъектов КАК ПубличныеИдентификаторыСинхронизируемыхОбъектов
		|ГДЕ
		|	ПубличныеИдентификаторыСинхронизируемыхОбъектов.Ссылка = &ОбъектТекущейИБ
...
Функция ПолучитьОтборПоДаннымКонсолидации(ВИБ,База, ПостроительЗапроса, ИмяПараметра, ЗначениеКонсолидации, Отбор)
	...
	Для Каждого ТекЗначениеКонсолидации ИЗ МассивЗначений Цикл
		...
		| ГДЕ " + Сред(ГдеЗапрос, (СтрДлина("И")+2));
...
Функция ОтборПоГраницамПериода(ТипРасчета) Экспорт
	
	Возврат ТипРасчета="ПериодыНачало"
		ИЛИ ТипРасчета="ПериодыНачалоСдвиг"
		ИЛИ ТипРасчета="ПериодыКонец"
		ИЛИ ТипРасчета="ПериодыКонецСдвиг"
		ИЛИ ТипРасчета="ПериодыИнтервал"
		ИЛИ ТипРасчета="ГодНачало"
		ИЛИ ТипРасчета="ГодНачалоСдвиг"
		ИЛИ ТипРасчета="ГодКонец"
		ИЛИ ТипРасчета="ГодКонецСдвиг";
			
КонецФункции // ОтборПоГраницамПериода() 
...
Функция ПолучитьРабочуюТаблицуВычисленияПараметров(ТаблицаВычисленияПараметров) Экспорт
	...
	Для Каждого СтрПараметр ИЗ ТаблицаВычисленияПараметров Цикл
		...
		Если ОтборПоГраницамПериода(СтрПараметр.ТипРасчета)
...
Функция НеГруппироватьПриОбмене(СтрРеквизит,ВыгрузкаДанных)
	...
	Если СтрРеквизит.СинхронизацияПоGUID=Истина Тогда
	...
	Если ВыгрузкаДанных Тогда
		...
		Если СтрРеквизит.НеИспользоватьДляСинхронизации 
			И (НЕ СтрРеквизит.РеквизитВИБ=Неопределено)
			И СтрНайти(СтрРеквизит.РеквизитВИБ,"Владелец")=0 Тогда
	...
	Иначе 
		...
		Если СтрРеквизит.НеИспользоватьДляСинхронизации 
			И (НЕ СтрРеквизит.РеквизитКонсолидации=Неопределено)
			И СтрНайти(СтрРеквизит.РеквизитКонсолидации,"Владелец")=0 Тогда
...
Функция ПодготовитьТаблицуПолейДляЗапроса(ПравилаИспользованияПолей,ДанныеСчетов77=Ложь,ВыгрузкаДанных=Ложь,ПолучатьПредставление=Ложь,ПервичноеЗаполнение=Истина,ОбменНСИ=Ложь) Экспорт
	...
	Пока Результат.Следующий() Цикл
		...
		Если ВыгрузкаДанных И СтрНайти(Результат.РеквизитКонсолидации,"ПоискРодителяvzp")>0 Тогда
		...
		ИначеЕсли СтрНайти(Результат.РеквизитКонсолидации,"ПоискРодителяvzp")>0 Тогда
			
			СтрокаТаблицы.Поле    = Результат.Поле+?(СтрДлина(РабочийРеквизитВыгрузки)>0,"."+ РабочийРеквизитВыгрузки,""); 
			СтрокаТаблицы.Синоним = Результат.КодАналитики+"vzv"+СтрЗаменить(Результат.РеквизитКонсолидации,".","vzv");			
		...
		МассивПутей=ОбщегоНазначенияУХ.РазложитьСтрокуВМассивПодстрок(СтрокаТаблицы.КодАналитики,"vzv");
...
Функция ЗаполнитьАналитикуПоПравилам(ТаблицаДанных,ПравилаИспользованияПолей,СтруктураКлючевыхРеквизитов=Неопределено,СтруктураПодчиненныхРеквизитов=Неопределено,КонтекстОтчета=Неопределено) Экспорт
	...
	Если ПравилаИспользованияПолей.Количество()=0
		ИЛИ ТаблицаДанных.Колонки.Количество()=0
		ИЛИ (ПравилаИспользованияПолей.Количество()=1 И ПустаяСтрока(ПравилаИспользованияПолей[0].КодАналитики)) Тогда
