...
Процедура ОбработатьСтрокуТЧ(Объект, 
	...
	КонецЕсли;
	Если СтруктураДействий.Свойство("РассчитатьНДССтроки") Тогда
...
Процедура ПересчитатьИтоговыеПоказателиКроссТаблицы(
	...
	Для Каждого СтрокаКроссТаблицы Из КроссТаблица Цикл
		ПриИзмененииКоличестваСуммыСтроки(
			...
			ПериодыЗакупок, 
			СтрокаКроссТаблицы);
			Если ТипЗнч(СтрокаКроссТаблицы.Номенклатура) <>	Тип("СправочникСсылка.ТоварныеКатегории") 
				И ТипЗнч(СтрокаКроссТаблицы.Номенклатура) <> Тип("СправочникСсылка.КатегорииЗакупок")	
				И СтрокаКроссТаблицы.Сумма<>0 Тогда
			...
			ИначеЕсли ТипЗнч(СтрокаКроссТаблицы.Номенклатура) <> Тип("СправочникСсылка.ТоварныеКатегории")
				И ТипЗнч(СтрокаКроссТаблицы.Номенклатура) <> Тип("СправочникСсылка.КатегорииЗакупок")
				И СтрокаКроссТаблицы.Сумма=0 Тогда
			Иначе
...
Процедура ОбработатьИзменениеСуммТККроссТаблицы(ЦенаВключаетНДС,
											   ПериодыЗакупок,
											   ТекущаяСтрока) Экспорт
	Если ТекущаяСтрока <> Неопределено Тогда
		МассивСумм = МассивСумм(
			ПериодыЗакупок,
			ТекущаяСтрока);       
	   	РаспределениеСумм = ЦентрализованныеЗакупкиВызовСервераУХ.РаспределитьСуммуПропорциональноКоэффициентам(ТекущаяСтрока.Сумма,МассивСумм);
		НазначитьНовыеСуммы(ПериодыЗакупок,ТекущаяСтрока,РаспределениеСумм);
		ПриИзмененииКоличестваСуммыСтроки(
			Истина, 
			ПериодыЗакупок, 
			ТекущаяСтрока,
...
Функция МассивСумм(ПериодыЗакупок, СтрокаТаблицы) Экспорт
	Массив = Новый Массив;
	Для каждого СтрокаПериода Из ПериодыЗакупок Цикл
		Если НЕ СтрокаПериода.Активная Тогда
			Продолжить;
		КонецЕсли;    
		Массив.Добавить(СтрокаТаблицы["Сумма_" + СтрокаПериода.ИмяКолонки]);
	КонецЦикла;  
	Возврат Массив;
КонецФункции
...
Процедура ПриИзмененииКоличестваСуммыСтроки(ЦенаВключаетНДС, 
	...
	РассчитатьИтогКоличестваСуммыСтроки(
		ПериодыЗакупок, 
		СтрокаТоваров);
	РассчитатьНДССтроки(Истина, СтрокаТоваров);
...
Процедура НазначитьНовыеСуммы(ПериодыЗакупок, СтрокаТаблицы, РаспределениеСумм) 
	ЭлементМассива = 0;
	Для каждого СтрокаПериода Из ПериодыЗакупок Цикл
		Если НЕ СтрокаПериода.Активная Тогда
			Продолжить;
		КонецЕсли;
		СтрокаТаблицы["Сумма_" + СтрокаПериода.ИмяКолонки] = РаспределениеСумм[ЭлементМассива]; 
		...
		Если НЕ ЦенаВключаетНДС Тогда
			СтрокаТаблицы["Сумма_" + УИДПериода] =
				...
				+ УчетНДСКлиентСервер.РассчитатьСуммуНДС(
					СуммаБазовая, 
					Ложь,	
					?(ЗначениеЗаполнено(СтрокаТаблицы.СтавкаНДС), ОбщегоНазначенияУХ.ПолучитьЗначениеРеквизита(ВстраиваниеУХВызовСервера.ПолучитьСтавкуНДС(
						СтрокаТаблицы.СтавкаНДС), "Ставка"),0));
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура РасчитатьСуммыПоНДССтроки(СтрокаТаблицы, СуммаВключаетНДС, ЕстьСуммаСНДС) Экспорт
	РассчитатьНДССтроки(СуммаВключаетНДС, СтрокаТаблицы);
...
Процедура РассчитатьНДССтроки(ЦенаВключаетНДС, СтрокаТаблицы) Экспорт
	...
	Если ТипЗнч(СтавкаНДС) = Тип("Число") Тогда
		СтрокаТаблицы.СуммаНДС =
			...
			СтрокаТаблицы.Сумма, 
			ЦенаВключаетНДС,	
			ВстраиваниеУХВызовСервера.ПолучитьСтавкуНДС(СтавкаНДС));
	ИначеЕсли ТипЗнч(СтавкаНДС) = Тип("СправочникСсылка.СтавкиНДС") Тогда
		ЗначениеСтавки = ОбщегоНазначенияУХ.ПолучитьЗначениеРеквизита(СтавкаНДС, "Ставка");
		...
		СтрокаТаблицы.СуммаНДС =
			...
			ЗначениеСтавки);
	Иначе
			ТекстСообщения = НСтр("ru = 'Неизвестный вариант ставки НДС: %СтавкаНДС%. Расчет НДС отменен'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СтавкаНДС%", Строка(СтавкаНДС));
...
Процедура СообщитьОшибкиПользователю(РезультатРаботы,
	...
	Если РезультатРаботы.Ошибки = Неопределено
			ИЛИ РезультатРаботы.Ошибки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура СообщитьПользователюОРезультатахРаботыФункции(
												...
												ЗаголовокОповещенияОбОшибке="") Экспорт
	СообщитьОшибкиПользователю(РезультатРаботы, ЗаголовокОповещенияОбОшибке);
...
Функция ИменаКлючевыхПолейПредложенияПоставщика() Экспорт
	...
	Возврат "Номенклатура,Характеристика,МестоПоставки,Назначение,ЦенаПервоначальная,Коэффициент,Цена,Проект,СтавкаНДС";
КонецФункции
...
Функция ИменаКлючевыхПолейВерсииСоглашенияКоммерческийДоговор() Экспорт
	Возврат "Лот,Приоритет,Номенклатура,СтавкаНДС,Характеристика,Организация,МестоПоставки,Проект,ДоговорСПокупателем,Менеджер,Цена,СтатьяБюджета,Аналитика1,Аналитика2,Аналитика3,Аналитика4,Аналитика5,Аналитика6";
КонецФункции
...
Функция ЭтоСпособВЭлектроннойФорме(СпособВыбораВход) Экспорт
	...
	СпособВыбора = ОбщегоНазначенияУХ.ЗначениеРеквизитаОбъекта(СпособВыбораВход,"СпособыВыбораПоставщика");
...
//Процедура ОбработкаЗаполненияПоступлениеТоваровУслугУХОбработкаЗаполнения(Источник, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
//	ОткрытиеФормыПоступлениеТоваровУслуг(Источник);
//КонецПроцедуры


//Процедура ОткрытиеФормыПоступлениеТоваровУслуг(Источник) Экспорт
//		ОткрытьФорму("Документ.ПоступлениеТоваровУслуг.Форма.ФормаВыбораСклада");
//КонецПроцедуры
