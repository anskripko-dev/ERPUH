			...
			Иначе
				...
				+ ?(ВидПериода = ВидыПериода.ОтчетныйИМежотчетныйПериод,
					НСтр("ru = ', включая межотчетный период';
						|en = 'including a mid-term accounting period'"), "");
			КонецЕсли;
		Иначе
			ТекстПериод = "";
		КонецЕсли;
...
// Изменяем видимость и доступность флагов показателей на закладке "Показатели" у настроек стандартного отчета.
//
// Параметры:
...
Процедура УправлениеВидимостьюВидПериода(Элемент, Отчет) Экспорт
	...
	Если Отчет.ВидПериода = ВидыПериода.МежотчетныйПериод Тогда
		Элемент.Подсказка = НСтр("ru = 'Вид периода: межотчетный период';
								|en = 'Period kind: mid-term accounting period'");
	...
	ИначеЕсли Отчет.ВидПериода = ВидыПериода.ОтчетныйИМежотчетныйПериод Тогда
		Элемент.Подсказка = НСтр("ru = 'Вид периода: отчетный и межотчетный периоды';
								|en = 'Period kind: accounting and mid-term accounting periods'");
	...
	Иначе
		Элемент.Подсказка = НСтр("ru = 'Вид периода: отчетный период';
								|en = 'Period kind: accounting period'");
