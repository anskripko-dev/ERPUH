...
Функция ДанныеФизЛиц(Организация, ФизЛицо, ДатаСреза) Экспорт
	...
	ДолжностьФизЛиц = ДолжностьФизЛиц(Организация, ФизЛицо, ДатаСреза);
	...
	Пока Данные.Следующий() Цикл
		...
		Результат.Должность = ДолжностьФизЛиц.Получить(Данные.ФизЛицо);
...
// 
// Возвращаемое значение:
//  Соответствие - Должность физ. лиц
Функция ДолжностьФизЛиц(Организация, ФизЛицо, ДатаСреза) Экспорт
	
	СписокФизическихЛиц = ?(ТипЗнч(ФизЛицо) = Тип("Массив"), ФизЛицо, ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ФизЛицо));
	...
	ДолжностьФизЛиц = Новый Соответствие();

	Для Каждого Элемент Из СписокФизическихЛиц Цикл
		
		ДанныеСотрудника = КадровыеДанные.Найти(Элемент, "ФизическоеЛицо");
		
		Если ДанныеСотрудника <> Неопределено Тогда
			Должность = Строка(ДанныеСотрудника.Должность);
		ИначеЕсли ОтветственныеЛица.Руководитель = Элемент Тогда
			Должность = ОтветственныеЛица.РуководительДолжность;
		ИначеЕсли ОтветственныеЛица.ГлавныйБухгалтер = Элемент Тогда
			Должность = ОтветственныеЛица.ГлавныйБухгалтерДолжность;
		Иначе
			Должность = "";
		КонецЕсли;
		
		ДолжностьФизЛиц.Вставить(Элемент, Строка(Должность));

	КонецЦикла;
	
	Возврат ДолжностьФизЛиц;
