	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьДоговорыСПоставщиками" ИЛИ РеквизитПутьКДанным = "" Тогда
		...
		ТекстКомментария = ТекстКомментария
			...
			Символы.ПС + НСтр("ru = '— включена поддержка платежей в соответствии с 275-ФЗ в разделе ""Казначейство""';
								|en = '- payment support is enabled as per 275-FZ in the ""Treasury"" section'"),
			"");
...
Процедура ПриСозданииНаСервере_Предприятие(Форма) Экспорт
	...
	#Область УХ_СтраныОрганизаций
	//отменяем врезку "локализация" для стран
	Форма.Элементы.ГруппаМногострановойУчет.Видимость = Истина;
	Форма.Элементы.МногострановойУчет.Видимость = Истина;
	//Форма.Элементы.ГруппаОсновнаяСтрана.Видимость = Истина;
	#КонецОбласти
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьДоговорыСКлиентами"
		...
		ТекстКомментария = ТекстКомментария
			...
			Символы.ПС + НСтр("ru = '— включена опция ""Использовать обмен с ЕИС для целей электронного актирования поставщика"" в разделе ""Обмен электронными документами""';
								|en = '— ""Use exchange with UIS for electronic certification of rendered services of the vendor"" option in the ""Electronic document exchange"" section is enabled.'"),
			"");
...
Процедура УстановитьДоступность_Производство(РеквизитПутьКДанным, Форма) Экспорт
	...
	//++ НЕ УТКА
	Если РеквизитПутьКДанным = ""
		ИЛИ СтрНайти(РеквизитПутьКДанным, "УчетТрудозатратВРазрезеСотрудников") > 0
		ИЛИ СтрНайти(РеквизитПутьКДанным, "ИспользоватьПроизводство") > 0
		ИЛИ СтрНайти(РеквизитПутьКДанным, "ИспользоватьУправлениеПроизводством") > 0 Тогда
		Элементы.ДатаНачалаВеденияУчетаТрудозатратПоСотрудникам.Доступность = Булево(Форма.УчетТрудозатратВРазрезеСотрудников);
		ОбщегоНазначенияУТКлиентСервер.ОтображениеПредупрежденияПриРедактировании(
			Элементы.УчетТрудозатратВРазрезеСотрудников, Булево(Форма.УчетТрудозатратВРазрезеСотрудников));
		Элементы.ГруппаКомментарийУчетТрудозатратВРазрезеСотрудников.Видимость = Булево(Форма.УчетТрудозатратВРазрезеСотрудников)
			И НЕ ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет")
			И НЕ  ПолучитьФункциональнуюОпцию("ИспользуетсяОбменСЗарплатаИУправлениеПерсоналом");
	КонецЕсли;
	//-- НЕ УТКА
