...
Процедура ЗаполнитьЭтапыОплатыДокументаПоЗаказам(ЭтапыГрафикаОплаты, Параметры) Экспорт
	...
	// УХ_Казначейство ++
	Если Параметры.ТабличнаяЧасть.Количество() Тогда
		...
		ЗаказПоставщикуСсылка = Параметры.ТабличнаяЧасть[0].Заказ;
		Если ЗначениеЗаполнено(ЗаказПоставщикуСсылка) 
				И Не ТипыЗаказов.Найти(ТипЗнч(ЗаказПоставщикуСсылка)) = Неопределено Тогда
				
			ВерсияСоглашения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаказПоставщикуСсылка, "Договор.ВерсияСоглашения"); 
			СтруктураРеквизитов = Новый Структура();
			СтруктураРеквизитов.Вставить("СпособРасчетаКоличестваДней");
			СтруктураРеквизитов.Вставить("ПроизводственныеКалендари");

		   	РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВерсияСоглашения, СтруктураРеквизитов);  
			ПроизводственныеКалендари = РеквизитыДоговора.ПроизводственныеКалендари.Выгрузить();
			Если РеквизитыДоговора.СпособРасчетаКоличестваДней = Перечисления.СпособыРасчетаКоличестваДнейВПериоде.ПоРабочимДням
					И ПроизводственныеКалендари.Количество() Тогда
				Календарь = ПроизводственныеКалендари[0].ПроизводственныйКалендарь;
			КонецЕсли;
		КонецЕсли;  
	КонецЕсли;
	// УХ_Казначейство --
