...
Функция ПолучитьРекомендуемыеУсловия(СтепеньРиска, СтепеньДоходности, РольКонтрагента) Экспорт
	...
	Результат.Вставить("ПроцентПросроченнойЗадолженности", 	0);
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	МатрицаРекомендуемыхУсловий.ПроцентАванса КАК ПроцентАванса,
	|	МатрицаРекомендуемыхУсловий.СрокОтсрочки КАК СрокОтсрочки,
	|	МатрицаРекомендуемыхУсловий.ДопустимыйУровеньЗадолженности КАК ДопустимыйУровеньЗадолженности,
	|	МатрицаРекомендуемыхУсловий.ПроцентПросроченнойЗадолженности КАК ПроцентПросроченнойЗадолженности
	|ИЗ
