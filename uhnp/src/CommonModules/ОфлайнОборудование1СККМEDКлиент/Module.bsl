...
Процедура НачатьВыгрузкуДанныхПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	ПараметрыОборудования = ДополнительныеПараметры.Параметры;
	...
	Если Результат.Успешно Тогда
		...
		ОфлайнОборудование1СККМEDВызовСервера.ПакетыОбработаны(Отказ, ПакетыОбработаны, МассивФайлов, ВыходныеПараметры, ПараметрыОборудования);
		
		Если НЕ Отказ И НЕ ПакетыОбработаны Тогда
			СоздатьСообщениеОбОшибке(ВыходныеПараметры, НСтр("ru = 'Файл предыдущей выгрузки не обработан (не загружен)';
															|en = 'Файл предыдущей выгрузки не обработан (не загружен)'"));
			Отказ = Истина;
		КонецЕсли;
	Иначе
...
Процедура НачатьЗагрузкуДанныхЗавершение(АдресФайлаЗагрузки, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(АдресФайлаЗагрузки)
		...
		ПараметрыОперации.Вставить("АдресФайлаЗагрузки", АдресФайлаЗагрузки);
		ПараметрыОперации.Вставить("ЭкземплярОборудования", ДополнительныеПараметры.Параметры.Идентификатор);
		
		ДлительнаяОперация = ОфлайнОборудование1СККМEDВызовСервера.ЗагрузитьДанныеНаСервере(ПараметрыОперации);
...
Процедура ОповещениеПоискФайловУстановкаФлагаДанныеЗагружены(НайденныеФайлы, ДополнительныеПараметры) Экспорт
	...
	ВыходныеПараметры = ДополнительныеПараметры.ВыходныеПараметры;
	Параметры = ДополнительныеПараметры.Параметры;
	Успешно = УстановитьФлагДанныеЗагружены(ИменаФайлов, ВыходныеПараметры, Параметры);
...
Функция УстановитьФлагДанныеЗагружены(ИменаФайлов, ВыходныеПараметры, Параметры)
	
#Если Не МобильныйКлиент И Не МобильноеПриложениеКлиент Тогда 
	Для Каждого ИмяФайла Из ИменаФайлов Цикл
		
		ТекстXML = ОфлайнОборудование1СККМEDВызовСервера.ПолучитьТекстXMLДанныеЗагружены(Параметры);
