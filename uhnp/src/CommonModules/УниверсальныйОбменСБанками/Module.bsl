...
Функция СервисСуществует(ИмяСервиса) Экспорт
	...
	Результат = (ИмяСервиса = "ФинансоваяОтчетность");
...
#Область РазборОписанияКонтейнера
	...
	Банк = НайтиБанкПоИдентификатору(ИдентификаторБанка);
	Если Банк = Неопределено Тогда
		Результат.ОписаниеОшибки = СтрШаблон("ru = 'Не найден банк с идентификатором %1';
											|en = 'Bank with ID %1 is not found'", ИдентификаторБанка);
		ЗаписатьСобытиеВЖурнал(Результат.ОписаниеОшибки, , Документооборот);
