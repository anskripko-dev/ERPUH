...
Процедура ОтразитьОперациюСебестоимостиРеализация(ПараметрыОтражения)
	...
	// Количественные и суммовые показатели
	|	(ВЫБОР
	|		КОГДА ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтчетДавальцуКорректировкаПрошлогоПериода)
	|			ТОГДА -ДанныеДокумента.Количество
	|		ИНАЧЕ ДанныеДокумента.Количество КОНЕЦ) КАК Количество,
	|	0											КАК Стоимость,
	|	0											КАК СтоимостьБезНДС,
	|	0											КАК СтоимостьРегл,
	|	0											КАК СтоимостьУпр,
