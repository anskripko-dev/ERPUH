...
Процедура НадписьАналитикиПланированияДокументаНажатие(Форма, Элемент, СтандартнаяОбработка, Оповещение = Неопределено) Экспорт
	...
	Если Форма.Объект.Свойство("РежимИспользованияГрафика") Тогда
	    ПараметрыОткрытия.Вставить("РежимИспользованияГрафика", Форма.Объект.РежимИспользованияГрафика);
	КонецЕсли;      
	Если Форма.Объект.Свойство("ПорядокРасчетов") Тогда
	    ПараметрыОткрытия.Вставить("ПорядокРасчетов", Форма.Объект.ПорядокРасчетов);
	КонецЕсли;      
...
Процедура ЗагрузитьАналитикиПланированияДокумента(Форма, РезультатЗакрытия)
	...
	Для Каждого Строка Из РезультатЗакрытия Цикл
		...
		Если Форма.Объект.Свойство("РежимИспользованияГрафика")
				И Строка.Свойство("РежимИспользованияГрафика") Тогда
		   Форма.Объект.РежимИспользованияГрафика = Строка.РежимИспользованияГрафика;
		КонецЕсли;
