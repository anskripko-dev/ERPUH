...
Функция ТекстБазаРаспределенияНоменклатурыНаПроизводство()
	...
	|				И НазначенияМатериалов.Договор = ДД.Назначение.Договор)
	|	ЛЕВОЕ СОЕДИНЕНИЕ ПартииДавальческогоПроизводства КАК ПартииДавальцев
	...
	// Если у ДавальческихМатериалов нет назначения, то по назначениям работают условия выше
	|	И (НазначенияМатериалов.НазначениеМатериала ЕСТЬ NULL
	|		ИЛИ ДД.ТипЗапасов <> ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.МатериалДавальца)
			// Если у ДавальческихМатериалов есть назначение, тогда оно должно совпадать с назначением данных по продукции, в части партнера и договора 
	|		ИЛИ НазначенияМатериалов.Партнер 		= НазначенияПродукции.Партнер
	|				И НазначенияМатериалов.Договор 	= НазначенияПродукции.Договор)
	//-- НЕ УТКА
	|
	|СГРУППИРОВАТЬ ПО
	|	ДД.Регистратор,
