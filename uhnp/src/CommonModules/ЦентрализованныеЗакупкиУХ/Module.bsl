...
Процедура ЗапускОценкиПоставщиков() Экспорт
...
Процедура ДозаполнитьТаблицуПотребностейИзРезультатаЗапроса(
										КроссТаблица,
										РезультатЗапроса,
										ИменаКлючевыхПолей, Процент = Ложь) Экспорт
...
Функция ПолучитьМассивНоменклатурыИРодителей(мНоменклатуры) Экспорт
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	Справочник.КатегорииЗакупок КАК КатегорииЗакупок
		|ГДЕ
		|	КатегорииЗакупок.Ссылка В(&мНоменклатуры)
		|ИТОГИ ПО
...
Функция ТребуетсяОбновлениеХарактеристик(Объект, Характеристика) Экспорт
	...
	Если Объект.ИспользованиеХарактеристик 
		...
		Возврат Владелец = Объект.Ссылка;
		
	ИначеЕсли Объект.ИспользованиеХарактеристик 
		= Перечисления.ВариантыИспользованияХарактеристикНоменклатуры.ОбщиеДляВидаНоменклатуры Тогда
		
		Возврат Владелец = Объект.ВидНоменклатуры
		
	ИначеЕсли Объект.ИспользованиеХарактеристик 
		= Перечисления.ВариантыИспользованияХарактеристикНоменклатуры.ОбщиеДляВидаНоменклатуры Тогда
		
		Возврат Владелец = Объект.ВладелецХарактеристик;
...
Функция ПолучитьЛотДоговора(ДоговорКонтрагента) Экспорт
	Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
		Запрос = Новый Запрос;
		 ...
		 |	ВерсияСоглашенияКоммерческийДоговор.Лот КАК Ссылка
		 |ИЗ
		 |	Документ.ВерсияСоглашенияКоммерческийДоговор.Номенклатура КАК ВерсияСоглашенияКоммерческийДоговор
		 |ГДЕ
		 |	ВерсияСоглашенияКоммерческийДоговор.Ссылка.ДоговорКонтрагента = &ДоговорКонтрагента
		 |	И ТИПЗНАЧЕНИЯ(ВерсияСоглашенияКоммерческийДоговор.Лот) = ТИП(Документ.Лот)";
...
Функция ПолучитьДоговорыСодержащиеЛотЗакупочнойПроцедуры(
	...
	Запрос.Текст = 
		...
		|	Документ.ВерсияСоглашенияКоммерческийДоговор.Номенклатура КАК ВерсияСоглашенияКоммерческийДоговорНоменклатура
		|ГДЕ
		|	ВерсияСоглашенияКоммерческийДоговорНоменклатура.Лот.ЗакупочнаяПроцедура = &ЗакупочнаяПроцедура
		|	И НЕ ВерсияСоглашенияКоммерческийДоговорНоменклатура.Ссылка.ПометкаУдаления";
	Запрос.УстановитьПараметр("ЗакупочнаяПроцедура", ЗакупочнаяПроцедура);
	РезультатЗапроса = Запрос.Выполнить();
	Возврат РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Ссылка");
...
Функция ПолучитьПериодичностьЗакупок() Экспорт
	Периодичность = ОбщегоНазначенияПовтИспУХ.ПолучитьЗначениеКонстанты("ПериодичностьЦентрализованныхЗакупок");
...
Функция ЕстьЗапрещенныеСтадии(КонтекстВход, СтадияВход) Экспорт
	...
	Если ТипЗнч(КонтекстВход) = Тип("СправочникСсылка.ЗакупочныеПроцедуры") Тогда
		// Проверка квалификационного отбора.
		Если НЕ КонтекстВход.ТребуетсяКвалификационныйОтбор Тогда
			Если ЭтоСтадияКвалификационногоОтбора(СтадияВход) Тогда			
				ЕстьЗапрещенныеСтадии = Истина;
			КонецЕсли;	
		Иначе
			Если ЭтоСтадияКвалификационногоОтбораОкончаниеКвалификации(СтадияВход) Тогда			
				ЕстьЗапрещенныеСтадии = Истина;
			КонецЕсли;	
		КонецЕсли;
		// Проверка переторжки.
		Если НЕ КонтекстВход.Переторжка Тогда
			Если ЭтоСтадияПереторжка(СтадияВход) Тогда			
				ЕстьЗапрещенныеСтадии = Истина;
			КонецЕсли;	
		КонецЕсли;
	КонецЕсли;
	Возврат ЕстьЗапрещенныеСтадии;
КонецФункции

// Получает дату начала для стадии СтадияВход.
Функция ВычислитьДатуСтадии(ДатаПоУмолчаниюВход, СтадияВход, ДатаОфициальногоОбъявленияЗакупки) Экспорт
...
Функция ДатыСтадийОпределеныВЗакупочнойПроцедуре(СтадияВход) Экспорт
	РезультатФункции = Ложь;
	// Подготовка закупки.
...
Функция ВернутьМассивСтадийШаблона(ШаблонВход, КонтекстВход) Экспорт
	...
	СтадииКвалификационногоОтбора = ВернутьСтадииКвалификационногоОтбора();
	СтадииПереторжки = ВернутьСтадииПереторжка();
	ЭтоКвалификационныйОтбор = Ложь;
	ЭтоЗакупочнаяПроцедура = Ложь;
	ЭтоПереторжка = Ложь;
	Имя = "СправочникСсылка.ЗакупочныеПроцедуры"; 
	Если ТипЗнч(КонтекстВход) = Тип(Имя) Тогда
...
Процедура ОбновитьДатыЗакупочнойПроцедуры(Объект) Экспорт 
	ЕстьЗакупочнаяПроцедура = ТипЗнч(Объект.Контекст) = Тип("СправочникСсылка.ЗакупочныеПроцедуры") И ЗначениеЗаполнено(Объект.Контекст);
	Если ЕстьЗакупочнаяПроцедура Тогда       
		Результат 	= ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Контекст, "ДатаНачалаПодготовкиЗакупки, ДатаОфициальногоОбъявленияЗакупки");
		Объект.ДатаНачала								= Результат.ДатаНачалаПодготовкиЗакупки;
		Объект.ДатаОфициальногоОбъявленияЗакупки	 	= Результат.ДатаОфициальногоОбъявленияЗакупки;
	КонецЕсли;
...
Процедура ПроверитьНаличиеОбязательныхСтадийЗакупочнойПроцедуры(Обьект, Отказ) Экспорт
	Если Обьект.ВидМероприятия = Перечисления.ВидыМероприятий.ЗакупочнаяПроцедура  Тогда
		МассивОфициальноеОбъявлениеЗакупки		 = ВернутьСтадииОфициальноеОбъявлениеЗакупки();
		МассивНачалоПодготовкиЗакупки			 = ВернутьСтадииНачалоПодготовкиЗакупкиКПубликации();
		НайденоОфициальноеОбъявлениеЗакупки	 = Ложь;
		НайденоНачалоПодготовкиЗакупки		 = Ложь;
		Для Каждого ТекСтадии Из Обьект.Стадии Цикл
		...
		Если Не НайденоОфициальноеОбъявлениеЗакупки Тогда
			...
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Шаблон%", Строка(Обьект.Наименование));
		...
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(Обьект.СпособыВыбораПоставщика) И 
			НЕ ЗначениеЗаполнено(Обьект.СпособВыбораПоставщика) Тогда
			ТекстСообщения = НСтр("ru = 'В шаблоне мероприятий %Шаблон% требуется заполнить поле ""Способ выбора поставщика"". Запись отменена.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Шаблон%", Строка(Обьект.Ссылка));
	...
	КонецЕсли;
КонецПроцедуры		// ПроверитьНаличиеОбязательныхСтадий();	


Процедура ШаблоныМероприятийЦентрализованныеЗакупкиПередЗаписьюПередЗаписью(Источник, Отказ) Экспорт
	...
	Если Источник.ВидМероприятия = Перечисления.ВидыМероприятий.ЗакупочнаяПроцедура Тогда
		Источник.ИспользоватьКонтрольЗатрат = Ложь;
		Источник.ИспользоватьПроцентВыполнения = Ложь;
	КонецЕсли;

	ПроверитьНаличиеОбязательныхСтадийЗакупочнойПроцедуры(Источник, Отказ);

КонецПроцедуры


#Область СтадииМероприятий
...
Функция СформироватьСоответствиеДатЗакупочнойПроцедурыСтадиямМероприятиям() Экспорт
	...
	Попытка
		...
		НастройкиДатЗакупочныхПроцедур = ПоставляемыеНастройкиДатЗакупочныхПроцедур();
		Для Счетчик = 0 По НастройкиДатЗакупочныхПроцедур.Количество() - 1 Цикл
			ТекстДатаЗакупочнойПроцедуры = СокрЛП(НастройкиДатЗакупочныхПроцедур[Счетчик].ДатаЗакупочнойПроцедуры);
			ТекстСтадияМероприятия       = СокрЛП(НастройкиДатЗакупочныхПроцедур[Счетчик].СтадияМероприятия);
...
Функция ВернутьСтадииКвалификационногоОтбора() Экспорт
	...
	РезультатФункции.Добавить(Справочники.СтадииМероприятий.Аукцион_ОкончаниеКвалификационногоОтбора);
	РезультатФункции.Добавить(Справочники.СтадииМероприятий.ЗапросКотировок_ОкончаниеКвалификационногоОтбора);	
	РезультатФункции.Добавить(Справочники.СтадииМероприятий.ЗапросПредложений_ОкончаниеКвалификационногоОтбора);	
	РезультатФункции.Добавить(Справочники.СтадииМероприятий.КвалификационныйОтбор_ОкончаниеКвалификационногоОтбора);	
	РезультатФункции.Добавить(Справочники.СтадииМероприятий.КонкурентныеПереговоры_ОкончаниеКвалификационногоОтбора);	
	РезультатФункции.Добавить(Справочники.СтадииМероприятий.Конкурс_ОкончаниеКвалификационногоОтбора);	
	РезультатФункции.Добавить(Справочники.СтадииМероприятий.Аукцион_МСП_НачалоКвалификационногоОтбора);	
	РезультатФункции.Добавить(Справочники.СтадииМероприятий.Аукцион_МСП_ОкончаниеКвалификационногоОтбора);	
	РезультатФункции.Добавить(Справочники.СтадииМероприятий.ЗапросКотировок_МСП_НачалоКвалификационногоОтбора);	
	РезультатФункции.Добавить(Справочники.СтадииМероприятий.ЗапросКотировок_МСП_ОкончаниеКвалификационногоОтбора);	
	РезультатФункции.Добавить(Справочники.СтадииМероприятий.ЗапросПредложений_МСП_НачалоКвалификационногоОтбора);	
	РезультатФункции.Добавить(Справочники.СтадииМероприятий.ЗапросПредложений_МСП_ОкончаниеКвалификационногоОтбора);	
	РезультатФункции.Добавить(Справочники.СтадииМероприятий.Конкурс_МСП_НачалоКвалификационногоОтбора);	
	РезультатФункции.Добавить(Справочники.СтадииМероприятий.Конкурс_МСП_ОкончаниеКвалификационногоОтбора);	
	
	Возврат РезультатФункции;
КонецФункции		// ВернутьСтадииКвалификационногоОтбора() 

Функция ВернутьСтадииКвалификационногоОтбораОкончаниеКвалификации() Экспорт
	РезультатФункции = Новый Массив;
	РезультатФункции.Добавить(Справочники.СтадииМероприятий.Аукцион_ОкончаниеКвалификационногоОтбора);
	РезультатФункции.Добавить(Справочники.СтадииМероприятий.ЗапросКотировок_ОкончаниеКвалификационногоОтбора);	
	РезультатФункции.Добавить(Справочники.СтадииМероприятий.ЗапросПредложений_ОкончаниеКвалификационногоОтбора);	
...
Функция ВернутьСтадиюДатыЗакупочнойПроцедуры(СпособВыбораПоставщикаВход, ВидДатыВход) Экспорт
	...
	Если НЕ ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ЦентрализованныеЗакупки") Тогда
		Возврат РезультатФункции;
	КонецЕсли;	
...
Функция ЭтоСтадияКвалификационногоОтбораОкончаниеКвалификации(СтадияВход) Экспорт
	РезультатФункции = Ложь;
	МассивИсключаемыхСтадий = ВернутьСтадииКвалификационногоОтбораОкончаниеКвалификации();
	НайденноеЗначение = МассивИсключаемыхСтадий.Найти(СтадияВход);
	РезультатФункции = (НайденноеЗначение <> Неопределено);
	Возврат РезультатФункции;
КонецФункции		// ЭтоСтадияКвалификационногоОтбора()
...
Функция ПоставляемыеНастройкиДатЗакупочныхПроцедур()
	
	ОписаниеТиповСтрока = ОбщегоНазначения.ОписаниеТипаСтрока(250);
	
	НастройкиДатЗакупочныхПроцедур = Новый ТаблицаЗначений;
	НастройкиДатЗакупочныхПроцедур.Колонки.Добавить("ДатаЗакупочнойПроцедуры", ОписаниеТиповСтрока);
	НастройкиДатЗакупочныхПроцедур.Колонки.Добавить("СтадияМероприятия",       ОписаниеТиповСтрока);
	
	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаКвалификационногоОтбора";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КонкурентныеПереговоры_НачалоКвалификационногоОтбора.ИмяПредопределенныхДанных;
	
	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаКвалификационногоОтбора";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросПредложений_НачалоКвалификационногоОтбора.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаКвалификационногоОтбора";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросКотировок_НачалоКвалификационногоОтбора.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаКвалификационногоОтбора";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Аукцион_НачалоКвалификационногоОтбора.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаКвалификационногоОтбора";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КвалификационныйОтбор_НачалоКвалификационногоОтбора.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаКвалификационногоОтбора";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Конкурс_НачалоКвалификационногоОтбора.ИмяПредопределенныхДанных;
	
	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПодготовкиЗакупки";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Аукцион_НачалоПодготовкиЗакупкиКПубликации.ИмяПредопределенныхДанных;
	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПодготовкиЗакупки";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросКотировок_НачалоПодготовкиЗакупкиКПубликации.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПодготовкиЗакупки";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросПредложений_НачалоПодготовкиЗакупкиКПубликации.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПодготовкиЗакупки";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КвалификационныйОтбор_НачалоПодготовкиЗакупкиКПубликации.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПодготовкиЗакупки";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КонкурентныеПереговоры_НачалоПодготовкиЗакупкиКПубликации.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПодготовкиЗакупки";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Конкурс_НачалоПодготовкиЗакупкиКПубликации.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПредоставленияРазьясненийИзвещенияДокументации";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Аукцион_НачалоПредоставленияРазъяснений.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПредоставленияРазьясненийИзвещенияДокументации";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросКотировок_НачалоПредоставленияРазъяснений.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПредоставленияРазьясненийИзвещенияДокументации";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросПредложений_НачалоПредоставленияРазъяснений.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПредоставленияРазьясненийИзвещенияДокументации";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КвалификационныйОтбор_НачалоПредоставленияРазъяснений.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПредоставленияРазьясненийИзвещенияДокументации";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КонкурентныеПереговоры_НачалоПредоставленияРазъяснений.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПредоставленияРазьясненийИзвещенияДокументации";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Конкурс_НачалоПредоставленияРазъяснений.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПриемаЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КонкурентныеПереговоры_НачалоПриемаЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПриемаЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросПредложений_НачалоПриемаЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПриемаЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Аукцион_НачалоПриемаЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПриемаЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросКотировок_НачалоПриемаЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПриемаЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КвалификационныйОтбор_НачалоПриемаЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаПриемаЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Конкурс_НачалоПриемаЗаявок.ИмяПредопределенныхДанных;
	
	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаРассмотренияЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Аукцион_ВскрытиеЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаРассмотренияЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросКотировок_ВскрытиеЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаРассмотренияЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросПредложений_ВскрытиеЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаРассмотренияЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КвалификационныйОтбор_ВскрытиеЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаРассмотренияЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КонкурентныеПереговоры_ВскрытиеЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаНачалаРассмотренияЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Конкурс_ВскрытиеЗаявок.ИмяПредопределенныхДанных;
	
	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОбъявленияЗакупки";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Аукцион_ОфициальноеОбъявлениеЗакупки.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОбъявленияЗакупки";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросКотировок_ОфициальноеОбъявлениеЗакупки.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОбъявленияЗакупки";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросПредложений_ОфициальноеОбъявлениеЗакупки.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОбъявленияЗакупки";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КвалификационныйОтбор_ОфициальноеОбъявлениеЗакупки.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОбъявленияЗакупки";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КонкурентныеПереговоры_ОфициальноеОбъявлениеЗакупки.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОбъявленияЗакупки";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Конкурс_ОфициальноеОбъявлениеЗакупки.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОглашенияРезультатов";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Аукцион_ОглашениеРезультатов.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОглашенияРезультатов";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросКотировок_ОглашениеРезультатов.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОглашенияРезультатов";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросПредложений_ОглашениеРезультатов.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОглашенияРезультатов";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КвалификационныйОтбор_ОглашениеРезультатов.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОглашенияРезультатов";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КонкурентныеПереговоры_ОглашениеРезультатов.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОглашенияРезультатов";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Конкурс_ОглашениеРезультатов.ИмяПредопределенныхДанных;
	
	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияКвалификационногоОтбора";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КонкурентныеПереговоры_ОкончаниеКвалификационногоОтбора.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияКвалификационногоОтбора";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросПредложений_ОкончаниеКвалификационногоОтбора.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияКвалификационногоОтбора";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Аукцион_ОкончаниеКвалификационногоОтбора.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияКвалификационногоОтбора";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросКотировок_ОкончаниеКвалификационногоОтбора.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияКвалификационногоОтбора";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КвалификационныйОтбор_ОкончаниеКвалификационногоОтбора.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияКвалификационногоОтбора";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Конкурс_ОкончаниеКвалификационногоОтбора.ИмяПредопределенныхДанных;
	
	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияПредоставленияРазьясненийИзвещенияДокументации";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Конкурс_ОкончаниеПредоставленияРазъяснений.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияПредоставленияРазьясненийИзвещенияДокументации";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КонкурентныеПереговоры_ОкончаниеПредоставленияРазъяснений.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияПредоставленияРазьясненийИзвещенияДокументации";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КвалификационныйОтбор_ОкончаниеПредоставленияРазъяснений.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияПредоставленияРазьясненийИзвещенияДокументации";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросПредложений_ОкончаниеПредоставленияРазъяснений.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияПредоставленияРазьясненийИзвещенияДокументации";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросКотировок_ОкончаниеПредоставленияРазъяснений.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияПредоставленияРазьясненийИзвещенияДокументации";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Аукцион_ОкончаниеПредоставленияРазъяснений.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияПриемаЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Аукцион_ОкончаниеПриемаЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияПриемаЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросКотировок_ОкончаниеПриемаЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияПриемаЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросПредложений_ОкончаниеПриемаЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияПриемаЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КвалификационныйОтбор_ОкончаниеПриемаЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияПриемаЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КонкурентныеПереговоры_ОкончаниеПриемаЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияПриемаЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Конкурс_ОкончаниеПриемаЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияРассмотренияЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КонкурентныеПереговоры_ОкончаниеРассмотренияЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияРассмотренияЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросПредложений_ОкончаниеРассмотренияЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияРассмотренияЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросКотировок_ОкончаниеРассмотренияЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияРассмотренияЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Аукцион_ОкончаниеРассмотренияЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияРассмотренияЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КвалификационныйОтбор_ОкончаниеРассмотренияЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОкончанияРассмотренияЗаявок";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Конкурс_ОкончаниеРассмотренияЗаявок.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаОфициальногоОбъявленияЗакупки";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Аукцион_ОфициальноеОбъявлениеЗакупки.ИмяПредопределенныхДанных;
	...
	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаПодведенияИтогов";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Аукцион_ПодведениеИтогов.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаПодведенияИтогов";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросКотировок_ПодведениеИтогов.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаПодведенияИтогов";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.ЗапросПредложений_ПодведениеИтогов.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаПодведенияИтогов";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КвалификационныйОтбор_ПодведениеИтогов.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаПодведенияИтогов";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.КонкурентныеПереговоры_ПодведениеИтогов.ИмяПредопределенныхДанных;

	НоваяСтрока = НастройкиДатЗакупочныхПроцедур.Добавить();
	НоваяСтрока.ДатаЗакупочнойПроцедуры = "ДатаПодведенияИтогов";
	НоваяСтрока.СтадияМероприятия       = Справочники.СтадииМероприятий.Конкурс_ПодведениеИтогов.ИмяПредопределенныхДанных;

	Возврат НастройкиДатЗакупочныхПроцедур;
		...
		Для Каждого ТекНоменклатура Из ТаблицаВход Цикл
			...
			Если ЗначениеЗаполнено(НоменклатураСтроки) Тогда
				Если ТипЗнч(НоменклатураСтроки) = Тип("СправочникСсылка.Номенклатура") Тогда
					ТекТоварнаяКатегория = НоменклатураСтроки.КатегорияЗакупок;
					Если НоменклатураСтроки.КатегорияЗакупок <> ТоварнаяКатегорияВход Тогда
						ТекстСообщения = НСтр("ru = 'Категория закупок ""%ТоварнаяКатегорияСтроки%"" для номенклатуры ""%Номенклатура%"" не совпадает с товарной категорией документа ""%ТоварнаяКатегорияДокумента%""'");
					...
					Иначе
						// Проверка пройдена успешно.
					КонецЕсли;
				ИначеЕсли ТипЗнч(НоменклатураСтроки) = Тип("СправочникСсылка.ТоварныеКатегории") ИЛИ
					ТипЗнч(НоменклатураСтроки) = Тип("СправочникСсылка.КатегорииЗакупок") Тогда
...
Функция ПолучитьДанныеЗаполненияПоДоговору(ДоговорВход, ПоступлениеОбъект) Экспорт
	...
	|	ПланированиеПотребностейОстатки.КоличествоОстаток КАК КоличествоОстаток,
	|	ПланированиеПотребностейОстатки.СуммаОстаток КАК СуммаОстаток,
	...
	|	&ДоговорВход КАК ДоговорВход,
	|	ПланированиеПотребностейОстатки.АналитикаПланирования.АналитикаПоставки.СтавкаНДС КАК СтавкаНДС,
	|	ПланированиеПотребностейОстатки.АналитикаПланирования.АналитикаПоставки.СтавкаНДС КАК АналитикаПланированияАналитикаПоставкиСтавкаНДС
	|ИЗ
	|	РегистрНакопления.ПланированиеПотребностей.Остатки(&Период, ) КАК ПланированиеПотребностейОстатки
	|ГДЕ
	|	ПланированиеПотребностейОстатки.АналитикаПланирования.ОбъектПланирования = &ДоговорВход
	|	И ПланированиеПотребностейОстатки.СуммаОстаток > 0";	
	...
	КонецЕсли;
	Запрос.УстановитьПараметр("ДоговорВход", ОбъектыПланирования.ПолучитьОбъектПланирования(ДоговорВход));
	РезультатЗапроса = Запрос.Выполнить();
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		СтруктураЗаполнения.Вставить("Цена",				 ВыборкаДетальныеЗаписи.Цена);
		СтруктураЗаполнения.Вставить("Характеристика",		 ВыборкаДетальныеЗаписи.Характеристика);
		СтруктураЗаполнения.Вставить("АналитикаПланирования",ВыборкаДетальныеЗаписи.АналитикаПланирования);
		СтруктураЗаполнения.Вставить("АналитикаИсточника",	 ВыборкаДетальныеЗаписи.АналитикаИсточника);
		РезультатФункции.Добавить(СтруктураЗаполнения);
	КонецЦикла;
	Возврат РезультатФункции;
КонецФункции		 // ПолучитьДанныеЗаполненияПоДоговору()

Функция ОбновитьПредопределенныеДанные(ИмяСправочника, УникальныйИдентификатор) Экспорт     
	
	ПараметрыОбновления = Новый Структура("ИмяСправочника", ИмяСправочника);
	
	Если МонопольныйРежим() Тогда
		
		ЦентрализованныеЗакупкиУХ.ЗаполнитьПредопределеннымиДанными(ПараметрыОбновления);	
	...
	Иначе
		...
		ДлительнаяОперация = ДлительныеОперации.ВыполнитьФункцию(
							...
							"ЦентрализованныеЗакупкиУХ.ЗаполнитьПредопределеннымиДанными",
							ПараметрыОбновления);
...
Функция ЗаполнитьПредопределеннымиДанными(ПараметрыЗагрузки) Экспорт  
	
	ПараметрыЗагрузкиСправочника = Справочники[ПараметрыЗагрузки.ИмяСправочника].ПараметрыЗагрузкиПредопределенныхДанных(ПараметрыЗагрузки);  
	
	Если ПараметрыЗагрузкиСправочника.Загружать = Ложь Тогда
		Возврат Истина;
	КонецЕсли;     
	
	УстановитьПривилегированныйРежим(Истина);
	
	Макет = Справочники[ПараметрыЗагрузкиСправочника.ИмяСправочника].ПолучитьМакет("ПредопределенныеДанные");
	...
	Для Каждого Колонка Из ТаблицаРезультат.Колонки Цикл   
		ПоляТаблицы.Добавить(Символы.Таб + "ПредопределенныеДанные." + Колонка.Имя + " КАК " + Колонка.Имя);
	КонецЦикла;
	
	Запрос = Новый Запрос;
	ОбщегоНазначенияУХ.ЗагрузитьТаблицуВоВременнуюТаблицуЗапроса(Запрос, "ПредопределенныеДанные", ТаблицаРезультат); 
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               |	ТаблицаПриемник.Ссылка КАК Ссылка,
	               |	&ПоляЗагрузки
	               |ИЗ
	               |	ПредопределенныеДанные КАК ПредопределенныеДанные
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.&ИмяСправочника КАК ТаблицаПриемник
	               |		ПО ТаблицаПриемник.&СравнениеПо = ПредопределенныеДанные.&СравнениеПо";                  
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&СравнениеПо", 		ПараметрыЗагрузкиСправочника.СравнениеПо);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяСправочника", 	ПараметрыЗагрузкиСправочника.ИмяСправочника);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПоляЗагрузки", 		СокрЛП(СтрСоединить(ПоляТаблицы, ", " + Символы.ПС)));
	
	РезультатЗапроса = Запрос.Выполнить(); 
	ДанныеЗагрузки = РезультатЗапроса.Выбрать();
	
	Пока ДанныеЗагрузки.Следующий() Цикл  
		Если Не ЗначениеЗаполнено(ДанныеЗагрузки.Ссылка) Тогда 
			НовыйОбъект = Справочники[ПараметрыЗагрузкиСправочника.ИмяСправочника].СоздатьЭлемент();
			ЗаполнитьЗначенияСвойств(НовыйОбъект, ДанныеЗагрузки);
