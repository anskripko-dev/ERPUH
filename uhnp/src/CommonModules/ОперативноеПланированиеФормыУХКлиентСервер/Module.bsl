...
Процедура УправлениеФормойДокументаДДС(Форма) Экспорт
	...
	Объект = Форма.Объект;
	Элементы = Форма.Элементы;
	Если Элементы.Найти("ГруппаАналитикиСтатьиДДСУХ") <> Неопределено Тогда
		Элементы.ГруппаАналитикиСтатьиДДСУХ.Видимость = Элементы.СтатьяДвиженияДенежныхСредств.Видимость;
	КонецЕсли;	
	 
	Если Элементы.Найти("РасшифровкаПлатежаЭлементСтруктурыЗадолженностиУХ") <> Неопределено Тогда
		Если Элементы.Найти("РасшифровкаПлатежаВидПлатежаПоКредитамЗаймам") <> Неопределено Тогда // списание со счета, РКО
			Элементы.РасшифровкаПлатежаВидПлатежаПоКредитамЗаймам.Видимость = Ложь; // используем наш реквизит
		КонецЕсли;	
	КонецЕсли;
...
Функция ВидПлатежаПоКредитамЗаймамПоЭлементуСтруктурыЗадолженности(ЭлементСтруктурыЗадолженности) Экспорт
