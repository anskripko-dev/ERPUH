...
Процедура НастройкиПередЗаписью(Объект, Отказ) Экспорт
	...
	Для Каждого Запись Из Объект Цикл
		Если НЕ ЗначениеЗаполнено(Запись.ВидДоступа) Тогда
			ТекстОшибки = НСтр("ru = 'В наборе записей присутствуют записи с незаполненным видом доступа.'");
...
Функция ПолучитьИмяИзмеренияИсточник(ИмяИзмерения)
	
	Если ИмяИзмерения="Пользователь" Тогда
		
		Возврат "ПользовательИсточник";
		
	ИначеЕсли ИмяИзмерения="Организация" Тогда
		
		Возврат "ОрганизацияИсточник";
		
	ИначеЕсли ИмяИзмерения="ВидОтчета" Тогда
		
		Возврат "ВидОтчетаИсточник";
		
	ИначеЕсли ИмяИзмерения="Бланк" Тогда
		
		Возврат "БланкИсточник";
		
	КонецЕсли;
			
КонецФункции // ПолучитьИмяИзмеренияИсточник()  

Функция ПолучитьИмяРегистраРазвернутые(Раздел)
	
	Если Раздел="ВидыОтчетов" Тогда
		
		Возврат "НастройкиПравДоступаПользователейВидыОтчетовРазвернутые";
		
	ИначеЕсли Раздел="Бланки" Тогда
		
		Возврат "НастройкиПравДоступаПользователейБланкиРазвернутые";
		
	ИначеЕсли Раздел="БланкиДанные" Тогда
		
		Возврат "НастройкиПравДоступаПользователейБланкиДанныеРазвернутые";
		
	КонецЕсли;
		
	
КонецФункции // ПолучитьИмяРегистраРазвернутые() 
...
Процедура НастройкиПриЗаписи(Объект, Раздел) Экспорт
	...
	Запрос = Новый Запрос;
	ТекстОтбора = "ИСТИНА";
	НастройкиРазвернутые = РегистрыСведений[ПолучитьИмяРегистраРазвернутые(Раздел)].СоздатьНаборЗаписей();
			
	Если Раздел = "ВидыОтчетов" Тогда
		
		Для Каждого Элемент Из Объект.Отбор Цикл
			Если Элемент.Использование И (НЕ (Элемент.Имя="УдалитьПользователь" ИЛИ Элемент.Имя="Пользователь32")) Тогда
				Запрос.УстановитьПараметр(Элемент.Имя, Элемент.Значение);
				ТекстОтбора = ТекстОтбора + " И Т." + Элемент.Имя + " = &" + Элемент.Имя;
				НастройкиРазвернутые.Отбор[ПолучитьИмяИзмеренияИсточник(Элемент.Имя)].Установить(Элемент.Значение);
			КонецЕсли;
		КонецЦикла;
	...
	ИначеЕсли  Раздел = "БланкиДанные" Тогда
		...
		|		ПО Т.ВидОтчетаИсточник = ГруппыДоступаКВидамОтчетовВидыОтчетов.Ссылка
		|			И (Т.ВидОтчетаГруппа)
		|			И Т.БланкОтчета.Владелец=ГруппыДоступаКВидамОтчетовВидыОтчетов.ВидОтчета";
...
Процедура ОбновитьПраваДоступаБланки(Объект)
	...
	Для Каждого Элемент Из Объект.Отбор Цикл
		Если Элемент.Использование И (НЕ Элемент.Имя="Пользователь32") Тогда
...
Процедура НастройкиРазвернутыеПриЗаписи(Объект, Раздел) Экспорт
	...
	МенеджерРегистраПравДоступа = РегистрыСведений["ПраваДоступаПользователейВидыОтчетов"];
...
Процедура УстановитьПраваБланкаПоВидуОтчета(ВидОтчета,БланкОтчета) Экспорт
	...
	Если Константы.ИспользоватьГруппыДоступаКВидамОтчетов.Получить() Тогда
		...
		|	ГруппыДоступаКВидамОтчетовВидыОтчетов.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.ГруппыДоступаКВидамОтчетов.ВидыОтчетов КАК ГруппыДоступаКВидамОтчетовВидыОтчетов
		|ГДЕ
		|	ГруппыДоступаКВидамОтчетовВидыОтчетов.ВидОтчета = &ВидОтчета";
