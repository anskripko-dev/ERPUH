	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Не найден вид приложения по имени ""%1""';
				|en = 'Cannot find application type by name ""%1""'"), Имя);
	ИначеЕсли НайденныеСтроки.Количество() > 1 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Найдено несколько видов приложений по имени ""%1""';
				|en = 'Multiple application types found by name ""%1""'"), Имя);
	КонецЕсли;
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Не найден вид приложения по списку услуг: %1';
				|en = 'Cannot find application type by service list: %1'"),
			ПредставлениеСпискаУслуг(Услуги, Символы.ПС + "- "));
	ИначеЕсли НайденныеСтроки.Количество() > 1 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Найдено несколько видов приложений по списку услуг:%1';
				|en = 'Multiple application types found by service list: %1'"),
			ПредставлениеСпискаУслуг(Услуги, Символы.ПС + "- "));
	КонецЕсли;
...
Процедура УстановитьПараметрыВидаПриложения(Параметры) Экспорт
	...
	ИначеЕсли ВидыПриложенийКонфигурации().Найти(Параметры.Имя, "Имя") = Неопределено Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Имя вида приложения ""%1"" не найдено в таблице видов приложений конфигурации. Параметры удалены';
				|en = 'The ""%1"" application type name is not found in the configuration application type table. Parameters are deleted'"), Параметры.Имя);
	...
	Иначе
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Имя вида приложения: %2%1Представление вида приложения: %3%1Имя вида приложения пользовательское:%4';
				|en = 'Application type name: %2%1Application type presentation: %3%1User application type name:%4'"),
	...
	ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Виды приложений. Изменение параметров';
				|en = 'Application types. Change parameters'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция НовыйПараметрыУведомленияОЗавершенииПерехода() Экспорт
	...
	ПараметрыУведомления.Вставить("Заголовок", НСтр("ru = 'Выполнен переход на новый вид приложения';
													|en = 'Migration to new application type completed'"));
