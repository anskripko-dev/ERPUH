		...
		Если ТипЗнч(ДокументОперации) = Тип("ДокументСсылка.ЧекККМ")
			...
			Если СтатусОплаты = Перечисления.ТипыСтатусовОплатыСБП.Отменена Тогда
				Если ДокументОперацииОбъект.Статус = Перечисления.СтатусыЧековККМ.Пробит Тогда 
					...
					|Чек был пробит, однако по данным НСПК данная операция была отменена, по причине: %4';
					|en = 'In the %1 %2 document dated %3, the FPS payment transaction was confirmed manually.
					|The receipt was issued, however, according to the NSPK, this transaction was canceled due to: %4'", ОбщегоНазначения.КодОсновногоЯзыка());
