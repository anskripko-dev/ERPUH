...
Процедура ОтразитьЗаданияКЗакрытиюМесяца(ИсточникСвойствДокумента, МенеджерВременныхТаблиц) Экспорт
	...
	//++ НЕ УТ
	РеглУчетСервер.ДополнитьТекстЗапросаЗаданийКЗакрытиюМесяца(
		Запрос,
		ТекстВложенногоЗапроса,
		ТекстЗапросаВременныхТаблиц,
		ИменаВременныхТаблиц);
...
Процедура ЗаданияКЗакрытиюМесяцаПередЗаписью(Источник, Отказ, Замещение) Экспорт
	...
	//++ НЕ УТ
	ИмяРегистра = "";
	СтарыеЗначенияПолейИсточника = Новый ТаблицаЗначений;
	...
	Если ЗначениеЗаполнено(ИмяРегистра) Тогда
		...
		Если ТипЗнч(Регистратор) = Тип("ДокументСсылка.РасчетСебестоимостиТоваров")
		 И ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Регистратор, "ТипЗаписи")) Тогда
			// Это служебный документ расчета себестоимости с движениями конвертации данных, по которому возможно отражение в учете в заблокированном периоде.
			ИмяРегистра = "";
		КонецЕсли;
	...
	//++ НЕ УТ
	Для Каждого ТекСтр Из СтарыеЗначенияПолейИсточника Цикл
		ЗаполнитьЗначенияСвойств(ТаблицаЗаданий.Добавить(), ТекСтр);
		ЗаполнитьЗначенияСвойств(ТаблицаЗаданийПоМесяцам.Добавить(), ТекСтр);
	КонецЦикла;
	//-- НЕ УТ
...
Функция ОписаниеГруппПараметровОперацииЗакрытияМесяца() Экспорт
	...
	ОписаниеГруппы.Описание 	=
		...
		|en = 'Parameters that restrict background jobs used in month-end closing transactions.
		|You can use the parameters only in client/server infobases
		|as the Platform does not support parallel processing of background jobs in file mode.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ОписаниеТехнологическихПараметровЗакрытияМесяца() Экспорт
	...
	ОписаниеПараметра.Наименование 		  = НСтр("ru = 'Максимальное количество одновременно выполняемых потоков формирования служебных документов';
													|en = 'Maximum number of simultaneously performed threads of internal document creation'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	ОписаниеПараметра.Описание =
		...
		|Используется в некоторых этапах закрытия месяца, задачей которых является формирование документов.';
		|en = 'Maximum number of simultaneously started background jobs for batch creation of internal documents.
		|It is used during month-end closing stages intended for creating documents.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПроверитьИзменениеСтатусаВЗаписи(Источник, СтарыеЗначенияПолейИсточника, Организация, Период)
	...
	Если НЕ ЗначениеЗаполнено(СтарыеЗначенияПолейИсточника) Тогда
		Возврат Результат;
	КонецЕсли;
