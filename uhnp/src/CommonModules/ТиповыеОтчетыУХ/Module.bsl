...
Процедура СкопироватьНастройкиКомпоновкиДанных(НастройкиПриемник, НастройкиИсточник) Экспорт
	...
	Если ТипЗнч(НастройкиИсточник) = Тип("ГруппировкаТаблицыКомпоновкиДанных")
		...
		ТипКоллекцииСКД = Тип("КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных");
		Для каждого ЭлементСтруктурыИсточник Из НастройкиИсточник.Структура Цикл
			
			Если ТипЗнч(НастройкиПриемник.Структура) = ТипКоллекцииСКД Тогда
...
// Параметры:
//	Компоновщик - КомпоновщикНастроекКомпоновкиДанных, для поиска элементов пользовательского отбора.
...
Функция СформироватьСхемуКомпоновкиДанных(Объект, УникальныйИдентификатор = Неопределено) Экспорт
	...
	Если Объект.ВидПроизвольногоОтчета = 0 Тогда
		...
		Иначе
			...
			СКД = Объект.СхемаКомпоновкиДанных.Получить();
...
Функция ПолучитьТаблицуДоступныхВариантов(НастраиваемыйОбъект, ТипНастройки, Пользователь = Неопределено, СПомеченнымиНаУдаление = Ложь) Экспорт
	...
	|			ИЛИ СохраненныеНастройкиПользователи.Пользователь В
	|				(ВЫБРАТЬ
	...
	|			ИЛИ СохраненныеНастройкиПользователи.Пользователь = ЗНАЧЕНИЕ(Справочник.ГруппыПользователей.ВсеПользователи))
	|	И (&СПомеченнымиНаУдаление
	|			ИЛИ НЕ СохраненныеНастройки.ПометкаУдаления)
	|	И НЕ СохраненныеНастройки.ЭтоГруппа
	|	И СохраненныеНастройки.ТипНастройки = &ТипНастройки
	|
	|СГРУППИРОВАТЬ ПО
	|	СохраненныеНастройки.Ссылка,
	|	СохраненныеНастройки.Наименование,
	|	СохраненныеНастройки.Описание
	|
	|УПОРЯДОЧИТЬ ПО
	|	Наименование
	|АВТОУПОРЯДОЧИВАНИЕ";
...
Процедура ОбновитьЦветаДиаграммы(Результат, ТаблицаЦветовСерий) Экспорт
	...
	Для каждого Рисунок Из Рисунки Цикл
		Если ТипЗнч(Рисунок.Объект) <> Тип("Диаграмма") Тогда
			Продолжить;
		КонецЕсли;
		ДиаграммаОбъект = Рисунок.Объект; // Диаграмма
		Серии = ДиаграммаОбъект.Серии;
...
Функция ПолучитьСхемуКомпоновкиПоИсточникуДанных(ИсточникДанных) Экспорт
	...
	СправочникПакетыДанных                          = "ПакетыДанных";
	...
	ИначеЕсли ТипЗнч(ИсточникДанных) = Тип("СправочникСсылка."+СправочникПакетыДанных) Тогда
		СКДИсточника = ИсточникДанных.СхемаКомпоновкиДанныхпоУмолчанию.Получить();
	Иначе	
		СКДИсточника = Неопределено;
	КонецЕсли;
...
Функция СформироватьСКДПоИсточникуПоказатель(ИсточникДанныхДляРасчетов)
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			ИначеЕсли  Выборка.Поле="Версия.Валюта" Тогда
				Поле.ТипЗначения = Новый ОписаниеТипов("СправочникСсылка.Валюты");
			КонецЕсли;	
	...
	НовыйПараметр = СКД.Параметры.Добавить();
	НовыйПараметр.Имя = НСтр("ru = 'Сценарий'");
	...
	Если ОбъектИсточник.СпособПолучения = Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеПоказательОтчета Тогда
		Поле = НаборДанных.Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
		Поле.Заголовок   = НСтр("ru = 'Показатель'");
...
Функция СформироватьСКДПоИсточнику(ИсточникДанныхДляРасчетов)
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.КодАналитики = "Значение" Тогда
			...
			Поле.Заголовок   = НСтр("ru = 'Значение'");
			Поле.Поле        = "Значение";
