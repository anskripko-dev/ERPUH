...
Процедура ИсправитьПроблемыВДвиженияхИДокументах(ПараметрыРасчета)
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// При выпуске продукции по рассчитываемой стоимости и применении учета продукции по плановой стоимости движения 
	// по выпуску продукции должны выполняться по разделу учета "Выпуск продукции" для расчета отклонений между плановой и
	// фактической стоимостью.
	// При выключенном учете продукции по плановой стоимости не должно быть движений по разделу учета "Выпуск продукции".
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		""СебестоимостьТоваров"",
	|		Т.Регистратор,
	|		Т.Организация,
	|		Т.ХозяйственнаяОперация,
	|		(ВЫБОР
	|			КОГДА Т.УчетГотовойПродукцииПоПлановойСтоимости ТОГДА 78
	|			ИНАЧЕ 79 КОНЕЦ) КАК КодОшибки
	|	ИЗ
	|		ВТДвиженияСебестоимостьТоваров КАК Т
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДвиженияСебестоимостьТоваров КАК Движения
	|			ПО Движения.Регистратор = Т.Регистратор
	|			И Движения.ИдентификаторФинЗаписи = Т.ИдентификаторФинЗаписи
	|			И Движения.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ВыпускПродукции)
	|	ГДЕ
	|		Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		И Т.Количество <> 0
	|		И Т.ХозяйственнаяОперация В (
	...
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыпускПродукцииВПодразделение))
	|		И НЕ Т.Сторно
	|		И (Т.УчетГотовойПродукцииПоПлановойСтоимости И Движения.Регистратор ЕСТЬ NULL
	|			ИЛИ НЕ Т.УчетГотовойПродукцииПоПлановойСтоимости И НЕ Движения.Регистратор ЕСТЬ NULL)
	//++ Устарело_Производство21
	|		И ТИПЗНАЧЕНИЯ(Т.Регистратор) <> ТИП(Документ.ВыпускПродукции)
	//-- Устарело_Производство21
	|
	...
	|		И (Т.УчетГотовойПродукцииПоПлановойСтоимости И Движения.Регистратор ЕСТЬ NULL
	|			ИЛИ НЕ Т.УчетГотовойПродукцииПоПлановойСтоимости И НЕ Движения.Регистратор ЕСТЬ NULL)
	|		И ТИПЗНАЧЕНИЯ(Т.Регистратор) <> ТИП(Документ.ДвижениеПродукцииИМатериалов)
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// При выпуске продукции по фиксированной стоимости не должен использоваться раздел учета "Выпуск продукции".
	// Раздел "Выпуск продукции" применяется только для продукции по рассчитываемой стоимости для расчета отклонений
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// При выпуске продукции в цеховую кладовую в разделе "Выпуск продукции" должна так же указываться цеховая кладовая
	// для раздельного учета фактической себестоимости по каждой цеховой кладовой.
	// Это соответствует движениям при выключенном учете по плановой стоимости - продукция сразу выпускается в цеховую кладовую.
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		""СебестоимостьТоваров"",
	|		Т.Регистратор,
	|		Т.Организация,
	...
	|		И Т.Количество <> 0
	|		И Т.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ВыпускПродукции)
	|		И ЕСТЬNULL(Т.КорАналитикаУчетаНоменклатуры.СкладскаяТерритория.ЦеховаяКладовая, ЛОЖЬ)
	...
	|
	//++ Устарело_Переработка24
	|		И ТИПЗНАЧЕНИЯ(Т.Регистратор) <> ТИП(Документ.ПоступлениеОтПереработчика)
	//-- Устарело_Переработка24
	|
	//-- НЕ УТ
	|
	...
	|		И ТИПЗНАЧЕНИЯ(Т.ДокументИсточник) <> ТИП(Документ.РаспределениеВозвратныхОтходов)
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|		ВТДвиженияСебестоимостьТоваров КАК Т
	|	ГДЕ
	|		Т.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтчетДавальцуКорректировкаПрошлогоПериода)
	|		И Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		И Т.Количество > 0
	...
	РасшифровкаКодовОшибок.Вставить(75, НСтр("ru = 'Не правильный тип записи для сторно движения фактуровки поставки в регистре ""Себестоимость товаров"".';
											|en = 'Incorrect record type for the invoice entry record storno in the ""Inventory cost"" register.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(76, НСтр("ru = 'Не заполнен вид деятельности НДС в движениях с типом записи ""Дополнение"" в регистре ""Себестоимость товаров"".';
											|en = 'The VAT activity type is not filled in register records with the ""Addition"" record type in the ""Inventory cost"" register.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(77, НСтр("ru = 'Заполнен вид деятельности НДС документа в движениях по регистру ""Себестоимость товаров"" у документов ""Передача товаров хранителю"" и ""Поступление товаров от хранителя"".';
											|en = 'The document VAT activity type is filled in the ""Inventory cost"" register records for the ""VMI fill-up"" and ""VMI return"" documents.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(78, НСтр("ru = 'При выпуске продукции по плановой стоимости нет движений по разделу учета ""Выпуск продукции"".';
											|en = 'When you release products at a standard cost, the ""Product release"" accounting section has no records.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(79, НСтр("ru = 'При выпуске продукции по фактической стоимости есть движения по разделу учета ""Выпуск продукции"".';
											|en = 'When you release products at an actual cost, the ""Product release"" accounting section has records.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(80, НСтр("ru = 'При выпуске продукции по фиксированной стоимости есть движения по разделу учета ""Выпуск продукции"".';
											|en = 'When you release products at a fixed cost, the ""Product release"" accounting section has records.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(81, НСтр("ru = 'При выпуске продукции в цеховую кладовую не правильно заполнено место хранения в разделе учета ""Выпуск продукции"".';
											|en = 'Storage location is filled incorrectly in the ""Product release"" accounting section upon releasing products to the shop floor stockroom.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(82, НСтр("ru = 'Не корректно заполнено место хранения в регистре ""Отклонения в стоимости товаро"".';
											|en = 'Storage location is filled incorrectly in the ""Goods value variances"" register.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(83, НСтр("ru = 'Не корректно заполнен период отклонений для корректировки приобретения прошлого периода в регистре ""Отклонения в стоимости товаров"".';
											|en = 'Variance period for the previous period purchase adjustment is filled incorrectly in the ""Goods value variances"" register.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(84, НСтр("ru = 'Не заполнена партия в движениях себестоимости товаров по разделу учета ""Неотфактурованные поставки"".';
											|en = 'A lot in the inventory cost records is not filled for the ""Goods received not invoiced"" accounting section.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(85, НСтр("ru = 'Не рассчиталась стоимость по методу оценки ""Среднескользящая"".';
											|en = 'Cannot calculate cost by the ""Moving average"" evaluation method.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(86, НСтр("ru = 'Не корректно отражена стоимость фактуровки поставки в регистре ""Отклонения в стоимости товаро"".';
											|en = 'Invoiced delivery cost is recorded incorrectly in the ""Goods value variances"" register.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(87, НСтр("ru = 'Заполнены различные коды строк в аналитике учета партии приходного документа при одинаковых аналитиках в строках документа';
											|en = 'Different line codes are filled in the lot accounting dimension of the incoming document for the same dimensions in the document lines'", ОбщегоНазначения.КодОсновногоЯзыка()));	
	РасшифровкаКодовОшибок.Вставить(88, НСтр("ru = 'Не заполнены реквизиты расчета партий и расчета себестоимости в движениях по регистру ""Движения Прочие активы-пассивы""';
											|en = 'The lot calculation attributes and cost calculation  attributes are not filled in the ""Flow of other assets and liabilities"" register records'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(89, НСтр("ru = 'Не правильный тип записи в сторно движениях перемещения товаров';
											|en = 'Incorrect record type in storno records of the goods transfer'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//++ НЕ УТ
	РасшифровкаКодовОшибок.Вставить(90, НСтр("ru = 'Аналитика финансового учета в расходом движении не соответствует партии производства.';
											|en = 'Financial accounting dimension in the expense document register record does not correspond to the production lot.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
	РасшифровкаКодовОшибок.Вставить(91, НСтр("ru = 'Не заполнен реквизит ""Документ движения"" в движениях отчета давальцу между организациями.';
											|en = 'The ""Transfer record"" attribute is required for records of Intercompany consumption report — Subcontracting services delivered.'"));
	РасшифровкаКодовОшибок.Вставить(92, НСтр("ru = 'Не заполнен идентификатор строки в движениях документа ""Таможенная декларация на импорт"".';
											|en = 'Не заполнен идентификатор строки в движениях документа ""Таможенная декларация на импорт"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(93, НСтр("ru = 'Не корректно заполнен кор. вид деятельности НДС в расходном движении ""Реализация (товары в пути)"".';
											|en = 'Не корректно заполнен кор. вид деятельности НДС в расходном движении ""Реализация (товары в пути)"".'"));
	РасшифровкаКодовОшибок.Вставить(97, НСтр("ru = 'Не корректно заполнен вид деятельности НДС в движениях документа ""Возврат товаров поставщику"".';
											|en = 'Не корректно заполнен вид деятельности НДС в движениях документа ""Возврат товаров поставщику"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(98, НСтр("ru = 'Заполнены различные коды строк в аналитике учета партии приходного документа при одинаковых аналитиках в строках документа';
											|en = 'Different line codes are filled in the lot accounting dimension of the incoming document for the same dimensions in the document lines'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(99, НСтр("ru = 'Неправильный знак у количества для операции ""Отчет давальцу корректировка прошлого периода""';
											|en = 'Неправильный знак у количества для операции ""Отчет давальцу корректировка прошлого периода""'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(100, НСтр("ru = 'Не заполнен реквизит ""Заказ клиента"" в движениях реализации товаров и услуг.';
												|en = 'Не заполнен реквизит ""Заказ клиента"" в движениях реализации товаров и услуг.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
//-- НЕ УТ
	...
	РасшифровкаКодовОшибок.Вставить(5, НСтр("ru = 'В движениях по регистру ""Прочие расходы"" излишне заполнена сумма постоянной разницы.';
											|en = 'In records of the ""Other expenses"" register, the permanent difference amount is unnecessarily filled.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	|	Т.Период МЕЖДУ &НачалоПериода И &КонецПериода
	//++ НЕ УТ
	|	И ЛОЖЬ // проверка выполняется только в конфигурации ""Управление торговлей""
	//-- НЕ УТ
	|	И Т.Организация В (&МассивОрганизаций)
	|	И Т.Активность
	|	И НЕ Т.РасчетПартий
...
//-- НЕ УТ
	...
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru = 'Включение учета себестоимости по назначениям';
											|en = 'Включение учета себестоимости по назначениям'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru = 'Выключение учета себестоимости по назначениям';
											|en = 'disable cost accounting by assignments'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
//++ НЕ УТКА
	...
	|	ПО СебестоимостьТоваров.Регистратор = ТаблицаВыходныеИзделия.Ссылка
	|	И СебестоимостьТоваров.ИдентификаторСтроки = ТаблицаВыходныеИзделия.ИдентификаторСтроки
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ПобочныеИзделия КАК ТаблицаПобочныеИзделия
	|	ПО СебестоимостьТоваров.Регистратор = ТаблицаПобочныеИзделия.Ссылка
	|	И СебестоимостьТоваров.ИдентификаторСтроки = ТаблицаПобочныеИзделия.ИдентификаторСтроки
	|
	|ГДЕ
	...
	// Исключаем выпуск по плановой стоимости.
	|	И СебестоимостьТоваров.РазделУчета <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ВыпускПродукции)
	// Исключаем только непрослеживаемые работы.
	...
	|		ИЛИ СебестоимостьТоваров.АналитикаУчетаНоменклатуры.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|			И СебестоимостьТоваров.АналитикаУчетаНоменклатуры.Номенклатура.ПрослеживаемыйТовар
	...
	|			ИЛИ СебестоимостьТоваров.ВидЗапасов = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка))
	|		)
	...
	|	И НЕ ЕСТЬNULL(ТаблицаВыходныеИзделия.СписатьНаРасходы, ЛОЖЬ)
	|	И НЕ ЕСТЬNULL(ТаблицаПобочныеИзделия.СписатьНаРасходы, ЛОЖЬ)
...
//-- НЕ УТКА
	...
	|	И СебестоимостьТоваров.НастройкаХозяйственнойОперации <> ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ПередачаПродукцииИзПроизводства)
	...
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru = 'Неправильное движение возврата на другой склад в документе ""Возврат товаров от клиента"".';
											|en = 'Incorrect record of return to another warehouse in the ""Sales return"" document.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(2, НСтр("ru = 'Неправильное движение возврата на другой склад в документе ""Возврат товаров между организациями"".';
											|en = 'Incorrect record of return to another warehouse in the ""Goods return — Intercompany"" document.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//++ Локализация

	//++ НЕ УТ
	РасшифровкаКодовОшибок.Вставить(3, НСтр("ru = 'Неправильное движение возврата на другой склад в документе ""Возврат сырья давальцу"".';
											|en = 'Incorrect record of return to another warehouse in the ""Goods return — Subcontracting services delivered"" document.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
	...
	|ГДЕ
	|	СебестоимостьТоваров.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И СебестоимостьТоваров.Регистратор ССЫЛКА Документ.ВозвратСырьяОтПереработчика
	|	И СебестоимостьТоваров.ХозяйственнаяОперация В (
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СторноРеализацииВозвратНаДругойСклад),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СторноПереданнойТарыВозвратНаДругойСклад))
	|	И СебестоимостьТоваров.Количество < 0 И СебестоимостьТоваров.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	...
	ПараметрыРасчета,
		Запрос,
		РасшифровкаКодовОшибок,
		НСтр("ru = 'исправление движений документов при возврате товара/тары на другой склад';
			|en = 'Correction of document records when returning goods/packages to another warehouse'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасшифровкаКодовОшибок.Вставить(3, НСтр("ru = 'Сформированы движения по регистру ""Партии прочих расходов"" для организации с методом оценки стоимости ""ФИФО (взвешенная)"".';
											|en = 'The ""Material overheads for allocation"" register records are created for a company with the ""FIFO (weighted estimate)"" cost evaluation method.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасшифровкаКодовОшибок.Вставить(2,
		НСтр("ru = 'Отличаются суммы в движения по регистру ""Закупки"" от регистров ""Себестоимость товаров"" и ""Отклонения в стоимости товаров""';
			|en = 'Amounts in the ""Purchases"" register records differ from amounts in the ""Inventory cost"" and ""Goods value variances"" register records'",
			ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(3,
		НСтр("ru = 'Отличаются суммы в движения по регистру ""Отклонения в стоимости товаров"" от регистра ""Закупки""';
			|en = 'Amounts in the ""Goods value variances"" register records differ from amounts in the ""Purchases"" register records'",
			ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасшифровкаКодовОшибок = Новый Соответствие;
	РасшифровкаКодовОшибок.Вставить(1,
		НСтр("ru = 'Сформированы некорректные движения по регистру ""Отклонения в стоимости товаров""';
			|en = 'Incorrect records are created in the ""Goods value variances"" register'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(2,
		НСтр("ru = 'Движения по регистру ""Отклонения в стоимости товаров"" не требуются для используемой учетной политики';
			|en = 'The accounting policy does not require the ""Goods value variances"" register records'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументыПоОтдельнымРегистрамНакопления(
		...
		НСтр("ru = 'исправление движений документов по регистру ""Отклонения в стоимости товаров"".';
			|en = 'Adjusting document records in the ""Goods value variances"" register.'",
			ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ТекстЗапросаВключениеУчетаСебестоимости() Экспорт
	Возврат "
		...
		// Исключаем перенос РНПТ материалов на продукцию (документы заполняют только номер ГТД, в себестоимости не участвуют)
		|		И ТИПЗНАЧЕНИЯ(ТоварыОрганизаций.Регистратор) <> ТИП(Документ.ПереносРНПТМатериаловНаПродукцию)
		//-- НЕ УТ
