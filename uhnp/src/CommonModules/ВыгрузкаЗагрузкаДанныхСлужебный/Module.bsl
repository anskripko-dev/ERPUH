	...
	Для Каждого Расширение Из ДанныеРасширений.Расширения1СПлюс Цикл
		...
		Исключение
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось получить файл расширения
				|%1
				|%2';
				|en = 'Cannot get the extension file
				|%1
				|%2'"),
				,
				ТехнологияСервиса.ПодробныйТекстОшибки(ИнформацияОбОшибке()));
			ИмяСобытияЖР = НСтр("ru = 'Расширения 1С:Плюс.Ошибка хранилища МС';
								|en = '1C:Plus extensions.Service Manager storage error'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПолучитьЗаданиеПараллельнойВыгрузкиЗагрузкиДанных() Экспорт
	...
	Если Задание = Неопределено И РаботаВМоделиСервиса.РазделениеВключено() Тогда
		ВызватьИсключение НСтр(
			...
			|en = 'You can perform parallel data export\import only in the background job.'");
	КонецЕсли;
...
Функция ПрочитатьИнформациюОВыгрузке(Контейнер)
	...
	Если Файл = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'В информации о составе файлов выгрузки отсутствует описание файла с информацией о выгрузке';
								|en = 'The information about export files does not contain details of the file with export information.'");
	КонецЕсли;
