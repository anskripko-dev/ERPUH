...
Функция ПутьКлассОбъектаУчета(ТипОбъектаУчета) Экспорт
	
	Если ТипОбъектаУчета = ВстраиваниеУХКлиентСервер.ПолучитьТипОС() Тогда
		Возврат "КлассВНА";
	ИначеЕсли ТипОбъектаУчета = Тип("СправочникСсылка.НематериальныеАктивы") Тогда
		Возврат "КлассВНА";
	ИначеЕсли ТипОбъектаУчета = Тип("СправочникСсылка.Контрагенты") Тогда
		Возврат "ГруппаКонтрагента";
	ИначеЕсли ТипОбъектаУчета = Тип("СправочникСсылка.Организации") Тогда
		Возврат "ГруппаКонтрагента";
	ИначеЕсли ТипОбъектаУчета = Тип("СправочникСсылка.ЦенныеБумаги") Тогда
		Возврат "ПараметрыУчетаФИ";
	КонецЕсли;

КонецФункции

Функция СценарийМСФО(СценарийКогдаНеЗадан = Неопределено) Экспорт

	Сценарий = УчетМСФОПовтИспУХ.ПолучитьСценарийМСФО();
	Если Не ЗначениеЗаполнено(Сценарий) Тогда
		Если СценарийКогдаНеЗадан = Неопределено Тогда
			Сценарий = ПредопределенноеЗначение("Справочник.Сценарии.Факт");
		Иначе 
			Сценарий = СценарийКогдаНеЗадан;
		КонецЕсли;
	КонецЕсли;
...
Функция СубконтоПоШаблону(ШаблонИменСубконто = "Субконто%1") Экспорт
	
	Результат = Новый Структура;
	
	ЧислоСубконто = УчетМСФОПовтИспУХ.ПолучитьМаксКоличествоСубконто();
	Для Индекс = 1 По ЧислоСубконто Цикл
		Результат.Вставить("Субконто" + Индекс, СтрШаблон(ШаблонИменСубконто, Индекс));
	КонецЦикла;
...
// Процедура заполняет условное оформление ключевых субконто
		...
		Если Индекс <= ДанныеСчета.КоличествоСубконто И НЕ НужноСкрытьСубконто(СкрыватьСубконто, ТипЗначенияСубконто) Тогда
			Если ЭтоТаблица Тогда
				Если ПоляФормы.Свойство("Субконто" + Индекс) Тогда
					...
					Форма.Элементы[ПоляФормы["Субконто" + Индекс]].ПодсказкаВвода	 = Строка(ДанныеСчета["ВидСубконто" + Индекс]);
				КонецЕсли;
			Иначе
				...
				Если (ЗаголовкиПолей = Неопределено) Тогда
					...
					Если ПоляФормы.Свойство(ИмяСубконто)  Тогда
						Форма.Элементы[ПоляФормы[ИмяСубконто]].Заголовок = Строка(ДанныеСчета["ВидСубконто" + Индекс]);
					КонецЕсли;
				...
				ИначеЕсли ЗаголовкиПолей.Свойство(ИмяСубконто) Тогда
					...
					КонецЕсли;
					Форма[ЗаголовкиПолей[ИмяСубконто]] = Строка(ДанныеСчета["ВидСубконто" + Индекс]) + ":";
...
Процедура ПриИзмененииСчета(Счет, Объект, ПоляОбъекта, ЭтоТаблица = Ложь, ЗначенияСубконто = Неопределено, СкрыватьСубконто = Неопределено) Экспорт
	...
	Для каждого ПолеИзмерение Из ПоляОбъекта Цикл
		Если Не ДанныеСчета.Свойство(ПолеИзмерение.Ключ) Тогда
			Продолжить;// не измерение, а субконто
		ИначеЕсли ДанныеСчета[ПолеИзмерение.Ключ] <> Ложь Тогда
			Продолжить;// измерение отображается - не очищаем
		ИначеЕсли Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
			ДанныеСчета, ПолеИзмерение.Значение) Тогда
			Продолжить;// нет поля заполнения
		ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
				Объект, ПолеИзмерение.Значение) Тогда
				
			Объект[ПолеИзмерение.Значение] = Неопределено;
...
Процедура ИзменитьПараметрыВыбораПолейСубконто(Форма, Объект, ШаблонИмяПоляОбъекта, ШаблонИмяЭлементаФормы, СписокПараметров) Экспорт
	...
	ТипПодразделения = Неопределено;
	Если УчетМСФОПовтИспУХ.ЕстьРСБУ() Тогда
		ИмяМД = ВстраиваниеУХКлиентСервер.ИмяСправочникаПодразделения();
		Попытка
			ТипПодразделения = Тип(СтрЗаменить(ИмяМД, ".", "Ссылка" + "."));
		Исключение
		КонецПопытки;
	КонецЕсли;
	
	ВидыПараметров = Новый Соответствие;
	ВидыПараметров.Вставить(ТипПодразделения, "Подразделение");
	...
	Для Индекс = 1 По 3 Цикл
		...
		Если ВидПараметра <> Неопределено Тогда
			...
			ИначеЕсли ВидПараметра = "Подразделение" Тогда
				...
				МодульБУ = ОбщийМодуль("БухгалтерскийУчетКлиентСерверПереопределяемый");
				ИмяРеквизитаОрганизация = МодульБУ.ИмяРеквизитаОрганизацияПодразделения();
		...
		Если ОчищатьСвязанныеСубконто 
			...
			Если ТипыСвязанныхСубконто = Неопределено Тогда
				ВсеТипыСвязанныхСубконто = УчетМСФОПовтИспУХ.ВсеТипыСвязанныхСубконто();
...
Функция КлючевыеСубконтоСчета(СчетУчета) Экспорт
	
	//счет учета - скрываем необоротные субконто
	//счет доходов/расходов - скрываем субконто счета учета и его зависимые
	
	КлючевыеСубконто = Новый Соответствие;	
	
	ЗависимыеТипы =	ЗависимыеТипы();
	КоличествоСубконто = 3;
	
	СвойстваСчета = УчетМСФОПовтИспУХ.ПолучитьСвойстваСчета(СчетУчета);
	Для й = 1 По КоличествоСубконто Цикл

		ТекущееОТ = СвойстваСчета["ВидСубконто" + й + "ТипЗначения"];
		Если (ТекущееОТ = Неопределено) Или (СвойстваСчета["ВидСубконто" + й + "ТолькоОбороты"]) Тогда 
			Продолжить;
		КонецЕсли;
		
		Для каждого ТекущийТип Из ТекущееОТ.Типы() Цикл
			
			КлючевыеСубконто.Вставить(ТекущийТип, "Субконто");
			ТекущиеЗависимые = ЗависимыеТипы.Получить(ТекущийТип);
			Если ТекущиеЗависимые = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Для каждого ЗависимыйТип Из ТекущиеЗависимые Цикл
				КлючевыеСубконто.Вставить(ЗависимыйТип, "Субконто");
			КонецЦикла;
...
Процедура УстановитьПланСчетов(Форма, ИменаСубконто = Неопределено) Экспорт
	...
	КонтекстФормы = ПоляОбъекта(Форма, "Объект, Запись, КэшируемыеЗначения");
	ИмяОбъект = ?(КонтекстФормы.Запись=Неопределено, "Объект", "Запись");
	
	КонтекстОбъекта = ПоляОбъекта(КонтекстФормы, "ПланСчетовБД, ПланСчетов");
	ИмяПланСчетов = ?(КонтекстОбъекта.ПланСчетов=Неопределено, "ПланСчетовБД", "ПланСчетов");
	
	Если Не ЗначениеЗаполнено(Форма[ИмяОбъект][ИмяПланСчетов]) Тогда
		
		Для каждого ИмяСубконто Из Форма.КэшируемыеЗначения.ИменаСубконто Цикл
			СчетБД = Форма[ИмяОбъект][ИмяСубконто.Ключ];
			Если ЗначениеЗаполнено(СчетБД) Тогда
				Форма[ИмяОбъект][ИмяПланСчетов] = ОбщегоНазначенияУХ.ЗначениеРеквизитаОбъекта(СчетБД, "Владелец");
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		Если Форма[ИмяОбъект][ИмяПланСчетов].Пустая() Тогда
			Форма[ИмяОбъект][ИмяПланСчетов] = МСФОВызовСервераУХ.ПланСчетовУП(Неопределено);
		КонецЕсли;
		
	КонецЕсли;
	
	ПараметрыВыбораИсточник = Новый Массив;
	ПараметрыВыбораИсточник.Добавить(Новый ПараметрВыбора("Владелец", Форма[ИмяОбъект][ИмяПланСчетов]));
	...
	Для каждого ИмяСчета Из ИменаСубконто Цикл
		...
		Если ТипЗнч(ИмяСчета.Значение) = Тип("Соответствие") Тогда
			...
			ОбновитьСубконтоСчета(Форма, ИмяСчета.Ключ, Форма[ИмяОбъект]);
			
		КонецЕсли;

		// это ТЧ
		Для каждого ИмяСчетаТЧ Из ИмяСчета.Значение Цикл

			Элемент = Форма.Элементы.Найти(ИмяСчетаТЧ.Ключ);
			Если Элемент = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Элемент.ПараметрыВыбора = ПараметрыВыбора;
			ОбновитьСубконтоСчета(Форма, ИмяСчета.Ключ,, ИмяСчета.Ключ);
...
#Область РаздельныеЗакупки

Функция ЭтоОрдер(Объект) Экспорт
	
	ПоляОбъекта = ПоляОбъекта(Объект, "ЭтапРаздельнойЗакупки");
	
	Возврат
	
		ЗначениеЗаполнено(ПоляОбъекта.ЭтапРаздельнойЗакупки)
		И ПоляОбъекта.ЭтапРаздельнойЗакупки
			= ПредопределенноеЗначение("Справочник.ЭтапыРаздельнойЗакупки.ОрдерПередНакладной")
		Или ПоляОбъекта.ЭтапРаздельнойЗакупки
			= ПредопределенноеЗначение("Справочник.ЭтапыРаздельнойЗакупки.ОрдерПослеНакладной");

КонецФункции

Процедура УправлениеФормой_ПТУ(Форма) Экспорт
	
	Объект = Форма.Объект;
	Элементы = Форма.Элементы;
	
	//ЭлементСФ = Элементы.Найти("ГруппаУПД_СФ");
	//Если (ЭлементСФ <> Неопределено) И Не ЭлементСФ.РазрешитьИзменениеСостава Тогда
	//	
	//	//Были скрыты СФ, вернем по умолчанию

	//	УстановитьСвойство(Элементы, "ПредставлениеУПД",	"Видимость", Истина);
	//	УстановитьСвойство(Элементы, "СчетФактураСсылка",	"Видимость", Истина);
	//	УстановитьСвойство(Элементы, "СчетФактураКнопка",	"Видимость", Истина);
	//	
	//КонецЕсли;
	
	ЭлементЭтап = Форма.Элементы.Найти("ЭтапРаздельнойЗакупкиУХ");
	Если ЭлементЭтап = Неопределено Тогда
		Возврат;// не было этапа на форме
	ИначеЕсли Не ЭлементЭтап.Видимость Тогда
		Возврат;// по договору нет этапов
	КонецЕсли;
	
	ЭтоОрдер = ЭтоОрдер(Форма.Объект);
	Если Не ЭтоОрдер Тогда
		Возврат;// по фактуровке (накладной) уже настроено
	КонецЕсли;
	
	УстановитьСвойство(Элементы, "ГруппаВхНомерДата",		"Видимость", Ложь);
	УстановитьСвойство(Элементы, "ОригиналСчетаФактуры",	"Видимость", Ложь);

	УстановитьСвойство(Элементы, "ГруппаУПД_СФ", 			"Видимость", Ложь);
	УстановитьСвойство(Элементы, "СчетФактураПросмотр",		"Видимость", Ложь);
	УстановитьСвойство(Элементы, "УПД",						"Видимость", Ложь);
	
КонецПроцедуры

Функция УстановитьСвойство(Элементы, ИмяЭлемента, ИмяСвойства, Значение, ЗначениеИзменено = Ложь) Экспорт
...
Процедура ОбновитьЗависимыеРеквизиты(СтрокаИсточник, ФормаДокумента, КолонкаИсточник) Экспорт
	...
	Для каждого Реквизит Из ФормаДокумента.КэшируемыеЗначения.ЗависимостиРеквизитов Цикл
		...
		ЗначениеИсточник = ОбщегоНазначенияУХ.ПолучитьЗначениеРеквизита(
			СтрокаИсточник[КолонкаИсточник], Реквизит.РеквизитГруппы);

		Если (КолонкаИсточник = "ПараметрыАмортизацииВНАМСФО") 
			И (ТипЗнч(ФормаДокумента) = Тип("ФормаКлиентскогоПриложения")) Тогда
			
			Поле = ФормаДокумента.Элементы.Найти("ВНА" + Реквизит.Приемник);
			Если Поле <> Неопределено Тогда
				Поле.ТолькоПросмотр = ЗначениеЗаполнено(ЗначениеИсточник);
			КонецЕсли;
			
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ЗначениеИсточник) Тогда
			Продолжить;//оставляем предыдущим заполнением
		КонецЕсли;
		
		СтрокаИсточник[Реквизит.Приемник] = ЗначениеИсточник;
...
Процедура УстановитьВидимостьЭлементаФормы(ЭтаФорма, СоответствиеИменРеквизитовПараметрам, ИмяПараметра = Неопределено, УстанавливаемаяВидимость = Истина)

	Если (ИмяПараметра <> Неопределено) И СоответствиеИменРеквизитовПараметрам.Свойство(ИмяПараметра) Тогда	
		ЭтаФорма.Элементы[СоответствиеИменРеквизитовПараметрам[ИмяПараметра]].Видимость = УстанавливаемаяВидимость;	
	КонецЕсли;

КонецПроцедуры

Процедура ПриИзмененииРеквизита(Форма, ДействияИсточник, ЗаполнитьЗависимые = Ложь) Экспорт 
	
	Действия = ПолучитьДействия(ДействияИсточник, ЗаполнитьЗависимые);
	
	Если Действия.Свойство("ОбновитьКэшОперации") Тогда
		ОбновитьКэшОперации(Форма);
	КонецЕсли;
	
	Если Действия.Свойство("ЗаполнитьУП") Тогда
		ОбновитьРеквизитыУП(Форма);
	КонецЕсли;
	
	Если Действия.Свойство("ЗаполнитьПОУ") Тогда
		ОбновитьПОУ(Форма);
	КонецЕсли;
	
	Если Действия.Свойство("ЗаполнитьСтрану") Тогда
		ОбновитьПараметрыВыбораСтрана(Форма);
	КонецЕсли;

	Если Действия.Свойство("ЗаполнитьБанковскийСчет") Тогда
		ЗаполнитьБанковскийСчет(Форма, Истина);
	КонецЕсли;
	
	Если Действия.Свойство("ЗаполнитьБСчетКонтрагента") Тогда
		ЗаполнитьБанковскийСчет(Форма, Ложь);
	КонецЕсли;
	
	Если Действия.Свойство("ЗаполнитьДоговор") Тогда
		
		МСФОВызовСервераУХ.УстановитьДоговорКонтрагента(
	...
	Если Действия.Свойство("ЗаполнитьТипЦен") Тогда
		
		ИмяТипЦен = "ТипЦен";
		Если ИдентификацияПродуктаУХКлиентСервер.ЭтоЕХ() Тогда
			ИмяТипЦен = "ВидЦенПродажи";
		КонецЕсли;
		
		ТипЦен = ОбщегоНазначенияУХ.ЗначениеРеквизитаОбъекта(
					Форма.Объект.ДоговорКонтрагента, ИмяТипЦен);
		Если ЗначениеЗаполнено(ТипЦен) Тогда
			Форма.Объект.ТипЦен = ТипЦен;
		КонецЕсли;
		
	КонецЕсли;
	
	ОбновитьВалютуКурс(Форма, Действия.Свойство("ЗаполнитьВалюту"), Действия.Свойство("ЗаполнитьКурс"));
	
	Если Действия.Свойство("ЗаполнитьДКЗ") Тогда
		ЗаполнитьДКЗ(Форма, Действия);
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьПоСчетФактуре(Форма, РеквизитыСФ = Неопределено, ТребуетсяВсегда = Ложь, СтруктураОтбора = Неопределено, ИмяРеквизитаСчетФактура = "СчетФактура", ИмяРеквизитаСсылка = "Ссылка") Экспорт
	
	Если РеквизитыСФ <> Неопределено Тогда
		ИсходныеДанные = РеквизитыСФ;
	ИначеЕсли Форма.КэшируемыеЗначения.Свойство(ИмяРеквизитаСчетФактура) Тогда
		ИсходныеДанные = Форма.КэшируемыеЗначения[ИмяРеквизитаСчетФактура];
	Иначе
	...
	Иначе
		ТребуетсяСчетФактура = Истина;//Форма.КэшируемыеЗначения.ТребуетсяСчетФактура;
	КонецЕсли;
	...
	СФ = ?(ЗначениеЗаполнено(ДанныеНадписи.СчетФактура), 
		ДанныеНадписи.СчетФактура, 
		ПредопределенноеЗначение("Документ.СчетФактураМСФО.ПустаяСсылка"));
		
	Форма.КэшируемыеЗначения.Вставить(ИмяРеквизитаСчетФактура, СФ);
	...
	Если ДанныеНадписи.СчетФактура <> Неопределено Тогда
		...
		Если ДанныеНадписи.Проведен = Ложь Тогда
			СтатусПроведения = НСтр("ru = ' (не проведен)'");
...
//
Процедура УправлениеФормой_ВидОперации(Форма) Экспорт
	
	Объект = Форма.Объект;
	ЭлементыФормы = Форма.Элементы;
	
	КэшОперации = Форма.КэшируемыеЗначения.КэшОперации;
	
	Форма.КэшируемыеЗначения.Вставить("ДокументБезНДС", Не КэшОперации.ЕстьКосвенныеНалоги);
	
	ЭлементыФормы.ПозицииАктив.Заголовок 			= КэшОперации.ЗаголовокАктив;
	ЭлементыФормы.ПозицииПараметрыУчета.Заголовок	= КэшОперации.ЗаголовокПараметрыУчетаАктива;
	ЭлементыФормы.ПозицииКоличество.Видимость		= КэшОперации.ЕстьКоличество;
	ЭлементыФормы.ПозицииКоличествоДоп.Видимость	= КэшОперации.ЕстьДоли;//ПрямаяДоля Нов
	ЭлементыФормы.ПозицииЭффективнаяДоля.Видимость	= КэшОперации.ЕстьДоли;//ЭффективнаяДоля Стар
	ЭлементыФормы.ПозицииОтношениеКГруппе.Видимость	= КэшОперации.ЕстьДоли;
	ЭлементыФормы.ПозицииСодержание.Видимость		= КэшОперации.ВидимостьСодержание;
	
	ЭлементыФормы.НДСВключенВСтоимость.Видимость 	= КэшОперации.ЕстьКосвенныеНалоги;
	ЭлементыФормы.ПозицииСтавкаНДС.Видимость 		= КэшОперации.ЕстьКосвенныеНалоги;
	ЭлементыФормы.ПозицииСуммаНДС.Видимость 		= КэшОперации.ЕстьКосвенныеНалоги;
	ЭлементыФормы.ПозицииСуммаСНДС.Видимость 		= КэшОперации.ЕстьКосвенныеНалоги;
	ЭлементыФормы.ПозицииИтогСуммаНДС.Видимость 	= КэшОперации.ЕстьКосвенныеНалоги;
	
	ЭлементыФормы.ПараметрыУчета.Видимость 			= КэшОперации.ВидимостьПараметровУчета;
	ЭлементыФормы.ПозицииПараметрыУчета.Видимость 	= КэшОперации.ВидимостьПараметровУчета;
	ЭлементыФормы.ПозицииАктив.ОграничениеТипа 		= КэшОперации.АктивОграничениеТипа;
	
	Если ЭлементыФормы.Найти("ПозицииПараметрыАмортизации") <> Неопределено Тогда //ПТУ ВНА
		ЭлементыФормы.ПозицииПараметрыАмортизации.Видимость = КэшОперации.ВидимостьПараметровАмортизации;
	КонецЕсли;
	
КонецПроцедуры

Процедура УправлениеФормой_ПараметрыУчета(Форма, РеквизитПараметры = "ПараметрыУчетаВзаиморасчетов") Экспорт
	
	Элементы = Форма.Элементы;
	
	РасчетыПУ = Форма.Объект[РеквизитПараметры];
	
	Элементы.ПараметрыУчета.Заголовок = ?(РасчетыПУ.Пустая(), НСтр("ru = '<...>'"), РасчетыПУ);
	
КонецПроцедуры

Процедура УправлениеФормой_ЦеныИВалюта(Форма) Экспорт

	Элементы = Форма.Элементы;
	Объект = Форма.Объект;

	ВалютныйДокумент = (Объект.ВалютаДокумента <> Форма.КэшируемыеЗначения.ФункциональнаяВалюта);
	
	ЦеныВалюта = Новый Массив;
	Если (Элементы.Найти("НДСВключенВСтоимость") <> Неопределено)
		И Элементы.НДСВключенВСтоимость.Видимость Тогда //исп. косвенные налоги
		ВклНДС = Объект.НДСВключенВСтоимость;
		ЦеныВалюта.Добавить(?(ВклНДС, НСтр("ru = 'НДС в сумме'"), НСтр("ru = 'НДС сверху'")));
	КонецЕсли;
	
	Если ВалютныйДокумент Тогда
		ФВ = Форма.КэшируемыеЗначения.ФункциональнаяВалюта;
		Кратность = Объект.КратностьВзаиморасчетов;
		ЦеныВалюта.Добавить(СтрШаблон("%1 %2 = %3 %4", Кратность, Объект.ВалютаДокумента, Объект.КурсВзаиморасчетов, ФВ));
	КонецЕсли;
	
	Если ЦеныВалюта.Количество() = 0 
		И (Элементы.Найти("ТипЦен") <> Неопределено)
		И Элементы.ТипЦен.Видимость Тогда
		ЦеныВалюта.Добавить(?(ЗначениеЗаполнено(Объект.ТипЦен), Объект.ТипЦен, "<...>"));
	КонецЕсли;
	
	Элементы.ЦеныИВалюта.Заголовок 		= СтрСоединить(ЦеныВалюта, ", ");
	Элементы.ГруппаВалюта.Видимость 	= ВалютныйДокумент;

КонецПроцедуры

Процедура УправлениеФормой_СФ(Форма) Экспорт

	СчетФактура = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Форма.КэшируемыеЗначения, "СчетФактура");
	
	НастроитьПоляСчетаФактуры(
		Форма.Элементы.СчетФактураКнопка,
		Форма.Элементы.СчетФактураСсылка,
		Форма.Элементы.НадписьСчетФактура,
		Форма.КэшируемыеЗначения.ДокументБезНДС,
		Не Форма.КэшируемыеЗначения.ДокументБезНДС,
		СчетФактура);

КонецПроцедуры

Процедура УправлениеФормой_Итоги(Форма, ПолеИтога = "Сумма", ИмяТЧ = "Позиции") Экспорт

	ОбщегоНазначенияКлиентСерверУХ.УстановитьНовоеЗначение(
			Форма.Объект.СуммаДокумента,
			Форма.Объект[ИмяТЧ].Итог(ПолеИтога),
...
Процедура ЗаполнитьНачислятьАмортизациюМСФО(Форма, ЗаполнитьИзПараметровУчетаВНА = Ложь) Экспорт
	...
	Для каждого СтрокаВНА Из Объект.ВНА Цикл
		...
		СтрокаВНА.НачислятьАмортизациюМСФО = ОбщегоНазначенияУХ.ПолучитьЗначениеРеквизита(
			СтрокаВНА.ГруппаВНА, "НачислятьАмортизацию");
			
			Если Не СтрокаВНА.НачислятьАмортизациюМСФО 
				И Не СтрокаВНА.СпособНачисленияАмортизацииМСФО.Пустая() Тогда
...
Процедура УправлениеФормойВНА(Форма) Экспорт
	...
	Если Элементы.Найти("ГруппаАмортизация") <> Неопределено Тогда
...
Процедура ОбновитьЗаголовкиГруппВНА(Форма, ЗаголовокОбновления = Неопределено) Экспорт
	...
	Если ЭтоОС И Форма.Элементы.Найти("РемонтныйЦикл") <> Неопределено Тогда
		...
		Если ВозможноОбновитьЗаголовокВНА(Форма, ЗаголовокОбновления, "РемонтныйЦикл") Тогда
	...
	Если ВозможноОбновитьЗаголовокВНА(Форма, ЗаголовокОбновления, "ПравоСобственности") Тогда
	...
	Если ВозможноОбновитьЗаголовокВНА(Форма, ЗаголовокОбновления, "Страхование") Тогда
	...
	Если ВозможноОбновитьЗаголовокВНА(Форма, ЗаголовокОбновления, "ДоговорыЗалога") Тогда
...
Функция ПолучитьВНАИзСтрок(КоллекцияСтрок, КолонкиИсточника = Неопределено) Экспорт
	...
	КолонкиИсточник = Новый Массив;
	ТипыВНА = Неопределено;
	Если КолонкиИсточника <> Неопределено Тогда
		КолонкиИсточник = Новый Структура(КолонкиИсточника);
	...
	Для каждого СтрокаВНА Из КоллекцияСтрок Цикл
		...
		Если ТипыВНА = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ИсточникВНА = Неопределено;
		
		Для каждого ИмяИсточника Из КолонкиИсточник Цикл
			ИсточникВНА = СтрокаВНА[ИмяИсточника.Ключ];
...
Процедура ОбновитьРеквизитыПриИзмененииПараметровАмортизации(ФормаДокумента, СтрокаВНА = Неопределено) Экспорт
	...
	ПолучаемыеРеквизиты = "СчетАмортизации,СчетАмортизацииСубконто1,СчетАмортизацииСубконто2,СчетАмортизацииСубконто3,
	...
	РеквизитыСсылки = ОбщегоНазначенияУХ.ПолучитьЗначенияРеквизитов(ТекущаяСтрока.ПараметрыАмортизацииВНАМСФО, ПолучаемыеРеквизиты);
...
Функция ПоляОбъекта(Объект = Неопределено, ИменаПолей) Экспорт
...
Процедура ЗаполнитьПоля(Приемник, Источник, СоответствиеПолей) Экспорт

	Для каждого Замена Из СоответствиеПолей Цикл
		Приемник[Замена.Ключ] = Источник[Замена.Значение];
	КонецЦикла;

КонецПроцедуры

Функция ИмяВидСубконто(ИмяСубконто) Экспорт
	Возврат "ВидСубконто" + Прав(ИмяСубконто, 1);//ПФ
КонецФункции
...
Процедура УправлениеФормой(Форма, ИмяТЧ = "ВНА", ОбновитьВидимостьПериодаОтчета = Истина, ИспользоватьПериодОтчета = Неопределено) Экспорт
	...
	СоответствиеКолонок = ПолучитьСоответствиеКолонок(Форма);
	...
	Если ЗначениеЗаполнено(КэшируемыеЗначения.ПланСчетовНСБУ) Тогда
		...
		ЭтоТИБ = (ТипБД = ПредопределенноеЗначение("Справочник.ТипыБазДанных.ТекущаяИБ"));
		НаправлениеТрансляции = ОбщегоНазначенияУХ.ЗначениеРеквизитаОбъекта(КэшируемыеЗначения.ШаблонТрансляции, "НаправлениеТрансляции");
		
		НаправленияТУ = Новый Массив;
		НаправленияТУ.Добавить(ПредопределенноеЗначение("Перечисление.НаправленияТрансляцииДанных.РегистрБухгалтерииВРегистрБухгалтерии"));
		НаправленияТУ.Добавить(ПредопределенноеЗначение("Перечисление.НаправленияТрансляцииДанных.РегистрБухгалтерииВПоказатели"));
		
		ИсточникТУ = (НаправленияТУ.Найти(НаправлениеТрансляции) <> Неопределено);
		
	КонецЕсли;
	
	ВидЗаполнения = ВидЗаполненияВНА(Форма);

	// Колонки ТЧ
	Для каждого КолонкаОС Из СоответствиеКолонок Цикл
	...
	// команды формы
	УстановитьВидимость(Элементы, "ФормаСоздатьДокументМСФО", ВидЗаполнения.ЕстьНСБУ И НЕ ВидЗаполнения.ЕстьМСФО);
	
	// группы команд ТЧ ВНА
	УстановитьВидимость(Элементы, "ЗаполнитьМСФО", ВидЗаполнения.ЕстьМСФО);
	УстановитьВидимость(Элементы, "ЗаполнитьНСБУ", ВидЗаполнения.ЕстьНСБУ);
	
	// команды НСБУ
	УстановитьВидимость(Элементы, "НСБУЗаполнитьИзУчетнойСистемы",
		ВидЗаполнения.ЕстьНСБУ И ЭтоТИБ И ИсточникТУ);
	...
	ОбновитьВидимостьДокументыОснования(Форма);
	
	Форма.КэшируемыеЗначения.Вставить("ВидЗаполнения", ВидЗаполнения);
	Форма.КэшируемыеЗначения.Вставить("ИспользоватьДанныеУчетнойСистемы", ЭтоТИБ);
	
	УстановитьПланыСчетовВПараметрахВыбора(Форма, ИмяТЧ, СоответствиеКолонок);
	
	НастроитьПрофильАмортизацииНСБУ(Форма, ИмяТЧ);
	
КонецПроцедуры

Процедура НастроитьПрофильАмортизацииНСБУ(Форма, ИмяТЧ)
	
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоЕХ() Тогда
		Возврат;//в ЕРПУХ выбор из списка документов
	ИначеЕсли Не УчетМСФОПовтИспУХ.ЕстьРСБУ() Тогда
		Возврат;//в ВЕ нет списка выбора
	КонецЕсли;
	
	ПланСчетовНСБУ = Форма.КэшируемыеЗначения.ПланСчетовНСБУ;
	ЭтоРСБУ = УчетМСФОПовтИспУХ.ЭтоРСБУ(ПланСчетовНСБУ);
	
	ИмяЭлемента = ИмяТЧ + "СпособОтраженияРасходовПоАмортизацииНСБУ";
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы, ИмяЭлемента, "ВыбиратьТип", Не ЭтоРСБУ);
		
	ИмяТипа = "СправочникСсылка.СпособыОтраженияРасходовПоАмортизации";
	ОграничениеТипа = Новый ОписаниеТипов(?(ЭтоРСБУ, ИмяТипа, Неопределено));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы, ИмяЭлемента, "ОграничениеТипа", ОграничениеТипа);
...
Процедура ОбновитьВидимостьДокументыОснования(ФормаДокумента) Экспорт
	...
	Если Элементы.Найти("СтраницыДокументыОснования") = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли Объект.ДокументыОснования.Количество() > 1 Тогда
		...
		Для Каждого СтрокаТаблицы Из ФормаДокумента.Объект.ДокументыОснования Цикл
			ФормаДокумента.ТекстДокументыОснования = ФормаДокумента.ТекстДокументыОснования 
				+ ?(ФормаДокумента.ТекстДокументыОснования = "", "", ", " + Символы.ПС) 
...
Функция ПараметрыТаблицыПроводок(ПутьКДаннымТЧ = "Объект.Проводки", ИмяТЧ = "Проводки", ПланСчетов = "МСФО", СтороныПроводок = "Дт, Кт") Экспорт

	Возврат Новый Структура(
		"ПутьКДаннымТЧ, ИмяТЧ, ПланСчетов, СтороныПроводок", 
		ПутьКДаннымТЧ, ИмяТЧ, ПланСчетов, СтороныПроводок);

КонецФункции

Процедура ПриИзмененииСчетаПроводки(Форма, ДтКт = "Дт", ПутьКДаннымТЧ = "Объект.Проводки", ИмяТЧ = "Проводки", СтрокаТаблицы = Неопределено) Экспорт
	...
	ИмяРеквизита = "УчетПоПодразделениям" + ДтКт;
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, ИмяРеквизита + "Доступность") Тогда
		ПоляОбъекта.Вставить("Подразделение", ИмяРеквизита);
	КонецЕсли;
	
	ИмяРеквизита = "УчетПоНаправлениямДеятельности" + ДтКт;
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, ИмяРеквизита + "Доступность") Тогда
		ПоляОбъекта.Вставить("НаправлениеДеятельности", ИмяРеквизита);
	КонецЕсли;
...
Функция ПараметрыПересчета(ИмяФВ = "СуммаВВалютеУчета", ИмяВО = "СуммаВВалютеОтчетности",
	ИмяВД = "СуммаВВалютеДт", ИмяПериод = "Период", ИмяВалюта = "ВалютаДт",
	КорСторона = "ВалютаКт, СуммаВВалютеКт") Экспорт
	
	ДругаяСторона = Новый Структура;
	Если ЗначениеЗаполнено(КорСторона) Тогда
		ЧастиКор = СтрРазделить(КорСторона, ",");
		ДругаяСторона = Новый Структура("Валюта, ВД", ЧастиКор[0], СокрЛП(ЧастиКор[1]));
	КонецЕсли;
	
	Возврат Новый Структура(
		"ФВ,	ВО,		ВД,		Дата,		Валюта,		Кор",
		ИмяФВ,	ИмяВО,	ИмяВД,	ИмяПериод,	ИмяВалюта,	ДругаяСторона);

КонецФункции

Процедура ПересчитатьПроводкуПоСумме(Форма, СтрокаТаб, Знач КодПересчета = "ФВ", Коды = Неопределено) Экспорт 
	
	Если Коды = Неопределено Тогда
		Коды = ПараметрыПересчета();
	КонецЕсли;
	
	КэшируемыеЗначения = Форма.КэшируемыеЗначения;
	
	Если НЕ ЗначениеЗаполнено(КэшируемыеЗначения.Организация) Тогда
		Возврат;
	КонецЕсли;
	
	Если ПоляОбъекта(Форма.Объект, "НезависимыеСуммы").НезависимыеСуммы = Истина Тогда
		Возврат;
	КонецЕсли;
	
	Если Не КэшируемыеЗначения.Свойство("КэшКурсов") Тогда
	...
	Организация		= Форма.КэшируемыеЗначения.Организация;
	
	ЕстьКорВалюта	= ЗначениеЗаполнено(Коды.Кор);
	КорВалюта 		= ?(ЕстьКорВалюта, СтрокаТаб[Коды.Кор.Валюта], Неопределено);
	КодКорВД		= ?(ЕстьКорВалюта, Коды.Кор.ВД, Неопределено);
	
	Дата 			= СтрокаТаб[Коды.Дата];
	Валюта 			= СтрокаТаб[Коды.Валюта];
	Сумма			= СтрокаТаб[Коды[КодПересчета]];
	
	Если КодПересчета = "ФВ" Тогда
		
		Если ЗначениеЗаполнено(Валюта) Тогда
			СтрокаТаб[Коды.ВД] = ВВалюту(Сумма, Валюта, ФВ, Дата, Организация, КэшКурсов);
			КодПересчета = "ВД";
		ИначеЕсли ЗначениеЗаполнено(КорВалюта) Тогда 
			СтрокаТаб[КодКорВД] = ВВалюту(Сумма, КорВалюта, ФВ, Дата, Организация, КэшКурсов);
			КодПересчета = "КорВД";
		Иначе 
			СтрокаТаб[Коды.ВО] = ВВалюту(Сумма, ВО, ФВ, Дата, Организация, КэшКурсов);
		КонецЕсли;
		
	ИначеЕсли КодПересчета = "ВО" Тогда 
		
		Если ЗначениеЗаполнено(Валюта) Тогда
			СтрокаТаб[Коды.ВД] = ВВалюту(Сумма, Валюта, ВО, Дата, Организация, КэшКурсов);
			КодПересчета = "ВД";
		ИначеЕсли ЗначениеЗаполнено(КорВалюта) Тогда 
			СтрокаТаб[КодКорВД] = ВВалюту(Сумма, КорВалюта, ВО, Дата, Организация, КэшКурсов);
			КодПересчета = "КорВД";
		Иначе 
			СтрокаТаб[Коды.ФВ] = ВВалюту(Сумма, ФВ, ВО, Дата, Организация, КэшКурсов);
		КонецЕсли;
		
	КонецЕсли;
	
	Если КодПересчета = "ВД" И ЗначениеЗаполнено(Валюта) Тогда
		
		Сумма = СтрокаТаб[Коды.ВД];
		
		СтрокаТаб[Коды.ФВ]	= ВВалюту(Сумма, ФВ,	Валюта, Дата, Организация, КэшКурсов);
		СтрокаТаб[Коды.ВО] 	= ВВалюту(Сумма, ВО,	Валюта, Дата, Организация, КэшКурсов);
		Если ЕстьКорВалюта Тогда
			СтрокаТаб[КодКорВД]	= ВВалюту(Сумма, КорВалюта, Валюта, Дата, Организация, КэшКурсов);
		КонецЕсли;
		
	ИначеЕсли КодПересчета = "КорВД" И ЗначениеЗаполнено(КорВалюта) Тогда
		
		Сумма = СтрокаТаб[КодКорВД];
	
		СтрокаТаб[Коды.ФВ]	= ВВалюту(Сумма, ФВ, 		КорВалюта, Дата, Организация, КэшКурсов);
		СтрокаТаб[Коды.ВО]	= ВВалюту(Сумма, ВО, 		КорВалюта, Дата, Организация, КэшКурсов);
		СтрокаТаб[Коды.ВД]	= ВВалюту(Сумма, Валюта,	КорВалюта, Дата, Организация, КэшКурсов);
	
	КонецЕсли;
	
КонецПроцедуры

Процедура ПересчитатьСуммыПроводки(Форма, СтрокаТаб, Знач ИсточникПересчета = "СуммаВВалютеДт") Экспорт 
	
	Если НЕ ЗначениеЗаполнено(Форма.Объект.Организация) Тогда
		Возврат;
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма.Объект, "НезависимыеСуммы")
		И Форма.Объект.НезависимыеСуммы = Истина Тогда
		Возврат;
	КонецЕсли;
	
	Если Не Форма.КэшируемыеЗначения.Свойство("КэшКурсов") Тогда
		Форма.КэшируемыеЗначения.Вставить("КэшКурсов", Новый Соответствие);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ИсточникПересчета) Тогда 
		
		Если СтрНайти(Форма.ИмяФормы,"КорректировкиЗначенийПоказателей")>0 Тогда 
			
			Если ЗначениеЗаполнено(СтрокаТаб.ВалютаКт) И (СтрокаТаб.ЗначениеВалютаКт <> 0) Тогда
				ИсточникПересчета = "ЗначениеВалютаКт";
			ИначеЕсли ЗначениеЗаполнено(СтрокаТаб.ВалютаДт) И (СтрокаТаб.ЗначениеВалютаДт <> 0) Тогда
				ИсточникПересчета = "ЗначениеВалютаДт";
			Иначе
				ИсточникПересчета = "Значение";
			КонецЕсли;
			
		Иначе
			
			Если ЗначениеЗаполнено(СтрокаТаб.ВалютаКт) И (СтрокаТаб.СуммаВВалютеКт <> 0) Тогда
				ИсточникПересчета = "СуммаВВалютеКт";
			ИначеЕсли ЗначениеЗаполнено(СтрокаТаб.ВалютаДт) И (СтрокаТаб.СуммаВВалютеДт <> 0) Тогда
	...
	СуммаИсточник	= СтрокаТаб[ИсточникПересчета];
	Дата 			= СтрокаТаб.Период;
	ФВ				= Форма.КэшируемыеЗначения.ФункциональнаяВалюта;
	ВО 				= Форма.КэшируемыеЗначения.ВалютаПредоставленияОтчетности;
	КэшКурсов		= Форма.КэшируемыеЗначения.КэшКурсов;
	...
	Если ИсточникПересчета = "СуммаВВалютеУчета" Тогда 
		...
		Если ЗначениеЗаполнено(СтрокаТаб.ВалютаДт) Тогда
			СтрокаТаб.СуммаВВалютеДт = ВВалюту(СуммаИсточник, СтрокаТаб.ВалютаДт, ФВ, Дата, Организация, КэшКурсов);
			ИсточникПересчета = "СуммаВВалютеДт";
		ИначеЕсли ЗначениеЗаполнено(СтрокаТаб.ВалютаКт) Тогда 
			СтрокаТаб.СуммаВВалютеКт = ВВалюту(СуммаИсточник, СтрокаТаб.ВалютаКт, ФВ, Дата, Организация, КэшКурсов);
			ИсточникПересчета = "СуммаВВалютеКт";
		Иначе 
			СтрокаТаб.СуммаВВалютеОтчетности = ВВалюту(СуммаИсточник, ВО, ФВ, Дата, Организация, КэшКурсов);
		КонецЕсли;
		
	ИначеЕсли ИсточникПересчета = "СуммаВВалютеОтчетности" Тогда 
		
		Если ЗначениеЗаполнено(СтрокаТаб.ВалютаДт) Тогда
			СтрокаТаб.СуммаВВалютеДт = ВВалюту(СуммаИсточник, СтрокаТаб.ВалютаДт, ВО, Дата, Организация, КэшКурсов);
			ИсточникПересчета = "СуммаВВалютеДт";
		ИначеЕсли ЗначениеЗаполнено(СтрокаТаб.ВалютаКт) Тогда 
			СтрокаТаб.СуммаВВалютеКт = ВВалюту(СуммаИсточник, СтрокаТаб.ВалютаКт, ВО, Дата, Организация, КэшКурсов);
			ИсточникПересчета = "СуммаВВалютеКт";
		Иначе 
			СтрокаТаб.СуммаВВалютеУчета = ВВалюту(СуммаИсточник, ФВ, ВО, Дата, Организация, КэшКурсов);
		КонецЕсли;
		
	ИначеЕсли ИсточникПересчета = "Значение" Тогда 
		
		Если ЗначениеЗаполнено(СтрокаТаб.ВалютаДт) Тогда
			СтрокаТаб.ЗначениеВалютаДт = ВВалюту(СуммаИсточник, СтрокаТаб.ВалютаДт, ФВ, Дата, Организация, КэшКурсов);
			ИсточникПересчета = "ЗначениеВалютаДт";
		ИначеЕсли ЗначениеЗаполнено(СтрокаТаб.ВалютаКт) Тогда 
			СтрокаТаб.ЗначениеВалютаКт = ВВалюту(СуммаИсточник, СтрокаТаб.ВалютаКт, ФВ, Дата, Организация, КэшКурсов);
			ИсточникПересчета = "ЗначениеВалютаКт";
		Иначе 
			СтрокаТаб.ПредставлениеЗначения = ВВалюту(СуммаИсточник, ВО, ФВ, Дата, Организация, КэшКурсов);
		КонецЕсли;
		
	ИначеЕсли ИсточникПересчета = "ПредставлениеЗначения" Тогда 
		
		Если ЗначениеЗаполнено(СтрокаТаб.ВалютаДт) Тогда
			СтрокаТаб.ЗначениеВалютаДт = ВВалюту(СуммаИсточник, СтрокаТаб.ВалютаДт, ВО, Дата, Организация, КэшКурсов);
			ИсточникПересчета = "ЗначениеВалютаДт";
		ИначеЕсли ЗначениеЗаполнено(СтрокаТаб.ВалютаКт) Тогда 
			СтрокаТаб.ЗначениеВалютаКт = ВВалюту(СуммаИсточник, СтрокаТаб.ВалютаКт, ВО, Дата, Организация, КэшКурсов);
			ИсточникПересчета = "ЗначениеВалютаКт";
		Иначе 
			СтрокаТаб.Значение = ВВалюту(СуммаИсточник, ФВ, ВО, Дата, Организация, КэшКурсов);
		КонецЕсли;
	...
	Если ИсточникПересчета = "СуммаВВалютеКт" И ЗначениеЗаполнено(СтрокаТаб.ВалютаКт) Тогда
		
		СтрокаТаб.СуммаВВалютеУчета 		= ВВалюту(СуммаИсточник, ФВ, 				СтрокаТаб.ВалютаКт, Дата, Организация, КэшКурсов);
		СтрокаТаб.СуммаВВалютеОтчетности 	= ВВалюту(СуммаИсточник, ВО, 				СтрокаТаб.ВалютаКт, Дата, Организация, КэшКурсов);
		СтрокаТаб.СуммаВВалютеДт 			= ВВалюту(СуммаИсточник, СтрокаТаб.ВалютаДт, СтрокаТаб.ВалютаКт, Дата, Организация, КэшКурсов);
		
	ИначеЕсли ИсточникПересчета = "СуммаВВалютеДт" И ЗначениеЗаполнено(СтрокаТаб.ВалютаДт) Тогда
		
		СтрокаТаб.СуммаВВалютеУчета 		= ВВалюту(СуммаИсточник, ФВ, 				СтрокаТаб.ВалютаДт, Дата, Организация, КэшКурсов);
		СтрокаТаб.СуммаВВалютеОтчетности 	= ВВалюту(СуммаИсточник, ВО, 				СтрокаТаб.ВалютаДт, Дата, Организация, КэшКурсов);
		СтрокаТаб.СуммаВВалютеКт 			= ВВалюту(СуммаИсточник, СтрокаТаб.ВалютаКт, СтрокаТаб.ВалютаДт, Дата, Организация, КэшКурсов);
		
	ИначеЕсли ИсточникПересчета = "ЗначениеВалютаДт" И ЗначениеЗаполнено(СтрокаТаб.ВалютаДт) Тогда
		
		СтрокаТаб.Значение			 		= ВВалюту(СуммаИсточник, ФВ, 				СтрокаТаб.ВалютаДт, Дата, Организация, КэшКурсов);
		СтрокаТаб.ПредставлениеЗначения 	= ВВалюту(СуммаИсточник, ВО, 				СтрокаТаб.ВалютаДт, Дата, Организация, КэшКурсов);
		СтрокаТаб.ЗначениеВалютаКт 			= ВВалюту(СуммаИсточник, СтрокаТаб.ВалютаКт, СтрокаТаб.ВалютаДт, Дата, Организация, КэшКурсов);
		
	ИначеЕсли ИсточникПересчета = "ЗначениеВалютаКт" И ЗначениеЗаполнено(СтрокаТаб.ВалютаКт) Тогда
		
		СтрокаТаб.Значение 					= ВВалюту(СуммаИсточник, ФВ, 				СтрокаТаб.ВалютаКт, Дата, Организация, КэшКурсов);
		СтрокаТаб.ПредставлениеЗначения 	= ВВалюту(СуммаИсточник, ВО, 				СтрокаТаб.ВалютаКт, Дата, Организация, КэшКурсов);
		СтрокаТаб.ЗначениеВалютаДт 			= ВВалюту(СуммаИсточник, СтрокаТаб.ВалютаДт, СтрокаТаб.ВалютаКт, Дата, Организация, КэшКурсов);
...
Функция ВВалюту(Сумма, ВалютаНовая, ВалютаТекущая, ДатаКурса, Организация = Неопределено, КэшКурсов = Неопределено) Экспорт
	
	Если Не ЗначениеЗаполнено(ВалютаНовая) Тогда
		Возврат 0;
	КонецЕсли;
	...
	Если ВалютаНовая = ВалютаТекущая Тогда
		Возврат Сумма;
	КонецЕсли;
	
	КурсыВалют = КроссКурс(ВалютаТекущая, ВалютаНовая, ДатаКурса, Организация, КэшКурсов);
	КурсИсточник = Новый Структура("Валюта, Курс, Кратность", ВалютаТекущая, 1, 1);
	
	Возврат РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(Сумма, КурсИсточник, КурсыВалют);

КонецФункции

Функция КроссКурс(ВалютаТекущая, ВалютаНовая, ДатаКурса, Организация = Неопределено, КэшКурсов = Неопределено) Экспорт 
	
	Если КэшКурсов <> Неопределено Тогда
		
		Кэш1 = КэшКурсов.Получить(ДатаКурса);
		Если Кэш1 = Неопределено Тогда
			
			Кэш3 = НовоеСоответствие(ВалютаНовая, Неопределено);
			КэшКурсов.Вставить(ДатаКурса, НовоеСоответствие(ВалютаТекущая, Кэш3));
			
		Иначе
			
			Кэш2 = Кэш1.Получить(ВалютаТекущая);
			Если Кэш2 = Неопределено Тогда
				Кэш1.Вставить(ВалютаТекущая, НовоеСоответствие(ВалютаНовая, Неопределено));
			Иначе 	
				
				Курс = Кэш2.Получить(ВалютаНовая);
	...
	КроссКурс = МСФОВызовСервераУХ.КроссКурс(ДатаКурса, ВалютаТекущая, ВалютаНовая, Организация);
	КроссКурс.Вставить("Валюта", ВалютаНовая);
	
	Если КэшКурсов <> Неопределено Тогда
		КэшКурсов[ДатаКурса][ВалютаТекущая].Вставить(ВалютаНовая, КроссКурс);
	КонецЕсли;
...
Процедура ЗаполнитьНадписиВПроводке(Проводка, ЕстьНадписиНУ = Ложь, СтороныПроводки = "Дт,Кт") Экспорт
	
	Стороны = СтрРазделить(СтороныПроводки, ",");

	Для каждого Сторона Из Стороны Цикл
		Доступность = Проводка["Количественный" + Сторона + "Доступность"];
		Проводка["НадписьКоличество" + Сторона] = ?(Доступность, НСтр("ru = 'Кол.:'"), "");
	КонецЦикла;
...
Функция СтруктураДанныхПроводки(ТекущиеДанные, Форма) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
		Форма.КэшируемыеЗначения, "ЗаменыКолонокПроводок")  Тогда
		
		Для каждого ИмяКолонки Из Форма.КэшируемыеЗначения.ЗаменыКолонокПроводок Цикл
			СтруктураПроводки.Вставить(ИмяКолонки.Ключ, ТекущиеДанные[ИмяКолонки.Значение]);
		КонецЦикла;
...
Функция ПолучитьСписокПараметров(Форма, ТекущиеДанные, ШаблонИмяПоляОбъекта, ИмяПоляСчетУчета) Экспорт
	...
	ТипСклады = Неопределено;
	Если УчетМСФОПовтИспУХ.ЕстьРСБУ() Тогда
		ИмяТипаРСБУ = "СправочникСсылка.Склады";
	    ТипСклады = Тип(ИмяТипаРСБУ);
	КонецЕсли;
	
	ТипыДоговора = ОбщегоНазначенияКлиентСерверУХ.ПолучитьОписаниеТиповДоговора().Типы();
	...
	Для Индекс = 1 По 3 Цикл
		...
		ИначеЕсли ТипПоля = ТипСклады Тогда
...
Функция ЗависимыеТипы()

	ЗависимыеТипы = Новый Соответствие;
	
	ЭтоЕРПУХ = ИдентификацияПродуктаУХКлиентСервер.ЭтоЕХ();
	
	ИмяТипаНГ = "СправочникСсылка.НоменклатурныеГруппы";
	Если ЭтоЕРПУХ Тогда
		ИмяТипаНГ = "СправочникСсылка.ГруппыФинансовогоУчетаНоменклатуры";
	КонецЕсли;
	
	ТекущиеТипы = Новый Массив;
	ТекущиеТипы.Добавить(Тип(ИмяТипаНГ));
	ТекущиеТипы.Добавить(Тип("СправочникСсылка.СтавкиНалоговМСФО"));
	Если УчетМСФОПовтИспУХ.ЕстьРСБУ() Тогда
		ТекущиеТипы.Добавить(Тип(Строка("ПеречислениеСсылка.СтавкиНДС")));
	КонецЕсли;
	Если ЭтоЕРПУХ Тогда
		ТекущиеТипы.Добавить(Тип(Строка("СправочникСсылка.СтавкиНДС")));
	КонецЕсли;
	
	ЗависимыеТипы.Вставить(Тип("СправочникСсылка.Номенклатура"), ТекущиеТипы);
...
Функция ПолучитьДействия(ДействияИсточник, ЗаполнитьЗависимые = Ложь)

	Действия = ?(ТипЗнч(ДействияИсточник) = Тип("Строка"), 
		Новый Структура(ДействияИсточник), ДействияИсточник);
	
	Если Не ЗаполнитьЗависимые Тогда
		Возврат Действия;
	КонецЕсли;

	Если Действия.Свойство("ЗаполнитьУП") Тогда
		Действия.Вставить("ЗаполнитьПОУ");//меняется план счетов
		Действия.Вставить("ЗаполнитьКурс");//меняется ФВ
	КонецЕсли;
	
	Если Действия.Свойство("ЗаполнитьДоговор") Тогда
		Действия.Вставить("ЗаполнитьВалюту");
		Действия.Вставить("ЗаполнитьТипЦен");
	КонецЕсли;
	
	Если Действия.Свойство("ЗаполнитьТипЦен") Тогда //Считаем что есть только у ПТУ, РТУ(но не у ПП)
		Действия.Вставить("ЗаполнитьДКЗ");
	КонецЕсли;
	
	Если Действия.Свойство("ЗаполнитьВалюту") Тогда
		Действия.Вставить("ЗаполнитьКурс");
	КонецЕсли;
...
Процедура ЗаполнитьБанковскийСчет(Форма, СчетОрганизации = Истина)
	...
	Если СчетОрганизации Тогда
		...
		Валюта = ФВ;
	Иначе 
		ВладелецСчета = Объект.Контрагент; 
		Валюта = Объект.ВалютаДокумента;
	КонецЕсли;
	
	НовыйСчет = Неопределено;
	МСФОВызовСервераУХ.УстановитьБанковскийСчет(НовыйСчет, ВладелецСчета, Валюта, ФВ = Валюта);
	...
	Если СчетОрганизации Тогда
		Объект.СчетОрганизации = НовыйСчет;
	Иначе 
		Объект.СчетКонтрагента = НовыйСчет;
	КонецЕсли;
	
КонецПроцедуры

Процедура ОбновитьПараметрыВыбораСтрана(Форма) Экспорт
	
	Элементы 			= Форма.Элементы;
	Объект 				= Форма.Объект;
	КэшируемыеЗначения 	= Форма.КэшируемыеЗначения;
	
	ПолеСтраны = Неопределено;
	Если Элементы.Найти("ПозицииСтавкаНДС") <> Неопределено Тогда
		ПолеСтраны = Элементы.ПозицииСтавкаНДС;
	ИначеЕсли Элементы.Найти("РасшифровкаПлатежаСтавкаНДС") <> Неопределено Тогда
		ПолеСтраны = Элементы.РасшифровкаПлатежаСтавкаНДС;
	Иначе 
		Возврат;
	КонецЕсли;
	
	Страна = ОбщегоНазначенияУХ.ЗначениеРеквизитаОбъекта(Объект.Организация, "СтранаРегистрации");
	...
	Если Страна = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(КэшируемыеЗначения, "СтранаРегистрации") Тогда
		Возврат;
	КонецЕсли;
	
	КэшируемыеЗначения.Вставить("СтранаРегистрации", Страна);
	ПолеСтраны.ПараметрыВыбора = МСФОКлиентСерверУХ.ПолучитьПараметрыВыбора(Новый Структура("Страна", Страна));

КонецПроцедуры

Процедура ОбновитьВалютуКурс(Форма, ОбновитьВалюту = Истина, ОбновитьКурс = Истина)
   	
	Объект = Форма.Объект;
	ФВ = Форма.КэшируемыеЗначения.ФункциональнаяВалюта;	
	
	Если ОбновитьВалюту Тогда
		
		ВалютаДокумента = Неопределено;
		Если Объект.Свойство("ДоговорКонтрагента") И ЗначениеЗаполнено(Объект.ДоговорКонтрагента) Тогда
			ВалютаДокумента = ОбщегоНазначенияУХ.ЗначениеРеквизитаОбъекта(
					Объект.ДоговорКонтрагента, "ВалютаВзаиморасчетов");
		ИначеЕсли Объект.Свойство("СчетОрганизации") И ЗначениеЗаполнено(Объект.СчетОрганизации) Тогда
			ВалютаДокумента = ОбщегоНазначенияУХ.ЗначениеРеквизитаОбъекта(
					Объект.СчетОрганизации, "ВалютаДенежныхСредств");
		КонецЕсли;
	...
	Если ОбновитьКурс Тогда
		...
		Если Объект.ВалютаДокумента = ФВ Тогда
			КроссКурс = Новый Структура("Курс, Кратность", 1, 1);
		Иначе 
			КроссКурс = МСФОВызовСервераУХ.КроссКурс(Объект.Дата, ФВ, Объект.ВалютаДокумента, Объект.Организация);
		КонецЕсли;
		
		Объект.КурсВзаиморасчетов = КроссКурс.Курс;
		Объект.КратностьВзаиморасчетов = КроссКурс.Кратность;

	КонецЕсли;

КонецПроцедуры

Процедура ЗаполнитьДКЗ(Форма, Действия)

	Объект = Форма.Объект;
	
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
			Объект, "ПараметрыУчетаВзаиморасчетов") Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыУчетаВзаиморасчетов = МСФОВызовСервераУХ.ПолучитьПараметрыУчета(
				Объект.ДоговорКонтрагента, 
				Форма.КэшируемыеЗначения.ПланСчетовМСФО);
				
	Если Не ЗначениеЗаполнено(ПараметрыУчетаВзаиморасчетов) Тогда
	...
	Если Не ЗначениеЗаполнено(ПараметрыУчетаВзаиморасчетов)
		И Не ЗначениеЗаполнено(Объект.ПараметрыУчетаВзаиморасчетов) Тогда
		
		ВидДКЗ = ПредопределенноеЗначение("Справочник.ВидыОбъектовУчетаМСФО.КЗ");
		Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.РеализацияМСФО") Тогда
			ВидДКЗ = ПредопределенноеЗначение("Справочник.ВидыОбъектовУчетаМСФО.ДЗ");
		КонецЕсли;
		
		ПараметрыУчетаВзаиморасчетов = МСФОВызовСервераУХ.ПараметрыОбъектаУчетаПоУмолчанию(
			Форма.КэшируемыеЗначения.ПланСчетовМСФО,
			ВидДКЗ);
	
	КонецЕсли;
			
	Если ЗначениеЗаполнено(ПараметрыУчетаВзаиморасчетов) Тогда
		Объект.ПараметрыУчетаВзаиморасчетов = ПараметрыУчетаВзаиморасчетов;
	КонецЕсли;

КонецПроцедуры

Процедура ОбновитьПОУ(Форма)
	
	ЭтоОплатаМСФО = (ТипЗнч(Форма.Объект.Ссылка) = Тип("ДокументСсылка.ОплатаМСФО"));
	
	РазделыАктивов = "ВнеоборотныеАктивы, Запасы, НезавершенныеОперации, НезавершенноеСтроительство,
					|ПрочиеАктивыОбязательства, КоммерческиеАдминистративныеРасходы,
					|ВнереализационныеДоходыИРасходы, Себестоимость";
		
	РазделыЭлементов = Новый СписокЗначений;//Имя элемента формы с разделами плана счетов
	РазделыЭлементов.Добавить("ПараметрыУчетаВзаиморасчетов", 					"Взаиморасчеты");
	РазделыЭлементов.Добавить("ПозицииПараметрыУчета", 							РазделыАктивов);
	РазделыЭлементов.Добавить("ПозицииПараметрыАмортизации", 					"ВнеоборотныеАктивы");
	РазделыЭлементов.Добавить("ПараметрыУчетаДС", 								"ДенежныеСредства");
	РазделыЭлементов.Добавить("РасшифровкаПлатежаПараметрыУчетаВзаиморасчетов",	"Взаиморасчеты");
	
	ПараметрыВыбора = Новый Структура("ПланСчетовБД, РазделПланаСчетов", 
						Форма.КэшируемыеЗначения.ПланСчетовМСФО);
	
	Для каждого РазделыДляЭлемента Из РазделыЭлементов Цикл
		
		Элемент = Форма.Элементы.Найти(РазделыДляЭлемента.Значение);
		Если Элемент = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		РазделПС = Новый Массив;
		ИменаРазделов = Новый Структура(РазделыДляЭлемента.Представление);
		Для каждого ТекущийРаздел Из ИменаРазделов Цикл
			РазделПС.Добавить(ПредопределенноеЗначение("Справочник.РазделыПланаСчетов." + ТекущийРаздел.Ключ));
		КонецЦикла;
		
		ПараметрыВыбора.Вставить("РазделПланаСчетов", Новый ФиксированныйМассив(РазделПС));
		Если Не ЭтоОплатаМСФО И РазделыДляЭлемента.Представление = "Взаиморасчеты" Тогда
			ПараметрыВыбора.Вставить("Авансы", Ложь);//в документах(кроме оплат) авансы не выбираются
		КонецЕсли;
		Элемент.ПараметрыВыбора = ПолучитьПараметрыВыбора(ПараметрыВыбора);
		
	КонецЦикла;
	
КонецПроцедуры

Процедура ОбновитьКэшОперации(Форма)

	СтарыйВидОперации = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Форма.КэшируемыеЗначения, "ВидОперации");
	Если СтарыйВидОперации = Форма.Объект.ВидОперации Тогда
		Возврат;
	КонецЕсли;
	
	КэшОперации = МСФОВызовСервераУХ.ПараметрыВидаОперации(Форма.Объект.ВидОперации);
	
	Форма.КэшируемыеЗначения.Вставить("КэшОперации", КэшОперации);
	Форма.КэшируемыеЗначения.Вставить("ВидОперации", Форма.Объект.ВидОперации);
...
//
Процедура НастроитьПоляСчетаФактуры(Кнопка, Ссылка, Надпись, ДокументБезНДС, ТребуетсяСчетФактура, СчетФактура)
...
Функция ДанныеОСчетеФактуре(Знач ИсходныеДанные, ВидСчетаФактуры, ТребуетсяСФ, СтруктураОтбора = Неопределено)
	...
	ИначеЕсли ЗначениеЗаполнено(ИсходныеДанные) Тогда
		...
		РеквизитыСФ = МСФОВызовСервераУХ.ОписаниеСчетФактуры(ИсходныеДанные, СтруктураОтбора);
...
Функция ВозможноОбновитьЗаголовокВНА(Форма, ЗаголовокОбновления, ИмяЭлемента)

	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "Заголовок" + ИмяЭлемента) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат (ЗаголовокОбновления = Неопределено) Или (ЗаголовокОбновления = ИмяЭлемента);
...
Функция ВидЗаполненияВНА(Форма)

	КэшируемыеЗначения = Форма.КэшируемыеЗначения;
	
	КонтекстЗаполнения = Новый Структура("РежимЗаполнения");
	ЗаполнитьЗначенияСвойств(КонтекстЗаполнения, Форма.Объект);
	
	РежимЗаполнения = КонтекстЗаполнения.РежимЗаполнения;
	Если Не ЗначениеЗаполнено(РежимЗаполнения) Тогда
		РежимЗаполнения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
							КэшируемыеЗначения, "РежимЗаполнения");
	КонецЕсли;
	
	Возврат УчетМСФОПовтИспУХ.ПолучитьВидыУчетаРежимаЗаполнения(РежимЗаполнения);

КонецФункции

Процедура УстановитьПланыСчетовВПараметрахВыбора(Форма, ИмяТЧ, СоответствиеКолонок)
	
	КэшируемыеЗначения = Форма.КэшируемыеЗначения;
	Если Не КэшируемыеЗначения.Свойство("ИменаСубконто") Тогда
		Возврат;
	КонецЕсли;  
	
	ЗаданПланСчетовНСБУ = ЗначениеЗаполнено(КэшируемыеЗначения.ПланСчетовНСБУ);
	
	ПараметрыВыбораНСБУ = ПолучитьПараметрыВыбора(Новый Структура("Владелец", КэшируемыеЗначения.ПланСчетовНСБУ));
	ПараметрыВыбораМСФО = ПолучитьПараметрыВыбора(Новый Структура("Владелец", КэшируемыеЗначения.ПланСчетовМСФО));
	...
	Для Каждого Соответствие Из СоответствиеКолонок Цикл
		...
		Если (Лев(Соответствие.Ключ,4) = "Счет" ИЛИ (Лев(Соответствие.Ключ,8) = "КоррСчет") 
			...
			Если ЗаданПланСчетовНСБУ И Соответствие.Значение.Свойство("НСБУ") Тогда
				Форма.Элементы[ИмяТЧ + Соответствие.Ключ + "НСБУ"].ПараметрыВыбора = ПараметрыВыбораНСБУ;
			КонецЕсли;
			
			Если Соответствие.Значение.Свойство("МСФО") Тогда
				Форма.Элементы[ИмяТЧ + Соответствие.Ключ + "МСФО"].ПараметрыВыбора = ПараметрыВыбораМСФО;
			КонецЕсли;
	...
	ИдентификаторТекущейСтроки = Форма.Элементы[ИмяТЧ].ТекущаяСтрока;
	Если ИдентификаторТекущейСтроки <> Неопределено Тогда
		СтрокаТЧ = Форма.Объект[ИмяТЧ].НайтиПоИдентификатору(ИдентификаторТекущейСтроки);
		ОбновитьВидимостьСубконтоГруппыПодробно(Форма, СтрокаТЧ, ИмяТЧ);
	КонецЕсли;
