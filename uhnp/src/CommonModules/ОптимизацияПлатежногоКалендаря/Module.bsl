...
Процедура ОтменитьОптимизациюКалендаря(ТекущийПользователь) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(КлючЖурналаРегистрации(), 
...
Функция ТекущееСостояниеКалендаря(ПараметрыОтчета) Экспорт
	...
	ТаблицаОстатков.Индексы.Добавить("БанковскийСчетКасса");
	ТаблицаОстатков.Индексы.Добавить("БанковскийСчетКасса, Дата");
	...
	ТаблицаЗаявок.Индексы.Добавить("БанковскийСчетКасса");
	...
	ТаблицаДоговорыОвердрафта.Индексы.Добавить("БанковскийСчетКасса");
...
Процедура ВыполнитьОптимизациюПлатежногоКалендаря(СостояниеКалендаря, НастройкиОптимизации) Экспорт	
	...
	ТаблицаОстатков.Сортировать("БанковскийСчетКасса, Дата Возр", Новый СравнениеЗначений); // сравнение по уид
...
Процедура СохранитьДоступныеВариантыОптимизации(ТаблицаВариантов) Экспорт
...
Функция ТекстЗапросаСостояниеКалендаря_Остатки()
	...
	|		И &УсловиеПредставлениеПлатежногоКалендаряСчетЗакрыт
	...
	|		ПО ВТ_ОборотыСгруппированные.БанковскийСчетКасса = Остатки.БанковскийСчетКасса
	...
	|			И ВТ_ОборотыСгруппированные.СтатьяДвиженияДенежныхСредств = Остатки.СтатьяДвиженияДенежныхСредств
	|ГДЕ
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&УсловиеПредставлениеПлатежногоКалендаряСчетЗакрыт"
											,Отчеты.ПлатежныйКалендарьУХ.ТекстУсловияСчетЗакрытДляЗапроса());
...
Функция ТекстЗапросаСостояниеКалендаря_Заявки()
	...
	|	ВЫБОР
	|		КОГДА ПредставлениеПлатежногоКалендаря.БанковскийСчетКасса = НЕОПРЕДЕЛЕНО
	|				ИЛИ ПредставлениеПлатежногоКалендаря.БанковскийСчетКасса = ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка)
	|				ИЛИ ПредставлениеПлатежногоКалендаря.БанковскийСчетКасса = ЗНАЧЕНИЕ(Справочник.Кассы.ПустаяСсылка)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК БанковскийСчетКассаПустой
	|ПОМЕСТИТЬ ВТ_Заявки
	|ИЗ
	|	РегистрСведений.ПредставлениеПлатежногоКалендаря КАК ПредставлениеПлатежногоКалендаря
...
Функция ТекстЗапросаСостояниеКалендаря_ЛимитыВнутригрупповыхПеремещений()
	...
	|	ЗаявкаНаРасходованиеДенежныхСредств.ДоговорКонтрагента,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ВТ_Объединение.Организация КАК ОрганизацияСписания,
	|	ВТ_Объединение.Контрагент.ОрганизационнаяЕдиница КАК ОрганизацияЗачисления,
	|	ВТ_Объединение.Контрагент КАК КонтрагентЗачисления,
	|	ВТ_Объединение.ДоговорКонтрагента КАК ДоговорКонтрагентаЗачисления,
	|	ВТ_Объединение.Валюта КАК Валюта,
	...
	|	ВТ_Объединение.ДоговорКонтрагента,
	|	ВТ_Объединение.Валюта,
	|	ВТ_Объединение.Контрагент.ОрганизационнаяЕдиница
	|
	|ИМЕЮЩИЕ
	|	СУММА(ВТ_Объединение.Сумма) > 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ЛимитыРамочныхДоговоровКредитовЗаймовОбороты.Организация,
	|	ЛимитыРамочныхДоговоровКредитовЗаймовОбороты.Контрагент.ОрганизационнаяЕдиница,
	|	ЛимитыРамочныхДоговоровКредитовЗаймовОбороты.Контрагент,
	|	ЛимитыРамочныхДоговоровКредитовЗаймовОбороты.РамочныйДоговор,
	|	ЛимитыРамочныхДоговоровКредитовЗаймовОбороты.Валюта,
	|	СУММА(ЛимитыРамочныхДоговоровКредитовЗаймовОбороты.СуммаЛимитаОборот - ЛимитыРамочныхДоговоровКредитовЗаймовОбороты.ИсполнениеГрафикОборот)
	|ИЗ
	|	РегистрНакопления.ЛимитыРамочныхДоговоровКредитовЗаймов.Обороты КАК ЛимитыРамочныхДоговоровКредитовЗаймовОбороты
	|ГДЕ
	|	ЛимитыРамочныхДоговоровКредитовЗаймовОбороты.Контрагент.ОрганизационнаяЕдиница <> &ПустаяОрганизация
	|
	|СГРУППИРОВАТЬ ПО
	|	ЛимитыРамочныхДоговоровКредитовЗаймовОбороты.Организация,
	|	ЛимитыРамочныхДоговоровКредитовЗаймовОбороты.Контрагент,
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(ЛимитыРамочныхДоговоровКредитовЗаймовОбороты.СуммаЛимитаОборот - ЛимитыРамочныхДоговоровКредитовЗаймовОбороты.ИсполнениеГрафикОборот) > 0";
...
Функция ТекстЗапросаСостояниеКалендаря_ДоговорыОвердрафта()
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ВЫРАЗИТЬ(ВерсииРасчетовСрезПоследних.ПредметГрафика КАК Справочник.ДоговорыКредитовИДепозитов).БанковскийСчет КАК БанковскийСчетКасса,
...
Функция ТекстЗапросаСостояниеКалендаря_ДоговорыКредита()
	...
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).ВидСоглашения = ЗНАЧЕНИЕ(Перечисление.ВидыСоглашений.Спецификация)
	|			ТОГДА ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).БазовыйДоговор
	|		ИНАЧЕ ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).ДоговорКонтрагента
	|	КОНЕЦ КАК БазовыйДоговор,
	|	ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит) КАК ВерсияСоглашения,
	|	ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).ВозобновляемыйЛимит КАК ВозобновляемыйЛимит,
	|	ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).Сумма КАК Сумма,
	|	ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).МинимальныйСрок КАК МинимальныйСрок,
	|	ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).МаксимальныйСрок КАК МаксимальныйСрок,
	|	ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).МинимальнаяСуммаСделки КАК МинимальнаяСуммаСделки,
	|	ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).МаксимальнаяСуммаСделки КАК МаксимальнаяСуммаСделки,
	|	ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).ПроцентнаяСтавка КАК ПроцентнаяСтавка,
	|	ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).ДатаНачалаДоступности КАК ДатаНачалаДоступности,
	|	ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).ДатаОкончанияДоступности КАК ДатаОкончанияДоступности,
	|	ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).ВидСоглашения КАК ВидСоглашения,
	|	ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).ФиксированныйСчетОрганизации КАК ФиксированныйСчетОрганизации,
	|	ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).БанковскийСчет КАК БанковскийСчетКасса,
	...
	|		ПО ВерсииГрафиковРасчетовСрезПоследних.ПредметГрафика = ДоговорыКонтрагентов.Ссылка
	|			И (ДоговорыКонтрагентов.ВидДоговораУХ = ЗНАЧЕНИЕ(Справочник.ВидыДоговоровКонтрагентовУХ.Кредит))
	|			И (ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).РазрешеноПланированиеТраншейИзПлатежногоКалендаря = ИСТИНА)
	|ГДЕ
	|	ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).ДатаНачалаДоступности <= &ГоризонтОперативногоПланирования
	|	И ВЫРАЗИТЬ(ВерсииГрафиковРасчетовСрезПоследних.Регистратор КАК Документ.ВерсияСоглашенияКредит).ДатаОкончанияДоступности >= &НачалоПериодаОтбора
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	БазовыйДоговор
	|;
...
Функция ТекстЗапросаСостояниеКалендаря_Выборка()
	...
	|	ВТ_ДоговораКредитов.БанковскийСчетКасса КАК БанковскийСчетКасса,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ДоговорыОвердрафта.БанковскийСчетКасса КАК БанковскийСчетКасса,
	...
	|	ПРЕДСТАВЛЕНИЕ(Заявки.БанковскийСчетКасса) КАК БанковскийСчетКассаПредставление,
	...
	|	Заявки.БанковскийСчетКассаПустой КАК БанковскийСчетКассаПустой,
	|	Заявки.ВидДенежныхСредств КАК ВидДенежныхСредств,
	|	Заявки.СтатьяДвиженияДенежныхСредств КАК СтатьяДвиженияДенежныхСредств
	|ИЗ
	...
	|	ВЫБОР
	|		КОГДА ВТ_ДанныеКалендаря.БанковскийСчетКасса ССЫЛКА Справочник.Кассы
	|			ТОГДА 0
	|		ИНАЧЕ ЕСТЬNULL(ВТ_ДанныеКалендаря.БанковскийСчетКасса.СрокИсполненияПлатежа, 0)
	|	КОНЕЦ КАК СрокИсполненияПлатежаДней,
	|	ПРЕДСТАВЛЕНИЕ(ВТ_ДанныеКалендаря.Организация) КАК ОрганизацияПредставление,
	|	ПРЕДСТАВЛЕНИЕ(ВТ_ДанныеКалендаря.БанковскийСчетКасса) КАК БанковскийСчетКассаПредставление,
	...
	|	ВТ_ДанныеКалендаря КАК ВТ_ДанныеКалендаря
	|ГДЕ
	|	ВТ_ДанныеКалендаря.БанковскийСчетКасса <> ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка)
	|	И ВТ_ДанныеКалендаря.БанковскийСчетКасса ССЫЛКА Справочник.БанковскиеСчетаОрганизаций
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		ИНАЧЕ ЕСТЬNULL(ВТ_ДанныеКалендаря.БанковскийСчетКасса.СрокИсполненияПлатежа, 0)
	|	КОНЕЦ,
	|	ПРЕДСТАВЛЕНИЕ(ВТ_ДанныеКалендаря.Организация),
	|	ПРЕДСТАВЛЕНИЕ(ВТ_ДанныеКалендаря.БанковскийСчетКасса),
	|	ПРЕДСТАВЛЕНИЕ(ВТ_ДанныеКалендаря.Валюта),
	|	ЛОЖЬ,
	|	ВЫБОР
	|		КОГДА &УсловиеОтбораОстатков
	|			ТОГДА ИСТИНА
	...
	|	ВТ_ДанныеКалендаря КАК ВТ_ДанныеКалендаря
	|ГДЕ
	|	ВТ_ДанныеКалендаря.БанковскийСчетКасса <> ЗНАЧЕНИЕ(Справочник.Кассы.ПустаяСсылка)
	|	И ВТ_ДанныеКалендаря.БанковскийСчетКасса ССЫЛКА Справочник.Кассы
	|
	|УПОРЯДОЧИТЬ ПО
	|	Остаток УБЫВ";
...
Функция ТекстЗапросаУдалитьТекущуюОптимизацию()
	...
	|	ВТ_ОперацииОптимизации.БанковскийСчетКасса КАК БанковскийСчетКасса,
	...
	|	ВТ_ОперацииОптимизации.ВидДенежныхСредств КАК ВидДенежныхСредств,
	|	ВТ_ОперацииОптимизации.СтатьяДвиженияДенежныхСредств КАК СтатьяДвиженияДенежныхСредств
	|ИЗ
...
Функция МинимальныйОстатокПоСчетуКассеСДаты(Остатки, БанковскийСчетКасса, Дата)
	
	ОтборОстатков = Новый Структура("БанковскийСчетКасса", БанковскийСчетКасса);
	...
	Для Каждого СтрокаОстатка Из ОстаткиПоСчету Цикл
		...
		Если СтрокаОстатка.Дата >= Дата Тогда
			Если МинимальныйОстаток = Неопределено 
				ИЛИ МинимальныйОстаток > СтрокаОстатка.Остаток Тогда
	...
	МинимальныйОстаток = ?(МинимальныйОстаток = Неопределено, 0, МинимальныйОстаток);
...
Процедура ИзменитьСохраненныеОстаткиПоСчетуКассеСДаты(Остатки, БанковскийСчетКасса, Дата, Сумма)
	ИзменитьСохраненныеОстаткиПоОтборуЗаПериод(Остатки, Дата,, Сумма, Новый Структура("БанковскийСчетКасса", БанковскийСчетКасса));
КонецПроцедуры

Процедура ИзменитьСохраненныеОстаткиСДаты(Остатки, Дата, Сумма)
	ИзменитьСохраненныеОстаткиПоОтборуЗаПериод(Остатки, Дата,, Сумма);
КонецПроцедуры

Процедура ИзменитьСохраненныеОстаткиЗаПериод(Остатки, ДатаНачала, ДатаОкончания, Сумма)
	ИзменитьСохраненныеОстаткиПоОтборуЗаПериод(Остатки, ДатаНачала, ДатаОкончания, Сумма);
КонецПроцедуры

Процедура ИзменитьСохраненныеОстаткиПоОтборуЗаПериод(Остатки, ДатаНачалаКорректировки = Неопределено, 
					ДатаОкончанияКорректировки = Неопределено, СуммаКорректировки, ОтборОстатков = Неопределено)
...
Процедура СкорректироватьСохраненныеОстаткиКалендаря(Остатки, КорректировкаОстатков)
	...
	// Остатки: Дата, БанковскийСчетКасса, Остаток
	// Корректировка: Дата, БанковскийСчетКасса, Корректировка
	ОтборОстатков = Новый Структура("БанковскийСчетКасса");
...
Функция СуммарнаяОптимизацияПоСчетуКассеЗаПериод(Остатки, БанковскийСчетКасса, ДатаПоступления, ДатаВозврата = Неопределено, СуммаПоступления)
	
	СуммарнаяОптимизация = 0;                                                        
	ОтборПоСчету = Новый Структура("БанковскийСчетКасса", БанковскийСчетКасса);
...
Функция РезультатОптимизацииПлатежногоКалендаря(СтрокаДня, ВариантКорректировки, СостояниеКалендаря, НастройкиОптимизации)
	...
	Иначе
		ВызватьИсключение "Неподдерживаемый вариант оптимизации платежного календаря: " + ВариантКорректировки;
	КонецЕсли;
...
Функция НовыйУточнениеОперации()
	...
	СтрокаРеквизитов = "БанковскийСчетКассаСписания,БанковскийСчетКассаЗачисления,ДатаСписания,СуммаСписания,
		|ЭтоПополнениеСчетаКассы,СуммаЗачисления,ДатаЗачисления,КурсКонвертации,КратностьКонвертации,
		// ЕРПУХ ++
		|ВидОперацииУХ, ВидОперацииУХВстречный,ПлатежиПо275ФЗ, ТипПлатежаФЗ275,ДоговорСЗаказчиком,
		// ЕРПУХ --
		|ВалютаСписания,ВалютаЗачисления,ОрганизацияСписания,ОрганизацияЗачисления,Контрагент,ДоговорКонтрагента,
		|ЦФО,СтатьяДвиженияДенежныхСредств,СтатьяДвиженияДенежныхСредствЗачисление";
...
Процедура РаспределитьЗаявкиСНезаполненнымСчетом(СостояниеКалендаря, НастройкиОптимизации)
	...
	Отбор = Новый Структура("БанковскийСчетКассаПустой",Истина);
	...
	Для Каждого СтрокаЗаявки Из ЗаявкиБезСчета Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаЗаявки.БанковскийСчетКасса) Тогда
...
Процедура РазместитьЗаявкуНаСчете(СостояниеКалендаря, ТекущиеПараметрыЗаявки, ПараметрыРазмещения)
	...
	НовыеПараметрыЗаявки = Новый Структура("Банк, БанковскийСчетКасса, БанковскийСчетКассаПредставление");
	ЗаполнитьЗначенияСвойств(НовыеПараметрыЗаявки, ПараметрыРазмещения, "Банк, БанковскийСчетКасса, БанковскийСчетКассаПредставление");
	ПереместитьЗаявкуКалендаря(НовыеПараметрыЗаявки, ТекущиеПараметрыЗаявки, СостояниеКалендаря);
	
	ПараметрыКорректировкиОстатков = Новый Структура("БанковскийСчетКасса, Дата, Сумма");
	ЗаполнитьЗначенияСвойств(ПараметрыКорректировкиОстатков, ПараметрыРазмещения, "БанковскийСчетКасса, Дата");
...
Функция РезультатОптимизацииИзменениеДатыИСчетаЗаявки(СтрокаДня, ВариантОптимизации, СостояниеКалендаря, НастройкиОптимизации)
	...
	// Заявки по счету
	ОтборЗаявокПоСчету = Новый Структура("БанковскийСчетКасса");
	ОтборЗаявокПоСчету.БанковскийСчетКасса = СтрокаДня.БанковскийСчетКасса;	
	...
	СписокВидовДенежныхСредств = СписокВидовДенежныхСредств(СтрокаДня.ВидДенежныхСредств);

	Для Каждого ВидДенежныхСредств Из СписокВидовДенежныхСредств Цикл
		...
		ДобавитьОтборПоВидуДенежныхСредств(ОтборДоступныхОстатков,ВидДенежныхСредств);
		...
		Для Каждого СтрокаЗаявки Из ЗаявкиПоСчету Цикл
			Если СтрокаЗаявки.Дата <= КорректируемыйДень Тогда // перенос повлияет на остаток
					...
					И (ВариантОптимизации = Перечисления.ВариантыОптимизацииПлатежногоКалендаря.ИзменениеСчетаЗаявки
						...
						Если НЕ СтрокаЗаявки.ФиксированныйБанковскийСчет Тогда 
							Для Каждого СтрокаОстатка Из ДоступныеОстатки Цикл
								Если  СтрокаОстатка.БанковскийСчетКасса <> СтрокаЗаявки.БанковскийСчетКасса 
									И СтрокаОстатка.Дата = СтрокаЗаявки.Дата
									И СтрокаОстатка.Остаток >= СтрокаЗаявки.Сумма Тогда 
									
									МинимальныйОстатокПоСчету = МинимальныйОстатокПоСчетуКассеСДаты(ДоступныеОстатки, СтрокаОстатка.БанковскийСчетКасса, СтрокаОстатка.Дата);
					...
					И (ВариантОптимизации = Перечисления.ВариантыОптимизацииПлатежногоКалендаря.ИзменениеДатыЗаявки
						...
						Если ЗначениеЗаполнено(СтрокаЗаявки.МаксимальнаяДатаЗаявки) И 
							...
							Если НоваяДатаЗаявки > СтрокаЗаявки.Дата И НоваяДатаЗаявки > КорректируемыйДень Тогда 
								ОтборПоСчетуДате = Новый Структура("БанковскийСчетКасса, Дата");
								ОтборПоСчетуДате.БанковскийСчетКасса = СтрокаЗаявки.БанковскийСчетКасса;	
								...
								Иначе
									...
									НайденнаяСтрокаОстатка = Новый Структура("Дата, Банк, БанковскийСчетКасса, БанковскийСчетКассаПредставление");
				...
				Если НайденнаяСтрокаОстатка <> Неопределено Тогда 
					...
					ИзменитьСохраненныеОстаткиПоСчетуКассеСДаты(ДоступныеОстатки, СтрокаЗаявки.БанковскийСчетКасса, СтрокаЗаявки.Дата, СтрокаЗаявки.Сумма);
					ИзменитьСохраненныеОстаткиПоСчетуКассеСДаты(ДоступныеОстатки, НайденнаяСтрокаОстатка.БанковскийСчетКасса, НайденнаяСтрокаОстатка.Дата, -СтрокаЗаявки.Сумма);
					ЗаполнитьЗначенияСвойств(СтрокаЗаявки, НайденнаяСтрокаОстатка, "Дата, Банк, БанковскийСчетКасса, БанковскийСчетКассаПредставление");
...
Функция ПредставлениеОперацииПеремещенияЗаявки(НовыеПараметрыЗаявки, СтарыеПараметрыЗаявки)
	...
	Если СтарыеПараметрыЗаявки.БанковскийСчетКасса <> НовыеПараметрыЗаявки.БанковскийСчетКасса Тогда
		Если ЗначениеЗаполнено(СтарыеПараметрыЗаявки.БанковскийСчетКасса) Тогда
			...
			Если СтарыеПараметрыЗаявки.Дата = НовыеПараметрыЗаявки.Дата Тогда
				ПредставлениеОперации = СтрШаблон(НСтр("ru = 'Перемещение ""%1"" со счета %2 на счет %3'"),
													...
													СтарыеПараметрыЗаявки.БанковскийСчетКассаПредставление,
													НовыеПараметрыЗаявки.БанковскийСчетКассаПредставление);
...
Процедура СоздатьОперациюПеремещенияЗаявки(СостояниеКалендаря, СтрокаЗаявки)
	...
	Операция.БанковскийСчетКассаЗачисления = ИсходнаяЗаявка.БанковскийСчетКасса;
	...
	Операция.БанковскийСчетКассаСписания = СтрокаЗаявки.БанковскийСчетКасса;
	Операция.ВалютаСписания = СтрокаЗаявки.Валюта;
	Операция.ДатаСписания = СтрокаЗаявки.Дата;
	
	ЗаполнитьУточнениеОперацииПеремещения(Операция, СтрокаЗаявки);
	...
	Попытка
		...
		УправлениеДенежнымиСредствамиУХКлиентСервер.УстановитьВидДенежныхСредствВПредставленииПлатежногоКалендаря(МЗ);
		МЗ.Записать();
...
Функция РезультатОптимизацииПереводСобственныхСредств(СтрокаДня, СостояниеКалендаря, НастройкиОптимизации)
	...
	Для Каждого ВидДенежныхСредств Из СписокВидовДенежныхСредств Цикл		
		...
		ДобавитьОтборПоВидуДенежныхСредств(ОтборОстатков,ВидДенежныхСредств);
		...
		Для Каждого СтрокаОстатка Из ДоступныеОстатки Цикл
			Если СтрокаОстатка.БанковскийСчетКасса <> СтрокаДня.БанковскийСчетКасса
				...
				Если ДатаСписанияПоКалендарю <> Неопределено И СтрокаОстатка.Дата = ДатаСписанияПоКалендарю Тогда
					...
					Если МинимальныйОстатокПоСчету > 0 Тогда
						...
						СтрокаПеревода.БанковскийСчетКассаЗачисления = СтрокаДня.БанковскийСчетКасса;
						СтрокаПеревода.БанковскийСчетКассаЗачисленияПредставление = СтрокаДня.БанковскийСчетКассаПредставление;
						...
						СтрокаПеревода.БанковскийСчетКассаСписания = СтрокаОстатка.БанковскийСчетКасса;
						СтрокаПеревода.БанковскийСчетКассаСписанияПредставление = СтрокаОстатка.БанковскийСчетКассаПредставление;
						СтрокаПеревода.ПулЛиквидностиСписания = СтрокаОстатка.ПулЛиквидности;
						
						Если ЗначениеЗаполнено(СтрокаОстатка.БанковскийСчетКасса) Тогда
							СтрокаПеревода.ЦФО = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаОстатка.БанковскийСчетКасса, "Владелец");    
						КонецЕсли;   
						
						ВидОперацииУХ = Справочники.ВидыОперацийУХ.ПереводНаДругойСчетОрганизации; 
						
						СтрокаПеревода.СтатьяДвиженияДенежныхСредств = ВидыОперацийУХВызовСервераПовтИсп.ОсновнаяСтатьяБюджетаПоВидуОперации(ВидОперацииУХ);
	
						СтрокаПеревода.СтатьяДвиженияДенежныхСредствЗачисление = ВидыОперацийУХВызовСервераПовтИсп.ОсновнаяСтатьяБюджетаПоВидуОперации(
								ВидыОперацийУХВызовСервераПовтИсп.ВстречныйВидОперацииУХ(ВидОперацииУХ));
						
				        // изменяем остатки после перевода
						ИзменитьСохраненныеОстаткиПоСчетуКассеСДаты(ДоступныеОстатки, СтрокаПеревода.БанковскийСчетКассаЗачисления, СтрокаПеревода.ДатаЗачисления, СтрокаПеревода.Сумма);
						ИзменитьСохраненныеОстаткиПоСчетуКассеСДаты(ДоступныеОстатки, СтрокаПеревода.БанковскийСчетКассаСписания, СтрокаПеревода.ДатаСписания, -СтрокаПеревода.Сумма);
	...
	РезультатОптимизации.Выполнена = (ОстатокВДне > НачальныйОстаток) И ТаблицаПереводов.Количество();
	РезультатОптимизации.ДеньПолностьюСкорректирован = (ОстатокВДне >= 0);
	Если РезультатОптимизации.Выполнена Тогда
		СуммаПоступления = ОстатокВДне - НачальныйОстаток;
		СуммарнаяОптимизацияКалендаря = СуммарнаяОптимизацияПоСчетуКассеЗаПериод(СостояниеКалендаря.Остатки, СтрокаДня.БанковскийСчетКасса, СтрокаДня.Дата,,СуммаПоступления); // в валюте остатка
...
Функция НоваяТаблицаПереводовСобственныхСредств()
	...
	ТипБанковскийСчетКасса = Отчеты.ПлатежныйКалендарьУХ.ОписаниеТипаБанковскиеСчетаКассы();
	...
	ТаблицаПереводов.Колонки.Добавить("БанковскийСчетКассаСписания", ТипБанковскийСчетКасса);
	ТаблицаПереводов.Колонки.Добавить("БанковскийСчетКассаСписанияПредставление", ТипПредставление);
	...
	ТаблицаПереводов.Колонки.Добавить("БанковскийСчетКассаЗачисления", ТипБанковскийСчетКасса);
	ТаблицаПереводов.Колонки.Добавить("БанковскийСчетКассаЗачисленияПредставление", ТипПредставление);
	ТаблицаПереводов.Колонки.Добавить("ПулЛиквидностиЗачисления", Новый ОписаниеТипов("СправочникСсылка.ПулыЛиквидности"));
    ТаблицаПереводов.Колонки.Добавить("ЦФО", Новый ОписаниеТипов("СправочникСсылка.Организации"));
	ТаблицаПереводов.Колонки.Добавить("СтатьяДвиженияДенежныхСредств", Новый ОписаниеТипов("СправочникСсылка.СтатьиДвиженияДенежныхСредств"));
	ТаблицаПереводов.Колонки.Добавить("СтатьяДвиженияДенежныхСредствЗачисление", Новый ОписаниеТипов("СправочникСсылка.СтатьиДвиженияДенежныхСредств"));
...
Функция ПредставлениеОперацииПереводСобственныхСредств(ПараметрыПеревода)
	...
	ПредставлениеОперации = СтрШаблон(НСтр("ru = 'Перевод %1 %2 со счета %3 на счет %4 в дату %5 (%6)'"),
									...
									ПараметрыПеревода.БанковскийСчетКассаСписанияПредставление,		
									ПараметрыПеревода.БанковскийСчетКассаЗачисленияПредставление,
...
Функция УточнениеОперацииПереводСобственныхСредств(ПараметрыПеревода)
	...
	РеквизитыЗаполнения = "БанковскийСчетКассаЗачисления, БанковскийСчетКассаСписания, ДатаСписания, ДатаЗачисления, 
							|ЦФО, СтатьяДвиженияДенежныхСредств, СтатьяДвиженияДенежныхСредствЗачисление"; 
	
	УточнениеОперации = НовыйУточнениеОперации();
	ЗаполнитьЗначенияСвойств(УточнениеОперации, ПараметрыПеревода, РеквизитыЗаполнения);
	...
	УточнениеОперации.ЭтоПополнениеСчетаКассы = Истина;   
	// ЕРПУХ ++
...
Процедура ВыполнитьПереводСобственныхСредств(СостояниеКалендаря, ПараметрыПеревода)
	...
	Операция.БанковскийСчетКассаСписания = ПараметрыПеревода.БанковскийСчетКассаСписания;
	Операция.БанковскийСчетКассаЗачисления = ПараметрыПеревода.БанковскийСчетКассаЗачисления;
	...
	Попытка
		...
		МЗ.БанковскийСчетКасса = ПараметрыПеревода.БанковскийСчетКассаСписания;
		...
		МЗ.БанковскийСчетКасса = ПараметрыПеревода.БанковскийСчетКассаЗачисления;
...
Функция СписокВидовДенежныхСредств(ТекущийВидДенежныхСредств)

	СписокВидовДенежныхСредств = Новый СписокЗначений;
	
	Если ТекущийВидДенежныхСредств = Перечисления.ФормыОплаты.Безналичная Тогда
		СписокВидовДенежныхСредств.Добавить( Перечисления.ФормыОплаты.Безналичная);
		СписокВидовДенежныхСредств.Добавить( Перечисления.ФормыОплаты.Наличная);
	Иначе
		СписокВидовДенежныхСредств.Добавить( Перечисления.ФормыОплаты.Наличная);
		СписокВидовДенежныхСредств.Добавить( Перечисления.ФормыОплаты.Безналичная);
	КонецЕсли;
	
	Возврат СписокВидовДенежныхСредств;
	
КонецФункции

Процедура ДобавитьОтборПоВидуДенежныхСредств(Отбор,ВидДенежныхСредств)
	
	Отбор.Вставить("ВидДенежныхСредств",ВидДенежныхСредств.Значение);
...
Функция РезультатОптимизацииВнутригрупповыеПереводы(СтрокаДня, СостояниеКалендаря, НастройкиОптимизации)
	...
	Если РезультатОптимизации.Выполнена Тогда
		СуммаПоступления = ОстатокВДне - НачальныйОстаток;
		СуммарнаяОптимизацияКалендаря = СуммарнаяОптимизацияПоСчетуКассеЗаПериод(СостояниеКалендаря.Остатки, СтрокаДня.БанковскийСчетКасса, СтрокаДня.Дата,,СуммаПоступления);
		РезультатОптимизации.ОтносительнаяСтоимостьОптимизации = СтоимостьИзменения / СуммарнаяОптимизацияКалендаря;
	КонецЕсли;	

	РезультатОптимизации.ВнутригрупповыеПереводы = ТаблицаПереводов;
	
	ОтборИзменений = Новый Структура("Изменен", Истина);
	НовыеОстатки = ДоступныеОстатки.Скопировать(ОтборИзменений, "ИндексСтроки, Остаток");
	РезультатОптимизации.НовыеОстатки = НовыеОстатки;
	
	НовыеЛимиты = ДоступныеЛимитыПеремещений.Скопировать(ОтборИзменений, "ИндексСтроки, Сумма");
...
Функция НоваяТаблицаВнутригрупповыхПереводов()
	...
	ТипСумма = Новый ОписаниеТипов("Число",Новый КвалификаторыЧисла(18,2));
	ТипПредставление = Новый ОписаниеТипов("Строка");
	ТипБанковскиеСчетаКассы = Отчеты.ПлатежныйКалендарьУХ.ОписаниеТипаБанковскиеСчетаКассы();
	...
	ТаблицаПереводов.Колонки.Добавить("ДатаСписания", ТипДата);
	ТаблицаПереводов.Колонки.Добавить("БанкСписания", Новый ОписаниеТипов("СправочникСсылка.КлассификаторБанков"));
	ТаблицаПереводов.Колонки.Добавить("БанковскийСчетКассаСписания", ТипБанковскиеСчетаКассы);
	ТаблицаПереводов.Колонки.Добавить("БанковскийСчетКассаСписанияПредставление", ТипПредставление);
	ТаблицаПереводов.Колонки.Добавить("ПулЛиквидностиСписания", Новый ОписаниеТипов("СправочникСсылка.ПулыЛиквидности"));
	...
	ТаблицаПереводов.Колонки.Добавить("ДатаЗачисления", ТипДата);
	ТаблицаПереводов.Колонки.Добавить("БанкЗачисления", Новый ОписаниеТипов("СправочникСсылка.КлассификаторБанков"));
	ТаблицаПереводов.Колонки.Добавить("БанковскийСчетКассаЗачисления", ТипБанковскиеСчетаКассы);
	ТаблицаПереводов.Колонки.Добавить("БанковскийСчетКассаЗачисленияПредставление", ТипПредставление);
	ТаблицаПереводов.Колонки.Добавить("ПулЛиквидностиЗачисления", Новый ОписаниеТипов("СправочникСсылка.ПулыЛиквидности"));
...
Функция УточнениеОперацииВнутригрупповойПеревод(ПараметрыПеревода)
	...
	УточнениеОперации = НовыйУточнениеОперации();
	ЗаполнитьЗначенияСвойств(УточнениеОперации, ПараметрыПеревода, "ОрганизацияСписания, ОрганизацияЗачисления, БанковскийСчетКассаЗачисления, БанковскийСчетКассаСписания, ДатаСписания, ДатаЗачисления");
	УточнениеОперации.СуммаСписания = ПараметрыПеревода.Сумма;
	УточнениеОперации.СуммаЗачисления = ПараметрыПеревода.Сумма;
	УточнениеОперации.ВалютаСписания = ПараметрыПеревода.Валюта;
	УточнениеОперации.ВалютаЗачисления = ПараметрыПеревода.Валюта;
...
Процедура ОбновитьЛимитыВнутригрупповыхПереводовКалендаря(СостояниеКалендаря, НовыеЛимитыПеремещений)
	...
	Для Каждого СтрокаНовыеЛимиты Из НовыеЛимитыПеремещений Цикл
		...
		СтрокиЛимита = ЛимитыПеремещений.НайтиСтроки(Новый Структура("ИндексСтроки",СтрокаНовыеЛимиты.ИндексСтроки));
		Если СтрокиЛимита.Количество() Тогда  
	...
	Для Каждого СтрокаОстатка Из ДоступныеОстатки Цикл
		Если СтрокаОстатка.Валюта <> СтрокаДня.Валюта
			...
			Если МинимальныйОстатокПоСчету > 0 Тогда
				...
				СтрокаКонвертации.БанковскийСчетКассаЗачисления = СтрокаДня.БанковскийСчетКасса;
				СтрокаКонвертации.БанковскийСчетКассаЗачисленияПредставление = СтрокаДня.БанковскийСчетКассаПредставление;
				...
				СтрокаКонвертации.БанковскийСчетКассаСписания = СтрокаОстатка.БанковскийСчетКасса;
				СтрокаКонвертации.БанковскийСчетКассаСписанияПредставление = СтрокаОстатка.БанковскийСчетКассаПредставление;
				...
				// Корректировка доступных остатков
				ИзменитьСохраненныеОстаткиПоСчетуКассеСДаты(ДоступныеОстатки, СтрокаКонвертации.БанковскийСчетКассаЗачисления, СтрокаКонвертации.ДатаЗачисления, СтрокаКонвертации.СуммаЗачисления); 
				ИзменитьСохраненныеОстаткиПоСчетуКассеСДаты(ДоступныеОстатки, СтрокаКонвертации.БанковскийСчетКассаСписания, СтрокаКонвертации.ДатаСписания, -СтрокаКонвертации.СуммаСписания);
...
Функция НоваяТаблицаКонвертацииВалюты()
	...
	ТаблицаКонвертации.Колонки.Добавить("БанковскийСчетКассаСписания", ТипБанковскиеСчетаКассы);
	ТаблицаКонвертации.Колонки.Добавить("БанковскийСчетКассаСписанияПредставление", ТипПредставление);
	...
	ТаблицаКонвертации.Колонки.Добавить("БанковскийСчетКассаЗачисления", ТипБанковскиеСчетаКассы);
	ТаблицаКонвертации.Колонки.Добавить("БанковскийСчетКассаЗачисленияПредставление", ТипПредставление);
...
Функция ПредставлениеОперацииКонвертацияВалюты(ПараметрыОперации)
	...
	ПредставлениеОперации = СтрШаблон(НСтр("ru = 'Конвертация валюты: списание %1 %2 со счета %3 в дату %4, зачисление %5 %6 на счет %7 в дату %8 (%9)'"),
								...
								ПараметрыОперации.БанковскийСчетКассаСписанияПредставление,
								...
								ПараметрыОперации.БанковскийСчетКассаЗачисления,
...
Функция УточнениеОперацииКонвертацияВалюты(ПараметрыОперации)
	...
	ЗаполнитьЗначенияСвойств(УточнениеОперации, ПараметрыОперации, 
		"БанковскийСчетКассаЗачисления, БанковскийСчетКассаСписания, ДатаСписания, ДатаЗачисления, 
		|СуммаСписания, СуммаЗачисления, ВалютаСписания, ВалютаЗачисления, КурсКонвертации, КратностьКонвертации");
	...
	УточнениеОперации.ДоговорКонтрагента = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
	//УточнениеОперации.ДоговорКонтрагента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
	//								УточнениеОперации.БанковскийСчетКассаСписания, "ДоговорРасчетноКассовогоОбслуживания");
	УточнениеОперации.Контрагент = РегистрыСведений.СвязьКонтрагентовИБанков.ПолучитьКонтрагентаПоБанку(
									УточнениеОперации.БанковскийСчетКассаСписания);
	УточнениеОперации.ЭтоПополнениеСчетаКассы = Истина;
	
	Если ЗначениеЗаполнено(УточнениеОперации.ДоговорКонтрагента) Тогда
		УточнениеОперации.ЦФО = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
									УточнениеОперации.ДоговорКонтрагента, "ОсновнойЦФО");	
	КонецЕсли;	
	...
	УточнениеОперации.СтатьяДвиженияДенежныхСредств = ВидыОперацийУХВызовСервераПовтИсп.ОсновнаяСтатьяБюджетаПоВидуОперации(ВидОперацииУХ);
	
	ВидОперацииУХВстречный = ВидыОперацийУХВызовСервераПовтИсп.ВстречныйВидОперацииУХ(ВидОперацииУХ);
	УточнениеОперации.СтатьяДвиженияДенежныхСредствЗачисление = ВидыОперацийУХВызовСервераПовтИсп.ОсновнаяСтатьяБюджетаПоВидуОперации(ВидОперацииУХВстречный);
...
Процедура ВыполнитьОперациюКонвертацииВалюты(СостояниеКалендаря, ПараметрыОперации)
	...
	Операция.БанковскийСчетКассаСписания = ПараметрыОперации.БанковскийСчетКассаСписания;
	Операция.БанковскийСчетКассаЗачисления = ПараметрыОперации.БанковскийСчетКассаЗачисления;
	...
	Попытка
		...
		МЗ.БанковскийСчетКасса = ПараметрыОперации.БанковскийСчетКассаСписания;
		...
		МЗ.БанковскийСчетКасса = ПараметрыОперации.БанковскийСчетКассаЗачисления;
...
Функция РезультатОптимизацииОвердрафт(СтрокаДня, СостояниеКалендаря, НастройкиОптимизации)
	...
	Если ЕстьОвердрафтПоСчетуНаДату(СостояниеКалендаря.ОвердрафтПоСчетам, СтрокаДня.БанковскийСчетКасса, СтрокаДня.Дата) Тогда
		Возврат РезультатОптимизации; // может быть только один овердрафт по счету
	КонецЕсли;	
	
	ДоговорОвердрафта = ДействующийДоговорОвердрафта(СостояниеКалендаря.ДоговорыОвердрафта, СтрокаДня.БанковскийСчетКасса, СтрокаДня.Дата);	
	Если ДоговорОвердрафта = Неопределено Тогда 
		Возврат РезультатОптимизации;
	КонецЕсли;
	
	СуммаВыплатыПроцентовНаЭтотДень = СуммаВыплатыПроцентовОвердрафтаПоСчетуНаДату(СостояниеКалендаря.ОвердрафтПоСчетам, СтрокаДня.БанковскийСчетКасса, СтрокаДня.Дата);
	...
	ЗаполнитьЗначенияСвойств(Овердрафт, СтрокаДня, "Организация, ОрганизацияПредставление, Банк, БанковскийСчетКасса, БанковскийСчетКассаПредставление, ПулЛиквидности, Валюта, ВалютаПредставление");
	...
	ОтборОстатковПоСчету = Новый Структура("БанковскийСчетКасса", СтрокаДня.БанковскийСчетКасса);
	ОстаткиПоСчету = СостояниеКалендаря.Остатки.Скопировать(ОтборОстатковПоСчету, "ИндексСтроки, БанковскийСчетКасса, Дата, Остаток");
	...
	ОтборИзменений = Новый Структура("Изменен", Истина);
	НовыеОстатки = ОстаткиПоСчету.Скопировать(ОтборИзменений, "ИндексСтроки, Остаток");
	РезультатОптимизации.НовыеОстатки = НовыеОстатки;
...
Функция СуммаВыплатыПроцентовОвердрафтаПоСчетуНаДату(ТаблицаОвердрафтаПоСчетам, БанковскийСчетКасса, Дата)
	
	Если Дата = НачалоДня(КонецМесяца(Дата)) Тогда // проценты выплачиваются в конце месяца	
		ОтборПроцентовПоСчету = Новый Структура("БанковскийСчетКасса, ДатаВыплатыПроцентов");
		ОтборПроцентовПоСчету.БанковскийСчетКасса = БанковскийСчетКасса;
...
Функция ЕстьОвердрафтПоСчетуНаДату(ТаблицаОвердрафтаПоСчетам, БанковскийСчетКасса, Дата)
	
	ОтборПоСчету = Новый Структура("БанковскийСчетКасса");
	ОтборПоСчету.БанковскийСчетКасса = БанковскийСчетКасса;
...
Функция ДействующийДоговорОвердрафта(ДоговорыОвердрафта, БанковскийСчетКасса, Дата)
...
Функция НовыйОвердрафтПоСчету()
	...
	НовыйОвердрафт = Новый Структура("Организация, ОрганизацияПредставление, Банк, БанковскийСчетКасса, БанковскийСчетКассаПредставление, ПулЛиквидности, Валюта,
...
Функция НоваяТаблицаОвердрафтПоСчетам()
	...
	ТаблицаОвердрафтаПоСчетам.Колонки.Добавить("БанковскийСчетКасса",ТипБанковскиеСчетаКассы);
	ТаблицаОвердрафтаПоСчетам.Колонки.Добавить("БанковскийСчетКассаПредставление", ТипПредставление);
	...
	ТаблицаОвердрафтаПоСчетам.Индексы.Добавить("БанковскийСчетКасса");
	ТаблицаОвердрафтаПоСчетам.Индексы.Добавить("БанковскийСчетКасса, ДатаВозвратаСредств, ДатаВыплатыПроцентов, Сумма");
...
Процедура ДобавитьОвердрафтВСписокОпераций(СостояниеКалендаря, Овердрафт)
	...
	ОтборОвердрафта = Новый Структура("БанковскийСчетКасса, ДатаВозвратаСредств, ДатаВыплатыПроцентов, Сумма");
	ЗаполнитьЗначенияСвойств(ОтборОвердрафта, Овердрафт, "БанковскийСчетКасса, ДатаВыплатыПроцентов, Сумма");
...
Функция ПредставлениеОперацииОвердрафта(Овердрафт)
	...
	ПредставлениеОперации = СтрШаблон(НСтр("ru = 'Овердрафт %1 %2 на счете %3 с %4 по %5 (%6)'"),
									...
									Овердрафт.БанковскийСчетКассаПредставление,		
...
Процедура СоздатьОперациюОвердрафта(СостояниеКалендаря, Овердрафт)
	...
	Операция.БанковскийСчетКассаЗачисления = Овердрафт.БанковскийСчетКасса;
	...
	ОбщиеПараметры.Вставить("БанковскийСчетКасса", Овердрафт.БанковскийСчетКасса);
...
Функция РезультатОптимизацииКредит(СтрокаДня, СостояниеКалендаря, НастройкиОптимизации)
	...
	ЗаполнитьЗначенияСвойств(Кредит, СтрокаДня, "Организация, ОрганизацияПредставление, Банк, БанковскийСчетКасса, БанковскийСчетКассаПредставление, ПулЛиквидности, Валюта, ВалютаПредставление");
	...
	ОтборИзменений = Новый Структура("Изменен", Истина);
	НовыеЛимиты = ЛимитыПоДоговору.Скопировать(ОтборИзменений, "ИндексСтроки, Лимит");
...
Функция НовыйКредитПоСчету()
	...
	НовыйКредит = Новый Структура("Организация, ОрганизацияПредставление, Банк, БанковскийСчетКасса, БанковскийСчетКассаПредставление, ПулЛиквидности, Валюта, ВалютаПредставление, Сумма,
...
Функция НоваяТаблицаКредитыПоСчетам()
	...
	ТаблицаКредитыПоСчетам.Колонки.Добавить("БанковскийСчетКасса", ТипБанковскиеСчетаКассы);
	ТаблицаКредитыПоСчетам.Колонки.Добавить("БанковскийСчетКассаПредставление", ТипПредставление);
	...
	ТаблицаКредитыПоСчетам.Колонки.Добавить("БазовыйДоговорКредита", Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов"));
	ТаблицаКредитыПоСчетам.Колонки.Добавить("ДоговорКредита", Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов"));
	...
	ТаблицаКредитыПоСчетам.Индексы.Добавить("БанковскийСчетКасса");
...
Функция НовоеУточнениеОперацииКредита()
	...
	СтрокаРеквизитов = "Банк, БанковскийСчетКасса, Валюта, КоличествоДней, Организация, ПериодИспользования, ПроцентнаяСтавка, 
...
Функция ПараметрыПолученияНовогоКредита(СостояниеКалендаря, СущественнаяРазницаСтоимостиКредитов, СтрокаДня)
	...
	Для Каждого СтрокаДоговора Из МассивДоговоров Цикл
		...
		Если СтрокаДоговора.ФиксированныйСчетОрганизации И СтрокаДоговора.БанковскийСчетКасса <> СтрокаДня.БанковскийСчетКасса Тогда
			Продолжить;
		КонецЕсли;
		...
		СуммарнаяОптимизацияКалендаря = СуммарнаяОптимизацияПоСчетуКассеЗаПериод(СостояниеКалендаря.Остатки, СтрокаДня.БанковскийСчетКасса, ДатаПоступленияСредств, ДатаВозвратаСредств, СуммаКредита);                                                        
...
Функция ПараметрыИзмененияТекущегоКредита(СостояниеКалендаря, СущественнаяРазницаСтоимостиКредитов, СтрокаДня)
	...
	ОтборКредитов = Новый Структура("БанковскийСчетКасса", СтрокаДня.БанковскийСчетКасса);
	...
	Для Каждого СтрокаКредита Из МассивВзятыхКредитов Цикл
		...
		Если СтрокаКредита.ДатаВозвратаСредств  <= ТребуемаяДатаПоступленияСредств Тогда
			...
			ДополнениеОптимизацииДобавПериод = СуммарнаяОптимизацияПоСчетуКассеЗаПериод(СостояниеКалендаря.Остатки, СтрокаДня.БанковскийСчетКасса, СтрокаКредита.ДатаВозвратаСредств, НоваяДатаВозвратаСредств, НоваяСуммаКредита);	
		...
		Иначе
			...
			ДополнениеОптимизации = СуммарнаяОптимизацияПоСчетуКассеЗаПериод(СостояниеКалендаря.Остатки, СтрокаДня.БанковскийСчетКасса, СтрокаКредита.ДатаПоступленияСредств, СтрокаКредита.ДатаВозвратаСредств, СуммаУвеличенияКредита);                                                        
...
Процедура ИзменитьСохраненныеЛимитыПоКредиту(ЛимитыПоДоговору, ДатаПоступленияСредств, ДатаВозвратаСредств, СуммаКорректировки)
...
Процедура ОптимизироватьРассчитанныеКредиты(СостояниеКалендаря)
	...
	Для Каждого СтрокаКредита Из ТаблицаКредитыПоСчетам Цикл
		ОтборПоСчету = Новый Структура("БанковскийСчетКасса", СтрокаКредита.БанковскийСчетКасса);
...
Функция УточнениеОперацииКредита(Кредит)
	...
	ЗаполнитьЗначенияСвойств(УточнениеОперации, Кредит, "Банк, БанковскийСчетКасса, Валюта, Организация, ПроцентнаяСтавка, Сумма, СуммаПроцентов, КоличествоДней");
...
Процедура СоздатьОперациюКредита(СостояниеКалендаря, Кредит)
	...
	Операция.БанковскийСчетКассаЗачисления = Кредит.БанковскийСчетКасса;
	...
	ОбщиеПараметры.Вставить("БанковскийСчетКасса", Кредит.БанковскийСчетКасса);
