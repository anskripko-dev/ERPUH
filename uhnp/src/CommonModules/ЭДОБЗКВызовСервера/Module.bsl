...
Процедура УдалитьПодписиИОбновитьОсновныеДокументыПриКонвертацииДокументов(Результат) Экспорт
	...
	Попытка
		...
		Для Каждого ДанныеФайла Из ДанныеФайловПечатныхФорм Цикл
			...
			Иначе
				СообщениеПротокола = СтрШаблон(НСтр("ru = 'Электронный документ %1 - удалены подписи, сконвертирован в формат PDF/A-1A';
													|en = 'The %1 electronic document: signatures are deleted, and the document is converted into PDF/A-1A'"), ДокументЭДОБЗКСсылка);
			КонецЕсли;
		...
		ЗаписьЖурналаРегистрации(
			ЭДОБЗК.ИмяСобытияЖурналаРегистрации(
				НСтр("ru = 'Подготовка файла представления электронного документа';
					|en = 'Prepare a file of the electronic document presentation'", ОбщегоНазначения.КодОсновногоЯзыка())),
			УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(Ошибка));
		ДанныеФайлаНаПечать = Неопределено;
	КонецПопытки;
...
Функция УстановитьПризнакФорматФайлаPDF_A_1(ДокументЭДОБЗК) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Не удалось заблокировать документ ""%1"" для проверки (конвертации) на соответсиве формату PDF/A-1A';
										|en = 'Cannot lock the ""%1"" document for check (conversion) for compliance with PDF/A-1A format'"), ДокументЭДОБЗК);
	КонецПопытки;
	Возврат Результат;
КонецФункции
...
Процедура СконвертироватьДанныеДокументаВФорматPDF_A_1(ДокументЭДОБЗК, ДанныеДокумента, УникальныйИдентификатор) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Не удалось заблокировать файл ""%1"" для конвертации в формат PDF/A-1A';
										|en = 'Cannot lock the ""%1"" file for conversion to PDF/A-1A format'"), ДанныеДокумента.ИмяФайла);
	КонецПопытки;
...
Функция СконвертироватьДанныеДокументовВФорматPDF_A_1(ДокументыЭДОБЗК, УникальныйИдентификатор) Экспорт
	...
	Результат.ДобавитьСтроку(СтрШаблон(
		НСтр("ru = 'Проверка документов ЭДО на соответствие формату PDF/A-1A от %1';
			|en = 'Check of EDI documents for compliance with PDF/A-1A format dated %1'"),
		ТекущаяДатаСеанса()));
	...
	Если ЗначениеЗаполнено(НеЧитаются) Тогда
		...
		Результат.ДобавитьСтроку(НСтр("ru = 'Не удалось прочитать как PDF документ:';
										|en = 'Cannot read as PDF document:'"));
...
Функция ИсходныйФайлPDF(АдресВХранилище, ИмяФайла)
	Попытка
		...
		Возврат ФайлPDF;
	Исключение
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Не удается прочитать файл ""%1""';
										|en = 'Cannot read the ""%1"" file'"), ИмяФайла);
	КонецПопытки;
КонецФункции
...
Функция СконвертироватьДанныеФайлаВPDF_A_1(АдресВХранилище, ИмяФайла, УникальныйИдентификатор) Экспорт
	...
	Исключение
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Не удалось сконвертировать в формат PDF/A-1A файл ""%1""';
				|en = 'Cannot convert the ""%1"" file to PDF/A-1A format'"), ИмяФайла);
	КонецПопытки;
	Возврат Ложь;
КонецФункции
