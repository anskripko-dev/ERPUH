...
Функция ИсточникиДоступныхПолей(ИсточникиДоступныхПолей, НастройкиСписка, Реквизит = Неопределено, УникальныйИдентификаторФормы = Неопределено)
	...
	Для Каждого Тип Из Реквизит.Тип.Типы() Цикл
		...
		Если ИсточникиДоступныхПолей.Количество() > 0 Тогда
			Попытка
				Если ИсточникиДоступныхПолей[0].ИсточникДанных = "Справочник.ДоговорыКонтрагентов" 
					И Реквизит.Имя = "ВерсияСоглашения" Тогда 
					
					Если НастройкиСписка.Свойство("СсылкаНаОбъект") Тогда
						Если Не ТипЗнч(НастройкиСписка.СсылкаНаОбъект[Реквизит.Имя]) = Тип Тогда
							Продолжить;	
						КонецЕсли;
					КонецЕсли;
