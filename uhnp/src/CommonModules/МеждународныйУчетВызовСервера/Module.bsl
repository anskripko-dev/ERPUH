...
// Возвращает родительский предопределенный вариант отчета для настройки расшифровки
//
// Параметры:
//  КлючВарианта - Строка - ключ исходного варианта отчета
//  ИмяОтчета - Строка - имя отчета
//
// Возвращаемое значение:
//  Строка - ключ родительского предопределенного варианта отчета
//
Функция КлючРодительскогоПредопределенногоВариантаОтчета(КлючВарианта, ИмяОтчета) Экспорт
	
	ВариантОтчета = ВариантыОтчетов.ВариантОтчета(ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Отчет."
		+ ИмяОтчета), КлючВарианта);
	Пока Истина Цикл
		РеквизитыВарианта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВариантОтчета,
			"ПредопределенныйВариант, КлючВарианта, Родитель");
		КлючВарианта = РеквизитыВарианта.КлючВарианта;
		Если ЗначениеЗаполнено(РеквизитыВарианта.ПредопределенныйВариант) Тогда
			Прервать;
		ИначеЕсли Не ЗначениеЗаполнено(РеквизитыВарианта.Родитель) Тогда
			Прервать;
		Иначе
			ВариантОтчета = РеквизитыВарианта.Родитель;
		КонецЕсли;
	КонецЦикла;
...
Функция НастроитьОтчетРасшифровки(Знач Параметры) Экспорт
	...
	КлючВарианта = КлючРодительскогоПредопределенногоВариантаОтчета(Параметры.КлючВарианта, Параметры.ИмяОтчета);
	НастройкиВарианта = СхемаРасшифровки.ВариантыНастроек[КлючВарианта].Настройки;
...
Функция ПолучитьРасшифровкуОтчета(ДанныеРасшифровки, ИдентификаторРасшифровки)
	...
	ПоляРасшифровки.Добавить("ПериодСекунда");
	ПоляРасшифровки.Добавить("ПериодМинута");
	ПоляРасшифровки.Добавить("ПериодЧас");
	ПоляРасшифровки.Добавить("ПериодДень");
	ПоляРасшифровки.Добавить("ПериодНеделя");
	ПоляРасшифровки.Добавить("ПериодДекада");
	ПоляРасшифровки.Добавить("ПериодМесяц");
	ПоляРасшифровки.Добавить("ПериодКвартал");
	ПоляРасшифровки.Добавить("ПериодПолугодие");
	ПоляРасшифровки.Добавить("ПериодГод");
