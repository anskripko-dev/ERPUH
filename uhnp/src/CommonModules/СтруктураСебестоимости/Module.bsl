...
Процедура РазузловатьУзелДереваСебестоимости(ПараметрыДерева, ПараметрыУзла, ТаблицаРезультата, ОписаниеПродукции)
	...
	Для Каждого ТекПартия Из СебестоимостьУзла Цикл
		...
		Если ПараметрыДерева.ТипРезультата = "ДеревоЗначений"
			...
			// Смотри комментарий 1893.
			СуммаВключениеИсключениеНДСВСтоимости = ?(ТекПартия.ТребуетсяРазузлование,
				0, ТекПартия.ВключениеИсключениеНДСВСтоимости);
			СтрокаПартия.Количество			= ТекПартия.КоличествоЗатрата;
			СтрокаПартия.Сумма				= Окр(ТекПартия.СуммаЗатрата - СуммаВключениеИсключениеНДСВСтоимости, 2);
			СтрокаПартия.ДопРасходы			= Окр(СтрокаПартия.ДопРасходы, 2);
			СтрокаПартия.НалоговыйУчет		= Окр(СтрокаПартия.НалоговыйУчет - СуммаВключениеИсключениеНДСВСтоимости, 2);
			СтрокаПартия.СуммаЗабалансовая	= ТекПартия.СуммаЗатратаЗабалансовая;
			СтрокаПартия.Материальные		= СтрокаПартия.Материальные - СуммаВключениеИсключениеНДСВСтоимости;
...
#Область ТекстМатериальныеЗатраты
	...
	#Область ДетализацияПартий
		...
		// Комментарий 1893.
		// По ПФ не должен включатся / исключатся НДС, т.к. это затраты других переделов.
