...
Процедура ПроверитьЗаполнениеАкцизныхМарок(Объект, Отказ, НаДату = Неопределено, РеквизитыОбъекта = Неопределено, СообщатьОбОшибках = Истина, ФормированиеЭлектронногоДокумента = Ложь) Экспорт
	...
	ПодсказкаНетМарок = НСтр("ru = 'Отсканируйте коды маркировки.';
							|en = 'Отсканируйте коды маркировки.'");
	ПодсказкаМногоМарок = НСтр("ru = 'Проверьте количество товара или удалите лишние коды маркировки через форму проверки.';
								|en = 'Проверьте количество товара или удалите лишние коды маркировки через форму проверки.'");
	Для Каждого СтрокаТовары Из Объект[ИмяТаблицыТовары] Цикл
		...
		Если КоличествоМарок <> КоличествоЕдиницТоваров Тогда
			...
			Шаблон = НСтр("ru = 'Товара ""%1"" по документу - %2 %4, по кодам маркировки - %3 %4. %5';
							|en = 'Товара ""%1"" по документу - %2 %4, по кодам маркировки - %3 %4. %5'");
			...
			Для Каждого СтрокаТовар Из СтрокиТоваров Цикл
				...
				Если КоличествоМарок <> КоличествоЕдиницТоваров Тогда
					...
					Шаблон = НСтр("ru = 'Для маркировки ""%1"" по серии должно быть указано единиц - %2, а указано - %3. %4';
									|en = 'Для маркировки ""%1"" по серии должно быть указано единиц - %2, а указано - %3. %4'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						Шаблон,
						ПредставлениеНоменклатуры,
						КоличествоЕдиницТоваров,
...
//++ НЕ УТКА
		...
		Или ИмяФормы = "Документ.ПроизводственнаяОперация2_2.Форма.ФормаДокумента"
...
// Текст запроса производство без заказа с детализацией до номера группы затрат.
// 
// Возвращаемое значение:
//  Строка - Текст запроса производство без заказа
Функция ТекстЗапросаПроизводствоБезЗаказаСДетализациейДоНомераГруппыЗатрат() Экспорт
	...
	|	ТоварыПроизводствоБезЗаказа.Номенклатура КАК Номенклатура,
	|	ТоварыПроизводствоБезЗаказа.Характеристика КАК Характеристика,
	|	ТоварыПроизводствоБезЗаказа.НомерГруппыЗатрат КАК НомерГруппыЗатрат
	|ПОМЕСТИТЬ ПроизводствоБезЗаказаБезСерий
	|ИЗ
	|	ТоварыПроизводствоБезЗаказа КАК ТоварыПроизводствоБезЗаказа
	|ГДЕ
	...
	|	ДвиженияСерийТоваров.Характеристика КАК Характеристика,
	|	ДвиженияСерийТоваров.Серия КАК Серия,
	|	ПроизводствоБезЗаказаБезСерий.НомерГруппыЗатрат КАК НомерГруппыЗатрат,
	|	СУММА(ДвиженияСерийТоваров.КоличествоОборот) КАК Количество
	|ПОМЕСТИТЬ СкладскиеДвиженияПроизводствоБезЗаказа
	|ИЗ
	|	РегистрНакопления.ДвиженияСерийТоваров.Обороты(,,, Документ В
	...
	|	ДвиженияСерийТоваров.Характеристика,
	|	ДвиженияСерийТоваров.Серия,
	|	ПроизводствоБезЗаказаБезСерий.НомерГруппыЗатрат
	|ИМЕЮЩИЕ
	|	СУММА(ДвиженияСерийТоваров.КоличествоОборот) > 0
	|;
	|
	...
	|	СкладскиеДвиженияПроизводствоБезЗаказа.Характеристика,
	|	СкладскиеДвиженияПроизводствоБезЗаказа.Серия,
	|	СкладскиеДвиженияПроизводствоБезЗаказа.НомерГруппыЗатрат,
	|	СкладскиеДвиженияПроизводствоБезЗаказа.Количество
	|ИЗ
	|	СкладскиеДвиженияПроизводствоБезЗаказа КАК СкладскиеДвиженияПроизводствоБезЗаказа
	|
...
// Текст запроса отчет переработчика 2.5 с детализацией до номера группы затрат.
// 
// Возвращаемое значение:
//  Строка - Текст запроса производство без заказа
Функция ТекстЗапросаОтчетПереработчика2_5СДетализациейДоНомераГруппыЗатрат() Экспорт
	...
	|	ТаблицаТовары.НомерГруппыЗатрат КАК НомерГруппыЗатрат,
	|	СУММА(ТаблицаТовары.Количество) КАК Количество
	|ПОМЕСТИТЬ ТоварыОтчетПереработчика2_5
	|ИЗ
	...
	|	Товары.НомерГруппыЗатрат     КАК НомерГруппыЗатрат,
	|	Товары.Количество            КАК Количество
	|ПОМЕСТИТЬ ТаблицаТоварыОтчетПереработчика2_5
	|ИЗ
	|	ТоварыОтчетПереработчика2_5 КАК Товары
	|
	|";
