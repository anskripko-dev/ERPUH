...
Процедура НадписьЭтапыОплатыНажатие(Форма, Элемент, СтандартнаяОбработка, Оповещение = Неопределено) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма.Объект, "РежимИспользованияГрафика")
		...
		РежимИспользованияГрафика = Форма.Объект.РежимИспользованияГрафика;
	Иначе
		ДолжныФормироватьсяПозиции = Ложь;
		РежимИспользованияГрафика = Неопределено;
	КонецЕсли;
	ПараметрыОткрытия.Вставить("ДолжныФормироватьсяПозиции", ДолжныФормироватьсяПозиции);   
	ПараметрыОткрытия.Вставить("РежимИспользованияГрафика", РежимИспользованияГрафика); 	
	
	Если СтруктураПараметров.Свойство("ДатаЗаказа") Тогда
		ПараметрыОткрытия.Вставить("ДатаЗаказа", СтруктураПараметров.ДатаЗаказа);	
	КонецЕсли;
	#КонецОбласти
	...
	#Область УХ_Встраивание	  
	Если ТипЗнч(Форма.Объект.Ссылка) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда  
		
		АдресГрафикОплатыУХ = Неопределено;
		СтруктураПараметров.Свойство("АдресГрафикОплатыУХ", АдресГрафикОплатыУХ);
		ПараметрыОткрытия.Вставить("АдресВоВременномХранилищеГрафикОплатыУХ", АдресГрафикОплатыУХ);
	
		АдресТовары = Неопределено;
		СтруктураПараметров.Свойство("АдресТовары", АдресТовары);
		ПараметрыОткрытия.Вставить("АдресВоВременномХранилищеТовары", АдресТовары);      	
		
		ОткрытьФорму("Документ.ЗаказПоставщику.Форма.ГрафикОплатыУХ", ПараметрыОткрытия, Форма,,,,
...
Процедура НадписьЭтапыОплатыНажатиеЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	// УХ_Казначейство ++
	Если РезультатЗакрытия.Свойство("РежимИспользованияГрафика") Тогда
	    Форма.Объект.РежимИспользованияГрафика 	= РезультатЗакрытия.РежимИспользованияГрафика;
	КонецЕсли;
	// УХ_Казначейство --
...
Процедура ОбъектРасчетовНачалоВыбора(Форма, Элемент, СтандартнаяОбработка, ВыборОснованияПлатежа = ЛОЖЬ, ТекущееЗначение = Неопределено, ДополнительныеОтборы = Неопределено) Экспорт
	...
	Если ТипЗнч(Форма.Объект.Ссылка) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств")
		И ЗначениеЗаполнено(Форма.Объект.Договор)  Тогда
		ЗначенияОтбора.Вставить("Договор" , Форма.Объект.Договор); 
	КонецЕсли;
	#КонецОбласти
...
Процедура ВалютыИКурсДокументаНажатие(Форма, Элемент, СтандартнаяОбработка, Оповещение = Неопределено) Экспорт
	...
	#Область УХ_Встраивание
	ПараметрыОткрытия.Вставить("ВариантКурсаДоговораУХ",           ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Форма, СтруктураПараметров.ВариантКурсаДоговораУХ));
	ПараметрыОткрытия.Вставить("СпособОпределенияКурсаПлатежа",    ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Форма, СтруктураПараметров.СпособОпределенияКурсаПлатежа));
	#КонецОбласти	
