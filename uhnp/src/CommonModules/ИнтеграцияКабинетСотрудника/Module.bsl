...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий 		= НСтр("ru = 'Регистрация будущих событий применяемых вычетов НДФЛ для сервиса 1С:Кабинет сотрудника.';
											|en = 'Register future events of applicable personal income tax deductions for the 1C:Employee account service.'");
	...
	Обработчик.Комментарий 		= НСтр("ru = 'Начальное заполнение типов данных для оформления на работу.';
											|en = 'Initial filling of data types for employment registration.'");
	...
	Обработчик.Комментарий 		= НСтр("ru = 'Регистрация объектов, выгружаемых по ссылке в сервис 1С:Кабинет сотрудника.';
											|en = 'Register objects to export to 1C:Employee account by link.'");
	...
	Обработчик.Комментарий 		= НСтр("ru = 'Обновление применяемых вычетов НДФЛ в сервисе 1С:Кабинет сотрудника.';
											|en = 'Update the applicable personal income tax deductions in the 1C:Employee account service.'");
...
Процедура УстановитьБлокировкуПубликацииРЛ(ПараметрыБлокировки)
	...
	Исключение
		...
		|en = 'Cannot lock the data to publish payslips.
		|The publication might be already in progress for the selected employees. Try again later.'", ОбщегоНазначения.КодОсновногоЯзыка()); 
	...
	Если Письма.Количество() > 0 Тогда
		...
		Исключение
			ШаблонОписания = НСтр("ru = 'Ошибка отправки файлов справок.';
									|en = 'An error occurred while sending the certificate files.'");
			ПодробноеПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			Комментарий = СтрШаблон("%1%2%3", ШаблонОписания, Символы.ПС, ПодробноеПредставлениеОшибки);
			ЗаписьЖурналаРегистрации(ИменаСобытийЖР().ПрочиеСобытия, УровеньЖурналаРегистрации.Ошибка,,, Комментарий);
		КонецПопытки;
...
Функция ПараметрыПисьма(АдресПолучателя, ФайлыПечатныхФорм)
	...
	ПараметрыПисьма.Вставить("Тема", НСтр("ru = 'Документы';
											|en = 'Documents'") + ": " + СтрСоединить(ИменаФайлов, "; "));
