	...
	Если ЗначениеЗаполнено(ДокументыЭДОБЗК) Тогда
		...
		Если ЭлектронныеДокументы.ГоловныеОрганизации.Количество() > 1 Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Для подписания необходимо выбрать документы одной организации';
											|en = 'To sign, select documents of one company'"));
			Возврат;
		КонецЕсли;
		...
		Если ЭлектронныеДокументы.Организации.Количество() > 1 Тогда
			...
			НСтр("ru = 'Для подписания выбраны документы разных филиалов.
				...
				|Продолжить?';
				|en = 'Documents from different branches are selected for signing.
				|All the documents will be signed by one of the branches.
				|Continue?'");
		...
		Если ДокументыЗаявлений <> Неопределено
			...
			ТекстПредупреждения =
				НСтр("ru = 'Среди выбранных документов есть документы,
					...
					|организацией.';
					|en = 'The selected documents include documents
					|received from employees.
					|You can update only documents prepared
					|by the company.'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
			Возврат;
		КонецЕсли;
...
Процедура ОбработчикПодключаемойКомандыПроверитьФорматНаСоответствиеPDF_A_1(ДокументыЭДОБЗК, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗарплатаКадрыВызовСервера.ЭтоПолноправныйПользователь() Тогда
		ТекстПредупреждения = НСтр("ru = 'Для конвертации электронных документов необходимы полные права на базу данных';
									|en = 'To convert electronic documents, you need to have full database rights'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	...
	Если ДокументыЭДОБЗК.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru = 'Выберите строки документы которых хотите сконвертировать';
									|en = 'Select lines with documents that you want to convert'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
	КонецЕсли;
	...
	Если ДокументыЭДОБЗК.Количество() = 1 Тогда
		...
		Если ЭДОБЗКВызовСервера.ФорматЭлектронногоДокументаPDF_A_1(ДанныеДокумента) Тогда
			...
			ПоказатьПредупреждение(, НСтр("ru = 'Формат документа соответсвует требуемому формату (PDF/A-1A).';
											|en = 'The document format matches the required format (PDF/A-1A).'"));
		Иначе
			ТекстВопроса = НСтр("ru = 'При конвертации данные файла будут ПЕРЕЗАПИСАНЫ в требуемом формате (PDF/A-1A).
				...
				|en = 'During the conversion, the file data will be OVERWRITTEN in the required format (PDF/A-1A).
				|Once the conversion is completed, aks the responsible person of the company to resign it, and the document will be sent to the employees for preview.
				|
				|Convert the file?'");
	...
	Иначе
		ТекстВопроса =
			...
			|en = 'After the check, an attempt will be made to convert files whose format does not meet the requirements.
			|In this case, the file data will be OVERWRITTEN in the required format (PDF/A-1A).
			|The previous digital signatures will no longer be valid and will be DELETED.
			|Once the conversion is completed, ask the responsible person of the company to sign the print forms again, and they will be sent to employees for preview.
			|
			|Before the conversion, we strongly recommend that you BACK UP THE DATABASE.
			|We also recommend that you convert documents in small PORTIONS to avoid issues due to the lack of memory.
			|
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		Если РезультатПроверки <> Неопределено Тогда
			РезультатПроверки.Показать(
				СтрШаблон(НСтр("ru = 'Протокол проверки (конвертации) документов ЭДО на соответсвие формату PDF/A-1A от %1';
								|en = 'Check (conversion) protocol of EDI documents for compliance with PDF/A-1A format dated %1'"), Формат(ОбщегоНазначенияКлиент.ДатаСеанса(), "ДЛФ=DT")));
		КонецЕсли;
		ДополнительныеПараметры.Источник.Обновить();
	КонецЕсли;
КонецПроцедуры
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		Иначе
			...
			ТекстВопроса = НСтр("ru = 'Среди выбранных документов есть документы с печатными формами и документы с файлами.
				...
				|en = 'Among the selected documents there are documents with print forms and documents with files.
				|Within one iteration, you can sign documents with print forms or documents with files.'");
			
			КнопкиВопроса = Новый СписокЗначений;
			КнопкиВопроса.Добавить("ПодписатьДокументыСПечатнымиФормами", НСтр("ru = 'Подписать документы с печатными формами';
																				|en = 'Sign documents with print forms'"));
			КнопкиВопроса.Добавить("ПодписатьДокументыСФайлами", НСтр("ru = 'Подписать документы с файлами';
																		|en = 'Sign documents with files'"));
	...
	Если ЗначениеЗаполнено(СогласияНаПрисоединениеКЭДОБЗК) Тогда
		...
		Если Результат.Ошибка Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Для подписания необходимо выбрать ""Согласия на присоединение к ЭДО""
				...
				|en = 'To sign, select ""Consents to join EDI""
				|with the ""Signed with employee''s AQES"" receipt state. '"));
		Иначе
	...
	Если ЗначениеЗаполнено(СогласияНаПрисоединениеКЭДОБЗК) Тогда
		...
		Если Результат.Ошибка Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Для подтверждения получения сканкопий необходимо выбрать
				...
				|en = 'To confirm the receipt of scanned copies, select
				|""Consents to join EDI"" with the ""Scanned copies are received"" receipt state. '"));
		Иначе
	...
	Если ЗначениеЗаполнено(СогласияНаПрисоединениеКЭДОБЗК) Тогда
		...
		Если Результат.Ошибка Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Для подтверждения получения оригиналов необходимо выбрать
				...
				|en = 'To confirm the receipt of originals, select
				|""Consents to join EDI"" with the ""Pending original"" receipt state. '"));
		Иначе
			ЭДОБЗКВызовСервера.УстановитьСостояниеПриемаСогласийНаПрисоединениеКЭДОБЗК(
...
Процедура ПередПодписаниемФайловСертификатом(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(НаборДанных) Тогда
		...
		Состояние(НСтр("ru = 'Подпись уже установлена';
						|en = 'The signature is already installed'"));
	КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(ФайлыЭлектронныхДокументов) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Не задан электронный документ';
					|en = 'An electronic document is not specified'"), ДокументыЭДОБЗК[0], "ИмяФайлаСРасширением", "");
			Возврат;
		КонецЕсли;
...
Процедура НачатьПроверкуПодписей(ДанныеДляПроверкиПодписей, Индекс) Экспорт
	...
	Если ВыводитьСообщения Тогда
		Состояние(НСтр("ru = 'Проверка подписей';
						|en = 'Check signatures'"), , Строка(ДанныеЭлектронногоДокумента.ДокументЭДОБЗК));
	Иначе
		Состояние(НСтр("ru = 'Проверка подписей';
						|en = 'Check signatures'"), Цел(Индекс / ДанныеДляПроверкиПодписей.Количество() * 100), Строка(ДанныеЭлектронногоДокумента.ДокументЭДОБЗК));
	КонецЕсли;
