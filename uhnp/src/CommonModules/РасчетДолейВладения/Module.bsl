...
Функция ТекстЗапроса_ИзмененияПоследовательностей() Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НАЧАЛОПЕРИОДА(т.Период, ДЕНЬ) КАК ПериодДень,
	|	т.Период КАК Период,
	...
	|		ПО (т.Сценарий = &Сценарий)
	|			И (т.Инвестор = &Инвестор)
	|			И т.ОбъектИнвестирования = втДатыНачала.ОбъектИнвестирования
	|			И (т.Период МЕЖДУ втДатыНачала.ДатаНачала И &Период)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ПериодДень,
	|	ОбъектИнвестирования,
	...
	|	т.Период КАК Период,
	|	т.ЭффективнаяДоля КАК ЭффективнаяДоля,
	|	втВнутриДня.ПериодМаксимум КАК ПериодМаксимум,
	|	втВнутриДня._ПериодМинимум КАК _ПериодМинимум,
	|	втВнутриДня.ВозможноИзменениеПоследовательности КАК ВозможноИзменениеПоследовательности
	|ПОМЕСТИТЬ втИзмененияДолей
	|ИЗ
	|	втИзмененияДолейВозможноВнутриДня КАК т
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|			МАКСИМУМ(т.Регистратор ССЫЛКА Документ.ПоступлениеИнвестиций)
	|				И МАКСИМУМ(т.Регистратор ССЫЛКА Документ.ВыбытиеИнвестиций) КАК ВозможноИзменениеПоследовательности,
	|			т.ПериодДень КАК ПериодДень,
	|			т.ОбъектИнвестирования КАК ОбъектИнвестирования,
	|			МАКСИМУМ(т.Период) КАК ПериодМаксимум,
	|			МИНИМУМ(т.Период) КАК _ПериодМинимум,
	|			МАКСИМУМ(т.ЭффективнаяДоля) КАК _ЭффективнаяДоля,
	|			МАКСИМУМ(т.Регистратор) КАК _Регистратор
	|		ИЗ
	|			втИзмененияДолейВозможноВнутриДня КАК т
	|		
	|		СГРУППИРОВАТЬ ПО
	...
	|		
	|		ИМЕЮЩИЕ
	|			КОЛИЧЕСТВО(т.Регистратор) > 1) КАК втВнутриДня
	|		ПО т.ОбъектИнвестирования = втВнутриДня.ОбъектИнвестирования
	|			И т.ПериодДень = втВнутриДня.ПериодДень
	|ГДЕ
	|	ЕСТЬNULL(т.Период = втВнутриДня.ПериодМаксимум, ИСТИНА)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОбъектИнвестирования,
	|	Период
	...
	|	МАКСИМУМ(втИзмененияДолей.ВозможноИзменениеПоследовательности) КАК ВозможноИзменениеПоследовательности,
	...
	|			ТОГДА ВЫБОР
	|					КОГДА т.ВозможноИзменениеПоследовательности
...
Процедура СформироватьДвиженияПоследовательности(Движения, Реквизиты, ПоследовательностиУчастия, Отказ = Ложь) Экспорт
	...
	ОтношенияПоОбъектам = Новый Соответствие;
	Если Реквизиты.Свойство("ОбъектИнвестирования") Тогда
		ОтношенияПоОбъектам.Вставить(Реквизиты.ОбъектИнвестирования, Реквизиты.ОтношениеКГруппе);
	ИначеЕсли Движения.Свойство("ДвижениеИнвестиций") 
		И Движения.ДвижениеИнвестиций.Колонки.Найти("ОтношениеКГруппе") <> Неопределено Тогда
		
		Для Каждого СтрокаТаб Из Движения.ДвижениеИнвестиций Цикл
			ОтношенияПоОбъектам.Вставить(СтрокаТаб.ОбъектИнвестирования, СтрокаТаб.ОтношениеКГруппе);
		КонецЦикла;
		
	КонецЕсли;
	Период = ?(Реквизиты.Свойство("Период"), Реквизиты.Период, Реквизиты.Дата);
	ТабПАО = ПолучитьПАО();
	...
	Для каждого УзелИнвестор Из ПоследовательностиУчастия Цикл		
		Для каждого УзелОбъектИнвестирования Из УзелИнвестор.Значение Цикл
			...
			ОтношениеКГруппе = Неопределено;
			Если Инвестор = УзелИнвестор.Ключ Тогда 
				ОтношениеКГруппе = ОтношенияПоОбъектам.Получить(УзелОбъектИнвестирования.Ключ);
			КонецЕсли;
			
			Если ОтношениеКГруппе = Неопределено Тогда
			...
			Иначе	
				Движение.ОтношениеКГруппе = ОтношениеКГруппе;
			КонецЕсли;
		...
		Если ТекущийУзел.ИностранныйНалоговыйРезидент Тогда
			...
			Если ЗначениеЗаполнено(ТекущийУзел.СтранаРегистрации) Тогда
				...
				Если УчетМСФОПовтИспУХ.ЕстьРСБУ() Тогда
					МодульКИК = ОбщегоНазначения.ОбщийМодуль("РегистрыСведений.ГеографическиеПризнакиНалоговогоОсвобожденияКИК");
					ОформлениеКартинка = МодульКИК.ПолучитьФлагСтраны(ТекущийУзел.КодСтраны, КэшСтран);
				КонецЕсли;
...
Функция ПолучитьДанныеСтруктурыВладения(Сценарий, ДатаАктуальности, Моделирование = Ложь, ЭффективныеДоли = Ложь) Экспорт
	...
	Если Не ОбщегоНазначенияПовтИспУХ.ЕстьРСБУ() Тогда //ВЕ
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Т.Узел.КодОКОПФ", " """" "); // @query-part
	КонецЕсли;
...
Функция ПоследовательностьАктуальна(Период, ТекстОшибки = Неопределено) Экспорт
	...
	Если Метаданные.Последовательности.Найти("ДокументыДвиженияИнвестиций") = Неопределено Тогда
		Возврат Истина;	
	КонецЕсли;
	
	МодульПоследовательности = Последовательности["ДокументыДвиженияИнвестиций"];//ВЕ
	
	Если МодульПоследовательности.Проверить(Период) Тогда
		Возврат Истина;
	КонецЕсли;
	
	ШаблонОшибки = 
	НСтр("ru = 'Последовательность документов поступления и выбытия инвестиций актуальная на дату: %1. 
	|Необходимо восстановить последовательность'");
	
	ГраницаПоследовательности = МодульПоследовательности.ПолучитьГраницу();
	
	ТекстОшибки = СтрШаблон(ШаблонОшибки, Формат(ГраницаПоследовательности.Дата, "ДФ=dd.MM.yyyy"));
...
Процедура ЗаполнитьПоИнвесторуРекурсивно(ПоследовательностиУчастия, ТабПрямыеДоли, СтрокаИсточник, Знач ТекущаяПоследовательность, Кольца)
	...
	//Кто инвестировал в текущего инвестора
	...
	Для каждого СтрокаИнвестор Из СтрокиИнвестора Цикл
		...
		Если НоваяПоследовательность.Последовательность.Количество() Тогда
			...
			Если СтрокаИнвестор.СтрокаИзменение Тогда
				...
				//операция документа и выше по инвесторам:
				//последовательности с этой инвестицией изменились(ЕстьСтрокаИзменение=Истина)
				//поэтому их эффективные доли изменились - надо записать в регистр 
	...
	Для каждого УзелИнвестор Из ПоследовательностиУчастия Цикл
		Для каждого УзелОбъектИнвестирования Из УзелИнвестор.Значение Цикл
			...
			Для каждого УзелПоследовательность Из УзелОбъектИнвестирования.Значение Цикл
				...
				ГлубинаВыводаКолец = ГлубинаВыводаКолец(ИспользуемыеКольца, Кольца, ТабПрямыеДоли, ПоследовательностиУчастия, УзелПоследовательность, ГлубинаПоУмолчанию);	
				...
				Для НомерИтерации = 1 По КоличествоИтераций Цикл					
					...
					НоваяПоследовательность = ПолучитьНовуюПоследовательностьСКольцами(
						УзелПоследовательность,
						ИтераторКолец,
						ИспользуемыеКольца,
						ТабПрямыеДоли);
					...
					ТекПредставление = ПредставлениеПоследовательности(
						НоваяПоследовательность.Последовательность,
...
Функция ГлубинаВыводаКолец(ИспользуемыеКольца, Кольца, ТабПрямыеДоли, ПоследовательностиУчастия, УзелПоследовательность, ГлубинаПоУмолчанию)
	
	ГлубинаМинимальная = 2;//(высокая производительность)когда много объектов инвестирования
	ГлубинаМаксимальная = 8;//(точность - сотые доли процента)когда мало объектов инвестирования
	
	Если ТабПрямыеДоли.Количество() < 50 Тогда
		Возврат ГлубинаМаксимальная;
	КонецЕсли;
	
	Возврат ГлубинаМинимальная;
...
Функция ПолучитьИспользуемыеКольца(УзелПоследовательность, Кольца)
	...
	ТекПоследовательность = УзелПоследовательность.Значение.Последовательность;
	
	Для каждого ТекКольцо Из Кольца Цикл	
		
		СтрокаВставки = СтрокаПоследовательностиВставкиКольца(
			ТекПоследовательность, ТекКольцо.Кольцо);
...
Функция СтрокаПоследовательностиВставкиКольца(Последовательность, Кольцо)
	
	СтрокаПоиска = Последовательность.Найти(Кольцо[0], "ИнвесторПоследовательности");
	Если СтрокаПоиска <> Неопределено Тогда
		Возврат СтрокаПоиска;//перед найденным
	КонецЕсли;
	
	ДлинаПоследовательности = Последовательность.Количество();
	ПоследнийОИ = Последовательность.Получить(ДлинаПоследовательности-1);
	Если ПоследнийОИ.ОбъектИнвестированияПоследовательности = Кольцо[0] Тогда
		Возврат Истина;//после последнего элемента
	КонецЕсли;
	
	Возврат Неопределено;//кольцо не используется
	
КонецФункции

Функция ПолучитьНовуюПоследовательностьСКольцами(УзелПоследовательность, ИтераторКолец, ИспользуемыеКольца, ТабПрямыеДоли)
	
	//последовательность с кольцами не учитывается, если эффективная доля меньше точности
	ТочностьДоли = ТочностьДолиПоследовательностиСКольцами();
	...
	Для ИндексКольца = 0 По КоличествоКолец-1 Цикл // вставляем каждое кольцо
		...
		ПредСтрокаПоследовательности = СтрокаПоследовательностиВставкиКольца(
			НоваяПоследовательность.Последовательность, УзлыКольца);
			
		Если ПредСтрокаПоследовательности = Неопределено Тогда
			Продолжить; //инвестор из кольца не найден в последовательности
		ИначеЕсли ПредСтрокаПоследовательности = Истина Тогда	
			
			ИндексДляВставкиКольца = НоваяПоследовательность.Последовательность.Количество();
			ПоследняяСтрока = НоваяПоследовательность.Последовательность.Получить(ИндексДляВставкиКольца-1);
			ПредСтрокаПоследовательности = Новый Структура(
				"ИнвесторПоследовательности", ПоследняяСтрока.ОбъектИнвестированияПоследовательности);
		...
		Для НомерПовтораКольца = 1 По ЭлементИтератора Цикл // повторяем кольцо
			...
			ВставитьКольцоПовтораВПоследовательность(
				...
				УзлыКольца,
				ТабПрямыеДоли,
				ПредСтрокаПоследовательности,
				ИндексДляВставкиКольца);
	...
	ПроверитьТочность = ЕстьНеобязательныеПовторыКолец(ИтераторКолец);
...
Функция ЕстьНеобязательныеПовторыКолец(ИтераторКолец)
	
	Возврат ОбщегоНазначенияКлиентСерверУХ.МаксимумВМассиве(ИтераторКолец)>1;

КонецФункции

Функция ТочностьДолиПоследовательностиСКольцами()
				...
				Если СтрокиПред.Количество() Тогда
					...
					Для Каждого СтрокаПред Из СтрокиПред Цикл
						...
						Если УзелПриемник = Неопределено Тогда
							Сообщение = НСтр("ru = 'Не найден узел приемник: <%1>'");
							Сообщить(СтрШаблон(Сообщение, СтрокаПред.Приемник));
							Продолжить;
						КонецЕсли;
