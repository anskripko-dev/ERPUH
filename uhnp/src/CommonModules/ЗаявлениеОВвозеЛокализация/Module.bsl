...
Процедура СформироватьТекстОтраженияВРеглУчете(ТекстЗапроса) Экспорт
	...
	// Неотфактуровка
	ТекстыОтражения.Добавить(ТекстОтклонениеПриФактуровкеТоваровРабот());
	ТекстыОтражения.Добавить(ТекстОтклонениеПриФактуровкеУслуг());
...
Функция ВыгрузитьЗаявлениеОВвозеТоваров(УникальныйИдентификатор, Объект, ТолькоПроверка = Ложь) Экспорт
	...
	Текст = ?(НЕ ТолькоПроверка, НСтр("ru = 'Документ не записан. Выгрузка прервана.';
	...
	Текст = ?(НЕ ТолькоПроверка, НСтр("ru = 'Выгрузка прервана.';
...
Функция ТекстОтклонениеПриФактуровкеТоваровРабот()
	...
	|	ДоговорНеотфактуровки.ГруппаФинансовогоУчета							КАК АналитикаУчетаКт,
	|	НЕОПРЕДЕЛЕНО		 													КАК МестоУчетаКт,
	|
	|	НЕОПРЕДЕЛЕНО 									КАК ВалютаКт,
	|	ДоговорНеотфактуровки.Подразделение				КАК ПодразделениеКт,
	|	ДоговорНеотфактуровки.НаправлениеДеятельности	КАК НаправлениеДеятельностиКт,
	|	НЕОПРЕДЕЛЕНО									КАК СчетКт,
	|
	|	ДоговорНеотфактуровки.Контрагент	КАК СубконтоКт1,
	|	ДоговорНеотфактуровки.Ссылка		КАК СубконтоКт2,
	...
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СебестоимостьТоваров КАК Строки
	|	ПО ДокументыКОтражению.Ссылка = Строки.Регистратор
	|		И Строки.Количество = 0 И НЕ Строки.РасчетСебестоимости
	|		И Строки.Активность И НЕ Строки.Сторно И Строки.РасчетПартий
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорНеотфактуровки
	|	ПО Аналитика.МестоХранения = ДоговорНеотфактуровки.Ссылка
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК КорАналитика
	|	ПО Строки.КорАналитикаУчетаНоменклатуры = КорАналитика.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Назначения КАК Назначения
	|	ПО КорАналитика.Назначение = Назначения.Ссылка";

	Возврат ТекстЗапроса;
	
КонецФункции

Функция ТекстОтклонениеПриФактуровкеУслуг()
	
	ТекстЗапроса =
	"ВЫБРАТЬ //// Отклонения в стоимости при фактуровке по услугам (Дт 25, 26, 20 :: Кт 60.НП)
	|	Строки.Регистратор КАК Ссылка,
	|	Строки.Период КАК Период,
	|	Строки.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	...
	|	0 КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|	Расходы.СуммаРегл - Расходы.ПостояннаяРазница - Расходы.ВременнаяРазница КАК СуммаНУДт,
	|	Расходы.ПостояннаяРазница КАК СуммаПРДт,
	...
	|	ПО Расходы.Регистратор = ДокументыКОтражению.Ссылка
	...
	|	ПО Расходы.АналитикаРасходов = ОбъектыСтроительства.Ссылка";
