...
Процедура СформироватьКомплектОтчетов(Параметры, АдресХранилища) Экспорт
	...
	ВсегоЭкземпляров = Параметры.ЭкземплярыОтчетов.Количество();
	Счетчик = 1;
	
	СообщениеПрогресса = НСтр("ru = 'Формирование комплектов отчетов, пожалуйста подождите...';
								|en = 'Generating report sets. Please, wait...'") + Символы.ПС;
	...
	Для Каждого Экземпляр Из Параметры.ЭкземплярыОтчетов Цикл
		...
		ДлительныеОперации.СообщитьПрогресс(Цел((Счетчик/ВсегоЭкземпляров)*100), 
			СообщениеПрогресса + "Текущий отчет: " + Экземпляр.Ключ);
...
Процедура ВывестиПоказатель(Параметры, ЭлементВывода, ДокументРезультат)
	...
	МакетЯчейки.ТекущаяОбласть.Формат = Параметры.ФорматЗначений;
...
Процедура ВывестиЯчейкиСтроки(ДанныеСтрокиТаблицы, ДокументРезультат, ОписаниеТаблицы, Параметры, Уровень)
	...
	Для Каждого Колонка Из ОписаниеТаблицы.КолонкиОтчета Цикл
		...
		КонецЕсли;
		МакетЯчейки.ТекущаяОбласть.Формат = ФорматнаяСтрока;
		ОбластьМакета = ДокументРезультат.Присоединить(МакетЯчейки);
			...
			Если СтрокаДляРасшифровки.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.Измерение Тогда
				...
				Если ЗначениеЗаполнено(ИмяКолонкиГруппировки) Тогда
					...
					Если ОписаниеПоказателя <> Неопределено Тогда
						СубконтоПоказателя = ОписаниеПоказателя.Субконто; // Структура из КлючИЗначение
						Если СубконтоПоказателя.Свойство(ИмяКолонкиГруппировки) Тогда
							Субконто = СубконтоПоказателя[ИмяКолонкиГруппировки]; // Структура из КлючИЗначение
						...
						Для каждого ОтборСтроки из Расшифровка.Отбор Цикл
							Если СубконтоПоказателя.Свойство(ОтборСтроки.Ключ) Тогда
								Субконто = СубконтоПоказателя[ОтборСтроки.Ключ]; // Структура из КлючИЗначение
								Если СписокВидовСубконто.НайтиПоЗначению(Субконто.Вид) = Неопределено Тогда
									СписокВидовСубконто.Добавить(Субконто.Вид);
								КонецЕсли;
							КонецЕсли;
...
Процедура УстановитьГруппировкуВалюты(КомпоновщикНастроек, Включить)
	...
	Если ГруппировкаВалюты <> Неопределено Тогда
		...
		Для Каждого ПодчиненнаяГруппировка Из ГруппировкаВалюты.Структура Цикл
			СуществующаяГруппировка = Неопределено;
			Для Каждого Элемент Из ПодчиненнаяГруппировка.ПоляГруппировки.Элементы Цикл
				НайтиГруппировкуПоПолю(ГруппировкаРодитель.Структура, Строка(Элемент.Поле), СуществующаяГруппировка, ГруппировкаВалюты);
				Прервать;
			КонецЦикла;
			Если Не СуществующаяГруппировка = Неопределено Тогда 
				ГруппировкаРодитель.Структура.Удалить(СуществующаяГруппировка);
			КонецЕсли;
			Если НЕ ГруппировкаВалюты.Использование Тогда
...
Процедура НайтиГруппировкуПоПолю(Группировки, ИмяПоля, НайденнаяГруппировка, ГруппировкаИсключение = Неопределено)
	
	Для каждого Группировка Из Группировки Цикл

		Если Группировка = ГруппировкаИсключение Тогда
			Продолжить;
		КонецЕсли;
		...
		Если КоличествоСубконто < МаксКоличествоСубконто Тогда 
			...
			Если ГруппировкаСубконто <> Неопределено Тогда
				...
				Если ТипЗнч(ОсновнойНаборДанных) = Тип("НаборДанныхОбъединениеСхемыКомпоновкиДанных") Тогда
					ТекстЗапросаОстатки = ОсновнойНаборДанных.Элементы.Найти("Остатки_Международный").Запрос;
					ТекстЗапросаОбороты = ОсновнойНаборДанных.Элементы.Найти("Обороты_Международный").Запрос;
					...
					КонецЦикла;
					ОсновнойНаборДанных.Элементы.Найти("Остатки_Международный").Запрос = ТекстЗапросаОстатки;
					ОсновнойНаборДанных.Элементы.Найти("Обороты_Международный").Запрос = ТекстЗапросаОбороты;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
