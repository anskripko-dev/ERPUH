...
Функция СоздатьЗаписьОтправленныеЗадачи(ЗадачаВход, ПользовательВход, ИдентификаторСообщенияВход = Неопределено) Экспорт
	...
	КонецЕсли;
	Попытка
		Если НЕ ЗадачаВход.ПометкаУдаления Тогда
...
Функция ПараметрыОповещения() Экспорт
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("КатегорияОповещений", Неопределено);
	ПараметрыОповещения.Вставить("ВидОповещения", Неопределено);
	ПараметрыОповещения.Вставить("Пользователь", Неопределено);
	ПараметрыОповещения.Вставить("Сообщение", "");
	ПараметрыОповещения.Вставить("Задача", Справочники.Задачи.ПустаяСсылка());
	ПараметрыОповещения.Вставить("ТекстПочтовогоСообщения", "");
	ПараметрыОповещения.Вставить("Вложение", Неопределено);
	ПараметрыОповещения.Вставить("ИдентификаторОповещения", Новый УникальныйИдентификатор);
	
	Возврат ПараметрыОповещения;

КонецФункции

Процедура СоздатьОповещениеПользователю(СтруктураПараметров, Отказ=Ложь) Экспорт
	...
	ТекстОшибки = "<"+Строка(СтруктураПараметров.ВидОповещения)+">"
		+"<"+Строка(Пользователь)+">";

	ОповещениеОбъект = РегистрыСведений.ОповещенияПользователей.СоздатьМенеджерЗаписи();
	ЗаполнитьЗначенияСвойств(ОповещениеОбъект, СтруктураПараметров);
	
	ДокФормат = Новый ФорматированныйДокумент;
	ДокФормат.УстановитьHTML(СтруктураПараметров.Сообщение, Новый Структура);
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось записать оповещение пользователю ""%1"", по причине: %2'"),
			ТекстОшибки,
	...
	ДанныеПользователя = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Пользователь, 
		"ОповещатьЧерезСистемуВзаимодействия,ОповещатьЧерезТелеграм,ОповещатьПоEmail"); 
		
	Если ДанныеПользователя.ОповещатьЧерезСистемуВзаимодействия Тогда
		Попытка
			СистемаВзаимодействияСерверУХ.СоздатьСообщение(Новый Структура("ФорматированныйДокумент, ТекстСообщения, Пользователь", ДокФормат, ОповещениеОбъект.Тема, Пользователь));
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось создать уведомление пользователю ""%1"", по причине: %2'"),
				ТекстОшибки,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			ОбщегоНазначенияУХ.СообщитьОбОшибке(ТекстСообщения, Отказ);
		КонецПопытки
	...
	Если ДанныеПользователя.ОповещатьЧерезТелеграм Тогда
		ТелеграмСерверУХ.СоздатьСообщение(
			Новый Структура("ФорматированныйДокумент, ТекстСообщения, Пользователь, ИдентификаторОповещения, Источник",
				ДокФормат, ОповещениеОбъект.Тема, Пользователь, Строка(СтруктураПараметров.ИдентификаторОповещения), СтруктураПараметров.Источник), Отказ);
	КонецЕсли;
	
	Если ДанныеПользователя.ОповещатьПоEmail Тогда
		...
		Если НЕ СтруктураПараметров.Вложение = Неопределено Тогда
			Письмо.Вложения.Добавить(СтруктураПараметров.Вложение);                
		КонецЕсли;	
		
		Если СтруктураПараметров.ТекстПочтовогоСообщения = "" Тогда
			// Специализированный текст почтового сообщения не задан. Укажем тот же, что и для оповещения в системе.
			ТекстОтправки = СтруктураПараметров.Сообщение;					
		Иначе
			// Текст для почтового сообщения задан отдельно. Укажем его.
			ТекстОтправки = СтруктураПараметров.ТекстПочтовогоСообщения;		
		КонецЕсли;
		...
		Письмо.Тема = НСтр("ru = 'Оповещение 1С:УХ: '") + Строка(СтруктураПараметров.ВидОповещения);
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось записать уведомление по электронной почте пользователю ""%1"", по причине: %2'"),
				ТекстОшибки,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			ОбщегоНазначенияУХ.СообщитьОбОшибке(ТекстСообщения, Отказ);
		КонецПопытки;
...
Функция ПодготовитьТекстОповещенияПоШаблону(ТекстШаблона,ПараметрыПроцесса=Неопределено,ОбъектССылка,ФлОшибки=Ложь) Экспорт
	...
	ТекстШаблонаИтог = ТекстШаблонаИтог + СпецСимволШапкаСообщения;
...
Процедура СоздатьСобытиеОповещения(Параметры)
	...
	СоздатьНапоминание	 = Параметры.СоздатьНапоминание;
	КатегорияСобытия	 = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидСобытия, "КатегорияСобытия");
	...
	Для Каждого ТекВыгрузка Из Выгрузка Цикл
		...
		Для Каждого Пользователь Из ТаблицаОповещаемых Цикл
			...
			Если ЗначениеЗаполнено(ЗадачаВход) И ЗначениеЗаполнено(ВидСобытия) И (НЕ ЭтоНапоминание) Тогда
				Если ПользовательПолучилОповещениеОЗадаче(Пользователь.Получатель, ЗадачаВход, ВидСобытия) Тогда
					// Не требуется отправка такого оповещения
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			...
			Если ТипЗнч(Пользователь.Получатель) = Тип("СправочникСсылка.Пользователи") Тогда
		...
		Если СоздатьНапоминание И СписокОповещаемых.Количество() > 0 Тогда
			СоздатьНапоминание(ВидСобытия, ИсточникСсылка, СписокОповещаемых);
		КонецЕсли;
...
Функция ОповеститьПользователей(ВидСобытия, ШаблонОповещения = Неопределено, ИсточникСсылка = Неопределено, 
	СписокРассылки = Неопределено, ДопПараметры = Неопределено, ЗадачаВход = Неопределено, ЭтапПроцессаВход = Неопределено,
	СоздатьНапоминание = Ложь) Экспорт
	...
	Попытка
		...
		ПараметрыЗадания.Вставить("СоздатьНапоминание",	СоздатьНапоминание);
		...
		Если ДопПараметры <> Неопределено тогда
			...
			ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДопПараметры, "Этап") тогда
				ПараметрыЗадания.Вставить("ТекущийЭтап", ДопПараметры.Этап);
			КонецЕсли;
		КонецЕсли;
...
Функция СоздатьНапоминание(ВидСобытия, ИсточникСсылка, СписокРассылки, ТекущийДокументПроцесса = Неопределено) Экспорт
	...
	ИначеЕсли ВидСобытия = Справочники.ВидыСобытийОповещений.ЗапросНалоговогоМониторинга Тогда	
		ТипОбъектаОповещения = ПолучитьТипОбъектаОповещенияПоСсылке(ИсточникСсылка);
		СтруктураНастроек = ПолучитьНастройкиОповещенийПоВидуСобытияИТипуОбъекта(ВидСобытия, ТипОбъектаОповещения);
		
		Если СтруктураНастроек.ОповещениеВключено тогда
			
			Для Каждого ПользовательРассылки Из СписокРассылки Цикл
				
				УдалитьНапоминанияПоОтбору(Новый Структура("Источник, ВидСобытияОповещения", ИсточникСсылка, ВидСобытия));
				
				СтруктураНапоминание = СоздатьСтруктуруНапоминанияПоУмолчанию(СтруктураНастроек, ПользовательРассылки, ИсточникСсылка.Дата, ИсточникСсылка);
		...
		КонецЕсли;	
	Иначе
		ТекущийЭтап = МодульУправленияПроцессамиУХ.ПолучитьТекущийЭтапСогласования(ИсточникСсылка, ТекущийДокументПроцесса);
...
Функция ПолучитьТаблицуШаблоновПоПараметрамРассылки(ВидСобытияВход, ИсточникСсылкаВход, ТипОбъектаОповещения)  Экспорт
	...
	Если КатегорияСобытия = Справочники.КатегорииСобытийОповещений.СобытияВыполненияПроцесса Тогда
		...
		Если ТипЗнч(ИсточникСсылкаВход) = Тип("ДокументСсылка.ЭкземплярПроцесса") Тогда
	...
	ИначеЕсли КатегорияСобытия = Справочники.КатегорииСобытийОповещений.СобытияСогласования Тогда
		...
		Если ДействияСогласованиеУХСервер.ЭтоЗаявкаНаОперацию(ИсточникСсылкаВход) Тогда
	...
	ИначеЕсли КатегорияСобытия = Справочники.КатегорииСобытийОповещений.СобытияПроцессаПодготовкиОтчетности Тогда
		...
		Запрос.УстановитьПараметр("ТипОбъектаОповещения", ТипОбъектаОповещения);
		Если ТипЗнч(ИсточникСсылкаВход) = Тип("СправочникСсылка.ЭтапыУниверсальныхПроцессов")
			И ЗначениеЗаполнено(ИсточникСсылкаВход) Тогда
	...
	ИначеЕсли КатегорияСобытия = Справочники.КатегорииСобытийОповещений.АккредитацияПоставщиков Тогда
		...
		Если ВидСобытияВход = Справочники.ВидыСобытийОповещений.ОповещениеОбОткрытииДоступаВнешнемуПоставщику ИЛИ
			...
			Запрос.УстановитьПараметр("ТипОбъектаОповещения", ТипОбъектаОповещения);		ИначеЕсли ВидСобытияВход = Справочники.ВидыСобытийОповещений.ОповещениеОВозможностиУчаствоватьВЛотеВнешнемуПоставщику ИЛИ
		...
		ИначеЕсли ВидСобытияВход = Справочники.ВидыСобытийОповещений.ОповещениеОбОтменеАккредитации Тогда
			...
			Запрос.УстановитьПараметр("ТипОбъектаОповещения", ТипОбъектаОповещения);     
		ИначеЕсли ВидСобытияВход = Справочники.ВидыСобытийОповещений.ОповещениеОНеобходимостиПроверкиАккредитацииПоставщика Тогда
			ТипОбъектаОповещения = Справочники.ДокументыБД.НайтиПоНаименованию("АккредитацияПоставщика", , , ТекущаяИБ);
			Запрос.УстановитьПараметр("ТипОбъектаОповещения", ТипОбъектаОповещения);    
		ИначеЕсли ВидСобытияВход = Справочники.ВидыСобытийОповещений.ОповещениеОНеобходимостиПроверкиКвалификацииПоставщика Тогда
			ТипОбъектаОповещения = Справочники.ДокументыБД.НайтиПоНаименованию("АккредитацияПоставщика", , , ТекущаяИБ);
			Запрос.УстановитьПараметр("ТипОбъектаОповещения", ТипОбъектаОповещения);     
		Иначе
			ТипОбъектаОповещения = Справочники.ДокументыБД.НайтиПоНаименованию("АккредитацияПоставщика", , , ТекущаяИБ);
	...
	ИначеЕсли КатегорияСобытия = Справочники.КатегорииСобытийОповещений.Напоминания Тогда
		...
		Если ВидСобытияВход = Справочники.ВидыСобытийОповещений.Напоминание_СтадияМероприятия Тогда
			Если ОбщегоНазначенияУХ.ПроверитьТипЗначенияВПодсистеме(ИсточникСсылкаВход, "ДокументСсылка.Мероприятие", "ПодсистемыУХ.ИнтеграцияУправлениеРисками") Тогда
	...
	ИначеЕсли КатегорияСобытия = Справочники.КатегорииСобытийОповещений.СобытияПоКорректировкамЛимитов Тогда
		...
		Запрос.УстановитьПараметр("ТипОбъектаОповещения",ТипОбъектаОповещения);
		Запрос.УстановитьПараметр("ВидОбъектаОповещения",ВидОбъектаОповещения);  
	ИначеЕсли КатегорияСобытия = Справочники.КатегорииСобытийОповещений.ОценкаАльтернатив Тогда
		ТипОбъектаОповещения = ПолучитьТипОбъектаОповещенияПоСсылке(ИсточникСсылкаВход);
		Запрос.УстановитьПараметр("ТипОбъектаОповещения",ТипОбъектаОповещения);
		Запрос.УстановитьПараметр("ВидОбъектаОповещения",ВидОбъектаОповещения);  
...
// УДАЛИТЬ: Нигде не вызывается
Процедура ОповеститьПользователейВФоне(Параметры, АдресХранилища, АдресХранилищаДополнительный, Отказ=Ложь) Экспорт
	...
	ПараметрыОповещения = ПараметрыОповещения();
	ПараметрыОповещения.Вставить("КатегорияОповещений", КатегорияСобытия);
	ПараметрыОповещения.Вставить("ВидОповещения", ВидСобытия);
	ПараметрыОповещения.Вставить("Задача", ЗадачаВход);
	...
	Для Каждого ТекВыгрузка Из Выгрузка Цикл
		...
		Для Каждого Пользователь Из ТаблицаОповещаемых Цикл
			...
			ПараметрыОповещения.Вставить("Пользователь", Пользователь.Получатель);
			ПараметрыОповещения.Вставить("Сообщение", ТекстСообщенияСистемы);
			ПараметрыОповещения.Вставить("ТекстПочтовогоСообщения", ТекстПочтовогоСообщения);
			
			СоздатьОповещениеПользователю(ПараметрыОповещения, Отказ);
...
Функция ПолучитьПоставляемыеДанныеПоВидуСобытия(ВидСобытияВход, КатегорияВход = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ВидСобытияВход) ИЛИ ЗначениеЗаполнено(КатегорияВход) Тогда
		...
		Если ВидСобытияСтрока <> "" Тогда
			НайденнаяСтрока = ПоставляемыеНастройки.Найти(ВидСобытияСтрока, "ВидСобытия");
			Если НайденнаяСтрока <> Неопределено Тогда
				// Считаем данные из таблицы.
				ТекстКатегорияСобытия        = НайденнаяСтрока.КатегорияСобытия;
				ТекстШаблонОповещения        = НайденнаяСтрока.ШаблонОповещенияПоУмолчанию;
				ТекстДополнительныеПараметры = НайденнаяСтрока.ДополнительныеПараметры;
				ТекстОписание                = НайденнаяСтрока.Описание;
		...
		Если (НЕ СобытиеНайдено) ИЛИ (РезультатФункции.ШаблонОповещенияПоУмолчанию = "") Тогда
			...
			Если КатегорияСобытияСтрока <> "" Тогда
				ПараметрыОтбора = Новый Структура;
				ПараметрыОтбора.Вставить("ВидСобытия",       "-");
				ПараметрыОтбора.Вставить("КатегорияСобытия", КатегорияСобытияСтрока);
				НайденныеСтроки = ПоставляемыеНастройки.НайтиСтроки(ПараметрыОтбора);
				Для Каждого Стр Из НайденныеСтроки Цикл
					// Считаем данные из таблицы.
					ТекстШаблонОповещения        = Стр.ШаблонОповещенияПоУмолчанию;
					ТекстДополнительныеПараметры = Стр.ДополнительныеПараметры;
					ТекстОписание                = Стр.Описание;
...
Функция ПолучитьОповещаемыхПоУмолчанию(КатегорияСобытия,ВидСобытия,ИсточникСсылка,ИсходныйСписокРассылки)
	...
	ИначеЕсли ТипЗнч(ИсточникСсылка) = Тип("ДокументСсылка.НастраиваемыйОтчет") Тогда
		// В настраиваемом отчете вместо реквизита Ответственный используем Исполнителя из регламента
		Исполнитель = УправлениеОтчетамиУХ.ПолучитьОтветственногоЗаТипОбъекта(ИсточникСсылка,,,"Исполнитель");
	Иначе
	...
	Если КатегорияСобытия = Справочники.КатегорииСобытийОповещений.СобытияВыполненияПроцесса Тогда
		...
		Если ЗначениеЗаполнено(ИсточникСсылка) Тогда
			Если ТипЗнч(ИсточникСсылка) = Тип("СправочникСсылка.ШаблоныУниверсальныхПроцессов") Тогда
				Исполнитель = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИсточникСсылка, "АдминистраторПроцесса");
			ИначеЕсли ТипЗнч(ИсточникСсылка) = Тип("ДокументСсылка.ЭкземплярПроцесса") Тогда
				Исполнитель = ИсточникСсылка.ШаблонПроцесса.АдминистраторПроцесса;
			КонецЕсли;
		КонецЕсли;
		
	ИначеЕсли КатегорияСобытия = Справочники.КатегорииСобытийОповещений.СобытияСогласования Тогда
		
		Если ВидСобытия = Справочники.ВидыСобытийОповещений.ТребуетсяСогласование
			ИЛИ ВидСобытия = Справочники.ВидыСобытийОповещений.ЭтапПринятКИсполнению 
			ИЛИ ВидСобытия = Справочники.ВидыСобытийОповещений.ОтмененоПринятиеЭтапаКИсполнению Тогда
			Возврат Получатели;
		КонецЕсли;
...
Функция ПолучитьТипОбъектаПоВидуОповещения(ВидСобытияВход) Экспорт
	...
	ИначеЕсли ВидСобытияВход = ПредопределенноеЗначение("Справочник.ВидыСобытийОповещений.ОповещениеОбУтвержденииАккредитации") ИЛИ
			  ...
			  ВидСобытияВход = ПредопределенноеЗначение("Справочник.ВидыСобытийОповещений.ОповещениеОбОтправкеНаИсправлениеАккредитации") ИЛИ 
			  ВидСобытияВход = ПредопределенноеЗначение("Справочник.ВидыСобытийОповещений.ОповещениеОНеобходимостиПроверкиАккредитацииПоставщика") Тогда
...
Функция ПолучитьТаблицуНаступившихСобытий(ДатаВход = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|	МАКСИМУМ(ЕСТЬNULL(ОтложеннаяОбработкаЭтаповПроцессов.Выполнено, ЛОЖЬ)) КАК Выполнено
		|ПОМЕСТИТЬ ВТНапоминанияПользователя
		|ИЗ
		|	РегистрСведений.НапоминанияПользователяУХ КАК НапоминанияПользователя
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОтложеннаяОбработкаЭтаповПроцессов КАК ОтложеннаяОбработкаЭтаповПроцессов
		|		ПО НапоминанияПользователя.Задача = ОтложеннаяОбработкаЭтаповПроцессов.Задача
		|ГДЕ
		...
		|	НапоминанияПользователя.НачалоНапоминаний
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТНапоминанияПользователя.Пользователь КАК Пользователь,
		|	ВТНапоминанияПользователя.ВремяСобытия КАК ВремяСобытия,
		|	ВТНапоминанияПользователя.Источник КАК Источник,
		|	ВТНапоминанияПользователя.ВидСобытияОповещения КАК ВидСобытияОповещения,
		|	ВТНапоминанияПользователя.СрокНапоминания КАК СрокНапоминания,
		|	ВТНапоминанияПользователя.ИнтервалВремениНапоминания КАК ИнтервалВремениНапоминания,
		|	ВТНапоминанияПользователя.ВремяПоследнегоНапоминания КАК ВремяПоследнегоНапоминания,
		|	ВТНапоминанияПользователя.Описание КАК Описание,
		|	ВТНапоминанияПользователя.Задача КАК Задача,
		|	ВТНапоминанияПользователя.НачалоНапоминаний КАК НачалоНапоминаний,
		|	НапоминанияПользователя.ПараметрыСобытия КАК ПараметрыСобытия
		|ИЗ
		|	ВТНапоминанияПользователя КАК ВТНапоминанияПользователя
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НапоминанияПользователяУХ КАК НапоминанияПользователя
		|		ПО ВТНапоминанияПользователя.Пользователь = НапоминанияПользователя.Пользователь
		|			И ВТНапоминанияПользователя.ВремяСобытия = НапоминанияПользователя.ВремяСобытия
		|			И ВТНапоминанияПользователя.Источник = НапоминанияПользователя.Источник
		|			И ВТНапоминанияПользователя.ВидСобытияОповещения = НапоминанияПользователя.ВидСобытияОповещения
		|			И ВТНапоминанияПользователя.СрокНапоминания = НапоминанияПользователя.СрокНапоминания
		|			И ВТНапоминанияПользователя.ИнтервалВремениНапоминания = НапоминанияПользователя.ИнтервалВремениНапоминания
		|			И ВТНапоминанияПользователя.ВремяПоследнегоНапоминания = НапоминанияПользователя.ВремяПоследнегоНапоминания
		|			И ВТНапоминанияПользователя.Описание = НапоминанияПользователя.Описание
		|			И ВТНапоминанияПользователя.Задача = НапоминанияПользователя.Задача
		|			И ВТНапоминанияПользователя.НачалоНапоминаний = НапоминанияПользователя.НачалоНапоминаний
		|ГДЕ
		|	ВТНапоминанияПользователя.Выполнено = ЛОЖЬ";
...
Процедура ОбновитьОповещенияПоНапоминаниям(ПользовательВход, ИсточникВход, ВидСобытияОповещенияВход, ЗадачаВход, ДопПараметрыВход = Неопределено)
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		#Область УХ_НалоговыйМониторинг		
		Если ОбщегоНазначенияУХ.ПроверитьТипЗначенияВПодсистеме(ИсточникВход, "ДокументСсылка.ЗапросДанныхНалоговогоМониторинга", "ПодсистемыУХ.ИнтеграцияНалоговыйМониторинг") Тогда
			...
			Пока ВыборкаЗапроса.Следующий() Цикл
				Если МассивОповещаемых.Найти(ВыборкаЗапроса.Пользователь) = Неопределено Тогда
					МассивОповещаемых.Добавить(ВыборкаЗапроса.Пользователь);
				КонецЕсли;
			КонецЦикла;
			
			БотЗадачИОповещений = СистемаВзаимодействияСерверУХ.ПолучитьПользователяБотаЗадачИОповещений();
			Если МассивОповещаемых.Найти(БотЗадачИОповещений) = Неопределено Тогда
				МассивОповещаемых.Добавить(БотЗадачИОповещений);
			КонецЕсли;
...
Функция СоздатьСтруктуруНапоминанияПоУмолчанию(СтруктураНастроекВход, ПользовательВход, ВремяСобытияВход, ИсточникСобытияВход) Экспорт
	...
	НовыйСрокНапоминания = ВремяСобытияВход - (НовыйПериод*60*60*24);
...
#Область Документ_Мероприятие

Процедура Документ_Мероприятие_ПриЗаписи(Объект) Экспорт
	...
	СтруктураНастроек = ПолучитьНастройкиОповещенийПоВидуСобытия(ВидСобытияСтадияМероприятия, Объект.ВидМероприятия);
	...
	Если (ЕстьНастройкиОповещения) Тогда
		Для Каждого ТекСтадии Из Объект.Стадии Цикл
			Если (ТекСтадии.НапомнитьОСтадии) И (НЕ ТекСтадии.Завершено) Тогда
				...
				ПользовательАдресат = Объект.Ответственный;
				...
				СтруктураНапоминание = СоздатьСтруктуруНапоминанияПоУмолчанию(СтруктураНастроек, ПользовательАдресат, ДатаСтадии, Объект.Ссылка);
				СтруктураНапоминание.Вставить("ДопПараметры", СтруктураДопПараметры);
				ДобавитьНапоминаниеПользователяСЗадачей(СтруктураНапоминание);
			КонецЕсли;
		КонецЦикла;	
