...
Процедура ПроверитьДоговорНаВозможностьКорреспонденцииСоСчетомЦифровогоРубля(ДоговорОбъект, Отказ) Экспорт
	...
	Если Не ПараметрыКорреспонденции.ДопустимаКорреспонденцияСоСчетомЦифровогоРубля Тогда
		...
		ТекстОшибки = НСтр("ru = 'Расчетный счет не может корреспондировать со счетом цифрового рубля.
		                         |Укажите другой счет';
		                         |en = 'Расчетный счет не может корреспондировать со счетом цифрового рубля.
		                         |Укажите другой счет'");
...
Процедура ПроверитьКорреспонденциюБанковскихСчетов(ДокументОбъект, Отказ, ОшибкиЗаполнения = "") Экспорт
	...
	Если ЭтоЗапрещеннаяОперация И КорреспонденцияСЦифровымСчетом Тогда
		...
		Текст = СтрШаблон(НСтр("ru = 'Счет цифрового рубля не может быть использован для хозяйственной операции ""%1"".
		                             |Укажите другой счет';
		                             |en = 'Счет цифрового рубля не может быть использован для хозяйственной операции ""%1"".
		                             |Укажите другой счет'"),
...
Процедура ПроверитьЗаполнениеНалоговыхРеквизитов(Объект, Отказ, НепроверяемыеРеквизиты, ФлагОбменСБанками = Ложь, ОшибкиЗаполнения = Неопределено) Экспорт
	...
	ПараметрыПроверкиЗаполнения.Вставить("ПрименениеПриказа126н",
		ДатаНачалаПримененияПриказа126н <> '00010101' 
			И Дата >= ДатаНачалаПримененияПриказа126н Или Не ЗначениеЗаполнено(Дата));
	ПараметрыПроверкиЗаполнения.Вставить("ПрименениеПриказа58н",
		ПлатежиВБюджетКлиентСервер.ДействуетПриказ58н_202604(Дата));
	ПараметрыПроверкиЗаполнения.Вставить("ЕдиныйНалоговыйПлатеж",
	...
	Если ПеречислениеВБюджет И МассивОпераций.Найти(ХозяйственнаяОперация) <> Неопределено Тогда
		...
		Если ПлатежиВБюджетКлиентСервер.СтатусыПлательщика(
			...
			ТекстОшибки = НСтр("ru = 'Неверное значение поля ""Статус плательщика""';
								|en = 'Неверное значение поля ""Статус плательщика""'");
...
//
// Параметры:
//   Объект - ДокументОбъект.ЗаявкаНаРасходованиеДенежныхСредств, ДокументОбъект.СписаниеБезналичныхДенежныхСредств, ДанныеФормыСтруктура - документ оплаты.
...
Функция ОшибкиЗаполненияРеквизитовПлательщикаПолучателя(ЭтоПлательщик, ДокументОбъект, РеквизитыСтороны, ВыводитьСообщение, Отказ = Ложь) Экспорт
	...
	Если ПустаяСтрока(РеквизитыСтороны.Наименование) Тогда
		...
		Если ЭтоПлательщик Тогда
			ОшибкиЗаполнения.Добавить(НСтр("ru = 'Наименование плательщика не заполнено';
											|en = 'Наименование плательщика не заполнено'"));
		Иначе
			ОшибкиЗаполнения.Добавить(НСтр("ru = 'Наименование получателя не заполнено';
											|en = 'Наименование получателя не заполнено'"));
		КонецЕсли;
	...
	ПолеИНН = ?(ЭтоПлательщик, НСтр("ru = 'ИНН плательщика';
									|en = 'ИНН плательщика'"), НСтр("ru = 'ИНН получателя';
																	|en = 'ИНН получателя'"));
	ПолеКПП = ?(ЭтоПлательщик, НСтр("ru = 'КПП плательщика';
									|en = 'КПП плательщика'"), НСтр("ru = 'КПП получателя';
																	|en = 'КПП получателя'"));
	...
	Если ЭтоИнЮрЛицо Тогда
		...
		Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ИНН) Тогда
			...
			Шаблон = НСтр("ru = '%1 должен состоять только из цифр';
							|en = '%1 должен состоять только из цифр'");
	...
	Если Лев(ИНН, 2) = "00" Тогда
		...
		Шаблон = НСтр("ru = 'Первый и второй знаки %1 не могут одновременно принимать значение ""0""';
						|en = 'Первый и второй знаки %1 не могут одновременно принимать значение ""0""'");
		ОшибкиЗаполнения.Добавить(СтрШаблон(Шаблон, ПолеИНН));
	...
	Если ЭтоЮрЛицо И КПП <> "" И СтрДлина(КПП) <> 9
		...
		Шаблон = НСтр("ru = '%1 должен содержать 9 символов';
						|en = '%1 должен содержать 9 символов'");
		ОшибкиЗаполнения.Добавить(СтрШаблон(Шаблон, ПолеКПП));
	...
	Если ЭтоФизЛицо И КПП <> "" И КПП <> "0" Тогда
		...
		Шаблон = НСтр("ru = '%1 для физлица не заполняется';
						|en = '%1 для физлица не заполняется'");
		ОшибкиЗаполнения.Добавить(СтрШаблон(Шаблон, ПолеКПП));
...
Процедура ПроверитьЗаполнениеБанковскихРеквизитов(Объект, Отказ, ВалютныйСчет) Экспорт
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Банк)
		...
		Если НациональныеРеквизитыБанковскихСчетов Тогда
			...
			ПараметрыДопСообщения.Текст = НСтр("ru = 'Необходимо заполнить поле ""Банк"" или банковские реквизиты счета:';
												|en = 'Fill in the ""Bank"" field or bank account details:'");
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.Банк) Тогда
		...
		ПараметрыСообщения.Текст = НСтр("ru = 'Поле ""%1"" у банка не заполнено.';
										|en = 'The ""%1"" bank field is required.'");
	...
	Если Не ЗначениеЗаполнено(Объект.БанкДляРасчетов)
		...
		Если НациональныеРеквизитыБанковскихСчетов Тогда
			...
			ПараметрыСообщения.Текст = НСтр("ru = 'Поле ""%1"" банка-корреспондента не заполнено.';
											|en = 'The ""%1"" field of the correspondent bank is required.'");
			...
			ПараметрыДопСообщения.Текст = НСтр("ru = 'Необходимо заполнить поле ""Банк корреспондент"" или реквизиты банка-корреспондента счета:';
												|en = 'Fill in the ""Correspondent bank"" field or the details of the account''s correspondent bank:'");
			ПараметрыДопСообщения.КлючДанных = Объект;
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.БанкДляРасчетов) Тогда
		...
		ПараметрыДопСообщения.Текст = НСтр("ru = 'Необходимо заполнить реквизиты банка-корреспондента:';
											|en = 'Fill the correspondent bank details:'");
		ПараметрыДопСообщения.КлючДанных = Объект;
		ПараметрыДопСообщения.Поле = "БанкДляРасчетов";
...
Процедура ПроверитьЗаполнениеРеквизитовНалоговыхПлатежей(Объект, ПараметрыПроверкиЗаполнения, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения)
	...
	Если ПараметрыПроверкиЗаполнения.ЕдиныйНалоговыйПлатеж Тогда
		...
		Если ПараметрыПроверкиЗаполнения.ПрименениеПриказа58н
			...
			ТекстОшибки = НСтр("ru = 'В поле ""Статус плательщика"" следует указать значение ""01""';
								|en = 'В поле ""Статус плательщика"" следует указать значение ""01""'");
			Если ФлагОбменСБанками Тогда
				ДобавитьОшибкуЗаполнения(ОшибкиЗаполнения, ТекстОшибки);
			Иначе
		...
		ИначеЕсли ДействуетПриказ199н_202302
			...
			ТекстОшибки =
				НСтр("ru = 'В поле ""Статус плательщика"" следует указать значение ""01"" (для кредитных организаций ""27"")';
					|en = 'В поле ""Статус плательщика"" следует указать значение ""01"" (для кредитных организаций ""27"")'");
			Если ФлагОбменСБанками Тогда
				ДобавитьОшибкуЗаполнения(ОшибкиЗаполнения, ТекстОшибки);
			Иначе
				ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, , "НадписьРеквизитыПлатежаВБюджет", , Отказ);
	...
	Если ПлатежиВБюджетКлиентСервер.ПлатежАдминистрируетсяНалоговымиОрганами(Объект.КодБК)
		И Не ПлатежиВБюджетКлиентСервер.ЭтоЕдиныйНалоговыйПлатеж(Объект.КодБК) Тогда
		
		Если ПараметрыПроверкиЗаполнения.ПрименениеПриказа58н
			И Объект.СтатусСоставителя <> "01"
			И Объект.СтатусСоставителя <> "04"
			И Объект.СтатусСоставителя <> "13" Тогда
			
			ТекстОшибки =
				НСтр("ru = 'В поле ""Статус плательщика"" следует указать значение ""01"", ""13"" или ""04""';
					|en = 'В поле ""Статус плательщика"" следует указать значение ""01"", ""13"" или ""04""'");
			Если ФлагОбменСБанками Тогда
				ДобавитьОшибкуЗаполнения(ОшибкиЗаполнения, ТекстОшибки);
			Иначе
				ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, , "НадписьРеквизитыПлатежаВБюджет", , Отказ);
		...
		ИначеЕсли Не ПараметрыПроверкиЗаполнения.ПрименениеПриказа58н
			...
			ТекстОшибки =
				НСтр("ru = 'В поле ""Статус плательщика"" следует указать значение ""01"", ""13"" (для кредитных организаций ""27"") или ""02"" (для уведомления об исчисленных суммах налога)';
					|en = 'В поле ""Статус плательщика"" следует указать значение ""01"", ""13"" (для кредитных организаций ""27"") или ""02"" (для уведомления об исчисленных суммах налога)'");
			Если ФлагОбменСБанками Тогда
				ДобавитьОшибкуЗаполнения(ОшибкиЗаполнения, ТекстОшибки);
			Иначе
				ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, , "НадписьРеквизитыПлатежаВБюджет", , Отказ);
...
Процедура ПроверитьЗаполнениеРеквизитовТаможенныхПлатежей(Объект, ПараметрыПроверкиЗаполнения, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения)
	...
	Если ПараметрыПроверкиЗаполнения.ПрименениеПриказа58н
		...
		ТекстОшибки = НСтр("ru = 'Поле ""Основание"" не заполнено';
							|en = 'Поле ""Основание"" не заполнено'");
		Если ФлагОбменСБанками Тогда
			ДобавитьОшибкуЗаполнения(ОшибкиЗаполнения, ТекстОшибки);
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,, "НадписьРеквизитыПлатежаВБюджет",, Отказ);
			...
			Если Объект[ЭлементСтруктуры.Ключ] <> "0" Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При статусе плательщика ""08"" следует указать ""0"" в поле ""%1""';
						|en = 'При статусе плательщика ""08"" следует указать ""0"" в поле ""%1""'"),
...
Процедура ПроверитьЗаполнениеКБК(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверки)
	...
	Если Объект.ВидПеречисленияВБюджет = Перечисления.ВидыПеречисленийВБюджет.ИнойПлатеж
		...
		ТекстОшибки = НСтр("ru = 'КБК должен состоять из 20 знаков или ""0""';
							|en = 'КБК должен состоять из 20 знаков или ""0""'");
	...
	ИначеЕсли Не СтроковыеФункцииКлиентСервер.ТолькоЛатиницаВСтроке(ВРег(КБК), Истина, "0123456789АБВГДЕЖИКЛМНПРСТУФЦЧШЩЭЮЯ")
		...
		ТекстОшибки = НСтр("ru = 'В КБК допустимо вводить только цифры и буквы (8 - 17 разряды, кроме ""О"" и ""Х"")';
							|en = 'В КБК допустимо вводить только цифры и буквы (8 - 17 разряды, кроме ""О"" и ""Х"")'");
...
Процедура ПроверитьЗаполнениеОКТМО(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверки)
	...
	Если ПустаяСтрока(КодОКАТО)
		...
		ТекстОшибки =  НСтр("ru = 'ОКТМО должен состоять из 8 знаков или ""0""';
							|en = 'ОКТМО должен состоять из 8 знаков или ""0""'");
...
Процедура ПроверитьЗаполнениеКодТаможенногоОргана(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверки)
	...
	ИначеЕсли ПустаяСтрока(СокрЛП(СтрЗаменить(КодТаможенногоОргана, "0", "")))
		...
		ТекстОшибки = НСтр("ru = 'Все знаки Кода таможенного органа не могут одновременно принимать значение ""0""';
							|en = 'Все знаки Кода таможенного органа не могут одновременно принимать значение ""0""'");
	...
	ИначеЕсли СтрДлина(СокрП(КодТаможенногоОргана)) <> 8
		...
		ТекстОшибки = НСтр("ru = 'Код таможенного органа должен состоять из 8 знаков';
							|en = 'Код таможенного органа должен состоять из 8 знаков'");
...
Процедура ПроверитьЗаполнениеПоказателяНомера(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверки)
	...
	Если Не ЗначениеЗаполнено(ПоказательНомера) Тогда
		ТекстОшибки = НСтр("ru = 'Поле ""Номер документа"" не заполнено';
							|en = 'Поле ""Номер документа"" не заполнено'");
	ИначеЕсли ПараметрыПроверки.ПрименениеПриказа58н
		...
		ТекстОшибки = НСтр("ru = 'Поле ""Номер документа"" должно быть заполнено конкретным значением и не ""0""';
							|en = 'Поле ""Номер документа"" должно быть заполнено конкретным значением и не ""0""'");
	ИначеЕсли ПараметрыПроверки.ПрименениеПриказа58н
...
Процедура ПроверитьЗаполнениеПоказателяДаты(Объект, Отказ, ФлагОбменСБанками, ОшибкиЗаполнения, ПараметрыПроверки)
	...
	Если Не ЗначениеЗаполнено(ПоказательДаты) Тогда
		ТекстОшибки = НСтр("ru = 'Поле ""Дата документа"" не заполнено';
							|en = 'Поле ""Дата документа"" не заполнено'");
	ИначеЕсли ПараметрыПроверки.ПрименениеПриказа58н
		...
		ТекстОшибки = НСтр("ru = 'Поле ""Дата документа"" должно быть заполнено значением ""00""';
							|en = 'Поле ""Дата документа"" должно быть заполнено значением ""00""'");
	КонецЕсли;
			...
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл 
				...
				Если НайденнаяСтрока.СуммаДокумента + НайденнаяСтрока.СуммаРасходовВТекущемМесяце > Лимит Тогда
					...
					ЧастиОписания.Добавить(НСтр("ru = '%1 руб. с учетом ранее созданных распоряжений на сумму %2 руб. превышает лимит %3 руб. в месяц.';
												|en = '%1 rub. considering previously created references to the amount of %2 rub. exceeds the limit of %3 rub. per month.'"));
					...
					Если НайденнаяСтрока.ЭтоСписание Тогда
						ЧастиОписания.Добавить(
							НСтр("ru = 'Сумма платежа %1 руб. с учетом ранее произведенных оплат на сумму %2 руб.';
								|en = 'Payment amount is %1 RUB, including previous payments in the amount of %2 RUB'"));
