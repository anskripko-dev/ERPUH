...
Процедура ДополнитьСоответствияРегистраторовОтчетаОДвижениях(Документ, СоответствиеРегистров) Экспорт
	...
	Если Метаданные.ОпределяемыеТипы.РегистрацияДанныхБюджетированияСсылки.Тип.СодержитТип(ТипЗнч(Документ)) Тогда
		СоответствиеРегистров.Вставить(Метаданные.РегистрыСведений.ВерсииГрафиковРасчетов, 					"ВерсияГрафика");
...
Процедура ОтразитьВРегистрах(Регистратор, ОсновнойДокумент, СтруктураТаблиц)
	...
	НаборЗаписей = РегистрыСведений.ВерсииГрафиковРасчетов.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Регистратор.Установить(Регистратор);
...
Функция ТекстЗапросаПодготовкаДанныхДокумента()
	...
	|	ВТ_АналитикиИсполнения.ДокументРезервирования КАК ДокументРезервирования,
	...
	|	ВТ_АналитикиОплаты.ДокументРезервирования КАК ДокументРезервирования,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ПараметрыЛимитирования КАК ПараметрыОперативногоПланирования
	...
	|			И (ПараметрыОперативногоПланирования.ПериодичностьПланирования = Периоды.Периодичность)
	|			И (ПараметрыОперативногоПланирования.ПериодичностьПланирования = Периоды.Периодичность)
...
Функция ТекстЗапросаТаблицыДвижений()
	...
	|	0 КАК Лимит,
	|	ВЫБОР 
	|		КОГДА ВТ_ОперативныйПлан.ДокументРезервирования <> ЗНАЧЕНИЕ(Документ.ОперативныйПлан.ПустаяСсылка) 
	|			ТОГДА -ВТ_ОперативныйПлан.Заявлено
	|		ИНАЧЕ 0 
...
Функция СборДанныхПоЗаказуПоставщику()
	...
	|	ЗаказПоставщикуАналитикиПланирования.ДокументРезервирования КАК ДокументРезервирования
	...
	|ГДЕ
	|	ЗаказПоставщикуЭтапыГрафикаОплаты.Ссылка = &Документ
	|	И &ТребуютсяПлатежныеПозиции = ИСТИНА"; 
...
Функция СборДанныхПоПриобретениеТоваровУслуг()
	...
	|	ДокументАналитикиПланирования.ДокументРезервирования КАК ДокументРезервирования
	|ПОМЕСТИТЬ ВТ_АналитикиПланирования
	|ИЗ
...
Функция СборДанныхПоЗаказуКлиента()
	...
	|				И ДокументТовары.Ссылка.ДатаОтгрузки <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|			ТОГДА ДокументТовары.Ссылка.ДатаОтгрузки                               
	|		КОГДА ДокументТовары.ДатаОтгрузки <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|			ТОГДА ДокументТовары.ДатаОтгрузки
	|		ИНАЧЕ	ДокументТовары.Ссылка.Дата
	|	КОНЕЦ КАК ДатаИсполнения, 
	|	ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийПриходРасход.Расход) КАК ПриходРасход,
