...
Функция ИсчисленныйНалогСДоходами(ИсчисленныйНалог, НачисленныеДоходы, ДатаПолученияДохода) Экспорт
	...
	Для каждого СтрокаТаблицы Из ТаблицаРезультатов Цикл
		...
		РезультатРаспределения = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(СтрокаТаблицы.СуммаДоходаСКоторогоНеУдержанНалог, Коэффициенты, 0);
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Уточнение отражения кода дохода для начислений с категорией ""Компенсация не использованных дней часов отдыха""';
									|en = 'Specify the recording of the income code for accruals with the ""Compensation of unused leave days hours"" category'");
