...
Процедура ЗаписатьДействияПользователя(СписокОкон, ДатаДействий, ИдентификаторПользовательИБ = Неопределено) Экспорт
	...
	Для каждого НовоеОкно Из СписокОкон Цикл
		...
		ИначеЕсли ОбщегоНазначения.ЭтоСсылка(ТипТекущегоЗначения) Тогда
			Если ОбщегоНазначения.ЭтоПеречисление(НовоеОкно.Значение.Метаданные()) Тогда
				ТекущееОписание = НовоеОкно.Значение;
			Иначе	
...
Функция МассивУведомленийНалоговогоМониторинга() Экспорт
	СписокВидовУведомлений = Новый СписокЗначений;
	ОбьектыПодсистемы = Метаданные.Подсистемы.ПодсистемыУХ.Подсистемы.ИнтеграцияНалоговыйМониторинг.Подсистемы.РегламентированнаяОтчетностьНМ.Состав;
	СоответствиеИмен = УведомлениеОСпецрежимахНалогообложенияПовтИсп.ПолучитьСоответствиеВидовУведомленийИменамОтчетов();
	Для каждого Стр Из ОбьектыПодсистемы Цикл
		Для каждого Ключ Из СоответствиеИмен Цикл
			Если Ключ.Значение = Стр.Имя Тогда
				СписокВидовУведомлений.Добавить(Ключ.Ключ);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	СписокВидовУведомлений.СортироватьПоЗначению();
	Результат = СписокВидовУведомлений.ВыгрузитьЗначения();
	
	НалоговыйМониторингПереопределяемый.МассивУведомленийНалоговогоМониторинга(Результат);
	
	Возврат Результат;
КонецФункции

Функция ПлановаяДатаОтветаНаЗапросНМ(ДатаЗапросаНМ) Экспорт
	УстановитьПривилегированныйРежим(Истина);
	СрокПредоставленияДокументов = Константы.СрокПредоставленияДокументовНалоговогоМониторинга.Получить();
	УстановитьПривилегированныйРежим(Ложь);
	Плановая = КалендарныеГрафики.ДатаПоКалендарю(КалендарныеГрафики.ОсновнойПроизводственныйКалендарь(), ДатаЗапросаНМ, СрокПредоставленияДокументов, Ложь);
	Если Не ЗначениеЗаполнено(Плановая) Тогда
		Плановая = ДатаЗапросаНМ + (86400 * СрокПредоставленияДокументов);
	КонецЕсли;
	Возврат Плановая;
КонецФункции
