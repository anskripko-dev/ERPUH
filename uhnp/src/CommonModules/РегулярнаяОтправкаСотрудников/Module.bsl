...
Функция ДанныеСтатусаОтправкиСотрудникаПоУсловию(СистемаБронирования, ФизическоеЛицо)
	...
	ТаблицаСотрудников = МестаРаботыСотрудников(ФизическоеЛицо, Неопределено, Неопределено, Ложь, ПоСтруктуреПредприятия);
...
Функция ПодразделенияБронированияКомандировок(СистемаБронирования) Экспорт
	...
	Если МассивВключатьПодчиненные.Количество() > 0 Тогда
		...
		Иначе
			ДобавленыВСписок = Справочники.ПодразделенияОрганизаций.ПодчиненныеПодразделения(МассивВключатьПодчиненные);
		КонецЕсли;
...
Процедура ЗаполнитьДанныеОтправляемыхСотрудников(СистемаБронирования, ДанныеФизическиЛиц, ТаблицаФизическихЛиц)
	...
	ТаблицаМестРаботыСКадровымиДанными = МестаРаботыСотрудников(
		МассивФизическихЛиц, МассивПолейКадровыхДанных(), Неопределено, Истина);
