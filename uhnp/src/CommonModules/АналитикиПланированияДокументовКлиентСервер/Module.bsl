...
Функция ТребуетсяОтражениеВБюджетировании(Источник) Экспорт
	...
	Если Источник.ПорядокРасчетов <> ПредопределенноеЗначение("Перечисление.ПорядокРасчетов.ПоДоговорамКонтрагентов")
		 И Источник.ПорядокРасчетов <> ПредопределенноеЗначение("Перечисление.ПорядокРасчетов.ПоДоговорамНакладным") Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Функция ДолжныФормироватьсяПозиции(Источник) Экспорт
	...
	//Если Источник.ИспользованиеГрафикаРасчетовБДДС
