...
Процедура ЗаполнитьСторнированиеНачислений(МенеджерВременныхТаблиц, Организация, ДанныеПерерасчета, КатегорииНачисленийПособий, РаспределенияТерриторииПоказатели, МесяцНачисления)
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(Начисления.СторнируемыйДокумент, НЕОПРЕДЕЛЕНО) КАК СторнируемыйДокумент
		|ИЗ
		|	РегистрСведений.ИсходныеДанныеПерерасчетов КАК ИсходныеДанныеПерерасчетов
		...
		|		ПО (Начисления.Регистратор = ИсходныеДанныеПерерасчетов.Регистратор)
		|			И (Начисления.ИдентификаторСтроки = ИсходныеДанныеПерерасчетов.ИдентификаторСтроки)
		|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрокаСторноНачисленийПерерасчет.СторнируемыйИсходный = ?(ЗначениеЗаполнено(Выборка.СторнируемыйДокумент), Выборка.СторнируемыйДокумент, Выборка.Ссылка);
