...
// 
// Возвращаемое значение:
//  Строка - Имя регистра место нахождения ОС
...
Процедура СуммаНДССМежценовойРазницы(МенеджерВременныхТаблиц, ПараметрыОсновногоЗапроса) Экспорт
	...
	"ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА СчетФактураВыданныйТовары.СтавкаНДС.ПеречислениеСтавкаНДС = &СтавкаНДССтрока030
	|			ТОГДА СчетФактураВыданныйТовары.СуммаНДС
	|	КОНЕЦ КАК СуммаНДСМежценоваяРазницаСтрока030, 
	|	ВЫБОР
	|		КОГДА СчетФактураВыданныйТовары.СтавкаНДС.ПеречислениеСтавкаНДС = &СтавкаНДССтрока005
	|			ТОГДА СчетФактураВыданныйТовары.СуммаНДС
	...
	|	ВЫБОР
	|		КОГДА СчетФактураВыданныйТовары.СтавкаНДС = &СтавкаНДС7_107
	|			ТОГДА СчетФактураВыданныйТовары.СуммаНДС
	|	КОНЕЦ КАК СуммаНДСМежценоваяРазницаСтрока033,
	|	ВЫБОР
	|		КОГДА СчетФактураВыданныйТовары.СтавкаНДС = &СтавкаНДС5_105
	|			ТОГДА СчетФактураВыданныйТовары.СуммаНДС
...
Процедура ТекстЗапросаРеестрНДСПриложение(Запрос, ПараметрыОтчета) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПодтвержденныеОтгрузки.ДокументОтгрузки КАК ДокументОтгрузки,
	...
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ПодтвержденныеОтгрузки.ДокументОтгрузки";
	
	Если ПараметрыОтчета.ВключатьПримечания Тогда
		...
		|		ПО ПодтвержденныеОтгрузки.ДокументОтгрузки = СведенияТаможенныхДекларацийЭкспорт.ДокументОтгрузки
		|ГДЕ
		|	ЕСТЬNULL(СведенияТаможенныхДекларацийЭкспорт.Примечание, """") <> """"";
	КонецЕсли;
	
	Если ПараметрыОтчета.ВключатьСведенияОКонтракте Тогда
		Запрос.Текст =Запрос.Текст + 
		";
		|"
		+
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВЫБОР
		|		КОГДА ДокументыОтгрузкиИТД.ДокументОтгрузки ССЫЛКА Документ.РеализацияТоваровУслуг
		|			ТОГДА ВЫРАЗИТЬ(ДокументыОтгрузкиИТД.ДокументОтгрузки КАК Документ.РеализацияТоваровУслуг).Договор
		|		КОГДА ДокументыОтгрузкиИТД.ДокументОтгрузки ССЫЛКА Документ.РеализацияУслугПрочихАктивов
		|			ТОГДА ВЫРАЗИТЬ(ДокументыОтгрузкиИТД.ДокументОтгрузки КАК Документ.РеализацияУслугПрочихАктивов).Договор
		|		КОГДА ДокументыОтгрузкиИТД.ДокументОтгрузки ССЫЛКА Документ.АктВыполненныхРабот
		|			ТОГДА ВЫРАЗИТЬ(ДокументыОтгрузкиИТД.ДокументОтгрузки КАК Документ.АктВыполненныхРабот).Договор
		//++ НЕ УТКА
		|		КОГДА ДокументыОтгрузкиИТД.ДокументОтгрузки ССЫЛКА Документ.ОтчетДавальцу
		|			ТОГДА ВЫРАЗИТЬ(ДокументыОтгрузкиИТД.ДокументОтгрузки КАК Документ.ОтчетДавальцу).Договор
		|		КОГДА ДокументыОтгрузкиИТД.ДокументОтгрузки ССЫЛКА Документ.ОтчетДавальцу2_5
		|			ТОГДА ВЫРАЗИТЬ(ДокументыОтгрузкиИТД.ДокументОтгрузки КАК Документ.ОтчетДавальцу2_5).Договор
		//-- НЕ УТКА
		|		КОГДА ДокументыОтгрузкиИТД.ДокументОтгрузки ССЫЛКА Документ.ОтчетПереработчика
		|			ТОГДА ВЫРАЗИТЬ(ДокументыОтгрузкиИТД.ДокументОтгрузки КАК Документ.ОтчетПереработчика).Договор
		|		КОГДА ДокументыОтгрузкиИТД.ДокументОтгрузки ССЫЛКА Документ.ОтчетПереработчика2_5
		|			ТОГДА ВЫРАЗИТЬ(ДокументыОтгрузкиИТД.ДокументОтгрузки КАК Документ.ОтчетПереработчика2_5).Договор
		|	КОНЕЦ КАК ДоговорКонтрагента,
		|	ДокументыОтгрузкиИТД.КодОперации КАК КодОперации,
		|	ДокументыОтгрузкиИТД.НомерТаможеннойДекларации КАК НомерТаможеннойДекларации
		|ПОМЕСТИТЬ ДоговорыОтгрузок
		|ИЗ
		|	ДокументыОтгрузкиИТД КАК ДокументыОтгрузкиИТД
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ДоговорКонтрагента
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ДоговорыОтгрузок.КодОперации КАК КодОперации,
		|	ДоговорыОтгрузок.НомерТаможеннойДекларации КАК НомерТаможеннойДекларации,
		...
		|ПОМЕСТИТЬ Контракты
		|ИЗ
		|	ДоговорыОтгрузок КАК ДоговорыОтгрузок
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
		|		ПО ДоговорыОтгрузок.ДоговорКонтрагента = ДоговорыКонтрагентов.Ссылка
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ДоговорыОтгрузок";
	КонецЕсли;
	...
	Если ПараметрыОтчета.ИДОтчета = "РегламентированныйОтчетРеестрНДСПриложение5" Тогда
		...
		|ПОМЕСТИТЬ ТоваросопроводительныеДокументы
		|ИЗ
		|	ДокументыОтгрузкиИТД КАК ДокументыОтгрузкиИТД
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СведенияТаможенныхДекларацийЭкспорт КАК СведенияТаможенныхДекларацийЭкспорт
		|		ПО ДокументыОтгрузкиИТД.НомерТаможеннойДекларации = СведенияТаможенныхДекларацийЭкспорт.НомерТаможеннойДекларации";
	КонецЕсли;
	...
	|	ДокументыОтгрузкиИТД.КодОперации КАК КодОперации,
	|	ДокументыОтгрузкиИТД.НомерТаможеннойДекларации КАК НомерТаможеннойДекларации,
	|	ПодтвержденныеОтгрузки.ПодтвержденнаяНалоговаяБаза КАК ПодтвержденнаяНалоговаяБаза
	|ИЗ
	|	ПодтвержденныеОтгрузки КАК ПодтвержденныеОтгрузки
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ПодтвержденныеОтгрузки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ДокументыОтгрузкиИТД";
