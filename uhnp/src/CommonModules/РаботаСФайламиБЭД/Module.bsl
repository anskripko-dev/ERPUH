...
Функция РасширениеФайлаПоСигнатуре(ОписаниеФайла) Экспорт
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось определить расширение файла ""%1"" по сигнатуре';
										|en = 'Cannot identify the ""%1"" file extension by signature'"),
	...
	Если ДоступнаРаботаСоСтраницамиPDFДокумента() Тогда
		...
		Исключение 
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Ошибка чтения файла pdf';
					|en = 'Error reading the PDF file'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
#Если Не ВнешнееСоединение Тогда
	...
	Исключение
		...
		ПодробныйТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Ошибка установки штампа в pdf';
				|en = 'Error setting the stamp in PDF'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробныйТекстОшибки);
		Возврат Результат;
	КонецПопытки;
	ДокументPDF = Неопределено;
...
Функция ИзменениеИменФайловАрхиваВПотоке(ПотокИсходный, СоответствиеИменФайлов, КонтекстДиагностики)
	...
	Если Не ТекущаяСигнатура = 67324752 Тогда
		ТекстОшибки = НСтр("ru = 'Файл архива содержит неверную сигнатуру заголовка. Распаковка невозможна.';
							|en = 'The archive file contains an invalid header signature. Extraction is impossible.'");
	...
	Пока Истина Цикл
		...
		Иначе
			ТекстОшибки = НСтр("ru = 'Файл архива содержит неверную сигнатуру блока файла. Распаковка невозможна.';
								|en = 'The archive file contains an invalid file block signature. Extraction is impossible.'");
	...
	Пока Истина Цикл
		...
		Иначе
			ТекстОшибки = НСтр("ru = 'Файл архива содержит неверную сигнатуру блока смещений. Распаковка невозможна.';
								|en = 'The archive file contains an invalid offset block signature. Extraction is impossible.'");
...
Функция БуферИмениИзСоответствия(БуферИмени, СоответствиеИменФайлов)
	...
	Если ДанныеФайла = Неопределено Тогда
		...
		ТекстОшибки = СтрШаблон(НСтр("ru = '
			...
			|en = '
			|Error reading the %1 file name. Contact the infobase administrator. Follow the steps on the server:
			|- Unpack the transport container archive
			|- Check the names of the unpacked files
...
Функция ПоследняяСтраницаДокументаPDF(ДокументPDF)
	...
	Если ДоступнаРаботаСоСтраницамиPDFДокумента() Тогда
		...
		Исключение
			...
			ПодробныйТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Ошибка чтения страниц файла pdf';
					|en = 'Error reading the pages of the PDF file'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробныйТекстОшибки);
			Возврат Неопределено;
		КонецПопытки;
