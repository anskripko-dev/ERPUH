...
Функция ЭтоВалютныйДоговорМСФО(ДоговорОбъект) Экспорт
	...
	Иначе
		...
		ВалютаРУ = УчетМСФОПовтИспУХ.ВалютаРУ(Организация);
		Возврат (ВалютаВзаиморасчетов <> ВалютаРУ);
...
Функция ВидыДоговоровСКомиссионером() Экспорт
	
	Результат = Новый Массив;
	Результат.Добавить(ПредопределенноеЗначение("Справочник.ВидыДоговоровКонтрагентовУХ.СКомиссионером"));
...
Функция ВидыДоговоровАккредитивыБанковскиеГарантии() Экспорт
	
	Результат = Новый Массив;
	Результат.Добавить(ПредопределенноеЗначение("Справочник.ВидыДоговоровКонтрагентовУХ.АккредитивВыданный"));
	Результат.Добавить(ПредопределенноеЗначение("Справочник.ВидыДоговоровКонтрагентовУХ.ГарантияВыданная"));
...
Функция ВидыДоговоровКредит() Экспорт
	
	Результат = Новый Массив;
	Результат.Добавить(ПредопределенноеЗначение("Справочник.ВидыДоговоровКонтрагентовУХ.Кредит"));
	
	Возврат Результат;
	
КонецФункции

Функция ВидыДоговоровДепозит() Экспорт
	
	Результат = Новый Массив;
	Результат.Добавить(ПредопределенноеЗначение("Справочник.ВидыДоговоровКонтрагентовУХ.Депозит"));
	
	Возврат Результат;
	
КонецФункции

Функция ВидыДоговоровЗаем() Экспорт
	
	Результат = Новый Массив;
	Результат.Добавить(ПредопределенноеЗначение("Справочник.ВидыДоговоровКонтрагентовУХ.ЗаемВыданный"));
	Результат.Добавить(ПредопределенноеЗначение("Справочник.ВидыДоговоровКонтрагентовУХ.ЗаемПолученный"));
...
Функция ЭтоДоговорСКомиссионером(ВидДоговораУХ) Экспорт
	
	Возврат (ВидыДоговоровСКомиссионером().Найти(ВидДоговораУХ) <> Неопределено);
...
Функция ФормироватьПозицииЗаявокПоГрафику(Объект) Экспорт
	...
	Возврат Объект.ИспользованиеГрафикаРасчетовБДДС = ПредопределенноеЗначение("Перечисление.РежимыИспользованияГрафика.АвтоматическоеСозданиеПлатежныхПозиций");
...
Функция КомандаДляДоговора(Команда) Экспорт
	
	Значение = Ложь;  

	КоллекцияИсключений = Новый Массив;
	КоллекцияИсключений.Добавить("КорректировкаПланов");
	КоллекцияИсключений.Добавить("ЗаявкаНаКорректировкуПланов");
	КоллекцияИсключений.Добавить("ЗакрытиеПлатежнойПозиции");

	Для Каждого ЭлементКоллеции Из КоллекцияИсключений Цикл
		Если СтрНайти(Команда.Имя, ЭлементКоллеции) > 0
				Тогда
			Возврат Ложь;			
		КонецЕсли;
	КонецЦикла;	
    ...
    КоллекцияИсключений = Новый Массив;
	КоллекцияИсключений.Добавить("ПодменюПечатьОбычное");
	КоллекцияИсключений.Добавить("ПодменюОтчетыОбычное");
	КоллекцияИсключений.Добавить("ПодменюСоздатьНаОсновании");
