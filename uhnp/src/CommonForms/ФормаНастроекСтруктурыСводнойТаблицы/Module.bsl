...
Процедура ТаблицаСтраницыНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	...
	Если Элемент.ТекущиеДанные.Код = "Фильтр_Валюты" ИЛИ Элемент.ТекущиеДанные.Код = "Фильтр_Сценарии" Тогда
		Выполнение = Ложь;
	КонецЕсли;	
...
Процедура ОбработатьНастраиваемыеИзмерения(ТекДанные,ТекущаяТаблица=Неопределено)
	...
	Если КодЭлементаНастройки = "Фильтр_ШаблонДокументаБД" ИЛИ КодЭлементаНастройки = "Фильтр_Проект" 
