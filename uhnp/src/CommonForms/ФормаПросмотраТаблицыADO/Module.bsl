	...
	Если НЕ ЗначениеЗаполнено(Параметры.ТаблицаADO) Тогда
		...
		Если Не ЗначениеЗаполнено(ТаблицаADO) Тогда
			...
			Если Не ЗначениеЗаполнено(ТаблицаADO) Тогда
				...
				ОбщегоНазначенияУХ.СообщитьОбОшибке(СтрШаблон(Нстр("ru = 'Не найдена таблица %1 в подключении %2'"), 
					Параметры.СвязаннаяТаблица, ТипБД),,,СтатусСообщения.Внимание);
...
Процедура ОбновитьДанныеТаблицы()
	...
	ТекТаблицаВИБ=УправлениеСоединениямиВИБУХ.ПолучитьДанныеИзТаблицы(ВнешняяИнформационнаяБаза,ТаблицаADO,Параметры.КоличествоЗаписей);
