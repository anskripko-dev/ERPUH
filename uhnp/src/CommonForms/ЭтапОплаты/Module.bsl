...
Процедура СдвигПредоплатаПриИзменении(Элемент)
	...
	ТекущийКалендарь = ?(ЭтаФорма["ПроизводственныеКалендари"].Количество(),ЭтаФорма["ПроизводственныеКалендари"].ВыгрузитьЗначения(),Календарь);
	ЭтапыОплатыКлиент.ЭтапыГрафикаОплатыСдвигВариантОтсчетаПриИзменении(
		Структура,
		ДатаЗаказа,
		ДатаСогласования,
...
Процедура ДобавитьРеквизитыФормы()
	...
	РеквизитПроизводственныеКалендари = Новый РеквизитФормы("ПроизводственныеКалендари",Новый ОписаниеТипов("СписокЗначений"));
	
	ДобавленныеРеквизиты = Новый Массив;
	ДобавленныеРеквизиты.Добавить(РеквизитПроизводственныеКалендари);	
...
&НаСервере
Процедура ЗаполнитьПроизводственныеКалендари()
	
	Если Параметры.Свойство("Календарь") Тогда
		Если ТипЗнч(Параметры.Календарь) = Тип("Массив") Тогда
			ЭтаФорма["ПроизводственныеКалендари"].ЗагрузитьЗначения(Параметры.Календарь);  
		КонецЕсли;
	КонецЕсли;
