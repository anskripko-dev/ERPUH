...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоЗаказКлиента = ТипИсточника = Тип("ДокументСсылка.ЗаказКлиента"); 
	Если Параметры.Свойство("ПорядокРасчетов") Тогда 
		ПорядокРасчетовПоЗаказам = Параметры.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоЗаказам;
    Иначе
		ПорядокРасчетовПоЗаказам = Ложь;
	КонецЕсли;	

	Элементы.АвтоматическоеСозданиеПлатежныхПозиций.Видимость = ЭтоЗаказКлиента И ПорядокРасчетовПоЗаказам;
	Если ЭтоЗаказКлиента Тогда
		Если Параметры.Свойство("РежимИспользованияГрафика") Тогда
			РежимИспользованияГрафика = Параметры.РежимИспользованияГрафика;
		КонецЕсли;
...
Процедура ОК(Команда)
	...
	Если Не Модифицированность Или ТолькоПросмотр Тогда
		Закрыть();
	Иначе
		ИменаРеквизитов = "ВидБюджета, ДокументРезервирования, ЦФО, Проект, СтатьяБюджета,РежимИспользованияГрафика, Аналитика1, Аналитика2, Аналитика3, Аналитика4, Аналитика5, Аналитика6";
