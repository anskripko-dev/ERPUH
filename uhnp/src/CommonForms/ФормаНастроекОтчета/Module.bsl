...
Процедура ИзменитьЭлементСтруктуры(Строка, ИмяСтраницы = Неопределено, ИспользоватьФормуВарианта = Неопределено)
	...
	ШаблонЗаголовка = НСтр("ru = 'Настройка %1 отчета ""%2""';
							|en = 'The %1 setting of the %2 report'");
	Если Строка.Тип = "ДиаграммаКомпоновкиДанных" Тогда
...
Функция ДоступныеРежимыОтображения(ПоказыватьРежимыФлажков)
	...
	Если ПоказыватьРежимыФлажков Тогда
		ДоступныеРежимыОтображения.Добавить("ПоказыватьТолькоФлажокВШапкеОтчета", НСтр("ru = 'Только флажок в шапке отчета';
																						|en = 'Check boxes in report header'"), , БиблиотекаКартинок.БыстрыйДоступСФлажком);
	КонецЕсли;
	...
	Если ПоказыватьРежимыФлажков Тогда
		ДоступныеРежимыОтображения.Добавить("ПоказыватьТолькоФлажокВНастройкахОтчета", НСтр("ru = 'Только флажок в настройках отчета';
																							|en = 'Check boxes in report settings'"), , БиблиотекаКартинок.ОбычныйДоступСФлажком);
	КонецЕсли;
