...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ФильтрПоОбъектамМетаданных.Количество() > 0 Тогда
		...
		Для Каждого ОбъектМетаданныхПолноеИмя Из Параметры.ФильтрПоОбъектамМетаданных Цикл
			ИмяБазовогоТипа = ОбщегоНазначенияУХ.ИмяБазовогоТипаПоОбъектуМетаданных(Метаданные.НайтиПоПолномуИмени(ОбъектМетаданныхПолноеИмя));
	...
	Параметры.Свойство("ВыводитьТабличныеЧасти", ВыводитьТабличныеЧасти);
...
Процедура ВыбратьВыполнить()
	...
	КонецЕсли;
	Оповестить("ВыборОбъектовМетаданныхУХ", ВыбранныеОбъектыМетаданных, Параметры.УникальныйИдентификаторИсточник);
...
Процедура ДеревоОбъектовМетаданныхЗаполнить()
	...
	ЭлементОбщие        = НоваяСтрокаДерева("Общие",        "", Нстр("ru = 'Общие'"),             0, ЭлементКонфигурация);
...
&НаСервере
Функция ОбъектМетаданныхСодержитСтандартныеТабличныеЧасти(ОбъектМетаданных)
	Возврат Метаданные.ПланыСчетов.Содержит(ОбъектМетаданных) Или Метаданные.ПланыВидовРасчета.Содержит(ОбъектМетаданных);
КонецФункции

&НаСервере
Функция ОбъектМетаданныхСодержитТабличныеЧасти(ОбъектМетаданных)
...
Функция ВыводКоллекцииОбъектовМетаданных(Имя, ПолноеИмя, Синоним, Картинка, КартинкаОбъекта, Родитель = Неопределено, Подсистемы = Неопределено, Проверять = Истина)
	...
	Если Подсистемы = Неопределено Тогда
		...
		Попытка
			КартинкаТабличнаяЧасть = БиблиотекаКартинок.ТабличнаяЧасть;
		Исключение
			КартинкаТабличнаяЧасть = Неопределено;
		КонецПопытки;
		...
		Для каждого ЭлементКоллекцииМетаданных ИЗ Метаданные[Имя] Цикл
			Если Параметры.ФильтрПоОбъектамМетаданных.Количество() > 0 
				...
				Продолжить;
			КонецЕсли;
			СтрокаЭлементМетаданных = НоваяСтрокаДерева(ЭлементКоллекцииМетаданных.Имя,
			...
			Если ВыводитьТабличныеЧасти Тогда
				
				Если ОбъектМетаданныхСодержитСтандартныеТабличныеЧасти(ЭлементКоллекцииМетаданных) Тогда 
					
					Попытка	
						
						Для Каждого МетаданныеТабличнаяЧасть Из ЭлементКоллекцииМетаданных.СтандартныеТабличныеЧасти Цикл
							
							НоваяСтрокаДерева(МетаданныеТабличнаяЧасть.Имя,
							ЭлементКоллекцииМетаданных.ПолноеИмя() + ".СтандартнаяТабличнаяЧасть." + МетаданныеТабличнаяЧасть.Имя,
							МетаданныеТабличнаяЧасть.Синоним,
							КартинкаТабличнаяЧасть,
							СтрокаЭлементМетаданных,
							Истина);
				...
				Если ОбъектМетаданныхСодержитТабличныеЧасти(ЭлементКоллекцииМетаданных) Тогда
					Попытка
						
						Если ЭлементКоллекцииМетаданных.ТабличныеЧасти.Количество() Тогда
							
							Для Каждого МетаданныеТабличнаяЧасть Из ЭлементКоллекцииМетаданных.ТабличныеЧасти Цикл
								
								НоваяСтрокаДерева(МетаданныеТабличнаяЧасть.Имя,
								МетаданныеТабличнаяЧасть.ПолноеИмя(),
								МетаданныеТабличнаяЧасть.Синоним,
								КартинкаТабличнаяЧасть,
								СтрокаЭлементМетаданных,
								Истина);
