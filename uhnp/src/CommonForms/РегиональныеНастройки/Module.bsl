...
Процедура ОтобразитьНастройкиДополнительныхЯзыков(Настройки, КоличествоЯзыков)
	...
	Для ПорядковыйНомерЯзыка = 1 По КоличествоЯзыков Цикл
		...
		ПолеДополнительныйЯзык.ПодсказкаВвода= НСтр("ru = 'Дополнительный язык для ведения учета';
													|en = 'Additional accounting language'");
...
Функция ДанныеКорректны()
	...
	Если ПустаяСтрока(ОсновнойЯзык) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не выбран основной язык приложения';
														|en = 'Main application language not set'"),, Элементы.ОсновнойЯзык);
		Возврат Ложь;
	КонецЕсли;
	
	Если ПустаяСтрока(ЧасовойПоясПрограммы) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не выбран часовой пояс приложения';
														|en = 'Application time zone not set'"),, Элементы.ЧасовойПоясПрограммы);
		Возврат Ложь;
	КонецЕсли;
	...
	Для ТекущийПорядковыйНомерЯзыка = 1 По КоличествоДополнительныхЯзыков Цикл
		...
		Если ЭтотОбъект[ИмяЭлементаВключенияЯзыка(ТекущийПорядковыйНомерЯзыка)] Тогда
			...
			Если ПустаяСтрока(ЭтотОбъект[ИмяТекущегоЭлемента]) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не выбран дополнительный язык учета';
																|en = 'Additional accounting language not set'"),, ИмяТекущегоЭлемента);
				Возврат Ложь;
			КонецЕсли;
