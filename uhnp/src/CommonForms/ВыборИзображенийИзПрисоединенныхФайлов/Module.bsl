	...
	Если НайденныеСтрокиИзображения.Количество() = 0 
		...
		ТекстСообщения = 
			НСтр("ru = 'Не удается выбрать данное изображение. Выберите другое или обратитесь к администратору';
				|en = 'Не удается выбрать данное изображение. Выберите другое или обратитесь к администратору'", 
...
Процедура Выбрать(Команда)
	...
	Если Не ЗначениеЗаполнено(ВыбранныйФайл) Тогда
		ТекстСообщения = НСтр("ru = 'Выберите файл изображения';
								|en = 'Select an image file'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);	
		Возврат;
	КонецЕсли;
...
Процедура ЗадатьПредставлениеВладельца()
	...
	Если ЗначениеЗаполнено(ПерваяСтрока.Владелец) Тогда
		...
		ШаблонСтроки = НСтр("ru = '<span style=""color: НедоступныйДляВыбораЭлементБЭД"">
			|Изображения из присоединенных файлов</span> <a href=""%1"">%2</a>';
			|en = '<span style=""color: НедоступныйДляВыбораЭлементБЭД"">
			|Images from attachments</span> <a href=""%1"">%2</a>'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Иначе 
		...
		ШаблонСтроки = НСтр("ru = '<span style=""color: НедоступныйДляВыбораЭлементБЭД"">
			|Изображения из присоединенных файлов %1</span>';
			|en = '<span style=""color: НедоступныйДляВыбораЭлементБЭД"">
			|Images from attachments %1</span>'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура УказатьДоступныеРасширения()
	...
	ТекстНадписи = СтрШаблон(НСтр("ru = 'Доступные форматы: %1';
									|en = 'Доступные форматы: %1'"), ТекстДоступныхФорматов);
