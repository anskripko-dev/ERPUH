...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ФормыУХ.СоздатьДобавитьРеквизитФормы(ЭтотОбъект, ДобавляемыеРеквизиты, "СпособОпределенияКурсаПлатежа", "ПеречислениеСсылка.СпособыОпределенияКурсаПлатежа");
	ФормыУХ.СоздатьДобавитьРеквизитФормы(ЭтотОбъект, ДобавляемыеРеквизиты, "ВариантКурсаДоговораТекст", "Строка");
	ИзменитьРеквизиты(ДобавляемыеРеквизиты);
	ЭтотОбъект["СпособОпределенияКурсаПлатежа"] = Параметры.СпособОпределенияКурсаПлатежа;   
	ДобавитьИЗаполнитьТекстВариантаКурсаДоговора();
	#КонецОбласти
...
Процедура ЗаполнитьСписокКурсовВалют(ИзмененаВалюта=Ложь)
	...
	СписокКурсовВалют = Новый СписокЗначений;
	МассивДнейДляСпискаВыбора = Новый Массив;   
	ИспользоватьДатуКурсаИзДоговора = Ложь;    
	Если ВариантКурсаДоговора = Перечисления.ВариантыКурсаДоговора.УстановленныйВДоговоре Тогда 
		Если ЭтотОбъект["СпособОпределенияКурсаПлатежа"] = Перечисления.СпособыОпределенияКурсаПлатежа.ФиксированныйКурс Тогда 
			МассивДнейДляСпискаВыбора = ПолучитьДатыКурсаВалютыДоговора();  
		ИначеЕсли ЭтотОбъект["СпособОпределенияКурсаПлатежа"] = Перечисления.СпособыОпределенияКурсаПлатежа.КурсНаФиксированнуюДату Тогда 
			ДатаФиксацииКурсаПлатежа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "ДатаФиксацииКурсаПлатежа");
			ДатаКурса = ДатаФиксацииКурсаПлатежа;  
			ИспользоватьДатуКурсаИзДоговора = Истина;  
		ИначеЕсли ЭтотОбъект["СпособОпределенияКурсаПлатежа"] = Перечисления.СпособыОпределенияКурсаПлатежа.КурсНаДатуПлатежа Тогда 
			СдвигДатыФиксацииКурсаПлатежа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "СдвигДатыФиксацииКурсаПлатежа");
            ДатаКурса = ДатаДокумента - (СдвигДатыФиксацииКурсаПлатежа * 86400);
			ИспользоватьДатуКурсаИзДоговора = Истина;
		КонецЕсли;
	Иначе   
		Для ДеньМинус = 1 По 5 Цикл
	...
	#Область УХ_Встраивание
	Если  ИспользоватьДатуКурсаИзДоговора Тогда   
		ДатаДляКурса = ?(ОтразитьПоКурсуНаДату, ДатаКурсаВалютыДокумента, ДатаКурса);
	Иначе      
...
Функция ПолучитьКурсВалюты(Валюта, Дата, БазоваяВалюта)
	...
	Если ВариантКурсаДоговора = Перечисления.ВариантыКурсаДоговора.УстановленныйВДоговоре 
		...
		Результат = РегистрыСведений.КурсыВалютРасчетовПоДоговорам.ПолучитьЗначенияКурсаВалютыДоговораУХ(Договор, Дата, ВалютаДокумента, ВалютаВзаиморасчетов);
		#КонецОбласти
...
Процедура УстановитьВидимостьЭлементов()
	...
	Элементы["ВариантКурсаДоговораТекст"].Видимость = ВалютаВзаиморасчетов <> ВалютаРегУчета;
	Элементы["ВариантКурсаДоговораТекст"].Гиперссылка = ЭтотОбъект["СпособОпределенияКурсаПлатежа"] = Перечисления.СпособыОпределенияКурсаПлатежа.ФиксированныйКурс;
	#КонецОбласти
...
#Область УХ_Встраивание
&НаСервере
Процедура ДобавитьИЗаполнитьТекстВариантаКурсаДоговора()  
	
	ПараметрыЭлемента = Новый Структура;
	ПараметрыЭлемента.Вставить("Заголовок", НСтр("ru = 'Курс'"));
	СобытиеЭлемента = Новый Структура("Нажатие", "ВариантКурсаДоговораТекстНажатие"); 
	
	ЭлементВариантКурсаДоговораТекст = ФормыУХ.СоздатьПолеФормы(Элементы,"ВариантКурсаДоговораТекст",,"ВариантКурсаДоговораТекст", ВидПоляФормы.ПолеНадписи,Элементы.ГруппаЛево,Элементы.ГруппаДатаКурсаВалютыДокумента, ПараметрыЭлемента, СобытиеЭлемента);
	ЭлементВариантКурсаДоговораТекст.Гиперссылка = ЭтотОбъект["СпособОпределенияКурсаПлатежа"] = Перечисления.СпособыОпределенияКурсаПлатежа.ФиксированныйКурс;
	...
	Если ЭтотОбъект["СпособОпределенияКурсаПлатежа"] = Перечисления.СпособыОпределенияКурсаПлатежа.ФиксированныйКурс Тогда	
		ЭтотОбъект["ВариантКурсаДоговораТекст"] = НСтр("ru = 'Явно указанный'"); 
	ИначеЕсли ЭтотОбъект["СпособОпределенияКурсаПлатежа"] = Перечисления.СпособыОпределенияКурсаПлатежа.КурсНаФиксированнуюДату Тогда  
		ДатаФиксацииКурсаПлатежа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "ДатаФиксацииКурсаПлатежа");
		ЭтотОбъект["ВариантКурсаДоговораТекст"] = НСтр("ru = 'На указанную дату:'") +" "+Формат(ДатаФиксацииКурсаПлатежа, "ДЛФ=D"); 
	ИначеЕсли ЭтотОбъект["СпособОпределенияКурсаПлатежа"] = Перечисления.СпособыОпределенияКурсаПлатежа.КурсНаДатуПлатежа Тогда  
   		СдвигДатыФиксацииКурсаПлатежа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "СдвигДатыФиксацииКурсаПлатежа");
		ДатаФиксацииКурсаПлатежа = ?(ОтразитьПоКурсуНаДату И ЗначениеЗаполнено(ДатаКурсаВалютыДокумента),ДатаКурсаВалютыДокумента, ДатаДокумента-(СдвигДатыФиксацииКурсаПлатежа * 86400)); 	
		КурсВалютыВзаиморасчетов = РаботаСКурсамиВалютУТ.ПолучитьКурсВалюты(ВалютаВзаиморасчетов,ДатаФиксацииКурсаПлатежа,ВалютаРегУчета);
		ЭтотОбъект["ВариантКурсаДоговораТекст"] = НСтр("ru = 'На дату операции:'")+" "+Формат(ДатаФиксацииКурсаПлатежа, "ДЛФ=D");
	ИначеЕсли ЭтотОбъект["СпособОпределенияКурсаПлатежа"] = Перечисления.СпособыОпределенияКурсаПлатежа.КурсНаДатуОтгрузки Тогда 
		ЭтотОбъект["ВариантКурсаДоговораТекст"] = НСтр("ru = 'На дату отгрузки'");  
	КонецЕсли;
	
КонецПроцедуры   

&НаКлиенте
Процедура ВариантКурсаДоговораТекстНажатие(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ПараметрыОткрытия = Новый Структура("Отбор", Новый Структура("Договор", Договор));
	ОткрытьФорму("РегистрСведений.КурсыВалютРасчетовПоДоговорам.Форма.ФормаСпискаПоДоговору", ПараметрыОткрытия);
КонецПроцедуры 
