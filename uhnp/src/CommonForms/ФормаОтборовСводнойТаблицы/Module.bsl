...
Процедура СформироватьПредставлениеОтборов();
	  ...
	  СтрокаШаблона = Нстр("ru = '%1  Период: с %2 по %3 '");
	  
	  ПредставлениеОтборов = "";
	  Если Не ПустаяСтрока(СтрокаШаблона) тогда		  
		  ПредставлениеОтборов = СтрШаблон(СтрокаШаблона, СокрЛП(Строка(Бланк)), ПериодПланированияОтборНачало, ПериодПланированияОтборОкончание);
	  КонецЕсли;
