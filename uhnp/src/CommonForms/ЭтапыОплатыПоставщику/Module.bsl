...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Ключ) 
		И ТипЗнч(Ключ) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") 
		И ЗначениеЗаполнено(Договор) Тогда
		
		ФормыУХ.СоздатьКоманду(ЭтотОбъект, "ЗаполнитьПоДоговору", "Заполнить по договору", 
						"Подключаемый_ЗаполнитьПоДоговору", БиблиотекаКартинок.ЗаполнитьЭтапыГрафикаОплаты);	
		Команды.ЗаполнитьПоДоговору.ИзменяетСохраняемыеДанные = Истина;
		
		ФормыУХ.СоздатьКнопкуФормы(Элементы, "ЭтапыГрафикаОплатыЗаполнитьПоДоговору", "Заполнить по договору", "ЗаполнитьПоДоговору", , 
						Элементы.ЭтапыГрафикаОплаты.КоманднаяПанель, Элементы.ЗаполнитьЭтапыГрафикаОплатыТаблица);
	КонецЕсли;	
	#КонецОбласти
...
&НаКлиенте
Процедура Подключаемый_ЗаполнитьПоДоговору(Команда)  

	ЗаполнитьЭтапыПоЗаказамПоДоговоруСервер();
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьЭтапыПоЗаказамПоДоговоруСервер()
	...
	ТаблицаЭтапыГрафикаОплаты = ВерсияСоглашения.ЭтапыГрафикаОплаты.Выгрузить(); 
	...
	ПараметрыЗаполнения.Вставить("ДатаЗаказа", ДатаЗаказа);
	ПараметрыЗаполнения.Вставить("ДатаЗаказа", Ключ.ДатаПоступления);
	ПараметрыЗаполнения.Вставить("ШаблонГрафика", ТаблицаЭтапыГрафикаОплаты);
	ПараметрыЗаполнения.Вставить("СуммаОплаты", СуммаОплатыПоДокументу);
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ВерсияСоглашения,"СпособРасчетаКоличестваДней") Тогда  
		Если ВерсияСоглашения.СпособРасчетаКоличестваДней = Перечисления.СпособыРасчетаКоличестваДнейВПериоде.ПоРабочимДням Тогда
			Календарь = ВерсияСоглашения.ПроизводственныеКалендари.Выгрузить().ВыгрузитьКолонку("ПроизводственныйКалендарь")  
		КонецЕсли;
	КонецЕсли;
	...
	ЭтапыОплатыСервер.ЗаполнитьЭтапыОплатыДокументаПоШаблону(ТаблицаОплат, ПараметрыЗаполнения);  	
	ЭтапыОплатыСервер.ЗаполнитьПроцентыПоСуммам(ТаблицаОплат);	
	ЭтапыОплатыКлиентСервер.СортироватьТаблицуЭтапов(ТаблицаОплат);
	
	ЭтапыГрафикаОплаты.Загрузить(ТаблицаОплат);
...
Функция ТаблицаЭтапыГрафикаОплаты()
	...
	ИменаКолонок = "ВариантОплаты,ДатаПлатежа,ПроцентПлатежа,СуммаПлатежа,СуммаЗадолженности,
					|СуммаОтгрузок,СуммаПлатежей,Комментарий,ДатаПоступления, Замещен,Сдвиг,ВариантОтсчета"; 
