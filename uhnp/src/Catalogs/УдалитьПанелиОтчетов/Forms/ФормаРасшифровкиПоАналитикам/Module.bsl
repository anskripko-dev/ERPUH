...
Функция ПолучитьАдресТаблицыДинамики(ПоказательВход, КонтекстВход, СтруктураАналитикСтроки, ИмяОперанда = "")
	РезультатФункции = Справочники.УдалитьПанелиОтчетов.ПолучитьАдресТаблицыДинамики(ПоказательВход, КонтекстВход, ИмяОперанда, СтруктураАналитикСтроки);
	Возврат РезультатФункции;
КонецФункции
...
Процедура СформироватьЗаголовокФормы()
	Если ЗначениеЗаполнено(Показатель.ЕдиницаИзмерения) Тогда
		ЗаголовокФормы = НСтр("ru = '%Показатель% (%ЕдиницаИзмерения%) - %ПериодОтчета%'");
...
Процедура РасшифроватьПоДинамике(ИмяПоляВход)
	...
	Если ТекДанные <> Неопределено Тогда
		...
		Если СтруктураКонтекста.Количество() > 0 Тогда
			...
			ОткрытьФорму("Справочник.УдалитьПанелиОтчетов.Форма.ФормаДинамикиПоказателя", ПараметрыФормы);	
		Иначе
