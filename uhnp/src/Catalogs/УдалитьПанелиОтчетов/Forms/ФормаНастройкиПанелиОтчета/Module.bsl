	...
	Если ЗначениеЗаполнено(Монитор) Тогда
		Если Монитор.ИсточникиДанных.Количество() > 0 Тогда
			...
			Если ЗначениеЗаполнено(ПерваяСтрокаИсточники.ИсточникДанных) Тогда
				...
				АдресТаблицыРасшифровки = Справочники.УдалитьПанелиОтчетов.ПолучитьАдресТаблицыРасшифровкиПоАналитикам(Показатель, Контекст, Истина);
...
Функция ОпределитьВидПроизвольногоОтчета(ТекущийОтчет)
	...
	Возврат Справочники.УдалитьПанелиОтчетов.ОпределитьВидПроизвольногоОтчетаДляОтображения(ТекущийОтчет);
