...
Функция НаименованиеВидаЗапасов(СправочникОбъект)
	...
	Если СправочникОбъект.ТипЗапасов = Перечисления.ТипыЗапасов.КомиссионныйТовар Тогда
		ПредставлениеВладельца = НСтр("ru = 'Комитент:';
										|en = 'Consignor:'", КодОсновногоЯзыка) + Символы.НПП
		...
		Если ЗначениеЗаполнено(СправочникОбъект.Соглашение) Тогда
			ПредставлениеСоглашения = НСтр("ru = 'Соглашение:';
											|en = 'Agreement:'", КодОсновногоЯзыка) + Символы.НПП
		...
		Если ЗначениеЗаполнено(СправочникОбъект.Договор) Тогда
			ПредставлениеДоговора = НСтр("ru = 'Договор:';
										|en = 'Contract:'", КодОсновногоЯзыка) + Символы.НПП
	...
	ИначеЕсли СправочникОбъект.ТипЗапасов = Перечисления.ТипыЗапасов.ПродукцияДавальца
		...
		ПредставлениеВладельца = НСтр("ru = 'Давалец:';
										|en = 'Material provider:'", КодОсновногоЯзыка) + Символы.НПП
										+ Строка(СправочникОбъект.ВладелецТовара);
	...
	ИначеЕсли СправочникОбъект.ТипЗапасов = Перечисления.ТипыЗапасов.ТоварНаХраненииСПравомПродажи Тогда
		ПредставлениеВладельца = НСтр("ru = 'Поклажедатель:';
										|en = 'Depositor:'", КодОсновногоЯзыка) + Символы.НПП
										+ Строка(СправочникОбъект.ВладелецТовара);
		ПредставлениеДоговора = НСтр("ru = 'Договор:';
									|en = 'Contract:'", КодОсновногоЯзыка) + Символы.НПП
											+ Строка(СправочникОбъект.Договор);
		...
		Если ЗначениеЗаполнено(СправочникОбъект.ВидЦены) Тогда
			ПредставлениеВидаЦены = НСтр("ru = 'Вид цены:';
										|en = 'Price type:'", КодОсновногоЯзыка) + Символы.НПП
	...
	Если ЗначениеЗаполнено(СправочникОбъект.ГруппаФинансовогоУчета) Тогда
		ПредставлениеГФУ = НСтр("ru = 'Группа:';
								|en = 'Group:'", КодОсновногоЯзыка) + Символы.НПП
	...
	ПредставлениеОрганизации = НСтр("ru = 'Организация:';
									|en = 'Company:'", КодОсновногоЯзыка) + Символы.НПП
	...
	Если ЗначениеЗаполнено(СправочникОбъект.ГруппаПродукции) Тогда
		ПредставлениеГруппыПродукции = НСтр("ru = 'Группа продукции:';
											|en = 'Product group:'", КодОсновногоЯзыка) + Символы.НПП
	...
	Если СправочникОбъект.РеализацияЗапасовДругойОрганизации Тогда
		ЧастиНаименования.Добавить(НСтр("ru = 'Запасы другой организации';
...
Процедура ПроверитьДубли(СправочникОбъект)
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru = 'Запись Видов запасов';
							|en = 'Save Inventory owner attributes'", ОбщегоНазначения.КодОсновногоЯзыка());
