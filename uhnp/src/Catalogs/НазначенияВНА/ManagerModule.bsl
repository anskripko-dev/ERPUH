...
Процедура ПриИзмененииНазначенияВНА(Форма) Экспорт
	...
	ТипВНА = Перечисления.ТипыВНА.ПолучитьКлассВНАПоСправочнику(Объект.Ссылка.Метаданные());
	
	ЕстьНазначение = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "НазначениеВНА");
	...
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоЕХ() 
		...
		И (ТипВНА = Перечисления.ТипыВНА.ОсновноеСредство) Тогда
		ТекущийВид = Объект.Класс;//в ЕРП для ОС видимость брать из класса ОЭ
	КонецЕсли;

	УстановитьСвойстваЭлементовПоНазначениюВНАНаСервере(Элементы, ТекущийВид, ТипВНА);
...
//		элементов формы.
//  ТипВНАПоУмолчанию - ПеречислениеСсылка.ТипыВНА - класс
//		описывающий объект ВНА для обработки случая, когда
//		НазначениеВНА не заполнен.
//
Процедура УстановитьСвойстваЭлементовПоНазначениюВНАНаСервере(
				Элементы, НазначениеВНА, ТипВНАПоУмолчанию) Экспорт
	
	Если ЗначениеЗаполнено(НазначениеВНА) Тогда
		
		Если ИдентификацияПродуктаУХКлиентСервер.ЭтоЕХ() Тогда //скроем все УХ
			
			ТаблицаРеквизитов = ЭлементыФормыВНАДляУправленияВидимостью(ТипВНАПоУмолчанию);
...
//
Функция МетаданныеСправочникаПоКлассуВНА(ТипВНА) Экспорт
	Если ТипВНА = Перечисления.ТипыВНА.ОсновноеСредство Тогда
		Возврат ВстраиваниеУХКлиентСервер.ПолучитьПустуюСсылкуОС().Метаданные();
	ИначеЕсли ТипВНА = Перечисления.ТипыВНА.НематериальныйАктив Тогда
		Возврат Метаданные.Справочники.НематериальныеАктивы;
	ИначеЕсли ТипВНА = Перечисления.ТипыВНА.НезавершенноеСтроительство Тогда
...
//
// Параметры:
//	ТипВНА - ПеречислениеСсылка.ТипыВНА.
//
// Возвращаемое значение:
//	Структура - поля:
//		МетаданныеСправочника - ОбъектМетаданных: Справочник -
//			метаданные справочника хранящего элементы ВНА данного класса.
...
//			справочника, которые не участвуют в управления видимостью.
//
Функция ОписаниеКлассаВНАДляУправленияВидимостью(ТипВНА) Экспорт
	...
	ОписаниеКлассаВНА.Вставить("МетаданныеСправочника",
		МетаданныеСправочникаПоКлассуВНА(ТипВНА));
	...
	Если ТипВНА = Перечисления.ТипыВНА.ОсновноеСредство Тогда
		...
		Если ИдентификацияПродуктаУХКлиентСервер.ЭтоЕХ() Тогда
			ОписаниеКлассаВНА.ЭлементыОтображения.Вставить("СостояниеВНА", 		НСтр("ru = 'Состояние'")				);
		Иначе
...
//
Функция ЭлементыФормыВНАДляУправленияВидимостью(ТипВНА) Экспорт
	ОписаниеКлассаВНА = ОписаниеКлассаВНАДляУправленияВидимостью(ТипВНА);
...
Процедура ЗаписатьНаборЗаписей(Форма, ИмяРеквизитаНаборЗаписей, Отбор, Отказ = Ложь, Знач ПредставлениеРеквизита = Неопределено) Экспорт
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, ИмяРеквизитаНаборЗаписей) Тогда
		Возврат;	
	КонецЕсли;
...
Процедура ПриСозданииНаСервереВНА(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если ПраваДоступаМСФО() Тогда
		...
		ТипыВНА = Перечисления.ТипыВНА.ПолучитьКлассВНАПоСправочнику(Объект.Ссылка.Метаданные());
		Если ТипыВНА <> Перечисления.ТипыВНА.ОсновноеСредство Тогда
		...
		Если ЕстьРСБУ И Не Объект.Ссылка.Пустая() Тогда
...
Процедура ПриЗаписиНаСервереВНА(Форма, Отказ, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ДоговорыЗалога") Тогда
...
Процедура УстановитьТипыВНА() Экспорт
	...
	|	ЗНАЧЕНИЕ(Перечисление.ТипыВНА.НематериальныйАктив) КАК ТипыВНА
	|ПОМЕСТИТЬ НесгруппированныеНазначенияВНА
	|ИЗ
	|	Справочник.НематериальныеАктивы КАК НематериальныеАктивы
	|ГДЕ
	|	НематериальныеАктивы.НазначениеВНА <> &НазначениеВНА
	|	И НематериальныеАктивы.НазначениеВНА.ТипыВНА = &ТипыВНА
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ОбъектыСтроительства.НазначениеВНА,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыВНА.НезавершенноеСтроительство)
	|ИЗ
	|	Справочник.ОбъектыСтроительства КАК ОбъектыСтроительства
	|ГДЕ
	|	ОбъектыСтроительства.НазначениеВНА <> &НазначениеВНА
	|	И ОбъектыСтроительства.НазначениеВНА.ТипыВНА = &ТипыВНА
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И ОсновныеСредства.НазначениеВНА.ТипыВНА = &ТипыВНА
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	т.НазначениеВНА КАК НазначениеВНА,
	|	МАКСИМУМ(т.ТипыВНА) КАК ТипыВНА
	|ИЗ
	|	несгруппированныеНазначенияВНА КАК т
	|
	|СГРУППИРОВАТЬ ПО
	|	т.НазначениеВНА";
	...
	Запрос.УстановитьПараметр("ТипыВНА", Перечисления.ТипыВНА.ПустаяСсылка());
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		НазначениеВНАОбъект.ТипВНА = ВыборкаДетальныеЗаписи.ТипВНА;
