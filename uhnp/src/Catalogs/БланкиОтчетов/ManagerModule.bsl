...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Ключ") И ЗначениеЗаполнено(Параметры.Ключ) Тогда
		...
		ИначеЕсли ОбщегоНазначенияПовтИспУХ.ИспользуетсяМногопериоднаяОтчетность() Тогда
			ВыбраннаяФорма = "ФормаМакетаАналитическийБланк";
		Иначе
	...
	ИначеЕсли Параметры.Свойство("ЗначенияЗаполнения") И ЗначениеЗаполнено(Параметры.ЗначенияЗаполнения) Тогда
		
		Если Параметры.ЗначенияЗаполнения.РежимБланка = 1 Тогда
			Параметры.Вставить("РежимыДоступаКФункциямСводнойТаблицы"
				,ПредопределенноеЗначение("Перечисление.РежимыДоступаКФункциямСводнойТаблицы.РазработкаМакета"));
			ВыбраннаяФорма = "Обработка.АналитическийБланкСводнаяТаблица.Форма.ФормаМакетаСводнаяТаблица";
		ИначеЕсли Параметры.ЗначенияЗаполнения.РежимБланка = 2 Тогда
			ВыбраннаяФорма = "ФормаМакетаАналитическийБланк";
		ИначеЕсли ОбщегоНазначенияПовтИспУХ.ИспользуетсяМногопериоднаяОтчетность() Тогда
			ВыбраннаяФорма = "ФормаМакетаАналитическийБланк";
		Иначе
...
Функция ПолучитьЗаголовокОбластиРаскрытия(Расшифровка) Экспорт
	
	НастройкиАналитикРаскрытия = Расшифровка.НастройкиАналитикРаскрытия;
	
	ЗаголовокИсходный = "";
	
	Для Каждого СтрАналитика Из НастройкиАналитикРаскрытия Цикл
		
		Если ЗаголовокИсходный = "" Тогда
			Если СтрАналитика.ЭтоПроизводноеПоле Тогда
				ЗаголовокИсходный = ЗаголовокИсходный+СтрАналитика.АналитикаПредставление
				+СтрЗАменить(СтрАналитика.ПутьКРеквизиту,СтрАналитика.АналитикаКод,"");	
			Иначе	
				ЗаголовокИсходный = ЗаголовокИсходный+СтрАналитика.АналитикаПредставление;
			КонецЕсли;	
		Иначе	
...
//При переходе на версию 3.2.2.18 + Отборы ячеек и макетов раскрытий всех бланков
//должны использовать перечисление ВидыСравненияПоказателейОтчетов, вместо строковоых представлений ранее.
Процедура ВыполнитьКонвертациюОтборовБланков(ВсеБланки = Истина,Бланк=Неопределено) Экспорт
	...
	Если ВсеБланки Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ОбщегоНазначения.ВыполнитьМетодКонфигурации("Обработки.АналитическийБланк.СконверироватьБланк", Параметры);
...
Процедура ВыполнитьКонвертациюАналитикБланков(ВсеБланки = Истина,Бланк=Неопределено) Экспорт
	...
	Если ВсеБланки Тогда
		...
		Пока Выборка.Следующий() Цикл		
			Параметры = Новый Массив();
			Параметры.Добавить(Выборка.Ссылка);
			ОбщегоНазначения.ВыполнитьМетодКонфигурации("Обработки.АналитическийБланк.СконверироватьАналитикиБланка", Параметры);			
		КонецЦикла;
...
Процедура ОбновитьКэшБланков(ВсеБланки = Истина,Бланк=Неопределено) Экспорт
	...
	Если ВсеБланки Тогда
		...
		Пока Выборка.Следующий() Цикл		
			Параметры = Новый Массив();
			Параметры.Добавить(Выборка.Ссылка);
			ОбщегоНазначения.ВыполнитьМетодКонфигурации("Обработки.АналитическийБланк.ПерезаписатьБланки", Параметры);			
		КонецЦикла;
...
Процедура СконвертироватьМакетыРазработкиБланковЭО(ВсеБланки = Истина,Бланк=Неопределено) Экспорт
	...
	Если ВсеБланки Тогда
		...
		Пока Выборка.Следующий() Цикл		
			Параметры = Новый Массив();
			Параметры.Добавить(Выборка.Ссылка);
			ОбщегоНазначения.ВыполнитьМетодКонфигурации("Обработки.АналитическийБланк.СконверироватьМакетБланка", Параметры);			
		КонецЦикла;
