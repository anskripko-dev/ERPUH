...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыНастроек.ШаблонДокумента = Объект.Владелец;
	ЗначенияНастроек = УправлениеОтчетамиУХ.НастройкиОбработкиШаблонов(ПараметрыНастроек);
		
	ПравилоОбработки = ЗначенияНастроек.ПравилоОбработки;
	...
	Иначе
		ПоУмолчаниюИмпорт     = ЗначенияНастроек.БланкДляИмпорта = Объект.Ссылка;
		ПоУмолчаниюОтображение = ЗначенияНастроек.БланкДляОтображения = Объект.Ссылка;
	КонецЕсли;
...
Процедура ГенерацияМакета(Команда)
	...
	ВнИмяФормы="Обработка.МастерГенерацииМакетаОтчета.Форма.Форма";
	ОткрытьФорму(ВнИмяФормы, ПодготовитьПараметрыДляМакета(), ЭтаФорма);
...
Процедура Подключаемый_СопоставитьОбластьПоказателюЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
    ...
    Область = ДополнительныеПараметры.Область; // ОбластьЯчеекТабличногоДокумента
...
Функция УстановитьПроцедуруПоУмолчанию(Элемент, ВидОтчета, БланкДляИмпорта = Ложь)
	...
	Отказ = Ложь;
	УправлениеОтчетамиУХ.ЗаписатьНастройкуПоУмолчаниюОбработкиШаблонов(ВидОтчета, 
		?(БланкДляИмпорта, "БланкДляИмпорта", "БланкДляОтображения"), Элемент, , Отказ);
...
Процедура Подключаемый_ВыборДействияВолшебнойПалочки(РежимДействие, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если РежимДействие = 0 Тогда
		...
		ВнИмяФормы="Обработка.МастерГенерацииМакетаОтчета.Форма.Форма";
		ОткрытьФорму(ВнИмяФормы, ВолшебнаяПалочкаСформироватьМакетНаСервере(), ЭтаФорма);
