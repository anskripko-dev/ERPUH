...
Функция СформироватьПоказатели()
	...
	Запрос.Текст="ВЫБРАТЬ
	             ...
	             |	ЕСТЬNULL(СтрокиОтчетов.СчетБД.Валютный, ЛОЖЬ) КАК Валютный,
	...
	ФормироватьПоказателиРекурсивно(Карта, Показатели,Запрос.Выполнить().Выгрузить());
...
&НаСервере
Процедура ФормироватьПоказателиРекурсивно(Строки, Показатели,ДанныеСтрок)
	...
	Для Каждого Элемент Из ВнЭлементы Цикл
		...
		ТекСтрока=ДанныеСтрок.Найти(Элемент.Строка,"Ссылка");
		...
		Для Каждого КлючИЗначение Из СтруктураКодовИСсылокНаКолонки Цикл
			...
			Если Элемент[КлючИЗначение.Ключ] Тогда
				...
				Если Использование <> 0 Тогда
					...
					Справочники.ВидыОтчетов.СформироватьПоказатель(ВидОтчета,ТекСтрока,КлючИЗначение.Значение,Использование, ОписаниеНовогоПоказателя,,,СтруктураКодовИСсылокНаКолонки.Количество()=1,ТаблицаИзмененныхПоказателей);
		...
		ФормироватьПоказателиРекурсивно(Элемент, Показатели,ДанныеСтрок);
