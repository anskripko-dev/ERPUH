...
Процедура ПередЗаписью(Отказ)
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
		...
		Если НЕ ОбъектДляСогласованияЗначений=Ссылка.ОбъектДляСогласованияЗначений Тогда
			...
			ПроверитьИзменениеОбъектаДляСогласования(Отказ);
	...
	// Обновим число аналитик
	КоличествоАналитик = 
		?(ЗначениеЗаполнено(ВидАналитики1), 1, 0) + 
		?(ЗначениеЗаполнено(ВидАналитики2), 1, 0) + 
		?(ЗначениеЗаполнено(ВидАналитики3), 1, 0) + 
		?(ЗначениеЗаполнено(ВидАналитики4), 1, 0) + 
		?(ЗначениеЗаполнено(ВидАналитики5), 1, 0) + 
		?(ЗначениеЗаполнено(ВидАналитики6), 1, 0);
...
Процедура ПриЗаписи(Отказ)
	...
	Если Предназначение=Перечисления.ПредназначенияЭлементовСтруктурыОтчета.ОборотноСальдоваяВедомость 
		И ИспользоватьПоУмолчанию Тогда
	...
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом() И НЕ ПометкаУдаления Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ПланированиеИКонтрольСлужебный")
		И ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом() Тогда
...
Процедура ПроверитьИзменениеОбъектаДляСогласования(Отказ)
	
	СогласованиеЧерезРакурс = РасширениеПроцессыИСогласованиеУХ.СогласованиеЧерезРакурс(Ссылка);
	...
	Если СогласованиеЧерезРакурс Тогда
		Если МаксАналитикОтчета = 0 Тогда
			Запрос.Текст = ТекстЗапросаСостоянияСтатусовПоПоказателямСинтетика();
		Иначе
			Запрос.Текст = ТекстЗапросаСостоянияСтатусовПоПоказателямАналитика(МаксАналитикОтчета);
		КонецЕсли;
	Иначе
		Запрос.Текст = ТекстЗапросаСостоянияСтатусовКлючевогоСреза();
	КонецЕсли;
	
	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	Выборка = Запрос.Выполнить().Выбрать();
	
	ТекстОшибки = ТекстОшибкиИзмененияОбъектаДляСогласования(Выборка);
	Если Не ПустаяСтрока(ТекстОшибки) Тогда
	...
	Если НЕ Отказ Тогда
		...
		Иначе
			...
			РегистрыСведений.ЗначенияСтатусовКлючевыхСрезов.УдалитьЗаписиПоВидуОтчета(Ссылка, Отказ);
...
Функция ТекстЗапросаСостоянияСтатусовПоПоказателямСинтетика()
...
Функция ТекстЗапросаСостоянияСтатусовПоПоказателямАналитика(МаксАналитикОтчета)
...
Функция ТекстЗапросаСостоянияСтатусовКлючевогоСреза()
	
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЗначенияСтатусовКлючевыхСрезов.ОрганизационнаяЕдиница.Наименование КАК ОрганизацияНаименование,
	|	ЗначенияСтатусовКлючевыхСрезов.ПериодОтчета.Наименование КАК ПериодОтчетаНаименование,
	|	ЗначенияСтатусовКлючевыхСрезов.Сценарий.Наименование КАК СценарийНаименование,
	|	ЗначенияСтатусовКлючевыхСрезов.Проект.Наименование КАК ПроектНаименование,
	|	ЗначенияСтатусовКлючевыхСрезов.ИндексСостояния,
	...
	|ГДЕ
	|	ЗначенияСтатусовКлючевыхСрезов.ВидОтчета = &Ссылка
	|	И (ЗначенияСтатусовКлючевыхСрезов.ИндексСостояния > 0
	|			ИЛИ ЗначенияСтатусовКлючевыхСрезов.ЗначениеЗаблокировано > 1)";

	Возврат ТекстЗапроса;
	
КонецФункции

Функция ТекстОшибкиИзмененияОбъектаДляСогласования(Выборка)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ПустаяСтрока(Выборка.ПроектНаименование) тогда
			
			СтрокаШаблона = НСтр("ru = 'Организация: %1, период: %2, сценарий: %3'");
			
			Если Не ПустаяСтрока(СтрокаШаблона) тогда
				СодержаниеОшибки = СтрШаблон(СтрокаШаблона, Выборка.ОрганизацияНаименование,
					Выборка.ПериодОтчетаНаименование, Выборка.СценарийНаименование);
			КонецЕсли;
		...
		Иначе
			...
			Если Не ПустаяСтрока(СтрокаШаблона) тогда
				СодержаниеОшибки = СтрШаблон(СтрокаШаблона, Выборка.ОрганизацияНаименование,
					Выборка.ПериодОтчетаНаименование, Выборка.СценарийНаименование, Выборка.ПроектНаименование);
			КонецЕсли;
