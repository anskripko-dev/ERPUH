...
Процедура ОпределитьПараметрыЗагрузкиДанныхИзФайла(Параметры) Экспорт
	...
	//++ НЕ УТ
	Параметры.ТипДанныхКолонки.Вставить("ПереноситьРНПТМатериала",  ТипБулево);
	//-- НЕ УТ
...
Функция ТабличныйДокументКарточкиНоменклатуры(Номенклатура, НастройкиВидимостиИЗаголовков, ДляПечати) Экспорт
	...
	Если ГлаваВидна("ПланированиеИМаркетинг", СтруктураВидимости) Тогда 
		...
		Если РазделВиден("ТоварнаяКатегория", СтруктураВидимости) Тогда
            ...
            КатегорияЗакупок = Макет.Область("R49C2:R49C9");   
			Если КатегорияЗакупок.Текст = "Товарная категория:" Тогда
			КатегорияЗакупок.Текст = "Категория закупок";
			ПараметрыВыводаРаздела = ПараметрыВыводаРазделаСУчетомРазмераЗначений();
			ПараметрыВыводаРаздела.ИмяРаздела = "ТоварнаяКатегория";
			ПараметрыВыводаРаздела.ИмяОбластиСтрока = "ТоварнаяКатегорияСтрока";
			ПараметрыВыводаРаздела.ИмяПараметраРеквизит = "ТоварнаяКатегорияРеквизит";
			ПараметрыВыводаРаздела.ТекстЗначенияРеквизита =  Объект.КатегорияЗакупок;
			ВывестиРазделСУчетомРазмераЗначений(ТабличныйДокументЛево, Макет, ДляПечати, ПараметрыВыводаРаздела);    
			КонецЕсли;
			#КонецОбласти
		КонецЕсли;
	...
	Если РазделВиден("ГиперссылкаПерейтиСоставНабора", ВидимостьЭлементов) Тогда
		...
		Если Количество <> 0 Тогда
			...
			ЗаголовокГиперссылки = Новый ФорматированнаяСтрока(НСтр("ru = 'Состав набора';
																	|en = 'Set content'") + " (" + Количество + ")",,,,ИмяГиперссылки);
		Иначе
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Не удалось обработать справочник ""Номенклатура"" по обработчику:';
								|en = 'Cannot process the ""Items"" catalog by the handler:'"));
	СписокОписаний.Добавить(НСтр("ru = '- замена код ОКВЭД2 с типом справочник Классификатор ОКВЭД2 на строка';
								|en = '- Replace OKVED2 code of the OKVED2 classifier catalog type with a string'"));
	Запрос = Новый Запрос;
	Запрос.Текст = 
