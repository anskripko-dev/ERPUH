...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПраваДоступаМСФО() Тогда
		Если (ВариантУчетаВНА = 0) И НеРеклассифицироватьПоПорогу Тогда
			Справочники.ГруппыВНАМСФО.СохранитьПараметрыПоУмолчаниюВНА(ТекущийОбъект.Ссылка, Справочники.ГруппыВНАМСФО.ПустаяСсылка(), Истина, Отказ);
		Иначе 
			Справочники.ГруппыВНАМСФО.СохранитьПараметрыПоУмолчаниюВНА(ТекущийОбъект.Ссылка, ПараметрыУчетаМСФО, , Отказ);
		КонецЕсли;
	КонецЕсли;
	#КонецОбласти
...
Процедура СтранаПроисхожденияПриИзменении(Элемент)
	...
	//++ НЕ УТ
	УстановитьДоступностьПрослеживаемыйТоварЛокализация(Элементы.ПрослеживаемыйТовар, Объект);
	//-- НЕ УТ
...
Процедура ПереноситьРНПТМатериалаПриИзменении(Элемент)
	...
	//++ НЕ УТ
	Если Объект.ПереноситьРНПТМатериала Тогда
		Объект.ПрослеживаемыйТовар = Истина;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформлениеЛокализация()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетДрагоценныхМатериалов") Тогда
		...
		ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ДрагоценныеМатериалы.Расположение");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = Перечисления.РасположениеДрагоценныхКамней.ПустаяСсылка();
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьДоступностьПрослеживаемыйТоварЛокализация(ПолеЭлемента, Объект)
	
	//++ Локализация
	Если Не Объект.ПрослеживаемыйТовар
		И Объект.ПереноситьРНПТМатериала Тогда
		
		ПолеЭлемента.Доступность = Истина;
		
	Иначе
		ПолеЭлемента.Доступность = Не Объект.ПереноситьРНПТМатериала;
	КонецЕсли;
	//-- Локализация
...
Процедура УстановитьПодсказкуПрослеживаемыйТоварЛокализация(ПолеЭлемента, ТипНоменклатуры, ТипНоменклатурыРабота)
	...
	Если ТипНоменклатуры = ТипНоменклатурыРабота Тогда
		ПолеЭлемента.Подсказка = НСтр("ru = 'Если установлен признак, то можно получать и передавать прослеживаемые товары в составе работы';
										|en = 'If the checkbox is selected, you can get and transfer traceable goods as a part of the work'");
	Иначе
		ПолеЭлемента.Подсказка = НСтр("ru = 'Если установлен признак ""Прослеживаемый товар"", тогда должен быть указан прослеживаемый ""Код ТН ВЭД""';
										|en = 'If the ""Traceable goods"" check box is selected, then the traceable ""FEACN code"" must be specified'");
	КонецЕсли;
	//-- Локализация
...
Процедура ПриСозданииНаСервереУХ(Отказ, СтандартнаяОбработка)
	...
	Если ПраваДоступаМСФО() Тогда
		ОбновитьПараметрыМСФО(ЭтаФорма);
        УправлениеФормойУХ(ЭтаФорма);
	Иначе
		Элементы.ГруппаУХ.Видимость = Ложь;
	КонецЕсли;

КонецПроцедуры

&НаСервереБезКонтекста
Процедура ОбновитьПараметрыМСФО(Форма)
	
	ДатаСведений = ТекущаяДата();
	ПараметрыМСФО = МСФОВНАВызовСервераУХ.ПолучитьПараметрыОбъектаВНА(Форма.Объект.Ссылка, ДатаСведений);
	
	Если Не ПараметрыМСФО.Свойство("ПринятоКУчетуМСФО") Или Не ЗначениеЗаполнено(ПараметрыМСФО.ПринятоКУчетуМСФО) Тогда
		
		ИсточникГруппы = ?(Форма.Объект.Ссылка.Пустая(), Форма.Объект.Родитель, Форма.Объект.Ссылка);
	...
	Форма.НеРеклассифицироватьПоПорогу = (ПараметрыМСФО.ПараметрыУчетаМСФО = Справочники.ГруппыВНАМСФО.ПустаяСсылка());
...
Процедура УправлениеФормойУХ(Форма)
	...
	ПринятоКУчетуМСФО = ЗначениеЗаполнено(Форма.ПринятоКУчетуМСФО);
	ЕстьМСФО = ПринятоКУчетуМСФО Или Форма.ВариантУчетаВНА;
	ЗаполненыПараметрыУчета = ЕстьМСФО И Не Форма.ПараметрыУчетаМСФО.Пустая();
	
	Элементы.ГруппаРасчетаРезерваМСФО.Видимость = Не ЕстьМСФО;
	Элементы.ВидВыручкиМСФО.Видимость 			= Не ЕстьМСФО;
	
	Элементы.ПараметрыУчетаВНА.Видимость		= ЕстьМСФО;
	
	Элементы.ВариантУчетаВНА.ТолькоПросмотр 	= ПринятоКУчетуМСФО;
	Элементы.ПараметрыУчетаВНА.ТолькоПросмотр 	= ПринятоКУчетуМСФО;
	...
	Элементы.ПараметрыУчетаВНА.Заголовок = ?(ПринятоКУчетуМСФО, "Параметры учета", "Параметры по умолчанию");
	
	Элементы.НеРеклассифицироватьПоПорогу.Видимость = Форма.ВариантУчетаВНА = 0;
