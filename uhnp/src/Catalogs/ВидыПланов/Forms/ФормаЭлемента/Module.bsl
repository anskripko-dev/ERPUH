...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ВстраиваниеУХВидыПланов.ПередЗаписьюНаСервере(ЭтотОбъект, Отказ, ТекущийОбъект, ПараметрыЗаписи);
	#КонецОбласти
КонецПроцедуры
...
Процедура ТипПланаПриИзменении(Элемент)
	...
	Если Объект.ТипПлана = ПредопределенноеЗначение("Перечисление.ТипыПланов.ПланЗакупок") Тогда
		...
		// План оплат по бюджету закупок не заполняется
		Объект.ЗаполнятьПланОплат = Ложь;
		Объект.ОтражаетсяВБюджетированииОплаты = Ложь;
		Объект.ОтражаетсяВБюджетированииОплатыКредит = Ложь;
		
		// Заполнение по партнерам и соглашениям невозможно
		Если ЭтоПланированияПотребностей(ЭтаФорма.ФормированиеПотребностейПланомЗакупок) Тогда
			// 
			ЗаполнятьПартнера = Ложь;
			Объект.ЗаполнятьПартнера = Ложь;
			Объект.ЗаполнятьПартнераВТЧ = Ложь;
...
Процедура ПриЧтенииСозданииНаСервере()
	#Область УХ_Внедрение
	ВстраиваниеУХВидыПланов.ПриЧтенииСозданииНаСервере(ЭтаФорма);
	#КонецОбласти
...
Процедура ОбновитьДоступностьЭлементов()
	...
	КонецЕсли;
	#Область УХ_Внедрение
	ОбновитьДоступностьЭлементовСервер();
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоПланированияПотребностей(Режим)
...
&НаСервере
Процедура ОбновитьДоступностьЭлементовСервер()
	
	Перем ФлагВозможностьУказанияПланаОплат, ФлагЗаполненияПоПартнерамИСоглашениям;
	
	ФлагЗаполненияПоПартнерамИСоглашениям = Истина;
	ФлагВозможностьУказанияПланаОплат = Истина;
	Если Объект.ТипПлана = ПредопределенноеЗначение("Перечисление.ТипыПланов.ПланЗакупок") Тогда
		ФлагВозможностьУказанияПланаОплат = Истина;
		Если ЭтоПланированияПотребностей(ЭтаФорма.ФормированиеПотребностейПланомЗакупок) Тогда
			ФлагЗаполненияПоПартнерамИСоглашениям = Ложь;
		КонецЕсли;
	КонецЕсли;
	
	// Возможность планировать по партнерам и соглашениям
	Элементы.ЗаполнятьПартнераЗакупки.Доступность = ФлагЗаполненияПоПартнерамИСоглашениям;
	Элементы.ЗаполнятьПартнераВТЧЗакупки.Доступность = ФлагЗаполненияПоПартнерамИСоглашениям;
	Элементы.ЗаполнятьСоглашение.Доступность = ФлагЗаполненияПоПартнерамИСоглашениям;
	Элементы.ЗаполнятьСоглашениеВТЧЗакупки.Доступность = ФлагЗаполненияПоПартнерамИСоглашениям;
	Если ФлагЗаполненияПоПартнерамИСоглашениям Тогда
		//
		Элементы.ЗаполнятьПартнераЗакупки.Подсказка = "";
		Элементы.ЗаполнятьПартнераЗакупки.ОтображениеПодсказки = ОтображениеПодсказки.Авто;
		//
		Элементы.ЗаполнятьСоглашение.Подсказка = "";
		Элементы.ЗаполнятьСоглашение.ОтображениеПодсказки = ОтображениеПодсказки.Авто;
	Иначе
		//
		Элементы.ЗаполнятьПартнераЗакупки.Подсказка = НСтр("ru = 'Для планов закупок, регистрирующих потребности для подсистемы ""Корпоративные закупки"", планировение по поставщикам невозможно.'");
		Элементы.ЗаполнятьПартнераЗакупки.ОтображениеПодсказки = ОтображениеПодсказки.Кнопка;
		//
		Элементы.ЗаполнятьСоглашение.Подсказка = НСтр("ru = 'Для планов закупок, регистрирующих потребности для подсистемы ""Корпоративные закупки"", планировение по соглашениям невозможно.'");
		Элементы.ЗаполнятьСоглашение.ОтображениеПодсказки = ОтображениеПодсказки.Кнопка;
	КонецЕсли;
	
	// Для плана закупок формирование плана оплат невозможно
	Элементы.ЗаполнятьПланОплатЗакупки.Доступность = ФлагВозможностьУказанияПланаОплат;
	Если ФлагВозможностьУказанияПланаОплат Тогда
		Элементы.ЗаполнятьПланОплатЗакупки.Подсказка = "";
		Элементы.ЗаполнятьПланОплатЗакупки.ОтображениеПодсказки = ОтображениеПодсказки.Авто;
	Иначе
		Элементы.ЗаполнятьПланОплатЗакупки.Подсказка = НСтр("ru = 'Для планов закупок заполнение плана оплат невозможно.'");
		Элементы.ЗаполнятьПланОплатЗакупки.ОтображениеПодсказки = ОтображениеПодсказки.Кнопка;
	КонецЕсли;
	
	ВстраиваниеУХВидыПланов.УстановитьЗначенияИСвойстваФормы(ЭтотОбъект);
