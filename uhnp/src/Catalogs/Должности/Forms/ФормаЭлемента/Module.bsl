...
Процедура ПрочитатьДанныеТрудовойФункции()
	...
	ТекстПодсказки = НСтр("ru = 'Не выбрана трудовая функция';
							|en = 'Не выбрана трудовая функция'");
	Если ЗначениеЗаполнено(Объект.ТрудоваяФункция) Тогда
		...
		Если ЗначениеЗаполнено(РеквизитыТрудовойФункции.ОКПДТР2026Код) Тогда
			ТекстыПодсказки.Добавить(СтрШаблон(НСтр("ru = 'Код по ОКПДТР: %1';
													|en = 'Код по ОКПДТР: %1'"), РеквизитыТрудовойФункции.ОКПДТР2026Код));
			Если ЗначениеЗаполнено(РеквизитыТрудовойФункции.ОКПДТР2026Категория) Тогда
				ТекстыПодсказки.Добавить(СтрШаблон(НСтр("ru = 'Категория: %1';
														|en = 'Категория: %1'"), РеквизитыТрудовойФункции.ОКПДТР2026Категория));
			КонецЕсли;
		КонецЕсли;
		Если ЗначениеЗаполнено(ТекстыПодсказки) Тогда
			ТекстПодсказки = СтрШаблон(
				НСтр("ru = 'С 2026 действуют сведения ОКПДТР: %1';
					|en = 'С 2026 действуют сведения ОКПДТР: %1'"), СтрСоединить(ТекстыПодсказки, ", "));
		Иначе
			ТекстПодсказки = НСтр("ru = 'Не выбран код классификатора ОКПДТР 2026 года';
									|en = 'Не выбран код классификатора ОКПДТР 2026 года'");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(РеквизитыТрудовойФункции.КодПоОКЗ2026) Тогда
			ТекстПодсказки = СтрШаблон(НСтр("ru = '%1, Код ОКЗ: %2';
											|en = '%1, Код ОКЗ: %2'"), ТекстПодсказки, РеквизитыТрудовойФункции.КодПоОКЗ2026);
		КонецЕсли;
