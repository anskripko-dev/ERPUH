...
Процедура ЗагрузитьТаблицей(Команда)
	...
	ДопПараметры = Новый Структура("ИмяОбъекта, ИмяТЧ, ИсточникКолонок",
		"Справочник.НастройкиРаспределенияЗатратМСФО",
		"БазаРаспределенияПриемника",
		"ТЧ");
	
	ОписаниеКоманды = Новый Структура("ДополнительныеПараметры", ДопПараметры);
	ПараметрыВыполненияКоманды = Новый Структура(
		"ОписаниеКоманды, Форма", ОписаниеКоманды, ЭтотОбъект);
	
	МСФОКлиентУХ.ЗагрузкаДанных_Подключаемый(
...
Процедура УправлениеФормой(Форма)
	...
	ЭтоПСРСБУ = Форма.Объект.ПланСчетовБД = Форма.КэшируемыеЗначения.ПСРСБУ;
	
	ГруппаПриемникПростыеОтборы = Ложь;
	ГруппаТаблица = Ложь;
	ГруппаЗапрос = Ложь;
	
	ВидимостьАнализБазы = Неопределено;
	...
	Если Способ = ПредопределенноеЗначение("Перечисление.СпособыЗакрытияСчетовЗатрат.ПропорциональноОборотуНсбуДтКт") Тогда
		...
		ВидимостьАнализБазы = Истина;
		
		Если ЭтоПСРСБУ Тогда
			
			Подсказка.Добавить(НСтр(
			"ru = 'При закрытии будут сделаны проводки закрытия с теми же субконто, которые были в проводках учета. 
			|При этом, будут проигнорированы субконто проводок, которые не соответствуют установленным отборам по счету Дт и 
			|счету Кт. Сумма проводок закрытия рассчитывается как сумма поступлений на закрываемый счет.'"));
		...
		Иначе
			...
			|При этом, будут проигнорированы субконто проводок, которые не соответствуют установленным отборам по счету Дт и 
			|счету Кт. Сумма проводок по МСФО рассчитывается как сумма поступлений на закрываемый счет.'"));
	...
	Если ВидимостьАнализБазы <> Неопределено Тогда
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы,
			"ФормаСправочникНастройкиРаспределенияЗатратМСФОАнализСчетаБазыМСФО",
			"Видимость",
			ВидимостьАнализБазы);
	...
	Элементы.CписыватьЗатратыНСБУ.Видимость 	= Не ЭтоПСРСБУ;
	Элементы.СпособЗакрытия.РежимВыбораИзСписка = ЭтоПСРСБУ;//представления способов для РСБУ
	...
	Если ЭтоПСРСБУ Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(
			Форма.КэшируемыеЗначения.СпособыРСБУ,
...
Процедура ПодготовитьФормуНаСервере(ТекущийОбъект = Неопределено)
	...
	КэшируемыеЗначения.Вставить("ПСРСБУ", 		Справочники.ПланыСчетовБД.ПолучитьПланСчетовБД("Хозрасчетный"));
...
Процедура ЗаполнитьПоПлануСчетов()
	...
	Если Объект.ПланСчетовБД.Пустая() Тогда
		Объект.ПланСчетовБД = Константы.ПланСчетовПоУмолчанию.Получить();
	КонецЕсли;
	...
	РазделПС.Добавить(ПредопределенноеЗначение("Справочник.РазделыПланаСчетов.НезавершенноеСтроительство"));
...
Процедура ОбновитьСубконтоСчетаСтроки(Форма, СтрокаТаблицы)
	...
	ТекущаяСтрока = Форма.Объект.БазаРаспределенияПриемника
		.НайтиПоИдентификатору(Форма.Элементы.БазаРаспределенияПриемника.ТекущаяСтрока);
	
	МСФОКлиентСерверУХ.ОбновитьСубконтоСчета(
		Форма, "Счет", ТекущаяСтрока, "БазаРаспределенияПриемника", Ложь);
...
Функция ТекстЗапроса_БазаРаспределенияПриемника(СчетИсточник)
	...
	РБ = Справочники.РегистрыБухгалтерииБД.ПолучитьПоПлануСчетовБД(СчетИсточник.Владелец);
	Возврат Документы.РасчетСебестоимости.ТекстЗапроса_БазаРаспределенияПриемника(РБ);
