...
Процедура УстановитьВидимость(ВидимыеЭлементы)
	...
	ВсеЭлементы.Добавить("ИсточникШаблон");
	ВсеЭлементы.Добавить("ГруппаШаблон");
	ВсеЭлементы.Добавить("ГруппаРесурсыШ"); 
	ВсеЭлементы.Добавить("ДекорацияЗависимыеИсточники");
	
	ВсеЭлементы.Добавить("ПересчитатьВОсновнуюВалютуРБ");
	ВсеЭлементы.Добавить("ПересчитатьВОсновнуюВалютуРН");
	ВсеЭлементы.Добавить("ПересчитатьВОсновнуюВалютуРС");
	ВсеЭлементы.Добавить("ПересчитатьВОсновнуюВалютуADO");
	ВсеЭлементы.Добавить("ПересчитатьВОсновнуюВалютуЗапрос");
	
	ВсеЭлементы.Добавить("РесурсРегистраСведений");
	ВсеЭлементы.Добавить("РесурсРегистраБД");
	ВсеЭлементы.Добавить("РесурсРегистра");
...
Процедура УстановитьВидимостьДляОбразцовогоИсточника(ВидимыеЭлементы)
	...
	ПроверитьВозможностьИзмененияИсходныхДанных(ВидимыеЭлементы);
		
КонецПроцедуры // УстановитьВидимостьДляОбразцовогоИсточника()

&НаСервере
Процедура ПроверитьВозможностьИзмененияИсходныхДанных(ВидимыеЭлементы)
	...
	Запрос.Текст="ВЫБРАТЬ
	             |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ИсточникиДанныхДляРасчетов.Ссылка) КАК КоличествоИсточников
	             |ИЗ
	...
	Запрос.УстановитьПараметр("ИсточникШаблон",Объект.Ссылка);
	
	Результат=Запрос.Выполнить().Выбрать();
	
	Если Результат.Следующий() 
		И Результат.КоличествоИсточников>0 Тогда
		
		ВидимыеЭлементы.Вставить("ДекорацияЗависимыеИсточники", Истина);

		Элементы.ДекорацияЗависимыеИсточники.Заголовок=СтрШаблон(Нстр("ru = 'Шаблон используется в %1 источниках данных.'"),Результат.КоличествоИсточников);
		
		Элементы.ГруппаСпособПолучения.ТолькоПросмотр=Истина;
		Элементы.РедактироватьТекстЗапроса.Доступность=Ложь;
		Элементы.КонструкторЗапроса.Доступность=Ложь;	
		
	Иначе
		
		ВидимыеЭлементы.Вставить("ДекорацияЗависимыеИсточники", Ложь);	
		
	КонецЕсли;
		
КонецПроцедуры // ПроверитьВозможностьИзмененияИсходныхДанных() 
...
Процедура УстановитьВидимостьПоСпособуИспользования(ВидимыеЭлементы)
	...
	Иначе
		...
		Если Объект.СпособИспользования = Перечисления.СпособыИспользованияОперандов.ДляКонтрольныхСоотношений Тогда
			...
			Элементы.ПотребительРасчета.Заголовок = НСтр("ru = 'Соотношение'");
		...
		ИначеЕсли Объект.СпособИспользования = Перечисления.СпособыИспользованияОперандов.ДляЗаполненияОбъектовБД Тогда
			...
			Если СтрокаТипПотребительРасчета="СправочникиБД" Тогда
				...
				Элементы.ПотребительРасчета.Заголовок = НСтр("ru = 'Справочник'");
				
			ИначеЕсли СтрокаТипПотребительРасчета="ДокументыБД" Тогда
				
				Элементы.ПотребительРасчета.Заголовок = НСтр("ru = 'Документ'");
				
			ИначеЕсли СтрокаТипПотребительРасчета="ТабличныеЧастиБД" Тогда
				
				Элементы.ПотребительРасчета.Заголовок = НСтр("ru = 'Табличная часть'");
...
Процедура ВидимостьДоступность()
	...
	ВидимыеЭлементы.Вставить("ТаблицаСоответствияПолеБД", Ложь);
	...
	ВидимыеЭлементы.Вставить("КоманднаяПанельОтборы", Истина); 
	ВидимыеЭлементы.Вставить("ГруппаКод",	Истина);
	
	ВидимыеЭлементы.Вставить("ГруппаШаблон",ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом());
	
	Элементы.КоманднаяПанельОтборы.Доступность=Истина;
	...
	ИначеЕсли ТипЗнч(Объект.ПотребительРасчета)=Тип("СправочникСсылка.СоответствияСчетовДляТрансляции")
		ИЛИ ЗначениеЗаполнено(СтрокаПриемник) Тогда
		
		ВидимыеЭлементы.Вставить("ГруппаНазначение", Ложь);
		ВидимыеЭлементы.Вставить("ГруппаСпособПолучения", Ложь);
		Элементы.ГруппаНаименование.Видимость=Ложь;
		ВидимыеЭлементы.Вставить("ГруппаШаблон", Ложь);
		ВидимыеЭлементы.Вставить("ИсточникШаблон", Ложь);
		...
		ИначеЕсли ЗначениеЗаполнено(СтрокаПриемник) Тогда
			
			ВидимыеЭлементы.Вставить("ГруппаЗаполнениеБюджетов",Истина);
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.ОбъектУчета) Тогда
		...
		ВидимыеЭлементы.Вставить("ГруппаЗаполнениеБюджетов",Ложь);
	...
	Иначе
		...
		ВидимыеЭлементы.Вставить("ИсточникШаблон",ЗначениеЗаполнено(Объект.ИсточникШаблон));
	...
	ИначеЕсли Объект.СпособПолучения = Перечисления.СпособыПолученияОперандов.ВнешниеДанныеРегистрНакопления
		...
		ВидимыеЭлементы.Вставить("ГруппаРесурсыШ", Истина);
	...
	ИначеЕсли Объект.СпособПолучения = Перечисления.СпособыПолученияОперандов.ВнешниеДанныеПроизвольныйЗапрос
		...
		ВидимыеЭлементы.Вставить("ТипЗначения", Истина);
		Элементы.КоманднаяПанельОтборы.Доступность=Ложь;
		
		Элементы.ПанельПравила.ОтображениеСтраниц=ОтображениеСтраницФормы.ЗакладкиСверху;
	...
	Если ОбщегоНазначенияПовтИспУХ.ИспользуетсяМногопериоднаяОтчетность()
	...
	ТолькоПросмотр = (Не Справочники.ИсточникиДанныхДляРасчетов.РедактированиеНазначенияРасчетов(Объект.НазначениеРасчетов))
			ИЛИ РедактированиеЗапрещено;
				
	Если (НЕ Объект.СпособИспользования=Перечисления.СпособыИспользованияОперандов.ПроизвольныеДанные)
		И (Объект.ПотребительРасчета = Неопределено ИЛИ СтрокаТипПотребительРасчета="ШаблоныУниверсальныхПроцессов") Тогда
		ВидимыеЭлементы.Вставить("ГруппаПоказатель", Истина);
	...
	Элементы.ВидИтогаБД.Видимость=Элементы.РесурсРегистраБД.Видимость И Элементы.ВидИтогаБД.СписокВыбора.Количество()>0;
	Элементы.ТаблицаРесурсовРесурсПриемник.Видимость=ЗначениеЗаполнено(Объект.ОбъектУчета);
	Элементы.ВидИтогаШ.Видимость=(Не Элементы.ТаблицаРесурсовРесурсПриемник.Видимость);
		
	ВидимыеЭлементы.Вставить("ПересчитатьВОсновнуюВалютуРБ", ВозможенПересчетВВалютуОтчета);
	ВидимыеЭлементы.Вставить("ПересчитатьВОсновнуюВалютуРН", ВозможенПересчетВВалютуОтчета);
	ВидимыеЭлементы.Вставить("ПересчитатьВОсновнуюВалютуРС", ВозможенПересчетВВалютуОтчета);
	ВидимыеЭлементы.Вставить("ПересчитатьВОсновнуюВалютуADO", ВозможенПересчетВВалютуОтчета); 
	ВидимыеЭлементы.Вставить("ПересчитатьВОсновнуюВалютуЗапрос", ВозможенПересчетВВалютуОтчета);

	ОтображатьРесурсы=(НЕ Объект.СпособИспользования=Перечисления.СпособыИспользованияОперандов.ДляЗаполненияОбъектовБД);
	ВидимыеЭлементы.Вставить("РесурсРегистра", ОтображатьРесурсы);
	ВидимыеЭлементы.Вставить("РесурсРегистраБД", ОтображатьРесурсы);
	ВидимыеЭлементы.Вставить("РесурсРегистраСведений", ОтображатьРесурсы);
	
	ВидимыеЭлементы.Вставить("ГруппаКод", ВидимыеЭлементы["ГруппаКод"] И ОтображатьРесурсы);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ИсточникШаблон=Справочники.ИсточникиДанныхДляРасчетов.ВводОстатков Тогда
		
		ТекстПредупреждения= НСтр("ru = 'Дополнительная настройка не предусмотрена.'");
		Возврат;
	...
	Иначе
		...
		СоответствиеСчетов=(ЗначениеЗаполнено(Объект.СоответствиеСчетов)
			ИЛИ ТипЗнч(Объект.ПотребительРасчета)=Тип("СправочникСсылка.СоответствияСчетовДляТрансляции"));
				
	КонецЕсли;
	
	Если СоответствиеСчетов Тогда
		
		ДанныеСоответствия=ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.СоответствиеСчетов,"СчетИсточник,КоррСчетИсточник,СчетПриемник");
		...
		Если Объект.НазначениеРасчетов.НаправлениеТрансляции=Перечисления.НаправленияТрансляцииДанных.ФинансовыеРегистрыВРегистрБухгалтерии Тогда
			...
			ЗаполнитьДанныеПоСоответствиюРегистры();
			
		ИначеЕсли ТипЗнч(ДанныеСоответствия.СчетПриемник)=Тип("СправочникСсылка.СтрокиОтчетов") Тогда
			
			СтрокаПриемник=ДанныеСоответствия.СчетПриемник;
	...
	ВозможенПересчетВВалютуОтчета=Справочники.ВидыОтчетов.ЕстьВалютнаяСумма(Объект.ПотребительРасчета);

	Если Параметры.Свойство("РедактированиеЗапрещено") Тогда
		
		РедактированиеЗапрещено=Параметры.РедактированиеЗапрещено;
	...
	Если ЗначениеЗаполнено(Объект.ТипБДПриемник) Тогда
		...
		ИспользуютсяПланыОбмена=Объект.ТипБДПриемник.ИспользуютсяПланыОбмена;
		
	Иначе
		
		ИспользуютсяПланыОбмена=Объект.ТипБД.ИспользуютсяПланыОбмена; 
		
	КонецЕсли;
		
КонецПроцедуры  

Процедура ЗаполнитьДанныеПоСоответствиюРегистры()
...
КонецПроцедуры // ЗаполнитьДанныеПоСоответствиюРегистры()
...
&НаСервере
Процедура ПоместитьРабочийОбъект(РабочийОбъект,СохранятьНастройкиПолей=Ложь,СохранятьНастройкиОтборов=Ложь)
	...
	Если СохранятьНастройкиПолей ИЛИ СохранятьНастройкиОтборов Тогда
		
		Если НЕ СохранятьНастройкиПолей Тогда
		...
		Если НЕ СохранятьНастройкиОтборов Тогда
...
&НаСервере
Функция ПроверитьЗначенияДляПроверки() 
	...
	Для Каждого Строка ИЗ Объект.ТаблицаПараметровОтбораБД Цикл
		...
		Если ТребуетсяЗначениеДляПроверки(Строка.СпособВычисленияПараметра)
			И (НЕ ЗначениеЗаполнено(Строка.ЗначениеДляПроверки)) Тогда
			
			ТекстОшибки=ТекстОшибки+"
			|"+Строка.НаименованиеБД;
	...
	Если Не ПустаяСтрока(ТекстОшибки) Тогда
		...
		Возврат НСтр("ru = 'Не указаны значения для проверки в строках'")+ТекстОшибки;
...
КонецФункции // ПроверитьЗначенияДляПроверки() 

&НаСервере
Функция ТребуетсяЗначениеДляПроверки(СпособВычисленияПараметра)

	Возврат НЕ (СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.Меньше
	ИЛИ СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.Больше
	ИЛИ СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.БольшеИлиРавно
	ИЛИ СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.Булево
	...
	ИЛИ СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.Дата
	ИЛИ СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.МеньшеИлиРавно
	...
	ИЛИ СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.СохраненнаяНастройкаОтбора
	...
	ИЛИ СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.СписокФиксированныхЗначений
	ИЛИ СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.Строка
	...
	ИЛИ СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ФункцияНаВстроенномЯзыкеСписокЗначений
	ИЛИ СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.Число);
	

	
	
	
КонецФункции // ТребуетсяЗначениеДляПроверки()
...
Функция ВыполнитьПроверкуОперанда()
	...
	Попытка
		...
		ТекстОшибки=ПроверитьЗначенияДляПроверки();
		...
		Если Объект.ТрансляцияДебет ИЛИ Объект.ТрансляцияКредит Тогда 
						
			ДанныеКонтрИсточника=УправлениеОтчетамиУХ.ДанныеИсточникаДляСтороныПроводки(Объект.Ссылка);
			
			Если ДанныеКонтрИсточника.Количество()=0 Тогда
				
				Возврат ТабличныйДокумент;
				
			КонецЕсли;
			
			УправлениеОтчетамиУХ.ОбновитьНастройкиДляСтороныПроводки(Объект.Ссылка);
			
			РезультатЗапроса = УправлениеОтчетамиУХ.ПолучитьТаблицуДанныхПоИсточнику(Неопределено,ДанныеКонтрИсточника[0].Ссылка,Истина); 
...
Процедура ПоместитьДанныеОбъектаВРеквизитыФормы(РабочийОбъект)
	...
	Если РабочийОбъект.СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеРегистрБухгалтерии
		...
		ОбщегоНазначенияУХ.ЗагрузитьВСписокЗначений(РабочийОбъект.СписокВидИтога,Элементы.ВидИтогаШ.СписокВыбора);
...
Процедура ПриИзмененииВидаИтога()
	...
	РабочееДеревоПолей=РеквизитФормыВЗначение("ДеревоПолейБД");
	...
	ЕстьПолеКоррСчет=РабочееДеревоПолей.Строки.НайтиСтроки(Новый Структура("ПолеБД","КоррСчетОборот"),Истина).Количество()>0;
	
	Если Объект.ВидИтога = "Оборот#" Тогда
		
		Если НЕ ЕстьПолеКоррСчет Тогда
			
			ПриИзмененииСчета(Истина);
			
		КонецЕсли;
		
	ИначеЕсли ЕстьПолеКоррСчет Тогда
...
Процедура ПриИзмененииСпособаПолучения()
	...
	Если Объект.СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеПоказательОтчета Тогда
		...
		Объект.ПересчитатьВОсновнуюВалюту=Ложь;
...
Процедура СформироватьСписокВозможныхСпособовПолучения()
	...
	Если НЕ (Объект.СпособИспользования=Перечисления.СпособыИспользованияОперандов.ПроизвольныеДанные
		ИЛИ Объект.СпособИспользования=Перечисления.СпособыИспользованияОперандов.ДляЗаполненияОбъектовБД) Тогда
	...
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом() Тогда
		...
		Если НЕ ДляФормированияКорректировок Тогда
...
Процедура СформироватьСписокВозможныхСпособовЗаполнения(СпособИспользования)
	...
	Если (СпособИспользования=Перечисления.СпособыИспользованияОперандов.ДляФормулРасчета
		ИЛИ СпособИспользования=Перечисления.СпособыИспользованияОперандов.ДляЗаполненияОбъектовБД) И 
...
Процедура ОбработатьИзменениеТекстаПроизвольногоЗапроса(СохранятьНастройки)
	...
	РабочийОбъект = ПолучитьРабочийОбъект();
	ТаблицаПолейСтар=РабочийОбъект.мТаблицаПоказателейБД.Скопировать(Новый Структура("Измерение", Истина)); 
	
	СохранятьНастройкиПолей=СохранятьНастройки;
	СохранятьНастройкиОтборов=СохранятьНастройки;
	...
	Если РабочийОбъект.ОбработатьИзменениеТекстаЗапроса(ТекстЗапросаФорма) Тогда
		...
		Если СохранятьНастройки Тогда
			...
			ТаблицаПолейНов=РабочийОбъект.мТаблицаПоказателейБД.Скопировать(Новый Структура("Измерение", Истина));

			Если НЕ ОбщегоНазначения.КоллекцииИдентичны(ТаблицаПолейСтар,ТаблицаПолейНов,"НаименованиеБД,ТипЗначения") Тогда
				СохранятьНастройкиПолей = Ложь;
			КонецЕсли;
			
			Если НЕ ОбщегоНазначения.КоллекцииИдентичны(РеквизитФормыВЗначение("ДеревоПараметровОтбораБД").Строки,РабочийОбъект.ДеревоПараметровОтбораБД.Строки,"НаименованиеБД,ТипЗначения") Тогда
				СохранятьНастройкиОтборов = Ложь;
			КонецЕсли;
		...
		ПоместитьРабочийОбъект(РабочийОбъект, СохранятьНастройкиПолей,СохранятьНастройкиОтборов);
...
Функция ПолучитьНастройкуСоответствия(ТекущаяСтрокаАналитики)
	...
	Если ЗначениеЗаполнено(ТекущаяСтрокаАналитики.ВидАналитики) Тогда
		
		ТаблицаАналитики=Метаданные.НайтиПоТипу(ТекущаяСтрокаАналитики.ВидАналитики.ТипЗначения.Типы()[0]).ПолноеИмя();
		ДанныеАналитики=СтрРазделить(ТаблицаАналитики,".");
		
		ТипОбъектаКонсолидации=ДанныеАналитики[0];
		ИмяОбъектаМетаДанных=ДанныеАналитики[1];
	...
	Если ТекНастройкаСоответствия=Неопределено Тогда
		...
		ОписаниеОбъектаВИБ=ОбщегоНазначенияУХ.ПолучитьОписаниеОбъектаБД(ТекСоединениеВИБ,ТипМетаДанных,СправочникБД,ТекущаяСтрокаАналитики.ТипБД);
			
		СтруктураСоответствие=Новый Структура;
		СтруктураСоответствие.Вставить("ОписаниеОбъектаВИБ",ОписаниеОбъектаВИБ);
		...
		Если ИмяобъектаМетаданных="ПроизвольныйКлассификаторУХ" Тогда
			...
			СтруктураСоответствие.Вставить("ОписаниеОбъектаТек",ТекущаяСтрокаАналитики.ВидАналитики);
...
Процедура КоманднаяПанельСоответствияКнопкаСопоставитьАналитики(Команда)
	...
	СоответствиеСоздать(СформироватьСтруктуруСтрокиБД(ДанныеИсточника,Истина), ТекущаяСтрокаАналитики);
				
КонецПроцедуры

&НаКлиенте
Функция СформироватьСтруктуруСтрокиБД(ДанныеСтрокиАналитики,ЗаменятьПредставлением=Ложь)
	...
	Если Объект.СпособПолучения = ПредопределенноеЗначение("Перечисление.СпособыПолученияОперандов.ВнешниеДанныеADO") Тогда
		...
		СтрокаБД.Вставить("ПредставлениеПоля",			);
		
	Иначе
		
		Если Не ЗаменятьПредставлением Тогда
		...
		Иначе
			...
			СтрокаБД.Вставить("НаименованиеБД",				ДанныеСтрокиАналитики.ПредставлениеПоля);
			СтрокаБД.Вставить("ПредставлениеПоля",			ДанныеСтрокиАналитики.ПредставлениеПоля);
...
Функция СформироватьДанныеОбъекта()
	...
	ДанныеОбъекта.Вставить("ТипБД",						?(ЗначениеЗаполнено(Объект.ТипБД),Объект.ТипБД,ПредопределенноеЗначение("Справочник.ТипыБазДанных.ТекущаяИБ")));
	...
	ИсточникТрансляции=(Объект.ТрансляцияДебет 
		ИЛИ Объект.ТрансляцияКредит
		ИЛИ ЗначениеЗаполнено(Объект.СоответствиеСчетов));
	
	Если ИсточникТрансляции Тогда	
		
		ДанныеОбъекта.Вставить("ИсточникТрансляции",Истина);
...
Процедура ДеревоПолейИПараметровОтбораБДНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка)
	...
	Иначе
		...
		ПараметрыПеретаскивания.Значение=СформироватьСтруктуруСтрокиБД(ДанныеПеретаскивания,Истина);
...
Процедура ТаблицаСоответствияПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если Элемент.ТекущиеДанные=Неопределено ИЛИ РедактированиеЗапрещено Тогда
...
&НаКлиенте
Процедура УстановитьПолеСоответствия(СтрокаСоответствия,СтрокаВИБ) 
	
	РабочаяСтрока=СтрокаСоответствия;	
	
	ЗаполнитьЗначенияСвойств(РабочаяСтрока,СтрокаВИБ,,"ВидАналитики");
	
	РабочаяСтрока.ПсевдонимБД = ?(ЗначениеЗаполнено(СтрокаВИБ.ПредставлениеПоля),СтрокаВИБ.ПредставлениеПоля,СтрокаВИБ.НаименованиеБД);
...
Процедура ОбновитьНастройкуСоответствияВИБ(СтрокаСоответствия,СтрокаВИБ)
	...
	Если СтрокаВИБ.ТипЗначения.Количество()>1 Тогда
		...
		Если СтрКонсолидации.ТипЗначения.Количество()>1 Тогда
			
			УстановитьПолеСоответствия(СтрокаСоответствия,СтрокаВИБ);
			Возврат;
	...
	Если ИсточникДляОтчетов Тогда
		РабочаяСтрока.ПсевдонимБД=ОбщегоНазначенияУХ.ВернутьАлфавитноЦифровоеПредставление(СтрокаВИБ.ПолеБД);
	ИначеЕсли ТаблицаADO И РабочаяСтрока.АналитикаКонсолидации="Значение" Тогда
		РабочаяСтрока.ПсевдонимБД=СтрокаВИБ.ПолеБД;
	Иначе
		РабочаяСтрока.ПсевдонимБД = ?(ЗначениеЗаполнено(СтрокаВИБ.ПредставлениеПоля),СтрокаВИБ.ПредставлениеПоля,СтрокаВИБ.НаименованиеБД);
	КонецЕсли;
	...
	Если (НЕ (РабочаяСтрока.АналитикаКонсолидации="Значение" ИЛИ Объект.ТолькоПримитивныеТипы))
		...
		Если ТекНастройкаСоответствия=Неопределено Тогда
			...
			Если НЕ (СтрокаВИБ.ТипМетаДанных="Простой" Или ИспользуютсяПланыОбмена) Тогда
		...
		ИначеЕсли ТипЗнч(ТекНастройкаСоответствия)=Тип("Структура") И (Не ИспользуютсяПланыОбмена) Тогда // Возвращены данные для создания нового объекта
...
Процедура ОбновитьНастройкуСоответствияТек(СтрокаСоответствия,СтрокаИсточника)
	...
	Иначе
		...
		РабочаяСтрока.ПсевдонимБД=?(ЗначениеЗаполнено(СтрокаИсточника.ПредставлениеПоля),СтрокаИсточника.ПредставлениеПоля,СтрокаИсточника.НаименованиеБД);
...
Процедура УстановитьОграничениеТипа(ДанныеДляОграничения)
	...
	Если ДанныеДляОграничения.Свойство("ТипЗначения") Тогда
		...
		Если ДанныеДляОграничения.ПолеБД="КоррСчетОборот" Тогда
			
			НовыйПараметр = Новый ПараметрВыбора("Отбор.Владелец", ПланСчетов);
			НовыйМассив = Новый Массив();
			НовыйМассив.Добавить(НовыйПараметр);				
			НовыеПараметры = Новый ФиксированныйМассив(НовыйМассив);
			ЭлементУправления.ПараметрыВыбора=НовыеПараметры;
...
Процедура ТаблицаADOНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ ПроверитьНаличиеТаблицADO(Объект.ТипБД) Тогда
		...
		ШаблонСообщения = НСтр("ru = 'Нет данных по таблицам из внешнего подключения %1'");
...
Функция ПодготовитьДанныеОбъекта()
	...
	ДанныеОбъекта.Вставить("ЕстьНестандартныеОтборы",	Объект.ЕстьНестандартныеОтборы);
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ДанныеОбъекта.ТрансляцияДебет 
		ИЛИ ДанныеОбъекта.ТрансляцияКредит
		ИЛИ ЗначениеЗаполнено(Объект.СоответствиеСчетов) Тогда 
...
Процедура СоответствиеСоздать(СтрокаИсточника, СтрокаПриемника)
	...
	ИначеЕсли ВнутренниеДанные 
		ИЛИ СтрокаИсточника.Свойство("КонтекстОтчета")
		 ИЛИ СтрокаИсточника.ПолеБД="КоррСчетОборот" Тогда
...
Процедура ТаблицаСоответствияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя="ТаблицаСоответствияПолеБД" ИЛИ Поле.Имя="ТаблицаСоответствияПсевдонимБД" Тогда
		...
		Если Элементы.ТаблицаСоответствия.ТекущиеДанные.СпособЗаполнения=ПредопределенноеЗначение("Перечисление.СпособыЗаполненияПолейИсточника.КонтекстОбъектаЗагрузки") Тогда
			...
			ОткрытьФорму("ОбщаяФорма.ФормаВыбораПоляОбъекта",Новый Структура("ВидОтчета,ПотребительРасчета,СпособИспользования",ВидОтчета,Объект.ПотребительРасчета,Объект.СпособИспользования));
		...
		ИначеЕсли Элементы.ТаблицаСоответствия.ТекущиеДанные.СпособЗаполнения=ПредопределенноеЗначение("Перечисление.СпособыЗаполненияПолейИсточника.ФиксированноеЗначение") Тогда
			...
			ИначеЕсли ЗначениеЗаполнено(ДанныеАналитики) И ЗначениеЗаполнено(СтрокаДанныхПоля.ВидАналитики) Тогда
				...
				ИначеЕсли ТекВидАналитики.Свойство("Владелец") Тогда
					...
					СтрокаВладелец=ОбщегоНазначенияКлиентСерверУХ.НайтиЭлементКоллекцииПоЗначениюРеквизита(ТаблицаСоответствияБД,"АналитикаКонсолидации",ТекВидАналитики.Владелец);
			...
			Иначе  
				...
				Если СтрокаДанныхПоля.ТипДанныхПоля.Количество()>0 Тогда
					...
					ОписаниеТипов=ПолучитьОписаниеТиповФиксированногоЗначения(СтрокаДанныхПоля);
...
&НаСервере
Функция ПолучитьОписаниеТиповФиксированногоЗначения(СтрокаДанныхПоля)
	
	Если ТипЗнч(СтрокаДанныхПоля.ТипДанныхПоля[0].Значение)=Тип("СправочникСсылка.СоответствиеВнешнимИБ") Тогда 
		...
		|ГДЕ
		|	СоответствиеВнешнимИБ.Ссылка В(&МассивСсылок)"; 
		
		Запрос.УстановитьПараметр("МассивСсылок",СтрокаДанныхПоля.ТипДанныхПоля.ВыгрузитьЗначения());
		...
		Пока Результат.Следующий() Цикл
			...
			ДанныеТипа=СтрРазделить(Результат.ТаблицаАналитикиТек,".");
			ДанныеТипа[0]=ДанныеТипа[0]+"Ссылка";
			МассивТипов.Добавить(Тип(СтрСоединить(ДанныеТипа,".")));
	...
	Иначе
		...
		Возврат Новый ОписаниеТипов(СтрокаДанныхПоля.ТипДанныхПоля.ВыгрузитьЗначения());
		
	КонецЕсли;	
	
КонецФункции // ПолучитьОписаниеТиповФиксированногоЗначения() 
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(ТекущийОбъект.НазначениеРасчетов) 
		И ТипЗнч(ТекущийОбъект.НазначениеРасчетов.Владелец)=Тип("СправочникСсылка.ШаблоныТрансляций") Тогда
		
		Обработки.КорректировкиЗначенийПоказателей.УстановитьПризнакИзмененияШаблона(ТекущийОбъект.НазначениеРасчетов.Владелец);
	...
	Если СтрПоле.ТипЗначения.Количество()>0 И СоответствиеГруппыУсловия.Получить(СтрПоле.СпособВычисленияПараметра)=Неопределено Тогда
		...
		Если ДанныеДляОграничения.Количество()>0 Тогда
			...
			Для Каждого СтрОграничение ИЗ ДанныеДляОграничения Цикл
				...
				ТекЗначениеПоля=СтрПоле[СтрОграничение.ИмяКолонки];
				СтрОграничение.Вставить("ПолеБД",Элементы.ДеревоПараметровОтбораБД.ТекущиеДанные.ПолеБД);
				
				Если ЗначениеЗаполнено(ТекЗначениеПоля) Тогда
					
					Если СтрОграничение.ТипЗначения=Тип("СписокЗначений") И (НЕ ТипЗнч(ТекЗначениеПоля)=Тип("СписокЗначений")) Тогда
						
						ЗначениеСписок=Новый СписокЗначений;
						ЗначениеСписок.Добавить(ТекЗначениеПоля);
						
						ТекЗначениеПоля=ЗначениеСписок;
						УстановитьОграничениеТипа(СтрОграничение);
						
					ИначеЕсли ТипЗнч(ТекЗначениеПоля)=Тип("СписокЗначений") И (НЕ СтрОграничение.ТипЗначения=Тип("СписокЗначений")) Тогда
						
						Если ТекЗначениеПоля.Количество()>0 Тогда
							
							ЗначениеПоля=ТекЗначениеПоля[0].Значение;
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(Элементы.ДеревоПараметровОтбораБД.ТекущиеДанные.УточнениеСпособаОпределения) Тогда
			...
			Элементы.ДеревоПараметровОтбораБД.ПодчиненныеЭлементы.УточнениеСпособаОпределения.ПараметрыВыбора	= Новый ФиксированныйМассив(Новый Массив);
			Элементы.ДеревоПараметровОтбораБД.ПодчиненныеЭлементы.ЗначениеДляПроверки.ПараметрыВыбора			= Новый ФиксированныйМассив(Новый Массив);
...
Процедура ОбновитьНаименованиеСервер()
	...
	РабочийОбъект=ПолучитьРабочийОбъект();
	РабочийОбъект.СформироватьКодОперанда(Истина,Истина);
	РабочийОбъект.СформироватьКодУпрощеннойФормулы();
	КодУпрощеннойФормулы=РабочийОбъект.КодУпрощеннойФормулы;
	ПоместитьРабочийОбъект(РабочийОбъект);
...
Процедура ПриОткрытии(Отказ)
	...
	Для Каждого Элемент ИЗ ДеревоПолейБД.ПолучитьЭлементы() Цикл
		...
		Если ПустаяСтрока(Элемент.ПолеБД) Тогда
...
Процедура СоответствиеРесурсовДляТрансляцииПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
 	...
 	Если НоваяСтрока Тогда
		...
		Элементы.СоответствиеРесурсовДляТрансляции.ТекущиеДанные.ВидИтога=Элементы.ВидИтогаШ.СписокВыбора[0].Значение;
...
&НаКлиенте
Процедура ЗаполнитьРесурсыШаблона(Команда)
	
	СписокРесурсов=Элементы.ТаблицаРесурсовРесурсИсточник.СписокВыбора;
	СписокИтогов=Элементы.ВидИтогаШ.СписокВыбора; 
	
	Объект.СоответствиеРесурсовДляТрансляции.Очистить();
	
	Для Каждого ЭлементРесурс ИЗ СписокРесурсов Цикл
		
		Для Каждого ЭлементИтог ИЗ СписокИтогов Цикл
			
			НоваяСтрока=Объект.СоответствиеРесурсовДляТрансляции.Добавить();
			НоваяСтрока.РесурсИсточник=ЭлементРесурс.Значение;
			НоваяСтрока.ВидИтога=ЭлементИтог.Значение;
...
&НаКлиенте
Процедура ДекорацияЗависимыеИсточникиНажатие(Элемент)
	
	ОткрытьФорму("Справочник.ИсточникиДанныхДляРасчетов.Форма.ФормаСписка",Новый Структура("Отбор",Новый Структура("ИсточникШаблон",Объект.Ссылка)));
