...
Перем КэшВидовСубконто Экспорт;
Перем ВидыСубконтоДляПодбора Экспорт;

Перем ПолеСиноним;
Перем ДанныеДляФормул;
...
Процедура СформироватьКодОперанда(ОбновитьКод=Ложь,ОбновитьНаименование=Истина) Экспорт
	...
	Если ЗначениеЗаполнено(Наименование)
		И (СпособИспользования=Перечисления.СпособыИспользованияОперандов.ДляАналитическихОтчетов
		ИЛИ СпособИспользования=Перечисления.СпособыИспользованияОперандов.ПроизвольныеДанные) Тогда
	...
	Если СпособПолучения = Перечисления.СпособыПолученияОперандов.ВнешниеДанныеРегистрБухгалтерии
	 ...
	 Если НЕ ЗначениеЗаполнено(Счет) Тогда
		 ...
		 ТекНаименование=""+ПолучитьСинонимДляФормулРегистр()+" - "+ПотребительРасчета.Наименование;
		 ТекКод=ОбщегоНазначенияУХ.СформироватьКодНаОснованииНаименования(ТекНаименование,ДлинаКода);
		 
	 Иначе
		 
		 ТекНаименование = ?(ВнешниеДанные,ПолучитьСинонимВИБ(),"")+"РБ" + ?(СпособПолучения = Перечисления.СпособыПолученияОперандов.ВнешниеДанныеОстаткиИОборотыИзПланаСчетов, "77", "") + "_"+ПолучитьСинонимРесурса()+"_";
		 
		 ТекНаименование=СтрШаблон(Нстр("ru = '%1, счет %2'; en = '%1, account %2';tr = '%1, hesap %2'", ДанныеДляФормул.КодЯзыка), ТекНаименование, Счет);
		 ТекКод="Счет_"+Счет+"_";
		 
		 Если ВидИтога = "НачальныйОстаток#" ИЛИ ВидИтога = "СН#" Тогда
			 ТекНаименование=СтрШаблон(Нстр("ru = '%1 начальный остаток (%2)'; en = '%1 opening balance (%2)';tr = '%1 açılış bakiyesi (%2)'", ДанныеДляФормул.КодЯзыка), ТекНаименование, ДтКт);
			 ТекКод=ТекКод+"СН"+?(ДтКт="Дт","Д","К");
		 ИначеЕсли ВидИтога = "НачальныйРазвернутыйОстаток#" ИЛИ ВидИтога = "СН#РС" Тогда
			 ТекНаименование=СтрШаблон(Нстр("ru = '%1 развернутый начальный остаток (%2)'; en = '%1 detailed opening balance (%2)';tr = '%1 ayrıntılı açılış bakiyesi (%2)'", ДанныеДляФормул.КодЯзыка), ТекНаименование, 
			                 	ДтКт);
			 ТекКод=ТекКод+"СН"+?(ДтКт="Дт","Д","К")+"РС";
		 ИначеЕсли ВидИтога = "КонечныйОстаток#" ИЛИ ВидИтога = "СК#" Тогда
			 ТекНаименование=СтрШаблон(Нстр("ru = '%1 конечный остаток (%2)'; en = '%1 closing balance (%2)';tr = '%1 kapanış bakiyesi (%2)'", ДанныеДляФормул.КодЯзыка), ТекНаименование, ДтКт);
			 ТекКод=ТекКод+"СК"+?(ДтКт="Дт","Д","К");
		 ИначеЕсли ВидИтога = "КонечныйРазвернутыйОстаток#" ИЛИ ВидИтога = "СК#РС" Тогда
			 ТекНаименование=СтрШаблон(Нстр("ru = '%1 конечный развернутый остаток (%2)'; en = '%1 detailed closing balance (%2)';tr = '%1 ayrıntılı kapanış bakiyesi (%2)'", ДанныеДляФормул.КодЯзыка), ТекНаименование, 
			                 	ДтКт);
			 ТекКод=ТекКод+"СК"+?(ДтКт="Дт","Д","К")+"РС";
		 ИначеЕсли ВидИтога = "Оборот#" ИЛИ ВидИтога = "#О" Тогда
			 ТекНаименование=СтрШаблон(Нстр("ru = '%1 оборот (%2)'; en = '%1 turnover (%2)';tr = '%1 ciro (%2)'", ДанныеДляФормул.КодЯзыка), ТекНаименование, ДтКт);
			 ТекКод=ТекКод+?(ДтКт="Дт","Д","К")+"О";	
		 ИначеЕсли ВидИтога = "Оборот" ИЛИ ВидИтога = "КорДО" Тогда
			 ТекНаименование=СтрШаблон(Нстр("ru = '%1 корреспондирующий оборот , коррсчет %2'; en = '%1 corresponding turnover, correspondent account %2';tr = '%1 karşılık gelen ciro, karşılık gelen hesap %2'", ДанныеДляФормул.КодЯзыка), ТекНаименование, 
			                 	КоррСчет);
 ...
 ИначеЕсли СпособПолучения = Перечисления.СпособыПолученияОперандов.ВнешниеДанныеРегистрНакопления
		...
		ТекКод = ТекКод + ?(ВнешниеДанные,ПолучитьСинонимВИБ(),"")+ПолучитьСинонимРН();
		ТекКод = ТекКод + ОбщегоНазначенияУХ.СформироватьКодНаОснованииНаименования(ПолучитьСинонимДляФормулРегистр(),9)
				 + "_" + ОбщегоНазначенияУХ.СформироватьКодНаОснованииНаименования(ПолучитьСинонимРесурса(),9);
		ТекКод = ТекКод + "_" + Лев(ПолучитьСинонимВидаИтога(), 1);
		
		ТекНаименование = ?(ВнешниеДанные,ПолучитьСинонимВИБ(),"")+ПолучитьСинонимРН() + ПолучитьСинонимДляФормулРегистр() + "_" + ПолучитьСинонимРесурса() + "(" + ПолучитьСинонимВидаИтога() + ")";
	...
	ИначеЕсли СпособПолучения = Перечисления.СпособыПолученияОперандов.ВнешниеДанныеРегистрСведений
		...
		ТекКод = ТекКод + ?(ВнешниеДанные,ПолучитьСинонимВИБ(),"")+ПолучитьСинонимРС();
		ТекКод = ТекКод + ОбщегоНазначенияУХ.СформироватьКодНаОснованииНаименования(ПолучитьСинонимДляФормулРегистр(),9)
				 + "_" + ОбщегоНазначенияУХ.СформироватьКодНаОснованииНаименования(ПолучитьСинонимРесурса(),9);
		ТекКод = ТекКод + "_" + Лев(ПолучитьСинонимВидаИтога(), 1);
		
		ТекНаименование = ?(ВнешниеДанные,ПолучитьСинонимВИБ(),"")+ПолучитьСинонимРС() + ПолучитьСинонимДляФормулРегистр() + "_" + ПолучитьСинонимРесурса() + "(" + ВидСреза + ")";
						 
	ИначеЕсли СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеПроизвольныйЗапрос Тогда
		
		ТекНаименование=Нстр("ru = 'Произвольный запрос к ВИБ'; en = 'Arbitrary EIB query';tr = 'Serbest harici infobase sorgusu'", ДанныеДляФормул.КодЯзыка);
		ТекКод=ОбщегоНазначенияУХ.СформироватьКодНаОснованииНаименования(ТекНаименование,ДлинаКода);
	...
	ИначеЕсли СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеADO Тогда
		...
		Если СпособИспользования=Перечисления.СпособыИспользованияОперандов.ДляЗаполненияОбъектовБД Тогда
			...
			ТекНаименование=СтрШаблон(Нстр("ru = '%1'; en = '%1';tr = '%1'", "en"), ОбъектШапка);
			
			Если ЗначениеЗаполнено(ОбъектТЧ) Тогда
				
				ТекНаименование=ТекНаименование+", "+СтрШаблон(Нстр("ru = 'ТЧ %1'; en = 'TS %1';tr = 'Tablo bölümü %1'", ДанныеДляФормул.КодЯзыка), ОбъектТЧ);
		...
		Иначе
			...
			ТекНаименование=СтрШаблон(Нстр("ru = 'ADO: %1'; en = 'ADO: %1';tr = 'ADO: %1'", ДанныеДляФормул.КодЯзыка), ТаблицаADO.Наименование);
			
			Если СтрАналитика.Количество()>0 Тогда 
				
				ТекНаименование=СтрШаблон(Нстр("ru = '%1, поле %2%3%4'; en = '%1, field %2%3%4';tr = '%1, alan %2%3%4'", ДанныеДляФормул.КодЯзыка), ТекНаименование, 
	...
	ИначеЕсли СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеПроизвольныйЗапрос Тогда
		...
		ТекНаименование=Нстр("ru = 'Произвольный запрос к текущей ИБ'; en = 'Arbitrary query to current infobase';tr = 'Geçerli Infobase''e serbest sorgu'", ДанныеДляФормул.КодЯзыка);
		ТекКод=ОбщегоНазначенияУХ.СформироватьКодНаОснованииНаименования(ТекНаименование,ДлинаКода);
		
	ИначеЕсли СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеПараметрическаяНастройка Тогда
		
		ТекНаименование=Нстр("ru = 'Параметрический запрос к значениям показателей'; en = 'Parametric indicator value query ';tr = 'Gösterge değerlerine parametre sorgusu'",ДанныеДляФормул.КодЯзыка);
		ТекКод=ОбщегоНазначенияУХ.СформироватьКодНаОснованииНаименования(ТекНаименование,ДлинаКода);
	...
	ИначеЕсли СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеПоказательОтчета Тогда
		...
		Если ПоказательОтбор.Предназначение=Перечисления.ПредназначенияЭлементовСтруктурыОтчета.ОборотноСальдоваяВедомость
			...
			СтрокаШаблона = Нстр("ru = 'ОСВ_%1, счет %2 %3'; en = 'TB_%1, account %2 %3';tr = 'Mizan_%1, hesap %2 %3'");
			...
			СтрокаШаблона = Нстр("ru = 'ОСВ_%1_%2_%3'; en = 'TB_%1_%2_%3';tr = 'Mizan_%1_%2_%3'");
	...
	ИначеЕсли СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеСправочники Тогда
		...
		СтрокаШаблона = Нстр("ru = ', табличная часть %1'; en = ', table %1';tr = ', tablo bölümü %1'");
		...
		ТекНаименование=СтрШаблон(Нстр("ru = 'Справочник %1%2'; en = 'Catalog %1%2';tr = 'Katalog %1%2'", ДанныеДляФормул.КодЯзыка), ОбъектБД.Наименование, ?(ЗначениеЗаполнено(ТабличнаяЧастьБД), ТабличнаяЧастьБДНаименование,""));
		ТекКод=ОбщегоНазначенияУХ.СформироватьКодНаОснованииНаименования(ТекНаименование,ДлинаКода);
	...
	ИначеЕсли СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеДокументы Тогда
		...
		ТекНаименование=СтрШаблон(Нстр("ru = 'Документ %1%2'; en = 'Document %1%2';tr = 'Belge %1%2'", ДанныеДляФормул.КодЯзыка), ОбъектБД.Наименование, ?(ЗначениеЗаполнено(ТабличнаяЧастьБД), ТабличнаяЧастьБДНаименование,""));
		ТекКод=ОбщегоНазначенияУХ.СформироватьКодНаОснованииНаименования(ТекНаименование,ДлинаКода);
	...
	ИначеЕсли СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеСправочники Тогда
		...
		ТекНаименование=СтрШаблон(Нстр("ru = 'База %1, справочник %2%3'; en = 'Base %1, catalog %2%3';tr = 'Taban %1, katalog %2%3'", ДанныеДляФормул.КодЯзыка), ТипБД, ОбъектБД.Наименование, 
	...
	ИначеЕсли СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеДокументы Тогда
		...
		ТекНаименование=СтрШаблон(Нстр("ru = 'База %1, документ %2%3'; en = 'Base %1, document %2%3';tr = 'Taban %1, belge %2%3'", ДанныеДляФормул.КодЯзыка), ТипБД, ОбъектБД.Наименование, 
		                	?(ЗначениеЗаполнено(ТабличнаяЧастьБД), ТабличнаяЧастьБДНаименование,""));
		ТекКод=ОбщегоНазначенияУХ.СформироватьКодНаОснованииНаименования(ТекНаименование,ДлинаКода);
...
Процедура СформироватьНаименованиеОперанда(ОбновитьНаименование=Ложь) Экспорт
	...
	Если СпособПолучения = Перечисления.СпособыПолученияОперандов.ВнешниеДанныеРегистрБухгалтерии
	 ...
	 ИЛИ СпособПолучения = Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеРегистрБухгалтерии Тогда
		...
		Иначе	
			...
			ТекНаименование = ?(ВнешниеДанные,ПолучитьСинонимВИБ(),"")+ПолучитьСинонимРБ() + ?(СпособПолучения = Перечисления.СпособыПолученияОперандов.ВнешниеДанныеОстаткиИОборотыИзПланаСчетов, "77", "") + "_"+ПолучитьСинонимРесурса()+"_";
			
			Если ВидИтога = "НачальныйОстаток" ИЛИ ВидИтога = "СН#" Тогда
				ТекНаименование=СтрШаблон(Нстр("ru = '%1начальный остаток (%2)'; en = '%1opening balance (%2)';tr = '%1açılış bakiyesi (%2)'", ДанныеДляФормул.КодЯзыка), ТекНаименование, ДтКт);
			ИначеЕсли ВидИтога = "НачальныйРазвернутыйОстаток" ИЛИ ВидИтога = "СН#РС" Тогда
				ТекНаименование=СтрШаблон(Нстр("ru = '%1развернутый начальный остаток (%2)'; en = '%1detailed opening balance (%2)';tr = '%1ayrıntılı açılış bakiyesi (%2)'", ДанныеДляФормул.КодЯзыка), ТекНаименование, 
				                	ДтКт);
			ИначеЕсли ВидИтога = "КонечныйОстаток" ИЛИ ВидИтога = "СК#" Тогда
				ТекНаименование=СтрШаблон(Нстр("ru = '%1конечный остаток (%2)'; en = '%1closing balance (%2)';tr = '%1kapanış bakiyesi (%2)'", ДанныеДляФормул.КодЯзыка), ТекНаименование, ДтКт);
			ИначеЕсли ВидИтога = "КонечныйРазвернутыйОстаток" ИЛИ ВидИтога = "СК#РС" Тогда
				ТекНаименование=СтрШаблон(Нстр("ru = '%1конечный развернутый остаток (%2)'; en = '%1detailed closing balance (%2)';tr = '%1ayrıntılı kapanış bakiyesi (%2)'", ДанныеДляФормул.КодЯзыка), ТекНаименование, 
				                	ДтКт);
			ИначеЕсли ВидИтога = "Оборот#" ИЛИ ВидИтога = "#О" Тогда
				ТекНаименование=СтрШаблон(Нстр("ru = '%1оборот (%2)'; en = '%1turnover (%2)';tr = '%1ciro (%2)'", ДанныеДляФормул.КодЯзыка), ТекНаименование, ДтКт);
			ИначеЕсли ВидИтога = "Оборот" ИЛИ ВидИтога = "КорДО" Тогда
				ТекНаименование=СтрШаблон(Нстр("ru = '%1корреспондирующий оборот '; en = '%1correspondent turnover ';tr = '%1karşılık gelen ciro'", ДанныеДляФормул.КодЯзыка), ТекНаименование);	
			КонецЕсли;
			
			Если НЕ ПустаяСтрока(КоррСчет) Тогда
				ТекНаименование = СтрШаблон(Нстр("ru = '%1между счетами № %2 и № %3'; en = '%1between accounts %2 and %3';tr = '%1hesap No%2 ve hesap No%3 arasında'", ДанныеДляФормул.КодЯзыка), ТекНаименование, 
				                  	СокрЛП(Счет), СокрЛП(КоррСчет));
			Иначе
				ТекНаименование = СтрШаблон(Нстр("ru = '%1 по счету № %2'; en = '%1 by account %2';tr = 'Hesap No%2''ya göre %1'", ДанныеДляФормул.КодЯзыка), ТекНаименование, Счет);
			КонецЕсли;
	...
	Иначе
		...
		ТекНаименование=Нстр("ru = 'Не указан способ заполнения'; en = 'Filling method is not specified';tr = 'Doldurma yöntemi belirtilmedi'");
...
Процедура ЗаполнитьСоответствиеСпособовИТиповКонтекст()
	...
	НоваяСтрока.ИмяИзмерения 	 = "Версия.Сценарий";
	НоваяСтрока.ИзКонтекста 	 = Истина;
	НоваяСтрока.ИзмерениеРегистра= Ложь;
	НоваяСтрока.ТаблицаАналитики = "Справочник.Сценарии";
...
Функция ПолучитьПредставлениеДаты(ПолеДата)
	
	Если СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеПроизвольныйЗапрос
	...
	СписокГраниц=Новый СписокЗначений;
	
	Если (СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеРегистрСведений
		ИЛИ СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеРегистрСведений)
		И РегистрБД.Периодический=Истина Тогда
		
		СписокГраниц.Добавить("ДатаНач",Нстр("ru = 'Дата среза'; en = 'Slice date';tr = 'Kesit tarihi'"));
		
	Иначе
		
		СписокГраниц.Добавить("ДатаНач",Нстр("ru = 'Дата начала отбора данных'; en = 'Data filter start date';tr = 'Veri filtresi başlangıç tarihi'"));
		СписокГраниц.Добавить("ДатаКон",Нстр("ru = 'Дата окончания отбора данных'; en = 'Data filter end date';tr = 'Veri filtresi bitiş tarihi'"));
		
	КонецЕсли;
	
	ЭлементГраница=СписокГраниц.НайтиПоЗначению(ПолеДата);
	
	Если ЭлементГраница=Неопределено Тогда
		
		Возврат ПолеДата;
		
	Иначе
		
		Возврат ЭлементГраница.Представление;
...
Функция ПолучитьМассивСпособРасчеты(СтруктураПоиска)
	
	МассивСпособРасчеты=Новый Массив;
	
	Для Индекс=0 ПО мСоответствиеИзмеренийСпособовИТипов.Количество()-1 Цикл
		
		СтрокаНайдена=Истина;
		
		СтрокаИзмерения=мСоответствиеИзмеренийСпособовИТипов[Индекс]; 
		
		Для Каждого КлючИЗначение ИЗ СтруктураПоиска Цикл
			
			Если НЕ СтрокаИзмерения[КлючИЗначение.Ключ]=КлючИЗначение.Значение Тогда
		...
		Если СтрокаНайдена Тогда
			...
			МассивСпособРасчеты.Добавить(СтрокаИзмерения);
			
			Возврат МассивСпособРасчеты;
...
КонецФункции // ПолучитьМассивСпособРасчеты()
...
Процедура ЗаполнитьПараметрыОперанда() Экспорт
	...
	Если (НЕ мТаблицаПоказателейБД.Найти(Истина,"Регистратор")=Неопределено)
		...
		СтрокаПараметр=ДеревоПараметровОтбораБД.Строки.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаПараметр,СтрПоле,,"НаименованиеБД");
		СтрокаПараметр.НаименованиеБД=?(ЗначениеЗаполнено(СтрПоле.ПредставлениеПоля),СтрПоле.ПредставлениеПоля,СтрПоле.НаименованиеБД);
	...
	Если СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеРегистрБухгалтерии
		...
		Для Каждого СтрСоответствия ИЗ СопоставленныеСтроки Цикл
			...
			МассивСпособРасчеты=ПолучитьМассивСпособРасчеты(СтруктураПоиска);
		...
		ИначеЕсли СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеПроизвольныйЗапрос Тогда
			...
			ТабПараметрыЗапроса=мТаблицаПоказателейБД.Скопировать(Новый Структура("Измерение", Ложь)); 
			
			Для Каждого ПараметрЗапроса ИЗ ТабПараметрыЗапроса Цикл
				
				Если НЕ ПроверитьНеобходимостьИзмененияПараметра(ПараметрЗапроса.ПолеБД) Тогда
					
					Продолжить;
					
				КонецЕсли;
				
				СтрокаПараметр	= ДеревоПараметровОтбораБД.Строки.Найти(ПараметрЗапроса.ПолеБД,"ПолеБД");
				
				Если СтрокаПараметр=Неопределено Тогда
					
					НовыйПараметр=ДеревоПараметровОтбораБД.Строки.Добавить();
					
					ЗаполнитьЗначенияСвойств(НовыйПараметр,ПараметрЗапроса,,"НаименованиеБД");
					НовыйПараметр.НаименованиеБД=ПараметрЗапроса.ПредставлениеПоля;
					
					Если ПараметрЗапроса.ТипДанныхПоля.Количество()>0 Тогда
						НовыйПараметр.НастройкаСоответствия=ПараметрЗапроса.ТипДанныхПоля[0].Значение;
					КонецЕсли;  
	...
	ИначеЕсли СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеРегистрНакопления
		...
		Для Каждого СтрСоответствия ИЗ МассивСоответствие Цикл
			...
			СтрПоле=НайтиСтрокуДанныхБДПоПолю(СтрСоответствия.ПолеБД);	
			СтрокаПараметр	= ДеревоПараметровОтбораБД.Строки.Найти(СтрСоответствия.ПолеБД,"ПолеБД"); 			
	...
	ИначеЕсли СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеПроизвольныйЗапрос Тогда
		...
		Для каждого СтрОтбор Из мТаблицаВозможныхПолейОтбор Цикл
			...
			Если СтрокаПараметр = Неопределено
				...
				СтрокаПараметр=ДеревоПараметровОтбораБД.Строки.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаПараметр,СтрПоле,,"НаименованиеБД");
				СтрокаПараметр.НаименованиеБД=СтрПоле.ПредставлениеПоля;
			КонецЕсли;
	...
	Для Каждого СтрКонтекст ИЗ МассивПолейКонтекст Цикл
		...
		МассивСпособРасчеты=ПолучитьМассивСпособРасчеты(новый Структура("ТаблицаАналитики,ИзКонтекста",СтрКонтекст.ТаблицаАналитики,Истина));
...
Процедура УстановитьСоответствие(ПолеБД,АналитикаКонсолидации,НастройкаСоответствия=Неопределено) Экспорт
	...
	СтрокаСоответствия.ПсевдонимБД=СтрПолеИсточника.ПредставлениеПоля;
...
Процедура ЗаполнитьПоляПоРеквизитамОбъектаБД()
	...
	Для Каждого СтрРеквизит ИЗ ПотребительРасчета.Реквизиты Цикл
		...
		ПолеАналитика=мТаблицаВозможныхПолей.Добавить();
		ПолеАналитика.Поле=СтрРеквизит.Имя;
		ПолеАналитика.НаименованиеАналитики=СтрРеквизит[ПолеСиноним];
		ПолеАналитика.Синоним=СтрРеквизит[ПолеСиноним];
		ПолеАналитика.АналитикаОтчета=Ложь;
		ПолеАналитика.РеквизитОбъекта=СтрРеквизит.Имя;
...
Процедура ЗаполнитьПоляПоИзмерениямРегистраБД()
	...
	Для Каждого СтрИзмерение ИЗ ПотребительРасчета.Измерения Цикл
		...
		ПолеАналитика.НаименованиеАналитики=СтрИзмерение[ПолеСиноним];
		ПолеАналитика.Синоним=СтрИзмерение[ПолеСиноним];
		ПолеАналитика.АналитикаОтчета=Ложь;
...
Процедура ЗаполнитьПоляПоРеквизитамОписанияОбъектаБД()
	...
	Если ТипПотребителяСтрока="ТабличныеЧастиБД" Тогда
				
		ЗаполнитьПоляДляИмпортаТЧ();
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	Если Ссылка.Пустая()
		И ОбщегоНазначенияПовтИспУХ.ИспользуетсяМногопериоднаяОтчетность()
...
Процедура ПередЗаписью(Отказ)
			...
			Если ЗначениеЗаполнено(ЭлементОтбораДатаНач) И ЗначениеЗаполнено(ЭлементОтбораДатаКон) Тогда 
				...
				Если ВидИтога = "НачальныйОстаток"
					...
					Иначе
						ЭлементОтбораДатаКон = УточненияСпособовОпределения.Найти("ДатаКон","ПолеБД");
						Если ЭлементОтбораДатаКон <> Неопределено Тогда
							УточненияСпособовОпределения.Удалить(ЭлементОтбораДатаКон);    
						КонецЕсли;
					КонецЕсли; 			
				...
				ИначеЕсли ВидИтога = "КонечныйОстаток"
					...
					Иначе
						ЭлементОтбораДатаНач = УточненияСпособовОпределения.Найти("ДатаНач","ПолеБД");
						Если ЭлементОтбораДатаНач <> Неопределено Тогда
							УточненияСпособовОпределения.Удалить(ЭлементОтбораДатаНач);    
						КонецЕсли;
					КонецЕсли;
	...
	Если НЕ (ОбщегоНазначенияПовтИспУХ.ИспользуетсяМногопериоднаяОтчетность()
		И СпособПолучения = Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеПроизвольныйЗапрос) Тогда
		ИспользоватьМногопериодныйКонтекст = Ложь;
	КонецЕсли;
	
	ЗаполнитьТекстовыйТипПараметровОтбора();
...
Процедура ЗаполнитьТекстовыйТипПараметровОтбора()
	
	Для Каждого СтрОтбор ИЗ ТаблицаПараметровОтбораБД Цикл
		
		МассивПутей = СтрРазделить(СтрОтбор.ПолеБД, ".");
		Если СтрНачинаетсяС(МассивПутей[0], "Аналитика") Тогда
			
			СтрПоле = мТаблицаВозможныхПолейОтбор.Найти(МассивПутей[0], "Поле");
			Если СтрПоле <> Неопределено Тогда
				СтрОтбор.ТипЗначенияАналитики = СтрПоле.ТаблицаАналитики;
			КонецЕсли;
			
		ИначеЕсли СтрНачинаетсяС(МассивПутей[0], "Субконто") Тогда
			
			СтрПоле = мТаблицаПоказателейБД.Найти(МассивПутей[0],"ПолеБД");
			Если СтрПоле <> Неопределено И СтрПоле.ТипЗначения.Количество() = 1 Тогда
				СтрОтбор.ТипЗначенияАналитики = СтрПоле.ТипЗначения[0].Значение;
			КонецЕсли;
...
Функция СформироватьКодУпрощеннойФормулы() Экспорт
	...
	Если (СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеПоказательОтчета ИЛИ СпособПолучения=Перечисления.СпособыПолученияОперандов.ПроводкаТрансформационнойКорректировки)
		...
		ИЛИ НазначениеРасчетов.Владелец=ПоказательОтбор.Владелец) Тогда
...
Функция ПолучитьТекстАналитик(СтруктураОтбор) Экспорт
	...
	Для Каждого СтрПоле ИЗ ПравилаИспользованияПолейЗапроса Цикл
		...
		Если Лев(СтрПоле.Поле,СтрДлина("Аналитика"))="Аналитика" И (НЕ Сред(СтрПоле.Поле,(СтрДлина("Аналитика")+1),1)="_") Тогда
			
			ТекстАналитики=ТекстАналитики+",А"+Сред(СтрПоле.Поле,(СтрДлина("Аналитика")+1),1);
		...
		ИначеЕсли Лев(СтрПоле.Поле,СтрДлина("Субконто"))="Субконто" Тогда
			
			Если Лев(СтрПоле.Поле,СтрДлина("СубконтоКт"))="СубконтоКт" Тогда
					
				ТекстАналитики=ТекстАналитики+",КС"+Сред(СтрПоле.Поле,(СтрДлина("СубконтоКт")+1),1);
				
			ИначеЕсли Лев(СтрПоле.Поле,СтрДлина("СубконтоДт"))="СубконтоДт" Тогда
					
				ТекстАналитики=ТекстАналитики+",С"+Сред(СтрПоле.Поле,(СтрДлина("СубконтоДт")+1),1);
				
			Иначе
				
				ТекстАналитики=ТекстАналитики+",С"+Сред(СтрПоле.Поле,СтрДлина("Аналитика"),1);
...
Функция ВернутьСоставПериметра(ВладелецПериметра=Неопределено,СоставПериметра=Неопределено) Экспорт
	...
	Запрос.Текст=Запрос.Текст+"
	|И (НЕ СоставПериметраСостав.Организация.ЭлиминирующаяОрганизация)";
...
Функция ПроверитьВедениеУчетаПоРесурсуБД(ЭлементЗначения)
	...
	ИначеЕсли ЭлементЗначения.Измерение 
		ИЛИ ЭлементЗначения.Регистратор
		ИЛИ ЭлементЗначения.ПолеБД="КоррСчетОборот" Тогда
...
Функция НайтиСтрокуДанныхБДПоПолю(ПолеБД) Экспорт;
	...
	Если СтрПоле=Неопределено И (СтрНайти(ПолеБД,".")>0 ИЛИ СтрНайти(ПолеБД,"@")>0) Тогда
		
		СтрПоле=мТаблицаПроизводныхБД.Найти(ПолеБД,"ПолеБД");
		
		Если СтрПоле=Неопределено Тогда
			
			Возврат ОпределитьРабочуюСтроку("ПолеБД",ПолеБД);
...
Процедура ОпределитьТипЗначенияИсточника()
	...
	Иначе
		...
		Если МассивСопоставлено.Количество()>0 Тогда
			...
			СтрДанные=МассивСопоставлено[0];
			
			Если СтрДанные.СпособЗаполнения=Перечисления.СпособыЗаполненияПолейИсточника.ФиксированноеЗначение Тогда
				
				ТипЗначения = ОбщегоНазначенияУХ.ПолучитьТипЗначенияПоказателяПоТипу(ТипЗнч(СтрДанные.ФиксированноеЗначение));
				
			Иначе
				
				СтрПоле=НайтиСтрокуДанныхБДПоПолю(СтрДанные.ПолеБД);
				...
				Если СтрПоле.ТипЗначения.Количество() = 1 Тогда
					ТипЗначения = ОбщегоНазначенияУХ.ПолучитьТипЗначенияПоказателяПоТипу(СтрПоле.ТипЗначения[0].Значение);
				Иначе
					// Тип не определен однозначно
					ТипЗначения = Перечисления.ТипыЗначенийПоказателейОтчетов.ПустаяСсылка();	
				КонецЕсли; 
...
Процедура ПодготовитьТаблицуПолейДляРегистраВИБ() Экспорт
	...
	Если СпособПолучения = Перечисления.СпособыПолученияОперандов.ВнешниеДанныеРегистрБухгалтерии
		...
		Иначе
			...
			Если (ТрансляцияДебет ИЛИ ТрансляцияКредит ИЛИ ИтогЕстьОборотПоСчетам() ИЛИ УчитыватьКоррСчета) Тогда
				...
				Иначе
					...
					Если ИтогЕстьОборотПоСчетам() ИЛИ УчитыватьКоррСчета Тогда 
...
Функция ЗаполнитьДеревоПоРегиструБухгалтерииБД() Экспорт
	...
	Если СписокИзмерений.Количество()>0 Тогда
		...
		Для Каждого Элемент Из СписокИзмерений Цикл
			...
			Если Элемент.ПолеБД="КоррСчетОборот" Тогда
				
				ПолеКоррСчет=ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(Элемент);
				Продолжить;
	...
	Если ТипЗнч(ПолеКоррСчет)=Тип("Структура") Тогда
		
		СтрокаКоррСчет=ДеревоПолейБД.Строки.Добавить();
        ЗаполнитьЗначенияСвойств(СтрокаКоррСчет,ПолеКоррСчет);
...
Функция ЗаполнитьДеревоПоКорреспонденцииФинасовыхРегистров() Экспорт
	...
	ДанныеРегистраДт.НаименованиеБД=РегистрБД[ПолеСиноним];
	...
	ДанныеРегистраКт.НаименованиеБД=КоррРегистрБД[ПолеСиноним];
...
Функция ПолучитьПсевдонимБД(СтрПоле)
	
	Если СпособИспользования=Перечисления.СпособыИспользованияОперандов.ДляАналитическихОтчетов Тогда
		
		Возврат ОбщегоНазначенияУХ.ВернутьАлфавитноЦифровоеПредставление(СтрПоле.НаименованиеБД);
		
	Иначе
		
		Возврат СтрПоле.ПредставлениеПоля;
		
	КонецЕсли;
	
КонецФункции // ПолучитьПсевдонимБД() 
...
Функция СформироватьТаблицуСоответствияТек() Экспорт
	...
	Если ЗначениеИсточникаОпределяетсяВручную() Тогда	
		...
		НоваяСтрока.ПсевдонимКонсолидации = НСтр("ru = 'Значение'");
		...
		Для Каждого СтрЗначение ИЗ МассивЗначений Цикл
			...
			Если СтрЗначение.ТипЗначения[0].Значение=ОжидаемыйТип Тогда
				...
				НоваяСтрока.ПсевдонимБД    		= ПолучитьПсевдонимБД(СтрЗначение);
		...
		Если ИспользоватьМногопериодныйКонтекст Тогда
			...
			НоваяСтрока.ПсевдонимКонсолидации 	= НСтр("ru = 'Период'");
	...
	Для Каждого Поле Из мТаблицаВозможныхПолей Цикл  
		...
		Если Поле.Поле="Версия.Валюта" И (НЕ СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеПоказательОтчета) Тогда
		...
		ТаблицаПолей=ПодобратьТаблицуСоответствийДляПоля(Поле,СтрокаТаблицы);
		
		Если ТаблицаПолей.Количество()>0 Тогда
			
			ТаблицаПолей.Сортировать("КоличествоТипов");
			СтрокаТаблицы.ПолеБД=ТаблицаПолей[0].ПолеБД;
			СтрокаТаблицы.ПсевдонимБД=ТаблицаПолей[0].ПсевдонимБД;
	...
	Если СпособИспользования=Перечисления.СпособыИспользованияОперандов.ДляАналитическихОтчетов Тогда
		...
		Если мТаблицаПоказателейБД.Количество()>1 
			И СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеПроизвольныйЗапрос Тогда
			...
			Для Каждого СтрПоказатель ИЗ мТаблицаПоказателейБД Цикл
				...
				Если СтрПоказатель.Измерение Тогда
					...
					НоваяСтрока.ПсевдонимБД=ПолучитьПсевдонимБД(СтрПоказатель);
					НоваяСтрока.Сопоставлено=Истина;
					НоваяСтрока.СпособЗаполнения	= Перечисления.СпособыЗаполненияПолейИсточника.ПолеИсходнойТаблицы;
	...
	ИначеЕсли СпособИспользования=Перечисления.СпособыИспользованияОперандов.ПроизвольныеДанные Тогда
		
		Для Каждого СтрПоказатель ИЗ мТаблицаПоказателейБД Цикл
			
			Если СтрПоказатель.ТипДанныхПоля.Количество()=1 
				И (НЕ СтрПоказатель.ТипМетаДанных="Простой") Тогда  
				
				НоваяСтрока=ТаблицаСоответствияБД.Добавить();
				НоваяСтрока.ПолеБД=СтрПоказатель.ПолеБД;
				НоваяСтрока.ПсевдонимБД=ПолучитьПсевдонимБД(СтрПоказатель);
				НоваяСтрока.Сопоставлено=Истина;
				НоваяСтрока.ВидАналитики=ПолучитьВидСубконтоПоТипуПоля(СтрПоказатель.ТипДанныхПоля[0].Значение);
				НоваяСтрока.СпособЗаполнения	= Перечисления.СпособыЗаполненияПолейИсточника.ПолеИсходнойТаблицы;
		...
		Если НЕ СтрокаВалюта=Неопределено Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрокаВалюта.ПолеБД) Тогда
				...
				Если МассивИзмерений.Количество()>0 Тогда
					...
					СтрокаВалюта.ПсевдонимБД		= ПолучитьПсевдонимБД(МассивИзмерений[0]);
...
Процедура ЗаполнитьСоответствияИзмеренийПоУмолчанию(ТаблицаСоответствияБД)
	...
	Пока Результат.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаСоответствия.ПолеБД) Тогда
			...
			СтрПоле=ОпределитьРабочуюСтроку("ПолеБД",Результат.Поле); 
			
			Если НЕ СтрПоле=Неопределено Тогда
				
				СтрокаСоответствия.ПсевдонимБД=ПолучитьПсевдонимБД(СтрПоле);
...
Функция ЗначениеИсточникаОпределяетсяВручную() Экспорт
	...
	Возврат ((СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеПроизвольныйЗапрос
		...
		И (НЕ СпособИспользования=Перечисления.СпособыИспользованияОперандов.ПроизвольныеДанные)
		...
		Для Каждого ОбъектКонсолидации ИЗ ВозможныеОбъекты Цикл
			...
			Иначе
				...
				Если СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеПроизвольныйЗапрос Тогда
					...
					ОбъектыВИБ         = мТаблицаПоказателейБД.НайтиСтроки(Новый Структура("СправочникБД,Измерение", Выгрузка.ИмяОбъектаВнешнейИБ,Истина))
			...
			Если СтрокаТаблицы=Неопределено Тогда
				...
				СтрокаТаблицы.ПсевдонимБД                  	= ПолучитьПсевдонимБД(Элемент);
		...
		Если ТаблицаСоответствияБД.Найти(Поле.Поле, "АналитикаКонсолидации")=Неопределено Тогда
			...
			Если Поле.ТипМетаДанных="Простой" Тогда
				...
				Если ВозможнаУстановкаСоответствия(Поле,СтрокаИсточник) Тогда
					...
					СтрокаТаблицы.ПсевдонимБД               = ПолучитьПсевдонимБД(СтрокаИсточник);
			...
			Если Поле.ТаблицаАналитики="Справочник.СчетаБД" И (НЕ СтрокаТаблицы.АналитикаОтчета) Тогда
				
				СтрокаКоррСчет=мТаблицаПоказателейБД.Найти("КоррСчетОборот","ПолеБД");
				
				Если НЕ СтрокаКоррСчет=Неопределено Тогда
					
					СтрокаТаблицы.ПолеБД	            	= СтрокаКоррСчет.ПолеБД;
					СтрокаТаблицы.ПсевдонимБД               = ПолучитьПсевдонимБД(СтрокаКоррСчет);
					СтрокаТаблицы.СправочникБД              = СтрокаКоррСчет.СправочникБД;
					СтрокаТаблицы.ТипМетаДанных             = СтрокаКоррСчет.ТипМетаДанных;
					СтрокаТаблицы.Измерение                 = СтрокаКоррСчет.Измерение;
					СтрокаТаблицы.Дебетовый                 = СтрокаКоррСчет.Дебетовый;
					СтрокаТаблицы.Сопоставлено              = Истина;
	...
	Если СпособИспользования=Перечисления.СпособыИспользованияОперандов.ДляАналитическихОтчетов Тогда
		...
		Если мТаблицаПоказателейБД.Количество()>1 Тогда
			...
			Если СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеПроизвольныйЗапрос Тогда
				...
				|	ТекТаблицаВИБ.ПредставлениеПоля,
				...
				Пока ПоляЗапроса.Следующий() Цикл
					...
					СтрокаТаблицы.ПсевдонимБД                  	= ПолучитьПсевдонимБД(ПоляЗапроса);
...
Функция ПолучитьРодителяДляСопоставления(СводнаяТаблицаПоказателей,АналитикаОтчета)
	...
	Если АналитикаОтчета Тогда
		...
		ТекстРеквизита=?(СпособИспользования=Перечисления.СпособыИспользованияОперандов.ДляТрансформационныхКорректировок,НСтр("ru = 'Реквизиты корректировки'"),НСтр("ru = 'Реквизиты отчета'"));
	...
	Иначе
		...
		Иначе	
			...
			ТекстАналитики=?(СпособИспользования=Перечисления.СпособыИспользованияОперандов.ДляТрансформационныхКорректировок,НСтр("ru = 'Аналитика проводки'"),НСтр("ru = 'Аналитика показателя'"));
...
Функция ПолучитьСинонимДопРеквизита(ПолеДопРеквизита)
	
	ДанныеПоля=СтрРазделить(ПолеДопРеквизита,"@");	
	СтрокаРеквизит=ДанныеПоля[1];	
	ДанныеРеквизита=СтрРазделить(СтрокаРеквизит,"#");
		
	Возврат ДанныеПоля[0]+"_"+ДанныеРеквизита[2];
		
КонецФункции // ПолучитьСинонимДопРеквизита() 
...
Процедура ПодготовитьТаблицуИспользованияПолейВнутр() Экспорт
	...
	Если СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеРегистрНакопления Тогда
		...
		Если ОсновнаяТаблицаРегистра Тогда
			...
			СтрокаСоответствия = ПравилаИспользованияПолейЗапроса.Добавить();
			СтрокаСоответствия.Поле    = РесурсРегистра;
			СтрокаСоответствия.Синоним = РесурсРегистра;
			СтрокаСоответствия.КодАналитики="Значение";
			Оборот = Ложь;
		...
		Иначе
			...
			СтрокаСоответствия = ПравилаИспользованияПолейЗапроса.Добавить();
			СтрокаСоответствия.Поле    = Поле;
			СтрокаСоответствия.Синоним = Поле;
			СтрокаСоответствия.КодАналитики="Значение";
			Оборот = Ложь;
	...
	ИначеЕсли СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеРегистрБухгалтерии Тогда
		...
		Если (ТрансляцияДебет ИЛИ ТрансляцияКредит ИЛИ ИтогЕстьОборотПоСчетам() ИЛИ УчитыватьКоррСчета) И ЗначениеЗаполнено(РесурсРегистра) Тогда
			...
			ИначеЕсли ТрансляцияДебет ИЛИ ТрансляцияКредит Тогда
		...
		СтрокаСоответствия         = ПравилаИспользованияПолейЗапроса.Добавить();
		СтрокаСоответствия.Поле    = ИмяРесурса;
		СтрокаСоответствия.Синоним = ИмяРесурса;
		СтрокаСоответствия.КодАналитики="Значение";
	...
	Для Каждого СтрокаСоответствия Из МассивСопоставлено Цикл
		...
		Если СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеРегистрБухгалтерии Тогда
			...
			Если СпособИспользования=Перечисления.СпособыИспользованияОперандов.ДляАналитическихОтчетов Тогда
				НоваяСтрока.Синоним=ОбщегоНазначенияУХ.ВернутьАлфавитноЦифровоеПредставление(СтрокаСоответствия.ПсевдонимБД);
				НоваяСтрока.КодАналитики=НоваяСтрока.Синоним;
			ИначеЕсли СтрНайти(СтрокаСоответствия.ПолеБД,"@")>0 Тогда
				НоваяСтрока.Синоним=ПолучитьСинонимДопРеквизита(СтрокаСоответствия.ПолеБД);	
			Иначе
				НоваяСтрока.Синоним=ОбщегоНазначенияУХ.ВернутьАлфавитноЦифровоеПредставление(СтрокаСоответствия.ПолеБД);
			КонецЕсли;
...
Процедура ДобавитьПолеРесурсаДляЗаполнения(ПолеБД,КодАналитики,ПравилаИспользованияПолейЗапроса)
	
	Если СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеРегистрНакопления
		ИЛИ СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеРегистрНакопления Тогда
		
		Если ОсновнаяТаблицаРегистра Тогда
			
			СтрокаСоответствия = ПравилаИспользованияПолейЗапроса.Добавить();
			СтрокаСоответствия.Поле    = ПолеБД;
			СтрокаСоответствия.Синоним = ПолеБД+"_РесурсРегистра";
			СтрокаСоответствия.КодАналитики=КодАналитики;
			
		Иначе
			
			Поле = ПолеБД + ВидИтога;
	...
	ИначеЕсли СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеРегистрСведений
	...
	ИначеЕсли СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеРегистрБухгалтерии
		ИЛИ СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеРегистрБухгалтерии Тогда
		
		Если (ИтогЕстьОборотПоСчетам() ИЛИ УчитыватьКоррСчета) Тогда
			
			ДанныеРесурса=РегистрБД.Ресурсы.Найти(ПолеБД,"Имя");
			...
			ИначеЕсли ТрансляцияДебет ИЛИ ТрансляцияКредит Тогда
				...
				ИмяРесурса=ПолеБД+?(ТрансляцияДебет,"Дт","Кт");
				
			Иначе
				
				ИмяРесурса=ПолеБД+ДтКт;
		...
		ИначеЕсли НЕ ВидИтога="Оборот" Тогда
			...
			КоррСчетТекущейИБ="";	
			ИмяРесурса=ПолеБД+СтрЗаменить(ВидИтога,"#",ДтКт);
							
		Иначе
			
			ТекРесурс=РегистрБД.Ресурсы.Найти(ПолеБД,"Имя");
			
			Если ТекРесурс.Балансовый Тогда
				ИмяРесурса=ПолеБД+"Оборот";
			Иначе
				ИмяРесурса=ПолеБД+"Оборот"+ДтКт;
			КонецЕсли;			
...
КонецПроцедуры // ДобавитьПолеРесурсаДляЗаполнения() 

Функция ИспользуютсяДанныеРегистра()
...
КонецФункции // ИспользуютсяДанныеРегистра() 

Процедура ПодготовитьТаблицуИспользованияПолейОбъектБД() Экспорт
	
	ИспользуютсяДанныеРегистра=ИспользуютсяДанныеРегистра();
	ПравилаИспользованияПолейЗапроса.Очистить();	
	МассивСопоставлено=ТаблицаСоответствияБД.НайтиСтроки(Новый Структура("Сопоставлено,АналитикаОтчета",Истина,Ложь));
	...
	Для Каждого СтрокаСоответствия Из МассивСопоставлено Цикл
		...
		Если ИспользуютсяДанныеРегистра И (НЕ РегистрБД.Ресурсы.Найти(СтрокаСоответствия.ПолеБД,"Имя")=Неопределено) Тогда
			
			ДобавитьПолеРесурсаДляЗаполнения(СтрокаСоответствия.ПолеБД,СтрокаСоответствия.АналитикаКонсолидации,ПравилаИспользованияПолейЗапроса);
			
		Иначе
			
			Если СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеРегистрБухгалтерии
				ИЛИ СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеРегистрБухгалтерии Тогда
...
Функция ОбработатьИзменениеТекстаЗапроса(ТекстЗапросаФорма) Экспорт
	...
	Иначе
		...
		Иначе 
			...
			Для Каждого СтрПоле ИЗ ОписаниеЗапроса.МассивПолей Цикл
				
				Если СтрПоле.ПолеБД="Ссылка" ИЛИ СтрПоле.ПолеБД="ИсходныйОбъектСсылка" Тогда
					
					ТекстОшибки=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Имя колонки ""%1"" зарезервировано, выберите другой псевдоним.'"),СтрПоле.ПолеБД);					
	...
	ЗаполнитьДеревоПоПроизвольномуЗапросуБД();
	...
	Иначе
		...
		Для Каждого СтрРесурс ИЗ РегистрБД.Ресурсы Цикл
			...
			Если ПустаяСтрока(СтрРесурс.ПризнакУчета) ИЛИ СтрНайти(Счет.ПризнакиУчета,СтрРесурс.ПризнакУчета)>0
				...
				СписокРесурсовРегистра.Добавить(СтрРесурс.Имя,СтрРесурс[ПолеСиноним]);
...
Процедура УстановитьВозможностьДтКт() Экспорт
	...
	Если Использование77() Тогда
		...
		Если ВидИтога="#О" ИЛИ Счет.Вид=Перечисления.ВидыСчетовБД.АктивноПассивный Тогда	
			СписокДтКт.Добавить("Д",Нстр("ru = 'Дт'; en = 'Dr';tr = 'Borç'"));
			СписокДтКт.Добавить("К",Нстр("ru = 'Кт'; en = 'Cr';tr = 'Alacak'")); 
			
			Если СтрНайти(ВидИтога,"РС")=0 Тогда 		
				СписокДтКт.Добавить("СВ",Нстр("ru = 'Дт - Кт'; en = 'Dr - Cr';tr = 'Borç - Alacak'"));	
			КонецЕсли;
	...
	Иначе
		...
		Если ВидИтога="Оборот" Тогда
			...
			Если НЕ (ТекРесурс=Неопределено ИЛИ ТекРесурс.Балансовый) Тогда
				...
				СписокДтКт.Добавить("Дт",Нстр("ru = 'Дт'; en = 'Dr';tr = 'Borç'"));
				СписокДтКт.Добавить("Кт",Нстр("ru = 'Кт'; en = 'Cr';tr = 'Alacak'"));
...
Процедура ПриИзмененииРегистраБД(РегистрНакопления=Истина) Экспорт
	...
	РаботаСОбъектамиМетаданныхУХ.ПроверитьЗаполнитьСинонимы(РегистрБД);
...
Процедура ЗаполнитьИзмеренияРегистраБД(РегистрБухгалтерии = Истина) Экспорт
	...
	Если (РегистрБухгалтерии И (ТрансляцияДебет ИЛИ ТрансляцияКредит ИЛИ ИтогЕстьОборотПоСчетам())) ИЛИ 
		(ОсновнаяТаблицаРегистра И СтрДлина(РегистрБД.Регистраторы)>0) Тогда
	...
	Для Каждого СтрИзмерение ИЗ РегистрБД.Измерения Цикл
		...
		Если РегистрБухгалтерии Тогда
			...
			Иначе
				...
				СтрокаПоказателей.НаименованиеБД 	= СтрШаблон(Нстр("ru = '%1 (дт.)'"), СтрИзмерение[ПолеСиноним]);
				СтрокаПоказателей.ПредставлениеПоля = "["+СтрокаПоказателей.НаименованиеБД+"]";
				СтрокаПоказателей.Измерение       	= Истина;
				СтрокаПоказателей.Балансовый 					= СтрИзмерение.Балансовый;
				СтрокаПоказателей.НеЯвляетсяРазделителемДанных 	= СтрИзмерение.НеЯвляетсяРазделителемДанных;
				...
				СтрокаПоказателей.НаименованиеБД 	= СтрШаблон(Нстр("ru = '%1 (кт.)'"), СтрИзмерение[ПолеСиноним]);
				СтрокаПоказателей.ПредставлениеПоля = "["+СтрокаПоказателей.НаименованиеБД+"]";
				СтрокаПоказателей.Измерение       	= Истина;
				СтрокаПоказателей.Балансовый 					= СтрИзмерение.Балансовый;
				СтрокаПоказателей.НеЯвляетсяРазделителемДанных 	= СтрИзмерение.НеЯвляетсяРазделителемДанных;
	...
	Если СпособИспользования=Перечисления.СпособыИспользованияОперандов.ДляЗаполненияОбъектовБД
		И (НЕ ТипЗнч(НазначениеРасчетов)=Тип("СправочникСсылка.ШаблоныТрансляций")) Тогда
		
		Для Каждого СтрРесурс ИЗ РегистрБД.Ресурсы Цикл
			
			СтрокаПоказателей                 	= мТаблицаПоказателейБД.Добавить();
			СтрокаПоказателей.ПолеБД         	= СтрРесурс.Имя;
			СтрокаПоказателей.НаименованиеБД 	= СтрРесурс[ПолеСиноним];
			СтрокаПоказателей.ПредставлениеПоля = "["+СтрокаПоказателей.НаименованиеБД+"]";
			СтрокаПоказателей.Измерение       	= Ложь;
			СтрокаПоказателей.Балансовый = Ложь;
			
			Если ТипЗнч(РегистрБД)=Тип("СправочникСсылка.РегистрыСведенийБД") Тогда
				
				РаботаСПолямиАналитикиУХ.ЗаполнитьПоляТиповРеквизитовБД(СтрокаПоказателей, СтрРесурс.ТипДанных,ТипБД,ТипБДПриемник);
				
			Иначе
				
				РаботаСПолямиАналитикиУХ.ЗаполнитьПоляТиповРеквизитовБД(СтрокаПоказателей, "Число",ТипБД,ТипБДПриемник);
...
Процедура ЗаполнитьПоКорреспонденцииФинансовыхРегистров()
	...
	Для Каждого СтрИзмерение ИЗ РегистрБД.Измерения Цикл
		...
		СтрокаПоказателей                 	= мТаблицаПоказателейБД.Добавить();
		СтрокаПоказателей.ПолеБД         	= СтрИзмерение.Имя;
		СтрокаПоказателей.НаименованиеБД 	= СтрИзмерение[ПолеСиноним]+ "( дт.)";
		СтрокаПоказателей.ПредставлениеПоля = "["+СтрокаПоказателей.НаименованиеБД+"]";
		СтрокаПоказателей.Измерение       	= Истина;
		СтрокаПоказателей.Балансовый 		= Истина;
	...
	Для Каждого СтрРеквизит ИЗ РегистрБД.Реквизиты Цикл
		...
		СтрокаПоказателей                 	= мТаблицаПоказателейБД.Добавить();
		СтрокаПоказателей.ПолеБД         	= СтрРеквизит.Имя;
		СтрокаПоказателей.НаименованиеБД 	= СтрРеквизит[ПолеСиноним]+ "( дт.)";
		СтрокаПоказателей.ПредставлениеПоля = "["+СтрокаПоказателей.НаименованиеБД+"]";
		СтрокаПоказателей.Измерение       	= Ложь;
		СтрокаПоказателей.Балансовый 		= Ложь;
		СтрокаПоказателей.Дебетовый			= Истина;
	...
	Для Каждого СтрИзмерение ИЗ КоррРегистрБД.Измерения Цикл
		...
		СтрокаПоказателей.НаименованиеБД 	= СтрИзмерение[ПолеСиноним]+ "( кт.)";
		СтрокаПоказателей.ПредставлениеПоля = "["+СтрокаПоказателей.НаименованиеБД+"]";
		СтрокаПоказателей.Измерение       	= Истина;
		СтрокаПоказателей.Балансовый 		= Истина;
	...
	Для Каждого СтрРеквизит ИЗ КоррРегистрБД.Реквизиты Цикл
		...
		СтрокаПоказателей.НаименованиеБД 	= СтрРеквизит[ПолеСиноним]+ "( кт.)";
		СтрокаПоказателей.ПредставлениеПоля = "["+СтрокаПоказателей.НаименованиеБД+"]";
		СтрокаПоказателей.Измерение       	= Ложь;
		СтрокаПоказателей.Балансовый 		= Ложь;
		СтрокаПоказателей.Дебетовый			= Ложь;
...
Процедура ЗаполнитьПоляДляИмпортаТЧ()
...
Функция ЗаполнитьДеревоПоОбъектуВИБ() Экспорт
	...
	ЛокальноеДерево.Колонки.Добавить("ПредставлениеПоля", 	мОписаниеТиповСтрока);
...
Процедура ЗаполнитьАналитикуСчетаБД(Дебетовый = Истина) Экспорт
	...
	Если Дебетовый И ВидИтога="Оборот#" И (НЕ (ТрансляцияДебет ИЛИ ТрансляцияКредит)) Тогда
		
		СтрокаПоказателей = мТаблицаПоказателейБД.Добавить();
		СтрокаПоказателей.ПолеБД="КоррСчетОборот";
		СтрокаПоказателей.НаименованиеБД=НСтр("ru = 'Корр. счет'");
		СтрокаПоказателей.ПредставлениеПоля="["+СтрокаПоказателей.НаименованиеБД+"]";
		СтрокаПоказателей.Дебетовый=Истина;
		
		РаботаСПолямиАналитикиУХ.ЗаполнитьПоляТиповРеквизитовБД(СтрокаПоказателей, "Справочник.СчетаБД",Справочники.ТипыБазДанных.ТекущаяИБ,Справочники.ТипыБазДанных.ТекущаяИБ);
	...
	Для Каждого ВидСубконто Из ТекСчет.ВидыСубконто Цикл
		...
		Если СтрНайти(СтрокаПоказателей.ПолеБД,"СубконтоДт")>0 Тогда
			...
			СтрокаПоказателей.НаименованиеБД  = СтрЗаменить(СтрокаПоказателей.ПолеБД,"СубконтоДт", Нстр("ru = 'Субконто'; en = 'Extra dimension';tr = 'Ekstra boyut'"))+": "+ВидСубконто.ВидСубконто;
			
		ИначеЕсли СтрНайти(СтрокаПоказателей.ПолеБД,"СубконтоКт")>0 Тогда
			
			СтрокаПоказателей.НаименованиеБД  = СтрЗаменить(СтрокаПоказателей.ПолеБД,"СубконтоКт", Нстр("ru = 'Корр. субконто'; en = 'Corr. extra dimension';tr = 'Karşılık gelen ekstra boyut'"))+": "+ВидСубконто.ВидСубконто;
			
		Иначе
			
			СтрокаПоказателей.НаименованиеБД  =  СтрЗаменить(СтрокаПоказателей.ПолеБД,"Субконто", Нстр("ru = 'Субконто'; en = 'Extra dimension';tr = 'Ekstra boyut'"))+": "+ВидСубконто.ВидСубконто;
...
Процедура ПодготовитьСтрокуОтбораВИБ(СтрокаТаблицы) Экспорт
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(СтрокаТаблицы.ТаблицаАналитикиВИБ) Тогда
		
		Если ЗначениеЗаполнено(СтрокаТаблицы.ТипМетаданных) И ЗначениеЗаполнено(СтрокаТаблицы.СправочникБД) Тогда
...
Функция СформироватьОбщийЗапросКДокументуБД() Экспорт
	...
	СтрДатаНач=ТаблицаПараметровОтбораБД.Найти("ДатаНач","ИмяПараметра");
	СтрДатаКон=ТаблицаПараметровОтбораБД.Найти("ДатаКон","ИмяПараметра");
...
Функция СформироватьТекстЗапросаПоГруппеРаскрытия()
	...
	Для Каждого СтрПоле ИЗ ПравилаИспользованияПолейЗапроса Цикл
	 ...
	 |ИМЕЮЩИЕ "+Сред(ТекстУсловиеИтог,СтрДлина("ИЛИ")+2);
...
Процедура ДобавитьОтборИзмерениеПланСчетов(Таблица,ИмяИзмерения,ЗначениеОтбора) Экспорт
	...
	Если ИмяИзмерения="ПланСчетов" Тогда
		...
		Если ТипЗнч(ЗначениеОтбора)=Тип("Строка") Тогда
			...
			НоваяСтрока.НаименованиеБД=НСтр("ru = 'План счетов'");
		...
		Иначе 
			...
			ИмяСправочникаПланыСчетов="ПланыСчетовМеждународногоУчета";
			
			СтрокаУточнение=УточненияСпособовОпределения.Добавить();
			СтрокаУточнение.Значение=Справочники[ИмяСправочникаПланыСчетов].НайтиПоНаименованию(ЗначениеОтбора.Наименование,Истина);
			СтрокаУточнение.ИмяПараметра="ПланСчетов";
			СтрокаУточнение.ПолеБД="ПланСчетов";
	...
	Иначе
		...
		Иначе
			...
			НоваяСтрока.ИдентификаторСтроки=Таблица.Индекс(НоваяСтрока)+1;
			НоваяСтрока.ИмяПараметра="ПланСчетовБД";
			НоваяСтрока.НаименованиеБД=НСтр("ru = 'План счетов БД'");
			НоваяСтрока.ПолеБД="ПланСчетовБД";
			НоваяСтрока.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ФиксированноеЗначение;
...
Процедура ДобавитьГраницыИнтервала(Строка)
	
	Если НЕ Строка.УточнениеСпособаОпределения.Количество()=2 Тогда
		
		Возврат;
		
	КонецЕсли;
	
	ДанныеГраницаНач=Строка.УточнениеСпособаОпределения[0];
	
	НоваяСтрока=УточненияСпособовОпределения.Добавить();
			
	НоваяСтрока.ПолеБД		=Строка.ПолеБД;
	НоваяСтрока.ИмяПараметра=Строка.ИмяПараметра+"_1";
		
	ДобавитьПараметрыГраницы(НоваяСтрока,ДанныеГраницаНач.Значение);
	
	ДанныеГраницаКон=Строка.УточнениеСпособаОпределения[1];
	
	НоваяСтрока=УточненияСпособовОпределения.Добавить();
			
	НоваяСтрока.ПолеБД		=Строка.ПолеБД;
	НоваяСтрока.ИмяПараметра=Строка.ИмяПараметра+"_2";
	
	ДобавитьПараметрыГраницы(НоваяСтрока,ДанныеГраницаКон.Значение);
		
КонецПроцедуры // ДобавитьГраницыИнтервала() 

Процедура ДобавитьПараметрыГраницы(НоваяСтрока,ДанныеГраницы)
	
	Если ТипЗнч(ДанныеГраницы)=Тип("Соответствие") Тогда
		
		Для Каждого КлючИЗначение ИЗ ДанныеГраницы Цикл
			
			НоваяСтрока.Значение=КлючИЗначение.Ключ;
			НоваяСтрока.Представление=КлючИЗначение.Значение;
			
		КонецЦикла;
		
	Иначе
			
		НоваяСтрока.Значение=ДанныеГраницы;
	
	КонецЕсли; 
	
	СтрСоответствия=мСоответствиеИзмеренийСпособовИТипов.Найти(НоваяСтрока.Значение,"СпособВычисления");
	
	Если Не СтрСоответствия=Неопределено Тогда
		
		НоваяСтрока.Тип=СтрСоответствия.ТипРасчета;
		
	Иначе
		
		НоваяСтрока.Тип="Простой";
		
	КонецЕсли;
		
КонецПроцедуры // ДобавитьПараметрыГраницы() 

Процедура ДобавитьСтрокиУточненияОпределения(Строка) 
	
	Если Строка.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ИнтервалПериодов Тогда
		
		ДобавитьГраницыИнтервала(Строка);
	
	ИначеЕсли ТипЗнч(Строка.УточнениеСпособаОпределения)=Тип("СписокЗначений") Тогда
...
Процедура СохранитьНастройкиОперанда() Экспорт
	...
	Если ИсточникШаблон=Справочники.ИсточникиДанныхДляРасчетов.ВводОстатков Тогда
	...
	Если ЗначениеЗаполнено(ИсточникШаблон) Тогда
		
		ТекстОтбора=ТекстОтбора+"%ОтборШаблон%";
	...
	Если ЗначениеЗаполнено(СоответствиеСчетов) И ТипЗнч(СоответствиеСчетов.СчетПриемник)=Тип("СправочникСсылка.СтрокиОтчетов") Тогда
	...
	ПредставлениеСопоставления	= УправлениеОтчетамиУХ.СформироватьПредставлениеСопоставления(ТаблицаСоответствияБД,ПрефиксАналитики);
	...
	УчитыватьКоррСчета=НЕ (ТаблицаСоответствияБД.Найти("КоррСчетОборот","ПолеБД")=Неопределено И  
		ТаблицаПараметровОтбораБД.Найти("КоррСчетОборот","ПолеБД")=Неопределено);
	...
	ИначеЕсли СпособПолучения = Перечисления.СпособыПолученияОперандов.ВнешниеДанныеРегистрБухгалтерии
		  ...
		  Если ТипЗнч(НазначениеРасчетов)=Тип("СправочникСсылка.ПравилаЗаполненияОбъектовБД") Тогда
			  ...
			  ПодготовитьТаблицуИспользованияПолейОбъектБД();
	...
	СформироватьПредставлениеРесурсов(); 
		
	Если УчитыватьКоррСчета Тогда
		
		ТекстОтбора=СтрЗаменить(ТекстОтбора,"КоррСчетОборот",?(ДтКт="Дт","СчетКт.Код","СчетДт.Код"));
		
	КонецЕсли;
	
	Если (СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеДокументы
			ИЛИ СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеДокументы)
		И ЗначениеЗаполнено(ОбъектБД) И (НЕ ПотребительРасчета.Метаданные().Имя="ШаблоныУниверсальныхПроцессов") Тогда
...
Процедура ПриЗаписи(Отказ)
	...
	Если СпособИспользования=Перечисления.СпособыИспользованияОперандов.ПроизвольныеДанные Тогда
		...
		ОчиститьПараметрическиеНастройкиПоШаблону();
		Возврат;
		
	КонецЕсли;
	
	Если ТипЗнч(ПотребительРасчета)=Тип("СправочникСсылка.СоответствияСчетовДляТрансляции") Тогда
		
		Если ТрансляцияДебет Тогда
			
			КорректировкиЗначенийПоказателей32.ОбработатьИзменениеНастройкиТрансляции(ПотребительРасчета,Истина);
	 ...
	 Для Каждого Строка ИЗ ТаблицаПараметровОтбораБД Цикл
		 ...
		 Если ЗначениеЗаполнено(Строка.ПолеБД) Тогда
			 ...
			 Если Строка.ПолеБД="ДатаНач" ИЛИ Строка.ПолеБД="ДатаКон" Тогда
				 ...
				 ПредставлениеДаты=ПолучитьПредставлениеДаты(Строка.ПолеБД);
				 
				 Если НЕ ПредставлениеДаты=Строка.ПолеБД Тогда
					 
					 СтрокаПараметр.НаименованиеБД=ПредставлениеДаты;
					 НоваяСтрока.НаименованиеБД=ПредставлениеДаты;
			 ...
			 Иначе	 
				 ...
				 Если СтрПоле=Неопределено Тогда
					 ...
					 мТабПравилаВычисленияПараметров.Удалить(СтрокаПараметр); 
					 
					 Если СтрокаРодитель=Неопределено Тогда
						 
						 ДеревоПараметровОтбораБД.Строки.Удалить(НоваяСтрока);
						 
					 Иначе
						 
						 СтрокаРодитель.Строки.Удалить(НоваяСтрока);
				 ...
				 Если НоваяСтрока.СпособВычисленияПараметра=Перечисления.СпособыВычисленияПараметровОперандов.ИнтервалПериодов Тогда
					 
					 УточнениеИнтервала=ПолучитьУточнениеГраницыИнтервала(Строка.ПолеБД,Строка.ИмяПараметра);
					 НоваяСтрока.УточнениеСпособаОпределения=УточнениеИнтервала;
					 СтрокаПараметр.УточнениеСпособаОпределения=УточнениеИнтервала;
...
Функция ПолучитьЭлементГраницы(СтрокаДанныеГраницы)
	
	СтруктураГраницы=Новый Структура;
	
	Если ЗначениеЗаполнено(СтрокаДанныеГраницы.Представление) Тогда
		
		ДанныеГраницы=Новый Соответствие;
		ДанныеГраницы.Вставить(СтрокаДанныеГраницы.Значение,СтрокаДанныеГраницы.Представление); 
		СтруктураГраницы.Вставить("Значение",ДанныеГраницы);
		СтруктураГраницы.Вставить("Представление",Строка(СтрокаДанныеГраницы.Значение)+" "+СтрокаДанныеГраницы.Представление);
		
	Иначе
		
		СтруктураГраницы.Вставить("Значение",СтрокаДанныеГраницы.Значение); 
		СтруктураГраницы.Вставить("Представление","");
		
	КонецЕсли;

	Возврат СтруктураГраницы;
	
КонецФункции // ПолучитьЭлементГраницы() 
 
Функция ПолучитьУточнениеГраницыИнтервала(ПолеБД,ИмяПараметра)
	
	УточнениеГраницы=Новый СписокЗначений;
	
	ДанныеГраницаНач=УточненияСпособовОпределения.Найти(ИмяПараметра+"_1");
	ДанныеГраницаКон=УточненияСпособовОпределения.Найти(ИмяПараметра+"_2");
	
	Если ДанныеГраницаНач=Неопределено ИЛИ ДанныеГраницаКон=Неопределено Тогда  
		
		Для Индекс=0 По УточненияСпособовОпределения.Количество()-1 Цикл
			
			СтрокаУточнения=УточненияСпособовОпределения[Индекс];
			
			Если СтрокаУточнения.ИмяПараметра=ИмяПараметра Тогда
				
				УточнениеГраницы.Добавить(СтрокаУточнения.Значение,СтрокаУточнения.Представление);
				
			КонецЕсли;
			
		КонецЦикла;				
		
		Возврат УточнениеГраницы;
		
	КонецЕсли;
	
	ЭлементСпискаНач=ПолучитьЭлементГраницы(ДанныеГраницаНач);
	ЭлементСпискаКон=ПолучитьЭлементГраницы(ДанныеГраницаКон);
	
	УточнениеГраницы.Вставить(0,ЭлементСпискаНач.Значение,ЭлементСпискаНач.Представление);
	УточнениеГраницы.Вставить(1,ЭлементСпискаКон.Значение,ЭлементСпискаКон.Представление);
	
	Возврат УточнениеГраницы;	 
	 
КонецФункции // ПолучитьУточнениеГраницыИнтервала() 
	...
	ПравилаИспользованияПолейЗапроса.Загрузить(ТекПравилаИспользованияПолей);
	 ...
	 Для Каждого СтрПоле ИЗ ПравилаИспользованияПолейЗапроса Цикл
		 ...
		 Если ЗначениеЗаполнено(СтрПоле.Поле) Тогда
			 ...
			 Если СпособИспользования=Перечисления.СпособыИспользованияОперандов.ДляЗаполненияОбъектовБД
				 И СтрНайти(СтрПоле.Синоним,"_РесурсРегистра")>0 Тогда
				 
				 СтрокаНастройки=ОпределитьРабочуюСтроку("ПолеБД",СтрЗаменить(СтрПоле.Синоним,"_РесурсРегистра",""));
			 ...
			 Иначе
				 ...
				 Иначе
					 ...
					 СтрокаСоответствия.ПсевдонимБД=СтрокаНастройки.ПредставлениеПоля;
...
Функция ОпределитьРабочуюСтроку(ИмяПоля,ЗначениеПоля) Экспорт
	...
	Если СтрПоле=Неопределено Тогда
		...
		Если СтрНайти(ЗначениеПоля,".")=0
			И СтрНайти(ЗначениеПоля,"@")=0 Тогда 
...
Функция ПолучитьРазыменованноеПоле(КоллекцияСтрок,ИмяПоля,ЗначениеПоля,ЧислоШагов)
	...
	РазделительПути=?(СтрНайти(ЗначениеПоля,"@")>0,"@",".");
	
	МассивПутей=ОбщегоНазначенияУХ.РазложитьСтрокуВМассивПодстрок(ЗначениеПоля,РазделительПути);
	...
	Для Индекс=1 ПО ЧислоШагов Цикл
		...
		ТекущийПуть=ТекущийПуть+РазделительПути+МассивПутей[Индекс-1];
	...
	Если СтрокаРодитель.Строки.Количество()=0 
		ИЛИ (СтрокаРодитель.Строки.Количество()=1 И ПустаяСтрока(СтрокаРодитель.Строки[0].ПолеБД)) Тогда
		
		СтруктураСтрока=СформироватьСтруктуруСтрокиБД(СтрокаРодитель);
		ДанныеОбъекта=СформироватьДанныеОбъекта(Ложь);
		
		РаботаСПолямиАналитикиУХ.ПередРазворачиваниемСтрокиРеквизитовАналитики(ДанныеОбъекта,СтруктураСтрока);
		
		ВосстановитьПеременныеДляРасчета(ДанныеОбъекта.РабочийОбъект);
...
Функция ПолучитьРазыменованноеПолеРекурсивно(СтрокаРодитель,ИмяПоля,ЗначениеПоля,ЧислоШагов)
	...
	Если ЧислоШагов > МассивПутейИсх.Количество() Тогда
	...
	Для Индекс=1 ПО ЧислоШагов Цикл
		...
		Если Индекс<=МассивПутейТек.Количество() Тогда 
...
Функция СформироватьДанныеОбъекта(ПомещатьВоВременноеХранилище=Истина) Экспорт
	...
	Иначе
		...
		ДанныеОбъекта.Вставить("РабочийОбъект",	ПодготовитьСтруктуруПеременныхДляРасчета());
	...
	Если ДанныеОбъекта.ВнутренниеДанные И (ТрансляцияДебет 
		ИЛИ ТрансляцияКредит
		ИЛИ ЗначениеЗаполнено(СоответствиеСчетов)) Тогда
		
		ДанныеОбъекта.Вставить("ИспользоватьДополнительныеРеквизиты",Истина);
...
Функция ПодготовитьСтруктуруПеременныхДляРасчета() Экспорт
	...
	СтруктураПеременных.Вставить("КэшВидовСубконто",КэшВидовСубконто);
	СтруктураПеременных.Вставить("ВидыСубконтоДляПодбора",ВидыСубконтоДляПодбора);
...
Процедура ОбработатьПредзаполненныеРеквизиты() Экспорт
	...
	ИначеЕсли СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеПроизвольныйЗапрос
		ИЛИ СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеПроизвольныйЗапрос Тогда
		
		ЗаполнитьПоИсточникуПроизвольныйЗапрос();
...
Процедура ЗаполнитьПоШаблонуИсточнику() Экспорт
	...
	Если ИсточникШаблон=Справочники.ИсточникиДанныхДляРасчетов.ВводОстатков Тогда
		...
		ТипЗначения=Перечисления.ТипыЗначенийПоказателейОтчетов.Число;
		Наименование=НСтр("ru = 'Ввод остатков (служебный)'");
		Код="ВводОстатков";
		Возврат;
...
Функция ПодобратьТаблицуСоответствийДляПоля(Поле,СтрокаТаблицы)
	
	мТаблицаПоказателейБД.Сортировать("Измерение Убыв");
	
	Если СтрНайти(Поле.Поле,"Версия.")>0 Тогда
		
		ПолеДляПоиска=СтрЗаменить(Поле.Поле,"Версия.","");
		
	ИначеЕсли Поле.Поле="АналитикаВалюта" Тогда
		
		ПолеДляПоиска="Валюта";
		
	Иначе
		
		ПолеДляПоиска=Поле.Поле;
		
	КонецЕсли; 
	
	ТекТаблицаПолей=Новый ТаблицаЗначений;
	ТекТаблицаПолей.Колонки.Добавить("ПолеБД");
	ТекТаблицаПолей.Колонки.Добавить("ПсевдонимБД");
	ТекТаблицаПолей.Колонки.Добавить("КоличествоТипов");
	
	Если ЗначениеЗаполнено(Поле.ВидАналитики) Тогда // Ищем по виду субконто
		
		МассивПоказатели=мТаблицаПоказателейБД.НайтиСтроки(Новый Структура("ВидАналитики",Поле.ВидАналитики));
		
		Если МассивПоказатели.Количество()>0 Тогда
			
			Для Каждого СтрПоказатель ИЗ МассивПоказатели Цикл
				
				Если (СтрПоказатель.Балансовый ИЛИ (НЕ Поле.АналитикаОтчета)) Тогда 
					
					Если СтрПоказатель.ПолеБД=ПолеДляПоиска Тогда
						
						ТекТаблицаПолей.Очистить();
						НоваяСтрока=ТекТаблицаПолей.Добавить();
						НоваяСтрока.ПолеБД=СтрПоказатель.ПолеБД;
						НоваяСтрока.ПсевдонимБД=СтрПоказатель.ПредставлениеПоля;
						Возврат ТекТаблицаПолей;
		...
		Для Каждого СтрПоказатель ИЗ мТаблицаПоказателейБД Цикл
			...
			Если НЕ СтрПоказатель.ТипДанныхПоля.НайтиПоЗначению(ИскомыйТип)=Неопределено Тогда
				...
				Иначе
					...
					НоваяСтрока.ПолеБД			= СтрПоказатель.ПолеБД;
					НоваяСтрока.ПсевдонимБД		= СтрПоказатель.ПредставлениеПоля;
					НоваяСтрока.КоличествоТипов = СтрПоказатель.ТипДанныхПоля.Количество();
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы.ВидАналитики) Тогда
		...
		МассивШаблон=ТаблицаСоответствияБДШаблон.НайтиСтроки(Новый Структура("ВидАналитики",СтрокаТаблицы.ВидАналитики));
		
		Если МассивШаблон.Количество()>0 Тогда
			
			Для Каждого СтрокаШаблон ИЗ МассивШаблон Цикл
				
				ДанныеПоля=СтрРазделить(СтрокаШаблон.ПолеБД,".");
				ИмяПоля=ДанныеПоля[ДанныеПоля.Количество()-1];
				
				Если ИмяПоля=ПолеДляПоиска Тогда 
					
					ТекТаблицаПолей.Очистить();
					НоваяСтрока=ТекТаблицаПолей.Добавить();
					НоваяСтрока.ПолеБД=СтрокаШаблон.ПолеБД;
					НоваяСтрока.ПсевдонимБД=СтрокаШаблон.ПсевдонимБД;
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы.ТаблицаАналитики) Тогда
		...
		МассивШаблон=ТаблицаСоответствияБДШаблон.НайтиСтроки(Новый Структура("ТаблицаАналитики",СтрокаТаблицы.ТаблицаАналитики));
...
КонецФункции // ПодобратьТаблицуСоответствийДляПоля()

Процедура ПолучитьТаблицуСубконто()
	...
	ВидыСубконтоДляПодбора= Запрос.Выполнить().Выгрузить();	
	
КонецПроцедуры // ПолучитьТаблицуСубконто() 

Функция ПолучитьВидСубконтоПоТипуПоля(ТипЗначения)

	Если КэшВидовСубконто=Неопределено Тогда
		
		КэшВидовСубконто=Новый Соответствие;
		
	КонецЕсли; 
	
	ВидСубконтоСсылка= КэшВидовСубконто.Получить(ТипЗначения);
	...
	Если ВидыСубконтоДляПодбора=Неопределено Тогда
		
		ПолучитьТаблицуСубконто();
		
	КонецЕсли;
	
	Для Индекс=0 ПО ВидыСубконтоДляПодбора.Количество()-1 Цикл
		
		ВидСубконтоСсылка=ВидыСубконтоДляПодбора[Индекс];
				
		Если ВидСубконтоСсылка.ТипЗначения.Типы().Количество()=1
			И ВидСубконтоСсылка.ТипЗначения.СодержитТип(ТипЗначения) Тогда
			
			КэшВидовСубконто.Вставить(ТипЗначения,ВидСубконтоСсылка.Ссылка);
...
КонецФункции // ПолучитьВидСубконтоПоВидуПоля() 

Процедура СформироватьПредставлениеРесурсов() 
	
	Если (НЕ СпособИспользования=Перечисления.СпособыИспользованияОперандов.ПроизвольныеДанные)
		ИЛИ ЗначениеЗаполнено(ХозяйственнаяОперация)
		ИЛИ ЗначениеЗаполнено(ОбъектУчета) Тогда
		
		Возврат;
		
	КонецЕсли;
	
	Если СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеОстаткиИОборотыИзПланаСчетов
		ИЛИ СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнешниеДанныеРегистрБухгалтерии
		ИЛИ СпособПолучения=Перечисления.СпособыПолученияОперандов.ВнутренниеДанныеРегистрБухгалтерии Тогда
		
		СоответствиеРесурсовДляТрансляции.Очистить();
		
	КонецЕсли;
	
	Если СоответствиеРесурсовДляТрансляции.Количество()=0 Тогда
		
		НоваяСтрока=СоответствиеРесурсовДляТрансляции.Добавить();
		НоваяСтрока.РесурсИсточник=РесурсРегистра;
	...
	Для Каждого Строка ИЗ СоответствиеРесурсовДляТрансляции Цикл
		
		Если Строка.Коэффициент=1 Тогда
			
			ПрефиксРесурса="";
			
		ИначеЕсли Строка.Коэффициент=-1 Тогда
			
			ПрефиксРесурса="-";
			
		Иначе
			
			ПрефиксРесурса="("+Строка.Коэффициент+")*";
			
		КонецЕсли;	
		
		Если ЗначениеЗаполнено(Строка.ДтКт) Тогда
			
			ТекстВидИтога=СтрЗаменить(Строка.ВидИтога,"#",ДтКт);
			
		Иначе
			
			ТекстВидИтога=Строка.ВидИтога;
			
		КонецЕсли;	
		
		Строка.ПредставлениеИсточник=ПрефиксРесурса+Строка.РесурсИсточник+ТекстВидИтога;
		
	КонецЦикла;
	
КонецПроцедуры // СформироватьПредставлениеРесурсов()

Процедура ОчиститьПараметрическиеНастройкиПоШаблону()
	...
	|	Справочник.ИсточникиДанныхДляРасчетов КАК ИсточникиДанныхДляРасчетов
	|ГДЕ
	|	ИсточникиДанныхДляРасчетов.ИсточникШаблон = &ИсточникШаблон";
	
	Запрос.УстановитьПараметр("ИсточникШаблон",Ссылка);
...
КонецПроцедуры // ОчиститьПараметрическиеНастройкиПоШаблону() 

Процедура ЗаполнитьПоИсточникуПроизвольныйЗапрос()
...
Функция ПолучитьСинонимДляФормулРегистр()
	
	ПолеСиноним="Синоним"+?(ДанныеДляФормул.НомерЯзыка=0,"",ДанныеДляФормул.НомерЯзыка);
	
	ДанныеСиноним=ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РегистрБД,"Наименование,"+ПолеСиноним);
	
	Если Не ЗначениеЗаполнено(ДанныеСиноним[ПолеСиноним]) Тогда
		
		Возврат ДанныеСиноним.Наименование;
		
	Иначе
		
		Возврат ДанныеСиноним[ПолеСиноним];
		
	КонецЕсли;
	
КонецФункции // ПолучитьСинонимДляФормулРегистр()  

Функция ПолучитьСинонимРесурса(ИмяРесурса="")
	...
	Если ДанныеДляФормул.НомерЯзыка=0 Тогда
	...
	Иначе
		...
		|	РегистрыНакопленияБДРесурсы.Синоним1 КАК Синоним1,
		|	РегистрыНакопленияБДРесурсы.Синоним2 КАК Синоним2
		|ИЗ
		|	Справочник.РегистрыНакопленияБД.Ресурсы КАК РегистрыНакопленияБДРесурсы
		|ГДЕ
		|	РегистрыНакопленияБДРесурсы.Имя = &РесурсРегистра
		|	И РегистрыНакопленияБДРесурсы.Ссылка = &РегистрБД";
		
		Запрос.Текст=СтрЗаменить(Запрос.Текст,"Справочник.РегистрыНакопленияБД",РегистрБД.Метаданные().ПолноеИмя());
		Запрос.УстановитьПараметр("РесурсРегистра",ИмяРесурса);
		Запрос.УстановитьПараметр("РегистрБД",РегистрБД);
		
		СтрРесурс=Запрос.Выполнить().Выгрузить()[0];
		
		ПолеСиноним="Синоним"+?(ДанныеДляФормул.НомерЯзыка=0,"",ДанныеДляФормул.НомерЯзыка);
		
		Если ЗначениеЗаполнено(СтрРесурс[ПолеСиноним]) Тогда
			
			Возврат СтрРесурс[ПолеСиноним];
			
		Иначе
			
			Возврат РесурсРегистра;
			
		КонецЕсли;
		
	КонецЕсли;	
	
КонецФункции // ПолучитьСинонимРесурса() 

Функция ПолучитьСинонимВидаИтога()
	
	СписокСиноним=Новый СписокЗначений;
	СписокСиноним.Добавить("НачальныйОстаток", Нстр("ru = 'Начальный Остаток'; en = 'Opening balance';tr = 'Açılış bakiyesi'",ДанныеДляФормул.КодЯзыка));
	СписокСиноним.Добавить("КонечныйОстаток", Нстр("ru = 'Конечный Остаток'; en = 'Closing balance';tr = 'Kapanış bakiyesi'",ДанныеДляФормул.КодЯзыка));	
	СписокСиноним.Добавить("Приход", НСтр("ru = 'Приход'; en = 'Income';tr = 'Giriş'",ДанныеДляФормул.КодЯзыка));
	СписокСиноним.Добавить("Расход", НСтр("ru = 'Расход'; en = 'Expense';tr = 'Çıkış'",ДанныеДляФормул.КодЯзыка));	
	СписокСиноним.Добавить("Оборот", НСтр("ru = 'Оборот'; en = 'Turnover';tr = 'Ciro'",ДанныеДляФормул.КодЯзыка));		
	СписокСиноним.Добавить("СН#",Нстр("ru = 'Сальдо начальное'; en = 'Opening balance';tr = 'Açılış bakiyesi'",ДанныеДляФормул.КодЯзыка));	
	СписокСиноним.Добавить("СН#РС",Нстр("ru = 'Сальдо начальное (развернутое по субконто)'; en = 'Opening balance (broken down by extra dimensions)';tr = 'Başlangıç bakiyesi (ekstra boyuta göre detaylandırılan)'",ДанныеДляФормул.КодЯзыка));	
	СписокСиноним.Добавить("СК#",Нстр("ru = 'Сальдо конечное'; en = 'Closing balance';tr = 'Kapanış bakiyesi'",ДанныеДляФормул.КодЯзыка));	
	СписокСиноним.Добавить("СК#РС",Нстр("ru = 'Сальдо конечное (развернутое по субконто)'; en = 'Closing balance (broken down by extra dimensions)';tr = 'Kapanış bakiyesi (ekstra boyuta göre detaylandırılan)'",ДанныеДляФормул.КодЯзыка));		
	СписокСиноним.Добавить("#О",Нстр("ru = 'Оборот по счету'; en = 'Account turnover ';tr = 'Hesap cirosu'",ДанныеДляФормул.КодЯзыка));
	СписокСиноним.Добавить("КорДО",Нстр("ru = 'Оборот в корреспонденции'; en = 'Correspondent turnover';tr = 'Muhaberat devri'",ДанныеДляФормул.КодЯзыка));
	СписокСиноним.Добавить("НачальныйОстаток#",Нстр("ru = 'Сальдо начальное'; en = 'Opening balance';tr = 'Açılış bakiyesi'",ДанныеДляФормул.КодЯзыка));
	СписокСиноним.Добавить("НачальныйРазвернутыйОстаток#",Нстр("ru = 'Сальдо начальное (развернутое по субконто)'; en = 'Opening balance (broken down by extra dimensions)';tr = 'Başlangıç bakiyesi (ekstra boyuta göre detaylandırılan)'",ДанныеДляФормул.КодЯзыка));	
	СписокСиноним.Добавить("КонечныйОстаток#",Нстр("ru = 'Сальдо конечное'; en = 'Closing balance';tr = 'Kapanış bakiyesi'",ДанныеДляФормул.КодЯзыка));
	СписокСиноним.Добавить("КонечныйРазвернутыйОстаток#",Нстр("ru = 'Сальдо конечное (развернутое по субконто)'; en = 'Closing balance (broken down by extra dimensions)';tr = 'Kapanış bakiyesi (ekstra boyuta göre detaylandırılan)'",ДанныеДляФормул.КодЯзыка));		
	СписокСиноним.Добавить("Оборот#",Нстр("ru = 'Оборот по счету'; en = 'Account turnover ';tr = 'Hesap cirosu'",ДанныеДляФормул.КодЯзыка));
	СписокСиноним.Добавить("Оборот",Нстр("ru = 'Оборот в корреспонденции'; en = 'Correspondent turnover';tr = 'Muhaberat devri'",ДанныеДляФормул.КодЯзыка));	
	
	ЭлементСиноним=СписокСиноним.НайтиПоЗначению(ВидИтога);
	...
	Иначе
		...
		Возврат ЭлементСиноним.Представление; 
		
	КонецЕсли;
	
КонецФункции // ПолучитьСинонимВидаИтога()

Функция ПолучитьСинонимВИБ()
	
    Возврат НСтр("ru = 'ВИБ_'; en = 'ExtDB_'",ДанныеДляФормул.КодЯзыка);
	
КонецФункции // ПолучитьСинонимВИБ() 

Функция ПолучитьСинонимРН()
	
    Возврат НСтр("ru = 'РН_'; en = 'AR_'",ДанныеДляФормул.КодЯзыка);
	
КонецФункции // ПолучитьСинонимВИБ()

Функция ПолучитьСинонимРС()
	
    Возврат НСтр("ru = 'РС_'; en = 'IR_'",ДанныеДляФормул.КодЯзыка);
	
КонецФункции // ПолучитьСинонимВИБ()

Функция ПолучитьСинонимРБ()
	
    Возврат НСтр("ru = 'РБ_'; en = 'CA_'",ДанныеДляФормул.КодЯзыка);
	
КонецФункции // ПолучитьСинонимВИБ()

Функция ПолучитьСинонимДтКт()
	
	СписокСиноним=Новый СписокЗначений;				
	СписокСиноним.Добавить("Дт",Нстр("ru = 'Дт'; en = 'Dr';tr = 'Borç'",ДанныеДляФормул.КодЯзыка));
	СписокСиноним.Добавить("Кт",Нстр("ru = 'Кт'; en = 'Cr';tr = 'Alacak'",ДанныеДляФормул.КодЯзыка));
	СписокСиноним.Добавить("СВ",Нстр("ru = 'Дт - Кт'; en = 'Dr - Cr';tr = 'Borç - Alacak'",ДанныеДляФормул.КодЯзыка));
...
КонецФункции // ПолучитьСинонимДтКт()
