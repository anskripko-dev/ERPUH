...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МестоПрименения = Параметры.МестоПрименения;
...
Процедура Далее(Команда)
	...
	Если Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.ГруппаСтраницы.ПодчиненныеЭлементы.СтраницаМестоПрименения
		И ПроверитьЗаполнение() Тогда
		
		ДанныеМестаПрименения = ИнтеграцияСАТУРНКлиентСервер.СтруктураДанныхМестаПрименения();
		
		ДанныеМестаПрименения.Наименование       = Наименование;
		ДанныеМестаПрименения.Ссылка             = МестоПрименения;
		ДанныеМестаПрименения.Идентификатор      = Идентификатор;
		ДанныеМестаПрименения.Широта             = Формат(Широта, "ЧГ=;");
		ДанныеМестаПрименения.Долгота            = Формат(Долгота, "ЧГ=;");
		ДанныеМестаПрименения.ОКТМО              = ОКТМО;
		ДанныеМестаПрименения.ВидОтношений       = ВидОтношений;
		ДанныеМестаПрименения.Адрес              = АдресСтрокой;
		ДанныеМестаПрименения.ТипМестаПрименения = ТипМестаПрименения;
		
		ПараметрыОбработкиКлассификаторов = ИнтеграцияСАТУРНСлужебныйКлиентСервер.ПараметрыПередачиДанныхКлассификаторов();
		ПараметрыОбработкиКлассификаторов.Ссылка             = МестоПрименения;
		...
		ПараметрыОбработкиКлассификаторов.ДополнительныеПараметры.Вставить("ДанныеМестаПрименения", ДанныеМестаПрименения);
...
Процедура Назад(Команда)
	...
	Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаМестоПрименения;
	УправлениеЭлементамиФормы(ЭтотОбъект);
...
Процедура Подключаемый_ПриЗавершенииОперации(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого ЭлементОбработки Из Результат Цикл
		...
		Если ЭлементОбработки.Объект.Количество() Тогда
			...
			МестоПрименения = ЭлементОбработки.Объект[0];
...
Процедура ВывестиИнформациюОУспешномРезультатеОбмена()
	...
	Строки.Добавить(
		Новый ФорматированнаяСтрока(
			СтрШаблон(НСтр("ru = 'На запрос о %1 месте применения %2 получен ответ.';
							|en = 'На запрос о %1 месте применения %2 получен ответ.'"), СтрокаТипОперации("Получение"), Наименование)));
			
	Строки.Добавить(" ");
	Строки.Добавить(
		Новый ФорматированнаяСтрока(
			СтрШаблон(НСтр("ru = '%1 место применения';
							|en = '%1 место применения'"), СтрокаТипОперации("Завершено"))));
	Строки.Добавить(" ");
	Строки.Добавить(Новый ФорматированнаяСтрока(Строка(МестоПрименения),, ЦветГиперссылки,, "ОткрытьРезультат"));
	Строки.Добавить(".");
...
Процедура ОповеститьОбЗаписиОбъектов()
	...
	ПараметрОповещения = МестоПрименения;
	ОбщегоНазначенияКлиент.ОповеститьОбИзмененииОбъекта(МестоПрименения, ПараметрОповещения);
...
Процедура МестоХраненияПриИзмененииНаСервере()
	...
	|ГДЕ
	|	МестаПримененияСАТУРН.Ссылка = &Ссылка
	|");
	Запрос.УстановитьПараметр("Ссылка", МестоПрименения);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		
		РезультатВыполненияЗапроса = ИнтерфейсСАТУРНВызовСервера.МестоПримененияПоИдентификатору(Выборка.Идентификатор);
...
Процедура УправлениеЭлементамиФормы(Форма)
	...
	Если Форма.РежимИзменения Тогда
		...
		Форма.Заголовок = НСтр("ru = 'Изменение места применения';
								|en = 'Изменение места применения'");
		
	Иначе
		
		Форма.Заголовок = НСтр("ru = 'Создание места применения';
								|en = 'Создание места применения'");
...
Процедура УправлениеЭлементамиНавигацииПомощника(Форма)
	...
	Если Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаМестоПрименения Тогда
		...
		Иначе
			...
			Элементы.МестоПрименения.Видимость = Форма.РежимИзменения;
