...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РазместитьЭлементыОповещенияУХ();
	//#КонецОбласти
	//	
	//#Область УХ_Встраивание
	РеквизитОсновнойЯзык = Новый РеквизитФормы("ОсновнойЯзыкУХ", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(1, 0, ДопустимыйЗнак.Неотрицательный)), , "Язык справочников и отчетности", Истина);
	ЭтаФорма.ИзменитьРеквизиты(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(РеквизитОсновнойЯзык));
	ГруппаФормы = ЭтаФорма.Элементы.Найти("СвойстваДополнительные");
	Если ГруппаФормы <> Неопределено Тогда
		Если ЭтаФорма.Элементы.Найти("ОсновнойЯзыкУХ") = Неопределено Тогда
				ЭлементФормы = ЭтаФорма.Элементы.Добавить("ОсновнойЯзыкУХ", Тип("ПолеФормы"), ГруппаФормы);        
				ЭлементФормы.Вид = ВидПоляФормы.ПолеВвода;
				ЭлементФормы.ПутьКДанным = "ОсновнойЯзыкУХ";
				ЭлементФормы.РежимВыбораИзСписка = Истина;
				ЭлементФормы.Ширина = 20;
			КонецЕсли;   
			
		КонецЕсли;
	ДополнительныеЯзыкиУХ.ПриСозданииНаСервере(ЭтотОбъект, Отказ, СтандартнаяОбработка);
	#КонецОбласти
КонецПроцедуры
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	КонецЕсли;
	#Область УХ_Встраивание
	ДополнительныеЯзыкиУХ.ПриЗаписиНаСервере(ЭтотОбъект, Отказ, ТекущийОбъект, ПараметрыЗаписи);
	#КонецОбласти
КонецПроцедуры
...
// Размещает на форме элементы для оповещения.
&НаСервере
Процедура РазместитьЭлементыОповещенияУХ()
		
	СтраницаОповещенияИНапоминания = ДобавитьГруппуЭлементов("СтраницаОповещенияИНапоминанияУХ", Элементы.Страницы, НСтр("ru = 'Оповещения и напоминания'"), Элементы.СтраницаКомментарий, ВидГруппыФормы.Страница,, Истина);
		
	// Группа E-mail.
	ГруппаEMail = ДобавитьГруппуЭлементов("ГруппаEMail", СтраницаОповещенияИНапоминания, НСтр("ru = 'e-mail'"), Неопределено, ВидГруппыФормы.ОбычнаяГруппа, ОтображениеОбычнойГруппы.Нет, Ложь);
	// Флажок Оповещать по E-mail. 
	ДобавитьЭлементФлажок("ОповещатьПоEmailУХ", ГруппаEMail, НСтр("ru = 'Оповещать по e-mail'"), 
		"Объект.ОповещатьПоEmail", Неопределено, 
		НСтр("ru = 'Установка флага означает, что системные оповещения будут отправляться  на  почтовый ящик пользователя'"));
	// Декорация Требуется настройка.
	ДобавитьЭлементДекорация("ДекорацияEMailУХ", ГруппаEMail, НСтр("ru = 'Требуется настройка'"), ВидДекорацииФормы.Надпись, Неопределено);
	// Команда ОткрытьСистемнуюУчетнуюЗаписьУХ. 
	ЭтаФорма.Команды.Добавить("ОткрытьСистемнуюУчетнуюЗаписьУХ");
	ЭтаФорма.Команды["ОткрытьСистемнуюУчетнуюЗаписьУХ"].Действие = "НажатиеОткрытьСистемнуюУчетнуюЗаписьУХ_Подключаемый";
	// Гипер-ссылка "системной учетной записи".
	ДобавитьЭлементКнопка("ОткрытьСистемнуюУчетнуюЗаписьУХ", ГруппаEMail, НСтр("ru = 'системной учетной записи'"), 
		ВидКнопкиФормы.Гиперссылка, "ОткрытьСистемнуюУчетнуюЗаписьУХ", Неопределено);
			
	// Группа система взаимодействия.
	ГруппаСистемаВзаимодействия = ДобавитьГруппуЭлементов("ГруппаСистемаВзаимодействия", СтраницаОповещенияИНапоминания, НСтр("ru = 'Система взаимодействия'"), Неопределено, ВидГруппыФормы.ОбычнаяГруппа, ОтображениеОбычнойГруппы.Нет, Ложь);
	// Флажок Оповещать через систему взаимодействия. 
	ДобавитьЭлементФлажок("ОповещатьЧерезСистемуВзаимодействияУХ", ГруппаСистемаВзаимодействия, НСтр("ru = 'Оповещать через систему взаимодействия'"), 
		"Объект.ОповещатьЧерезСистемуВзаимодействия", Неопределено, 
		НСтр("ru = 'Установка флага означает, что оповещения будут отправляться через систему взаимодействия'"));
	// Декорация Требуется настройка.
	ДобавитьЭлементДекорация("ДекорацияСистемаВзаимодействияУХ", ГруппаСистемаВзаимодействия, НСтр("ru = 'Требуется настройка'"), ВидДекорацииФормы.Надпись, Неопределено);
	// Команда НастроитьСистемуВзаимодействия. 
	ЭтаФорма.Команды.Добавить("НастроитьСистемуВзаимодействияУХ");
	ЭтаФорма.Команды["НастроитьСистемуВзаимодействияУХ"].Действие = "НажатиеНастроитьСистемуВзаимодействия_Подключаемый";
	// Гипер-ссылка "системы взаимодействия".
	ДобавитьЭлементКнопка("НастроитьСистемуВзаимодействияУХ", ГруппаСистемаВзаимодействия, НСтр("ru = 'системы взаимодействия'"), 
		ВидКнопкиФормы.Гиперссылка, "НастроитьСистемуВзаимодействияУХ", Неопределено);
	ГруппаСистемаВзаимодействия.Видимость = ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.ИнтеграцияССистемойВзаимодействия");
	
	// Группа Телеграм.
	ГруппаТелеграм = ДобавитьГруппуЭлементов("ГруппаТелеграм", СтраницаОповещенияИНапоминания, НСтр("ru = 'Телеграм'"), Неопределено, ВидГруппыФормы.ОбычнаяГруппа, ОтображениеОбычнойГруппы.Нет, Ложь);
	// Флажок Оповещать через Телеграм. 
	ДобавитьЭлементФлажок("ОповещатьЧерезТелеграмУХ", ГруппаТелеграм, НСтр("ru = 'Оповещать через Telegram'"), 
		"Объект.ОповещатьЧерезТелеграм", Неопределено, 
		НСтр("ru = 'Установка флага означает, что системные оповещения будут отправляться через Telegram'"));
	// Декорация Требуется настройка.
	ДобавитьЭлементДекорация("ДекорацияТелеграмУХ", ГруппаТелеграм, НСтр("ru = 'Требуется настройка'"), ВидДекорацииФормы.Надпись, Неопределено);
	// Команда ОткрытьУчетнуюЗаписьТелеграмУХ. 
	ЭтаФорма.Команды.Добавить("ОткрытьУчетнуюЗаписьТелеграмУХ");
	ЭтаФорма.Команды["ОткрытьУчетнуюЗаписьТелеграмУХ"].Действие = "НажатиеОткрытьУчетнуюЗаписьТелеграм_Подключаемый";
	// Гипер-ссылка "учетной записи Telegram".
	ДобавитьЭлементКнопка("ОткрытьУчетнуюЗаписьТелеграмУХ", ГруппаТелеграм, НСтр("ru = 'учетной записи Telegram'"), 
		ВидКнопкиФормы.Гиперссылка, "ОткрытьУчетнуюЗаписьТелеграмУХ", Неопределено);
	
КонецПроцедуры		// РазместитьЭлементыОповещенияУХ()

&НаСервере
Функция ДобавитьЭлементДекорация(НаименованиеЭлементаВход, ГруппаРодительВход, ЗаголовокВход, ВидДекорацииВход, МестоположениеВход)
	НовыйЭлемент = Элементы.Вставить(НаименованиеЭлементаВход, Тип("ДекорацияФормы"), ГруппаРодительВход);
	НовыйЭлемент.Вид		 = ВидДекорацииВход;
	НовыйЭлемент.Заголовок	 = ЗаголовокВход;
	НовыйЭлемент.ЦветТекста  = ЦветаСтиля.ПоясняющийТекст;
	Элементы.Переместить(НовыйЭлемент, ГруппаРодительВход, МестоположениеВход);
	Возврат НовыйЭлемент;
КонецФункции

&НаСервере
Функция ДобавитьЭлементКнопка(НаименованиеЭлементаВход, ГруппаРодительВход, ЗаголовокВход, ВидКнопкиВход, ИмяКомандыВход, МестоположениеВход)
	НовыйЭлемент = Элементы.Добавить(НаименованиеЭлементаВход, Тип("КнопкаФормы"), ГруппаРодительВход);
	НовыйЭлемент.ИмяКоманды	 = ИмяКомандыВход;
	НовыйЭлемент.Заголовок	 = ЗаголовокВход;
	НовыйЭлемент.Вид		 = ВидКнопкиВход;
	Элементы.Переместить(НовыйЭлемент, ГруппаРодительВход, МестоположениеВход);
	Возврат НовыйЭлемент;
КонецФункции

// Размещает на форме в группе ГруппаРодительВход элемент с наименованием НаименованиеЭлементаВход,
// заголовком ЗаголовокВход. Подключает его к реквизиту ПутьКДаннымВход. Размещает перед элементом
// МестоположениеВход. Добавляет подсказку ПодсказкаВход. Возвращает новый элемент формы.
&НаСервере
Функция ДобавитьЭлементФлажок(НаименованиеЭлементаВход, ГруппаРодительВход, ЗаголовокВход, ПутьКДаннымВход, МестоположениеВход, ПодсказкаВход = "")
	ТипЭлемента = Тип("ПолеФормы");
	НовыйЭлемент = Элементы.Вставить(НаименованиеЭлементаВход, ТипЭлемента, ГруппаРодительВход);
	НовыйЭлемент.ПутьКДанным			 = ПутьКДаннымВход;
	НовыйЭлемент.Заголовок				 = ЗаголовокВход;
	...
	Элементы.Переместить(НовыйЭлемент, ГруппаРодительВход, МестоположениеВход);
	Возврат НовыйЭлемент;
КонецФункции		// ДобавитьЭлементФлажок()

// Добавляет группу элементов формы в родительскую группу ГруппаРодительВход с наименованеим
// НаименованиеРеквизитаВход и заголовком ЗаголовокВход. Размещает перед элементом
// МестоположениеВход.
&НаСервере
Функция ДобавитьГруппуЭлементов(НаименованиеРеквизитаВход, ГруппаРодительВход, ЗаголовокВход, МестоположениеВход, ВидГруппы, ОтображениеГруппыОбычная = Неопределено, ОтображатьЗаголовокВход = Ложь)
	ТипЭлемента = Тип("ГруппаФормы");
	НовыйЭлемент									 = Элементы.Вставить(НаименованиеРеквизитаВход, ТипЭлемента, ГруппаРодительВход);
	НовыйЭлемент.Вид								 = ВидГруппы;	
	НовыйЭлемент.ОтображатьЗаголовок				 = ОтображатьЗаголовокВход;
	НовыйЭлемент.Заголовок							 = ЗаголовокВход;
	НовыйЭлемент.ГоризонтальноеПоложениеПодчиненных	 = ГоризонтальноеПоложениеЭлемента.Лево;	
	
	Если ВидГруппы = ВидГруппыФормы.ОбычнаяГруппа Тогда
		Если ОтображениеГруппыОбычная = Неопределено тогда
			НовыйЭлемент.Отображение					= ОтображениеОбычнойГруппы.Нет;
		Иначе
			НовыйЭлемент.Отображение					= ОтображениеГруппыОбычная;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура НажатиеОткрытьСистемнуюУчетнуюЗаписьУХ_Подключаемый(Элемент, СтандартнаяОбработка)
	ПоказатьЗначение(, ПредопределенноеЗначение("Справочник.УчетныеЗаписиЭлектроннойПочты.СистемнаяУчетнаяЗаписьЭлектроннойПочты"));	
КонецПроцедуры		// НажатиеОткрытьСистемнуюУчетнуюЗаписьУХ_Подключаемый()

&НаКлиенте
Процедура НажатиеОткрытьУчетнуюЗаписьТелеграм_Подключаемый(Команда)
	
	Если Модифицированность Тогда
				
		ОповещениеОЗаписи = Новый ОписаниеОповещения("ПриОтветеНаВопросОСохраненииИзмененныхДанныхТелеграм", ЭтотОбъект);
		ТекстВопроса = НСтр("ru = 'Для настройки учетной записи Telegram Пользователь должен быть записан. Сохранить изменения?'");	
		ПоказатьВопрос(ОповещениеОЗаписи, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Возврат;
		
	КонецЕсли;
	
	ОткрытьФормуУчетнойЗаписиТелеграм();
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьФормуУчетнойЗаписиТелеграм()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда				// не БМ, НМ
		...
		СтруктураНастроекПользователя = МодульТелеграмСерверУХ.ПолучитьНастройкиПользователя(Объект.Ссылка);
				
		Если СтруктураНастроекПользователя <> Неопределено тогда
			
			СтруктураЗаписи = Новый Структура("ТелеграмБот, Пользователь", СтруктураНастроекПользователя.ТелеграмБот, Объект.Ссылка);
			МассивСтруктураЗаписи = Новый Массив;
			МассивСтруктураЗаписи.Добавить(СтруктураЗаписи);
			КлючЗаписи = Новый ("РегистрСведенийКлючЗаписи.ТелеграмПользователи", МассивСтруктураЗаписи);
			
			Параметрыформы = Новый Структура("Ключ", КлючЗаписи);                            
			ОткрытьФорму("РегистрСведений.ТелеграмПользователи.ФормаЗаписи", Параметрыформы, ЭтотОбъект);
			
		Иначе
			
			ФормаНастроекПользователя = ПолучитьФорму("РегистрСведений.ТелеграмПользователи.ФормаЗаписи");
			ФормаНастроекПользователя.Запись.ТелеграмБот = МодульТелеграмСерверУХ.ПолучитьТелеграмБотаПоУмолчанию();			
			ФормаНастроекПользователя.Запись.Пользователь = Объект.Ссылка;
			ФормаНастроекПользователя.ВладелецФормы = ЭтотОбъект;
			ФормаНастроекПользователя.Открыть();
...
&НаКлиенте
Процедура ПриОтветеНаВопросОСохраненииИзмененныхДанныхТелеграм(Результат, ДополнительныеПараметры) Экспорт

	Если Результат = КодВозвратаДиалога.Да Тогда
		Попытка
			Записать();
		Исключение
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось записать пользователя: %1'"), ОписаниеОшибки());
...
&НаКлиенте
Процедура НажатиеНастроитьСистемуВзаимодействия_Подключаемый(Команда)
	
	Если Модифицированность Тогда
				
		ОповещениеОЗаписи = Новый ОписаниеОповещения("ПриОтветеНаВопросОСохраненииИзмененныхДанныхСистемаВзаимодействия", ЭтотОбъект);
		ТекстВопроса = НСтр("ru = 'Для настройки системы взаимодействия Пользователь должен быть записан. Сохранить изменения?'");	
		ПоказатьВопрос(ОповещениеОЗаписи, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Возврат;
		
	КонецЕсли;
	
	ОткрытьФормуНастройкиСистемыВзаимодействия();
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьФормуНастройкиСистемыВзаимодействия()
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ТехнологияСервиса.ИнтеграцияССистемойВзаимодействия") Тогда
		
		ОповещениеОЗакрытии = Новый ОписаниеОповещения("ПослеНастройкиСистемыВзаимодействия", ЭтотОбъект);
		ИмяФормыОбработки = "Обработка.УправлениеСистемойВзаимодействия.Форма";
		ОткрытьФорму(ИмяФормыОбработки, , ЭтотОбъект, , , , ОповещениеОЗакрытии);
...
&НаКлиенте
Процедура ПриОтветеНаВопросОСохраненииИзмененныхДанныхСистемаВзаимодействия(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ПослеНастройкиСистемыВзаимодействия(Результат, ДополнительныеПараметры) Экспорт

	СистемаВзаимодействияКлиентУХ.ПодключениеСистемыВзаимодействияДляОповещений(Объект.Ссылка);
