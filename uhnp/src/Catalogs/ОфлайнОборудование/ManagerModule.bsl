...
Процедура УстановитьВерсиюФорматаОбменаККМED(Параметры = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ОтменитьТранзакцию();
			ШаблонТекстаОшибки = НСтр("ru = 'Не удалось обработать обновление Офлайн-оборудования по причине: %1';
										|en = 'Не удалось обработать обновление Офлайн-оборудования по причине: %1'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
			...
			ОбщегоНазначенияБПО.ЗаписатьОшибкуВЖурналРегистрации(
				НСтр("ru = 'Обновление информационной базы';
					|en = 'Обновление информационной базы'", ОбщегоНазначенияБПО.КодОсновногоЯзыка()),
				ТекстОшибки);
		КонецПопытки;
	КонецЦикла;
