...
Процедура ДобавитьГруппыСчетовУчета()
	...
	ГруппыСчетов = СчетаУчета.Выгрузить(Новый Структура("ЭтоГруппа", Истина), "ИмяПредопределенныхДанных, Ссылка");
	...
	Для каждого ГруппаСчетов Из ГруппыСчетов Цикл
		...
		ГруппаУзел.Заголовок	= Строка(ГруппаСчетов.Ссылка);//на языке пользователя
		...
		ГруппаУзел.Группировка 	= ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
...
Процедура ДобавитьРеквизитыСчетаУчета(Узел, ТекущийОбъектИсточник = Неопределено)
	...
	Для каждого СчетУчета Из СчетаУчета Цикл
		...
		ЗаголовокСчета = Строка(СчетУчета.Ссылка);// на языке пользователя
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(ИмяСчета, отСчетБД, , ЗаголовокСчета));
		Для НомерСубконто = 1 По ЧислоСубконто Цикл
