...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом() // не используем мастер в БМ, НМ
		И УчетМСФОПовтИспУХ.ЕстьРСБУ() // не используем мастер в ВЕ
		...
		Лизинг_Сумма 		= Получить_Лизинг_Сумма(ФИ, Параметры.Стоимость, ЭтоЦБ, ИмяСуммаФИ);
		...
		Заполнить_ВалютныйФИ(Организация);
		
		ЗаполнитьПоСтарымПараметрам();
...
Процедура ОбновитьОтборыАренда(Форма)
	...
	Представление = НСтр("ru = 'параметры учета задолженности'");
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ГруппаАренда, 
		"Ссылка.ВидОбъектаФинансовогоХарактера.Задолженность", Истина, , Представление, ВозможнаДКЗ, ,"ЯвляетсяЗадолженностью");
	Представление = НСтр("ru = 'параметры учета аренды'");
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ГруппаАренда, "Ссылка.ВидОбъектаФинансовогоХарактера.Лизинг", Истина, , Представление, ВозможнаАренда, , "ЯвляетсяАрендой");
...
Процедура ДобавитьРекурсивно(Мас, ЭлементОтбора)
	...
	ИначеЕсли ЭлементОтбора.ПравоеЗначение = Ложь Тогда
		Мас.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не %1'"),
...
&НаСервере
Процедура Заполнить_ВалютныйФИ(Организация)
	...
	ИначеЕсли ТипЗнч(Параметры.ФИ) = Тип("СправочникСсылка.ЦенныеБумаги") И НЕ Параметры.ФИ.Пустая() Тогда
		...
		ВалютныйФИ = (ФИ.ВалютаКотировки <> РеквизитыУП.ФункциональнаяВалюта);
...
&НаСервере
Функция Получить_Лизинг_Сумма(ФИ, Стоимость, ЭтоЦБ, ИмяСуммаФИ)
	
	Если ЗначениеЗаполнено(Параметры.Стоимость) Тогда
		Возврат Параметры.Стоимость;
	ИначеЕсли ЭтоЦБ Тогда
		Возврат ФИ.Номинал * ФИ.Количество;
	ИначеЕсли ЗначениеЗаполнено(ИмяСуммаФИ) Тогда 
		Возврат ФИ[ИмяСуммаФИ];
	Иначе 
		Возврат СуммаЛизингаЕРП(ФИ, ДатаСведений);
	КонецЕсли;

КонецФункции

&НаСервереБезКонтекста
Функция СуммаЛизингаЕРП(ДоговорАренды, Дата = Неопределено) Экспорт
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	т.СтоимостьПредметовАренды КАК СтоимостьПредметовАренды
	|ИЗ
	|	РегистрСведений.УсловияДоговоровАренды.СрезПоследних(&Дата, Договор = &ДоговорАренды) КАК т");
	...
	Если Выборка.Следующий() Тогда
		Возврат Выборка.СуммаВыкупаПредметовАренды;
	Иначе 
		Возврат 0;
	КонецЕсли; 
...
&НаСервере
Процедура ЗаполнитьПоСтарымПараметрам()
