...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ТипЗначения = Перечисления.ТипыЗначенийПоказателейОтчетов.Число Тогда
		Валютный = Ложь;
		ПересчитыватьВалютнуюСумму=Ложь;
		НеСуммируется = Истина;
		КонтролируемыйПоказатель = Ложь;
		ОстаточныйПоказатель = Ложь;
	КонецЕсли;
	
	Если ОстаточныйПоказатель Тогда
		СпособРасчетаИтогаПоПериоду = Перечисления.ВидыИтоговПоказателяПоПериоду.Остаток;
	Иначе
		СпособРасчетаИтогаПоПериоду = Перечисления.ВидыИтоговПоказателяПоПериоду.Сумма;
	КонецЕсли;
		
	Если НЕ ЗначениеЗаполнено(ВидКурса) Тогда
		ВидКурса = Перечисления.ВидыКурсов.СреднийКурсЗаПериод;
	КонецЕсли;
	...
	Если ВнутриГрупповой Тогда
		...
		Если НЕ (УправлениеОтчетамиУХ.ПроверитьНаличиеАналитикиОрганизацииВГруппеРаскрытия(ГруппаРаскрытия)
			...
			ТекстСообщения = Нстр("ru = 'Показателю %1 не установлен признак ""Внутригрупповой"".'");
			ТекстСообщения=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения,Наименование);
			ТекстСообщения=ТекстСообщения+Символы.ПС+Нстр("ru = 'Нельзя установить признак ""Внутригрупповой"" вычисляемому показателю,'");
			ТекстСообщения=ТекстСообщения+Символы.ПС+Нстр("ru = 'для которого не определено раскрытие по аналитике ""Организации"".'");
	...
	Если НЕ Валютный И ПересчитыватьВалютнуюСумму Тогда
		ПересчитыватьВалютнуюСумму = Ложь;
	КонецЕсли;
	...
	Если НЕ Отказ Тогда
		...
		Если ТипЗнч(Владелец)=Тип("СправочникСсылка.ВидыОтчетов") И (Не ПометкаУдаления) Тогда
	...
	Если (Предназначение=Перечисления.ПредназначенияЭлементовСтруктурыОтчета.БюджетДвиженияДенежныхСредств
		ИЛИ Предназначение=Перечисления.ПредназначенияЭлементовСтруктурыОтчета.БюджетДоходовИРасходов) Тогда		
	...
	Если ТипЗначения = Перечисления.ТипыЗначенийПоказателейОтчетов.Число Тогда
		...
		Если ВидИтога = Перечисления.ВидыИтоговПоказателя.ПустаяСсылка() Тогда
	...
	ИначеЕсли ТипЗначения = Перечисления.ТипыЗначенийПоказателейОтчетов.Булево Тогда
		...
		Если ВидИтога = Перечисления.ВидыИтоговПоказателя.ПустаяСсылка() Тогда
			ВидИтога = Перечисления.ВидыИтоговПоказателя.Максимум;
		КонецЕсли;
	...
	ИначеЕсли ТипЗначения = Перечисления.ТипыЗначенийПоказателейОтчетов.Строка Тогда
		...
		Если ВидИтога = Перечисления.ВидыИтоговПоказателя.Сумма 
			ИЛИ ВидИтога = Перечисления.ВидыИтоговПоказателя.Среднее Тогда
			
			СтрокаШаблона = Нстр("ru = 'Невозможно записать показатель с кодом ""%1"": вид итога по группе раскрытия ""%2"" не применим к типу Строка '");
			ОбщегоНазначенияУХ.СообщитьОбОшибке(СтрШаблон(СтрокаШаблона, СокрЛП(Код),СокрЛП(ВидИтога)), Отказ,, СтатусСообщения.Важное);
