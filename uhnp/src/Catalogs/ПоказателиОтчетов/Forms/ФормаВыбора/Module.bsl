	...
	ИначеЕсли ЗначениеЗаполнено(МассивПоказателей) Тогда
		...
		Иначе
			...
			Если МножественныйВыбор Тогда				
				...
				//КонецЦикла;
				ОповеститьОВыборе(Новый Структура("ВидОтчета,МассивПоказателей",ВидОтчета,МассивПоказателей));
...
Процедура ЗаполнитьТаблицуПоказателейИБланковОтчетов()
	...
	БланкОтчетаПоУмолчанию = Справочники.ВидыОтчетов.ЗначениеНастройкиПоУмолчанию(ВидОтчета, "БланкДляОтображения");
...
Процедура ВывестиТабличныйДокументБланкаОтчета()
	...
	Если ЗначениеЗаполнено(БланкОтчета) Тогда
		...
		МакетОтчетаОбъект = БланкОтчета.Макет.Получить();
		
		Если НЕ МакетОтчетаОбъект=Неопределено Тогда
			
			ЕстьРабочиеБланки=Истина;
			ПолеБланкаОтчета.Вывести(МакетОтчетаОбъект);
			...
			Если ТекущаяСтрокаДерева=Неопределено Тогда   //Не было такой строки
				...
				Иначе
					...
					Если СтрокаРодитель=Неопределено Тогда
						...
						// Проверим, что группирующая строка не попала под отбор
						Если СтрокаРодитель <> Неопределено Тогда
