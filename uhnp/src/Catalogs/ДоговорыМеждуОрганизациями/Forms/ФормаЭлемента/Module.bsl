...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УправлениеВидимостьюЭлементовГруппыКурсПлатежейПоДоговору(ЭтотОбъект);
	#КонецОбласти
КонецПроцедуры
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ВстраиваниеУХДоговорыМеждуОрганизациями.ПриЧтенииСозданииНаСервере(ЭтаФорма);
	#КонецОбласти
...
Процедура УстановитьТекстПодсказкиПорядокЗачетаДокументов()
	...
	Если Объект.ПорядокЗачетаОплатНакладных = ПредопределенноеЗначение("Перечисление.ПорядкиЗачетаОплатНакладных.ПоДатеВидуДокумента") Тогда
		...
		|Documents within one day are offset by type: first, prepayment and debt offsets, invoices, returns of goods and invoice adjustments, payments, refunds, and so on.'");
...
&НаКлиенте
Процедура Подключаемый_ПриИзмененииРеквизитовГруппыКурсПлатежейПоДоговору(Элемент)
	
	Если Элемент.Имя = "ОсновнаяВалютаПлатежей" Тогда
		//ДоговорыКонтрагентовФормыУХКлиент.ПриИзмененииОсновнойВалютыПлатежей(ЭтотОбъект, Элемент);
		УправлениеВидимостьюЭлементовГруппыКурсПлатежейПоДоговору(ЭтотОбъект);
	ИначеЕсли Элемент.Имя = "СпособОпределенияКурса" Тогда
...
&НаКлиентеНаСервереБезКонтекста
Процедура УправлениеВидимостьюЭлементовГруппыКурсПлатежейПоДоговору(Форма)
	...
	ФлагТолькоПросмотр = (РазличаютсяВалютыВзаиморасчетовИПлатежа
		И Объект.СпособОпределенияКурсаПлатежа = ПредопределенноеЗначение("Перечисление.СпособыОпределенияКурсаПлатежа.ФиксированныйКурс"));
	УстановитьТолькоПросмотрКонтролируемыхЭлементов(Форма, "ЗапретПлатежаВПрочихВалютах", ФлагТолькоПросмотр);
	
КонецПроцедуры


&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьТолькоПросмотрКонтролируемыхЭлементов(Форма, ИмяРеквизита, ФлагТолькоПросмотр)
	
	Параметры = Форма.ПараметрыЗапретаРедактированияРеквизитов.НайтиСтроки(Новый Структура("ИмяРеквизита", ИмяРеквизита));
	...
	Если НЕ Параметр.РедактированиеРазрешено Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого КлючЗначение Из Параметр.БлокируемыеЭлементы Цикл
		Форма.Элементы[КлючЗначение.Значение].ТолькоПросмотр = ФлагТолькоПросмотр;
	КонецЦикла;
