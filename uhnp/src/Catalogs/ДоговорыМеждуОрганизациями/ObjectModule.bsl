...
Процедура ИнициализироватьСправочник(ДанныеЗаполнения = Неопределено, ЗаполнятьВсеРеквизиты = Истина) Экспорт
	...
	Если ЗаполнятьВсеРеквизиты Тогда
		...
		Рубли =  ЗначениеНастроекПовтИсп.ВалютаРегламентированногоУчетаОрганизации(Организация);
		Если ОплатаВВалюте Тогда
			ОсновнаяВалютаПлатежей = ВалютаВзаиморасчетов;
		Иначе 
			ОсновнаяВалютаПлатежей = Рубли;
		КонецЕсли;
		#КонецОбласти 
