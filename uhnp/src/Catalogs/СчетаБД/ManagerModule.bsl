...
Функция ПолучитьСчетПоСчетСсылка(СчетСсылка) Экспорт
	...
	Возврат ?(РезультатЗапроса.Пустой(), ПустаяСсылка(), РезультатЗапроса.Выгрузить().Получить(0).Получить(0));
