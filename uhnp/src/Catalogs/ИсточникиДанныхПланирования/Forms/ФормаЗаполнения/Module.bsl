...
Процедура НастроитьФорму()
	...
	Элементы.ПравилоЗаполненияКоличествоИсточник.Заголовок = ?(ПланироватьПоСумме, НСтр("ru = 'Источник';
																						|en = 'Source'"),  НСтр("ru = 'Источник количества';
																												|en = 'Quantity source'"));
...
Процедура ИнициализироватьКомпоновщикиНастроекИсточников(ЗаполнениеЦен)
	...
	Исключение
		ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В источнике заполнения данных ""%1"" имеются ошибки в настройках. Установлены настройки по умолчанию.';
				|en = 'There are errors in the settings in the ""%1"" data filling source. The default settings are set.'"), ЭлементИсточник.Источник));
			Модифицированность = Истина;
	КонецПопытки;
