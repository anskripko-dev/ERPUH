...
Процедура ОбновитьОтборПоВладельцу()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(ЭтотОбъект.Список, "ТипБД", ТипБД);		
...
&НаКлиенте
Процедура ЗагрузитьВыборочно(Команда)
	
	ОписаниеОЗакрытии = Новый ОписаниеОповещения("ЗагрузкаТаблиц_Завершение", ЭтотОбъект);
	ОткрытьФорму("Справочник.ТаблицыADO.Форма.ФормаЗагрузкиПоИменам",Новый Структура("ТипБД",ТипБД),ЭтаФорма,,,,ОписаниеОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);

КонецПроцедуры 

&НаКлиенте
Процедура ЗагрузкаТаблиц_Завершение(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
...
&НаСервереБезКонтекста
Процедура ОбновитьСуществующиеНаСервере(ТипБД)
	
	Запрос=Новый Запрос;
	Запрос.Текст="ВЫБРАТЬ
	|	ТаблицыADO.Имя КАК Имя
	|ИЗ
	...
	МассивИмен=Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Имя");
	
	Справочники.ТаблицыADO.ОбновитьТаблицыADO(ТипБД,МассивИмен);
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьСуществующие(Команда)
	
	ОбновитьСуществующиеНаСервере(ТипБД);
