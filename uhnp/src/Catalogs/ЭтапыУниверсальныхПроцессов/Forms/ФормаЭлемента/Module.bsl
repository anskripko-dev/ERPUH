...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Объект.ТипЭтапа = Перечисления.ТипыЭтаповУниверсальныхПроцессов.ОбработкаУниверсальногоПроцесса Тогда	
	...
	ИначеЕсли Объект.ТипЭтапа = Перечисления.ТипыЭтаповУниверсальныхПроцессов.ЭтапУниверсальногоЭкспорта Тогда	
		...
		Для Каждого дПараметр Из  ДеревоОтборов_.Строки[0].Строки Цикл
			нПараметр = КэшПараметров.Добавить();
			ЗаполнитьЗначенияСвойств(нПараметр,дПараметр);
		КонецЦикла;
...
Процедура СохранитьТаблицыВоВременномХранилище(АдресДеревоОтборов, АдресТабличноеПолеПереходов, АдресКэшВидовСубконто,АдресТаблицаДоступныхПараметров, ОбычныйЭтап)
	...
	МассивТиповОбычногоЭтапа.Добавить(Перечисления.ТипыЭтаповУниверсальныхПроцессов.ОбработкаУниверсальногоПроцесса);
...
Процедура УправлениеДоступностью()
	...
	ЭтоЭтапОбработка	 = (Объект.ТипЭтапа = Перечисления.ТипыЭтаповУниверсальныхПроцессов.ОбработкаУниверсальногоПроцесса);
...
#Область ФункцииРаботыСДеревомКонтекста
...
// Возвращает строку для вызова формы выбора статуса утверждения по согласуемому 
// объекту СогласуемыйОбъектВход.
Функция ПолучитьСтрокуФормыВыбораСтатусаУтверждения(СогласуемыйОбъектВход)
	РезультатФункции = "";
...
Процедура СтатусСогласованияПриУтвержденииНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Если ЗначениеЗаполнено(СогласуемыйОбъект) Тогда
		СтрокаФормы = ПолучитьСтрокуФормыВыбораСтатусаУтверждения(СогласуемыйОбъект);
