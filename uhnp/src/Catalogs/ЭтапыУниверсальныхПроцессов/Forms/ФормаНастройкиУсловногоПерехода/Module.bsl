...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ОбычныйЭтап Тогда
		...
		Элементы.ДеревоОтборовДобавитьИсточникДанных.Видимость = Ложь;
	Иначе
		Если Параметры.ЗаписыватьЭтапУниверсальногоПроцесса Тогда
			ЭтапУниверсальногоПроцесса = Параметры.ЭтапУниверсальногоПроцесса;
			НеОповещатьВладельца       = Параметры.НеОповещатьВладельца;
			Наименование               = ЭтапУниверсальногоПроцесса.Наименование;
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				Нстр("ru = '%1 (Этап универсального процесса)'"),
				Наименование);
		Иначе
	...
	Если ЭтоАдресВременногоХранилища(Параметры.АдресТабличноеПолеПереходов) Тогда
		...
		Если Вн_ТабличноеПолеПереходов.Колонки.Найти("ОтображениеДействияТекст") = Неопределено Тогда
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыТабличноеПолеПерехода
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыТаблицаДоступныхПараметров
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Иначе
			Если ИмяТипа <> Тип("Неопределено")  Тогда	 //Простой тип
				...
				Если ИмяТипа[0].Значение = Тип("Число") ИЛИ ИмяТипа[0].Значение = "Число"
					ИЛИ ИмяТипа[0].Значение = Тип("Строка") ИЛИ ИмяТипа[0].Значение = "Строка"
						ИЛИ ИмяТипа[0].Значение = Тип("Дата") ИЛИ ИмяТипа[0].Значение = "Дата"
						   ИЛИ ИмяТипа[0].Значение = Тип("Булево") ИЛИ ИмяТипа[0].Значение = "Булево" Тогда
		...
		Если ЭтоСравнениеСоСписком Тогда
			...
			Элементы.ТабличноеПолеПереходаЗначениеОтбора.ВыбиратьТип = Ложь;
			Элементы.ТабличноеПолеПереходаЗначениеОтбора.ОграничениеТипа = Новый ОписаниеТипов("Неопределено");
...
&НаСервере
Процедура ОбновитьЭтапУниверсальногоПроцесса()
	...
	Попытка
		...
		ДеревоПереходов  = РеквизитФормыВЗначение("ТабличноеПолеПерехода");
		ДеревоПереходов.Колонки.Удалить("ОтображениеДействияТекст");
		ПараметрыТаблица = РеквизитФормыВЗначение("ТаблицаДоступныхПараметров");
		
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("Справочник.ЭтапыУниверсальныхПроцессов");
		ЭлементБлокировки.УстановитьЗначение("Ссылка", ЭтапУниверсальногоПроцесса);
		Блокировка.Заблокировать();
		
		ЭтапОбъект = ЭтапУниверсальногоПроцесса.ПолучитьОбъект();
		ЭтапОбъект.Наименование = Наименование;
		ЗаблокироватьДанныеДляРедактирования(ЭтапУниверсальногоПроцесса);
		
		ЭтапОбъект.ЗначенияПараметров.Загрузить(ПараметрыТаблица);
			...
			Если ЗначениеЗаполнено(СтрУсловие.ЗначениеОтбора) Тогда
				...
				Если ТипЗнч(СтрУсловие.ЗначениеОтбора)=Тип("СписокЗначений") Тогда
					...
					Для Каждого УсловиеОтбора ИЗ СтрУсловие.ЗначениеОтбора Цикл
						...
						ДобавитьЗначениеУсловия(ЭтапОбъект, УсловиеОтбора.Значение);
						
					КонецЦикла;
					
				Иначе
					
					ДобавитьЗначениеУсловия(ЭтапОбъект, СтрУсловие.ЗначениеОтбора);
		...
		КонецЦикла;
		ЭтапОбъект.УсловияСогласования = Новый ХранилищеЗначения(ДеревоПереходов);
		ЭтапОбъект.Записать();
		
		Справочники.ЭтапыУниверсальныхПроцессов.ОбновитьЭтапыПредшественники(
			ЭтапОбъект.Ссылка, ПоместитьВоВременноеХранилище(ДеревоПереходов));
...
&НаСервере
Процедура ДобавитьЗначениеУсловия(ЭтапОбъект, ЗначениеУсловия)
	...
	НоваяСтрока=ЭтапОбъект.ЗначенияУсловийОтборов.Добавить();
...
Процедура ПередРазворачиваниемСтрокиРеквизитовАналитики(СтруктураСтрока) 
	...
	СписокОбъектовБД = Новый СписокЗначений;
	ПредставлениеСписка = "";
	...
	Для Каждого СтрТип Из СтруктураСтрока.ТипЗначения Цикл 
		...
		Если СтрНайти(СтрТип.Значение,"Справочник.")>0 Тогда
			...
			ОбъектМетаданных = Метаданные.Справочники.Найти(ОбъектИмя);
			Если ОбъектМетаданных <> Неопределено Тогда
				ПредставлениеСписка = СокрЛП(ОбъектМетаданных.ПредставлениеСписка);	
			КонецЕсли;
		...
		ИначеЕсли СтрНайти(СтрТип.Значение,"Документ.")>0 Тогда
			...
			ОбъектМетаданных = Метаданные.Документы.Найти(ОбъектИмя);
			Если ОбъектМетаданных <> Неопределено Тогда
				ПредставлениеСписка = СокрЛП(ОбъектМетаданных.ПредставлениеСписка);	
			КонецЕсли;
		...
		ИначеЕсли СтрНайти(СтрТип.Значение,"Перечисление.")>0 Тогда
			...
			ОбъектМетаданных = Метаданные.Перечисления.Найти(ОбъектИмя);
			Если ОбъектМетаданных <> Неопределено Тогда
				ПредставлениеСписка = СокрЛП(ОбъектМетаданных.ПредставлениеСписка);	
			КонецЕсли;
		...
		ИначеЕсли СтрНайти(СтрТип.Значение,"ПланСчетов.")>0 Тогда
			...
			ОбъектМетаданных = Метаданные.ПланыСчетов.Найти(ОбъектИмя);
			Если ОбъектМетаданных <> Неопределено Тогда
				ПредставлениеСписка = СокрЛП(ОбъектМетаданных.ПредставлениеСписка);	
			КонецЕсли;
		...
		СписокОбъектовБД.Добавить(ОбъектБД, ПредставлениеСписка);
	...
	Для Каждого ОбъектСписка Из СписокОбъектовБД Цикл
		ОбъектБД = ОбъектСписка.Значение;
		
		Для Каждого СтрРеквизит ИЗ ОбъектБД.Реквизиты Цикл
			
			Если СтрНачинаетсяС(НРег(СтрРеквизит.Синоним), Нстр("ru = '(не использовать)'"))
				Или СтрНачинаетсяС(НРег(СтрРеквизит.Синоним), Нстр("ru = '(не используется)'"))
				Или СтрНачинаетсяС(НРег(СтрРеквизит.Синоним), Нстр("ru = 'удалить'"))Тогда
				Продолжить;
			КонецЕсли;
		...
		ДеревоОтборов_.Строки.Сортировать("НаименованиеБД");
		...
		ТабличныеЧасти.Строки.Сортировать("Имя");
		
		// Табличные части добавляем только для первого уровня дерева отборов.
		Если СтрНайти(СтруктураСтрока.ПолныйПуть, "ТабличнаяЧасть_") = 0 Тогда
			Для Каждого ТЧ Из ТабличныеЧасти.Строки Цикл
				
				Если СтрНачинаетсяС(НРег(ТЧ.Синоним), Нстр("ru = '(не использовать)'"))
					Или СтрНачинаетсяС(НРег(ТЧ.Синоним), Нстр("ru = '(не используется)'"))
					Или ТЧ.Имя = "ДополнительныеРеквизиты" Тогда
					Продолжить;
				КонецЕсли;
				
				СтрокаПоказателей = ДеревоОтборов_.Строки.Добавить();
				СтрокаПоказателей.ПолеБД         = ТЧ.Имя;
				СтрокаПоказателей.НаименованиеБД = ТЧ.Синоним;
				СтрокаПоказателей.ТипПоля        = "Параметры";
				СтрокаПоказателей.Элемент        = ТЧ.Синоним;
				СтрокаПоказателей.ИндексКартинки = 5;
				
				Для Каждого Стр Из ТЧ.Строки Цикл
					
					Если СтрНачинаетсяС(НРег(Стр.Синоним), Нстр("ru = '(не использовать)'"))
						Или СтрНачинаетсяС(НРег(Стр.Синоним), Нстр("ru = '(не используется)'"))
						Или СтрНачинаетсяС(НРег(Стр.Синоним), Нстр("ru = 'удалить'"))Тогда
						Продолжить;
					КонецЕсли;
					
					СтрокаТЧ = СтрокаПоказателей.Строки.Добавить();
					СтрокаТЧ.ПолеБД         = Стр.Имя;
					СтрокаТЧ.НаименованиеБД = Стр.Синоним;
					СтрокаТЧ.ТипПоля        = "Параметры";
					СтрокаТЧ.Элемент        = Стр.Синоним;
					СтрокаТЧ.ПолныйПуть     = СтруктураСтрока.ПолныйПуть + "." + "ТабличнаяЧасть_" + ТЧ.Имя +"." + Стр.Имя;
					СтрокаТЧ.ИндексКартинки =10;
					
					Если Стр.Имя = "Ссылка" Тогда
						
						Если ТипЗнч(ОбъектБД) = Тип("СправочникСсылка.ДокументыБД") ИЛИ ТипЗнч(ОбъектБД) = Тип("СправочникСсылка.СправочникиБД") Тогда
							
							РаботаСПолямиАналитикиУХ.ЗаполнитьПоляТиповРеквизитовБД(СтрокаТЧ, ?(ТипЗнч(ОбъектБД)=Тип("СправочникСсылка.СправочникиБД"),"Справочник","Документ")+"."+ОбъектБД.Наименование,Справочники.ТипыБазДанных.ТекущаяИБ);
					...
					Иначе
						...
						Если ТипЗнч(ОбъектБД) = Тип("СправочникСсылка.ДокументыБД") ИЛИ ТипЗнч(ОбъектБД)=Тип("СправочникСсылка.СправочникиБД") Тогда
							РаботаСПолямиАналитикиУХ.ЗаполнитьПоляТиповРеквизитовБД(СтрокаТЧ, Стр.ТипДанных, Справочники.ТипыБазДанных.ТекущаяИБ);
						Иначе
							РаботаСПолямиАналитикиУХ.ЗаполнитьПоляТиповРеквизитовБД(СтрокаТЧ, "", Справочники.ТипыБазДанных.ТекущаяИБ);
						КонецЕсли;
						
					КонецЕсли;
					
					Если (СтрокаТЧ.ТипМетаДанных = "Справочник" ИЛИ СтрокаТЧ.ТипМетаДанных = "Документ" ИЛИ СтрокаТЧ.ТипЗначения.Количество() > 1)  Тогда 
						// Добавим строку для дальнейшего раскрытия
				...
				СтрокаПоказателей.Строки.Сортировать("НаименованиеБД");
		...
		Если ДополнительныеРеквизиты <> Неопределено тогда
			...
			МассивВозможныхНаименований.Добавить(ОбъектСписка.Представление);
			МассивВозможныхНаименований.Добавить(Строка(ОбъектБД));
			ЗапросПоДопРеквизитамИСведениям.УстановитьПараметр("МассивВозможныхНаименований", МассивВозможныхНаименований);
		...
		|ГДЕ
		|	НаборыДополнительныхРеквизитовИСведенийДополнительныеСведения.Ссылка В(&МассивВозможныхСсылок)");
				
		МассивВозможныхСсылок = Новый Массив;
		МассивВозможныхСсылок.Добавить(Справочники.НаборыДополнительныхРеквизитовИСведений.НайтиПоНаименованию(ОбъектСписка.Представление, Истина));
		МассивВозможныхСсылок.Добавить(Справочники.НаборыДополнительныхРеквизитовИСведений.НайтиПоНаименованию(Строка(ОбъектБД), Истина));
		ЗапросПоДопРеквизитамИСведениям.УстановитьПараметр("МассивВозможныхСсылок", МассивВозможныхСсылок);
...
Процедура ПередЗакрытиемЗавершение(Результат, Параметры) Экспорт 
	Если Результат = КодВозвратаДиалога.Да Тогда 
		Если ВладелецФормы <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(ЭтапУниверсальногоПроцесса) Тогда
				Если Не ЗначениеЗаполнено(Наименование) Тогда
					...
					Возврат;
				КонецЕсли;
			ОбновитьЭтапУниверсальногоПроцесса();
			Оповестить("ОбновитьОтчет", , ШаблонПроцесса);
			Закрыть();
			Иначе
...
&НаКлиенте
Процедура ЗаписатьИЗакрытьФорму(ЗакрытьФорму = Истина)
