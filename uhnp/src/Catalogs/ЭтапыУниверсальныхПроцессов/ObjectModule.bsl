...
Процедура УстановитьЛинейныйПереход(ЭтапПерехода) Экспорт
	...
	Если ТипЗнч(ТекУсловияСогласования) <> Тип("ДеревоЗначений") Тогда
		...
		ТекУсловияСогласования.Колонки.Добавить("ЗначениеОтбора", Новый ОписаниеТипов("СправочникСсылка.Пользователи, СписокЗначений, СправочникСсылка.ПроизвольныйКлассификаторУХ, СправочникСсылка.Организации, СправочникСсылка.ВидыОтчетов"));
...
Процедура ПриЗаписи(Отказ)
	...
	Если НЕ Отказ Тогда
		УправлениеПроцессамиПодготовкиОтчетности.РассчитатьУровниЭтапов(Владелец, Отказ);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ТипЭтапа = Перечисления.ТипыЭтаповУниверсальныхПроцессов.ЭтапПроцессаПодготовкиОтчетности Тогда
		...
		тДокументы = ФормируемыеДокументы.Выгрузить(, "ДокументБД, ШаблонДокумента");
		тДокументы.Колонки.Добавить("Количество");
		тДокументы.ЗаполнитьЗначения(1, "Количество");
		тДокументы.Свернуть("ДокументБД, ШаблонДокумента", "Количество");
		
		Для Каждого СтрокаДокументы Из тДокументы Цикл
			
			Если СтрокаДокументы.Количество > 1 Тогда
				ТекстСообщения = НСтр("ru = 'Обнаружены дублирующие строки в таблице формируемых документов: ""%1"" ""%2""'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения,
					СтрокаДокументы.ДокументБД, СтрокаДокументы.ШаблонДокумента);
				ОбщегоНазначенияУХ.СообщитьОбОшибке(ТекстСообщения, Отказ);
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(СтрокаДокументы.ШаблонДокумента)
				И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаДокументы.ДокументБД, "ИспользоватьШаблоны") = ИСТИНА Тогда
				ТекстСообщения = НСтр("ru = 'Не заполнен шаблон для документа: ""%1""'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения,
					СтрокаДокументы.ДокументБД);
				ОбщегоНазначенияУХ.СообщитьОбОшибке(ТекстСообщения, Отказ);
			КонецЕсли;
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	// Сбросим порядковый номер, который будет установлен при записи.
	ПорядковыйНомер = 0;
...
Функция ПроверитьСоответствиеЭтапаРежимуПроцесса()
	...
	Если РежимПроцесса = Перечисления.НазначенияУниверсальныхПроцессов.МаршрутСогласования Тогда
		Если ТипЭтапа = Перечисления.ТипыЭтаповУниверсальныхПроцессов.ЭтапПроцессаПодготовкиОтчетности 
			ИЛИ ТипЭтапа = Перечисления.ТипыЭтаповУниверсальныхПроцессов.ЭтапУниверсальногоИмпорта 
			ИЛИ ТипЭтапа = Перечисления.ТипыЭтаповУниверсальныхПроцессов.ЭтапУниверсальногоЭкспорта Тогда
			Возврат Ложь;
		КонецЕсли;
