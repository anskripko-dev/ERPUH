...
Процедура СоздатьПериоды(НеСуществующиеПериоды,КлючПериода,Периодичность)
	...
	Для Каждого Период ИЗ ПериодыПолгода Цикл 
		...
		Если Месяц(нПериод.ДатаНачала) = 1 Тогда
			...
			СтрокаШаблона = Нстр("ru = '1 Полугодие %1'");
			
			нПериод.Наименование = ПредставлениеПериода(НачалоГода(нПериод.ДатаНачала), НачалоГода(нПериод.ДатаОкончания));
			Если Не ПустаяСтрока(СтрокаШаблона) тогда								
				нПериод.Наименование = СтрШаблон(СтрокаШаблона, ПредставлениеПериода(НачалоГода(нПериод.ДатаНачала), НачалоГода(нПериод.ДатаОкончания)));
			КонецЕсли;
			
			нПериод.КраткоеНаименование  = Нстр("ru = '1 Полугодие'");
		Иначе
			
			СтрокаШаблона = Нстр("ru = '2 Полугодие %1'");
			...
			нПериод.КраткоеНаименование  = Нстр("ru = '2 Полугодие'");
		КонецЕсли;
