	...
	Если СертификатСписокВыбора.Количество() = 0 Тогда
		СертификатСписокВыбора.Добавить("СертификатыЗагружаются", 
			НСтр("ru = 'Сертификаты загружаются, откройте список повторно через 5 с.';
				|en = 'Certificates are loading, please open the list again in 5 sec.'"));
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	СпособАутентификацииПользователяПриИзменении(Неопределено);
...
&НаКлиенте
Процедура СпособАутентификацииПользователяПриИзменении(Элемент)
	
	Если СпособАутентификацииПользователя Тогда
...
Процедура Далее(Команда)
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаПолучениеССервера Тогда
		...
		Если СпособАутентификацииПользователя = 0 И Не ЗначениеЗаполнено(СертификатАутентификации) Тогда  // по сертификату
		...
		Если СпособАутентификацииПользователя = 1 И НЕ ВведенЛогинИПароль() Тогда
			Отказ = Истина;
		КонецЕсли;
		...
		Если СпособАутентификацииПользователя = 0 Тогда // по сертификату
...
Процедура ПереключитьШагПослеВыбораОрганизацииИБанка()
	...
	Элементы.СпособАутентификацииПользователя.Видимость = Истина;
	ОбновитьОтображениеДанных();
	...
	ИначеЕсли НастройкиОбменаСБанком.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.АсинхронныйОбмен") Тогда
		...
		Если ЗначениеЗаполнено(НастройкиОбменаСБанком.СпособАутентификации) Тогда
			...
			ИначеЕсли НастройкиОбменаСБанком.СпособАутентификации = "ПоСертификату" Тогда
				СпособАутентификацииПользователя = 0;
				Элементы.СпособАутентификацииПользователя.Видимость = Ложь;
				Оповещение = Новый ОписаниеОповещения(
...
Процедура ДополнитьПредставлениеСерийнымНомером(Представление, СерийныйНомер)
	...
	ТекстШаблон = ?(ЗначениеЗаполнено(Представление), НСтр("ru = '%1, SN %2';
															|en = '%1, SN %2'"), НСтр("ru = 'SN %2';
																					|en = 'SN %2'"));
...
Функция СтруктураСозданияНастройкиОбмена()
	...
	Если ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.АсинхронныйОбмен") Тогда
		СтруктураПараметров.Вставить("АутентификацияПоСертификату", СпособАутентификацииПользователя = 0);
	КонецЕсли;
	СтруктураПараметров.Вставить("АутентификацияПоСертификату", СпособАутентификацииПользователя = 0);
	СтруктураПараметров.Вставить("ИмяВнешнегоМодуля", ИмяВнешнегоМодуля);
