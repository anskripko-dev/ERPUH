...
Процедура ПроверитьИспользованиеОбъекта(Параметры, АдресХранилища) Экспорт
	...
	|	ИСТИНА
	|ИЗ
	|	РегистрСведений.ОбластиДействияАналоговМатериалов КАК ТаблицаПоиска
	|ГДЕ
	|	ТаблицаПоиска.Спецификация = &Объект
	//-- НЕ УТКА
	|
...
//++ НЕ УТКА
	Если ДанныеСпецификации.Свойство("ВидыРабочихЦентров")
		...
		РассчитатьЗагрузку(ПартияВыпуска, ДанныеСпецификации, ДанныеСпецификации.ВидыРабочихЦентров);
	...
	Если ДанныеСпецификации.Свойство("АльтернативныеВидыРабочихЦентров")
		...
		РассчитатьЗагрузку(ПартияВыпуска, ДанныеСпецификации, ДанныеСпецификации.АльтернативныеВидыРабочихЦентров);
	КонецЕсли;
...
Процедура РассчитатьВремяРаботы(ПартияВыпуска, ДанныеСпецификации, Коллекция, ДанныеДляРасчета = Неопределено)
	...
	Для каждого Строка Из Коллекция Цикл
		...
		Если ПустаяСтрока(Строка.АлгоритмРасчетаКоличества) Тогда
			Если Строка.ПараллельнаяЗагрузка
				И Строка.МаксимальнаяЗагрузка <> 0 Тогда
				
				Загрузка = Строка.ЗагрузкаНаЕдиницуПартииВыпуска * ПартияВыпуска.Числитель / ПартияВыпуска.Знаменатель;
				Коэффициент = Цел(Загрузка / Строка.МаксимальнаяЗагрузка)
					+ ?(Загрузка % Строка.МаксимальнаяЗагрузка > 0, 1, 0);
				Строка.ВремяРаботы = Строка.ВремяРаботыНаЕдиницуПартииВыпуска * Коэффициент;
				
			ИначеЕсли Строка.ОдновременноПроизводимоеКоличествоЕдиницПартийИзделий > 0 Тогда
				...
				Если ДанныеСпецификации.ВыпускПроизвольнымиПорциями Тогда
					...
					Коэффициент = Цел(ОсновноеИзделие.Количество / ОдновременноПроизводимое)
						+ ?(ОсновноеИзделие.Количество % ОдновременноПроизводимое > 0, 1, 0);
...
Процедура РассчитатьЗагрузку(ПартияВыпуска, ДанныеСпецификации, Коллекция)
	
	Для каждого Строка Из Коллекция Цикл
		Строка.Загрузка = Строка.ЗагрузкаНаЕдиницуПартииВыпуска
			* ПартияВыпуска.Числитель / ПартияВыпуска.Знаменатель;
	КонецЦикла;
...
// 	МассивЗначений - Массив из см. УправлениеДаннымиОбИзделияхКлиентСервер.СтруктураЗначенияПараметраНазначенияСпецификаций - массив значений
// Возвращаемое значение:
...
Процедура СоздатьПромежуточнуюНоменклатуруХарактеристику(Строка, КэшСсылокПромежуточногоВыпуска)
	...
	Попытка
		...
		Если СоздатьХарактеристику Тогда
			Строка.ХарактеристикаПромежуточногоВыпуска = Справочники.ХарактеристикиНоменклатуры.ИдентичнаяХарактеристика(
				Строка.НоменклатураПромежуточногоВыпуска, Строка.Характеристика);
			КэшСсылокПромежуточногоВыпуска.Вставить(Строка.Характеристика, Строка.ХарактеристикаПромежуточногоВыпуска);
		КонецЕсли;
