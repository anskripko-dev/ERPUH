...
Процедура ОсновноеИзделиеНоменклатураПриИзмененииНаСервере(КэшированныеЗначения)
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтруктураСтроки, СтруктураДействий, КэшированныеЗначения);
...
Процедура ПроизводственныйПроцессИнициализироватьЭтап(ДанныеСтроки, КэшЭлемента)
	...
	РедакторПроизводственногоПроцесса.ЗаполнитьВидыРабочихЦентров(
		ПроизводственныйПроцессЭтап, ЭтапВидыРабочихЦентров, ЭтапЕстьПараллельнаяЗагрузка, ЭтапЕстьСинхроннаяЗагрузка);
		...
		Для каждого Строка Из Объект[ИмяТЧ] Цикл
			...
			Если ЗначениеЗаполнено(Строка.ОперацияРедактирование) И ТехПроцессОперацииСтрока <> Неопределено Тогда
				...
				Если КлючИЗначение.Значение.ДобавитьВТехпроцесс Тогда
					...
					НоваяСтрока.ОперацияРедактирование = ТехПроцессОперацииСтрока.ОперацияОбъект.ПолучитьСсылкуНового();
...
Процедура ЭтапВидыРабочихЦентровВидРабочегоЦентраПриИзмененииНаСервере()
	...
	РедакторПроизводственногоПроцессаКлиентСервер.РассчитатьСлужебныеПризнакиЗагрузкиВРЦ(
		ЭтапВидыРабочихЦентров,
		ЭтапЕстьПараллельнаяЗагрузка,
		ЭтапЕстьСинхроннаяЗагрузка);
		
	РедакторПроизводственногоПроцессаКлиентСервер.ПроверитьОчиститьОдновременноПроизводимоеКоличество(
		ПроизводственныйПроцессЭтап, ЭтотОбъект, "Этап");
...
Процедура НастроитьЗависимыеЭлементыФормыЭтапа(Форма, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("ВидыРабочихЦентров")
		...
		РедакторПроизводственногоПроцессаКлиентСервер.НастроитьЭлементОдновременноПроизводимоеКоличество(Объект, Форма, "Этап");
...
Процедура ЭтапЗаполнитьВидыРабочихЦентровПоМаршрутнойКартеНаСервере()
	...
	РедакторПроизводственногоПроцесса.ЗаполнитьВидыРабочихЦентровПоМаршрутнойКарте(
		ПроизводственныйПроцессЭтап, ЭтотОбъект, ЭтапВидыРабочихЦентров, ЭтапЕстьПараллельнаяЗагрузка, ЭтапЕстьСинхроннаяЗагрузка, "Этап");
...
//++ НЕ УТКА
	...
	Элемент.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.МатериалыИУслугиСпособПолученияМатериалаРедактирование.Имя);
...
Процедура ДобавитьПромежуточныйВыпускПриВыбореЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ПромежуточныйВыпуск, СтруктураДействий, КэшированныеЗначения);
