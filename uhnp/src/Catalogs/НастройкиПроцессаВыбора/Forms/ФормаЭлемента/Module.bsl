...
Процедура УстановитьУсловноеОформление()
	...
	Элементы.НастройкаЗавершена.Видимость = Объект.ИмяТипаОбъекта = "ДокументСсылка.ПредложениеПоставщика";
...
Процедура ЗакупочнаяКомиссияПриИзменении(Элемент)
	Если Элементы.ЗакупочнаяКомиссия.Видимость Тогда
		Элементы.ЗакупочнаяКомиссия.Обновить();             
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьЭкспертнуюКомиссию_Сервер()
	Если ТипЗнч(ВладелецОбъектов) = Тип("ДокументСсылка.Лот") Тогда
		ТекЗакупка = ВладелецОбъектов.ЗакупочнаяПроцедура;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.ИмяТипаОбъекта = "ДокументСсылка.ПредложениеПоставщика" И Объект.НастройкаЗавершена Тогда
		Отказ = ПроверитьНастройкуОценкиЗавершенияДляПредложений(Отказ);
	КонецЕсли;   
	
КонецПроцедуры        

&НаСервере
Функция ПроверитьНастройкуОценкиЗавершенияДляПредложений(Отказ)
	
	Если Объект.ЭтапыВыбора.Количество() = 0  Тогда
		ТекстСообщения = НСтр("ru = 'Необходимо заполнить таблицу этапов выбора'");
		ОбщегоНазначенияУХ.СообщитьОбОшибке(ТекстСообщения,Отказ);
		Возврат Истина;
	КонецЕсли;	
	Если Объект.КритерииОценки.Количество() = 0  Тогда
		ТекстСообщения = НСтр("ru = 'Необходимо заполнить таблицу критериев'");
		ОбщегоНазначенияУХ.СообщитьОбОшибке(ТекстСообщения,Отказ);
		Возврат Истина;
	КонецЕсли;	      
