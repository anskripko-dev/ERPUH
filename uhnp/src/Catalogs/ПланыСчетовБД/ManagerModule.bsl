...
Функция ПолучитьПланСчетовБД(ИмяПланаСчетов = Неопределено, ТипБД = Неопределено) Экспорт
	...
	Если ИмяПланаСчетов = Неопределено Тогда
		...
		ПланСчетовПоУмолчанию=Константы.ПланСчетовПоУмолчанию.Получить(); 
		
		Если ЗначениеЗаполнено(ПланСчетовПоУмолчанию) Тогда
			
			Возврат ПланСчетовПоУмолчанию;
			
		Иначе
			
			Возврат Справочники.ПланыСчетовБД.НайтиПоНаименованию("МСФО", Истина, , Справочники.ТипыБазДанных.ТекущаяИБ);
