...
Процедура ЗаполнитьРеквизитыОбъекта(Кэш = Неопределено, ЕстьИзменения = Истина) Экспорт
	...
	Если ТекущаяИБ 
		И Наименование="МСФО" 
		И СтруктураОписания.ПланСчетовМетаданные="МСФО" Тогда
		
		СтруктураОписания.Вставить("ПланСчетовМСФО",Истина);
	...
	Если СтруктураОписания.Свойство("Синоним") Тогда
		
		ОбщегоНазначенияКлиентСерверУХ.УстановитьНовоеЗначение(ЭтотОбъект[ПолеСиноним], СтруктураОписания.Синоним, ЕстьИзменения);
		
		Если ПолеСиноним="Синоним1" Тогда
			
			ОбщегоНазначенияКлиентСерверУХ.УстановитьНовоеЗначение(Наименование1, СтруктураОписания.Синоним, ЕстьИзменения); 
			
		ИначеЕсли ПолеСиноним="Синоним2" Тогда
			
			ОбщегоНазначенияКлиентСерверУХ.УстановитьНовоеЗначение(Наименование2, СтруктураОписания.Синоним, ЕстьИзменения);
...
Процедура ЗаполнитьТаблицуСчетов(СчетБД=Неопределено)
	...
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(ПланСчетовМетаданные) Тогда
			
			Если  ЗначениеЗаполнено(СчетБД) Тогда		
				
				ПланСчетовМетаданные=СчетБД.Метаданные().Имя;
		...
		| ИЗ ПланСчетов." + ПланСчетовМетаданные + " КАК ПланСчетов_МД";
		
		Если НЕ Реквизиты.Найти("ПланСчетовБД")=Неопределено Тогда
			
			Если Владелец=Справочники.ТипыБазДанных.ТекущаяИБ Тогда
				
				ТекстЗапроса=ТекстЗапроса+"
				|ГДЕ ПланСчетов_МД.ПланСчетовБД=&ПланСчетовБД";
				Запрос.УстановитьПараметр("ПланСчетовБД",Ссылка); 
				
			Иначе
				
				ТекстЗапроса=ТекстЗапроса+"
				|ГДЕ ПланСчетов_МД.ПланСчетовБД.Наименование=&ПланСчетовБДНаименование";
				Запрос.УстановитьПараметр("ПланСчетовБДНаименование",Наименование);
...
Функция ОбновитьСчет(СтрокаСчет, Родитель, ВерхнийУровень, ДопСвойства = Неопределено, МассивСчетов)
	...
	Для Каждого Элемент ИЗ СписокРеквизитовДляМСФО Цикл
		...
		ИначеЕсли Элемент.Значение="СчетаУчетаКурсовыхРазниц" Тогда
			
			СчетаУчетаКурсовыхРазниц = ЗаполненноеЗначение(СтрокаСчет.СчетаУчетаКурсовыхРазниц, Справочники.СчетаУчетаКурсовыхРазниц.ПустаяСсылка());
			РеквизитыСчета.Вставить("СчетаУчетаКурсовыхРазниц", СчетаУчетаКурсовыхРазниц);
		
		ИначеЕсли Элемент.Значение="ПересчетВФВ" Тогда
			
			ПересчетВФВ = ЗаполненноеЗначение(СтрокаСчет.ПересчетВФВ, Перечисления.ПересчетыВФВ.ПустаяСсылка());
			РеквизитыСчета.Вставить("ПересчетВФВ", ПересчетВФВ);
			
		ИначеЕсли Элемент.Значение="ПересчетВВО" Тогда
			
			РеквизитыСчета.Вставить("ПересчетВВО", СтрокаСчет.ПересчетВВО);
		ИначеЕсли Элемент.Значение = "ДетализацияКорреспонденцииСчета" Тогда
			
			РеквизитыСчета.Вставить("ДетализацияКорреспонденцииСчета", СтрокаСчет.ДетализацияКорреспонденцииСчета);
...
Процедура ПриЗаписи(Отказ)
	...
	// НАЧАЛО Ситалов Дмитрий 29.03.2024 Выполнение задач №000049053
	Если ОбщегоНазначения.СсылкаСуществует(ВидОтчетаОСВ) Тогда
	// КОНЕЦ Ситалов Дмитрий 29.03.2024 Выполнение задач №00004905
	...
	// НАЧАЛО Ситалов Дмитрий 29.03.2024 Выполнение задач №000049053
	КонецЕсли;
	// КОНЕЦ Ситалов Дмитрий 29.03.2024 Выполнение задач №000049053
...
СписокРеквизитовДляМСФО.Добавить("ДетализацияКорреспонденцииСчета");
СписокРеквизитовДляМСФО.Добавить("СчетаУчетаКурсовыхРазниц");
СписокРеквизитовДляМСФО.Добавить("ПересчетВФВ");
СписокРеквизитовДляМСФО.Добавить("ПересчетВВО");
