...
Процедура ОбновитьЗаголовокФормы()
	...
	ИначеЕсли Объект.ТипОС = Перечисления.ТипыОС.КапитальноеВложение Тогда
		...
		Заголовок = 
			?(ЗначениеЗаполнено(Объект.Ссылка),
				СтрШаблон(НСтр("ru = '%1 (Кап. вложения в арендованное имущество)';
								|en = '%1 (Capital expenditure in leased assets)'"), Объект.Наименование),
				НСтр("ru = 'Кап. вложения в арендованное имущество (создание)';
					|en = 'Capital expenditure in leased assets (Create)'"));
...
#Область ОбработчикиСобытийФормыУХ
...
#Область ОбработчикиСобытийЭлементовФормыУХ

&НаКлиенте
Процедура СтрахованиеПриИзменении(Элемент)
	МСФОКлиентСерверУХ.ОбновитьЗаголовкиГруппВНА(ЭтотОбъект);
КонецПроцедуры

&НаКлиенте
Процедура РемонтныйЦиклПриИзменении(Элемент)
	МСФОКлиентСерверУХ.ОбновитьЗаголовкиГруппВНА(ЭтотОбъект);
КонецПроцедуры
...
Процедура ПриИзмененииНазначенияВНА()
	...
	Контекст = Новый Структура("НазначениеВНА, Класс");
	ЗаполнитьЗначенияСвойств(Контекст, Объект);
	ТекущийВид = ?(ЗначениеЗаполнено(Контекст.НазначениеВНА), Контекст.НазначениеВНА, Контекст.Класс);
	Справочники.НазначенияВНА.УстановитьСвойстваЭлементовПоНазначениюВНАНаСервере(
		Элементы, ТекущийВид, Перечисления.ТипыВНА.ОсновноеСредство);
...
&НаКлиенте
Процедура ПринятоКУчетуМСФОНажатие(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	Если ЗначениеЗаполнено(ДокументВвода) Тогда
		ПоказатьЗначение(, ДокументВвода);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура СнятоСУчетаМСФОНажатие(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	Если ЗначениеЗаполнено(ДокументВыбытия) Тогда
		ПоказатьЗначение(, ДокументВыбытия);
	КонецЕсли;
КонецПроцедуры
