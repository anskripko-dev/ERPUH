...
Процедура ЗаполнитьПериоды()
	...
	Если СписокВыбора.НайтиПоЗначению(ВерсияХранимогоФайла) = Неопределено Тогда
		Если СписокВыбора.Количество() > 0 Тогда
			ВерсияХранимогоФайла = СписокВыбора[0].Значение;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьСписокИменЛистов()
	...
	СписокЛистов = ОбработкаФайловMicrosoftExcelКлиентСерверУХ.ВернутьСписокЛистов(ИмяФайла, РасширениеФайла);
...
Процедура ОК(Команда)
	...
	Иначе
		...
		НуженПароль   = Ложь;
	КонецЕсли;
	Закрыть(Новый Структура("ХранимыйФайл, ВерсияХранимогоФайла, НуженПароль, ИмяЛиста, Пароль", ХранимыйФайл, ВерсияХранимогоФайла, НуженПароль, ИмяЛиста, Пароль));
