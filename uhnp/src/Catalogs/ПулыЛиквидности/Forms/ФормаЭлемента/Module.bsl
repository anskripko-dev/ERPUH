...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Контрагент = РаботаСКонтрагентамиУХ.КонтрагентОрганизации(Объект.Организация);
...
Процедура ПриОткрытии(Отказ)
	ПроверитьОшибкиПоДоговору(ЭтотОбъект);
КонецПроцедуры
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ТипЗнч(ПараметрыЗаписи) = Тип("Структура") И ПараметрыЗаписи.Свойство("ЗаписыватьДоговор") Тогда
		Возврат;
	КонецЕсли;
	
	СтруктураОтбора = Новый Структура("ВходитВПул, Договор", Истина, ПредопределенноеЗначение("Справочник.ДоговорыКонтрагентов.ПустаяСсылка"));
	
	НезаполненныеСтроки = УчастникиПула.НайтиСтроки(СтруктураОтбора); 
	
	ЕстьОшибки = Ложь;
	
	Для Каждого СтрокаТаблицы Из НезаполненныеСтроки Цикл  
	...
	Если ЕстьОшибки Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Для организаций, входящих в пул, необходимо указать договор'"),,,,Отказ);
	КонецЕсли;   
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Для Каждого ТекСтрокаСчет Из БанковскиеСчетаПула Цикл
		...
		Если Не ТекСтрокаСчет.Организация = Объект.Организация И Не ТекСтрокаСчет.СчетОрганизации = Объект.МастерСчет Тогда
...
Процедура ПроверитьОшибкиПоДоговору(Форма)
	
	УчастникиПула = Форма.УчастникиПула;
	Для Каждого ТекДанные Из УчастникиПула Цикл
		Если ЗначениеЗаполнено(ТекДанные.Договор) Тогда
			ТекДанные.ЕстьОшибкиДоговор = Не ЕстьВстречныйДоговорКонтрагента(ТекДанные.Договор);
		Иначе
			ТекДанные.ЕстьОшибкиДоговор = Ложь;
...
Процедура ОбновитьСписокСчетовНаСервере()
	...
	ВыгрузкаДоговоров.Колонки.Добавить("ВходитВПул");
