...
Процедура ЗаполнитьПроцедурыРедактирования()
	...
	Для Каждого Строка ИЗ ТаблицаКорректировок Цикл
		...
		РабочийТекст=Строка.Процедура;
		
		Если Строка.ПроизвольныйКод Тогда
			
			Строка.ПроцедураРедактирования=РабочийТекст;
		...
		Иначе
			...
			Пока Результат.Следующий() Цикл
				...
				РабочийТекст=СтрЗаменить(РабочийТекст,"["+СокрЛП(Результат.КодИсточника)+"]",ТекстОперанда);
