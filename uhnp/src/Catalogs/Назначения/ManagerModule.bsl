...
//++ Локализация
	ИначеЕсли ИсточникДанных = "РНПТМатериаловВПроизводстве" Тогда
		
		ВозвращаемоеЗначение = ДобавитьОписаниеРНПТМатериаловВПроизводстве(МакетФормы, ДобавитьСторно, Префикс);
...
Функция ДобавитьОписаниеРНПТМатериаловВПроизводстве(МакетФормы, ДобавитьСторно, Префикс)
	
	Префикс = ?(ПустаяСтрока(Префикс), "РНПТМатериаловВПроизводстве", Префикс);
	
	ТекстЗапроса = 
	"// Запрос метода ""ДобавитьОписаниеРНПТМатериаловВПроизводстве""
	|// Префикс
	|
	|ВЫБРАТЬ
	|	СпрКлючиАналитики.Назначение     КАК Назначение,
	|	&ПоляВыбораПредшествующие,
	|	ТаблицаОстатки.КоличествоОстаток КАК ПрефиксВНаличии
	|	,&ПоляВыбораПоследующие
	|ИЗ
	|	РегистрНакопления.РНПТМатериаловВПроизводстве.Остатки(, Организация = &ПрефиксОрганизация) КАК ТаблицаОстатки
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК СпрКлючиАналитики
	|	ПО СпрКлючиАналитики.Ссылка = ТаблицаОстатки.АналитикаУчетаНоменклатуры
	|
	|ГДЕ
	|	СпрКлючиАналитики.Назначение <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|	И СпрКлючиАналитики.Номенклатура   = &ПрефиксНоменклатура
	|	И СпрКлючиАналитики.МестоХранения  = &ПрефиксПодразделение
	|	И СпрКлючиАналитики.Характеристика = &ПрефиксХарактеристика
	|";
	...
	Если ДобавитьСторно Тогда
		...
		|	СпрКлючиАналитики.Назначение   КАК Назначение,
		|	&ПоляВыбораПредшествующие,
		|	ТаблицаСторно.КоличествоОборот КАК ПрефиксВНаличии
		|	,&ПоляВыбораПоследующие
		|ИЗ
		|	РегистрНакопления.РНПТМатериаловВПроизводстве.Обороты(,, Регистратор, Организация = &ПрефиксОрганизация) КАК ТаблицаСторно
		|	
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК СпрКлючиАналитики
		|	ПО СпрКлючиАналитики.Ссылка = ТаблицаСторно.АналитикаУчетаНоменклатуры
		|
		|ГДЕ
		|	ТаблицаСторно.Регистратор = &ПрефиксРегистратор
		|	И СпрКлючиАналитики.Назначение <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|	И СпрКлючиАналитики.Номенклатура   = &ПрефиксНоменклатура
		|	И СпрКлючиАналитики.МестоХранения  = &ПрефиксПодразделение
		|	И СпрКлючиАналитики.Характеристика = &ПрефиксХарактеристика
		|	";
	...
	ПараметрПредставления = "%" + Префикс + "_Подразделение%";
	ПредставлениеКолонки  = СтрШаблон(НСтр("ru = 'В наличии в подразделении ""%1""';
											|en = 'Available at business unit ""%1""'"), ПараметрПредставления);
	
	Колонки = Новый СписокЗначений();
	Колонки.Добавить("ВНаличии", ПредставлениеКолонки, Истина);
	
	КолонкиПоУмолчанию = Новый Массив();
	КолонкиПоУмолчанию.Добавить("ВНаличии");
