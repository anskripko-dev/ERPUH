	...
	Если Не КриптографияБЭДКлиент.ЭтоТипПодписиCAdESBES(ТипПодписи) Тогда
		...
		Если ДатаВыдачиОтличаетсяБолееЧемНаДень Тогда
			ТекстОшибки = НСтр("ru = 'Штамп времени в электронной подписи должен совпадать с моментом подписания МЧД и не отличаться от даты совершения доверенности более или менее, чем на день';
								|en = 'The time stamp in the digital signature must match the moment of MR LOA signing and must not differ from the date of authorization letter issue by more or less than one day'");
