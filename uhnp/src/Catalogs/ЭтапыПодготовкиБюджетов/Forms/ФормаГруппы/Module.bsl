...
Процедура УсловиеЗапускаПриИзменении(Элемент)
	...
	Если Объект.УсловиеЗапуска = ПредопределенноеЗначение(
		"Перечисление.ТипыУсловийЗапускаЭтаповПодготовкиБюджетов.Вручную") Тогда
		Объект.Срок = 0;
		Периодическая = Ложь;
		УстановитьПараметрыПериодичности();
	КонецЕсли;
...
Процедура УстановитьВидимость(Объект, Элементы)
	...
	Элементы.УточненияПорядкаЗапуска.Видимость = (Объект.УсловиеЗапуска <> ПредопределенноеЗначение(
		"Перечисление.ТипыУсловийЗапускаЭтаповПодготовкиБюджетов.Вручную"));
