...
Процедура ПередЗаписью(Отказ)
	...
	Если ИспользоватьПроверкуКонтрольнойСуммыПриУтверждении Тогда
		НастройкиПроверки = НастройкиПроверкиКонтрольнойСуммы.Получить();
		Если НастройкиПроверки = Неопределено Или (НастройкиПроверки.Реквизиты.Количество() = 0 И НастройкиПроверки.ТабличныеЧасти.Количество() = 0) Тогда
			ОбщегоНазначенияУХ.СообщитьОбОшибке(Нстр("ru = 'Не настроен состав реквизитов для проверки версии объекта при утверждении этапов.'"));
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если НазначениеПроцесса = Перечисления.НазначенияУниверсальныхПроцессов.ПроцессПодготовкиОтчетности Тогда
		...
		ШаблонПроцессаПодготовкиОтчетности = Истина;
	...
	УдалитьЭкземплярыПроцесса(Отказ);
...
Процедура УдалитьЭкземплярыПроцесса(Отказ)
	...
	Запрос.Текст = 
		...
		|	И ЭкземплярПроцесса.ПометкаУдаления <> &ПометкаУдаления";
	
	Запрос.УстановитьПараметр("ПометкаУдаления", ПометкаУдаления);
	Запрос.УстановитьПараметр("ШаблонПроцесса", Ссылка);
