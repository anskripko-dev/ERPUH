...
Функция СформироватьДеревоКонтекстаПроцесса(ШаблонПроцесса, ДеревоОтборов, СоставДерева, ТаблицаДоступныхПараметров = Неопределено) Экспорт
	...
	Если СоставДерева.Свойство("Функции") Тогда
		...
		ДоступныеФункцииИПроцедуры = Справочники.ЭтапыУниверсальныхПроцессов.ДоступныеФункцииИПроцедуры();
		Для НомСтр = 0 По ДоступныеФункцииИПроцедуры.Количество() - 1 Цикл
			
			Если СтрНайти(СокрЛП(ДоступныеФункцииИПроцедуры[НомСтр].Имя),"#")=0 Тогда
				НовСтр = СтрокаФункций.Строки.Добавить();
				НовСтр.Элемент             = ДоступныеФункцииИПроцедуры[НомСтр].Имя + "()";
				НовСтр.ИндексКартинки      = 2;
				НовСтр.Описание            = ДоступныеФункцииИПроцедуры[НомСтр].Описание;
				НовСтр.ТипПоля             = "Функция";
				НовСтр.ИдентификаторОтбора = Строка(Новый УникальныйИдентификатор);
			ИначеЕсли  СокрЛП(ДоступныеФункцииИПроцедуры[НомСтр].Имя) = "#ВозвращаемыйПараметр" Тогда
				СтрокаРесурс = НовСтр.Строки.Добавить();
				СтрокаРесурс.Элемент             = ДоступныеФункцииИПроцедуры[НомСтр].ПараметрИмя;
				СтрокаРесурс.ИндексКартинки      = 4;
				СтрокаРесурс.Описание            = ДоступныеФункцииИПроцедуры[НомСтр].ТипПараметра;
				СтрокаРесурс.ТипЗначения.Добавить(ДоступныеФункцииИПроцедуры[НомСтр].ТипПараметра);
			...
			ИначеЕсли  СокрЛП(ДоступныеФункцииИПроцедуры[НомСтр].Имя)="#Параметр" Тогда
				
				Если НЕ ТаблицаДоступныхПараметров = Неопределено Тогда
					Нп = ТаблицаДоступныхПараметров.Добавить();
					Нп.ИмяПараметра        = ДоступныеФункцииИПроцедуры[НомСтр].ПараметрИмя;
					Нп.ТипОтбораПараметра  = Перечисления.СпособыВычисленияПараметровОперандов.ФиксированноеЗначение;
					Нп.ТипЗначения.Добавить((ДоступныеФункцииИПроцедуры[НомСтр].ТипПараметра));
