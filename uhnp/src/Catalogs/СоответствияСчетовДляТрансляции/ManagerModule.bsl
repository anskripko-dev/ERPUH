...
Процедура ИзменитьОбъектПоПараметрамБюджет(СтруктураПараметров) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(СтруктураПараметров.Ссылка) Тогда
		...
		|	СоответствияСчетовДляТрансляции.СчетИсточник = &СчетИсточник
		|	И СоответствияСчетовДляТрансляции.КоррСчетИсточник = &КоррСчетИсточник
		|	И СоответствияСчетовДляТрансляции.СчетПриемник = &СтрокаПриемник
		|	И СоответствияСчетовДляТрансляции.ИдентификаторСоответствия = &ИдентификаторСоответствия
		|	И СоответствияСчетовДляТрансляции.Владелец = &Владелец";
		
		Запрос.УстановитьПараметр("СчетИсточник",	СтруктураПараметров.СчетИсточник);
		Запрос.УстановитьПараметр("КоррСчетИсточник",СтруктураПараметров.КоррСчетИсточник);
		Запрос.УстановитьПараметр("СтрокаПриемник",	СтруктураПараметров.СтрокаПриемник);
		Запрос.УстановитьПараметр("ИдентификаторСоответствия",	СтруктураПараметров.ИдентификаторСоответствия);
		Запрос.УстановитьПараметр("Владелец",		СтруктураПараметров.Владелец);
		...
		Иначе
			...
			ЗаполнитьЗначенияСвойств(СправочникОбъект,СтруктураПараметров);
			СправочникОбъект.СчетПриемник=СтруктураПараметров.СтрокаПриемник;
						
			СправочникОбъект.Наименование=СтрШаблон(Нстр("ru = 'Корреспонденция счет дт: %1  - счет кт: %2 в строку бюджета %3'"), СтруктураПараметров.СчетИсточник, 
				СтруктураПараметров.КоррСчетИсточник,
				СтруктураПараметров.СтрокаПриемник);
	...
	СправочникОбъект.ГруппаРаскрытия=СтруктураПараметров.СтрокаПриемник.ГруппаРаскрытия;
...
Процедура ОбработкаПолученияПолейПредставления(Поля, СтандартнаяОбработка)
	...
	Поля.Добавить("СчетИсточник");
	Поля.Добавить("КоррСчетИсточник"); 
	Поля.Добавить("СчетПриемник");
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Данные.КоррСчетИсточник) Тогда
		
		ТекстШаблона=НСтр("ru = 'Трансляция со счета %1 (корр. счет %2) в счет %3'");
		Представление =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстШаблона,Данные.СчетИсточник, Данные.КоррСчетИсточник,Данные.СчетПриемник);
		
	Иначе
		
		ТекстШаблона=НСтр("ru = 'Трансляция со счета %1 в счет %2'");
		Представление =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстШаблона,Данные.СчетИсточник,Данные.СчетПриемник); 
