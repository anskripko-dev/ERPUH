...
Функция ПодготовитьДеревоНастройкиОтображенияПолей()
	...
	ТабНастройкаОтображенияПолей.Колонки.Добавить("ВозможнаАналитикаВГО",	мОписаниеТиповБулево);
	ТабНастройкаОтображенияПолей.Колонки.Добавить("ВозможнаАналитикаПроект",мОписаниеТиповБулево);
	ТабНастройкаОтображенияПолей.Колонки.Добавить("НомерАналитики",			ОбщегоНазначенияУХ.ПолучитьОписаниеТиповЧисла(1));
...
Процедура ЗаполнитьАналитикуГруппыРаскрытия(ЗаполнениеПоУмолчанию=Истина,ДобавлятьСтрокиРаскрытия=Ложь) Экспорт
	...
	Для Индекс=1 По ПараметрыСеанса.ЧислоДопАналитик Цикл
		...
		Если Элемент=Неопределено Тогда
			...
			НоваяСтрока.НаименованиеБД = НСтр("ru = 'Аналитика'") + " " + Индекс;
			НоваяСтрока.ВыбиратьВидАналитики = Истина; 
			НоваяСтрока.НомерАналитики=Индекс;
		...
		Иначе
			...
			ТекущаяСтрока.НомерАналитики			= Индекс;
			ТекущаяСтрока.ВозможнаАналитикаВГО  	= Элемент.ВидАналитики.ТипЗначения.СодержитТип(Тип("СправочникСсылка.Контрагенты")) 
														ИЛИ Элемент.ВидАналитики.ТипЗначения.СодержитТип(Тип("СправочникСсылка.Организации"));
			ТекущаяСтрока.ВозможнаАналитикаПроект	= Элемент.ВидАналитики.ТипЗначения.СодержитТип(Тип("СправочникСсылка.Проекты"));

			
			// Добавляем строку для возможного раскрытия по реквизитам объекта
			
			Если ДобавлятьСтрокиРаскрытия И (ТекущаяСтрока.ТипМетаДанных="Справочник" ИЛИ ТекущаяСтрока.ТипМетаДанных="Документ" ИЛИ ТекущаяСтрока.ТипЗначения.Количество()>1) Тогда 
...
Процедура ПодготовитьТаблицуОтображенияПолей(ОбновлятьПоля=Ложь,ДобавлятьСтрокиРаскрытия=Ложь) Экспорт
	
	ЗаполнитьАналитикуГруппыРаскрытия(ОбновлятьПоля ИЛИ ПравилаОтображенияПолей.Количество()=0,ДобавлятьСтрокиРаскрытия); 
...
Процедура ПриИзмененииВидаАналитики(ДанныеОбъекта,СтруктураСтрока) Экспорт
	...
	Если Не ПустаяСтрока(СтруктураСтрока.ВидАналитики) Тогда
		...
		СтрокаПоказателей.НаименованиеБД = СтруктураСтрока.НаименованиеБД+": "+Строка(СтруктураСтрока.ВидАналитики);
		...
		СтруктураСтрока.Вставить("ВозможнаАналитикаВГО",СтруктураСтрока.ВидАналитики.ТипЗначения.СодержитТип(Тип("СправочникСсылка.Контрагенты")) 
														ИЛИ СтруктураСтрока.ВидАналитики.ТипЗначения.СодержитТип(Тип("СправочникСсылка.Организации")));
		СтруктураСтрока.Вставить("ВозможнаАналитикаПроект",СтруктураСтрока.ВидАналитики.ТипЗначения.СодержитТип(Тип("СправочникСсылка.Проекты")));
			...
			Если Стр.МультиязычныйРеквизит Тогда
				...
				Для каждого СтрЯзык из ТаблицаЯзыков Цикл
					...
					Исключение
						...
						Если Не ПустаяСтрока(СтрокаШаблона) тогда
							ОбщегоНазначенияУХ.СообщитьОбОшибке(СтрШаблон(СтрокаШаблона, Стр.Поле, ТребуемоеПоле),,, СтатусСообщения.Важное);
						КонецЕсли;
...
Функция ЗаполнитьАналитикиПоСубконто(СтрокаОтбор=Неопределено,КэшВидовАналитик=Неопределено) Экспорт
	...
	Для Индекс=1 По 3 Цикл
		...
		Если РабочаяТаблицаАналитикСчет.Количество()>=Индекс Тогда
			...
			РабочийВидАналитики = Справочники.ГруппыРаскрытия.ПолучитьВидАналитики(ТипБД, РабочаяТаблицаАналитикСчет[Индекс-1], СчетБД,КэшВидовАналитик);
	...
	Если Обороты Тогда
		...
		Если СчетБД.ДетализацияКорреспонденцииСчета=Перечисления.СпособыДетализацииКорреспонденцииСчета.РаскрыватьПоВидамДвижений Тогда
		
		ЭтотОбъект["ВидАналитики"+(ЧислоАналитикДт+1)]= ПланыВидовХарактеристик.ВидыСубконтоКорпоративные.ВидыДвижений;
		ЧислоАналитикДт=ЧислоАналитикДт+1;
		
		ИначеЕсли СчетБД.ДетализацияКорреспонденцииСчета=Перечисления.СпособыДетализацииКорреспонденцииСчета.РаскрыватьПоКоррСчетам Тогда
			
			ЭтотОбъект["ВидАналитики"+(ЧислоАналитикДт+1)]= ПланыВидовХарактеристик.ВидыСубконтоКорпоративные.СчетаБД2; 
			ЧислоАналитикДт=ЧислоАналитикДт+1;
