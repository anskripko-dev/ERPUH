...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Если ПолучитьФункциональнуюОпцию("ХранитьИсториюРассылкиОтчетов") И НЕ Объект.Личная И Объект.ИспользоватьЭлектроннуюПочту Тогда
		СостояниеРассылки = тн_РассылкаОтчетовБюджетирования.ПолучитьСостояниеРассылкиОтчетов(Объект.Ссылка);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстОшибкиПриОткрытии = тн_РассылкаОтчетовБюджетирования.ТекстОшибкиПроверкиПраваДобавления();
	...
	Если ЭтоНовый Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Автор) Тогда
			...
			тн_РассылкаОтчетовБюджетирования.ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка, Текст);
	...
	Элементы.ПанельНастроекОтборов.Видимость = Истина;
...
Процедура ОбработчикАктивизацииСтрокиТаблицыОтчеты()
	...
	Если СтрокаОтчеты = Неопределено Тогда
		...
		Элементы.СтраницыДопОтборов.ТекущаяСтраница = Элементы.ПустаяСтраницаОтборов;
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьИзмеренияИОтборы(ПараметрыОтчета, ДополнительныеОтборы)
	
	Если ПараметрыОтчета.РежимБланка = 1 Тогда  
		
        Обработки.АналитическийБланкСводнаяТаблица.ПрочитатьНастройкиВариантаСводнойТаблицы(ЭтаФорма, ПараметрыОтчета.ВариантСводнойТаблицы);
		...
		ЗаполнитьДеревоОтборов(ДополнительныеОтборы);
	...
	Иначе
		...
		ЭтаФорма["ТаблицаДоступныеПоля"].Очистить();  
		ПрочитатьСоставДоступныхАналитик(ЭтаФорма["ТаблицаДоступныеПоля"],глТаблицаПоказателей,ЭтаФорма["НастройкиПолейСводнойТаблицы"]);
		...
		Для Каждого СтрДоступноеПоле Из ЭтаФорма["ТаблицаДоступныеПоля"] Цикл
			...
			Если СтрДоступноеПоле.ТипСубконто = Перечисления.ПостоянныеПоляСводнойТаблицы.Организация Тогда
				Продолжить;
			КонецЕсли;	
			
			Если СтрДоступноеПоле.ТипСубконто = Перечисления.ПостоянныеПоляСводнойТаблицы.Проект Тогда
				Продолжить;
			КонецЕсли;	
...
&НаСервере
Процедура ЗаполнитьДеревоОтборов(ДополнительныеОтборы)
	...
	Если НастройкиОтборовДерево.Строки.Количество() = 0 Тогда 
		...
		Для Каждого СтрПолеОтбор Из СТ_ДополнительныеНастройки.Настройки.Отбор.Элементы Цикл
			...
			Если ДополнительныеОтборы <> Неопределено Тогда
				СтрокаДопОтбора = ДополнительныеОтборы.Найти(нЭлемент.ПолеКод, "ПолеКод");
				Если СтрокаДопОтбора <> Неопределено Тогда        
					ЗаполнитьЗначенияСвойств(нЭлемент, СтрокаДопОтбора, "Использовать, ЗначениеОтбора, ВидСравнения, ТипЗначения"); 
				КонецЕсли;  
			КонецЕсли;
		...
		Для Каждого ЭлементДереваОтборов Из НастройкиОтборовДерево.Строки[0].Строки Цикл 
			...
			Если ПолеСКД = Неопределено Тогда
			 ...
			 Иначе
				 ...
				 Если ДополнительныеОтборы <> Неопределено Тогда
					 СтрокаДопОтбора = ДополнительныеОтборы.Найти(ЭлементДереваОтборов.ПолеКод, "ПолеКод");
					 Если СтрокаДопОтбора <> Неопределено Тогда        
						 ЗаполнитьЗначенияСвойств(ЭлементДереваОтборов, СтрокаДопОтбора, "Использовать, ЗначениеОтбора, ВидСравнения, ТипЗначения"); 
					 КонецЕсли;  
				 КонецЕсли;
...
&НаСервере
Функция ПодготовитьТаблицуДопОтборов(ВернутьМассив = Ложь);
	
	ИменаКолонок = "Реквизит, Представление, Значение, Использование, ИндексКартинки, Тип";
	ТЗОтборы = НастройкиТекущегоОтчета.Выгрузить().Скопировать(, ИменаКолонок);
	ПараметрыОтчета = Новый Структура;	
	Для Каждого СтрокаНастройка Из ТЗОтборы Цикл           
		ПараметрыОтчета.Вставить(СтрокаНастройка.Реквизит, СтрокаНастройка.Значение);
	КонецЦикла;  	
	Результат = Справочники.тн_РассылкиОтчетовБюджетирования.ПодготовитьТаблицуОтборов(ПараметрыОтчета, ЭтаФорма);
	
	Если ВернутьМассив Тогда
		
		Строки = Результат.НайтиСтроки(Новый Структура ("Использовать", Истина));  
		
		МассивСтрок = Новый Массив;
		Для Каждого Строка Из Строки Цикл    
			МассивСтрок.Добавить(ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(Строка));
		КонецЦикла;
...
&НаСервере
Функция ПроверитьПолеВДопОтборах(ИмяПоля)
	...
	Для Каждого ЭлементДереваОтборов Из НастройкиОтборовДерево.Строки[0].Строки Цикл 
		...
		Если БазовоеПоле = ИмяПоля И  ЭлементДереваОтборов.Использовать Тогда
			Возврат Истина;
		КонецЕсли;	
...
Процедура КомандаВыполнитьСейчас(Команда) 
	...
	ТекстОшибкиПроверкиПраваРассылки = тн_РассылкаОтчетовБюджетирования.ТекстОшибкиПроверкиПраваРассылки();
	Если ЗначениеЗаполнено(ТекстОшибкиПроверкиПраваРассылки) Тогда
		ПоказатьПредупреждение(, ТекстОшибкиПроверкиПраваРассылки);  
		 Возврат;
	КонецЕсли;
...
Процедура ДобавитьОтчет(Команда)
	...
	ОткрытьФорму("Справочник.БланкиОтчетов.ФормаСписка", ПараметрыФормыВыбора, Элементы.Отчеты);      
...
Процедура ПредварительныйПросмотрОтчета(Команда)
	...
	ТекстОшибкиПроверкиПрав = тн_РассылкаОтчетовБюджетирования.ТекстОшибкиПроверкиПраваНаАтическиеБланки();
	Если ЗначениеЗаполнено(ТекстОшибкиПроверкиПрав) Тогда
		ПоказатьПредупреждение(, ТекстОшибкиПроверкиПрав);  
		Возврат;
	КонецЕсли;
	...
	ПараметрыОтчета = Новый Структура;
	ПараметрыОтчета.Вставить("Бланк",                СтрокаОтчеты.Отчет);
	ПараметрыОтчета.Вставить("РежимБланка",          СтрокаОтчеты.РежимБланка);
    ПараметрыОтчета.Вставить("ВидОтчета",            СтрокаОтчеты.ВидОтчета);    
    ПараметрыОтчета.Вставить("Периодичность",        СтрокаОтчеты.Периодичность);    

	ПараметрыОтчета.Вставить("РежимыДоступаКФункциямСводнойТаблицы", ПредопределенноеЗначение("Перечисление.РежимыДоступаКФункциямСводнойТаблицы.РаботаСДанными")); 

	Для Каждого СтрокаНастройка Из НастройкиТекущегоОтчета Цикл           
		ПараметрыОтчета.Вставить(СтрокаНастройка.Реквизит, СтрокаНастройка.Значение);
	КонецЦикла;           
	
	ПараметрыОтчета.Вставить("ДополнительныеОтборы", ПодготовитьТаблицуДопОтборов(Истина)); 
	
	Если ПараметрыОтчета.РежимБланка = 1 Тогда
				
		тн_РассылкаОтчетовБюджетирования.ЗаполнитьПериоды(ПараметрыОтчета);
		Имя = "Обработка.АналитическийБланкСводнаяТаблица.Форма.ФормаМакетаСводнаяТаблица";
		ОткрытьФорму(Имя, ПараметрыОтчета); 
		
	ИначеЕсли ПараметрыОтчета.РежимБланка = 2 Тогда
		
		Если ТипЗнч(ПараметрыОтчета.ПериодОтчета) = Тип("СтандартныйПериод") Тогда
			тн_РассылкаОтчетовБюджетирования.ЗаполнитьПериоды(ПараметрыОтчета);
			ПараметрыОтчета.ПериодОтчета = ПараметрыОтчета.ПериодС;
			ПараметрыОтчета.Вставить("ПериодОкончания", ПараметрыОтчета.ПериодПо);   
		Иначе
			ПараметрыОтчета.Вставить("ПериодОкончания", ПараметрыОтчета.ПериодОтчета);
		КонецЕсли;
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru = 'Рассылка и просмотр в режиме данных доступны только для бланков сводных таблиц и многопериодных бланков'");
...
Функция ОпределитьНастройку()
	...
	Результат.Вставить("СКД", Неопределено);
	...
	Результат.Вставить("ЭтоЭлементОтбора", Ложь);
	Возврат Результат;
...
Функция ПринадлежитГруппеЛичныхРассылок(Группа)
	...
	Возврат тн_РассылкаОтчетовБюджетирования.ПринадлежитГруппеЛичныхРассылок(Группа);
...
Процедура ЗаписатьНастройкиСтрокиОтчеты(ИдентификаторСтроки)
	...
	Иначе   
		...
		ИменаКолонок = "Реквизит, Представление, Значение, Использование, ИндексКартинки, Тип";
		СохраняемоеЗначение.Вставить("ОсновныеОтборы", НастройкиТекущегоОтчета.Выгрузить().Скопировать(, ИменаКолонок));   
		СохраняемоеЗначение.Вставить("ДополнительныеОтборы", ПодготовитьТаблицуДопОтборов());   
	КонецЕсли;
...
Функция ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, ПользовательскиеНастройки, Интерактивно = Истина)
	...
	// Параметры журнала
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ru = 'Рассылка отчетов бюджетирования (тн). Инициализация отчета';
	...
	ПараметрыОтчета = Новый Структура("Отчет, Настройки", СтрокаОтчеты.Отчет, ПользовательскиеНастройки);
		тн_РассылкаОтчетовБюджетирования.ИнициализироватьОтчет(
			ПараметрыЖурнала,
			ПараметрыОтчета,
	...
	Если ПараметрыОтчета.Инициализирован Тогда
		...
		СтрокаОтчеты.РежимБланка     = ПараметрыОтчета.РежимБланка;
		СтрокаОтчеты.ВидОтчета       = ПараметрыОтчета.ВидОтчета;
		СтрокаОтчеты.Периодичность   = ПараметрыОтчета.Периодичность;
	...
	Если ТипЗнч(ПользовательскиеНастройки) = Тип("Структура")
		И ТипЗнч(ПользовательскиеНастройки.ОсновныеОтборы) = Тип("ТаблицаЗначений") Тогда
		НастройкиТекущегоОтчета.Загрузить(ПользовательскиеНастройки.ОсновныеОтборы);      
	Иначе
		НастройкиТекущегоОтчета.Очистить();
	КонецЕсли;
	
	// Заполнение основных отборов
	Если НастройкиТекущегоОтчета.Количество() = 0 Тогда
		
		Если ПараметрыОтчета.РежимБланка = 1 Тогда
			
			СтрокаНастройка = НастройкиТекущегоОтчета.Добавить();
			СтрокаНастройка.Реквизит       = "ВариантСводнойТаблицы"; 
			СтрокаНастройка.Представление  = "Вариант сводной таблицы";
			СтрокаНастройка.Тип            = Новый ОписаниеТипов("СправочникСсылка.ВариантыСводныхТаблиц");
			СтрокаНастройка.Обнаружен      = Истина;
			СтрокаНастройка.ИндексКартинки = 3;           
			СтрокаНастройка.Использование  = Истина; 
			Если ПараметрыОтчета.Свойство("ВариантСводнойТаблицы") Тогда				
				СтрокаНастройка.Значение   = ПараметрыОтчета.ВариантСводнойТаблицы;				
			КонецЕсли;   
			
			СтрокаНастройка = НастройкиТекущегоОтчета.Добавить();
			СтрокаНастройка.Реквизит       = "ВалютаОтображения"; 
			СтрокаНастройка.Представление  = "Валюта отображения";
			СтрокаНастройка.Тип            = Новый ОписаниеТипов("СправочникСсылка.Валюты");
			СтрокаНастройка.Обнаружен      = Истина;
			СтрокаНастройка.ИндексКартинки = 3;  				
			СтрокаНастройка.Использование  = Истина;
			
			СтрокаНастройка = НастройкиТекущегоОтчета.Добавить();
			СтрокаНастройка.Реквизит       = "ПериодОтчета"; 
			СтрокаНастройка.Представление  = "Период отчета";
			СтрокаНастройка.Тип            = Новый ОписаниеТипов("СтандартныйПериод");
			СтрокаНастройка.Обнаружен      = Истина;
			СтрокаНастройка.ИндексКартинки = 3;   
			СтрокаНастройка.Использование  = Истина;
		...
		Иначе    
			...
			СписокТипов.Добавить(Тип("СправочникСсылка.Периоды"));
			СписокТипов.Добавить(Тип("СтандартныйПериод"));
			ОписаниеСоставногоТипа = Новый ОписаниеТипов(СписокТипов);
			
			СтрокаНастройка = НастройкиТекущегоОтчета.Добавить();
			СтрокаНастройка.Реквизит       = "ПериодОтчета"; 
			СтрокаНастройка.Представление  = "Период отчета";
			СтрокаНастройка.Тип            = ОписаниеСоставногоТипа;
			СтрокаНастройка.Обнаружен      = Истина;
			СтрокаНастройка.ИндексКартинки = 3;  				
			СтрокаНастройка.Использование  = Истина;
			
			СтрокаНастройка = НастройкиТекущегоОтчета.Добавить();
			СтрокаНастройка.Реквизит       = "Валюта"; 
			СтрокаНастройка.Представление  = "Валюта";
			СтрокаНастройка.Тип            = Новый ОписаниеТипов("СправочникСсылка.Валюты");
			СтрокаНастройка.Обнаружен      = Истина;
			СтрокаНастройка.ИндексКартинки = 3;   
			СтрокаНастройка.Использование  = Истина;
			...
			СтрокаНастройка = НастройкиТекущегоОтчета.Добавить();
			СтрокаНастройка.Реквизит       = "Периодичность"; 
			СтрокаНастройка.Представление  = "Периодичность";
			СтрокаНастройка.Тип            = Новый ОписаниеТипов("ПеречислениеСсылка.Периодичность");
			СтрокаНастройка.Обнаружен      = Истина;
			СтрокаНастройка.ИндексКартинки = 3;      				
			СтрокаНастройка.Использование  = Истина;
		...
		СтрокаНастройка = НастройкиТекущегоОтчета.Добавить();
		СтрокаНастройка.Реквизит       = "Сценарий"; 
		СтрокаНастройка.Представление  = "Сценарий";
		СтрокаНастройка.Тип            = Новый ОписаниеТипов("СправочникСсылка.Сценарии");
		СтрокаНастройка.Обнаружен      = Истина;
		СтрокаНастройка.ИндексКартинки = 3;   
		СтрокаНастройка.Использование  = Истина;
		
		СтрокаНастройка = НастройкиТекущегоОтчета.Добавить();
		СтрокаНастройка.Реквизит       = "Организация"; 
		СтрокаНастройка.Представление  = "Организация";
		СтрокаНастройка.Тип            = Новый ОписаниеТипов("СправочникСсылка.Организации");
		СтрокаНастройка.Обнаружен      = Истина;    
		СтрокаНастройка.Использование  = Истина;
		СтрокаНастройка.ИндексКартинки = 3;
		
		СтрокаНастройка = НастройкиТекущегоОтчета.Добавить();
		СтрокаНастройка.Реквизит       = "Проект"; 
		СтрокаНастройка.Представление  = "Проект";
		СтрокаНастройка.Тип            = Новый ОписаниеТипов("СправочникСсылка.Проекты");
		СтрокаНастройка.Обнаружен      = Истина;
		СтрокаНастройка.ИндексКартинки = 3;
		СтрокаНастройка.Использование  = Истина;
	...
	Иначе
		ДополнительныеОтборы = ПользовательскиеНастройки.ДополнительныеОтборы;
	КонецЕсли;      
		
    ОбновитьИзмеренияИОтборы(ПараметрыОтчета, ДополнительныеОтборы);
	
	Элементы.СтраницыНастроекОтчета.ТекущаяСтраница = Элементы.СтраницаНастройкиТекущегоОтчета;	 
	Элементы.СтраницыДопОтборов.ТекущаяСтраница = Элементы.СтраницаДопОтборов;
...
Процедура УстановитьВидРассылки()
	...
	Если тн_РассылкаОтчетовБюджетирования.ПринадлежитГруппеЛичныхРассылок(Объект.Родитель)Тогда
		 ВидРассылки = "Личная";
...
Процедура ПреобразоватьПараметрыНастроекОтчетов(ТекущийОбъект, ВариантКонвертации)
	...
	Для каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		...
		Если ТипЗнч(ОбъектСтрокаОтчеты) = Тип("СправочникТабличнаяЧастьСтрока.тн_РассылкиОтчетовБюджетирования.Отчеты") И ВнесеныИзменения Тогда
			ОбъектСтрокаОтчеты.Настройки = Новый ХранилищеЗначения(ПользовательскиеНастройки, Новый СжатиеДанных(9));
