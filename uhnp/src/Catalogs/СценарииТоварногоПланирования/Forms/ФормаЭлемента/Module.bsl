...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ВстраиваниеУХСценарииТоварногоПланирования.ПриЧтенииСозданииНаСервере(ЭтаФорма);
	#КонецОбласти
...
&НаКлиенте
Процедура Подключаемый_РежимФормированияПотребностейПланомЗакупокПриИзменении(Элемент)

	ВозможностьПланированияПотребностейДляКЗ = ЭтаФорма.ИспользоватьЦентрализованноеУправлениеЗакупками
					И Объект.Периодичность = ЭтаФорма.ПериодичностьЦентрализованныхЗакупок; 
					
	ЭтоПланПотребностей = ЭтоПланированияПотребностей(Объект.ФормированиеПотребностейПланомЗакупок);  
	
	Если ЭтоПланПотребностей Тогда
		ПланЗакупокПланировать = 1;
		Элементы.ПланЗакупокПланировать.ТолькоПросмотр = Истина;
		Объект.Периодичность = ЭтаФорма.ПериодичностьЦентрализованныхЗакупок;
	Иначе
		Элементы.ПланЗакупокПланировать.ТолькоПросмотр = Ложь;
	КонецЕсли;
	
	// 
	Если ВозможностьПланированияПотребностейДляКЗ Тогда
		Если ЭтоПланПотребностей Тогда
			// Для планирования план должен быть количественно-суммовым
			ПланЗакупокПланировать = 1;
		КонецЕсли;
		ПланироватьПриИзменении(Элементы.ПланЗакупокПланировать);
		НастроитьЗависимыеЭлементыФормы(ЭтаФорма, "ФормированиеПотребностейПланомЗакупок");
	КонецЕсли;
