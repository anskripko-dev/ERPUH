...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Объект.Владелец)=Тип("СправочникСсылка.ДокументыБД") Тогда
		...
		Элементы.Владелец.Заголовок=НСтр("ru = 'Документ'");
		
	Иначе
		
		Элементы.Владелец.Заголовок=НСтр("ru = 'Справочник'");
...
Процедура УстановитьВидимость()
	...
	Иначе
		...
		Элементы.ГруппаИсточникиТЧ.Видимость					=(Не Объект.ПроизвольныйКод);
...
Процедура ПроизвольныйКодПриИзменении(Элемент)
	...
	Если ПустаяСтрока(Объект.ПроцедураЗаполнения) Тогда
		...
		Объект.ПроцедураЗаполнения=НСтр("ru = '
...
&НаКлиенте
Процедура СформироватьНаименованиеОбъекта()

	Если Объект.ПоОбъектуБД Тогда
		
		Объект.Наименование=""+Объект.ТипБД+", "+Объект.ИсходныйОбъектБД;
		
	Иначе
		
		Объект.Наименование=""+Объект.ТипБД+", "+НСтр("ru = 'произвольные источники'");
...
Функция СоздатьИсточникДанных(НазначениеРасчетов,ПотребительРасчета,ИсходныйОбъектБД,ТипБДПриемник)
	...
	Если ЗначениеЗаполнено(ИсходныйОбъектБД) Тогда
		...
		СправочникОбъект.ПриИзмененииОбъектаБД();
