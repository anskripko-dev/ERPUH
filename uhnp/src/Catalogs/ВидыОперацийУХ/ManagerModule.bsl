...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Элемент, "Наименование", "ru = 'БДДС, Расход';
		...
		#Область ПлатежФакторинговойКомпании
		ИмяПредопределенныхДанных = "ПлатежФакторинговойКомпании";
		Элемент = Элементы.Добавить(); // см. СтрокаТЧПредопреденногоЭлемента
		Элемент.ИмяПредопределенныхДанных = ИмяПредопределенныхДанных;
		Элемент.Родитель = Справочники.ВидыОперацийУХ.БДДС_Расход;
		МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Элемент, "Наименование", "ru = 'Платеж факторинговой компании';
																						|en = ''", КодыЯзыков); // @НСтр-2
		Элемент.Ссылка = СсылкаНаПредопределенныйЭлемент(ИмяПредопределенныхДанных) ;
		ЗаполнитьСвойстваПредопределенногоЭлементаИзМакета(Элемент,СвойстваПредопределенныхЭлементовИзМакета);
		#КонецОбласти //ПлатежФакторинговойКомпании         
...
Функция ПолучитьСтруктуруПараметров() Экспорт
	...
	СтруктураПараметров.Вставить("РасшифровкаПлатежаВТабличнойЧасти");
	СтруктураПараметров.Вставить("ЭтоРасчетыСФизическимЛицом");
	СтруктураПараметров.Вставить("ЭтоРасчетыБезДоговора");
	СтруктураПараметров.Вставить("ЭтоПеремещениеСобственныхСредств");
	СтруктураПараметров.Вставить("ЭтоРасчетыСКонтрагентом");
	СтруктураПараметров.Вставить("ИспользоватьСуммуСтавкуНДС");
	СтруктураПараметров.Вставить("ЭтоРасчетыПоНалогамИСборам");
	СтруктураПараметров.Вставить("ЭтоРасчетыСНаемнымиРаботниками");
	СтруктураПараметров.Вставить("ЭтоИнкассация");
...
Процедура ЗаполнитьПредопределенныеЭлементыПоДанымИзКода()
	...
	Для Каждого ДанныеЭлемента Из ПредопределенныеДанные Цикл
		...
		Если НЕ ЭлементОбъект.ЭтоГруппа Тогда
			СписокРеквизитов = СписокРеквизитов + ",ВидБюджета,Предназначение,ТипОперацииБюджетирование,ПриходРасход,ДопустимБезналичныйРасчет,
			|ВидОперацииДДСБезналичныйРасчет,ДопустимНаличныйРасчет,ВидОперацииДДСНаличныйРасчет,ЭтоПеремещениеСобственныхСредств,
			|ВстречныйВидОперацииПереводСобственныхСредств,ВстречныйВидОперацииВГО,ИспользуетсяВЗаявках,ЭтоРасчетыСКонтрагентом,
			|ЭтоРасчетыСФизическимЛицом,ИспользоватьСуммуСтавкуНДС,ЭтоРасчетыПоЦеннымБумагам,ЭтоКонвертацияВалюты,ОсновнойВидДоговора,
			|ЭтоРасчетыПоНалогамИСборам,ЭтоРасчетыБезДоговора,Использовать,ЭтоОсновнойВидОперацииДляХозяйственнойОперации,ИспользуетсяВПлатежныхПоручениях,
			|РольКонтрагента, ВидДоговора";
		КонецЕсли;
...
// получаем список доступных видов операций с учетом установленных параметров выбора 
Функция ПолучитьСписокДоступныхЗначений(Организация, ПараметрыВыбора)  Экспорт
		
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВидыОперацийУХ.Ссылка КАК Ссылка,
		|	ВидыОперацийУХ.Код КАК Код
		|ИЗ
	...
	Если ПараметрыВыбора.Количество() = 0 Тогда 
		Запрос.УстановитьПараметр("ПараметрыОтбора", Истина);	
	Иначе 
		МассивОтборов = Новый Массив;
		Для Каждого Параметр Из ПараметрыВыбора Цикл
			ИмяПараметра = СтрЗаменить(Параметр.Имя,"Отбор.","");
			МассивОтборов.Добавить(СтрЗаменить(Параметр.Имя,"Отбор", " И ВидыОперацийУХ")+" =&" + ИмяПараметра);
			Запрос.УстановитьПараметр(ИмяПараметра, Параметр.Значение); 
		КонецЦикла; 
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И &ПараметрыОтбора", СтрСоединить(МассивОтборов));
	КонецЕсли;
	РезультатЗапроса 		= Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	СписокДоступныхЗначений = Новый СписокЗначений();
	СписокДоступныхЗначений.ЗагрузитьЗначения(РезультатЗапроса);
...
Функция СвойстваПредопределенныхЭлементовИзМакета(ИмяМакета)
	...
	Для Каждого КлючИЗначение Из ПараметрыВидовОпераций Цикл
		...
		Если ЗначениеЗаполнено(ДанныеОбъекта.ИспользуетсяВЗаявках) Тогда
