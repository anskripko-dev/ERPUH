...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Отбор.Свойство("Владелец") И ЗначениеЗаполнено(Параметры.Отбор.Владелец) Тогда
		...
		ИначеЕсли ТипБД.ВерсияПлатформы=Перечисления.ПлатформыВнешнихИнформационныхБаз.Предприятие77 Тогда
			...
			ОбщегоНазначенияУХ.СообщитьОбОшибке(НСтр("ru = 'Работа с регистрами сведений возможна только на платформе 1С:Предприятие 8.'"), Отказ, , СтатусСообщения.Информация);
			Возврат;
	...
	РаботаСОбъектамиМетаданныхУХ.ПроверитьЗаполнитьСинонимы("РегистрыСведенийБД",ТипБД);
...
Процедура ЗаполнитьСписокРегистровСведений(ТипБД)
	...
	РаботаСОбъектамиМетаданныхУХ.ЗаполнитьСписокРегистровБД(ТипБД,,,,Истина,Истина);
...
Процедура ПометитьНаУдалениеОтсутствующие(Команда)
	ОбщегоНазначенияУХ.ПометитьНаУдалениеОтсутствующие("Справочник.РегистрыСведенийБД");
	Элементы.Список.Обновить();
КонецПроцедуры
