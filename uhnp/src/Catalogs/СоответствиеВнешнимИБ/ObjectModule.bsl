...
Перем ОбновитьИспользованиеИмпорт;
Перем ОбновитьИспользованиеЭкспорт;

Перем СоздатьПравилаКонтроляНСИ;
...
Функция ЗаполнитьСписокПолейПоОписанию()
	...
	Если НЕ ЗначениеЗаполнено(ОписаниеОбъектаТек) Тогда
		
		Возврат мТаблицаВозможныхПолей;
		
	КонецЕсли;
	
	РаботаСОбъектамиМетаданныхУХ.ПроверитьЗаполнитьСинонимы(ОписаниеОбъектаТек);
	...
	Иначе
		...
		Для Каждого СтрИзмерение ИЗ ОписаниеОбъектаТек.Реквизиты Цикл
			...
			Если СтрИзмерение.Имя="Ссылка" Тогда
			...
			Если ТипОбъектаКонсолидации="Справочник"
				ИЛИ ТипОбъектаКонсолидации="ПланВидовХарактеристик" Тогда
	...
	Если ТипОбъектаКонсолидации="Справочник" И НЕ (Использование77() ИЛИ ИмяОбъектаМетаданных = "ПроизвольныйКлассификаторУХ") И ОписаниеОбъектаТек.Владельцы.Количество() > 0  Тогда
			
			НоваяСтрока = Справочники.СоответствиеВнешнимИБ.ДобавитьСтрокуВТаблицу(мТаблицаВозможныхПолей, "Владелец", "Владелец", Ложь);
			
			СписокТипов=Новый СписокЗначений;
			
			Для Каждого ТекВладелец ИЗ ОписаниеОбъектаТек.Владельцы Цикл
				
				СписокТипов.Добавить(ОбщегоНазначенияУХ.ПреобразоватьТипИзСтроки(ТекВладелец.ТипДанных,Истина,Ложь));
	...
	Для Каждого Строка ИЗ ТекТаблицаОписания Цикл
		...
		НоваяСтрока.Использование=Строка.Использование;
...
Процедура ЗаполнитьРеквизитыОбъектаВИБ() Экспорт
	...
	Если НЕ ТипЗнч(ОписаниеОбъектаВИБ)=Тип("СправочникСсылка.ТаблицыADO") Тогда
		
		РаботаСОбъектамиМетаданныхУХ.ПроверитьЗаполнитьСинонимы(ОписаниеОбъектаВИБ);
	...
	Если ТипЗнч(ОписаниеОбъектаВИБ)=Тип("СправочникСсылка.ПеречисленияБД") Тогда
		...
		Иначе
			...
			СтрокаПоказателей.НаименованиеБД = Нстр("ru = 'Наименование'; en = 'Name';tr = 'Ad'");
			СтрокаПоказателей.ПримитивныйТип  = Истина;
			СтрокаПоказателей.ТипМетаДанных    = "Простой";
	...
	ИначеЕсли ТипЗнч(ОписаниеОбъектаВИБ)=Тип("СправочникСсылка.ПланыСчетовБД") Тогда
		...
		СтрокаПоказателей.НаименованиеБД = Нстр("ru = 'Код'; en = 'Code';tr = 'Kod'");
		СтрокаПоказателей.ПримитивныйТип  = Истина;
		СтрокаПоказателей.ТипМетаДанных    = "Простой";
	...
	Если ТипОбъектаВИБ="Справочник" И ОписаниеОбъектаВИБ.Владельцы.Количество()>0 Тогда
		...
		Для Каждого ТекВладелец ИЗ ОписаниеОбъектаВИБ.Владельцы Цикл 
			...
			СтрокаПоказателей.НаименованиеБД = СтрШаблон(Нстр("ru = 'Владелец (%1)'; en = 'Owner (%1)';tr = 'Sahip (%1)'"), ТекВладелец[ПолеСиноним]);
...
Процедура ПередЗаписью(Отказ)
	...
	ОпределитьТаблицыАналитик();
	
	Если ТипОбъектаКонсолидации="ВидСубконто" Тогда
		
		Наименование=ТипОбъектаКонсолидации+"."+ИмяОбъектаМетаданных+" -> "+ТаблицаАналитикиВИБ;
		
	Иначе
		
		Наименование=ТаблицаАналитикиТек+" -> "+ТаблицаАналитикиВИБ;
	...
	Если НЕ ЭтоНовый() Тогда
		...
		ОбновитьИспользованиеИмпорт	= НЕ НеИспользоватьПриИмпорте=Ссылка.НеИспользоватьПриИмпорте;
		ОбновитьИспользованиеЭкспорт= НЕ НеИспользоватьПриЭкспорте=Ссылка.НеИспользоватьПриЭкспорте;
		
	КонецЕсли;
	
	СоздатьПравилаКонтроляНСИ=ЭтоНовый() И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Владелец,"КонтролироватьНСИ"); 
			
КонецПроцедуры 

Процедура ОпределитьТаблицуАналитикиВИБ() Экспорт
	
	Если ОписаниеОбъектаВИБ=Неопределено Тогда
	...
	Если ТипОбъектаВИБ="ТаблицаADO" Тогда
		...
		ТаблицаАналитикиВИБ=ТипОбъектаВИБ+"."+ОписаниеОбъектаВИБ.Имя;
		
	Иначе
		
		ТаблицаАналитикиВИБ=ТипОбъектаВИБ+"."+ОписаниеОбъектаВИБ.Наименование;
		
	КонецЕсли;	
		
КонецПроцедуры // ОпределитьТаблицуАналитикиВИБ() 

Процедура ОпределитьТаблицуАналитикиТек() Экспорт
	...
	Если ТипОбъектаКонсолидации="ВидСубконто" Тогда
		...
		ТаблицаАналитикиТек=Метаданные.НайтиПоТипу(ИмяОбъектаМетаданных.ТипЗначения.Типы()[0]).ПолноеИмя();
		
	Иначе
		
		ТаблицаАналитикиТек=ТипОбъектаКонсолидации+"."+ИмяОбъектаМетаданных;
		
	КонецЕсли;	
		
КонецПроцедуры // ОпределитьТаблицуАналитикиТек()

Процедура ОпределитьТаблицыАналитик() Экспорт
	
	ОпределитьТаблицуАналитикиТек();
	ОпределитьТаблицуАналитикиВИБ();
		
КонецПроцедуры // ОпределитьТаблицыАналитик() 
...
Процедура ПриЗаписи(Отказ)
	...
	Если ОбновитьИспользованиеИмпорт Тогда
		
		ОбновитьПризнакИспользованияИмпорт();
		
	КонецЕсли; 
	
	Если ОбновитьИспользованиеЭкспорт Тогда
		
		ОбновитьПризнакИспользованияЭкспорт();
		
	КонецЕсли;
	
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Владелец,"КонтролироватьНСИ") Тогда
		
		Если СоздатьПравилаКонтроляНСИ Тогда 
			
			МодульОбменаДаннымиНСИ=ОбщегоНазначения.ОбщийМодуль("ОбменДаннымиНСИУХ");	
			МодульОбменаДаннымиНСИ.СоздатьНастройкиПравилКонтроляНСИ(Владелец,Ссылка); 
		...
		Иначе
			...
			НаборЗаписей.Отбор.НастройкаСоответствия.Установить(Ссылка);
...
Процедура ОбновитьПризнакИспользованияИмпорт()
	
	Если НеИспользоватьПриИмпорте Тогда
		...
		Запрос.Текст="ВЫБРАТЬ
		             ...
		             |	Справочник.СоответствиеВнешнимИБ КАК СоответствиеВнешнимИБ
		             |ГДЕ
		             |	НЕ СоответствиеВнешнимИБ.Ссылка = &Ссылка
		             |	И СоответствиеВнешнимИБ.ТаблицаАналитикиВИБ = &ТаблицаАналитикиВИБ
		             |	И СоответствиеВнешнимИБ.Владелец = &Владелец
		             |
		             |УПОРЯДОЧИТЬ ПО
		             |	НеИспользоватьПриИмпорте";
		
		Запрос.УстановитьПараметр("Ссылка",Ссылка);
		Запрос.УстановитьПараметр("ТаблицаАналитикиВИБ",ТаблицаАналитикиВИБ);
		Запрос.УстановитьПараметр("Владелец",Владелец);
		...
		Пока Результат.Следующий() Цикл
			...
			Если НЕ Результат.НеИспользоватьПриИмпорте Тогда
				
				Если ФлагСнят Тогда
					
					СправочникОбъект=Результат.Ссылка.ПолучитьОбъект();
					СправочникОбъект.НеИспользоватьПриИмпорте=Истина;
			...
			Иначе
				...
				Если НЕ ФлагСнят Тогда
					
					СправочникОбъект=Результат.Ссылка.ПолучитьОбъект();
					СправочникОбъект.НеИспользоватьПриИмпорте=Ложь;
					СправочникОбъект.ОбменДанными.Загрузка=Истина;
					СправочникОбъект.Записать();
...
Процедура ОбновитьПризнакИспользованияЭкспорт()
	
	Если НеИспользоватьПриЭкспорте Тогда
		
		Запрос=Новый Запрос;
		Запрос.Текст="ВЫБРАТЬ
		             |	СоответствиеВнешнимИБ.Ссылка КАК Ссылка,
		             |	СоответствиеВнешнимИБ.НеИспользоватьПриЭкспорте КАК НеИспользоватьПриЭкспорте
		             |ИЗ
		             |	Справочник.СоответствиеВнешнимИБ КАК СоответствиеВнешнимИБ
		             |ГДЕ
		             |	НЕ СоответствиеВнешнимИБ.Ссылка = &Ссылка
		             |	И СоответствиеВнешнимИБ.ТаблицаАналитикиТек = &ТаблицаАналитикиТек
		             |	И СоответствиеВнешнимИБ.Владелец = &Владелец
		             |
		             |УПОРЯДОЧИТЬ ПО
		             |	НеИспользоватьПриЭкспорте";
		
		Запрос.УстановитьПараметр("Ссылка",Ссылка);
		Запрос.УстановитьПараметр("ТаблицаАналитикиТек",ТаблицаАналитикиТек);
		Запрос.УстановитьПараметр("Владелец",Владелец);
		...
		Пока Результат.Следующий() Цикл
			...
			Если НЕ Результат.НеИспользоватьПриЭкспорте Тогда
				
				Если ФлагСнят Тогда
					
					СправочникОбъект=Результат.Ссылка.ПолучитьОбъект();
					СправочникОбъект.НеИспользоватьПриЭкспорте=Истина;
					СправочникОбъект.ОбменДанными.Загрузка=Истина;
					СправочникОбъект.Записать();
			...
			Иначе
				...
				Если НЕ ФлагСнят Тогда
					...
					СправочникОбъект=Результат.Ссылка.ПолучитьОбъект();
					СправочникОбъект.НеИспользоватьПриЭкспорте=Ложь;
					СправочникОбъект.ОбменДанными.Загрузка=Истина;
					СправочникОбъект.Записать();
...
Процедура ПередРазворачиваниемСтрокиРеквизитовАналитикиУправляемыйРежим(Строка, Отказ,РежимОтбора=Ложь) Экспорт 
	...
	ИначеЕсли Строка.ТипЗначения.Количество()>1 Тогда
		...
		Для Каждого СтрТип ИЗ Строка.ТипЗначения Цикл
			...
			ТекущаяСтрока.НаименованиеБД   	= Строка.НаименованиеБД+" ("+Тип+")";
...
ОбновитьИспользованиеИмпорт=Ложь;
ОбновитьИспользованиеЭкспорт=Ложь;
