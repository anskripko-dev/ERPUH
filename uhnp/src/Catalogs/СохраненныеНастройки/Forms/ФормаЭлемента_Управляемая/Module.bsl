...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ РежимРедактирования И ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом() Тогда
		СоответствиеФорм = Новый Соответствие;
		СоответствиеФорм.Вставить("СправочникСсылка.УдалитьПанелиОтчетов", "Справочник.УдалитьПанелиОтчетов.Форма.ФормаЭлемента_Управляемая");
		Для каждого ЭлементСоответствия Из СоответствиеФорм Цикл
			Если ТипЗнч(Объект.НастраиваемыйОбъект) = Тип(ЭлементСоответствия.Ключ) Тогда
			    Отказ = Истина;
				СтруктураПараметров = Новый Структура;
				СтруктураПараметров.Вставить("Ключ", Объект.НастраиваемыйОбъект);
				СтруктураПараметров.Вставить("СохраненнаяНастройка", Объект.Ссылка);
				ОткрытьФорму(ЭлементСоответствия.Значение, СтруктураПараметров);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
