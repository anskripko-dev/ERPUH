...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДинамическаяСтруктураЗаказовНаПроизводство = НастройкиПроизводства.ДинамическаяСтруктураЗаказовНаПроизводство;
	//-- НЕ УТ
	
	//++ НЕ УТКА
	ИспользоватьИнтервалПланированияЧас = Константы.ИспользоватьИнтервалПланированияЧас.Получить();
	
	НастроитьСписокВыбораИнтервалов();
	...
	//++ НЕ УТКА
	Если НЕ ПланируетсяГрафикПроизводства И НЕ Объект.ИспользоватьПооперационноеПланирование И НЕ ДинамическаяСтруктураЗаказовНаПроизводство Тогда
...
Процедура УправлениеВидимостью()
	...
	Элементы.ПериодичностьОформленияВыработки.Видимость = Константы.ИспользоватьПериодичностьОформленияВыработки.Получить();
...
Процедура УправлениеДоступностью()
	...
	ДоступноПооперационноеПланирование = НЕ ДинамическаяСтруктураЗаказовНаПроизводство;
	...
	Если РеквизитДоступенДляРедактирования("ИспользоватьСменныеЗадания", ЗапретРедактированияРеквизитов) Тогда
		Элементы.ИспользоватьСменныеЗадания.Доступность = Объект.ИспользоватьПооперационноеУправление
			И (НЕ Объект.ИспользоватьПооперационноеПланирование ИЛИ НЕ ДоступноПооперационноеПланирование);
	КонецЕсли;
		
	Если Не ДоступноПооперационноеПланирование Тогда
		Элементы.ИспользоватьПооперационноеПланирование.Видимость = Ложь;
	Иначе
...
Процедура УстановитьТекстЧасовДнейМесяцевНедель()
	...
	Если Объект.ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Час
		ИЛИ Объект.ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Смена
...
Процедура ИспользоватьГрафикПриИзмененииНаСервере()
	//++ НЕ УТКА
	Если Объект.ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Смена Тогда
		Объект.ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.День;
		ИнтервалПланированияПриИзмененииНаСервере();
	КонецЕсли;
	НастроитьСписокВыбораИнтервалов();
...
&НаСервере
Процедура СохранитьНастройкуИнтервалаПланирования()
	...
	ИначеЕсли Объект.ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Смена Тогда
...
&НаСервере
Процедура ИнициализироватьПараметрыИнтервалаПланирования(Сбросить = Ложь)
...
&НаСервере
Процедура НастроитьСписокВыбораИнтервалов()
	
	Элементы.ИнтервалПланирования.СписокВыбора.Очистить();
	
	Если ИспользоватьИнтервалПланированияЧас
		Или Объект.ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Час Тогда
		Элементы.ИнтервалПланирования.СписокВыбора.Добавить(Перечисления.ТочностьГрафикаПроизводства.Час);
	КонецЕсли;
	
	Если ДинамическаяСтруктураЗаказовНаПроизводство
		И Объект.СпособНастройкиГрафикаРаботы = Перечисления.СпособыНастройкиГрафикаРаботыПодразделений.Смены Тогда
		Элементы.ИнтервалПланирования.СписокВыбора.Добавить(Перечисления.ТочностьГрафикаПроизводства.Смена);
	КонецЕсли;
	
	Элементы.ИнтервалПланирования.СписокВыбора.Добавить(Перечисления.ТочностьГрафикаПроизводства.День);
	Элементы.ИнтервалПланирования.СписокВыбора.Добавить(Перечисления.ТочностьГрафикаПроизводства.Неделя);
	Элементы.ИнтервалПланирования.СписокВыбора.Добавить(Перечисления.ТочностьГрафикаПроизводства.Месяц);
