...
Процедура ПередУдалением(Отказ)
	...
	Если НЕ БезусловноеУдаление Тогда
		...
		Если Отказ Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Нельзя удалить версии для экземпляра отчета %1, содержащего утвержденные данные'"), ЭкземплярОтчета);
