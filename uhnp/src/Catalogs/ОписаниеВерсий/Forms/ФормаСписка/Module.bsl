...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РежимФормы = 2 ИЛИ РежимФормы = 3 тогда
		...
		Если РежимФормы = 2 тогда
			...
			|	И ОписаниеВерсий.УправлениеПериодом.ПериодСценария.ДатаНачала <= &ДатаОкончания
			|	И ОписаниеВерсий.УправлениеПериодом.ПериодСценарияОкончание.ДатаОкончания >= &ДатаНачала
			|	И ОписаниеВерсий.УправлениеПериодом.ТекущаяПериодичность = &ТекущаяПериодичность";
			...
			Элементы.ФормаКоманднаяПанельГруппаРедактирование.ПодчиненныеЭлементы.ФормаКоманднаяПанельКнопкаУдалить.Видимость = Ложь;
			Элементы.СписокКонтекстноеМенюКнопкаУдалить.Видимость = Ложь;
		...
		Если РежимФормы = 2 ИЛИ РежимФормы = 3 тогда
			...
			Список.Параметры.УстановитьЗначениеПараметра("ТекущаяПериодичность", Параметры.ПериодОтчета.Периодичность);
		КонецЕсли;
		...
		КоманднаяПанель.ПодчиненныеЭлементы.СписокУдалитьВерсии.Видимость = ПраваДоступа.ОписаниеВерсий_Удаление;
	...
	Иначе
		...
		Элементы.ФормаКоманднаяПанельКнопкаУдалить.Видимость = ПраваДоступа.ОписаниеВерсий_Удаление;
		Элементы.СписокКонтекстноеМенюКнопкаУдалить.Видимость = ПраваДоступа.ОписаниеВерсий_Удаление;
...
Процедура СписокПриОбновленииСоставаПользовательскихНастроекНаСервере(СтандартнаяОбработка)
	...
	Если ЕстьДатаНачала И ЕстьДатаОкончания тогда
		...
		ЭлементДекорация.Заголовок = НСтр("ru = 'Период с:'");
		ЭлементДекорация.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Лево;
		ЭлементДекорация.РастягиватьПоГоризонтали = Ложь;
		ЭлементДекорация.РастягиватьПоВертикали = Ложь;
		...
		ЭлементДекорация.Заголовок = НСтр("ru = 'по:'");
		ЭлементДекорация.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Лево;
		ЭлементДекорация.РастягиватьПоГоризонтали = Ложь;
		ЭлементДекорация.РастягиватьПоВертикали = Ложь;
...
Процедура УдалитьНепосредственно(Команда)
	...
	Если ВыделенныеСтрокиСписка.Количество() = 1 тогда
		ТекстВопроса = Нстр("ru = 'Вы хотите удалить версию'");
	Иначе
		ТекстВопроса = Нстр("ru = 'Вы хотите удалить версии:'");
	КонецЕсли;
...
Процедура ВыполнитьУдалениеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
    ...
    Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		ЭтаФорма.Модифицированность = Истина;
		Оповестить("ОбработатьУдалениеВерсий", ЭкземплярОтчета);
    КонецЕсли;
...
Процедура ИзменитьПериодЗавершение(РезультатВыбора, ДопПараметры) Экспорт
	...
	Если ТипЗнч(РезультатВыбора) = Тип("СтандартныйПериод") Тогда
		...
		ИспользоватьДатуНачала = ЗначениеЗаполнено(РезультатДатаОкончания);
		Список.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Элементы[ИндексЭлементаДатаНачала].ПравоеЗначение = РезультатДатаОкончания;
		...
		ИспользоватьДатуОкончания = ЗначениеЗаполнено(РезультатДатаНачала);
		Список.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Элементы[ИндексЭлементаДатаОкончания].ПравоеЗначение = РезультатДатаНачала;
