...
//++ НЕ УТКА
	...
	Если РедакторПроизводственногоПроцессаКлиентСервер.РассчитыватьДлительностьАвтоматически(ЭтотОбъект,
		НастройкиПроизводства.ИспользуетсяПроизводство21,
		НастройкиПроизводства.ИспользуетсяПланированиеПоМатериальнымРесурсам,
		НастройкиПроизводства.ИспользуетсяПланированиеПоПроизводственнымРесурсам) Тогда
		ЗаполнитьЕдиницуИзмеренияИДлительностьЭтапаББВ();
	КонецЕсли; 
	
	КоличествоВидыРЦ = ВидыРабочихЦентров.Количество();
	
	Если КоличествоВидыРЦ > 0 Тогда
		...
		Если НастройкиПроизводства.ИспользуетсяПроизводство21
			И КоличествоВидыРЦ > 1 И ПорядокРаботыВидовРабочихЦентров.Пустая() Тогда
			
			ПорядокРаботыВидовРабочихЦентров = Перечисления.ПорядокРаботыВидовРабочихЦентров.Независимо;
			
		ИначеЕсли НастройкиПроизводства.ДинамическаяСтруктураЗаказовНаПроизводство
			И КоличествоВидыРЦ > 1 И ПорядокРаботыВидовРабочихЦентров <> Перечисления.ПорядокРаботыВидовРабочихЦентров.Одновременно Тогда
			
			ПорядокРаботыВидовРабочихЦентров = ?(КоличествоВидыРЦ > 1, Перечисления.ПорядокРаботыВидовРабочихЦентров.Одновременно, Перечисления.ПорядокРаботыВидовРабочихЦентров.ПустаяСсылка());
			ТекстСообщения = НСтр("ru = 'При динамическом планировании не допускается устанавливать порядок работы видов РЦ отличный от ""Одновременно"". Текущий вариант был сброшен.';
									|en = 'For dynamic planning, you cannot set the operating procedure of work center types other than ""Simultaneously"". The current option is reset.'");
...
Процедура ПроверитьЗаполнениеРеквизитовШапки(КлючДанных, СообщитьНаименованиеЭтапа, ЕстьОшибки, ПутьКДанным)
	...
	Если ПланироватьРаботуВидовРабочихЦентров Тогда
		...
		Если ВидыРабочихЦентров.Количество() > 1 
			И НастройкиПроизводства.ИспользуетсяПроизводство22 И Не НастройкиПроизводства.ДинамическаяСтруктураЗаказовНаПроизводство Тогда
...
Процедура ПроверитьЗаполнениеВидовРабочихЦентров(КлючДанных, СообщитьНаименованиеЭтапа, Отказ)
	...
	НомерСтроки = 0;
	КоличествоСтрокИспользуютсяВариантыНаладки = 0;
	...
	Для каждого СтрокаОсновнойВидРЦ Из ВидыРабочихЦентров Цикл
		...
		КоличествоСтрокИспользуютсяВариантыНаладки = КоличествоСтрокИспользуютсяВариантыНаладки + ?(РеквизитыВидаРЦ.ИспользуютсяВариантыНаладки, 1, 0);
	...
	Если ПолучитьФункциональнуюОпцию("ДинамическаяСтруктураЗаказовНаПроизводство")
		И КоличествоСтрокИспользуютсяВариантыНаладки > 1 Тогда
		ТекстСообщения = НСтр("ru = 'При включенной динамической структуре заказов на производство, запрещено задавать более одного ВРЦ, использующего варианты наладки.';
								|en = 'When the dynamic production order structure is enabled, you cannot specify more than one work center type using setup settings.'");
