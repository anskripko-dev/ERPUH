...
//  ГруппаЭтаповПроцесса - СправочникСсылка.ЭтапыПодготовкиБюджетов - группа этапов бюджетного процесса
...
//
Функция ПолучитьБюджетныеЗадачи(МодельБюджетирования, ГруппаЭтаповПроцесса, Знач НачалоПериода, Знач КонецПериода,
...
Процедура ПроверитьЗаполнениеГрафиковЗаПериод(ДатаНачала, ДатаОкончания, График, Отказ)
	...
	ПараметрыЖурнала.ГруппаСобытий = НСтр("ru = 'Формирование бюджетных задач';
											|en = 'Budget task generation'");
	ПараметрыЖурнала.Метаданные = Метаданные.НайтиПоТипу(Тип("РегистрСведенийНаборЗаписей.КалендарныеГрафики"));
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СообщениеОбОшибке, Формат(
			ВыборкаДетальныеЗаписи.Год, "ЧГ="));
		ОбщегоНазначенияУТ.ЗаписатьВЖурналСообщитьПользователю(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка, "",
			СообщениеОбОшибке);
		Отказ = Истина;
	КонецЦикла;
...
Функция ПолучитьПериодЗадачи(Выборка, ТаблицаЗадач, МенеджерГрафика, 
	...
	Если Выборка.УсловиеЗапуска = Перечисления.ТипыУсловийЗапускаЭтаповПодготовкиБюджетов.Вручную Тогда
		ДатаНачалаЗадачи = НачалоДня(ТекущаяДатаСеанса());
		Возврат Новый Структура("ДатаНачалаЗадачи, ДатаОкончанияЗадачи", ДатаНачалаЗадачи, КонецДня(ДатаНачалаЗадачи));
	КонецЕсли;
...
Функция ОбработатьИсполнениеШагаПроцесса(Выборка,
	...
	Если Не Выборка.ЭтоГруппа Тогда
		...
		Если Не ПропуститьЭтап
			И Выборка.УсловиеЗапуска = Перечисления.ТипыУсловийЗапускаЭтаповПодготовкиБюджетов.Вручную Тогда
...
Процедура ДополнитьДействующимиПроцессамиПодготовкиБюджетов(ДеревоШагов, МодельБюджетирования, ГруппаЭтаповПроцесса,
	...
	Запрос.УстановитьПараметр("МодельБюджетирования", МодельБюджетирования);
	Если ЗначениеЗаполнено(ГруппаЭтаповПроцесса) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоГруппеЭтаповПроцесса",
			"ПроцессПодготовкиБюджетов.ГруппаЭтапов В ИЕРАРХИИ (&ГруппаЭтаповПроцесса)"); // query-part
		Запрос.УстановитьПараметр("ГруппаЭтаповПроцесса", ГруппаЭтаповПроцесса);
	Иначе
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если НастройкиИерархии.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
...
// Параметры:
// 	МодельБюджетирования - СправочникСсылка.МоделиБюджетирования - 
// 	ГруппаЭтаповПроцесса - СправочникСсылка.ЭтапыПодготовкиБюджетов - 
// Возвращаемое значение:
...
// 	 *ПроцессПодготовкиБюджетов - ДокументСсылка.ПроцессПодготовкиБюджетов - 
//
Функция ТаблицаФактЗадачПоМодели(МодельБюджетирования, ГруппаЭтаповПроцесса)
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	Если ЗначениеЗаполнено(ГруппаЭтаповПроцесса) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоГруппеЭтаповПроцесса",
			"БюджетнаяЗадача.ЭтапПодготовкиБюджетов В ИЕРАРХИИ (&ГруппаЭтаповПроцесса)"); // query-part
		Запрос.УстановитьПараметр("ГруппаЭтаповПроцесса", ГруппаЭтаповПроцесса);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоГруппеЭтаповПроцесса", "ИСТИНА");
	КонецЕсли;
...
// 	 *ЭтапПодготовкиБюджетовОснование - СправочникСсылка.ЭтапыПодготовкиБюджетов - 
//
Функция ДеревоШаговПоМодели(МодельБюджетирования, ГруппаЭтаповПроцесса)
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|	ЭтапыПодготовкиБюджетов.НастройкаДействия КАК НастройкаДействия,
		|	ВЫБОР
		|		КОГДА ЭтапыПодготовкиБюджетов.Ссылка = &ГруппаЭтаповПроцесса
		|			ТОГДА NULL
		|		ИНАЧЕ ЭтапыПодготовкиБюджетов.Родитель.Периодичность
		|	КОНЕЦ КАК ПериодичностьРодитель,
		|	ВЫБОР
		|		КОГДА ЭтапыПодготовкиБюджетов.Ссылка = &ГруппаЭтаповПроцесса
		|			ТОГДА NULL
		|		ИНАЧЕ ЭтапыПодготовкиБюджетов.Родитель.ПорядокВыполненияЭтапов
		|	КОНЕЦ КАК ПорядокВыполненияЭтапов,
		|	ВЫБОР
		|		КОГДА ЭтапыПодготовкиБюджетов.Ссылка = &ГруппаЭтаповПроцесса
		|			ТОГДА NULL
		|		ИНАЧЕ ЭтапыПодготовкиБюджетов.Родитель.УсловиеЗапуска
		|	КОНЕЦ КАК УсловиеЗапуска,
		|	ВЫБОР
		|		КОГДА ЭтапыПодготовкиБюджетов.Ссылка = &ГруппаЭтаповПроцесса
		|			ТОГДА NULL
		|		ИНАЧЕ ЭтапыПодготовкиБюджетов.Родитель.Срок
		|	КОНЕЦ КАК Срок,
		|	ВЫБОР
		|		КОГДА ЭтапыПодготовкиБюджетов.Ссылка = &ГруппаЭтаповПроцесса
		|			ТОГДА NULL
		|		ИНАЧЕ ЭтапыПодготовкиБюджетов.Родитель.ТипСрока
		|	КОНЕЦ КАК ТипСрока,
	...
	Если ЗначениеЗаполнено(ГруппаЭтаповПроцесса) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоГруппеЭтаповПроцесса",
			"ЭтапыПодготовкиБюджетов.Ссылка В ИЕРАРХИИ (&ГруппаЭтаповПроцесса)"); // query-part
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоГруппеЭтаповПроцесса", "ИСТИНА");
	КонецЕсли;
...
//
Функция ЗадачиБюджетирования(МодельБюджетирования, ГруппаЭтаповПроцесса, ДатаНачала, ДатаОкончания, ДополнятьФактом, ПроверятьЗадачиДругихПериодов)
	...
	ДеревоШагов = ДеревоШаговПоМодели(МодельБюджетирования, ГруппаЭтаповПроцесса);
	...
	Если ДополнятьФактом Тогда
		...
		ТаблицаФакт = ТаблицаФактЗадачПоМодели(МодельБюджетирования, ГруппаЭтаповПроцесса);
