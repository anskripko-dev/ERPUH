...
Процедура ВвестиТранзитныйСчетНажатие(Элемент)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ТекстСообщения = НСтр("ru = 'Счет будет записан перед вводом транзитного счета.';
								|en = 'The account will be saved before entering a transit account.'");
	ИначеЕсли Модифицированность Тогда
		ТекстСообщения = НСтр("ru = 'Счет изменен и будет записан перед вводом транзитного счета.';
								|en = 'The account is changed and will be saved before entering a transit account.'");
	КонецЕсли;
...
Процедура НастроитьЭлементыФормы(Форма, ИзмененныйРеквизит = "", ДополнительныеРеквизиты = Неопределено)
	...
	Форма.Элементы.РазрешитьПлатежиБезУказанияЗаявок.Заголовок = НСтр("ru = 'Разрешить списание денежных средств без документов планирования'");
	Форма.Элементы.РазрешитьПлатежиБезУказанияЗаявок.Подсказка = НСтр("ru = 'Загружаемые из других программ документы списания денежных средств будут автоматически проводиться, только если разрешено списание без ""документов планирования""'");	
	#КонецОбласти
...
&НаКлиенте
Процедура Подключаемый_ИнструментРазмещенияСвободныхОстатковПриИзмененииУХ(Элемент)
	ВстраиваниеУХБанковскиеСчетаОрганизацийКлиентСервер.УправлениеЭлементамиФормыУХ(ЭтотОбъект);
КонецПроцедуры 
