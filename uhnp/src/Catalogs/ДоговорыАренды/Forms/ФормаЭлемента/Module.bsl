...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ДоговорыКонтрагентовФормыУХКлиент.ОбработатьОповещенияДоговораЛизинга(ЭтотОбъект, ИмяСобытия, Параметр, Источник);
	#КонецОбласти     
...
Процедура ТипДоговораПриИзменении(Элемент)
	...
	#Область УХ_Встраивание
	ТипДоговораПриИзмененииУХ(Элемент);	
	#КонецОбласти
...
Процедура ПараметрыУчетаФИПриИзменении(Элемент)
	...
	Элементы.ДекорацияПараметрыУчетаФИ.Видимость = не Объект.ПараметрыУчетаФИ.Пустая();
	Если Не Элементы.ДекорацияПараметрыУчетаФИ.Видимость Тогда
		Возврат;
	КонецЕсли;
	
	Описание = МСФОВызовСервераУХ.ПолучитьОписаниеДекорацииПараметровФИ(ФИ, Объект.ПараметрыУчетаФИ, ЭтотОбъект.ВыборПараметровУчетаФИУХ);	
	ЗаполнитьЗначенияСвойств(Элементы.ДекорацияПараметрыУчетаФИ, Описание);
	
	ДоговорыКонтрагентовФормыУХКлиентСервер.УправлениеФормой_ПараметрыУчетаФИ(ЭтотОбъект);
	
КонецПроцедуры

&НаКлиенте
Процедура ТипДоговораПриИзмененииУХ(Элемент)
	
	Если Объект.ТипДоговора = ПредопределенноеЗначение("Перечисление.ТипыДоговоровАренды.Лизинг") Тогда
		
		Объект.ВидДоговораУХ = ПредопределенноеЗначение("Справочник.ВидыДоговоровКонтрагентовУХ.ЛизингПолученный");
		
	ИначеЕсли Объект.ТипДоговора = ПредопределенноеЗначение("Перечисление.ТипыДоговоровАренды.Аренда") Тогда
		
		Объект.ВидДоговораУХ = ПредопределенноеЗначение("Справочник.ВидыДоговоровКонтрагентовУХ.Аренда");
...
&НаКлиенте
Процедура Подключаемый_АвтоматическоеСозданиеПлатежныхПозицийПриИзменении(Элемент)
	
	Если ЭтотОбъект.АвтоматическоеСозданиеПлатежныхПозиций Тогда		
...
&НаСервере
Процедура ОпределитьСостояниеОбъекта(ОбновитьОтветственныхВход = Ложь) Экспорт
