...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Элемент, "Наименование", "ru = 'Без НДС';
																					|en = 'Excluding VAT'", КодыЯзыков); // @НСтр-2
...
Процедура СоздатьЭлементыПервоначальногоЗаполнения(ПервыйЗапуск = Ложь) Экспорт
	...
	Попытка
		...
		ОбщегоНазначения.УстановитьЗначениеРеквизита(СтавкаБезНДСОбъект, "Наименование", НСтр("ru = 'Без НДС';
																								|en = 'Excluding VAT'", ОбщегоНазначения.КодОсновногоЯзыка()));
