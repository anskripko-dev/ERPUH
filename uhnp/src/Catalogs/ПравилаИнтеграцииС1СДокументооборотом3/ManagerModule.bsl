...
Функция РеквизитыОбъектаДО(ТипОбъектаДО, ВидДокументаДОID, ПолучитьВсе, НастройкиВида = Неопределено,
	...
	Если ИнтеграцияС1СДокументооборот3.ЭтоДокументДО3(ТипОбъектаДО) Тогда
		...
		ДобавитьРеквизит(Реквизиты, "deletionMark", НСтр("ru = 'Пометка удаления';
														|en = 'Deletion mark'"), "Булево");
		...
		Если ПолучитьВсе Или НастройкиВида.productRowsEnabled = Истина Тогда
			...
			Если ИнтеграцияС1СДокументооборотБазоваяФункциональностьПовтИсп.ДоступенФункционалВерсииСервиса("3.0.16.33") Тогда
				ДобавитьРеквизитТаблицы(Реквизиты,
					"productRows.rows", "VATRateUniversal", НСтр("ru = 'Ставка НДС';
																|en = 'VAT rate'"), "DMVATRateUniversal");
			Иначе
				ДобавитьРеквизитТаблицы(Реквизиты,
					...
					Если ЗначениеЗаполнено(Строка[Колонка.Имя]) И Колонка.Имя <> "НомерСтроки" Тогда
						Если КолонкиТаблицы.Найти(Колонка.Имя) = Неопределено Тогда
							ВызватьИсключение СтрШаблон(НСтр("ru = 'Ошибка заполнения таблицы %1 из выражения на встроенном языке.
									...
									|en = 'Ошибка заполнения таблицы %1 из выражения на встроенном языке.
									|В таблице отсутствует колонка %2, которую возвращает результат выражения.'"),
								ПравилоТаблицы.ИмяРеквизитаОбъектаИС,
								Колонка.Имя);
						КонецЕсли;
...
Процедура ДобавитьДополнительныйРеквизит(Реквизиты, Представление, Тип, ДопРеквизитID, ДопРеквизитТип)
	...
	НоваяСтрока.Представление = СтрШаблон(НСтр("ru = 'Доп. реквизит: %1';
												|en = 'Additional attribute: %1'"), Представление);
