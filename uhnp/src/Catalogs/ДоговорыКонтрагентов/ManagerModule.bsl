...
Процедура ЗаполнитьБанковскиеСчетаПоДоговору(Договор, БанковскийСчетОрганизации, БанковскийСчетКонтрагента) Экспорт
	...
	Если Метаданные.ОбщиеМодули.Найти("УправлениеФормойНСИСервер") <> Неопределено Тогда
		...
		Если МодульУправлениеНСИСервер.ЭлементНСИНеАктивный(БанковскийСчетОрганизации) Тогда
			БанковскийСчетОрганизации = Справочники.БанковскиеСчетаОрганизаций.ПустаяСсылка();
		КонецЕсли;
		Если МодульУправлениеНСИСервер.ЭлементНСИНеАктивный(БанковскийСчетКонтрагента) Тогда
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Для Каждого Договор Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			ОчиститьПереоцениватьТоварыУслугиКОтчетуКомитентуВВерсияхСоглашения(Договор.Ссылка);	
			#КонецОбласти
...
#Область УХ_Казначейство
Процедура ОчиститьПереоцениватьТоварыУслугиКОтчетуКомитентуВВерсияхСоглашения(ДоговорСсылка)
	...
	Пока Выборка.Следующий() Цикл
		...
		ЭлементБлокировки = Блокировка.Добавить(Метаданные.Документы.ВерсияСоглашенияКоммерческийДоговор.ПолноеИмя());
		ЭлементБлокировки.УстановитьЗначение("Ссылка", Выборка.Ссылка);
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		
		Блокировка.Заблокировать();
		
		ВерсияОбъект = Выборка.Ссылка.ПолучитьОбъект(); // ДоговорОбъект
		ВерсияОбъект.ПереоцениватьТоварыУслугиКОтчетуКомитенту = Ложь;
		ОбновлениеИнформационнойБазы.ЗаписатьДанные(ВерсияОбъект);
