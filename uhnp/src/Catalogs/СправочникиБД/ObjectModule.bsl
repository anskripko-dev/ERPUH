...
Процедура ЗаполнитьРеквизитыОбъекта(Кэш = Неопределено, ЕстьИзменения = Истина) Экспорт
	...
	Иначе
		...
		Если Метаданные.Справочники.Найти(Наименование) = Неопределено Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не найден справочник %Наименование%. Возможно, требуется обновить данные по метаданным текущей ИБ.'; en = 'The %Наименование% catalog was not found. You might need to update data by metadata of the current infobase.';tr = '%Наименование% kataloğu bulunamadı. Mevcut infobase''in meta verilerine göre verileri güncellemeniz gerekebilir.'");
	...
	СтруктураОписания = РаботаСОбъектамиМетаданныхУХ.ПолучитьСтруктуруОписанияСправочникаБД(
	...
	РаботаСОбъектамиМетаданныхУХ.ЗаполнитьИзмененныеРеквизиты(ЭтотОбъект, СтруктураОписания, "Реквизиты,Владельцы,ВидИерархии,ДополнительныеРеквизиты,ДополнительныеСведения,НаборыРеквизитовИСведений,Синоним", ЕстьИзменения);
	...
	Для Каждого СтрРеквизит ИЗ СтруктураОписания.Реквизиты Цикл
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрРеквизит,, "Использование,Синоним");
	...
	КонецЦикла;	
	РаботаСОбъектамиМетаданныхУХ.ЗагрузитьИзмененнуюТабЧасть(ЭтотОбъект.Владельцы, ТабНовыеВладельцы, ЕстьИзменения);
	
	// *** Дополнительные реквизиты и сведения ***
	
	Если НЕ Использование77() Тогда
		
		РаботаСОбъектамиМетаданныхУХ.ЗагрузитьДополнительныеРеквизитыИСведения(ЭтотОбъект,СтруктураОписания, ЕстьИзменения);
...
Процедура ПередЗаписью(Отказ)
	...
	Если Владелец = Справочники.ТипыБазДанных.ТекущаяИБ  Тогда
		...
		Если Не ЗначениеЗаполнено(РеквизитСостояния) Тогда
			...
			ИначеЕсли Наименование = "Проекты" Тогда
				...
				РеквизитСостояния = Справочники.ПеречисленияБД.НайтиПоНаименованию(РеквизитСостоянияСтрока,,,Справочники.ТипыБазДанных.ТекущаяИБ);
			ИначеЕсли Наименование = "ИнвестиционныеПрограммы" Тогда
			...
			ИначеЕсли Наименование = "Лоты" Тогда
				...
				ПеречисленияСтатусыЛотов="СтатусыЛотов";
				
				ЗначениеРеквизитаСостоянияЧерновик =  Перечисления[ПеречисленияСтатусыЛотов].Черновик;
				ЗначениеРеквизитаСостоянияНаУтверждении = Перечисления[ПеречисленияСтатусыЛотов].НаУтверждении;
				ЗначениеРеквизитаСостоянияУтвержден = Перечисления[ПеречисленияСтатусыЛотов].ВключенВГПЗ;
				ЗначениеРеквизитаСостоянияОтклонен = Перечисления[ПеречисленияСтатусыЛотов].Черновик;	
	...
	ЕстьРеквизитыИсторииИзменений	= ЕстьРеквизитЦНСИ("НСИ_ПоследнийРедактировавший");	
	...
	Если Владелец = Справочники.ТипыБазДанных.ТекущаяИБ
		И ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ИнтеграцияВИБ") 
		И НЕ ЗначениеЗаполнено(ЭтотОбъект["СогласованиеСвязанныхОбъектов"]) Тогда
		
		НеСогласовыватьСтрока = "Перечисление.СпособыСогласованияСвязанныхОбъектов.НеСогласовывать";
		СогласованиеСвязанныхОбъектов = ПредопределенноеЗначение(НеСогласовыватьСтрока);
