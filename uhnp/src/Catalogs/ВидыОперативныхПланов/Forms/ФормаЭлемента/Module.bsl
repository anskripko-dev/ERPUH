...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Справочники.ВидыОперативныхПланов.УстановитьУсловноеОформлениеНастройкиАналитик(ЭтаФорма, Объект.ВидБюджета);
	
	УстановитьУсловноеОформлениеАналитикСтатьиБюджета();

	УстановитьЗапретИзмененияФормы();
	
	ПараметрыПланирования = ОперативноеПланированиеПовтИспУХ.ПолучитьПараметрыОперПланирования(Объект.ВидБюджета);
    ПериодичностьПланирования = ПараметрыПланирования.ПериодичностьОперативногоПланирования;
...
Процедура АналитикиПриАктивизацииСтроки(Элемент)
	...
	Если ТД.Аналитика = ПредопределенноеЗначение("Справочник.АналитикиОперативногоПланирования.СтатьяБюджета") Тогда
		Если Объект.ВидБюджета = ПредопределенноеЗначение("ПланВидовХарактеристик.ВидыБюджетов.БюджетДвиженияДенежныхСредств") Тогда
			ЭлементСтатья.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.СтатьиДвиженияДенежныхСредств");
		ИначеЕсли Объект.ВидБюджета = ПредопределенноеЗначение("ПланВидовХарактеристик.ВидыБюджетов.БюджетДоходовИРасходов") Тогда
...
&НаСервере
Функция ПолучитьПВХПоВидуАналитики(Аналитика)
	Возврат Аналитика.ТипДанных;
КонецФункции

&НаКлиенте
Процедура АналитикиАналитикаПриИзменении(Элемент)
	ПВХ = ПолучитьПВХПоВидуАналитики(Элементы.НастройкаАналитик.ТекущиеДанные.Аналитика);
КонецПроцедуры
...
Процедура УправлениеФормой(Форма)
	...
	//
	ВидимостьКоличества = Объект.ВидБюджета = ПредопределенноеЗначение("ПланВидовХарактеристик.ВидыБюджетов.БюджетДоходовИРасходов");
	...
	Элементы.ЗаполнитьПериодичность.Видимость = Форма.ПериодичностьПланирования <> Объект.Периодичность;
...
Процедура ПерезаполнитьПериодичностьНаСервере()
	...
	Объект.Периодичность = ПериодичностьПланирования;
	Объект.ПервыйПериод = Справочники.Периоды.ПустаяСсылка();
	Объект.КоличествоПериодов = 0;
