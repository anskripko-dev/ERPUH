...
Функция ЕстьДокументыПоОперПлану(ВариантНастройки) экспорт
	...
	|ИЗ
	|	Документ.УдалитьОперативныйПлан КАК ОперативныйПлан
	|ГДЕ
	|	ОперативныйПлан.Проведен = ИСТИНА
	|	И ОперативныйПлан.ВидОперативногоПлана = &ВариантНастройки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	ОперативныйПлан.Проведен = ИСТИНА
	|	И ОперативныйПлан.ВариантНастройки = &ВариантНастройки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И Резервирование.ВариантНастройки = &ВариантНастройки";
...
Процедура УстановитьУсловноеОформлениеНастройкиАналитик(Форма, ДанныеВРеквизитеФормы = Ложь) экспорт
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, Элементы.НастройкаАналитикЗначениеАналитики.Имя);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		ПрефиксДанных + "НастройкаАналитик.Использование", ВидСравненияКомпоновкиДанных.ВСписке, СписокАналитикСоЗначением);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр",	Истина);
...
Процедура ЗаполнитьВидОперПланаПоВидуБюджета(ВидОперПлана) Экспорт
	...
	ВидОперПлана.УдалитьПредназначение = ПланыВидовХарактеристик.ВидыБюджетов.Предназначение(ВидОперПлана.ВидБюджета);
	...
	Если ВидОперПлана.Ссылка = Справочники.ВидыОперативныхПланов.КонсолидированныйПланПотребностей Тогда
		...
		ДобавитьАналитику(ВидОперПлана, Аналитики.ПриходРасход,			Использование.НеИспользуется, Истина,,ПриходРасход.Приход);
	...
	|ИЗ
	|	РегистрСведений.ПараметрыЛимитирования.СрезПоследних(, ВидБюджета = &ВидБюджета) КАК ПараметрыЛимитированияСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Выборка.Следующий();
	ВидОперПлана.Периодичность = Выборка.ПериодичностьПланирования;	    
	
	Выборка = Результаты[1].Выбрать();
	Пока Выборка.Следующий() Цикл
		Параметры = ПолучитьПараметрыАналитикиДляБюджета(ВидОперПлана.ВидБюджета, Выборка.Аналитика);
		ДобавитьАналитику(ВидОперПлана, Выборка.Аналитика,	Параметры.Использование, Выборка.ОбязательноеЗаполнение, 
						  Параметры.ГруппаСтрок, Параметры.ПоУмолчанию, Выборка.КонтрольЛимитов, Выборка.ТипДанных);
	КонецЦикла;
...
Процедура ПервоначальноеЗаполнение() Экспорт
	...
	ВидОперПлана.УдалитьПредназначение = ПланыВидовХарактеристик.ВидыБюджетов.Предназначение(ВидБюджета);
...
Функция ПолучитьПараметрыАналитикиДляБюджета(ВидБюджета, Аналитика)
	...
	ИначеЕсли Аналитика = Аналитики.ПриходРасход Тогда
		Результат.ПоУмолчанию = ПриходРасход.Расход;
