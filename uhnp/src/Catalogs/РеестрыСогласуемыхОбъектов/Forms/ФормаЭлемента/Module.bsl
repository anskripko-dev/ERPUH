...
Процедура ЗаполнитьРеестрНаСервере()
	...
	|	И СогласуемыеОбъекты.ПометкаУдаления = ЛОЖЬ";
	Если ИмяСправочника = "ДокументыБД" Тогда
		Запрос.Текст = Запрос.Текст + Символы.ПС +"	И СогласуемыеОбъекты.Проведен = ИСТИНА";
	КонецЕсли;
