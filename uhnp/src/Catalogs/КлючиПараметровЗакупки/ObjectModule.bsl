...
Процедура ПередЗаписью(Отказ)
	...
	Для Каждого Параметр Из ПараметрыЗакупки Цикл               
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ЗапретИзменения_"+Параметр.ПараметрИмя, ЭтотОбъект.Метаданные()) Тогда
			ЭтотОбъект["ЗапретИзменения_"+Параметр.ПараметрИмя] = ПроверитьБит(ФлагиВозможностиИзменения, Параметр.ПараметрНомер-ПараметрыЗакупки[0].ПараметрНомер);  
		КонецЕсли;
	КонецЦикла;
