...
Процедура ПроверитьПодразделениеИДолжность(Ссылка, ДатаУтверждения, Подразделение, Должность, Отказ)
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.ОшибкаПодразделение Тогда
			ТекстСообщения = НСтр(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ru = 'Подразделение %1 уже расформировано.';
																							|en = 'The operating unit %1 is already disbanded.'", Выборка.ПодразделениеНаименование));
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.Подразделение", , Отказ);
		КонецЕсли;
		
		Если Выборка.ОшибкаДолжность Тогда
			ТекстСообщения = НСтр(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ru = 'Должность %1 уже исключена из штатного расписания.';
																							|en = 'The position %1 is already excluded from headcount.'", Выборка.ДолжностьНаименование));
	...
	Пока ВыборкаПоНачислениям.СледующийПоЗначениюПоля("НомерСтроки") Цикл
		...
		Пока ВыборкаПоНачислениям.Следующий() Цикл
			...
			Если ВыборкаПоНачислениям.ОшибкаПоказателя Тогда
				ТекстСообщения = НСтр(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ru = 'Минимальное значение показателя ""%1"" не может быть больше максимального';
																								|en = 'The minimum value of the ""%1"" indicator cannot exceed the maximum one'", ВыборкаПоНачислениям.ПоказательНаименование));
