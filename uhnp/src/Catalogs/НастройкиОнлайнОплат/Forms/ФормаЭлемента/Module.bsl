...
Процедура ОткрытьСтраницуШаблоныСообщений()
	...
	Если ЕстьОтправкаПисем И ЕстьОтправкаSMS Тогда
		...
		Заголовок = НСтр("ru = 'Выставление счетов клиентам по электронной почте и SMS';
	...
	ИначеЕсли ЕстьОтправкаПисем Тогда
		ПредставлениеТиповШаблонов = НСтр("ru = 'писем';
											|en = 'emails'");
		Заголовок = НСтр("ru = 'Выставление счетов клиентам по электронной почте';
						|en = 'Issue customer invoices via email'");;
	ИначеЕсли ЕстьОтправкаSMS Тогда
		ПредставлениеТиповШаблонов = НСтр("ru = 'SMS';
											|en = 'SMS'");
		Заголовок = НСтр("ru = 'Выставление счетов клиентам по SMS';
...
Процедура ПроверитьДоступНаСервереЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если РезультатПроверки.НастройкаВыполнена Тогда
		ДоступККассеПредоставлен = Истина;
	...
	Иначе
		...
		Если ПустаяСтрока(РезультатПроверки.СообщениеОбОшибке) Тогда
			ТекстСообщения = НСтр("ru = 'Ошибка доступа, проверьте настройки.';
									|en = 'Access error. Check the settings.'");
		Иначе
			ТекстСообщения = РезультатПроверки.СообщениеОбОшибке;
		КонецЕсли;
	КонецЕсли;
...
Процедура СоздатьНастройкуОнлайнОплаты(Отказ)
	...
	Попытка
		...
		НастройкиОнлайнОплатОбъект.Заполнить(Неопределено);
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru = 'Ошибка при записи настройки по ЮKassa';
								|en = 'Error while saving YooMoney setting'");
...
Процедура УстановитьОтображениеЭлементовФормы()
	...
	Если Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаОСервисе Тогда
		...
		Заголовок = НСтр("ru = 'Настройка подключения к ЮКаssа';
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаДлительнаяОперация Тогда
		Элементы.КнопкаНазад.Видимость = Истина;
		Элементы.КнопкаДалее.Видимость = ДоступККассеПредоставлен;
		ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиФормы.Нет;
