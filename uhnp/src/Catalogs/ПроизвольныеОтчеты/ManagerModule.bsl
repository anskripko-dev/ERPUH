	...
	Для каждого ПолеРасшифровки Из МассивПолейРасшифровкиСРесурсами Цикл
		Если ТипЗнч(ПолеРасшифровки) = Тип("ЗначениеПоляРасшифровкиКомпоновкиДанных") Тогда 
			...
			Если ПолеРасшифровки.Поле = "Источник" 
				...
				И ТипЗнч(ПолеРасшифровки.Значение) = Тип("СправочникСсылка.ИсточникиДанныхДляРасчетов") Тогда
...
Процедура ОбновитьОтчет(Результат, Объект, СохраненнаяНастройка = Неопределено, ДанныеРасшифровки, ОбщийОтбор = Неопределено, ПараметрыОтчета = Неопределено, URLСхемы, ВнешнийКонтекст = Неопределено, ИмяОтчета = "", Знач ИспользоватьДанныеРегистра = Ложь, НастройкиКомпоновщика = Неопределено, ГрафСхема = Неопределено) Экспорт
	...
	Если Объект.ВидПроизвольногоОтчета = 0 Тогда
		...
		ОбновитьДанныеАналитическогоОтчетаНаборДанных(URLСхемы,Объект,Результат,ОбщийОтбор, ПараметрыОтчета,ИспользоватьДанныеРегистра,СохраненнаяНастройка,НастройкиКомпоновщика,ДанныеРасшифровки);
				
	ИначеЕсли Объект.ВидПроизвольногоОтчета = 1 Тогда // Монитор ключевых показателей.	
		
		ТаблицаПоказателей = Новый ТаблицаЗначений;
		ТЗОтвета           = Новый ТаблицаЗначений;
		ОбновитьДанныеМонитораКлючевыхПоказателей(Объект,ИмяОтчета,ОбщийОтбор,НастройкиКомпоновщика, ПараметрыОтчета,ИспользоватьДанныеРегистра,СохраненнаяНастройка,ГрафСхема,ТЗОтвета);
...
Процедура ОбновитьДанныеМонитораКлючевыхПоказателей(Объект,ИмяОтчета,ОбщийОтбор,НастройкиКомпоновщика, ПараметрыОтчета,ИспользоватьДанныеРегистра,СохраненнаяНастройка,ГрафСхема,ТЗОтвета)
...
Процедура ОбновитьДанныеАналитическогоОтчетаНаборДанных(URLСхемы,Объект,Результат,ОбщийОтбор, ПараметрыОтчета,ИспользоватьДанныеРегистра,СохраненнаяНастройка,НастройкиКомпоновщика,ДанныеРасшифровки)
	...
	КомпоновщикНастроек.ЗагрузитьНастройки(СохраненнаяНастройка.НастройкиКомпоновщика);
	...
	//Соберем единую таблицу из разных источников
	ИсточникОбъект  = Объект.ИсточникДанныхОтчета.ПолучитьОбъект();
	ТаблицаЗначений = ПолучитьТаблицуПоНаборуДанных(Объект,ОбщийОтбор,ИсточникОбъект);
	//
...
Функция ПолучитьТаблицуПоНаборуДанных(Объект,ОбщийОтбор,ИсточникОбъект)
		...
		Для Каждого ВложенныйПериод Из ПериодыОтбора Цикл
			...
			Для Каждого Параметр Из ОбщийОтбор Цикл
				...
				ИначеЕсли Параметр.Ключ = "ПериодОтчета" Тогда
					Контекст.Вставить(Строка(Параметр.Ключ), ВложенныйПериод);
				КонецЕсли;	
	...
	Для Каждого СтрАналитика Из СтруктураУникальныхАналитик Цикл
		ТаблицаЗначений.Колонки.Добавить(СтрАналитика.Ключ,Новый ОписаниеТипов(СтрАналитика.Значение));					
	КонецЦикла;
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Ключ") И ЗначениеЗаполнено(Параметры.Ключ) Тогда
		...
		Если  Параметры.Ключ.ВидПроизвольногоОтчета = 0  Тогда
			ВыбраннаяФорма = "ФормаНастройкиИсточник";
		ИначеЕсли Параметры.Ключ.ВидПроизвольногоОтчета = 1  Тогда	 
			ВыбраннаяФорма = "ФормаНастройкиМКП";
		Иначе
			ВыбраннаяФорма = "ФормаНастройкиБланк";
		КонецЕсли;
...
Процедура КонвертироватьИсточникиДанныхПроизвольныхОтчетов() Экспорт
   ...
   Запрос.Текст = "ВЫБРАТЬ
                  ...
                  |ГДЕ
                  |	ПроизвольныеОтчеты.ПометкаУдаления = ЛОЖЬ
                  |	И ПроизвольныеОтчеты.ВидПроизвольногоОтчета = 0";
   ...
   Пока Выборка.Следующий() Цикл
	   ...
	   Если ТипЗнч(Выборка.ИсточникДанныхОтчета) = Тип("СправочникСсылка.ИсточникиДанныхДляРасчетов") 
		   И ЗначениеЗаполнено(Выборка.ИсточникДанныхОтчета) Тогда
		   Попытка
			   
			   КонвертироватьИсточникВПакет(Выборка.Ссылка);		
			   
		   Исключение	 
			   
			   ТекстСообщения = НСтр("ru = 'При конвертации источника данных аналитического отчета %Отчет% произошла ошибка
			   |			%ОписаниеОшибки%'");
			   ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Отчет%", Строка(Выборка.Ссылка));
			   ...
			   Метаданные.Справочники.ПроизвольныеОтчеты,
			   Выборка.Ссылка, 
			   ТекстСообщения);
...
Процедура КонвертироватьИсточникВПакет(АОтчет)
	
	ИсточникИсходный = АОтчет.ИсточникДанныхОтчета;
	
	Если НЕ ЗначениеЗАполнено(ИсточникИсходный) 
		ИЛИ НЕ ТипЗнч(ИсточникИсходный) = Тип("СправочникСсылка.ИсточникиДанныхДляРасчетов") Тогда
		Возврат;	
	КонецЕсли;	
	
	НовыйПакет 								= Справочники.ПакетыДанных.СоздатьЭлемент();
	НовыйПакет.Наименование 				= АОтчет.Наименование;
	НовыйПакет.РасширеннаяНастройка 		= Истина;
	НовыйПакет.Записать();
	
	ИсточникОбъект  = ИсточникИсходный.ПолучитьОбъект();
	ИсточникОбъект.ПотребительРасчета = НовыйПакет.Ссылка; 
	ИсточникОбъект.ОбменДанными.Загрузка = Истина;
	ИсточникОбъект.Записать();
	
	ИсточникПакет = НовыйПакет.ИсточникиДанных.Добавить();
	ИсточникПакет.Источник = ИсточникОбъект.Ссылка;
	ИсточникПакет.ТипИБ    = ИсточникОбъект.ТипБД;
	
	КэшДоступныхИзмеренийИРесурсов = ПолучитьОписаниеКэшаДоступныхИзмеренийИРесурсов();
	ОбновитьКэшДоступныхИзмеренийИРесурсов(КэшДоступныхИзмеренийИРесурсов,НовыйПакет);
	
	
	//заполним развернутые таблицы измерений - ресурсов
	НовыйПакет.ИзмеренияИсточников.Очистить();
	
	Для Каждого СтрИзмерение Из КэшДоступныхИзмеренийИРесурсов Цикл
		
		Если СтрИзмерение.ЭтоРесурс Тогда
			 Продолжить;
		КонецЕсли;	
		
		нИзмерение = НовыйПакет.ИзмеренияИсточников.Добавить();
		нИзмерение.ТипЗначенияСтрока        = СтрИзмерение.ТипЗначенияСтрока;
		нИзмерение.ИмяВКубе                 = СтрИзмерение.ИмяВКубе;
		
			
		нИзмерение.ПутьКИзмерению 			= СтрИзмерение.Путь;
		нИзмерение.Источник                 = СтрИзмерение.Источник;
		нИзмерение.ИсточникПредставление    = СтрИзмерение.ИсточникПредставление;
		нИзмерение.ПриоритетСортировки      = СтрИзмерение.ПриоритетСортировки;
		нИзмерение.СинонимПоля              = СтрИзмерение.СинонимПоля;
		
		
	КонецЦикла;	
	
	//заполним развернутые таблицы измерений - ресурсов
	НовыйПакет.Ресурсы.Очистить();
	Для Каждого СтрРесурс Из КэшДоступныхИзмеренийИРесурсов Цикл
		
		Если НЕ СтрРесурс.ЭтоРесурс Тогда
			 Продолжить;
		КонецЕсли;
		
		нИзмерение = НовыйПакет.Ресурсы.Добавить();
		нИзмерение.ТипЗначенияСтрока        = СтрРесурс.ТипЗначенияСтрока;
		нИзмерение.ИмяВКубе                 = СтрРесурс.ИмяВКубе;
			
		нИзмерение.ПутьКРесурсу 			= СтрРесурс.Путь;
		нИзмерение.Источник                 = СтрРесурс.Источник;
		нИзмерение.ИсточникПредставление    = СтрРесурс.ИсточникПредставление;
		нИзмерение.ПриоритетСортировки      = СтрРесурс.ПриоритетСортировки;
		нИзмерение.СинонимПоля              = СтрРесурс.СинонимПоля;
	...
	СКДИстчоника = Справочники.ПакетыДанных.СформироватьСКДПоНаборуДанных(НовыйПакет);
	НовыйПакет.СхемаКомпоновкиДанныхпоУмолчанию  = Новый ХранилищеЗначения(СКДИстчоника);
	
	НовыйПакет.Записать();
	
	АотчетОбъект = аОтчет.ПолучитьОбъект();
	АотчетОбъект.ИсточникДанныхОтчета = НовыйПакет.Ссылка;
	АотчетОбъект.Записать();
	
КонецПроцедуры

Функция ПолучитьОписаниеКэшаДоступныхИзмеренийИРесурсов()
	
	 КэшДоступныхИзмеренийИРесурсов = Новый ТаблицаЗначений;
	 КэшДоступныхИзмеренийИРесурсов.Колонки.Добавить("Путь");
	 КэшДоступныхИзмеренийИРесурсов.Колонки.Добавить("ТипЗначенияСтрока");
	 КэшДоступныхИзмеренийИРесурсов.Колонки.Добавить("Источник");
	 КэшДоступныхИзмеренийИРесурсов.Колонки.Добавить("ИмяВКубе");
	 КэшДоступныхИзмеренийИРесурсов.Колонки.Добавить("СинонимПоля");
	 КэшДоступныхИзмеренийИРесурсов.Колонки.Добавить("ИсточникПредставление");
	 КэшДоступныхИзмеренийИРесурсов.Колонки.Добавить("ЭтоРесурс");
	 КэшДоступныхИзмеренийИРесурсов.Колонки.Добавить("ПриоритетСортировки");
	 КэшДоступныхИзмеренийИРесурсов.Колонки.Добавить("ЭтоПоказательОтчета");
	 КэшДоступныхИзмеренийИРесурсов.Колонки.Добавить("КолонкаОтчета");
	 
	 Возврат КэшДоступныхИзмеренийИРесурсов;
	 
	 
КонецФункции	

Процедура ОбновитьКэшДоступныхИзмеренийИРесурсов(КэшДоступныхИзмеренийИРесурсов,тОбъект)
	
	Для Каждого СтрИсточник Из тОбъект.ИсточникиДанных Цикл
		...
		Для Каждого ИсточникИзмерение Из ИсточникОбъект.мТаблицаПоказателейБД Цикл
			...
			нИзмерение.ИмяВКубе 	 			= ОбщегоНазначенияУХ.СформироватьКодНаОснованииНаименования(нИзмерение.СинонимПоля,100);
		...
		Если ЗначениеЗаполнено(ИсточникОбъект.РесурсРегистра) Тогда
			...
			нРесурс.Путь 	 						= ИсточникОбъект.РесурсРегистра;
		...
		Иначе	
			...
			Для Каждого ИсточникИзмерение Из ИсточникОбъект.мТаблицаПоказателейБД Цикл
				...
				Если ОписаниеТипов.СодержитТип(Тип("Число"))  
					...
					нРесурс.ИмяВКубе 	 					= ОбщегоНазначенияУХ.СформироватьКодНаОснованииНаименования(ИсточникИзмерение.НаименованиеБД,100);		
