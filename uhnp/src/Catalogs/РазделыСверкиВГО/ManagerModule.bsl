...
Функция ПолучитьОписаниеИД(РазделВГО, ИсточникДанных, флВключитьОрганизациюВАналитики=Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(ГруппаРаскрытия.Владелец.ВидАналитики1) Тогда
		СтрОписания.ЧислоАналитик = ЧислоАналитикУчитываяВидОтчета(ГруппаРаскрытия);
	КонецЕсли;
...
Функция ЧислоАналитикУчитываяВидОтчета(ГруппаРаскрытия)

	Для й = 0 По 5 Цикл
		Если ЗначениеЗаполнено(ГруппаРаскрытия["ВидАналитики" + (6-й)]) Тогда
			Возврат (6-й);
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбновитьФСД(ОбъектРазделВГО)
	...
	РазделВГО 	= ОписаниеРазделаВГО(ИмяШаблона);
	
	ОбъектРазделВГО.ДополнительныеСвойства.Вставить("ОписанияФСД", Новый Массив);
	
	Для Каждого Стр Из РазделВГО Цикл
		
		Аналитики		= ПолучитьАналитики(Стр.Дт, Стр.Кт, Стр.Код);		
		ВО 				= ПодготовитьВидОтчета(Стр.Код, Стр.Наименование, ГруппаВО);
		СтрокаОтчета 	= СоздатьСтроку(ВО, Стр.Код, Стр.Наименование);
		ГруппаРаскрытия = СоздатьГруппуРаскрытия(ВО, Аналитики, Стр.Код, Стр.Наименование);
		
		Для каждого Показатель Из Стр.Показатели Цикл
			Номер = Стр.Показатели.Найти(Показатель);
			КолонкаОтчета = СоздатьКолонку(ВО, Показатель, НаименованиеПоказателя(Показатель), Номер);
			ПоказательОтчета = СоздатьПоказательПоКолонке(ВО, КолонкаОтчета, СтрокаОтчета, ГруппаРаскрытия);
		КонецЦикла;
		
		ОбъектРазделВГО.ДополнительныеСвойства.ОписанияФСД
			.Добавить(Новый Структура("ГруппаРаскрытия", ГруппаРаскрытия));
...
Функция ОписаниеРазделаВГО(ИмяШаблона)
	
	Результат = Новый Массив;	
	Имена = "Сумма, Количество, СуммаНДС, СуммаФВ";
	
	Если ИмяШаблона = "МПЗ" Тогда
		
		Наименование = НСтр("ru = 'ВГО запасы: продавец'");
		Результат.Добавить(ОписаниеВГО("ДКЗ", "ДР", "ВгоМпзОтправитель", Наименование, Имена));
		Наименование = НСтр("ru = 'ВГО запасы: покупатель'");
		Результат.Добавить(ОписаниеВГО("МПЗ", "ДКЗ", "ВгоМпзПолучатель", Наименование, Имена));
		Наименование = НСтр("ru = 'ВГО запасы: себестоимость продавца'");
		Результат.Добавить(ОписаниеВГО("ДР", "МПЗ", "ВгоМпзСсОтправитель", Наименование, "СуммаФВ, Количество"));

	КонецЕсли;
	
	Если ИмяШаблона = "ВНА" Тогда
		
		Наименование = НСтр("ru = 'ВГО запасы: продавец'");
		Результат.Добавить(ОписаниеВГО("ДКЗ", "ПДР", "ВгоВнаОтправитель", Наименование, Имена));
		Наименование = НСтр("ru = 'ВГО запасы: покупатель'");
		Результат.Добавить(ОписаниеВГО("ВНА", "ДКЗ", "ВгоВнаПолучатель", Наименование, Имена));
		Наименование = НСтр("ru = 'ВГО запасы: себестоимость продавца'");
		Результат.Добавить(ОписаниеВГО("ПДР", "ВНА", "ВгоВнаCcОтправитель", Наименование, "ОстаточнаяСтоимость, НакопленнаяАмортизация"));

	КонецЕсли;

	Если ИмяШаблона = "ДДС" Тогда
		
		Наименование = НСтр("ru = 'ВГО ДДС: денежные средства'");
		Результат.Добавить(ОписаниеВГО("ДКЗ", "ДДС", "ВгоДДС", Наименование, "Сумма, СуммаФВ"));

	КонецЕсли;
	
	Если ИмяШаблона = "ФИ" Тогда
		
		Наименование = НСтр("ru = 'ВГО ФИ: финансовая деятельность'");
		Результат.Добавить(ОписаниеВГО("ФИ", "ДКЗ", "ВгоФИ", Наименование, "Сумма, СуммаФВ"));

	КонецЕсли;

	Если ИмяШаблона = "Услуги" Тогда
		
		Наименование = НСтр("ru = 'ВГО услуги: продажа услуг'");
		Результат.Добавить(ОписаниеВГО("ДКЗ", "ДР", "ВгоУслуги", Наименование, "Сумма, СуммаНДС, СуммаФВ"));
...
Функция ОписаниеВГО(Дт,	Кт, Код, Наименование, ПоказателиЧерезЗапятую)
	
	Показатели = СтрРазделить(СтрЗаменить(ПоказателиЧерезЗапятую, " ", ""), ",");
	Возврат Новый Структура("Дт, Кт, Код, Наименование, Показатели",
						Дт, Кт, Код, Наименование, Показатели);
...
Функция ПолучитьАналитики(Дт = "ДР", Кт = "ДКЗ", КодВО = "МПЗ")
	...
	ТипСчета = ?(Себестоимость, Дт, ?(Дт = "ДКЗ", Кт, Дт));//для элиминации важнее аналитика стороны Дт(91.2), чем Кт - списание актива
...
Функция ТекстПроцедурыЗаполнения(ОбъектПравило, ОбъектРазделВГО)
	...
	|
	|//ЭтотОбъект - Экземпляр отчета" + // @Non-NLS
...
Функция ТекстЗапроса_ОтсутствующиеПравила()
	...
	|	РБ.ПланСчетов В
	|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|				УП.ПланСчетовУчета.Владелец КАК ТипВИБ
	|			ИЗ
	|				Документ.УчетнаяПолитикаМСФО КАК УП)
	|
	|СГРУППИРОВАТЬ ПО
	|	РБ.ПланСчетов,
	|	РБ.Владелец
