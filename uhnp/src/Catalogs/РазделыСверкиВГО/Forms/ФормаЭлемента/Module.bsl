...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьАналитикиИИДФормыПоОбъекту();
...
Процедура флРасширеннаяНастройкаИсточниковДанныхПриИзменении(Элемент)
	НовоеЗначениеТипаРедактированияИД =
		флРасширеннаяНастройкаИсточниковДанных;
...
Процедура УстановитьОформлениеЭлементовРасширеннойНастройки(
	...
	Иначе
		...
		Элементы.ФСДОбщий.Заголовок =
				?(флДтКтПоЗнакуОперации, 
					НСтр("ru='Данные обеих сторон сверки'"), 
					НСтр("ru='Данные дебитора (покупателя)'"));
...
Процедура ОбработатьОповещенияОВыбореПоляИсточника(Результат, ДопПараметры) Экспорт
	...
	Иначе
		...
		// Показатель
		Если Не  СтрНачинаетсяС(ВыбраннаяСтрока.НаименованиеБД, НСтр("ru='Показатель :'")) Тогда
			Сообщение = Новый СообщениеПользователю;
...
Процедура ДобавитьНовуюАналитику()
	...
	Если Элементы.АналитикиСверки.Видимость Тогда
...
Процедура СпособЭлиминацииПриИзмененииНаСервере()
	...
	Если (НЕ ЗначениеЗаполнено(Объект.Наименование)
		...
		Объект.Наименование = Название + " " 
			+ СледующийНомерНазвания(Объект.Владелец, Название);
	КонецЕсли;
...
&НаСервере
Функция СледующийНомерНазвания(Владелец, Название)
...
&НаСервере
Процедура ЗаполнитьАналитикиИИДФормыПоОбъекту()
...
Процедура ЗаписатьОсновныеИсточникиДанныхФормы(ТекущийОбъект)
	...
	// Записываем объекты
	ОбновитьПотребитель(ТекущийОбъект, ПараметрыИД.РабочийОбъект);
	...
	Если СпособОпределенияНаправленияУчетаНаФорме = 1 Тогда
		ОбновитьПотребитель(ТекущийОбъект, ПараметрыИДКредитора.РабочийОбъект);
...
&НаСервере
Процедура ОбновитьПотребитель(ОбъектРазделВГО, ДанныеФормыИД)
	
	СсылкаРазделВГО = ОбъектРазделВГО.Ссылка;
	Если Не ЗначениеЗаполнено(ОбъектРазделВГО.Ссылка) Тогда // новый
		СсылкаРазделВГО = ОбъектРазделВГО.ПолучитьСсылкуНового();
		Если Не ЗначениеЗаполнено(СсылкаРазделВГО) Тогда // еще не получали
			СсылкаРазделВГО = Справочники.РазделыСверкиВГО.ПолучитьСсылку();
			ОбъектРазделВГО.УстановитьСсылкуНового(СсылкаРазделВГО);
		КонецЕсли;
	КонецЕсли;
	
	Если СсылкаРазделВГО = ДанныеФормыИД.ПотребительРасчета Тогда
		Возврат;
	КонецЕсли;
	
	//ситуация с ошибочной старой ссылкой, например после копирования
	ОбъектИД = ДанныеФормыИД.Ссылка.Скопировать();
	ОбъектИД.ПотребительРасчета = СсылкаРазделВГО;
	ОбъектИД.Записать();
	
	ЗначениеВДанныеФормы(ОбъектИД, ДанныеФормыИД);
...
&НаСервереБезКонтекста
Функция НайтиИсточникДанныхВГО(Наименование, ПотребительРасчета, Отбор)
	...
	|ГДЕ
	|	т.ПотребительРасчета = &ПотребительРасчета
	|	И т.Наименование = &Наименование
	|	И т.НазначениеРасчетов = &НазначениеРасчетов
	|	И т.ТипБД = &ТипБД
	|	И т.СпособИспользования = &СпособИспользования
	|	И т.СпособПолучения = &СпособПолучения
	|	И т.ТипЗначения = &ТипЗначения
	|
	|УПОРЯДОЧИТЬ ПО
	|	т.ПометкаУдаления");
	...
	Если РезультатЗапроса.Пустой() Тогда
		Возврат ПредопределенноеЗначение("Справочник.ИсточникиДанныхДляРасчетов.ПустаяСсылка");
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция СоздатьЗаписатьИсточникДанныхНаСервере(Наименование, ПотребительРасчета)
	
	Отбор = Новый Структура;
	Отбор.Вставить("НазначениеРасчетов",	ПредопределенноеЗначение("Справочник.ПравилаОбработки.ПустаяСсылка"));
	Отбор.Вставить("ТипБД",					ПредопределенноеЗначение("Справочник.ТипыБазДанных.ТекущаяИБ"));
	Отбор.Вставить("СпособИспользования",	ПредопределенноеЗначение("Перечисление.СпособыИспользованияОперандов.ДляСверкиВГО"));
	Отбор.Вставить("СпособПолучения",		ПредопределенноеЗначение("Перечисление.СпособыПолученияОперандов.ВнутренниеДанныеГруппаРаскрытия"));
	Отбор.Вставить("ТипЗначения",			ПредопределенноеЗначение("Перечисление.ТипыЗначенийПоказателейОтчетов.Число"));
	
	Если ЗначениеЗаполнено(Наименование) 
		И ЗначениеЗаполнено(ПотребительРасчета) Тогда
		
		ИсточникДанныхВГО = НайтиИсточникДанныхВГО(
			Наименование, ПотребительРасчета, Отбор);
			
		Если ЗначениеЗаполнено(ИсточникДанныхВГО) Тогда
			Возврат ИсточникДанныхВГО;
		КонецЕсли;
		
	КонецЕсли;

	ТекКод = ОбщегоНазначенияУХ.СформироватьКодНаОснованииНаименования(
		Наименование, 25);
	...
	ЗаполнитьЗначенияСвойств(ИДОбъект, Отбор);
...
Функция СформироватьСтруктуруСтрокиБД(ДанныеСтрокиАналитики, СпособПолучения)
	...
	СтрокаБД.Вставить("ПредставлениеПоля",			ДанныеСтрокиАналитики.НаименованиеБД);
...
//
&НаСервереБезКонтекста
Процедура ОбновитьЗаписатьИсточникДанныхПоДаннымФормы(ПараметрыИД, ТаблицаСоответствияФормы, УИДФормы)
...
Процедура ИсточникДанныхОбъектПередЗаписью(СтруктураПараметровИД, РабочийОбъект)
	...
	Если РабочийОбъект.ТаблицаСоответствияБД.Колонки.Найти("ТаблицаАналитики") = Неопределено Тогда
		отКолонки = ОбщегоНазначенияУХ.ПолучитьОписаниеТиповСтроки(150);
		РабочийОбъект.ТаблицаСоответствияБД.Колонки.Добавить("ТаблицаАналитики", отКолонки);
	КонецЕсли;
