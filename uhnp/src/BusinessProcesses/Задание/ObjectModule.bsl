	...
	Если ЗадачаДанные.Выполнена Тогда
		...
		Иначе
			СтрокаФормат = НСтр("ru = '%1, %4 (исполнителем назначен %2) выполнил(а) задачу:
				|%3';
				|en = '%1, %4 (assigned to %2) completed the task:
				|%3'") + Символы.ПС;
		КонецЕсли;
	Иначе
		Если ИсполнительЭтоТекущийПользователь Тогда
		...
		Иначе
			СтрокаФормат = НСтр("ru = '%1, %4 (исполнителем назначен %2) отклонил(а) задачу:
				|%3';
				|en = '%1, %4 (assigned to %2) rejected the task:
				|%3'") + Символы.ПС;
		КонецЕсли;
	КонецЕсли;
	...
	Если Не Подтверждено Тогда
		...
		Иначе
			СтрокаФормат = НСтр("ru = '%1, %4 (проверяющим назначен %2) вернул(а) задачу на доработку:
									  |%3';
									  |en = '%1, %4 (%2 is appointed as a supervisor) sent task back for revision:
									  |%3'") + Символы.ПС;
		КонецЕсли;
	Иначе
		Если Выполнено Тогда
			...
			Иначе
				СтрокаФормат = НСтр("ru = '%1, %4 (проверяющим назначен %2) подтвердил(а) выполнение задачи:
										  |%3';
										  |en = '%1, %4 (%2 is appointed as a supervisor) confirmed task completion:
										  |%3'") + Символы.ПС;
			КонецЕсли;
		Иначе
			Если ИсполнительЭтоТекущийПользователь Тогда
			...
			Иначе
				СтрокаФормат = НСтр("ru = '%1, %4 (проверяющим назначен %2) подтвердил(а) отмену задачи:
										  |%3';
										  |en = '%1, %4 (%2 is appointed as a supervisor) confirmed task cancellation:
										  |%3'") + Символы.ПС;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
