...
// Процедура - Загрузить план счетов
//
// Параметры:
//  ДанныеТаблицы					 - 	<Структура(ДанныеТаблицы)> - Таблица для загрузки
//  ПолучитьСубконтоИзСчетаИсточника - 	<Булево> - загружать ли субконто из таблицы, или получать субконто из счета источника
//  ПланСчетовМСФО					 - <СправочникСсылка.ПланСчетовБД> - загружаемый план счетов
//
Процедура ЗагрузитьПланСчетов(ДанныеТаблицы, ПолучитьСубконтоИзСчетаИсточника = Ложь, ПланСчетовМСФО) Экспорт
	
	КэшСозданных = ПолучитьКэшПометитьНаУдаление(ДанныеТаблицы.ТаблицаДанных);
	
	РодительПоКоду = ДанныеТаблицы.ТаблицаДанных.Колонки.Найти("Родитель") = Неопределено;
	
	КэшЗначений = Новый Соответствие;
	ПоляТЧ = ПолучитьПоляТЧ();
	
	КэшироватьПланыСчетов(КэшЗначений, ПланСчетовМСФО, ПолучитьСубконтоИзСчетаИсточника);
	Если ПолучитьСубконтоИзСчетаИсточника Тогда
		
		КэшСубконто = ПолучитьСопоставленияВидовСубконто(
			КэшЗначений.Получить("ПланСчетовИсточник"),
			КэшЗначений.Получить("ПланСчетовПриемник"));
	...
	Для каждого СтрокаИсточник Из ДанныеТаблицы.ТаблицаДанных Цикл
		...
		Для каждого ТекущаяКолонка Из ДанныеТаблицы.ТаблицаДанных.Колонки Цикл
			Если Не ПоляТЧ.Свойство(ТекущаяКолонка.Имя) Тогда
				УстановитьЗначение(ТекущийОбъект[ТекущаяКолонка.Имя],
					СтрокаИсточник[ТекущаяКолонка.Имя], КэшЗначений, ЕстьИзменение);
			КонецЕсли;
		КонецЦикла;
	...
	ОбновитьСчетаИБ(ПланСчетовМСФО);
...
Функция ПроверитьЗацикливаниеПоРодителю(ТекущийОбъект)
	...
	Если (ТекущийОбъект.Родитель = ТекущийОбъект.Ссылка) 
		Или ТекущийОбъект.Родитель.ПринадлежитЭлементу(ТекущийОбъект.Ссылка) Тогда
...
Процедура ОбновитьСчетаИБ(ПланСчетовМСФО) Экспорт
	
	АОСВ = Справочники.ВидыОтчетов.ПолучитьВидОтчетаОСВ(ПланСчетовМСФО);
	Если ЗначениеЗаполнено(АОСВ) Тогда
		Справочники.ВидыОтчетов.СоздатьНедостающиеПоказателиОСВ(ПланСчетовМСФО, , АОСВ, , Ложь);
	КонецЕсли;
	ОбновитьПовторноИспользуемыеЗначения();

КонецПроцедуры

Функция РазделыПересчетаВВО() Экспорт
...
Процедура УсловноеОформлениеВидСчета(Форма, ИмяСписка = "Список", ИмяВид = "Вид") Экспорт
	
	УсловноеОформление = Форма.УсловноеОформление;
	ПутьВид = ИмяСписка + "." + ИмяВид;
	
	// Активные счета

	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, ИмяВид);
	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		ПутьВид, ВидСравненияКомпоновкиДанных.Равно, ВидСчета.Активный);
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		ПутьВид, ВидСравненияКомпоновкиДанных.Равно, ВидСчета.Пассивный);
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		ПутьВид, ВидСравненияКомпоновкиДанных.Равно, ВидСчета.АктивноПассивный);
...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	псПоУмолчанию = Справочники.ПланыСчетовБД.ПолучитьПланСчетовБД("МСФО");
	
	Элемент = Элементы.Добавить();
	Элемент.ИмяПредопределенныхДанных = "Вспомогательный";
	Элемент.Забалансовый = Ложь;
	Элемент.ПланСчетовБД = псПоУмолчанию;
	ДополнительныеЯзыкиУХ.ЗаполнитьМультиязычныйРеквизит(Элемент, "Наименование", "ru = 'Вспомогательный'", КодыЯзыков); // @NStr-2
...
Процедура КэшироватьПланыСчетов(КэшЗначений, ПланСчетовМСФО, ПолучитьСубконтоИзСчетаИсточника)
	
	ПланСчетовИсточник = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПланСчетовМСФО, "ПланСчетовИсточник");

	
	КэшЗначений.Вставить("ПланСчетовПриемник", ПланСчетовМСФО);
	КэшЗначений.Вставить("ПланСчетовИсточник", ПланСчетовИсточник);
	
	Если ПолучитьСубконтоИзСчетаИсточника И Не ЗначениеЗаполнено(ПланСчетовИсточник) Тогда
...
Функция ПолучитьИзКэша(ТекущийТип, ЗначениеИсточник, КэшЗначений)
	...
	Если Метаданные.Перечисления.Содержит(мдТипа) Тогда
		...
		Для каждого ТекущееЗначениеПеречисления Из мдТипа.ЗначенияПеречисления Цикл
			Если ТекущееЗначениеПеречисления.Синоним = ЗначениеИсточник Тогда
				ПолноеИмя = мдТипа.ПолноеИмя() + "." + ТекущееЗначениеПеречисления.Имя;
				Результат = ПредопределенноеЗначение(ПолноеИмя);
				Прервать;
			КонецЕсли;
		КонецЦикла;
	...
	Иначе 
		...
		Иначе 
			...
			ПолеПоиска = МСФОКлиентСерверУХ.ПоляОбъекта(мдТипа, "ОсновноеПредставление");
			Если ПолеПоиска.ОсновноеПредставление = Метаданные.СвойстваОбъектов.ОсновноеПредставлениеСчета.ВВидеКода
				ИЛИ ПолеПоиска.ОсновноеПредставление = Метаданные.СвойстваОбъектов.ОсновноеПредставлениеСправочника.ВВидеКода
				ИЛИ ПолеПоиска.ОсновноеПредставление = Метаданные.СвойстваОбъектов.ОсновноеПредставлениеВидаХарактеристики.ВВидеКода Тогда
				ШаблонЗапроса = СтрЗаменить(ШаблонЗапроса, "т.Наименование", "т.Код");
			КонецЕсли;
...
Процедура Обновление_СчетаУчетаКурсовыхРазниц(Параметры = Неопределено) Экспорт

	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	т.Ссылка КАК СчетМСФО,
	|	СчетБД.СчетаУчетаКурсовыхРазниц КАК СчетаУчетаКурсовыхРазниц
	|ИЗ
	|	ПланСчетов.МСФО КАК т
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СчетаБД КАК СчетБД
	|		ПО т.Ссылка = СчетБД.СчетСсылка
	|			И (СчетБД.СчетаУчетаКурсовыхРазниц <> ЗНАЧЕНИЕ(Справочник.СчетаУчетаКурсовыхРазниц.ПустаяСсылка))
	|ГДЕ
	|	т.СчетаУчетаКурсовыхРазниц = ЗНАЧЕНИЕ(Справочник.СчетаУчетаКурсовыхРазниц.ПустаяСсылка)");
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Объект = Выборка.СчетМСФО.ПолучитьОбъект();
		Объект.СчетаУчетаКурсовыхРазниц = Выборка.СчетаУчетаКурсовыхРазниц;
		ОбновлениеИнформационнойБазы.ЗаписатьОбъект(Объект);
	КонецЦикла;

КонецПроцедуры

Процедура Обновление_ПересчетыВалют(Параметры = Неопределено) Экспорт
	
	//ПересчетВФВ
	Заполнение = Новый Структура("ПересчетВФВ", Перечисления.ПересчетыВФВ.Монетарный);
	...
	Обновление_ЗаполнитьПоВыборке(Запрос, Заполнение);	
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПланСчетов.МСФО", "Справочник.СчетаБД");
	Обновление_ЗаполнитьПоВыборке(Запрос, Заполнение);	
	
	//ПересчетВВО
	Заполнение = Новый Структура("ПересчетВВО", Истина);
	...
	|ГДЕ
	|	НЕ т.ПересчетВВО
	|	И т.РазделПланаСчетов В(&Капитал)");
	Запрос.УстановитьПараметр("Капитал", РазделыПересчетаВВО());
...
Процедура Обновление_ЗаполнитьПоВыборке(Запрос, Заполнение)

	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		
		Объект = Выборка.Счет.ПолучитьОбъект();
		ЗаполнитьЗначенияСвойств(Объект, Заполнение);
		ОбновлениеИнформационнойБазы.ЗаписатьОбъект(Объект);
