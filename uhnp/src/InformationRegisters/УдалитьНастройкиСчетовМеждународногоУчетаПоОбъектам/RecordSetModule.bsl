...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Для Каждого ЗаписьНабора Из ЭтотОбъект Цикл
		Если ЗначениеЗаполнено(ЗаписьНабора.СчетУчета) ИЛИ ЗначениеЗаполнено(ЗаписьНабора.СчетУчетаДолгосрочный) Тогда
			СчетаУчетаТребующиеНастройки = РегистрыСведений.УдалитьСчетаМеждународногоУчетаТребующиеНастройки.СоздатьНаборЗаписей();
		...
		Если ЗаписьНабора.ОбъектУчета = Перечисления.УдалитьОбъектыФинансовогоУчета.РасчетыПоНДС
	...
	Если СчетаРасчетовПоНДС.Количество() > 0 Тогда
		...
		|	СчетаРасчетовПоНДС.НастройкаФормированияПроводок КАК НастройкаФормированияПроводок,
		|	СчетаРасчетовПоНДС.ТипНастройки КАК ТипНастройки,
		|	ЗНАЧЕНИЕ(Перечисление.УдалитьОбъектыФинансовогоУчета.РасчетыПоНалогамИВзносам) КАК ОбъектУчета,
		|	ЗНАЧЕНИЕ(Справочник.ВидыНалоговВзносов.НДС) КАК ОбъектНастройки,
		|	СчетаРасчетовПоНДС.Организация КАК Организация,
		|	СчетаРасчетовПоНДС.МестоУчета КАК МестоУчета,
		|	ВЫБОР
		...
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УдалитьНастройкиСчетовМеждународногоУчетаПоОбъектам КАК НастройкиСчетовУчета
		...
		|	 И НастройкиСчетовУчета.ОбъектУчета = ЗНАЧЕНИЕ(Перечисление.УдалитьОбъектыФинансовогоУчета.РасчетыПоНалогамИВзносам)
		|	 И НастройкиСчетовУчета.ОбъектНастройки = ЗНАЧЕНИЕ(Справочник.ВидыНалоговВзносов.НДС)
		|ГДЕ
		...
		Пока Выборка.Следующий() Цикл
			МенеджерЗаписи = РегистрыСведений.УдалитьНастройкиСчетовМеждународногоУчетаПоОбъектам.СоздатьМенеджерЗаписи();
