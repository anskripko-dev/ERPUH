				...
				Если ТекущиеДанные.ВестиУчетПоГТД И ЗначениеЗаполнено(ТекущиеДанные.ДокументОтгрузки) Тогда
					ШаблонСообщения =
						НСтр("ru = 'Обеспечьте наличие остатка по номерам ГТД и информации о них в видах запасов документа отгрузки.';
							|en = 'Make sure there is stock balance by CCD numbers and information about them in the inventory owner attributes of the shipping document.'");
				Иначе
					ШаблонСообщения =
						НСтр("ru = 'Проверьте настройки учетной системы, признак ""Импортный товар"" в номенклатуре ""%1"", обеспечьте наличие остатка по номерам ГТД и информации о них в видах запасов документа отгрузки.';
							|en = 'Check the settings of the accounting system, the ""Imported goods"" flag in the ""%1"" item, make sure there is stock balance by CCD numbers and information about them in the inventory owner attributes of the shipping document.'");
				КонецЕсли;
			Иначе
				Если ТекущиеДанные.ПрослеживаемыйТовар И ЗначениеЗаполнено(ТекущиеДанные.ДокументОтгрузки) Тогда
					ШаблонСообщения =
						НСтр("ru = 'Обеспечьте наличие остатка по РНПТ и информации о них в видах запасов документа отгрузки.';
							|en = 'Make sure there is stock balance by RNPT and information about them in the inventory owner attributes of the shipment document.'");
				Иначе
					ШаблонСообщения =
						НСтр("ru = 'Проверьте настройки учетной системы, признак ""Прослеживаемый товар"" в номенклатуре ""%1"", обеспечьте наличие остатка по РНПТ и информации о них в видах запасов документа отгрузки.';
							|en = 'Check the settings of the accounting system, the ""Traceable goods"" flag in the ""%1"" item, make sure there is stock balance by RNPT and information about them in the inventory owner attributes of the shipping document.'");
				КонецЕсли;
			КонецЕсли;
...
Процедура РазбитьСтроку(Команда)
	...
	ИначеЕсли ТекущиеДанные.РодительскаяГруппировкаСтроки = Неопределено Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Для разбивки выберите строку с товаром.';
														|en = 'To split, select a line with goods.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПеренестиВОтправление(Команда)
	...
	ИначеЕсли Элементы.Список.ВыделенныеСтроки.Количество() = 1
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Для переноса выберите строки с товарами.';
														|en = 'To transfer, select a line with goods.'"));
		Возврат;
	ИначеЕсли Элементы.Список.ВыделенныеСтроки.Количество() = 1
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Товар отменен в заказе. Выберите другую позицию.';
														|en = 'The item is canceled in the order. Select another item.'"));
		Возврат;
	КонецЕсли;
	...
	Если СтрокиГруппы.Количество() > 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Для отмены доступны только строки с товарами.';
				|en = 'You can only cancel lines with goods.'"));
	...
	Если ОтменяемыеСтроки.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Отмена невозможна, т.к. для выбранных строк установлена отмена или оформлены документы отгрузки. Для отмены строк отмените проведение документов отгрузки.';
				|en = 'Cannot cancel the selected lines as they are already canceled, or shipping documents are generated for them. To cancel the lines, cancel shipping document posting.'"));
		Возврат;
	КонецЕсли;
	...
	Если ОтменяемыеСтроки.Количество() <> Элементы.Список.ВыделенныеСтроки.Количество() Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения,
			НСтр("ru = 'К отмене доступны только неотмененные строки и без отгрузки. Продолжить?';
				|en = 'You can cancel only uncanceled lines and lines without shipments. Continue?'"),
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<указан, но не найден в ИБ>';
																|en = '<specified but not found in the infobase>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветПоясняющийТекст);
...
Процедура ПриАктивизацииСтрокиСписка()
	...
	Если ЭтоГруппировка Тогда
		...
		Если ЭтоНовоеОтправление Тогда
			Подсказка = НСтр("ru = 'Отправление создано при переносе товаров. Требуется оформление документа отгрузки. Проверьте товары в отправлении.';
							|en = 'The shipment was created upon goods transfer. A shipping document is required. Check the goods in the shipment.'");
		ИначеЕсли ЭтоОтправлениеБезДокументаОтгрузки Тогда
			Подсказка = НСтр("ru = 'Требуется оформление документа отгрузки. Проверьте товары в отправлении.';
							|en = 'A shipping document is required. Check the goods in the shipment.'");
		ИначеЕсли ЭтоИзмененноеОтправление Тогда
			Подсказка = НСтр("ru = 'В отправлении изменен состав товаров. Требуется оформление документа отгрузки. Проверьте товары в отправлении.';
							|en = 'The goods in the shipment are changed. A shipping document is required. Check the goods in the shipment.'");
		ИначеЕсли ЭтоОтправлениеСНезаполненнымиДанными Тогда
			Подсказка = НСтр("ru = 'В отправлении есть незаполненные данные по товарам. Проверьте товары в отправлении.';
							|en = 'The shipment has unfilled goods data. Check the goods in the shipment.'");
	...
	Если ТекущиеДанные.НетВЗаказе Тогда
		Подсказка = НСтр("ru = 'Товар будет отменен на торговой площадке, т.к. не найден в документе заказа.';
						|en = 'The item will be canceled on the marketplace as it is not found in the order document.'");
		Возврат;
	КонецЕсли;
		...
		Если ТекущиеДанные.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.ОжидаетСборки") Тогда
			Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Товар будет отменен на торговой площадке, т.к. в документе заказа отменен по причине: ""%1"".';
					|en = 'The item will be canceled on the marketplace as it is canceled in the order document due to ""%1"".'"),
		...
		Иначе
			Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Товар отменен на торговой площадке и в документе заказа по причине: ""%1"".';
					|en = 'The item is canceled on the marketplace and in the order document due to ""%1"".'"),
				?(ЗначениеЗаполнено(ТекущиеДанные.ПричинаОтмены), СокрЛП(ТекущиеДанные.ПричинаОтмены), НСтр("ru = '<не указано>';
																											|en = '<none>'")));
		КонецЕсли;
	...
	ИначеЕсли ТекущиеДанные.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.Отменен") Тогда
		Подсказка = НСтр("ru = 'Товар отменен на торговой площадке.';
						|en = 'The goods are cancelled on the marketplace.'");
		Возврат;
	КонецЕсли;
		...
		МассивЗаполнения.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'реквизит ""%1"" в номенклатуре ""%2""';
					|en = 'the ""%1"" attribute in the ""%2"" item'"),
				ПредставлениеРеквизитаСтранаПроисхождения,
				ТекущиеДанные.Номенклатура));
	КонецЕсли;
	...
	Если ТекущиеДанные.ПозицияКОтгрузке
		...
		Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Требуется переоформить документы отгрузки или перенести товар в отправление %1, по которому он отгружается текущим документом отгрузки.';
				|en = 'Re-register the shipping documents or transfer the goods to the %1 shipment, which it is shipped with by the current shipping document.'"),
	...
	ИначеЕсли ТекущиеДанные.ПозицияКОтгрузке
		...
		Подсказка = НСтр("ru = 'Требуется переоформить текущий документ отгрузки из-за изменения состава отправления.';
						|en = 'Re-register the current shipping document as the shipment content has changed.'");
	ИначеЕсли ТекущиеДанные.ПозицияКОтгрузке
			И ТекущиеДанные.ПереоформитьДокументОтгрузки
		...
		Подсказка = НСтр("ru = 'Требуется переоформить документы отгрузки или провести текущий документ отгрузки.';
						|en = 'Re-register shipping documents or post the current shipping document.'");
	ИначеЕсли ТекущиеДанные.ПозицияКОтгрузке И ПустаяСтрока(ТекущиеДанные.ИдентификаторСтроки) Тогда
		Подсказка = НСтр("ru = 'Требуется оформить документ отгрузки.';
						|en = 'Register a shipping document.'");
	ИначеЕсли Не ТекущиеДанные.ПозицияКОтгрузке Тогда
		Подсказка = НСтр("ru = 'Товар не отгружается в заказе. Необходимо поставить товар к отгрузке или перенести в другое отправление.';
						|en = 'Cannot ship goods in the order. Set ""To ship"" for goods or move the goods to another shipment.'");
	КонецЕсли;
		...
		Если ТекущиеДанные.Статус <> ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.ОжидаетСборки")
			...
			ДополнительнаяИнформация.Добавить(СтрЗаменить(ТекущиеДанные.ОписаниеОшибки, 
				ТекстСообщения, НСтр("ru = 'В документе отгрузки требуется указать код маркировки для товара';
									|en = 'Specify the item marking code in the shipping document.'")));
		Иначе
				...
				Если РезультатПроверкиЗаказа.ОтправленияВСборке.Количество() = 0 Тогда
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'По заказу ""%1"" нет отправлений в статусе ""%2"".';
							|en = 'The ""%1"" order has no shipments with the ""%2"" status.'"),
						НомерЗаказа,
					...
					Если РезультатПроверкиЗаказа.ОтправленияНеВСборке.Количество() > 0 Тогда
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Исключены отправления, для которых не требуется сборка: %1.';
								|en = 'Excluded shipments that do not require picking: %1.'"),
					...
					Если РезультатПроверкиЗаказа.ОтправленияБезТоваров.Количество() > 0 Тогда
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Исключены отправления, для которых нет данных по товарам: %1. Возможно, товары этих отправлений были перенесены в другие отправления.';
								|en = 'Excluded shipments with no goods data: %1. Goods of these shipments might have been transferred to other shipments.'"),
	...
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Ошибка заполнения сведений по товарам. Подробности см. в журнале регистрации.';
				|en = 'An error occurred when filling in the goods information. For details, see the event log.'"));
	КонецЕсли;
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось разделить строку по причине: %1. Повторите действие.';
				|en = 'Cannot split the line. Reason: %1. Please try again.'"),
	...
	Попытка
		...
		Если ЕстьИзменения И ЗаписиРодительскихОтправлений.Количество() > 1 Тогда
			ТекстСообщения = НСтр("ru = 'Недопустим перенос позиций, изначально входящих в состав разных отправлений.';
									|en = 'You cannot transfer items primarily included in different shipments.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		ИначеЕсли ЕстьИзменения Тогда
			...
			Если ЕстьОтмененныеЭкземпляры Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Перенос выполнен только для неотмененных позиций.';
															|en = 'Only uncanceled items are transferred.'"));
			КонецЕсли;
		Иначе
			Если ЕстьОтмененныеЭкземпляры Тогда
				...
				Иначе
					ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Выделенные товары отменены в заказе. Выберите другие позиции.';
																|en = 'The selected items are canceled in the order. Select other items.'"));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ОтклоненныеНомераОтправлений) Тогда
			...
			Если ОтклоненныеНомераОтправлений.Количество() = 1 Тогда
				ШаблонСообщения = НСтр("ru = 'В отправлении %1 должны остаться товары. Невозможен перенос выделенных позиций этого отправления.';
										|en = 'The %1 shipment must contain goods. You cannot transfer the selected items of the shipment.'");
			Иначе
				ШаблонСообщения = НСтр("ru = 'В отправлениях %1 должны остаться товары. Невозможен перенос выделенных позиций этих отправлений.';
										|en = 'The %1 shipments must contain goods. You cannot transfer the selected items of these shipments.'");
			КонецЕсли;
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось перенести выделенные позиции в другое отправление по причине: %1. Повторите действие.';
				|en = 'Cannot transfer the selected items to another shipment. Reason: %1. Please try again.'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
	КонецПопытки;
			...
			ТекстСообщения =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для номенклатуры ""%1"" требуется вести учет операций с импортными товарами.';
						|en = 'For the ""%1"" item, keep records of operations with imported goods.'"),
					ТекущиеДанные.Номенклатура);
		ИначеЕсли ТекущиеДанные.ВестиУчетПоГТД И Не ЗначениеЗаполнено(ТекущиеДанные.ДокументОтгрузки) Тогда
			Если ТекущиеДанные.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.ОжидаетСборки") Тогда
				ТекстСообщения = НСтр("ru = 'Номер ГТД будет заполнен автоматически из данных документа отгрузки после его оформления.';
										|en = 'CCD number is autofilled from shipping document data once the document is registered.'");
			Иначе
				ТекстСообщения = НСтр("ru = 'Номер ГТД заполнен автоматически по данным торговой площадки.';
										|en = 'CCD number is autofilled according to the marketplace data.'");
			КонецЕсли;
		ИначеЕсли ТекущиеДанные.ВестиУчетПоГТД И ЗначениеЗаполнено(ТекущиеДанные.ДокументОтгрузки) Тогда
			Если ЗначениеЗаполнено(ТекущиеДанные.НомерГТД) Тогда
				Если ТекущиеДанные.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.ОжидаетСборки") Тогда
					ТекстСообщения = НСтр("ru = 'Номер ГТД заполнен автоматически из данных документа отгрузки.';
											|en = 'CCD number is autofilled from the shipping document data.'");
				Иначе
					ТекстСообщения = НСтр("ru = 'Номер ГТД заполнен автоматически по данным торговой площадки.';
											|en = 'CCD number is autofilled according to the marketplace data.'");
				КонецЕсли;
			Иначе
				ТекстСообщения = НСтр("ru = 'В документе отгрузки не указан номер ГТД, который требуется для торговой площадки.';
										|en = 'CCD number required for the marketplace is not specified in the shipping document.'");
			КонецЕсли;
		КонецЕсли;
			...
			ТекстСообщения =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для номенклатуры ""%1"" требуется вести учет операций с прослеживаемыми товарами.';
						|en = 'For the ""%1"" item, keep records of operations with traceable goods.'"),
					ТекущиеДанные.Номенклатура);
		ИначеЕсли ТекущиеДанные.ПрослеживаемыйТовар И Не ЗначениеЗаполнено(ТекущиеДанные.ДокументОтгрузки) Тогда
			Если ТекущиеДанные.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.ОжидаетСборки") Тогда
				ТекстСообщения = НСтр("ru = 'РНПТ будет заполнен автоматически из данных документа отгрузки после его оформления.';
										|en = 'RNPT is autofilled from the shipping document data once the document is registered.'");
			Иначе
				ТекстСообщения = НСтр("ru = 'РНПТ заполнен автоматически по данным торговой площадки.';
										|en = 'RNPT is autofilled according to the marketplace data.'");
			КонецЕсли;
		ИначеЕсли ТекущиеДанные.ПрослеживаемыйТовар И ЗначениеЗаполнено(ТекущиеДанные.ДокументОтгрузки) Тогда
			Если ЗначениеЗаполнено(ТекущиеДанные.НомерГТД) Тогда
				Если ТекущиеДанные.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.ОжидаетСборки") Тогда
					ТекстСообщения = НСтр("ru = 'РНПТ заполнен автоматически из данных документа отгрузки.';
											|en = 'RNPT is autofilled from the shipping document data.'");
				Иначе
					ТекстСообщения = НСтр("ru = 'РНПТ заполнен автоматически по данным торговой площадки.';
											|en = 'RNPT is autofilled according to the marketplace data.'");
				КонецЕсли;
			Иначе
				ТекстСообщения = НСтр("ru = 'В документе отгрузки не указан РНПТ, который требуется для торговой площадки.';
										|en = 'RNPT required for the marketplace is not specified in the shipping document.'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(ТекущиеДанные.ШтрихкодУпаковки) 
			...
			ТекстСообщения =
				НСтр("ru = 'Код маркировки будет заполнен автоматически из данных документа отгрузки после его оформления, если товар там будет отсканирован.';
					|en = 'Marking code is autofilled from the shipping document data once the document is registered and the item is scanned.'");
		ИначеЕсли Не ЗначениеЗаполнено(ТекущиеДанные.ШтрихкодУпаковки) 
			...
			Если ДанныеРазбора = Неопределено Тогда
				ТекстСообщения = НСтр("ru = 'Код маркировки был указан в личном кабинете торговой площадки, но не найден в информационной базе.';
										|en = 'The marking code was specified in the personal marketplace account, but was not found in the infobase.'")
			...
			Иначе
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Код маркировки %1 был указан в личном кабинете торговой площадки, но не найден в информационной базе.';
						|en = 'Marking code %1 was specified in the personal marketplace account, but was not found in the infobase.'"),
			...
			Если ЗначениеЗаполнено(ТекущиеДанные.ШтрихкодУпаковки) Тогда
				Если ТекущиеДанные.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовТорговыхПлощадок.ОжидаетСборки") Тогда
					ТекстСообщения = НСтр("ru = 'Код маркировки заполнен автоматически из данных документа отгрузки.';
											|en = 'Marking code is autofilled from the shipping document data.'");
				Иначе
					ТекстСообщения = НСтр("ru = 'Код маркировки заполнен автоматически по данным торговой площадки.';
											|en = 'The marking code is autofilled according to the marketplace data.'");
				КонецЕсли;
			Иначе
				ТекстСообщения =
					НСтр("ru = 'В документе отгрузки нужно отсканировать код маркировки потребительской упаковки, который требуется для торговой площадки.';
						|en = 'In the shipping document, scan the marking code of a consumer packaging unit for the marketplace.'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось сохранить выбранное значение по причине: %1. Повторите действие.';
				|en = 'Cannot save the selected value. Reason: %1. Please try again.'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
	КонецПопытки;
	УстановитьПривилегированныйРежим(Ложь);
