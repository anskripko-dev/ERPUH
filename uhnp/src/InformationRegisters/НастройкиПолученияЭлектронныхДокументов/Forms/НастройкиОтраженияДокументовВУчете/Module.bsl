...
Процедура ЗагрузитьТекущиеНастройкиНаСервере(НеПроверять = Истина)
	...
	ВыборкаНастроек = ОбменСКонтрагентами.ТекущиеНастройкиОтраженияВУчете(Организация, Контрагент,
		ИдентификаторКонтрагента, НеПроверять);
	
	Если ВыборкаНастроек.Количество() = 0 Тогда
	...
	Пока ВыборкаНастроек.Следующий() Цикл
		...
		ИдентификаторОрганизации = ВыборкаНастроек.ИдентификаторПолучателя;
		
		Отбор = Новый Структура();
		Отбор.Вставить("ВидДокумента"    , ВыборкаНастроек.ВидДокумента);
		
		НайденныеСтроки = ВходящиеДокументы.НайтиСтроки(Отбор);
		
		Для Каждого СтрокаТЧ Из НайденныеСтроки Цикл
			
			ЗаполнитьЗначенияСвойств(СтрокаТЧ, ВыборкаНастроек);
			
		КонецЦикла;
		
	КонецЦикла;
	
	Если ВыборкаНастроек.Количество() <> ВходящиеДокументы.Количество() Тогда
		Модифицированность = Истина;
	КонецЕсли;
