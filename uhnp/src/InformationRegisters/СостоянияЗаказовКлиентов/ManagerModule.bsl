...
Функция ТипыЗаказовДляОтраженияСостояний() Экспорт
	...
	//++ Устарело_Переработка24
	ТипыЗаказов.Добавить(Тип("ДокументСсылка.ЗаказДавальца"));
	//-- Устарело_Переработка24
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Ошибка отражения состояния заказов в очереди';
				|en = 'Error recording order states in the queue'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ТекстОшибкиОтражения(Заказ, ИнформацияОбОшибке)
	...
	Если ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		Шаблон = НСтр("ru = 'Не удалось отразить состояние заказа клиента: %1 по причине: %2';
						|en = 'Cannot record the state of the sales order: %1. Reason: %2'");
	...
	ИначеЕсли ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
		...
		Шаблон = НСтр("ru = 'Не удалось отразить состояние заявки на возврат: %1 по причине: %2';
						|en = 'Cannot record the state of the return request: %1. Reason: %2'");
	...
	ИначеЕсли ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказДавальца") Тогда
		...
		Шаблон = НСтр("ru = 'Не удалось отразить состояние заказа давальца: %1 по причине: %2';
						|en = 'Cannot record the state of the subcontracting sales order: %1. Reason: %2'");
		
	//-- Устарело_Переработка24
	ИначеЕсли ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказДавальца2_5") Тогда
...
Функция ТаблицаСостоянийЗаказов(МассивСсылок)
	...
	//++ НЕ УТКА
	ТекстыПакета.Добавить(ТекстЗапросаДляРасчетаУслугДавальцуКОформлению());
	//-- НЕ УТКА
...
Функция ТекстЗапросаДляРасчетаУслугДавальцуКОформлению()
	...
	|		,
	|		Распоряжение В (&МассивЗаказов)
	|		И ТИПЗНАЧЕНИЯ(Распоряжение) = ТИП(Документ.ЗаказДавальца2_5)) КАК РаспоряженияНаОтгрузкуОбороты
	//++ Устарело_Переработка24
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
