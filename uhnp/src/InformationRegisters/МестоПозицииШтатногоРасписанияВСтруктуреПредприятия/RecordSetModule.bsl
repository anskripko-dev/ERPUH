...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	СформироватьТекущиеПодразделенияПозиций(ЭтотОбъект, Замещение);
...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	ОбновитьДанныеВторичныеДанные(Замещение);
...
Процедура СформироватьТекущиеПодразделенияПозиций(НаборЗаписей, Замещение)
	
	Если НаборЗаписей.Количество() = 0 Или РегистрыБЗК.ЭтоУдалениеНабораЗаписей(Замещение) Тогда
		Возврат;
	КонецЕсли;
	
	СписокПозиций = РегистрыБЗК.ЗначенияОтбораЗаписейНабора(НаборЗаписей, Замещение, "Позиция");
	Запрос = Новый Запрос;
...
Процедура ОбновитьДанныеВторичныеДанные(Замещение)
	
	Если Не ДополнительныеСвойства.Свойство("ТекущиеПодразделенияПозиций") Тогда
		ИнтеграцияУправлениеПерсоналомСобытияРасширенный.ПриЗаписиМестаПозицииШтатногоРасписанияВСтруктуреПредприятия(Неопределено, ЭтотОбъект, Замещение);
		Возврат;
	КонецЕсли;
	...
	ИнтеграцияУправлениеПерсоналомСобытияРасширенный.ПриЗаписиМестаПозицииШтатногоРасписанияВСтруктуреПредприятия(
		ДополнительныеСвойства.ТекущиеПодразделенияПозиций,
		Неопределено,
		Неопределено);
