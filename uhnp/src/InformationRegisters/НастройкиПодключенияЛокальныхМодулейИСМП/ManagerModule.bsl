	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось установить формат обмена по умолчанию по настройке ЛМ ЧЗ: %1 - % 2 по причине: %3';
									|en = 'Не удалось установить формат обмена по умолчанию по настройке ЛМ ЧЗ: %1 - % 2 по причине: %3'");
			ТекстСообщения = СтрШаблон(ТекстСообщения, Выборка.Организация, ?(ЗначениеЗаполнено(Выборка.РабочееМесто), Выборка.РабочееМесто, НСтр("ru = '<без отбора по рабочему месту>';
																																					|en = '<без отбора по рабочему месту>'")), ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
