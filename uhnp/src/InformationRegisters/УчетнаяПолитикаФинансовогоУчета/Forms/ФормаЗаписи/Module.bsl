	...
	Если ИспользуетсяСреднескользящая И ВариантУчетаСтоимостиПродукции <> "ПоПлановой" Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'При методе оценки стоимости ""Среднескользящая"" используется ведение учета выпуска продукции по плановой стоимости.';
				|en = 'If you use the ""Moving average"" cost evaluation method, records of product release are kept at the standard cost.'"));
				...
				Если НЕ Запрос.Выполнить().Пустой() Тогда
					ОбщегоНазначения.СообщитьПользователю(
						НСтр("ru = 'Установка метода оценки стоимости ""Среднескользящая"" возможно только с месяца, начиная с которого нет движений в регистре накопления ""Себестоимость товаров""';
							|en = 'You can set the ""Moving average"" cost evaluation method only from the month starting from which there are no records in the ""Inventory cost"" accumulation register.'"),
						,
...
Процедура УстановитьВидимостьЭлементов(Форма)
	...
	Если МетодОценкиСтоимостиСреднескользящая Тогда
		ЭлементСписка.Представление = НСтр("ru = 'Распределять по направлениям выбытия продукции';
											|en = 'Allocate to product outflow lines'");
	Иначе
		ЭлементСписка.Представление = НСтр("ru = 'Включать в стоимость (доводя до фактической)';
											|en = 'Include in cost (bringing to actual cost)'");
	КонецЕсли;
