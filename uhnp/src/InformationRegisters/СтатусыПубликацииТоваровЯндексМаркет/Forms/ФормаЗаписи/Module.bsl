...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если (ЭтоНоваяЗапись ИЛИ ДоступноИзменениеИдентификатора()) И Результат.ИдентификаторПредложенияНеУникален Тогда    
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Введенный идентификатор предложения уже используется для товара - %1 в магазине  - %2.
																	    |Укажите другой идентификатор.';
																	    |en = 'The entered offer ID is already used for the %1 item in the %2 store.
																	    |Specify another ID.'"),Результат.ПредставлениеТовара,Результат.УчетнаяЗапись));      
	КонецЕсли;
...
Процедура ИнициироватьПодсказки()
	...
	Элементы.ПодсказкаЯНеЗнаюSKU.Заголовок = 
		НСтр("ru = 'Попробуйте найти товар в поиске по разделу ""Покупки"" на Маркете. Если товар найдется, SKU на Яндексе можно взять из URL его страницы.
					...
					|en = 'Try to find the item by the search in the ""Purchases"" section on Yandex.Market. If the item is found, take its SKU on Yandex from its page URL.
					|For example, if the item page URL is
					|https://market.yandex.ru/product--pir-2821ak/649875256?sponsored=1&sku=100848394231
					|, the item SKU on Yandex is 100848394231.
					|If you managed to find the SKU on Yandex, fill it and send the item to moderation (click ""Send to moderation"").
					|If you could not find the SKU, describe the item in detail and send the item for moderation without specifying the SKU (click ""Send to moderation""). Yandex.Market employees can pick up or create cards for your items in your personal account if the item is not published for sale on Yandex.Market yet.'");
