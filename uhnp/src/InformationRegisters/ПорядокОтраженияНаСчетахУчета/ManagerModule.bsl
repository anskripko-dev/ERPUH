...
Функция СписокСчетовУчетаПоРазделуУчета(РазделУчета) Экспорт
	...
	Если РазделУчета = "НоменклатураСобственная" Или РазделУчета = "Номенклатура" Тогда
		...
		СчетаУчетаРегл.Добавить("ОтклоненияВСтоимости", НСтр("ru = 'Отклонения';
															|en = 'Variances'"), РасчетСебестоимостиПовтИсп.ИспользуетсяСреднескользящаяОценка());
	КонецЕсли;
	...
	Если РазделУчета = "НоменклатураПринятая" Или РазделУчета = "Номенклатура" Тогда
		...
		СчетаУчетаРегл.Добавить("ТМЦВПроизводстве", НСтр("ru = 'В производстве';
														|en = 'Production batch tracking'"), ПолучитьФункциональнуюОпцию("ИспользоватьОтветственноеХранениеВПроцессеЗакупки") ИЛИ ИспользоватьПроизводствоИзДавальческогоСырья);
		...
		СчетаУчетаРегл.Добавить("ТМЦКСписанию", НСтр("ru = 'К списанию';
													|en = 'To be written off'"), ПолучитьФункциональнуюОпцию("ИспользоватьОтветственноеХранениеВПроцессеЗакупки") ИЛИ ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоНаСтороне2_5"));
		...
		СчетаУчетаРегл.Добавить("МатериалыПринятыеВПереработкуВПроизводстве", НСтр("ru = 'В производстве';
																					|en = 'Production batch tracking'"), ИспользоватьПроизводствоИзДавальческогоСырья);
		...
		СчетаУчетаРегл.Добавить("МатериалыПринятыеВПереработкуКСписанию", НСтр("ru = 'К списанию';
																				|en = 'To be written off'"), ИспользоватьПроизводствоИзДавальческогоСырья2_5 И ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоНаСтороне2_5"));
	...
	Если РазделУчета = "РасчетыСАрендодателями" Тогда
		СчетаУчетаРегл.Добавить("АрендныеОбязательства", НСтр("ru = 'Арендные обязательства';
																|en = 'Rental liabilities'"), ВнеоборотныеАктивы.ИспользуетсяУправлениеВНА());
	...
	Если РазделУчета = "ВнеоборотныеАктивы" Тогда
		...
		СчетаУчетаРегл.Добавить("НалогНаПрибыльОтОперацийПереоценки", НСтр("ru = 'Налог на прибыль от операций переоценки';
																			|en = 'Income tax from revaluation transactions'"), ВНА2_4);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение новых забалансовых видов счетов потребленных в производстве значениями по умолчанию.
		...
		|en = 'Fill new off-balance account kinds consumed in production by default.
		|Delete records of the outdated ""Claim AR/AP"" account kind. Fill a new ""Income tax from revaluation transactions"" account kind.'");
