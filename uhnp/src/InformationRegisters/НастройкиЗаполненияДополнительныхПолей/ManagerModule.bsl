...
// Параметры:
//  ДанныеНастройки - См. НовыеДанныеНастройкиЗаполненияПолей
Процедура ЗаписатьНастройкуЗаполненияПолей(ДанныеНастройки) Экспорт

	МенеджерЗаписи = СоздатьМенеджерЗаписи();
	
	ЗаполнитьЗначенияСвойств(МенеджерЗаписи, ДанныеНастройки,, "Настройка");
	
	МенеджерЗаписи.Настройка = Новый ХранилищеЗначения(ДанныеНастройки.Настройка, Новый СжатиеДанных(9));
...
//  * Формат - Строка 
//  * Настройка - Строка
Функция НовыеДанныеНастройкиЗаполненияПолей() Экспорт
	
	НовыеДанныеНастройки = Новый Структура;
	НовыеДанныеНастройки.Вставить("Отправитель", Неопределено);
	НовыеДанныеНастройки.Вставить("Получатель", Неопределено);
	НовыеДанныеНастройки.Вставить("Договор", Неопределено);
	НовыеДанныеНастройки.Вставить("ВидЭлектронногоДокумента", Справочники.ВидыДокументовЭДО.ПустаяСсылка());
	НовыеДанныеНастройки.Вставить("Формат", "");
	НовыеДанныеНастройки.Вставить("Настройка", "");
	Возврат НовыеДанныеНастройки;
...
//  - Неопределено
//  - ХранилищеЗначения
Функция НастройкаЗаполненияПолей(КлючНастройки) Экспорт
	
	Результат = Неопределено;
	
	НаборЗаписей = СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Отправитель.Установить(КлючНастройки.Отправитель);
	НаборЗаписей.Отбор.Получатель.Установить(КлючНастройки.Получатель);
	НаборЗаписей.Отбор.Договор.Установить(КлючНастройки.Договор);
	НаборЗаписей.Отбор.ВидЭлектронногоДокумента.Установить(КлючНастройки.ВидЭлектронногоДокумента);
	НаборЗаписей.Отбор.Формат.Установить(КлючНастройки.Формат);
...
Функция НовыйКлючНастройки() Экспорт
	...
	НовыйКлюч.Вставить("Отправитель", Неопределено);
	НовыйКлюч.Вставить("Получатель", Неопределено);
	НовыйКлюч.Вставить("Договор", Неопределено);
	НовыйКлюч.Вставить("ВидЭлектронногоДокумента", Справочники.ВидыДокументовЭДО.ПустаяСсылка());
	НовыйКлюч.Вставить("Формат", "");
	Возврат НовыйКлюч;
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Если Параметры.ПрогрессВыполнения.ВсегоОбъектов = 0 Тогда
		Параметры.ПрогрессВыполнения.ВсегоОбъектов = ВыбранныеДанные.Количество();
	КонецЕсли;
