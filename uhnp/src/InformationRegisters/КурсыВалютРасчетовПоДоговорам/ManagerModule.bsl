...
Функция ПолучитьЗначенияКурсаВалютыДоговораУХ(Договор, ДатаКурса, ВалютаДокумента, ВалютаВзаиморасчетов, ЭтоОплата = Ложь)Экспорт
	...
	Реквизиты.Вставить("ВалютаВзаиморасчетов","ВалютаВзаиморасчетов");
	Реквизиты.Вставить("ОсновнаяВалютаПлатежей","ОсновнаяВалютаПлатежей");  
	...
	ПараметрыКурса.ВалютаДокумента = ВалютаВзаиморасчетов;
	ПараметрыКурса.ВалютаОплаты = ВалютаДокумента;
	...
	Иначе
		СтруктураКурсаПлатежа = ДвиженияБюджетированиеУХ.ПолучитьАктуальныйКурсПоПараметрам(ПараметрыКурса);
		Результат.КурсЧислитель = СтруктураКурсаПлатежа.Курс;   
		Результат.КурсЗнаменатель = СтруктураКурсаПлатежа.Кратность;   
		Если Не (ЗначениеЗаполнено(Результат.КурсЧислитель) И ЗначениеЗаполнено(Результат.КурсЗнаменатель)) Тогда    
			Результат.КурсЧислитель = 1;
			Результат.КурсЗнаменатель = 1;
			Результат.Вставить("КурсНеЗадан", Истина);
