...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Устанавливает признак принадлежности склада к схеме работы FBS или к RealFBS в регистре сведений ""Соответствия объектов маркетплейсов"".
										...
										|Delete register records for outdated metadata objects.'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт   
	...
	//++ НЕ УТ
	|	ИЛИ ДанныеРегистра.Объект1С ССЫЛКА Справочник.УдалитьКлючиАналитикиУчетаПартийПроизводства
	//-- НЕ УТ
	|";
	ТекстыЗапросов.Добавить(ТекстЗапроса);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Для Каждого УчетнаяЗапись Из УчетныеЗаписи Цикл
		ПараметрыЗапроса = ИнтеграцияСМаркетплейсомOzonСервер.НовыеПараметрыЗапросаПолученияСпискаСкладов();
		МассивСкладовПоУчетнойЗаписи = ИнтеграцияСМаркетплейсомOzonСервер.СкладыИзСервиса(
			УчетнаяЗапись, ПараметрыЗапроса);
	...
	Для Каждого Строка Из ТаблицаОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если
				...
				//++ НЕ УТ
				И ТипЗнч(Строка.Объект1С) <> Тип("СправочникСсылка.УдалитьКлючиАналитикиУчетаПартийПроизводства")
				//-- НЕ УТ
				Тогда
