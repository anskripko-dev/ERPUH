...
Процедура ОбъектУчетаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка, ДтКт)
	...
	ПолноеИмяФормы = "ПланВидовХарактеристик.СтатьиАктивовПассивов.Форма.ФормаВыбораОбъектаУчета";
	...
	Для Каждого ПараметрВыбора Из Элемент.ПараметрыВыбора Цикл
		Если СтрНачинаетсяС(ПараметрВыбора.Имя, "Отбор") Тогда
			ПозицияТочки = СтрНайти(ПараметрВыбора.Имя, ".");
			ПараметрыФормы.Отбор.Вставить(Сред(ПараметрВыбора.Имя, ПозицияТочки + 1), ПараметрВыбора.Значение);
		Иначе
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ОписаниеОбъектаУчетаДт = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ОписаниеОбъектаФинансовогоУчета(
		Запись.ОбъектУчетаДт);
		
	ОписаниеОбъектаУчетаКт = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ОписаниеОбъектаФинансовогоУчета(
		Запись.ОбъектУчетаКт);
...
Процедура УправлениеФормой(Форма)
	...
	Если ЗначениеЗаполнено(Форма.ОписаниеОбъектаУчетаДт) Тогда
		СчетаУчетаВНастройкахПроводокДт = Форма.ОписаниеОбъектаУчетаДт.СчетаУчетаВНастройкахПроводок;
	Иначе
		СчетаУчетаВНастройкахПроводокДт = Ложь;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Форма.ОписаниеОбъектаУчетаКт) Тогда
		СчетаУчетаВНастройкахПроводокКт = Форма.ОписаниеОбъектаУчетаКт.СчетаУчетаВНастройкахПроводок;
	Иначе
		СчетаУчетаВНастройкахПроводокКт = Ложь;
	КонецЕсли;
	...
	Элементы.СчетУчетаИзНастройкиОбъектаДт.СписокВыбора[1].Представление = ?(
		СчетаУчетаВНастройкахПроводокДт,
	...
	Элементы.СчетУчетаИзНастройкиОбъектаКт.СписокВыбора[1].Представление = ?(
		СчетаУчетаВНастройкахПроводокКт,
		СчетОпределяетсяНастройкойПоОбъектам,
		СчетОпределяетсяНастройкамиДокументов);
	...
	Элементы.ГруппаСчетаОбъектаУчетаДт.Видимость = Форма.СчетУчетаИзНастройкиОбъектаДт И СчетаУчетаВНастройкахПроводокДт;
	Элементы.ГруппаСчетаОбъектаУчетаКт.Видимость = Форма.СчетУчетаИзНастройкиОбъектаКт И СчетаУчетаВНастройкахПроводокКт;
	
	Элементы.СчетаПрочихОперацийДт.Видимость = Форма.СчетУчетаИзНастройкиОбъектаДт И НЕ СчетаУчетаВНастройкахПроводокДт;
	Элементы.СчетаПрочихОперацийКт.Видимость = Форма.СчетУчетаИзНастройкиОбъектаКт И НЕ СчетаУчетаВНастройкахПроводокКт;
...
Процедура УправлениеЭлементамиФормыСчетовОбъектаУчета(ДтКт)
	...
	Иначе
		СчетаОбъектаУчетаМестоУчета.Видимость = ОписаниеОбъектаУчета.ОписаниеТиповДетальнойНастройки.Типы().Количество() > 0;
		СчетаОбъектаУчетаМестоУчета.Заголовок = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ЗаголовокОбъектаПоОписаниюТипов(
		...
		СчетаОбъектаУчетаОбъектНастройки.Заголовок = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ЗаголовокОбъектаПоОписаниюТипов(
...
Процедура ОбъектУчетаПриИзмененииСервер(ДтКт)
	...
	ЭтотОбъект["ОписаниеОбъектаУчета" + ДтКт] = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ОписаниеОбъектаФинансовогоУчета(
		Запись["ОбъектУчета" + ДтКт]);
...
Процедура ИнициализироватьКомпоновщикиОтбораПоОбъектуУчета(ДтКт)
	...
	Если ПустаяСтрока(ОписаниеОбъектаУчета.ИсточникДанных) Тогда
		ПараметрыОтраженияВУчете = Неопределено;
	Иначе
		ПараметрыОтраженияВУчете = МеждународныйУчетПоДаннымФинансовыхРегистров.ПараметрыОтраженияДвиженийВМеждународномУчете(
			ОписаниеОбъектаУчета.ИсточникДанных);
	КонецЕсли;
	...
	ФиксированныйОтбор = КомпоновщикНастроек.ФиксированныеНастройки.Отбор;
	ФиксированныйОтбор.Элементы.Очистить();
	
	МеждународныйУчетПоДаннымФинансовыхРегистров.СкопироватьОтборКомпоновкиДанных(
		ФиксированныйОтбор, ОписаниеОбъектаУчета.ОтборДанных);
	...
	Если ОбъектМетаданных <> Неопределено Тогда
		Если ДтКт = "Дт" Тогда
			...
			ПредставлениеФиксированногоОтбораОбъектУчетаДт = Строка(ФиксированныйОтбор);
		Иначе
			ИсточникДанныхОбъектУчетаКт = ОбъектМетаданных.Синоним;
			ПредставлениеФиксированногоОтбораОбъектУчетаКт = Строка(ФиксированныйОтбор);
		КонецЕсли;
	КонецЕсли;
...
Функция ИсточникиСубконто(ИмяРегистра) 
	...
	Возврат ПараметрыОтраженияВУчете.ИсточникиАналитики;
