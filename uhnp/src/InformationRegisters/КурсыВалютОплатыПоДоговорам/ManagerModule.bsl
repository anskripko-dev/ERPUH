...
//
Функция ПолучитьЗначенияКурсаВалютыДоговора(Договор, ДатаКурса, ВалютаВзаиморасчетов, ВалютаОплаты, ВалютаРеглУчета = Неопределено)Экспорт
	
	Реквизиты = Новый Структура;
	Реквизиты.Вставить("ВалютаДокумента","ВалютаВзаиморасчетов");
	Реквизиты.Вставить("ВалютаОплаты","ОсновнаяВалютаПлатежей");
	Реквизиты.Вставить("СпособОпределенияКурсаПлатежа","СпособОпределенияКурсаПлатежа");
	Реквизиты.Вставить("КурсПлатежа","КурсПлатежа");
	Реквизиты.Вставить("КурсПлатежаНеБолее","КурсПлатежаНеБолее");
	Реквизиты.Вставить("КурсПлатежаНеБолееВВалютеОплаты","КурсПлатежаНеБолееВВалютеОплаты");
	Реквизиты.Вставить("КурсПлатежаНеМенее","КурсПлатежаНеМенее");
	Реквизиты.Вставить("КурсПлатежаНеМенееВВалютеОплаты","КурсПлатежаНеМенееВВалютеОплаты");
	Реквизиты.Вставить("ДатаФиксацииКурсаПлатежа","ДатаФиксацииКурсаПлатежа");
	Реквизиты.Вставить("СдвигДатыФиксацииКурсаПлатежа","СдвигДатыФиксацииКурсаПлатежа");
	...
	Если ЗначениеЗаполнено(ВалютаВзаиморасчетов) Тогда  
		РеквизитыДоговора.ВалютаДокумента = ВалютаВзаиморасчетов;
	КонецЕсли;   
	
	Если ЗначениеЗаполнено(ВалютаОплаты) Тогда  
		РеквизитыДоговора.ВалютаОплаты = ВалютаОплаты;
	КонецЕсли;
	
	ПараметрыКурса = ПлатежныеПозицииКлиентСервер.СтруктураПараметровРасчетаКурса();
	ЗаполнитьЗначенияСвойств(ПараметрыКурса, РеквизитыДоговора);
	ПараметрыКурса.ДатаПлатежа = ДатаКурса;
	...
	Если РеквизитыДоговора.СпособОпределенияКурсаПлатежа =  Перечисления.СпособыОпределенияКурсаПлатежа.ФиксированныйКурс Тогда
		Результат = ПолучитьПоследнее(ДатаКурса, Новый Структура("Договор, ВалютаВзаиморасчетов, ВалютаОплаты", Договор, РеквизитыДоговора.ВалютаДокумента, РеквизитыДоговора.ВалютаОплаты));
		...
		КонецЕсли;  
		ПараметрыКурса.КурсПлатежа = Результат.КурсЧислитель;
		ПараметрыКурса.КратностьПлатежа = Результат.КурсЗнаменатель; 
	Иначе
		КурсПлатежа = ДвиженияБюджетированиеУХ.ПолучитьАктуальныйКурсПоПараметрам(ПараметрыКурса);
		Результат.КурсЧислитель = КурсПлатежа;   
		Если Не (ЗначениеЗаполнено(Результат.КурсЧислитель) И ЗначениеЗаполнено(Результат.КурсЗнаменатель)) Тогда    
			Результат.КурсЧислитель = 1;
			Результат.КурсЗнаменатель = 1;
			Результат.Вставить("КурсНеЗадан", Истина);
	...
	Результат.Вставить("ВалютаВзаиморасчетов", ВалютаВзаиморасчетов);
	Результат.Вставить("ВалютаОплаты", ВалютаОплаты);
...
//
Процедура ЗаписатьЗначенияКурсаВалютыДоговора(Договор, Знач ДатаКурса,ВалютаВзаиморасчетов, ВалютаОплаты, КурсЧислитель, КурсЗнаменатель)Экспорт 
	...
	Если НаборЗаписей.Количество() = 0 Тогда
		...
		НоваяЗапись.ВалютаОплаты         = ВалютаОплаты;
