...
Функция ДанныеЗаполненияОпераций(СменноеЗадание, ТаблицаОпераций = Неопределено)
	...
	Если ПолучитьФункциональнуюОпцию("ХранитьОперацииВРесурсныхСпецификациях") Тогда
		...
		Для каждого КлючИЗначение Из ДанныеЭтапов Цикл
			
			ДанныеЭтапа = КлючИЗначение.Значение;
			
			НоваяСтрока = СписокНоменклатуры.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ДанныеЭтапа);
			НоваяСтрока.Количество = ДанныеЭтапа.КоличествоУпаковокПлан * ДанныеЭтапа.КоэффициентУпаковки;
			НоваяСтрока.Этап       = КлючИЗначение.Ключ;
		...
		СписокНоменклатуры.Индексы.Добавить("Этап");
		Для каждого ДанныеЗаполнения Из Результат Цикл
			
			Строка = СписокНоменклатуры.Найти(ДанныеЗаполнения.КлючОперации.Этап, "Этап");
