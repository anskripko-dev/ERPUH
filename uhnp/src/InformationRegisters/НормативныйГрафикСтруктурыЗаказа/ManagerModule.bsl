...
Процедура Рассчитать() Экспорт
	...
	// Порционная очистка буфера
	Запрос = Новый Запрос(
		"ВЫБРАТЬ ПЕРВЫЕ 100000
		|	Буфер.Область             КАК Область,
	...
	Для Сч = 1 По 10 Цикл
		УстановитьПривилегированныйРежим(Истина);
		ЗаписиБуфера = Запрос.Выполнить().Выгрузить();
		УстановитьПривилегированныйРежим(Ложь);
		
		Если ЗаписиБуфера.Количество() = 0 Тогда
			Прервать;
		КонецЕсли;
		
		Набор = РегистрыСведений.БуферРасчетаНормативногоГрафика.СоздатьНаборЗаписей();
		Набор.Загрузить(ЗаписиБуфера);
		УстановитьПривилегированныйРежим(Истина);
...
Процедура ПересчитатьГрафикЗаказа(ЗаказНаПроизводство) Экспорт
	...
	Попытка
		...
		РегистрыСведений.ЗаданияКРасчетуСтруктурыЗаказаГрафикПроизводства.ДобавитьЗадание(ЗаказНаПроизводство);
...
Процедура ОбработатьРезультатРасчета(Задание, Результат)
	...
	Если РезультатРасчета <> Неопределено
		И НЕ РезультатРасчета.Отказ Тогда
		Для каждого Строка Из Задание.Строки Цикл
			...
			Набор.Отбор.ЭтапПроизводства.Установить(Строка.ЭтапПроизводства);
			УстановитьПривилегированныйРежим(Истина);
			Набор.Записать();
			УстановитьПривилегированныйРежим(Ложь);
		КонецЦикла;
...
Функция РассчитатьГрафикЗаказаВнутриЗамераВремени(ЗаказНаПроизводство, Граница)
	...
	// Чтение данных заказа (поддерживается только размещение "К окончанию" и сборочный процесс)
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	ДанныеЗаказа.Ссылка КАК ЗаказНаПроизводство,
		|	ВЫБОР
		|		КОГДА НЕ ДанныеЗаказа.ДатаПотребностиУказываетсяВСтроках ТОГДА 
		|			ДанныеЗаказа.ДатаПотребности
		|		ИНАЧЕ
		|			ISNULL(МАКСИМУМ(ПродукцияЗаказа.ДатаПотребности), ДанныеЗаказа.Дата)
		|	КОНЕЦ               КАК НачалоРазмещения
		|ПОМЕСТИТЬ ГраницыРазмещения
		|ИЗ
		...
		|		ПО ДанныеЗаказа.Ссылка = ПродукцияЗаказа.Ссылка
		|				И НЕ ПродукцияЗаказа.Отменено
		|				И ДанныеЗаказа.ДатаПотребностиУказываетсяВСтроках 
		|ГДЕ
		...
		|		ГДЕ
		|			Т.ЗаказНаПроизводство = ДанныеЗаказа.Ссылка) КАК ПервыйРасчет,
		|	ИСТИНА                                               КАК ЭтоСборка,
		|	ЛОЖЬ                                                 КАК РазмещениеКНачалу,
		|	ГраницыРазмещения.НачалоРазмещения                   КАК НачалоРазмещения
		|ИЗ
		|	Документ.ЗаказНаПроизводство2_2 КАК ДанныеЗаказа, ГраницыРазмещения 
		|ГДЕ
		|	ДанныеЗаказа.Ссылка = &ЗаказНаПроизводство");
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(ДанныеИсточника.НачалоРазмещения) Тогда
		ОписаниеОшибки = НовыйОписаниеОшибки();
		ОписаниеОшибки.Код = КодОшибкиНеЗаполненаДатаПотребности();
		ОписаниеОшибки.РазмещениеКНачалу = Ложь;
		ЗаписатьОшибку(ЗаказНаПроизводство, ЗаказНаПроизводство.УникальныйИдентификатор(), ОписаниеОшибки);
		...
		Пока ИндексУзел < ДанныеРазмещения.Узлы.Количество() Цикл
			...
			Пока ИндексЭтап < Узел.Этапы.Количество() Цикл
				...
				Размещать = Этап.РазмещатьВТекущемТакте
					...
					И Этап.НачалоРазмещения >= ПараметрыТакта.РазделительЗаписи;
...
Функция НачальныйИнтервалРазмещенияББВ(ПараметрыТакта, Этап, Исполнитель, НачалоРазмещения)
	...
	Иначе
		...
		Если Этап.Интервал = Перечисления.ТочностьГрафикаПроизводства.Смена Тогда
	...
	Иначе
		...
		Если НЕ ПараметрыТакта.РазмещениеКНачалу
			И Исполнитель.Интервал = Перечисления.ТочностьГрафикаПроизводства.Смена Тогда
			ИмяКолонки = "ОкончаниеСмены";
		Иначе
			ИмяКолонки = "Дата";
		КонецЕсли;
	КонецЕсли;
	...
	Для Сч = 1 По КоличествоИтерацийПоиска Цикл
		...
		СтруктураПоиска[ИмяКолонки] = Дата;
		НайденныеСтроки = ПараметрыТакта.РасписаниеББВ.НайтиСтроки(СтруктураПоиска);
	...
	Если Результат = Неопределено Тогда
		...
		Для Сч = 0 По НайденныеСтроки.ВГраница() Цикл
			Если ПараметрыТакта.РазмещениеКНачалу И НайденныеСтроки[Сч][ИмяКолонки] >= Дата
				ИЛИ Не ПараметрыТакта.РазмещениеКНачалу И НайденныеСтроки[Сч][ИмяКолонки] <= Дата Тогда
...
Функция ЗагрузкаВРЦНаИнтервале(Узел, Исполнитель)
	...
	КоличествоПартий = ?(Цел(Узел.КоличествоПартий) = Узел.КоличествоПартий И Узел.КоличествоПартий <> 0,
...
Функция РазместитьПоРасписаниюУББВ(ПараметрыТакта, Подразделение, НачалоРазмещения, Время, РазмещатьВДнях)
	...
	Если Начало <> Окончание Тогда
		...
		Если РазмещатьВДнях
			...
			КонецЕсли;
			Если ЗначениеЗаполнено(Границы.Окончание)
				И Границы.Окончание <> '00010101235959' Тогда
...
Функция ПервыеУзлы(ДанныеИсточника, СтруктураЗаказаКеш)
	...
	Если ДанныеИсточника.ЭтоСборка Тогда
		...
		ТекстыЗапроса.Добавить(
			...
			|ПОМЕСТИТЬ ВТПартииБезВыпуска
			|ИЗ
			|	ВТКешСтруктураЗаказа КАК СтруктураЗаказа
			|
			|	ЛЕВОЕ СОЕДИНЕНИЕ ВТКешКоличествоПартий КАК ВыпускПартий
			|	ПО СтруктураЗаказа.КлючПартия = ВыпускПартий.КлючПартия
			|ГДЕ
			|	НЕ СтруктураЗаказа.ЭтоВыпуск
			|	И ВыпускПартий.КлючПартия ЕСТЬ NULL
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	КлючПартия
			|;
			...
			|	КОНЕЦ                       КАК ВидУзла,
			|	ЕСТЬNULL(МИНИМУМ(ПродукцияЗаказа.ДатаПотребности), &НачалоРазмещения) КАК НачалоРазмещения
			|	 
			|ПОМЕСТИТЬ ВТУзлы
			|ИЗ
			...
			|	
			|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказНаПроизводство2_2.Продукция КАК ПродукцияЗаказа
			|	ПО ПродукцияЗаказа.Ссылка = &ЗаказНаПроизводство
			|		И ПродукцияЗаказа.КлючНоменклатура = СтруктураЗаказа1.КлючНоменклатура
			|		И ПродукцияЗаказа.ДатаПотребности <> ДатаВремя(1,1,1) 
			|		И СтруктураЗаказа1.ЭтоВыпуск = &ЭтоВыпуск
			|	
			|	ЛЕВОЕ СОЕДИНЕНИЕ ВТПартииБезВыпуска КАК ПартииОтправительБезВыпуска
			|	ПО СтруктураЗаказа1.КлючПартия = ПартииОтправительБезВыпуска.КлючПартия
			|	
			|	ЛЕВОЕ СОЕДИНЕНИЕ ВТПартииБезВыпуска КАК ПартииПолучательБезВыпуска
			|	ПО СтруктураЗаказа2.КлючПартия = ПартииПолучательБезВыпуска.КлючПартия
			|	
			|СГРУППИРОВАТЬ ПО
			...
			|		ИЛИ КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СтруктураЗаказа2.КлючПартия) = КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПартииПолучательБезВыпуска.КлючПартия)
			|	)
			|	И МИНИМУМ(ПартииОтправительБезВыпуска.КлючПартия ЕСТЬ NULL) = ИСТИНА
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	ВидУзла");
	...
	Если ДанныеИсточника.ЭтоСборка Тогда
		...
		Иначе
			...
			ТекстыЗапроса.Добавить(
				...
				|		ИНАЧЕ &ВидУзлаПартияСтруктуры
				|	КОНЕЦ                      КАК ВидУзла,
				|	ISNULL(МИНИМУМ(ПродукцияЗаказа.ДатаПотребности), &НачалоРазмещения) КАК НачалоРазмещения
				|ПОМЕСТИТЬ ВТУзлы
				|ИЗ
				|	ВТКешСтруктураЗаказа КАК СтруктураЗаказа
				|
				...
				|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказНаПроизводство2_2.Продукция КАК ПродукцияЗаказа
				|	ПО ПродукцияЗаказа.Ссылка = &ЗаказНаПроизводство
				|		И ПродукцияЗаказа.КлючНоменклатура = СтруктураЗаказа.КлючНоменклатура
				|		И ПродукцияЗаказа.ДатаПотребности <> ДатаВремя(1,1,1) 
				|
				|СГРУППИРОВАТЬ ПО
				|	СтруктураЗаказа.КлючПартия,
...
Функция ТекстЗапросаУзлыЭтапыВидыРЦ(РазмещениеКНачалу)
	...
	ТекстЗапроса = 
		...
		|	ЛОЖЬ                                               КАК Размещен,
		|	Узлы.НачалоРазмещения                              КАК НачалоРазмещения,
...
Функция ТекстЗапросаСвязиУзлов(РазмещениеКНачалу, ИспользоватьБуфер)
	...
	ТекстЗапроса = 
		...
		|	Связи.НомерЭтапа               КАК НомерЭтапа
		|ИЗ
...
Процедура ЗакешироватьСтруктуруЗаказа(СтруктураЗаказаКеш, ДанныеИсточника, ПолныйРасчет)
	...
	ТекстЗапроса = 
		...
		|	МАКСИМУМ(ВложенныйЗапрос.НомерЭтапа)         КАК НомерЭтапа,
		|	МАКСИМУМ(ВложенныйЗапрос.ЭтапАктуален)       КАК ЭтапАктуален,
		...
		|			ЕСТЬNULL(ЭтапыДокумент.ПартияПроизводства, ЗНАЧЕНИЕ(Справочник.ПартииПроизводства.ПустаяСсылка)) КАК ПартияПроизводства,
		|			ЕСТЬNULL(ЭтапыСправочник.НомерЭтапа, ЕСТЬNULL(ЭтапыДокумент.НомерЭтапа, 0)) КАК НомерЭтапа,
		|			ЕСТЬNULL(НЕ ЭтапыСправочник.ПометкаУдаления, ЕСТЬNULL(ЭтапыДокумент.Проведен, ИСТИНА)) КАК ЭтапАктуален,
		...
		|				ПО СтруктураЗаказа.Этап = ЭтапыСправочник.Ссылка
		|				
		|				ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2 КАК ЭтапыДокумент
		|				ПО СтруктураЗаказа.Этап = ЭтапыДокумент.Ссылка
		|		ГДЕ
		...
		|	ИЛИ СУММА(ВложенныйЗапрос.Запланировано) > 0
		|	ИЛИ СУММА(ВложенныйЗапрос.Запущено) > 0
		|	ИЛИ СУММА(ВложенныйЗапрос.РаспределеноИзПартий) > 0
		|
		|ИНДЕКСИРОВАТЬ ПО НАБОРАМ (
		|	(КлючНоменклатура),
		|	(КлючПартия, ЭтоВыпуск)
		|)
		|;
		|////////////////////////////////////////////////////////////////////////////////
		|
		...
		|			СУММА((Узлы.Запланировано + Узлы.Запущено)
		...
		|	Последователи.НомерЭтапа      КАК НомерЭтапа,
		...
		|
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТКешКоличествоПартий КАК ВыпускПартий
		|	ПО Предшестенники.КлючПартия = ВыпускПартий.КлючПартия
		|ГДЕ
		|	Последователи.ЭтоВыпуск = &ПоследовательЭтоВыпуск
		|	И Последователи.ЭтапАктуален
		|	И Предшестенники.ЭтоВыпуск <> &ПоследовательЭтоВыпуск
		|	И Предшестенники.ЭтапАктуален
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	КлючПартия
		|;
...
Процедура ПрочитатьРасписание(ПараметрыТакта, ГрафикиКеш, Графики, Начало, Окончание)
	...
	КалендарныеГрафики.СоздатьВТРасписанияРаботыНаПериод(ГрафикиКеш, Графики, Начало, Окончание, Ложь);
	...
	ТекстЗапроса = 
		...
		|	Графики.КлючГрафика КАК КлючГрафика,
		|	Графики.Интервал    КАК Интервал,
		|	МИНИМУМ(ДОБАВИТЬКДАТЕ(РасписанияРаботы.ДатаГрафика, МИНУТА,
		|		ЧАС(ЕСТЬNULL(РасписанияРаботы.ВремяНачала, ДАТАВРЕМЯ(1, 1, 1)))*60
		|		+ МИНУТА(ЕСТЬNULL(РасписанияРаботы.ВремяНачала, ДАТАВРЕМЯ(1, 1, 1))))) КАК Дата,
		|	МАКСИМУМ(
		|		ВЫБОР
		...
		|				ЧАС(РасписанияРаботы.ВремяОкончания)*60 + МИНУТА(РасписанияРаботы.ВремяОкончания))
		...
		|	ПО Графики.График = РасписанияРаботы.ГрафикРаботы
		|ГДЕ
		|	Графики.Интервал = ЗНАЧЕНИЕ(Перечисление.ТочностьГрафикаПроизводства.Смена)
		|	И РасписанияРаботы.Период >= &Начало
		|
		|СГРУППИРОВАТЬ ПО
		|	Графики.КлючГрафика,
		|	Графики.Интервал,
		|	РасписанияРаботы.ГрафикРаботы,
		|	РасписанияРаботы.Период
		|
		|УПОРЯДОЧИТЬ ПО
		|	КлючГрафика,
		|	Интервал,
		...
		|		ИНАЧЕ РасписанияРаботы.ВремяОкончания
	...
	РасписаниеББВ.Индексы.Добавить("КлючГрафика, Интервал, ОкончаниеСмены");
...
Функция КодОшибкиНеЗаполненаДатаПотребности()
...
Функция ЖурналОшибок(Заказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли ОписаниеОшибки.Код = КодОшибкиНеЗаполненаДатаПотребности() Тогда
			
			НоваяСтрока = Результат.Добавить();
			НоваяСтрока.ТекстСообщения = НСтр("ru = 'В заказе не заполнена желаемая дата выпуска.';
												|en = 'В заказе не заполнена желаемая дата выпуска.'");
			НоваяСтрока.Расшифровка = Заказ;
...
Функция ПривестиКНачалуПериода(Дата, Интервал)
	...
	ИначеЕсли Интервал = Перечисления.ТочностьГрафикаПроизводства.Час Тогда
		Возврат НачалоЧаса(Дата);
	ИначеЕсли Интервал = Перечисления.ТочностьГрафикаПроизводства.Смена Тогда
		Возврат Дата;
	КонецЕсли;
...
Функция ПривестиКОкончаниюПериода(Дата, Интервал)
	...
	ИначеЕсли Интервал = Перечисления.ТочностьГрафикаПроизводства.Неделя Тогда
		Возврат КонецНедели(Дата) + 1;
	ИначеЕсли Интервал = Перечисления.ТочностьГрафикаПроизводства.Месяц Тогда
		Возврат КонецМесяца(Дата) + 1;
	ИначеЕсли Интервал = Перечисления.ТочностьГрафикаПроизводства.Час Тогда
		Возврат КонецЧаса(Дата) + 1;
	ИначеЕсли Интервал = Перечисления.ТочностьГрафикаПроизводства.Смена Тогда
		Возврат Дата;
	КонецЕсли;
