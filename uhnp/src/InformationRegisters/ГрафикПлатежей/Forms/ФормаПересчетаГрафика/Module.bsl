...
Процедура РассчитатьГрафикПлатежейПоФинансовымИнструментам()
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ПоступлениеСредств.ОбъектОплаты
	|ИЗ
	|	ВТПоступленияИСписанияСредств КАК ПоступлениеСредств
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДоговорыАренды КАК ОписаниеДоговоров
	|		ПО ПоступлениеСредств.ОбъектОплаты = ОписаниеДоговоров.Ссылка
	|ГДЕ
	|	(ОписаниеДоговоров.Организация = &Организация
	|			ИЛИ &ВсеОрганизации)
...
Процедура УдалитьЗаписиГрафикаПлатежей(ТаблицаОбъектовОплаты)
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Пересчет графика платежей';
										|en = 'Recalculate payment schedule'", ОбщегоНазначения.КодОсновногоЯзыка()),
