...
Процедура Аннулировать(Команда)
	...
	ЭлектронныеДокументы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объект.Ссылка);
	Если Не ЭлектронныеДокументыЭДОВызовСервера.КаждыйДокументБезПодтвержденияИлиОтветнаяПодписьОтправлена(ЭлектронныеДокументы,
		ПакетнаяОбработка) Тогда
		ОтклонитьДокументВместоАннулирования(ПакетнаяОбработка);
		Возврат;
	КонецЕсли;
	...
	Если РезультатПроверки.АннулированиеЗапрещено Тогда
		ШаблонСообщения = НСтр("ru = 'Действие не выполнено по причине: %1';
								|en = 'Действие не выполнено по причине: %1'");
...
Процедура ПоказатьВыборСозданияДокументаУчета()
	...
	Если СписокСоздания.Количество() = 0 Тогда
		...
		|en = 'No record methods for this type of document are set.
...
&НаКлиенте
Асинх Процедура ОтклонитьДокументВместоАннулирования(ЭтоПакетнаяОбработка)
	...
	Если ПользовательСогласен Тогда
		Действие = ?(ЭтоПакетнаяОбработка, Элементы.Отклонить, Элементы.ОтклонитьДокумент);
		Отклонить(Действие);
	КонецЕсли;
КонецПроцедуры
