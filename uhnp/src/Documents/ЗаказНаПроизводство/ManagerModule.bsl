...
Функция ТребуетсяПланировать(МассивЗаказов) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|ГДЕ
		|	Т.Период < ДОБАВИТЬКДАТЕ(&ТекущаяДатаСеанса, ДЕНЬ, ЕСТЬNULL(СпрСпособ.ГарантированныйСрокОбеспечения, 0))
		|	И ЕСТЬNULL(СпрСпособ.ГарантированныйСрокОбеспечения, 0) > 0
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
