...
Процедура УстановитьУсловноеОформление()
	...
	ПараметрыУсловногоОформления = НоменклатураСервер.НовыеПараметрыУсловногоОформленияЕдиницИзмерения();
	ПараметрыУсловногоОформления.ИмяПоляЕдиницаИзмерения = "ГрафикВыпускаПродукцииНоменклатураЕдиницаИзмерения";
	ПараметрыУсловногоОформления.ПутьКПолюУпаковка = "ГрафикВыпускаПродукции.Упаковка";
	НоменклатураСервер.УстановитьУсловноеОформлениеЕдиницИзмерения(ЭтаФорма, ПараметрыУсловногоОформления);
...
Процедура ЗаполнитьНазначениеВТабличныхЧастях()
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Объект.Продукция, СтруктураДействий);
...
Процедура ДобавитьПолуфабрикаты(АдресВХранилище, ИдентификаторТекущейСтроки)
	...
	Для каждого СтрокаПолуфабрикат Из МассивНовыхСтрок Цикл
...
Процедура ОбработкаПодбораТоваровИзРаспоряжения(АдресТоваровВХранилище, КэшированныеЗначения)
	...
	Для Каждого ПодобраннаяСтрока Из РезультатПодбора Цикл
		...
		ОбрабатываемыеСтроки.Добавить(СтрокаТабличнойЧасти);
...
Процедура ОбработатьРезультатРедактированияНаСервере(РезультатРедактирования, КэшированныеЗначения)
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОчиститьСлужебныйКэш(КэшированныеЗначения);
	
	ОбрабатываемыеСтроки = Новый Соответствие();
	ОбрабатываемыеСтроки.Вставить(Объект.ВыходныеИзделия, Новый Массив);
	ОбрабатываемыеСтроки.Вставить(Объект.ВозвратныеОтходы, Новый Массив);
	ОбрабатываемыеСтроки.Вставить(Объект.МатериалыИУслуги, Новый Массив);
	
	ОбрабатываемыеСтрокиНеОкруглять = Новый Соответствие();
	ОбрабатываемыеСтрокиНеОкруглять.Вставить(Объект.ВыходныеИзделия, Новый Массив);
	ОбрабатываемыеСтрокиНеОкруглять.Вставить(Объект.ВозвратныеОтходы, Новый Массив);
	ОбрабатываемыеСтрокиНеОкруглять.Вставить(Объект.МатериалыИУслуги, Новый Массив);
	...
	Для каждого ВыходныеИзделияЭлемент Из СтруктураДанных.ВыходныеИзделия Цикл
		...
		Если СтруктураДействий.ПересчитатьКоличествоЕдиниц <> Неопределено
			И СтруктураДействий.ПересчитатьКоличествоЕдиниц.Свойство("НужноОкруглять")
			И Не СтруктураДействий.ПересчитатьКоличествоЕдиниц.НужноОкруглять Тогда
				ОбрабатываемыеСтрокиНеОкруглять[Объект.ВыходныеИзделия].Добавить(СтруктураДействий);
		Иначе
			ОбрабатываемыеСтроки[Объект.ВыходныеИзделия].Добавить(СтруктураДействий);
		КонецЕсли;
	...
	Для каждого ВозвратныеОтходыЭлемент Из СтруктураДанных.ВозвратныеОтходы Цикл
		...
		Если СтруктураДействий.ПересчитатьКоличествоЕдиниц <> Неопределено
			И СтруктураДействий.ПересчитатьКоличествоЕдиниц.Свойство("НужноОкруглять")
			И Не СтруктураДействий.ПересчитатьКоличествоЕдиниц.НужноОкруглять Тогда
				ОбрабатываемыеСтрокиНеОкруглять[Объект.ВозвратныеОтходы].Добавить(СтруктураДействий);
		Иначе
			ОбрабатываемыеСтроки[Объект.ВозвратныеОтходы].Добавить(СтруктураДействий);
		КонецЕсли;
	...
	Для каждого МатериалыИУслугиЭлемент Из СтруктураДанных.МатериалыИУслуги Цикл
		...
		Если СтруктураДействий.ПересчитатьКоличествоЕдиниц <> Неопределено
			И СтруктураДействий.ПересчитатьКоличествоЕдиниц.Свойство("НужноОкруглять")
			И Не СтруктураДействий.ПересчитатьКоличествоЕдиниц.НужноОкруглять Тогда
				ОбрабатываемыеСтрокиНеОкруглять[Объект.МатериалыИУслуги].Добавить(СтруктураДействий);
		Иначе
			ОбрабатываемыеСтроки[Объект.МатериалыИУслуги].Добавить(СтруктураДействий);
		КонецЕсли;
	...
	Для Каждого ТабЧасть Из ОбрабатываемыеСтроки Цикл
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			ТабЧасть.Значение,
			СтруктураДействий,
			ТабЧасть.Ключ,
	...
	Для Каждого ТабЧасть Из ОбрабатываемыеСтрокиНеОкруглять Цикл
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			ТабЧасть.Значение,
			СтруктураДействий,
			ТабЧасть.Ключ,
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	Для Каждого ТекущаяСтрока Из МассивНовыхСтрок Цикл
		МассивДанныхПоНоменклатуре.Добавить(ДанныеПоНоменклатуре(ТекущаяСтрока, РеквизитыЗаказа));
	КонецЦикла;
