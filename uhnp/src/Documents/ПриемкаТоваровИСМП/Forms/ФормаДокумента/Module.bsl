...
Процедура НастроитьЭлементыФормы(Форма)
	...
	Элементы.ТоварыДоляСпирта.Видимость =
		ИнтеграцияИСМПСлужебныйКлиентСервер.ТребуетсяУказаниеДолиСпиртаПриПриемкеОтгрузке(Операция, Объект.ВидПродукции);
	
	ДоступенВСД = (ЭтоПриемкаИзЕАЭС
		И ОбщегоНазначенияИСКлиентСервер.ЭтоПродукцияПодконтрольнаяВЕТИС(Объект.ВидПродукции));
	
	Элементы.ЗаполнитьДокументыСертификации.Видимость               = ЭтоПриемкаИзЕАЭС;
	Элементы.ТоварыИдентификаторПроисхожденияВЕТИС.Видимость        = (ДоступенВСД И Форма.ИспользуетсяПодсистемаВЕТИС);
	Элементы.ТоварыИдентификаторПроисхожденияВЕТИССтрокойНаФорме.Видимость = (ДоступенВСД И Не Форма.ИспользуетсяПодсистемаВЕТИС);
