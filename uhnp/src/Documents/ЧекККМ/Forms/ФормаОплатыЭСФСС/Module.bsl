	...
	Если Форма.ПредварительноеОдобрениеПолучено Тогда
		...
		ИначеЕсли Форма.ОплатаЭСФССПрошлаЧекНеПробит Тогда
			...
			ТекстПодсказки = НСтр("ru = 'Оплата электронным сертификатом уже прошла, необходимо пробить чек.';
									|en = 'The payment with the digital certificate has already been made; it is necessary to issue a receipt.'");
		Иначе
...
Процедура ОбработатьПолучениеСпискаЭлектронныхСертификатовФСС(ДоступныеСертификаты)
	...
	Если ДоступныеСертификаты.Количество() Тогда
		...
		Для Каждого СертификатНСПК Из ДоступныеСертификаты Цикл //Структура
			...
			ПредставлениеНомера = НСтр("ru = '*';
										|en = '*'") + Прав(ИдентификаторСертификатаНСПК, 6);
