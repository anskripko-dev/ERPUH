...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если Инициализация
		...
		Если ГруппировкаПоЭтапам Тогда
			МассивСвязейПараметровВыбора.Добавить(
				Новый СвязьПараметраВыбора("НаправлениеДеятельности", "Объект.НаправлениеДеятельности", РежимИзмененияСвязанногоЗначения.Очищать));
		КонецЕсли;
		//-- НЕ УТКА
...
Процедура УстановитьВозможностьРедактированияНД()
	...
	// Если заказ по этапам, то в нем запрещено менять направление деятельности
	ПараметрыДоступностиНД.ЗаказКакСчет =
...
Процедура ТаблицаГруппаЗатратНачалоВыбораЗавершениеНаСервере(Результат, ИмяТЧ, Знач ВыделенныеСтроки)
	...
	Если ИмяТЧ = "ОбеспечениеМатериаламиИРаботами" Тогда
		...
		Для Каждого ТекущиеДанные Из ОбрабатываемыеСтроки Цикл
			ЗаполнитьСлужебныеРеквизитыОбеспеченияЭтапаВСтроке(ЭтотОбъект, ТекущиеДанные);
		КонецЦикла;
