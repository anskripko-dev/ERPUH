...
Функция ТекстЗапросаСписокКОформлению()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПроизводством2_2") Тогда
		...
		ТекстЗапроса = 
			...
			|		КОГДА &ПланируетсяГрафикПроизводства
			|				И НЕ ГрафикПроизводства.Этап ЕСТЬ NULL 
			|			ТОГДА ГрафикПроизводства.Начало < &ТекущаяДата
			|		КОГДА НормативныйГрафикСтруктурыЗаказа.Начало ЕСТЬ НЕ NULL
			...
			|			И ГрафикПроизводства.ЗаказНаПроизводство = ПереработкаПоГрафикуПереопределяемый.Распоряжение
			|			И ГрафикПроизводства.Этап = ПереработкаПоГрафикуПереопределяемый.Ссылка
			...
			|		ПО НЕ &ПланируетсяГрафикПроизводства
			|			И ПереработкаПоГрафикуПереопределяемый.Ссылка = НормативныйГрафикСтруктурыЗаказа.Этап
			|ГДЕ
