...
Функция ТекстЗапроса_ТаблицаВыбытияВНА(НомераТаблиц)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаВНА.ЧастичноеВыбытиеМСФО
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ (ТаблицаВНА.НомерСтроки, ТаблицаВНА.ВНА) В
	|				(ВЫБРАТЬ
	|					МАКСИМУМ(Номера.НомерСтроки),
	|					Номера.ВНА
	|				ИЗ
	|					Документ.ВыбытиеВНАМСФО.ВНА КАК Номера
	|				ГДЕ
	|					ТаблицаВНА.Ссылка = &Ссылка
	|				СГРУППИРОВАТЬ ПО
	|					Номера.ВНА)
	|	КОНЕЦ КАК СтрокаОстатка,
...
Функция ТекстЗапроса_втТаблицаВНА(НомераТаблиц)
	...
	|	т.СтрокаОстатка КАК СтрокаОстатка,
...
Функция ТекстЗапроса_ТаблицаОшибок(НомераТаблиц)
	...
	//При заполнении из РСБУ(выбытие с возвратом нескольких материалов) могут быть дубли в ВНА
...
Процедура СформироватьДвижения(Движения, ДополнительныеСвойства, Отказ) Экспорт
	...
	ТаблицаПроводкиВНА.ЗаполнитьЗначения(Ложь, 	"ИсточникВНАДт");//Субконто на стороне Дт заменяем на ключевые ВНА
	ТаблицаПроводкиВНА.ЗаполнитьЗначения(Ложь, 	"ИсточникВНАКт");//Субконто на стороне Кт заменяем на ключевые ВНА
	
	Для каждого ПроводкаВНА Из ТаблицаПроводкиВНА Цикл
		Если ПроводкаВНА.ВидОперации = ПредопределенноеЗначение("Справочник.ВидыОпераций.НачислениеАмортизации") Тогда
			ПроводкаВНА.ИсточникВНАДт = Неопределено;//для амортизации не заменяем ключевые ВНА
		КонецЕсли;
	КонецЦикла;
