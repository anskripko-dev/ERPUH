...
Процедура ПроверитьЗаполнениеПлановойДаты(Отказ)
	...
	АктуальнаяВерсияСоглашения = РегистрыСведений.ВерсииГрафиковРасчетов.ПолучитьАктуальнуюВерсиюФинансовогоИнструмента(ДоговорКонтрагента, Дата);
	...
	Запрос.УстановитьПараметр("КовенантыДокумента", Ковенанты.Выгрузить());
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ Выборка.НайденаПлановаяДата 
			...
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'Некорректно указана плановая дата для ковенанта: %1 в строке: %2'"),
						Выборка.Ковенант,
						Выборка.НомерСтроки),
				ЭтотОбъект,
				ПутьКРеквизиту,
