...
Функция ТаблицаОстатковТоваровКОформлениюЗаявленийОВвозеТоваров(ДокументПоступления, ЗаполнениеНаОсновании = Ложь, ПериодОформления = Неопределено) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	Запрос.Текст = ТекстЗапросаТоварыКОформлениюЗаявленийОВвозе(Запрос);
...
Функция ТекстЗапросаТоварыКОформлениюЗаявленийОВвозе(Запрос, ЗаполнениеНаОсновании = Ложь)
	...
	|	МИНИМУМ(КорректировкаПриобретенияТовары.НомерСтроки),
	|	КорректировкаПриобретенияТовары.Номенклатура,
	|	КорректировкаПриобретенияТовары.НоменклатураПартнера,
	|	КорректировкаПриобретенияТовары.Характеристика,
	|	КорректировкаПриобретенияТовары.Серия,
	|	КорректировкаПриобретенияТовары.Склад,
	|	ЕСТЬNULL(КорректировкаПриобретенияТовары.Назначение.ВидДеятельностиНДС, ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)),
	|	КорректировкаПриобретения.ЗакупкаПодДеятельность,
	|	КорректировкаПриобретенияТовары.НомерГТД.СтранаПроисхождения,
	...
	|		Документ.КорректировкаПриобретения КАК КорректировкаПриобретения
	|	ПО 
	|		КорректировкаПриобретенияТовары.Ссылка = КорректировкаПриобретения.Ссылка
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
	|		ДанныеПоследнейКорректировки КАК ДанныеПоследнейКорректировки
	|	ПО 
	|		КорректировкаПриобретенияТовары.Ссылка = ДанныеПоследнейКорректировки.Ссылка
	|
	|ГДЕ
	...
	|	КорректировкаПриобретенияТовары.Серия,
	|	КорректировкаПриобретенияТовары.Склад,
	|	КорректировкаПриобретенияТовары.Назначение.ВидДеятельностиНДС,
	|	КорректировкаПриобретения.ЗакупкаПодДеятельность,
	|	КорректировкаПриобретенияТовары.НомерГТД,
	|	КорректировкаПриобретенияТовары.Количество,
	|	КорректировкаПриобретенияТовары.Сумма
	...
	|ВЫБРАТЬ
	|	ОтчетПереработчика2_5.Ссылка,
	|	МИНИМУМ(ОтчетПереработчикаВидыЗапасов.НомерСтроки),
	...
	|	НЕОПРЕДЕЛЕНО,
	|	"""",
	|	ОтчетПереработчикаВидыЗапасов.Количество,
	|	ОтчетПереработчикаВидыЗапасов.Сумма,
	|	""""
	|ИЗ
