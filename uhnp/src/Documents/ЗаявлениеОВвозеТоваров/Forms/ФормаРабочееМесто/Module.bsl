...
Процедура СформироватьЗаявлениеОВвозеТоваров(Команда)
	...
	Для Каждого Заявление Из МассивСсылок Цикл
		...
		ИначеЕсли ТипЗнч(Заявление.Распоряжение) = Тип("ДокументСсылка.ЗаключениеДоговораАренды") Тогда 
			ОткрытьФорму("Документ.ЗаявлениеОВвозеТоваров.ФормаОбъекта", Новый Структура("Основание", ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(Заявление.Распоряжение, "Договор")), , Истина);
