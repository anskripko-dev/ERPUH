...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ТабБюджетДвиженияДенежныхСредствСтатьяДвиженияДенежныхСредств.ОграничениеТипа = ВидыБюджетов.БюджетДвиженияДенежныхСредств.ТипЗначения;
	Элементы.ТабБюджетДоходовИРасходовСтатьяДоходовИРасходов.ОграничениеТипа = ВидыБюджетов.БюджетДоходовИРасходов.ТипЗначения;
	
	//
	Предназначение = Перечисления.ПредназначенияЭлементовСтруктурыОтчета;
	ПараметрыОП = ОперативноеПланированиеПовтИспУХ.ПолучитьПараметрыОперПланирования(Предназначение.БюджетДвиженияДенежныхСредств, Объект.Дата);
	ПериодичностьБДДС	= ПараметрыОП.ПериодичностьОперативногоПланирования;
	ПараметрыОП = ОперативноеПланированиеПовтИспУХ.ПолучитьПараметрыОперПланирования(Предназначение.БюджетДоходовИРасходов, Объект.Дата);
	...
	Если ТипЗнч(Объект.ИсходныйДокумент) = Тип("ДокументСсылка.ПоступлениеБезналичныхДенежныхСредств") Тогда 
		ХозяйственнаяОперация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ИсходныйДокумент, "ХозяйственнаяОперация");
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратДенежныхСредствОтПоставщика Тогда
			Состояния.Добавить(Перечисления.СостоянияИсполненияЗаявки.Исполнена);    
		КонецЕсли;
	КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	Если Объект.Скорректирован Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, Объект.ДатаКорректировки, Объект.Пользователь);
...
Процедура УстановитьПризнакВеденияРасчетовПоЗаказам()
	...
	ВестиРасчетыПоЗаказам = ВстраиваниеОПК.ВестиРасчетыПоЗаказам(Объект.ДоговорКонтрагента);
...
Процедура ПроверитьИзменениеКлючевыхРеквизитов(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ИсходныйДокумент) ИЛИ Объект.Скорректирован Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьНаСервере()
	...
	Объект.ДатаКорректировки = '00010101000000';
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц(Форма)
	...
	СвойстваДоговоров = ВстраиваниеОПК.ПараметрыРасчетовДоговоров(Договоры);
...
Процедура РассчитатьСуммуВзаиморасчетов(Форма, СтрокаФакт) Экспорт
	...
	ЗаполнитьПустыеКурсыВалют(Форма, СтрокаФакт);
	ПараметрыТекущегоКурса = Новый Структура("Валюта, Курс, Кратность", Форма.Объект.ВалютаДокумента, Форма.КурсДокумента, Форма.КратностьДокумента);
	ПараметрыНовогоКурса   = Новый Структура("Валюта, Курс, Кратность", СтрокаФакт.ВалютаВзаиморасчетов, СтрокаФакт.КурсВзаиморасчетов, СтрокаФакт.КратностьВзаиморасчетов);
	
	СтрокаФакт.СуммаВзаиморасчетов = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(СтрокаФакт.Сумма, ПараметрыТекущегоКурса, ПараметрыНовогоКурса);
...
Процедура ФактБюджетовДоговорКонтрагентаПриИзмененииНаСервере(ИмяТЧ, ИдСтроки)
	...
	СвойстваДоговора = ВстраиваниеОПК.ПолучитьПараметрыПланированияПоДоговору(СтрокаФакт.ДоговорКонтрагента, ЭтоБДДС);
...
&НаСервере
Процедура УстановитьОтборКонтрагента()
	
	// обрабатываем отбор по договорам ЕРПУХ 
	Если ОбщегоНазначения.ПодсистемаСуществует("БухгалтерияПредприятияПодсистемы") Тогда
		ФормыУХКлиентСервер.ДобавитьПараметрВыбора(Элементы.ТабБюджетДвиженияДенежныхСредствДоговорКонтрагента,
		"Отбор.Владелец", Объект.Контрагент);
		ФормыУХКлиентСервер.ДобавитьПараметрВыбора(Элементы.ТабБюджетДоходовИРасходовДоговорКонтрагента,
		"Отбор.Владелец", Объект.Контрагент);
	Иначе   
		ФормыУХКлиентСервер.ДобавитьПараметрВыбора(Элементы.ТабБюджетДвиженияДенежныхСредствДоговорКонтрагента,
