...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		тДанные = Объект.КорректировкаСинтетика.Выгрузить(, "ВидОтчета, Организация");
		Если НЕ ПолныеПраваУХ.ПроверитьВозможностьОбработкиДокумента(тДанные) Тогда
			ТекстСообщения = НСтр("ru = 'Недостаточно прав для просмотра документа.
...
Процедура ЗаполнитьТаблицуКлючевыхРеквизитов(Организация=Неопределено,ПериодОтчета=Неопределено)
	...
	Запрос.Текст=Запрос.Текст+"
	|Упорядочить ПО СтатусыПериодовСценариев.Периоды.ДатаНачала";
...
Процедура УстановитьПараметрыОтбора()
	...
	Элементы.ОрганизацияДляАктуализации.ПараметрыВыбора 		= НовыеПараметры;
...
Процедура ПодготовитьРабочиеДанные()
	...
	Если Объект.СпособВводаКорректировки=Перечисления.СпособыВводаКорректировок.ОбратноеРаспределение Тогда
		
		Возврат;
		
	ИначеЕсли Объект.СпособВводаКорректировки=Перечисления.СпособыВводаКорректировок.АктуализацияБюджетов Тогда
		
		ПоНесколькимОрганизациям=(Объект.КлючевыеРеквизиты.Количество()>1);
		Возврат;
...
Процедура УстановитьУсловноеОформление()
	...
	//Возможность ручного редактировавния корректировки. Не корректируются вручную значения, рассчитывающиеся
	//по формулам и синтетические значения при наличии у показателя аналитического раскрытия.
...
Процедура УстановитьВидимость()
	...
	Элементы.Сбалансированная.ТолькоПросмотр=(ЗначениеЗаполнено(Объект.ШаблонКорректировки) И Объект.Сбалансированная);
	...
	Если Объект.СпособВводаКорректировки=Перечисления.СпособыВводаКорректировок.АктуализацияБюджетов Тогда
		...
		Элементы.ТаблицаОрганизацийДляАктуализации.Видимость=ПоНесколькимОрганизациям;
	...
	Если ЗначениеЗаполнено(Объект.ШаблонКорректировки) ИЛИ Объект.СпособВводаКорректировки=Перечисления.СпособыВводаКорректировок.ОбратноеРаспределение Тогда
		...
		Если ЗначениеЗаполнено(Объект.ШаблонКорректировки) Тогда
			...
			Если Объект.СпособВводаКорректировки=Перечисления.СпособыВводаКорректировок.АвтоматическоеЗаполнение Тогда
				...
				Элементы.Расшифровать.Видимость= Истина;
	...
	Элементы.НезависимаяСуммаВО.Видимость=(Объект.СпособВводаКорректировки=Перечисления.СпособыВводаКорректировок.РучнойВвод
		И ТаблицаКорректировок.НайтиСтроки(Новый Структура("ПересчитыватьВалютнуюСумму",Истина)).Количество()>0);
...
Процедура ЗаполнитьДанныеСтрокиКорректировки(ТекущиеДанные,ДанныеЗаполнения)
	...
	Для Индекс=1 По ПараметрыСеанса.ЧислоДопАналитик Цикл
		...
		Если ЗначениеЗаполнено(ТекущиеДанные["ВидАналитики"+Индекс]) Тогда
			
			ТекущиеДанные.АналитикВидаОтчета=Индекс;
...
Процедура ПолучитьИсходныеЗначенияТаблица(ТабКорректировкиРаскрытия=Неопределено)
	...
	Для Каждого Строка ИЗ ТаблицаКорректировок Цикл
		...
		Если НЕ Строка.ЧислоАналитикГруппыРаскрытия=0 Тогда
			...
			|ОБЪЕДИНИТЬ ВСЕ
			|"+ПолучитьТекстДляВыборкиАналитика(Строка.АналитикВидаОтчета + Строка.ЧислоАналитикГруппыРаскрытия,Строка.ИдентификаторСинтетика);
			
		КонецЕсли;
		
	КонецЦикла;
	
	Запрос.Текст=Сред(ТекстСинтетики,СтрДлина("ОБЪЕДИНИТЬ ВСЕ") + 2);
	...
	Если Не ПустаяСтрока(ТекстАналитики) Тогда
		...
		ДополнитьДаннымиКорректировки(Запрос,Сред(ТекстАналитики,СтрДлина("ОБЪЕДИНИТЬ ВСЕ") + 2),ТабКорректировкиРаскрытия);
...
Процедура ТаблицаКорректировокПоказательПриИзмененииНаСервере(ИдентификаторСтроки)
	...
	ТекущиеДанные.ПересчитыватьВалютнуюСумму= Результат.ПересчитыватьВалютнуюСумму;
	ТекущиеДанные.ВидКурса					= Результат.ВидКурса;
...
Процедура ПриИзмененииКлючевогоРеквизита(ИдентификаторСтроки)
	...
	Если ТаблицаКорректировок.НайтиСтроки(Новый Структура("ПересчитыватьВалютнуюСумму",Истина)).Количество()=0 Тогда
		
		Объект.НезависимыеСуммы=Ложь;
		Элементы.НезависимаяСуммаВО.Видимость=Ложь;
...
Процедура ПодготовитьДанныеДляЗаписиОбъекта(ТекущийОбъект)
	...
	ТекущийОбъект.ТаблицаСобытийЗагрузки=РеквизитФормыВЗначение("ТаблицаСобытийЗагрузки");
...
Функция ПолучитьАдресДанныхКорректировки(ИдентификаторСтроки)
	...
	ДанныеКорректировки.Вставить("НезависимыеСуммы",Объект.НезависимыеСуммы);
...
&НаКлиенте
Процедура УстановитьВидимостьБаланса()
				
	Элементы.ДекорацияБалансКорректировки.Видимость=Объект.Сбалансированная;
	ОбновитьБалансКорректировки();
			
КонецПроцедуры // УстановитьВидимостьБаланса()
...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	ИначеЕсли Объект.КлючевыеРеквизиты.Количество()=1 Тогда
		
		Объект.КлючевыеРеквизиты[0].Организация=Объект.Организация;
		
	ИначеЕсли Объект.КлючевыеРеквизиты.НайтиСтроки(Новый Структура("Организация",Объект.Организация)).Количество()=0 Тогда
...
&НаКлиенте
Процедура ТаблицаКорректировокПослеУдаления(Элемент)
...
&НаСервере
Процедура ЗаполнитьОрганизацииДляАктуализацииНаСервере()
	...
	Запрос.Текст="ВЫБРАТЬ РАЗЛИЧНЫЕ
	             ...
	             |ГДЕ
	             |	СтатусыПериодовСценариев.УправлениеПериодомСценария = &УправлениеПериодомСценария
	             |	И НЕ СтатусыПериодовСценариев.Организации = ЗНАЧЕНИЕ(Справочник.Организации.Пустаяссылка)
	             |
	             |УПОРЯДОЧИТЬ ПО
	             |	СтатусыПериодовСценариев.Организации.Наименование";
...
&НаКлиенте
Процедура ЗаполнитьОрганизацииДляАктуализации(Команда) 
	
	Если Не ЗначениеЗаполнено(Объект.УправлениеПериодом) Тогда
		
		ПоказатьПредупреждение(,НСтр("ru = 'Укажите документ управления периодом.'"));
		Возврат;
		
	КонецЕсли;
	
	ЗаполнитьОрганизацииДляАктуализацииНаСервере();

КонецПроцедуры

&НаКлиенте
Процедура ПоНесколькимОрганизациямПриИзменении(Элемент)
...
&НаКлиенте
Процедура ТаблицаОрганизацийДляАктуализацииПередУдалением(Элемент, Отказ)
	
	Отказ=(Элемент.ТекущиеДанные.Организация=Объект.Организация);
...
Процедура НезависимаяСуммаВОПриИзменении(Элемент)
	...
	Если НЕ Объект.НезависимыеСуммы Тогда
		...
		ОбновитьЗначенияВОсновнойВалюте();
...
&НаСервере
Процедура ОбновитьЗначенияВОсновнойВалюте() 
	...
	Для Каждого ТекущиеДанные ИЗ ТаблицаКорректировок Цикл
		
		Если ТекущиеДанные.ПересчитыватьВалютнуюСумму Тогда
			
			РассчитатьЗначениеВОсновнойВалюте(ТекущиеДанные,СоответствиеВидовКурса,ДанныеОбъекта);
...
КонецПроцедуры // ОбновитьЗначенияВОсновнойВалюте()

&НаСервере
Процедура РассчитатьЗначениеВОсновнойВалюте(ТекущиеДанные,СоответствиеВидовКурса,ДанныеОбъекта)
				
	ТаблицаРаскрытия=ДанныеОбъекта.ТаблицыРаскрытия.Получить(ТекущиеДанные.ИдентификаторСинтетика);
	
	Если ТаблицаРаскрытия=Неопределено Тогда
		
		Возврат;
		
	КонецЕсли; 
	
	Если НЕ ЗначениеЗаполнено(ТекущиеДанные.ВидКурса) Тогда
		
		СтрокаШаблона = Нстр("ru = 'Не задан вид курса для показателя %1'");
		
		Если Не ПустаяСтрока(СтрокаШаблона) тогда			
			Сообщить(СтрШаблон(СтрокаШаблона, ТекущиеДанные.Показатель));
		КонецЕсли;
	...
	ПериодическиеКурсы=УправлениеРабочимиПроцессамиУХ.ПолучитьТаблицуКурсов(Объект.Сценарий,ТекущиеДанные.ПериодОтчета);
	
	МассивВалюта=КэшВалютыОтчетности.НайтиСтроки(Новый Структура("Организация",ТекущиеДанные.Организация));
	
	ОсновнаяВалюта=?(МассивВалюта.Количество()>0,МассивВалюта[0].ОсновнаяВалюта,Справочники.Валюты.ПустаяСсылка());
	
	Для Каждого СтрокаРаскрытия ИЗ ТаблицаРаскрытия Цикл
		
		СтрокаРаскрытия.Значение=ПересчитатьИзВалютыВВалюту(СтрокаРаскрытия.ЗначениеВалюта,
				СтрокаРаскрытия.АналитикаВалюта,
				ОсновнаяВалюта,
				ТекущиеДанные.ВидКурса,
				СоответствиеВидовКурса,
				ПериодическиеКурсы);
				
		СтрокаРаскрытия.НовоеЗначение=СтрокаРаскрытия.ИсходноеЗначение+СтрокаРаскрытия.Значение;
...
КонецПроцедуры // РассчитатьЗначениеВОсновнойВалюте()

&НаСервере
Функция ПересчитатьИзВалютыВВалюту(Значение,ИсходнаяВалюта,НоваяВалюта,ВидКурса,СоответствиеВидовКурса,ПериодическиеКурсы)
