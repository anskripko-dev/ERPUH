...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НезависимыеСуммы		= ДанныеКорректировки.НезависимыеСуммы;
	...
	Элементы.НезависимыеСуммы.Видимость=Валютный И ПересчитыватьВалютнуюСумму;
...
Процедура Расшифровать(Команда)
	...
	Если Элементы.ТаблицаРаскрытия.ТекущаяСтрока <> Неопределено Тогда
		...
		ОткрытьФорму("Документ.НастраиваемыйОтчет.Форма.РасшифровкаПоказателей"
...
Процедура РассчитатьИтогиДляКолонок(ТекТаблицаРаскрытия=Неопределено)
	...
	Если Не СтруктураОтбора=Неопределено Тогда
		...
		Если Не ПустаяСтрока(ТекстОтбор) Тогда
			...
			Запрос.Текст=Запрос.Текст+" ГДЕ "+Сред(ТекстОтбор,СтрДлина("И") + 2);
...
Процедура ТаблицаРаскрытияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если СтрНайти(КодАналитики,"Аналитика")=0 
		ИЛИ КодАналитики="АналитикаВалюта"
		ИЛИ (НЕ ЗначениеЗаполнено(Элементы.ТаблицаРаскрытия.ТекущиеДанные[КодАналитики])) Тогда
...
&НаКлиенте
Функция ПересчитыватьВалютныеЗначения(ТекущиеДанные)
	
	Возврат ПересчитыватьВалютнуюСумму 
		И ЗначениеЗаполнено(ТекущиеДанные.АналитикаВалюта)
		И НЕ НезависимыеСуммы
	
КонецФункции // ПересчитыватьВалютныеЗначения() 
...
Процедура РассчитатьПоЗначение()
	...
	Если ПересчитыватьВалютныеЗначения(ТекущиеДанные) Тогда
