...
Процедура ПриЗаписи(Отказ)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ПланированиеИКонтрольСлужебный") Тогда
		Параметры = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Ссылка);
		ОбщегоНазначения.ВыполнитьМетодКонфигурации("УстановкаЛимитовУХ.СнятьАктуальностьЛимитовПоДокументу", Параметры);
	КонецЕсли;
...
Процедура ВыполнитьЗаписьКорректировки(Отказ)
	...
	Для Индекс=1 По ПараметрыСеанса.ЧислоДопАналитик Цикл
		...
		|Выбор Когда КорректировкаСинтетика.Аналитика" // @query-part
		+Индекс+"=Неопределено
		|Тогда КорректировкаАналитика.Аналитика"+Индекс+"
		|Иначе КорректировкаСинтетика.Аналитика"+Индекс+" Конец КАК Аналитика"+Индекс+",";
...
Процедура ПолучитьТаблицуКорректировокПоШаблону() Экспорт
	...
	ТаблицаСобытийЗагрузки=ПротоколируемыеСобытияУХ.Завершить("КорректировкаЗначенийПоказателей_Заполнение",,,,Ложь);
...
Процедура РассчитатьЗначениеКорректировки(СтрокаКорректировки)
	...
	СтруктураКлючевыхРеквизитов.Вставить("ВидОтчета",	СтрокаКорректировки.ВидОтчета);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	тДанные = КорректировкаСинтетика.Выгрузить(, "ВидОтчета, Организация");
	Если НЕ ПолныеПраваУХ.ПроверитьВозможностьОбработкиДокумента(тДанные, "Запись") Тогда
...
Процедура ОбработатьТаблицуСобытийЗагрузки()
	...
	Событие="КорректировкаЗначенийПоказателей_АвтоматическоеЗаполнение";
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Иначе 
		...
		ВерсияРегламента		= ДанныеРегламента.ВерсияОрганизационнойСтруктуры;
		РассчитыватьЗависимые 	= ВерсияРегламента.АвтоматическиПересчитыватьЗависимыеПоказатели; 
		...
		ОбработкаЗаписи.НезависимыеСуммы		= НезависимыеСуммы;
...
Процедура ЗавершитьПротоколЗагрузки(ДанныеСсылки)
	...
	ТаблицаОшибок=ПротоколируемыеСобытияУХ.Завершить(Нстр("ru = 'Корректировка значений показателей'"),,,,Ложь);
	
	Для Каждого Строка ИЗ ТаблицаОшибок Цикл
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Строка.Сообщение, Ссылка, 
				"ЗаписьКорректировкиПоказателей", "Объект");
		
	КонецЦикла;
			
	ПротоколОшибок = ПротоколируемыеСобытияУХ.Завершить(
		"Документ.КорректировкаЗначенийПоказателей",
		,
		Ссылка);
...
Процедура ВыполнитьАктуализациюБюджетов(Отказ)
	...
	Если НовыйПериод.ДатаНачала<ПериодОтчета.ДатаНачала Тогда
		
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Некорректно установлены периоды для актуализации:
					           |период актуализации %1, новый период %2'"),
					ПериодОтчета,
					НовыйПериод);
	...
	Запрос.Текст= "ВЫБРАТЬ
	              ...
	              |	И НЕ ШаблоныКорректировокЗначенийПоказателейШаблонКорректировки.ДействияПриАктуализации = ЗНАЧЕНИЕ(Перечисление.ДействияПриАктуализацииБюджетов.ПустаяСсылка)
	...
	Если ПериодОтчета=НовыйПериод Тогда
		
		Запрос.Текст=Запрос.Текст+"
		|И ШаблоныКорректировокЗначенийПоказателейШаблонКорректировки.ДействияПриАктуализации=ЗНАЧЕНИЕ(Перечисление.ДействияПриАктуализацииБюджетов.НеПереносить)";
		
	КонецЕсли;
	
	Запрос.УстановитьПараметр("ШаблонКорректировки",ШаблонКорректировки);
	
	ТаблицаДействийПриАктуализации=Запрос.Выполнить().Выгрузить(); 
	
	Если ТаблицаДействийПриАктуализации.Количество()=0 Тогда
	...
	Для Каждого Строка ИЗ КлючевыеРеквизиты Цикл
		...
		АктуализироватьДанныеПоОрганизации(Строка.Организация,ТаблицаДействийПриАктуализации,Отказ);
		
	КонецЦикла;
				
КонецПроцедуры // ВыполнитьАктуализациюБюджетов() 

Процедура АктуализироватьДанныеПоОрганизации(ОрганизацияАктуализации,ТаблицаДействийПриАктуализации,Отказ)
	...
	ОбработкаАктуализации.Организация	= ОрганизацияАктуализации;
	...
	Если ОбработкаАктуализации.ПериодыГоризонта.Найти(СледующийПериод)=Неопределено Тогда
		
		ОбработкаАктуализации.НовыйПериод	= ПериодОтчета;
		
	Иначе
		
		ОбработкаАктуализации.НовыйПериод	= СледующийПериод;
	...
	ОбработкаАктуализации.ТаблицаДействийПриАктуализации.Загрузить(ТаблицаДействийПриАктуализации);
	Отказ=НЕ ОбработкаАктуализации.ВыполнитьАктуализациюБюджетов();	
	
КонецПроцедуры // АктуализироватьДанныеПоОрганизации() 
