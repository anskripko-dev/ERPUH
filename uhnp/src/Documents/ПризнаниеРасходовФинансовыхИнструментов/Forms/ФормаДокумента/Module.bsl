...
Процедура УправлениеФормой()
	...
	МСФОУХ.ЗаполнитьКэшируемыеЗначения(ЭтотОбъект);
	
	ПараметрыВыбораМСФО = МСФОКлиентСерверУХ.ПолучитьПараметрыВыбораДляСчетаБД(
								КэшируемыеЗначения.ПланСчетовМСФО, Неопределено);
	Элементы.СторноСчетДтМСФО.ПараметрыВыбора = ПараметрыВыбораМСФО;
	Элементы.СторноСчетКтМСФО.ПараметрыВыбора = ПараметрыВыбораМСФО;
	
	ПараметрыВыбораИсточник = МСФОКлиентСерверУХ.ПолучитьПараметрыВыбораДляСчетаБД(
								КэшируемыеЗначения.ПланСчетовНСБУ, Неопределено);
												
	Элементы.СторноСчетДтИсточник.ПараметрыВыбора = ПараметрыВыбораИсточник;
	Элементы.СторноСчетКтИсточник.ПараметрыВыбора = ПараметрыВыбораИсточник;
	
	Элементы.ФормаЗаполнитьДокумент.Доступность = КэшируемыеЗначения.ФормироватьПроводкиМСФО;
