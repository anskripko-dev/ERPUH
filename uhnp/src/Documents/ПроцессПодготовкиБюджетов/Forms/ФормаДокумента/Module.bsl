...
&НаКлиенте
Процедура ВернутьсяКЗадаче(Команда)
	
	Если Объект.Статус <> ПредопределенноеЗначение("Перечисление.СтатусыПроцессовПодготовкиБюджетов.Выполняется") Тогда
		ПоказатьПредупреждение(, СтрШаблон(НСтр("ru = 'Бюджетный процесс находится в статусе %1';
												|en = 'The budget process is in the %1 status'"), Объект.Статус));
		Возврат;
	КонецЕсли;
	...
	КонецЕсли;
	Если ТекущиеДанные.ЭтоГруппа Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Установите курсор на этап подготовки бюджетов для возврата к выполнению задачи';
										|en = 'Select a budgeting step to go back to the task'"));
	ИначеЕсли ТекущиеДанные.ИндексСтатуса = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Задача еще не началась';
										|en = 'Task is not started yet'"));
	Иначе
		Оповещение = Новый ОписаниеОповещения("ВернутьсяКЗадачеЗавершение", ЭтотОбъект, Новый Структура("ЭтапПодготовкиБюджетов", ТекущиеДанные.ЭтапПодготовкиБюджетов));
		ПоказатьВопрос(Оповещение, НСтр("ru = 'Текущая и все последующие задачи будут отмечены как неактивные. Продолжить?';
										|en = 'The current task and the following ones will be marked as inactive. Continue?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
&НаКлиенте
Процедура ВернутьсяКЗадачеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Нет Тогда
		Возврат;
	КонецЕсли;
	
	ВернутьсяКЗадачеСервер(ДополнительныеПараметры.ЭтапПодготовкиБюджетов);
	
КонецПроцедуры

&НаСервере
Процедура ВернутьсяКЗадачеСервер(ЭтапПодготовкиБюджетов)
	
	Документы.ПроцессПодготовкиБюджетов.ОтключитьАктивностьЗадачБюджетногоПроцесса(Объект.Ссылка, ЭтапПодготовкиБюджетов);
...
Функция ЗадачаПоОбъекту(Документ, ПроцессПодготовкиБюджетов)
	...
	Запрос.Текст = 
		...
		|	И НЕ БюджетныеЗадачи.Неактивна
