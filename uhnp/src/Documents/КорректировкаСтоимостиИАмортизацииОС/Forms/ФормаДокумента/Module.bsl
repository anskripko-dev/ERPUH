		...
		ИЛИ ОбновитьВсе Тогда
			...
			ЗначениеСвойства = ?(Объект.ОтражениеУвеличенияАмортизационнойПремии,
					НСтр("ru = 'Амортизационная премия (увеличение) (%1)';
						|en = 'Bonus depreciation (increase) (%1)'"),
					НСтр("ru = 'Амортизационная премия (уменьшение) (%1)';
						|en = 'Bonus depreciation (decrease) (%1)'"));
			...
			Элементы.ОСАмортизационнаяПремия.Подсказка = 
				?(Объект.ОтражениеУвеличенияАмортизационнойПремии,
					НСтр("ru = 'На какую сумму будет увеличена амортизационная премия';
						|en = 'Amount to increase the depreciation bonus by'"),
					НСтр("ru = 'На какую сумму будет уменьшена амортизационная премия';
						|en = 'Amount to decrease the depreciation bonus by'"));
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("КорректировкаАмортизации") 
		...
		Если Объект.ОтражениеУменьшенияАмортизацииСторно Тогда
			Элементы.ГруппаУменьшениеАмортизацииПрошлыхЛет.Заголовок = НСтр("ru = 'Уменьшение амортизации прошлых лет';
																			|en = 'Depreciation decrease of previous years'");
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("КорректировкаАмортизационнойПремии") 
		...
		Если Объект.ОтражениеУменьшенияАмортизационнойПремииСторно Тогда
			Элементы.ГруппаУвеличениеАмортизационнойПремии.Заголовок = НСтр("ru = 'Уменьшение амортизационной премии (сторно)';
																			|en = 'Bonus depreciation decrease (storno)'");
		...
		Иначе
			Элементы.ГруппаУвеличениеАмортизационнойПремии.Заголовок = НСтр("ru = 'Увеличение амортизационной премии';
																			|en = 'Bonus depreciation increase'");
...
Процедура НастроитьЗависимыеЭлементыФормыНаСервере(ИзмененныеРеквизиты = "")
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("Организация") 
		...
		Элементы.ОСАмортизацияНаНачалоГодаБУ.Заголовок = СтрШаблон(НСтр("ru = 'БУ (%1)';
																		|en = 'AC (%1)'"), ВалютаРеглПредставление);
		Элементы.ОСАмортизацияНаНачалоГодаНУ.Заголовок = СтрШаблон(НСтр("ru = 'НУ (%1)';
																		|en = 'TA (%1)'"), ВалютаРеглПредставление);
		Элементы.ОСАмортизацияНаНачалоГодаПР.Заголовок = СтрШаблон(НСтр("ru = 'ПР (%1)';
																		|en = 'PD (%1)'"), ВалютаРеглПредставление);
		Элементы.ОСАмортизацияНаНачалоГодаУУ.Заголовок = СтрШаблон(НСтр("ru = 'УУ (%1)';
																		|en = 'MA (%1)'"), ВалютаУпрПредставление);
		Элементы.ОСАмортизацияНаНачалоГодаЦФ.Заголовок = СтрШаблон(НСтр("ru = 'Амортизация на начало года (целевое финансирование) (%1)';
																		|en = 'Depreciation as of the year start (target financing) (%1)'"), ВалютаРеглПредставление);
		
		Элементы.ОСАмортизацияНаНачалоГодаБУ_Отдельно.Заголовок = СтрШаблон(НСтр("ru = 'Амортизация на начало года (бухгалтерский учет) (%1)';
																				|en = 'Depreciation as of the year start (local accounting) (%1)'"), ВалютаРеглПредставление);
		Элементы.ОСАмортизацияНаНачалоГодаУУ_Отдельно.Заголовок = СтрШаблон(НСтр("ru = 'Амортизация на начало года (управленческий учет) (%1)';
																				|en = 'Depreciation as of the year start (management accounting) (%1)'"), ВалютаУпрПредставление);
	КонецЕсли;
