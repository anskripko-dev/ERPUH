...
Функция ТекстЗапроса_СчетаДокумента(НомераТаблиц) Экспорт
	...
	Возврат
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ЕСТЬNULL(СправочникКонтрагенты.Ссылка, СчетаУчета.ОбъектИнвестирования) КАК ФинансовыйИнструмент,
	...
	|		ПО СчетаУчета.ОбъектИнвестирования = СправочникКонтрагенты.ОрганизационнаяЕдиница
	|ГДЕ
	...
	|	Документ.СчетРасчетовСубконто2,
	|	Документ.СчетРасчетовСубконто3
	|ИЗ
...
Функция ТекстЗапроса_ПриобретениеФИ()
	...
	//ПЕРВЫЕ 1 - исключаем дубли контрагентов
	
	Возврат
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	т.ОбъектИнвестирования КАК ФинансовыйИнструмент,
	|	т.Дата КАК Период,
