...
Процедура ДополнитьПараметрамиВНА(Запрос, Реквизиты)
	...
	Запрос.УстановитьПараметр("ВидУчетаНСБУ", 	Неопределено);
	Запрос.УстановитьПараметр("ВидУчетаМСФО", 	Перечисления.ВидыУчета.МСФО);
...
Функция ТекстЗапроса_СуммыВНА(НомераТаблиц)
	...
	|	ЕСТЬNULL(втТаблицаРасчетВНА.СПИ, ТаблицаВНА.СрокПолезногоИспользованияМСФО) КАК СрокПолезногоИспользованияМСФО,
	|	ЕСТЬNULL(втТаблицаРасчетВНА.СпособНачисленияАмортизации, ТаблицаВНА.СпособНачисленияАмортизацииМСФО) КАК СпособНачисленияАмортизацииМСФО,
	|	ЕСТЬNULL(втТаблицаРасчетВНА.ЛиквидационнаяСтоимость, ТаблицаВНА.ЛиквидационнаяСтоимостьМСФО) КАК ЛиквидационнаяСтоимостьМСФО,
	|	ЕСТЬNULL(втТаблицаРасчетВНА.ПрофильРаспределения, ТаблицаВНА.СпособОтраженияРасходовПоАмортизацииМСФО) КАК СпособОтраженияРасходовПоАмортизацииМСФО,
	|	ЕСТЬNULL(втТаблицаРасчетВНА.КоэффициентУскорения, ТаблицаВНА.КоэффициентУскоренияМСФО) КАК КоэффициентУскоренияМСФО,
	|	ТаблицаВНА.ПараметрыАмортизацииВНАМСФО КАК ПараметрыАмортизацииВНАМСФО,
	|	ЕСТЬNULL(втТаблицаРасчетВНА.ПараметрВыработки, ТаблицаВНА.ПараметрВыработкиМСФО) КАК ПараметрВыработкиМСФО,
	|	ЕСТЬNULL(втТаблицаРасчетВНА.ОбъемПродукцииРабот, ТаблицаВНА.ОбъемВыработкиМСФО) КАК ОбъемВыработкиМСФО
...
Функция ТекстЗапроса_ТаблицаВводВНА(НомераТаблиц)
	...
	|	втТаблицаВНА.ПараметрыУчетаВНА.ТипВНА КАК ТипВНА,
	...
	|	втТаблицаВНА.ПараметрыУчетаВНА.ТипВНА,
...
Функция ТекстЗапроса_ЗадолженностьПоИнтервалам(НомераТаблиц)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(Интервалы.Интервал) КАК Интервал,
	...
	|		ПО Интервалы.ДатаОкончания > ПересекающийНачалоИнтервал.ДатаОкончания
	...
	|
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Справочник.ИнтервалыЗадолженностей.ПустаяСсылка),
	|	КОНЕЦПЕРИОДА(ДОБАВИТЬКДАТЕ(&ДатаОкончанияПериода, МЕСЯЦ, 12), ДЕНЬ),
	|	КОНЕЦПЕРИОДА(ЕСТЬNULL(ДОБАВИТЬКДАТЕ(&ДатаОкончанияПериода, МЕСЯЦ, 0), ДАТАВРЕМЯ(1, 1, 1)), ДЕНЬ)
	|ГДЕ
	|	НЕ ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	|					втИнтервалыВозможныПересечения)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	""Долгосрочная часть, не заданы интервалы"",
	|	КОНЕЦПЕРИОДА(ДОБАВИТЬКДАТЕ(&ДатаОкончанияПериода, МЕСЯЦ, 12 * 100), ДЕНЬ),
	|	КОНЕЦПЕРИОДА(ЕСТЬNULL(ДОБАВИТЬКДАТЕ(&ДатаОкончанияПериода, МЕСЯЦ, 12), ДАТАВРЕМЯ(1, 1, 1)), ДЕНЬ)
	|ГДЕ
	|	НЕ ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	...
	|
	|ВЫБРАТЬ
	|	МАКСИМУМ(МаксимальныйИнтервал.Ссылка),
	|	NULL,
	|	МАКСИМУМ(Интервалы.ДатаОкончания)
	|ИЗ
	|	Перечисление.ВидыИнтервалов КАК МаксимальныйИнтервал
	|		ЛЕВОЕ СОЕДИНЕНИЕ втИнтервалыВозможныПересечения КАК Интервалы
	|		ПО (ИСТИНА)
	|ГДЕ
	|	МаксимальныйИнтервал.Ссылка = ЗНАЧЕНИЕ(Перечисление.ВидыИнтервалов.СрокПогашенияПоФИ)
	|
	|ИМЕЮЩИЕ
	|	НЕ МАКСИМУМ(Интервалы.ДатаОкончания) ЕСТЬ NULL
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	МИНИМУМ(ВЫБОР
	|			КОГДА втГрафикиФИ.КодПериодаОперации В (3, 4)
	|					ИЛИ втГрафикиФИ.Период = &ДатаОкончанияПериода
	|				ТОГДА втГрафикиФИ.Проценты_Задолженность
	|		КОНЕЦ) КАК Проценты_ЗадолженностьМинимум,
	|	МИНИМУМ(ВЫБОР
	|			КОГДА втГрафикиФИ.КодПериодаОперации В (3, 4)
	|					ИЛИ втГрафикиФИ.Период = &ДатаОкончанияПериода
	|				ТОГДА втГрафикиФИ.ЭСП_Проценты_Задолженность
	|		КОНЕЦ) КАК ЭСП_Проценты_ЗадолженностьМинимум,
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА ЕСТЬNULL(втГрафикиФИ.Период <= втИнтервалы.ДатаНачала, втГрафикиФИ.КодПериодаОперации = 3)
	|				ТОГДА втГрафикиФИ.Период
	|		КОНЕЦ) КАК ДатаНачала,
	|	МАКСИМУМ(втГрафикиФИ.Период) КАК ДатаОкончания
	|ПОМЕСТИТЬ втДатыИнтерваловПоГрафику
	|ИЗ
	|	втГрафикиФИ КАК втГрафикиФИ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втИнтервалы КАК втИнтервалы
	|		ПО (ЕСТЬNULL(втГрафикиФИ.Период <= втИнтервалы.ДатаОкончания, ИСТИНА))
	|ГДЕ
	|	втГрафикиФИ.КодПериодаОперации В (1, 2, 3, 4)
	|
	|СГРУППИРОВАТЬ ПО
	|	втГрафикиФИ.ФИ,
	...
	|
	|ИМЕЮЩИЕ
	|	МАКСИМУМ(втГрафикиФИ.Период) >= МАКСИМУМ(втИнтервалы.ДатаНачала)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ФИ,
	|	ДатаНачала,
	...
	|	СтрокаГрафикаНачало.ДисконтНаБалансе КАК ДисконтНаБалансе,
	|	СтрокаГрафикаНачало.Валюта КАК Валюта,
	|	СтрокаГрафикаНачало.ЭтоАктив КАК ЭтоАктив,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ПредыдущийИнтервалПоФИ.Проценты_ЗадолженностьМинимум, 0) - ЕСТЬNULL(втДатыГрафикаДляИнтервалов.Проценты_ЗадолженностьМинимум, 0) > 0
	|			ТОГДА ЕСТЬNULL(ПредыдущийИнтервалПоФИ.Проценты_ЗадолженностьМинимум, 0) - ЕСТЬNULL(втДатыГрафикаДляИнтервалов.Проценты_ЗадолженностьМинимум, 0)
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Проценты_ДолгосрочнаяЧастьПоИнтервалу,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ПредыдущийИнтервалПоФИ.ЭСП_Проценты_ЗадолженностьМинимум, 0) - ЕСТЬNULL(втДатыГрафикаДляИнтервалов.ЭСП_Проценты_ЗадолженностьМинимум, 0) > 0
	|			ТОГДА ЕСТЬNULL(ПредыдущийИнтервалПоФИ.ЭСП_Проценты_ЗадолженностьМинимум, 0) - ЕСТЬNULL(втДатыГрафикаДляИнтервалов.ЭСП_Проценты_ЗадолженностьМинимум, 0)
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ЭСП_Проценты_ДолгосрочнаяЧастьПоИнтервалу,
	...
	|						ТОГДА ЗНАЧЕНИЕ(Справочник.СчетаУчетаФИ.СчетДолгосрочныхПроцентов)
	|					ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СчетаУчетаФИ.СчетДолгосрочныхПроцентов)
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА СтрокаГрафикаНачало.ЭтоАктив
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ втДатыИнтерваловПоГрафику КАК ПредыдущийИнтервалПоФИ
	|		ПО втДатыГрафикаДляИнтервалов.ФИ = ПредыдущийИнтервалПоФИ.ФИ
	|			И втДатыГрафикаДляИнтервалов.ДатаНачала = ПредыдущийИнтервалПоФИ.ДатаОкончания";
...
Функция ТекстЗапроса_Проводки(НомераТаблиц)
	...
	ТекстПроводки.Добавить(ТекстЗапроса_РеклассПоИнтерваламПроценты(НомераТаблиц));
...
Функция ТекстЗапроса_ПолучениеДолга(НомераТаблиц)
	...
	|	И НЕ т.ЭтоДКЗ
	|	И НЕ т.ЭтоЛизинг
	|	И втФИ.ДатаПризнания ЕСТЬ NULL";
...
Функция ТекстЗапроса_РеклассПоИнтерваламПроценты(НомераТаблиц)
	...
	|		ИНАЧЕ -1
	|	КОНЕЦ * ВЫБОР
	|		КОГДА втОперацииИнтервалов.ДисконтНаБалансе = ИСТИНА
	|			ТОГДА втОперацииИнтервалов.ЭСП_Проценты_ДолгосрочнаяЧастьПоИнтервалу
	|		ИНАЧЕ втОперацииИнтервалов.Проценты_ДолгосрочнаяЧастьПоИнтервалу
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР
	|		КОГДА втОперацииИнтервалов.ЭтоАктив
	|			ТОГДА ""Счет""
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втСчетаФИ КАК втСчетаФИ
	|		ПО втОперацииИнтервалов.ФИ = втСчетаФИ.ФИ
	|			И втОперацииИнтервалов.СчетПроцентов = втСчетаФИ.СчетУчета
	|ГДЕ
	|	ВЫБОР
	|			КОГДА втОперацииИнтервалов.ДисконтНаБалансе = ИСТИНА
	|				ТОГДА втОперацииИнтервалов.ЭСП_Проценты_ДолгосрочнаяЧастьПоИнтервалу
