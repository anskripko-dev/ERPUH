...
Функция ПолучитьТаблицуПроводок(КонтекстДокумента) Экспорт
	...
	Запрос.УстановитьПараметр("УчетнаяПолитика", 		УПМСФО);//для счета НРП
...
Функция ПолучитьТекстЗапроса()
	...
	|	ПроводкиТК.Содержание КАК Комментарий,
	...
	|		ПО (СчетНРП.Ссылка = ЗНАЧЕНИЕ(Справочник.ФиксированныеСчетаУчетаБД.НераспределеннаяПрибыльНепокрытыйУбытокПредыдущихПериодов))
