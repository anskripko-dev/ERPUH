...
Функция ПолучитьТекстЗапросаЗаполнение(НомераТаблиц) Экспорт
	...
	|ГДЕ
	|	ХодПретензионноИсковойРаботы.ЭтапРаботы <> &ЭтапЗакрыта
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|					ГДЕ
	|						ЗакрытыеПретензии.ЭтапРаботы = &ЭтапЗакрыта)) КАК ТаблицаСторноРезервы";
	
	ПараметрыЗамены = Новый Структура;
	ПараметрыЗамены.Вставить("ЭтапЗакрыта", "ЗНАЧЕНИЕ(Справочник.ЭтапыПретензионноИсковойРаботы.Закрыта)");
	Возврат МСФОЗапросыУХ.ЗаменитьПараметрыЗапроса(ТекстЗапроса, ПараметрыЗамены);
...
Процедура СформироватьДвижения(Движения, ДополнительныеСвойства, Отказ) Экспорт
	...
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом() Тогда
		...
		МодульТрансформационныеКорректировкиУХ = ОбщегоНазначения.ОбщийМодуль("ТрансформационныеКорректировкиУХ");
		
		Реквизиты.Вставить("ВидОперации", Справочники.ВидыОпераций.РезервПоПретензиям);	
		МодульТрансформационныеКорректировкиУХ.СформироватьКорректировку(Реквизиты, Отказ, ТаблицыДляДвижений.ПроводкиРезервы);
		Если Отказ = Истина Тогда
			Возврат;
		КонецЕсли;	
		
		Реквизиты.Вставить("ВидОперации", Справочники.ВидыОпераций.СторноРезерваПоПретензиям);	
		МодульТрансформационныеКорректировкиУХ.СформироватьКорректировку(Реквизиты, Отказ, ТаблицыДляДвижений.ПроводкиСторноРезервов);
		Если Отказ = Истина Тогда
			Возврат;
		КонецЕсли;
