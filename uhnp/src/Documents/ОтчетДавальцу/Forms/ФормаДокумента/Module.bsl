...
Процедура УстановитьУсловноеОформление()
	...
	ПараметрыУсловногоОформления.ИмяПоляЕдиницаИзмерения = "УслугиНоменклатураЕдиницаИзмерения";
...
Процедура ПриИзмененииНалогообложенияНДССервер(КэшированныеЗначения)
	...
	Если Не СтавкаНДС = Объект.СтавкаНДС Тогда
		Для Каждого ТекущаяСтрока Из Объект.Продукция Цикл
			ТекущаяСтрока.СтавкаНДС = Объект.СтавкаНДС;
		КонецЦикла;
	КонецЕсли;
...
Процедура ЗаполнитьПоЗаказуСервер()
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Объект.Продукция, СтруктураДействий, КэшируемыеЗначения);	
