...
Процедура ЗаполнитьПоОперациям(ДанныеДляЗаполнения = Неопределено) Экспорт
	...
	Если НастройкиУчета.ИспользоватьВыходныеИзделия Тогда
		...
		ШтрихкодыУпаковок.Загрузить(ДанныеОпераций.ШтрихкодыУпаковок);
	...
	Если НастройкиУчета.ИспользоватьМатериалы Тогда
		...
		ШтрихкодыУпаковокМатериалы.Загрузить(ДанныеОпераций.ШтрихкодыУпаковокМатериалы);
	...
	Если РежимЗаписи <> РежимЗаписиДокумента.Запись Тогда
		...
		Иначе
			...
			Если ДополнительныеСвойства.Свойство("ПериодПерезаписиДвижений") Тогда
				ПериодПерезаписиДвижений = ДополнительныеСвойства.ПериодПерезаписиДвижений;
			Иначе
...
Процедура ПриЗаписи(Отказ)
	...
	Если ЗначениеЗаполнено(ПартияПроизводства) Тогда
		...
		ЭлементБлокировки.УстановитьЗначение("Код", НомерПартииЗапуска);
		ЭлементБлокировки.УстановитьЗначение("Документ", Распоряжение);
		...
		Исключение
			Событие = НСтр("ru = 'Этап производства';
							|en = 'Этап производства'", ОбщегоНазначения.КодОсновногоЯзыка()) + "."
				+ НСтр("ru = 'Изменение партии производства';
						|en = 'Изменение партии производства'", ОбщегоНазначения.КодОсновногоЯзыка());
			...
			ТекстСообщения = НСтр("ru = 'Не удалось заблокировать этапы партии.';
									|en = 'Не удалось заблокировать этапы партии.'");
	...
	Если НеобходимоВыполнитьОбработчик("ЗаписатьРучноеРазмещениеЭтапаВГрафике") Тогда
		КлючПартия = ?(ДинамическаяСтруктура,
			ПартияПроизводства.УникальныйИдентификатор(),
			Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000"));
		Измерения = Новый Структура;
		Измерения.Вставить("ЗаказНаПроизводство", Распоряжение);
		Измерения.Вставить("Этап", Ссылка);
		Измерения.Вставить("КлючПартия", КлючПартия);
		
		Если КоличествоНаЕдиницуПартииВыпуска <> 0 Тогда
			Коэффициент = Справочники.УпаковкиЕдиницыИзмерения.КоэффициентУпаковки(УпаковкаПлан, Неопределено);
			КоличествоПартий = КоличествоУпаковокПлан * Коэффициент / КоличествоНаЕдиницуПартииВыпуска;
		Иначе
			КоличествоПартий = 0;
		КонецЕсли;
		Реквизиты = Новый Структура;
		Реквизиты.Вставить("Подразделение", Подразделение);
		Реквизиты.Вставить("Спецификация", Спецификация);
		Реквизиты.Вставить("КоличествоПартий", КоличествоПартий);
		
		РегистрыСведений.ГрафикЭтаповПроизводства2_2.ЗаписатьРучноеРазмещениеЭтапаВГрафике(
			Измерения, ДополнительныеСвойства.ГрафикПроизводства, Реквизиты);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЭтоВыпускающийЭтап
		...
		Если ВыходныеИзделия.Найти(Ложь, "Отменено") = Неопределено Тогда
			...
			Если ПроизводствоНаСтороне Тогда
				// Для производства на стороне проверка не является исключительной - проведение не прекращается.
				ОбщегоНазначения.СообщитьПользователю(
					ТекстСообщения,
					ЭтотОбъект,
					"ВыходныеИзделия");
	...
	ПроизводствоСервер.ПроверкаЗаполненияИсполнителей(ЭтотОбъект,, Отказ);
	...
	Если Статус <> Перечисления.СтатусыЭтаповПроизводства2_2.Формируется Тогда
		...
		Если ЭтотОбъект.ПланироватьРаботуВидовРабочихЦентров Тогда
			...
			Если ЭтотОбъект.ВидыРабочихЦентров.Количество() > 0 Тогда
				...
				Ссылки = ВидыРабочихЦентров.ВыгрузитьКолонку("ВидРабочегоЦентра");
				Для каждого СтрокаАльтернативныйВидРЦ Из АльтернативныеВидыРабочихЦентров Цикл
					Ссылки.Добавить(СтрокаАльтернативныйВидРЦ.ВидРабочегоЦентра);
				КонецЦикла;
				ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(Ссылки, "ПараллельнаяЗагрузка");
				...
				Для Каждого СтрокаОсновнойВидРЦ Из ЭтотОбъект.ВидыРабочихЦентров Цикл
					...
					ПроверитьВидыРабочихЦентров(
						СтрокаОсновнойВидРЦ, НомерСтроки, ЗначенияРеквизитов[СтрокаОсновнойВидРЦ.ВидРабочегоЦентра], Отказ);
					...
					Для Каждого СтрокаАльтернативныйВидРЦ Из ЭтотОбъект.АльтернативныеВидыРабочихЦентров.НайтиСтроки(СтруктураПоиска) Цикл
						...
						ПроверитьВидыРабочихЦентров(
							СтрокаАльтернативныйВидРЦ, НомерСтроки, ЗначенияРеквизитов[СтрокаАльтернативныйВидРЦ.ВидРабочегоЦентра], Отказ);
...
Процедура ПроверитьВидыРабочихЦентров(ТекущаяСтрока, НомерСтроки, РеквизитыВидаРЦ, Отказ)
	...
	Если РеквизитыВидаРЦ.ПараллельнаяЗагрузка
		И НЕ ЗначениеЗаполнено(ТекущаяСтрока.Загрузка) Тогда
		
		Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
			ПараметрыТабличнойЧасти.Имя, НомерСтроки, "Загрузка");
...
Процедура ЗаполнитьВидыРабочихЦентровПоСпецификации(ДанныеСпецификации, ДанныеЭтапа)
	...
	Для Сч = 1 По 2 Цикл
		ИмяТаблицы = ?(Сч = 1, "ВидыРабочихЦентров", "АльтернативныеВидыРабочихЦентров");
		Если Не ДанныеСпецификации.Свойство(ИмяТаблицы) Тогда
			Возврат;
		КонецЕсли;
		ТабличнаяЧасть = ЭтотОбъект[ИмяТаблицы]; // ДокументТабличнаяЧасть.ЭтапПроизводства2_2.ВидыРабочихЦентров
		
		Если Сч = 1 И ДанныеСпецификации.ВидыРабочихЦентров.Колонки.Найти("Использовать") = Неопределено Тогда
		...
		Для Каждого Строка Из НайденныеСтроки Цикл
			...
			НоваяСтрока.ВремяРаботыНорматив = Строка.ВремяРаботыНаЕдиницуПартииВыпуска;
			НоваяСтрока.ЗагрузкаНорматив = Строка.ЗагрузкаНаЕдиницуПартииВыпуска;
...
Процедура УменьшитьВыполненноеКоличествоВСтрокеТЧ(ИмяТЧ, СтрокаТЧ, КоличествоНеПроизведено, ИмяФлагаПроизведено)
	...
	Иначе
		...
		КолонкиИсключения = ?(ИмяТЧ = "Трудозатраты",
			"КодСтроки, КлючВидРабот",
			"КодСтроки, ИдентификаторСтроки, КлючНоменклатура");
		
		НоваяСтрока = ТабличнаяЧастьПоИмени(ИмяТЧ).Добавить();
		
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТЧ,, КолонкиИсключения);
...
Процедура ДобавитьДанныеОперацииВСтрокуНеПроизведено(ИмяТЧ, ДанныеОперации, СтруктураПоиска, ИменаПолей)
	...
	Для каждого СтрокаВыпуск Из НайденныеСтроки Цикл
		...
		Иначе
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаВыпуск,, КолонкиИсключения);
...
Процедура ЗаполнитьМатериалыПоОперациям(МатериалыОпераций)
	...
	СтрокиКПересчетуОперации = Новый Массив;
	СтрокиКПересчетуРасход = Новый Массив;
	...
	Для Индекс = -РасходМатериаловИРабот.Количество()+1 По 0 Цикл
		...
		Для каждого СтрокаОперация Из МатериалыОпераций.НайтиСтроки(СтруктураПоиска) Цикл
			...
			Иначе
				СтрокаОперация.Количество = СтрокаОперация.Количество - КоличествоЗачет;
				СтрокиКПересчетуОперации.Добавить(СтрокаОперация);
			КонецЕсли;
		...
		ИначеЕсли Количество < Строка.Количество Тогда
			...
			СтрокиКПересчетуРасход.Добавить(Строка);
		КонецЕсли;
		
	КонецЦикла;
	
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		СтрокиКПересчетуОперации,
		СтруктураДействий,
		МатериалыОпераций,
		КэшированныеЗначения);
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		СтрокиКПересчетуРасход,
		СтруктураДействий,
		РасходМатериаловИРабот,
		КэшированныеЗначения);
...
Процедура ЗаполнитьИзделияПоОперациям(ИмяТЧ, ИзделияОпераций, ИзделияОперацийСерии)
	...
	ТаблицаКопия = ЭтотОбъект[ИмяТЧ].Выгрузить(); //ТаблицаЗначений
	ТаблицаКопия.Колонки.Добавить("ЦеховаяКладовая", Новый ОписаниеТипов("Булево"));
	ТаблицаКопия.Колонки.Добавить("НужноОкруглять", Новый ОписаниеТипов("Булево"));
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ТаблицаКопия, СтруктураДействий, КэшированныеЗначения);
	
	СтруктураДействийОкругление = Новый Структура;
	СтруктураДействийОкругление.Вставить("ЗаполнитьПризнакЦеховаяКладовая", Новый Структура("ИмяПоляСклад", "Получатель"));
	СтруктураДействийОкругление.Вставить("ПересчитатьКоличествоЕдиниц", ПроизводствоКлиентСервер.ПараметрыПересчетаКоличестваЕдиниц());
	
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ТаблицаКопия, СтруктураДействийОкругление, КэшированныеЗначения);
	...
	Для Индекс = 0 По ЭтотОбъект[ИмяТЧ].Количество() - 1 Цикл
		...
		СтрокаКопия = ТаблицаКопия[Индекс];
		
		Если Строка.Количество <> СтрокаКопия.Количество Тогда
		...
		ЗаполнитьЗначенияСвойств(Строка, СтрокаКопия, "Количество, КоличествоУпаковок");
...
Процедура ЗагрузитьДанныеОперацийВИзделия(ИмяТЧ, ИзделияОпераций)
	...
	ПараметрыДействия = Новый Структура("Подразделение, ИмяПоляСклад", Неопределено, "Получатель");
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	Если ПроизводствоНаСтороне
		И Не ПроизводствоНаСтороне2_5 Тогда
...
Процедура ЗаполнитьДополнительныеКолонкиВидыЗапасовМатериалы(МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов, Отказ)
	...
	|		ТаблицаОбеспечение.КорАналитикаУчетаНоменклатуры		КАК КорАналитикаУчетаНоменклатурыДляЗаполнения,
	|		ТаблицаОбеспечение.Количество							КАК Количество,
	|		&ХозОперацияОбеспечение									КАК ХозяйственнаяОперация,
	...
	|			ПО ТаблицаОбеспечение.Номенклатура = СправочникНоменклатура.Ссылка
	|	
	|	ГДЕ
	...
	|		И ТаблицаОбеспечение.Период >= &ПериодСКоторогоНужноПересчитыватьВидыЗапасов
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("ПроизводствоНаСтороне2_5", ПроизводствоНаСтороне2_5);
...
Процедура ЗаполнитьВидыЗапасовИзделия(МенеджерВременныхТаблиц, ТаблицаВидыЗапасов)
	...
	|ПОМЕСТИТЬ ВТПриходуемыеВидыЗапасовИзделияПромежуточная
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТВидыЗапасовИзделия.Организация				КАК Организация,
	|	ВТВидыЗапасовИзделия.АналитикаУчетаНоменклатуры	КАК АналитикаУчетаНоменклатуры,
	|	ВТВидыЗапасовИзделия.ВидЗапасов					КАК ВидЗапасов,
	|	ВТВидыЗапасовИзделия.НомерГТД					КАК НомерГТД,
	|	ВТВидыЗапасовИзделия.Количество					КАК Количество,
	|	ВТВидыЗапасовИзделия.КоличествоПоРНПТ			КАК КоличествоПоРНПТ,
	|	ВТВидыЗапасовИзделия.ДатаОперации				КАК ДатаОперации,
	|	ВТВидыЗапасовИзделия.ХозяйственнаяОперация		КАК ХозяйственнаяОперация,
	|	ВТВидыЗапасовИзделия.СтатьяКалькуляции			КАК СтатьяКалькуляции,
	|	ВТВидыЗапасовИзделия.СписатьНаРасходы			КАК СписатьНаРасходы,
	...
	|ИЗ
	|	ВТПриходуемыеВидыЗапасовИзделияПромежуточная КАК ВТВидыЗапасовИзделия
	...
	Запрос.Текст ="УНИЧТОЖИТЬ ВТПриходуемыеВидыЗапасовИзделияПромежуточная";
	Запрос.Выполнить();
...
Функция ВременныеТаблицыДанныхДокумента()
	...
	Если ПроизводствоНаСтороне2_5 Тогда
		Запрос.УстановитьПараметр("ВидыЗапасовИзделия", ВидыЗапасовИзделия.ВыгрузитьКолонки());
	Иначе
...
Процедура ВременныеТаблицыДляЗаполненияВидыЗапасовМатериалы(МенеджерВременныхТаблиц)
	...
	|	ТаблицаОбеспечение.АналитикаУчетаНоменклатуры		КАК АналитикаУчетаНоменклатуры,
	|	ТаблицаОбеспечение.КорАналитикаУчетаНоменклатуры	КАК КорАналитикаУчетаНоменклатуры,
	|	СУММА(ТаблицаОбеспечение.Количество)				КАК КоличествоРасход,
	|	0													КАК КоличествоПриход,
	|	СУММА(ТаблицаОбеспечение.КоличествоПоРНПТ)			КАК КоличествоПоРНПТРасход,
	|	0													КАК КоличествоПоРНПТПриход,
	|	ТаблицаОбеспечение.Период							КАК Период,
	|	ТаблицаОбеспечение.НомерГТД							КАК НомерГТД
	|ИЗ
...
Процедура ПериодСКоторогоНужноПересчитыватьВидыЗапасов(МенеджерВременныхТаблиц)
	...
	|				ИЛИ (ВыходныеИзделия.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|					И ВыходныеИзделия.Номенклатура.ПрослеживаемыйТовар))
	...
	|				ИЛИ (ПобочныеИзделия.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|					И ПобочныеИзделия.Номенклатура.ПрослеживаемыйТовар))
...
Процедура ЗарегистрироватьОбновлениеПараметровРазмещенияВГрафике(ДанныеДоИзменения)
	...
	МожетБытьЗапланирован = Статус = Перечисления.СтатусыЭтаповПроизводства2_2.Формируется
		ИЛИ Статус = Перечисления.СтатусыЭтаповПроизводства2_2.Сформирован
		ИЛИ Статус = Перечисления.СтатусыЭтаповПроизводства2_2.КВыполнению;
	
	Если СвойстваДокумента.РежимЗаписи <> РежимЗаписиДокумента.Проведение
		ИЛИ (НЕ ДинамическаяСтруктура И НЕ ИспользуетсяГрафикПроизводства)
		ИЛИ (НЕ РучноеРазмещениеВГрафике И НЕ МожетБытьЗапланирован)
...
Процедура ЗарегистрироватьОчисткуЗаданийКРасчетуГрафикаПроизводства()
	...
	Если ПроведениеДокументов.СвойстваДокумента(ЭтотОбъект).ЭтоНовый
		ИЛИ НЕ УправлениеПроизводствомПовтИсп.ИспользуетсяГрафикПроизводства() Тогда
		Возврат;
	КонецЕсли;
	
	ЗаданияКРасчетуГрафика = Неопределено;
	ЕстьЗаданияКРасчетуГрафика = Ложь;
	Если ДинамическаяСтруктура Тогда
		ЕстьЗаданияКРасчетуГрафика = РегистрыСведений.ЗаданияКРасчетуСтруктурыЗаказаГрафикПроизводства.ЕстьЗадания(Ссылка);
	Иначе
		ЗаданияКРасчетуГрафика = РегистрыСведений.ЗаданияКРасчетуГрафикаПроизводства.ЗаданияПоЭтапу(Распоряжение, Ссылка);
		ЕстьЗаданияКРасчетуГрафика = ЗаданияКРасчетуГрафика.Количество() > 0;
	КонецЕсли;
	...
	Если ЕстьЗаданияКРасчетуГрафика
		И (ОтменаПроведения
			ИЛИ НЕ МожетБытьЗапланирован
			ИЛИ РедактированиеРучногоРазмещения) Тогда
		ПараметрыОбработки = Новый Структура("Задания", ЗаданияКРасчетуГрафика);
		ДобавитьОбработчикОбновленияСвязанныхДанных("ОчиститьЗаданияКРасчетуГрафикаПроизводства", ПараметрыОбработки);
	КонецЕсли;
		...
		Если ДанныеДоИзменения.Реквизиты[Реквизит] <> ЭтотОбъект[Реквизит] Тогда
			...
			Если Реквизит = "ПланироватьНеРанее"
				...
				Если ЗначениеЗаполнено(ДанныеГрафика)
					И ДанныеГрафика[0].Начало >= ПланироватьНеРанее Тогда
...
Процедура ОбновитьСвязанныеДанные(Отказ, УдалениеПроведения = Ложь)
	...
	Если НеобходимоВыполнитьОбработчик("ИзменилисьПараметрыРазмещенияВГрафике") Тогда
		...
		Иначе
			...
			Если ДинамическаяСтруктура Тогда
				Автозапуск = НЕ УправлениеПроизводствомПовтИсп.ИспользуетсяГрафикПроизводства()
					И НЕ ДополнительныеСвойства.Свойство(КлючНеЗаполнятьНормативныйГрафик());
				РегистрыСведений.ЗаданияКРасчетуСтруктурыЗаказаГрафикПроизводства.ДобавитьЗадание(Распоряжение, Ссылка,, Автозапуск);
			Иначе
	...
	Если НеобходимоВыполнитьОбработчик("ОчиститьГрафикПроизводства") Тогда
		...
		РегистрыСведений.ГрафикЭтаповПроизводства2_2.ОчиститьГрафикЭтапа(Распоряжение, Ссылка);
		
	КонецЕсли;
	
	Если НеобходимоВыполнитьОбработчик("ОчиститьЗаданияКРасчетуГрафикаПроизводства", ПараметрыОбработки) Тогда
		
		Если ДинамическаяСтруктура Тогда
			РегистрыСведений.ЗаданияКРасчетуСтруктурыЗаказаГрафикПроизводства.УдалитьЗаданияПоЭтапу(Распоряжение, Ссылка);
		Иначе
			РегистрыСведений.ЗаданияКРасчетуГрафикаПроизводства.УдалитьЗадания(ПараметрыОбработки.Задания);
		КонецЕсли;
	...
	Если НеобходимоВыполнитьОбработчик("РассчитатьОчередьПроизводственныхОпераций") Тогда
		...
		Если ВыполняетсяПакетнаяПередачаКВыполнению() Тогда
			РегистрыСведений.ЗаданияКРасчетуОчередиПроизводственныхОпераций.ДобавитьЗадания(Ссылка, Ложь);
		Иначе
	...
	Если НеобходимоВыполнитьОбработчик("ОбновитьСостояниеЗаказаВОчереди") Тогда
		...
		СостоянияДокументов.ДобавитьЗаданияКОтражениюСостоянияЗаказов(Неопределено, Распоряжение,, РегистрыКОтражению);
...
Процедура ЗаполнитьНовыйКодСтрокиПриПереработкеНаСтороне(Отказ)
	...
	ПространствоБлокировки = ?(ТипЗнч(ЗаказПереработчику) = Тип("ДокументСсылка.ЗаказПереработчику2_5"),
		"Документ.ЗаказПереработчику2_5",
		"Документ.ЗаказПереработчику");
	
	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить(ПространствоБлокировки);
	ЭлементБлокировки.УстановитьЗначение("Ссылка", ЗаказПереработчику);
	ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
	Попытка
		Блокировка.Заблокировать();
	Исключение
		Событие = НСтр("ru = 'Этап производства';
						|en = 'Этап производства'", ОбщегоНазначения.КодОсновногоЯзыка()) + "."
			+ НСтр("ru = 'Заполнение кодов строк';
					|en = 'Заполнение кодов строк'", ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(Событие,
			УровеньЖурналаРегистрации.Ошибка,
			Метаданные(),
			Ссылка,
		...
		ТекстСообщения = НСтр("ru = 'Не удалось заблокировать этапы, включенные в заказ переработчику.';
								|en = 'Не удалось заблокировать этапы, включенные в заказ переработчику.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка,,, Отказ);
		Возврат;
	КонецПопытки;
...
Процедура ЗаполнитьМаксимальнуюДатуПроизводства()
	
	Если НЕ ПроизводствоОднойДатой Тогда
		ДатаПроизводства = Макс(Документы.ЭтапПроизводства2_2.МаксимальнаяДатаВТЧ(НайтиНеОтмененныеСтроки("ВыходныеИзделия"), "ДатаПроизводства"),
								Документы.ЭтапПроизводства2_2.МаксимальнаяДатаВТЧ(НайтиНеОтмененныеСтроки("ПобочныеИзделия"), "ДатаПроизводства"));
	КонецЕсли;
...
Процедура ЗаполнитьМаксимальнуюДатуРасхода()
...
Процедура Пересчитать(Коэффициент) Экспорт
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ОбеспечениеМатериаламиИРаботами, СтруктураДействий, КэшированныеЗначения);
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ВыходныеИзделия, СтруктураДействий, КэшированныеЗначения);
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ПобочныеИзделия, СтруктураДействий, КэшированныеЗначения);
