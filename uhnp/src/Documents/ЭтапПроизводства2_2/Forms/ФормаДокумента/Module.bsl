...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОтметкаПланироватьНеРанее
		...
		ОбщегоНазначения.СообщитьПользователю(
			ТекстСообщения, 
			Объект.Ссылка,
			?(Объект.ДинамическаяСтруктура, "ПланироватьНеРанееДата", "НадписьПланироватьНеРанее"),
			,
			Отказ);
...
Процедура ГиперСсылкаНадписьРасходМатериаловИРаботНажатие(Элемент)
	...
	УстановитьСвойстваСворачиваемойГруппы(
		Элементы.ТаблицаИзрасходованныеМатериалы, 
...
&НаКлиенте
Процедура ПланироватьНеРанееДатаПриИзменении(Элемент)
	
	Объект.ПланироватьНеРанее = ПланироватьНеРанееДата;
...
Процедура ОтметкаПланироватьНеРанееПриИзменении(Элемент)
	...
	Если Не ОтметкаПланироватьНеРанее Тогда
		...
		Если Объект.ДинамическаяСтруктура Тогда
			ПланироватьНеРанееДата = '00010101';
		Иначе
...
Процедура ВыходныеИзделияНазначениеПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(СтруктураДействий) Тогда
		...
		ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ТекущиеДанные, СтруктураДействий, КэшируемыеЗначения);
...
Процедура РасходМатериаловИРаботПередУдалением(Элемент, Отказ)
	РаботаСТабличнымиЧастямиКлиент.КэшироватьТекущуюСтроку(Элементы.РасходМатериаловИРабот, ЭтотОбъект);
КонецПроцедуры

&НаКлиенте
Процедура РасходМатериаловИРаботПослеУдаления(Элемент)
	РасходМатериаловИРаботПослеУдаленияНаСервере();
КонецПроцедуры
...
Процедура РасходМатериаловИРаботПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	ДополнительныеПараметры = Новый Структура;
	СобытияФормКлиент.ПриИзмененииЭлемента(ЭтотОбъект, Элементы.РасходМатериаловИРабот, ДополнительныеПараметры);
	Если ДополнительныеПараметры.Свойство("ТребуетсяСерверныйВызов") Тогда
		ПодключитьОбработчикОжидания("РасходМатериаловИРаботПриОкончанииРедактированияДляСерверногоВызова", 0.5, Истина);
	КонецЕсли;
...
Процедура ВидыРабочихЦентровВидРабочегоЦентраПриИзменении(Элемент)
	...
	ВидыРабочихЦентровПересчитатьЗагрузку(Элементы.ВидыРабочихЦентров.ТекущиеДанные);
...
&НаКлиенте
Процедура ВидыРабочихЦентровВремяРаботыНормативПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВидыРабочихЦентровЗагрузкаНормативПриИзменении(Элемент)
...
Процедура ИзменитьДатуОтгрузки(Команда)
	...
	Если ТипЗнч(ГрафикПроизводства) = Тип("Структура") Тогда
		ИзменитьДатуОтгрузкиНаСервере(ГрафикПроизводства.Начало);
	КонецЕсли;
...
Процедура ВыходныеИзделия_ИзменитьКачествоОтменитьСтроку(Команда)
	...
	ДобавитьВСтруктуруДействияПриИзмененииНоменклатуры("ВыходныеИзделия", ЭтотОбъект, СтруктураДействий);
...
Процедура ПобочныеИзделия_ИзменитьКачествоОтменитьСтроку(Команда)
	...
	ДобавитьВСтруктуруДействияПриИзмененииНоменклатуры("ПобочныеИзделия", ЭтотОбъект, СтруктураДействий);
...
Процедура ЗаполнитьПолучателяНаСервере(ПараметрыТЧ, ВыделенныеСтроки, Получатель)
	...
	ПараметрыУказанияСерий = ЭтотОбъект[ПараметрыРедактированияЭтапа.ИмяРеквизитаПараметрыУказанияСерий];
...
Процедура ИзменитьКачествоЗавершение(ТекущаяСтрока, ДополнительныеПараметры) Экспорт
	...
	ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, КэшируемыеЗначения);
...
Процедура ПослеИзменениеКачествоИОтменыСтроки(ИмяТЧ, ИдентификаторыНовойСтроки, ИдентификаторыСтаройСтроки)
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтрокаТаблицы, СтруктураДействий, КэшируемыеЗначения);
...
Процедура Подключаемый_РазрешитьРедактированиеРеквизитовОбъекта(Команда)
	НастроитьПравоРедактированияРеквизитовГруппыЗапланировано(ЭтотОбъект);
...
Процедура УстановитьУсловноеОформлениеВидыРабочихЦентров()
	...
	#Область ВремяРаботыТолькоПросмотр
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВидыРабочихЦентров.ВремяРаботыНорматив");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Заполнено;
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВидыРабочихЦентров.ВремяРаботы");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветТекстаОтмененнойСтрокиДокумента);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<расчет от норматива>';
																|en = '<calculation from standard>'"));
	
	#КонецОбласти
	
	#Область ЗагрузкаТолькоПросмотр
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВидыРабочихЦентров.ЗагрузкаНорматив");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Заполнено;
...
Процедура НастроитьЭлементыФормы()
	...
	Если ЗначениеЗаполнено(ДанныеПартииПроизводства.ОсновноеИзделиеНоменклатура) Тогда
		Элементы.УпаковкаПлан.ПодсказкаВвода = ОсновноеИзделиеЕдиницаИзмеренияНаименование;
		Элементы.УпаковкаФакт.ПодсказкаВвода = ОсновноеИзделиеЕдиницаИзмеренияНаименование;
		Элементы.УпаковкаОтменено.ПодсказкаВвода = ОсновноеИзделиеЕдиницаИзмеренияНаименование;
	КонецЕсли;
	...
	КратныйРасчет = ОдновременноПроизводимоеКоличество > 1;
	ТекстШаблона = ?(КратныйРасчет, НСтр("ru = 'Норматив на каждые %1 %2';
										|en = 'Standard for each %1 %2'"), НСтр("ru = 'Норматив на %1 %2';
																					|en = 'Standard for %1 %2'"));
	Элементы.ВидыРабочихЦентровГруппаНормативы.Заголовок = СтрШаблон(ТекстШаблона,
		Формат(КоличествоНормативРасчетаВидовРЦ, "ЧГ=;"),
		ОсновноеИзделиеЕдиницаИзмеренияНаименование);
	
	Элементы.ПланироватьНеРанееДата.ФорматРедактирования = ФорматДатыГрафика;
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если Инициализация
		...
		Если ЗначениеЗаполнено(Объект.ОрганизацияДавалец) Тогда
			...
			ПараметрВыбораОрганизации = Новый ПараметрВыбора("Организация", Объект.ОрганизацияДавалец);
			ПараметрыВыбораИсполнителя.Добавить(ПараметрВыбораОрганизации);
	...
	Если СтруктураРеквизитов.Свойство("СпособПланирования")
		...
		Если Форма.ПланируетсяГрафикПроизводства
			...
			Элементы.СпособПланированияВручную.Видимость = Форма.ПланируетсяГрафикПроизводства;
			Элементы.РучноеРедактированиеГрафикаПроизводства.Видимость = Форма.Объект.РучноеРазмещениеВГрафике;
			
			Элементы.ГруппаПланироватьНеРанее.Видимость = Форма.ПланируетсяГрафикПроизводства;
			Элементы.ГруппаПланироватьНеРанее.Доступность = НЕ Форма.Объект.РучноеРазмещениеВГрафике
															И Объект.Статус <> СтатусНачат()
															И Объект.Статус <> СтатусЗавершен();
			
			КоличествоПолейСлева = ?(Элементы.СпособПланированияУББВ.Видимость, 1, 0)
				+ ?(Элементы.СпособПланированияББВ.Видимость, 1, 0)
				+ ?(Элементы.СпособПланированияВручную.Видимость, 1, 0);
			
			КоличествоПолейСправа = ?(Элементы.ГруппаДлительностьУББВ.Видимость, 1, 0)
				+ ?(Элементы.РучноеРедактированиеГрафикаПроизводства.Видимость, 1, 0);
			
			Элементы.ГруппаСпособОтступ1.Видимость = КоличествоПолейСправа > 0
				И (КоличествоПолейСлева - КоличествоПолейСправа) >= 1;
			
			Элементы.ГруппаСпособОтступ2.Видимость = КоличествоПолейСправа > 0
				И (КоличествоПолейСлева - КоличествоПолейСправа) = 2;
	...
	Если СтруктураРеквизитов.Свойство("ГрафикПроизводства")
		...
		Если Форма.ПланируетсяГрафикПроизводства Тогда
			...
			Если Форма.ГрафикПроизводства.Рассчитан Тогда
				...
				ГрафикПроизводства = Форма.ГрафикПроизводства;// Структура
				
				ШаблонПериодПроизводства = НСтр("ru = 'с %1 по %2';
												|en = 'from %1 to %2'");
				
				ТекстПериодПроизводства = СтрШаблон(ШаблонПериодПроизводства,
											Формат(ГрафикПроизводства.Начало, Форма.ФорматДатыГрафика),
											Формат(ГрафикПроизводства.Окончание, Форма.ФорматДатыГрафика));
	...
	Если СтруктураРеквизитов.Свойство("ОтметкаПланироватьНеРанее")
		...
		Элементы.ПланироватьНеРанееДата.Видимость = Форма.ОтметкаПланироватьНеРанее
	...
	Если СтруктураРеквизитов.Свойство("НеОтгружатьЧастями")
		...
		ПеренестиДатуОтгрузки = Объект.НеОтгружатьЧастями
			И Форма.ПланируетсяГрафикПроизводства
			И ЗначениеЗаполнено(ГрафикПроизводства.Начало)
			И Объект.ДатаОтгрузки <> НачалоДня(ГрафикПроизводства.Начало);
		...
		Если ПеренестиДатуОтгрузки Тогда
			...
			Форма.Элементы.ИзменитьДатуОтгрузки.Заголовок = СтрШаблон(
				...
				Формат(ГрафикПроизводства.Начало, "ДЛФ=D"));
...
//		ИЛИ СтруктураРеквизитов.Свойство("ПобочныеИзделия")
	...
	Если СтруктураРеквизитов.Свойство("ВидыРабочихЦентров")
		...
		Элементы.ПорядокРаботыВидовРабочихЦентров.Доступность = НесколькоВидовРЦ
			И (НЕ Объект.ДинамическаяСтруктура
				ИЛИ Объект.ПорядокРаботыВидовРабочихЦентров <> ПредопределенноеЗначение("Перечисление.ПорядокРаботыВидовРабочихЦентров.Одновременно"));
...
&НаКлиентеНаСервереБезКонтекста
Процедура НастроитьПравоРедактированияРеквизитовГруппыЗапланировано(Форма)
	
	Если НЕ ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(Форма, "ПараметрыЗапретаРедактированияРеквизитов") Тогда
		Возврат;
	КонецЕсли;
	
	ПравоРедактироватьЗапланировано = (Форма.ОперацииНазначены = Ложь);
	
	Для каждого ПараметрЗапрета Из Форма.ПараметрыЗапретаРедактированияРеквизитов Цикл
		Если ПараметрЗапрета.ИмяРеквизита = "КоличествоУпаковокПлан"
			ИЛИ ПараметрЗапрета.ИмяРеквизита = "УпаковкаПлан" Тогда
			ПараметрЗапрета.ПравоРедактирования = ПравоРедактироватьЗапланировано;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьПлановуюДатуПоступления()
	...
	Если ПланируетсяГрафикПроизводства Тогда
		ПлановаяДатаПоступления = НачалоДня(ГрафикПроизводства.Окончание);
	Иначе
...
&НаСервере
Процедура ЗаполнитьСпецификациюВСтроках(Строки, РежимПроверитьЗаполнить = Ложь)
	...
	Для каждого Строка Из Строки Цикл
		ДанныеОбИзделии = УправлениеДаннымиОбИзделияхКлиентСервер.СобратьДанныеОбИзделииДляВыбораСпецификации(
			Объект, Строка, ПараметрыВыбораСпецификаций.ОбеспечениеМатериаламиИРаботами, ЭтотОбъект);
		Если РежимПроверитьЗаполнить Тогда
			ДанныеОбИзделии.Вставить("ТекущаяСпецификация", Строка.Спецификация);
		КонецЕсли;
		
		ДанныеСтрок.Добавить(ДанныеОбИзделии);
	КонецЦикла;
	
	УправлениеДаннымиОбИзделиях.ЗаполнитьСпецификациюВСтроках(
		Строки, ДанныеСтрок, ПараметрыВыбораСпецификаций.ОбеспечениеМатериаламиИРаботами);
...
&НаСервере
Процедура РасходМатериаловИРаботПослеУдаленияНаСервере()
	СобытияФорм.ПриИзмененииЭлемента(ЭтотОбъект, "Сырье");
КонецПроцедуры

&НаКлиенте
Процедура РасходМатериаловИРаботПриОкончанииРедактированияДляСерверногоВызова()
	РасходМатериаловИРаботПриОкончанииРедактированияНаСервере();
КонецПроцедуры

&НаСервере
Процедура РасходМатериаловИРаботПриОкончанииРедактированияНаСервере()
	СобытияФорм.ПриИзмененииЭлемента(ЭтотОбъект, "Сырье");
КонецПроцедуры
...
Процедура Изделия_ИзменитьКачествоОтменитьСтрокуЗавершение(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	ЗаполнитьЗначенияСвойств(НоваяСтрока, СтараяСтрока,, "КодСтроки,КлючНоменклатура");
...
Процедура ПолучитьСтрокиИзБуфераОбменаНаСервере(ИмяТЧ)
	...
	СтрокиКПодборуСпецификаций = Новый Массив;
	...
	Для каждого СтрокаИзБуфера Из СтрокиИзБуфера Цикл
		...
		Если ПодборТоваров Тогда 
			...
			СтрокиКОбработке.Добавить(ТекущаяСтрока);
			
		КонецЕсли;
		
		МассивИдентификаторов.Добавить(ТекущаяСтрока.ПолучитьИдентификатор());
		
		Если ИмяТЧ = "ОбеспечениеМатериаламиИРаботами"
			И ТекущаяСтрока.Производится Тогда
			СтрокиКПодборуСпецификаций.Добавить(ТекущаяСтрока);
		КонецЕсли;
	...
	Если СтрокиКПодборуСпецификаций.ВГраница() <> -1 Тогда
		ЗаполнитьСпецификациюВСтроках(СтрокиКПодборуСпецификаций, Истина);
	КонецЕсли;
	
	ПослеИзмененияСтрокТЧПрограммноНаСервере(ИмяТЧ, МассивИдентификаторов, СтруктураДействий);
...
Процедура ОбработатьШтрихкодыСервер(СтруктураПараметровДействия, КэшированныеЗначения)
	...
	Если СтруктураПараметровДействия.ИмяТЧ = "РасходМатериаловИРабот" Тогда
		СобытияФорм.ПриИзмененииЭлемента(ЭтотОбъект, "Сырье");
	КонецЕсли;
...
Процедура ОбработкаВыбораПодборНаСервере(ТаблицаТоваров, ИмяТЧ)
	...
	ИначеЕсли ИмяТЧ = "ПобочныеИзделия" Тогда
		ВнутренняяПереработкаКлиентСервер.ДобавитьДействиеЗаполнитьВладелецИзделияДоступен(ЭтотОбъект, СтруктураДействий);
	КонецЕсли;
	ДобавитьВСтруктуруДействияПриИзмененииНоменклатуры(ИмяТЧ, ЭтотОбъект, СтруктураДействий, Истина);
	...
	Если СтрокиКПодборуСпецификаций.ВГраница() <> -1 Тогда
		ЗаполнитьСпецификациюВСтроках(СтрокиКПодборуСпецификаций);
	КонецЕсли;
...
Процедура ЗагрузитьРезультатПодбораМатериаловПоСпецификации(ТаблицаТоваров)
	...
	Для каждого Строка Из ТаблицаТоваров Цикл
		Если Строка.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Набор Тогда
			Для каждого НоваяСтрока Из ДобавитьСоставНабора(Строка, ИмяТЧ) Цикл
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка, "Назначение, НазначениеОбеспечения");
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ИменаРеквизитов.Вставить("Наименование");
	ИменаРеквизитов.Вставить("ВидНоменклатуры");
	ИменаРеквизитов.Вставить("ЕдиницаИзмеренияНаименование", "ЕдиницаИзмерения.Наименование");
	ДанныеОсновногоИзделия = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		ДанныеПартииПроизводства.ОсновноеИзделиеНоменклатура, ИменаРеквизитов);
	ОсновноеИзделиеНаименование = ДанныеОсновногоИзделия.Наименование;
	ОсновноеИзделиеВидНоменклатуры = ДанныеОсновногоИзделия.ВидНоменклатуры;
	ОсновноеИзделиеЕдиницаИзмеренияНаименование = ДанныеОсновногоИзделия.ЕдиницаИзмеренияНаименование;
	...
	ЗаполнитьРеквизитыПоСпецификации();
	...
	РаботаСТабличнымиЧастями.ИнициализироватьКэшСтрок(Элементы.РасходМатериаловИРабот);
...
&НаСервере
Процедура ЗаполнитьРеквизитыПоСпецификации()
	
	ОдновременноПроизводимоеКоличество = 0;
	Если Не Объект.Спецификация.Пустая()
		И Не Объект.Этап.Пустая() Тогда
		ТекстЗапроса = 
			"ВЫБРАТЬ
			|	ВЫБОР
			|		КОГДА РесурсныеСпецификации.ВыпускПроизвольнымиПорциями
			|			ТОГДА ЭтапыПроизводства.ОдновременноПроизводимоеКоличествоЕдиницПартийИзделий
			|		ИНАЧЕ ЭтапыПроизводства.ОдновременноПроизводимоеКоличествоЕдиницПартийИзделий
			|				* СпецификацииИзделий.КоличествоУпаковокНаЕдиницуПартииЗапуска
			|				* ЕСТЬNULL(&КоэффициентУпаковки, 1)
			|	КОНЕЦ КАК ОдновременноПроизводимоеКоличество
			|ИЗ
			...
			|	ПО НЕ РесурсныеСпецификации.ВыпускПроизвольнымиПорциями
			|		И СпецификацииИзделий.ВидНоменклатуры = &ВидНоменклатуры
			|		И СпецификацииИзделий.Номенклатура = &Номенклатура
			|		И СпецификацииИзделий.Характеристика = &Характеристика
			|		И СпецификацииИзделий.Спецификация = &Спецификация
			|ГДЕ
			|	ЭтапыПроизводства.Ссылка = &Этап";
		...
		Запрос = Новый Запрос(ТекстЗапроса);
		Запрос.УстановитьПараметр("ВидНоменклатуры", ОсновноеИзделиеВидНоменклатуры);
		Запрос.УстановитьПараметр("Номенклатура", ДанныеПартииПроизводства.ОсновноеИзделиеНоменклатура);
		Запрос.УстановитьПараметр("Характеристика", ДанныеПартииПроизводства.ОсновноеИзделиеХарактеристика);
		Запрос.УстановитьПараметр("Спецификация", Объект.Спецификация);
		Запрос.УстановитьПараметр("Этап", Объект.Этап);
		
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			ОдновременноПроизводимоеКоличество = Выборка.ОдновременноПроизводимоеКоличество;
		КонецЕсли;
	КонецЕсли;
	
	Если ОдновременноПроизводимоеКоличество <> 0 Тогда
		КоличествоНормативРасчетаВидовРЦ = ОдновременноПроизводимоеКоличество;
	ИначеЕсли Объект.КоличествоНаЕдиницуПартииВыпуска <> 0 Тогда
		КоличествоНормативРасчетаВидовРЦ = Объект.КоличествоНаЕдиницуПартииВыпуска;
	Иначе
		КоличествоНормативРасчетаВидовРЦ = 1;
	КонецЕсли;
...
Процедура ПрочитатьВнешниеРеквизиты()
	...
	Запрос = Новый Запрос(
		...
		|
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	ИСТИНА КАК ОперацииНазначены
		|ИЗ
		|	РегистрСведений.ОперацииКСозданиюСменныхЗаданий КАК НазначенныеОперации
		|ГДЕ
		|	НазначенныеОперации.СменноеЗадание <> ЗНАЧЕНИЕ(Документ.СменноеЗадание.ПустаяСсылка)
		|	И НазначенныеОперации.Этап = &Этап
		|
		|");
	Запрос.УстановитьПараметр("Этап", Объект.Ссылка);
	...
	ВыборкаРеквизитыДеления = РезультатыЗапроса[0].Выбрать();
	Если ВыборкаРеквизитыДеления.Следующий() Тогда
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, ВыборкаРеквизитыДеления);
	КонецЕсли;
	
	ВыборкаНазначенныеОперации = РезультатыЗапроса[1].Выбрать();
	Если ВыборкаНазначенныеОперации.Следующий() Тогда
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, ВыборкаНазначенныеОперации);
	КонецЕсли;
...
Процедура ЗаполнитьСлужебныеРеквизиты()
	...
	Если Объект.ДинамическаяСтруктура Тогда
		ПланироватьНеРанееДата = Объект.ПланироватьНеРанее;
	Иначе
		НадписьПланироватьНеРанее = ФорматированнаяСтрокаПланироватьНеРанее(
			Объект.ПланироватьНеРанее,
			ФорматДатыГрафика);
...
Функция ФорматированнаяСтрокаНадписьИзделие()
	...
	Если ЗначениеЗаполнено(ДанныеПартииПроизводства.ОсновноеИзделиеНоменклатура) Тогда
		...
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(ОсновноеИзделиеНаименование,,,,"#ОткрытьИзделие"));
...
Процедура ПриИзмененииНоменклатуры(СтрокаТабличнойЧасти, ИмяТЧ)
	...
	Если ИмяТЧ = "ОбеспечениеМатериаламиИРаботами"
		И ОтражениеВыполненного Тогда
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ВариантОбеспечения",
			ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.Отгрузить"));
		ДополнительныеПараметры.Вставить("СкладТолькоСобственнаяЦеховаяКладовая", Истина);
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ДобавитьВСтруктуруДействияПриИзмененииНоменклатуры(ИмяТЧ, Форма, СтруктураДействий, ОбработкаПодбора = Ложь)
	...
	Если НЕ СтруктураДействий.Свойство("ПроверитьСериюРассчитатьСтатус")
		...
		ПараметрыПроверкиСерий = Новый Структура("ПараметрыУказанияСерий", Форма.ПараметрыУказанияСерий[ИмяТЧ]);
		СтруктураДействий.Вставить("ПроверитьСериюРассчитатьСтатус", ПараметрыПроверкиСерий);
	...
	Если ИмяТЧ = "ОбеспечениеМатериаламиИРаботами" Тогда
		...
		Если ОбработкаПодбора Тогда
			ДополнительныеПараметры.Вставить("ЗаполнитьОбособленно", Ложь);
			ДополнительныеПараметры.Вставить("ЗаполнятьТолькоПустые", Истина);
		КонецЕсли;
...
Процедура ЗаполнитьСтруктурыДействийДляОбработкиШтрихкодов(ИмяТЧ, Форма, СтруктураДействийСДобавленнымиСтроками, СтруктураДействийСИзмененнымиСтроками)
	...
	ДобавитьВСтруктуруДействияПриИзмененииНоменклатуры(ИмяТЧ, Форма, СтруктураДействийСДобавленнымиСтроками);
...
&НаКлиенте
Процедура ВидыРабочихЦентровПересчитатьЗагрузку(ТекущиеДанные)
	
	Если Не ТекущиеДанные.ПараллельнаяЗагрузка Тогда
		Возврат;
	КонецЕсли;
	
	Если КоличествоНормативРасчетаВидовРЦ <> 0 Тогда
		ТекущиеДанные.Загрузка = КоличествоПлан / КоличествоНормативРасчетаВидовРЦ * ТекущиеДанные.ЗагрузкаНорматив;
	Иначе
		ТекущиеДанные.Загрузка = 0;
	КонецЕсли;
...
Процедура ВидыРабочихЦентровПересчитатьВремяРаботы(ТекущиеДанные)
	...
	Если ТекущиеДанные.ПараллельнаяЗагрузка Тогда
		
		КоличествоТактовЗагрузки = 1;
		Если ТекущиеДанные.МаксимальнаяЗагрузка <> 0 Тогда
			КоличествоТактовЗагрузки = Цел(ТекущиеДанные.Загрузка / ТекущиеДанные.МаксимальнаяЗагрузка)
				+ ?(ТекущиеДанные.Загрузка % ТекущиеДанные.МаксимальнаяЗагрузка > 0, 1, 0);
		ИначеЕсли КоличествоНормативРасчетаВидовРЦ <> 0 Тогда
			КоличествоТактовЗагрузки = Цел(КоличествоПлан / КоличествоНормативРасчетаВидовРЦ)
				+ ?(КоличествоПлан % КоличествоНормативРасчетаВидовРЦ > 0, 1, 0);
		КонецЕсли;
		
		ТекущиеДанные.ВремяРаботы = ТекущиеДанные.ВремяРаботыНорматив * КоличествоТактовЗагрузки;
		
	ИначеЕсли КоличествоНормативРасчетаВидовРЦ <> 0 Тогда
		
		Если ОдновременноПроизводимоеКоличество > 1 Тогда
			КоличествоТактов = Цел(КоличествоПлан / КоличествоНормативРасчетаВидовРЦ)
				+ ?(КоличествоПлан % КоличествоНормативРасчетаВидовРЦ > 0, 1, 0);
			
			ТекущиеДанные.ВремяРаботы = ТекущиеДанные.ВремяРаботыНорматив * КоличествоТактов;
		Иначе
			ТекущиеДанные.ВремяРаботы = КоличествоПлан / КоличествоНормативРасчетаВидовРЦ * ТекущиеДанные.ВремяРаботыНорматив;
		КонецЕсли;
...
Процедура ПриИзмененииКоличестваВШапке(ИзмененныеРеквизиты)
	...
	Если КоличествоФакт >= КоличествоПлан И КоличествоОтменено = 0 Тогда
	...
	ИначеЕсли ЗавершеноВыполнениеЭтапа И КоличествоПлан > КоличествоФакт + КоличествоОтменено Тогда
