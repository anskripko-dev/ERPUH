...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ДанныеЗаполнения) Тогда
		...
		Иначе
			...
			ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("СправочникСсылка.ПретензииУХ") Тогда 
				
				ДанныеПретензии = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения, "Организация, Контрагент"); 
				Организация = ДанныеПретензии.Организация;  
				...
				Запрос.Текст = 
					...
					|ГДЕ
					|	АккредитованыеПоставщикиСрезПоследних.АнкетаПоставщика.Контрагент = &Контрагент
					|	И АккредитованыеПоставщикиСрезПоследних.Контрагент = &Контрагент";
				
				Запрос.УстановитьПараметр("Контрагент", 	ДанныеПретензии.Контрагент);
				Запрос.УстановитьПараметр("Организация", 	ДанныеПретензии.Организация);
				
				РезультатЗапроса = Запрос.Выполнить();
				ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
				
				Если ВыборкаДетальныеЗаписи.Следующий() Тогда
					АнкетаПоставщика = ВыборкаДетальныеЗаписи.АнкетаПоставщика;
				КонецЕсли;   
				
				НоваяПретензия = Претензии.Добавить();  
				НоваяПретензия.Претензия = ДанныеЗаполнения;
