...
Функция СвойстваСтатьиОтражения(Статья) Экспорт
	...
	ИначеЕсли ТипЗнч(Статья) = Тип("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов") Тогда
		//++ НЕ УТКА
		Результат.ДоступнаНастройкаСчетовУчета = НЕ
			НастройкаСчетовУчетаСлужебныйВызовСервера.СчетаУчетаУказываютсяВНастройкахПроводок(Статья);
		//-- НЕ УТКА
		Возврат Результат;
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Обновление документов ""Отражение зарплаты в финансовом учете (2.5)"":';
								|en = 'Update the ""Post payroll expenses to accounting (2.5)"" documents:'"));
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Не удалось обработать документы ""Отражение зарплаты в финансовом учете (2.5)"" по обработчику:';
								|en = 'Cannot process the ""Post payroll expenses to accounting (2.5)"" documents by the handler:'"));
