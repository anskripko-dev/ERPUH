...
Процедура ПродлениеОсновногоОтпускаЗавершение(Результат, КоличествоДнейПродления) Экспорт
	...
	Иначе
		...
		"НадписьПродлитьОтпуск",
		"Заголовок",
		СтрШаблон(НСтр("ru = 'Перенести дату окончания основного отпуска на %1 дн...';
						|en = 'Перенести дату окончания основного отпуска на %1 дн...'"), КоличествоДнейПродления));
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, 
			...
			СтрШаблон(НСтр("ru = 'Отменить перенос даты окончания дополнительного отпуска на %1 дн...';
							|en = 'Отменить перенос даты окончания дополнительного отпуска на %1 дн...'"), 
...
Процедура ЗаполнитьОтработанныеДни()
	...
	Если ОтработанныеДниВсего < КоличествоОтработанныхДней Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'У сотрудника недостаточно отработанных дней для отгула';
													|en = 'The employee does not have enough days worked for day off'"), , "РасходДнейОтгула", "Объект");
	КонецЕсли;
