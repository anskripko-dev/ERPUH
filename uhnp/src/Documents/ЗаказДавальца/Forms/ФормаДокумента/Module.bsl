...
Процедура ХарактеристикаПриИзменении(Элемент)
	...
	ПарамУслуги = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияСодержанияУслугиВСтрокеТЧ(Объект, Истина);
...
Процедура ПродукцияОтменитьВыделенныеСтроки(Команда)
	...
	Если ПродукцияОтменитьВыделенныеСтрокиНаСервере(Элементы.Продукция.ВыделенныеСтроки) Тогда
...
Процедура ПриИзмененииСтавкиНДССервер(КэшированныеЗначения)
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Объект.Продукция,
		СтруктураДействий, КэшированныеЗначенияПакетнаяОбработка);
...
Процедура ОбновитьСостояниеСервер()
	...
	Документы.ЗаказДавальца.РассчитатьСостояние(Объект.Ссылка, Объект.Договор, ЭтотОбъект);
...
Процедура ПолучитьСтрокиИзБуфераОбмена(Отгрузка = Ложь)
	...
	ОбрабатываемаяТабличнаяЧасть = ?(Отгрузка, Объект.Продукция, Объект.Материалы);
	...
	Если Отгрузка Тогда
		Для Каждого ТекущаяСтрока Из ОбработанныеСтроки Цикл
			МассивНовыхСтрок.Добавить(ТекущаяСтрока);
			МассивДанныхПоНоменклатуре.Добавить(Документы.ЗаказДавальца.ДанныеПоНоменклатуре(Объект, ТекущаяСтрока));
		КонецЦикла;
	КонецЕсли;
...
&НаСервере
Функция ПродукцияОтменитьВыделенныеСтрокиНаСервере(Знач ВыделенныеСтроки)
	...
	Для Каждого ТекущийИдентификатор Из ВыделенныеСтроки Цикл
		...
		СтрокаТаблицы = Объект.Продукция.НайтиПоИдентификатору(ТекущийИдентификатор);
