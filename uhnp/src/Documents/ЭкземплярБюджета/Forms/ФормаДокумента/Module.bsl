	...
	Если Параметры.Свойство("Основание") И ТипЗнч(Параметры.Основание) = Тип("ДокументСсылка.ЭкземплярБюджета") Тогда
		...
		Если СтатусОснования = Перечисления.СтатусыПланов.ВПодготовке
			...
			ВызватьИсключение НСтр(
				...
				|en = 'You cannot enter a budget adjustment with the ""Under development"" or ""Canceled"" status'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ДействиеПриИзменении(Элемент)
	...
	Если Объект.Действие = ПредопределенноеЗначение(
		...
		ТекстВопроса = НСтр("ru = 'Документ будет заполнен актуальными данными бюджета. Продолжить?';
							|en = 'The document will be filled with relevant budget data. Continue?'");
...
Процедура КорректировкиПоСтатьямБюджетовПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Не ЗначениеЗаполнено(Объект.ВидБюджета) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не указан вид бюджета.';
														|en = 'Plan profile is not specified.'"), , "ВидБюджета", "Объект", Отказ);
	КонецЕсли;
...
Процедура УстановитьДоступностьЭлементовПоБюджетнойЗадачеСервер()
	...
	Если Не ДоступностьЭлементовПоБюджетнойЗадаче = Неопределено И ДоступностьЭлементовПоБюджетнойЗадаче.ЕстьАктивнаяЗадача Тогда
		...
		КонецЕсли;
		Если Не Элементы.Подразделение.ТолькоПросмотр И НЕ ДоступностьЭлементовПоБюджетнойЗадаче.ДоступностьПодразделение Тогда
