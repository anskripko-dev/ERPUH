...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДанныеРедактируемойСтроки = Новый Структура;
	ДанныеРедактируемойСтроки.Вставить("ЭлементСтруктурыЗадолженности");	
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если Результат.Свойство("ОбновитьГрафикРасчетов") Тогда 
		ДанныеРедактируемойСтроки.Вставить("ИдентификаторДобавленнойСтроки", Результат.ИдентификаторДобавленнойСтроки);
		ПриОкончанииРедактированияДетализацииГрафика()
	КонецЕсли;
...
Процедура ВидСоглашенияПриИзменении(Элемент)
	...
	ЭтоРамочныйДоговор = Объект.ВидСоглашения = ПредопределенноеЗначение("Перечисление.ВидыСоглашений.РамочныйДоговор");
...
Процедура ДатаОтсчетаПроцентныхПериодовПриИзменении(Элемент)
	...
	Если НЕ Объект.ДатаОтсчетаПроцентныхПериодов
				= ПредопределенноеЗначение("Перечисление.ВидыДатОтсчетаПериодичности.ДатаПервогоПогашения") Тогда
...
&НаКлиенте
Процедура ГраницаПериодаПоследнийДеньМесяцаПриИзменении(Элемент)
...
Процедура УправлениеИзменениямиСтавкиПриИзменении(Элемент)  
	...
	Если Объект.РучноеУправлениеИзменениямиСтавки Тогда
...
Процедура ЭтоКредитнаяЛинияЧислоПриИзменении(Элемент)
	...
	ОбновитьСводнуюИнформациюПоДоговору();
...
&НаКлиенте
Процедура ДатаОкончанияДоступностиПриИзменении(Элемент)
	ОбновитьСводнуюИнформациюПоДоговору();
КонецПроцедуры
...
Процедура ВидДоговораУХПриИзменении(Элемент)	
	ФинансовыеИнструментыФормыКлиентСервер.УправлениеФормой_СчетаУчета(ЭтаФорма, Истина, Ложь);
	ДоговорыКонтрагентовФормыУХКлиентСервер.УправлениеФормойВерсияСоглашения(ЭтотОбъект);
...
Процедура ДатаНачалаДействияПриИзменении(Элемент)    
	ПриИзмененииПериодаДействияИнструмента();
	РаботаСДоговорамиКонтрагентовУХКлиентСервер.ЗаполнитьДатуОтсчетаПроверкиКовенантов(Объект, Объект.ДатаНачалаДействия);  
	Если ЗначениеЗаполнено(Объект.БазовыйДоговор) Тогда
		ОбновитьОграничения();	
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриОкончанииРедактированияДетализацииГрафика()
	...
	Если ДанныеРедактируемойСтроки.Свойство("ИдентификаторДобавленнойСтроки") Тогда
		СтрокаРедактирования = Объект.ГрафикРасчетов.НайтиПоИдентификатору(ДанныеРедактируемойСтроки.ИдентификаторДобавленнойСтроки);
		ДанныеРедактируемойСтроки.Вставить("ЭлементСтруктурыЗадолженности", СтрокаРедактирования.ЭлементСтруктурыЗадолженности); 
		ДанныеРедактируемойСтроки.Удалить("ИдентификаторДобавленнойСтроки");
	Иначе
		СтрокаРедактирования = Элементы.ГрафикРасчетовЛинейный.ТекущиеДанные;
	КонецЕсли;
	
	Если РедактированиеОсновногоДолга() тогда
		ОбработатьИзменениеСекцииНаСервере(Объект.ГрафикРасчетов.Индекс(СтрокаРедактирования), "ОсновнойДолг");
	Иначе
		ОбработатьИзменениеСекцииНаСервере(Объект.ГрафикРасчетов.Индекс(СтрокаРедактирования), "Проценты");
	КонецЕсли;
...
Процедура Подключаемый_ГрафикРасчетовЛинейныйДатаПриИзменении(Элемент)	
	...
	ДанныеРедактируемойСтроки.Вставить("ЭлементСтруктурыЗадолженности", ТекущиеДанные.ЭлементСтруктурыЗадолженности); 
...
&НаСервере
Процедура РассчитатьСуммыПроцентовНаСервере(ИмяСекции = Неопределено,
											ПересчетОтТекущейДаты = Ложь,
											ЗагружатьФакт = Ложь,
											ВыполнитьПересчетГрафика = Истина)

	СтруктураДействий = Новый Структура;
										
	Если ВыполнитьПересчетГрафика Тогда										
		...
		Если ПересчетОтТекущейДаты Тогда
			ДопПараметры = Новый Структура("ДатаНачалаПересчета", НачалоДня(ТекущаяДатаСеанса()));
		КонецЕсли;

		СтруктураДействий.Вставить("Пересчитать", Новый Структура("СекцииГрафика,ДопПараметры", Секции, ДопПараметры));
		
		Если Не ИмяСекции = Неопределено Тогда
			Если Объект.АвтоматическиеВычисления Тогда
		...
		Если ЗагружатьФакт Тогда
			СтруктураДействий.Вставить("ЗагружатьФакт");
		КонецЕсли;
		
		Отказ = Ложь;
		
		ФинансовыеИнструментыУХ.ПересчетГрафика(Объект,
												0,
												ОписаниеГрафика,
												ОперацииГрафика,
												СтруктураДействий,
...
Процедура ЗаполнитьВыборкуПогашение()
	...
	Если Не Объект.ВидСоглашения = Перечисления.ВидыСоглашений.РамочныйДоговор Тогда
		Если Не РаботаСДоговорамиКонтрагентовУХКлиентСервер.ПроверитьЗаполнениеУсловийФинансовогоИнструмента(ЭтотОбъект)
			И Не РаботаСДоговорамиКонтрагентовУХКлиентСервер.ДоступенГрафикРасчетовВерсияСоглашенияКредит(Объект.ВидДоговораУХ) Тогда
			Возврат;
		КонецЕсли;
...
Процедура ИнициализироватьИнструмент()
	...
	ОбновитьСводнуюИнформациюПоДоговору();
	Элементы.ГруппаИтогоПоДоговору.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Всего по договору (на %1)'"), Формат(ТекущаяДатаСеанса(),"ДЛФ=D"));
...
Процедура УстановитьУсловноеОформление()
	...
	ФинансовыеИнструментыФормы.УстановитьУсловноеОформлениеФормыГрафика(УсловноеОформление, ОписаниеГрафика, ОперацииГрафика);	
КонецПроцедуры
...
Процедура ОбновитьВстречныеДанные()
	...
	ВстречныйДоговор = РегистрыСведений.СвязанныеДоговоры.ПолучитьВстречныйДоговор(Объект.ДоговорКонтрагента);
...
Процедура УстановитьТекстЛимит(ТекстЛимит, Объект, КэшируемыеЗначения)
	...
	Если ОстатокЛимитаПослеПривлечения >=0 Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Доступно %1 %2 (%3)'"), ОстатокЛимитаПослеПривлечения, Объект.ВалютаВзаиморасчетов, ТекстВидаЛимита);
		ТекстЛимит = Новый ФорматированнаяСтрока(Текст,,WebЦвета.СинийСоСтальнымОттенком);
	Иначе
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Лимит превышен на %1 %2 (%3)'"), -ОстатокЛимитаПослеПривлечения, Объект.ВалютаВзаиморасчетов, ТекстВидаЛимита);
...
Процедура КонтрагентПриИзмененииНаСервере()
	...
	ЗаполнитьБанковскийСчетКонтрагентаПоУмолчанию(Объект.Контрагент, ОплатаВВалюте, Объект.БанковскийСчетКонтрагента, Объект.Организация);
...
&НаСервере
Процедура ОбновитьСводнуюИнформациюПоДоговору()

	КэшируемыеЗначения.Вставить("СводнаяИнформацияПоДоговору", Документы.ВерсияСоглашенияКредит.СводнаяИнформацияПоДоговору(Объект.ДоговорКонтрагента,
																Объект.Сумма,
																Объект.ВозобновляемыйЛимит,
																,
																Объект.ДатаОкончанияДоступности,
																Объект.ЭтоКредитнаяЛиния));
...
Функция РедактированиеОсновногоДолга()
	...
	Если ЗначениеЗаполнено(ДанныеРедактируемойСтроки.ЭлементСтруктурыЗадолженности) Тогда  
		ЭлементСтруктурыЗадолженности = ДанныеРедактируемойСтроки.ЭлементСтруктурыЗадолженности; 
		ДанныеРедактируемойСтроки.ЭлементСтруктурыЗадолженности = Неопределено;
	Иначе
		ТекущиеДанные = Элементы.ГрафикРасчетовЛинейный.ТекущиеДанные;   
	...
	Возврат ЭлементСтруктурыЗадолженности = ПредопределенноеЗначение("Перечисление.ЭлементыСтруктурыЗадолженности.ОсновнойДолг")
	      	ИЛИ ЭлементСтруктурыЗадолженности = ПредопределенноеЗначение("Перечисление.ЭлементыСтруктурыЗадолженности.ОсновнойДолгКонтрагента");
КонецФункции
...
Процедура ВидДоговораУХПриИзмененииНаСервере()
	...
	ОписаниеГрафика = Документы.ВерсияСоглашенияКредит.ОписаниеГрафика(Объект.ВидДоговораУХ); 
...
&НаСервере
Процедура ОбновитьТаблицуГрафикРасчетов()
	ДоговорыКонтрагентовФормыУХ.ОбновитьТаблицуГрафикРасчетов(ЭтотОбъект);
КонецПроцедуры
...
Процедура ПеренестиФактНаСервере(ДатаПереноса)
	...
	// Пересчет основного долга и процентов
	СтруктураДействий = Новый Структура;
	...
	ДопПараметры = Новый Структура("ДатаНачалаПересчета", ДатаПереноса + 1);
	СтруктураДействий.Вставить("Пересчитать", Новый Структура("СекцииГрафика, ДопПараметры", СекцииПроценты, ДопПараметры));
	
	ДоговорыКонтрагентовФормыУХ.АктуализироватьГрафикРасчетовНаФорме(ЭтотОбъект, ДатаПереноса, СтруктураДействий);
...
Процедура ЗаполнитьПараметрыОперацийПоУмолчанию()	
	ДоговорыКонтрагентовФормыУХ.ЗаполнитьПараметрыОперацийГрафикаПоУмолчанию(ЭтотОбъект);
КонецПроцедуры
