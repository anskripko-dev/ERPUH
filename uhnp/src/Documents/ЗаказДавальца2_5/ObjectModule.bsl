...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ЗаполненныеРеквизиты = ЗаполнениеОбъектовПоСтатистике.ЗаполнитьРеквизитыОбъекта(ЭтотОбъект, ДанныеЗаполнения);
	Если ЗаполненныеРеквизиты.Свойство("Организация")
		Или ЗаполненныеРеквизиты.Свойство("Склад") Тогда
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	Если Не КэшированныеЗначения.ОбработанныеСтроки.Количество() = 0 Тогда
		...
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ТаблицаПродукция, СтруктураДействий, Неопределено);
...
Процедура ЗаполнитьДокументНаОснованииЗаказПереработчику(ДанныеЗаполнения)
	...
	ПараметрыВыбораСпецификаций = УправлениеДаннымиОбИзделиях.ПараметрыВыбораСпецификаций(ЭтотОбъект, Документы.ЗаказДавальца2_5);

	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ПересчитатьКоличествоУпаковок", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.СтруктураПересчетаКоличестваУпаковок());
	СтруктураДействий.Вставить("ПересчитатьЦенуПоСумме",        "Количество");
	СтрокиДляЗаполненияСпецификации = Новый Массив;
	...
	Для Каждого ТекущиеДанные Из Продукция Цикл
		...
		Если Не ЗначениеЗаполнено(ТекущиеДанные.Спецификация) Тогда
			...
			ДанныеПоНоменклатуре.Добавить(Документы.ЗаказДавальца2_5.ДанныеПоНоменклатуре(ЭтотОбъект, ТекущиеДанные));
			СтрокиДляЗаполненияСпецификации.Добавить(ТекущиеДанные);
	...
	УправлениеДаннымиОбИзделиях.ЗаполнитьСпецификациюВСтроках(
		СтрокиДляЗаполненияСпецификации, ДанныеПоНоменклатуре, ПараметрыВыбораСпецификаций,, Истина);
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Материалы, СтруктураДействий, КэшированныеЗначения);
