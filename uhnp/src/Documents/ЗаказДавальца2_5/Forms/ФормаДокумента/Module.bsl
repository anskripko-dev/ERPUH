...
Процедура МатериалыНоменклатураПартнераПриИзменении(Элемент)
	...
	СтруктураДействий.Вставить(
		...
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияЦеныЗакупкиВСтрокеТЧ(
...
Процедура ПриИзмененииЦенаВключаетНДССервер(КэшированныеЗначения)
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Объект.Продукция, СтруктураДействий, КэшированныеЗначения);
...
Процедура ПриИзмененииСтавкиНДССервер(КэшированныеЗначения)
	...
	Если КэшированныеЗначения = Неопределено Тогда
		КэшированныеЗначенияПакетнаяОбработка =
			ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
	Иначе
		КэшированныеЗначенияПакетнаяОбработка = ОбщегоНазначения.СкопироватьРекурсивно(КэшированныеЗначения);
	КонецЕсли;
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(
		Объект.Продукция, СтруктураДействий, КэшированныеЗначенияПакетнаяОбработка);
...
Процедура МатериалыУстановитьДействиеВСтрокахНаСервере(Действие, Знач ИдентификаторыСтрок, КэшированныеЗначения)
	...
	Для каждого ИдентификаторСтроки Из ИдентификаторыСтрок Цикл
		...
		ТекущиеДанные = Объект.Материалы.НайтиПоИдентификатору(ИдентификаторСтроки);
		
		// фильтры
		
		ТекущиеДанные.Действие = Действие;
		МатериалыПриИзмененииДействияВСтроке(ЭтотОбъект, ТекущиеДанные);
	...
	СтруктураДействий.Вставить(
		...
		Новый Структура("Склад, ПараметрыУказанияСерий, ТаблицаТоваров",
			Объект.Склад, ПараметрыУказанияСерий.Материалы, ОбрабатываемыеСтроки));
	
	Если Действие = ПредопределенноеЗначение("Перечисление.ВариантыДействийПоПерерабатываемымМатериалам.ПринятьНаХранение") Тогда
		
		СтруктураДействий.Вставить(
			"ЗаполнитьЦенуЗакупки",
			ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияЦеныЗакупкиВСтрокеТЧ(Объект, "ВидЦеныМатериалы"));
		СтруктураДействий.Вставить("ПересчитатьСумму");
		
	КонецЕсли;
	
	ДобавитьВСтруктуруДействияПриИзмененииДействияВСтрокеМатериалов(ЭтотОбъект, Неопределено, СтруктураДействий);
...
Процедура МатериалыЗаполнитьПоСпецификациямНаСервере()
	...
	СтруктураДействий.Вставить(
		"ПроверитьСериюРассчитатьСтатус",
		Новый Структура(
				"Склад, ПараметрыУказанияСерий, ТаблицаТоваров",
				Объект.Склад, ПараметрыУказанияСерий.Материалы, ОбрабатываемыеСтроки));
...
&НаКлиентеНаСервереБезКонтекста
Процедура ДобавитьВСтруктуруДействиеЗаполнитьСтавкуНДС(Объект, СтруктураДействий)
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение, ИмяТЧ)
	...
	Если ИмяТЧ = "Продукция" Тогда
		...
		ТребуетсяЗаполнитьВариантОбеспечения =
			ТаблицаТоваров.НайтиСтроки(Новый Структура("ВариантОбеспечения", 
				ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.ПустаяСсылка"))).Количество();
		
		Если ТребуетсяЗаполнитьВариантОбеспечения Тогда
		...
		Если ИмяТЧ = "Продукция" Тогда
			...
			ДанныеОбИзделиях.Добавить(
				УправлениеДаннымиОбИзделияхКлиентСервер.СобратьДанныеОбИзделииДляВыбораСпецификации(
					Объект, ТекущаяСтрока, ПараметрыВыбораСпецификаций, ЭтотОбъект));
...
Функция ПараметрыЗаполненияСкладаВСтроке(ИмяТЧ)
	...
	Если ИмяТЧ = "Продукция" Тогда
		...
		Возврат ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияСкладаВСтрокеТЧ(Объект, СкладГруппа);
		
	ИначеЕсли ИмяТЧ = "Материалы" Тогда
		
		Возврат ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияСкладаВСтрокеТЧ(
...
Процедура ОбновитьСостояниеКлиент(ПроверитьЗаданияКОтражениюСостоянияЗаказов = Ложь, СостояниеЗаказа = Неопределено)
	...
	Если Состояние = ПредопределенноеЗначение("Перечисление.СостоянияЗаказовКлиентов.ГотовКОбеспечению") Тогда
		Состояние = НСтр("ru = 'Готов к производству';
						|en = 'Ready for production'");
	КонецЕсли;
...
Процедура ЗаполнитьСтавкуНДСНаСервере()
	...
	СтруктураДействий = Новый Структура;
	ДобавитьВСтруктуруДействиеЗаполнитьСтавкуНДС(Объект, СтруктураДействий);
...
Процедура ЗаполнитьДатуОтгрузкиЗавершение(ВыбраннаяДата, ДополнительныеПараметры) Экспорт
	...
	Если ВыбраннаяДата <> Неопределено И ЗначениеЗаполнено(ВыбраннаяДата) Тогда
		...
		ЗаполнитьДатуОтгрузкиЗавершениеНаСервере(ВыбраннаяДата, ДополнительныеПараметры);
		ПродажиКлиент.ОповеститьОбОкончанииЗаполненияДатОтгрузки(ВыбраннаяДата, (ДополнительныеПараметры.МассивВыделенныхСтрок <> Неопределено));
...
&НаСервере
Процедура ЗаполнитьДатуОтгрузкиЗавершениеНаСервере(ВыбраннаяДата, ДополнительныеПараметры)
	
	МассивВыделенныхСтрок = ДополнительныеПараметры.МассивВыделенныхСтрок;
	
	ЗаполнитьДатыОтгрузки(ЭтотОбъект, ВыбраннаяДата, МассивВыделенныхСтрок);
...
Процедура ПолучитьСтрокиИзБуфераОбмена(ИмяТЧ)
	...
	Если ИмяТЧ = "Продукция" Тогда
		...
		ТаблицаПродукция.Колонки.Добавить("СтавкаНДС", Новый ОписаниеТипов("СправочникСсылка.СтавкиНДС"));
	...
	Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		Если ИмяТЧ = "Продукция" Тогда
			...
			ТекущаяСтрока = ТаблицаПродукция.Добавить();
			ТекущаяСтрока.СтавкаНДС = Объект.СтавкаНДС;
			ТабличнаяЧасть = ТаблицаПродукция;
	...
	Если ИмяТЧ = "Продукция" Тогда
		Для Каждого ОбрабатываемаяСтрока Из ОбрабатываемыеСтроки Цикл
			...
			ЗаполнитьЗначенияСвойств(ТекущаяСтрока, ОбрабатываемаяСтрока);
		КонецЦикла;
		Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
...
Процедура ОтменитьСвязанныеМатериалыСервер(КлючиСвязиМатериалов)
	...
	ОбрабатываемыеСтроки = Новый Массив;
	ОбрабатываемыеРазбиваемыеСтроки = Новый Массив;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Количество = Выборка.КоличествоОтмены Тогда
			...
			ОбрабатываемыеСтроки.Добавить(СтрокаТЧМатериалы);
		...
		Иначе
			...
			ОбрабатываемыеРазбиваемыеСтроки.Добавить(СтрокаТЧМатериалы);
			...
			ОбрабатываемыеНовыеСтроки.Добавить(СтрокаОтмены);
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		ОбрабатываемыеРазбиваемыеСтроки,
		СтруктураДействий,
		Объект.Материалы);
...
Процедура ОбновитьПредставлениеНазначения(ШаблонНазначенияМатериалы = Неопределено)
	...
	ТипОбъектаНазначения = Справочники.Назначения.ОпределитьТипОбъектаНазначения(ШаблонНазначенияМатериалы);
	ПредставлениеНазначения = Справочники.Назначения.ПредставлениеНазначенияДляЗаписиВИнформационнуюБазу(
								ШаблонНазначенияМатериалы, РеквизитыДокумента, ТипОбъектаНазначения);
