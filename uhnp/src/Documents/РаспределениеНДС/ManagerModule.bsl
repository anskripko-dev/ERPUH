...
Функция ДанныеДляРаспределения(Реквизиты, ДопПараметры)
	...
	|	
	|ГДЕ
	|	НЕ ИСТИНА В (ВЫБРАТЬ ПЕРВЫЕ 1 ИСТИНА ИЗ ВтВнеоборотныеАктивы КАК Т ГДЕ Т.Объект = ТаблицаОстатков.АналитикаРасходов)
	...
	|ГДЕ
	|	НЕ ИСТИНА В (ВЫБРАТЬ ПЕРВЫЕ 1 ИСТИНА ИЗ ВтВнеоборотныеАктивы КАК Т ГДЕ Т.Объект = ТаблицаОстатков.АналитикаРасходов)
	|	И ТаблицаОстатков.НалогообложениеНДСПартии <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортСырьевыхТоваровУслуг)
...
Функция ТаблицаДокументыПоОС(ПартииНДСКРаспределению, Реквизиты)
	...
	Для каждого Строка Из ПартииНДСКРаспределению Цикл
		...
		НоваяСтрока.ТипСсылки				= Реквизиты.ИдентификаторМетаданных;
		НоваяСтрока.Организация				= Реквизиты.Организация;
		НоваяСтрока.Подразделение			= Строка.Подразделение;
