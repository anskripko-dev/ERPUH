...
Функция ТекстЗапросаИзмененияПериода(НомераТаблиц)
	...
	ТекстЗапроса.Добавить(МСФОВстраиваниеУХВызовСервера.ТекстЗапроса_втДКЗ(НомераТаблиц));
	ТекстЗапроса.Добавить(МСФОВстраиваниеУХВызовСервера.ТекстЗапроса_втФИ(НомераТаблиц));
	...
	ТекстЗапроса.Добавить(МСФОВстраиваниеУХВызовСервера.ТекстЗапроса_АналитикаУчетаФИ(НомераТаблиц));
...
Процедура ЗаполнитьВНА(ТабВНАФИ, Объект)
	...
	КэшируемыеЗначения = Новый Структура("ШаблонТрансляции, ПланСчетовНСБУ",
		ПараметрыЗаполнения.ШаблонТрансляции,
		ПараметрыЗаполнения.ШаблонТрансляции.ПланСчетовИсточник);
		
	КонтекстФормы = Новый Структура("Элементы, Объект, КэшируемыеЗначения", 
						Новый Массив, Объект, КэшируемыеЗначения);
