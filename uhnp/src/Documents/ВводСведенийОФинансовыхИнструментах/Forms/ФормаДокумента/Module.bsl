...
Процедура РедактироватьГрафикФИ(Команда)
	...
	ПараметрыФормы = Новый Структура(
		"ДанныеФИ, Документ, ФИ, ВидОперации",
		ДанныеФИ, ЭтаФорма.Объект.Ссылка, ТекущийФИ, ЭтаФорма.Объект.ВидОперации);
		
	ОткрытьФорму("Обработка.ГрафикФИМСФО.Форма", ПараметрыФормы, ЭтаФорма, ЭтаФорма);
...
Процедура ПодготовитьФормуНаСервере(Форма)
	...
	Если Не УчетМСФОПовтИспУХ.ЕстьРСБУ() Тогда
		Форма.Элементы.ФормаЗаполнитьДокумент.Видимость = Ложь;
	КонецЕсли;
	...
	МСФОУХ.ДобавитьУсловноеОформлениеТолькоПросмотр(Форма, ПоляОформления, ПутьВидФИ, Истина);
