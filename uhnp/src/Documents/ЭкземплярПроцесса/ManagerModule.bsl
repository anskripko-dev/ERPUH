...
// этапу.
Процедура ЗаписатьВРегистрДанныеПоВозвратуОбъекта(ЭкземплярПроцессаВход, ТекстСообщенияВход = "", ПользовательВход = Неопределено, ЭтапПроцессаВход = Неопределено, ДопПараметры = Неопределено) Экспорт
	
	ТекущийПользователь = ОбщегоНазначенияУХ.ПолучитьЗначениеПеременной("глТекущийПользователь");
	ТаблицаДопСогласующих = МодульСогласованияДокументовУХ.ПолучитьТаблицуДопСогласующихПоЭтапу(ЭкземплярПроцессаВход, ЭтапПроцессаВход);
	
	Если ТаблицаДопСогласующих.Найти(ТекущийПользователь, "Согласующий") <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	ЗавершатьЭтапы = Истина;
	Если ДопПараметры <> Неопределено Тогда
		ДопПараметры.Свойство("ЗавершатьЭтапы", ЗавершатьЭтапы);
		ЗавершатьЭтапы = ?(ЗавершатьЭтапы = Неопределено, Истина, ЗавершатьЭтапы);
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗавершатьЭтапы ИЛИ Строка.СостояниеЭтапа = Перечисления.СостоянияЭтаповУниверсальныхПроцессов.ВОбработке Тогда
