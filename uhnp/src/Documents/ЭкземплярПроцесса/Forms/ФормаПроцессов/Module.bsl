...
Процедура ОбновитьДанныеФормы()
	...
	КонецЕсли;	
	Если ЗначениеЗаполнено(ОтборИнициатор) Тогда
		ОбщегоНазначенияКлиентСерверУХ.УстановитьЭлементОтбора(Список.Отбор, "ИнициаторПроцесса", ОтборИнициатор, ВидСравненияКомпоновкиДанных.Равно,, Истина);	
	Иначе
		// Не устанавливаем отбор по виду объекта согласования.
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		ОтборШаблонПроцесса = Справочники.ШаблоныУниверсальныхПроцессов.ПустаяСсылка();
	КонецЕсли;
	...
	КонецЕсли;	
	// Параметр Инициатор процесса.
	Если ЗначениеЗаполнено(Параметры.ИнициаторПроцесса) Тогда
		ОтборИнициатор = Параметры.ИнициаторПроцесса;
	Иначе
...
Процедура ОткрытьКлючевойОбъект(Команда)
	...
	ОткрытьКлючевойОбъектЭкземпляраПроцесса();
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьКлючевойОбъектЭкземпляраПроцесса()
