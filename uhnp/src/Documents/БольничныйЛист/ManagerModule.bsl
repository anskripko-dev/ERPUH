	...
	Если Реквизиты.ОтказВНазначенииПособия Тогда
		...
		Иначе
			ТекстПричины = НСтр("ru = 'Нет дней оплачиваемых за счет СФР';
								|en = 'Нет дней оплачиваемых за счет СФР'");
		КонецЕсли;
		...
		Если Записывать Тогда
			...
			Исключение
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Обновление информационной базы.Ошибка записи набора регистра ""Периоды ухода за больными родственниками""';
												|en = 'Infobase update.An error occurred when writing the ""Periods of sick relative care"" register set'",
...
Функция ДанныеДляБухучетаЗарплатыПервичныхДокументов(Объект) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСтатьиФинансированияЗарплатаРасширенный")
		И ДоступноИзменениеСпособаРасчетов(Объект) И ЗначениеЗаполнено(Объект.ВидОплатыЗаСчетРаботодателя)
		И ЗначениеЗаполнено(Объект.СпособРасчетовСФизическимиЛицами) Тогда
		
		ОписаниеОбъекта = Новый Структура("
		|Ссылка,БухучетУказываетсяРаспределением,СпособРасчетовСФизическимиЛицами,
		|СтатьяФинансирования,СпособОтраженияЗарплатыВБухучете,СтатьяРасходов,ОтношениеКЕНВД");
		ЗаполнитьЗначенияСвойств(ОписаниеОбъекта, Объект);
		ОписаниеОбъекта.БухучетУказываетсяРаспределением = Ложь;
		...
		Начисления = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объект.ВидОплатыЗаСчетРаботодателя);
		ТаблицаБухучетЗарплаты = ОтражениеЗарплатыВБухучетеРасширенный.ДанныеДляБухучетаЗарплатыПервичныхДокументов(ОписаниеОбъекта, Начисления);
