...
Процедура ЗагрузитьИзФайла(Команда)
	...
	ПараметрыЗагрузки.Диалог.Заголовок   = НСтр("ru = 'Выберите файл ЭЛН, полученный из СФР';
												|en = 'Select ESLR file received from Social Insurance Fund of Russia'");
...
Процедура ПроверитьПересечениеПериодовОтсутствияНаКлиенте(Отказ, ПараметрыЗаписи, ЗакрытьПослеЗаписи)
	...
	ТекстВопроса = ?(ДанныеСотрудника.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияСотрудника.Увольнение"),
		НСтр("ru = 'Сотрудник был уволен %1 ';
			|en = 'Employee was dismissed on %1'"),
		НСтр("ru = 'На период %1 сотруднику уже зарегистрировано отсутствие документом ';
			|en = 'Absence has already been registered for the employee on period %1 by the document'"));
...
Процедура ПроверитьПересечениеПериодовОтсутствияНаКлиентеЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("Простой") Тогда
		ТекстПредупреждения = СтрШаблон(НСтр("ru = 'По сотруднику %1 требуется пересчитать и провести документ простоя ';
											|en = 'Recalculate and post the downtime document for employee %1'"), Объект.Сотрудник);
		...
		Иначе
			...
			Иначе
				ЗаголовокНадписи = НСтр("ru = 'Данный больничный не является последним для ЭЛН %3.
					...
					|Information for Social Insurance Fund of Russia can be changed in <a href=""%1"">%2</a>.'");
			КонецЕсли;
...
Процедура ПослеУспешнойЗагрузкиЭЛН()
	ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		Объект.НомерЛисткаНетрудоспособности);
	ПоказатьОповещениеПользователя(НСтр("ru = 'Данные ЭЛН загружены из СФР';
										|en = 'ESLR data is imported from Social Insurance Fund of Russia'"), , ТекстОповещения, БиблиотекаКартинок.Успешно32);
КонецПроцедуры
	...
	Исключение
		...
		Подробно = СтрШаблон(
			НСтр("ru = 'Ошибка загрузки файла ""%1"":
				...
				|en = 'An error occurred while importing the ""%1"" file:
				|%2
				|
				|The file probably does not match web service response specifications of ESLR Social Insurance Fund of Russia.'"), 
			ОписаниеФайла.Имя,
...
Процедура ОбновитьГруппуПериодыПростоя(Кэш)
	...
	ЗаголовокНадписи = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
		НСтр("ru = ';Простой %1 день;;Простой %1 дня;Простой %1 дней;';
			|en = ';Downtime %1 day;;;;Downtime %1 days'"),
		ДнейПростоя) + ": ";
...
Функция ПредставлениеНеучтенныхПериодовПростоя(Кэш)
	...
	Если ВсегоИзбыточныхДат > 0 И ВсегоИзбыточныхДат <> УчтенныеДаты.Количество() Тогда
		...
		КонецЦикла;
		Представление = Представление
			+ СтрШаблон(НСтр("ru = 'Избыточно добавлены периоды простоя %1.';
							|en = 'Downtime periods %1 are excessively added.'"), СтрСоединить(ПредставленияПериодов, ", "))
			+ " ";
	КонецЕсли;
	Возврат Представление;
КонецФункции
...
Процедура ОбновитьЭлементыПараметровОплаты(Кэш = Неопределено)
	...
	Если Элементы.ДатаНачалаОплатыСФРКартинка.Видимость Тогда
		ТекстЗаголовка = НСтр("ru = 'Дата начала оплаты за счет СФР сдвинута на %1';
								|en = 'Start date of payment out of Social Insurance Fund of Russia funds is shifted to %1'");
	...
	ИначеЕсли Мин(ДатаНачалаОплатыЗаСчетСФР, ДатаНачалаОплатыЗаСчетСФРПоОбщимПравилам) <= Объект.ДатаОкончания Тогда
		ТекстЗаголовка = НСтр("ru = 'Дата начала оплаты за счет СФР %1';
								|en = 'Start date of payment out of Social Insurance Fund of Russia funds %1'");
		Элементы.НадписьДатаНачалаОплатыСФР.Заголовок = СтрШаблон(ТекстЗаголовка, Формат(ДатаНачалаОплатыЗаСчетСФР, "ДФ=dd.MM.yyyy"));
	Иначе
