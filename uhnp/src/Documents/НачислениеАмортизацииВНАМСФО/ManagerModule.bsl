...
Функция ТекстЗапроса_втНачислено(НомераТаблиц)
	...
	|	втСтоимость.ВНА КАК ВНА,
	|	втСтоимость.ГруппаВНА КАК ГруппаВНА,
	|	МАКСИМУМ(втСтоимость.Регистратор ССЫЛКА Документ.ИзменениеПараметровВНАМСФО) КАК ЕстьИзменениеПараметровВНА,
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(втСтоимость.АмортизацияОборот) <> 0 И
	|	МАКСИМУМ(втСтоимость.Регистратор ССЫЛКА Документ.ИзменениеПараметровВНАМСФО) = ИСТИНА
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ВНА,
	|	ГруппаВНА";
