...
Процедура УправлениеЭлементамиФормы()
	...
	УстановитьЗаголовокСчетКассаОрганизации();
	
	ЕстьЗаявка					= ЗначениеЗаполнено(Объект.ЗаявкаНаРасходованиеДенежныхСредств);   
	Элементы.Контрагент.ТолькоПросмотр					 	= ЕстьЗаявка;
	Элементы.Организация.ТолькоПросмотр					 	= ЕстьЗаявка;
	Элементы.Договор.ТолькоПросмотр			 	= ЕстьЗаявка;
	Элементы.ВалютаОплаты.ТолькоПросмотр					= ЕстьЗаявка;
	Элементы.СуммаДокумента.ТолькоПросмотр					= ЕстьЗаявка;
	Элементы.ДатаПлатежаУХ.ТолькоПросмотр						= ЕстьЗаявка;
	//Элементы.ИдентификаторПлатежа.ТолькоПросмотр			= ЕстьЗаявка;
	//Элементы.КрайняяДата.ТолькоПросмотр						= ЕстьЗаявка;
	Элементы.СчетКассаОрганизации.ТолькоПросмотр			= ЕстьЗаявка;
	//Элементы.СчетКонтрагента.ТолькоПросмотр					= ЕстьЗаявка;
	
	Элементы.ЗаявкаНаРасходованиеДенежныхСредств.Видимость = ЕстьЗаявка;
	
	#Область ИсточникЛимитов     
	ГрафикДоговораИспользуется = ЗначениеЗаполнено(Объект.ДоговорКонтрагента)
		И ОбщегоНазначения.ЕстьРеквизитОбъекта("ГрафикДоговора", Объект.ДоговорКонтрагента.Метаданные())
		И (Не Объект.ДоговорКонтрагента.ГрафикДоговора = ПредопределенноеЗначение("Перечисление.ГрафикиДоговора.НеИспользовать"));
...
&НаСервере
Процедура УстановитьЗаголовокСчетКассаОрганизации()
	
	Если Объект.ФормаОплаты = Перечисления.ФормыОплаты.Безналичная Тогда
		Элементы.СчетКассаОрганизации.Заголовок			= НСтр("ru = 'Счет организации'");
	Иначе
		Элементы.СчетКассаОрганизации.Заголовок			= НСтр("ru = 'Касса организации'");			
	КонецЕсли; 
...
&НаКлиенте
Процедура Подключаемый_ЗаявкаНаРасходованиеДенежныхСредствПриИзменении(Элемент)

	Если ЗначениеЗаполнено(Объект.ЗаявкаНаРасходованиеДенежныхСредств) Тогда
	
		ПоказатьЗначение(,Объект.ЗаявкаНаРасходованиеДенежныхСредств);	
