...
Процедура ДобавитьСтатьяЗатрат(Команда)
	...
	КэшируемыеЗначения.Вставить("ТипВНА", Новый (ВстраиваниеУХКлиентСервер.ТипыСтатьиЗатрат()[0]));
	Элементы.ВНА.ДобавитьСтроку();
...
Процедура ПодготовитьФормуНаСервере(ТекущийОбъект = Неопределено)
	...
	ЭтаФорма.КэшируемыеЗначения.Вставить("РежимЗаполнения", Перечисления.РежимЗаполненияВидовУчета.НСБУИМСФО);//Видимость счетов
