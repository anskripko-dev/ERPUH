...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если РешениеПоДокументу = Перечисления.ВидыРешенийПоДокументуАккредитации.ПоложительноеРешение Тогда
		...
		Для Каждого СтрокаНоменклатуры Из ТоварныеКатегории Цикл
			...
			Движение.Номенклатура = СтрокаНоменклатуры.ТоварнаяКатегория;
		КонецЦикла;
...
Процедура ПроверитьПередЗаписью(РежимЗаписи, Ошибки) Экспорт
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		Если ОбщиеТребования.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(
				Ошибки,
				"",
				НСтр("ru='Нет данных для аккредитации. Необходимо заполнить требования к поставщику и список поставляемой товарной категории.'"),
				Неопределено);
		КонецЕсли;
		ДанныеАккредитации =
