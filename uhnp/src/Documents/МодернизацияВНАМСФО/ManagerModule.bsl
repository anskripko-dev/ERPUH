...
Функция ПодготовитьПараметрыПроведения(ДополнительныеСвойства, Отказ) Экспорт
	...
	ТекстЗапроса.Добавить(	ТекстЗапроса_НесколькоМодернизаций(НомераТаблиц));
...
Функция ТекстЗапроса_ТаблицаДокумента(НомераТаблиц)
	...
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаВНА.СчетЗатратСубконто1МСФО) В (&ТипыВНА)
	|			ТОГДА ТаблицаВНА.СчетЗатратСубконто1МСФО
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаВНА.СчетЗатратСубконто2МСФО) В (&ТипыВНА)
	|			ТОГДА ТаблицаВНА.СчетЗатратСубконто2МСФО
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаВНА.СчетЗатратСубконто3МСФО) В (&ТипыВНА)
...
Функция ТекстЗапроса_НесколькоМодернизаций(НомераТаблиц)

	НомераТаблиц.Вставить("втНесколькоМодернизаций", НомераТаблиц.Количество());
	
	Возврат
	"ВЫБРАТЬ
	|	т.ВНА КАК ВНА,
	|	МАКСИМУМ(т.ЛиквидационнаяСтоимостьМСФО) КАК ЛиквидационнаяСтоимостьМСФО,
	|	МАКСИМУМ(т.ОбъемВыработкиМСФО) КАК ОбъемВыработкиМСФО,
	|	МАКСИМУМ(т.ОбъемВыработкиНСБУ) КАК ОбъемВыработкиНСБУ,
	|	МАКСИМУМ(т.СПИМСФО) КАК СПИМСФО,
	|	МАКСИМУМ(т.СПИНСБУ) КАК СПИНСБУ,
	|	СУММА(т.СуммаМодернизацииМСФО) КАК СуммаМодернизацииМСФО,
	|	СУММА(т.СуммаМодернизацииНСБУ) КАК СуммаМодернизацииНСБУ,
	|	МАКСИМУМ(т.СторнироватьДанныеНСБУ) КАК СторнироватьДанныеНСБУ,
	|	КОЛИЧЕСТВО(т.НомерСтроки) КАК КоличествоМодернизаций,
	|	МАКСИМУМ(т.НомерСтроки) КАК НомерСтроки
	|ПОМЕСТИТЬ втНесколькоМодернизаций
	|ИЗ
...
Функция ТекстЗапроса_втТаблицаВНА(НомераТаблиц)
	...
	|	ЕстьNULL(т.НомерСтроки <> втНесколькоМодернизаций.НомерСтроки, ЛОЖЬ) КАК ИсключитьОбъединение,
	|	ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА р.ВидУчета = ЗНАЧЕНИЕ(Перечисление.ВидыУчета.НСБУ)
	|			ТОГДА р.ПервоначальнаяСтоимость + ЕстьNULL(втНесколькоМодернизаций.СуммаМодернизацииНСБУ, т.СуммаМодернизацииНСБУ) - р.Амортизация - р.АмортизацияРасчет
	|		КОГДА &АлгоритмРСБУ
	|			ТОГДА р.ПервоначальнаяСтоимость + р.Переоценка + ЕстьNULL(втНесколькоМодернизаций.СуммаМодернизацииМСФО, т.СуммаМодернизацииМСФО) - т.ЛиквидационнаяСтоимостьМСФО - р.Амортизация - р.АмортизацияРасчет
	|		ИНАЧЕ р.ПервоначальнаяСтоимость + р.Переоценка + ЕстьNULL(втНесколькоМодернизаций.СуммаМодернизацииМСФО, т.СуммаМодернизацииМСФО) - т.ЛиквидационнаяСтоимостьМСФО - р.Амортизация - р.АмортизацияРасчет
	...
	|		ПО т.ВНА = р.ВНА
	|		ЛЕВОЕ СОЕДИНЕНИЕ втНесколькоМодернизаций КАК втНесколькоМодернизаций
	|		ПО т.ВНА = втНесколькоМодернизаций.ВНА
...
Функция ТекстЗапроса_ТаблицаВНА(НомераТаблиц)
	...
	|	втТаблицаВНА.ИсключитьОбъединение КАК ИсключитьОбъединение,
	...
	|	втТаблицаРасчетВНА.ГруппаВНА <> ЗНАЧЕНИЕ(Справочник.ГруппыВНАМСФО.ПустаяСсылка)
	|		И НЕ втТаблицаРасчетВНА.ВНА ЕСТЬ NULL
	|		И втТаблицаВНА.СчетЗатратМСФО = втТаблицаРасчетВНА.ГруппаВНА.СчетУчетаПервоначальнойСтоимости КАК СписатьИсточник,
