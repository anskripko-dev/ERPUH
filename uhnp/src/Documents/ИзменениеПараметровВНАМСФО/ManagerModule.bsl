...
Функция ТекстЗапроса_ПараметрыВНА(НомераТаблиц)
	...
	ЗапросПараметрыВНА = УчетМСФОПовтИспУХ.ПолучитьКэшированныйТекстЗапросаВНА(
		ПоляВыбораСтрокой, "втТаблицаРасчетВНА", Истина);
...
Функция ТекстЗапроса_втТаблицаВНА(НомераТаблиц)
	...
	|	р.ГруппаВНА <> т.НоваяГруппаВНАМСФО
	|		ИЛИ т.Подразделение <> р.Подразделение
	|		ИЛИ т.НаправлениеДеятельности <> р.НаправлениеДеятельности КАК ИзменениеАналитикиУчета,
	|	р.ГруппаВНА = ЗНАЧЕНИЕ(Справочник.ГруппыВНАМСФО.ПустаяСсылка) КАК РанееНеПараллельно,
...
Функция ТекстЗапроса_ТаблицаВНА(НомераТаблиц)
	...
	|	И НЕ втТаблицаВНА.РанееНеПараллельно";
...
Функция ТекстЗапроса_ПараметрыУчетаВНАМСФО(НомераТаблиц)
	...
	|	втТаблицаВНА.ВидУчета КАК ВидУчета,
	|	втТаблицаВНА.НоваяГруппаВНА КАК ГруппаВНА,
	|	втТаблицаВНА.НоваяГруппаВНА.ТипВНА КАК ТипВНА,
	|	втТаблицаВНА.ПодразделениеДт КАК Подразделение,
	|	втТаблицаВНА.НаправлениеДеятельностиДт КАК НаправлениеДеятельности,	
	...
	|	втТаблицаВНА КАК втТаблицаВНА
	|ГДЕ
	|	втТаблицаВНА.ИзменениеАналитикиУчета";
...
Функция ТекстЗапроса_ПроводкиПеревода(НомераТаблиц)
	...
	|ГДЕ
	|	(втТаблицаВНА.ИзменениеАналитикиУчета
	|			ИЛИ втТаблицаВНА.НоваяГруппаВНА.СчетУчетаПервоначальнойСтоимости <> втТаблицаВНА.СчетУчетаПервоначальнойСтоимости)
	...
	|	ЗНАЧЕНИЕ(Справочник.ВидыОпераций.ВыбытиеАмортизации),
	|	0,
	...
	|		КОГДА втТаблицаВНА.НоваяГруппаВНА.НачислятьАмортизацию
	|			ТОГДА втТаблицаВНА.НоваяГруппаВНА.СчетНакопленнойАмортизацииСубконто1
	|		ИНАЧЕ втТаблицаВНА.НоваяГруппаВНА.СчетУчетаПервоначальнойСтоимостиСубконто1
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА втТаблицаВНА.НоваяГруппаВНА.НачислятьАмортизацию
	|			ТОГДА втТаблицаВНА.НоваяГруппаВНА.СчетНакопленнойАмортизацииСубконто2
	|		ИНАЧЕ втТаблицаВНА.НоваяГруппаВНА.СчетУчетаПервоначальнойСтоимостиСубконто2
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА втТаблицаВНА.НоваяГруппаВНА.НачислятьАмортизацию
	|			ТОГДА втТаблицаВНА.НоваяГруппаВНА.СчетНакопленнойАмортизацииСубконто3
	|		ИНАЧЕ втТаблицаВНА.НоваяГруппаВНА.СчетУчетаПервоначальнойСтоимостиСубконто3
	|	КОНЕЦ
	|ИЗ
	|	втТаблицаВНА КАК втТаблицаВНА
	|ГДЕ
	...
	|	втТаблицаВНА.ВНА,
	|	втТаблицаВНА.НоваяГруппаВНА,
	|	ЗНАЧЕНИЕ(Справочник.ВидыОпераций.ОбесценениеВыбытиеОбесценения),
	|	0,
	|	втТаблицаВНА.Обесценение,
	|	втТаблицаВНА.ПодразделениеДт,
	|	втТаблицаВНА.НаправлениеДеятельностиДт,
	|	втТаблицаВНА.ПодразделениеКт,
	|	втТаблицаВНА.НаправлениеДеятельностиКт,
	|	втТаблицаВНА.НоваяГруппаВНА.СчетУчетаПервоначальнойСтоимости,
	|	втТаблицаВНА.НоваяГруппаВНА.СчетУчетаПервоначальнойСтоимости.СчетСсылка,
	|	втТаблицаВНА.НоваяГруппаВНА.СчетУчетаПервоначальнойСтоимостиСубконто1,
	|	втТаблицаВНА.НоваяГруппаВНА.СчетУчетаПервоначальнойСтоимостиСубконто2,
	|	втТаблицаВНА.НоваяГруппаВНА.СчетУчетаПервоначальнойСтоимостиСубконто3,
	|	втТаблицаВНА.ПараметрыОбесценения.СчетНакопленногоОбесценения,
	|	втТаблицаВНА.ПараметрыОбесценения.СчетНакопленногоОбесценения.СчетСсылка,
	|	втТаблицаВНА.ПараметрыОбесценения.СчетНакопленногоОбесцененияСубконто1,
	|	втТаблицаВНА.ПараметрыОбесценения.СчетНакопленногоОбесцененияСубконто2,
	|	втТаблицаВНА.ПараметрыОбесценения.СчетНакопленногоОбесцененияСубконто3
	|ИЗ
	|	втТаблицаВНА КАК втТаблицаВНА
	|ГДЕ
	|	втТаблицаВНА.ИзменениеПараметровУчета
	|	И втТаблицаВНА.ВидУчета = ЗНАЧЕНИЕ(Перечисление.ВидыУчета.МСФО)
	|	И втТаблицаВНА.НоваяГруппаВНА.ТипВНА <> втТаблицаВНА.ГруппаВНА.ТипВНА
	|	И втТаблицаВНА.Обесценение <> 0";
...
Процедура ЗаполнитьСтроки(КоллекцияСтрок, Источник, ПараметрыЗаполненияТЧ = Неопределено) Экспорт
	...
	Если ПараметрыЗаполненияТЧ.ЗаполнитьМСФО Или ПараметрыЗаполненияТЧ.ЗаполнитьНСБУ Тогда
		...
		ОбъектыВНА = МСФОКлиентСерверУХ.ПолучитьВНАИзСтрок(КоллекцияСтрок);
