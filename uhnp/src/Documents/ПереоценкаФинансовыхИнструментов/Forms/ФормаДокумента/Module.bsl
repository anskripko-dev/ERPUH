...
Процедура УправлениеФормой(Форма)
	...
	ФВ = МСФОВызовСервераУХ.ЗначениеУП("ВалютаУчета", Объект.Организация, КонецДня(Объект.Дата), Объект.Сценарий);
		
	Форма.КэшируемыеЗначения.Вставить("ФункциональнаяВалюта", ФВ);
	
	Рекласс = Форма.КэшируемыеЗначения.ВидыОпераций.Рекласс;
	
	Форма.Элементы.ФинансовыеИнструментыЗаполнитьСтроки.Видимость = (Объект.ВидОперации <> Рекласс);
	Форма.Элементы.ФинансовыеИнструментыПараметрыУчетаФИДо.Видимость = (Объект.ВидОперации = Рекласс);
...
Функция ПолучитьКэшируемыеЗначения(Форма, СтруктураДействий, ПараметрыДоступностьСписанияПереоценки)
	...
	ДатаОкончанияПериода = ОбщегоНазначенияУХ.ПолучитьЗначениеРеквизита(Форма.Объект.ПериодОтчета, "ДатаОкончания");
	ДатаОкончанияПериода = НачалоДня(?(ДатаОкончанияПериода = Неопределено, ТекущаяДата(), ДатаОкончанияПериода));
