	...
	Если Объект.СреднийЗаработокСФР < Объект.МинимальныйСреднедневнойЗаработок Тогда
		ТекстРасчетаПределаИзМРОТ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru = 'В расчете предела дохода, не облагаемого НДФЛ, используется среднедневной заработок из МРОТ: %1 р.';
											|en = 'The average daily earnings from the minimum monthly wage are used to calculate the maximum income that is not subject to personal income tax: %1 rub.'"),
...
Функция ЗаголовокСреднемесячногоКоличестваДнейЧасов()
	...
	Иначе
		Возврат НСтр("ru = 'Среднемесячное количество дней';
					|en = 'Average monthly number of days'");
	КонецЕсли; 
КонецФункции
...
Процедура ПроверитьОстатокОтработанныхДней()
	...
	Если ОтработанныеДни.Количество() < КоличествоОтработанныхДней Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'У сотрудника недостаточно отработанных дней для компенсации отгула';
													|en = 'The employee does not have enough days worked to compensate days off'"), , "ДниНакопленныхОтгулов", "Объект");
	КонецЕсли;
