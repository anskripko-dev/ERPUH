...
Процедура НастроитьЗависимыеЭлементыФормыНаСервере(ИзмененныеРеквизиты = "")
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("Организация") 
		...
		Элементы.НМААмортизацияНаНачалоГодаУУ.Заголовок = СтрШаблон(НСтр("ru = 'УУ (%1)';
																		|en = 'MA (%1)'"), ВалютаУпрПредставление);
		Элементы.НМААмортизацияНаНачалоГодаУУ_Отдельно.Заголовок = СтрШаблон(НСтр("ru = 'Амортизация на начало года (управленческий учет) (%1)';
		...
		Если ЗначениеЗаполнено(СлужебныеПараметрыФормы.ВалютаРегл) Тогда
			...
			Элементы.НМААмортизацияНаНачалоГодаБУ.Заголовок = СтрШаблон(НСтр("ru = 'БУ (%1)';
																			|en = 'AC (%1)'"), ВалютаРеглПредставление);
			Элементы.НМААмортизацияНаНачалоГодаНУ.Заголовок = СтрШаблон(НСтр("ru = 'НУ (%1)';
																			|en = 'TA (%1)'"), ВалютаРеглПредставление);
			Элементы.НМААмортизацияНаНачалоГодаПР.Заголовок = СтрШаблон(НСтр("ru = 'ПР (%1)';
																			|en = 'PD (%1)'"), ВалютаРеглПредставление);
			Элементы.НМААмортизацияНаНачалоГодаБУ_Отдельно.Заголовок = СтрШаблон(НСтр("ru = 'Амортизация на начало года (бухгалтерский учет) (%1)';
		...
		Иначе
			...
			Элементы.НМААмортизацияНаНачалоГодаБУ_Отдельно.Заголовок = НСтр("ru = 'Амортизация на начало года (бухгалтерский учет)';
																			|en = 'Depreciation as of the year start (local accounting)'");
