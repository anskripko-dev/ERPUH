...
Функция ПодготовитьПараметрыПроведения(ДополнительныеСвойства, Отказ, ФормироватьПроводкиМСФО = Неопределено) Экспорт
	...
	Реквизиты.Вставить("ЕстьСуммаВВалютеОтчетности", Истина); //Для ЗаполнитьСуммуВВалютеОтчетности
...
Функция ТекстЗапросаТаблицаПроводок(НомераТаблиц = Неопределено)
	...
	|	&Период КАК Период,
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(&Ссылка КАК Документ.НачислениеОперацийМСФО).СторнироватьАвтоматически = ИСТИНА
	|			ТОГДА НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(&Период, МЕСЯЦ, 1), МЕСЯЦ)
	|	КОНЕЦ КАК ПериодСторно,
	|	ЗНАЧЕНИЕ(Справочник.ВидыОпераций.НачислениеДоходовРасходовМСФО) КАК ВидОперации,
	|	ЗНАЧЕНИЕ(Справочник.ВидыОпераций.НачислениеДоходовРасходовМСФО) КАК Событие,
	|	ДвиженияНачислений.Ссылка.Организация КАК Организация,
	|	ДвиженияНачислений.Ссылка.Сценарий КАК Сценарий,
	|	ВЫБОР
	|		КОГДА ДвиженияНачислений.СчетДт.ПризнакиУчета ПОДОБНО ""%УчетПоПодразделениям%""
	|			ТОГДА ДвиженияНачислений.ПодразделениеДт
	|	КОНЕЦ КАК ПодразделениеДт,
	|	ВЫБОР
	|		КОГДА ДвиженияНачислений.СчетДт.ПризнакиУчета ПОДОБНО ""%УчетПоНаправлениямДеятельности%""
	|			ТОГДА ДвиженияНачислений.НаправлениеДеятельностиДт
	|	КОНЕЦ КАК НаправлениеДеятельностиДт,
	|	ВЫБОР
	|		КОГДА ДвиженияНачислений.СчетКт.ПризнакиУчета ПОДОБНО ""%УчетПоПодразделениям%""
	|			ТОГДА ДвиженияНачислений.ПодразделениеКт
	|	КОНЕЦ КАК ПодразделениеКт,
	|	ВЫБОР
	|		КОГДА ДвиженияНачислений.СчетКт.ПризнакиУчета ПОДОБНО ""%УчетПоНаправлениямДеятельности%""
	|			ТОГДА ДвиженияНачислений.НаправлениеДеятельностиКт
