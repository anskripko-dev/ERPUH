...
Процедура ПроверитьКоличествоВДокументеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		ОбщегоНазначенияУТКлиент.ВывестиСообщениеПревышенияПоКоличеству(Результат, НСтр("ru = 'введенного на основании распоряжения.';
																						|en = 'entered based on the reference.'"));
		РассчитатьИтоговыеПоказатели(ЭтотОбъект);
	КонецЕсли;
...
Процедура ЗаполнитьВидыЦен()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5
		...
		ПараметрыВыбораВидаЦен.Добавить(Новый ПараметрВыбора("ИспользоватьПриВыпускеПродукции", Истина));
		Если Не ТолькоПросмотр
				И ЗначениеЗаполнено(Объект.ВидЦены)
				И Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ВидЦены, "ИспользоватьПриВыпускеПродукции") Тогда
