...
Процедура ЗаполнитьПоЗаказамОрдерам(Товары, Регистратор, ПараметрыЗаполнения, ШтрихкодыУпаковок = Неопределено) Экспорт
	...
	Если ПараметрыЗаполнения.ЗаполнятьПоОрдеру
		И ЗначениеЗаполнено(ПараметрыЗаполнения.МассивЗаказов)
		И ТипЗнч(ПараметрыЗаполнения.МассивЗаказов[0]) = Тип("ДокументСсылка.ЗаказПереработчику2_5") Тогда
		
		// Передаем копию таблицы ордеров, так как в алгоритме необходимо обнулять подобранное количество,
		// а оно будет использоваться далее.
		ЗаполнитьСерииЗаказовПоТаблицеОрдеров(ТаблицаЗаказы, ТаблицаОрдера.Скопировать());
...
Функция ТекстЗапросаТаблицаДатыПоступленияТоваровОрганизаций(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ПО КлючиАналитики.КлючАналитики = ТаблицаВидыЗапасов.АналитикаУчетаНоменклатуры
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДатыПоступленияТоваровОрганизаций КАК ПоступленияТоваров
	|	ПО ПоступленияТоваров.ВидЗапасов = ТаблицаВидыЗапасов.КорВидЗапасов
	|	И ПоступленияТоваров.Номенклатура = КлючиАналитики.Номенклатура
	|	И ПоступленияТоваров.Характеристика = КлючиАналитики.Характеристика
	|	И ПоступленияТоваров.Серия = КлючиАналитики.Серия
	|	И ПоступленияТоваров.Назначение = КлючиАналитики.Назначение
	|	И ПоступленияТоваров.НомерГТД = ТаблицаВидыЗапасов.НомерГТД
	|
	|ГДЕ
...
Процедура ЗаполнитьСерииЗаказовПоТаблицеОрдеров(ТаблицаЗаказы, ТаблицаОрдера)
	
	Ключ = "Распоряжение, Номенклатура, Характеристика, Назначение";
	Отбор = Новый Структура(Ключ);
	...
	Для Каждого СтрокаЗаказа Из ТаблицаЗаказы Цикл
		...
		КоличествоВЗаказе = СтрокаЗаказа.КоличествоВЗаказе;
		ЗаполнитьЗначенияСвойств(Отбор, СтрокаЗаказа);
		
		Для Каждого СтрокаОрдера Из ТаблицаОрдера.НайтиСтроки(Отбор) Цикл
			
			Если Не ЗначениеЗаполнено(СтрокаОрдера.Серия) Тогда
				Продолжить;
			КонецЕсли;
			
			Количество = Мин(КоличествоВЗаказе, СтрокаОрдера.Количество);
			
			Если СтрокаЗаказа.КоличествоВЗаказе = Количество Тогда
				ТекущиеДанные = СтрокаЗаказа;
			Иначе
				
				ТекущиеДанные = ТаблицаЗаказы.Добавить();
				ЗаполнитьЗначенияСвойств(ТекущиеДанные, СтрокаЗаказа);
				
				ТекущиеДанные.КоличествоВЗаказе = Количество;
				СтрокаЗаказа.КоличествоВЗаказе  = СтрокаЗаказа.КоличествоВЗаказе - Количество;
				
			КонецЕсли;
			
			ТекущиеДанные.Серия     = СтрокаОрдера.Серия;
			
			КоличествоВЗаказе       = КоличествоВЗаказе - Количество;
			СтрокаОрдера.Количество = СтрокаОрдера.Количество - Количество;
	...
	Отбор = Новый Структура("КоличествоВЗаказе", 0);
	Для Каждого ТекущиеДанные Из ТаблицаЗаказы.НайтиСтроки(Отбор) Цикл
		ТаблицаЗаказы.Удалить(ТекущиеДанные);
	КонецЦикла;
