...
Процедура ИзменитьСостояниеЗаявкиКлиент(ВыбранноеЗначение)
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда
		ОМ = ОбщегоНазначенияКлиент.ОбщийМодуль("РасширениеПроцессыИСогласованиеКлиентУХ");
		ОМ.ИзменитьСостояниеЗаявкиКлиент(ВыбранноеЗначение, ЭтаФорма);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПринятьКСогласованию_Подключаемый() Экспорт
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда
		ОМ = ОбщегоНазначенияКлиент.ОбщийМодуль("ДействияСогласованиеУХКлиент");
		ОМ.ПринятьКСогласованию(ЭтаФорма, Объект.Ссылка);
	КонецЕсли;
КонецПроцедуры
...
Процедура ИсторияСогласования_Подключаемый() Экспорт
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда
		ОМ = ОбщегоНазначенияКлиент.ОбщийМодуль("ДействияСогласованиеУХКлиент");
		ОМ.ИсторияСогласования(ЭтаФорма, Объект.Ссылка);
	КонецЕсли
КонецПроцедуры
...
Процедура СогласоватьДокумент_Подключаемый() Экспорт
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда
		ОМ = ОбщегоНазначенияКлиент.ОбщийМодуль("ДействияСогласованиеУХКлиент");
		ОМ.СогласоватьДокумент(ЭтаФорма);
	КонецЕсли
КонецПроцедуры
...
Процедура ОтменитьСогласование_Подключаемый() Экспорт
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда
		ОМ = ОбщегоНазначенияКлиент.ОбщийМодуль("ДействияСогласованиеУХКлиент");
		ОМ.ОтменитьСогласование(ЭтаФорма, Объект.Ссылка);
	КонецЕсли
КонецПроцедуры
...
Процедура МаршрутСогласования_Подключаемый() Экспорт
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда
		ОМ = ОбщегоНазначенияКлиент.ОбщийМодуль("ДействияСогласованиеУХКлиент");
		ОМ.МаршрутСогласования(ЭтаФорма, Объект.Ссылка);
	КонецЕсли
КонецПроцедуры
...
Процедура ВложенияСогласования_Подключаемый() Экспорт
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда
		ОМ = ОбщегоНазначенияКлиент.ОбщийМодуль("ДействияСогласованиеУХКлиент");
		ОМ.ТелеграмВложенныеФайлы(ЭтаФорма, Объект.Ссылка);
	КонецЕсли
КонецПроцедуры
...
Функция СохранитьВложенияВСоответствие(ОбъектВход)
	...
	Для Каждого ТекСтадии Из ОбъектВход.Стадии Цикл
		РезультатФункции.Вставить(ТекСтадии.ИдентификаторСтроки, ТекСтадии.Вложения);
	КонецЦикла;	
	Возврат РезультатФункции;
...
Процедура СчитатьВложенияИзСоответствия(ОбъектПриемник, СоответствиеВложенийВход)
	Для Каждого ТекСтадии Из ОбъектПриемник.Стадии Цикл
		ХранилищеВложений = СоответствиеВложенийВход.Получить(ТекСтадии.ИдентификаторСтроки);
		Если ХранилищеВложений <> Неопределено Тогда
			...
			Если ТаблицаВложений <> Неопределено Тогда
				ТекСтадии.ВложенияФорма.Загрузить(ТаблицаВложений);	// Вложение
			Иначе
				// Пустое хранилище.
				ТекСтадии.ВложенияФорма.Очистить();
			КонецЕсли;	
...
Процедура ЗаполнитьДействияПоШаблону_Сервер(ОчищатьСписокВход = Истина)
	...
	Если ЗначениеЗаполнено(НоваяДатаИзменения) И ОчищатьСписокВход = Ложь Тогда
	...
	Если ЗначениеЗаполнено(СтадияВход) Тогда
		Попытка
			Для Каждого ТекТипыИтоговыхДокументов Из СтадияВход.ТипыИтоговыхДокументов Цикл
				Если ЗначениеЗаполнено(ТекТипыИтоговыхДокументов.ТипИтоговогоДокумента) И ТекТипыИтоговыхДокументов.ТипИтоговогоДокумента.ЗначениеПустойСсылки <> Неопределено Тогда
					//НовыйТип = ВернутьТипПоСсылкеБД(СправочникБД);	
					РезультатФункции.Добавить(ТипЗнч(ТекТипыИтоговыхДокументов.ТипИтоговогоДокумента.ЗначениеПустойСсылки));
				КонецЕсли;
			КонецЦикла;	
...
Процедура УправлениеДоступностью()
	...
	Элементы.ОтправитьПредписание.Видимость = ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование");	
	...
	Элементы.ШаблонМероприятия.Видимость						 = ЭтоКонтрольноеМероприятие ИЛИ ЭтоМероприятиеЗакупочнаяПроцедура;  
	Если ЭтоМероприятиеЗакупочнаяПроцедура И ЗначениеЗаполнено(Объект.Контекст) Тогда  
		ПараметрыВыбораШаблонаМероприятий = Новый Массив();
		Отбор =  Новый ПараметрВыбора("Отбор.СпособВыбораПоставщика",Объект.СпособВыбораПоставщика);
		ПараметрыВыбораШаблонаМероприятий.Добавить(Отбор);
		Элементы.ШаблонМероприятия.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыВыбораШаблонаМероприятий);
	КонецЕсли;	
	Элементы.ШаблонМероприятия.ТолькоПросмотр					 = ЭтоКонтрольноеМероприятие;
	...
	Элементы.СпособВыбораПоставщика.Видимость = ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ЦентрализованныеЗакупки"); 
	Если НЕ Элементы.Найти("ГруппаСтатусыСогласования") = Неопределено Тогда
		Элементы["ГруппаСтатусыСогласования"].Видимость				 = НЕ (ЭтоМероприятиеЗакупочнаяПроцедура);
	КонецЕсли;            
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ЦентрализованныеЗакупки") Тогда 
  		НовыйМассив.Добавить(Новый СвязьПараметраВыбора("Отбор.СпособВыбораПоставщика", "Объект.СпособВыбораПоставщика"));
	КонецЕсли;
	...
	Если ЭтоМероприятиеЗакупочнаяПроцедура Тогда
		...
		Элементы.РисковоеСобытие.Заголовок =  "Контекст";
	...
	КонецЕсли;
	Элементы.ОтправитьПредписание.Видимость = ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ЦентрализованныеЗакупки");
	Элементы.ОтправитьЗадачуОтветственному.Видимость = ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ЦентрализованныеЗакупки");
...
Процедура ЗаписатьДеревоВСтадии(ТекущийОбъект)
	...
	ТекущийОбъект.ИтоговыеДокументы.Очистить();
	Для Каждого ТекСтроки Из ДеревоВрем.Строки Цикл
		...
		Если ТекСтроки.ГруппирующаяСтрока Тогда
			...
			Для Каждого ТекДочерниеСтроки Из ТекСтроки.Строки Цикл                    // Вложение
				...
				Для каждого Вложение Из ТекДочерниеСтроки.ВложенияФорма Цикл
					НоваяСтрокаВложения = ТекущийОбъект.ИтоговыеДокументы.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокаВложения, Вложение);
				КонецЦикла;
				//НоваяДочерняяСтрока.Вложения			 = Новый ХранилищеЗначения(ТекДочерниеСтроки.ВложенияФорма);
			КонецЦикла;	
		Иначе
			НоваяСтрока.Действие			 = ТекСтроки.Действие;
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтроки);
			//НоваяСтрока.Вложения			 = Новый ХранилищеЗначения(ТекСтроки.ВложенияФорма);
			НоваяСтрока.ГруппаРодитель		 = 0;
			НоваяСтрока.ГруппирующаяСтрока	 = Ложь;
			НоваяСтрока.ПредставлениеГруппы	 = "";
			Для каждого Вложение Из ТекСтроки.ВложенияФорма Цикл
				НоваяСтрокаВложения = ТекущийОбъект.ИтоговыеДокументы.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрокаВложения, Вложение);
			КонецЦикла;
		КонецЕсли;	
...
Процедура СчитатьВложения()
	Для каждого ТекФормаСтадии Из Объект.Стадии Цикл
	    Вложения = Объект.ИтоговыеДокументы.Выгрузить(Новый Структура("ИдентификаторСтроки", ТекФормаСтадии.ИдентификаторСтроки));
		ТекФормаСтадии.ВложенияФорма.Загрузить(Вложения);
		Для каждого Строка Из ТекФормаСтадии.ВложенияФорма Цикл
			Строка.ИмяФайла = Строка.СсылкаНаФайл;
		КонецЦикла;
	КонецЦикла;
...
Процедура ОбновитьЗаголовокФормы()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ЦентрализованныеЗакупки") И ТипЗнч(Объект.Контекст) = Тип(Имя) Тогда
...
Функция ПолучитьОбщуюДлительностьСтадий()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ЦентрализованныеЗакупки") Тогда 
		...
		СтадииПодготовка = Модуль.ВернутьСтадииНачалоПодготовкиЗакупкиКПубликации();
	Иначе
		СтадииПодготовка = Новый Массив;
	КонецЕсли;
	// Получим исходные данные из формы.
	ДеревоВрем = РеквизитФормыВЗначение("ДеревоСтадий");
	ТаблицаСтадий = ПреобразоватьДеревоЗначенийВТаблицу(ДеревоВрем);
...
Функция ВернутьМассивСтадийШаблона(ШаблонВход, КонтекстВход)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ЦентрализованныеЗакупки") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("ЦентрализованныеЗакупкиУХ");
		РезультатФункции = Модуль.ВернутьМассивСтадийШаблона(ШаблонВход, КонтекстВход);
	Иначе
...
Функция ПолучитьДатуПоКалендарю_Сервер(ПроизводственныйКалендарьВход, ДатаВход, ДлительностьВход)
	//Возврат КалендарныеГрафики.ДатаПоКалендарю(ПроизводственныйКалендарьВход, ДатаВход, ДлительностьВход);   
...
Функция РазностьДатПоКалендарю_Сервер(КалендарьВход, ДатаНачалаВход, ДатаОкончанияВход)          
	//Возврат КалендарныеГрафики.РазностьДатПоКалендарю(КалендарьВход, ДатаНачалаВход, ДатаОкончанияВход); 
	КалендарныеГрафикиОМ = Вычислить("КалендарныеГрафики");
	Если КалендарныеГрафикиОМ = Неопределено Тогда
		Возврат КалендарныеГрафики.РазностьДатПоКалендарю(КалендарьВход, ДатаНачалаВход, ДатаОкончанияВход); 	
	КонецЕсли;		
	РезультатФункции = КалендарныеГрафикиОМ.РазностьДатПоКалендарю(КалендарьВход, ДатаНачалаВход, ДатаОкончанияВход);
	Возврат РезультатФункции;
...
Процедура ВыставитьФактическуюДлительностьПоМассиву(ДеревоВход, МассивДанныхСтрокВход, ГлубинаРекурсииВход = 0)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("БухгалтерияПредприятияПодсистемы") Тогда 
		ПустойКалендарь = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Справочник.ПроизводственныеКалендари.ПустаяСсылка");
	Иначе
		ПустойКалендарь = Неопределено;
	КонецЕсли;
	...
	Для Каждого ТекСтроки Из ДеревоВход.ПолучитьЭлементы() Цикл
		Если НЕ ТекСтроки.ГруппирующаяСтрока Тогда
			...
			Иначе
				...
				Иначе
					НоваяДлительность = ЗначениеРеквизита(ТекСтроки.Действие, "Длительность");
				КонецЕсли;
			КонецЕсли;
			...
			КонецЕсли;	
		ИначеЕсли Объект.ИспользоватьПроизводственныйКалендарь Тогда
...
Процедура КонтрольПоследовательностиСтадий(ДеревоВход, Отказ)
	...
	Если ЗначениеЗаполнено(ТекШаблон) Тогда
		КонтролироватьПоследовательностьСтадий = ЗначениеРеквизита(ТекШаблон, "КонтролироватьПоследовательностьСтадий");
		Если КонтролироватьПоследовательностьСтадий Тогда
			...
			ОбщегоНазначенияКлиентСервер.УдалитьВсеВхожденияЗначенияИзМассива(МассивСтадийДерева, ПредопределенноеЗначение("Справочник.СтадииМероприятий.ПустаяСсылка"));
...
Процедура ВыставитьДатуОкончанияПоСтадиям()
	...
	Если ЗначениеЗаполнено(МаксимальнаяСтадия) Тогда
		ДлительностьМаксимальнойСтадии = ЗначениеРеквизита(МаксимальнаяСтадия, "Длительность");
...
Процедура ОткрытьФормуПоТипуОбъекта(ТипДокументаВход, ДополнительныеПараметрыВход) 
	НаименованиеДокумента = ЗначениеРеквизита(ТипДокументаВход, "Наименование");
...
Функция ПолучитьСтруктуруЗаполненияОбъектаБД(ОбъектБдВход, МероприятиеВход)
	...
	НаименованиеОбъекта = ЗначениеРеквизита(ОбъектБдВход, "Наименование");
...
Функция РассчитатьДлительностьСтадии(ШаблонВход, ДатаНачалаВход, ДатаОкончанияВход)
	РезультатФункции = 0;
	Если ЗначениеЗаполнено(ШаблонВход)  И (ОбщегоНазначенияКлиент.ПодсистемаСуществует("БухгалтерияПредприятияПодсистемы")
		ИЛИ ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ЦентрализованныеЗакупки")) Тогда
...
Процедура ОткрытьФормуНастройки(ВладелецНастройкиВход)
	...
	Если НастройкаПроцессаВыбора = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ЦентрализованныеЗакупки") Тогда
...
Процедура ВыборЛотаДляНастройкиПроцесса_Завершение(ЗначениеВыбора, ДопПараметры) Экспорт
	Если ЗначениеВыбора <> Неопределено Тогда
		МетодОценки = ЗначениеРеквизита(ЗначениеВыбора, "МетодОценкиПредложенийПоставщиков");
		Имя = "Перечисление.МетодыОценкиПредложенийПоставщиков.ПоЦенам";
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") И МетодОценки = ПредопределенноеЗначение(Имя) Тогда
...
Процедура ВыборЛота_Завершение(ЗначениеВыбора, ДопПараметры) Экспорт
    Если ЗначениеВыбора <> Неопределено Тогда
		...
		Если ТекОбъектБД <> Неопределено Тогда
			НаименованиеДокумента = ЗначениеРеквизита(ТекОбъектБД, "Наименование");
			Если ТекОбъектБД <> Неопределено Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда
		...
		Модуль = ОбщегоНазначения.ОбщийМодуль("МодульСогласованияДокументовУХ");
		Модуль.ПриСозданииНаСервереОбъект(ЭтаФорма);
	КонецЕсли;
	УправлениеДоступностью();
	СортироватьСтадииПоДате();
	...
	Если Метаданные.ФункциональныеОпции.Найти(СтратегическиеИнинциативы) <> Неопределено Тогда
		Если ПолучитьФункциональнуюОпцию(СтратегическиеИнинциативы) = Ложь Тогда
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ЦентрализованныеЗакупки") Тогда
		...
		Элементы.ВидМероприятия.СписокВыбора.Удалить(Элементы.ВидМероприятия.СписокВыбора.НайтиПоЗначению(ПредопределенноеЗначение("Перечисление.ВидыМероприятий." + ИмяЭлемента)));
		Модуль = ОбщегоНазначения.ОбщийМодуль("ЦентрализованныеЗакупкиВызовСервераУХ");
		СпособВыбораПоставщика = Модуль.ПолучитьИнформациюОЗакупочнойПроцедуре(Объект.Контекст);
	КонецЕсли;
...
Процедура ДобавитьФайл(ЗаписатьОбъект = Ложь)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ТекстСообщения = НСтр("ru = 'Мероприятие необходимо записать.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	Если Элементы.ДеревоСтадийВложенияФорма.ТекущиеДанные <> Неопределено И Элементы.ДеревоСтадий.ТекущиеДанные <> Неопределено Тогда
		ОписаниеОбработчикаДобавленияФайла = Новый ОписаниеОповещения("ОбработчикДобавленияФайла", ЭтаФорма);
...
Процедура ОбработчикДобавленияФайла(СсылкаНаФайл, ДопПараметры) Экспорт
	Если СсылкаНаФайл = Неопределено ИЛИ Элементы.ДеревоСтадий.ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Если ТекДанныеТаблица <> Неопределено Тогда
		...
		ТекДанныеТаблица.СсылкаНаФайл	 = СсылкаНаФайл.ФайлСсылка;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОткрытьФайл()
	...
	Если ТекДанныеДерево <> Неопределено Тогда
		ТекДанныеТаблица = Элементы.ДеревоСтадийВложенияФорма.ТекущиеДанные;
		Если ТекДанныеТаблица <> Неопределено Тогда
			Если ЗначениеЗаполнено(ТекДанныеТаблица.СсылкаНаФайл) Тогда
				ДанныеФайла = РаботаСФайламиКлиент.ДанныеФайла(ТекДанныеТаблица.СсылкаНаФайл, УникальныйИдентификатор);
...
Процедура РисковоеСобытиеПриИзменении(Элемент)
	...
	ИнформацияОРисковомСобытии = ПолучитьТекстИнформацииОРисковомСобытии(Объект.Контекст);
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ЦентрализованныеЗакупки") Тогда
		Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ЦентрализованныеЗакупкиВызовСервераУХ");
		СпособВыбораПоставщика = Модуль.ПолучитьИнформациюОЗакупочнойПроцедуре(Объект.Контекст);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПодтверждениеОчисткиСтадий_Завершение(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = КодВозвратаДиалога.Да Тогда 
		Объект.СпособВыбораПоставщика = ЗначениеРеквизита(Объект.ШаблонМероприятия, "СпособВыбораПоставщика");
		СпособВыбораПоставщика = Объект.СпособВыбораПоставщика;
...
Процедура ОтправитьПредписание(Команда)
	...
	Если ПроверкаПройдена Тогда
		...
		Если ЗначениеЗаполнено(НоваяЗадачаИсполнитель) Тогда
			...
			ОМ = ОбщегоНазначенияКлиент.ОбщийМодуль("МодульУправленияОповещениямиУХ");
			ОМ.ОповеститьПользователей(ВидСобытия, , Объект.Ссылка, СписокРассылки, , НоваяЗадачаИсполнитель);
			...
			Если Объект.КонтролироватьИсполненияПредписания Тогда
				...
				Если ЗначениеЗаполнено(НоваяЗадачаКонтролирующий) Тогда
					...
					ОМ.ОповеститьПользователей(ВидСобытия, , Объект.Ссылка, СписокРассылки, , НоваяЗадачаКонтролирующий);
				Иначе
	...
	Если ТекДанные <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ТекОтветственныйЗаСтадию) Тогда
			...
			Если ЗначениеЗаполнено(НоваяЗадача) Тогда
				...
				ОМ = ОбщегоНазначенияКлиент.ОбщийМодуль("МодульУправленияОповещениямиУХ");
				РезультатОтправки = ОМ.ОповеститьПользователей(ВидСобытия, , Объект.Ссылка, СписокРассылки, СтруктураДополнительныхПараметров, НоваяЗадача);
...
Процедура ДеревоСтадийСтадияГруппаПриИзменении(Элемент)
	...
	Если ТекДанные <> Неопределено Тогда
		...
		Если СокрЛП(ТекДанные.ОписаниеРеакции) = "" Тогда
			НовоеОписание = ЗначениеРеквизита(ТекДанные.Действие, "Описание");
...
Процедура ДеревоСтадийВложенияФормаИтоговыйДокументНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Если Элементы.ДеревоСтадий.ТекущиеДанные <> Неопределено Тогда
		ТекДанныеТаблица = Элементы.ДеревоСтадийВложенияФорма.ТекущиеДанные;
		Если ТекДанныеТаблица <> Неопределено И ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ЦентрализованныеЗакупки") Тогда
			// Ограничение выбора типа прикрепляемого документа.   
			МассивТиповИтоговыхДокументов = Новый Массив;
			НовыйТип = ВернутьТипПоСсылкеБД(ТекДанныеТаблица.ТипИтоговогоДокумента);	
			...
			Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ЦентрализованныеЗакупкиВызовСервераУХ");
			МассивОтбор = Модуль.ВернутьМассивОтбораПоТипуДокумента(ТекДанныеТаблица.ТипИтоговогоДокумента, Объект);
...
&НаСервереБезКонтекста
Функция ВернутьТипПоСсылкеБД(СправочникТекущейИБ)
	Если ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.ОбъектыНастраиваемойОтчетности") Тогда
		ОМ = ОбщегоНазначения.ОбщийМодуль("ОбщегоНазначенияСерверУХ");
		Возврат ОМ.ВернутьТипПоСсылкеБД(СправочникТекущейИБ);
	КонецЕсли;
КонецФункции
...
Процедура ДеревоСтадийПередУдалением(Элемент, Отказ)
	ТекДанные = Элементы.ДеревоСтадий.ТекущиеДанные;
	Если ТекДанные <> Неопределено И ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ЦентрализованныеЗакупки") Тогда
		// Ограничение доступности стадии ФЗ-223.
		Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ЦентрализованныеЗакупкиВызовСервераУХ");
		Отказ = Модуль.ЭтоФз223(Объект) И ТекДанные.ИзШаблона;
		Если Отказ Тогда
...
Процедура ДеревоСтадийПередНачаломИзменения(Элемент, Отказ)
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ЦентрализованныеЗакупки") И Элементы.ДеревоСтадий.ТекущиеДанные <> Неопределено Тогда
		Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ЦентрализованныеЗакупкиВызовСервераУХ");
		Модуль.ДеревоСтадийПередНачаломИзменения((Элемент.ТекущийЭлемент.Имя), Отказ, (Объект), (Элементы.ДеревоСтадий.ТекущиеДанные.Действие), (Элементы.ДеревоСтадий.ТекущиеДанные.ИзШаблона));
	КонецЕсли;
КонецПроцедуры
...
Процедура ДеревоСтадийВложенияФормаПередНачаломИзменения(Элемент, Отказ)
	...
	Если ТекДанные <> Неопределено И Элемент.ТекущийЭлемент.Имя = "ДеревоСтадийВложенияФормаПредставлениеДокумента" Тогда
...
&НаСервереБезКонтекста
Функция ЗначениеРеквизита(ОбъектСРеквизитом, Имя)
	Возврат	ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектСРеквизитом, Имя);
КонецФункции
