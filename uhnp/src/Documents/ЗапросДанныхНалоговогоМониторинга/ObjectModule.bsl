...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ЭтотОбъект.ПлановаяДатаОтвета = НалоговыйМониторингВызовСервера.ПлановаяДатаОтветаНаЗапросНМ(ТекущаяДата());
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда
		...
		ИмяВидыСобытия = "Справочник.ВидыСобытийОповещений.ЗапросНалоговогоМониторинга";
		ВидСобытия = ПредопределенноеЗначение(ИмяВидыСобытия);
		
		Модуль.УдалитьНапоминанияПоОтбору(Новый Структура("Источник, ВидСобытияОповещения", Ссылка, ВидСобытия));
	КонецЕсли;
...
Процедура ЗарегистрироватьОповещение()
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		МассивРассылки = РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Пользователь");
					
		МодульСистемаВзаимодействияСерверУХ = ОбщегоНазначения.ОбщийМодуль("СистемаВзаимодействияСерверУХ");
		БотЗадачИОповещений = МодульСистемаВзаимодействияСерверУХ.ПолучитьПользователяБотаЗадачИОповещений();			
		Если МассивРассылки.Найти(БотЗадачИОповещений) = Неопределено Тогда
			МассивРассылки.Добавить(БотЗадачИОповещений);
		КонецЕсли;
		
		Модуль.ОповеститьПользователей(ВидСобытия, , Ссылка, МассивРассылки);
		КатегорииСобытийОповещенийНапоминания = "Справочник.КатегорииСобытийОповещений.Напоминания";
		
		Если ВидСобытия.КатегорияСобытия = ПредопределенноеЗначение(КатегорииСобытийОповещенийНапоминания) Тогда
			Модуль.СоздатьНапоминание(ВидСобытия, Ссылка, МассивРассылки);
		КонецЕсли;		
	КонецЕсли;
