...
Функция СписаниеКодовМаркировкиJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено, Повторно = Ложь)
	...
	ИспользуетсяОсновнойДокументСписания = Шапка.ПричинаСписания = Перечисления.ПричиныСписанияКодовМаркировкиИСМП.ВыбытиеДоОбязательностиМаркировки
		Или ИнтеграцияИСКлиентСервер.ИспользуетсяОсновнойДокументСписания(Шапка.ПричинаСписания)
			И Не (ПередачаОтчетаЧерезСУЗ
				И ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМОТП(Шапка.ВидПродукции)
				И Шапка.Операция = Перечисления.ВидыОперацийИСМП.СписаниеЭмитированныхКодовМаркировки);
		...
		Для Каждого СтрокаДанных Из РезультатОбработкиДанныхОтчета.ОбработанныеДанныеОтчета Цикл
			...
			Для Каждого ИменаКолонок Из РезультатОбработкиДанныхОтчета.ГруппыКолонок Цикл
				...
				Если КешДанныхРазбора[НормализованныйКодМаркировки] = Неопределено Тогда
					...
					Если ПередачаОтчетаЧерезСУЗ И Не ЭтоМОТП И Не ИспользуетсяОсновнойДокументСписания Тогда
				...
				ИначеЕсли ВидыУпаковокДляПередачиОтчетаОНанесении.Получить(ВидУпаковки) <> Неопределено
					...
					И Не ЭтоМОТП
					И Не ИспользуетсяОсновнойДокументСписания Тогда
		...
		Если ПередачаОтчетаЧерезСУЗ Тогда
			...
			Для Каждого СтрокаТЧМаркированныеТовары Из РезультатПоиска.МаркированныеТовары Цикл
				...
				Если СтрокаТЧМаркированныеТовары.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
					...
					Если ЭтоМОТП Или ИспользуетсяОсновнойДокументСписания Тогда
...
#КонецОбласти
	...
	ИначеЕсли ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(Шапка.ВидПродукции) Или ИспользуетсяОсновнойДокументСписания Тогда
		...
		Если ЭтоПиво И ИспользуетсяОсновнойДокументСписания Тогда
...
Функция СообщенияJSONСписаниеКодовМаркировкиИСМП(Шапка, СведенияОТоварах, ДополнительныеПараметры)
	...
	ИспользуетсяОсновнойДокументСписания = ИнтеграцияИСКлиентСервер.ИспользуетсяОсновнойДокументСписания(Шапка.ПричинаСписания);
	
	ТелоЗапроса = Новый Структура;
	Если ИспользуетсяОсновнойДокументСписания Тогда
	...
	Если Шапка.ПричинаСписания = Перечисления.ПричиныСписанияКодовМаркировкиИСМП.ВыбытиеДоОбязательностиМаркировки Тогда
