	...
	Если НеобходимоОформитьЗаявлениеОВвозеТоваровПолученное(Объект.Ссылка) Тогда
		...
		Если КоличествоЗаявлений = 0 Тогда
			ЧастиФорматированнойСтроки.Добавить(Новый ФорматированнаяСтрока(
				НСтр("ru = 'Оформить заявление о ввозе от покупателя';
					|en = 'Оформить заявление о ввозе от покупателя'"),,,,"ОформитьЗаявлениеОВвозеПолученное"));
		ИначеЕсли КоличествоЗаявлений = 1 Тогда
			Если Выборка.Следующий() Тогда
				...
				МассивПодстрок.Добавить(СтрШаблон(НСтр("ru = 'Отметка о регистрации заявления %1 от %2';
														|en = 'Отметка о регистрации заявления %1 от %2'"),
		...
		Иначе
			...
			Пока Выборка.Следующий() Цикл
				...
				МассивПодстрок.Добавить(СтрШаблон(НСтр("ru = 'Заявление с отметкой о регистрации %1 от %2';
														|en = 'Заявление с отметкой о регистрации %1 от %2'"),
					Выборка.НомерОтметкиОРегистрацииЗаявления,
					Формат(Выборка.ДатаОтметкиОРегистрацииЗаявления, "ДЛФ=D")));
			...
			ЧастиФорматированнойСтроки.Добавить(Новый ФорматированнаяСтрока(
				СтрШаблон(НСтр("ru = 'Заявление о ввозе от покупателя из ЕАЭС (%1)';
								|en = 'Заявление о ввозе от покупателя из ЕАЭС (%1)'"),
...
Функция СуммаОтгрузкиТоваров(ДокументОтгрузки, Период, КодПричиныЗаявления) Экспорт
	...
	Если КодПричиныЗаявления = "1"
		...
		//++ НЕ УТКА
		ИначеЕсли ТипЗнч(ДокументОтгрузки) = Тип("ДокументСсылка.ОтчетДавальцу2_5") Тогда
			Запрос.Параметры.Вставить("ЗаказКлиента", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОтгрузки, "ЗаказДавальца"));
			Запрос.Текст =
			"ВЫБРАТЬ
			|	ЕСТЬNULL(СУММА(ВыручкаИСебестоимостьПродаж.СуммаБезНДСВВалютеДокумента), 0) КАК СуммаБезНДСВВалютеДокумента
			|ИЗ
...
Функция ЕстьСчетаФактурыКОформлению(Параметры) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВЫРАЗИТЬ(ДокументыОтгрузки.ДокументРеализации КАК Документ.ОтчетДавальцу2_5) КАК ДокументРеализации,
	|	ВЫРАЗИТЬ(ДокументыОтгрузки.ДокументРеализации КАК Документ.ОтчетДавальцу2_5).ЗаказДавальца КАК ЗаказДавальца
	|ПОМЕСТИТЬ ОтчетыДавальцам0
	|ИЗ
	|	ДокументыОтгрузки КАК ДокументыОтгрузки
	|ГДЕ
	|	ДокументыОтгрузки.ДокументРеализации ССЫЛКА Документ.ОтчетДавальцу2_5
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ЗаказДавальца
	|;
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|ВЫБРАТЬ
	|	ОтчетыДавальцам0.ДокументРеализации КАК ДокументРеализации,
	|	СУММА(ЕСТЬNULL(ВыручкаИСебестоимостьПродаж.СуммаБезНДСВВалютеДокумента, 0)) КАК СуммаОтгрузки
	|
	|ИЗ
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ОтчетыДавальцам0.ДокументРеализации
	//-- НЕ УТКА
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Функция СформироватьГиперссылкуКОформлению(Параметры) Экспорт
	...
	ТекстГиперссылки = НСтр("ru = 'Заявления о ввозе от покупателей';
							|en = 'Заявления о ввозе от покупателей'");
...
Функция НеобходимоОформитьЗаявлениеОВвозеТоваровПолученное(ДокументОтгрузки)
	...
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ОтчетДавальцу2_5") Тогда
		...
		Если РеквизитыДокументаОтгрузки.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтчетДавальцу2_5 Тогда
