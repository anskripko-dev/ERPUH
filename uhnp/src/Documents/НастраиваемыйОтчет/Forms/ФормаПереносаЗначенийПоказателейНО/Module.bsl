...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.СпособПереноса="Копирование" Тогда
		...
		Элементы.ГруппаСинтетика.Заголовок=Нстр("ru = 'Параметры копирования'");
		Элементы.ПроцентПереноса.Заголовок=Нстр("ru = 'Процент копирования'");
		Элементы.ЗначениеКПереносу.Заголовок=Нстр("ru = 'Значение для вставки'");
		
		Если ТаблицаАналитики.Количество()>0 Тогда
			
			Элементы["ПроцентПереноса_Раскрытие"].Заголовок=Нстр("ru = 'Процент копирования'");
			Элементы["ЗначениеКПереносу_Раскрытие"].Заголовок=Нстр("ru = 'Значение для вставки'");
			
		КонецЕсли;
		
		ЭтаФорма.Заголовок=Нстр("ru = 'Копирование значения показателя'");
				
	ИначеЕсли Параметры.СпособПереноса="Перенос" Тогда
		
		Элементы.ГруппаСинтетика.Заголовок=Нстр("ru = 'Параметры переноса'");
		Элементы.ПроцентПереноса.Заголовок=Нстр("ru = 'Процент переноса'");
		Элементы.ЗначениеКПереносу.Заголовок=Нстр("ru = 'Переносимое значение'");
		
		Если ТаблицаАналитики.Количество()>0 Тогда
			
			Элементы["ПроцентПереноса_Раскрытие"].Заголовок=Нстр("ru = 'Процент переноса'");
			Элементы["ЗначениеКПереносу_Раскрытие"].Заголовок=Нстр("ru = 'Переносимое значение'");
			
		КонецЕсли;
		
		ЭтаФорма.Заголовок=Нстр("ru = 'Перенос значения показателя'");
...
Процедура ЗаполнитьТаблицуРаскрытия(ИсходнаяТаблицаАналитики,ГруппаРаскрытияПриемник)
	...
	Для Каждого СтрКолонка ИЗ СтруктураПерекладки Цикл			
		...
		Если ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом() Тогда
			МодульУХ = ОбщегоНазначения.ОбщийМодуль("ДенежныеСредстваВстраиваниеУХКлиентСервер");
			ТипБанковскиеСчета = МодульУХ.ТипБанковскиеСчетаОрганизаций();
		Иначе
			// Это БМ или НМ.
			ИмяТипа 			= "СправочникСсылка.БанковскиеСчета";
			ТипБанковскиеСчета 	= Тип(ИмяТипа);
		КонецЕсли;
		...
		ИначеЕсли ОграничениеТипа.СодержитТип(ТипБанковскиеСчета) Тогда
