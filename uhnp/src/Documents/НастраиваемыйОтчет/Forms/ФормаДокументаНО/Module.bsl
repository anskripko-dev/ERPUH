...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Иначе
			...
			АдресПараметровИнициализации = Параметры.АдресПараметровИнициализации;
...
Процедура СвернутьДоВерсии(ВерсияДляСвертки)
	...
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом() Тогда
		...
		|ГДЕ
		|	ВерсииЗначенийПоказателей.Регистратор ССЫЛКА Документ.КорректировкаЗначенийПоказателей
...
Процедура ЗаполнитьАвтоматически(Команда)
	...
	ЗаполнитьДокумент("ПоПравилуОбработки");
	...
	Если ЗначениеЗаполнено(РежимЗаполненияВход) Тогда
		...
		Если РежимЗаполненияВход = "ПоследнийСпособ" Тогда
			...
			Иначе
				...
				РабочийОбъект.ЗаполнитьОтчетПоУмолчанию(СпособЗаполнения);
...
Процедура ПолеТабличногоДокументаМакетПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Иначе
		...
		Если НЕ ПустаяСтрока(СтруктураДанныеЯчейки.ТекстПоказатель) Тогда
			...
			ОткрытьФорму("Документ.НастраиваемыйОтчет.Форма.ФормаВыбораСпособаОбработкиРаскрытияНО",
...
Процедура ОткрытьФормуРаскрытияПоАналитике(ИмяОбласти, СтруктураОтбора = Неопределено,ДанныеСтрокиАналитики=Неопределено) Экспорт
	...
	ФормаАналитик = ПолучитьФорму("Документ.НастраиваемыйОтчет.Форма.ФормаОтображенияАналитикиНО",СтруктураПараметров,ЭтаФорма,Истина,);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия="ОбработатьУдалениеВерсий" И Параметр=Объект.Ссылка Тогда
...
Процедура РасшифровкаБланк(Команда)
		...
		Если ТипЗнч(ТекДанныеЯчейки)=Тип("Структура") И ТекДанныеЯчейки.Свойство("Показатель") Тогда
			...
			Если ТекДанныеЯчейки.Раскрытие Тогда
				...
				Для Каждого КлючИЗначение ИЗ ТекДанныеЯчейки Цикл
					...
					Если Лев(КлючИЗначение.Ключ,СтрДлина("Аналитика"))="Аналитика"
	...
	ОткрытьФорму("Документ.НастраиваемыйОтчет.Форма.РасшифровкаПоказателей",СтруктураПараметров,ЭтаФорма,,,,,РежимОткрытияОкнаФормы.Независимый);
...
Функция ЕстьПротоколируемыеОшибки()
	...
	РезультатФункции = Ложь;
	ОбъектРасчета = Документы.НастраиваемыйОтчет.ОбъектРасчетаДляПроверки(РеквизитФормыВЗначение("Объект"));
	ЕстьПротоколы = ПротоколируемыеСобытияУХ.ЕстьПротоколыПоОбъекту(Объект.Ссылка);
	ЕстьОшибкиОбъекта = ПротоколируемыеСобытияУХ.ЕстьОшибкиПоОбъекту(ОбъектРасчета);
...
Функция ОбработкаСтрокиРаскрытия(ТипОбработки, ВывестиСообщение = Истина)
	...
	ОткрытьФорму("Документ.НастраиваемыйОтчет.Форма.ФормаВводаАналитикиДляБланкаНО",ПараметрыОбработки);
...
Процедура ПанельБланкаУстановитьФильтр(Команда)
	...
	Если СтрНайти(ТекущаяОбласть, "_Показатель_") > 0 Тогда
		...
		ОткрытьФорму("Документ.НастраиваемыйОтчет.Форма.ФормаВводаУсловияНаЧисловойПоказательНО",
...
Процедура ПолеТабличногоДокументаМакетПриАктивизацииОбласти(Элемент)
	...
	Элемент.ТекущаяОбласть.Защита = НЕ ВозможноРедактирование;
...
Процедура СохранитьКак(Команда)
	...
	ОткрытьФорму("Документ.НастраиваемыйОтчет.Форма.СохранениеИПечатьДокумента",Новый Структура("ДокументДляВывода",ТабДок));
...
Процедура КомментироватьПоказатель(Команда)
	...
	ОткрытьФорму("Документ.НастраиваемыйОтчет.Форма.ФормаКомментариевНО",СтруктураПараметров);
...
Процедура ВызватьОтчетСценарныйАнализ(МассивПоказателей=Неопределено,Моделирование=Ложь) Экспорт
	...
	СтруктураПараметров.Вставить("Организация",Объект.Организация);
	СтруктураПараметров.Вставить("Проект",Объект.Проект);
	СтруктураПараметров.Вставить("ПериодС",Объект.ПериодОтчета);
	СтруктураПараметров.Вставить("ПериодПо",Объект.ПериодОтчета);
...
Процедура ИзменитьПоказатели(Команда)
	...
	ОткрытьФорму("Документ.НастраиваемыйОтчет.Форма.ФормаИзмененияПоказателейНО", 
					СтруктураПараметров, ЭтаФорма,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВосстановитьЗначениеИзБуфера(МассивПоказателей=Неопределено) Экспорт
	...
	ОткрытьФорму("Документ.НастраиваемыйОтчет.Форма.ФормаПереносаЗначенийПоказателейНО", СтруктураПараметров, ЭтаФорма,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);	
...
Процедура ПринятьКСогласованию_Подключаемый() Экспорт
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда
		Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ДействияСогласованиеУХКлиент");
		Модуль.ПринятьКСогласованию(ЭтаФорма, Объект.Ссылка);
	КонецЕсли;
КонецПроцедуры
...
Процедура ИсторияСогласования_Подключаемый() Экспорт
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда
		Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ДействияСогласованиеУХКлиент");
		Модуль.ИсторияСогласования(ЭтаФорма, Объект.Ссылка);
	КонецЕсли;
КонецПроцедуры
...
Процедура СогласоватьДокумент_Подключаемый() Экспорт
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда
		Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ДействияСогласованиеУХКлиент");
		Модуль.СогласоватьДокумент(ЭтаФорма);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОтменитьСогласование_Подключаемый() Экспорт
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда
		Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ДействияСогласованиеУХКлиент");
		Модуль.ОтменитьСогласование(ЭтаФорма);
	КонецЕсли;
КонецПроцедуры
...
Процедура МаршрутСогласования_Подключаемый() Экспорт
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПодсистемыУХ.ПроцессыИСогласование") Тогда
		Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ДействияСогласованиеУХКлиент");
		Модуль.МаршрутСогласования(ЭтаФорма, Объект.Ссылка);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроизводительностьЗаполнение(Команда)
	...
	АдресДанныхПроизводительности=ПолучитьАдресДанныхПроизводительности("ПоПравилуОбработки");
	ОткрытьФорму("Документ.НастраиваемыйОтчет.Форма.ФормаТаблицаПроизводительностиНО",Новый Структура("РабочийОбъектАдрес",АдресДанныхПроизводительности),ЭтаФорма);
...
Функция ПолучитьАдресДанныхПроизводительности(ВидАнализа)
	...
	Если ВидАнализа="ПоПравилуОбработки" Тогда
		
		РабочийОбъект.ЗаполнитьОтчетПоУмолчанию(Перечисления.СпособыФормированияОтчетов.ПоПравилуОбработки);
...
Процедура ЗаполнитьПрочимСпособом(Команда)
	...
	ОткрытьФорму("Документ.НастраиваемыйОтчет.Форма.ФормаВыбораСпособаЗаполненияНО", СтруктураПараметров, , , , , ОписаниеОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВыборСпособаЗаполнения_Завершение(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйСпособ = ПредопределенноеЗначение("Перечисление.СпособыФормированияОтчетов.ПоПравилуОбработки") Тогда
	...
	ИначеЕсли ВыбранныйСпособ = ПредопределенноеЗначение("Перечисление.СпособыФормированияОтчетов.Консолидация")
		Или ВыбранныйСпособ = ПредопределенноеЗначение("Перечисление.СпособыФормированияОтчетов.КонсолидацияПропорционально") Тогда
		
		ЗаполнитьДокумент("Консолидация");
		
	ИначеЕсли ВыбранныйСпособ = ПредопределенноеЗначение("Перечисление.СпособыФормированияОтчетов.СвернувПоАналитикам") Тогда
		
		ЗаполнитьДокумент("СвернувПоАналитикам");
		
	ИначеЕсли ВыбранныйСпособ = ПредопределенноеЗначение("Перечисление.СпособыФормированияОтчетов.СвернувПоПериоду") Тогда
		...
		Если ЗначениеЗаполнено(ПериодичностьСвертки) Тогда
			...
			ЗаполнитьДокумент("СвернувПоПериоду", ПериодичностьСвертки);
		Иначе
			ТекстСообщения = НСтр("ru = 'Периодичность не выбрана. Заполнение отчета отменено.'");
			ОбщегоНазначенияУХ.СообщитьОбОшибке(ТекстСообщения);
		КонецЕсли;
		
	ИначеЕсли ВыбранныйСпособ = ПредопределенноеЗначение("Перечисление.СпособыФормированияОтчетов.СвернувПоПроектам") Тогда
		
		ЗаполнитьДокумент("СвернувПоПроектам");
	...
	ИначеЕсли ВыбранныйСпособ = ПредопределенноеЗначение("Перечисление.СпособыФормированияОтчетов.Элиминация") Тогда        		
		
		ЗаполнитьДокумент("Элиминация");
...
Процедура ОткрытьНастройки(Команда)
	...
	ОткрытьФорму("Документ.НастраиваемыйОтчет.Форма.ФормаНастроекОтчетаНО", СтруктураПараметров, ЭтаФорма, , , , ОписаниеОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура ИнициализироватьФормуНовогоСервер() 
	...
	Если ЗначениеЗаполнено(АдресПараметровИнициализации) Тогда 
...
Функция ПрочитатьКлючевыеРеквизиты()
	...
	Возврат ПолучитьИзВременногоХранилища(АдресПараметровИнициализации);
