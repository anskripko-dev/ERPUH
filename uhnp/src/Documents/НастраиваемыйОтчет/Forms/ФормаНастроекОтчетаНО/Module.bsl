	...
	Если ЗначениеЗаполнено(ВидОтчета) Тогда
		...
		Если ЗначениеЗаполнено(Организация) И ЗначениеЗаполнено(ШаблонОтчета) Тогда
			...
			ТолькоПросмотр = ТолькоПросмотр 
				ИЛИ НЕ ПолныеПраваУХ.ПроверитьВозможностьЗаписиПоБланкуОтчета(ШаблонОтчета, Организация, , , , Истина);
			
		КонецЕсли;
		
	КонецЕсли;
	
	Элементы.Периодичность.Доступность = НЕ ТолькоПросмотр;
...
Процедура ОбновитьРеквизитыПоРегламенту()
	...
	Если Не ЗначениеЗаполнено(ШаблонОтчета) Тогда
		Если ЗначениеЗаполнено(СтруктураРеквизитов.БланкДляОтображения) Тогда
			ШаблонОтчета 		= СтруктураРеквизитов.БланкДляОтображения;
