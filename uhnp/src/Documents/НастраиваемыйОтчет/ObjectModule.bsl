...
// Переменные для расчета показателей
Перем АдресИсходныхНастроек Экспорт;
Перем АдресКэшаДанных Экспорт;
Перем АдресТаблицыСобытий Экспорт;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	// Проверка заполнения ключевой аналитики составного типа
	КлючевыеАналитики = УправлениеОтчетамиУХ.КлючевыеАналитикиСоставногоТипа(ВидОтчета);
	Для Каждого Аналитика Из КлючевыеАналитики Цикл
		
		Если ПроверяемыеРеквизиты.Найти(Аналитика.Ключ) = Неопределено Тогда
			ПроверяемыеРеквизиты.Добавить(Аналитика.Ключ);
		КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("СправочникСсылка.ВидыОтчетов") Тогда
		...
		ПараметрыНастроек = УправлениеОтчетамиУХ.ПараметрыНастроекОбработкиШаблонов(ЭтотОбъект);
		ШаблонОтчета = УправлениеОтчетамиУХ.ЗначениеНастройкиОбработкиШаблонов(ПараметрыНастроек, "БланкДляОтображения");
		СпособВывода = "Бланк";
		
	КонецЕсли;
	
	УправлениеОтчетамиУХ.ЗаполнитьПараметрыОтображения(ЕдиницаИзмерения, УровеньТочности, ШаблонОтчета);

	Если ЗначениеЗаполнено(ВидОтчета) И ВидОтчета.ПараметрыОтчета.Количество()>0 Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ ПолныеПраваУХ.ПроверитьВозможностьЗаписиОтчета(ВидОтчета, Организация) Тогда
		ТекстСообщения = НСтр("ru = 'Недостаточно прав для записи документа.
	...
	Если ЗначениеЗаполнено(Ссылка) И Ссылка.ПометкаУдаления <> ПометкаУдаления Тогда
		...
		Если ЕстьКлючевойСрез() И Состояние <> Перечисления.СостоянияОтчетов.Утвержден И ПометкаУдаления Тогда
			ЭтотОбъект.НеВноситьИзменения = Ложь;
			МодульСводнаяТаблицаУХ = ОбщегоНазначения.ОбщийМодуль("СводнаяТаблицаУХ");
			МодульСводнаяТаблицаУХ.УстановитьСтатусЭкземпляра(Ссылка, Перечисления.СостоянияОтчетов.Запланирован, 0);
		КонецЕсли;
		...
		Если ОбрабатыватьПометкуУдаленияОписанияВерсий Тогда
			...
			Запрос.Текст =
				...
				|ГДЕ
				|	ВерсииЗначенийПоказателей.ПометкаУдаления = &ПометкаУдаления
		...
		Если ПометкаУдаления Тогда 
			...
			// Снятие пометки обрабатываем, только при записи данных в базу
		...
		// Было установлена/снята пометка удаления с отчета. Никаких изменений значений показателей и/или реквизитов отчета не было
	...
	Если НЕ ЗначениеЗаполнено(ОсновнаяВалюта) Тогда
		...
		Если ЗначениеЗаполнено(ВерсияРегламента) Тогда
			...
			СтрокиВалюты = ТаблицаВалютОтчетности.НайтиСтроки(Новый Структура("Организация, ВидОтчета", Организация, ВидОтчета));
			Если СтрокиВалюты.Количество() = 0 Тогда
				СтрокиВалюты = ТаблицаВалютОтчетности.НайтиСтроки(Новый Структура("Организация, ВидОтчета", Организация, Справочники.ВидыОтчетов.ПустаяСсылка()));
			КонецЕсли;
	...
	Если ОбщегоНазначенияПовтИспУХ.ИспользуетсяМногопериоднаяОтчетность() Тогда
		...
		// Обработаем изменение ключевых срезов
		Если ИзмененКлючевойРеквизит = Истина Тогда
			
			ОбработкаРасчета = РасширениеБизнесЛогикиУХ.ПолучитьОбработкуЗаполненияДанных();
			ОбработкаРасчета.УникальныйИдентификаторСеанса = ИдентификаторСеанса;
			ОбработкаРасчета.АдресКэшаДанных = АдресКэшаДанных;
			Если Ссылка.Пустая() Тогда
				СсылкаНового = Документы.НастраиваемыйОтчет.ПолучитьСсылку(Новый УникальныйИдентификатор);
				УстановитьСсылкуНового(СсылкаНового);
				ОбработкаРасчета.Регистратор = СсылкаНового;
			Иначе
				ОбработкаРасчета.Регистратор = Ссылка;
			КонецЕсли;
			
			Если Не ОбработкаРасчета.ОбработатьИзменениеКлючевогоСрезаЗначенийПоказателейОтчетов(ЭтотОбъект, 
				?(ЗначениеЗаполнено(АдресИсходныхНастроек), АдресИсходныхНастроек, ОбработкаРасчета.Регистратор)) Тогда
				ОбработкаРасчета.СообщитьОСобытиях();
				Отказ = Истина;
			КонецЕсли;
			
			АдресТаблицыСобытий = ОбработкаРасчета.АдресТаблицыСобытий;
			
			Если ЗначениеЗаполнено(АдресИсходныхНастроек) Тогда
				ДанныеИсточника = ПолучитьИзВременногоХранилища(АдресИсходныхНастроек);
				РегистрыСведений.ЗначенияСтатусовКлючевыхСрезов.УдалитьЗаписиКлючевогоСреза(ДанныеИсточника, Отказ);
			КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	Если ОбщегоНазначенияПовтИспУХ.ИспользуетсяМногопериоднаяОтчетность() Тогда
		...
		// Записываем лог измененных показателей
		Если Не ОбработкаРасчета.ЗаписатьЛогИзмененныхПоказателей() Тогда
			ОбработкаРасчета.СообщитьОСобытиях();
			Отказ = Истина;
		КонецЕсли;
	...
	Иначе
		...
		Если ОбменДанными.Загрузка ИЛИ мПоказатели=Неопределено Тогда 
			// Проверки и запись движений в регистры значений показателей не осуществляются.
			Возврат;
		КонецЕсли;
		...
		Если ФормированиеДвиженийПриЗаписи Тогда
			...
			ОбъектРасчета.Вставить("АктивностьДвижений", УправлениеОтчетамиУХ.ПолучитьПризнакАктивностиДвижений(ЭтотОбъект));
	...
	ИзмененоСостояние = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ДополнительныеСвойства, "ИзмененоСостояние", ИзмененоСостояние);
	Если ЕстьКлючевойСрез() И НЕ ИзмененоСостояние И НЕ ПометкаУдаления Тогда
		ИндексБлокировки = ?(НеВноситьИзменения, 1, 0);
		МодульСводнаяТаблицаУХ = ОбщегоНазначения.ОбщийМодуль("СводнаяТаблицаУХ");
		МодульСводнаяТаблицаУХ.УстановитьСтатусЭкземпляра(Ссылка, СтатусОбъекта, ИндексБлокировки);
	КонецЕсли;
...
Функция ЕстьКлючевойСрез()
	
	Возврат ОбщегоНазначенияПовтИспУХ.ИспользуетсяМногопериоднаяОтчетность()
		И НЕ РасширениеПроцессыИСогласованиеУХ.СогласованиеЧерезРакурс(ВидОтчета)
...
Процедура ОбновитьОбъектХранимыйФайл()
	...
	Если НЕ (СпособФормированияОтчета=Перечисления.СпособыФормированияОтчетов.ПоПравилуОбработки
...
Функция ЕстьИзмененияДополнительныхВалют()
	...
	Возврат НЕ ОбщегоНазначенияУХ.СравнитьТаблицыЗначений(
		ДополнительныеВалюты.Выгрузить(),
...
Процедура ОчиститьВсеВнешнееОбращение() Экспорт
	...
	УдалитьДатаИзменения = Неопределено;
...
Функция ЗаполнитьПоПараметрическойНастройке() Экспорт
	...
	Если ЗаполнениеВыполнено Тогда
		...
		СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.ПоПравилуОбработки;
	Иначе
...
Функция ЗапуститьМодульЗаполнения(Режим) Экспорт
	...
	Если Режим = Перечисления.СпособыФормированияОтчетов.РучноеЗаполнение Тогда
		Возврат Истина;
	ИначеЕсли Режим = Перечисления.СпособыФормированияОтчетов.СвернувПоПериоду Тогда
		ЗаполнениеВыполнено = ПроцедураЗаполненияПоПериодам();
	ИначеЕсли Режим = Перечисления.СпособыФормированияОтчетов.СвернувПоАналитикам Тогда
		ЗаполнениеВыполнено = ПроцедураЗаполненияПоАналитикамОтчетов();
	ИначеЕсли Режим = Перечисления.СпособыФормированияОтчетов.СвернувПоПроектам Тогда
		ЗаполнениеВыполнено = ПроцедураЗаполненияПоПроектам();
	ИначеЕсли Режим = Перечисления.СпособыФормированияОтчетов.СвернувПоОрганизациямПроекта Тогда
		ЗаполнениеВыполнено = ПроцедураЗаполненияПоОрганизациямПроекта();
	ИначеЕсли Режим = Перечисления.СпособыФормированияОтчетов.Консолидация
		ИЛИ Режим = Перечисления.СпособыФормированияОтчетов.КонсолидацияПропорционально Тогда
	...
	ИначеЕсли Режим = Перечисления.СпособыФормированияОтчетов.Импорт И ЗначениеЗаполнено(ИспользуемаяИБ) И ИспользуемаяИБ.ТипБД.Предопределенный Тогда
		ЗаполнениеВыполнено = ИмпортироватьДанные(ФайлИмпорта, УдалитьДатаИзменения,ИмяЛиста) = 1;
	ИначеЕсли Режим = Перечисления.СпособыФормированияОтчетов.Элиминация Тогда
		
		Документы.НастраиваемыйОтчет.ВыполнитьПростуюЭлиминацию(ЭтотОбъект);
...
Функция ПроцедураЗаполненияДляКонсолидирующихОрганизаций()
	...
	СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.Консолидация;
	Возврат Истина;
...
Функция ПроцедураЗаполненияПоПериодам() Экспорт
	...
	СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.СвернувПоПериоду;
...
Функция ПроцедураЗаполненияПоАналитикамОтчетов() Экспорт
	...
	Если МаксАналитикОтчета>0 Тогда
		...
		Если НЕ ПустаяСтрока(ТекстАналитики) Тогда
			...
			Запрос.Текст=Запрос.Текст+"
			|ГДЕ
			|"+Сред(ТекстАналитики,СтрДлина("И")+2);
	...
	СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.СвернувПоАналитикам;
...
Функция ПроцедураЗаполненияПоПроектам() Экспорт
	...
	СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.СвернувПоПроектам;
...
Функция ПроцедураЗаполненияПоОрганизациямПроекта() Экспорт
	...
	СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.СвернувПоОрганизациямПроекта;
...
Функция ПолучитьРасшифровкуСверткиПоПериодам(МассивПоказателей, ОтборПоАналитикам = Неопределено) Экспорт
	...
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом() Тогда
		...
		Запрос.Текст=Запрос.Текст+"	
		|	ВЫБОР
		|		КОГДА ЗначенияПоказателейОтчетов.Регистратор ССЫЛКА Документ.КорректировкаЗначенийПоказателей ТОГДА
		|			ЗначенияПоказателейОтчетов.Значение
		|		ИНАЧЕ 0
	...
	Если НЕ ПустаяСтрока(ТекстАналитики) Тогда
		...
		Запрос.Текст=Запрос.Текст+"
		|ГДЕ"+Сред(ТекстАналитики,СтрДлина("И")+2);
...
Функция ПолучитьТекстЗапросаБезУчетаПриобретений_Синтетика(ТекстВложенногоЗапроса, НомераТаблиц)
	...
	|			ТОГДА 1
	|		КОГДА ТаблицаПериметр.ВидОтношенияКГруппе = ЗНАЧЕНИЕ(Перечисление.ВидыОтношенийКГруппе.СовместныеОперации)
	|			ТОГДА ТаблицаПериметр.ДоляВладения / 100
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК РасчетнаяДоля,
	|	ТаблицаПериметр.ВидОтношенияКГруппе КАК МетодКонсолидации,
	|	СОКРП(ВЫРАЗИТЬ(ТаблицаПериметр.ОтношениеКГруппеРасшифровка КАК СТРОКА(150))) КАК ОтношениеКГруппеРасшифровка,
	...
	|	ДолиОрганизаций.ОтношениеКГруппеРасшифровка,
...
Функция ПолучитьТекстЗапросаБезУчетаПриобретений_Отбор(Запрос, ТекстВложенногоЗапроса, ОтборПоАналитикам)
	...
	|	ЗначенияПоказателейОтчетов.ЭкземплярОтчета КАК Регистратор,
	|	ЗначенияПоказателейОтчетов.Активность КАК Активность,
	|	СУММА(ЗначенияПоказателейОтчетов.Значение * ДолиОрганизаций.РасчетнаяДоля) КАК ИтоговоеЗначение,
	|   СУММА(ЗначенияПоказателейОтчетов.Значение) КАК Значение,
	|	ДолиОрганизаций.Доля КАК Доля,
	|	ДолиОрганизаций.ОтношениеКГруппеРасшифровка КАК ОтношениеКГруппеРасшифровка,
	|	ДолиОрганизаций.МетодКонсолидации
	|ИЗ 
	|	(" + ТекстВложенногоЗапроса + ") КАК ЗначенияПоказателейОтчетов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втДолиОрганизаций КАК ДолиОрганизаций
...
Функция ПолучитьРасшифровкуКонсолидированныхПоказателей(РежимРасшифровки = Ложь, ОтборПоАналитикам = Неопределено) Экспорт
	...
	ПоляВыбора = Новый Структура("ДоляВладения, ВидОтношенияКГруппе, ОтношениеКГруппеРасшифровка",
		"ЭффективнаяДоля", "ВидОтношенияКГруппе", "ОтношениеКГруппеРасшифровка");
	ТаблицаПериметр = УправлениеРабочимиПроцессамиУХ.ПолучитьДанныеРегламентаОтчета(ЭтотОбъект, ПоляВыбора);
...
Функция ПолучитьРасшифровкуПоказателяСвернутогоПоАналитике(МассивПоказателей, ОтборПоАналитикам = Неопределено)
	...
	Если Не ПустаяСтрока(ТекстОтборАналитик) Тогда
		ЗапросПоЗначениюИКорректировкеИтоговогоПоказателя = ЗапросПоЗначениюИКорректировкеИтоговогоПоказателя + "
		|ГДЕ
		|"+Сред(ТекстОтборАналитик,СтрДлина("И")+2);
	КонецЕсли;
...
Функция ЗагрузитьДанныеИзМакета(ТаблДок, ТихийРежим, ПараметрыСохраненияПослеУдачногоИмпорта = Неопределено, СоответствиеИмпортируемыхОбластей = Неопределено) Экспорт
	...
	Если БланкИмпорта.Пустая() Тогда
		ПараметрыНастроек = УправлениеОтчетамиУХ.ПараметрыНастроекОбработкиШаблонов(ЭтотОбъект);
		БланкИмпорта = УправлениеОтчетамиУХ.ЗначениеНастройкиОбработкиШаблонов(ПараметрыНастроек, "БланкДляИмпорта");
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		МодульИмпорта=ОбщегоНазначения.ОбщийМодуль("ИмпортДанныхБланкУХ");
		
		ТабГруппыРаскрытия = МодульИмпорта.ТрансформироватьВнешниеДанные(ДанныеОбъекта,ТаблицаЗагружаемыхЗначений,ПравилаИспользованияПолей);
...
Функция ИмпортироватьДанные(Знач Источник = Неопределено, ДатаФайла = Неопределено, ИмяЛиста = Неопределено, ПарольНаЛист = Неопределено,ЭтаФорма = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Источник) = Тип("СправочникСсылка.ХранимыеФайлыОрганизаций") Тогда
		...
		УдалитьДатаИзменения = ДатаФайла;
		...
		Если СтрНайти(ВРЕГ(Источник.РасширениеФайла),"XLS")>0 Тогда // на входе таблица Microsoft Excel
			...
			АдресХранилищаФайла = РаботаСФайламиУХ.ПоместитьФайлВоВременноеХранилище(ИмяФайла);
...
Функция ПроверитьВозможностьИзмененияСостояния(НовоеСостояние)
	...
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом() Тогда
		...
		Отказ = НЕ РасширениеПроцессыИСогласованиеУХ.ВозможноУстановитьСостояние(ЭтотОбъект, НовоеСостояние, ШапкаОшибки);
		Если Отказ Тогда
			Возврат Ложь;
		КонецЕсли;
		...
		Если НовоеСостояние = Перечисления.СостоянияОтчетов.Утвержден
			...
			И (НЕ РасширениеПроцессыИСогласованиеУХ.ПроверитьВозможностьУтвержденияОтчета(ВидОтчета, Организация, ТекущийПользователь)
...
Процедура СвернутьВерсии(НачальнаяВерсия, КонечнаяВерсия) Экспорт
	...
	Документы.НастраиваемыйОтчет.СвернутьВерсииПоРеквизитам(УправлениеПериодом,
													ВидОтчета,
...
Процедура ИнициализироватьКонтекст(тПериодОтчета = Неопределено) Экспорт
	...
	ДанныеРегламента = УправлениеОтчетамиУХ.ДанныеРегламентаПодготовкиОтчетности(УправлениеПериодом);
	АвтоматическийПересчетЗависимыхПоказателей = ДанныеРегламента.АвтоматическиПересчитыватьЗависимыеПоказатели;
	ПодставлятьЗначенияПоУмолчаниюВместоПустых = ДанныеРегламента.ПодставлятьЗначенияАналитикПоУмолчанию;
...
Функция Проверить(ПротоколироватьОшибки=Ложь,РежимВывода="",СтруктураВозврата=Неопределено) Экспорт
	...
	ДанныеОбъекта.Вставить("ВидОтчета", ВидОтчета);
	...
	ДанныеОбъекта.Вставить("ПериодОкончания", ПериодОкончания);
	
	МаксАналитикОтчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидОтчета, "КоличествоАналитик");
	ДанныеОбъекта.Вставить("ЧислоДопАналитик", МаксАналитикОтчета);
	Для Инд = 1 По МаксАналитикОтчета Цикл
		ДанныеОбъекта.Вставить("Аналитика" + Инд, ЭтотОбъект["Аналитика" + Инд]);
	КонецЦикла;
	
	АнализЧувствительности = (АнализЧувствительности ИЛИ ПометкаУдаления);
	ДанныеОбъекта.Вставить("АнализЧувствительности", АнализЧувствительности);
	
	ДанныеОбъекта.Вставить("ПравилоПроверки",
		Документы.НастраиваемыйОтчет.ЗначениеНастройкиОбработкиДокумента(ЭтотОбъект, "ПравилоПроверки"));
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.МногопериоднаяОтчетность") Тогда
		ОбщегоНазначения.ВыполнитьМетодКонфигурации(
...
Функция ЗаполнитьОтчетПоУмолчанию(Режим = Неопределено) Экспорт
	
	Если ЗначениеЗаполнено(Режим) Тогда
		Возврат ЗаполнитьОтчет(Режим);
	КонецЕсли;
	
	ПараметрыНастроек = УправлениеОтчетамиУХ.ПараметрыНастроекОбработкиШаблонов(ЭтотОбъект);
	ЗначенияНастроек = УправлениеОтчетамиУХ.НастройкиОбработкиШаблонов(ПараметрыНастроек);
		
	ТекущийРежим = ЗначенияНастроек.СпособФормированияОтчета;
	...
	Если НЕ ЗначениеЗаполнено(ТекущийРежим) Тогда
		...
		Если Результат.Следующий() Тогда
			...
			ТекущийРежим=Перечисления.СпособыФормированияОтчетов.ПоПравилуОбработки;
	...
	ИначеЕсли ТекущийРежим = Перечисления.СпособыФормированияОтчетов.СвернувПоПериоду Тогда
		
		ПериодичностьСвертывания = ЗначенияНастроек.ПериодичностьСвертывания;
		
	ИначеЕсли ТекущийРежим = Перечисления.СпособыФормированияОтчетов.Импорт Тогда
		
		ИспользуемаяИБ     = ЗначенияНастроек.ВнешняяИнформационнаяБаза;
		БланкИмпорта       = ЗначенияНастроек.БланкДляИмпорта;
		...
		Если НЕ (ТипБД=Справочники.ТипыБазДанных.ОбменТаблДок ИЛИ ТипБД.ВерсияПлатформы=Перечисления.ПлатформыВнешнихИнформационныхБаз.ADO) Тогда
			...
			УдалитьДатаИзменения      = ТекущаяДата();
...
Функция ЗаполнитьМногопериодныйОтчет(Режим)	
	...
	Иначе
		...
		Если ОбработкаОбъект.Метаданные().Имя = "РасчетИЗаписьПоказателей"
			И ЗначениеЗаполнено(ОбработкаОбъект.АдресТаблицыСобытий) Тогда
			ТаблицаСобытийЗагрузки = ПолучитьИзВременногоХранилища(ОбработкаОбъект.АдресТаблицыСобытий);
		Иначе	
			ЗавершитьПротоколЗагрузки();
		КонецЕсли;
...
Функция ЗаполнитьОтчет(Режим)
	...
	Попытка
		...
		Если Не (Режим = Перечисления.СпособыФормированияОтчетов.ПоПравилуОбработки ИЛИ РежимКорректировки) Тогда
...
Функция ЕстьЗаполненныеЗначения() Экспорт
	...
	Возврат РасширениеБизнесЛогикиУХ.ЕстьДанныеВЛогеИзмененныхПоказателей(ЭтотОбъект.ИдентификаторСеанса);
...
Процедура ПроверитьПередЗаписью(Отказ, ШапкаОшибки) Экспорт
	...
	Если Отказ или Не ПолныеПраваУХ.ПроверитьВозможностьЗаписиДокументовПоПериодуСценария(ЭтотОбъект, Отказ, ШапкаОшибки) Тогда
		Возврат;
	КонецЕсли;
	...
	ПравилоОбработкиНастройка =
		Документы.НастраиваемыйОтчет.ЗначениеНастройкиОбработкиДокумента(ЭтотОбъект, "ПравилоОбработки");
	Если (Состояние = Перечисления.СостоянияОтчетов.Утвержден ИЛИ Состояние = Перечисления.СостоянияОтчетов.Подготовлен)
		И НЕ УправлениеОтчетамиУХ.ПроверитьНаличиеУтвержденныхПравилИШаблонов(
			ПравилоОбработкиНастройка, ШаблонОтчета, ШапкаОшибки, Состояние) Тогда
...
Функция ВычислитьПараметрПоФункции(СтрокаПараметр) Экспорт
	...
	Попытка
		...
		// Временное решение для совместимости в расширенным алгоритмом расчета показателей
		ОбъектРасчета = Новый Структура;
		ОбъектРасчета.Вставить("ВидОтчета", ЭтотОбъект.ВидОтчета);
		ОбъектРасчета.Вставить("ПравилоОбработки", ЭтотОбъект.ПравилоОбработки);
		ОбъектРасчета.Вставить("Организация", ЭтотОбъект.Организация);
		ОбъектРасчета.Вставить("Сценарий", ЭтотОбъект.Сценарий);
		ОбъектРасчета.Вставить("Проект", ЭтотОбъект.Проект);
		ОбъектРасчета.Вставить("ОсновнаяВалюта", ЭтотОбъект.ОсновнаяВалюта);
		ОбъектРасчета.Вставить("Регистратор", ЭтотОбъект.Регистратор);
		ОбъектРасчета.Вставить("СпособФормированияОтчета", ЭтотОбъект.СпособФормированияОтчета);
		ОбъектРасчета.Вставить("ИспользуемаяИБ", ЭтотОбъект.ИспользуемаяИБ);
		ОбъектРасчета.Вставить("ДополнительныеВалюты", ЭтотОбъект.ДополнительныеВалюты);
		ОбъектРасчета.Вставить("Периодичность", ОбщегоНазначенияУХ.ЗначениеРеквизитаОбъекта(ЭтотОбъект.ПериодОтчета,"Периодичность"));
		ОбъектРасчета.Вставить("ПериодОтчета", ЭтотОбъект.ПериодОтчета);
		ОбъектРасчета.Вставить("Аналитика1", ЭтотОбъект.Аналитика1);
		ОбъектРасчета.Вставить("Аналитика2", ЭтотОбъект.Аналитика2);
		ОбъектРасчета.Вставить("Аналитика3", ЭтотОбъект.Аналитика3);
		ОбъектРасчета.Вставить("Аналитика4", ЭтотОбъект.Аналитика4);
		ОбъектРасчета.Вставить("Аналитика5", ЭтотОбъект.Аналитика5);
		ОбъектРасчета.Вставить("Аналитика6", ЭтотОбъект.Аналитика6);
		...
		ОбъектРасчета.Вставить("УправлениеПериодом", ДанныеПериода.УправлениеПериодом);
		ОбъектРасчета.Вставить("МассивПериодов", ПериодыУХ.ПолучитьПодчиненныеПериоды(ДанныеПериода.ПериодСценария, ДанныеПериода.ПериодОкончания, ОбъектРасчета.Периодичность));	
		ТекстВычисленияПараметра = СтрЗаменить(ТекстВычисленияПараметра, "Результат", "ЗначениеПараметра");
		
		CalcStructure = ОбъектРасчета;
	...
	Исключение
		...
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НаименованиеБД%", Строка(СокрЛП(СтрокаПараметр.ПолеБД)));
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%", ОписаниеОшибки());
		ОбщегоНазначенияУХ.СообщитьОбОшибке(ТекстСообщения, , , СтатусСообщения.Важное);
	КонецПопытки;
...
Функция ВернутьТабличныйДокументРасшифровки(АналитикаВверху, ОтборПоАналитикам = Неопределено, МассивПоказателей) Экспорт
	...
	Если СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.Консолидация
		ИЛИ СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.КонсолидацияПропорционально Тогда
	...
	Если СпособФормированияОтчета=Перечисления.СпособыФормированияОтчетов.Элиминация Тогда
		...
		Документы.НастраиваемыйОтчет.ВыполнитьПростуюЭлиминацию(ОбъектРасчета, Истина, ОтборПоАналитикам);
	...
	Если СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.СвернувПоАналитикам Тогда
	...
	Если СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.СвернувПоПроектам Тогда
	...
	Если СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.СвернувПоОрганизациямПроекта Тогда
	...
	Если СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.СвернувПоПериоду Тогда
...
Процедура ОтобразитьРасшифровкуКонсолидированныхПоказателей(ТаблДок, АналитикаВверху, Сжатый = Истина, ОтборПоАналитикам = Неопределено, МассивПоказателей)
	...
	КолонкиКопирования = "Организация, Доля, Активность, ОтношениеКГруппеРасшифровка";
	ТаблОрганизаций = мТабРасшифровкиПоказателей.Скопировать(, КолонкиКопирования);
	
	Если Не Константы.ПроверятьУтверждениеДанных.Получить() Тогда
		ТаблОрганизаций.ЗаполнитьЗначения(Истина, "Активность");
	КонецЕсли;
	
	ТаблОрганизаций.Свернуть(КолонкиКопирования);
	...
	ШаблонНаименования = Нстр("ru = '
		| Отношение к группе: %ОтношениеКГруппеРасшифровка%
		| Эффективная доля: %Доля%%'");
	...
	Для Каждого ТекОрганизация Из ТаблОрганизаций Цикл
		...
		ДополнениеНаименования = СтрЗаменить(ШаблонНаименования,
			"%ОтношениеКГруппеРасшифровка%", ТекОрганизация.ОтношениеКГруппеРасшифровка);
...
Процедура ОтобразитьРасшифровкуПоказателейСвернутыхПоПериоду(ТаблДок, АналитикаВверху, ОтборПоАналитикам = Неопределено, МассивПоказателей)
	...
	ОбластьЗаголовокСтроки  = Макет.ПолучитьОбласть("ЗаголовокСтроки" + ?(АналитикаВверху, "", "_Верх"));
	ОбластьЗаголовокКолонки = Макет.ПолучитьОбласть("ЗаголовокКолонки" + ?(АналитикаВверху, "", "_Верх"));
	ОбластьДанныеСтроки     = Макет.ПолучитьОбласть("ДанныеСтроки" + ?(АналитикаВверху, "", "_Верх"));
	ОбластьДанныеКолонки    = Макет.ПолучитьОбласть("ДанныеКолонки" + ?(АналитикаВверху, "", "_Верх"));
...
Процедура ОчиститьмПоказателиРаскрытия()
	...
	Для Каждого Строка Из мПоказателиРаскрытия Цикл
		...
		Если ЗначениеЗаполнено(ТекСтрокамПоказатели) Тогда
			ЗаписьВЛоге = ЛогИзмененийРаскрытийПоказателей.Добавить();
...
Процедура СформироватьТаблицуРаскрытияДерево()
	...
	ТекстВТ=ТекстВТ+"
	|"+Сред(ТекстОбъединениеРаскрытие,СтрДлина("ОБЪЕДИНИТЬ ВСЕ") + 2);
...
Процедура ОбновитьКонтекстРаскрытия() Экспорт
	...
	Для Каждого СтрРаскрытие ИЗ ТаблицаРаскрытийСтрок Цикл
		...
		Если Не ТекстДанныеРаскрытие="" Тогда
			...
			ТекстЗапросаОбщий=ТекстЗапросаОбщий+ТекстДанныеРаскрытие+Сред(ТекстОбъединениеРаскрытие,СтрДлина("ОБЪЕДИНИТЬ ВСЕ") + 2)+"
			|;";
			
		Иначе
			
			ТекстЗапросаОбщий=ТекстЗапросаОбщий+Сред(ТекстОбъединениеРаскрытие,СтрДлина("ОБЪЕДИНИТЬ ВСЕ") + 2)+"
			|;";
...
Процедура ОбновитьЗначенияВТаблице(ЗапросИтоги)
	...
	Если СпособВывода="ДеревоГруппировка" ИЛИ СпособВывода="ДеревоГруппировкаСИерархией" Тогда
		...
		|ПО" +Сред(ТекстСвязи,СтрДлина("И") + 2);
...
Процедура ПолучитьДанныеАналитики(СтрокаРаскрытия,СтрокаДерева,СтрокаКоординат,МассивПропускаемыхАналитик=Неопределено,ДобавленоСтрок=0) Экспорт
	...
	Если Не ТекстДанныеРаскрытие="" Тогда
		...
		ЗапросДанныеРаскрытия.Текст=ТекстДанныеРаскрытие+Сред(ТекстОбъединениеРаскрытие,СтрДлина("ОБЪЕДИНИТЬ ВСЕ") + 2)+"
		|;";
		
	Иначе
		
		ЗапросДанныеРаскрытия.Текст=ЗапросДанныеРаскрытия.Текст+Сред(ТекстОбъединениеРаскрытие,СтрДлина("ОБЪЕДИНИТЬ ВСЕ") + 2)+"
		|;";
...
Функция ПодготовитьСтруктуруПеременныхДляРасчета() Экспорт
	...
	ДанныеОбИзмеенении = РегистрыСведений.СведенияОбИзмененииДокументов.ПолучитьСведения(Ссылка);
	СтруктураПеременных.Вставить("ПоследнийРедактировавший",ДанныеОбИзмеенении.ПоследнийРедактировавший);
	СтруктураПеременных.Вставить("ДатаИзменения",ДанныеОбИзмеенении.ДатаИзменения);
