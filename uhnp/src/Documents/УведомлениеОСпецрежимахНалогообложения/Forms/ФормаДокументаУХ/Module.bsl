...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.ИмяОтчета) Тогда
			Сообщить("Автоматческое заполнение формы возможно только при создании из Реестра уведомлений об иностранных компаниях.
					|Для ручного заполнения необходимо установить константу Отключить автоматическое заполнение уведомлений ИК");
			Отказ = Истина;
			Возврат;
		КонецЕсли;
...
Процедура ВывестиДеревоСтраниц(СтрокиДерева, ИмяОтчета)
	...
	ПодменаФормы.Вставить("АдресВХранилищеЭкранныеФормы", АдресВХранилищеЭкранныеФормы);
	ПодменаФормы.Вставить("УникальныйИдентификатор", ЭтаФорма.УникальныйИдентификатор);
	Для каждого Строка Из СтрокиДерева Цикл
		Если Не ПустаяСтрока(Строка.ИмяМакета) Тогда
			//Макет = Отчеты[ИмяОтчета].ПолучитьМакет(Строка.ИмяМакета);
			Макет = УведомлениеОСпецрежимахНалогообложения.ПолучитьМакетТабличногоДокумента(ПодменаФормы, Строка.ИмяМакета);
...
Процедура ЗаполнитьТаблицы()
	...
	Если Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ФормаУ_ИО И Объект.ИмяФормы = "Форма2015_1" Тогда
		Сообщить("Форма 2015 года не доступна для автозаполнения");
		Возврат;
	...
	ИначеЕсли Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ФормаКИК Тогда
		...
		СтруктураФормы.Вставить("Объект", Новый Структура("ИмяОтчета, ИмяФормы", Объект.ИмяОтчета, Объект.ИмяФормы));
		СтруктураФормы.Вставить("ИмяМакетаАрхиваТабличныхДокументов", ИмяМакетаАрхиваТабличныхДокументов);
		СтруктураФормы.Вставить("АдресВХранилищеЭкранныеФормы", ИмяМакетаАрхиваТабличныхДокументов);
		СтруктураФормы.Вставить("УникальныйИдентификатор", ЭтаФорма.УникальныйИдентификатор);
		УведомлениеОСпецрежимахНалогообложения.СформироватьСтруктуруДанныхУведомленияНовогоОбразца(СтруктураФормы);
		СтруктураФормы.Удалить("Объект");
		ЗаполнитьЗначенияСвойств(ЭтаФорма, СтруктураФормы, , "ИмяФормы, ДеревоСтраниц");
		ЗначениеВРеквизитФормы(ДанныеФормыВЗначение(СтруктураФормы.ДеревоСтраниц, Тип("ДеревоЗначений")), "ДеревоСтраниц");
...
Процедура ЗаполнитьФормуУведомления(Цепочки, СоответствиеИмен, ЭтоУведомлениеКИК, ИмяОбластиНомерЛистГ, ПереченьЛистов)
	...
	МассивКосвенныхОрганизаций = РезультатЗапроса[4].Выгрузить().ВыгрузитьКолонку("ОбъектИнвестированияПоследовательности");
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		//
		Если ВыборкаДетальныеЗаписи.ИностранныйНалоговыйРезидент И Объект.ИностранныеИнвестиции.НайтиСтроки(Новый Структура("Инвестиция", ВыборкаДетальныеЗаписи.Инвестиция)).Количество() = 0 Тогда
			Продолжить;
		ИначеЕсли МассивКосвенныхОрганизаций.Найти(ВыборкаДетальныеЗаписи.Инвестиция) = 0 Тогда
			Продолжить;
		КонецЕсли;
	...
	Пока ВыборкаИнвестиция.Следующий() Цикл
		...
		Пока ВыборкаПоследовательность.Следующий() Цикл
			...
			Если НЕ РазделНеЗаполнен Тогда
				...
				НоваяСтруктураРаздела.ДоляКосвПослИт = ВыборкаПоследовательность.ДоляКосвПослИт;
			...
			Иначе
				...
				ДанныеТекущегоРаздела.ДоляУчКосвИт = ВыборкаИнвестиция.ДоляКосвПослИт;
				ДанныеТекущегоРаздела.ДоляКосвПослИт = ВыборкаПоследовательность.ДоляКосвПослИт;
			КонецЕсли;
