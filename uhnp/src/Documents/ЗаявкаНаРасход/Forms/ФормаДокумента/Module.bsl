...
&НаКлиенте
Процедура ЖелательнаяДатаРасходаПриИзменении(Элемент)
...
Процедура ПриИзмененииДоговораНаСервере()
	...
	Если ТипЗнч(Объект.ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыКредитовИДепозитов") Тогда
		ЗаявкиНаОперации.ПриИзмененииДоговораКредитаДепозита(ЭтотОбъект, Объект.ДоговорКонтрагента);
...
Процедура ПриИзмененииКонтрагентаНаСервере()
	...
	Объект.ЭтоВнутригрупповоеПеремещение 	= РаботаСКонтрагентамиУХ.ЭтоВнутригрупповойКонтрагент(Объект.Контрагент);
...
Процедура УправлениеФормой()
	...
	ЭтоРасчетыПоКредитам		= Объект.ВидОперацииУХ = Справочники.ВидыОперацийУХ.РасходыПоКредитамИЗаймамКонтрагенты;
	...
	КонецЕсли;
	Элементы.ДоговорКонтрагента.ОграничениеТипа 		= Новый ОписаниеТипов(ТипОграничения);	
	...
	ГрафикДоговораИспользуется = ЗначениеЗаполнено(Объект.ДоговорКонтрагента) 
							И (ЭтоРасчетыПоКредитам ИЛИ 
							(Не Объект.ДоговорКонтрагента.ГрафикДоговора = Перечисления.ГрафикиДоговора.НеИспользовать));
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	СтавкаНДСПоДоговору = ДоговорыКонтрагентовВстраиваниеУХ.ПолучитьСтавкуНДСПоДоговору(Объект.ДоговорКонтрагента, Объект.ЖелательнаяДатаРасхода);
...
Процедура ОбработатьПодборОбъектов(АдресОбъектовРасчетовВХранилище)
	...
	Для каждого СтрокаЗаявки Из ТаблицаОбъектов Цикл
		...
		СтрокаТЧ.СтавкаНДС = ?(ИспользоватьСуммуСтавкуНДС, СтавкаНДСПоДоговору, 
