...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если СтрокиБезВладельца.Количество() > 0 Тогда
		...
		|en = 'Detailed records with an empty ""Payment type"" field are found.
		|Go to the ""Detailed records"" table and disable the filter by AR/AP object'");
		...
		Для каждого СтрокаТабличнойЧасти Из СтрокиБезВладельца Цикл
			ТекстСообщения = НСтр("ru = 'В строке %1 не заполнено поле ""Тип расчетов"".';
									|en = '""Payment type"" is required in line %1'");
