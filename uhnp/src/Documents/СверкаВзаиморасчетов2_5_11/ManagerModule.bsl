	...
	Если НастройкиПечати.ДетализацияВзаиморасчетов <> "Валюта" Тогда
		...
		Иначе
			ФрагментыЗаголовка.Добавить("<" + НСтр("ru = 'не указан';
													|en = 'not specified'", КодОсновногоЯзыка) + ">");
		КонецЕсли;
	КонецЕсли;
...
Функция ЗапросДанныеНаПечать(МассивОбъектов, НастройкиПечати) Экспорт
	...
	ЗапросДанныеДокумента.УстановитьПараметр("ТипРасчетов", НСтр("ru = 'Расчеты';
																|en = 'Settlements'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ПредставленияДокументов() Экспорт
	...
	ВидыДокументовКонтрагента.Вставить("ВыкупАрендованныхОС",                   НСтр("ru = 'Выкуп арендованных ОС';
	...
	Если ДокументОбъект.ПроверитьЗаполнение() Тогда
		...
		Исключение
			...
			ТекстОшибки = НСтр("ru = 'Не удалось провести %Документ%. %ОписаниеОшибки%';
								|en = 'Cannot post %Документ%. %ОписаниеОшибки%'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%",       ДокументОбъект);
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ОписаниеОшибки%", ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, ДокументОбъект.Ссылка);
		КонецПопытки;
...
Функция ЕстьВводОстатков(ТаблицаРезультатаСКД) Экспорт
	...
	//++ НЕ УТ
	|	ИЛИ ТИПЗНАЧЕНИЯ(Т.РасчетныйДокумент) = ТИП(Документ.ВводОстатковВзаиморасчетовПоАренде)
	//-- НЕ УТ
	|;
	|
	|///////////////////////////////////////////////////////////////////////////////
	...
	Если ВводОстатков.Количество() > 0 Тогда
		...
		|en = 'In the specified reconciliation period, a balance entry document is found.
		|Reconciliation for such period is incorrect. The reconciliation period start must be after entering the balance. 
		|The documents were deleted from the document table. Set the start of the reconciliation period to %1.'");
...
Функция ТекстыЗапросовДоступныхТаблиц()
	...
	//++ НЕ УТ
	Если ПравоДоступа("Просмотр", Метаданные.Справочники.ДоговорыАренды) Тогда
		Результат.Вставить("#3", ТекстЗапросаДоговоровАренды()); //СправочникДоговорыАренды
	КонецЕсли;
	//-- НЕ УТ
...
Функция ТекстЗапросаДоговоровАренды()
