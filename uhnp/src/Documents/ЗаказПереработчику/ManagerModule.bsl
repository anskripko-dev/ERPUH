...
Функция ДоступныеОстаткиПараметрыВстраивания(ДокументОбъект = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ТестЗапроса =
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПереработчику КАК Реквизиты
		|		ПО Реквизиты.ПереработкаПоЗаказу
		|ИНДЕКСИРОВАТЬ ПО
		|	НомерГруппыЗатрат
		|;
		|
		...
		//++ Устарело_Производство21
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаУслуги КАК ТаблицаУслуги
...
Функция ДоступныеОстаткиПараметрыВстраиванияРаспоряжениеСпецификация() Экспорт
	...
	ПараметрыВстраивания.ОписаниеПолученияДанныхДокумента =
		...
		|	ЕСТЬNULL(ТаблицаУслуги.Распоряжение, НЕОПРЕДЕЛЕНО) КАК ЗаказНаОтгрузку,
		|	ТабЧасть.ВариантОбеспечения КАК ВариантОбеспечения,
		|	ТабЧасть.Обособленно КАК Обособленно,
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Обновление документов ""Заказ переработчику (2.4)"":';
								|en = 'Update the ""Subcontracting purchase order (2.4)"" documents:'"));
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Не удалось обработать документы ""Заказ переработчику (2.4)"" по обработчику:';
								|en = 'Cannot process the ""Subcontracting purchase order (2.4)"" documents by the handler:'"));
