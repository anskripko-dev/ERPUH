...
Процедура РедактированиеЭтаповОбеспечениеМатериаламиИРаботамиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока И НЕ Копирование Тогда
		ЗаполнитьГруппуЗатратВНовойСтроке(ТекущиеДанные, ЭтотОбъект);
	КонецЕсли;
	
	УправлениеПроизводствомКлиент.ОбеспечениеМатериаламиИРаботамиЭтапаПриНачалеРедактирования(
		ЭтотОбъект, ТекущиеДанные, НоваяСтрока, Копирование);
	...
	Если НоваяСтрока Тогда
		...
		Если Копирование Тогда
			...
			СтруктураДействий = Новый Структура;
			ДобавитьДействияОбеспеченияЭтаповНаКлиенте(
				СтруктураДействий,
...
Процедура УстановитьУсловноеОформление()
	...
	ПараметрыУсловногоОформления.ИмяПоляЕдиницаИзмерения = "РедактированиеЭтаповВыходныеИзделияНоменклатураЕдиницаИзмерения";
	ПараметрыУсловногоОформления.ПутьКПолюУпаковка = "РедактированиеЭтапов.ВыходныеИзделия.Упаковка";
	...
	ПараметрыУсловногоОформления = НоменклатураСервер.НовыеПараметрыУсловногоОформленияЕдиницИзмерения();
	ПараметрыУсловногоОформления.ИмяПоляЕдиницаИзмерения = "РедактированиеЭтаповПобочныеИзделияНоменклатураЕдиницаИзмерения";
	ПараметрыУсловногоОформления.ПутьКПолюУпаковка = "РедактированиеЭтапов.ПобочныеИзделия.Упаковка";
	НоменклатураСервер.УстановитьУсловноеОформлениеЕдиницИзмерения(ЭтаФорма, ПараметрыУсловногоОформления);
...
Процедура ЭтапыЗаполнитьПолучателяНаСервере(ПараметрыТЧ, ВыделенныеСтроки, Получатель)
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		ОбрабатываемыеСтроки,
		СтруктураДействий,
		РедактированиеЭтапов[ИмяТЧОбъект],
		КэшированныеЗначения);
...
Процедура РазбитьСтрокуТЧЭтапаЗавершение(НоваяСтрока, ИмяТЧ) Экспорт
	...
	ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ТекущиеДанные, СтруктураДействий, КэшированныеЗначения);
	Если ИмяТЧ = "РедактированиеЭтаповОбеспечениеМатериаламиИРаботами" Тогда
		ДобавитьДействияОбеспеченияЭтаповНаКлиенте(СтруктураДействий,"ОбновитьКэшНоменклатура");
...
Процедура ЗаполнитьСлужебныеРеквизитыОбеспеченияЭтапаВСтроке(ТекущиеДанные, Форма, ДанныеЭтапов = Неопределено)
	...
	Если ДанныеЭтапа = Неопределено Тогда
		...
		Иначе
			ДанныеЭтапа = Новый Структура("Подразделение, СписокВыбораНазначений", Неопределено, Новый СписокЗначений);
		КонецЕсли;
...
Процедура ЗаполнитьСлужебныеРеквизитыВыходныеИзделияЭтапаВСтроке(ТекущиеДанные, Форма, ДанныеЭтапов = Неопределено)
	...
	Если ДанныеЭтапа = Неопределено Тогда
		...
		Иначе
			ДанныеЭтапа = Новый Структура("СпособРаспределенияЗатратНаВыходныеИзделия, Подразделение");
		КонецЕсли;
...
Процедура ЗаполнитьСлужебныеРеквизитыПобочныеИзделияЭтапаВСтроке(ТекущиеДанные, Форма, ДанныеЭтапов = Неопределено)
	...
	Если ДанныеЭтапа = Неопределено Тогда
		...
		Иначе
			ДанныеЭтапа = Новый Структура("Статус, Подразделение");
		КонецЕсли;
...
Процедура ВыбратьГруппуЗатратЗавершениеНаСервере(РезультатЗакрытия, ДополнительныеПараметры)
	...
	Для каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
		...
		ИначеЕсли ИмяТЧ = "РедактированиеЭтаповВыходныеИзделия" Тогда
			...
			ЗаполнитьСлужебныеРеквизитыВыходныеИзделияЭтапаВСтроке(ТекущиеДанные, ЭтотОбъект, ДанныеЭтапов);
		...
		ИначеЕсли ИмяТЧ = "РедактированиеЭтаповПобочныеИзделия" Тогда
			...
			ТекущиеДанные.Распоряжение = РезультатЗакрытия.Распоряжение;
			ЗаполнитьСлужебныеРеквизитыПобочныеИзделияЭтапаВСтроке(ТекущиеДанные, ЭтотОбъект, ДанныеЭтапов);
			
		ИначеЕсли ИмяТЧ = "РедактированиеЭтаповОбеспечениеМатериаламиИРаботами" Тогда
			
			ТекущиеДанные.Распоряжение = РезультатЗакрытия.Распоряжение;
			ЗаполнитьСлужебныеРеквизитыОбеспеченияЭтапаВСтроке(ТекущиеДанные, ЭтотОбъект, ДанныеЭтапов);
...
Процедура ПолучитьСтрокиИзБуфераОбмена(ИмяТЧ) 
	...
	ИначеЕсли ИмяТЧ = "РедактированиеЭтаповОбеспечениеМатериаламиИРаботами" Тогда
		...
		ДобавитьДействияОбеспеченияЭтаповНаСервере(СтруктураДействий,
			"ВариантОбеспечения,СкладОбязателен,ДатаОтгрузкиОбязательна");
	...
	Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		//++ НЕ УТКА
		ИначеЕсли ИмяТЧ = "РедактированиеЭтаповОбеспечениеМатериаламиИРаботами" Тогда
			ЗаполнитьГруппуЗатратВНовойСтроке(ТекущаяСтрока, ЭтотОбъект);
			ЗаполнитьСлужебныеРеквизитыОбеспеченияЭтапаВСтроке(ТекущаяСтрока, ЭтотОбъект);
		ИначеЕсли ИмяТЧ = "РедактированиеЭтаповВыходныеИзделия" Тогда
			ЗаполнитьГруппуЗатратВНовойСтроке(ТекущаяСтрока, ЭтотОбъект);
			ЗаполнитьДатуПроизводстваВСтроке(ТекущаяСтрока, ЭтотОбъект);
			ЗаполнитьСлужебныеРеквизитыВыходныеИзделияЭтапаВСтроке(ТекущаяСтрока, ЭтотОбъект);
			МассивИдентификаторовСтрок.Добавить(ТекущаяСтрока.ПолучитьИдентификатор());
		ИначеЕсли ИмяТЧ = "РедактированиеЭтаповПобочныеИзделия" Тогда
			ЗаполнитьГруппуЗатратВНовойСтроке(ТекущаяСтрока, ЭтотОбъект);
			ЗаполнитьДатуПроизводстваВСтроке(ТекущаяСтрока, ЭтотОбъект);
			ЗаполнитьСлужебныеРеквизитыПобочныеИзделияЭтапаВСтроке(ТекущаяСтрока, ЭтотОбъект);
		//-- НЕ УТКА
		КонецЕсли;
...
Процедура ПослеЗаполненияОбеспеченияЭтапов(Изменения, Режим = Неопределено, ПараметрыЗаполнения = Неопределено)
	...
	Для Каждого Изменение Из Изменения Цикл
		...
		Если ОбъектЭтап = Неопределено Тогда
			...
			НайденныеСтроки = РедактированиеЭтапов.Этапы.НайтиСтроки(ПараметрыОтбора);
			Если Не НайденныеСтроки.Количество() = 0 Тогда
				ОбъектЭтап = НайденныеСтроки[0];
		...
		Если ОбеспечениеВДокументахКлиентСервер.НужноПроверитьЗаполнитьДатуОтгрузкиДляСтрокОтгрузить(Режим) Тогда
			...
			Если Изменение.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить Тогда
				...
				Если Не ОбъектЭтап = Неопределено
					И (ОбъектЭтап.Статус = Перечисления.СтатусыЭтаповПроизводства2_2.Формируется
			...
			ИначеЕсли Изменение.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить Тогда
				...
				Если Не ОбъектЭтап = Неопределено
					И ОбъектЭтап.Статус = Перечисления.СтатусыЭтаповПроизводства2_2.Формируется Тогда
