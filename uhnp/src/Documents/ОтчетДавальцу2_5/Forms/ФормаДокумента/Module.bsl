...
Процедура ОбработкаВыбораПодборТоваров(ВыбранноеЗначение)
	...
	Для каждого СтрокаТаблицы Из ТаблицаПродукции Цикл
		...
		ОбрабатываемыеСтроки.Добавить(СтрокаТЧПродукция);
...
Процедура ОбновитьТекстСчетаФактуры()
	...
	ТекстыДокументыНаОсновании.Добавить(СчетаФактурыВыданныеНаОсновании.Представление);
...
Функция ПривязатьСтрокиКЗаказам(ОтобратьПоЗаказу)
	...
	|ПОМЕСТИТЬ РаспоряженияОборотыСверхЗаказа
	|ИЗ
	...
	|	РаспоряженияОбороты.Склад,
	|	РаспоряженияОбороты.Серия
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ИЗ
	|			РаспоряженияОборотыСверхЗаказа КАК ЗаказыОбороты
	|
	|		ОБЪЕДИНИТЬ ВСЕ
	|
	|		ВЫБРАТЬ
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока.Цена) И Не ЗначениеЗаполнено(ТекущаяСтрока.ВидЦены) Тогда
			ОбрабатываемыеСтрокиСЦеной.Добавить(ТекущаяСтрока);
		Иначе
