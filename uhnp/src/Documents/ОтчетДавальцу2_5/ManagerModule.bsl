...
Процедура ЗаполнитьПоОстаткамЗаказов(ДанныеОтбора, Таблица, МассивЗаказов, ПараметрыЗаполнения) Экспорт
	...
	Пока ВыборкаПоОстаткам.Следующий() Цикл
		...
		Если ПересчитатьСуммы Тогда
			СтрокаТаблицы.СуммаСНДС = ВыборкаПоОстаткам.СуммаОстаток;
		КонецЕсли;
...
Функция СтруктураОснованияДляПечати(Объект) Экспорт
	...
	Возврат СтруктураОснования(Объект);
...
Функция ТекстЗапросаВтПродукция(Запрос, ТекстыЗапроса) Экспорт
	...
	|	И ТаблицаПродукции.АналитикаУчетаНоменклатуры НЕ В
	|		(ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Функция ТекстЗапросаТаблицаРаспоряженияНаОтгрузку(Запрос, ТекстыЗапроса, Регистры)
	...
	|	&Период                                КАК Период,
	|	ТаблицаПродукции.ЗаказДавальца         КАК Распоряжение,
	|	ТаблицаПродукции.Номенклатура          КАК Номенклатура,
	|	ТаблицаПродукции.Характеристика        КАК Характеристика,
...
Процедура СформироватьСуммыДокументаВВалютахУчета(Запрос, ТекстыЗапроса, Регистры = Неопределено) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетДавальцу2_5.ВидыЗапасов КАК ВидыЗапасов
...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	|		ПО ТаблицаВидыЗапасов.Ссылка = ТаблицаДокумента.Ссылка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетДавальцу2_5.ВидыЗапасов КАК ЗапасыДокумента
	|		ПО ТаблицаВидыЗапасов.Ссылка = ЗапасыДокумента.Ссылка
	|			И ТаблицаВидыЗапасов.АналитикаУчетаНоменклатуры = ЗапасыДокумента.АналитикаУчетаНоменклатуры
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|		ПО ТаблицаВидыЗапасов.Номенклатура = СправочникНоменклатура.Ссылка
	|ГДЕ
	|	ТаблицаДокумента.Ссылка В (&Ссылка)
	|	И ЗапасыДокумента.Ссылка ЕСТЬ NULL
	|	И СправочникНоменклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|	И НЕ ТаблицаВидыЗапасов.СписатьНаРасходы
	|
	|ОБЪЕДИНИТЬ ВСЕ
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетДавальцу2_5.ВидыЗапасов КАК ДанныеЗапасов
	|		ПО ТаблицаЗапасов.Ссылка = ДанныеЗапасов.Ссылка
	|			И ТаблицаЗапасов.АналитикаУчетаНоменклатуры = ДанныеЗапасов.АналитикаУчетаНоменклатуры
	|	
	|ГДЕ
	|	ТаблицаЗапасов.Ссылка В(&Ссылка)
	|	И ДанныеЗапасов.Ссылка ЕСТЬ NULL
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Изменение", Метаданные.Документы.ОтчетДавальцу2_5) Тогда
		...
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
		КомандаПечати.ОсновнаяПечатнаяФорма = Истина;
		КомандаПечати.НаименованиеПечатнойФормы = НСтр("ru = 'Отчет о продукции и материалах';
														|en = 'Report on manufactured products and materials'");
...
Функция ДанныеДляПечатныхФормСчетаНаОплатуИзвещения(ПараметрыПечати, МассивОбъектов, КодЯзыка = Неопределено) Экспорт
	...
	|	ЕСТЬNULL(ОтчетыДавальцу.БанковскийСчетОрганизации.СчетЦифровогоРубля, ЛОЖЬ) КАК ЭтоСчетЦифровогоРубля,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ОтчетыДавальцу.БанковскийСчетОрганизации.СчетЦифровогоРубля, ЛОЖЬ)
	|			ТОГДА ОтчетыДавальцу.БанковскийСчетОрганизации.ИдентификаторСчетаЦифровогоРубля
	|		ИНАЧЕ ОтчетыДавальцу.БанковскийСчетОрганизации.НомерСчета
		...
		Пока ВыборкаСтрок.Следующий() Цикл
			...
			ПараметрыОбласти.Вставить(
				...
				НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати(
					ВыборкаСтрок.НоменклатураНаименованиеПолное,
					ВыборкаСтрок.ХарактеристикаНаименованиеПолное));
...
Функция СформироватьПечатнуюФормуОтчетОПродукцииИМатериалах(ДанныеПечати, ОбъектыПечати, ТабличныйДокумент) Экспорт
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если Не ПараметрыУчетнойПолитики = Неопределено
			...
			// Продукция
			|	Продукция.АналитикаУчетаПродукции                                                   КАК АналитикаУчетаПродукции,
			|	СпрКлючиАналитикиПродукции.Номенклатура                                      КАК ТоварПродукция,
			|	СпрКлючиАналитикиПродукции.Номенклатура.НаименованиеПолное                   КАК ТоварНаименованиеПродукция,
			|	СпрКлючиАналитикиПродукции.Номенклатура.Код                                  КАК КодПродукция,
			|	СпрКлючиАналитикиПродукции.Номенклатура.Артикул                              КАК АртикулПродукция,
			|	ЕСТЬNULL(СпрКлючиАналитикиПродукции.Характеристика.НаименованиеПолное, """") КАК ХарактеристикаПродукция,
			...
			|	СпрНоменклатураЗатрат.НаименованиеПолное                   КАК ТоварНаименование,
			|	СпрНоменклатураЗатрат.Код                                  КАК Код,
			|	СпрНоменклатураЗатрат.Артикул                              КАК Артикул,
			|	ЕСТЬNULL(СпрХарактеристикиЗатрат.НаименованиеПолное, """") КАК Характеристика,
			...
			|	
			|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК СпрКлючиАналитикиПродукции
			|	ПО СпрКлючиАналитикиПродукции.Ссылка = Продукция.АналитикаУчетаПродукции
			|	
			|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СпрНоменклатураЗатрат
			|	ПО СпрНоменклатураЗатрат.Ссылка = Затраты.Номенклатура
			|	
			|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатуры КАК СпрХарактеристикиЗатрат
			|	ПО СпрХарактеристикиЗатрат.Ссылка = Затраты.Характеристика
			|	
			|СГРУППИРОВАТЬ ПО
			// Продукция
			|	Продукция.АналитикаУчетаПродукции,
			|	СпрКлючиАналитикиПродукции.Номенклатура,
			|	ЕСТЬNULL(СпрКлючиАналитикиПродукции.Характеристика.НаименованиеПолное, """"),
			|	ПРЕДСТАВЛЕНИЕССЫЛКИ(Продукция.ЕдиницаИзмерения),
			// Затрата
			|	Затраты.Номенклатура,
			|	СпрНоменклатураЗатрат.Ссылка,
			|	ЕСТЬNULL(СпрХарактеристикиЗатрат.НаименованиеПолное, """"),
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, МассивЗаказов = Неопределено)
	...
	|						ИЗ
	|							ТаблицаЗаказов КАК ТаблицаЗаказов)
	|					И КодСтроки <> 0) КАК РаспоряженияОбороты
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		-РаспоряженияНаОтгрузку.КОформлению,
	|		-РаспоряженияНаОтгрузку.Сумма
	|	ИЗ
	|		РегистрНакопления.РаспоряженияНаОтгрузку КАК РаспоряженияНаОтгрузку
	|	ГДЕ
	|		РаспоряженияНаОтгрузку.Активность   
	|		И РаспоряженияНаОтгрузку.Период >= &ДатаНачала
	|		И РаспоряженияНаОтгрузку.ВидДвиженияРегистра = ЗНАЧЕНИЕ(Перечисление.ВидыДвиженияНакопления.Расход)
	|		И РаспоряженияНаОтгрузку.Регистратор = &Регистратор
	|		И РаспоряженияНаОтгрузку.Распоряжение В
	|				(ВЫБРАТЬ
	|					ТаблицаЗаказов.Ссылка КАК Ссылка
	|				ИЗ
	|					ТаблицаЗаказов КАК ТаблицаЗаказов)
	|		И РаспоряженияНаОтгрузку.КодСтроки <> 0) КАК ТаблицаЗаказовКОформлению
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	СУММА(ТаблицаЗаказовСверхЗаказа.КОформлению),
	|	0
	|ИЗ
	|	(ВЫБРАТЬ
	|		РаспоряженияОбороты.Распоряжение КАК ЗаказКлиента,
	...
	|				ТОГДА РаспоряженияОбороты.КПередачеОборот
	|			ИНАЧЕ 0
	|		КОНЕЦ - ВЫБОР
	...
	|				ИЗ
	|					ТаблицаЗаказов КАК ТаблицаЗаказов)
	|		И РаспоряженияНаОтгрузку.КодСтроки = 0) КАК ТаблицаЗаказовСверхЗаказа
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаЗаказовСверхЗаказа.ЗаказКлиента,
	|	ТаблицаЗаказовСверхЗаказа.Номенклатура,
	...
	|	ТаблицаОстатков.Сумма КАК СуммаОстаток,
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ТаблицаПродукции.Упаковка, ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|			ТОГДА 1
...
// 
// Параметры:
//  СсылкаОбъект - ДокументСсылка.ОтчетДавальцу2_5 - Документ, по которому нужно получить данные.
...
Функция СуммыПоЗаказам(СсылкаОбъект, ЗначенияРеквизитов = Неопределено) Экспорт
	...
	|	&ЗаказШапки						КАК ЗаказДавальца
	|ПОМЕСТИТЬ ВТПродукция
	|ИЗ &ТаблицаЗначений КАК Продукция
	|ГДЕ &УсловиеСсылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	Если ЗначенияРеквизитов = Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТаблицаЗначений", "Документ.ОтчетДавальцу2_5.Продукция");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ЗаказШапки", "Продукция.Ссылка.ЗаказДавальца");
...
Функция СтруктураОснования(Объект)
	...
	Если Объект.ОтчетПоЗаказам Тогда
		...
		Если ЗначениеЗаполнено(Объект.Договор) Тогда
			...
			СтруктураОснование.Основание = СтруктураОснование.Основание + НСтр("ru = ', переработка давальческого сырья %1';
																				|en = ', processing provided materials %1'");
			СтруктураОснование.Основание = СтрШаблон(СтруктураОснование.Основание, РеквизитыДоговораТекст);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Документы.ОтчетДавальцу2_5.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "2.5.20.26";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Порядок = Перечисления.ПорядокОбработчиковОбновления.Обычный;
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("a486f62b-dd5a-4216-941d-412dd0a17d9d");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ОтчетДавальцу2_5.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	
	СписокОписаний = Новый Массив;
	СписокОписаний.Добавить(НСтр("ru = 'Обновление документов ""Отчет давальцу"":';
								|en = 'Update the ""Consumption report — Subcontracting services delivered"" documents:'"));
	...
	Изменяемые.Добавить(Метаданные.Документы.ОтчетДавальцу2_5.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ОтчетДавальцу2_5.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ОтчетДавальцу2_5.Продукция КАК ОтчетДавальцу2_5
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(ОтчетДавальцу2_5.АналитикаАктивовПассивов) = ТИП(Перечисление.УдалитьТипыНалогов)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ОтчетДавальцу2_5.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ОтчетДавальцу2_5.ВидыЗапасов КАК ОтчетДавальцу2_5
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(ОтчетДавальцу2_5.АналитикаАктивовПассивов) = ТИП(Перечисление.УдалитьТипыНалогов)";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний = Новый Массив;
	СписокОписаний.Добавить(НСтр("ru = 'Не удалось обработать документы ""Отчет давальцу"" по обработчику:';
								|en = 'Cannot process the ""Consumption report — Subcontracting services delivered"" documents by the handler:'"));
	...
	Запрос.Текст = 
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетДавальцу2_5 КАК ДанныеДокумента
