...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого СтрокаТаблицы Из МестаПрименения Цикл
		...
		Если (СтрокаТаблицы.ТипМестаПрименения = Перечисления.ТипыМестПримененияСАТУРН.ЗарегистрованноеМестоПрименения
			Или Не ЗначениеЗаполнено(СтрокаТаблицы.ТипМестаПрименения))
		...
		ИначеЕсли Не СтрокаТаблицы.ТипМестаПрименения = Перечисления.ТипыМестПримененияСАТУРН.ЗарегистрованноеМестоПрименения
			И ЗначениеЗаполнено(СтрокаТаблицы.ТипМестаПрименения)
	...
	Если Не ПоПриказу420 Тогда
		МассивНепроверяемыхРеквизитов.Добавить("Товары.ВнесеноНаПлощадь");
	КонецЕсли;
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Для Каждого СтрокаТаблицы Из МестаПрименения Цикл
		СтрокаТаблицы.ТипМестаПрименения      = Неопределено;
		СтрокаТаблицы.ОписаниеМестаПрименения = Неопределено;
	КонецЦикла;
...
Процедура ЗаполнитьПоПлануПримененияСАТУРН(ДанныеЗаполнения)
	...
	ИнтеграцияСАТУРНКлиентСервер.ЗаполнитьТипИзмеряемойВеличиныИКоличествоВУпаковке(Товары);
