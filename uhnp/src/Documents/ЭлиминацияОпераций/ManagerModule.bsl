...
Функция ПодготовитьПараметрыПроведения(ДополнительныеСвойства, Отказ) Экспорт
	...
	ТекстЗапроса.Добавить(ТекстЗапроса_втРБВИБ(НомераТаблиц));
...
Функция ТекстЗапроса_втРБВИБ(НомераТаблиц)

	НомераТаблиц.Вставить("втРБВИБ", НомераТаблиц.Количество());

	Возврат
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	т.РегистрБухгалтерииИсточник КАК РегистрИсточник
	|ПОМЕСТИТЬ втРБВИБ
	|ИЗ
	|	Справочник.ШаблоныТрансляций КАК т
	|ГДЕ
	|	т.РегистрБухгалтерииИсточник.Владелец <> ЗНАЧЕНИЕ(Справочник.ТипыБазДанных.ТекущаяИБ)
	|	И т.РегистрБухгалтерииИсточник <> ЗНАЧЕНИЕ(Справочник.РегистрыБухгалтерииБД.ПустаяСсылка)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	РегистрИсточник";
...
Функция ТекстЗапроса_ПроводкиСторно(НомераТаблиц, ЭтоМСФО)
	...
	Если ЭтоМСФО Тогда
		...
		УсловиеОтбора = 
		"	ВЫБОР 
		|		КОГДА Регистратор ССЫЛКА Документ.Трансляция
		|			И ВидОперации = ЗНАЧЕНИЕ(Справочник.ВидыОпераций.Трансляция) 
		|			ТОГДА ВЫРАЗИТЬ(Регистратор КАК Документ.Трансляция).ИсходныйДокумент <> НЕОПРЕДЕЛЕНО
		|				ИЛИ ВЫРАЗИТЬ(Регистратор КАК Документ.Трансляция).РегистрБухгалтерииИсточник В (ВЫБРАТЬ т.РегистрИсточник ИЗ втРБВИБ КАК т)
		|		КОГДА Регистратор ССЫЛКА Документ.ПоступлениеМСФО
		|			ТОГДА ИСТИНА
		|		КОГДА Регистратор ССЫЛКА Документ.РеализацияМСФО
		|			ТОГДА ИСТИНА
		|		КОГДА Регистратор ССЫЛКА Документ.ВводВНАВЭксплуатациюМСФО
		|			ТОГДА ИСТИНА
		|		КОГДА Регистратор ССЫЛКА Документ.ВыбытиеВНАМСФО
		|			ТОГДА ИСТИНА
		|		КОГДА Регистратор ССЫЛКА Документ.МодернизацияВНАМСФО
		|			ТОГДА ИСТИНА
		|		КОГДА Регистратор ССЫЛКА Документ.ПоступлениеВНАМСФО
		|			ТОГДА ИСТИНА
		|	КОНЕЦ
		|	";
...
Функция ТекстЗапроса_ПроводкиНеДКЗ(НомераТаблиц, ЭтоМСФО = Истина)
	...
	|	ЕСТЬNULL(ПроводкиСебестоимость.ВидСубконтоДт1, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)) КАК ВидСубконтоДт1,
	|	ПроводкиСебестоимость.СубконтоДт2 КАК СубконтоДт2,
	|	ЕСТЬNULL(ПроводкиСебестоимость.ВидСубконтоДт2, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)) КАК ВидСубконтоДт2,
	|	ПроводкиСебестоимость.СубконтоДт3 КАК СубконтоДт3,
	|	ЕСТЬNULL(ПроводкиСебестоимость.ВидСубконтоДт3, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)) КАК ВидСубконтоДт3,
	|	ПроводкиСебестоимость.СчетКт КАК СчетКт,
	|	ПроводкиСебестоимость.СубконтоКт1 КАК СубконтоКт1,
	|	ЕСТЬNULL(ПроводкиСебестоимость.ВидСубконтоКт1, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)) КАК ВидСубконтоКт1,
	|	ПроводкиСебестоимость.СубконтоКт2 КАК СубконтоКт2,
	|	ЕСТЬNULL(ПроводкиСебестоимость.ВидСубконтоКт2, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)) КАК ВидСубконтоКт2,
	|	ПроводкиСебестоимость.СубконтоКт3 КАК СубконтоКт3,
	|	ЕСТЬNULL(ПроводкиСебестоимость.ВидСубконтоКт3, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)) КАК ВидСубконтоКт3,
...
Функция ТекстЗапроса_Проводки(НомераТаблиц, ЭтоМСФО)
	...
	|	ЕСТЬNULL(ПроводкиВыручка.ВидСубконтоДт1, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)) КАК ВидСубконтоДт1,
	|	ПроводкиВыручка.СубконтоДт2 КАК СубконтоДт2,
	|	ЕСТЬNULL(ПроводкиВыручка.ВидСубконтоДт2, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)) КАК ВидСубконтоДт2,
	|	ПроводкиВыручка.СубконтоДт3 КАК СубконтоДт3,
	|	ЕСТЬNULL(ПроводкиВыручка.ВидСубконтоДт3, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)) КАК ВидСубконтоДт3,
	|	ПроводкиВыручка.СчетКт КАК СчетКт,
	|	ПроводкиВыручка.СубконтоКт1 КАК СубконтоКт1,
	|	ЕСТЬNULL(ПроводкиВыручка.ВидСубконтоКт1, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)) КАК ВидСубконтоКт1,
	|	ПроводкиВыручка.СубконтоКт2 КАК СубконтоКт2,
	|	ЕСТЬNULL(ПроводкиВыручка.ВидСубконтоКт2, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)) КАК ВидСубконтоКт2,
	|	ПроводкиВыручка.СубконтоКт3 КАК СубконтоКт3,
	|	ЕСТЬNULL(ПроводкиВыручка.ВидСубконтоКт3, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)) КАК ВидСубконтоКт3,
	...
	|	ВЫБОР
	|		КОГДА &ПроводкиВыручка_СчетДт_НалоговыйУчет
	|			ТОГДА -ПроводкиВыручка.СуммаВВалютеОтчетности
	|	КОНЕЦ КАК НУ_СуммаВВалютеОтчетности,
	|	ВЫБОР
	|		КОГДА &ПроводкиВыручка_СчетКт_НалоговыйУчет
	|			ТОГДА -ПроводкиВыручка.СуммаВВалютеОтчетности
	...
	|	ЕСТЬNULL(Продавец.ВидСубконтоДт1, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)),
	|	Продавец.СубконтоДт2,
	|	ЕСТЬNULL(Продавец.ВидСубконтоДт2, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)),
	|	Продавец.СубконтоДт3,
	|	ЕСТЬNULL(Продавец.ВидСубконтоДт3, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)),
	|	Покупатель.СчетДт,
	|	Покупатель.СубконтоДт1,
	|	ЕСТЬNULL(Покупатель.ВидСубконтоДт1, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)),
	|	Покупатель.СубконтоДт2,
	|	ЕСТЬNULL(Покупатель.ВидСубконтоДт2, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)),
	|	Покупатель.СубконтоДт3,
	|	ЕСТЬNULL(Покупатель.ВидСубконтоДт3, ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоКорпоративные.ПустаяСсылка)),
	|	NULL,
	|	NULL,
	|	ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА &Продавец_СчетДт_НалоговыйУчет
	|			ТОГДА -Продавец.СуммаВВалютеОтчетности КОНЕЦ,
	|	ВЫБОР
	|		КОГДА &Покупатель_СчетДт_НалоговыйУчет
	|			ТОГДА -Продавец.СуммаВВалютеОтчетности КОНЕЦ,
	|	0,
	|	0,
	|	NULL,
	...
	КодЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "%СторноВГО%", НСтр("ru = 'Сторно себестоимости актива ВГО'", КодЯзыка));
	
	Замены = Новый Структура(
		"ПроводкиВыручка_СчетДт_НалоговыйУчет,
		|ПроводкиВыручка_СчетКт_НалоговыйУчет,
		|Продавец_СчетДт_НалоговыйУчет, 
		|Покупатель_СчетДт_НалоговыйУчет");
	
	Для каждого _ Из Замены Цикл
...
Процедура СформироватьДокументы(Сценарий, ПериодОтчета, Организация, РегистрУчета = Неопределено) Экспорт
	...
	ТекстЗапроса = Новый Массив;
	ТекстЗапроса.Добавить(ТекстЗапроса_Контрагенты(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_РазделыПланаСчетов(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_СчетаДКЗ(НомераТаблиц));
...
Процедура ОбновитьДокументы(ПарыОрганизаций, Созданные, Параметры)
	...
	Параметры.Вставить("РегистрУчета", Параметры.ВозможныеРБ.Получить(0));
	...
	Для каждого ТекущаяПара Из ПарыОрганизаций Цикл
		...
		Исключение
			ОшибкаПроведения(Объект.Ссылка, ИнформацияОбОшибке(), Отказ);
		КонецПопытки;
...
Функция ТекстЗапроса_Созданные(НомераТаблиц)
	...
	Возврат
	"ВЫБРАТЬ
	|	МАКСИМУМ(ЭлиминацияОпераций.Ссылка) КАК Документ,
	...
	|ГДЕ
	|	ЭлиминацияОпераций.Проведен
	|	И ЭлиминацияОпераций.Организация = &Организация
	|	И ЭлиминацияОпераций.ПериодОтчета = &ПериодОтчета
	|	И ЭлиминацияОпераций.Сценарий = &Сценарий
	|	И ЭлиминацияОпераций.РегистрУчета В(&ВозможныеРБ)
	|
	|СГРУППИРОВАТЬ ПО
	|	ЭлиминацияОпераций.ОрганизацияА,
	|	ЭлиминацияОпераций.ОрганизацияБ";

КонецФункции

Процедура ОшибкаПроведения(Ссылка = Неопределено, ИнформацияОбОшибке = "",  Отказ = Ложь, ИдентификаторФормы = Неопределено)

	ТекстСообщения = НСтр("ru = 'Не удалось провести документ ""%1"" по причине: '");
	ТекстСообщения = СтрШаблон(ТекстСообщения, Ссылка) + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);

	ОбщегоНазначенияУХ.СообщитьОбОшибке(ТекстСообщения, Отказ, , , ИдентификаторФормы, Ссылка);
