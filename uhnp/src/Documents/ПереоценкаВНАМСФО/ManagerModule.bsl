...
Функция ПодготовитьПараметрыПроведения(ДополнительныеСвойства, Отказ) Экспорт
	...
	//Реквизиты.АлгоритмРСБУ не используется, т.к. требуются ПараметрыВНА 
	//на дату документа (АмортизацияРасчет - не требуется, т.к. уже посчитана в ТЧ)
	АлгоритмРСБУ = Ложь;
...
Функция ТекстЗапроса_втТаблицаВНА(НомераТаблиц)
	...
	|	т.ПропорциональныйПересчет
	|		ИЛИ р.Переоценка <> 0 КАК ПропорциональныйПересчет,
...
Функция ТекстЗапроса_ТаблицаВНА(НомераТаблиц)
	...
	|	ЕстьNULL(втТаблицаВНА.ИзменениеНакопленнойАмортизации, 0) + ЕстьNULL(втТаблицаВНА.АмортизацияРасчет, 0) КАК Амортизация,
	|	ЕСТЬNULL(втТаблицаВНА.ИзменениеПервоначальнойСтоимости, 0) + ВЫБОР
	|		КОГДА НЕ втТаблицаВНА.ПропорциональныйПересчет
	|			ТОГДА ЕстьNULL(втТаблицаВНА.ИзменениеНакопленнойАмортизации, 0)
	|		ИНАЧЕ 0
...
Функция ТекстЗапроса_ТаблицаПроводкиВНА(НомераТаблиц)
	...
	Результат.Добавить(ТекстЗапроса_ДооценкаНА()); 		// Дт 02 Кт 83 (ПропорциональныйПересчет)
	Результат.Добавить(ТекстЗапроса_УценкаНА());		// Дт 91.2 Кт 02 (ПропорциональныйПересчет)
	Результат.Добавить(ТекстЗапроса_СальдоНА()); 		// Дт 02 Кт 01 (Сальдовый метод = Не ПропорциональныйПересчет)
...
Функция ТекстЗапроса_ПроверкаПараметровПереоценкиВНА(НомераТаблиц = Неопределено, ИмяТаблицы = Неопределено) Экспорт
	...
	Параметры.Добавить("ЕстьNULL(ТаблицаВНА.ГруппаВНА.Наименование, ""..."")"); // @query-part
	
	Шаблон = НСтр("ru = 'Для параметров учета ВНА <%2> (для ВНА <%1>) не указаны параметры переоценки'");
...
Функция ТекстЗапроса_ДооценкаНА()
	...
	|ГДЕ
	|	втТаблицаВНА.ИзменениеПервоначальнойСтоимости > 0
	|	И втТаблицаВНА.ПропорциональныйПересчет";
...
Функция ТекстЗапроса_СальдоНА()
	...
	|	втТаблицаВНА КАК втТаблицаВНА
	|ГДЕ
	|	втТаблицаВНА.ИзменениеНакопленнойАмортизации <> 0
	|	И НЕ втТаблицаВНА.ПропорциональныйПересчет";
...
Процедура ЗаполнитьДокумент(ДокументОбъект) Экспорт
	...
	Если АлгоритмРСБУ Тогда
		...
		ТекстЗапроса.Добавить(ТекстЗапроса_втСписокВНАПУ(НомераТаблиц));
...
Функция ТекстЗапроса_втСписокВНАПУ(НомераТаблиц)

	НомераТаблиц.Вставить("втСписокВНАПУ", НомераТаблиц.Количество());
...
Функция ТекстЗапроса_ТаблицаВНА_Заполнение(НомераТаблиц, АлгоритмРСБУ = Ложь)
	...
	Если АлгоритмРСБУ Тогда
		Возврат СтрЗаменить(ЗапросПараметрыВНА, "&ВНА", "ВЫБРАТЬ т.ВНА ИЗ втСписокВНАПУ КАК т");
	Иначе	
