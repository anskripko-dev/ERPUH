	...
	Если Объект.РежимДоначисления Тогда
		...
		Иначе 
			...
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (создание)';
																					|en = '%1 (Create)'"), НСтр("ru = 'Доначисление, перерасчет';
	...
	Если Не АвтоЗаголовок Тогда
		Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '%1 (создание)';
								|en = '%1 (Create)'"), НаименованиеВидаОперации);
		Иначе
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '%1 %2 от %3';
...
Процедура РассчитатьИтогНачисленоПерерасчет(Форма)
	...
	ОписанияСоставляющихДоговоры.Добавить(
		ЗарплатаКадрыКлиентСервер.ИтогиТабличныхЧастейОписаниеСуммируемогоРеквизита("Результат", НСтр("ru = 'перерасчет по договорам ГПХ';
																										|en = 'recalculation under civil law contracts'")));
