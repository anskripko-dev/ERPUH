...
&НаКлиенте
Процедура ЗаполнитьИзУчетнойСистемыПоПроводкам(Команда)
	ДанныеЗаполнения = Новый Структура("ИсточникЗаполнения, ПоПроводкам", "ДанныеНСБУ", Истина);
	МСФОКлиентУХ.ЗаполнитьДокумент(ЭтаФорма, ДанныеЗаполнения, "ВНА");
КонецПроцедуры
...
Процедура ВНАГруппаВНАПриИзменении(Элемент)
	...
	ОбновитьОтображениеГруппыПодробно(Элементы.ВНА.ТекущиеДанные);
...
Процедура УправлениеФормой(Форма)
	...
	//** в ПФ дб заполнение по проводкам
	ВидимостьПоПроводкам = Элементы.НСБУЗаполнитьИзУчетнойСистемы.Видимость 
		И УчетМСФОПовтИспУХ.ЕстьРСБУ();
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ВНАЗаполнитьИзУчетнойСистемыПоПроводкам", "Видимость", ВидимостьПоПроводкам);
		
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоЕХ() Тогда
		Элементы.НСБУЗаполнитьИзУчетнойСистемы.Видимость = Ложь; // в ЕРПУХ только по проводкам
	ИначеЕсли Не УчетМСФОПовтИспУХ.ЕстьРСБУ() Тогда
		Элементы.НСБУЗаполнитьИзУчетнойСистемы.Видимость = Ложь; // в ПФ нет Хозрасчетный
	КонецЕсли;
...
Процедура ОбновитьОтображениеГруппыПодробно(ТекущаяСтрока)
	...
	ВидимостьСпособПоступления			= Ложь;
	...
	ИначеЕсли ВидыСобытийВНА.ВводВНАВЭксплуатациюМСФО.Найти(ТекущаяСтрока.ВидОперации) <> Неопределено Тогда
		...
		ВидимостьПодразделенияНаправления	= Истина;
		ВидимостьСпособПоступления			= Истина;
	...
	ИначеЕсли ВидыСобытийВНА.ОбесценениеВНАМСФО.Найти(ТекущаяСтрока.ВидОперации) <> Неопределено Тогда	
	...
	Элементы.ВНАСпособПоступления.Видимость 		= ВидимостьСпособПоступления;
	...
	СвойстваКоррСчета = УчетМСФОПовтИспУХ.ПолучитьСвойстваСчета(ТекущаяСтрока.КоррСчетМСФО);
...
Процедура ВНАДатаСобытияМСФОПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.ДатаСобытияНСБУ) Тогда
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ВНАДатаСобытияМСФОПриИзмененииЗавершение",ЭтотОбъект,Новый Структура("ТекущаяСтрока", ТекущаяСтрока)),
			Нстр("ru = 'Перезаполнить дату события НСБУ по дате события МСФО?'"), РежимДиалогаВопрос.ДаНет);
		Возврат;
	КонецЕсли;
