...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипДанныхЗаполнения = Тип("Структура") Тогда 
		...
		Если ДанныеЗаполнения.Свойство("ЗаполнитьДокумент") 
			...
			Если Не ТекущиеДанныеЗаполнения.Свойство("ТипВНА") Тогда
				ТекущиеДанныеЗаполнения.Вставить("ТипВНА", Неопределено);
			КонецЕсли;
			...
			ЗаполнитьИзПодсистемыНСБУ(Неопределено, ТекущиеДанныеЗаполнения);

		ИначеЕсли ДанныеЗаполнения.Свойство("ИсточникЗаполнения") 
			И ДанныеЗаполнения.ИсточникЗаполнения = "ДанныеНСБУ" Тогда
			
			ЗаполнитьИзПодсистемыНСБУ(Неопределено, ДанныеЗаполнения);
...
Процедура ЗаполнитьИзПодсистемыНСБУ(ДокументыОснованияИсточник = Неопределено, ДанныеЗаполнения = Неопределено)
	
	ПараметрыЗаполнения = МСФОКлиентСерверУХ.ПолучитьПараметрыЗаполнения(ЭтотОбъект, ДокументыОснованияИсточник);
	Если (ДанныеЗаполнения <> Неопределено) И ДанныеЗаполнения.Свойство("ПоПроводкам") Тогда
		ПараметрыЗаполнения.Вставить("ПоПроводкам", ДанныеЗаполнения.ПоПроводкам);
	КонецЕсли;
	...
	КэшируемыеЗначения = Новый Структура("ПланСчетовНСБУ, ШаблонТрансляции", 
		ПараметрыЗаполнения.ШаблонТрансляции.ПланСчетовИсточник, ПараметрыЗаполнения.ШаблонТрансляции);
		
	КонтекстФормы = Новый Структура("Элементы,Объект,КэшируемыеЗначения", Новый Массив, ЭтотОбъект, КэшируемыеЗначения);
	...
	Документы.ВводСобытийВНАМСФО.ЗаполнитьЗависимостиРеквизитовДокумента(КонтекстФормы);
	
	Для каждого СтрокаВНА Из ЭтотОбъект.ВНА Цикл
		МСФОКлиентСерверУХ.ОбновитьЗависимыеРеквизиты(СтрокаВНА, КонтекстФормы, "ПараметрыУчетаВНА");
	КонецЦикла;
