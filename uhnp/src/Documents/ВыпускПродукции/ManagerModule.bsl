...
Функция ОписаниеОперацийУчетаСебестоимости(ДокументОбъект) Экспорт
	...
	#Область ВнутреннееПоступление_РаботыПоВыпускуДавальческойПродукции
	...
	// Поля учета номенклатуры
	|	ТаблицаДокумента.Организация						КАК Организация,
	|	ВтВидыЗапасов.АналитикаУчетаУДавальца				КАК АналитикаУчетаНоменклатуры,
	|	НЕОПРЕДЕЛЕНО										КАК ВидЗапасов,
	|	ВтВидыЗапасов.ВидДеятельностиНДС					КАК ВидДеятельностиНДС,
	|	ВтВидыЗапасов.ВидДеятельностиНДС					КАК ВидДеятельностиНДСДокумента,
	|
	...
	|	ТаблицаДокумента.Ссылка				КАК Партия,
	|	ВтВидыЗапасов.АналитикаУчетаПартий	КАК АналитикаУчетаПартий,
	|	ВтВидыЗапасов.КодСтроки				КАК КодСтроки,
	|
	// Корреспондирующие поля
	|	ВтВидыЗапасов.АналитикаУчетаНоменклатуры	КАК КорАналитикаУчетаНоменклатуры,
	|	НЕОПРЕДЕЛЕНО								КАК КорГруппаПродукции, 
	|	ВЫБОР КОГДА &ПартионныйУчетВерсии22 И НЕ &ФИФОСкользящаяОценка
	|		ТОГДА ВтВидыЗапасов.АналитикаУчетаПартий
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	|ГДЕ
	|	ТаблицаДокумента.Ссылка В (&Ссылка)
	|	И ВтВидыЗапасов.ПередатьДавальцу
	|	И &ПартионныйУчетВерсии22
	|";
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Обновление документов ""Выпуск продукции и выполнение работ"":';
								|en = 'Update the ""Product release and work performance"" documents:'"));
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Не удалось обработать документы ""Выпуск продукции и выполнение работ"" по обработчику:';
								|en = 'Cannot process the ""Product release and work performance"" documents by the handler:'"));
