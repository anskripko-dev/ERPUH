...
Процедура АналитикиПередУдалением(Элемент, Отказ)
	...
	Если СписокТиповАналитикСчетов.НайтиПоЗначению(ТекДанные.ТипДляЭлиминации) <> Неопределено Тогда
		...
		Если Элементы.ЗначенияСубконто.Видимость Тогда
...
Процедура УстановитьВидимость()
	...
	Элементы.ФормаЗаполнитьПроводки.ЦветТекста = ?(Объект.ПроводкиУжеЗаполненыАвтоматически, Новый Цвет, WebЦвета.ТемноКрасный);
...
Процедура ЗаполнитьПроводкиНаСервере()
	...
	Если Элементы.Проводки.Видимость Тогда
		Элементы.Проводки.Обновить();
	КонецЕсли;
	
	Если Элементы.ДополнительныеКорректировки.Видимость Тогда
...
Функция ЗначениеСверки(ВидАналитики, ТипДляЭлиминации, ИмяВГО, флДопАналитика, ТЗ_ЗначенияРеквизитовДопАналитик)
	...
	Исключение
		 ЗначениеАналитики_ = ОбщегоНазначенияУХ.ПустоеЗначениеТипа(ВидАналитики.ТипЗначения.Типы()[0]);
	КонецПопытки;
...
Процедура ИсточникДанныхПриИзмененииНаСервере()
	...
	Для Ном_ = 1 По ЧислоАналитик Цикл
		...
		НоваяСтрока.ВидАналитики = ВидАналитикиРаскрытия(ГруппаАналитик, Ном_);
...
Функция ВидАналитикиРаскрытия(ГруппаРаскрытия, НомерАналитики)
	
	ИмяРеквизита = "ВидАналитики" + НомерАналитики;
	
	Раскрытие = ОбщегоНазначенияУХ.ЗначенияРеквизитовОбъекта(ГруппаРаскрытия, ИмяРеквизита + ", Владелец");
	Если ЗначениеЗаполнено(Раскрытие[ИмяРеквизита]) Тогда
		Возврат Раскрытие[ИмяРеквизита];
	КонецЕсли;
	
	Возврат ОбщегоНазначенияУХ.ЗначениеРеквизитаОбъекта(Раскрытие.Владелец, ИмяРеквизита);
...
Процедура ДобавитьСчетКТаблице(ТипДляЭлиминации, СчетНСБУ, ЗначенияПоУмолчанию)
	...
	Для каждого СтрокаСубконто Из ТЧВидыСубконто Цикл
		...
		НоваяСтрока.ПроизвольныйКлассификатор = ЕстьТипСубконто(ТипЗначенияСубконто);
...
Функция ПолучитьНастройкиЭлементаФормыЗначенияСубконто(Знач Объект, ИдентификаторСтроки)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ВидСубконтоСсылка) Тогда
		...
		Если ЕстьТипСубконто(ТипЗначенияСубконто) Тогда
		...
		ИначеЕсли мВладельцев.Количество() > 0 Тогда
			...
			Если НЕ флОтборУстановлен Тогда
				...
				ТипыРеквизитовОбъекта = ОбщегоНазначенияУХ.ТипыРеквизитовОбъекта(
...
&НаСервереБезКонтекста
Функция ЕстьТипСубконто(ТипЗначенияСубконто)

	Если Метаданные.Справочники.Найти("Субконто") <> Неопределено Тогда
		ТипСубконто = Тип("СправочникСсылка." + "Субконто");
	Иначе 
		ТипСубконто = Тип("СправочникСсылка.ПроизвольныйКлассификаторУХ");
	КонецЕсли;
	
	Возврат ТипЗначенияСубконто.СодержитТип(ТипСубконто)
