	...
	Для Каждого СтрокаТовары Из Товары Цикл
		...
		ИначеЕсли ЭтоМаркировкаВнеПроизводстваИмпорта Тогда
			...
			Если ДоступныеПотребительскиеШаблоны.НайтиПоЗначению(СтрокаТовары.Шаблон) = Неопределено Тогда
				ТекстСообщения = СтрШаблон(
					НСтр("ru = 'Для способа ввода в оборот %1 доступно указание только потребительских упаковок';
						|en = 'Для способа ввода в оборот %1 доступно указание только потребительских упаковок'"), СпособВводаВОборот);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,
					ЭтотОбъект,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
		...
		Если КонтролироватьЕМРЦ Тогда
			Если СтрокаТовары.МаксимальнаяРозничнаяЦена < ЕМРЦ Тогда
				ТекстСообщения = СтрШаблон(НСтр(
					...
					|en = 'В строке %1 списка ""Товары"" значение МРЦ меньше единой минимальной цены на табачные изделия равной %2 руб.'"),
