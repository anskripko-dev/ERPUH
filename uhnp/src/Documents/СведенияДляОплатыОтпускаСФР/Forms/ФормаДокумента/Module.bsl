...
Процедура ВложенияПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		ПоказатьВопрос(Оповещение, 
			НСтр("ru = 'Выполнение действия возможно только после записи данных. Документ будут записан.';
				|en = 'You can perform the action only after the data is saved. The document will be saved.'"), РежимДиалогаВопрос.ОКОтмена);
	Иначе
		ДобавитьФайл();
	КонецЕсли;
КонецПроцедуры
...
Функция СостояниеОтправки()
	...
	ИначеЕсли Состояние = ВариантыСостояний.ТребуетсяСоздать Тогда
		Представление = НСтр("ru = '<a href=""1"">Требуется предоставление недостающих документов</a>';
							|en = '<a href=""1"">Provide the missing documents</a>'");
	...
	ИначеЕсли Состояние = ВариантыСостояний.Принят Тогда
		Представление = НСтр("ru = '<a href=""1"">На рассмотрении СФР</a>';
							|en = '<a href=""1"">On Social Insurance Fund review</a>'");
	ИначеЕсли Состояние = ВариантыСостояний.НеПринят И РегистрацияСведений.РегистрацияСтатус = "VACATIONRETURN" Тогда
		Представление = НСтр("ru = 'Рассмотрен и возвращен страхователю , см. <a href=""1"">протокол</a>';
							|en = 'Reviewed and returned to the insurant. See the <a href=""1"">protocol</a>'");
...
Процедура ОбновитьЭлементыГруппыСведенияОЗаработке(Группа, Представление, Заполнена, Развернута, Видимость)
	...
	Если Не Развернута Тогда
		...
		Если ЕстьСуммаПремии Тогда
			Представления.Добавить(СтрШаблон(НСтр("ru = 'Премия %1';
													|en = 'Bonus %1'"), Объект.СуммаПремии));
		КонецЕсли;
		Если ЕстьЗаработокВключаяПремию Тогда
			Представления.Добавить(СтрШаблон(НСтр("ru = 'Заработок включая премию %1';
													|en = 'Earnings including bonuses %1'"), Объект.ЗаработокВключаяПремию));
		КонецЕсли;
		Если ЕстьСреднийДневнойЗаработок Тогда
			Представления.Добавить(СтрШаблон(НСтр("ru = 'Средний дневной заработок %1';
													|en = 'Average daily earnings %1'"), Объект.СреднийДневнойЗаработок));
		КонецЕсли;
		Если ЕстьСуммаКВыплате Тогда
			Представления.Добавить(СтрШаблон(НСтр("ru = 'К выплате %1';
													|en = 'To be paid %1'"), Объект.СуммаКВыплате));
		КонецЕсли;
		Представление = СтрСоединить(Представления, ", ");
	КонецЕсли;
КонецПроцедуры
...
Процедура ДобавитьСтрокуВложения(Файл)
	...
	Иначе
		...
		ПоказатьОповещениеПользователя(СтрШаблон(НСтр("ru = 'Файл %1 уже выбран.';
														|en = 'The %1 file is already selected.'"), НоваяСтрока.Файл));
	КонецЕсли;
КонецПроцедуры
