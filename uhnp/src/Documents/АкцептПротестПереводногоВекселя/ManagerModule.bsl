...
Процедура ОбновитьРасходНаСервере(Объект) Экспорт
	...
	ТабГрафик = ЦенныеБумагиУХ.ПолучитьГрафикНачисленияДоходаВексель(Объект, Объект.Дата);
...
Функция ТекстЗапросаРеквизитыДокумента(НомераТаблиц = Неопределено)
	...
	|	Реквизиты.ОбъектРасчетовУХ КАК ОбъектРасчетовУХ,
...
Функция ТекстЗапросаПараметрыОпераций(НомераТаблиц)
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	АкцептПротестПереводногоВекселяПараметрыОпераций.ОперацияГрафика КАК ОперацияГрафика,
	|	АкцептПротестПереводногоВекселяПараметрыОпераций.СтатьяБюджета КАК СтатьяБюджета,
	|	АкцептПротестПереводногоВекселяПараметрыОпераций.Аналитика1 КАК Аналитика1,
	|	АкцептПротестПереводногоВекселяПараметрыОпераций.Аналитика2 КАК Аналитика2,
	|	АкцептПротестПереводногоВекселяПараметрыОпераций.Аналитика3 КАК Аналитика3,
	|	АкцептПротестПереводногоВекселяПараметрыОпераций.Аналитика4 КАК Аналитика4,
	|	АкцептПротестПереводногоВекселяПараметрыОпераций.Аналитика5 КАК Аналитика5,
	|	АкцептПротестПереводногоВекселяПараметрыОпераций.Аналитика6 КАК Аналитика6
	|ПОМЕСТИТЬ ВТ_ПараметрыОпераций
	|ИЗ
	|	Документ.АкцептПротестПереводногоВекселя.ПараметрыОпераций КАК АкцептПротестПереводногоВекселяПараметрыОпераций
	|ГДЕ
	|	АкцептПротестПереводногоВекселяПараметрыОпераций.Ссылка = &Ссылка";
...
Функция ТекстЗапроса_ТаблицаГрафикИсходный(НомераТаблиц)
	...
	|		ПО РасчетыСКонтрагентамиГрафики.ВерсияГрафика = ТаблицаВекселя.Регистратор
...
Функция ТекстЗапросаРасчетыСКонтрагентамиФакт(НомераТаблиц, ИмяТаблицы)
	...
	|	втРеквизиты.ОбъектРасчетовУХ КАК ОбъектРасчетов,
