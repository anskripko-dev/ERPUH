...
Функция ТекстСообщенияУстаревшийДокумент() Экспорт
	...
	Возврат НСтр("ru = 'Документ ""Сверка взаиморасчетов (с фин.инструментами)"" устарел и будет удален.
				...
				|en = 'The ""AR/AP reconciliation (with financial instruments)"" document is outdated and will be deleted.
				|We recommend that you use the ""AR/AP reconciliation (2.5.11)"" document'");
		...
		Иначе
			...
			Иначе
				ПредставлениеПериода = СтрШаблон(НСтр("ru = 'за %1';
														|en = '%1 before'", ОбщегоНазначения.КодОсновногоЯзыка()), ПредставлениеПериода);
			...
			Если СтрокаТаблицы.ИзменятьДокумент = "УдалитьСоздатьНовый" Тогда
				...
				Исключение
					ТекстОшибки    = НСтр("ru = 'Не удалось пометить на удаление %Документ%. %ОписаниеОшибки%';
											|en = 'Cannot mark %Документ% for deletion. %ОписаниеОшибки%'");
			...
			Если СФДокумента.Количество() > 0 Тогда
				...
				ОписаниеСФ = СтрШаблон("(" + ШаблонДокумента + ")",
										НСтр("ru = 'Счет-фактура';
											|en = 'Tax invoice'"), СокрЛП(НомерНаПечать), Формат(СФ.Дата, "ДЛФ=D"));
...
Функция УстаревшиеПредставленияДокументов()
	...
	ВидыДокументовКонтрагента.Вставить("АвансовыйОтчет",                        НСтр("ru = 'Приходный кассовый ордер';
																					|en = 'Incoming payment — Cash account'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ПредставленияВладельцевДокументов()
	...
	ВидыДокументовКонтрагента.Вставить("АвансовыйОтчет",                        НСтр("ru = 'получатель';
																					|en = 'recipient'", ОбщегоНазначения.КодОсновногоЯзыка()));
