...
Процедура ФинансовыйИнструментПриИзмененииНаСервере()
	...
	ЦенныеБумагиКлиентСерверУХ.УстановитьПараметрыЦеннойБумаги(ЭтотОбъект);
...
Процедура ЗаполнитьДокументНаСервере(СтруктураДействий = Неопределено)
	...
	Если ЗначениеЗаполнено(Объект.ФинансовыйИнструмент) Тогда
		...
		Если ЗначениеЗаполнено(РеквизитыФИ.Количество) И РеквизитыФИ.Количество > 0 Тогда
			Объект.Количество = РеквизитыФИ.Количество;
		КонецЕсли;
		
		Если СтруктураДействий.Свойство("РассчитатьСтоимость") Тогда
			Объект.Стоимость = ?(РеквизитыФИ.ДисконтнаяЦБ = Истина, РеквизитыФИ.ЦенаВыпуска, РеквизитыФИ.Номинал) * Объект.Количество;
		КонецЕсли;
		
		ЭтотОбъект.СтоимостьНоминала = ?(РеквизитыФИ.ДисконтнаяЦБ, РеквизитыФИ.ЦенаВыпуска, РеквизитыФИ.Номинал) * Объект.Количество;
		НоминалЦБ = ?(РеквизитыФИ.ДисконтнаяЦБ = Истина, РеквизитыФИ.ЦенаВыпуска, РеквизитыФИ.Номинал);
		Объект.Премия = Объект.Стоимость - ?(ЗначениеЗаполнено(НоминалЦБ), НоминалЦБ, 0) * Объект.Количество;
		
		Если ЗначениеЗаполнено(РеквизитыФИ.ДатаВыпуска) И (Объект.Дата > КонецДня(РеквизитыФИ.ДатаВыпуска)) Тогда
