...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.Основание) = Тип("СправочникСсылка.ДоговорыКонтрагентов") И ЗначениеЗаполнено(Параметры.Основание) Тогда
		ВстраиваниеУХЗаказКлиента.ДобавитьИЗаполнитьРеквизитФормыДоговорОснование(ЭтотОбъект, Параметры.Основание); 
	КонецЕсли;
	#КонецОбласти
...
&НаКлиенте
Процедура ЗаполнитьДоговорОснование()
	
	Если ЗаполнитьДоговорОснованиеНаСервере() Тогда
...
&НаСервере
Функция ЗаполнитьДоговорОснованиеНаСервере()
	
	ЭлементДоговор = Элементы.Найти("Договор");
	
	Если ЭлементДоговор = Неопределено тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, "ДоговорОснование")
		И ЗначениеЗаполнено(ЭтотОбъект["ДоговорОснование"]) 
		И Не ЗначениеЗаполнено(Объект.Договор)
		И ЭлементДоговор.Видимость Тогда
	
		РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЭтотОбъект["ДоговорОснование"],"ВалютаВзаиморасчетов,Контрагент,Организация,Партнер");
		
		Если РеквизитыДоговора.ВалютаВзаиморасчетов = Объект.Валюта
			И РеквизитыДоговора.Контрагент = Объект.Контрагент
			И РеквизитыДоговора.Организация = Объект.Организация
			И РеквизитыДоговора.Партнер = Объект.Партнер Тогда
		
			Объект.Договор = ЭтотОбъект["ДоговорОснование"];  
			Возврат Истина;
		КонецЕсли;
