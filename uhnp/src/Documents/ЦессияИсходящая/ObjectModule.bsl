...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ПараметрыПроведения = Документы.ЦессияИсходящая.ПодготовитьПараметрыПроведения(Ссылка, Отказ);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ ЗначениеЗаполнено(Партнер) И ЗначениеЗаполнено(Контрагент) Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ТекущаяЗадолженность.Итог("Сумма") <> СуммаОбязательства Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "СуммаОбязательства", "Объект", Отказ);
	
	КонецЕсли;     
	
	Если ВидОбязательства = Перечисления.ВидыОбязательств.ЗадолженностьПоДоговоруЗайма Тогда
...
Процедура ПриЗаписи(Отказ)
	...
	Если Отказ ИЛИ НЕ Проведен Тогда
		Возврат;		
	КонецЕсли; 
	
	ЦессииУХ.СоздатьВходящуюЦессиюВГО(Ссылка);
