...
Процедура ПересчитатьСекциюГрафика(ИмяСекции, ОписаниеГрафика, ОперацииГрафика, ДопПараметры = Неопределено, Отказ = Ложь) Экспорт
	...
	Если ИмяСекции = "ОсновнойДолг" Тогда
		ФинансовыеИнструментыУХ.НачальноеЗаполнениеОсновногоДолга(
			ЭтотОбъект, ИмяСекции, ОписаниеГрафика, ОперацииГрафика);
	ИначеЕсли ИмяСекции = "Проценты" Тогда
		ФинансовыеИнструментыУХ.ПересчитатьСекциюГрафика(
			ЭтотОбъект, ИмяСекции, ОписаниеГрафика, ОперацииГрафика, ДопПараметры);
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЗначениеЗаполнено(СтруктураОграничений) Тогда
		...
		Если Сумма < СтруктураОграничений.МинимальнаяСуммаСделки Тогда
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Размещение средств возможно в размере не менее %1 %3, текущая сумма сделки %2 %3'"), 
									СтруктураОграничений.МинимальнаяСуммаСделки, Сумма, ВалютаВзаиморасчетов);
		...
		Если Сумма > СтруктураОграничений.МаксимальнаяСуммаСделки И СтруктураОграничений.МаксимальнаяСуммаСделки <> 0 Тогда
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Размещение средств возможно в размере не более %1 %3, текущая сумма сделки %2 %3'"), 
									СтруктураОграничений.МаксимальнаяСуммаСделки, Сумма, ВалютаВзаиморасчетов);
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки,,,,Отказ);
		КонецЕсли;
