...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(Менеджер) Тогда
		...
		Если НекорректныеРеквизитыМенеджера.Количество() > 0 Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Для менеджера %1 не указаны: %2.';
											|en = '%1 is not specified for manager %2.'"), Менеджер,
	...
	Если Не ЗначениеЗаполнено(Склад) Тогда
		Если ТипЗнч(Склад) = Тип("Строка") Тогда
			ТекстСообщения = НСтр("ru = 'Поле ""Адрес доставки"" не заполнено.';
									|en = 'Поле ""Адрес доставки"" не заполнено.'");
		Иначе
			ТекстСообщения = НСтр("ru = 'Поле ""Склад"" не заполнено.';
									|en = 'Поле ""Склад"" не заполнено.'");
		КонецЕсли;
