...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого ТекВНА Из ЭтотОбъект.ВНА Цикл
		...
		Если Не ЗначениеЗаполнено(ТекВНА.ГруппаВНА) 
			Или Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекВНА.ГруппаВНА, "НачислятьАмортизацию") Тогда 
			Продолжить;
...
Процедура ИнициализироватьДокумент(ДокументИсточник = Неопределено)
	...
	Если ДокументИсточник <> Неопределено Тогда
		...
		СвойстваЗаполнения = "Организация,Сценарий,ТипВНА";
