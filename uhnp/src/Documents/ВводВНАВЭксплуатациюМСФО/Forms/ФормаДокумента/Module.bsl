...
Процедура КомментарийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	МСФОКлиентУХ.ОткрытьФормуРедактированияМногострочногоТекста(
		ЭтаФорма, Элемент.ТекстРедактирования, Объект.Комментарий, Модифицированность);
...
Процедура ОтобратьСтрокиРасхождений(Команда)
	...
	Если (Элементы.ВНА.ОтборСтрок = Неопределено)
		ИЛИ НЕ Элементы.ВНА.ОтборСтрок.Свойство("ЕстьРасхождения") Тогда
...
Процедура ОтобратьСтрокиИзменений(Команда)
	...
	Если (Элементы.ВНА.ОтборСтрок = Неопределено)
		ИЛИ НЕ Элементы.ВНА.ОтборСтрок.Свойство("ЕстьИзменение") Тогда
...
Процедура ВНАПараметрыАмортизацииВНАМСФОПриИзменении(Элемент)
	...
	МСФОКлиентСерверУХ.ОбновитьЗависимыеРеквизиты(
		Элементы.ВНА.ТекущиеДанные, ЭтаФорма, КолонкаИсточник);
...
Процедура ВНАСчетПоступленияМСФОПриИзменении(Элемент)
	...
	ВидимостьВалютнаяСумма(ЭтотОбъект, Элементы.ВНА.ТекущиеДанные);
...
Процедура ВНАПриАктивизацииСтроки(Элемент)
	...
	МСФОКлиентСерверУХ.ОбновитьВидимостьСубконтоГруппыПодробно(ЭтотОбъект, ТекущаяСтрока, "ВНА");
	МСФОКлиентСерверУХ.ОбновитьДоступностьЗависимыхРеквизитов(ЭтотОбъект, ТекущаяСтрока, "ВНА");

	ОбновитьВидимостьПараметровАмортизации(ТекущаяСтрока);
	МСФОКлиентСерверУХ.ВидимостьПодробно(ЭтотОбъект, Ложь, "ВНА");
	ВидимостьВалютнаяСумма(ЭтотОбъект, ТекущаяСтрока);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ВидимостьВалютнаяСумма(Форма, ТекущаяСтрока)
	
	Элементы = Форма.Элементы;
	
	СвойстваКоррСчета = УчетМСФОПовтИспУХ.ПолучитьСвойстваСчета(ТекущаяСтрока.СчетПоступленияМСФО);
	Элементы.ВНАВалюта.Видимость = СвойстваКоррСчета.Валютный;
	Элементы.ВНАСуммаВВалютеНСБУ.Видимость = СвойстваКоррСчета.Валютный;
...
Процедура ЗаполнитьДокумент(ДанныеЗаполнения)
	...
	МСФОУХ.ЗаполнитьПоКолонкамНСБУСервер(ЭтотОбъект);
	МСФОКлиентСерверУХ.ОбновитьВидимостьДокументыОснования(ЭтотОбъект);
	МСФОКлиентСерверУХ.ЗаполнитьНачислятьАмортизациюМСФО(ЭтотОбъект);
...
Процедура ОчиститьТаблицы()
	...
	МСФОУХ.ОчиститьТаблицы(ЭтотОбъект);
...
Процедура ЗаполнитьТекущимиДаннымиСервер(ЗаполнитьРеквизиты = Ложь, ЗаполнитьНСБУ = Ложь, ЗаполнитьМСФО = Ложь, СтрокаВНА = Ложь) 
	...
	Иначе
		ТекСтрока = Объект.ВНА.НайтиПоИдентификатору(Элементы.ВНА.ТекущаяСтрока);
		СтрокиДляЗаполнения = Новый Массив;
		СтрокиДляЗаполнения.Добавить(ТекСтрока);
	КонецЕсли;
	
	ПараметрыЗаполнения = Новый Структура(
		"ЗаполнитьРеквизиты, ЗаполнитьНСБУ, ЗаполнитьМСФО",
		ЗаполнитьРеквизиты, ЗаполнитьНСБУ, ЗаполнитьМСФО);
	
	Документы[ЭтаФорма.КэшируемыеЗначения.МетаданныеДокумента.Имя]
		.ЗаполнитьСтроки(СтрокиДляЗаполнения, ЭтаФорма, ПараметрыЗаполнения);
