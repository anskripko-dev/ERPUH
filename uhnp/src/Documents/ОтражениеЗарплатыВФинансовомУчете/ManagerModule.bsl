...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Обновление документов ""Отражение зарплаты в финансовом учете"":';
								|en = 'Update the ""Post payroll expenses to accounting"" documents:'"));
	СписокОписаний.Добавить(НСтр("ru = '- заполнение вида резерва в табличной части ""Выплата за счет резервов"";';
								|en = '- Fill the reserve kind in the ""Payment from payroll funds"" table.'"));
	СписокОписаний.Добавить(НСтр("ru = '- заполнение колонки ""Тип налога"" и очистка колонки ""Тип налога (Не используется)"" в табличной части ""НДФЛ"";';
								|en = '- Fill the ""Tax type"" column and clear the ""(not used) Tax type"" column in the ""PIT"" table.'"));
	СписокОписаний.Добавить(НСтр("ru = '- заполнение колонки ""Аналитика"" из справочника ""Виды налогов и взносов"" в табличной части ""Удержания"";';
								|en = '- Fill the ""Dimension"" column from the ""Tax and contribution kinds"" catalog in the ""Deductions"" table.'"));
...
Функция СобратьДанныеЧеков(ДокументСсылка) Экспорт
	...
	РеквизитыКассира = РозничныеПродажи.РеквизитыКассира(Кассир, ДанныеЧека.Организация, ТекущаяДатаСеанса());
	Если ЗначениеЗаполнено(РеквизитыКассира.Должность) Тогда
		ДанныеЧека.Вставить("Кассир", РеквизитыКассира.Наименование + " - " + РеквизитыКассира.Должность);
	Иначе
		ДанныеЧека.Вставить("Кассир", РеквизитыКассира.Наименование);
	КонецЕсли;
	ДанныеЧека.Вставить("КассирИНН", РеквизитыКассира.ИНН);
...
Функция ПараметрыОперацииФискализацииЧека(ДанныеЧека)
	...
	ПараметрыФискализации.Кассир = ДанныеЧека.Кассир;
	ПараметрыФискализации.КассирИНН = ДанныеЧека.КассирИНН;
