...
&НаКлиенте
Процедура ЗаполнитьСтрокамиПлана();
	Объект.СтрокиПланаЗакупок.Очистить();
	ИнновационныйПлан = Объект.ЗакупкаИнновационнойВысокотехнологичнойПродукцииИЛекарственныхСредств;
	ТекОрганизация = Объект.ОрганизацияДляЗаключенияДоговора;
	МассивСтрокиПланаЗакупок = ПолучитьСтрокиПланаВВалютеДокумента(ИнновационныйПлан, ТекОрганизация, Объект.ПериодЗакупок, Объект.ВалютаДокумента);
...
Функция СформироватьНастройкиОтчетаПрограммаЗакупок(ПериодВход, ОрганизацияВход, ИнновационныйПланВход, ПланЗакупки)
	// Инициализация.
	...
	// ПериодЗакупки.
	ПараметрПериодНачала = Новый ПараметрКомпоновкиДанных("ПериодЗакупки");
	ЭлементНастройки = Настройки.ПараметрыДанных.НайтиЗначениеПараметра(ПараметрПериодНачала);
	ЭлементНастройки.Значение = ПериодВход;
	ИдентификаторНастройки = ЭлементНастройки.ИдентификаторПользовательскойНастройки;
	Если ЗначениеЗаполнено(ИдентификаторНастройки) Тогда
	...
	// ПланЗакупки.
	ПараметрПериодНачала = Новый ПараметрКомпоновкиДанных("ПланЗакупки");
	ЭлементНастройки = Настройки.ПараметрыДанных.НайтиЗначениеПараметра(ПараметрПериодНачала);
	ЭлементНастройки.Значение = ПланЗакупки;
	ИдентификаторНастройки = ЭлементНастройки.ИдентификаторПользовательскойНастройки;
	Если ЗначениеЗаполнено(ИдентификаторНастройки) Тогда
		ПараметрСКД = КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(ИдентификаторНастройки);
		Если ТипЗнч(ПараметрСКД) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
...
Процедура ОтчетПрограммаЗакупок(Команда)      
	Если ЭтотОбъект.Модифицированность Тогда 
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ОтчетПрограммаЗакупокЗавершение", ЭтотОбъект), ТекстВопроса, РежимДиалогаВопрос.ОКОтмена);   
	Иначе
		ОтчетПрограммаЗакупокЗавершение(КодВозвратаДиалога.ОК, Новый Структура);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ОтчетПрограммаЗакупокЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт 
	...
	Если РезультатВопроса = КодВозвратаДиалога.ОК Тогда  
		...
		СтруктураНастроек = СформироватьНастройкиОтчетаПрограммаЗакупок(Объект.ПериодЗакупок, 
		...
		СтруктураПараметров.Вставить("ПланЗакупки", 				Объект.Ссылка);  
...
&НаКлиенте
Процедура СоздатьСтрокуПлана(Команда)
	Если НЕ ЭтотОбъект.Модифицированность Тогда
		ОписаниеОповещенияОЗакрытии = 
		Новый ОписаниеОповещения("ОбработкаЗакрытияФормыСозданияСтрокиПлана", ЭтаФорма);
		Структура = Новый Структура();
		Структура.Вставить("ПрограммаЗакупок", Объект.Ссылка);
    	ОткрытьФорму("Документ.СтрокаПланаЗакупок.Форма.ФормаДокумента",Структура,ЭтаФорма,,,,ОписаниеОповещенияОЗакрытии,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ЗаписатьПрограммуЗакупок", ЭтотОбъект),
...
&НаКлиенте
Процедура ОбработкаЗакрытияФормыСозданияСтрокиПлана(Результат, ДопПараметры) Экспорт
	ЗаполнитьСтрокамиПлана();
КонецПроцедуры

&НаКлиенте
Процедура ЗаписатьПрограммуЗакупок(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаСервере
Процедура ОбновитьПараметрыЗакупкиМСПНаСервере()
	ПараметрыЗакупкиМСП = Документы.ПрограммаЗакупок.ПараметрыЗакупкиМСП(Объект.Ссылка);    
	Документы.ПрограммаЗакупок.ОбновитьПараметрыЗакупкиМСПНаСервере(Объект, ПараметрыЗакупкиМСП);
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьПараметрыЗакупкиМСП(Команда) 
	
	Если ЭтотОбъект.Модифицированность Тогда 
		
		ТекстВопроса = НСтр("ru = 'Данные еще не записаны.
			|Данные будут записаны.'");
		ПоказатьВопрос(Новый ОписаниеОповещения("ОбновитьПараметрыЗакупкиМСПЗавершение", ЭтотОбъект), ТекстВопроса, РежимДиалогаВопрос.ОКОтмена);
		
	Иначе      
		
		ОбновитьПараметрыЗакупкиМСПЗавершение(КодВозвратаДиалога.ОК, Новый Структура);
...
&НаКлиенте
Процедура ОбновитьПараметрыЗакупкиМСПЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.ОК Тогда  
		...
		ОбновитьПараметрыЗакупкиМСПНаСервере();  
...
&НаСервере
Функция ПутьЭлемента(ИмяЭлемента)
...
&НаКлиенте
Процедура ПроверитьЗначениеПоляУчастияМСП(Знач Элемент)
	
	Перем Значение, ПутьЭлемента;
	
	ПутьЭлемента = ПутьЭлемента(Элемент.Имя);
	Значение = Объект[СтрЗаменить(ПутьЭлемента, "Объект.", "")];
	Если Значение > ЭтотОбъект.СуммаДокумента Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Значение не может превышать сумму документа'"), Объект.Ссылка,, ПутьЭлемента); 
		Объект[СтрЗаменить(ПутьЭлемента, "Объект.", "")] = ЭтотОбъект.СуммаДокумента;
	КонецЕсли;

КонецПроцедуры  

&НаКлиенте
Процедура ОбъемЗакупокИнновационныхПриИзменении(Элемент)     
	ПроверитьЗначениеПоляУчастияМСП(Элемент);
КонецПроцедуры

&НаКлиенте
Процедура ОбъемЗакупокИсключаемыхМСППриИзменении(Элемент)
	ПроверитьЗначениеПоляУчастияМСП(Элемент);
КонецПроцедуры

&НаКлиенте
Процедура ГодовойОбъемЗакупокМСППриИзменении(Элемент)
	ПроверитьЗначениеПоляУчастияМСП(Элемент);
КонецПроцедуры

&НаКлиенте
Процедура ОбъемЗакупокИнновационныхИсключаемыхПриИзменении(Элемент)
	ПроверитьЗначениеПоляУчастияМСП(Элемент);
КонецПроцедуры

&НаКлиенте
Процедура ОбъемЗакупокИнновационныхМСППриИзменении(Элемент)
	ПроверитьЗначениеПоляУчастияМСП(Элемент);
КонецПроцедуры

&НаКлиенте
Процедура ОбъемЗакупокИнновационныхОтчетныйПриИзменении(Элемент)
	ПроверитьЗначениеПоляУчастияМСП(Элемент);
КонецПроцедуры
