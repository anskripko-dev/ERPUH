...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ЭтотОбъект.ПериодЗакупокОкончание = ЭтотОбъект.ПериодЗакупокОкончание;
...
Процедура ПроверитьПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение ИЛИ НЕ ПометкаУдаления Тогда
		...
		Если ЗначениеЗаполнено(СсылкаНаАналог) Тогда
			...
			ОбщегоНазначенияУХ.СообщитьОбОшибке(ТекстСообщения,	Отказ, , СтатусСообщения.Важное);
		КонецЕсли;   
		Отказ = ПроверкаПоследовательностиНомеровГПЗ(Отказ);
...
Функция ПроверкаПоследовательностиНомеровГПЗ(Отказ)
	
	Если Отказ Тогда 
		Возврат Отказ;
	КонецЕсли; 
	КолСтрок = СтрокиПланаЗакупок.Количество() + СтрокиПланаВалютные.Количество();
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ПрограммаЗакупокСтрокиПланаЗакупок",СтрокиПланаЗакупок.Выгрузить());
	Запрос.УстановитьПараметр("ПрограммаЗакупокСтрокиПланаЗакупокВалютные",СтрокиПланаВалютные.Выгрузить());
	Запрос.Текст =  "ВЫБРАТЬ
	                |	ВЫРАЗИТЬ(ПрограммаЗакупокСтрокиПланаЗакупок.СтрокаПланаЗакупок КАК Документ.СтрокаПланаЗакупок) КАК СтрокаПланаЗакупок
	                |ПОМЕСТИТЬ ВТ
	                |ИЗ
	                |	&ПрограммаЗакупокСтрокиПланаЗакупок КАК ПрограммаЗакупокСтрокиПланаЗакупок
	                |;
	                |
	                |////////////////////////////////////////////////////////////////////////////////
	                |ВЫБРАТЬ
	                |	ВЫРАЗИТЬ(ПрограммаЗакупокСтрокиПланаЗакупокВалютные.СтрокаПланаЗакупок КАК Документ.СтрокаПланаЗакупок) КАК СтрокаПланаЗакупок
	                |ПОМЕСТИТЬ ВТ1
	                |ИЗ
	                |	&ПрограммаЗакупокСтрокиПланаЗакупокВалютные КАК ПрограммаЗакупокСтрокиПланаЗакупокВалютные
	                |;
	                |
	                |////////////////////////////////////////////////////////////////////////////////
	                |ВЫБРАТЬ
	                |	ВТ.СтрокаПланаЗакупок.НомерВГПЗ КАК НомерВГПЗ
	                |ИЗ
	                |	ВТ КАК ВТ
	                |
	                |ОБЪЕДИНИТЬ ВСЕ
	                |
	                |ВЫБРАТЬ
	                |	ВТ1.СтрокаПланаЗакупок.НомерВГПЗ
	                |ИЗ
	                |	ВТ1 КАК ВТ1"; 
	ВыгрузкаНомеров = Запрос.Выполнить().Выгрузить();
	Индекс = 1; 
	Пока  Индекс <= КолСтрок Цикл 
		Если  ВыгрузкаНомеров.НайтиСтроки(Новый Структура("НомерВГПЗ",Индекс)).Количество() = 0 Тогда
			ТекстСообщения = НСтр("ru = 'Нарушена последовательная нумерация ГПЗ в выгружаемых строках'");
			ОбщегоНазначенияУХ.СообщитьОбОшибке(ТекстСообщения);
			Отказ = Истина;
			Возврат Отказ;	
		КонецЕсли;	
