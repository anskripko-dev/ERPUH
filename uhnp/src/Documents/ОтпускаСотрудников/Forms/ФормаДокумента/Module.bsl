...
Процедура ЗаполнитьПоГрафикуОтпусковНаСервере(Результат)
	...
	Для Каждого ДанныеГрафика Из ГрафикОтпусков Цикл                                                                                      
		Если ДанныеГрафика.ИсходныйДокумент <> Неопределено тогда                                                                        
			ОписаниеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 - %2';
																							|en = '%1 - %2'"),Формат(ДанныеГрафика.ДатаНачала, ФорматДаты), Формат(ДанныеГрафика.ДатаОкончания, ФорматДаты));
			ТекстСообщения = НСтр("ru = 'По сотруднику %1 исключен период %2, учтенный в документе %3';
									|en = 'For the %1 employee, the %2 period included in the %3 document is excluded.'");
