...
Процедура ЗаполнитьПоОстаткамЗаказов(ДанныеОтбора, Товары, СкладПоступления, МассивЗаказов = Неопределено, ПроверятьПорядокРасчетов = Истина) Экспорт
	...
	ТаблицаПараметров.Колонки.Добавить("КодСтроки",Новый ОписаниеТипов("Число"));  
	ТаблицаПараметров.Колонки.Добавить("ЗаказПоставщику",Новый ОписаниеТипов("ДокументСсылка.ЗаказПоставщику"));	
	#КонецОбласти
	...
	Пока ВыборкаТовары.Следующий() Цикл
		...
		#Область УХ_Встраивание	
		ДобСтр = ТаблицаПараметров.Добавить();
		ДобСтр.КодСтроки = ВыборкаТовары.КодСтроки;  
		ДобСтр.ЗаказПоставщику = ВыборкаТовары.ЗаказПоставщику;
		#КонецОбласти
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	ТаблицаПараметров.КодСтроки КАК КодСтроки,
	               |	ТаблицаПараметров.ЗаказПоставщику КАК ЗаказПоставщику
	               ...
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаСтрок КАК ТаблицаСтрок
	               |		ПО ЗаказПоставщикуТовары.КодСтроки = ТаблицаСтрок.КодСтроки
	               |			И ЗаказПоставщикуТовары.Ссылка = ТаблицаСтрок.ЗаказПоставщику";
	...
	Пока ВыборкаТовары.Следующий() Цикл
		...
		Отбор.Вставить("Ссылка", ВыборкаТовары.ЗаказПоставщику);
	    Отбор.Вставить("КодСтроки",ВыборкаТовары.КодСтроки);
		
		Строки = ТаблицаАналитик.НайтиСтроки(Отбор);
		Если Строки.Количество()>0 И 
			((ТИП(Товары) = ТИП("ДанныеФормыКоллекция") И СтрокаТаб.Свойство("ЦФО"))
			ИЛИ (ТИП(Товары) = ТИП("ДокументТабличнаяЧасть.ПриобретениеТоваровУслуг.Товары"))) Тогда
			СтрокаАналитик = Строки[0];	
			СтрокаТаб.ЦФО = СтрокаАналитик.ЦФО;
			СтрокаТаб.Проект = СтрокаАналитик.Проект;
			
			СтрокаТаб.СтатьяБюджетаБДДС = СтрокаАналитик.СтатьяБюджетаБДДС;
			СтрокаТаб.АналитикаБДДС1 = СтрокаАналитик.АналитикаБДДС1;
			СтрокаТаб.АналитикаБДДС2 = СтрокаАналитик.АналитикаБДДС2;
			СтрокаТаб.АналитикаБДДС3 = СтрокаАналитик.АналитикаБДДС3;
			СтрокаТаб.АналитикаБДДС4 = СтрокаАналитик.АналитикаБДДС4;
			СтрокаТаб.АналитикаБДДС5 = СтрокаАналитик.АналитикаБДДС5;
			СтрокаТаб.АналитикаБДДС6 = СтрокаАналитик.АналитикаБДДС6;
			СтрокаТаб.СтатьяБюджетаБДР = СтрокаАналитик.СтатьяБюджетаБДР;
			СтрокаТаб.АналитикаБДР1 = СтрокаАналитик.АналитикаБДР1;
			СтрокаТаб.АналитикаБДР2 = СтрокаАналитик.АналитикаБДР2;
			СтрокаТаб.АналитикаБДР3 = СтрокаАналитик.АналитикаБДР3;
			СтрокаТаб.АналитикаБДР4 = СтрокаАналитик.АналитикаБДР4;
			СтрокаТаб.АналитикаБДР5 = СтрокаАналитик.АналитикаБДР5;
			СтрокаТаб.АналитикаБДР6 = СтрокаАналитик.АналитикаБДР6;      
		КонецЕсли;
		#КонецОбласти
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, СкладПоступления = Неопределено,
	...
	Иначе
		...
		ТоварыНакладной.Колонки.Добавить("СтранаПроизводителя", Новый ОписаниеТипов("СправочникСсылка.СтраныМира"));
		ТоварыНакладной.Колонки.Добавить("СтранаПроисхождения223ФЗ", Новый ОписаниеТипов("СправочникСсылка.СтраныМира"));
		#КонецОбласти
