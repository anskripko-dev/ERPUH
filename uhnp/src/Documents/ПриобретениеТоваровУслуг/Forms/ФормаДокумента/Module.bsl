...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	#Область УХ_Закупки
	Если ИмяСобытия = "ПеренестиДанныеСтатейПоступления" Тогда         
		Модифицированность = Истина;
		...
		Если ТекДанные <> Неопределено Тогда
			...
			ТекДанные.ЦФО = Параметр.ЦФО;  		
			ТекДанные.Проект = Параметр.Проект;  		
			           
			ТекДанные.УХ_СтатьиИАналитики = Строка(ТекДанные.СтатьяБюджетаБДР)+";"+ Строка(ТекДанные.СтатьяБюджетаБДДС);
		КонецЕсли;	 
	КонецЕсли;
	#КонецОбласти
КонецПроцедуры
...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элементы.Товары.ТекущиеДанные <> Неопределено Тогда
		...
		#Область УХ_Встраивание  		
		ТекДанные = Элементы.Товары.ТекущиеДанные;
		Если Поле.Имя = "ТоварыУХ_СтатьиИАналитики" Тогда  
			...
			СтруктураАналитик.Вставить("АналитикаБДР6", ТекДанные.АналитикаБДР6);   
			СтруктураАналитик.Вставить("ЦФО",ТекДанные.ЦФО);
			СтруктураАналитик.Вставить("Проект",ТекДанные.Проект);
			
			ОткрытьФорму("Документ.ПриобретениеТоваровУслуг.Форма.УХ_ФормаСтатейИАналитик",СтруктураАналитик,ЭтаФорма,,,,,);     
		КонецЕсли;  
		#КонецОбласти 
	КонецЕсли;
...
&НаСервере
Процедура УХ_СоздатьРеквизитыАналитик()
	
	УХ_СтатьиИАналитики = Новый РеквизитФормы("УХ_СтатьиИАналитики",Новый ОписаниеТипов("Строка"),"Объект.Товары","Статьи и аналитики");
	Реквизиты = Новый Массив;
	Реквизиты.Добавить(УХ_СтатьиИАналитики);
	ИзменитьРеквизиты(Реквизиты);
	ТоварыУХ_СтатьиИАналитики = Элементы.Добавить("ТоварыУХ_СтатьиИАналитики",ТИП("ПолеФормы"),Элементы.Товары);
	ТоварыУХ_СтатьиИАналитики.ПутьКДанным = "Объект.Товары.УХ_СтатьиИАналитики";
	ТоварыУХ_СтатьиИАналитики.Вид = ВидПоляФормы.ПолеНадписи; 
	Элементы.Переместить(ТоварыУХ_СтатьиИАналитики,Элементы.Товары,Элементы.ТоварыСуммаНДС);
...
Процедура  УХ_УстановитьТекстСтатейИАналитик()
	...
	Если ПланированиеПотребностей.ИспользуемыеВидыБюджетовЗакупок(Объект.Дата) = Неопределено Тогда
		Элементы.ТоварыУХ_СтатьиИАналитики.Видимость = Ложь; 
	Иначе 
		Элементы.ТоварыУХ_СтатьиИАналитики.Видимость = Истина;
	КонецЕсли;
	Для Каждого ТекДанные ИЗ Объект.Товары Цикл
		ТекДанные.УХ_СтатьиИАналитики =  Строка(ТекДанные.СтатьяБюджетаБДДС)+";" +Строка(ТекДанные.СтатьяБюджетаБДР);
	КонецЦикла;	
...
Процедура УстановитьУсловноеОформление()
	...
	НовыйРеквизит = Элементы.Добавить("ТоварыСтранаПроизводителя", Тип("ПолеФормы"), ТаблицаТовары);
	НовыйРеквизит.Заголовок = "Страна производителя";
	НовыйРеквизит.Вид = ВидПоляФормы.ПолеВвода;
	НовыйРеквизит.ПутьКДанным = "Объект.Товары.СтранаПроизводителя";

	НовыйРеквизит = Элементы.Добавить("ТоварыСтранаПроисхождения223ФЗ", Тип("ПолеФормы"), ТаблицаТовары);
	НовыйРеквизит.Заголовок = "Страна происхождения";
	НовыйРеквизит.Вид = ВидПоляФормы.ПолеВвода;
	НовыйРеквизит.ПутьКДанным = "Объект.Товары.СтранаПроисхождения223ФЗ";
	
	Элементы.ТоварыСтранаПроизводителя.Видимость = ПолучитьФункциональнуюОпцию("ВестиУчетПоСтранамКорпоративныеЗакупки")  ИЛИ Объект.Организация.ЗакупкаПоФЗ223;;
	Элементы.ТоварыСтранаПроисхождения223ФЗ.Видимость = (ПолучитьФункциональнуюОпцию("ВестиУчетПоСтранамКорпоративныеЗакупки")  ИЛИ Объект.Организация.ЗакупкаПоФЗ223)  И НЕ Элементы.ТоварыНомерГТДСтранаПроисхождения.Видимость;   
	#КонецОбласти
...
Процедура ПолучитьТоварыИзХранилища(АдресТоваровВХранилище)
	...
	Для Каждого СтрокаТоваров Из ТоварыИзХранилища Цикл
		...
		Если СтрокаТоваров.КодСтроки <> 0 Тогда
			СтрокаТЧТовары = УправлениеЗакупкамиВстраиваниеПереопределяемыйУХ.ДополнитьСтрокиЗаказаПоПТУ(СтрокаТЧТовары);
			СтрокаТЧТовары.УХ_СтатьиИАналитики =  Строка(СтрокаТЧТовары.СтатьяБюджетаБДДС)+";" +Строка(СтрокаТЧТовары.СтатьяБюджетаБДР);
		КонецЕсли;
		#КонецОбласти
...
Процедура РаспределитьПоЗаказам(ДобавлятьНераспределенные = Истина)
	...
	#Область УХ_Встраивание
	ТаблицаТоваров.Колонки.Добавить("ЦФО",  Новый ОписаниеТипов("СправочникСсылка.Организации"));
	ТаблицаТоваров.Колонки.Добавить("Проект",  Новый ОписаниеТипов("СправочникСсылка.Проекты"));
	ТаблицаТоваров.Колонки.Добавить("СтатьяБюджетаБДДС",  Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ВидыБюджетов"));
	ТаблицаТоваров.Колонки.Добавить("АналитикаБДДС1",  Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ВидыСубконтоКорпоративные"));
 	ТаблицаТоваров.Колонки.Добавить("АналитикаБДДС2",  Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ВидыСубконтоКорпоративные"));
	ТаблицаТоваров.Колонки.Добавить("АналитикаБДДС3",  Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ВидыСубконтоКорпоративные"));
	ТаблицаТоваров.Колонки.Добавить("АналитикаБДДС4",  Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ВидыСубконтоКорпоративные"));
	ТаблицаТоваров.Колонки.Добавить("АналитикаБДДС5",  Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ВидыСубконтоКорпоративные"));
	ТаблицаТоваров.Колонки.Добавить("АналитикаБДДС6",  Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ВидыСубконтоКорпоративные"));
	ТаблицаТоваров.Колонки.Добавить("СтатьяБюджетаБДР",  Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ВидыБюджетов"));
	ТаблицаТоваров.Колонки.Добавить("АналитикаБДР1",  Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ВидыСубконтоКорпоративные"));
 	ТаблицаТоваров.Колонки.Добавить("АналитикаБДР2",  Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ВидыСубконтоКорпоративные"));
	ТаблицаТоваров.Колонки.Добавить("АналитикаБДР3",  Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ВидыСубконтоКорпоративные"));
	ТаблицаТоваров.Колонки.Добавить("АналитикаБДР4",  Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ВидыСубконтоКорпоративные"));
	ТаблицаТоваров.Колонки.Добавить("АналитикаБДР5",  Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ВидыСубконтоКорпоративные"));
	ТаблицаТоваров.Колонки.Добавить("АналитикаБДР6",  Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ВидыСубконтоКорпоративные"));
...
Процедура Подключаемый_УХ_ПодборИзПланаПоставок(Команда)
	Если Объект.Ссылка.Пустая() Тогда
		ПоказатьВопрос(
			Новый ОписаниеОповещения("УХ_ПодборЗавершение", ЭтотОбъект),
			НСтр("ru='Необходимо записать документ перед заполнением. Продолжить?'"),
			РежимДиалогаВопрос.ДаНет);
	Иначе
		УХ_ОткрытьПодборИзПланаПоставок();
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_УХ_ЗаполнитьИзПланаПоставок(Команда)
	УХ_ОбработатьЗаполнениеИзПланаПоставок();
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_УХ_ТоварыОбработкаВыбора(Элемент,
												ВыбранноеЗначение, 
												СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ОбновляемаяСтрока = УХ_ДобавитьИзменитьСтрокуНоменклатуры(ВыбранноеЗначение);
	УХ_ЗаполнитьСлужебныеРеквизитыСтрокиТоваровНаСервере(
		ОбновляемаяСтрока.ПолучитьИдентификатор());
КонецПроцедуры
...
&НаКлиенте
Процедура УХ_ПодборЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура УХ_ОткрытьПодборИзПланаПоставок()
	ЦентрализованныеЗакупкиКлиентУХ.ОткрытьФормуПодбораИзПланаПоставок(
		Объект, Элементы.Товары);    
КонецПроцедуры

&НаКлиенте
Процедура УХ_ОбработатьЗаполнениеИзПланаПоставок()
	ТекстВопроса = "";
	ДопПараметры = Новый Структура;
	Если Объект.Ссылка.Пустая() Тогда
		ДопПараметры.Вставить("Записать", Истина);
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"УХ_ЗаполнитьПлановымиПотребностямиОтвет",
		ЭтотОбъект,
		ДопПараметры);
...
Процедура УХ_ЗаполнитьПлановымиПотребностямиОтвет(РезультатВопроса, 
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Если флЗаписан Тогда
			УХ_ЗаполнитьПоступлениеПлановымиПотребностямиНаСервере();
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Функция УХ_ДобавитьИзменитьСтрокуНоменклатуры(ЗначенияЗаполненияСтроки)
	ОтборСтрок = Новый Структура;
	ОтборСтрок.Вставить(
		"Номенклатура",
		ЗначенияЗаполненияСтроки.Номенклатура);
	...
	Если НЕ ЗначенияЗаполненияСтроки.ЭтоТоварнаяКатегория Тогда
		...
		ЗначениеСтавки = ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(ОбновляемаяСтрока.СтавкаНДС, "Ставка");
...
&НаСервере
Процедура УХ_ЗаполнитьПоступлениеПлановымиПотребностямиНаСервере()
	УХ_ЗаполнитьТоварыИзПланаПоставок(Объект);
	УХ_ЗаполнитьСлужебныеРеквизитыТоваров();
	УХ_ОбновитьРеквизитыФормы();    
	СкладПриИзмененииСервер();  
	РассчитатьИтоговыеПоказателиПоступления(ЭтаФорма);	
...
&НаСервереБезКонтекста
Процедура УХ_ЗаполнитьТоварыИзПланаПоставок(ПриобретениеТоваровОбъект)
	РезультатЗапроса = УХ_ПолучитьРезультатЗапросаОстатковДляПриобретенияТоваров(
		ПриобретениеТоваровОбъект);
	ПриобретениеТоваровОбъект.Товары.Загрузить(РезультатЗапроса.Выгрузить());
	Для каждого Стр Из ПриобретениеТоваровОбъект.Товары Цикл
		Стр.СуммаНДС = УчетНДСКлиентСервер.РассчитатьСуммуНДС(
			Стр.СуммаСНДС, 
			Истина, 
			ВстраиваниеУХВызовСервера.ПолучитьСтавкуНДС(
				Стр.СтавкаНДС).Ставка);
		Если ПриобретениеТоваровОбъект.ЦенаВключаетНДС Тогда     
		Стр.Сумма = Стр.СуммаСНДС;			
		Иначе			
		Стр.Сумма = Стр.СуммаСНДС - Стр.СуммаНДС;
		КонецЕсли;
		Стр.СуммаСНДСБезВозвратнойТары = СТР.СуммаСНДС;
		Стр.СуммаНДСБезВозвратнойТары = Стр.СуммаНДС;
		Если НЕ ПриобретениеТоваровОбъект.ЦенаВключаетНДС Тогда
...
&НаСервереБезКонтекста
Функция УХ_ПолучитьРезультатЗапросаОстатковДляПриобретенияТоваров(
													ПриобретениеТоваров)
	ДоговорКонтрагента = 
		УправлениеЗакупкамиВстраиваниеПереопределяемыйУХ.ПолучитьДоговор(
			ПриобретениеТоваров);
	ОписаниеКоэффициента = 
		ЦентрализованныеЗакупкиУХ.ПолучитьКоэффициентПересчетаВалют(
			...
			ПриобретениеТоваров.Валюта,
			УправлениеЗакупкамиВстраиваниеПереопределяемыйУХ.ПолучитьДатуВерсииСоглашенияИзОбъекта(
				ПриобретениеТоваров));
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		...
		|ИЗ
		|	РегистрНакопления.ПланПоставокПоДоговорам.Остатки(&Момент, Договор = &Договор) КАК ПланПоставокПоДоговорамОстатки
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	СУММА(ПланированиеПотребностейОстатки.КоличествоОстаток),
		|	СУММА(ПланированиеПотребностейОстатки.КоличествоОстаток),
		|	ПланированиеПотребностейОстатки.АналитикаПланирования.Цена,
		|	СУММА(ПланированиеПотребностейОстатки.СуммаОстаток),
		|	ПланированиеПотребностейОстатки.АналитикаПланирования.АналитикаПоставки.СтавкаНДС,
		|	СУММА(ПланированиеПотребностейОстатки.СуммаОстаток),
		|	NULL,
		...
		|ИЗ
		|	РегистрНакопления.ПланированиеПотребностей.Остатки(&Момент, ) КАК ПланированиеПотребностейОстатки
		|ГДЕ
		|	ПланированиеПотребностейОстатки.АналитикаПланирования.ОбъектПланирования.Объект = &Объект
		|
		|СГРУППИРОВАТЬ ПО
		|	ПланированиеПотребностейОстатки.АналитикаПланирования.АналитикаПоставки.СтавкаНДС,
		|	ПланированиеПотребностейОстатки.АналитикаПланирования.АналитикаПотребностей.МестоПоставки,
		...
		|	ПланированиеПотребностейОстатки.АналитикаПланирования.АналитикаНоменклатуры.Характеристика,
		|	ПланированиеПотребностейОстатки.АналитикаПланирования.АналитикаСтруктуры.Организация";
	Запрос.УстановитьПараметр("Договор", ПриобретениеТоваров.Договор);  
	Если ЗначениеЗаполнено(ПриобретениеТоваров.ЗаказПоставщику) Тогда
	Запрос.УстановитьПараметр("Объект", ПриобретениеТоваров.ЗаказПоставщику);
	Иначе
	Запрос.УстановитьПараметр("Объект", ПриобретениеТоваров.Договор);
    КонецЕсли;
	Запрос.УстановитьПараметр("Момент", 
		Новый Граница(ПриобретениеТоваров.Ссылка.МоментВремени(), ВидГраницы.Исключая));
...
&НаСервере
Процедура УХ_ЗаполнитьСлужебныеРеквизитыТоваров()
	СтруктураДействий =
			УХ_ПолучитьСтруктуруДействийДляЗаполнениястрокиТоваров();
	КэшированныеЗначения =
...
&НаСервере
Функция УХ_ПолучитьСтруктуруДействийДляЗаполнениястрокиТоваров()
	СтруктураПересчетаСуммы =
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(
			Объект);
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ПроверитьЗаполнитьСклад", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияСкладаВСтрокеТЧ(Объект, СкладГруппа));
	СтруктураДействий.Вставить("ЗаполнитьПризнакТипНоменклатуры", Новый Структура("Номенклатура", "ТипНоменклатуры"));
...
&НаСервере
Процедура УХ_ЗаполнитьСлужебныеРеквизитыСтрокиТоваровНаСервере(ИдентификаторСтроки)
	ОбновляемаяСтрока = Объект.Товары.НайтиПоИдентификатору(ИдентификаторСтроки);
	СтруктураДействий =
			УХ_ПолучитьСтруктуруДействийДляЗаполнениястрокиТоваров();
	КэшированныеЗначения =
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
	Если Объект.ПоступлениеПоЗаказам Тогда
		ОбновляемаяСтрока.РасхождениеЗаказ = 1;
	КонецЕсли;
	СтруктураДействий.Вставить(
		"ПроверитьХарактеристикуПоВладельцу",
		ОбновляемаяСтрока.Характеристика);
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(
		ОбновляемаяСтрока, СтруктураДействий, КэшированныеЗначения);
	УХ_ОбновитьРеквизитыФормы();
КонецПроцедуры

&НаСервере
Процедура УХ_ОбновитьРеквизитыФормы()
	СтруктураНаименованийТабличныхЧастей = Новый Структура("Товары");
	ЗаполнитьСлужебныеРеквизитыПоНоменклатуре(
		СтруктураНаименованийТабличныхЧастей);
	ЗаполнитьПризнакСписанияНаРасходыДляУслуг();
	ПриИзмененииСкладаВТабличнойЧастиСервер();
	РассчитатьИтоговыеПоказателиПоступления(ЭтаФорма);
	//ЕХ31 СформироватьНадписьВалюты(ЭтаФорма);
	ЗаполнитьСтатусыУказанияСерийСервер();
	МногооборотнаяТараСервер.ОбновитьСостояниеЗаполненияМногооборотнойТары(Объект.СостояниеЗаполненияМногооборотнойТары);
КонецПроцедуры
...
Процедура Подключаемый_АналитикиПланированияДокументаНажатие(Элемент, СтандартнаяОбработка)
	...
	АналитикиПланированияДокументовКлиент.НадписьАналитикиПланированияДокументаНажатие(ЭтаФорма, Элемент, СтандартнаяОбработка);
...
&НаКлиенте
Процедура ТоварыУХ_СтатьиИАналитикиНажатие(Элемент, СтандартнаяОбработка)
