...
Процедура ОбновитьВидимостьПодробно(Форма)
	...
	ТекущийТипВНА = ОбщегоНазначенияУХ.ПолучитьЗначениеРеквизита(ТекущаяСтрока.ГруппаВНА, "ТипВНА");
	
	Элементы.ВНАКоличествоНСБУ.Видимость = (ТекущийТипВНА = ПредопределенноеЗначение("Перечисление.ТипыВНА.НезавершенноеСтроительство"));
