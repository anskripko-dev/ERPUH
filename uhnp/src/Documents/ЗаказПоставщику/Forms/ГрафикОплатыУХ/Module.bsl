...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("АдресВоВременномХранилищеТовары") Тогда
		ЭтотОбъект.АдресВоВременномХранилищеТовары = Параметры.АдресВоВременномХранилищеТовары;	
	КонецЕсли;  
	#КонецОбласти 
	...
	Если ЗначениеЗаполнено(РежимИспользованияГрафика) Тогда
		АвтоматическоеСозданиеПлатежныхПозиций = РежимИспользованияГрафика = ПредопределенноеЗначение("Перечисление.РежимыИспользованияГрафика.АвтоматическоеСозданиеПлатежныхПозиций");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Параметры.АдресВоВременномХранилище) Тогда
		Этапы.Загрузить(ПолучитьИзВременногоХранилища(Параметры.АдресВоВременномХранилище));
		УдалитьИзВременногоХранилища(Параметры.АдресВоВременномХранилище);
	КонецЕсли;    	
	
	Если ЗначениеЗаполнено(Параметры.АдресВоВременномХранилищеГрафикОплатыУХ) Тогда
		ЭтапыГрафикаОплаты.Загрузить(ПолучитьИзВременногоХранилища(Параметры.АдресВоВременномХранилищеГрафикОплатыУХ));
		АналитикиПланированияДокументов.ОбщаяФормаЭтапыОплаты_ПослеЗагрузкиЭтаповОплаты(ЭтотОбъект);
		УдалитьИзВременногоХранилища(Параметры.АдресВоВременномХранилищеГрафикОплатыУХ);
	КонецЕсли;    
	...
	Если ЗначениеЗаполнено(Соглашение) Тогда
		РеквизитыСоглашения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Соглашение, "СрокПереходаПраваСобственности");
	...
	СоздатьЭлементыАналитикиСтатейБюджетов();
	
	СуммаОплатыПоДокументу = Параметры.СуммаОплатыПоДокументу;
	
	ВидимостьНадписьОплата = Ложь;
	ВерсияСоглашения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "ВерсияСоглашения");  
	Если ЗначениеЗаполнено(ВерсияСоглашения) Тогда   
    	РеквизитыВерсии = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВерсияСоглашения, "СпособРасчетаКоличестваДней,ПроизводственныеКалендари,ЭтапыГрафикаОплаты");
		СпособРасчетаКоличестваДней = РеквизитыВерсии.СпособРасчетаКоличестваДней; 
		Если СпособРасчетаКоличестваДней = Перечисления.СпособыРасчетаКоличестваДнейВПериоде.ПоРабочимДням Тогда
			ЗаполнитьПроизводственныеКалендари(РеквизитыВерсии.ПроизводственныеКалендари.Выгрузить());
		КонецЕсли;
		ВидимостьНадписьОплата = (РеквизитыВерсии.ЭтапыГрафикаОплаты.Выгрузить().Количество() = 0);
	КонецЕсли;  
	
	Элементы.КомандаПереходаКОплате.Видимость = ВидимостьНадписьОплата;
	НадписьОплата = НадписьОплата(СпособРасчетаКоличестваДней, Этапы.Количество());	
...
Процедура ЗаполнитьПоДоговору(Команда)  
	...
	Если Не ЗначениеЗаполнено(Договор) Тогда 		
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Значение реквизита ""%1"" не заполнено'"), "Договор");  
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);   	
	Иначе  	
		ЗаполнитьЭтапыГрафикаОплатыПоДоговору();	
	КонецЕсли;
...
Процедура ОК(Команда)
	...
	Иначе
		...
		СтруктураОбъекта.Вставить("РежимИспользованияГрафика", 			РежимИспользованияГрафика);
		...
		Если УпрощенныйРежим Тогда
			...
			Если УпрощенныйРежим И СуммаОплатыПоДокументу + СуммаЗалогаПоДокументу > 0 И НЕ ПропуститьПроверку Тогда
				...
				ИначеЕсли ДатаКредит < ДатаЗаказа И СуммаПлатежаКредит + СуммаЗалогаЗаТаруКредит > 0 И ЭтоЗаказ Тогда
					ТекстОшибки = НСтр("ru = 'Дата платежа до отгрузки должна быть не меньше даты отгрузки %ДатаКредит%';
										|en = 'Date of payment before shipment must be later than the shipment date %ДатаКредит%'");
					ТекстОшибки = СтрЗаменить(ТекстОшибки,"%ДатаКредит%", Формат(ДатаКредит, "ДЛФ=DD"));
					ПолеОшибки = "ДатаКредит";
				КонецЕсли;
			КонецЕсли;
...
Процедура СдвигПредоплатаПриИзменении(Элемент)
	...
	ЭтапыОплатыКлиент.ЭтапыГрафикаОплатыСдвигВариантОтсчетаПриИзменении(
		...
		ПроизводственныеКалендари.ВыгрузитьЗначения());
...
Процедура АвтоматическоеСозданиеПлатежныхПозицийПриИзменении(Элемент)
	...
	РежимИспользованияГрафика = ?(АвтоматическоеСозданиеПлатежныхПозиций, ПредопределенноеЗначение("Перечисление.РежимыИспользованияГрафика.АвтоматическоеСозданиеПлатежныхПозиций"), 
									ПредопределенноеЗначение("Перечисление.РежимыИспользованияГрафика.РезервированиеБюджетаИКонтрольСоблюденияГрафика"));
...
Процедура ЭтапыГрафикаОплатыСдвигПриИзменении(Элемент)
	...
	мДатаОтгрузки = ?(ЗначениеЗаполнено(ТекущиеДанные.ДатаОтгрузки), ТекущиеДанные.ДатаОтгрузки, ДатаОтгрузки);
...
Процедура ЭтапыГрафикаОплатыПередУдалением(Элемент, Отказ)
	...
	Если ЭтотОбъект.ДолжныФормироватьсяПозиции 
		И ТекущиеДанные <> Неопределено
		И ТекущиеДанные.НеУдалять Тогда
		Отказ = Истина;
	КонецЕсли;
	#КонецОбласти
...
Функция ПроверитьОплатуПоместитьВХранилище()
	...
	Для каждого СтрокаГрафикОплатыУХ Из ЭтапыГрафикаОплаты Цикл
...
Процедура НастроитьЭлементыФормыПоПараметрам()
	...
	Элементы.АвтоматическоеСозданиеПлатежныхПозиций.Видимость = ПорядокРасчетов = ПредопределенноеЗначение("Перечисление.ПорядокРасчетов.ПоЗаказам");
...
Процедура УстановитьДоступностьКнопокЗаполнения()
	...
	Элементы.ЭтапыГрафикаОплатыЗаполнитьПоДоговору.Видимость =  ПолучитьФункциональнуюОпцию("ИспользоватьДоговорыСПоставщиками");
...
Функция ПараметрыРедактирования()
	...
	СтруктураОткрытия.Вставить("Календарь",  				    ПроизводственныеКалендари.ВыгрузитьЗначения());
...
&НаСервере
Процедура СоздатьЭлементыАналитикиСтатейБюджетов() 
	
	Контрагент = Договор.Контрагент;
	
	ПараметрыЭлементов = ПолучитьПараметрыЭлементовПоУмолчанию();
	
	ВставитьПередЭлементом = Элементы.ЭтапыГрафикаОплатыЗаказ;
	Родитель = Элементы.ЭтапыГрафикаОплаты;
	...
	АналитикиСтатейБюджетовУХ.СтатьяИАналитикиТаблицыЗначенийРеквизитаФормыВТаблицеФормы(МассивОписанийСтатей,
						"ЭтапыГрафикаОплаты", "ЭтапыГрафикаОплаты",
						"СтатьяБюджета", "ЭтапыГрафикаОплатыСтатьяБюджета",
						"Аналитика%1", "ЭтапыГрафикаОплатыАналитика%1",
						ФормыУХ.РазместитьПередЭлементомСтрокой(ВставитьПередЭлементом));
	...
	ПараметрыЭлемента = Новый Структура("Группировка, ОтображатьВШапке", ГруппировкаКолонок.Вертикальная, Ложь);
	ГруппаСтатьяАналитики = ФормыУХ.СоздатьГруппуФормы(Элементы, "ГруппаСтатьяАналитики", Нстр("ru = 'Статья / аналитики'"), 
							ВидГруппыФормы.ГруппаКолонок, Родитель, ВставитьПередЭлементом, ПараметрыЭлемента);   
							
	Элементы.Переместить(Элементы.ЭтапыГрафикаОплатыСтатьяБюджета, ГруппаСтатьяАналитики);						
							
	ПараметрыЭлемента = Новый Структура("Группировка, ОтображатьВШапке", ГруппировкаКолонок.Горизонтальная, Ложь);
	ГруппаАналитикиСтатьи = ФормыУХ.СоздатьГруппуФормы(Элементы, "ГруппаАналитикиСтатьи", , 
							ВидГруппыФормы.ГруппаКолонок, Элементы.ГруппаСтатьяАналитики, , ПараметрыЭлемента);  
							
	Элементы.Переместить(Элементы.ЭтапыГрафикаОплатыАналитика1, ГруппаАналитикиСтатьи);	
	Элементы.Переместить(Элементы.ЭтапыГрафикаОплатыАналитика2, ГруппаАналитикиСтатьи);
	Элементы.Переместить(Элементы.ЭтапыГрафикаОплатыАналитика3, ГруппаАналитикиСтатьи);
	Элементы.Переместить(Элементы.ЭтапыГрафикаОплатыАналитика4, ГруппаАналитикиСтатьи);
	Элементы.Переместить(Элементы.ЭтапыГрафикаОплатыАналитика5, ГруппаАналитикиСтатьи);
	Элементы.Переместить(Элементы.ЭтапыГрафикаОплатыАналитика6, ГруппаАналитикиСтатьи);  
		
	ПараметрыЭлемента = Новый Структура("Группировка, ОтображатьВШапке", ГруппировкаКолонок.Вертикальная, Ложь);
	ГруппаЦФОПроект = ФормыУХ.СоздатьГруппуФормы(Элементы, "ГруппаЦФОПроект", Нстр("ru = 'ЦФО / Проект'"), 
							ВидГруппыФормы.ГруппаКолонок, Родитель, ВставитьПередЭлементом, ПараметрыЭлемента);  

	ФормыУХ.СоздатьПолеФормы(Элементы, "ЦФО", "", "ЭтапыГрафикаОплаты.ЦФО", ВидПоляФормы.ПолеВвода, ГруппаЦФОПроект); 
	ФормыУХ.СоздатьПолеФормы(Элементы, "Проект", "", "ЭтапыГрафикаОплаты.Проект", ВидПоляФормы.ПолеВвода, ГруппаЦФОПроект);

	ПараметрыЭлемента = Новый Структура("Группировка, ОтображатьВШапке", ГруппировкаКолонок.Вертикальная, Ложь);
	ГруппаБанковскиеСчета = ФормыУХ.СоздатьГруппуФормы(Элементы, "ГруппаБанковскиеСчета", Нстр("ru = 'Банковские счета'"), 
							ВидГруппыФормы.ГруппаКолонок, Родитель, ВставитьПередЭлементом, ПараметрыЭлемента);  

	СчетОрганизации = ФормыУХ.СоздатьПолеФормы(Элементы, "СчетОрганизации", "", "ЭтапыГрафикаОплаты.СчетОрганизации", ВидПоляФормы.ПолеВвода, ГруппаБанковскиеСчета);      
	СвязиПараметровВыбора = Новый Массив();
	СвязиПараметровВыбора.Добавить(Новый СвязьПараметраВыбора("Отбор.Владелец", "Организация"));
	СчетОрганизации.СвязиПараметровВыбора = Новый ФиксированныйМассив(СвязиПараметровВыбора);

	СчетКонтрагента = ФормыУХ.СоздатьПолеФормы(Элементы, "СчетКонтрагента", "", "ЭтапыГрафикаОплаты.СчетКонтрагента", ВидПоляФормы.ПолеВвода, ГруппаБанковскиеСчета);
    СвязиПараметровВыбора = Новый Массив();
	СвязиПараметровВыбора.Добавить(Новый СвязьПараметраВыбора("Отбор.Владелец", "Контрагент"));
	СчетКонтрагента.СвязиПараметровВыбора = Новый ФиксированныйМассив(СвязиПараметровВыбора);

	ВидБюджетаБДДС = ПланыВидовХарактеристик.ВидыБюджетов.БюджетДвиженияДенежныхСредств;
	Элементы.ЭтапыГрафикаОплатыСтатьяБюджета.СвязьПоТипу = Новый СвязьПоТипу("ВидБюджетаБДДС");      
...
&НаКлиенте
Процедура ЗаполнитьЭтапыГрафикаОплатыПоДоговору()
	
	ПараметрыЗаполнения = Новый Структура;
	ПараметрыЗаполнения.Вставить("ПерезаполнитьЭтапы", Ложь);
	
	Если ЭтапыГрафикаОплаты.Количество() = 0 Тогда
    	ПараметрыЗаполнения.Вставить("ПерезаполнитьЭтапы", Истина); 
		ЗаполнитьЭтапыГрафикаОплатыПоДоговоруЗавершение(ПараметрыЗаполнения);
	Иначе        	
		ВариантыОтветов = Новый СписокЗначений;
		ВариантыОтветов.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Перезаполнить этапы оплаты';
								|en = 'Refill payment steps'"));
		ВариантыОтветов.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru = 'Отменить';
								|en = 'Cancel'"));
		ТекстВопроса = Символы.ПС +  НСтр("ru = 'Перезаполнить этапы оплаты по договору?';
						|en = 'Refill steps of payment by agreement?'");
		
		Оповещение = Новый ОписаниеОповещения("ВыборИсточникаЗаполненияПоДоговоруЗавершение", 
												ЭтотОбъект, ПараметрыЗаполнения);
		ПоказатьВопрос(Оповещение, ТекстВопроса, ВариантыОтветов); 	
	КонецЕсли;
...
&НаКлиенте
Процедура ВыборИсточникаЗаполненияПоДоговоруЗавершение(РезультатВопроса, ПараметрыЗаполнения) Экспорт
...
&НаКлиенте
Процедура ЗаполнитьЭтапыГрафикаОплатыПоДоговоруЗавершение(ПараметрыЗаполнения)
	
	Если ПараметрыЗаполнения.ПерезаполнитьЭтапы Тогда 	
		ЗаполнитьЭтапыПоЗаказамПоДоговоруСервер(ПараметрыЗаполнения);   		
	КонецЕсли;  
...
&НаСервере
Процедура ЗаполнитьЭтапыПоЗаказамПоДоговоруСервер(Знач ПараметрыЭтапы)
	
	ПараметрыЭтапы.Вставить("Дата", ДатаЗаказа); 
	ПараметрыЭтапы.Вставить("ДатаОтгрузки", ДатаОтгрузки);
	ПараметрыЭтапы.Вставить("Договор", Договор);     
	ПараметрыЭтапы.Вставить("Товары", АдресВоВременномХранилищеТовары); 
	ПараметрыЭтапы.Вставить("СуммаДокумента", СуммаОплатыПоДокументу); 
	ПараметрыЭтапы.Вставить("Этапы", Этапы.Выгрузить()); 

    СтруктураДанных = АналитикиПланированияДокументов.ПолучитьЭтапыОплатПоДоговору(ПараметрыЭтапы);
	
	Если СтруктураДанных.ЭтапыГрафикаОплаты = Неопределено Тогда
		Возврат;
	КонецЕсли;  
	
	ЭтапыГрафикаОплаты.Загрузить(СтруктураДанных.ЭтапыГрафикаОплаты);  
	СуммаОплатыПоДокументу = ЭтапыГрафикаОплаты.Итог("СуммаПлатежа");
...
Процедура НадписьОплатаНажатие(Элемент, СтандартнаяОбработка)
	...
	Календари = ПроизводственныеКалендари.ВыгрузитьЗначения(); 
	...
	ПараметрыФормы.Вставить("Календарь",                 Календарь);
	ПараметрыФормы.Вставить("УникальныйИдентификатор",   УникальныйИдентификатор);  	
	ПараметрыФормы.Вставить("ПорядокРасчетов",           РеквизитыДоговора.ПорядокРасчетов);
	ПараметрыФормы.Вставить("ХозяйственнаяОперация",     РеквизитыДоговора.ХозяйственнаяОперация);	
	ПараметрыФормы.Вставить("АдресВоВременномХранилище", ПоместитьЭтапыВоВременноеХранилищеНаСервере());
	
	ОткрытьФорму("Справочник.СоглашенияСПоставщиками.Форма.ФормаОплаты", ПараметрыФормы, ЭтаФорма,,,, 
				Новый ОписаниеОповещения("НадписьОплатаНажатиеЗавершение", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура НадписьОплатаНажатиеЗавершение(ПараметрыОплаты, ДополнительныеПараметры) Экспорт
	...
	ЗаполнитьЭтапы(ПараметрыОплаты);        
	НадписьОплата = НадписьОплата(СпособРасчетаКоличестваДней, Этапы.Количество());
...
&НаСервере
Процедура ЗаполнитьЭтапы(ПараметрыОплаты)

	Этапы.Загрузить(ПолучитьИзВременногоХранилища(ПараметрыОплаты.АдресВоВременномХранилище));
...
Функция ПолучитьРеквизитыДоговора(Договор)
	...
	Возврат ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Договор, "ПорядокРасчетов, ХозяйственнаяОперация");
	
КонецФункции  

&НаСервере
Функция ПоместитьЭтапыВоВременноеХранилищеНаСервере()
	
	Возврат ПоместитьВоВременноеХранилище(Этапы.Выгрузить());
	
КонецФункции

&НаСервере
Процедура ЗаполнитьПроизводственныеКалендари(ТаблицаПроизводственныеКалендари)  
	
	Для Каждого СтрокаТаблицы Из ТаблицаПроизводственныеКалендари Цикл  
		ПроизводственныеКалендари.Добавить(СтрокаТаблицы.ПроизводственныйКалендарь);
	КонецЦикла;
