...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		#Область УХ_Встраивание
	    ЗаказПоставщикуСерверВнедрениеЕХ.ДобавитьТекстыЗапросов(Запрос, ТекстыЗапроса, Регистры); 
		#КонецОбласти
	КонецЕсли;
	...
	#Область УХ_Встраивание  
	Если ТипЗнч(Документ) = Тип("ДокументОбъект.ЗаказПоставщику") Тогда
		ДанныеДокументаДляПроведения = ПроведениеДокументов.ИнициализироватьДанныеДокументаДляПроведения(Запрос, ТекстыЗапроса, ДопПараметры);
		
		Если Документ <> Неопределено И  ЗначениеЗаполнено(Документ.Ссылка) Тогда
			Запрос.МенеджерВременныхТаблиц = ЗаказПоставщикуСерверВнедрениеЕХ.ПланыДокумента(Документ, Запрос.МенеджерВременныхТаблиц); 
			ПланированиеПотребностей.ПодготовитьТаблицуДвиженийПоПланамДокумента(Запрос, ДанныеДокументаДляПроведения, "ВТ_ПланыДокумента"); 
		КонецЕсли;
