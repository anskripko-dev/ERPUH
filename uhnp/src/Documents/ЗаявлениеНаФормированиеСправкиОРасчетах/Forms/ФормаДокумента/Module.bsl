...
Функция ПрорисоватьСтатус()
	...
	ИначеЕсли Статус = НСтр("ru = 'Не принято';
							|en = 'Не принято'") Тогда
...
Функция ПроверитьВозможностьВыгрузкиЗаявленияНаФормированиеСправкиОРасчетахСФР()
	...
	Если НЕ ЗначениеЗаполнено(Объект.КодПодчиненностиФСС) Тогда
		Результат.Добавить(Новый Структура("ТекстОшибки, Поле",
			НСтр("ru = 'В карточке организации не задан код подчиненности СФР или код органа СФР';
				|en = 'В карточке организации не задан код подчиненности СФР или код органа СФР'"), "КодПодчиненностиФСС"));
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.РегистрационныйНомерФСС)
		И НЕ ЗначениеЗаполнено(Объект.РегистрационныйНомерСФР) Тогда
		Результат.Добавить(Новый Структура("ТекстОшибки, Поле",
			НСтр("ru = 'В карточке организации не задан регистрационный номер страхователя';
				|en = 'В карточке организации не задан регистрационный номер страхователя'"), "РегистрационныйНомерСФР"));
	КонецЕсли;
		
	Если СтрДлина(Объект.РегистрационныйНомерФСС) <> 10
		И СтрДлина(Объект.РегистрационныйНомерСФР) <> 10 Тогда
		Результат.Добавить(Новый Структура("ТекстОшибки, Поле",
			НСтр("ru = 'В карточке организации указан некорректный регистрационный номер страхователя';
				|en = 'В карточке организации указан некорректный регистрационный номер страхователя'"), "РегистрационныйНомерСФР"));
	КонецЕсли;
	
	Если Объект.Страхователь_ЮридическоеЛицо Тогда
		Если НЕ ЗначениеЗаполнено(Объект.СтраховательЮЛ_НаимОрг) Тогда
			Результат.Добавить(Новый Структура("ТекстОшибки, Поле",
				НСтр("ru = 'В карточке организации не задано наименование страхователя';
					|en = 'В карточке организации не задано наименование страхователя'"), "СтраховательЮЛ_НаимОрг"));
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(Объект.СтраховательЮЛ_ИНН) Тогда
			Результат.Добавить(Новый Структура("ТекстОшибки, Поле",
				НСтр("ru = 'В карточке организации не задан ИНН страхователя';
					|en = 'В карточке организации не задан ИНН страхователя'"), "СтраховательЮЛ_ИНН"));
		ИначеЕсли НЕ РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(
			Объект.СтраховательЮЛ_ИНН, Истина, "") Тогда
			
			Результат.Добавить(Новый Структура("ТекстОшибки, Поле",
				НСтр("ru = 'В карточке организации указан некорректный ИНН организации доверителя';
					|en = 'В карточке организации указан некорректный ИНН организации доверителя'"), "СтраховательЮЛ_ИНН"));
		КонецЕсли;
