	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		...
		Если ДанныеЗаполнения.РаспоряжениеОснование.Количество() > 0 Тогда
			Если ТипЗнч(ДанныеЗаполнения.РаспоряжениеОснование[0]) = Тип("ДокументСсылка.РаспоряжениеНаПеремещениеДенежныхСредств") Тогда
				...
				// Почему-то в ПКО банковский счет отправитель называется БанковскийСчет
				ДанныеЗаполнения.Вставить("БанковскийСчет", ДанныеЗаполнения.БанковскийСчетОтправитель);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ВстраиваниеУХПриходныйКассовыйОрдер.ОбработкаПроверкиЗаполнения(ЭтотОбъект, Отказ, 
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	КонецЕсли;
	#Область УХ_Встраивание
	ВстраиваниеУХПриходныйКассовыйОрдер.ПередЗаписью(ЭтотОбъект, Отказ, РежимЗаписи, РежимПроведения);
	#КонецОбласти
КонецПроцедуры
...
Процедура ПриЗаписи(Отказ)
	...
	#Область УХ_Внедрение
	ВстраиваниеУХПриходныйКассовыйОрдер.ПриЗаписи(ЭтотОбъект, Отказ);
	#КонецОбласти
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзБанка
		...
		Если Организация <> РеквизитыСчета.Организация Тогда
			Текст = НСтр("ru = 'Организация счета не соответствует организации документа';
						|en = 'The company mentioned in the account does not match the company mentioned in the document'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				Текст,
				ЭтотОбъект,
				"БанковскийСчет",
