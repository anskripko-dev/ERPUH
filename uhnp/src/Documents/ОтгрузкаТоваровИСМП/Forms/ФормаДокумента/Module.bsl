...
Процедура НастроитьЭлементыФормы(Форма)
	...
	Элементы.ТоварыЦена.АвтоОтметкаНезаполненного = ИнтеграцияИСМПСлужебныйКлиентСервер.ТребуетсяУказаниеЦеныПриПриемкеОтгрузке(
		 Объект.Операция, Объект.ВидПродукции);
