...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ПрослеживаемыйКомплект
		...
		Если КонтролироватьНомераГТД
			...
			ТекстСообщения = НСтр("ru = 'Для непрослеживаемого комплекта в качестве основного компонента указана прослеживаемая по РНПТ комплектующая `%НазваниеТовара%`.
				...
				|en = 'Для непрослеживаемого комплекта в качестве основного компонента указана прослеживаемая по РНПТ комплектующая `%НазваниеТовара%`.
				|Укажите непрослеживаемый основной компонент, по которому ведется учет по ГТД.'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеТовара%", ПредставлениеОсновногоКомпонента);
			...
			Если Не РеквизитыОсновнойКомпоненты.ВестиУчетПоГТД Тогда
				Если ИспользоватьУчетПрослеживаемыхИмпортныхТоваров Тогда
					ТекстРекомендации = НСтр("ru = 'Укажите непрослеживаемый основной компонент, по которому ведется учет по ГТД';
											|en = 'Укажите непрослеживаемый основной компонент, по которому ведется учет по ГТД'");
				Иначе
					ТекстРекомендации = НСтр("ru = 'Укажите основной компонент, по которому ведется учет по ГТД';
											|en = 'Укажите основной компонент, по которому ведется учет по ГТД'");
				КонецЕсли;
				
				ТекстСообщения = НСтр("ru = 'По основному компоненту `%НазваниеТовара%` учет по ГТД не ведется.
					|%ТекстРекомендации%.';
					|en = 'По основному компоненту `%НазваниеТовара%` учет по ГТД не ведется.
					|%ТекстРекомендации%.'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НазваниеТовара%", ПредставлениеОсновногоКомпонента);
	...
	ИначеЕсли Не ПрослеживаемыйКомплект
		...
		ТекстСообщения = НСтр("ru = 'Требуется определять страну происхождения и номера ГТД комплекта.
			|%1.';
			|en = 'Требуется определять страну происхождения и номера ГТД комплекта.
			|%1.'");
	...
	ИначеЕсли ПрослеживаемыйКомплект
		...
		Если РезультатЗапроса.Пустой() Тогда
			...
			|en = 'The ""Components"" list does not include any RNPT-traceable components.
			|To assemble a traceable kit, specify at least one traceable component.'");
	...
	ИначеЕсли (КонтролироватьНомераГТД
		...
		Если ИмпортныеТовары.Количество() > 0 Тогда
			ШаблонСообщения = НСтр("ru = 'Для комплектующей в строке %НомерСтроки% списка ""Комплектующие"" ведется учет по ГТД.
				...
				|en = 'CCD records are kept for the component in line %НомерСтроки% of the ""Components"" list.
				|These items are invalid if the disassembled kit has no CCD records.'");
		...
		Если ПрослеживаемыеТовары.Количество() > 0 Тогда
			ШаблонСообщения = НСтр("ru = 'В строке %НомерСтроки% списка ""Комплектующие"" указана прослеживаемая по РНПТ комплектующая.
				...
				|en = 'In line %НомерСтроки% of the ""Components"" list, the RNPT-traceable component is specified.
				|These items are invalid if an untraceable kit is being disassembled.'");
	...
	Если ВидыЗапасовДляСозданияСоставныхНомеровГТД.Количество() > 0 Тогда
		...
		ИначеЕсли ПрослеживаемыйКомплект
			...
			ТекстСообщения = НСтр("ru = 'Отсутствуют прослеживаемые по РНПТ комплектующие при прослеживаемом комплекте.
				...
				|en = 'There are no RNPT-traceable components in a traceable kit.
				|Traceable kit will be disposed.'");
		...
		ИначеЕсли Не ПрослеживаемыйКомплект
			...
			|en = 'There is an RNPT-traceable component in an untraceable set.
			|Traceable components will be disposed.'");
