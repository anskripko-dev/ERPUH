...
&НаКлиенте
Процедура РасчетОтложенныхНалоговРазницаПРПриИзменении(Элемент)
	ОбработатьСтрокуТЧ();
КонецПроцедуры

&НаКлиенте
Процедура РасчетОтложенныхНалоговОстатокНСБУПриИзменении(Элемент)
	ОбработатьСтрокуТЧ();
КонецПроцедуры

&НаКлиенте
Процедура РасчетОтложенныхНалоговОстатокМСФОПриИзменении(Элемент)
	ОбработатьСтрокуТЧ();
КонецПроцедуры
...
Функция ПоказатьРасшифровкуНаСервере(Организация, Сценарий, ПериодОтчета)
	...
	ИначеЕсли ЗначенияУП.ФормироватьПроводкиМСФО Тогда
		...
		ЗаголовокОтчета = СтрШаблон(НСтр("ru = 'Расшифровка (модель: <%1>) расчета для <%2> за период <%3>'"), ЗначенияУП.МодельУчетаМСФО, Организация, ПериодОтчета);
	...
	Построитель.ИсточникДанных.Колонки.Ставка.Итог = "МАКСИМУМ(Ставка)";
...
Процедура ОбработатьСтрокуТЧ()
	...
	Идентификатор = Элементы.РасчетОтложенныхНалогов.ТекущаяСтрока;
	Если Идентификатор = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТекущаяСтрока = Объект.РасчетОтложенныхНалогов.НайтиПоИдентификатору(Идентификатор);

	ЗначенияУП = Документы.РасчетОтложенныхНалогов.ЗначенияУП(Объект);
	КэшированныеЗначения = Новый Структура(
		"СтавкаНалога", ЗначенияУП.СтавкаОтложенногоНалога);
	
	СтруктураДействий = Новый Структура("ЗаполнитьСуммыОН, ЗаполнитьРазницу");
	Документы.РасчетОтложенныхНалогов.ОбработатьСтрокуТЧ(
