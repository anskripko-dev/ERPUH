...
//    * РНПТМатериалов - Структура - состав полей задается в функции НоменклатураКлиентСервер.ПараметрыУказанияСерий.
//
Функция ПараметрыУказанияСерий(Объект) Экспорт
	
	#Область ПараметрыРНПТМатериалов
	
	ПараметрыСерийПроизводства = ПроизводствоСервер.ИспользованиеСерийВПроизводстве22();
	
	ПараметрыРНПТМатериалов = НоменклатураКлиентСервер.ПараметрыУказанияСерий();
	ПараметрыРНПТМатериалов.ПолноеИмяОбъекта = "Документ.ОтчетДавальцуМеждуОрганизациями";
	
	ПараметрыРНПТМатериалов.ИмяПоляСклад = "Подразделение";
	ПараметрыРНПТМатериалов.ИмяТЧТовары  = "РНПТМатериалов";
	ПараметрыРНПТМатериалов.ИмяТЧСерии   = "РНПТМатериалов";
	
	ПараметрыРНПТМатериалов.УчитыватьСебестоимостьПоСериям = ПараметрыСерийПроизводства.УчитыватьСебестоимостьПоСериям;
	ПараметрыРНПТМатериалов.ИспользоватьСерииНоменклатуры  = ПараметрыСерийПроизводства.ИспользоватьСерииНоменклатуры;
	
	ПараметрыРНПТМатериалов.СкладскиеОперации.Добавить(Перечисления.СкладскиеОперации.ПустаяСсылка());
	
	ПараметрыРНПТМатериалов.ПоляСвязи.Добавить("Подразделение");
	ПараметрыРНПТМатериалов.ИменаПолейДополнительные.Добавить("Назначение");
	
	ПараметрыРНПТМатериалов.ЭтоНакладная        = Истина;
	ПараметрыРНПТМатериалов.РегистрироватьСерии = Ложь;
	
	ПараметрыРНПТМатериалов.ПланированиеОтгрузки = Ложь;
	ПараметрыРНПТМатериалов.ПланированиеОтбора   = Ложь;
	
	ПараметрыРНПТМатериалов.Дата = Объект.Дата;
	
	#КонецОбласти
	
	ПараметрыУказанияСерий = Новый Структура;
	ПараметрыУказанияСерий.Вставить("РНПТМатериалов", ПараметрыРНПТМатериалов);
...
Функция МакетФормыВыбораНазначений() Экспорт
	...
	// РНПТМатериалов
	ШаблонНазначения = Справочники.Назначения.ДобавитьШаблонНазначений(МакетФормы, "Объект.РНПТМатериалов.Назначение");
	ШаблонНазначения.НаправлениеДеятельности = "Объект.НаправлениеДеятельности";
	
	// РНПТ материалов в производстве
	ОписаниеКолонок = Справочники.Назначения.ДобавитьОписаниеКолонок(МакетФормы, "РНПТМатериаловВПроизводстве", Истина, "Объект.РНПТМатериалов.Назначение", "Материалы");
	
	ОписаниеКолонок.ПутиКДанным.Организация    = "Объект.Организация";
	ОписаниеКолонок.ПутиКДанным.Номенклатура   = "Объект.РНПТМатериалов.Номенклатура";
	ОписаниеКолонок.ПутиКДанным.Характеристика = "Объект.РНПТМатериалов.Характеристика";
	ОписаниеКолонок.ПутиКДанным.Подразделение  = "Объект.РНПТМатериалов.Подразделение";
...
Функция ЗапросОстаткиОтчетовКОформлению(Параметры) Экспорт
	...
	|		ВнутренниеПартииПроизводства КАК ВнутренниеПартииПроизводства
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходы КАК ПриходыНЗП
	|		ПО ВнутренниеПартииПроизводства.ПартияПроизводства = ПриходыНЗП.АналитикаРасходов
	|		И ВнутренниеПартииПроизводства.Организация = ПриходыНЗП.Организация
	|	
	|	ГДЕ
	|		НАЧАЛОПЕРИОДА(ПриходыНЗП.Период, МЕСЯЦ) > ВнутренниеПартииПроизводства.МесяцКрайнегоОтчета
...
//		* Товары - ТаблицаЗначений, ТабличнаяЧасть, Неопределено - данные о работах переработчика.
//	
...
Функция ТекстЗапросаТаблицаВтТовары(Запрос, ТекстыЗапроса)
	...
	|	ЕСТЬNULL(Суммы.СуммаБезНДСРегл, 0)       КАК СтоимостьРегл,
	...
	|	АналитикаНоменклатуры.Номенклатура.КодТНВЭД КАК КодТНВЭД,
	|	ТаблицаТовары.Количество                 КАК Количество,
	...
	|	&ХозяйственнаяОперация                   КАК ХозяйственнаяОперация,
	|	&НастройкаХозяйственнойОперации          КАК НастройкаХозяйственнойОперации,
	|	АналитикаНоменклатуры.Номенклатура.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар
	|ИЗ
...
Функция ТекстЗапросаТаблицаМатериалыИРаботыВПроизводстве(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВтТовары КАК ТаблицаТовары
	|ГДЕ
	|	НЕ ТаблицаТовары.ПрослеживаемыйТовар
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Услуга)										КАК ТипЗапасов,
	|	ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)									КАК ВидЗапасов,
	// Заполнение поля "Раздел учета" должно соответствовать функции ОписаниеОперацийУчетаСебестоимости()
	...
	|	(ВЫБОР
	|		КОГДА &СреднеСкользящая ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)
	|		ИНАЧЕ ТаблицаПродукции.АналитикаФинансовогоУчета КОНЕЦ)						КАК АналитикаФинансовогоУчета,
	|	(ВЫБОР
	|		КОГДА &СреднеСкользящая ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)
	|		ИНАЧЕ ТаблицаПродукции.ВидДеятельностиНДСПереработчика КОНЕЦ)				КАК ВидДеятельностиНДС,
	|
	|	&Ответственный					КАК Менеджер,
	...
	|	(ВЫБОР
	|		КОГДА &ФИФОСкользящаяОценка ТОГДА &Ссылка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) 	КАК Партия,
	|	(ВЫБОР
	|		КОГДА &ФИФОСкользящаяОценка ТОГДА ТаблицаПродукции.АналитикаУчетаПартий
...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	|
	// Прочие поля
	// Заполнение поля "Раздел учета" должно соответствовать функции ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж(). 
	|	(ВЫБОР
	|		КОГДА &УчетГотовойПродукцииПоПлановойСтоимости
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ВыпускПродукции)
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ)						КАК РазделУчета,
	...
	|	ВтТовары.АналитикаУчетаНоменклатурыПереработчика	КАК АналитикаУчетаНоменклатуры,
	|	ВтТовары.КорВидЗапасов							КАК ВидЗапасов,
	...
	// Количественные и суммовые показатели
	|	ВтТовары.Количество		КАК Количество,
	|	ВтТовары.Стоимость		КАК Стоимость,
	|	ВтТовары.СтоимостьБезНДС КАК СтоимостьБезНДС,
	...
	#Область СписаниеНаДругиеПартии_РасходРаботыИзПодразделенияИПриходНаПартиюПроизводства_УДавальца_5
	...
	|	ВтТовары.Количество КАК Количество,
	|	(ВЫБОР
	|		КОГДА &СреднеСкользящая ТОГДА ВтТовары.Стоимость
	|		ИНАЧЕ 0 КОНЕЦ)		КАК Стоимость,
	|	(ВЫБОР
	|		КОГДА &СреднеСкользящая ТОГДА ВтТовары.СтоимостьБезНДС
	|		ИНАЧЕ 0 КОНЕЦ)		КАК СтоимостьБезНДС,
	|	(ВЫБОР
	|		КОГДА &СреднеСкользящая ТОГДА ВтТовары.СтоимостьРегл
	|		ИНАЧЕ 0 КОНЕЦ)		КАК СтоимостьРегл,
	|	(ВЫБОР
	|		КОГДА &СреднеСкользящая ТОГДА ВтТовары.СтоимостьУпр
	...
	|	(ВЫБОР
	|		КОГДА &СреднеСкользящая ТОГДА ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимостиТоваров.ИзДокумента)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимостиТоваров.ПустаяСсылка) КОНЕЦ)	КАК СпособОпределенияСебестоимости
	|
	|ИЗ
	|	Документ.ОтчетДавальцуМеждуОрганизациями КАК ТаблицаДокумента
...
Функция ПолучитьДанныеДляПечатнойФормыАктОбОказанииУслуг(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ТаблицаТоваров.Номенклатура.НаименованиеПолное КАК УслугаНаименованиеПолное,
	...
	Для Каждого ТекущиеДанные Из ТаблицаТоваров Цикл
		Если ТекущиеДанные.ЭтоТовар Тогда
			ТекущиеДанные.УслугаНаименованиеПолное = СтрШаблон(ШаблонСодержания, ТекущиеДанные.УслугаНаименованиеПолное);
		КонецЕсли;
	КонецЦикла;
...
Функция ДанныеДляПечатныхФормСчетаНаОплатуИзвещения(ПараметрыПечати, МассивОбъектов, КодЯзыка = Неопределено) Экспорт
	...
	|	ЕСТЬNULL(Документы.БанковскийСчетПереработчика.СчетЦифровогоРубля, ЛОЖЬ) КАК ЭтоСчетЦифровогоРубля,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(Документы.БанковскийСчетПереработчика.СчетЦифровогоРубля, ЛОЖЬ)
	|			ТОГДА Документы.БанковскийСчетПереработчика.ИдентификаторСчетаЦифровогоРубля
	|		ИНАЧЕ Документы.БанковскийСчетПереработчика.НомерСчета
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Документы.ОтчетДавальцуМеждуОрганизациями.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "2.5.20.26";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Порядок = Перечисления.ПорядокОбработчиковОбновления.Некритичный;
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("fb491ac4-33ad-4242-8bfb-32982a9eb04f");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ОтчетДавальцуМеждуОрганизациями.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	
	СписокОписаний = Новый Массив;
	СписокОписаний.Добавить(НСтр("ru = 'Обновление документов ""Отчет давальцу между организациями"":';
								|en = 'Update the ""Intercompany consumption report — Subcontracting services delivered"" documents:'"));
	...
	Читаемые.Добавить(Метаданные.Документы.ОтчетДавальцуМеждуОрганизациями.ПолноеИмя());
	Обработчик.ЧитаемыеОбъекты = СтрСоединить(Читаемые, ",");
	
	Изменяемые = Новый Массив;
	Изменяемые.Добавить(Метаданные.Документы.ОтчетДавальцуМеждуОрганизациями.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Документы.ОтчетДавальцуМеждуОрганизациями.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ОтчетДавальцуМеждуОрганизациями.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ОтчетДавальцуМеждуОрганизациями.Товары КАК ОтчетДавальцуМеждуОрганизациями
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(ОтчетДавальцуМеждуОрганизациями.АналитикаАктивовПассивов) = ТИП(Перечисление.УдалитьТипыНалогов)";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний = Новый Массив;
	СписокОписаний.Добавить(НСтр("ru = 'Не удалось обработать документы ""Отчет давальцу между организациями"" по обработчику:';
								|en = 'Cannot process the ""Intercompany consumption report — Subcontracting services delivered"" documents by the handler:'"));
	...
	Запрос.Текст = 
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетДавальцуМеждуОрганизациями КАК ДанныеДокумента
