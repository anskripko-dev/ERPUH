...
Функция ТаблицыОтложенногоФормированияДвижений(МенеджерВременныхТаблиц) Экспорт
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ЗаполнитьПараметрыИнициализацииПоПериодам(Запрос, Запрос.Параметры,, "ВтПериодыДокументов");
...
Процедура ПолучитьДанныеДокументаОтложенногоПроведения(Запрос)
	...
	#Область ВтПериодыДокументов
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ДанныеДокументаРеквизиты.Организация КАК Организация,
	|	НАЧАЛОПЕРИОДА(ДанныеДокументаРеквизиты.Период, МЕСЯЦ) КАК Период
	|
	...
	|ВЫБРАТЬ
	|	ДанныеДокументаРеквизиты.Организация КАК Организация,
	|	НАЧАЛОПЕРИОДА(ДанныеДокументаРеквизиты.ДатаПереходаПраваСобственности, МЕСЯЦ) КАК Период
	|
	|ИЗ
	|	ДанныеДокументаРеквизиты КАК ДанныеДокументаРеквизиты
	|
	|ГДЕ
	|	ДанныеДокументаРеквизиты.ДатаПереходаПраваСобственности <> ДАТАВРЕМЯ(1,1,1)
	|	И НАЧАЛОПЕРИОДА(ДанныеДокументаРеквизиты.ДатаПереходаПраваСобственности, МЕСЯЦ) <> НАЧАЛОПЕРИОДА(ДанныеДокументаРеквизиты.Период, МЕСЯЦ)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Период,
	|	Организация
...
Функция ТекстЗапросаТаблицаДокументыПоОС(ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	|	И ЕСТЬNULL(ТаблицаОС.АрендныйКомпонентДоговора, ЛОЖЬ)
	|";
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Обновление документов ""Реализация услуг и прочих активов"":';
								|en = 'Update the ""Customer invoice — Services and Assets"" documents:'"));
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Не удалось обработать документы ""Реализация услуг и прочих активов"" по обработчику:';
								|en = 'Cannot process the ""Customer invoice — Services and Assets"" documents by the handler:'"));
