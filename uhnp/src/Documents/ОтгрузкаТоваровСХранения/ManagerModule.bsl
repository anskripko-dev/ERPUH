...
Процедура ИнициализироватьПараметрыЗаполнения(ПараметрыЗаполнения, РеквизитыШапки, МассивЗаказов) Экспорт
	...
	Если ПараметрыЗаполнения.ЗаполнятьПоОрдеру = Неопределено Тогда
		...
		|	РаспоряженияНаОтгрузку.КПередачеОборот КАК КПередачеОстаток
		|ИЗ
		|	РегистрНакопления.РаспоряженияНаОтгрузку.Обороты(,,, Распоряжение В (&ЗаказыДавальца)) КАК РаспоряженияНаОтгрузку
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Склады КАК СпрСклады
		|		ПО СпрСклады.Ссылка = РаспоряженияНаОтгрузку.Склад
		|		ЛЕВОЕ СОЕДИНЕНИЕ Константы КАК Константы
		|		ПО ИСТИНА
		|ГДЕ
		|	РаспоряженияНаОтгрузку.КПередачеОборот > 0
...
Процедура ЗаполнитьПоЗаказамОрдерам(Товары, Регистратор, ПараметрыЗаполнения, ШтрихкодыУпаковок = Неопределено) Экспорт
	...
	ТекстЗапросаРегистраЗаказы =
		РегистрыНакопления.РаспоряженияНаОтгрузку.ТекстЗапросаОстатки("ВтДанныеУчета", Отбор, "КПередаче <> 0");
	...
	Если ЗначениеЗаполнено(ПараметрыЗаполнения.МассивЗаказов) Тогда
		ОтгрузкаТоваровСХраненияЛокализация.ЗаполнитьШтрихКодыУпаковок(ПараметрыЗаполнения.МассивЗаказов, Товары, ШтрихкодыУпаковок);
	КонецЕсли;
...
Функция СтруктураОснования(Объект)
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратДавальцу2_5 Тогда
		...
		Если ЗначениеЗаполнено(Объект.Договор) Тогда
			...
			РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Договор, "Дата, Номер");
			РеквизитыДоговораТекст = СтрШаблон(НСтр("ru = 'по договору №%1 от %2';
													|en = 'under contract No. %1 dated %2'"), РеквизитыДоговора.Номер, Формат(РеквизитыДоговора.Дата, "ДЛФ=ДД"));
			
			СтруктураОснование.ОснованиеДата  = РеквизитыДоговора.Дата;
			СтруктураОснование.ОснованиеНомер = РеквизитыДоговора.Номер;
			
		Иначе
			РеквизитыДоговораТекст = "";
		КонецЕсли;
		
		СтруктураОснование.ОснованиеДляПечати = СокрЛП(СтрШаблон(НСтр("ru = 'Возврат давальческого сырья %1';
...
Функция ТекстЗапросаТаблицаЗаказы(ИмяВременнойТаблицы = "") Экспорт
	...
	|					И ВЫБОР
	|						КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
	|							ТОГДА ТИПЗНАЧЕНИЯ(Распоряжение) = ТИП(Документ.ЗаказДавальца2_5)
	|						ИНАЧЕ ТИПЗНАЧЕНИЯ(Распоряжение) <> ТИП(Документ.ЗаказДавальца2_5)
	|					КОНЕЦ
	//-- НЕ УТКА
	|					И (Склад = &Склад
	...
	//++ НЕ УТКА
	|			КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
	|				ТОГДА -РаспоряженияДвижения.КПередаче
	//-- НЕ УТКА
	|			КОГДА ИСТИНА
	...
	|		И ВЫБОР
	|			КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
	|				ТОГДА ТИПЗНАЧЕНИЯ(РаспоряженияДвижения.Распоряжение) = ТИП(Документ.ЗаказДавальца2_5)
	|			ИНАЧЕ ТИПЗНАЧЕНИЯ(РаспоряженияДвижения.Распоряжение) <> ТИП(Документ.ЗаказДавальца2_5)
	|		КОНЕЦ
	//-- НЕ УТКА
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, Склад = Неопределено, МассивЗаказов = Неопределено)
	...
	|		РаспоряженияОбороты.КПередачеОборот КАК КПередаче
	|	ИЗ
	|		РегистрНакопления.РаспоряженияНаОтгрузку.Обороты(,,,
	|			ВЫБОР
	|				КОГДА &ОтобратьПоЗаказу
	...
	|		-РаспоряженияДвижения.КПередаче      КАК КПередаче
	|	ИЗ
	|		РегистрНакопления.РаспоряженияНаОтгрузку КАК РаспоряженияДвижения
	|	ГДЕ
	|		РаспоряженияДвижения.Регистратор = &Регистратор
	...
	|			ИНАЧЕ РаспоряженияДвижения.Склад В (
	|									ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка),
	|									&Склад)
	|		КОНЕЦ
	|	) КАК ТаблицаЗаказы
	...
	|		ВЫБОР
	|			КОГДА ЗаказТовары.Обособленно
	|				ТОГДА ЗаказыОстатки.Распоряжение.Назначение
	|			ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|		КОНЕЦ                                                                        КАК Назначение,
	|		0                                                                            КАК КоличествоСобирается,
	|		ВЫБОР 
	|			КОГДА ЗаказыОстатки.ВидДвиженияРегистра = ЗНАЧЕНИЕ(Перечисление.ВидыДвиженияНакопления.Расход)
	|				ТОГДА ЗаказыОстатки.КПередачеОборот
	|			ИНАЧЕ 0
	|		КОНЕЦ                                                                        КАК Количество
	|	ИЗ
	|		РегистрНакопления.РаспоряженияНаОтгрузку.Обороты(,, РЕГИСТРАТОР,
	|			Распоряжение В
	|				(ВЫБРАТЬ
	|					Заказ.Ссылка
	|				ИЗ
