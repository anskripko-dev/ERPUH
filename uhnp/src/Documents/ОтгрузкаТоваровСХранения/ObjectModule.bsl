	...
	Если ТипДанныхЗаполнения = Тип("Структура") Тогда
		...
		ИначеЕсли ДанныеЗаполнения.Свойство("ДокументОснование") Тогда
			...
			Если ТипОснование = Тип("Массив") Тогда
				...
				//++ НЕ УТКА
				ИначеЕсли ТипЗнч(ДанныеЗаполнения.ДокументОснование[0]) = Тип("ДокументСсылка.ЗаказДавальца2_5") Тогда
			...
			ИначеЕсли ТипОснование = Тип("ДокументСсылка.ЗаказДавальца2_5") Тогда
	...
	Для ТекИндекс = 0 По Товары.Количество()-1 Цикл
		...
		Если ПоЗаказу
			...
			ТекстОшибки = НСтр("ru = 'Не заполнено поле ""%ПредставлениеЗаказа%"" в строке %НомерСтроки% списка ""Товары""';
								|en = 'The ""%ПредставлениеЗаказа%"" field in line %НомерСтроки% of the ""Goods"" list is not filled'");
...
Процедура ЗаполнитьНаОснованииЗаказДавальца2_5(ДанныеЗаполнения)
	...
	|		ИНАЧЕ Продукция.Ссылка.Назначение
	|	КОНЕЦ                                      КАК Назначение,
	|	ТаблицаТовары.Серия                        КАК Серия,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(Продукция.Упаковка, ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)) =
	|				ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|			ТОГДА ТаблицаТовары.КПередачеОборот
	|		ИНАЧЕ ТаблицаТовары.КПередачеОборот / &ТекстЗапросаКоэффициентУпаковки
	|	КОНЕЦ                                      КАК КоличествоУпаковок,
	|	ТаблицаТовары.КПередачеОборот              КАК Количество,
	|	ЕСТЬNULL(Продукция.Упаковка, ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)) КАК Упаковка,
	|	0                                          КАК Цена,
	|	0                                          КАК Сумма,
	|	ТаблицаТовары.Склад                        КАК Склад,
	|	ТаблицаТовары.Распоряжение                 КАК ЗаказКлиента,
	|	ТаблицаТовары.КодСтроки                    КАК КодСтроки
	|ИЗ
	|	РегистрНакопления.РаспоряженияНаОтгрузку.Обороты(,,, Распоряжение = &ДокументОснование) КАК ТаблицаТовары
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказДавальца2_5.Продукция КАК Продукция
	|	ПО ТаблицаТовары.Распоряжение = Продукция.Ссылка
...
Процедура ЗаполнитьДокументНаОснованииОтчетДавальцу2_5(ДанныеЗаполнения)
	...
	|				И ВЫРАЗИТЬ(Распоряжение КАК Документ.ЗаказДавальца2_5).Договор = &Договор) КАК Обороты
	|
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетДавальцу2_5.Продукция КАК Продукция
	|			ПО Обороты.Распоряжение = Продукция.ЗаказДавальца
	|			 И Обороты.КодСтроки    = Продукция.КодСтроки
	|			 И Продукция.Ссылка.Проведен
	|	ГДЕ
	|		НЕ ВЫРАЗИТЬ(Обороты.Распоряжение КАК Документ.ЗаказДавальца2_5).Ссылка ЕСТЬ NULL // для отработки RLS
	...
	|			ГДЕ
	|				РеестрДокументов.Ссылка = Обороты.Распоряжение
	...
	|			ПО Обороты.Распоряжение = ТаблицаТовары.ЗаказКлиента
