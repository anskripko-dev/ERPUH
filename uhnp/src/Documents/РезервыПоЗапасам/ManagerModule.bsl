...
Функция ПодготовитьПараметрыПроведения(ДополнительныеСвойства, Отказ) Экспорт
	...
	ИменаДоп = "ПериодОтчета, ДатаНачала, ДатаОкончания, ВидОтчета";
	Реквизиты = МСФОУХ.РеквизитыДокумента(Запрос, ИменаДоп, "ПериодОтчета", Отказ);
	ГраницыПериода = МСФОВНАВызовСервераУХ.ПолучитьГраницыПериодаДокумента(Реквизиты,,Истина);
	
	Реквизиты.Вставить("ГраницыПериода", 		ГраницыПериода);
...
Функция ПолучитьТаблицуРезервов(Контекст) Экспорт
	...
	ПутиУП = МСФОВызовСервераУХ.ПутиРеквизитовУП(
		"ФормироватьПроводкиМСФО, ПланСчетовМСФО, 
		|ФункциональнаяВалюта, ИсточникДляЧистойЦеныПродажиЗапасов");
	...
	Иначе
		...
		ТаблицаИспользованныеРезервы = Справочники.ВидыОтчетов.ПолучитьТаблицуТК(
			Контекст, ГруппаАналитик,,ОтборАналитик);
		...
		ТаблицаРезервы = Справочники.ВидыОтчетов.ПолучитьТаблицуТК(
			Контекст, ГруппаАналитик, ПоляВыборки, ОтборАналитик);

	КонецЕсли;
	
	Возврат Новый Структура("ТаблицаРезервы,ТаблицаИспользованныеРезервы",
		ТаблицаРезервы, ТаблицаИспользованныеРезервы);	
...
Процедура СформироватьДвиженияРезервыПоЗапасам(Движения, ДополнительныеСвойства, Отказ) Экспорт
	...
	МСФОВызовСервераУХ.ОтразитьДвиженияПоТаблицеПроводок(
		Движения, ТаблицаПроводок, Реквизиты, Отказ, Ложь);
