...
#Область ТекстСторноИсправление
	...
	|	ВЫБОР
	|		КОГДА НЕ ПодтверждениеСтавки0.ДатаПодтверждения ЕСТЬ NULL
	|				ТОГДА &Исправление
	|		ИНАЧЕ ЛОЖЬ
...
Функция ТекстЗапросаТаблицаОперацииСПрослеживаемымиТоварами(Запрос, ТекстыЗапроса, Регистры)
	...
	|ПОМЕСТИТЬ ВтОперацииСПТОснования
	|ИЗ
	|	РегистрСведений.ОперацииСПрослеживаемымиТоварами КАК ОперацииСПрослеживаемымиТоварами
	|ГДЕ
	|	ОперацииСПрослеживаемымиТоварами.Регистратор = &ДокументОснование
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|ГДЕ
	|	ОперацииСПрослеживаемымиТоварамиРасширенный.Регистратор = &ДокументОснование
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	&Период КАК Период,
	|	&Организация КАК Организация,
	|	ВтТаблицаТовары.ТипЗапасов КАК ТипЗапасов,
	...
	|	ВЫБОР
	|		КОГДА ВтТаблицаТовары.ТипЗапасов В (&ТипыСобственныхТоваров)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ПорядокОтраженияВОтчетностиПоПрослеживаемости.КнигаПродаж)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ПорядокОтраженияВОтчетностиПоПрослеживаемости.ЖурналСФ)
	|	КОНЕЦ КАК ОтражениеВОтчетности
	|ИЗ
	|	ВтТаблицаТовары КАК ВтТаблицаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтОперацииСПТОснования КАК ВтОперацииСПТОснования
	|		ПО ИСТИНА
	|ГДЕ
	|	&Исправление
	|	И &ДокументОснованиеКорректировкаРеализации
	|	И ВтТаблицаТовары.ТипНомераГТД В (ЗНАЧЕНИЕ(Перечисление.ТипыНомеровГТД.НомерРНПТ),
	|		ВтТаблицаТовары.ТипНомераГТД = ЗНАЧЕНИЕ(Перечисление.ТипыНомеровГТД.НомерРНПТКомплекта))
	|	И ВтОперацииСПТОснования.Регистратор ЕСТЬ NULL";
...
Функция ТекстЗапросаВтТаблицаТовары(Запрос, ТекстыЗапроса)
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СчетФактураВыданныйТовары.ТипЗапасов,
	...
	|		ПО СчетФактураВыданныйТовары.НомерГТД = НомераГТД.Ссылка
	|ГДЕ
	|	&Исправление
	|	И &ДокументОснованиеКорректировкаРеализации
...
Процедура ЗаполнитьТаблицуПлатежноРасчетныхДокументовПоВыборке(ПлатежноРасчетныеДокументы, ДанныеВыборки)
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка, "НомерСчетаФактурыНаАванс, ДатаСчетаФактурыНаАванс, НомерИсправленияСчетаФактурыНаАванс, ДатаИсправленияСчетаФактурыНаАванс");
		
	КонецЦикла;
	ПлатежноРасчетныеДокументы.Свернуть("ДатаПлатежноРасчетногоДокумента, НомерПлатежноРасчетногоДокумента, НомерСчетаФактурыНаАванс, ДатаСчетаФактурыНаАванс, НомерИсправленияСчетаФактурыНаАванс, ДатаИсправленияСчетаФактурыНаАванс", "");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение табличной части Платежно-расчетные документы Счет-фактур выданных';
									|en = 'Заполнение табличной части Платежно-расчетные документы Счет-фактур выданных'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.СчетФактураВыданный КАК СчетФактураВыданный
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.ПлатежноРасчетныеДокументы КАК ПлатежныеДокументы
	|		ПО (ПлатежныеДокументы.Ссылка = СчетФактураВыданный.Ссылка)
	|ГДЕ
	|	СчетФактураВыданный.Дата >= &Период
	|	И СчетФактураВыданный.Проведен
	|	И НЕ СчетФактураВыданный.Корректировочный
	|	И НЕ СчетФактураВыданный.Перевыставленный
	|	И ПлатежныеДокументы.Ссылка ЕСТЬ NULL
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Обновление документов ""Счет-фактура выданный"":';
								|en = 'Обновление документов ""Счет-фактура выданный"":'"));
	СписокОписаний.Добавить(НСтр("ru = '- заполнение ТЧ ""Платежно-расчетные документы"" по данным взаиморасчетов';
								|en = '- заполнение ТЧ ""Платежно-расчетные документы"" по данным взаиморасчетов'")); 
	...
	Пока ВыборкаСчетФактур.Следующий() Цикл
		...
		Попытка
			...
			Рекомендация = НСтр("ru = 'Заполните ТЧ Платежно-расчетные документы вручную или по кнопке Заполнить над ТЧ.';
								|en = 'Заполните ТЧ Платежно-расчетные документы вручную или по кнопке Заполнить над ТЧ.'");
			...
			Иначе
				...
				Пока Выборка.Следующий() Цикл
					...
					ЗаполнитьЗначенияСвойств(Строка, Выборка, "НомерСчетаФактурыНаАванс, ДатаСчетаФактурыНаАванс, НомерИсправленияСчетаФактурыНаАванс, ДатаИсправленияСчетаФактурыНаАванс");
	...
	Иначе
		...
		ШаблонСообщения = НСтр("ru = 'Обработана порция документов ""Счет-фактура выданный"": %1';
								|en = 'Обработана порция документов ""Счет-фактура выданный"": %1'");
