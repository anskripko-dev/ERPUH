...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ПроверитьСтранаПроисхождения(Отказ);
	#КонецОбласти
...
Процедура ЗаполнитьДокументНаОснованииПоступленияТоваровИУслуг(Знач ДокументОснование)
	...
	Запрос = Новый Запрос(
		...
		//++ УХ_Встраивание
		|	NULL  КАК СтранаПроисхождения223ФЗ,
		|	NULL  КАК СтранаПроизводителя,
		//-- УХ_Встраивание
		...
		|	ТаблицаТовары.Склад,
		//++ УХ_Встраивание
		|	ТаблицаТовары.СтранаПроисхождения223ФЗ,
		|	ТаблицаТовары.СтранаПроизводителя,
		//-- УХ_Встраивание
		...
		//++ УХ_Встраивание
		|	ТоварыДокументаПоступления.СтранаПроисхождения223ФЗ,
		|	ТоварыДокументаПоступления.СтранаПроизводителя,
		//-- УХ_Встраивание
		...
		//++ УХ_Встраивание
		|	ЕСТЬNULL(ТоварыПоступления.СтранаПроисхождения223ФЗ, ТоварыВозврата.СтранаПроисхождения223ФЗ) КАК СтранаПроисхождения223ФЗ,
		|	ЕСТЬNULL(ТоварыПоступления.СтранаПроизводителя, ТоварыВозврата.СтранаПроизводителя) КАК СтранаПроизводителя,
		//-- УХ_Встраивание
...
Процедура ПроверитьСтранаПроисхождения(Отказ) 
	Если ПолучитьФункциональнуюОпцию("ВестиУчетПоСтранамКорпоративныеЗакупки") Тогда
		Для Каждого СтрокаУсловия Из Товары Цикл 
