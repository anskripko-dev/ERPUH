...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПереданКПроизводству Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_ГрафикПроизводства" Тогда
		
		ЗаполнитьТекущееСостояниеЗаказа();
		НастроитьЗависимыеЭлементыФормы(ЭтаФорма, "ГрафикПроизводства");
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	// Обработчик подсистемы запрета редактирования реквизитов объектов
	Если НЕ ПереданКПроизводству Тогда
...
Процедура ДинамическаяСтруктураПриИзменении(Элемент)
	...
	ДинамическаяСтруктураПриИзмененииНаСервере();
...
Процедура ДатаПотребностиПриИзменении(Элемент)
	...
	Объект.ДатаПотребности = ДатаПотребностиНаФорме; // действует, только если дата вводится в шапке
	ЗаполнитьДатуВТЧПродукция(Объект.ДатаПотребности);
	НастроитьЗависимыеЭлементыФормы(ЭтаФорма, "ДатаПотребности");
	
КонецПроцедуры

&НаКлиенте
Процедура ДатаПотребностиОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Модифицированность = Истина;

	Если ВыбранноеЗначение = "УказываетсяВСтроках" Тогда
			
		ДатаПотребностиНаФорме = Элементы.ДатаПотребности.СписокВыбора.НайтиПоЗначению(ВыбранноеЗначение);
		
		Объект.ДатаПотребностиУказываетсяВСтроках = Истина;
	...
	Иначе
		...
		Если ВыбранноеЗначение = "ВыбратьИзКалендаря" Тогда
		
			ПараметрыФормы = Новый Структура("ПоясняющийТекст, НачальноеЗначение", НСтр("ru = 'Желаемая дата выпуска:';
																						|en = 'Requested release date:'") + " ", Объект.ДатаПотребности);
			Оповещение = Новый ОписаниеОповещения("ПриВыбореДатыПотребностиЗавершение", ЭтотОбъект);
		...
		Иначе
			...
			Если ТипЗнч(ВыбранноеЗначение) = Тип("Дата") Тогда
				ВыбраннаяДата = ВыбранноеЗначение;
			Иначе
				
				ВыбраннаяДата = '00010101';
				ДатаСобытия = НачалоДня(ОбщегоНазначенияУТВызовСервера.ДатаСеанса());
				
				Если ВыбранноеЗначение = "Сегодня" Тогда
					ВыбраннаяДата = ДатаСобытия;
				ИначеЕсли ВыбранноеЗначение = "Завтра" Тогда
					ВыбраннаяДата = ДатаСобытия + 86400;
				ИначеЕсли ВыбранноеЗначение = "ЧерезНеделю" Тогда
					ВыбраннаяДата = ДатаСобытия + 7*86400;
				Иначе
					// другие действия в текущей конфигурации не предусмотрены
				КонецЕсли;
				
			КонецЕсли;
			
			ПриВыбореДатыПотребностиЗавершение(ВыбраннаяДата);
...
&НаКлиенте
Процедура ПриВыбореДатыПотребностиЗавершение(ВыбраннаяДата, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если ВыбраннаяДата = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ДатаПотребностиНаФорме = ВыбраннаяДата;
	Объект.ДатаПотребностиУказываетсяВСтроках = Ложь;
	Объект.ДатаПотребности = ВыбраннаяДата;
	ЗаполнитьДатуВТЧПродукция(Объект.ДатаПотребности);
...
&НаКлиенте
Процедура ДатаПотребностиОчистка(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь; 
	
	ДатаПотребностиНаФорме = '00010101';
	Объект.ДатаПотребностиУказываетсяВСтроках = Ложь;
	Объект.ДатаПотребности = '00010101';
	ЗаполнитьДатуВТЧПродукция(Объект.ДатаПотребности);
...
Процедура РазмещениеВыпускаПриИзменении(Элемент)
	...
	УстановитьПризнакДатаПотребностиОбязательна(ЭтаФорма);
...
Процедура ПродукцияПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если ТекущиеДанные <> Неопределено И Не Объект.ДатаПотребностиУказываетсяВСтроках Тогда
		ТекущиеДанные.ДатаПотребности = Объект.ДатаПотребности;
	КонецЕсли;
...
Процедура ПродукцияНоменклатураПриИзменении(Элемент)
	...
	ЗаполнитьСкладПоУмолчаниюВСтроке(ЭтотОбъект, ТекущаяСтрока);
	...
	ДобавитьДействиеПроверитьЗаполнитьДолюСтоимости(СтруктураДействий, ЭтотОбъект);
	
	СтруктураЗаполненияСпецификации = УправлениеДаннымиОбИзделияхКлиентСервер.ПараметрыДействияПроверитьЗаполнитьСпецификацию(Объект, ТекущаяСтрока, ПараметрыВыбораСпецификаций);
	...
	Если Не НазначениеУказываетсяВШапкеДокумента(ЭтотОбъект) Тогда
...
&НаКлиенте
Процедура ПланироватьГрафик(Команда)
	
	Если Объект.Проведен И Не Модифицированность Тогда
		
		ОткрытьПланированиеГрафика();
		
	Иначе
		
		Оповещение = Новый ОписаниеОповещения("ПланироватьГрафикЗавершение", ЭтотОбъект);
		Кнопки = Новый СписокЗначений();
		ТекстВопроса = НСтр("ru = 'Для планирования заказ на производство должен быть проведен. Провести?';
							|en = 'You can only schedule posted production orders. Post the order?'");
		Кнопки.Добавить(КодВозвратаДиалога.Да,НСтр("ru = 'Провести';
													|en = 'Post'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена,НСтр("ru = 'Отмена';
														|en = 'Cancel'"));
...
&НаКлиенте
Процедура ПланироватьГрафикЗавершение(ОтветНаВопрос, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ПродукцияЗаполнитьДатуПотребности(Команда)
	
	МассивВыделенныхСтрок = Элементы.Продукция.ВыделенныеСтроки;
	Если МассивВыделенныхСтрок.Количество() > 0 Тогда
		ДатаПотребности = МинимальнаяДатаПотребности(Объект.Продукция);
		
		Оповещение = Новый ОписаниеОповещения("ЗаполнитьДатуПотребностиЗавершение", ЭтотОбъект, Новый Структура("МассивВыделенныхСтрок", МассивВыделенныхСтрок));
		
		ОбщегоНазначенияУТКлиент.ВвестиДатуСКонтролемПустогоЗначения(ДатаПотребности, НСтр("ru = 'Введите желаемую дату выпуска';
																							|en = 'Enter requested release date'"), ФорматДатыГрафикаЧастиДаты, Оповещение);
		
	Иначе
		ТекстПредупреждения = НСтр("ru = 'В документе не выбраны строки для заполнения. Желаемая дата не будет заполнена.';
									|en = 'No lines to fill are selected in the document. Requested date will not be filled.'");
...
#Область НоменклатураХарактеристика
	...
	ПараметрыУсловногоОформления.ИмяПоляЕдиницаИзмерения = "ПродукцияЕдиницаИзмерения";
...
#Область ДатаПотребности
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ДатаПотребностиОбязательна");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Ложь;
...
Процедура НастроитьЭлементыФормы()
	...
	ДоступенТипПроизводственногоПроцесса = УправлениеДаннымиОбИзделиях.ДоступноОписаниеТипаПроизводственногоПроцесса()
		ИЛИ (ЗначениеЗаполнено(Объект.ТипПроизводственногоПроцесса) И НЕ Объект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Сборка);
	
	Элементы.ТипПроизводственногоПроцесса.Видимость = ДоступенТипПроизводственногоПроцесса;
	Если ДоступенТипПроизводственногоПроцесса Тогда
	...
	Элементы.ДинамическаяСтруктура.Видимость = Не ПереданКПроизводству;
	...
	Элементы.ПродукцияДатаПотребности.Формат = ФорматДатыГрафика;
	Элементы.ПродукцияДатаПотребности.ФорматРедактирования = ФорматДатыГрафика;
	
	#Область ДатаПотребностиНаФорме
	
	ДоступноНесколькоДатПотребности = Объект.ДинамическаяСтруктура;
	
	ДатаПотребностиСписокВыбора = Элементы.ДатаПотребности.СписокВыбора;
	ДатаПотребностиСписокВыбора.Очистить();
	
	Если ДоступноНесколькоДатПотребности Тогда;
		ДатаПотребностиСписокВыбора.Добавить("УказываетсяВСтроках", НСтр("ru = 'Указывается построчно';
																		|en = 'Specified line by line'"));
		ДатаПотребностиСписокВыбора.Добавить("ВыбратьИзКалендаря", НСтр("ru = 'Выбрать...';
																		|en = 'Select...'"));
		ДатаПотребностиСписокВыбора.Добавить("Сегодня", НСтр("ru = 'Сегодня';
															|en = 'Today'"));
		ДатаПотребностиСписокВыбора.Добавить("Завтра", НСтр("ru = 'Завтра';
															|en = 'Tomorrow'"));
		ДатаПотребностиСписокВыбора.Добавить("ЧерезНеделю", НСтр("ru = 'Через неделю';
																|en = 'In a week'"));
	КонецЕсли; 
	
	Элементы.ДатаПотребности.КнопкаВыпадающегоСписка = ?(ДоступноНесколькоДатПотребности, Истина, Неопределено);
	Элементы.ДатаПотребности.КнопкаВыбора = ?(ДоступноНесколькоДатПотребности, Ложь, Неопределено);
	
	Если ДоступноНесколькоДатПотребности И Объект.ДатаПотребностиУказываетсяВСтроках Тогда
		ДатаПотребностиНаФорме = Элементы.ДатаПотребности.СписокВыбора.НайтиПоЗначению("УказываетсяВСтроках");
	Иначе
		ДатаПотребностиНаФорме = Объект.ДатаПотребности;		
	КонецЕсли;
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("ГрафикПроизводства")
		...
		Если Объект.ДинамическаяСтруктура И Форма.ПланируетсяГрафикПроизводства
			И Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовНаПроизводство2_2.КПроизводству") Тогда
			Элементы.ПланироватьГрафик.Видимость = Истина;
			Элементы.ПланироватьГрафик.ЦветТекста = ?(Форма.СтатусГрафикТребуетсяРассчитать = ПредопределенноеЗначение("Перечисление.СтатусыГрафикаПроизводстваВЗаказеНаПроизводство.ТребуетсяРассчитать"),
				Форма.ЦветаГиперссылок.Синий, Форма.ЦветаГиперссылок.Серый);
		Иначе
			Элементы.ПланироватьГрафик.Видимость = Ложь;
		КонецЕсли;
	...
	// Размещение выпуска не используется в заказах на разборку и в заказах с динамической структурой
	Если СтруктураРеквизитов.Свойство("ДинамическаяСтруктура") 
		ИЛИ СтруктураРеквизитов.Свойство("ТипПроизводственногоПроцесса")
		ИЛИ Инициализация Тогда
		
		Элементы.РазмещениеВыпуска.Видимость = Не (Объект.ДинамическаяСтруктура 
														Или Объект.ТипПроизводственногоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.Разборка"));
			
	КонецЕсли;
	
	// Желаемая дата выпуска обязательна к заполнению для динамических заказов и при планировании "К окончанию"
	// Для заказов с динамической структурой возможно указание желаемых дат выпуска в ТЧ
	Если СтруктураРеквизитов.Свойство("ДинамическаяСтруктура")
		ИЛИ СтруктураРеквизитов.Свойство("ТипПроизводственногоПроцесса")
		ИЛИ СтруктураРеквизитов.Свойство("РазмещениеВыпуска")
		ИЛИ СтруктураРеквизитов.Свойство("ДатаПотребности")
		ИЛИ Инициализация Тогда
		...
		Элементы.ДатаПотребности.Видимость = ДатаПотребностиИспользуется;
		Элементы.ПродукцияДатаПотребности.Видимость = ДатаПотребностиИспользуется И Объект.ДатаПотребностиУказываетсяВСтроках;
		Элементы.ПродукцияПродукцияЗаполнитьДатуПотребности.Видимость = ДатаПотребностиИспользуется И Объект.ДатаПотребностиУказываетсяВСтроках;
		
		Элементы.ДатаПотребности.АвтоОтметкаНезаполненного = Форма.ДатаПотребностиОбязательна И Не ЗначениеЗаполнено(Объект.ДатаПотребности);
		Элементы.ДатаПотребности.ОтметкаНезаполненного = Форма.ДатаПотребностиОбязательна И Не Объект.ДатаПотребностиУказываетсяВСтроках И Не ЗначениеЗаполнено(Объект.ДатаПотребности);
		Элементы.ПродукцияДатаПотребности.ОтметкаНезаполненного = Форма.ДатаПотребностиОбязательна;
		
		Элементы.ДатаПотребности.РедактированиеТекста = Не Объект.ДатаПотребностиУказываетсяВСтроках;
		Элементы.ДатаПотребности.ЦветФона = ?(Объект.ДатаПотребностиУказываетсяВСтроках, WebЦвета.ТопленоеМолоко, Новый Цвет());
	
	КонецЕсли;
	
	// Прочие элементы динамической структуры структура
	Если СтруктураРеквизитов.Свойство("ДинамическаяСтруктура")
		Или СтруктураРеквизитов.Свойство("Статус")
		Или Инициализация Тогда
...
&НаСервере
Процедура ДинамическаяСтруктураПриИзмененииНаСервере() 

	Если Объект.ДинамическаяСтруктура 
		И НЕ Объект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Сборка Тогда
		Объект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Сборка;
		ПроверитьОбновитьРеквизитыПриИзмененииТипаПроизводственногоПроцесса();
	КонецЕсли;

	УстановитьПризнакДатаПотребностиОбязательна(ЭтаФорма);
	
	НастроитьЭлементыФормы(); // Изменение поля серьезно меняет поведение документа 
...
&НаСервере
Процедура ПроверитьОбновитьРеквизитыПриИзмененииТипаПроизводственногоПроцесса()
...
Процедура ПродукцияОбработкаВыбораПодборПоЗаказамДавальцаНаСервере(ВыбранноеЗначение)
	...
	ТаблицаТоваров = Документы.ЗаказНаПроизводство2_2.ОстаткиПродукцииКОбеспечениюДавальцу(
		ВыбранноеЗначение, 
	...
	СтруктураДействий.Вставить("ЗаполнитьПризнакХарактеристикиИспользуются", Новый Структура("Номенклатура", "ХарактеристикиИспользуются"));
	ДобавитьДействиеПроверитьЗаполнитьДолюСтоимости(СтруктураДействий, ЭтотОбъект);
	ДоходыИРасходыКлиентСервер.ВставитьДействиеИнициализироватьСтатьюИАналитикуПослеДобавленияСтроки(СтруктураДействий, ЭтотОбъект, "Объект.Продукция");
	...
	ПослеИзмененияСтрокТЧПрограммноНаСервере(МассивСтрок, СтруктураДействий, КэшированныеЗначения);
	
КонецПроцедуры

&НаСервере
Процедура ПослеИзмененияСтрокТЧПрограммноНаСервере(КоллекцияСтрок, СтруктураРанееВыполненныхДействий = Неопределено, КэшированныеЗначения = Неопределено)
	...
	Если СтруктураРанееВыполненныхДействий = Неопределено
			Или НЕ СтруктураРанееВыполненныхДействий.Свойство("ЗаполнитьПризнакХарактеристикиИспользуются") Тогда
	...
	КонецЕсли;
	Если СтруктураРанееВыполненныхДействий = Неопределено 
			Или НЕ СтруктураРанееВыполненныхДействий.Свойство("ЗаполнитьПризнакАртикул") Тогда
		СтруктураДействий.Вставить("ЗаполнитьПризнакАртикул",
									Новый Структура("Номенклатура", "Артикул"));
	КонецЕсли;
	
	Если КоллекцияСтрок.Количество() > 0 И ТипЗнч(КоллекцияСтрок[0]) = Тип("ДанныеФормыЭлементКоллекции") Тогда
		МассивСтрок = КоллекцияСтрок;
	Иначе
		МассивСтрок = Новый Массив;
		Для Каждого ЭлементКоллекции Из КоллекцияСтрок Цикл
			МассивСтрок.Добавить(Объект.Продукция.НайтиПоИдентификатору(ЭлементКоллекции));
		КонецЦикла;
	КонецЕсли;
	...
	Для каждого Строка Из МассивСтрок Цикл
		...
		Если Не ИспользоватьНесколькоСкладов Тогда
			Строка.Склад = ?(Строка.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Работа, 
				Справочники.Склады.ПустаяСсылка(),
				СкладПоУмолчанию);
		КонецЕсли;
		
		Если Не Объект.ДатаПотребностиУказываетсяВСтроках Тогда
			Строка.ДатаПотребности = Объект.ДатаПотребности;
		КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ДобавитьДействиеПроверитьЗаполнитьДолюСтоимости(СтруктураДействий, Форма)
	
	Объект = Форма.Объект;
	
	Если Не Объект.ТипПроизводственногоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.БезСпецификаций") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьДатуПотребностиЗавершение(ВыбраннаяДата, ДополнительныеПараметры) Экспорт
	...
	Если ВыбраннаяДата <> Неопределено И ЗначениеЗаполнено(ВыбраннаяДата) Тогда
		...
		ЗаполнитьДатуВТЧПродукция(ДатаПотребности, МассивВыделенныхСтрок);
		
		ПоказатьОповещениеПользователя(
			НСтр("ru = 'Желаемая дата выпуска заполнена';
				|en = 'Requested release date is filled'"),,
			СтрШаблон(НСтр("ru = 'Для выделенных строк установлена желаемая дата выпуска отгрузки %1';
							|en = 'Requested shipment release date %1 is set for the selected lines'"), Формат(ДатаПотребности, "ДЛФ=D")),
			БиблиотекаКартинок.Информация32);
...
Процедура РассчитатьПроцентРаспределенияЗатратНаВыходныеИзделия()
	...
	Если Не Объект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.БезСпецификаций Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	Колонки = "Номенклатура,Характеристика,Упаковка,КоличествоУпаковок,Спецификация,Склад,Подразделение,ДатаПотребности";
...
Процедура ЗаполнитьСлужебныеРеквизиты()
	...
	ЦветаГиперссылок.Вставить("Серый", WebЦвета.Серый);
	ЦветаГиперссылок.Вставить("Красный", WebЦвета.Красный);
	ЦветаГиперссылок.Вставить("Синий", ЦветаСтиля.ГиперссылкаЦвет);
...
&НаСервере
Процедура ЗаполнитьТекущееСостояниеЗаказа()
	
	ТекущееСостояниеЗаказа = Новый Структура("ПереданКПроизводству,СтатусГрафикТребуетсяРассчитать",Ложь,Перечисления.СтатусыГрафикаПроизводстваВЗаказеНаПроизводство.ПустаяСсылка());
	
	Если Не Объект.Ссылка.Пустая() Тогда
		
		Запрос = Новый Запрос(
			"ВЫБРАТЬ
			|	Таблица.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовНаПроизводство2_2.КПроизводству), ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовНаПроизводство2_2.Закрыт)) КАК ПереданКПроизводству,
			|	ВЫБОР
			...
			|					ГДЕ
			|						ДД.ЗаказНаПроизводство = Таблица.Ссылка)
			|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтатусыГрафикаПроизводстваВЗаказеНаПроизводство.ТребуетсяРассчитать)
			|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СтатусыГрафикаПроизводстваВЗаказеНаПроизводство.ПустаяСсылка)
			|	КОНЕЦ КАК СтатусГрафикТребуетсяРассчитать
			|ИЗ
		...
		Запрос.УстановитьПараметр("ПланируетсяГрафикПроизводства", ПланируетсяГрафикПроизводства);
		РезультатЗапроса = Запрос.Выполнить();
		Если Не РезультатЗапроса.Пустой() Тогда
			Выборка = Запрос.Выполнить().Выбрать();
			Выборка.Следующий();
			ЗаполнитьЗначенияСвойств(ТекущееСостояниеЗаказа, Выборка);
		КонецЕсли;
		
	КонецЕсли;
	
	ЗаполнитьЗначенияСвойств(ЭтаФорма, ТекущееСостояниеЗаказа);
...
&НаКлиенте
Процедура ОткрытьПланированиеГрафика()
	
	СтруктураПараметров = Новый Структура("Распоряжение", Объект.Ссылка);
	ОткрытьФорму("Обработка.ПланированиеГрафикаПроизводства2_2.Форма.ПланированиеГрафикаЗаказа", СтруктураПараметров,, Объект.Ссылка);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьПризнакДатаПотребностиОбязательна(Форма)
	
	Объект = Форма.Объект;
	
	Форма.ДатаПотребностиОбязательна = Объект.ДинамическаяСтруктура И Не Форма.ПланируетсяГрафикПроизводства
		ИЛИ Не Объект.ДинамическаяСтруктура И Объект.РазмещениеВыпуска = ПредопределенноеЗначение("Перечисление.СпособыПривязкиОперацийПроизводства.КОкончанию");
...
&НаСервере
Процедура ЗаполнитьДатуВТЧПродукция(Знач НоваяДата, Знач ВыделенныеСтроки = Неопределено)

	ПроизводствоСервер.ЗаполнитьРеквизитВКоллекции(Объект.Продукция, "ДатаПотребности", НоваяДата, ВыделенныеСтроки);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция МинимальнаяДатаПотребности(Знач ТабличнаяЧасть)
	
	Даты = ТабличнаяЧасть.Выгрузить(Новый Структура("Отменено", Ложь), "ДатаПотребности");
	Даты.Сортировать("ДатаПотребности");
	Если Даты.Количество() > 0 Тогда
		Возврат Даты[0].ДатаПотребности;
	Иначе
		Возврат '00010101';
	КонецЕсли;
