...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		ТекстЗапросаТаблицаОбеспечениеПроизводственныхПроцессов(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект);
		ТекстЗапросаТаблицаЗаказыНаПроизводство(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект);  
...
Процедура ДобавитьКомандыОтчетов(КомандыОтчетов, Параметры) Экспорт
	...
	КомандаОтчета = Отчеты.ГрафикПроизводстваПродукции2_2.ДобавитьКомандуОтчета(КомандыОтчетов);
	Если Не КомандаОтчета = Неопределено Тогда
		КомандаОтчета.Порядок = 1;
	КонецЕсли;
	...
	Отчеты.МониторингЗаказаНормативныйГрафик.ДобавитьКомандуОтчета(КомандыОтчетов);
	...
	Если Выборка.Следующий() Тогда
		...
		Попытка
			...
			Если ПриоритетИзменен Тогда
				УниверсальнаяОчередьОперативныхЗаданий.ОбработатьЗадание(ТекущийЗаказ, Перечисления.ЗаданияУниверсальнойОчереди.ОбновлениеПриоритетаВЭтапахПроизводства);
			КонецЕсли;
	
			Если ЗаказОбъект.ДинамическаяСтруктура Тогда
				УниверсальнаяОчередьОперативныхЗаданий.ОбработатьЗадание(ТекущийЗаказ, Перечисления.ЗаданияУниверсальнойОчереди.ОбновлениеСтруктурыЗаказаПриИзмененииМестаВОчереди);
			КонецЕсли;
...
//
Процедура УстранитьКонфликтыВОчереди(ТекущийЗаказ, Отказ) Экспорт
	...
	Если ВыборкаНоваяПозиция.Следующий() Тогда
		...
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущийЗаказ, "Подразделение") = ВыборкаНоваяПозиция.Подразделение Тогда
			ВставитьДокументВОчередь(ТекущийЗаказ, ВыборкаНоваяПозиция.НоваяПозиция);
		Иначе
			...
			Запрос.Текст =
				...
				|	ГДЕ
				|		ЗаказНаПоступление.Ссылка = &ТекущийЗаказ
			...
			Если ВыборкаОчередь.Следующий() Тогда
				...
				Исключение
					ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось записать %1. %2';
						...
						ТекущийЗаказ,
						КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
					ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, ТекущийЗаказ,,, Отказ);
				КонецПопытки;
...
//  ДанныеЗаказа	 - Структура					 - номер и дата заказа.
//
// Возвращаемое значение:
//  Строка - представление заказа на производство.
//
Функция ПредставлениеЗаказа(Ссылка, Знач Префикс = Неопределено, Знач ДанныеЗаказа = Неопределено) Экспорт
	
	Если ДанныеЗаказа = Неопределено Тогда
		ДанныеЗаказа = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "Номер,Дата");
	КонецЕсли;

	НомерЗаказа = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеЗаказа.Номер, Ложь, Истина);
...
//
Функция ПолноеПредставлениеЗаказа(Ссылка) Экспорт
	
	ДанныеЗаказа = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка,"Номер,Дата,ДатаПотребности,ДатаПотребностиУказываетсяВСтроках");
	
	НомерЗаказа = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеЗаказа.Номер, Ложь, Истина);
	НомерЗаказа = СокрЛП(НомерЗаказа);
	
	Если ДанныеЗаказа.ДатаПотребностиУказываетсяВСтроках Тогда
		ПредставлениеЗаказа = СтрШаблон(НСтр("ru = 'Заказ № %1 от %2, дата потребности %3';
											|en = 'Order No. %1 from %2, demand date %3'"), НомерЗаказа, Формат(ДанныеЗаказа.Дата, "ДЛФ=D"), Формат(ДанныеЗаказа.ДатаПотребности, "ДЛФ=D"));
	Иначе
		ПредставлениеЗаказа = СтрШаблон(НСтр("ru = 'Заказ № %1 от %2';
											|en = 'Order No. %1 from %2'"), НомерЗаказа, Формат(ДанныеЗаказа.Дата, "ДЛФ=D"));
	КонецЕсли; 
...
Процедура ЗадатьОписаниеЗаполняемыхРеквизитовПоСтатистике(ОписаниеРеквизитов) Экспорт
	...
	ЗаполнениеОбъектовПоСтатистике.ДобавитьОписаниеЗаполняемыхРеквизитов(ОписаниеРеквизитов,
		"Подразделение,Организация", Параметры);
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	Таблица.ДатаПотребностиУказываетсяВСтроках КАК ДатаПотребностиУказываетсяВСтроках
	|ИЗ
	|	Документ.ЗаказНаПроизводство2_2 КАК Таблица
	|ГДЕ
...
Функция ТекстЗапросаТаблицаОбеспечениеПроизводственныхПроцессов(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если ДокументОбъект <> Неопределено
	   И Не ((ДокументОбъект.Статус = Перечисления.СтатусыЗаказовНаПроизводство2_2.КПроизводству
				Или ДокументОбъект.Статус = Перечисления.СтатусыЗаказовНаПроизводство2_2.Закрыт)
			 И Не ДокументОбъект.ДинамическаяСтруктура) Тогда
		Возврат "";
	КонецЕсли;
	
	Если Не ПроведениеДокументов.ЕстьТаблицаЗапроса("ДанныеДокументаТаблицаПродукция", ТекстыЗапроса) Тогда
		ТекстЗапросаДанныеДокументаТаблицаПродукция(Запрос, ТекстыЗапроса, ДокументОбъект);
	КонецЕсли;
	...
	|
	|	ВЫБОР 
	|		КОГДА ТабличнаяЧасть.НоменклатураТипНоменклатуры В (
	...
	|
	|	ВЫБОР 
	|		КОГДА ТабличнаяЧасть.НоменклатураТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа) 
	...
	|
	|ИЗ
	|	ДанныеДокументаТаблицаПродукция КАК ТабличнаяЧасть
	|ГДЕ
	|	&Статус В (
	|		&СтатусКПроизводству,
	|		&СтатусЗакрыт)
...
Функция ТекстЗапросаТаблицаЗаказыНаПроизводство(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если ДокументОбъект <> Неопределено
	   И Не (ДокументОбъект.Проведен
			 И ДокументОбъект.ДинамическаяСтруктура
			 И (ДокументОбъект.Статус = Перечисления.СтатусыЗаказовНаПроизводство2_2.КПроизводству
				Или ДокументОбъект.Статус = Перечисления.СтатусыЗаказовНаПроизводство2_2.Закрыт)) Тогда
		Возврат "";
	КонецЕсли;
	...
	|		ТабличнаяЧасть.Склад
	|	КОГДА ТабличнаяЧасть.СписатьНаРасходы
	|		ИЛИ ТабличнаяЧасть.НоменклатураТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа) ТОГДА
	|		ТабличнаяЧасть.Подразделение
	|	ИНАЧЕ
	|		НЕОПРЕДЕЛЕНО
	|	КОНЕЦ                                                                     КАК Склад,
	...
	|	ВЫБОР
	|		КОГДА &ДатаПотребностиУказываетсяВСтроках
	|				И НЕ ТабличнаяЧасть.ДатаПотребности = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА РАЗНОСТЬДАТ(ТабличнаяЧасть.ДатаПотребности, НАЧАЛОПЕРИОДА(&Период, ДЕНЬ), СЕКУНДА)
	|		КОГДА &ДатаПотребностиУказываетсяВСтроках
	|				И НЕ &ДатаПотребности = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА РАЗНОСТЬДАТ(&ДатаПотребности, НАЧАЛОПЕРИОДА(&Период, ДЕНЬ), СЕКУНДА)
	|		ИНАЧЕ 0
	|	КОНЕЦ                                                                     КАК ДлительностьДоВыпуска,
	|	ТабличнаяЧасть.Количество                                                 КАК Заказано,
	|	ВЫБОР
	|		КОГДА &Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовНаПроизводство2_2.КПроизводству) ТОГДА
	|			ТабличнаяЧасть.Количество
	|	ИНАЧЕ
	|		0
...
Функция АдаптированныйТекстЗапросаДвиженийПоРегистру(ИмяРегистра) Экспорт
	...
	ПереопределениеРасчетаПараметров = ПереопределениеРасчетаПараметров(
										СтрШаблон("%1.Ссылка", СинонимТаблицыДокумента),
										ИмяРегистра);
	
	Если ИмяРегистра = "ЗаказыНаПроизводство" Тогда
		
		ТекстЗапроса = ТекстЗапросаТаблицаЗаказыНаПроизводство(Запрос, ТекстыЗапроса, ИмяРегистра, Неопределено);
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
									"ДанныеДокументаТаблицаПродукция",
	...
	ИначеЕсли ИмяРегистра = "ОбеспечениеПроизводственныхПроцессов" Тогда
		...
		ТекстЗапроса = ТекстЗапросаТаблицаОбеспечениеПроизводственныхПроцессов(
						Запрос,
						ТекстыЗапроса,
						ИмяРегистра,
						Неопределено);
	...
	ИначеЕсли ИмяРегистра = "РеестрДокументов" Тогда
		...
		ТекстЗапроса = ТекстЗапросаТаблицаРеестрДокументов(Запрос, ТекстыЗапроса, ИмяРегистра, Неопределено);
...
Функция ПереопределениеРасчетаПараметров(ОсновнаяТаблица, ИмяРегистра)
	
	Результат = Новый Структура;
	
	Если ИмяРегистра = "ОбеспечениеПроизводственныхПроцессов" Тогда
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	Если ДокументОбъект = Неопределено
	 Или (ДокументОбъект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Сборка
		   Или ДокументОбъект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.БезСпецификаций) Тогда
		...
		ТекстЗапросаТабЧасть =
			...
			|			КОГДА ТабЧасть.НоменклатураТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа) ТОГДА
			|					ТабЧасть.Подразделение
			|				ИНАЧЕ
			|					ТабЧасть.Склад
			|		КОНЕЦ КАК Склад,
			|	
			|	ВЫБОР КОГДА &ТипПроизводственногоПроцесса = ЗНАЧЕНИЕ(Перечисление.ТипыПроизводственныхПроцессов.БезСпецификаций) ТОГДА
			|			&НазначениеПродукция
			|		ИНАЧЕ
			|			ТабЧасть.Назначение
			|		КОНЕЦ КАК Назначение,
			...
			|	
			|	ВЫБОР КОГДА НЕ ГрафикВыпускаПродукции.ЗаказНаПроизводство ЕСТЬ NULL ТОГДА
			|											
			|			НАЧАЛОПЕРИОДА(ГрафикВыпускаПродукции.ДатаГрафика, ДЕНЬ)
			|												
			|		ИНАЧЕ
			|									
			...
			|	
			|	ВЫБОР КОГДА &Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовНаПроизводство2_2.КПроизводству)
			|			И &ДинамическаяСтруктура ТОГДА
			|											
			|			ISNULL(ГрафикВыпускаПродукции.ДатаГрафика, ТабЧасть.ДатаПотребности) <> ДАТАВРЕМЯ(1, 1, 1)
			|												
			|		ИНАЧЕ
			|									
			|			ЛОЖЬ
			...
			|	
			|	ISNULL(ГрафикВыпускаПродукции.Количество, ТабЧасть.Количество) КАК Количество,
			|	
			|	ЛОЖЬ                            КАК ПоГрафику,
			|	НЕОПРЕДЕЛЕНО                    КАК РаспоряжениеВГрафике,
			|	0                               КАК КоличествоВГрафике
			|ИЗ
			|	ДанныеДокументаТаблицаПродукция КАК ТабЧасть
			|		
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикВыпускаПродукцииЗаказа КАК ГрафикВыпускаПродукции
			|		ПО ТабЧасть.Номенклатура        = ГрафикВыпускаПродукции.Номенклатура
			|		 И ТабЧасть.Характеристика      = ГрафикВыпускаПродукции.Характеристика
			|		 И ТабЧасть.Склад               = ГрафикВыпускаПродукции.Склад
			|		 И ТабЧасть.Назначение          = ГрафикВыпускаПродукции.Назначение
			|		 И ТабЧасть.Ссылка              = ГрафикВыпускаПродукции.ЗаказНаПроизводство
			|		 И ТабЧасть.КлючНоменклатура    = ГрафикВыпускаПродукции.КлючНоменклатура
			|		
			|ГДЕ
			|	НЕ ТабЧасть.Отменено
			|		И НЕ &Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовНаПроизводство2_2.Закрыт)
			|		И &ТипПроизводственногоПроцесса В(
		...
		РаспределениеЗапасовДвижения.ЗапланироватьПриходЗапаса(
			Запрос,
			ТекстыЗапроса,
			Регистры,
			ТекстЗапросаТабЧасть,
	...
	Если ДокументОбъект = Неопределено
	 Или (ДокументОбъект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Сборка
		   Или ДокументОбъект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.БезСпецификаций)
		 И ДокументОбъект.Продукция.НайтиСтроки(Новый Структура("СписатьНаРасходы", Истина)).Количество() > 0 Тогда
...
Функция ТекстЗапросаТаблицаРеестрДокументов(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект)
...
Функция ТекстЗапросаДанныеДокументаТаблицаПродукция(Запрос, ТекстыЗапроса, ДокументОбъект)
	
	ИмяТаблицы = "ДанныеДокументаТаблицаПродукция";
	...
	|	ТабличнаяЧасть.НомерСтроки                  КАК НомерСтроки,
	|	ТабличнаяЧасть.Номенклатура                 КАК Номенклатура,
	|	ТабличнаяЧасть.Номенклатура.ТипНоменклатуры КАК НоменклатураТипНоменклатуры,
	|	ТабличнаяЧасть.Характеристика               КАК Характеристика,
	|	ТабличнаяЧасть.Спецификация                 КАК Спецификация,
	...
	|	ТабличнаяЧасть.КлючНоменклатура             КАК КлючНоменклатура
	|ПОМЕСТИТЬ ДанныеДокументаТаблицаПродукция
	|ИЗ
...
Функция ПротоколРасчетаСтруктурыЗаказа(ЗаказНапроизводство) Экспорт
	...
	ВыполняетсяРасчет = СтруктураЗаказа.ВыполняетсяРасчет()
		Или Не УправлениеПроизводствомПовтИсп.ИспользуетсяГрафикПроизводства()
			И РегистрыСведений.НормативныйГрафикСтруктурыЗаказа.ВыполняетсяРасчет();
	...
	Запрос.УстановитьПараметр("ИспользуетсяНормативныйГрафикПроизводства",
		Не УправлениеПроизводствомПовтИсп.ИспользуетсяГрафикПроизводства());
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	1 КАК Код
	|ПОМЕСТИТЬ Ошибки
	...
	|				ГДЕ
	|					Т.ЗаказНаПроизводство = Таблица.Ссылка)
	|					И &ИспользуетсяНормативныйГрафикПроизводства
	|					И НЕ ИСТИНА В
	|						(ВЫБРАТЬ ПЕРВЫЕ 1
	|							ИСТИНА
	|						ИЗ
...
//             * Объект                 - ДокументСсылка          - ссылка на заказ
...
//
Процедура ОбновитьПланируемоеПоступлениеТоваров(
			Задания,
	...
	МассивДокументов = ОбщегоНазначенияУТ.ВыгрузитьЗначенияКолонки(Задания, "Объект");
	...
	Запрос.Текст = 
		...
		|	СУММА(СтруктураЗаказа.Требуется + СтруктураЗаказа.Готово) КАК Требуется,
		...
		|
		|ГДЕ
		|	СтруктураЗаказа.ЗаказНаПроизводство В (&Заказы)
		...
		|
		|ИМЕЮЩИЕ
		|	СУММА(СтруктураЗаказа.Требуется + СтруктураЗаказа.Готово) > 0
		...
		|	СтруктураЗаказа.ЗаказНаПроизводство                       КАК ЗаказНаПроизводство,
		|	СтруктураЗаказа.КлючНоменклатура                          КАК КлючНоменклатура,
		|	ТабЧасть.ДатаПотребности                                  КАК ДатаГрафика,
		...
		|	СтруктураЗаказа КАК СтруктураЗаказа
		|
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказНаПроизводство2_2.Продукция КАК ТабЧасть
		|		ПО СтруктураЗаказа.ЗаказНаПроизводство = ТабЧасть.Ссылка
		|		 И СтруктураЗаказа.Номенклатура        = ТабЧасть.Номенклатура
		|		 И СтруктураЗаказа.КлючНоменклатура    = ТабЧасть.КлючНоменклатура
		|
		|ГДЕ
		...
		|	ЗаказНаПроизводство
		|";
	Запрос.УстановитьПараметр("Заказы", МассивДокументов);
	
	ВыборкаЗаказы = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам); 
	
	МассивЗаказовКОбновлению = Новый Массив();
	
	НаборЗаписей = РегистрыСведений.ГрафикВыпускаПродукцииЗаказа.СоздатьНаборЗаписей();
	
	Пока ВыборкаЗаказы.Следующий() Цикл
		
		НаборЗаписей.Отбор.ЗаказНаПроизводство.Установить(ВыборкаЗаказы.ЗаказНаПроизводство);
		...
		МассивЗаказовКОбновлению.Добавить(ВыборкаЗаказы.ЗаказНаПроизводство);
		
	КонецЦикла;
	
	Для каждого ДокументСсылка Из МассивЗаказовКОбновлению Цикл 
		...
		Попытка
			...
			Если ДокументОбъект.Статус = Перечисления.СтатусыЗаказовНаПроизводство2_2.КПроизводству
		...
		Исключение
			...
			ДанныеОшибки.Объект = ДокументСсылка;
...
Функция ТекстЗапросаДляРасчетаСостоянийЗаказов(ВыборкаТипыЗаказов) Экспорт
	
	ТекстыЗапроса = Новый Массив();
	
	ТекстыЗапроса.Добавить(ТекстЗапросаВТНезавершенныеЗаказы(ВыборкаТипыЗаказов));
	ТекстыЗапроса.Добавить(ТекстЗапросаВТТребуетсяСформироватьЭтапы(ВыборкаТипыЗаказов));
	ТекстыЗапроса.Добавить(ТекстЗапросаВТСостоянияЭтапов());
	...
	ТекстЗапроса =
		...
		|						И ISNULL(ВТНезавершенныеЗаказы.ДатаПотребности, ДАТАВРЕМЯ(1,1,1)) <> ДАТАВРЕМЯ(1,1,1)
		|						И ИСТИНА В
		|							(ВЫБРАТЬ ПЕРВЫЕ 1
		|								ИСТИНА
		|							ИЗ
		...
		|							ГДЕ
		|								График.Окончание > ВТНезавершенныеЗаказы.ДатаПотребности
		|								И График.ЗаказНаПроизводство = ВТНезавершенныеЗаказы.Ссылка
		...
		|
		|	ВЫБОР
		|		КОГДА ВТНезавершенныеЗаказы.Ссылка ЕСТЬ НЕ NULL ТОГДА 
		|			ВТНезавершенныеЗаказы.ДатаПотребности
		|		ИНАЧЕ
		|			ДАТАВРЕМЯ(1,1,1)
		|	КОНЕЦ                                 КАК ДатаСобытия,
		|
		|	ВЫБОР
		|		КОГДА РеквизитыДокумента.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовНаПроизводство2_2.КПроизводству)
		|				И РеквизитыДокумента.Проведен ТОГДА 
		|			ИСТИНА
		|		ИНАЧЕ
		|			ЛОЖЬ
...
Функция ТекстЗапросаВТНезавершенныеЗаказы(ВыборкаТипыЗаказов)
	
	ТекстыВыборка = Новый Массив;
	ТекстыВременныеТаблицы = Новый Массив;
	
	Если ВыборкаТипыЗаказов.ЕстьЗаказыСтатическаяСтруктура Тогда
		...
		ТекстЗапроса =
			"ВЫБРАТЬ
			|	Таблица.Распоряжение КАК Ссылка,
			|   Таблица.Распоряжение.ДатаПотребности КАК ДатаПотребности
			|ИЗ
			|	(ВЫБРАТЬ
			|		Таблица.Распоряжение   КАК Распоряжение,
			|		Таблица.Номенклатура   КАК Номенклатура,
			...
			|
			|	ГДЕ
			|		(Таблица.Распоряжение, Таблица.Номенклатура, Таблица.Характеристика, Таблица.Назначение) В
			|				(ВЫБРАТЬ
			|					Отбор.Распоряжение,
			|					Отбор.Номенклатура,
	...
	Если ВыборкаТипыЗаказов.ЕстьЗаказыДинамическаяСтруктура Тогда
		
		ТекстЗапроса =
			"ВЫБРАТЬ
			|	Таблица.ЗаказНаПроизводство КАК Ссылка,
			|   МИНИМУМ(Таблица.ДатаПотребности) КАК ДатаПотребности
			|ИЗ
			|	(ВЫБРАТЬ
			|		Таблица.Номенклатура        КАК Номенклатура,
			|		Таблица.Характеристика      КАК Характеристика,
			...
			|		МИНИМУМ(ПродукцияЗаказа.ДатаПотребности) КАК ДатаПотребности
			|	ИЗ
			...
			|			ПО ПродукцияЗаказа.Ссылка = Таблица.ЗаказНаПроизводство
			|				И ПродукцияЗаказа.КлючНоменклатура = Таблица.КлючНоменклатура
			|
			|	ГДЕ
			|		Таблица.ЗаказНаПроизводство В (&МассивСсылок)
			|			И Таблица.ВидСтроки = ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.Продукция)
			|
			|СГРУППИРОВАТЬ ПО
			|		Таблица.Номенклатура,
			|		Таблица.Характеристика,
	...
	ТекстЗапроса = ?(ТекстыВременныеТаблицы.Количество() = 0, 
		"", СтрСоединить(ТекстыВременныеТаблицы, ОбщегоНазначения.РазделительПакетаЗапросов()) + ОбщегоНазначения.РазделительПакетаЗапросов())
		+ СтрЗаменить(
			"ВЫБРАТЬ
			|	Таблица.Ссылка КАК Ссылка,
			|	Таблица.ДатаПотребности КАК ДатаПотребности
...
Функция ТекстЗапросаВТТребуетсяСформироватьЭтапы(ВыборкаТипыЗаказов)
	
	Если Не ВыборкаТипыЗаказов.ЕстьЗаказыСтатическаяСтруктура Тогда
...
//  ТекущиеДанныеОбъекта - Структура, ФормаКлиентскогоПриложения - Текущие данные редактируемого документа. Если вызов происходит из формы,
//                                                           тогда ФормаКлиентскогоПриложения или структура с ключами:
//                                                             * Объект - ДокументОбъект.ЗаказНаПроизводство2_2 - редактируемый объект.
//                                                             * Модифицированность - Булево - признак модифицированность
// Возвращаемое значение:
//	ТаблицаЗначений - таблица для заполнения табличной части продукции заказа на производство по заказу давальца.
//
Функция ОстаткиПродукцииКОбеспечениюДавальцу(МассивЗаказов, ТипПроизводственногоПроцесса = Неопределено, ТекущиеДанныеОбъекта = Неопределено) Экспорт

	ТекстыЗапроса = Новый Массив();
	
	// Товары из заказа давальца
	ТекстЗапроса =
		"ВЫБРАТЬ
	...
	// Товары поставленные ранее (с поправкой обеспечения)
	ТекстЗапроса = "";
	Если ТекущиеДанныеОбъекта <> Неопределено И ТекущиеДанныеОбъекта.Модифицированность Тогда
		...
		ТекстЗапроса = 
			...
			|	ВЫБОР
			|		КОГДА ТИПЗНАЧЕНИЯ(ТЧ.Склад) = ТИП(Справочник.Склады)
			|			ТОГДА ТЧ.Склад
			|		ИНАЧЕ ТЧ.Подразделение
			|	КОНЕЦ             КАК Склад,
		...
		ТекстЗапроса = 
			...
			|	ТЧ КАК ТЧ
			|ГДЕ
			|	(ТЧ.Номенклатура, ТЧ.Характеристика, ТЧ.Склад, ТЧ.Назначение) В
			|				(ВЫБРАТЬ
			|					ДД.Номенклатура,
			|					ДД.Характеристика,
			|					ДД.Склад,
			...
			|ГДЕ
			|	(Движения.Номенклатура, Движения.Характеристика, Движения.Склад, Движения.Назначение) В
			|				(ВЫБРАТЬ
			|					ДД.Номенклатура,
			|					ДД.Характеристика,
			|					ДД.Склад,
	...
	ТекстЗапроса = ТекстЗапроса + 
		...
		|ГДЕ
		|	(ТаблицаОстатков.Номенклатура, ТаблицаОстатков.Характеристика, ТаблицаОстатков.Склад, ТаблицаОстатков.Назначение) В
		|			(ВЫБРАТЬ
		|				ДД.Номенклатура,
		|				ДД.Характеристика,
		|				ДД.Склад,
	...
	ТекстЗапроса = СтрЗаменить(
		...
		|ИЗ
		|	ОбъединениеТаблицВыборкиДанных КАК Набор
		|		
		|СГРУППИРОВАТЬ ПО
		...
		"ОбъединениеТаблицВыборкиДанных", "(" + ТекстЗапроса + ")");
	...
	ТекстЗапроса =
		...
		|
		|УПОРЯДОЧИТЬ ПО
		|	ЗаказТовары.ДатаПотребности, ЗаказТовары.НомерСтроки
		|"; 
	ТекстЗапроса = СтрЗаменить(
		ТекстЗапроса, 
		"&ТекстЗапросаКоэффициентУпаковки", 
		Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки("ЗаказТовары.Упаковка", "ЗаказТовары.Номенклатура"));  
	ТекстыЗапроса.Добавить(ТекстЗапроса);
	
	ЗначенияЗаполнения = Новый Структура();
	ЗначенияЗаполнения.Вставить("НачатьНеРанее",     НачалоДня(ТекущаяДатаСеанса()));
	ЗначенияЗаполнения.Вставить("РазмещениеВыпуска", Перечисления.СпособыПривязкиОперацийПроизводства.КНачалу);
	
	ТаблицаТоваров = Новый ТаблицаЗначений();  
	ТаблицаТоваров.Колонки.Добавить("Номенклатура", Метаданные.Документы.ЗаказНаПроизводство2_2.ТабличныеЧасти.Продукция.Реквизиты.Номенклатура.Тип);
	ТаблицаТоваров.Колонки.Добавить("Характеристика", Метаданные.Документы.ЗаказНаПроизводство2_2.ТабличныеЧасти.Продукция.Реквизиты.Характеристика.Тип);
	ТаблицаТоваров.Колонки.Добавить("Склад", Метаданные.Документы.ЗаказНаПроизводство2_2.ТабличныеЧасти.Продукция.Реквизиты.Склад.Тип);
	ТаблицаТоваров.Колонки.Добавить("Подразделение", Метаданные.Документы.ЗаказНаПроизводство2_2.ТабличныеЧасти.Продукция.Реквизиты.Подразделение.Тип);
	ТаблицаТоваров.Колонки.Добавить("Назначение", Метаданные.Документы.ЗаказНаПроизводство2_2.ТабличныеЧасти.Продукция.Реквизиты.Назначение.Тип);
	ТаблицаТоваров.Колонки.Добавить("Спецификация", Метаданные.Документы.ЗаказНаПроизводство2_2.ТабличныеЧасти.Продукция.Реквизиты.Спецификация.Тип);
	ТаблицаТоваров.Колонки.Добавить("ДатаПотребности", Метаданные.Документы.ЗаказНаПроизводство2_2.ТабличныеЧасти.Продукция.Реквизиты.ДатаПотребности.Тип);
	ТаблицаТоваров.Колонки.Добавить("Упаковка", Метаданные.Документы.ЗаказНаПроизводство2_2.ТабличныеЧасти.Продукция.Реквизиты.Упаковка.Тип);
	ТаблицаТоваров.Колонки.Добавить("КоличествоУпаковок", Метаданные.Документы.ЗаказНаПроизводство2_2.ТабличныеЧасти.Продукция.Реквизиты.КоличествоУпаковок.Тип);
	ТаблицаТоваров.Колонки.Добавить("Количество", Метаданные.Документы.ЗаказНаПроизводство2_2.ТабличныеЧасти.Продукция.Реквизиты.Количество.Тип);
	// доп поля
	ТаблицаТоваров.Колонки.Добавить("НачатьНеРанее", Метаданные.Документы.ЗаказНаПроизводство2_2.Реквизиты.НачатьНеРанее.Тип);
	ТаблицаТоваров.Колонки.Добавить("РазмещениеВыпуска", Метаданные.Документы.ЗаказНаПроизводство2_2.Реквизиты.РазмещениеВыпуска.Тип);
	...
	Если ТекущиеДанныеОбъекта <> Неопределено И ТекущиеДанныеОбъекта.Модифицированность Тогда 
		Объект = ТекущиеДанныеОбъекта.Объект;
		Запрос.УстановитьПараметр("ТекущийДокумент", Объект.Ссылка);
		Запрос.УстановитьПараметр("ТЧ", Объект.Продукция.Выгрузить(Новый Структура("Отменено", Ложь)));
	КонецЕсли;
	...
	ОформленоКРаспределению = РезультатЗапроса[КоличествоПакетов-2].Выгрузить(); 
	ОформленоКРаспределению.Индексы.Добавить("Номенклатура,Характеристика,Склад,Назначение");
	СтруктураОтбораОформлено = Новый Структура("Номенклатура,Характеристика,Склад,Назначение");
		
	ВыборкаПродукция = РезультатЗапроса[КоличествоПакетов-1].Выбрать();
	Пока ВыборкаПродукция.Следующий() Цикл
		
		ЗаполнитьЗначенияСвойств(СтруктураОтбораОформлено, ВыборкаПродукция); 
		СтруктураОтбораОформлено.Склад = ?(ЗначениеЗаполнено(ВыборкаПродукция.Склад), ВыборкаПродукция.Склад, 
			ВыборкаПродукция.Подразделение);
		
		НайденныеСтроки = ОформленоКРаспределению.НайтиСтроки(СтруктураОтбораОформлено);   
		Если НайденныеСтроки.Количество() = 0 Тогда  
			КоличествоКОформлению = ВыборкаПродукция.Количество;
		Иначе 
			Распределено = Мин(ВыборкаПродукция.Количество, НайденныеСтроки[0].Количество); 
			КоличествоКОформлению = ВыборкаПродукция.Количество - Распределено ; 
			НайденныеСтроки[0].Количество = НайденныеСтроки[0].Количество - Распределено;
		КонецЕсли;
		...
		Если КоличествоКОформлению > 0 Тогда
			...
			НоваяСтрока.Количество = КоличествоКОформлению;
			НоваяСтрока.КоличествоУпаковок = НоваяСтрока.Количество / ВыборкаПродукция.КоэффициентУпаковки;
		КонецЕсли;
	...
	Пока ВыборкаШапка.Следующий() Цикл
		...
		Если ВыборкаПродукция.НайтиСледующий(Новый Структура("Ссылка", ВыборкаШапка.Ссылка)) Тогда
			...
			Если ВыводитьКоды Тогда
				...
				ОбластьСтрока.Области.СтрокаТаблицы_СКодом_ДатаПотребности.Формат = ФорматДатыГрафика;
			...
			Иначе
				...
				ОбластьСтрока.Области.СтрокаТаблицы_БезКода_ДатаПотребности.Формат = ФорматДатыГрафика;
			КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("12db78d0-0d3c-5ae2-939e-38ab73fc2444");
	...
	СписокОписаний.Добавить(НСтр("ru = 'Обновление документов ""Заказ на производство"":';
								|en = 'Update the ""Production order"" documents:'"));
	СписокОписаний.Добавить(НСтр("ru = '- заполняет реквизит ""Желаемая дата выпуска"" табличной части ""Продукция"".';
								|en = '- Fills the ""Requested release date"" attribute of the ""Manufactured products"" table.'"));
	СписокОписаний.Добавить(НСтр("ru = '- заменяет аналитику с типом перечисление типы налогов на справочник виды налогов и взносов';
								|en = '- Replaces the dimension value ""Tax types"" enumeration type with the ""Tax and contribution kinds"" catalog.'"));
	...
	Изменяемые.Добавить(Метаданные.Документы.ЗаказНаПроизводство2_2.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Документы.ЗаказНаПроизводство2_2.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.ЗаказНаПроизводство2_2";
	...
	|	Документ.ЗаказНаПроизводство2_2.Продукция КАК ТабличнаяЧасть
	|ГДЕ
	|	НЕ ТабличнаяЧасть.ДатаПотребности = ТабличнаяЧасть.Ссылка.ДатаПотребности
	|	И НЕ ТабличнаяЧасть.Ссылка.ДатаПотребностиУказываетсяВСтроках
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	Документ.ЗаказНаПроизводство2_2.Продукция КАК ТабличнаяЧасть
	|ГДЕ
	|	ТабличнаяЧасть.АналитикаАктивовПассивов ССЫЛКА Перечисление.УдалитьТипыНалогов";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказНаПроизводство2_2 КАК ДанныеДокумента
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если Не ДокументОбъект.ДатаПотребностиУказываетсяВСтроках Тогда
					ПроизводствоСервер.ЗаполнитьРеквизитВКоллекции(ДокументОбъект.Продукция, "ДатаПотребности", ДокументОбъект.ДатаПотребности);
				КонецЕсли;
