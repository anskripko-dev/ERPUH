...
Процедура РассчитатьСотрудниковНаСервере(Знач Сотрудники, ОписаниеТаблицы, СохранятьИсправления = Ложь, ВыводитьСообщения = Ложь)
	...
	ОбновитьДанныеФормыПослеРасчета(Сотрудники); 
...
Функция ФизическиеЛицаСотрудниковДокумента(СотрудникиФизическиеЛица)
	...
	Документ = РеквизитФормыВЗначение("Объект");
    Возврат Документ.ФизическиеЛицаСотрудниковДокумента(СотрудникиФизическиеЛица);
...
&НаСервере
Процедура ОбновитьДанныеФормыПослеРасчета(Сотрудники)
	
	Для Каждого СтрокаСотрудника Из Объект.Начисления Цикл
		ФизическоеЛицо = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаСотрудника.Сотрудник, "ФизическоеЛицо");
		Если ФизическоеЛицо <> Неопределено Тогда
