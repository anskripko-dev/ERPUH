...
Процедура РедактироватьОперацию(НоваяОперация)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ПериодОтчета) Тогда
		...
		Сообщить(Нстр("ru = 'Не заполнено поле ""ПериодОтчета"".'"),СтатусСообщения.Внимание);
		Возврат;
		
	ИначеЕсли НЕ ЗначениеЗаполнено(Объект.Сценарий) Тогда
		
		Сообщить(Нстр("ru = 'Не заполнено поле ""Сценарий"".'"),СтатусСообщения.Внимание);
		Возврат;
		
	ИначеЕсли НЕ ЗначениеЗаполнено(Объект.ОсновнаяВалюта) Тогда
		
		Сообщить(Нстр("ru = 'Не заполнено поле ""Валюта"".'"),СтатусСообщения.Внимание);
		Возврат;
			
	ИначеЕсли НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		
		Сообщить(Нстр("ru = 'Не заполнено поле ""Организация"".'"),СтатусСообщения.Внимание);
		Возврат;
