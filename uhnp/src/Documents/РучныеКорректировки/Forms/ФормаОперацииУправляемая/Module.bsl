...
Процедура УстановитьНадписьСуммаБаланса()
	...
	Если Сбалансированная Тогда
		...
		СтрокаШаблона = Нстр("ru = 'Сумма активных показателей: %1, сумма пассивных показателей: %2. Баланс операции: %3'");
		
		ТекстЗаголовка = "";
		Если Не ПустаяСтрока(СтрокаШаблона) тогда			
			ТекстЗаголовка = СтрШаблон(СтрокаШаблона, СтруктураСумм.Дебет, СтруктураСумм.Кредит, СтруктураСумм.Значение);
		КонецЕсли;
	
		Элементы.НадписьСуммаБаланса.Заголовок = ТекстЗаголовка; 
		
	Иначе
		
		СтрокаШаблона = Нстр("ru = 'Сумма операции: %1 %2'");
		
		ТекстЗаголовка = "";
		Если Не ПустаяСтрока(СтрокаШаблона) тогда			
			ТекстЗаголовка = СтрШаблон(СтрокаШаблона, Обработки.КорректировкиЗначенийПоказателей.ПолучитьСуммуБаланса(Новый Структура("Сбалансированная", Сбалансированная), ТекМассивЗаписей).Значение, Параметры.Валюта);
		КонецЕсли;
...
Функция СформироватьПредвТаблицуРедактируемыхЗаписей(РабочийДокумент=Неопределено)
	...
	Для Каждого Запись Из ЗаписиРучные Цикл
		...
		Если НЕ ЗначениеЗаполнено(Запись.Показатель) Тогда
			...
			СтрокаШаблона = Нстр("ru = 'В строке %1 таблицы ручных корректировок не указан показатель.'");
			
			Если Не ПустаяСтрока(СтрокаШаблона) тогда			
				Сообщить(СтрШаблон(СтрокаШаблона, (ЗаписиРучные.Индекс(Запись) + 1)), СтатусСообщения.Внимание);
			КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(Запись.ЭкземплярОтчета) Тогда
			...
			ТекКомментарий = Нстр("ru = 'Отчет создан при формировании ручной корректировки.'");
			...
			Если НовыйЭкземплярОтчета=Неопределено Тогда
				...
				СтрокаШаблона = Нстр("ru = 'В строке %1 таблицы ручных корректировок не указан экземпляр отчета.'");
				
				Если Не ПустаяСтрока(СтрокаШаблона) тогда			
					Сообщить(СтрШаблон(СтрокаШаблона, ЗаписиРучные.Индекс(Запись)), СтатусСообщения.Внимание);
				КонецЕсли;
...
Процедура СбалансированнаяПриИзмененииЗавершение(РезультатВопроса = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
    ...
    Если Сбалансированная Тогда
        ...
        ПоказатьВопрос(Оповещение, Нстр("ru = 'Строки операции с показателями, для которых не установлен вид, будут удалены.
...
Процедура ОбновитьДанныеПоВалюте(ИдентификаторСтроки,ИмяПоля="")
	...
	Если НЕ ЗначениеЗаполнено(ДанныеСтроки.ВидКурса) Тогда
		...
		ПоказатьПредупреждение(, Нстр("ru = 'Не задан вид курса для показателя '") + ДанныеСтроки.Показатель);
		Возврат;
