...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	СтрокаШаблона = Нстр("ru = 'Документ ""%1"" не может быть проведен:'");
	...
	Если НЕ ЗначениеЗаполнено(Сценарий) Тогда		
		ОбщегоНазначенияУХ.СообщитьОбОшибке(Нстр("ru = ' - не заполнен реквизит ""Сценарий""!'"), Отказ, ШапкаОшибки); 
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПериодОтчета) Тогда
		ОбщегоНазначенияУХ.СообщитьОбОшибке(Нстр("ru = ' - не заполнен реквизит ""ПериодОтчета""!'"), Отказ, ШапкаОшибки); 
	КонецЕсли;
...
Функция ПроверитьОперацию(Операция, ЗаписиОперации,Отказ=Ложь) Экспорт 
	...
	Если Операция.Сбалансированная Тогда
		...
		Если Обработки.КорректировкиЗначенийПоказателей.ПолучитьСуммуБаланса(Операция, МассивСумм).Значение <> 0 Тогда
			...
			СтрокаШаблона = Нстр("ru = 'Операция %1: для сбалансированных операций сумма по всем записям операции должна быть равна нулю!
			|'");
			
			Если Не ПустаяСтрока(СтрокаШаблона) тогда				
				ТекстОшибки = ТекстОшибки + СтрШаблон(СтрокаШаблона, Операция.НомерОперации);
			КонецЕсли;
	...
	Для Каждого Запись Из ЗаписиОперации Цикл
		Если НЕ ЗначениеЗаполнено(Запись.Организация) Тогда
			...
			СтрокаШаблона = Нстр("ru = 'Операция %1: в строке %2 не указана организационная единица!
			|'");
			
			Если Не ПустаяСтрока(СтрокаШаблона) тогда				
				ТекстОшибки = ТекстОшибки + СтрШаблон(СтрокаШаблона, Операция.НомерОперации, (ЗаписиОперации.Индекс(Запись) + 1));
			КонецЕсли;
		
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(Запись.ЭкземплярОтчета) Тогда
			
			СтрокаШаблона = Нстр("ru = 'Операция %1: в строке %2 не указан экземпляр отчета
			|'");
		...
		Если  ЗначениеЗаполнено(Запись.ЭкземплярОтчета) Тогда
			...
			СтрокаШаблона = Нстр("ru = 'Операция %1: в строке %2
			|указан %3, который в настоящий момент редактируется.
			|Запрещено вводить ручные корректировки
			|экземпляра отчета в случае, когда он редактируется в основной форме.
			|'");
			
			Если Не ПустаяСтрока(СтрокаШаблона) тогда				
				ТекстБлокировки = СтрШаблон(СтрокаШаблона, Операция.НомерОперации, (ЗаписиОперации.Индекс(Запись) + 1), Запись.ЭкземплярОтчета);
			КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(Запись.Показатель) Тогда
			...
			СтрокаШаблона = Нстр("ru = 'Операция %1: в строке %2 не выбран показатель!
			|'");
		...
		Если ЗаписиОперации.НайтиСтроки(СтруктураОтбора).Количество() > 1 И МассивДублей.Найти(Запись)=Неопределено Тогда
			...
			СтрокаШаблона = Нстр("ru = 'Операция %1 содержит корректировки с одинаковыми измерениями:
			|'");
...
Функция ПроверитьВозможностьУрегулированияПоАналитике(СтрокаЭлиминации) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ГруппаРаскрытияБазис) Тогда
		...
		#Если клиент Тогда
			СтрокаШаблона = Нстр("ru = '%1: не указана группа аналитик для показателя %2'");
			
			Если Не ПустаяСтрока(СтрокаШаблона) тогда			
				Сообщить(СтрШаблон(СтрокаШаблона, СокрЛП(Ссылка), СокрЛП(СтрокаЭлиминации.ПоказательБазис)));
			КонецЕсли;
		#КонецЕсли
	...
	Если НЕ ЗначениеЗаполнено(ГруппаРаскрытияСравнение) Тогда
		...
		#Если клиент Тогда
			...
			Если Не ПустаяСтрока(СтрокаШаблона) тогда			
				Сообщить(СтрШаблон(СтрокаШаблона, СокрЛП(Ссылка), СокрЛП(СтрокаЭлиминации.ПоказательСравнение)));
			КонецЕсли;
		#КонецЕсли
		Возврат Ложь;
...
Функция РассчитатьРучнуюОперацию(СтрОперация)
	...
	Для Каждого Запись ИЗ ЗаписиОперации Цикл
		...
		Если НЕ ЗначениеЗаполнено(Запись.Показатель) Тогда
			#Если Клиент Тогда
				СтрокаШаблона = Нстр("ru = '%1 некорректно указаны показатели отчетов. Расчет не выполнен.'");
				
				Если Не ПустаяСтрока(СтрокаШаблона) тогда			
					Сообщить(СтрШаблон(СтрокаШаблона, СокрЛП(Ссылка)));
				КонецЕсли;
			#КонецЕсли
			Возврат Ложь;
		КонецЕсли;
		...
		Если Запись.РасчетПоФормуле И (НЕ ЗначениеЗаполнено(Запись.Процедура)) Тогда
			#Если Клиент Тогда
				СтрокаШаблона = Нстр("ru = '%1 не указана процедура расчета. Расчет не выполнен.'");
		...
		Если Не ЗначениеЗаполнено(Запись.ЭкземплярОтчета) Тогда
			...
			СтрокаШаблона = Нстр("ru = 'Отчет создан при автоматическом расчете ручной корректировки %1'");
			
			ТекКомментарий = "";
			Если Не ПустаяСтрока(СтрокаШаблона) тогда								
				ТекКомментарий = СтрШаблон(СтрокаШаблона, СокрЛП(Ссылка));
			КонецЕсли;
			...
			Если НовыйЭкземплярОтчета=Неопределено Тогда
				...
				#Если Клиент Тогда
					СтрокаШаблона = Нстр("ru = '%1 не получены экземпляры отчетов для корректировки. Расчет не выполнен.'");
	...
	Если НЕ ПустаяСтрока(РезультатПроверки) Тогда
		...
		#Если Клиент Тогда
			...
			СтрокаШаблона = Нстр("ru = '%1 ошибки при выполнении расчета: 
			|'");
...
Функция СохранитьИзмененияЭлиминации(Операция) Экспорт
	...
	Если (НЕ ЗначениеЗаполнено(СценарийДляНеурегулированныхОтклонений)) И СтруктураСверки.ЗаписиСверкаВГО.Итог("ОтклонениеПослеУрегулированияАбс")<>0 Тогда
		...
		#Если Клиент Тогда
			СтрокаШаблона = Нстр("ru = '%1: не указан сценарий для отнесения неурегулированных отклонений.'");
			
			Если Не ПустаяСтрока(СтрокаШаблона) тогда			
				Сообщить(СтрШаблон(СтрокаШаблона, СокрЛП(Ссылка)), СтатусСообщения.Важное);
			КонецЕсли;
		#КонецЕсли
	...
	Для Каждого СтрСверка ИЗ СтруктураСверки.ЗаписиСверкаВГО Цикл
		...
		Если СтрСверка.Урегулирование<>0 И (НЕ ЗначениеЗаполнено(СтрСверка.ЭкземплярОтчетаСравнение)) Тогда
			...
			ТекКомментарий = Нстр("ru = 'Отчет создан для урегулировании внутригрупповых операций.'");
			...
			Если НовыйЭкземплярОтчета=Неопределено Тогда
				...
				#Если Клиент Тогда
					...
					СтрокаШаблона = Нстр("ru = '%1: показатель (сравнение): %2, Организация: %3
					|не получен экземпляр отчета для урегулирования.'");
					
					Если Не ПустаяСтрока(СтрокаШаблона) тогда			
						Сообщить(СтрШаблон(СтрокаШаблона, СокрЛП(Ссылка), СокрЛП(СтрСверка.ПоказательСравнение), СокрЛП(СтрСверка.ОрганизацияСравнение)));
					КонецЕсли;
		...
		Если СтрСверка.ОтнесеноНаСценарийДляНеурегулированных<>0 И (НЕ ЗначениеЗаполнено(СтрСверка.ЭкземплярОтчетаЭлиминацияНеурегулированных)) Тогда
			...
			ТекКомментарий = Нстр("ru = 'Отчет создан для отнесения неурегулированных отклонений.'");
	...
	Для Каждого СтрЭлиминация ИЗ СтруктураСверки.ЗаписиЭлиминации Цикл
		...
		Если СтрЭлиминация.ЭлиминационнаяПоправка<>0 И (НЕ ЗначениеЗаполнено(СтрЭлиминация.ЭкземплярОтчетаЭлиминацияБазис)) Тогда
			...
			ТекКомментарий = Нстр("ru = 'Отчет создан для элиминационных поправок.'");
			...
			Если НовыйЭкземплярОтчета=Неопределено Тогда
				...
				#Если Клиент Тогда
					...
					СтрокаШаблона = Нстр("ru = '%1: показатель (базис): %2, Организация: %3
					|не получен экземпляр отчета для элиминации.'");
					
					Если Не ПустаяСтрока(СтрокаШаблона) тогда			
						Сообщить(СтрШаблон(СтрокаШаблона, СокрЛП(Ссылка), СокрЛП(СтрЭлиминация.ПоказательБазис), СокрЛП(СтрЭлиминация.ОрганизацияБазис)));
					КонецЕсли;
		...
		Если СтрЭлиминация.ЭлиминационнаяПоправка<>0 И (НЕ ЗначениеЗаполнено(СтрЭлиминация.ЭкземплярОтчетаЭлиминацияСравнение)) Тогда
			...
			Если НовыйЭкземплярОтчета=Неопределено Тогда
				...
				#Если Клиент Тогда
					...
					Если Не ПустаяСтрока(СтрокаШаблона) тогда			
						Сообщить(СтрШаблон(СтрокаШаблона, СокрЛП(Ссылка), СокрЛП(СтрЭлиминация.ПоказательСравнение), СокрЛП(СтрЭлиминация.ОрганизацияСравнение)));
					КонецЕсли;
				#КонецЕсли
	...
	Для Каждого СтрСверкаВГОУХ ИЗ СтруктураСверки.ЗаписиСверкаВГО Цикл
		...
		Если СтрСверкаВГОУХ.Урегулирование<>0 И (НЕ СтрСверкаВГОУХ.УрегулированоПоАналитике) Тогда
			...
			#Если Клиент Тогда
				...
				Если Не ПустаяСтрока(СтрокаШаблона) тогда			
					Сообщить(СтрШаблон(СтрокаШаблона, СокрЛП(Ссылка), СокрЛП(СтрСверкаВГОУХ.ПоказательСравнение), СокрЛП(СтрСверкаВГОУХ.ОрганизацияСравнение)));
				КонецЕсли;
			#КонецЕсли
	...
	Для Каждого СтрЭлиминация ИЗ СтруктураСверки.ЗаписиЭлиминации Цикл
		...
		Если НЕ СтрЭлиминация.ТабЭлиминации.Количество()=2 Тогда
			...
			#Если Клиент Тогда
				СтрокаШаблона = Нстр("ru = '%1: строка %2: не сформированы элиминационные таблицы.'");
				
				Если Не ПустаяСтрока(СтрокаШаблона) тогда			
					Сообщить(СтрШаблон(СтрокаШаблона, СокрЛП(Ссылка), СтрЭлиминация.НомерСтроки), СтатусСообщения.Важное);
				КонецЕсли;
			#КонецЕсли
		...
		Если ТабЭлиминацииБазис.Количество()=0 Тогда
			...
			#Если Клиент Тогда
				СтрокаШаблона = Нстр("ru = '%1: строка %2: не заполнена элиминационная таблица для базисного показателя.'");
		...
		Если ТабЭлиминацииСравнение.Количество()=0 Тогда
			...
			#Если Клиент Тогда
				...
				СтрокаШаблона = Нстр("ru = '%1: строка %2: не заполнена элиминационная таблица для сравниваемого показателя.'");
