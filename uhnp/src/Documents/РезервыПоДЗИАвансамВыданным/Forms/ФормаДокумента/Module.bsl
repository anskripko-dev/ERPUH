...
Процедура ОбновитьВидимостьПодробноПоСтроке(ТекущаяСтрока = Неопределено)
	...
	Если ЭтаФорма.Элементы.ГруппаПодробно.Видимость Тогда
		...
		Договор = ОбработкаТабличныхЧастейКлиентСерверУХ.ПолучитьСубконтоПоТипу(
			ТекущаяСтрока, Тип("СправочникСсылка.ДоговорыКонтрагентов"));
		Если ЗначениеЗаполнено(Договор) Тогда
			РучнойРасчет = ОбщегоНазначенияУХ.ПолучитьЗначениеРеквизита(
								Договор, "РучнойРасчетРезерваПоЗадолженности");
		КонецЕсли;	
			
		Элементы.ПараметрыРасчета.Видимость = (РучнойРасчет = Ложь);
		Элементы.РезервыДокументРасчета.Видимость = ЗначениеЗаполнено(ТекущаяСтрока.ДокументРасчета);
...
Процедура РезервыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		Если ТекущиеДанные.КлючСвязи = ПустойУИД Тогда
			ТекущиеДанные.КлючСвязи = Новый УникальныйИдентификатор;
		КонецЕсли;
		
	КонецЕсли;
	
	ПоляФормы = МСФОКлиентСерверУХ.СубконтоПоШаблону("РезервыСубконто%1");
	МСФОКлиентСерверУХ.ПриВыбореСчета(
		ТекущиеДанные.СчетЗадолженностиМСФО, ЭтотОбъект, 
		ПоляФормы, Неопределено, Истина, Ложь);
...
Процедура РезервыКоэффициентРезерваПриИзменении(Элемент)
	...
	ОбработкаТабличныхЧастейКлиентСерверУХ.ОбработатьСтрокуТЧ_РезервыДЗ(
		Элементы.Резервы.ТекущиеДанные, СтруктураДействий);
...
Процедура РезервыСчетЗадолженностиНСБУПриИзменении(Элемент)
	...
	СтруктураДействий.Вставить("ЗаполнитьСчетаЗадолженностиПоСчетуНСБУ",
		Новый Структура("Контекст", Объект));
		
	ОбработкаТабличныхЧастейКлиентСерверУХ.ОбработатьСтрокуТЧ_РезервыДЗ(
		Элементы.Резервы.ТекущиеДанные, СтруктураДействий, Неопределено);
...
Процедура РезервыНачислениеРезерваВВалютеМСФОПриИзменении(Элемент)
	...
	СтруктураДействий = Новый Структура;	
	СтруктураДействий.Вставить("РассчитатьНачислениеРезерваМСФО",
		Новый Структура("Контекст", Объект));
...
Процедура ДвижениеРезервовНСБУВидДвиженияПриИзменении(Элемент)
	...
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ЗаполнитьСчетаПоВидуДвижений",
		Новый Структура("Контекст", Объект));
	
	ОбработкаТабличныхЧастейКлиентСерверУХ.ОбработатьСтрокуТЧ_РезервыДЗ(
		Элементы.ДвижениеРезервовНСБУ.ТекущиеДанные, СтруктураДействий);
...
Процедура ПодготовитьФормуНаСервере(Форма)
	...
	Форма.КэшируемыеЗначения.Вставить("ИменаСубконто", 		Документы.РезервыПоДЗИАвансамВыданным.ПолучитьИменаСубконто());
	Форма.КэшируемыеЗначения.Вставить("РазделыСчетов", 		Документы.РезервыПоДЗИАвансамВыданным.ПолучитьРазделыСчетов());
	Форма.КэшируемыеЗначения.Вставить("КлючевыеСубконто", 	МСФОКлиентСерверУХ.ПолучитьКлючевыеСубконтоФИ(Истина,Ложь));
	...
	Для каждого ИмяТЧ Из Форма.КэшируемыеЗначения.РазделыСчетов Цикл	
		Для каждого ИмяСчета Из ИмяТЧ.Значение Цикл
			...
			Форма.Элементы[ИмяТЧ.Ключ + ИмяСчета.Ключ].ПараметрыВыбора 
				= МСФОКлиентСерверУХ.ПолучитьПараметрыВыбораДляСчетаБД(ПланСчетовМСФО, ИмяСчета.Значение);
				
		КонецЦикла;
	КонецЦикла;
	
	ОбновитьДоступностьРезервы(Форма);
	МСФОУХ.ДобавитьОформлениеДоступностиСубконто(
		Форма, Форма.КэшируемыеЗначения.ИменаСубконто.Резервы, "Резервы");
	
	СчетаГруппыПодробно = МСФОУХ.ПолучитьСчетаГруппыПодобно(Форма, "Резервы", Истина);
	Форма.КэшируемыеЗначения.Вставить("СчетаГруппыПодробно", СчетаГруппыПодробно);
	
	Форма.Элементы.ГруппаПодробно.Видимость 
		= Форма.Элементы.РезервыПоказатьПодробноДоговора.Пометка;
...
Процедура ЗаполнитьРассчитать()
	...
	ОбновитьДоступностьРезервы(ЭтотОбъект);
...
Процедура ТолькоДоговорыСРучнымРасчетом(Команда)
	...
	Элементы.РезервыТолькоДоговорыСРучнымРасчетом.Пометка 
		= НЕ Элементы.РезервыТолькоДоговорыСРучнымРасчетом.Пометка;
...
Процедура ТолькоДоговорыСРучнымРасчетомСервер()
	...
	Запрос.УстановитьПараметр("Договор", Объект.Резервы.Выгрузить(,"Субконто2").ВыгрузитьКолонку("Субконто2"));
	...
	Пока ВыборкаДоговоры.Следующий() Цикл
		...
		СтрокиПоДоговору = Объект.Резервы.НайтиСтроки(Новый Структура("Субконто2", ВыборкаДоговоры.Договор));
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОбновитьДоступностьРезервы(Форма)
	
	Объект = Форма.Объект;
	ПоляОбъекта = МСФОКлиентСерверУХ.СубконтоПоШаблону("Субконто%1");
	
	Для каждого СтрокаТаб Из Объект.Резервы Цикл
	
		МСФОКлиентСерверУХ.УстановитьДоступностьСубконто(
			СтрокаТаб.СчетЗадолженностиМСФО, СтрокаТаб, ПоляОбъекта, Истина);
			
		СтрокаТаб.ДокументРасчетовДоступность 
			= ЗначениеЗаполнено(СтрокаТаб.Субконто3);
