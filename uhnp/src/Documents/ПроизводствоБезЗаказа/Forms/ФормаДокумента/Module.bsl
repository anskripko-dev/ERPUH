...
Процедура НаправлениеДеятельностиПриИзмененииСервер()
	...
	ДавальческаяСхемаКлиентСервер.ДобавитьДействиеЗаполнитьПризнакДоступноОформлениеОтчетаДавальцу(ЭтотОбъект, СтруктураДействий);
	//-- Устарело_Переработка24
	...
	Если ЗначениеЗаполнено(СтруктураДействий) Тогда
		...
		ОбрабатываемыеСтрокиПобочныеИзделия = Новый Массив;
		
		Для Каждого ТекСтрока Из СтрокиТЧ Цикл
			
			Если Не Объект.ВыходныеИзделия.Индекс(ТекСтрока) = -1 Тогда
				ОбрабатываемыеСтрокиВыходныеИзделия.Добавить(ТекСтрока);
			КонецЕсли;
			
			Если Не Объект.ПобочныеИзделия.Индекс(ТекСтрока) = -1 Тогда
				ОбрабатываемыеСтрокиПобочныеИзделия.Добавить(ТекСтрока);
			КонецЕсли;
			
		КонецЦикла;
		
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			ОбрабатываемыеСтрокиПобочныеИзделия,
			СтруктураДействий,
...
Процедура ТаблицаГруппаЗатратВыборНаСервере(РезультатЗакрытия, ПараметрыОповещения)
	...
	Для Каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
		...
		//++ НЕ УТКА
		Если ПараметрыОповещения.ИмяТЧ = "ПобочныеИзделия" Тогда
	...
	Если ПараметрыОповещения.ИмяТЧ = "МатериалыИРаботы" Тогда
...
Процедура НастройкаЗависимыхЭлементовФормы(Форма, СписокРеквизитов = "")
	...
	Если Инициализация
		...
		Если ЗначениеЗаполнено(Объект.Организация) Тогда
			...
			ПараметрВыбораОрганизации = Новый ПараметрВыбора("Организация", Объект.Организация);
			ПараметрыВыбораИсполнителя.Добавить(ПараметрВыбораОрганизации);
...
Процедура ЗаменитьНаАналоги(АдресВХранилище = Неопределено)
	...
	ДобавитьВСтруктуруДействияПоЦеховойКладовой(ИмяТЧ, СтруктураДействий, Истина, Истина);
...
Функция ПараметрыВыбораАналогов(ИмяТЧ, Режим)
	...
	Если Режим = 1 Тогда
		...
		ВыделенныеСтроки = Элементы[ИмяТЧ].ВыделенныеСтроки;
		Для каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
			МассивВыделенныхСтрок.Добавить(Объект[ИмяТЧ].НайтиПоИдентификатору(ИдентификаторСтроки));
		КонецЦикла;
		
		ТаблицаТоваров = Объект[ИмяТЧ].Выгрузить(МассивВыделенныхСтрок);
		
	Иначе	
		
		ТаблицаТоваров = Объект[ИмяТЧ].Выгрузить();
		
	КонецЕсли;
	
	ТаблицаТоваров.Колонки.Добавить("Обособленно", Новый ОписаниеТипов("Булево"));
	Для каждого СтрокаТоваров Из ТаблицаТоваров Цикл
		СтрокаТоваров.Обособленно = ЗначениеЗаполнено(СтрокаТоваров.Назначение);
	КонецЦикла;
	...
	ДанныеФормы[ИмяОбъекта].Вставить(ИмяТЧ, ТаблицаТоваров);
	
	ДанныеТоваров = АналогиМатериалов.ПолучитьДанныеТоваров(
		ДанныеФормы, "Объект", ИмяТЧ, Режим, Ложь, СтруктураПолей);
