	...
	Иначе
		...
		Элементы.НадписьСообщенияОжидаемыеОтСФР.Заголовок = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
			НСтр("ru = ';Ожидается %1 ответ на заявление в СФР;;Ожидается %1 ответа на заявления в СФР;Ожидается %1 ответов на заявления в СФР;';
				|en = ';Expecting %1 response to the application to the Social Insurance Fund of Russia;;Expecting %1 responses to the applications to the Social Insurance Fund of Russia;Expecting %1 responses to the applications to the Social Insurance Fund of Russia;'"),
		...
		Иначе
			ЗаголовокКнопки = НСтр("ru = 'Проверить наличие ответов СФР';
									|en = 'Check for Social Insurance Fund responses'");
		КонецЕсли;
