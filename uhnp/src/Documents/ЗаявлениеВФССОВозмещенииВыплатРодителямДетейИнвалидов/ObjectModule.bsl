...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ТипСообщенияЧислом = 12010 Тогда
		...
		Если ВсегоПособий >= 1000000000 Тогда
			Текст = СтрШаблон(НСтр("ru = 'Сумма пособий (%1) превышает предельно допустимое значение (%2)';
									|en = 'The amount of allowances (%1) exceeds the maximum value (%2)'"), ВсегоПособий, 1000000000);
		...
		Если ЗначениеЗаполнено(ТелефонСоставителя) Тогда
			...
			Если СтрДлина(ПредставлениеТелефона) < 5 Тогда
				Текст = НСтр("ru = 'Номер телефона (%1) должен содержать не менее %2 символов.';
							|en = 'The phone number (%1) must contain  at least %2 characters.'");
		...
		Если ЗначениеЗаполнено(АдресЭлектроннойПочтыОрганизации) И СтрДлина(АдресЭлектроннойПочтыОрганизации) < 4 Тогда
			Текст = НСтр("ru = 'Адрес электронной почты (%1) должен содержать не менее %2 символов.';
						|en = 'The email address (%1) must contain at least %2 characters.'");
	...
	ИначеЕсли ТипСообщенияЧислом = 120 Тогда
		Если СтрДлина(АдресЭлектроннойПочтыОрганизации) > 40 Тогда
			Текст = НСтр("ru = 'Адрес электронной почты (%1) должен содержать не более %2 символов.';
						|en = 'The email address (%1) must contain no more than %2 characters.'");
		...
		Если КадровыйУчетКлиентСервер.СНИЛСЗаполнен(СтрокаТаблицы.РебенокСНИЛС) Тогда
			Если Не РегламентированныеДанныеКлиентСервер.СтраховойНомерПФРСоответствуетТребованиям(СтрокаТаблицы.РебенокСНИЛС, Текст) Тогда
				Текст = СтрШаблон(НСтр("ru = 'СНИЛС ребенка указан не верно: %1';
										|en = 'Child''s SNILS is incorrect: %1'"), НРег(Текст));
		...
		Если КадровыйУчетКлиентСервер.СНИЛСЗаполнен(СтрокаТаблицы.СотрудникСНИЛС) Тогда
			Если Не РегламентированныеДанныеКлиентСервер.СтраховойНомерПФРСоответствуетТребованиям(СтрокаТаблицы.СотрудникСНИЛС, Текст) Тогда
				Текст = СтрШаблон(НСтр("ru = 'СНИЛС сотрудника указан не верно: %1';
										|en = 'Employee''s SNILS is incorrect: %1'"), НРег(Текст));
	...
	Если ЭтоФормыПосле2024 И ОтправлятьЧерезСЭДО Тогда
		...
		Если ВнешнееСовместительство <> ФлажкиВидовЗанятости.ВнешнееСовместительство Тогда
			Текст = НСтр("ru = 'Флажок ""Внешнее совместительство"" в шапке не соответствует видам занятости сотрудников в таблице.';
						|en = 'The ""External secondary employment"" checkbox in the header does not match the employment kinds of employees in the table.'");
		...
		ИначеЕсли ОсновноеМестоРаботы <> ФлажкиВидовЗанятости.ОсновноеМестоРаботы Тогда
			Текст = НСтр("ru = 'Флажок ""Основное место работы"" в шапке не соответствует видам занятости сотрудников в таблице.';
						|en = 'The ""Primary employment"" checkbox in the header does not match the employment kinds of employees in the table.'");
	...
	ИначеЕсли Значение < Минимум Тогда
		Текст = СтрШаблон(
			НСтр("ru = 'В строке %1 в поле ""%2"" указано значение %3, что меньше минимального %4.';
				|en = 'In line %1 of the ""%2"" field, the %3 value is specified, which is less than the minimum one (%4).'"),
	...
	ИначеЕсли Значение > Максимум Тогда
		Текст = СтрШаблон(
			НСтр("ru = 'В строке %1 в поле ""%2"" указано значение %3, что больше максимального %4.';
				|en = 'In line %1 of the ""%2"" field, the %3 value is specified, which is greater than the maximum one (%4).'"),
			СтрокаТаблицы.НомерСтроки,
			ПроверкиБЗК.ПредставлениеРеквизитаТаблицыОбъекта(ЭтотОбъект, Неопределено, ИмяТаблицы, ИмяРеквизита),
			Значение,
			Максимум);
