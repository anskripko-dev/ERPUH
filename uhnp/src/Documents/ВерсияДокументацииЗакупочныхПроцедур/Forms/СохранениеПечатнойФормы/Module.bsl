...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьПоддерживаемыеФорматыСохранения(ВыбранныеФорматыСохранения);
	ВыбранныеФорматыСохранения.Удалить(ВыбранныеФорматыСохранения.НайтиПоЗначению("PDF"));
	ДоговорКонтрагента = Параметры.ОбъектПечати;
	МакетЗакупок = Параметры.МакетЗакупок;
...
Процедура Сохранить(Команда)
	...
	РезультатВыбора.Вставить("МакетЗакупок", МакетЗакупок);
...
&НаКлиенте
Процедура ВыбранныеФорматыСохраненияПометкаПриИзменении(Элемент)
	// Вставить содержимое обработчика. 
	Если Элементы.ВыбранныеФорматыСохранения.ТекущиеДанные = Неопределено Тогда
		Возврат;
	Иначе
		ТекДанные = Элементы.ВыбранныеФорматыСохранения.ТекущиеДанные;
	КонецЕсли;
	
	Для Каждого Стр ИЗ ВыбранныеФорматыСохранения Цикл
		Если Стр.Значение <> ТекДанные.Значение Тогда
			Стр.Пометка = Ложь;
		КонецЕсли;	
	КонецЦикла;
