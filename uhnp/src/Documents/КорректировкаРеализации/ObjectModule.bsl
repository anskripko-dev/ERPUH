	...
	Если ЗначениеЗаполнено(ДокументОснование) Тогда
		...
		Если Дата < ДатаОснования Тогда
			ТекстОшибки = НСтр("ru = 'Дата документа не должна быть меньше даты документа основания.';
								|en = 'The document date must be later than the base document date.'");
...
Процедура ЗаполнитьДопКолонкиВидовЗапасов(ВидыЗапасов, МассивВариантовОтражения, ЗаполнятьВидЗапасовОтгрузки = Ложь, ЗаполнятьСебестоимость = Ложь) Экспорт
	...
	ЗаполнятьКорВидЗапасов =
		...
		ИЛИ Не МассивВариантовОтражения.Найти(Перечисления.ВариантыОтраженияКорректировокРеализаций.УменьшитьРеализацию) = Неопределено);
		
	Если ЗаполнятьКорВидЗапасов Тогда
		
		НоменклатураАналитикУчетаНоменклатуры =
			ОбщегоНазначения.ЗначениеРеквизитаОбъектов(
				ОбщегоНазначенияКлиентСервер.СвернутьМассив(ВидыЗапасов.ВыгрузитьКолонку("АналитикаУчетаНоменклатуры")),
				"Номенклатура");
		
		ИсточникиГФУ = Новый Массив;
		Для Каждого КлючИЗначение Из НоменклатураАналитикУчетаНоменклатуры Цикл
			ИсточникиГФУ.Добавить(КлючИЗначение.Значение);
		КонецЦикла;
		ИсточникиГФУ = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ИсточникиГФУ);
		
		ГФУАналитикУчетаНоменклатуры =
			ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ИсточникиГФУ, "ГруппаФинансовогоУчета");
		
		КорВидыЗапасовГФУ = Новый Соответствие;
		...
		РеквизитыДокумента.Вставить("ГруппаФинансовогоУчета",     Справочники.ГруппыФинансовогоУчетаНоменклатуры.ПустаяСсылка());
	...
	Для Каждого СтрокаТоваров Из ТаблицаТовары Цикл
		...
		Для Каждого СтрокаЗапасов Из ВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			...
			Если ЗаполнятьКорВидЗапасов Тогда
				...
				НоменклатураСтроки = НоменклатураАналитикУчетаНоменклатуры.Получить(СтрокаЗапасов.АналитикаУчетаНоменклатуры);
				ГФУСтроки          = ГФУАналитикУчетаНоменклатуры.Получить(НоменклатураСтроки);
				КорВидЗапасов      = КорВидыЗапасовГФУ.Получить(ГФУСтроки);
				
				Если Не ЗначениеЗаполнено(КорВидЗапасов) Тогда
					
					РеквизитыДокумента.ГруппаФинансовогоУчета = ГФУСтроки;
					КорВидЗапасов =
					...
					КорВидыЗапасовГФУ.Вставить(ГФУСтроки, КорВидЗапасов);
