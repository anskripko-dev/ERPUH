...
Процедура ЗарегистрироватьУчетныеМеханизмы(МеханизмыДокумента) Экспорт
	...
	МеханизмыДокумента.Добавить("ФинансовыеИнструментыУХ"); 
	#КонецОбласти
...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		#Область УХ_Встраивание
		ТекстЗапросаТаблицаРасчетыСКонтрагентамиФакт(Запрос, ТекстыЗапроса, Регистры); 
		#КонецОбласти
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Обновление документов ""Начисления по кредитам и депозитам"":';
								|en = 'Update the ""Credit and deposit interest charge"" documents:'"));
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Не удалось обработать документы ""Начисления по кредитам и депозитам"" по обработчику:';
								|en = 'Cannot process the ""Credit and deposit interest charge"" documents by the handler:'"));
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Рекомендация = НСтр("ru = 'Перезаполните таблицу ""Начисления"" вручную.';
									|en = 'Refill the ""Accruals"" table manually.'");
...
Функция ТекстЗапросаТаблицаРасчетыСКонтрагентамиФакт(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	НачисленияКредитовИДепозитовНачисления.Ссылка.Организация КАК Организация,
	|	НачисленияКредитовИДепозитовНачисления.Контрагент КАК Контрагент,
	|	НачисленияКредитовИДепозитовНачисления.Договор КАК ДоговорКонтрагента,
	|	НачисленияКредитовИДепозитовНачисления.Дата КАК Период,
	|	НачисленияКредитовИДепозитовНачисления.СуммаВзаиморасчетов КАК Сумма,
	|	НачисленияКредитовИДепозитовНачисления.Договор КАК ОбъектРасчетов,
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ЭлементыСтруктурыЗадолженности.Проценты)
	|		КОГДА НачисленияКредитовИДепозитовНачисления.ТипСуммыГрафика = ЗНАЧЕНИЕ(Перечисление.ТипыСуммГрафикаКредитовИДепозитов.Комиссия)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ЭлементыСтруктурыЗадолженности.Комиссии)
	|	КОНЕЦ КАК ЭлементСтруктурыЗадолженности,
	|	НачисленияКредитовИДепозитовНачисления.ВалютаВзаиморасчетов КАК Валюта,
	|	НачисленияКредитовИДепозитовНачисления.Договор.ВидДоговораУХ КАК ВидДоговораУХ,
	|	ВЫБОР
	|		КОГДА НачисленияКредитовИДепозитовНачисления.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.НачисленияПоДепозитам)
	|				ИЛИ НачисленияКредитовИДепозитовНачисления.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.НачисленияПоЗаймамВыданным)
	|			ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		ИНАЧЕ ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|	КОНЕЦ КАК ВидДвижения,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыБюджетов.БюджетДоходовИРасходов) КАК ВидБюджета
	|ПОМЕСТИТЬ ВТ_НачисленияКредитовИДепозитовНачисления
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_НачисленияКредитовИДепозитовНачисления.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ЕСТЬNULL(ВерсияСоглашенияДепозитПараметрыОпераций.ОперацияГрафика, ВерсияСоглашенияКредитПараметрыОпераций.ОперацияГрафика) КАК ОперацияГрафика,
	|	ЕСТЬNULL(ВерсияСоглашенияДепозитПараметрыОпераций.СтатьяБюджета, ВерсияСоглашенияКредитПараметрыОпераций.СтатьяБюджета) КАК СтатьяБюджета,
	|	ЕСТЬNULL(ВерсияСоглашенияДепозитПараметрыОпераций.Ссылка.ВидДоговораУХ, ВерсияСоглашенияКредитПараметрыОпераций.Ссылка.ВидДоговораУХ) КАК ВидДоговораУХ,
	|	ВТ_НачисленияКредитовИДепозитовНачисления.ЭлементСтруктурыЗадолженности КАК ЭлементСтруктурыЗадолженности
	|ПОМЕСТИТЬ ВТ_ПараметрыОпераций
	|ИЗ
	|	ВТ_НачисленияКредитовИДепозитовНачисления КАК ВТ_НачисленияКредитовИДепозитовНачисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВерсияСоглашенияКредит.ПараметрыОпераций КАК ВерсияСоглашенияКредитПараметрыОпераций
	|		ПО (ВЫБОР
	|				КОГДА ВТ_НачисленияКредитовИДепозитовНачисления.ДоговорКонтрагента.ВерсияСоглашения ССЫЛКА Документ.ВерсияСоглашенияКредит
	|					ТОГДА ВЫРАЗИТЬ(ВТ_НачисленияКредитовИДепозитовНачисления.ДоговорКонтрагента.ВерсияСоглашения КАК Документ.ВерсияСоглашенияКредит)
	|			КОНЕЦ = ВерсияСоглашенияКредитПараметрыОпераций.Ссылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВерсияСоглашенияДепозит.ПараметрыОпераций КАК ВерсияСоглашенияДепозитПараметрыОпераций
	|		ПО (ВЫБОР
	|				КОГДА ВТ_НачисленияКредитовИДепозитовНачисления.ДоговорКонтрагента.ВерсияСоглашения ССЫЛКА Документ.ВерсияСоглашенияДепозит
	|					ТОГДА ВЫРАЗИТЬ(ВТ_НачисленияКредитовИДепозитовНачисления.ДоговорКонтрагента.ВерсияСоглашения КАК Документ.ВерсияСоглашенияДепозит)
	|			КОНЕЦ = ВерсияСоглашенияДепозитПараметрыОпераций.Ссылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ПараметрыОпераций.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	МАКСИМУМ(ВТ_ПараметрыОпераций.СтатьяБюджета) КАК СтатьяБюджета
	|ПОМЕСТИТЬ ВТ_СтатьиБюджетов
	|ИЗ
	|	ВТ_ПараметрыОпераций КАК ВТ_ПараметрыОпераций
	|ГДЕ
	|	ВЫБОР
	|			КОГДА ВТ_ПараметрыОпераций.ЭлементСтруктурыЗадолженности = ЗНАЧЕНИЕ(Перечисление.ЭлементыСтруктурыЗадолженности.Проценты)
	|				ТОГДА ВЫБОР
	|						КОГДА ВТ_ПараметрыОпераций.ВидДоговораУХ = ЗНАЧЕНИЕ(Справочник.ВидыДоговоровКонтрагентовУХ.Кредит)
	|							ТОГДА ВТ_ПараметрыОпераций.ОперацияГрафика = ЗНАЧЕНИЕ(Справочник.ОперацииГрафиковДоговоров.КредитПолученный_ПроцентыНачислено)
	|						КОГДА ВТ_ПараметрыОпераций.ВидДоговораУХ = ЗНАЧЕНИЕ(Справочник.ВидыДоговоровКонтрагентовУХ.ЗаемПолученный)
	|							ТОГДА ВТ_ПараметрыОпераций.ОперацияГрафика = ЗНАЧЕНИЕ(Справочник.ОперацииГрафиковДоговоров.ЗаемПолученный_ПроцентыНачислено)
	|						КОГДА ВТ_ПараметрыОпераций.ВидДоговораУХ = ЗНАЧЕНИЕ(Справочник.ВидыДоговоровКонтрагентовУХ.ЗаемВыданный)
	|							ТОГДА ВТ_ПараметрыОпераций.ОперацияГрафика = ЗНАЧЕНИЕ(Справочник.ОперацииГрафиковДоговоров.ЗаемВыданный_ПроцентыНачислено)
	|						КОГДА ВТ_ПараметрыОпераций.ВидДоговораУХ = ЗНАЧЕНИЕ(Справочник.ВидыДоговоровКонтрагентовУХ.Депозит)
	|							ТОГДА ВТ_ПараметрыОпераций.ОперацияГрафика = ЗНАЧЕНИЕ(Справочник.ОперацииГрафиковДоговоров.Депозит_ПроцентыНачислено)
	|					КОНЕЦ
	|			КОГДА ВТ_ПараметрыОпераций.ЭлементСтруктурыЗадолженности = ЗНАЧЕНИЕ(Перечисление.ЭлементыСтруктурыЗадолженности.Комиссии)
	|				ТОГДА ВЫБОР
	|						КОГДА ВТ_ПараметрыОпераций.ВидДоговораУХ = ЗНАЧЕНИЕ(Справочник.ВидыДоговоровКонтрагентовУХ.Кредит)
	|							ТОГДА ВТ_ПараметрыОпераций.ОперацияГрафика = ЗНАЧЕНИЕ(Справочник.ОперацииГрафиковДоговоров.КредитПолученный_КомиссииНачислено)
	|						КОГДА ВТ_ПараметрыОпераций.ВидДоговораУХ = ЗНАЧЕНИЕ(Справочник.ВидыДоговоровКонтрагентовУХ.ЗаемПолученный)
	|							ТОГДА ВТ_ПараметрыОпераций.ОперацияГрафика = ЗНАЧЕНИЕ(Справочник.ОперацииГрафиковДоговоров.ЗаемПолученный_КомиссииНачислено)
	|						КОГДА ВТ_ПараметрыОпераций.ВидДоговораУХ = ЗНАЧЕНИЕ(Справочник.ВидыДоговоровКонтрагентовУХ.ЗаемВыданный)
	|							ТОГДА ВТ_ПараметрыОпераций.ОперацияГрафика = ЗНАЧЕНИЕ(Справочник.ОперацииГрафиковДоговоров.ЗаемВыданный_КомиссииНачислено)
	|					КОНЕЦ
	|		КОНЕЦ
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ПараметрыОпераций.ДоговорКонтрагента
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_НачисленияКредитовИДепозитовНачисления.Организация КАК Организация,
	|	ВТ_НачисленияКредитовИДепозитовНачисления.Контрагент КАК Контрагент,
	|	ВТ_НачисленияКредитовИДепозитовНачисления.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ВТ_НачисленияКредитовИДепозитовНачисления.Период КАК Период,
	|	ВТ_НачисленияКредитовИДепозитовНачисления.Сумма КАК Сумма,
	|	ВТ_НачисленияКредитовИДепозитовНачисления.ОбъектРасчетов КАК ОбъектРасчетов,
	|	ВТ_НачисленияКредитовИДепозитовНачисления.ПриходРасход КАК ПриходРасход,
	|	ВТ_НачисленияКредитовИДепозитовНачисления.ЭлементСтруктурыЗадолженности КАК ЭлементСтруктурыЗадолженности,
	|	ВТ_НачисленияКредитовИДепозитовНачисления.Валюта КАК Валюта,
	|	ВТ_НачисленияКредитовИДепозитовНачисления.ВидДоговораУХ КАК ВидДоговораУХ,
	|	ВТ_НачисленияКредитовИДепозитовНачисления.ВидДвижения КАК ВидДвижения,
	|	ВТ_НачисленияКредитовИДепозитовНачисления.ВидБюджета КАК ВидБюджета,
	|	ВТ_СтатьиБюджетов.СтатьяБюджета КАК СтатьяБюджета
	|ИЗ
	|	ВТ_НачисленияКредитовИДепозитовНачисления КАК ВТ_НачисленияКредитовИДепозитовНачисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СтатьиБюджетов КАК ВТ_СтатьиБюджетов
	|		ПО ВТ_НачисленияКредитовИДепозитовНачисления.ДоговорКонтрагента = ВТ_СтатьиБюджетов.ДоговорКонтрагента";
