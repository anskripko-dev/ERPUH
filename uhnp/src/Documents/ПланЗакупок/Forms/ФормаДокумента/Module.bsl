...
Процедура ПриОткрытии(Отказ)
	...
	КонецЕсли;   
	ВстраиваниеУХПланЗакупокКлиент.СкрытьПанельСогласования(ЭтотОбъект.Элементы); 
	#КонецОбласти   
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ВстраиваниеУХПланЗакупок.ПередЗаписьюНаСервереНачало(ЭтотОбъект, Отказ, ТекущийОбъект, ПараметрыЗаписи); 
	Если Не Объект.ВидБюджетаПланирования.Пустая() И Объект.ФормированиеПотребностейПланомЗакупок И Отказ Тогда //Не нужны лишние уведомления при копировании ТЧ
		Возврат;
	КонецЕсли;
	#КонецОбласти
	...
	#Область УХ_Внедрение
	ВстраиваниеУХПланЗакупок.ПередЗаписьюНаСервере(ЭтотОбъект, Отказ, ТекущийОбъект, ПараметрыЗаписи); 
	#КонецОбласти
	// ИнтеграцияС1СДокументооборотом
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ИзмениласьТаблицаНоменклатуры" Тогда
		ИзменилосьРаспределениеПоНоменклатуре(Элементы.ТоварыПоПериодам.ТекущиеДанные.НомерСтроки-1,Параметр.АдресРезультата);
	#КонецОбласти	
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.КроссТаблица Тогда
		...
		#Область УХ_Внедрение
		Если ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) Тогда
			//
			СтруктураДействий.Удалить("ПроверкаКоличестваПоПериодам");
			СтруктураДействий.Удалить("ЗаполнятьНоменклатуру");
			СтруктураДействий.Удалить("ЗаполнятьХарактеристикуВТЧ");
			СтруктураДействий.Вставить("ЗаполнятьТоварнуюПозицию");
			
			СтруктураДействий.ПоляГруппировки = "ТоварнаяПозиция, Характеристика, Назначение, Упаковка, Склад, Партнер, Соглашение, ВидЦеныПоставщика";   
			СтруктураДействий.ПоляГруппировки = СтруктураДействий.ПоляГруппировки + ", СтатьяБюджета, "+ АналитикиСтатейБюджетовУХКлиентСервер.РеквизитыАналитикИзШаблона("Аналитика%1");
	...
	#Область УХ_Внедрение
	ВстраиваниеУХПланЗакупок.КонтрольСценарияПланаЗакупок(ЭтотОбъект, Отказ);   
	ВстраиваниеУХПланЗакупок.КонтрольВидаПланаЗакупок(ЭтотОбъект, Отказ);
	#КонецОбласти 
...
Процедура УХ_РаспределитьТоварнуюКатегорию(Команда)
	Если Элементы.ТоварыПоПериодам.ТекущиеДанные <> Неопределено 
		И ЗначениеЗаполнено(Элементы.ТоварыПоПериодам.ТекущиеДанные.ТоварнаяКатегория)
		И ТипЗнч(Элементы.ТоварыПоПериодам.ТекущиеДанные.ТоварнаяКатегория) =  Тип("СправочникСсылка.ТоварныеКатегории")
		Тогда
	ПараметрыФормы = УХ_РаспределитьТоварнуюКатегориюНаСервере(Элементы.ТоварыПоПериодам.ТекущиеДанные.НомерСтроки);
	...
	#Область УХ_Внедрение
	ИначеЕсли Элементы.ТоварыПоПериодам.ТекущиеДанные <> Неопределено 
		И ЗначениеЗаполнено(Элементы.ТоварыПоПериодам.ТекущиеДанные.ТоварнаяКатегория)
		И ТипЗнч(Элементы.ТоварыПоПериодам.ТекущиеДанные.ТоварнаяКатегория) =  Тип("СправочникСсылка.КатегорииЗакупок")
		Тогда
	ПараметрыФормы = УХ_РаспределитьТоварнуюКатегориюНаСервере(Элементы.ТоварыПоПериодам.ТекущиеДанные.НомерСтроки);
	ОткрытьФорму("Обработка.РаспределениеТоварнойКатегории.Форма.Форма",ПараметрыФормы , ЭтаФорма, ЭтаФорма.УникальныйИдентификатор,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);	
	#КонецОбласти
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Распределение доступно только по товарной категории'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ДатаПриИзменении(Элемент)
	...
	КонецЕсли;   
	#Область УХ_Внедрение
	ИспользуемыеВидыБюджетовЗакупок();     
	#КонецОбласти
...
Процедура ТоварыПриИзменении(Элемент)
	...
	Если Объект.ФормированиеПотребностейПланомЗакупок Тогда    
		ПодключитьОбработчикОжидания("ПриОжиданииПерезаполнитьПланыОплаты", 1, Истина);
	КонецЕсли;
	#КонецОбласти
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	//Унификаты         
	Если ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) Тогда
		УнификатыПриИзмененииНоменклатурыСервер("Товары", Элементы.Товары.ТекущиеДанные.ПолучитьИдентификатор(), КэшированныеЗначения);    
	КонецЕсли;
	#КонецОбласти
	...
	#Область УХ_Внедрение   
	ЕстьРеквизит = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущаяСтрока, "СтатьяБюджета");
	Если ЕстьРеквизит Тогда
		СтараяСтатья = ТекущаяСтрока.СтатьяБюджета; 
	КонецЕсли;
	ТоварыНоменклатураПриИзмененииСервер(ТекущаяСтрока.ПолучитьИдентификатор(), "Товары", КэшированныеЗначения);  
	Если ЕстьРеквизит И СтараяСтатья <> ТекущаяСтрока.СтатьяБюджета Тогда   
		Если Элемент = Элементы.ТоварыПоПериодамНоменклатура Тогда
			Подключаемый_СтатьяБюджета_ПриИзменении(Элементы.ТоварыПоПериодамСтатьяБюджета);
		Иначе
			Подключаемый_СтатьяБюджета_ПриИзменении(Элементы.ТоварыСтатьяБюджета);
		КонецЕсли;
	КонецЕсли;
	#КонецОбласти
...
Процедура ТоварыПоПериодамНоменклатураПриИзменении(Элемент)
	...
	//Унификаты   
	Если ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) Тогда
		УнификатыПриИзмененииНоменклатурыСервер("ТоварыПоПериодам", Элементы.ТоварыПоПериодам.ТекущиеДанные.ПолучитьИдентификатор(), КэшированныеЗначения);  
	КонецЕсли;
	#КонецОбласти
	...
	Если ЕстьРеквизит Тогда
		СтараяСтатья = ТекущаяСтрока.СтатьяБюджета; 
	КонецЕсли;
	ТоварыНоменклатураПриИзмененииСервер(ТекущаяСтрока.ПолучитьИдентификатор(), "ТоварыПоПериодам", КэшированныеЗначения);  
	Если ЕстьРеквизит И СтараяСтатья <> ТекущаяСтрока.СтатьяБюджета Тогда   
		Если Элемент = Элементы.ТоварыПоПериодамНоменклатура Тогда
			Подключаемый_СтатьяБюджета_ПриИзменении(Элементы.ТоварыПоПериодамСтатьяБюджета);
		Иначе
...
Процедура Подключаемый_ПриИзмененииКоличества(Элемент)
	...
	Если НЕ Объект.ЗаполнятьПоФормуле Тогда                         
		...
		#Область УХ_Внедрение
		Если СтрНайти(Элемент.Имя, "ТоварыПоПериодамПериодыТаблицы")>0 Тогда
			ТекущееИмяКолонки = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Элемент.Имя, "ТоварыПоПериодамПериодыТаблицы" + "Количество_")[1];
		Иначе 
		#КонецОбласти 
...
Процедура Подключаемый_ПриИзмененииСуммы(Элемент)
	...
	#Область УХ_Внедрение     
	ИмяКолонки = СтрЗаменить(Элемент.Имя, "ТоварыПоПериодамПериодыТаблицы" + "Сумма_", "");
	Если ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) И ЗначениеЗаполнено(СтрокаТоваров.ТоварнаяПозиция) Тогда
		СтрокаТоваров["Цена_" + ИмяКолонки] = 0;
		СтрокаТоваров["Количество_" + ИмяКолонки] = 0;
	Иначе
	#КонецОбласти 
...
Процедура ЗаполнитьЭтапыГрафикаОплаты(Команда)
	...
	Если Объект.ФормированиеПотребностейПланомЗакупок Тогда   
		Если Объект.КроссТаблица Тогда
			ПерезаполнитьКроссТаблицуНаСервере(КэшированныеЗначения);
		КонецЕсли;
	КонецЕсли;
	#КонецОбласти
...
Функция УХ_РаспределитьТоварнуюКатегориюНаСервере(НомерСтроки)
	...
	ПараметрыФормы = Новый Структура();
	Товары = ТоварыПоПериодам.Выгрузить(Новый Структура("НомерСтроки",НомерСтроки));
	Товары = ПоместитьВоВременноеХранилище(Товары, УникальныйИдентификатор);
	ОбъектПлана = Объект.Ссылка;	
	ПараметрыФормы.Вставить("ОбъектПлана",ОбъектПлана);
	ПараметрыФормы.Вставить("ОперативныйПлан",Товары);
	ПараметрыФормы.Вставить("Периоды",ПоместитьВоВременноеХранилище(ЭтотОбъект["Периоды"].Выгрузить(Новый Структура("Активная", Истина)), УникальныйИдентификатор));
...
Процедура УстановитьУсловноеОформление()
	...
	#Область УХ_Встраивание  
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Товары.ТоварнаяПозиция");
	...
	#Область УХ_Встраивание  
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТоварыПоПериодам.ТоварнаяПозиция");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	#КонецОбласти
	...
	Если ФормыУХ.ЭлементыФормыУХУжеСозданы(ЭтаФорма) Тогда
		ВстраиваниеУХПланЗакупок.УстановитьУсловноеОформление(ЭтаФорма);
	КонецЕсли;
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Товары.ТоварнаяКатегория");
	...
	//Унификаты
	ЦентрализованныеЗакупкиФормыУХ.УстановитьУсловноеОформлениеУнификатов(ЭтотОбъект, "Товары");  
	ЦентрализованныеЗакупкиФормыУХ.УстановитьУсловноеОформлениеУнификатов(ЭтотОбъект, "ТоварыПоПериодам");
	#КонецОбласти
...
Процедура УстановитьУсловноеОформлениеКроссТаблицы()
	...
	Для каждого Период Из АктивныеПериоды Цикл
		...
		Если Объект.ПланироватьПоСумме Тогда
			...
			КонецЕсли; 
			#Область УХ_Внедрение
			Если ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) И НЕ ЭтоОдинПериод Тогда
				ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
		...
		#Область УХ_Внедрение
		ВстраиваниеУХПланЗакупок.УстановитьУсловноеОформлениеКроссТаблицы(УсловноеОформление, Объект.ПланироватьПоСумме, Период.ИмяКолонки);
		#КонецОбласти 
...
Процедура ПриИзмененииКоличестваСуммыСтроки(Объект, Форма, СтрокаТоваров)
	...
	Для каждого СтрокаПериода Из Форма.Периоды Цикл
		...
		Если Объект.ПланироватьПоСумме Тогда
			#Область УХ_Внедрение
			Если НЕ (ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) И ЗначениеЗаполнено(СтрокаТоваров.ТоварнаяКатегория)) Тогда
			#КонецОбласти 
...
Процедура РассчитатьИтоговыеПоказатели(Форма, РассчитыватьСтроки = Истина, ЗаполнятьПустыеЦены = Неопределено)
	...
	Для каждого СтрокаТЧ Из Товары Цикл
		#Область УХ_Внедрение
		ЭтоТК = ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) И ЗначениеЗаполнено(СтрокаТЧ.ТоварнаяКатегория);
		#КонецОбласти 
		...
		Для каждого СтрокаПериода Из Форма.Периоды Цикл
			...
			Если РассчитыватьСтроки Тогда
				#Область УХ_Внедрение
				Если ЭтоТК Тогда
					СтрокаТЧ["Количество_" + СтрокаПериода.ИмяКолонки] = 0;
					СтрокаТЧ["Цена_" + СтрокаПериода.ИмяКолонки] = 0;
...
Процедура ПриЧтенииСозданииНаСервере()
	#Область УХ_Внедрение
	ВстраиваниеУХПланЗакупок.ПриЧтенииСозданииНаСервере(ЭтаФорма);
	#КонецОбласти
...
Процедура ОбновитьСтруктуруКроссТаблицы(ТаблицаПериоды, ПересчитыватьИтоговыеПоказатели = Истина)
	...
	#Область УХ_Внедрение
	ПараметрыВывода.Вставить("ЭлементФормыКроссТаблицы", "ТоварыПоПериодамПериодыТаблицы");
	#КонецОбласти
	...
	#Область УХ_Внедрение
	Если ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) И НЕ (Объект.ПланироватьПоСумме И ЭтоОдинПериод) Тогда
		Планирование.ДобавитьПолеКроссТаблицыСумма(ПараметрыВывода, Объект.ПланироватьПоСумме И (ЭтоОдинПериод ИЛИ ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок)), СтруктураДействий);
	Иначе
	#КонецОбласти 
...
Процедура СценарийПриИзмененииСервер()
	...
	#Область УХ_Внедрение
	Реквизиты = Реквизиты + ", РежимФормированияПотребностейПланомЗакупок, ФормированиеПотребностейПланомЗакупок";    
	ВстраиваниеУХПланЗакупок.КонтрольСценарияПланаЗакупок(ЭтаФорма);
	#КонецОбласти 
...
Процедура ВидПланаПриИзмененииСервер(ОбновлятьПравило = Ложь)
	...
	#Область УХ_Внедрение
	Если ЭтотОбъект.Объект.ВидБюджетаПланирования.Пустая() Тогда
		ИспользуемыеВидыБюджетовЗакупок();                          
	КонецЕсли;
	#КонецОбласти
	...
	#Область УХ_Внедрение
	ВстраиваниеУХПланЗакупок.КонтрольВидаПланаЗакупок(ЭтотОбъект);
	#КонецОбласти
...
Процедура УстановитьВидимость()
	...
	#Область УХ_Внедрение
	ВстраиваниеУХПланЗакупок.ПриУстановкеВидимости(ЭтотОбъект);
	#КонецОбласти
...
Процедура ОбновитьСвязанныеРеквизитыОбъекта(Объект, Форма)
	...
	Для каждого СтрокаТЧ Из Товары Цикл
		...
		#Область УХ_Встраивание
		Если ЗначениеЗаполнено(СтрокаТЧ.Номенклатура) И НЕ ЗначениеЗаполнено(СтрокаТЧ.ТоварнаяПозиция) Тогда
			СтрокаТЧ.ТоварнаяПозиция = СтрокаТЧ.Номенклатура;
		КонецЕсли;
		#КонецОбласти
...
Процедура ДобавитьТовары(ТаблицаТоваров, КэшированныеЗначения = Неопределено, ЗагрузкаИзФайла = Ложь)
	...
	Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		Если Объект.КроссТаблица И НЕ ЗагрузкаИзФайла Тогда
			...
			#Область УХ_Внедрение          
			Если ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) Тогда
				ТекущаяСтрока.ТоварнаяПозиция = ТекущаяСтрока.Номенклатура; 
				УнификатыПриИзмененииНоменклатурыСервер("ТоварыПоПериодам", ТекущаяСтрока.ПолучитьИдентификатор(), КэшированныеЗначения);
				ТоварыНоменклатураПриИзмененииСервер(ТекущаяСтрока.ПолучитьИдентификатор(), "ТоварыПоПериодам", КэшированныеЗначения); 
			КонецЕсли;
			#КонецОбласти
		...
		Иначе
			...
			Если ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) Тогда
				ТекущаяСтрока.ТоварнаяПозиция = ТекущаяСтрока.Номенклатура; 
				УнификатыПриИзмененииНоменклатурыСервер("Товары", ТекущаяСтрока.ПолучитьИдентификатор(), КэшированныеЗначения);	
				ТоварыНоменклатураПриИзмененииСервер(ТекущаяСтрока.ПолучитьИдентификатор(), "Товары", КэшированныеЗначения);   
			КонецЕсли;
			#КонецОбласти   
...
Процедура СкопироватьИзКроссТаблицы(КэшированныеЗначения, ЭтотОбъект)
	...
	Если ЭтотОбъект.КроссТаблица
		...
		#Область УХ_Встраивание           
		Для Каждого ПериодТаблицы Из ПараметрыПреобразования.ТаблицаПериоды Цикл 
			Для Каждого КлючИЗначение Из ПараметрыПреобразования.СтруктураРеквизитовПериода Цикл 
				ПериодТаблицы[КлючИЗначение.Ключ] = ПланированиеПотребностейКлиентСервер.РассчитатьДатуОперации(ПериодТаблицы[КлючИЗначение.Ключ], ПараметрыПреобразования.Периодичность);    
			КонецЦикла;
		КонецЦикла;
		#КонецОбласти
		...
		СтруктураДействий.Вставить("ПересчитатьСумму");
		#Область УХ_Внедрение
		СтруктураДействий.Вставить("ПересчетТабличнойЧасти");
...
Процедура СкопироватьВКроссТаблицу(КэшированныеЗначения)
	...
	Если Объект.КроссТаблица
		...
		Для каждого СтрокаТЧ Из Объект.Товары Цикл
			...
			#Область УХ_Внедрение
			Если ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) 
				И (ТипЗнч(СтрокаТЧ.ТоварнаяПозиция) = Тип("СправочникСсылка.ТоварныеКатегории") ИЛИ 
					ТипЗнч(СтрокаТЧ.ТоварнаяПозиция) = Тип("СправочникСсылка.КатегорииЗакупок")) Тогда
				СтрокаТЧ.ТоварнаяКатегория = СтрокаТЧ.ТоварнаяПозиция;
			КонецЕсли;
			#КонецОбласти 
...
Функция ПараметрыПреобразованияКроссТаблицы()
	...
	ПоляГруппировки = ПоляГруппировки+",ТоварнаяПозиция";
	ПоляГруппировки = ПоляГруппировки+",СтатьяБюджета, "+ АналитикиСтатейБюджетовУХКлиентСервер.РеквизитыАналитикИзШаблона("Аналитика%1");
	#КонецОбласти
...
Процедура НадписьОплатаНажатиеЗавершение(ПараметрыОплаты, ДополнительныеПараметры) Экспорт
	...
	Если ПараметрыОплаты <> Неопределено Тогда
		...
		#Область УХ_Внедрение     
		Если ЭтотОбъект.Команды.Найти("ЗаполнитьЭтапыГрафикаОплаты") <> Неопределено Тогда
			ЗаполнитьЭтапыГрафикаОплаты(ЭтотОбъект.Команды.ЗаполнитьЭтапыГрафикаОплаты);
		КонецЕсли;
		#КонецОбласти
...
Процедура ЗаполнитьПланОплаты(ГрафикСоглашенияЗаполнен, ГрафикЗаполнен)
	...
	#Область УХ_Внедрение 
	Если Объект.ФормированиеПотребностейПланомЗакупок И Объект.ЭтапыГрафикаОплаты.Количество()=0 Тогда
		Объект.ЭтапыГрафикаОплаты.Очистить();
		ЭтапГрафикаОплаты = Объект.ЭтапыГрафикаОплаты.Добавить();
		...
		Возврат;
	ИначеЕсли Объект.ФормированиеПотребностейПланомЗакупок Тогда 
		ЗаполнитьПланОплатНаСервере();
		Возврат;
	КонецЕсли;
	#КонецОбласти
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров)
	...
	#Область УХ_Внедрение
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='План закупок с учетом законтрактованного объема'"), "[ПланыЗакупок] - [Законтрактовано]");
	#КонецОбласти
	Идентификатор = КонструкцияДляВставки;
...
Функция ПодготовитьПараметрыЗаполненияДокументаДляЗамещения(ПараметрыЗадания, КэшированныеЗначения)
	...
	#Область УХ_Внедрение
	ПараметрыЗадания.Вставить("ЭтоПланированиеПотребностей", ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок));
	Если ПараметрыЗадания.ЭтоПланированиеПотребностей Тогда
...
Функция ПолучитьРезультатЗаполненияНаСервере(КэшированныеЗначения)
	...
	Если ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) Тогда
		Для Каждого Строка Из Объект.Товары Цикл
			Если ЗначениеЗаполнено(Строка.ТоварнаяПозиция) Тогда
				Продолжить;
			КонецЕсли;
			Если ТипЗнч(Строка.Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
				Строка.ТоварнаяПозиция = Строка.Номенклатура;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	#КонецОбласти 
	...
	Пока Индекс > 0 Цикл
		...
		#Область УХ_Внедрение
		Если ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) И ТипЗнч(СтрокаТЧ.ТоварнаяПозиция) = Тип("СправочникСсылка.ТоварныеКатегории") Тогда
			// Проверять тип номенклатуры для ТоварнойКатегории не надо
			Продолжить;
		КонецЕсли;
		#КонецОбласти 
...
Процедура УдалитьПустыеСтрокиНаСервере()
	...
	Пока Индекс >= 0 Цикл
		...
		Для каждого Период Из АктивныеПериоды Цикл
			...
			#Область УХ_Внедрение
			Если ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) И ЗначениеЗаполнено(СтрокаТЧ.ТоварнаяКатегория)
					И СтрокаТЧ["Сумма_"+Период.ИмяКолонки] <> 0 Тогда
...
Функция ПараметрыЗамещения(Знач Объект)
	...
	#Область УХ_Внедрение
	Если ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) Тогда
		// Добавляем доп.параметры замещения
		ПараметрыЗамещения.Вставить("РежимФормированияПотребностейПланомЗакупок", Объект.РежимФормированияПотребностейПланомЗакупок);
		ПараметрыЗамещения.Вставить("ФормированиеПотребностейПланомЗакупок", Объект.ФормированиеПотребностейПланомЗакупок);
		ПараметрыЗамещения.Вставить("ЭтоПланированиеПотребностей", Истина);
		ПараметрыЗамещения.Вставить("ЦФО", Объект.ЦФО);
		ПараметрыЗамещения.Вставить("Проект", Объект.Проект);
		ПараметрыЗамещения.Вставить("Организация", Объект.Организация);
		ПараметрыЗамещения.Вставить("МестоПоставки", Объект.МестоПоставки);
		ПараметрыЗамещения.Вставить("Приоритет", Объект.Приоритет);
		ПараметрыЗамещения.Вставить("Менеджер", Объект.Менеджер);
		
		ПараметрыЗамещения.Вставить("СтатьяБюджета", Объект.СтатьяБюджета);
		ПараметрыЗамещения.Вставить("Аналитика1", Объект.Аналитика1);
		ПараметрыЗамещения.Вставить("Аналитика2", Объект.Аналитика2);
		ПараметрыЗамещения.Вставить("Аналитика3", Объект.Аналитика3);
...
Процедура ЗаменитьНаАналоги(АдресВХранилище)
	...
	#Область УХ_Внедрение
	ДополнительныеПоляЗаполнения = ДополнительныеПоляЗаполнения + ", " + "Склад, МестоПоставки, СтатьяБюджета, " + АналитикиСтатейБюджетовУХКлиентСервер.РеквизитыАналитикИзШаблона("Аналитика%1");
	#КонецОбласти
...
Процедура СтатусОбъектаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение = Объект.Статус Тогда
		Возврат;
	КонецЕсли;
...
#Область Унификаты_УХ

&НаКлиенте
Процедура Подключаемый_НеЗаменятьНаУнификатПриИзменении(Элемент)    
	
	ЭлементИмя = СтрЗаменить(Элемент.Имя, "_УнификатыНеЗаменятьНаУнификат", "");
	Настройки = ЭтотОбъект.УнификатыНастройкиФормы[ЭлементИмя];      
	
	Если ЭлементИмя = "ТоварыПоПериодам" Тогда
		Потребность = Элементы.ТоварыПоПериодам.ТекущиеДанные;
	Иначе
		Потребность = Элементы.Товары.ТекущиеДанные; 
	КонецЕсли;
	
	Если Потребность.УнификатыНеЗаменятьНаУнификат Тогда   
		Если ЗначениеЗаполнено(Потребность.УнификатыНоменклатураИсходная) Тогда     
			
			Потребность[Настройки.ПолеНоменклатура]			= Потребность.УнификатыНоменклатураИсходная;
			Потребность[Настройки.ПолеХарактеристика]		= Потребность.УнификатыХарактеристикаИсходная;
			Потребность[Настройки.ПолеЕдиницаИзмерения]		= Потребность.УнификатыЕдиницаИзмеренияИсходная;
			Потребность.УнификатыЗамененоНаУнификат 	= Ложь;  
			
			Подключаемый_ОбновитьПараметрыНоменклатуруТК(Потребность);  
			
			Если ЭлементИмя = "ТоварыПоПериодам" Тогда
				ТоварыПоПериодамНоменклатураПриИзменении(Элементы.ТоварыПоПериодамНоменклатура);
			Иначе
				ТоварыНоменклатураПриИзменении(Элементы.ТоварыНоменклатура);
			КонецЕсли;        
			
			Подключаемый_ОбновитьПараметрыПотребности(Потребность, ЭлементИмя, ЭтотОбъект.ИмяФормы);

		КонецЕсли;  
	Иначе                  
		Если ЭлементИмя = "ТоварыПоПериодам" Тогда
			ПриИзмененииТоварнойПозиции(Потребность, "ТоварыПоПериодам");
		Иначе
			ПриИзмененииТоварнойПозиции(Потребность, "Товары"); 
		КонецЕсли;
	КонецЕсли; 
...
&НаКлиенте
Процедура Подключаемый_ОбновитьПараметрыПотребности(Знач Потребность, Знач ЭлементИмя, Знач ИмяФормы)
	...
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействий);
	СтруктураДействий.Вставить("ЗаполнитьПризнакХарактеристикиИспользуются", Новый Структура("Номенклатура", "ХарактеристикиИспользуются"));
	СтруктураДействий.Вставить("ПроверитьХарактеристикуПоВладельцу",         Потребность.Характеристика);
	СтруктураДействий.Вставить("ЗаполнитьПризнакАртикул",                    Новый Структура("Номенклатура", "Артикул"));
	СтруктураДействий.Вставить("ЗаполнитьПризнакТипНоменклатуры",            Новый Структура("Номенклатура", "ТипНоменклатуры"));
	СтруктураДействий.Вставить("ПроверитьЗаполнитьУпаковкуПоВладельцу",      Потребность.Упаковка);
	
	СтруктураДействий.Вставить("НоменклатураПриИзмененииПереопределяемый", 	Новый Структура("ИмяФормы, ИмяТабличнойЧасти",
																			ИмяФормы, ЭлементИмя));
	
	ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(Потребность, СтруктураДействий, Неопределено);

КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура Подключаемый_ОбновитьПараметрыНоменклатуруТК(Знач Потребность)
	
	Если ТипЗнч(Потребность.ТоварнаяПозиция) = Тип("СправочникСсылка.ТоварныеКатегории") Тогда
		Потребность.Номенклатура = ПредопределенноеЗначение("Справочник.Номенклатура.ПустаяСсылка");
		Потребность.ТоварнаяКатегория = Потребность.ТоварнаяПозиция;     
	ИначеЕсли ТипЗнч(Потребность.ТоварнаяПозиция) = Тип("СправочникСсылка.КатегорииЗакупок") Тогда
		Потребность.Номенклатура = ПредопределенноеЗначение("Справочник.Номенклатура.ПустаяСсылка");
		Потребность.ТоварнаяКатегория = Потребность.ТоварнаяПозиция;
	Иначе
		Потребность.Номенклатура = Потребность.ТоварнаяПозиция;
		Потребность.ТоварнаяКатегория = ПредопределенноеЗначение("Справочник.ТоварныеКатегории.ПустаяСсылка");
	КонецЕсли;

КонецПроцедуры  

&НаСервере
Процедура УнификатыПриИзмененииНоменклатурыСервер(ЭлементИмя, Идентификатор, КэшированныеЗначения)
	
	ПутьКДанным = Элементы[ЭлементИмя].ПутьКДанным;  
	ЧастиПути = СтрРазделить(ПутьКДанным, ".", Ложь);
	Источник = ЭтотОбъект;
	Для Каждого ЧастьПути Из ЧастиПути Цикл
		Источник = Источник[ЧастьПути];
	КонецЦикла;
	ТекущаяСтрока = Источник.НайтиПоИдентификатору(Идентификатор); 
	Если ТекущаяСтрока.УнификатыНеЗаменятьНаУнификат Тогда 
		Возврат;
	Иначе
		ЦентрализованныеЗакупкиФормыУХ.УстановитьСвойстваУнификат(ЭтотОбъект, ЭлементИмя, ТекущаяСтрока);   
		Подключаемый_ОбновитьПараметрыНоменклатуруТК(ТекущаяСтрока); 
	КонецЕсли;      
	
	УстановитьЦенуНоменклатурыУХ(Идентификатор, ПутьКДанным, КэшированныеЗначения);	
                                        
КонецПроцедуры  

&НаКлиенте
Процедура ТоварыПоПериодамХарактеристикаПриИзменении(Элемент)
	//Унификаты      
	Если ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) Тогда 
		УнификатыПриИзмененииНоменклатурыСервер("ТоварыПоПериодам", Элементы.ТоварыПоПериодам.ТекущиеДанные.ПолучитьИдентификатор(), КэшированныеЗначения);  
		Подключаемый_ОбновитьПараметрыПотребности(Элементы.ТоварыПоПериодам.ТекущиеДанные, "ТоварыПоПериодам", ЭтотОбъект.ИмяФормы);  
	КонецЕсли;
КонецПроцедуры

#КонецОбласти

#Область УХ_ПереопределяемыеСобытияЭлементовДляКонтроляДокумента
...
&НаКлиенте
Процедура СтатьяБюджетовОплатыДоОтгрузкиПриИзменении(Элемент)
	к=1;
КонецПроцедуры

&НаКлиенте
Процедура СтатьяБюджетовОплатыПослеОтгрузкиПриИзменении(Элемент)
	к=1;
КонецПроцедуры
...
&НаСервере
Процедура ИспользуемыеВидыБюджетовЗакупок()
	ВстраиваниеУХПланЗакупок.ИспользуемыеВидыБюджетовЗакупок(ЭтотОбъект);	
КонецПроцедуры

&НаСервере
Процедура ТоварыНоменклатураПриИзмененииСервер(ИдентификаторСтроки, ИмяТабличнойЧасти, КэшированныеЗначения)   
	
	Источник = ?(ВРег(ИмяТабличнойЧасти) = ВРег("Товары"), 
					Объект.Товары, 
					ТоварыПоПериодам);	
	
	ТекущаяСтрока = Источник.НайтиПоИдентификатору(ИдентификаторСтроки);

	ЦентрализованныеЗакупкиВызовСервераУХ.УстановитьДоступныеСтатьиЭлементаПоПараметруПотребностей(ЭтотОбъект, 
																									ИмяТабличнойЧасти+"СтатьяБюджета", 
																									ТекущаяСтрока.ТоварнаяПозиция, 
																									ТекущаяСтрока, 
																									Объект.ВидБюджетаПланирования, 
																									"СтатьяБюджета, Аналитика1, Аналитика2, Аналитика3, Аналитика4, Аналитика5, Аналитика6");	
																									
	ВстраиваниеУХПланЗакупок.ТоварыНоменклатураПриИзмененииСервер(ЭтотОбъект, ТекущаяСтрока, ИдентификаторСтроки, ИмяТабличнойЧасти);	
	
	Если Источник = ТоварыПоПериодам Или Не ЗначениеЗаполнено(ТекущаяСтрока.Цена) Тогда  
		УстановитьЦенуНоменклатурыУХ(ИдентификаторСтроки, ИмяТабличнойЧасти, КэшированныеЗначения);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура УстановитьЦенуНоменклатурыУХ(ИдентификаторСтроки, ИмяТабличнойЧасти, КэшированныеЗначения)
	
	Перем ВидЦеныКорпЗак, ОтборСтрок, Результат, Строки, Хранилище;  
	...
	ТекущаяСтрока = Источник.НайтиПоИдентификатору(ИдентификаторСтроки);   
	ТекущаяНоменклатура = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ТекущаяСтрока, "Номенклатура", Неопределено);
	
	Если ЗначениеЗаполнено(ТекущаяНоменклатура) Тогда 
		
		ВидЦеныКорпЗак = Константы.ТипЦенДляРасценкиЗаявокНаПотребность.Получить();
		Если ЗначениеЗаполнено(ВидЦеныКорпЗак) Тогда                
			
			//
			СтруктураДействий = Новый Структура;
			СтруктураДействий.Вставить(?(ЗначениеЗаполнено(ТекущаяНоменклатура), "УстановитьЦену", "УстановитьЦенуТК"));
			
			ЗаменаИменРеквизитов = Новый Структура;
			ЗаменаИменРеквизитов.Вставить("ДатаОкончания", "ОкончаниеПериода");
			ЗаменаИменРеквизитов.Вставить("ВалютаДокумента", "Валюта");
			
			ЦентрализованныеЗакупкиКлиентСерверУХ.ОбработатьСтрокуТЧ(Объект, ТекущаяСтрока, 
																		?(Источник = ТоварыПоПериодам, ЭтотОбъект.Периоды, Новый Массив), 
																		СтруктураДействий, ЗаменаИменРеквизитов);
			
			Если Источник = ТоварыПоПериодам Тогда   
...
Процедура ЗаполнитьПланОплатНаСервере() 
	ВстраиваниеУХПланЗакупок.ЗаполнитьПланОплатНаСервере(ЭтотОбъект);
КонецПроцедуры

&НаСервере 
Процедура ИзменилосьРаспределениеПоНоменклатуре(ТекущаяСтрока,АдресРезультата);
	
	ТаблицаРаспределения = ПолучитьИзВременногоХранилища(АдресРезультата);
	ТаблицаДобавления = ТоварыПоПериодам.Выгрузить().СкопироватьКолонки();
	ДанныеЗаполнения = ТоварыПоПериодам[ТекущаяСтрока];
	СтрокаСворачивания = "Количество,Сумма,КоличествоУпаковок";
	Для Каждого Строка Из ТаблицаРаспределения Цикл
		Доб = ТаблицаДобавления.Добавить();
		Доб.Номенклатура = Строка.Номенклатура;
		Доб.Цена = Строка.Цена;
		Доб.ТоварнаяПозиция = Строка.Номенклатура;
		ЗаголовокПериода = Строка.ПериодПотребности;
		Доб.Характеристика = Строка.Характеристика;
		Доб.Цена = Строка.Цена;
		Доб.ТипНоменклатуры = ОбщегоНазначенияУХ.ЗначениеРеквизитаОбъекта(Строка.Номенклатура,"ТипНоменклатуры");
		Доб["Количество_"+ЗаголовокПериода] = Строка.Количество;
		Доб["Сумма_"+ЗаголовокПериода] = Строка.Сумма;
		Доб["Цена_"+ЗаголовокПериода] = Строка.Цена;
		
		ДанныеЗаполнения["Количество_"+ЗаголовокПериода] = ДанныеЗаполнения["Количество_"+ЗаголовокПериода] - Строка.Количество;
		ДанныеЗаполнения["Сумма_"+ЗаголовокПериода] = ДанныеЗаполнения["Сумма_"+ЗаголовокПериода] - Строка.Сумма;
		ДанныеЗаполнения["Сумма"] = ДанныеЗаполнения["Сумма"] - Строка.Сумма;
		
		Доб.Количество = Строка.Количество;
		Доб.КоличествоУпаковок = Строка.Количество;
		
		Доб.Сумма = Строка.Сумма;
		Если СтрНайти(СтрокаСворачивания,"Сумма_"+ЗаголовокПериода) = 0 Тогда
			 СтрокаСворачивания = СтрокаСворачивания +",Сумма_"+ЗаголовокПериода;
		КонецЕсли;
		Если СтрНайти(СтрокаСворачивания,"Количество_"+ЗаголовокПериода) = 0 Тогда
			 СтрокаСворачивания = СтрокаСворачивания +",Количество_"+ЗаголовокПериода;
		КонецЕсли;
		Если СтрНайти(СтрокаСворачивания,"Цена_"+ЗаголовокПериода) = 0 Тогда
			 СтрокаСворачивания = СтрокаСворачивания +",Цена_"+ЗаголовокПериода;
		КонецЕсли;
	КонецЦикла;	
	СтрокиИсключения = "ТоварнаяКатегория,ТипНоменклатуры,Количество,КоличествоУпаковок";
	Для Каждого Период Из ТаблицаРаспределения Цикл
		СтрокиИсключения = СтрокиИсключения+",Количество_"+Период.ПериодПотребности+",Сумма_"+Период.ПериодПотребности+",Цена_"+Период.ПериодПотребности;	
	КонецЦикла;	
	ТаблицаДобавления.Свернуть("Номенклатура,Характеристика,Цена,ТоварнаяПозиция",СтрокаСворачивания);
	Периоды = ЭтотОбъект["Периоды"].Выгрузить(Новый Структура("Активная", Истина));
	Для Каждого Строка ИЗ ТаблицаДобавления Цикл
		ДобСтрока = ТоварыПоПериодам.Добавить();
		ЗаполнитьЗначенияСвойств(ДобСтрока,ДанныеЗаполнения,,СтрокиИсключения);
		ЗаполнитьЗначенияСвойств(ДобСтрока,Строка);
		Для Каждого Период ИЗ Периоды Цикл
			Если ДобСтрока.Цена <> ДобСтрока["Цена_"+Период.ИмяКолонки] 
				И ДобСтрока["Цена_"+Период.ИмяКолонки] <> 0 Тогда
				 ДобСтрока.Цена = 0;
				 Прервать;
			Конецесли;	 
		КонецЦикла;	
	КонецЦикла;	
	Если ДанныеЗаполнения.Сумма = 0 Тогда
		ТоварыПоПериодам.Удалить(ТекущаяСтрока);
	КонецЕсли;	
КонецПроцедуры	

&НаКлиенте
Процедура ПриОжиданииПерезаполнитьПланыОплаты() Экспорт  
	ГрафикСоглашенияЗаполнен = ЭтапыОплатыВызовСервера.ГрафикСоглашенияЗаполнен(Объект.Соглашение);
	ГрафикЗаполнен = Объект.ЭтапыГрафикаОплаты.Количество() > 0;
	ЗаполнитьПланОплаты(ГрафикСоглашенияЗаполнен, ГрафикЗаполнен);
...
Процедура Подключаемый_ПолеФормы_ПриИзменении(Элемент)
	...
	Если ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) Тогда
		// проверяем лимиты
		ВыполнитьКонтрольДокументаНаСервере(КэшированныеЗначения);
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_ПриИзмененииТоварыТоварнаяПозиция(Элемент)
	
	ПриИзмененииТоварнойПозиции(Элементы.Товары.ТекущиеДанные, "Товары");
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ЗафиксироватьТипНоменклатуры(Команда)
	
	Если Объект.КроссТаблица Тогда
		ЦентрализованныеЗакупкиКлиентУХ.ЗафиксироватьТипНоменклатуры(
			ЭтаФорма,
			"ТоварыПоПериодамЗафиксироватьТипНоменклатуры",
			"ТоварыПоПериодамТоварнаяПозиция");       
	Иначе
		ЦентрализованныеЗакупкиКлиентУХ.ЗафиксироватьТипНоменклатуры(
			ЭтаФорма,
			"ТоварыЗафиксироватьТипНоменклатуры",
			"ТоварыТоварнаяПозиция");
	КонецЕсли;
...
Процедура Подключаемый_Команда(Команда) 
	...
	ПостояннаяКоманда = ЭтотОбъект["ПостоянныеКоманды"].НайтиПоЗначению(Команда.Имя);
...
&НаКлиенте
Процедура Подключаемый_ПересчитатьСтроку() Экспорт

	Если ЭтотОбъект.Объект.КроссТаблица Тогда                    
		СтрокаТоваров = Элементы.ТоварыПоПериодам.ТекущиеДанные;
		ПриИзмененииКоличестваСуммыСтроки(Объект, ЭтаФорма, СтрокаТоваров);
		РассчитатьИтоговыеПоказатели(ЭтаФорма, Ложь);
	Иначе
	...
	ПриОжиданииПерезаполнитьПланыОплаты();
	ПриОжиданииВыполнитьКонтрольДокумента();

КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ПриИзмененииТоварыПоПериодамТоварнаяПозиция(Элемент)
	
	ПриИзмененииТоварнойПозиции(Элементы.ТоварыПоПериодам.ТекущиеДанные, "ТоварыПоПериодам");
...
Процедура Подключаемый_СкладПриИзменении(Элемент)
	...
	Если Элемент = Элементы.Склад Тогда
		СкладПриИзменении(Элемент);
	КонецЕсли;      
	
	Если ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) Тогда
		Если Элемент.Имя = Элементы.Склад.Имя Тогда
			Объект.МестоПоставки = ПолучитьМестоПоставки(Объект.Склад); 
		ИначеЕсли Элемент.Имя = Элементы.ТоварыПоПериодамСклад.Имя Тогда
			Элементы.ТоварыПоПериодам.ТекущиеДанные.МестоПоставки = ПолучитьМестоПоставки(Элементы.ТоварыПоПериодам.ТекущиеДанные.Склад);
		ИначеЕсли Элемент.Имя = Элементы.ТоварыСклад.Имя Тогда
			Элементы.Товары.ТекущиеДанные.МестоПоставки = ПолучитьМестоПоставки(Элементы.Товары.ТекущиеДанные.Склад);
		КонецЕсли;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПолучитьМестоПоставки(Склад)
	
	Возврат РегистрыСведений.МестаПоставкиДляСкладов.ПолучитьМестоПоставки(Склад)

КонецФункции

&НаКлиенте
Процедура ПриИзмененииТоварнойПозиции(ТекущиеДанные, Положение)
	...
	Если ТипЗнч(ТекущиеДанные.ТоварнаяПозиция) = Тип("СправочникСсылка.ТоварныеКатегории") Тогда
		...
		ТекущиеДанные.ТоварнаяКатегория = ТекущиеДанные.ТоварнаяПозиция;    
	ИначеЕсли ТипЗнч(ТекущиеДанные.ТоварнаяПозиция) = Тип("СправочникСсылка.КатегорииЗакупок") Тогда
		ТекущиеДанные.Номенклатура = ПредопределенноеЗначение("Справочник.Номенклатура.ПустаяСсылка");
		ТекущиеДанные.ТоварнаяКатегория = ТекущиеДанные.ТоварнаяПозиция;
	Иначе
		ТекущиеДанные.Номенклатура = ТекущиеДанные.ТоварнаяПозиция;
		ТекущиеДанные.ТоварнаяКатегория = ПредопределенноеЗначение("Справочник.ТоварныеКатегории.ПустаяСсылка");
	КонецЕсли;
	
	Если Положение = "ТоварыПоПериодам" Тогда
		ТоварыПоПериодамНоменклатураПриИзменении(Элементы.ТоварыПоПериодамНоменклатура);
	ИначеЕсли Положение = "Товары" Тогда
		ТоварыНоменклатураПриИзменении(Элементы.ТоварыНоменклатура);
	КонецЕсли;
...
Процедура ВыполнитьКомандуУХ(Команда)
	...
	//Если Команда.Имя = "КонтрольЛимитов" Тогда
	//	Модифицированность = Истина;
	//	ВыполнитьПроверкуЛимитовНаСервере(Истина, КэшированныеЗначения);
	//КонецЕсли;
	//
...
&НаКлиенте
Процедура Подключаемый_ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	
	Если НЕ ЭтоПланированиеПотребностей(Объект.ФормированиеПотребностейПланомЗакупок) Тогда
		Возврат;
	КонецЕсли;
	...
	Если НоваяСтрока Тогда
		...
		Если ЗначениеЗаполнено(ЭтаФорма.ТипНоменклатуры) Тогда
			Элемент.ТекущиеДанные.ТоварнаяПозиция = Новый(ЭтаФорма.ТипНоменклатуры);
		КонецЕсли;
...
&НаСервере
Процедура ВыполнитьКонтрольДокументаНаСервере(КэшированныеЗначения)
	
	// Сохранить реквизит формы в объект
	Если Объект.КроссТаблица Или ИспользуютсяПланируемыеОстатки Тогда
...
&НаСервере
Процедура ПерезаполнитьКроссТаблицуНаСервере(КэшированныеЗначения)
	...
	//Планирование.ЗаполнитьТаблицуИзКроссТаблицы(Объект.Товары, Объект.Товары, ТоварыПоПериодам, ПараметрыПреобразованияКроссТаблицы(), 
	//СтруктураДействий, КэшированныеЗначения);  
