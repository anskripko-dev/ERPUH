...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") 
		И ДанныеЗаполнения.Свойство("Сценарий") И ЗначениеЗаполнено(ДанныеЗаполнения.Сценарий) 
		И (НЕ ДанныеЗаполнения.Свойство("РежимФормированияПотребностейПланомЗакупок") 
		ИЛИ НЕ ЗначениеЗаполнено(ДанныеЗаполнения.РежимФормированияПотребностейПланомЗакупок)) Тогда
		
		//
		Режим = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения.Сценарий, "РежимФормированияПотребностейПланомЗакупок");
		ДанныеЗаполнения.Вставить("РежимФормированияПотребностейПланомЗакупок", Режим); 
		ДанныеЗаполнения.Вставить("ФормированиеПотребностейПланомЗакупок", 		ВстраиваниеУХСценарииТоварногоПланирования.ФормированиеПотребностей(ДанныеЗаполнения.Сценарий)); 
	КонецЕсли;
	#КонецОбласти
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ Документы.ПланЗакупок.ЭтоПотребностиДляКЗ(ЭтотОбъект) Тогда
		
		// Чистим ненужное
		УдалитьКонтроль.Очистить();
		РезультатыКонтроля.Очистить();
		
		ИменаРеквизитов = "Приоритет, МестоПоставки, Организация,Проект,Менеджер,Контрагент,ДоговорКонтрагента,СтатьяБюджета," + 
						  "СтатьяБюджетаОплат,СтатьяБюджетаОплатКредит,ЕстьПревышениеЛимитов,ЕстьПревышениеЛимитыБюджет," + 
						  "ЕстьПревышениеЛимитыРезерв,ЕстьПревышениеЛимитыВзаиморасчеты,ЦФО,Аналитика1,Аналитика2,Аналитика3,"+
						  "АналитикаОплат1,АналитикаОплат2,АналитикаОплат3,АналитикаОплатКредит1,АналитикаОплатКредит2,АналитикаОплатКредит3";
		
		ПустыеДанные = Новый Структура(Именареквизитов);
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, ПустыеДанные, ИменаРеквизитов);
...
Процедура ОбновитьЗамещениеПлана(РежимЗаписи, ОбновлениеИБ = Ложь) Экспорт
	...
	#Область УХ_Внедрение
	Если Документы.ПланЗакупок.ЭтоПотребностиДляКЗ(ЭтотОбъект) Тогда
		// Удаляем лишние реквизиты
		ТекстЗамены = "";
		
		ТекстДляЗамены = "И ПланЗакупокЗамещающий.Подразделение = &Подразделение";
		Запрос.Текст = СтрЗаменить(Запрос.Текст, ТекстДляЗамены, ТекстЗамены);
			
		ТекстДляЗамены = "И ПланЗакупокЗамещающий.Склад = &Склад";
		Запрос.Текст = СтрЗаменить(Запрос.Текст, ТекстДляЗамены, ТекстЗамены);
...
Процедура ОбновитьЗамещенныеПланы()
	...
	Если Документы.ПланЗакупок.ЭтоПотребностиДляКЗ(ЭтотОбъект) Тогда
		...
		ТекстДляЗамены = "И ПланЗакупок.Подразделение = ПланЗакупокЗамещенный.Подразделение";
		Запрос.Текст = СтрЗаменить(Запрос.Текст, ТекстДляЗамены, ТекстЗамены);
			
		ТекстДляЗамены = "И ПланЗакупок.Склад = ПланЗакупокЗамещенный.Склад";
		Запрос.Текст = СтрЗаменить(Запрос.Текст, ТекстДляЗамены, ТекстЗамены);
		
		// Новые реквизиты
		ТекстДляЗамены = "И ПланЗакупок.Назначение = ПланЗакупокЗамещенный.Назначение";
		ТекстЗамены = ТекстДляЗамены + "
			|	И ПланЗакупок.СтатьяБюджета = ПланЗакупокЗамещенный.СтатьяБюджета
			|	И ПланЗакупок.Аналитика1 = ПланЗакупокЗамещенный.Аналитика1
			|	И ПланЗакупок.Аналитика2 = ПланЗакупокЗамещенный.Аналитика2
			|	И ПланЗакупок.Аналитика3 = ПланЗакупокЗамещенный.Аналитика3
			|	И ПланЗакупок.ЦФО = ПланЗакупокЗамещенный.ЦФО
			|	И ПланЗакупок.Проект = ПланЗакупокЗамещенный.Проект
			|	И ПланЗакупок.Организация = ПланЗакупокЗамещенный.Организация
			|	И ПланЗакупок.МестоПоставки = ПланЗакупокЗамещенный.МестоПоставки
			|	И ПланЗакупок.Менеджер = ПланЗакупокЗамещенный.Менеджер
			|	И ПланЗакупок.Приоритет = ПланЗакупокЗамещенный.Приоритет";
		Запрос.Текст = СтрЗаменить(Запрос.Текст, ТекстДляЗамены, ТекстЗамены);
	КонецЕсли;
	#КонецОбласти	
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	#Область УХ_Встраивание    
	Если ЭтотОбъект.Статус = Перечисления.СтатусыПланов.Утвержден Тогда  
		Если ВстраиваниеУХСценарииТоварногоПланирования.ФормированиеПотребностей(Сценарий) Тогда   
			
			ИнтеграцияИС.ИнициализироватьДополнительныеСвойстваДляПроведения(Ссылка, ДополнительныеСвойства, РежимПроведения);
			ВидБюджетаЗаявки = ВидБюджетаПланирования;
	...
	Движения.ПотребностиВНоменклатуре.Очистить();
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не КроссТаблица
		...
		#Область УХ_Внедрение
		Если ЭтотОбъект.ФормированиеПотребностейПланомЗакупок = Истина Тогда
	...
	#Область УХ_Внедрение
	ВстраиваниеУХПланЗакупок.ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты, ЭтотОбъект, МассивНепроверяемыхРеквизитов);
	#КонецОбласти
...
Процедура ЗаполнитьДанныеПоУмолчанию()
	...
	#Область УХ_Внедрение
	ТекстДляЗамены = "ДанныеДокумента.КроссТаблица КАК КроссТаблица,";
	ТекстЗамены = ТекстДляЗамены + "
	|	ЕСТЬNULL(СценарииТоварногоПланирования.РежимФормированияПотребностейПланомЗакупок, ЛОЖЬ) КАК РежимФормированияПотребностейПланомЗакупок,
	|	ЕСТЬNULL(СценарииТоварногоПланирования.ФормированиеПотребностейПланомЗакупок, ЛОЖЬ) КАК ФормированиеПотребностейПланомЗакупок,";
	Запрос.Текст = СтрЗаменить(Запрос.Текст, ТекстДляЗамены, ТекстЗамены);
	#КонецОбласти 
...
Процедура ЗаполнитьРеквизитыПланаПоСценариюВидуПлана()
	...
	#Область УХ_Внедрение
	РеквизитыСценария = РеквизитыСценария + ", РежимФормированияПотребностейПланомЗакупок, ФормированиеПотребностейПланомЗакупок";
	#КонецОбласти
