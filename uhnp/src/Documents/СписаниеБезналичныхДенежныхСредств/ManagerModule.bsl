...
Функция ТекстЗапросаТаблицаРеестрДокументов(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект = Неопределено)
	...
	Если ДокументОбъект = Неопределено 
		Или ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаАрендодателю Тогда
		...
		|	КОНЕЦ КАК Сумма,
		|	&ВалютаДенежныхСредств                  КАК Валюта,
		|	ЕСТЬNULL(ВложенныйЗапрос.ДоговорАренды, НЕОПРЕДЕЛЕНО) КАК Договор,
		|	&НаправлениеДеятельности                КАК НаправлениеДеятельности,
		|	&Исправление                            КАК СторноИсправление,
		|	&СторнируемыйДокумент                   КАК СторнируемыйДокумент,
		|	&ИсправляемыйДокумент                   КАК ИсправляемыйДокумент,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			ДанныеДокумента.Ссылка КАК Ссылка,
		|			МАКСИМУМ(ТаблицаРасшифровкаПлатежа.ДоговорАренды) КАК ДоговорАренды
		|		ИЗ
		|			ДанныеДокументаШапка КАК ДанныеДокумента
		...
		|
		|		ИМЕЮЩИЕ
		|			КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаРасшифровкаПлатежа.ДоговорАренды) = 1) КАК ВложенныйЗапрос
		|		ПО (ДанныеДокумента.Ссылка = ВложенныйЗапрос.Ссылка)
		|
		|ГДЕ
		|	ДанныеДокумента.Ссылка = &Ссылка
		|	И &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОплатаАрендодателю)
		|";
...
Функция ТекстЗапросаИсточникиДанныхПравилПолученияФакта() Экспорт
	...
	|ГДЕ
	|	НастройкиХозяйственныхОпераций.ХозяйственнаяОперация В (ВЫБОР
	...
	|					ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОплатаВыкупнойСтоимостиПредметовАрендыАванс)
	|				ИНАЧЕ &ХозяйственнаяОперация
	|			КОНЕЦ, ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КонвертацияВалюты), ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КурсовыеРазницыДСПрибыль), ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КурсовыеРазницыДСУбыток))
...
Функция ТекстЗапросаИсточникФактаПоТипамДанныхУчета(Расход, Приход, ИмяВременнойТаблицы = "") Экспорт
	...
	|	&ХозяйственнаяОперация КАК ИсточникДанных,
	|	&Организация КАК Организация,
	|	&Подразделение КАК Подразделение,
	|	&НаправлениеДеятельности КАК НаправлениеДеятельности,
	...
	|	&Сумма КАК СуммаКВыплатеСверхЛимита,
	|	&Сумма КАК СуммаКВыплате
	|ПОМЕСТИТЬ ИмяВременнойТаблицы
	|ИЗ
	|	РегистрНакопления.ДенежныеСредстваБезналичные КАК ТаблицаДанных
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ КорРегистрНакопления КАК ТаблицаДанныхКор
	|		ПО ТаблицаДанных.Период = ТаблицаДанныхКор.Период
	|			И ТаблицаДанных.Регистратор = ТаблицаДанныхКор.Регистратор
	|			И ТаблицаДанных.ИдентификаторФинЗаписи = ТаблицаДанныхКор.ИдентификаторФинЗаписи
	|ГДЕ
	|	ТаблицаДанных.Активность
	|	И ТаблицаДанных.ИдентификаторФинЗаписи <> &ИдентификаторНеиспользуемойФинЗаписи
	|	И ТаблицаДанных.Регистратор = &Документ
	|	И ТаблицаДанных.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И &УсловиеПоЗаявкеНаРасходованиеДенежныхСредств
	|	И ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	|				Справочник.НастройкиХозяйственныхОпераций КАК НастройкиХозяйственныхОпераций
	|			ГДЕ
	|				&НастройкаХозОперации = НастройкиХозяйственныхОпераций.Ссылка
	|				И НастройкиХозяйственныхОпераций.Приход = &Приход
	|				И НастройкиХозяйственныхОпераций.Расход = &Расход)";
	ШаблонТекстаЗапроса = СтрЗаменить(ШаблонТекстаЗапроса, "&Приход",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(Приход));
	ШаблонТекстаЗапроса = СтрЗаменить(ШаблонТекстаЗапроса, "&Расход",
		ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(Расход));
	Если ПолучитьФункциональнуюОпцию("КонтрольЛимитовПоДаннымБюджетирования2_5") Тогда
		ШаблонТекстаЗапроса = СтрЗаменить(ШаблонТекстаЗапроса, "&УсловиеПоЗаявкеНаРасходованиеДенежныхСредств", "ИСТИНА");
	Иначе
		ШаблонТекстаЗапроса = СтрЗаменить(ШаблонТекстаЗапроса, "&УсловиеПоЗаявкеНаРасходованиеДенежныхСредств", "ИСТИНА В
			|		(ВЫБРАТЬ ПЕРВЫЕ 1
			|			ИСТИНА
			|		ИЗ
			|			Документ.СписаниеБезналичныхДенежныхСредств КАК ДанныеДокумента
			|				ЛЕВОЕ СОЕДИНЕНИЕ Документ.СписаниеБезналичныхДенежныхСредств.РасшифровкаПлатежа КАК ТаблицаРасшифровкаПлатежа
			|				ПО ДанныеДокумента.Ссылка = ТаблицаРасшифровкаПлатежа.Ссылка
			|		ГДЕ
			|			ТаблицаДанных.Регистратор = ДанныеДокумента.Ссылка
			|			И (ТаблицаРасшифровкаПлатежа.ЗаявкаНаРасходованиеДенежныхСредств ЕСТЬ NULL
			|			ИЛИ ТаблицаРасшифровкаПлатежа.ЗаявкаНаРасходованиеДенежныхСредств В (НЕОПРЕДЕЛЕНО,
			|				ЗНАЧЕНИЕ(Документ.ЗаявкаНаРасходованиеДенежныхСредств.ПустаяСсылка),
			|				ЗНАЧЕНИЕ(Документ.ЗаявкаНаКомандировку.ПустаяСсылка)))
			|			И ДанныеДокумента.ЗаявкаНаРасходованиеДенежныхСредств В (НЕОПРЕДЕЛЕНО,
			|				ЗНАЧЕНИЕ(Документ.ЗаявкаНаРасходованиеДенежныхСредств.ПустаяСсылка),
			|				ЗНАЧЕНИЕ(Документ.ЗаявкаНаКомандировку.ПустаяСсылка)))"); // @query-part
	КонецЕсли;
	
	ПсевдонимИсточникаДанныхПоУмолчанию = ФинансовыйУчетПоДаннымБалансовыхРегистров.ПсевдонимИсточникаДанныхПоУмолчанию();
	ИмяРегистраРасхода = Метаданные.РегистрыНакопления.ДенежныеСредстваБезналичные.Имя;
	ИмяВидаДокумента = Метаданные.Документы.СписаниеБезналичныхДенежныхСредств.Имя;
	
	ОстаточныеРегистры = БюджетированиеПовтИсп.ОстаточныеФинансовыеРегистрыПоИмениВидаДокумента(ИмяВидаДокумента, Приход);
	
	Если Приход = Перечисления.ТипыДанныхУчета.ДенежныеСредства Тогда
		ПоляКорАналитики = СтрРазделить("НаправлениеДеятельности,ДенежныеСредства,ТипДенежныхСредств", ",");
	ИначеЕсли Приход = Перечисления.ТипыДанныхУчета.ДоходыРасходы Тогда
		ПоляКорАналитики = СтрРазделить(
			"Подразделение,НаправлениеДеятельности,СтатьяДоходовРасходов,АналитикаРасходов,АналитикаДоходов", ",");
	ИначеЕсли Приход = Перечисления.ТипыДанныхУчета.Контрагенты Тогда
		ПоляКорАналитики = СтрРазделить("НаправлениеДеятельности,Партнер,Контрагент,Договор,ОбъектРасчетов", ",");
	ИначеЕсли Приход = Перечисления.ТипыДанныхУчета.ПрочиеАктивыПассивы Тогда
		ПоляКорАналитики = СтрРазделить(
			"Подразделение,НаправлениеДеятельности,СтатьяАктивовПассивов,АналитикаАктивовПассивов", ",");
	Иначе
		ПоляКорАналитики = Новый Массив;
	КонецЕсли;

	ПараметрыОтраженияДвиженийРасход = РегистрыНакопления[ИмяРегистраРасхода].ПараметрыОтраженияДвиженийВФинансовомУчете();
	ПутиКДаннымПолейРасход = ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписанияПутейКДаннымПолейРегистра(
		ПараметрыОтраженияДвиженийРасход, "ТаблицаДанных"); // - Структура -
	СтруктураАналитикиРасход = ПараметрыОтраженияДвиженийРасход.СтруктураАналитики; // - Структура -
	Если СтруктураАналитикиРасход.Количество() = 0 Тогда
		СтруктураАналитикиРасход = ИсточникиДанныхПовтИсп.СтруктураАналитикиПоТипуДанныхУчета(
			ПараметрыОтраженияДвиженийРасход.ТипДанныхУчета); // - Структура -
	КонецЕсли;
	
	ТекстыЗапросов = Новый Массив;
	
	Для Каждого ОстаточныйРегистр Из ОстаточныеРегистры Цикл
		Если ОстаточныйРегистр.Ключ = ИмяРегистраРасхода Тогда
			Продолжить;
		КонецЕсли;
		ПараметрыОтраженияДвиженийПриход = ОстаточныйРегистр.Значение;
		ПутиКДаннымПолейПриход = ФинансовыйУчетПоДаннымБалансовыхРегистров.ОписанияПутейКДаннымПолейРегистра(
			ПараметрыОтраженияДвиженийПриход, "ТаблицаДанныхКор"); // - Структура -
		СтруктураАналитикиПриход = ПараметрыОтраженияДвиженийПриход.СтруктураАналитики; // - Структура -
		Если СтруктураАналитикиПриход.Количество() = 0 Тогда
			СтруктураАналитикиПриход = ИсточникиДанныхПовтИсп.СтруктураАналитикиПоТипуДанныхУчета(
				ПараметрыОтраженияДвиженийПриход.ТипДанныхУчета); // - Структура -
		КонецЕсли;
		
		// Источник
		ТекстЗапроса = СтрЗаменить(ШаблонТекстаЗапроса, "КорРегистрНакопления", "РегистрНакопления."
			+ ОстаточныйРегистр.Ключ);
		
		// Общие поля
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТаблицаДанных.Регистратор", ПутиКДаннымПолейРасход.Регистратор);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТаблицаДанныхКор.Регистратор", ПутиКДаннымПолейПриход.Регистратор);
		Если Приход = Перечисления.ТипыДанныхУчета.Контрагенты Тогда
			ПутьКДаннымНастройкаХозОперации = "(" + СтрЗаменить(ПутиКДаннымПолейРасход.ДопНастройкаХозОперации,
				ПсевдонимИсточникаДанныхПоУмолчанию, "ТаблицаДанных") + ")";
		Иначе
			ПутьКДаннымНастройкаХозОперации = "(" + СтрЗаменить(ПутиКДаннымПолейПриход.ДопНастройкаХозОперации,
				ПсевдонимИсточникаДанныхПоУмолчанию, "ТаблицаДанныхКор") + ")";
		КонецЕсли;
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&НастройкаХозОперации", ПутьКДаннымНастройкаХозОперации);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ХозяйственнаяОперация", ПутьКДаннымНастройкаХозОперации
			+ ".ХозяйственнаяОперация");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Организация", ПутиКДаннымПолейРасход.Организация);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Подразделение", ПутиКДаннымПолейРасход.Подразделение);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&НаправлениеДеятельности",
			ПутиКДаннымПолейРасход.НаправлениеДеятельности);
		
		// Аналитика денежных средств
		Для Каждого Аналитика Из СтруктураАналитикиРасход Цикл
			Если Аналитика.Ключ = "Подразделение" Или Аналитика.Ключ = "НаправлениеДеятельности" Тогда
				Если Приход <> Перечисления.ТипыДанныхУчета.ДенежныеСредства Тогда
					ИмяПоляАналитики = Аналитика.Ключ;
					ИмяПоляАналитики = ИсточникиДанныхПовтИсп.ИмяПоляПоТипуДанныхУчета(ИмяПоляАналитики, "",
						Расход).ИмяПоля;
					Если ИмяПоляАналитики <> Аналитика.Ключ Тогда
						ТекстЗапроса = СтрЗаменить(ТекстЗапроса, " КАК " + Аналитика.Ключ + ",", " КАК "
							+ ИмяПоляАналитики + ",");
					КонецЕсли;
				КонецЕсли;
				Продолжить;
			ИначеЕсли Аналитика.Значение.Свойство("Значение") Тогда
				ТекстАналитика = ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(
					Аналитика.Значение.Значение);
			Иначе
				ТекстАналитика = СтрЗаменить(
					ФинансовыйУчетПоДаннымБалансовыхРегистров.ПутьКДаннымСПсевдонимомИсточника(
					Аналитика.Значение.ПутьКДанным), ПсевдонимИсточникаДанныхПоУмолчанию, "ТаблицаДанных");
			КонецЕсли;
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&" + Аналитика.Ключ, ТекстАналитика);
		КонецЦикла;
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Валюта", ПутиКДаннымПолейРасход.Валюта);
		
		// Кор. аналитика
		ПоляКорАналитикиЗапроса = Новый Массив;
		Для Каждого Аналитика Из СтруктураАналитикиПриход Цикл
			Если ПоляКорАналитики.Найти(Аналитика.Ключ) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Если Аналитика.Ключ = "Подразделение" Или Аналитика.Ключ = "НаправлениеДеятельности" Тогда
				ТекстАналитика = ПутиКДаннымПолейПриход[Аналитика.Ключ];
			ИначеЕсли Аналитика.Значение.Свойство("Значение") Тогда
				ТекстАналитика = ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиЗначения(
					Аналитика.Значение.Значение);
			Иначе
				ТекстАналитика = СтрЗаменить(
					ФинансовыйУчетПоДаннымБалансовыхРегистров.ПутьКДаннымСПсевдонимомИсточника(
					Аналитика.Значение.ПутьКДанным), ПсевдонимИсточникаДанныхПоУмолчанию, "ТаблицаДанныхКор");
			КонецЕсли;
			Если ПараметрыОтраженияДвиженийРасход.ТипДанныхУчета = ПараметрыОтраженияДвиженийПриход.ТипДанныхУчета Тогда
				ПоляКорАналитикиЗапроса.Добавить(ТекстАналитика + " КАК " + Аналитика.Значение.ИмяКор);
			Иначе
				ИмяПоляАналитики = Аналитика.Ключ;
				ИмяПоляАналитики = ИсточникиДанныхПовтИсп.ИмяПоляПоТипуДанныхУчета(ИмяПоляАналитики, "", Приход).ИмяПоля;
				ПоляКорАналитикиЗапроса.Добавить(ТекстАналитика + " КАК " + ИмяПоляАналитики);
			КонецЕсли;
		КонецЦикла;
		Если ПутиКДаннымПолейПриход.Свойство("Валюта")
			И ПараметрыОтраженияДвиженийРасход.ТипДанныхУчета = ПараметрыОтраженияДвиженийПриход.ТипДанныхУчета Тогда
			ПоляКорАналитикиЗапроса.Добавить(ПутиКДаннымПолейПриход.Валюта + " КАК КорВалюта");
		КонецЕсли;
		СтрокаПоляКорАналитикиЗапроса = СтрСоединить(ПоляКорАналитикиЗапроса, "," + Символы.ПС + Символы.Таб);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПоляКорАналитики", СтрокаПоляКорАналитикиЗапроса);
		
		// Суммовые показатели
		Если ПараметрыОтраженияДвиженийПриход.РесурсыВал.Количество() = 0 Тогда
			ТекстСумма = ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиРесурса(
				ПараметрыОтраженияДвиженийРасход.РесурсыВал);
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Сумма", СтрЗаменить(ТекстСумма, ПсевдонимИсточникаДанныхПоУмолчанию
				+ ".", "ТаблицаДанных."));
		Иначе
			ТекстСумма = ФинансовыйУчетПоДаннымБалансовыхРегистров.СтрокаВыборкиРесурса(
				ПараметрыОтраженияДвиженийПриход.РесурсыВал);
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Сумма", СтрЗаменить(ТекстСумма, ПсевдонимИсточникаДанныхПоУмолчанию
				+ ".", "ТаблицаДанныхКор."));
		КонецЕсли;
		
		// Помещение во временную таблицу
		Если Не ПустаяСтрока(ИмяВременнойТаблицы) И ТекстыЗапросов.Количество() = 0 Тогда
		...
		Иначе
			...
			Если ПустаяСтрока(ИмяВременнойТаблицы) Тогда
				УсловиеПоРегистратору = "ИСТИНА В
				|			(ВЫБРАТЬ ПЕРВЫЕ 1
				|				ИСТИНА
				|			ИЗ
				|				РегистрНакопления.ЛимитыПоДаннымБюджетирования КАК ЛимитыПоДаннымБюджетирования
				|			ГДЕ
				|				ТаблицаДанных.Регистратор = ЛимитыПоДаннымБюджетирования.Регистратор)
				|И ИСТИНА В
				|			(ВЫБРАТЬ ПЕРВЫЕ 1
				|				ИСТИНА
				|			ИЗ
				|				Документ.СписаниеБезналичныхДенежныхСредств.РасшифровкаПлатежа КАК РасшифровкаПлатежа
				|			ГДЕ
				|				ТаблицаДанных.Регистратор = РасшифровкаПлатежа.Ссылка
				|				И РасшифровкаПлатежа.ЗаявкаНаРасходованиеДенежныхСредств В (НЕОПРЕДЕЛЕНО,
				|				ЗНАЧЕНИЕ(Документ.ЗаявкаНаРасходованиеДенежныхСредств.ПустаяСсылка),
				|				ЗНАЧЕНИЕ(Документ.ЗаявкаНаКомандировку.ПустаяСсылка)))"; // @query-part
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТаблицаДанных.Регистратор = &Документ", УсловиеПоРегистратору);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = '- заполнение реквизитов ""Сумма в валюте списания"" и ""Сумма НДС в валюте списания"" расшифровки платежа';
								|en = '- fill the ""Amount in write-off currency"" and ""VAT amount in write-off currency"" attributes of the payment details'"));
	...
	Приоритет.Процедура = "Документы.СписаниеБезналичныхДенежныхСредств.ОбработатьДокументыДляПереходаНаНовуюВерсию";
	Приоритет.Порядок = "До";
	
	//++ Локализация
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "Документы.СписаниеБезналичныхДенежныхСредств.ОбработатьДокументыДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "11.5.22.173";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("daade3ef-05b0-4e5d-aa91-0015a85601fa");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.СписаниеБезналичныхДенежныхСредств.ЗарегистрироватьДокументыКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	...
	ТекстОбработчика = НСтр("ru = 'Заполнение реквизитов ""ИНН плательщика"", ""КПП плательщика"" и ""Текст плательщика"" в документе ""Списание безналичных денежных средств""';
							|en = 'Заполнение реквизитов ""ИНН плательщика"", ""КПП плательщика"" и ""Текст плательщика"" в документе ""Списание безналичных денежных средств""'");
...
Процедура ЗарегистрироватьДокументыКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	// НАЧАЛО Николаев Павел 22.05.2026 Выполнение задач №000057727 : нереально долгое время монопольного обработчика, унесем во внешнюю обработку
	...
	// КОНЕЦ Николаев Павел 22.05.2026 Выполнение задач №000057727
...
Процедура ОбработатьДокументыДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Не удалось заполнить реквизиты документа ""Списание безналичных денежных средств"" по обработчику';
								|en = 'Не удалось заполнить реквизиты документа ""Списание безналичных денежных средств"" по обработчику'"));
