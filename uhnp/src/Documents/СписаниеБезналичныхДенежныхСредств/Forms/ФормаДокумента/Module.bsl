	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыдачаДенежныхСредствПодотчетнику")
		...
		Если Объект.РасшифровкаПлатежа.Итог("Сумма") <> Объект.СуммаДокумента Тогда
			...
			ТекстСообщения = НСтр("ru = 'Сумма по заявкам на оплату не равна сумме документа.';
									|en = 'The amount of the payment requests is not equal to the document amount.'");
...
Процедура БанковскийСчетПриИзмененииСервер(СтруктураРеквизитов, ПересчитыватьСуммы)
	...
	// УХ_Казначейство ++
	Если Справочники.Организации.ФилиалыСРасчетамиЧерезГоловнуюОрганизацию(СтруктураРеквизитов.Организация).Найти(Объект.Организация) <> Неопределено
		Тогда 
		СтруктураРеквизитов.Организация = Объект.Организация;
	КонецЕсли;	
	// УХ_Казначейство --
...
Процедура ЗаявкаНаРасходованиеДенежныхСредствПриИзмененииСервер()
	...
	// УХ_Казначейство ++
	// отключено по причине совместимости с механизмами УХ 
	// ДокументОбъект = РеквизитФормыВЗначение("Объект");
...
Процедура ПриИзмененииОплатыПоНесколькимЗаявкам()
	...
	Если Не Объект.ОплатаПоЗаявкам Тогда
		...
		#Область УХ_Встраивание
		Для Каждого Строка Из Объект.РасшифровкаПлатежа Цикл
			Строка.ДокументПланирования = Неопределено;
		КонецЦикла;
...
Процедура РасшифровкаПлатежаЗаявкаПриИзмененииСервер(ЗаявкаСсылка)
	...
	Если ТипЗнч(ЗаявкаСсылка) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств") Тогда
		...
		КонецЕсли;
		#Область УХ_Встраивание   
		Для каждого СтрокаТекущиеДанные Из Объект.РасшифровкаПлатежа Цикл
			Если НЕ ЗначениеЗаполнено(СтрокаТекущиеДанные.Партнер) Тогда
				Если Не ЗначениеЗаполнено(Партнер) Тогда
					ПолучитьПартнераПоУмолчаниюСервер(Объект.Контрагент, Партнер, Объект.ХозяйственнаяОперация);
				КонецЕсли;
				СтрокаТекущиеДанные.Партнер = Партнер;
			КонецЕсли;
		КонецЦикла; 	
		#КонецОбласти
	КонецЕсли;
...
Процедура РасшифровкаПлатежаОбъектРасчетовОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		#Область УХ_Встраивание 
     	ТекущиеДанныеПлатежа =  Элементы.РасшифровкаПлатежа.ТекущиеДанные;
		Если  ТекущиеДанныеПлатежа <> Неопределено 
			И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущиеДанныеПлатежа, "Договор") Тогда 
			Объект.Договор = ТекущиеДанныеПлатежа.Договор;
		КонецЕсли;	
...
Процедура ПодборПоОстаткамЗавершениеСервер()
	...
	Для каждого СтрокаТаблицы Из Объект.РасшифровкаПлатежа Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ЭлементСтруктурыЗадолженности) Тогда 
			СтрокаТаблицы.ЭлементСтруктурыЗадолженности = Перечисления.ЭлементыСтруктурыЗадолженности.ОсновнойДолг;
		КонецЕсли;	
		// УХ_Казначейство --
...
Процедура ОбработкаПодбораИзЗаявок(АдресЗаявокВХранилище)
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеВБюджет
		И ВидыНалоговСНастройкойСчетов.НайтиПоЗначению(Объект.ТипНалога) <> Неопределено
...
Процедура ПередВыгрузкойРеестраНаЗачислениеДенежныхСредств()
	...
	Если Не ЗначениеЗаполнено(Объект.ВидЗачисления) Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для реестра требуется заполнить вид зачисления';
								|en = 'Fill the transfer kind for the registry'");
...
Процедура ПриЧтенииСозданииНаСервереЛокализация()
	...
	ВстраиваниеУХСписаниеБезналичныхДенежныхСредств.УправлениеЭлементамиФормыПриЧтенииСозданииНаСервере(ЭтаФорма);
	// УХ_Казначейство --
...
&НаКлиенте
Процедура Подключаемый_ПриИзмененииДокПланирования(Элемент)
	ОбновитьИнформациюПоЗаявкам();
КонецПроцедуры   

&НаКлиенте
Процедура Подключаемый_НачалоВыбораДокументПланирования(Элемент, ДанныеВыбора, СтандартнаяОбработка)	
	УстановитьСвязиПараметровВыбораДокументаПланирования(Элемент.Имя);
КонецПроцедуры
...
Процедура Подключаемый_РасшифровкаПлатежаЭлементСтруктурыЗадолженностиПриИзменении(Элемент)
	...
	ТекущиеДанные.ТипПлатежаПоАренде = ПолучитьТипПлатежаПоАренде(ЭлементСтруктурыЗадолженности);	
...
&НаСервереБезКонтекста
Функция ПолучитьТипПлатежаПоАренде(ЭлементСтруктурыЗадолженности)

	Возврат ЗаявкиНаОперацииВстраивание.ПолучитьТипПлатежаПоАренде(ЭлементСтруктурыЗадолженности);	
...
Процедура ОбработкаПодбораИзПлатежныхПозиций(АдресЗаявокВХранилище)
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыПоЗарплатномуПроекту
		Или Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыНаЛицевыеСчета
		Или (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеВБюджет
				И (Объект.ТипНалога = ПредопределенноеЗначение("Справочник.ВидыНалоговВзносов.НДФЛ")
						 ИЛИ Объект.ТипНалога = ПредопределенноеЗначение("Справочник.ВидыНалоговВзносов.НДФЛСПревышения"))
				И Объект.НДФЛПоВедомостям) Тогда
		СтруктураПоиска.Вставить("Ведомость");
	КонецЕсли;
	//-- НЕ УТ
	...
	Для каждого СтрокаПлатежа Из ТаблицаПлатежей Цикл
		...
		Если Строки.Количество() И НЕ СтрокаПлатежа.СтрокаВыбрана Тогда

		   Для каждого СтрокаНайдена Из Строки Цикл
			   МассивУдаленияСтрок.Добавить(СтрокаНайдена);
		   КонецЦикла;
		   
		ИначеЕсли Строки.Количество() = 0 И СтрокаПлатежа.СтрокаВыбрана Тогда
			
			Строка = Объект.РасшифровкаПлатежа.Добавить();
			ЗаполнитьЗначенияСвойств(Строка, СтрокаПлатежа);
			Если ТипЗнч(СтрокаПлатежа.ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
				СтруктураРеквизитов = Новый Структура;
				СтруктураРеквизитов.Вставить("ОбъектРасчетов",	"ОбъектРасчетов");
				ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаПлатежа.ДоговорКонтрагента, СтруктураРеквизитов);
				Если ЗначениеЗаполнено(СтрокаПлатежа.ДоговорКонтрагента) Тогда
					Строка.ОбъектРасчетов = ЗначенияРеквизитов.ОбъектРасчетов;                                                                         	
				КонецЕсли; 
			ИначеЕсли  ТипЗнч(СтрокаПлатежа.ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыАренды") Тогда
				Строка.ДоговорАренды = СтрокаПлатежа.ДоговорКонтрагента;                                                                         	
			КонецЕсли;
			
			Если Строка.СуммаВзаиморасчетов = 0  Тогда
				Строка.СуммаВзаиморасчетов 		= СтрокаПлатежа.Сумма;
				Строка.ВалютаВзаиморасчетов 	= СтрокаПлатежа.Валюта;							
			КонецЕсли;
			
			ЭлементСтруктурыЗадолженностиПоУмолчанию = ДоговорыКонтрагентовВстраиваниеУХ.ПолучитьЭлементСтруктурыЗадолженностиПоДоговоруПоУмолчанию(Объект,СтрокаПлатежа.ДоговорКонтрагента); 
			ЗаполнитьЗначенияСвойств(Строка, ЭлементСтруктурыЗадолженностиПоУмолчанию);
	...
	Если МассивУдаленияСтрок.Количество() Тогда
		Для каждого СтрокаНайдена Из МассивУдаленияСтрок Цикл
			Объект.РасшифровкаПлатежа.Удалить(СтрокаНайдена);
		КонецЦикла;
	КонецЕсли;
...
&НаСервере
Процедура УстановитьСвязиПараметровВыбораДокументаПланирования(ИмяЭлемента)
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаДенежныхСредствПодотчетнику
		...
		Иначе
			СвязиПараметров.Добавить(Новый СвязьПараметраВыбора("Отбор.Контрагент", "Объект.ПодотчетноеЛицо"));  
		КонецЕсли;
	Иначе   
		СвязиПараметров.Добавить(Новый СвязьПараметраВыбора("Отбор.Контрагент", "Объект.Контрагент"));
	КонецЕсли;
	
	Элементы[ИмяЭлемента].СвязиПараметровВыбора = Новый ФиксированныйМассив(СвязиПараметров); 
