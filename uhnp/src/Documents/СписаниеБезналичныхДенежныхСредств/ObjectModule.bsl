...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипОснования = Тип("Структура")
		...
		ИспользоватьРеестрПлатежей = Константы.ИспользоватьРеестрПлатежей.Получить();
	
		Если ИспользоватьРеестрПлатежей И НЕ ДополнительныеСвойства.Свойство("ВызовИзРеестраПлатежей") Тогда
			ВызватьИсключение НСтр("ru = 'Для генерации платежных поручений необходимо использовать реестр платежей.'");
		КонецЕсли;
		#КонецОбласти
		...
		КонецЕсли;
		#Область УХ_Встраивание
		// Заполнение реквизита ДокументОснование.
		Если НЕ ЗначениеЗаполнено(ДокументОснование) Тогда
			Если ДанныеЗаполнения.Свойство("ЗаявкаНаРасходованиеДенежныхСредств") Тогда
				ДокументОснование = ДанныеЗаполнения.ЗаявкаНаРасходованиеДенежныхСредств;
			Иначе
				// Нет заявки в данных заполнения.
			КонецЕсли;
		Иначе
			// Заявка уже установлена.
		КонецЕсли;
		#КонецОбласти
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств") Тогда
		...
		#Область УХ_Внедрение
		ЗаявкиНаОперацииВстраивание.ЗаполнитьДокументПоЗаявкамНаРасходованиеДенежныхСредств(ЭтотОбъект,
		#КонецОбласти
			ДанныеЗаполнения,
			РасшифровкаПлатежа,
			Перечисления.ФормыОплаты.Безналичная
	...
	#Область УХ_Встраивание 
	ВстраиваниеУХСписаниеБезналичныхДенежныхСредств.ОбработкаЗаполнения(ЭтотОбъект, ДанныеЗаполнения, Основание); 
	#КонецОбласти   
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	#Область УХ_Встраивание
	ВстраиваниеУХСписаниеБезналичныхДенежныхСредств.ОбработкаПроверкиЗаполнения(ЭтотОбъект, Отказ, 
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	КонецЕсли;
	#Область УХ_Встраивание
	ВстраиваниеУХСписаниеБезналичныхДенежныхСредств.ПередЗаписью(ЭтотОбъект, Отказ, РежимЗаписи, РежимПроведения);
	#КонецОбласти
КонецПроцедуры
...
Процедура ПриЗаписи(Отказ)
	...
	#Область УХ_Внедрение
	ВстраиваниеУХСписаниеБезналичныхДенежныхСредств.ПриЗаписи(ЭтотОбъект, Отказ);
	#КонецОбласти
		...
		Текст =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не требуется вводить Списание безналичных ДС на основании документа %1, так как расчеты ведутся ""%2"".';
					|en = 'It is not required to enter ""Outgoing payment — Bank account"" based on the %1 document, as the AR/AP accounting option is ""%2"".'"),
				ДокументОснование,
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	КонецЕсли;
	#Область УХ_Встраивание
	ВстраиваниеУХСписаниеБезналичныхДенежныхСредств.ИнициализироватьДокумент(ЭтотОбъект, ДанныеЗаполнения);
	#КонецОбласти
КонецПроцедуры
...
Процедура ЗаполнитьПоАктуПремииКлиенту(Знач ДокументОснование, ДанныеЗаполнения)
	...
	ШаблонСообщения = НСтр("ru = 'Документ %1 находится в статусе ""%2"". Ввод на основании разрешен только в статусе ""Получен"".';
							|en = 'The %1 document is in the ""%2"" status. Generation is only allowed from reference documents in the ""Received"" status.'");
