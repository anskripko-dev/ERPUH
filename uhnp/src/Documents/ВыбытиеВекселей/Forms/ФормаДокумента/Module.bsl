...
Процедура ПрочитатьПараметрыВекселя()
	...
	Если Не Объект.ПредусмотренПроцентныйДоход Тогда
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ВерсииГрафиковРасчетов.СрезПервых(
		|				,
		|				Организация = &Организация
		|					И ПредметГрафика = &Вексель) КАК ВерсииГрафиковРасчетовСрезПервых
		|		ПО РасчетыСКонтрагентамиГрафики.ВерсияГрафика = ВерсииГрафиковРасчетовСрезПервых.Регистратор
		|			И РасчетыСКонтрагентамиГрафики.ПредметГрафика = ВерсииГрафиковРасчетовСрезПервых.ПредметГрафика
		|ГДЕ
		...
		ЭтоПогашение = (Объект.ВидОперации = Перечисления.ВидыОперацийВыбытиеВекселя.ПогашениеВекселя);
		
		Запрос.УстановитьПараметр("Организация", 	Объект.Организация);
		Запрос.УстановитьПараметр("Вексель", 		Объект.ФинансовыйИнструмент);
		Запрос.УстановитьПараметр("ВидДвижения", 	?(ЭтоПогашение, ВидДвиженияНакопления.Расход, ВидДвиженияНакопления.Приход));
...
Процедура РассчитатьТекущуюСтоимостьВекселя(Объект)
	...
	Объект.ПроцентыПриДосрочномПогашении = 0; 
	
	Если Объект.ПредусмотренПроцентныйДоход Тогда
		
		ДатаОкончания = Мин(ДатаВыбытия, Объект.ДатаОкончанияНачисленияПроцентов);
		
		ПроцентыВсе = ФинансоваяМатематикаКлиентСервер.СуммаПроцентовЗаПериод(
				Объект.ДатаНачалаНачисленияПроцентов,
				ДатаОкончания,
				Объект.Номинал,
				Объект.ПроцентнаяСтавка);
				
		Объект.ТекущаяСтоимость = Объект.Номинал + ПроцентыВсе;
		
		Если Не Объект.ФинансовыйИнструмент.Пустая() 
			И Не ЗначениеРеквизита(Объект.ФинансовыйИнструмент, "НачислениеПроцентовНаОтчетныеДаты") Тогда
		
			Объект.ПроцентыПриДосрочномПогашении = ПроцентыВсе;
		
		ИначеЕсли КонецДня(ДатаОкончания) <> КонецМесяца(ДатаОкончания) Тогда 
			
			//при выбытии ФИ надо списать проценты документом
			ДатаНачала = НачалоМесяца(ДатаОкончания);
		
			Объект.ПроцентыПриДосрочномПогашении 
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ПроцентыПриДосрочномПогашении.Видимость = Объект.ПредусмотренПроцентныйДоход;
...
Процедура УстановитьКонтрагентаОрганизации()
	...
	КонтрагентыОрганизации = ОрганизацииВызовСервераУХ.ПолучитьКонтрагентовПоОрганизации(Объект.Организация);
