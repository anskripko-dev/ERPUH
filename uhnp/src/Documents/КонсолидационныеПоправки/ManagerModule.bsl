...
Функция ПодготовитьПараметрыПроведения(ДополнительныеСвойства, Отказ) Экспорт
	...
	ТекстЗапроса.Добавить(ТекстЗапроса_втМетодДолевогоУчастия(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_втИнвестиции(НомераТаблиц));	
	ТекстЗапроса.Добавить(ТекстЗапроса_втТчИзменениеДолей(НомераТаблиц));	
	ТекстЗапроса.Добавить(ТекстЗапроса_втИзменениеДолей(НомераТаблиц));
...
Функция ТекстЗапроса_втМетодДолевогоУчастия(НомераТаблиц)
...
Функция ТекстЗапроса_втИнвестиции(НомераТаблиц)

	НомераТаблиц.Вставить("втИнвестиции", НомераТаблиц.Количество());
	
	Возврат
	"ВЫБРАТЬ
	|	&ВалютаОтчетности КАК ВалютаИнвестора,
...
Функция ТекстЗапроса_втТчИзменениеДолей(НомераТаблиц)
	...
	|	тчЭлиминация.Гудвил КАК Гудвилл,
...
Функция ТекстЗапроса_втИзменениеДолей(НомераТаблиц)
	...
	|	СчетБал.Счет КАК СчетБал,
	|	СчетБал.Субконто1 КАК СчетБалСубконто1,
	|	СчетБал.Субконто2 КАК СчетБалСубконто2,
	|	СчетБал.Субконто3 КАК СчетБалСубконто3,
	...
	|		ПО (ВЫБОР
	|				КОГДА тчЭлиминация.Гудвилл > 0
	|					ТОГДА СчетРезультатаПриобретения.Ссылка = ЗНАЧЕНИЕ(Справочник.ФиксированныеСчетаУчетаБД.Гудвилл)
	...
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ФиксированныеСчетаУчетаБД КАК СчетБал
	|		ПО (СчетБал.Ссылка = ЗНАЧЕНИЕ(Справочник.ФиксированныеСчетаУчетаБД.ВспомогательныйБалансовый))
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	КлючСтроки,
...
Функция ТекстЗапроса_ТаблицаПроводок(НомераТаблиц)
	...
	ТекстЗапроса.Добавить(ТекстЗапроса_РасчетГудвиллаИсключениеИнвестиций(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_РасчетГудвиллаЭлиминацияЧистыхАктивов(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_РасчетГудвиллаРасчетНДУ(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_РасчетГудвиллаОтражениеГудвилла(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_РасчетГудвиллаОтражениеБэдвилл(НомераТаблиц));
	
	ТекстЗапроса.Добавить(ТекстЗапроса_ОтражениеНДУ(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_ИзменениеДолиОтражениеНДУВКапитале(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_ИзменениеДолиОтражениеИнвестицииВКапитале(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_БалансоваяСтоимостьПриВыбытииДоли(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_ОбесценениеГудвилла(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_ДоляГруппыВФинансовомРезультатеАссоциата(НомераТаблиц));
	ТекстЗапроса.Добавить(ТекстЗапроса_ОбесценениеИнвестицийНаОтчетнуюДату(НомераТаблиц));
	
	Возврат СтрСоединить(ТекстЗапроса, " ОБЪЕДИНИТЬ ВСЕ ");

КонецФункции

Функция ТекстЗапроса_РасчетГудвиллаИсключениеИнвестиций(НомераТаблиц)
	...
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСобытийРегламента.РасчетГудвиллаИсключениеИнвестиций) КАК Событие,
	...
	|	втИзменениеДолей.СчетБал КАК СчетКт,
	|	втИзменениеДолей.СчетБалСубконто1 КАК СубконтоКт1,
	|	втИзменениеДолей.СчетБалСубконто2 КАК СубконтоКт2,
	|	втИзменениеДолей.СчетБалСубконто3 КАК СубконтоКт3,
	...
	|	втИзменениеДолей.СчетБал,
	|	втИзменениеДолей.СчетБалСубконто1,
	|	втИзменениеДолей.СчетБалСубконто2,
	|	втИзменениеДолей.СчетБалСубконто3,
...
Функция ТекстЗапроса_РасчетГудвиллаЭлиминацияЧистыхАктивов(НомераТаблиц)
	...
	|	NULL,
	|	втИзменениеДолей.Контрагент,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСобытийРегламента.РасчетГудвиллаЭлиминацияЧистыхАктивов),
	|	&ВалютаОтчетности,
	|	втИзменениеДолей.СчетБал,
	|	втИзменениеДолей.СчетБалСубконто1,
	|	втИзменениеДолей.СчетБалСубконто2,
...
Функция ТекстЗапроса_РасчетГудвиллаРасчетНДУ(НомераТаблиц)

	Возврат 
	"ВЫБРАТЬ
	|	втИзменениеДолей.ДатаИзмененияДоли КАК ДатаИзмененияДоли,
	|	втИзменениеДолей.ОбъектИнвестирования КАК ОбъектИнвестирования,
	|	NULL КАК Поле1,
	|	втИзменениеДолей.Контрагент КАК Контрагент,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСобытийРегламента.РасчетГудвиллаРасчетНДУ) КАК Поле2,
	|	&ВалютаОтчетности КАК ВалютаОтчетности,
	|	втИзменениеДолей.СчетБал КАК СчетБал,
	|	втИзменениеДолей.СчетБалСубконто1 КАК СчетБалСубконто1,
	|	втИзменениеДолей.СчетБалСубконто2 КАК СчетБалСубконто2,
	|	втИзменениеДолей.СчетБалСубконто3 КАК СчетБалСубконто3,
	|	СчетНДУ.Счет КАК Счет,
	|	СчетНДУ.Субконто1 КАК Субконто1,
	|	СчетНДУ.Субконто2 КАК Субконто2,
	|	СчетНДУ.Субконто3 КАК Субконто3,
	|	СУММА(втИзменениеДолей.НДУ) КАК НДУ,
	|	NULL КАК Поле3,
	|	NULL КАК Поле4
	|ИЗ
	|	втИзменениеДолей КАК втИзменениеДолей
...
Функция ТекстЗапроса_РасчетГудвиллаОтражениеГудвилла(НомераТаблиц)
	...
	|	NULL КАК Поле1,
	|	втИзменениеДолей.Контрагент КАК Контрагент,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСобытийРегламента.РасчетГудвилла) КАК Поле2,
	|	&ВалютаОтчетности КАК ВалютаОтчетности,
	|	втИзменениеДолей.СчетРезультатаПриобретения КАК СчетРезультатаПриобретения,
	|	втИзменениеДолей.СчетРезультатаПриобретенияСубконто1 КАК СчетРезультатаПриобретенияСубконто1,
	|	втИзменениеДолей.СчетРезультатаПриобретенияСубконто2 КАК СчетРезультатаПриобретенияСубконто2,
	|	втИзменениеДолей.СчетРезультатаПриобретенияСубконто3 КАК СчетРезультатаПриобретенияСубконто3,
	|	втИзменениеДолей.СчетБал КАК Счет,
	|	втИзменениеДолей.СчетБалСубконто1 КАК СчетБалСубконто1,
	|	втИзменениеДолей.СчетБалСубконто2 КАК СчетБалСубконто2,
	|	втИзменениеДолей.СчетБалСубконто3 КАК СчетБалСубконто3,
	|	втИзменениеДолей.Гудвилл КАК Гудвилл,
	|	NULL КАК Поле3,
	|	NULL КАК Поле4
	|ИЗ
	|	втИзменениеДолей КАК втИзменениеДолей
	...
	|	втИзменениеДолей.ТипОтношенияКГруппеНаКонец = 1
	|	И втИзменениеДолей.ТипОтношенияКГруппеНаНачало = 0
	|	И втИзменениеДолей.Гудвилл > 0";

КонецФункции

Функция ТекстЗапроса_РасчетГудвиллаОтражениеБэдвилл(НомераТаблиц)
...
Функция ТекстЗапроса_ОтражениеНДУ(НомераТаблиц)
...
Функция ТекстЗапроса_ИзменениеДолиОтражениеНДУВКапитале(НомераТаблиц)
...
Функция ТекстЗапроса_ИзменениеДолиОтражениеИнвестицииВКапитале(НомераТаблиц)
...
Функция ТекстЗапроса_БалансоваяСтоимостьПриВыбытииДоли(НомераТаблиц)
...
Функция ТекстЗапроса_ОбесценениеГудвилла(НомераТаблиц)
	...
	|	NULL,
	|	втИзменениеДолей.Контрагент,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСобытийРегламента.ОбесценениеГудвилла),
	|	&ВалютаОтчетности,
	|	СчетОбесцененияГудвилла.Счет,
	|	СчетОбесцененияГудвилла.Субконто1,
	|	СчетОбесцененияГудвилла.Субконто2,
	|	СчетОбесцененияГудвилла.Субконто3,
	|	СчетГудвилла.Счет,
	|	СчетГудвилла.Субконто1,
	|	СчетГудвилла.Субконто2,
	|	СчетГудвилла.Субконто3,
	|	СУММА(втИзменениеДолей.ОбесценениеГудвилаИнвестицииПоМДУ),
	|	NULL,
	|	NULL
	|ИЗ
	|	втИзменениеДолей КАК втИзменениеДолей
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ФиксированныеСчетаУчетаБД КАК СчетОбесцененияГудвилла
	|		ПО (СчетОбесцененияГудвилла.Ссылка = ЗНАЧЕНИЕ(Справочник.ФиксированныеСчетаУчетаБД.УбытокОтОбесцененияГудвилла))
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ФиксированныеСчетаУчетаБД КАК СчетГудвилла
	|		ПО (СчетГудвилла.Ссылка = ЗНАЧЕНИЕ(Справочник.ФиксированныеСчетаУчетаБД.Гудвилл))
	|ГДЕ
	|	втИзменениеДолей.ТипОтношенияКГруппеНаКонец = 1
...
Функция ТекстЗапроса_ДоляГруппыВФинансовомРезультатеАссоциата(НомераТаблиц)
...
Функция ТекстЗапроса_ОбесценениеИнвестицийНаОтчетнуюДату(НомераТаблиц)
...
Функция ЗаполнитьПоМСФО(ДокументОбъект, ДанныеЗаполнения) Экспорт
	...
	ЗагрузитьДанныеДляРасчетаОбесцененияГудвилла(ДокументОбъект, ТаблицаПериметр, КонсолидирующаяОрганизация);
...
Процедура ЗагрузитьДанныеДляРасчетаОбесцененияГудвилла(ДокументОбъект, ТаблицаПериметр, КонсолидирующаяОрганизация)
	
	РеквизитыУП = РеквизитыУП(ДокументОбъект);
	
   	ВидыИтога = МСФОВызовСервераУХ.ПолучитьСтруктуруСоЗначениямиПеречисления("ВидыБухгалтерскихИтогов");
	СчетГудвилла = Документы.УчетнаяПолитикаМСФО.ФиксированныйСчет(
		Справочники.ФиксированныеСчетаУчетаБД.Гудвилл, РеквизитыУП.УчетнаяПолитика);
	...
	ОтборПоказателей = Новый Соответствие;
	ОтборПоказателей.Вставить("СчетБД",				СчетГудвилла);
	ОтборПоказателей.Вставить("ВидИтогаПоСчету",	МасВидИтогаПоСчету);	
	...
	Для каждого СтрокаДоля Из ДокументОбъект.ИзменениеДолей Цикл
		Если СтрокаДоля.ДатаИзмененияДоли = ДокументОбъект.ПериодОтчета.ДатаОкончания Тогда
			ДанныеГудвилла = ТаблицаПриемник.Найти(СтрокаДоля.ОбъектИнвестирования, "ОбъектИнвестирования");
			Если ДанныеГудвилла <> Неопределено Тогда
				СтрокаДоля.ГудвилНаОтчетнуюДату = ДанныеГудвилла.ГудвилНаОтчетнуюДату;
				СтрокаДоля.ЧистыеАктивыНаОтчетнуюДату = ДанныеГудвилла.ЧистыеАктивыНаОтчетнуюДату;
			КонецЕсли;
		КонецЕсли;	
	КонецЦикла;
...
Функция ПолучитьТаблицуОборотовПрямыеИнвестиции(Сценарий, ПериодОтчета, ТаблицаПериметр, ВалютаПредставленияГруппы)
	...
	|	ВЫБОР
	|		КОГДА &ВалютаПредставленияГруппы = &ВалютаПредставленияГруппы
	|			ТОГДА 1
	...
	|		ПО (&ВалютаПредставленияГруппы = втКурсыВалют.Валюта)
...
Функция РеквизитыУП(ДокументОбъект)
	...
	ИменаУП = "УчетнаяПолитика, ФормироватьПроводкиМСФО, ПланСчетовМСФО, 
			|ФункциональнаяВалюта, ВалютаПредоставленияОтчетности, ВидОтчетаОСВ";
	ПутиУП = МСФОВызовСервераУХ.ПутиРеквизитовУП(ИменаУП);
	ТабУП = МСФОВызовСервераУХ.ЗначенияУП(ПутиУП, Организация, ДокументОбъект.Дата, ДокументОбъект.Сценарий);
