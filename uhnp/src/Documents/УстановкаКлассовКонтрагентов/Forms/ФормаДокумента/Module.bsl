...
&НаКлиенте
Процедура ОценкиПроцентАвансаВручнуюПриИзменении(Элемент)

	СтрокаОценки = Элементы.Оценки.ТекущиеДанные;
	Если НЕ СтрокаОценки.ПроцентАвансаВручную Тогда
		РекомендуемыеУсловия = УправлениеОбязательствамиУХВызовСервера.ПолучитьРекомендуемыеУсловия(СтрокаОценки.СтепеньРиска, СтрокаОценки.СтепеньДоходности, Объект.РольКонтрагента);
		СтрокаОценки.ПроцентАванса   = РекомендуемыеУсловия.ПроцентАванса;
	КонецЕсли;

КонецПроцедуры 

&НаКлиенте
Процедура ОценкиСрокОтсрочкиВручнуюПриИзменении(Элемент)
	
	СтрокаОценки = Элементы.Оценки.ТекущиеДанные;
	Если НЕ СтрокаОценки.СрокОтсрочкиВручную Тогда
		РекомендуемыеУсловия = УправлениеОбязательствамиУХВызовСервера.ПолучитьРекомендуемыеУсловия(СтрокаОценки.СтепеньРиска, СтрокаОценки.СтепеньДоходности, Объект.РольКонтрагента);
		СтрокаОценки.СрокОтсрочки   = РекомендуемыеУсловия.СрокОтсрочки;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ОценкиЗадолженностьВручнуюПриИзменении(Элемент)
	
	СтрокаОценки = Элементы.Оценки.ТекущиеДанные;
	Если НЕ СтрокаОценки.ЗадолженностьВручную Тогда
		РекомендуемыеУсловия = УправлениеОбязательствамиУХВызовСервера.ПолучитьРекомендуемыеУсловия(СтрокаОценки.СтепеньРиска, СтрокаОценки.СтепеньДоходности, Объект.РольКонтрагента);
		СтрокаОценки.Задолженность   = РекомендуемыеУсловия.ДопустимыйУровеньЗадолженности;
	КонецЕсли; 
	
КонецПроцедуры

&НаКлиенте
Процедура ОценкиПроцентПросроченнойЗадолженностиВручнуюПриИзменении(Элемент)

	СтрокаОценки = Элементы.Оценки.ТекущиеДанные;
	Если НЕ СтрокаОценки.ПроцентПросроченнойЗадолженностиВручную Тогда
		РекомендуемыеУсловия = УправлениеОбязательствамиУХВызовСервера.ПолучитьРекомендуемыеУсловия(СтрокаОценки.СтепеньРиска, СтрокаОценки.СтепеньДоходности, Объект.РольКонтрагента);
		СтрокаОценки.ПроцентПросроченнойЗадолженности  = РекомендуемыеУсловия.ПроцентПросроченнойЗадолженности;
	КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ОценкиГруппаПросроченнойЗадолженности.Видимость 	= Не ЭтоБанк;
...
Процедура УстановитьРекомендуемыеУсловия(Строка, РольКонтрагента)
	...
	Если Не Строка.ПроцентПросроченнойЗадолженностиВручную Тогда
		Строка.ПроцентПросроченнойЗадолженности = РекомендуемыеУсловия.ПроцентПросроченнойЗадолженности;
	КонецЕсли;
