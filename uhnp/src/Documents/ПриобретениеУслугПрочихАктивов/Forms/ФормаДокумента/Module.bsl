...
Процедура РасходыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	#Область УХ_Встраивание
	ТекДанные = Элементы.Расходы.ТекущиеДанные;
	Если Поле.Имя = "РасходыУХ_СтатьиИАналитики" Тогда
		...
		ОткрытьФорму("Документ.ПриобретениеУслугПрочихАктивов.Форма.УХ_ФормаСтатейИАналитик",СтруктураАналитик,ЭтаФорма,,,,,);
	КонецЕсли;
	#КонецОбласти
	Возврат;
...
Процедура УХ_СоздатьРеквизитыАналитик()
	...
	УХ_СтатьиИАналитики = Новый РеквизитФормы("УХ_СтатьиИАналитики",Новый ОписаниеТипов("Строка"),"Объект.Расходы","Статьи и аналитики");
	Реквизиты = Новый Массив;
	Реквизиты.Добавить(УХ_СтатьиИАналитики);
	ИзменитьРеквизиты(Реквизиты);
	РасходыУХ_СтатьиИАналитики = Элементы.Добавить("РасходыУХ_СтатьиИАналитики",ТИП("ПолеФормы"),Элементы.Расходы);
	РасходыУХ_СтатьиИАналитики.ПутьКДанным = "Объект.Расходы.УХ_СтатьиИАналитики";
	РасходыУХ_СтатьиИАналитики.Вид = ВидПоляФормы.ПолеНадписи; 
	Элементы.Переместить(РасходыУХ_СтатьиИАналитики,Элементы.Расходы,Элементы.РасходыСуммаНДС);
...
Процедура  УХ_УстановитьТекстСтатейИАналитик()
	...
	Если ПланированиеПотребностей.ИспользуемыеВидыБюджетовЗакупок(Объект.Дата) = Неопределено Тогда
		Элементы.РасходыУХ_СтатьиИАналитики.Видимость = Ложь; 
	КонецЕсли;
	Для Каждого ТекДанные ИЗ Объект.Расходы Цикл
