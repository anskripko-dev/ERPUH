...
Процедура УстановитьПредставлениеДокументаОснования()
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		...
		КомандаИзменить = Новый ФорматированнаяСтрока(НСтр("ru = '<Изменить>';
															|en = '<Change>'"),,,, "ВыбратьДокумент");
		ПредставлениеДокументаОснования = Новый ФорматированнаяСтрока(ОписаниеДокументаОснования, "  ", КомандаИзменить);
	Иначе
		ПредставлениеДокументаОснования = Новый ФорматированнаяСтрока(НСтр("ru = 'Выберите документ-основание';
																			|en = 'Select a base document'"),, ЦветаСтиля.ЦветОсобогоТекста,, "ВыбратьДокумент");
	КонецЕсли;
...
Процедура ОбработкаВыбораДокументаОснованияКлиент(ВыбранноеЗначение)
	...
	Если ЕстьКорректировки
		...
		ТекстВопроса = НСтр("ru = 'При изменении документа-основания текущая таблица с данными по корректировке будет перезаполнена по выбранному основанию с учетом предыдущих корректировок. Продолжить?';
							|en = 'If you change the base document, the current table with adjustment data will be refilled by the selected base document, considering the previous adjustments. Continue?'");
