...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЭтоИмпортИзЕАЭС Тогда
		МассивНепроверяемыхРеквизитов.Добавить("СтранаЭкспортер");
		МассивНепроверяемыхРеквизитов.Добавить("Грузоотправитель");
		МассивНепроверяемыхРеквизитов.Добавить("ТипТранспорта");
		МассивНепроверяемыхРеквизитов.Добавить("Товары.НомерПартии");
	КонецЕсли;
	
	Если Не ЭтоПредварительноеУведомление Тогда
		МассивНепроверяемыхРеквизитов.Добавить("ТерриториальноеУправление");
	КонецЕсли;
	
	Если ЭтоПредварительноеУведомление Тогда
		МассивНепроверяемыхРеквизитов.Добавить("ДатаПолучения");
	КонецЕсли;
	
	Если Не ЭтоПредварительноеУведомление
		И Не ЭтоИмпортИзЕАЭС Тогда
	...
	Если ЭтоИмпортИзЕАЭС Или ЭтоПредварительноеУведомление Тогда
		
		Для Каждого СтрокаТовары Из Товары Цикл
			Если Не ЗначениеЗаполнено(СтрокаТовары.НомерПартии) Или Не ЗначениеЗаполнено(СтрокаТовары.СрокГодности) Тогда
	...
	Если ЭтоИмпортИзЕАЭС
		Или ЭтоПредварительноеУведомление Тогда
		
		МассивНепроверяемыхРеквизитов.Добавить("Товары.КоличествоСАТУРН");
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Если Не ЗначениеЗаполнено(ОбъектКопирования.Операция) Тогда
		Операция = Перечисления.ВидыОперацийСАТУРН.ИмпортПродукции;
	КонецЕсли;
...
Процедура ЗаполнитьОбъектПоСтатистике()
	...
	ДанныеСтатистики = ЗаполнениеОбъектовПоСтатистикеСАТУРН.ДанныеЗаполненияИмпортПродукцииСАТУРН(ОрганизацияСАТУРН, Операция);
...
Функция ДанныеИмпортныхПартий(МассивИмпортныхПартий)
	...
	|ГДЕ
	|	СоответствиеНоменклатуры.ПАТ В
	|		(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|			ПАТ
	|		ИЗ
	|			Справочник.ИмпортируемаяПартияСАТУРН
	|		ГДЕ
	...
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСАТУРН.ИмпортПродукции) КАК Операция
	|ИЗ
	...
	|	ИмпортируемаяПартияСАТУРНТовары.ПАТ                    КАК ПАТ,
	|	ИмпортируемаяПартияСАТУРНТовары.КоличествоВУпаковкеСАТУРН КАК КоличествоВУпаковкеСАТУРН,
	|	ИмпортируемаяПартияСАТУРНТовары.КоличествоУпаковок * ИмпортируемаяПартияСАТУРНТовары.КоличествоВУпаковкеСАТУРН КАК
	|		КоличествоСАТУРН,
	|	ИмпортируемаяПартияСАТУРНТовары.ТипИзмеряемойВеличиныСАТУРН КАК ТипИзмеряемойВеличиныСАТУРН,
	|	ИмпортируемаяПартияСАТУРНТовары.ИдентификаторСтроки    КАК Идентификатор,
	|	СоответствиеНоменклатуры.Номенклатура                  КАК Номенклатура,
	|	СоответствиеНоменклатуры.Характеристика                КАК Характеристика
	|ИЗ
	|	Справочник.ИмпортируемаяПартияСАТУРН КАК ИмпортируемаяПартияСАТУРН
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ИмпортируемаяПартияСАТУРН.Товары КАК ИмпортируемаяПартияСАТУРНТовары
	|			ЛЕВОЕ СОЕДИНЕНИЕ СоответствиеНоменклатуры КАК СоответствиеНоменклатуры
	|				ПО ИмпортируемаяПартияСАТУРНТовары.ПАТ = СоответствиеНоменклатуры.ПАТ
	|			ПО ИмпортируемаяПартияСАТУРН.Ссылка = ИмпортируемаяПартияСАТУРНТовары.Ссылка
	|ГДЕ
	|	ИмпортируемаяПартияСАТУРН.Ссылка В (&МассивСсылок)";
