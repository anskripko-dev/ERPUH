...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		// Проверка заполнения табличной частии Движения операции.
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("Предназначение", ВидБюджета) Тогда
		ИначеЕсли ДанныеЗаполнения.Свойство("ВидБюджета") Тогда
			ВидБюджета = ПланыВидовХарактеристик.ВидыБюджетов.Предназначение(ДанныеЗаполнения.ВидБюджета);
		КонецЕсли;
		...
		Если ДанныеЗаполнения.Свойство("ВариантНастройки") Тогда
			Документы.ОперативныйПлан.ЗаполнитьПоВидуОперативногоПлана(ЭтотОбъект, ДанныеЗаполнения.ВариантНастройки);
		КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Движения.ОперацииБюджетов.ДополнительныеСвойства.Вставить("ФиксироватьЛимиты");
...
Процедура ПроверитьЗаполнениеХарактеристик(Отказ)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ТекстСообщения = НСтр("ru = 'Для номенклатуры %1 не заполнена характеристика. Запись отменена.'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Строка(ВыборкаДетальныеЗаписи.Номенклатура));
