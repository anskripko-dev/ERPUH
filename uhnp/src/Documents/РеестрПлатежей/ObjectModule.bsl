...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Для Каждого ТекПлатеж Из ЗаявкиНаОплату Цикл
		...
		НеПроверятьЗаписьВРегистрЗаявкиВРеестрахПлатежей = ДополнительныеСвойства.Свойство("НеПроверятьЗаписьВРегистрЗаявкиВРеестрахПлатежей");
		
		Если НеПроверятьЗаписьВРегистрЗаявкиВРеестрахПлатежей Тогда
			ЗаписыватьВРеестрПлатежей = Истина;
		Иначе     
			ЗаписыватьВРеестрПлатежей = ЗаписыватьВРеестрПлатежей(ТекПлатеж.ЗаявкаНаОплату, ТекПлатеж.ИдентификаторПозиции);
		КонецЕсли;
		
		Если ЗаписыватьВРеестрПлатежей Тогда
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЗаявкиВРеестрахПлатежей.ЗаявкаНаОперацию,
	|	ЗаявкиВРеестрахПлатежей.ИдентификаторПозиции,
	...
	Запрос.УстановитьПараметр("РеестрПлатежей", Ссылка);
	...
	Пока Выборка.Следующий() Цикл
		...
		ПлатежныеПозиции.ВернутьПозициюНаИсполнение(Выборка.ИдентификаторПозиции, , , Истина);
...
Процедура ПроверитьПравилаЗапретаОпераций(Отказ) Экспорт
	...
	|			И (РеестрПлатежейЗаявкиНаОплату.Исключена = ЛОЖЬ)
	...
	Пока ВыборкаПоЗапретамПлатежей.Следующий() Цикл
		...
		Если СтрокаШаблона <> "" тогда
			...
			СтрокаОрганизация 		= ?(ЗначениеЗаполнено(ВыборкаПоЗапретамПлатежей.Организация), 
									Строка(ВыборкаПоЗапретамПлатежей.Организация), НСтр("ru = 'Все организации'"));
			СтрокаСчетОрганизации 	= ?(ЗначениеЗаполнено(ВыборкаПоЗапретамПлатежей.СчетОрганизации), 
									Строка(ВыборкаПоЗапретамПлатежей.СчетОрганизации), НСтр("ru = 'Все счета'"));
			СтрокаКонтрагент 		= ?(ЗначениеЗаполнено(ВыборкаПоЗапретамПлатежей.Контрагент), 
									Строка(ВыборкаПоЗапретамПлатежей.Контрагент), НСтр("ru = 'Все контрагенты'"));
			СтрокаДоговорКонтрагента = ?(ЗначениеЗаполнено(ВыборкаПоЗапретамПлатежей.ДоговорКонтрагента), 
									Строка(ВыборкаПоЗапретамПлатежей.ДоговорКонтрагента), НСтр("ru = 'Все договоры'"));
			СтрокаСДДС 				= ?(ЗначениеЗаполнено(ВыборкаПоЗапретамПлатежей.СтатьяДвиженияДенежныхСредств),
									Строка(ВыборкаПоЗапретамПлатежей.СтатьяДвиженияДенежныхСредств), НСтр("ru = 'Все СДДС'"));
...
Функция ЗаписыватьВРеестрПлатежей(ЗаявкаНаОплату, ИдентификаторПозиции)
	...
	|ГДЕ
	|	СостоянияИсполненияДокументовПланирования.Период > &Период
	|	И СостоянияИсполненияДокументовПланирования.ДокументПланирования = &ЗаявкаНаОплату
	|	И СостоянияИсполненияДокументовПланирования.ИдентификаторПозиции = &ИдентификаторПозиции
	|	И СостоянияИсполненияДокументовПланирования.СостояниеИсполнения = &СостояниеИсполнения
	|	И СостоянияИсполненияДокументовПланирования.ДокументИзменившийСостояние = &ДокументИзменившийСостояние";
	
	Запрос.УстановитьПараметр("ЗаявкаНаОплату", ЗаявкаНаОплату);
	...
	Запрос.УстановитьПараметр("СостояниеИсполнения", Перечисления.СостоянияИсполненияЗаявки.НеОбработана);
