...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ЗакупочнаяПроцедура) Тогда
		...
		Сообщение.Текст = НСтр(
			"ru='Форму выбора контрагентов квалифицированных по лоту"
			+ " можно открыть только с указанием закупочной процедуры!'");
		Сообщение.Сообщить();
	КонецЕсли;
	Список.Параметры.УстановитьЗначениеПараметра("ДатаСреза", ДатаСреза);
	Список.Параметры.УстановитьЗначениеПараметра(
		"ДопустимыеРазмеры",
		Перечисления.РазмерыКонтрагентовДляЕИС.ПолучитьРазмерыСМСП());
КонецПроцедуры
