...
Функция ПолучитьПредставлениеДействия(Знач НастройкиДействия) Экспорт
	...
	Если ЗначениеЗаполнено(ВидыБюджетов) Тогда
		...
		Если ВидыБюджетов.Колонки.Найти("Действие") <> Неопределено И ЗначениеЗаполнено(ВидыБюджетов[0].Действие) Тогда
		
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон,
					НСтр("ru = 'Действие';
						|en = 'Action'"),
					ВидыБюджетов[0].Действие);
			Результат = Результат + ?(ЗначениеЗаполнено(Результат), ", ", "") + Текст;
...
Функция РеквизитыПоТипуДействия(Форма, ТипДействия, МодельБюджетирования) Экспорт
	...
	Если ТипДействия = Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.ВводБюджетов
		Или ТипДействия = Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.КорректировкаИсправлениеБюджетов Тогда
