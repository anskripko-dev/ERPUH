...
Функция УникальныйИдентификатор(Значение) Экспорт
	...
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Неизвестный тип события документа ЭДО: %1';
										|en = 'Unknown EDI document event type: %1'"), Значение);
	КонецЕсли;
...
Функция ЗначениеПоИдентификатору(Идентификатор) Экспорт
	...
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Неизвестный тип события документа ЭДО: %1';
										|en = 'Unknown EDI document event type: %1'"), Идентификатор);
	КонецЕсли;
