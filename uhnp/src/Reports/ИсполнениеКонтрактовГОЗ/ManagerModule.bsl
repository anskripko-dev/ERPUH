	...
	Для каждого НаправлениеДеятельности Из НаправленияДеятельности Цикл
		...
		Если ОшибкиПоКонтракту.Количество() > 0 Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = 'При заполении данных на Стр. %1 (контракт %2) обнаружены ошибки.
											...
											|en = 'Errors occurred while filling data on page %1 (contract %2).
											|Operations are recorded in the accounting by the indicators that are not supposed to be filled in the report.
											|To analyze, drill down the data by the report line.'"),
