...
Процедура ДиаграммаСмежныхЭтапов(Команда)
	...
	Если ЗначениеЗаполнено(ЭтапПроизводства) Тогда
		...
		Если ТипЗнч(ЭтапПроизводства) = Тип ("ДокументСсылка.ЭтапПроизводства2_2") Тогда
			ПараметрыФормы.Вставить("ПараметрКоманды", ЭтапПроизводства);
		Иначе
			ПараметрыФормы.Вставить("Этап", ЭтапПроизводства);
			ПараметрыФормы.Вставить("КлючПартия", КлючПартия);
...
Процедура УстановитьЗаголовокФормы()
	...
	Если ЗначениеЗаполнено(ЭтапПроизводства) Тогда
		...
		Если ТипЗнч(ЭтапПроизводства) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
			ПредставлениеЭтапа = Документы.ЭтапПроизводства2_2.ПредставлениеЭтапа(ЭтапПроизводства);
			
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Диагностика графика этапа ""%1"", %2';
					|en = 'The ""%1"" stage resource chart, %2'"),
				ПредставлениеЭтапа);
		Иначе
			ПредставлениеЭтапа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭтапПроизводства, "Наименование");
			ПредставлениеСпецификации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭтапПроизводства, "Владелец.Наименование");
			ПредставлениеЗаказа = Документы.ЗаказНаПроизводство2_2.ПредставлениеЗаказа(ЗаказНаПроизводство, НСтр("ru = 'по заказу';
																												|en = 'against order'"));
			
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Диагностика графика этапа ""%1: %2"" %3';
					|en = 'The ""%1"" stage resource chart: %2 %3'"),
				ПредставлениеСпецификации,
				ПредставлениеЭтапа,
				ПредставлениеЗаказа);
		КонецЕсли;
...
Процедура ПоказатьСкрытьГрафикНеАктуален()
	...
	Документ = ?(ТипЗнч(ЭтапПроизводства) = Тип("ДокументСсылка.ЭтапПроизводства2_2"), ЭтапПроизводства, ЗаказНаПроизводство);
	
	ДинамическаяСтруктура = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Документ, "ДинамическаяСтруктура");
	
	Если ДинамическаяСтруктура Тогда
		ГрафикАктуален = Истина; // Для динамического этапа пересчет недоступен, не нужно читать статус графика
	Иначе
		ГрафикАктуален = РегистрыСведений.ЗаданияКРасчетуГрафикаПроизводства.ГрафикЭтапаАктуален(ЭтапПроизводства);
	КонецЕсли;
	
	Элементы.ГруппаГрафикНеАктуален.Видимость = НЕ ДинамическаяСтруктура
		И НЕ ГрафикАктуален
...
Процедура ОткрытьДиагностикуСмежногоЭтапа(Расшифровка)
	...
	Если ТипЗнч(Расшифровка.ЭтапПроизводства) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
		ПараметрыФормы.Вставить("ПараметрКоманды", Расшифровка.ЭтапПроизводства);
	Иначе
		ПараметрыФормы.Вставить("Этап", Расшифровка.ЭтапПроизводства);
		ПараметрыФормы.Вставить("КлючПартия", Расшифровка.КлючПартия);
		ПараметрыФормы.Вставить("ЗаказНаПроизводство", ЗаказНаПроизводство);
	КонецЕсли;
