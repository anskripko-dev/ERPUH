	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ИсточникДанных) Тогда
			ТекстСообщения = СтрЗаменить(НСтр("ru = 'В заполнении показателя применен неизвестный источник данных. Возможно,
				...
				|информационной базы еще раз.';
				|en = 'An unknown data source is used to fill the indicator. The form
				|might have been filled in previous application versions. It is recommended that you fill the form according to the
				|infobase data again.'"), Символы.ПС, " ");
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;
