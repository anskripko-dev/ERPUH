...
Функция ЗаполнитьРеглОтчетЕНВДФормаОтчета2018Кв4(ИмяФормыОтчета, Организация, ДатаНачала, ДатаОкончания, РазделыОтчета, ПараметрыАвтоФормирования)
	...
	Если НЕ ИП Тогда
		...
		Если НЕ ЗначениеЗаполнено(КПП) Тогда
			...
			|en = 'Декларация по ЕНВД (Организация: %1, КПП: %2, код ИФНС: %3) не сформирована, т. к. не указан КПП.'",
		...
		ИначеЕсли НЕ СтрДлина(КПП) = 9 Тогда
			...
			|en = 'Декларация по ЕНВД (Организация: %1, КПП: %2, код ИФНС: %3) не сформирована, т. к. КПП указан некорректно.'",
			СокрЛП(Организация.Наименование), СокрЛП(КПП), СокрЛП(КодНО))));
			СтатусПроверки = Ложь;
	...
	Если НЕ ЗначениеЗаполнено(КодНО) Тогда
		...
		|en = 'Декларация по ЕНВД (Организация: %1, КПП: %2, код ИФНС: %3) не сформирована, т. к. не указан код налогового органа.'",
		СокрЛП(Организация.Наименование), СокрЛП(КПП), СокрЛП(КодНО))));
	...
	ИначеЕсли НЕ (СтрДлина(КодНО) = 4 И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КодНО)) Тогда
		...
		|en = 'Декларация по ЕНВД (Организация: %1, КПП: %2, код ИФНС: %3) не сформирована, т. к. код налогового органа указан некорректно.'",
		СокрЛП(Организация.Наименование), СокрЛП(КПП), СокрЛП(КодНО))));
	...
	Иначе
		...
		|en = 'Декларация по ЕНВД (Организация: %1, КПП: %2, код ИФНС: %3) не сформирована, т. к. не найдена соответствующая запись в справочнике ""Регистрации в налоговом органе"".'",
		СокрЛП(Организация.Наименование), СокрЛП(КПП), СокрЛП(КодНО))));
