...
Процедура ПередЗагрузкойНастроекВКомпоновщик(Контекст, КлючСхемы, КлючВарианта,
									Настройки, ПользовательскиеНастройки) Экспорт

	Если Настройки = Неопределено Тогда
		Возврат;//заполнение настроек только при открытии или изменении варианта
	КонецЕсли;								
									
	ЭлементОтбора = ТиповыеОтчетыУХ.ПолучитьЭлементОтбора("Роль", Настройки, ПользовательскиеНастройки);
	Если КлючВарианта = "ВГО" Тогда
		
		Отмеченные = Новый Массив;
		Отмеченные.Добавить(Константы.РольИсполнительВГО.Получить());
		Отмеченные.Добавить(Константы.РольКураторВГО.Получить());
		ЭлементОтбора.ПравоеЗначение.ЗагрузитьЗначения(Отмеченные);
		ЭлементОтбора.Использование = Истина;
		
	Иначе	
		Отмеченные = ЭлементОтбора.ПравоеЗначение.ВыгрузитьЗначения();
		ЭлементОтбора.ПравоеЗначение.ЗагрузитьЗначения(Отмеченные);
	КонецЕсли;
	
	Если КлючВарианта = "ВГО" Тогда
		
		Ссылка = Справочники.СправочникиБД.ПолучитьСправочникБД("Организации");
		ТиповыеОтчетыУХ.УстановитьПараметр(Настройки, "Справочник", Ссылка);
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ПодготовитьСКД(НастройкиСКД);
		
КонецПроцедуры

Процедура ПодготовитьСКД(НастройкиСКД)
	
	СправочникИБ = ТиповыеОтчетыУХ.ПолучитьЗначениеПараметра(НастройкиСКД, "Справочник");
	ТекстЗапроса = СхемаКомпоновкиДанных.НаборыДанных[0].Запрос;
	
	Если СправочникИБ.Пустая() Тогда
	...
	Иначе	
		...
		ПолноеИмя = "Справочник." + СправочникИБ.ИмяОбъектаМетаданных;
		Если СтрНайти(ТекстЗапроса, ПолноеИмя) > 0 Тогда
			Возврат;//уже установлен
		КонецЕсли;
		
		СКД = ПолучитьМакет("ОсновнаяСхемаКомпоновкиДанных");
		ТекстЗапроса = СКД.НаборыДанных.Получить(0).Запрос;
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Справочник.Организации", ПолноеИмя);
