...
Процедура ОбновитьДанные(СохранитьПараметрыОтбора = Ложь, ОбновитьСвойстваЭлементовФормы = Ложь)
	...
	РезультатФоновогоЗадания = ОбновитьДанныеНаСервере(СохранитьПараметрыОтбора, ОбновитьСвойстваЭлементовФормы);
...
&НаКлиенте
Процедура ЗагрузитьДанныеОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		ЗагрузитьДанные(Результат.АдресРезультата);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузитьДанные(АдресРезультата)

	ЗагрузитьДанныеНаСервере(АдресРезультата);
	РазвернутьВсе();
...
&НаСервере
Процедура ЗагрузитьДанныеНаСервере(АдресХранилища)
...
Процедура ОткрытьФормуДлительнойОперации()
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ЗагрузитьДанныеОповещение", ЭтотОбъект);
