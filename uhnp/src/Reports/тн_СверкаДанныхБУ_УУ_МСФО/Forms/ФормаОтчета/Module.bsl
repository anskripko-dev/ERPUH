...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
    ...
    СтруктураДанных = ПолучитьДанныеРасшифровки(Расшифровка);
	
	// НАЧАЛО Чекордина Ирина 19.08.2024 Выполнение задач №000050429
	Если (СтруктураДанных.Свойство("РасхождениеУУБУ")) Тогда
		СтандартнаяОбработка = Ложь;
		ПараметрыВыбора = Новый Структура("КонецПериода", СтруктураДанных.Период);
		ОписаниеОповещения = Новый ОписаниеОповещения("ВыбратьПериодЗавершение", ЭтотОбъект, СтруктураДанных);
		ОткрытьФорму("Отчет.тн_СверкаДанныхБУ_УУ_МСФО.Форма.ВыборПериода", ПараметрыВыбора, ЭтаФорма, , , , ОписаниеОповещения); 
		Возврат;
	КонецЕсли;
	// КОНЕЦ Чекордина Ирина 19.08.2024 Выполнение задач №000050429

	ОткрытьОСВПоСчету(СтруктураДанных,СтандартнаяОбработка);
...
Функция ПолучитьДанныеРасшифровки(Расшифровка)
	...
	Для Каждого Элемент Из Данные.Настройки.ПараметрыДанных.Элементы Цикл
		
		Если Строка(Элемент.Параметр) = "Период"
			Или Строка(Элемент.Параметр) = "ОрганизацияУУ" Тогда
			СтруктураДанных.Вставить(Строка(Элемент.Параметр), Элемент.Значение);	
			Если  Строка(Элемент.Параметр) = "ОрганизацияУУ"  
				И Не Элемент.Использование Тогда
				СтруктураДанных.Вставить(Строка(Элемент.Параметр), Неопределено);
			ИначеЕсли Строка(Элемент.Параметр) = "Период" Тогда
				СтруктураДанных.Вставить(Строка(Элемент.Параметр), КонецДня(Элемент.Значение.Дата));
			КонецЕсли;
		// НАЧАЛО Чекордина Ирина 19.08.2024 Выполнение задач №000050429
		ИначеЕсли Строка(Элемент.Параметр) = "Организация" И Элемент.Использование Тогда
			СтруктураДанных.Вставить("СписокОрганизаций", Элемент.Значение);
		// КОНЕЦ Чекордина Ирина 19.08.2024 Выполнение задач №000050429
		КонецЕсли;
	...
	Для Каждого Элемент Из Поля Цикл
		...
		Если  Элемент.Поле = "ВидВзаиморасчетов" 
			Или Элемент.Поле = "ВидДоговора" 
			Или Элемент.Поле = "Субконто1" 
			Или Элемент.Поле = "Субконто2"  
			Или Элемент.Поле = "СчетБУ"   
			Или Элемент.Поле = "СчетУУ" 
			Или Элемент.Поле = "Организация"
			// НАЧАЛО Чекордина Ирина 19.08.2024 Выполнение задач №000050429
			Или Элемент.Поле = "РасхождениеУУБУ"
			// КОНЕЦ Чекордина Ирина 19.08.2024 Выполнение задач №000050429
			Или Элемент.Поле = "СчетМСФО" Тогда   
			
			СтруктураДанных.Вставить(Элемент.Поле, Элемент.Значение);   
			
			Если Элемент.Поле = "Субконто2" 
				И ЗначениеЗаполнено(СтруктураДанных.Субконто2)
				И Не ОбщегоНазначения.СсылкаСуществует(СтруктураДанных.Субконто2) Тогда
				СтруктураДанных.Вставить(Элемент.Поле, Неопределено);   
			КонецЕсли;
...
КонецФункции  
 ...
 &НаСервере
 Функция ПроверитьВидСубконто(Счет, ПорядокСубконто, Вид = Неопределено)
	 
	 Если Вид = Неопределено И Счет.ВидыСубконто.Количество() > (ПорядокСубконто - 1)
		 И Не Счет.ВидыСубконто[ПорядокСубконто - 1].ТолькоОбороты Тогда
		 Возврат Истина;
	 ИначеЕсли Вид <> Неопределено И Счет.ВидыСубконто.Количество() > (ПорядокСубконто - 1)
		 И Счет.ВидыСубконто[ПорядокСубконто - 1].ВидСубконто  = Вид 
		 И Не Счет.ВидыСубконто[ПорядокСубконто - 1].ТолькоОбороты Тогда
		 Возврат Истина;
	 КонецЕсли;
...
КонецФункции   
 ...
 &НаСервере
Функция ПолучитьПланСчетовУчета(СтруктураДанных)
	...
	Если СтруктураДанных.Свойство("Организация") И ЗначениеЗаполнено(СтруктураДанных.Организация) Тогда
		ОтборДляПолученияУчетнойПолитики.Вставить("Организация", СтруктураДанных.Организация);
	КонецЕсли; 

	УчетнаяПолитикаМСФО = РегистрыСведений.УчетныеПолитикиМСФО.ПолучитьПоследнее(,ОтборДляПолученияУчетнойПолитики);
	Если УчетнаяПолитикаМСФО <> Неопределено Тогда
		Возврат УчетнаяПолитикаМСФО.УчетнаяПолитика.ПланСчетовУчета;
	КонецЕсли;  
...
&НаКлиенте
Процедура ОткрытьОСВПоСчету(СтруктураДанных, СтандартнаяОбработка)
	
	Если СтруктураДанных.Свойство("СчетБУ") 
		И СтруктураДанных.СчетБУ <> Неопределено Тогда	
		ИмяОтчета = "ОборотноСальдоваяВедомостьПоСчету";
	ИначеЕсли (СтруктураДанных.Свойство("СчетУУ")
		И СтруктураДанных.СчетУУ <> Неопределено)
		Или (СтруктураДанных.Свойство("СчетМСФО")
		И СтруктураДанных.СчетМСФО <> Неопределено) Тогда
		ИмяОтчета = "ОСВМСФО";
	Иначе Возврат;
	КонецЕсли; 
	...
	Если СтруктураДанных.Свойство("СчетБУ") Тогда
		Счет = СтруктураДанных.СчетБУ;
	ИначеЕсли СтруктураДанных.Свойство("СчетУУ") Тогда
		Счет = СтруктураДанных.СчетУУ;
	Иначе
		Счет = СтруктураДанных.СчетМСФО;
	КонецЕсли;
		
	Если ИмяОтчета = "ОСВМСФО" Тогда 
		
	Отбор = Новый Структура; 
	
		Отбор.Вставить("Период", Новый СтандартныйПериод(Дата(1,1,1), СтруктураДанных.Период));

		Отбор.Вставить("Счет", Счет);   
		Если СтруктураДанных.Свойство("Организация") И ЗначениеЗаполнено(СтруктураДанных.Организация)
			И СтруктураДанных.Свойство("СчетМСФО") Тогда
			Отбор.Вставить("Организация", СтруктураДанных.Организация);
		КонецЕсли;    
		Если ЗначениеЗаполнено(СтруктураДанных.ОрганизацияУУ) 
			И СтруктураДанных.Свойство("СчетУУ")Тогда
			Отбор.Вставить("Организация", СтруктураДанных.ОрганизацияУУ);
		КонецЕсли;    

		//НАЧАЛО Блинова М.А. 03.07.2025 Выполнение задач №000054161
		// тут надо по отбору найти все-таки учетную
		//ПланСчетовУчета = ПолучитьПланСчетовУчета(СтруктураДанных); 
        ПланСчетовУчета = ПолучитьПланСчетовУчета(Отбор);
		//КОНЕЦ Блинова М.А. 03.07.2025 Выполнение задач №000054161 
		
		Если ПланСчетовУчета <> Неопределено Тогда
			Отбор.Вставить("ПланСчетовБД", ПланСчетовУчета);  
	   КонецЕсли;  
	   		
		Отбор.Вставить("Субконто1"  , СтруктураДанных.Субконто1); 
		Если СтруктураДанных.Свойство("Субконто2")
			И ЗначениеЗаполнено(СтруктураДанных.Субконто2)
			И ПроверитьВидСубконто(Счет, 2) Тогда
			Отбор.Вставить("Субконто2", СтруктураДанных.Субконто2);
		КонецЕсли;    	    
		
	Иначе
		
		ПользовательскиеНастройкиКомпоновкиДанных.ДополнительныеСвойства.Вставить("Счет"          , Счет);    
		ПользовательскиеНастройкиКомпоновкиДанных.ДополнительныеСвойства.Вставить("Период" , Новый СтандартныйПериод(Дата(1,1,1), СтруктураДанных.Период));
		ПользовательскиеНастройкиКомпоновкиДанных.ДополнительныеСвойства.Вставить("НачалоПериода" , Дата(1,1,1));
		ПользовательскиеНастройкиКомпоновкиДанных.ДополнительныеСвойства.Вставить("КонецПериода"  , СтруктураДанных.Период);
		
		Если СтруктураДанных.Свойство("Организация") И ЗначениеЗаполнено(СтруктураДанных.Организация) Тогда
			ПользовательскиеНастройкиКомпоновкиДанных.ДополнительныеСвойства.Вставить("Организация", СтруктураДанных.Организация);
		КонецЕсли;
		...
		ЗаполнитьЭлементОтбораКомпоновкиДанных(НовыйОтбор, Истина, ВидСравненияКомпоновкиДанных.Равно, "Субконто1", СтруктураДанных.Субконто1);
		
		Если Не СтруктураДанных.Свойство("Субконто2") 
			И ПроверитьВидСубконто(Счет, 2, ПредопределенноеЗначение("ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры")) Тогда 
			Если ЗначениеЗаполнено (СтруктураДанных.ВидДоговора) Тогда
				ЗаполнитьЭлементОтбораКомпоновкиДанных(НовыйОтбор,Истина,ВидСравненияКомпоновкиДанных.Равно,"Субконто2.ВидДоговораУХ", СтруктураДанных.ВидДоговора);	
			КонецЕсли;   
			Если ЗначениеЗаполнено (СтруктураДанных.ВидВзаиморасчетов) Тогда
				ЗаполнитьЭлементОтбораКомпоновкиДанных(НовыйОтбор,Истина,ВидСравненияКомпоновкиДанных.Равно,"Субконто2.тн_ВидВзаиморасчетов", СтруктураДанных.ВидВзаиморасчетов);			
			КонецЕсли;    
		КонецЕсли;   
		...
		Если СтруктураДанных.Свойство("Субконто2")  
			И ЗначениеЗаполнено(СтруктураДанных.Субконто2)
			И ПроверитьВидСубконто(Счет, 2, ПредопределенноеЗначение("ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры")) Тогда	
			ЗаполнитьЭлементОтбораКомпоновкиДанных(НовыйОтбор, Истина, ВидСравненияКомпоновкиДанных.Равно, "Субконто2", СтруктураДанных.Субконто2);
		КонецЕсли;  	
	...
	Если ИмяОтчета = "ОСВМСФО" Тогда 
		Отбор.Вставить("ВыводимыеРесурсы"  , "СуммаВВалютеОтчетности"); 
        ПараметрыОтчета.Вставить("Расшифровка" , Истина);
	    ПараметрыОтчета.Вставить("Отбор", Отбор);
		//НАЧАЛО Качикина 15.08.2024 Выполнение задач №000050514
		//ПараметрыОтчета.Вставить("КлючВарианта","ОСВПоСчету");
		ПараметрыОтчета.Вставить("КлючВарианта","ОСВпоСчету");
		//КОНЕЦ Качикина 15.08.2024 Выполнение задач №000050514
	КонецЕсли;	
    
    ОткрытьФорму("Отчет." + ИмяОтчета + ".Форма", ПараметрыОтчета, ЭтаФорма, Истина);    
...
// Чекордина Ирина 19.08.2024 Выполнение задач №000050429
&НаКлиенте
Процедура ВыбратьПериодЗавершение(РезультатВыбора, ДопПараметры) Экспорт
	...
	ПараметрыДанных.Вставить("НачалоПериода", РезультатВыбора.НачалоПериода);
	ПараметрыДанных.Вставить("КонецПериода", РезультатВыбора.КонецПериода);
	Если ДопПараметры.Свойство("СписокОрганизаций") Тогда
		ПараметрыДанных.Вставить("Организация", ДопПараметры.СписокОрганизаций);
	КонецЕсли;
	Если ЗначениеЗаполнено(ДопПараметры.ОрганизацияУУ) Тогда
		ПараметрыДанных.Вставить("ОрганизацияУУ", ДопПараметры.ОрганизацияУУ);
	КонецЕсли;
	ПараметрыДанных.Вставить("Контрагент", ДопПараметры.Субконто1);
	//НАЧАЛО Блинова М.А. 09.07.2025 Выполнение задач №000054161 
	ПараметрыДанных.Вставить("ДоговорКонтрагента", ДопПараметры.Субконто2);
	//КОНЕЦ Блинова М.А. 09.07.2025 Выполнение задач №000054161	
	ИмяОтчета = "тн_СверкаДанныхБУ_УУ_ВРазрезеРегистраторов";
	ПараметрыОтчета = ПолучитьПараметрыРасшифровкиВРазрезеРегистраторов(ИмяОтчета, ПараметрыДанных);
...
Функция ПолучитьПараметрыРасшифровкиВРазрезеРегистраторов(ИмяОтчета, ПараметрыДанных)
	...
	Для Каждого Параметр Из ПараметрыДанных Цикл
		...
		ПараметрДляЗаполнения = КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти(Параметр.Ключ);
		Если ЗначениеЗаполнено(ПараметрДляЗаполнения.ИдентификаторПользовательскойНастройки) Тогда
			ПользовательскиеНастройки = КомпоновщикНастроек.ПользовательскиеНастройки;
			ПользовательскийПараметр = ПользовательскиеНастройки.Элементы.Найти(ПараметрДляЗаполнения.ИдентификаторПользовательскойНастройки);
	...
	ПараметрыОтчета = Новый Структура;
	ПараметрыОтчета.Вставить("СформироватьПриОткрытии", Истина);
	ПараметрыОтчета.Вставить("Вариант", КомпоновщикНастроек.Настройки);
