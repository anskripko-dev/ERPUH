...
//  	* МодельБюджетирования - СправочникСсылка.МоделиБюджетирования - Модель, для которой будет выведен бюджетный процесс.
...
Процедура ЗаполнитьДиаграмму(Параметры, АдресХранилища) Экспорт
	...
	Если Группировки = "Вариант1" Тогда
		ТаблицаЗадач = Справочники.МоделиБюджетирования.ПолучитьБюджетныеЗадачи(МодельБюджетирования, Неопределено,
...
Функция РазбитьТаблицуШаговПоПериодам(ДеревоРезультат, МодельБюджетирования, ПоИсполнителям = Ложь)
	...
	ДатаНачалаДействияМодели = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(МодельБюджетирования, "НачалоДействия");
