...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ОтзывЗапросаМотивированногоМнения;
	...
	ПоказатьТекущуюСтраницу("Титульная_2024");
...
Процедура ПредставлениеУведомленияВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли ЭтотОбъект["Подключена1СОтчетность"] И Область.Имя = "НаимДокОсн" Тогда 
...
Процедура УстановитьПредставителяПоОрганизации()
	...
	СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(Объект.Организация, Объект.ДатаПодписи, "ДолжнРук");
	ЕстьОбласть = (Неопределено <> ПредставлениеУведомления.Области.Найти("Долж"));
	Если ЕстьОбласть Тогда 
		ПредставлениеУведомления.Области["Долж"].Значение = СведенияОбОрганизации.ДолжнРук;
	КонецЕсли;
КонецПроцедуры
...
Процедура ВыборЗапроса(СтандартнаяОбработка, Область)
	...
	ДополнительныеПараметры.Вставить("ВидУведомления", 
		ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ЗапросМотивированногоМнения"));
