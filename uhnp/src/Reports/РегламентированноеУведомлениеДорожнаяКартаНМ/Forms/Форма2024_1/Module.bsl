...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ДорожнаяКартаНМ;
...
Процедура СформироватьДеревоСтраниц() Экспорт
	...
	Стр001.МногострочныеЧасти.Добавить("ВиртМнгСтр");
	
	Стр001 = КорневойУровень.Добавить();
	Стр001.Наименование = "Дорожная карта";
	Стр001.ИндексКартинки = 1;
	...
	Стр0001.Наименование = "Стр. 1";
	Стр0001.ИндексКартинки = 1;
	Стр0001.ИмяМакета = "ПланМер_2024";
	Стр0001.Многостраничность = Истина;
	Стр0001.Многострочность = Истина;
	Стр0001.УИД = Новый УникальныйИдентификатор;
	Стр0001.ИДНаименования = "ПланМер";
...
Процедура УдалитьСтроку(ИмяОбласти)
	УведомлениеОСпецрежимахНалогообложения.УдалитьСтроку(ЭтотОбъект, ИмяОбласти, 3);
КонецПроцедуры
...
Процедура ПостОбработка()
	Для Каждого Признак Из ПолучитьИзВременногоХранилища(ЭтотОбъект["ДанныеДопСтрок"].ВиртМнгСтр) Цикл 
		ЗаполнитьЗначенияСвойств(ЭтотОбъект["ДанныеУведомления"].Титульная, Новый Структура("ПрУчаст" + Признак["ПрУчаст"], Истина));
	КонецЦикла;
КонецПроцедуры
