...
Процедура Сохранить(Команда)
	...
	ТекстПредложения = НСтр("ru = 'Для сохранения результата отчета в файл рекомендуется установить расширение для работы с 1С:Предприятием.';
							|en = 'It is recommended that you install 1C:Enterprise Extension before you save the report result to a file.'");
...
Функция ПодсказкаПоляОКВЭД()
	...
	Возврат
		...
		|en = 'Features of information search with filter by OKVED code: 
		| • Automatic selection of a value when entering text, performed only 
		| according to the current version of OKVED. 
		| • When opening the general list of OKVD, it is possible to select only current 
		| values. 
		| • If it is necessary to search by outdated versions of the classifier 
		| and the required code is not in the selection list, then it is allowed to specify 
		| the code manually. In this case, only numbers and 
		| the ""."" character (dot) can be entered. For example, you can specify the code ""14.40"", which 
		| is present only in the first version of the classifier. 
		| • Information is searched for regardless of the classifier version. 
		| For example, the code ""01.21"" of the first version of the classifier corresponds 
		| to the value ""Cattle breeding"", and the classifier 
		| of the second version - to the value ""Grape growing"". 
		| When searching for information on the specified code, the results will include both
		| those counterparties whose OKVED is ""01.21 Cattle 
		|breeding"" and those whose OKVED contains the value 
		|""01.21 Grape growing"".'");
...
Процедура НажатиеКнопкиПерехода(НомерСтраницы)
	...
	Если НомерСтраницы > Отчет.ДоступноСтраниц Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'Going to a page number greater than %1 is not supported. 
...
Функция ОбновитьДоступныеОтборыИПоляНаСервере(Знач УникальныйИдентификатор)
	...
	НаименованиеЗадания = НСтр("ru = 'Поиск контрагентов. Обновление доступных критериев и полей.';
								|en = 'Search for counterparties. Updating available criteria and fields.'");
		...
		Если РезультатОбновления.ДоступныеОтборы.Количество() > 0
			...
			Если Успешно Тогда
				ТекстСообщения = НСтр("ru = 'Обновление информации выполнено успешно.';
										|en = 'The information update is completed.'");
			Иначе
				ТекстСообщения = НСтр("ru = 'Обновление информации выполнено с ошибками.';
										|en = 'The information update is completed with errors.'");
			КонецЕсли;
		...
		Если Отчет.ДоступноСтраниц < Отчет.ВсегоСтраниц Тогда
			...
			Элементы.ДекорацияНайденоКоличество.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для просмотра/сохранения доступны первые %1. Уточните параметры поиска.';
					|en = 'You can view/save the first %1. Specify search parameters.'"),
				ОрганизацийМаксимум);
...
Функция ПараметрыОтчетаСПроверкой(
	...
	Если ПараметрыОтчета.ПараметрыЗапроса.Поля.Количество() = 0 Тогда
		ТекстОшибки.Добавить(НСтр("ru = 'Необходимо выбрать хотя бы одно выводимое поле.';
									|en = 'Select at least one output field.'"));
	КонецЕсли;
...
Функция ЗагрузкаДанныхДляСохранения(
	...
	НаименованиеЗадания = НСтр("ru = 'Поиск контрагентов. Загрузка информации для сохранения в файл.';
								|en = 'Search for counterparties. Downloading information to save to a file.'");
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НаименованиеЗадания;
				...
				Попытка
					...
					ПоказатьОповещениеПользователя(
						НСтр("ru = 'Отчет сохранен в файл';
							|en = 'The report is saved to the file'"),
						Обработчик,
						ДополнительныеПараметры.ПолноеИмяФайлаОтчета);
