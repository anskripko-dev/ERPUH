...
Функция ПериодичностьИнтервалов(Распоряжение)
	...
	|ГДЕ
	|	ГрафикЭтаповПроизводства2_2.ЗаказНаПроизводство = &Распоряжение
	...
	|			ТОГДА 2
	|		КОГДА ВложенныйЗапрос.ИнтервалПланирования = ЗНАЧЕНИЕ(Перечисление.ТочностьГрафикаПроизводства.Смена)
	|			ТОГДА 2
