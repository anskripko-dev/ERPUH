...
Процедура СформироватьОтчетСостояниеВыполненияДокументов(ВходящиеДокументы, ТаблицаОтчета) Экспорт
	...
	ВсегоТаблиц = 0;
	//Если СтруктураПараметров.ВыводитьТаблицуРасчетыСПоставщиками <> 0 Тогда
	//	
	...
	Если СтруктураПараметров.ВыводитьТаблицуРасчетыСПоставщиками <> 0
		...
		Запрос.УстановитьПараметр("Поступление", НСтр("ru = 'Поступление';
														|en = 'Receipt'"));
		ТекстыЗапроса.Добавить(ТекстЗапросаВтДанныеПоРасчетамСПоставщиками(ИмяТаблицы, СтруктураПараметров), "ВтДанныеПоРасчетамСПоставщиками");
		ТекстыЗапроса.Добавить(ТекстЗапросаВТКПоставке(ИмяТаблицы, СтруктураПараметров), "ВТКПоставке");
		ТекстыЗапроса.Добавить(ТекстЗапросаВТЭтапПоставлено(ИмяТаблицы, СтруктураПараметров), "ЭтапПоставлено");
		ТекстыЗапроса.Добавить(ТекстЗапросаВТДанныеПоПоставке(ИмяТаблицы, СтруктураПараметров), "ДанныеПоПоставке");
		ТекстыЗапроса.Добавить(ТекстЗапросаВТКОплате(ИмяТаблицы, СтруктураПараметров), "КОплате");
		ТекстыЗапроса.Добавить(ТекстЗапросаВТЭтапОплачено(ИмяТаблицы, СтруктураПараметров), "ЭтапОплачено");
		ТекстыЗапроса.Добавить(ТекстЗапросаВТДанныеПоОплате(ИмяТаблицы, СтруктураПараметров), "ДанныеПоОплате");
	...
	КонецЕсли;
 	...
 	Если СтруктураПараметров.ВыводитьТаблицыПоступление <> 0  Тогда
	...
	Для каждого СтрокаДокумент Из ТаблицаДокументов Цикл
		...
		//		
		//	Иначе
		Если СтруктураПараметров.ВыводитьТаблицуРасчетыСПоставщиками = НомерТаблицы Тогда
		//		
...
Функция ТекстЗапросаТаблицаПодчиненныеДокументы(ИмяТаблицы, СтруктураПараметров)
	...
	Если (ИмяТаблицы = "Документ.ЗаказКлиента" 
		...
		ТекстЗапроса = "
			...
			|ГДЕ
			|	ВтДокументы.ДокументСсылка.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоНакладным)
			|	И НЕ ТчДокументРеализациии.Ссылка.ПометкаУдаления И ТчДокументРеализациии.Ссылка.Проведен";
	...
	Если ИмяТаблицы = "Документ.ЗаказПоставщику" 
		...
		ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		               |	ВтДокументы.ДокументСсылка КАК Документ,
		               |	ТчДокументРеализациии.Ссылка.Договор.ПорядокРасчетов КАК ПорядокРасчетов,
		               |	ТчДокументРеализациии.Ссылка.Валюта КАК Валюта,
		               |	СУММА(ТчДокументРеализациии.Сумма) КАК СуммаДокумента,
		               |	ТчДокументРеализациии.Ссылка КАК ПодчиненныйДокумент
		               |ИЗ
		               |	Документ.ПриобретениеТоваровУслуг.Товары КАК ТчДокументРеализациии
		               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтДокументы КАК ВтДокументы
		               |		ПО ТчДокументРеализациии.Ссылка.ЗаказПоставщику = ВтДокументы.ДокументСсылка
		               |ГДЕ
		               |	ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным) = ТчДокументРеализациии.Ссылка.Договор.ПорядокРасчетов
		               |	И НЕ ТчДокументРеализациии.Ссылка.ПометкаУдаления
		               |	И ТчДокументРеализациии.Ссылка.Проведен
		               |
		               |СГРУППИРОВАТЬ ПО
		               |	ВтДокументы.ДокументСсылка,
		               |	ТчДокументРеализациии.Ссылка.ДоговорКонтрагента.ПорядокРасчетов,
		               |	ТчДокументРеализациии.Ссылка.ВалютаДокумента,
		               |	ТчДокументРеализациии.Ссылка";
...
Функция ДобавитьКомандуСостояниеВыполненияЗаказКлиента(КомандыОтчетов) Экспорт
	...
	Если ПравоДоступа("Просмотр", Метаданные.Отчеты.РасчетыПоЗаказу) Тогда
		
		КомандаОтчет = КомандыОтчетов.Добавить();
		
		КомандаОтчет.Менеджер = Метаданные.Отчеты.РасчетыПоЗаказу.ПолноеИмя();
...
Функция ТекстЗапросаВтОбъектыРасчетов(ИмяТаблицы, СтруктураПараметров)
	...
	|	Истина КАК ТребуетсяГрафик, 
	|	Документы.ДокументСсылка.ДоговорКонтрагента.ПорядокРасчетов КАК ПорядокРасчетов
...
Функция ТекстЗапросаТаблицаГрафикиОплаты(ИмяТаблицы, СтруктураПараметров)
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	ОбъектыРасчетов.ОбъектРасчетов КАК Документ,
	               |	условиеоплаты.НомерСтроки КАК НомерСтроки,
	               |	условиеоплаты.ВариантОплаты КАК ВариантОплаты,
	               |	ОбъектыРасчетов.ОбъектРасчетов.дата КАК ДатаПлатежа,
	               |	ДокументЭтапыГрафикаОплаты.СуммаОборот КАК СуммаПлатежа
	               |ИЗ
	               ...
	               |				) КАК ДокументЭтапыГрафикаОплаты
				   |        ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОбъектыРасчетов КАК ОбъектыРасчетов
				   |        По ДокументЭтапыГрафикаОплаты.ОбъектПланирования.Объект = ОбъектыРасчетов.Документ
				   |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УсловияОплаты.ЭтапыОплаты КАК условиеоплаты
	               |		ПО (условиеоплаты.Ссылка = ОбъектыРасчетов.ОбъектРасчетов.ДоговорКонтрагента.УсловиеОплаты);";  
	
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	ОбъектыРасчетов.ОбъектРасчетов КАК Документ,
	               |	ДокументЭтапыГрафикаОплаты.Период КАК ДатаПлатежа,
	               |	СУММА(ДокументЭтапыГрафикаОплаты.СуммаОборот) КАК СуммаПлатежа
	               |ИЗ
	               |	РегистрНакопления.ОперацииБюджетов.Обороты(, , Запись, ВидБюджета = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыБюджетов.БюджетДвиженияДенежныхСредств)) КАК ДокументЭтапыГрафикаОплаты
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОбъектыРасчетов КАК ОбъектыРасчетов
	               |		ПО ДокументЭтапыГрафикаОплаты.ОбъектПланирования.Объект = ОбъектыРасчетов.Документ
	               |ГДЕ
	               |	ДокументЭтапыГрафикаОплаты.СуммаОборот > 0
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ОбъектыРасчетов.ОбъектРасчетов,
	               |	ДокументЭтапыГрафикаОплаты.Период;";
	...
	//		Или ИмяТаблицы = "Документ.ЗаказПереработчику2_5" Тогда
...
Функция ТекстЗапросаВтДанныеПоРасчетамСКлиентами(ИмяТаблицы, СтруктураПараметров)
	...
	|		ПО СправочникОбъектыРасчетов.Ссылка = РасчетыСКлиентами.ОбъектРасчетов) КАК ДанныеПоРасчетам
	|
	|	СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаВтДанныеПоРасчетамСПоставщиками(ИмяТаблицы, СтруктураПараметров)
	...
	КонецЕсли;
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	ДанныеПоРасчетам.ОбъектПланирования.Объект КАК ОбъектРасчетов,
	               |	ДанныеПоРасчетам.Период КАК Период,
	               |	СУММА(ДанныеПоРасчетам.ОплатаПлан) КАК ОплатаПлан,
	               |	СУММА(ДанныеПоРасчетам.ОплатаФакт) КАК ОплатаФакт
	               ...
	               |		СУММА(ВЫБОР
	               |				КОГДА ОперацииБюджетовОбороты.СуммаОборот > 0
	               |					ТОГДА ОперацииБюджетовОбороты.СуммаОборот
	               |				ИНАЧЕ 0
	               |			КОНЕЦ) КАК ОплатаПлан,
	               |		СУММА(ОперацииБюджетовОбороты.СуммаИсполненоОборот) КАК ОплатаФакт
	               |	ИЗ
	               |		ОбъектыРасчетов КАК ОбъектыРасчетов
	               |			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ОперацииБюджетов.Обороты(, , Запись, ВидБюджета = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыБюджетов.БюджетДвиженияРесурсов)) КАК ОперацииБюджетовОбороты
	               |			ПО (ОперацииБюджетовОбороты.ОбъектПланирования.Объект = ОбъектыРасчетов.ОбъектРасчетов)
	               |	
	               |	СГРУППИРОВАТЬ ПО
	               ...
	               |		ОперацииБюджетовОбороты.ПриходРасход) КАК ДанныеПоРасчетам
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ДанныеПоРасчетам.Период,
	               |	ДанныеПоРасчетам.ОбъектПланирования,
	               |	ДанныеПоРасчетам.ОбъектПланирования.Объект;";
	Возврат ТекстЗапроса;

КонецФункции

Функция ТекстЗапросаВтКПоставке(ИмяТаблицы, СтруктураПараметров)
	СтруктураДопЗапросов = СтруктураПараметров.СтруктураДопЗапросов;
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	""Поставка"" КАК Этап,
	               |	ОперацииБюджетовОбороты.Валюта КАК Валюта,
	               |	СУММА(ОперацииБюджетовОбороты.СуммаОборот) КАК КПоставкеСумма
	               |ПОМЕСТИТЬ КПоставке
	               |ИЗ
	               |	РегистрНакопления.ОперацииБюджетов.Обороты(
	               |			,
	               |			,
	               |			Запись,
	               |			ВидБюджета = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыБюджетов.БюджетДвиженияРесурсов)
	               |				И ОбъектПланирования.Объект В (&МассивДокументов)) КАК ОперацииБюджетовОбороты
	               |ГДЕ
	               |	ОперацииБюджетовОбороты.СуммаОборот > 0
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ОперацииБюджетовОбороты.Период,
...
Функция ТекстЗапросаВтЭтапПоставлено(ИмяТаблицы, СтруктураПараметров)
	СтруктураДопЗапросов = СтруктураПараметров.СтруктураДопЗапросов;
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	""Поставка"" КАК Этап,
	               |	ОперацииБюджетовОбороты.Валюта КАК Валюта,
	               |	СУММА(ОперацииБюджетовОбороты.СуммаИсполненоОборот) КАК Поставлено
	               |ПОМЕСТИТЬ ЭтапПоставлено
	               |ИЗ
	               |	РегистрНакопления.ОперацииБюджетов.Обороты(
	               |			,
	               |			,
...
Функция ТекстЗапросаВтДанныеПоПоставке(ИмяТаблицы, СтруктураПараметров)
	СтруктураДопЗапросов = СтруктураПараметров.СтруктураДопЗапросов;
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               |	КПоставке.ОбъектПланированияОбъект КАК ОбъектПланированияОбъект,
	               |	КПоставке.Период КАК Период,
	               |	КПоставке.Этап КАК Этап,
	               |	КПоставке.Валюта КАК Валюта,
	               |	КПоставке.КПоставкеСумма КАК Сумма,
	               |	ВЫБОР
	               |		КОГДА &ТекущаяДата > КПоставке.Период
	               |			ТОГДА КПоставке.КПоставкеСумма - ЕСТЬNULL(ЭтапПоставлено.Поставлено, 0)
	               |	КОНЕЦ КАК Просрочено,
	               |	0 КАК Поставлено
	               |ПОМЕСТИТЬ ДанныеПоПоставке
	               |ИЗ
	               |	КПоставке КАК КПоставке
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ЭтапПоставлено КАК ЭтапПоставлено
	               |		ПО КПоставке.ОбъектПланированияОбъект = ЭтапПоставлено.ОбъектПланированияОбъект
	               |			И КПоставке.Период = ЭтапПоставлено.Период
	               |			И КПоставке.Валюта = ЭтапПоставлено.Валюта
	               |
	               |ОБЪЕДИНИТЬ
	               |
	               |ВЫБРАТЬ
	               |	ЭтапПоставлено.ОбъектПланированияОбъект,
	               |	ЭтапПоставлено.Период,
	               |	ЭтапПоставлено.Этап,
	               |	ЭтапПоставлено.Валюта,
	               |	0,
	               |	0,
	               |	ЭтапПоставлено.Поставлено
	               |ИЗ
	               |	ЭтапПоставлено КАК ЭтапПоставлено
	               |;";
	Возврат ТекстЗапроса;

КонецФункции

Функция ТекстЗапросаВтКОплате(ИмяТаблицы, СтруктураПараметров)
	СтруктураДопЗапросов = СтруктураПараметров.СтруктураДопЗапросов;
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	ОперацииБюджетовОбороты.ОбъектПланирования.Объект КАК ОбъектПланированияОбъект,
	               |	ОперацииБюджетовОбороты.Период КАК Период,
	               |	""Оплата по графику"" КАК Этап,
	               |	ОперацииБюджетовОбороты.Валюта КАК Валюта,
	               |	СУММА(ОперацииБюджетовОбороты.СуммаОборот) КАК КОплатеСумма
	               |ПОМЕСТИТЬ КОплате
	               |ИЗ
	               |	РегистрНакопления.ОперацииБюджетов.Обороты(
	               |			,
...
Функция ТекстЗапросаВтЭтапОплачено(ИмяТаблицы, СтруктураПараметров)
	СтруктураДопЗапросов = СтруктураПараметров.СтруктураДопЗапросов;
	...
	КонецЕсли;
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	ОперацииБюджетовОбороты.АналитикаПартнеров.ОбъектРасчетов КАК ОбъектПланированияОбъект,
	               |	ОперацииБюджетовОбороты.Период КАК Период,
	               |	ВЫБОР
	               |		КОГДА КОплате.ОбъектПланированияОбъект = ОперацииБюджетовОбороты.АналитикаПартнеров.ОбъектРасчетов
	               |			ТОГДА ""Оплата по графику""
	               |		ИНАЧЕ ""Оплата не по графику""
	               |	КОНЕЦ КАК Этап,
	               |	ОперацииБюджетовОбороты.Валюта КАК Валюта,
	               |	СУММА(ОперацииБюджетовОбороты.СуммаИсполненоОборот) КАК ОплаченоСумма
	               |ПОМЕСТИТЬ ЭтапОплачено
	               |ИЗ
	               |	РегистрНакопления.ОперацииБюджетов.Обороты(
	               |			,
	               |			,
	               |			Запись,
	               |			ВидБюджета = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыБюджетов.БюджетДвиженияДенежныхСредств)
	               |				И ВЫРАЗИТЬ(АналитикаПартнеров.ОбъектРасчетов КАК Документ.ЗаказПоставщику) В (&МассивДокументов)) КАК ОперацииБюджетовОбороты
	               |		ЛЕВОЕ СОЕДИНЕНИЕ КОплате КАК КОплате
	               |		ПО ОперацииБюджетовОбороты.АналитикаПартнеров.ОбъектРасчетов = КОплате.ОбъектПланированияОбъект
	               |			И ОперацииБюджетовОбороты.Период = КОплате.Период
	               |			И ОперацииБюджетовОбороты.Валюта = КОплате.Валюта
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ОперацииБюджетовОбороты.Период,
	               |	ОперацииБюджетовОбороты.АналитикаПартнеров.ОбъектРасчетов,
	               |	ОперацииБюджетовОбороты.Валюта,
	               |	КОплате.ОбъектПланированияОбъект
	               |;";
	Возврат ТекстЗапроса;

КонецФункции

Функция ТекстЗапросаВтДанныеПоОплате(ИмяТаблицы, СтруктураПараметров)
	СтруктураДопЗапросов = СтруктураПараметров.СтруктураДопЗапросов;
	...
	КонецЕсли;
	ТекстЗапроса = "ВЫБРАТЬ
	               |	КОплате.ОбъектПланированияОбъект КАК ОбъектПланированияОбъект,
	               |	КОплате.Период КАК Период,
	               |	КОплате.Этап КАК Этап,
	               |	КОплате.Валюта КАК Валюта,
	               |	КОплате.КОплатеСумма КАК Сумма,
	               |	ВЫБОР
	               |		КОГДА &ТекущаяДата > КОплате.Период
	               |			ТОГДА КОплате.КОплатеСумма - ЕСТЬNULL(ЭтапОплачено.ОплаченоСумма, 0)
	               |	КОНЕЦ КАК Просрочено,
	               |	0 КАК Оплачено
	               |ПОМЕСТИТЬ ДанныеПоОплате
	               |ИЗ
	               |	КОплате КАК КОплате
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ЭтапОплачено КАК ЭтапОплачено
	               |		ПО КОплате.ОбъектПланированияОбъект = ЭтапОплачено.ОбъектПланированияОбъект
	               |			И КОплате.Период = ЭтапОплачено.Период
	               |			И КОплате.Валюта = ЭтапОплачено.Валюта
	               |
	               |ОБЪЕДИНИТЬ
	               |
	               |ВЫБРАТЬ
	               |	ЭтапОплачено.ОбъектПланированияОбъект,
	               |	ЭтапОплачено.Период,
	               |	ЭтапОплачено.Этап,
	               |	ЭтапОплачено.Валюта,
	               |	0,
	               |	0,
	               |	ЭтапОплачено.ОплаченоСумма
	               |ИЗ
	               |	ЭтапОплачено КАК ЭтапОплачено
	               |;";
	Возврат ТекстЗапроса;
...
Функция ТекстЗапросаТаблицаРасчетыСПоставщиками(СтруктураПараметров)
	...
	ТекстЗапроса = "
				   ...
				   |ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	ОбъектыРасчетов.Документ КАК Документ,
	               |	РасчетыСПоставщиками.ОбъектПланирования.Объект КАК ОбъектРасчетов,
	               |	ОбъектыРасчетов.ПорядокРасчетов КАК ПорядокРасчетов,
	               |	РасчетыСПоставщиками.Валюта КАК Валюта,
	               |	ОбъектыРасчетов.ТребуетсяГрафик КАК ТребуетсяГрафик,
	               |	ВЫБОР
	               ...
	               |	ЕСТЬNULL(ДанныПоРасчетамПоХО.ОплатаПлан, 0) КАК ОплатаПлан,
	               |	ЕСТЬNULL(ДанныПоРасчетамПоХО.ОплатаФакт, 0) КАК ОплатаФакт,
	               |	0 КАК ОплатаПросрочено,
	               |	0 КАК ОплатаПросроченоИтого,
	               |	РасчетыСПоставщиками.СуммаИсполненоОборот КАК ОтгрузкаФакт,
	               |	РасчетыСПоставщиками.СуммаОборот КАК ОтгрузкаПлан,
	               |	0 КАК ОтгрузкаПросрочено,
	               |	0 КАК ОтгрузкаПросроченоИтого
	               |ИЗ
	               |	РегистрНакопления.ОперацииБюджетов.Обороты(
	               |			,
	               |			,
	               |			Запись,
	               |			ВидБюджета = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыБюджетов.БюджетДвиженияДенежныхСредств)
	               |				И ОбъектПланирования.Объект В
	               |					(ВЫБРАТЬ
	               |						ОбъектыРасчетов.ОбъектРасчетов
	               |					ИЗ
	               |						ОбъектыРасчетов КАК ОбъектыРасчетов)) КАК РасчетыСПоставщиками
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ОбъектыРасчетов КАК ОбъектыРасчетов
	               |		ПО (ОбъектыРасчетов.ОбъектРасчетов = РасчетыСПоставщиками.ОбъектПланирования.Объект)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВтДанныеПоРасчетамСПоставщиками КАК ДанныПоРасчетамПоХО
	               |		ПО РасчетыСПоставщиками.ОбъектПланирования.Объект = ДанныПоРасчетамПоХО.ОбъектРасчетов
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	РасчетыСПоставщиками.Период
	               |ИТОГИ
	...
	ТекстЗапроса = "
				   |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ДанныеПоПоставкеИОплате.ОбъектПланированияОбъект КАК Документ,
	               |	ДанныеПоПоставкеИОплате.ОбъектПланированияОбъект КАК ОбъектРасчетов,
	               |	ОбъектыРасчетов.ПорядокРасчетов КАК ПорядокРасчетов,
	               |	ИСТИНА КАК ТребуетсяГрафик,
	               |	ДанныеПоПоставкеИОплате.Валюта КАК Валюта,
	               |	ДанныеПоПоставкеИОплате.Этап КАК ЭтапОплатыОтгрузки,
	               |	ДанныеПоПоставкеИОплате.Период КАК ДатаПлатежа,
	               |	ДанныеПоПоставкеИОплате.ОплатаПлан КАК ОплатаПлан,
	               |	ДанныеПоПоставкеИОплате.ОплатаФакт КАК ОплатаФакт,
	               |	ДанныеПоПоставкеИОплате.ОплатаПросрочено КАК ОплатаПросрочено,
	               |	ДанныеПоПоставкеИОплате.ОтгрузкаПлан КАК ОтгрузкаПлан,
	               |	ДанныеПоПоставкеИОплате.ОтгрузкаФакт КАК ОтгрузкаФакт,
	               |	ДанныеПоПоставкеИОплате.ОтгрузкаПросрочено КАК ОтгрузкаПросрочено
	               |ИЗ
	               |	(ВЫБРАТЬ
	               |		ДанныеПоОплате.ОбъектПланированияОбъект КАК ОбъектПланированияОбъект,
	               |		ДанныеПоОплате.Валюта КАК Валюта,
	               |		ДанныеПоОплате.Этап КАК Этап,
	               |		ДанныеПоОплате.Сумма КАК ОплатаПлан,
	               |		ДанныеПоОплате.Оплачено КАК ОплатаФакт,
	               |		ДанныеПоОплате.Просрочено КАК ОплатаПросрочено,
	               |		NULL КАК ОтгрузкаПлан,
	               |		NULL КАК ОтгрузкаФакт,
	               |		NULL КАК ОтгрузкаПросрочено,
	               |		ДанныеПоОплате.Период КАК Период
	               |	ИЗ
	               |		ДанныеПоОплате КАК ДанныеПоОплате
	               |	
	               |	ОБЪЕДИНИТЬ ВСЕ
	               |	
	               |	ВЫБРАТЬ
	               |		ДанныеПоПоставке.ОбъектПланированияОбъект,
	               |		ДанныеПоПоставке.Валюта,
	               |		ДанныеПоПоставке.Этап,
	               |		NULL,
	               |		NULL,
	               |		NULL,
	               |		ДанныеПоПоставке.Сумма,
	               |		ДанныеПоПоставке.Поставлено,
	               |		ДанныеПоПоставке.Просрочено,
	               |		ДанныеПоПоставке.Период
	               |	ИЗ
	               |		ДанныеПоПоставке КАК ДанныеПоПоставке) КАК ДанныеПоПоставкеИОплате
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ОбъектыРасчетов КАК ОбъектыРасчетов
	               |		ПО (ОбъектыРасчетов.ОбъектРасчетов = ДанныеПоПоставкеИОплате.ОбъектПланированияОбъект)
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	ДатаПлатежа
	               |ИТОГИ
	               ...
	               |	СУММА(ОплатаПлан),
	               |	СУММА(ОплатаФакт),
	               |	СУММА(ОплатаПросрочено),
	               |	СУММА(ОтгрузкаПлан),
	               |	СУММА(ОтгрузкаФакт),
	               |	СУММА(ОтгрузкаПросрочено)
	               |ПО
	               |	Документ,
	               |	ОбъектРасчетов
				   |;";
...
Функция ТекстЗапросаВТТоварыРаспоряжениеЗаказОтгружено(ИмяТаблицы,СтруктураПараметров, ТекстыЗапроса)
	...
	ИначеЕсли ИмяТаблицы = "Документ.ЗаказПереработчику2_5" Тогда
...
Функция ТекстЗапросаВТТоварыРаспоряженияПоступлениеЗаказ(ИмяТаблицы,СтруктураПараметров,ТекстыЗапроса)
	...
	Если ИмяТаблицы = "Документ.ЗаказПоставщику" Тогда
		Если ПравоДоступа("Чтение", Метаданные.Документы.ПриобретениеТоваровУслуг) Тогда
			...
			|	ДокументТовары.Характеристика как Характеристика,
			|	СУММА(ДокументТовары.Количество),
			|	СУММА(ДокументТовары.СуммаНДС+ДокументТовары.Сумма),
			|	ДокументТовары.Ссылка.Склад,
			|	ВтДокументы.ДокументСсылка
			|ИЗ
			|	Документ.ПриобретениеТоваровУслуг.Товары КАК ДокументТовары
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтДокументы КАК ВтДокументы
			|		ПО (ВтДокументы.ДокументСсылка = ДокументТовары.Ссылка.ЗаказПоставщику)
			|ГДЕ
			|	ДокументТовары.Ссылка.Проведен
			|
			|СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаТаблицаПоступило(СтруктураПараметров)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ 
	|	ПланированиеПотребностейОстатки.АналитикаПланирования.ОбъектПланирования.Объект.ЗаказПоставщику КАК Документ,
	...
	|ИЗ
	|	РегистрНакопления.ПланированиеПотребностей.Остатки КАК ПланированиеПотребностейОстатки
	|ГДЕ
	|	ПланированиеПотребностейОстатки.АналитикаПланирования.Этап = ЗНАЧЕНИЕ(Перечисление.ЭтапыПланированияПотребностей.Исполнено)
	|	И ПланированиеПотребностейОстатки.АналитикаПланирования.ОбъектПланирования.Объект.ЗаказПоставщику В(&МассивДокументов)
	|ИТОГИ ПО
	|	Документ;
	|";
	Возврат ТекстЗапроса;
...
Процедура ВывестиЗаголовокОтчета(ТаблицаОтчета, Макет, Выборка)
	...
	СтруктураПараметров = Новый Структура;
		СтруктураПараметров.Вставить("ИмяДокумента", Выборка.Ссылка);
	...
	Если КлиентскоеПриложение.ТекущийВариантИнтерфейса() = ВариантИнтерфейсаКлиентскогоПриложения.Такси Тогда
		...
		ОбластьЗаголовок.ЦветТекста = ЦветаСтиля.ЦветЗаголовкаГруппы;
	КонецЕсли;
...
Процедура ВывестиТаблицуРасчетовПоОбъектуРасчетов(ТаблицаОтчета, Макет, ЗадолженностьПоДокументу, ВыводитьОтступ, ЭтоОтгрузка)
	...
	Иначе
		ЗаголовокТаблицы = НСтр("ru = 'Наша задолженность';
								|en = 'Payments / Invoices reconсiliation'");
	КонецЕсли; 
...
Функция ВывестиТаблицуНеобеспеченныеТовары(ТаблицаОтчета, Макет, Документ, ТаблицыРезультатов, СтруктураПараметров)
	...
	Для каждого СтрокаТовар Из НеобеспеченныеТовары.Строки Цикл
		...
		ПредставлениеНоменклатурыДляПечати = СтрокаТовар.Номенклатура;
...
Функция ВывестиТаблицуПоступило(ТаблицаОтчета, Макет, Документ, ТаблицыРезультатов, СтруктураПараметров)
	...
	Для каждого СтрокаТовар Из ПоступилоТовары.Строки Цикл
		...
		Область.Параметры.Заполнить(СтрокаТовар);
		ПредставлениеНоменклатурыДляПечати = НоменклатураКлиентСервер.ПредставлениеНоменклатуры(СтрокаТовар.Номенклатура,СтрокаТовар.Характеристика);
		Область.Параметры.Товар = ПредставлениеНоменклатурыДляПечати;
...
Процедура ВывестиТаблицуПоступленияПоСкладу(ТаблицаОтчета, Макет, ТоварыПоСкладу, СтруктураПараметров)
	...
	Если ЗначениеЗаполнено(ТоварыПоСкладу.Склад) Тогда 
		ЗаголовокТаблицы = НСтр("ru = 'В процессе поступления на склад ""%СкладОтгрузки%"" (%1%)';
								|en = 'Goods received / Invoices received reconciliation ""%СкладОтгрузки%"" (%1%)'");
		ЗаголовокТаблицы = СтрЗаменить(ЗаголовокТаблицы, "%СкладОтгрузки%", ТоварыПоСкладу.Склад);
	Иначе
		ЗаголовокТаблицы = НСтр("ru = 'Услуги и работы в процессе оформления (%1%)';
								|en = 'Services and labor under registration (%1%)'");
...
Функция ВывестиТаблицуПродукцияКОформлениюВОтчетеПереработчику(ТаблицаОтчета, Макет, Документ, ТаблицыРезультатов, СтруктураПараметров)
	...
	Для каждого СтрокаТовар Из ПродукцияКОформлению.Строки Цикл
		...
		Область.Параметры.Заполнить(СтрокаТовар);
		Область.Параметры.Товар = СтрокаТовар.НоменклатураПредставление;
...
Функция ЕстьПравоДоступаКНаборуДанных(ИмяНабораДанных, СтруктураПараметров)
	...
	Если ИмяНабораДанных = "Поступление" Тогда
		...
		Если СтруктураПараметров.ИмяРегистраПоступлениеУслуг = "ЗаказыПоставщикам" Тогда
			СтруктураПараметров.Вставить("ЕстьПравоДоступаРегистрПоступлениеУслуг",Истина);
