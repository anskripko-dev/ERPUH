...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки, ЕстьПредупреждения = Ложь)
	...
	Если НЕ (ПараметрыВыгрузки.ОтчетГод >= 2023 И ПараметрыВыгрузки.ОтчетГод <= 2026) Тогда
		СообщитьВТС("ОШИБКА: Задан год представления отчета, не входящий в интервал с 2023 г. по 2026 г.", "Титульный", "NamePeriod");
		СтатусПроверки = Ложь;
	КонецЕсли;
