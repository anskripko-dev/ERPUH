...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ГрафикПереходаНалоговыйМониторинг;
...
Процедура ПредставлениеУведомленияВыбор(Элемент, Область, СтандартнаяОбработка)
	Если СтрНачинаетсяС(Область.Имя, "ПрУчаст") Тогда 
...
Процедура СформироватьДеревоСтраниц() Экспорт
	...
	СтрРег.ИндексКартинки = 1;
	СтрРег.ИмяМакета = "ПланГрафик_2024";
	СтрРег.Многостраничность = Истина;
	СтрРег.Многострочность = Истина;
	СтрРег.УИД = Новый УникальныйИдентификатор;
	СтрРег.ИДНаименования = "ПланГрафик";
	СтрРег.МногострочныеЧасти.Добавить("ВиртМнгСтр");
...
Процедура ПостОбработка()
	ТаблицаПризнаков = ПолучитьИзВременногоХранилища(ЭтотОбъект["ДанныеДопСтрок"].ВиртМнгСтр);
	Для Каждого Стр Из ЭтотОбъект["ДанныеМногостраничныхРазделов"].ПланГрафик Цикл 
		ПланГрафик = Стр.Значение;
		Для Каждого Признак Из ТаблицаПризнаков.НайтиСтроки(Новый Структура("УИД", ПланГрафик.УИД)) Цикл 
			ЗаполнитьЗначенияСвойств(ПланГрафик, Новый Структура("ПрУчаст" + Признак["ПрУчаст"], Истина));
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	Для Каждого Стр Из ЭтотОбъект["ДанныеМногостраничныхРазделов"].МатрицРискКП Цикл 
		...
		СтрРегТр.ИмяМакета = "ПланГрафик_2024";
