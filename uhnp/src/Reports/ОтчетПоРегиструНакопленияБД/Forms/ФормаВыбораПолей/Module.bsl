...
Процедура ТабличноеПолеИзмеренийВИБПередРазворачиванием(Элемент, Строка, Отказ)
	...
	Если (НЕ (СтрокаРазворота.ТипМетаДанных="Справочник" ИЛИ СтрокаРазворота.ТипМетаДанных="Документ" ИЛИ СтрокаРазворота.ТипМетаДанных="ПланВидовХарактеристик" ИЛИ СтрокаРазворота.ТипЗначения.Количество()>1)) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ТабличноеПолеИзмеренийВИБВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТекстОповещения="ВыбраноПолеОтбора" И Элементы.ТабличноеПолеИзмеренийБД.ТекущиеДанные.ТипЗначения.Количество()>1 Тогда
