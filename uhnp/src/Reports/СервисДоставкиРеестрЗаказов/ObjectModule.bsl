...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ОтборПериодДоставки = ПолучитьЗначениеПараметра(КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы, "ПараметрПериодДоставки");	//СтандартныйПериод
	ОтборПериодСоздания = ПолучитьЗначениеПараметра(КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы, "ПараметрПериодСоздания");	//СтандартныйПериод
	...
	Если КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Свойство("КлючВарианта") Тогда
		Если КомпоновщикНастроек.Настройки.ДополнительныеСвойства.КлючВарианта = "РеестрЗаказовПоКурьеру" Тогда
			...
			Если Не ЗначениеЗаполнено(ОтборКурьерИдентификатор) Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Укажите курьера';
															|en = 'Specify the courier'"), , "Курьер", ,
					ЕстьОшибкиЗаполненияПараметров);
			КонецЕсли;
			Если Не ЗначениеЗаполнено(ОтборПериодДоставки.ДатаНачала) Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Укажите дату начала периода отчета';
															|en = 'Specify the start date of the report period'"), ,
					"ПериодДоставки.ДатаНачала", , ЕстьОшибкиЗаполненияПараметров);
			КонецЕсли;
			Если Не ЗначениеЗаполнено(ОтборПериодДоставки.ДатаОкончания) Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Укажите дату окончания периода отчета';
															|en = 'Specify the end date of the report period'"), ,
					"ПериодДоставки.ДатаОкончания", , ЕстьОшибкиЗаполненияПараметров);
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(ОтборПериодДоставки.ДатаНачала) Тогда
		ПараметрыЗапроса.Вставить("ДатаДоставкиОт", ОтборПериодДоставки.ДатаНачала);
	КонецЕсли;
	Если ЗначениеЗаполнено(ОтборПериодДоставки.ДатаОкончания) Тогда
		ПараметрыЗапроса.Вставить("ДатаДоставкиДо", ОтборПериодДоставки.ДатаОкончания);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ОтборПериодСоздания.ДатаНачала) Тогда
		ПараметрыЗапроса.Вставить("ДатаСозданияОт", ОтборПериодСоздания.ДатаНачала);
	КонецЕсли;
	Если ЗначениеЗаполнено(ОтборПериодСоздания.ДатаОкончания) Тогда
		ПараметрыЗапроса.Вставить("ДатаСозданияДо", ОтборПериодСоздания.ДатаОкончания);
	КонецЕсли;
	...
	ПараметрПериодДоставки = НастройкиКомпоновки.ПараметрыДанных.Элементы.Найти("ПараметрПериодДоставки");
	Если ПараметрПериодДоставки <> Неопределено Тогда
		ПараметрПериодДоставки.Использование = ЗначениеЗаполнено(ПараметрПериодДоставки.Значение.ДатаНачала)
			Или ЗначениеЗаполнено(ПараметрПериодДоставки.Значение.ДатаОкончания);
	КонецЕсли;
	
	ПараметрПериодСоздания = НастройкиКомпоновки.ПараметрыДанных.Элементы.Найти("ПараметрПериодСоздания");
	Если ПараметрПериодСоздания <> Неопределено Тогда
		ПараметрПериодСоздания.Использование = ЗначениеЗаполнено(ПараметрПериодСоздания.Значение.ДатаНачала)
			Или ЗначениеЗаполнено(ПараметрПериодСоздания.Значение.ДатаОкончания);
	КонецЕсли;
