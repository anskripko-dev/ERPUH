...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнициализироватьНастройкиРасшифровки();      
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПустаяСтрока(РегистрПредопределенный) И НЕ Отчет.РежимРасшифровки Тогда
...
&НаСервере
Процедура ИнициализироватьНастройкиРасшифровки()
	
	Если Параметры.ПользовательскиеНастройки = Неопределено 
		ИЛИ НЕ Параметры.РежимРасшифровки Тогда
	    Возврат;
	КонецЕсли;

	Если НЕ(Параметры.ПользовательскиеНастройки.ДополнительныеСвойства.Свойство("КлючВарианта") 
			И Параметры.ПользовательскиеНастройки.ДополнительныеСвойства.КлючВарианта = "ДоходыОтРеализацииТоваровИУслуг") Тогда
	    Возврат;
	КонецЕсли;  
	
	ЗаполнитьЗначенияСвойств(Отчет, Параметры.ПользовательскиеНастройки.ДополнительныеСвойства, "Организация, НачалоПериода, КонецПериода"); 	
	
	НоваяСтрока = Отчет.СписокВидовСубконто.Добавить();
	НоваяСтрока.Предопределенная = Истина;
	НоваяСтрока.ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы;

	НоваяСтрока = Отчет.СписокВидовСубконто.Добавить();
	НоваяСтрока.Предопределенная = Истина;
	НоваяСтрока.ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Номенклатура;
