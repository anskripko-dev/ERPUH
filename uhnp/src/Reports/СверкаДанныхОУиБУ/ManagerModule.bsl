...
Функция ТекстЗапросаПроизводственныеЗатраты()
	...
	|	СебестоимостьТоваров.Организация,
	|	ВЫБОР
	|		КОГДА Аналитика.МестоХранения ССЫЛКА Справочник.ДоговорыКонтрагентов
	|			ТОГДА Аналитика.МестоХранения.Партнер
	|		ИНАЧЕ Аналитика.Договор.Подразделение
	|	КОНЕЦ,
	|	ЕСТЬNULL(АналитикиФинансовогоУчета.Ссылка, ЗНАЧЕНИЕ(Справочник.ГруппыАналитическогоУчетаНоменклатуры.ПустаяСсылка)),
	...
	|	СебестоимостьТоваров.Организация КАК Организация,
	|	ВЫБОР
	|		КОГДА КлючиАналитикиУчетаНоменклатуры.МестоХранения ССЫЛКА Справочник.ДоговорыКонтрагентов
	|			ТОГДА КлючиАналитикиУчетаНоменклатуры.МестоХранения.Партнер
	|		ИНАЧЕ КлючиАналитикиУчетаНоменклатуры.Договор.Подразделение
	|	КОНЕЦ,
	|	ЕСТЬNULL(АналитикиФинансовогоУчета.Ссылка, ЗНАЧЕНИЕ(Справочник.ГруппыАналитическогоУчетаНоменклатуры.ПустаяСсылка)),
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.Производство),
	|	0 КАК НачальныйОстаток,
