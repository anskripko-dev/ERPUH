	...
	Для Каждого ВидСубконто Из Отчет.СписокВидовСубконто Цикл
		Если ЗначениеЗаполнено(ВидСубконто.Значение) Тогда
			...
			Если Поле <> Неопределено Тогда
				...
				Если УчетОСПереопределяемый.ВедетсяУчетОсновныхСредств() И Поле.ТипЗначения.СодержитТип(БухгалтерскийУчетКлиентСерверПереопределяемый.ТипОсновныеСредства()) Тогда
...
Функция ЭлементыПоказателейНалогНаПрибыль(Форма)
	...
	Если БухгалтерскийУчетКлиентСерверПереопределяемый.ИспользуетсяПоказательСверкиНУ() Тогда
