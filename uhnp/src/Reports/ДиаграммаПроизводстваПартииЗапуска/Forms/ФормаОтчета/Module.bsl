...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.ПараметрКоманды) = Тип("СправочникСсылка.ПартииПроизводства") Тогда
		ПартияПроизводства = Параметры.ПараметрКоманды;
	...
	Если ЗначениеЗаполнено(ПартияПроизводства) ИЛИ ЗначениеЗаполнено(КлючПартия) Тогда
		ЗаполнитьВыходныеИзделия();
		УстановитьЗаголовокФормы();
		СформироватьОтчетВФоне();
	Иначе
		ТекстСообщения = НСтр("ru = 'Непосредственное формирование этого отчета не предусмотрено. Перейти к этому отчету можно из отчетов ""Мониторинг заказа"" или ""Структура заказа на производство"".';
								|en = 'Direct generation of this report is not provided. You can access the report from the ""Order monitoring"" or ""Production order structure"" reports.'");
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	
	Элементы.ПартияПроизводства.Видимость = ЗначениеЗаполнено(ПартияПроизводства);
...
Процедура ДиаграммаГантаОбработкаРасшифровки(Элемент, Расшифровки, СтандартнаяОбработка, Дата)
	...
	Если ТипЗнч(Расшифровки) = Тип("Массив")
		...
		Если ТипЗнч(ЗначениеРасшифровки) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
			ПараметрыФормы.Вставить("ПараметрКоманды", ЗначениеРасшифровки);
		Иначе
			ПараметрыФормы.Вставить("Этап", ЗначениеРасшифровки);
