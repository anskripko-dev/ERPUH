...
Процедура ОтчетТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область, ДополнительныеПараметры)
	ПересчитатьДанныеПросрочек(Область);
КонецПроцедуры
...
Процедура ЗагрузитьРезультатФормированияОтчета()
	...
	//НАЧАЛО Нетиповое 
	НомерУдаляемойСтроки = 16;
	Если НЕ СтрокаСодержитДанные(Результат.ТабличныйДокумент, НомерУдаляемойСтроки) Тогда
		УдаляемаяСтрока = Результат.ТабличныйДокумент.Область("R" + Формат(НомерУдаляемойСтроки, "ЧГ=0"));
		Результат.ТабличныйДокумент.УдалитьОбласть(УдаляемаяСтрока, ТипСмещенияТабличногоДокумента.ПоВертикали);
	КонецЕсли;
	//КОНЕЦ Нетиповое
...
&НаСервере
Процедура УстановитьОтборыПоСчетамМСФО()
	
	СчетаМСФО = Новый СписокЗначений;
	СчетаМСФО.Добавить(ПланыСчетов.МСФО.НайтиПоКоду("6120"));
	СчетаМСФО.Добавить(ПланыСчетов.МСФО.НайтиПоКоду("6121"));
	СчетаМСФО.Добавить(ПланыСчетов.МСФО.НайтиПоКоду("6132"));
	СчетаМСФО.Добавить(ПланыСчетов.МСФО.НайтиПоКоду("6133"));
	СчетаМСФО.Добавить(ПланыСчетов.МСФО.НайтиПоКоду("6122"));
	СчетаМСФО.Добавить(ПланыСчетов.МСФО.НайтиПоКоду("6123")); 
	
	ПолеКомпоновкиСчетМСФО = Новый ПолеКомпоновкиДанных("СчетМСФО");
	...
	Для Каждого Элемент Из Отбор.Элементы Цикл
		...
		Если Элемент.ЛевоеЗначение = ПолеКомпоновкиСчетМСФО Тогда
	...
	Если ЭлементОтбора <> Неопределено Тогда
		...
		ЭлементОтбора.ПравоеЗначение	= СчетаМСФО;
		
		//польз. настройки приоритетнее настроек
		...
		Если ЗначениеЗаполнено(ИдентификаторПользовательскойНастройки) Тогда
			ЭлементПользовательскихНастроек = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(ИдентификаторПользовательскойНастройки);
			Если ЭлементПользовательскихНастроек <> Неопределено Тогда
				НастройкаИспользуется = ЭлементПользовательскихНастроек.Использование;
				УстановленныеЗначения = ЭлементПользовательскихНастроек.ПравоеЗначение;
				Если НастройкаИспользуется И УстановленныеЗначения.Количество() = 0 Тогда
					ЭлементПользовательскихНастроек.Использование = НастройкаИспользуется;	
					ЭлементПользовательскихНастроек.ПравоеЗначение	= СчетаМСФО;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;  
...
&НаСервере
Функция СтрокаСодержитДанные(ТабДокумент, НомерСтроки)
	
	Для НомерКолонки = 1 По ТабДокумент.ШиринаТаблицы Цикл
		Ячейка = ТабДокумент.Область(НомерСтроки, НомерКолонки);
		// Сначала проверяем, хранит ли ячейка типизированное значение
		Если Ячейка.СодержитЗначение И ЗначениеЗаполнено(Ячейка.Значение) Тогда
			Возврат Истина;
...
&НаКлиенте
Процедура ПересчитатьДанныеПросрочек(Область)
	
	Колонки = Новый Структура;
	Колонки.Вставить("Дни"                , "9");
	Колонки.Вставить("КоличествоДней"     , "10");
	Колонки.Вставить("ДатаНачалаОтсрочки" , "13");
	Колонки.Вставить("ДатаПросрочки"      , "16");
	Колонки.Вставить("СуммаБУ"            , "19");
	Колонки.Вставить("Сумма60"            , "20");
	Колонки.Вставить("Сумма60_180"        , "21");
	Колонки.Вставить("Сумма180_360"       , "22");
	Колонки.Вставить("Сумма360"           , "23");
	Колонки.Вставить("Проверка"           , "24");
	Колонки.Вставить("СуммаПросроченнойКЗ", "25");
	Колонки.Вставить("Примечание"         , "26");
	
	// Колонки, изменения в которых приводят к изменению в других колонках
	ЗначимыеКолонки = Новый Массив;
	ЗначимыеКолонки.Добавить("10");
	ЗначимыеКолонки.Добавить("13");
	
	НомерТекущейКолонки = Прав(Область.Имя, СтрДлина(Область.Имя) - Найти(Область.Имя,"C"));
	ИмяСтроки = Лев(Область.Имя,Найти(Область.Имя, "C")); 
	
	Если ЗначимыеКолонки.Найти(НомерТекущейКолонки) = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	//НАЧАЛО Блинова М.А. 29.04.2026 Выполнение задач №000057501
	//ДатаОтчета = ТекущаяДата();  
	ДатаОтчета = ТекущаяДатаСервера();
	//КОНЕЦ Блинова М.А. 29.04.2026 Выполнение задач №000057501
	ПараметрКДПериод = Новый ПараметрКомпоновкиДанных("Период");
	ЗначениеПараметраКДПериод = НастройкиОтчета.СвойстваРезультата.ИтоговыеНастройки.ПараметрыДанных.НайтиЗначениеПараметра(ПараметрКДПериод);
	Если ЗначениеПараметраКДПериод = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ЗначениеПараметра = ЗначениеПараметраКДПериод.Значение; 
	Если ТипЗнч(ЗначениеПараметра) = Тип("СтандартнаяДатаНачала") Тогда
		ДатаОтчета = ЗначениеПараметра.Дата;
	Иначе
		ДатаОтчета = ЗначениеПараметра;
	КонецЕсли;
	
	Попытка 
		ДатаНачалаОтсрочки = ДатаИзСтроки(ОтчетТабличныйДокумент.Область(ИмяСтроки + Колонки.ДатаНачалаОтсрочки).Текст);
		Дни                = ОтчетТабличныйДокумент.Область(ИмяСтроки + Колонки.Дни).Текст;
		КоличествоДней     = Число(ОтчетТабличныйДокумент.Область(ИмяСтроки + Колонки.КоличествоДней).Текст);
		СуммаБУ            = Число(ОтчетТабличныйДокумент.Область(ИмяСтроки + Колонки.СуммаБУ).Текст);
		ДатаПросрочки      = ДатаПросрочки(ДатаНачалаОтсрочки,КоличествоДней, Дни);
		РазницаДат         = Число(КонецДня(ДатаПросрочки) - КонецДня(ДатаОтчета)) / 86400;
	Исключение
		ОбщегоНазначенияКлиент.СообщитьПользователю("Не удалось пересчитать даты! Проверьте корректность введенного значения.");
		Возврат;
	КонецПопытки;
	
	ФорматДляНуля = "ЧЦ=15; ЧДЦ=2; ЧН=";
	
	//Очистим суммы
	ОтчетТабличныйДокумент.Область(ИмяСтроки + Колонки.Сумма60).Текст      = Формат(0,ФорматДляНуля);
	ОтчетТабличныйДокумент.Область(ИмяСтроки + Колонки.Сумма60_180).Текст  = Формат(0,ФорматДляНуля);
	ОтчетТабличныйДокумент.Область(ИмяСтроки + Колонки.Сумма180_360).Текст = Формат(0,ФорматДляНуля);
	ОтчетТабличныйДокумент.Область(ИмяСтроки + Колонки.Сумма360).Текст     = Формат(0,ФорматДляНуля);
	ОтчетТабличныйДокумент.Область(ИмяСтроки + Колонки.СуммаПросроченнойКЗ).Текст = Формат(0,ФорматДляНуля);
	
	Если ЗначениеЗаполнено(ДатаПросрочки) Тогда 
	  	ОтчетТабличныйДокумент.Область(ИмяСтроки + Колонки.Проверка).Текст = Формат(0,ФорматДляНуля);
		ОтчетТабличныйДокумент.Область(ИмяСтроки + Колонки.Примечание).Текст = "";
	КонецЕсли;
	
	//Заполним дату просрочки
	ОтчетТабличныйДокумент.Область(ИмяСтроки + Колонки.ДатаПросрочки).Текст = Формат(ДатаПросрочки,"ДФ=dd.MM.yyyy");
	
	//Заполним сумму 
	Если РазницаДат <= 0 Тогда
		ОтчетТабличныйДокумент.Область(ИмяСтроки + Колонки.СуммаПросроченнойКЗ).Текст = Формат(СуммаБУ,ФорматДляНуля);
	ИначеЕсли РазницаДат < 60 Тогда
		ОтчетТабличныйДокумент.Область(ИмяСтроки + Колонки.Сумма60).Текст      = Формат(СуммаБУ,ФорматДляНуля);
	ИначеЕсли РазницаДат < 180 Тогда
		ОтчетТабличныйДокумент.Область(ИмяСтроки + Колонки.Сумма60_180).Текст  = Формат(СуммаБУ,ФорматДляНуля);
	ИначеЕсли РазницаДат < 360 Тогда
		ОтчетТабличныйДокумент.Область(ИмяСтроки + Колонки.Сумма180_360).Текст = Формат(СуммаБУ,ФорматДляНуля);
	Иначе
		ОтчетТабличныйДокумент.Область(ИмяСтроки + Колонки.Сумма360).Текст     = Формат(СуммаБУ,ФорматДляНуля);
	КонецЕсли;
...
Функция ДатаПросрочки(ДатаНачала, КоличествоДней, Дни)
	
	ДатаПросрочки = ДатаНачала + КоличествоДней * 24 * 60 *60;
	
	Если Дни = "Рабочие дни" Тогда  
		
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	&ДатаНачала КАК ДатаНачалаОтсрочки,
		|	&КоличествоДней КАК СрокОтсрочки,
		|	ДанныеПроизводственногоКалендаря.Дата КАК РабочаяДата
		|ПОМЕСТИТЬ ВТ_РабочиеДаты
		|ИЗ
		|	РегистрСведений.ДанныеПроизводственногоКалендаря КАК ДанныеПроизводственногоКалендаря
		|ГДЕ
		|	ДанныеПроизводственногоКалендаря.Дата > &ДатаНачала
		|	И ДОБАВИТЬКДАТЕ(&ДатаНачала, ДЕНЬ, &КоличествоДней * 2) > ДанныеПроизводственногоКалендаря.Дата
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МАКСИМУМ(ВТ_РабочиеДаты.СрокОтсрочки) КАК СрокОтсрочки,
		|	ВТ_РабочиеДаты.ДатаНачалаОтсрочки КАК ДатаНачалаОтсрочки,
		|	ВТ_РабочиеДаты.РабочаяДата КАК РабочаяДата
		|ИЗ
		|	ВТ_РабочиеДаты КАК ВТ_РабочиеДаты
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_РабочиеДаты КАК ВТ_РабочиеДаты1
		|		ПО ВТ_РабочиеДаты.РабочаяДата >= ВТ_РабочиеДаты1.РабочаяДата
		|			И ВТ_РабочиеДаты.ДатаНачалаОтсрочки = ВТ_РабочиеДаты1.ДатаНачалаОтсрочки
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_РабочиеДаты.ДатаНачалаОтсрочки,
		|	ВТ_РабочиеДаты.РабочаяДата
		|
		|ИМЕЮЩИЕ
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТ_РабочиеДаты1.РабочаяДата) = МАКСИМУМ(ВТ_РабочиеДаты.СрокОтсрочки)";
		...
		Если Выборка.Следующий() Тогда
			ДатаПросрочки = Выборка.РабочаяДата;
		КонецЕсли;
	КонецЕсли;
	
	Возврат ДатаПросрочки;
