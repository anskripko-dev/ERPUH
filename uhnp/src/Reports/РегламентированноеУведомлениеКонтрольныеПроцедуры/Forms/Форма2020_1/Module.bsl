...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.КонтрольныеПроцедуры;
...
Процедура СформироватьДеревоСтраниц() Экспорт
	...
	СтрРег.ИндексКартинки = 1;
	СтрРег.ИмяМакета = "КонтрПроцНМ_2020";
	СтрРег.Многостраничность = Истина;
	СтрРег.Многострочность = Ложь;
	СтрРег.УИД = Новый УникальныйИдентификатор;
	СтрРег.ИДНаименования = "КонтрПроцНМ";
	
	СтрРег = СтрРег.ПолучитьЭлементы().Добавить();
	СтрРег.Наименование = "Сведения об" + символы.ПС + "информационной системе";
	СтрРег.ИндексКартинки = 1;
	СтрРег.Многостраничность = Истина;
	СтрРег.Многострочность = Истина;
	...
	СтрРег.Наименование = "Стр. 1";
	СтрРег.ИндексКартинки = 1;
	СтрРег.ИмяМакета = "СвИнфСист_2020";
	СтрРег.Многостраничность = Истина;
	СтрРег.Многострочность = Ложь;
	СтрРег.УИД = Новый УникальныйИдентификатор;
	СтрРег.ИДНаименования = "СвИнфСист";
...
Функция ЗаполнитьАвтоНаСервере()
	...
	ПараметрыОтчета.Вставить("ГодНМ", ЭтотОбъект["ДанныеУведомления"].Титульная["ГодНМ"]);
	ПараметрыОтчета.Вставить("Период", ЭтотОбъект["ДанныеУведомления"].Титульная["Период"]);
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	СтраницаСвИнфСист = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(СтруктураДанных.Контейнер.СвИнфСист.СкопироватьКолонки().Добавить());
	
	Для Каждого КЗ Из ЭтотОбъект["ДанныеМногостраничныхРазделов"] Цикл 
		КЗ.Значение.Очистить();
	КонецЦикла;
	
	Для Каждого КЗ Из СтруктураДанных.Контейнер.КонтрПроцНМ Цикл 
		СтруктураСтраницы = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(КЗ);
		ЭтотОбъект["ДанныеМногостраничныхРазделов"].КонтрПроцНМ.Добавить(СтруктураСтраницы);
		ОтбСтроки = СтруктураДанных.Контейнер.СвИнфСист.НайтиСтроки(Новый Структура("УИДРодителя", СтруктураСтраницы.УИД));
		Если ОтбСтроки.Количество() = 0 Тогда
			СвИнфСист = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(СтраницаСвИнфСист);
			СвИнфСист.УИД = Новый УникальныйИдентификатор;
			СвИнфСист.УИДРодителя = СтруктураСтраницы.УИД;
			ЭтотОбъект["ДанныеМногостраничныхРазделов"].СвИнфСист.Добавить(СвИнфСист);
		Иначе
			Для Каждого Стр Из ОтбСтроки Цикл
				СвИнфСист = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(СтраницаСвИнфСист);
				ЗаполнитьЗначенияСвойств(СвИнфСист, Стр);
				ЭтотОбъект["ДанныеМногостраничныхРазделов"].СвИнфСист.Добавить(СвИнфСист);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	
	Если СтруктураДанных.Контейнер.КонтрПроцНМ.Количество() = 0 Тогда
		ЭтотОбъект["ДанныеМногостраничныхРазделов"].КонтрПроцНМ.Добавить(ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(СтруктураДанных.Контейнер.КонтрПроцНМ.Добавить()));
		ЭтотОбъект["ДанныеМногостраничныхРазделов"].КонтрПроцНМ[0].Значение.УИД = Новый УникальныйИдентификатор;
		СвИнфСист = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(СтраницаСвИнфСист);
		СвИнфСист.УИД = Новый УникальныйИдентификатор;
		СвИнфСист.УИДРодителя = ЭтотОбъект["ДанныеМногостраничныхРазделов"].КонтрПроцНМ[0].Значение.УИД;
		ЭтотОбъект["ДанныеМногостраничныхРазделов"].СвИнфСист.Добавить(СвИнфСист);
	КонецЕсли;
	...
	СтрРег.Многостраничность = Истина;
	СтрРег.Многострочность = Истина;
	Для Каждого Стр Из ЭтотОбъект["ДанныеМногостраничныхРазделов"].КонтрПроцНМ Цикл 
		СтрРегТр = СтрРег.ПолучитьЭлементы().Добавить();
		СтрРегТр.Наименование = "Стр. " + (ЭтотОбъект["ДанныеМногостраничныхРазделов"].КонтрПроцНМ.Индекс(Стр) + 1);
		СтрРегТр.ИндексКартинки = 1;
		СтрРегТр.ИмяМакета = "КонтрПроцНМ_2020";
		СтрРегТр.Многостраничность = Истина;
		СтрРегТр.Многострочность = Ложь;
		СтрРегТр.УИД = Стр.Значение.УИД;
		СтрРегТр.ИДНаименования = "КонтрПроцНМ";
		
		СтрРегДок0 = СтрРегТр.ПолучитьЭлементы().Добавить();
		СтрРегДок0.Наименование = "Сведения об" + символы.ПС + "информационной системе";
		СтрРегДок0.ИндексКартинки = 1;
		СтрРегДок0.Многостраничность = Истина;
		СтрРегДок0.Многострочность = Истина;
		
		Инд = 0;
		Для Каждого Стр2 Из ЭтотОбъект["ДанныеМногостраничныхРазделов"].СвИнфСист Цикл 
