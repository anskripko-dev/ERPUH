			...
			Если ЗначениеЗаполнено(ЗначПоказателя)
				...
				Если ЗначениеЗаполнено(ОблПарногоПоказателя1.Значение) И ЗначениеЗаполнено(ОблПарногоПоказателя2.Значение) Тогда
					ТекстСообщения = "ru = '%1 не допускается одновременное заполнение строк %2 и %3. Значения в строке %3 очищены';
									|en = '%1 не допускается одновременное заполнение строк %2 и %3. Значения в строке %3 очищены'";
				Иначе
					ТекстСообщения = "ru = '%1 не допускается одновременное заполнение строк %2 и %3. Значение в строке %3 очищено';
									|en = '%1 не допускается одновременное заполнение строк %2 и %3. Значение в строке %3 очищено'";
				КонецЕсли;
...
Процедура ДобавитьСтроку(Команда)
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1Прил31" Тогда
		...
		Если ТабличныйДокумент.Области.Найти(ИмяПервойГрафы + "_2") <> Неопределено Тогда
			...
			|en = 'Возможно не более двух групп строк 001-030 в приложении 3.1 к разделу 1'");
		КонецЕсли;
...
Процедура Проверить(Команда)
	...
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) > 2026 Тогда
		...
		|en = 'Проверка контрольных соотношений для 2027 и более поздних 
		| календарных годов будет реализована в будущих версиях программы'"));
	...
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) < 2026 Тогда
		...
		|en = 'Проверка контрольных соотношений для 2025 и более ранних 
		| отчетных периодов не предусмотрена данной формой отчета'"));
