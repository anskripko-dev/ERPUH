...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ПараметрДокумент	= КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "ДокументРезервирования");
	...
	Если ПараметрВыводОтчета.Значение = 0 
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "ДокументРезервирования", НСтр(
			"ru = 'Не заполнено поле ""Лимиты, применяемые для документа"".';
	...
	ИначеЕсли ПараметрВыводОтчета.Значение = 1
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Валюта", НСтр(
			"ru = 'Не заполнено поле ""Валюта"".';
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	РеквизитыДокумента = Новый Структура("Дата, ЖелательнаяДатаПлатежа, Валюта, ХозяйственнаяОперация");
	Если ЗначениеЗаполнено(ПараметрДокумент.Значение) Тогда
		...
		Иначе
			РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПараметрДокумент.Значение,
				"Дата, ЖелательнаяДатаПлатежа, Валюта, ХозяйственнаяОперация, ХозяйственнаяОперацияПоЗарплате");
		...
		Если ЗначениеЗаполнено(РеквизитыДокумента.ЖелательнаяДатаПлатежа) Тогда
			ИспользуемаяДата = РеквизитыДокумента.ЖелательнаяДатаПлатежа;
		Иначе 
			ИспользуемаяДата = РеквизитыДокумента.Дата;
		КонецЕсли;
	Иначе
	...
	Если ПараметрВыводОтчета.Значение = 0 Тогда // В валюте документа
		ПараметрВалюта.Значение = РеквизитыДокумента.Валюта;
	...
	Если ЗначениеЗаполнено(ПараметрДокумент.Значение) Тогда
		...
		Параметры.ХозяйственнаяОперация = РеквизитыДокумента.ХозяйственнаяОперация;
		МассивПараметров.Добавить(Параметры);
	Иначе
		ВидыДокументов = ИсточникиДанныхПовтИсп.ИменаВидовДокументовДляКонтроляЛимитов(
			Перечисления.ТипыДанныхУчета.ДенежныеСредства);
		Для Каждого ВидДокумента Из ВидыДокументов Цикл
			Параметры.ВидДокумента = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ВидДокумента.Значение);
			МассивПараметров.Добавить(ОбщегоНазначения.СкопироватьРекурсивно(Параметры));
		КонецЦикла;
	КонецЕсли;
	ПравилаЛимитовНаДату = Неопределено;
	Для Каждого Параметры Из МассивПараметров Цикл
		...
		Если НастройкиЛимитовНаДату = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Если ПравилаЛимитовНаДату = Неопределено Тогда
			ПравилаЛимитовНаДату = НастройкиЛимитовНаДату.НастройкаПравилЛимитов.ПравилаЛимитов;
		Иначе
			Для Каждого СтрокаПравило Из НастройкиЛимитовНаДату.НастройкаПравилЛимитов.ПравилаЛимитов Цикл
				Если ПравилаЛимитовНаДату.Найти(СтрокаПравило.ПравилоЛимитов, "ПравилоЛимитов") = Неопределено Тогда
					НоваяСтрока = ПравилаЛимитовНаДату.Добавить();
	...
	ВнешниеНаборы = Новый Структура("ИерархияАналитикПоПравилам");
	
	Если ПравилаЛимитовНаДату <> Неопределено И ПравилаЛимитовНаДату.Количество() > 0 Тогда
		...
		ВнешниеНаборы.ИерархияАналитикПоПравилам = ИерархияАналитикПоПравиламЛимитов(ПравилаЛимитовНаДату, ИспользуемаяДата);

	Иначе
		ИерархияАналитикПоПравилам = Новый ТаблицаЗначений;
		Для Каждого ПолеНабораДанных Из СхемаКомпоновкиДанных.НаборыДанных.ИерархияАналитикПоПравилам.Поля Цикл 
			ИерархияАналитикПоПравилам.Колонки.Добавить(ПолеНабораДанных.Поле, ПолеНабораДанных.ТипЗначения, ПолеНабораДанных.Заголовок);
		КонецЦикла;
		
		ВнешниеНаборы.ИерархияАналитикПоПравилам = ИерархияАналитикПоПравилам;
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоВалют") Тогда
		
		КолонкиПересчетаСумм = Новый Массив;
		КолонкиПересчетаСумм.Добавить("Лимит");
		КолонкиПересчетаСумм.Добавить("СуммаЗаявки");
		КолонкиПересчетаСумм.Добавить("СуммаФакт");
		КолонкиПересчетаСумм.Добавить("СуммаПотребленияЛимита");
		
		ПараметрыПересчетаПлана = БюджетированиеСервер.ШаблонПараметровПересчетаВВалюту();
		ПараметрыПересчетаПлана.ИмяНабораДанныхИсточника = "НаборДанных";
		ПараметрыПересчетаПлана.ИсходнаяВалюта = "ВалютаПлан";
		ПараметрыПересчетаПлана.КолонкиПересчета = КолонкиПересчетаСумм;
		ПараметрыПересчетаПлана.ПериодКурса = ИспользуемаяДата;
		ПараметрыПересчетаПлана.ЦелеваяВалюта = ПараметрВалюта.Значение;
		
		БюджетированиеСервер.ДобавитьПересчетДанныхВВалюту(СхемаКомпоновкиДанных, КомпоновщикНастроек, ПараметрыПересчетаПлана);
		
		ПолеРасчетнойВалюты = СхемаКомпоновкиДанных.ВычисляемыеПоля.Найти("Валюта");
		ПолеРасчетнойВалюты.Выражение = "&ЦелеваяВалюта" + "НаборДанных" + "1";
		
	КонецЕсли;
	
	#Область ЗапускФоновогоЗаданияОтраженияЛимитовПоДаннымБюджетирования
	
	ДопСвойства = КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства;
	Если ДопСвойства.Свойство("КоличествоДокументовКОтражениюЛимитовПоДаннымБюджетирования") Тогда
		ДопСвойства.Удалить("КоличествоДокументовКОтражениюЛимитовПоДаннымБюджетирования");
		ПользовательскиеНастройкиМодифицированы = Истина;
	КонецЕсли;
	
	КоличествоДокументов = 0;
	Если РегистрыСведений.ЗаданияКОтражениюЛимитовПоДаннымБюджетирования.ТребуетсяОтражениеЛимитовПоДаннымБюджетирования(
		КоличествоДокументов) Тогда
		ОтложенныеЗадания.Запустить("ЗаданияКОтражениюЛимитовПоДаннымБюджетирования");
		ДопСвойства.Вставить("КоличествоДокументовКОтражениюЛимитовПоДаннымБюджетирования", КоличествоДокументов);
	КонецЕсли;
	...
	Отчеты.ЛимитыПоДаннымБюджетирования.ВывестиАктуальностьОтраженияРезервовПоЛимитам(ДокументРезультат, ДопСвойства);
...
Процедура ПриСозданииНаСервере(ЭтаФорма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Параметры = ЭтаФорма.Параметры; // - Структура -
	...
	Если Параметры.Свойство("ПараметрКоманды") Тогда
		...
		ЭтаФорма.ФормаПараметры.Отбор.Вставить("ДокументРезервирования", Параметры.ПараметрКоманды);
...
Процедура ПередЗагрузкойНастроекВКомпоновщик(Контекст, КлючСхемы, КлючВарианта, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД) Экспорт
	...
	Если КлючВарианта = "ИспользованияЛимитовПоДаннымБюджетированияКонтекст" Тогда
		ПараметрВалюта = СхемаКомпоновкиДанных.Параметры.Найти("Валюта");
...
Функция ИерархияАналитикПоПравиламЛимитов(ПравилаЛимитов, ДатаОтчета)
	...
	Для Каждого ПолеНабораДанных Из СхемаКомпоновкиДанных.НаборыДанных.ИерархияАналитикПоПравилам.Поля Цикл 
		ТаблицаРезультат.Колонки.Добавить(ПолеНабораДанных.Поле, ПолеНабораДанных.ТипЗначения, ПолеНабораДанных.Заголовок);
	КонецЦикла;
	
	Для Каждого КлючИЗначение Из ИерархияАналитикПоПравилам Цикл
		НоваяСтрока = ТаблицаРезультат.Добавить();
		НоваяСтрока.ПравилоЛимитов = КлючИЗначение.Ключ;
		Периодичность = ПравилаЛимитов.Найти(НоваяСтрока.ПравилоЛимитов, "ПравилоЛимитов").Периодичность;
		НоваяСтрока.ПериодЛимитирования = БюджетированиеКлиентСервер.ДатаНачалаПериода(ДатаОтчета, Периодичность);
		МаксимальныйНомерАналитики = 0;
		Для Каждого СтрокаАналитика Из КлючИЗначение.Значение Цикл
			Если СтрокаАналитика.НомерВИерархииСредиИспользуемых = 0 Тогда
				Продолжить;
			КонецЕсли;
			НоваяСтрока["ПутьКАналитике"
				+ СтрокаАналитика.НомерВИерархииСредиИспользуемых] = СтрокаАналитика.ПутьКАналитике;
			НоваяСтрока["ПредставлениеПрочейАналитики"
				+ СтрокаАналитика.НомерВИерархииСредиИспользуемых] = БюджетнаяОтчетностьКлиентСервер.ПредставлениеПрочейАналитикиБюджетирования(
				СтрокаАналитика.ИзмерениеВидАналитики, "Прочие");
			МаксимальныйНомерАналитики = СтрокаАналитика.НомерВИерархииСредиИспользуемых;
		КонецЦикла;
