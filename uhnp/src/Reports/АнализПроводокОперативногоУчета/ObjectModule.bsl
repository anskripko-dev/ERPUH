...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ИсключитьДанныеУправленческойОрганизации(НастройкиОтчета, ПараметрыЗапроса);
...
Процедура УстановитьПараметрПоНастройкеОтбора(ПараметрыЗапроса, ЭлементыОтбора, ИмяПоля, ТекстЗапроса = "")
	...
	ИначеЕсли НЕ ПустаяСтрока(ТекстЗапроса) Тогда
		...
		АдресСхемыКомпоновкиДанных = ПоместитьВоВременноеХранилище(СлужебнаяСхемаКомпоновкиДанных);
...
Процедура ИсключитьДанныеУправленческойОрганизации(НастройкиОтчета, ПараметрыЗапроса)
	
	НеФормироватьОтчет = Ложь;
	...
	ИначеЕсли ПараметрыЗапроса.Организация = Неопределено Тогда
	...
	ИначеЕсли ТипЗнч(ПараметрыЗапроса.Организация) = Тип("Массив") Тогда
		Если ПараметрыЗапроса.Организация.Количество() = 0 Тогда
		ИначеЕсли ПараметрыЗапроса.Организация.Количество() > 1 Тогда
			ИндексЗначения = ПараметрыЗапроса.Организация.Найти(УправленческаяОрганизация);
			Если ИндексЗначения <> Неопределено Тогда
				ПараметрыЗапроса.Организация.Удалить(ИндексЗначения);
			КонецЕсли;
		Иначе
			НеФормироватьОтчет = ПараметрыЗапроса.Организация[0] = УправленческаяОрганизация;
		КонецЕсли;
	Иначе
		НеФормироватьОтчет = ПараметрыЗапроса.Организация = УправленческаяОрганизация;
	КонецЕсли;
	
	Если НеФормироватьОтчет Тогда
		ВызватьИсключение(СтрШаблон(
			НСтр("ru = 'Для организации ""%1"" анализ оперативного учета не предусмотрен';
				|en = 'Operational accounting analysis is not provided for ""%1"" company.'"),
...
Функция ТекстЗапросаНабораДвижений()
	...
	|	ВЫБОР
	|		КОГДА НаборДвижений.ОбъектУчетаДт = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности)
	|		  ИЛИ НаборДвижений.ОбъектУчетаКт = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности)
	|			ТОГДА НаборДвижений.НастройкаХозОперации
	|		КОГДА НаборДвижений.ПланСчетов.ПроводкиБезКорреспонденции
	|		  И НаборДвижений.ОбъектУчетаДт <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ВводОстатков)
	|		  И НаборДвижений.ОбъектУчетаКт <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ВводОстатков)
	...
	|		КОГДА ЧастиДвойнойЗаписи.ДтКт = &Кт
	|			ТОГДА 0
	|		КОГДА НаборДвижений.ОбъектУчетаКт = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности)
	|			ТОГДА 0
	|		КОГДА НаборДвижений.ПланСчетов.ПроводкиБезКорреспонденции
	|			ТОГДА НаборДвижений.СуммаУпрДт
	|		КОГДА НаборДвижений.ИмяРегистраДт <> &ИмяРегистраСлужебныхОбъектовУчета
	|			ТОГДА 0
	|		КОГДА НаборДвижений.ОбъектУчетаДт = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ВводОстатков)
	|			ТОГДА 0
	|		ИНАЧЕ НаборДвижений.СуммаУпрДт
	...
	|			ТОГДА 0
	|		КОГДА НаборДвижений.ОбъектУчетаКт = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ВводОстатков)
	|			ТОГДА 0
	...
	|		И НаборДвижений.ОбъектУчетаДт <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности)
	|		И (НаборДвижений.ПланСчетов.ПроводкиСКорреспонденцией
	|			ИЛИ НаборДвижений.ИмяРегистраДт <> &ИмяРегистраСлужебныхОбъектовУчета
	|			ИЛИ НаборДвижений.ОбъектУчетаДт = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ВводОстатков)))
	|	ИЛИ
	|	(ЧастиДвойнойЗаписи.ДтКт = &Кт
	|		И НаборДвижений.ОбъектУчетаКт <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НесобственныеЦенности)
	|		И (НаборДвижений.ПланСчетов.ПроводкиСКорреспонденцией
	|			ИЛИ НаборДвижений.ИмяРегистраКт <> &ИмяРегистраСлужебныхОбъектовУчета
	|			ИЛИ НаборДвижений.ОбъектУчетаКт = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ВводОстатков)))
	|";
