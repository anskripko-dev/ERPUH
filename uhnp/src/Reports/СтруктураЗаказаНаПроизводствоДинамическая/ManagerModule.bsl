...
Функция ДеревоПолуфабрикатов(Заказ, КлючиПартий, Этап, ПараметрыВывода, Назначение) Экспорт
	...
	Если ПоказыватьЭтапы Тогда
		ТекстыЗапроса.Добавить(ТекстЗапросаКлючиПартийЭтаповОбеспечениеВТ());
		ТекстыЗапроса.Добавить(ТекстЗапросаПроцентОбеспеченияЭтаповВТ(ОтборПартийОбеспечения, "Полуфабрикаты", "Полуфабрикаты"));
		ТекстыЗапроса.Добавить(ТекстЗапросаЭтапы("ВТКлючиПартийЭтаповОбеспечение", "ВТРазмерПартий"));
...
Функция ДеревоЗаказа(Параметры)
	...
	ТекстыЗапроса.Добавить(ТекстЗапросаПроцентОбеспеченияВТ(ОтборПартийОбеспечения, "Полуфабрикаты", "Полуфабрикаты"));
	Если ПоказыватьЭтапы Тогда
		ТекстыЗапроса.Добавить(ТекстЗапросаКлючиПартийЭтаповЗаказаноВТ());
		ТекстыЗапроса.Добавить(ТекстЗапросаКлючиПартийЭтаповОбеспечениеВТ());
		ТекстыЗапроса.Добавить(ТекстЗапросаПроцентОбеспеченияЭтаповВТ(ОтборПартийОбеспеченияЗаказ, "Изделия", ""));
		ТекстыЗапроса.Добавить(ТекстЗапросаПроцентОбеспеченияЭтаповВТ(ОтборПартийОбеспечения, "Полуфабрикаты", "Полуфабрикаты"));
		ТекстыЗапроса.Добавить(ТекстЗапросаЭтапы("ВТКлючиПартийЭтаповЗаказано", "ВТРазмерПартийЗаказ"));
		ТекстыЗапроса.Добавить(ТекстЗапросаЭтапы("ВТКлючиПартийЭтаповОбеспечение", "ВТРазмерПартий"));
	...
	РезультатыЗапроса = Запрос.ВыполнитьПакет();
	Индекс = РезультатыЗапроса.ВГраница();
	ЭтапыЗаказ = ?(ПоказыватьЭтапы, РезультатыЗапроса[Индекс-14].Выгрузить(), Неопределено);
	Этапы = ?(ПоказыватьЭтапы, РезультатыЗапроса[Индекс-13].Выгрузить(), Неопределено);
	ПроцентОбеспеченияЭтапыЗаказ = ?(ПоказыватьЭтапы, РезультатыЗапроса[Индекс-12].Выгрузить(), Неопределено);
	ПроцентОбеспеченияЭтапы = ?(ПоказыватьЭтапы, РезультатыЗапроса[Индекс-11].Выгрузить(), Неопределено);
	Выборка = РезультатыЗапроса[Индекс-10].Выбрать(); // Потребности заказа
	ОбеспечениеЗаказ = РезультатыЗапроса[Индекс-9].Выгрузить(); // Обеспечение потребностей заказа
	ОстаткиЗакрытогоЗаказа = РезультатыЗапроса[Индекс-8].Выгрузить();
	...
	ОбеспечениеЗаказ.Индексы.Добавить("КлючНоменклатура");
	Обеспечение.Индексы.Добавить("КлючНоменклатура");
	ОстаткиПоДругимНазначениям.Индексы.Добавить("Номенклатура, Характеристика, Склад, Назначение");
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаДерева = Результат.Строки.Добавить();
		ЗаполнитьСтрокуИзделие(СтрокаДерева, Выборка, ОбеспечениеЗаказ, ПроцентОбеспеченияЗаказ, ВыпускПартийОбеспеченияЗаказ, ЭтоРежимРасшифровки);
	...
	// Распределение остатков закрытого заказа
	Если ОстаткиЗакрытогоЗаказа.Количество() > 0 Тогда
		Результат.Строки.Сортировать("ЖелаемаяДата Убыв");
		
		ОстаткиЗакрытогоЗаказа.Индексы.Добавить("Номенклатура, Характеристика, Склад, Назначение");
		СтруктураПоиска = Новый Структура("Номенклатура, Характеристика, Склад, Назначение");
		
		Для Индекс = 0 По Результат.Строки.Количество() - 1 Цикл
			СтрокаЗаказа = Результат.Строки[Индекс];
			ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаЗаказа);
			НайденныеСтроки = ОстаткиЗакрытогоЗаказа.НайтиСтроки(СтруктураПоиска);
			Если НайденныеСтроки.Количество() > 0 Тогда
				СтрокаОстатки = НайденныеСтроки[0];
				Количество = Мин(СтрокаЗаказа.Готово, СтрокаОстатки.Остаток);
				
				СтрокаЗаказа.Готово = СтрокаЗаказа.Готово - Количество;
				СтрокаЗаказа.Осталось = СтрокаЗаказа.Требуется - СтрокаЗаказа.Готово;
				
				СтрокаОстатки.Остаток = СтрокаОстатки.Остаток - Количество;
			КонецЕсли;
		КонецЦикла;
		
		Результат.Строки.Сортировать("НомерСтроки");
	КонецЕсли;
...
Процедура ЗаполнитьСтрокуИзделие(СтрокаДерева, ЗначенияЗаполнения, Обеспечение, ПроцентОбеспечения, ВыпускПартийОбеспечения, ЭтоРежимРасшифровки)
...
Функция ТекстЗапросаЗаказаноВТ()
	...
	Возврат
		...
		|	ТабличнаяЧасть.ДатаПотребности  КАК ЖелаемаяДата
		...
		|	ТабличнаяЧасть.КлючНоменклатура,
		...
		|	0                                  КАК Запланировано,
		|	0                                  КАК Запущено,
		|	ТабличнаяЧасть.Требуется           КАК Готово
		|ИЗ
...
Функция ТекстЗапросаКлючиПартийЭтаповЗаказаноВТ()
	
	Возврат
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Обеспечение.КлючПартия КАК КлючПартия,
		|	ВЫБОР
		|		КОГДА Обеспечение.ПартияПроизводства = ЗНАЧЕНИЕ(Справочник.ПартииПроизводства.ПустаяСсылка)
		|			ТОГДА Обеспечение.Спецификация
		|		ИНАЧЕ Обеспечение.ПартияПроизводства
		|	КОНЕЦ                  КАК Ссылка
		|ПОМЕСТИТЬ ВТКлючиПартийЭтаповЗаказано
		|ИЗ
...
Функция ТекстЗапросаЗаказано()
	...
	ТекстЗапросаЗаказано = "
		...
		|	Заказано.Склад                         КАК Склад,
		|	Заказано.Назначение                    КАК Назначение,
		|	Заказано.КлючНоменклатура              КАК КлючНоменклатура,
		...
		|	ПО Обеспечение.ЗаказНаПроизводство = График.ЗаказНаПроизводство
		|		И График.СтатусГрафика = &СтатусРабочийГрафик
		|		И Обеспечение.КлючПартия = График.КлючПартия
	...
	УправлениеПроизводством.ПодстановкаАктуальногоРегистраГрафика(ТекстЗапросаЗаказано);
	
	Возврат ТекстЗапросаЗаказано;
...
Функция ТекстЗапросаЗапущеноВЭтапахВТ()
	...
	ТекстЗапросаЗапущеноВЭтапахВТ = "
		...
		|	ВЫБОР
		|		КОГДА СтруктураЗаказа.Этап ССЫЛКА Документ.ЭтапПроизводства2_2
		|			ТОГДА ВЫРАЗИТЬ(СтруктураЗаказа.Этап КАК Документ.ЭтапПроизводства2_2).ПартияПроизводства
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПартииПроизводства.ПустаяСсылка)
		|	КОНЕЦ                                 КАК ПартияПроизводства,
		|	СтруктураЗаказа.ЗаказНаПроизводство   КАК ЗаказНаПроизводство,
		...
		|	ПартииЗапуска.ПартияПроизводства    КАК ПартияПроизводства,
		...
		|	ПартииЗапуска.ПартияПроизводства,
		...
		|	ДАТАВРЕМЯ(1,1,1) КАК ЖелаемаяДата,
		|	0                КАК НомерСтроки,
		|	0                КАК Требуется
		|ПОМЕСТИТЬ ВТДанныеЗакрытогоЗаказа";
	
	УправлениеПроизводством.ПодстановкаАктуальногоРегистраГрафика(ТекстЗапросаЗапущеноВЭтапахВТ);
	
	Возврат ТекстЗапросаЗапущеноВЭтапахВТ;
КонецФункции

Функция ТекстЗапросаПотребностьВТ(УсловиеОтбора)
	
	ТекстЗапросаПотребностьВТ = СтрЗаменить( 
		"ВЫБРАТЬ
		|	СтруктураЗаказа.Номенклатура        КАК Номенклатура,
		|	СтруктураЗаказа.Характеристика      КАК Характеристика,
		|	СтруктураЗаказа.Склад               КАК Склад,
		...
		|	МАКСИМУМ(ЕСТЬNULL(График.Начало, ДАТАВРЕМЯ(1,1,1))) КАК ЖелаемаяДата
	...
	УправлениеПроизводством.ПодстановкаАктуальногоРегистраГрафика(ТекстЗапросаПотребностьВТ);
	
	Возврат ТекстЗапросаПотребностьВТ;
...
Функция ТекстЗапросаОбеспечениеВТ()
	...
	ТекстЗапросаОбеспечениеВТ = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Т.КлючПартия КАК КлючПартия
		...
		|		ВЫБОР
		|			КОГДА СтруктураЗаказа.ЗаказНаПоступление ССЫЛКА Документ.ЭтапПроизводства2_2
		|				ТОГДА ЕСТЬNULL(ГрафикПартийЭтапов.Начало, ДАТАВРЕМЯ(1,1,1))
		|			КОГДА СтруктураЗаказа.ВидСтроки = ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.Зарезервировано)
		|				ТОГДА ДАТАВРЕМЯ(1,1,1)
		|			ИНАЧЕ ЕСТЬNULL(График.Начало, ДАТАВРЕМЯ(1,1,1))
		|		КОНЕЦ)                           КАК Начало,
		|
		|	МАКСИМУМ(
		|		ВЫБОР
		|			КОГДА СтруктураЗаказа.ЗаказНаПоступление ССЫЛКА Документ.ЭтапПроизводства2_2
		|				ТОГДА ЕСТЬNULL(ГрафикПартийЭтапов.Окончание, ДАТАВРЕМЯ(1,1,1))
		|			КОГДА СтруктураЗаказа.ВидСтроки = ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.Зарезервировано)
		|				ТОГДА ДАТАВРЕМЯ(1,1,1)
		|			ИНАЧЕ ЕСТЬNULL(График.Окончание, ДАТАВРЕМЯ(1,1,1))
		|		КОНЕЦ)                           КАК Окончание
		|ПОМЕСТИТЬ ВТОбеспечение
		|ИЗ
		|	ВТСтруктураЗаказаОбеспечение КАК СтруктураЗаказа
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикЭтаповПроизводства2_2 КАК ГрафикЭтапов
		|	ПО СтруктураЗаказа.ЗаказНаПоступление ССЫЛКА Документ.ЭтапПроизводства2_2
		|		И ГрафикЭтапов.СтатусГрафика = &СтатусРабочийГрафик
		|		И СтруктураЗаказа.ЗаказНаПоступление = ГрафикЭтапов.Этап
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикЭтаповПроизводства2_2 КАК ГрафикПартийЭтапов
		|	ПО ГрафикЭтапов.ЗаказНаПроизводство = ГрафикПартийЭтапов.ЗаказНаПроизводство
		|		И ГрафикПартийЭтапов.СтатусГрафика = &СтатусРабочийГрафик
		|		И ГрафикЭтапов.КлючПартия = ГрафикПартийЭтапов.КлючПартия
		|		И ГрафикПартийЭтапов.Этап= ЗНАЧЕНИЕ(Справочник.ЭтапыПроизводства.ПустаяСсылка)
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикЭтаповПроизводства2_2 КАК График
		|	ПО СтруктураЗаказа.ЗаказНаПроизводство = График.ЗаказНаПроизводство
		|		И График.СтатусГрафика = &СтатусРабочийГрафик
	...
	УправлениеПроизводством.ПодстановкаАктуальногоРегистраГрафика(ТекстЗапросаОбеспечениеВТ);
	
	Возврат ТекстЗапросаОбеспечениеВТ;
	
КонецФункции

Функция ТекстЗапросаКлючиПартийЭтаповОбеспечениеВТ()
	...
	Возврат
		...
		|		ИНАЧЕ Обеспечение.ПартияПроизводства
		|	КОНЕЦ                  КАК Ссылка
		|ПОМЕСТИТЬ ВТКлючиПартийЭтаповОбеспечение
		|ИЗ
		|	ВТОбеспечение КАК Обеспечение
		|ГДЕ
		|	НЕ Обеспечение.ЭтоРезерв
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Ссылка";
...
Функция ТекстЗапросаЭтапы(ИмяТаблицыКлючи, ИмяТаблицыРазмерПартий)
	...
	Если УправлениеПроизводствомПовтИсп.ИспользуетсяГрафикПроизводства() Тогда
		ТекстЗапроса =
			...
			|	ПО Партии.КлючПартия = РазмерПартий.КлючПартия
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
