...
Процедура ЗарегистрироватьИзменениеОбъекта(Источник, Отказ, ДополнительныеПараметры = Неопределено)
	...
	Если НЕ Метаданные.ПланыОбмена.НалоговыйМониторинг.Состав.Содержит(Источник.Метаданные()) Тогда
		Возврат;
	КонецЕсли;
	
	МассивУзлов = ПолучитьУзлыПоПравилу(Источник, УзлыПланаОбмена, (ДополнительныеПараметры <> Неопределено) И ДополнительныеПараметры.Свойство("РежимЗаписи"));

	Если МассивУзлов.Количество() Тогда 
		ОбменДаннымиСобытия.ДополнитьПолучателей(Источник, МассивУзлов);
	КонецЕсли;
