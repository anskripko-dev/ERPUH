...
Процедура ОткрытьСписокОтправляемыхДоговоровЭквайринга(Команда)
	...
	Если Не Объект.ИспользоватьОтборПоОрганизациям  Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнен фильтр по организациям.';
														|en = 'Filter by companies is required.'"),, "Объект.ИспользоватьОтборПоОрганизациям");
	...
	ПараметрыФормы.Вставить("ЗаголовокФормыВыбора",                   НСтр("ru = 'Выберите договоры подключения (к платежным системам)';
																			|en = 'Select payment system contracts'"));
	ПараметрыФормы.Вставить("МассивВыбранныхЗначений",                СформироватьМассивВыбранныхЗначений(ПараметрыФормы));
	ПараметрыФормы.Вставить("ПараметрыВнешнегоСоединения",            Неопределено);
	ПараметрыФормы.Вставить("КоллекцияФильтров",                      КоллекцияФильтров);
...
Процедура СформироватьЗаголовокОтбора(Количество,Наименование,НовыйЗаголовок)
	...
	ИначеЕсли Количество >= 2 Тогда 
		...
		ШаблонПредставления = Нстр("ru = '%1 (и еще %2).';
									|en = '%1 (and %2 more).'", ОбщегоНазначения.КодОсновногоЯзыка());
