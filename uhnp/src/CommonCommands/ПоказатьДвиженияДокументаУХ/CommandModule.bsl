...
Функция ПолучитьПараметрыДокумента(Ссылка)
	...
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоЕХ() Тогда 
		ОбщийМодульБухгалтерскийУчет = ОбщегоНазначения.ОбщийМодуль("БухгалтерскийУчетКлиентСерверУХ");
	Иначе	                                                                          
		ОбщийМодульБухгалтерскийУчет = ОбщегоНазначения.ОбщийМодуль("БухгалтерскийУчетКлиентСерверПереопределяемый");
	КонецЕсли;	
	Результат.Вставить("РегОперация",	ОбщийМодульБухгалтерскийУчет.ЭтоРегламентнаяОперация(Ссылка));  
...
Процедура ОткрытьФормуПоТипуДокумента(ДокументСсылка, ПараметрыВыполненияКоманды)
	...
	ТипыУХ.Добавить(Тип("ДокументСсылка.ПокрытиеПотребностиВФинансировании"));
	ТипыУХ.Добавить(Тип("ДокументСсылка.ЗаявкаНаПривлечениеФинансирования"));
	ТипыУХ.Добавить(Тип("ДокументСсылка.ПодтверждениеЗакупки"));
	ТипыУХ.Добавить(Тип("ДокументСсылка.Лот"));
	ТипыУХ.Добавить(Тип("ДокументСсылка.СтрокаПланаЗакупок"));
 	ТипыУХ.Добавить(Тип("ДокументСсылка.ПротоколОтменыЗакупочнойПроцедуры"));
