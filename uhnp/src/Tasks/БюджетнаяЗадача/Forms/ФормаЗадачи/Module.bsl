...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Если ТаблицаНастроекДействия <> Неопределено Тогда
			...
			Если Объект.Действие = Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.ВводБюджетов
				Или Объект.Действие = Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.КорректировкаИсправлениеБюджетов Тогда
...
Процедура СоздатьСвязаннуюЗадачу(Команда)
	...
	ОчиститьСообщения();
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Перед созданием связанной задачи необходимо записать объект';
														|en = 'Before creating a linked task, save the object'"));
		Возврат;
	КонецЕсли;
...
Функция ЧтениеИБлокировкаЗадачи()
	...
	Исключение
		...
		ТекстОшибкиКраткий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстОшибкиКраткий, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибкиКраткий);
...
&НаСервереБезКонтекста
Функция ИмяФормыНастройкиНаСервере(Знач Действие)
	
	Если Действие = Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.КорректировкаИсправлениеБюджетов Тогда
...
Процедура УстановитьПредставлениеДействия()
	...
	ИначеЕсли Действие = Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.КорректировкаИсправлениеБюджетов Тогда
		Документ = ТекущийДокумент(Объект.СписокДокументов);
		ТекстДействия = НСтр("ru = '<Скорректировать бюджет>';
							|en = '<Adjust budget>'");
		ПредставлениеДействия = 
			?(ЗначениеЗаполнено(Документ), Строка(Документ), ТекстДействия);
...
Процедура СформироватьКонтрольныйОтчет()
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("СформироватьКонтрольныйОтчетОповещение", ЭтотОбъект);
...
&НаКлиенте
Процедура СформироватьКонтрольныйОтчетОповещение(Результат, ДополнительныеПараметры) Экспорт
...
&НаСервере
Процедура ДействияПослеФормированияБюджетов(АдресХранилищаФормированиеБюджетов)
	...
	Если РезультатВыполнения.ЕстьОшибки Тогда
		ОбщегоНазначения.СообщитьПользователю(РезультатВыполнения.ТекстОшибки);
	Иначе
		Объект.СписокДокументов.Очистить();
...
&НаСервере
Процедура ЗагрузитьПодготовленныеДанные(Знач АдресХранилища)
...
Процедура СформироватьАвтоматическиЗавершение(РезультатВопроса, Параметры=Неопределено) Экспорт
	...
	ПараметрыОжидания.ОповещениеОПрогрессеВыполнения =
		Новый ОписаниеОповещения("СформироватьАвтоматическиВывестиПрогресс", ЭтотОбъект);
	
	ОповещениеОЗавершении = Новый ОписаниеОповещения("СформироватьАвтоматическиОповещение", ЭтотОбъект);
...
&НаКлиенте
Процедура СформироватьАвтоматическиОповещение(Результат, ДполнительныеПараметры) Экспорт
	...
	Попытка
		ДействияПослеФормированияБюджетов(Результат.АдресХранилища);
		РазвернутьДеревоДокументов();
	Исключение
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаРезультат;
...
&НаКлиенте
Процедура СформироватьАвтоматическиВывестиПрогресс(Результат, ДполнительныеПараметры) Экспорт
	...
	ФоновоеЗаданиеПроцент = Результат.Процент;
