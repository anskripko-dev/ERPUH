...
Процедура ФормированиеБюджетныхЗадач(МодельБюджетирования = Неопределено, ПроцессПодготовкиБюджетов=Неопределено) Экспорт
	...
	|	И &УсловиеПоСсылкеБюджетногоПроцесса
	...
	Если ПроцессПодготовкиБюджетов <> Неопределено Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоСсылкеБюджетногоПроцесса",
			"БюджетнаяЗадача.ПроцессПодготовкиБюджетов = &БюджетныйПроцесс");
		Запрос.УстановитьПараметр("ЭтапПроцесса",
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПроцессПодготовкиБюджетов, "ГруппаЭтапов"));
		Запрос.УстановитьПараметр("БюджетныйПроцесс", ПроцессПодготовкиБюджетов);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоГруппеЭтаповПроцесса", "ИСТИНА");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоСсылкеБюджетногоПроцесса", "ИСТИНА");
	КонецЕсли; 
...
// Возвращает признак наличия аналогичной задачи с признаком Неактивна = Истина.
// Это необходимо, в частности, для того, чтобы не создавать новые документы, а использовать созданные ранее
//
// Параметры:
//  Задача - ЗадачаСсылка.БюджетнаяЗадача - проверяемая бюджетная задача
//
// Возвращаемое значение:
//  Булево - Истина, если найдена аналогичная неактивная задача
Функция ЕстьНеактивнаяАналогичнаяЗадача(Задача) Экспорт
	...
	|ИЗ
	|	Задача.БюджетнаяЗадача КАК БюджетнаяЗадача
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Задача.БюджетнаяЗадача КАК БюджетнаяЗадача_Неактивная
	|		ПО БюджетнаяЗадача.ЭтапПодготовкиБюджетов = БюджетнаяЗадача_Неактивная.ЭтапПодготовкиБюджетов
	|			И БюджетнаяЗадача.ЭтапПодготовкиБюджетовОснование = БюджетнаяЗадача_Неактивная.ЭтапПодготовкиБюджетовОснование
	|			И БюджетнаяЗадача.ПроцессПодготовкиБюджетов = БюджетнаяЗадача_Неактивная.ПроцессПодготовкиБюджетов
	|			И БюджетнаяЗадача.Период = БюджетнаяЗадача_Неактивная.Период
	|			И БюджетнаяЗадача.Ссылка <> БюджетнаяЗадача_Неактивная.Ссылка
	|ГДЕ
	|	БюджетнаяЗадача.Ссылка = &Задача
	|	И БюджетнаяЗадача_Неактивная.Неактивна
	|	И НЕ БюджетнаяЗадача_Неактивная.ПометкаУдаления";
...
Функция СоздатьЭкземплярБюджета(ПараметрыНовогоДокумента, СтруктураДействия, Проводить = Ложь, НеПроверятьСуществующиеДокументы = Ложь) Экспорт
	...
	Если МассивДокументы = Неопределено Тогда
		...
		ЗаполнениеНовогоДокумента = Не (СтруктураДействия.Действие
			= Перечисления.ТипыДействийЭкземпляровБюджетов.КорректировкаПоСтатьямБюджета);
	Иначе
	...
	Если Проводить Тогда
		Проводить = (СтруктураДействия.Действие = Перечисления.ТипыДействийЭкземпляровБюджетов.ВводЭкземпляраБюджета);
	КонецЕсли;
...
Функция ВыполнитьДействие(Объект, НастройкаДействия, Записывать = Ложь, НеПроверятьСуществующиеДокументы = Ложь, ТекстОшибки = "") Экспорт
	...
	Если Задачи.БюджетнаяЗадача.НеобходимоСозданиеНовогоДокумента(Документ, Действие, ВидимостьСписка) Тогда
		...
		Если НеПроверятьСуществующиеДокументы Тогда
			НеПроверятьСуществующиеДокументы = Не Задачи.БюджетнаяЗадача.ЕстьНеактивнаяАналогичнаяЗадача(Объект.Ссылка);
		КонецЕсли;
		...
		Попытка
			...
			Если Действие = Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.ВводБюджетов
				Или Действие = Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.КорректировкаИсправлениеБюджетов Тогда
...
Функция ПараметрыВводаБюджетовПоНастройкамДействия(Знач ТаблицаДействий,
	...
	Иначе
		...
		ВидыБюджетов.Колонки.Добавить("Действие", Новый ОписаниеТипов("ПеречислениеСсылка.ТипыДействийЭкземпляровБюджетов"));
	...
	"ВЫБРАТЬ
		...
		|	ВидыБюджетов.Действие КАК Действие,
		...
		|	ВЫБОР
		|		КОГДА ТаблицаВидыБюджетов.Действие = ЗНАЧЕНИЕ(Перечисление.ТипыДействийЭкземпляровБюджетов.ПустаяСсылка)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыДействийЭкземпляровБюджетов.ВводЭкземпляраБюджета)
		|		ИНАЧЕ ТаблицаВидыБюджетов.Действие
		|	КОНЕЦ КАК Действие,
	...
	"ВЫБРАТЬ
		...
		|	ПараметрыВидовБюджетов.Действие КАК Действие,
		|	ВЫБОР
		...
		|	ПараметрыВидовБюджетов.Действие,
		|	ВЫБОР
		|		КОГДА ПараметрыВидовБюджетов.Периодичность = ЗНАЧЕНИЕ(Перечисление.Периодичность.Неделя)
		|			ТОГДА НАЧАЛОПЕРИОДА(КалендарныеГрафики.ДатаГрафика, НЕДЕЛЯ)
		|		КОГДА ПараметрыВидовБюджетов.Периодичность = ЗНАЧЕНИЕ(Перечисление.Периодичность.Декада)
	...
	"ВЫБРАТЬ
		...
		|	И ЭкземплярБюджета.Действие = ТабПериодов.Действие
		...
		|			И ЭкземплярБюджета.Действие = ПараметрыВидовБюджетов.Действие
		...
		|	ТабВидыБюджетов.Действие КАК Действие,
		...
		|			И ТабПериодов.Действие = ТабВидыБюджетов.Действие
		...
		|			И ТабВидыБюджетов.Действие = СписокДокументов.Действие
		...
		|			И ТабВидыБюджетов.Действие = ДокументыПоЗадаче.Действие
		...
		|		И ЭкземплярБюджета.Действие = ВидыБюджетов.Действие
		|ГДЕ
		|	ЭкземплярБюджета.Ссылка В(&СписокДокументов)
		|
		|УПОРЯДОЧИТЬ ПО
			...
			Для Каждого СтрокаАналитикиПериода Из СтрокиАналитикиПериода Цикл
				Если СтрокаАналитикиПериода.Строки.Количество() > 0 Тогда
					Для Каждого СтрокаАналитикиПериода1 Из СтрокаАналитикиПериода.Строки Цикл
						...
						КонецЕсли;
						СообщитьПрогресс(ОбработаноСтрок, ВсегоОбрабатываемыхСтрок, ПредыдущийПрогресс);
					КонецЦикла;
				Иначе
...
Процедура СообщитьПрогресс(ОбработаноСтрок, ВсегоОбрабатываемыхСтрок, ПредыдущийПрогресс)
	
	ОбработаноСтрок = ОбработаноСтрок + 1;
	ПроцентВыполнения = Цел((ОбработаноСтрок / ВсегоОбрабатываемыхСтрок) * 100);
	Если ПроцентВыполнения <> ПредыдущийПрогресс Тогда
		ДлительныеОперации.СообщитьПрогресс(ПроцентВыполнения);
		ПредыдущийПрогресс = ПроцентВыполнения;
	КонецЕсли;
...
Процедура СформироватьБюджетныеЗадачиПоМодели(МодельБюджетирования, ПроцессПодготовкиБюджетов = Неопределено, СозданныеЗадачи = Неопределено)
	
	ГруппаЭтаповПроцесса = Неопределено;
	Если ЗначениеЗаполнено(ПроцессПодготовкиБюджетов) Тогда
		ГруппаЭтаповПроцесса = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПроцессПодготовкиБюджетов, "ГруппаЭтапов");
	КонецЕсли;
	ТаблицаЗадач = Справочники.МоделиБюджетирования.ПолучитьБюджетныеЗадачи(МодельБюджетирования, ГруппаЭтаповПроцесса,
		Неопределено, ТекущаяДатаСеанса(), Истина, Истина);
	...
	Для Каждого ЗадачаКФормированию Из НомерЗадачКФормированию Цикл
		...
		Если Задачи.БюджетнаяЗадача.НеобходимоСозданиеНовогоДокумента(Неопределено, ЗадачаОбъект.Действие, Ложь) Тогда
			...
			Если ЗадачаОбъект.Действие = Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.ВводБюджетов
				Или ЗадачаОбъект.Действие
				= Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.КорректировкаИсправлениеБюджетов Тогда
				ЗадачаОбъект.КСозданиюДокументов = Истина;
			ИначеЕсли ЗадачаОбъект.Действие
				= Перечисления.ТипыДействийЭтаповПодготовкиБюджетов.УстановкаЗначенийНефинансовыхПоказателей Тогда
				ЗадачаОбъект.КСозданиюДокументов = Истина;
			КонецЕсли;
...
Процедура СформироватьОповещенияПоБюджетнымЗадачам()
	...
	"ВЫБРАТЬ
		...
		|	И НЕ Задача_БюджетнаяЗадача.Неактивна
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("2b11699a-2b7c-41a6-abda-c5e00e582b2c");
	...
	Читаемые.Добавить(Метаданные.Задачи.БюджетнаяЗадача.ПолноеИмя());
	Обработчик.ЧитаемыеОбъекты = СтрСоединить(Читаемые, ",");
	
	Изменяемые = Новый Массив;
	Изменяемые.Добавить(Метаданные.Задачи.БюджетнаяЗадача.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Задачи.БюджетнаяЗадача.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры = Неопределено) Экспорт
	...
	ИмяОбъектаМетаданных = Метаданные.Задачи.БюджетнаяЗадача.ПолноеИмя();
	...
	|	Задача.БюджетнаяЗадача КАК БюджетнаяЗадача
	|ГДЕ
	|	БюджетнаяЗадача.Действие = ЗНАЧЕНИЕ(Перечисление.ТипыДействийЭтаповПодготовкиБюджетов.ВводБюджетов)";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ОбъектМетаданных = Метаданные.Задачи.БюджетнаяЗадача;
	...
	Для Каждого Выборка Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Задача = Выборка.Ссылка; // ЗадачаСсылка.БюджетнаяЗадача - 
			...
			Если ТаблицаВидыБюджетов.Колонки.Найти("Действие") = Неопределено Тогда
				...
				ЗадачаОбъект.НастройкаДействия = Новый ХранилищеЗначения(НастройкаДействия);
		...
		Исключение
			...
			Шаблон = НСтр("ru = 'Не удалось записать бюджетную задачу %1 по причине: %2';
							|en = 'Cannot save the %1 budget task. Reason: %2'");
