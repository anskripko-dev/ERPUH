		...
		Если Начисления.Количество() > 0 И Начисления.Найти(Ссылка) = Неопределено Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Начисление с назначением ""%1"" и основным видом расчета ""%2"" уже существует';
					|en = 'Accrual with the ""%1"" assignment and ""%2"" main calculation type already exists'"),
				КатегорияНачисленияИлиНеоплаченногоВремени,
				ОсновнойВидРасчета);
			ОбщегоНазначения.СообщитьПользователю(
		...
		Если Начисления.Количество() > 0 И Начисления.Найти(Ссылка) = Неопределено Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Начисление с назначением ""%1"", редактируемое в отдельных полях и основным видом расчета ""%2"" уже существует';
					|en = 'Начисление с назначением ""%1"", редактируемое в отдельных полях и основным видом расчета ""%2"" уже существует'"),
				КатегорияНачисленияИлиНеоплаченногоВремени,
				ОсновнойВидРасчета);
			ОбщегоНазначения.СообщитьПользователю(
				ТекстСообщения, , "Объект.ОсновнойВидРасчета", , Отказ);
