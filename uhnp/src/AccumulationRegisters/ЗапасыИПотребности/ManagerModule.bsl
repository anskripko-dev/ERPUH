...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Удаляет движения документов ""Корректировка отчета переработчика"".
		...
		|en = 'Deletes register records of the ""Adjust subcontracting services notification received"" documents.
		|Deletes register records of the ""Subcontracting services notification received"" version 2.5 document for items with the ""Work"" type if the document is based on the order.
		|Corrects the receipt schedule for production orders.
		|Fills the ""Order to ship"" attribute in records of orders with the ""Ship"" action'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	МассивТиповРегистраторов.Добавить(Метаданные.Документы.ЗаказМатериаловВПроизводство.ПолноеИмя());
	МассивТиповРегистраторов.Добавить(Метаданные.Документы.ЗаказПереработчику2_5.ПолноеИмя());
	
	//++ Устарело_Переработка24
	
	МассивТиповРегистраторов.Добавить(Метаданные.Документы.ЗаказПереработчику.ПолноеИмя());
	...
	МассивТиповРегистраторов.Добавить(Метаданные.Документы.ЗаказНаРемонт.ПолноеИмя());
	МассивТиповРегистраторов.Добавить(Метаданные.Документы.ЗаказДавальца2_5.ПолноеИмя());
	МассивТиповРегистраторов.Добавить(Метаданные.Документы.ЭтапПроизводства2_2.ПолноеИмя());
	
	//++ Устарело_Переработка24
	
	МассивТиповРегистраторов.Добавить(Метаданные.Документы.ЗаказДавальца.ПолноеИмя());
	
	//-- Устарело_Переработка24
	
	//++ Устарело_Производство21
	
	МассивТиповРегистраторов.Добавить(Метаданные.Документы.ЗаказНаПроизводство.ПолноеИмя());
	МассивТиповРегистраторов.Добавить(Метаданные.Документы.КорректировкаЗаказаМатериаловВПроизводство.ПолноеИмя());
	...
	МассивТиповРегистраторов.Добавить(Метаданные.Документы.КорректировкаПриобретения.ПолноеИмя());
	...
	МассивТиповРегистраторов.Добавить(Метаданные.Документы.ЗаказНаПроизводство2_2.ПолноеИмя());
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Исправление ошибки обработчика на версию 2.5.20.34, когда открывался график поступления
	// по ранее закрытым заказам, которые перепроводились для заполнения реквизита Отгружаемый заказ.
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Остатки.Заказ КАК Регистратор
	|ИЗ
	...
	|			ТИП(Документ.ЗаказПереработчику2_5))) КАК Остатки
	|ГДЕ
	|	Остатки.ЗаказаноОстаток > 0
	|	И ИСТИНА В(
	|		ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА КАК ЕстьЗаписиЗакрытияПоНакладным
	|		ИЗ
	|			РегистрНакопления.ЗапасыИПотребности КАК ДвиженияЗакрытия
	|		ГДЕ
	|			ДвиженияЗакрытия.Заказ = Остатки.Заказ
	|				И ДвиженияЗакрытия.ЗакрытьГрафикПоступления > 0
	|				И ДвиженияЗакрытия.Активность)
	|	И НЕ ИСТИНА В(
	|		ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА КАК ЕстьЗаписиЗакрытияПоЗаказу
	|		ИЗ
	|			РегистрНакопления.ЗапасыИПотребности КАК ДвиженияЗакрытия
	|		ГДЕ
	|			ДвиженияЗакрытия.Регистратор = Остатки.Заказ
	|				И ДвиженияЗакрытия.Заказано < 0
	|				И ДвиженияЗакрытия.Активность)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	Движения.Регистратор КАК Регистратор
	|ИЗ
	|	Документ.КорректировкаПриобретения КАК КорректировкиОтчетов
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ЗапасыИПотребности КАК Движения
	|	ПО Движения.Регистратор = КорректировкиОтчетов.Ссылка
	|	И Движения.Активность
	|ГДЕ
	|	КорректировкиОтчетов.ХозяйственнаяОперация =
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаОтчетаПереработчика)
	...
	// Исправление графика поступления для заказов на производство. 
	// График поступления не формируется для закрытых заказов на производство, для заказов со статической структурой график поступления формируется неподтвержденным. 
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ЗапасыИПотребности КАК Движения
	|	ПО Движения.Регистратор = ЗаказНаПроизводство2_2.Ссылка
	|	И Движения.Активность
	|ГДЕ
	|	((Движения.Заказано <> 0 ИЛИ Движения.НеОбеспечивать <> 0)
	|				И ЗаказНаПроизводство2_2.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовНаПроизводство2_2.Закрыт)
	|			ИЛИ Движения.Поступит <> 0
	|				И НЕ ЗаказНаПроизводство2_2.ДинамическаяСтруктура)
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Описание = НСтр("ru = 'Не удалось заполнить реквизит ""Отгружаемый заказ"" в регистре накопления ""Запасы и потребности"".';
					|en = 'Cannot fill the ""Order to ship"" attribute in the ""Inventory and demands"" accumulation register.'");
	...
	Для Каждого СтрокаТаблицы Из ОбновляемыеДанные Цикл
		...
		ЗаполнитьОтгружаемыйЗаказРегистратором =
			ТипЗнч(СтрокаТаблицы.Регистратор) = Тип("ДокументСсылка.ЗаказКлиента")
				...
				Или ТипЗнч(СтрокаТаблицы.Регистратор) = Тип("ДокументСсылка.ЗаказМатериаловВПроизводство")
				
				// -- НЕ УТ
				
				// ++ НЕ УТКА
				
				Или ТипЗнч(СтрокаТаблицы.Регистратор) = Тип("ДокументСсылка.ЗаказНаРемонт")
				Или ТипЗнч(СтрокаТаблицы.Регистратор) = Тип("ДокументСсылка.ЗаказДавальца2_5")
				
				//++ Устарело_Переработка24
				
				Или ТипЗнч(СтрокаТаблицы.Регистратор) = Тип("ДокументСсылка.ЗаказДавальца")
				
				//-- Устарело_Переработка24
				
				//++ Устарело_Производство21
				
				Или ТипЗнч(СтрокаТаблицы.Регистратор) = Тип("ДокументСсылка.ЗаказНаПроизводство")
		...
		ЗаполнитьОтгружаемыйЗаказЗапросом = ЛОЖЬ
				...
				Или ТипЗнч(СтрокаТаблицы.Регистратор) = Тип("ДокументСсылка.ЭтапПроизводства2_2")
				
				//++ Устарело_Производство21
				
				Или ТипЗнч(СтрокаТаблицы.Регистратор) = Тип("ДокументСсылка.КорректировкаЗаказаМатериаловВПроизводство")
		...
		Если ЗаполнитьОтгружаемыйЗаказРегистратором Или ЗаполнитьОтгружаемыйЗаказЗапросом Тогда
			...
			Попытка
				...
				ИначеЕсли ЗаполнитьОтгружаемыйЗаказЗапросом Тогда
					...
					Если ТипЗнч(СтрокаТаблицы.Регистратор) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
						
						ЗапросРеквизита = Новый Запрос(
							"ВЫБРАТЬ
							|	ВЫБОР
							|		КОГДА Этапы.ЗаказПереработчику.Проведен
							|			ТОГДА Этапы.ЗаказПереработчику
							|		ИНАЧЕ Этапы.Ссылка
							|	КОНЕЦ КАК ОтгружаемыйЗаказ
							|ИЗ
							|	Документ.ЭтапПроизводства2_2 КАК Этапы
							|ГДЕ
							|	Этапы.Ссылка = &Регистратор");
						ЗапросРеквизита.УстановитьПараметр("Регистратор", СтрокаТаблицы.Регистратор);
						ОтгружаемыйЗаказ = ЗапросРеквизита.Выполнить().Выгрузить()[0].ОтгружаемыйЗаказ;
						
					//++ Устарело_Производство21
					
					ИначеЕсли ТипЗнч(СтрокаТаблицы.Регистратор) = Тип("ДокументСсылка.КорректировкаЗаказаМатериаловВПроизводство") Тогда
						
						ОтгружаемыйЗаказ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТаблицы.Регистратор,
							"Распоряжение");
	...
	ТипыРегистраторов.Добавить("Документ.ЗаказПереработчику2_5");
	...
	ТипыРегистраторов.Добавить("Документ.ЗаказНаПроизводство2_2");
