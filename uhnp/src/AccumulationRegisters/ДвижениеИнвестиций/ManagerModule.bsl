...
Процедура ПроверитьПроведение(Реквизиты, Отказ = Ложь) Экспорт
	...
	Если Не Реквизиты.Свойство("ОбъектИнвестирования") Тогда
		Возврат;//документы учета МСФО без инвестиций	
	КонецЕсли;
	...
	|			&ДатаОстатков,
	|			ОбъектИнвестирования В (&ОбъектИнвестирования)
	...
	ИмяВыбытие = ?(ОбщегоНазначенияПовтИспУХ.ЕстьРСБУ(), "ДокументСсылка.ВыбытиеИнвестиций", "ДокументСсылка.РеализацияМСФО");
	ЭтоВыбытие = ТипЗнч(Реквизиты.Ссылка) = Тип(ИмяВыбытие) 
		Или ТипЗнч(Реквизиты.Ссылка) = Тип("ДокументСсылка.РеализацияМСФО");
	...
	Запрос.УстановитьПараметр("Выбытие", 				ЭтоВыбытие);//для для поступлений проверка только > 100
