...
Функция СформироватьГиперссылкуСмТакжеВРаботе(Параметры) Экспорт
	...
	ТекстГиперссылки = НСтр("ru = 'Учет РНПТ в производстве';
							|en = 'RNPT accounting in manufacturing'");
...
Функция ЗавершенныеПартииПроизводстваРНПТ(МенеджерВТ)
	...
	|		ВЫБОР
	//++ НЕ УТКА
	|			КОГДА РеквизитыЗаказаНаПроизводство.Ссылка ЕСТЬ НЕ NULL
	|				И НЕ РеквизитыЗаказаНаПроизводство.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовНаПроизводство2_2.Закрыт)
	|				ТОГДА ЛОЖЬ
	//-- НЕ УТКА
	...
	|		ВЫБОР
	//++ НЕ УТКА
	|			КОГДА ТИПЗНАЧЕНИЯ(РеквизитыЗаказовНазначений.Ссылка) = ТИП(Документ.ЗаказНаПроизводство2_2)
	|				И РеквизитыЗаказовНазначений.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовНаПроизводство2_2.Закрыт)
	|				И НЕ РеквизитыЗаказовНазначений.Ссылка = РеквизитыЗаказаНаПроизводство.Ссылка
	|				ТОГДА ИСТИНА
	//-- НЕ УТКА
	|			КОГДА ИСТИНА
	|				ТОГДА ЛОЖЬ
	...
	|		
	//++ НЕ УТКА
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказНаПроизводство2_2 КАК РеквизитыЗаказаНаПроизводство
	|		ПО РеквизитыЗаказаНаПроизводство.Ссылка = СпрПартииПроизводства.Документ
	//-- НЕ УТКА
	|		
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// Этап производства (выходные изделия)
	|	ВЫБРАТЬ
	|		ПроверяемыеПартииПроизводства.ПартияПроизводства КАК ПартияПроизводства,
	|		ЕСТЬNULL(СпрНазначения.Заказ, НЕОПРЕДЕЛЕНО)      КАК ДокументВыпуска,
	|		ВЫБОР
	...
	|		ПО РеквизитыЗаказаНаПроизводство.Ссылка = СпрПартииПроизводства.Документ
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ВыходныеИзделия КАК ТаблицаПродукции
	|		ПО ТаблицаПродукции.Ссылка.Распоряжение = СпрПартииПроизводства.Документ
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Назначения КАК СпрНазначения
	|		ПО СпрНазначения.Ссылка = ТаблицаПродукции.Назначение
	|		
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// Этап производства (побочные изделия)
	|	ВЫБРАТЬ
	|		ПроверяемыеПартииПроизводства.ПартияПроизводства КАК ПартияПроизводства,
	|		ЕСТЬNULL(СпрНазначения.Заказ, НЕОПРЕДЕЛЕНО)      КАК ДокументВыпуска,
	|		ВЫБОР
	...
	|		ПО РеквизитыЗаказаНаПроизводство.Ссылка = СпрПартииПроизводства.Документ
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ПобочныеИзделия КАК ТаблицаПродукции
	|		ПО ТаблицаПродукции.Ссылка.Распоряжение = СпрПартииПроизводства.Документ
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Назначения КАК СпрНазначения
	|		ПО СпрНазначения.Ссылка = ТаблицаПродукции.Назначение
	...
	Пока Истина Цикл
		...
		Если Индекс = 10000 Тогда
			ВызватьИсключение НСтр("ru = 'Выполнено 10000 циклов проверки завершенности партий производства. Вероятно произошло зацикливание проверки. Операция прервана.';
									|en = '10,000 batch completion check cycles are performed. The check might be looped. The operation is terminated.'");
		КонецЕсли;
