...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	|';
	|en = 'Clears the ""Being paid"" resource records of payment requests with empty AR/AP objects.
	|Corrects the order of the record operation for the ""%1"" resource.
	|Refills the ""Offset procedure by the payment date"" field for payment documents.
	|Resets the ""By data of source AR/AP object"" attribute if the destination attributes are not filled in the document records.
	|Corrects the ""%2"" resource balance with AR/AP defined as ""Invoice for debt, Order for prepayment"" and ""Invoice for debt, Contract for prepayment"".
	|Corrects a business transaction for ""AR/AP offset"" document records.
	|Fills the ""By data of source AR/AP object"" attribute in AR/AP by deadlines.
	|Fills the ""%3"" attribute.
	|For debt transfer records of offline settlements:
	| - Fills new ""Destination AR/AP object"", ""Destination accounting dimension by partners"", and ""Destination amount"" attributes.
	| - Refills the financial record IDs.
	|Corrects the cor. dimension for ""AR/AP offset"" document records.
	|'"), 
	...
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
	Приоритет = Обработчик.ПриоритетыВыполнения.Добавить();
	Приоритет.Процедура = "Документы.ЗаявкаНаРасходованиеДенежныхСредств.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Приоритет.Порядок = "До";
		...
		Если Запись.ПоДаннымОбъектаРасчетовИсточника Тогда
			...
			Иначе
				...
				СтрокиТекста.Добавить(НСтр("ru = 'Не удалось актуализировать кор. аналитику. Обновление документа отменено.';
											|en = 'Failed to update cor. dimention. Document update cancelled.'"));
				СтрокиТекста.Добавить(ТекстПерепроведитеДокументВРучную());
				ТекстРекомендации = СтрСоединить(СтрокиТекста, Символы.ПС);
				ОбновлениеИнформационнойБазы.ЗарегистрироватьПроблемуСДанными(
					ПроблемныйРегистратор,
