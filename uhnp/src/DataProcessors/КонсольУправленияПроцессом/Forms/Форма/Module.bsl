...
Процедура Запуск(Команда)
	...
	Элементы.ИнициироватьПовторно.Доступность = ПроцессЗапущен;
...
Процедура ОбновитьСхему(Команда)
	...
	ОбъектПроцесса = Объект.УправлениеОтчетнымПериодом;
	Если НЕ ЗначениеЗаполнено(Объект.УправлениеОтчетнымПериодом) Тогда
		ОбъектПроцесса = СогласуемыйОбъект;	
	КонецЕсли;
	
	ПроцессЗапущен = МодульУправленияПроцессамиУХ.ПроверитьПроцессИнициализирован(ОбъектПроцесса, Объект.ЭкземплярПроцесса,, Объект.ШаблонПроцесса);
...
Процедура ЗапускСервер()
	...
	Если Объект.ШаблонПроцесса.НазначениеПроцесса = Перечисления.НазначенияУниверсальныхПроцессов.ПроцессПодготовкиОтчетности Тогда
		...
		РезультатЗапускаУОП = УправлениеПроцессамиПодготовкиОтчетности.ЗапуститьПроцессПодготовкиОтчетности(Объект.УправлениеОтчетнымПериодом);
				
		Если РезультатЗапускаУОП Тогда
	...
	ИначеЕсли Объект.ШаблонПроцесса.НазначениеПроцесса = Перечисления.НазначенияУниверсальныхПроцессов.МаршрутСогласования Тогда	
		...
		Если МодульУправленияПроцессамиУХ.ИнициализироватьПроцесс(Объект.ПараметрыПроцесса,Объект.ШаблонПроцесса,,СогласуемыйОбъект,,,Объект.ЭкземплярПроцесса, , , Истина) Тогда
			Сообщить(НСтр("ru = 'Процесс успешно запущен'")); 
			ПроцессЗапущен = Истина;	
		Иначе	
			 ПроцессЗапущен = Ложь;
	...
	ИначеЕсли Объект.ШаблонПроцесса.НазначениеПроцесса = Перечисления.НазначенияУниверсальныхПроцессов.ПроизвольныйПроцесс Тогда	
		...
		Если МодульУправленияПроцессамиУХ.ИнициализироватьПроцесс(Объект.ПараметрыПроцесса, Объект.ШаблонПроцесса,, КлючевыеПараметры[0].ЗначениеПараметра,,, Объект.ЭкземплярПроцесса) Тогда
			Сообщить(НСтр("ru = 'Процесс успешно запущен'")); 
			ПроцессЗапущен = Истина;
...
Функция ОпределитьТекущийПроцессПоКлючам(ИмяКлюча,ЗначениеКлюча)
	...
	Если Объект.ШаблонПроцесса.НазначениеПроцесса = Перечисления.НазначенияУниверсальныхПроцессов.ПроцессПодготовкиОтчетности Тогда
		Если ИмяКлюча = Нстр("ru = 'Управление отчетным периодом'") Тогда
			Объект.ЭкземплярПроцесса = Неопределено;
	...
	ИначеЕсли Объект.ШаблонПроцесса.НазначениеПроцесса = Перечисления.НазначенияУниверсальныхПроцессов.МаршрутСогласования Тогда
		...
		Если ИмяКлюча = Нстр("ru = 'Согласуемый объект'") Тогда
			Объект.ЭкземплярПроцесса = Неопределено;
...
&НаСервере
Функция ЭтоЭтапВложенногоПроцессаИлиУсловныйПереход(ТекущийЭтап, ПараметрыОткрытияФормы, ТипЭтапа = "")
	...
	Если ТекущийЭтап.ТипЭтапа = Перечисления.ТипыЭтаповУниверсальныхПроцессов.ДочернийМаршрут Тогда
		ТипЭтапа = "ДочернийМаршрут";
		Возврат Истина;
	ИначеЕсли ТекущийЭтап.ТипЭтапа = Перечисления.ТипыЭтаповУниверсальныхПроцессов.УсловныйПереход Тогда
		ИнициализацияУсловийСогласования(ТекущийЭтап, ПараметрыОткрытияФормы);
		ТипЭтапа = "УсловныйПереход";
		Возврат Истина;
	Иначе	
		Возврат Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьДеревоПараметров(СтруктураЗначенийПараметров=Неопределено)
	...
	Нстр.ИмяПараметра = Нстр("ru = 'Ключевые параметры'");
	Нстр.ИндексКартинки = 0;
	Нстр.ТипСтроки = 1;
	
	Нстр1 = ДеревоП.Строки.Добавить();
	Нстр1.ИмяПараметра = Нстр("ru = 'Параметры'");
	...
	Если Объект.ШаблонПроцесса = Справочники.ШаблоныУниверсальныхПроцессов.Автоутверждение Тогда
		...
		СтрокаПараметра.ИмяПараметра = Нстр("ru = 'Согласуемый объект'");
		СтрокаПараметра.КодПараметра = "СогласуемыйОбъект";
		СтрокаПараметра.Ключевой = Истина;
				
		Если СогласуемыйОбъект = Неопределено Тогда
			ТипЗначенияПараметраСсылка = Неопределено;
		ИначеЕсли Справочники.ТипВсеСсылки().СодержитТип(ТипЗнч(СогласуемыйОбъект)) Тогда
		...
		ИначеЕсли Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(СогласуемыйОбъект)) Тогда	
...
Функция ОпределитьТипМетаданных(ИмяОбъекта)
	...
	ИначеЕсли ИмяОбъекта = Нстр("ru = 'Управление отчетным периодом'") Тогда	
...
Процедура ТабДокПроцессаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ЭтоЭтапВложенногоПроцессаИлиУсловныйПереход(ВернутьВыбранныйЭтап(), ПараметрыОткрытияФормы, ТипЭтапа) Тогда
		
		Если ТипЭтапа = "ДочернийМаршрут" Тогда
			СтандартнаяОбработка = Ложь;
			СтруктураПараметров = ПолучитьПараметрыВложенногоПроцесса(ВернутьВыбранныйЭтап());
			ОткрытьФорму("Обработка.КонсольУправленияПроцессом.форма.Форма",
				СтруктураПараметров,
				ЭтаФорма,
				Истина,,,,
				РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		ИначеЕсли ТипЭтапа = "УсловныйПереход" Тогда
			СтандартнаяОбработка = Ложь;
...
&НаКлиенте
Процедура ИнициироватьПовторно(Команда)
		
	Если Не ЗначениеЗаполнено(СогласуемыйОбъект) Тогда
		Возврат;	
	КонецЕсли;
	
	ИнициироватьПовторноНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура ИнициироватьПовторноНаСервере()
		
	ДокументПроцесса = МодульУправленияПроцессамиУХ.ПолучитьЭкземплярПроцесса(СогласуемыйОбъект);
	
	ТекущийЭтап = МодульУправленияПроцессамиУХ.ПолучитьТекущийЭтапСогласования(СогласуемыйОбъект, ДокументПроцесса);
	Если Не ЗначениеЗаполнено(ТекущийЭтап) Тогда
		Возврат;	
	КонецЕсли;
	
	СтруктураПараметровВыгрузки = Новый Структура;
	СтруктураПараметровВыгрузки.Вставить("Этап", ТекущийЭтап);
	ОтветственныеЭтапа = ДокументПроцесса.ДополнительныеСогласующие.Выгрузить(СтруктураПараметровВыгрузки);
	ОрганизацияОбъекта = МодульУправленияПроцессамиУХ.ПолучитьОрганизациюОбъекта(СогласуемыйОбъект);
		
	МодульРегламентныхЗаданийУХ.СброситьЗаданияПоЭтапуПроцесса(ДокументПроцесса, ТекущийЭтап);
	МодульУправленияПроцессамиУХ.ПометитьНаУдалениеЗадачиПоОбъекту(СогласуемыйОбъект);
	
	ТабИсполнителей = МодульУправленияПроцессамиУХ.ПолучитьИсполнителейПоЭтапамМаршрута(ДокументПроцесса.ШаблонПроцесса);
	
	ДокументПроцессаОбъект = ДокументПроцесса.ПолучитьОбъект();
	ДокументПроцессаОбъект.ДополнительныеСогласующие.Очистить();
	Для Каждого Исп из ТабИсполнителей Цикл
		НовыйСогласующий = ДокументПроцессаОбъект.ДополнительныеСогласующие.Добавить();
		НовыйСогласующий.Этап = Исп.Этап;
		НовыйСогласующий.Организация = Исп.Организация;
		НовыйСогласующий.Пользователь = Исп.Пользователь;		
	КонецЦикла;
	ДокументПроцессаОбъект.Записать(РежимЗаписиДокумента.Запись);
	
	МодульУправленияПроцессамиУХ.ИнициироватьСогласованиеЭтапа(ДокументПроцесса, ОрганизацияОбъекта, ТекущийЭтап,,,, Истина);
