	...
	Попытка
		...
		Если ЗначениеЗаполнено(ВходящийДокументОтФонда) Тогда
			Комментарий = НСтр("ru = 'Установлен регномер СФР ""%1"" по данным документа ""%2""';
								|en = 'Registration number in the Social Insurance Fund of Russia ""%1"" is set by the ""%2"" document data'");
		...
		Иначе
			Комментарий = НСтр("ru = 'Установлен регномер СФР ""%1"" по данным сообщения СЭДО %2 от %3 (%4 %5)';
								|en = 'Registration number in the Social Insurance Fund of Russia ""%1"" is set by the %2 EDI message data dated %3 (%4 %5)'");
