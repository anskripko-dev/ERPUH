...
Функция ВыгрузитьТаблицуОпераций(ПараметрыПланирования, МенеджерВременныхТаблиц)
	...
	|	График.Начало КАК ДатаЗапуска,
	|	График.Окончание КАК ДатаВыпуска,
...
Функция ТекстЗапросаОперацииДляПланирования()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикЭтаповПроизводства2_2 КАК График
	|		ПО ОчередьПроизводственныхОпераций.Распоряжение = График.ЗаказНаПроизводство
	|			И ОчередьПроизводственныхОпераций.Этап = График.Этап
	|			И (График.СтатусГрафика = &СтатусРабочийГрафик)
	...
	|		ПО ПроизводственныеОперации.Этап = ГрафикЭтаповПроизводства2_2.Этап
	|ГДЕ
...
Функция ПриоритетыЭтаповОпераций(ПараметрыПланирования)
	...
	|	График.ЗаказНаПроизводство.Приоритет.РеквизитДопУпорядочивания КАК РаспоряжениеПриоритет,
	...
	|СГРУППИРОВАТЬ ПО
	|	График.Этап,
	|	График.ЗаказНаПроизводство.Приоритет.РеквизитДопУпорядочивания";
			...
			Если ЗначениеЗаполнено(НачальнаяТочка) Тогда
				...
				Для Каждого ВариантРасписания Из ВариантыРасписания Цикл
					...
					Если ЗначениеЗаполнено(ЗаписьРасписания.ОценкаВремя) Тогда
						...
						ЗаписьРасписания.ОценкаВремя = ?(МинимальноеОтклонение = 0, ЗаписьРасписания.ОценкаВремя, ЗаписьРасписания.ОценкаВремя/МинимальноеОтклонение);
