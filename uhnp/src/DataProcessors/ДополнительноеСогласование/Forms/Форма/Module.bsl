...
&НаКлиенте
Процедура ТаблицаДопСогласованияКомментарийПриИзменении(Элемент)
		
	ТекущиеДанные = Элементы.ТаблицаДопСогласования.ТекущиеДанные;
	СтруктураДанных = Новый Структура("ЭкземплярПроцесса, ЭтапПроцесса, Согласующий, Комментарий", 
		Объект.ЭкземплярПроцесса, Объект.ЭтапСогласования, ТекущиеДанные.Согласующий, ТекущиеДанные.Комментарий);
	
	ТаблицаДопСогласованияКомментарийПриИзмененииНаСервере(СтруктураДанных);
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ТаблицаДопСогласованияКомментарийПриИзмененииНаСервере(СтруктураДанных)
	
	// Изменим комментарий в регистре доп согласующих.
	МенеджерЗаписи = РегистрыСведений.ВизаДополнительныхСогласующих.СоздатьМенеджерЗаписи();
	МенеджерЗаписи.ЭкземплярПроцесса = СтруктураДанных.ЭкземплярПроцесса;
	МенеджерЗаписи.ЭтапПроцесса = СтруктураДанных.ЭтапПроцесса;
	МенеджерЗаписи.Согласующий = СтруктураДанных.Согласующий;
	
	МенеджерЗаписи.Прочитать();
	Если МенеджерЗаписи.Выбран() Тогда
		МенеджерЗаписи.Комментарий = СтруктураДанных.Комментарий;
	КонецЕсли;
	
	Попытка
		МенеджерЗаписи.Записать();
	Исключение
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Не записан комментарий %1'"), ОписаниеОшибки());
		ОбщегоНазначенияУХ.СообщитьОбОшибке(ТекстОшибки);
	КонецПопытки;
