	...
	Если Объект.НастройкиПодключения.Количество() = 0 Тогда
		...
		Если СообщитьПользователюОбОшибках Тогда
			ШаблонСообщения = НСтр("ru = 'Не найдена настройка подключения к СБП для договора %1, поддерживающая сверку взаиморасчетов с банком';
									|en = 'The FPS connection setting which supports bank reconciliation is not found for the %1 contract'");
			...
			Иначе
				ТекстСообщения = СтрШаблон(
					НСтр("ru = 'Настройка подключения ""%1"", дата отчета ""%2"" - более одного документа ""Отчет банка по электронным платежным операциям""';
						|en = 'The ""%1"" connection setting, report date ""%2"": more than one ""Bank report on electronic payment transactions"" document'"),
			...
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru = 'Настройка подключения ""%1"", дата отчета ""%2"" - в данных сверки банка по операциям СБП (""Строка отчета"" №%3) не указан тип операции. ""Строка отчета"" №%3 не загружен.';
						|en = 'The ""%1"" connection setting, report date ""%2"": the bank reconciliation data on FPS transactions (""Report line"" No. %3) does not contain the transaction type. ""Report line"" No. %3 is not imported.'"),
					НастройкаПодключения,
					Формат(НачалоДня(ДатаПлатежа), "ДФ=dd.MM.yyyy"),
			...
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(
					НСтр("ru = 'Настройка подключения ""%1"", дата отчета ""%2"" - при загрузке данных сверки в документе ""Отчет банка по электронным платежным операциям"" не заполнены платежи. Документ не создан.';
						|en = 'The ""%1"" connection setting, report date ""%2"": when importing reconciliation data, payments are not filled in the ""Bank report on electronic payment transactions"" document. The document is not created.'"),
					НастройкаПодключения,
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Настройка платежа ""%1"", дата отчета ""%2"" - документ ""Отчет банка по электронным платежным операциям"" №%3 от %4 не %5. Произошли ошибки при %6.';
					|en = 'The ""%1"" connection setting, report date ""%2"": the ""Bank report on electronic payment transactions"" document No.%3 dated %4 is not %5. Errors occurred when %6.'"),
				НастройкаПодключения,
				Формат(ДатаОтчета, "ДФ=dd.MM.yyyy"),
			...
			ИначеЕсли НайденныеДокументы.Количество() = 0 Тогда
				ТекстСообщения = СтрШаблон(
					НСтр("ru = 'Настройка подключения ""%1"", дата отчета ""%2"" - некорректные данные банка или ""Отчет банка по электронным платежным операциям"" не загружен';
						|en = 'The ""%1"" connection setting, report date ""%2"": incorrect bank information or the ""Bank report on electronic payment transactions"" is not imported'"),
					НастройкаПодключения,
					Формат(НачалоДня(ДатаПлатежа), "ДФ=dd.MM.yyyy"));
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
