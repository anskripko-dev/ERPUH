	...
	Попытка
		...
		Если НЕ ЗначениеЗаполнено(ПутьКФайлам) Тогда
			ОбщегоНазначенияУХ.СообщитьОбОшибке(СтрШаблон(Нстр("ru = 'Не заполнен путь к %1'"), 
				?(ИмпортИзФайла,  Нстр("ru = 'файлу.'"),  Нстр("ru = 'файлам.'"))),,Нстр("ru = 'Общая ошибка импорта'"),СтатусСообщения.Важное);
			Возврат;
		КонецЕсли;
		...
		Для Каждого Строка ИЗ ТаблицаКонтекстов Цикл
			...
			КонтекстПроцесса = ПолучитьКонтекстУправленияПериодомСценария(Строка.Сценарий, Строка.ПериодОтчета);
			...
			Если КонтекстПроцесса <> Неопределено Тогда
				...
				Если НЕ ПолныеПраваУХ.ПроверитьВозможностьЗаписиДокументовПоПериодуСценария(Строка, ЕстьОшибки, ШапкаОшибки) Тогда
		...
		Пока Результат.Следующий() Цикл
			...
			ПараметрыНастроек = УправлениеОтчетамиУХ.ПараметрыНастроекОбработкиШаблонов(Результат);
			Реквизиты = УправлениеОтчетамиУХ.НастройкиОбработкиШаблонов(ПараметрыНастроек);
			
			Если НЕ ЗначениеЗаполнено(Реквизиты.ПравилоОбработки) Тогда
			...
			ОтчетОбъект.СпособФормированияОтчета	= Перечисления.СпособыФормированияОтчетов.ПоПравилуОбработки;
			...
			Если ОтчетОбъект.ЗаполнитьОтчетПоУмолчанию(СпособЗаполнения) Тогда
...
Функция ПолучитьКонтекстУправленияПериодомСценария(СценарийИсточник, ПериодОтчетаИсточник)
	...
	|	УправлениеПериодомСценария.ДатаНачалаПроцесса КАК ДатаНачалаПроцесса,
	...
	Иначе
		...
		Возврат Новый Структура("Ссылка,ВерсияОрганизационнойСтруктуры,ДатаНачалаПроцесса,ПериодСценарияОкончание", 
			Выборка.Ссылка, Выборка.ВерсияОрганизационнойСтруктуры, Выборка.ДатаНачалаПроцесса, Выборка.ПериодСценарияОкончание);
