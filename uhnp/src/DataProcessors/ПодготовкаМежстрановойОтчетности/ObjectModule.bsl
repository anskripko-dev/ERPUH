...
Процедура ЗаполнитьСтрановойОтчет(ПараметрыОтчета, Контейнер) Экспорт
	...
	Если ТаблицаИнвестиций.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	АдресаОрганизаций = ПолучитьТаблицуАдресовОрганизаций(ТаблицаИнвестиций, ПараметрыОтчета.Организация);
...
Функция ПолучитьТаблицуАдресовОрганизаций(СписокИнвестиций, Организация)
	...
	АдресаОрганизаций = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(МассивОрганизацийДляАдресов, Перечисления.ТипыКонтактнойИнформации.Адрес, Виды, Ложь);
