...
Процедура ВыборФайла()
	...
	Если ТипФайла = Истина Тогда
		Текст = Нстр("ru = 'Проект MS Project в формате XML'");
		ДиалогВыбораФайла.Фильтр = НСтр(Текст) + "(*.xml)|*.xml";
	Иначе
		Текст = Нстр("ru = 'Проект MS Project в формате mpp'");
...
Процедура ОбработатьФайлЧерезCOM(ПутьКФайлу)
	...
	Исключение
		ОбщегоНазначенияУХ.СообщитьОбОшибке(СтрШаблон(Нстр("ru = 'Не удалось открыть файл!%1%2'"), Символы.ПС, 
...
Процедура ОбработатьCOM(мДанныеПроектов, ДанныеГлПроекта)
	...
	Исключение
		...
		ОбщегоНазначенияУХ.СообщитьОбОшибке(СтрШаблон(Нстр("ru = '%1.%2%3'"), НСтр("ru = 'Ошибка импорта проекта из файла '") + ПутьКФайлу, Символы.ПС, 
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
