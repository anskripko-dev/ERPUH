...
Функция ПолучитьТекстОписанияДоговораЗакупки(ОписаниеОбмена, ЕИСФабрикаXDTO)
	...
	Попытка
		...
		ДанныеДоговора.provider =
			Лот.ЗакупочнаяПроцедура.ПриоритетУчастияМалогоИСреднегоПредпринимательства;
...
&НаСервере
Функция ЗаполнитьlongTermContractVolumes(ЕИСФабрикаXDTO,T_contractData,Договор)
	
	Если Год(Договор.ДатаНачалаДействия) = Год(Договор.ДатаОкончанияДействия) Тогда
		Возврат Неопределено;	
	КонецЕсли;	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Договор",Договор);
	Запрос.Текст = "ВЫБРАТЬ
	               |	СУММА(ГОД(ВерсияСоглашенияКоммерческийДоговорНоменклатура.ДатаПоставки)) КАК ГОД,
	               |	ВерсияСоглашенияКоммерческийДоговорНоменклатура.Ссылка.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
	               ...
	               |ГДЕ
	               |	ВерсияСоглашенияКоммерческийДоговорНоменклатура.Ссылка = &Договор
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ВерсияСоглашенияКоммерческийДоговорНоменклатура.СуммаНДС,
	               |	ВерсияСоглашенияКоммерческийДоговорНоменклатура.Ссылка.ВалютаВзаиморасчетов,
	               |	ВерсияСоглашенияКоммерческийДоговорНоменклатура.Сумма";
	Тип = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(T_contractData, "longTermContractVolumes");
	Данные =  ЕИСФабрикаXDTO.Создать(Тип);
    ТЗ = Запрос.Выполнить().Выгрузить();
	Данные.volume = ТЗ.Итог("Сумма");  
	ВалютаРеглУчета = Константы.ВалютаРегламентированногоУчета.Получить();
	
	КоэффициентПересчетаВалюты = РаботаСКурсамиВалют.ПересчитатьВВалюту(
				1, Договор.ВалютаВзаиморасчетов, ВалютаРеглУчета, Договор.Дата);	
	Данные.volumeRub = ТЗ.Итог("Сумма")*КоэффициентПересчетаВалюты;
	
	ТипСвойства = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(Тип,"details");
	Данные.details = ЕИСФабрикаXDTO.Создать(ТипСвойства);
	
	Для Каждого стр из ТЗ Цикл
		Описание = ЕИСФабрикаXDTO.Создать(ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(ТипСвойства, "longTermContractVolumeDetail"));
		Описание.year = стр.Год;
		Описание.summ = стр.Сумма;
		Описание.summRub = стр.Сумма*КоэффициентПересчетаВалюты;
		Данные.details.longTermContractVolumeDetail.Добавить(Описание);
	КонецЦикла;
...
Функция ПолучитьТекстОписанияИсполненияДоговора(ОписаниеОбмена, ЕИСФабрикаXDTO)
	...
	Попытка
		...
		Если НЕ ЗначениеЗаполнено(Лот.ЗакупочнаяПроцедура.ОрганизаторЗакупки) Тогда
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
					ПоляОписанияТранзакции, 
					Нстр("ru = 'Не заполнена организация организатор закупки! '"));
			Возврат Неопределено;
		...
		placer = ПолучитьОписаниеОрганизации(
						ЕИСФабрикаXDTO, 
						T_customer, 
						Лот.ЗакупочнаяПроцедура.ОрганизаторЗакупки, 
						ПоляОписанияТранзакции);
		Если ПоляОписанияТранзакции.ОшибкаОбменаСЭТП Тогда
			Возврат Неопределено;
		КонецЕсли;
...
Функция ПолучитьТекстЗапросаНаОтменуЗакупки(ОписаниеОбмена, ЕИСФабрикаXDTO)
	...
	Попытка
		...
		КодСпособаВыбора = ЗакупочнаяПроцедура.СпособВыбораПоставщика.КодЕИС;
...
Функция ПолучитьТекстОписанияЗакупки(ОписаниеОбмена, ЕИСФабрикаXDTO)
	...
	ПоляОписанияТранзакции = ОписаниеОбмена.ПоляОписанияТранзакции;
	ЗакупочнаяПроцедура = ОписаниеОбмена.ОбъектВыгрузки;
	СпособВыбораПоставщика = ЗакупочнаяПроцедура.СпособВыбораПоставщика.СпособыВыбораПоставщика;
...
Функция ПолучитьpurchaseNoticeAE(ЭТП, ЕИСФабрикаXDTO, ЗакупочнаяПроцедура, ДопПоля, ПоляОписанияТранзакции)
	...
	Попытка
		...
		Если НЕ ЗначениеЗаполнено(
					ЗакупочнаяПроцедура.ВнешняяСистемаДляПроведенияАукциона) И ЗакупочнаяПроцедура.ВЭлектроннойФорме Тогда
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
				ПоляОписанияТранзакции,
...
Функция ПолучитьpurchaseNoticeAEMSP(ЭТП, ЕИСФабрикаXDTO, ЗакупочнаяПроцедура, ДопПоля, ПоляОписанияТранзакции)
	...
	Попытка           
		...
		Если НЕ ЗначениеЗаполнено(
					ЗакупочнаяПроцедура.ЭТППубликации)  И ЗакупочнаяПроцедура.ВЭлектроннойФорме Тогда
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
				ПоляОписанияТранзакции,
...
Функция ЗаполнитьДатыПоСтадии(Стадия,ЗакупочнаяПроцедура,Мероприятие)
		...
		Если ЗакупочнаяПроцедура.СпособВыбораПоставщика.СпособыВыбораПоставщика = Перечисления.СпособыВыбораПоставщика.ЗапросПредложений Тогда	
			Стадия = Справочники.СтадииМероприятий.ЗапросПредложений_НачалоПриемаЗаявок;
		ИначеЕсли ЗакупочнаяПроцедура.СпособВыбораПоставщика.СпособыВыбораПоставщика = Перечисления.СпособыВыбораПоставщика.Конкурс Тогда	
			Стадия = Справочники.СтадииМероприятий.Конкурс_НачалоПриемаЗаявок;
		ИначеЕсли ЗакупочнаяПроцедура.СпособВыбораПоставщика.СпособыВыбораПоставщика = Перечисления.СпособыВыбораПоставщика.КонкурентныеПереговоры Тогда	
			Стадия = Справочники.СтадииМероприятий.КонкурентныеПереговоры_НачалоПриемаЗаявок;
		ИначеЕсли ЗакупочнаяПроцедура.СпособВыбораПоставщика.СпособыВыбораПоставщика = Перечисления.СпособыВыбораПоставщика.Аукцион Тогда	
...
Функция ПолучитьЛотЗакупочнойПроцедуры(ЭТП,
	...
	ОписаниеЛота.ordinalNumber = Лот.Номер;
...
&НаСервере 
Функция ПолучитьТекстОписанияГодовогоОтчетаОЗакупкеУМСП(ОписаниеОбмена, ЕИСФабрикаXDTO)
	ЭТП = ОписаниеОбмена.ЭТП;
	ПоляОписанияТранзакции = ОписаниеОбмена.ПоляОписанияТранзакции;
	Отчет = ОписаниеОбмена.ОбъектВыгрузки;
	ТекстОписания = Неопределено;
	Попытка
		ОписаниеОтчета = ИнтеграцияЦУЗсЭТПСерверУХ.ПолучитьВерсиюОбъектаДляЕИС(Отчет);
		УИДОтчета = ОписаниеОтчета.ГУИДВерсии;
		СохранитьУИДОбъекта(Отчет,, УИДОтчета);
		ПространствоИмен = "http://zakupki.gov.ru/223fz/purchase/1";
		КорневыеОбъекты =
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПолучитьКорневыеОбъектыПространстваИмен(
				ЕИСФабрикаXDTO,
				ПространствоИмен);    
		T_volumePurchase = КорневыеОбъекты.Получить("volumePurchase").Тип;
		T_header = 
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
				T_volumePurchase, "header");	
		T_body = 
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
				T_volumePurchase, "body");
		T_volumePurchaseItem = 
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
				T_body, "item");
		T_volumePurchaseData = 
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
				T_volumePurchaseItem, "volumePurchaseData");
		T_customer = 
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
				T_volumePurchaseData, "placer");
		ОтчетЗакупок = ЕИСФабрикаXDTO.Создать(T_volumePurchase);
		
 		ОтчетЗакупок.header = ЕИСФабрикаXDTO.Создать(T_header);
 		ОтчетЗакупок.header.guid = ПоляОписанияТранзакции.КлючСообщения;
 		ОтчетЗакупок.header.createDateTime = ТекущаяДата();
		// Тело пакета
		ОтчетЗакупок.body = ЕИСФабрикаXDTO.Создать(T_body);
		ОтчетЗакупок.body.item = ЕИСФабрикаXDTO.Создать(T_volumePurchaseItem);
		ОтчетЗакупок.body.item.guid = Строка(Новый УникальныйИдентификатор);
		ОтчетЗакупок.body.item.volumePurchaseData =
			ЕИСФабрикаXDTO.Создать(T_volumePurchaseData);
		ДанныеОтчета = ОтчетЗакупок.body.item.volumePurchaseData;  
		ДанныеОтчета.guid = ОписаниеОтчета.ГУИДВерсии;
		ДанныеОтчета.urlVSRZ = "zakupki.gov.ru";//заглушка   
		ДанныеОтчета.createDateTime = Отчет.Дата;
		

		ДанныеОтчета.customer = ПолучитьОписаниеОрганизации(
									ЕИСФабрикаXDTO, 
									T_customer, 
									Отчет.ОрганизацияЗаказчик, 
									ПоляОписанияТранзакции);
		
		ДанныеОтчета.placer = ПолучитьОписаниеОрганизации(
								ЕИСФабрикаXDTO, 
								T_customer, 
								Отчет.Организация, 
								ПоляОписанияТранзакции);;
		//ОбъектXDTO.period = Дата(Отчет.Дата, Отчет.Месяц, 1, 0, 0, 0);
		//ДанныеОтчета.contractsNumber = Отчет.КоличествоДокументов;
		//ДанныеОтчета.contractsOverallCost = Отчет.СтоимостьДоговоров;
		//ДанныеОтчета.contractsNumberInnovation = Отчет.КоличествоДокументовИнновационнаяПродукция;
		//ДанныеОтчета.contractsOverallCostInnovation = Отчет.СтоимостьДоговоровИнновационнаяПродукция;
		ДанныеОтчета.version = ОписаниеОтчета.НомерВерсии+1;
		
		ДанныеОтчета.Year = Отчет.ПериодРегистрации.Код;      
		purchaseIndicatorListType = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(T_volumePurchaseData, "purchaseIndicatorListType");		
		ДанныеОтчетаПоказатели = ЕИСФабрикаXDTO.Создать(purchaseIndicatorListType);
		T_purchaseIndicatorType = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(purchaseIndicatorListType, "purchaseIndicatorType");

		T_contractsEIS = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(T_purchaseIndicatorType, "contractsEIS");
		T_contractsNotEIS = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(T_purchaseIndicatorType, "contractsNotEIS");
		НомерСтроки = 0;
		Для Каждого Показатель ИЗ  Отчет.ЗначенияПоказателей Цикл
			Если Показатель.СтоимостнойОбъемОплаты = 0 Тогда//И СтрНайти(Показатель.Код,".") = 0 Тогда 
				Продолжить;
			КонецЕсли; 
			НомерСтроки = НомерСтроки+1;
			
			purchaseIndicatorType = ЕИСФабрикаXDTO.Создать(T_purchaseIndicatorType); 
			КодЕИС = СтрЗаменить(Строка(Показатель.КодЕИС),",",".");
			purchaseIndicatorType.code = СтрЗаменить(КодЕИС,"1.0","1.");
			
			purchaseIndicatorType.number = НомерСтроки;   
			
			purchaseIndicatorType.name = Показатель.Наименование;
			//// Итого по показателю
			purchaseIndicatorType.volume = Окр(Показатель.Сумма / 1000, 5) ;
			purchaseIndicatorType.count = Показатель.Количество;
			purchaseIndicatorType.volumeRepYear = Окр(Показатель.СтоимостнойОбъемОплаты / 1000, 5);
			purchaseIndicatorType.countPrevRepYear = Показатель.КоличествоДоговоровПредыдущихПериодов;
			//
			//// Значения по договорам, размещенным в ЕИС
			contractsEIS = ЕИСФабрикаXDTO.Создать(T_contractsEIS);
			purchaseIndicatorType.contractsEIS = contractsEIS;
			
			contractsEIS.volume = Окр((Показатель.Сумма-Показатель.СуммаНеЕИС) / 1000, 5);
			contractsEIS.count = Показатель.Количество-Показатель.КоличествоНеЕИС;
			contractsEIS.volumeRepYear = Окр((Показатель.СтоимостнойОбъемОплаты - Показатель.СтоимостнойОбъемОплатыНеЕИС)/ 1000, 5);
			contractsEIS.countPrevRepYear = Показатель.КоличествоДоговоровПредыдущихПериодов-Показатель.КоличествоДоговоровПредыдущихПериодовНеЕИС;
 
			//
			//// Значения по договорам, НЕ размещенным в ЕИС
			contractsNotEIS = ЕИСФабрикаXDTO.Создать(T_contractsNotEIS);
			purchaseIndicatorType.contractsNotEIS = contractsNotEIS;
			//
			contractsNotEIS.volume =  Окр(Показатель.СуммаНеЕИС / 1000, 5);
			contractsNotEIS.count = Показатель.КоличествоНеЕИС;
			contractsNotEIS.volumeRepYear = Окр(Показатель.СтоимостнойОбъемОплатыНеЕИС/ 1000, 5);
			contractsNotEIS.countPrevRepYear = Показатель.КоличествоДоговоровПредыдущихПериодовНеЕИС;   
			
			
			//
			//// Добавляем показатель в коллекцию
			ДанныеОтчетаПоказатели.purchaseIndicatorType.Добавить(purchaseIndicatorType);
			
		КонецЦикла;	    
		ДанныеОтчета.purchaseIndicatorListType = ДанныеОтчетаПоказатели;	
		
		Если ОтчетЗакупок = Неопределено Тогда
			Возврат Неопределено;
		КонецЕсли;
		ОтчетЗакупок.Проверить();	
		ТекстОписания =
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПолучитьТекстОписанияОбъекта(
								ЕИСФабрикаXDTO, 
								ОтчетЗакупок, 
	...
	Исключение
		Инфо = ИнформацияОбОшибке();
		ТекстОшибки =
			СтрШаблон(Нстр("ru = 'Заполнение данных для годового отчета о закупке УМСП закончилось ошибкой!%1%2'"), Символы.ПС, 
				ПодробноеПредставлениеОшибки(Инфо));
		ИнтеграцияЦУЗсЭТПСерверУХ.ЗарегистрироватьОшибку(
			ПоляОписанияТранзакции, ТекстОшибки, "ЕИС");
		Возврат Неопределено;
...
&НаСервере
Функция ИсключенияНациональногоРежимаЛота(Лот)
	
	Перем Запрос, Исключения;
	
	Запрос = Новый Запрос; 
	Запрос.УстановитьПараметр("Ссылка", Лот);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЛотОбоснованияИсключенияНациональногоРежима.Номенклатура КАК Номенклатура,
	|	ЛотОбоснованияИсключенияНациональногоРежима.Исключение КАК Исключение,
	|	ЛотОбоснованияИсключенияНациональногоРежима.ДополнительнаяИнформация КАК ДополнительнаяИнформация,
	|	ИсключенияНациональногоРежима.Код КАК ИсключениеКод,
	|	ИсключенияНациональногоРежима.Наименование КАК ИсключениеПредставление
	|ИЗ
	|	Документ.Лот.ОбоснованияИсключенияНациональногоРежима КАК ЛотОбоснованияИсключенияНациональногоРежима
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ИсключенияНациональногоРежима КАК ИсключенияНациональногоРежима
	|		ПО ЛотОбоснованияИсключенияНациональногоРежима.Исключение = ИсключенияНациональногоРежима.Ссылка
	|ГДЕ
	|	ЛотОбоснованияИсключенияНациональногоРежима.Ссылка = &Ссылка";
	
	Исключения = Запрос.Выполнить().Выгрузить();
	Исключения.Индексы.Добавить("Номенклатура");
	Возврат Исключения;
...
Функция ПолучитьLotCustomerData(ЭТП,
	...
	Если Лот.ВалютаДокумента <> ВалютаРеглУчета Тогда
		...
		ДатаКурса = Лот.ЗакупочнаяПроцедура.ДатаНачалаПодготовкиЗакупки;
		КоэффициентПересчетаВалюты = РаботаСКурсамиВалют.ПересчитатьВВалюту(
	...
	lotCustomerData.excludePurchaseFromPlan =
		Лот.ЗакупочнаяПроцедура.НеВключенВПланЗакупкиЧасть15Ст4ФЗ223;
	Если НЕ ПустаяСтрока(Лот.ПорядокФормированияЦеныДоговора) Тогда
...
Функция ПолучитьlotPlanInfo(ЕИСФабрикаXDTO, 
	...
	ОписаниеПланаЗакупкиДляЛота.planGuid =     ОписаниеЛотаВПлане.УИД_Плана; //  "a64499f3-9801-11ed-b796-4cedfb43af6d"; //  
	...
	ОписаниеПланаЗакупкиДляЛота.lotPlanPosition =
		ЕИСФабрикаXDTO.Создать(
			...
			КодТипаПозиции);
	ОписаниеПланаЗакупкиДляЛота.positionGuid =  ОписаниеЛотаВПлане.УИД_ЛотаВПлане; // "71971e77-9800-11ed-b796-4cedfb43af6d"; //
...
Функция ПолучитьТекстОписанияПроектаИлиПланаЗакупки(ОписаниеОбмена, ЕИСФабрикаXDTO, ИменаТипов)
	...
	Попытка
		...
		//Если НЕ ЗначениеЗаполнено(ПланЗакупок.ДокументОснование) Тогда  изменено
		...
		Для Каждого СтрокаПлана Из МСтрокиПлана Цикл
			КодСпособаВыбора = СтрокаПлана.СпособВыбораПоставщика.КодЕИС; 
			...
			Если НЕ СтрокаПлана.ЗакупкаОтменена	Тогда
				...
				Иначе
					ЗаполнитьОбычнуюСтрокуПланаЗакупок(
						...
						ПоляОписанияТранзакции,	ДанныеСтроки);
				КонецЕсли;    
				ДополнитьСПЗ_positionFinancialProvisionVolumes(ДанныеСтроки, ЕИСФабрикаXDTO, СтрокаПлана, ТипСтрокиПлана);				
		...
		ДанныеПлана.annualVolumeSMBLess20Percent	 = (ИтоговыеСуммыПланаЗакупок.ПроцентМСП < 20);
			ДанныеПлана.reportingYear					 = Число(ОтчетныйГод);         
		
		ДанныеПлана.previousYearAnnualVolumeHiTech	 = ИтоговыеСуммыПланаЗакупок.СуммаИнновационныхПрошлыйГод;
		ДанныеПлана.previousYearAnnualVolumeHiTechSMB= ИтоговыеСуммыПланаЗакупок.СуммаИнновационныхМСППрошлыйГод;
...
//	ДанныеСтроки.longTerm = ЭтоДолгосрочнаяЗакупка; 
	ДанныеСтроки.shared = ЗначениеЗаполнено(СтрокаПлана.СтрокаПереходящегоПериода);//ложь;//ЭтоДолгосрочнаяЗакупка;   
...
Функция ГуидПрограммыОснования(ДокументОснование)
	 ...
	 Запрос.Текст = "ВЫБРАТЬ
	                ...
	                |	ЕСТЬNULL(ВложенныйЗапрос.КлючСообщения, """") КАК КлючСообщения
	                |ИЗ
	                ...
	                |
	                |УПОРЯДОЧИТЬ ПО
	                |	ПрограммаЗакупокСтрокиПланаЗакупок.Ссылка.НомерВерсии УБЫВ";
	 Результат = Запрос.Выполнить().Выбрать();
	 Пока Результат.Следующий() Цикл
		 Если Строка(Результат.КлючСообщения) <>"" И Строка(Результат.Ссылка.УИД_ЕИС) = "" Тогда 
...
&НаСервере
Функция ЕдиноеМестоПоставки(СтрокаПлана)

	СтруктураМестаПоставки = Новый Структура();
	Если СтрокаПлана.Номенклатура.Количество() > 0 Тогда
		МестоПоставки = СтрокаПлана.Номенклатура[0].МестоПоставки;
		Для Каждого Номенклатура ИЗ СтрокаПлана.Номенклатура Цикл
			Если Номенклатура.МестоПоставки<>МестоПоставки Тогда   
				СтруктураМестаПоставки.Вставить("ЕдиноеМестоПоставки",Ложь);
				Возврат СтруктураМестаПоставки;
			КонецЕсли;
		КонецЦикла;
		СтруктураМестаПоставки.Вставить("ЕдиноеМестоПоставки",Истина);
		СтруктураМестаПоставки.Вставить("МестоПоставки",МестоПоставки);
		СтруктураМестаПоставки.Вставить("КодПоОКАТО",МестоПоставки.КодПоОКАТО);
		Возврат  СтруктураМестаПоставки;
	Иначе      
		СтруктураМестаПоставки.Вставить("ЕдиноеМестоПоставки",Ложь);
		Возврат СтруктураМестаПоставки;
...
Функция ЗаполнитьОбычнуюСтрокуПланаЗакупок(ЭТП,
											...
											ПоляОписанияТранзакции, 
											ДанныеСтроки)
	СтруктураМестаПоставки = ЕдиноеМестоПоставки(СтрокаПлана);
	Если СтруктураМестаПоставки.ЕдиноеМестоПоставки Тогда      
		 Если СокрЛП(СтруктураМестаПоставки.КодПоОКАТО)<>"" Тогда
		 ДанныеСтроки.okato = СокрЛП(СтруктураМестаПоставки.КодПоОКАТО);  
		 КонецЕсли;
		 ДанныеСтроки.region = СокрЛП(СтруктураМестаПоставки.МестоПоставки); 
    	 ДанныеСтроки.isGeneralAddress = Истина;          
	Иначе 	
		ДанныеСтроки.isGeneralAddress = Ложь;          
	КонецЕсли;	 
...
Функция ОпределитьКодТипаОбъектаЗакупки(НоменклатураВход)
	РезультатФункции = "G";
	Если ТипЗнч(НоменклатураВход) = Тип("СправочникСсылка.Номенклатура") 
		ИЛИ ТипЗнч(НоменклатураВход) = Тип("СправочникСсылка.ТоварныеКатегории")
		ИЛИ ТипЗнч(НоменклатураВход) = Тип("СправочникСсылка.КатегорииЗакупок") Тогда
		Если НоменклатураВход.ВидНоменклатуры.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Работа Тогда
			РезультатФункции = "W";
		ИначеЕсли НоменклатураВход.ВидНоменклатуры.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга Тогда
			РезультатФункции = "S";
		Иначе
			РезультатФункции = "G";
		КонецЕсли;
...
Функция ПолучитьpurchasePlanDataItemRows(ЭТП,
	...
	Для Каждого СтрокаНоменклатуры Из ТЗНоменклатуры Цикл
		...
		Если ЗначениеЗаполнено(СтрокаНоменклатуры.МестоПоставкиОКАТО) Тогда
			...
			//ОписаниеПозиции.region = СокрЛП(СтрокаНоменклатуры.МестоПоставки);
		КонецЕсли;
		Если СтрокаНоменклатуры.Количество > 0 Тогда
			// Единицы измерения
			ОписаниеПозиции.okei = ЕИСФабрикаXDTO.Создать(T_okei);
...
Функция ПолучитьОписаниеВалюты(ЕИСФабрикаXDTO, ПоляОписанияТранзакции, T_currency, Валюта = Неопределено)
	...
	Если ЗначениеЗаполнено(Валюта) Тогда
		ДанныеВалюты.code = Лев(СокрЛП(Валюта.Наименование), 3);
...
&НаСервереБезКонтекста
Функция ПолучитьОписаниеСтраны(ЕИСФабрикаXDTO, ПоляОписанияТранзакции, countriesOfOrigin, Страна = Неопределено)
	
	T_countryes = ЕИСФабрикаXDTO.Создать(countriesOfOrigin);
	T_country = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(countriesOfOrigin, "country");
	ДанныеСтраны = ЕИСФабрикаXDTO.Создать(T_country);
	
	Если ЗначениеЗаполнено(Страна) Тогда
		ДанныеСтраны.Digitalcode = СокрЛП(Страна.Код);
		Если НЕ ЗначениеЗаполнено(Страна.Код) Тогда
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
											ПоляОписанияТранзакции,
											СтрШаблон(Нстр("ru = 'У страны ""%1"" не указан числовой код! '"), Страна));
			Возврат Неопределено;
		КонецЕсли;
		ДанныеСтраны.name = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(
		Страна.Наименование);
	//Иначе
	//	ДанныеВалюты.code = "RUB";
	//	ДанныеВалюты.digitalCode = "643";
...
Функция СоздатьВложениеФайлов(ЭТП, ЕИСФабрикаXDTO, ВладелецФайлов, T_purchasePlanData)
	...
	Если НЕ ТИПЗНЧ(ВладелецФайлов) = ТИП("СправочникСсылка.ЗакупочныеПроцедуры") ИЛИ НЕ ВладелецФайлов.ГруппироватьФайлыЗакупочнойДокументацииВАрхив Тогда    
		...
		КонецЦикла;
	ИначеЕсли ТИПЗНЧ(ВладелецФайлов) = ТИП("СправочникСсылка.ЗакупочныеПроцедуры") И ВладелецФайлов.ГруппироватьФайлыЗакупочнойДокументацииВАрхив Тогда	           
		
		КаталогКорневой=КаталогВременныхФайлов()+"procurement documentation";
		СоздатьКаталог(КаталогКорневой);
		ЗаписьZIP = Новый ЗаписьZipФайла(КаталогКорневой+"\archive.zip");
		...
		Для Каждого ОписаниеФайла Из МассивОписанийФайлов Цикл
			...
			Если НЕ флЗарегистрированНаЕИС ИЛИ ФайлУИД = Неопределено Тогда
				...
				Файл = Новый Файл(КаталогКорневой+"\"+ОписаниеФайла.Наименование);  
				ПолноеИмяФайла=КаталогКорневой+"\"+ ОписаниеФайла.Наименование+"."+ОписаниеФайла.Расширение; 
				ПередаваемыеФайлы = Новый Массив;
				ПараметрыВыполнения = Новый Структура;
				ПараметрыВыполнения.Вставить("ДанныеФайла", Base64Строка(ДвоичныеДанныеФайла));
				ПараметрыВыполнения.Вставить("ПутьКФайлуВКэше", "");
				
				Описание = Новый ОписаниеПередаваемогоФайла(ОписаниеФайла.Наименование, ПолноеИмяФайла);
				...
				ДВ=ДвоичныеДанныеФайла;
				ДВ.записать(файл.полноеимя+"."+ОписаниеФайла.Расширение);  //файл.полноеимя+"."+ОписаниеФайла.Расширение
				
				ЗаписьZIP.Добавить(файл.полноеимя+"."+ОписаниеФайла.Расширение);
			Иначе
				// Не измененный документ
				prevDocument = ЕИСФабрикаXDTO.Создать(T_prevDocument);
				prevDocument.guid = ФайлУИД;
		...
		ИмяФайлаДанных =КаталогКорневой+"\archive.zip";
		
		ДвоичныеДанные = Новый ДвоичныеДанные(ИмяФайлаДанных);	
		document = ЕИСФабрикаXDTO.Создать(T_document);
		document.guid = ФайлУИД;
		document.fileName = "Архив закупочной документации.zip";  			
		document.content = Base64Строка(ДвоичныеДанные);
		Вложения.document.Добавить(document);  
...
&НаСервере
Функция ПолучитьТекстОписанияОтчетаОЗаключенныхДоговорах(ОписаниеОбмена, ЕИСФабрикаXDTO)
	ЭТП = ОписаниеОбмена.ЭТП;
	ПоляОписанияТранзакции = ОписаниеОбмена.ПоляОписанияТранзакции;
	РегистрацияОтчетаОЗаключенныхДоговорах = ОписаниеОбмена.ОбъектВыгрузки;
	ТекстОписания = Неопределено;
	Попытка
		Если НЕ ЗначениеЗаполнено(РегистрацияОтчетаОЗаключенныхДоговорах) Тогда
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
				ПоляОписанияТранзакции, 
				Нстр("ru = 'Не выбран отчет о заключении договора '"));
			Возврат Неопределено;
		КонецЕсли;
		ОписаниеРегистрации = ИнтеграцияЦУЗсЭТПСерверУХ.ПолучитьВерсиюОбъектаДляЕИС(РегистрацияОтчетаОЗаключенныхДоговорах, Истина);
		СохранитьУИДОбъекта(РегистрацияОтчетаОЗаключенныхДоговорах,, ОписаниеРегистрации.ГУИДВерсии);
		ОписаниеВерсии = ИнтеграцияЦУЗсЭТПСерверУХ.ПолучитьВерсиюОбъектаДляЕИС(РегистрацияОтчетаОЗаключенныхДоговорах);
		УИДИсполнения = ОписаниеВерсии.ГУИДВерсии;
		СохранитьУИДОбъекта(РегистрацияОтчетаОЗаключенныхДоговорах,, УИДИсполнения);
		ОписаниеВерсииДоговора = ИнтеграцияЦУЗсЭТПСерверУХ.ПолучитьВерсиюОбъектаДляЕИС(РегистрацияОтчетаОЗаключенныхДоговорах, Истина);
		СохранитьУИДОбъекта(РегистрацияОтчетаОЗаключенныхДоговорах,, ОписаниеВерсииДоговора.ГУИДВерсии);
		ПространствоИмен = "http://zakupki.gov.ru/223fz/purchase/1";
		//ПространствоИмен = "http://zakupki.gov.ru/223fz/purchaseContractAccountDataType/1";
		КорневыеОбъекты = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПолучитьКорневыеОбъектыПространстваИмен(
									ЕИСФабрикаXDTO, 
									ПространствоИмен);
		// Типы данных веб-сервиса
		T_purchaseContractAccount = КорневыеОбъекты.Получить("purchaseContractAccount").Тип;
		T_header = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
					T_purchaseContractAccount, "header");
		T_body = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(T_purchaseContractAccount, "body");

		T_performanceContractItem = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
			T_body, "item");
		T_performanceContractData = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
			T_performanceContractItem, "purchaseContractAccountData");
		T_customer = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
			T_performanceContractData, "customer"); // также используется для placer
		//T_mainInfo = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
		//	T_customer, "mainInfo");//T_performanceContractData, "contractInfo");
		ПакетДанныхИсполненияДоговора = ЕИСФабрикаXDTO.Создать(T_purchaseContractAccount);
		// Заголовок пакета.
 		ПакетДанныхИсполненияДоговора.header = ЕИСФабрикаXDTO.Создать(T_header);
 		ПакетДанныхИсполненияДоговора.header.guid = ПоляОписанияТранзакции.КлючСообщения;
 		ПакетДанныхИсполненияДоговора.header.createDateTime = ТекущаяДата();
		...
		ПакетДанныхИсполненияДоговора.body.item = ЕИСФабрикаXDTO.Создать(T_performanceContractItem);
		ПакетДанныхИсполненияДоговора.body.item.guid = Строка(Новый УникальныйИдентификатор);
		// Данные договора     purchaseContractAccountData		
		ПакетДанныхИсполненияДоговора.body.item.purchaseContractAccountData = 
								ЕИСФабрикаXDTO.Создать(T_performanceContractData);
		ДанныеИсполненияДоговора = ПакетДанныхИсполненияДоговора.body.item.purchaseContractAccountData;
		ДанныеИсполненияДоговора.guid = УИДИсполнения;

		ДанныеИсполненияДоговора.urlVSRZ = "zakupki.gov.ru";
		ДанныеИсполненияДоговора.creationDateTime = РегистрацияОтчетаОЗаключенныхДоговорах.Дата;
		Если НЕ ЗначениеЗаполнено(РегистрацияОтчетаОЗаключенныхДоговорах.Организация) Тогда
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
				ПоляОписанияТранзакции, 
				Нстр("ru = 'Не заполнена организация которая заключила договор! '"));
			Возврат Неопределено;
		...
		customer = ПолучитьОписаниеОрганизации(
						ЕИСФабрикаXDTO, 
						T_customer, 
						РегистрацияОтчетаОЗаключенныхДоговорах.ОрганизацияЗаказчик, 
						ПоляОписанияТранзакции);
		Если ПоляОписанияТранзакции.ОшибкаОбменаСЭТП Тогда
			Возврат Неопределено;
		КонецЕсли;
		...
		placer = ПолучитьОписаниеОрганизации(
						ЕИСФабрикаXDTO, 
						T_customer, 
						РегистрацияОтчетаОЗаключенныхДоговорах.Организация, 
						ПоляОписанияТранзакции);
		Если ПоляОписанияТранзакции.ОшибкаОбменаСЭТП Тогда
			Возврат Неопределено;
		КонецЕсли;
		...
		ДанныеИсполненияДоговора.customer = customer;       
		МесяцРегистрации = РегистрацияОтчетаОЗаключенныхДоговорах.ПериодРегистрации.ДатаНачала;
		ДанныеИсполненияДоговора.period = МесяцРегистрации;  
		//ДанныеИсполненияДоговора.docType = "MAIN";   
		//ДанныеИсполненияДоговора.typeCode = 6;   
		ДанныеИсполненияДоговора.docType = "MAIN";   
		ДанныеИсполненияДоговора.typeCode = 6;  
		
		phone = ЕИСФабрикаXDTO.Создать(ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(T_performanceContractData,"phone"));
		phone.code = Лев(customer.MainInfo.phone,4);	
		phone.number = Сред(customer.MainInfo.phone,5,7);	
		ДанныеИсполненияДоговора.phone = phone; 
		ДанныеИсполненияДоговора.Email = customer.MainInfo.Email; 
		
		T_contractAccountCaseInfo = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(T_performanceContractData,"contractAccountCaseInfo");
		
		Для Каждого Договор ИЗ РегистрацияОтчетаОЗаключенныхДоговорах.Договоры Цикл
			//Табличная часть в отчете может заполняться по реестру самого ЕИС, поэтому значения выгрузки могут отличаться от отправленных 
			contractAccountCaseInfo = ЕИСФабрикаXDTO.Создать(T_contractAccountCaseInfo);  
			T_contractInfo = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(T_contractAccountCaseInfo,"contractInfo");
            contractInfo = ЕИСФабрикаXDTO.Создать(T_contractInfo);
			contractInfo.Guid = ?(ЗначениеЗаполнено(Договор.Договор.УИД_ЕИС),Договор.Договор.УИД_ЕИС,Строка(Договор.Договор.УникальныйИдентификатор()));
			contractInfo.contractNumber = СТРЗаменить(Договор.Договор.РегистрационныйНомерЕИС," ","");
			contractAccountCaseInfo.contractInfo = contractInfo;       
			contractAccountCaseInfo.caseCode = Договор.КодСлучаяЗаключенияДоговораЕИС.КодЕИС; 
			contractAccountCaseInfo.subjectContract = Договор.Договор.КомментарийДляЕИС;
			contractAccountCaseInfo.contractsOverallCost = Договор.ЦенаДоговора;
			
			ДанныеИсполненияДоговора.contractAccountCaseInfo.Добавить(contractAccountCaseInfo);
		КонецЦикла;	    
		T_contractAccountTotal = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(T_performanceContractData,"contractAccountTotal");
		contractAccountTotal = ЕИСФабрикаXDTO.Создать(T_contractAccountTotal);     
		contractAccountTotal.purchaseCategory = "TOTAL";
		contractAccountTotal.purchaseCategoryDescription = "";
		contractAccountTotal.quantityNumber = "";  
		contractAccountTotal.amountNumber = РегистрацияОтчетаОЗаключенныхДоговорах.Договоры.Итог("ЦенаДоговора");
		ДанныеИсполненияДоговора.contractAccountTotal.Добавить(contractAccountTotal);
		
		Для Каждого Договор ИЗ РегистрацияОтчетаОЗаключенныхДоговорах.Договоры Цикл
			contractAccountTotal = ЕИСФабрикаXDTO.Создать(T_contractAccountTotal);     
			contractAccountTotal.purchaseCategory = "NOT_PLACED_320";
			contractAccountTotal.purchaseCategoryDescription = Договор.КодСлучаяЗаключенияДоговораЕИС.Наименование;
			contractAccountTotal.quantityNumber = 1;//Договор.КодСлучаяЗаключенияДоговораЕИС.КодЕИС;  
			contractAccountTotal.amountNumber = Договор.ЦенаДоговора;
			
			ДанныеИсполненияДоговора.contractAccountTotal.Добавить(contractAccountTotal);
		КонецЦикла;	    
		contractAccountTotal = ЕИСФабрикаXDTO.Создать(ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(T_performanceContractData, "contractAccountTotal"));
		
		ПакетДанныхИсполненияДоговора.Проверить();	
		ТекстОписания = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПолучитьТекстОписанияОбъекта(
							ЕИСФабрикаXDTO, 
							ПакетДанныхИсполненияДоговора, 
							"purchaseContractAccount");
...
&НаСервере
Процедура ДополнитьСПЗ_positionFinancialProvisionVolumes(Знач ДанныеСтроки, Знач ЕИСФабрикаXDTO, СтрокаПлана, Знач ТипСтрокиПлана)
	
	Перем positionFinancialProvisionVolumeData, positionFinancialProvisionVolumeDataType, positionFinancialProvisionVolumeItem, positionFinancialProvisionVolumes, СтрокаПланаФинансирование, Финансирование;
	
	СтрокаПланаФинансирование = ИнтеграцияЦУЗсЭТПСерверУХ.ИсточникиФинансированияИзБюджетаСПЗ(СтрокаПлана);
	
	Если СтрокаПлана.ИсточникиФинансированияЗаСчетБюджета.Количество() Тогда   
		
		ДанныеСтроки.maximumContractPriceFromBudget = Истина;
		
		positionFinancialProvisionVolumes = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(ТипСтрокиПлана, "positionFinancialProvisionVolumes");
		positionFinancialProvisionVolumeData = ЕИСФабрикаXDTO.Создать(positionFinancialProvisionVolumes);
		positionFinancialProvisionVolumeDataType = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(positionFinancialProvisionVolumes, "positionFinancialProvisionVolumeItem");
		
		Для Каждого Финансирование Из СтрокаПланаФинансирование Цикл   
			
			positionFinancialProvisionVolumeItem = ЕИСФабрикаXDTO.Создать(positionFinancialProvisionVolumeDataType); 
			positionFinancialProvisionVolumeItem.targetCode = СтрЗаменить(СокрЛП(Финансирование.ЦелеваяСтатьяРасходовКод), " ", "");
			positionFinancialProvisionVolumeItem.typeCode = СтрЗаменить(СокрЛП(Финансирование.ВидРасходовКод), " ", "");
			positionFinancialProvisionVolumeItem.financialVolumePositionRub = Финансирование.ОбъемФинансовогоОбеспечения;     
			
			positionFinancialProvisionVolumeData.positionFinancialProvisionVolumeItem.Добавить(positionFinancialProvisionVolumeItem); 
			
		КонецЦикла;
		
		ДанныеСтроки.positionFinancialProvisionVolumes = positionFinancialProvisionVolumeData;  
...
//	ДопПараметры.Вставить("packet_guid", ПоляОписанияТранзакции.КлючСообщения);
//	ДопПараметры.Вставить("packet_createDateTime", ТекущаяДатаСеанса());
//	РезультатТекстXML = 
//		ПолучитьКонвертерXDTO().ПреобразоватьПоУмолчаниюОбъект1СВТекстXML(
//			ОписаниеОбмена.ОбъектВыгрузки,
//			ТипЗнч(ОписаниеОбмена.ОбъектВыгрузки),
//			"ns1:Пакет.purchaseContractAccount",
//			ДопПараметры);
//	Если РезультатТекстXML.ЕстьОшибка Тогда
//		ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
//			ПоляОписанияТранзакции, РезультатТекстXML.Ошибка);
//		Возврат Неопределено;
//	Иначе
//		Возврат РезультатТекстXML.Результат;
//	КонецЕсли;
//КонецФункции

//&НаСервере
//Функция ПолучитьКонвертерXDTO()
//	КонвертерXDTOОбъект = ПолучитьЗначениеВКонтексте("КонвертерXDTOОбъект");
//	Если КонвертерXDTOОбъект = Неопределено Тогда
//		КонвертерXDTOОбъект =
//			РеквизитФормыВЗначение("КонвертерXDTOНаФорме");
//		УстановитьЗначениеВКонтексте("КонвертерXDTOОбъект", КонвертерXDTOОбъект);
//		УстановитьЗначениеВКонтексте("ФабрикаXDTO", 
//			КонвертерXDTOОбъект.ПолучитьФабрикуXDTO());
//	КонецЕсли;
//	Возврат КонвертерXDTOОбъект;
//КонецФункции
