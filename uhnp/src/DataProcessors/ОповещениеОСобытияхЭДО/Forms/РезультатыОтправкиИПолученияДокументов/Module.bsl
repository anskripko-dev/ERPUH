...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "УдаленаУчетнаяЗапись" Тогда
		Отбор = Новый Структура("Идентификатор", Параметр);
		НайденныеСтроки = УчетныеЗаписи.НайтиСтроки(Отбор);
		Для Каждого СтрокаУчетнойЗаписи Из НайденныеСтроки Цикл
			УчетныеЗаписи.Удалить(СтрокаУчетнойЗаписи);
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
