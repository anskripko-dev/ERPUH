...
//			// НАЧАЛО Мещеряков Алексей 10.10.2024 Выполнение задач №
...
//			//ИмяМакета = "КонтурЭДО_4_47_" + ИмяМодуля;
//			ИмяМакета = "КонтурДиадок_" + ИмяМодуля;
//			// КОНЕЦ Мещеряков Алексей 23.05.2025 Выполнение задач №000053776
//								
//			Если Метаданные.Обработки.Найти(ИмяМакета) <> Неопределено Тогда
//				Результат = Обработки[ИмяМакета].Создать();
//			КонецЕсли; 
//			// КОНЕЦ Мещеряков Алексей 10.10.2024 Выполнение задач № 
			
			Если ИмяМодуля = "Модуль_ИнтеграцияУТ11" Тогда
				ИмяМакета = "КонтурДиадок_" + ИмяМодуля;
				Если Метаданные.Обработки.Найти(ИмяМакета) <> Неопределено Тогда
					Результат = Обработки[ИмяМакета].Создать(); 
				КонецЕсли;							
...
Функция ЗначениеСвойстваОбъектаИспользуемоеИмяФайла()
	...
	ЕстьРеквизитИлиСвойствоОбъектаЛок(ЭтотОбъект, "ИспользуемоеИмяФайла", Результат);
