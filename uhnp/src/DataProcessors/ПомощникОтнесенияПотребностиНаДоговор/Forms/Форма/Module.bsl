...
Функция ПолучитьТаблицуПотребностей(НачалоПериодаВход = Неопределено, КонецПериодаВход = Неопределено, ОрганизацияВход = Неопределено, МестоПоставкиВход = Неопределено, НоменклатураВход = Неопределено, ТоварнаяКатегорияВход = Неопределено)
	...
	|	ВыбранныеПотребностиВНоменклатуре.АналитикаПланирования.АналитикаПотребностей.Приоритет КАК Приоритет,
	|	Периоды.Ссылка как Период,
	|	ВыбранныеПотребностиВНоменклатуре.АналитикаПланирования.ДатаОперации КАК ДатаПоставки,
	...
	|	ВыбранныеПотребностиВНоменклатуре.АналитикаПланирования.АналитикаПотребностей.Менеджер КАК Менеджер,
	...
	|	ВыбранныеПотребностиВНоменклатуре.КоличествоОстаток КАК Количество,
	|	ВыбранныеПотребностиВНоменклатуре.СуммаОстаток КАК Сумма,
	...
	|			ТОГДА ВыбранныеПотребностиВНоменклатуре.СуммаОстаток / ВыбранныеПотребностиВНоменклатуре.КоличествоОстаток
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Цена,
	|	ВЫБОР
	|		КОГДА ВыбранныеПотребностиВНоменклатуре.АналитикаПланирования.АналитикаНоменклатуры.Номенклатура ССЫЛКА Справочник.Номенклатура
	|			ТОГДА ВыбранныеПотребностиВНоменклатуре.АналитикаПланирования.АналитикаНоменклатуры.Номенклатура.ТоварнаяКатегория
	|		КОГДА ВыбранныеПотребностиВНоменклатуре.АналитикаПланирования.АналитикаНоменклатуры.Номенклатура ССЫЛКА Справочник.ТоварныеКатегории
	|			ТОГДА ВыбранныеПотребностиВНоменклатуре.АналитикаПланирования.АналитикаНоменклатуры.Номенклатура
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ТоварныеКатегории.ПустаяСсылка)
	|	КОНЕЦ КАК ТоварнаяКатегория
	|ИЗ
	|	РегистрНакопления.ПланированиеПотребностей.Остатки(, АналитикаПланирования.Этап = &ЭтапПланирования) КАК ВыбранныеПотребностиВНоменклатуре
	|		Левое СОЕДИНЕНИЕ Справочник.Периоды КАК Периоды
	|		ПО ВыбранныеПотребностиВНоменклатуре.АналитикаПланирования.ДатаОперации >= Периоды.ДатаНачала
	|			И ВыбранныеПотребностиВНоменклатуре.АналитикаПланирования.ДатаОперации <= Периоды.ДатаОкончания 
	|		и &ПериодичностьЦентрализованныхЗакупок = Периоды.Периодичность
	|ГДЕ
	|	ВЫБОР
	...
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ВыбранныеПотребностиВНоменклатуре.АналитикаПланирования.ДатаОперации >= &ДатаНачала
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА &ДатаОкончания = НЕОПРЕДЕЛЕНО
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ВыбранныеПотребностиВНоменклатуре.АналитикаПланирования.ДатаОперации <= &ДатаОкончания
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА НЕ &ЕстьНоменклатура
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ВыбранныеПотребностиВНоменклатуре.АналитикаПланирования.АналитикаНоменклатуры.Номенклатура В (&Номенклатура)
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА &Организация = НЕОПРЕДЕЛЕНО
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ВыбранныеПотребностиВНоменклатуре.АналитикаПланирования.АналитикаСтруктуры.Организация В ИЕРАРХИИ (&Организация)
	|		КОНЕЦ
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВыбранныеПотребностиВНоменклатуре.АналитикаПланирования.ДатаОперации,
	|	ВыбранныеПотребностиВНоменклатуре.АналитикаПланирования.АналитикаСтруктуры.Организация,
	|	ВыбранныеПотребностиВНоменклатуре.АналитикаПланирования.АналитикаНоменклатуры.Номенклатура";
	ПериодичностьЦентрализованныхЗакупок = ЦентрализованныеЗакупкиУХ.ПолучитьПериодичностьЗакупок();
	Запрос.УстановитьПараметр("ПериодичностьЦентрализованныхЗакупок",ПериодичностьЦентрализованныхЗакупок);
	Запрос.УстановитьПараметр("ЭтапПланирования",ПредопределенноеЗначение("Перечисление.ЭтапыПланированияПотребностей.ЗаявкиСогласованные"));
...
Функция ПолучитьСтрокуСвертки()
	РезультатФункции = "АналитикаИсточника,АналитикаПланирования,ДатаПоставки,Номенклатура, Период, Приоритет, Характеристика, Организация, МестоПоставки, Проект, Менеджер, ДоговорСПокупателем, СтавкаНДС, Коэффициент, ЕдиницаИзмерения, Цена, ТоварнаяКатегория";
	Возврат РезультатФункции;	
...
Функция СкорректироватьТаблицуПотребностиНаРазмещение(ТаблицаПотребностиВход, ТаблицаРазмещениеВход)
	...
	Запрос.Текст = 
		...
		|	ТЗ_Потребность.ДатаПоставки КАК ДатаПоставки,
		|	ТЗ_Потребность.АналитикаИсточника КАК АналитикаИсточника,
		|	ТЗ_Потребность.АналитикаПланирования КАК АналитикаПланирования,
		...
		|	ВТ_Потребность.ДатаПоставки КАК ДатаПоставки,
		|	ВТ_Потребность.АналитикаИсточника КАК АналитикаИсточника,
		|	ВТ_Потребность.АналитикаПланирования КАК АналитикаПланирования
		...
		|	ВТ_Остатки.АналитикаИсточника КАК АналитикаИсточника,
		|	ВТ_Остатки.АналитикаПланирования КАК АналитикаПланирования
		|ИЗ
...
Процедура ПеренестиДанныеРазмещенияВСтрокуТаблицы(ДанныеРазмещенияВход, СтрокаТаблицыИзм)
	...
	СтрокаТаблицыИзм.АналитикаИсточника		 = ДанныеРазмещенияВход.АналитикаИсточника;      
	СтрокаТаблицыИзм.АналитикаПланирования	 = ДанныеРазмещенияВход.АналитикаПланирования;      
	СтрокаТаблицыИзм.ДатаПоставки			 = ДанныеРазмещенияВход.ДатаПоставки;      
...
Функция СкорректироватьТаблицуНоменклатураДоговоров(ТаблицаНоменклатураДоговоровВход, ТаблицаРазмещениеВход, ДоговорВход)
	...
	Попытка
		...
		|	ТЗ_НоменклатураДоговора.ТоварнаяКатегория КАК ДатаПоставки,
		|	ТЗ_НоменклатураДоговора.ТоварнаяКатегория КАК АналитикаИсточника,
		|	ТЗ_НоменклатураДоговора.ТоварнаяКатегория КАК АналитикаПланирования
		...
		|	ТЗ_Размещение.ДатаПоставки КАК ДатаПоставки,
		|	ТЗ_Размещение.АналитикаПланирования КАК АналитикаПланирования,
		|	ТЗ_Размещение.АналитикаИсточника КАК АналитикаИсточника
		|ПОМЕСТИТЬ ВТ_Размещение
		|ИЗ
		|	&ТЗ_Размещение КАК ТЗ_Размещение
		|;
		...
		|	ВТ_НоменклатураДоговора.ДатаПоставки КАК ДатаПоставки,
		|	ВТ_НоменклатураДоговора.АналитикаИсточника КАК АналитикаИсточника,
		|	ВТ_НоменклатураДоговора.АналитикаПланирования КАК АналитикаПланирования
		...
		|	ВТ_Размещение.ДатаПоставки КАК ДатаПоставки,
		|	ВТ_Размещение.АналитикаИсточника КАК АналитикаИсточника,
		|	ВТ_Размещение.АналитикаПланирования КАК АналитикаПланирования
		...
		|	ЕСТЬNULL(ВТ_НоменклатураВерсии.ДатаПоставки, ВТ_РазмещениеВерсия.ДатаПоставки) КАК ДатаПоставки,
		|	ЕСТЬNULL(ВТ_НоменклатураВерсии.АналитикаИсточника, ВТ_РазмещениеВерсия.АналитикаИсточника) КАК АналитикаИсточника,
		|	ЕСТЬNULL(ВТ_НоменклатураВерсии.АналитикаПланирования, ВТ_РазмещениеВерсия.АналитикаПланирования) КАК АналитикаПланирования
		...
		|	ЕСТЬNULL(ВТ_НоменклатураВерсии.ДатаПоставки, ВТ_РазмещениеВерсия.ДатаПоставки),
		|	ЕСТЬNULL(ВТ_НоменклатураВерсии.АналитикаИсточника, ВТ_РазмещениеВерсия.АналитикаИсточника),
		|	ЕСТЬNULL(ВТ_НоменклатураВерсии.АналитикаПланирования, ВТ_РазмещениеВерсия.АналитикаПланирования)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВТ_Объединение.ДатаПоставки КАК ДатаПоставки,
		|	ВТ_Объединение.АналитикаИсточника КАК АналитикаИсточника,
		|	ВТ_Объединение.АналитикаПланирования КАК АналитикаПланирования
		|ИЗ
...
Функция ЗаполнитьВерсиюСоглашения(ВерсияОбъектИзм, МассивСтрокВход)
	...
	Попытка
		Для Каждого ТекМассивСтрокВход Из МассивСтрокВход Цикл
			...
			ЦелеваяСтрока.Лот					 = ТекМассивСтрокВход.АналитикаПланирования.ОбъектПланирования.Объект;
			ЦелеваяСтрока.АналитикаПланирования	 = ТекМассивСтрокВход.АналитикаПланирования;
			ЦелеваяСтрока.АналитикаИсточника	 = ТекМассивСтрокВход.АналитикаИсточника;
			ЦелеваяСтрока.ДатаПоставки			 = ТекМассивСтрокВход.ДатаПоставки;
			ЦелеваяСтрока.Цена 					 = ТекМассивСтрокВход.Цена;	
