...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УведомлениеОДоступностиЧастичнойОплаты = Нстр("ru = 'Частичная доставка недоступна';
													|en = 'Partial delivery is unavailable'");
	...
	Если Значение.Значение = 1 Тогда
		...
		Обещание = ВопросАсинх(
			НСтр("ru = 'Товары заказа будут перезаполнены из документов-оснований. Продолжить?';
				|en = 'The order goods will be refilled from the base documents. Continue?'"),
			РежимДиалогаВопрос.ДаНет,,
	...
	ИначеЕсли Значение.Значение = 2 Тогда
		...
		Обещание = ВопросАсинх(
			НСтр("ru = 'Изменения товарного состава заказа не будут отправлены в сервис. Продолжить?';
				|en = 'The order goods changes will not be sent to the service. Continue?'"),
			РежимДиалогаВопрос.ДаНет,,
			КодВозвратаДиалога.Нет);
