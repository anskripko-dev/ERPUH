	...
	Исключение
		...
		Если НЕ ТихийРежим Тогда
			...
			#Иначе
				...
				СтрокаШаблона = Нстр("ru = 'Ошибка открытия файла MS Excel: %1'");
...
Функция Импортировать(СоответствиеИмпортируемыхОбластей = Неопределено, ДеревоКоординатРеквизитов = Неопределено) Экспорт
	...
	Если ИмяЛиста = Неопределено Тогда
		...
		Иначе
			...
			#Если Клиент Тогда
				...
				Иначе
					ВыбранныйЛист = ЛистыОтчета.ВыбратьЭлемент(Нстр("ru = 'Выберите лист для импорта'"));
				КонецЕсли;
			#Иначе
	...
	Исключение
		...
		СтрокаШаблона = Нстр("ru = 'Ошибка при обращении к ячейкам файла Excel.
		|%1'");
	...
	Если НЕ РежимИмпортаДанных Тогда
		Если НЕ ТихийРежим Тогда
			#Если Клиент Тогда
			Состояние(Нстр("ru = 'Установка ширины столбцов ...'"));
			#КонецЕсли
		КонецЕсли;
		Сдвиг = 0;
		...
		Если НЕ ТихийРежим Тогда
			#Если Клиент Тогда
			Состояние(Нстр("ru = 'Установка высоты строк ...'"));
			#КонецЕсли
		КонецЕсли;
		Сдвиг = 0;
	...
	#Если Клиент Тогда
		// именование секций
		Если НЕ ТихийРежим Тогда
			Состояние(Нстр("ru = 'Именование горизонтальных секций ...'"));
		КонецЕсли;
	#КонецЕсли
	...
	Если ИспользованиеГлобальныхИмен Тогда
		...
		Если НЕ ТихийРежим Тогда
			#Если Клиент Тогда
				...
				Если ИспользованиеГлобальныхИмен тогда
					Состояние(Нстр("ru = 'Именование горизонтальных секций. Используются глобальные имена ...'"));
				Иначе
					Состояние(Нстр("ru = 'Именование горизонтальных секций. Используются локальные имена ...'"));
				КонецЕсли;
	...
	Для Стр = 1 По Низ Цикл
		...
		Если НЕ СкрытыеСтроки[Стр - 1] Тогда
			...
			Для Столб = 1 По Право Цикл
				...
				Если НЕ ТихийРежим Тогда
					#Если Клиент Тогда
						СтрокаШаблона = Нстр("ru = '%1 %% завершено ...'");
						
						Если Не ПустаяСтрока(СтрокаШаблона) тогда							
							Состояние(СтрШаблон(СтрокаШаблона, Строка(Цел((((Стр - 1) * Право + Столб) / ВсегоЯчеек) * 100))));
						КонецЕсли;
					#КонецЕсли
				КонецЕсли;
	...
	Если НЕ ТихийРежим Тогда
		#Если Клиент Тогда
		Состояние(Нстр("ru = 'Объединение ячеек ...'"));
		#КонецЕсли
	КонецЕсли;
