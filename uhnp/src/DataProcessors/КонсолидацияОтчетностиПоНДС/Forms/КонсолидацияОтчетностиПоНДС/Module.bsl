...
Процедура ИсключитьФайлы(Команда)
	...
	ИдентификаторыВыделенныхСтрок = Элементы.ФайлыВыгрузки.ВыделенныеСтроки;
	
	ИсключитьФайлыНаСервере(ИдентификаторыВыделенныхСтрок);
...
&НаСервере
Процедура ИсключитьФайлыНаСервере(ИдентификаторыВыделенныхСтрок)
	
	Для Каждого ФайлВыгрузкиИдентификаторСтроки Из ИдентификаторыВыделенныхСтрок Цикл
...
Функция КонсолидироватьНаСервере(ТекстВыгрузки, ОшибкаНаСервере, ЗаполнитьДекларацию = Ложь, ЗаполнитьТолькоПараметры = Ложь, СохраненныйДок = Неопределено) Экспорт
	...
	ПараметрыВыгрузки.Вставить("ИмяФайлаСхемы2026", "NO_NDS_1_003_00_05_12_01.xsd");
		...
		Если СохраненныйДок <> Неопределено
			...
			ОшибкаНаСервере.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Версия файла выгрузки ""%1"" пока не поддерживается';
					|en = 'Export file version %1 is not supported yet'"), ПараметрыВыгрузки.ВерсФорм));
			Возврат Ложь;
		КонецЕсли;
	...
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2026Кв1" Тогда
		ИДРезультатаВыгрузки = "НДС2026Разделы1_7";
	КонецЕсли;
...
Функция СформироватьПараметрыВыгрузки(ФайлВыгрузки, ОписаниеОшибки = Неопределено)
	...
	ИначеЕсли ПараметрыВыгрузки.ВерсФорм = "5.12" Тогда
		ПараметрыВыгрузки.Вставить("ПравилаЗаполнения", "2026Кв1");
	КонецЕсли;
...
Процедура ИнициализироватьТаблицыРазделов9_91(СтруктураПараметров)
	...
	КонецЕсли;
	Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
		ИтогиРаздел9.Вставить("СтПродБезНДС20Или18", 0);
		ИтогиРаздел9.Вставить("СтПродБезНДС22",      0);
		ИтогиРаздел9.Вставить("СумНДСВсКПр20Или18",  0);
		ИтогиРаздел9.Вставить("СумНДСВсКПр22",       0);
	КонецЕсли;
	...
	КонецЕсли;
	Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
		ИтогиРаздел91.Вставить("ИтСтПродКПр22",        0);
		ИтогиРаздел91.Вставить("ИтСтПродКПр20Или18",   0);
		ИтогиРаздел91.Вставить("СумНДСИтКПр22",        0);
		ИтогиРаздел91.Вставить("СумНДСИтКПр20Или18",   0);
		ИтогиРаздел91.Вставить("СтПродВсП1Р9_22",      0);
		ИтогиРаздел91.Вставить("СтПродВсП1Р9_20Или18", 0);
		ИтогиРаздел91.Вставить("СумНДСВсП1Р9_22",      0);
		ИтогиРаздел91.Вставить("СумНДСВсП1Р9_20Или18", 0);
	КонецЕсли;
		...
		Пока ОбъектЧтениеXML.Прочитать() Цикл
			Если ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				...
				ИначеЕсли ИмяУзла = "ДокПдтвУпл" Тогда
					АтрУзла = Новый Структура("НомДокПдтвУпл,ДатаДокПдтвУпл",
						СокрЛП(АтрибутыУзла["НомДокПдтвУпл"]), ФорматСтрДата(СокрЛП(АтрибутыУзла["ДатаДокПдтвУпл"])));
					ТекущиеДанные.ДокПдтвУпл.Добавить(АтрУзла);
				ИначеЕсли ИмяУзла = "СвЧастСчФ" Тогда
					АтрУзла = Новый Структура("НомЧастСчФ,ДатаЧастСчФ",
						СокрЛП(АтрибутыУзла["НомЧастСчФ"]), ФорматСтрДата(СокрЛП(АтрибутыУзла["ДатаЧастСчФ"])));
					ТекущиеДанные.СвЧастСчФ.Добавить(АтрУзла);
		...
		Пока ОбъектЧтениеXML.Прочитать() Цикл
			Если ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				...
				Если ИмяУзла = "КнигаПрод" Тогда
					...
					КонецЕсли;
					Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
						СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС20Или18 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС20Или18
							+ ЧислоИзСтроки(АтрибутыУзла["СтПродБезНДС20"]) + ЧислоИзСтроки(АтрибутыУзла["СтПродБезНДС18"]);
						СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС22      = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС22 + ЧислоИзСтроки(АтрибутыУзла["СтПродБезНДС22"]);
						СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр20Или18  = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр20Или18
							+ ЧислоИзСтроки(АтрибутыУзла["СумНДСВсКПр20"]) + ЧислоИзСтроки(АтрибутыУзла["СумНДСВсКПр18"]);
						СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр22       = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр22 + ЧислоИзСтроки(АтрибутыУзла["СумНДСВсКПр22"]);
					КонецЕсли;
				...
				ИначеЕсли ИмяУзла = "КнПродСтр" Тогда
					...
					КонецЦикла;
					Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
						ТекущиеДанные.СтоимПродСФ20Или18 = ТекущиеДанные.СтоимПродСФ20 + ТекущиеДанные.СтоимПродСФ18;
						ТекущиеДанные.СумНДССФ20Или18    = ТекущиеДанные.СумНДССФ20    + ТекущиеДанные.СумНДССФ18;
					КонецЕсли;
				ИначеЕсли ИмяУзла = "РегНомТД"
					И (СтруктураПараметров.ПравилаЗаполнения = "2017Кв1"
				   ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		...
		Пока ОбъектЧтениеXML.Прочитать() Цикл
			Если ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				...
				Если ИмяУзла = "КнигаПродДЛ" Тогда
					...
					КонецЕсли;
					Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
						СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр22        = СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр22        + ЧислоИзСтроки(АтрибутыУзла["ИтСтПродКПр22"]);
						СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр20Или18   = СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр20Или18
							+ ЧислоИзСтроки(АтрибутыУзла["ИтСтПродКПр20"]) + ЧислоИзСтроки(АтрибутыУзла["ИтСтПродКПр18"]);
						СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр22        = СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр22        + ЧислоИзСтроки(АтрибутыУзла["СумНДСИтКПр22"]);
						СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр20Или18   = СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр20Или18
							+ ЧислоИзСтроки(АтрибутыУзла["СумНДСИтКПр20"]) + ЧислоИзСтроки(АтрибутыУзла["СумНДСИтКПр18"]);
						СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_22      = СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_22      + ЧислоИзСтроки(АтрибутыУзла["СтПродВсП1Р9_22"]);
						СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_20Или18 = СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_20Или18
							+ ЧислоИзСтроки(АтрибутыУзла["СтПродВсП1Р9_20"]) + ЧислоИзСтроки(АтрибутыУзла["СтПродВсП1Р9_18"]);
						СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_22      = СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_22      + ЧислоИзСтроки(АтрибутыУзла["СумНДСВсП1Р9_22"]);
						СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_20Или18 = СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_20Или18
							+ ЧислоИзСтроки(АтрибутыУзла["СумНДСВсП1Р9_20"]) + ЧислоИзСтроки(АтрибутыУзла["СумНДСВсП1Р9_18"]);
					КонецЕсли;
...
Функция ВыгрузкаРаздел8(СтруктураПараметров)
		...
		Для Каждого СтрокаДанных Из Данные.Записи Цикл
			...
			Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
				Для Инд = 0 По СтрокаДанных.СвЧастСчФ.Количество() - 1 Цикл
					КомпоновщикXML_СоздатьУзел(КомпоновщикXML, "СвЧастСчФ");
					
					КомпоновщикXML_ВывестиАтрибут(КомпоновщикXML, "НомЧастСчФ", СтрокаДанных.СвЧастСчФ[Инд].НомЧастСчФ, "Т,1000");
					КомпоновщикXML_ВывестиАтрибут(КомпоновщикXML, "ДатаЧастСчФ", СтрокаДанных.СвЧастСчФ[Инд].ДатаЧастСчФ, "Т,10");
			...
			Если СтруктураПараметров.ПравилаЗаполнения = "2017Кв1"
			 ...
			 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4" Тогда
				...
				КонецЦикла;
			ИначеЕсли СтруктураПараметров.ПравилаЗаполнения <> "2015Кв1" Тогда // "2021Кв3", "2022Кв1", "2023Кв1", "2025Кв1", "2026Кв1"
	...
	ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
		ПрефиксРезультатаВыгрузки = "НДС2026";
	КонецЕсли;
...
Функция ВыгрузкаРаздел9(СтруктураПараметров)
	...
	Если ПараметрыВыгрузки.ВидДок = "0" ИЛИ ПараметрыВыгрузки["АктуальностьСведений" + ИмяРаздела] = "0" Тогда
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СтПродБезНДС22", Данные.Итоги.СтПродБезНДС22, "Ч,19,2");
		КонецЕсли;
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
		...
		КонецЕсли;
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СтПродБезНДС20", Данные.Итоги.СтПродБезНДС20Или18, "Ч,19,2");
		Иначе
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДСВсКПр22", Данные.Итоги.СумНДСВсКПр22, "Ч,19,2");
		КонецЕсли;
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
		...
		КонецЕсли;
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДСВсКПр20", Данные.Итоги.СумНДСВсКПр20Или18, "Ч,19,2");
		Иначе
		...
		Для Каждого СтрокаДанных Из Данные.Записи Цикл
			...
			Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
				КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СтоимПродСФ22", СтрокаДанных.СтоимПродСФ22, "Ч,19,2");
			КонецЕсли;
			...
			КонецЕсли;
			Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
				КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СтоимПродСФ20", СтрокаДанных.СтоимПродСФ20Или18, "Ч,19,2");
			Иначе
			...
			Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
				...
				Если ВыгружатьНулевойАтрибут_СумНДССФ22 Тогда
					КомпоновщикXML_ВывестиАтрибут(КомпоновщикXML, "СумНДССФ22", СтрокаДанных.СумНДССФ22, "Ч,19,2");
				Иначе
					КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДССФ22", СтрокаДанных.СумНДССФ22, "Ч,19,2");
				КонецЕсли;
			КонецЕсли;
			...
			Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
				ВыгружатьНулевойАтрибут_СумНДССФ20 = ЗначениеЗаполнено(СтрокаДанных.СтоимПродСФ20Или18);
				Если ВыгружатьНулевойАтрибут_СумНДССФ20 Тогда
					КомпоновщикXML_ВывестиАтрибут(КомпоновщикXML, "СумНДССФ20", СтрокаДанных.СумНДССФ20Или18, "Ч,19,2");
				Иначе
					КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДССФ20", СтрокаДанных.СумНДССФ20Или18, "Ч,19,2");
				КонецЕсли;
			Иначе
...
Функция ВыгрузкаРаздел9_Прил1(СтруктураПараметров)
	...
	Если ПараметрыВыгрузки.ВидДок = "0" ИЛИ ПараметрыВыгрузки["АктуальностьСведений" + ИмяРаздела] = "0" Тогда
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "ИтСтПродКПр22", Данные.Итоги.ИтСтПродКПр22, "Ч,19,2");
		КонецЕсли;
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
		...
		КонецЕсли;
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "ИтСтПродКПр20",   Данные.Итоги.ИтСтПродКПр20Или18, "Ч,19,2");
		Иначе
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДСИтКПр22", Данные.Итоги.СумНДСИтКПр22, "Ч,19,2");
		КонецЕсли;
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
		...
		КонецЕсли;
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДСИтКПр20",   Данные.Итоги.СумНДСИтКПр20Или18, "Ч,19,2");
		Иначе
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СтПродВсП1Р9_22", Данные.Итоги.СтПродВсП1Р9_22, "Ч,19,2");
		КонецЕсли;
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
		...
		КонецЕсли;
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СтПродВсП1Р9_20",   Данные.Итоги.СтПродВсП1Р9_20Или18, "Ч,19,2");
		Иначе
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДСВсП1Р9_22", Данные.Итоги.СумНДСВсП1Р9_22, "Ч,19,2");
		КонецЕсли;
		Если СтруктураПараметров.ПравилаЗаполнения = "2019Кв1"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4"
		 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2021Кв3"
		...
		КонецЕсли;
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML, "СумНДСВсП1Р9_20",   Данные.Итоги.СумНДСВсП1Р9_20Или18, "Ч,19,2");
		Иначе
...
Процедура ВыгрузитьСведенияОРегНомереДеклНаТоварыИлиОПрослеживаемомТоваре(КомпоновщикXML, СтрокаДанных)
	...
	Для Инд = 0 По СтрокаДанных.СвРегНом.Количество() - 1 Цикл
		...
		КомпоновщикXML_ВывестиНеобязательныйАтрибут(КомпоновщикXML,
			"СтоимТовПросл", СтрокаДанных.СвРегНом[Инд].СтоимТовПросл, "Ч,19,2");
...
Процедура ПостобработкаДереваВыгрузки(ДеревоВыгрузки)
	...
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2017Кв1"
		  ...
		  ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2025Кв1"
		  ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2026Кв1" Тогда
...
Функция ИзвлечьСтруктуруXML()
	...
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2026Кв1" Тогда
		ИмяМакетаСтруктурыВыгрузки = "СхемаВыгрузки_NO_NDS_1_003_00_05_12_01";
	КонецЕсли;
...
Функция ПолучитьФормуОтчетаНаКлиенте(ПараметрыФормыОтчета)
	...
	Если СтрВыбраннаяФорма <> "ФормаОтчета2015Кв1"
	   ...
	   И СтрВыбраннаяФорма <> "ФормаОтчета2025Кв1"
	   И СтрВыбраннаяФорма <> "ФормаОтчета2026Кв1" Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Консолидация отчетов по НДС реализована только для форм:
			...
			|""Приложение № 1 к приказу ФНС России от 5 ноября 2024 г. № ЕД-7-3/989@
			|(в ред. приказа ФНС России от 18.12.2025 № ЕД-7-3/1227@)""!';
		...
		Для Каждого Раздел Из ПоказателиОтчета Цикл
			...
			Если ИмяРаздела = "Титульный" Тогда
				...
				Если ПараметрыВыгрузки.ПравилаЗаполнения <> "2020Кв4"
				   ...
				   И ПараметрыВыгрузки.ПравилаЗаполнения <> "2025Кв1"
				   И ПараметрыВыгрузки.ПравилаЗаполнения <> "2026Кв1" Тогда
...
Процедура ЗаписатьВСегментДанныхСтрокуРаздела8(СтрокаРаздела8, СтруктураПараметров)
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
		СведенияОЧастСчФ = "";
		Для Каждого НомерДатаЧастСчФ Из СтрокаРаздела8.СвЧастСчФ Цикл
			НомерЧастСчФ = Неопределено;
			НомерДатаЧастСчФ.Свойство("НомЧастСчФ", НомерЧастСчФ);
			ДатаЧастСчФ  = Неопределено;
			НомерДатаЧастСчФ.Свойство("ДатаЧастСчФ", ДатаЧастСчФ);
			НомДатаЧастСчФ = ?(ЗначениеЗаполнено(НомерЧастСчФ), "" + НомерЧастСчФ + НСтр("ru = ' от ';
																						|en = ' от '"), "") + ДатаЧастСчФ;
			СведенияОЧастСчФ = СведенияОЧастСчФ + ?(ЗначениеЗаполнено(СведенияОЧастСчФ), "," + Символы.ПС, "") + НомДатаЧастСчФ;
		КонецЦикла;
		ПараметрыСтроки.НомерОтДатаПриОтгрузке = СведенияОЧастСчФ;
	КонецЕсли;
...
Процедура ЗаписатьВСегментДанныхСтрокуРаздела81(СтрокаРаздела81, СтруктураПараметров)
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
		СведенияОЧастСчФ = "";
		Для Каждого НомерДатаЧастСчФ Из СтрокаРаздела81.СвЧастСчФ Цикл
			НомерЧастСчФ = Неопределено;
			НомерДатаЧастСчФ.Свойство("НомЧастСчФ", НомерЧастСчФ);
			ДатаЧастСчФ  = Неопределено;
			НомерДатаЧастСчФ.Свойство("ДатаЧастСчФ", ДатаЧастСчФ);
...
Процедура ЗаписатьВСегментДанныхСтрокуРаздела9(СтрокаРаздела9, СтруктураПараметров)
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
		СведенияОЧастСчФ = "";
		Для Каждого НомерДатаЧастСчФ Из СтрокаРаздела9.СвЧастСчФ Цикл
			НомерЧастСчФ = Неопределено;
			НомерДатаЧастСчФ.Свойство("НомЧастСчФ", НомерЧастСчФ);
			ДатаЧастСчФ  = Неопределено;
			НомерДатаЧастСчФ.Свойство("ДатаЧастСчФ", ДатаЧастСчФ);
			...
			СведенияОЧастСчФ = СведенияОЧастСчФ + ?(ЗначениеЗаполнено(СведенияОЧастСчФ), "," + Символы.ПС, "") + НомДатаЧастСчФ;
		КонецЦикла;
		ПараметрыСтроки.АвансовыеСчетаФактуры = СведенияОЧастСчФ;
	КонецЕсли;
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
		ПараметрыСтроки.СуммаБезНДС22 = ?(ЗначениеЗаполнено(СтрокаРаздела9.СтоимПродСФ22), СтрокаРаздела9.СтоимПродСФ22, Неопределено);
		ПараметрыСтрокиПром.Вставить("СуммаБезНДС20Или18",
			?(ЗначениеЗаполнено(СтрокаРаздела9.СтоимПродСФ20Или18), СтрокаРаздела9.СтоимПродСФ20Или18, Неопределено));
	Иначе
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
		ПараметрыСтроки.НДС22 = ?(ЗначениеЗаполнено(СтрокаРаздела9.СумНДССФ22), СтрокаРаздела9.СумНДССФ22, Неопределено);
		ПараметрыСтрокиПром.Вставить("НДС20Или18",
			?(ЗначениеЗаполнено(СтрокаРаздела9.СумНДССФ20Или18), СтрокаРаздела9.СумНДССФ20Или18, Неопределено));
	Иначе
	...
	Если ПараметрыРаздела.ПоследняяСтрока = Истина Тогда
		...
		КонецЕсли;
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			СтруктураПараметров.ИтогиРаздел9.Свойство("СтПродБезНДС22", ЗначениеСвойства);
			СекцияВсего.Параметры.СуммаБезНДС22 = ?(ЗначениеЗаполнено(ЗначениеСвойства), ЗначениеСвойства, Неопределено);
			СтруктураПараметров.ИтогиРаздел9.Свойство("СтПродБезНДС20Или18", ЗначениеСвойства);
			СекцияВсего.Параметры.СуммаБезНДС20Или18 = ?(ЗначениеЗаполнено(ЗначениеСвойства), ЗначениеСвойства, Неопределено);
		Иначе
		...
		КонецЕсли;
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			СтруктураПараметров.ИтогиРаздел9.Свойство("СумНДСВсКПр22", ЗначениеСвойства);
			СекцияВсего.Параметры.НДС22 = ?(ЗначениеЗаполнено(ЗначениеСвойства), ЗначениеСвойства, Неопределено);
			СтруктураПараметров.ИтогиРаздел9.Свойство("СумНДСВсКПр20Или18", ЗначениеСвойства);
			СекцияВсего.Параметры.НДС20Или18 = ?(ЗначениеЗаполнено(ЗначениеСвойства), ЗначениеСвойства, Неопределено);
		Иначе
...
Процедура ЗаписатьВСегментДанныхСтрокуРаздела91(СтрокаРаздела91, СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП = 0 Тогда
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			ИтСтПродКПр22 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС22;
			ПараметрыРаздела.СекцияИтого.Параметры.СуммаБезНДС22 = ?(ЗначениеЗаполнено(ИтСтПродКПр22), ИтСтПродКПр22, Неопределено);
			ИтСтПродКПр20Или18 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС20Или18;
			ПараметрыРаздела.СекцияИтого.Параметры.СуммаБезНДС20Или18 = ?(ЗначениеЗаполнено(ИтСтПродКПр20Или18), ИтСтПродКПр20Или18, Неопределено);
		Иначе
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			СумНДСИтКПр22 = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр22;
			ПараметрыРаздела.СекцияИтого.Параметры.НДС22 = ?(ЗначениеЗаполнено(СумНДСИтКПр22), СумНДСИтКПр22, Неопределено);
			СумНДСИтКПр20Или18 = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр20Или18;
			ПараметрыРаздела.СекцияИтого.Параметры.НДС20Или18 = ?(ЗначениеЗаполнено(СумНДСИтКПр20Или18), СумНДСИтКПр20Или18, Неопределено);
		Иначе
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
		СведенияОЧастСчФ = "";
		Для Каждого НомерДатаЧастСчФ Из СтрокаРаздела91.СвЧастСчФ Цикл
			НомерЧастСчФ = Неопределено;
			НомерДатаЧастСчФ.Свойство("НомЧастСчФ", НомерЧастСчФ);
			ДатаЧастСчФ  = Неопределено;
			НомерДатаЧастСчФ.Свойство("ДатаЧастСчФ", ДатаЧастСчФ);
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
		ПараметрыСтроки.СуммаБезНДС22 = ?(ЗначениеЗаполнено(СтрокаРаздела91.СтоимПродСФ22), СтрокаРаздела91.СтоимПродСФ22, Неопределено);
		ПараметрыСтрокиПром.Вставить("СуммаБезНДС20Или18",
			?(ЗначениеЗаполнено(СтрокаРаздела91.СтоимПродСФ20Или18), СтрокаРаздела91.СтоимПродСФ20Или18, Неопределено));
	Иначе
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
		ПараметрыСтроки.НДС22 = ?(ЗначениеЗаполнено(СтрокаРаздела91.СумНДССФ22), СтрокаРаздела91.СумНДССФ22, Неопределено);
		ПараметрыСтрокиПром.Вставить("НДС20Или18",
			?(ЗначениеЗаполнено(СтрокаРаздела91.СумНДССФ20Или18), СтрокаРаздела91.СумНДССФ20Или18, Неопределено));
	Иначе
	...
	Если ПараметрыРаздела.ПоследняяСтрока = Истина Тогда
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			СтПродВсП1Р9_22 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС22 - ИтогиРаздел91.ИтСтПродКПр22 + ИтогиРаздел91.СтПродВсП1Р9_22;
			СекцияВсего.Параметры.СуммаБезНДС22 = ?(ЗначениеЗаполнено(СтПродВсП1Р9_22), СтПродВсП1Р9_22, Неопределено);
			СтПродВсП1Р9_20Или18 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС20Или18 - ИтогиРаздел91.ИтСтПродКПр20Или18 + ИтогиРаздел91.СтПродВсП1Р9_20Или18;
			СекцияВсего.Параметры.СуммаБезНДС20Или18 = ?(ЗначениеЗаполнено(СтПродВсП1Р9_20Или18), СтПродВсП1Р9_20Или18, Неопределено);
		Иначе
		...
		Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			СумНДСВсП1Р9_22 = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр22 - ИтогиРаздел91.СумНДСИтКПр22 + ИтогиРаздел91.СумНДСВсП1Р9_22;
			СекцияВсего.Параметры.НДС22 = ?(ЗначениеЗаполнено(СумНДСВсП1Р9_22), СумНДСВсП1Р9_22, Неопределено);
			СумНДСВсП1Р9_20Или18 = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр20Или18 - ИтогиРаздел91.СумНДСИтКПр20Или18 + ИтогиРаздел91.СумНДСВсП1Р9_20Или18;
			СекцияВсего.Параметры.НДС20Или18 = ?(ЗначениеЗаполнено(СумНДСВсП1Р9_20Или18), СумНДСВсП1Р9_20Или18, Неопределено);
		Иначе
...
Процедура ЗаписатьВСегментДанныхСтрокуРаздела10(СтрокаРаздела10, СтруктураПараметров)
	...
	Если ПараметрыРаздела.НомерПП = 0 Тогда
		...
		ИначеЕсли СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
			ИмяМакета = "ЖурналУчетаСчетовФактур26";
			ПараметрыРаздела.Вставить("Часть1СтрокаРНПТ");
		КонецЕсли;
		ПараметрыРаздела.Вставить("Макет", РеквизитФормыВЗначение("Объект").ПолучитьМакет(ИмяМакета));
...
Процедура КорректироватьИтоги81_91(СтруктураПараметров)
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
		СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_22 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС22 + СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_22 - СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр22;
		СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр22   = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС22;
		
		СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_20Или18 = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС20Или18
			+ СтруктураПараметров.ИтогиРаздел91.СтПродВсП1Р9_20Или18 - СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр20Или18;
		СтруктураПараметров.ИтогиРаздел91.ИтСтПродКПр20Или18   = СтруктураПараметров.ИтогиРаздел9.СтПродБезНДС20Или18;
	КонецЕсли;
	...
	Если СтруктураПараметров.ПравилаЗаполнения = "2026Кв1" Тогда
		СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_22 = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр22  + СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_22 - СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр22;
		СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр22   = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр22;
		
		СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_20Или18 = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр20Или18
			+ СтруктураПараметров.ИтогиРаздел91.СумНДСВсП1Р9_20Или18 - СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр20Или18;
		СтруктураПараметров.ИтогиРаздел91.СумНДСИтКПр20Или18   = СтруктураПараметров.ИтогиРаздел9.СумНДСВсКПр20Или18;
	КонецЕсли;
