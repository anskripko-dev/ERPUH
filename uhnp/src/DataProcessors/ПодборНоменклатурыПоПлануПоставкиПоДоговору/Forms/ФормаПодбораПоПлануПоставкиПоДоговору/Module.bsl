...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЭтотОбъект.Параметры.Свойство("Документ") Тогда
...
Процедура ОбновитьПодобраноВСпискеПланаПоставки()
	...
	Если Элементы.ПланПоставокПоДоговору.Видимость Тогда
...
Процедура ЗаполнитьДеревоЗначенийОтборов(Форма, ДеревоОтборов)
	...
	Запрос.Текст = Форма.СписокПланПоставокПоДоговору.ТекстЗапроса+"
		|	И ОбъектыПланирования.Объект =&Договор";
...
Функция ПолучитьОтсортированныймассивПериодов(МассивЗначений)
	...
	Для Каждого ДатаМассива Из МассивЗначений Цикл
		Если ЗначениеЗаполнено(ДатаМассива) Тогда 
			ПериодПотребности = СоответствиеПериодДата.Получить(ДатаМассива);
			Если Не ЗначениеЗаполнено(ПериодПотребности) Тогда
				ПериодПотребности = ОбщегоНазначенияУХ.ПолучитьПериодПоДате(ДатаМассива, ПериодичностьЗакупокГод);
			КонецЕсли;
			СоответствиеПериодДата.Вставить(ДатаМассива, ПериодПотребности); 
		КонецЕсли;  
	КонецЦикла; 
	...
	Для Каждого ДатаМассива Из МассивЗначений Цикл
		...
		Строка.Период = СоответствиеПериодДата.Получить(ДатаМассива);
		Строка.Дата = ДатаМассива;
	КонецЦикла;
...
Функция ВариантДействийПриВыбореЗначенияДереваОтборов(Форма)
	...
	ИначеЕсли ТекущийЭлемент = Форма.Элементы.ДеревоОтборовПредставление Тогда
		...
		ИначеЕсли Не ТекущиеДанные.ОтборДоступен И 
			Форма.Элементы.ДеревоОтборов.Видимость Тогда
