...
Функция ПрочитатьТекстКнигиXML(ИмяФайла, ИмяКниги, ПараметрыАдресации, КлючЗадания)
	...
	Область = Неопределено; // ОбластьЯчеекТабличногоДокумента
...
Функция ПересчитатьАдресациюКниг(ИмяСохраненногоФайла, ИмяВременногоКаталога, ПараметрыАдресации, КлючЗадания)
	...
	Иначе
		...
		СтрокаШаблона = Нстр("ru = 'Неизвестная версия сохраненной модели в файле ""%1""'");
		
		ТекстИсключения = "";	
		Если Не ПустаяСтрока(СтрокаШаблона) тогда				
			ТекстИсключения = СтрШаблон(СтрокаШаблона, ИмяСохраненногоФайла);
		КонецЕсли;
...
Функция НайтиТаблицуADO(ТипБД, ИмяФайла, ИмяЛиста)
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		СтрокаШаблона = Нстр("ru = 'Не найдена таблица ADO.
		|Тип БД: %1 (%2)
		|Имя книги: %3
		|Имя листа: %4
		|Имя таблицы ADO: %5'");
		...
		Если Не ПустаяСтрока(СтрокаШаблона) тогда				
			ТекстОшибки = СтрШаблон(СтрокаШаблона, ТипБД, ТипБД.УникальныйИдентификатор(), ИмяФайла, ИмяЛиста, ИмяТаблицыADO);
		КонецЕсли;
...
Функция СоздатьВидыОтчетов_(АдресСтруктурыКниг, МассивОтчетов, МассивПакетовИмпорта)
	...
	СтрокаШаблона = Нстр("ru = 'Импорт бланков из Microsoft Excel от %1'");
	
	ОбщееНаименованиеИмпорта = "";
	Если Не ПустаяСтрока(СтрокаШаблона) тогда								
		ОбщееНаименованиеИмпорта = СтрШаблон(СтрокаШаблона, Формат(ТекущаяДата(), "ДФ='yyyy.MM.dd HH:mm:ss'"));
	КонецЕсли;
	...
	Для Каждого ВидОтчета Из МассивОтчетов Цикл
		...
		Если ВидОтчетаСсылка = Неопределено Тогда
			...
			СтрокаШаблона = Нстр("ru = 'Правило проверки: %1'");
			
			Если Не ПустаяСтрока(СтрокаШаблона) тогда								
				ПравилоПроверкиОбъект.Наименование = СтрШаблон(СтрокаШаблона, ВидОтчета.Имя);
			Иначе
				ПравилоПроверкиОбъект.Наименование = ВидОтчета.Имя;
			КонецЕсли;
			...
			Если НЕ ЗначениеЗаполнено(ВидОтчета.Параметры["ВидОтчетаСуществующий"]) Тогда
				УправлениеОтчетамиУХ.ЗаписатьНастройкуПоУмолчаниюОбработкиШаблонов(ПравилоПроверкиОбъект.Владелец, "ПравилоПроверки", ПравилоПроверкиОбъект.Ссылка);
			КонецЕсли;
			
			ПравилоОбработкиОбъект = Справочники.ПравилаОбработки.СоздатьЭлемент();
			ПравилоОбработкиОбъект.Владелец = ВидОтчетаСсылка;
			
			СтрокаШаблона = Нстр("ru = 'Правило обработки: %1'");
			
			Если Не ПустаяСтрока(СтрокаШаблона) тогда								
				ПравилоОбработкиОбъект.Наименование = СтрШаблон(СтрокаШаблона, ВидОтчета.Имя);
			Иначе
				ПравилоОбработкиОбъект.Наименование = ВидОтчета.Имя;
			КонецЕсли;
			...
			Если НЕ ЗначениеЗаполнено(ВидОтчета.Параметры["ВидОтчетаСуществующий"]) Тогда
				УправлениеОтчетамиУХ.ЗаписатьНастройкуПоУмолчаниюОбработкиШаблонов(ПравилоОбработкиОбъект.Владелец, "ПравилоОбработки", ПравилоОбработкиОбъект.Ссылка);
			КонецЕсли;
	...
	Для Каждого ЛистГруппыРасрытия Из ГруппыРаскрытия Цикл
		...
		Если НовыйТипБД.ВерсияПлатформы = Перечисления.ПлатформыВнешнихИнформационныхБаз.ADO Тогда
			...
			Если ТаблицаADO = Неопределено Тогда
				ТекстИсключения = Нстр("ru = 'Не найдена таблица ADO: по данной таблице не могут быть настроены соответствия сопоставленных реквизитов.'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
		...
		Для Каждого ГруппаРаскрытия Из ЛистГруппыРасрытия.Значение Цикл
			...
			Иначе
				...
				Если ГруппаРаскрытияСсылка.Пустая() Тогда
					...
					СтрокаШаблона = Нстр("ru = 'Не найдена группа раскрытия <%1> у вида отчета <%2>'");
					
					ТекстИсключения = "";	
					Если Не ПустаяСтрока(СтрокаШаблона) тогда				
						ТекстИсключения = СтрШаблон(СтрокаШаблона, ГруппаРаскрытия.Имя, ВидОтчета.Имя);
					КонецЕсли;
			...
			Для Каждого ОбластьПоказатель Из ГруппаРаскрытия.Показатели Цикл
				...
				Если ВидОтчетаСуществует Тогда
					...
					Если ПоказательСсылка.Пустая() Тогда
						...
						СтрокаШаблона = Нстр("ru = 'Не найден показатель <%1> у вида отчета <%2>'");
						
						ТекстИсключения = "";	
						Если Не ПустаяСтрока(СтрокаШаблона) тогда				
							ТекстИсключения = СтрШаблон(СтрокаШаблона, ИмяПоказателя, ВидОтчета.Имя);
						КонецЕсли;
	...
	Для Каждого ЭлементБланкОтчетов Из БланкиОтчетов Цикл
		...
		Для Каждого ЭлементОбластьИменования Из ОбластиИменования Цикл
			...
			Исключение
				...
				СтрокаШаблона = Нстр("ru = 'Системная ошибка в %1,%2,%3,%4. Подробности в полном протоколе.'");
				
				ТекстИсключения = "";	
				Если Не ПустаяСтрока(СтрокаШаблона) тогда				
					ТекстИсключения = СтрШаблон(СтрокаШаблона, Адрес[0], Адрес[1], Адрес[2], ИмяОбласти);
				КонецЕсли;
				
				ПротоколируемыеСобытияУХ.ДобавитьЗаписьОшибка(ПРОТОКОЛИРУЕМОЕ_СОБЫТИЕ,,, ТекстИсключения, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				ВызватьИсключение;
			КонецПопытки;
		...
		Если НЕ ЗначениеЗаполнено(ВидОтчета.Параметры["ВидОтчетаСуществующий"]) Тогда
			УправлениеОтчетамиУХ.ЗаписатьНастройкуПоУмолчаниюОбработкиШаблонов(БланкОтчетаОбъект.Владелец, "БланкДляИмпорта", БланкОтчетаОбъект.Ссылка);
			УправлениеОтчетамиУХ.ЗаписатьНастройкуПоУмолчаниюОбработкиШаблонов(БланкОтчетаОбъект.Владелец, "БланкДляОтображения", БланкОтчетаОбъект.Ссылка);
		КонецЕсли;
	...
	Для Каждого Строка Из Строки Цикл
		...
		ИначеЕсли Тип = "Функция" Тогда
			...
			Иначе
				...
				СтрокаШаблона = Нстр("ru = 'Не возможно преобразовать функцию: %1'");
				
				ТекстОшибки = "";
				Если Не ПустаяСтрока(СтрокаШаблона) тогда								
					ТекстОшибки = СтрШаблон(СтрокаШаблона, Имя);
				КонецЕсли;
				
				ПротоколируемыеСобытияУХ.ДобавитьЗаписьПредупреждение(ПРОТОКОЛИРУЕМОЕ_СОБЫТИЕ,,, ТекстОшибки);
