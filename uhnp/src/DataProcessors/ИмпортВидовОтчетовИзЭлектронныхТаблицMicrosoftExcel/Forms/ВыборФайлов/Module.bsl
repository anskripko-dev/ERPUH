...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НажатаКнопкаЗагрузить Тогда
		...
		Если ЕстьОшибки Тогда
			Отказ = Истина;
			ПоказатьПредупреждение(, Нстр("ru = 'Во время проверки файлов произошла ошибка.
	...
	Если ЕстьДвойные Тогда
		Для Каждого Книга Из ИменаКниг Цикл
			Если Книга.Значение > 1 Тогда
				...
				СтрокаШаблона = Нстр("ru = 'Книга ""%1"" встречается %2'");
				НаименованиеЧислительного = Нстр("ru = 'раз,раза,раз,м,,,,,0'");
				
				Если Не ПустаяСтрока(СтрокаШаблона) И Не ПустаяСтрока(НаименованиеЧислительного) тогда	
					Сообщить(СтрШаблон(СтрокаШаблона, Книга.Ключ, НРег(ЧислоПрописью(Книга.Значение,, НаименованиеЧислительного))));
				КонецЕсли;
...
Процедура ВыбратьФайл(ПолноеИмяФайла = "", ТекущиеДанные = Неопределено)
	...
	Фильтр = ?(Excel = Неопределено, "", Нстр("ru = 'Все импортируемые файлы'") + " (*.xls, *.xlsx, *.hm)|*.xls; *.xlsx; *.hm|" + Нстр("ru = 'Листы Microsoft Excel'") + " (*.xls, *.xlsx)|*.xls; *.xlsx|") 
	+ Нстр("ru = 'Файлы Управления Холдингом'") +" (*.hm)|*.hm|" + Нстр("ru = 'Все файлы'") + " (*.*)|*.*";
	
	ДиалогВыборФайла = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
	ДиалогВыборФайла.Заголовок = Нстр("ru = 'Выбор файлов'");
...
Процедура Подключаемый_ДобавитьФайлКаталог_ФайлСуществует(Существует, ДополнительныеПараметры) Экспорт
    ...
    Иначе
		...
		СтрокаШаблона = Нстр("ru = 'Файл(каталог) <%1> не существует.'");
		
		Если Не ПустаяСтрока(СтрокаШаблона) тогда	
			Сообщить(СтрШаблон(СтрокаШаблона, ДополнительныеПараметры.ФайлКаталог.ПолноеИмя));
		КонецЕсли;
...
Процедура ДобавитьФайл(ИдентификаторСтроки, Файл)
	...
	Если УжеДобавлен Тогда
		...
		СтрокаШаблона = Нстр("ru = 'Файл %1 уже содержится в списке.'");
		
		Если Не ПустаяСтрока(СтрокаШаблона) тогда	
			Сообщить(СтрШаблон(СтрокаШаблона, Файл.Имя));
		КонецЕсли;
    ...
    Иначе
		...
		Иначе
            ...
            Исключение
				...
				СтрокаШаблона = Нстр("ru = 'Ошибка при чтении файла %1. %2'");
				
				Если Не ПустаяСтрока(СтрокаШаблона) тогда	
					Сообщить(СтрШаблон(СтрокаШаблона, Файл.ПолноеИмя, ОписаниеОшибки()));
				КонецЕсли;
...
Функция КонвертироватьВXML(КлючЗадания)
		...
		Для Каждого СтрокаФайл Из Файлы.ПолучитьЭлементы() Цикл
			...
			Если СтрокаФайл.Тип = 0 Тогда
				...
				Исключение
					...
					СтрокаШаблона = Нстр("ru = 'Не удалось открыть файл ""%1""
					|%2'");
					
					ОписаниеОшибки = "";
					Если Не ПустаяСтрока(СтрокаШаблона) тогда			
						ОписаниеОшибки = СтрШаблон(СтрокаШаблона, СтрокаФайл.Имя, ОписаниеОшибки());
					КонецЕсли;
					...
					Если СтрокаЛист.ЛистЗащищен Тогда
						...
						Если НЕ ПустаяСтрока(Пароль) Тогда
							...
							Исключение
								...
								СтрокаШаблона = Нстр("ru = 'В файле ""%1"" не найден лист ""%2""
								|%3'");
								
								ОписаниеОшибки = "";
								Если Не ПустаяСтрока(СтрокаШаблона) тогда			
									ОписаниеОшибки = СтрШаблон(СтрокаШаблона, СтрокаФайл.Имя, СтрокаЛист.Имя, ОписаниеОшибки());
								КонецЕсли;
							...
							Исключение
								...
								СтрокаШаблона = Нстр("ru = 'Для файла ""%1"" указан неверный пароль защиты листа ""%2""
								|%3'");
	...
	Если БылиОшибки Тогда
		...
		СтрокаШаблона = Нстр("ru = 'При конвертации файлов Microsoft Excel возникла ошибка: %1'");
		
		Если Не ПустаяСтрока(СтрокаШаблона) тогда	
			Сообщить(СтрШаблон(СтрокаШаблона, ОписаниеОшибки));
		КонецЕсли;
