	...
	Если Не Результат
		...
		ШаблонСообщения = 
			НСтр("ru = 'Количество товара ""%1"" для заказа у ""%2"" должно быть кратно %3.';
				|en = 'Количество товара ""%1"" для заказа у ""%2"" должно быть кратно %3.'");
...
Процедура ЗагрузитьРезультатыПоиска(АдресРезультата)
	...
	Для каждого ЭлементСписка Из Контрагенты Цикл
		...
		НовыйЭлемент.РастягиватьПоГоризонтали = Ложь;
		НовыйЭлемент.Ширина = 5;
		НовыйЭлемент.Заголовок = НСтр("ru = 'Кратность упаковки';
										|en = 'Кратность упаковки'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПослеСозданияКорзиныИДобавленияТовара(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не РезультатыДобавленияПредложений.ЕстьОшибкиДобавленияПредложений Тогда
		Закрыть();
	КонецЕсли;
...
Функция РезультатыДобавленияПредложений(Знач АдресРезультата)
	...
	Если ЭтоАдресВременногоХранилища(АдресРезультата) Тогда
		...
		Результат.ЕстьДобавленныеПредложения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			РезультатыОперации,
			"ЕстьДобавленныеПредложения",
			Ложь);
		
		Результат.ЕстьОшибкиДобавленияПредложений = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			РезультатыОперации,
			"ЕстьОшибкиДобавленияПредложений",
			Ложь);
