...
Процедура КоличествоУпаковокПриИзменении(Элемент)
	...
	ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(Элементы.Партии.ТекущиеДанные, СтруктураДействий, КэшированныеЗначения);
...
Процедура ПростойРежимКоличествоУпаковокРазделитьПриИзмененииНаСервере()
	...
	Если ВыпускИзделий.Количество() > 0 Тогда
		...
		Если Не ПоследняяПартия.АвтовыборНоменклатурыЭтоРабота Тогда
			...
			СтруктураДействий = Новый Структура;
			СтруктураДействий.Вставить("ПересчитатьКоличествоЕдиниц", ПроизводствоКлиентСервер.ПараметрыПересчетаКоличестваЕдиниц());
			ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ПоследняяПартия, СтруктураДействий, Неопределено);
		КонецЕсли;
	...
	Иначе
		...
		Если Не ПерваяПартия.АвтовыборНоменклатурыЭтоРабота Тогда
			...
			СтруктураДействий = Новый Структура;
			СтруктураДействий.Вставить("ПересчитатьКоличествоЕдиниц", ПроизводствоКлиентСервер.ПараметрыПересчетаКоличестваЕдиниц());
			ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ПерваяПартия, СтруктураДействий, Неопределено);
		КонецЕсли;
		...
		Если Не ВтораяПартия.АвтовыборНоменклатурыЭтоРабота Тогда
			...
			СтруктураДействий = Новый Структура;
			СтруктураДействий.Вставить("ПересчитатьКоличествоЕдиниц", ПроизводствоКлиентСервер.ПараметрыПересчетаКоличестваЕдиниц());
			ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ВтораяПартия, СтруктураДействий, Неопределено);
		КонецЕсли;
...
Процедура ПродолжитьНоменклатураПриИзмененииНаСервере()
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтрокаКопия, СтруктураДействий, Неопределено);
...
Процедура УстановитьУсловноеОформление()
	...
	ПараметрыУсловногоОформления.ИмяПоляЕдиницаИзмерения = "ПартииНоменклатураЕдиницаИзмерения";
	ПараметрыУсловногоОформления.ПутьКПолюУпаковка = "Объект.Партии.Упаковка";
	НоменклатураСервер.УстановитьУсловноеОформлениеЕдиницИзмерения(ЭтотОбъект, ПараметрыУсловногоОформления);
	
	//
	
	ПараметрыУсловногоОформления = НоменклатураСервер.НовыеПараметрыУсловногоОформленияЕдиницИзмерения();
	ПараметрыУсловногоОформления.ИмяПоляЕдиницаИзмерения = "ПартииНоменклатураПартииЕдиницаИзмерения";
	ПараметрыУсловногоОформления.ПутьКПолюУпаковка = "Объект.Партии.УпаковкаПартии";
	НоменклатураСервер.УстановитьУсловноеОформлениеЕдиницИзмерения(ЭтотОбъект, ПараметрыУсловногоОформления);
	
	//
	
	ПараметрыУсловногоОформления = НоменклатураСервер.НовыеПараметрыУсловногоОформленияЕдиницИзмерения();
	ПараметрыУсловногоОформления.ИмяПоляЕдиницаИзмерения = "РезультатВыполненияНоменклатураПартииЕдиницаИзмерения";
	ПараметрыУсловногоОформления.ПутьКПолюУпаковка = "РезультатВыполнения.Упаковка";
	НоменклатураСервер.УстановитьУсловноеОформлениеЕдиницИзмерения(ЭтотОбъект, ПараметрыУсловногоОформления);
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("КоличествоПартийРазделить")
		...
		Если Форма.ДелениеОтменаСНачалаПартии Тогда
			...
			Элементы.ДекорацияКоличествоПартий.Заголовок = 
				ОбщегоНазначенияУТКлиентСервер.СклонениеСлова(
					...
					НСтр("ru = 'партию, партии, партий, ж';
						|en = 'lot, lots, lots, f'"));
	...
	Если СтруктураРеквизитов.Свойство("Этап")
		...
		Элементы.ДействиеОтменить.СписокВыбора[0].Представление = ?(Форма.ДелениеОтменаСНачалаПартии,
...
Процедура СоздатьНоменклатуруАвтовыбора()
	...
	ПартииСАвтовыбором = Объект.Партии.НайтиСтроки(СтруктураПоиска);
	
	СтрокиКОбработке = Новый ТаблицаЗначений;
	СтрокиКОбработке.Колонки.Добавить("Строка");
	СтрокиКОбработке.Колонки.Добавить("Номенклатура");
	СтрокиКОбработке.Колонки.Добавить("Характеристика");
	СтрокиКОбработке.Колонки.Добавить("Упаковка");
	
	Для каждого Строка Из ПартииСАвтовыбором Цикл
		...
		Если ЗначениеЗаполнено(Строка.Характеристика)
			...
			НоваяСтрока = СтрокиКОбработке.Добавить();
			НоваяСтрока.Строка = Строка;
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка, "Номенклатура, Характеристика, Упаковка");
		КонецЕсли;
	...
	Если СтрокиКОбработке.Количество() > 0 Тогда
		...
		// Подбор характеристик и упаковок
		СтруктураДействий = Новый Структура;
		...
		СтруктураДействий.Вставить("ЗаполнитьУпаковкуНекачественногоТовара");
		
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			СтрокиКОбработке.ВыгрузитьКолонку("Строка"),
			СтруктураДействий,
			Объект.Партии,
			КэшированныеЗначения);
		
		// Создание характеристик и упаковок
		СозданныеОбъекты = Новый Соответствие;
		Для каждого ЗначенияЗаполнения Из СтрокиКОбработке Цикл
			СтрокаПартии = ЗначенияЗаполнения.Строка;
			Если ЗначениеЗаполнено(ЗначенияЗаполнения.Характеристика)
				И НЕ ЗначениеЗаполнено(СтрокаПартии.Характеристика) Тогда
				Если СозданныеОбъекты[ЗначенияЗаполнения.Характеристика] = Неопределено Тогда
					СтрокаПартии.Характеристика = Справочники.ХарактеристикиНоменклатуры.ИдентичнаяХарактеристика(
						СтрокаПартии.Номенклатура, ЗначенияЗаполнения.Характеристика);
					СозданныеОбъекты.Вставить(ЗначенияЗаполнения.Характеристика, СтрокаПартии.Характеристика);
				Иначе
					СтрокаПартии.Характеристика = СозданныеОбъекты[ЗначенияЗаполнения.Характеристика];
				КонецЕсли;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ЗначенияЗаполнения.Упаковка)
				И НЕ ЗначениеЗаполнено(СтрокаПартии.Упаковка) Тогда
				Если СозданныеОбъекты[ЗначенияЗаполнения.Упаковка] = Неопределено Тогда
					СтрокаПартии.Упаковка = Справочники.УпаковкиЕдиницыИзмерения.ИдентичнаяУпаковка(
						ЗначенияЗаполнения.Номенклатура, СтрокаПартии.Номенклатура, ЗначенияЗаполнения.Упаковка);
					СозданныеОбъекты.Вставить(ЗначенияЗаполнения.Упаковка, СтрокаПартии.Упаковка);
				Иначе
					СтрокаПартии.Упаковка = СозданныеОбъекты[ЗначенияЗаполнения.Упаковка];
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
Функция ДанныеЭтапа(Объект, ИзделиеЕдиницаИзмерения)
	...
	Если Этап.Пустая() Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Результат.Вставить("ДелениеОтменаСНачалаПартии", Истина);
	...
	Иначе
		...
		ДелениеОтменаСНачалаЭтапа = Не (
			ВыборкаЭтап.ЭтапНачат
			Или (Не Операция.Пустая() И Не ЭтоПерваяОперация)
		);
		СНачалаПартии = ВыборкаЭтап.ЭтоПервыйЭтап И СНачалаЭтапа;
		ДелениеОтменаСНачалаПартии = ВыборкаЭтап.ЭтоПервыйЭтап И ДелениеОтменаСНачалаЭтапа;
		...
		Запрос.Текст = 
			"ВЫБРАТЬ
			|	СУММА(&КоличествоУпаковок * Очередь.Брак / Очередь.Требуется) КАК КоличествоУпаковокБрак
			|ИЗ
		...
		Запрос.УстановитьПараметр("КоличествоУпаковок", ВыборкаЭтап.КоличествоУпаковок);
		ВыборкаБрак = Запрос.Выполнить().Выбрать();
		Если ВыборкаБрак.Следующий() Тогда
			КоличествоУпаковокБрак = ВыборкаБрак.КоличествоУпаковокБрак;
		...
		Результат.Вставить("ДелениеОтменаСНачалаПартии", ДелениеОтменаСНачалаПартии);
