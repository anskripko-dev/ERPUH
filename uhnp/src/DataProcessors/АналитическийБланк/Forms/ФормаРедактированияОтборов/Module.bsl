...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ГрупповойРежим Тогда
		ЭтаФорма.Заголовок = Нстр("ru = 'Групповое редактирование отборов'");
	Иначе
		ЭтаФорма.Заголовок = Нстр("ru = 'Настройка отбора значений показателей'");
	КонецЕсли;
	...
	Для Каждого Стр Из  ТзРасшифровкаОтборовЭталон Цикл	
		...
		Если  ПериодЗаблокирован Тогда
			Если  Стр.ПолеКод = "ПериодС" Тогда
				нСтр.Отбор = Перечисления.ВидыСравненияПоказателейОтчетов.ПериодНачалаСоСдвигом;
			...
			Если  Стр.ПолеКод = "ПериодПо" Тогда
				нСтр.Отбор = Перечисления.ВидыСравненияПоказателейОтчетов.ПериодОкончанияСоСдвигом;
