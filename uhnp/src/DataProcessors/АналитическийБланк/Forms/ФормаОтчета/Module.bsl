...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Регламент                       = Параметры.Регламент;
	
	Если ЗначениеЗаполнено(Регламент) Тогда
		 Заблокирован = Истина;	
	КонецЕсли;	
	
	Если ЗначениеЗаполнено(Проект) И  Не ЗначениеЗаполнено(Бланк)  Тогда
		...
		ОтображениеСостояния.Текст                          = НСтр("ru = 'Не выбран основной бланк отображения модели текущего проекта'");
	ИначеЕсли НЕ  ЗначениеЗаполнено(Бланк) Тогда	
		ЭтаФорма.Доступность = Ложь;
		ОтображениеСостояния = ЭтаФорма.Элементы.ПолеТабличногоДокументаМакет.ОтображениеСостояния;
		ОтображениеСостояния.Видимость                      = Истина;
		ОтображениеСостояния.ДополнительныйРежимОтображения = ДополнительныйРежимОтображения.Неактуальность;
		ОтображениеСостояния.Картинка                       = Новый Картинка;
		ОтображениеСостояния.Текст                          = НСтр("ru = 'Не выбран бланк'");		
	КонецЕсли;	
	...
	Если ЗначениеЗаполнено(Проект) Тогда 
		ИспользоватьБыстрыйОтборПоПроекту 						= Истина;
	КонецЕсли;	
	...
	Элементы.ИспользоватьБыстрыйОтборПоПроекту.Видимость 		= Истина;
	Элементы.Элемент_Фильтр_Проект.Видимость 					=  ИспользоватьБыстрыйОтборПоПроекту;
	
	Элементы.ОсновнаяПанель.ОтображениеСтраниц 					= ОтображениеСтраницФормы.ЗакладкиСверху;
    РасположениеЗакладок                                        = Перечисления.РасположениеЗакладокБланка.Сверху;
	...
	Если Параметры.Свойство("ЭкземплярОтчета") Тогда   //Открываем форму в режиме просмотра версий
		...
		УправлениеОтчетамиУХ.ЗаполнитьПараметрыОкругления(тЕдиница, ТекущийДелитель);
	...
	АдресЛогаИзмененныхПоказателей 		 = ПоместитьВоВременноеХранилище(УправлениеРасчетомПоказателей.ОписаниеЛогаИзмененныхПоказателей(),ЭтаФорма.УникальныйИдентификатор);
    АдресЛогаИзмененныхПоказателейREDO 	 = ПоместитьВоВременноеХранилище(УправлениеРасчетомПоказателей.ОписаниеЛогаИзмененныхПоказателей(),ЭтаФорма.УникальныйИдентификатор);
...
Процедура ПриСохраненииДанныхВНастройкахНаСервере(Настройки)
	...
	Если Заблокирован Тогда
		УправлениеРасчетомПоказателей.ОчиститьЛогИзмененныхПоказателей(ЭтаФорма.УникальныйИдентификатор);
	КонецЕсли;	
...
Процедура ПрочитатьНастройкиВарианта()
	...
	Если ЗначениеСохраняемойНастройки = Неопределено Тогда
		Возврат;
	КонецЕсли;			
	Если  ЗначениеСохраняемойНастройки.Свойство("ПериодОтчета") И Не ЗначениеЗаполнено(ПериодОтчета) Тогда		
		ПериодОтчета = ЗначениеСохраняемойНастройки.ПериодОтчета;
	КонецЕсли;
	Если  ЗначениеСохраняемойНастройки.Свойство("ПериодОкончания") И Не ЗначениеЗаполнено(ПериодОкончания) Тогда		
		ПериодОкончания = ЗначениеСохраняемойНастройки.ПериодОкончания;
	КонецЕсли;
	Если  ЗначениеСохраняемойНастройки.Свойство("Валюта") И Не ЗначениеЗаполнено(Валюта) Тогда		
		Валюта = ЗначениеСохраняемойНастройки.Валюта;
	КонецЕсли;
	Если  ЗначениеСохраняемойНастройки.Свойство("Сценарий") И Не ЗначениеЗаполнено(Сценарий) Тогда		
		Сценарий = ЗначениеСохраняемойНастройки.Сценарий;
	КонецЕсли;
    Если  ЗначениеСохраняемойНастройки.Свойство("Организация") И Не ЗначениеЗаполнено(Организация) Тогда		
		Организация = ЗначениеСохраняемойНастройки.Организация;
	КонецЕсли;
	Если  ЗначениеСохраняемойНастройки.Свойство("Периодичность") И Не ЗначениеЗаполнено(Периодичность) Тогда		
		Периодичность = ЗначениеСохраняемойНастройки.Периодичность;
	КонецЕсли;
    Если  ЗначениеСохраняемойНастройки.Свойство("Проект") И Не ЗначениеЗаполнено(Проект) Тогда		
	...
	Элементы.Элемент_Фильтр_Проект.Видимость = Бланк.Владелец.РазделениеПоПроектам;
	
	УправлениеОтчетамиУХ.ЗаполнитьПараметрыОтображения(Делитель, УровеньТочности, Бланк);
	УправлениеОтчетамиУХ.ЗаполнитьПараметрыОкругления(Делитель, ТекущийДелитель);
...
&НаКлиенте
Процедура Элемент_Фильтр_ПроектПриИзменении(Элемент)
...
Процедура ИспользоватьБыстрыйОтборПоОрганизацииПриИзменении(Элемент)
	...
	Если ИспользоватьБыстрыйОтборПоОрганизации Тогда
		Если ПроверитьПолеВДопОтборах("Организация") Тогда
			Оповещение = Новый ОписаниеОповещения("ВопросОчиститьОтборыОрганизацияЗавершение",ЭтаФорма);
...
&НаКлиенте
Процедура ИспользоватьБыстрыйОтборПоПроектамПриИзменении(Элемент)
	
	//Предупредим пользователя, что если установлены отборы по организациям в доп. панели, то они потеряются 
	Если ИспользоватьБыстрыйОтборПоПроекту Тогда
		Если ПроверитьПолеВДопОтборах("Проект") Тогда
			Оповещение = Новый ОписаниеОповещения("ВопросОчиститьОтборыПроектЗавершение",ЭтаФорма);
			ПоказатьВопрос(Оповещение, Нстр("ru = 'Установленный отбор по проекту будет очищен. Продолжить?'"), РежимДиалогаВопрос.ДаНет);		
		Иначе	
			Элементы.Элемент_Фильтр_Проект.Видимость =  ИспользоватьБыстрыйОтборПоПроекту;
			ОбновитьИзмеренияИОтборы();
			АналитическийБланкУХКлиент.УстановитьСостояниеНеИспользовать(ЭтаФорма,НСтр("ru = 'Таблица не актуальна. Нажмите ""Сформировать (F5)"" для обновления данных.'"));	 
...
&НаКлиенте
Процедура ВопросОчиститьОтборыОрганизацияЗавершение(Результат, ДополнительныеПараметры) Экспорт      
...
&НаКлиенте
Процедура ВопросОчиститьОтборыПроектЗавершение(Результат, ДополнительныеПараметры) Экспорт      
	
	Если Результат = Неопределено  Или Результат = КодВозвратаДиалога.Нет  Тогда
		ИспользоватьБыстрыйОтборПоПроекту = Ложь;
		Возврат;
	КонецЕсли;	
...
&НаСервере
Функция ПодготовитьПараметрыВызоваФормыРаскрытий(СтруктураРасшифровки,СтруктураНезаполненныхАналитик,СтруктураОтбираемыхАналитик)
	
	Возврат Обработки.АналитическийБланк.ПодготовитьПараметрыВызоваФормыРаскрытий(ЭтаФорма,СтруктураРасшифровки,СтруктураНезаполненныхАналитик,СтруктураОтбираемыхАналитик);
...
Функция РасшифроватьОперандНаСервере(Расшифровка,Текст,ОбластьИмя)
	...
	СтруктураРасшифровки = 	Обработки.АналитическийБланк.ПолучитьСтруктуруРасшифровки(ЭтаФорма,СтруктураКлючевыхРеквизитов,ОбластьИмя,Расшифровка,СтруктураНезаполненныхАналитик,СтруктураОтбираемыхАналитик);
	...
	ИначеЕсли  ВозможнаРасшифровкаПоФормулеРасчета(СтруктураРасшифровки)  Тогда	//Достаточно для расшифровки ключевого среза
		...
		Если НЕ СтруктураПараметровДляВызоваРасшифровки = Неопределено Тогда
			...
			СтруктураПараметровДляВызоваРасшифровки.ОбъектРасчета.Вставить("ВалютаОтображения", Валюта);
...
Процедура ОбновитьИзмеренияИОтборы()
	...
	Для Каждого СтрДоступноеПоле Из ЭтаФорма["ТаблицаДоступныеПоля"] Цикл
		...
		Если ИспользоватьБыстрыйОтборПоПроекту 
			И СтрДоступноеПоле.ТипСубконто = Перечисления.ПостоянныеПоляСводнойТаблицы.Проект Тогда
			Продолжить;
		КонецЕсли;	
...
Процедура ПолеТабличногоДокументаМакетВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли  ТипЗнч(Область.Расшифровка) = Тип("Структура") И Область.Расшифровка.Свойство("Ссылка") Тогда			
		...
		Если НЕ СтруктураОтвета.СтруктураРасшифровки = Неопределено 
			И СтруктураОтвета.СтруктураНезаполненныхАналитик.Количество()=0 Тогда
...
Процедура ОткрытьФормуРаскрытияПоказателей(тОбласть = Неопределено)
	...
	Иначе	
		...
		ПараметрыФормыРаскрытия = ПодготовитьПараметрыВызоваФормыРаскрытий(СтруктураРасшифровки,СтруктураНезаполненныхАналитик,СтруктураОтбираемыхАналитик);	
...
Процедура ЗаполнитьПериоды(ОтборДатаНачалаПериода,ОтборДатаОкончанияПериода)
	...
	Иначе // Периодичность.День
		Запрос.УстановитьПараметр("ДатаНачала",ОтборДатаНачалаПериода);
		Запрос.УстановитьПараметр("ДатаОкончания", НачалоДня(ОтборДатаОкончанияПериода));
	КонецЕсли;
...
Процедура ВывестиБланки(МассивСтраниц,ДанныеДиаграмм = Ложь)
	...
	КлючевыеПараметры.Вставить("ИспользоватьБыстрыйОтборПоПроекту",ИспользоватьБыстрыйОтборПоПроекту);
	...
	КлючевыеПараметры.Вставить("МакетСтрок",МакетСтрок);
...
Процедура РасположениеЗакладокПриИзмененииСервер()
	...
	Если РасположениеЗакладок = Перечисления.РасположениеЗакладокБланка.Сверху Тогда
		Элементы.ОсновнаяПанель.ОтображениеСтраниц = ОтображениеСтраницФормы.ЗакладкиСверху;
	ИначеЕсли РасположениеЗакладок = Перечисления.РасположениеЗакладокБланка.Снизу Тогда
		Элементы.ОсновнаяПанель.ОтображениеСтраниц = ОтображениеСтраницФормы.ЗакладкиСнизу;
	ИначеЕсли РасположениеЗакладок = Перечисления.РасположениеЗакладокБланка.Слева Тогда
		Элементы.ОсновнаяПанель.ОтображениеСтраниц = ОтображениеСтраницФормы.ЗакладкиСлеваГоризонтально;
	ИначеЕсли РасположениеЗакладок = Перечисления.РасположениеЗакладокБланка.Справа Тогда	
		 Элементы.ОсновнаяПанель.ОтображениеСтраниц = ОтображениеСтраницФормы.ЗакладкиСправаГоризонтально;
	ИначеЕсли РасположениеЗакладок = Перечисления.РасположениеЗакладокБланка.НеВыводить Тогда	
		 Элементы.ОсновнаяПанель.ОтображениеСтраниц = ОтображениеСтраницФормы.Нет;	 
	КонецЕсли;	
...
Функция ПолучитьСтруктуруРасшифровки(ОбластьИмя,Расшифровка,СтруктураНезаполненныхАналитик,СтруктураОтбираемыхАналитик)
	...
	Возврат АналитическийБланкУХСервер.ПолучитьСтруктуруРасшифровки(тМакет,СтруктураКлючевыхРеквизитов,тОбъектАБ,ОбластьИмя,Расшифровка,СтруктураНезаполненныхАналитик,СтруктураОтбираемыхАналитик,ТзСвойстваПоказателей);
...
&НаКлиенте
Процедура СкопироватьИзИсточника(Команда)
