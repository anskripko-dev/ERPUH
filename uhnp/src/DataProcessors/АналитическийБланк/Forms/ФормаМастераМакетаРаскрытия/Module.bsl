...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяОбластиРодителя = Параметры.ИмяОбластиРодителя;
...
Процедура ДеревоСтруктурыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	СтруктураПараметров.Вставить("ИерархияАналитикСтрока"	,Элемент.ТекущиеДанные.ИерархияАналитикСтрока);
...
Процедура ФормаРедактированияСоставаГруппировкиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекЭлДерева.ИерархияАналитикСтрока				 	 = Результат.НастройкиИерархии;
...
Функция ПодготовитьСтруктуруПараметров() 
	...
	Для Каждого Обл Из ТабДокОбразец.Области Цикл	
		...
		Если Обл.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник 
			И ТипЗнч(Обл.Расшифровка) = Тип("Структура")
			И Обл.Расшифровка.Свойство("Ссылка") Тогда 
	...
	СтруктураОтвета.Вставить("СтруктураИерархии",		ПоместитьВоВременноеХранилище(СтруктураИерархии,Новый УникальныйИдентификатор()));
	СтруктураОтвета.Вставить("ИмяОбластиРодителя",		ИмяОбластиРодителя);
