...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнвестиционнаяПрограмма 		= Параметры.ИнвестиционнаяПрограмма;
	РазделИнвестиционнойПрограммы   = Параметры.РазделИнвестиционнойПрограммы;
	ВладелецОбъектовВыбора          = РазделИнвестиционнойПрограммы;
	...
	//Проверяем, что проект можно оценивать 
	Если ЗначениеЗаполнено(Проект) И Проект.НеОцениваетсяПоКритериям Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтрШаблон(Нстр("ru = 'Для проекта  ""%1""  отключена возможность использования экспертных оценок'"), Проект);
		Сообщение.Сообщить();	
		Отказ = Истина;
	КонецЕсли;	
...
Процедура ЗначенияКритериевПередНачаломИзменения(Элемент, Отказ)
	...
	Если ТекДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Если Элемент.ТекущийЭлемент.Имя = "ЗначенияКритериевПояснение" Тогда
		Структура = Новый Структура;
		Структура.Вставить("Критерий",ТекДанные.Критерий);
		ОткрытьФорму("Справочник.КритерииВыбора.Форма.ФормаКритерия",Структура);
	    Возврат;
	КонецЕсли;	
...
Процедура ЗначенияРассчетныхКритериевПередНачаломИзменения(Элемент, Отказ)
	Отказ = Истина;     
	ТекДанные = Элементы.ЗначенияРассчетныхКритериев.ТекущиеДанные;
	Если ТекДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	                    
	Если Элемент.ТекущийЭлемент.Имя = "ЗначенияРассчетныхКритериевПояснение" Тогда
		Структура = Новый Структура;
		Структура.Вставить("Критерий",ТекДанные.Критерий);
		ОткрытьФорму("Справочник.КритерииВыбора.Форма.ФормаКритерия",Структура);
	    Возврат;
...
Процедура РассчитатьЗначенияПоказателей(Команда)
	...
	Элементы.ЗначенияРассчетныхКритериев.Обновить();	
	Элементы.ДекорацияРассчитайтеЗначения.Видимость = Ложь;
...
Процедура Отчет(Команда)
	...
	ПараметрыФормы.Вставить("ПользовательскиеНастройки",
		ОбщегоНазначенияУХВызовСервера.ЗаполнитьПользовательскиеНастройкиОтчета(
...
Процедура УстановитьУсловноеОформлениеФормы()
	...
	Элементы.ЭтапОценки.Видимость = Истина;
	Элементы.Ответственный.Заголовок = "Эксперт: " + СокрЛП(Сотрудник);
	Элементы.ФункциональноеНаправлениеСотрудника.Видимость = Истина;	
	...
	ИначеЕсли ТипЗнч(ВладелецОбъектовВыбора) = Тип("СправочникСсылка.Лоты") Тогда
		...
		ЭтаФорма.Заголовок = НСтр("ru = 'Оценка поставщиков'");
	ИначеЕсли ТипЗнч(ВладелецОбъектовВыбора) = Тип("ДокументСсылка.Мероприятие") Тогда
		Элементы.ОбъектыОценкиКонтрагент.Заголовок = НСтр("ru = 'Поставщик'");
		Контекст =  ОбщегоНазначенияУХ.ЗначениеРеквизитаОбъекта(ВладелецОбъектовВыбора,"Контекст");
		ФункциональноеНаправления = ОбщегоНазначенияУХ.ЗначениеРеквизитаОбъекта(Контекст,"ФункциональноеНаправления");
		ЭтаФорма.АвтоЗаголовок = Ложь;
		ЗаголовокФормы = НСтр("ru = 'Оценка поставщиков по функциональному направлению: &ФункциональноеНаправления'");
		ЗаголовокФормы = СтрЗаменить(ЗаголовокФормы,"&ФункциональноеНаправления",Строка(ФункциональноеНаправления));
		ЭтаФорма.Заголовок = ЗаголовокФормы;
		Элементы.ЭтапОценки.Видимость = Ложь;
		Элементы.Ответственный.Заголовок = Элементы.Ответственный.Заголовок + " по функциональному направлению " + СокрЛП(ФункциональноеНаправления);
		Элементы.ФункциональноеНаправлениеСотрудника.Видимость = Ложь;	
	Иначе
		ЭтаФорма.Заголовок = НСтр("ru = 'Оценка альтернатив'");
	КонецЕсли;
	
	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("НастройкиПроцессаВыбора",НастройкаПроцессаВыбора);
	Запрос.УстановитьПараметр("Мероприятие",ВладелецОбъектовВыбора);
	Запрос.Текст = "ВЫБРАТЬ
	               |	НастройкиПроцессаВыбораКритерииОценки.Ссылка.Ссылка КАК Ссылка,
	               ...
	               |	НастройкиПроцессаВыбораКритерииОценки.РассчитатьЗначение КАК РассчитатьЗначение
	               |ИЗ
	               ...
	               |		ПО НастройкиПроцессаВыбораКритерииОценки.КритерийОценки = ЗначенияКритериевОценки.Критерий
	               |			И (НастройкиПроцессаВыбораКритерииОценки.РассчитатьЗначение = ИСТИНА)
	               |			И (ЗначенияКритериевОценки.Владелец = &Мероприятие)
	               |ГДЕ
	               |	НастройкиПроцессаВыбораКритерииОценки.Ссылка = &НастройкиПроцессаВыбора
	               |	И ЗначенияКритериевОценки.Владелец ЕСТЬ NULL
				   |	И НастройкиПроцессаВыбораКритерииОценки.РассчитатьЗначение = ИСТИНА";
	Выборка = Запрос.Выполнить();
	Если НЕ Выборка.Пустой() Тогда
		Элементы.ДекорацияРассчитайтеЗначения.Видимость = Истина;
	Иначе 
		Элементы.ДекорацияРассчитайтеЗначения.Видимость = Ложь;
	КонецЕсли;	
	Если ТипЗнч(ВладелецОбъектовВыбора) = Тип("ДокументСсылка.Мероприятие") Тогда
		Элементы.ОбъектыОценкиКонтрагент.Видимость = Ложь;
	КонецЕсли;	
КонецПроцедуры
	...
	Если ЗначениеЗаполнено(ВладелецОбъектовВыбора) И ЗначениеЗаполнено(Сотрудник)
		...
		Иначе // это оценка в рамках процедуры выбора
			Запрос.Текст = 
				...
				|ПОМЕСТИТЬ ЭтапыПроцессаВыбора
				|ИЗ
				|	Справочник.НастройкиПроцессаВыбора.ЭтапыВыбора КАК НастройкиПроцессаВыбораЭтапыВыбора
				|ГДЕ
				|	НастройкиПроцессаВыбораЭтапыВыбора.Ссылка = &НастройкаПроцессаВыбора
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ
				|	ЭтапыПроцессаВыбора.Этап КАК ЭтапВыбора,
				|	Проекты.Ссылка КАК Объект,
				|	&РазделИнвестиционнойПрограммы КАК Владелец
				|ПОМЕСТИТЬ ОбъектыВыбора
				|ИЗ
				|	Проекты КАК Проекты
				|		ЛЕВОЕ СОЕДИНЕНИЕ ЭтапыПроцессаВыбора КАК ЭтапыПроцессаВыбора
				|		ПО (ИСТИНА)
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				...
				|	&КоличествоКритериев - ЕСТЬNULL(КоличествоКритериевСотрудника.КоличествоЗаполненных, 0) КАК НеЗаполнено,
				|	ЕСТЬNULL(СостояниеОценкиОбъектов.Завершен, ЛОЖЬ) КАК ОценкаЗавершена,
				|	ОбъектыВыбора.Объект.ОсновнойКонтрагент КАК Контрагент
				|ИЗ
				|	ОбъектыВыбора КАК ОбъектыВыбора
				|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоКритериевСотрудника КАК КоличествоКритериевСотрудника
				|		ПО ОбъектыВыбора.Объект = КоличествоКритериевСотрудника.ОбъектОценки
				|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостояниеОценкиОбъектов КАК СостояниеОценкиОбъектов
				|		ПО ОбъектыВыбора.Объект = СостояниеОценкиОбъектов.ОбъектОценки
		...
		Запрос.УстановитьПараметр("РазделИнвестиционнойПрограммы",РазделИнвестиционнойПрограммы);
