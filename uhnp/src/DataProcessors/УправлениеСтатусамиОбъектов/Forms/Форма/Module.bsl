...
Процедура УстановитьПараметрыЗапросаСпискаОбъектов(УстанавливатьВидОбъектаПоУмолчаниюВход = Ложь)
	...
	Если ЗначениеЗаполнено(ВыбранныйВидОбъекта) Тогда
		...
		Иначе
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "НастраиваемыйОтчет.Организация = &Организация", "ИСТИНА");
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "НастраиваемыйОтчет.Организация КАК Организация", "ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка) КАК Организация");
			СписокОбъектов.ТекстЗапроса = ТекстЗапроса;
			СписокОбъектов.Параметры.УстановитьЗначениеПараметра("Организация", Неопределено);	
		КонецЕсли;
	Иначе
	...
	Если ЗначениеЗаполнено(ВыбранныйВидОбъекта) Тогда
		Если ТипЗнч(ВыбранныйВидОбъекта) = Тип("СправочникСсылка.ДокументыБД") Тогда
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Документ.НастраиваемыйОтчет", "Документ." + ВыбранныйВидОбъекта.Наименование);
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "НастраиваемыйОтчет", НаименованиеДокумента);
			СписокОбъектов.ТекстЗапроса = ТекстЗапроса;
		...
		ИначеЕсли ТипЗнч(ВыбранныйВидОбъекта) = Тип("СправочникСсылка.СправочникиБД") Тогда
			// Справочники.
			НаименованиеДокумента = ВыбранныйВидОбъекта.Наименование;
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Документ.НастраиваемыйОтчет", "Справочник." + ВыбранныйВидОбъекта.Наименование);
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "НастраиваемыйОтчет", НаименованиеДокумента);
			СписокОбъектов.ТекстЗапроса = ТекстЗапроса;
		...
		Иначе
			...
			СписокОбъектов.ОсновнаяТаблица = "Документ.НастраиваемыйОтчет";
		КонецЕсли;
	Иначе
		...
		Если УстанавливатьВидОбъектаПоУмолчаниюВход Тогда
			Объект.ВидОбъектаБд = Справочники.ДокументыБД.НайтиПоНаименованию("НастраиваемыйОтчет", , , ТекИБ);
		Иначе
