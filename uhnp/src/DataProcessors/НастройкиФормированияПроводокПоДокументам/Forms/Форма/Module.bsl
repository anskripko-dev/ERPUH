...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОпределитьПланСчетов();
	РегистрБухгалтерии=Справочники.РегистрыБухгалтерииБД.НайтиПоНаименованию("МСФО",Истина,,Справочники.ТипыБазДанных.ТекущаяИБ);
			
КонецПроцедуры

&НаСервере
Процедура ОпределитьПланСчетов() 
	
	Запрос=Новый Запрос;
	Запрос.Текст="ВЫБРАТЬ
	             |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ШаблоныТрансформационныхКорректировок.Ссылка) КАК КоличествоШаблонов,
	             |	ШаблоныТрансформационныхКорректировок.ПланСчетов КАК ПланСчетов
	             |ПОМЕСТИТЬ ПланыСчетовШаблоны
	             |ИЗ
	             |	Справочник.ШаблоныТрансформационныхКорректировок КАК ШаблоныТрансформационныхКорректировок
	             |
	             |СГРУППИРОВАТЬ ПО
	             |	ШаблоныТрансформационныхКорректировок.ПланСчетов
	             |;
	             |
	             |////////////////////////////////////////////////////////////////////////////////
	             |ВЫБРАТЬ
	             |	СчетаБД.Владелец КАК ПланСчетов,
	             |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СчетаБД.Ссылка) КАК КоличествоСчетов
	             |ПОМЕСТИТЬ ПланыСчетовСчета
	             |ИЗ
	             ...
	             |////////////////////////////////////////////////////////////////////////////////
	             |ВЫБРАТЬ ПЕРВЫЕ 1
	             |	ПланыСчетовШаблоны.ПланСчетов КАК ПланСчетов,
	             |	ПланыСчетовШаблоны.КоличествоШаблонов КАК КоличествоШаблонов
	             |ИЗ
	             |	ПланыСчетовШаблоны КАК ПланыСчетовШаблоны
	             |
	             |УПОРЯДОЧИТЬ ПО
	             |	ПланыСчетовШаблоны.КоличествоШаблонов УБЫВ
	             |;
	             |
	             |////////////////////////////////////////////////////////////////////////////////
	             |ВЫБРАТЬ ПЕРВЫЕ 1
	             |	ПланыСчетовСчета.ПланСчетов КАК ПланСчетов
	             |ИЗ
	             |	ПланыСчетовСчета КАК ПланыСчетовСчета
	             |
	             |УПОРЯДОЧИТЬ ПО
	             |	ПланыСчетовСчета.КоличествоСчетов УБЫВ";
	
	Результат=Запрос.ВыполнитьПакет();
	
	ТаблицаШаблонов=Результат[2].Выгрузить();
	
	Если ТаблицаШаблонов.Количество()>0 И ТаблицаШаблонов[0].КоличествоШаблонов>0 Тогда
		
		ПланСчетов=ТаблицаШаблонов[0].ПланСчетов;
		
	Иначе
		
		ТаблицаСчетов=Результат[3].Выгрузить();
		
		Если ТаблицаСчетов.Количество()>0 Тогда
			
			ПланСчетов=ТаблицаСчетов[0].ПланСчетов;
			
		КонецЕсли;
		
	КонецЕсли;
	
КонецПроцедуры // ОпределитьПланСчетов() 
