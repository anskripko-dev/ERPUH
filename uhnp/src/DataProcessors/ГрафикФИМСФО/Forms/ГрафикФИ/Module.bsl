...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ДатаПризнанияДоРекласса <> Дата(1,1,1) Тогда
		...
		ПараметрыПредыдущегоГрафика = УчетФинансовыхИнструментовМСФОСерверУХ.ПолучитьПараметрыПредыдущегоГрафика(ДанныеГрафика);
	...
	КэшируемыеЗначения.Вставить("КлючевыеСубконто", 		Новый Соответствие);
	
	МСФОУХ.ЗаполнитьДоступностьСубконто(ЭтотОбъект, КэшируемыеЗначения.ИменаСубконто.ОсновнойДолг, 			"ОсновнойДолг");
	МСФОУХ.ЗаполнитьДоступностьСубконто(ЭтотОбъект, КэшируемыеЗначения.ИменаСубконто.ДополнительныеРасходы,	"ДополнительныеРасходы");
	МСФОУХ.ЗаполнитьДоступностьСубконто(ЭтотОбъект, КэшируемыеЗначения.ИменаСубконто.АвансовыеПлатежи,		"АвансовыеПлатежи");
	
	УстановитьУсловноеОформление();
	НастроитьСчетаУчета();
	
	УчетФинансовыхИнструментовМСФОКлиентСерверУХ.УправлениеФормой(ЭтотОбъект);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если (ИмяСобытия = "ЗагрузитьГрафикИзТабличногоДокумента") И (Источник = ЭтотОбъект) Тогда
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если (ЭтотОбъект = ИсточникВыбора.ВладелецФормы) Тогда
...
Процедура ЗаписатьГрафик(Команда)
	...
	Оповестить("ЗагрузитьИзФормыГрафика", АдресДанныхГрафика, ЭтотОбъект.ВладелецФормы);
...
Процедура ЗагрузитьПогашениеПроцентов(Команда)
	...
	ОткрытьФорму("Обработка.ЗагрузкаГрафикаФИ.Форма.Форма", ПараметрыФормы, ЭтотОбъект);
...
Процедура ПодборВНА(Команда)
	...
	ОткрытьФорму(ИмяФормыОткрытия, ПараметрыВыбора, ЭтотОбъект);
...
Процедура СтавкаНДСПриИзменении(Элемент)
	...
	Для каждого ТекущаяСтрока Из Объект.ОсновнойДолг Цикл
		ТекущаяСтрока.НДС = ОбработкаТабличныхЧастейКлиентСерверУХ
			.СуммаНДС(ТекущаяСтрока.Получение, Истина, СтавкаНДС);
	КонецЦикла;
	
	Для каждого ТекущаяСтрока Из Объект.АвансовыеПлатежи Цикл
		ТекущаяСтрока.НДСВВалюте = ОбработкаТабличныхЧастейКлиентСерверУХ
			.СуммаНДС(ТекущаяСтрока.СуммаВВалюте, Истина, СтавкаНДС);
	КонецЦикла;
...
Процедура ОсновнойДолгПолучениеПриИзменении(Элемент)
	...
	Если ТекущаяСтрока <> Неопределено Тогда
		ТекущаяСтрока.НДС = ОбработкаТабличныхЧастейКлиентСерверУХ
			.СуммаНДС(ТекущаяСтрока.Получение, Истина, ОБъект.СтавкаНДС);
	КонецЕсли;
...
Процедура ОсновнойДолгСчетПриИзменении(Элемент)
	МСФОКлиентСерверУХ.ПриИзмененииСчетаТЧ(ЭтотОбъект, Элемент, "ОсновнойДолг");
КонецПроцедуры
...
Процедура ОсновнойДолгПередНачаломИзменения(Элемент, Отказ)
	...
	ИменаСубконто = ЭтотОбъект.КэшируемыеЗначения.ИменаСубконто.ОсновнойДолг;
	
	Для каждого СчетСубконто Из ИменаСубконто Цикл
		МСФОКлиентСерверУХ.ОбновитьСубконтоСчетаТЧ(
			ЭтотОбъект, ТекущаяСтрока, СчетСубконто.Ключ, "ОсновнойДолг", ИменаСубконто);
	КонецЦикла;
...
Процедура ПроцентыДенежныйПотокПриИзменении(Элемент)
	...
	ТекущаяСтрока.НДС = ОбработкаТабличныхЧастейКлиентСерверУХ
			.СуммаНДС(ТекущаяСтрока.Уплата, Истина, Объект.СтавкаНДС);
...
Процедура ДополнительныеРасходыСчетРасходаНСБУПриИзменении(Элемент)
	МСФОКлиентСерверУХ.ПриИзмененииСчетаТЧ(ЭтотОбъект, Элемент, "ДополнительныеРасходы");
КонецПроцедуры
...
Процедура ДополнительныеРасходыПередНачаломИзменения(Элемент, Отказ)
	...
	ИменаСубконто = ЭтотОбъект.КэшируемыеЗначения.ИменаСубконто.ДополнительныеРасходы;
	
	Для каждого СчетСубконто Из ИменаСубконто Цикл
		МСФОКлиентСерверУХ.ОбновитьСубконтоСчетаТЧ(
			ЭтотОбъект, ТекущаяСтрока, СчетСубконто.Ключ, "ДополнительныеРасходы", ИменаСубконто);
	КонецЦикла;
...
Процедура ВНАПередНачаломИзменения(Элемент, Отказ)
	...
	ИменаСубконто = ЭтотОбъект.КэшируемыеЗначения.ИменаСубконто.ВНА;
	
	Для каждого СчетСубконто Из ИменаСубконто Цикл
		МСФОКлиентСерверУХ.ОбновитьСубконтоСчетаТЧ(
			ЭтотОбъект, ТекущаяСтрока, СчетСубконто.Ключ, "ВНА", ИменаСубконто);
	КонецЦикла;
...
Процедура АвансовыеПлатежиСуммаБезНДСВВалютеПриИзменении(Элемент)
	...
	ТекущаяСтрока.НДСВВалюте = ОбработкаТабличныхЧастейКлиентСерверУХ
			.СуммаНДС(ТекущаяСтрока.СуммаБезНДСВВалюте, Ложь, Объект.СтавкаНДС);
...
Процедура АвансовыеПлатежиСчетВзаиморасчетовМСФОПриИзменении(Элемент)
	МСФОКлиентСерверУХ.ПриИзмененииСчетаТЧ(ЭтотОбъект, Элемент, "АвансовыеПлатежи");
КонецПроцедуры
...
Процедура АвансовыеПлатежиСуммаВВалютеПриИзменении(Элемент)
	...
	Если ТекущаяСтрока <> Неопределено Тогда
		ТекущаяСтрока.НДСВВалюте = ОбработкаТабличныхЧастейКлиентСерверУХ
			.СуммаНДС(ТекущаяСтрока.СуммаВВалюте, Истина, Объект.СтавкаНДС);
	КонецЕсли;
...
Процедура АвансовыеПлатежиПередНачаломИзменения(Элемент, Отказ)
	...
	ИменаСубконто = ЭтотОбъект.КэшируемыеЗначения.ИменаСубконто.АвансовыеПлатежи;
	
	Для каждого СчетСубконто Из ИменаСубконто Цикл
		МСФОКлиентСерверУХ.ОбновитьСубконтоСчетаТЧ(
			ЭтотОбъект, ТекущаяСтрока, СчетСубконто.Ключ, "АвансовыеПлатежи", ИменаСубконто);
	КонецЦикла;
...
Процедура ЗаполнитьГрафик(Знач СтруктураДействий)
	...
	Если ТипЗнч(ЭтотОбъект.ТекущийЭлемент) = Тип("ТаблицаФормы") Тогда
		ИмяТаблицы = ЭтотОбъект.ТекущийЭлемент.Имя;
	ИначеЕсли ТипЗнч(ЭтотОбъект.ТекущийЭлемент) = Тип("ПолеФормы") 
			И ТипЗнч(ЭтотОбъект.ТекущийЭлемент.Родитель) = Тип("ТаблицаФормы") Тогда
		ИмяТаблицы = ЭтотОбъект.ТекущийЭлемент.Родитель.Имя;
	КонецЕсли;
	
	Если ИмяТаблицы <> Неопределено Тогда
		
		ТекущаяСтрока = Элементы[ИмяТаблицы].ТекущаяСтрока;
		ЭтотОбъект.Объект[ИмяТаблицы].Сортировать("Дата Возр");
...
Процедура НастроитьСчетаУчета()
	...
	МСФОУХ.ОбновитьКэшируемыеЗначенияОрганизации(ЭтотОбъект, Документ.Организация);
	...
	псМСФО = ЭтотОбъект.КэшируемыеЗначения.ПланСчетовМСФО;	
	...
	МСФОУХ.ЗаполнитьДоступностьСубконто(ЭтотОбъект, ЭтотОбъект.КэшируемыеЗначения.ИменаСубконто.АвансовыеПлатежи, "АвансовыеПлатежи");
	МСФОУХ.ДобавитьОформлениеДоступностиСубконто(ЭтотОбъект, ЭтотОбъект.КэшируемыеЗначения.ИменаСубконто.АвансовыеПлатежи, "АвансовыеПлатежи");
	
	МСФОУХ.ЗаполнитьДоступностьСубконто(ЭтотОбъект, ЭтотОбъект.КэшируемыеЗначения.ИменаСубконто.ДополнительныеРасходы, "ДополнительныеРасходы");
	МСФОУХ.ДобавитьОформлениеДоступностиСубконто(ЭтотОбъект, ЭтотОбъект.КэшируемыеЗначения.ИменаСубконто.ДополнительныеРасходы, "ДополнительныеРасходы");
	
	МСФОУХ.ЗаполнитьДоступностьСубконто(ЭтотОбъект, ЭтотОбъект.КэшируемыеЗначения.ИменаСубконто.ОсновнойДолг, "ОсновнойДолг");	
	МСФОУХ.ДобавитьОформлениеДоступностиСубконто(ЭтотОбъект, ЭтотОбъект.КэшируемыеЗначения.ИменаСубконто.ОсновнойДолг, "ОсновнойДолг");
...
Процедура УстановитьУсловноеОформление()
	...
	ЭтоИзменениеГрафика = Объект.ДатаПризнанияДоРекласса <> Дата(1,1,1);
	ЭтоВыбытие = ЭтоИзменениеГрафика И (Объект.ДатаПризнания <= ДатаОкончанияПериодаОтчета);
	...
	Для каждого ИмяТЧ Из ИменаТЧ Цикл
		...
		Если Не ЭтоИзменениеГрафика И ИмяТЧ = "АвансовыеПлатежи" Тогда
			Продолжить;//авансы при признании доступны на более ранние даты
		КонецЕсли;
...
Процедура ДобавитьПоля(ЭлементОформления, Поля)
	...
	Для каждого ИмяПоля Из СтрРазделить(Поля, ",") Цикл
		ПолеОформления = Новый Структура("Поле, Использование", Новый ПолеКомпоновкиДанных(СокрЛП(ИмяПоля)), Истина);
		ЗаполнитьЗначенияСвойств(ЭлементОформления.Поля.Элементы.Добавить(), ПолеОформления);
	КонецЦикла;
...
Процедура УсловноеОформление_ВНА()
	...
	//ВводВЭксплуатацию 
	ИмяПоляОформления = 
		"ВНАПараметрыУчетаВНА, ВНАСпособНачисленияАмортизацииМСФО,
		|ВНАСпособОтраженияРасходовПоАмортизацииМСФО, ВНАСрокПолезногоИспользованияМСФО, 
		|ВНАЛиквидационнаяСтоимостьМСФО";
	...
	//УменьшениеЛизинга 
	ИмяПоляОформления = "ВНАСписатьНаВНА, ВНАСуммаСписанияВНА";
	ЭлементОформления = УсловноеОформление.Элементы.Добавить();
	УстановитьОформление(ЭлементОформления.Оформление, ОформлениеСкрыть);
	ДобавитьПоля(ЭлементОформления, ИмяПоляОформления);
	...
	// другие операции - 
	ИмяПоляОформления = "ВНАПустаяСтрокаШапки";
	ЭлементОформления = УсловноеОформление.Элементы.Добавить();
	УстановитьОформление(ЭлементОформления.Оформление, ОформлениеСкрыть);
	ДобавитьПоля(ЭлементОформления, ИмяПоляОформления);
...
Процедура ЗаполнитьТекущимиДаннымиСервер(ЗаполнитьРеквизиты = Ложь, ЗаполнитьНСБУ = Ложь, ЗаполнитьМСФО = Ложь, СтрокаВНА = Ложь) 
	...
	Документы.ВыбытиеВНАМСФО.ЗаполнитьСтроки(СтрокиДляЗаполнения, ЭтотОбъект, ПараметрыЗаполнения);
