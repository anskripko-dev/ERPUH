...
&НаСервере
Процедура ДобавитьЭлементыКоллекции(ДеревоПолей, ДеревоЗначений, ПолеПоиска, Операнд, ДеревоОтборов = Неопределено)
	
	ЭтоПараметрыОтбора = (ПолеПоиска = "ПолеБД");
	
	Если ДеревоОтборов = Неопределено Тогда
	...
	Для Каждого Строка ИЗ ДеревоПолей.Строки Цикл
		...
		Если ЭтоПараметрыОтбора И ЭтоГруппаОтборов(Строка.НаименованиеБД) Тогда
			СтруктураПоиска = Новый Структура("НаименованиеБД", "");
		КонецЕсли;
		ЗаполнитьЗначенияСвойств(СтруктураПоиска, Строка);
		
		Если ЭтоПараметрыОтбора Тогда
			МассивСтрок = ДеревоОтборов.Строки.НайтиСтроки(СтруктураПоиска, Истина);
		Иначе
		...
		Иначе
			...
			Если ЭтоПараметрыОтбора И СпособыОтбораРазличаются(СтрокаДерева, Строка) тогда
				
				УстановитьСпособВычисленияПоУмолчанию(СтрокаДерева);
				СтрокаДерева.УточнениеСпособаОпределения = Неопределено;
				ПеренестиСтрокуДереваВКорень(СтрокаДерева, ДеревоОтборов);
		...
		Если Строка.Строки.Количество() > 0 Тогда
			ДобавитьЭлементыКоллекции(Строка, СтрокаДерева, ПолеПоиска, Операнд, ДеревоЗначений);
		КонецЕсли;
...
&НаСервере
Функция ЭтоГруппаОтборов(НаименованиеБД)
	
	ЭтоГруппаОтборов = Ложь;
	Если СтрНайти(НаименованиеБД, "Группа") Тогда
		ЭтоГруппаОтборов = Истина;
	КонецЕсли;
	
	Возврат ЭтоГруппаОтборов;
	
КонецФункции

&НаСервере
Функция СпособыОтбораРазличаются(СтрокаДерева, Строка)
...
&НаСервере
Процедура УстановитьСпособВычисленияПоУмолчанию(СтрокаДерева)
	
	Если СтрокаДерева.ПолеБД = "" Тогда
		Возврат;
	КонецЕсли;
	
	Если СтрокаДерева.ПолеБД = "Версия.Валюта" Тогда
		СтрокаДерева.СпособВычисленияПараметра = Перечисления.СпособыВычисленияПараметровОперандов.ВалютаОтчета;
	ИначеЕсли СтрокаДерева.ПолеБД = "Версия.Организация" Тогда
		СтрокаДерева.СпособВычисленияПараметра = Перечисления.СпособыВычисленияПараметровОперандов.ОрганизацияОтчета;
	ИначеЕсли СтрокаДерева.ПолеБД = "Версия.ПериодОтчета" Тогда
		СтрокаДерева.СпособВычисленияПараметра = Перечисления.СпособыВычисленияПараметровОперандов.ПериодОтчета;
	ИначеЕсли СтрокаДерева.ПолеБД = "Версия.Проект" Тогда
		СтрокаДерева.СпособВычисленияПараметра = Перечисления.СпособыВычисленияПараметровОперандов.ПроектОтчета;
	ИначеЕсли СтрокаДерева.ПолеБД = "Версия.Сценарий" Тогда
		СтрокаДерева.СпособВычисленияПараметра = Перечисления.СпособыВычисленияПараметровОперандов.СценарийОтчета;
	Иначе
...
&НаСервере
Процедура ПеренестиСтрокуДереваВКорень(СтрокаДерева, ДеревоОтборов)
	
	Если СтрокаДерева.Родитель = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ЭтоГруппаОтборов(СтрокаДерева.Родитель.НаименованиеБД) Тогда
...
&НаСервере
Процедура УдалитьПустыеГруппы(ДеревоОтборов)
	
	Для Каждого Строка Из ДеревоОтборов.Строки Цикл
		
		Если ЭтоГруппаОтборов(Строка.НаименованиеБД) Тогда
			
			Если Строка.Строки.Количество() = 0 Тогда
				ДеревоОтборов.Строки.Удалить(Строка);
			КонецЕсли;
...
Процедура ВосстановитьНастройкиОперанда()
	...
	Для Каждого Операнд Из СписокОперандов Цикл
		...
		УдалитьПустыеГруппы(ДеревоПараметровОтбораДанные);
