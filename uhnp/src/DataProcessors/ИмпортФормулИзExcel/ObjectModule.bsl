...
Функция ИнициализацияДокумента() Экспорт
	...
	Исключение
		ОбщегоНазначенияУХ.СообщитьОбОшибке(Нстр("ru = 'Не удалось создать объект Microsoft Excel'"));
		Возврат Ложь;
	КонецПопытки;
...
Функция АнализФормулы(Формула)
	...
	Для Каждого Строка Из ТаблицаПеременных Цикл
		Если СтрДлина(Строка.Х1) > 2 Тогда
			ОбщегоНазначенияУХ.СообщитьОбОшибке(СтрШаблон(Нстр("ru = 'Формула: %1 не импортирована. Формула должна содержать только арифметические функции'"), 
				Формула));
...
Функция ПолучитьТекстФормул() Экспорт
	...
	Исключение
		ОбщегоНазначенияУХ.СообщитьОбОшибке(Нстр("ru = 'Документ не содержит ячеек с формулами'"));
	...
	ОбщегоНазначенияУХ.СообщитьОбОшибке(СтрШаблон(Нстр("ru = 'Обнаружено %1 ячеек с формулами'"), 
		Формат(ЯчейкиСФормулами.Count, "ЧДЦ=0; ЧН=0; ЧГ=3,0")), , , СтатусСообщения.Информация);
	...
	ОбщегоНазначенияУХ.СообщитьОбОшибке(СтрШаблон(Нстр("ru = 'Удалось распознать формулы для %1 ячеек'"), 
		Формат(ТаблицаФормул.Количество(), "ЧДЦ=0; ЧН=0; ЧГ=3,0")), , , СтатусСообщения.Информация);
		...
		Если ТекОбласть.СодержитЗначение Тогда
			...
			Если ТекПоказатель <> Неопределено Тогда
				...
				Иначе
					...
					ОбщегоНазначенияУХ.СообщитьОбОшибке(СтрШаблон(Нстр("ru = 'Не удалось сопоставить ячейки с операндами для показателя: %1'"), 
						ТекПоказатель.Наименование));
	...
	ОбщегоНазначенияУХ.СообщитьОбОшибке(СтрШаблон(Нстр("ru = 'Обработано %1 показателей'"), ОбщееЧислоПоказателей), , , СтатусСообщения.Информация);
	ОбщегоНазначенияУХ.СообщитьОбОшибке(СтрШаблон(Нстр("ru = 'Создана формула для %1 показателей'"), ОбщееЧислоПоказателей), , , СтатусСообщения.Информация);
