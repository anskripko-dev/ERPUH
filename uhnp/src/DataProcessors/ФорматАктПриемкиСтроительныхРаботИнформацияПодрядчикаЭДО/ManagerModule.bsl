...
Функция НалоговыеСтавки() Экспорт
	...
	Данные.Вставить("НДС9_09", "9,09%");
	Данные.Вставить("НДС10", "10%");
	Данные.Вставить("НДС16_67", "16,67%");
	Данные.Вставить("НДС18_03", "18,03%");
	Данные.Вставить("НДС20", "20%");
	Данные.Вставить("НДС22", "22%");
	Данные.Вставить("НДС5_105", "5/105");
	Данные.Вставить("НДС7_107", "7/107");
	Данные.Вставить("НДС10_110", "10/110");
	Данные.Вставить("НДС20_120", "20/120");
	Данные.Вставить("НДС22_122", "22/122");
...
Процедура ЗаполнитьСведенияОВыполненнойРаботе(Объект, Работа, КонструкторЭД, Ошибки)
	...
	Если ТипЗнч(Работа) = ТипыРабот.Работа Тогда
		...
		Если ИнформацияПодрядчика.НастройкиДокумента.СпособРасчетаНДС = СпособыРасчетаНДС().ТолькоВИтогах Тогда
			Если ФорматыЭДО_ФНС.ДействуютСтавкиНДС_2026() Тогда
				ОсновнаяСтавка = НалоговыеСтавки.НДС22;
			Иначе
				ОсновнаяСтавка = НалоговыеСтавки.НДС20;
			КонецЕсли;
			Если Работа.СтавкаНДС <> ОсновнаяСтавка Тогда
...
Процедура ЗаполнитьСуммуНДСВРазбивкеПоСтавкам(Объект, СтавкаИНалог, КонструкторЭД, Ошибки)
	...
	Иначе
		ОсновнаяСтавка = НалоговыеСтавки.НДС20;
	КонецЕсли;
	Если Ставка <> ОсновнаяСтавка И ЗначениеЗаполнено(Налог.ПозицииАкта) Тогда
	...
	Если ТипЗнч(Работа) = ТипыРабот.Работа Тогда
		...
		Если Не ЗначениеЗаполнено(Ставка) Тогда
			...
			Иначе
				ОсновнаяСтавка = НалоговыеСтавки().НДС20;
			КонецЕсли;
			Ставка = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Объект, "НалСтВИтог", ОсновнаяСтавка);
		КонецЕсли;
...
Процедура ПрочитатьСуммуНДСВРазбивкеПоСтавкам(Объект, СтавкаИНалог, КонструкторЭД)
	...
	Иначе
		ОсновнаяСтавка = НалоговыеСтавки.НДС20;
	КонецЕсли;
	Если Ставка <> ОсновнаяСтавка И ТипЗнч(ПозицииАкта) = Тип("СписокXDTO") Тогда
