...
Перем ДокументРезультат; // ТабличныйДокумент
Перем ЕстьПериод;
...
Перем ТекстОтбораИсточник Экспорт;
Перем ТекстОтбораИтог Экспорт;
Перем СтрокаОтборовИтог Экспорт;
...
Функция СформироватьСтрокуВыводаПараметровПоДатам() Экспорт
	...
	Если ДатаНач = '00010101000000' И ДатаКон = '00010101000000' Тогда		
		ОписаниеПериода     = НСтр("ru = 'Период: без ограничения.'; en = 'Period: unlimited.';tr = 'Dönem: sınırsız.'");
	Иначе		
		Если ДатаНач = '00010101000000' ИЛИ ДатаКон = '00010101000000' Тогда			
			ОписаниеПериода = НСтр("ru = 'Период: '; en = 'Period:';tr = 'Dönem: '") + Формат(ДатаНач, "Л=ru_RU; ДФ = ""дд.ММ.гггг""; ДП = ""без ограничения""") 
							+ " - "      + Формат(ДатаКон, "Л=ru_RU; ДФ=дд.ММ.гггг; ДП='без ограничения'");			
		Иначе
			Если ДатаНач > ДатаКон Тогда
				ОписаниеПериода = "";
			Иначе				
				ОписаниеПериода = НСтр("ru = 'Период: '; en = 'Period:';tr = 'Dönem: '") + ПредставлениеПериода(НачалоДня(ДатаНач), КонецДня(ДатаКон), "ФП = Истина");
			КонецЕсли;
		КонецЕсли;		
	КонецЕсли;
...
Функция СформироватьСтрокуИзмерений()
	...
	Для Каждого СтрДетализация ИЗ ИспользуемыеПоляДетализации Цикл
		...
		ТекстДетализация=ТекстДетализация+", "+?(ПустаяСтрока(СтрДетализация.ПредставлениеПоля),СтрДетализация.ИмяПоля,СтрДетализация.ПредставлениеПоля);
 ...
 Функция ЗаголовокОтчета() Экспорт
	 ...
	 Если ВидОтчета="ОборотноСальдоваяВедомостьПоСчету" Тогда
		 ...
		 Возврат СтрШаблон(НСтр("ru = 'Оборотно-сальдовая ведомость по счету %1'; en = 'Trial balance for account %1';tr = 'Hesap geçici mizanı%1'"), ?(ЗначениеЗаполнено(ДанныеСчета.Код), ДанныеСчета.Код, НСтр("ru = ' не выбран счет'; en = ' no account selected';tr = ' hesap seçilmedi'")));
		 
	 ИначеЕсли ВидОтчета="КарточкаСчета" Тогда
		 
		 Возврат СтрШаблон(НСтр("ru = 'Карточка счета %1'; en = 'Account card %1';tr = 'Hesap kartı%1'"), ?(ЗначениеЗаполнено(ДанныеСчета.Код), ДанныеСчета.Код, НСтр("ru = ' не выбран счет'; en = ' no account selected';tr = ' hesap seçilmedi'")));
		 
	ИначеЕсли ВидОтчета="ОтчетПоПроводкам" Тогда
		 
		 Возврат СтрШаблон(НСтр("ru = 'Отчет по проводкам между счетами %1 и %2'; en = 'Report on entries between accounts %1 and %2';tr = 'Hesaplar arasındaki işlemler hakkında rapor %1 ve%2'"), ДанныеСчета.Код, ДанныеКоррСчета.Код);
		 
	 ИначеЕсли ВидОтчета = "АнализСчета" Тогда
		 		 
		 Возврат СтрШаблон(НСтр("ru = 'Анализ счета %1'; en = 'Analysis of account %1';tr = 'Hesap analizi%1'"), ?(ЗначениеЗаполнено(ДанныеСчета.Код), ДанныеСчета.Код, НСтр("ru = ' не выбран счет'; en = ' no account selected';tr = ' hesap seçilmedi'")));
		 
	 ИначеЕсли ВидОтчета = "ОборотноСальдоваяВедомость" Тогда
		 
		 Возврат НСтр("ru = 'Оборотно - сальдовая ведомость'; en = 'Trial balance';tr = 'Geçici mizan'");
		 
	 ИначеЕсли ВидОтчета = "ОстаткиИОбороты" Тогда
		 
		 Если ДанныеРегистра.ВидРегистра="Обороты" Тогда
			 
			 Возврат НСтр("ru = 'Обороты'; en = 'Turnovers';tr = 'Cirolar'");
			 
		 Иначе
			 
			 Возврат НСтр("ru = 'Остатки и обороты'; en = 'Balance and turnovers';tr = 'Bakiye ve cirolar'");
...
Функция СформироватьЗаголовокОСВПоСчету() 
	...
	Если ДанныеДляЗаголовка.Свойство("Организация") Тогда
		ЗаголовокОтчета.Параметры.НазваниеОрганизации = СтрШаблон(Нстр("ru = 'Организация: %1'; en = 'Company: %1';tr = 'Kuruluş:%1'"), ДанныеДляЗаголовка.Организация);
	КонецЕсли;
	...
	Если ПоСубсчетам Тогда
		ТекстПроИтоги = СтрШаблон(Нстр("ru = '%1субсчетам, '; en = '%1subaccounts, ';tr = '%1 alt hesaplar,'"), ТекстПроИтоги)КонецЕсли;

	ТекстДетализация=СформироватьСтрокуИзмерений();
	
	Если Не ПустаяСтрока(ТекстДетализация) Тогда
		ТекстПроИтоги = СтрШаблон(Нстр("ru = '%1субконто: %2'; en = '%1extra dimension: %2';tr = '%1ekstra boyut:%2'"), ТекстПроИтоги, ТекстДетализация);
	КонецЕсли;
	...
	Если Не ПустаяСтрока(ТекстПроИтоги) Тогда
		ЗаголовокОтчета.Параметры.ТекстПроИтоги = СтрШаблон(Нстр("ru = 'Детализация по %1'; en = 'Details by %1';tr = '%1 için ayrıntılar'"), ТекстПроИтоги);
	КонецЕсли;

	ЗаголовокОтчета.Параметры.Заголовок = ЗаголовокОтчета();
	
	Если ВнешняяИБ=1 Тогда 
		ЗаголовокОтчета.Параметры.ВнешняяИБ = НСтр("ru = 'Внешняя информационная база: '; en = 'External infobase:';tr = 'Dış Infobase:'") + ДанныеДляЗаголовка.ИспользуемаяИБ;
	КонецЕсли;
	
	Если ДанныеРегистра.Свойство("ПолеПланСчетов") Тогда
		
		ЗаголовокОтчета.Параметры.РегистрБухгалтерии = СтрШаблон(Нстр("ru = 'Регистр бухгалтерии: %1, план счетов: %2'; en = 'Accounting register: %1, chart of accounts: %2';tr = 'Muhasebe kaydı: %1, hesap planı: %2'"), 
	                                               	ДанныеРегистра.Синоним,ДанныеРегистра.ПланСчетов);
		
	Иначе
	
	ЗаголовокОтчета.Параметры.РегистрБухгалтерии = СтрШаблон(Нстр("ru = 'Регистр бухгалтерии: %1'; en = 'Accounting register: %1';tr = 'Muhasebe kaydı: %1'"), 
	                                               	ДанныеРегистра.Синоним);
													
	КонецЕсли;

	// Вывод списка фильтров:
	СтрОтбор =СтрокаОтборовИтог;

	Если Не ПустаяСтрока(СтрОтбор) Тогда
		ОбластьОтбор = Макет.ПолучитьОбласть("СтрокаОтбор");
		ОбластьОтбор.Параметры.ТекстПроОтбор = СтрШаблон(Нстр("ru = 'Отбор: %1'; en = 'Filter: %1';tr = 'Filtre: %1'"), СтрОтбор);
		ЗаголовокОтчета.НачатьГруппуСтрок("Отбор",Истина);
		ЗаголовокОтчета.Вывести(ОбластьОтбор);
		ЗаголовокОтчета.ЗакончитьГруппуСтрок();
	КонецЕсли;
...
Функция СформироватьЗаголовокАнализСчета()
	...
	Если ПоКорСубсчетамИСубконто Тогда
		ТекстПроИтоги = СтрШаблон(Нстр("ru = '%1, кор.субсчетам и субконто'; en = '%1, cor. subaccounts and extra dimension';tr = '%1,düzeltme alt hesapları ve ekstra boyut'"), ТекстПроИтоги)КонецЕсли;
	...
	ЗаголовокОтчета.Параметры.ТекстПроСписокПоказателей = Сред(ТекстПроСписокПоказателей,2);
	Если Не ПустаяСтрока(ТекстПроИтоги) Тогда
		ЗаголовокОтчета.Параметры.ТекстПроИтоги = СтрШаблон(Нстр("ru = 'Детализация по %1'; en = 'Details by %1';tr = '%1için ayrıntılar'"), ТекстПроИтоги);
	КонецЕсли;
	...
	Если Не ПустаяСтрока(СтрОтбор) Тогда
		...
		ЗаголовокОтчета.НачатьГруппуСтрок("Отбор",Истина);
		ЗаголовокОтчета.Вывести(ОбластьОтбор);
		ЗаголовокОтчета.ЗакончитьГруппуСтрок();	КонецЕсли;
...
Функция СформироватьЗаголовокОстаткиИОбороты() 
	...
	Если Не ПустаяСтрока(ТекстПроИтоги) Тогда
		ЗаголовокОтчета.Параметры.ТекстПроИтоги = НСтр("ru = 'Детализация по '; en = 'Details by ';tr = 'Ayrıntılar'") + ТекстПроИтоги;
	КонецЕсли;

	ЗаголовокОтчета.Параметры.Заголовок = НСтр("ru = 'Отчет по регистру накопления: '; en = 'Accumulation register report: ';tr = 'Birikim kayıt raporu: '") + ДанныеРегистра.Синоним;
	...
	Если Не ПустаяСтрока(СтрокаОтборовИтог) Тогда
		ОбластьОтбор = Макет.ПолучитьОбласть("СтрокаОтбор");
		ОбластьОтбор.Параметры.ТекстПроОтбор = НСтр("ru = 'Отбор: '; en = 'Filter: ';tr = 'Filtre: '") + СтрокаОтборовИтог;
		ЗаголовокОтчета.НачатьГруппуСтрок("Отбор",Истина);
		ЗаголовокОтчета.Вывести(ОбластьОтбор);
		ЗаголовокОтчета.ЗакончитьГруппуСтрок();
	КонецЕсли;
...
Функция НастроитьОбъектЗапрос()
	...
	РабочийЗапрос=ТекСоединениеВИБ.NewObject("Запрос");
	
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Определим параметры запроса
	
	РабочийЗапрос.SetParameter("ДатаНач",ДатаНач);
	
	Если ДатаКон <> '00010101000000' Тогда
		РабочийЗапрос.SetParameter("ДатаКон", КонецДня(ДатаКон));
	Иначе
		РабочийЗапрос.SetParameter("ДатаКон", ДатаКон);
	КонецЕсли;
	
	Если ИспользуетсяПараметрСчет() Тогда
		
		РабочийЗапрос.SetParameter("Счет", СчетСсылкаБД);
			
	КонецЕсли;
	
	Если ВнешняяИБ=0 Тогда
		
		УстановитьПараметрыЗапросаТек(РабочийЗапрос);

	Иначе
		
		РаботаСОбъектамиМетаданныхВнешнийУХ.УстановитьПараметрыЗапроса(ТекСоединениеВИБ,РабочийЗапрос,ИспользуемыеПараметры);
		
	КонецЕсли;
		
	Возврат РабочийЗапрос;	
...
Процедура ВывестиГруппировкуОборотноСальдовойВедомостиПоСчету(Выборка, Знач ИндексТекущейГруппировки, СтруктураПараметров) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Лев(ИмяГруппировки, СтрДлина("Субконто")) = "Субконто" Тогда
	...
	Для Каждого Ресурс Из МассивВыводимыхРесурсов Цикл
		...
		Если ВыведеноПоказателей > 0 Тогда
			...
			Если ИмяПоказателя = "ВалютнаяСумма" Тогда
				ВыводимаяОбласть.Параметры.СубконтоПредставление = Нстр("ru = 'Валютная сумма'; en = 'Currency amount';tr = 'Para birimi tutarı'");
			Иначе
...
Процедура ЗаполнитьРасшифровкуДляСчета(Область, Выборка, СтруктураПараметров, Знач РасшифровываемыйСчет, Знач РасшифровываемыйСчетПредставление) Экспорт
	...
	Если Лев(ИмяГруппировки, СтрДлина("Субконто")) = "Субконто" Тогда
		...
		СписокРасшифровки.Добавить(ПараметрыКарточкиСчета, СтрШаблон(Нстр("ru = 'Карточка счета %1'; en = 'Account card %1';tr = 'Hesap kartı%1'"), 
...
Процедура СформироватьТекстЗапросаОСВ(СтруктураПараметров,ПолнаяОСВ=Ложь)
	...
	Текст=Текст+"
	|"+?(ДанныеРегистра.МаксимальноеКоличествоСубконто>0,", ","") + ТекстОтбораИтог + ") КАК ОстаткиИОбороты
	|";
	
	ТекстИтогов = ВернутьЧастьЗапросаПоВыборкеПолейОборотноСальдоваяВедомость(Ложь,,, РазвернутоеСальдо);
	
	Текст = Текст + Символы.ПС + "ИТОГИ ТекстИтогов
		|	ПО ОБЩИЕ"; // @query-part
	
	Текст = СтрЗаменить(Текст, "ТекстИтогов", Символы.ПС + Сред(ТекстИтогов, 2));
...
Функция ПолучитьОСВПоСчету()
    ...
    Если Не ЗначениеЗаполнено(ДанныеСчета.Код) Тогда
		...
		ТекстОшибки= Нстр("ru = 'Не выбран счет!'; en = 'No account selected!';tr = 'Hesap seçilmedi!'");
...
Функция ПолучитьАнализСчета()
	...
	ТекстОбороты = ТекстОбороты + "
	|ИЗ
	|	РегистрБухгалтерии."+ДанныеРегистра.НаименованиеРегистра+".Обороты(&ДатаНач, &ДатаКон, "+Периодичность+", Счет В ИЕРАРХИИ (&Счет), , " + ТекстОтбораИтог + ") КАК Таблица
	|";
	
	ТекстОстатки = ТекстОстатки + "
	|ИЗ
	|	РегистрБухгалтерии."+ДанныеРегистра.НаименованиеРегистра+".ОстаткиИОбороты(&ДатаНач, &ДатаКон, "+Периодичность+", ДвиженияИГраницыПериода, счет В ИЕРАРХИИ (&счет),"+?(ДанныеРегистра.МаксимальноеКоличествоСубконто>0,", ","") + ТекстОтбораИтог + ") КАК Таблица
	|";
	...
	ТекстОбороты = ТекстОбороты + "
	|ИТОГИ ТекстИтогиОбороты
	|	ПО ОБЩИЕ";
	
	ТекстОбороты = СтрЗаменить(ТекстОбороты, "ТекстИтогиОбороты", Сред(ТекстИтогиОбороты, 2));
	
	ТекстОстатки = ТекстОстатки + "
	|ИТОГИ ТекстИтогиОстатки
	|	ПО ОБЩИЕ";
	
	ТекстОстатки = СтрЗаменить(ТекстОстатки, "ТекстИтогиОстатки", Сред(ТекстИтогиОстатки, 2));
	...
	Если ЗначениеЗаполнено(Период) Тогда
		...
		Если Врег(Период) = "ГОД" Тогда
			ФорматПериода = Нстр("ru = 'ДФ = ""гггг """"г.""""""'; en = 'DF = ""yyyy""';tr = 'DF = ""yyyy """"yıl.""""""'");
		ИначеЕсли Врег(Период) = "КВАРТАЛ" Тогда
			ФорматПериода = Нстр("ru = 'ДФ = ""к"""" квартал"""" гггг """"г.""""""'; en = 'DF=""Q"""" quarter"""" yyyy""';tr = 'DF = ""q"""" çeyrek"""" yyyy """"yıl.""""""'");
		ИначеЕсли Врег(Период) = "МЕСЯЦ" Тогда
			ФорматПериода = Нстр("ru = 'ДФ = ""ММММ гггг """"г.""""""'; en = 'DF = ""MMMM yyyy';tr = 'DF =""MMMM yyyy """"yıl.""""""'");
		ИначеЕсли Врег(Период) = "НЕДЕЛЯ" Тогда
			ФорматПериода = Нстр("ru = 'ДФ = """"""Неделя с"""" дд.ММ.гггг """"""'; en = 'DF = """"""Week from"""" MM/dd/yyyy """"""';tr = 'DF =""""""Haftadan"""" dd.MM.yyyy """"""'");
...
//
// Параметры:
//	Выборка - ВыборкаИзРезультатаЗапроса - выборка из результата запроса по выводимой группировке,
...
Процедура ВывестиПоказателиОборотноСальдовойВедомости(ДокументРезультат, Выборка, СтрокаРазвернутогоСальдо, СтруктураОбщийИтог, СтруктураПараметров)
	...
	Для Каждого Ресурс Из МассивВыводимыхРесурсов Цикл
		...
		Если ВыведеноПоказателей > 1 Тогда
			...
			Если ИмяПоказателя = "ВалютнаяСумма" Тогда
				ОбластьСтрокаПоказателяСчет.Параметры.СубконтоПредставление = Нстр("ru = 'Валютная сумма'; en = 'Currency amount';tr = 'Para birimi tutarı'");
			Иначе
...
Функция СформироватьТаблицуДанныхРазворотаСчетов(СтруктураПараметров, Знач ПравилаИтоги)
	...
	Для Каждого ПравилаСчета из ПравилаИтоги Цикл
		...
		Для Каждого СтрокаПоСубконто Из ПравилаСчета.Субконто Цикл
			...
			Сч = ДанныеСубконто.ВидСубконто.Номер;
			...
			Если ЕстьПредставление Тогда
				...
				ТекстВнеш  = ТекстВнеш + ", Представление(Субконто"+Сч+") КАК Субконто" + Сч + "Представление";
				
			Иначе
				
				ТекстВнеш  = ТекстВнеш + ", Субконто"+Сч+" КАК Субконто" + Сч + "Представление";
		...
		|
		|ИЗ
		|	РегистрБухгалтерии."+ДанныеРегистра.НаименованиеРегистра+".ОстаткиИОбороты(&ДатаНач, &ДатаКон, , ,Счет В ИЕРАРХИИ (&Счет),"+?(ДанныеРегистра.МаксимальноеКоличествоСубконто>0,", ","") + ТекстОтбораИтог + ") КАК ОстаткиИОбороты";
		...
		Если Не ПустаяСтрока(ТекстИтогиПо) Тогда
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТекстИтогов", Сред(ТекстИтогов, 2));
...
Процедура ВывестиДанныеОстатковКарточкиСчетаВМакет(ЗапросПоОстаткам, Знач ОбластьМакет, Знач ЗапросНаНачало,СтруктураСальдо) Экспорт
	...
	Для Каждого СтрРесурс ИЗ МассивВыводимыхРесурсов Цикл
		...
		Если ЗапросНаНачало Тогда
			...
			ОбластьМакет.Параметры.ОписательСальдо = СтрШаблон(Нстр("ru = '%1: сальдо на начало'; en = '%1: opening balance';tr = '%1: dönem başı bakiyesi'"), СтрРесурс.ПредставлениеПоля);
			
		Иначе
			
			ОбластьМакет.Параметры.ОписательСальдо = СтрШаблон(Нстр("ru = '%1: сальдо на конец'; en = '%1: closing balance';tr = '%1:dönem sonu bakiyesi'"), СтрРесурс.ПредставлениеПоля);
...
Функция ЗначениеПоляСоответствуетОтбору(Выборка, Знач СторонаПроводки) Экспорт
	...
	Для Каждого ЭлементОтбора Из ИспользуемыеПараметры Цикл
		...
		ПолеБД=?(ЗначениеЗаполнено(ДанныеКоррСчета.Код),ЭлементОтбора.ПолеБД,СтрЗаменить(ЭлементОтбора.ПолеБД,"Дт",""));
		
		ИмяПоля = СтрЗаменить(ПолеБД, "Субконто", "Субконто"+СторонаПроводки);
		
		Если ВРЕГ(Лев(ПолеБД, СтрДлина("СУБКОНТО"))) = "СУБКОНТО" Тогда
			ИмяПоля = "Субконто" + СторонаПроводки + Сред(ПолеБД,СтрДлина("СУБКОНТО")+1);
		КонецЕсли;
		Если ВРЕГ(Лев(ПолеБД, СтрДлина("ВАЛЮТА"))) = "ВАЛЮТА" Тогда
			ИмяПоля = "Валюта" + СторонаПроводки + Сред(ПолеБД,СтрДлина("ВАЛЮТА")+1);
		КонецЕсли;
		
		ЗначениеПоля = Выборка[ИмяПоля];
		ЗначениеОтбора=ОбъектЗапрос.Параметры[ЭлементОтбора.ИмяПараметра];
...
Функция ПолучитьКарточкуСчета(ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	+ ТекстОтбораИтог + ") КАК ОсновнойОстатки 	
	|
	|) КАК Выборка";
	...
	|		&НачПериода,
	|		&КонПериода,
	| (Активность = ИСТИНА) И (Счет В ИЕРАРХИИ (&СчетАнализа))"+?(ПустаяСтрока(ТекстОтбораИтог),""," И "+ТекстОтбораИтог)+") КАК ОсновнойДвиженияССубконто
	|
	...
	Если НужныПромежуточныеИтогиПоПериоду Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТекстРесурсы", Сред(ТекстРесурсы,2));
		
	КонецЕсли;
		
	ЗапросПоПроводкам.Текст = ТекстЗапроса;
	Проводки = ЗапросПоПроводкам.Выполнить().Выбрать(); // ВыборкаИзРезультатаЗапроса
	...
	Пока Проводки.Следующий() Цикл
		...
		Если Проводки.ТипЗаписи() = ?(ВнешняяИБ=0,QueryRecordType.GroupTotal,ТекСоединениеВИБ.QueryRecordType.GroupTotal) Тогда
			...
			Если ОбластьПериод=Неопределено Тогда
				...
				ОбластьПериод.Параметры.ОписательПериода = СтрШаблон(Нстр("ru = 'Обороты за %1'; en = 'Turnovers for %1';tr = '%1için Cirolar'"), ПолучитьПериодДатСтрокой(Период, Проводки.Период));
	...
	ОписательОборотов.Параметры.ОписательПериода = Нстр("ru = 'Обороты за период'; en = 'Turnover for a period';tr = 'Dönem cirosu'");
...
//
Функция ПолучитьТекстЗапроса(МаксКоличествоСубконто)
	
	Если КоррСчетСсылкаБД=Неопределено Тогда
		
		МаксКоличествоСубконто=ПолучитьМаксКоличествоСубконто(СчетСсылкаБД);
		ТекстОтбораСчета="Активность = ИСТИНА И СчетДт В ИЕРАРХИИ(&Счет)";
		
	ИначеЕсли СчетСсылкаБД=Неопределено Тогда
		
		МаксКоличествоСубконто=ПолучитьМаксКоличествоСубконто(КоррСчетСсылкаБД); 
		ТекстОтбораСчета= "Активность = ИСТИНА И СчетКт В ИЕРАРХИИ(&КоррСчет)";
		
	Иначе
		
		МаксКоличествоСубконто=Макс(ПолучитьМаксКоличествоСубконто(СчетСсылкаБД),ПолучитьМаксКоличествоСубконто(КоррСчетСсылкаБД));
		ТекстОтбораСчета = "Активность = ИСТИНА И СчетДт В ИЕРАРХИИ(&Счет) И СчетКт В ИЕРАРХИИ(&КоррСчет)";
	...
	Если ПустаяСтрока(ТекстОтбораИтог) Тогда
		
		ТекстОтбораПроводки = ТекстОтбораСчета;
		
	Иначе
		
		ТекстОтбораПроводки = ТекстОтбораИтог+" И "+ТекстОтбораСчета;
		
	КонецЕсли;
	
	ТекстИсточника = "РегистрБухгалтерии."+ДанныеРегистра.НаименованиеРегистра+".ДвиженияССубконто(&ДатаНач, &ДатаКон, "+ТекстОтбораПроводки+") КАК Проводки";
	...
	ТекстЗапросаВнеш = ТекстЗапросаВнеш+"
	|УПОРЯДОЧИТЬ ПО Период, Регистратор"; // @query-part
	
	Возврат ТекстЗапросаВнеш;
	
КонецФункции // ПолучитьТекстЗапроса()

Функция ПолучитьМаксКоличествоСубконто(СчетСсылка)

	Запрос=ТекСоединениеВИБ.NewObject("Запрос");
	Запрос.Текст="ВЫБРАТЬ ПЕРВЫЕ 1
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВидыСубконтоПС.ВидСубконто) КАК КоличествоСубконто
	|ИЗ
	|	ПланСчетов.МСФО.ВидыСубконто КАК ВидыСубконтоПС
	|ГДЕ
	|	ВидыСубконтоПС.Ссылка В ИЕРАРХИИ(&СчетСсылка)
	|
	|СГРУППИРОВАТЬ ПО
	|	ВидыСубконтоПС.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	КоличествоСубконто УБЫВ";
	
	Запрос.Текст=СтрЗаменить(Запрос.Текст,"МСФО",ДанныеРегистра.ПланСчетовРегистр);
	...
	Если Результат.Следующий() Тогда
		...
		Возврат Результат.КоличествоСубконто;
...
КонецФункции // ПолучитьМаксКоличествоСубконто()
...
Функция ПолучитьОтчетПоПроводкам(ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	ОбъектЗапрос.Текст = ПолучитьТекстЗапроса(МаксКоличествоСубконто);
	
	#Если Клиент Тогда
	Состояние(Нстр("ru = 'Выполнение запроса'; en = 'Executing query';tr = 'Sorgu yürütülüyor'"));
	#КонецЕсли
	...
	Пока Выборка.Следующий() Цикл
		...
		КолвоСубконтоДт = ?(Выборка.КолвоСубконтоДт <> NULL, ?(Выборка.КолвоСубконтоДт<=МаксКоличествоСубконто,Выборка.КолвоСубконтоДт,МаксКоличествоСубконто),0);
		КолвоСубконтоКт = ?(Выборка.КолвоСубконтоКт <> NULL, ?(Выборка.КолвоСубконтоКт<=МаксКоличествоСубконто,Выборка.КолвоСубконтоКт,МаксКоличествоСубконто),0);
			...
			Если ПустаяСтрока(СтрРесурс.ПризнакУчета) Тогда
				...
				Если УжеВыведено>0 Тогда
					...
					Иначе
						...
						ОблРесурсБезИзмерений.Параметры.СубконтоДт="";
						ОблРесурсБезИзмерений.Параметры.СубконтоКт="";
		...
		Для Каждого СтрРесурс ИЗ ТаблицаДопИзмерений Цикл
			...
			Если СтрРесурс.ИзмеренияРесурса.Количество()=0 Тогда
				...
				Если (УжеВыведено+1)<=МаксКоличествоСубконто Тогда
			...
			Иначе
				...
				Для Каждого СтрИзмерение ИЗ СтрРесурс.ИзмеренияРесурса Цикл
					...
					Иначе
						...
						ОблРесурсСИзмерением.Параметры.СубконтоДт="";
						ОблРесурсСИзмерением.Параметры.СубконтоКт="";
			...
			Для н=УжеВыведено+1 По ВыводитьСтрокСубконто Цикл
				
				ОблСтрокаСубконто.Параметры.СубконтоДт  = Выборка["СубконтоДт"+н];
				ОблСтрокаСубконто.Параметры.СубконтоКт  = Выборка["СубконтоКт"+н];
...
Функция ПолучитьСчетБД(ТекСчет)
	...
	Иначе
		ТекстОшибки=ТекстОшибки+Символы.ПС+СтрШаблон(НСтр("ru = 'В информационной базе %1 на плане счетов %2 не найден счет с кодом %3'; en = 'No account with code %3 found in the chart of accounts %2 in infobase %1';tr = '%1Infobase''deki %2hesap planında  %3 kodlu hesap bulunamadı'"), 
...
Функция ПолучитьДанныеСчетаПС(КодСчетаПС)
	...
	Иначе
		...
		ТекстОшибки=ТекстОшибки+Символы.ПС+СтрШаблон(НСтр("ru = 'На плане счетов %1 не найден счет с кодом %2'; en = 'Account with code %2 was not found in the chart of accounts %1';tr = '%1Hesap planında %2 kodlu hesap bulunamadı'"), 
...
Функция СформироватьОтчет() Экспорт
	...
	Если ИспользуетсяПараметрСчет() Тогда
		...
		Если ВидОтчета="ОтчетПоПроводкам" Тогда
			...
			Если СчетСсылкаБД=Неопределено И КоррСчетСсылкаБД=Неопределено Тогда
				
				ТекстОшибки=Нстр("ru = 'Не указаны счета для отбора проводок'");
				
			КонецЕсли;
			
		ИначеЕсли СчетСсылкаБД=Неопределено Тогда
			
			ТекстОшибки=Нстр("ru = 'Не указан счет'");
			
		КонецЕсли;
		
		Если Не ПустаяСтрока(ТекстОшибки) Тогда
			
			Возврат Новый Структура("ТаблицаДляОтображения,ТекстОшибки,ЗаголовокОтчета",
	...
	ТаблицаДляОтображения=Неопределено;
	...
	Если ВидОтчета="ОборотноСальдоваяВедомостьПоСчету" Тогда
		...
		Если НЕ ОбъектЗапрос=Неопределено Тогда
			...
			ТаблицаДляОтображения=ПолучитьОСВПоСчету();
	...
	ИначеЕсли ВидОтчета="АнализСчета" Тогда
		...
		Если НЕ ОбъектЗапрос=Неопределено Тогда
			...
			ТаблицаДляОтображения=ПолучитьАнализСчета();
			
		КонецЕсли;
		
	ИначеЕсли ВидОтчета="КарточкаСчета" Тогда
		
		ТаблицаДляОтображения=ПолучитьКарточкуСчета();
		
	ИначеЕсли ВидОтчета="ОборотноСальдоваяВедомость" Тогда
		
		ОбъектЗапрос=НастроитьОбъектЗапрос();
		
		ТаблицаДляОтображения=ПолучитьОборотноСальдовуюВедомость();
	...
	ИначеЕсли ВидОтчета="ОтчетПоПроводкам" Тогда
		...
		ТаблицаДляОтображения=ПолучитьОтчетПоПроводкам();
		
	ИначеЕсли ВидОтчета="ОстаткиИОбороты" Тогда
		
		ТаблицаДляОтображения=ПолучитьОтчетПоРегиструНакопления();
		
	ИначеЕсли ВидОтчета="ПроизвольныйЗапрос" Тогда
		
		ТаблицаДляОтображения= ПолучитьТаблицуПоЗапросу();
	
	КонецЕсли;
	
	Возврат Новый Структура("ТаблицаДляОтображения,ТекстОшибки,ЗаголовокОтчета",
				?(ТаблицаДляОтображения=Неопределено,Новый ТабличныйДокумент,ТаблицаДляОтображения),
				ТекстОшибки,ЗаголовокОтчета()+?(ЗначениеЗаполнено(ДанныеДляЗаголовка.ИспользуемаяИБ),", "+ДанныеДляЗаголовка.ИспользуемаяИБ,""));
...
Функция ПолучитьОтображениеОбъектаБД() Экспорт
	...
	ТипСсылки=ТекСоединениеВИБ.NewObject("ОписаниеТипов", ПараметрыОбъектаДляОтображения.ТипЗначения).Types().Get(0);
	...
	Если ТипМетаданных="НеПоддерживается" Тогда
		...
		Возврат Новый Структура("ТаблицаДляОтображения,НаименованиеФормы",Новый ТабличныйДокумент,"");
		
	КонецЕсли;
	
	ТекСсылка = РаботаСОбъектамиМетаданныхВнешнийУХ.ПолучитьXMLValue(ТекСоединениеВИБ,ТипСсылки, ПараметрыОбъектаДляОтображения.Значение);
	...
	Если ТипМетаданных = "Документ" Тогда
		...
		ВывестиПоКоординатам(Неопределено, ТабличныйДокумент,3, 1, Нстр("ru = 'Номер '; en = 'Number ';tr = 'Numara '"), СоответствиеШиринКолонок, 10, Истина);
		ВывестиПоКоординатам(Неопределено, ТабличныйДокумент,3, 2, ТекСсылка.Номер, СоответствиеШиринКолонок, 10, Истина);
		ВывестиПоКоординатам(Неопределено, ТабличныйДокумент,4, 1, Нстр("ru = ' от '; en = ' dated ';tr = 'kimden'"), СоответствиеШиринКолонок, 10, Истина);
		ВывестиПоКоординатам(Неопределено, ТабличныйДокумент,4, 2, ТекСсылка.Дата, СоответствиеШиринКолонок, 10, Истина);
		
		НаименованиеФормы      = СтрШаблон(Нстр("ru = 'Документ %1 №%2 от %3'; en = 'Document %1 No.%2 dated %3';tr = '%1belge no%2 %3tarihli '"), ТекМетаданные.Синоним, 
		                         	ТекСсылка.Номер, ТекСсылка.Дата); 
		
	Иначе
		
		ВывестиПоКоординатам(Неопределено, ТабличныйДокумент,3, 1, Нстр("ru = 'Код '; en = 'Code';tr = 'Kod '"), СоответствиеШиринКолонок, 10, Истина);
		...
		НаименованиеФормы      = СтрШаблон(Нстр("ru = 'Справочник %1: %2'; en = 'Catalog %1: %2';tr = 'Katalog %1 :%2'"), ТекМетаданные.Синоним, ТекСсылка.Наименование);
		
	КонецЕсли;
	
	ВывестиПоКоординатам(Неопределено, ТабличныйДокумент,6, 1, Нстр("ru = 'Реквизиты:  '; en = 'Attributes:  ';tr = 'Öznitelikler:'"), СоответствиеШиринКолонок, 10, Истина);
	...
	Возврат Новый Структура("ТаблицаДляОтображения,НаименованиеФормы",ТабличныйДокумент,НаименованиеФормы);
...
Процедура ВывестиПоКоординатам(ОписаниеПримитивныхТипов, ТабличныйДокумент, Строка, Столбец, Значение, СоответствиеШиринКолонок = Неопределено, Размер = 8, Жирный = Ложь, Обвести = Ложь, ЦветФона = Неопределено, ЦветРамки = Неопределено, Расшифровка = Неопределено)
	...
	Если ТипЗнч(Значение) = Тип("Булево") Тогда
		Если Значение Тогда
			ТекОбласть.Текст = Нстр("ru = 'да'; en = 'yes';tr = 'evet'");
			ТекОбласть.ЦветТекста = WebЦвета.Зеленый;
		Иначе
			ТекОбласть.Текст = Нстр("ru = 'нет'; en = 'no';tr = 'hayır'");
...
Функция ПолучитьОтчетПоРегиструНакопления()
	...
	Для Каждого СтрПоказатель ИЗ МассивПоказателей Цикл
	 ...
	 Если ТекстПоля="" Тогда
		 ...
		 ТекстОшибки=Нстр("ru = 'Не выбрано ни одного поля отчета.'; en = 'No report fields are selected.';tr = 'Hiçbir rapor alanı seçilmedi.'");
		 Возврат ДокументРезультат;
	...
	Если ОсновнаяТаблицаРегистра Тогда
		...
		Если ЗначениеЗаполнено(ВидДвижения) И ДанныеРегистра.ВидРегистра="Остатки" Тогда
			...
			ТекстОтбораИтог=ТекстОтбораИтог+?(ПустаяСтрока(ТекстОтбораИтог),""," И ")+"ВидДвижения=ЗНАЧЕНИЕ(ВидДвиженияНакопления."+ВидДвижения+")";
			
		КонецЕсли;		
				
		Если НЕ ПустаяСтрока(ТекстОтбораИтог) Тогда
	...
	ИначеЕсли ДанныеРегистра.ВидРегистра="Обороты" Тогда
		...
		ТекстЗапроса=ТекстЗапроса+"
		|ИЗ РегистрНакопления."+ДанныеРегистра.Наименование+".Обороты(&ДатаНач,&ДатаКон,"+?(ПоДокументамДвижения,"Регистратор","")+","+ТекстОтбораИтог + ") КАК "+ДанныеРегистра.Наименование+"Обороты";
		
	Иначе
		
		ТекстЗапроса=ТекстЗапроса+"
		|ИЗ РегистрНакопления."+ДанныеРегистра.Наименование+".ОстаткиИОбороты(&ДатаНач,&ДатаКон,"+?(ПоДокументамДвижения,"Регистратор","")+",,"+ТекстОтбораИтог + ") КАК "+ДанныеРегистра.Наименование+"ОстаткиИОбороты";
	...
	ТекстЗапроса=ТекстЗапроса+"
	| ИТОГИ ТекстИтогиПоказатели ПО ОБЩИЕ"+ТекстИтогиИзмерения;
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТекстИтогиПоказатели", Сред(ТекстИтогиПоказатели,2));
...
//
// Параметры:
//	Выборка - ВыборкаИзРезультатаЗапроса - выборка из результата отчета, которая обходится в процедуре,
...
Процедура ВывестиСтроку(Выборка, СтруктураПараметров, Номер,МассивРасшифровки,СтруктураЗначенийПоказателей)
	...
	ИначеЕсли ТипЗаписиВыборки = ?(ВнешняяИБ=0,QueryRecordType.Overall,ТекСоединениеВИБ.QueryRecordType.Overall) Тогда
		...
		ЗначениеТекущейГруппировки=Нстр("ru = 'ИТОГО:'; en = 'TOTAL:';tr = 'TOPLAM:'");
