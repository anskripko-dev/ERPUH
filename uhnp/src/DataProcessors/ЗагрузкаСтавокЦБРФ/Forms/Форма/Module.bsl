...
&НаКлиенте
Процедура СписокИндикаторовЗагружатьПриИзменении(Элемент)
	УправлениеФормой(ЭтотОбъект);
КонецПроцедуры
...
Процедура ЗаполнитьТекущиеСтавки()
	...
	Запрос.УстановитьПараметр("СписокВидовКотировок", Обработки.ЗагрузкаСтавокЦБРФ.ПереченьСтавок());
...
Процедура ЗагрузитьИЗакрытьНаСервере()
	...
	Обработки.ЗагрузкаСтавокЦБРФ.ЗагрузитьСтавки(Объект.НачалоПериодаЗагрузки, Объект.ОкончаниеПериодаЗагрузки, НужныеСтавки);
