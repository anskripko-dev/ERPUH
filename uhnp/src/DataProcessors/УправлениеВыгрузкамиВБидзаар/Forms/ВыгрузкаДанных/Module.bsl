...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьИнтеграциюСЭТПБидзаарДляПоставщиков") Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(НСтр(
			...
			|en = 'Integration with the Bidzaar electronic trading platform is not activated for vendors. Contact the administrator.'"));
		Возврат;
...
Процедура ПоляФормированияПредставленияПередНачаломИзменения(Элемент, Отказ)
	...
	Если Отказ Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр(
				...
				|en = 'The ""Item"" field is the main one, you cannot disable it'"));
	КонецЕсли;
...
Процедура ДействияПоОтбору(Команда)
	...
	Если Не ЗначениеЗаполнено(УчетнаяЗапись) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Поле ""Учетная запись"" не заполнено';
														|en = 'The ""Account"" field is not filled'"), ,
...
Процедура УдалитьВсеСтроки(Команда)
	...
	ОписаниеВопроса = НСтр("ru = 'Данное действие не влияет на данные, ранее выгруженные на ЭТП ""Бидзаар""';
							|en = 'This action does not affect the data previously exported to the Bidzaar electronic trading platform'");
		...
		Для Каждого КлючЗаписи Из Элементы.ДанныеВыгрузки.ВыделенныеСтроки Цикл
			...
			Иначе
				ОбщегоНазначения.СообщитьПользователю(НСтр(
					...
					|en = 'The data is not up to date. Update the table and try again'"));
				ПараметрыОперации = Неопределено;
				Возврат;
			КонецЕсли;
		КонецЦикла;
...
Процедура ЗаполнитьПоляПредставления()
	...
	Если ТипыНоменклатураЭТП.Количество() = 0 Тогда
		ВызватьИсключение (НСтр("ru = 'Не определен тип Номенклатуры, обратитесь к администратору';
								|en = 'The item type is not defined. Please contact the administrator'"));
	КонецЕсли;
				...
				ИначеЕсли ИмяКоманды = "УдалитьПоОтбору" Тогда
					...
					Исключение
						...
						ТекстИсключения = НСтр("ru = '%1';
												|en = '%1'", КодОсновногоЯзыка);
						ТекстИсключения = СтрШаблон(ТекстИсключения, 
							НСтр("ru = 'УправлениеВыгрузкамиВБидзаар.УдалитьПоОтбору';
								|en = 'УправлениеВыгрузкамиВБидзаар.УдалитьПоОтбору'", КодОсновногоЯзыка));
...
Процедура ВыполнитьЗапросЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если СтруктураРезультата.Свойство("КоличествоОпубликованныхПозиций", КоличествоОпубликованныхПозиций) Тогда
		...
		Иначе
			...
			"ru = 'На ЭТП ""Бидзаар"" успешно опубликовано позиций: %1';
			|en = 'Positions successfully published on the Bidzaar electronic trading platform: %1'"), Строка(
...
Процедура ПослеОтветаВыгрузкаДанных(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли РезультатВопроса = 2 И Не ЗначениеЗаполнено(Элементы.ДанныеВыгрузки.ВыделенныеСтроки) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Нет выделенных строк для выгрузки';
														|en = 'There are no selected lines to export'"));
		Возврат;
	КонецЕсли;
...
Процедура УдалитьВсеСтрокиНаСервере()
	...
	Исключение
		...
		ТекстИсключения = НСтр("ru = '%1';
								|en = '%1'", КодОсновногоЯзыка);
		ТекстИсключения = СтрШаблон(ТекстИсключения, 
			НСтр("ru = 'УправлениеВыгрузкамиВБидзаар.ДанныеВыгрузкиПередУдалением';
				|en = 'УправлениеВыгрузкамиВБидзаар.ДанныеВыгрузкиПередУдалением'", КодОсновногоЯзыка));
...
Процедура УстановитьУсловноеОформление()
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПросмотрИРедактированиеДанныхВыгрузки Тогда
		Заголовок = НСтр("ru = 'Выгрузка товаров и услуг в Бидзаар';
						|en = 'Export goods and services to Bidzaar'");
	Иначе
		Заголовок = НСтр("ru = 'Настройки выгрузки в Бидзаар';
						|en = 'Settings of export to Bidzaar'");
	КонецЕсли;
...
Процедура ВыгрузитьВExcelЗавершениеПодключенияРасширения(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РасширениеРаботыСФайламиПодключено Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Расширение работы с файлами не подключено.';
													|en = '1C:Enterprise Extension is not attached.'"));
		Возврат;
	КонецЕсли;
...
Функция ПоместитьДанныеВExcelНаСервере(ВариантВыгрузки, Отказ)
	...
	Исключение
		...
		ТекстИсключения = НСтр("ru = '%1';
								|en = '%1'", КодОсновногоЯзыка);
		ТекстИсключения = СтрШаблон(ТекстИсключения, 
			НСтр("ru = 'УправлениеВыгрузкамиВБидзаар.ВыгрузкаВExcel''';
				|en = 'УправлениеВыгрузкамиВБидзаар.ВыгрузкаВExcel'''", КодОсновногоЯзыка));
