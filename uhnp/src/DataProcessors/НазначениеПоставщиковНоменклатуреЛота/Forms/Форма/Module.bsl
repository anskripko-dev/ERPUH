...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Лот = Неопределено Тогда
		Возврат;
	КонецЕсли;	
...
Процедура УстановитьДоступностьЗаписи()
	...
	//условноеоформление.Элементы.НоменклатураПоПоставщикам.
КонецПроцедуры	
...
Процедура ЗаписатьНазначенныхНоменклатуреПоставщиков()
	...
	Если НЕ ЗначениеЗаполнено(Лот) 
			ИЛИ ПроверитьСообщитьНеВыбраныПоставщикиПоСтрокам() 
			Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыФормыИзПараметров(Форма, Параметры)
	...
	КонецЕсли;
	Форма.ЭтоФЗ223 = Справочники.Лоты.ЭтоФЗ223(Форма.Лот);
	Форма.НациональныйРежим = Форма.Лот.ЗакупочнаяПроцедура.НациональныйРежимЗакупок; 
...
Процедура НастроитьКолонкиКроссТаблицы()
	...
	ПараметрыВывода.СвойстваГруппыПериода.Вставить("Группировка", ГруппировкаКолонок.Вертикальная);
	...
	Если ЭтоФЗ223 Или ПолучитьФункциональнуюОпцию("ВестиУчетПоСтранамКорпоративныеЗакупки") Тогда
		...
		ДобавитьПолеКроссТаблицыПреференции(ПараметрыВывода, Истина);  
		ДобавитьПолеКроссТаблицыОграничениеЗакупки(ПараметрыВывода, Ложь); 
		ДобавитьПолеКроссТаблицыНациональныйРежим(ПараметрыВывода, Ложь);
	КонецЕсли;       
	...
	АктивныеПериоды = ПредложенияПоставщиков.Выгрузить(Новый Структура("Активная", Истина));
	Для Каждого АктивныйПериод Из АктивныеПериоды Цикл   
		ИмяКолонки = АктивныйПериод.ИмяКолонки;
		Если Элементы.Найти("ГруппаТаблицыСтрока1_"+ИмяКолонки) = Неопределено Тогда           
			ГруппаПредложения = Элементы.Найти("НоменклатураПоПоставщикамГруппа" + ИмяКолонки);  
			//1
			ГруппаТаблицыСтрока1 = Элементы.Добавить("ГруппаТаблицыСтрока1_"+ИмяКолонки, Тип("ГруппаФормы"), ГруппаПредложения);  
			ГруппаТаблицыСтрока1.Группировка = ГруппировкаКолонок.ВЯчейке;           
			Элементы.Переместить(Элементы.Найти("НоменклатураПоПоставщикамВыбор_"+ИмяКолонки), 		ГруппаТаблицыСтрока1); 
			Элементы.Переместить(Элементы.Найти("НоменклатураПоПоставщикамСуммаВыбрано_"+ИмяКолонки), ГруппаТаблицыСтрока1);  
			Элементы.Переместить(Элементы.Найти("НоменклатураПоПоставщикамСтавкаНДС_"+ИмяКолонки), 	ГруппаТаблицыСтрока1);
			Элементы.Переместить(Элементы.Найти("НоменклатураПоПоставщикамКоличество_"+ИмяКолонки), ГруппаТаблицыСтрока1);
			//2                                   
			Если ЭтоФЗ223 Или ПолучитьФункциональнуюОпцию("ВестиУчетПоСтранамКорпоративныеЗакупки") Тогда
				ГруппаТаблицыСтрока2 = Элементы.Добавить("ГруппаТаблицыСтрока2_"+ИмяКолонки, Тип("ГруппаФормы"), ГруппаПредложения);
				ГруппаТаблицыСтрока2.Группировка = ГруппировкаКолонок.ВЯчейке;  
				Элементы.Переместить(Элементы.Найти("НоменклатураПоПоставщикамСтранаПроизводителя_"+ИмяКолонки),ГруппаТаблицыСтрока2); 
				Элементы.Переместить(Элементы.Найти("НоменклатураПоПоставщикамСтранаПроисхождения_"+ИмяКолонки),ГруппаТаблицыСтрока2);  
				Элементы.Переместить(Элементы.Найти("НоменклатураПоПоставщикамПреференции_"+ИмяКолонки), 		ГруппаТаблицыСтрока2);
			КонецЕсли;  
			//3
			Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Лот, "ВидЗакупки") = Перечисления.ВидЛотовойЗакупки.Зонтичная Тогда
				ГруппаТаблицыСтрока3 = Элементы.Добавить("ГруппаТаблицыСтрока3_"+ИмяКолонки, Тип("ГруппаФормы"), ГруппаПредложения);
				ГруппаТаблицыСтрока3.Группировка = ГруппировкаКолонок.ВЯчейке;   
				Элементы.Переместить(Элементы.Найти("НоменклатураПоПоставщикамКоличествоПоПобедителю_"+ИмяКолонки),	ГруппаТаблицыСтрока3);  
				Элементы.Переместить(Элементы.Найти("НоменклатураПоПоставщикамМестоПоПобедителю_"+ИмяКолонки), 		ГруппаТаблицыСтрока3);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура УстановитьУсловноеОформлениеКроссТаблицы()
	...
	Для каждого Колонка_ Из АктивныеКолонки Цикл
		...
		//НоменклатураПоПоставщикамСуммаВыбор_
		Элемент = УсловноеОформление.Элементы.Добавить();
		ЦентрализованныеЗакупкиУХ.ДобавитьПолеУО(
			Элемент, 
			"НоменклатураПоПоставщикамВыбор_"+Колонка_.ИмяКолонки);
		ЦентрализованныеЗакупкиУХ.ДобавитьОтборЭлементаУО(Элемент,
			Новый ПолеКомпоновкиДанных("НоменклатураПоПоставщикам.ОграничениеЗакупки_"+Колонка_.ИмяКолонки), 
			Истина);
		Элемент.Оформление.УстановитьЗначениеПараметра(
...
Процедура ПрочитатьДанныеПредложенийПоставщиков()
	...
	|	ЛотыНоменклатура.Ссылка.ОрганизацияДляЗаключенияДоговора КАК Организация,
	...
	|	УсловияПредложенийПоставщиков.Цена  КАК Цена,   //* &КоэффициентПересчетаВалюты / &Кратность
	|	ВЫБОР
	|		КОГДА УсловияПредложенийПоставщиков.Номенклатура ССЫЛКА Справочник.Номенклатура
	|			ТОГДА ЛотыНоменклатура.Количество * УсловияПредложенийПоставщиков.Цена
	|		ИНАЧЕ УсловияПредложенийПоставщиков.Сумма
	|	КОНЕЦ  КАК Сумма, //* &КоэффициентПересчетаВалюты / &Кратность
	|	УсловияПредложенийПоставщиков.СтавкаНДС КАК СтавкаНДС,
	...
	|	УсловияПредложенийПоставщиков.ЦенаПервоначальная КАК ЦенаПервоначальная,
	|	УсловияПредложенийПоставщиков.ЦФО КАК ЦФО	
	|ИЗ
	...
	|			И (ЛотыНоменклатура.Цена = УсловияПредложенийПоставщиков.ЦенаПервоначальная
	|           ИЛИ УсловияПредложенийПоставщиков.ЦенаПервоначальная = 0)
	...
	Если Лот.ЗакупочнаяПроцедура.НациональныйРежимЗакупок = Истина Тогда
		РезультатыСПреференциями = ДанныеПреференцийЛота();	
		ПреференцииКодовПоНоменклатуре = РегистрыСведений.ПараметрыПреференцийОКПД2.ПреференцииКодовПоНоменклатуре(Лот.Дата, Объект.РаспределениеПоПотребностям.Выгрузить(, "Номенклатура")); 
	Иначе
		РезультатыСПреференциями = Неопределено;
	КонецЕсли;  
	...
	Для Каждого ТекСтрока Из Объект.РаспределениеПоПотребностям Цикл 
		...
		Если РезультатыСПреференциями <> Неопределено Тогда        
			
			СтруктураПоиска = Новый Структура("Номенклатура, Характеристика, ЕдиницаИзмерения, ПредложениеПоставщика, МестоПоставки");  
			ЗаполнитьЗначенияСвойств(СтруктураПоиска, ТекСтрока);
			СтрокиПараметров = РезультатыСПреференциями.НайтиСтроки(СтруктураПоиска);  
			ТекСтрока.Преференции = 0;  
			ДанныеПреференций = ПреференцииКодовПоНоменклатуре.Получить(ТекСтрока.Номенклатура); 
			ТекСтрока.НациональныйРежим = ДанныеПреференций.НациональныйРежимЗакупок; 
			
			Если СтрокиПараметров.Количество() Тогда
				КоэффициентПреференции 	= СтрокиПараметров[0].КоэффициентПреференции;  
				Если КоэффициентПреференции = 1 Тогда
					ТекСтрока.Преференции = 0;
				Иначе
					ТекСтрока.Преференции = СтрокиПараметров[0].Преференция; 
				КонецЕсли;    
				
				Если ТекСтрока.НациональныйРежим = Перечисления.НациональныйРежимЗакупок.Ограничение И СтрокиПараметров[0].УчастникЕАЭС Тогда
					ТекСтрока.НациональныйРежим = Перечисления.НациональныйРежимЗакупок.ПустаяСсылка();
				КонецЕсли;      
				
				Если ТекСтрока.НациональныйРежим = Перечисления.НациональныйРежимЗакупок.Запрет Тогда
					ТекСтрока.ОграничениеЗакупки = Истина И Не СтрокиПараметров[0].УчастникЕАЭС;
				КонецЕсли;
				
				ТекСтрока.Цена = ТекСтрока.Цена * КоэффициентПреференции;  
				Если ТипЗнч(ТекСтрока.Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
					ТекСтрока.Сумма = ТекСтрока.Сумма * КоэффициентПреференции;   
				КонецЕсли; 
			Иначе
				ТекСтрока.ОграничениеЗакупки = Истина;
			КонецЕсли;
			
		Иначе    
			
			КоэффициентПреференции = 1;
		...
		Если Лот.ОцениватьПредложенияБезНДС Тогда
...
Процедура ОбновитьНазначенныхНоменклатуреПоставщиков()
	ПоляГруппировки = "Номенклатура,Характеристика,ЦФО,ЦенаПервоначальная,Организация,ДоговорСПокупателем,Менеджер,МестоПоставки,Приоритет,Проект";
	
	ТаблицаНомеклатурыПоставщиков = Объект.РаспределениеПоПотребностям.Выгрузить();
	ТаблицаНомеклатурыПоставщиков.Свернуть(ПоляГруппировки + ",ОграничениеЗакупки,НациональныйРежим,ЦенаПервоначальная,ТоварНеПоставляется,МестоПобедителя,ПредложениеПоставщика,Поставщик,СтавкаНДС,ДатаПредложения,СтраныПроисхождения,СтранаПроизводителя,НоменклатураПоставщика, Преференции","Сумма,Количество");
	...
	Для Каждого ТекСтрокаНоменклатурыПоставщиков ИЗ ТаблицаНомеклатурыПоставщиков Цикл
		...
		КонецЕсли;
		Если Элементы.Найти("НоменклатураПоПоставщикамГруппаНомПоставщика"+ИмяКолонки) = Неопределено Тогда             
			НовыйЭлемент = Элементы.Добавить("НоменклатураПоПоставщикамГруппаНомПоставщика"+ИмяКолонки,Тип("ГруппаФормы"),Элементы["НоменклатураПоПоставщикам"]);
			Элементы.Переместить(Элементы["НоменклатураПоПоставщикамГруппа" + ИмяКолонки],элементы["НоменклатураПоПоставщикамГруппаНомПоставщика"+ИмяКолонки]);	
		...
		Если ЭтоФЗ223 Или ПолучитьФункциональнуюОпцию("ВестиУчетПоСтранамКорпоративныеЗакупки") Тогда
			...
			НоваяСтрока["ОграничениеЗакупки_" + ИмяКолонки] = ТекСтрокаНоменклатурыПоставщиков.ОграничениеЗакупки;
			НоваяСтрока["НациональныйРежим_" + ИмяКолонки] = ТекСтрокаНоменклатурыПоставщиков.НациональныйРежим;  
			
			Если ТекСтрокаНоменклатурыПоставщиков.НациональныйРежим = Перечисления.НациональныйРежимЗакупок.Преференции Тогда
				НоваяСтрока["Преференции_" + ИмяКолонки] = СтрШаблон(НСтр("ru = 'Преференции = %1'"), ТекСтрокаНоменклатурыПоставщиков.Преференции) + "%"; 
			ИначеЕсли ТекСтрокаНоменклатурыПоставщиков.НациональныйРежим = Перечисления.НациональныйРежимЗакупок.Ограничение Тогда     
				Если ТекСтрокаНоменклатурыПоставщиков.ОграничениеЗакупки Тогда
					НоваяСтрока["Преференции_" + ИмяКолонки] = СтрШаблон(НСтр("ru = 'Ограничение закупки'"));    
				Иначе
					НоваяСтрока["Преференции_" + ИмяКолонки] = СтрШаблон(НСтр("ru = 'Ограничение снято'"));
				КонецЕсли; 
			ИначеЕсли ТекСтрокаНоменклатурыПоставщиков.НациональныйРежим = Перечисления.НациональныйРежимЗакупок.Запрет Тогда     
				Если ТекСтрокаНоменклатурыПоставщиков.ОграничениеЗакупки Тогда
					НоваяСтрока["Преференции_" + ИмяКолонки] = СтрШаблон(НСтр("ru = 'Запрет'"));    
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СтрокаНоменклатуры Из НоменклатураПоПоставщикам Цикл
		Для Каждого  ТекСтрокаНоменклатурыПоставщиков из ТаблицаНомеклатурыПоставщиков Цикл
			...
			Если СтрокаНоменклатуры["Сумма_"+ИмяКолонки]=0 Тогда
				...
				ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных("НоменклатураПоПоставщикамВыбор_"+ИмяКолонки);
				
				ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("НоменклатураПоПоставщикамСуммаВыбрано_"+ИмяКолонки);
...
Процедура РассчитатьИтогПоВыбраннымВКолонке(Форма, ИмяКолонки)
	...
	Форма.Элементы["НоменклатураПоПоставщикамСуммаВыбрано_" + ИмяКолонки].ТекстПодвала = ИтогоСуммаПоКолонке; 
    Форма.Элементы["НоменклатураПоПоставщикамСуммаВыбрано_" + ИмяКолонки].ГоризонтальноеПоложениеВПодвале = ГоризонтальноеПоложениеЭлемента.Лево;
КонецПроцедуры
...
Функция ПолучитьИтоговуюТаблицуВыбранныхПоставщиков()
	...
	СтруктураПоиска = Новый Структура("Номенклатура,ЦФО,ЦенаПервоначальная,Организация,ДоговорСПокупателем,Менеджер,МестоПоставки,Приоритет,Проект,Поставщик,ПредложениеПоставщика");
...
Процедура УстановитьОформлениеФормы()
	...
	Элементы.НоменклатураПоПоставщикамВыбратьПоПредложениюСМинимальнойОбщейСуммой.Видимость =
		 НЕ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Лот, "ВидЗакупки") = Перечисления.ВидЛотовойЗакупки.Зонтичная;
...
Процедура ОбработатьВыборПредложенияПоставщикаПоСтроке(
	...
	Если НЕ РазрешенаЧастичнаяПоставка Тогда
...
Процедура УстановитьВыборПоМассивуЯчеек(Форма, ЯчейкиДляУстановки, ЗначениеФлагаВыбора=Истина)
	...
	Для Каждого Пара Из ЯчейкиДляУстановки Цикл
		Если Пара.СтрокаТаблицы["Сумма_"+Пара.КодКолонки] > 0 Тогда 
			Если Форма.НациональныйРежим Тогда
				Если Пара.СтрокаТаблицы["ОграничениеЗакупки_"+Пара.КодКолонки] Тогда 
					Пара.СтрокаТаблицы["Выбор_"+Пара.КодКолонки] = Ложь;
				Иначе
...
&НаСервере
Функция ДанныеПреференцийЛота()
	
	Перем ВариантОтчетаИмя, ДанныеРасшифровкиСКД, ДеревоОбъект, ЗаголовокДокумента, ИмяПараметра, КомпоновщикМакета, КэшИменПараметровМакетов, МакетКомпоновки, МассивИменПараметров, Настройки, НастройкиВывода, ОтчетОбъект, ПростаяОценка, ПроцессорВывода, ПроцессорКомпоновкиДанных, СхемаКомпоновкиДанных, ШрифтТипаКритерия, ЭлементРезультата;
	
	СхемаКомпоновкиДанных = Обработки.НазначениеПоставщиковНоменклатуреЛота.ПолучитьМакет("РасчетПреференцийПотребостей");
	...
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Лот");
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ЭлементОтбора.Использование = Истина;     
	ЭлементОтбора.ПравоеЗначение = ЭтотОбъект.Лот;
	//
...
&НаСервереБезКонтекста
// Добавляет в настройки параметров вывода поле кросс-таблицы "Преференции"
//
// Параметры:
//  ПараметрыВывода				 - Структура	 - Параметры вывода полей кросс-таблицы
//  СоздаватьЭлемент			 - Булево		 - Флаг определяет, создавать ли элемент формы
//  СтруктураДействий			 - Структура	 - Структура действий элемента, Ключ - имя события, значение - имя процедуры обработчика события
Процедура ДобавитьПолеКроссТаблицыПреференции(ПараметрыВывода,
										СоздаватьЭлемент = Истина, 
										СтруктураДействий = Неопределено) Экспорт 
	Если НЕ ПараметрыВывода.Свойство("Поля") Тогда
		ПараметрыВывода.Вставить("Поля", Новый Массив());
	КонецЕсли;
	ПараметрыПоля = Новый Структура;
	ПараметрыПоля.Вставить("ПрефиксРеквизитаКолонки", 	"Преференции_");
	ПараметрыПоля.Вставить("УдалятьРеквизитыТаблицы", 	Истина);
	ПараметрыПоля.Вставить("СоздаватьЭлемент", 			СоздаватьЭлемент);
	ПараметрыПоля.Вставить("ТипЭлемента", 				ОбщегоНазначенияУХ.ПолучитьОписаниеТиповСтроки(150));
	ПараметрыПоля.Вставить("Заголовок", 				НСтр("ru = 'Преференции'"));
	ПараметрыПоля.Вставить("СоздаватьИтоговыеРеквизиты", Ложь);
	ПараметрыПоля.Вставить("СвойстваЭлемента", 			Новый Структура());
	ПараметрыПоля.СвойстваЭлемента.Вставить("Вид", 		ВидПоляФормы.ПолеНадписи);
	ПараметрыПоля.СвойстваЭлемента.Вставить("ОтображатьВШапке", 		Истина);
...
&НаСервереБезКонтекста
// Добавляет в настройки параметров вывода поле кросс-таблицы "ОграничениеЗакупки"
//
// Параметры:
//  ПараметрыВывода				 - Структура	 - Параметры вывода полей кросс-таблицы
//  СоздаватьЭлемент			 - Булево		 - Флаг определяет, создавать ли элемент формы
//  СтруктураДействий			 - Структура	 - Структура действий элемента, Ключ - имя события, значение - имя процедуры обработчика события
Процедура ДобавитьПолеКроссТаблицыОграничениеЗакупки(ПараметрыВывода,
										СоздаватьЭлемент = Истина, 
										СтруктураДействий = Неопределено) Экспорт 
	Если НЕ ПараметрыВывода.Свойство("Поля") Тогда
		ПараметрыВывода.Вставить("Поля", Новый Массив());
	КонецЕсли;
	ПараметрыПоля = Новый Структура;
	ПараметрыПоля.Вставить("ПрефиксРеквизитаКолонки", 	"ОграничениеЗакупки_");
	ПараметрыПоля.Вставить("УдалятьРеквизитыТаблицы", 	Истина);
	ПараметрыПоля.Вставить("СоздаватьЭлемент", 			Ложь);
	ПараметрыПоля.Вставить("ТипЭлемента", 				ОбщегоНазначенияУХ.ПолучитьОписаниеТиповБулево());
	ПараметрыПоля.Вставить("Заголовок", 				НСтр("ru = 'Ограничение закупки'"));
	ПараметрыПоля.Вставить("СоздаватьИтоговыеРеквизиты", Ложь);
	ПараметрыПоля.Вставить("СвойстваЭлемента", 			Новый Структура());
	ПараметрыПоля.СвойстваЭлемента.Вставить("Вид", 		ВидПоляФормы.ПолеНадписи);
	ПараметрыПоля.СвойстваЭлемента.Вставить("ОтображатьВШапке", 		Истина);
...
&НаСервереБезКонтекста
// Добавляет в настройки параметров вывода поле кросс-таблицы "НациональныйРежим"
//
// Параметры:
//  ПараметрыВывода				 - Структура	 - Параметры вывода полей кросс-таблицы
//  СоздаватьЭлемент			 - Булево		 - Флаг определяет, создавать ли элемент формы
//  СтруктураДействий			 - Структура	 - Структура действий элемента, Ключ - имя события, значение - имя процедуры обработчика события
Процедура ДобавитьПолеКроссТаблицыНациональныйРежим(ПараметрыВывода,
										СоздаватьЭлемент = Истина, 
										СтруктураДействий = Неопределено) Экспорт 
	Если НЕ ПараметрыВывода.Свойство("Поля") Тогда
		ПараметрыВывода.Вставить("Поля", Новый Массив());
	КонецЕсли;
	ПараметрыПоля = Новый Структура;
	ПараметрыПоля.Вставить("ПрефиксРеквизитаКолонки", 	"НациональныйРежим_");
	ПараметрыПоля.Вставить("УдалятьРеквизитыТаблицы", 	Истина);
	ПараметрыПоля.Вставить("СоздаватьЭлемент", 			СоздаватьЭлемент);
	ПараметрыПоля.Вставить("ТипЭлемента", 				Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.МассивЗначений(Тип("ПеречислениеСсылка.НациональныйРежимЗакупок"))));
	ПараметрыПоля.Вставить("Заголовок", 				НСтр("ru = 'Национальный режим'"));
	ПараметрыПоля.Вставить("СоздаватьИтоговыеРеквизиты", Ложь);
	ПараметрыПоля.Вставить("СвойстваЭлемента", 			Новый Структура());
	ПараметрыПоля.СвойстваЭлемента.Вставить("Вид", 		ВидПоляФормы.ПолеНадписи);
	ПараметрыПоля.СвойстваЭлемента.Вставить("ОтображатьВШапке", 		Истина);
