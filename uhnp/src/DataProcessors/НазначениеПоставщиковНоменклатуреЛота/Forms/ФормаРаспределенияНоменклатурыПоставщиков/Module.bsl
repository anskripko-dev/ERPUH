...
Процедура НастроитьФормуКроссТаблицы()
	...
	Если ЭтоНоменклатура Тогда
		Победители.Свернуть("ПредложениеПоставщика,Номенклатура,МестоПоставки,Поставщик,Характеристика,Организация,Менеджер,Цена,ЦенаПервоначальная,Назначение,ЦФО,СтавкаНДС","Количество,Сумма,СуммаНДС,СуммаСНДС");
	Иначе
		Победители.Свернуть("ПредложениеПоставщика,Номенклатура,МестоПоставки,Характеристика,Поставщик,Организация,Менеджер,Назначение,ЦФО,СтавкаНДС,","Количество,Сумма,СуммаНДС,СуммаСНДС");
	КонецЕсли;	
...
Процедура ОбработчикИзмененияКоличества(Элемент)
	...
	СуммаНДС = УчетНДСКлиентСервер.РассчитатьСуммуНДС(Сумма,ЦенаВключаетНДС,ОбщегоНазначенияУХ.ПолучитьЗначениеРеквизита(строкаактивации.СтавкаНДС,"Ставка"));
...
Процедура ОбработчикИзмененияСуммы(Элемент)
	...
	СуммаНДС = УчетНДСКлиентСервер.РассчитатьСуммуНДС(Сумма,ЦенаВключаетНДС,ВстраиваниеУХВызовСервера.ПолучитьСтавкуНДС(СтрокаАктивации.СтавкаНДС));
