...
Процедура НачатьПроверкуНастроекРегламентныхЗаданий()
	...
	ПроверкаНастроек = СинхронизацияЭДО.НачатьПроверкуНастроекРегламентныхЗаданий(
		УникальныйИдентификатор, Операция);
...
Процедура ОтобразитьРезультатПроверки()
	...
	Заголовки.Вставить(Операции.ПолучениеПоТокенам, НСтр("ru = 'Автоматическое получение документов';
														|en = 'Automatic receipt of documents'"));
	...
	ИначеЕсли Операция = Операции.ПолучениеПоТокенам Тогда
		ЗаголовокПодсказка = НСтр("ru = 'Для работы с электронными документами в автоматическом режиме требуется дополнительная настройка.';
									|en = 'Для работы с электронными документами в автоматическом режиме требуется дополнительная настройка.'");
	Иначе
	...
	Если Операция = Операции.ПолучениеПоТокенам Тогда
		...
		Элементы.ПарольСертификата.Видимость = Ложь;
		Элементы.КриптоПровайдерПодпись.Видимость = Ложь;
