		...
		Для каждого ВариантОбеспечения Из ПорядокВыборкиВариантовОбеспечения Цикл
			...
			Для каждого СтрокаМатериал Из СписокСтрок Цикл
				...
				Если СтрокаМатериал.Количество > Дефицит Тогда
					...
					ДобавленныеИИзмененныеСтроки.Добавить(СтрокаМатериал);
					ДобавленныеИИзмененныеСтроки.Добавить(СтрокаДляКорректировки);
				Иначе
...
&НаСервере
Функция ОтгрузитьСтрокуСверхЗаказа(ТекущиеДанные, Количество, УпаковкаПоУмолчанию, ДанныеПоМатериаламКопия)
