...
Функция ЗапуститьЗаданиеОбновитьДанные(СтруктураДействий)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение данных для диспетчирования производства';
...
Процедура УправлениеДоступностьюКнопокМасштабаДиаграммы()
	...
	Если ОтборПериод = "Текущий" ИЛИ  ОтборПериод = "Следующий" Тогда
		...
		Если ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Час
			ИЛИ ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.День
			ИЛИ ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Смена Тогда
...
Функция ЗапуститьФоновыеЗаданияНаСервере(Знач ПараметрыЗаданий)
	...
	Для каждого ДанныеЗадания Из ТекущиеФоновыеЗадания Цикл
		...
		ДанныеЗадания.РезультатЗапуска = РезультатЗапускаЗадания;
		
		Если НЕ РезультатЗапускаЗадания.Статус = "Выполнено" Тогда
...
Процедура ОткрытьФормуДлительнойОперации()
	...
	Для Каждого ФоновоеЗадание Из ТекущиеФоновыеЗадания Цикл
		...
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбработатьЗавершениеЗаданийОповещение", ЭтотОбъект);
	
		ДлительныеОперацииКлиент.ОжидатьЗавершение(
			ФоновоеЗадание.РезультатЗапуска, ОповещениеОЗавершении, ПараметрыОжидания);
	КонецЦикла;

КонецПроцедуры

&НаКлиенте
Процедура ОбработатьЗавершениеЗаданийОповещение(Результат, Дополнительныепараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Для Каждого ФоновоеЗадание Из ТекущиеФоновыеЗадания Цикл
			Если ФоновоеЗадание.ИдентификаторЗадания = Результат.ИдентификаторЗадания Тогда
				ФоновоеЗадание.РезультатЗапуска.Статус = "Выполнено";
			КонецЕсли;
			
			Если ФоновоеЗадание.РезультатЗапуска.Статус <> "Выполнено" Тогда
...
//		НаименованиеЗадания);
	...
	ПараметрыВыполнения.ОжидатьЗавершение = 0;
	ПараметрыВыполнения.ЗапуститьВФоне = Истина;
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Формирование маршрутных листов';
