...
Процедура Подключаемый_ВыполнитьСохранениеМодели(РасширениеПодключено, ДополнительныеПараметры) Экспорт
	...
	Если РасширениеПодключено = Истина Тогда
		...
		ДиалогВыбораФайла.Фильтр = НСтр("ru = 'Файлы Управления Холдингом'") + " (*.hm)|*.hm|" + НСтр("ru = 'Все файлы'") + " (*.*)|*.*";
...
Функция ПолучитьИмяФайлаМодели(Дата = Неопределено)
	...
	СтрокаШаблона = Нстр("ru = 'Модель ОСВ от %1.hm'");
	
	Если Не ПустаяСтрока(СтрокаШаблона) тогда							
		Возврат СтрШаблон(СтрокаШаблона, Формат(ДатаИмени, "ДФ='yyyy.MM.dd HH.mm.ss'"));
	Иначе
		Возврат Формат(ДатаИмени, "ДФ='yyyy.MM.dd HH.mm.ss'") + ".hm";
	КонецЕсли;
...
Процедура ОткрытьНастройкиИзФайла(Команда)
	...
	ДиалогВыбораФайла.Вставить("Фильтр", НСтр("ru = 'Файлы Управления Холдингом'") + " (*.hm)|*.hm|" + НСтр("ru = 'Все файлы") + " (*.*)|*.*");	
		...
		Иначе
			...
			Если НЕ (Блокировка = Неопределено ИЛИ Блокировка = 0) Тогда
				...
				Если Блокировка = 1 Тогда
					СтрокаШаблона = Нстр("ru = 'Показатель вида отчета %1 используется в формуле формирования ОСВ'");
				Иначе
					СтрокаШаблона = Нстр("ru = 'Показатели вида отчета %1 используются в формуле формирования ОСВ'");
				КонецЕсли;
				
				Если Не ПустаяСтрока(СтрокаШаблона) тогда							
					Состояние(СтрШаблон(СтрокаШаблона, ТекущиеДанные.Ссылка),,, БиблиотекаКартинок.Предупреждение32);
				Иначе
					Состояние(ТекущиеДанные.Ссылка,,, БиблиотекаКартинок.Предупреждение32);
				КонецЕсли;
...
Процедура Создать()
	...
	Если Не ПустаяСтрока(СтрокаШаблона) Тогда
		ПравилоПроверкиОбъект.Наименование = СтрШаблон(СтрокаШаблона, ВидОтчетаНаименование);
	Иначе
		ПравилоПроверкиОбъект.Наименование = ВидОтчетаНаименование;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(СтрокаШаблона) Тогда
		ПравилоОбработкиОбъект.Наименование = СтрШаблон(СтрокаШаблона, ВидОтчетаНаименование);
	Иначе
		ПравилоОбработкиОбъект.Наименование = ВидОтчетаНаименование;
	КонецЕсли;
	...
	Для Каждого ФункцияПоказателя Из ФункцииПоказателей() Цикл
		...
		СтрокаШаблона = Нстр("ru = 'Колонка по итогу %1'");
		
		Если Не ПустаяСтрока(СтрокаШаблона) Тогда
			ИмяКолонки = СтрШаблон(СтрокаШаблона, ФункцияПоказателя);
		Иначе
			ИмяКолонки = ФункцияПоказателя;
		КонецЕсли;
		
		Колонка = СоздатьКолонку(ВидОтчетаСсылка, ИмяКолонки, НомерКолонки, Перечисления.ВидыБухгалтерскихИтогов[ФункцияПоказателя]);
...
Функция СоздатьПоказатели(ЭлементыОСВ, ФункцияПоказателя, ВидОтчета, ПравилоОбработки, ГруппыРаскрытия, Колонка, Строки, СтрокаРодитель = Неопределено)
	...
	Для Каждого Элемент Из ЭлементыОСВ Цикл
		...
		Если ГруппаРаскрытия = Неопределено Тогда
			...
			СтрокаШаблона = Нстр("ru = 'Группа аналитик по счету %1'");
			
			Если Не ПустаяСтрока(СтрокаШаблона) тогда							
				ИмяГруппы = СтрШаблон(СтрокаШаблона, Элемент.Имя);
			Иначе
				ИмяГруппы = Элемент.Имя;
			КонецЕсли;
			
			ГруппаРаскрытия = СоздатьГруппуРаскрытия(ВидОтчета, Элемент.Ссылка, ИмяГруппы);
...
Функция СоздатьИсточник(ПравилоОбработки, Показатель, Потребитель, СтрокаФормулы = Неопределено)
	...
	Если НЕ СтрокаФормулы = Неопределено Тогда
		...
		Для НомерАналитики = 1 По СтрокаФормулы.АналитикиКоличество Цикл
			НомерАналитикиСтрока = Формат(НомерАналитики, "ЧГ=");
