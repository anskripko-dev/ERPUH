...
Функция ПроверитьСоответствиеТипаИБ(ПравилоИсточник,ПравилоПриемник)
	...
	Если ЗначениеЗаполнено(ТипИБИсточник) И (НЕ ТипИБИсточник=Справочники.ТипыБазДанных.ТекущаяИБ) Тогда
		...
		Если НЕ ТипИБИсточник=ТипИБПриемник Тогда
			...
			СтрокаШаблона = Нстр("ru = 'Не совпадают типы информационных баз, используемых в источнике и приемнике.
			|Источник: %1,
			|приемник: %2'");
			
			ТекстОшибки = "";
			Если Не ПустаяСтрока(СтрокаШаблона) тогда
				ТекстОшибки = СтрШаблон(СтрокаШаблона, ТипИБИсточник, ТипИБПриемник);
			КонецЕсли;
...
Процедура СоздатьНовыйПоказатель(Копирование=Ложь)
	...
	Если (ЗначениеЗаполнено(Объект.СтрокаПриемник) И ЗначениеЗаполнено(Объект.СтрокаПриемник.ГруппаРаскрытия)) 
		...
		Сообщить(Нстр("ru = 'Не совпадают аналитики показателя - источника и строки - приемника.'"));
		Возврат;
	...
	Если АналитикВидаОтчетаИсточник>0 ИЛИ АналитикВидаОтчетаПриемник>0 
		...
		Сообщить(Нстр("ru = 'Не совпадают аналитики вида отчета - источника и вида отчета - приемника.'"));
		Возврат;
	...
	Иначе
		...
		Исключение
			...
			Если Копирование тогда			                                                                    
				СтрокаШаблона = Нстр("ru = 'Ошибка при копировании показателя: %1'");
			Иначе
				СтрокаШаблона = Нстр("ru = 'Ошибка при переносе показателя: %1'");
			КонецЕсли;
			
			Если Не ПустаяСтрока(СтрокаШаблона) тогда		
				ОбщегоНазначенияУХ.СообщитьОбОшибке(СтрШаблон(СтрокаШаблона, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())),,, СтатусСообщения.Важное);
			КонецЕсли;
...
Функция ПолучитьНовуюВерсию(ВерсияСтар,Отказ)
	...
	Исключение
		...
		СтрокаШаблона = Нстр("ru = 'Ошибка при создании версии показателей: %1'");
...
Функция ПолучитьНовоеОписаниеВерсии(ОписаниеВерсииСтар)
	...
	ДанныеПериода=Документы.НастраиваемыйОтчет.ПолучитьДанныеПериода(ОписаниеВерсииСтар.Сценарий,ОписаниеВерсииСтар.ПериодОтчета,ОписаниеВерсииСтар.Организация);
	
	УправлениеПериодом=ДанныеПериода.УправлениеПериодом;
	
	Если НЕ ЗначениеЗаполнено(УправлениеПериодом) Тогда
		
		СтрокаШаблона = Нстр("ru = 'Невозможно определить документ управления периодами сценария для организации %1, периода %2 и сценария %3'");
		
		Если Не ПустаяСтрока(СтрокаШаблона) тогда
			ОбщегоНазначенияУХ.СообщитьОбОшибке(СтрШаблон(СтрокаШаблона, ОписаниеВерсииСтар.Организация, ОписаниеВерсииСтар.ПериодОтчета, ОписаниеВерсииСтар.Сценарий));
		КонецЕсли;
	...
	ОписаниеВерсии.ДатаИзмененияMs			= 	ТекущаяУниверсальнаяДатаВМиллисекундах();
