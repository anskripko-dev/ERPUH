...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		Если ВыбранноеЗначение.Действие = "РассчитатьГрафикВыпускаПродукции" Тогда
			...
			ОповещениеОЗавершении = Новый ОписаниеОповещения("РассчитатьГрафикВыпускаОповещение", ЭтотОбъект);
...
&НаКлиенте
Процедура РассчитатьГрафикВыпускаОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		Попытка
			ЗагрузитьРезультатРасчетаГрафика(Результат.АдресРезультата);
		Исключение
			ВызватьИсключение;
		КонецПопытки;
	КонецЕсли;
...
&НаСервере
Процедура ЗагрузитьРезультатРасчетаГрафика(АдресХранилища)
...
Функция РассчитатьГрафикВыпуска()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Расчет графика выпуска продукции';
