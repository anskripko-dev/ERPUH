...
Функция КоличествоВыходныхИзделийПоКоличествуПоследнихЭтапов(КоличествоПоследнихЭтапов)
	...
	ВыходныеИзделия = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаказНаПроизводство, "ВыходныеИзделия");
	
	СтруктураОтбора = Новый Структура("КлючСвязи", КлючСвязиВыходноеИзделие);
	ВыходноеИзделие = ВыходныеИзделия.НайтиСтроки(СтруктураОтбора)[0];
...
Процедура ЗавершитьРедактированиеНаСервере(Режим, Отказ, ОписаниеОшибки, ПараметрыФормы = Неопределено)
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(Продукция, СтруктураДействий, Неопределено);
