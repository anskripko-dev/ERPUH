	...
	Если СтрНайти(ВидыФорм, "," + ВидФормы + ",") = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка в процедуре %1. %2 ""%3"" не поддерживается.';
				|en = 'Ошибка в процедуре %1. %2 ""%3"" не поддерживается.'"),
	...
	Если ВидФормы = "ОтправитьНаПодписание" Тогда
		...
		Если ИмяНовойФормы = "" Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				"ОткрытьНовуюФорму", "ВидФормы", ВидФормы);
		КонецЕсли;
	ИначеЕсли ВидФормы = "ПодписаниеДанных" Тогда
		ИмяНовойФормы = "Обработка.ПодписаниеДанныхВМобильномЭДО.Форма." + ВидФормы;
	Иначе
		ИмяНовойФормы = СтрШаблон("%1.%2",
			"Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.Форма",
			ВидФормы);
	КонецЕсли;
