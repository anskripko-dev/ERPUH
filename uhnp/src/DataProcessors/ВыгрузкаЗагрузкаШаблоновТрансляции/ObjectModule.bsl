...
Процедура ВыполнитьВыгрузку(Знач ИмяФайла, ТолькоПроверкаНедопустимыхСимволов = Ложь) Экспорт
	...
	Иначе
		...
		Если ТолькоПроверкаНедопустимыхСимволов Тогда
			...
			ТекстНачалоПроверки=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Начало проверки: %1'"),ТекущаяДатаСеанса());	
			СообщитьПользователю(ТекстНачалоПроверки);
			
		Иначе
			
			ТекстНачалоВыгрузки=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Начало выгрузки: %1'"),ТекущаяДатаСеанса());
			СообщитьПользователю(ТекстНачалоВыгрузки);
	...
	Если ТолькоПроверкаНедопустимыхСимволов Тогда
		...
		ТекстПроверено=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Проверено объектов: %1'"),ВсегоОбработанныхЗаписей());
		СообщитьПользователю(ТекстПроверено);
		
		ТекстОкончаниеПроверки=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Окончание проверки: %1'"),ТекущаяДатаСеанса());
		СообщитьПользователю(ТекстОкончаниеПроверки);
		
	Иначе
		
		ТекстВыгружено=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Выгружено объектов: %1'"),ВсегоОбработанныхЗаписей());
        СообщитьПользователю(ТекстВыгружено);
		
		ТекстОкончанияВыгрузки=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Окончание выгрузки: %1'"),ТекущаяДатаСеанса());	
		СообщитьПользователю(ТекстОкончанияВыгрузки);
...
Процедура ВыполнитьЗагрузку(Знач ИмяФайла) Экспорт
	...
	МассивИсключенийИмен.Добавить("Справочник.СоответствияСчетовДляТрансляции");
	...
	ТекстСообщения=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Начало загрузки: %1'"),ТекущаяДатаСеанса());		
	...
	Пока Сериализатор.ВозможностьЧтенияXML(ЧтениеXML) Цикл
		...
		Попытка
			...
			ЗаписатьВТаблицу=(СтрНайти(ИмяОбъекта,"Справочник")>0 ИЛИ СтрНайти(ИмяОбъекта,"ПланВидовХарактеристик")>0)
				И МетаданныеОбъекта.ДлинаНаименования>0
				И ЗаписанноеЗначение.ЭтоНовый()
				И (НЕ ЗаписанноеЗначение.Предопределенный)
				И МассивИсключенийИмен.Найти(ИмяОбъекта)=Неопределено;

			ЗаписанноеЗначение.Записать();
								
			Если ЗаписатьВТаблицу Тогда
	...
	ТекстСообщения=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Загружено объектов: %1'"),Загружено);
	СообщитьПользователю(ТекстСообщения);
	
	ТекстСообщения=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Окончание загрузки: %1'"),ТекущаяДатаСеанса());
	СообщитьПользователю(ТекстСообщения);
...
Процедура ВыполнитьПроверкуДублей(Строка,ТаблицаЗагруженныхСправочников)
	...
	Если Результат.Следующий() Тогда
		...
		РезультатЗамены=ВыполнитьЗаменуЭлементов(Результат.Ссылка,МассивДублей,Ложь);
...
#Область КлонФункцийНСИ

Функция ВыполнитьЗаменуЭлементов(ПравильныйЭлемент, МассивДублей, УдалятьДубли=Истина, ТаблицаДокументовСОшибками = Неопределено) 
