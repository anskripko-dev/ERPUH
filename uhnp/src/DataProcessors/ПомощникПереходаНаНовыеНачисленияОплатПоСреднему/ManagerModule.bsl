...
Процедура ПоместитьВидРасчетаВАрхив(ВидРасчета)
	...
	ВидРасчетаОбъект.Наименование = СтрШаблон(НСтр("ru = '%1 (до 2026)';
													|en = '%1 (до 2026)'"), СокрЛП(ВидРасчетаОбъект.Наименование));
	КраткоеНаименование = СокрЛП(ВидРасчетаОбъект.КраткоеНаименование);
	Если КраткоеНаименование <> "" Тогда
		ВидРасчетаОбъект.КраткоеНаименование = СтрШаблон(НСтр("ru = '%1 (до 2026)';
																|en = '%1 (до 2026)'"), КраткоеНаименование);
	КонецЕсли;
...
Процедура СоздатьДополнительныйВидРасчета(ОсновнойВидРасчета, ПрежнийВидРасчета, СозданиеДолиРК, ДополнительныеВидыРасчета)
	...
	Описание.Наименование = СтрШаблон(НСтр("ru = '%1 %2';
											|en = '%1 %2'"), Лев(СокрЛП(ЗначенияСвойствОсновногоВидаРасчета.Наименование), 99 - СтрДлина(Постфикс)), Постфикс);
