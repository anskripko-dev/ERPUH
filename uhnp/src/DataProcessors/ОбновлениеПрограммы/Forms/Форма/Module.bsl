	...
	ИначеЕсли ОбщегоНазначения.ЭтоВебКлиент()
		...
		СообщениеЖурнала =
			НСтр("ru = 'Обновление программы невозможно. Обновление программы недоступно при работе в режиме веб-клиента и мобильного клиента.';
				|en = 'Cannot update the application. Web client and mobile client modes do not support the application update.'");
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		ОтобразитьВнутреннююОшибку(
			НСтр("ru = '<b>Обновление программы невозможно</b>
				...
				|en = '<b>Cannot update the application</b>
				|Web client and mobile client modes do not support the application update.
				|Open the application in the thin or thick client and try again.'"),
			,
			СообщениеЖурнала);
		Элементы.КнопкаДалее.Видимость  = Ложь;
		Элементы.КнопкаОтмена.Заголовок = НСтр("ru = 'Закрыть';
	...
	Если ЗапрещенаРаботаСХостом Тогда
		Ответ = Ждать ВопросАсинх(
			НСтр("ru = 'Обращение к сервисам получения обновления программы запрещена настройками.
				...
				|en = 'Accessing application update services is disabled in the settings.
		...
		Если ВнедренаПодсистемаЗавершенияРаботыПользователей Тогда
			Если ЭтоБазоваяВерсияКонфигурации Тогда
				Текст = НСтр(
					...
					|en = '<a href=""open:ActiveUsers"">Active application user sessions</a> will be automatically closed. An infobase backup will be created. An application session will be launched in the new platform version.'");
			Иначе
				Текст = НСтр(
					"ru = 'Будет автоматически завершена работа <a href=""open:ActiveUsers"">активных пользователей</a> программы, создана резервная копия информационной базы и запущен сеанс работы с программой на новой версии платформы.
					|На компьютерах других пользователей программы необходимо обновить платформу 1С:Предприятие вручную.';
					|en = '<a href=""open:ActiveUsers"">Active user sessions</a> of the application will be automatically closed. An infobase backup will be created, and the application session will be started on the new platform version.
		...
		Иначе
			Если ЭтоБазоваяВерсияКонфигурации Тогда
				Текст = НСтр(
					...
					|en = 'Active application user sessions will be automatically closed. An infobase backup will be created. An application session will be launched in the new platform version.'");
			Иначе
				Текст = НСтр(
					"ru = 'Будет автоматически завершена работа активных пользователей программы, создана резервная копия информационной базы и запущен сеанс работы с программой на новой версии платформы.
					|На компьютерах других пользователей программы необходимо обновить платформу 1С:Предприятие вручную.';
					|en = 'Active user sessions of the application will be automatically closed. An infobase backup will be created, and the application session will be started on the new platform version.
					|Update the 1C:Enterprise platform on the computers of other application users manually.'");
			КонецЕсли;
		КонецЕсли;
		...
		Если ВнедренаПодсистемаЗавершенияРаботыПользователей Тогда
			Если ЭтоБазоваяВерсияКонфигурации Тогда
				Текст = НСтр(
					...
					|en = 'To use the new platform version: 
					| - Close all <a href=""open:ActiveUsers"">active user sessions</a> of the application.
					| - Close the current application session.
					| - Open the application in the new platform version.
					|
					|Create a backup before using the new platform version.'");
			Иначе
				Текст = НСтр(
					"ru = 'Чтобы начать работать на новой версии платформы:
					|  - завершите работу всех <a href=""open:ActiveUsers"">активных пользователей</a> программы;
					...
					| - Close all <a href=""open:ActiveUsers"">active user sessions</a> of the application.
					| - Close the current application session.
					| - Install the new platform version to computers of other application users.
					| - Open the application in the new platform version.
					|
					|Create a backup before using the new platform version.'");
			КонецЕсли;
		Иначе
			Если ЭтоБазоваяВерсияКонфигурации Тогда
				Текст = НСтр(
					...
					|Перед началом работы на новой версии платформы рекомендуется создать резервную копию.';
					|en = 'To use the new platform version: 
					| - Close all active user sessions of the application.
					| - Close the current application session.
					| - Open the application in the new platform version.
					|
					|Create a backup before using the new platform version.'");
		...
		Если ВнедренаПодсистемаЗавершенияРаботыПользователей Тогда
			Если ЭтоБазоваяВерсияКонфигурации Тогда
				Текст = НСтр(
					...
					| - Open the application in the new platform version.
					|
					|<a href=""open:RCInstruction"">Create a backup</a> before using the new platform version.'");
			Иначе
				Текст = НСтр(
					"ru = 'Чтобы начать работать на новой версии платформы:
					|  - завершите работу всех <a href=""open:ActiveUsers"">активных пользователей</a> программы;
