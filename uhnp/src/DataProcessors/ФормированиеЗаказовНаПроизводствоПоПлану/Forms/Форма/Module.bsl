...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура")
		...
		Или ВыбранноеЗначение.Свойство("РазбиватьЗаказыПоДатамЗапускаВыпуска")) Тогда
...
Процедура ПараметрыСозданияЗаказа(Команда)
	...
	ПараметрыСозданияЗаказа.Вставить("ДинамическаяСтруктура", ДинамическаяСтруктура);
...
Процедура УстановитьВидимостьЭлементовФормы()
	...
	ВидимостьДатыЗапускаВыпуска = ИспользоватьУправлениеПроизводством2_2 И (РазбиватьЗаказыПоДатамЗапускаВыпуска Или Не ДинамическаяСтруктура);
	Элементы.КорзинаЗаказовДатаПотребности.Видимость = ВидимостьДатыЗапускаВыпуска;
	Элементы.КорзинаЗаказовНачатьНеРанее.Видимость   = ВидимостьДатыЗапускаВыпуска;
	...
	Если ДинамическаяСтруктура Тогда 
		Элементы.РазмещениеВыпуска.Видимость = Ложь; // в заказах с динамической структурой размещение выпуска не используется
	КонецЕсли;
...
Процедура СохранитьНастройки()
	...
	Настройки.Вставить("РазбиватьЗаказыПоДатамЗапускаВыпуска",  РазбиватьЗаказыПоДатамЗапускаВыпуска);
...
Процедура ВосстановитьНастройки()
	...
	Если ТипЗнч(ЗначениеНастроек) = Тип("Структура") Тогда
		...
		Если Не ЗначениеНастроек.Свойство("РазбиватьЗаказыПоДатамЗапускаВыпуска") Тогда
			РазбиватьЗаказыПоДатамЗапускаВыпуска = Истина;
		КонецЕсли;
...
Процедура СоздатьЗаказыНаСервере()
	...
	Если ИспользоватьУправлениеПроизводством2_2 Тогда
	//-- Устарело_Производство21	
		Колонки = Колонки + ",Давалец,Договор,ТипДоговора";
		Если (РазбиватьЗаказыПоДатамЗапускаВыпуска Или Не ДинамическаяСтруктура) Тогда
	...
	Для каждого СтрокаДокумента Из ТаблицаДокументов Цикл; 
		...
		Если Не ДинамическаяСтруктура Тогда
			ДанныеЗаполнения.Вставить("РазмещениеВыпуска", Объект.РазмещениеВыпуска); // в заказах с динамической структурой размещение выпуска не используется
		КонецЕсли;
...
Процедура ЗаписатьЗаказНаСервере(ДокументОбъект, СписокОшибок)
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ДокументОбъект.Продукция, СтруктураДействий, КэшированныеЗначения);
