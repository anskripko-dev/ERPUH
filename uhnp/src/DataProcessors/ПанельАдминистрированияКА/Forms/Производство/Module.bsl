...
//++ НЕ УТКА
	...
	Если НаборКонстант.ДинамическаяСтруктураЗаказовНаПроизводство
		И НаборКонстант.МетодикаУправленияПроизводством = ПредопределенноеЗначение("Перечисление.МетодикаУправленияПроизводством.БезПланирования") Тогда
		// Обработка имеющихся заданий на расчет структуры и графика заказов
		ЗапуститьРасчетСтруктурыЗаказа();
	КонецЕсли;
//-- НЕ УТКА
...
Процедура ДинамическаяСтруктураЗаказовНаПроизводствоПриИзменении(Элемент)
	...
	//++ НЕ УТКА
	ПереченьИзменяемыхПараметров = Новый Структура;
	НуженДиалог = Ложь;
	
	Если НаборКонстант.ДинамическаяСтруктураЗаказовНаПроизводство Тогда
		
		Если НаборКонстант.МетодикаУправленияПроизводством = ПредопределенноеЗначение("Перечисление.МетодикаУправленияПроизводством.ПланированиеПотребностиВМатериалах") Тогда
			ПереченьИзменяемыхПараметров.Вставить("МетодикаУправленияПроизводством", ПредопределенноеЗначение("Перечисление.МетодикаУправленияПроизводством.ПланированиеПроизводственныхРесурсов"));
		КонецЕсли;

		Если НаборКонстант.ВариантОбособленияМатериаловВПроизводстве = ПредопределенноеЗначение("Перечисление.ВариантыОбособленияМатериаловВПроизводстве.ЭтапПроизводства") Тогда
			НуженДиалог = Истина;
			ПереченьИзменяемыхПараметров.Вставить("ВариантОбособленияМатериаловВПроизводстве", ПредопределенноеЗначение("Перечисление.ВариантыОбособленияМатериаловВПроизводстве.ЗаказНаПроизводство"));
		КонецЕсли;
	...
	Если НуженДиалог Тогда
		...
		ДополнительныеПараметры = Новый Структура("Элемент, ПереченьИзменяемыхПараметров", Элемент, ПереченьИзменяемыхПараметров);
		ОписаниеОповещения = Новый ОписаниеОповещения("ВопросУстановитьСвязанныеНастройкиПриИзмененииВариантаСтруктурыЗаказаНаПроизводство", ЭтотОбъект, ДополнительныеПараметры);
	...
	УстановитьСвязанныеНастройкиПриИзмененииВариантаСтруктурыЗаказаНаПроизводство(ПереченьИзменяемыхПараметров);
	Подключаемый_ПриИзмененииРеквизита(Элемент);
...
//++ НЕ УТКА
	...
	Если КонстантаИмя = "ДинамическаяСтруктураЗаказовНаПроизводство"
		ИЛИ КонстантаИмя = "МетодикаУправленияПроизводством" Тогда
		Константы.ИспользуетсяНормативныйГрафикСтруктурыЗаказа.Установить(НаборКонстант.ДинамическаяСтруктураЗаказовНаПроизводство
			И НаборКонстант.МетодикаУправленияПроизводством = Перечисления.МетодикаУправленияПроизводством.БезПланирования);
	КонецЕсли;
//-- НЕ УТКА
...
//++ НЕ УТКА
		...
		Элементы.ЛокализацияГруппаИспользоватьВыпускПоНесколькимРаспоряжениям.Доступность = ИспользуетсяПроизводство();
		Элементы.ГруппаИспользоватьПараметрыНазначенияСпецификаций.Доступность = ИспользуетсяПроизводство();
		Элементы.ГруппаИспользоватьАналогиМатериалов.Доступность = ИспользуетсяПроизводство();
		Элементы.ГруппаИспользоватьНаборыВПроизводстве.Доступность = ИспользуетсяПроизводство();
...
//++ НЕ УТКА
	...
	Если РеквизитПутьКДанным = ""
		ИЛИ РеквизитПутьКДанным = "НаборКонстант.ДинамическаяСтруктураЗаказовНаПроизводство"
		ИЛИ РеквизитПутьКДанным = "НаборКонстант.МетодикаУправленияПроизводством"
		ИЛИ СтрНайти(РеквизитПутьКДанным, "ИспользоватьПроизводство") > 0
		ИЛИ СтрНайти(РеквизитПутьКДанным, "ИспользоватьУправлениеПроизводством") > 0 Тогда
		
		Элементы.ГруппаЗаказыНаПроизводство.Видимость = ИспользуетсяПроизводство();
		
		ОбщегоНазначенияУТКлиентСервер.ОтображениеПредупрежденияПриРедактировании(Элементы.ДинамическаяСтруктураЗаказовНаПроизводство, НаборКонстант.ДинамическаяСтруктураЗаказовНаПроизводство);
		Элементы.НастройкиРасчетаСтруктурыЗаказаНаПроизводство.Доступность = ИспользуетсяТолькоПроизводство22() И НаборКонстант.ДинамическаяСтруктураЗаказовНаПроизводство;
...
//-- Устарело_Производство21
		...
		СписокВыбораПоля = Элементы.МетодикаУправленияПроизводством.СписокВыбора;
		СписокВыбораПоля.Очистить();
		СписокВыбораПоля.Добавить(Перечисления.МетодикаУправленияПроизводством.БезПланирования,
			?(НаборКонстант.ДинамическаяСтруктураЗаказовНаПроизводство,
				НСтр("ru = 'Планирование по нормативным длительностям';
					|en = 'Planning by standard duration'"),
				НСтр("ru = 'Без планирования графика производства';
					|en = 'Without creating production schedule'")));
		Если НЕ НаборКонстант.ДинамическаяСтруктураЗаказовНаПроизводство Тогда
			СписокВыбораПоля.Добавить(Перечисления.МетодикаУправленияПроизводством.ПланированиеПотребностиВМатериалах);
		КонецЕсли;
		СписокВыбораПоля.Добавить(Перечисления.МетодикаУправленияПроизводством.ПланированиеПроизводственныхРесурсов,
			?(НаборКонстант.ДинамическаяСтруктураЗаказовНаПроизводство,
				НСтр("ru = 'Планирование с учетом ресурсных ограничений';
					|en = 'Planning based on resource limitations'"),
				НСтр("ru = 'Планирование по материальным и производственным ресурсам';
					|en = 'Planning by material and production resources'")));
		СписокВыбораПоля = Неопределено;
		
		Элементы.ПодсказкаМетодикаУправленияПроизводством.Подсказка = ПодсказкаПоМетодике(НаборКонстант.МетодикаУправленияПроизводством, НаборКонстант.ДинамическаяСтруктураЗаказовНаПроизводство);
...
//++ Устарело_Производство21
		Элементы.ГруппаСтруктураЗаказа.Видимость = ИспользуетсяПроизводство22() И Не ИспользуетсяПроизводство21();
...
//-- Устарело_Производство21
		...
		Элементы.ГруппаПорядокРасчетаГрафика.Доступность = ИспользуетсяПроизводство22();
	...
	Если РеквизитПутьКДанным = ""
		...
		Если НаборКонстант.ДинамическаяСтруктураЗаказовНаПроизводство Тогда
			ДинамическаяСтруктураЗаказаДоступна = Истина;
		Иначе
			ДинамическаяСтруктураЗаказаДоступна = НЕ РемонтИлиРазборкаИспользуется И ИспользуетсяПроизводство22();
		КонецЕсли;
		...
		Элементы.ГруппаНастройкаВариантаФормированияСтруктурыЗаказа.Доступность = ДинамическаяСтруктураЗаказаДоступна;
		Элементы.ГруппаНедоступнаНастройкаВариантаФормированияСтруктурыЗаказа.Видимость = НЕ ДинамическаяСтруктураЗаказаДоступна;
...
&НаКлиенте
Процедура ВопросУстановитьСвязанныеНастройкиПриИзмененииВариантаСтруктурыЗаказаНаПроизводство(ОтветНаВопрос, ДополнительныеПараметры) Экспорт
	...
	УстановитьСвязанныеНастройкиПриИзмененииВариантаСтруктурыЗаказаНаПроизводство(ДополнительныеПараметры.ПереченьИзменяемыхПараметров);
...
&НаСервере
Процедура УстановитьСвязанныеНастройкиПриИзмененииВариантаСтруктурыЗаказаНаПроизводство(ПереченьИзменяемыхПараметров)
	
	Для каждого КлючИЗначение Из ПереченьИзменяемыхПараметров Цикл
...
&НаСервереБезКонтекста
Процедура ЗапуститьРасчетСтруктурыЗаказа()
...
&НаСервереБезКонтекста
Функция ПодсказкаПоМетодике(Методика, ДинамическаяСтруктура)
	
	Текст = "";
	
	Если Методика = Перечисления.МетодикаУправленияПроизводством.ПланированиеПроизводственныхРесурсов Тогда
		
		Текст = НСтр("ru = 'График производства рассчитывается на основе доступности видов рабочих центров и потребности в материалах. Этапам назначаются сроки выполнения.';
					|en = 'Production schedule is calculated based on capacity of work center types and demand for materials. Execution dates are assigned to stages.'");
		
	ИначеЕсли Методика = Перечисления.МетодикаУправленияПроизводством.ПланированиеПотребностиВМатериалах Тогда
		
		Текст = НСтр("ru = 'Расчет графика производства выполняется с учетом потребности в материалах. Виды рабочих центров не используются при расчете графика. Этапам назначаются сроки выполнения.';
	...
	Иначе
		...
		Если ДинамическаяСтруктура Тогда
			...
			Текст = НСтр("ru = 'Расчет графика производства выполняется по производственным циклам от желаемых дат выпуска. Этапам назначаются нормативные сроки выполнения.';
						|en = 'Production schedule is calculated by production cycles from the requested release dates. Standard due dates are assigned to stages.'");
		
		Иначе
			
			Текст = НСтр("ru = 'График производства не рассчитывается. Сроки выполнения этапов не назначаются, этапы выполняются согласно очереди.';
