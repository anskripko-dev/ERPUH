...
Процедура ОбновитьПолеНастройки()
	...
	|	СтрокиОрганизаций.Ряд КАК КоординатыВерх,
	...
	ТЗКоординат = ЗапросКоординаты.Выполнить().Выгрузить();
	КолонкаКоординатыВерх = ТЗКоординат.Колонки.Найти("КоординатыВерх");
	КолонкаКоординатыВерх.Имя = "Верх";
	
	ЗначениеВРеквизитФормы(ТЗКоординат,"ТаблицаКоординат");
...
Процедура ПолучитьТаблицуНастроек()
	...
	Запрос.Текст="ВЫБРАТЬ
	             ...
	             |	НастройкиЗаполненияОбъектовБДСрезПоследних.ВыполнятьАвтоматически КАК ВыполнятьАвтоматически,
	             ...
	             |	КОНЕЦ КАК ТипБД,
	             |	НастройкиЗаполненияОбъектовБДСрезПоследних.СпособФормирования КАК СпособФормирования
	             ...
	             |////////////////////////////////////////////////////////////////////////////////
	             |ВЫБРАТЬ РАЗЛИЧНЫЕ
	             |	ИсходнаяТаблицаРегистра.Организация КАК Организация,
	             |	ИсходнаяТаблицаРегистра.ОбъектБД КАК ОбъектБД,
	             |	ИсходнаяТаблицаРегистра.ОбластьГенерации КАК ОбластьГенерации
	             |ИЗ
	...
	ОрганизационныеЕдиницы.ПолучитьЭлементы()[0].Значение=Нстр("ru = '(Все организации)'");
	...
	|	ИсходнаяТаблицаРегистра.СпособФормирования,
...
Функция СформироватьПредставлениеНастроекСервер(ТабНастройкиЗаполнения)
	...
	Для Инд=0 по МаксНомерСтроки цикл
		...
		ТекстНастройки=ТекстНастройки+"; "
			+?(ЗначениеЗаполнено(ТабНастройкиЗаполнения[Инд].ИспользуемаяИБ),Строка(ТабНастройкиЗаполнения[Инд].ИспользуемаяИБ)+": ","")
			+?(ЗначениеЗаполнено(ТабНастройкиЗаполнения[Инд].ИсходныйОбъектБД),ТабНастройкиЗаполнения[Инд].ИсходныйОбъектБД,ТабНастройкиЗаполнения[Инд].ПравилоЗаполнения)
			+","+ТабНастройкиЗаполнения[Инд].СпособФормирования;
		
	КонецЦикла;
	
	ТекстНастройки=Сред(ТекстНастройки,3);
...
Процедура СформироватьДеревоОбъектовБД()
	...
	НоваяСтрока.Значение=Нстр("ru = '(Все объекты)'");
...
Процедура ОбработатьПолеОтбора(ВыбранноеЗначение)
	...
	Если ВыбранноеЗначение=Нстр("ru = '(Все организации)'") Тогда
		
		СписокОрганизацийОтбор.Очистить();
		
	ИначеЕсли ВыбранноеЗначение=Нстр("ru = '(Все объекты)'") Тогда
