...
Процедура ВычислитьКолонкуТаблицыПоСтрокам(Таблица, ИмяКолонки, ФормулаВычисления)
	
	Для Каждого СтрокаТаб Из Таблица Цикл
		
		Попытка
			
			Результат = ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(
				СтрокаТаб, ФормулаВычисления);
				
			СтрокаТаб[ИмяКолонки] = Результат;
