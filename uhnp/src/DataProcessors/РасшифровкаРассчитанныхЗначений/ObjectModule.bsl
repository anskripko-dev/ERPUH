...
Функция ОбработатьРасшифровкуАналитикПоказателяПоПравилуОбработки(ОбъектРасчета,Знач Пок, Знач СтруктураОтбора) Экспорт
	...
	Если ОбъектРасчета.Свойство("ДокументКорректировки")
		ИЛИ ОбъектРасчета.ЗаполнениеВыполнено 
		ИЛИ НЕ ОбъектРасчета.мТабПоказателиЗапросы=Неопределено Тогда
	...
	Если ПараметрыРасчета.СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.Консолидация
		ИЛИ ПараметрыРасчета.СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.КонсолидацияПропорционально
		ИЛИ ПараметрыРасчета.СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.СвернувПоАналитикам
		ИЛИ ПараметрыРасчета.СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.СвернувПоПроектам
		ИЛИ ПараметрыРасчета.СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.СвернувПоОрганизациямПроекта
		ИЛИ ПараметрыРасчета.СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.СвернувПоПериоду
		ИЛИ ПараметрыРасчета.СпособФормированияОтчета = Перечисления.СпособыФормированияОтчетов.Элиминация Тогда
	...
	Делитель = 0;
	ФорматнаяСтрока = "";
	УправлениеОтчетамиУХ.ЗаполнитьПараметрыОкругления(
		ОбъектРасчета.ЕдиницаИзмеренияОтображения, Делитель, ФорматнаяСтрока);
	...
	Иначе	
		...
		Для Каждого СтрОперанд ИЗ ДанныеДляРасчета.мОперанды Цикл
			...
			Для Каждого СтрАналитика ИЗ СтрОперанд.ТаблицаАналитик Цикл
				...
				Если СтрАналитика.ЗаполнениеПоДругимИсточникам
					ИЛИ (НЕ СтруктураОтбора.Свойство(СтрАналитика.КодАналитики))
					ИЛИ (СтрАналитика.КодАналитики="АналитикаВалюта" И СтруктураОтбора[СтрАналитика.КодАналитики]=Справочники.Валюты.ПустаяСсылка())
					ИЛИ СтруктураОтбора[СтрАналитика.КодАналитики]=Неопределено Тогда
	...
	Если НЕ (ЗначениеПоказателяНач=ЗначениеПоказателяТек 
			...
			ИЛИ ОбъектРасчета.Свойство("УчетнаяПолитика")
...
Процедура ДополнитьДетализациейРасчета(ОбъектРасчета,РеквизитыОперандов,Пок,ТабличныйДокумент,ПоказателиТекущегоОтчета=Ложь)
	...
	Для каждого СтрокаТаблицы из РабочаяТаблица цикл
		...
		Для каждого ДанныеОперанда ИЗ РеквизитыОперандов цикл
			...
			Если НЕ РабочаяТаблица.Колонки.Найти(ИмяКолонки)=Неопределено Тогда
