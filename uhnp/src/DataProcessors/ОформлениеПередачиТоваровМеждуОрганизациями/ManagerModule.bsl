...
Функция СостояниеПередачиМеждуОрганизациями(ДокументСсылка) Экспорт
	...
	Если ОбработкаОбъект.РежимОжидания Тогда
		Возврат Новый ФорматированнаяСтрока(НСтр("ru = 'Выполняется передача товаров между организациями';
												|en = 'Transferring goods between companies'"),,,,
	...
	ИначеЕсли ОбработкаОбъект.ТребуетсяОформлениеПередачиТоваров() Тогда
		Возврат Новый ФорматированнаяСтрока(НСтр("ru = 'Требуется передача товаров между организациями';
												|en = 'Customer invoice  — Intercompany is required'"),,
...
Процедура ПроверитьСтатусОформленияОснования(РеквизитыСтроки, НастройкиПроверки) Экспорт
	...
	Если ЗначениеВМассиве.Количество() = 0 Тогда
		...
		РеквизитыСтроки.Описание = НСтр("ru = 'Документ не относится к выбранному складу';
										|en = 'The document is not related to the selected warehouse'");
		Возврат;
	КонецЕсли;
	...
	Если СостояниеКоманды.Видимость
		...
		РеквизитыСтроки.Описание = НСтр("ru = 'Проверка кодов маркировки по документу не завершена';
										|en = 'Document marking code check is in progress'");
		Возврат;
	КонецЕсли;
...
Функция СформироватьГиперссылкуСмТакжеВРаботе(ПараметрыФормирования) Экспорт
	...
	ТекстГиперссылки = СтрШаблон(
		НСтр("ru = 'Оформление передачи товаров между организациями (%1)';
			|en = 'Create Customer invoice — Intercompany (%1)'"),
...
Функция ДокументыВПроцессеПередачиМеждуОрганизациями(ОбработкаОбъект) Экспорт
	...
	Запрос.УстановитьПараметр("ТекстДокументНеСоздан", НСтр("ru = 'Документ не создан';
															|en = 'The document is not created'"));
...
//++ НЕ УТ
	...
	МассивТекстовЗапросов.Добавить(ИнтеграцияИСУТ.ТекстЗапросаДвижениеПродукцииИМатериалов());
	МассивТекстовЗапросов.Добавить(ИнтеграцияИС.РазделительЗапросовВПакете());
	
	МассивТекстовЗапросов.Добавить(СтрЗаменить(ИнтеграцияИСУТ.ТекстЗапросаПроизводствоБезЗаказа(),
		"ТаблицаТовары.Ссылка В(&Документ)",
		"ТаблицаТовары.Ссылка В(&Документ) И ТаблицаТовары.Склад = &Склад"));
	МассивТекстовЗапросов.Добавить(ИнтеграцияИС.РазделительЗапросовВПакете());
...
//++ НЕ УТ
	...
	МассивТекстовЗапросов.Добавить(ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении());
	МассивТекстовЗапросов.Добавить(ТекстЗапросаИтог("ТаблицаТоварыДвижениеПродукцииИМатериалов"));
	МассивТекстовЗапросов.Добавить(ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении());
	МассивТекстовЗапросов.Добавить(ТекстЗапросаИтог("ТаблицаТоварыПроизводствоБезЗаказа"));
...
// ++ НЕ УТКА
	...
	|	Штрихкоды.Ссылка.Организация КАК ОрганизацияПолучатель
	|ИЗ
	|	Документ.ЭтапПроизводства2_2.ШтрихкодыУпаковокМатериалы КАК Штрихкоды
	|ГДЕ
	|	Штрихкоды.Ссылка В (&Документы)
	|
...
// ++ НЕ УТ
	...
	|	Штрихкоды.Ссылка.Организация КАК ОрганизацияПолучатель
	|ИЗ
	|	Документ.ПроизводствоБезЗаказа.ШтрихкодыУпаковокМатериалы КАК Штрихкоды
	|ГДЕ
	|	Штрихкоды.Ссылка В (&Документы)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	Штрихкоды.Ссылка.Организация КАК ОрганизацияПолучатель
	|ИЗ
	|	Документ.ДвижениеПродукцииИМатериалов.ШтрихкодыУпаковок КАК Штрихкоды
	|ГДЕ
	|	Штрихкоды.Ссылка В (&Документы)
...
//++ НЕ УТ
	...
	|ГДЕ
	|	ДвижениеПродукцииИМатериалов.Отправитель = &Склад
	|	И ДвижениеПродукцииИМатериалов.Ссылка В (&Ссылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ИЗ
	|	Документ.ПроизводствоБезЗаказа.МатериалыИРаботы КАК ПроизводствоБезЗаказа
	|ГДЕ
	|	ПроизводствоБезЗаказа.Склад = &Склад
	|	И ПроизводствоБезЗаказа.Ссылка В (&Ссылка)
...
//++ НЕ УТКА
	...
	|ИЗ
	|	Документ.ЭтапПроизводства2_2.РасходМатериаловИРабот КАК ЭтапПроизводства2_2
	|ГДЕ
	|	ЭтапПроизводства2_2.АналитикаУчетаНоменклатуры.МестоХранения = &Склад
	|	И ЭтапПроизводства2_2.Ссылка В (&Ссылка)
	...
	Исключение
		ОбщегоНазначения.СообщитьПользователю(
			СтрШаблон(НСтр("ru = 'Созданный документ %1 не проведен';
							|en = 'The created %1 document is not posted'"),
				ДокументПередачи));
	КонецПопытки;
