...
Функция ПроверитьНачалоПериодаСверки()
	...
	Если ПериодСверки.ДатаНачала < ОграничениеСнизу Тогда
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Сверка возможна с %1';
										|en = 'Сверка возможна с %1'"), Формат(ОграничениеСнизу, "ДЛФ=DD"));
	...
	Если ПериодСверки.ДатаНачала > ПериодСверки.ДатаОкончания Тогда
		ТекстСообщения = НСтр("ru = 'Дата начала периода не может быть больше даты конца периода';
								|en = 'Дата начала периода не может быть больше даты конца периода'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "ПериодГлавное.ДатаНачала");
		Возврат Ложь;
	КонецЕсли;
...
Процедура ОформитьВнутренниеОшибки(КодОшибки)
	...
	ИначеЕсли КодОшибки = "НетПодписки1СОтчетность" Тогда
		...
		ТекстВтораяСтрокаОшибкиРаботыСервиса.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'договор.';
																						|en = 'договор.'"), , ЦветГиперссылки, , "ЗаявкаНаПодключение1СОтчетности"));
	...
	ИначеЕсли КодОшибки = "НеНайденаМЧДВРеестре" Тогда
		...
		ТекстВтораяСтрокаОшибкиРаботыСервиса.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Создайте';
																						|en = 'Создайте'"), , ЦветГиперссылки, , "e1cib/list/Справочник.МашиночитаемыеДоверенности"));
	...
	ИначеЕсли КодОшибки = "ОшибкаМенеджераКриптографии" Тогда
		...
		ТекстВтораяСтрокаОшибкиРаботыСервиса.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Установите';
																						|en = 'Установите'"), , ЦветГиперссылки, , "УстановитьМенеджерКриптографии"));
	...
	ИначеЕсли КодОшибки = "НеверныйЛогинИлиПароль" Тогда
		...
		ТекстВтораяСтрокаОшибкиРаботыСервиса.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'интернет-поддержки';
																						|en = 'интернет-поддержки'"), , ЦветГиперссылки, , "ИнтернетПоддержкаИСервисы"));
		Элементы.ТекстВтораяСтрокаОшибкиРаботыСервиса.Заголовок = Новый ФорматированнаяСтрока(ТекстВтораяСтрокаОшибкиРаботыСервиса);
		ОбновлениеДанныхДоступно = Ложь;
...
Функция ТекстПредупрежденияОбОшибкеОтразитьВУчете()
	...
	|en = 'Не удалось отразить в учете все расхождения.
	|Подробности см. в Журнале регистрации.'");
