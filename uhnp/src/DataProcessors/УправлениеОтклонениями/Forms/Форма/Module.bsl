...
Функция ТекстЗапросаЗаявки()
	...
	|			ТОГДА ДвиженияБюджетированияПоПозициям.СуммаВзаиморасчетов
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	0,
	|	0,
...
ТекстЗапроса = "
	...
	// ЕРПУХ ++
	|	ОперацииБюджетовОбороты.АналитикаПартнеров.ОбъектРасчетов.Менеджер,
	// ЕРПУХ --
	|	ОперацииБюджетовОбороты.ОбъектПланирования.Объект.ОбъектРасчетов,
	...
	|	ОперацииБюджетовОбороты.Период,                   
		// ЕРПУХ ++
	|	ЕСТЬNULL(ОперацииБюджетовОбороты.АналитикаПартнеров.ОбъектРасчетов.Менеджер, ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)),
		// ЕРПУХ --
	|	0,
	|	0,
	|	0,
	...
	|ИЗ
	|	РегистрНакопления.ОперацииБюджетов.Обороты(&ДатаОперации,&ГоризонтПланирования, День , 
	|	(ОбъектПланирования.Объект  ССЫЛКА Документ.ЗаявкаНаРасход 
	|		ИЛИ ОбъектПланирования.Объект ССЫЛКА Документ.ПланируемыйДоход)
	|		) КАК ОперацииБюджетовОбороты
	|ГДЕ
	...
	Если ТипОбъектовРасчетов = ТипОбъектаРасчетовДоговорыСПоставщиками() Тогда
		...
		"ОперацииБюджетовОбороты.АналитикаПартнеров.Договор.ВидДоговораУХ  В (&ВидыДоговоровСПоставщиком)");
	
	ИначеЕсли ТипОбъектовРасчетов = ТипОбъектаРасчетовДоговорыСПокупателями() Тогда
			
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборТипОбъектаОперативныйПлан", 
		"ОперацииБюджетовОбороты.АналитикаПартнеров.Договор.ВидДоговораУХ  В (&ВидыДоговоровСПокупателем)");
		
	ИначеЕсли ТипОбъектовРасчетов = ТипОбъектаРасчетовДоговорыЛизинг() Тогда
			
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборТипОбъектаОперативныйПлан", 
		"ОперацииБюджетовОбороты.АналитикаПартнеров.Договор.ВидДоговораУХ  В (&ВидыДоговоровЛизинг)");
			
	ИначеЕсли ТипОбъектовРасчетов = ТипОбъектаРасчетовАккредитивыБанковскиеГарантии() Тогда
			
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборТипОбъектаОперативныйПлан", 
		"ОперацииБюджетовОбороты.АналитикаПартнеров.Договор.ВидДоговораУХ  В (&ВидыДоговоровАккредитивыБанковскиеГарантии)");
		
	ИначеЕсли ТипОбъектовРасчетов = ТипОбъектаРасчетовПроизводныеИструменты() Тогда
			
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборТипОбъектаОперативныйПлан", 
		"ОперацииБюджетовОбороты.АналитикаПартнеров.Договор.ВидДоговораУХ  В (&ВидыДоговоровПроизводныеИструменты)");
		 	
	ИначеЕсли ТипОбъектовРасчетов = ТипОбъектаРасчетовПривлечениеСредств() Тогда
		
		ОтборТипОбъектаРасчетовФакт = "ОперацииБюджетовОбороты.АналитикаПартнеров.Договор.ВидДоговораУХ В ИЕРАРХИИ (Значение(Справочник.ВидыДоговоровКонтрагентовУХ.ПривлечениеСредств))";
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборТипОбъектаОперативныйПлан", ОтборТипОбъектаРасчетовФакт);
		
	ИначеЕсли ТипОбъектовРасчетов = ТипОбъектаРасчетовРазмещениеСредств() Тогда
		
		ОтборТипОбъектаРасчетовФакт = "ОперацииБюджетовОбороты.АналитикаПартнеров.Договор.ВидДоговораУХ В ИЕРАРХИИ (Значение(Справочник.ВидыДоговоровКонтрагентовУХ.РазмещениеСредств))";
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборТипОбъектаОперативныйПлан", ОтборТипОбъектаРасчетовФакт);
	...
	Если ЗначениеЗаполнено(Ответственный)
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборОтветственныйОперативныйПлан", 	"ЕСТЬNULL(ОперацииБюджетовОбороты.АналитикаПартнеров.ОбъектРасчетов.Менеджер, ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)) = &Ответственный");
		
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Организация) Тогда

		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборОрганизацияОперативныйПлан", 	"ОперацииБюджетовОбороты.АналитикаСтруктуры.Организация = &Организация");

	КонецЕсли;	

	Если ЗначениеЗаполнено(Контрагент) Тогда
	
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборКонтрагентОперативныйПлан", 	"ОперацииБюджетовОбороты.АналитикаПартнеров.Контрагент = &Контрагент");
...
Процедура ОбновитьДанные()
	...
	Запрос.УстановитьПараметр("ДатаОперации", 			КонецДня(ДатаОбновления));
...
Процедура ОбработатьПозициюКПогашению(Позиция, Знач ДатаПогашения, Знач ТаблицаНастроекОпераций, ТаблицаСформированныхЗаявок)
	...
	СтруктураПараметровЗаявки.Вставить("Договор",			Позиция.ДоговорКонтрагента);
	...
	ДенежныеСредстваСервер.ЗаполнитьТекстНазначенияПлатежа(НоваяЗаявка);
