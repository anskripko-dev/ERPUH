...
Процедура СформироватьДокументыВводаОстатковЗаказыДавальцу()
	...
	|ПОМЕСТИТЬ Продукция
	|ИЗ РегистрНакопления.РаспоряженияНаОтгрузку.Обороты(,&ГраницаОст,, Распоряжение ССЫЛКА Документ.ЗаказДавальца
	|									И Распоряжение.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовДавальцев.Закрыт)) КАК ТабРег
	|ЛЕВОЕ СОЕДИНЕНИЕ
	...
	|	И ТабРег.Распоряжение = ТабУслуги.ЗаказДавальца
...
Процедура СформироватьДокументыВводаОстатковАналогиВПроизводстве()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТабРег.ПериодЗавершения КАК ДатаОкончанияДействия,
		|	ТабРег.Номенклатура КАК Номенклатура,
		|	ТабРег.Характеристика КАК Характеристика,
		|	ТабРег.РольНоменклатуры КАК РольНоменклатуры,
		|	ТабРег.КоличествоУпаковок КАК КоличествоУпаковок,
		|	ТабРег.Упаковка КАК Упаковка,
		|	ТабРег.Регистратор КАК Разрешение
		|ИЗ
		|	РегистрСведений.АналогиМатериалов.СрезПоследних(&ГраницаОст, ) КАК ТабРег
		|ГДЕ
		|	(ТабРег.ПериодЗавершения > &ДатаОст
		|			ИЛИ ТабРег.ПериодЗавершения = ДАТАВРЕМЯ(1, 1, 1))
		|ИТОГИ ПО
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ОбластиДействияАналоговМатериалов.Разрешение КАК Разрешение,
		|	ОбластиДействияАналоговМатериалов.Спецификация КАК Спецификация,
		|	ОбластиДействияАналоговМатериалов.Этап КАК Этап,
		|	ОбластиДействияАналоговМатериалов.Подразделение КАК Подразделение,
		|	ОбластиДействияАналоговМатериалов.ЗаказКлиента КАК ЗаказКлиента,
		|	ОбластиДействияАналоговМатериалов.ЗаказНаПроизводство КАК ЗаказНаПроизводство,
		|	ОбластиДействияАналоговМатериалов.Изделие КАК Изделие,
		|	ОбластиДействияАналоговМатериалов.ХарактеристикаИзделия КАК ХарактеристикаИзделия,
		|	ОбластиДействияАналоговМатериалов.НаправлениеДеятельности КАК НаправлениеДеятельности
		|ИЗ
		|	РегистрСведений.ОбластиДействияАналоговМатериалов КАК ОбластиДействияАналоговМатериалов";
		
	ЗаполнитьПараметрыЗапросаДатаСвертки(Запрос);
	МассивРезультатов = Запрос.ВыполнитьПакет();
	КоличествоРезультатов = МассивРезультатов.ВГраница();
	РезультатАналоги = МассивРезультатов[КоличествоРезультатов - 1];
	ТаблицаОбластиДействия = МассивРезультатов[КоличествоРезультатов].Выгрузить();
	ТаблицаОбластиДействия.Индексы.Добавить("Разрешение");
	РольАналог = Перечисления.РолиАналогов.Аналог;
	ВариантНабор = Перечисления.ВариантЗаменыАналогов.НаборНаНабор;
	ВариантСписок = Перечисления.ВариантЗаменыАналогов.СписокАналогов;
	
	Если РезультатАналоги.Пустой() Тогда
		Возврат;
	КонецЕсли;
	
	КоличествоДокументовВводаОстатков = 1;
	ВыборкаПериод = РезультатАналоги.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	СтруктураПоиска = Новый Структура("Разрешение");
	
	Пока ВыборкаПериод.Следующий() Цикл
	
		ВыборкаРазрешение = ВыборкаПериод.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаРазрешение.Следующий() Цикл
			...
			ДокОст.Ответственный =  Пользователи.АвторизованныйПользователь();
			ДокОст.Статус = Перечисления.СтатусыРазрешенийНаЗаменуМатериалов.Утверждено;
			
			ЕстьЗаписиВТЧАналоги = Ложь;
			
			ВыборкаНоменклатура = ВыборкаРазрешение.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			Пока ВыборкаНоменклатура.Следующий() Цикл
				
				Если ВыборкаНоменклатура.РольНоменклатуры = РольАналог Тогда
					ТабличнаяЧасть = ДокОст.Аналоги;
					ЕстьЗаписиВТЧАналоги = Истина;
				Иначе
					ТабличнаяЧасть = ДокОст.Материалы;
				КонецЕсли;
			...
			СтруктураПоиска.Разрешение = ВыборкаРазрешение.Разрешение;
			МассивСтрокОбластиДействия = ТаблицаОбластиДействия.НайтиСтроки(СтруктураПоиска);
			Для Каждого СтрокаОбластиДействия Из МассивСтрокОбластиДействия Цикл
			
				СтрокаТЧ = ДокОст.ОбластиДействия.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТЧ, СтрокаОбластиДействия);
			
			КонецЦикла;
			
			Если ЕстьЗаписиВТЧАналоги Тогда
				ДокОст.ВариантЗамены = ВариантНабор;
			Иначе
				ДокОст.ВариантЗамены = ВариантСписок;
			КонецЕсли;
