...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Структура = Новый Структура("СписокВыбора, РежимВыбораИзСписка", СписокВыбора, Ложь);
	НалоговыйМониторинг.ЗаполнитьСпискиВыбораВЭлементахУправления(Структура, "РегламентированноеУведомлениеРискиОрганизацииНалоговыйМониторинг", "СпискиВыбора2020_1", "СписокКодНалог");
	
	Список.УсловноеОформление.Элементы.Очистить();
	
	Для каждого Стр Из Структура.СписокВыбора Цикл
		ЭлементУО = УсловноеОформление.Элементы.Добавить();
		КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, Элементы.СписокКодНалога.Имя);
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,	"Список.КодНалога", ВидСравненияКомпоновкиДанных.Равно, Стр.Значение,,Истина);
		ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст", Стр.Представление);
	КонецЦикла;
