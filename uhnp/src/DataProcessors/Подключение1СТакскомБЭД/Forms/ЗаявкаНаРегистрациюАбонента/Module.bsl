...
Процедура ПриОткрытии(Отказ)
	...
	Подключение1СТакскомКлиентБЭД.ОбработатьОткрытиеФормы(КонтекстВзаимодействия, ЭтотОбъект);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ПрограммноеЗакрытие Тогда
		...
		Если Не ЗавершениеРаботы И СтатусФормы <> "show" И Не НажатаКнопкаОтправить Тогда
			...
			Подключение1СТакскомКлиентБЭД.ОбработкаКомандСервиса(
...
Процедура НадписьПричинаНажатие(Элемент)
	...
	Подключение1СТакскомКлиентБЭД.ПоказатьПричинуОтклоненияЗаявкиЭДО(КонтекстВзаимодействия);
...
Процедура НадписьВыходНажатие(Элемент)
	...
	Подключение1СТакскомКлиентБЭД.ОбработатьВыходПользователя(КонтекстВзаимодействия, ЭтотОбъект);
...
Процедура ДекорацияТехПоддержкаОбработкаНавигационнойСсылки(
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "TechSupport" Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстСообщения,
			ЧтоНеПолучилось,
			Подключение1СТакскомКлиентБЭД.ТекстТехническихПараметровЭДО(КонтекстВзаимодействия, СертификатЭП));
...
Процедура ОтправитьЗаявку(Команда)
	...
	Если СтатусФормы <> "show" Тогда
		...
		Если ЗначениеЗаполнено(СертификатЭП) Тогда
			...
			ПрежнийСертификат = Подключение1СТакскомКлиентБЭД.ЗначениеСессионногоПараметра(
				КонтекстВзаимодействия.КСКонтекст,
				"IDCertificateED");
			
			ПредставлениеСертификата = "";
			ДвоичныеДанныеСертификата = Подключение1СТакскомВызовСервераБЭД.ДвоичныеДанныеСертификата(СертификатЭП, ПредставлениеСертификата);
...
Процедура ВыбратьАдрес(ТолькоДляПросмотра = Ложь)
	...
	ФормаВводаАдреса = ОткрытьФорму("Обработка.Подключение1СТакскомБЭД.Форма.АдресУчастникаОбменаЭД",
		ПараметрыФормы,
		,
		,
		,
	...
	Подключение1СТакскомКлиентБЭД.ОбработатьОткрытиеФормы(КонтекстВзаимодействия, ФормаВводаАдреса);
