Процедура СформироватьОтчетПоПоказателям(ДиаграммаГанта, ДанныеОбъекта, Сценарий) Экспорт
	...
	ИначеЕсли ТаблицаИнтервала[0].ДатаНачала = Null ИЛИ ТаблицаИнтервала[0].ДатаОкончания = Null Тогда
		...
		Сообщение.Текст = Нстр("ru = 'Проверьте интервалы проекта! Диаграмма не может быть построена.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрПроект ИЗ ДанныеПроектов Цикл
		...
		ТочкаПроект.Текст=?(ПустаяСтрока(СтрПроект.Стадия),"", СтрШаблон(Нстр("ru = 'Стадия: %1%2'"), 
			СтрПроект.Стадия, 
			Символы.ПС)) + СтрПроект.Наименование+?(ПустаяСтрока(ТекстИтог),"",Символы.ПС+Сред(ТекстИтог,3));
