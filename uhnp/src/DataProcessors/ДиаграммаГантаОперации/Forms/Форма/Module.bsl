...
Процедура ДиаграммаГантаОбработкаРасшифровки(Элемент, Расшифровки, СтандартнаяОбработка, Дата)
	...
	Если ТипЗнч(Расшифровки)=Тип("Массив") И Расшифровки.Количество()>0 И ТипЗнч(Расшифровки[Расшифровки.Количество()-1])=Тип("Структура") Тогда
		...
		ИначеЕсли ДанныеРасшифровки.Свойство("ОперативныйПлан") Тогда
			...
			ОткрытьФорму("Документ."+ИмяДокумента+".ФормаОбъекта",Новый Структура("Ключ",ДанныеРасшифровки.ОперативныйПлан));
...
Процедура БюджетДДС(Команда)
	...
	ОткрытьФорму("Обработка.АналитическийБланкСводнаяТаблица.Форма.ФормаМакетаСводнаяТаблица",СтруктураПараметров);
...
Процедура СформироватьОтчетПоОперациям(ДиаграммаГанта,ДанныеОбъекта, Сценарий)
	...
	Запрос.Текст="ВЫБРАТЬ
	             ...
	             |	ВЫБОР
	             |		КОГДА ОперативныйПлан.Предназначение = ЗНАЧЕНИЕ(Перечисление.ПредназначенияЭлементовСтруктурыОтчета.БюджетДоходовИРасходов)
	             ...
	             |	ВЫБОР
	             |		КОГДА ОперативныйПлан.Предназначение = ЗНАЧЕНИЕ(Перечисление.ПредназначенияЭлементовСтруктурыОтчета.БюджетДвиженияДенежныхСредств)
	             |			ТОГДА ОперативныйПлан.СуммаДокументаУпр*(ВЫБОР КОГДА ОперативныйПлан.ПриходРасход=ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийПриходРасход.Приход) Тогда 1 Иначе -1 Конец)
	             |		ИНАЧЕ 0
	             |	КОНЕЦ КАК СуммаБДДС_Упр,
	             |	0 КАК СуммаРесурсы_Упр
	...
	Если ТаблицаИнтервала.Количество()=0
		...
		ОбщегоНазначенияУХ.СообщитьОбОшибке(Нстр("ru = 'Ошибка при определении интервала диаграммы.'"),,,СтатусСообщения.Внимание);
...
Процедура СформироватьОтчетПоПроектам(ДиаграммаГанта,ДанныеОбъекта, Сценарий)
	...
	Запрос.Текст=
		...
		|	СУММА(ВЫБОР
		|			КОГДА ОперативныйПлан.ВидБюджета = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыБюджетов.БюджетДоходовИРасходов)
		|				ТОГДА ОперативныйПлан.СуммаДокументаУпр*(ВЫБОР КОГДА ОперативныйПлан.ПриходРасход=ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийПриходРасход.Приход) Тогда 1 Иначе -1 Конец)
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК СуммаБДР,
		|	СУММА(ВЫБОР
		|			КОГДА ОперативныйПлан.ВидБюджета = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыБюджетов.БюджетДвиженияДенежныхСредств)
		|				ТОГДА ОперативныйПлан.СуммаДокументаУпр*(ВЫБОР КОГДА ОперативныйПлан.ПриходРасход=ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийПриходРасход.Приход) Тогда 1 Иначе -1 Конец)
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК СуммаБДДС,
		|	0 КАК СуммаРесурсы
	...
	Для Каждого СтрПроект ИЗ ДанныеПроектов Цикл
		...
		Если НЕ ИтогБДР=0 Тогда
			...
			ТекстИтог=СтрШаблон(Нстр("ru = '%1, БДР = %2'"), ТекстИтог, Формат(ИтогБДР,"ЧДЦ=2; ЧГ=3,0"));
			
		КонецЕсли;
		
		Если НЕ ИтогБДДС=0 Тогда
			
			ТекстИтог=СтрШаблон(Нстр("ru = '%1, БДДС = %2'"), ТекстИтог, Формат(ИтогБДДС,"ЧДЦ=2; ЧГ=3,0"));
			
		КонецЕсли;
		
		Если НЕ ИтогРесурсы=0 Тогда
			
			ТекстИтог=СтрШаблон(Нстр("ru = '%1, Ресурсы = %2'"), ТекстИтог, Формат(ИтогРесурсы,"ЧДЦ=2; ЧГ=3,0"));
			
		КонецЕсли;
	
		ТочкаПроект=ДиаграммаГанта.УстановитьТочку(СтрПроект.Ссылка,СтрПроект.Родитель);
		ТочкаПроект.Текст = ?(ПустаяСтрока(СтрПроект.Стадия),"", 
			СтрШаблон(Нстр("ru = 'Стадия: %1'"), СтрПроект.Стадия) + 
			Символы.ПС +
			СтрПроект.Наименование + ?(ПустаяСтрока(ТекстИтог),"", Символы.ПС + Сред(ТекстИтог,3) ) );
		...
		Пока ДанныеТранзакций.Следующий() Цикл
			...
			Если НЕ ДанныеТранзакций.СуммаБДР=0 Тогда
				...
				ТекстИтог=СтрШаблон(Нстр("ru = '%1, БДР = %2'"), ТекстИтог, Формат(ДанныеТранзакций.СуммаБДР,"ЧДЦ=2; ЧГ=3,0"));
				
			КонецЕсли;
			
			Если НЕ ДанныеТранзакций.СуммаБДДС=0 Тогда
				
				ТекстИтог=СтрШаблон(Нстр("ru = '%1, БДДС = %2'"), ТекстИтог, Формат(ДанныеТранзакций.СуммаБДДС,"ЧДЦ=2; ЧГ=3,0"));
				
			КонецЕсли;
			
			Если НЕ ДанныеТранзакций.СуммаРесурсы=0 Тогда
				
				ТекстИтог=СтрШаблон(Нстр("ru = '%1, Ресурсы = %2'"), ТекстИтог, 
					Формат(ДанныеТранзакций.СуммаРесурсы,"ЧДЦ=2; ЧГ=3,0"));
...
Процедура ПеренестиДокументПланирования(ДокументПланирования,ДатаНачала)
	...
	НовыйПериод = ОбщегоНазначенияУХ.ПолучитьПериодПоДате(ДатаНачала, ДокументПланирования.ПериодОтчета.Периодичность);
...
Процедура ПеренестиПроект(Проект, Сценарий, СмещениеНачало, СмещениеКонец, МассивОбработанныхПроектов)
	...
	Если НЕ МенеджерЗаписи.Выбран() Тогда
		ОбщегоНазначенияУХ.СообщитьОбОшибке(
			СтрШаблон(
				Нстр("ru = 'Не удалось переместить проект %1.
						|Отсутствует информация о датах.'"), 
				Проект) );	
		Возврат;
	КонецЕсли;
	...
	Исключение
		ОбщегоНазначенияУХ.СообщитьОбОшибке(
			СтрШаблон(Нстр("ru = 'Не удалось переместить проект %1.'"), Проект) + 
			Символы.ПС + 
...
Процедура ИзменитьГраницыРодителя(Сценарий, Родитель, ДатаНачала, ДатаОкончания, МассивОбработанныхПроектов)
	...
	Если НЕ МенеджерЗаписи.Выбран() Тогда
		ОбщегоНазначенияУХ.СообщитьОбОшибке(СтрШаблон(
			Нстр("ru = 'Не удалось изменить дату начала проекта %1
				|Отсутствует информация о датах.'"), 
			Родитель));	
		Возврат;
	КонецЕсли;
	...
	Исключение
		ОбщегоНазначенияУХ.СообщитьОбОшибке(
			СтрШаблон(Нстр("ru = 'Не удалось изменить даты проекта %1.'"), Родитель) +
			Символы.ПС +
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()) );	
		Возврат;	
	КонецПопытки;
...
Процедура ПеренестиТранзакцию(ОперативныйПлан, Сценарий, СмещениеНачало, СмещениеКонец, МассивОбработанныхПроектов)
	...
	ПериодОтчетаНачало = ОбщегоНазначенияУХ.ПолучитьПериодПоДате(ДатаНачала, Периодичность);
	...
	Исключение
		ОбщегоНазначенияУХ.СообщитьОбОшибке(
			СтрШаблон(Нстр("ru = 'Не удалось изменить дату транзакции %1.'"), ОперативныйПланОбъект) +
			Символы.ПС + 
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()) );	
		Возврат;	
	КонецПопытки;
...
Процедура УстановитьВидимость()
	...
	Иначе
		Элементы.АтрибутыПроектаЗначение.Заголовок = Нстр("ru = 'Значение'");
