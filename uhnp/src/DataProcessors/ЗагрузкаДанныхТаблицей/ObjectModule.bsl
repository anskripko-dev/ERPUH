...
Функция ПолучитьДанныеТаблицы(ТабличныйДокументИсточник, СтараяТаблица, КэшируемыеЗначения = Неопределено) Экспорт
	...
	ТабличныйДокументИсточник.Записать(ПотокПоТаблице, ТипФайлаТабличногоДокумента.TXT);
	ТаблицаТекстом = ПолучитьСтрокуИзДвоичныхДанных(ПотокПоТаблице.ЗакрытьИПолучитьДвоичныеДанные());
	
	ТаблицаТекстом = СтрЗаменить(ТаблицаТекстом, Символы.ВК, "");
	СтрокиТекста = СтрРазделить(ТаблицаТекстом, Символы.ПС, Ложь);
