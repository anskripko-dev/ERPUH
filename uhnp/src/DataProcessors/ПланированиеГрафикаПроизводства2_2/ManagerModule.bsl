...
Функция НастройкиПланированияПредварительногоГрафика() Экспорт
	...
	Результат.Вставить("ДинамическаяСтруктура");
...
//		* ЗаказНаПроизводство - ДокументСсылка.ЗаказНаПроизводство2_2 - планируемый заказ на производство.
...
//		* КлючПартия - УникальныйИдентификатор - необязательный параметр.
//
Процедура СоздатьВТДоступностьВидовРЦ(МенеджерВременныхТаблиц, ИмяВременнойТаблицы, МассивВРЦ, МассивЭтапов, Начало, Окончание, Параметры) Экспорт
	...
	Распоряжение = Параметры.ЗаказНаПроизводство;
	КНачалу = Параметры.РазмещениеКНачалу;
	ОтсутствиеПрочихЗаказов = Параметры.ОтсутствиеПрочихЗаказов;
	ОтменитьРучныеИзмененияГрафика = Параметры.ОтменитьРучныеИзмененияГрафика;
	КруглосуточнаяРаботаБезВыходных = Параметры.КруглосуточнаяРаботаБезВыходных;
	...
	Запрос.УстановитьПараметр("КруглосуточнаяРаботаБезВыходных", КруглосуточнаяРаботаБезВыходных);
	...
	Если Параметры.Свойство("КлючПартия") Тогда
		Запрос.УстановитьПараметр("КлючПартия", Параметры.КлючПартия);
	Иначе
		Запрос.УстановитьПараметр("КлючПартия", Новый УникальныйИдентификатор());
	КонецЕсли;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВидыРЦ.Исполнитель КАК Исполнитель,
	|	ВЫБОР
	...
	|	И НЕ &КруглосуточнаяРаботаБезВыходных
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ГрафикРаботы
	|;
	...
	Графики = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ГрафикРаботы");
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ГрафикиРаботы.Исполнитель КАК Исполнитель,
	...
	|ГДЕ
	|	Расписание.Период >= &НачалоДня
	|
	|СГРУППИРОВАТЬ ПО
	|	ГрафикиРаботы.Исполнитель,
	...
	|			И ВидыРЦ.ИнтервалПланирования = ЗНАЧЕНИЕ(Перечисление.ТочностьГрафикаПроизводства.Смена)
	|			ТОГДА НАЧАЛОПЕРИОДА(Доступность.ДатаИнтервала, ДЕНЬ)
	|		ИНАЧЕ Доступность.ДатаИнтервала
	|	КОНЕЦ КАК ДатаИнтервала,
	|	ВЫБОР
	|		КОГДА НЕ &КруглосуточнаяРаботаБезВыходных
	|			И ВидыРЦ.ИнтервалПланирования = ЗНАЧЕНИЕ(Перечисление.ТочностьГрафикаПроизводства.Смена)
	...
	|				ИНАЧЕ ВидыРЦ.ИнтервалПланирования
	...
	|		ПО Доступность.Распоряжение = ПриоритетныеЗаказы2_2.Ссылка
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТГраницыСмен КАК ГраницыСмен
	|		ПО Доступность.ВидРабочегоЦентра = ГраницыСмен.Исполнитель
	...
	|			ИЛИ НЕ Доступность.ЭтоДвижениеВводаДоступности
	|				И Доступность.Регистратор ССЫЛКА Документ.РегистраторГрафикаПроизводства
	|				И НЕ Доступность.КлючПартия = &КлючПартия)
...
Функция НастройкиПланированияРабочийГрафикРаспоряжений(ЗаказНаПроизводство, Параметры = Неопределено) Экспорт
	...
	НастройкиПланирования.ЗаказНаПроизводство = ЗаказНаПроизводство;
...
Функция НастройкиПланированияРабочийГрафикЭтапов(ЗаказНаПроизводство, Этапы, Параметры) Экспорт
...
Функция ПланироватьИЗаписатьГрафик(НастройкиПланирования) Экспорт
...
Функция НаименованиеЗаданияРасчетаГрафика() Экспорт
	
	Возврат НСтр("ru = 'Планирование графика производства заказа';
				|en = 'Планирование графика производства заказа'");
...
Функция ПланироватьИЗаписатьГрафикВнутриЗамераВремени(НастройкиПланирования)
	...
	РегистрыСведений.ЗаданияКРасчетуНормативногоГрафикаПроизводства.ЗапуститьЗадание(
		НастройкиПланирования.ЗаказНаПроизводство);
...
Функция ИнициализироватьПараметрыРазмещенияЭтапа(ПараметрыПланирования, Этап)
	...
	Результат.Этап = Этап.ЭтапПроизводства;
	...
	Если ЭтапРазмещен Тогда
		...
		Иначе
			...
			Если НЕ ЭтоЭтапУББВ(Этап) Тогда
				СместитьПредшествующиеЭтапыУББВ(ПараметрыПланирования, Этап, ПараметрыРазмещения.Начало);
			КонецЕсли;
...
Функция НарушенаГраницаРазмещенияЭтапа(ПараметрыПланирования, Этап, ПараметрыРазмещения)
	...
	НарушенаДатаОбеспечения = ЗначениеЗаполнено(ПараметрыРазмещения.ДатаОбеспечения)
		И ПараметрыРазмещения.Начало < ПараметрыРазмещения.ДатаОбеспечения;
	
	НарушеноПредшествование = ЭтапРазмещенВручную(ПараметрыПланирования, Этап)
		И ЗначениеЗаполнено(Этап.ДатаРазмещенияПредшествующих)
		И Этап.ДатаРазмещенияПредшествующих < ПараметрыРазмещения.Окончание;
	
	Возврат НарушенаДатаОбеспечения
		ИЛИ НарушеноПредшествование
		ИЛИ ПараметрыРазмещения.Начало < ПараметрыРазмещения.ПланироватьНеРанее
		ИЛИ ПараметрыРазмещения.Начало < ТекущаяДатаСеанса();
...
Процедура ПослеЗавершенияПланирования(ПараметрыПланирования)
	...
	Если ПараметрыПланирования.РезультатыПланирования.ГрафикЗапланирован Тогда
		...
		ПараметрыПланирования.ЗагрузкаВРЦ.Свернуть("Этап,ВидРабочегоЦентра,ДатаИнтервала,ОкончаниеИнтервала","Занято");
...
Процедура ЗаполнитьВзаимозависимыеРеквизитыЭтаповГрафика(ПараметрыПланирования, РазмещенныеЭтапы)
	...
	Документы.ЭтапПроизводства2_2.СоздатьВТСвязиЭтаповПоРаспоряжению(
		МенеджерВременныхТаблиц,
		ПараметрыПланирования.ЗаказНаПроизводство,
		Истина);
...
Процедура РассчитатьКритическийПуть(ПараметрыПланирования, МенеджерВременныхТаблиц, РазмещенныеЭтапы)
	...
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("Распоряжение", ПараметрыПланирования.ЗаказНаПроизводство);
...
Процедура ДополнитьГрафикЗаписямиДляРасчетаКритическогоПути(ПараметрыПланирования)
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ГрафикЭтаповПроизводства2_2.ЗаказНаПроизводство КАК ЗаказНаПроизводство,
	...
	|	ДОБАВИТЬКДАТЕ(ГрафикЭтаповПроизводства2_2.Окончание, СЕКУНДА, 1) КАК Окончание,
	|	ГрафикЭтаповПроизводства2_2.ОкончаниеПредварительногоБуфера КАК ОкончаниеПредварительногоБуфера,
	|	ВЫБОР
	|		КОГДА ГрафикЭтаповПроизводства2_2.НачалоЗавершающегоБуфера = ГрафикЭтаповПроизводства2_2.Окончание
	...
	|	И НЕ ГрафикЭтаповПроизводства2_2.Этап В (&ЭтапыПроизводства)
	|	И ГрафикЭтаповПроизводства2_2.Этап.Статус В(&СтатусыЭтапа)");
	
	Запрос.УстановитьПараметр("ЗаказНаПроизводство", ПараметрыПланирования.ЗаказНаПроизводство);
	...
	Запрос.УстановитьПараметр("ЭтапыПроизводства", 
		ПараметрыПланирования.ГрафикПроизводства.ВыгрузитьКолонку("Этап"));
	Запрос.УстановитьПараметр("СтатусыЭтапа", СтатусыЭтапМожетБытьЗапланирован());
...
Процедура ОтметитьЭтапНаКритическомПутиРекурсивно(ПараметрыПланирования, ЭтапПроизводства, ЗависимостиЭтапов, ЭтапыРазмещенныеВручную)
	...
	ЗаписьГрафика = ПараметрыПланирования.ГрафикПроизводства.Найти(ЭтапПроизводства, "Этап");
...
Функция ПоследующиеЭтапыКритическогоПути(ПараметрыПланирования, ЗависимостиЭтапов, ЭтапПроизводства)
	...
	Для каждого Этап Из ПредшествующиеЭтапы Цикл
		...
		ЗаписьГрафика = ПараметрыПланирования.ГрафикПроизводства.Найти(Этап, "Этап");
		Если НЕ ЗаписьГрафика = Неопределено Тогда
...
Процедура ДополнитьГрафикЗаписямиСмежныхЭтапов(ПараметрыПланирования, МенеджерВременныхТаблиц, РазмещенныеЭтапы)
	...
	|	ГрафикЭтаповПроизводства2_2.ЗаказНаПроизводство = &ЗаказНаПроизводство
	|	И ГрафикЭтаповПроизводства2_2.СтатусГрафика = &СтатусРабочийГрафик
	|	И ГрафикЭтаповПроизводства2_2.Этап В
	|			(ВЫБРАТЬ
	|				ВТЭтапы.ЭтапПроизводства
	|			ИЗ
	|				ВТЭтапы)
	|	И НЕ ГрафикЭтаповПроизводства2_2.Этап В (&ЭтапыГрафика)");
...
Процедура ЗаполнитьНачалоСледующегоЭтапа(ПараметрыПланирования, ЗависимостиЭтапов, ЭтапПроизводства)
	...
	Для каждого СледующийЭтап Из СледующиеЭтапы Цикл
		...
		ЗаписьСледующийЭтап = ПараметрыПланирования.ГрафикПроизводства.Найти(СледующийЭтап, "Этап");
		...
		Если НЕ ЗаписьСледующийЭтап = Неопределено Тогда
			...
			Если ЗначениеЗаполнено(ЗаписьГрафика.НачалоСледующегоЭтапа) Тогда
				...
				ЗаписьГрафика.НачалоСледующегоЭтапа = МИН(
					...
					ЗаписьСледующийЭтап.Начало);
					
			Иначе
				
				ЗаписьГрафика.НачалоСледующегоЭтапа = ЗаписьСледующийЭтап.Начало;
...
Процедура РазместитьЭтапСРучнымРазмещением(ПараметрыПланирования, Этап, ПараметрыРазмещения)
	...
	Если НЕ ДанныеГрафика = Неопределено Тогда
		...
		СтруктураПоиска = Новый Структура("Этап", Этап.ЭтапПроизводства);
...
Процедура ВыполнитьПроверкиПередПланированием(ПараметрыПланирования, Отказ)
	...
	Если Документы.ЭтапПроизводства2_2.ЕстьЦиклыВЦепочкеЭтапов(ПараметрыПланирования.ЗаказНаПроизводство, ЭтапОшибка) Тогда
...
Процедура РазместитьЭтапУББВ(ПараметрыПланирования, Этап, ПараметрыРазмещения)
	...
	Если ЗначениеЗаполнено(Этап.ДлительностьЭтапа) Тогда
		...
		Если РезультатыРазмещения.РазмещениеВыполнено Тогда
			...
			ЭтапРазмещен = Истина;
			ПараметрыРазмещения.Начало = РезультатыРазмещения.Начало;
			ПараметрыРазмещения.Окончание = РезультатыРазмещения.Окончание;
	...
	Иначе
		...
		ЭтапРазмещен = Истина;
		ПараметрыРазмещения.Начало = ПараметрыРазмещения.МоментРазмещения;
		ПараметрыРазмещения.Окончание = ПараметрыРазмещения.МоментРазмещения;
	...
	Для каждого ПредшествующийЭтап Из ПредшествующиеЭтапы Цикл
		...
		Если ЭтоЭтапУББВ(ПредшествующийЭтап)
			...
			Если РезультатыРазмещения.РазмещениеВыполнено Тогда
				...
				ЗаписьГрафика = ПараметрыПланирования.ГрафикПроизводства.Найти(ПредшествующийЭтап.ЭтапПроизводства, "Этап");
				Если НЕ ЗаписьГрафика = Неопределено Тогда
					
					ЗаписьГрафика.Начало = РезультатыРазмещения.Начало;
					ЗаписьГрафика.Окончание = РезультатыРазмещения.Окончание;
...
Функция ГраницыДняПоРасписанию(ПараметрыПланирования, ГрафикРаботы, ДатаГрафика)
	...
	Если НЕ ПараметрыПланирования.КруглосуточнаяРаботаБезВыходных Тогда
		...
		Для Индекс = 0 По НайденныеСтроки.ВГраница() Цикл
			...
			Если Индекс = 0 Тогда
				Результат.Начало = НайденныеСтроки[Индекс].Начало;
				Результат.Окончание = НайденныеСтроки[Индекс].Окончание;
			Иначе
				Результат.Начало = МИН(Результат.Начало, НайденныеСтроки[Индекс].Начало);
				Результат.Окончание = МАКС(Результат.Окончание, НайденныеСтроки[Индекс].Окончание);
			КонецЕсли;
	...
	Иначе
		...
		Если ИсполнительЭтапа.УчитыватьДоступность Тогда
			...
			Для Сч = 1 По КоличествоИтерацийПоиска Цикл
				...
				Если НЕ ПараметрыПланирования.РазмещениеКНачалу
					И Этап.ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Смена Тогда
					СтруктураПоиска.Вставить("ОкончаниеСмены", ДатаИнтервала);
				Иначе
			...
			Если Результат = Неопределено И ВыполнитьОбходИнтервалов Тогда
				...
				Иначе
					...
					ИмяКолонки = ?(Этап.ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Смена,
						"ОкончаниеСмены",
						"ДатаИнтервала");
					
					Для Сч = 0 По МаксИндекс Цикл
						Строка = НайденныеСтроки[МаксИндекс-Сч];
						Если Строка[ИмяКолонки] <= ДатаИнтервала Тогда
...
Процедура ДобавитьЗагрузкуВРЦ(ПараметрыПланирования, Этап, ИсполнительЭтапа, ПараметрыРазмещения, ИнтервалРазмещения, Длительность)
	...
	НоваяСтрока.ОкончаниеИнтервала = ИнтервалРазмещения.ОкончаниеСмены;
	НоваяСтрока.Этап = Этап.ЭтапПроизводства;
...
Функция ДатаНачальногоИнтервалаББВ(ПараметрыПланирования, Этап, МоментРазмещения)
	...
	Если Этап.ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Смена Тогда
...
Процедура РассчитатьГраницыЗагрузкиВРЦ(ПараметрыПланирования, Этап, ПараметрыРазмещения)
	...
	Если Этап.ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Смена Тогда
		Для каждого ИнтервалЗагрузки Из ПараметрыРазмещения.ЗагрузкаВРЦ Цикл
			...
			ОкончаниеЗагрузки = МАКС(ОкончаниеЗагрузки, ИнтервалЗагрузки.ОкончаниеИнтервала);
		КонецЦикла;
	Иначе
		Для каждого ИнтервалЗагрузки Из ПараметрыРазмещения.ЗагрузкаВРЦ Цикл
			НачалоЗагрузки = МИН(НачалоЗагрузки, ИнтервалЗагрузки.ДатаИнтервала);
...
Процедура РассчитатьГраницыБуферовЭтапа(ПараметрыПланирования, Этап, ПараметрыРазмещения, Отказ)
	...
	Иначе
		...
		ПараметрыРазмещения.Начало = ОкончаниеПредварительногоБуфера;
	...
	Иначе
		...
		ПараметрыРазмещения.Окончание = НачалоЗавершающегоБуфера;
...
Функция ИнициализироватьИнтервалРазмещенияКруглосуточнаяРабота()
	...
	Результат.Вставить("ОкончаниеСмены");
...
// * Заказы - ДокументСсылка.ЗаказНаПроизводство2_2 -
...
Функция ПараметрыПланирования(НастройкиПланирования, ИзменитьНаправлениеПланирования, Отказ)
	...
	ПрочитатьРеквизитыРаспоряжения(ПараметрыПланирования, НастройкиПланирования.ЗаказНаПроизводство);
...
Процедура ПрочитатьЭтапыСЗависимостями(ПараметрыПланирования, Распоряжение, МассивЭтапов, Отказ)
	...
	Если Этапы.Количество() > 0 Тогда
		...
		ЗагрузкаЭтаповРазмещенныхВручную.Индексы.Добавить("Этап");
...
Функция ИнициализироватьЗапросЭтапыСЗависимостями(ПараметрыПланирования, Распоряжение, МассивЭтапов)
	...
	|	ВЫБОР
	|		КОГДА &КруглосуточнаяРаботаБезВыходных
	|			И ЭтапПроизводства2_2.Подразделение.ИнтервалПланирования = ЗНАЧЕНИЕ(Перечисление.ТочностьГрафикаПроизводства.Смена)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТочностьГрафикаПроизводства.ДЕНЬ)
	|		ИНАЧЕ ЭтапПроизводства2_2.Подразделение.ИнтервалПланирования
	|	КОНЕЦ                                                            КАК ИнтервалПланирования,
	...
	|			ТОГДА ЕСТЬNULL(ГрафикПланироватьНеРанее.Окончание, ДАТАВРЕМЯ(1, 1, 1))
	...
	|		ПО ЭтапПроизводства2_2.ПланироватьНеРанее = ГрафикПланироватьНеРанее.Этап
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ГрафикЭтаповПроизводства2_2.Этап                            КАК ЭтапПроизводства,
	|	ГрафикЭтаповПроизводства2_2.Начало                          КАК Начало,
	...
	|	РегистрСведений.ГрафикЭтаповПроизводства2_2 КАК ГрафикЭтаповПроизводства2_2
	|ГДЕ
	|	ГрафикЭтаповПроизводства2_2.Этап В
	|			(ВЫБРАТЬ
	|				ВТЭтапы.ЭтапПроизводства
	|			ИЗ
	|				ВТЭтапы)
	|	И ГрафикЭтаповПроизводства2_2.Этап.РучноеРазмещениеВГрафике
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЭтапПроизводства2_2ЗанятостьВидовРабочихЦентров.Ссылка            КАК Этап,
	...
	Запрос.УстановитьПараметр("СтатусГрафика",
		РегистрыСведений.ГрафикЭтаповПроизводства2_2.СтатусРабочийГрафик());
	Запрос.УстановитьПараметр("КруглосуточнаяРаботаБезВыходных",
		ПараметрыПланирования.КруглосуточнаяРаботаБезВыходных);
...
Процедура ПрочитатьИсполнителиЭтапов(ПараметрыПланирования)
	...
	|	ВЫБОР
	|		КОГДА &КруглосуточнаяРаботаБезВыходных
	|			И ВТИсполнители.ЭтапПроизводства.Подразделение.ИнтервалПланирования = ЗНАЧЕНИЕ(Перечисление.ТочностьГрафикаПроизводства.Смена)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТочностьГрафикаПроизводства.ДЕНЬ)
	|		ИНАЧЕ ВТИсполнители.ЭтапПроизводства.Подразделение.ИнтервалПланирования
	|	КОНЕЦ КАК ИнтервалПланирования,
	|	ВЫБОР
	|		КОГДА &КруглосуточнаяРаботаБезВыходных 
...
Функция РеквизитыГрафикаПроизводства()
	...
	Результат.Вставить("Этап", Новый ОписаниеТипов("ДокументСсылка.ЭтапПроизводства2_2"));
	Результат.Вставить("Начало", Новый ОписаниеТипов("Дата"));
	Результат.Вставить("Окончание", Новый ОписаниеТипов("Дата"));
...
Функция ИнициализироватьПараметрРезультатыПланирования(ПараметрыПланирования)
	...
	РезультатыПланирования.Вставить("Оповещения", Новый Соответствие);
...
Функция ИнициализироватьЗапросДоступностьВРЦ(ПараметрыПланирования)
	...
	|	ВложенныйЗапрос.ОкончаниеИнтервала КАК ОкончаниеСмены,
	|	ВЫБОР
	...
	|		ТДоступность.ОкончаниеИнтервала КАК ОкончаниеИнтервала,
	|		ВЫБОР
...
Функция ИнициализироватьТаблицуУчетаЗагрузкиВРЦ()
	...
	ЗагрузкаВРЦ.Колонки.Добавить("Этап", Новый ОписаниеТипов("ДокументСсылка.ЭтапПроизводства2_2"));
	...
	ЗагрузкаВРЦ.Колонки.Добавить("ОкончаниеИнтервала", Новый ОписаниеТипов("Дата"));
...
Функция РасписаниеББВ(ТаблицаГрафики, Начало, Окончание)
	...
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	КалендарныеГрафики.СоздатьВТРасписанияРаботыНаПериод(
		МенеджерВременныхТаблиц, ТаблицаГрафики.ВыгрузитьКолонку("ГрафикРаботыСсылка"), Начало, Окончание, Ложь);
	...
	|		ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1, 1, 1, 1)
	|	КОНЕЦ КАК ДатаИнтервала,
	|	ДАТАВРЕМЯ(1, 1, 1, 1, 1, 1) КАК ОкончаниеСмены
	|ИЗ
	|	Графики КАК Графики
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТРасписанияРаботы КАК Расписание
	|		ПО Графики.ГрафикРаботыСсылка = Расписание.ГрафикРаботы
	|ГДЕ
	|	Графики.ИнтервалПланирования В (
	...
	|			ИНАЧЕ ДОБАВИТЬКДАТЕ(Расписание.ДатаГрафика, МИНУТА,
	|				ЧАС(Расписание.ВремяОкончания)*60 + МИНУТА(Расписание.ВремяОкончания))
	|		КОНЕЦ) КАК ОкончаниеСмены
	|ИЗ
	|	Графики КАК Графики
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТРасписанияРаботы КАК Расписание
	|		ПО Графики.ГрафикРаботыСсылка = Расписание.ГрафикРаботы
	|ГДЕ
	|	Графики.ИнтервалПланирования = ЗНАЧЕНИЕ(Перечисление.ТочностьГрафикаПроизводства.Смена)
	|	И Расписание.Период >= &Начало
	|
	|СГРУППИРОВАТЬ ПО
	|	Графики.ГрафикРаботы,
	|	Графики.ИнтервалПланирования,
...
Процедура ЗафиксироватьОшибкуЕстьЦиклыВЦепочкеЭтапов(ПараметрыПланирования, ЭтапОшибка)
	...
	Ключ = ПараметрыПланирования.ЗаказНаПроизводство;
	СтатусГрафика = ПараметрыПланирования.СтатусГрафика;
	ОбластьПланирования = ПредставлениеОбластиПланирования(СтатусГрафика);
	...
	Расшифровка.Вставить("Распоряжение", ПараметрыПланирования.ЗаказНаПроизводство);
...
Процедура ДобавитьОповещениеОбИзмененииНаправленияПланирования(ПараметрыПланирования)
	...
	НомерРаспоряжения = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыПланирования.ЗаказНаПроизводство, "Номер"),
		Ложь,
		Истина);
	...
	ПараметрыПланирования.РезультатыПланирования.Оповещения.Вставить(Оповещение, Неопределено);
...
Процедура ПрочитатьСрокиВыпускаПродукции(Параметры, АдресХранилища) Экспорт
	...
	ДинамическаяСтруктура = Параметры.ДинамическаяСтруктура;
	...
	Запрос.УстановитьПараметр("ПустойКлючСвязи",
	...
	Если ДинамическаяСтруктура Тогда
		Запрос.Текст = ТекстЗапросаСрокиВыпускаПродукцииДинамические(МодельЗапланирована);
	Иначе
		Запрос.Текст = ТекстЗапросаСрокиВыпускаПродукции(ГрафикЗапланирован, МодельЗапланирована);
...
Функция ИнициализироватьСрокиВыпускаПродукции()
	...
	Результат.Колонки.Добавить("ДатаПотребности", Новый ОписаниеТипов("Дата"));
	
	Возврат Результат;
	
КонецФункции

Функция ТекстЗапросаСрокиВыпускаПродукцииДинамические(МодельЗапланирована)
	...
	Возврат
		...
		|	ТабличнаяЧасть.Ссылка.ТипПроизводственногоПроцесса КАК ТипПроцесса
		...
		|	ДанныеЗаказа.Склад              КАК Получатель,
		|	ДанныеЗаказа.Назначение         КАК Назначение,
		|	ДанныеЗаказа.КоличествоУпаковок КАК КоличествоУпаковок,
		|	ДанныеЗаказа.Упаковка           КАК Упаковка,
		|	ДанныеЗаказа.НомерСтроки        КАК НомерСтроки,
		|	ДанныеЗаказа.ДатаПотребности    КАК ДатаПотребности,
		|	ДанныеЗаказа.ТипПроцесса        КАК ТипПроцесса,
		|	МИНИМУМ(График.Начало)            КАК НачалоПродукция,
		|	МАКСИМУМ(График.Окончание)        КАК ОкончаниеПродукция,
		|	МИНИМУМ(МодельГрафика.Начало)     КАК НачалоМодельПродукция,
		|	МАКСИМУМ(МодельГрафика.Окончание) КАК ОкончаниеМодельПродукция,
		|	МИНИМУМ(ГрафикИтоги.Начало)            КАК НачалоРаспоряжение,
		|	МАКСИМУМ(ГрафикИтоги.Окончание)        КАК ОкончаниеРаспоряжение,
		|	МИНИМУМ(МодельГрафикаИтоги.Начало)     КАК НачалоМодельРаспоряжение,
		|	МАКСИМУМ(МодельГрафикаИтоги.Окончание) КАК ОкончаниеМодельРаспоряжение
		|ИЗ
		|	ВТДанныеЗаказа КАК ДанныеЗаказа
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтруктураЗаказа КАК СтруктураЗаказа
		|	ПО ДанныеЗаказа.Номенклатура = СтруктураЗаказа.Номенклатура
		|		И ДанныеЗаказа.Характеристика = СтруктураЗаказа.Характеристика
		|		И ДанныеЗаказа.Склад = СтруктураЗаказа.Склад
		|		И ДанныеЗаказа.Назначение = СтруктураЗаказа.Назначение
		|		И ДанныеЗаказа.ЗаказНаПроизводство = СтруктураЗаказа.ЗаказНаПроизводство
		|		И ДанныеЗаказа.КлючНоменклатура = СтруктураЗаказа.КлючНоменклатура
		...
		|	ПО ДанныеЗаказа.ЗаказНаПроизводство = График.ЗаказНаПроизводство
		...
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикЭтаповПроизводства2_2 КАК ГрафикИтоги
		|	ПО ДанныеЗаказа.ЗаказНаПроизводство = ГрафикИтоги.ЗаказНаПроизводство
		|		И ГрафикИтоги.СтатусГрафика = &СтатусРабочийГрафик
		|		И ГрафикИтоги.Этап = ЗНАЧЕНИЕ(Справочник.ЭтапыПроизводства.ПустаяСсылка)
		|		И ГрафикИтоги.КлючПартия = &ПустойКлючСвязи
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикЭтаповПроизводства2_2 КАК МодельГрафика
		|	ПО ДанныеЗаказа.ЗаказНаПроизводство = МодельГрафика.ЗаказНаПроизводство
		|		И МодельГрафика.СтатусГрафика = &СтатусМодельГрафика
		|		И МодельГрафика.Этап = ЗНАЧЕНИЕ(Справочник.ЭтапыПроизводства.ПустаяСсылка)
		|		И СтруктураЗаказа.КлючПартия = МодельГрафика.КлючПартия
		|		И &МодельЗапланирована
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикЭтаповПроизводства2_2 КАК МодельГрафикаИтоги
		|	ПО ДанныеЗаказа.ЗаказНаПроизводство = МодельГрафикаИтоги.ЗаказНаПроизводство
		|		И МодельГрафикаИтоги.СтатусГрафика = &СтатусМодельГрафика
		|		И МодельГрафикаИтоги.Этап = ЗНАЧЕНИЕ(Справочник.ЭтапыПроизводства.ПустаяСсылка)
		|		И МодельГрафикаИтоги.КлючПартия = &ПустойКлючСвязи
		|		И &МодельЗапланирована
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ДанныеЗаказа.Склад,
		|	ДанныеЗаказа.Назначение,
		|	ДанныеЗаказа.КоличествоУпаковок,
		|	ДанныеЗаказа.Упаковка,
		|	ДанныеЗаказа.НомерСтроки,
		|	ДанныеЗаказа.ДатаПотребности,
		|	ДанныеЗаказа.ТипПроцесса
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки";
...
Функция ТекстЗапросаСрокиВыпускаПродукции(ГрафикЗапланирован, МодельЗапланирована)
	...
	Возврат 
		...
		|ИЗ
		|	(ВЫБРАТЬ
		|		РабочийГрафик.Этап КАК ЭтапПроизводства,
		|		РабочийГрафик.Начало КАК НачалоЭтапа,
		|		РабочийГрафик.Окончание КАК ОкончаниеЭтапа,
		...
		|			ПО РабочийГрафик.Этап = ПредварительныйГрафик.Этап
		|				И (ПредварительныйГрафик.СтатусГрафика = &СтатусПредварительныйГрафик)
		|				И (&ГрафикЗапланирован)
		|	ГДЕ
		|		РабочийГрафик.ЗаказНаПроизводство = &Распоряжение
		|		И РабочийГрафик.СтатусГрафика = &СтатусРабочийГрафик
		|		И ПредварительныйГрафик.ЗаказНаПроизводство ЕСТЬ NULL
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|	ГДЕ
		|		ПредварительныйГрафик.ЗаказНаПроизводство = &Распоряжение
		|		И ПредварительныйГрафик.СтатусГрафика = &СтатусПредварительныйГрафик
		|		И &ГрафикЗапланирован
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		...
		|		NULL,
		|		NULL,
		|		МодельГрафика.Начало,
		|		МодельГрафика.Окончание
		|	ИЗ
		|		РегистрСведений.ГрафикЭтаповПроизводства2_2 КАК МодельГрафика
		|	ГДЕ
		|		МодельГрафика.ЗаказНаПроизводство = &Распоряжение
		|		И МодельГрафика.СтатусГрафика = &СтатусМодельГрафика
		...
		|	ТабличнаяЧасть.ДатаПотребности
		|
		|УПОРЯДОЧИТЬ ПО
		|	ТабличнаяЧасть.НомерСтроки";
...
Функция РезультатыПланированияПредварительногоГрафика(
	...
	Для Сч = 1 По 2 Цикл
		...
		Если ЗначениеЗаполнено(РезультатыПланирования) Тогда
			...
			Для каждого КлючИЗначение Из РезультатыПланирования.Оповещения Цикл
...
Процедура ПривестиДатыГрафикаКПользовательскомуПредставлению(ПараметрыПланирования)
	...
	Для каждого Строка Из ПараметрыПланирования.ГрафикПроизводства Цикл
		...
		Если Строка.НачалоЗавершающегоБуфера = Строка.Окончание Тогда
			Строка.НачалоЗавершающегоБуфера = Строка.НачалоЗавершающегоБуфера - 1;
		КонецЕсли;
		Строка.Окончание = Строка.Окончание - 1;
...
Процедура ПриЗаписиРабочегоГрафика(Распоряжение, Этапы, НомерЗадания, ОтменитьРучныеИзмененияГрафика)
	...
	РегистрыСведений.ЗаданияКРасчетуГрафикаПроизводства.УдалитьЗаданияПоНомер(Распоряжение, Этапы, НомерЗадания);
	...
	Если ОтменитьРучныеИзмененияГрафика Тогда
		...
		ЭтапыКОтмене = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Этап");
		
		Документы.ЭтапПроизводства2_2.ОтменитьРучноеРазмещениеЭтаповВГрафике(ЭтапыКОтмене);
	КонецЕсли;
	
	УправлениеПроизводством.ОбновитьДвиженияДокументовПослеЗаписиГрафикаПроизводства(Распоряжение, Этапы);
...
Процедура ПрочитатьКоличествоОбъектовДляПланирования(
	...
	|
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЭтапПроизводства2_2.Ссылка) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаданияКРасчетуГрафикаПроизводства.ЭтапПроизводства) > 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|ГДЕ
	|	Задания.ЗаказНаПроизводство В (&Распоряжения)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|ГДЕ
	|	График.ЗаказНаПроизводство В (&Распоряжения)
	|	И График.СтатусГрафика = &СтатусРабочийГрафик
	|	И График.Этап <> ЗНАЧЕНИЕ(Справочник.ЭтапыПроизводства.ПустаяСсылка)");
	...
	ЕстьЗаписиВГрафике = НЕ МассивРезультатов[3].Пустой();
...
Функция СтатусыЭтапМожетБытьЗапланирован()
