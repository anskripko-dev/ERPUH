		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Выберите соглашение организации ""%1"", для которого возможна отгрузка без перехода права собственности.';
					|en = 'Select the %1 company terms which allow shipment without transfer of title to goods.'"),
				Организация),
			,
			Элемент.Имя);
...
Процедура ЗаполнитьДанныеАвторизацииНаЯндексДиск()  
	...
	ИначеЕсли СрокДоступаНаЯндексДиск > ТекущаяДатаСеанса
		...
		СостояниеАвторизацииТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
									НСтр("ru = 'Доступ предоставлен до %1 (истекает через %2). Для продления авторизации воспользуйтесь командой ""Подключить Яндекс Диск"".';
										|en = 'Access is granted till %1 (expires in %2). To prolong authorization, click Connect Yandex Disk.'"),
	...
	ЧастиСтроки.Добавить(НСтр("ru = 'Авторизация на сервисе Яндекс Диск необходима для выгрузки изображений товара.';
								|en = 'Authorization on the Yandex Disk service is required to export item images.'"));	
	...
	Если СрокДоступаНаЯндексДиск > ТекущаяДатаСеанса
		...
		Иначе
			...
			ПараметрыОжидания.ТекстСообщения                   = НСтр("ru = 'Обновление данных авторизации для доступа на Яндекс Диск.';
																		|en = 'Update authorization data to access Yandex Disk.'");
			...
			ПараметрыОжидания.ОповещениеПользователя.Пояснение = НСтр("ru = 'Завершено Обновление данных авторизации для доступа на Яндекс Диск.';
																		|en = 'Updating authorization data to access Yandex Disk is completed.'");
...
Функция ОбновитьКлючиДоступаПоУчетнойЗаписиНаСервере()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания  = НСтр("ru = 'Яндекс Диск. Обновление данных авторизации.';
															|en = 'Yandex Disk. Update authorization data.'");
...
Процедура ЗаполнениеПодсказкиРеализацияВПути()
	...
	ПодсказкаРеализации = НСтр("ru = 'Заказы покупателей загружаются в документы ""Заказ клиента"" с видом операции ""Реализация"".
		...
		|en = 'Sales orders are imported into the ""Sales order"" documents with the ""Sales"" transaction type.
		|Order shipments are registered using the ""Customer invoice"" documents with the ""Customer invoice — Goods-in-transit"" transaction type with the ""In transit"" status upon transfer to the delivery service or with the ""Sold"" status upon delivery.'")
		+ Символы.ПС;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКорректировкиРеализаций") Тогда
		ПодсказкаРеализации = ПодсказкаРеализации
			+ НСтр("ru = 'Невыкупы и возвраты товаров оформляются документами ""Корректировка реализации"".';
					|en = 'Non-purchases and returns of goods are registered using the ""Sales adjustment"" documents.'");
	Иначе
		ПодсказкаРеализации = ПодсказкаРеализации
			+ НСтр("ru = 'Невыкупы и возвраты товаров оформляются документами ""Возврат товаров от клиента"".';
					|en = 'Non-purchases and returns of goods are registered using the ""Sales return"" documents.'");
	КонецЕсли;

	ПодсказкаРеализации = ПодсказкаРеализации
			+ НСтр("ru = 'Данный способ рекомендуется при ведении учета маркируемых товаров.';
					|en = 'This method is recommended when accounting of marked goods is kept.'");
	...
	Если ПродажиРазделяютсяПоДоговорам Тогда
		...
		Элементы.ТорговаяПлощадкаДоговорFBO.Подсказка =
			НСтр("ru = 'Для передачи товаров на склады торговой площадки и оформления продаж с этих складов.';
				|en = 'For transferring goods to marketplace warehouses and marking sales from those warehouses.'");
		...
		Элементы.ТорговаяПлощадкаДоговорFBS.Заголовок = НСтр("ru = 'Договор для продажи с собственных складов (FBS, RealFBS)';
															|en = 'Contract of sales from own warehouses (FBS, RealFBS)'");
	...
	Иначе
		Элементы.РучнаяЗагрузкаДанныхОРеализованныхТоварах.Заголовок =
			НСтр("ru = 'Доступна загрузка данных за выбранный месяц или из файла отчета о реализации товара';
				|en = 'Data import is available for the selected month or from the sales report file'");
		ДоступноAPI = Истина;
	КонецЕсли;
