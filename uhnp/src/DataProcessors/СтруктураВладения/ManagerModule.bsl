...
Функция СформироватьДвиженияИнвестицийПоТаблицеМоделирования(Сценарий, ВыделенныеСтроки, Втранзакции = Ложь, Отказ = Ложь) Экспорт 
	...
	Для каждого СтрокаТаб Из ВыделенныеСтроки Цикл
		...
		ДокументСоздан = СформироватьДокумент(СтрокаТаб, Отказ, СтрокаТаб.Сценарий);
...
Функция СформироватьДокумент(СтрокаКорректировка, Отказ, Сценарий)
	...
	Если ЗначениеЗаполнено(СтрокаКорректировка.ПрямаяДоляВладения) 
		...
		Если УчетМСФОПовтИспУХ.ЕстьРСБУ() Тогда
			ДокументОбъект = СформироватьДокументРСБУ(СтрокаКорректировка, Сценарий);
		Иначе 
			ДокументОбъект = СформироватьДокументМСФО(СтрокаКорректировка, Сценарий);
		КонецЕсли;
...
Процедура ЗаполнитьВидДоговора(Договор)
	
	ВидДоговора = Неопределено;
	Если ОбщегоНазначения.ПодсистемаСуществует("БухгалтерияПредприятияПодсистемы") Тогда
		ВидДоговора = ПредопределенноеЗначение(Строка("Перечисление.ВидыДоговоровКонтрагентов.Прочее"));		
	Иначе
		ВидДоговора = ПредопределенноеЗначение(Строка("Перечисление.ТипыДоговоров.СПоставщиком"));
	КонецЕсли;
	
	ИмяВидДоговора = ВстраиваниеУХКлиентСервер.ИмяРеквизитаВидДоговора();
	Договор[ИмяВидДоговора] = ВидДоговора;
...
Функция ПолучитьСоответствиеОрганизаций(Таб, СоздаватьОрганизации = Ложь, ПоИНН = Ложь, ВыводитьСообщения = Истина)
	...
	Для Каждого Источник Из ТабПроверки.ВыгрузитьКолонку("тИнвестиция") Цикл
		...
		Если СоздаватьОрганизации И Не ЗначениеЗаполнено(Приемник) Тогда			
			...
			Приемник = ВстраиваниеУХ.СоздатьОрганизацию(Источник, ПоИНН, ДопРеквизиты, ВыводитьСообщения);
...
Функция ПолучитьСоответствиеКонтрагентов(Таб, Создавать = Ложь, ПоИНН = Ложь, ВыводитьСообщения = Истина)
	...
	Для Каждого Источник Из ТабПроверки.ВыгрузитьКолонку("тКонтрагент") Цикл
		...
		Если Создавать И Не ЗначениеЗаполнено(Приемник) Тогда
			ТекстСообщения = Неопределено;
			Приемник = ВстраиваниеУХ.СоздатьКонтрагента(Источник, ПоИНН, ТекстСообщения);
			Если ВыводитьСообщения И Приемник.Пустая() Тогда
				ОбщегоНазначенияУХ.СообщитьОбОшибке(ТекстСообщения);
			КонецЕсли;
		КонецЕсли;
...
Функция СформироватьДокументМСФО(СтрокаКорректировка, Сценарий)
	
	Если СтрокаКорректировка.ПрямаяДоляВладения > 0 Тогда
		ДокументОбъект = Документы["ПоступлениеМСФО"].СоздатьДокумент();
	Иначе 
		ДокументОбъект = Документы["РеализацияМСФО"].СоздатьДокумент();
	КонецЕсли;
	...
	ПрямаяДоля = ОбщегоНазначенияУХ.ЗначениеПоМодулю(
		СтрокаКорректировка.ПрямаяДоляВладения);
		
		
	СтрокаТаб = ДокументОбъект.Позиции.Добавить();
	СтрокаТаб.Актив = СтрокаКорректировка.ОбъектИнвестирования;
	СтрокаТаб.Количество = ПрямаяДоля;
	СтрокаТаб.КоличествоДоп = ПрямаяДоля;
...
Функция СформироватьДокументРСБУ(СтрокаКорректировка, Сценарий)
	...
	ДокументОбъект.ПрямаяДоля = ОбщегоНазначенияУХ.ЗначениеПоМодулю(СтрокаКорректировка.ПрямаяДоляВладения);
	ДокументОбъект.Организация = СтрокаКорректировка.Инвестор;
...
Функция ИмяДокументаИнвестиций(Поступление = Истина)
	
	ИмяДокумента = "ПоступлениеИнвестиций";
	Если Не Поступление Тогда
		ИмяДокумента = "ВыбытиеИнвестиций";
	КонецЕсли;
	
	Если Не УчетМСФОПовтИспУХ.ЕстьРСБУ() Тогда
		ИмяДокумента = "ПоступлениеМСФО";
		Если Не Поступление Тогда
			ИмяДокумента = "РеализацияМСФО";
		КонецЕсли;
	КонецЕсли;
