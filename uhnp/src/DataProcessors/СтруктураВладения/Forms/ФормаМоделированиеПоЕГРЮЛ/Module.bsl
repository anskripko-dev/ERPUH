	...
	Для каждого СтрокаТаб Из Инвестиции Цикл
		...
		Если СтрокаТаб.ОбъектИнвестирования.Пустая() Тогда
			...
			Если СтрокаТаб.ОбъектИнвестирования.Пустая() Тогда
				...
				СтрокаТаб.ОбъектИнвестирования = ВстраиваниеУХ.СоздатьОрганизацию(СтрокаТаб.ИНН, Истина, ДопРеквизиты);
				Если СтрокаТаб.ОбъектИнвестирования.Пустая() Тогда
					Продолжить;				 
				КонецЕсли;		
			КонецЕсли;			
...
Процедура ДосьеОбъектаИнвестирования(Команда)
	...
	ИмяФормы = "Отчет.ДосьеКонтрагента.Форма";
	ОткрытьФорму(ИмяФормы, Новый Структура("ИНН", ТекущаяСтрока.ИНН));
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЕстьДосье = Метаданные.Отчеты.Найти("ДосьеКонтрагента") <> Неопределено;
	Элементы.ИнвестицииОткрытьДосьеОбъектаИнвестирования.Видимость = ЕстьДосье;
