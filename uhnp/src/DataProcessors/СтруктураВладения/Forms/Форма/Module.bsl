...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.СтраницыСвязи.ТекущаяСтраница 
		= ?(Объект.ОтображатьСвязиКИК, Элементы.ГруппаКИК, Элементы.ГруппаМСФО);
...
Процедура СценарийПриИзменении(Элемент)
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		Корректировки, "Сценарий", Объект.Сценарий);
КонецПроцедуры
...
Процедура ЗагрузитьКорректировкиИзФайла(Команда)
	...
	ОбщегоНазначенияКлиентУХ.ПоказатьПомещениеФайла(
		Оповещение, УникальныйИдентификатор, "", ДиалогВыбораФайла);
...
Процедура СоздатьДокументы(Команда)
	...
	Если СозданныеДокументы.Количество() Тогда
		...
		ОтборФормы = Новый Структура("Ссылка", СозданныеДокументы);
		ПараметрыФормы = Новый Структура("Отбор", ОтборФормы);
		ИмяФормыОткрытия = "ЖурналДокументов.ДвижениеИнвестиций.ФормаСписка";
		ОткрытьФорму(ИмяФормыОткрытия, ПараметрыФормы, ЭтаФорма, Истина);
...
Процедура ПредставлениеУзлаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПоляНабораДанных.Добавить(Новый Структура("Поле,ТипЗначения", "Организация",
...
Функция ЗаписатьКорректировкиНаСервере()
	...
	ВыделенныеСтроки = РегистрыСведений.МоделированиеДвиженийИнвестиций
	...
	Попытка
		...
		СозданныеДокументы = Обработки.СтруктураВладения
			.СформироватьДвиженияИнвестицийПоТаблицеМоделирования(
				Объект.Сценарий, ВыделенныеСтроки);
...
Функция ПолучитьВФайлКорректировки()
	...
	Возврат ТиповыеОтчетыУХ.ПолучитьТабличныйДокументПоТаблицеЗначений(
...
Процедура ЗаполнитьУзелПоследовательности(Узел, ПоследовательностьИсточник)
	...
	ИмяКолонки = "ОбъектИнвестирования";
	Если ПоследовательностьТаб.Колонки.Найти(ИмяКолонки) = Неопределено Тогда
		ИмяКолонки = "ОбъектИнвестированияПоследовательности";
	КонецЕсли;
	
	Узел.Представление 
	= Строка(ПоследовательностьИсточник.Инвестор) 
	+ РасчетДолейВладения.ПредставлениеПоследовательностиТаб(ПоследовательностьТаб, ИмяКолонки);
...
Процедура УстановитьЦветПоследовательности()
	ЭтаФорма.УсловноеОформление.Элементы[0].Оформление
		.УстановитьЗначениеПараметра("ЦветФона", Объект.ЦветПоследовательности);
КонецПроцедуры
...
Процедура ОбработатьВыделениеПоследовательностей(Выделить = Истина, СтрокиИсточники = Неопределено)
	...
	Для каждого ИдентификаторСтроки Из СтрокиИсточники Цикл
		...
		Для каждого СтрокаОбработки Из СтрокиОбработки Цикл
			...
			ОтборСтрок = Новый Структура("Последовательность", СтрокаОбработки.Последовательность);
			СтрокиОтбора = Объект.ПоследовательностиУчастия.НайтиСтроки(ОтборСтрок);
			Для каждого СтрокаТаб Из СтрокиОтбора Цикл
...
Процедура ГруппировкаПоИнвестору(Команда)	
	...
	Если Не Элементы.ПоследовательностиУчастия.Видимость Тогда
		Возврат;	
	КонецЕсли;	
...
Процедура ПоследовательностиУчастияВыбранПриИзменении(Элемент)
	...
	ОбработатьВыделениеПоследовательностей(
		Элементы.ПоследовательностиУчастия.ТекущиеДанные.Выбран,
		ОбщегоНазначенияКлиентСерверУХ.ЗначениеВМассиве(ТекущийИдентификатор));
...
Процедура СкомпоноватьОтчетСервер()
	...
	ДанныеРасшифровкиТекущие = Неопределено;
	
	АдресОтчетПоследовательностиУчастия = ПоместитьВоВременноеХранилище(
		Отчет.СхемаКомпоновкиДанных, УникальныйИдентификатор);
	
	Отчет.ЗаполнитьПоОтбору(
		Объект.Сценарий,
		Объект.ДатаАктуальности,
		Объект.Инвестор,
		Объект.ОбъектИнвестирования);
	
	Отчет.СкомпоноватьРезультат(Результат, ДанныеРасшифровкиТекущие);
	
	ДанныеРасшифровки = ПоместитьВоВременноеХранилище(
		ДанныеРасшифровкиТекущие, УникальныйИдентификатор);
...
Процедура РезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ОбработкаРасшифровки = Новый ОбработкаРасшифровкиКомпоновкиДанных(
		ДанныеРасшифровки,
		Новый ИсточникДоступныхНастроекКомпоновкиДанных(АдресОтчетПоследовательностиУчастия));
	...
	ДопПараметры = Новый Структура("ВыбранноеДействие, ПараметрыВыбранногоДействия, Расшифровка",
		ВыбранноеДействие, ПараметрыВыбранногоДействия, Расшифровка);
...
Процедура РезультатОбработкаРасшифровкиЗавершение(ВыбранноеДействие,  ПараметрыВыбранногоДействия, ДополнительныеПараметры) Экспорт
	...
	ОписаниеРасшифровки = Новый ОписаниеОбработкиРасшифровкиКомпоновкиДанных(
...
Функция ПолучитьПараметрыОткрытияФормы(ИмяЭлемента)
	...
	Если ДанныеЛинии.Количество() < 2 Тогда
		...
		Возврат Новый Структура("ИмяФормы,Параметры", 
			...
			Новый Структура("Ключ", ТекОрганизация));
	...
	Возврат Новый Структура("ИмяФормы,Параметры", 
		"ЖурналДокументов.ДвижениеИнвестиций.ФормаСписка",
...
Процедура ДобавитьИзЕГРЮЛ(Команда)
	...
	ПараметрыФормы = Новый Структура("ДанныеЕГРЮЛ,Инвестор,Сценарий", 
		ДанныеЕГРЮЛ, ТекущаяОрганизация, Объект.Сценарий);
		
	ОповещениеОЗакрытии = Новый ОписаниеОповещения(
		"Подключаемый_ОбработкаЗакрытияДанныеЕГРЮЛ", 
		ЭтаФорма, 
		Новый Структура("Инвестор", ТекущаяОрганизация));
	
	ОткрытьФорму("Обработка.СтруктураВладения.Форма.ФормаМоделированиеПоЕГРЮЛ",
...
Функция ПолучитьДанныеПоОбъектамИнвестирования(ТекущаяОрганизация) Экспорт
	...
	Возврат ВстраиваниеУХ.ПолучитьДанныеПоОбъектамИнвестирования(ТекущаяОрганизация.ИНН);
