...
Функция ПолучитьГраф() Экспорт
	...
	РеквизитыСвязи = СтрРазделить(СтрЗаменить(СтрЗаменить(СтрЗаменить(СтрЗаменить(
		ТекущееПредставлениеСвязи, "ЭффективнаяДоля", "ПрямаяДоля"), "ОбъектИнвестирования", "Приемник"), "Инвестор", "Источник"), ".", "_"), ";", Ложь);
...
Функция ПолучитьЦветЛинииЭффективнойДоли(СтрокаТаб, ЦветаОформления)
	...
	ЕстьКИК = ОтображатьСвязиКИК И Метаданные.Перечисления.Найти("СтатусыИностранныхКомпаний") <> Неопределено; 
	
	Если ЕстьКИК Тогда
		
		СтатусКИК = "Перечисление.СтатусыИностранныхКомпаний.КонтролируемаяИностраннаяКомпания";
		СтатусОСУ = "Перечисление.СтатусыИностранныхКомпаний.ОрганизацияСУчастием";
		
		Если СтрокаТаб.СтатусИностраннойКомпании = ПредопределенноеЗначение(СтатусКИК) Тогда
			ЦветаОформления.Свойство("ЦветКИК", ЦветЛинии);
		ИначеЕсли СтрокаТаб.СтатусИностраннойКомпании = ПредопределенноеЗначение(СтатусОСУ) Тогда
...
Процедура ЗаполнитьПоследовательности(ТабПрямыеДоли, ДеревоПоследовательностиУчастия)
	...
	ВыбранныеПоследовательности = ПоследовательностиУчастия
		.Выгрузить(Новый Структура("Выбран", Истина), "Последовательность")
		.ВыгрузитьКолонку("Последовательность");
		...
		Для Каждого УзелОбъектИнвестирования Из УзелИнвестор.Значение Цикл		
			...
			Для Каждого ЭлементПоследовательность Из УзелОбъектИнвестирования.Значение Цикл
				...
				Иначе
					...
					ИмяОбъъектИнвестирования = "ОбъектИнвестированияПоследовательности";
					ИмяИнвестор = "ИнвесторПоследовательности";
					Если ТабПоследовательность.Колонки.Найти(ИмяИнвестор) = Неопределено Тогда
						
						ИмяОбъъектИнвестирования = "ОбъектИнвестирования";
						ИмяИнвестор = "Инвестор";
						
					КонецЕсли;
					
					ПредставлениеПоследовательности = РасчетДолейВладения.ПредставлениеПоследовательности(
						ТабПоследовательность, ИмяОбъъектИнвестирования, ИмяИнвестор);
