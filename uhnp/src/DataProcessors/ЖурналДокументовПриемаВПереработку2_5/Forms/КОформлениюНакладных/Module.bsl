...
Процедура ОформитьОтчеты(Команда)
	...
	Иначе
		...
		Если Не ПараметрыСозданныхДокументов.Отказ Тогда
...
Функция ОформитьПриобретениеПоТаблицеКонтрольОрдеров(ДокументыПоТипамНакладных)
	...
	Если ПолноеИмяРаспоряжения = ПолноеИмяНакладной Тогда
		...
		ИсправительныйДокумент = ИсправлениеДокументов.ПоследнийИсправительныйДокумент(ВыделеннаяСтрока.Ссылка);
		...
		Иначе
			ПараметрыФормы.Вставить("ЕстьИсправление", Истина);
			ПараметрыФормы.Вставить("Ключ",            ИсправительныйДокумент.Ссылка);
			ПараметрыФормы.Вставить("ИмяФормы",        "Документ.ПриемкаТоваровНаХранение.ФормаОбъекта");
		КонецЕсли;
...
Процедура ОбработатьРезультатОформленияДокументов(ПараметрыСозданныхДокументов, ИмяСобытия)
	...
	Если ПараметрыСозданныхДокументов.Свойство("Параметры") Тогда
		...
		Если ПараметрыСозданныхДокументов.Параметры.Свойство("ЕстьИсправление") Тогда
			
			НакладныеКлиент.ПредупреждениеОНаличииИсправления(ПараметрыСозданныхДокументов.Параметры.Ключ,
				ПараметрыСозданныхДокументов.Параметры.ИмяФормы);
...
Функция ОформитьОтчетыНаСервере(ГруппировкиРаспоряжений, ПараметрыФормыСозданныхДокументов, СписокОшибок, ПолеОшибки)
	...
	ПараметрыСозданныхДокументов = Новый Структура("Отказ, ИмяФормы, Параметры", Ложь);
	...
	Если ГруппировкиРаспоряжений.Количество() = 1 И ГруппировкиРаспоряжений[0].МассивЗаказов.Количество() = 1 Тогда
		...
		ПараметрыСозданныхДокументов.Отказ = (РеквизитыШапки = Неопределено);
