...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ПустаяСтрока(ИдентификаторПоля) Тогда
		...
		Если ПользовательскоеПоле =Неопределено Тогда
			...
			СообщениеПользователю.Текст = Нстр("ru = 'Не удалось прочитать данные по произвольному полю'");
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Поле = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = Нстр("ru = 'Не выбран ""Анализируемый объект""'");
		Сообщение.Поле  = "Поле";
		Сообщение.Сообщить();
		Отказ = истина;
	...
	Если Показатель = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = Нстр("ru = 'Не выбран ""Анализируемый параметр""'");
	...
	Если Процент1 + Процент2 + Процент3 <> 100 Тогда
		...
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = Нстр("ru = 'Сумма процентов не равна 100%'");
...
Процедура ПоказательПриИзменении(Элемент)
	...
	Если ПустаяСтрока(Наименование) Тогда
		...
		СтрокаШаблона = Нстр("ru = 'ABC - классификация (%1)'");
		
		Если Не ПустаяСтрока(СтрокаШаблона) тогда							
			Наименование = СтрШаблон(СтрокаШаблона, Показатель);
		Иначе
...
Процедура КомандаОК(Команда)
	...
	Вариант.Представление = НСтр("ru = 'A - Класс'");
	...
	Вариант.Представление = НСтр("ru = 'B - Класс'");
	// Добавим фиктивный отбор
	ЭлементОтбора = Вариант.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.Использование = Ложь;
	...
	Вариант.Представление = НСтр("ru = 'C - Класс'");
	// Добавим фиктивный отбор
	ЭлементОтбора = Вариант.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.Использование = Ложь;
