...
Процедура ОтобратьДанныеДляВыгрузки()
	...
	/////////////////////////////////////////////////////////////////////////////////////////
	// Соответствия полей для обмена ВИБ
	...
	|ГДЕ
	|	ИсточникиДанныхДляРасчетовПравилаИспользованияПолейЗапроса.Ссылка В(&СписокИсточников)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СоответствиеПолейДляОбменаВИБ.Ссылка КАК СоответствиеПолей,
	|	НастройкиСоответствия.НастройкаСоответствия КАК НастройкаСоответствия
	|ИЗ
	|	НастройкиСоответствия КАК НастройкиСоответствия
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СоответствиеПолейДляОбменаВИБ КАК СоответствиеПолейДляОбменаВИБ
	|		ПО НастройкиСоответствия.НастройкаСоответствия = СоответствиеПолейДляОбменаВИБ.Владелец";
	
	Запрос.УстановитьПараметр("СписокИсточников",СписокИсточников);
	
	СписокНастроек=Новый СписокЗначений;
	СписокНастроек.ЗагрузитьЗначения(Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("СоответствиеПолей"));
	
	СтрНастройки=ДеревоМетаданных.Строки.Найти(Метаданные.Справочники.СоответствиеПолейДляОбменаВИБ.Имя,"ПолноеИмяМетаданных",Истина);
	...
	Если Не СтрНастройки=Неопределено Тогда
		...
		НЭлементОтбора.Использование  = Истина;
		НЭлементОтбора.ВидСравнения   = ВидСравненияКомпоновкиДанных.ВСписке;
		НЭлементОтбора.ПравоеЗначение = СписокНастроек;
	...
	СтрНастройки = ДеревоМетаданных.Строки.Найти(Метаданные.РегистрыСведений.НастройкиОбработкиШаблоновДокументов.Имя,"ПолноеИмяМетаданных",Истина);
	
	СтрНастройки.Выгружать=1;
	НЭлементОтбора = СтрНастройки.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ПолеОтбора = Новый ПолеКомпоновкиДанных("ШаблонДокумента");
	НЭлементОтбора.ЛевоеЗначение  = ПолеОтбора;
	НЭлементОтбора.Использование  = Истина;
	НЭлементОтбора.ВидСравнения   = ВидСравненияКомпоновкиДанных.ВСписке;
	...
	НЭлементОтбора = СтрНастройки.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ПолеОтбора = Новый ПолеКомпоновкиДанных("ВерсияРегламента");
	НЭлементОтбора.ЛевоеЗначение  = ПолеОтбора;
	НЭлементОтбора.Использование  = Истина;
	НЭлементОтбора.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
	НЭлементОтбора.ПравоеЗначение = Справочники.ВерсииРегламентовПодготовкиОтчетности.ПустаяСсылка();
	
	НЭлементОтбора = СтрНастройки.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ПолеОтбора = Новый ПолеКомпоновкиДанных("УправлениеПериодом");
	НЭлементОтбора.ЛевоеЗначение  = ПолеОтбора;
	НЭлементОтбора.Использование  = Истина;
	НЭлементОтбора.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
	НЭлементОтбора.ПравоеЗначение = Документы.УправлениеПериодомСценария.ПустаяСсылка();
	...
	СтрВариантыСводныхТаблиц = ДеревоМетаданных.Строки.Найти(Метаданные.Справочники.ВариантыСводныхТаблиц.Имя, "ПолноеИмяМетаданных", Истина);
	
	Если Не СтрВариантыСводныхТаблиц = Неопределено Тогда
		
		СтрВариантыСводныхТаблиц.Выгружать = 1;	
		НЭлементОтбора = СтрВариантыСводныхТаблиц.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		...
		ВыделитьПодчиненные(ДеревоМетаданных, СтрВариантыСводныхТаблиц);
		
	КонецЕсли;
		
	// Настройки состава колонок сводной таблицы 
	Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	ВариантыСводныхТаблиц.Бланк В(&СписокСИерархией)
		|	И ВариантыСводныхТаблиц.ПометкаУдаления = ЛОЖЬ";
		
	Запрос.УстановитьПараметр("СписокСИерархией", СписокБланковСт);
	
	РезЗапроса = Запрос.Выполнить();

	СписокВариантовСводныхТаблиц = Новый СписокЗначений;
	СписокВариантовСводныхТаблиц.ЗагрузитьЗначения(РезЗапроса.Выгрузить().ВыгрузитьКолонку("Ссылка"));
	
	нСтр = ДеревоМетаданных.Строки.Найти(Метаданные.РегистрыСведений.НастройкиСоставаКолонокСводнойТаблицы.Имя, "ПолноеИмяМетаданных", Истина);
	
	Если Не нСтр=Неопределено Тогда
		
		нСтр.Выгружать = 1;
		НЭлементОтбора = нСтр.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ПолеОтбора = Новый ПолеКомпоновкиДанных("ВариантСводнойТаблицы");
		НЭлементОтбора.ЛевоеЗначение  = ПолеОтбора;
		НЭлементОтбора.Использование  = Истина;
		НЭлементОтбора.ВидСравнения   = ВидСравненияКомпоновкиДанных.ВСписке;
		НЭлементОтбора.ПравоеЗначение = СписокВариантовСводныхТаблиц;
		
	КонецЕсли;

	// Настройки состава строк сводной таблицы
	нСтр = ДеревоМетаданных.Строки.Найти(Метаданные.РегистрыСведений.НастройкиСоставаСтрокСводнойТаблицы.Имя, "ПолноеИмяМетаданных", Истина);
	...
	нСтр = ДеревоМетаданных.Строки.Найти(Метаданные.РегистрыСведений.НастройкиРасположенияПолейСводнойТаблицы.Имя, "ПолноеИмяМетаданных", Истина);
	...
	нСтр = ДеревоМетаданных.Строки.Найти(Метаданные.РегистрыСведений.НастройкиПериодовСводнойТаблицы.Имя, "ПолноеИмяМетаданных", Истина);
	...
	нСтр = ДеревоМетаданных.Строки.Найти(Метаданные.РегистрыСведений.НастройкиПолейСводнойТаблицы.Имя, "ПолноеИмяМетаданных", Истина);
	...
	нСтр = ДеревоМетаданных.Строки.Найти(Метаданные.РегистрыСведений.НастройкиВариантаСводнойТаблицы.Имя, "ПолноеИмяМетаданных", Истина);
	...
	нСтр = ДеревоМетаданных.Строки.Найти(Метаданные.РегистрыСведений.НастройкиСоставаАналитикиСтрокСводнойТаблицы.Имя, "ПолноеИмяМетаданных", Истина);
	...
	нСтр = ДеревоМетаданных.Строки.Найти(Метаданные.РегистрыСведений.НастройкиПроизводныхПолейСводнойТаблицы.Имя, "ПолноеИмяМетаданных", Истина);
	...
	нСтр = ДеревоМетаданных.Строки.Найти(Метаданные.РегистрыСведений.НастройкиВычисляемыхВыраженийСводнойТаблицы.Имя, "ПолноеИмяМетаданных", Истина);
	...
	нСтр = ДеревоМетаданных.Строки.Найти(Метаданные.РегистрыСведений.НастройкиУсловногоОформленияСводнойТаблицы.Имя, "ПолноеИмяМетаданных", Истина);
	...
	нСтр = ДеревоМетаданных.Строки.Найти(Метаданные.РегистрыСведений.НастройкиЦелевыхПоказателейМоделирование.Имя, "ПолноеИмяМетаданных", Истина);
	
	Если Не нСтр=Неопределено Тогда
		
		нСтр.Выгружать = 1;
		НЭлементОтбора = нСтр.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ПолеОтбора = Новый ПолеКомпоновкиДанных("ВариантАнализа");
		НЭлементОтбора.ЛевоеЗначение  = ПолеОтбора;
		НЭлементОтбора.Использование  = Истина;
		НЭлементОтбора.ВидСравнения   = ВидСравненияКомпоновкиДанных.ВСписке;
		НЭлементОтбора.ПравоеЗначение = СписокВариантовСводныхТаблиц;
	...
	нСтр = ДеревоМетаданных.Строки.Найти(Метаданные.РегистрыСведений.НастройкиАнализируемыхПоказателейМоделирование.Имя, "ПолноеИмяМетаданных", Истина);
	...
	// Расшифровка групп отборов
	нСтр = ДеревоМетаданных.Строки.Найти(Метаданные.РегистрыСведений.РасшифровкаГруппОтборов.Имя,"ПолноеИмяМетаданных",Истина);
	
	СписокРасшифровокГруппОтборов = Новый СписокЗначений;
	Для Каждого ЭлементСписка Из СписокБланковСт Цикл
		СписокРасшифровокГруппОтборов.Добавить(ЭлементСписка.Значение, ЭлементСписка.Представление);
	КонецЦикла;
	Для Каждого ЭлементСписка Из СписокВариантовСводныхТаблиц Цикл
		СписокРасшифровокГруппОтборов.Добавить(ЭлементСписка.Значение, ЭлементСписка.Представление);
	КонецЦикла;
	...
	Если Не нСтр = Неопределено Тогда
		...
		НЭлементОтбора.Использование  = Истина;
		НЭлементОтбора.ВидСравнения   = ВидСравненияКомпоновкиДанных.ВСписке;
		НЭлементОтбора.ПравоеЗначение = СписокРасшифровокГруппОтборов;
	...
	нСтр = ДеревоМетаданных.Строки.Найти(Метаданные.РегистрыСведений.НастройкиТаблицРаскрываемыхОбластей.Имя, "ПолноеИмяМетаданных", Истина);
	...
	нСтр = ДеревоМетаданных.Строки.Найти(Метаданные.РегистрыСведений.КэшФормулОбластейПоказателей.Имя, "ПолноеИмяМетаданных", Истина);
	...
	Для Каждого ЭлементСписка Из СписокБланковМП Цикл
		СписокРасшифровокГруппОтборов.Добавить(ЭлементСписка.Значение, ЭлементСписка.Представление);
	КонецЦикла;
