...
Процедура ЗаполнитьДоступность()
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(ДоступностьВидовРабочихЦентровОбороты.ЗанятоОборот) КАК ЗанятоОборот,
	|	СУММА(ДоступностьВидовРабочихЦентровОбороты.ДоступностьПоВидуРЦОборот) КАК ДоступностьПоВидуРЦОборот,
	|	СУММА(ДоступностьВидовРабочихЦентровОбороты.ДоступностьПоРЦОборот) КАК ДоступностьПоРЦОборот,
	|	ДоступностьВидовРабочихЦентровОбороты.ВидРабочегоЦентра КАК ВидРабочегоЦентра,
	|	НАЧАЛОПЕРИОДА(ДоступностьВидовРабочихЦентровОбороты.ДатаИнтервала, ДЕНЬ) КАК ДатаИнтервала
	|ИЗ
...
Процедура ЗаполнитьПериодДляВводаНедостающейДоступности()
	...
	Если Выборка.ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Час
		ИЛИ Выборка.ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Смена
...
Функция ЗаписатьНастройкуНаСервере()
	...
	Ссылки = ДоступностьВидовРЦ.Выгрузить(, "ВидРабочегоЦентра").ВыгрузитьКолонку("ВидРабочегоЦентра");
	
	Если ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Смена Тогда
		ПроизводствоСервер.СоздатьВТГрафикиРаботыВидовРЦ(Ссылки, МенеджерВременныхТаблиц, "ВТГрафикиРаботы");
	Иначе
		ТекстЗапроса = 
			"ВЫБРАТЬ ПЕРВЫЕ 0
			|	ЗНАЧЕНИЕ(Справочник.ВидыРабочихЦентров.ПустаяСсылка) КАК Ссылка,
	...
	ТекстЗапроса = 
		...
		|	ДоступностьРабочихЦентров.ВидРабочегоЦентра КАК ВидРабочегоЦентра,
		|	ДоступностьРабочихЦентров.ДатаГрафика КАК ДатаГрафика
		|ПОМЕСТИТЬ ВТДоступностьРЦ
		|ИЗ
		|	Документ.ДоступностьРабочихЦентров КАК ДоступностьРабочихЦентров
		|ГДЕ
		|	ДоступностьРабочихЦентров.Проведен
		|	И ДоступностьРабочихЦентров.ВидРабочегоЦентра В (&ВидыРабочихЦентров)
		|	И ДоступностьРабочихЦентров.ДатаГрафика В (&ДатыГрафика)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Ссылка
		|;
		...
		|	ДоступностьВидаРабочихЦентров.ВидРабочегоЦентра КАК ВидРабочегоЦентра,
		...
		|ГДЕ
		|	ДоступностьВидаРабочихЦентров.Проведен
		|	И ДоступностьВидаРабочихЦентров.ВидРабочегоЦентра В (&ВидыРабочихЦентров)
		|	И ДоступностьВидаРабочихЦентров.ДатаИнтервала В (&ДатыИнтервалов)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ДоступностьРабочихЦентров.ДатаГрафика КАК ДатаГрафика
		|ИЗ
		|	ВТДоступностьРЦ КАК ДоступностьРабочихЦентров
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДоступностьРабочихЦентров.Ссылка КАК Ссылка,
		|	ТабличнаяЧасть.Смена КАК Смена
		|ИЗ
		|	ВТДоступностьРЦ КАК ДоступностьРабочихЦентров
		|
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ДоступностьРабочихЦентров.Доступность КАК ТабличнаяЧасть
		|	ПО ДоступностьРабочихЦентров.Ссылка = ТабличнаяЧасть.Ссылка
		|ГДЕ
		|	НЕ ИСТИНА В (
		|		ВЫБРАТЬ ПЕРВЫЕ 1
		|			ИСТИНА
		...
		|		ГДЕ
		|			Графики.Ссылка = ДоступностьРабочихЦентров.ВидРабочегоЦентра
		|			И Графики.ГрафикРаботы = ТабличнаяЧасть.Смена)
		|	И ТабличнаяЧасть.Смена <> ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
		|";
	ТекстыЗапроса.Добавить(ТекстЗапроса);
	...
	Запрос.УстановитьПараметр("ДатыИнтервалов",
		ДатыВыбранногоПериода(НачалоПериода, ОкончаниеПериода, ИнтервалПланирования));
	...
	ДатыГрафика = РасписаниеГрафиковРаботыРЦ.Скопировать(, "ДатаГрафика");
	ДатыГрафика.Свернуть("ДатаГрафика");
	Запрос.УстановитьПараметр("ДатыГрафика", ДатыГрафика.ВыгрузитьКолонку("ДатаГрафика"));
	
	РезультатыЗапроса = Запрос.ВыполнитьПакет();
	КоличествоРезультатов = РезультатыЗапроса.Количество();
	
	ДоступностьВидовРабочихЦентров = РезультатыЗапроса[КоличествоРезультатов - 3].Выгрузить();
	ДоступностьВидовРабочихЦентров.Индексы.Добавить("ВидРабочегоЦентра, ДатаИнтервала");
	
	ДоступностьРабочихЦентров = РезультатыЗапроса[КоличествоРезультатов - 2].Выгрузить();
	ДоступностьРабочихЦентров.Индексы.Добавить("ВидРабочегоЦентра, ДатаГрафика");
	
	СменыКУдалению = РезультатыЗапроса[КоличествоРезультатов - 1].Выгрузить();
	СменыКУдалению.Индексы.Добавить("Ссылка");
	
	// Запись доступности по виду РЦ
	Для каждого СтрокаДоступность Из ДоступностьВидовРЦ Цикл
		Если НЕ ЗаписатьДоступностьПоВидуРЦ(СтрокаДоступность, 1, ДоступностьВидовРабочихЦентров) 
			ИЛИ НЕ ЗаписатьДоступностьПоВидуРЦ(СтрокаДоступность, 2, ДоступностьВидовРабочихЦентров)
			ИЛИ НЕ ЗаписатьДоступностьПоВидуРЦ(СтрокаДоступность, 3, ДоступностьВидовРабочихЦентров) Тогда
			Возврат Ложь
		КонецЕсли;
	КонецЦикла;
	...
	Для каждого ДатаИВидРЦ Из РасписаниеПоДатам Цикл
		...
		ЗаписьВыполнена = ЗаписатьДоступностьПоРЦ(
			РасписаниеВидаРЦ,
			ДатаИВидРЦ.ВидРабочегоЦентра,
			ДатаИВидРЦ.ДатаГрафика,
...
&НаСервере
Функция ЗаписатьДоступностьПоВидуРЦ(СтрокаДоступность, НомерОтображаемойДоступности, ТаблицаДокументов)
	...
	СтруктураПоиска.Вставить("ВидРабочегоЦентра", СтрокаДоступность.ВидРабочегоЦентра);
	...
	Для каждого ЭлДата Из СписокОтображаемыхДат Цикл
		...
		СтруктураПоиска.ДатаИнтервала = ДатаПериода;
		НайденныеСтроки = ТаблицаДокументов.НайтиСтроки(СтруктураПоиска);
		
		Попытка
			Если НайденныеСтроки.Количество() <> 0 Тогда
				СтрокаТаблицыДокументов = НайденныеСтроки[0]; // СтрокаТаблицыЗначений
			...
			КонецЕсли;
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Ввод доступности видов рабочих центров';
											|en = 'Enter capacity of work center types'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
&НаСервере
Функция ЗаписатьДоступностьПоРЦ(РасписаниеВидаРЦ, ВидРабочегоЦентра, ДатаГрафика, ТаблицаДокументов, СменыКУдалению)
	
	СтруктураПоиска = Новый Структура("ВидРабочегоЦентра, ДатаГрафика", ВидРабочегоЦентра, ДатаГрафика);
	НайденныеСтроки = ТаблицаДокументов.НайтиСтроки(СтруктураПоиска);
	...
	Попытка
		Если НайденныеСтроки.Количество() <> 0 Тогда
			...
			СтруктураПоиска = Новый Структура("РабочийЦентр, Смена");
			Для каждого СтрокаРасписание Из РасписаниеВидаРЦ Цикл
				ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаРасписание);
				
				Для каждого СтрокаИнтервал Из ДокументОбъект.Интервалы.НайтиСтроки(СтруктураПоиска) Цикл
					ДокументОбъект.Интервалы.Удалить(СтрокаИнтервал);
				КонецЦикла;
				
				Для каждого СтрокаДоступность Из ДокументОбъект.Доступность.НайтиСтроки(СтруктураПоиска) Цикл
					ДокументОбъект.Доступность.Удалить(СтрокаДоступность);
				КонецЦикла;
			КонецЦикла;
			
			СтруктураПоиска = Новый Структура("Смена");
			Для каждого СтрокаСмена Из СменыКУдалению.НайтиСтроки(Новый Структура("Ссылка", ДокументОбъект.Ссылка)) Цикл
				СтруктураПоиска.Смена = СтрокаСмена.Смена;
		...
		РучныеИзменения = Новый ТаблицаЗначений();
		РучныеИзменения.Колонки.Добавить("РабочийЦентр", Новый ОписаниеТипов("СправочникСсылка.РабочиеЦентры"));
		РучныеИзменения.Колонки.Добавить("Смена", Новый ОписаниеТипов("СправочникСсылка.Календари"));
		
		ТаблицаДоступность = Новый ТаблицаЗначений();
		ТаблицаДоступность.Колонки.Добавить("РабочийЦентр", Новый ОписаниеТипов("СправочникСсылка.РабочиеЦентры"));
		ТаблицаДоступность.Колонки.Добавить("Смена", Новый ОписаниеТипов("СправочникСсылка.Календари"));
		ТаблицаДоступность.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла()));
		...
		Для каждого СтрокаРасписание Из РасписаниеВидаРЦ Цикл
			...
			ЗаполнитьЗначенияСвойств(ТаблицаДоступность.Добавить(), СтрокаРасписание);
			
			Если СтрокаРасписание.ВнесеныРучныеИзменения Тогда
				ЗаполнитьЗначенияСвойств(РучныеИзменения.Добавить(), СтрокаРасписание);
			КонецЕсли;
		КонецЦикла;
		
		СтруктураПоиска = Новый Структура("РабочийЦентр, Смена");
		ТаблицаДоступность.Свернуть("РабочийЦентр, Смена", "Количество");
		
		Для каждого СтрокаДоступность Из ТаблицаДоступность Цикл
			НоваяСтрока = ДокументОбъект.Доступность.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаДоступность);
			
			ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаДоступность);
			Если РучныеИзменения.НайтиСтроки(СтруктураПоиска).Количество() > 0 Тогда
				НоваяСтрока.ВнесеныРучныеИзменения = Истина;
			КонецЕсли;
		КонецЦикла;
		
		СтрокаРучныеИзменения = ДокументОбъект.Доступность.Найти(Истина, "ВнесеныРучныеИзменения");
		ДокументОбъект.ВнесеныРучныеИзменения = СтрокаРучныеИзменения <> Неопределено;
...
Процедура ПроверитьЗагрузкаНеПревышаетДоступностьГрафика()
	...
	Если ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Смена Тогда
		...
		ДанныеСмен.Колонки.Добавить("ДатаГрафика", Новый ОписаниеТипов("Дата"));
		ДанныеСмен.Индексы.Добавить("РабочийЦентр, Смена, ДатаГрафика");
		СтруктураПоиска = Новый Структура("РабочийЦентр, Смена, ДатаГрафика");
		
		Расписание = ПолучитьИзВременногоХранилища(АдресДанныхГрафикиРаботыРЦ);
		Для Индекс = 0 По Расписание.Количество()-1 Цикл
			Строка = Расписание[Индекс];
			
			ЗаполнитьЗначенияСвойств(СтруктураПоиска, Строка);
			Если ДанныеСмен.НайтиСтроки(СтруктураПоиска).Количество() > 0 Тогда
				Продолжить;
			КонецЕсли;
			
			НоваяСтрока = Интервалы.Добавить();
			НоваяСтрока.ВидРабочегоЦентра = Строка.ВидРабочегоЦентра;
			НоваяСтрока.ДатаИнтервала = Строка.ДатаГрафика
				+ Час(Строка.ВремяНачала) * 3600
				+ Минута(Строка.ВремяНачала) * 60;
			
			ЗаполнитьЗначенияСвойств(ДанныеСмен.Добавить(), Строка);
		КонецЦикла;
		
		Интервалы.Свернуть("ВидРабочегоЦентра, ДатаИнтервала");
	Иначе
...
Процедура ЗаполнитьДоступностьПоВидамРЦ(СписокВидовРЦ)
	...
	РасписанияРаботы = ГрафикиРаботы.РасписанияРаботыНаПериод(
		...
		НачалоПериода,
		ОкончаниеПериода,
		ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Смена);
...
Процедура НастроитьФорму()
	...
	Если ПраваПользователяПовтИсп.ПравоНаВводГрафиковРаботыРЦ() И УправлениеПроизводствомПовтИсп.ИспользоватьПланированиеРаботыРабочихЦентров() Тогда
...
Функция РекомендуемыйПериод(ИмяИзменяемойДаты, НачалоПериода, ОкончаниеПериода, ИнтервалПланирования)
	...
	Если ИнтервалПланирования = ПредопределенноеЗначение("Перечисление.ТочностьГрафикаПроизводства.Час")
		ИЛИ ИнтервалПланирования = ПредопределенноеЗначение("Перечисление.ТочностьГрафикаПроизводства.Смена")
