...
Процедура Подключаемый_ДоступностьНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("Смена",                      ТекущиеДанные.Смена);
	ПараметрыФормы.Вставить("ГрафикиРаботы",              ПолучитьГрафикиРаботы(ТекущиеДанные));
...
Процедура ЗаполнитьДоступность()
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(ГрафикиРаботы.ГрафикРаботы, ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)) КАК Смена,
		|	ЕСТЬNULL(ВЫРАЗИТЬ(ГрафикиРаботы.ГрафикРаботы КАК Справочник.Календари).Наименование, """") КАК ГрафикРаботыПредставление
		|ИЗ
		|	Справочник.РабочиеЦентры КАК РабочиеЦентры
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТГрафикиРаботы КАК ГрафикиРаботы
		|	ПО РабочиеЦентры.Ссылка = ГрафикиРаботы.Ссылка
		|		И &ИнтервалСмена
		|ГДЕ
		...
		|	ДоступностьРабочихЦентровДоступность.Смена КАК Смена
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ГрафикиРаботы.Ссылка КАК РабочийЦентр,
		|	ГрафикиРаботы.ГрафикРаботы КАК ГрафикРаботы,
		|	ВЫРАЗИТЬ(ГрафикиРаботы.ГрафикРаботы КАК Справочник.Календари).Наименование КАК ГрафикРаботыПредставление
		|ИЗ
	...
	Запрос.УстановитьПараметр("ИнтервалСмена", ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Смена);
	
	Результат = Запрос.ВыполнитьПакет();
	ВыборкаРЦ = Результат[0].Выбрать();
	
	ДоступностьРЦ = Результат[1].Выгрузить();
	ДоступностьРЦ.Индексы.Добавить("РабочийЦентр, Смена");
	...
	НоваяДоступностьРЦ.Свернуть("РабочийЦентр,Смена,ДатаГрафика,ВнесеныРучныеИзменения", "Количество");
	
	СтруктураПоискаРабочийЦентрСмена = Новый Структура("РабочийЦентр, Смена");
	СтруктураПоискаРабочийЦентр = Новый Структура("РабочийЦентр");
	...
	Пока ВыборкаРЦ.Следующий() Цикл
		...
		ЗаполнитьЗначенияСвойств(СтруктураПоискаРабочийЦентрСмена, ВыборкаРЦ);
		
		// Сначала возьмем новые данные
		СписокСтрок = НоваяДоступностьРЦ.НайтиСтроки(СтруктураПоискаРабочийЦентрСмена);
		...
		СписокСтрок = ДоступностьРЦ.НайтиСтроки(СтруктураПоискаРабочийЦентрСмена);
		Для каждого ДанныеОДоступности Из СписокСтрок Цикл
			ИмяКолонкиПериода = ИмяКолонкиПериода(ДанныеОДоступности.ДатаГрафика);
		...
		Если НЕ ЗначениеЗаполнено(СтрокаДоступность.ГрафикРаботыПредставление) Тогда
			ЗаполнитьЗначенияСвойств(СтруктураПоискаРабочийЦентр, ВыборкаРЦ);
			СписокСтрок = Графики.НайтиСтроки(СтруктураПоискаРабочийЦентр);
...
Процедура УстановитьРасписаниеЗавершениеНаСервере(РезультатЗакрытия, ДополнительныеПараметры)
	...
	СтруктураПоиска.Вставить("РабочийЦентр", СтрокаДоступностьРЦ.РабочийЦентр);
	СтруктураПоиска.Вставить("Смена", СтрокаДоступностьРЦ.Смена);
	СтруктураПоиска.Вставить("ДатаГрафика", ДополнительныеПараметры.ДатаГрафика);
	...
	Для каждого ДанныеИнтервала Из РезультатЗакрытия.РасписаниеДня Цикл
		...
		СтрокаРасписание.Смена = СтрокаДоступностьРЦ.Смена;
...
Функция ПолучитьДанныеПоРЦ()
	...
	Для Каждого Строка Из ВыделенныеСтроки Цикл
		...
		Если ВыделеннаяСтрока <> Неопределено Тогда
			...
			Графики = ПолучитьГрафикиРаботы(ВыделеннаяСтрока);
			...
			Если НЕ Отказ Тогда
				...
				ЭлементДанных.Вставить("Смена", ВыделеннаяСтрока.Смена);
...
Процедура ЗаполнитьРасписаниеПоГрафикуРабочегоЦентраНаСервере(ДанныеПоРЦ)
	...
	СведенияОНовойДоступностиТаблица.Свернуть("РабочийЦентр,ДатаГрафика,Смена", "Количество");
	СтруктураПоиска = Новый Структура("РабочийЦентр, Смена");
	
	// Обновим данные на форме
	Для каждого СтрокаРЦ Из ДанныеПоРЦ Цикл
		
		ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаРЦ);
...
Процедура ЗавершитьРедактирование()
	...
	Отказ = Ложь;
	СохранитьРасписаниеГрафиковРаботыРЦ(Отказ);
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура СохранитьРасписаниеГрафиковРаботыРЦ(Отказ)
	...
	Для каждого СтрокаДоступность Из ДанныеРЦ Цикл
		Для каждого ДатаПериода Из ДатыВыбранногоПериода Цикл
			...
			СтруктураПоиска = Новый Структура;
			СтруктураПоиска.Вставить("РабочийЦентр", СтрокаДоступность.РабочийЦентр);
			СтруктураПоиска.Вставить("Смена", СтрокаДоступность.Смена);
			...
			Иначе
				...
				НовоеРасписание.Смена = СтрокаДоступность.Смена;
	...
	Если ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Смена
		И РасписаниеГрафиковРаботыРЦ.Количество() > 0 Тогда
		...
		Расписание.Колонки.Добавить("РабочийЦентр", Новый ОписаниеТипов("СправочникСсылка.РабочиеЦентры"));
		Расписание.Колонки.Добавить("Смена", Новый ОписаниеТипов("СправочникСсылка.Календари"));
		Расписание.Колонки.Добавить("ДатаГрафика", Новый ОписаниеТипов("Дата"));
		Расписание.Колонки.Добавить("ВремяНачала", Новый ОписаниеТипов("Дата"));
		Расписание.Колонки.Добавить("ВремяОкончания", Новый ОписаниеТипов("Дата"));
		
		Для Индекс = 0 По РасписаниеГрафиковРаботыРЦ.Количество()-1 Цикл
			СтрокаРасписание = РасписаниеГрафиковРаботыРЦ[Индекс];
			Если СтрокаРасписание.Количество = 0 Тогда
				Продолжить;
			КонецЕсли;
			ЗаполнитьЗначенияСвойств(Расписание.Добавить(), СтрокаРасписание);
		КонецЦикла;
		
		Запрос = Новый Запрос(
			"ВЫБРАТЬ
			|	Расписание.РабочийЦентр КАК РабочийЦентр,
			|	Расписание.Смена КАК Смена,
			...
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	РабочийЦентр, Смена, ДатаГрафика
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			|	Интервалы.ДатаГрафика КАК ДатаГрафика,
			|	ВЫРАЗИТЬ(Интервалы.ВремяНачала КАК ДАТА) КАК ВремяНачала,
			|	ВЫРАЗИТЬ(Интервалы.ВремяОкончания КАК ДАТА) КАК ВремяОкончания
			|ИЗ
			|	Документ.ДоступностьРабочихЦентров.Интервалы КАК Интервалы
			|ГДЕ
			|	Интервалы.Ссылка.Проведен
			|	И (Интервалы.РабочийЦентр, Интервалы.ДатаГрафика) В
			|		(ВЫБРАТЬ
			|			Т.РабочийЦентр КАК РабочийЦентр,
			|			Т.ДатаГрафика КАК ДатаГрафика
			|		ИЗ
			|			ВТРасписание КАК Т)
			|	И (Интервалы.РабочийЦентр, Интервалы.ДатаГрафика, Интервалы.Смена) НЕ В
			|		(ВЫБРАТЬ
			|			Т.РабочийЦентр КАК РабочийЦентр,
			|			Т.ДатаГрафика КАК ДатаГрафика,
			|			Т.Смена КАК Смена
		...
		Запрос.УстановитьПараметр("Расписание", РасписаниеГрафиковРаботыРЦ);
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			ЗаполнитьЗначенияСвойств(Расписание.Добавить(), Выборка);
		КонецЦикла;
		
		ПроверкаИнтервалов = Новый ТаблицаЗначений;
		ПроверкаИнтервалов.Колонки.Добавить("РабочийЦентр", Новый ОписаниеТипов("СправочникСсылка.РабочиеЦентры"));
		ПроверкаИнтервалов.Колонки.Добавить("Начало", Новый ОписаниеТипов("Дата"));
		ПроверкаИнтервалов.Колонки.Добавить("Окончание", Новый ОписаниеТипов("Дата"));
		
		Для Индекс = 0 По Расписание.Количество()-1 Цикл
			СтрокаРасписание = Расписание[Индекс];
			
			ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаРасписание);
			НайденныеСтроки = ДанныеСмен.НайтиСтроки(СтруктураПоиска);
			Если НайденныеСтроки.Количество() = 0 Тогда
				СтрокаСмена = ДанныеСмен.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаСмена, СтрокаРасписание);
			Иначе
				СтрокаСмена = НайденныеСтроки[0];
			КонецЕсли;
			
			СтрокаПроверка = ПроверкаИнтервалов.Добавить();
			СтрокаПроверка.РабочийЦентр = СтрокаРасписание.РабочийЦентр;
			СтрокаПроверка.Начало = СтрокаРасписание.ДатаГрафика
				+ Час(СтрокаРасписание.ВремяНачала) * 3600
				+ Минута(СтрокаРасписание.ВремяНачала) * 60;
			
			Если СтрокаСмена.ПереходящееРасписание
					ИЛИ СтрокаРасписание.ВремяНачала < СтрокаСмена.ОкончаниеДня Тогда
				...
				СтрокаПроверка.Начало = СтрокаПроверка.Начало + 86400;
				
				Если ЗначениеЗаполнено(СтрокаРасписание.ВремяОкончания) Тогда
					СтрокаПроверка.Окончание = СтрокаРасписание.ДатаГрафика
						+ Час(СтрокаРасписание.ВремяОкончания) * 3600
						+ Минута(СтрокаРасписание.ВремяОкончания) * 60
						+ 86400;
				Иначе
					СтрокаПроверка.Окончание = КонецДня(СтрокаПроверка.Начало) + 1;
				КонецЕсли;
			Иначе
				Если НЕ ЗначениеЗаполнено(СтрокаРасписание.ВремяОкончания) Тогда
					СтрокаСмена.ПереходящееРасписание = Истина;
					СтрокаПроверка.Окончание = КонецДня(СтрокаПроверка.Начало) + 1;
				ИначеЕсли СтрокаРасписание.ВремяНачала = СтрокаРасписание.ВремяОкончания
					ИЛИ СтрокаРасписание.ВремяНачала > СтрокаРасписание.ВремяОкончания Тогда
					СтрокаСмена.ПереходящееРасписание = Истина;
					СтрокаПроверка.Окончание = СтрокаРасписание.ДатаГрафика
						+ Час(СтрокаРасписание.ВремяОкончания) * 3600
						+ Минута(СтрокаРасписание.ВремяОкончания) * 60
			...
			СтрокаСмена.ОкончаниеДня = СтрокаРасписание.ВремяОкончания;
		КонецЦикла;
		
		ПроверкаИнтервалов.Сортировать("РабочийЦентр, Начало, Окончание");
		Для Индекс = 1 По ПроверкаИнтервалов.Количество()-1 Цикл
			Если ПроверкаИнтервалов[Индекс].РабочийЦентр = ПроверкаИнтервалов[Индекс-1].РабочийЦентр
				И ПроверкаИнтервалов[Индекс].Начало < ПроверкаИнтервалов[Индекс-1].Окончание Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Графики работы рабочего центра ""%1"" пересекаются %2.';
					...
					ПроверкаИнтервалов[Индекс].РабочийЦентр,
					Формат(ПроверкаИнтервалов[Индекс].Начало, "ДЛФ=D"));
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ПроверкаИнтервалов[Индекс].РабочийЦентр,,, Отказ);
				
				ПоместитьВоВременноеХранилище(РасписаниеГрафиковРаботыРЦКопия, АдресДанныхГрафикиРаботыРЦ);
				СведенияОНовойДоступности.Очистить();
...
&НаКлиенте
Функция ПолучитьГрафикиРаботы(Строка)
	
	Результат = Неопределено;
	
	Если Строка.Смена.Пустая() Тогда
		СтруктураПоиска = Новый Структура("РабочийЦентр", Строка.РабочийЦентр);
		НайденныеСтроки = ГрафикиРаботыРЦ.НайтиСтроки(СтруктураПоиска);
		
		Если НайденныеСтроки.Количество() <> 0 Тогда
			Результат = Новый Массив;
			Для Каждого СтрокаГрафики Из НайденныеСтроки Цикл
				Результат.Добавить(СтрокаГрафики.ГрафикРаботы);
			КонецЦикла;
		КонецЕсли;
	Иначе
		Результат = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Строка.Смена);
	КонецЕсли;
