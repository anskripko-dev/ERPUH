...
Функция СоздатьТаблицуРасписаниеГрафиковРаботыРЦ() Экспорт
	...
	РасписаниеГрафиковРаботыРЦ.Колонки.Добавить("Смена", Новый ОписаниеТипов("СправочникСсылка.Календари"));

	Возврат РасписаниеГрафиковРаботыРЦ;
	
КонецФункции

Функция ЗаполнитьГрафикиРаботыРЦ(ВидРабочегоЦентра, ДанныеПоРЦ, НачалоПериода, ОкончаниеПериода, РасписаниеГрафиковРаботыРЦ, ИнтервалПланирования) Экспорт
	
	РасписаниеГрафиковРаботыРЦКопия = РасписаниеГрафиковРаботыРЦ.Скопировать();
	ИнтервалСмена = (ИнтервалПланирования = Перечисления.ТочностьГрафикаПроизводства.Смена);
	...
	Для каждого СтрокаРЦ Из ДанныеПоРЦ Цикл
		...
		КонецЦикла;
	КонецЦикла;
	РасписанияРаботы = ПланированиеПроизводства.РасписаниеРаботыПоГрафику(
		СписокГрафиков,
		НачалоПериода,
		ОкончаниеПериода,
		ИнтервалСмена);
	...
	СведенияОНовойДоступности.Колонки.Добавить("Смена", Новый ОписаниеТипов("СправочникСсылка.Календари"));
	...
	ПроверкаИнтервалов.Колонки.Добавить("Начало", Новый ОписаниеТипов("Дата",,, Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя)));
	ПроверкаИнтервалов.Колонки.Добавить("Окончание", Новый ОписаниеТипов("Дата",,, Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя)));
	...
	Для каждого СтрокаРЦ Из ДанныеПоРЦ Цикл
		...
		Для ИндексСтроки = 0 По СтарыеДанныеПоВидуРЦ.ВГраница() Цикл
			...
			Если СтрокаРасписание.ДатаГрафика >= НачалоПериода 
				...
				И СтрокаРасписание.РабочийЦентр = СтрокаРЦ.РабочийЦентр
				И (НЕ ИнтервалСмена
					ИЛИ СтрокаРЦ.ГрафикиРаботы.Найти(СтрокаРасписание.Смена) <> Неопределено) Тогда
		...
		Пока ДатаПериода <= ОкончаниеПериода Цикл
			...
			Для каждого ГрафикРаботы Из СтрокаРЦ.ГрафикиРаботы Цикл
				...
				Для каждого СтрокаИнтервал Из РасписаниеПоГрафику Цикл
					...
					Если ИнтервалСмена Тогда
						СтрокаРасписание.Смена = ГрафикРаботы;
					КонецЕсли;
					...
					ОкончаниеДня = СтрокаРасписание.ВремяОкончания;
					СтрокиДобавлены = Истина;
				КонецЦикла;
				
				Если РасписаниеПоГрафику.Количество() = 0 И ИнтервалСмена Тогда
					СтрокаРасписание = РасписаниеГрафиковРаботыРЦ.Добавить();
					СтрокаРасписание.ВидРабочегоЦентра = ВидРабочегоЦентра;
					СтрокаРасписание.РабочийЦентр = СтрокаРЦ.РабочийЦентр;
			...
			Если НЕ СтрокиДобавлены И НЕ ИнтервалСмена Тогда
				СтрокаРасписание = РасписаниеГрафиковРаботыРЦ.Добавить();
				СтрокаРасписание.ВидРабочегоЦентра = ВидРабочегоЦентра;
				СтрокаРасписание.РабочийЦентр = СтрокаРЦ.РабочийЦентр;
				СтрокаРасписание.ДатаГрафика = ДатаПериода;
		...
		ПроверкаИнтервалов.Сортировать("Начало, Окончание");
		Для Индекс = 1 По ПроверкаИнтервалов.Количество()-1 Цикл
			Если ПроверкаИнтервалов[Индекс].Начало < ПроверкаИнтервалов[Индекс-1].Окончание Тогда
				...
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, СтрокаРЦ.РабочийЦентр);
