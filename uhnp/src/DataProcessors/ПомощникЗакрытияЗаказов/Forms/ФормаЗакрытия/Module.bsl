...
Процедура ЗаполнитьПодсказкиИДекорации()
	...
	//++ НЕ УТКА
	ИначеЕсли ТипЗаказа = "ЭтапПроизводства2_2" Тогда
		Заголовок = НСтр("ru = 'Помощник закрытия этапов';
						|en = 'Stage closing wizard'");
		Если ЗаказыКоличество = 1 Тогда
			Заголовок = НСтр("ru = 'Помощник закрытия этапа';
							|en = 'Stage closing wizard'");
		КонецЕсли;
	//-- НЕ УТКА
	Иначе
		Заголовок = НСтр("ru = 'Помощник закрытия заказов';
						|en = 'Order closure wizard'");
		Если ЗаказыКоличество = 1 Тогда
			Заголовок = НСтр("ru = 'Помощник закрытия заказа';
							|en = 'Order closing wizard'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьОтклоненияПоЗаказамПереработчикам()
	...
	|		ОстаткиКОформлению.Склад                      КАК Склад,
	|		ОстаткиКОформлению.Серия                      КАК Серия,
	|		СУММА(ОстаткиКОформлению.КОформлениюОборот)   КАК КОформлениюОстаток,
	|		СУММА(0)                                      КАК ОстатокПоОрдерам,
	|		СУММА(ВЫБОР 
	|			КОГДА ОстаткиКОформлению.ВидДвиженияРегистра = ЗНАЧЕНИЕ(Перечисление.ВидыДвиженияНакопления.Приход)
	|				ТОГДА - ОстаткиКОформлению.КОформлениюОборот
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК СверхЗаказаПоОрдерам,
	|		СУММА(ВЫБОР 
	|			КОГДА ОстаткиКОформлению.ВидДвиженияРегистра = ЗНАЧЕНИЕ(Перечисление.ВидыДвиженияНакопления.Приход)
	|				ТОГДА ОстаткиКОформлению.ЗаказаноОборот
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК Заказано,
	|		СУММА(ВЫБОР 
	|			КОГДА ОстаткиКОформлению.ВидДвиженияРегистра = ЗНАЧЕНИЕ(Перечисление.ВидыДвиженияНакопления.Приход)
	|				ТОГДА ОстаткиКОформлению.ЗаказаноОборот - ОстаткиКОформлению.КОформлениюОборот
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК КВыполнению
	|	ИЗ
	|		РегистрНакопления.РаспоряженияНаОтгрузку.Обороты(&НачПериодЗаказыКлиентов, &КонПериодЗаказыКлиентов,
	|			, Распоряжение В (&ЗаказыПереработчику) И КодСтроки <> 0) КАК ОстаткиКОформлению
	|	СГРУППИРОВАТЬ ПО
	|		ОстаткиКОформлению.Распоряжение,
	|		ОстаткиКОформлению.Номенклатура,
	|		ОстаткиКОформлению.Характеристика,
	|		ИСТИНА,
	|		ОстаткиКОформлению.Склад,
	|		ОстаткиКОформлению.Серия
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|ИЗ
	|	РегистрНакопления.РаспоряженияНаОтгрузку.Обороты(&НачПериодЗаказыКлиентов, &КонПериодЗаказыКлиентов,
	|		, Распоряжение В (&ЗаказыПереработчику)
	|			И ВидДвиженияРегистра = ЗНАЧЕНИЕ(Перечисление.ВидыДвиженияНакопления.Приход)) КАК ТоварыКОтгрузке
	|ГДЕ
	|	ТоварыКОтгрузке.Номенклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины В (&МерныеТипыЕдиницИзмерений)
	|СГРУППИРОВАТЬ ПО
	...
	УстановитьПривилегированныйРежим(Истина);
	ГраницыОборотов = ОбщегоНазначенияУТ.ГраницыОборотовРегистра("РаспоряженияНаОтгрузку",
		"Распоряжение В (&ЗаказыПереработчику)", Запрос.Параметры);
...
Процедура ЗаполнитьОтклоненияПоЗаказамДавальца()
	...
	Запрос = Новый Запрос(" 
	|ВЫБРАТЬ
	|	ОстаткиКОформлению.Распоряжение               КАК ЗаказКлиента,
	|	ОстаткиКОформлению.Номенклатура               КАК Номенклатура,
	|	ОстаткиКОформлению.Характеристика             КАК Характеристика,
	|	ОстаткиКОформлению.Склад                      КАК Склад,
	|	ОстаткиКОформлению.Серия                      КАК Серия, 
	|	СУММА(ОстаткиКОформлению.КОформлениюОборот)   КАК КОформлениюКонечныйОстаток,
	|	СУММА(ОстаткиКОформлению.КПередачеОборот)     КАК КПередачеКонечныйОстаток,
	|	СУММА(ВЫБОР 
	|		КОГДА ОстаткиКОформлению.ВидДвиженияРегистра = ЗНАЧЕНИЕ(Перечисление.ВидыДвиженияНакопления.Приход)
	|		ТОГДА  ОстаткиКОформлению.КОформлениюОборот
	|		ИНАЧЕ 0
	...
	|	СУММА(ВЫБОР 
	|		КОГДА ОстаткиКОформлению.ВидДвиженияРегистра = ЗНАЧЕНИЕ(Перечисление.ВидыДвиженияНакопления.Приход)
	|		ТОГДА  ОстаткиКОформлению.КПередачеОборот
	|		ИНАЧЕ 0
	...
	|ИЗ 
	|	РегистрНакопления.РаспоряженияНаОтгрузку.Обороты(&НачПериодЗаказыКлиентов, &КонПериодЗаказыКлиентов,
	|			, Распоряжение В (&ЗаказыДавальцев) И КодСтроки <> 0) КАК ОстаткиКОформлению
	|СГРУППИРОВАТЬ ПО
	|	ОстаткиКОформлению.Распоряжение,
	|	ОстаткиКОформлению.Номенклатура,
	|	ОстаткиКОформлению.Характеристика,
	...
	|ИЗ
	|	РегистрНакопления.РаспоряженияНаОтгрузку.Обороты(&НачПериодЗаказыКлиентов, &КонПериодЗаказыКлиентов,
	|		, Распоряжение В (&ЗаказыДавальцев)
	|			И ВидДвиженияРегистра = ЗНАЧЕНИЕ(Перечисление.ВидыДвиженияНакопления.Приход)) КАК ТоварыКОтгрузке
	|ГДЕ
	|	ТоварыКОтгрузке.Номенклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины В (&МерныеТипыЕдиницИзмерений)
	|СГРУППИРОВАТЬ ПО
	...
	УстановитьПривилегированныйРежим(Истина);
	ГраницыОборотов = ОбщегоНазначенияУТ.ГраницыОборотовРегистра("РаспоряженияНаОтгрузку",
		"Распоряжение В (&ЗаказыДавальцев)", Запрос.Параметры);
	...
	ТаблицаТоварыКОтмене = Результат[3].Выгрузить();
	ТоварыКОтмене.Загрузить(ТаблицаТоварыКОтмене);
	ДопустимыеОтклонения = Результат[4].Выгрузить();
