...
Процедура ЗаполнитьИнформациюПоЗаказуНаСервере()
	...
	ПредставлениеЗаказа = Новый ФорматированнаяСтрока(СтрШаблон(НСтр("ru = 'Заказ № %1';
																	|en = 'Order No. %1'"), СвойстваЗаказа.Номер), ШрифтыСтиля.ШрифтЗаголовкаОбозначений);
	ДеталиЗаказа = Новый ФорматированнаяСтрока(СтрШаблон(НСтр("ru = 'Сумма: %1 %2';
																|en = 'Amount: %1 %2'"), Формат(СвойстваЗаказа.СуммаДокумента, "ЧДЦ=2; ЧН=0"), СвойстваЗаказа.Валюта));

	ЧастиФорматированнойСтроки = Новый Массив;
	Если ЗначениеЗаполнено(СвойстваЗаказа.Партнер) Тогда
		ЧастиФорматированнойСтроки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Клиент';
																			|en = 'Customer'") + ": ", ШрифтыСтиля.ВажнаяНадписьШрифт));
	...
	Если Не ПустаяСтрока(ТелефонКлиента) Тогда
		...
		КонецЕсли;
		ЧастиФорматированнойСтроки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Телефон';
																			|en = 'Phone number'") + ": ", ШрифтыСтиля.ВажнаяНадписьШрифт));
	...
	Если Не ПустаяСтрока(СвойстваЗаказа.АдресДоставки) Тогда
		Если ЧастиФорматированнойСтроки.Количество() Тогда
			ЧастиФорматированнойСтроки.Добавить(" ");
		КонецЕсли;
		ЧастиФорматированнойСтроки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Адрес доставки';
																			|en = 'Delivery address'") + ": ", ШрифтыСтиля.ВажнаяНадписьШрифт));
	...
	Если Не ПустаяСтрока(СвойстваЗаказа.Комментарий) Тогда
		Если ЧастиФорматированнойСтроки.Количество() Тогда
			ЧастиФорматированнойСтроки.Добавить(" ");
		КонецЕсли;
		ЧастиФорматированнойСтроки.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Комментарий';
																			|en = 'Comment'") + ": ", ШрифтыСтиля.ВажнаяНадписьШрифт));
...
Функция УстановитьКурьераВРаспоряжениях(Распоряжения, Курьер)
	...
	Попытка
		...
		ТекстОшибки = НСтр("ru = 'Не удалось установить блокировку. Установка курьера не выполнена.';
							|en = 'Cannot set the lock. The courier is not set.'");
...
Функция УстановитьСборщикаВРаспоряжениях(Распоряжения, Сборщик)
	...
	Попытка
		...
		ТекстОшибки = НСтр("ru = 'Не удалось установить блокировку. Установка сборщика не выполнена.';
							|en = 'Cannot set the lock. The picker is not set.'");
		Блокировка.Заблокировать();
