...
Процедура УстановитьЗначениеРеквизитаВСтроке(Таблица, СтрокаТаблицы, ИмяРеквизита, ЗначениеРеквизита)
	...
	Исключение
		...
		Если ИсточникДанныхОбъект.ФорматОбмена = Перечисления.ЗагрузкаНСИФорматы.XML Тогда
			МассивСтрок.Добавить(НСтр("ru = 'Неверное значение элемента или атрибута в файле данных.';
										|en = 'Incorrect item or attribute value in the data file.'"));
		ИначеЕсли ИсточникДанныхОбъект.ФорматОбмена = Перечисления.ЗагрузкаНСИФорматы.JSON Тогда
			МассивСтрок.Добавить(НСтр("ru = 'Неверное значение ключа в файле данных.';
										|en = 'Incorrect key value in the data file.'"));
		Иначе
			МассивСтрок.Добавить(НСтр("ru = 'Ошибка установки значения реквизита в строке набора данных.';
										|en = 'An error occurred when setting an attribute value in the data set row.'"));
		КонецЕсли;
		МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Имя: %1';
											|en = 'Name: %1'"), ИмяРеквизита));
		МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Значение: %1';
											|en = 'Value: %1'"), ЗначениеРеквизита));
		ДобавитьЗаписьВПротоколОшибокПоИсключению(Перечисления.ЗагрузкаНСИТипыПроблем.ФорматФайлаДанных,
			СтрСоединить(МассивСтрок, Символы.ПС), ИнформацияОбОшибке());
		Кэш.ОшибкаЗанесенаВПротокол = Истина;
...
Процедура СоздатьОбновитьЗаменыМатериалов()
	...
	Для каждого СтрокаТаблицыДанных Из ТаблицаДанных Цикл
		...
		УстановитьЗначениеРеквизитаОбъекта(ЗагружаемыеДанные, "ВариантЗамены", Перечисления.ВариантЗаменыАналогов.НаборНаНабор);
		
		ЗаполнитьДополнительныеРеквизитыОбъекта(ЗагружаемыеДанные);
		
		ОбластиДействия = ТекущийОбъект.ОбластиДействия.ВыгрузитьКолонки();
		НоваяСтрока = ОбластиДействия.Добавить();
		...
		Если ДанныеЗаполнения.Свойство("ИдентификаторИзделия", ИдентификаторИзделия)
			...
			НоваяСтрока.Изделие = ИдентификаторИзделия.Номенклатура;
			НоваяСтрока.ХарактеристикаИзделия = ИдентификаторИзделия.Характеристика;
		Иначе
			НоваяСтрока.Изделие = Неопределено;
			НоваяСтрока.ХарактеристикаИзделия = Неопределено;
		КонецЕсли;
		
		ИдентификаторТехнологии = Неопределено;
		Если ДанныеЗаполнения.Свойство("ИдентификаторТехнологии", ИдентификаторТехнологии)
				И НЕ ЗначениеЗаполнено(НоваяСтрока.Изделие) Тогда
			НоваяСтрока.Спецификация = ИдентификаторТехнологии;
		Иначе
			НоваяСтрока.Спецификация = Неопределено;
		КонецЕсли;
		
		ЗаполнитьРеквизитыТабличнойЧасти(ЗагружаемыеДанные, ОбластиДействия, "ОбластиДействия");
