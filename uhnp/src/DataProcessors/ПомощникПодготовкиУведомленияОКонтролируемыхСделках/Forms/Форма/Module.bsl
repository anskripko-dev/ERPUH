...
Процедура КомандаВыгрузитьУведомление(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Уведомление) Тогда
		ТекстСообщения = НСтр("ru = 'Уведомление не создано. Его выгрузка не возможна.';
								|en = 'Уведомление не создано. Его выгрузка не возможна.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , "ПредставлениеСозданияУведомления");
	Иначе
		...
		Если НЕ НомераКонтролируемыхСделокКорректны(Объект.Уведомление) Тогда
			...
			|en = 'Нумерация листов 1А не корректна.
			|Перенумеровать листы 1А?
			|(операция может занять продолжительное время)'");
...
Функция ПредставлениеСделок(СделкиСуществуют, ДатаФормированияСпискаСделок)
	...
	СделкиЗаполнены = ?(СделкиСуществуют, НСтр("ru = 'Сформирован %ДатаФормированияСпискаСделок%';
												|en = 'Generated %ДатаФормированияСпискаСделок%'"),
...
Функция ПредставлениеЗаполненияУведомления(УведомлениеЗаполнено, ДатаЗаполненияУведомления)
	...
	Представление = ?(УведомлениеЗаполнено, НСтр("ru = 'Сформировано %ДатаЗаполненияУведомления%';
												|en = 'Generated %ДатаЗаполненияУведомления%'"),
	...
	Если ЗначениеЗаполнено(Объект.Организация)
		...
		Исключение
			...
			ОбщегоНазначения.СообщитьПользователю(НСтр(
				"ru = 'Не удалось создать новое уведомление.';
		...
		Если ПоследнийНомерКорректировкиДокументации > НомерКорректировкиДокументации Тогда
			...
			Если ТипДокументации = 0 Тогда
				ТекстПредупреждения = НСтр("ru = 'Первичная документация относится к другому уведомлению.
					...
					|en = 'Первичная документация относится к другому уведомлению.
					|Переключиться на нее нельзя. Измените уведомление'");
			Иначе
				ТекстПредупреждения = НСтр("ru = 'Корректировка №%1 документации относится к другому уведомлению.
					|Переключиться на нее нельзя. Измените уведомление';
					|en = 'Корректировка №%1 документации относится к другому уведомлению.
					|Переключиться на нее нельзя. Измените уведомление'");
...
Процедура ВыгрузитьДокументациюВФайлыЗавершение(ПолученныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Для Каждого ПолученныйФайл Из ПолученныеФайлы Цикл
		Если ПолученныйФайл <> Неопределено Тогда
			ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				Нстр("ru = 'Выгружен файл %1';
					|en = 'Выгружен файл %1'"), ПолученныйФайл.Имя);
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОповещения);
		КонецЕсли;
	КонецЦикла;
