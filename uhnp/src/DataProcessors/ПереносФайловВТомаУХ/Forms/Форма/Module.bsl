...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ДополнительныеПараметры.ПриОткрытииХранитьФайлыВТомахНаДиске Тогда
		ПоказатьПредупреждение(ОписаниеОповещения, НСтр("ru = 'Не установлен тип хранения файлов ""В томах на диске""'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ ДополнительныеПараметры.ПриОткрытииЕстьТомаХраненияФайлов Тогда 
		ПоказатьПредупреждение(ОписаниеОповещения, НСтр("ru = 'Нет ни одного тома для размещения файлов'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьПереносФайловВТомаЗавершение(Ответ, ПараметрыВыполнения) Экспорт
	...
	Если МассивФайловСОшибками.Количество() <> 0 Тогда
		...
		ОткрытьФорму("Обработка.ПереносФайловВТомаУХ.Форма.ФормаОтчета", ПараметрыФормы);
...
Функция РазмерВерсийВБазе()
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЕСТЬNULL(СУММА(ХранимыеФайлыОрганизацийПрисоединенныеФайлы.Размер), 0) КАК Размер
	|ИЗ
	...
	Если ЗначениеЗаполнено(Параметры.ХранимыйФайл) Тогда
		
		Запрос.Текст=Запрос.Текст+"
		|И ХранимыеФайлыОрганизацийПрисоединенныеФайлы.ВладелецФайла=&ХранимыйФайл";
		
		Запрос.УстановитьПараметр("ХранимыйФайл",Параметры.ХранимыйФайл);
...
Функция ЧислоВерсийВБазе()
	...
	Возврат ХранимыеФайлыУХ.ЧислоВерсийВБазе(Параметры.ХранимыйФайл);
...
Функция ВерсииВБазе()
	...
	|	ХранимыеФайлыОрганизацийПрисоединенныеФайлы.Наименование КАК Наименование,
	|	ХранимыеФайлыОрганизацийПрисоединенныеФайлы.Размер КАК Размер,
	|	ХранимыеФайлыОрганизацийПрисоединенныеФайлы.ВладелецФайла
	|ИЗ
	|	Справочник.ХранимыеФайлыОрганизацийПрисоединенныеФайлы КАК ХранимыеФайлыОрганизацийПрисоединенныеФайлы
	|ГДЕ
	|	ХранимыеФайлыОрганизацийПрисоединенныеФайлы.ТипХраненияФайла = &ТипХраненияФайла";
	...
	Для Каждого Строка Из ТаблицаВыгрузки Цикл
		ВерсияСтруктура = Новый Структура("Ссылка,Владелец, Текст, Размер", 
			Строка.Ссылка, Строка.ВладелецФайла, Строка.Наименование, Строка.Размер);
...
Функция ПеренестиВерсиюВТом(ВерсияСтруктура, МаксимальныйРазмерФайла, МассивФайловСОшибками)
	...
	ФайлСсылка = ВерсияСтруктура.Владелец;
	...
	ДанныеВерсии=РаботаСФайлами.ДанныеФайла(ВерсияСсылка);
	
	Если Не ЗначениеЗаполнено(ДанныеВерсии.СсылкаНаДвоичныеДанныеФайла) Тогда
	...
	Попытка
		...
		ДвоичныеДанные=ПолучитьИзВременногоХранилища(ДанныеВерсии.СсылкаНаДвоичныеДанныеФайла);
		
		СведенияОФайле = Обработки.ПереносФайловВТомаУХ.ДобавитьФайлВТом(ДвоичныеДанные, ВерсияОбъект.ДатаМодификацииУниверсальная,
				ВерсияОбъект.Наименование, ВерсияОбъект.Расширение, , ВерсияОбъект.Зашифрован,ВерсияОбъект.ДатаМодификацииУниверсальная);
