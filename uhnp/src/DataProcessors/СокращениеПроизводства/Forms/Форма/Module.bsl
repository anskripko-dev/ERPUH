...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РезультатЗаданияСостояниеЗаказов = РезультатРасчета;
...
Функция ПолучитьСостояниеЗаказовНаСервере()
	...
	ПарметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПарметрыВыполнения.ЗапуститьВФоне = Истина;
	ПарметрыВыполнения.ОжидатьЗавершение = 0;
	ПарметрыВыполнения.НаименованиеФоновогоЗадания =
		НСтр("ru = 'Получение состояния выполнения заказов на производство';
...
Процедура ЗаполнитьСостояниеЗаказовНаСервере(СохранитьПараметрыОтбора = Ложь)
	...
	ПараметрыКоманды = Новый Структура;
	ПараметрыКоманды.Вставить("АдресХранилища", РезультатЗаданияСостояниеЗаказов.АдресРезультата);
...
&НаКлиенте
Процедура ЗаполнитьСостояниеЗаказовОповщение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ОткрытьФормуДлительнойОперации()
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ЗаполнитьСостояниеЗаказовОповщение", ЭтотОбъект);
	
	ДлительныеОперацииКлиент.ОжидатьЗавершение(
		РезультатЗаданияСостояниеЗаказов, ОповещениеОЗавершении, ПараметрыОжидания);
