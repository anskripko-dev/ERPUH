...
Процедура ЗаполнитьОтбор(Штрихкод)
	...
	ИначеЕсли (ТипЗнч(Ссылка) = Тип("СправочникСсылка.ФизическиеЛица")
		...
		Исполнитель = Ссылка;
		//++ Локализация
		Исполнитель = ПодобратьИсполнителя(Ссылка);
		//-- Локализация
...
Функция ПодобратьИсполнителя(Ссылка)
	Возврат Обработки.ВыполнениеОпераций2_2.ПодобратьИсполнителя(Ссылка);
КонецФункции
...
Функция ПолучитьСсылку(Штрихкод)
	...
	//++ Локализация
	Менеджеры.Добавить(ПредопределенноеЗначение("Справочник.Сотрудники.ПустаяСсылка"));
	//-- Локализация
