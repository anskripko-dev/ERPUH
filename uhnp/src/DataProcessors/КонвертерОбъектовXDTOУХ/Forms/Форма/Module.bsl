...
&НаКлиенте
Процедура РедактироватьФункциюПолученияДопПараметровПеробразования(Команда)
	Подсказка = Новый ФорматированнаяСтрока(
		НСтр("ru='Значение доп. параметров необходимо добавить в структуру"
			   + " с именем Параметры'"));
	ВвестиБольшуюСтрокуВПолеФормы(
		"ТекстФункцииПолученияДопПараметровДляПреобразованияВXML",
		НСтр("ru='Введите текст функции для получения доп. параметров'"),
		"",
		Подсказка);
КонецПроцедуры

&НаКлиенте
Процедура ИспользоватьДопПараметрыДляПреобразованияПриИзменении(Элемент)
	Элементы.РедактироватьФункциюПолученияДопПараметровПеробразования.Доступность = 
		ИспользоватьДопПараметрыДляПреобразования;
КонецПроцедуры
...
Процедура КодыПространствИменXSDТекстXSDНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ВвестиБольшуюСтрокуВЯчейкуТЗ("Объект.ПространстваИменXSD",
		"ТекстXSD",
		"",
		ИдСтроки,
...
Процедура МакросыТекстНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ВвестиБольшуюСтрокуВЯчейкуТЗ("Объект.Макросы",
		"Текст",
		"",
		ИдСтроки,
...
&НаКлиенте
Процедура Значения1СВXDTOВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Поле.Имя = "Значения1СВXDTOОстановкаПриОтладке" Тогда
		СтандартнаяОбработка = Ложь;
		ПереключитьФлагОтладкиВСтрокеТаблицы(Элемент.Имя);
	КонецЕсли;
КонецПроцедуры
...
Процедура Объекты1СВXDTOФункцияПередЗаполнениемНачалоВыбора(Элемент,
	...
	Если СпособПолучения = "МассивПравил" Тогда
		СтандартнаяОбработка = Ложь;
		ВвестиБольшуюСтрокуВЯчейкуТЗ("Объект.Объекты1СВXDTO",
...
&НаКлиенте
Процедура Объекты1СВXDTOВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Поле.Имя = "Объекты1СВXDTOОстановкаПриОтладке" Тогда
		СтандартнаяОбработка = Ложь;
		ПереключитьФлагОтладкиВСтрокеТаблицы(Элемент.Имя);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ПравилаПреобразованияРеквизитов1СВXDTOВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Поле.Имя = "ПравилаПреобразованияРеквизитов1СВXDTOОстановкаПриОтладке" Тогда
		СтандартнаяОбработка = Ложь;
		ПереключитьФлагОтладкиВСтрокеТаблицы(Элемент.Имя);
	КонецЕсли;	
КонецПроцедуры
...
Процедура ПравилаПреобразованияРеквизитов1СВXDTOИсточникПолученияНачалоВыбора(
	...
	Если ТекДанные <> Неопределено Тогда
		Если ТекДанные.СпособПолучения = "Функция" Тогда
			ВвестиБольшуюСтрокуВЯчейкуТЗ("Объект.ПравилаПреобразованияРеквизитов1СВXDTO",
				"ИсточникПолучения",
				"",
				Элементы.ПравилаПреобразованияРеквизитов1СВXDTO.ТекущаяСтрока,
				Нстр("ru='Введите текст функции'"),
...
&НаКлиенте
Процедура ИмяТипаXSDДляПреобразованияНачалоВыбора(Элемент,
												ДанныеВыбора,
												СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ДанныеВыбора = Неопределено;
	Оповещение = Новый ОписаниеОповещения(
		"ОбработкаВыбораИмениТипаXSDДляПреобразования",
		ЭтаФорма);
	ПоказатьВыборИзСписка(
		Оповещение,
		Элементы.ИмяТипаXSDДляПреобразования.СписокВыбора,
		Элемент);
КонецПроцедуры
	
&НаКлиенте
Процедура ИмяТипаXSDДляПреобразованияПриИзменении(Элемент)
	УстановитьОформлениеДопПараметровПреобразованияНаСервере();
КонецПроцедуры
...
Процедура ВыбратьЗначениеЗначенияXDTO(ИдСтроки, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если СпособПолучения = "XML" ИЛИ СпособПолучения = "Функция" Тогда
		СтандартнаяОбработка = Ложь;
		ВвестиБольшуюСтрокуВЯчейкуТЗ("Объект.Значения1СВXDTO",
			"БольшаяСтрока",
...
// Процедура - Ввести большую строку в ячейку таблицы значений.
//
// Параметры:
...
//  ИмяПодсказки - Строка - имя подсказки, которая будет показана в форме
//		редактирования.
//
&НаКлиенте
Процедура ВвестиБольшуюСтрокуВЯчейкуТЗ(ПутьКТаблицеФормы,
										ИмяРеквизитаЗначения, 
	...
	ОбработчикИзмененияТекста = Новый ОписаниеОповещения(
		"ОбработатьИзменениеТекстаДляЯчейкиТЗ", ЭтаФорма, ДопПараметры);
	ОткрытьФормуРедактированияФорматированногоТекста(
		ТекЗначение, ОбработчикИзмененияТекста, Заголовок, ИмяПодсказки, Подсказка);
КонецПроцедуры
...
//
&НаКлиенте
Процедура ВвестиБольшуюСтрокуВПолеФормы(ИмяРеквизитаЗначения, 
										Заголовок="",
										ИмяПодсказки="",
										Подсказка="")
	ТекЗначение = ЭтаФорма[ИмяРеквизитаЗначения];
	ДопПараметры = Новый Структура;
	ДопПараметры.Вставить("ИмяРеквизитаЗначения", ИмяРеквизитаЗначения);
	ДопПараметры.Вставить("СтароеЗначение", ТекЗначение);
	ОбработчикИзмененияТекста = Новый ОписаниеОповещения(
		"ОбработатьИзменениеТекстаДляПоляФормы", ЭтаФорма, ДопПараметры);
	ОткрытьФормуРедактированияФорматированногоТекста(
		ТекЗначение, ОбработчикИзмененияТекста, Заголовок, ИмяПодсказки, Подсказка);
КонецПроцедуры
...
Процедура ОткрытьФормуРедактированияФорматированногоТекста(ТекстПрограммы,
	...
	ПараметрыРедактирования.Вставить("ТекстПодсказки", ТекстПодсказки);
...
&НаКлиенте
Процедура ОбработатьИзменениеТекстаДляЯчейкиТЗ(Результат, ДопПараметры) Экспорт
...
&НаКлиенте
Процедура ОбработатьИзменениеТекстаДляПоляФормы(Результат, ДопПараметры) Экспорт
	Если Результат = Неопределено ИЛИ
			ДопПараметры.СтароеЗначение = Результат Тогда
		Возврат;
	КонецЕсли;
	ЭтаФорма[ДопПараметры.ИмяРеквизитаЗначения] = Результат;
КонецПроцедуры
...
Процедура ОбработатьИзменениеСсылки1СДляПреобразования()
	УстановитьСписокТиповXSDДляПреобразованияНаСервере();
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция ПолучитьСтрокиПравилДляИмениТипа1С(Объект, ИмяТипа1СДляПреобразования)
	Отбор = Новый Структура;
	Отбор.Вставить("ИмяТипаИсточника", ИмяТипа1СДляПреобразования);
	Отбор.Вставить("Отключить", Ложь);
	мСтрок = Объект.Объекты1СВXDTO.НайтиСтроки(Отбор);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		мСтрок,
		Объект.Значения1СВXDTO.НайтиСтроки(Отбор));
	Возврат мСтрок;
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Процедура ДобавитьВСписокИменТиповXSD(СписокПриемник, мСтрокиПравил)
	Для Каждого СтрокаПравила Из мСтрокиПравил Цикл
		ДобавитьПравилоВСписокИменТиповXSD(СписокПриемник, СтрокаПравила);
	КонецЦикла;
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ДобавитьПравилоВСписокИменТиповXSD(СписокПриемник, СтрокаПравила)
	ПредставлениеПравила = СтрокаПравила["ИмяТипаПриемника"];
	Если СтрокаПравила.ЕстьДопПараметры Тогда
		ПредставлениеПравила = ПредставлениеПравила + " (доп. парам.)";
	КонецЕсли;
	СписокПриемник.Добавить(
		СтрокаПравила["ИмяТипаПриемника"],
		ПредставлениеПравила,
		СтрокаПравила.ЕстьДопПараметры);
КонецПроцедуры
...
Процедура УстановитьОтладкуДляФормы(Отладка);
	...
	Элементы.Объекты1СВXDTOОстановкаПриОтладке.Видимость = Отладка;
	Элементы.Значения1СВXDTOОстановкаПриОтладке.Видимость = Отладка;
	Элементы.ПравилаПреобразованияРеквизитов1СВXDTOОстановкаПриОтладке.Видимость = Отладка;
КонецПроцедуры
...
Процедура УстановитьФлагОтладки(ИспользоватьОтладку)
	...
	УстановитьОтладкуДляФормы(ИспользоватьОтладку);
КонецПроцедуры

&НаКлиенте
Процедура ОбработкаВыбораИмениТипаXSDДляПреобразования(ВыбранныйЭлемент,
													   ДопПараметры) Экспорт
	Если ВыбранныйЭлемент <> Неопределено Тогда
		ИспользоватьДопПараметрыДляПреобразования = ВыбранныйЭлемент.Пометка;
		ИмяТипаXSDДляПреобразования = ВыбранныйЭлемент.Значение;
	Иначе
		ИспользоватьДопПараметрыДляПреобразования = Ложь;
		ИмяТипаXSDДляПреобразования = "";
	КонецЕсли;
	ИмяТипаXSDДляПреобразованияПриИзменении(
		Элементы.ИмяТипаXSDДляПреобразования);
КонецПроцедуры

&НаКлиенте
Процедура ПереключитьФлагОтладкиВСтрокеТаблицы(ИмяТаблицы)
	ТекДанные = Элементы[ИмяТаблицы].ТекущиеДанные;
	Если ТекДанные <> Неопределено Тогда
		ТекДанные.ОстановкаПриОтладке = ?(ТекДанные.ОстановкаПриОтладке <> 0, 0, 1);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПеробразоватьОбъект1СВXMLНаСервере()
	...
	Если Элементы.ФормаОтладка.Пометка Тогда
		...
		Конвертер.УстановитьПравилаДляОстанова(
			ПолучитьПравилаДляОстанова());
	Иначе
	...
	РезультатТекстXML = Конвертер.ПреобразоватьПоУмолчаниюОбъект1СВТекстXML(
		...
		Неопределено,
		ИмяТипаXSDДляПреобразования,
		ПолучитьДопПараметрыДляПреобразованияНаФорме());
...
&НаСервере
Функция ПолучитьДопПараметрыДляПреобразованияНаФорме()
	Перем Параметры;
	Если ИспользоватьДопПараметрыДляПреобразования Тогда
		Если ПустаяСтрока(
				ТекстФункцииПолученияДопПараметровДляПреобразованияВXML) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Не задана функция получения доп. параметров!";
			Сообщение.Поле = "ИспользоватьДопПараметрыДляПреобразования";
			Сообщение.Сообщить();
		Иначе
			Параметры = Новый Структура;
			УпакованРезультат =
				Обработки.КонвертерОбъектовXDTOУХ.ВыполнитьВБезопасномРежиме(
					ТекстФункцииПолученияДопПараметровДляПреобразованияВXML,
					Параметры);
			Если НЕ УпакованРезультат.ЕстьОшибка Тогда
				Возврат Параметры;
			КонецЕсли;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = УпакованРезультат.Ошибка;
			Сообщение.Поле = "ИспользоватьДопПараметрыДляПреобразования";
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыФормыМетаданных1С()
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		мТипы1С,
		Объект.Значения1СВXDTO.Выгрузить(, "ИмяТипаИсточника")
				.ВыгрузитьКолонку(0),
		Истина);
...
&НаСервере
Процедура УстановитьСписокТиповXSDДляПреобразованияНаСервере()
	СписокВыбора = Элементы.ИмяТипаXSDДляПреобразования.СписокВыбора;
	СписокВыбора.Очистить();
	Если ЗначениеЗаполнено(СсылкаДляПреобразования) Тогда
		ДобавитьВСписокИменТиповXSD(
			СписокВыбора,
			ПолучитьСтрокиПравилДляИмениТипа1С(
				Объект, ИмяТипа1СДляПреобразования));
	Иначе
		ДобавитьВСписокИменТиповXSD(СписокВыбора, Объект.Объекты1СВXDTO);
		ДобавитьВСписокИменТиповXSD(СписокВыбора, Объект.Значения1СВXDTO);
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура УстановитьОформлениеДопПараметровПреобразованияНаСервере()
	Если ЗначениеЗаполнено(ИмяТипаXSDДляПреобразования) Тогда
		Элементы.ИспользоватьДопПараметрыДляПреобразования.Видимость =
			ИспользоватьДопПараметрыДляПреобразования;
		Элементы.РедактироватьФункциюПолученияДопПараметровПеробразования.Видимость
			= ИспользоватьДопПараметрыДляПреобразования;
	Иначе
		Элементы.ИспользоватьДопПараметрыДляПреобразования.Видимость = Ложь;
		Элементы.РедактироватьФункциюПолученияДопПараметровПеробразования.Видимость
			= Ложь;
	КонецЕсли;
КонецПроцедуры

// Возвращает соответствие, у которого:
//	- ключ - состоит из имени таблицы и номера строки правила.
//			 Имя таблицы - это Объекты1СВXDTO, Значения1СВXDTO
//			 или ПравилаПреобразованияРеквизитов1СВXDTO.
//	- значение - номер строки правила.
//
&НаСервере
Функция ПолучитьПравилаДляОстанова()
	Результат = Новый Соответствие;
	ДобавитьПравилаОстановкиИзТаблицы(
		Результат, "Значения1СВXDTO", Объект.Значения1СВXDTO);
	ДобавитьПравилаОстановкиИзТаблицы(
		Результат, "Объекты1СВXDTO", Объект.Объекты1СВXDTO);
	ДобавитьПравилаОстановкиИзТаблицы(
		Результат, 
		"ПравилаПреобразованияРеквизитов1СВXDTO",
		Объект.ПравилаПреобразованияРеквизитов1СВXDTO);
	Возврат Результат;
КонецФункции

&НаСервереБезКонтекста
Процедура ДобавитьПравилаОстановкиИзТаблицы(ПравилаДляОтладки, ИмяТЧ, ТЧПравил)
	Для Каждого СтрокаПравила Из ТЧПравил ЦИкл
		Если ОтладкаВключенаВСтрокеПравила(СтрокаПравила) Тогда
			ПравилаДляОтладки.Вставить(
				ИмяТЧ + Строка(СтрокаПравила.НомерСтроки),
				СтрокаПравила.НомерСтроки);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция ОтладкаВключенаВСтрокеПравила(СтрокаПравила)
	Возврат СтрокаПравила.ОстановкаПриОтладке = 1;
КонецФункции
