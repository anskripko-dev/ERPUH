...
//
// Параметры:
//  ИмяСобытия - Строка - Возможные варианты:
//			- "Старт" - функция начала работу.
//			- "Конец" - функция закончила работу.
...
// Здесь можно установить точку останова. Функция будет
//  вызвана, для правил с пометкой "Точка останова".
//
// Параметры:
//  КонтекстОтладки	 - Структура - см. функцию СоздатьКонтекстОтладки.
//
Процедура ТочкаОстанова(КонтекстОтладки)
	// Точка останова
КонецПроцедуры
...
// Запоминает соответствие, описывающее правила, для которых будет
// выполнена процедура ТочкаОстанова().
//
// Параметры:
//  ПравилаДляОстанова - Соответствие - где:
...
//
Процедура УстановитьПравилаДляОстанова(ПравилаДляОстанова) Экспорт
	УстановитьЗначениеВКонтексте("ПравилаДляОстанова", ПравилаДляОстанова);
КонецПроцедуры
...
Функция ОтладкаПриНачалеФункции(ИмяФункции, 
	...
	Если ОтладкаВключена() Тогда
		...
		ОстановитьсяЕслиЕстьТочкаОстанова(КонтекстОтладки);
...
// Проверяет, что переданное правило требует остановки для отладки.
//
Функция ЭтоПравилоДляОстановки(ИмяТаблицыПравил, Правило)
	Если ОтладкаВключена() Тогда
		ПравилаДляОстанова = ПолучитьЗначениеВКонтексте(
			"ПравилаДляОстанова", Новый Массив);
		НомерСтрокиПравила = ПравилаДляОстанова[
			ИмяТаблицыПравил + Строка(Правило.НомерСтроки)];
		Возврат НомерСтрокиПравила <> Неопределено;
	КонецЕсли;
	Возврат Ложь;
КонецФункции

// Проверяет, что есть настройка для остановки, и если
//  есть вызывает процедуру ТочкаОстанова().
//
// Параметры:
//  КонтекстОтладки - Структура - см. функцию СоздатьКонтекстОтладки.
//
Процедура ОстановитьсяЕслиЕстьТочкаОстанова(КонтекстОтладки)
	Если ОтладкаВключена() Тогда
		ОписаниеПравила = ПолучитьОписаниеПравилаИзКонтекстаОтладки(КонтекстОтладки);
		Если ОписаниеПравила <> Неопределено 
				И ЭтоПравилоДляОстановки(ОписаниеПравила.ИмяТаблицыПравил,
												ОписаниеПравила.Правило) Тогда
			ТочкаОстанова(КонтекстОтладки);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

Функция ПолучитьОписаниеПравилаИзКонтекстаОтладки(КонтекстОтладки)
	Если ОтладкаВключена() Тогда
		ИмяТаблицыПравил = Неопределено;
		Правило = Неопределено;
		Если КонтекстОтладки.ИмяФункции = "ВыполнитьПравилоПреобразованияОбъекта1С" Тогда
			ИмяТаблицыПравил = "Объекты1СВXDTO";
			Правило = КонтекстОтладки.мАргументы[0];
		ИначеЕсли КонтекстОтладки.ИмяФункции = "ВыполнитьПравилоПреобразованияЗначения1С" Тогда
			ИмяТаблицыПравил = "Значения1СВXDTO";
			Правило = КонтекстОтладки.мАргументы[0];
		ИначеЕсли КонтекстОтладки.ИмяФункции = "ЗаполнитьРеквизитОбъектаXDTOПоПравилу" Тогда
			ИмяТаблицыПравил = "ПравилаПреобразованияРеквизитов1СВXDTO";
			Правило = КонтекстОтладки.мАргументы[0].ПравилоРеквизита;
		КонецЕсли;
		Если НЕ ПустаяСтрока(ИмяТаблицыПравил) 
					И Правило <> Неопределено Тогда
			Возврат Новый Структура("ИмяТаблицыПравил, Правило",
									 ИмяТаблицыПравил, Правило);
		КонецЕсли;
	КонецЕсли;
	Возврат Неопределено;
КонецФункции
...
Функция ИнформацияОбОшибкеВСтруктуру(ИнформацияОбОшибке)
	Описание = Новый Структура("ИмяМодуля,ИсходнаяСтрока,НомерСтроки");
	ЗаполнитьЗначенияСвойств(Описание, ИнформацияОбОшибке);
	Причина = Неопределено;
	Если ИнформацияОбОшибке.Причина <> Неопределено Тогда
		Причина = ИнформацияОбОшибкеВСтруктуру(ИнформацияОбОшибке.Причина);
	КонецЕсли;
