...
Функция ОбработкаОбъект() Экспорт
	...
	Если ОбработкаОбъект = Неопределено Тогда
		...
		Попытка
			СтруктураОбработки= ПолучитьИзВременногоХранилища(Объект.ПараметрыКлиентСервер.ВременноеХранилище.АдресОбработкаОбъект);
		Исключение
			СтруктураОбработки = Неопределено;
			Объект.ПараметрыКлиентСервер = Новый Структура("ВременноеХранилище, ИмяОбработки", Новый Структура("АдресОбработкаОбъект"), Метаданные.Обработки.тн_ИнтеграцияСДиадок.Имя);
		КонецПопытки;
...
//{ ОБРАБОТЧИКИ ФОРМЫ
	...
	Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
		...
		Иначе
			// НАЧАЛО Федоров С 03.09.2021 Выполнение задач №000038925
			//Добавляем попытку, если в параметры не передали массив документов пакета
			Попытка
			// КОНЕЦ Федоров С 03.09.2021 Выполнение задач №000038925
...
//{ ФОРМИРОВАНИЕ ПЕЧАТНЫХ ФОРМ
	...
	// НАЧАЛО Федоров С 03.09.2021 Выполнение задач №000038925
	//Добавляем параметр Экспорт 
	&НаКлиенте
	//Функция СформироватьПечатнуюФормуПоДокументуДиадока(Document)
	Функция СформироватьПечатнуюФормуПоДокументуДиадока(Document) Экспорт  //Вставка
	// КОНЕЦ Федоров С 03.09.2021 Выполнение задач №000038925
		...
		АвторизацияПоСертификату = ПользовательАвторизованПоСертификату();
  		...
  		Элементы.НастроитьПараметрыПодписи.Видимость = ТочкаВызова = "ТаблицаДокументовНаОтправку"
															...
															И (Найти(ТипДокумента, "Торг-12")>0
																...
																ИЛИ (Найти(ТипДокумента, "Корректировочный СФ")>0 И АвторизацияПоСертификату)
																ИЛИ (Найти(ТипДокумента, "Исправление СФ")>0 И АвторизацияПоСертификату)
																ИЛИ (Найти(ТипДокумента, "Исправление КСФ")>0 И АвторизацияПоСертификату)
																ИЛИ (Найти(ТипДокумента, "Счет-фактура")>0 И АвторизацияПоСертификату
																	И (ФункцияУПД = "СЧФ" ИЛИ ФункцияУПД = "КСЧФ" ИЛИ ФункцияУПД = "default")));	
	...
	Функция ВнестиИзмененияВДокумент(ДокументСсылка1С)
		...
		Попытка
			...
			Если НЕ ДокументОбъект1С.Метаданные().Реквизиты.Найти("ДатаВыставления") = Неопределено Тогда 
				...
				Если НЕ ДокументОбъект1С.ДатаВыставления = НачалоДня(ТекущаяДатаСервера()) Тогда
					ДокументОбъект1С.ДатаВыставления= НачалоДня(ТекущаяДатаСервера()); 
