...
Процедура УстановитьНачальныеНастройки()
	...
	Если НЕ ЗначениеЗаполнено(НачалоПериода)
		...
		ПолеГод=		НачалоГода(ТекущаяДатаСервера());
		ПолеКвартал=	НачалоКвартала(ТекущаяДатаСервера());
		ПолеМесяц=		НачалоМесяца(ТекущаяДатаСервера());
		ПолеДень=		НачалоДня(ТекущаяДатаСервера());
		
		НачалоПериода=	ТекущаяДатаСервера();
		КонецПериода=	ТекущаяДатаСервера();
...
Процедура УправлениеПереключателями(ИмяТекущегоЭлемента)
	...
	Если ТекущаяСтраницаФормы = "Интервал" Тогда
		...
		ИначеЕсли  ИмяТекущегоЭлемента = "ПереключательНачИнтервалаНачалоГода" Тогда
			...
			НачалоПериода=	НачалоГода(ТекущаяДатаСервера());
			//КОНЕЦ Блинова М.А. 29.04.2026 Выполнение задач №000057501
			ПроверкаИнтервала(2);
		...
		ИначеЕсли  ИмяТекущегоЭлемента = "ПереключательНачИнтервалаНачалоКвартала" Тогда
			...
			НачалоПериода=	НачалоКвартала(ТекущаяДатаСервера());
			//КОНЕЦ Блинова М.А. 29.04.2026 Выполнение задач №000057501
			ПроверкаИнтервала(2);
		...
		ИначеЕсли  ИмяТекущегоЭлемента = "ПереключательНачИнтервалаНачалоМесяца" Тогда
			...
			НачалоПериода=	НачалоМесяца(ТекущаяДатаСервера());
			//КОНЕЦ Блинова М.А. 29.04.2026 Выполнение задач №000057501
			ПроверкаИнтервала(2);
		...
		ИначеЕсли  ИмяТекущегоЭлемента = "ПереключательНачИнтервалаНачалоНедели" Тогда
			...
			НачалоПериода=	НачалоНедели(ТекущаяДатаСервера());
			//КОНЕЦ Блинова М.А. 29.04.2026 Выполнение задач №000057501
			ПроверкаИнтервала(2);
		...
		ИначеЕсли  ИмяТекущегоЭлемента = "ПереключательНачИнтервалаНачалоДня" Тогда
			...
			НачалоПериода=	НачалоДня(ТекущаяДатаСервера());
			//КОНЕЦ Блинова М.А. 29.04.2026 Выполнение задач №000057501
			ПроверкаИнтервала(1);
		...
		ИначеЕсли  ИмяТекущегоЭлемента = "ПереключательКонИнтервалаКонецГода" Тогда
			...
			КонецПериода=	КонецГода(ТекущаяДатаСервера());
			//КОНЕЦ Блинова М.А. 29.04.2026 Выполнение задач №000057501
			ПроверкаИнтервала(1);
		...
		ИначеЕсли  ИмяТекущегоЭлемента = "ПереключательКонИнтервалаКонецКвартала" Тогда
			...
			КонецПериода=	КонецКвартала(ТекущаяДатаСервера());
			//КОНЕЦ Блинова М.А. 29.04.2026 Выполнение задач №000057501
			ПроверкаИнтервала(1);
		...
		ИначеЕсли  ИмяТекущегоЭлемента = "ПереключательКонИнтервалаКонецМесяца" Тогда
			...
			КонецПериода=	КонецМесяца(ТекущаяДатаСервера());
			//КОНЕЦ Блинова М.А. 29.04.2026 Выполнение задач №000057501
			ПроверкаИнтервала(1);
		...
		ИначеЕсли  ИмяТекущегоЭлемента = "ПереключательКонИнтервалаКонецНедели" Тогда
			...
			КонецПериода=	КонецНедели(ТекущаяДатаСервера());
			//КОНЕЦ Блинова М.А. 29.04.2026 Выполнение задач №000057501
			ПроверкаИнтервала(1);
		...
		ИначеЕсли  ИмяТекущегоЭлемента = "ПереключательКонИнтервалаКонецДня" Тогда
			...
			КонецПериода=	КонецДня(ТекущаяДатаСервера());
			//КОНЕЦ Блинова М.А. 29.04.2026 Выполнение задач №000057501
			ПроверкаИнтервала(2);
