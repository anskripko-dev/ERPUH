...
Процедура НачатьСозданиеНоменклатурыИнформационнойБазы(ЭлементСопоставления)
	...
	ПараметрыФормы.Вставить("Родитель", тн_ПолучитьГруппуНоменклатуры());
	ПараметрыФормы.Вставить("ВариантОформленияПродажи", ПредопределенноеЗначение("Перечисление.ВариантыОформленияПродажи.РеализацияТоваровУслуг"));
...
// НАЧАЛО Федоров С 03.09.2021 Выполнение задач №000038925
&НаСервере
Функция тн_ПолучитьГруппуНоменклатуры()
	СтруктураНастроек = Новый Структура("ГруппаНоменклатуры");
	РегистрыСведений.тн_НастройкиДиадок.ВернутьЗначенияНастроек(СтруктураНастроек);
	Возврат СтруктураНастроек.ГруппаНоменклатуры;
КонецФункции
// Конец Федоров С 03.09.2021 Выполнение задач №000038925
...
Процедура НайтиВариантыСопоставленияНоменклатуры()
	...
	Для Каждого СтрокаТаблицы Из Сопоставление Цикл
		...
		// НАЧАЛО Федоров С 02.09.2021 Выполнение задач №000038911
		//Если Не ЗначениеЗаполнено(НоменклатураИБ) Тогда
		//	НоменклатураИБ = тн_ПолучитьНоменклатуруПоНаименованию(НоменклатураКонтрагента.Наименование);
		//КонецЕсли;
		// КОНЕЦ Федоров С 02.09.2021 Выполнение задач №000038911
...
// Федоров С 02.09.2021 Выполнение задач №000038911
//Осуществляем автосопоставление номенклатуры
&НаКлиенте
Процедура тн_Автосопоставление(Команда)
	Для Каждого Строка ИЗ Сопоставление Цикл
		Если НЕ ЗначениеЗаполнено(Строка.Номенклатура) Тогда
			Сопоставлено = Ложь;
			Строка.Номенклатура = тн_ПолучитьНоменклатуруПоНаименованию(Строка.НоменклатураНаименование, Сопоставлено);
			Строка.Сопоставлено = Сопоставлено;
		КонецЕсли;
	КонецЦикла;
	УстановитьСвойстваЭлементаОтбораПоСопоставлению();
КонецПроцедуры
...
// Федоров С 02.09.2021 Выполнение задач №000038911
//Осуществляем поиск номенклатуры по наименованию для автосопоставления
&НаСервере
Функция тн_ПолучитьНоменклатуруПоНаименованию(НаименованиеНоменклатуры, Сопоставлено)
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
