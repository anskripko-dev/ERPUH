...
Функция ФункцииМодуля() Экспорт
	...
	Платформа.ДобавитьФункциюВМанифест(Результат, "ПолучитьДействующиеМЧДпоУмолчанию");
	
	//Отправка приглашений контрагентам
	Платформа.ДобавитьФункциюВМанифест(Результат, "ОтправитьПринятьПриглашениеКонтрагенту");
	Платформа.ДобавитьФункциюВМанифест(Результат, "Новый_ПараметрыОтправкиПриглашений");
	Платформа.ДобавитьФункциюВМанифест(Результат, "Новый_РезультатОтправкиПриглашений");
	Платформа.ДобавитьФункциюВМанифест(Результат, "Новый_КонтрагентДляПриглашения");
	Платформа.ДобавитьФункциюВМанифест(Результат, "СформироватьHTMLСписокОшибокПриОтправкеВложенийРоуминг");
	Платформа.ДобавитьФункциюВМанифест(Результат, "Перечисление_ДействияНаФормеПредупрежденияОтправкиВложенийВРоуминг");
...
Функция ДокументВУстаревшемФормате(ФорматОтправки, Знач ДатаОтправки = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДатаОтправки) Тогда
		...
		ДатаОтправки = ТекущаяДатаСервера();
...
Процедура ЗарегистрироватьОшибкуАвтообновленияМодуляВЖурналеРегистрации(ОписаниеОшибки) Экспорт
	...
	ДатаСобытия = ТекущаяДатаСервера();
	//КОНЕЦ Блинова М.А. 29.04.2026 Выполнение задач №000057501
...
Процедура МЧД_ЗаполнитьКонтракт(КонтрактМЧД, PowerOfAttorney)
	...
	ЕстьПолеKPP = ЕстьРеквизитИлиСвойствоОбъекта(Issuer, "KPP");
	
	Если ЕстьПолеKPP Тогда
...
&НаКлиенте
Функция МЧД_ВыполнитьПроверкиДоверенностиПоУмолчаниюПередОтправкойДокументов(ДанныеДляОбработки)
	...
	КонтрактыМЧД = ПолучитьДействующиеМЧДпоУмолчанию(ИдентификаторыЯщиков, ОшибкиПроверкиМЧД);
...
Процедура МЧД_ПроверитьДоверенностьПоУмолчаниюИВыполнитьОповещение(ОписаниеОповещения) Экспорт
    ...
    ОшибкиПроверокМЧД = МЧД_ВыполнитьПроверкиДоверенностиПоУмолчаниюПередОтправкойДокументов(ДанныеДляОбработки);
	
	МЧД_ВыполнитьДействияПослеПроверкиМЧДпоУмолчанию(ОписаниеОповещения, ОшибкиПроверокМЧД);
	
КонецПроцедуры

&НаКлиенте
Процедура МЧД_ВыполнитьДействияПослеПроверкиМЧДпоУмолчанию(ОписаниеОповещения, ОшибкиПроверокМЧД) Экспорт
...
//Отменяем проверку версии компоненты и ее скаивание с сайта
...
//КОНЕЦ Федоров С 03.09.2021 Выполнение задач №000038925
	...
	Если Не КомпонентаПодключена Тогда
		...
		Если Не КомпонентаПодключена Тогда
			// НАЧАЛО Османов Н.М. 05.08.2022 Выполнение задач №000042279
			//КомпонентаПодключена = ПодключитьКомпонентуДиадокаЧерезРегистрацию(ВерсияКомпоненты);
			// КОНЕЦ Османов Н.М. 05.08.2022 Выполнение задач №000042279
		КонецЕсли;
	...
	//Отключаем перезапуск, если появилась новая версия
	ТребуетсяПерезапуск = Ложь; //(ВерсияКомпоненты <> ПодключеннаяВерсия);
	// КОНЕЦ Федоров С 03.09.2021 Выполнение задач №000038925
...
Функция ПодключитьКомпонентуДиадокаБезРегистрации(ТребуемаяВерсия)
	...
	стрСообщение = СтрШаблон("Попытка подключения компоненты из файла '%1': %2", ПутьКФайлу, Формат(КомпонентаПодключена, "БЛ=Ошибка; БИ=Успешно"));
	ЗаписьЖурналаРегистрацииНаСервере("Диадок", стрСообщение);
...
Функция ПутьКФайлуКомпонентыДиадока(ВерсияКомпоненты)
	...
	//НАЧАЛО Федоров С 03.09.2021 Выполнение задач №000038925
	//Компонента хранится в макете в двоичных данных 
	тн_ПоместитьКомпонентуВоВременноеХранилище(Архитектура);
	ДвоичныеДанные = ПолучитьИзВременногоХранилища(тн_АдресВоВременномХранилище);
	Попытка
		ДвоичныеДанные.Записать(Результат);
	Исключение
		// если повторно открывается обработка, компонента уже подключена и файл компоненты занят процессом 1С
		// подаваляем исключение записи заблокированного файла
	КонецПопытки;
	//КОНЕЦ Федоров С 03.09.2021 Выполнение задач №000038925
...
// НАЧАЛО Ситалов Дмитрий 03.09.2021 Выполнение задач №000038925
&НаСервере
Процедура ЗаписьЖурналаРегистрацииНаСервере(ИмяСобытия, стрСообщение)
	
	ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Информация, ,, стрСообщение);
	
КонецПроцедуры

// Федоров С 03.09.2021 Выполнение задач №000038925
&НаСервере
Процедура тн_ПоместитьКомпонентуВоВременноеХранилище(АрхитектураКомпоненты)
	
	Если АрхитектураКомпоненты = "x86_64" Тогда
		// НАЧАЛО Османов Нагим 17.08.2022 Выполнение задач №000042279
		//МакетКомпоненты = Обработки.тн_ИнтеграцияСДиадок.ПолучитьМакет("тн_ВнешняяКомпонентаDiadoc_5_35_4_736_x64_dll");
		МакетКомпоненты = Обработки.тн_ИнтеграцияСДиадок.ПолучитьМакет("тн_ВнешняяКомпонентаDiadoc_5_37_5_836_x64_dll");
		// КОНЕЦ Османов Нагим 17.08.2022 Выполнение задач №000042279
		тн_АдресВоВременномХранилище = ПоместитьВоВременноеХранилище(МакетКомпоненты);
	ИначеЕсли АрхитектураКомпоненты = "x86" Тогда
		// НАЧАЛО Османов Нагим 17.08.2022 Выполнение задач №000042279
		//МакетКомпоненты = Обработки.тн_ИнтеграцияСДиадок.ПолучитьМакет("тн_ВнешняяКомпонентаDiadoc_5_35_4_736_dll");
		МакетКомпоненты = Обработки.тн_ИнтеграцияСДиадок.ПолучитьМакет("тн_ВнешняяКомпонентаDiadoc_5_37_5_836_dll");
		// КОНЕЦ Османов Нагим 17.08.2022 Выполнение задач №000042279
		тн_АдресВоВременномХранилище = ПоместитьВоВременноеХранилище(МакетКомпоненты);
	Иначе
		ВызватьИсключение "Неизвестная архитектура компоненты " + Строка(АрхитектураКомпоненты);
	КонецЕсли;
...
//{ СЛУЖЕБНЫЕ ПРОЦЕДУРЫ И ФУНКЦИИ
	...
	Функция ТекстИнформацииОНовомФормате(ConfirmationDate) Экспорт
		...
		ДатаПрименения = ?(ConfirmationDate<'20010101', ТекущаяДатаСервера(), ConfirmationDate);
		//КОНЕЦ Блинова М.А. 29.04.2026 Выполнение задач №000057501
	...
	Функция ДатаГоризонта() Экспорт
		...
		Возврат ДобавитьМесяц(ТекущаяДатаСервера(), 12);
		//КОНЕЦ Блинова М.А. 29.04.2026 Выполнение задач №000057501
...
//{ ИНТЕГРАЦИЯ
	...
	Функция ДатаЗапретаГОСТ2001_Наступила()
		...
		Если УниверсальноеВремя(ТекущаяДатаСервера()) > ДатаПерехода Тогда
		//КОНЕЦ Блинова М.А. 29.04.2026 Выполнение задач №000057501
			Результат = Истина;
		КонецЕсли;
...
//{ ИНТЕРАКТИВНЫЕ ДЕЙСТВИЯ
	...
	&НаКлиенте
	Функция ОтправитьПринятьПриглашениеКонтрагенту(Organization,
													CounteragentId,
													ИНН,
													Знач Комментарий,
													ИмяФайла = "",
		...
		Если ЗначениеЗаполнено(ИмяФайла) Тогда
			...
			МетодКлиента("Модуль_РаботаССерверомДиадок", "ЗаполнитьМЧДдляОтправки", TrustConnectionRequest, КонтрактМЧД, Organization.ID);
	...
	// Параметры для отправки приглашений контрагентам
	//
	// Возвращаемое значение:
	//  Структура - 
	//		* ТекстПриглашения					- Строка 
	//		* ИмяФайла 							- Строка
	//		* КонтрактМЧД  						- Структура, Неопределено - см. Новый_Контракт_МЧД()
	//		* МассивКонтрагентов				- Массив - элемент массива структура, см. Новый_КонтрагентДляПриглашения() 
	//		* ОбработчикПослеОтправкиПриглашений- ОписаниеОповещения - вызывается после отправки приглашений 
	//		* Organization						- ComОбъект - см. ПолучитьЯщикДиадокДляОрганизации() 
	//		* ОтправитьПриглашенияВРоуминг	 	- Булево - Если Истина, то отправка приглашения в роуминг выполняется
	//													,если Ложь, то приглашения контрагентам из роуминга не отправляются
	//
	&НаКлиенте
	Функция Новый_ПараметрыОтправкиПриглашений() Экспорт
		
		Результат = Новый Структура;
		Результат.Вставить("ТекстПриглашения"	, "");
		Результат.Вставить("ИмяФайла"			, "");
		Результат.Вставить("КонтрактМЧД"		, Неопределено);
		Результат.Вставить("МассивКонтрагентов"	, Новый Массив);
		Результат.Вставить("ОбработчикПослеОтправкиПриглашений"	, Неопределено);
		Результат.Вставить("Organization"		, Неопределено);
		Результат.Вставить("ОтправитьПриглашенияВРоуминг"	, Истина);
	...
	//
	// Параметры:
	//	ПараметрыПриглашения - Структура - см Новый_ПараметрыОтправкиПриглашений()
	//
	// Возвращаемое значение:
	//  Структура - 
	//		* КоличествоУспешноОтправлено		- Число 
	//		* СообщенияПользователю 			- Массив - сообщения пользователю, элемент массива Строка
	//		* ПараметрыПриглашения  			- Структура - см. Новый_ПараметрыОтправкиПриглашений()
	//
	&НаКлиенте
	Функция Новый_РезультатОтправкиПриглашений(ПараметрыПриглашения) Экспорт
		
		Результат = Новый Структура("КоличествоУспешноОтправлено,СообщенияПользователю,ПараметрыПриглашения");
		Результат.КоличествоУспешноОтправлено	 = 0;
		Результат.СообщенияПользователю			 = Новый Массив;
		Результат.ПараметрыПриглашения			 = ПараметрыПриглашения;
		
		Возврат Результат;
		
	КонецФункции
	
	// Описывает структуру контрагента для отправки приглашений
	//
	// Возвращаемое значение:
	//  Структура - см. Контракт_КонтрагентаДляПриглашения()
	//
	&НаКлиенте
	Функция Новый_КонтрагентДляПриглашения(Идентификатор, Представление, ИНН, Роуминговый = Ложь) Экспорт
		
		Результат = Контракт_КонтрагентДляПриглашения(); 
		Результат.ID			 = Идентификатор;
		Результат.Представление	 = Представление;
		Результат.ИНН			 = ИНН;
		Результат.Роуминговый	 = Роуминговый;
	...
	//
	&НаКлиенте
	Функция Контракт_КонтрагентДляПриглашения()
	...
	//
	// Параметры:
	//	Контрагенты - Массив -  массив контрактов контрагентов, см. Новый_КонтрактКонтрагентаДляПриглашения()
	//
	// Возвращаемое значение:
	//	Строка
	//
	&НаКлиенте
	Функция СформироватьHTMLСписокОшибокПриОтправкеВложенийРоуминг(Контрагенты) Экспорт
		...
		Для каждого КонтрактКонтрагента Из Контрагенты Цикл
			...
			Если КонтрактКонтрагента.Роуминговый Тогда
				ТекстСтрокиСпискаHTML = СтрШаблон(ШаблонСтрокиСпискаHTML, КонтрактКонтрагента.Представление);
	...
	// Перечисление. Варианты действий при отправке вложений в роуминг
	//
	// Возвращаемое значение:
	//  Строка - Идентификатор действия прльзователя на форме предупреждения ПоказатьПредупреждение_ОтправкаВложенийВРоуминг()
	//		* "ПродолжитьБезВложения"
	//		* "ОткрытьСписокОшибок"
	//		* "Закрыть"
	//
	&НаКлиенте
	Функция Перечисление_ДействияНаФормеПредупрежденияОтправкиВложенийВРоуминг() Экспорт
		
		Результат = Новый Структура; 
		Результат.Вставить("ПродолжитьБезВложения"	, "ПродолжитьБезВложения");
		Результат.Вставить("ОткрытьСписокОшибок"	, "ОткрытьСписокОшибок");
		Результат.Вставить("Закрыть"				, "Закрыть");
	...
	Процедура ПоказатьСостояниеОбработкиСписка(ТекстСостояния, ТекущийНомер, РазмерСписка, ОтображатьДеталиПрогресса = Истина, ПояснениеСостояния = Неопределено) Экспорт
		...
		Если ОтображатьДеталиПрогресса Тогда
			ДеталиПрогресса = СтрШаблон(НСтр("ru = ' (%1 из %2)'"), ТекущийНомер, РазмерСписка);
		КонецЕсли;
...
Функция АдресаИнтернетРесурсов() Экспорт
	...
	Результат.Вставить("Инструкция_РуководствоПользователя",	 "https://support.kontur.ru/pages/viewpage.action?pageId=83863870");
	Результат.Вставить("Инструкция_ПараметрыОтправки",			 "https://support.kontur.ru/pages/viewpage.action?pageId=83863858");
	Результат.Вставить("Инструкция_ПереотправкаДокументов",		 "https://support.kontur.ru/pages/viewpage.action?pageId=83863839");
	Результат.Вставить("Инструкция_НетДокументовНаОтправку",	 "https://support.kontur.ru/pages/viewpage.action?pageId=83863871");
	Результат.Вставить("Инструкция_ЗаполнитьОснованиеПередачи",	 "https://support.kontur.ru/pages/viewpage.action?pageId=83863837");
	Результат.Вставить("Инструкция_ЗаполнитьГТД",				 "https://support.kontur.ru/pages/viewpage.action?pageId=83863899");
	Результат.Вставить("Инструкция_ФормаВводаНакладной",		 "https://support.kontur.ru/pages/viewpage.action?pageId=83863894");
	Результат.Вставить("Инструкция_ИспользоватьСервисКонтурМаркировка", "https://support.kontur.ru/pages/viewpage.action?pageId=83863897");
	Результат.Вставить("Инструкция_ОбновлениеМодуляВручную", 	 "https://support.kontur.ru/pages/viewpage.action?pageId=83863872");
	Результат.Вставить("Инструкция_ОбновлениеИОписаниеВерсий", 	 "https://support.kontur.ru/plugins/viewsource/viewpagesrc.action?pageId=83861490");
	Результат.Вставить("Инструкция_ОписаниеВерсий", 	 		 "https://support.kontur.ru/plugins/viewsource/viewpagesrc.action?pageId=83861482");
