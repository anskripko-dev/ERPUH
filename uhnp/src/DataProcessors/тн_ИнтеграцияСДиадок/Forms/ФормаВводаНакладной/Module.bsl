...
КонецФункции
	...
	Процедура ЗаполнитьШапкуДокумента_БП30_ПоступлениеДопРасходов(ДокументОбъект)
		...
		ДокументОбъект.Дата= ТекущаяДатаСервера();
	...
	//Закомпентировано - нет такой процедуры в ММ справочника Организации
	//&НаСервереБезКонтекста
...
Процедура ВыборСоглашенияСКонтрагентом(Партнер, Элемент)
	...
	ПарметрыФормы= Новый Структура("ДатаДокумента, ТекущаяСтрока, Отбор", ТекущаяДатаСервера(), Соглашение, Новый Структура);
	//КОНЕЦ Блинова М.А. 29.04.2026 Выполнение задач №000057501
