...
// Определяет поля в документе 1С, которые являются полями маршрутизации и возвращает значение этих полей
//
// Параметры:
//	Документ1С - ДокументСсылка - документ, по которому будут определены поля маршрутизации и их значения
//
// Возвращаемое значение:
//	Структура с ключами: 
//		* ПоляМаршрутизацииОтправителя	- Массив - значения полей маршрутизации в подразделение отправителя
//		* ПоляМаршрутизацииПолучателя	- Массив - значения полей маршрутизации в подразделение получателя
Функция ПолучитьЗначенияПолейМаршрутизации(Документ1С) Экспорт
	
	МенеджерИнтеграции = МодульМенеджерИнтеграции();
	Результат = МенеджерИнтеграции.ЗначенияПолейМаршрутизацииПоПодразделениям(Документ1С);
...
Процедура ЗаполнитьКонтрагентаИнформационнойБазы(РеквизитыОрганизации)
	...
	Если НЕ ЗначениеЗАполнено(РеквизитыОрганизации.Ссылка) Тогда
		РеквизитыОрганизации.Ссылка = НайтиСоздатьКонтрагента(РеквизитыОрганизации);			
	КонецЕсли;
...
Функция ИдентификаторБазы()  
	...
	// НАЧАЛО Гутарова Екатерина 26.01.2023 Выполнение задач №000044115
	#Если Сервер Или ВнешнееСоединение Тогда
	// КОНЕЦ Гутарова Екатерина 26.01.2023 Выполнение задач №000044115
...
Функция ОписанияПодсистемКонфигурации()
	...
	// НАЧАЛО Федоров С 03.09.2021 Выполнение задач №000038925
	//Открываем условие репроцессора
	...
	#Иначе
		Результат = Истина;
	#КонецЕсли
	//Закрываем условие репроцессора
	// КОНЕЦ Федоров С 03.09.2021 Выполнение задач №000038925
	Возврат Результат;
...
Процедура ПодобратьНоменклатуруПоАртикулу(НайденнаяНоменклатура, АртикулИлиМассив)
	...
	//Открываем условие препроцессора
	#Если Сервер ИЛИ ВнешнееСоединение Тогда
		ЕстьРеквизитАртикул = ОбщегоНазначения.ЕстьРеквизитОбъекта("Артикул", МетаданныеНоменклатуры);
	#Иначе
		ЕстьРеквизитАртикул = Истина;
	#КонецЕсли
	//Закрываем условие препроцессора
	// КОНЕЦ Федоров С 03.09.2021 Выполнение задач №000038925
...
Функция КлючРазработчикаПоУмолчанию()
	Возврат "1S83-5-56-85654b47-01d5-4540-8e16-2f6066223d54";
КонецФункции
...
// НАЧАЛО Федоров С 03.09.2021 Выполнение задач №000038925
Функция НайтиСоздатьКонтрагента(РеквизитыОрганизации)
	...
	Запрос.УстановитьПараметр("КПП", РеквизитыОрганизации.КПП);
