...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	ДанныеКОбработке.Колонки.Добавить("ПроизводитсяВПроцессе", Новый ОписаниеТипов("Булево"));
	ДанныеКОбработке.Колонки.Добавить("ИзЗапасовПодразделения", Новый ОписаниеТипов("Булево"));
	ДанныеКОбработке.Колонки.Добавить("СтрокиКОбработке",  Новый ОписаниеТипов("Массив"));
	
	СтруктураПоиска = Новый Структура("ПроизводитсяВПроцессе, ИзЗапасовПодразделения");
	
	Для Каждого НоваяСтрока Из МассивНовыхСтрок Цикл
		
		Если ЗначениеЗаполнено(НоваяСтрока.ВариантОбеспечения) Тогда
			Продолжить;
		КонецЕсли;
		
		СтруктураПоиска.ПроизводитсяВПроцессе = НоваяСтрока.ПроизводитсяВПроцессе;
		СтруктураПоиска.ИзЗапасовПодразделения = Не ЗначениеЗаполнено(НоваяСтрока.Склад);
		
		НайденныеСтроки = ДанныеКОбработке.НайтиСтроки(СтруктураПоиска);
		Если НайденныеСтроки.Количество() = 0 Тогда
			СтрокаДанныеКОбработке = ДанныеКОбработке.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаДанныеКОбработке, СтруктураПоиска);
			СтрокаДанныеКОбработке.СтрокиКОбработке.Добавить(НоваяСтрока);
		Иначе
			НайденныеСтроки[0].СтрокиКОбработке.Добавить(НоваяСтрока);
		КонецЕсли;
	...
	Для каждого Строка Из ДанныеКОбработке Цикл
		...
		ВариантОбеспеченияИФлагОбособленно = ПланированиеПроизводстваКлиентСервер.ВариантОбеспеченияИФлагОбособленноПоУмолчанию(
			Строка.ПроизводитсяВПроцессе, Строка.ИзЗапасовПодразделения);
		
		ПараметрыДокумента = Новый Структура();
		ПараметрыДокумента.Вставить("Форма",                              ЭтотОбъект);
		ПараметрыДокумента.Вставить("Коллекция",                          Объект.МатериалыИУслуги);
...
Функция ПолучитьОбъектОснования()
	...
	СтруктураОтвета.Вставить("Проведен", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектОснования, "Проведен"));
...
Процедура ПослеЗаполненияОбеспечения(Изменения, Режим = Неопределено, ПараметрыЗаполнения = Неопределено)
	...
	ОбеспечениеВДокументахСервер.ПересчитатьКоличествоЕдиниц(Изменения, Объект.МатериалыИУслуги);
...
Процедура ЗаменитьНаАналоги(АдресВХранилище)
	...
	Для каждого ИндексСтроки Из ИндексыНовыхСтрок Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧ.ВариантОбеспечения) Тогда
			СтруктураПоиска.ПроизводитсяВПроцессе = СтрокаТЧ.ПроизводитсяВПроцессе;
			СтруктураПоиска.ИзЗапасовПодразделения = Не ЗначениеЗаполнено(СтрокаТЧ.Склад);
			
			НайденныеСтроки = ДанныеКОбработке.НайтиСтроки(СтруктураПоиска);
			Если НайденныеСтроки.Количество() = 0 Тогда
				СтрокаДанныеКОбработке = ДанныеКОбработке.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаДанныеКОбработке, СтруктураПоиска);
				СтрокаДанныеКОбработке.СтрокиКОбработке.Добавить(СтрокаТЧ);
			Иначе
				НайденныеСтроки[0].СтрокиКОбработке.Добавить(СтрокаТЧ);
			КонецЕсли;
		КонецЕсли;
	...
	Для каждого Строка Из ДанныеКОбработке Цикл
		...
		ПараметрыДокумента = Новый Структура();
		ПараметрыДокумента.Вставить("Форма",                              ЭтотОбъект);
		ПараметрыДокумента.Вставить("Коллекция",                          Объект[ИмяТаблицы]);
		ПараметрыДокумента.Вставить("ВариантОбеспеченияИФлагОбособленно", ВариантОбеспеченияИФлагОбособленно);
...
Процедура МатериалыИУслугиПриИзмененииНоменклатурыНаСервере(Идентификатор)
	...
	ДобавитьДействияОбеспеченияНаСервере(СтруктураДействий, "ВариантОбеспечения", ТекущиеДанные);
	СтруктураДействий.Вставить("ПересчитатьКоличествоЕдиниц", ПланированиеПроизводстваКлиентСервер.ПараметрыПересчетаКоличестваЕдиниц(ТекущиеДанные, "МатериалыИУслуги"));
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ТекущиеДанные, СтруктураДействий, ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения());
...
Процедура ИзменитьДанныеЭтапа(АдресВХранилище, Идентификатор, КэшированныеЗначения)
	...
	Если ИзмененСпособПроизводства ИЛИ ИзмененоНаименование ИЛИ ИзмененоПодразделение Тогда
		...
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			ОбрабатываемыеСтроки, СтруктураДействий, Объект.МатериалыИУслуги, КэшированныеЗначения);
...
Процедура ЗаполнитьСкладМатериалаНаОснованииСтрокиИзделия(ТекущиеДанные, КлючСвязи)
	...
	ТребуетсяОбновитьКолонкуДоступно = Ложь;
	Для каждого СтрокаМатериал Из СписокСтрок Цикл
		...
		Если СтрокаМатериал.Номенклатура = ТекущиеДанные.Номенклатура
			...
			ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(СтрокаМатериал, СтруктураДействий, КэшированныеЗначения);
			Если Не ТребуетсяОбновитьКолонкуДоступно
						И ОбеспечениеВДокументахКлиент.ТребуетсяОбновитьКолонкуДоступно(СтруктураДействий) Тогда
				ТребуетсяОбновитьКолонкуДоступно = Истина;
			КонецЕсли;
	...
	Если ТребуетсяОбновитьКолонкуДоступно Тогда
