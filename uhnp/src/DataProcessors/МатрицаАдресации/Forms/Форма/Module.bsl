	...
	Если ЗначениеЗаполнено(СправочникТекущейИБ) Тогда
		...
		Для Каждого СтрокаАдресации Из ДеревоИерархииЗначение.Строки Цикл
			Если Не ЗначениеЗаполнено(СтрокаАдресации.ЭлементАдресации) тогда
				СтрокаАдресации.ЭлементАдресации = ПолучитьПредставлениеМетаданных(СправочникТекущейИБ);
			КонецЕсли;
		КонецЦикла;
...
Процедура ВыполнитьПолучениеЛистовДерева(УзелВход, МассивРезультатИзм, ТолькоЛистьяВход = Истина, ГлубинаРекурсииВход = 0)
	...
	Если ЭлементыУзла.Количество() > 0 Тогда
		Для Каждого ТекЭлементыУзла Из ЭлементыУзла Цикл
			...
			Если НЕ ТолькоЛистьяВход Тогда				
				Если ТекЭлементыУзла.ЭлементАдресации = ПолучитьПредставлениеМетаданных(СправочникТекущейИБ) тогда
	...
	Иначе
		Если ТипЗнч(УзелВход) <> Тип("ДанныеФормыДерево") тогда
			Если УзелВход.ЭлементАдресации = ПолучитьПредставлениеМетаданных(СправочникТекущейИБ) тогда
				МассивРезультатИзм.Добавить(ПолучитьПустоеЗначениеСправочника(СправочникТекущейИБ));	
			Иначе
...
Процедура ПереместитьМассивЭлементовВГруппу(ДанныеВыделенныхСтрокВход, ЭлементПриемникВход, СправочникИбВход)
	Для Каждого ТекДанныеВыделенныхСтрок Из ДанныеВыделенныхСтрокВход Цикл
		...
		Если ТекЭлементАдресации = ПолучитьПредставлениеМетаданных(СправочникИбВход) тогда
			Продолжить;
		КонецЕсли;		
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого Справочник Из МассивОбъектовМетаданных Цикл	
		...
		Вставка = Вставка + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ИмяСправочника, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '<%1: все элементы>'"), ПредставлениеСправочника));
	КонецЦикла;
...
Процедура ТаблицаОтветственныхОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТекДанные <> Неопределено Тогда
		Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.Пользователи") Тогда			
			Если ТекДанные.ЭлементАдресации = ПолучитьПредставлениеМетаданных(СправочникТекущейИБ) тогда
...
Процедура УдалитьЭлементИерархии(Команда)
	...
	Если ТекДанные <> Неопределено Тогда
		...
		Для Каждого ТекВыделенныеСтроки Из Элементы.ДеревоИерархии.ВыделенныеСтроки Цикл
			...
			СтруктураВыделеннойСтроки = Новый Структура;			
			Если ДанныеСтрока.ЭлементАдресации = ПолучитьПредставлениеМетаданных(СправочникТекущейИБ) тогда
		...
		Если ЭлементАдресации <> Неопределено Тогда
			Если ТипЗнч(ЗначениеВыбора) = Тип("Строка") Тогда
				Если ЗначениеВыбора = ПредставлениеКореньДерева Тогда
					...
					КонецЕсли;
				ИначеЕсли ЗначениеВыбора <> ПолучитьПредставлениеМетаданных(СправочникТекущейИБ) тогда				
...
Процедура ТаблицаОтветственныхПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если ТекДанные <> Неопределено Тогда
		Если ТекДанные.ЭлементАдресации = ПолучитьПредставлениеМетаданных(СправочникТекущейИБ) тогда
			ТипСправочник = ВернутьТипПоСсылкеБД(СправочникТекущейИБ);
...
Процедура ДобавитьПустойЭлемент(Команда)
	...
	ЭлементыДерева = ДеревоИерархии.ПолучитьЭлементы();
	ТекущаяОрганизация = ПолучитьПредставлениеМетаданных(СправочникТекущейИБ);
...
Процедура ДеревоИерархииПередНачаломИзменения(Элемент, Отказ)
	...
	Если Элемент.ТекущиеДанные.ЭлементАдресации = ПолучитьПредставлениеМетаданных(СправочникТекущейИБ) тогда
		Отказ = Истина;
	КонецЕсли;
...
Функция ВернутьТипПоСсылкеБД(СправочникТекущейИБ)
	Возврат ОбщегоНазначенияСерверУХ.ВернутьТипПоСсылкеБД(СправочникТекущейИБ);
КонецФункции
...
Процедура ТаблицаОтветственныхОбработкаЗаписиНового(НовыйОбъект, Источник, СтандартнаяОбработка)
	...
	ЭлементыДерева = ДеревоИерархии.ПолучитьЭлементы();
	ТекущаяОрганизация = ?(ЗначениеЗаполнено(Источник.Запись.Организация), Источник.Запись.Организация, ПолучитьПредставлениеМетаданных(СправочникТекущейИБ));
	ЭлементНайден = Ложь;
	...
	Если Не ЭлементНайден тогда		
		ДобавитьЭлементАдресацииВКорень(
			?(ТекущаяОрганизация = ПолучитьПредставлениеМетаданных(СправочникТекущейИБ), ПолучитьПредставлениеМетаданных(СправочникТекущейИБ), ТекущаяОрганизация), 
			СправочникТекущейИБ);
...
Функция ПолучитьПредставлениеПустогоОбъекта(СправочникИбВход)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '<%1: все элементы>'"), ИмяСправочника);
	
КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьПредставлениеМетаданных(СправочникИбВход)
	
	ТипСправочник = ОбщегоНазначенияСерверУХ.ВернутьТипПоСсылкеБД(СправочникИбВход);
	ИмяСправочника = Метаданные.НайтиПоТипу(ТипСправочник).Представление();
