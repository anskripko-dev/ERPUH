...
Процедура ДеревоВыбранныхСтрокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ДеревоВыбранныхСтрокНаименование" 
		И НЕ Элемент.ТекущиеДанные = Неопределено 
		И Элемент.ТекущиеДанные.ЭтоФормула Тогда
	Иначе	
		СтандартнаяОбработка = Ложь;
		Возврат;	
	КонецЕсли;
...
Процедура ПрименитьИзмененияСервер()
	...
	ЭтаФорма["НастройкиСоставаСтрокСводнойТаблицы"].Очистить();
	...
	Для Каждого Стр Из СтрокиДерева Цикл
		...
		нСтрокаСостава 						= ЭтаФорма["НастройкиСоставаСтрокСводнойТаблицы"].Добавить();
		...
		нСтрокаСостава.ЭтоФормула    		= Стр.ЭтоФормула;
        нСтрокаСостава.ЗаголовокФормулы 	= Стр.Наименование;
		нСтрокаСостава.ФормулаСводнойТаблицы= Стр.ФормулаСводнойТаблицы;
		
		НомерПоПорядку = НомерПоПорядку+1;
		
	КонецЦикла;	
	
	тНастройкиСоставаСтрокСводнойТаблицы = ЭтаФорма["НастройкиСоставаСтрокСводнойТаблицы"].Выгрузить();
...
Процедура ЗаполнитьДеревоВыбранныхСтрок()
	...
	ДеревоНастройкиСервер_Корень.ИндексКартинки = -1;
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	тНастройкиСоставаСтрокСводнойТаблицы.ЭтоФормула КАК ЭтоФормула,
	               |	тНастройкиСоставаСтрокСводнойТаблицы.ЗаголовокФормулы КАК ЗаголовокФормулы,
				   |	тНастройкиСоставаСтрокСводнойТаблицы.ФормулаСводнойТаблицы КАК ФормулаСводнойТаблицы,
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	втНастройкиСоставаСтрокСводнойТаблицы.СтрокаОтчета,
				   |	ВЫБОР
	               |		КОГДА втНастройкиСоставаСтрокСводнойТаблицы.ЭтоФормула
	               |			ТОГДА 0
	               |		ИНАЧЕ -1
				   ...
				   |	втНастройкиСоставаСтрокСводнойТаблицы.ЭтоФормула КАК ЭтоФормула,
				   |	втНастройкиСоставаСтрокСводнойТаблицы.ФормулаСводнойТаблицы КАК ФормулаСводнойТаблицы,
				   |	СтрокиОтчетов.Владелец КАК ВидОтчета,
	               |	СтрокиОтчетов.ГруппаРаскрытия КАК ГруппаРаскрытия,
	               |	СтрокиОтчетов.Код КАК Код,
				   |	ВЫБОР
	               |		КОГДА втНастройкиСоставаСтрокСводнойТаблицы.ЭтоФормула
	               |			ТОГДА втНастройкиСоставаСтрокСводнойТаблицы.ЗаголовокФормулы
	               |		ИНАЧЕ СтрокиОтчетов.Наименование
...
&НаКлиенте
Процедура ДобавитьФормулуСводнойТаблицы(Команда)
	
	Оповещение = Новый ОписаниеОповещения("ДобавлениеФормулыЗавершение", ЭтаФорма);
	
	ОтборСтруктура = Новый Структура;
	ОтборСтруктура.Вставить("Владелец",ВариантСводнойТаблицы);
	ОтборСтруктура.Вставить("ЭтоФормулаКолонки",Ложь);

	
	Параметры_ = Новый Структура("Отбор",ОтборСтруктура);
	ОткрытьФорму("Справочник.ФормулыСводнойТаблицы.ФормаВыбора",Параметры_,ЭтаФорма,Истина,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
			
КонецПроцедуры

&НаКлиенте
Процедура ДобавлениеФормулыЗавершение(Результат, ДополнительныеПараметры) Экспорт      
	
	Если Результат = Неопределено  Или Результат = КодВозвратаДиалога.Отмена  Тогда
		Возврат;
	КонецЕсли;	
	
	НоваяСтрока = ДеревоВыбранныхСтрок.НайтиПоИдентификатору(Элементы.ДеревоВыбранныхСтрок.ТекущаяСтрока).ПолучитьЭлементы().Добавить();
	НоваяСтрока.ЭтоФормула 		= Истина;
	НоваяСтрока.ИндексКартинки  = 0;
	НоваяСтрока.Наименование 	= Результат;
	НоваяСтрока.ФормулаСводнойТаблицы 	= Результат;
