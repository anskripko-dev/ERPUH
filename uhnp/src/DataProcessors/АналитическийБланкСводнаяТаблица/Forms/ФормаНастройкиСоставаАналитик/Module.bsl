...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтаФорма["НастройкиСоставаСтрокСводнойТаблицы"].Загрузить(СтруктураТаблицИнтерфейса.НастройкиСоставаСтрокСводнойТаблицы);
	ЭтаФорма["НастройкиСоставаКолонокСводнойТаблицы"].Загрузить(СтруктураТаблицИнтерфейса.НастройкиСоставаКолонокСводнойТаблицы);
	ЭтаФорма["НастройкиПолейСводнойТаблицы"].Загрузить(СтруктураТаблицИнтерфейса.НастройкиПолейСводнойТаблицы);
	ЭтаФорма["НастройкиСоставаАналитикиСтрокСводнойТаблицы"].Загрузить(СтруктураТаблицИнтерфейса.НастройкиСоставаАналитикиСтрокСводнойТаблицы);
	
	АналитикиСтрок  = СтруктураТаблицИнтерфейса.НастройкиРасположенияПолейСводнойТаблицы.НайтиСтроки(Новый Структура("ОсьТаблицы",Перечисления.ОсиСводнойТаблицы.Строки));
	АналитикиКолонок  = СтруктураТаблицИнтерфейса.НастройкиРасположенияПолейСводнойТаблицы.НайтиСтроки(Новый Структура("ОсьТаблицы",Перечисления.ОсиСводнойТаблицы.Колонки));
	Для Каждого Стр Из АналитикиСтрок Цикл
		Если Стр.Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.СтрокиОтчетов
			ИЛИ Стр.Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.КолонкиОтчетов
				ИЛИ  Стр.Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.ПериодОтчета
					ИЛИ Стр.ЭтоПроизводноеПоле Тогда
			        Продолжить;
		КонецЕсли;			
		нСтр = ЭтаФорма["НастройкиРасположенияПолейСводнойТаблицы"].Добавить();
		ЗаполнитьЗначенияСвойств(нСтр,Стр);
	КонецЦикла;	
	
	Для Каждого Стр Из АналитикиКолонок Цикл
		Если Стр.Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.СтрокиОтчетов
			ИЛИ Стр.Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.КолонкиОтчетов
				ИЛИ  Стр.Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.ПериодОтчета 
					ИЛИ Стр.ЭтоПроизводноеПоле Тогда
	...
	ИндПредставления = 1;
	Для Каждого Стр Из ЭтаФорма["НастройкиРасположенияПолейСводнойТаблицы"] Цикл
		Стр.ПредставлениеАналитики  = "Представление" + Формат(ИндПредставления,"ЧГ=");
		ИндПредставления = ИндПредставления+1
	КонецЦикла;	
...
Процедура ВыборИзМенюОкончание(Результат, ДополнительныеПараметры) Экспорт
	...
	Если  ТекущийВыбор.Значение = Нстр("ru = 'Все элементы'") Тогда
		Элементы.ДеревоНастроек.ТекущиеДанные[ИмяПоля] = Нстр("ru = 'Все элементы'");
		...
		Элементы.ДеревоНастроек.ТекущиеДанные["АналитикаИзменена"] = Истина;
	ИначеЕсли  ТекущийВыбор.Значение = Нстр("ru = 'Отбор'") Тогда	
		Если  НЕ СтруткураРасшифровки=Неопределено  Тогда 	
			...
			ОткрытьФорму("Обработка.АналитическийБланкСводнаяТаблица.Форма.ФормаОтборовАналитик", 
			СтруткураРасшифровки,ЭтаФорма,,,,УстановкаОтбораЗавершение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца); 	 	
		КонецЕсли;	
	ИначеЕсли  ТекущийВыбор.Значение =Нстр("ru = 'Синхронизировать по функц. направлениям'") Тогда		
		Элементы.ДеревоНастроек.ТекущиеДанные[ИмяПоля] = Нстр("ru = 'Синхронизировать по функц. направлениям'");
		Поле_ЗначенниеАналитики = СтрЗАменить(ИмяПоля,"Представление","ЗначениеНастройки");
		Элементы.ДеревоНастроек.ТекущиеДанные[Поле_ЗначенниеАналитики] = "";
		Элементы.ДеревоНастроек.ТекущиеДанные["АналитикаИзменена"] = Истина;			
	КонецЕсли;
...
Процедура СкопироватьНастройку(Команда)
	...
	Для Каждого Стр Из Элементы.ДеревоНастроек.ВыделенныеСтроки Цикл
		...
		Если  ВыделеннаяСтрока.Заголовок = Нстр("ru = 'Состав аналитики'") Тогда
...
Процедура Применить(Команда)
	...
	Закрыть(Новый Структура("Изменено,НастройкиСоставаАналитикиСтрокСводнойТаблицыАдрес",Истина,НастройкиСоставаАналитикиСтрокСводнойТаблицыАдрес));
...
Процедура ДеревоНастроекВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Найти(Поле.Имя,"Представление")>0 Тогда
		...
		СтруткураРасшифровки.Вставить("Тип_",ОпределитьТипТекущейКолонки(Поле.Имя));
		СтруткураРасшифровки.Вставить("ТекНастройка",Элемент.ТекущиеДанные.Ссылка);
		СтруткураРасшифровки.Вставить("НомерНастройки",Элемент.ТекущиеДанные.НомерНастройки);
		СтруткураРасшифровки.Вставить("ТекПоле",Поле.Имя);	
		СтруткураРасшифровки.Вставить("ТекОтбор_","");
		СтруткураРасшифровки.Вставить("ВидНастройки", "Отбор");
		СтруткураРасшифровки.Вставить("НазначениеНастройки",ЭтаФорма["НастройкиРасположенияПолейСводнойТаблицы"].НайтиСтроки(Новый Структура("ПредставлениеАналитики",Поле.Имя))[0].Поле);			
		...
		СписокДоступныхЗначений.Добавить(Нстр("ru = 'Все элементы'"));
		СписокДоступныхЗначений.Добавить(Нстр("ru = 'Отбор'"));
...
Процедура СоздатьДеревоНастроек()
	...
	ДоступныеАналитики = ЭтаФорма["НастройкиРасположенияПолейСводнойТаблицы"].Выгрузить();
	
	Запрос 		=  СоздатьТекстЗапросаНастроекСоставаАналитик(ДоступныеАналитики);
	...
	Если Элементы.ДеревоНастроек.ПодчиненныеЭлементы.Количество()=0 Тогда
		Для Каждого Колонка ИЗ ТЗданные.Колонки Цикл	
			...
			ТекКолонка = ДоступныеАналитики.НайтиСтроки(Новый Структура("ПредставлениеАналитики",Колонка.Имя));
			Если ТекКолонка.Количество()=1 Тогда
				ЭтаФорма.Элементы[Колонка.Имя].Заголовок = Строка(ТекКолонка[0].Поле);
				Если ТекКолонка[0].Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.Организация  
					ИЛИ  ТекКолонка[0].Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.Проект 
					ИЛИ  ТекКолонка[0].Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.Сценарий Тогда 
					ЭтаФорма.Элементы[Колонка.Имя].КартинкаШапки = БиблиотекаКартинок.Зашифрован;	
				КонецЕсли;
				
				Если Строка(ТекКолонка[0].Поле) = "Проекты" Тогда
					Если НЕ РазделениеПоПроектам Тогда
						ЭтаФорма.Элементы[Колонка.Имя].Видимость = Ложь;
					КонецЕсли;	
				КонецЕсли;	
			КонецЕсли;
			Если ИндКолонки>ДоступныеАналитики.Количество()+1  Тогда
				ЭтаФорма.Элементы[Колонка.Имя].Видимость = Ложь;
			КонецЕсли;
...
Процедура ОбойтиДЗ(СтрокиДЗ,СтркиКУдалению,ТЗДанныеЗначенийОтборов)
	...
	кэшАналитикПоказателей = АналитическийБланкГенерацияМакетаУХСервер.ПолучитьАналитикиРаскрытияПоказателей(текПоказатели.ВыгрузитьКолонку("Ссылка"));
	кэшАналитикПоказателей.Индексы.Добавить("Показатель"); 

	//ЗначенияОборовНастроек = РегистрыСведений.НастройкиСоставаАналитикиСтрокСводнойТаблицы.СоздатьНаборЗаписей();
	//ЗначенияОборовНастроек.Отбор.ВариантСводнойТаблицы.Установить(ВариантСводнойТаблицы);
	//ЗначенияОборовНастроек.Прочитать();
	тЗначенияОтборов = ЭтаФорма["НастройкиСоставаАналитикиСтрокСводнойТаблицы"].Выгрузить();
	//ЗначенияОборовНастроек.Выгрузить(); 
	...
	Для Каждого СтрокаДерева Из СтрокиДЗ Цикл
		...
		Если СтрокаДерева.Заголовок="<...>" 
			ИЛИ СтрокаДерева.Заголовок=Нстр("ru = 'Состав аналитики'")  Тогда
			
			СтрокаДерева.Заголовок=Нстр("ru = 'Состав аналитики'");  	
			
			Для Каждого Поле из ЭтаФорма["НастройкиРасположенияПолейСводнойТаблицы"] Цикл
				
				СохраненныеЗначения = тЗначенияОтборов.НайтиСтроки(Новый Структура("СтрокаОтчета,Назначение",СтрокаДерева.Ссылка,Поле.Поле));
				...
				Если СтрокаДерева[Поле.ПредставлениеАналитики] = Null Тогда
					СтрокаДерева[Поле.ПредставлениеАналитики] = Нстр("ru = 'Все элементы'");
				КонецЕсли;
				
				Если Поле.Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.Организация 
					ИЛИ Поле.Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.Проект 
					ИЛИ Поле.Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.Сценарий Тогда
					ЕстьАналитика = Истина;
					Продолжить;	
				КонецЕсли;	
				
				НужныйПоказатель = КэшАналитикПоказателей.НайтиСтроки(Новый Структура("СтрокаОтчета",СтрокаДерева.СтрокаОтчета));
				...
				Иначе	
					...
					Для Инд = 1 По ПараметрыСеанса.ЧислоДопАналитик Цикл
						Если НужныйПоказатель[0]["ВидАналитики"+Инд+"Код"] = СокрЛП(Поле.КодПоля) Тогда   
							ЕстьАналитика = Истина;
							Прервать;
						КонецЕсли;	
					КонецЦикла;	
					Если Поле.КодПоля ="ДопВалюты" Тогда
						Если ЗначениеЗаполнено(НужныйПоказатель[0]["ВидАналитикиВалютаКод"]) Тогда
							ЕстьАналитика = Истина;
						КонецЕсли;	
					КонецЕсли;	
...
Процедура ДобавитьНастройкуСоставаСервер(ИдентификаторСтроки,Источник=Неопределено)
	...
	Новстр.Заголовок=Нстр("ru = 'Состав аналитики'");   	
	...
	Для Каждого Поле из СтруктураТаблицИнтерфейса.ДоступныеАналитики Цикл
		...
		Если Источник=Неопределено Тогда
			Новстр[Поле.ПредставлениеАналитики] = Нстр("ru = 'Все элементы'");
		Иначе
...
Функция ОпределитьТипТекущейКолонки(ИмяТекЭлемента)
	...
	ТекущаяАналитика =  ЭтаФорма["НастройкиРасположенияПолейСводнойТаблицы"].НайтиСтроки(Новый Структура("ПредставлениеАналитики",ИмяТекЭлемента))[0];
	 
	Если ТекущаяАналитика.Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.Организация
		ИЛИ ТекущаяАналитика.Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.Проект
		ИЛИ ТекущаяАналитика.Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.Сценарий Тогда
		Возврат Тип(ТекущаяАналитика.ТипСтрока);
	Иначе	
		Возврат ТекущаяАналитика.Поле;
	КонецЕсли;	
...
Процедура УстановитьЗначениеСтруктурыОтбора(СтруткураРасшифровки,Значение)
	...
	Если  Значение = Нстр("ru = 'Все элементы'") Тогда
...
Процедура СохранитьЭлементыНастроек()
	...
	ЭтаФорма["НастройкиСоставаАналитикиСтрокСводнойТаблицы"].Очистить();
	...
	ИзмененныеСтроки = ДеревоНастроек_.Строки.НайтиСтроки(Новый Структура("ИндексКартинки",8),Истина);
	...
	НастройкиСоставаАналитикиСтрокСводнойТаблицыАдрес = ПоместитьВоВременноеХранилище(ЭтаФорма["НастройкиСоставаАналитикиСтрокСводнойТаблицы"].Выгрузить(),Новый УникальныйИдентификатор);
...
Процедура ОбновитьСвойстваСтроки(СтрокаНастройки)
	...
	Для Каждого Поле Из ЭтаФорма["НастройкиРасположенияПолейСводнойТаблицы"] Цикл
		...
		Если ЗначениеЗаполнено(СтрокаНастройки[ЗначениеНастройки]) Тогда
			...
			нНастройкиСоставаБланков = ЭтаФорма["НастройкиСоставаАналитикиСтрокСводнойТаблицы"].Добавить();	
			нНастройкиСоставаБланков.СтрокаОтчета = СтрокаНастройки.Ссылка;
			нНастройкиСоставаБланков.Назначение = Поле.Поле;
...
&НаСервере
Функция СоздатьТекстЗапросаНастроекСоставаАналитик(ДоступныеАналитики) Экспорт
	...
	СтрокиОтчетов = ЭтаФорма["НастройкиСоставаСтрокСводнойТаблицы"].Выгрузить().ВыгрузитьКолонку("СтрокаОтчета");
	...
	Для Каждого Аналитика из ДоступныеАналитики Цикл
		...
		Запрос.УстановитьПараметр("Назначение"+НомерАналитики,Аналитика.Поле);			
		НомерАналитики = НомерАналитики+1;	
	КонецЦикла;
	
	ВсеЭлементыТекст = Нстр("ru = 'Значения контекста'");
	...
	Запрос.Текст ="ВЫБРАТЬ
	              ...
	              |///////////////////////////////////////////////////////////////////////////////	              
				  |ВЫБРАТЬ
				  |	 НастройкиСоставаАналитикиСтрокСводнойТаблицы.СтрокаОтчета КАК СтрокаОтчета,
				  |	 НастройкиСоставаАналитикиСтрокСводнойТаблицы.Назначение КАК Назначение,
				  |	 НастройкиСоставаАналитикиСтрокСводнойТаблицы.НомерНастройки КАК НомерНастройки,
                  |	 НастройкиСоставаАналитикиСтрокСводнойТаблицы.ЗначениеНастройки КАК ЗначениеНастройки,
				  |	 НастройкиСоставаАналитикиСтрокСводнойТаблицы.ПредставлениеНастройки КАК ПредставлениеНастройки
				  |ПОМЕСТИТЬ ВтНастройкиСоставаАналитикиСтрокСводнойТаблицы
				  |ИЗ 
				  |	&НастройкиСоставаАналитикиСтрокСводнойТаблицы КАК НастройкиСоставаАналитикиСтрокСводнойТаблицы
	              |;
	              |
	              |///////////////////////////////////////////////////////////////////////////////	   
				  |ВЫБРАТЬ
				  ...
				  |	НастройкиСоставаАналитикиБланков.НомерНастройки,
	              |	НастройкиСоставаАналитикиБланков.Назначение,
	              |	СтрокиОтчетов.Код КАК ПредставлениеПоказателяКод,
				  |	СтрокиОтчетов.Наименование КАК ПредставлениеПоказателя,
				  |	2 КАК Приоритет,
	              |	СтрокиОтчетов.Ссылка
				  |ПОМЕСТИТЬ НастройкиАналитик
				  |ИЗ
	              |	Справочник.СтрокиОтчетов КАК СтрокиОтчетов
	              |		ЛЕВОЕ СОЕДИНЕНИЕ ВтНастройкиСоставаАналитикиСтрокСводнойТаблицы КАК НастройкиСоставаАналитикиБланков
	              |		ПО СтрокиОтчетов.Ссылка = НастройкиСоставаАналитикиБланков.СтрокаОтчета
				  |ГДЕ
	              |	СтрокиОтчетов.Ссылка В (&СтрокиОтчетов)
	              |	
	              |;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         
	              |////////////////////////////////////////////////////////////////////////////////
	              |ВЫБРАТЬ
				  ...
				  |	НастройкиАналитик.Ссылка КАК СтрокаОтчета,
    ...
    Запрос.УстановитьПараметр("СтрокиОтчетов",СтрокиОтчетов);
	Запрос.УстановитьПараметр("НастройкиСоставаАналитикиСтрокСводнойТаблицы",ЭтаФорма["НастройкиСоставаАналитикиСтрокСводнойТаблицы"].Выгрузить());
	Запрос.УстановитьПараметр("ВсеЭлементыТекст",ВсеЭлементыТекст);
	Запрос.УстановитьПараметр("СоставАналитикиТекст",СоставАналитикиТекст);

	
	Запрос.УстановитьПараметр("ПустойПоказатель",Справочники.СтрокиОтчетов.ПустаяСсылка());
