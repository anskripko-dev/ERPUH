...
Процедура ПолучитьТекущиеОбласти() 
	...
	|					ТОГДА РакурсДанных.Организация = ЕСТЬNULL(втОрганизации.Организации, ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка))
	...
	|					ТОГДА РакурсДанных.Проект = ЕСТЬNULL(втПроекты.Проекты, ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка))
	...
	Пока Выборка.Следующий() Цикл
		 ...
		 Если НЕ ЗначениеЗаполнено(Выборка.ДокументОбласти) Тогда	
			 ПолучитьДокументСогласованияПоРакурсу(ДанныеСогласования.Индекс(Нстр));
		 КонецЕсли;
...
Процедура ДанныеСогласованияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если НЕ Поле.Имя = "ДанныеСогласованияСтатусСогласования" Тогда
		...
		Если НЕ Элементы.ДанныеСогласования.ТекущиеДанные = Неопределено Тогда	
			...
			ОткрытьФорму("Обработка.АналитическийБланкСводнаяТаблица.Форма",ПараметрыФормы);	
		КонецЕсли;			
	КонецЕсли;	
