...
Процедура OLEDBПровайдерНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если СписокПровайдеров.Количество() = 0 Тогда
		Состояние(Нстр("ru = 'Заполнение списка провайдеров OLE DB'"));
...
Функция ВернутьМаскуДиалогаВыбораФайла()
	...
	Если ТипИспользуемогоФайла = "dBase IV" Тогда
		Возврат Нстр("ru = 'Файл Fox Pro (*.dbf)|*.dbf'");
	ИначеЕсли ТипИспользуемогоФайла = "Excel 8.0" Тогда
		Возврат Нстр("ru = 'Файл Microsoft Excel (*.xls)|*.xls'");
	Иначе
		Возврат Нстр("ru = 'Файл Microsoft Access (*.mdb)|*.mdb|Все файлы|*.*'");
	КонецЕсли;
...
Процедура ИзменитьЗаголовкиПолей()
	...
	ИначеЕсли СтрЧислоВхождений(OLEDBПровайдер, "Microsoft.Jet") > 0 Тогда 
		Элементы.ГруппаСервер.Заголовок = Нстр("ru = 'Имя файла'");
	Иначе
		Элементы.ГруппаСервер.Заголовок  = Нстр("ru = 'Источник данных'");
		Элементы.ГруппаТаблица.Заголовок = Нстр("ru = 'Начальный каталог'");
	КонецЕсли;
