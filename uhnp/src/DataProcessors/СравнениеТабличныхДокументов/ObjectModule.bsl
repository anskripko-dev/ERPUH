...
Процедура СравнитьСвойстваЯчеек(ПараметрыСравнения)
	...
	Если ТипЗнч(Ячейка1) <> ТипЗнч(Ячейка2) Тогда
		...
		Если ТипЗнч(Ячейка1) = Тип("РисунокТабличногоДокумента")
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, "Примечание");
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Ячейка1.Имя);
		КонецЕсли;
	...
	Иначе
		...
		СравнитьСвойствоЯчеек(ПараметрыСравнения, Строка(ТипРисункаТабличногоДокумента.Примечание));// РисунокТабличногоДокумента
		СравнитьСвойствоЯчеек(ПараметрыСравнения, "РазмерКартинки"); 						// РазмерКартинки
...
Процедура СравнитьСвойствоЯчеек(ПараметрыСравнения, Свойство)
	...
	Значение1 = Ячейка1[Свойство]; // Картинка, Структура
	Значение2 = Ячейка2[Свойство]; // Картинка, Структура
	...
	Если НЕ ДанныеСовпадают Тогда
		...
		Если ТипЗнч(Ячейка1) = Тип("РисунокТабличногоДокумента")
			И Ячейка1.ТипРисунка = ТипРисункаТабличногоДокумента.Примечание Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Свойство, "Примечание", Значение1, Значение2);
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Свойство, Ячейка1.Имя, Значение1, Значение2);
		КонецЕсли;
		Ошибки.Добавить(ТекстОшибки);
		Ячейка1.ЦветФона = Метаданные.ЭлементыСтиля.ПросроченныеДанныеЦвет.Значение;
		Ячейка2.ЦветФона = Метаданные.ЭлементыСтиля.ПросроченныеДанныеЦвет.Значение;
...
Функция СравнитьСтрокиПоШаблону(Знач Строка1, Знач Строка2)
	...
	Строка1 = СтрЗаменить(Строка1, "_", ".");
	Строка1 = СтрЗаменить(Строка1, "[", ".");
	Строка1 = СтрЗаменить(Строка1, "]", ".");
	
	Строка2 = СтрЗаменить(Строка2, "%", ".");
	Строка2 = СтрЗаменить(Строка2, "_", ".");
	Строка2 = СтрЗаменить(Строка2, "[", ".");
	Строка2 = СтрЗаменить(Строка2, "]", ".");
	Строка2 = СтрЗаменить(Строка2, "*", "%");
	...
	Запрос.УстановитьПараметр("Строка2", Строка2);
