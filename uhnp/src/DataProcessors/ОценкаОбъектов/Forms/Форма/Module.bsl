...
Процедура ЗаполнитьТаблицуОбъектовВыбора()
	...
	Если ЗначениеЗаполнено(ВладелецОбъектовВыбора) И ЗначениеЗаполнено(Сотрудник)
		...
		ИначеЕсли ТипЗнч(ВладелецОбъектовВыбора) = Тип("ДокументСсылка.Лот") Тогда
			Запрос.УстановитьПараметр("ЗакупочнаяПроцедура", ВладелецОбъектовВыбора.ЗакупочнаяПроцедура);
		Иначе
			Запрос.УстановитьПараметр("ЗакупочнаяПроцедура", Документы.Лот.ПустаяСсылка());
		КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ДопПараметры = Новый Структура("Владелец,Идентификатор", ВладелецОбъектовВыбора, ЭтаФорма.УникальныйИдентификатор);
	Оповестить("ОбновитьСостояниеВыбораАльтернативЗакрытиеФормы", ДопПараметры);
