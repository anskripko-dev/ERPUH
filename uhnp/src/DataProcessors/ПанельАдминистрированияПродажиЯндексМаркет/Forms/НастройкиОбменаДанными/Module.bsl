		...
		Если ЗначениеЗаполнено(ПредыдущийПартнер) 
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(
			   НСтр("ru = 'Не указан партнер, представляющий торговую площадку Яндекс Маркет.';
					|en = 'The partner that represents Yandex Market is not specified.'"), ,
			   "Партнер");
			Возврат;
		...
		ИначеЕсли ЗначениеЗаполнено(ПредыдущийПартнер) 
			...
			ПоказатьВопрос(ОповещениеОЗавершении, 
				НСтр("ru = 'Данные о партнере используются в процессе импорта товарного каталога. Для корректной работы
							...
							|en = 'Partner data is used during the goods directory import. For the correct
							|import, update the partner data in the information about the goods to import.
		...
		Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
			...
			Иначе
				...
				ПараметрыОжидания.ТекстСообщения                   = НСтр("ru = 'Перенос позиций товарного каталога Яндекс Маркет.';
																			|en = 'Transfer items of the Yandex.Market goods directory.'");
				...
				ПараметрыОжидания.ОповещениеПользователя.Пояснение = НСтр("ru = 'Завершен перенос позиций товарного каталога Яндекс Маркет.';
																			|en = 'Items of the Yandex.Market goods directory are transferred.'");
...
Функция ПеренестиНоменклатуруПартнераНаСервере()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания  = НСтр("ru = 'Яндекс Маркет. Перенос позиций товарного каталога.';
															|en = 'Yandex.Market. Transfer goods directory items.'");
		...
		Если СтрНайти(СообщениеПользователю, СтрокаПоиска) > 0 Тогда 
			СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'У пользователя недостаточно прав на исполнение операции над базой данных. Проверьте доступность работы с организацией ""%1"" в настройках профиля доступа.';
					|en = 'The user has insufficient rights to perform operations with the database. Check whether it is possible to operate with the ""%1"" company in the access profile settings.'"), 
				Организация);
		КонецЕсли;    
