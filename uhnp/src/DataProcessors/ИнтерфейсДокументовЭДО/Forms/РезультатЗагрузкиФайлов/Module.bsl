...
Процедура УстановитьВидимостьИЗаголовки()
	...
	ИначеЕсли СчетчикЗагруженных = 0
		И Параметры.РезультатЗагрузки.Количество() = 0 Тогда
		Заголовок = НСтр("ru = 'Файл не загружен';
						|en = 'The file is not imported'");
	...
	Если СчетчикЗагруженных > 0 Тогда
		...
		НаименованиеРазделаСНовымиДокументами = НСтр("ru = '""Исходящие""';
													|en = '""Outgoing""'");
		ВариантыИнтерфейса = ИнтерфейсДокументовЭДОКлиентСервер.ВариантыИнтерфейсаДокументовЭДО();
		Если ИнтерфейсДокументовЭДО.ВариантИнтерфейсаДокументовЭДО() = ВариантыИнтерфейса.ЛегкийКлиентЭДО
			И НастройкиЭДО.ИспользуютсяЧерновикиЭлектронныхДокументов() Тогда
			НаименованиеРазделаСНовымиДокументами = НСтр("ru = '""Черновики""';
														|en = '""Drafts""'");
		КонецЕсли;
			
		ТекстСообщения =  СтрЗаменить(ТекстСообщения, "&2", НаименованиеРазделаСНовымиДокументами);
