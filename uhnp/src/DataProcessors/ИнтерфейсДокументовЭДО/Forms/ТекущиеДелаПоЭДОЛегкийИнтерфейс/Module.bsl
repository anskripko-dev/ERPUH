...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.Состояние.СписокВыбора.Вставить(0, 
		"ТребуетВнимания", НСтр("ru = 'Требует внимания';
								|en = 'Requires attention'"),, БиблиотекаКартинок.ТребуетВниманияЖелтыйШарБЭД);
...
Процедура ПриглашенияПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Для Каждого ТекущаяСтрока Из Строки Цикл
		...
		Если Не ЗначениеЗаполнено(ДанныеСтроки.Контрагент) Тогда
			ДанныеСтроки.Контрагент = СтрШаблон(НСтр("ru = '%1 (ИНН: %2)';
													|en = '%1 (TIN: %2)'", "ru"), 
...
Процедура ЗагрузитьНовыйДокументСДиска(Команда)
	...
	Шаблон = НСтр("ru = 'Для создания документа на основании файла необходимо подключиться к ЭДО и повторить действие.';
					|en = 'To create the document based on the file, connect to EDI and try again.'");
	Операция = НСтр("ru = 'Создание документа на основании файла';
					|en = 'Create document based on file'");
...
Процедура Создать(Команда)
	...
	Шаблон = НСтр("ru = 'Для продолжения отправки документа необходимо подключиться к ЭДО и повторить действие.';
					|en = 'To continue sending the document, connect to EDI and try again.'");
...
Процедура НовоеПриглашение(Команда)
	...
	Шаблон = НСтр("ru = 'Для продолжения отправки приглашений необходимо подключиться к ЭДО и повторить действие.';
					|en = 'To continue sending invitations, connect to EDI and try again.'");
	Операция = НСтр("ru = 'Отправка приглашений';
					|en = 'Sending invitations'");
	Если ИнтерфейсДокументовЭДОКлиент.ПрерватьВыполнениеОперацииДляПодключенияКЭДО(ЭтотОбъект, Шаблон, Операция) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьФайлы(Команда)
	...
	Шаблон = НСтр("ru = 'Для выполнения массовой отправки файлов необходимо подключиться к ЭДО и повторить действие.';
					|en = 'To transfer files in bulk, connect to EDI and try again.'");
	Операция = НСтр("ru = 'Массовая отправка файлов';
					|en = 'Bulk file sending'");
	Если ИнтерфейсДокументовЭДОКлиент.ПрерватьВыполнениеОперацииДляПодключенияКЭДО(ЭтотОбъект, Шаблон, Операция) Тогда
		Возврат;
	КонецЕсли;
