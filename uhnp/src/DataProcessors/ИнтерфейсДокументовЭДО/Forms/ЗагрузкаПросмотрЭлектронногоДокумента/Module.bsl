...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЭлектронныеДокументыЭДО.ЗагрузкаФорматаПоддерживается(ПараметрыФайла) Тогда
		Отказ = Истина;
		СообщитьЗагрузкаФорматаНеПоддерживается(ПараметрыФайла.Формат);
	КонецЕсли;
...
Процедура ПерезаполнитьТабличныйДокумент()
	...
	Если ЗначениеЗаполнено(ДанныеФайлаРазбора) Тогда
		...
		Если ЗначениеЗаполнено(ПараметрыФайла) Тогда
			...
			ПараметрыПредставления.Формат = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыФайла, "Формат", "");
...
&НаСервереБезКонтекста
Процедура СообщитьЗагрузкаФорматаНеПоддерживается(Формат)
