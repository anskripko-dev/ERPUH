...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	ИмяСпискаДокументов = Элементы.Список.Имя;
	ВидыЭлементовОтбораСпискаДокументов = ВидыЭлементовОтбораСпискаДокументов(ЭтотОбъект);
	
	Для Каждого ВидЭлементаОтбора Из ВидыЭлементовОтбораСпискаДокументов Цикл
		ОтборПоЭлементуПриЗагрузкеИзНастроек(ЭтотОбъект, ИмяСпискаДокументов, ВидЭлементаОтбора.Значение, Настройки);
	КонецЦикла;
...
Процедура ВидЭДПриИзменении(Элемент)
	...
	УстановитьОтборВСпискеПоВидуЭлектронногоДокумента(Элементы.Список.Имя,
		ВидыЭлементовОтбораСпискаДокументов(ЭтотОбъект).ВидЭД);
...
Процедура НаправлениеЭДОПриИзменении(Элемент)
	...
	УстановитьОтборВСпискеПоЭлементуКлиентСервер(Список, ВидыЭлементовОтбораСпискаДокументов(ЭтотОбъект).НаправлениеЭДО,
		НаправлениеЭДО);
	ПриИзмененииОтборовДокументов();
...
Процедура СостояниеЭДОПриИзменении(Элемент)
	...
	УстановитьОтборВСпискеПоСостояниюЭДОКлиентСервер(Список,
		ВидыЭлементовОтбораСпискаДокументов(ЭтотОбъект).СостояниеЭДО, СостояниеЭДО, СостоянияЗавершенногоЭДО);
	ПриИзмененииОтборовДокументов();
...
Процедура ОтраженВУчетеПриИзменении(Элемент)
	...
	УстановитьОтборВСпискеОтраженВУчетеКлиентСервер(Список,
		ВидыЭлементовОтбораСпискаДокументов(ЭтотОбъект).ОтраженВУчете, ОтраженВУчете);
	ПриИзмененииОтборовДокументов();
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	УстановитьОтборВСпискеПоЭлементуКлиентСервер(Список, ВидыЭлементовОтбораСпискаДокументов(ЭтотОбъект).Контрагент,
		Контрагент);
	ПриИзмененииОтборовДокументов();
...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	УстановитьОтборВСпискеПоЭлементуКлиентСервер(Список, ВидыЭлементовОтбораСпискаДокументов(ЭтотОбъект).Организация,
		Организация);
	ПриИзмененииОтборовДокументов();
...
Процедура УстановитьСтатусПодготовленКОтправке(Команда)
	...
	ТекстОповещения = НСтр("ru = 'Изменен статус контейнеров на ""Подготовлен к отправке""';
							|en = 'Status of the containers is changed to ""Prepared to send""'" + ": (%1)");
	ТекстОповещения = СтрЗаменить(ТекстОповещения, "%1", Количество);
	ПоказатьОповещениеПользователя(НСтр("ru = 'Обмен электронными документами';
										|en = 'Electronic document exchange'"), , ТекстОповещения);
	Элементы[ТаблицаКонтейнеров].Обновить();
...
Процедура СохранитьКонтейнерыЭДНаДиск(Команда)
	...
	Если МассивФайлов.Количество() Тогда
		ФайловаяСистемаКлиент.СохранитьФайлы(Неопределено, МассивФайлов);
	КонецЕсли;
...
Функция ПолучитьДанныеПрисоединенныхФайловКонтейнеровЭДНаСервере(Знач КонтейнерыЭД, УникальныйИдентификатор)
	...
	Для Каждого ФайлыКонтейнера Из ФайлыКонтейнеровЭД Цикл
		Для Каждого ФайлКонтейнера Из ФайлыКонтейнера.Значение Цикл
...
Процедура ОтборПоЭлементуПриЗагрузкеИзНастроек(Форма, ИмяСписка, ВидЭлемента, Настройки)
	...
	Если ЗначениеЗаполнено(ЗначениеЭлемента) Тогда
		...
		Если ВидЭлемента = ВидыЭлементовОтбораСпискаДокументов.ВидЭД Тогда
			УстановитьОтборВСпискеПоВидуЭлектронногоДокумента(ИмяСписка, ВидЭлемента);
		ИначеЕсли ВидЭлемента = ВидыЭлементовОтбораСпискаДокументов.Период Тогда
			УстановитьОтборВСпискеПоПериодуКлиентСервер(СписокДанных, "ДатаДокумента", ЗначениеЭлемента);
		ИначеЕсли ВидЭлемента = ВидыЭлементовОтбораСпискаДокументов.СостояниеЭДО Тогда
			УстановитьОтборВСпискеПоСостояниюЭДОКлиентСервер(СписокДанных, "СостояниеЭДО",
				ЗначениеЭлемента, СостоянияЗавершенногоЭДО);
		ИначеЕсли ВидЭлемента = ВидыЭлементовОтбораСпискаДокументов.ОтраженВУчете Тогда
			УстановитьОтборВСпискеОтраженВУчетеКлиентСервер(СписокДанных, "ОтраженВУчете", ЗначениеЭлемента);
		Иначе
...
//   * Контрагент - Строка
//   * ВидЭД - Строка
//   * НаправлениеЭДО - Строка
//   * СостояниеЭДО - Строка
//   * ОтраженВУчете - Строка
//
&НаКлиентеНаСервереБезКонтекста
Функция ВидыЭлементовОтбораСпискаДокументов(Форма)
	
	ВидыЭлементовОтбора = Новый Структура;
	ВидыЭлементовОтбора.Вставить("Период", Форма.Элементы.Период.Имя);
	ВидыЭлементовОтбора.Вставить("Организация", Форма.Элементы.Организация.Имя);
	ВидыЭлементовОтбора.Вставить("Контрагент", Форма.Элементы.Контрагент.Имя);
	ВидыЭлементовОтбора.Вставить("ВидЭД", Форма.Элементы.ВидЭД.Имя);
	ВидыЭлементовОтбора.Вставить("НаправлениеЭДО", Форма.Элементы.НаправлениеЭДО.Имя);
	ВидыЭлементовОтбора.Вставить("СостояниеЭДО", Форма.Элементы.СостояниеЭДО.Имя);
	ВидыЭлементовОтбора.Вставить("ОтраженВУчете", Форма.Элементы.ОтраженВУчете.Имя);

	Возврат ВидыЭлементовОтбора;
