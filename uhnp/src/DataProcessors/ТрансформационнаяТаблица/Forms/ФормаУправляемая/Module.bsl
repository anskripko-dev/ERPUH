...
Процедура СформироватьСервер(Результат)
	...
	НастройкиКомпоновки.ПараметрыДанных.УстановитьЗначениеПараметра("СокращеннаяОСВ",	ЭкземплярОтчета.ВидОтчета.СокращеннаяОСВ);
...
Процедура СценарийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.Сценарии.Форма.ФормаВыбора", Новый Структура("ТолькоОткрытые", Истина), Элемент,);
