...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.РежимРаботыОбработки) Тогда
		 Объект.РежимРаботыОбработки = Перечисления.РежимыКопированияДанныхПоказателей.УОП;
		 УстановитьРежимКопирования();
	КонецЕсли;	
	
	Если Параметры.Свойство("тПоказатели")  Тогда	
		...
		СводнаяТаблицаУХ.ЗаполнитьДоступныеАналитикиДляОбработкиПереноса(ВыбранныеПоказатели,ОтборыРасширеннойНастройки);
...
Процедура ВыполнитьКопирование(Команда)
	...
	СтруктураВыполнения = ВыполнитьКопированиеДанных();
	
	Если СтруктураВыполнения.СтатусВыполнения = "Успех" Тогда
		ОбщегоНазначенияУХ.СообщитьОбОшибке(Нстр("ru = 'Копирование выполнено успешно'"), , , СтатусСообщения.Важное);
	ИначеЕсли СтруктураВыполнения.СтатусВыполнения = "Пусто" Тогда 
		ОбщегоНазначенияУХ.СообщитьОбОшибке(Нстр("ru = 'Не найдены данные для копирования'"), , , СтатусСообщения.Важное);
	Иначе
		ОбщегоНазначенияУХ.СообщитьОбОшибке(Нстр("ru = 'Возникли ошибки при копировании'"), , , СтатусСообщения.Важное);
	КонецЕсли;
	
	ПоказатьПротокол(СтруктураВыполнения.ИнформацияОбОшибке);
...
Функция ВыполнитьКопированиеДанных()
	...
	Если ТЗВерсийИсточник.Количество() + ТЗВерсийПриемник.Количество() = 0 Тогда		
		СтруктураВыполнения.Вставить("СтатусВыполнения", "Пусто");
		Возврат СтруктураВыполнения;
	КонецЕсли;

	АдресХранилищаСтатусаКопирования = ПоместитьВоВременноеХранилище(СтруктураВыполнения, Новый УникальныйИдентификатор);
	
	СводнаяТаблицаУХ.ВыполнитьКопированиеВФоне(АдресХранилищаСтатусаКопирования, СтруктураТаблиц); 
	
	Возврат ПолучитьИзВременногоХранилища(АдресХранилищаСтатусаКопирования);
...
Функция ПодготовитьСтруктуруПараметров()
	...
	СтруктураПараметров.Вставить("УОППриемник",Объект.УОППриемник);
...
&НаСервере
Функция ЗаполнитьСоставСтрокИПоказателейУОП()
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТекущаяВерсияРегламента", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.УОПИсточник, "ВерсияОрганизационнойСтруктуры"));
	
	Если УправлениеПроцессамиПодготовкиОтчетности.ЗапущенПроцессПодготовкиОтчетности(Объект.УОПИсточник) Тогда
	...
	Иначе
		...
		Запрос.Текст = 
			...
			|ГДЕ
			|	ВидыОтчетов.Ссылка В ИЕРАРХИИ
			|			(ВЫБРАТЬ
			|				ВерсииРегламентовПодготовкиОтчетности.ВидОтчета КАК ВидОтчета
			|			ИЗ
			|				Справочник.ВерсииРегламентовПодготовкиОтчетности КАК ВерсииРегламентовПодготовкиОтчетности
			|			ГДЕ
			|				ВерсииРегламентовПодготовкиОтчетности.Ссылка = &ТекущаяВерсияРегламента)
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
...
Процедура ОпределитьСдвигПоПериоду()
	...
	ПериодыУХПовтИсп.ПроверитьИнтервалПериодовВФоне(ДатаНачалаИнтервала, ДатаОкончанияИнтервала, Объект.ПериодичностьИсточника);
	...
	ПериодыУХПовтИсп.ПроверитьИнтервалПериодовВФоне(ДатаНачалаИнтервала, ДатаОкончанияИнтервала, Объект.ПериодичностьПриемника);
...
Процедура УправлениеФормой()
	...
	Если Объект.РежимРаботыОбработки = Перечисления.РежимыКопированияДанныхПоказателей.УОП  Тогда	 	 
		...
		Элементы.УОПИсточник.Видимость 				= Истина;
		Элементы.УОППриемник.Видимость	 			= Истина;
		Элементы.ГруппаПериоды.Видимость 			= Ложь;
		Элементы.ГруппаПериоды1.Видимость 			= Ложь;
		Элементы.ПериодичностьИсточника.Доступность = Ложь;
		Элементы.ПериодичностьПриемника.Доступность 	= Ложь;
	
		Элементы.ПеренестиЭкземплярыОтчетов.Видимость = Истина;
			
		Элементы.СценарийИсточник.ТолькоПросмотр = Истина;
		Элементы.СценарийПриемник.ТолькоПросмотр = Истина;
		
		Элементы.УОППриемник.Доступность = ЗначениеЗаполнено(Объект.УОПИсточник);
		
		Если ЗначениеЗаполнено(Объект.УОПИсточник) Тогда
			
			Если ЗначениеЗаполнено(Объект.УОППриемник)
				И НЕ Объект.УОПИсточник.ТекущаяПериодичность = Объект.УОППриемник.ТекущаяПериодичность Тогда
			...
			Объект.ПериодичностьИсточника = Объект.УОПИсточник.ТекущаяПериодичность;
			Объект.ПериодичностьПриемника = Объект.ПериодичностьИсточника;
			
			Объект.СценарийИсточник = Объект.УОПИсточник.Сценарий;
			
			НовыйПараметр = Новый ПараметрВыбора("Отбор.ТекущаяПериодичность", Объект.УОПИсточник.ТекущаяПериодичность);
			НовыйМассив = Новый Массив();
			НовыйМассив.Добавить(НовыйПараметр);
			НовыеПараметры = Новый ФиксированныйМассив(НовыйМассив);
			...
			Если ЗначениеЗАполнено(Объект.УОППриемник) Тогда
				
				Объект.ПериодПриемникС 	= Объект.УОППриемник.ПериодСценария;
				Объект.ПериодПриемникПо = Объект.УОППриемник.ПериодСценарияОкончание;
				Объект.СценарийПриемник = Объект.УОППриемник.Сценарий;
				
			Иначе
				
				Объект.ПериодПриемникС 	= Справочники.Периоды.ПустаяСсылка();
				Объект.ПериодПриемникПо = Справочники.Периоды.ПустаяСсылка();
				Объект.СценарийПриемник = Справочники.Сценарии.ПустаяСсылка();
	...
	ИначеЕсли Объект.РежимРаботыОбработки = Перечисления.РежимыКопированияДанныхПоказателей.ПроизвольныеОтборы  Тогда	 	 
		...
		Элементы.ПериодичностьИсточника.Доступность 	= Истина;
		Элементы.ПериодичностьПриемника.Доступность 	= Истина;
		
		Элементы.СценарийИсточник.ТолькоПросмотр = Ложь;
		Элементы.СценарийПриемник.ТолькоПросмотр = Ложь;
...
&НаКлиенте
Процедура УОППриемникПриИзменении(Элемент)
