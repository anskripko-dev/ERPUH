...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	...
	УдалитьРеквизитИзНастроек("ВидОбъекта", Настройки);
	УдалитьРеквизитИзНастроек("ВидДокументаОтбор", Настройки);
	УдалитьРеквизитИзНастроек("ВерсияРегламента", Настройки);
...
Процедура ПолеТабличногоДокументаМакетВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ОткрытьФорму("Обработка.МатрицаПолномочий.Форма.ФормаРедактированияОтветственных", 
	Параметры_,,,,,Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца); 
...
Процедура СоздатьРеквизитФильтраШаблонаДокумента()
	...
	РеквизитШаблона = ВидДокументаОтбор.РеквизитШаблонаДокумента;
...
Процедура ОбновитьПолеФильтров()
	...
	Для Каждого СтрПоле из ТаблицаСтраницы Цикл
		...
		Если ЗначениеЗаполнено(ВидДокументаОтбор) Тогда
			...
			Если КодПоля = "Фильтр_ШаблонДокументаБД" И НЕ ИспользуетсяВНастройкахРеглмента Тогда
...
Процедура СоздатьСводнуюТаблицуСКД(ОбновитьСтруктуру = Неопределено)
	...
	Если Не ИспользуетсяВНастройкахРеглмента Тогда
		СТ.ОрганизацииИзРегламента = Ложь;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПодсистемыУХ.СогласованиеЧерез1СДокументооборот") Тогда
		Если ЗначениеЗаполнено(ВидДокументаОтбор) Тогда
			СогласуетсяВДокументообороте = ВидДокументаОтбор.СогласуетсяВДокументообороте;
			
			Элементы.ВерсияРегламента.Доступность = НЕ СогласуетсяВДокументообороте;
			Элементы.ГруппаДополнительныеФильтры.Доступность = НЕ СогласуетсяВДокументообороте;	
			Элементы.Команды.Доступность = НЕ СогласуетсяВДокументообороте;
			Элементы.Группа3.Доступность = НЕ СогласуетсяВДокументообороте;
			Элементы.РегистрПолномочий.Доступность = НЕ СогласуетсяВДокументообороте;
			Элементы.ГруппаСписок.Видимость = НЕ СогласуетсяВДокументообороте;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура УдалитьРеквизитИзНастроек(ИмяРеквизита, Настройки)

	ЗначениеРеквизита = ЭтаФорма[ИмяРеквизита];
	Если ЗначениеЗаполнено(ЗначениеРеквизита) Тогда
		Если ЗначениеРеквизита <> Настройки.Получить(ИмяРеквизита) Тогда
...
&НаКлиенте
Процедура ОрганизацииИзРегламентаПриИзменении(Элемент)
	
	Прменить(Неопределено);
...
&НаСервере
Функция ОбновитьНастройкиРегламента()
	
	Если ЗначениеЗаполнено(ВидДокументаОтбор) И ВидОбъекта = "Документ" Тогда
		ИспользуетсяВНастройкахРеглмента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидДокументаОтбор, "ИспользуетсяВНастройкахРеглмента");
	Иначе
		ИспользуетсяВНастройкахРеглмента = Ложь;
	КонецЕсли;
	
	Элементы.ГруппаРегламент.Видимость = ИспользуетсяВНастройкахРеглмента;
...
Процедура ВидДокументаОтборПриИзмененииСервер()
	...
	Если НЕ ИспользуетсяВНастройкахРеглмента И НЕ ВидДокументаОтбор.Наименование = "ЗаявкаНаИзменениеНСИ" Тогда
