...
// 	ОбъектУчета - ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов - Объект финансового учета, по которому получаются используемые счета учета.
...
Функция НастройкиЗаполненияСубконто(Отбор, КомпоновщикНастроек, ИспользоватьРучныеНастройки = Истина) Экспорт
	...
	|	КОНЕЦ КАК СчетУчета
	|ПОМЕСТИТЬ втНастройкиСчетовПоОбъектам
	|ИЗ
	...
	|ИЗ
	|	втНастройкиСчетовПоОбъектам КАК НастройкиСчетовПоОбъектам
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.НастройкиЗаполненияСубконтоНаСчетахМеждународногоУчета КАК НастройкиЗаполненияСубконто
	|	ПО
	|		НастройкиСчетовПоОбъектам.ПланСчетов = НастройкиЗаполненияСубконто.ПланСчетов
	|		И НастройкиСчетовПоОбъектам.НастройкаФормированияПроводок = НастройкиЗаполненияСубконто.НастройкаФормированияПроводок
	|		И НастройкиСчетовПоОбъектам.ОбъектУчета = НастройкиЗаполненияСубконто.ОбъектУчета
	|		И НастройкиСчетовПоОбъектам.СчетУчета = НастройкиЗаполненияСубконто.СчетУчета
	...
	ОписаниеОбъектаУчета = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ОписаниеОбъектаФинансовогоУчета(Отбор.ОбъектУчета);
...
Процедура ЗаполнитьСостояниеАвтоматическогоЗаполненияСубконто(СтрокаСостоянияНастроек, ОписаниеОбъектаУчета)
	...
	Для НомерСубконто = 1 По 3 Цикл
		...
		ПутьКДаннымСубконто = ОписаниеОбъектаУчета.ПутиКДаннымСубконто[ТекущийВидСубконто];
		
		Если ПутьКДаннымСубконто <> Неопределено Тогда
			СтрокаСостоянияНастроек["ВыражениеЗаполненияСубконто" + НомерСубконто] = ПутьКДаннымСубконто;
		Иначе
