...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОповеститьОНеобработанныхНастройкахЗаполненияСчетовИСубконто();
	ОповеститьОНеобработанныхНастройкахОтраженияКорреспонденций();
	ОповеститьОНеобработанныхНастройкахМеждународногоУчета();
...
&НаКлиенте
Процедура НадписьЕстьНеобработанныеНастройкиЗаполненияСчетовИСубконтоОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Если НавигационнаяСсылкаФорматированнойСтроки = ГиперссылкаОткрытьНеобработанныеНастройкиСчетов()
	 ИЛИ НавигационнаяСсылкаФорматированнойСтроки = ГиперссылкаОткрытьНеобработанныеНастройкиСубконто() Тогда
		ПолноеИмяФормы = ?(
			НавигационнаяСсылкаФорматированнойСтроки = ГиперссылкаОткрытьНеобработанныеНастройкиСчетов(),
			"РегистрСведений.УдалитьНастройкиСчетовМеждународногоУчетаПоОбъектам.ФормаСписка",
			"РегистрСведений.УдалитьНастройкиЗаполненияСубконтоНаСчетахМеждународногоУчета.ФормаСписка");
		ПараметрыФормы = Новый Структура();
		ПараметрыФормы.Вставить("Отбор", Новый Структура("НеОбработанаАвтоматически", Истина));
		ОткрытьФорму(ПолноеИмяФормы, ПараметрыФормы, ЭтотОбъект);
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = ГиперссылкаСкрытьОповещениеНастройкиСчетов() Тогда
		СкрытьОповещениеЕстьНеобработанныеНастройки(НавигационнаяСсылкаФорматированнойСтроки);
		Элементы.ГруппаЕстьНеобработанныеНастройкиЗаполненияСчетовИСубконто.Видимость = Ложь;
		Элементы.ГруппаЕстьНеобработанныеНастройкиМеждународногоУчета.Видимость =
			Элементы.ГруппаЕстьНеобработанныеНастройкиОтраженияКорреспонденций.Видимость;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура НадписьЕстьНеобработанныеНастройкиОтраженияКорреспонденцийОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Если НавигационнаяСсылкаФорматированнойСтроки = ГиперссылкаОткрытьНеобработанныеНастройкиКорреспонденций() Тогда
		ПолноеИмяФормы = "РегистрСведений.УдалитьНастройкиОтраженияКорреспонденцийВМеждународномУчете.ФормаСписка";
		ПараметрыФормы = Новый Структура();
		ПараметрыФормы.Вставить("Отбор", Новый Структура("НеОбработанаАвтоматически", Истина));
		ОткрытьФорму(ПолноеИмяФормы, ПараметрыФормы, ЭтотОбъект);
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = ГиперссылкаСкрытьОповещениеНастройкиКорреспонденций() Тогда
		СкрытьОповещениеЕстьНеобработанныеНастройки(НавигационнаяСсылкаФорматированнойСтроки);
		Элементы.ГруппаЕстьНеобработанныеНастройкиОтраженияКорреспонденций.Видимость = Ложь;
		Элементы.ГруппаЕстьНеобработанныеНастройкиМеждународногоУчета.Видимость =
			Элементы.ГруппаЕстьНеобработанныеНастройкиЗаполненияСчетовИСубконто.Видимость;
	КонецЕсли;
...
&НаКлиенте
Процедура ДеревоОбъектовФинансовогоУчетаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ОбъектУчета) Тогда
...
&НаКлиенте
Процедура ДеревоОбъектовФинансовогоУчетаПоКорреспонденциямВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
...
Процедура НастройкиОтраженияКорреспонденцийПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	ОписаниеОбъектовУчета = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ОписаниеОбъектовФинансовогоУчета(
		НастройкиУчета);
...
&НаКлиенте
Процедура НастройкиОтраженияКорреспонденцийОбработкаЗаписиНового(Элемент, НовыйОбъект, Источник, СтандартнаяОбработка)
	ЗаполнитьИнформациюПоНастройкам();
КонецПроцедуры
...
&НаКлиенте
Процедура ОткрытьУстаревшиеНастройкиСчетовПоОбъектамУчета(Команда)
	
	ПолноеИмяФормы = "РегистрСведений.УдалитьНастройкиСчетовМеждународногоУчетаПоОбъектам.ФормаСписка";
	ОткрытьФорму(ПолноеИмяФормы,, ЭтотОбъект, "Все");
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьУстаревшиеНастройкиЗаполненияСубконто(Команда)
	
	ПолноеИмяФормы = "РегистрСведений.УдалитьНастройкиЗаполненияСубконтоНаСчетахМеждународногоУчета.ФормаСписка";
	ОткрытьФорму(ПолноеИмяФормы,, ЭтотОбъект, "Все");
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьУстаревшиеНастройкиОтраженияКорреспонденций(Команда)
...
&НаСервере
Процедура ОповеститьОНеобработанныхНастройкахЗаполненияСчетовИСубконто()
	...
	|	ИСТИНА
	|ИЗ
	|	РегистрСведений.УдалитьНастройкиСчетовМеждународногоУчетаПоОбъектам КАК НастройкиСчетовУчета
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|	РегистрСведений.УдалитьНастройкиСчетовМеждународногоУчетаПоОбъектам КАК НастройкиСчетовУчета
	|ГДЕ
	|	НастройкиСчетовУчета.НеОбработанаАвтоматически
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|	РегистрСведений.УдалитьНастройкиЗаполненияСубконтоНаСчетахМеждународногоУчета КАК НастройкиЗаполненияСубконто
	|ГДЕ
	|	НастройкиЗаполненияСубконто.НеОбработанаАвтоматически
	|";
	
	Результат = Запрос.ВыполнитьПакет();
	ЕстьУстаревшиеНастройкиСчетов = НЕ Результат[0].Пустой();
	ЕстьУстаревшиеНастройкиСубконто = НЕ Результат[1].Пустой();
	ЕстьНеобработанныеНастройкиСчетов = НЕ Результат[2].Пустой();
	ЕстьНеобработанныеНастройкиСубконто = НЕ Результат[3].Пустой();
	
	Элементы.ОткрытьУстаревшиеНастройкиСчетовПоОбъектамУчета.Видимость = ЕстьУстаревшиеНастройкиСчетов;
	Элементы.ОткрытьУстаревшиеНастройкиЗаполненияСубконто.Видимость = ЕстьУстаревшиеНастройкиСубконто;
	
	Если НЕ Константы.УдалитьОповещениеЕстьНеобработанныеНастройкиМеждународногоУчета.ОтображатьОповещениеЕстьНеобработанныеНастройкиСчетовИСубконто() Тогда
		Элементы.ГруппаЕстьНеобработанныеНастройкиЗаполненияСчетовИСубконто.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	
	Если ЕстьНеобработанныеНастройкиСчетов ИЛИ ЕстьНеобработанныеНастройкиСубконто Тогда
		СтрокиЗаголовка = Новый Массив();
		СтрокиЗаголовка.Добавить(НСтр(
			"ru = '<span style=""color: %1"">При обновлении программы на новую версию
			|не все настройки были автоматически сконвертированны.</span>';
			|en = '<span style=""color: %1"">When updating the program to a new version,
			|not all settings were automatically converted.</span>'"));
		
		Если ЕстьНеобработанныеНастройкиСчетов И ЕстьНеобработанныеНастройкиСубконто Тогда
			СтрокиЗаголовка.Добавить(НСтр(
				"ru = '<span style=""color: %1"">Необработанные</span>
				|<a style=""color: %1; font: %2"" href=""%3"">настройки счетов учета</a>
				|<span style=""color: %1"">и</span>
				|<a style=""color: %1; font: %2"" href=""%4"">настройки заполнения субконто</a>
				|<span style=""color: %1"">не отображаются в списке и не участвуют в формировании проводок.</span>';
				|en = '<span style=""color: %1"">Unprocessed</span>
				|<a style=""color: %1; font: %2"" href=""%3"">ledger account settings</a>
				|<span style=""color: %1"">and</span>
				|<a style=""color: %1; font: %2"" href=""%4"">extra dimension filling settings</a>
				|<span style=""color: %1"">are not displayed in the list and do not generate entries.</span>'"));
		ИначеЕсли ЕстьНеобработанныеНастройкиСчетов Тогда
			СтрокиЗаголовка.Добавить(НСтр(
				"ru = '<span style=""color: %1"">Необработанные</span>
				|<a style=""color: %1; font: %2"" href=""%3"">настройки счетов учета</a>
				|<span style=""color: %1"">не отображаются в списке и не участвуют в формировании проводок.</span>';
				...
				|<a style=""color: %1; font: %2"" href=""%3"">ledger account settings</a>
				|<span style=""color: %1"">are not displayed in the list and do not generate entries.</span>'"));
		ИначеЕсли ЕстьНеобработанныеНастройкиСубконто Тогда
			СтрокиЗаголовка.Добавить(НСтр(
				"ru = '<span style=""color: %1"">Необработанные</span>
				|<a style=""color: %1; font: %2"" href=""%4"">настройки заполнения субконто</a>
				|<span style=""color: %1"">не отображаются в списке и не участвуют в формировании проводок.</span>';
		...
		СтрокиЗаголовка.Добавить(НСтр(
			"ru = '<span style=""color: %1"">Настройки для этих объектов учета необходимо выполнить вручную.</span>
			|<a style=""color: %1; font: %2"" href=""%5"">Больше не показывать это сообщение.</a>';
			|en = '<span style=""color: %1"">Set up these accounting objects manually.</span>
			|<a style=""color: %1; font: %2"" href=""%5"">Do not show this message again.</a>'"));
		
		ШаблонЗаголовка = СтрСоединить(СтрокиЗаголовка, " ");
		СтрокаЗаголовка = СтроковыеФункции.ФорматированнаяСтрока(
			СтрЗаменить(ШаблонЗаголовка, Символы.ПС, " "),
			Метаданные.ЭлементыСтиля.ТекстИнформационнойНадписи.Имя,
			Метаданные.ЭлементыСтиля.ПодчеркнутыйШрифт.Имя,
			ГиперссылкаОткрытьНеобработанныеНастройкиСчетов(),
			ГиперссылкаОткрытьНеобработанныеНастройкиСубконто(),
			ГиперссылкаСкрытьОповещениеНастройкиСчетов());
		
		Элементы.НадписьЕстьНеобработанныеНастройкиЗаполненияСчетовИСубконто.Заголовок = СтрокаЗаголовка;
		Элементы.ГруппаЕстьНеобработанныеНастройкиЗаполненияСчетовИСубконто.Видимость = Истина;
	Иначе
		Элементы.ГруппаЕстьНеобработанныеНастройкиЗаполненияСчетовИСубконто.Видимость = Ложь;
	КонецЕсли;
...
&НаСервере
Процедура ОповеститьОНеобработанныхНастройкахОтраженияКорреспонденций()
	...
	|	РегистрСведений.УдалитьНастройкиОтраженияКорреспонденцийВМеждународномУчете КАК НастройкиОтраженияКорреспонденций
	|ГДЕ
	|	НастройкиОтраженияКорреспонденций.НеОбработанаАвтоматически
	|";
	
	Результат = Запрос.ВыполнитьПакет();
	ЕстьУстаревшиеНастройкиКорреспонденций = НЕ Результат[0].Пустой();
	ЕстьНеобработанныеНастройкиКорреспонденций = НЕ Результат[1].Пустой();

	Элементы.ОткрытьУстаревшиеНастройкиОтображенияКорреспонденций.Видимость = ЕстьУстаревшиеНастройкиКорреспонденций;
	
	Если НЕ Константы.УдалитьОповещениеЕстьНеобработанныеНастройкиМеждународногоУчета.ОтображатьОповещениеЕстьНеобработанныеНастройкиКорреспонденций() Тогда
		Элементы.ГруппаЕстьНеобработанныеНастройкиОтраженияКорреспонденций.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	
	Если ЕстьНеобработанныеНастройкиКорреспонденций Тогда
		ШаблонЗаголовка = НСтр("ru = '<span style=""color: %1"">При обновлении программы на новую версию
			|не все настройки отражения корреспонденций были автоматически сконвертированны. Необработанные</span>
			|<a style=""color: %1; font: %2"" href=""%3"">настройки отражения корреспонденций</a>
			|<span style=""color: %1"">не отображаются в списке и не участвуют в формировании проводок.
			|Настройки для этих корреспонденций необходимо выполнить вручную.</span>
			|<a style=""color: %1; font: %2"" href=""%4"">Больше не показывать это сообщение.</a>';
			|en = '<span style=""color: %1"">When updating the program to a new version,
			|not all correspondence recording settings were automatically converted. Unprocessed</span>
			|<a style=""color: %1; font: %2"" href=""%3"">correspondence recording settings</a>
			|<span style=""color: %1"">are not displayed in the list and do not generate entries.
			|Set up these correspondences manually.</span>
			|<a style=""color: %1; font: %2"" href=""%4"">Do not show this message again.</a>'");
		
		СтрокаЗаголовка = СтроковыеФункции.ФорматированнаяСтрока(
			СтрЗаменить(ШаблонЗаголовка, Символы.ПС, " "),
			Метаданные.ЭлементыСтиля.ТекстИнформационнойНадписи.Имя,
			Метаданные.ЭлементыСтиля.ПодчеркнутыйШрифт.Имя,
			ГиперссылкаОткрытьНеобработанныеНастройкиКорреспонденций(),
			ГиперссылкаСкрытьОповещениеНастройкиКорреспонденций());
		
		Элементы.НадписьЕстьНеобработанныеНастройкиОтраженияКорреспонденций.Заголовок = СтрокаЗаголовка;
		Элементы.ГруппаЕстьНеобработанныеНастройкиОтраженияКорреспонденций.Видимость = Истина;
	Иначе
		Элементы.ГруппаЕстьНеобработанныеНастройкиОтраженияКорреспонденций.Видимость = Ложь;
	КонецЕсли;
...
&НаСервере
Процедура ОповеститьОНеобработанныхНастройкахМеждународногоУчета()
	
	ОповещатьОНеобработанныхНастройкахСчетовИСубконто = Константы.УдалитьОповещениеЕстьНеобработанныеНастройкиМеждународногоУчета.ОтображатьОповещениеЕстьНеобработанныеНастройкиСчетовИСубконто();
	ОповещатьОНеобработанныхНастройкахКорреспонденций = Константы.УдалитьОповещениеЕстьНеобработанныеНастройкиМеждународногоУчета.ОтображатьОповещениеЕстьНеобработанныеНастройкиКорреспонденций();
	
	Если НЕ ОповещатьОНеобработанныхНастройкахСчетовИСубконто И НЕ ОповещатьОНеобработанныхНастройкахКорреспонденций Тогда
		Элементы.ГруппаЕстьНеобработанныеНастройкиМеждународногоУчета.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	
	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("ОповещатьОНеобработанныхНастройкахСчетовИСубконто", ОповещатьОНеобработанныхНастройкахСчетовИСубконто);
	Запрос.УстановитьПараметр("ОповещатьОНеобработанныхНастройкахКорреспонденций", ОповещатьОНеобработанныхНастройкахКорреспонденций);
	...
	|	РегистрСведений.УдалитьНастройкиСчетовМеждународногоУчетаПоОбъектам КАК НастройкиСчетовУчета
	|ГДЕ
	|	&ОповещатьОНеобработанныхНастройкахСчетовИСубконто
	|	И НастройкиСчетовУчета.НеОбработанаАвтоматически
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|ГДЕ
	|	&ОповещатьОНеобработанныхНастройкахСчетовИСубконто
	|	И НастройкиЗаполненияСубконто.НеОбработанаАвтоматически
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|	РегистрСведений.УдалитьНастройкиОтраженияКорреспонденцийВМеждународномУчете КАК НастройкиОтраженияКорреспонденций
	|ГДЕ
	|	&ОповещатьОНеобработанныхНастройкахКорреспонденций
	|	И НастройкиОтраженияКорреспонденций.НеОбработанаАвтоматически
	|";
	
	Результат = Запрос.Выполнить();
	ЕстьНеобработанныеНастройки = НЕ Результат.Пустой();
	
	Если ЕстьНеобработанныеНастройки Тогда
		ШаблонЗаголовка = НСтр("ru = '<span style=""color: %1"">При обновлении программы на новую версию
			|не все настройки были автоматически сконвертированны. Необработанные настройки не отображаются
			|в списке и не участвуют в формировании проводок. Открыть список необработанных настроек
			|и выполнить их ручной перенос можно на других страницах этой формы.</span>';
			|en = '<span style=""color: %1"">When updating the program to a new version,
			|not all settings were automatically converted. Unprocessed settings are not displayed
			|in the list and do not generate entries. You can open the list of unprocessed settings
			|and transfer them manually on other pages of this form.</span>'");
		
		СтрокаЗаголовка = СтроковыеФункции.ФорматированнаяСтрока(
			СтрЗаменить(ШаблонЗаголовка, Символы.ПС, " "),
			Метаданные.ЭлементыСтиля.ТекстИнформационнойНадписи.Имя);
		
		Элементы.НадписьЕстьНеобработанныеНастройкиМеждународногоУчета.Заголовок = СтрокаЗаголовка;
		Элементы.ГруппаЕстьНеобработанныеНастройкиМеждународногоУчета.Видимость = Истина;
	Иначе
		Элементы.ГруппаЕстьНеобработанныеНастройкиМеждународногоУчета.Видимость = Ложь;
	КонецЕсли;
...
&НаСервереБезКонтекста
Процедура СкрытьОповещениеЕстьНеобработанныеНастройки(НавигационнаяСсылка)
	
	Если НавигационнаяСсылка = ГиперссылкаСкрытьОповещениеНастройкиСчетов() Тогда
		Константы.УдалитьОповещениеЕстьНеобработанныеНастройкиМеждународногоУчета.СкрытьОповещениеЕстьНеобработанныеНастройкиСчетовИСубконто();
	ИначеЕсли НавигационнаяСсылка = ГиперссылкаСкрытьОповещениеНастройкиКорреспонденций() Тогда
		Константы.УдалитьОповещениеЕстьНеобработанныеНастройкиМеждународногоУчета.СкрытьОповещениеЕстьНеобработанныеНастройкиКорреспонденций();
	КонецЕсли;
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция ГиперссылкаОткрытьНеобработанныеНастройкиСчетов()
	
	Возврат "ОткрытьСписокНеобработанныеНастройкиСчетов";
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ГиперссылкаОткрытьНеобработанныеНастройкиСубконто()
	
	Возврат "ОткрытьСписокНеобработанныеНастройкиСубконто";
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ГиперссылкаОткрытьНеобработанныеНастройкиКорреспонденций()
	
	Возврат "ОткрытьСписокНеобработанныеНастройкиКорреспонденций";
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ГиперссылкаСкрытьОповещениеНастройкиСчетов()
	
	Возврат "СкрытьОповещениеЕстьНеобработанныеНастройкиСчетовИСубконто";
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ГиперссылкаСкрытьОповещениеНастройкиКорреспонденций()
	
	Возврат "СкрытьОповещениеЕстьНеобработанныеНастройкиКорреспонденций";
...
Процедура ЗаполнитьДеревоОбъектовФинансовогоУчета()
	...
	ОбщегоНазначения.ЗаполнитьКоллекциюЭлементовДереваДанныхФормы(
		...
		ПланыВидовХарактеристик.СтатьиАктивовПассивов.ДеревоОбъектовФинансовогоУчета(НастройкиУчета, Истина));
...
Процедура НастроитьЭлементыФормыПриИзмененииОбъектаУчета()
	...
	ЗаголовокОбъектНастройки = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ЗаголовокОбъектаПоОписаниюТипов(
		ТекущаяСтрокаДерева.ОписаниеТиповОбъектНастройки);
	...
	Элементы.СписокИсключенияМестоУчета.Видимость = ТекущаяСтрокаДерева.ОписаниеТиповДетальнойНастройки.Типы().Количество() > 0;
	Элементы.СписокИсключенияМестоУчета.ОграничениеТипа = ТекущаяСтрокаДерева.ОписаниеТиповДетальнойНастройки;
	Элементы.СписокИсключенияМестоУчета.Заголовок = ПланыВидовХарактеристик.СтатьиАктивовПассивов.ЗаголовокОбъектаПоОписаниюТипов(
		ТекущаяСтрокаДерева.ОписаниеТиповДетальнойНастройки);
	...
	ПланыВидовХарактеристик.СтатьиАктивовПассивов.ИнициализироватьКомпоновщикНастроекИсточникомДанных(
		КомпоновщикНастроек,
		ТекущаяСтрокаДерева.ИсточникДанных,,
