...
Процедура РассчитатьЗначениеПоказателя(СтруктураКлючевыхРеквизитов,ТекВидОтчета,ТекПотребительРасчета,ТекНазначениеРасчетов,ПериодРасчета) Экспорт
	...
	ОбъектРасчета.Вставить("ПериодОтчета",		ПериодРасчета);
	ОбъектРасчета.Вставить("СпособФормированияОтчета",Перечисления.СпособыФормированияОтчетов.ПоПравилуОбработки);
...
Функция ПолучитьСтруктуруРеквизитов(ВидОтчетаПроверка,ОрганизацияПроверка,СценарийПроверка,ПериодОтчетаПроверка,НазначениеРасчетовПроверка=Неопределено)
	...
	Иначе
		...
		ПараметрыНастроек = УправлениеОтчетамиУХ.ПараметрыНастроекОбработкиШаблонов(СтруктураПоиска);
		ПараметрыНастроек.ПериодОтчета = ПериодОтчетаПроверка;
		ЗначенияНастроек = УправлениеОтчетамиУХ.НастройкиОбработкиШаблонов(ПараметрыНастроек);
		
		СтруктураРеквизитов = Новый Структура;
		СтруктураРеквизитов.Вставить("ПравилоОбработки", ЗначенияНастроек.ПравилоОбработки);
		
		Если ЗначениеЗаполнено(ЗначенияНастроек.ВнешняяИнформационнаяБаза) Тогда
			СтруктураРеквизитов.Вставить("ВИБ", ЗначенияНастроек.ВнешняяИнформационнаяБаза);
		Иначе
			СтруктураРеквизитов.Вставить("ВИБ", ПересчетПоказателейУХ.ПолучитьВИБПоУмолчанию(ЗначенияНастроек.ПравилоОбработки));
		КонецЕсли;
...
Функция ЗаписатьИзмененыеПоказателиРасширенный()
	
	Отказ=Ложь;
	ТаблицаКЗаписи=ПолучитьТаблицуКЗаписи(Отказ);
	...
	ОбработкаРасчета.Регистратор = Регистратор; 
	ОбработкаРасчета.НеЗаписыватьПротокол=Истина; 
	ОбработкаРасчета.НезависимыеСуммы=НезависимыеСуммы;
	...
	ПараметрыРасчета = ОбработкаРасчета.ПараметрыРасчета(); // Получаем параметры расчета
	ПараметрыРасчета.АдресТаблицыИзмененныхПоказателей 		= ПоместитьВоВременноеХранилище(ТаблицаКЗаписи); // Адрес таблицы с измененными показателями
	ПараметрыРасчета.НомерТранзакции 						= 1; // При помещении в лог, необходимо указывать номер транзакции. Нужно для функционала Откатить/Отменить
 ...
 КонецФункции // ЗаписатьИзмененыеПоказателиРасширенный()  

Функция ПолучитьТаблицуКЗаписи(Отказ) 
	 
	 ТаблицаКЗаписи = УправлениеРасчетомПоказателей.ОписаниеТаблицыИзмененныхПоказателей();
	 ...
	 |	ТаблицаИзмененныхПоказателей.ПоказательОтчета КАК ПоказательОтчета,
	 ...
	 |	ТаблицаИзмененныхПоказателей.ТекстКомментария КАК ТекстКомментария,
	 ...
	 |	ПоказателиОтчетов.Владелец.СохранятьИсториюИзменений КАК СохранятьИсториюИзменений
	 |ИЗ
	 |	Справочник.ПоказателиОтчетов КАК ПоказателиОтчетов
	 |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ИзмененныеПоказатели КАК ИзмененныеПоказатели
	 |		ПО ПоказателиОтчетов.Ссылка = ИзмененныеПоказатели.ПоказательОтчета
	 |;
	 |
	 |////////////////////////////////////////////////////////////////////////////////
	 |ВЫБРАТЬ
	 ...
	 ЗапросДанные.УстановитьПараметр("ТаблицаИзмененныхПоказателей",ТаблицаИзмененныхПоказателей.Выгрузить());
	 ЗапросДанные.УстановитьПараметр("ВалютыОтчетности",ВалютыОтчетности.Выгрузить()); 
	 ЗапросДанные.УстановитьПараметр("ПравилоОбработки",ПравилоОбработки);
	 ЗапросДанные.УстановитьПараметр("ИспользуемаяИБ",ИспользуемаяИБ);
	 ЗапросДанные.УстановитьПараметр("СпособФормирования",?(ЗначениеЗаполнено(СпособФормированияОтчета),СпособФормированияОтчета,Перечисления.СпособыФормированияОтчетов.ПоПравилуОбработки));
	 
	РезультатЗапроса=ЗапросДанные.ВыполнитьПакет();
	
	ВидыОтчетов=РезультатЗапроса[4].Выбрать();
	
	Пока ВидыОтчетов.Следующий() Цикл
		
		Если НЕ ВидыОтчетов.СохранятьИсториюИзменений Тогда 
			
			Попытка
				
				СправочникОбъект=ВидыОтчетов.ВидОтчета.ПолучитьОбъект();
				СправочникОбъект.СохранятьИсториюИзменений=Истина;
				СправочникОбъект.Записать();
				
			Исключение
				
				ТекстСообщения=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Не удалось включить версионирование у вида отчета %1, необходимое для корректировки значений показателей'"),ВидыОтчетов.ВидОтчета);
	...
	ДанныеТаблицы=РезультатЗапроса[5].Выбрать();
				
	Пока ДанныеТаблицы.Следующий() Цикл
		
		Если ДанныеТаблицы.СинтетическоеЗначение И (НЕ ДанныеТаблицы.СинтетическийПоказатель) Тогда
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрока,ДанныеТаблицы);
		
	КонецЦикла;
	
	Возврат ТаблицаКЗаписи; 
	 	 
КонецФункции // ПолучитьТаблицуКЗаписи()	 
...
Функция ЗаписатьЗначенияТаблицыПоказателей() Экспорт
	...
	Если ИдентификацияПродуктаУХКлиентСервер.ЭтоУправлениеХолдингом() Тогда
		...
		Возврат ЗаписатьИзмененыеПоказателиРасширенный();
	...
	Если ПустаяСтрока(ТекстОшибки) Тогда
		...
		Если ЗаблокированныеПоказатели.Количество()>0 Тогда
			...
			ШаблонСообщения = НСтр("ru = 'Организация: %1, сценарий: %2 %3,
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				Строка(Строка.Сценарий),
				Строка(ПредставлениеПроекта),
				Строка(Строка.ПериодОтчета),
				Строка(ВидОтчета),
				Строка(Строка.Показатель));
	...
	Для каждого Строка ИЗ ТабКлючевыхИзмерений Цикл
		...
		Для каждого Колонка Из ТабКлючевыхИзмерений.Колонки  Цикл
			...
			ТекстОтбор=ТекстОтбор+ТекстИ+Колонка.Имя+"=&"+Колонка.Имя;
		...
		ЗапросДанные.Текст=ЗапросДанные.Текст+"
		|ГДЕ "+Сред(ТекстОтбор, СтрДлина(ТекстИ))+"
		|;
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Для Каждого Строка ИЗ ТаблицаКЗаписи Цикл
		...
		Если ЗначениеЗаполнено(Строка.Проект) Тогда
			...
			Справочники.Проекты.РассчитатьПоказателиЭффективности(Строка.Сценарий,Строка.Проект);
	...
	Для Каждого СтрокаДанные ИЗ ТаблицаДанных Цикл
		...
		Если ЗначениеЗаполнено(СтрокаДанные.Значение)
			...
			Если Показатель.Валютный И Показатель.ПересчитыватьВалютнуюСумму Тогда
				...
				РассчитатьСтрокуВалютногоПоказателя(СтруктураКлючевыхРеквизитов,СтрокаДанные,Показатель,Ложь);
...
Процедура РассчитатьСтрокуВалютногоПоказателя(СтруктураКлючевыхРеквизитов,СтрокаДанные,Показатель,РасчитыватьНулевые=Истина)
	...
	Если ЗначениеЗаполнено(СтрокаДанные.Значение) Тогда
		...
		Если РасчитыватьНулевые ИЛИ СтрокаДанные.ЗначениеВалюта <> 0 Тогда
	...
	ИначеЕсли ЗначениеЗаполнено(СтрокаДанные.ЗначениеВалюта) Тогда
		...
		Если РасчитыватьНулевые ИЛИ СтрокаДанные.Значение <> 0 Тогда
...
Процедура РассчитатьЗначенияПоказателей(ВсегдаЗаписыватьЭкземпляр=Ложь,ИскатьСуществующийЭкземпляр=Истина) Экспорт
	...
	Для Каждого КлючевыеРеквизиты ИЗ ДанныеДляПересчетаПоказателей Цикл
		...
		АвтоматическийПересчет = Справочники.ВерсииРегламентовПодготовкиОтчетности.АвтоматическиПересчитыватьЗависимыеПоказатели(КлючевыеРеквизиты.ПериодОтчета,КлючевыеРеквизиты.Сценарий); 
		...
		Если НЕ ПолныеПраваУХ.ПроверитьВозможностьЗаписиДокументовПоПериодуСценария(КлючевыеРеквизиты,
										Отказ,
		...
		Режим20 = ОбщегоНазначенияПовтИспУХ.ИспользуетсяМногопериоднаяОтчетность();
		...
		Иначе
			...
			ПараметрыНастроек = УправлениеОтчетамиУХ.ПараметрыНастроекОбработкиШаблонов(КлючевыеРеквизиты);
			ПараметрыНастроек.ШаблонДокумента = ВидОтчета;
			СпособФормированияОтчета = УправлениеОтчетамиУХ.ЗначениеНастройкиОбработкиШаблонов(ПараметрыНастроек, "СпособФормированияОтчета");
			
			СпособФормированияОтчета=?(ЗначениеЗаполнено(СпособФормированияОтчета),СпособФормированияОтчета,Перечисления.СпособыФормированияОтчетов.ПоПравилуОбработки);
		...
		ДокументСсылка=Документы.НастраиваемыйОтчет.ПолучитьЭкземплярОтчета(КлючевыеРеквизиты.Организация,
											...
											Истина,ВсегдаЗаписыватьЭкземпляр ИЛИ (НЕ СпособФормированияОтчета=Перечисления.СпособыФормированияОтчетов.ПоПравилуОбработки),,КлючевыеРеквизиты.Проект);
		...
		Иначе
			...
			Иначе
				...
				ПараметрыНастроек = УправлениеОтчетамиУХ.ПараметрыНастроекОбработкиШаблонов(КлючевыеРеквизиты);
				ПараметрыНастроек.ШаблонДокумента = ТекВидОтчета;
				
				ДокументОбъект.ИспользуемаяИБ = УправлениеОтчетамиУХ.ЗначениеНастройкиОбработкиШаблонов(ПараметрыНастроек, "ВнешняяИнформационнаяБаза");
		...
		ДокументОбъект.ЗаполнитьОтчетПоУмолчанию(?(ЗначениеЗаполнено(СпособФормированияОтчета),СпособФормированияОтчета,Перечисления.СпособыФормированияОтчетов.ПоПравилуОбработки));
		...
		Если НЕ ОбъектРасчета.Свойство("ЧерноваяВерсия") Тогда
			...
			Если ВсегдаЗаписыватьЭкземпляр ИЛИ (НЕ (Режим20 ИЛИ СпособФормированияОтчета=Перечисления.СпособыФормированияОтчетов.ПоПравилуОбработки)) Тогда
...
Функция ПодготовитьСтруктуруПеременныхДляРасчета(СтруктураКлючевыхРеквизитов) Экспорт
	...
	СтруктураПеременных.Вставить("СпособФормированияОтчета",Перечисления.СпособыФормированияОтчетов.ПоПравилуОбработки);
	...
	СтруктураПеременных.Вставить("Регистратор",		Регистратор);
	СтруктураПеременных.Вставить("ЭкземплярОтчета",	Регистратор);
	Если ТипЗнч(Регистратор) <> Тип("ДокументСсылка.НастраиваемыйОтчет") Тогда
		
		ЭО = Документы.НастраиваемыйОтчет.ПолучитьЭкземплярОтчета(
				СтруктураКлючевыхРеквизитов.Организация,
				...
				Истина,
				Истина,
				"Создан из " + Регистратор,
				СтруктураКлючевыхРеквизитов.Проект);
		Если ЗначениеЗаполнено(ЭО) Тогда
			СтруктураПеременных.Вставить("ЭкземплярОтчета",	ЭО);
		КонецЕсли;									
...
// Сделать получение данной настройки из регламента
// ПодставлятьЗначенияПоУмолчаниюВместоПустых = Справочники.ВерсииРегламентовПодготовкиОтчетности.ПодставлятьЗначенияАналитикПоУмолчанию(ПериодОтчета, Сценарий); 
ПодставлятьЗначенияПоУмолчаниюВместоПустых = Ложь;
...
ФоновыйПересчетЗависимыхПоказателей=Ложь;
