...
Процедура ВыходныеИзделия_ИзменитьКачество(Команда)
	...
	ДобавитьВСтруктуруДействияПриИзмененииНоменклатуры(
			ОбъектЭтапПоСсылке(ЭтотОбъект, ТекущиеДанные.Ссылка),
			"ВыходныеИзделия",
			ЭтотОбъект,
			СтруктураДействий);
...
Процедура ВыходныеИзделия_ИзменитьКачествоОтменитьСтроку(Команда)
	...
	ДобавитьВСтруктуруДействияПриИзмененииНоменклатуры(
			ОбъектЭтапПоСсылке(ЭтотОбъект, ТекущаяСтрока.Ссылка),
			"ВыходныеИзделия", 
			ЭтотОбъект,
			СтруктураДействий);
...
Функция ПроверитьИЗаписать()
	...
	Если ПроверитьЗаполнение() Тогда
		...
		РезультатФоновойОперации = ДлительныеОперации.ВыполнитьВФоне(
			ИмяЭкспортнойПроцедуры,
			ПараметрыЗапуска,
			ПараметрыВыполнения);
		
		ИдентификаторЗадания = РезультатФоновойОперации.ИдентификаторЗадания;
		АдресХранилищаФоноваяОперация = РезультатФоновойОперации.АдресРезультата;
		
		РезультатВыполнения.ЗаданиеВыполнено = РезультатФоновойОперации.Статус = "Выполнено";
...
&НаКлиенте
Процедура ЗавершениеФоновойОперацииОповещение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура НачатьОжиданиеФоновойОперации()
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ЗавершениеФоновойОперацииОповещение", ЭтотОбъект);

	ДлительныеОперацииКлиент.ОжидатьЗавершение(РезультатФоновойОперации, ОповещениеОЗавершении, ПараметрыОжидания);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ДобавитьВСтруктуруДействияПриИзмененииНоменклатуры(ОбъектЭтап, ИмяТЧ, Форма, СтруктураДействий, ДополнительныеПараметры = Неопределено)
...
Функция ОбеспечениеУстановитьДействие(КодДействия)
	...
	Если КодДействия = "ФЛАГ_ОБОСОБЛЕННО" Или КодДействия = "ФЛАГ_НЕОБОСОБЛЕННО" Тогда
		...
		Для каждого Строка Из Изменения.ТаблицаИзменений Цикл
			ОбрабатываемыеСтроки.Добавить(Строка.Строка);
		КонецЦикла;
...
Процедура ПослеЗаполненияОбеспечения(Изменения, Режим = Неопределено, ПараметрыЗаполнения = Неопределено)
	...
	СтрокиКОбработке = Изменения.ВыгрузитьКолонку("Строка");
...
Процедура ЗаполнитьОбеспечениеПоСпецификацииНаСервере()
	...
	Для каждого Строка Из ДанныеСпецификации.МатериалыИУслуги Цикл
		...
		ОбъектЭтап = ОбъектЭтапПоСсылке(ЭтотОбъект, СоответствиеЭтапов[Строка.Этап]);
		...
		Если СтрокиКОбработке[ОбъектЭтап] = Неопределено Тогда
			СтрокиКОбработке.Вставить(ОбъектЭтап, Новый Массив);
		КонецЕсли;
		СтрокиКОбработке[ОбъектЭтап].Добавить(НоваяСтрока);
	...
	Для Каждого СтрокаКОбработке Из СтрокиКОбработке Цикл
		...
		СтруктураДействий.Вставить(
			"ЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства",
			СтрокаКОбработке.Ключ.ПараметрыЗаполненияНазначений);
			
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
			СтрокаКОбработке.Значение,
			СтруктураДействий,
			Объект.ОбеспечениеМатериаламиИРаботами,
			КэшированныеЗначения);
...
Процедура ЗаменитьНаАналоги(АдресВХранилище = Неопределено)
	...
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(ИмяТаблицы, ЭтотОбъект, СтруктураДействий);
	...
	Для каждого ИндексСтроки Из ИндексыНовыхСтрок Цикл
		СтрокаТЧ = Объект[ИмяТаблицы][ИндексСтроки]; //ДанныеФормыЭлементКоллекции
		СтрокаТЧ.НазначениеОбеспечения = СтрокаТЧ.Назначение;
		
		ОбъектЭтап = ОбъектЭтапПоСсылке(ЭтотОбъект, СтрокаТЧ.Ссылка);
		СтрокаТЧ.Подразделение = ОбъектЭтап.Подразделение;
		...
		Если СтрокиКОбработке[СтрокаТЧ.Ссылка] = Неопределено Тогда
			СтрокиКОбработке.Вставить(СтрокаТЧ.Ссылка, Новый Массив);
		КонецЕсли;
		СтрокиКОбработке[СтрокаТЧ.Ссылка].Добавить(СтрокаТЧ);
	...
	Для каждого КлючИЗначение Из СтрокиКОбработке Цикл
		...
		ДобавитьВСтруктуруДействияПриИзмененииНоменклатуры(
			ОбъектЭтапПоСсылке(ЭтотОбъект, КлючИЗначение.Ключ), ИмяТаблицы, ЭтотОбъект, СтруктураДействий, ДополнительныеПараметры);
...
Процедура ПолучитьСтрокиИзБуфераОбменаНаСервере(ИмяТЧ)
	...
	Если ПодборТоваров Тогда 
		ДобавитьВСтруктуруДействияПриИзмененииНоменклатуры(
			ОбъектЭтапПоСсылке(ЭтотОбъект, СсылкаПоУмолчанию), ИмяТЧ, ЭтотОбъект, СтруктураДействий);
		Если ИмяТЧ = "ПобочныеИзделия" Тогда
			ВнутренняяПереработкаКлиентСервер.ДобавитьДействиеЗаполнитьВладелецИзделияДоступен(ЭтотОбъект, СтруктураДействий);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаВыбораПодборНаСервере(ТаблицаТоваров, ИмяТЧ)
	...
	ОбъектЭтапПоУмолчанию = ОбъектЭтапПоСсылке(ЭтотОбъект, СсылкаПоУмолчанию);
	
	СтруктураДействий = Новый Структура;
	ДобавитьВСтруктуруДействияПриИзмененииНоменклатуры(
		ОбъектЭтапПоУмолчанию,
		ИмяТЧ,
		ЭтотОбъект,
		СтруктураДействий);
	...
	Для каждого Строка Из ТаблицаТоваров Цикл
		...
		Для каждого ТекущаяСтрока Из ДобавленныеСтроки Цикл
			ЗаполнитьСлужебныеРеквизитыПоДаннымЭтаповВСтроке(ТекущаяСтрока, ИмяТЧ, ОбъектЭтапПоУмолчанию);
			СтрокиКОбработке.Добавить(ТекущаяСтрока);
			ИдентификаторыСтрок.Добавить(ТекущаяСтрока.ПолучитьИдентификатор());
		КонецЦикла;
	КонецЦикла;
...
Процедура УстановитьУсловноеОформлениеЭтапы()
	...
	ПараметрыУсловногоОформления.ИмяПоляЕдиницаИзмерения = "ЭтапыОсновноеИзделиеНоменклатураЕдиницаИзмерения";
	ПараметрыУсловногоОформления.ПутьКПолюУпаковка = "Объект.Этапы.УпаковкаПлан";
