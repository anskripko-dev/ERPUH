...
Процедура ИспользуетсяЭДОБЗКПриИзменении(Элемент)
	...
	Иначе
		...
		|Действие необратимо.';
		|en = 'After the exchange is completed, the root certificate and user certificates will be deleted in 1C:Employee account.
...
Процедура ИзменитьДанныеСервиса(Команда)
	...
	ОткрытьФорму("Обработка.ИнтеграцияКабинетСотрудника.Форма.ИзменитьНастройкиСервиса",
		ПараметрыОткрытия,ЭтаФорма,,,,ОповещениеОЗакрытии,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ИзменитьАбонентаСервиса()
	...
	ОткрытьФорму("Обработка.ИнтеграцияКабинетСотрудника.Форма.ИзменитьАбонентаСервиса",
		ПараметрыОткрытия,ЭтаФорма,,,,ОповещениеОЗакрытии,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПриЗавершенииЗадания(Результат, ИмяЗадания) Экспорт
	...
	Если Результат = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Ошибка фонового задание:';
								|en = 'Background job error:'") + ИмяЗадания;
...
Процедура ОбновитьРезультатОбменаНаСервере()
	...
	Если Не ЗначениеЗаполнено(РезультатОбмена.ДатаНачала) Тогда
		Элементы.ДекорацияРезультатОбмена.Заголовок = НСтр("ru = 'Обмен с сервисом еще не выполнялся';
															|en = 'The exchange with the service was not performed yet'");
...
Процедура ЗаполнитьИнформациюОСервисеИзНастроек()
	...
	АдресСервисаАвторизации = СтруктураНастроек.АдресСервисаАвторизации;
	...
	РегламентныеЗаданияЗаблокированы = СтруктураНастроек.РегламентныеЗаданияЗаблокированы;
	...
	Иначе
		...
		МесяцПубликацииРасчетныхЛистков = НСтр("ru = 'Публикация не выполнялась';
												|en = 'Publishing was not performed'");
	КонецЕсли;
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ИзменитьАдресСервисаАвторизации", "Видимость", СервисКабинетСотрудникаВЛокальнойСети);
...
Процедура ОбновитьЭУОтложенноеПодключение(ОшибкаСоздания)
	...
	ТекстЗаголовка = НСтр("ru = 'Заявка на подключение к сервису отправлена.
		...
		|en = 'The application for connecting to the service has been sent.
		|This form can be closed, the connection will continue.
		|The connection will take some time (from several minutes to several hours). After completing the connection, an email will be sent to the email address of the responsible person.'");
...
Процедура ОбновитьДоступностьЭУИспользуемойФункциональности()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			...
			"Видимость",
			ИспользуетсяВерсия_3_1 И ИспользуетсяЭДОБЗК);
...
Процедура СохранитьНастройкуИспользоватьСборДанныхДляОформленияНаРаботу(ИспользоватьСборДанныхДляОформленияНаРаботу)
	...
	Исключение
		...
		СообщениеОбОшибке = НСтр("ru = 'Не удалось сохранить настройку. Подробности см. в журнале регистрации.';
								|en = 'Cannot save the setting. See the event log for details.'");
		ШаблонОписания = НСтр("ru = 'Не удалось сохранить настройки исподьзования сбора данных для оформления на работу.';
								|en = 'Cannot save the data collection usage settings for employment registration.'");
...
Процедура УстановитьДоступностьБлокировкаРегламентныхЗаданий()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			...
			"Видимость",
			РегламентныеЗаданияЗаблокированы);
...
Процедура ВключитьРегламентныеЗаданияНаСервере()
	...
	РегламентныеЗаданияЗаблокированы = Ложь;
	УстановитьДоступностьБлокировкаРегламентныхЗаданий();
