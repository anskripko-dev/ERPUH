...
Процедура ЗаполнитьПоОбъектуБД()
	...
	Для Каждого Строка ИЗ ПравилоЗаполнения.ИсточникиДляЗаполненияТЧ Цикл
		...
		Если Строка.Используется И ЗначениеЗаполнено(Строка.ИсточникДанных) Тогда
...
Процедура ЗаполнитьПоПроизвольномуКоду(Отказ = Ложь)
	...
	Если ЗначениеЗаполнено(ИсходныйОбъектСсылка) Тогда
		...
		Исключение
			...
			ОбщегоНазначенияУХ.СообщитьОбОшибке(ТекстОшибки,ЕстьОшибки,Отказ,СтатусСообщения.Внимание);
	...
	Иначе
		...
		Исключение
			...
			|%3'"), ОбъектБД, ПравилоЗаполнения, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())), Отказ);
...
Процедура ОбработатьСтруктуруТаблицТекущаяИБ_Объект()
	...
	КэшСооветствияОбъектов=Новый Соответствие;
	
	Результат=Запрос.Выполнить().Выбрать();
	
	Пока Результат.Следующий() Цикл
		
		КэшСооветствияОбъектов.Вставить(Результат.ИсходныйОбъектСсылка,Результат.ОбработанныйОбъектСсылка);
		
	КонецЦикла; 
	
	ТабОбработанныхОбъектов=Новый ТаблицаЗначений;
	ТабОбработанныхОбъектов.Колонки.Добавить("ОбработанныйОбъектСсылка");
	ТабОбработанныхОбъектов.Колонки.Добавить("ИсходныйОбъектСсылка");
	ТабОбработанныхОбъектов.Колонки.Добавить("ЕстьОшибки",КэшируемыеПроцедурыУХ.ПолучитьОписаниеТиповБулево()); 
	ТабОбработанныхОбъектов.Колонки.Добавить("ТекстОшибки");
	ТабОбработанныхОбъектов.Колонки.Добавить("ОбработкаЗавершена",КэшируемыеПроцедурыУХ.ПолучитьОписаниеТиповБулево());
	
	ИмяДокумента=ОбъектБД.Наименование;
	СтруктураПоиска=Новый Структура("ИсходныйОбъектСсылка");
	
	Для Каждого СтрокаШапка ИЗ СтруктураТаблиц.Шапка Цикл
		
		СуществующийОбъект=КэшСооветствияОбъектов.Получить(СтрокаШапка.ИсходныйОбъектСсылка);
		...
		Иначе
			...
			ДокументОбъект=СуществующийОбъект.ПолучитьОбъект();
			
		КонецЕсли;
		
		Для Каждого КолонкаШапка ИЗ СтруктураТаблиц.Шапка.Колонки Цикл 
		...
		ЗаполнитьТЧДокумента(СтруктураПоиска,ДокументОбъект,Новый Массив); 
		...
		Если ПустаяСтрока(ТекстОшибки) Тогда
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрока,СтрокаШапка);
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ДокументОбъект.Ссылка) Тогда
				...
				ТекстСообщения=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Не удалось провести документ %1:
				|%2'"),ДокументОбъект.Ссылка,ТекстОшибки);
				
				НоваяСтрока.ОбработанныйОбъектСсылка=ДокументОбъект.Ссылка;
				
			Иначе
				
				ТекстСообщения=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Не удалось записать документ со следующими реквизитами:
				|%1'"),ТекстОшибки);
			...
			ОбщегоНазначенияУХ.СообщитьОбОшибке(ТекстСообщения,ЕстьОшибки);
	...
	ОтразитьОбработкуТаблицыОбъектов(ТабОбработанныхОбъектов);
	
КонецПроцедуры // ОбработатьСтруктуруТаблицТекущаяИБ_Объект()
	
Процедура ЗаполнитьТЧДокумента(СтруктураПоиска,ДокументОбъект,МассивСлужебных)
...
Процедура ОбработатьСтруктуруТаблицТекущаяИБ()
	...
	Если ПравилоЗаполнения.ПоОбъектуБД=1 Тогда
		...
		ОбработатьСтруктуруТаблицТекущаяИБ_Объект();
		
	Иначе
		
		ОбработатьСтруктуруТаблицТекущаяИБ_Произвольный();
		
	КонецЕсли;
	
КонецПроцедуры // ЗаполнитьПоОбъектуБД() 

Функция ПолучитьМассивРеквизитовДляПоиска()
	...
	Запрос.УстановитьПараметр("Ссылка",ПравилоЗаполнения);
	
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Имя");
		
КонецФункции // ПолучитьМассивРеквизитовДляПоиска()

Процедура ТипизироватьДанныеШапки()
	
	МетаданныеОбъекта=Метаданные.Документы[ОбъектБД.Наименование];
	
	ТаблицаШапки=Новый ТаблицаЗначений;
	
	Для Каждого Колонка ИЗ СтруктураТаблиц.Шапка.Колонки Цикл
		
		РеквизитОбъекта=МетаданныеОбъекта.Реквизиты.Найти(Колонка.Имя);
		
		Если Не РеквизитОбъекта=Неопределено Тогда
			
			НоваяКолонка=ТаблицаШапки.Колонки.Добавить(Колонка.Имя,РеквизитОбъекта.Тип);
	...
	Для Каждого Строка ИЗ СтруктураТаблиц.Шапка Цикл
	...
	СтруктураТаблиц.Вставить("Шапка",ТаблицаШапки);
		
КонецПроцедуры // ТипизироватьДанныеШапки()

Функция ПроверитьНаличиеКлючевыхПолей(РеквизитыДляПоиска) 
	...
	Для Каждого КлючИЗначение ИЗ СтруктураТаблиц Цикл
		...
		Для Каждого РеквизитПоиска ИЗ РеквизитыДляПоиска Цикл
			
			Если КлючИЗначение.Значение.Колонки.Найти(РеквизитПоиска)=Неопределено Тогда
				
				ТекстОшибкиТаблица=ТекстОшибкиТаблица+","+РеквизитПоиска;
				
			КонецЕсли;
			
		КонецЦикла;
		
		Если НЕ ПустаяСтрока(ТекстОшибкиТаблица) Тогда
			
			ТекстОшибки=ТекстОшибки+Символы.ПС
				+СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = '%1 - %2'"),КлючИЗначение.Ключ,Сред(ТекстОшибкиТаблица,2));
	...
	Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
		...
		ТекстСообщения=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Заполнение по правилу %1, не найдены поля, соответствующие реквизитам шапки для поиска, в таблицах:%2 '"),
			ПравилоЗаполнения,ТекстОшибки);
...
КонецФункции // ПроверитьНаличиеКлючевыхПолей() 

Процедура ОбработатьСтруктуруТаблицТекущаяИБ_Произвольный() 
	
	РеквизитыДляПоиска=ПолучитьМассивРеквизитовДляПоиска();
	
	Если НЕ ПроверитьНаличиеКлючевыхПолей(РеквизитыДляПоиска) Тогда
	...
	Для Каждого Реквизит ИЗ РеквизитыДляПоиска Цикл
	...
	Если Не ИспользуемаяИБ=Справочники.ВнешниеИнформационныеБазы.ТекущаяИБ Тогда
		
		ТипизироватьДанныеШапки();
	...
	ТекстЗапроса=СтрЗаменить(ТекстЗапроса,"&Документ","Документ."+ИмяДокумента);
	Запрос.Текст=ТекстЗапроса;
	Запрос.УстановитьПараметр("ТаблицаШапка",СтруктураТаблиц.Шапка);
	...
	Для Каждого СтрокаШапка ИЗ ДанныеШапки Цикл
		...
		Для Каждого КолонкаШапка ИЗ ДанныеШапки.Колонки Цикл 
		...
		ЗаполнитьЗначенияСвойств(СтруктураПоиска,СтрокаШапка);	
		ЗаполнитьТЧДокумента(СтруктураПоиска,ДокументОбъект,РеквизитыДляПоиска); 
...
КонецПроцедуры // ОбработатьСтруктуруТаблицТекущаяИБ_Произвольный() 
...
Процедура ОбработатьСтруктуруТаблицВнешняяИБ_Объект()	
	
	ДанныеОбъектов=Новый Структура; 
		
	Для Каждого КлючИЗначение ИЗ СтруктураТаблиц Цикл  	
		
		ДанныеДляПереноса=ИнтеграцияСВнешнимиСистемамиУХ.Экспорт_ПолучитьДанныеДляПереноса(КлючИЗначение.Значение);	
		ДанныеОбъектов.Вставить(КлючИЗначение.Ключ,ИнтеграцияСВнешнимиСистемамиУХ.Экспорт_ПолучитьТаблицуПоДаннымПереноса(ИспользуемаяИБ,ДанныеДляПереноса));
	...
	ТабОбработанныхОбъектов=Новый ТаблицаЗначений;
	ТабОбработанныхОбъектов.Колонки.Добавить("ОбработанныйОбъектСсылка",КэшируемыеПроцедурыУХ.ПолучитьОписаниеТиповСтроки(40));
	ТабОбработанныхОбъектов.Колонки.Добавить("ИсходныйОбъектСсылка",КэшируемыеПроцедурыУХ.ПолучитьОписаниеТиповСтроки(40));
	ТабОбработанныхОбъектов.Колонки.Добавить("ЕстьОшибки",КэшируемыеПроцедурыУХ.ПолучитьОписаниеТиповБулево()); 
	ТабОбработанныхОбъектов.Колонки.Добавить("ТекстОшибки");
	ТабОбработанныхОбъектов.Колонки.Добавить("ОбработкаЗавершена",КэшируемыеПроцедурыУХ.ПолучитьОписаниеТиповБулево());
	...
	Пока Результат.Следующий() Цикл
		...
		НоваяСтрока=ТабОбработанныхОбъектов.Добавить();
		НоваяСтрока.ИсходныйОбъектСсылка=XMLСтрока(Результат.ИсходныйОбъектСсылка);
		НоваяСтрока.ОбработанныйОбъектСсылка=Результат.ОбработанныйОбъектСсылка;
			
	КонецЦикла;
	
	ДанныеДляПереноса=Новый Структура;
	ДанныеДляПереноса.Вставить("ДанныеОбъектов",ДанныеОбъектов);
	ДанныеДляПереноса.Вставить("ТабОбработанныхОбъектов",ТабОбработанныхОбъектов);
	ДанныеДляПереноса.Вставить("ИмяОбъекта",ОбъектБД.Наименование);
	ДанныеДляПереноса.Вставить("ПроводитьДокументы",ПроводитьДокументы);
	ДанныеДляПереноса.Вставить("ДатаОбработки",ПериодОтчета.ДатаОкончания);
	
	Если ПравилоЗаполнения.ПроизвольныйКодПослеЗаполнения И ЗначениеЗаполнено(ПравилоЗаполнения.ПроцедураЗаполнения) Тогда
		
		ДанныеДляПереноса.Вставить("ПроизвольныйКод",ПравилоЗаполнения.ПроцедураЗаполнения);
		
	КонецЕсли; 
	
	РезультатОбработки=СоздатьОбновитьОбъектыВнешнейИБ(ДанныеДляПереноса);
	
	КэшСсылок=Новый Соответствие;
	
	Для Каждого Строка ИЗ СтруктураТаблиц.Шапка Цикл
		
		КэшСсылок.Вставить(XMLСтрока(Строка.ИсходныйОбъектСсылка),Строка.ИсходныйОбъектСсылка);
	...
	Для Каждого Строка ИЗ РезультатОбработки Цикл
		...
		Если ЗначениеЗаполнено(Строка.ТекстОшибки) Тогда
			...
			ТекстСообщения=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Не удалось записать документ внешней ИБ %1 
			|на основании документа текущей ИБ %2: 
			|%3'"),ДанныеДляПереноса.ИмяОбъекта,КэшСсылок.Получить(Строка.ИсходныйОбъектСсылка),Строка.ТекстОшибки); 
	...
	ОтразитьОбработкуТаблицыОбъектов(РезультатОбработки,КэшСсылок);	
	
КонецПроцедуры // ОбработатьСтруктуруТаблицВнешняяИБ_Объект()

Функция СоздатьОбновитьОбъектыВнешнейИБ(ДанныеДляПереноса)

	Если ТипЗнч(ТекСоединениеВИБ) = Тип("WSProxy") Тогда 
				
		Возврат ТекСоединениеВИБ.CreateFillObjects(Новый ХранилищеЗначения(ДанныеДляПереноса, Новый СжатиеДанных(9))).Получить();
		
	ИначеЕсли ТипЗнч(ТекСоединениеВИБ) = Тип("Строка") Тогда
		
		Возврат РаботаСОбъектамиМетаданныхВнешнийУХ.ПолучитьОтвет1СШина(ТекСоединениеВИБ,"CreateFillObjects",ДанныеДляПереноса);
		
	Иначе
		
		Возврат РаботаСОбъектамиМетаданныхВнешнийУХ.CreateFillObjects(ТекСоединениеВИБ,ДанныеДляПереноса);
		
	КонецЕсли;
	
	
КонецФункции //  СоздатьОбновитьОбъектыВнешнейИБ() 

Процедура ОбработатьСтруктуруТаблицВнешняяИБ_Произвольный() 
	...
	ДанныеДляПереноса.Вставить("ИмяОбъекта",ОбъектБД.Наименование);
	ДанныеДляПереноса.Вставить("ПроводитьДокументы",ПроводитьДокументы);
	ДанныеДляПереноса.Вставить("РеквизитыДляПоиска",РеквизитыДляПоиска);
	...
	Для Каждого Строка ИЗ РезультатОбработки Цикл
		...
		Если ЗначениеЗаполнено(Строка.ТекстОшибки) Тогда
			...
			ТекстСообщения=СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = 'Не удалось записать документ внешней ИБ %1 : 
			|%2'"),ДанныеДляПереноса.ИмяОбъекта,Строка.ТекстОшибки); 
	...
	ОтразитьОбработкуТаблицыОбъектов(РезультатОбработки);	
	
КонецПроцедуры // ОбработатьСтруктуруТаблицВнешняяИБ_Произвольный() 
...
Процедура ОбработатьСтруктуруТаблицВнешняяИБ()
	...
	Если ПравилоЗаполнения.ПоОбъектуБД=1 Тогда
		...
		ОбработатьСтруктуруТаблицВнешняяИБ_Объект();
		
	Иначе
		
		ОбработатьСтруктуруТаблицВнешняяИБ_Произвольный();
...
Процедура ЗаполнитьОбъектПоИсточнику(Отказ=Ложь) Экспорт
	...
	Для Каждого Строка ИЗ ПравилоЗаполнения.ИсточникДляЗаполненияШапки.ПравилаИспользованияПолейЗапроса Цикл
		...
		Если Строка.СпособЗаполнения=Перечисления.СпособыЗаполненияПолейИсточника.ПолеИсходнойТаблицы Тогда
			...
			|ДанныеШапки."+Строка.Поле+" КАК "+Строка.КодАналитики;
	...
	Если НЕ ПустаяСтрока(ТекстПоляШапки) Тогда
		...
		Если ЗначениеЗаполнено(ПравилоЗаполнения.ИсточникДляЗаполненияШапки.ТабличнаяЧастьБД) Тогда
			
			Запрос.Текст="ВЫБРАТЬ ПЕРВЫЕ 1 "+Сред(ТекстПоляШапки,2)+"
			|ИЗ Документ."+ИсходныйОбъектСсылка.Метаданные().Имя+"."+ПравилоЗаполнения.ИсточникДляЗаполненияШапки.ТабличнаяЧастьБД.Имя+" КАК ДанныеШапки
			|Где ДанныеШапки.Ссылка=&Ссылка";
			
		Иначе	
			
			Запрос.Текст="ВЫБРАТЬ "+Сред(ТекстПоляШапки,2)+"
			|ИЗ Документ."+ИсходныйОбъектСсылка.Метаданные().Имя+" КАК ДанныеШапки
			|Где ДанныеШапки.Ссылка=&Ссылка";
	...
	Исключение
		...
		Если НЕ ЗначениеЗаполнено(ДокументОбъект.Ссылка) Тогда
			...
			ТекстОшибки=СтрШаблон(Нстр("ru = 'Не удалось создать на основании объекта %1 объект вида %2 по правилу %3:
			|%4'"), ИсходныйОбъектСсылка, ОбъектБД, ПравилоЗаполнения, 
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			
		Иначе
			
			ТекстОшибки=СтрШаблон(Нстр("ru = 'Не удалось на основании объекта %1 обновить объект %2 по правилу %3:
			|%4'"), ИсходныйОбъектСсылка, ДокументОбъект, ПравилоЗаполнения, 
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ОбработатьОбъектыБД(Отказ = Ложь) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПравилоЗаполнения) Тогда
		...
		|Обработка данных не выполнена.'"), ОбъектБД, Организация),Отказ,,СтатусСообщения.Внимание);
	...
	Если ПравилоЗаполнения.ПроизвольныйКод Тогда
		...
		ЗаполнитьПоПроизвольномуКоду(Отказ);
		ЗавершитьПротоколОбмена();
		Возврат;
		
	ИначеЕсли ЗначениеЗаполнено(ИсходныйОбъектСсылка) Тогда
		
		Если ОбластьГенерации=Перечисления.ОбластиГенерацииОбъектов.ТекущаяИБ Тогда 
			
			ЗаполнитьОбъектПоИсточнику(Отказ);
			ЗавершитьПротоколОбмена();
			Возврат;
			
		Иначе
			
			ВыгрузитьОбъектВоВнешнююИБ(Отказ);
			Возврат;
	...
	Если НЕ ЗначениеЗаполнено(ПравилоЗаполнения.ИсточникДляЗаполненияШапки) Тогда
		...
		ОбщегоНазначенияУХ.СообщитьОбОшибке(СтрШаблон(Нстр("ru = 'В правиле импорта %1 не указан источник для заполнения шапки.'"), 
			ПравилоЗаполнения),Отказ,,СтатусСообщения.Внимание);
		ЗавершитьПротоколОбмена();
		Возврат;
...
Процедура ВыгрузитьОбъектВоВнешнююИБ(Отказ)
	...
	Для Каждого Строка ИЗ ПравилоЗаполнения.ИсточникДляЗаполненияШапки.ПравилаИспользованияПолейЗапроса Цикл
		...
		Если Строка.СпособЗаполнения=Перечисления.СпособыЗаполненияПолейИсточника.ПолеИсходнойТаблицы
			иЛИ СтрНайти(Строка.КодАналитики,"ИсходныйОбъект")>0 Тогда
		...
		ИначеЕсли Строка.СпособЗаполнения=Перечисления.СпособыЗаполненияПолейИсточника.ФиксированноеЗначение Тогда
			...
			ТекстПоляШапки=ТекстПоляШапки+",
			|&"+Строка.КодАналитики+" КАК "+Строка.КодАналитики;
			
			Запрос.УстановитьПараметр(Строка.КодАналитики,Строка.ФиксированноеЗначение);
	...
	Если НЕ ПустаяСтрока(ТекстПоляШапки) Тогда
		...
		СтруктураТаблиц.Вставить("Шапка",Запрос.Выполнить().Выгрузить());
	...
	Для Каждого Строка ИЗ ПравилоЗаполнения.ИсточникиДляЗаполненияТЧ Цикл
		...
		Если ЗначениеЗаполнено(Строка.ИсточникДанных) Тогда
			...
			ИмяТЧ=Строка.ТабличнаяЧасть.Имя;
			
			СтруктураТаблиц.Вставить(ИмяТЧ,ПолучитьТаблицуТЧДляВыгрузки(ИмяТЧ,Строка.ИсточникДанных));
		КонецЕсли;
...
КонецПроцедуры // ВыгрузитьОбъектВоВнешнююИБ() 

Функция ПолучитьТаблицуТЧДляВыгрузки(ИмяТЧ,ИсточникТЧ)
	
	Запрос=Новый Запрос;
	ТекстПоляТЧ="";
	
	Для Каждого Строка ИЗ ИсточникТЧ.ПравилаИспользованияПолейЗапроса Цикл
		
		Если Строка.СпособЗаполнения=Перечисления.СпособыЗаполненияПолейИсточника.ПолеИсходнойТаблицы
			ИЛИ СтрНайти(Строка.КодАналитики,"ИсходныйОбъект")>0 Тогда
			
			ТекстПоляТЧ=ТекстПоляТЧ+",
			|ДокументТЧ."+Строка.Поле+" КАК "+Строка.КодАналитики;
	...
	Если НЕ ПустаяСтрока(ТекстПоляТЧ) Тогда
		...
		Запрос.Текст="ВЫБРАТЬ "+Сред(ТекстПоляТЧ,2)+"
		|ИЗ Документ."+ИсходныйОбъектСсылка.Метаданные().Имя+"."+ИмяТЧ+" КАК ДокументТЧ
		|Где ДокументТЧ.Ссылка=&Ссылка";
...
КонецФункции // ПолучитьТаблицуТЧДляВыгрузки() 
...
Процедура ОтразитьОбработкуОбъектаПоСсылке(Ссылка, ЕстьОшибки)
	...
	ЭлементБлокировки.УстановитьЗначение("ОбработанныйОбъектСсылка",Ссылка);
...
Процедура ОтразитьОбработкуТаблицыОбъектов(ТаблицаОбъектов,КэшСсылок=Неопределено)
	...
	Для Каждого Объект ИЗ ТаблицаОбъектов Цикл
		...
		СсылкаПриемник=Объект.ОбработанныйОбъектСсылка;
		
		Если ЗначениеЗаполнено(Объект.ИсходныйОбъектСсылка) Тогда
			
			Если КэшСсылок=Неопределено Тогда
				
				СсылкаИсточник=Объект.ИсходныйОбъектСсылка;
				
			Иначе
				
				СсылкаИсточник=КэшСсылок.Получить(Объект.ИсходныйОбъектСсылка);
		...
		Если ЗначениеЗаполнено(СсылкаПриемник) Тогда
			НаборЗаписей.Отбор.ОбработанныйОбъектСсылка.Установить(СсылкаПриемник);
		ИначеЕсли ЗначениеЗаполнено(СсылкаИсточник) Тогда			
			НаборЗаписей.Отбор.ИсходныйОбъектСсылка.Установить(СсылкаИсточник);
		КонецЕсли;
		...
		НоваяСтрока.ИсходныйОбъектСсылка=СсылкаИсточник;
		НоваяСтрока.ОбработанныйОбъектСсылка=СсылкаПриемник;
