...
Функция ОтразитьФактическоеДвижениеПоБюджетамПоПредопределенномуПравилу(ИсточникВход)
	...
	Для Каждого ТекМассивКСозданию Из МассивКСозданию Цикл
		...
		ИмяДокументаОФД="ОтражениеФактическихДанныхБюджетирования";
		Документы[ИмяДокументаОФД].ОтразитьФактическоеДвижениеПоБюджетамПоПредопределенномуПравилу(ДокументОбъект, ОтказИзм);
	КонецЦикла;	
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ДатаОбработки=ТекущаяДата();
	Объект.ПериодОтчета=ОбщегоНазначенияУХ.глОтносительныйПериодПоДате(Объект.ДатаОбработки,Константы.ПериодичностьБюджетирования.Получить(),0);
...
Процедура ЗаполнитьСписокНаСервере()
	...
	МодульДвиженияБюджетирования=ОбщегоНазначения.ОбщийМодуль("ДвиженияБюджетированиеУХ");
	
	Если МодульДвиженияБюджетирования=Неопределено Тогда
	...
	Если Объект.ОбъектБД = ДокументБдОтражениеФактическихДанных Тогда
		СписокПредопределенныхОбъектов = МодульДвиженияБюджетирования.ОбъектыСПредопределеннымиПравиламиГенерации(Ложь);		
...
Процедура ОбработанныеОбъектыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя="ОбработанныеОбъектыОбработанныйОбъектСсылка" Тогда
		...
		Если ТипЗнч(Запись.ОбработанныйОбъектСсылка)=Тип("Строка") Тогда
			...
			ДанныеОбъекта=ПолучитьДанныеОбработанногоОбъектаВИБ(Запись.ОбработанныйОбъектСсылка,Запись);
...
&НаСервере
Функция ПолучитьДанныеОбработанногоОбъектаВИБ(СтрокаGUID,Запись)
	
	Запрос=Новый Запрос;
	Запрос.Текст="ВЫБРАТЬ
	             |	ПравилаЗаполненияОбъектовБД.Владелец.Наименование КАК Наименование
	             |ИЗ
	             |	Справочник.ПравилаЗаполненияОбъектовБД КАК ПравилаЗаполненияОбъектовБД
	             |ГДЕ
	             |	ПравилаЗаполненияОбъектовБД.Ссылка = &Ссылка";
...
&НаКлиенте
Процедура ТаблицаПравилНастройкиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	Если Поле.Имя="ТаблицаПравилНастройкиПравилоЗаполнения" Тогда
		
		ПоказатьЗначение(,Элементы.ТаблицаПравилНастройки.ТекущиеДанные.ПравилоЗаполнения);
