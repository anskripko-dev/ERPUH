...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ОписаниеОбмена.Свойство("ИмяФункции")
		...
		ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
			...
			СтрШаблон(Нстр("ru = 'Не распознана команда %1 для обмена с ЕИС.'"), ОписаниеОбмена.КомандаОбмена));
		Возврат;
	КонецЕсли;
	Если НЕ ПроверитьЗаполненаОрганизацияНаСервере(ОписаниеОбмена) Тогда
		ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
			ОписаниеОбмена.ПоляОписанияТранзакции,
			СтрШаблон(Нстр("ru = 'Для команды %1 требуется указание организации.'"), ОписаниеОбмена.КомандаОбмена));
		Возврат;
	КонецЕсли;
	Попытка
		ОбработатьКомандуОбмена();
		// В процедуре ПриОткрытии() отправим сообщение.
	Исключение
...
#Область ПолучениеОписанияОтчетов
...
Функция ПолучитьТекстОписанияДоговораЗакупки(ОписаниеОбмена, ЕИСФабрикаXDTO)
	...
	Попытка
		...
		ДанныеДоговора.subjectContract = 
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(Лот.ПредметДоговора);
		ДанныеДоговора.purchaseTypeInfo = ЕИСФабрикаXDTO.Создать(
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(T_contractData, "purchaseTypeInfo"));
		ДанныеДоговора.purchaseTypeInfo.code = РеквизитыЭлектроннойЗакупки.КодСпособаЗакупки;
		ДанныеДоговора.purchaseTypeInfo.name = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(
		...
		Если НЕ ПустаяСтрока(Договор.КомментарийДляЕИС) Тогда
			ДанныеДоговора.additionalInfo =
				ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(Договор.КомментарийДляЕИС);
		КонецЕсли;
	...
	Для Каждого СтрокаОснования Из ТЗОснования Цикл
		...
		Если ЗначениеЗаполнено(СтрокаОснования.Комментарий) Тогда
			ОписаниеДокументаОснования.additionalInfo = 
				ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(СтрокаОснования.Комментарий);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(СтрокаОснования.Номер) Тогда
			ОписаниеДокументаОснования.docNum =
				ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(СтрокаОснования.Номер);
		КонецЕсли;
...
Функция ПолучитьИнформациюОПоставщикеПоДоговору(ЭТП,
	...
	ОписаниеКонтрагента.name = 
		ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(Контрагент.Наименование);
	ОписаниеКонтрагента.shortName = 
		ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(Контрагент.Наименование);
	Если ЗначениеЗаполнено(Контрагент.НаименованиеПолное) Тогда
		ОписаниеКонтрагента.brandName = 
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(Контрагент.НаименованиеПолное);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Контрагент.КодОКОПФ) Тогда
		...
		Если ЗначениеЗаполнено(Контрагент.НаименованиеОКОПФ) Тогда
			ОписаниеКонтрагента.okopfName = 
				ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(Контрагент.НаименованиеОКОПФ);
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьПозицииДоговора(ЭТП,
	...
	Для Каждого СтрокаНоменклатуры Из ТЗНоменклатуры Цикл
		...
		ОписаниеПозиции.name =
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(СтрокаНоменклатуры.НоменклатураНаименование);
...
Функция ЗаполнитьПозицииСведенийОбИсполнении(ЭТП,
											...
											ПоляОписанияТранзакции)
	ТекстОшибки = "";
	ТЗИсполнения = ИнтеграцияЦУЗсЭТПСерверУХ.ПолучитьОписаниеПозицийИсполненияДоговора(
						ИсполнениеДоговора, ТекстОшибки);
	Если (ТЗИсполнения = Неопределено) ИЛИ (ТЗИсполнения.Количество() = 0) Тогда
		ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
			ПоляОписанияТранзакции,
			СтрШаблон(
				Нстр("ru = 'Не удалось получить перечень документов исполнения договора. %1'"), 
				?(ПустаяСтрока(ТекстОшибки), 
					"", 
					СтрШаблон(Нстр("ru='Возникла ошибка: %1'", ТекстОшибки)) ) ) );
		Возврат Неопределено;
	КонецЕсли;
	...
	Для Каждого СтрокаИсполнения Из ТЗИсполнения Цикл
		...
		currency = ПолучитьОписаниеВалюты(ПолучитьКонвертерXDTO(),
										ЕИСФабрикаXDTO, 
										ПоляОписанияТранзакции,
										ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
											T_position, "currency"), 
...
Функция ПолучитьТекстОписанияЗакупки(ОписаниеОбмена, ЕИСФабрикаXDTO)
	...
	Если СпособВыбораПоставщика = Перечисления.СпособыВыбораПоставщика.КонкурентныеПереговоры
		ИЛИ СпособВыбораПоставщика = Перечисления.СпособыВыбораПоставщика.Редукцион Тогда
		ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
			ПоляОписанияТранзакции,
	...
	Если СпособВыбораПоставщика =
		...
		Возврат ПолучитьpurchaseNoticeOK(
...
Функция ПолучитьpurchaseNoticeОбщаяЧасть(ЭТП, 
	...
	Попытка
		...
		Если НЕ ЗначениеЗаполнено(ЗакупочнаяПроцедура.ОрганизацияДляЗаключенияДоговора)
			...
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
				ПоляОписанияТранзакции,
				Нстр("ru = 'В закупочной процедуре не заполнена организация заказчик. '"));
			Возврат Неопределено;
		КонецЕсли;
		...
		КодСпособаВыбора = ИнтеграцияЦУЗсЭТПСерверУХ.ПолучитьКодЗакупкиВЕИС(
			ЗакупочнаяПроцедура.СпособВыбораПоставщика, НаименованиеСпособаВыбора);
		Если ЗначениеЗаполнено(КодСпособаВыбора) Тогда
...
Функция ПолучитьplacingProcedure(ЕИСФабрикаXDTO, T_placingProcedure, ЗакупочнаяПроцедура, ПоляОписанияТранзакции)
		...
		placingProcedure.summingupDateTime = ЗакупочнаяПроцедура.ДатаПодведенияИтогов;
...
#Область ПолучениеОписанияПроектаИлиПланаЗакупки


&НаСервере
Функция ПолучитьТекстОписанияПроектаПланаЗакупки(ОписаниеОбмена, ЕИСФабрикаXDTO)
	Возврат ПолучитьТекстОписанияПроектаПланаЗакупкиИзКонвертера(
		ОписаниеОбмена, "prj:Пакет.purchasePlanProject");
	//ИменаТипов = Новый Структура;
	//ИменаТипов.Вставить("ПространствоИмен",
	//	"http://zakupki.gov.ru/223fz/purchasePlanProject/1");
	//ИменаТипов.Вставить("T_purchasePlan", "purchasePlanProject");
	//ИменаТипов.Вставить("T_purchasePlanData", "purchasePlanProjectData");
	//Возврат ПолучитьТекстОписанияПроектаИлиПланаЗакупки(
	//	ОписаниеОбмена,
	//	ЕИСФабрикаXDTO, 
	//	ИменаТипов);
КонецФункции
...
Функция ПолучитьТекстОписанияПланаЗакупки(ОписаниеОбмена, ЕИСФабрикаXDTO)
	Возврат ПолучитьТекстОписанияПроектаПланаЗакупкиИзКонвертера(
		ОписаниеОбмена, "p:Пакет.purchasePlan");
	//ИменаТипов = Новый Структура;
	//ИменаТипов.Вставить("ПространствоИмен",
	//	"http://zakupki.gov.ru/223fz/purchasePlan/1");
...
&НаСервере
Функция ПолучитьТекстОписанияПроектаПланаЗакупкиИзКонвертера(ОписаниеОбмена,
															ИмяТипаXSD)
	ПоляОписанияТранзакции = ОписаниеОбмена.ПоляОписанияТранзакции;
	ДопПараметры = Новый Структура;
	ДопПараметры.Вставить("packet_guid", ПоляОписанияТранзакции.КлючСообщения);
...
&НаСервере
Функция ПолучитьТекстОписанияПроектаИлиПланаЗакупки(ОписаниеОбмена,
													ЕИСФабрикаXDTO, 
													ИменаТипов)
	ЭТП = ОписаниеОбмена.ЭТП;
	ПоляОписанияТранзакции = ОписаниеОбмена.ПоляОписанияТранзакции;
	...
	Попытка
		...
		T_header = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
													T_purchasePlan, "header");
		T_body = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
													T_purchasePlan, "body");
		T_purchasePlanItem = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
													T_body, "item");
		T_purchasePlanData = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
													T_purchasePlanItem,
													ИменаТипов.T_purchasePlanData);
		// также используется для placer
		T_customer = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
													T_purchasePlanData, "customer");
		// Объект XDTO План закупки
		...
		ОписаниеПлана.body.item[ИменаТипов.T_purchasePlanData] =
			ЕИСФабрикаXDTO.Создать(T_purchasePlanData);
		// План закупки
		ДанныеПлана = ОписаниеПлана.body.item[ИменаТипов.T_purchasePlanData];
		// Определим УИД плана для ЕИС
		ОписаниеРазмещеннойВерсии = 
			ИнтеграцияЦУЗсЭТПСерверУХ.ПолучитьВерсиюОбъектаДляЕИС(ПланЗакупок, Истина);
		...
		Если ОписаниеВерсии.Размещена И ОписаниеВерсии.НомерВерсии > 0 Тогда
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
					ПоляОписанияТранзакции, Нстр("ru = 'Для отправки, создайте новую версию плана для ЕИС.'"));
			Возврат Неопределено;
		КонецЕсли;
		...
		Если мСтрокиПлана.Количество() = 0 Тогда
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
				ПоляОписанияТранзакции,
				Нстр("ru = 'У плана закупок нет строк. '"));
			Возврат Неопределено;
		КонецЕсли;
		Если флИнновационный Тогда
		...
		T_purchasePlanItems = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
												T_purchasePlanData, ИмяРеквизитаСтрокПлана);
		СтрокиПлана = ЕИСФабрикаXDTO.Создать(T_purchasePlanItems);
		ТипСтрокиПлана = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
											T_purchasePlanItems, ИмяРеквизитаСтрокиПлана);
		
		СтрокиПланаМСП = ЕИСФабрикаXDTO.Создать(
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ТипСвойстваИзТипаОбъекта(
						T_purchasePlanData, ИмяРеквизитаСтрокПланаМСП));
		...
		Для Каждого СтрокаПлана Из мСтрокиПлана Цикл
			...
			Если НЕ флИнновационный
				 И СтрокаПлана.ПриоритетУчастияМалогоИСреднегоПредпринимательства = Истина Тогда
				ДанныеСтроки = ПолучитьОписаниеСтрокиПланаЗакупок(
									ЭТП,
									ЕИСФабрикаXDTO,
									ТипСтрокиПлана,
		...
		Если ОписаниеРазмещеннойВерсии.Размещена Тогда
			Если НЕ ИнтеграцияЦУЗсЭТПСерверУХ.КорректныйНомерРегистрацииЕИС(ПланЗакупок) Тогда
				ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
					ПоляОписанияТранзакции,
					Нстр("ru = 'У плана закупок не заполнен (или заполнен некорректно) регистрационный номер в ЕИС. '"));
				Возврат Неопределено;
			КонецЕсли;
		...
		ДанныеПлана.name = ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(
			СтрШаблон(Нстр("ru = 'План закупки товаров (работ, услуг) ""%1"" на %2 г.'"), СокрЛП(ПланЗакупок.ОрганизацияДляЗаключенияДоговора), 
				ОтчетныйГод));
		//ДанныеПлана.additionalInfo = ""; // Дополнительные сведения
		ДанныеПлана.startDate = ПланЗакупок.ПериодЗакупок.ДатаНачала;
		ДанныеПлана.endDate = ПланЗакупок.ПериодЗакупок.ДатаОкончания; 
		...
		Если ОписаниеВерсии.НомерВерсии > 0 Тогда
			...
			Если НЕ ПустаяСтрока(ОписаниеВерсии.ОбоснованиеИзменений) Тогда
				ДанныеПлана.modificationDescription =
					ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(
						ОписаниеВерсии.ОбоснованиеИзменений);
			КонецЕсли;
		КонецЕсли;
		...
		КонецЕсли;
		ИтоговыеСуммыПланаЗакупок =
			ИнтеграцияЦУЗсЭТПСерверУХ.ПолучитьСуммыПланаЗакупок(ПланЗакупок);
		...
		ДанныеПлана.smbPartitionChanged =
			ИнтеграцияЦУЗсЭТПСерверУХ.ИзмененРазделСМППланаЗакупок(ПланЗакупок);
		ДанныеПлана.annualVolumeSMBLess18Percent =
			(ИтоговыеСуммыПланаЗакупок.ПроцентМСП < 18);
		...
		ТекстОписания = 
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПолучитьТекстОписанияОбъекта(
				ЕИСФабрикаXDTO, 
				ОписаниеПлана, 
				ИменаТипов.T_purchasePlan);
	Исключение
		Инфо = ИнформацияОбОшибке();
		ТекстОшибки = 
			СтрШаблон(Нстр("ru = 'Заполнение данных для отправки плана закупки закончилось ошибкой!%1%2'"), 
				Символы.ПС, ПодробноеПредставлениеОшибки(Инфо));
		ИнтеграцияЦУЗсЭТПСерверУХ.ЗарегистрироватьОшибку(
			ПоляОписанияТранзакции, 
			ТекстОшибки, 
...
&НаСервере
Функция ПолучитьОписаниеСтрокиПланаЗакупок(ЭТП,
								  ЕИСФабрикаXDTO, 
								  ТипСтрокиПлана, 
								  ПланЗакупок, 
								  СтрокаПлана, 
	...
	Если флЗакрытСОтменой Тогда
		...
		КонецЕсли;
	КонецЕсли;
	ЭтоДолгосрочнаяЗакупка = (Год(СтрокаПлана.ДатаОфициальногоОбъявленияЗакупки) < Год(СтрокаПлана.ДатаИсполненияДоговора));
	...
	// Special часть
	Если флЗакупкаУжеПередавалась Тогда
		ДанныеСтроки.parentId = parentGuid;
	КонецЕсли;
...
Функция ПолучитьpurchasePlanDataItemRows(ЭТП,
	...
	Для Каждого СтрокаНоменклатуры Из ТЗНоменклатуры Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаНоменклатуры.КодОКПД2) Тогда
			ТестОшибкиСтроки = НСтр("ru = 'В строке плана закупок №%НомерВГПЗ% у номенклатуры %НоменклатураНаименование% не указан ОКПД2! ""'");
			ТестОшибкиСтроки = СтрЗаменить(ТестОшибкиСтроки, "%НомерВГПЗ%", Строка(СтрокаПлана.НомерВГПЗ));
			ТестОшибкиСтроки = СтрЗаменить(ТестОшибкиСтроки, "%НоменклатураНаименование%", Строка(СтрокаНоменклатуры.НоменклатураНаименование));
			ТекстОшибки = ТекстОшибки + ТестОшибкиСтроки + Символы.ПС;
		Иначе
			ОписаниеПозиции.okpd2 = ЕИСФабрикаXDTO.Создать(T_okpd2);
			ОписаниеПозиции.okpd2.code = СокрЛП(СтрокаНоменклатуры.КодОКПД2);
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(СтрокаНоменклатуры.КодОКВЭД2) Тогда
			ТестОшибкиСтроки = НСтр("ru = 'В строке плана закупок №%НомерВГПЗ% у номенклатуры %НоменклатураНаименование% не указан ОКВЭД2! ""'");
			ТестОшибкиСтроки = СтрЗаменить(ТестОшибкиСтроки, "%НомерВГПЗ%", Строка(СтрокаПлана.НомерВГПЗ));
			ТестОшибкиСтроки = СтрЗаменить(ТестОшибкиСтроки, "%НоменклатураНаименование%", Строка(СтрокаНоменклатуры.НоменклатураНаименование));
			ТекстОшибки = ТекстОшибки + ТестОшибкиСтроки + Символы.ПС;
		Иначе
...
Функция ПолучитьinnovationPlanDataItemRows(
	...
	Для Каждого СтрокаНоменклатуры Из ТЗНоменклатуры Цикл
		...
		// ОКПД2
		Если НЕ ЗначениеЗаполнено(СтрокаНоменклатуры.КодОКПД2) Тогда
			ТекстОшибкиПлана = НСтр("ru = 'В лоте %СтрокаПлана% у номенклатуры %НоменклатураНаименование% не указан ОКПД2. '");
			ТекстОшибкиПлана = СтрЗаменить(ТекстОшибкиПлана, "%СтрокаПлана%", Строка(СтрокаПлана));
			ТекстОшибкиПлана = СтрЗаменить(ТекстОшибкиПлана, "%НоменклатураНаименование%", Строка(СтрокаНоменклатуры.НоменклатураНаименование));
			ТекстОшибки = ТекстОшибки + ТекстОшибкиПлана + Символы.ПС;
		Иначе
			ОписаниеПозиции.okpd2 = ЕИСФабрикаXDTO.Создать(T_okpd2);
			ОписаниеПозиции.okpd2.code = СокрЛП(СтрокаНоменклатуры.КодОКПД2);
		КонецЕсли;
		// ОКВЭД2
		Если НЕ ЗначениеЗаполнено(СтрокаНоменклатуры.КодОКВЭД2) Тогда
			ТекстОшибкиПлана = НСтр("ru = 'В лоте %СтрокаПлана% у номенклатуры %НоменклатураНаименование% не указан ОКВЭД2. '");
			ТекстОшибкиПлана = СтрЗаменить(ТекстОшибкиПлана, "%СтрокаПлана%", Строка(СтрокаПлана));
			ТекстОшибкиПлана = СтрЗаменить(ТекстОшибкиПлана, "%НоменклатураНаименование%", Строка(СтрокаНоменклатуры.НоменклатураНаименование));
			ТекстОшибки = ТекстОшибки + ТекстОшибкиПлана + Символы.ПС;
		Иначе
...
&НаСервере
Процедура ЗаполнитьВДанныхСтрокиПланаЗакупокСуммуИВалюту(
											ЕИСФабрикаXDTO,
											ПоляОписанияТранзакции,
											ТипСтрокиПлана, 
											СтрокаПлана, 
...
Функция ПолучитьОписаниеВалюты(КонвертерXDTO,
								ЕИСФабрикаXDTO, 
								ПоляОписанияТранзакции, 
								T_currency, 
								Валюта = Неопределено)
	Если ЗначениеЗаполнено(Валюта) Тогда
		ВалютаДляКонвертации = Валюта;
		...
		//	ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
		//		ПоляОписанияТранзакции,
		//		"Не у валюты """ + Валюта + """ не указан числовой код! ");
		//	Возврат Неопределено;
		//КонецЕсли;
	...
	Иначе
		ВалютаДляКонвертации = Новый Структура;
		ВалютаДляКонвертации.Вставить("Наименование", "RUB");
		ВалютаДляКонвертации.Вставить("Код", "643");
		ВалютаДляКонвертации.Вставить("НаименованиеПолное", Нстр("ru = 'Российский рубль'"));
		//ДанныеВалюты = ЕИСФабрикаXDTO.Создать(T_currency);
		//ДанныеВалюты.code = "RUB";
		//ДанныеВалюты.digitalCode = "643";
		//ДанныеВалюты.name = "Российский рубль";
	КонецЕсли;
	Возврат Обработки.КонвертерОбъектовXDTOУХ.РаспаковатьРезультат(
		ПреобразоватьВОбъектXDTO(
			КонвертерXDTO,
			ВалютаДляКонвертации,
...
Функция ПолучитьCustomerMainInfoType(ЕИСФабрикаXDTO,
	...
	Если ЗначениеЗаполнено(Организация.НаименованиеПолное) Тогда
		customerMainInfo.fullName =
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(Организация.НаименованиеПолное);
	КонецЕсли;
	customerMainInfo.shortName =
		ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(Организация.Наименование);
	...
	Если ЗначениеЗаполнено(КонтактнаяИнформация.ЮридическийАдрес) Тогда
		customerMainInfo.legalAddress =
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(КонтактнаяИнформация.ЮридическийАдрес);
	КонецЕсли;
	//почтовый адрес
	Если ЗначениеЗаполнено(КонтактнаяИнформация.ПочтовыйАдрес) Тогда
		customerMainInfo.postalAddress =
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(КонтактнаяИнформация.ПочтовыйАдрес);
	КонецЕсли;
	// телефон
	...
	Если ЗначениеЗаполнено(Организация.КодОКОПФ) Тогда
		customerMainInfo.okopf =
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(Организация.КодОКОПФ);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Организация.КодПоОКПО) Тогда
		customerMainInfo.okpo =
			ИнтеграцияЦУЗсЭТПКлиентСерверУХ.ПодготовитьWhiteSpaceCollapse(Организация.КодПоОКПО);
	КонецЕсли;
	Если ЗначениеЗаполнено(Организация.ДатаРегистрации) Тогда
		customerMainInfo.customerRegistrationDate = Организация.ДатаРегистрации;
	КонецЕсли;
...
Функция РасшифроватьОтветXDTO(ТекстСообщения, СтруктураОтвета)
	...
	ЕИСФабрикаXDTO = ПолучитьФабрикуXDTO();
...
Процедура ОбработатьКомандуОбмена()
	...
	ТекстОписанияОбъекта = ПолучитьТекстСообщенияОбмена(
		ОписаниеОбмена.ИмяФункции,
		ОписаниеОбмена);
	...
	ОписаниеОбмена.ПоляОписанияТранзакции.ОписаниеСобытия =
		СтрШаблон(Нстр("ru = '%1успешно.%2Записываем пакет в базу данных: '"), ОписаниеОбмена.ПоляОписанияТранзакции.ОписаниеСобытия, 
			Символы.ПС);
	...
	Если НЕ Результат Тогда
		ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
			ОписаниеОбмена.ПоляОписанияТранзакции,
			Нстр("ru = 'Не удалось записать сообщение обмена с укникальным ключом.'"));
		Возврат;
	КонецЕсли;
	ОписаниеОбмена.ПоляОписанияТранзакции.ОписаниеСобытия =
			СтрШаблон(Нстр("ru = '%1 пакет сообщения записан в базу с кодом: %2%3'"), ОписаниеОбмена.ПоляОписанияТранзакции.ОписаниеСобытия, 
				ОписаниеОбмена.ПоляОписанияТранзакции.КлючСообщения, Символы.ПС);
КонецПроцедуры

&НаСервере
Функция ПолучитьТекстСообщенияОбмена(ИмяФункции, ОписаниеОбмена)
	ИнициализироватьКонвертерXDTOНаСервере();
	ЕИСФабрикаXDTO = ПолучитьКонвертерXDTO().ПолучитьФабрикуXDTO();
	//ЕИСФабрикаXDTO = ПолучитьФабрикуXDTO();
	ТекстОписанияОбъекта = "";
	Если ИмяФункции = "ОтправитьПроцедуруЗакупкиНаЭТП" Тогда
		ПрефиксОшибки =
			СтрШаблон(Нстр("ru = 'Отправка закупки в ЕИС закончилась ошибкой!%1'"), Символы.ПС);
		ТекстОписанияОбъекта = ПолучитьТекстОписанияЗакупки(
									ОписаниеОбмена, ЕИСФабрикаXDTO);
	ИначеЕсли ИмяФункции = "ОтправитьПланЗакупокНаЭТП" Тогда
		ПрефиксОшибки =
			СтрШаблон(Нстр("ru = 'Отправка плана закупки в ЕИС закончилась ошибкой!%1'"), Символы.ПС);
		ТекстОписанияОбъекта = ПолучитьТекстОписанияПланаЗакупки(
					ОписаниеОбмена, ЕИСФабрикаXDTO);
	ИначеЕсли ИмяФункции = "ОтправитьПроектПланаЗакупки" Тогда
		ПрефиксОшибки =
			СтрШаблон(Нстр("ru = 'Отправка проекта плана закупки в ЕИС закончилась ошибкой!%1'"), Символы.ПС);
		ТекстОписанияОбъекта = ПолучитьТекстОписанияПроектаПланаЗакупки(
									ОписаниеОбмена, ЕИСФабрикаXDTO);
	ИначеЕсли ИмяФункции = "ПолучитьСтатусОбъекта" Тогда
		ПрефиксОшибки =
			СтрШаблон(Нстр("ru = 'Получение статуса объекта в ЕИС закончилась ошибкой!%1'"), Символы.ПС);
		ТекстОписанияОбъекта = ПолучитьТекстЗапросаНаВыгрузкуДанныхОтЕИС(
									ОписаниеОбмена, ЕИСФабрикаXDTO);
	...
	ИначеЕсли ИмяФункции = "ОтправитьПротоколВыбораПобедителей" Тогда
		ПрефиксОшибки =
			СтрШаблон(Нстр("ru = 'Отравка протокола выбора поставщиков в ЕИС закончилась ошибкой!%1'"), 
				Символы.ПС);
		ТекстОписанияОбъекта = ПолучитьТекстПротоколаВыбораПобедителя(
										ОписаниеОбмена, ЕИСФабрикаXDTO);
	ИначеЕсли ИмяФункции = "ОтменитьЗакупкуНаЭТП" Тогда
		ПрефиксОшибки =
			СтрШаблон(Нстр("ru = 'Отмена закупки в ЕИС закончилась ошибкой!%1'"), Символы.ПС);
		ТекстОписанияОбъекта = ПолучитьТекстЗапросаНаОтменуЗакупки(
										ОписаниеОбмена, ЕИСФабрикаXDTO);
	ИначеЕсли ИмяФункции = "ОтправитьДоговорНаЭТП" Тогда
		ПрефиксОшибки =
			СтрШаблон(Нстр("ru = 'Отправка договора закупки в ЕИС закончилась ошибкой!%1'"), Символы.ПС);
		ТекстОписанияОбъекта = ПолучитьТекстОписанияДоговораЗакупки(
										ОписаниеОбмена, ЕИСФабрикаXDTO);
	ИначеЕсли ИмяФункции = "ОтправитьИсполнениеДоговора" Тогда
		ПрефиксОшибки =
			СтрШаблон(Нстр("ru = 'Отправка данных об исполнении договора в ЕИС закончилась ошибкой!%1'"), 
				Символы.ПС);
		ТекстОписанияОбъекта = ПолучитьТекстОписанияИсполненияДоговора(
										ОписаниеОбмена, ЕИСФабрикаXDTO);
	ИначеЕсли ИмяФункции = "ОтправитьОбъемЗакупокУМCП" Тогда
		ПрефиксОшибки =
			СтрШаблон(Нстр("ru = 'Отправка годового отчета о закупках у МСП в ЕИС закончилась ошибкой!%1'"), 
				Символы.ПС);
		ТекстОписанияОбъекта = ПолучитьТекстОписанияГодовогоОтчетаОЗакупкеУМСП(
					ОписаниеОбмена, ЕИСФабрикаXDTO);
	ИначеЕсли ИмяФункции = "ОтправитьОтчетОЗаключенныхДоговорах" Тогда
		ПрефиксОшибки =
			СтрШаблон(Нстр("ru = 'Отправка отчета о заключенных договорах в ЕИС закончилась ошибкой!%1'"), 
				Символы.ПС);
		ТекстОписанияОбъекта = ПолучитьТекстОписанияОтчетаОЗаключенныхДоговорах(
					ОписаниеОбмена, ЕИСФабрикаXDTO);
	Иначе
		ОписаниеОбмена.флТранзакцияЗавершиласьУспешно = Ложь;
		ИнтеграцияЦУЗсЭТПКлиентСерверУХ.УстановитьЗначениеОшибки(
...
Функция ПолучитьФабрикуXDTO()
	ЕИСФабрикаXDTO = ПолучитьЗначениеВКонтексте("ФабрикаXDTO");
	Если ЕИСФабрикаXDTO = Неопределено Тогда
		ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
		ЕИСФабрикаXDTO = ОбработкаОбъект.ПолучитьФабрикуXDTO();
		ОбработкаОбъект = Неопределено;
		УстановитьЗначениеВКонтексте("ФабрикаXDTO", ЕИСФабрикаXDTO);
	КонецЕсли;
	Возврат ЕИСФабрикаXDTO;
КонецФункции
...
#Область РаботаСКонвертеромXDTO


&НаСервере
Процедура ИнициализироватьКонвертерXDTOНаСервере()
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ТекстПравил = 
		ОбработкаОбъект.ПолучитьПравилаКонвертацииВXDTO();
	ОбработкаОбъект = Неопределено;
	Если НЕ ПустаяСтрока(ТекстПравил) Тогда
		КонвертерXDTO = ПолучитьКонвертерXDTO();
		КонвертерXDTO.ПрочитатьИзТекста(ТекстПравил);
		ЗначениеВРеквизитФормы(КонвертерXDTO, "КонвертерXDTOНаФорме");
	КонецЕсли;
КонецПроцедуры
...
// см. Обработки.КонвертерОбъектовXDTOУХ.УпаковатьРезультат().
//
&НаСервереБезКонтекста
Функция ПреобразоватьВОбъектXDTO(КонвертерXDTO, Объект1С, ИмяТипа1С, ТипXDTO)
	Возврат КонвертерXDTO.ПреобразоватьПоУмолчаниюОбъект1СВОбъектXDTO(
		Объект1С, ИмяТипа1С, ТипXDTO);
КонецФункции
