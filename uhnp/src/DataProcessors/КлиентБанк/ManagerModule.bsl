...
Процедура ВывестиОшибкуЗагрузкиФайла(Счет, ИмяФайла, ИнформацияОбОшибке) Экспорт
	...
	ШаблонТекста = НСтр("ru = 'Ошибка загрузки файла ""%1"" по счету %2:
								|%3';
								|en = 'An error occurred when importing the ""%1"" file for the ""%2"" account:
								|%3'");
	...
	Событие = СтрШаблон("%1 %2", НСтр("ru = 'Загрузка платежей из банка.';
										|en = 'Import payments from the bank.'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ЗаписатьФайлыПлатежныхПорученийТОФК(Организация, ТаблицаДокументов, КаталогФайлов)
	...
	//++ НЕ УТ
	ДанныеРасшифровки = ДанныеИсполняемогоКонтракта(ТаблицаДокументов);
	//-- НЕ УТ
	...
	Если Исходящий Тогда
		...
		Если ДанныеДокумента.Операция = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыПоЗарплатномуПроекту
			...
			Если ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплаты") Тогда
				...
				ТаблицаВедомостей = ДанныеРасшифровки.ТаблицаВедомостей;
				Результат = ТаблицаВедомостей.Скопировать(ОтборПоДокументу, "ПериодРегистрации");
				Результат.Свернуть("ПериодРегистрации");
				
				Если Результат.Количество() > 1 Тогда
					
					ТекстОшибки = СтрШаблон(
						НСтр("ru = 'В одном файле выгрузки допустимо указывать один период выплаты зарплаты. В документе %1 указано несколько периодов выплаты';
							|en = 'В одном файле выгрузки допустимо указывать один период выплаты зарплаты. В документе %1 указано несколько периодов выплаты'"),
						ДанныеДокумента.Ссылка);
					ВызватьИсключение ТекстОшибки;
					
				ИначеЕсли Результат.Количество() = 1 Тогда
					ПериодВыплаты = Результат[0].ПериодРегистрации;
				Иначе
					
					ТекстОшибки = СтрШаблон(НСтр("ru = 'В документе %1 не заполнена ведомость в банк';
												|en = 'В документе %1 не заполнена ведомость в банк'"), ДанныеДокумента.Ссылка);
					ВызватьИсключение ТекстОшибки;
		...
		Для Каждого СтрокаРасшифровки Из РасшифровкаПлатежа Цикл
			...
			РеквизитыРасшифровки.ContractID = СтрокаРасшифровки.ИГК;
			РеквизитыРасшифровки.NumPP = СтрокаРасшифровки.НомерСтрокиОснования;
			РеквизитыРасшифровки.ViewC = "Государственный контракт";
			РеквизитыРасшифровки.ViewC_code = "5";
			
			Если ЗначениеЗаполнено(РеквизитыРасшифровки.DocDate) Тогда
				РеквизитыРасшифровки.DocDate = СтрокаРасшифровки.ДатаЗаключенияКонтракта;
			Иначе
				РеквизитыРасшифровки.Удалить("DocDate");
			КонецЕсли;
			
			Если ЗначениеЗаполнено(РеквизитыРасшифровки.DocNum) Тогда
				РеквизитыРасшифровки.DocNum = СтрокаРасшифровки.НомерКонтракта;
			Иначе
				РеквизитыРасшифровки.Удалить("DocNum");
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(СтрокаРасшифровки.СтатьяЦелевыхСредств) Тогда
				...
				Если НайденнаяСсылка = Неопределено Тогда
					...
					РеквизитыРасшифровки.CodeObjectPayer = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НайденнаяСсылка, "Код");
				Иначе
					ДанныеСтатьи = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НайденнаяСсылка, "УкрупненныйКодРасходов, Код");
					РеквизитыРасшифровки.CodeObjectPayer = ДанныеСтатьи.УкрупненныйКодРасходов;
					РеквизитыРасшифровки.DetCodeObjectPayer = ДанныеСтатьи.Код;
				КонецЕсли;
				
			Иначе
				РеквизитыРасшифровки.Удалить("CodeObjectPayer");
				РеквизитыРасшифровки.Удалить("DetCodeObjectPayer");
			КонецЕсли;
		...
		ДанныеДокумента.TranscriptPP_TransuseSign = ?(РасшифровкаПлатежа.Количество() > 1, "1", "0");
		
		Если ДанныеДокумента.TranscriptPP_TransuseSign = "1" Тогда
			ДанныеДокумента.TranscriptPP_DocNum = ДанныеДокумента.Номер;
			ДанныеДокумента.TranscriptPP_DocDate = ДанныеДокумента.Дата;
		Иначе
			ДанныеДокумента.Удалить("TranscriptPP_DocNum");
			ДанныеДокумента.Удалить("TranscriptPP_DocDate");
		КонецЕсли;
		
		ДанныеДокумента.RegistryDocsBaseAttechmentSix_RDOuseSign = "0";
		ДанныеДокумента.SumKBKTotal = РасшифровкаПлатежа.Итог("СуммаКОплате");
		ДанныеДокумента.SumNDSTotal = РасшифровкаПлатежа.Итог("СуммаНДС");
...
Функция ОписаниеРеквизитовРасшифровкиПлатежаТОФК()
	...
	РасшифровкаПлатежа.Вставить("ViewC_code");
...
Функция ДанныеИсполняемогоКонтракта(ТаблицаДокументов)
	...
	ЗапросСДополнениями.Текст =
		...
		|	СписаниеБезналичныхДенежныхСредствРасшифровкаПлатежа.НомерСтроки КАК НомерСтрокиОснования,
		|	СписаниеБезналичныхДенежныхСредствРасшифровкаПлатежа.Сумма КАК СуммаКОплате,
		|	СписаниеБезналичныхДенежныхСредствРасшифровкаПлатежа.СуммаНДС КАК СуммаНДС,
		...
		|	МИНИМУМ(ДанныеРасшифровки.НомерСтрокиОснования) КАК НомерСтрокиОснования,
		|	СУММА(ДанныеРасшифровки.СуммаКОплате) КАК СуммаКОплате,
		|	СУММА(ДанныеРасшифровки.СуммаНДС) КАК СуммаНДС,
		|	ДанныеРасшифровки.СтатьяЦелевыхСредств КАК СтатьяЦелевыхСредств,
		|	ДанныеРасшифровки.ИГК КАК ИГК,
		|	ДанныеРасшифровки.НомерКонтракта КАК НомерКонтракта,
		|	ДанныеРасшифровки.ДатаЗаключенияКонтракта КАК ДатаЗаключенияКонтракта,
		|	ДанныеРасшифровки.РазделЛицевогоСчета КАК РазделЛицевогоСчета
		|ИЗ
		|	ДанныеРасшифровки КАК ДанныеРасшифровки
		|
		|СГРУППИРОВАТЬ ПО
		|	ДанныеРасшифровки.СтатьяЦелевыхСредств,
		|	ДанныеРасшифровки.ИГК,
		|	ДанныеРасшифровки.РазделЛицевогоСчета,
		|	ДанныеРасшифровки.НомерКонтракта,
		|	ДанныеРасшифровки.ДатаЗаключенияКонтракта,
		...
		|ВЫБРАТЬ
		|	ДанныеРасшифровки.Ссылка КАК Ссылка,
		|	ДанныеРасшифровки.Ведомость КАК Ведомость,
		...
		|		ПО ДанныеРасшифровки.Ведомость = ВедомостьНаВыплатуЗарплатыВБанк.Ссылка
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ДанныеРасшифровки.Ссылка,
		|	ДанныеРасшифровки.Ведомость,
		...
		|		ПО ДанныеРасшифровки.Ведомость = ВедомостьПрочихДоходовВБанк.Ссылка";
	
	РезультатыЗапросов = ЗапросСДополнениями.ВыполнитьПакет();
	ТаблицаРасшифровки = РезультатыЗапросов[РезультатыЗапросов.ВГраница() - 1].Выгрузить();
	ТаблицаВедомостей = РезультатыЗапросов[РезультатыЗапросов.ВГраница()].Выгрузить();
	
	ОбщегоНазначенияУТ.УничтожитьВременныеТаблицы(МенеджерВременныхТаблиц, "ИсходнаяТаблица");
	
	Результат = Новый Структура;
	Результат.Вставить("ТаблицаРасшифровки", ТаблицаРасшифровки);
	Результат.Вставить("ТаблицаВедомостей", ТаблицаВедомостей);
		...
		Если СтрокаТаблицы.ВсегоПлатежей > 1
			...
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Существует несколько одинаковых платежей с номером %1 от %2 по счету контрагента %3';
					|en = 'There are several identical payments with number %1 dated %2 on the ""%3"" counterparty''s account'"),
