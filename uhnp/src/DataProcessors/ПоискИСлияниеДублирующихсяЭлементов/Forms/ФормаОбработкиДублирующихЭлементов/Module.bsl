...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СпособОбработки=1
		...
		Элементы.ДанныеДляСлиянияВыполнитьОбработку.Заголовок=Нстр("ru = 'Удалить дублирующие элементы'");
		Элементы.ОбработкаДубликатовПослеЗамены.Видимость = Истина;
		
	Иначе
		
		Элементы.ДанныеДляСлиянияВыполнитьОбработку.Заголовок=Нстр("ru = 'Выполнить синхронизацию'");
...
Функция ОбновитьДанныеИсходногоЭлемента()
	...
	Исключение
		...
		|: %2'"), ОбъектЭталон, ОписаниеОшибки()),,,СтатусСообщения.Внимание);
	...
	Если ОбщегоНазначенияУХ.ЕстьРеквизитСправочника(СтрЗаменить(ТаблицаАналитики,"Справочник.",""),"НСИ_ВИБ") Тогда
		...
		Для Каждого Элемент ИЗ СписокОбъектовДляОбработки Цикл
			...
			Исключение
				...
				ОбщегоНазначенияУХ.СообщитьОбОшибке(СтрШаблон(Нстр("ru = 'Не удалось записать объект %1
				|: %2'"), ОбъектДубль, ОписаниеОшибки()),,,СтатусСообщения.Внимание);
...
Процедура ОпределитьЭталонныйЭлементПоСсылкам()
	...
	Иначе
		...
		Сообщить(Нстр("ru = 'Ссылки на объекты не найдены.'"),СтатусСообщения.Информация);
