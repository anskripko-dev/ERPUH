...
Функция КоличествоКЗаказуДляПоддержанияЗапаса(Товар) Экспорт
	...
	Результат.КЗаказу = Результат.КЗаказу + Товар.КоличествоЦЗ;
	#КонецОбласти
...
Процедура СформироватьЗаказы(Параметры, АдресРезультата) Экспорт
	...
	#Область УХ_Внедрение
	ПоляКлючаЗаказа = "НаправлениеДеятельности, Партнер, Организация, Подразделение, Контрагент, Договор";
...
Функция ВременнаяТаблицаИзТаблицыКЗаказу()
	...
	ТекстЗамены = ТекстДляЗамены + "
		...
		|	Таблица.АналитикаИсточника		КАК АналитикаИсточника,
		|	Таблица.АналитикаПланирования	КАК АналитикаПланирования,";
...
Функция ВременнаяТаблицаДанныеЗаполнения()
	...
	ТекстЗамены = ТекстДляЗамены + "
		...
		|	Т.СпособОбеспечения							КАК СпособОбеспечения,
		|	Т.АналитикаИсточника						КАК АналитикаИсточника,
		|	Т.АналитикаПланирования						КАК АналитикаПланирования,";
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ТекстДляЗамены, ТекстЗамены);
	#КонецОбласти
	Возврат ТекстЗапроса;
...
Функция ТаблицаЗаказыПоставщикуКОформлению()
	...
	ТекстДляЗамены = "|	Т.НаправлениеДеятельности КАК НаправлениеДеятельности,";
	ТекстЗамены = ТекстДляЗамены + "
	|	Т.АналитикаИсточника						КАК АналитикаИсточника,
	|	Т.АналитикаПланирования						КАК АналитикаПланирования,"; 
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ТекстДляЗамены, ТекстЗамены);    
	
	ТекстДляЗамены = "|	Т.ИсточникОбеспечения,";
	ТекстЗамены = ТекстДляЗамены + "
	|	Т.АналитикаИсточника,
	|	Т.АналитикаПланирования,"; 
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ТекстДляЗамены, ТекстЗамены);
	#КонецОбласти
	Возврат ТекстЗапроса;
...
Функция ПолучитьДокумент(МенеджерДокументов, ДанныеЗаполнения, Статус)
	...
	Если ТИПЗНЧ(МенеджерДокументов) = ТИП("ДокументМенеджер.ЗаказПоставщику")
		И ДанныеЗаполнения.Свойство("Товары") И ДанныеЗаполнения.Свойство("Договор")
		И ЗначениеЗаполнено(ДанныеЗаполнения.Договор) Тогда
		
		
		ДокументОбъект.ЦенаВключаетНДС =  ДанныеЗаполнения.Договор.ВерсияСоглашения.ЦенаВключаетНДС;
		ДокументОбъект.Товары.Очистить();
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Товары",ДанныеЗаполнения.Товары);
		...
		Запрос.Текст ="ВЫБРАТЬ
		              ...
		              |	Т.МестоПоставки КАК МестоПоставки,
		              |	Т.Приоритет КАК Приоритет,
		              |	Т.НоменклатураИсходная КАК НоменклатураИсходная,
		              |	Т.Цена КАК Цена
		              |ПОМЕСТИТЬ Т
		              |ИЗ
		              |	&Товары КАК Т
		              ...
		              |	ПланированиеПотребностей.АналитикаПланирования.АналитикаСтатейБюджетовПоставки.СтатьяБюджета КАК СтатьяБюджетаБДР,
		              |	ПланированиеПотребностей.АналитикаПланирования.АналитикаСтатейБюджетовПоставки.Аналитика1 КАК АналитикаБДР1,
		              |	ПланированиеПотребностей.АналитикаПланирования.АналитикаСтатейБюджетовПоставки.Аналитика2 КАК АналитикаБДР2,
		              |	ПланированиеПотребностей.АналитикаПланирования.АналитикаСтатейБюджетовПоставки.Аналитика3 КАК АналитикаБДР3,
		              |	ПланированиеПотребностей.АналитикаПланирования.АналитикаСтатейБюджетовПоставки.Аналитика4 КАК АналитикаБДР4,
		              |	ПланированиеПотребностей.АналитикаПланирования.АналитикаСтатейБюджетовПоставки.Аналитика5 КАК АналитикаБДР5,
		              |	ПланированиеПотребностей.АналитикаПланирования.АналитикаСтатейБюджетовПоставки.Аналитика6 КАК АналитикаБДР6,
		              |	ПланированиеПотребностей.АналитикаПланирования.АналитикаСтруктуры.ЦФО КАК ЦФО,
		              |	ПланированиеПотребностей.АналитикаПланирования.АналитикаСтруктуры.Проект КАК Проект
		              |ИЗ
		              |	Т КАК Т
		              |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ПланированиеПотребностей.Остатки(, АналитикаПланирования.Этап = ЗНАЧЕНИЕ(Перечисление.ЭтапыПланированияПотребностей.Законтрактовано)) КАК ПланированиеПотребностей
		              |		ПО Т.Номенклатура = ПланированиеПотребностей.АналитикаПланирования.АналитикаНоменклатуры.Номенклатура
		              |			И Т.Характеристика = ПланированиеПотребностей.АналитикаПланирования.АналитикаНоменклатуры.Характеристика
		              |			И Т.Цена = ПланированиеПотребностей.АналитикаПланирования.Цена
		              |			И Т.МестоПоставки = ПланированиеПотребностей.АналитикаИсточника.АналитикаПотребностей.МестоПоставки
		              |			И (ПланированиеПотребностей.АналитикаПланирования.ОбъектПланирования.Объект = &Договор)
		              |			И Т.СтавкаНДС = ПланированиеПотребностей.АналитикаПланирования.АналитикаПоставки.СтавкаНДС
		              |			И Т.Приоритет = ПланированиеПотребностей.АналитикаПланирования.АналитикаПотребностей.Приоритет
		              |			И Т.ОрганизацияПолучатель = ПланированиеПотребностей.АналитикаПланирования.АналитикаСтруктуры.Организация";
		Запрос.УстановитьПараметр("ПериодичностьЦентрализованныхЗакупок",ЦентрализованныеЗакупкиУХ.ПолучитьПериодичностьЗакупок());
		ТоварыСАналитиками = Запрос.Выполнить().Выгрузить();
		ТоварыСАналитиками = ПланированиеПотребностей.ЗаменаСтатейИАналитикПоНастройкамЛимитов(ДокументОбъект.Дата,ТоварыСАналитиками);	
		...
		Для Каждого СтрокаТовары ИЗ ДанныеЗаполнения.Товары Цикл
			ДобТовары = ДокументОбъект.Товары.Добавить();
			ЗаполнитьЗначенияСвойств(ДобТовары,СтрокаТовары);
			Отбор = Новый Структура();
			Отбор.Вставить("Номенклатура",СтрокаТовары.Номенклатура);
			Отбор.Вставить("Характеристика",СтрокаТовары.Характеристика);
			Отбор.Вставить("СтавкаНДС",СтрокаТовары.СтавкаНДС);
			Отбор.Вставить("Назначение",СтрокаТовары.Назначение);
			Отбор.Вставить("ДатаПоступления",СтрокаТовары.ДатаПоступления);
			Отбор.Вставить("ОрганизацияПолучатель",СтрокаТовары.ОрганизацияПолучатель);
			Отбор.Вставить("МестоПоставки",СтрокаТовары.МестоПоставки);
			Отбор.Вставить("Приоритет",СтрокаТовары.Приоритет);
			Отбор.Вставить("НоменклатураИсходная",СтрокаТовары.НоменклатураИсходная);
			Отбор.Вставить("Цена",СтрокаТовары.Цена);
			СтрокиАналитик = ТоварыСАналитиками.НайтиСтроки(Отбор);
			Для Каждого Стр ИЗ СтрокиАналитик Цикл
				ДобТовары.АналитикаИсточника = Стр.АналитикаИсточника;
				ДобТовары.АналитикаПланирования = Стр.АналитикаПланирования;
				ДобТовары.ЦФО = Стр.ЦФО;
				ДобТовары.Проект = Стр.Проект;
				ВидыБюджетаЗакупок = ПланированиеПотребностей.ИспользуемыеВидыБюджетовЗакупок(ДокументОбъект.Дата);	
				Если ВидыБюджетаЗакупок = ПланыВидовХарактеристик.ВидыБюджетов.БюджетДоходовИРасходов Тогда 	
					ДобТовары.СтатьяБюджетаБДР= Стр.СтатьяБюджета;
					ДобТовары.АналитикаБДР1 	 = Стр.АналитикаБДР1;	
					ДобТовары.АналитикаБДР2 	 = Стр.АналитикаБДР2;	
					ДобТовары.АналитикаБДР3 	 = Стр.АналитикаБДР3;	
					ДобТовары.АналитикаБДР4 	 = Стр.АналитикаБДР4;	
					ДобТовары.АналитикаБДР5 	 = Стр.АналитикаБДР5;	
					ДобТовары.АналитикаБДР6 	 = Стр.АналитикаБДР6;
				ИначеЕсли ВидыБюджетаЗакупок = ПланыВидовХарактеристик.ВидыБюджетов.БюджетДвиженияДенежныхСредств Тогда 	
					ДобТовары.СтатьяБюджетаБДДС= Стр.СтатьяБюджета;
					ДобТовары.АналитикаБДДС1 	 = Стр.Аналитика1;	
					ДобТовары.АналитикаБДДС2 	 = Стр.Аналитика2;	
					ДобТовары.АналитикаБДДС3 	 = Стр.Аналитика3;	
					ДобТовары.АналитикаБДДС4 	 = Стр.Аналитика4;	
					ДобТовары.АналитикаБДДС5 	 = Стр.Аналитика5;	
					ДобТовары.АналитикаБДДС6 	 = Стр.Аналитика6;
				КонецЕсли;
				
				Прервать;
			КонецЦикла;	
			ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ДобТовары,СтруктураДействий,Неопределено);
...
//++ НЕ УТКА
		...
		|			ПО &ПодстановкаОсновногоСпособаОбеспечения
		|
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикЭтаповПроизводства2_2 КАК ДатыГрафика
		|			ПО Товары.ЗаказНаПроизводство = ДатыГрафика.ЗаказНаПроизводство
		|			 И ДатыГрафика.СтатусГрафика = 0
...
Функция ТаблицаЗаказыПоставщикуСогласноПланамПоставкиКОформлениюУХ()
	...
	ТекстЗапроса =
		...
		|	Т2.Договор.УдалитьСоглашение	КАК Соглашение,
