...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолуфабрикатыПроизводитьОбособленно = ПолучитьФункциональнуюОпцию("ПолуфабрикатыВнутриЗаказаПроизводитьОбособленно");
	
	Если ПолуфабрикатыПроизводитьОбособленно Тогда
		ОбособитьВыпускПолуфабрикатов = Истина; // полуфабрикаты внутри заказа всегда производятся обособленно
		ПараметрыСозданияДокументов.ОбособитьВыпускПолуфабрикатов = Истина;
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	СтруктураПоиска = Новый Структура;
	СтруктураПоиска.Вставить("НачатьНеРанее", '00010101');
	
	ДеревоИзделийКПредпросмотру = РеквизитФормыВЗначение("ИзделияКПредпросмотру");
	НайденныеСтроки = ДеревоИзделийКПредпросмотру.Строки.НайтиСтроки(СтруктураПоиска);
	Если НайденныеСтроки.Количество() > 0 Тогда
		ТекстСообщения = НСтр("ru = 'Не заполнена колонка ""Начать не ранее"" в строке списка ""Предварительный просмотр""';
								|en = 'The ""Earliest start date"" column is not populated in the ""Preview"" list line.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,, "ИзделияКПредпросмотру",, Отказ);
	КонецЕсли;
...
&НаКлиенте
Процедура ПараметрыСозданияДокументовГиперссылка(Элемент)

	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВыбораПараметровСозданияДокументовНаШагеОтборЗавершение", ЭтотОбъект); 
	
	ПараметрыОткрытияФормы = Новый Структура();
	ПараметрыОткрытияФормы.Вставить("ПараметрыСозданияДокументов", ПараметрыСозданияДокументов); 
	ПараметрыОткрытияФормы.Вставить("ПолуфабрикатыПроизводитьОбособленно", ПолуфабрикатыПроизводитьОбособленно);
	ПараметрыОткрытияФормы.Вставить("ИспользоватьАналоги", ИспользоватьАналоги);
	
	ОткрытьФорму(
		"Обработка.ФормированиеЭтаповПроизводства.Форма.ПараметрыСозданияДокументов",
...
&НаКлиенте
Процедура ПослеВыбораПараметровСозданияДокументовНаШагеОтборЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда 
		Возврат;
	КонецЕсли;

	ЗаполнитьЗначенияСвойств(ПараметрыСозданияДокументов, Результат);
	ОбособитьВыпускПолуфабрикатов = Результат.ОбособитьВыпускПолуфабрикатов;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыИзделияКПредпросмотру

&НаКлиенте
Процедура ИзделияКПредпросмотруНачатьНеРанееПриИзменении(Элемент)
	
	ТекущиеДанные = Элементы.ИзделияКПредпросмотру.ТекущиеДанные;
	ТекущиеДанные.СтрокаИзменена = Истина;
	ИзделияКПредпросмотруИзменены = Истина;
...
Процедура ПоказатьНастройки(Команда)
	...
	ОткрытьФорму(
		"Обработка.ФормированиеЭтаповПроизводства.Форма.НастройкиФормы",
...
&НаКлиенте
Процедура ЗаполнитьНачатьНеРанее(Команда)
	
	Если Элементы.ИзделияКПредпросмотру.ВыделенныеСтроки.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru = 'Необходимо выбрать строки, для которых требуется заполнить дату.';
									|en = 'Select the lines for which it is required to populate the date.'");
		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения(
		"ИзделияКПредпросмотру_ЗаполнитьНачатьНеРанееЗавершение",
		ЭтотОбъект,
		Элементы.ИзделияКПредпросмотру.ВыделенныеСтроки);
	
	ОбщегоНазначенияУТКлиент.ВвестиДатуСКонтролемПустогоЗначения(
		ОбщегоНазначенияКлиент.ДатаСеанса(),
		НСтр("ru = 'Введите дату начала';
			|en = 'Enter the start date'"),
		?(ИспользоватьИнтервалПланированияЧас, ЧастиДаты.ДатаВремя, ЧастиДаты.Дата),
		Оповещение);
	
КонецПроцедуры

&НаКлиенте
Процедура ИзделияКПредпросмотру_ЗаполнитьНачатьНеРанееЗавершение(ВыбраннаяДата, МассивВыделенныхСтрок) Экспорт
	
	Если ЗначениеЗаполнено(ВыбраннаяДата) Тогда
		Для Каждого ИдентификаторСтроки Из МассивВыделенныхСтрок Цикл
			СтрокаТаблицы = ИзделияКПредпросмотру.НайтиПоИдентификатору(ИдентификаторСтроки);
			СтрокаРодитель = ?(СтрокаТаблицы.ЭтоИтог, СтрокаТаблицы, СтрокаТаблицы.ПолучитьРодителя());
			СтрокаРодитель.НачатьНеРанее = ВыбраннаяДата;
			СтрокаРодитель.СтрокаИзменена = Истина;
		КонецЦикла;
		ИзделияКПредпросмотруИзменены = Истина;
	КонецЕсли; 
...
Функция ЗаполнитьПланКЗапуску(ДанныеСтроки, АдресПланХранилище, АдресПользовательскиеОтборы, ОбособитьВыпускПолуфабрикатов, ДетальныеЗаписиПакет = Неопределено)
	...
	Если Остаток > 0 Тогда
		...
		НоваяСтрока.ИдентификаторВГруппеИзделий = "";
		НоваяСтрока.ДатаЗапуска = '00010101';
		НоваяСтрока.ДатаВыпуска = '00010101';
		НоваяСтрока.ЖелаемаяДатаВыпуска = '00010101';
...
Функция ДетальныеЗаписиПакетно(ТаблицаОтбор, АдресПользовательскиеОтборы, ОбособитьВыпускПолуфабрикатов)
	...
	ТекстНоменклатурныйПлан = 
		...
		|	ЕСТЬNULL(График.Начало, ДАТАВРЕМЯ(1,1,1))          КАК ДатаЗапуска,
		|	ЕСТЬNULL(График.Окончание, ДАТАВРЕМЯ(1,1,1))       КАК ДатаВыпуска,
		|	ЕСТЬNULL(ГрафикПотребления.Начало, ДАТАВРЕМЯ(1,1,1)) КАК ЖелаемаяДатаВыпуска,
		...
		|	МИНИМУМ(План.ИдентификаторВГруппеИзделий)          КАК ИдентификаторВГруппеИзделий,
		...
		|	ПО План.ЗаказНаПроизводство = График.ЗаказНаПроизводство
		|		И График.СтатусГрафика = 0
		|		И План.КлючПартия = График.КлючПартия
		...
		|	
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикЭтаповПроизводства2_2 КАК ГрафикПотребления
		|	ПО Потребление.ЗаказНаПроизводство = ГрафикПотребления.ЗаказНаПроизводство
		|		И ГрафикПотребления.СтатусГрафика = 0
		|		И Потребление.КлючПартия = ГрафикПотребления.КлючПартия
		|		И Потребление.Этап = ГрафикПотребления.Этап
		|ГДЕ
		|	(
		|		План.Номенклатура,
		|		План.Характеристика,
		...
		|	ЕСТЬNULL(График.Окончание, ДАТАВРЕМЯ(1,1,1)),
		|	ЕСТЬNULL(ГрафикПотребления.Начало, ДАТАВРЕМЯ(1,1,1))
		|
		|ИМЕЮЩИЕ
	...
	// Подстановка соединений и отборов с дополнительными реквизитами
	Если ПользовательскиеОтборы <> Неопределено
		И ПользовательскиеОтборы.СоединенияСДопРеквизитами.Количество() > 0 Тогда
		СхемаЗапроса = Новый СхемаЗапроса;
		СхемаЗапроса.УстановитьТекстЗапроса(ТекстНоменклатурныйПлан);
		
		Оператор = СхемаЗапроса.ПакетЗапросов[0].Операторы[0]; // ОператорВыбратьСхемыЗапроса
		НовыеИсточники = Новый Соответствие;
		...
		Для каждого Источник Из Оператор.Источники Цикл
			...
			СоединенияСДопРеквизитами = ПользовательскиеОтборы.СоединенияСДопРеквизитами; // ТаблицаЗначений
			Для каждого Строка Из СоединенияСДопРеквизитами Цикл
				НовыйИсточник = НовыеИсточники.Получить(Строка.Псевдоним);
				Если НовыйИсточник = Неопределено Тогда
					НовыйИсточник = Оператор.Источники.Добавить(Строка.ИмяТаблицы, Строка.Псевдоним);
					НовыеИсточники.Вставить(Строка.Псевдоним, НовыйИсточник);
				КонецЕсли;
				НовоеСоединение = Источник.Соединения.Добавить(НовыйИсточник, Строка.Условие);
			КонецЦикла;
			
			Прервать;
		КонецЦикла;
		
		ТекстНоменклатурныйПлан = СхемаЗапроса.ПолучитьТекстЗапроса();
		
		ПараметрыСоединенийСДопРеквизитами = ПользовательскиеОтборы.ПараметрыСоединенийСДопРеквизитами; // Соответствие
		Для каждого КлючИЗначение Из ПараметрыСоединенийСДопРеквизитами Цикл
			Запрос.УстановитьПараметр(КлючИЗначение.Ключ, КлючИЗначение.Значение);
		КонецЦикла;
	КонецЕсли;
	
	// Подстановка простых отборов
	Если ПользовательскиеОтборы = Неопределено
		ИЛИ ПользовательскиеОтборы.Отборы.Количество() = 0 Тогда
		ТекстНоменклатурныйПлан = СтрЗаменить(ТекстНоменклатурныйПлан, "И &ПользовательскиеОтборы", "");
	Иначе
		ТекстОтборов = "";
		Для каждого Отбор Из ПользовательскиеОтборы.Отборы Цикл
			ТекстОтборов = ТекстОтборов + " И " + Отбор.Текст;
			Запрос.УстановитьПараметр(Отбор.Имя, Отбор.Значение);
		КонецЦикла;
		ТекстНоменклатурныйПлан = СтрЗаменить(ТекстНоменклатурныйПлан, "И &ПользовательскиеОтборы", ТекстОтборов);
	КонецЕсли;
	
	ТекстыЗапроса.Добавить(ТекстНоменклатурныйПлан);
	...
	ТекстыЗапроса.Добавить(
		...
		|				И &ОбособитьВыпускПолуфабрикатов
		|				И ПродукцияЗаказа.Ссылка ЕСТЬ NULL
		|			ТОГДА 
		...
		|					ВЫБОР
		|						КОГДА ПродукцияЗаказа.Ссылка ЕСТЬ НЕ NULL
		...
		|
		|	ПродукцияЗаказа.Ссылка ЕСТЬ НЕ NULL                              КАК ПродукцияЗаказа,
		|	ЕСТЬNULL(ПродукцияЗаказа.НомерСтроки, 10000000)                  КАК НомерСтрокиПродукция,
		|	ЕСТЬNULL(ПродукцияЗаказа.СписатьНаРасходы, ЛОЖЬ)                 КАК СписатьНаРасходы,
		|	ЕСТЬNULL(ПродукцияЗаказа.СтатьяРасходов, НЕОПРЕДЕЛЕНО)           КАК СтатьяРасходов,
		|	ЕСТЬNULL(ПродукцияЗаказа.АналитикаРасходов, НЕОПРЕДЕЛЕНО)        КАК АналитикаРасходов,
		|	ЕСТЬNULL(ПродукцияЗаказа.АналитикаАктивовПассивов, НЕОПРЕДЕЛЕНО) КАК АналитикаАктивовПассивов,
		|
		|	План.ДатаЗапуска                                КАК ДатаЗапуска,
		|	План.ДатаВыпуска                                КАК ДатаВыпуска,
		|
		|	ЕСТЬNULL(ПродукцияЗаказа.ДатаПотребности, План.ЖелаемаяДатаВыпуска) КАК ЖелаемаяДатаВыпуска,
		|
		|	План.КлючПартия                                 КАК КлючПартия,
		|	План.КлючНоменклатура                           КАК КлючНоменклатура,
		...
		|		ПО План.ЗаказНаПроизводство = ПродукцияЗаказа.Ссылка
		|			И План.Номенклатура = ПродукцияЗаказа.Номенклатура
		|			И План.КлючНоменклатура = ПродукцияЗаказа.КлючНоменклатура
		|		
		...
		|	План.ИдентификаторВГруппеИзделий,
		|	План.ДатаЗапуска,
		|	План.Требуется УБЫВ,
		|	План.КлючПартия,
		|	План.КлючНоменклатура");
	...
	ПоляИсключения = "КлючНоменклатура,Уровень,ДлительностьДоВыпуска,ИдентификаторВГруппеИзделий,КоличествоНаПартию,Требуется,КЗапуску,СвободныйВыпускИзОбособленныхМатериалов";
...
Процедура ЗаполнитьИзделияКПредпросмотру(Отказ)
	...
	Для Индекс = 0 По Данные.Изделия.Количество() - 1 Цикл
		...
		Если НомерГруппы <> СтрокаИзделия.НомерГруппы
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрокаИтог, СтрокаИзделия,
				"Номенклатура, Характеристика, НомерГруппы, НомерПартииВГруппе");
			НоваяСтрокаИтог.ЗаказНаПроизводство = СтрокаГруппы.Распоряжение;
			НоваяСтрокаИтог.НачатьНеРанее = СтрокаИзделия.ДатаЗапуска;
			НоваяСтрокаИтог.ДатаПотребности = СтрокаИзделия.ЖелаемаяДатаВыпуска;
	...
	ИзделияКПредпросмотруИзменены = Ложь;
...
Процедура СоздатьДокументыНаСервере(Отказ)
	...
	Если ЭтоАдресВременногоХранилища(АдресДанныеКФормированию) Тогда
		...
		Если ИзделияКПредпросмотруИзменены Тогда
			Группы = Данные.Группы; // ТаблицаЗначений
			Изделия = Данные.Изделия; // ТаблицаЗначений
			НомераГруппКИзменению = Новый Массив;
			
			Группы.Индексы.Добавить("НомерГруппы");
			Изделия.Индексы.Добавить("НомерГруппы");
			Изделия.Индексы.Добавить("НомерГруппы, НомерПартииВГруппе");
			
			СтруктураПоиска1 = Новый Структура("НомерГруппы");
			СтруктураПоиска2 = Новый Структура("НомерГруппы, НомерПартииВГруппе");
			
			ДеревоИзделийКПредпросмотру = РеквизитФормыВЗначение("ИзделияКПредпросмотру");
			Для каждого СтрокаПредпросмотр Из ДеревоИзделийКПредпросмотру.Строки.НайтиСтроки(Новый Структура("СтрокаИзменена", Истина)) Цикл
				ЗаполнитьЗначенияСвойств(СтруктураПоиска2, СтрокаПредпросмотр, "НомерГруппы, НомерПартииВГруппе");
				Для каждого СтрокаИзделие Из Изделия.НайтиСтроки(СтруктураПоиска2) Цикл
					СтрокаИзделие.ДатаЗапуска = СтрокаПредпросмотр.НачатьНеРанее;
					Если НомераГруппКИзменению.Найти(СтрокаИзделие.НомерГруппы) = Неопределено Тогда
						НомераГруппКИзменению.Добавить(СтрокаИзделие.НомерГруппы);
					КонецЕсли;
				КонецЦикла;
			КонецЦикла;
			
			Для каждого НомерГруппы Из НомераГруппКИзменению Цикл
				СтруктураПоиска1.НомерГруппы = НомерГруппы;
				СтрокиИзделия = Изделия.НайтиСтроки(СтруктураПоиска1);
				Если СтрокиИзделия.Количество() = 0 Тогда
					Продолжить;
				КонецЕсли;
				НачатьНеРанее = СтрокиИзделия[0].ДатаЗапуска;
				Для Индекс = 1 По СтрокиИзделия.ВГраница() Цикл
					НачатьНеРанее = Мин(НачатьНеРанее, СтрокиИзделия[Индекс].ДатаЗапуска);
				КонецЦикла;
				
				СтрокаГруппа = Группы.Найти(НомерГруппы, "НомерГруппы");
				СтрокаГруппа.НачатьНеРанее = НачатьНеРанее;
				Если СтрокаГруппа.НачатьНеРанее > СтрокаГруппа.ДатаПотребности Тогда
					СтрокаГруппа.ДатаПотребности = СтрокаГруппа.НачатьНеРанее;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	Иначе
	...
	ПараметрыПроцедуры.Вставить("КлючиПартий", Данные.КлючиПартий);
...
Функция ДанныеКФормированию()
	...
	Запрос.Текст = 
		...
		|	Изделия.Требуется                КАК КоличествоНорматив,
		|	Изделия.КЗапуску                 КАК Количество,
		|	Изделия.ПродукцияЗаказа          КАК ПродукцияЗаказа, 
		|	Изделия.КоличествоНаПартию       КАК КоличествоНаПартию, 
		|	Изделия.НомерСтрокиПродукция     КАК НомерСтрокиПродукция,
		...
		|	ВЫБОР
		|		КОГДА Изделия.ДатаЗапуска <> ДАТАВРЕМЯ(1, 1, 1)
		|			ТОГДА Изделия.ДатаЗапуска
		|		КОГДА Изделия.ЖелаемаяДатаВыпуска <> ДАТАВРЕМЯ(1, 1, 1)
		|			И Изделия.ЖелаемаяДатаВыпуска < &ТекущаяДата
		|			ТОГДА Изделия.ЖелаемаяДатаВыпуска
		|		ИНАЧЕ &ТекущаяДата
		|	КОНЕЦ                            КАК ДатаЗапуска,
		|	Изделия.ДатаЗапуска              КАК ДатаЗапускаНорматив,
		|	ВЫБОР
		|		КОГДА Изделия.ЖелаемаяДатаВыпуска <> ДАТАВРЕМЯ(1, 1, 1)
		|			ТОГДА Изделия.ЖелаемаяДатаВыпуска
		|		КОГДА Изделия.ДатаЗапуска <> ДАТАВРЕМЯ(1, 1, 1)
		|			И Изделия.ДатаЗапуска > &ТекущаяДата
		|			ТОГДА Изделия.ДатаЗапуска
		|		ИНАЧЕ &ТекущаяДата
		|	КОНЕЦ                               КАК ЖелаемаяДатаВыпуска,
		|	Изделия.Уровень                     КАК Уровень,
		|	Изделия.ДлительностьДоВыпуска       КАК ДлительностьДоВыпуска,
		|	Изделия.ИдентификаторВГруппеИзделий КАК ИдентификаторВГруппеИзделий,
	...
	РассчитатьГрафикОтТекущейДаты = ПараметрыСозданияДокументов.ВариантЗаполненияГрафикаЭтапов = "РассчитатьОтТекущейДаты";
	
	Если НЕ ИспользуетсяГрафикПроизводства
		И РассчитатьГрафикОтТекущейДаты Тогда
		ТекстЗапроса =
			"ВЫБРАТЬ
			|	ВЫБОР
			...
			|	ПО СледующийДеньВГрафике.Календарь = &ОсновнойКалендарь
			...
			|ГДЕ
			|	ТекущаяДата.Календарь = &ОсновнойКалендарь
			...
			|	ПО ПоследняяДатаГода.Календарь = &ОсновнойКалендарь
			...
			|		И ВыпускВТекущемГоду.Календарь = &ОсновнойКалендарь
			...
			|		И ВыпускВСледующемГоду.Календарь = &ОсновнойКалендарь
			...
			|ГДЕ
			|	Т.Спецификация В(ВЫБРАТЬ ДД.Спецификация ИЗ Группы КАК ДД)
	...
	Иначе
		ТекстЗапроса =
			...
			|	ДАТАВРЕМЯ(1,1,1)                                        КАК ДатаЗапуска,
	...
	Если ИспользуетсяГрафикПроизводства Тогда
		УправлениеПроизводством.СоздатьВТОграниченияГрафикаПроизводстваПоМатериаламДинамическаяСтруктура(
			...
			"&ТекстИмяТаблицы.КлючПартия В (ВЫБРАТЬ Т.КлючПартия ИЗ Изделия КАК Т)",
			Ложь,
			Ложь);
	Иначе
		ТекстЗапроса = 
	...
	ТекстЗапроса =
		...
		|	ВЫБОР
		|		КОГДА НЕ &ИспользуетсяГрафикПроизводства
		|			ТОГДА ISNULL(НормативныеДатыЗапускаВыпускаОтТекущейДаты.ДатаЗапуска, Изделия.ДатаЗапуска)
		|		КОГДА Изделия.ДатаЗапуска <= &ТекущаяДатаОкончаниеДня
		|				ИЛИ &РассчитатьГрафикОтТекущейДаты
		|			ТОГДА &ТекущаяДата
		|		ИНАЧЕ Изделия.ДатаЗапуска
		|	КОНЕЦ        КАК ДатаЗапуска,
		|	ISNULL(НормативныеДатыЗапускаВыпускаОтТекущейДаты.ДатаВыпуска, Изделия.ЖелаемаяДатаВыпуска) КАК ЖелаемаяДатаВыпуска
		|ПОМЕСТИТЬ ДатыИзделий
		|ИЗ
		|	Изделия КАК Изделия
		|	
		|	ЛЕВОЕ СОЕДИНЕНИЕ НормативныеДатыЗапускаВыпускаОтТекущейДаты КАК НормативныеДатыЗапускаВыпускаОтТекущейДаты
		|	ПО Изделия.Спецификация = НормативныеДатыЗапускаВыпускаОтТекущейДаты.Спецификация
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Ключ
		|;
		...
		|	
		|	МИНИМУМ(ДатыИзделий.ДатаЗапуска)                       КАК ДатаЗапуска,
		|	МИНИМУМ(ДатыИзделий.ЖелаемаяДатаВыпуска)               КАК ЖелаемаяДатаВыпуска,
		|	МАКСИМУМ(Изделия.ДлительностьДоВыпуска)                КАК ДлительностьДоВыпуска,
		|	МИНИМУМ(Изделия.ИдентификаторВГруппеИзделий)           КАК ИдентификаторВГруппеИзделий
		|ПОМЕСТИТЬ ПараметрыЗапуска
		|ИЗ
		|	Изделия КАК Изделия
		|	
		|	ЛЕВОЕ СОЕДИНЕНИЕ ДатыИзделий КАК ДатыИзделий
		|	ПО Изделия.Ключ = ДатыИзделий.Ключ
		|	
		|ГДЕ
		...
		|	ЕСТЬNULL(ПараметрыЗапуска.ДатаЗапуска, &ТекущаяДата)                КАК НачатьНеРанее,
		|	ЕСТЬNULL(ПараметрыЗапуска.ЖелаемаяДатаВыпуска, &ТекущаяДата)        КАК ДатаПотребности,
		|	ЕСТЬNULL(ПараметрыЗапуска.ДлительностьДоВыпуска, 0)                 КАК ДлительностьДоВыпуска,
		|	ЕСТЬNULL(ПараметрыЗапуска.ИдентификаторВГруппеИзделий, """")        КАК ИдентификаторВГруппеИзделий
		|ИЗ
		|	Группы КАК Группы
		|
		...
		|////////////////////////////////////////////////////////////////////////////////
		|
		|// Состав полей запроса тесно связан с процедурой деления на оптимальные партии
		|
		|ВЫБРАТЬ
		|	Изделия.НомерГруппы              КАК НомерГруппы,
		...
		|		ЗНАЧЕНИЕ(Документ.ЗаказНаПроизводство2_2.ПустаяСсылка)) КАК Распоряжение,
		|	
		|	СУММА(Изделия.КоличествоНорматив) КАК КоличествоНорматив,
		|	
		|	ЕСТЬNULL(ПараметрыЗапуска.ДатаЗапуска, &ТекущаяДата)   КАК ДатаЗапуска,
		|	МИНИМУМ(Изделия.ДатаЗапускаНорматив)                   КАК ДатаЗапускаНорматив,
		|	ВЫБОР
		|		КОГДА ГруппыКДелениюНаОптимальныеПартии.НомерГруппы ЕСТЬ НЕ NULL
		|			ТОГДА ДатыИзделий.ДатаЗапуска
		|		ИНАЧЕ ДАТАВРЕМЯ(1,1,1)
		|	КОНЕЦ                                                  КАК ДатаЗапускаГруппировка,
		|	
		|	ЕСТЬNULL(ПараметрыЗапуска.ЖелаемаяДатаВыпуска, &ТекущаяДата) КАК ЖелаемаяДатаВыпуска,
		|	ВЫБОР
		|		КОГДА ГруппыКДелениюНаОптимальныеПартии.НомерГруппы ЕСТЬ НЕ NULL
		|			ТОГДА ДатыИзделий.ЖелаемаяДатаВыпуска
		|		ИНАЧЕ ДАТАВРЕМЯ(1,1,1)
		|	КОНЕЦ                                                  КАК ЖелаемаяДатаВыпускаГруппировка,
		|	
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ(Изделия.КлючПартия))              КАК КоличествоПартий,
		|	ВЫБОР
		|		КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ(Изделия.КлючПартия)) = 1
		|			ТОГДА МИНИМУМ(Изделия.КлючПартия)
		|		ИНАЧЕ &ПустойКлючСвязи
		|	КОНЕЦ                                                  КАК КлючПартия,
		|	ВЫБОР
		|		КОГДА ГруппыКДелениюНаОптимальныеПартии.НомерГруппы ЕСТЬ НЕ NULL
		|			ТОГДА Изделия.КлючПартия
		|		ИНАЧЕ &ПустойКлючСвязи
		|	КОНЕЦ                                                  КАК КлючПартияГруппировка,
		|	
		...
		|	
		|	ЕСТЬNULL(ОчередьГрупп.Порядок, 1000000)            КАК Порядок1,
		|	МИНИМУМ(ЕСТЬNULL(ОчередьИзделий.Порядок, 1000000)) КАК Порядок2
		|ИЗ
		|	Изделия КАК Изделия
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ПараметрыЗапуска КАК ПараметрыЗапуска
		...
		|	ПО Изделия.Ключ = ДатыИзделий.Ключ
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ГруппыКДелениюНаОптимальныеПартии КАК ГруппыКДелениюНаОптимальныеПартии
		|	ПО Изделия.НомерГруппы = ГруппыКДелениюНаОптимальныеПартии.НомерГруппы
		|
		|СГРУППИРОВАТЬ ПО
		|	Изделия.НомерГруппы,
		...
		|	ЕСТЬNULL(ПараметрыЗапуска.ДатаЗапуска, &ТекущаяДата),
		|	ЕСТЬNULL(ПараметрыЗапуска.ЖелаемаяДатаВыпуска, &ТекущаяДата),
		|	ЕСТЬNULL(ОчередьГрупп.Порядок, 1000000),
		|	ВЫБОР
		|		КОГДА ГруппыКДелениюНаОптимальныеПартии.НомерГруппы ЕСТЬ НЕ NULL
		|			ТОГДА ДатыИзделий.ДатаЗапуска
	...
	Запрос.УстановитьПараметр("ТекущаяДатаОкончаниеДня", КонецДня(ТекущаяДата));
	...
	Запрос.УстановитьПараметр("РассчитатьГрафикОтТекущейДаты", РассчитатьГрафикОтТекущейДаты);
	Запрос.УстановитьПараметр("ОсновнойКалендарь", Константы.ОсновнойКалендарьПредприятия.Получить());
	Запрос.УстановитьПараметр("ДатаОтсчета", НачалоДня(ТекущаяДата));
	Запрос.УстановитьПараметр("ТекущийГод", Год(ТекущаяДата));
	Запрос.УстановитьПараметр("СледующийГод", Год(ТекущаяДата)+1);
	Запрос.УстановитьПараметр("ПоследнийДеньГода", НачалоДня(КонецГода(ТекущаяДата)));
	...
	Группы = РезультатЗапроса[КоличествоПакетов-5].Выгрузить();
	Изделия = РезультатЗапроса[КоличествоПакетов-4].Выгрузить();
	ПараметрыОптимальныхПартий = РезультатЗапроса[КоличествоПакетов-3].Выгрузить();
	КлючиПартий = РезультатЗапроса[КоличествоПакетов-2].Выгрузить();
	...
	Результат = Новый Структура;
	Результат.Вставить("Группы", Группы);
	Результат.Вставить("Изделия", Изделия);
	Результат.Вставить("КлючиПартий", КлючиПартий);
	
	Возврат Результат;
	
КонецФункции

// Разбить на оптимальные партии.
// 
// Параметры:
//  Изделия - ТаблицаЗначений
//  ПараметрыОптимальныхПартий - ТаблицаЗначений
//  ДанныеУпаковок - ТаблицаЗначений
//
&НаСервере
Процедура РазбитьНаОптимальныеПартии(Изделия, ПараметрыОптимальныхПартий, ДанныеУпаковок)
	...
	СтруктураПоискаНомерГруппы = Новый Структура("НомерГруппы");
	СтруктураПоискаНоменклатура = Новый Структура("Номенклатура, Характеристика");
	
	ДанныеСпецификации = Новый Структура;
	ДанныеСпецификации.Вставить("ОптимальнаяПартияВыпуска");
	ДанныеСпецификации.Вставить("МинимальнаяПартияВыпуска");
	ДанныеСпецификации.Вставить("ДопустимоеПревышениеОптимальнойПартииВыпуска");
	ДанныеСпецификации.Вставить("ВыпускПроизвольнымиПорциями");
	ДанныеСпецификации.Вставить("ОсновноеИзделие");
	
	ИзделияПартии = Новый Массив;
	ПустойКлючПартия = Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000");
	
	ДатаЗапускаПартии = Неопределено;
	ЖелаемаяДатаВыпускаПартии = Неопределено;
	
	ДатаЗапускаНормативПартии = Неопределено;
	Порядок2Партии = Неопределено;
	УровеньПартии = Неопределено;
	ДлительностьДоВыпускаПартии = Неопределено;
	ИдентификаторВГруппеИзделийПартии = Неопределено;
	
	Для каждого Группа Из ПараметрыОптимальныхПартий Цикл
		
		// Имитация структур данных спецификации
		ДанныеСпецификации.ОптимальнаяПартияВыпуска = Группа.ОптимальнаяПартияВыпуска;
		ДанныеСпецификации.МинимальнаяПартияВыпуска = Группа.МинимальнаяПартияВыпуска;
		ДанныеСпецификации.ДопустимоеПревышениеОптимальнойПартииВыпуска = Группа.ДопустимоеПревышениеОптимальнойПартииВыпуска;
		ДанныеСпецификации.ВыпускПроизвольнымиПорциями = Группа.ВыпускПроизвольнымиПорциями;
		...
		Если Не Группа.ЭтоОсновноеИзделие Тогда
			...
			УправлениеДаннымиОбИзделиях.ЗаполнитьКоэффициентПересчетаВОсновноеИзделие(
				ОсновноеИзделие, ОсновноеИзделиеСпецификации);
		КонецЕсли;
		ДанныеСпецификации.ОсновноеИзделие = ОсновноеИзделие;
		...
		СтруктураПоискаНомерГруппы.НомерГруппы = Группа.НомерГруппы;
		ИзделияГруппы = Изделия.НайтиСтроки(СтруктураПоискаНомерГруппы);
		...
		Для Индекс = 0 По Партии.ВГраница() Цикл
			...
			Для каждого Изделие Из ИзделияГруппы Цикл
				...
				ЗаполнитьЗначенияСвойств(СтруктураПоискаНоменклатура, Изделие);
				СтрокаОстаток = ИзделияОстатки.НайтиСтроки(СтруктураПоискаНоменклатура)[0];
				...
				ИзделияПартии.Добавить(НоваяСтрока);
				
				Если ДатаЗапускаПартии = Неопределено Тогда
					ДатаЗапускаПартии = НоваяСтрока.ДатаЗапускаГруппировка;
					ЖелаемаяДатаВыпускаПартии = НоваяСтрока.ЖелаемаяДатаВыпускаГруппировка;
					
					ДатаЗапускаНормативПартии = НоваяСтрока.ДатаЗапускаНорматив;
					Порядок2Партии = НоваяСтрока.Порядок2;
					УровеньПартии = НоваяСтрока.Уровень;
					ДлительностьДоВыпускаПартии = НоваяСтрока.ДлительностьДоВыпуска;
					ИдентификаторВГруппеИзделийПартии = НоваяСтрока.ИдентификаторВГруппеИзделий;
				Иначе
					ДатаЗапускаПартии = Мин(ДатаЗапускаПартии, НоваяСтрока.ДатаЗапускаГруппировка);
					ЖелаемаяДатаВыпускаПартии = Мин(ЖелаемаяДатаВыпускаПартии, НоваяСтрока.ЖелаемаяДатаВыпускаГруппировка);
					
					ДатаЗапускаНормативПартии = Мин(ДатаЗапускаНормативПартии, НоваяСтрока.ДатаЗапускаНорматив);
					Порядок2Партии = Мин(Порядок2Партии, НоваяСтрока.Порядок2);
					УровеньПартии = Макс(УровеньПартии, НоваяСтрока.Уровень);
					ДлительностьДоВыпускаПартии = Макс(ДлительностьДоВыпускаПартии, НоваяСтрока.ДлительностьДоВыпуска);
					ИдентификаторВГруппеИзделийПартии = Макс(ИдентификаторВГруппеИзделийПартии, НоваяСтрока.ИдентификаторВГруппеИзделий);
				КонецЕсли;
			КонецЦикла;
			
			Для каждого Изделие Из ИзделияПартии Цикл
				Изделие.ДатаЗапуска = ДатаЗапускаПартии;
				Изделие.ДатаЗапускаГруппировка = '00010101';
				Изделие.ЖелаемаяДатаВыпуска = ЖелаемаяДатаВыпускаПартии;
				Изделие.ЖелаемаяДатаВыпускаГруппировка = '00010101';
				
				Изделие.ДатаЗапускаНорматив = ДатаЗапускаНормативПартии;
				Изделие.Порядок2 = Порядок2Партии;
				Изделие.Уровень = УровеньПартии;
				Изделие.ДлительностьДоВыпуска = ДлительностьДоВыпускаПартии;
				Изделие.ИдентификаторВГруппеИзделий = ИдентификаторВГруппеИзделийПартии;
				
				Изделие.КлючПартия = ?(ИзделияПартии.Количество() = 1, Изделие.КлючПартия, ПустойКлючПартия);
				Изделие.КлючПартияГруппировка = ПустойКлючПартия;
			КонецЦикла;
			
			ИзделияПартии.Очистить();
	...
	Для каждого Колонка Из Изделия.Колонки Цикл
		Если Колонка.Имя <> "КоличествоНорматив"
			И Колонка.Имя <> "Количество"
			И Колонка.Имя <> "КоличествоПартий" Тогда
			ИменаКолонок.Добавить(Колонка.Имя);
		КонецЕсли;
	КонецЦикла;
	КолонкиГруппировок = СтрСоединить(ИменаКолонок, ",");
	
	Изделия.Свернуть(КолонкиГруппировок, "КоличествоНорматив, Количество, КоличествоПартий");
	Изделия.Сортировать("Порядок1, Порядок2, НомерГруппы");
...
Функция ОтразитьОтборКомпоновкиВПростыхОтборах()
	...
	Для каждого ЭлементОтбора Из КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		...
		Если ЭлементПользовательскойНастройки <> Неопределено
			...
			ЗаполнитьПростойОтбор(ОтборПоОтветственномуПодразделению,
...
#Область ПланИтоги
	...
	Элемент.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.ПланИтогиДатаЗапускаМинимум.Имя);
	Элемент.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.ПланИтогиДатаЗапускаМинимумПоГруппе.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ПланИтоги.ДатаЗапускаМинимумПоГруппе");
	...
	Элемент.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.ПланИтогиДатаВыпускаМинимум.Имя);
	Элемент.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.ПланИтогиДатаВыпускаМинимумПоГруппе.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ПланИтоги.ДатаВыпускаМинимумПоГруппе");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
...
#Область План
	...
	Элемент.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.ПланДатаЗапуска.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.План.ДатаЗапуска");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	...
	Элемент.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.ПланДатаВыпуска.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.План.ДатаВыпуска");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	...
	Элемент.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.ПланЖелаемаяДатаВыпуска.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.План.ЖелаемаяДатаВыпуска");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
...
#Область ПредварительныйПросмотр
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ИзделияКПредпросмотруНачатьНеРанее.Имя);
...
Процедура НастроитьЭлементыФормы()
	...
	Элементы.СтруктураЗаказа.Видимость = ПравоДоступа("Просмотр", Метаданные.Отчеты.СтруктураЗаказаНаПроизводствоДинамическая);

	ПараметрыВыбораСпецификаций = УправлениеДаннымиОбИзделиях.ПараметрыВыбораСпецификаций(Объект, Обработки.ФормированиеЭтаповПроизводства);
	УправлениеДаннымиОбИзделияхКлиентСервер.УстановитьПараметрыВыбораСпецификаций(Элементы.ПланИтогиСпецификация, ПараметрыВыбораСпецификаций); 
	
	ФорматДатыГрафика = УправлениеПроизводством.ФорматнаяСтрокаДляДатыГрафикаПроизводства();
	Элементы.ИзделияКПредпросмотруНачатьНеРанее.Формат = ФорматДатыГрафика;
	Элементы.ИзделияКПредпросмотруНачатьНеРанее.ФорматРедактирования = ФорматДатыГрафика;
	Элементы.ИзделияКПредпросмотруДатаПотребности.Формат = ФорматДатыГрафика;
	
	Если ИспользуетсяГрафикПроизводства Тогда
		Элементы.ИзделияКПредпросмотруНачатьНеРанее.ТолькоПросмотр = Ложь;
		Элементы.ИзделияКПредпросмотруНачатьНеРанее.КартинкаШапки = БиблиотекаКартинок.Изменить;
		Элементы.ЗаполнитьНачатьНеРанее.Видимость = Истина;
	Иначе
		Элементы.ИзделияКПредпросмотруНачатьНеРанее.ТолькоПросмотр = Истина;
		Элементы.ИзделияКПредпросмотруНачатьНеРанее.КартинкаШапки = Новый Картинка();
		Элементы.ЗаполнитьНачатьНеРанее.Видимость = Ложь;
	КонецЕсли;
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("ПараметрыОтображения")
		...
		Элементы.ПланИтогиДатаВыпускаМинимум.Видимость = Не Форма.ПараметрыОтображения.КроссТаблица;
		Элементы.ПланИтогиДатаЗапускаМинимумПоГруппе.Видимость = Форма.ПараметрыОтображения.КроссТаблица;
		Элементы.ПланИтогиДатаВыпускаМинимумПоГруппе.Видимость = Форма.ПараметрыОтображения.КроссТаблица;
...
Процедура ВосстановитьНастройки()
	...
	Иначе
		...
		ПараметрыПоУмолчанию = Обработки.ФормированиеЭтаповПроизводства.ПараметрыСозданияДокументовПоУмолчанию();
	...
	ОбособитьВыпускПолуфабрикатов = ПараметрыСозданияДокументов.ОбособитьВыпускПолуфабрикатов;
	...
	Если ЗначениеНастроек = Неопределено Или Не ОбщегоНазначенияУТКлиентСервер.СтруктураСодержитКлючи(ЗначениеНастроек, СписокПараметровВывода(), Истина) Тогда
