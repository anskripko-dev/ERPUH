...
Процедура НастроитьФорму()
	...
	ВсеВыполнены = Истина;
	Для каждого ТекущаяОперация Из Объект.ТаблицаОпераций Цикл
		
		Если ТекущаяОперация.Требуется
			И Не ТекущаяОперация.Справочно
			И ТекущаяОперация.Состояние = Перечисления.ВидыСостоянийРегламентныхОпераций.НеВыполнено Тогда
			ВсеВыполнены = Ложь;
			Прервать;
		ИначеЕсли ТекущаяОперация.Требуется
			И ТекущаяОперация.Состояние = Перечисления.ВидыСостоянийРегламентныхОпераций.ВыполненоСОшибками Тогда
			ВсеВыполнены = Ложь;
			Прервать;
		КонецЕсли;
	...
	ИначеЕсли МожноЗакрыватьМесяц И ВсеВыполнены Тогда
...
Процедура УстановитьМожноЗакрыватьМесяц()
	...
	УчетнаяПолитикаСуществует = ЗначениеЗаполнено(КэшируемыеЗначения.УчетнаяПолитика);
	МожноЗакрыватьМесяц = УчетнаяПолитикаСуществует;
	
	Если Не УчетнаяПолитикаСуществует Тогда
		Возврат;
	ИначеЕсли КэшируемыеЗначения.ФормироватьПроводкиМСФО Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеУОП = Документы.НастраиваемыйОтчет.ПолучитьДанныеПериода(
		Объект.Сценарий, Объект.ПериодРегистрации, Объект.Организация);
		
	УправлениеПериодом = ДанныеУОП.УправлениеПериодом;
	ПериодЗакрыт = ЗначениеЗаполнено(ДанныеУОП.УправлениеПериодом)
		И ДанныеУОП.УправлениеПериодом.ПериодЗакрыт;
	...
	Если Не ЗначениеЗаполнено(АОСВ) Тогда 
		МожноЗакрыватьМесяц = Ложь;
	КонецЕсли;
	
	МожноЗакрыватьМесяц = МожноЗакрыватьМесяц 
		И ЗначениеЗаполнено(УправлениеПериодом)
		И НЕ ПериодЗакрыт;
